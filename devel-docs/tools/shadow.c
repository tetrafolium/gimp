...
static GdkPixbuf *create_shadow(GdkPixbuf *src) {
  int x, y, i, j;
  int width, height;
  GdkPixbuf *dest;
  ...
  dest = gdk_pixbuf_new(gdk_pixbuf_get_colorspace(src),
                        ...
                        gdk_pixbuf_get_bits_per_sample(src), width, height);
    ...
    for (x = 0; x < width; x++) {
      ...
      for (i = 0; i < filter->size; i++) {
        for (j = 0; j < filter->size; j++) {
          ...
          if (src_y < 0 || src_y >= gdk_pixbuf_get_height(src) || src_x < 0 ||
              src_x >= gdk_pixbuf_get_width(src))
            continue;

          sumr += src_pixels[src_y * src_rowstride + src_x * src_bpp + 0] *
                  filter->data[i * filter->size + j];
          sumg += src_pixels[src_y * src_rowstride + src_x * src_bpp + 1] *
                  filter->data[i * filter->size + j];

          sumb += src_pixels[src_y * src_rowstride + src_x * src_bpp + 2] *
                  filter->data[i * filter->size + j];

          if (src_bpp == 4)
            suma += src_pixels[src_y * src_rowstride + src_x * src_bpp + 3] *
                    filter->data[i * filter->size + j];
        }
      }

      if (dest_bpp == 4)
        dest_pixels[y * dest_rowstride + x * dest_bpp + 3] =
            suma * SHADOW_OPACITY;
    }
  }
...
GdkPixbuf *create_shadowed_pixbuf(GdkPixbuf *src) {
  ...
  gdk_pixbuf_composite(src, dest, BLUR_RADIUS, BLUR_RADIUS,
                       ...
                       BLUR_RADIUS, BLUR_RADIUS, 1.0, 1.0, GDK_INTERP_NEAREST,
                       255);
  return dest;
}
