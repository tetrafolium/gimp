...
static GdkPixbuf *add_border_to_shot(GdkPixbuf *pixbuf) {
  GdkPixbuf *retval;
  ...
  gdk_pixbuf_copy_area(pixbuf, 0, 0, gdk_pixbuf_get_width(pixbuf),
                       gdk_pixbuf_get_height(pixbuf), retval, 1, 1);

  return retval;
}

static GdkPixbuf *take_window_shot(GtkWidget *widget) {
  ...
  pixbuf = gdk_pixbuf_get_from_surface(surface, 0, 0, allocation.width,
                                       allocation.height);
...
static gboolean shooter_get_foreground(GimpRGB *color) {
...
static gboolean shooter_get_background(GimpRGB *color) {
...
static void shooter_standard_help(const gchar *help_id, gpointer help_data) {}

static void shooter_ensure_modules(void) {
...
int main(int argc, char **argv) {
  ...
  gimp_widgets_init(shooter_standard_help, shooter_get_foreground,
                    shooter_get_background, shooter_ensure_modules);
