...
enum { OFFSETS_CHANGED, LAST_SIGNAL };
...
struct _PrintPreviewClass {
  ...
  void (*offsets_changed)(PrintPreview *print_preview, gint offset_x,
                          gint offset_y);
};
...
static gboolean print_preview_draw(GtkWidget *widget, cairo_t *cr);
...
static gboolean print_preview_is_inside(PrintPreview *preview, gdouble x,
                                        gdouble y);
static void print_preview_set_inside(PrintPreview *preview, gboolean inside);
...
static void marshal_VOID__DOUBLE_DOUBLE(GClosure *closure, GValue *return_value,
...
static void print_preview_class_init(PrintPreviewClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

  print_preview_signals[OFFSETS_CHANGED] = g_signal_new(
      "offsets-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(PrintPreviewClass, offsets_changed), NULL, NULL,
      marshal_VOID__DOUBLE_DOUBLE, G_TYPE_NONE, 2, G_TYPE_DOUBLE,
      G_TYPE_DOUBLE);
...
static void print_preview_init(PrintPreview *preview) {
...
static void print_preview_realize(GtkWidget *widget) {
  ...
  preview->cursor =
      gdk_cursor_new_for_display(gtk_widget_get_display(widget), GDK_HAND1);
}
...
static void print_preview_size_allocate(GtkWidget *widget,
  ...
  GTK_WIDGET_CLASS(print_preview_parent_class)
...
static gboolean print_preview_button_press_event(GtkWidget *widget,
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 1 &&
    ...
    if (gdk_seat_grab(seat, gdk_event_get_window((GdkEvent *)event),
                      ...
                      (GdkEvent *)event, NULL, NULL) == GDK_GRAB_SUCCESS) {
...
static gboolean print_preview_button_release_event(GtkWidget *widget,
  ...
  if (preview->dragging) {
    ...
    print_preview_set_inside(
        preview, print_preview_is_inside(preview, event->x, event->y));
  }
...
static gboolean print_preview_motion_notify_event(GtkWidget *widget,
  ...
  if (preview->dragging) {
    ...
    offset_x = (preview->orig_offset_x + (event->x - preview->start_x) / scale);
    offset_y = (preview->orig_offset_y + (event->y - preview->start_y) / scale);
    ...
    if (preview->image_offset_x != offset_x ||
      ...
      g_signal_emit(preview, print_preview_signals[OFFSETS_CHANGED], 0,
...
static gboolean print_preview_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  print_preview_get_page_size(preview, &paper_width, &paper_height);
  print_preview_get_page_margins(preview, &left_margin, &right_margin,
  ...
  /* draw page_margins */
  cairo_rectangle(cr, scale * left_margin, scale * top_margin,
  ...
  cairo_translate(cr, scale * (left_margin + preview->image_offset_x),
                  scale * (top_margin + preview->image_offset_y));

  if (preview->dragging || preview->inside) {
    cairo_rectangle(cr, 0, 0, scale * preview->image_width,
  ...
  if (preview->thumbnail == NULL &&
    ...
    preview->thumbnail = print_preview_get_thumbnail(
        preview->drawable, MIN(allocation.width, 1024),
 ...
 **/
GtkWidget *print_preview_new(GtkPageSetup *page, GimpDrawable *drawable) {
 ...
 * #PrintPreview.
 **/
void print_preview_set_image_dpi(PrintPreview *preview, gdouble xres,
                                 gdouble yres) {
  gdouble width;
  gdouble height;
 ...
 **/
void print_preview_set_page_setup(PrintPreview *preview, GtkPageSetup *page) {
  g_return_if_fail(PRINT_IS_PREVIEW(preview));
 ...
 **/
void print_preview_set_image_offsets(PrintPreview *preview, gdouble offset_x,
                                     gdouble offset_y) {
  g_return_if_fail(PRINT_IS_PREVIEW(preview));
...
static void print_preview_get_page_size(PrintPreview *preview,
                                        gdouble *paper_width,
                                        gdouble *paper_height) {
  *paper_width = gtk_page_setup_get_paper_width(preview->page, GTK_UNIT_POINTS);
  *paper_height =
...
static void print_preview_get_page_margins(PrintPreview *preview,
  ...
  if (preview->use_full_page) {
    ...
    *left_margin =
        gtk_page_setup_get_left_margin(preview->page, GTK_UNIT_POINTS);
    *right_margin =
        gtk_page_setup_get_right_margin(preview->page, GTK_UNIT_POINTS);
    *top_margin = gtk_page_setup_get_top_margin(preview->page, GTK_UNIT_POINTS);
    *bottom_margin =
        gtk_page_setup_get_bottom_margin(preview->page, GTK_UNIT_POINTS);
  }
}
