...
static GimpValueArray *print_run(GimpProcedure *procedure, GimpRunMode run_mode,
                                 GimpImage *image, GimpDrawable *drawable,
                                 const GimpValueArray *args, gpointer run_data);

static GimpPDBStatusType print_image(GimpImage *image, gboolean interactive,
                                     GError **error);
#ifndef EMBED_PAGE_SETUP
...
static void end_print(GtkPrintOperation *operation, GtkPrintContext *context,
                      GimpLayer **layer);
static void draw_page(GtkPrintOperation *print, GtkPrintContext *context,
...
static void print_class_init(PrintClass *klass) {
...
static void print_init(Print *print) {}
...
static GimpProcedure *print_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PRINT_PROC_NAME)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, print_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure, N_("Print the image"),
                                     "Print the image using the "
                                     "GTK+ Print API.",
                                     name);
    gimp_procedure_set_attribution(
        procedure, "Bill Skaggs, Sven Neumann, Stefan Röllin",
        "Bill Skaggs <weskaggs@primate.ucdavis.edu>", "2006 - 2008");
    ...
    gimp_procedure_set_attribution(procedure,
                                   ...
                                   "Sven Neumann <sven@gimp.org>", "2008");
  }
#endif
...
static GimpValueArray *print_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  if (strcmp(gimp_procedure_get_name(procedure), PRINT_PROC_NAME) == 0) {
...
#ifndef EMBED_PAGE_SETUP
  else if (strcmp(gimp_procedure_get_name(procedure), PAGE_SETUP_PROC_NAME) ==
           0) {
...
static GimpPDBStatusType print_image(GimpImage *image, gboolean interactive,
  ...
  g_signal_connect(operation, "end-print", G_CALLBACK(end_print), &layer);
  ...
  if (interactive) {
    ...
    g_signal_connect_swapped(operation, "end-print",
                             G_CALLBACK(print_settings_save), &data);
    ...
    result = gtk_print_operation_run(
        operation, GTK_PRINT_OPERATION_ACTION_PRINT_DIALOG, NULL, error);
    ...
    result = gtk_print_operation_run(
        operation, GTK_PRINT_OPERATION_ACTION_PRINT, NULL, error);
  }
...
static GimpPDBStatusType page_setup(GimpImage *image) {
  ...
  return_vals = gimp_pdb_run_procedure(gimp_get_pdb(), name, GIMP_TYPE_IMAGE,
                                       image, G_TYPE_NONE);
...
#ifndef EMBED_PAGE_SETUP
static GimpValueArray *print_temp_proc_run(GimpProcedure *procedure,
                                           const GimpValueArray *args,
                                           gpointer run_data) {
...
static gchar *print_temp_proc_install(GimpImage *image) {
  ...
  procedure = gimp_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_TEMPORARY,
  ...
  GIMP_PROC_ARG_IMAGE(procedure, "image", "Image", "The image to notify about",
