...
static void print_settings_add_to_key_file(const gchar *key, const gchar *value,
                                           gpointer data);
...
void print_settings_save(PrintData *data) {
  ...
  if (gimp_image_is_valid(data->image)) {
    ...
    if ((data->min_xres <= xres && ABS(xres - data->xres) > 0.1) ||
      ...
      g_key_file_set_double(key_file, "image-setup", "x-resolution",
                            data->xres);
      g_key_file_set_double(key_file, "image-setup", "y-resolution",
                            data->yres);
    }
    ...
    g_key_file_set_integer(key_file, "image-setup", "center-mode",
                           data->center);
    g_key_file_set_boolean(key_file, "image-setup", "use-full-page",
                           data->use_full_page);
    g_key_file_set_boolean(key_file, "image-setup", "crop-marks",
                           data->draw_crop_marks);

    print_utils_key_file_save_as_parasite(key_file, data->image,
                                          PRINT_SETTINGS_NAME);
  }
...
static GKeyFile *print_settings_key_file_from_settings(PrintData *data) {
  ...
  if (settings)
    gtk_print_settings_foreach(settings, print_settings_add_to_key_file,
                               key_file);
...
static GKeyFile *print_settings_key_file_from_parasite(GimpImage *image) {
  ...
  key_file =
      print_utils_key_file_load_from_parasite(image, PRINT_SETTINGS_NAME);
...
static gboolean print_settings_load_from_key_file(PrintData *data,
  ...
  for (i = 0; i < n_keys; i++) {
    ...
    value = g_key_file_get_value(key_file, PRINT_SETTINGS_NAME, keys[i], NULL);
  ...
  if (g_key_file_has_key(key_file, "image-setup", "unit", NULL)) {
    data->unit = g_key_file_get_integer(key_file, "image-setup", "unit", NULL);
  }

  if (g_key_file_has_key(key_file, "image-setup", "x-resolution", NULL) &&
      g_key_file_has_key(key_file, "image-setup", "y-resolution", NULL)) {
    data->xres =
        g_key_file_get_double(key_file, "image-setup", "x-resolution", NULL);
    data->yres =
        g_key_file_get_double(key_file, "image-setup", "y-resolution", NULL);
  }

  if (g_key_file_has_key(key_file, "image-setup", "x-offset", NULL) &&
      g_key_file_has_key(key_file, "image-setup", "y-offset", NULL)) {
    data->offset_x =
        g_key_file_get_double(key_file, "image-setup", "x-offset", NULL);
    data->offset_y =
        g_key_file_get_double(key_file, "image-setup", "y-offset", NULL);
  }

  if (g_key_file_has_key(key_file, "image-setup", "center-mode", NULL)) {
    data->center =
        g_key_file_get_integer(key_file, "image-setup", "center-mode", NULL);
  }

  if (g_key_file_has_key(key_file, "image-setup", "use-full-page", NULL)) {
    data->use_full_page =
        g_key_file_get_boolean(key_file, "image-setup", "use-full-page", NULL);
  }

  if (g_key_file_has_key(key_file, "image-setup", "crop-marks", NULL)) {
    data->draw_crop_marks =
        g_key_file_get_boolean(key_file, "image-setup", "crop-marks", NULL);
  }
...
static gboolean print_settings_check_version(GKeyFile *key_file) {
  ...
  major_version =
      g_key_file_get_integer(key_file, "meta", "major-version", NULL);

  if (major_version != PRINT_SETTINGS_MAJOR_VERSION)
    return FALSE;

  minor_version =
      g_key_file_get_integer(key_file, "meta", "minor-version", NULL);
