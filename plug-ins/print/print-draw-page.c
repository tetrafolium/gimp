...
static void print_draw_crop_marks(GtkPrintContext *context, gdouble x,
...
gboolean print_draw_page(GtkPrintContext *context, PrintData *data,
  ...
  if (surface) {
    ...
    if (data->draw_crop_marks)
      print_draw_crop_marks(context, 0, 0, width * scale_x, height * scale_y);
...
static cairo_surface_t *print_surface_from_drawable(GimpDrawable *drawable,
  ...
  if (status != CAIRO_STATUS_SUCCESS) {
    ...
    case CAIRO_STATUS_INVALID_SIZE:
      g_set_error_literal(
          error, GIMP_PLUGIN_PRINT_ERROR, GIMP_PLUGIN_PRINT_ERROR_FAILED,
          _("Cannot handle the size (either width or height) of the image."));
      break;
    default:
      g_set_error(error, GIMP_PLUGIN_PRINT_ERROR,
                  GIMP_PLUGIN_PRINT_ERROR_FAILED, "Cairo error: %s",
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    guchar *dest =
        pixels + iter->items[0].roi.y * stride + iter->items[0].roi.x * 4;
    gint y;
