...
enum { BOTTOM, TOP, RIGHT, LEFT, WIDTH, HEIGHT };
...
static GtkWidget *print_size_frame(PrintData *data, GtkSizeGroup *label_group,
                                   GtkSizeGroup *entry_group);
static GtkWidget *print_offset_frame(PrintData *data, GtkSizeGroup *label_group,
                                     GtkSizeGroup *entry_group);
...
static void print_size_info_preview_offset_changed(GtkWidget *widget,
                                                   gdouble offset_x,
                                                   gdouble offset_y);
static void print_size_info_center_changed(GtkWidget *widget);
static void print_size_info_center_none(void);
static void print_size_info_use_full_page_toggled(GtkWidget *widget);

static void print_size_info_set_resolution(PrintSizeInfo *info, gdouble xres,
                                           gdouble yres);
...
GtkWidget *print_page_layout_gui(PrintData *data, const gchar *help) {
  GtkWidget *main_hbox;
  GtkWidget *main_vbox;
  GtkWidget *button;
  GtkWidget *frame;
  ...
  g_signal_connect(button, "toggled",
                   G_CALLBACK(print_size_info_use_full_page_toggled), NULL);
  ...
  gtk_box_pack_start(GTK_BOX(main_vbox), button, FALSE, FALSE, 0);
  g_signal_connect(button, "toggled", G_CALLBACK(print_draw_crop_marks_toggled),
                   NULL);
  gtk_widget_show(button);
...
static GtkWidget *print_size_frame(PrintData *data, GtkSizeGroup *label_group,
  ...
  image_width =
      (info.image_width * gimp_unit_get_factor(data->unit) / data->xres);
  image_height =
      (info.image_height * gimp_unit_get_factor(data->unit) / data->yres);
  ...
  entry = gimp_size_entry_new(1, data->unit, "%p", FALSE, FALSE, FALSE,
  ...
  gimp_size_entry_add_field(GIMP_SIZE_ENTRY(entry), GTK_SPIN_BUTTON(height),
                            NULL);
  ...
  gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(entry), _("_Width:"), 0, 0, 0.0);
  label = gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(entry), _("_Height:"), 1,
                                       0, 0.0);

  gtk_size_group_add_widget(label_group, label);

  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(entry), 0, data->xres, FALSE);
  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(entry), 1, data->yres, FALSE);
  ...
  entry =
      gimp_size_entry_new(1, data->image_unit, _("pixels/%a"), FALSE, FALSE,
                          FALSE, SB_WIDTH, GIMP_SIZE_ENTRY_UPDATE_RESOLUTION);
  gtk_box_pack_start(GTK_BOX(hbox), entry, FALSE, FALSE, 0);
  gtk_widget_show(entry);
  ...
  g_signal_connect_swapped(button, "clicked",
                           G_CALLBACK(print_resolution_load_defaults), &info);
  gtk_widget_show(button);
  gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 0);

  gimp_size_entry_set_refval_boundaries(
      GIMP_SIZE_ENTRY(entry), 0, GIMP_MIN_RESOLUTION, GIMP_MAX_RESOLUTION);
  gimp_size_entry_set_refval_boundaries(
      GIMP_SIZE_ENTRY(entry), 1, GIMP_MIN_RESOLUTION, GIMP_MAX_RESOLUTION);
  ...
  g_signal_connect(info.resolution_entry, "value-changed",
                   G_CALLBACK(print_size_info_resolution_changed), NULL);
  g_signal_connect(info.size_entry, "unit-changed",
                   G_CALLBACK(print_size_info_unit_changed), NULL);
...
static GtkWidget *print_offset_frame(PrintData *data, GtkSizeGroup *label_group,
  ...
  gimp_size_entry_add_field(GIMP_SIZE_ENTRY(entry), GTK_SPIN_BUTTON(spinner),
                            NULL);
  ...
  g_signal_connect(info.size_entry, "value-changed",
                   G_CALLBACK(print_size_info_size_changed), NULL);
  ...
  combo = gimp_int_combo_box_new(C_("center-mode", "None"), CENTER_NONE,
                                 ...
                                 _("Vertically"), CENTER_VERTICALLY, _("Both"),
                                 CENTER_BOTH, NULL);
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), data->center,
                             G_CALLBACK(print_size_info_center_changed), NULL,
                             NULL);

  info.center_combo = combo;

  g_signal_connect(info.left_adj, "value-changed",
                   G_CALLBACK(print_size_info_center_none), NULL);
  g_signal_connect(info.right_adj, "value-changed",
                   G_CALLBACK(print_size_info_center_none), NULL);
...
static void print_size_info_get_page_dimensions(PrintSizeInfo *info,
                                                gdouble *page_width,
                                                gdouble *page_height,
                                                GtkUnit unit) {
  GtkPageSetup *setup;
...
static void gimp_size_info_get_max_offsets(gdouble *offset_x_max,
...
static void print_size_info_center_none_block(void) {
  g_signal_handlers_block_by_func(info.left_adj, print_size_info_center_none,
                                  NULL);
  g_signal_handlers_block_by_func(info.right_adj, print_size_info_center_none,
                                  NULL);
  g_signal_handlers_block_by_func(info.top_adj, print_size_info_center_none,
                                  NULL);
  g_signal_handlers_block_by_func(info.bottom_adj, print_size_info_center_none,
                                  NULL);
}

static void print_size_info_center_none_unblock(void) {
  g_signal_handlers_unblock_by_func(info.left_adj, print_size_info_center_none,
                                    NULL);
  g_signal_handlers_unblock_by_func(info.right_adj, print_size_info_center_none,
                                    NULL);
  g_signal_handlers_unblock_by_func(info.top_adj, print_size_info_center_none,
                                    NULL);
...
static void print_size_info_update_offsets(void) {
  ...
  g_signal_handlers_block_by_func(info.size_entry, print_size_info_size_changed,
                                  NULL);

  print_size_info_center_none_block();

  gimp_size_entry_set_refval_boundaries(info.size_entry, LEFT, 0, offset_x_max);
  gimp_size_entry_set_refval_boundaries(info.size_entry, RIGHT, 0,
                                        offset_x_max);
  gimp_size_entry_set_refval_boundaries(info.size_entry, TOP, 0, offset_y_max);
  gimp_size_entry_set_refval_boundaries(info.size_entry, BOTTOM, 0,
                                        offset_y_max);
  ...
  gimp_size_entry_set_refval(info.size_entry, LEFT, data->offset_x);
  gimp_size_entry_set_refval(info.size_entry, RIGHT,
                             offset_x_max - data->offset_x);
  gimp_size_entry_set_refval(info.size_entry, TOP, info.data->offset_y);
...
static void print_size_info_center_changed(GtkWidget *combo) {
  gint value;
...
static void print_size_info_set_resolution(PrintSizeInfo *info, gdouble xres,
  ...
  g_signal_handlers_block_by_func(info->resolution_entry,
                                  print_size_info_resolution_changed, NULL);
  ...
  print_preview_set_image_dpi(PRINT_PREVIEW(info->preview), data->xres,
                              data->yres);
  print_preview_set_image_offsets(PRINT_PREVIEW(info->preview), data->offset_x,
                                  data->offset_y);
...
static void print_size_info_set_page_setup(PrintSizeInfo *info) {
  ...
  print_size_info_get_page_dimensions(info, &page_width, &page_height,
                                      GTK_UNIT_INCH);
  ...
  if (info->area_label) {
    ...
    format = g_strdup_printf(
        "%%.%df x %%.%df %s", gimp_unit_get_digits(data->unit),
  ...
  data->min_xres = x;
  data->min_yres = y;
  gimp_size_entry_set_refval_boundaries(info->resolution_entry, 0, x,
                                        GIMP_MAX_RESOLUTION);
  gimp_size_entry_set_refval_boundaries(info->resolution_entry, 1, y,
                                        GIMP_MAX_RESOLUTION);
}
