...
enum { C_XMP_TAG = 0, C_XMP_VALUE, NUM_XMP_COLS };

enum { C_EXIF_TAG = 0, C_EXIF_VALUE, NUM_EXIF_COLS };

enum { C_IPTC_TAG = 0, C_IPTC_VALUE, NUM_IPTC_COLS };
...
static void metadata_dialog_set_metadata(GExiv2Metadata *metadata,
                                         GtkBuilder *builder);
static void metadata_dialog_append_tags(GExiv2Metadata *metadata, gchar **tags,
                                        GtkListStore *store, gint tag_column,
                                        gint value_column, gboolean load_iptc);
static void metadata_dialog_add_tag(GtkListStore *store, GtkTreeIter iter,
                                    gint tag_column, gint value_column,
                                    const gchar *tag, const gchar *value);
static void metadata_dialog_add_translated_tag(
    GExiv2Metadata *metadata, GtkListStore *store, GtkTreeIter iter,
    gint tag_column, gint value_column, const gchar *tag);
...
static gboolean metadata_viewer_dialog(GimpImage *image,
  ...
  ui_file = g_build_filename(gimp_data_directory(), "ui", "plug-ins",
                             "plug-in-metadata-viewer.ui", NULL);

  if (!gtk_builder_add_from_file(builder, ui_file, &local_error)) {
    if (!local_error)
      local_error = g_error_new_literal(
          G_FILE_ERROR, 0, _("Error loading metadata-viewer dialog."));
  ...
  dialog = gimp_dialog_new(title, "gimp-metadata-viewer-dialog", NULL, 0,
                           gimp_standard_help_func, PLUG_IN_PROC, _("_Close"),
  ...
  metadata_vbox = GTK_WIDGET(gtk_builder_get_object(builder, "metadata-vbox"));
...
static gchar *metadata_format_string_value(const gchar *value,
  ...
  if (!truncate || size <= TAG_VALUE_MAX_SIZE) {
    ...
    g_string_append(str, "... ");
    g_string_append_printf(str, _("(%lu more character(s))"),
...
static void metadata_dialog_add_tag(GtkListStore *store, GtkTreeIter iter,
  ...
  if (value) {
    ...
    gtk_list_store_set(store, &iter, tag_column, tag, value_column, value, -1);
  }
}
...
static void metadata_dialog_add_translated_tag(
  ...
  metadata_dialog_add_tag(store, iter, tag_column, value_column, tag,
                          gettext(value));
  g_free(value);
}
    ...
    if (load_iptc && metadata_tag_is_string(tag)) {
      ...
      if (values) {
        ...
        for (i = 0; values[i] != NULL; i++) {
          ...
          value =
              metadata_format_string_value(values[i], /* truncate = */ TRUE);

          gtk_list_store_set(store, &iter, tag_column, tag, value_column, value,
                             -1);
      ...
      if (!gps_done) {
        ...
        if (gexiv2_metadata_get_gps_longitude(GEXIV2_METADATA(metadata),
          ...
          metadata_dialog_add_tag(store, iter, tag_column, value_column,
                                  "Exif.GPSInfo.GPSLongitude", str);
          g_free(str);
        }
        metadata_dialog_add_translated_tag(metadata, store, iter, tag_column,
                                           value_column,
                                           "Exif.GPSInfo.GPSLongitudeRef");

        if (gexiv2_metadata_get_gps_latitude(GEXIV2_METADATA(metadata), &lat)) {
          str = metadata_format_gps_longitude_latitude(lat);
          metadata_dialog_add_tag(store, iter, tag_column, value_column,
                                  "Exif.GPSInfo.GPSLatitude", str);
          g_free(str);
        }
        metadata_dialog_add_translated_tag(metadata, store, iter, tag_column,
                                           value_column,
        ...
        if (gexiv2_metadata_get_gps_altitude(GEXIV2_METADATA(metadata), &alt)) {
          gchar *str2, *str3;
...
static gchar *metadata_dialog_format_tag_value(GExiv2Metadata *metadata,
  ...
  if (metadata_tag_is_string(tag)) {
    ...
    if (display_size < size) {
      g_string_append(str, " ... ");
      g_string_append_printf(str, _("(%llu more byte(s))"),
