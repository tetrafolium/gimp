...
void xml_parser_start_element(GMarkupParseContext *context,
...
void xml_parser_data(GMarkupParseContext *context, const gchar *text,
...
void set_tag_ui(metadata_editor *args, gint index, gchar *name, gchar *value,
                gchar *mode) {
  GtkWidget *widget;
  gchar *value_utf;
  ...
  if (!strcmp("single", mode)) {
    ...
    if (!strcmp(LICENSOR_HEADER, name)) {
      for (row = 1; row < row_count + 1; row++) {
        ...
        gtk_list_store_set(
            liststore, &iter, COL_LICENSOR_NAME, list_tag_data[row][1],
            COL_LICENSOR_ID, list_tag_data[row][2], COL_LICENSOR_PHONE1,
            list_tag_data[row][3], COL_LICENSOR_PHONE_TYPE1,
            list_tag_data[row][4], COL_LICENSOR_PHONE2, list_tag_data[row][5],
            COL_LICENSOR_PHONE_TYPE2, list_tag_data[row][6], COL_LICENSOR_EMAIL,
            list_tag_data[row][7], COL_LICENSOR_WEB, list_tag_data[row][8], -1);
      ...
      if (row_count < 2) {
        for (row = 0; row < 2 - row_count; row++) {
          gtk_list_store_append(liststore, &iter);
          gtk_list_store_set(
              liststore, &iter, COL_LICENSOR_NAME, NULL, COL_LICENSOR_ID, NULL,
              COL_LICENSOR_PHONE1, NULL, COL_LICENSOR_PHONE_TYPE1, NULL,
              COL_LICENSOR_PHONE2, NULL, COL_LICENSOR_PHONE_TYPE2, NULL,
      ...
      for (row = 1; row < row_count + 1; row++) {
        ...
        gtk_list_store_set(liststore, &iter, COL_IMG_CR8_NAME,
                           list_tag_data[row][1], COL_IMG_CR8_ID,
      ...
      for (row = 1; row < row_count + 1; row++) {
        gtk_list_store_append(liststore, &iter);
        gtk_list_store_set(
            liststore, &iter, COL_AOO_TITLE, list_tag_data[row][1],
            COL_AOO_DATE_CREAT, list_tag_data[row][2], COL_AOO_CREATOR,
            list_tag_data[row][3], COL_AOO_SOURCE, list_tag_data[row][4],
            COL_AOO_SRC_INV_ID, list_tag_data[row][5], COL_AOO_CR_NOT,
            list_tag_data[row][6], -1);
      ...
      if (row_count < 2) {
        for (row = 0; row < 2 - row_count; row++) {
          ...
          gtk_list_store_set(liststore, &iter, COL_AOO_TITLE, NULL,
                             COL_AOO_DATE_CREAT, NULL, COL_AOO_CREATOR, NULL,
                             COL_AOO_SOURCE, NULL, COL_AOO_SRC_INV_ID, NULL,
                             COL_AOO_CR_NOT, NULL, -1);
        }
      }
      ...
      if (row_count > 0) {
        for (row = 1; row < row_count + 1; row++) {
          ...
          gtk_list_store_set(liststore, &iter, COL_CR_OWNER_NAME,
                             list_tag_data[row][1], COL_CR_OWNER_ID,
                             list_tag_data[row][2], -1);
      ...
      for (row = 1; row < row_count + 1; row++) {
        gtk_list_store_append(liststore, &iter);
        gtk_list_store_set(
            liststore, &iter, COL_LOC_SHO_SUB_LOC, list_tag_data[row][1],
            COL_LOC_SHO_CITY, list_tag_data[row][2], COL_LOC_SHO_STATE_PROV,
            list_tag_data[row][3], COL_LOC_SHO_CNTRY, list_tag_data[row][4],
            COL_LOC_SHO_CNTRY_ISO, list_tag_data[row][5],
            COL_LOC_SHO_CNTRY_WRLD_REG, list_tag_data[row][6], -1);
      ...
      for (row = 1; row < row_count + 1; row++) {
        gtk_list_store_append(liststore, &iter);
        gtk_list_store_set(liststore, &iter, COL_ORG_IMG_NAME,
                           list_tag_data[row][1], -1);
      ...
      for (row = 1; row < row_count + 1; row++) {
        gtk_list_store_append(liststore, &iter);
        gtk_list_store_set(liststore, &iter, COL_ORG_IMG_CODE,
                           list_tag_data[row][1], -1);
        if (list_tag_data[row][1]) {
          if (list_tag_data[row][1])
            g_free(list_tag_data[row][1]);
      ...
      for (row = 1; row < row_count + 1; row++) {
        gtk_list_store_append(liststore, &iter);
        gtk_list_store_set(liststore, &iter, COL_PROP_REL_ID,
                           list_tag_data[row][1], -1);
        if (list_tag_data[row][1]) {
          if (list_tag_data[row][1])
            g_free(list_tag_data[row][1]);
      ...
      for (row = 1; row < row_count + 1; row++) {
        gtk_list_store_append(liststore, &iter);
        gtk_list_store_set(liststore, &iter, COL_MOD_REL_ID,
                           list_tag_data[row][1], -1);
        if (list_tag_data[row][1]) {
          if (list_tag_data[row][1])
            g_free(list_tag_data[row][1]);
...
const gchar *get_tag_ui_text(metadata_editor *args, gchar *name, gchar *mode) {
  GObject *object;
...
gchar *get_tag_ui_list(metadata_editor *args, gchar *name, gchar *mode) {
  GObject *object;
  GtkWidget *widget;
  GtkTreeModel *treemodel;
  GtkListStore *liststore;
  ...
  for (row = 0; row < number_of_rows; row++) {
    if (gtk_tree_model_iter_nth_child(treemodel, &iter, NULL, row)) {
      if (!strcmp(LICENSOR_HEADER, name)) {
        gtk_tree_model_get(
            treemodel, &iter, COL_LICENSOR_NAME, &tagdata[row][0],
            COL_LICENSOR_ID, &tagdata[row][1], COL_LICENSOR_PHONE1,
            &tagdata[row][2], COL_LICENSOR_PHONE_TYPE1, &tagdata[row][3],
            COL_LICENSOR_PHONE2, &tagdata[row][4], COL_LICENSOR_PHONE_TYPE2,
            &tagdata[row][5], COL_LICENSOR_EMAIL, &tagdata[row][6],
            COL_LICENSOR_WEB, &tagdata[row][7], -1);
        ...
        gtk_tree_model_get(treemodel, &iter, COL_CR_OWNER_NAME,
                           &tagdata[row][0], COL_CR_OWNER_ID, &tagdata[row][1],
                           -1);
        ...
        gtk_tree_model_get(treemodel, &iter, COL_IMG_CR8_NAME, &tagdata[row][0],
                           COL_IMG_CR8_ID, &tagdata[row][1], -1);
        ...
        gtk_tree_model_get(treemodel, &iter, COL_AOO_TITLE, &tagdata[row][0],
                           COL_AOO_DATE_CREAT, &tagdata[row][1],
                           COL_AOO_CREATOR, &tagdata[row][2], COL_AOO_SOURCE,
                           &tagdata[row][3], COL_AOO_SRC_INV_ID,
                           &tagdata[row][4], COL_AOO_CR_NOT, &tagdata[row][5],
                           -1);
        ...
        gtk_tree_model_get(treemodel, &iter, COL_LOC_SHO_SUB_LOC,
                           &tagdata[row][0], COL_LOC_SHO_CITY, &tagdata[row][1],
                           COL_LOC_SHO_STATE_PROV, &tagdata[row][2],
                           COL_LOC_SHO_CNTRY, &tagdata[row][3],
                           COL_LOC_SHO_CNTRY_ISO, &tagdata[row][4],
                           COL_LOC_SHO_CNTRY_WRLD_REG, &tagdata[row][5], -1);
        ...
        gtk_tree_model_get(treemodel, &iter, COL_ORG_IMG_NAME, &tagdata[row][0],
                           -1);
        ...
        gtk_tree_model_get(treemodel, &iter, COL_ORG_IMG_CODE, &tagdata[row][0],
                           -1);
        ...
        gtk_tree_model_get(treemodel, &iter, COL_PROP_REL_ID, &tagdata[row][0],
                           -1);
        ...
        gtk_tree_model_get(treemodel, &iter, COL_MOD_REL_ID, &tagdata[row][0],
                           -1);
...
gint get_tag_ui_combo(metadata_editor *args, gchar *name, gchar *mode) {
  GObject *object;
...
void xml_parser_end_element(GMarkupParseContext *context,
...
#endif
          if (force_write == TRUE)
            gexiv2_metadata_set_tag_string(args->metadata, str_tag_name,
                                           str_tag_value);
          break;
        }
      }
...
gboolean xml_parser_parse_file(GimpXmlParser *parser, const gchar *filename,
                               GError **error) {
  GIOChannel *io;
  gboolean success;
...
GimpXmlParser *xml_parser_new(const GMarkupParser *markup_parser,
                              gpointer user_data) {
  ...
  parser->context =
      g_markup_parse_context_new(markup_parser, 0, user_data, NULL);

  return parser;
}

void xml_parser_free(GimpXmlParser *parser) {
...
gboolean parse_encoding(const gchar *text, gint text_len, gchar **encoding) {
  const gchar *start;
  const gchar *end;
  gint i;
...
gboolean xml_parser_parse_io_channel(GimpXmlParser *parser, GIOChannel *io,
                                     GError **error) {
  GIOStatus status;
  gchar buffer[4096];
  gsize len = 0;
  ...
  while (TRUE) {
    ...
    status = g_io_channel_read_chars(io, buffer, sizeof(buffer), &len, error);
