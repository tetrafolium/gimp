...
struct _EXRLoader {
  _EXRLoader(const char *filename)
      : refcount_(1), file_(filename),
        data_window_(file_.header().dataWindow()),
        channels_(file_.header().channels()) {
    const Channel *chan;

    if (channels_.findChannel("R") || channels_.findChannel("G") ||
      ...
      if ((chan = channels_.findChannel("R")))
        ...
        pt_ = channels_.findChannel("B")->type;
               ...
               (channels_.findChannel("RY") || channels_.findChannel("BY"))) {
      format_string_ = "RGB";
      image_type_ = IMAGE_TYPE_RGB;
  ...
  int readPixelRow(char *pixels, int bpp, int row) {
  ...
  int getWidth() const { return data_window_.max.x - data_window_.min.x + 1; }

  int getHeight() const { return data_window_.max.y - data_window_.min.y + 1; }
  ...
  EXRImageType getImageType() const { return image_type_; }

  int hasAlpha() const { return has_alpha_ ? 1 : 0; }
  ...
  GimpColorProfile *getProfile() const {
    ...
    cmsCIExyY whitePoint = {chromaticities.white.x, chromaticities.white.y,
                            whiteLuminance};
    cmsCIExyYTRIPLE CameraPrimaries = {
        {chromaticities.red.x, chromaticities.red.y, whiteLuminance},
        {chromaticities.green.x, chromaticities.green.y, whiteLuminance},
        {chromaticities.blue.x, chromaticities.blue.y, whiteLuminance}};
    ...
    gimp_r_XYZ =
        (cmsCIEXYZ *)cmsReadTag(linear_srgb_lcms, cmsSigRedColorantTag);
    gimp_g_XYZ =
        (cmsCIEXYZ *)cmsReadTag(linear_srgb_lcms, cmsSigGreenColorantTag);
    gimp_b_XYZ =
        (cmsCIEXYZ *)cmsReadTag(linear_srgb_lcms, cmsSigBlueColorantTag);
    gimp_w_XYZ =
        (cmsCIEXYZ *)cmsReadTag(linear_srgb_lcms, cmsSigMediaWhitePointTag);
    ...
    // ... and check if the data stored in the EXR matches GIMP's internal
    // profile
    ...
    Gamma[0] = Gamma[1] = Gamma[2] =
        cmsBuildParametricToneCurve(0, 1, Parameters);
  ...
  gchar *getComment() const {
    char *result = NULL;
    const Imf::StringAttribute *comment =
        file_.header().findTypedAttribute<Imf::StringAttribute>("comment");
    if (comment)
  ...
  guchar *getExif(guint *size) const {
    ...
    *size = 0;
    ...
    const Imf::BlobAttribute *exif =
        file_.header().findTypedAttribute<Imf::BlobAttribute>("exif");

    if (exif) {
      exif_data = (guchar *)(exif->value().data.get());
      *size = exif->value().size;
      // darktable appends a jpg-compatible exif00 string, so get rid of that
      // again:
  ...
  guchar *getXmp(guint *size) const {
    guchar *result = NULL;
    *size = 0;
    const Imf::StringAttribute *xmp =
        file_.header().findTypedAttribute<Imf::StringAttribute>("xmp");
    if (xmp) {
      *size = xmp->value().size();
