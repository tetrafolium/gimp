...
static GimpValueArray *exr_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);

static GimpImage *load_image(GFile *file, gboolean interactive, GError **error);
...
static void exr_class_init(ExrClass *klass) {
...
static GimpProcedure *exr_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, exr_load, NULL, NULL);

    gimp_procedure_set_menu_label(procedure, N_("OpenEXR image"));

    gimp_procedure_set_documentation(
        procedure, "Loads files in the OpenEXR file format",
        "This plug-in loads OpenEXR files. ", name);
    ...
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-exr");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "exr");
...
static GimpValueArray *exr_load(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  image = load_image(file, run_mode == GIMP_RUN_INTERACTIVE, &error);
...
static GimpImage *load_image(GFile *file, gboolean interactive,
  ...
  layer = gimp_layer_new(image, _("Background"), width, height, layer_type, 100,
  ...
  for (begin = 0; begin < height; begin += tile_height) {
    ...
    for (i = 0; i < num; i++) {
      ...
      retval = exr_loader_read_pixel_row(loader, pixels + (i * width * bpp),
                                         bpp, begin + i);
      if (retval < 0) {
        g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
    ...
    gegl_buffer_set(buffer, GEGL_RECTANGLE(0, begin, width, num), 0, NULL,
  ...
  if (exif_data || xmp_data) {
    ...
    gimp_image_metadata_load_finish(image, "image/exr", metadata, flags);
