...
static void save_dialog_toggle_scale(GObject *config, const GParamSpec *pspec,
                                     GtkWidget *scale) {
  gboolean lossless;

  g_object_get(config, "lossless", &lossless, NULL);

  gtk_widget_set_sensitive(scale, !lossless);
}

static void show_maxkeyframe_hints(GObject *config, const GParamSpec *pspec,
                                   GtkLabel *label) {
  gint kmax;

  g_object_get(config, "keyframe-distance", &kmax, NULL);
...
gboolean save_dialog(GimpImage *image, GimpProcedure *procedure,
  ...
  toggle = gimp_prop_check_button_new(config, "lossless", _("_Lossless"));
  gtk_grid_attach(GTK_GRID(grid), toggle, 0, row, 3, 1);
  row++;

  /* Create the slider for image quality */
  quality_scale = gimp_prop_scale_entry_new(
      config, "quality", _("Image _quality:"), 1.0, FALSE, 0, 0);
  ...
  alpha_quality_scale = gimp_prop_scale_entry_new(
      config, "alpha-quality", _("Alpha q_uality:"), 1.0, FALSE, 0, 0);
  ...
  g_signal_connect(config, "notify::lossless",
                   G_CALLBACK(save_dialog_toggle_scale), quality_scale);
  g_signal_connect(config, "notify::lossless",
                   G_CALLBACK(save_dialog_toggle_scale), alpha_quality_scale);

  save_dialog_toggle_scale(config, NULL, quality_scale);
  save_dialog_toggle_scale(config, NULL, alpha_quality_scale);

  /* Create the combobox containing the presets */
  store = gimp_int_store_new("Default", WEBP_PRESET_DEFAULT, "Picture",
                             WEBP_PRESET_PICTURE, "Photo", WEBP_PRESET_PHOTO,
                             "Drawing", WEBP_PRESET_DRAWING, "Icon",
                             WEBP_PRESET_ICON, "Text", WEBP_PRESET_TEXT, NULL);
  combo = gimp_prop_int_combo_box_new(config, "preset", GIMP_INT_STORE(store));
  g_object_unref(store);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("Source _type:"),
                                   0.0, 0.5, combo, 2);
  gimp_help_set_help_data(label, _("WebP encoder \"preset\""), NULL);
  ...
  if (animation_supported) {
    ...
    toggle = gimp_prop_check_button_new(config, "animation", text);
    g_free(text);
    ...
    g_object_bind_property(toggle, "active", frame, "visible",
    ...
    kf_distance =
        gimp_prop_spin_button_new(config, "keyframe-distance", 1.0, 1.0, 0);
    ...
    g_signal_connect(config, "notify::keyframe-distance",
                     G_CALLBACK(show_maxkeyframe_hints), label_kf);
     ...
     * option is selected
     */
    g_object_bind_property(config, "minimize-size", hbox_kf, "sensitive",
    ...
    /* default delay */
    delay = gimp_prop_spin_button_new(config, "default-delay", 1, 10, 0);
  ...
  /* XMP metadata */
  toggle = gimp_prop_check_button_new(config, "save-xmp", _("Save _XMP data"));
  gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);
