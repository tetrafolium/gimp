...
int webp_file_writer(const uint8_t *data, size_t data_size,
                     const WebPPicture *picture);
int webp_file_progress(int percent, const WebPPicture *picture);
const gchar *webp_error_string(WebPEncodingError error_code);

static void webp_decide_output(GimpImage *image, GObject *config,
                               GimpColorProfile **profile,
                               gboolean *out_linear);

int webp_anim_file_writer(FILE *outfile, const uint8_t *data,
                          size_t data_size) {
  int ok = 0;
...
gboolean save_layer(GFile *file, GimpImage *image, GimpDrawable *drawable,
  ...
  g_object_get(config, "preset", &preset, "lossless", &lossless, "quality",
               &quality, "alpha-quality", &alpha_quality, NULL);
  ...
  do {
    ...
    g_free(buffer);
    if (!status) {
      g_printerr("%s: memory error in WebPPictureImportRGB(A)().", G_STRFUNC);
      break;
    }

    /* Perform the actual encode */
    if (!WebPEncode(&webp_config, &picture)) {
      g_printerr("WebP error: '%s'", webp_error_string(picture.error_code));
      g_set_error(error, G_FILE_ERROR, picture.error_code,
                  _("WebP error: '%s'"), webp_error_string(picture.error_code));
      status = FALSE;
      break;
    }
    ...
    if (res > 0) {
      ...
      if (mux) {
        ...
        if (profile) {
          ...
          icc_data =
              gimp_color_profile_get_icc_profile(profile, &icc_data_size);
      ...
      g_printerr(
          "ERROR: No data read for features. Can't save features update.\n");
    }
  } else {
...
static gint get_layer_delay(GimpLayer *layer) {
  gchar *layer_name;
  gint delay_ms;
...
static gboolean parse_combine(const char *str) {
  gint offset = 0;
...
static gboolean get_layer_needs_combine(GimpLayer *layer) {
  gchar *layer_name;
...
static GeglBuffer *combine_buffers(GeglBuffer *layer_buffer,
  ...
  source = gegl_node_new_child(graph, "operation", "gegl:buffer-source",
                               "buffer", layer_buffer, NULL);
  backdrop = gegl_node_new_child(graph, "operation", "gegl:buffer-source",
                                 "buffer", prev_frame_buffer, NULL);

  over = gegl_node_new_child(graph, "operation", "gegl:over", NULL);
  target = gegl_node_new_child(graph, "operation", "gegl:write-buffer",
                               "buffer", buffer, NULL);
  gegl_node_link_many(backdrop, over, target, NULL);
  gegl_node_connect_to(source, "output", over, "aux");
...
gboolean save_animation(GFile *file, GimpImage *image, gint n_drawables,
  ...
  g_object_get(config, "preset", &preset, "lossless", &lossless, "animation",
               &animation, "animation-loop", &loop, "minimize-size",
               &minimize_size, "keyframe-distance", &keyframe_distance,
...
static void webp_decide_output(GimpImage *image, GObject *config,
  ...
  g_object_get(config, "save-color-profile", &save_profile, NULL);
