...
/* ----- Known Image Resource Block Types -----
   ...
   PSD_PS2_IMAGE_INFO    = 1000,    Dropped    * 0x03e8 - Obsolete - ps 2.0
   image info * PSD_MAC_PRINT_INFO    = 1001,    PS Only    * 0x03e9 - Optional
   - Mac print manager print info record * PSD_PS2_COLOR_TAB     = 1003, Dropped
   * 0x03eb - Obsolete - ps 2.0 indexed color table * PSD_RESN_INFO         =
   1005,    Loaded     * 0x03ed - ResolutionInfo structure * PSD_ALPHA_NAMES =
   1006,    Loaded     * 0x03ee - Alpha channel names * PSD_DISPLAY_INFO      =
   1007,    Loaded     * 0x03ef - DisplayInfo structure * PSD_CAPTION = 1008,
   Loaded     * 0x03f0 - Optional - Caption string * PSD_BORDER_INFO       =
   1009,               * 0x03f1 - Border info * PSD_BACKGROUND_COL    = 1010,
   * 0x03f2 - Background color * PSD_PRINT_FLAGS       = 1011,               *
   0x03f3 - Print flags * PSD_GREY_HALFTONE     = 1012,               * 0x03f4 -
   Greyscale and multichannel halftoning info * PSD_COLOR_HALFTONE    = 1013,
   * 0x03f5 - Color halftoning info * PSD_DUOTONE_HALFTONE  = 1014, * 0x03f6 -
   Duotone halftoning info * PSD_GREY_XFER         = 1015,               *
   0x03f7 - Greyscale and multichannel transfer functions * PSD_COLOR_XFER =
   1016,               * 0x03f8 - Color transfer functions * PSD_DUOTONE_XFER =
   1017,               * 0x03f9 - Duotone transfer functions * PSD_DUOTONE_INFO
   = 1018,               * 0x03fa - Duotone image information * PSD_EFFECTIVE_BW
   = 1019,               * 0x03fb - Effective black & white values for dot range
   * PSD_OBSOLETE_01       = 1020,    Dropped    * 0x03fc - Obsolete *
   ...
   PSD_WORKING_PATH      = 1025,               * 0x0401 - Working path (not
   saved) * PSD_LAYER_GROUP       = 1026,               * 0x0402 - Layers group
   info * PSD_OBSOLETE_03       = 1027,    Dropped    * 0x0403 - Obsolete *
   PSD_IPTC_NAA_DATA     = 1028,    Loaded     * 0x0404 - IPTC-NAA record
   (IMV4.pdf) * PSD_IMAGE_MODE_RAW    = 1029,               * 0x0405 - Image
   mode for raw format files * PSD_JPEG_QUAL         = 1030,    PS Only    *
   0x0406 - JPEG quality * PSD_GRID_GUIDE        = 1032,    Loaded     * 0x0408
   - Grid & guide info * PSD_THUMB_RES         = 1033,    Special    * 0x0409 -
   Thumbnail resource * PSD_COPYRIGHT_FLG     = 1034,               * 0x040a -
   Copyright flag * PSD_URL               = 1035,               * 0x040b - URL
   string * PSD_THUMB_RES2        = 1036,    Special    * 0x040c - Thumbnail
   resource * PSD_GLOBAL_ANGLE      = 1037,               * 0x040d - Global
   angle * PSD_COLOR_SAMPLER     = 1038,               * 0x040e - Color samplers
   resource * PSD_ICC_PROFILE       = 1039,    Loaded     * 0x040f - ICC Profile
   * PSD_WATERMARK         = 1040,               * 0x0410 - Watermark *
   PSD_ICC_UNTAGGED      = 1041,               * 0x0411 - Do not use ICC profile
   flag * PSD_EFFECTS_VISIBLE   = 1042,               * 0x0412 - Show   hide all
   effects layers * PSD_SPOT_HALFTONE     = 1043,               * 0x0413 - Spot
   halftone * PSD_DOC_IDS           = 1044,               * 0x0414 - Document
   specific IDs * PSD_ALPHA_NAMES_UNI   = 1045,    Loaded     * 0x0415 - Unicode
   alpha names * PSD_IDX_COL_TAB_CNT   = 1046,    Loaded     * 0x0416 - Indexed
   color table count * PSD_IDX_TRANSPARENT   = 1047,               * 0x0417 -
   Index of transparent color (if any) * PSD_GLOBAL_ALT        = 1049, * 0x0419
   - Global altitude * PSD_SLICES            = 1050,               * 0x041a -
   Slices * PSD_WORKFLOW_URL_UNI  = 1051,               * 0x041b - Workflow URL
   - Unicode string * PSD_JUMP_TO_XPEP      = 1052,               * 0x041c -
   Jump to XPEP (?) * PSD_ALPHA_ID          = 1053,    Loaded     * 0x041d -
   Alpha IDs * PSD_URL_LIST_UNI      = 1054,               * 0x041e - URL list -
   unicode * PSD_VERSION_INFO      = 1057,               * 0x0421 - Version info
   ...
   * PSD_XMP_DATA          = 1060,    Loaded     * 0x0424 - XMP data block *
   ...
   PSD_ALT_DUOTONE_COLOR = 1066,               * 0x042A - Alternative Duotone
   colors * PSD_ALT_SPOT_COLOR    = 1067,               * 0x042B - Alternative
   Spot colors * PSD_LAYER_SELECT_ID   = 1069,               * 0x042D - Layer
   selection ID * PSD_HDR_TONING_INFO   = 1070,               * 0x042E - HDR
   toning information * PSD_PRINT_INFO_SCALE  = 1071,               * 0x042F -
   Print scale * PSD_LAYER_GROUP_E_ID  = 1072,               * 0x0430 - Layer
   group(s) enabled ID * PSD_COLOR_SAMPLER_NEW = 1073,               * 0x0431 -
   Color sampler resource for ps CS3 and higher PSD files * PSD_MEASURE_SCALE =
   1074,               * 0x0432 - Measurement scale * PSD_TIMELINE_INFO     =
   1075,               * 0x0433 - Timeline information * PSD_SHEET_DISCLOSE    =
   1076,               * 0x0434 - Sheet discloser * PSD_DISPLAY_INFO_NEW  =
   1077,    Loaded     * 0x0435 - DisplayInfo structure for ps CS3 and higher
   PSD files * PSD_ONION_SKINS       = 1078,               * 0x0436 - Onion
   skins * PSD_COUNT_INFO        = 1080,               * 0x0438 - Count
   information* PSD_PRINT_INFO        = 1082,               * 0x043A - Print
   information added in ps CS5* PSD_PRINT_STYLE       = 1083,               *
   0x043B - Print style * PSD_MAC_NSPRINTINFO   = 1084,               * 0x043C -
   Mac NSPrintInfo* PSD_WIN_DEVMODE       = 1085,               * 0x043D -
   Windows DEVMODE * PSD_AUTO_SAVE_PATH    = 1086,               * 0x043E - Auto
   save file path * PSD_AUTO_SAVE_FORMAT  = 1087,               * 0x043F - Auto
   save format * PSD_PATH_INFO_FIRST   = 2000,    Loaded     * 0x07d0 - First
   path info block * PSD_PATH_INFO_LAST    = 2998,    Loaded     * 0x0bb6 - Last
   path info block * PSD_CLIPPING_PATH     = 2999,               * 0x0bb7 - Name
   of clipping path * PSD_PLUGIN_R_FIRST    = 4000,               * 0x0FA0 -
   First plugin resource * PSD_PLUGIN_R_LAST     = 4999,               * 0x1387
   - Last plugin resource * PSD_IMAGEREADY_VARS   = 7000,    PS Only    * 0x1B58
   - Imageready variables * PSD_IMAGEREADY_DATA   = 7001,    PS Only    * 0x1B59
   - Imageready data sets * PSD_LIGHTROOM_WORK    = 8000,    PS Only    * 0x1F40
   - Lightroom workflow * PSD_PRINT_FLAGS_2     = 10000               * 0x2710 -
   Print flags *
 */
...
static gint load_resource_unknown(const PSDimageres *res_a, GimpImage *image,
                                  FILE *f, GError **error);

static gint load_resource_ps_only(const PSDimageres *res_a, GimpImage *image,
                                  FILE *f, GError **error);

static gint load_resource_1005(const PSDimageres *res_a, GimpImage *image,
                               FILE *f, GError **error);

static gint load_resource_1006(const PSDimageres *res_a, GimpImage *image,
                               PSDimage *img_a, FILE *f, GError **error);

static gint load_resource_1007(const PSDimageres *res_a, GimpImage *image,
                               PSDimage *img_a, FILE *f, GError **error);

static gint load_resource_1008(const PSDimageres *res_a, GimpImage *image,
                               FILE *f, GError **error);

static gint load_resource_1022(const PSDimageres *res_a, GimpImage *image,
                               PSDimage *img_a, FILE *f, GError **error);

static gint load_resource_1024(const PSDimageres *res_a, GimpImage *image,
                               PSDimage *img_a, FILE *f, GError **error);

static gint load_resource_1028(const PSDimageres *res_a, GimpImage *image,
                               FILE *f, GError **error);

static gint load_resource_1032(const PSDimageres *res_a, GimpImage *image,
                               FILE *f, GError **error);

static gint load_resource_1033(const PSDimageres *res_a, GimpImage *image,
                               FILE *f, GError **error);

static gint load_resource_1039(const PSDimageres *res_a, PSDimage *img_a,
                               GimpImage *image, FILE *f, GError **error);

static gint load_resource_1045(const PSDimageres *res_a, GimpImage *image,
                               PSDimage *img_a, FILE *f, GError **error);

static gint load_resource_1046(const PSDimageres *res_a, GimpImage *image,
                               FILE *f, GError **error);

static gint load_resource_1053(const PSDimageres *res_a, GimpImage *image,
                               PSDimage *img_a, FILE *f, GError **error);

static gint load_resource_1058(const PSDimageres *res_a, GimpImage *image,
                               FILE *f, GError **error);

static gint load_resource_1077(const PSDimageres *res_a, GimpImage *image,
                               PSDimage *img_a, FILE *f, GError **error);

static gint load_resource_2000(const PSDimageres *res_a, GimpImage *image,
                               FILE *f, GError **error);
...
gint get_image_resource_header(PSDimageres *res_a, FILE *f, GError **error) {
  ...
  if (fread(&res_a->type, 4, 1, f) < 1 || fread(&res_a->id, 2, 1, f) < 1) {
  ...
  IFDBG(2)
  g_debug("Type: %.4s, id: %d, start: %d, len: %d", res_a->type, res_a->id,
          res_a->data_start, res_a->data_len);
...
gint load_image_resource(PSDimageres *res_a, GimpImage *image, PSDimage *img_a,
  ...
  if (memcmp(res_a->type, "8BIM", 4) != 0 &&
    ...
    IFDBG(1) g_debug("Unknown image resource type signature %.4s", res_a->type);
  } else {
    switch (res_a->id) {
    ...
    case PSD_OBSOLETE_03:
      ...
      IFDBG(2) g_debug("Obsolete image resource block: %d", res_a->id);
      break;

    case PSD_THUMB_RES:
    case PSD_THUMB_RES2:
      /* Drop thumbnails from standard file load */
      IFDBG(2) g_debug("Thumbnail resource block: %d", res_a->id);
      break;
    ...
    default:
      if (res_a->id >= 2000 && res_a->id < 2999)
        load_resource_2000(res_a, image, f, error);
      else
...
gint load_thumbnail_resource(PSDimageres *res_a, GimpImage *image, FILE *f,
  ...
  /* Process image resource blocks */
  if (res_a->id == PSD_THUMB_RES || res_a->id == PSD_THUMB_RES2) {
...
static gint load_resource_unknown(const PSDimageres *res_a, GimpImage *image,
  ...
  name =
      g_strdup_printf("psd-image-resource-%.4s-%.4x", res_a->type, res_a->id);
...
static gint load_resource_1005(const PSDimageres *res_a, GimpImage *image,
  ...
  IFDBG(3)
  g_debug("Resolution: %d, %d, %d, %d, %d, %d", res_info.hRes,
          res_info.hResUnit, res_info.widthUnit, res_info.vRes,
          res_info.vResUnit, res_info.heightUnit);
  ...
  gimp_image_set_resolution(image, res_info.hRes / 65536.0,
                            res_info.vRes / 65536.0);
  ...
  for (cidx = 0; cidx < tot_rec; ++cidx) {
    ...
    case PSD_CS_RGB:
      gimp_rgb_set(&gimp_rgb, ps_color.rgb.red / 65535.0,
                   ps_color.rgb.green / 65535.0, ps_color.rgb.blue / 65535.0);
      break;
    ...
    case PSD_CS_GRAYSCALE:
      gimp_rgb_set(&gimp_rgb, ps_color.gray.gray / 10000.0,
                   ps_color.gray.gray / 10000.0, ps_color.gray.gray / 10000.0);
      break;
    ...
    default:
      if (CONVERSION_WARNINGS)
        g_message("Unsupported color space: %d", dsp_info.colorSpace);
...
static gint load_resource_1022(const PSDimageres *res_a, GimpImage *image,
  ...
  IFDBG(3)
  g_debug("Quick mask channel: %d, empty: %d", img_a->quick_mask_id,
          quick_mask_empty);
...
static gint load_resource_1028(const PSDimageres *res_a, GimpImage *image,
  ...
  if (iptc_buf_len > 0) {
    parasite = gimp_parasite_new(GIMP_PARASITE_IPTC, GIMP_PARASITE_PERSISTENT,
...
static gint load_resource_1032(const PSDimageres *res_a, GimpImage *image,
  ...
  IFDBG(3)
  g_debug("Grids & Guides: %d, %d, %d, %d", hdr.fVersion, hdr.fGridCycleV,
          hdr.fGridCycleH, hdr.fGuideCount);
  ...
  for (i = 0; i < hdr.fGuideCount; ++i) {
    ...
    IFDBG(3) g_debug("Guide: %d px, %d", guide.fLocation, guide.fDirection);
...
static gint load_resource_1033(const PSDimageres *res_a, GimpImage *image,
  ...
  IFDBG(2)
  g_debug("Process image resource block %d: Thumbnail Image", res_a->id);
  ...
  g_debug("\nThumbnail:\n"
          "\tFormat: %d\n"
          "\tDimensions: %d x %d\n",
          thumb_info.format, thumb_info.width, thumb_info.height);
  ...
  layer = gimp_layer_new(image, _("Background"), cinfo.output_width,
                         cinfo.output_height, GIMP_RGB_IMAGE, 100,
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
    jpeg_read_scanlines(&cinfo, (JSAMPARRAY)&rowbuf[cinfo.output_scanline], 1);
  }
  ...
  if (res_a->id == PSD_THUMB_RES) /* Order is BGR for resource 1033 */
  {
    ...
    for (i = 0;
         i < gegl_buffer_get_width(buffer) * gegl_buffer_get_height(buffer);
         ++i) {
      guchar r, g, b;
  ...
  gegl_buffer_set(buffer,
                  GEGL_RECTANGLE(0, 0, gegl_buffer_get_width(buffer),
...
static gint load_resource_1039(const PSDimageres *res_a, PSDimage *img_a,
  ...
  profile = gimp_color_profile_new_from_icc_profile((guint8 *)icc_profile,
                                                    res_a->data_len, NULL);
  if (profile) {
    if (img_a->color_mode == PSD_CMYK && gimp_color_profile_is_cmyk(profile)) {
...
static gint load_resource_1045(const PSDimageres *res_a, GimpImage *image,
  ...
  IFDBG(2)
  g_debug("Process image resource block 1045: Unicode Alpha Channel Names");
...
static gint load_resource_1046(const PSDimageres *res_a, GimpImage *image,
  ...
  IFDBG(2)
  g_debug("Process image resource block: 1046: Indexed Color Table Count");
...
static gint load_resource_1077(const PSDimageres *res_a, GimpImage *image,
  ...
  for (cidx = 0; cidx < tot_rec; ++cidx) {
    ...
    IFDBG(2)
    g_debug("PS cSpace: %d, col: %d %d %d %d, opacity: %d, mode: %d",
            dsp_info.colorSpace, ps_color.cmyk.cyan, ps_color.cmyk.magenta,
            ps_color.cmyk.yellow, ps_color.cmyk.black, dsp_info.opacity,
            dsp_info.mode);

    IFDBG(2)
    g_debug("cSpace: %d, col: %g %g %g, opacity: %d, mode: %d",
            dsp_info.colorSpace, gimp_rgb.r * 255, gimp_rgb.g * 255,
  ...
  while (path_rec > 0) {
    ...
    if (type == PSD_PATH_FILL_RULE) {
      ...
      while (num_rec > 0) {
        ...
        if (type == PSD_PATH_CL_LNK || type == PSD_PATH_CL_UNLNK ||
            type == PSD_PATH_OP_LNK || type == PSD_PATH_OP_UNLNK) {
          if (fread(&y[0], 4, 1, f) < 1 || fread(&x[0], 4, 1, f) < 1 ||
              fread(&y[1], 4, 1, f) < 1 || fread(&x[1], 4, 1, f) < 1 ||
              fread(&y[2], 4, 1, f) < 1 || fread(&x[2], 4, 1, f) < 1) {
            psd_set_error(feof(f), errno, error);
            return -1;
          }
          for (i = 0; i < 3; ++i) {
          ...
          IFDBG(3)
          g_debug("Path points (%d,%d), (%d,%d), (%d,%d)", x[0], y[0], x[1],
                  y[1], x[2], y[2]);
        } else {
      ...
      gimp_vectors_stroke_new_from_points(vectors,
                                          GIMP_VECTORS_STROKE_TYPE_BEZIER, cntr,
