...
static const gchar *get_enum_value_nick(GType type, gint value);
...
/* Utility function */
void psd_set_error(gboolean file_eof, gint err_no, GError **error) {
  if (file_eof) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, "%s",
                _("Unexpected end of file"));
  } else {
    g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(err_no), "%s",
                g_strerror(err_no));
  }
...
gchar *fread_pascal_string(gint32 *bytes_read, gint32 *bytes_written,
                           guint16 mod_len, FILE *f, GError **error) {
  /*
  ...
  *bytes_written = strlen(utf8_str);
  ...
  IFDBG(3)
  g_debug("Pascal string: %s, bytes_read: %d, bytes_written: %d", utf8_str,
          *bytes_read, *bytes_written);

  return utf8_str;
}

gint32 fwrite_pascal_string(const gchar *src, guint16 mod_len, FILE *f,
                            GError **error) {
  /*
  ...
  if (src == NULL) {
    ...
    if (fwrite(&null_str, 1, 1, f) < 1 || fwrite(&null_str, 1, 1, f) < 1) {
      psd_set_error(feof(f), errno, error);
      return -1;
    }
    bytes_written += 2;
    ...
    IFDBG(2)
    g_debug("Pascal string: %s, bytes_written: %d", pascal_str, bytes_written);
    g_free(pascal_str);
  }
...
gchar *fread_unicode_string(gint32 *bytes_read, gint32 *bytes_written,
                            guint16 mod_len, FILE *f, GError **error) {
  /*
  ...
  *bytes_written = utf8_str_len;
  ...
  IFDBG(3)
  g_debug("Unicode string: %s, bytes_read: %d, bytes_written: %d", utf8_str,
          *bytes_read, *bytes_written);

  return utf8_str;
}

gint32 fwrite_unicode_string(const gchar *src, guint16 mod_len, FILE *f,
                             GError **error) {
  /*
  ...
  if (src == NULL) {
    ...
    IFDBG(2)
    g_debug("Unicode string: %s, bytes_written: %d", src, bytes_written);
  }
...
gint decode_packbits(const gchar *src, gchar *dst, guint16 packed_len,
  ...
  while (unpack_left > 0 && pack_left > 0) {
    ...
    if (n < 0) /* replicate next gchar |n|+ 1 times */
    {
      ...
      if (unpack_left < n) {
        IFDBG(2)
        g_debug("Overrun in packbits replicate of %d chars", n - unpack_left);
        error_code = 2;
      }
      memset(dst, *src, n);
      src++;
...
gchar *encode_packbits(const gchar *src, guint32 unpacked_len,
  ...
  while (unpack_left > 0) {
    ...
    IFDBG(2)
    g_debug("buff %x, run len %d, curr char %x", char_buff[run_len], run_len,
            (gchar)curr_char);
    run_len++;
    ...
    if (run_len >= MIN_RUN) {
      ...
      if (i != 0) {
        ...
        if (run_len > MIN_RUN) {
          ...
          IFDBG(2)
          g_debug("(1) Number of chars: %d, run length tag: %d",
        ...
        IFDBG(2)
        g_debug("(2) Number of chars: %d, run length tag: %d", run_len,
                (1 - (gint)(run_len)));
  ...
  if (run_len != 0) {
    if (run_len <= 128) {
      ...
      IFDBG(2)
      g_debug("(4) Number of chars: %d, run length tag: %d", run_len,
              run_len - 1);
    } else {
...
void psd_to_gimp_blend_mode(const gchar *psd_mode, LayerModeInfo *mode_info) {
  gint i;
  ...
  if (CONVERSION_WARNINGS) {
    ...
    g_message("Unsupported blend mode: %s. Mode reverts to normal", mode_name);
    g_free(mode_name);
  }
}
...
GimpColorTag psd_to_gimp_layer_color_tag(guint16 layer_color_tag) {
  ...
  default:
    if (CONVERSION_WARNINGS)
      g_message("Unsupported Photoshop layer color tag: %i. GIMP layer color "
                "tag set to none.",
                layer_color_tag);
...
guint16 gimp_to_psd_layer_color_tag(GimpColorTag layer_color_tag) {
  ...
  default:
    if (CONVERSION_WARNINGS)
      g_message("Photoshop doesn't support GIMP layer color tag: %i. Photoshop "
                "layer color tag set to none.",
                layer_color_tag);
    color_tag = 0;
  }
