...
static GimpValueArray *psd_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *psd_load_thumb(GimpProcedure *procedure, GFile *file,
                                      gint size, const GimpValueArray *args,
                                      gpointer run_data);
static GimpValueArray *psd_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void psd_class_init(PsdClass *klass) {
...
static GimpProcedure *psd_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, psd_load, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "John Marshall", "John Marshall",
                                   "2007");

    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-psd");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "psd");
    ...
    procedure = gimp_thumbnail_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, psd_load_thumb, NULL, NULL);
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, psd_save, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "and guides.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Monigotes", "Monigotes", "2000");
...
static GimpValueArray *psd_load(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  image = load_image(
      file, strcmp(gimp_procedure_get_name(procedure), LOAD_MERGED_PROC) == 0,
      &resolution_loaded, &profile_loaded, &error);

  if (!image)
    return gimp_procedure_new_return_values(procedure, GIMP_PDB_EXECUTION_ERROR,
                                            error);

  metadata = gimp_image_metadata_load_prepare(image, "image/x-psd", file, NULL);
  if (metadata) {
    ...
    gimp_image_metadata_load_finish(image, "image/x-psd", metadata, flags);
...
static GimpValueArray *psd_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  metadata =
      gimp_image_metadata_save_prepare(image, "image/x-psd", &metadata_flags);

  if (save_image(file, image, &error)) {
    if (metadata) {
      gimp_metadata_set_bits_per_sample(metadata, 8);

      gimp_image_metadata_save_finish(image, "image/x-psd", metadata,
                                      metadata_flags, file, NULL);
    }
  } else {
