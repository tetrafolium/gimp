...
static gint read_header_block(PSDimage *img_a, FILE *f, GError **error);

static gint read_color_mode_block(PSDimage *img_a, FILE *f, GError **error);

static gint read_image_resource_block(PSDimage *img_a, FILE *f, GError **error);

static PSDlayer **read_layer_block(PSDimage *img_a, FILE *f, GError **error);

static gint read_merged_image_block(PSDimage *img_a, FILE *f, GError **error);

static GimpImage *create_gimp_image(PSDimage *img_a, GFile *file);

static gint add_color_map(GimpImage *image, PSDimage *img_a);

static gint add_image_resources(GimpImage *image, PSDimage *img_a, FILE *f,
                                gboolean *resolution_loaded,
                                gboolean *profile_loaded, GError **error);

static gint add_layers(GimpImage *image, PSDimage *img_a, PSDlayer **lyr_a,
                       FILE *f, GError **error);

static gint add_merged_image(GimpImage *image, PSDimage *img_a, FILE *f,
                             GError **error);
...
static gint read_channel_data(PSDchannel *channel, guint16 bps,
                              guint16 compression, const guint16 *rle_pack_len,
                              FILE *f, guint32 comp_len, GError **error);

static void convert_1_bit(const gchar *src, gchar *dst, guint32 rows,
                          guint32 columns);

static const Babl *get_layer_format(PSDimage *img_a, gboolean alpha);
...
GimpImage *load_image(GFile *file, gboolean merged_image_only,
  ...
  if (add_image_resources(image, &img_a, f, resolution_loaded, profile_loaded,
                          &error) < 0)
    goto load_error;
  ...
  IFDBG(2)
  g_debug("Close file & return, image id: %d", gimp_image_get_id(image));
  IFDBG(1)
  g_debug("\n----------------------------------------"
          "----------------------------------------\n");
...
static gint read_header_block(PSDimage *img_a, FILE *f, GError **error) {
  ...
  if (fread(sig, 4, 1, f) < 1 || fread(&version, 2, 1, f) < 1 ||
      fread(buf, 6, 1, f) < 1 || fread(&img_a->channels, 2, 1, f) < 1 ||
      fread(&img_a->rows, 4, 1, f) < 1 || fread(&img_a->columns, 4, 1, f) < 1 ||
  ...
  g_debug("\n\n\tSig: %.4s\n\tVer: %d\n\tChannels: "
          "%d\n\tSize: %dx%d\n\tBPS: %d\n\tMode: %d\n",
          sig, version, img_a->channels, img_a->columns, img_a->rows,
  ...
  if (img_a->rows < 1 || img_a->rows > GIMP_MAX_IMAGE_SIZE) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Unsupported or invalid image height: %d"), img_a->rows);
    return -1;
  }

  if (img_a->columns < 1 || img_a->columns > GIMP_MAX_IMAGE_SIZE) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Unsupported or invalid image width: %d"), img_a->columns);
    return -1;
  }
  ...
  if (img_a->columns > G_MAXINT32 / img_a->rows) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Unsupported or invalid image size: %dx%d"), img_a->columns,
                img_a->rows);
    return -1;
  }

  if (img_a->color_mode != PSD_BITMAP && img_a->color_mode != PSD_GRAYSCALE &&
      img_a->color_mode != PSD_INDEXED && img_a->color_mode != PSD_RGB &&
      img_a->color_mode != PSD_MULTICHANNEL && img_a->color_mode != PSD_CMYK &&
...
static gint read_color_mode_block(PSDimage *img_a, FILE *f, GError **error) {
  ...
  if (block_len == 0) {
    if (img_a->color_mode == PSD_INDEXED || img_a->color_mode == PSD_DUOTONE) {
...
static gint read_image_resource_block(PSDimage *img_a, FILE *f,
                                      GError **error) {
  guint32 block_len;
  guint32 block_end;
...
static PSDlayer **read_layer_info(PSDimage *img_a, FILE *f, GError **error) {
  PSDlayer **lyr_a = NULL;
  guint32 block_len;
  guint32 block_rem;
  gint32 read_len;
  ...
  if (!img_a->merged_image_only && img_a->num_layers) {
    ...
    for (lidx = 0; lidx < img_a->num_layers; ++lidx) {
      ...
      IFDBG(2)
      g_debug("Layer %d, Coords %d %d %d %d, channels %d, ", lidx,
              lyr_a[lidx]->left, lyr_a[lidx]->top, lyr_a[lidx]->right,
              lyr_a[lidx]->bottom, lyr_a[lidx]->num_channels);

      lyr_a[lidx]->chn_info =
          g_new(ChannelLengthInfo, lyr_a[lidx]->num_channels);
      ...
      for (cidx = 0; cidx < lyr_a[lidx]->num_channels; ++cidx) {
        ...
        IFDBG(3)
        g_debug("Channel ID %d, data len %d",
                lyr_a[lidx]->chn_info[cidx].channel_id,
                lyr_a[lidx]->chn_info[cidx].data_len);
      }
      ...
      if (memcmp(lyr_a[lidx]->mode_key, "8BIM", 4) != 0) {
        IFDBG(1)
        g_debug("Incorrect layer mode signature %.4s", lyr_a[lidx]->mode_key);
        g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                    _("The file is corrupt!"));
        return NULL;
      }
      ...
      g_debug("\n\tLayer mode sig: %.4s\n\tBlend mode: %.4s\n\t"
              "Opacity: %d\n\tClipping: %d\n\tExtra data len: %d\n\t"
              "Alpha lock: %d\n\tVisible: %d\n\tIrrelevant: %d",
              lyr_a[lidx]->mode_key, lyr_a[lidx]->blend_mode,
              lyr_a[lidx]->opacity, lyr_a[lidx]->clipping,
              lyr_a[lidx]->extra_len, lyr_a[lidx]->layer_flags.trans_prot,
      ...
      case 20:
        ...
        break;
      case 36: /* If we have a 36 byte mask record assume second data set is
                  correct */
        if (fread(&lyr_a[lidx]->layer_mask.top, 4, 1, f) < 1 ||
            fread(&lyr_a[lidx]->layer_mask.left, 4, 1, f) < 1 ||
            fread(&lyr_a[lidx]->layer_mask.bottom, 4, 1, f) < 1 ||
      ...
      if ((lyr_a[lidx]->layer_mask.right - lyr_a[lidx]->layer_mask.left) >
          G_MAXINT32 /
              MAX(lyr_a[lidx]->layer_mask.bottom - lyr_a[lidx]->layer_mask.top,
                  1)) {
        g_set_error(
      ...
      IFDBG(2)
      g_debug("Layer mask coords %d %d %d %d", lyr_a[lidx]->layer_mask.left,
              lyr_a[lidx]->layer_mask.top, lyr_a[lidx]->layer_mask.right,
              lyr_a[lidx]->layer_mask.bottom);

      IFDBG(3)
      g_debug("Default mask color, %d, %d", lyr_a[lidx]->layer_mask.def_color,
      ...
      lyr_a[lidx]->name =
          fread_pascal_string(&read_len, &write_len, 4, f, error);
      if (*error)
        return NULL;
    ...
    IFDBG(1) g_debug("Layer image data block size %d", img_a->layer_data_len);
  }
...
static gint read_merged_image_block(PSDimage *img_a, FILE *f, GError **error) {
  ...
  IFDBG(1)
  g_debug("Merged image data block: Start: %d, len: %d",
...
static gint add_image_resources(GimpImage *image, PSDimage *img_a, FILE *f,
  ...
  while (ftell(f) < img_a->image_res_start + img_a->image_res_len) {
    ...
    if (load_image_resource(&res_a, image, img_a, f, resolution_loaded,
                            profile_loaded, error) < 0)
      return -1;
  }
...
static guchar *psd_convert_cmyk_to_srgb(PSDimage *img_a, guchar *dst,
                                        guchar *src, gint width, gint height,
                                        gboolean alpha) {
  if (img_a->cmyk_profile) {
    if (alpha) {
      if (!img_a->cmyk_transform_alpha) {
        ...
        img_a->cmyk_transform_alpha = gimp_color_transform_new(
            img_a->cmyk_profile, babl_format("cmykA u8"), srgb,
            babl_format("R'G'B'A float"), 0, 0);

        g_object_unref(srgb);
      }

      gimp_color_transform_process_pixels(
          img_a->cmyk_transform_alpha, babl_format("cmykA u8"), src,
          babl_format("R'G'B'A float"), dst, width * height);
    } else {
      if (!img_a->cmyk_transform) {
        GimpColorProfile *srgb = gimp_color_profile_new_rgb_srgb();

        img_a->cmyk_transform = gimp_color_transform_new(
            img_a->cmyk_profile, babl_format("cmyk u8"), srgb,
            babl_format("R'G'B' float"), 0, 0);

        g_object_unref(srgb);
      }

      gimp_color_transform_process_pixels(
          img_a->cmyk_transform_alpha, babl_format("cmyk u8"), src,
          babl_format("R'G'B' float"), dst, width * height);
    }
  } else {
    const Babl *fish;
...
static gint add_layers(GimpImage *image, PSDimage *img_a, PSDlayer **lyr_a,
  ...
  for (lidx = 0; lidx < img_a->num_layers; ++lidx) {
    ...
    if (lyr_a[lidx]->drop) {
      ...
      IFDBG(3)
      g_debug("Empty mask %d, size %d %d", empty_mask,
              lyr_a[lidx]->layer_mask.bottom - lyr_a[lidx]->layer_mask.top,
              lyr_a[lidx]->layer_mask.right - lyr_a[lidx]->layer_mask.left);
      ...
      for (cidx = 0; cidx < lyr_a[lidx]->num_channels; ++cidx) {
        ...
        IFDBG(3)
        g_debug("Channel id %d, %dx%d", lyr_chn[cidx]->id,
        ...
        if (lyr_a[lidx]->chn_info[cidx].data_len > COMP_MODE_SIZE) {
          ...
          case PSD_COMP_RAW: /* Planar raw data */
            IFDBG(3)
            g_debug("Raw data length: %d",
                    lyr_a[lidx]->chn_info[cidx].data_len - 2);
            if (read_channel_data(lyr_chn[cidx], img_a->bps, PSD_COMP_RAW, NULL,
                                  f, 0, error) < 1)
              return -1;
            break;
          ...
          case PSD_COMP_RLE: /* Packbits */
            IFDBG(3)
            g_debug("RLE channel length %d, RLE length data: %d, "
                    "RLE data block: %d",
                    lyr_a[lidx]->chn_info[cidx].data_len - 2,
                    lyr_chn[cidx]->rows * 2,
                    (lyr_a[lidx]->chn_info[cidx].data_len - 2 -
            ...
            if (read_channel_data(lyr_chn[cidx], img_a->bps, PSD_COMP_RLE,
                                  rle_pack_len, f, 0, error) < 1)
              return -1;
          ...
          case PSD_COMP_ZIP_PRED:
            if (read_channel_data(lyr_chn[cidx], img_a->bps, comp_mode, NULL, f,
                                  lyr_a[lidx]->chn_info[cidx].data_len - 2,
                                  error) < 1)
              return -1;
            break;
      ...
      if (lyr_a[lidx]->group_type != 0) {
        ...
        layer = gimp_layer_new(image, lyr_a[lidx]->name, l_w, l_h, image_type,
      ...
      if (layer != NULL) {
        ...
        if (lyr_a[lidx]->group_type != 3) {
          ...
          /* Opacity */
          gimp_layer_set_opacity(layer, lyr_a[lidx]->opacity * 100.0 / 255.0);
        ...
        if (lyr_a[lidx]->group_type == 0) {
          ...
          if (empty) {
            ...
            while (gegl_buffer_iterator_next(iter)) {
              ...
              if (img_a->color_mode == PSD_CMYK) {
                dst0 = gegl_scratch_alloc(layer_channels * iter->length);
              }
              ...
              for (cidx = 0; cidx < layer_channels; ++cidx) {
                ...
                for (b = 0; b < bps; ++b) {
                  ...
                  for (y = 0; y < roi->height; y++) {
                    ...
                    src = (const guint8 *)&lyr_chn[channel_idx[cidx]]
                              ->data[((roi->y + y) * l_w + roi->x) * bps + b];
              ...
              if (img_a->color_mode == PSD_CMYK) {
                psd_convert_cmyk_to_srgb(img_a, iter->items[0].data, dst0,
                                         roi->width, roi->height, alpha);
        ...
        if (user_mask && lyr_a[lidx]->group_type != 3) {
          if (empty_mask) {
            ...
            if (lyr_a[lidx]->layer_mask.def_color == 255)
              ...
              mask = gimp_layer_create_mask(layer, GIMP_ADD_MASK_BLACK);
            gimp_layer_add_mask(layer, mask);
            gimp_layer_set_apply_mask(
                layer, !lyr_a[lidx]->layer_mask.mask_flags.disabled);
          } else {
            ...
            if (gegl_rectangle_intersect(
              ...
              IFDBG(3)
              g_debug("New layer mask %d", gimp_item_get_id(GIMP_ITEM(mask)));
              gimp_layer_add_mask(layer, mask);
              buffer = gimp_drawable_get_buffer(GIMP_DRAWABLE(mask));
              gegl_buffer_set(
                  buffer, &mask_rect, 0, get_mask_format(img_a),
                  lyr_chn[user_mask_chn]->data +
                      ((mask_rect.y - lm_y) * lm_w + (mask_rect.x - lm_x)) *
                          bps,
                  lm_w * bps);
              g_object_unref(buffer);
              gimp_layer_set_apply_mask(
...
static gint add_merged_image(GimpImage *image, PSDimage *img_a, FILE *f,
  ...
  if ((img_a->color_mode == PSD_BITMAP ||
       ...
       img_a->color_mode == PSD_GRAYSCALE || img_a->color_mode == PSD_DUOTONE ||
  ...
  if (img_a->merged_image_only || img_a->num_layers == 0 ||
    ...
    case PSD_COMP_RAW: /* Planar raw data */
      ...
      for (cidx = 0; cidx < total_channels; ++cidx) {
        ...
        if (read_channel_data(&chn_a[cidx], img_a->bps, PSD_COMP_RAW, NULL, f,
                              0, error) < 1)
          return -1;
      }
      break;
    ...
    case PSD_COMP_RLE: /* Packbits */
      ...
      IFDBG(3)
      g_debug("RLE length data: %d, RLE data block: %d",
      ...
      IFDBG(3) g_debug("RLE decode - data");
      for (cidx = 0; cidx < total_channels; ++cidx) {
        if (read_channel_data(&chn_a[cidx], img_a->bps, PSD_COMP_RLE,
                              rle_pack_len[cidx], f, 0, error) < 1)
          return -1;
      ...
      img_a->num_layers == 0) /* Merged image - Photoshop 2 style */
  {
    ...
    layer = gimp_layer_new(image, _("Background"), img_a->columns, img_a->rows,
                           image_type, 100,
    ...
    if (img_a->transparency) {
      ...
      iter = gegl_buffer_iterator_new(
          buffer, NULL, 0, babl_format("R'G'B'A float"), GEGL_ACCESS_READWRITE,
          GEGL_ABYSS_NONE, 1);
  ...
  if (extra_channels /* Extra alpha channels */
    ...
    for (i = 0; i < extra_channels; ++i) {
      ...
      memcpy(pixels, chn_a[cidx].data,
             chn_a[cidx].columns * chn_a[cidx].rows * bps);
      channel = gimp_channel_new(image, alpha_name, chn_a[cidx].columns,
                                 chn_a[cidx].rows, alpha_opacity, &alpha_rgb);
      ...
      gegl_buffer_set(buffer,
                      GEGL_RECTANGLE(0, 0, gegl_buffer_get_width(buffer),
                                     gegl_buffer_get_height(buffer)),
                      0, get_channel_format(img_a), pixels,
                      GEGL_AUTO_ROWSTRIDE);
      g_object_unref(buffer);
      g_free(chn_a[cidx].data);
    }
...
static gchar *get_psd_color_mode_name(PSDColorMode mode) {
  static gchar *const psd_color_mode_names[] = {
      ...
      "UNKNOWN (5)", "UNKNOWN (6)", "MULTICHANNEL", "DUOTONE", "LAB"};
...
static voidpf zzalloc(voidpf opaque, uInt items, uInt size) {
  /* overflow check missing */
  return g_malloc(items * size);
}

static void zzfree(voidpf opaque, voidpf address) { g_free(address); }
...
static gint read_channel_data(PSDchannel *channel, guint16 bps,
  ...
  IFDBG(3)
  g_debug("raw data size %d x %d = %d", readline_len, channel->rows,
          readline_len * channel->rows);
  ...
  case PSD_COMP_RLE:
    for (i = 0; i < channel->rows; ++i) {
      ...
      decode_packbits(src, raw_data + i * readline_len, rle_pack_len[i],
                      readline_len);
      gegl_scratch_free(src);
    }
    break;
  ...
  case PSD_COMP_ZIP_PRED: {
    ...
    if (inflateInit(&zs) == Z_OK && inflate(&zs, Z_FINISH) == Z_STREAM_END) {
      inflateEnd(&zs);
    } else {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
  ...
  case 8:
    ...
    if (compression == PSD_COMP_ZIP_PRED) {
      for (i = 0; i < channel->rows; ++i)
        for (j = 1; j < channel->columns; ++j)
          channel->data[i * channel->columns + j] +=
              channel->data[i * channel->columns + j - 1];
    }
    break;
...
static void convert_1_bit(const gchar *src, gchar *dst, guint32 rows,
  ...
  for (i = 0; i < rows * ((columns + 7) / 8); ++i) {
    ...
    for (j = 0; j < 8 && row_pos < columns; ++j) {
      *dst = (*src & mask) ? 0 : 1;
      IFDBG(3)
      g_debug("byte %d, bit %d, offset %d, src %d, dst %d", i, j, row_pos, *src,
              *dst);
      dst++;
      mask >>= 1;
      row_pos++;
...
static const Babl *get_layer_format(PSDimage *img_a, gboolean alpha) {
  ...
  case GIMP_RGB_IMAGE:
    ...
    case 8:
    case 1:
      format = babl_format(img_a->color_mode == PSD_CMYK ? "R'G'B' float"
                                                         : "R'G'B' u8");
  ...
  case GIMP_RGBA_IMAGE:
    ...
    case 8:
    case 1:
      format = babl_format(img_a->color_mode == PSD_CMYK ? "R'G'B'A float"
                                                         : "R'G'B'A u8");
