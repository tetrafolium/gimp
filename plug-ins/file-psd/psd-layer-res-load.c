 ...
 * Adjustment layer IDs *
   PSD_LADJ_LEVEL          "levl"    Drop Layer  * Adjustment layer - levels
 (PS4) * PSD_LADJ_CURVE          "curv"    Drop Layer  * Adjustment layer -
 curves (PS4) * PSD_LADJ_BRIGHTNESS     "brit"    Drop Layer  * Adjustment layer
 - brightness contrast (PS4) * PSD_LADJ_BALANCE        "blnc"    Drop Layer  *
 Adjustment layer - color balance (PS4) * PSD_LADJ_BLACK_WHITE    "blwh"    Drop
 Layer  * Adjustment layer - black & white (PS10) * PSD_LADJ_HUE            "hue
 "    Drop Layer  * Adjustment layer - old hue saturation (PS4) * PSD_LADJ_HUE2
 "hue2"    Drop Layer  * Adjustment layer - hue saturation (PS5) *
   PSD_LADJ_SELECTIVE      "selc"    Drop Layer  * Adjustment layer - selective
 color (PS4) * PSD_LADJ_MIXER          "mixr"    Drop Layer  * Adjustment layer
 - channel mixer (PS9) * PSD_LADJ_GRAD_MAP       "grdm"    Drop Layer  *
 Adjustment layer - gradient map (PS9) * PSD_LADJ_PHOTO_FILT     "phfl"    Drop
 Layer  * Adjustment layer - photo filter (PS9) * PSD_LADJ_EXPOSURE       "expA"
 Drop Layer  * Adjustment layer - exposure (PS10) * PSD_LADJ_INVERT "nvrt" Drop
 Layer  * Adjustment layer - invert (PS4) * PSD_LADJ_THRESHOLD      "thrs" Drop
 Layer  * Adjustment layer - threshold (PS4) * PSD_LADJ_POSTERIZE      "post"
 Drop Layer  * Adjustment layer - posterize (PS4) * PSD_LADJ_VIBRANCE "vibA" -
 * Adjustment layer - vibrance (PS10) * PSD_LADJ_COLOR_LOOKUP   "clrL"        -
 * Adjustment layer - color lookup (PS13) *

 * Fill Layer IDs *
   PSD_LFIL_SOLID          "SoCo"        -       * Solid color sheet setting
 (PS6) * PSD_LFIL_PATTERN        "PtFl"        -       * Pattern fill setting
 (PS6) * PSD_LFIL_GRADIENT       "GdFl"        -       * Gradient fill setting
 (PS6) *

 * Effects Layer IDs *
   PSD_LFX_FX              "lrFX"        -       * Effects layer info (PS5) *
   PSD_LFX_FX2             "lfx2"        -       * Object based effects layer
 info (PS6) *

 * Type Tool Layers *
   PSD_LTYP_TYPE           "tySh"        -       * Type tool layer (PS5) *
   PSD_LTYP_TYPE2          "TySh"        -       * Type tool object setting
 (PS6) *

 * Layer Properties *
   PSD_LPRP_UNICODE        "luni"     Loaded     * Unicode layer name (PS5) *
   PSD_LPRP_SOURCE         "lnsr"     Loaded     * Layer name source setting
 (PS6) * PSD_LPRP_ID             "lyid"     Loaded     * Layer ID (PS5) *
 ...
 * Other *
   ...
   PSD_LOTH_SECTION        "lsct"     Loaded     * Section divider setting (PS6)
 (Layer Groups) * PSD_LOTH_SECTION2       "lsdk"     Loaded     * Nested section
 divider setting (CS5) (Layer Groups) * PSD_LOTH_RESTRICT       "brst"        -
 * Channel blending restriction setting (PS6) * PSD_LOTH_FOREIGN_FX     "ffxi"
 -       * Foreign effect ID (PS6) * PSD_LOTH_PATT_DATA      "shpa"        -
 * Pattern data (PS6) * PSD_LOTH_META_DATA      "shmd"        -       * Meta
 data setting (PS6) * PSD_LOTH_LAYER_DATA     "layr"        -       * Layer data
 (PS6) * PSD_LOTH_CONTENT_GEN    "CgEd"        -       * Content generator extra
 data (PS12) * PSD_LOTH_TEXT_ENGINE    "Txt2"        -       * Text engine data
 (PS10) * PSD_LOTH_PATH_NAME      "pths"        -       * Unicode path name
 (PS13) * PSD_LOTH_ANIMATION_FX   "anFX"        -       * Animation effects
 (PS13) * PSD_LOTH_FILTER_MASK    "FMsk"        -       * Filter mask (PS10) *
 ...
 * Effects layer resource IDs *
   PSD_LFX_COMMON          "cmnS"        -       * Effects layer - common state
 (PS5) * PSD_LFX_DROP_SDW        "dsdw"        -       * Effects layer - drop
 shadow (PS5) * PSD_LFX_INNER_SDW       "isdw"        -       * Effects layer -
 inner shadow (PS5) * PSD_LFX_OUTER_GLW       "oglw"        -       * Effects
 layer - outer glow (PS5) * PSD_LFX_INNER_GLW       "iglw"        -       *
 Effects layer - inner glow (PS5) * PSD_LFX_BEVEL           "bevl"        -
 * Effects layer - bevel (PS5) *
 ...
 * Merged Transparency *
   PSD_LMT_MERGE_TRANS      "Mtrn"        -       * Merged transparency save
 flag (?) * PSD_LMT_MERGE_TRANS_16   "Mt16"        -       * Merged transparency
 save flag 2 * PSD_LMT_MERGE_TRANS_32   "Mt32"        -       * Merged
 transparency save flag 3 *
...
static gint load_resource_unknown(const PSDlayerres *res_a, PSDlayer *lyr_a,
                                  FILE *f, GError **error);

static gint load_resource_ladj(const PSDlayerres *res_a, PSDlayer *lyr_a,
                               FILE *f, GError **error);

static gint load_resource_lfil(const PSDlayerres *res_a, PSDlayer *lyr_a,
                               FILE *f, GError **error);

static gint load_resource_lfx(const PSDlayerres *res_a, PSDlayer *lyr_a,
                              FILE *f, GError **error);

static gint load_resource_ltyp(const PSDlayerres *res_a, PSDlayer *lyr_a,
                               FILE *f, GError **error);

static gint load_resource_luni(const PSDlayerres *res_a, PSDlayer *lyr_a,
                               FILE *f, GError **error);

static gint load_resource_lyid(const PSDlayerres *res_a, PSDlayer *lyr_a,
                               FILE *f, GError **error);

static gint load_resource_lclr(const PSDlayerres *res_a, PSDlayer *lyr_a,
                               FILE *f, GError **error);

static gint load_resource_lsct(const PSDlayerres *res_a, PSDlayer *lyr_a,
                               FILE *f, GError **error);

static gint load_resource_lrfx(const PSDlayerres *res_a, PSDlayer *lyr_a,
                               FILE *f, GError **error);

static gint load_resource_lyvr(const PSDlayerres *res_a, PSDlayer *lyr_a,
                               FILE *f, GError **error);

static gint load_resource_lnsr(const PSDlayerres *res_a, PSDlayer *lyr_a,
                               FILE *f, GError **error);

/* Public Functions */
gint get_layer_resource_header(PSDlayerres *res_a, FILE *f, GError **error) {
  if (fread(res_a->sig, 4, 1, f) < 1 || fread(res_a->key, 4, 1, f) < 1 ||
  ...
  IFDBG(2)
  g_debug("Sig: %.4s, key: %.4s, start: %d, len: %d", res_a->sig, res_a->key,
...
static gint load_resource_ladj(const PSDlayerres *res_a, PSDlayer *lyr_a,
  ...
  IFDBG(2)
  g_debug("Process layer resource block %.4s: Adjustment layer", res_a->key);
  lyr_a->drop = TRUE;
...
static gint load_resource_lfx(const PSDlayerres *res_a, PSDlayer *lyr_a,
  ...
  IFDBG(2)
  g_debug("Process layer resource block %.4s: Layer effects", res_a->key);
  if (!msg_flag && CONVERSION_WARNINGS) {
    g_message("Warning:\n"
...
static gint load_resource_ltyp(const PSDlayerres *res_a, PSDlayer *lyr_a,
  ...
  IFDBG(2)
  g_debug("Process layer resource block %.4s: Type tool layer", res_a->key);
  if (!msg_flag && CONVERSION_WARNINGS) {
    g_message("Warning:\n"
  ...
  if (memcmp(res_a->key, PSD_LTYP_TYPE2, 4) == 0) {
    if (fread(&version, 2, 1, f) < 1 || fread(&t_xx, 8, 1, f) < 1 ||
        fread(&t_xy, 8, 1, f) < 1 || fread(&t_yx, 8, 1, f) < 1 ||
        fread(&t_yy, 8, 1, f) < 1 || fread(&t_tx, 8, 1, f) < 1 ||
        fread(&t_ty, 8, 1, f) < 1 || fread(&text_desc_vers, 2, 1, f) < 1 ||
    ...
    IFDBG(2)
    g_debug("Version: %d, Text desc. vers.: %d, Desc. vers.: %d", version,
            text_desc_vers, desc_version);

    IFDBG(2)
    g_debug("Transform\n\txx: %f\n\txy: %f\n\tyx: %f"
            "\n\tyy: %f\n\ttx: %f\n\tty: %f",
            lyr_a->text.xx, lyr_a->text.xy, lyr_a->text.yx, lyr_a->text.yy,
            lyr_a->text.tx, lyr_a->text.ty);
...
static gint load_resource_lclr(const PSDlayerres *res_a, PSDlayer *lyr_a,
  ...
  IFDBG(2)
  g_debug("Process layer resource block %.4s: Sheet color", res_a->key);
...
static gint load_resource_lsct(const PSDlayerres *res_a, PSDlayer *lyr_a,
  ...
  IFDBG(2)
  g_debug("Process layer resource block %.4s: Section divider", res_a->key);
  ...
  if (res_a->data_len >= 12) {
    ...
    if (fread(signature, 4, 1, f) < 1 || fread(blend_mode, 4, 1, f) < 1) {
      psd_set_error(feof(f), errno, error);
      return -1;
    }
    if (memcmp(signature, "8BIM", 4) == 0) {
      ...
      IFDBG(3)
      g_debug("Section divider layer mode sig: %.4s, blend mode: %.4s",
...
static gint load_resource_lrfx(const PSDlayerres *res_a, PSDlayer *lyr_a,
  ...
  if (fread(&version, 2, 1, f) < 1 || fread(&count, 2, 1, f) < 1) {
    psd_set_error(feof(f), errno, error);
    return -1;
  }

  for (i = 0; i < count; i++) {
    if (fread(&signature, 4, 1, f) < 1 || fread(&effectname, 4, 1, f) < 1) {
      psd_set_error(feof(f), errno, error);
      return -1;
    }
    ...
    if (memcmp(signature, "8BIM", 4) != 0) {
      ...
      if (memcmp(effectname, "cmnS", 4) == 0) {
        ...
        if (fread(&size, 4, 1, f) < 1 || fread(&ver, 4, 1, f) < 1 ||
            fread(&visible, 1, 1, f) < 1 || fread(&unused, 2, 1, f) < 1) {
          psd_set_error(feof(f), errno, error);
          return -1;
        }
        ...
        if (fread(&size, 4, 1, f) < 1 || fread(&ver, 4, 1, f) < 1 ||
            fread(&blur, 4, 1, f) < 1 || fread(&intensity, 4, 1, f) < 1 ||
            fread(&angle, 4, 1, f) < 1 || fread(&distance, 4, 1, f) < 1 ||
            fread(&color[0], 2, 1, f) < 1 || fread(&color[1], 2, 1, f) < 1 ||
            fread(&color[2], 2, 1, f) < 1 || fread(&color[3], 2, 1, f) < 1 ||
            fread(&color[4], 2, 1, f) < 1 || fread(&blendsig, 4, 1, f) < 1 ||
            fread(&effect, 4, 1, f) < 1 || fread(&effecton, 1, 1, f) < 1 ||
            fread(&anglefx, 1, 1, f) < 1 || fread(&opacity, 1, 1, f) < 1 ||
        ...
        if (size == 43) {
          if (fread(&invert, 1, 1, f) < 1 || fread(&natcolor[0], 2, 1, f) < 1 ||
              fread(&natcolor[0], 2, 1, f) < 1 ||
              fread(&natcolor[1], 2, 1, f) < 1 ||
              fread(&natcolor[2], 2, 1, f) < 1 ||
              fread(&natcolor[3], 2, 1, f) < 1 ||
        ...
        if (fread(&size, 4, 1, f) < 1 || fread(&ver, 4, 1, f) < 1 ||
            fread(&angle, 4, 1, f) < 1 || fread(&strength, 4, 1, f) < 1 ||
            fread(&blur, 4, 1, f) < 1 || fread(&highlightsig, 4, 1, f) < 1 ||
            ...
            fread(&shadowcolor[4], 2, 1, f) < 1 || fread(&style, 1, 1, f) < 1 ||
            fread(&highlightopacity, 1, 1, f) < 1 ||
            fread(&shadowopacity, 1, 1, f) < 1 ||
            fread(&enabled, 1, 1, f) < 1 || fread(&global, 1, 1, f) < 1 ||
        ...
        if (fread(&size, 4, 1, f) < 1 || fread(&ver, 4, 1, f) < 1 ||
            fread(&key, 4, 1, f) < 1 || fread(&color[0], 2, 1, f) < 1 ||
            fread(&color[1], 2, 1, f) < 1 || fread(&color[2], 2, 1, f) < 1 ||
            fread(&color[3], 2, 1, f) < 1 || fread(&color[4], 2, 1, f) < 1 ||
            fread(&opacity, 1, 1, f) < 1 || fread(&enabled, 1, 1, f) < 1 ||
            fread(&natcolor[0], 2, 1, f) < 1 ||
            fread(&natcolor[1], 2, 1, f) < 1 ||
            fread(&natcolor[2], 2, 1, f) < 1 ||
            fread(&natcolor[3], 2, 1, f) < 1 ||
...
static gint load_resource_lyvr(const PSDlayerres *res_a, PSDlayer *lyr_a,
  ...
  IFDBG(2)
  g_debug("Process layer resource block %.4s: layer version", res_a->key);
...
static gint load_resource_lnsr(const PSDlayerres *res_a, PSDlayer *lyr_a,
  ...
  IFDBG(2)
  g_debug("Process layer resource block %.4s: layer source name", res_a->key);
