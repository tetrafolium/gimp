...
static const gchar *psd_lmode_layer(GimpLayer *layer, gboolean section_divider);

static void reshuffle_cmap_write(guchar *mapGimp);

static void save_header(FILE *fd, GimpImage *image);

static void save_color_mode_data(FILE *fd, GimpImage *image);

static void save_resources(FILE *fd, GimpImage *image);

static void save_layer_and_mask(FILE *fd, GimpImage *image);

static void save_data(FILE *fd, GimpImage *image);

static void xfwrite(FILE *fd, gconstpointer buf, glong len, const gchar *why);

static void write_pascalstring(FILE *fd, const gchar *val, gint padding,
                               const gchar *why);

static void write_string(FILE *fd, const gchar *val, const gchar *why);

static void write_gchar(FILE *fd, guchar val, const gchar *why);

static void write_gint16(FILE *fd, gint16 val, const gchar *why);

static void write_gint32(FILE *fd, gint32 val, const gchar *why);

static void write_datablock_luni(FILE *fd, const gchar *val, const gchar *why);

static void write_pixel_data(FILE *fd, GimpDrawable *drawable,
                             glong *ChanLenPosition, gint32 rowlenOffset,
                             gboolean write_mask);
...
static GList *image_get_all_layers(GimpImage *image, gint *n_layers);
...
static gint32 pack_pb_line(guchar *start, gint32 length, guchar *dest_ptr) {
  ...
  while (remaining > 0) {
    ...
    } else /* Look for characters different from the previous */
    {
      ...
      while ((i < 128) && (remaining - (i + 1) > 0) &&
             (start[i] != start[i + 1] || remaining - (i + 2) <= 0 ||
              start[i] != start[i + 2]))
        i++;
...
static gint gimpBaseTypeToPsdMode(GimpImageBaseType gimpBaseType) {
  switch (gimpBaseType) {
  case GIMP_RGB:
    return 3; /* RGB */
  case GIMP_GRAY:
...
static gint nChansLayer(gint gimpBaseType, gint hasAlpha, gint hasMask) {
  gint incAlpha = 0;
  gint incMask = 0;
...
static void save_header(FILE *fd, GimpImage *image) {
  ...
  write_gint32(
      fd, 0,
      "reserved 1"); /* 6 for the 'reserved' field + 4 bytes for a long */
...
static void save_color_mode_data(FILE *fd, GimpImage *image) {
  ...
  case GIMP_INDEXED:
    ...
    IFDBG printf(
        "\t\tLength of colormap returned by gimp_image_get_colormap: %d\n",
        nColors);
    ...
    if (nColors == 0) {
      ...
      xfwrite(fd, cmap_modified, 768,
              "colormap"); /* Write readjusted colormap */
    ...
    } else /* nColors equals 256 */
    {
      write_gint32(
          fd, 768,
          "color data length"); /* For this type, length is always 768 */
...
static void save_resources(FILE *fd, GimpImage *image) {
  ...
  for (iter = PSDImageData.lLayers, i = 0; iter;
  ...
  if (PSDImageData.nChannels > 0 ||
    ...
    write_gint32(fd, eof_pos - name_sec - sizeof(gint32),
                 "0x03EE resource size");
...
#undef DOUBLE_TO_INT16
  ...
  write_gint32(fd, eof_pos - rsc_pos - sizeof(gint32),
...
static int get_compress_channel_data(guchar *channel_data, gint32 channel_cols,
                                     gint32 channel_rows, gint32 stride,
                                     gint32 bpc, gint16 *LengthsTable,
                                     guchar *remdata) {
  gint i;
  ...
  for (i = 0; i < channel_rows; i++) {
    ...
    LengthsTable[i] = pack_pb_line(start, channel_cols * bpc, &remdata[len]);
    len += LengthsTable[i];
  }
...
static void save_layer_and_mask(FILE *fd, GimpImage *image) {
  ...
  for (iter = g_list_last(PSDImageData.lLayers), i = PSDImageData.nLayers; iter;
       iter = g_list_previous(iter), i--) {
    PSD_Layer *psd_layer = (PSD_Layer *)iter->data;
    gint hasMask = 0;

    if (psd_layer->type == PSD_LAYER_TYPE_LAYER) {
      gimp_drawable_offsets(GIMP_DRAWABLE(psd_layer->layer), &offset_x,
                            &offset_y);
    ...
    IFDBG {
      ...
      case PSD_LAYER_TYPE_LAYER:
        type = "normal layer";
        break;
      case PSD_LAYER_TYPE_GROUP_START:
        type = "group start marker";
        break;
      case PSD_LAYER_TYPE_GROUP_END:
        type = "group end marker";
        break;
      }
    ...
    nChannelsLayer = nChansLayer(
        PSDImageData.baseType,
        gimp_drawable_has_alpha(GIMP_DRAWABLE(psd_layer->layer)), hasMask);
    ...
    layerOpacity =
        RINT((gimp_layer_get_opacity(psd_layer->layer) * 255.0) / 100.0);
    ...
    flags = 0;
    if (gimp_layer_get_lock_alpha(psd_layer->layer))
      flags |= 1;
    if (!gimp_item_get_visible(GIMP_ITEM(psd_layer->layer)))
      flags |= 2;
    if (psd_layer->type != PSD_LAYER_TYPE_LAYER)
      flags |= 0x18;
    ...
    write_gint16(fd,
                 gimp_to_psd_layer_color_tag(
                     gimp_item_get_color_tag(GIMP_ITEM(psd_layer->layer))),
                 "sheet color code");
    ...
    write_gint32(fd, eof_pos - ExtraDataPos - sizeof(gint32),
                 "Extra data size");
  ...
  for (iter = g_list_last(PSDImageData.lLayers), i = PSDImageData.nLayers; iter;
    ...
    gimp_progress_update((PSDImageData.nLayers - i - 1.0) /
                         (PSDImageData.nLayers + 1.0));

    IFDBG printf("\t\tWriting pixel data for layer slot %d\n", i);
    write_pixel_data(fd, GIMP_DRAWABLE(psd_layer->layer), ChannelLengthPos[i],
                     0, psd_layer->type != PSD_LAYER_TYPE_GROUP_END);
  ...
  write_gint32(fd, eof_pos - LayerInfoPos - sizeof(gint32),
  ...
  write_gint32(fd, eof_pos - LayerMaskPos - sizeof(gint32),
...
static void write_pixel_data(FILE *fd, GimpDrawable *drawable,
                             glong *ChanLenPosition, gint32 ltable_offset,
                             gboolean write_mask) {
  ...
  if (gimp_drawable_has_alpha(drawable) && !gimp_drawable_is_indexed(drawable))
    colors -= 1;

  LengthsTable = g_new(gint16, height);
  rledata = g_new(
      guchar, (MIN(height, tile_height) * (width + 10 + (width / 100))) * bpc);
  ...
  for (i = 0; i < components; i++) {
    ...
    if (components != colors &&
        ltable_offset == 0) /* Need to write alpha channel first, except in
                               image data section */
    {
      if (i == 0) {
        chan = components - 1;
    ...
    if (ltable_offset > 0) {
      ...
      xfwrite(fd, LengthsTable, height * sizeof(gint16), "Dummy RLE length");
    ...
    for (y = 0; y < height; y += tile_height) {
      ...
      gegl_buffer_get(buffer,
                      GEGL_RECTANGLE(0, y, width, MIN(height - y, tile_height)),
                      1.0, format, data, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
      tlen = get_compress_channel_data(&data[chan * bpc], width,
                                       MIN(height - y, tile_height), bytes, bpc,
                                       &LengthsTable[y], rledata);
      len += tlen;
  ...
  if (mask != NULL) {
    ...
    for (y = 0; y < height; y += tile_height) {
      ...
      gegl_buffer_get(mbuffer,
                      GEGL_RECTANGLE(0, y, width, MIN(height - y, tile_height)),
                      1.0, mformat, data, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
      tlen = get_compress_channel_data(&data[0], width,
                                       MIN(height - y, tile_height), bpc, bpc,
                                       &LengthsTable[y], rledata);
      len += tlen;
...
static void save_data(FILE *fd, GimpImage *image) {
  ...
  write_pixel_data(fd, GIMP_DRAWABLE(PSDImageData.merged_layer), NULL, offset,
                   FALSE);
  ...
  for (iter = PSDImageData.lChannels; iter; iter = g_list_next(iter)) {
    ...
    write_pixel_data(fd, iter->data, NULL, offset + 2 * imageHeight * chan,
                     FALSE); // check how imgs are channels here
    chan++;
  }
}
...
static GimpLayer *create_merged_image(GimpImage *image) {
  ...
  if (gimp_image_base_type(image) != GIMP_INDEXED) {
    ...
    gimp_layer_flatten(projection); /* PSDs don't support transparency
                                       information in indexed images*/
  }

  return projection;
}

static void get_image_data(GimpImage *image) {
  ...
  PSDImageData.lLayers = image_get_all_layers(image, &PSDImageData.nLayers);
  IFDBG printf("\tGot number of layers: %d\n", PSDImageData.nLayers);
}

static void clear_image_data(void) {
...
gboolean save_image(GFile *file, GimpImage *image, GError **error) {
  ...
  if (gimp_image_width(image) > 30000 || gimp_image_height(image) > 30000) {
...
static GList *append_layers(GList *layers) {
  ...
  for (iter = layers; iter; iter = iter->next) {
    ...
    if (is_group) {
      ...
      psd_layers = g_list_concat(psd_layers, append_layers(group_layers));
