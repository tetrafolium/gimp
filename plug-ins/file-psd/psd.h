...
/* Copied from app/base/gimpimage-quick-mask.h - internal identifier for quick
 * mask channel */
...
#define PSD_LADJ_BRIGHTNESS                                                    \
  "brit" /* Adjustment layer - brightness/contrast (PS4) */
#define PSD_LADJ_BALANCE "blnc" /* Adjustment layer - color balance (PS4) */
#define PSD_LADJ_BLACK_WHITE                                                   \
  "blwh"                     /* Adjustment layer - black & white (PS10) */
...
#define PSD_LADJ_COLOR_LOOKUP                                                  \
  "clrL" /* Adjustment layer - color lookup (PS13) */
...
/* Other */
#define PSD_LOTH_SECTION                                                       \
  "lsct" /* Section divider setting - Layer groups (PS6) */
#define PSD_LOTH_SECTION2                                                      \
  "lsdk" /* Nested section divider setting - Layer groups (CS5) */
...
#define PSD_LOTH_RESTRICT                                                      \
  "brst" /* Channel blending restriction setting (PS6) */
...
typedef enum {
  ...
  PSD_MAC_PRINT_INFO =
      1001, /* 0x03e9 - Optional - Mac print manager print info record */
  ...
  PSD_DISPLAY_INFO = 1007, /* 0x03ef - Superceded by PSD_DISPLAY_INFO_NEW for ps
                              CS3 and higher - DisplayInfo structure */
  ...
  PSD_GREY_HALFTONE =
      1012, /* 0x03f4 - Greyscale and multichannel halftoning info */
  PSD_COLOR_HALFTONE = 1013,   /* 0x03f5 - Color halftoning info */
  PSD_DUOTONE_HALFTONE = 1014, /* 0x03f6 - Duotone halftoning info */
  PSD_GREY_XFER =
      1015, /* 0x03f7 - Greyscale and multichannel transfer functions */
  ...
  PSD_GLOBAL_ANGLE = 1037,   /* 0x040d - Superceded by PSD_NEW_COLOR_SAMPLER for
                                ps CS3 and higher - Global angle */
  PSD_COLOR_SAMPLER = 1038,  /* 0x040e - Superceded by PSD_NEW_COLOR_SAMPLER for
                                ps CS3 and higher - Color samplers resource */
  ...
  PSD_COLOR_SAMPLER_NEW = 1073, /* 0x0431 - Color sampler resource for ps CS3
                                   and higher PSD files */
  ...
  PSD_DISPLAY_INFO_NEW =
      1077, /* 0x0435 - DisplayInfo structure for ps CS3 and higher PSD files */
...
typedef enum {
  ...
  PSD_UNIT_COLUMN =
      5, /* columns ( column defined in ps prefs, default = 2.5 inches) */
} PSDUnit;
...
/* Gray Color Value
   A color value expressed in the Gray color space is composed of a single
   component, gray, represented as a numeric value within the range of 0 to
   10000.
 */
typedef struct {
  guint16 gray;
...
/* Channel display info data for Adobe Photoshop CS3 and higher to support
...
typedef struct {
  ...
  guchar group_type; /* 0 -> not a group; 1 -> open folder; 2 -> closed folder;
                        3 -> end of group */
