...
#define DLG_PREVIEW_MASK GDK_EXPOSURE_MASK | GDK_BUTTON_PRESS_MASK
...
typedef enum { GF_CIRCLE = 0, GF_POLYGON, GF_NUM_SHAPES } GFlareShape;
...
typedef void (*GradientMenuCallback)(const gchar *gradient_name, gpointer data);
...
typedef void (*PreviewRenderFunc)(Preview *preview, guchar *buffer, gint y,
                                  gpointer data);
...
static GFlare *gflare_dup(const GFlare *src, const gchar *new_name);
static void gflare_copy(GFlare *dest, const GFlare *src);
static GFlare *gflare_load(const gchar *filename, const gchar *name);
static void gflare_save(GFlare *gflare);
static void gflare_name_copy(gchar *dest, const gchar *src);
...
static void calc_init_params(GFlare *gflare, gint calc_type, gdouble xcenter,
                             gdouble ycenter, gdouble radius, gdouble rotation,
                             gdouble hue, gdouble vangle, gdouble vlength);
static gint calc_init_progress(void);
static void calc_deinit(void);
static void calc_glow_pix(guchar *dest_pix, gdouble x, gdouble y);
static void calc_rays_pix(guchar *dest_pix, gdouble x, gdouble y);
static void calc_sflare_pix(guchar *dest_pix, gdouble x, gdouble y,
                            guchar *src_pix);
static void calc_gflare_pix(guchar *dest_pix, gdouble x, gdouble y,
                            guchar *src_pix);
...
static Preview *preview_new(gint width, gint height, PreviewInitFunc init_func,
                            gpointer init_data, PreviewRenderFunc render_func,
                            gpointer render_data, PreviewDeinitFunc deinit_func,
                            gpointer deinit_data);
...
static void preview_rgba_to_rgb(guchar *dest, gint x, gint y, guchar *src);
...
static void gradient_name_copy(gchar *dest, const gchar *src);
static void gradient_name_encode(gchar *dest, const gchar *src);
static void gradient_name_decode(gchar *dest, const gchar *src);
static void gradient_init(void);
static void gradient_free(void);
static gchar **gradient_get_list(gint *num_gradients);
static void gradient_get_values(const gchar *gradient_name, guchar *values,
                                gint nvalues);
...
static void plugin_do_asupsample(GeglBuffer *src_buffer,
                                 GeglBuffer *dest_buffer);
static void plugin_render_func(gdouble x, gdouble y, GimpRGB *color,
                               gpointer data);
static void plugin_put_pixel_func(gint ix, gint iy, GimpRGB *color,
                                  gpointer data);
static void plugin_progress_func(gint y1, gint y2, gint curr_y, gpointer data);
...
static void calc_paint_func(guchar *dest, guchar *src1, guchar *src2,
                            gint opacity, GFlareMode mode);
static void calc_combine(guchar *dest, guchar *src1, guchar *src2,
                         gint opacity);
...
static gboolean dlg_preview_handle_event(GtkWidget *widget, GdkEvent *event);
static void dlg_preview_update(void);
static gint dlg_preview_init_func(Preview *preview, gpointer data);
static void dlg_preview_render_func(Preview *preview, guchar *dest, gint y,
                                    gpointer data);
...
static void dlg_selector_new_ok_callback(GtkWidget *widget,
                                         const gchar *new_name, gpointer data);
...
static void dlg_selector_do_delete_callback(GtkWidget *widget, gboolean delete,
                                            gpointer data);

static void ed_run(GtkWindow *parent, GFlare *target_gflare,
                   GFlareEditorCallback callback, gpointer calldata);
static void ed_destroy_callback(GtkWidget *widget, GFlareEditor *ed);
static void ed_response(GtkWidget *widget, gint response_id, GFlareEditor *ed);
...
static void ed_put_gradient_menu(GtkWidget *grid, gint x, gint y,
                                 const gchar *caption, GradientMenu *gm);
...
static void ed_preview_render_func(Preview *preview, guchar *buffer, gint y,
                                   gpointer data);
...
static void gm_gradient_combo_callback(GtkWidget *widget, gpointer data);
static void gm_preview_draw(GtkWidget *preview, const gchar *gradient_name);
static void gm_combo_destroy_callback(GtkWidget *widget, gpointer data);

static void gradient_get_values_internal(const gchar *gradient_name,
                                         guchar *values, gint nvalues);
static void gradient_get_blend(const guchar *fg, const guchar *bg,
                               guchar *values, gint nvalues);
static void gradient_get_random(guchar *values, gint nvalues);
static void gradient_get_default(const gchar *name, guchar *values,
                                 gint nvalues);
...
static void gradient_scale_entry_update_double(GimpLabelSpin *entry,
                                               gdouble *value);
static void gradient_scale_entry_update_int(GimpLabelSpin *entry, gint *value);
...
/* They are not translated since we want gflare files to be compatible
   across languages. */
static const gchar *gflare_modes[] = {"NORMAL", "ADDITION", "OVERLAY",
                                      "SCREEN"};

static const gchar *gflare_shapes[] = {"CIRCLE", "POLYGON"};

/* These are for menu entries, so they are translated. */
static const gchar *gflare_menu_modes[] = {N_("Normal"), N_("Addition"),
                                           N_("Overlay"), N_("Screen")};
...
static const gchar *internal_gradients[] = {"%white",       "%white_grad",
                                            "%red_grad",    "%blue_grad",
                                            "%yellow_grad", "%random"};
...
static void gflare_class_init(GflareClass *klass) {
...
static void gflare_init(Gflare *gflare) {}
...
static GimpProcedure *gflare_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, gflare_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "gflare-path already.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Eiichi Takamori",
                                   "Eiichi Takamori, and a lot of GIMP people",
                                   "1997");

    GIMP_PROC_ARG_STRING(procedure, "gflare-name", "GFlare name",
                         "Name of the GFlare to render", "Default",
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "center-x", "Center X",
                      "X coordinate of center of GFlare", -GIMP_MAX_IMAGE_SIZE,
                      GIMP_MAX_IMAGE_SIZE, 128, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "center-y", "Center Y",
                      "Y coordinate of center of GFlare", -GIMP_MAX_IMAGE_SIZE,
                      GIMP_MAX_IMAGE_SIZE, 128, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "radius", "Radius",
                         "Radius of GFlare (pixel)", 1, GIMP_MAX_IMAGE_SIZE,
                         100, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "rotation", "Rotation",
                         "Rotation of GFlare (degree)", 0, 360, 0,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "hue", "Hue",
                         "Hue rotation of GFlare (degree)", 0, 360, 0,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "vector-angle", "Vector angle",
                         "Vector angle for second flares (degree)", 0, 360, 60,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "vector-length", "Vector length",
                         "Vector length for second flares "
                         "(percentage of Radius)",
                         0, 10000, 400, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "use-asupsample", "Use asupsample",
                          "Use adaptive supersampling while rendering", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "asupsample-max-depth", "Asupsample max depth",
                      "Max depth for adaptive supersampling", 0, 10, 3,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "asupsample-threshold",
                         "Asupsample threshold",
                         "Threshold for adaptive supersampling", 0.0, 1.0, 0.2,
                         G_PARAM_READWRITE);
  }
...
static GimpValueArray *gflare_run(GimpProcedure *procedure,
                                  GimpRunMode run_mode, GimpImage *_image,
  ...
  if (!gimp_drawable_mask_intersect(drawable, &dinfo.x, &dinfo.y, &dinfo.w,
                                    &dinfo.h)) {
  ...
  if (path) {
    ...
    g_message(_("No %s in gimprc:\n"
                ...
                "(%s \"%s\")\n"
                "to your %s file."),
              "gflare-path", "gflare-path", esc_path,
  ...
  case GIMP_RUN_NONINTERACTIVE:
    gflare_name_copy(pvals.gflare_name, GIMP_VALUES_GET_STRING(args, 0));
  ...
  if (gimp_drawable_is_rgb(drawable) || gimp_drawable_is_gray(drawable)) {
    ...
    GError *error =
        g_error_new_literal(0, 0, _("Cannot operate on indexed color images."));
...
static void plugin_do(void) {
  ...
  calc_init_params(gflare, CALC_GLOW | CALC_RAYS | CALC_SFLARE, pvals.xcenter,
                   pvals.ycenter, pvals.radius, pvals.rotation, pvals.hue,
...
static void plugin_do_non_asupsample(GeglBuffer *src_buffer,
  ...
  iter = gegl_buffer_iterator_new(
      src_buffer, GEGL_RECTANGLE(dinfo.x, dinfo.y, dinfo.w, dinfo.h), 0,
      dinfo.format, GEGL_ACCESS_READ, GEGL_ABYSS_NONE, 2);

  gegl_buffer_iterator_add(iter, dest_buffer,
                           GEGL_RECTANGLE(dinfo.x, dinfo.y, dinfo.w, dinfo.h),
                           0, dinfo.format, GEGL_ACCESS_WRITE, GEGL_ABYSS_NONE);
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (row = 0, y = iter->items[0].roi.y; row < iter->items[0].roi.height;
         row++, y++) {
      const guchar *src = src_row;
      guchar *dest = dest_row;
      gint col, x;

      for (col = 0, x = iter->items[0].roi.x; col < iter->items[0].roi.width;
           col++, x++) {
        guchar src_pix[4];
        guchar dest_pix[4];
        gint b;
...
static void plugin_do_asupsample(GeglBuffer *src_buffer,
  ...
  gimp_adaptive_supersample_area(
      dinfo.x, dinfo.y, dinfo.x + dinfo.w - 1, dinfo.y + dinfo.h - 1,
      pvals.asupsample_max_depth, pvals.asupsample_threshold,
      plugin_render_func, src_buffer, plugin_put_pixel_func, dest_buffer,
      plugin_progress_func, NULL);
}
...
static void plugin_put_pixel_func(gint ix, gint iy, GimpRGB *color,
  ...
  gegl_buffer_set(dest_buffer, GEGL_RECTANGLE(ix, iy, 1, 1), 0, dinfo.format,
...
GFlare *gflare_load(const gchar *filename, const gchar *name) {
  FILE *fp;
  GFlareFile *gf;
  GFlare *gflare;
  gchar header[256];
...
static void gflare_read_gradient_name(GradientName name, GFlareFile *gf) {
...
static void gflare_save(GFlare *gflare) {
    ...
    if (gflare_path == NULL) {
      if (!message_ok) {
        ...
        g_message(_("GFlare '%s' is not saved. If you add a new entry "
                  ...
                  gflare->name, gimp_file_get_utf8_name(gimprc), gflare_dir,
  ...
  g_ascii_dtostr(buf[0], G_ASCII_DTOSTR_BUF_SIZE, gflare->glow_opacity);
  fprintf(fp, "%s %s\n", buf[0], gflare_modes[gflare->glow_mode]);
  g_ascii_dtostr(buf[0], G_ASCII_DTOSTR_BUF_SIZE, gflare->rays_opacity);
  fprintf(fp, "%s %s\n", buf[0], gflare_modes[gflare->rays_mode]);
  g_ascii_dtostr(buf[0], G_ASCII_DTOSTR_BUF_SIZE, gflare->sflare_opacity);
  ...
  g_ascii_dtostr(buf[0], G_ASCII_DTOSTR_BUF_SIZE, gflare->glow_size);
  g_ascii_dtostr(buf[1], G_ASCII_DTOSTR_BUF_SIZE, gflare->glow_rotation);
  g_ascii_dtostr(buf[2], G_ASCII_DTOSTR_BUF_SIZE, gflare->glow_hue);
  fprintf(fp, "%s %s %s\n", buf[0], buf[1], buf[2]);
  ...
  g_ascii_dtostr(buf[0], G_ASCII_DTOSTR_BUF_SIZE, gflare->rays_size);
  g_ascii_dtostr(buf[1], G_ASCII_DTOSTR_BUF_SIZE, gflare->rays_rotation);
  g_ascii_dtostr(buf[2], G_ASCII_DTOSTR_BUF_SIZE, gflare->rays_hue);
  fprintf(fp, "%s %s %s\n", buf[0], buf[1], buf[2]);
  g_ascii_dtostr(buf[0], G_ASCII_DTOSTR_BUF_SIZE, gflare->rays_thickness);
  ...
  g_ascii_dtostr(buf[0], G_ASCII_DTOSTR_BUF_SIZE, gflare->sflare_size);
  g_ascii_dtostr(buf[1], G_ASCII_DTOSTR_BUF_SIZE, gflare->sflare_rotation);
  g_ascii_dtostr(buf[2], G_ASCII_DTOSTR_BUF_SIZE, gflare->sflare_hue);
  fprintf(fp, "%s %s %s\n", buf[0], buf[1], buf[2]);
...
static void gflare_write_gradient_name(GradientName name, FILE *fp) {
  gchar enc[1024];
...
static gint gflare_compare(const GFlare *flare1, const GFlare *flare2) {
...
static gint gflares_list_insert(GFlare *gflare) {
  num_gflares++;
  gflares_list =
      g_list_insert_sorted(gflares_list, gflare, (GCompareFunc)gflare_compare);
  return gflares_list_index(gflare);
}

static gint gflare_compare_name(const GFlare *flare, const gchar *name) {
...
static GFlare *gflares_list_lookup(const gchar *name) {
  GList *llink;
  llink =
      g_list_find_custom(gflares_list, name, (GCompareFunc)gflare_compare_name);
...
static int calc_init_progress(void) {
...
static int calc_sample_one_gradient(void) {
  static struct {
    guchar **values;
    gint name_offset;
    gint hue_offset;
    gint gray;
      ...
      {&calc.glow_radial, G_STRUCT_OFFSET(GFlare, glow_radial),
       G_STRUCT_OFFSET(GFlare, glow_hue), FALSE},
      {&calc.glow_angular, G_STRUCT_OFFSET(GFlare, glow_angular), 0, FALSE},
      {&calc.glow_angular_size, G_STRUCT_OFFSET(GFlare, glow_angular_size), 0,
       TRUE},
      {&calc.rays_radial, G_STRUCT_OFFSET(GFlare, rays_radial),
       G_STRUCT_OFFSET(GFlare, rays_hue), FALSE},
      {&calc.rays_angular, G_STRUCT_OFFSET(GFlare, rays_angular), 0, FALSE},
      {&calc.rays_angular_size, G_STRUCT_OFFSET(GFlare, rays_angular_size), 0,
       TRUE},
      {&calc.sflare_radial, G_STRUCT_OFFSET(GFlare, sflare_radial),
       G_STRUCT_OFFSET(GFlare, sflare_hue), FALSE},
      {&calc.sflare_sizefac, G_STRUCT_OFFSET(GFlare, sflare_sizefac), 0, TRUE},
      {&calc.sflare_probability, G_STRUCT_OFFSET(GFlare, sflare_probability), 0,
       TRUE}};
...
static void calc_glow_pix(guchar *dest_pix, gdouble x, gdouble y) {
  ...
  if ((calc.type & CALC_GLOW) == 0 || x < calc.glow_bounds.x0 ||
      x > calc.glow_bounds.x1 || y < calc.glow_bounds.y0 ||
      y > calc.glow_bounds.y1) {
    memset(dest_pix, 0, 4);
    return;
  }
...
static void calc_rays_pix(guchar *dest_pix, gdouble x, gdouble y) {
  ...
  if ((calc.type & CALC_RAYS) == 0 || x < calc.rays_bounds.x0 ||
      x > calc.rays_bounds.x1 || y < calc.rays_bounds.y0 ||
      y > calc.rays_bounds.y1) {
    memset(dest_pix, 0, 4);
    return;
  }
 ...
 *  @JAPANESE
 *  sflare
 * は複数のフレアを順に(レイヤ的に)かぶせながら描画する必要が
 *  あるので、これだけ src_pix を引数にとって paint_func
 * を適用する。 glow, rays は簡易化のためになし。
 */
void calc_sflare_pix(guchar *dest_pix, gdouble x, gdouble y, guchar *src_pix) {
  ...
  while (list) {
    ...
    calc_paint_func(dest_pix, tmp_pix, radial_pix, calc.sflare_opacity,
                    calc.gflare->sflare_mode);
  }
}
...
static void calc_gflare_pix(guchar *dest_pix, gdouble x, gdouble y,
  ...
  if (calc.type & CALC_GLOW) {
    ...
    calc_paint_func(dest_pix, tmp_pix, glow_pix, calc.glow_opacity,
                    gflare->glow_mode);
  }
  if (calc.type & CALC_RAYS) {
    memcpy(tmp_pix, dest_pix, 4);
    calc_rays_pix(rays_pix, x, y);
    calc_paint_func(dest_pix, tmp_pix, rays_pix, calc.rays_opacity,
                    gflare->rays_mode);
  }
...
static gboolean dlg_run(void) {
  ...
  shell = dlg->shell = gimp_dialog_new(_("Gradient Flare"), PLUG_IN_ROLE, NULL,
                                       0, gimp_standard_help_func, PLUG_IN_PROC,
  ...
  gimp_dialog_set_alternative_button_order(GTK_DIALOG(shell), GTK_RESPONSE_OK,
  ...
  dlg->preview = preview_new(
      DLG_PREVIEW_WIDTH, DLG_PREVIEW_HEIGHT, dlg_preview_init_func, NULL,
      dlg_preview_render_func, src_buffer, dlg_preview_deinit_func, NULL);
  ...
  g_signal_connect(dlg->preview->widget, "realize",
                   G_CALLBACK(dlg_preview_realize), NULL);
  g_signal_connect(dlg->preview->widget, "event",
                   G_CALLBACK(dlg_preview_handle_event), NULL);
  ...
  g_signal_connect(button, "toggled", G_CALLBACK(dlg_update_preview_callback),
                   &dlg->update_preview);
...
static gboolean dlg_preview_handle_event(GtkWidget *widget, GdkEvent *event) {
  ...
  case GDK_BUTTON_PRESS:
    ...
    x = dlg->pwin.x0 +
        (double)(dlg->pwin.x1 - dlg->pwin.x0) * bx / DLG_PREVIEW_WIDTH;
    y = dlg->pwin.y0 +
        (double)(dlg->pwin.y1 - dlg->pwin.y0) * by / DLG_PREVIEW_HEIGHT;

    if ((x != pvals.xcenter || y != pvals.ycenter)) {
      if (x != pvals.xcenter) {
        pvals.xcenter = x;
        gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(dlg->sizeentry), 0, x);
      }
      if (y != pvals.ycenter) {
        pvals.ycenter = y;
        gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(dlg->sizeentry), 1, y);
      }
      dlg_preview_update();
    }
    return TRUE;
...
static gint dlg_preview_init_func(Preview *preview, gpointer data) {
  ...
  if (dlg->init_params_done == FALSE) {
    calc_init_params(dlg->gflare, CALC_GLOW | CALC_RAYS | CALC_SFLARE,
                     pvals.xcenter, pvals.ycenter, pvals.radius, pvals.rotation,
                     pvals.hue, pvals.vangle, pvals.vlength);
...
static void dlg_preview_render_func(Preview *preview, guchar *dest, gint y,
  ...
  gegl_buffer_get(src_buffer, GEGL_RECTANGLE(0, dy, width, 1), 1.0,
                  dinfo.format, src_row, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
...
static void dlg_make_page_settings(GFlareDialog *dlg, GtkWidget *notebook) {
  ...
  center = dlg->sizeentry = gimp_coordinates_new(
      gimp_image_get_unit(image), "%a", TRUE, TRUE, 75,
      GIMP_SIZE_ENTRY_UPDATE_SIZE,

      FALSE, FALSE,

      _("_X:"), pvals.xcenter, xres, -GIMP_MAX_IMAGE_SIZE, GIMP_MAX_IMAGE_SIZE,
      0, gimp_drawable_width(drawable),

      _("_Y:"), pvals.ycenter, yres, -GIMP_MAX_IMAGE_SIZE, GIMP_MAX_IMAGE_SIZE,
  ...
  g_signal_connect(center, "value-changed",
                   G_CALLBACK(dlg_position_entry_callback), NULL);
  ...
  scale = gimp_scale_entry_new(_("_Radius:"), pvals.radius, 0.0,
                               GIMP_MAX_IMAGE_SIZE, 1);
  gimp_scale_entry_set_bounds(GIMP_SCALE_ENTRY(scale), 0.0,
                              gimp_drawable_width(drawable) / 2, TRUE);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
                   &pvals.radius);
  g_signal_connect(scale, "value-changed", G_CALLBACK(dlg_preview_update),
                   NULL);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, row++, 3, 1);
  gtk_widget_show(scale);

  scale =
      gimp_scale_entry_new(_("Ro_tation:"), pvals.rotation, -180.0, 180.0, 1);
  ...
  scale =
      gimp_scale_entry_new(_("_Hue rotation:"), pvals.hue, -180.0, 180.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 1.0, 15.0);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double), &pvals.hue);
  g_signal_connect(scale, "value-changed", G_CALLBACK(dlg_preview_update),
                   NULL);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, row++, 3, 1);
  gtk_widget_show(scale);
  ...
  scale =
      gimp_scale_entry_new(_("Vector _angle:"), pvals.vangle, 0.0, 359.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 1.0, 15.0);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
                   &pvals.vangle);
  ...
  scale = gimp_scale_entry_new(_("Vector _length:"), pvals.vlength, 1,
                               GIMP_MAX_IMAGE_SIZE, 1);
  ...
  **/
  ...
  g_object_bind_property(button, "active", asup_grid, "sensitive",
                         G_BINDING_SYNC_CREATE);

  scale = gimp_scale_entry_new(_("_Max depth:"), pvals.asupsample_max_depth,
                               1.0, 10.0, 0);
  g_signal_connect(scale, "value-changed",
  ...
  scale = gimp_scale_entry_new(_("_Threshold"), pvals.asupsample_threshold, 0.0,
                               4.0, 2);
...
static void dlg_position_entry_callback(GtkWidget *widget, gpointer data) {
  ...
  if (pvals.xcenter != x || pvals.ycenter != y) {
    pvals.xcenter = x;
    pvals.ycenter = y;
...
static void dlg_make_page_selector(GFlareDialog *dlg, GtkWidget *notebook) {
  ...
  static struct {
    const gchar *label;
    GCallback callback;
  } buttons[] = {{N_("_New"), G_CALLBACK(dlg_selector_new_callback)},
  ...
  g_signal_connect(dlg->selection, "changed",
                   G_CALLBACK(dlg_selector_list_item_callback), NULL);

  renderer = gtk_cell_renderer_text_new();

  /* Note: the title isn't shown, so it doesn't need to be translated. */
  column = gtk_tree_view_column_new_with_attributes("GFlare", renderer, "text",
                                                    0, NULL);
  ...
  for (i = 0; i < G_N_ELEMENTS(buttons); i++) {
    ...
    g_signal_connect(button, "clicked", buttons[i].callback, button);
  }
...
static void dlg_selector_setup_listbox(void) {
  ...
  while (list) {
    ...
    gtk_list_store_set(dlg->selector_list, &iter, 0, gflare->name, 1, gflare,
                       -1);
...
static void dlg_selector_list_item_callback(GtkTreeSelection *selection) {
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    gtk_tree_model_get(GTK_TREE_MODEL(dlg->selector_list), &iter, 1,
                       &dlg->gflare, -1);
  }
...
static void dlg_selector_new_callback(GtkWidget *widget, gpointer data) {
  ...
  query_box = gimp_query_string_box(
      _("New Gradient Flare"), gtk_widget_get_toplevel(widget),
      gimp_standard_help_func, PLUG_IN_PROC,
      _("Enter a name for the new GFlare"), _("Unnamed"), NULL, NULL,
      dlg_selector_new_ok_callback, dlg, NULL);
...
static void dlg_selector_new_ok_callback(GtkWidget *widget,
  ...
  gtk_list_store_set(dlg->selector_list, &iter, 0, gflare->name, 1, gflare, -1);
  gtk_tree_selection_select_iter(dlg->selection, &iter);
...
static void dlg_selector_edit_callback(GtkWidget *widget, gpointer data) {
  ...
  ed_run(GTK_WINDOW(dlg->shell), dlg->gflare, dlg_selector_edit_done_callback,
         NULL);
}
...
static void dlg_selector_copy_callback(GtkWidget *widget, gpointer data) {
  ...
  query_box = gimp_query_string_box(
      _("Copy Gradient Flare"), gtk_widget_get_toplevel(widget),
      gimp_standard_help_func, PLUG_IN_PROC,
      _("Enter a name for the copied GFlare"), name, NULL, NULL,
      dlg_selector_copy_ok_callback, dlg, NULL);
  g_free(name);
...
static void dlg_selector_delete_callback(GtkWidget *widget, gpointer data) {
  ...
  dialog = gimp_query_boolean_box(
      _("Delete Gradient Flare"), dlg->shell, gimp_standard_help_func,
      PLUG_IN_PROC, GIMP_ICON_DIALOG_QUESTION, str, _("_Delete"), _("_Cancel"),
      NULL, NULL, dlg_selector_do_delete_callback, NULL, NULL);
...
static void ed_run(GtkWindow *parent, GFlare *target_gflare,
   ...
   *    Dialog Shell
   */
  ed->shell = shell = gimp_dialog_new(
      _("Gradient Flare Editor"), PLUG_IN_ROLE, GTK_WIDGET(parent), 0,
      gimp_standard_help_func, PLUG_IN_PROC,

      _("_Rescan Gradients"), RESPONSE_RESCAN, _("_Cancel"),
      GTK_RESPONSE_CANCEL, _("_OK"), GTK_RESPONSE_OK,

      NULL);

  gimp_dialog_set_alternative_button_order(GTK_DIALOG(shell), RESPONSE_RESCAN,
                                           GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL,
                                           -1);

  g_signal_connect(shell, "response", G_CALLBACK(ed_response), ed);
  g_signal_connect(shell, "destroy", G_CALLBACK(ed_destroy_callback), ed);
  ...
  ed->preview = preview_new(ED_PREVIEW_WIDTH, ED_PREVIEW_HEIGHT,
                            ed_preview_init_func, NULL, ed_preview_render_func,
                            NULL, ed_preview_deinit_func, NULL);
...
static void ed_make_page_general(GFlareEditor *ed, GtkWidget *notebook) {
  ...
  scale =
      gimp_scale_entry_new(_("Opacity:"), gflare->glow_opacity, 0.0, 100.0, 1);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
                   &gflare->glow_opacity);
  g_signal_connect(scale, "value-changed", G_CALLBACK(ed_preview_update), NULL);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 0, 3, 1);
  gtk_widget_show(scale);

  combo = ed_mode_menu_new(&gflare->glow_mode);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Paint mode:"), 0.0, 0.5,
                           combo, 1);
  ...
  scale =
      gimp_scale_entry_new(_("Opacity:"), gflare->rays_opacity, 0.0, 100.0, 1);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
                   &gflare->rays_opacity);
  g_signal_connect(scale, "value-changed", G_CALLBACK(ed_preview_update), NULL);
  ...
  scale = gimp_scale_entry_new(_("Opacity:"), gflare->sflare_opacity, 0.0,
                               100.0, 1);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
  ...
  g_signal_connect(vbox, "map", G_CALLBACK(ed_page_map_callback),
...
static void ed_make_page_glow(GFlareEditor *ed, GtkWidget *notebook) {
  ...
  scale =
      gimp_scale_entry_new(_("Size (%):"), gflare->glow_size, 0.0, G_MAXINT, 1);
  ...
  scale = gimp_scale_entry_new(_("Rotation:"), gflare->glow_rotation, -180.0,
                               180.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 1.0, 15.0);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
  ...
  scale = gimp_scale_entry_new(_("Hue rotation:"), gflare->glow_hue, -180.0,
                               180.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 1.0, 15.0);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
...
static void ed_make_page_rays(GFlareEditor *ed, GtkWidget *notebook) {
  ...
  scale =
      gimp_scale_entry_new(_("Size (%):"), gflare->rays_size, 0.0, G_MAXINT, 1);
  gimp_scale_entry_set_bounds(GIMP_SCALE_ENTRY(scale), 0.0, 200.0, TRUE);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
                   &gflare->rays_size);
  ...
  scale = gimp_scale_entry_new(_("Rotation:"), gflare->rays_rotation, -180.0,
                               180.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 1.0, 15.0);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
  ...
  scale = gimp_scale_entry_new(_("Hue rotation:"), gflare->rays_hue, -180.0,
                               180.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 1.0, 15.0);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
  ...
  scale = gimp_scale_entry_new(_("# of Spikes:"), gflare->rays_nspikes, 1,
                               G_MAXINT, 0);
  ...
  scale = gimp_scale_entry_new(_("Spike thickness:"), gflare->rays_thickness,
                               1.0, GIMP_MAX_IMAGE_SIZE, 1);
...
static void ed_make_page_sflare(GFlareEditor *ed, GtkWidget *notebook) {
  ...
  gm =
      gradient_menu_new((GradientMenuCallback)&ed_gradient_menu_callback,
  ...
  scale = gimp_scale_entry_new(_("Size (%):"), gflare->sflare_size, 0.0,
                               G_MAXINT, 1);
  ...
  scale = gimp_scale_entry_new(_("Rotation:"), gflare->sflare_rotation, -180.0,
                               180.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 1.0, 15.0);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
  ...
  scale = gimp_scale_entry_new(_("Hue rotation:"), gflare->sflare_hue, -180.0,
                               180.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 1.0, 15.0);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gradient_scale_entry_update_double),
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(ed_shape_radio_callback),
                   &gflare->sflare_shape);
  ...
  g_signal_connect(entry, "changed", G_CALLBACK(ed_ientry_callback),
                   &gflare->sflare_nverts);
  gtk_box_pack_start(GTK_BOX(polygon_hbox), entry, FALSE, FALSE, 0);
  gtk_widget_show(entry);

  g_object_bind_property(toggle, "active", entry, "sensitive",
                         G_BINDING_SYNC_CREATE);
  ...
  g_signal_connect(GIMP_RANDOM_SEED_SPINBUTTON_ADJ(seed), "value-changed",
                   G_CALLBACK(ed_preview_update), NULL);
...
GtkWidget *ed_mode_menu_new(GFlareMode *mode_var) {
  GtkWidget *combo =
      gimp_int_combo_box_new_array(GF_NUM_MODES, gflare_menu_modes);

  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), *mode_var,
                             G_CALLBACK(ed_mode_menu_callback), mode_var, NULL);
...
static gint ed_preview_init_func(Preview *preview, gpointer data) {
  ...
  if (ed->init_params_done == FALSE) {
    ...
    calc_init_params(ed->gflare, type, ED_PREVIEW_WIDTH / 2,
                     ED_PREVIEW_HEIGHT / 2, ED_PREVIEW_WIDTH / 2, 0.0, 0.0,
                     pvals.vangle, pvals.vlength);
...
static void preview_render_start(Preview *preview) {
  ...
  preview->drawn_y = 0;
  preview->timeout_tag =
      g_timeout_add(100, (GSourceFunc)preview_render_start_2, preview);
}
 ...
 */
static gboolean preview_handle_idle(Preview *preview) {
  gboolean done = FALSE;
  ...
  memcpy(preview->full_image_buffer + preview->width * 3 * preview->current_y,
         preview->buffer, preview->width * 3);

  if (++preview->current_y >= preview->height)
    done = TRUE;

  if (done) {
    gimp_preview_area_draw(GIMP_PREVIEW_AREA(preview->widget), 0, 0,
                           preview->width, preview->height, GIMP_RGB_IMAGE,
                           preview->full_image_buffer, preview->width * 3);
...
static GradientMenu *gradient_menu_new(GradientMenuCallback callback,
  ...
  gtk_widget_set_size_request(gm->preview, GM_PREVIEW_WIDTH, GM_PREVIEW_HEIGHT);

  gm->combo = g_object_new(GIMP_TYPE_INT_COMBO_BOX, NULL);

  g_signal_connect(gm->combo, "changed", G_CALLBACK(gm_gradient_combo_callback),
                   gm);
  ...
  g_signal_connect(gm->combo, "destroy", G_CALLBACK(gm_combo_destroy_callback),
                   gm);
...
static void gm_gradient_combo_fill(GradientMenu *gm,
  ...
  for (i = 0; i < num_gradient_names; i++) {
    ...
    gimp_int_combo_box_append(GIMP_INT_COMBO_BOX(gm->combo),
                              GIMP_INT_STORE_VALUE, i, GIMP_INT_STORE_LABEL,
                              name, -1);
  }
...
static void gm_gradient_combo_callback(GtkWidget *widget, gpointer data) {
  ...
  if (!gimp_int_combo_box_get_active(GIMP_INT_COMBO_BOX(widget), &index) ||
      index < 0 || index >= num_gradient_names)
    return;
...
static void gm_preview_draw(GtkWidget *preview, const gchar *gradient_name) {
  ...
  dest_total_preview_buffer =
      g_new(guchar, GM_PREVIEW_HEIGHT * GM_PREVIEW_WIDTH * 3);
    ...
    for (col = 0; col < GM_PREVIEW_WIDTH; col++) {
      ...
      if (src[alpha] == OPAQUE) {
        ...
        if (src[alpha] == 0) {
          ...
          for (b = 0; b < alpha; b++)
            dest[b] =
                (src[b] * src[alpha] + check * (OPAQUE - src[alpha])) / OPAQUE;
        }
      }
    }

    for (irow = 0; irow < GIMP_CHECK_SIZE_SM && row + irow < GM_PREVIEW_HEIGHT;
         irow++) {
      memcpy(dest_total_preview_buffer + (row + irow) * 3 * GM_PREVIEW_WIDTH,
             dest_row, GM_PREVIEW_WIDTH * 3);
    }
  }

  gimp_preview_area_draw(GIMP_PREVIEW_AREA(preview), 0, 0, GM_PREVIEW_WIDTH,
                         GM_PREVIEW_HEIGHT, GIMP_RGB_IMAGE,
...
static void gm_combo_destroy_callback(GtkWidget *w, gpointer data) {
  GradientMenu *gm = data;
...
static void gradient_free(void) { gradient_cache_flush(); }
