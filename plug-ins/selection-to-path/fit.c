...
static void find_curve_vectors(unsigned, curve_type, unsigned, vector_type *,
                               vector_type *, unsigned *);
static unsigned find_subdivision(curve_type, unsigned initial);
static void find_vectors(unsigned, pixel_outline_type, vector_type *,
                         vector_type *);
...
fitted_splines(pixel_outline_list_type pixel_outline_list) {
  ...
  for (this_list = 0; this_list < SPLINE_LIST_ARRAY_LENGTH(char_splines);
       this_list++)
    total += SPLINE_LIST_LENGTH(SPLINE_LIST_ARRAY_ELT(char_splines, this_list));
...
void fit_set_params(SELVALS *selVals) {
...
void fit_set_default_params(SELVALS *selVals) {
...
static spline_list_type fit_curve_list(curve_list_type curve_list) {
  curve_type curve;
  ...
  for (this_curve = 0; this_curve < curve_list_length; this_curve++) {
    ...
    if (curve_splines == NULL)
      ...
      for (this_spline = 0; this_spline < SPLINE_LIST_LENGTH(*curve_splines);
           this_spline++) {
        /*               LOG1 ("  %u: ", this_spline); */
        /*               if (logging) */
        /*                 print_spline (log_
           file, */
        /*                               SPLINE_LIST_ELT (*curve_splines,
         * this_spline)); */
      }
...
static spline_list_type *fit_curve(curve_type curve) {
  ...
  fitted_splines = CURVE_LENGTH(curve) < 4 ? fit_with_line(curve)
    ...
    if (fabs(corner_angle) <= corner_threshold) {
      ...
      if (best_corner_angle > corner_always_threshold &&
        ...
        for (i = 0; i < INDEX_LIST_LENGTH(equally_good_list); i++)
          APPEND_CORNER(GET_INDEX(equally_good_list, i), best_corner_angle,
                        '@');
        free_index_list(&equally_good_list);
      }
...
static void find_vectors(unsigned test_index, pixel_outline_type outline,
...
static void remove_adjacent_corners(index_list_type *list,
                                    unsigned last_index) {
  unsigned j;
  unsigned last;
...
static void remove_knee_points(curve_type curve, boolean clockwise) {
  int i;
  ...
  for (i = offset; i < CURVE_LENGTH(curve) - offset; i++) {
    coordinate_type current = real_to_int_coord(CURVE_POINT(curve, i));
...
static void filter(curve_type curve) {
  ...
  for (iteration = 0; iteration < filter_iteration_count; iteration++) {
    ...
    for (this_point = offset; this_point < CURVE_LENGTH(curve) - offset;
      ...
      /* Find the angle using the usual number of surrounding points
         on the curve. */
      find_curve_vectors(this_point, curve, filter_surround, &in, &out, &count);
      ...
      if (angle_alt - angle >= filter_epsilon && sum.dx != 0 && sum.dy != 0)
        FILTER_ASSIGN(alt);
...
static void find_curve_vectors(unsigned test_index, curve_type curve,
                               unsigned surround, vector_type *in,
                               vector_type *out, unsigned *count) {
  int i;
  ...
  if (out_count < in_count) {
    ...
    for (i = CURVE_PREV(curve, test_index), n_done = 0;
         i >= 0 && n_done < out_count; i = CURVE_PREV(curve, i), n_done++)
      *in = Vadd(*in, Psubtract(CURVE_POINT(curve, i), candidate));
    in_count = n_done;
  }
...
static real filter_angle(vector_type in, vector_type out) {
...
static spline_list_type *fit_with_least_squares(curve_type curve) {
  ...
  while (true) {
    ...
    iteration++;
    /*       LOG3 ("Reparameterization #%u (error was %.3f, a %u%%
     * improvement):\n", */
...
static spline_type fit_one_spline(curve_type curve) {
  ...
  for (i = 0; i < CURVE_LENGTH(curve); i++) {
    ...
    temp = make_vector(Vsubtract_point(
        CURVE_POINT(curve, i), Vadd(temp0, Vadd(temp1, Vadd(temp2, temp3)))));
  ...
  CONTROL1(spline) =
      Vadd_point(START_POINT(spline), Vmult_scalar(t1_hat, alpha1));
  CONTROL2(spline) =
      Vadd_point(END_POINT(spline), Vmult_scalar(t2_hat, alpha2));
...
static boolean reparameterize(curve_type curve, spline_type S) {
  unsigned p, i;
  ...
  for (p = 0; p < CURVE_LENGTH(curve); p++) {
    ...
    denominator =
        (SQUARE(S1_t.x) + d.x * S2_t.x + SQUARE(S1_t.y) + d.y * S2_t.y);
    ...
    if (new_distance > old_distance) {
      /* 	  REPORT ("!"); */
      /* 	  LOG3 ("  Stopped reparameterizing; %.3f > %.3f at point
       * %u.\n", */
...
static unsigned find_subdivision(curve_type curve, unsigned initial) {
  unsigned i, n_done;
  int best_point = -1;
...
static boolean test_subdivision_point(curve_type curve, unsigned index,
                                      vector_type *best) {
  unsigned count;
  vector_type in, out;
  boolean join = false;
...
static void find_tangent(curve_type curve, boolean to_start_point,
                         boolean cross_curve) {
  vector_type tangent;
...
static vector_type find_half_tangent(curve_type c, boolean to_start_point,
                                     unsigned *n_points) {
  unsigned p;
  ...
  for (p = 1; p <= tangent_surround; p++) {
    ...
    tangent = Vadd(tangent,
                   Vmult_scalar(Psubtract(this_point, tangent_point), factor));
    (*n_points)++;
  }
...
static real find_error(curve_type curve, spline_type spline,
                       unsigned *worst_point) {
  unsigned this_point;
  ...
  *worst_point = CURVE_LENGTH(curve) + 1; /* A sentinel value.  */
  ...
  if (*worst_point ==
      CURVE_LENGTH(curve) +
          1) { /* Didn't have any ``worst point''; the error should be zero.  */
...
static boolean spline_linear_enough(spline_type *spline, curve_type curve) {
  real A, B, C, slope;
  unsigned this_point;
  real distance = 0.0;
  ...
  for (this_point = 0; this_point < CURVE_LENGTH(curve); this_point++) {
    ...
    distance +=
        fabs(A * spline_point.x + B * spline_point.y + C) / sqrt(A * A + B * B);
  }
...
static void change_bad_lines(spline_list_type *spline_list) {
    ...
    for (this_spline = 0; this_spline < length; this_spline++) {
      ...
      if (SPLINE_DEGREE(s) == LINEAR) {
        ...
        if (SPLINE_LINEARITY(s) > line_reversion_threshold) {
          /*                 LOG ("reverted, "); */
          SPLINE_DEGREE(SPLINE_LIST_ELT(*spline_list, this_spline)) = CUBIC;
        }
        /*             LOG1 ("linearity %.3f.\n", SPLINE_LINEARITY (s)); */
      }
    }
  ...
  do {                                                                         \
    ...
    if (!epsilon_equal(delta, 0.0) && delta <= align_threshold) {              \
      ...
      spline_type *prev = &PREV_SPLINE_LIST_ELT(*l, this_spline);              \
                                                                               \
      START_POINT(*s).axis = END_POINT(*s).axis = END_POINT(*prev).axis =      \
          START_POINT(*next).axis = (start.axis + end.axis) / 2;               \
      spline_change = true;                                                    \
    }                                                                          \
  } while (0)

static void align(spline_list_type *l) {
  boolean change;
  unsigned this_spline;
...
static void append_index(index_list_type *list, unsigned new_index) {
  ...
  list->data = (unsigned *)g_realloc(list->data, (INDEX_LIST_LENGTH(*list)) *
                                                     sizeof(unsigned));
