...
curve_type init_curve(coordinate_type coord) {
...
void append_pixel(curve_type curve, coordinate_type coord) {
  append_point(curve, int_to_real_coord(coord));
}

void append_point(curve_type curve, real_coordinate_type coord) {
  CURVE_LENGTH(curve)++;
  curve->point_list =
      g_realloc(curve->point_list, CURVE_LENGTH(curve) * sizeof(point_type));
...
void append_curve(curve_list_type *curve_list, curve_type curve) {
  curve_list->length++;
  curve_list->data =
      g_realloc(curve_list->data, curve_list->length * sizeof(curve_type));
...
void append_curve_list(curve_list_array_type *l, curve_list_type curve_list) {
  CURVE_LIST_ARRAY_LENGTH(*l)++;
  l->data = g_realloc(l->data,
                      (CURVE_LIST_ARRAY_LENGTH(*l)) * sizeof(curve_list_type));
