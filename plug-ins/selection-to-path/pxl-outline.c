...
static pixel_outline_type find_one_outline(edge_type, unsigned, unsigned,
                                           bitmap_type *);
...
static bitmap_type local_new_bitmap(unsigned width, unsigned height) {
  bitmap_type answer;
  ...
  BITMAP_BITS(answer) =
      g_new0(one_byte, size); /* g_new returns NULL if size == 0 */
...
static void local_free_bitmap(bitmap_type *b) {
...
pixel_outline_list_type find_outline_pixels(void) {
...
static pixel_outline_type find_one_outline(edge_type original_edge,
                                           unsigned original_row,
                                           unsigned original_col,
                                           bitmap_type *marked) {
  ...
  do {
    ...
    append_coordinate(&outline, col, sel_get_height() - row, edge);
...
static void append_pixel_outline(pixel_outline_list_type *outline_list,
                                 pixel_outline_type outline) {
  O_LIST_LENGTH(*outline_list)++;
  outline_list->data = (pixel_outline_type *)g_realloc(
      outline_list->data, outline_list->length * sizeof(pixel_outline_type));
...
static void append_outline_pixel(pixel_outline_type *o, coordinate_type c) {
  O_LENGTH(*o)++;
  o->data = (coordinate_type *)g_realloc(o->data, O_LENGTH(*o) *
...
static void append_coordinate(pixel_outline_type *o, int x, int y,
                              edge_type edge) {
  coordinate_type c;
