...
spline_type new_spline(void) {
  real_coordinate_type coord = {-100.0, -100.0};
  spline_type spline;

  START_POINT(spline) = CONTROL1(spline) = CONTROL2(spline) =
      END_POINT(spline) = coord;
...
void print_spline(FILE *f, spline_type s) {
  if (SPLINE_DEGREE(s) == LINEAR)
    fprintf(f, "(%.3f,%.3f)--(%.3f,%.3f).\n", START_POINT(s).x,
            START_POINT(s).y, END_POINT(s).x, END_POINT(s).y);

  else if (SPLINE_DEGREE(s) == CUBIC)
    fprintf(f, "(%.3f,%.3f)..ctrls(%.3f,%.3f)&(%.3f,%.3f)..(%.3f,%.3f).\n",
            START_POINT(s).x, START_POINT(s).y, CONTROL1(s).x, CONTROL1(s).y,
            CONTROL2(s).x, CONTROL2(s).y, END_POINT(s).x, END_POINT(s).y);
...
real_coordinate_type evaluate_spline(spline_type s, real t) {
...
spline_list_type *init_spline_list(spline_type spline) {
  spline_list_type *answer = g_new(spline_list_type, 1);
...
void append_spline(spline_list_type *l, spline_type s) {
  ...
  SPLINE_LIST_DATA(*l) = g_realloc(
      SPLINE_LIST_DATA(*l), SPLINE_LIST_LENGTH(*l) * sizeof(spline_type));
...
void concat_spline_lists(spline_list_type *s1, spline_list_type s2) {
  ...
  SPLINE_LIST_DATA(*s1) =
      g_realloc(SPLINE_LIST_DATA(*s1), new_length * sizeof(spline_type));
...
void free_spline_list_array(spline_list_array_type *spline_list_array) {
  ...
  for (this_list = 0; this_list < SPLINE_LIST_ARRAY_LENGTH(*spline_list_array);
       this_list++)
...
void append_spline_list(spline_list_array_type *l, spline_list_type s) {
  ...
  SPLINE_LIST_ARRAY_DATA(*l) =
      g_realloc(SPLINE_LIST_ARRAY_DATA(*l),
