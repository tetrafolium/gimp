...
boolean epsilon_equal(real v1, real v2) {
  return v1 == v2 /* Usually they'll be exactly equal, anyway.  */
         || fabs(v1 - v2) <= REAL_EPSILON;
}

/* Return the Euclidean distance between P1 and P2.  */

real distance(real_coordinate_type p1, real_coordinate_type p2) {
  return hypot(p1.x - p2.x, p1.y - p2.y);
}

/* Same thing, for integer points.  */
real int_distance(coordinate_type p1, coordinate_type p2) {
...
real slope(real_coordinate_type coord1, real_coordinate_type coord2) {
...
real_coordinate_type int_to_real_coord(coordinate_type int_coord) {
...
coordinate_type real_to_int_coord(real_coordinate_type real_coord) {
...
boolean points_adjacent_p(int row1, int col1, int row2, int col2) {
  int row_diff = abs(row1 - row2);
  int col_diff = abs(col1 - col2);

  return (row_diff == 1 && col_diff == 1) || (row_diff == 0 && col_diff == 1) ||
         (row_diff == 1 && col_diff == 0);
}

/* Find the largest and smallest elements in an array of reals.  */

void find_bounds(real *values, unsigned value_count, real *min, real *max) {
  unsigned this_value;
...
real *map_to_unit(real *values, unsigned value_count) {
