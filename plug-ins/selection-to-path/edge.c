...
#define COMPUTE_DELTA(axis, dir)                                               \
  ((dir) % 2 != 0 ? COMPUTE_##axis##_DELTA((dir)-1) +                          \
...
void next_outline_edge(edge_type *edge, unsigned *row, unsigned *col) {
...
edge_type next_unmarked_outline_edge(unsigned row, unsigned col,
...
static boolean is_outline_edge(edge_type edge, unsigned row, unsigned col) {
  ...
  case right:
    return (col == sel_get_width() - 1) || sel_pixel_is_white(row, col + 1);

  case bottom:
    return (row == sel_get_height() - 1) || sel_pixel_is_white(row + 1, col);
...
void mark_edge(edge_type edge, unsigned row, unsigned col,
               bitmap_type *marked) {
  /* printf("row = %d, col = %d \n",row,col); */
...
static boolean is_marked_edge(edge_type edge, unsigned row, unsigned col,
                              bitmap_type marked) {
  return edge == no_edge ? false : BITMAP_PIXEL(marked, row, col) & (1 << edge);
}
