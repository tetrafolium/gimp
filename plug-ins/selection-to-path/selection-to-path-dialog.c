...
void scale_entry_update_double(GimpLabelSpin *entry, gdouble *value);
...
void reset_adv_dialog(void) {
  ...
  for (list = adjust_widgets; list; list = g_slist_next(list)) {
    ...
    value = (gdouble *)g_object_get_data(G_OBJECT(widget), "default_value");
    ...
    if (GTK_IS_ADJUSTMENT(widget)) {
      ...
      gimp_label_spin_set_value(GIMP_LABEL_SPIN(widget), *value);
    } else
      g_warning("Internal widget list error");
  }
}

static gpointer def_val(gdouble default_value) {
  gdouble *value = g_new0(gdouble, 1);
  *value = default_value;
  return (value);
}

GtkWidget *dialog_create_selection_area(SELVALS *sels) {
  ...
  scale = gimp_scale_entry_new(_("Align Threshold:"), sels->align_threshold,
                               0.2, 2.0, 2);
  ...
  scale = gimp_scale_entry_new(_("Corner Always Threshold:"),
                               sels->corner_always_threshold, 30, 180, 2);
  ...
  scale = gimp_scale_entry_new(_("Corner Surround:"), sels->corner_surround, 3,
                               8, 0);
  gimp_help_set_help_data(
      scale,
  ...
  scale = gimp_scale_entry_new(_("Corner Threshold:"), sels->corner_threshold,
                               0, 180, 2);
  gimp_help_set_help_data(
      scale,
  ...
  scale = gimp_scale_entry_new(_("Error Threshold:"), sels->error_threshold,
                               0.2, 10, 2);
  gimp_help_set_help_data(scale,
  ...
  scale = gimp_scale_entry_new(_("Filter Alternative Surround:"),
                               sels->filter_alternative_surround, 1, 10, 0);
  gimp_help_set_help_data(scale,
  ...
  scale = gimp_scale_entry_new(_("Filter Epsilon:"), sels->filter_epsilon, 5,
                               40, 2);
  gimp_help_set_help_data(scale,
  ...
  scale = gimp_scale_entry_new(_("Filter Iteration Count:"),
                               sels->filter_iteration_count, 4, 70, 0);
  gimp_help_set_help_data(scale,
  ...
  scale =
      gimp_scale_entry_new(_("Filter Percent:"), sels->filter_percent, 0, 1, 2);
  gimp_help_set_help_data(scale,
  ...
  scale = gimp_scale_entry_new(_("Filter Secondary Surround:"),
                               sels->filter_secondary_surround, 3, 10, 0);
  gimp_help_set_help_data(
      scale,
  ...
  scale = gimp_scale_entry_new(_("Filter Surround:"), sels->filter_surround, 2,
                               10, 0);
  gimp_help_set_help_data(
      scale, _("Number of adjacent points to consider when filtering."), NULL);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double),
  ...
  scale = gimp_scale_entry_new(_("Line Reversion Threshold:"),
                               sels->line_reversion_threshold, 0.01, 0.2, 3);
  gimp_help_set_help_data(
      scale,
  ...
  scale = gimp_scale_entry_new(_("Line Threshold:"), sels->line_threshold, 0.2,
                               4, 2);
  gimp_help_set_help_data(scale,
  ...
  scale = gimp_scale_entry_new(_("Reparametrize Improvement:"),
                               sels->reparameterize_improvement, 0, 1, 2);
  gimp_help_set_help_data(
      scale,
      _("If reparameterization doesn't improve the fit by this "
        "much percent, stop doing it. "
  ...
  scale = gimp_scale_entry_new(_("Reparametrize Threshold:"),
                               sels->reparameterize_threshold, 1, 50, 2);
  gimp_help_set_help_data(
      scale,
  ...
  scale = gimp_scale_entry_new(_("Subdivide Search:"), sels->subdivide_search,
                               0.05, 1, 2);
  gimp_help_set_help_data(scale,
  ...
  scale = gimp_scale_entry_new(_("Subdivide Surround:"),
                               sels->subdivide_surround, 2, 10, 0);
  gimp_help_set_help_data(
      scale,
  ...
  scale = gimp_scale_entry_new(_("Subdivide Threshold:"),
                               sels->subdivide_threshold, 0.01, 1, 2);
  gimp_help_set_help_data(
      scale,
  ...
  scale = gimp_scale_entry_new(_("Tangent Surround:"), sels->tangent_surround,
                               2, 10, 0);
  gimp_help_set_help_data(scale,
