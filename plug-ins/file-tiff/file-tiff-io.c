...
static void tiff_io_warning(const gchar *module, const gchar *fmt, va_list ap)
    G_GNUC_PRINTF(2, 0);
static void tiff_io_error(const gchar *module, const gchar *fmt, va_list ap)
    G_GNUC_PRINTF(2, 0);
static tsize_t tiff_io_read(thandle_t handle, tdata_t buffer, tsize_t size);
static tsize_t tiff_io_write(thandle_t handle, tdata_t buffer, tsize_t size);
static toff_t tiff_io_seek(thandle_t handle, toff_t offset, gint whence);
...
static void register_geotags(TIFF *tif) {
  ...
  TIFFMergeFieldInfo(
      tif, geotifftags_fieldinfo,
      (sizeof(geotifftags_fieldinfo) / sizeof(geotifftags_fieldinfo[0])));

  if (parent_extender)
    (*parent_extender)(tif);
}

static TiffIO tiff_io = {
    0,
};

TIFF *tiff_open(GFile *file, const gchar *mode, GError **error) {
  ...
  if (!strcmp(mode, "r")) {
    ...
    tiff_io.output = G_OUTPUT_STREAM(
    ...
    GIOStream *iostream = G_IO_STREAM(g_file_open_readwrite(file, NULL, error));
    if (!iostream)
      return NULL;
...
#warning FIXME !can_seek code is broken
  ...
  return TIFFClientOpen("file-tiff", mode, (thandle_t)&tiff_io, tiff_io_read,
                        tiff_io_write, tiff_io_seek, tiff_io_close,
                        tiff_io_get_file_size, NULL, NULL);
}
...
static void tiff_io_warning(const gchar *module, const gchar *fmt, va_list ap) {
  ...
  if (tag > 0) {
    ...
    return;
             ...
             !strcmp(fmt, "Sum of Photometric type-related color channels and "
                          "ExtraSamples doesn't match SamplesPerPixel."
...
static tsize_t tiff_io_read(thandle_t handle, tdata_t buffer, tsize_t size) {
  ...
  if (io->can_seek) {
    ...
    if (!g_input_stream_read_all(io->input, (void *)buffer, (gsize)size,
    ...
    if (io->position + size > io->used) {
      ...
      if (!g_input_stream_read_all(io->input, (void *)(io->buffer + io->used),
                                   missing, &bytes_read, NULL, &error)) {
        g_printerr("%s", error->message);
        g_clear_error(&error);
      }
...
static tsize_t tiff_io_write(thandle_t handle, tdata_t buffer, tsize_t size) {
  ...
  if (io->can_seek) {
    ...
    if (!g_output_stream_write_all(io->output, (void *)buffer, (gsize)size,
...
static GSeekType lseek_to_seek_type(gint whence) {
  switch (whence) {
  default:
  case SEEK_SET:
    return G_SEEK_SET;
...
static toff_t tiff_io_seek(thandle_t handle, toff_t offset, gint whence) {
  ...
  if (io->can_seek) {
    sought = g_seekable_seek(G_SEEKABLE(io->stream), (goffset)offset,
                             lseek_to_seek_type(whence), NULL, &error);
    if (sought) {
...
static gint tiff_io_close(thandle_t handle) {
  ...
  if (io->input && !io->output) {
    ...
    if (!io->can_seek && io->buffer && io->allocated) {
      if (!g_output_stream_write_all(io->output, (void *)io->buffer,
                                     io->allocated, NULL, NULL, &error)) {
        g_printerr("%s", error->message);
        g_clear_error(&error);
      }
    }
...
static toff_t tiff_io_get_file_size(thandle_t handle) {
  ...
  info = g_file_query_info(io->file, G_FILE_ATTRIBUTE_STANDARD_SIZE,
