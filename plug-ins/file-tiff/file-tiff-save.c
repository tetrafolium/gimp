...
static gboolean save_paths(TIFF *tif, GimpImage *image, gdouble width,
                           gdouble height, gint offset_x, gint offset_y);

static void byte2bit(const guchar *byteline, gint width, guchar *bitline,
                     gboolean invert);

static void save_thumbnail(GimpImage *image, TIFF *tif);

static void double_to_psd_fixed(gdouble value, gchar *target) {
  gdouble in, frac;
  gint i, f;
...
static gboolean save_paths(TIFF *tif, GimpImage *image, gdouble width,
  ...
  for (iter = vectors, v = 0; iter && v < 1000; iter = g_list_next(iter), v++) {
    GString *data;
    gchar *name, *nameend;
    gsize len;
    gint lenpos;
    gchar pointrecord[26] = {
        0,
    };
    gchar *tmpname;
    GError *err = NULL;
    ...
    for (s = 0; s < num_strokes; s++) {
      ...
      type = gimp_vectors_stroke_get_points(iter->data, strokes[s], &num_points,
                                            &points, &closed);

      if (type != GIMP_VECTORS_STROKE_TYPE_BEZIER || num_points > 65535 ||
          num_points % 6) {
      ...
      for (p = 0; p < num_points; p += 6) {
        ...
        double_to_psd_fixed((points[p + 1] - offset_y) / height,
                            pointrecord + 2);
        double_to_psd_fixed((points[p + 0] - offset_x) / width,
                            pointrecord + 6);
        double_to_psd_fixed((points[p + 3] - offset_y) / height,
                            pointrecord + 10);
        double_to_psd_fixed((points[p + 2] - offset_x) / width,
                            pointrecord + 14);
        double_to_psd_fixed((points[p + 5] - offset_y) / height,
                            pointrecord + 18);
        double_to_psd_fixed((points[p + 4] - offset_x) / width,
                            pointrecord + 22);
...
static gboolean save_layer(TIFF *tif, GObject *config, const Babl *space,
                           GimpImage *image, GimpLayer *layer, gint32 page,
                           gint32 num_pages,
                           GimpImage *orig_image, /* the export function might
                                                   * have created a duplicate */
                           gint origin_x, gint origin_y, gint *saved_bpp,
                           gboolean out_linear, GError **error) {
  ...
  g_object_get(config, "compression", &config_compression,
               "save-transparent-pixels", &config_save_transp_pixels,
               "save-geotiff", &config_save_geotiff_tags, NULL);
  ...
  *saved_bpp = bitspersample;
  ...
  case GIMP_RGB_IMAGE:
    ...
    alpha = FALSE;
    if (out_linear) {
      format = babl_format_new(babl_model("RGB"), type, babl_component("R"),
  ...
  case GIMP_RGBA_IMAGE:
    ...
    if (config_save_transp_pixels) {
      if (out_linear) {
        format = babl_format_new(babl_model("RGBA"), type, babl_component("R"),
                                 babl_component("G"), babl_component("B"),
                                 babl_component("A"), NULL);
      } else {
        format =
            babl_format_new(babl_model("R'G'B'A"), type, babl_component("R'"),
                            babl_component("G'"), babl_component("B'"),
                            babl_component("A"), NULL);
      }
    } else {
  ...
  case GIMP_GRAYA_IMAGE:
    ...
    if (config_save_transp_pixels) {
      if (out_linear) {
        format = babl_format_new(babl_model("YA"), type, babl_component("Y"),
                                 babl_component("A"), NULL);
      } else {
        format = babl_format_new(babl_model("Y'A"), type, babl_component("Y'"),
                                 babl_component("A"), NULL);
      }
    } else {
      if (out_linear) {
        format = babl_format_new(babl_model("YaA"), type, babl_component("Ya"),
                                 babl_component("A"), NULL);
      } else {
        format =
            babl_format_new(babl_model("Y'aA"), type, babl_component("Y'a"),
                            babl_component("A"), NULL);
      }
    }
    break;
  ...
  for (y = 0; y < rows; y = yend) {
    ...
    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, y, cols, yend - y), 1.0, format,
    ...
    if ((row % 32) == 0)
      gimp_progress_update(progress_base +
                           progress_fraction * (gdouble)row / (gdouble)rows);
  }
  ...
  if (config_save_geotiff_tags) {
    ...
    parasite =
        gimp_image_get_parasite(image, "Gimp_GeoTIFF_ModelTransformation");
    if (parasite) {
      TIFFSetField(
          tif, GEOTIFF_MODELTRANSFORMATION,
          (gimp_parasite_data_size(parasite) / TIFFDataWidth(TIFF_DOUBLE)),
          gimp_parasite_data(parasite));
      gimp_parasite_free(parasite);
    }
    ...
    if (parasite) {
      TIFFSetField(tif, GEOTIFF_ASCIIPARAMS, gimp_parasite_data(parasite));
      gimp_parasite_free(parasite);
    }
  }
...
#define EXIF_THUMBNAIL_SIZE 256
  ...
  thumb_pixbuf =
      gimp_image_get_thumbnail(image, thumbw, thumbh, GIMP_PIXBUF_KEEP_ALPHA);
 ...
 */
static void save_metadata(GFile *file, GObject *config, GimpImage *image,
                          GimpMetadata *metadata, gint saved_bpp) {
  gchar **exif_tags;
  ...
  g_object_get(config, "save-thumbnail", &save_thumbnail, NULL);

  /* never save metadata thumbnails for TIFF, see bug #729952 */
  g_object_set(config, "save-thumbnail", FALSE, NULL);

  gimp_procedure_config_save_metadata(GIMP_PROCEDURE_CONFIG(config), image,
                                      file);

  g_object_set(config, "save-thumbnail", save_thumbnail, NULL);
}

gboolean save_image(GFile *file, GimpImage *image,
                    GimpImage *orig_image, /* the export function might
                                            * have created a duplicate */
                    GObject *config, GimpMetadata *metadata, GError **error) {
  TIFF *tif = NULL;
  ...
  g_object_get(config, "gimp-comment", &config_comment, "save-color-profile",
               &config_save_profile, "save-thumbnail", &config_save_thumbnail,
               NULL);
  ...
  if (!save_layer(tif, config, space, image,
                  g_list_nth_data(layers, current_layer), current_layer,
                  num_layers, orig_image, origin_x, origin_y, &saved_bpp,
                  out_linear, error)) {
    goto out;
  }
  current_layer++;
  ...
  if (num_layers > 1) {
    ...
    for (; current_layer < num_layers; current_layer++) {
      ...
      if (!save_layer(tif, config, space, image,
                      g_list_nth_data(layers, current_layer), current_layer,
                      num_layers, orig_image, origin_x, origin_y,
...
gboolean save_dialog(GimpImage *image, GimpProcedure *procedure,
                     GObject *config, gboolean has_alpha,
                     gboolean is_monochrome, gboolean is_indexed,
  ...
  store = gimp_int_store_new(
      _("None"), GIMP_COMPRESSION_NONE, _("LZW"), GIMP_COMPRESSION_LZW,
      _("Pack Bits"), GIMP_COMPRESSION_PACKBITS, _("Deflate"),
      GIMP_COMPRESSION_ADOBE_DEFLATE, _("JPEG"), GIMP_COMPRESSION_JPEG,
      _("CCITT Group 3 fax"), GIMP_COMPRESSION_CCITTFAX3,
      _("CCITT Group 4 fax"), GIMP_COMPRESSION_CCITTFAX4, NULL);
  combo = gimp_procedure_dialog_get_int_combo(
      GIMP_PROCEDURE_DIALOG(dialog), "compression", GIMP_INT_STORE(store));
  ...
  gtk_widget_set_sensitive(
      gimp_procedure_dialog_get_widget(GIMP_PROCEDURE_DIALOG(dialog),
                                       "save-transparent-pixels", G_TYPE_NONE),
      has_alpha && !is_indexed);

  gimp_save_procedure_dialog_add_metadata(GIMP_SAVE_PROCEDURE_DIALOG(dialog),
                                          "save-geotiff");

  gimp_procedure_dialog_fill(GIMP_PROCEDURE_DIALOG(dialog), "compression",
                             "layers-frame", "save-transparent-pixels", NULL);

  g_object_get(config, "compression", &compression, NULL);
  ...
  g_object_set(config, "compression", compression, NULL);
