...
static void load_rgba(TIFF *tif, ChannelData *channel);
static void load_contiguous(TIFF *tif, ChannelData *channel, const Babl *type,
                            gushort bps, gushort spp, gboolean is_bw,
                            gboolean is_signed, gint extra);
static void load_separate(TIFF *tif, ChannelData *channel, const Babl *type,
                          gushort bps, gushort spp, gboolean is_bw,
                          gboolean is_signed, gint extra);
static void load_paths(TIFF *tif, GimpImage *image, gint width, gint height,
                       gint offset_x, gint offset_y);

static void fill_bit2byte(void);
static void fill_2bit2byte(void);
static void fill_4bit2byte(void);
static void convert_bit2byte(const guchar *src, guchar *dest, gint width,
                             gint height);
static void convert_2bit2byte(const guchar *src, guchar *dest, gint width,
                              gint height);
static void convert_4bit2byte(const guchar *src, guchar *dest, gint width,
                              gint height);

static void convert_int2uint(guchar *buffer, gint bps, gint spp, gint width,
                             gint height, gint stride);

static gboolean load_dialog(TIFF *tif, const gchar *help_id,
                            TiffSelectedPages *pages,
                            const gchar *extra_message,
                            DefaultExtra *default_extra);

/* returns a pointer into the TIFF */
static const gchar *tiff_get_page_name(TIFF *tif) {
  static gchar *name;
...
GimpPDBStatusType load_image(GFile *file, GimpRunMode run_mode,
  ...
  *image = NULL;
  ...
  gimp_get_data(LOAD_PROC "-keep-empty-space", &pages.keep_empty_space);
  ...
  if (run_mode != GIMP_RUN_INTERACTIVE) {
    ...
    for (li = 0; li < pages.n_pages; li++) {
      ...
      if (extra > 0 && (extra_types[0] == EXTRASAMPLE_UNSPECIFIED)) {
        ...
        break;
      } else if (extra == 0 && ((photomet == PHOTOMETRIC_RGB && spp > 3) ||
                                /* All other color space expect 1 channel
    ...
    if ((pages.n_pages > 1 || extra_message) &&
        !load_dialog(tif, LOAD_PROC, &pages, extra_message, &default_extra)) {
      TIFFClose(tif);
  ...
  gimp_set_data(LOAD_PROC "-target", &pages.target, sizeof(pages.target));
  gimp_set_data(LOAD_PROC "-keep-empty-space", &pages.keep_empty_space,
  ...
  for (li = 0; li < pages.n_pages; li++) {
    ...
    gboolean is_bw; /* FIXME Might need to change the name since we use it for
                       palette too now */
    gboolean is_signed;
    gint i;
    ...
    } else /* extra == 0 */
    {
      if ((photomet == PHOTOMETRIC_RGB && spp > 3) ||
        ...
        if (run_mode != GIMP_RUN_INTERACTIVE)
          g_message(
              "File '%s' does not conform to TIFF specification: "
      ...
      if ((bps == 1 || bps == 2 || bps == 4) && !alpha && spp == 1) {
        ...
        if (alpha) {
          if (save_transp_pixels) {
            if (profile_linear) {
              base_format =
                  babl_format_new(babl_model("YA"), type, babl_component("Y"),
                                  babl_component("A"), NULL);
            } else {
              base_format =
                  babl_format_new(babl_model("Y'A"), type, babl_component("Y'"),
                                  babl_component("A"), NULL);
            }
          } else {
            if (profile_linear) {
              base_format =
                  babl_format_new(babl_model("YaA"), type, babl_component("Ya"),
                                  babl_component("A"), NULL);
            } else {
              base_format = babl_format_new(babl_model("Y'aA"), type,
                                            babl_component("Y'a"),
                                            babl_component("A"), NULL);
            }
          }
          ...
          if (profile_linear) {
            base_format = babl_format_new(babl_model("Y"), type,
                                          babl_component("Y"), NULL);
          } else {
            base_format = babl_format_new(babl_model("Y'"), type,
                                          babl_component("Y'"), NULL);
          }
    ...
    case PHOTOMETRIC_RGB:
      ...
      if (alpha) {
        if (save_transp_pixels) {
          if (profile_linear) {
            base_format =
                babl_format_new(babl_model("RGBA"), type, babl_component("R"),
                                babl_component("G"), babl_component("B"),
                                babl_component("A"), NULL);
          } else {
            base_format = babl_format_new(
                babl_model("R'G'B'A"), type, babl_component("R'"),
                babl_component("G'"), babl_component("B'"), babl_component("A"),
                NULL);
          }
        } else {
          if (profile_linear) {
            base_format = babl_format_new(
                babl_model("RaGaBaA"), type, babl_component("Ra"),
                babl_component("Ga"), babl_component("Ba"), babl_component("A"),
                NULL);
          } else {
            base_format = babl_format_new(
                babl_model("R'aG'aB'aA"), type, babl_component("R'a"),
                babl_component("G'a"), babl_component("B'a"),
                babl_component("A"), NULL);
          }
        }
      } else {
        if (profile_linear) {
          base_format =
              babl_format_new(babl_model("RGB"), type, babl_component("R"),
                              babl_component("G"), babl_component("B"), NULL);
        } else {
          base_format =
              babl_format_new(babl_model("R'G'B'"), type, babl_component("R'"),
                              babl_component("G'"), babl_component("B'"), NULL);
        }
      }
      break;
    ...
    if (worst_case) {
      ...
      if (profile_linear) {
        base_format =
            babl_format_new(babl_model("RaGaBaA"), type, babl_component("Ra"),
                            babl_component("Ga"), babl_component("Ba"),
                            babl_component("A"), NULL);
      } else {
        base_format =
            babl_format_new(babl_model("R'aG'aB'aA"), type,
                            babl_component("R'a"), babl_component("G'a"),
                            babl_component("B'a"), babl_component("A"), NULL);
      }
    }
    ...
    if ((pages.target == GIMP_PAGE_SELECTOR_TARGET_IMAGES) || (!*image)) {
      *image = gimp_image_new_with_precision(cols, rows, image_type,
      ...
      if (pages.target == GIMP_PAGE_SELECTOR_TARGET_IMAGES) {
        gchar *fname = g_strdup_printf("%s-%d", g_file_get_uri(file), ilayer);
        ...
        gchar *fname =
            g_strdup_printf(_("%s-%d-of-%d-pages"), g_file_get_uri(file),
    ...
    /* attach parasites */
    {
      ...
      parasite = gimp_parasite_new("GimpProcedureConfig-file-tiff-save-last",
                                   GIMP_PARASITE_PERSISTENT, string->len + 1,
                                   string->str);
    ...
    /* Attach GeoTIFF Tags as Parasite, If available */
    {
      ...
      if (TIFFGetField(tif, GEOTIFF_MODELPIXELSCALE, &count, &geotag_data)) {
        parasite = gimp_parasite_new(
            "Gimp_GeoTIFF_ModelPixelScale", GIMP_PARASITE_PERSISTENT,
            (TIFFDataWidth(TIFF_DOUBLE) * count), geotag_data);
        gimp_image_attach_parasite(*image, parasite);
        gimp_parasite_free(parasite);
      }
      ...
      if (TIFFGetField(tif, GEOTIFF_MODELTIEPOINT, &count, &geotag_data)) {
        parasite = gimp_parasite_new(
            "Gimp_GeoTIFF_ModelTiePoint", GIMP_PARASITE_PERSISTENT,
            (TIFFDataWidth(TIFF_DOUBLE) * count), geotag_data);
        gimp_image_attach_parasite(*image, parasite);
        gimp_parasite_free(parasite);
      }

      if (TIFFGetField(tif, GEOTIFF_MODELTRANSFORMATION, &count,
                       &geotag_data)) {
        parasite = gimp_parasite_new(
            "Gimp_GeoTIFF_ModelTransformation", GIMP_PARASITE_PERSISTENT,
            (TIFFDataWidth(TIFF_DOUBLE) * count), geotag_data);
        gimp_image_attach_parasite(*image, parasite);
        gimp_parasite_free(parasite);
      }
      ...
      if (TIFFGetField(tif, GEOTIFF_KEYDIRECTORY, &count, &geotag_data)) {
        parasite = gimp_parasite_new(
            "Gimp_GeoTIFF_KeyDirectory", GIMP_PARASITE_PERSISTENT,
            (TIFFDataWidth(TIFF_SHORT) * count), geotag_data);
        gimp_image_attach_parasite(*image, parasite);
        gimp_parasite_free(parasite);
      }
      ...
      if (TIFFGetField(tif, GEOTIFF_DOUBLEPARAMS, &count, &geotag_data)) {
        parasite = gimp_parasite_new(
            "Gimp_GeoTIFF_DoubleParams", GIMP_PARASITE_PERSISTENT,
            (TIFFDataWidth(TIFF_DOUBLE) * count), geotag_data);
        gimp_image_attach_parasite(*image, parasite);
        gimp_parasite_free(parasite);
      }
      ...
      if (TIFFGetField(tif, GEOTIFF_ASCIIPARAMS, &count, &geotag_data)) {
        parasite = gimp_parasite_new(
            "Gimp_GeoTIFF_Asciiparams", GIMP_PARASITE_PERSISTENT,
            (TIFFDataWidth(TIFF_ASCII) * count), geotag_data);
        gimp_image_attach_parasite(*image, parasite);
        gimp_parasite_free(parasite);
      }
    }
    ...
    if (image_type == GIMP_INDEXED) {
      ...
      if (is_bw && photomet != PHOTOMETRIC_PALETTE) {
        ...
        if (!TIFFGetField(tif, TIFFTAG_COLORMAP, &redmap, &greenmap,
                          &bluemap)) {
          TIFFClose(tif);
    ...
    if (name) {
      layer = gimp_layer_new(*image, name, cols, rows, layer_type, 100,
    ...
    if (!base_format && image_type == GIMP_INDEXED) {
      ...
      base_format = babl_format_with_space(
          babl_format_get_encoding(base_format),
    ...
    if (extra > 0 && !worst_case) {
      ...
      for (i = 1; i <= extra; i++) {
        ...
        channel[i].drawable = GIMP_DRAWABLE(gimp_channel_new(
            *image, _("TIFF Channel"), cols, rows, 100.0, &color));
        gimp_image_insert_channel(*image, GIMP_CHANNEL(channel[i].drawable),
                                  NULL, 0);
        ...
        if (image_precision == GIMP_PRECISION_U8_NON_LINEAR)
          channel[i].format = babl_format_new(babl_model("Y'"), type,
                                              babl_component("Y'"), NULL);
        else
          channel[i].format =
              babl_format_new(babl_model("Y"), type, babl_component("Y"), NULL);
      }
    }
    ...
    if (worst_case) {
      ...
      load_contiguous(tif, channel, type, bps, spp, is_bw, is_signed, extra);
    } else {
      load_separate(tif, channel, type, bps, spp, is_bw, is_signed, extra);
    }
    ...
    if (TIFFGetField(tif, TIFFTAG_ORIENTATION, &orientation)) {
      ...
      if (flip_horizontal)
        gimp_item_transform_flip_simple(
            GIMP_ITEM(layer), GIMP_ORIENTATION_HORIZONTAL,
            TRUE /* auto_center */, -1.0 /* axis */);

      if (flip_vertical)
        gimp_item_transform_flip_simple(
            GIMP_ITEM(layer), GIMP_ORIENTATION_VERTICAL, TRUE /* auto_center */,
            -1.0 /* axis */);
    }
    ...
    if (pages.target != GIMP_PAGE_SELECTOR_TARGET_IMAGES) {
      ...
      if (layer_offset_x_pixel > 0 || layer_offset_y_pixel > 0) {
        gimp_layer_set_offsets(layer, layer_offset_x_pixel,
                               layer_offset_y_pixel);
      }
    }
  ...
  if (pages.target == GIMP_PAGE_SELECTOR_TARGET_IMAGES) {
    ...
    if (!(*image)) {
      TIFFClose(tif);
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                  _("No data could be read from TIFF '%s'. The file is "
                    "probably corrupted."),
                  gimp_file_get_utf8_name(file));
    ...
    gimp_image_resize(*image, max_col - min_col, max_row - min_row, -min_col,
                      -min_row);
...
static GimpColorProfile *load_profile(TIFF *tif) {
  ...
  if (TIFFGetField(tif, TIFFTAG_ICCPROFILE, &profile_size, &icc_profile)) {
    profile = gimp_color_profile_new_from_icc_profile(icc_profile, profile_size,
                                                      NULL);
  }
#endif
...
static void load_rgba(TIFF *tif, ChannelData *channel) {
  ...
  for (row = 0; row < image_height; row++) {
    ...
    gegl_buffer_set(channel[0].buffer,
                    GEGL_RECTANGLE(0, image_height - row - 1, image_width, 1),
                    0, channel[0].format,
...
static void load_paths(TIFF *tif, GimpImage *image, gint width, gint height,
  ...
  while (pos < n_bytes) {
    ...
    if (n_bytes - pos < 7 || strncmp(bytes + pos, "8BIM", 4) != 0)
      break;
     ...
     */
    name = bytes + pos + 1;
    if (len >= 3 && name[0] == '\xEF' && name[1] == '\xBB' &&
        name[2] == '\xBF' && g_utf8_validate(name, len, NULL)) {
          ...
          if (pointcount < expected_points) {
            ...
            for (j = 0; j < 6; j++) {
              ...
              f = (double)((gchar)((coord >> 24) & 0xFF)) +
                  (double)(coord & 0x00FFFFFF) / (double)0xFFFFFF;
            ...
            if (pointcount == expected_points) {
              gimp_vectors_stroke_new_from_points(
                  vectors, GIMP_VECTORS_STROKE_TYPE_BEZIER, pointcount * 6,
                  points, closed);
            }
          } else {
  ...
  for (y = 0; y < image_height; y += tile_height) {
    ...
    for (x = 0; x < image_width; x += tile_width) {
      ...
      gimp_progress_update(progress +
                           one_row * ((gdouble)x / (gdouble)image_width));
      ...
      if (TIFFIsTiled(tif))
        ...
        g_message(
            _("Reading scanline failed. Image may be corrupt at line %d."), y);
        g_free(buffer);
        g_free(bw_buffer);
        return;
      }
      ...
      src_buf = gegl_buffer_linear_new_from_data(
          is_bw ? bw_buffer : buffer, src_format,
          GEGL_RECTANGLE(0, 0, cols, rows), tile_width * bytes_per_pixel, NULL,
          NULL);
      ...
      for (i = 0; i <= extra; i++) {
        ...
        iter = gegl_buffer_iterator_new(
            src_buf, GEGL_RECTANGLE(0, 0, cols, rows), 0, NULL,
            GEGL_ACCESS_READ, GEGL_ABYSS_NONE, 2);
        gegl_buffer_iterator_add(
            iter, channel[i].buffer, GEGL_RECTANGLE(x, y, cols, rows), 0,
            channel[i].format, GEGL_ACCESS_WRITE, GEGL_ABYSS_NONE);
      ...
      for (y = 0; y < image_height; y += tile_height) {
        ...
        for (x = 0; x < image_width; x += tile_width) {
          ...
          if (TIFFIsTiled(tif))
            ...
            /* Error reading scanline, stop loading */
            g_message(
                _("Reading scanline failed. Image may be corrupt at line %d."),
                y);
            g_free(buffer);
            g_free(bw_buffer);
            return;
          ...
          src_buf = gegl_buffer_linear_new_from_data(
              is_bw ? bw_buffer : buffer, src_format,
              GEGL_RECTANGLE(0, 0, cols, rows), GEGL_AUTO_ROWSTRIDE, NULL,
              NULL);
          ...
          gegl_buffer_iterator_add(
              iter, channel[i].buffer, GEGL_RECTANGLE(x, y, cols, rows), 0,
              channel[i].format, GEGL_ACCESS_READWRITE, GEGL_ABYSS_NONE);
...
static gboolean load_dialog(TIFF *tif, const gchar *help_id,
  ...
  dialog = gimp_dialog_new(_("Import from TIFF"), PLUG_IN_ROLE, NULL, 0,
  ...
  if (pages->n_pages > 1) {
    ...
    crop_option = gtk_check_button_new_with_mnemonic(
  ...
  if (extra_message) {
    ...
    warning =
        g_object_new(GIMP_TYPE_HINT_BOX, "icon-name", GIMP_ICON_DIALOG_WARNING,
                     "hint", extra_message, NULL);
    gtk_box_pack_start(GTK_BOX(vbox), warning, TRUE, TRUE, 0);
    gtk_widget_show(warning);

    extra_radio = gimp_int_radio_group_new(
        TRUE, _("Process extra channel as:"),
        (GCallback)gimp_radio_button_update, default_extra, NULL,
        GIMP_TIFF_LOAD_UNASSALPHA, _("_Non-premultiplied alpha"),
        GIMP_TIFF_LOAD_UNASSALPHA, NULL, _("Pre_multiplied alpha"),
        GIMP_TIFF_LOAD_ASSOCALPHA, NULL, _("Channe_l"), GIMP_TIFF_LOAD_CHANNEL,
        NULL, NULL);
