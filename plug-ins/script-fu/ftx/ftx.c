...
struct named_constant file_type_constants[] = {
...
pointer foreign_time(scheme *sc, pointer args) {
  ...
  ret = sc->vptr->cons(
      sc, sc->vptr->mk_integer(sc, (long)now_tm->tm_year),
      sc->vptr->cons(
          sc, sc->vptr->mk_integer(sc, (long)now_tm->tm_mon),
          sc->vptr->cons(
              sc, sc->vptr->mk_integer(sc, (long)now_tm->tm_mday),
              sc->vptr->cons(
                  sc, sc->vptr->mk_integer(sc, (long)now_tm->tm_hour),
                  sc->vptr->cons(
                      sc, sc->vptr->mk_integer(sc, (long)now_tm->tm_min),
                      sc->vptr->cons(
                          sc, sc->vptr->mk_integer(sc, (long)now_tm->tm_sec),
                          sc->NIL))))));
...
pointer foreign_gettimeofday(scheme *sc, pointer args) {
  ...
  ret = sc->vptr->cons(
      sc, sc->vptr->mk_integer(sc, (long)time / G_USEC_PER_SEC),
...
void init_ftx(scheme *sc) {
  ...
  sc->vptr->scheme_define(sc, sc->global_env, sc->vptr->mk_symbol(sc, "getenv"),
                          sc->vptr->mk_foreign_func(sc, foreign_getenv));
  sc->vptr->scheme_define(sc, sc->global_env, sc->vptr->mk_symbol(sc, "time"),
  ...
  sc->vptr->scheme_define(sc, sc->global_env, sc->vptr->mk_symbol(sc, "usleep"),
  ...
  sc->vptr->scheme_define(
      sc, sc->global_env, sc->vptr->mk_symbol(sc, "dir-close-stream"),
