...
enum { RESPONSE_CLEAR, RESPONSE_SAVE };

/*
 *  Local Functions
 */
static void script_fu_console_response(GtkWidget *widget, gint response_id,
                                       ConsoleInterface *console);
static void script_fu_console_save_dialog(ConsoleInterface *console);
static void script_fu_console_save_response(GtkWidget *dialog, gint response_id,
                                            ConsoleInterface *console);

static void script_fu_browse_callback(GtkWidget *widget,
                                      ConsoleInterface *console);
static void script_fu_browse_response(GtkWidget *widget, gint response_id,
                                      ConsoleInterface *console);
...
static gboolean script_fu_cc_key_function(GtkWidget *widget, GdkEventKey *event,
                                          ConsoleInterface *console);

static void script_fu_output_to_console(TsOutputType type, const gchar *text,
...
GimpValueArray *script_fu_console_run(GimpProcedure *procedure,
  ...
  console.dialog =
      gimp_dialog_new(_("Script-Fu Console"), "gimp-script-fu-console", NULL, 0,
                      gimp_standard_help_func, PROC_NAME,

                      _("_Save"), RESPONSE_SAVE, _("C_lear"), RESPONSE_CLEAR,
                      _("_Close"), GTK_RESPONSE_CLOSE,

                      NULL);

  gimp_dialog_set_alternative_button_order(GTK_DIALOG(console.dialog),
                                           GTK_RESPONSE_CLOSE, RESPONSE_CLEAR,
                                           RESPONSE_SAVE, -1);
  ...
  g_signal_connect(console.dialog, "response",
                   G_CALLBACK(script_fu_console_response), &console);

  /*  The main vbox  */
  vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 12);
  gtk_container_set_border_width(GTK_CONTAINER(vbox), 12);
  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(console.dialog))), vbox,
      TRUE, TRUE, 0);
  ...
  gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(console.text_view), GTK_WRAP_WORD);
  ...
  gtk_text_buffer_create_tag(console.console, "strong", "weight",
                             PANGO_WEIGHT_BOLD, "scale", PANGO_SCALE_LARGE,
                             NULL);
  gtk_text_buffer_create_tag(console.console, "emphasis", "style",
                             PANGO_STYLE_OBLIQUE, NULL);
  ...
  g_signal_connect(console.cc, "key-press-event",
                   G_CALLBACK(script_fu_cc_key_function), &console);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(script_fu_browse_callback),
                   &console);
...
static void script_fu_console_save_dialog(ConsoleInterface *console) {
  if (!console->save_dialog) {
    console->save_dialog = gtk_file_chooser_dialog_new(
        _("Save Script-Fu Console Output"), GTK_WINDOW(console->dialog),
    ...
    gtk_file_chooser_set_do_overwrite_confirmation(
        GTK_FILE_CHOOSER(console->save_dialog), TRUE);
    ...
    g_signal_connect(console->save_dialog, "response",
                     G_CALLBACK(script_fu_console_save_response), console);
  }
...
static void script_fu_browse_callback(GtkWidget *widget,
  ...
  if (!console->proc_browser) {
    console->proc_browser = gimp_proc_browser_dialog_new(
        _("Script-Fu Procedure Browser"), "script-fu-procedure-browser",
        gimp_standard_help_func, PROC_NAME,

        _("_Apply"), GTK_RESPONSE_APPLY, _("_Close"), GTK_RESPONSE_CLOSE,
    ...
    g_signal_connect(console->proc_browser, "response",
                     G_CALLBACK(script_fu_browse_response), console);
    g_signal_connect(console->proc_browser, "row-activated",
                     G_CALLBACK(script_fu_browse_row_activated), console);
  }
...
static void script_fu_browse_response(GtkWidget *widget, gint response_id,
  ...
  gtk_editable_set_position(
      GTK_EDITABLE(console->cc),
      g_utf8_pointer_to_offset(text->str, text->str + strlen(proc_name) + 2));
...
static gboolean script_fu_console_idle_scroll_end(GtkWidget *view) {
...
static void script_fu_output_to_console(TsOutputType type, const gchar *text,
  ...
  if (console && console->text_view) {
    ...
    if (type == TS_OUTPUT_NORMAL) {
      ...
      gtk_text_buffer_insert_with_tags_by_name(console->console, &cursor, text,
                                               len, "emphasis", NULL);
    }
...
static gboolean script_fu_cc_key_function(GtkWidget *widget, GdkEventKey *event,
  ...
  case GDK_KEY_ISO_Enter:
    ...
    list = g_list_nth(console->history, (g_list_length(console->history) - 1));
    ...
    gtk_text_buffer_insert(console->console, &cursor, "\n", 1);
    gtk_text_buffer_insert_with_tags_by_name(console->console, &cursor, "> ", 2,
                                             "strong", NULL);
    ...
    if (ts_interpret_string(list->data) != 0) {
      script_fu_output_to_console(TS_OUTPUT_ERROR, output->str, output->len,
                                  console);
    } else {
      script_fu_output_to_console(TS_OUTPUT_NORMAL, output->str, output->len,
                                  console);
    }
    ...
    if (console->history_len == console->history_max) {
      console->history =
          g_list_remove(console->history, console->history->data);
  ...
  if (direction) {
    ...
    gtk_entry_set_text(
        GTK_ENTRY(console->cc),
        (gchar *)(g_list_nth(console->history, console->history_cur))->data);
