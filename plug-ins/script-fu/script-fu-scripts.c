...
static gboolean script_fu_run_command(const gchar *command, GError **error);
static void script_fu_load_directory(GFile *directory);
static void script_fu_load_script(GFile *file);
static gboolean script_fu_install_script(gpointer foo, GList *scripts,
                                         gpointer data);
static void script_fu_install_menu(SFMenu *menu);
static gboolean script_fu_remove_script(gpointer foo, GList *scripts,
                                        gpointer data);
...
static gint script_fu_menu_compare(gconstpointer a, gconstpointer b);
...
void script_fu_find_scripts(GimpPlugIn *plug_in, GList *path) {
  GList *list;

  /*  Make sure to clear any existing scripts  */
  if (script_tree != NULL) {
    g_tree_foreach(script_tree, (GTraverseFunc)script_fu_remove_script,
                   plug_in);
  ...
  g_tree_foreach(script_tree, (GTraverseFunc)script_fu_install_script, plug_in);

  script_menu_list =
      g_list_sort(script_menu_list, (GCompareFunc)script_fu_menu_compare);

  /*  Install and nuke the list of menu entries  */
  g_list_free_full(script_menu_list, (GDestroyNotify)script_fu_install_menu);
...
pointer script_fu_add_script(scheme *sc, pointer a) {
  ...
  /*  Create a new script  */
  script = script_fu_script_new(name, menu_label, blurb, author, copyright,
                                date, image_types, n_args);
  ...
  for (i = 0; i < script->n_args; i++) {
    ...
    if (a != sc->NIL) {
      if (!sc->vptr->is_integer(sc->vptr->pair_car(a)))
        return foreign_error(
            sc, "script-fu-register: argument types must be integer values", 0);
    ...
    if (a != sc->NIL) {
      if (!sc->vptr->is_string(sc->vptr->pair_car(a)))
        return foreign_error(
            sc, "script-fu-register: argument labels must be strings", 0);

      arg->label = g_strdup(sc->vptr->string_value(sc->vptr->pair_car(a)));
      a = sc->vptr->pair_cdr(a);
    } else
      return foreign_error(sc, "script-fu-register: missing arguments label",
                           0);
    ...
    if (a != sc->NIL) {
      ...
      case SF_DISPLAY:
        if (!sc->vptr->is_integer(sc->vptr->pair_car(a)))
          return foreign_error(
              sc, "script-fu-register: default IDs must be integer values", 0);

        arg->default_value.sfa_image = sc->vptr->ivalue(sc->vptr->pair_car(a));
        break;
      ...
      case SF_COLOR:
        if (sc->vptr->is_string(sc->vptr->pair_car(a))) {
          if (!gimp_rgb_parse_css(&arg->default_value.sfa_color,
                                  ...
                                  -1))
            return foreign_error(
                sc, "script-fu-register: invalid default color name", 0);
          ...
          return foreign_error(sc,
                               "script-fu-register: color defaults must be a "
                               "list of 3 integers or a color name",
                               0);
        }
        break;
      ...
      case SF_TOGGLE:
        if (!sc->vptr->is_integer(sc->vptr->pair_car(a)))
          return foreign_error(
              sc, "script-fu-register: toggle default must be an integer value",
              0);
      ...
      case SF_VALUE:
        if (!sc->vptr->is_string(sc->vptr->pair_car(a)))
          return foreign_error(
              sc, "script-fu-register: value defaults must be string values",
              0);
      ...
      case SF_TEXT:
        if (!sc->vptr->is_string(sc->vptr->pair_car(a)))
          return foreign_error(
              sc, "script-fu-register: string defaults must be string values",
              0);
      ...
      case SF_ADJUSTMENT: {
        ...
        if (!sc->vptr->is_list(sc, a))
          return foreign_error(
              sc, "script-fu-register: adjustment defaults must be a list", 0);
      ...
      case SF_FILENAME:
        if (!sc->vptr->is_string(sc->vptr->pair_car(a)))
          return foreign_error(
              sc, "script-fu-register: filename defaults must be string values",
              0);
        /* fallthrough */
      ...
      case SF_DIRNAME:
        if (!sc->vptr->is_string(sc->vptr->pair_car(a)))
          return foreign_error(
              sc, "script-fu-register: dirname defaults must be string values",
              0);
      ...
      case SF_FONT:
        if (!sc->vptr->is_string(sc->vptr->pair_car(a)))
          return foreign_error(
              sc, "script-fu-register: font defaults must be string values", 0);
      ...
      case SF_PALETTE:
        if (!sc->vptr->is_string(sc->vptr->pair_car(a)))
          return foreign_error(
              sc, "script-fu-register: palette defaults must be string values",
              0);
      ...
      case SF_PATTERN:
        if (!sc->vptr->is_string(sc->vptr->pair_car(a)))
          return foreign_error(
              sc, "script-fu-register: pattern defaults must be string values",
              0);
      ...
      case SF_BRUSH: {
        ...
        if (!sc->vptr->is_list(sc, a))
          return foreign_error(
              sc, "script-fu-register: brush defaults must be a list", 0);
      ...
      case SF_GRADIENT:
        if (!sc->vptr->is_string(sc->vptr->pair_car(a)))
          return foreign_error(
              sc, "script-fu-register: gradient defaults must be string values",
              0);
      ...
      case SF_OPTION: {
        ...
        if (!sc->vptr->is_list(sc, a))
          return foreign_error(
              sc, "script-fu-register: option defaults must be a list", 0);

        for (option_list = sc->vptr->pair_car(a); option_list != sc->NIL;
      ...
      case SF_ENUM: {
        ...
        if (!sc->vptr->is_list(sc, a))
          return foreign_error(
              sc, "script-fu-register: enum defaults must be a list", 0);

        option_list = sc->vptr->pair_car(a);
        if (!sc->vptr->is_string(sc->vptr->pair_car(option_list)))
          return foreign_error(sc,
                               "script-fu-register: first element in enum "
                               "defaults must be a type-name",
                               0);
        ...
        if (!G_TYPE_IS_ENUM(enum_type)) {
          ...
          return foreign_error(sc,
                               "script-fu-register: first element in enum "
                               "defaults must be the name of a registered type",
                               0);
        }
        ...
        if (!sc->vptr->is_string(sc->vptr->pair_car(option_list)))
          return foreign_error(sc,
                               "script-fu-register: second element in enum "
                               "defaults must be a string",
                               0);

        enum_value = g_enum_get_value_by_nick(
            g_type_class_peek(enum_type),
      ...
      return foreign_error(sc, "script-fu-register: missing default argument",
                           0);
    }
  }
...
pointer script_fu_add_menu(scheme *sc, pointer a) {
  ...
  if (sc->vptr->list_length(sc, a) != 2)
    return foreign_error(
        sc, "Incorrect number of arguments for script-fu-menu-register", 0);
 ...
 */
static gboolean script_fu_install_script(gpointer foo G_GNUC_UNUSED,
                                         GList *scripts, gpointer data) {
  ...
  for (list = scripts; list; list = g_list_next(list)) {
    ...
    script_fu_script_install_proc(plug_in, script, script_fu_script_proc);
  }
...
static void script_fu_install_menu(SFMenu *menu) {
  ...
  procedure = gimp_plug_in_get_temp_procedure(plug_in, menu->script->name);
 ...
 *  The following function is a GTraverseFunction.
 */
static gboolean script_fu_remove_script(gpointer foo G_GNUC_UNUSED,
                                        GList *scripts, gpointer data) {
  GimpPlugIn *plug_in = data;
  GList *list;
...
static gboolean script_fu_lookup_script(gpointer *foo G_GNUC_UNUSED,
                                        GList *scripts, gconstpointer *name) {
  GList *list;
...
static SFScript *script_fu_find_script(const gchar *name) {
  ...
  g_tree_foreach(script_tree, (GTraverseFunc)script_fu_lookup_script, &script);
...
static gint script_fu_menu_compare(gconstpointer a, gconstpointer b) {
  ...
  if (menu_a->menu_path && menu_b->menu_path) {
    ...
    if (retval == 0 && menu_a->script->menu_label &&
