...
static void server_start(const gchar *listen_ip, gint port,
                         const gchar *logfile);
...
static void server_log(const gchar *format, ...) G_GNUC_PRINTF(1, 2);
 ...
 *  Local variables
 */
static gint server_socks[2], server_socks_used = 0;
static const gint server_socks_len =
    sizeof(server_socks) / sizeof(server_socks[0]);
...
void script_fu_server_quit(void) { script_fu_done = TRUE; }

gint script_fu_server_get_mode(void) { return server_mode; }
...
static void script_fu_server_add_fd(gpointer key, gpointer value,
                                    gpointer data) {
  FD_SET(GPOINTER_TO_INT(key), (SELECT_MASK *)data);
}

static gboolean script_fu_server_read_fd(gpointer key, gpointer value,
                                         gpointer data) {
...
void script_fu_server_listen(gint timeout) {
  ...
  for (sockno = 0; sockno < server_socks_used; sockno++) {
    ...
    /* Lookup address */
    (void)getnameinfo(&(client.sa), size, clientname, sizeof(clientname), NULL,
                      0, NI_NUMERICHOST);

    g_hash_table_insert(clients, GINT_TO_POINTER(new), g_strdup(clientname));
...
static void server_progress_start(const gchar *message, gboolean cancelable,
                                  gpointer user_data) {
  /* do nothing */
}

static void server_progress_end(gpointer user_data) { /* do nothing */ }

static void server_progress_set_text(const gchar *message, gpointer user_data) {
  /* do nothing */
}

static void server_progress_set_value(gdouble percentage, gpointer user_data) {
  /* do nothing */
}
 ...
 */
static const gchar *server_progress_install(void) {
...
static void server_progress_uninstall(const gchar *progress) {
...
static void server_start(const gchar *listen_ip, gint port,
  ...
  for (ai_curr = ai, sockno = 0; ai_curr != NULL && sockno < server_socks_len;
  ...
  clients =
      g_hash_table_new_full(g_direct_hash, NULL, NULL, (GDestroyNotify)g_free);
...
static gboolean execute_command(SFCommand *cmd) {
  ...
  for (i = 0; i < RESPONSE_HEADER; i++)
    if (cmd->filedes > 0 &&
        send(cmd->filedes, (const void *)(buffer + i), 1, 0) < 0) {
      /*  Write error  */
...
static gint read_from_client(gint filedes) {
  ...
  server_log("Received request #%d from IP address %s: %s on %s,"
             ...
             cmd->request_no, clientaddr ? clientaddr : "<invalid>",
...
static gint make_socket(const struct addrinfo *ai) {
  ...
  if (ai->ai_family == AF_INET6) {
    v = 1;
    if (setsockopt(sock, IPPROTO_IPV6, IPV6_V6ONLY, (const void *)&v,
                   sizeof(v)) < 0) {
...
static void script_fu_server_shutdown_fd(gpointer key, gpointer value,
                                         gpointer data) {
...
static gboolean server_interface(void) {
  ...
  dlg = gimp_dialog_new(_("Script-Fu Server Options"), "gimp-script-fu", NULL,
                        0, gimp_standard_help_func, "plug-in-script-fu-server",

                        _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Start Server"),
                        GTK_RESPONSE_OK,
  ...
  g_signal_connect(dlg, "response", G_CALLBACK(response_callback), NULL);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Listen on IP:"), 0.0, 0.5,
                           sint.ip_entry, 1);

  /*  The server port  */
  sint.port_entry = gtk_entry_new();
  gtk_entry_set_text(GTK_ENTRY(sint.port_entry), "10008");
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Server port:"), 0.0, 0.5,
                           sint.port_entry, 1);

  /*  The server logfile  */
  sint.log_entry = gtk_entry_new();
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Server logfile:"), 0.0, 0.5,
                           sint.log_entry, 1);
