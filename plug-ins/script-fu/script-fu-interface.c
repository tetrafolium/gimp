...
static void script_fu_response(GtkWidget *widget, gint response_id,
                               SFScript *script);
static void script_fu_ok(SFScript *script);
static void script_fu_reset(SFScript *script);

static void script_fu_file_callback(GtkWidget *widget, SFFilename *file);
static void script_fu_combo_callback(GtkWidget *widget, SFOption *option);
static void script_fu_pattern_callback(gpointer data, const gchar *name,
                                       gint width, gint height, gint bytes,
                                       const guchar *mask_data,
                                       gboolean closing);
static void script_fu_gradient_callback(gpointer data, const gchar *name,
                                        gint width, const gdouble *mask_data,
                                        gboolean closing);
static void script_fu_font_callback(gpointer data, const gchar *name,
                                    gboolean closing);
static void script_fu_palette_callback(gpointer data, const gchar *name,
                                       gboolean closing);
static void script_fu_brush_callback(gpointer data, const gchar *name,
                                     gdouble opacity, gint spacing,
                                     GimpLayerMode paint_mode, gint width,
                                     gint height, const guchar *mask_data,
                                     gboolean closing);
...
gboolean script_fu_interface_is_active(void) { return (sf_interface != NULL); }
...
void script_fu_interface_report_cc(const gchar *command) {
  ...
  if (sf_interface->last_command &&
    ...
    if (!g_str_has_prefix(command, "gimp-progress-")) {
      ...
      new_command =
          g_strdup_printf("%s <%d>", command, sf_interface->command_count);
...
GimpPDBStatusType script_fu_interface(SFScript *script, gint start_arg) {
  ...
  sf_interface->dialog = dialog = gimp_dialog_new(
      title, "gimp-script-fu", NULL, 0, gimp_standard_help_func, script->name,

      _("_Reset"), RESPONSE_RESET, _("_Cancel"), GTK_RESPONSE_CANCEL, _("_OK"),
      GTK_RESPONSE_OK,
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(script_fu_response), script);

  g_signal_connect_swapped(dialog, "destroy",
                           G_CALLBACK(script_fu_interface_quit), script);
  ...
  for (i = start_arg; i < script->n_args; i++) {
    ...
    case SF_COLOR: {
      ...
      left_align = TRUE;
      widget = gimp_color_button_new(
          _("Script-Fu Color Selection"), COLOR_SAMPLE_WIDTH,
          COLOR_SAMPLE_HEIGHT, &arg->value.sfa_color, GIMP_COLOR_AREA_FLAT);

      gimp_color_button_set_update(GIMP_COLOR_BUTTON(widget), TRUE);

      config = gimp_get_color_configuration();
      gimp_color_button_set_color_config(GIMP_COLOR_BUTTON(widget), config);
    ...
    case SF_TOGGLE:
      ...
      g_signal_connect(widget, "toggled", G_CALLBACK(gimp_toggle_button_update),
                       &arg->value.sfa_toggle);
      break;
    ...
    case SF_ADJUSTMENT:
      ...
      case SF_SLIDER: {
        ...
        widget =
            gimp_scale_entry_new(label_text, arg->value.sfa_adjustment.value,
      ...
      case SF_SPINNER:
        ...
        arg->value.sfa_adjustment.adj =
            gtk_adjustment_new(arg->value.sfa_adjustment.value,
                               ...
                               arg->default_value.sfa_adjustment.upper,
                               arg->default_value.sfa_adjustment.step,
                               arg->default_value.sfa_adjustment.page, 0);
      ...
      g_signal_connect(arg->value.sfa_adjustment.adj, "value-changed",
    ...
    case SF_DIRNAME:
      if (arg->type == SF_FILENAME)
        ...
        widget =
            gtk_file_chooser_button_new(_("Script-Fu Folder Selection"),
    ...
    case SF_GRADIENT:
      left_align = TRUE;
      widget = gimp_gradient_select_button_new(
          _("Script-Fu Gradient Selection"), arg->value.sfa_gradient);
    ...
    case SF_BRUSH:
      left_align = TRUE;
      widget = gimp_brush_select_button_new(
          _("Script-Fu Brush Selection"), arg->value.sfa_brush.name,
          arg->value.sfa_brush.opacity, arg->value.sfa_brush.spacing,
    ...
    case SF_OPTION:
      ...
      for (list = arg->default_value.sfa_option.list; list;
      ...
      g_signal_connect(widget, "changed", G_CALLBACK(script_fu_combo_callback),
                       &arg->value.sfa_option);
      break;

    case SF_ENUM:
      widget = gimp_enum_combo_box_new(
    ...
    if (widget) {
      if (label_text) {
        gimp_grid_attach_aligned(GTK_GRID(sf_interface->grid), 0, row,
                                 label_text, 0.0, label_yalign, widget, 2);
        g_free(label_text);
      } else {
        gtk_grid_attach(GTK_GRID(sf_interface->grid), widget, 0, row, 3, 1);
  ...
  gtk_box_pack_start(GTK_BOX(vbox2), sf_interface->progress_bar, FALSE, FALSE,
                     0);
  ...
  gtk_box_pack_start(GTK_BOX(vbox2), sf_interface->progress_label, FALSE, FALSE,
                     0);
...
static void script_fu_string_update(gchar **dest, const gchar *src) {
  if (*dest)
    g_free(*dest);
...
static void unset_transient_for(GtkWidget *dialog) {
...
static void script_fu_ok(SFScript *script) {
  ...
  for (i = 0; i < script->n_args; i++) {
    ...
    case SF_STRING:
      ...
      arg_value->sfa_value = g_strdup(gtk_entry_get_text(GTK_ENTRY(widget)));
      break;
  ...
  if (ts_interpret_string(command)) {
    gchar *message =
        g_strdup_printf(_("Error while executing %s:"), script->name);
...
static void script_fu_reset(SFScript *script) {
  ...
  for (i = 0; i < script->n_args; i++) {
    ...
    case SF_COLOR:
      gimp_color_button_set_color(GIMP_COLOR_BUTTON(widget), &value->sfa_color);
      break;
    ...
    case SF_GRADIENT:
      gimp_gradient_select_button_set_gradient(
          GIMP_GRADIENT_SELECT_BUTTON(widget), value->sfa_gradient);
      break;

    case SF_BRUSH:
      gimp_brush_select_button_set_brush(
          GIMP_BRUSH_SELECT_BUTTON(widget), value->sfa_brush.name,
          value->sfa_brush.opacity, value->sfa_brush.spacing,
