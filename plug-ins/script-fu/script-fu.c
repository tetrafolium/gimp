...
#define SCRIPT_FU                                                              \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), SCRIPT_FU_TYPE, ScriptFu))
...
static void script_fu_class_init(ScriptFuClass *klass) {
...
static void script_fu_init(ScriptFu *script_fu) {}

static GList *script_fu_query_procedures(GimpPlugIn *plug_in) {
  GList *list = NULL;

  gimp_plug_in_set_translation_domain(plug_in, GETTEXT_PACKAGE "-script-fu",
                                      NULL);
...
static GimpProcedure *script_fu_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, "extension-script-fu")) {
    ...
    GIMP_PROC_ARG_STRING(procedure, "ip", "IP",
                         "The IP on which to listen for requests", NULL,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "port", "Port",
                      "The port on which to listen for requests", 0, G_MAXINT,
                      0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "logfile", "Log File",
                         "The file to log activity to", NULL,
                         G_PARAM_READWRITE);
  } else if (!strcmp(name, "plug-in-script-fu-eval")) {
    procedure = gimp_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                   script_fu_run, NULL, NULL);

    gimp_procedure_set_documentation(procedure, "Evaluate scheme code",
                                     "Evaluate the code under the scheme "
                                     "interpreter (primarily for batch mode)",
                                     name);
    gimp_procedure_set_attribution(procedure, "Manish Singh", "Manish Singh",
                                   "1998");
    ...
    GIMP_PROC_ARG_STRING(procedure, "code", "Code", "The code to run", NULL,
                         G_PARAM_READWRITE);
  }
...
static void script_fu_extension_init(GimpPlugIn *plug_in) {
  ...
  gimp_plug_in_add_menu_branch(plug_in, "<Image>/File/Create", N_("_Buttons"));
  gimp_plug_in_add_menu_branch(plug_in, "<Image>/File/Create", N_("_Logos"));
  gimp_plug_in_add_menu_branch(plug_in, "<Image>/File/Create", N_("_Patterns"));
  ...
  gimp_procedure_set_documentation(
      procedure, N_("Re-read all available Script-Fu scripts"),
      "Re-read all available Script-Fu scripts", "script-fu-refresh");
  gimp_procedure_set_attribution(procedure, "Spencer Kimball & Peter Mattis",
                                 "Spencer Kimball & Peter Mattis", "1997");
