...
static gboolean script_fu_script_param_init(SFScript *script,
                                            ...
                                            SFArgType type, gint n);

/*
 *  Function definitions
 */

SFScript *script_fu_script_new(const gchar *name, const gchar *menu_label,
                               const gchar *blurb, const gchar *author,
                               const gchar *copyright, const gchar *date,
                               const gchar *image_types, gint n_args) {
  SFScript *script;
...
void script_fu_script_install_proc(GimpPlugIn *plug_in, SFScript *script,
                                   GimpRunFunc run_func) {
  GimpProcedure *procedure;
  const gchar *menu_label = NULL;
  gint arg_count[SF_DISPLAY] = {
      0,
  };
  gint i;
  ...
  procedure =
      gimp_procedure_new(plug_in, script->name, GIMP_PDB_PROC_TYPE_TEMPORARY,
  ...
  gimp_procedure_set_documentation(procedure, script->blurb, NULL,
                                   script->name);
  gimp_procedure_set_attribution(procedure, script->author, script->copyright,
                                 script->date);

  gimp_procedure_add_argument(
      procedure, g_param_spec_enum("run-mode", "Run mode", "The run mode",
  ...
  for (i = 0; i < script->n_args; i++) {
    ...
    if (arg_count[script->args[i].type] == 0) {
      ...
      g_snprintf(numbered_name, sizeof(numbered_name), "%s-%d", name,
                 arg_count[script->args[i].type] + 1);
      g_snprintf(numbered_nick, sizeof(numbered_nick), "%s %d", nick,
                 arg_count[script->args[i].type] + 1);
    }
    ...
    case SF_IMAGE:
      pspec =
          gimp_param_spec_image(numbered_name, numbered_nick,
                                script->args[i].label, TRUE, G_PARAM_READWRITE);
      break;

    case SF_DRAWABLE:
      pspec = gimp_param_spec_drawable(numbered_name, numbered_nick,
                                       script->args[i].label, TRUE,
                                       G_PARAM_READWRITE);
      break;

    case SF_LAYER:
      pspec =
          gimp_param_spec_layer(numbered_name, numbered_nick,
                                script->args[i].label, TRUE, G_PARAM_READWRITE);
      break;

    case SF_CHANNEL:
      pspec = gimp_param_spec_channel(numbered_name, numbered_nick,
                                      script->args[i].label, TRUE,
                                      G_PARAM_READWRITE);
      break;

    case SF_VECTORS:
      pspec = gimp_param_spec_vectors(numbered_name, numbered_nick,
                                      script->args[i].label, TRUE,
                                      G_PARAM_READWRITE);
      break;

    case SF_DISPLAY:
      pspec = gimp_param_spec_display(numbered_name, numbered_nick,
                                      script->args[i].label, TRUE,
                                      G_PARAM_READWRITE);
      break;

    case SF_COLOR:
      pspec = gimp_param_spec_rgb(numbered_name, numbered_nick,
                                  script->args[i].label, TRUE, NULL,
                                  G_PARAM_READWRITE);
      break;

    case SF_TOGGLE:
      pspec =
          g_param_spec_boolean(numbered_name, numbered_nick,
                               script->args[i].label, FALSE, G_PARAM_READWRITE);
      break;

    case SF_VALUE:
      pspec =
          g_param_spec_string(numbered_name, numbered_nick,
                              script->args[i].label, NULL, G_PARAM_READWRITE);
      break;
    ...
    case SF_ADJUSTMENT:
      pspec = g_param_spec_double(numbered_name, numbered_nick,
                                  script->args[i].label, -G_MAXDOUBLE,
                                  G_MAXDOUBLE, 0, G_PARAM_READWRITE);
      break;

    case SF_FILENAME:
      pspec = g_param_spec_string(numbered_name, numbered_nick,
                                  script->args[i].label, NULL,
    ...
    case SF_OPTION:
      pspec =
          g_param_spec_int(numbered_name, numbered_nick, script->args[i].label,
...
void script_fu_script_reset(SFScript *script, gboolean reset_ids) {
  gint i;
...
gint script_fu_script_collect_standard_args(SFScript *script,
  ...
  if (script_fu_script_param_init(script, args, SF_DISPLAY, params_consumed)) {
    params_consumed++;
  }

  /*  an IMAGE id may come first or after the DISPLAY id  */
  if (script_fu_script_param_init(script, args, SF_IMAGE, params_consumed)) {
    params_consumed++;

    /*  and may be followed by a DRAWABLE, LAYER, CHANNEL or
     *  VECTORS id
     */
    if (script_fu_script_param_init(script, args, SF_DRAWABLE,
                                    params_consumed) ||
        script_fu_script_param_init(script, args, SF_LAYER, params_consumed) ||
        script_fu_script_param_init(script, args, SF_CHANNEL,
                                    params_consumed) ||
        script_fu_script_param_init(script, args, SF_VECTORS,
                                    params_consumed)) {
      params_consumed++;
    }
  }
...
gchar *script_fu_script_get_command(SFScript *script) {
  ...
  for (i = 0; i < script->n_args; i++) {
    ...
    case SF_COLOR: {
      ...
      g_string_append_printf(s, "'(%d %d %d)", (gint)r, (gint)g, (gint)b);
    } break;
    ...
    case SF_ADJUSTMENT: {
      ...
      g_ascii_dtostr(buffer, sizeof(buffer), arg_value->sfa_adjustment.value);
    ...
    case SF_BRUSH: {
      ...
      g_ascii_dtostr(buffer, sizeof(buffer), arg_value->sfa_brush.opacity);
      g_string_append_printf(s, "'(\"%s\" %s %d %d)", arg_value->sfa_brush.name,
                             buffer, arg_value->sfa_brush.spacing,
...
static gboolean script_fu_script_param_init(SFScript *script,
  ...
  if (script->n_args > n && arg->type == type &&
