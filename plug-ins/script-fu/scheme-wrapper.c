...
static void ts_init_enum(scheme *sc, GType enum_type);
static void ts_init_procedures(scheme *sc, gboolean register_scipts);
static void convert_string(gchar *str);
static pointer script_fu_marshal_procedure_call(scheme *sc, pointer a,
                                                gboolean permissive);
static pointer script_fu_marshal_procedure_call_strict(scheme *sc, pointer a);
...
static pointer script_fu_nil_call(scheme *sc, pointer a);

static gboolean ts_load_file(const gchar *dirname, const gchar *basename);
...
void ts_set_print_flag(gint print_flag) { sc.print_output = print_flag; }

void ts_print_welcome(void) {
  ts_output_string(TS_OUTPUT_NORMAL, "Welcome to TinyScheme, Version 1.40\n",
                   -1);
  ts_output_string(TS_OUTPUT_NORMAL, "Copyright (c) Dimitrios Souflis\n", -1);
}

void ts_interpret_stdin(void) { scheme_load_file(&sc, stdin); }
...
static void ts_init_procedures(scheme *sc, gboolean register_scripts) {
  gchar **proc_list;
  gint num_procs;
  gint i;
  pointer symbol;
  ...
  sc->vptr->scheme_define(
      sc, sc->global_env, symbol,
      sc->vptr->mk_foreign_func(sc, register_scripts ? script_fu_register_call
                                                     : script_fu_nil_call));
  sc->vptr->setimmutable(symbol);
  ...
  sc->vptr->scheme_define(
      sc, sc->global_env, symbol,
      sc->vptr->mk_foreign_func(sc, register_scripts
  ...
  sc->vptr->scheme_define(
      sc, sc->global_env, symbol,
      sc->vptr->mk_foreign_func(sc, script_fu_marshal_procedure_call_strict));
  sc->vptr->setimmutable(symbol);

  /*  register the internal database execution procedure; see comment below  */
  symbol = sc->vptr->mk_symbol(sc, "-gimp-proc-db-call");
  sc->vptr->scheme_define(sc, sc->global_env, symbol,
                          sc->vptr->mk_foreign_func(
                              sc, script_fu_marshal_procedure_call_permissive));
  sc->vptr->setimmutable(symbol);
...
#define type(p) (typeflag(p) & T_MASKTYPE)
  ...
  static const char *ts_types[] = {
      "T_NONE",    "T_STRING",    "T_NUMBER",      "T_SYMBOL",
      "T_PROC",    "T_PAIR",      "T_CLOSURE",     "T_CONTINUATION",
      "T_FOREIGN", "T_CHARACTER", "T_PORT",        "T_VECTOR",
      "T_MACRO",   "T_PROMISE",   "T_ENVIRONMENT", "T_ARRAY"};
  ...
  for (i = 0; i < n_arg_specs; i++) {
...
#if DEBUG_MARSHALL
    {
      ...
      gimp_enum_get_value(GIMP_TYPE_PDB_ARG_TYPE, params[i].type, &type_name,
                          NULL, NULL, NULL);
    ...
    if (G_VALUE_HOLDS_INT(&value)) {
      ...
      if (success) {
        g_value_set_int(&value, sc->vptr->ivalue(sc->vptr->pair_car(a)));
#if DEBUG_MARSHALL
      ...
      if (success) {
        g_value_set_uint(&value, sc->vptr->ivalue(sc->vptr->pair_car(a)));
#if DEBUG_MARSHALL
      ...
      if (success) {
        g_value_set_uchar(&value, sc->vptr->ivalue(sc->vptr->pair_car(a)));
#if DEBUG_MARSHALL
      ...
      if (success) {
        g_value_set_double(&value, sc->vptr->rvalue(sc->vptr->pair_car(a)));
#if DEBUG_MARSHALL
      ...
      if (success) {
        g_value_set_enum(&value, sc->vptr->ivalue(sc->vptr->pair_car(a)));
#if DEBUG_MARSHALL
      ...
      if (success) {
        g_value_set_boolean(&value, sc->vptr->ivalue(sc->vptr->pair_car(a)));
#if DEBUG_MARSHALL
      ...
      if (success) {
        ...
        if (n_elements < 0 || n_elements > sc->vptr->vector_length(vector)) {
          g_snprintf(error_str, sizeof(error_str),
                     ...
                     i + 1, proc_name, sc->vptr->vector_length(vector),
                     n_elements);
          return foreign_error(sc, error_str, 0);
        }
        ...
        for (j = 0; j < n_elements; j++) {
          ...
          if (!sc->vptr->is_number(v_element)) {
            g_snprintf(error_str, sizeof(error_str),
                       "Item %d in vector is not a number (argument %d for "
                       "function %s)",
                       j + 1, i + 1, proc_name);
            g_free(array);
      ...
      if (success) {
        ...
        if (n_elements < 0 || n_elements > sc->vptr->list_length(sc, vector)) {
          g_snprintf(error_str, sizeof(error_str),
                     "STRING vector (argument %d) for function %s has "
                     "length of %d but expected length of %d",
                     i + 1, proc_name, sc->vptr->list_length(sc, vector),
                     n_elements);
          return foreign_error(sc, error_str, 0);
        }
        ...
        for (j = 0; j < n_elements; j++) {
          ...
          if (!sc->vptr->is_string(v_element)) {
            g_snprintf(error_str, sizeof(error_str),
                       "Item %d in vector is not a string (argument %d for "
                       "function %s)",
                       j + 1, i + 1, proc_name);
            g_strfreev(array);
            return foreign_error(sc, error_str, vector);
...
#if DEBUG_MARSHALL
        {
          ...
          if (count > 0) {
            g_printerr("     ");
            for (j = 0; j < count; ++j)
              g_printerr(" \"%s\"", args[i].data.d_stringarray[j]);
            g_printerr("\n");
          }
        }
#endif
      ...
      if (sc->vptr->is_string(sc->vptr->pair_car(a))) {
        if (!gimp_rgb_parse_css(
                &color, sc->vptr->string_value(sc->vptr->pair_car(a)), -1))
          success = FALSE;
      ...
      if (success) {
        ...
        for (j = 0; j < n_elements; j++) {
          ...
          if (!(sc->vptr->is_list(sc, sc->vptr->pair_car(v_element)) &&
                sc->vptr->list_length(sc, sc->vptr->pair_car(v_element)) ==
                    3)) {
            g_snprintf(error_str, sizeof(error_str),
      ...
      if (success) {
        ...
        parasite.name = sc->vptr->string_value(sc->vptr->pair_car(temp_val));
#if DEBUG_MARSHALL
        ...
        parasite.flags = sc->vptr->ivalue(sc->vptr->pair_car(temp_val));
#if DEBUG_MARSHALL
        ...
        parasite.data = sc->vptr->string_value(sc->vptr->pair_car(temp_val));
      ...
      return foreign_error(sc, "Status is for return types, not arguments",
                           sc->vptr->pair_car(a));
    } else {
      g_snprintf(error_str, sizeof(error_str),
                 "Argument %d for %s is an unknown type %s", i + 1, proc_name,
...
#endif
      values = gimp_pdb_run_procedure_array(gimp_get_pdb(), proc_name, args);
#if DEBUG_MARSHALL
      g_printerr("  done.\n");
#endif
    }
...
#endif
    g_snprintf(error_str, sizeof(error_str),
               "Procedure execution of %s did not return a status", proc_name);
...
#if DEBUG_MARSHALL
  {
    ...
    gimp_enum_get_value(GIMP_TYPE_PDB_STATUS_TYPE, values[0].data.d_status,
  ...
  case GIMP_PDB_EXECUTION_ERROR:
    if (gimp_value_array_length(values) > 1 &&
      ...
      g_snprintf(error_str, sizeof(error_str),
                 "Procedure execution of %s failed: %s", proc_name,
                 GIMP_VALUES_GET_STRING(values, 1));
    } else {
      g_snprintf(error_str, sizeof(error_str),
                 "Procedure execution of %s failed", proc_name);
    }
    return foreign_error(sc, error_str, 0);
    break;
  ...
  case GIMP_PDB_CALLING_ERROR:
    if (gimp_value_array_length(values) > 1 &&
      ...
      g_snprintf(
          ...
          proc_name, GIMP_VALUES_GET_STRING(values, 1));
    } else {
      g_snprintf(error_str, sizeof(error_str),
  ...
  case GIMP_PDB_SUCCESS:
    ...
    for (i = gimp_value_array_length(values) - 2; i >= 0; --i) {
...
#if DEBUG_MARSHALL
      {
        ...
        gimp_enum_get_value(GIMP_TYPE_PDB_ARG_TYPE, return_vals[i].type,
      ...
      if (G_VALUE_HOLDS_OBJECT(value)) {
        ...
        return_val =
            sc->vptr->cons(sc, sc->vptr->mk_integer(sc, id), return_val);
      } else if (G_VALUE_HOLDS_INT(value)) {
        gint v = g_value_get_int(value);

        return_val =
            sc->vptr->cons(sc, sc->vptr->mk_integer(sc, v), return_val);
      } else if (G_VALUE_HOLDS_UINT(value)) {
        ...
        return_val = sc->vptr->cons(sc, sc->vptr->mk_real(sc, v), return_val);
      } else if (G_VALUE_HOLDS_ENUM(value)) {
        ...
        return_val = sc->vptr->cons(sc, sc->vptr->mk_string(sc, v), return_val);
      } else if (GIMP_VALUE_HOLDS_INT32_ARRAY(value)) {
        ...
        for (j = 0; j < n; j++) {
          sc->vptr->set_vector_elem(vector, j, sc->vptr->mk_integer(sc, v[j]));
        }
        ...
        for (j = 0; j < n; j++) {
          sc->vptr->set_vector_elem(vector, j, sc->vptr->mk_real(sc, v[j]));
        }
        ...
        for (j = n - 1; j >= 0; j--) {
          list = sc->vptr->cons(sc, sc->vptr->mk_string(sc, v[j] ? v[j] : ""),
                                list);
        }
        ...
        temp_val = sc->vptr->cons(
            sc, sc->vptr->mk_integer(sc, r),
            sc->vptr->cons(
                sc, sc->vptr->mk_integer(sc, g),
                sc->vptr->cons(sc, sc->vptr->mk_integer(sc, b), sc->NIL)));

        return_val = sc->vptr->cons(sc, temp_val, return_val);
      } else if (GIMP_VALUE_HOLDS_RGB_ARRAY(value)) {
        gint32 n = GIMP_VALUES_GET_INT(values, i);
        ...
        if (v->name == NULL) {
           ...
           */
          temp_val = sc->vptr->cons(
              sc, sc->vptr->mk_string(sc, v->name),
              sc->vptr->cons(
                  sc, sc->vptr->mk_integer(sc, v->flags),
                  sc->vptr->cons(
                      sc, sc->vptr->mk_counted_string(sc, data, char_cnt),
                      sc->NIL)));
        ...
        return foreign_error(
            sc, "Procedure execution returned multiple status values", 0);
      } else {
        g_snprintf(error_str, sizeof(error_str), "Unknown return type %s",
                   g_type_name(G_VALUE_TYPE(value)));
        return foreign_error(sc, error_str, 0);
      }
    }
...
static pointer script_fu_nil_call(scheme *sc, pointer a) { return sc->NIL; }
