...
void ts_register_output_func(ts_output_func func, gpointer user_data) {
  ts_output_handler = func;
  ts_output_data = user_data;
}

/* len is length of 'string' in bytes or -1 for null terminated strings */
void ts_output_string(TsOutputType type, const char *string, int len) {
  if (len < 0)
    len = strlen(string);
...
INTERFACE gunichar charvalue(pointer p) {
  return (gunichar)ivalue_unchecked(p);
}
...
static char *store_string(scheme *sc, int char_cnt, const char *str,
                          gunichar fill) {
  int len;
  int i;
  gchar utf8[7];
  ...
  if (pt->kind & port_file) {
    ...
    while (TRUE) {
      ...
      } else /* Byte is valid lead */
      {
        ...
        {
          return g_utf8_get_char_validated((char *)utf8, sizeof(utf8));
        }
    ...
    while (TRUE) {
      ...
      if (c != (gunichar)-1 && c != (gunichar)-2) /* Valid UTF-8 character? */
      {
...
static pointer readstrexp(scheme *sc) {
  ...
  for (;;) {
    ...
    case st_ok:
      ...
      case '"':
        *p = 0;
        return mk_counted_string(
            sc, sc->strbuff, g_utf8_strlen(sc->strbuff, sizeof(sc->strbuff)));
      default:
        len = g_unichar_to_utf8(c, p);
        p += len;
        break;
...
static void atom2str(scheme *sc, pointer l, int f, char **pp, int *plen) {
  ...
  if (l == sc->NIL) {
    ...
    if (!f) {
      ...
      *pp = sc->strbuff;
      *plen = 0;
      printslashstring(sc, strvalue(l), g_utf8_strlen(strvalue(l), -1));
      return;
    }
...
static pointer opexe_2(scheme *sc, enum scheme_opcodes op) {
  ...
  case OP_SUBSTR: { /* substring */
    ...
    if (str == NULL) {
      sc->no_memory = 1;
      Error_1(sc,
              "string-set!: No memory to extract substring:", car(sc->args));
    }
...
static struct scheme_interface vtbl = {scheme_define,    s_cons,
                                       ...
                                       mk_foreign_func,  mk_closure,
                                       putstr,           putcharacter,

                                       is_string,        string_length,
                                       string_value,     is_number,
                                       nvalue,           ivalue,
                                       rvalue,           is_integer,
                                       is_real,          is_character,
                                       charvalue,        is_list,
                                       is_vector,        list_length,
                                       ivalue,           fill_vector,
                                       vector_elem,      set_vector_elem,
                                       is_port,          is_pair,
                                       pair_car,         pair_cdr,
                                       set_car,          set_cdr,
