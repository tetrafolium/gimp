 ...
 * 1.2 support for load/save ranges, fixed SGI & SUN problems (I hope...), fixed
 FLC
...
static GimpValueArray *fli_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *fli_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static GimpImage *load_image(GFile *file, GObject *config, GError **error);
static gboolean load_dialog(GFile *file, GimpProcedure *procedure,
                            GObject *config);

static gboolean save_image(GFile *file, GimpImage *image, GObject *config,
                           GError **error);
static gboolean save_dialog(GimpImage *image, GimpProcedure *procedure,
                            GObject *config);

static gboolean get_info(GFile *file, gint32 *width, gint32 *height,
                         gint32 *frames, GError **error);

G_DEFINE_TYPE(Fli, fli, GIMP_TYPE_PLUG_IN)

GIMP_MAIN(FLI_TYPE)

static void fli_class_init(FliClass *klass) {
...
static GimpProcedure *fli_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, fli_load, NULL, NULL);

    gimp_procedure_set_menu_label(procedure, N_("AutoDesk FLIC animation"));

    gimp_procedure_set_documentation(procedure, "Load FLI-movies",
                                     "This is an experimental plug-in to "
                                     "handle FLI movies",
                                     name);
    gimp_procedure_set_attribution(procedure, "Jens Ch. Restemeier",
    ...
    GIMP_PROC_ARG_INT(procedure, "from-frame", "From frame",
                      "Load beginning from this frame", -1, G_MAXINT, -1,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "to-frame", "To frame",
                      "End loading with this frame", -1, G_MAXINT, -1,
                      G_PARAM_READWRITE);
  } else if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, fli_save, NULL, NULL);

    gimp_procedure_set_image_types(procedure, "INDEXED, GRAY");

    gimp_procedure_set_menu_label(procedure, N_("AutoDesk FLIC animation"));

    gimp_procedure_set_documentation(procedure, "Export FLI-movies",
                                     "This is an experimental plug-in to "
                                     "handle FLI movies",
                                     name);
    gimp_procedure_set_attribution(procedure, "Jens Ch. Restemeier",
    ...
    GIMP_PROC_ARG_INT(procedure, "from-frame", "From frame",
                      "Export beginning from this frame", -1, G_MAXINT, -1,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "to-frame", "To frame",
                      "End exporting with this frame", -1, G_MAXINT, -1,
                      G_PARAM_READWRITE);
    ...
    GIMP_PROC_ARG_FILE(procedure, "file", "File",
                       "The local file to get info about", G_PARAM_READWRITE);

    GIMP_PROC_VAL_INT(procedure, "width", "Width", "Width of one frame", 0,
                      GIMP_MAX_IMAGE_SIZE, 0, G_PARAM_READWRITE);

    GIMP_PROC_VAL_INT(procedure, "height", "Height", "Height of one frame", 0,
                      GIMP_MAX_IMAGE_SIZE, 0, G_PARAM_READWRITE);

    GIMP_PROC_VAL_INT(procedure, "frames", "Frames", "Number of frames", 0,
...
static GimpValueArray *fli_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "FLI",
...
static GimpValueArray *fli_info(GimpProcedure *procedure,
  ...
  if (!get_info(file, &width, &height, &frames, &error)) {
    return gimp_procedure_new_return_values(procedure, GIMP_PDB_EXECUTION_ERROR,
                                            error);
  }
...
static GimpImage *load_image(GFile *file, GObject *config, GError **error) {
  ...
  g_object_get(config, "from-frame", &from_frame, "to-frame", &to_frame, NULL);
  ...
  for (cnt = from_frame; cnt <= to_frame; cnt++) {
    ...
    layer = gimp_layer_new(image, name_buf, fli_header.width, fli_header.height,
    ...
    gegl_buffer_set(buffer,
                    GEGL_RECTANGLE(0, 0, fli_header.width, fli_header.height),
                    0, NULL, fb, GEGL_AUTO_ROWSTRIDE);
...
static gboolean save_image(GFile *file, GimpImage *image, GObject *config,
  ...
  fli_header.oframe1 = fli_header.oframe2 =
      0; /* will be fixed during the write */
  ...
  for (iter = g_list_nth(framelist, from_frame - 1), cnt = from_frame;
       iter && cnt <= to_frame; iter = g_list_next(iter), cnt++) {
    ...
    for (yc = 0, yy = offset_y; yc < rows; yc++, yy++) {
      if (yy >= 0 && yy < fli_header.height) {
        gegl_buffer_get(buffer, GEGL_RECTANGLE(0, yc, cols, 1), 1.0, format,
...
static gboolean load_dialog(GFile *file, GimpProcedure *procedure,
  ...
  g_object_set(config, "from-frame", 1, "to-frame", n_frames, NULL);
  ...
  spinbutton = gimp_prop_spin_button_new(config, "from-frame", 1, 10, 0);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, C_("frame-range", "_From:"),
                           0.0, 0.5, spinbutton, 1);

  spinbutton = gimp_prop_spin_button_new(config, "to-frame", 1, 10, 0);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, C_("frame-range", "_To:"), 0.0,
                           0.5, spinbutton, 1);
...
static gboolean save_dialog(GimpImage *image, GimpProcedure *procedure,
  ...
  gimp_labeled_set_text(
      GIMP_LABELED(gimp_procedure_dialog_get_widget(
          GIMP_PROCEDURE_DIALOG(dialog), "from-frame", GIMP_TYPE_LABEL_SPIN)),
      "_From:");
  gimp_labeled_set_text(
      GIMP_LABELED(gimp_procedure_dialog_get_widget(
          GIMP_PROCEDURE_DIALOG(dialog), "to-frame", GIMP_TYPE_LABEL_SPIN)),
      "_To:");
