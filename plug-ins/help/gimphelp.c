...
gboolean gimp_help_init(gint num_domain_names, const gchar **domain_names,
                        gint num_domain_uris, const gchar **domain_uris) {
  gint i;
...
void gimp_help_register_domain(const gchar *domain_name,
  ...
  if (!domain_hash)
    domain_hash = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                        (GDestroyNotify)gimp_help_domain_free);

  g_hash_table_insert(domain_hash, g_strdup(domain_name),
