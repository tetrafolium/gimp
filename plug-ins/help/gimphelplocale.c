...
static void locale_set_error(GError **error, const gchar *format, GFile *file);
...
const gchar *gimp_help_locale_map(GimpHelpLocale *locale,
  ...
  if (locale->help_id_mapping) {
    GimpHelpItem *item = g_hash_table_lookup(locale->help_id_mapping, help_id);
...
static gboolean locale_parser_parse(GMarkupParseContext *context,
                                    ...
                                    GInputStream *stream, goffset size,
...
static void locale_parser_end_element(GMarkupParseContext *context,
                                      const gchar *element_name,
                                      gpointer user_data, GError **error);
static void locale_parser_error(GMarkupParseContext *context, GError *error,
                                gpointer user_data);
...
static void locale_parser_parse_namespace(LocaleParser *parser,
                                          const gchar **names,
                                          const gchar **values);
static void locale_parser_parse_item(LocaleParser *parser, const gchar **names,
                                     const gchar **values);
static void locale_parser_parse_missing(LocaleParser *parser,
                                        const gchar **names,
                                        const gchar **values);

static const GMarkupParser markup_parser = {locale_parser_start_element,
...
gboolean gimp_help_locale_parse(GimpHelpLocale *locale, const gchar *uri,
  ...
  if (progress) {
    GFileInfo *info = g_file_query_info(file, G_FILE_ATTRIBUTE_STANDARD_SIZE, 0,
                                        cancellable, error);
    if (!info) {
  ...
  success = locale_parser_parse(context, progress, G_INPUT_STREAM(stream), size,
                                cancellable, error);
...
static gboolean locale_parser_parse(GMarkupParseContext *context,
  ...
  while ((len = g_input_stream_read(stream, buffer, sizeof(buffer), cancellable,
                                    error)) != -1) {
    switch (len) {
    case 0:
...
static void locale_parser_start_element(GMarkupParseContext *context,
  ...
  case LOCALE_START:
    if (strcmp(element_name, "gimp-help") == 0) {
      ...
      locale_parser_parse_namespace(parser, attribute_names, attribute_values);
    } else
  ...
  case LOCALE_IN_HELP:
    if (strcmp(element_name, "help-item") == 0) {
      ...
      locale_parser_parse_item(parser, attribute_names, attribute_values);
    } else if (strcmp(element_name, "help-missing") == 0) {
      parser->state = LOCALE_IN_MISSING;
      locale_parser_parse_missing(parser, attribute_names, attribute_values);
    } else
      locale_parser_start_unknown(parser);
    break;
  ...
  for (; *names && *values; names++, values++) {
    if (!strncmp(*names, "xmlns:", 6) &&
      ...
      g_printerr("help (%s): id attribute name for \"%s\" is \"%s\"\n",
                 parser->locale->locale_id, parser->help_domain,
                 parser->id_attr_name);
#endif
    }
  }
...
static void locale_parser_parse_item(LocaleParser *parser, const gchar **names,
  ...
  if (id && ref) {
    if (!parser->locale->help_id_mapping)
      parser->locale->help_id_mapping = g_hash_table_new_full(
          g_str_hash, g_str_equal, g_free, (GDestroyNotify)gimp_help_item_free);

    g_hash_table_insert(parser->locale->help_id_mapping, g_strdup(id),
...
static void locale_parser_parse_missing(LocaleParser *parser,
  ...
  if (ref && parser->locale->help_missing == NULL) {
