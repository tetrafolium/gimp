...
static void help_load(const gchar *procedure, const gchar *help_domain,
                      const gchar *help_locales, const gchar *help_id);
...
static void help_init(Help *help) {}
...
static GimpProcedure *help_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, GIMP_HELP_EXT_PROC)) {
    procedure = gimp_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_EXTENSION,
                                   help_run, NULL, NULL);
    ...
    gimp_procedure_set_attribution(
        ...
        "Michael Natterer <mitch@gimp.org>, "
        "Henrik Brix Andersen <brix@gimp.org>",
        "Sven Neumann, Michael Natterer & Henrik Brix Andersen", "1999-2008");

    GIMP_PROC_ARG_INT(procedure, "num-domain-names", "Num Domain Names",
                      "Num domain names", 0, G_MAXINT, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING_ARRAY(procedure, "domain-names", "Domain Names",
                               "Domain names", G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "num-domain-uris", "Num Domain URIs",
                      "Num domain URIs", 0, G_MAXINT, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING_ARRAY(procedure, "domain-uris", "Domain URIs",
...
static void help_temp_proc_install(GimpPlugIn *plug_in) {
  ...
  procedure = gimp_procedure_new(plug_in, GIMP_HELP_TEMP_EXT_PROC,
                                 GIMP_PDB_PROC_TYPE_TEMPORARY, help_temp_run,
                                 NULL, NULL);
  ...
  GIMP_PROC_ARG_STRING(
      procedure, "help-proc", "The procedure of the browser to use",
      "The procedure of the browser to use", NULL, G_PARAM_READWRITE);

  GIMP_PROC_ARG_STRING(procedure, "help-domain", "Help domain to use",
                       "Help domain to use", NULL, G_PARAM_READWRITE);

  GIMP_PROC_ARG_STRING(procedure, "help-locales", "Language to use",
                       "Language to use", NULL, G_PARAM_READWRITE);

  GIMP_PROC_ARG_STRING(procedure, "help-id", "Help ID to open",
                       "Help ID to open", NULL, G_PARAM_READWRITE);
...
static gboolean help_load_idle(gpointer data) {
  ...
  if (domain) {
    ...
    uri = gimp_help_domain_map(domain, locales, idle_help->help_id, progress,
                               NULL, &fatal_error);
    ...
    if (uri) {
      ...
      return_vals = gimp_pdb_run_procedure(gimp_get_pdb(), idle_help->procedure,
                                           G_TYPE_STRING, uri, G_TYPE_NONE);
...
static void help_load_progress_start(const gchar *message, gboolean cancelable,
                                     gpointer user_data) {
  gimp_progress_init(message);
}

static void help_load_progress_update(gdouble value, gpointer user_data) {
  gimp_progress_update(value);
}

static void help_load_progress_end(gpointer user_data) { gimp_progress_end(); }

static GimpHelpProgress *help_load_progress_new(void) {
  static const GimpHelpProgressVTable vtable = {help_load_progress_start,
