...
GimpHelpLocale *gimp_help_domain_lookup_locale(GimpHelpDomain *domain,
  ...
  if (domain->help_locales)
    ...
    domain->help_locales = g_hash_table_new_full(
        g_str_hash, g_str_equal, g_free, (GDestroyNotify)gimp_help_locale_free);
...
gchar *gimp_help_domain_map(GimpHelpDomain *domain, GList *help_locales,
  ...
  for (list = help_locales; list && !ref; list = list->next) {
    locale = gimp_help_domain_lookup_locale(domain, (const gchar *)list->data,
                                            progress);
  ...
  if (ref) {
    return g_strconcat(domain->help_uri, "/", locale->locale_id, "/", ref,
                       NULL);
  } else /*  try to assemble a useful error message  */
  {
    ...
    locale =
        gimp_help_domain_lookup_locale(domain, GIMP_HELP_DEFAULT_LOCALE, NULL);
    ...
    if (!domain_locale_parse(domain, locale, NULL, &error)) {
      ...
      case G_IO_ERROR_NOT_FOUND:
        if (domain->help_domain) {
          ...
          g_message("%s\n\n%s", _("The GIMP user manual is not available."),
...
static gboolean domain_locale_parse(GimpHelpDomain *domain,
  ...
  uri = g_strdup_printf("%s/%s/gimp-help.xml", domain->help_uri,
                        locale->locale_id);

  success =
      gimp_help_locale_parse(locale, uri, domain->help_domain, progress, error);
