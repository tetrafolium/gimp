...
static void circle_get_dimensions(Object_t *obj, gint *x, gint *y, gint *width,
                                  gint *height);
...
static void circle_resize(Object_t *obj, gint percentage_x, gint percentage_y) {
  ...
  circle->r = circle->r *
              ((percentage_x < percentage_y) ? percentage_x : percentage_y) /
              100;
}
...
static void r_changed_cb(GtkWidget *widget, gpointer data) {
  Object_t *obj = ((CircleProperties_t *)data)->obj;
...
static gpointer circle_create_info_widget(GtkWidget *frame) {
  ...
  g_signal_connect(props->r, "value-changed", G_CALLBACK(r_changed_cb),
                   (gpointer)props);
...
static void circle_write_ncsa(Object_t *obj, gpointer param,
                              OutputFunc_t output) {
  Circle_t *circle = ObjectToCircle(obj);
  output(param, "circle %s %d,%d %d,%d", obj->url, circle->x, circle->y,
         circle->x, circle->y + circle->r);
}

static const gchar *circle_get_icon_name(void) { return IMAP_CIRCLE; }

static gint _start_x, _start_y;

static Object_t *circle_factory_create_object1(gint x, gint y) {
  _start_x = x;
  _start_y = y;
  return create_circle(x, y, 0);
}

static void circle_factory_set_xy1(Object_t *obj, guint state, gint x, gint y) {
  Circle_t *circle = ObjectToCircle(obj);
...
static ObjectFactory_t circle_factory1 = {NULL, /* Object pointer */
                                          NULL, /* Finish func */
                                          NULL, /* Cancel func */
                                          circle_factory_create_object1,
                                          circle_factory_set_xy1};

static Object_t *circle_factory_create_object2(gint x, gint y) {
  return create_circle(x, y, 0);
}

static void circle_factory_set_xy2(Object_t *obj, guint state, gint x, gint y) {
  Circle_t *circle = ObjectToCircle(obj);
...
static ObjectFactory_t circle_factory2 = {NULL, /* Object pointer */
                                          NULL, /* Finish func */
                                          NULL, /* Cancel func */
