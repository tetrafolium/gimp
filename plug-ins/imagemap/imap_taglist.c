...
static Tag_t *tag_create(const gchar *name, const gchar *value) {
...
static Tag_t *tag_clone(Tag_t *src) {
  return tag_create(src->name, src->value);
}

static void tag_destruct(Tag_t *tag) {
  g_free(tag->name);
  g_free(tag->value);
  g_free(tag);
}

static void tag_write(Tag_t *tag) {
  printf("\"%s\"=\"%s\"\n", tag->name, tag->value);
}
...
static void taglist_remove_all(TagList_t *tlist) {
  GList *p;
...
void taglist_set(TagList_t *tlist, const gchar *name, const gchar *value) {
  GList *p;
  Tag_t *tag;
