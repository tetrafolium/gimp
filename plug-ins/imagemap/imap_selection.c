...
static void changed_cb(GtkTreeSelection *selection, gpointer param) {
...
static void selection_set_selected(Selection_t *selection, gint row) {
  GtkTreeIter iter;
...
static void object_added_cb(Object_t *obj, gpointer data) {
...
static gboolean selection_find_object(Selection_t *selection, Object_t *lookup,
                                      GtkTreeIter *iter) {
  if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(selection->store), iter)) {
    do {
      Object_t *obj;
    ...
    } while (gtk_tree_model_iter_next(GTK_TREE_MODEL(selection->store), iter));
  }
  return FALSE;
}

static void object_updated_cb(Object_t *obj, gpointer data) {
  Selection_t *selection = (Selection_t *)data;
  GtkTreeIter iter;
  ...
  if (selection_find_object(selection, obj, &iter)) {
    ...
    gtk_tree_model_row_changed(GTK_TREE_MODEL(selection->store), path, &iter);
  }
}

static void object_removed_cb(Object_t *obj, gpointer data) {
  Selection_t *selection = (Selection_t *)data;
  GtkTreeIter iter;
...
static void object_selected_cb(Object_t *obj, gpointer data) {
  Selection_t *selection = (Selection_t *)data;
  gint position = object_get_position_in_list(obj);
  selection_set_selected(selection, position);
}

static void object_moved_cb(Object_t *obj, gpointer data) {
  Selection_t *selection = (Selection_t *)data;
  selection->select_lock = TRUE;
}

static const GtkTargetEntry target_table[] = {{"STRING", 0, 1},
                                              {"text/plain", 0, 2}};

static Object_t *selection_get_object(GtkTreeModel *tree_model,
                                      GtkTreeIter *iter) {
  Object_t *obj;
...
static void handle_drop(GtkWidget *widget, GdkDragContext *context, gint x,
  ...
  if (gtk_selection_data_get_length(data) >= 0 &&
    ...
    if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(widget), x, y, &path, NULL,
                                      NULL, NULL)) {
...
static void render_image(GtkTreeViewColumn *column, GtkCellRenderer *cell,
...
static void render_nr(GtkTreeViewColumn *column, GtkCellRenderer *cell,
                      GtkTreeModel *tree_model, GtkTreeIter *iter,
                      gpointer data) {
  Object_t *obj = selection_get_object(tree_model, iter);
  gchar *scratch;
...
static void render_url(GtkTreeViewColumn *column, GtkCellRenderer *cell,
                       GtkTreeModel *tree_model, GtkTreeIter *iter,
                       gpointer data) {
  Object_t *obj = selection_get_object(tree_model, iter);
...
static void render_target(GtkTreeViewColumn *column, GtkCellRenderer *cell,
                          GtkTreeModel *tree_model, GtkTreeIter *iter,
                          gpointer data) {
  Object_t *obj = selection_get_object(tree_model, iter);
...
static void render_comment(GtkTreeViewColumn *column, GtkCellRenderer *cell,
                           GtkTreeModel *tree_model, GtkTreeIter *iter,
                           gpointer data) {
  Object_t *obj = selection_get_object(tree_model, iter);
...
Selection_t *make_selection(ObjectList_t *object_list) {
  ...
  column = gtk_tree_view_column_new_with_attributes(N_("#"), renderer, NULL);
  gtk_tree_view_column_set_cell_data_func(column, renderer, render_nr, data,
                                          NULL);
  ...
  gtk_tree_view_column_set_cell_data_func(column, renderer, render_image, data,
                                          NULL);
  ...
  renderer = gtk_cell_renderer_text_new();
  column =
      gtk_tree_view_column_new_with_attributes(_("ALT Text"), renderer, NULL);
  ...
  renderer = gtk_cell_renderer_text_new();
  column =
      gtk_tree_view_column_new_with_attributes(_("Target"), renderer, NULL);
  gtk_tree_view_column_set_cell_data_func(column, renderer, render_target, data,
                                          NULL);
  gtk_tree_view_column_set_min_width(column, 64);
  gtk_tree_view_column_set_resizable(column, TRUE);
  gtk_tree_view_column_set_alignment(column, 0.5);
  ...
  g_signal_connect(list, "button-press-event", G_CALLBACK(button_press_cb),
                   data);
  g_signal_connect(list, "button-release-event", G_CALLBACK(button_release_cb),
                   data);
...
void selection_toggle_visibility(Selection_t *selection) {
  ...
  /* Adapt to new state */
  gtk_widget_set_visible(selection->container, selection->is_visible);
}
