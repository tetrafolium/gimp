...
StatusBar_t *make_statusbar(GtkWidget *main_vbox, GtkWidget *window) {
  ...
  /* (x, y) coordinate */
  iconw = gtk_image_new_from_icon_name(IMAP_COORD, GTK_ICON_SIZE_SMALL_TOOLBAR);
  ...
  /* Dimension info */
  iconw =
      gtk_image_new_from_icon_name(IMAP_DIMENSION, GTK_ICON_SIZE_SMALL_TOOLBAR);
  gtk_box_pack_start(GTK_BOX(hbox), iconw, FALSE, FALSE, 10);
  gtk_widget_show(iconw);
...
void statusbar_set_status(StatusBar_t *statusbar, const gchar *format, ...) {
  ...
  statusbar->message_id = gtk_statusbar_push(GTK_STATUSBAR(statusbar->status),
...
void statusbar_clear_status(StatusBar_t *statusbar) {
  if (statusbar->message_id)
    gtk_statusbar_remove(GTK_STATUSBAR(statusbar->status), statusbar->status_id,
