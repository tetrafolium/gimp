...
static void draw_any_rectangle(cairo_t *cr, gint x, gint y, gint w, gint h) {
  if (w < 0) {
    x += w;
    w = -w;
  }
...
static void rectangle_draw(Object_t *obj, cairo_t *cr) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
  draw_any_rectangle(cr, rectangle->x, rectangle->y, rectangle->width,
                     rectangle->height);
}
...
static void MoveUpperSash(Object_t *obj, gint dx, gint dy) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
  rectangle->y += dy;
  rectangle->height -= dy;
}

static void MoveLeftSash(Object_t *obj, gint dx, gint dy) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
  rectangle->x += dx;
  rectangle->width -= dx;
}

static void MoveRightSash(Object_t *obj, gint dx, gint dy) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
  rectangle->width += dx;
}

static void MoveLowerSash(Object_t *obj, gint dx, gint dy) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
  rectangle->height += dy;
}

static void MoveUpperLeftSash(Object_t *obj, gint dx, gint dy) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
  rectangle->x += dx;
  rectangle->y += dy;
  rectangle->width -= dx;
...
static void MoveUpperRightSash(Object_t *obj, gint dx, gint dy) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
  rectangle->y += dy;
  rectangle->width += dx;
  rectangle->height -= dy;
...
static void MoveLowerLeftSash(Object_t *obj, gint dx, gint dy) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
  rectangle->x += dx;
  rectangle->width -= dx;
  rectangle->height += dy;
...
static void MoveLowerRightSash(Object_t *obj, gint dx, gint dy) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
  rectangle->width += dx;
  rectangle->height += dy;
}
...
static MoveSashFunc_t rectangle_near_sash(Object_t *obj, gint x, gint y) {
  ...
  else if (near_sash(rectangle->x, rectangle->y + rectangle->height / 2, x, y))
    return MoveLeftSash;
...
static void width_changed_cb(GtkWidget *widget, gpointer data) {
  Object_t *obj = ((RectangleProperties_t *)data)->obj;
...
static void height_changed_cb(GtkWidget *widget, gpointer data) {
  Object_t *obj = ((RectangleProperties_t *)data)->obj;
...
static gpointer rectangle_create_info_widget(GtkWidget *frame) {
  ...
  g_signal_connect(props->x, "value-changed", G_CALLBACK(x_changed_cb),
                   (gpointer)props);
  ...
  g_signal_connect(props->y, "value-changed", G_CALLBACK(y_changed_cb),
                   (gpointer)props);
  create_label_in_grid(grid, 1, 3, _("pixels"));

  label = create_label_in_grid(grid, 2, 0, _("_Width:"));
  props->width = create_spin_button_in_grid(grid, label, 2, 1, 1, 1, max_width);
  g_signal_connect(props->width, "value-changed", G_CALLBACK(width_changed_cb),
                   (gpointer)props);
  create_label_in_grid(grid, 2, 3, _("pixels"));

  label = create_label_in_grid(grid, 3, 0, _("_Height:"));
  props->height =
      create_spin_button_in_grid(grid, label, 3, 1, 1, 1, max_height);
...
static void rectangle_fill_info_tab(Object_t *obj, gpointer data) {
  ...
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(props->height), rectangle->height);
}
...
static void rectangle_update(Object_t *obj, gpointer data) {
  ...
  rectangle->width =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(props->width));
  rectangle->height =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(props->height));
}
...
static void rectangle_write_ncsa(Object_t *obj, gpointer param,
                                 OutputFunc_t output) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
  output(param, "rect %s %d,%d %d,%d", obj->url, rectangle->x, rectangle->y,
         rectangle->x + rectangle->width, rectangle->y + rectangle->height);
}

static const gchar *rectangle_get_icon_name(void) { return IMAP_RECTANGLE; }

static gboolean rectangle_factory_finish(Object_t *obj, gint x, gint y) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
...
static Object_t *rectangle_factory_create_object(gint x, gint y) {
  return create_rectangle(x, y, 0, 0);
}

static void rectangle_factory_set_xy(Object_t *obj, guint state, gint x,
                                     gint y) {
  Rectangle_t *rectangle = ObjectToRectangle(obj);
...
static ObjectFactory_t rectangle_factory = {
    NULL,                           /* Object pointer */
    rectangle_factory_finish, NULL, /* Cancel func */
    rectangle_factory_create_object, rectangle_factory_set_xy};
