...
static void select_cb(GtkWidget *dialog, gint response_id,
...
static void browse_cb(GtkWidget *widget, BrowseWidget_t *browse) {
  if (!browse->file_chooser) {
    GtkWidget *dialog;

    dialog = browse->file_chooser = gtk_file_chooser_dialog_new(
        browse->name, GTK_WINDOW(gtk_widget_get_toplevel(widget)),
    ...
    g_signal_connect(dialog, "destroy", G_CALLBACK(gtk_widget_destroyed),
                     &dialog);
    g_signal_connect(dialog, "response", G_CALLBACK(select_cb), browse);
  }
...
BrowseWidget_t *browse_widget_new(const gchar *name) {
  ...
  gtk_drag_dest_set(browse->file, GTK_DEST_DEFAULT_ALL, target_table, 2,
                    GDK_ACTION_COPY);
  g_signal_connect(browse->file, "drag-data-received", G_CALLBACK(handle_drop),
                   NULL);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(browse_cb), (gpointer)browse);
