...
static void gimp_guides_ok_cb(gpointer data) {
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(param->right_border)))
    vguides = g_slist_append(vguides, GINT_TO_POINTER(gimp_image_width(image)));
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(param->lower_border)))
    hguides =
        g_slist_append(hguides, GINT_TO_POINTER(gimp_image_height(image)));
  ...
  for (hg = hguides; hg && hg->next; hg = (all) ? hg->next : hg->next->next) {
    gint y = GPOINTER_TO_INT(hg->data);
    gint height = GPOINTER_TO_INT(hg->next->data) - y;
    for (vg = vguides; vg && vg->next; vg = (all) ? vg->next : vg->next->next) {
...
static GimpGuidesDialog_t *make_gimp_guides_dialog(void) {
...
static void init_gimp_guides_dialog(GimpGuidesDialog_t *dialog,
                                    ObjectList_t *list,
                                    GimpDrawable *drawable) {
  dialog->list = list;
...
static void do_create_gimp_guides_dialog(ObjectList_t *list,
                                         GimpDrawable *drawable) {
...
static CommandClass_t gimp_guides_command_class = {
    ...
    gimp_guides_command_execute, NULL, /* guides_command_undo */
...
Command_t *gimp_guides_command_new(ObjectList_t *list, GimpDrawable *drawable) {
