...
static void polygon_get_dimensions(Object_t *obj, gint *x, gint *y, gint *width,
                                   gint *height);
static void polygon_resize(Object_t *obj, gint percentage_x, gint percentage_y);
...
static ObjectClass_t polygon_class = {N_("_Polygon"),
                                      NULL, /* info_dialog */
...
static void polygon_free_list(Polygon_t *polygon) {
...
static void move_sash(Object_t *obj, gint dx, gint dy) {
  _sash_point->x += dx;
  _sash_point->y += dy;
}
...
static gboolean right_intersect(GdkPoint *p1, GdkPoint *p2, gint x, gint y) {
  gint dx = p2->x - p1->x;
  gint dy = p2->y - p1->y;
...
static void select_row_cb(GtkTreeSelection *selection,
...
static void set_buttons_sensitivity(PolygonProperties_t *data) {
  gint rows = gtk_tree_model_iter_n_children(GTK_TREE_MODEL(data->store), NULL);
...
static void insert_button_clicked(GtkWidget *widget,
                                  PolygonProperties_t *data) {
  GtkTreeIter iter;
  GtkTreeModel *model = GTK_TREE_MODEL(data->store);
  ...
  if (gtk_tree_selection_get_selected(data->selection, &model, &iter)) {
    ...
    polygon->points =
        g_list_insert_before(polygon->points, here, new_point(x, y));
    polygon_fill_info_tab(data->obj, data);
  }
}

static void append_button_clicked(GtkWidget *widget,
                                  PolygonProperties_t *data) {
  gint x = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(data->x));
  gint y = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(data->y));
...
static void remove_button_clicked(GtkWidget *widget,
                                  PolygonProperties_t *data) {
  GtkTreeIter iter;
  GtkTreeModel *model = GTK_TREE_MODEL(data->store);
...
static void x_changed_cb(GtkWidget *widget, gpointer data) {
...
static void y_changed_cb(GtkWidget *widget, gpointer data) {
  Object_t *obj = ((PolygonProperties_t *)data)->obj;
...
static void render_x(GtkTreeViewColumn *column, GtkCellRenderer *cell,
...
static void render_y(GtkTreeViewColumn *column, GtkCellRenderer *cell,
                     GtkTreeModel *tree_model, GtkTreeIter *iter,
                     gpointer data) {
  GdkPoint *point;
  gchar scratch[16];
...
static gpointer polygon_create_info_widget(GtkWidget *frame) {
  ...
  column =
      gtk_tree_view_column_new_with_attributes(_("x (pixels)"), renderer, NULL);
  gtk_tree_view_column_set_cell_data_func(column, renderer, render_x, props,
                                          NULL);
  ...
  renderer = gtk_cell_renderer_text_new();
  column =
      gtk_tree_view_column_new_with_attributes(_("y (pixels)"), renderer, NULL);
  gtk_tree_view_column_set_cell_data_func(column, renderer, render_y, props,
                                          NULL);
  gtk_tree_view_column_set_alignment(column, 0.5);
  gtk_tree_view_append_column(GTK_TREE_VIEW(view), column);
  ...
  props->x = create_spin_button_in_grid(grid, label, 0, 1, 1, 0, max_width - 1);
  g_signal_connect(props->x, "changed", G_CALLBACK(x_changed_cb),
                   (gpointer)props);
  ...
  props->y =
      create_spin_button_in_grid(grid, label, 1, 1, 1, 0, max_height - 1);
  g_signal_connect(props->y, "changed", G_CALLBACK(y_changed_cb),
                   (gpointer)props);
  ...
  g_signal_connect(props->update, "clicked", G_CALLBACK(update_button_clicked),
                   props);
  ...
  g_signal_connect(props->insert, "clicked", G_CALLBACK(insert_button_clicked),
                   props);
  ...
  g_signal_connect(props->append, "clicked", G_CALLBACK(append_button_clicked),
                   props);
  ...
  g_signal_connect(props->remove, "clicked", G_CALLBACK(remove_button_clicked),
                   props);
  ...
  g_signal_connect(props->selection, "changed", G_CALLBACK(select_row_cb),
                   props);
...
static void polygon_update_info_widget(Object_t *obj, gpointer data) {
  ...
  if (gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(props->store), &iter, NULL,
                                    _sash_index)) {
...
static gboolean point_near_edge(GdkPoint *first, GdkPoint *second, gint x,
                                gint y) {
  gint den, nom;
  gdouble u;
...
static gint polygon_near_edge(Object_t *obj, gint x, gint y) {
  Polygon_t *polygon = ObjectToPolygon(obj);
...
static void polygon_handle_popup(GdkEventButton *event, gboolean near_sash,
                                 gboolean near_edge) {
...
static const gchar *polygon_get_icon_name(void) { return IMAP_POLYGON; }

static GList *_prev_link;

static gboolean polygon_factory_finish(Object_t *obj, gint x, gint y) {
  Polygon_t *polygon = ObjectToPolygon(obj);
...
static gboolean polygon_factory_cancel(GdkEventButton *event, Object_t *obj) {
...
static Object_t *polygon_factory_create_object(gint x, gint y) {
  GList *points;
...
static void polygon_factory_set_xy(Object_t *obj, guint state, gint x, gint y) {
  Polygon_t *polygon = ObjectToPolygon(obj);
...
static ObjectFactory_t polygon_factory = {
    NULL, /* Object pointer */
    polygon_factory_finish, polygon_factory_cancel,
    polygon_factory_create_object, polygon_factory_set_xy};

ObjectFactory_t *get_polygon_factory(guint state) { return &polygon_factory; }
