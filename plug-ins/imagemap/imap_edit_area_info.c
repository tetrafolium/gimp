...
static gchar *relative_filter(const char *name, gpointer data) {
...
static void url_changed(GtkWidget *widget, gpointer data) {
  AreaInfoDialog_t *param = (AreaInfoDialog_t *)data;
...
static void set_url(GtkWidget *widget, AreaInfoDialog_t *param,
                    const gchar *prefix) {
  if (callback_lock) {
    callback_lock = FALSE;
  } else {
...
static void select_web_cb(GtkWidget *widget, AreaInfoDialog_t *param) {
  set_url(widget, param, "http://");
}

static void select_ftp_cb(GtkWidget *widget, AreaInfoDialog_t *param) {
  set_url(widget, param, "ftp://");
}

static void select_gopher_cb(GtkWidget *widget, AreaInfoDialog_t *param) {
  set_url(widget, param, "gopher://");
}

static void select_other_cb(GtkWidget *widget, AreaInfoDialog_t *param) {
  set_url(widget, param, "");
}

static void select_file_cb(GtkWidget *widget, AreaInfoDialog_t *param) {
  set_url(widget, param, "file:/");
}

static void select_wais_cb(GtkWidget *widget, AreaInfoDialog_t *param) {
  set_url(widget, param, "wais://");
}

static void select_telnet_cb(GtkWidget *widget, AreaInfoDialog_t *param) {
  set_url(widget, param, "telnet://");
}

static void select_email_cb(GtkWidget *widget, AreaInfoDialog_t *param) {
  set_url(widget, param, "mailto:");
}

static void append_page(GtkWidget *notebook, GtkWidget *page,
                        const gchar *icon_name, const gchar *label_name) {
...
static void create_link_tab(AreaInfoDialog_t *dialog, GtkWidget *notebook) {
  ...
  dialog->web_site =
      create_radio_button_in_grid(subgrid, NULL, 0, 0, _("_Web Site"));
  g_signal_connect(dialog->web_site, "toggled", G_CALLBACK(select_web_cb),
                   (gpointer)dialog);
  group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(dialog->web_site));

  dialog->ftp_site =
      create_radio_button_in_grid(subgrid, group, 0, 1, _("_Ftp Site"));
  g_signal_connect(dialog->ftp_site, "toggled", G_CALLBACK(select_ftp_cb),
                   (gpointer)dialog);
  group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(dialog->ftp_site));

  dialog->gopher =
      create_radio_button_in_grid(subgrid, group, 0, 2, _("_Gopher"));
  g_signal_connect(dialog->gopher, "toggled", G_CALLBACK(select_gopher_cb),
                   (gpointer)dialog);
  group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(dialog->gopher));

  dialog->other =
      create_radio_button_in_grid(subgrid, group, 0, 3, _("Ot_her"));
  g_signal_connect(dialog->other, "toggled", G_CALLBACK(select_other_cb),
                   (gpointer)dialog);
  group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(dialog->other));

  dialog->file = create_radio_button_in_grid(subgrid, group, 1, 0, _("F_ile"));
  g_signal_connect(dialog->file, "toggled", G_CALLBACK(select_file_cb),
                   (gpointer)dialog);
  group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(dialog->file));

  dialog->wais = create_radio_button_in_grid(subgrid, group, 1, 1, _("WAI_S"));
  g_signal_connect(dialog->wais, "toggled", G_CALLBACK(select_wais_cb),
                   (gpointer)dialog);
  group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(dialog->wais));

  dialog->telnet =
      create_radio_button_in_grid(subgrid, group, 1, 2, _("Tel_net"));
  g_signal_connect(dialog->telnet, "toggled", G_CALLBACK(select_telnet_cb),
                   (gpointer)dialog);
  group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(dialog->telnet));

  dialog->email =
      create_radio_button_in_grid(subgrid, group, 1, 3, _("e-_mail"));
  g_signal_connect(dialog->email, "toggled", G_CALLBACK(select_email_cb),
                   (gpointer)dialog);

  label = create_label_in_grid(
      grid, 2, 0, _("_URL to activate when this area is clicked: (required)"));
  ...
  g_signal_connect(dialog->url, "changed", G_CALLBACK(url_changed), dialog);
  gtk_label_set_mnemonic_widget(GTK_LABEL(label), dialog->url);

  dialog->relative_link =
      create_check_button_in_grid(grid, 4, 0, _("Relati_ve link"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog->relative_link), TRUE);
...
static void geometry_changed(Object_t *obj, gpointer data) {
...
static void toggle_preview_cb(GtkWidget *widget, AreaInfoDialog_t *param) {
  param->preview = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  edit_area_info_dialog_emit_geometry_signal(param);
}

static void create_info_tab(AreaInfoDialog_t *dialog, GtkWidget *notebook) {
  ...
  g_signal_connect(preview, "toggled", G_CALLBACK(toggle_preview_cb),
                   (gpointer)dialog);
...
static void create_java_script_tab(AreaInfoDialog_t *dialog,
                                   GtkWidget *notebook) {
...
static gboolean object_was_changed(AreaInfoDialog_t *dialog) {
...
static void edit_area_apply_cb(gpointer data) {
  AreaInfoDialog_t *param = (AreaInfoDialog_t *)data;
  ...
  object_set_mouse_over(obj, gtk_entry_get_text(GTK_ENTRY(param->mouse_over)));
...
static void edit_area_ok_cb(gpointer data) {
  AreaInfoDialog_t *param = (AreaInfoDialog_t *)data;
  Object_t *obj = param->obj;
...
static void edit_area_cancel_cb(gpointer data) {
  AreaInfoDialog_t *dialog = (AreaInfoDialog_t *)data;
  Object_t *obj = dialog->obj;
...
static void switch_page(GtkWidget *widget, gpointer page, gint page_num,
                        gpointer data) {
  AreaInfoDialog_t *param = (AreaInfoDialog_t *)data;
  if (page_num == 0) {
    gtk_widget_grab_focus(param->url);
...
AreaInfoDialog_t *create_edit_area_info_dialog(Object_t *obj) {
  ...
  g_signal_connect_after(notebook, "switch-page", G_CALLBACK(switch_page),
                         (gpointer)data);
