...
static gint parse_map_type(void) {
...
static gint parse_yes_no(void) {
  char *token = strtok(NULL, " )");
  return (gint)strcmp(token, "no");
}

static gint parse_int(void) {
  char *token = strtok(NULL, " )");
  return (gint)atoi(token);
}

static void parse_color(GdkRGBA *color) {
...
static void parse_mru_entry(void) {
  char *filename = strtok(NULL, " )");
  mru_add(get_mru(), filename);
}

static void parse_line(PreferencesData_t *data, char *line) {
  char *token;
...
static void preferences_ok_cb(gpointer data) {
  ...
  old_data->show_area_handle =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(param->show_area_handle));
  old_data->keep_circles_round = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(param->keep_circles_round));
  old_data->show_url_tip =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(param->show_url_tip));
  old_data->use_doublesized =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(param->use_doublesized));
...
static void get_button_color(GtkWidget *button, GdkRGBA *color) {
  GimpRGB rgb;
...
static void set_button_color(GtkWidget *button, GdkRGBA *color) {
  gimp_color_button_set_color(GIMP_COLOR_BUTTON(button), (GimpRGB *)color);
}

static void set_button_colors(PreferencesDialog_t *dialog,
                              ColorSelData_t *colors) {
...
static GtkWidget *create_tab(GtkWidget *notebook, const gchar *label) {
  GtkWidget *grid;
  GtkWidget *vbox;
...
static void create_general_tab(PreferencesDialog_t *data, GtkWidget *notebook) {
  ...
  data->use_doublesized = create_check_button_in_grid(
      grid, 6, 0, _("_Use double-sized grab handles"));
  gtk_widget_show(frame);
}

static void create_menu_tab(PreferencesDialog_t *data, GtkWidget *notebook) {
  GtkWidget *grid = create_tab(notebook, _("Menu"));
  GtkWidget *label;

  label =
      create_label_in_grid(grid, 0, 0, _("Number of _undo levels (1 - 99):"));
  data->undo_levels = create_spin_button_in_grid(grid, label, 0, 1, 1, 1, 99);

  label =
      create_label_in_grid(grid, 1, 0, _("Number of M_RU entries (1 - 16):"));
  data->mru_size = create_spin_button_in_grid(grid, label, 1, 1, 1, 1, 16);
}

static GtkWidget *create_color_field(PreferencesDialog_t *data, GtkWidget *grid,
                                     gint row, gint col) {
...
static void create_colors_tab(PreferencesDialog_t *data, GtkWidget *notebook) {
...
static void create_contiguous_regions_tab(PreferencesDialog_t *data,
                                          GtkWidget *notebook) {
  GtkWidget *grid = create_tab(notebook, _("Co_ntiguous Region"));
  GtkWidget *label;

  label = create_label_in_grid(grid, 0, 0, _("_Threshold:"));
  data->auto_convert =
      create_check_button_in_grid(grid, 1, 0, _("_Automatically convert"));
}
#endif

static PreferencesDialog_t *create_preferences_dialog(void) {
...
void do_preferences_dialog(void) {
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog->prompt_for_area_info),
