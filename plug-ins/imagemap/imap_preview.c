...
#define PREVIEW_MASK                                                           \
  (GDK_EXPOSURE_MASK | GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK |       \
   GDK_BUTTON_RELEASE_MASK | GDK_BUTTON_MOTION_MASK | GDK_KEY_PRESS_MASK |     \
...
static Preview_t *preview_user_data(GtkWidget *preview) {
...
static void render_background(Preview_t *preview_base) {
  GtkWidget *preview = preview_base->preview;

  gimp_preview_area_fill(GIMP_PREVIEW_AREA(preview), 0, 0, G_MAXINT, G_MAXINT,
                         255, 255, 255);
}

static void render_rgb_image(Preview_t *preview_base, GimpDrawable *drawable) {
  GeglBuffer *buffer;
  guchar *dest_buffer;
  ...
  gegl_buffer_get(buffer, GEGL_RECTANGLE(0, 0, pwidth, pheight),
                  ...
                  babl_format("R'G'B'A u8"), dest_buffer, GEGL_AUTO_ROWSTRIDE,
                  GEGL_ABYSS_NONE);

  g_object_unref(buffer);

  gimp_preview_area_draw(GIMP_PREVIEW_AREA(preview), 0, 0, pwidth, pheight,
                         GIMP_RGBA_IMAGE, dest_buffer, pwidth * 4);

  g_free(dest_buffer);
}

static void render_preview(Preview_t *preview_base, GimpDrawable *drawable) {
  render_background(preview_base);
  render_rgb_image(preview_base, drawable);
}

static gboolean arrow_cb(GtkWidget *widget, GdkEventButton *event,
                         gpointer data) {
  if (event->button == 1)
...
static gboolean preview_draw(GtkWidget *widget, cairo_t *cr) {
...
void preview_set_tmp_obj(Object_t *obj) { _tmp_obj = obj; }
...
static void preview_size_allocate(GtkWidget *widget, GtkAllocation *allocation,
...
static void scroll_adj_changed(GtkAdjustment *adj, GimpRuler *ruler) {
  gimp_ruler_set_range(ruler, gtk_adjustment_get_value(adj),
...
Preview_t *make_preview(GimpDrawable *drawable) {
  ...
  g_signal_connect_after(preview, "draw", G_CALLBACK(preview_draw), data);
  g_signal_connect(preview, "size-allocate", G_CALLBACK(preview_size_allocate),
                   data);

  /* Handle drop of links in preview widget */
  gtk_drag_dest_set(preview, GTK_DEST_DEFAULT_ALL, target_table, 2,
                    GDK_ACTION_COPY);

  g_signal_connect(preview, "drag-data-received", G_CALLBACK(handle_drop),
                   NULL);
  ...
  g_signal_connect(button, "button-press-event", G_CALLBACK(arrow_cb), NULL);

  arrow = gtk_image_new_from_icon_name(GIMP_ICON_GO_NEXT, GTK_ICON_SIZE_BUTTON);
  ...
  g_signal_connect_swapped(
      preview, "motion-notify-event",
      G_CALLBACK(GTK_WIDGET_GET_CLASS(ruler)->motion_notify_event), ruler);
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(window), GTK_POLICY_NEVER,
                                 GTK_POLICY_NEVER);
  ...
  g_signal_connect(hadj, "changed", G_CALLBACK(scroll_adj_changed),
                   data->hruler);
  g_signal_connect(hadj, "value-changed", G_CALLBACK(scroll_adj_changed),
                   data->hruler);

  vadj = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(window));

  g_signal_connect(vadj, "changed", G_CALLBACK(scroll_adj_changed),
                   data->vruler);
  g_signal_connect(vadj, "value-changed", G_CALLBACK(scroll_adj_changed),
                   data->vruler);
