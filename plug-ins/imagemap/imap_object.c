...
static gpointer object_list_callback_add(ObjectListCallback_t *list,
                                         ObjectListCallbackFunc_t func,
                                         gpointer data) {
...
static void object_list_callback_remove(ObjectListCallback_t *list,
                                        gpointer id) {
  list->list = g_list_remove(list->list, id);
}

static void object_list_callback_call(ObjectListCallback_t *list,
                                      Object_t *obj) {
  GList *p;
...
static void object_destruct(Object_t *obj) {
...
static Object_t *object_copy(Object_t *src, Object_t *des) {
  des->class = src->class;
...
Object_t *object_assign(Object_t *obj, Object_t *des) {
...
void object_lock(Object_t *obj) { obj->locked = TRUE; }

void object_unlock(Object_t *obj) { obj->locked = FALSE; }
...
void do_object_locked_dialog(void) {
  ...
  if (!dialog) {
    ...
    default_dialog_set_label(dialog,
...
static Object_t *object_factory_create_object(ObjectFactory_t *factory, gint x,
                                              gint y) {
  return factory->obj = factory->create_object(x, y);
}

static gboolean button_motion(GtkWidget *widget, GdkEventMotion *event,
  ...
  if (obj) {
    if (event->button == 1) {
      if (!factory->finish || factory->finish(obj, x, y)) {
        g_signal_handlers_disconnect_by_func(widget, button_motion, factory);
    ...
    if (event->button == 1) {
      ...
      g_signal_connect(widget, "motion-notify-event", G_CALLBACK(button_motion),
                       factory);
    }
  }
  return FALSE;
...
ObjectList_t *make_object_list(void) { return g_new0(ObjectList_t, 1); }
...
ObjectList_t *get_paste_buffer(void) { return _paste_buffer; }
...
gint object_list_select_region(ObjectList_t *list, gint x, gint y, gint width,
  ...
  for (p = list->list; p; p = p->next) {
    ...
    if (obj_x >= x && obj_x + obj_width <= x + width && obj_y >= y &&
        obj_y + obj_height <= y + height) {
      object_select(obj);
      count++;
    }
  }
...
static void object_list_swap_prev(ObjectList_t *list, GList *p) {
...
static void object_list_swap_next(ObjectList_t *list, GList *p) {
  gpointer swap = p->data;
  p->data = p->next->data;
  p->next->data = swap;
  object_list_callback_call(&list->move_cb, (Object_t *)p->data);
...
static void write_xml_attrib(const gchar *attrib, const gchar *value,
