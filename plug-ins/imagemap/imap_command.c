...
static void command_list_callback_add(CommandListCallback_t *list,
...
static void command_list_callback_call(CommandListCallback_t *list,
                                       Command_t *command) {
  GList *p;
...
static void command_list_clear(CommandList_t *list) {
  GList *p;
  for (p = list->list; p; p = p->next)
...
void command_list_remove_all(void) { command_list_clear(&_command_list); }

static void _command_list_add(CommandList_t *list, Command_t *command) {
  GList *p, *q;
...
static Command_t *command_list_start(CommandList_t *list, const gchar *name) {
...
static void command_list_end(CommandList_t *list) {
...
void subcommand_end(void) { command_list_end(_current_command_list); }

static void _command_list_set_undo_level(CommandList_t *list, gint level) {
  ...
  if (diff > 0) {
    ...
    for (p = g_list_last(list->list); diff && p != list->undo; p = q, diff--) {
      Command_t *curr = (Command_t *)p->data;
      q = p->prev;
      command_destruct(curr);
      list->list = g_list_remove_link(list->list, p);
    }
    command_list_callback_call(&list->update_cb, (Command_t *)list->undo->data);
  }
...
static void command_list_execute(CommandList_t *list) {
  GList *p;
  for (p = list->list; p; p = p->next) {
...
void last_command_undo(void) { command_list_undo(&_command_list); }
...
void last_command_redo(void) { command_list_redo(&_command_list); }
...
static CommandClass_t basic_command_class = {
    ...
    basic_command_execute, NULL, NULL /* basic_command_redo */
};
