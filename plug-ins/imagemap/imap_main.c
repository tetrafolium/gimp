...
static GimpValueArray *imap_run(GimpProcedure *procedure, GimpRunMode run_mode,
...
static PreferencesData_t _preferences = {CSIM,
                                         TRUE,
                                         FALSE,
                                         TRUE,
                                         TRUE,
...
static void imap_class_init(ImapClass *klass) {
...
static void imap_init(Imap *imap) {}
...
static GimpProcedure *imap_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, imap_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(
        procedure, N_("Create a clickable imagemap"), NULL, name);
    gimp_procedure_set_attribution(procedure, "Maurits Rijk", "Maurits Rijk",
                                   "1998-2005");
  }
...
GtkWidget *get_dialog(void) { return _dlg; }
...
MapInfo_t *get_map_info(void) { return &_map_info; }

PreferencesData_t *get_preferences(void) { return &_preferences; }
...
gint get_image_width(void) { return _image_width; }

gint get_image_height(void) { return _image_height; }

void set_busy_cursor(void) { preview_set_cursor(_preview, GDK_WATCH); }
...
gint get_real_coord(gint coord) { return GET_REAL_COORD(coord); }
...
void draw_rectangle(cairo_t *cr, gboolean filled, gint x, gint y, gint width,
  ...
  cairo_rectangle(cr, ZOOMED(x) + (filled ? 0. : .5),
                  ZOOMED(y) + (filled ? 0. : .5), ZOOMED(width),
                  ZOOMED(height));
  if (filled)
    cairo_fill(cr);
  else
...
void preview_redraw(void) { gtk_widget_queue_draw(_preview->preview); }
...
const gchar *get_image_name(void) { return _image_name; }

const char *get_filename(void) { return _filename; }

static gboolean arrow_on_button_press(GtkWidget *widget, GdkEventButton *event,
                                      gpointer data) {
...
static void set_arrow_func(void) {
  _button_press_func = arrow_on_button_press;
  _cursor = GDK_TOP_LEFT_ARROW;
}

static void set_object_func(gboolean (*func)(GtkWidget *, GdkEventButton *,
                                             gpointer),
                            gpointer param) {
...
void add_shape(Object_t *obj) { object_list_append(_shapes, obj); }

ObjectList_t *get_shapes(void) { return _shapes; }

void update_shape(Object_t *obj) { object_list_update(_shapes, obj); }

void do_edit_selected_shape(void) { object_list_edit_selected(_shapes); }
...
static void set_all_sensitivities(void) {
  gint count = object_list_nr_selected(_shapes);
  menu_shapes_selected(count);
}

static void main_set_title(const char *filename) {
  char *title, *p;

  g_strreplace(&_filename, filename);
  p = filename ? g_filename_display_basename(filename)
               : (gchar *)_("<Untitled>");
...
void main_clear_dimension(void) { statusbar_clear_dimension(_statusbar); }

void show_url(void) { _show_url = TRUE; }
...
void draw_shapes(cairo_t *cr) { object_list_draw(_shapes, cr); }

static void clear_map_info(void) {
...
static void do_data_changed_dialog(void (*continue_cb)(gpointer),
                                   gpointer param) {
  GtkWidget *dialog = gtk_message_dialog_new(
      NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_QUESTION,
      GTK_BUTTONS_YES_NO, _("Some data has been changed!"));
...
static void check_if_changed(void (*func)(gpointer), gpointer param) {
...
static void really_close(gpointer data) { close_current(); }

void do_close(void) { check_if_changed(really_close, NULL); }

static void really_quit(gpointer data) {
  preferences_save(&_preferences);
  run_flag = 1;
  gtk_widget_destroy(_dlg);
}

void do_quit(void) { check_if_changed(really_quit, NULL); }
...
static void write_cern_comment(gpointer param, OutputFunc_t output) {
  output(param, "rect (4096,4096) (4096,4096) imap:#$");
}

static void save_as_cern(gpointer param, OutputFunc_t output) {
  char *p;
  gchar *description;
  gchar *next_token;
...
static void save_as_csim(gpointer param, OutputFunc_t output) {
  char *p;
  gchar *description;
  ...
  output(param,
         ...
         "usemap=\"#%s\" />\n\n",
         _map_info.image_name, _image_width, _image_height, _map_info.title);
...
static void save_as_ncsa(gpointer param, OutputFunc_t output) {
  char *p;
  gchar *description;
...
static void save_to_file(gpointer param, const char *format, ...)
    G_GNUC_PRINTF(2, 3);
...
static void do_image_size_changed_dialog(void) {
  GtkWidget *dialog = gtk_message_dialog_new_with_markup(
      NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_QUESTION,
      GTK_BUTTONS_YES_NO,
      "<span weight=\"bold\" size=\"larger\">%s</span>\n\n%s",
      _("Image size has changed."), _("Resize area's?"));
...
static void really_load(gpointer data) {
...
void toggle_area_list(void) { selection_toggle_visibility(_selection); }

static gboolean close_callback(GtkWidget *widget, gpointer data) {
  do_quit();
  return TRUE;
}

static gboolean preview_move(GtkWidget *widget, GdkEventMotion *event) {
  gint x = GET_REAL_COORD((gint)event->x);
  gint y = GET_REAL_COORD((gint)event->y);
...
static void preview_enter(GtkWidget *widget, GdkEventCrossing *event) {
  preview_set_cursor(_preview, _cursor);
}

static void preview_leave(GtkWidget *widget, GdkEventCrossing *event) {
  gdk_window_set_cursor(gtk_widget_get_window(_dlg), NULL);
...
static void move_sash_selected_objects(gint dx, gint dy, gboolean fast) {
  if (fast) {
    dx *= 5;
    dy *= 5;
  }
...
static void move_selected_objects(gint dx, gint dy, gboolean fast) {
  if (fast) {
    dx *= 5;
    dy *= 5;
  }
...
static gboolean key_timeout_cb(gpointer data) {
  switch (_keyval) {
  case GDK_KEY_Left:
  case GDK_KEY_Right:
  case GDK_KEY_Up:
...
static void geometry_changed(Object_t *obj, gpointer data) { preview_redraw(); }

static void data_changed(Object_t *obj, gpointer data) {
  preview_redraw();
  set_all_sensitivities();
}

static void data_selected(Object_t *obj, gpointer data) {
  set_all_sensitivities();
}

void imap_help(void) { gimp_standard_help_func("plug-in-imagemap", NULL); }

void do_cut(void) { command_execute(cut_command_new(_shapes)); }

void do_copy(void) { command_execute(copy_command_new(_shapes)); }

void do_paste(void) { command_execute(paste_command_new(_shapes)); }

void do_select_all(void) { command_execute(select_all_command_new(_shapes)); }

void do_deselect_all(void) {
  command_execute(unselect_all_command_new(_shapes, NULL));
}

void do_clear(void) { command_execute(clear_command_new(_shapes)); }
...
static Command_t *factory_move_up(void) { return move_up_command_new(_shapes); }
