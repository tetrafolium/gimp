...
static CommandClass_t copy_object_command_class = {
    copy_object_command_destruct, copy_object_command_execute,
    copy_object_command_undo, NULL /* copy_object_command_redo */
};
...
Command_t *copy_object_command_new(Object_t *obj) {
  ...
  return command_init(&command->parent, _("Copy"), &copy_object_command_class);
}
...
static CmdExecuteValue_t copy_object_command_execute(Command_t *parent) {
  ...
  command->paste_buffer = object_list_copy(command->paste_buffer, paste_buffer);
  clear_paste_buffer();
