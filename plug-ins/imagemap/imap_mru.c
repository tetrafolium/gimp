...
static void mru_remove_link(MRU_t *mru, GList *link) {
  if (link) {
    g_free(link->data);
...
static GList *mru_find_link(MRU_t *mru, const gchar *filename) {
...
void mru_set_first(MRU_t *mru, const gchar *filename) {
  ...
  if (link)
    mru->list = g_list_prepend(g_list_remove_link(mru->list, link), link->data);
  else
