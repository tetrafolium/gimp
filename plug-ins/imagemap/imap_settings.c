...
static void settings_ok_cb(gpointer data) {
  SettingsDialog_t *param = (SettingsDialog_t *)data;
  MapInfo_t *info = get_map_info();
  gchar *description;
  GtkTextIter start, end;

  g_strreplace(&info->image_name,
               gtk_entry_get_text(GTK_ENTRY(param->imagename->file)));
  ...
  description =
      gtk_text_buffer_get_text(param->description, &start, &end, FALSE);
...
static SettingsDialog_t *create_settings_dialog(void) {
