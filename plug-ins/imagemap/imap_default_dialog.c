...
DefaultDialog_t *make_default_dialog(const gchar *title) {
  ...
  data->dialog = gimp_dialog_new(title, PLUG_IN_ROLE, get_dialog(), 0,
                                 gimp_standard_help_func, PLUG_IN_PROC, NULL);

  data->apply = gtk_dialog_add_button(GTK_DIALOG(data->dialog), _("_Apply"),
                                      GTK_RESPONSE_APPLY);

  data->cancel = gtk_dialog_add_button(GTK_DIALOG(data->dialog), _("_Cancel"),
                                       GTK_RESPONSE_CANCEL);

  data->ok = gtk_dialog_add_button(GTK_DIALOG(data->dialog), _("_OK"),
                                   GTK_RESPONSE_OK);
  ...
  g_signal_connect(data->dialog, "response", G_CALLBACK(dialog_response), data);
  ...
  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(data->dialog))),
