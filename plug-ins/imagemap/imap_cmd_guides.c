...
static void guides_ok_cb(gpointer data) {
  ...
  hspace =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(param->horz_spacing));
  vspace =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(param->vert_spacing));
...
static void recalc_bounds(GtkWidget *widget, gpointer data) {
  GuidesDialog_t *param = (GuidesDialog_t *)data;
  gint width, height, left, top, hspace, vspace, rows, cols;
  ...
  bounds =
      g_strdup_printf(_("Resulting Guide Bounds: %d,%d to %d,%d (%d areas)"),
...
static GuidesDialog_t *make_guides_dialog(void) {
  ...
  gtk_box_pack_start(GTK_BOX(dialog->vbox), data->image_dimensions, FALSE,
                     FALSE, 0);
  gtk_widget_show(data->image_dimensions);

  data->guide_bounds = gtk_label_new("");
  gtk_label_set_xalign(GTK_LABEL(data->guide_bounds), 0.0);
  gtk_box_pack_start(GTK_BOX(dialog->vbox), data->guide_bounds, FALSE, FALSE,
                     0);
  ...
  g_signal_connect(data->width, "changed", G_CALLBACK(recalc_bounds),
                   (gpointer)data);

  label = create_label_in_grid(grid, 0, 2, _("_Left start at:"));
  data->left = create_spin_button_in_grid(grid, label, 0, 3, 0, 0, 100);
  g_signal_connect(data->left, "changed", G_CALLBACK(recalc_bounds),
                   (gpointer)data);

  label = create_label_in_grid(grid, 1, 0, _("_Height:"));
  data->height = create_spin_button_in_grid(grid, label, 1, 1, 32, 1, 100);
  g_signal_connect(data->height, "changed", G_CALLBACK(recalc_bounds),
                   (gpointer)data);

  label = create_label_in_grid(grid, 1, 2, _("_Top start at:"));
  data->top = create_spin_button_in_grid(grid, label, 1, 3, 0, 0, 100);
  g_signal_connect(data->top, "changed", G_CALLBACK(recalc_bounds),
                   (gpointer)data);

  label = create_label_in_grid(grid, 2, 0, _("_Horz. spacing:"));
  data->horz_spacing = create_spin_button_in_grid(grid, label, 2, 1, 0, 0, 100);
  g_signal_connect(data->horz_spacing, "changed", G_CALLBACK(recalc_bounds),
                   (gpointer)data);

  label = create_label_in_grid(grid, 2, 2, _("_No. across:"));
  data->no_across = create_spin_button_in_grid(grid, label, 2, 3, 0, 0, 100);
  g_signal_connect(data->no_across, "changed", G_CALLBACK(recalc_bounds),
                   (gpointer)data);

  label = create_label_in_grid(grid, 3, 0, _("_Vert. spacing:"));
  data->vert_spacing = create_spin_button_in_grid(grid, label, 3, 1, 0, 0, 100);
  g_signal_connect(data->vert_spacing, "changed", G_CALLBACK(recalc_bounds),
                   (gpointer)data);

  label = create_label_in_grid(grid, 3, 2, _("No. _down:"));
  data->no_down = create_spin_button_in_grid(grid, label, 3, 3, 0, 0, 100);
  g_signal_connect(data->no_down, "changed", G_CALLBACK(recalc_bounds),
                   (gpointer)data);
...
static void init_guides_dialog(GuidesDialog_t *dialog, ObjectList_t *list) {
  gchar *dimension;

  dialog->list = list;
  dimension = g_strdup_printf(_("Image dimensions: %d × %d"), get_image_width(),
                              get_image_height());
...
static void do_create_guides_dialog_local(ObjectList_t *list) {
...
static CommandClass_t guides_command_class = {
    ...
    guides_command_execute, NULL, /* guides_command_undo */
