...
static void grid_settings_ok_cb(gpointer data) {
  GridDialog_t *param = (GridDialog_t *)data;
  gboolean new_snap;

  new_snap = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(param->snap));
  grid_width = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(param->width));
  grid_height =
      gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(param->height));
  grid_left = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(param->left));
  grid_top = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(param->top));
...
static void snap_toggled_cb(GtkWidget *widget, gpointer data) {
  GridDialog_t *param = (GridDialog_t *)data;
...
static void type_toggled_cb(GtkWidget *widget, gpointer data) {
...
static void toggle_preview_cb(GtkWidget *widget, GridDialog_t *param) {
  param->enable_preview =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
  preview_redraw();
}

static void grid_assign_value(GtkWidget *widget, gpointer data, gint *value) {
...
static void left_changed_cb(GtkWidget *widget, gpointer data) {
  GridDialog_t *dialog = (GridDialog_t *)data;

  grid_assign_value(widget, data, &grid_left);
  if (gimp_chain_button_get_active(GIMP_CHAIN_BUTTON(dialog->chain_left_top))) {
    gint value = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(widget));
    gtk_spin_button_set_value(GTK_SPIN_BUTTON(dialog->top), value);
  }
}

static void top_changed_cb(GtkWidget *widget, gpointer data) {
  GridDialog_t *dialog = (GridDialog_t *)data;
...
static GridDialog_t *create_grid_settings_dialog(void) {
  ...
  g_signal_connect(data->snap, "toggled", G_CALLBACK(snap_toggled_cb), data);
  ...
  data->hidden = button;
  g_signal_connect(button, "toggled", G_CALLBACK(type_toggled_cb),
                   (gpointer)GRID_HIDDEN);
  ...
  g_signal_connect(data->width, "value-changed", G_CALLBACK(width_changed_cb),
                   (gpointer)data);
  ...
  g_signal_connect(data->height, "value-changed", G_CALLBACK(height_changed_cb),
                   (gpointer)data);
  ...
  g_signal_connect(data->left, "value-changed", G_CALLBACK(left_changed_cb),
                   (gpointer)data);

  label = create_label_in_grid(grid, 1, 2, _("pixels from _top"));
  data->top = create_spin_button_in_grid(grid, label, 1, 0, 0, 0, 100);
  g_signal_connect(data->top, "value-changed", G_CALLBACK(top_changed_cb),
                   (gpointer)data);
  ...
  data->preview = create_check_button_in_grid(main_grid, 3, 0, _("_Preview"));
  g_signal_connect(data->preview, "toggled", G_CALLBACK(toggle_preview_cb),
                   (gpointer)data);
...
static void draw_lines(cairo_t *cr, gint width, gint height) {
  gint x, y;
  gdouble dash = 4.;
...
static void draw_crosses(cairo_t *cr, gint width, gint height) {
  gint x, y;
...
static gint grid_nearest_x(gint x) {
  return grid_left + (x - grid_left + grid_width / 2) / grid_width * grid_width;
}

static gint grid_nearest_y(gint y) {
  return grid_top +
         (y - grid_top + grid_height / 2) / grid_height * grid_height;
}
