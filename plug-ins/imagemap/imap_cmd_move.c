...
static CommandClass_t move_command_class = {
    move_command_destruct, move_command_execute, NULL, /* move_command_undo */
...
static void button_motion(GtkWidget *widget, GdkEventMotion *event,
                          gpointer data) {
...
static void button_release(GtkWidget *widget, GdkEventButton *event,
                           gpointer data) {
  MoveCommand_t *command = (MoveCommand_t *)data;

  g_signal_handlers_disconnect_by_func(widget, button_motion, data);
  g_signal_handlers_disconnect_by_func(widget, button_release, data);
  ...
  if (command->obj_x || command->obj_y)
    command_list_add(
        object_move_command_new(command->obj, command->obj_x, command->obj_y));
...
static CmdExecuteValue_t move_command_execute(Command_t *parent) {
  ...
  /*   preview_freeze(); */
  g_signal_connect(widget, "button-release-event", G_CALLBACK(button_release),
                   command);
