...
static gboolean gfig_preview_events(GtkWidget *widget, GdkEvent *event);
static gboolean gfig_preview_draw(GtkWidget *widget, cairo_t *cr);
...
static void gfig_pos_update(gint x, gint y);
...
GtkWidget *make_preview(void) {
  ...
  g_signal_connect(gfig_context->preview, "realize",
                   G_CALLBACK(gfig_preview_realize), NULL);

  g_signal_connect(gfig_context->preview, "event",
                   G_CALLBACK(gfig_preview_events), NULL);

  g_signal_connect_after(gfig_context->preview, "draw",
                         G_CALLBACK(gfig_preview_draw), NULL);

  gtk_widget_set_size_request(gfig_context->preview, preview_width,
                              preview_height);
  ...
  g_signal_connect_swapped(
      gfig_context->preview, "motion-notify-event",
      G_CALLBACK(
          GTK_WIDGET_CLASS(G_OBJECT_GET_CLASS(ruler))->motion_notify_event),
      ruler);
...
static void draw_background(cairo_t *cr) {
  if (!back_pixbuf)
    back_pixbuf =
        gimp_image_get_thumbnail(gfig_context->image, preview_width,
                                 preview_height, GIMP_PIXBUF_LARGE_CHECKS);
