...
static void d_draw_star(GfigObject *obj, cairo_t *cr);
...
void tool_options_star(GtkWidget *notebook) {
  ...
  sides = num_sides_widget(_("Star Number of Points"), &star_num_sides, NULL, 3,
                           200);
...
static void d_paint_star(GfigObject *obj) {
  ...
  if (gfig_context_get_current_style()->fill_type != FILL_NONE) {
    ...
    gimp_image_select_polygon(gfig_context->image, selopt.type, i, line_pnts);
    gimp_context_pop();
...
static void d_update_star(GdkPoint *pnt) {
  ...
  if ((outer_pnt = center_pnt->next)) {
    ...
    d_pnt_add_line(obj_creating,
                   ...
                   pnt->y + (2 * (center_pnt->pnt.y - pnt->y)) / 3, -1);
  }
}

void d_star_start(GdkPoint *pnt, gboolean shift_down) {
  obj_creating = d_new_object(STAR, pnt->x, pnt->y);
  obj_creating->type_data = star_num_sides;
}

void d_star_end(GdkPoint *pnt, gboolean shift_down) {
