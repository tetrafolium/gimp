...
static void draw_one_obj(GfigObject *obj, cairo_t *cr);
static void do_move_obj(GfigObject *obj, GdkPoint *to_pnt);
static void do_move_all_obj(GdkPoint *to_pnt);
static void do_move_obj_pnt(GfigObject *obj, GdkPoint *to_pnt);
static void remove_obj_from_list(GFigObj *obj, GfigObject *del_obj);
static gint scan_obj_points(DobjPoints *opnt, GdkPoint *pnt);

void d_save_object(GfigObject *obj, GString *string) {
...
static DobjType gfig_read_object_type(gchar *desc) {
  gchar *ptr = desc;
  DobjType type;
...
GfigObject *d_new_object(DobjType type, gint x, gint y) {
...
static DobjPoints *get_diffs(GfigObject *obj, gint *xdiff, gint *ydiff,
                             GdkPoint *to_pnt) {
  DobjPoints *spnt;
...
static gboolean inside_sqr(GdkPoint *cpnt, GdkPoint *testpnt) {
...
static gboolean scan_obj_points(DobjPoints *opnt, GdkPoint *pnt) {
  while (opnt) {
...
static GfigObject *get_nearest_objs(GFigObj *obj, GdkPoint *pnt) {
...
void object_operation_start(GdkPoint *pnt, gboolean shift_down) {
  GfigObject *new_obj;
  ...
  case MOVE_POINT:
    ...
    if ((operation_obj->type == POLY || operation_obj->type == STAR) &&
        shift_down) {
...
void object_operation_end(GdkPoint *pnt, gboolean shift_down) {
...
/* Move object around */
void object_operation(GdkPoint *to_pnt, gboolean shift_down) {
...
static void update_pnts(GfigObject *obj, gint xdiff, gint ydiff) {
  DobjPoints *spnt;
...
void do_save_obj(GfigObject *obj, GString *string) {
  DobjPoints *spnt;
...
void draw_objects(GList *objs, gboolean show_single, cairo_t *cr) {
...
void prepend_to_all_obj(GFigObj *fobj, GList *nobj) {
  setup_undo(); /* Remember ME */

  fobj->obj_list = g_list_concat(fobj->obj_list, nobj);
}

static void scale_obj_points(DobjPoints *opnt, gdouble scale_x,
                             gdouble scale_y) {
  while (opnt) {
...
void add_to_all_obj(GFigObj *fobj, GfigObject *obj) {
  GList *nobj = NULL;
 ...
 * widgets.
 */
void object_start(GdkPoint *pnt, gboolean shift_down) {
...
void object_end(GdkPoint *pnt, gboolean shift_down) {
...
void setup_undo(void) {
  ...
  undo_table[undo_level] = copy_all_objs(gfig_context->current_obj->obj_list);
...
void d_pnt_add_line(GfigObject *obj, gint x, gint y, gint pos) {
