...
static void d_draw_spiral(GfigObject *obj, cairo_t *cr);
...
void tool_options_spiral(GtkWidget *notebook) {
  ...
  sides = num_sides_widget(_("Spiral Number of Turns"), &spiral_num_turns,
                           &spiral_toggle, 1, 20);
...
static void d_draw_spiral(GfigObject *obj, cairo_t *cr) {
  ...
  for (loop = 0; loop <= abs(obj->type_data * 180) +
                             clock_wise * (gint)RINT(offset_angle / ang_grid);
       loop++) {
    gdouble lx, ly;
    GdkPoint calc_pnt;
...
static void d_paint_spiral(GfigObject *obj) {
  ...
  /* count - */
  seg_count = abs(obj->type_data * 180) +
              clock_wise * (gint)RINT(offset_angle / ang_grid);
