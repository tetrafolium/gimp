...
static void fp_pnt_add(gdouble p1, gdouble p2, gdouble p3, gdouble p4);
static gdouble *d_bz_get_array(gint *sz);
static void d_bz_line(cairo_t *cr);
static void DrawBezier(fp_pnt points, gint np, gdouble mid, gint depth);
...
static void fp_pnt_start(void) { fp_pnt_cnt = 0; }
  ...
  if (shift_down) {
    ...
    if (l_pnt) {
      if (bezier_closed) {
        ...
        d_pnt_add_line(tmp_bezier, tmp_bezier->points->pnt.x,
...
void tool_options_bezier(GtkWidget *notebook) {
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_update),
                   &bezier_closed);
  gimp_help_set_help_data(toggle, _("Close curve on completion"), NULL);
