...
static void d_draw_ellipse(GfigObject *obj, cairo_t *cr);
...
static void d_draw_ellipse(GfigObject *obj, cairo_t *cr) {
  ...
  gfig_draw_arc(center_pnt->pnt.x, center_pnt->pnt.y, bound_wx, bound_wy, 0,
                360, cr);
}
...
static void d_paint_ellipse(GfigObject *obj) {
  ...
  if (gfig_context_get_current_style()->fill_type != FILL_NONE) {
    ...
    gimp_image_select_ellipse(gfig_context->image, selopt.type, dpnts[0],
                              dpnts[1], dpnts[2], dpnts[3]);
    gimp_context_pop();
...
void d_ellipse_start(GdkPoint *pnt, gboolean shift_down) {
  obj_creating = d_new_object(ELLIPSE, pnt->x, pnt->y);
}

void d_ellipse_end(GdkPoint *pnt, gboolean shift_down) {
  /* Under control point */
