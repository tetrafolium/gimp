...
static void d_draw_rectangle(GfigObject *obj, cairo_t *cr);
...
static void d_draw_rectangle(GfigObject *obj, cairo_t *cr) {
  ...
  xmin = MIN(gfig_scale_x(first_pnt->pnt.x), gfig_scale_x(second_pnt->pnt.x));
  ymin = MIN(gfig_scale_y(first_pnt->pnt.y), gfig_scale_y(second_pnt->pnt.y));
  xmax = MAX(gfig_scale_x(first_pnt->pnt.x), gfig_scale_x(second_pnt->pnt.x));
  ymax = MAX(gfig_scale_y(first_pnt->pnt.y), gfig_scale_y(second_pnt->pnt.y));
...
static void d_paint_rectangle(GfigObject *obj) {
  ...
  if (gfig_context_get_current_style()->fill_type != FILL_NONE) {
    ...
    gimp_context_set_feather_radius(selopt.feather_radius,
                                    selopt.feather_radius);
    gimp_image_select_rectangle(gfig_context->image, selopt.type, dpnts[0],
                                dpnts[1], dpnts[2] - dpnts[0],
                                dpnts[3] - dpnts[1]);
    gimp_context_pop();
  ...
  if (obj->style.paint_type == PAINT_BRUSH_TYPE) {
    gdouble line_pnts[] = {dpnts[0], dpnts[1], dpnts[2], dpnts[1], dpnts[2],
                           dpnts[3], dpnts[0], dpnts[3], dpnts[0], dpnts[1]};
...
static GfigObject *d_copy_rectangle(GfigObject *obj) {
  ...
  new_rectangle =
      d_new_object(RECTANGLE, obj->points->pnt.x, obj->points->pnt.y);
...
void d_rectangle_start(GdkPoint *pnt, gboolean shift_down) {
  obj_creating = d_new_object(RECTANGLE, pnt->x, pnt->y);
}

void d_rectangle_end(GdkPoint *pnt, gboolean shift_down) {
  /* Under control point */
