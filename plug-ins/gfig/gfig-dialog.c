...
static void gfig_response(GtkWidget *widget, gint response_id, gpointer data);
static void gfig_load_action_callback(GtkAction *action, gpointer data);
static void gfig_save_action_callback(GtkAction *action, gpointer data);
...
static void gfig_grid_action_callback(GtkAction *action, gpointer data);
static void gfig_prefs_action_callback(GtkAction *action, gpointer data);
...
static void save_file_chooser_response(GtkFileChooser *chooser,
                                       gint response_id, GFigObj *obj);
static void paint_combo_callback(GtkWidget *widget, gpointer data);
...
static void raise_selected_obj_to_top(GtkWidget *widget, gpointer data);
static void lower_selected_obj_to_bottom(GtkWidget *widget, gpointer data);
static void raise_selected_obj(GtkWidget *widget, gpointer data);
static void lower_selected_obj(GtkWidget *widget, gpointer data);

static void toggle_obj_type(GtkRadioAction *action, GtkRadioAction *current,
                            gpointer data);
...
static void gfig_scale_entry_update_double(GimpLabelSpin *entry,
                                           gdouble *value);
static void gfig_scale_entry_update_int(GimpLabelSpin *entry, gint *value);
...
gboolean gfig_dialog(void) {
  ...
  if (parasite) {
    ...
    newlayer = gimp_layer_new(
        gfig_context->image, "GFig", img_width, img_height, img_type, 100.0,
  ...
  top_level_dlg = gimp_dialog_new(
      _("Gfig"), PLUG_IN_ROLE, NULL, 0, gimp_standard_help_func, PLUG_IN_PROC,
  ...
  gimp_dialog_set_alternative_button_order(
      GTK_DIALOG(top_level_dlg), GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL, -1);

  g_signal_connect(top_level_dlg, "response", G_CALLBACK(gfig_response),
                   top_level_dlg);
  ...
  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(top_level_dlg))), menubar,
      FALSE, FALSE, 0);
  gtk_widget_show(menubar);
  toolbar = gtk_ui_manager_get_widget(ui_manager, "/ui/gfig-toolbar");
  gtk_toolbar_set_style(GTK_TOOLBAR(toolbar), GTK_TOOLBAR_ICONS);
  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(top_level_dlg))), toolbar,
      FALSE, FALSE, 0);
  ...
  gtk_box_pack_end(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(top_level_dlg))),
  ...
  gfig_context->paint_type_toggle = toggle =
      gtk_check_button_new_with_mnemonic(_("_Stroke"));
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(set_paint_type_callback),
                   vbox);

  /* foreground color button in Stroke frame*/
  gfig_context->fg_color = g_new0(GimpRGB, 1);
  gfig_context->fg_color_button = gimp_color_button_new(
      "Foreground", SEL_BUTTON_WIDTH, SEL_BUTTON_HEIGHT, gfig_context->fg_color,
      GIMP_COLOR_AREA_SMALL_CHECKS);
  g_signal_connect(gfig_context->fg_color_button, "color-changed",
                   G_CALLBACK(set_foreground_callback), gfig_context->fg_color);
  gimp_color_button_set_color(GIMP_COLOR_BUTTON(gfig_context->fg_color_button),
                              &gfig_context->default_style.foreground);
  gtk_box_pack_start(GTK_BOX(vbox), gfig_context->fg_color_button, FALSE, FALSE,
                     0);
  gtk_widget_show(gfig_context->fg_color_button);

  /* brush selector in Stroke frame */
  gfig_context->brush_select = gimp_brush_select_button_new(
      "Brush", gfig_context->default_style.brush_name, -1.0, -1, -1);
  g_signal_connect(gfig_context->brush_select, "brush-set",
                   G_CALLBACK(gfig_brush_changed_callback), NULL);
  gtk_box_pack_start(GTK_BOX(vbox), gfig_context->brush_select, FALSE, FALSE,
                     0);
  ...
  gfig_context->fillstyle_combo = combo = gimp_int_combo_box_new(
      _("No fill"), FILL_NONE, _("Color fill"), FILL_COLOR, _("Pattern fill"),
      FILL_PATTERN, _("Shape gradient"), FILL_GRADIENT, _("Vertical gradient"),
      FILL_VERTICAL, _("Horizontal gradient"), FILL_HORIZONTAL, NULL);
  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), 0);
  g_signal_connect(combo, "changed", G_CALLBACK(select_filltype_callback),
                   NULL);
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(fill_type_notebook), empty_label, NULL);

  /* A page for the fill color button */
  gfig_context->bg_color = g_new0(GimpRGB, 1);
  gfig_context->bg_color_button = gimp_color_button_new(
      "Background", SEL_BUTTON_WIDTH, SEL_BUTTON_HEIGHT, gfig_context->bg_color,
      GIMP_COLOR_AREA_SMALL_CHECKS);
  g_signal_connect(gfig_context->bg_color_button, "color-changed",
                   G_CALLBACK(set_background_callback), gfig_context->bg_color);
  ...
  gfig_context->pattern_select = gimp_pattern_select_button_new(
      "Pattern", gfig_context->default_style.pattern);
  ...
  gfig_context->gradient_select = gimp_gradient_select_button_new(
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_update),
                   &gfig_context->show_background);
  g_signal_connect_swapped(toggle, "toggled", G_CALLBACK(gtk_widget_queue_draw),
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(draw_grid_clear), NULL);
  gtk_widget_show(toggle);
...
void gfig_dialog_action_set_sensitive(const gchar *name, gboolean sensitive) {
...
static gchar *gfig_get_user_writable_dir(void) {
  if (gfig_path) {
    GList *list;
    gchar *dir;
...
static void gfig_load_action_callback(GtkAction *action, gpointer data) {
  ...
  if (!dialog) {
    ...
    dialog = gtk_file_chooser_dialog_new(
        _("Load Gfig Object Collection"), GTK_WINDOW(data),
        GTK_FILE_CHOOSER_ACTION_OPEN,

        _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Open"), GTK_RESPONSE_OK,

        NULL);

    gimp_dialog_set_alternative_button_order(
        GTK_DIALOG(dialog), GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL, -1);

    gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_OK);

    g_object_add_weak_pointer(G_OBJECT(dialog), (gpointer)&dialog);

    g_signal_connect(dialog, "response", G_CALLBACK(load_file_chooser_response),
                     NULL);
...
static void gfig_save_action_callback(GtkAction *action, gpointer data) {
  ...
  if (!dialog) {
    ...
    dialog = gtk_file_chooser_dialog_new(
        _("Save Gfig Drawing"), GTK_WINDOW(data), GTK_FILE_CHOOSER_ACTION_SAVE,

        _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Save"), GTK_RESPONSE_OK,
     ...
     *        should be used here.
     */
    g_signal_connect(dialog, "response", G_CALLBACK(save_file_chooser_response),
...
static void gfig_close_action_callback(GtkAction *action, gpointer data) {
  gtk_dialog_response(GTK_DIALOG(data), GTK_RESPONSE_OK);
}

static void gfig_undo_action_callback(GtkAction *action, gpointer data) {
  if (undo_level >= 0) {
...
static void gfig_clear_action_callback(GtkWidget *widget, gpointer data) {
...
void draw_item(cairo_t *cr, gboolean fill) {
...
void draw_circle(GdkPoint *p, gboolean selected, cairo_t *cr) {
  if (!selvals.opts.showcontrol)
    return;

  cairo_arc(cr, gfig_scale_x(p->x) + .5, gfig_scale_y(p->y) + .5, SQ_SIZE / 2,
            0, 2 * G_PI);
...
void draw_sqr(GdkPoint *p, gboolean selected, cairo_t *cr) {
  if (!selvals.opts.showcontrol)
    return;

  cairo_rectangle(cr, gfig_scale_x(p->x) + .5 - SQ_SIZE / 2,
                  gfig_scale_y(p->y) + .5 - SQ_SIZE / 2, SQ_SIZE, SQ_SIZE);
...
static GtkUIManager *create_ui_manager(GtkWidget *window) {
  static GtkActionEntry actions[] = {
      ...
      {"open", GIMP_ICON_DOCUMENT_OPEN, N_("_Open..."), "<control>O", NULL,
       G_CALLBACK(gfig_load_action_callback)},

      {"save", GIMP_ICON_DOCUMENT_SAVE, N_("_Save..."), "<control>S", NULL,
       G_CALLBACK(gfig_save_action_callback)},

      {"close", GIMP_ICON_CLOSE, N_("_Close"), "<control>C", NULL,
       G_CALLBACK(gfig_close_action_callback)},

      {"gfig-edit-menu", NULL, "_Edit"},

      {"undo", GIMP_ICON_EDIT_UNDO, N_("_Undo"), "<control>Z", NULL,
       G_CALLBACK(gfig_undo_action_callback)},

      {"clear", GIMP_ICON_EDIT_CLEAR, N_("_Clear"), NULL, NULL,
       G_CALLBACK(gfig_clear_action_callback)},

      {"grid", GIMP_ICON_GRID, N_("_Grid"), "<control>G", NULL,
       G_CALLBACK(gfig_grid_action_callback)},

      {"prefs", GIMP_ICON_PREFERENCES_SYSTEM, N_("_Preferences..."),
       "<control>P", NULL, G_CALLBACK(gfig_prefs_action_callback)},

      {"raise", GIMP_ICON_GO_UP, N_("_Raise"), "<control>U",
       N_("Raise selected object"), G_CALLBACK(raise_selected_obj)},

      {"lower", GIMP_ICON_GO_DOWN, N_("_Lower"), "<control>D",
       N_("Lower selected object"), G_CALLBACK(lower_selected_obj)},

      {"top", GIMP_ICON_GO_TOP, N_("Raise to _top"), "<control>T",
       N_("Raise selected object to top"),
       G_CALLBACK(raise_selected_obj_to_top)},

      {"bottom", GIMP_ICON_GO_BOTTOM, N_("Lower to _bottom"), "<control>B",
       N_("Lower selected object to bottom"),
       G_CALLBACK(lower_selected_obj_to_bottom)},

      {"show_previous", GIMP_ICON_GO_PREVIOUS, N_("_Previous"), "<control>H",
       N_("Show previous object"), G_CALLBACK(select_button_clicked_lt)},

      {"show_next", GIMP_ICON_GO_NEXT, N_("_Next"), "<control>L",
       N_("Show next object"), G_CALLBACK(select_button_clicked_gt)},

      {"show_all", GFIG_ICON_SHOW_ALL, N_("Show _all"), "<control>A",
       N_("Show all objects"), G_CALLBACK(select_button_clicked_eq)}};
  static GtkRadioActionEntry radio_actions[] = {
      {"line", GFIG_ICON_LINE, NULL, "L", N_("Create line"), LINE},

      {"rectangle", GFIG_ICON_RECTANGLE, NULL, "R", N_("Create rectangle"),
       RECTANGLE},

      {"circle", GFIG_ICON_CIRCLE, NULL, "C", N_("Create circle"), CIRCLE},

      {"ellipse", GFIG_ICON_ELLIPSE, NULL, "E", N_("Create ellipse"), ELLIPSE},

      {"arc", GFIG_ICON_CURVE, NULL, "A", N_("Create arc"), ARC},

      {"polygon", GFIG_ICON_POLYGON, NULL, "P", N_("Create reg polygon"), POLY},

      {"star", GFIG_ICON_STAR, NULL, "S", N_("Create star"), STAR},

      {"spiral", GFIG_ICON_SPIRAL, NULL, "I", N_("Create spiral"), SPIRAL},

      {"bezier", GFIG_ICON_BEZIER, NULL, "B",
       N_("Create bezier curve. "
          "Shift + Button ends object creation."),
       BEZIER},

      {"move_obj", GFIG_ICON_MOVE_OBJECT, NULL, "M", N_("Move an object"),
       MOVE_OBJ},

      {"move_point", GFIG_ICON_MOVE_POINT, NULL, "V", N_("Move a single point"),
       MOVE_POINT},

      {"copy", GFIG_ICON_COPY_OBJECT, NULL, "Y", N_("Copy an object"),
       COPY_OBJ},

      {"delete", GFIG_ICON_DELETE_OBJECT, NULL, "D", N_("Delete an object"),
       DEL_OBJ},

      {"select", GFIG_ICON_SELECT_OBJECT, NULL, "A", N_("Select an object"),
       SELECT_OBJ}};
  ...
  gtk_action_group_add_actions(gfig_actions, actions, G_N_ELEMENTS(actions),
                               window);
  gtk_action_group_add_radio_actions(gfig_actions, radio_actions,
                                     G_N_ELEMENTS(radio_actions), LINE,
                                     G_CALLBACK(toggle_obj_type), window);
...
static void tool_option_no_option(GtkWidget *notebook) {
  GtkWidget *label;
...
static void raise_selected_obj_to_top(GtkWidget *widget, gpointer data) {
  ...
  if (g_list_find(gfig_context->current_obj->obj_list,
    ...
    gfig_context->current_obj->obj_list = g_list_remove(
        gfig_context->current_obj->obj_list, gfig_context->selected_obj);
    gfig_context->current_obj->obj_list = g_list_append(
        gfig_context->current_obj->obj_list, gfig_context->selected_obj);
  } else {
...
static void lower_selected_obj_to_bottom(GtkWidget *widget, gpointer data) {
  ...
  if (g_list_find(gfig_context->current_obj->obj_list,
    ...
    gfig_context->current_obj->obj_list = g_list_remove(
        gfig_context->current_obj->obj_list, gfig_context->selected_obj);
    gfig_context->current_obj->obj_list = g_list_prepend(
        gfig_context->current_obj->obj_list, gfig_context->selected_obj);
  } else {
  ...
  if (g_list_find(gfig_context->current_obj->obj_list,
    ...
    gfig_context->current_obj->obj_list =
        g_list_insert(gfig_context->current_obj->obj_list,
                      gfig_context->selected_obj, position + 1);
  } else {
    g_message("Trying to raise object that does not exist.");
    return;
  }
...
static void lower_selected_obj(GtkWidget *widget, gpointer data) {
  ...
  if (g_list_find(gfig_context->current_obj->obj_list,
    ...
    gfig_context->current_obj->obj_list =
        g_list_insert(gfig_context->current_obj->obj_list,
                      gfig_context->selected_obj, MAX(0, position - 1));
  } else {
    g_message("Trying to lower object that does not exist.");
    return;
  }
...
static gboolean gfig_paint_timeout(gpointer data) {
  gfig_paint_callback();
...
static void gfig_paint_delayed(void) {
  if (paint_timeout)
    g_source_remove(paint_timeout);

  paint_timeout = g_timeout_add(UPDATE_DELAY, gfig_paint_timeout, NULL);
}

static void gfig_prefs_action_callback(GtkAction *widget, gpointer data) {
  static GtkWidget *dialog = NULL;
  ...
  if (!dialog) {
    ...
    g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_update),
                     &selvals.showpos);
    g_signal_connect_after(toggle, "toggled", G_CALLBACK(gfig_pos_enable),
                           NULL);
    gtk_widget_show(toggle);
    ...
    g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_update),
                     &selvals.opts.showcontrol);
    g_signal_connect(toggle, "toggled", G_CALLBACK(toggle_show_image), NULL);
    gtk_widget_show(toggle);
    ...
    g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_update),
                     &selopt.antia);
    g_signal_connect(toggle, "toggled", G_CALLBACK(gfig_paint_callback), NULL);
    gtk_widget_show(toggle);
    ...
    undo_scale = gimp_scale_entry_new(_("Max undo:"), selvals.maxundo, MIN_UNDO,
                                      MAX_UNDO, 0);
    g_signal_connect(undo_scale, "value-changed",
                     G_CALLBACK(gfig_scale_entry_update_int), &selvals.maxundo);
    gtk_grid_attach(GTK_GRID(grid), undo_scale, 0, 0, 3, 1);
    gtk_widget_show(undo_scale);

    page_menu_bg = gimp_int_combo_box_new(
        _("Transparent"), LAYER_TRANS_BG, _("Background"), LAYER_BG_BG,
        _("Foreground"), LAYER_FG_BG, _("White"), LAYER_WHITE_BG, _("Copy"),
        LAYER_COPY_BG, NULL);
    gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(page_menu_bg),
                                  selvals.onlayerbg);

    g_signal_connect(page_menu_bg, "changed", G_CALLBACK(paint_combo_callback),
    ...
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Background:"), 0.0, 0.5,
                             page_menu_bg, 2);
    ...
    g_signal_connect(scale_data, "value-changed",
                     G_CALLBACK(gfig_paint_delayed), NULL);
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Radius:"), 0.0, 1.0,
                             scale, 1);
...
static void gfig_grid_action_callback(GtkAction *action, gpointer data) {
  ...
  if (!dialog) {
    ...
    dialog = gimp_dialog_new(_("Grid"), "gimp-gfig-grid", GTK_WIDGET(data), 0,
                             NULL, NULL,
    ...
    grid_scale = gimp_scale_entry_new(
        _("Grid spacing:"), selvals.opts.gridspacing, MIN_GRID, MAX_GRID, 0);
    g_signal_connect(grid_scale, "value-changed",
                     G_CALLBACK(gfig_scale_entry_update_int),
                     &selvals.opts.gridspacing);
    g_signal_connect(grid_scale, "value-changed", G_CALLBACK(draw_grid_clear),
                     NULL);
    ...
    sectors_scale =
        gimp_scale_entry_new(_("Polar grid sectors desired:"),
                             selvals.opts.grid_sectors_desired, 5, 360, 0);
    g_signal_connect(sectors_scale, "value-changed",
                     G_CALLBACK(gfig_scale_entry_update_int),
                     &selvals.opts.grid_sectors_desired);
    g_signal_connect(sectors_scale, "value-changed",
                     G_CALLBACK(draw_grid_clear), NULL);
    ...
    radius_scale =
        gimp_scale_entry_new(_("Polar grid radius interval:"),
                             selvals.opts.grid_radius_interval, 5, 50, 0);
    g_signal_connect(radius_scale, "value-changed",
                     G_CALLBACK(gfig_scale_entry_update_double),
                     &selvals.opts.grid_radius_interval);
    g_signal_connect(radius_scale, "value-changed", G_CALLBACK(draw_grid_clear),
                     NULL);
    ...
    combo = gimp_int_combo_box_new(_("Rectangle"), RECT_GRID, _("Polar"),
                                   POLAR_GRID, _("Isometric"), ISO_GRID, NULL);
    gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo),
                                  selvals.opts.gridtype);

    g_signal_connect(combo, "changed", G_CALLBACK(gridtype_combo_callback),
                     GINT_TO_POINTER(GRID_TYPE_MENU));

    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Grid type:"), 0.0, 0.5,
                             combo, 2);
    ...
    combo = gimp_int_combo_box_new(
        _("Normal"), GFIG_NORMAL_GC, _("Black"), GFIG_BLACK_GC, _("White"),
        GFIG_WHITE_GC, _("Grey"), GFIG_GREY_GC, _("Darker"), GFIG_DARKER_GC,
        _("Lighter"), GFIG_LIGHTER_GC, _("Very dark"), GFIG_VERY_DARK_GC, NULL);
    gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), grid_gc_type);

    g_signal_connect(combo, "changed", G_CALLBACK(gridtype_combo_callback),
                     GINT_TO_POINTER(GRID_RENDER_MENU));

    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Grid color:"), 0.0, 0.5,
                             combo, 2);
...
static GfigObject *gfig_select_obj_by_number(gint count) {
  GList *objs;
  ...
  for (objs = gfig_context->current_obj->obj_list, k = 0; objs;
...
GtkWidget *num_sides_widget(const gchar *d_title, gint *num_sides,
                            gint *which_way, gint adj_min, gint adj_max) {
  GtkWidget *grid;
  GtkWidget *scale;
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(gfig_scale_entry_update_int), num_sides);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 0, 3, 1);
  gtk_widget_show(scale);

  if (which_way) {
    GtkWidget *combo =
        gimp_int_combo_box_new(_("Right"), 0, _("Left"), 1, NULL);

    gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), *which_way);

    g_signal_connect(combo, "changed",
                     G_CALLBACK(gimp_int_combo_box_get_active), which_way);

    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Orientation:"), 0.0, 0.5,
                             combo, 1);
  }
  return grid;
}

void gfig_paint(BrushType brush_type, GimpDrawable *drawable, gint seg_count,
                gdouble line_pnts[]) {
  switch (brush_type) {
  case BRUSH_BRUSH_TYPE:
    gimp_paintbrush(drawable, selvals.brushfade, seg_count, line_pnts,
                    GIMP_PAINT_CONSTANT, selvals.brushgradient);
    break;

  case BRUSH_PENCIL_TYPE:
    gimp_pencil(drawable, seg_count, line_pnts);
    break;

  case BRUSH_AIRBRUSH_TYPE:
    gimp_airbrush(drawable, selvals.airbrushpressure, seg_count, line_pnts);
    break;

  case BRUSH_PATTERN_TYPE:
    gimp_clone(drawable, drawable, GIMP_CLONE_PATTERN, 0.0, 0.0, seg_count,
               line_pnts);
    break;
  }
}
...
void paint_layer_fill(gdouble x1, gdouble y1, gdouble x2, gdouble y2) {
  ...
  case FILL_VERTICAL:
    gimp_drawable_edit_gradient_fill(gfig_context->drawable,
                                     GIMP_GRADIENT_LINEAR, 0.0, FALSE, 0, 0.0,
                                     FALSE, x1, y1, x1, y2);
    return;
  ...
  gimp_drawable_edit_fill(gfig_context->drawable, fill_type);
...
void scale_to_original_xy(gdouble *list, gint size) {
  gint i;
...
/* Pairs x followed by y */
void scale_to_xy(gdouble *list, gint size) {
  gint i;
...
void gfig_draw_arc(gint x, gint y, gint width, gint height, gint angle1,
  ...
  if (angle1 < angle2)
    ...
    cairo_arc_negative(cr, 0., 0., 1., angle1 * G_PI / 180,
                       angle2 * G_PI / 180);
...
void gfig_draw_line(gint x0, gint y0, gint x1, gint y1, cairo_t *cr) {
