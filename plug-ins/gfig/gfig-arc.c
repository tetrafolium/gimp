...
static gdouble dist(gdouble x1, gdouble y1, gdouble x2, gdouble y2);

static void mid_point(gdouble x1, gdouble y1, gdouble x2, gdouble y2,
                      gdouble *mx, gdouble *my);

static gdouble line_grad(gdouble x1, gdouble y1, gdouble x2, gdouble y2);

static gdouble line_cons(gdouble x, gdouble y, gdouble lgrad);

static void line_definition(gdouble x1, gdouble y1, gdouble x2, gdouble y2,
                            gdouble *lgrad, gdouble *lconst);

static void arc_details(GdkPoint *vert_a, GdkPoint *vert_b, GdkPoint *vert_c,
                        GdkPoint *center_pnt, gdouble *radius);

static gdouble arc_angle(GdkPoint *pnt, GdkPoint *center);

static void arc_drawing_details(GfigObject *obj, gdouble *minang,
                                GdkPoint *center_pnt, gdouble *arcang,
                                gdouble *radius, gboolean draw_cnts,
                                gboolean do_scale);

static void d_draw_arc(GfigObject *obj, cairo_t *cr);
...
static void d_arc_line_start(GdkPoint *pnt, gboolean shift_down);
static void d_arc_line_end(GdkPoint *pnt, gboolean shift_down);
...
static void arc_details(GdkPoint *vert_a, GdkPoint *vert_b, GdkPoint *vert_c,
  ...
  /* Area */
  area = sqrt(sum_sides2 * (sum_sides2 - len_a) * (sum_sides2 - len_b) *
              (sum_sides2 - len_c));
...
static void d_draw_arc(GfigObject *obj, cairo_t *cr) {
  ...
  gfig_draw_arc(center_pnt.x, center_pnt.y, radius, radius, -minang,
                -(minang + arcang), cr);
}
...
static void d_paint_arc(GfigObject *obj) {
  ...
  minang =
      minang * (2.0 * G_PI / 360.0); /* min ang is in degrees - need in rads */
  ...
  for (loop = 0; loop < abs((gint)arcang); loop++) {
    ...
    ly = -radius * sin(ang_loop); /* y grows down screen and angs measured from
                                     x clockwise */
...
static void d_update_arc(GdkPoint *pnt) {
  ...
  if (!pnt1 || !(pnt2 = pnt1->next) || !(pnt3 = pnt2->next)) {
...
void d_arc_start(GdkPoint *pnt, gboolean shift_down) {
...
void d_arc_end(GdkPoint *pnt, gboolean shift_down) {
  /* Under control point */
  if (!tmp_line || !tmp_line->points || !tmp_line->points->next) {
