...
static GimpValueArray *gfig_run(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void gfig_class_init(GfigClass *klass) {
...
static void gfig_init(Gfig *gfig) {}
...
static GimpProcedure *gfig_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, gfig_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure, N_("Create geometric shapes"),
 ...
 */
void gfig_name_encode(gchar *dest, gchar *src) {
 ...
 */
void gfig_name_decode(gchar *dest, const gchar *src) {
  gint cnt = MAX_LOAD_LINE - 1;
  guint tmp;
...
GFigObj *gfig_new(void) { return g_new0(GFigObj, 1); }

static void gfig_load_objs(GFigObj *gfig, gint load_count, FILE *fp) {
  GfigObject *obj;
...
GFigObj *gfig_load(const gchar *filename, const gchar *name) {
  GFigObj *gfig;
  FILE *fp;
  gchar load_buf[MAX_LOAD_LINE];
...
static void gfig_save_obj_start(GfigObject *obj, GString *string) {
  g_string_append_printf(string, "<%s ", obj->class->name);
  gfig_style_save_as_attributes(&obj->style, string);
  g_string_append_printf(string, ">\n");
}

static void gfig_save_obj_end(GfigObject *obj, GString *string) {
  g_string_append_printf(string, "</%s>\n", obj->class->name);
}

static gboolean load_bool(gchar *opt_buf, gint *toset) {
...
GString *gfig_save_as_string(void) {
  ...
  for (objs = gfig_context->current_obj->obj_list; objs;
...
gboolean gfig_save_as_parasite(void) {
  ...
  parasite = gimp_parasite_new(
      "gfig", GIMP_PARASITE_PERSISTENT | GIMP_PARASITE_UNDOABLE, string->len,
      string->str);

  g_string_free(string, TRUE);

  if (!gimp_item_attach_parasite(GIMP_ITEM(gfig_context->drawable), parasite)) {
...
GFigObj *gfig_load_from_parasite(void) {
  ...
  fwrite(gimp_parasite_data(parasite), sizeof(guchar),
         gimp_parasite_data_size(parasite), fp);
  fclose(fp);
