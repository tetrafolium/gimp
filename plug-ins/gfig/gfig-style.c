...
static void gfig_read_parameter_string(gchar **text, gint nitems,
                                       const gchar *name, gchar **style_entry);

static void gfig_read_parameter_int(gchar **text, gint nitems,
                                    const gchar *name, gint *style_entry);

static void gfig_read_parameter_double(gchar **text, gint nitems,
                                       const gchar *name, gdouble *style_entry);

static void gfig_read_parameter_gimp_rgb(gchar **text, gint nitems,
                                         const gchar *name,
                                         GimpRGB *style_entry);
...
gboolean gfig_load_style(Style *style, FILE *fp) {
  gulong offset;
...
gboolean gfig_skip_style(Style *style, FILE *fp) {
  gulong offset;
  gchar load_buf2[MAX_LOAD_LINE];
 ...
 */
gboolean gfig_load_styles(GFigObj *gfig, FILE *fp) {
  if (gfig_context->debug_styles)
...
void gfig_save_style(Style *style, GString *string) {
...
void gfig_style_save_as_attributes(Style *style, GString *string) {
  gchar buffer[G_ASCII_DTOSTR_BUF_SIZE];
  gchar buffer_r[G_ASCII_DTOSTR_BUF_SIZE];
  gchar buffer_g[G_ASCII_DTOSTR_BUF_SIZE];
  gchar buffer_b[G_ASCII_DTOSTR_BUF_SIZE];
 ...
 */
void set_foreground_callback(GimpColorButton *button, gpointer data) {
  GimpRGB color2;
  Style *current_style;
  ...
  gimp_rgba_set(&current_style->foreground, color2.r, color2.g, color2.b,
                color2.a);

  gfig_paint_callback();
}

void set_background_callback(GimpColorButton *button, gpointer data) {
  GimpRGB color2;
  Style *current_style;
  ...
  gimp_color_button_get_color(button, &color2);
  gimp_rgba_set(&current_style->background, color2.r, color2.g, color2.b,
                color2.a);

  gfig_paint_callback();
}

void set_paint_type_callback(GtkToggleButton *toggle, gpointer data) {
  gboolean paint_type;
  Style *current_style;
...
void gfig_brush_changed_callback(GimpBrushSelectButton *button,
                                 const gchar *brush_name, gdouble opacity,
...
void gfig_pattern_changed_callback(GimpPatternSelectButton *button,
                                   const gchar *pattern_name, gint width,
...
void gfig_gradient_changed_callback(GimpGradientSelectButton *button,
                                    const gchar *gradient_name, gint width,
...
void gfig_rgba_copy(GimpRGB *color1, GimpRGB *color2) {
  color1->r = color2->r;
  color1->g = color2->g;
  color1->b = color2->b;
  color1->a = color2->a;
...
void gfig_style_copy(Style *style1, Style *style0, const gchar *name) {
  if (name)
 ...
 */
void gfig_read_gimp_style(Style *style, const gchar *name) {
  gint dummy;
  ...
  gimp_brush_get_info(style->brush_name, &style->brush_width,
                      &style->brush_height, &dummy, &dummy);
...
void gfig_style_set_context_from_style(Style *style) {
  ...
  gimp_brush_select_button_set_brush(
      GIMP_BRUSH_SELECT_BUTTON(gfig_context->brush_select), style->brush_name,
      -1.0, -1, -1); /* FIXME */

  gimp_pattern_select_button_set_pattern(
      GIMP_PATTERN_SELECT_BUTTON(gfig_context->pattern_select), style->pattern);
  ...
  gimp_int_combo_box_set_active(
      GIMP_INT_COMBO_BOX(gfig_context->fillstyle_combo),
      (gint)style->fill_type);

  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(gfig_context->paint_type_toggle), style->paint_type);
...
void gfig_style_set_style_from_context(Style *style) {
  ...
  if (gimp_int_combo_box_get_active(
          GIMP_INT_COMBO_BOX(gfig_context->fillstyle_combo), &value))
    style->fill_type = value;
  ...
  style->paint_type = gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(gfig_context->paint_type_toggle));
}

void mygimp_brush_info(gint *width, gint *height) {
