...
static gint calc_radius(GdkPoint *center, GdkPoint *edge);
static void d_draw_circle(GfigObject *obj, cairo_t *cr);
...
static void d_paint_circle(GfigObject *obj) {
  ...
  if (gfig_context_get_current_style()->fill_type != FILL_NONE) {
    ...
    paint_layer_fill(center_pnt->pnt.x - radius, center_pnt->pnt.y - radius,
                     center_pnt->pnt.x + radius, center_pnt->pnt.y + radius);
