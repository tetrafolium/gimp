...
#define LIGHTING                                                               \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), LIGHTING_TYPE, Lighting))
...
static void lighting_class_init(LightingClass *klass) {
...
static GimpProcedure *lighting_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    ...
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, lighting_run, NULL, NULL);
    ...
    GIMP_PROC_ARG_DRAWABLE(procedure, "bump-drawable", "Bump drawable",
                           "Bumpmap drawable (set to NULL if disabled)", TRUE,
                           G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "env-drawable", "Env drawable",
                           "Environmentmap drawable (set to NULL if disabled",
                           TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "do-bumpmap", "Do bumpmap",
                          "Enable bumpmapping", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "do-envmap", "Do envmap",
                          "Enable envmapping", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "bumpmap-type", "Bumpmap type",
    ...
    GIMP_PROC_ARG_BOOLEAN(procedure, "antialiasing", "Antialiasing",
                          "Apply antialiasing", FALSE, G_PARAM_READWRITE);
...
static void check_drawables(void) {
  ...
  if (mapvals.env_mapped) {
    if (!gimp_item_id_is_drawable(mapvals.envmap_id)) {
      ...
      if (gimp_drawable_is_gray(map) || gimp_drawable_has_alpha(map)) {
        mapvals.env_mapped = FALSE;
        mapvals.envmap_id = -1;
      }
    }
