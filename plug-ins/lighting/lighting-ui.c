...
static void toggle_update(GtkWidget *widget, gpointer data);

static void distance_update(GimpLabelSpin *scale, gpointer data);

static gboolean bumpmap_constrain(GimpImage *image, GimpItem *item,
                                  gpointer data);
static gboolean envmap_constrain(GimpImage *image, GimpItem *item,
                                 gpointer data);
static void envmap_combo_callback(GtkWidget *widget, gpointer data);
static void save_lighting_preset(GtkWidget *widget, gpointer data);
static void save_preset_response(GtkFileChooser *chooser, gint response_id,
                                 gpointer data);
static void load_lighting_preset(GtkWidget *widget, gpointer data);
static void load_preset_response(GtkFileChooser *chooser, gint response_id,
                                 gpointer data);
static void lightselect_callback(GimpIntComboBox *combo, gpointer data);
static void apply_settings(GtkWidget *widget, gpointer data);
static void isolate_selected_light(GtkWidget *widget, gpointer data);
...
static void mapmenu2_callback(GtkWidget *widget, gpointer data) {
...
static gint bumpmap_constrain(GimpImage *image, GimpItem *item, gpointer data) {
  GimpDrawable *dr = gimp_drawable_get_by_id(mapvals.drawable_id);

  return (
      (gimp_drawable_width(GIMP_DRAWABLE(item)) == gimp_drawable_width(dr)) &&
      (gimp_drawable_height(GIMP_DRAWABLE(item)) == gimp_drawable_height(dr)));
}

static gint envmap_constrain(GimpImage *image, GimpItem *item, gpointer data) {
...
static void envmap_combo_callback(GtkWidget *widget, gpointer data) {
  ...
  gimp_int_combo_box_get_active(GIMP_INT_COMBO_BOX(widget), &mapvals.envmap_id);
...
static GtkWidget *create_options_page(void) {
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(toggle_update),
                   &mapvals.transparent_background);
  ...
  gimp_help_set_help_data(toggle, _("Create a new image when applying filter"),
                          NULL);
  ...
  gimp_help_set_help_data(toggle, _("Enable/disable high quality preview"),
                          NULL);

  scale =
      gimp_scale_entry_new(_("Distance:"), mapvals.viewpoint.z, 0.0, 2.0, 3);
  gimp_help_set_help_data(scale, "Distance of observer from surface",
                          "plug-in-lighting");
  g_signal_connect(scale, "value-changed", G_CALLBACK(distance_update), NULL);
...
static GtkWidget *create_light_page(void) {
  ...
  lightselect_combo = gimp_int_combo_box_new(
      _("Light 1"), 0, _("Light 2"), 1, _("Light 3"), 2, _("Light 4"), 3,
      _("Light 5"), 4, _("Light 6"), 5, NULL);
  ...
  light_type_combo = gimp_int_combo_box_new(
      C_("light-source", "None"), NO_LIGHT, _("Directional"), DIRECTIONAL_LIGHT,
      _("Point"), POINT_LIGHT,
      /* _("Spot"),               SPOT_LIGHT, */
      NULL);
  ...
  g_signal_connect(light_type_combo, "changed", G_CALLBACK(apply_settings),
                   NULL);

  gimp_help_set_help_data(light_type_combo, _("Type of light source to apply"),
                          NULL);

  colorbutton = gimp_color_button_new(_("Select lightsource color"), 64, 16,
                                      &mapvals.lightsource[k].color,
  ...
  g_signal_connect(colorbutton, "color-changed", G_CALLBACK(apply_settings),
                   NULL);

  gimp_help_set_help_data(colorbutton, _("Set light source color"), NULL);

  spin_intensity = spin_button_new(&adj, mapvals.lightsource[k].intensity, 0.0,
                                   100.0, 0.01, 0.1, 0.0, 0.0, 2);
  gtk_widget_set_halign(spin_intensity, GTK_ALIGN_START);
  gtk_widget_set_margin_end(spin_intensity, 12);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("_Intensity:"), 0.0, 0.5,
                           spin_intensity, 1);

  g_signal_connect(adj, "value-changed", G_CALLBACK(apply_settings), NULL);

  gimp_help_set_help_data(spin_intensity, _("Light intensity"), NULL);
  ...
  spin_pos_x = spin_button_new(&adj, mapvals.lightsource[k].position.x, -100.0,
                               100.0, 0.1, 1.0, 0.0, 0.0, 2);
  gtk_widget_set_halign(spin_pos_x, GTK_ALIGN_START);
  gtk_widget_set_margin_end(spin_pos_x, 12);
  gimp_grid_attach_aligned(GTK_GRID(grid), 2, 1, _("_X:"), 0.0, 0.5, spin_pos_x,
                           1);

  g_signal_connect(adj, "value-changed", G_CALLBACK(apply_settings), NULL);

  gimp_help_set_help_data(spin_pos_x, _("Light source X position in XYZ space"),
                          NULL);

  spin_pos_y = spin_button_new(&adj, mapvals.lightsource[k].position.y, -100.0,
                               100.0, 0.1, 1.0, 0.0, 0.0, 2);
  gtk_widget_set_halign(spin_pos_y, GTK_ALIGN_START);
  gtk_widget_set_margin_end(spin_pos_y, 12);
  gimp_grid_attach_aligned(GTK_GRID(grid), 2, 2, _("_Y:"), 0.0, 0.5, spin_pos_y,
                           1);

  g_signal_connect(adj, "value-changed", G_CALLBACK(apply_settings), NULL);

  gimp_help_set_help_data(spin_pos_y, _("Light source Y position in XYZ space"),
                          NULL);

  spin_pos_z = spin_button_new(&adj, mapvals.lightsource[k].position.z, -100.0,
                               100.0, 0.1, 1.0, 0.0, 0.0, 2);
  gtk_widget_set_halign(spin_pos_z, GTK_ALIGN_START);
  gtk_widget_set_margin_end(spin_pos_z, 12);
  gimp_grid_attach_aligned(GTK_GRID(grid), 2, 3, _("_Z:"), 0.0, 0.5, spin_pos_z,
                           1);

  g_signal_connect(adj, "value-changed", G_CALLBACK(apply_settings), NULL);

  gimp_help_set_help_data(spin_pos_z, _("Light source Z position in XYZ space"),
                          NULL);
  ...
  spin_dir_x = spin_button_new(&adj, mapvals.lightsource[k].direction.x, -100.0,
                               100.0, 0.1, 1.0, 0.0, 0.0, 2);
  gtk_widget_set_halign(spin_dir_x, GTK_ALIGN_START);
  gimp_grid_attach_aligned(GTK_GRID(grid), 4, 1, _("X:"), 0.0, 0.5, spin_dir_x,
                           1);
  ...
  spin_dir_y = spin_button_new(&adj, mapvals.lightsource[k].direction.y, -100.0,
                               100.0, 0.1, 1.0, 0.0, 0.0, 2);
  gtk_widget_set_halign(spin_dir_y, GTK_ALIGN_START);
  gimp_grid_attach_aligned(GTK_GRID(grid), 4, 2, _("Y:"), 0.0, 0.5, spin_dir_y,
                           1);
  ...
  spin_dir_z = spin_button_new(&adj, mapvals.lightsource[k].direction.z, -100.0,
                               100.0, 0.1, 1.0, 0.0, 0.0, 2);
  gtk_widget_set_halign(spin_dir_z, GTK_ALIGN_START);
  gimp_grid_attach_aligned(GTK_GRID(grid), 4, 3, _("Z:"), 0.0, 0.5, spin_dir_z,
                           1);
  ...
  g_signal_connect(isolate_button, "toggled",
                   G_CALLBACK(isolate_selected_light), NULL);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(save_lighting_preset), NULL);
  gtk_widget_show(button);

  button = gtk_button_new_with_mnemonic(_("_Open"));
  gtk_grid_attach(GTK_GRID(grid), button, 4, 6, 2, 1);
  g_signal_connect(button, "clicked", G_CALLBACK(load_lighting_preset), NULL);
  gtk_widget_show(button);
...
static GtkWidget *create_material_page(void) {
  ...
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Glowing:"), 0.0,
                                   0.5, image, 1);
  gtk_size_group_add_widget(group, label);

  spinbutton = spin_button_new(&adj, mapvals.material.ambient_int, 0,
                               G_MAXFLOAT, 0.01, 0.1, 0.0, 0.0, 2);
  ...
  g_signal_connect(adj, "value-changed",
                   G_CALLBACK(interactive_preview_callback), NULL);
  ...
  image = gtk_image_new_from_icon_name(LIGHTING_INTENSITY_DIFFUSE_LOW,
                                       GTK_ICON_SIZE_BUTTON);
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Bright:"), 0.0,
                                   0.5, image, 1);
  gtk_size_group_add_widget(group, label);

  spinbutton = spin_button_new(&adj, mapvals.material.diffuse_int, 0,
                               G_MAXFLOAT, 0.01, 0.1, 0.0, 0.0, 2);
  ...
  image = gtk_image_new_from_icon_name(LIGHTING_REFLECTIVITY_SPECULAR_LOW,
                                       GTK_ICON_SIZE_BUTTON);
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("_Shiny:"), 0.0, 0.5,
                                   image, 1);
  gtk_size_group_add_widget(group, label);

  spinbutton = spin_button_new(&adj, mapvals.material.specular_ref, 0,
                               G_MAXFLOAT, 0.01, 0.1, 0.0, 0.0, 2);
  ...
  gimp_help_set_help_data(
      spinbutton, _("Controls how intense the highlights will be"), NULL);
  ...
  image = gtk_image_new_from_icon_name(LIGHTING_REFLECTIVITY_HIGHLIGHT_LOW,
                                       GTK_ICON_SIZE_BUTTON);
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("_Polished:"), 0.0,
                                   0.5, image, 1);
  gtk_size_group_add_widget(group, label);

  spinbutton = spin_button_new(&adj, mapvals.material.highlight, 0, G_MAXFLOAT,
                               0.01, 0.1, 0.0, 0.0, 2);
  ...
  gtk_label_set_mnemonic_widget(GTK_LABEL(label), spinbutton);
  gimp_help_set_help_data(
      spinbutton, _("Higher values makes the highlights more focused"), NULL);
  ...
  g_signal_connect(button, "toggled", G_CALLBACK(interactive_preview_callback),
                   NULL);
...
static GtkWidget *create_bump_page(void) {
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle), mapvals.bump_mapped);
  gtk_frame_set_label_widget(GTK_FRAME(frame), toggle);
  gtk_widget_show(toggle);

  g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_toggle_button_update),
                   &mapvals.bump_mapped);
  g_signal_connect(toggle, "toggled", G_CALLBACK(interactive_preview_callback),
                   NULL);

  gimp_help_set_help_data(toggle,
                          _("Enable/disable bump-mapping (image depth)"), NULL);
  ...
  g_signal_connect(combo, "changed", G_CALLBACK(mapmenu2_callback),
                   &mapvals.bumpmap_id);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Bumpm_ap image:"), 0.0, 0.5,
                           combo, 1);

  combo = gimp_int_combo_box_new(
      _("Linear"), LINEAR_MAP, _("Logarithmic"), LOGARITHMIC_MAP,
      _("Sinusoidal"), SINUSOIDAL_MAP, _("Spherical"), SPHERICAL_MAP, NULL);
  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), mapvals.bumpmaptype);

  g_signal_connect(combo, "changed", G_CALLBACK(mapmenu2_callback),
                   &mapvals.bumpmaptype);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Cu_rve:"), 0.0, 0.5, combo,
                           1);

  spinbutton = spin_button_new(&adj, mapvals.bumpmax, 0, G_MAXFLOAT, 0.01, 0.1,
                               0.0, 0.0, 2);
  gtk_widget_set_halign(spinbutton, GTK_ALIGN_START);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Ma_ximum height:"), 0.0,
                           0.5, spinbutton, 1);
  g_signal_connect(adj, "value-changed",
                   G_CALLBACK(gimp_double_adjustment_update), &mapvals.bumpmax);
  g_signal_connect(adj, "value-changed",
                   G_CALLBACK(interactive_preview_callback), NULL);

  gimp_help_set_help_data(spinbutton, _("Maximum height for bumps"), NULL);
...
static GtkWidget *create_environment_page(void) {
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle), mapvals.env_mapped);
  gtk_frame_set_label_widget(GTK_FRAME(frame), toggle);
  gtk_widget_show(toggle);
  ...
  gimp_help_set_help_data(
      toggle, _("Enable/disable environment-mapping (reflection)"), NULL);
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), mapvals.envmap_id,
                             G_CALLBACK(envmap_combo_callback), NULL, NULL);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("En_vironment image:"), 0.0,
                           0.5, combo, 1);
...
gboolean main_dialog(GimpDrawable *drawable) {
  ...
  appwin = gimp_dialog_new(_("Lighting Effects"), PLUG_IN_ROLE, NULL, 0,
  ...
  gimp_dialog_set_alternative_button_order(GTK_DIALOG(appwin), GTK_RESPONSE_OK,
  ...
  gtk_widget_set_events(previewarea,
                        (GDK_EXPOSURE_MASK | GDK_BUTTON1_MOTION_MASK |
                         GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK));
  g_signal_connect(previewarea, "event", G_CALLBACK(preview_events),
                   previewarea);
  g_signal_connect(previewarea, "draw", G_CALLBACK(preview_draw), previewarea);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(preview_callback), NULL);
  gtk_widget_show(button);
  ...
  gimp_help_set_help_data(
      toggle, _("Enable/disable real time preview of changes"), NULL);
...
static void save_lighting_preset(GtkWidget *widget, gpointer data) {
  ...
  if (!window) {
    window = gtk_file_chooser_dialog_new(
        _("Save Lighting Preset"), GTK_WINDOW(appwin),
    ...
    gimp_dialog_set_alternative_button_order(
        GTK_DIALOG(window), GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL, -1);
    ...
    g_signal_connect(window, "destroy", G_CALLBACK(gtk_widget_destroyed),
                     &window);
    g_signal_connect(window, "response", G_CALLBACK(save_preset_response),
                     NULL);
  }
...
static void save_preset_response(GtkFileChooser *chooser, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    if (!fp) {
      g_message(_("Could not open '%s' for writing: %s"), filename,
                g_strerror(errno));
    } else {
      ...
      for (k = 0; k < NUM_LIGHTS; k++)
        if (mapvals.lightsource[k].type != NO_LIGHT) {
          ...
          case SPOT_LIGHT:
            ...
            break;
          default:
            g_warning("Unknown light type: %d", mapvals.lightsource[k].type);
            continue;
          }
...
static void load_lighting_preset(GtkWidget *widget, gpointer data) {
  ...
  if (!window) {
    window = gtk_file_chooser_dialog_new(
        _("Load Lighting Preset"), GTK_WINDOW(appwin),
    ...
    g_signal_connect(window, "destroy", G_CALLBACK(gtk_widget_destroyed),
                     &window);
    g_signal_connect(window, "response", G_CALLBACK(load_preset_response),
                     NULL);
  }
...
static void load_preset_response(GtkFileChooser *chooser, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    if (!fp) {
      g_message(_("Could not open '%s' for reading: %s"), filename,
                g_strerror(errno));
    } else {
      ...
      for (k = 0; k < num_lights; k++) {
        ...
        snprintf(fmt_str, sizeof(fmt_str),
                 " Position: %%%" G_GSIZE_FORMAT "s %%%" G_GSIZE_FORMAT
                 "s %%%" G_GSIZE_FORMAT "s",
                 sizeof(buffer1) - 1, sizeof(buffer2) - 1, sizeof(buffer3) - 1);
        ...
        snprintf(fmt_str, sizeof(fmt_str),
                 " Direction: %%%" G_GSIZE_FORMAT "s %%%" G_GSIZE_FORMAT
                 "s %%%" G_GSIZE_FORMAT "s",
                 sizeof(buffer1) - 1, sizeof(buffer2) - 1, sizeof(buffer3) - 1);
        fscanf(fp, fmt_str, buffer1, buffer2, buffer3);
        ...
        snprintf(fmt_str, sizeof(fmt_str),
                 " Color: %%%" G_GSIZE_FORMAT "s %%%" G_GSIZE_FORMAT
                 "s %%%" G_GSIZE_FORMAT "s",
                 sizeof(buffer1) - 1, sizeof(buffer2) - 1, sizeof(buffer3) - 1);
        fscanf(fp, fmt_str, buffer1, buffer2, buffer3);
        ...
        snprintf(fmt_str, sizeof(fmt_str), " Intensity: %%%" G_GSIZE_FORMAT "s",
                 sizeof(buffer1) - 1);
