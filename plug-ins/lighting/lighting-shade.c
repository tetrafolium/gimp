...
static GimpRGB phong_shade(GimpVector3 *position, GimpVector3 *viewpoint,
                           GimpVector3 *normal, GimpVector3 *lightposition,
                           GimpRGB *diff_col, GimpRGB *light_col,
  ...
  if (nl >= 0.0) {
    ...
    if (mapvals.material
            .metallic) /* for metals, specular color = diffuse color */
    {
 ...
 * using the next row
 */
void interpol_row(gint x1, gint x2, gint y) {
  ...
  gegl_buffer_get(bump_buffer, GEGL_RECTANGLE(x1, y, x2 - x1, 1), 1.0,
                  bump_format, bumprow1, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);

  gegl_buffer_get(bump_buffer, GEGL_RECTANGLE(x1, y - 1, x2 - x1, 1), 1.0,
                  bump_format, bumprow2, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
  ...
  for (n = 0; n < (x2 - x1); n++) {
    ...
    if (bpp > 1) {
      mapval1 = (guchar)((float)((bumprow1[n * bpp] + bumprow1[n * bpp + 1] +
                                  bumprow1[n * bpp + 2]) /
                                 3.0));
      mapval2 = (guchar)((float)((bumprow2[n * bpp] + bumprow2[n * bpp + 1] +
                                  bumprow2[n * bpp + 2]) /
                                 3.0));
    } else {
...
void precompute_normals(gint x1, gint x2, gint y) {
  ...
  gegl_buffer_get(bump_buffer, GEGL_RECTANGLE(x1, y, x2 - x1, 1), 1.0,
                  bump_format, bumprow, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
  ...
  if (mapvals.bumpmaptype > 0) {
    ...
    for (n = 0; n < (x2 - x1); n++) {
      if (bpp > 1) {
        mapval = (guchar)((float)((bumprow[n * bpp + 0] + bumprow[n * bpp + 1] +
                                   bumprow[n * bpp + 2]) /
                                  3.0));
      } else {
...
static GimpVector3 compute_reflected_ray(GimpVector3 *normal,
                                         GimpVector3 *view) {
  GimpVector3 ref;
  gdouble nl;
  ...
  if (mapvals.transparent_background && heights[1][x] == 0) {
    ...
    for (k = 0; k < NUM_LIGHTS; k++) {
      ...
      if (mapvals.bump_mapped == FALSE || mapvals.bumpmap_id == -1) {
        light_color =
            phong_shade(position, &mapvals.viewpoint, &mapvals.planenormal, p,
                        &color, &color_int, mapvals.lightsource[k].type);
      } else {
        normal = vertex_normals[1][(gint)RINT(xf)];

        light_color =
            phong_shade(position, &mapvals.viewpoint, &normal, p, &color,
                        &color_int, mapvals.lightsource[k].type);
      }
...
GimpRGB get_ray_color_ref(GimpVector3 *position) {
  ...
  if (mapvals.transparent_background && heights[1][x] == 0) {
    ...
    for (k = 0; k < NUM_LIGHTS; k++) {
      ...
      light_color =
          phong_shade(position, &mapvals.viewpoint, &normal, p, &color,
                      &color_int, mapvals.lightsource[0].type);
    }
    ...
    env_color = peek_env_map(RINT(env_width * xf), RINT(env_height * yf));

    tmpval = mapvals.material.diffuse_int;
    mapvals.material.diffuse_int = 0.;

    light_color = phong_shade(position, &mapvals.viewpoint, &normal, &r, &color,
                              &env_color, DIRECTIONAL_LIGHT);
