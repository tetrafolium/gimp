...
static void compute_preview(gint startx, gint starty, gint w, gint h) {
...
static void compute_preview_rectangle(gint *xp, gint *yp, gint *wid,
                                      gint *heig) {
  gdouble x, y, w, h;
...
static gboolean check_handle_hit(gint xpos, gint ypos) {
  gint dx, dy, r;
...
static void draw_handles(cairo_t *cr) {
  gdouble dxpos, dypos;
  ...
  case SPOT_LIGHT:
    ...
    gimp_vector_3d_to_2d(startx, starty, pw, ph, &dxpos, &dypos, &viewpoint,
                         &light_position);
  ...
  if (mapvals.lightsource[k].type != NO_LIGHT) {
    ...
    case POINT_LIGHT:
    case SPOT_LIGHT:
      cairo_arc(cr, handle_xpos, handle_ypos, LIGHT_SYMBOL_SIZE / 2, 0,
                2 * G_PI);
      cairo_fill(cr);
      break;
    case DIRECTIONAL_LIGHT:
...
void update_light(gint xpos, gint ypos) {
  ...
  case POINT_LIGHT:
  case SPOT_LIGHT:
    gimp_vector_2d_to_3d(startx, starty, pw, ph, xpos, ypos, &vp,
                         &mapvals.lightsource[k].position);
    break;
  case DIRECTIONAL_LIGHT:
    gimp_vector_2d_to_3d(startx, starty, pw, ph, xpos, ypos, &vp,
                         &mapvals.lightsource[k].direction);
    break;
  }
}
...
void interactive_preview_callback(GtkWidget *widget) {
  ...
  preview_update_timer =
      g_timeout_add(100, interactive_preview_timer_callback, NULL);
}
