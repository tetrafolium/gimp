...
guchar peek_map(GeglBuffer *buffer, const Babl *format, gint x, gint y) {
  guchar data[4];
  guchar ret_val;

  gegl_buffer_sample(buffer, x, y, NULL, data, format, GEGL_SAMPLER_NEAREST,
                     GEGL_ABYSS_NONE);
...
GimpRGB peek_env_map(gint x, gint y) {
  GimpRGB color;
  ...
  gegl_buffer_sample(env_buffer, x, y, NULL, &color,
                     babl_format("R'G'B'A double"), GEGL_SAMPLER_NEAREST,
                     GEGL_ABYSS_NONE);
...
void poke(gint x, gint y, GimpRGB *color) {
  if (x < 0)
    x = 0;
  else if (x >= width)
    x = width - 1;
...
gdouble get_map_value(GeglBuffer *buffer, const Babl *format, gdouble u,
                      gdouble v, gint *inside) {
  gint x1, y1, x2, y2;
  gdouble p[4];
...
static void compute_maps(void) {
  ...
  for (x = 0; x < 256; x++) {
    sinemap[x] = (guchar)(
        255.0 * (0.5 * (sin((G_PI * c * (gdouble)x) - 0.5 * G_PI) + 1.0)));
...
gint image_setup(GimpDrawable *drawable, gint interactive) {
  ...
  ret = gimp_drawable_mask_intersect(drawable, &border_x1, &border_y1, &w, &h);
