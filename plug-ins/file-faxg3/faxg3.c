...
static GimpImage *emitgimp(gint hcol, gint row, const gchar *bitmap,
                           gint bperrow, GFile *file);

G_DEFINE_TYPE(Faxg3, faxg3, GIMP_TYPE_PLUG_IN)

GIMP_MAIN(FAXG3_TYPE)

static void faxg3_class_init(Faxg3Class *klass) {
...
static void faxg3_init(Faxg3 *faxg3) {}
...
static GimpProcedure *faxg3_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, faxg3_load, NULL, NULL);

    gimp_procedure_set_menu_label(procedure, N_("G3 fax image"));

    gimp_procedure_set_documentation(procedure, "Loads g3 fax files",
                                     "This plug-in loads Fax G3 Image files.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Jochen Friedrich",
    ...
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/g3-fax");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "g3");
  ...
  while (rs > 0 && cons_eol < 4) /* i.e., while (!EOF) */
  {
    ...
    if (nr_pels == -1) {
      ...
      if (col == 0) {
        ...
        if (row >= max_rows) {
          gchar *p = g_try_realloc(bitmap, (max_rows += 500) * MAX_COLS / 8);
          if (p == NULL) {
...
static GimpImage *emitgimp(gint hcol, gint row, const gchar *bitmap,
  ...
  layer = gimp_layer_new(image, _("Background"), hcol, row, GIMP_GRAY_IMAGE,
  ...
  for (y = 0; y < row; y++) {
    ...
    if ((y - yy) == tile_height - 1) {
      ...
      gegl_buffer_set(buffer, GEGL_RECTANGLE(0, yy, hcol, tile_height), 0, NULL,
  ...
  if (row - yy) {
    ...
    gegl_buffer_set(buffer, GEGL_RECTANGLE(0, yy, hcol, row - yy), 0, NULL, buf,
                    GEGL_AUTO_ROWSTRIDE);
  }
