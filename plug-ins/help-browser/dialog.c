...
enum { HISTORY_TITLE, HISTORY_URI };
...
static void back_callback(GtkAction *action, gpointer data);
static void forward_callback(GtkAction *action, gpointer data);
static void reload_callback(GtkAction *action, gpointer data);
static void stop_callback(GtkAction *action, gpointer data);
static void home_callback(GtkAction *action, gpointer data);
static void find_callback(GtkAction *action, gpointer data);
static void find_again_callback(GtkAction *action, gpointer data);
static void copy_location_callback(GtkAction *action, gpointer data);
static void copy_selection_callback(GtkAction *action, gpointer data);
static void show_index_callback(GtkAction *action, gpointer data);
static void zoom_in_callback(GtkAction *action, gpointer data);
static void zoom_out_callback(GtkAction *action, gpointer data);
static void close_callback(GtkAction *action, gpointer data);
static void website_callback(GtkAction *action, gpointer data);

static void update_actions(void);

static void row_activated(GtkTreeView *tree_view, GtkTreePath *path,
                          GtkTreeViewColumn *column);
static void dialog_unmap(GtkWidget *window, GtkWidget *paned);

static void view_realize(GtkWidget *widget);
static void view_unrealize(GtkWidget *widget);
static gboolean view_popup_menu(GtkWidget *widget, GdkEventButton *event);
static gboolean view_button_press(GtkWidget *widget, GdkEventButton *event);
static gboolean view_key_press(GtkWidget *widget, GdkEventKey *event);

static void title_changed(WebKitWebView *view, GParamSpec *pspec,
                          GtkWidget *window);

static void load_changed(WebKitWebView *view, WebKitLoadEvent event);

static void select_index(const gchar *uri);

static void search_entry_changed(GtkWidget *entry);
static gboolean search_entry_key_press(GtkWidget *entry, GdkEventKey *event);
static void search_prev_clicked(GtkWidget *button, GtkWidget *entry);
static void search_next_clicked(GtkWidget *button, GtkWidget *entry);
...
void browser_dialog_open(const gchar *plug_in_binary) {
  ...
  action =
      gtk_ui_manager_get_action(ui_manager, "/ui/help-browser-popup/forward");
  ...
  action = gtk_ui_manager_get_action(ui_manager, "/ui/help-browser-popup/back");
  gtk_activatable_set_related_action(GTK_ACTIVATABLE(item), action);
  g_object_notify(G_OBJECT(action), "tooltip");
  button_prev = GTK_WIDGET(item);

  item = GTK_TOOL_ITEM(
      gtk_ui_manager_get_widget(ui_manager, "/help-browser-toolbar/space"));
  ...
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(tree_view), -1, NULL, gtk_cell_renderer_text_new(), "text",
      1, NULL);

  g_signal_connect(tree_view, "row-activated", G_CALLBACK(row_activated), NULL);
  ...
  g_signal_connect(view, "realize", G_CALLBACK(view_realize), NULL);
  g_signal_connect(view, "unrealize", G_CALLBACK(view_unrealize), NULL);

  g_signal_connect(view, "popup-menu", G_CALLBACK(view_popup_menu), NULL);
  g_signal_connect(view, "button-press-event", G_CALLBACK(view_button_press),
                   NULL);
  ...
  g_signal_connect(view, "notify::title", G_CALLBACK(title_changed), window);

  g_signal_connect(view, "load-changed", G_CALLBACK(load_changed), NULL);

  gtk_widget_grab_focus(view);

  g_signal_connect(window, "unmap", G_CALLBACK(dialog_unmap), paned);
...
static gint help_item_compare(gconstpointer a, gconstpointer b) {
...
static void add_child(GtkTreeStore *store, GimpHelpDomain *domain,
                      GimpHelpLocale *locale, GtkTreeIter *parent,
                      GimpHelpItem *item, gint depth) {
  GtkTreeIter iter;
  GList *list;
  gchar *uri;
  ...
  gtk_tree_store_set(store, &iter, 0, item, 1, item->title, -1);

  uri = g_strconcat(domain->help_uri, "/", locale->locale_id, "/", item->ref,
                    NULL);

  g_hash_table_insert(uri_hash_table, uri, gtk_tree_iter_copy(&iter));
...
void browser_dialog_make_index(GimpHelpDomain *domain, GimpHelpLocale *locale) {
  ...
  if (!locale->toplevel_items) {
    g_hash_table_foreach(locale->help_id_mapping,
                         (GHFunc)browser_dialog_make_index_foreach, locale);

    locale->toplevel_items =
        g_list_sort(locale->toplevel_items, help_item_compare);
  }
...
static void select_index(const gchar *uri) {
  ...
  if (iter) {
    ...
    gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(tree_view), scroll_path, NULL,
                                 FALSE, 0.0, 0.0);
...
static GtkUIManager *ui_manager_new(GtkWidget *window) {
  static const GtkActionEntry actions[] = {
      {"back", GIMP_ICON_GO_PREVIOUS, NULL, "<alt>Left", N_("Go back one page"),
       G_CALLBACK(back_callback)},
      {"forward", GIMP_ICON_GO_NEXT, NULL, "<alt>Right",
       N_("Go forward one page"), G_CALLBACK(forward_callback)},
      {"reload", GIMP_ICON_VIEW_REFRESH, N_("_Reload"), "<control>R",
       N_("Reload current page"), G_CALLBACK(reload_callback)},
      {"stop", GIMP_ICON_PROCESS_STOP, N_("_Stop"), "Escape",
       N_("Stop loading this page"), G_CALLBACK(stop_callback)},
      {"home", GIMP_ICON_GO_HOME, NULL, "<alt>Home", N_("Go to the index page"),
       G_CALLBACK(home_callback)},
      {"copy-location", GIMP_ICON_EDIT_COPY, N_("C_opy location"), "",
       N_("Copy the location of this page to the clipboard"),
       G_CALLBACK(copy_location_callback)},
      {"copy-selection", GIMP_ICON_EDIT_COPY, NULL, "<control>C", NULL,
       G_CALLBACK(copy_selection_callback)},
      {"zoom-in", GIMP_ICON_ZOOM_IN, NULL, "<control>plus", NULL,
       G_CALLBACK(zoom_in_callback)},
      {"zoom-out", GIMP_ICON_ZOOM_OUT, NULL, "<control>minus", NULL,
       G_CALLBACK(zoom_out_callback)},
      {"find", GIMP_ICON_EDIT_FIND, NULL, "<control>F",
       N_("Find text in current page"), G_CALLBACK(find_callback)},
      {"find-again", NULL, N_("Find _Again"), "<control>G", NULL,
       G_CALLBACK(find_again_callback)},
      {"close", GIMP_ICON_WINDOW_CLOSE, NULL, "<control>W", NULL,
       G_CALLBACK(close_callback)},
      {"quit", GIMP_ICON_APPLICATION_EXIT, NULL, "<control>Q", NULL,
       G_CALLBACK(close_callback)}};

  static const GtkToggleActionEntry toggle_actions[] = {
      {"show-index", NULL, N_("S_how Index"), "<control>I",
  ...
  action = gimp_throbber_action_new("website", "docs.gimp.org",
                                    _("Visit the GIMP documentation website"),
                                    GIMP_ICON_HELP_USER_MANUAL);
  g_signal_connect_closure(
      action, "activate",
      g_cclosure_new(G_CALLBACK(website_callback), NULL, NULL), FALSE);
...
static void home_callback(GtkAction *action, gpointer data) {
  ...
  if (domain && locale) {
    gchar *uri =
        g_strconcat(domain->help_uri, "/", locale->locale_id, "/",
                    gimp_help_locale_map(locale, GIMP_HELP_DEFAULT_ID), NULL);
...
static void show_index_callback(GtkAction *action, gpointer data) {
  gtk_widget_set_visible(
      sidebar, gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action)));
}
...
static void menu_callback(GtkWidget *menu, gpointer data) {
...
static GtkWidget *build_menu(const GList *items, gboolean back) {
  GtkWidget *menu;
  const GList *iter;
  gint steps;
  ...
  for (iter = items, steps = 1; iter; iter = g_list_next(iter), steps++) {
    ...
    if (title) {
      ...
      g_signal_connect(menu_item, "activate", G_CALLBACK(menu_callback),
...
static void update_actions(void) {
  ...
  gtk_action_set_sensitive(
      action, webkit_web_view_can_go_forward(WEBKIT_WEB_VIEW(view)));

  if (back_forward_list) {
    const GList *list;

    list = webkit_back_forward_list_get_forward_list_with_limit(
        back_forward_list, 12);
  ...
  gtk_action_set_sensitive(
      action, webkit_web_view_get_uri(WEBKIT_WEB_VIEW(view)) != NULL);
...
static void dialog_unmap(GtkWidget *window, GtkWidget *paned) {
  ...
  data.zoom =
      (view ? webkit_web_view_get_zoom_level(WEBKIT_WEB_VIEW(view)) : 1.0);
...
static gboolean view_key_press(GtkWidget *widget, GdkEventKey *event) {
  if (event->keyval == GDK_KEY_slash) {
    ...
    action =
        gtk_ui_manager_get_action(ui_manager, "/ui/help-browser-popup/find");
...
static void title_changed(WebKitWebView *view, GParamSpec *pspec,
  ...
  full_title = g_strdup_printf("%s - %s", title ? title : _("Untitled"),
                               _("GIMP Help Browser"));
...
static void load_changed(WebKitWebView *view, WebKitLoadEvent event) {
  GtkAction *action =
      gtk_ui_manager_get_action(ui_manager, "/ui/help-browser-popup/stop");
  switch (event) {
...
static GtkWidget *build_searchbar(void) {
  ...
  g_signal_connect(entry, "key-press-event", G_CALLBACK(search_entry_key_press),
                   NULL);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(search_prev_clicked), entry);

  button = gtk_button_new_with_mnemonic(C_("search", "_Next"));
  gtk_button_set_relief(GTK_BUTTON(button), GTK_RELIEF_NONE);
  gtk_button_set_image(
      GTK_BUTTON(button),
      gtk_image_new_from_icon_name(GIMP_ICON_GO_NEXT, GTK_ICON_SIZE_BUTTON));
  gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, FALSE, 0);
  gtk_widget_show(button);

  g_signal_connect(button, "clicked", G_CALLBACK(search_next_clicked), entry);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(search_close_clicked), NULL);
