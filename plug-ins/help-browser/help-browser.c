...
#define HELP_BROWSER                                                           \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), HELP_BROWSER_TYPE, HelpBrowser))
...
static void help_browser_init(HelpBrowser *help_browser) {}
...
static GimpProcedure *help_browser_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, GIMP_HELP_BROWSER_EXT_PROC)) {
    ...
    gimp_procedure_set_documentation(procedure, "Browse the GIMP user manual",
...
static void temp_proc_install(GimpPlugIn *plug_in) {
  ...
  procedure = gimp_procedure_new(plug_in, GIMP_HELP_BROWSER_TEMP_EXT_PROC,
                                 GIMP_PDB_PROC_TYPE_TEMPORARY, temp_proc_run,
                                 NULL, NULL);

  gimp_procedure_set_documentation(procedure, "DON'T USE THIS ONE",
                                   "(Temporary procedure)", NULL);
  gimp_procedure_set_attribution(procedure,
                                 "Sven Neumann <sven@gimp.org>, "
                                 "Michael Natterer <mitch@gimp.org>"
                                 "Henrik Brix Andersen <brix@gimp.org>",
  ...
  GIMP_PROC_ARG_STRING(procedure, "help-domain", "Help domain",
                       "Help domain to use", NULL, G_PARAM_READWRITE);

  GIMP_PROC_ARG_STRING(procedure, "help-locales", "Help locales",
                       "Language to use", NULL, G_PARAM_READWRITE);

  GIMP_PROC_ARG_STRING(procedure, "help-id", "Help ID", "Help ID to open", NULL,
                       G_PARAM_READWRITE);
...
static gboolean help_browser_show_help(const gchar *help_domain,
  ...
  if (domain) {
    ...
    uri = gimp_help_domain_map(domain, locales, help_id, progress, &locale,
                               &fatal_error);
...
static void help_browser_progress_start(const gchar *message,
                                        gboolean cancelable,
                                        gpointer user_data) {
  gimp_progress_init(message);
}

static void help_browser_progress_update(gdouble value, gpointer user_data) {
  gimp_progress_update(value);
}

static void help_browser_progress_end(gpointer user_data) {
  gimp_progress_end();
}

static GimpHelpProgress *help_browser_progress_new(void) {
  static const GimpHelpProgressVTable vtable = {help_browser_progress_start,
