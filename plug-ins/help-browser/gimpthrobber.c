...
enum { PROP_0, PROP_ICON_NAME, PROP_IMAGE };
static guint toolbutton_signals[LAST_SIGNAL] = {0};

static void gimp_throbber_set_property(GObject *object, guint prop_id,
                                       const GValue *value, GParamSpec *pspec);
static void gimp_throbber_get_property(GObject *object, guint prop_id,
...
static void gimp_throbber_class_init(GimpThrobberClass *klass) {
  ...
  toolbutton_signals[CLICKED] =
      g_signal_new("clicked", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpThrobberClass, clicked), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);
}
...
static void gimp_throbber_init(GimpThrobber *button) {
  ...
  priv->button = g_object_new(GTK_TYPE_BUTTON, "yalign", 0.0, "focus-on-click",
                              FALSE, NULL);

  g_signal_connect_object(priv->button, "clicked",
                          G_CALLBACK(gimp_throbber_button_clicked), button, 0);
...
static void gimp_throbber_construct_contents(GtkToolItem *tool_item) {
  ...
  if (style == GTK_TOOLBAR_TEXT) {
    image = gtk_image_new_from_icon_name(priv->icon_name, GTK_ICON_SIZE_MENU);
    ...
    image = gtk_image_new_from_icon_name(priv->icon_name, GTK_ICON_SIZE_DND);
  }
...
GtkToolItem *gimp_throbber_new(const gchar *icon_name) {
  return g_object_new(GIMP_TYPE_THROBBER, "icon-name", icon_name, NULL);
}
