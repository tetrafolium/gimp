...
gboolean jpeg_detect_original_settings(struct jpeg_decompress_struct *cinfo,
  ...
  *dest++ = num_quant_tables;
  ...
  parasite = gimp_parasite_new("jpeg-settings", GIMP_PARASITE_PERSISTENT,
                               parasite_size, parasite_data);
 ...
 */
gboolean jpeg_restore_original_settings(GimpImage *image, gint *quality,
  ...
  if (parasite) {
    ...
    if (src_size >= 4) {
      ...
      if (src_size >= (4 + num_components * 2 + num_tables * 128) && q <= 100 &&
          num_tables <= 4) {
        *quality = q;
 ...
 */
guint **jpeg_restore_original_tables(GimpImage *image, gint num_quant_tables) {
  GimpParasite *parasite;
  const guchar *src;
  glong src_size;
  gint num_components;
      ...
      if (src_size >= (4 + num_components * 2 + num_tables * 128)) {
        ...
        *dest++ = *src++;
        ...
        for (t = 0; t < num_tables; t++) {
          ...
          if (src_size > (4 + num_components * 2 + num_tables * 128)) {
            memcpy(dest, src,
                   src_size - (4 + num_components * 2 + num_tables * 128));
          }
        }
        gimp_parasite_free(parasite);
        parasite = gimp_parasite_new("jpeg-settings", GIMP_PARASITE_PERSISTENT,
