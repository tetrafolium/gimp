...
static gpointer jpeg_load_cmyk_transform(guint8 *profile_data,
                                         gsize profile_len);
static void jpeg_load_cmyk_to_rgb(guchar *buf, glong pixels,
                                  gpointer transform);

GimpImage *volatile preview_image;
GimpLayer *preview_layer;

GimpImage *load_image(GFile *file, GimpRunMode runmode, gboolean preview,
                      gboolean *resolution_loaded, GError **error) {
  ...
  buf =
      g_new(guchar, tile_height * cinfo.output_width * cinfo.output_components);
  ...
  if (preview) {
    ...
    image =
        gimp_image_new_with_precision(cinfo.output_width, cinfo.output_height,
                                      image_type, GIMP_PRECISION_U8_NON_LINEAR);
    ...
    } else if (icc_data) /* don't attach the profile if we are transforming */
    {
      ...
      profile =
          gimp_color_profile_new_from_icc_profile(icc_data, icc_length, NULL);
      if (profile) {
  ...
  layer = gimp_layer_new(image, layer_name, cinfo.output_width,
                         cinfo.output_height, layer_type, 100,
  ...
  format =
      babl_format_with_space(image_type == GIMP_RGB ? "R'G'B' u8" : "Y' u8",
  ...
  set_buffer:
    gegl_buffer_set(buffer,
                    GEGL_RECTANGLE(0, start, cinfo.output_width, scanlines), 0,
...
static gboolean jpeg_load_resolution(GimpImage *image,
                                     struct jpeg_decompress_struct *cinfo) {
  if (cinfo->saw_JFIF_marker && cinfo->X_density != 0 &&
      cinfo->Y_density != 0) {
...
GimpImage *load_thumbnail_image(GFile *file, gint *width, gint *height,
                                GimpImageType *type, GError **error) {
...
static gpointer jpeg_load_cmyk_transform(guint8 *profile_data,
  ...
  transform =
      cmsCreateTransform(cmyk_lcms, TYPE_CMYK_8_REV, rgb_lcms, TYPE_RGB_8,
