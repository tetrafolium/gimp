...
static GimpValueArray *jpeg_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                 GFile *file, const GimpValueArray *args,
                                 gpointer run_data);
static GimpValueArray *jpeg_load_thumb(GimpProcedure *procedure, GFile *file,
                                       gint size, const GimpValueArray *args,
                                       gpointer run_data);
static GimpValueArray *jpeg_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void jpeg_class_init(JpegClass *klass) {
...
static void jpeg_init(Jpeg *jpeg) {}
...
static GimpProcedure *jpeg_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, jpeg_load, NULL, NULL);

    gimp_procedure_set_menu_label(procedure, N_("JPEG image"));

    gimp_procedure_set_documentation(
        procedure, "Loads files in the JPEG file format",
        "Loads files in the JPEG file format", name);
    gimp_procedure_set_attribution(
        procedure, "Spencer Kimball, Peter Mattis & others",
        "Spencer Kimball & Peter Mattis", "1995-2007");
    ...
    procedure = gimp_thumbnail_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, jpeg_load_thumb, NULL, NULL);
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, jpeg_save, NULL, NULL);
     ...
     * settings
     */
    GIMP_PROC_ARG_DOUBLE(procedure, "quality", "_Quality",
                         "Quality of exported image", 0.0, 1.0, 0.9,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "smoothing", "S_moothing",
                         "Smoothing factor for exported image", 0.0, 1.0, 0.0,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "optimize", "Optimi_ze",
                          "Use optimized tables during Huffman coding", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "progressive", "_Progressive",
                          "Create progressive JPEG images", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "sub-sampling", _("Su_bsampling"),
                      ...
                      JPEG_SUBSAMPLING_1x1_1x1_1x1, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "baseline", "Baseline",
                          "Force creation of a baseline JPEG "
                          "(non-baseline JPEGs can't be read by all decoders)",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "restart", _("Inter_val (MCU rows):"),
                      "Interval of restart markers "
                      "(in MCU rows, 0 = no restart markers)",
                      0, 64, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "dct", _("_DCT method"),
    ...
    GIMP_PROC_AUX_ARG_INT(procedure, "original-quality", NULL, NULL, -1, 100,
                          -1, G_PARAM_READWRITE);
    GIMP_PROC_AUX_ARG_INT(procedure, "original-sub-sampling", NULL, NULL,
                          JPEG_SUBSAMPLING_2x2_1x1_1x1,
                          JPEG_SUBSAMPLING_1x2_1x1_1x1,
                          JPEG_SUBSAMPLING_2x2_1x1_1x1, G_PARAM_READWRITE);
    GIMP_PROC_AUX_ARG_INT(procedure, "original-num-quant-tables", NULL, NULL,
                          -1, 4, -1, G_PARAM_READWRITE);

    GIMP_PROC_AUX_ARG_BOOLEAN(
        procedure, "show-preview", "Sho_w preview in image window",
        "Creates a temporary layer with an export preview", FALSE,
        G_PARAM_READWRITE);
    ...
    GIMP_PROC_AUX_ARG_BOOLEAN(procedure, "use-restart",
                              _("Use restart mar_kers"), NULL, FALSE,
                              G_PARAM_READWRITE);
...
static GimpValueArray *jpeg_load(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  image = load_image(file, run_mode, FALSE, &resolution_loaded, &error);

  if (image) {
    GimpMetadata *metadata;

    metadata =
        gimp_image_metadata_load_prepare(image, "image/jpeg", file, NULL);
    ...
    if (metadata) {
      ...
      gimp_image_metadata_load_finish(image, "image/jpeg", metadata, flags);
...
static GimpValueArray *jpeg_load_thumb(GimpProcedure *procedure, GFile *file,
  ...
  image = load_thumbnail_image(file, &width, &height, &type, &error);
...
static GimpValueArray *jpeg_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    case GIMP_EXPORT_EXPORT: {
      gchar *tmp =
          g_filename_from_utf8(_("Export Preview"), -1, NULL, NULL, NULL);
      if (tmp) {
  ...
  case GIMP_RUN_WITH_LAST_VALS: {
    ...
    jpeg_restore_original_settings(orig_image, &orig_quality, &orig_subsmp,
                                   &orig_num_quant_tables);

    g_object_get(config, "quality", &dquality, "sub-sampling", &subsmp, NULL);
  ...
  g_object_set(config, "original-sub-sampling", orig_subsmp, "original-quality",
               orig_quality, "original-num-quant-tables", orig_num_quant_tables,
               NULL);
  ...
  if (status == GIMP_PDB_SUCCESS) {
    if (!save_image(file, config, image, drawables[0], orig_image, FALSE,
                    &error)) {
