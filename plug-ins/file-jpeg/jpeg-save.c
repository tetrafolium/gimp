 ...
 */
static void background_error_exit(j_common_ptr cinfo) {
  if (prev_p)
    prev_p->abort_me = TRUE;
  (*cinfo->err->output_message)(cinfo);
}

static gboolean background_jpeg_save(PreviewPersistent *pp) {
  gint yend;
  ...
  if (pp->abort_me || (pp->cinfo.next_scanline >= pp->cinfo.image_height)) {
    ...
    if (!pp->abort_me) {
      ...
      info = g_file_query_info(pp->file, G_FILE_ATTRIBUTE_STANDARD_SIZE,
      ...
      load_image(pp->file, GIMP_RUN_NONINTERACTIVE, TRUE, NULL, NULL);
    }
    ...
    if ((pp->cinfo.next_scanline % pp->tile_height) == 0) {
      ...
      gegl_buffer_get(
          pp->buffer,
          GEGL_RECTANGLE(0, pp->cinfo.next_scanline, pp->cinfo.image_width,
                         (yend - pp->cinfo.next_scanline)),
          1.0, pp->format, pp->data, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
      pp->src = pp->data;
    }
...
gboolean save_image(GFile *file, GimpProcedureConfig *config, GimpImage *image,
  ...
  g_object_get(config, "quality", &dquality, "smoothing", &smoothing,
               "optimize", &optimize, "progressive", &progressive,
               "sub-sampling", &subsmp, "baseline", &baseline, "restart",
               &restart, "dct", &dct,
               ...
               "use-arithmetic-coding", &use_arithmetic_coding, "use-restart",
               &use_restart,

               "save-color-profile", &save_profile, "save-comment",
               &save_comment, "gimp-comment", &comment,
  ...
  cinfo.in_color_space =
      (drawable_type == GIMP_RGB_IMAGE || drawable_type == GIMP_RGBA_IMAGE)
          ? JCS_RGB
          : JCS_GRAYSCALE;
  ...
  if (use_orig_quality && orig_num_quant_tables > 0) {
    ...
    if (quant_tables) {
      for (t = 0; t < orig_num_quant_tables; t++) {
        jpeg_add_quant_table(&cinfo, t, quant_tables[t], 100, baseline);
  ...
  subsampling =
      (gimp_drawable_is_rgb(drawable) ? subsmp : JPEG_SUBSAMPLING_1x1_1x1_1x1);
...

  {
    ...
    if (xresolution > 1e-5 && yresolution > 1e-5) {
      ...
      if (factor == 2.54 /* cm */ || factor == 25.4 /* mm */) {
  ...
  if (save_comment && comment && *comment) {
    ...
    g_print("jpeg-save: saving image comment (%d bytes)\n",
            (int)strlen(comment));
#endif
    jpeg_write_marker(&cinfo, JPEG_COM, (guchar *)comment, strlen(comment));
  }
  ...
  if (preview) {
    ...
    gtk_label_set_text(GTK_LABEL(preview_size), _("Calculating file size..."));
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    if ((cinfo.next_scanline % gimp_tile_height()) == 0) {
      ...
      gegl_buffer_get(buffer,
                      GEGL_RECTANGLE(0, cinfo.next_scanline, cinfo.image_width,
...
static void make_preview(GimpProcedureConfig *config) {
  ...
  if (show_preview) {
    ...
    save_image(file, config, preview_image, drawable_global, orig_image_global,
               TRUE, NULL);
...
gboolean save_dialog(GimpProcedure *procedure, GimpProcedureConfig *config,
  ...
  g_object_get(config, "original-quality", &orig_quality, "restart", &restart,
               NULL);

  dialog = gimp_save_procedure_dialog_new(
      GIMP_SAVE_PROCEDURE(procedure), GIMP_PROCEDURE_CONFIG(config),
      gimp_item_get_image(GIMP_ITEM(drawable)));

  /* custom quantization tables - now used also for original quality */
  widget = gimp_procedure_dialog_get_widget(
      GIMP_PROCEDURE_DIALOG(dialog), "use-original-quality", G_TYPE_NONE);
  ...
  g_signal_connect(config, "notify::quality", G_CALLBACK(quality_changed),
                   NULL);
  g_signal_connect(config, "notify::use-original-quality",
                   G_CALLBACK(use_orig_qual_changed), NULL);

  /* File size label. */
  preview_size = gimp_procedure_dialog_get_label(
      GIMP_PROCEDURE_DIALOG(dialog), "preview-size", _("File size: unknown"));
  gtk_label_set_xalign(GTK_LABEL(preview_size), 0.0);
  gtk_label_set_ellipsize(GTK_LABEL(preview_size), PANGO_ELLIPSIZE_END);
  gimp_label_set_attributes(GTK_LABEL(preview_size), PANGO_ATTR_STYLE,
  ...
  gimp_procedure_dialog_fill_frame(GIMP_PROCEDURE_DIALOG(dialog),
                                   "arithmetic-frame", "use-arithmetic-coding",
                                   TRUE, "optimize");
#endif
  ...
  gimp_procedure_dialog_fill_frame(GIMP_PROCEDURE_DIALOG(dialog),
                                   ...
                                   "restart");
  if (restart == 0)
    g_object_set(config, "restart", DEFAULT_RESTART_MCU_ROWS, "use-restart",
                 FALSE, NULL);

  /* Subsampling */
  store = gimp_int_store_new(
      _("4:4:4 (best quality)"), JPEG_SUBSAMPLING_1x1_1x1_1x1,
      _("4:2:2 horizontal (chroma halved)"), JPEG_SUBSAMPLING_2x1_1x1_1x1,
      _("4:2:2 vertical (chroma halved)"), JPEG_SUBSAMPLING_1x2_1x1_1x1,
      _("4:2:0 (chroma quartered)"), JPEG_SUBSAMPLING_2x2_1x1_1x1, NULL);
  widget = gimp_procedure_dialog_get_int_combo(
      GIMP_PROCEDURE_DIALOG(dialog), "sub-sampling", GIMP_INT_STORE(store));
  ...
  /* DCT method */
  store = gimp_int_store_new(_("Fast Integer"), 1, _("Integer"), 0,
                             _("Floating-Point"), 2, NULL);
  gimp_procedure_dialog_get_int_combo(GIMP_PROCEDURE_DIALOG(dialog), "dct",
  ...
  if (gimp_drawable_is_rgb(drawable)) {
    g_signal_connect(config, "notify::sub-sampling",
                     G_CALLBACK(subsampling_changed), widget);
    subsampling_changed(config, NULL, widget);
    g_signal_connect(config, "notify::use-original-quality",
                     G_CALLBACK(use_orig_qual_changed_rgb), NULL);
  }

  gimp_procedure_dialog_fill_box(GIMP_PROCEDURE_DIALOG(dialog),
                                 "advanced-options", "smoothing", "progressive",
#ifdef C_ARITH_CODING_SUPPORTED
                                 "arithmetic-frame",
#else
                                 "optimize",
#endif
                                 "restart-frame", "sub-sampling", "dct", NULL);
  gimp_procedure_dialog_fill_frame(GIMP_PROCEDURE_DIALOG(dialog),
                                   "advanced-frame", "advanced-title", FALSE,
                                   "advanced-options");

  gimp_procedure_dialog_fill(GIMP_PROCEDURE_DIALOG(dialog), "quality",
                             "use-original-quality", "preview-size",
                             "show-preview", "advanced-frame", NULL);

  gtk_window_set_resizable(GTK_WINDOW(dialog), FALSE);

  /* Run make_preview() when various config are changed. */
  g_signal_connect(config, "notify", G_CALLBACK(make_preview), NULL);
...
static void quality_changed(GimpProcedureConfig *config) {
  ...
  g_object_get(config, "use-original-quality", &use_orig_quality,
               "original-quality", &orig_quality, "quality", &quality, NULL);
...
static void subsampling_changed(GimpProcedureConfig *config,
  ...
  g_object_get(config, "use-original-quality", &use_orig_quality,
               "original-sub-sampling", &orig_subsmp, "sub-sampling", &subsmp,
               NULL);
...
static void use_orig_qual_changed(GimpProcedureConfig *config) {
  ...
  g_object_get(config, "use-original-quality", &use_orig_quality,
               "original-quality", &orig_quality, NULL);
...
static void use_orig_qual_changed_rgb(GimpProcedureConfig *config) {
  ...
  g_object_get(config, "use-original-quality", &use_orig_quality,
               "original-sub-sampling", &orig_subsmp, "original-quality",
               &orig_quality, NULL);
