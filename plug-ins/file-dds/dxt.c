...
static const vec4_t V4ONETHIRD =
    VEC4_CONST3(1.0f / 3.0f, 1.0f / 3.0f, 1.0f / 3.0f);
static const vec4_t V4TWOTHIRDS =
    VEC4_CONST3(2.0f / 3.0f, 2.0f / 3.0f, 2.0f / 3.0f);
static const vec4_t V4GRID = VEC4_CONST3(31.0f, 63.0f, 31.0f);
static const vec4_t V4GRIDRCP =
    VEC4_CONST3(1.0f / 31.0f, 1.0f / 63.0f, 1.0f / 31.0f);
...
static void extract_block(const unsigned char *src, int x, int y, int w, int h,
                          unsigned char *block) {
  int i, j;
  int bw = MIN(w - x, 4);
  int bh = MIN(h - y, 4);
...
static inline unsigned short pack_rgb565(const unsigned char *c) {
  return (mul8bit(c[2], 31) << 11) | (mul8bit(c[1], 63) << 5) |
         (mul8bit(c[0], 31));
}
...
static void vec4_endpoints_to_565(int *start, int *end, const vec4_t a,
                                  const vec4_t b) {
...
static void dxtblock_init(dxtblock_t *dxtb, const unsigned char *block,
                          int flags) {
  int i, c0, c;
...
static void encode_color_block(unsigned char *dst, unsigned char *block,
                               int flags) {
  dxtblock_t dxtb;
  int max16, min16;
  ...
  {
    max16 = (omatch5[block[2]][0] << 11) | (omatch6[block[1]][0] << 5) |
            (omatch5[block[0]][0]);
    min16 = (omatch5[block[2]][1] << 11) | (omatch6[block[1]][1] << 5) |
            (omatch5[block[0]][1]);
  ...
  } else if ((flags & DXT_BC1) &&
             dxtb.alphamask) // DXT1 compression, non-opaque block
  {
...
static void scale_YCoCg(unsigned char *block, unsigned char *mincolor,
                        unsigned char *maxcolor) {
...
#define BLOCK_OFFSET(x, y, w, bs)                                              \
  (((y) >> 2) * ((bs) * (((w) + 3) >> 2)) + ((bs) * ((x) >> 2)))

static void compress_BC1(unsigned char *dst, const unsigned char *src, int w,
                         int h, int flags) {
...
static void compress_BC2(unsigned char *dst, const unsigned char *src, int w,
                         int h, int flags) {
  const unsigned int block_count = BLOCK_COUNT(w, h);
  unsigned int i;
  unsigned char block[64], *p;
...
static void compress_BC3(unsigned char *dst, const unsigned char *src, int w,
                         int h, int flags) {
  const unsigned int block_count = BLOCK_COUNT(w, h);
  unsigned int i;
  unsigned char block[64], *p;
...
static void compress_BC4(unsigned char *dst, const unsigned char *src, int w,
                         int h) {
  const unsigned int block_count = BLOCK_COUNT(w, h);
  unsigned int i;
  unsigned char block[64], *p;
...
static void compress_BC5(unsigned char *dst, const unsigned char *src, int w,
                         int h) {
  const unsigned int block_count = BLOCK_COUNT(w, h);
  unsigned int i;
  unsigned char block[64], *p;
...
static void compress_YCoCg(unsigned char *dst, const unsigned char *src, int w,
                           int h) {
  const unsigned int block_count = BLOCK_COUNT(w, h);
  unsigned int i;
  unsigned char block[64], *p;
...
static void decode_alpha_block_BC3(unsigned char *block, unsigned char *src,
                                   int w) {
  int x, y, code;
  unsigned char *d = block;
  ...
  for (y = 0; y < 4; ++y) {
    for (x = 0; x < 4; ++x) {
      if (format == DDS_COMPRESS_BC3) {
        tmp = block[y * 16 + (x * 4)];
        make_normal(&block[y * 16 + (x * 4)], block[y * 16 + (x * 4) + 3],
                    block[y * 16 + (x * 4) + 1]);
        block[y * 16 + (x * 4) + 3] = tmp;
      } else if (format == DDS_COMPRESS_BC5) {
        make_normal(&block[y * 16 + (x * 4)], block[y * 16 + (x * 4)],
                    block[y * 16 + (x * 4) + 1]);
      }
    }
  }
...
static void put_block(unsigned char *dst, unsigned char *block, unsigned int bx,
                      unsigned int by, unsigned int width, unsigned height,
                      int bpp) {
  int x, y, i;
  unsigned char *d;
