...
typedef void (*mipmapfunc_t)(unsigned char *, int, int, unsigned char *, int,
                             int, int, filterfunc_t, float, wrapfunc_t, int,
                             float);
typedef void (*volmipmapfunc_t)(unsigned char *, int, int, int, unsigned char *,
                                int, int, int, int, filterfunc_t, float,
                                wrapfunc_t, int, float);
...
unsigned int get_mipmapped_size(int width, int height, int bpp, int level,
                                int num, int format) {
  int w, h, n = 0;
  unsigned int size = 0;
...
unsigned int get_volume_mipmapped_size(int width, int height, int depth,
                                       int bpp, int level, int num,
                                       int format) {
  int w, h, d, n = 0;
  unsigned int size = 0;
...
int get_next_mipmap_dimensions(int *next_w, int *next_h, int curr_w,
                               int curr_h) {
...
static int wrap_clamp(int x, int max) { return MAX(0, MIN(max - 1, x)); }
...
static float mitchell(float t, const float B, const float C) {
  ...
  if (t < 1.0f) {
    t = (((12.0f - 9.0f * B - 6.0f * C) * (t * tt)) +
         ((-18.0f + 12.0f * B + 6.0f * C) * tt) + (6.0f - 2.0f * B));
...
static void scale_image_nearest(unsigned char *dst, int dw, int dh,
                                ...
                                filterfunc_t filter, float support,
                                wrapfunc_t wrap, int gc, float gamma) {
  int n, x, y;
  int ix, iy;
...
static void scale_image(unsigned char *dst, int dw, int dh, unsigned char *src,
                        int sw, int sh, int bpp, filterfunc_t filter,
                        float support, wrapfunc_t wrap, int gc, float gamma) {
...
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) private(x, y, d, row, col, center,  \
                                                   start, stop, nmax, s, i, n, \
                                                   density, r, t, contrib)
#endif
  for (y = 0; y < dh; ++y) {
      ...
      for (i = 0; i < bpp; ++i) {
        ...
        for (n = 0; n < nmax; ++n) {
          ...
          if (i == 3)
            ...
            t = linear_to_gamma(gc, col[(wrap(start + n, sh) * sstride) + i],
                                gamma);
          r += t * contrib;
        }
      ...
      for (i = 0; i < bpp; ++i) {
        ...
        for (n = 0; n < nmax; ++n) {
          ...
          if (i == 3)
            ...
            t = linear_to_gamma(gc, row[(wrap(start + n, sw) * bpp) + i],
                                gamma);
          r += t * contrib;
        }
...
static void scale_volume_image_nearest(unsigned char *dst, int dw, int dh,
                                       int dd, unsigned char *src, int sw,
                                       int sh, int sd, int bpp,
                                       filterfunc_t filter, float support,
                                       wrapfunc_t wrap, int gc, float gamma) {
  int n, x, y, z;
  int ix, iy, iz;
...
static void scale_volume_image(unsigned char *dst, int dw, int dh, int dd,
  ...
  if (dd == 1 && sd == 1) {
    scale_image(dst, dw, dh, src, sw, sh, bpp, filter, support, wrap, gc,
                gamma);
    return;
  }
...
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) private(x, y, slice, i, n, density, \
                                                   r, t, contrib)
#endif
    for (y = 0; y < sh; ++y) {
      for (x = 0; x < sw; ++x) {
        ...
        for (i = 0; i < bpp; ++i) {
          ...
          for (n = 0; n < nmax; ++n) {
            ...
            if (i == 3)
              ...
              t = linear_to_gamma(
                  gc, slice[(wrap(start + n, sd) * zstride) + i], gamma);
            r += t * contrib;
          }
...
#pragma omp parallel for schedule(dynamic) private(                            \
    x, y, col, center, start, stop, nmax, s, i, n, density, r, t, contrib)
#endif
    for (y = 0; y < dh; ++y) {
      center = ((float)y + 0.5f) / yfactor;
      start = (int)(center - ysupport + 0.5f);
...
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) private(                            \
    x, y, row, center, start, stop, nmax, s, i, n, density, r, t, contrib)
#endif
    for (y = 0; y < dh; ++y) {
      row = tmp2 + (y * sstride);
...
static struct {
  ...
  filterfunc_t func;
  float support;
} filters[] = {{DDS_MIPMAP_FILTER_BOX, box_filter, 0.5f},
 ...
 *      discard;
 */
static float calc_alpha_test_coverage(unsigned char *src, unsigned int width,
...
static void scale_alpha_to_coverage(unsigned char *img, unsigned int width,
                                    unsigned int height, int bpp,
  ...
  /* Binary search */
  for (i = 0; i < 10; i++) {
    float cur_coverage = calc_alpha_test_coverage(
        img, width, height, bpp, alpha_test_threshold, alpha_scale);
  ...
  for (y = 0; y < height; ++y) {
    for (x = 0; x < width; ++x) {
      float new_alpha =
          img[y * rowbytes + (x * bpp) + alpha_channel_idx] * alpha_scale;
      if (new_alpha > 255.0f) {
        new_alpha = 255.0f;
      }

      img[y * rowbytes + (x * bpp) + alpha_channel_idx] =
...
int generate_mipmaps(unsigned char *dst, unsigned char *src, unsigned int width,
                     unsigned int height, int bpp, int indexed, int mipmaps,
                     int filter, int wrap, int gc, float gamma,
  ...
  switch (wrap) {
  case DDS_MIPMAP_WRAP_MIRROR:
    wrap_func = wrap_mirror;
    break;
  case DDS_MIPMAP_WRAP_REPEAT:
    wrap_func = wrap_repeat;
    break;
  case DDS_MIPMAP_WRAP_CLAMP:
  ...
  for (i = 1; i < mipmaps; ++i) {
    ...
    mipmap_func(d, dw, dh, s, sw, sh, bpp, filter_func, support, wrap_func, gc,
                gamma);

    if (has_alpha && preserve_alpha_coverage) {
      scale_alpha_to_coverage(d, dw, dh, bpp, alpha_test_coverage,
...
int generate_volume_mipmaps(unsigned char *dst, unsigned char *src,
                            ...
                            int mipmaps, int filter, int wrap, int gc,
                            float gamma) {
  int i;
  ...
  for (i = 1; i < mipmaps; ++i) {
    ...
    mipmap_func(d, dw, dh, dd, s, sw, sh, sd, bpp, filter_func, support,
                wrap_func, gc, gamma);
