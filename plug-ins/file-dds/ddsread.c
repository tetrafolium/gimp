...
static gboolean read_header(dds_header_t *hdr, FILE *fp);
static gboolean read_header_dx10(dds_header_dx10_t *hdr, FILE *fp);
static gboolean validate_header(dds_header_t *hdr, GError **error);
static gboolean setup_dxgi_format(dds_header_t *hdr, dds_header_dx10_t *dx10hdr,
                                  GError **error);
static gboolean load_layer(FILE *fp, dds_header_t *hdr, dds_load_info_t *d,
                           GimpImage *image, guint level, gchar *prefix,
                           guint *l, guchar *pixels, guchar *buf,
                           gboolean decode_images, GError **error);
static gboolean load_mipmaps(FILE *fp, dds_header_t *hdr, dds_load_info_t *d,
                             GimpImage *image, gchar *prefix, guint *l,
                             guchar *pixels, guchar *buf, gboolean read_mipmaps,
                             gboolean decode_images, GError **error);
static gboolean load_face(FILE *fp, dds_header_t *hdr, dds_load_info_t *d,
                          GimpImage *image, char *prefix, guint *l,
                          guchar *pixels, guchar *buf, gboolean read_mipmaps,
                          gboolean decode_images, GError **error);
...
GimpPDBStatusType read_dds(GFile *file, GimpImage **ret_image,
                           gboolean interactive, GimpProcedure *procedure,
  ...
  g_object_get(config, "load-mipmaps", &read_mipmaps, "decode-images",
               &decode_images, NULL);
  ...
  if (!(hdr.caps.caps2 & DDSCAPS2_CUBEMAP) &&
      !(hdr.caps.caps2 & DDSCAPS2_VOLUME) && dx10hdr.arraySize == 0) {
    if (!load_layer(fp, &hdr, &d, image, 0, "", &l, pixels, buf, decode_images,
                    error)) {
      fclose(fp);
      gimp_image_delete(image);
      return GIMP_PDB_EXECUTION_ERROR;
    ...
    if (!load_mipmaps(fp, &hdr, &d, image, "", &l, pixels, buf, read_mipmaps,
                      decode_images, error)) {
      fclose(fp);
      gimp_image_delete(image);
      return GIMP_PDB_EXECUTION_ERROR;
  ...
  } else if ((hdr.caps.caps2 & DDSCAPS2_VOLUME) && (hdr.flags & DDSD_DEPTH)) {
    guint i, level;
    gchar *plane;
    ...
    if ((hdr.flags & DDSD_MIPMAPCOUNT) && (hdr.caps.caps1 & DDSCAPS_MIPMAP) &&
        read_mipmaps) {
...
static gboolean validate_header(dds_header_t *hdr, GError **error) {
  ...
  if (hdr->pixelfmt.flags & DDPF_FOURCC) {
    ...
    if (hdr->flags & DDSD_PITCH) {
      g_printerr("Warning: DDSD_PITCH is incorrectly set for DDPF_FOURCC! "
                 "(recovered)\n");
      hdr->flags &= DDSD_PITCH;
    }
    if (!(hdr->flags & DDSD_LINEARSIZE)) {
      g_printerr("Warning: DDSD_LINEARSIZE is incorrectly not set for "
                 "DDPF_FOURCC! (recovered)\n");
  ...
  if ((hdr->pixelfmt.flags & DDPF_FOURCC) &&
    ...
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                ...
                hdr->pixelfmt.fourcc[0], hdr->pixelfmt.fourcc[1],
                hdr->pixelfmt.fourcc[2], hdr->pixelfmt.fourcc[3],
                GETL32(hdr->pixelfmt.fourcc));
    return FALSE;
  }

  if (hdr->pixelfmt.flags & DDPF_RGB) {
    if ((hdr->pixelfmt.bpp != 8) && (hdr->pixelfmt.bpp != 16) &&
        (hdr->pixelfmt.bpp != 24) && (hdr->pixelfmt.bpp != 32)) {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                  _("Invalid bpp value for RGB data: %d"), hdr->pixelfmt.bpp);
      return FALSE;
    }
...
static gboolean setup_dxgi_format(dds_header_t *hdr, dds_header_dx10_t *dx10hdr,
  ...
  if ((dx10hdr->dxgiFormat >= DXGI_FORMAT_BC1_TYPELESS) &&
    ...
    default: /* unsupported DXGI format */
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                  _("Unsupported DXGI format (%d)"), dx10hdr->dxgiFormat);
      return FALSE;
    }
  }
...
static const Babl *premultiplied_variant(const Babl *format) {
...
static gboolean load_layer(FILE *fp, dds_header_t *hdr, dds_load_info_t *d,
                           GimpImage *image, guint level, char *prefix,
                           guint *l, guchar *pixels, guchar *buf,
                           gboolean decode_images, GError **error) {
  GeglBuffer *buffer;
  ...
  case 2:
    ...
    } else if (hdr->pixelfmt.bmask == 0x1f) /* R5G6B5 or RGB5A1 */
    {
      ...
      bablfmt = (hdr->pixelfmt.amask == 0x8000) ? babl_format("R'G'B'A u8")
  ...
  if ((hdr->flags & DDSD_LINEARSIZE) && !fread(buf, size, 1, fp)) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Unexpected EOF.\n"));
    return FALSE;
  }
  ...
  if ((hdr->pixelfmt.flags & DDPF_RGB) || (hdr->pixelfmt.flags & DDPF_ALPHA)) {
    ...
    for (y = 0, n = 0; y < height; ++y, ++n) {
      if (n >= d->tile_height) {
        gegl_buffer_set(buffer, GEGL_RECTANGLE(0, y - n, layerw, n), 0, bablfmt,
                        pixels, GEGL_AUTO_ROWSTRIDE);
        n = 0;
        gimp_progress_update((double)y / (double)hdr->height);
      }

      if ((hdr->flags & DDSD_PITCH) && !fread(buf, width * d->bpp, 1, fp)) {
        g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                    _("Unexpected EOF.\n"));
        return FALSE;
      }
      ...
      for (x = 0; x < layerw; ++x) {
        ...
        if (d->bpp > 1)
          pixel += ((guint)buf[z + 1] << 8);
        if (d->bpp > 2)
          pixel += ((guint)buf[z + 2] << 16);
        if (d->bpp > 3)
          pixel += ((guint)buf[z + 3] << 24);
        ...
        if (d->bpp >= 3) {
          if (hdr->pixelfmt.amask == 0xc0000000) /* handle RGB10A2 */
          {
            ...
            if (hdr->pixelfmt.flags & DDPF_ALPHAPIXELS)
              pixels[pos + 3] =
                  (pixel >> d->ashift << (8 - d->abits) & d->amask) * 255 /
                  d->amask;
          } else {
            pixels[pos] = (pixel >> d->rshift << (8 - d->rbits) & d->rmask) *
                          255 / d->rmask;
            pixels[pos + 1] =
                (pixel >> d->gshift << (8 - d->gbits) & d->gmask) * 255 /
                d->gmask;
            pixels[pos + 2] =
                (pixel >> d->bshift << (8 - d->bbits) & d->bmask) * 255 /
                d->bmask;
            if (hdr->pixelfmt.flags & DDPF_ALPHAPIXELS) {
              pixels[pos + 3] =
                  (pixel >> d->ashift << (8 - d->abits) & d->amask) * 255 /
    ...
    for (y = 0, n = 0; y < height; ++y, ++n) {
      ...
      memcpy(pixels + n * layerw * d->gimp_bpp, dst + y * layerw * d->gimp_bpp,
             width * d->gimp_bpp);
    }
...
static gboolean load_mipmaps(FILE *fp, dds_header_t *hdr, dds_load_info_t *d,
                             GimpImage *image, char *prefix, unsigned int *l,
                             guchar *pixels, guchar *buf, gboolean read_mipmaps,
                             gboolean decode_images, GError **error) {
  guint level;
...
static gboolean load_face(FILE *fp, dds_header_t *hdr, dds_load_info_t *d,
                          ...
                          guchar *pixels, guchar *buf, gboolean read_mipmaps,
                          gboolean decode_images, GError **error) {
  if (!load_layer(fp, hdr, d, image, 0, prefix, l, pixels, buf, decode_images,
                  error))
    return FALSE;

  return load_mipmaps(fp, hdr, d, image, prefix, l, pixels, buf, read_mipmaps,
                      decode_images, error);
}
...
static gboolean load_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  check =
      gimp_prop_check_button_new(config, "load-mipmaps", _("_Load mipmaps"));
