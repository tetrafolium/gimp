...
static gboolean write_image(FILE *fp, GimpImage *image, GimpDrawable *drawable,
                            GObject *config);
static gboolean save_dialog(GimpImage *image, GimpDrawable *drawable,
...
static struct {
  ...
  guint bmask;
  guint amask;
} format_info[] = {{DDS_FORMAT_RGB8, DXGI_FORMAT_UNKNOWN, 3, 0, 0x00ff0000,
                    0x0000ff00, 0x000000ff, 0x00000000},
                   {DDS_FORMAT_RGBA8, DXGI_FORMAT_B8G8R8A8_UNORM, 4, 1,
                    0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000},
                   {DDS_FORMAT_BGR8, DXGI_FORMAT_UNKNOWN, 3, 0, 0x000000ff,
                    0x0000ff00, 0x00ff0000, 0x00000000},
                   {DDS_FORMAT_ABGR8, DXGI_FORMAT_R8G8B8A8_UNORM, 4, 1,
                    0x000000ff, 0x0000ff00, 0x00ff0000, 0xff000000},
                   {DDS_FORMAT_R5G6B5, DXGI_FORMAT_B5G6R5_UNORM, 2, 0,
                    0x0000f800, 0x000007e0, 0x0000001f, 0x00000000},
                   {DDS_FORMAT_RGBA4, DXGI_FORMAT_B4G4R4A4_UNORM, 2, 1,
                    0x00000f00, 0x000000f0, 0x0000000f, 0x0000f000},
                   {DDS_FORMAT_RGB5A1, DXGI_FORMAT_B5G5R5A1_UNORM, 2, 1,
                    0x00007c00, 0x000003e0, 0x0000001f, 0x00008000},
                   {DDS_FORMAT_RGB10A2, DXGI_FORMAT_R10G10B10A2_UNORM, 4, 1,
                    0x000003ff, 0x000ffc00, 0x3ff00000, 0xc0000000},
                   {DDS_FORMAT_R3G3B2, DXGI_FORMAT_UNKNOWN, 1, 0, 0x000000e0,
                    0x0000001c, 0x00000003, 0x00000000},
                   {DDS_FORMAT_A8, DXGI_FORMAT_A8_UNORM, 1, 0, 0x00000000,
                    0x00000000, 0x00000000, 0x000000ff},
                   {DDS_FORMAT_L8, DXGI_FORMAT_R8_UNORM, 1, 0, 0x000000ff,
                    0x000000ff, 0x000000ff, 0x00000000},
                   {DDS_FORMAT_L8A8, DXGI_FORMAT_UNKNOWN, 2, 1, 0x000000ff,
                    0x000000ff, 0x000000ff, 0x0000ff00},
                   {DDS_FORMAT_AEXP, DXGI_FORMAT_B8G8R8A8_UNORM, 4, 1,
                    0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000},
                   {DDS_FORMAT_YCOCG, DXGI_FORMAT_B8G8R8A8_UNORM, 4, 1,
                    0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000}};

static gboolean check_mipmaps(GimpImage *image, gint savetype) {
  GList *layers;
  GList *list;
  gint num_layers;
  gint i, j;
...
static gboolean check_cubemap(GimpImage *image) {
  GList *layers;
  GList *list;
  gint num_layers;
  ...
  if (num_layers > 6) {
    ...
    for (i = 0, list = layers; i < num_layers; ++i, list = g_list_next(list)) {
  ...
  if (num_layers == 6) {
    ...
    for (i = 0, list = layers; i < 6; ++i, list = g_list_next(layers)) {
...
static gboolean check_volume(GimpImage *image) {
  GList *layers;
  GList *list;
  gint num_layers;
  ...
  if (num_layers > 1) {
    ...
    for (i = 1, list = layers->next; i < num_layers && volume;
...
static gboolean check_array(GimpImage *image) {
  GList *layers;
  gint num_layers;
  ...
  if (num_layers > 1) {
    ...
    for (i = 1, list = g_list_next(layers); i < num_layers && array;
         ++i, list = g_list_next(list)) {
      if ((gimp_drawable_width(list->data) != w) ||
          (gimp_drawable_height(list->data) != h))
        array = FALSE;
    ...
    if (array) {
      ...
      for (i = 1, list = g_list_next(layers); i < num_layers;
           ++i, list = g_list_next(list)) {
        if (gimp_drawable_type(list->data) != type) {
          array = FALSE;
          break;
...
static int get_array_size(GimpImage *image) {
  GList *layers;
  GList *list;
  gint num_layers;
  gint i;
...
GimpPDBStatusType write_dds(GFile *file, GimpImage *image,
  ...
  g_object_get(config, "compression-format", &compression, "mipmaps", &mipmaps,
               "save-type", &savetype, NULL);
  ...
  if (interactive) {
    if (!is_mipmap_chain_valid && mipmaps == DDS_MIPMAP_EXISTING)
    ...
    if (mipmaps == DDS_MIPMAP_EXISTING && !is_mipmap_chain_valid) {
      g_message("DDS: Cannot save with existing mipmaps as the mipmap chain is "
                "incomplete");
      return GIMP_PDB_EXECUTION_ERROR;
    }
  }
...
static void convert_pixels(unsigned char *dst, unsigned char *src, int format,
                           int w, int h, int d, int bpp, unsigned char *palette,
                           int mipmaps) {
  ...
  if (d > 0)
    num_pixels =
        get_volume_mipmapped_size(w, h, d, 1, 0, mipmaps, DDS_COMPRESS_NONE);
  else
...
static void write_layer(FILE *fp, GimpImage *image, GimpDrawable *drawable,
                        GObject *config, int w, int h, int bpp, int fmtbpp,
                        int num_mipmaps) {
  GeglBuffer *buffer;
  const Babl *format;
  ...
  g_object_get(config, "compression-format", &compression, "mipmaps", &mipmaps,
               "format", &pixel_format, "perceptual-metric", &perceptual_metric,
               NULL);
  ...
  if (compression == DDS_COMPRESS_BC3N) {
    if (bpp != 4) {
      ...
      convert_pixels(fmtdst, src, DDS_FORMAT_RGBA8, w, h, 0, bpp, palette, 1);
      g_free(src);
      src = fmtdst;
      bpp = 4;
    }
    ...
    for (y = 0; y < h; ++y) {
      for (x = 0; x < w; ++x) {
        /* set alpha to red (x) */
        src[y * (w * 4) + (x * 4) + 3] = src[y * (w * 4) + (x * 4) + 2];
        /* set red to 1 */
        src[y * (w * 4) + (x * 4) + 2] = 255;
      }
    }
      ...
      compression == DDS_COMPRESS_YCOCGS) /* convert to YCoCG */
  {
    fmtsize = w * h * 4;
    fmtdst = g_malloc(fmtsize);
    convert_pixels(fmtdst, src, DDS_FORMAT_YCOCG, w, h, 0, bpp, palette, 1);
    g_free(src);
    src = fmtdst;
    bpp = 4;
  }
  ...
  if (compression == DDS_COMPRESS_AEXP) {
    fmtsize = w * h * 4;
    fmtdst = g_malloc(fmtsize);
    convert_pixels(fmtdst, src, DDS_FORMAT_AEXP, w, h, 0, bpp, palette, 1);
    g_free(src);
    src = fmtdst;
    bpp = 4;
  }
  ...
  if (compression == DDS_COMPRESS_NONE) {
    if (num_mipmaps > 1) {
      ...
      if (pixel_format > DDS_FORMAT_DEFAULT && basetype == GIMP_INDEXED) {
        fmtsize =
            get_mipmapped_size(w, h, 3, 0, num_mipmaps, DDS_COMPRESS_NONE);
        fmtdst = g_malloc(fmtsize);
        convert_pixels(fmtdst, src, DDS_FORMAT_RGB8, w, h, 0, bpp, palette, 1);
        g_free(src);
        src = fmtdst;
        bpp = 3;
        palette = NULL;
      ...
      size = get_mipmapped_size(w, h, bpp, 0, num_mipmaps, DDS_COMPRESS_NONE);
      dst = g_malloc(size);
      if (mipmaps == DDS_MIPMAP_GENERATE) {
        ...
        g_object_get(config, "mipmap-filter", &mipmap_filter, "mipmap-wrap",
                     &mipmap_wrap, "gamma-correct", &gamma_correct, "srgb",
                     &srgb, "gamma", &gamma, "preserve-alpha-coverage",
                     &preserve_alpha_coverage, "alpha-test-threshold",
                     &alpha_test_threshold, NULL);

        generate_mipmaps(dst, src, w, h, bpp, palette != NULL, num_mipmaps,
                         mipmap_filter, mipmap_wrap, gamma_correct + srgb,
                         gamma, preserve_alpha_coverage, alpha_test_threshold);
      } else {
      ...
      if (pixel_format > DDS_FORMAT_DEFAULT) {
        fmtsize =
            get_mipmapped_size(w, h, fmtbpp, 0, num_mipmaps, DDS_COMPRESS_NONE);
        fmtdst = g_malloc(fmtsize);

        convert_pixels(fmtdst, dst, pixel_format, w, h, 0, bpp, palette,
                       num_mipmaps);
      ...
      if (pixel_format > DDS_FORMAT_DEFAULT) {
        ...
        convert_pixels(fmtdst, src, pixel_format, w, h, 0, bpp, palette, 1);
        g_free(src);
        src = fmtdst;
        bpp = fmtbpp;
      }
    ...
    if (basetype == GIMP_INDEXED) {
      ...
      fmtdst = g_malloc(fmtsize);
      convert_pixels(fmtdst, src, DDS_FORMAT_RGB8, w, h, 0, bpp, palette,
                     num_mipmaps);
      g_free(src);
      src = fmtdst;
      bpp = 3;
    ...
    if (num_mipmaps > 1) {
      fmtsize =
          get_mipmapped_size(w, h, bpp, 0, num_mipmaps, DDS_COMPRESS_NONE);
      fmtdst = g_malloc(fmtsize);
      if (mipmaps == DDS_MIPMAP_GENERATE) {
        gint mipmap_filter;
        gint mipmap_wrap;
        ...
        generate_mipmaps(fmtdst, src, w, h, bpp, 0, num_mipmaps, mipmap_filter,
                         mipmap_wrap, gamma_correct + srgb, gamma,
                         preserve_alpha_coverage, alpha_test_threshold);
      } else {
...
static void write_volume_mipmaps(FILE *fp, GimpImage *image, GObject *config,
                                 GList *layers, int w, int h, int d, int bpp,
                                 int fmtbpp, int num_mipmaps) {
  GList *list;
  gint i;
  gint size;
  gint offset;
  ...
  g_object_get(config, "compression-format", &compression, "format",
               &pixel_format, "mipmap-filter", &mipmap_filter, "mipmap-wrap",
               &mipmap_wrap, "gamma-correct", &gamma_correct, "srgb", &srgb,
               "gamma", &gamma, NULL);
  ...
  offset = 0;
  for (i = 0, list = layers; i < d; ++i, list = g_list_next(list)) {
  ...
  size = get_volume_mipmapped_size(w, h, d, bpp, 0, num_mipmaps, compression);

  dst = g_malloc(size);

  offset = get_volume_mipmapped_size(w, h, d, bpp, 0, 1, compression);

  generate_volume_mipmaps(dst, src, w, h, d, bpp, palette != NULL, num_mipmaps,
                          mipmap_filter, mipmap_wrap, gamma_correct + srgb,
                          gamma);

  if (pixel_format > DDS_FORMAT_DEFAULT) {
    size =
        get_volume_mipmapped_size(w, h, d, fmtbpp, 0, num_mipmaps, compression);
    offset = get_volume_mipmapped_size(w, h, d, fmtbpp, 0, 1, compression);
    fmtdst = g_malloc(size);

    convert_pixels(fmtdst, dst, pixel_format, w, h, d, bpp, palette,
                   num_mipmaps);
    g_free(dst);
    dst = fmtdst;
  }
...
static gboolean write_image(FILE *fp, GimpImage *image, GimpDrawable *drawable,
  ...
  g_object_get(config, "compression-format", &compression, "mipmaps", &mipmaps,
               "save-type", &savetype, "format", &pixel_format,
               "transparent-index", &transindex, NULL);
  ...
  if (is_dx10) {
    array_size =
        ((savetype == DDS_SAVE_SELECTED_LAYER) ? 1 : get_array_size(image));
  ...
  if ((basetype == GIMP_INDEXED) && (pixel_format == DDS_FORMAT_DEFAULT) &&
  ...
  if (savetype == DDS_SAVE_CUBEMAP) {
    for (i = 0; i < 6; ++i) {
      write_layer(fp, image, GIMP_DRAWABLE(cubemap_faces[i]), config, w, h, bpp,
                  fmtbpp, num_mipmaps);
      gimp_progress_update((float)(i + 1) / 6.0);
    }
  } else if (savetype == DDS_SAVE_VOLUMEMAP) {
    for (i = 0, list = layers; i < num_layers;
         ++i, list = g_list_next(layers)) {
      write_layer(fp, image, list->data, config, w, h, bpp, fmtbpp, 1);
      gimp_progress_update((float)i / (float)num_layers);
    }

    if (num_mipmaps > 1)
      write_volume_mipmaps(fp, image, config, layers, w, h, num_layers, bpp,
                           fmtbpp, num_mipmaps);
  } else if (savetype == DDS_SAVE_ARRAY) {
    for (i = 0, list = layers; i < num_layers;
         ++i, list = g_list_next(layers)) {
      if ((gimp_drawable_width(list->data) == w) &&
          (gimp_drawable_height(list->data) == h)) {
        write_layer(fp, image, list->data, config, w, h, bpp, fmtbpp,
                    num_mipmaps);
      }
    ...
    write_layer(fp, image, drawable, config, w, h, bpp, fmtbpp, num_mipmaps);
  }
...
static gboolean combo_sensitivity_func(gint value, gpointer data) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  ...
  if (gimp_int_store_lookup_by_value(model, value, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, GIMP_INT_STORE_USER_DATA, &insensitive,
                       -1);
...
static void combo_set_item_sensitive(GtkWidget *widget, gint value,
                                     gboolean sensitive) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  ...
  if (gimp_int_store_lookup_by_value(model, value, &iter)) {
    gtk_list_store_set(GTK_LIST_STORE(model), &iter, GIMP_INT_STORE_USER_DATA,
                       !GINT_TO_POINTER(sensitive), -1);
  }
}
...
static void config_notify(GObject *config, const GParamSpec *pspec,
  ...
  if (!strcmp(pspec->name, "compression-format")) {
    ...
    g_object_get(config, "compression-format", &compression, NULL);

    if (format_opt)
      gtk_widget_set_sensitive(format_opt, compression == DDS_COMPRESS_NONE);

    if (pm_check)
      gtk_widget_set_sensitive(pm_check, compression != DDS_COMPRESS_NONE);
  } else if (!strcmp(pspec->name, "save-type")) {
    gint savetype;

    g_object_get(config, "save-type", &savetype, NULL);
    ...
    case DDS_SAVE_VOLUMEMAP:
      g_object_set(config, "compression-format", DDS_COMPRESS_NONE, NULL);
    ...
    g_object_get(config, "mipmaps", &mipmaps, "gamma-correct", &gamma_correct,
                 "srgb", &srgb, "preserve-alpha-coverage",
                 &preserve_alpha_coverage, NULL);
    ...
    if (mipmap_wrap_opt)
      gtk_widget_set_sensitive(mipmap_wrap_opt, mipmaps == DDS_MIPMAP_GENERATE);

    if (gamma_check)
      gtk_widget_set_sensitive(gamma_check, mipmaps == DDS_MIPMAP_GENERATE);

    if (srgb_check)
      gtk_widget_set_sensitive(srgb_check, (mipmaps == DDS_MIPMAP_GENERATE) &&
                                               gamma_correct);

    if (gamma_spin)
      gtk_widget_set_sensitive(gamma_spin, (mipmaps == DDS_MIPMAP_GENERATE) &&
                                               gamma_correct && !srgb);
    ...
    g_object_get(config, "transparent-color", &transparent_color, NULL);

    if (transparent_spin)
      gtk_widget_set_sensitive(transparent_spin,
                               transparent_color && base_type == GIMP_INDEXED);
  } else if (!strcmp(pspec->name, "gamma-correct")) {
    gboolean gamma_correct;
    gboolean srgb;

    g_object_get(config, "gamma-correct", &gamma_correct, "srgb", &srgb, NULL);
    ...
    g_object_get(config, "preserve-alpha-coverage", &preserve_alpha_coverage,
                 NULL);
...
static gboolean save_dialog(GimpImage *image, GimpDrawable *drawable,
  ...
  if (is_cubemap || is_volume || is_array)
    g_object_set(config, "save-type", DDS_SAVE_SELECTED_LAYER, NULL);
  ...
  store = gimp_int_store_new(
      "None", DDS_COMPRESS_NONE, "BC1 / DXT1", DDS_COMPRESS_BC1, "BC2 / DXT3",
      DDS_COMPRESS_BC2, "BC3 / DXT5", DDS_COMPRESS_BC3, "BC3nm / DXT5nm",
      DDS_COMPRESS_BC3N, "BC4 / ATI1 (3Dc+)", DDS_COMPRESS_BC4,
      "BC5 / ATI2 (3Dc)", DDS_COMPRESS_BC5, "RXGB (DXT5)", DDS_COMPRESS_RXGB,
      "Alpha Exponent (DXT5)", DDS_COMPRESS_AEXP, "YCoCg (DXT5)",
      DDS_COMPRESS_YCOCG, "YCoCg scaled (DXT5)", DDS_COMPRESS_YCOCGS, NULL);
  compress_opt = gimp_prop_int_combo_box_new(config, "compression-format",
                                             GIMP_INT_STORE(store));
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Compression:"), 0.0, 0.5,
                           compress_opt, 1);
  ...
  store = gimp_int_store_new(
      "Default", DDS_FORMAT_DEFAULT, "RGB8", DDS_FORMAT_RGB8, "RGBA8",
      DDS_FORMAT_RGBA8, "BGR8", DDS_FORMAT_BGR8, "ABGR8", DDS_FORMAT_ABGR8,
      "R5G6B5", DDS_FORMAT_R5G6B5, "RGBA4", DDS_FORMAT_RGBA4, "RGB5A1",
      DDS_FORMAT_RGB5A1, "RGB10A2", DDS_FORMAT_RGB10A2, "R3G3B2",
      DDS_FORMAT_R3G3B2, "A8", DDS_FORMAT_A8, "L8", DDS_FORMAT_L8, "L8A8",
      DDS_FORMAT_L8A8, "AExp", DDS_FORMAT_AEXP, "YCoCg", DDS_FORMAT_YCOCG,
      NULL);
  format_opt =
      gimp_prop_int_combo_box_new(config, "format", GIMP_INT_STORE(store));
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("_Format:"), 0.0, 0.5,
                           format_opt, 1);

  store = gimp_int_store_new("Image / Selected layer", DDS_SAVE_SELECTED_LAYER,
                             "As cube map", DDS_SAVE_CUBEMAP, "As volume map",
                             DDS_SAVE_VOLUMEMAP, "As texture array",
                             DDS_SAVE_ARRAY, NULL);
  opt = gimp_prop_int_combo_box_new(config, "save-type", GIMP_INT_STORE(store));
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("_Save:"), 0.0, 0.5, opt, 1);

  gimp_int_combo_box_set_sensitivity(GIMP_INT_COMBO_BOX(opt),
                                     combo_sensitivity_func, opt, NULL);
  ...
  store = gimp_int_store_new("No mipmaps", DDS_MIPMAP_NONE, "Generate mipmaps",
                             DDS_MIPMAP_GENERATE, "Use existing mipmaps",
                             DDS_MIPMAP_EXISTING, NULL);
  mipmap_opt =
      gimp_prop_int_combo_box_new(config, "mipmaps", GIMP_INT_STORE(store));
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 4, _("_Mipmaps:"), 0.0, 0.5,
                           mipmap_opt, 1);

  gimp_int_combo_box_set_sensitivity(GIMP_INT_COMBO_BOX(mipmap_opt),
                                     combo_sensitivity_func, mipmap_opt, NULL);
  ...
  transparent_spin =
      gimp_prop_spin_button_new(config, "transparent-index", 1, 8, 0);
  ...
  store = gimp_int_store_new(
      "Default", DDS_MIPMAP_FILTER_DEFAULT, "Nearest",
      DDS_MIPMAP_FILTER_NEAREST, "Box", DDS_MIPMAP_FILTER_BOX, "Triangle",
      DDS_MIPMAP_FILTER_TRIANGLE, "Quadratic", DDS_MIPMAP_FILTER_QUADRATIC,
      "B-Spline", DDS_MIPMAP_FILTER_BSPLINE, "Mitchell",
      DDS_MIPMAP_FILTER_MITCHELL, "Lanczos", DDS_MIPMAP_FILTER_LANCZOS,
      "Kaiser", DDS_MIPMAP_FILTER_KAISER, NULL);
  mipmap_filter_opt = gimp_prop_int_combo_box_new(config, "mipmap-filter",
                                                  GIMP_INT_STORE(store));
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("F_ilter:"), 0.0, 0.5,
                           mipmap_filter_opt, 1);

  store = gimp_int_store_new(
      "Default", DDS_MIPMAP_WRAP_DEFAULT, "Mirror", DDS_MIPMAP_WRAP_MIRROR,
      "Repeat", DDS_MIPMAP_WRAP_REPEAT, "Clamp", DDS_MIPMAP_WRAP_CLAMP, NULL);
  mipmap_wrap_opt =
      gimp_prop_int_combo_box_new(config, "mipmap-wrap", GIMP_INT_STORE(store));
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Wrap mode:"), 0.0, 0.5,
                           mipmap_wrap_opt, 1);
  ...
  srgb_check =
      gimp_prop_check_button_new(config, "srgb", _("Use s_RGB colorspace"));
  gtk_grid_attach(GTK_GRID(grid), srgb_check, 1, 3, 1, 1);

  gamma_spin = gimp_prop_spin_button_new(config, "gamma", 0.1, 0.5, 1);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 4, _("_Gamma:"), 0.0, 0.5,
                           gamma_spin, 1);

  alpha_coverage_check = gimp_prop_check_button_new(
      config, "preserve-alpha-coverage", _("Preserve alpha _test coverage"));
  gtk_grid_attach(GTK_GRID(grid), alpha_coverage_check, 1, 5, 1, 1);

  alpha_test_threshold_spin =
      gimp_prop_spin_button_new(config, "alpha-test-threshold", 0.01, 0.1, 2);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 6, _("_Alpha test threshold:"),
                           0.0, 0.5, alpha_test_threshold_spin, 1);
  ...
  config_notify(
      config,
      g_object_class_find_property(G_OBJECT_GET_CLASS(config), "mipmaps"),
      image);

  config_notify(
      config,
      g_object_class_find_property(G_OBJECT_GET_CLASS(config), "save-type"),
      image);
  ...
  g_signal_connect(config, "notify", G_CALLBACK(config_notify), image);

  gtk_widget_show(dialog);

  run = gimp_procedure_dialog_run(GIMP_PROCEDURE_DIALOG(dialog));

  g_signal_handlers_disconnect_by_func(config, config_notify, image);
