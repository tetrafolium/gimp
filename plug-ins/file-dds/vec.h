...
#define VEC4_CONST4(x, y, z, w)                                                \
  { x, y, z, w }
#define VEC4_CONST3(x, y, z)                                                   \
  { x, y, z, 0.0f }
#define VEC4_CONST1(x)                                                         \
  { x, x, x, x }
...
#else
  return vec4_set(MIN(a[0], b[0]), MIN(a[1], b[1]), MIN(a[2], b[2]),
                  MIN(a[3], b[3]));
#endif
}
...
#else
  return vec4_set(MAX(a[0], b[0]), MAX(a[1], b[1]), MAX(a[2], b[2]),
                  MAX(a[3], b[3]));
#endif
}
