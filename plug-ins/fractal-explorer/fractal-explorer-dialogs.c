...
static explorer_vals_t standardvals = {0,    -2.0, 2.0, -1.5, 1.5, 50.0, -0.75,
...
static void save_file_chooser_response(GtkFileChooser *chooser,
                                       gint response_id, gpointer data);
static void create_load_file_chooser(GtkWidget *widget, GtkWidget *dialog);
static void create_save_file_chooser(GtkWidget *widget, GtkWidget *dialog);
...
static void dialog_redraw_callback(GtkWidget *widget, gpointer data) {
...
static void dialog_undo_zoom_callback(GtkWidget *widget, gpointer data) {
  if (zoomindex > 0) {
...
static void dialog_redo_zoom_callback(GtkWidget *widget, gpointer data) {
...
static void dialog_step_in_callback(GtkWidget *widget, gpointer data) {
  double xdifferenz;
  double ydifferenz;
...
static void dialog_step_out_callback(GtkWidget *widget, gpointer data) {
  gdouble xdifferenz;
  gdouble ydifferenz;
...
static void explorer_toggle_update(GtkWidget *widget, gpointer data) {
...
static void explorer_radio_update(GtkWidget *widget, gpointer data) {
...
static void explorer_double_adjustment_update(GimpLabelSpin *entry,
                                              gdouble *value) {
...
static void explorer_number_of_colors_callback(GtkAdjustment *adjustment,
                                               gpointer data) {
  ...
  gimp_gradient_get_uniform_samples(gradient_name, wvals.ncolors,
                                    wvals.gradinvert, &n_gradient_samples,
                                    &gradient_samples);
...
static void explorer_gradient_select_callback(
    GimpGradientSelectButton *gradient_button, const gchar *name, gint width,
    const gdouble *gradient_data, gboolean dialog_closing, gpointer data) {
...
static void preview_draw_crosshair(gint px, gint py) {
  gint x, y;
  guchar *p_ul;
...
static void preview_redraw(void) {
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(wint.preview), 0, 0, preview_width,
                         preview_height, GIMP_RGB_IMAGE, wint.wimage,
                         preview_width * 3);

  gtk_widget_queue_draw(wint.preview);
}

static gboolean preview_button_press_event(GtkWidget *widget,
...
static gboolean preview_motion_notify_event(GtkWidget *widget,
                                            GdkEventButton *event) {
  if (oldypos != -1) {
  ...
  if ((oldxpos >= 0.0) && (oldypos >= 0.0) && (oldxpos < preview_width) &&
...
static gboolean preview_leave_notify_event(GtkWidget *widget,
                                           GdkEventButton *event) {
  if (oldypos != -1) {
    preview_draw_crosshair(oldxpos, oldypos);
  }
...
static gboolean preview_enter_notify_event(GtkWidget *widget,
                                           GdkEventButton *event) {
...
static gboolean preview_button_release_event(GtkWidget *widget,
                                             GdkEventButton *event) {
  gdouble l_xmin;
  gdouble l_xmax;
  gdouble l_ymin;
  ...
  if (event->button == 1) {
    ...
    if ((x_press >= 0.0) && (y_press >= 0.0) && (x_release >= 0.0) &&
        (y_release >= 0.0) && (x_press < preview_width) &&
        (y_press < preview_height) && (x_release < preview_width) &&
        (y_release < preview_height)) {
      l_xmin =
          (wvals.xmin + (wvals.xmax - wvals.xmin) * (x_press / preview_width));
      l_xmax = (wvals.xmin +
                (wvals.xmax - wvals.xmin) * (x_release / preview_width));
      l_ymin =
          (wvals.ymin + (wvals.ymax - wvals.ymin) * (y_press / preview_height));
      l_ymax = (wvals.ymin +
...
gint explorer_dialog(void) {
  ...
  if (path) {
    ...
    g_message(_("No %s in gimprc:\n"
                ...
                "(%s \"%s\")\n"
                "to your %s file."),
              "fractalexplorer-path", "fractalexplorer-path", esc_path,
  ...
  dialog = maindlg = gimp_dialog_new(_("Fractal Explorer"), PLUG_IN_ROLE, NULL,
  ...
  g_signal_connect(wint.preview, "leave-notify-event",
                   G_CALLBACK(preview_leave_notify_event), NULL);
  g_signal_connect(wint.preview, "enter-notify-event",
                   G_CALLBACK(preview_enter_notify_event), NULL);

  gtk_widget_set_events(wint.preview,
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(dialog_step_in_callback),
                   dialog);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(dialog_step_out_callback),
                   dialog);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(dialog_undo_zoom_callback),
                   dialog);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(dialog_redo_zoom_callback),
                   dialog);
  ...
  g_signal_connect(elements->iter, "value-changed",
                   G_CALLBACK(explorer_double_adjustment_update), &wvals.iter);
  gtk_box_pack_start(GTK_BOX(vbox2), elements->iter, TRUE, TRUE, 2);
  gtk_widget_show(elements->iter);

  elements->cx = gimp_scale_entry_new(_("CX:"), wvals.cx, -2.5, 2.5, 5);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(elements->cx), 0.001, 0.01);
  gimp_help_set_help_data(elements->cx, _("Changes aspect of fractal"), NULL);
  g_signal_connect(elements->cx, "value-changed",
                   G_CALLBACK(explorer_double_adjustment_update), &wvals.cx);
  gtk_box_pack_start(GTK_BOX(vbox2), elements->cx, TRUE, TRUE, 2);
  gtk_widget_show(elements->cx);

  elements->cy = gimp_scale_entry_new(_("CY:"), wvals.cy, -2.5, 2.5, 5);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(elements->cy), 0.001, 0.01);
  gimp_help_set_help_data(elements->cy, _("Changes aspect of fractal"), NULL);
  g_signal_connect(elements->cy, "value-changed",
                   G_CALLBACK(explorer_double_adjustment_update), &wvals.cy);
  ...
  gtk_box_pack_start(GTK_BOX(bbox), button, TRUE, TRUE, 0);
  g_signal_connect(button, "clicked", G_CALLBACK(create_load_file_chooser),
                   dialog);
  gtk_widget_show(button);
  ...
  gtk_box_pack_start(GTK_BOX(bbox), button, TRUE, TRUE, 0);
  g_signal_connect(button, "clicked", G_CALLBACK(dialog_reset_callback),
                   dialog);
  gtk_widget_show(button);
  ...
  gtk_box_pack_start(GTK_BOX(bbox), button, TRUE, TRUE, 0);
  g_signal_connect(button, "clicked", G_CALLBACK(create_save_file_chooser),
                   dialog);
  gtk_widget_show(button);
  ...
  toggle_vbox = gimp_int_radio_group_new(
      FALSE, NULL, G_CALLBACK(explorer_radio_update), &wvals.fractaltype, NULL,
      wvals.fractaltype,

      _("Mandelbrot"), TYPE_MANDELBROT, &(elements->type[TYPE_MANDELBROT]),
      _("Julia"), TYPE_JULIA, &(elements->type[TYPE_JULIA]), _("Barnsley 1"),
      TYPE_BARNSLEY_1, &(elements->type[TYPE_BARNSLEY_1]), _("Barnsley 2"),
      TYPE_BARNSLEY_2, &(elements->type[TYPE_BARNSLEY_2]), _("Barnsley 3"),
      TYPE_BARNSLEY_3, &(elements->type[TYPE_BARNSLEY_3]), _("Spider"),
      TYPE_SPIDER, &(elements->type[TYPE_SPIDER]), _("Man'o'war"),
      TYPE_MAN_O_WAR, &(elements->type[TYPE_MAN_O_WAR]), _("Lambda"),
      TYPE_LAMBDA, &(elements->type[TYPE_LAMBDA]), _("Sierpinski"),
      TYPE_SIERPINSKI, &(elements->type[TYPE_SIERPINSKI]),
  ...
  for (i = TYPE_BARNSLEY_2; i <= TYPE_SPIDER; i++) {
    ...
    gtk_box_pack_start(GTK_BOX(toggle_vbox2), elements->type[i], FALSE, FALSE,
                       0);
  ...
  for (i = TYPE_MAN_O_WAR; i <= TYPE_SIERPINSKI; i++) {
    ...
    gtk_widget_hide(elements->type[i]);
    gtk_container_remove(GTK_CONTAINER(toggle_vbox), elements->type[i]);
    gtk_box_pack_start(GTK_BOX(toggle_vbox3), elements->type[i], FALSE, FALSE,
                       0);
    gtk_widget_show(elements->type[i]);
  ...
  elements->ncol = gimp_scale_entry_new(_("Number of colors:"), wvals.ncolors,
                                        2, MAXNCOLORS, 0);
  gimp_help_set_help_data(
      elements->ncol, _("Change the number of colors in the mapping"), NULL);
  ...
  elements->red = gimp_scale_entry_new(_("Red:"), wvals.redstretch, 0, 1, 2);
  ...
  elements->blue = gimp_scale_entry_new(_("Blue:"), wvals.bluestretch, 0, 1, 2);
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Red"), G_CALLBACK(explorer_radio_update), &wvals.redmode, NULL,
      wvals.redmode,

      _("Sine"), SINUS, &elements->redmode[SINUS], _("Cosine"), COSINUS,
      &elements->redmode[COSINUS], C_("color-function", "None"), NONE,
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Green"), G_CALLBACK(explorer_radio_update), &wvals.greenmode,
      NULL, wvals.greenmode,

      _("Sine"), SINUS, &elements->greenmode[SINUS], _("Cosine"), COSINUS,
      &elements->greenmode[COSINUS], C_("color-function", "None"), NONE,
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Blue"), G_CALLBACK(explorer_radio_update), &wvals.bluemode, NULL,
      wvals.bluemode,

      _("Sine"), SINUS, &elements->bluemode[SINUS], _("Cosine"), COSINUS,
      &elements->bluemode[COSINUS], C_("color-function", "None"), NONE,
  ...
  gtk_box_pack_start(GTK_BOX(toggle_vbox), toggle, FALSE, FALSE, 0);
  g_object_set_data(G_OBJECT(toggle), "gimp-item-data", GINT_TO_POINTER(0));
  g_signal_connect(toggle, "toggled", G_CALLBACK(explorer_radio_update),
                   &wvals.colormode);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle), wvals.colormode == 0);
  gtk_widget_show(toggle);
  gimp_help_set_help_data(toggle,
                          _("Create a color-map with the options you "
                            "specified above (color density/function). The "
                            "result is visible in the preview image"),
                          NULL);
  ...
  toggle = elements->colormode[1] = gtk_radio_button_new_with_label(
      group, _("Apply active gradient to final image"));
  group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(toggle));
  gtk_box_pack_start(GTK_BOX(hbox), toggle, TRUE, TRUE, 0);
  g_object_set_data(G_OBJECT(toggle), "gimp-item-data", GINT_TO_POINTER(1));
  g_signal_connect(toggle, "toggled", G_CALLBACK(explorer_radio_update),
                   &wvals.colormode);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle), wvals.colormode == 1);
  gtk_widget_show(toggle);
  gimp_help_set_help_data(toggle,
...
void dialog_update_preview(void) {
  ...
  if (ready_now && wvals.alwayspreview) {
    ...
    for (ycoord = 0; ycoord < preview_height; ycoord++) {
      explorer_render_row(NULL, p_ul, ycoord, preview_width, 3);
...
static void cmap_preview_size_allocate(GtkWidget *widget,
  ...
  gimp_preview_area_draw(preview, 0, 0, allocation->width, allocation->height,
...
void dialog_change_scale(void) {
  ...
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(elements->green),
                            wvals.greenstretch);
...
static void save_options(FILE *fp) {
  gchar buf[64];
...
static void save_callback(void) {
  ...
  if (ferror(fp))
    g_message(_("Could not write '%s': %s"), gimp_filename_to_utf8(savename),
              g_strerror(ferror(fp)));
...
static void file_chooser_set_default_folder(GtkFileChooser *chooser) {
  GList *path_list;
  gchar *dir;
...
static void create_load_file_chooser(GtkWidget *widget, GtkWidget *dialog) {
  ...
  if (!window) {
    window = gtk_file_chooser_dialog_new(
        _("Load Fractal Parameters"), GTK_WINDOW(dialog),
    ...
    g_signal_connect(window, "destroy", G_CALLBACK(gtk_widget_destroyed),
                     &window);
    g_signal_connect(window, "response", G_CALLBACK(load_file_chooser_response),
                     window);
  }
...
static void create_save_file_chooser(GtkWidget *widget, GtkWidget *dialog) {
  ...
  if (!window) {
    window = gtk_file_chooser_dialog_new(
        _("Save Fractal Parameters"), GTK_WINDOW(dialog),
    ...
    g_signal_connect(window, "destroy", G_CALLBACK(gtk_widget_destroyed),
                     &window);
    g_signal_connect(window, "response", G_CALLBACK(save_file_chooser_response),
                     window);
  }
...
gchar *get_line(gchar *buf, gint s, FILE *from, gint init) {
  gint slen;
  gchar *ret;
