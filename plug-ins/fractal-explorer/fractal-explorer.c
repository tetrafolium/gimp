...
static void delete_dialog_callback(GtkWidget *widget, gboolean value,
                                   gpointer data);
static gboolean delete_fractal_callback(GtkWidget *widget, gpointer data);
...
static void activate_fractal_callback(GtkTreeView *view, GtkTreePath *path,
                                      GtkTreeViewColumn *col, gpointer data);
static gboolean apply_fractal_callback(GtkWidget *widget, gpointer data);
...
static void fractalexplorer_rescan_list(GtkWidget *widget, gpointer data);
...
explorer_vals_t wvals = {0, -2.0, 2.0, -1.5, 1.5, 50.0, -0.75, -0.2,
...
static GimpProcedure *explorer_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    ...
    gimp_procedure_set_documentation(procedure, N_("Render fractal art"),
                                     "No help yet.", name);
    ...
    GIMP_PROC_ARG_INT(procedure, "fractal-type", "Fractal type",
    ...
    GIMP_PROC_ARG_DOUBLE(procedure, "xmin", "X min",
                         "xmin fractal image delimiter", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "xmax", "X max",
                         "xmax fractal image delimiter", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "ymin", "Y min",
                         "ymin fractal image delimiter", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "ymax", "Y max",
                         "ymax fractal image delimiter", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "iter", "Iter", "Iteration value",
                         -G_MAXDOUBLE, G_MAXDOUBLE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "cx", "CX", "cx value (only Julia)",
                         -G_MAXDOUBLE, G_MAXDOUBLE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "cy", "CY", "cy value (only Julia)",
                         -G_MAXDOUBLE, G_MAXDOUBLE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "color-mode", "Color mode",
                      "0: Apply colormap as specified by the parameters "
                      "below; 1: Apply active gradient to final image",
                      0, 1, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "red-stretch", "Red stretch",
                         "Red stretching factor", -G_MAXDOUBLE, G_MAXDOUBLE, 0,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "green-stretch", "Green stretch",
                         "Green stretching factor", -G_MAXDOUBLE, G_MAXDOUBLE,
                         0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "blues-tretch", "Blue stretch",
                         "Blue stretching factor", -G_MAXDOUBLE, G_MAXDOUBLE, 0,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "red-mode", "Red mode",
                      "Red application mode (0:SIN; 1:COS; 2:NONE)", 0, 2, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "green-mode", "Green mode",
                      "Green application mode (0:SIN; 1:COS; 2:NONE)", 0, 2, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "blue-mode", "Blue mode",
                      "Blue application mode (0:SIN; 1:COS; 2:NONE)", 0, 2, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "red-invert", "Red invert",
                          "Red inversion mode", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "green-invert", "Green invert",
                          "Green inversion mode", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "blue-invert", "Blue invert",
                          "Blue inversion mode", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "n-colors", "N volors",
                      "Number of Colors for mapping", 2, 8192, 512,
                      G_PARAM_READWRITE);
  }
...
static GimpValueArray *explorer_run(GimpProcedure *procedure,
  ...
  if (!gimp_drawable_mask_intersect(drawable, &sel_x, &sel_y, &sel_width,
                                    &sel_height)) {
  ...
  if (status == GIMP_PDB_SUCCESS) {
    ...
    if (run_mode == GIMP_RUN_INTERACTIVE)
      gimp_set_data("plug-in-fractalexplorer", &wvals, sizeof(explorer_vals_t));
  }
...
static void explorer(GimpDrawable *drawable) {
  ...
  for (row = y; row < y + h; row++) {
    gegl_buffer_get(src_buffer, GEGL_RECTANGLE(x, row, w, 1), 1.0, format,
                    src_row, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);

    explorer_render_row(src_row, dest_row, row, w, bpp);

    gegl_buffer_set(dest_buffer, GEGL_RECTANGLE(x, row, w, 1), 0, format,
                    dest_row, GEGL_AUTO_ROWSTRIDE);
...
static void delete_dialog_callback(GtkWidget *widget, gboolean delete,
                                   gpointer data) {
...
static gboolean delete_fractal_callback(GtkWidget *widget, gpointer data) {
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    delete_dialog = gimp_query_boolean_box(
        _("Delete Fractal"), gtk_widget_get_toplevel(view),
        gimp_standard_help_func, NULL, GIMP_ICON_DIALOG_QUESTION, str,
        _("_Delete"), _("_Cancel"), G_OBJECT(widget), "destroy",
        delete_dialog_callback, data, NULL);
    g_free(str);
...
static gint fractalexplorer_list_pos(fractalexplorerOBJ *fractalexplorer) {
...
static gint fractalexplorer_list_insert(fractalexplorerOBJ *fractalexplorer) {
  ...
  fractalexplorer_list =
      g_list_insert(fractalexplorer_list, fractalexplorer, n);
...
static void fractalexplorer_free(fractalexplorerOBJ *fractalexplorer) {
...
static void fractalexplorer_list_load_all(const gchar *explorer_path) {
  GList *path;
  GList *list;
    ...
    if (enumerator) {
      ...
      while ((info = g_file_enumerator_next_file(enumerator, NULL, NULL))) {
        ...
        if (file_type == G_FILE_TYPE_REGULAR &&
          ...
          fractalexplorer = fractalexplorer_load(filename, basename);
...
GtkWidget *add_objects_list(void) {
  ...
  g_signal_connect(view, "row-activated", G_CALLBACK(activate_fractal_callback),
                   NULL);
  ...
  gimp_help_set_help_data(button, _("Select folder and rescan collection"),
                          NULL);

  g_signal_connect(button, "clicked", G_CALLBACK(fractalexplorer_rescan_list),
                   view);
  ...
  gimp_help_set_help_data(button, _("Apply currently selected fractal"), NULL);

  g_signal_connect(button, "clicked", G_CALLBACK(apply_fractal_callback), view);

  button = gtk_button_new_with_mnemonic(_("_Delete"));
  gtk_grid_attach(GTK_GRID(grid), button, 2, 1, 1, 1);
  gtk_widget_show(button);

  gimp_help_set_help_data(button, _("Delete currently selected fractal"), NULL);

  g_signal_connect(button, "clicked", G_CALLBACK(delete_fractal_callback),
                   view);
...
static void fractalexplorer_rescan_list(GtkWidget *widget, gpointer data) {
  ...
  dlg = gimp_dialog_new(
      _("Rescan for Fractals"), PLUG_IN_ROLE, gtk_widget_get_toplevel(view),
  ...
  patheditor =
      gimp_path_editor_new(_("Add FractalExplorer Path"), fractalexplorer_path);
