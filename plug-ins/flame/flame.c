...
static GimpValueArray *flame_run(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void flame_scale_entry_update_int(GimpLabelSpin *entry, gint *value);
...
static void flame_class_init(FlameClass *klass) {
...
static void flame_init(Flame *flame) {}
...
static GimpProcedure *flame_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, flame_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Scott Draves", "Scott Draves",
                                   "1997");
  }
...
static void drawable_to_cmap(control_point *cp) {
  gint i, j;
  ...
  if (TABLE_DRAWABLE >= config.cmap_drawable_id) {
    ...
    gimp_gradient_get_uniform_samples(name, 256, FALSE, &num, &g);
    ...
    for (i = 0; i < 256; i++) {
      gegl_buffer_sample(buffer, i % width, (i / width) % height, NULL, p,
                         babl_format("R'G'B' u8"), GEGL_SAMPLER_NEAREST,
                         GEGL_ABYSS_NONE);
...
static void flame(GimpDrawable *drawable) {
  ...
  render_rectangle(&f, tmp, width, field_both, 4, gimp_progress_update);
  ...
  if (bytes == 4) {
    ...
    gegl_buffer_set(buffer, GEGL_RECTANGLE(0, 0, width, height), 0, format, tmp,
                    GEGL_AUTO_ROWSTRIDE);
    ...
    for (i = 0; i < height; i++) {
      ...
      gegl_buffer_get(src_buffer, GEGL_RECTANGLE(0, i, width, 1), 1.0, format,
                      sl, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
      ...
      gegl_buffer_set(dest_buffer, GEGL_RECTANGLE(0, i, width, 1), 0, format,
                      sl, GEGL_AUTO_ROWSTRIDE);
    }
...
static void file_response_callback(GtkFileChooser *chooser, gint response_id,
                                   gpointer data) {
...
static void make_file_dialog(const gchar *title, GtkWidget *parent) {
  file_dialog = gtk_file_chooser_dialog_new(
      title, GTK_WINDOW(parent),
      load_save ? GTK_FILE_CHOOSER_ACTION_OPEN : GTK_FILE_CHOOSER_ACTION_SAVE,

      _("_Cancel"), GTK_RESPONSE_CANCEL, load_save ? _("_Open") : _("_Save"),
      GTK_RESPONSE_OK,
  ...
  gimp_dialog_set_alternative_button_order(
      GTK_DIALOG(file_dialog), GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL, -1);
  ...
  g_signal_connect(file_dialog, "delete-event", G_CALLBACK(gtk_true), NULL);
  g_signal_connect(file_dialog, "response", G_CALLBACK(file_response_callback),
                   NULL);
}
...
static void edit_response(GtkWidget *widget, gint response_id, gpointer data) {
...
static void set_edit_preview(void) {
  ...
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) {
      ...
      gimp_preview_area_draw(GIMP_PREVIEW_AREA(edit_previews[mut]), 0, 0,
                             EDIT_PREVIEW_SIZE, EDIT_PREVIEW_SIZE,
                             GIMP_RGB_IMAGE, b, EDIT_PREVIEW_SIZE * 3);
    }
  g_free(b);
}

static void edit_preview_size_allocate(GtkWidget *widget) {
  set_edit_preview();
}
...
static void edit_callback(GtkWidget *widget, GtkWidget *parent) {
  edit_cp = config.cp;
  ...
  if (edit_dialog == NULL) {
    ...
    edit_dialog = gimp_dialog_new(
        _("Edit Flame"), PLUG_IN_ROLE, parent, GTK_DIALOG_DESTROY_WITH_PARENT,
    ...
    gimp_dialog_set_alternative_button_order(
        GTK_DIALOG(edit_dialog), GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL, -1);

    g_signal_connect(edit_dialog, "response", G_CALLBACK(edit_response),
                     edit_dialog);

    main_vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 12);
    gtk_container_set_border_width(GTK_CONTAINER(main_vbox), 12);
    gtk_box_pack_start(
        GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(edit_dialog))),
    ...
    for (i = 0; i < 3; i++)
      for (j = 0; j < 3; j++) {
        ...
        gtk_widget_set_size_request(edit_previews[mut], EDIT_PREVIEW_SIZE,
                                    EDIT_PREVIEW_SIZE);
        ...
        g_signal_connect(button, "clicked", G_CALLBACK(preview_clicked),
                         GINT_TO_POINTER(mut));
      }
    ...
    g_signal_connect(scale, "value-changed",
                     G_CALLBACK(flame_scale_entry_update_double), &pick_speed);
    g_signal_connect(scale, "value-changed", G_CALLBACK(set_edit_preview),
                     NULL);
    ...
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(randomize_callback),
                             NULL);

    combo = gimp_int_combo_box_new(
        _("Same"), VARIATION_SAME, _("Random"), variation_random, _("Linear"),
        0, _("Sinusoidal"), 1, _("Spherical"), 2, _("Swirl"), 3, _("Horseshoe"),
        4, _("Polar"), 5, _("Bent"), 6, _("Handkerchief"), 7, _("Heart"), 8,
        _("Disc"), 9, _("Spiral"), 10, _("Hyperbolic"), 11, _("Diamond"), 12,
        _("Ex"), 13, _("Julia"), 14, _("Waves"), 15, _("Fisheye"), 16,
        _("Popcorn"), 17, _("Exponential"), 18, _("Power"), 19, _("Cosine"), 20,
        _("Rings"), 21, _("Fan"), 22, _("Eyefish"), 23, _("Bubble"), 24,
        _("Cylinder"), 25, _("Noise"), 26, _("Blur"), 27, _("Gaussian"), 28,
        NULL);

    gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), VARIATION_SAME);

    g_signal_connect(combo, "changed", G_CALLBACK(combo_callback),
                     &config.variation);
...
static void set_flame_preview(void) {
  ...
  pf.cps = &pcp;
  pcp = config.cp;
  pcp.pixels_per_unit = (pcp.pixels_per_unit * preview_width) / pcp.width;
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(flame_preview), 0, 0, preview_width,
                         PREVIEW_SIZE, GIMP_RGB_IMAGE, b, preview_width * 3);
  g_free(b);
}

static void flame_preview_size_allocate(GtkWidget *preview) {
  set_flame_preview();
}
...
static void set_cmap_preview(void) {
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(cmap_preview), 0, 0, 32, 32,
                         GIMP_RGB_IMAGE, cmap_buffer, 32 * 3);
  g_free(cmap_buffer);
}

static void cmap_callback(GtkWidget *widget, gpointer data) {
...
static gboolean cmap_constrain(GimpImage *image, GimpItem *item,
                               gpointer data) {
...
static gboolean flame_dialog(void) {
  ...
  dialog = gimp_dialog_new(
      _("Flame"), PLUG_IN_ROLE, NULL, 0, gimp_standard_help_func, PLUG_IN_PROC,
...

  {
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(edit_callback), dialog);
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(save_callback), NULL);
  }
  ...
  scale =
      gimp_scale_entry_new(_("_Brightness:"), config.cp.brightness, 0, 5, 2);
  ...
  g_signal_connect(scale, "value-changed", G_CALLBACK(set_flame_preview), NULL);
  ...
  scale = gimp_scale_entry_new(_("Sample _density:"), config.cp.sample_density,
                               0.1, 20, 2);
  ...
  scale = gimp_scale_entry_new(_("Spa_tial oversample:"),
                               config.cp.spatial_oversample, 1, 4, 0);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 0.01, 0.1);
  ...
  scale = gimp_scale_entry_new(_("Spatial _filter radius:"),
                               config.cp.spatial_filter_radius, 0, 4, 2);
...

  {
    ...
    {
      static const gchar *names[] = {"sunny harvest",   "rose",
                                     "calcoast09",      "klee insula-dulcamara",
      ...
      for (i = 0; i < G_N_ELEMENTS(good); i++) {
        ...
        gimp_int_combo_box_prepend(GIMP_INT_COMBO_BOX(combo),
                                   ...
                                   GIMP_INT_STORE_LABEL, names[i], -1);
      }
    }

    gimp_int_combo_box_prepend(GIMP_INT_COMBO_BOX(combo), GIMP_INT_STORE_VALUE,
                               GRADIENT_DRAWABLE, GIMP_INT_STORE_LABEL,
                               _("Custom gradient"), GIMP_INT_STORE_ICON_NAME,
                               GIMP_ICON_GRADIENT, -1);

    gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo),
                               config.cmap_drawable_id,
                               G_CALLBACK(cmap_callback), NULL, NULL);
