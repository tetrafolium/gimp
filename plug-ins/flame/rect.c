...
void render_rectangle(frame_spec *spec, unsigned char *out, int out_width,
                      int field, int nchan, int progress(double)) {
  ...
  if (1) {
    filter_width =
        (2.0 * FILTER_CUTOFF * oversample * spec->cps[0].spatial_filter_radius);
  ...
  if (1) {
    ...
    int memory_rqd = (sizeof(bucket) * nbuckets + sizeof(abucket) * nbuckets +
    ...
    points =
        (point *)(last_block + (sizeof(bucket) + sizeof(abucket)) * nbuckets);
  }
  ...
  for (batch_num = 0; batch_num < nbatches; batch_num++) {
    ...
    for (j = 0; j < CMAP_SIZE; j++) {
      for (k = 0; k < 3; k++) {
#if 1
        cmap[j][k] = (int)(cp.cmap[(j * 256) / CMAP_SIZE][k] * cp.white_level);
#else
    ...
    nsamples = (int)(sample_density * nbuckets / (oversample * oversample));
    ...
    for (sub_batch = 0; sub_batch < batch_size; sub_batch += SUB_BATCH_SIZE) {
      ...
      for (j = 0; j < SUB_BATCH_SIZE; j++) {
         ...
         */
        if (p[0] >= bounds[0] && p[1] >= bounds[1] && p[0] <= bounds[2] &&
            p[1] <= bounds[3]) {
          ...
          b = buckets + (int)(width * (p[0] - bounds[0]) * size[0]) +
    ...
    if (1) {
      double k1 = (cp.contrast * cp.brightness * PREFILTER_WHITE * 268.0 *
                   temporal_filter[batch_num]) /
                  256;
