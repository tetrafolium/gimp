...
#define REGISTRY_KEY_BASE                                                      \
  "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\rawtherapee"
...
#define RAWTHERAPEE                                                            \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), RAWTHERAPEE_TYPE, Rawtherapee))
...
static void rawtherapee_class_init(RawtherapeeClass *klass) {
...
static void rawtherapee_init(Rawtherapee *rawtherapee) {}
...
static GList *rawtherapee_init_procedures(GimpPlugIn *plug_in) {
  ...
  gchar *exec_path = file_raw_get_executable_path(
      "rawtherapee", NULL, "RAWTHERAPEE_EXECUTABLE",
      "com.rawtherapee.rawtherapee", REGISTRY_KEY_BASE, &search_path);
#ifdef G_OS_WIN32
  ...
  if (g_spawn_sync(NULL, argv, NULL,
                   ...
                   NULL, NULL, &rawtherapee_stdout, NULL, NULL, NULL)) {
    gint rtmajor = 0;
    gint rtminor = 0;

    if (sscanf(rawtherapee_stdout, "RawTherapee, version %d.%d", &rtmajor,
               &rtminor) == 2 &&
...
static GimpProcedure *rawtherapee_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_THUMB_PROC)) {
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "rawtherapee-cli.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Alberto Griggio",
                                   "Alberto Griggio", "2017");
  } else {
    gint i;
    ...
    for (i = 0; i < G_N_ELEMENTS(file_formats); i++) {
      ...
      procedure =
          gimp_load_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                  rawtherapee_load, (gpointer)format, NULL);
...
static GimpImage *load_image(GFile *file, GimpRunMode run_mode,
  ...
  gchar *argv[] = {exec_path, "-gimp", (gchar *)filename, filename_out, NULL};
  ...
  if (g_spawn_sync(NULL, argv, NULL,
                   ...
                   G_SPAWN_STDERR_TO_DEV_NULL |
                       (search_path ? G_SPAWN_SEARCH_PATH : 0),
                   NULL, NULL, &rawtherapee_stdout, NULL, NULL, error)) {
...
static GimpImage *load_thumbnail_image(GFile *file, gint thumb_size,
  ...
  gchar *rawtherapee_stdout = NULL;
  const char *pp3_content = "[Version]\n"
                            "AppVersion=5.0\n"
                            "Version=326\n"
                            "\n"
                            "[Resize]\n"
  ...
  gboolean search_path = FALSE;
  gchar *exec_path = file_raw_get_executable_path(
      "rawtherapee", "-cli", "RAWTHERAPEE_EXECUTABLE",
      "com.rawtherapee.rawtherapee", REGISTRY_KEY_BASE, &search_path);
  gchar *argv[] = {
      exec_path, "-o", filename_out,     "-d", "-s", "-j", "-p", thumb_pp3,
      "-f",      "-c", (char *)filename, NULL};
