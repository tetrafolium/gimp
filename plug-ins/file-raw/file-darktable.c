...
#define REGISTRY_KEY_BASE                                                      \
  "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\darktable"
...
#define DARKTABLE                                                              \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), DARKTABLE_TYPE, Darktable))
...
static GimpValueArray *darktable_load_thumb(GimpProcedure *procedure,
                                            GFile *file, gint size,
                                            const GimpValueArray *args,
                                            gpointer run_data);

static GimpImage *load_image(GFile *file, GimpRunMode run_mode, GError **error);
static GimpImage *load_thumbnail_image(GFile *file, gint thumb_size,
...
static void darktable_class_init(DarktableClass *klass) {
...
static void darktable_init(Darktable *darktable) {}

static GList *darktable_init_procedures(GimpPlugIn *plug_in) {
  /* check if darktable is installed
   */
  gboolean search_path = FALSE;
  gchar *exec_path = file_raw_get_executable_path(
      "darktable", NULL, "DARKTABLE_EXECUTABLE", "org.darktable",
      REGISTRY_KEY_BASE, &search_path);
  ...
  if (g_spawn_sync(NULL, argv, NULL, (search_path ? G_SPAWN_SEARCH_PATH : 0),
                   NULL, NULL, &darktable_stdout, &darktable_stderr, NULL,
                   &error)) {
    GRegex *regex;
    GMatchInfo *matches;
    gint major;
     ...
     */
    regex = g_regex_new(
        "this is darktable [^0-9]*([0-9]+)\\.([0-9]+)\\.([0-9]+)", 0, 0, NULL);
    if (g_regex_match(regex, darktable_stdout, 0, &matches)) {
      ...
      if (((major == 1 && minor >= 7) || major >= 2)) {
        if (g_strstr_len(darktable_stdout, -1, "Lua support enabled")) {
          have_darktable = TRUE;
        }
      }
    }
...
static GimpProcedure *darktable_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_THUMB_PROC)) {
    procedure =
        gimp_thumbnail_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "darktable-cli.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Tobias Ellinghaus",
                                   "Tobias Ellinghaus", "2016");
  } else {
    gint i;
    ...
    for (i = 0; i < G_N_ELEMENTS(file_formats); i++) {
      ...
      procedure =
          gimp_load_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                  darktable_load, (gpointer)format, NULL);

      gimp_procedure_set_documentation(procedure, load_blurb, load_help, name);
      gimp_procedure_set_attribution(procedure, "Tobias Ellinghaus",
                                     "Tobias Ellinghaus", "2016");
      ...
      gimp_load_procedure_set_handles_raw(GIMP_LOAD_PROCEDURE(procedure), TRUE);
...
static GimpImage *load_image(GFile *file, GimpRunMode run_mode,
                             GError **error) {
  GimpImage *image = NULL;
  GFile *lua_file = gimp_data_directory_file(
      "file-raw", "file-darktable-export-on-exit.lua", NULL);
  ...
  gchar *export_filename =
      g_strdup_printf("lua/export_on_exit/export_filename=%s", filename_out);
  ...
  if (g_spawn_sync(NULL, argv, NULL,
                   ...
                   (search_path ? G_SPAWN_SEARCH_PATH : 0), NULL, NULL,
                   &darktable_stdout, &darktable_stderr, NULL, error)) {
...
static GimpImage *load_thumbnail_image(GFile *file, gint thumb_size,
  ...
  GFile *lua_file =
      gimp_data_directory_file("file-raw", "file-darktable-get-size.lua", NULL);
  gchar *lua_script = g_file_get_path(lua_file);
  gchar *lua_script_escaped = g_strescape(lua_script, "");
  gchar *lua_quoted = g_shell_quote(lua_script_escaped);
  gchar *lua_cmd = g_strdup_printf("dofile(%s)", lua_quoted);
  ...
  gboolean search_path = FALSE;
  gchar *exec_path = file_raw_get_executable_path(
      "darktable", "-cli", "DARKTABLE_EXECUTABLE", "org.darktable",
      REGISTRY_KEY_BASE, &search_path);
  gchar *argv[] = {exec_path,    (gchar *)filename,
                   filename_out, "--width",
                   size,         "--height",
                   size,         "--hq",
                   "false",      "--core",
  ...
  *width = *height = thumb_size;
  ...
  if (g_spawn_sync(NULL, argv, NULL,
                   ...
                   NULL, NULL, &darktable_stdout, NULL, NULL, error)) {
    ...
    if (image) {
       ...
       */
      gchar *start_of_size = g_strstr_len(darktable_stdout, -1, "[dt4gimp]");
      if (start_of_size)
