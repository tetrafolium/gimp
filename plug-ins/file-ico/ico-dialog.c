...
static void ico_dialog_bpp_changed(GtkWidget *combo, GObject *hbox);
static void ico_dialog_toggle_compress(GtkWidget *checkbox, GObject *hbox);
static void ico_dialog_check_compat(GtkWidget *dialog, IcoSaveInfo *info);
...
GtkWidget *ico_dialog_new(IcoSaveInfo *info) {
  ...
  dialog = gimp_export_dialog_new(_("Windows Icon"), PLUG_IN_BINARY,
                                  "plug-in-winicon");
  ...
  warning = g_object_new(GIMP_TYPE_HINT_BOX, "icon-name",
...
static GtkWidget *ico_preview_new(GimpDrawable *layer) {
  GtkWidget *image;
  GdkPixbuf *pixbuf;
  gint width = gimp_drawable_width(layer);
  gint height = gimp_drawable_height(layer);

  pixbuf = gimp_drawable_get_thumbnail(layer, MIN(width, 128), MIN(height, 128),
...
static GtkWidget *ico_create_icon_hbox(GtkWidget *icon_preview,
                                       GimpDrawable *layer, gint layer_num,
                                       IcoSaveInfo *info) {
  ...
  g_object_set_data(G_OBJECT(hbox), "icon_layer", layer);
  g_object_set_data(G_OBJECT(hbox), "icon_layer_num",
                    GINT_TO_POINTER(layer_num));
  ...
  combo =
      gimp_int_combo_box_new(_("1 bpp, 1-bit alpha, 2-slot palette"), 1,
                             _("4 bpp, 1-bit alpha, 16-slot palette"), 4,
                             _("8 bpp, 1-bit alpha, 256-slot palette"), 8,
                             _("24 bpp, 1-bit alpha, no palette"), 24,
                             _("32 bpp, 8-bit alpha, no palette"), 32, NULL);
  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo),
                                info->depths[layer_num]);

  g_signal_connect(combo, "changed", G_CALLBACK(ico_dialog_bpp_changed), hbox);
  ...
  g_signal_connect(checkbox, "toggled", G_CALLBACK(ico_dialog_toggle_compress),
                   hbox);
...
static GtkWidget *ico_dialog_get_layer_preview(GtkWidget *dialog,
                                               GimpDrawable *layer) {
  GtkWidget *preview;
  GtkWidget *icon_hbox;
...
static void ico_dialog_update_icon_preview(GtkWidget *dialog,
                                           GimpDrawable *layer, gint bpp) {
  ...
  if (bpp <= 8) {
    ...
    tmp_layer =
        gimp_layer_new(tmp_image, "temporary", w, h, gimp_drawable_type(layer),
                       100, gimp_image_get_default_new_layer_mode(tmp_image));
    ...
    if (num_colors == (1 << bpp) &&
      ...
      gimp_image_convert_indexed(tmp_image, GIMP_CONVERT_DITHER_FS,
                                 GIMP_CONVERT_PALETTE_GENERATE, (1 << bpp) - 1,
                                 TRUE, FALSE, "dummy");
    }
    ...
    pixbuf = gimp_drawable_get_thumbnail(GIMP_DRAWABLE(tmp_layer), MIN(w, 128),
                                         MIN(h, 128), GIMP_PIXBUF_SMALL_CHECKS);
    ...
    return_vals = gimp_pdb_run_procedure(
        ...
        GIMP_RUN_NONINTERACTIVE, GIMP_TYPE_IMAGE, tmp_image, GIMP_TYPE_DRAWABLE,
        tmp_layer, G_TYPE_INT, ICO_ALPHA_THRESHOLD, G_TYPE_NONE);
    ...
    pixbuf = gimp_drawable_get_thumbnail(layer, MIN(w, 128), MIN(h, 128),
                                         GIMP_PIXBUF_SMALL_CHECKS);
  }
...
static void ico_dialog_check_compat(GtkWidget *dialog, IcoSaveInfo *info) {
  ...
  for (iter = info->layers, i = 0; iter; iter = iter->next, i++) {
    if (gimp_drawable_width(iter->data) > 255 ||
        gimp_drawable_height(iter->data) > 255 || info->compress[i]) {
      warn = TRUE;
      break;
    }
  }
