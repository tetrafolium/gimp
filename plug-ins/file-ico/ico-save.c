...
static void ico_set_bit_in_data(guint8 *data, gint line_width, gint bit_num,
                                gint bit_val);
static void ico_set_nibble_in_data(guint8 *data, gint line_width,
                                   gint nibble_num, gint nibble_val);
static void ico_set_byte_in_data(guint8 *data, gint line_width, gint byte_num,
                                 gint byte_val);

static gint ico_get_layer_num_colors(GimpLayer *layer,
                                     gboolean *uses_alpha_levels);
static void ico_image_get_reduced_buf(GimpDrawable *layer, gint bpp,
                                      gint *num_colors, guchar **cmap_out,
                                      guchar **buf_out);
...
static void ico_save_init(GimpImage *image, IcoSaveInfo *info) {
  GList *iter;
  gint num_colors;
  gint i;
   ...
   */
  for (iter = info->layers, i = 0; iter; iter = g_list_next(iter), i++) {
  ...
  memcpy(info->depths, info->default_depths, sizeof(gint) * info->num_icons);
}

static gboolean ico_save_dialog(GimpImage *image, IcoSaveInfo *info) {
  GtkWidget *dialog;
  GList *iter;
  gint i;
  gint response;
  ...
  gtk_window_set_default_size(
      GTK_WINDOW(dialog), -1,
      200 + (info->num_icons > 4 ? 500 : info->num_icons * 120));
...
static guint32 *ico_create_palette(const guchar *cmap, gint num_colors,
                                   gint num_colors_used, gint *black_slot) {
  guchar *palette;
  gint i;
...
static GHashTable *ico_create_color_to_palette_map(const guint32 *palette,
                                                   gint num_colors) {
  GHashTable *hash;
  gint i;

  hash = g_hash_table_new_full(g_int_hash, g_int_equal, (GDestroyNotify)g_free,
...
static gint ico_get_palette_index(GHashTable *hash, gint red, gint green,
                                  gint blue) {
  gint color = 0;
  gint *slot;
...
static gint ico_get_layer_num_colors(GimpLayer *layer,
  ...
  gegl_buffer_get(buffer, GEGL_RECTANGLE(0, 0, w, h), 1.0, format, buf,
...
gboolean ico_cmap_contains_black(const guchar *cmap, gint num_colors) {
  gint i;

  for (i = 0; i < num_colors; i++) {
    if ((cmap[3 * i] == 0) && (cmap[3 * i + 1] == 0) &&
        (cmap[3 * i + 2] == 0)) {
      return TRUE;
    }
  }
...
static void ico_image_get_reduced_buf(GimpDrawable *layer, gint bpp,
  ...
  *num_colors = 0;
  ...
  if (bpp <= 8 || bpp == 24 || babl_format_get_bytes_per_pixel(format) != 4) {
    ...
    tmp_layer =
        gimp_layer_new(tmp_image, "tmp", w, h, gimp_drawable_type(layer), 100,
    ...
    if (bpp <= 8) {
      gimp_image_convert_indexed(tmp_image, GIMP_CONVERT_DITHER_FS,
                                 GIMP_CONVERT_PALETTE_GENERATE, 1 << bpp, TRUE,
                                 FALSE, "dummy");
      ...
      if (*num_colors == (1 << bpp) &&
        ...
        gegl_buffer_set(tmp, GEGL_RECTANGLE(0, 0, w, h), 0, format, buf,
                        GEGL_AUTO_ROWSTRIDE);
      ...
      return_vals = gimp_pdb_run_procedure(
          gimp_get_pdb(), "plug-in-threshold-alpha", GIMP_TYPE_RUN_MODE,
          GIMP_RUN_NONINTERACTIVE, GIMP_TYPE_IMAGE, tmp_image,
          GIMP_TYPE_DRAWABLE, tmp_layer, G_TYPE_INT, ICO_ALPHA_THRESHOLD,
          G_TYPE_NONE);
    ...
    gegl_buffer_get(tmp, GEGL_RECTANGLE(0, 0, w, h), 1.0, NULL, buf,
                    GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
...
static gboolean ico_write_png(FILE *fp, GimpDrawable *layer, gint32 depth) {
  png_structp png_ptr;
  png_infop info_ptr;
  ...
  ico_image_get_reduced_buf(layer, depth, &num_colors_used, &palette, &buf);
...
static gboolean ico_write_icon(FILE *fp, GimpDrawable *layer, gint32 depth) {
  ...
  D(("  header size %i, w %i, h %i, planes %i, bpp %i\n", header.header_size,
     header.width, header.height, header.planes, header.bpp));

  /* Reduce colors in copy of image */
  ico_image_get_reduced_buf(layer, header.bpp, &num_colors_used, &palette,
                            &buf);
  buf32 = (guint32 *)buf;
  ...
  if (header.bpp <= 8) {
    /* Create a colormap */
    palette32 =
        ico_create_palette(palette, num_colors, num_colors_used, &black_index);
  ...
  for (y = 0; y < height; y++)
    for (x = 0; x < width; x++) {
      ...
      ico_set_bit_in_data(and_map, width, (height - y - 1) * width + x,
  ...
  case 1:
    for (y = 0; y < height; y++)
      for (x = 0; x < width; x++) {
        pixel = (guint8 *)&buf32[y * width + x];
        palette_index =
            ico_get_palette_index(color_to_slot, pixel[0], pixel[1], pixel[2]);

        if (ico_get_bit_from_data(and_map, width,
                                  (height - y - 1) * width + x)) {
          ico_set_bit_in_data(xor_map, width, (height - y - 1) * width + x,
                              black_index);
        } else {
          ico_set_bit_in_data(xor_map, width, (height - y - 1) * width + x,
                              palette_index);
  ...
  case 4:
    for (y = 0; y < height; y++)
      for (x = 0; x < width; x++) {
        ...
        if (ico_get_bit_from_data(and_map, width,
                                  (height - y - 1) * width + x)) {
          ico_set_nibble_in_data(xor_map, width, (height - y - 1) * width + x,
                                 black_index);
        } else {
          ico_set_nibble_in_data(xor_map, width, (height - y - 1) * width + x,
                                 palette_index);
  ...
  case 8:
    for (y = 0; y < height; y++)
      for (x = 0; x < width; x++) {
        ...
        if (ico_get_bit_from_data(and_map, width,
                                  (height - y - 1) * width + x)) {
          ico_set_byte_in_data(xor_map, width, (height - y - 1) * width + x,
                               black_index);
        } else {
          ico_set_byte_in_data(xor_map, width, (height - y - 1) * width + x,
                               palette_index);
  ...
  default:
    for (y = 0; y < height; y++)
      for (x = 0; x < width; x++) {
        ...
        ((guint32 *)xor_map)[(height - y - 1) * width + x] = GUINT32_TO_LE(
...
static void ico_save_info_free(IcoSaveInfo *info) {
  g_free(info->depths);
...
GimpPDBStatusType ico_save_image(GFile *file, GimpImage *image, gint32 run_mode,
                                 GError **error) {
  gchar *filename;
  FILE *fp;
