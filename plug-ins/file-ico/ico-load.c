...
static guint32 ico_read_init(FILE *fp) {
  ...
  if (!ico_read_int16(fp, &header.reserved, 1) ||
      ...
      !ico_read_int16(fp, &header.icon_count, 1) || header.reserved != 0 ||
...
static gboolean ico_read_size(FILE *fp, IcoLoadInfo *info) {
  ...
  if (magic == ICO_PNG_MAGIC) {
    ...
    png_get_IHDR(png_ptr, info_ptr, &w, &h, &bpp, &color_type, NULL, NULL,
                 NULL);
...
static IcoLoadInfo *ico_read_info(FILE *fp, gint icon_count, GError **error) {
  gint i;
  ...
  if (fread(entries, sizeof(IcoFileEntry), icon_count, fp) <= 0) {
    g_set_error(error, G_FILE_ERROR, 0, _("Could not read '%lu' bytes"),
  ...
  for (i = 0; i < icon_count; i++) {
    ...
    D(("ico_read_info: %ix%i (%i bits, size: %i, offset: %i)\n", info[i].width,
       info[i].height, info[i].bpp, info[i].size, info[i].offset));
...
static gboolean ico_read_png(FILE *fp, guint32 header, guchar *buf,
                             gint maxsize, gint *width, gint *height) {
  png_structp png_ptr;
  png_infop info;
  png_uint_32 w;
  png_uint_32 h;
...
gint ico_get_nibble_from_data(const guint8 *data, gint line_width,
                              gint nibble) {
  gint line;
  gint width32;
  gint offset;
  ...
  result = (data[line * width32 * 4 + offset / 2] &
            (0x0F << (4 * (1 - offset % 2))));
...
static gboolean ico_read_icon(FILE *fp, guint32 header_size, guchar *buf,
                              gint maxsize, gint *width, gint *height) {
  ...
  D(("  header size %i, "
     ...
     data.header_size, data.width, data.height, data.planes, data.image_size,
     data.bpp, data.used_clrs, data.important_clrs));
  ...
  if (data.bpp != 1 && data.bpp != 4 && data.bpp != 8 && data.bpp != 24 &&
      data.bpp != 32) {
  ...
  case 4:
    for (y = 0; y < h; y++)
      for (x = 0; x < w; x++) {
        guint32 color =
            palette[ico_get_nibble_from_data(xor_map, w, y * w + x)];
        guint32 *dest = dest_vec + (h - 1 - y) * w + x;
...
static GimpLayer *ico_load_layer(FILE *fp, GimpImage *image, gint32 icon_num,
                                 guchar *buf, gint maxsize, IcoLoadInfo *info) {
  gint width, height;
  GimpLayer *layer;
  guint32 first_bytes;
  GeglBuffer *buffer;
  ...
  gegl_buffer_set(buffer, GEGL_RECTANGLE(0, 0, width, height), 0, NULL, buf,
                  GEGL_AUTO_ROWSTRIDE);
