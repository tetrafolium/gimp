...
#define SCREENSHOT                                                             \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), SCREENSHOT_TYPE, Screenshot))
...
static GimpPDBStatusType shoot(GdkMonitor *monitor, GimpImage **image,
                               GError **error);
...
static void screenshot_class_init(ScreenshotClass *klass) {
...
static void screenshot_init(Screenshot *screenshot) {}
...
static GimpProcedure *screenshot_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    ...
    gimp_procedure_set_documentation(
        procedure, N_("Create an image from an area of the screen"),
    ...
    gimp_procedure_set_attribution(procedure,
                                   ...
                                   "1998 - 2008", "v1.1 (2008/04)");

    gimp_procedure_set_icon_pixbuf(
        procedure,
        gdk_pixbuf_new_from_inline(-1, screenshot_icon, FALSE, NULL));

    GIMP_PROC_ARG_ENUM(procedure, "run-mode", "Run mode", "The run mode",
                       GIMP_TYPE_RUN_MODE, GIMP_RUN_NONINTERACTIVE,
                       G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "shoot-type", "Shoot type",
                      "The shoot type { SHOOT-WINDOW (0), SHOOT-ROOT (1), "
                      "SHOOT-REGION (2) }",
                      0, 2, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "x1", "X1",
                      "Region left x coord for SHOOT-WINDOW", G_MININT,
                      G_MAXINT, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "y1", "Y1",
                      "Region top y coord for SHOOT-WINDOW", G_MININT, G_MAXINT,
                      0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "x2", "X2",
                      "Region right x coord for SHOOT-WINDOW", G_MININT,
                      G_MAXINT, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "y2", "Y2",
                      "Region bottom y coord for SHOOT-WINDOW", G_MININT,
                      G_MAXINT, 0, G_PARAM_READWRITE);

    GIMP_PROC_VAL_IMAGE(procedure, "image", "Image", "Output image", FALSE,
                        G_PARAM_READWRITE);
  }
  ...
  if (status == GIMP_PDB_SUCCESS) {
    ...
    if (shootvals.profile_policy == SCREENSHOT_PROFILE_POLICY_SRGB) {
      ...
      gimp_image_convert_color_profile(
          image, srgb_profile,
          GIMP_COLOR_RENDERING_INTENT_RELATIVE_COLORIMETRIC, TRUE);
...
static void shoot_dialog_add_hint(GtkNotebook *notebook, ShootType type,
                                  const gchar *hint) {
  GtkWidget *label;

  label = g_object_new(GTK_TYPE_LABEL, "label", hint, "wrap", TRUE, "justify",
                       GTK_JUSTIFY_LEFT, "xalign", 0.0, "yalign", 0.0, NULL);
...
static void shoot_radio_button_toggled(GtkWidget *widget, GtkWidget *notebook) {
...
static gboolean shoot_dialog(GdkMonitor **monitor) {
  ...
  dialog = gimp_dialog_new(_("Screenshot"), PLUG_IN_ROLE, NULL, 0,
                           gimp_standard_help_func, PLUG_IN_PROC,

                           _("_Cancel"), GTK_RESPONSE_CANCEL, _("S_nap"),
                           GTK_RESPONSE_OK,
  ...
  notebook1 = g_object_new(GTK_TYPE_NOTEBOOK, "show-border", FALSE, "show-tabs",
                           FALSE, NULL);
  notebook2 = g_object_new(GTK_TYPE_NOTEBOOK, "show-border", FALSE, "show-tabs",
                           FALSE, NULL);
  ...
  if (capabilities & SCREENSHOT_CAN_SHOOT_WINDOW) {
    ...
    g_signal_connect(button, "toggled", G_CALLBACK(shoot_radio_button_toggled),
                     notebook1);
    g_signal_connect(button, "toggled", G_CALLBACK(shoot_radio_button_toggled),
                     notebook2);
    ...
    if (capabilities & SCREENSHOT_CAN_SHOOT_DECORATIONS) {
      ...
      toggle =
          gtk_check_button_new_with_mnemonic(_("Include window _decoration"));
      ...
      g_object_bind_property(button, "active", toggle, "sensitive",
    ...
    if (capabilities & SCREENSHOT_CAN_SHOOT_POINTER) {
      ...
      cursor_toggle =
          gtk_check_button_new_with_mnemonic(_("Include _mouse pointer"));
      ...
      g_object_bind_property(button, "active", cursor_toggle, "sensitive",
                             G_BINDING_SYNC_CREATE);
    }
  ...
  /*  Whole screen  */
  button =
      gtk_radio_button_new_with_mnemonic(radio_group, _("Take a screenshot of "
                                                        "the entire _screen"));
  radio_group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(button));
  gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 0);
  gtk_widget_show(button);
  ...
  if (capabilities & SCREENSHOT_CAN_SHOOT_POINTER) {
    ...
    if (cursor_toggle) {
      g_object_bind_property(cursor_toggle, "active", toggle, "active",
  ...
  adj =
      gtk_adjustment_new(shootvals.screenshot_delay, 0.0, 100.0, 1.0, 5.0, 0.0);
  spinner = gimp_spin_button_new(adj, 0, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinner), TRUE);
  gtk_grid_attach(GTK_GRID(grid), spinner, 1, 0, 1, 1);
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Color Profile"), G_CALLBACK(gimp_radio_button_update),
      &shootvals.profile_policy, NULL, shootvals.profile_policy,

      _("Tag image with _monitor profile"), SCREENSHOT_PROFILE_POLICY_MONITOR,
      NULL,

      _("Convert image to sR_GB"), SCREENSHOT_PROFILE_POLICY_SRGB, NULL,
