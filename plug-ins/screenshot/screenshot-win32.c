...
static gboolean doCaptureMagnificationAPI(HWND selectedHwnd, RECT rect);
static void doCaptureMagnificationAPI_callback(
...
static gboolean doCaptureBitBlt(HWND selectedHwnd, RECT rect);
...
gboolean screenshot_win32_available(void) { return TRUE; }

ScreenshotCapabilities screenshot_win32_get_capabilities(void) {
  return (SCREENSHOT_CAN_SHOOT_DECORATIONS | SCREENSHOT_CAN_SHOOT_WINDOW);
}

GimpPDBStatusType screenshot_win32_shoot(ScreenshotValues *shootvals,
                                         GdkMonitor *monitor,
                                         GimpImage **_image, GError **error) {
 ...
 * GIMP.
 */
static void flipRedAndBlueBytes(int width, int height) {
  int i, j;
  guchar *bufp;
  guchar temp;
 ...
 */
static void rgbaToRgbBytes(guchar *rgbBufp, guchar *rgbaBufp, int rgbaBufSize) {
...
static void sendBMPToGimp(HBITMAP hBMP, HDC hDC, RECT rect) {
  ...
  layer = gimp_layer_new(new_image, _("Background"), ROUND4(width), height,
                         layerType, 100,
 ...
 * buffer.
 */
static void formatWindowsError(char *buffer, int buf_size) {
  LPVOID lpMsgBuf;
 ...
 * captured.
 */
static HBITMAP primDoWindowCapture(HDC hdcWindow, HDC hdcCompat, RECT rect) {
  HBITMAP hbmCopy;
  HGDIOBJ oldObject;
  BITMAPINFO bmi;
  ...
  hbmCopy = CreateDIBSection(hdcCompat, (BITMAPINFO *)&bmi, DIB_RGB_COLORS,
   ...
   */
  if (!BitBlt(hdcCompat, 0, 0, width, height, hdcWindow, rect.left, rect.top,
              SRCCOPY)) {
    formatWindowsError(buffer, sizeof buffer);
...
static BOOL CALLBACK GetAccurateWindowRect_MonitorEnumProc(HMONITOR hMonitor,
                                                           HDC hdcMonitor,
                                                           LPRECT lprcMonitor,
                                                           LPARAM dwData) {
  ...
  if (!(*rectScreens))
    ...
    *rectScreens =
        (RECT *)g_realloc(rectScreens, sizeof(RECT) * (rectScreensCount + 1));
...
static BOOL GetAccurateWindowRect(HWND hwndTarget, RECT *outRect) {
  HRESULT dwmResult;
  WINDOWPLACEMENT windowplacment;

  ZeroMemory(outRect, sizeof(RECT));

  /* First we try to get the rect using the dwm api. it will also avoid us from
   * doing more ugly fixes when the window is maximized */
  if (LoadRequiredDwmFunctions()) {
    dwmResult = DwmGetWindowAttribute(hwndTarget, DWMWA_EXTENDED_FRAME_BOUNDS,
                                      (PVOID)outRect, sizeof(RECT));
    UnloadRequiredDwmFunctions();
    if (dwmResult == S_OK)
      return TRUE;
  }

  /* In this case, we did not got the rect from the dwm api so we try to get the
   * rect with the normal function */
  if (GetWindowRect(hwndTarget, outRect)) {
    ...
    if (GetWindowPlacement(hwndTarget, &windowplacment) &&
        windowplacment.showCmd == SW_SHOWMAXIMIZED) {
      ...
      /* If for some reason the array size is 0 then we fill it with the desktop
       * rect */
      if (!rectScreensCount) {
      ...
      if (rectScreens[i].left > outRect->left)
        outRect->left = rectScreens[i].left;
      if (rectScreens[i].right < outRect->right)
        outRect->right = rectScreens[i].right;
      if (rectScreens[i].top > outRect->top)
        outRect->top = rectScreens[i].top;
      if (rectScreens[i].bottom < outRect->bottom)
        outRect->bottom = rectScreens[i].bottom;
...
static BOOL isWindowIsAboveCaptureRegion(HWND hwndWindow, RECT rectCapture) {
  ...
  if (!GetWindowRect(hwndWindow, &rectWindow))
    return FALSE;
  if (((rectWindow.left >= rectCapture.left &&
        rectWindow.left < rectCapture.right) ||
       (rectWindow.right <= rectCapture.right &&
        rectWindow.right > rectCapture.left) ||
       (rectWindow.left <= rectCapture.left &&
        rectWindow.right >= rectCapture.right)) &&
      ((rectWindow.top >= rectCapture.top &&
        rectWindow.top < rectCapture.bottom) ||
       (rectWindow.bottom <= rectCapture.bottom &&
        rectWindow.bottom > rectCapture.top) ||
       (rectWindow.top <= rectCapture.top &&
        rectWindow.bottom >= rectCapture.bottom)))
    return TRUE;
  else
...
static gboolean doCaptureMagnificationAPI(HWND selectedHwnd, RECT rect) {
  ...
  round4Rect = rect;
  round4Rect.right =
      round4Rect.left + ROUND4(round4Rect.right - round4Rect.left);

  /* Create the host window that will store the mag child window */
  hwndHost = CreateWindowEx(0x08000000 | 0x080000 | 0x80 | 0x20, APP_NAME, NULL,
  ...
  hwndMag = CreateWindow(WC_MAGNIFIER, TEXT("MagnifierWindow"),
                         ...
                         0, 0, round4Rect.right - round4Rect.left,
                         round4Rect.bottom - round4Rect.top, hwndHost, NULL,
                         GetModuleHandle(NULL), NULL);
  ...
  if (!MagSetImageScalingCallback(
          hwndMag,
          (MagImageScalingCallback)doCaptureMagnificationAPI_callback)) {
  ...
  for (nextWindow = GetNextWindow(selectedHwnd, GW_HWNDPREV);
       nextWindow != NULL; nextWindow = GetNextWindow(nextWindow, GW_HWNDPREV))
    if (isWindowIsAboveCaptureRegion(nextWindow, rect)) {
       ...
       */
      if (excludeWinsCount >= 24)
        break;
    }

  if (excludeWinsCount)
    MagSetWindowFilterList(hwndMag, MW_FILTERMODE_EXCLUDE, excludeWinsCount,
                           excludeWins);
 ...
 */
static void highlightWindowFrame(HWND hWnd) {
  HDC hdc;
  RECT rc;
 ...
 * SELECT_WINDOW
 */
void setCaptureType(int capType) { captureType = capType; }
 ...
 */
static HWND myWindowFromPoint(POINT pt) {
  HWND myHwnd;
  HWND nextHwnd;
...
BOOL CALLBACK dialogProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_INITDIALOG: {
    ...
    nonclientHeight =
        (dlgRect.bottom - dlgRect.top) - (clientRect.bottom - clientRect.top);

    /* Calculate the bitmap rectangle */
    bitmapRect.top =
        ((groupRect.top + groupRect.bottom) / 2) - (bm.bmHeight / 2);
    bitmapRect.top -= nonclientHeight;
    bitmapRect.bottom = bitmapRect.top + bm.bmHeight;
    bitmapRect.left =
        ((groupRect.left + groupRect.right) / 2) - (bm.bmWidth / 2);
  ...
  case WM_PAINT: {
    ...
    if (!mouseCaptured) {
      ...
      DrawIconEx(hDC, bitmapRect.left, bitmapRect.top, selectCursor, 0, 0, 0,
                 NULL, DI_NORMAL);
      EndPaint(hwndDlg, &ps);
    }
  } break;
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) {
  ...
  if (User32Library) {
     ...
     */
    SetProcessDPIAware =
        (SET_PROC_DPI_AWARE)GetProcAddress(User32Library, "SetProcessDPIAware");
  ...
  /* Create our window */
  mainHwnd =
      CreateWindow(APP_NAME, APP_NAME, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam,
  ...
  case WM_DOCAPTURE:
    ...
    selectedHwnd = (HWND)DialogBox(hInst, MAKEINTRESOURCE(IDD_SELECT), hwnd,
                                   (DLGPROC)dialogProc);
    if (selectedHwnd)
