...
gboolean screenshot_gnome_shell_available(void) {
  ...
  if (proxy) {
    ...
    g_dbus_proxy_call_sync(proxy, "org.freedesktop.DBus.Peer.Ping", NULL,
...
ScreenshotCapabilities screenshot_gnome_shell_get_capabilities(void) {
  return (SCREENSHOT_CAN_SHOOT_DECORATIONS | SCREENSHOT_CAN_SHOOT_POINTER |
          SCREENSHOT_CAN_SHOOT_REGION | SCREENSHOT_CAN_SHOOT_WINDOW);
}
...
GimpPDBStatusType screenshot_gnome_shell_shoot(ScreenshotValues *shootvals,
  ...
  case SHOOT_ROOT:
    ...
    method = "Screenshot";
    args = g_variant_new("(bbs)", shootvals->show_cursor, TRUE, /* flash */
                         filename);
  ...
  case SHOOT_REGION:
    ...
    g_variant_get(retval, "(iiii)", &shootvals->x1, &shootvals->y1,
                  &shootvals->x2, &shootvals->y2);
    ...
    args = g_variant_new("(iiiibs)", shootvals->x1, shootvals->y1,
                         shootvals->x2 - shootvals->x1,
                         shootvals->y2 - shootvals->y1, TRUE, /* flash */
                         filename);

    monitor = gdk_display_get_monitor_at_point(
        gdk_monitor_get_display(monitor), (shootvals->x1 + shootvals->x2) / 2,
  ...
  case SHOOT_WINDOW:
    ...
    args = g_variant_new("(bbbs)", shootvals->decorate, shootvals->show_cursor,
                         TRUE, /* flash */
                         filename);
  ...
  retval = g_dbus_proxy_call_sync(proxy, method, args, G_DBUS_CALL_FLAGS_NONE,
                                  -1, NULL, error);
  if (!retval)
    goto failure;

  g_variant_get(retval, "(bs)", &success, &filename);
  ...
  if (success && filename) {
    ...
    *image =
        gimp_file_load(GIMP_RUN_NONINTERACTIVE, g_file_new_for_path(filename));
