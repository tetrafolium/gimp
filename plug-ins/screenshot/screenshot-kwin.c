...
gboolean screenshot_kwin_available(void) {
  proxy = g_dbus_proxy_new_for_bus_sync(
      ...
      "org.kde.KWin", "/Screenshot", "org.kde.kwin.Screenshot", NULL, NULL);
...
ScreenshotCapabilities screenshot_kwin_get_capabilities(void) {
  return (SCREENSHOT_CAN_SHOOT_DECORATIONS | SCREENSHOT_CAN_SHOOT_POINTER |
          SCREENSHOT_CAN_SHOOT_WINDOW | SCREENSHOT_CAN_PICK_WINDOW);
...
GimpPDBStatusType screenshot_kwin_shoot(ScreenshotValues *shootvals,
  ...
  case SHOOT_REGION:
    ...
    args = g_variant_new("(iiii)", shootvals->x1, shootvals->y1,
  ...
  case SHOOT_WINDOW:
    ...
    method = "interactive";
    mask =
        (shootvals->decorate ? 1 : 0) | (shootvals->show_cursor ? 1 << 1 : 0);
  ...
  g_variant_get(retval, "(s)", &filename);
