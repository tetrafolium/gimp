...
static guint32 select_window(ScreenshotValues *shootvals, GdkMonitor *monitor);
static GimpImage *create_image(cairo_surface_t *surface, cairo_region_t *shape,
                               const gchar *name);
...
static guint32 select_window(ScreenshotValues *shootvals, GdkMonitor *monitor) {
  ...
  status = XGrabPointer(x_dpy, x_root, False, mask, GrabModeSync, GrabModeAsync,
  ...
  if (gdk_keymap_get_entries_for_keyval(keymap, GDK_KEY_Escape, &keys,
                                        &num_keys)) {
...
#undef X_GRAB_KEY
  ...
  while (!cancel && ((x_win == None) || (buttons != 0))) {
    ...
    case KeyPress: {
      ...
      if (gdk_keymap_get_entries_for_keycode(NULL, x_event.xkey.keycode, NULL,
                                             &keyvals, &n)) {
        gint i;
...
static gchar *window_get_utf8_property(GdkDisplay *display, guint32 window,
                                       const gchar *name) {
  gchar *retval = NULL;
  Atom utf8_string;
  Atom type = None;
  ...
  XGetWindowProperty(GDK_DISPLAY_XDISPLAY(display), window,
                     gdk_x11_get_xatom_by_name_for_display(display, name), 0,
...
static gchar *window_get_title(GdkDisplay *display, guint window) {
#ifdef HAVE_X11_XMU_WINUTIL_H
...
static cairo_region_t *window_get_shape(GdkMonitor *monitor, guint32 window) {
  ...
  rects = XShapeGetRectangles(
      GDK_DISPLAY_XDISPLAY(gdk_monitor_get_display(monitor)), window,
      ShapeBounding, &rect_count, &rect_order);
  ...
  if (rects) {
    if (rect_count > 1) {
      ...
      for (i = 0; i < rect_count; i++) {
        cairo_rectangle_int_t rect = {rects[i].x, rects[i].y, rects[i].width,
                                      rects[i].height};
...
static void image_select_shape(GimpImage *image, cairo_region_t *shape) {
  gint num_rects;
  gint i;
  ...
  for (i = 0; i < num_rects; i++) {
    ...
    gimp_image_select_rectangle(image, GIMP_CHANNEL_OP_ADD, rect.x, rect.y,
...
static GimpImage *create_image(cairo_surface_t *surface, cairo_region_t *shape,
  ...
  layer = gimp_layer_new_from_surface(image, name ? name : _("Screenshot"),
                                      surface, 0.0, 1.0);
...
static void add_cursor_image(GimpImage *image, GdkDisplay *display) {
#ifdef HAVE_XFIXES
  ...
  layer = gimp_layer_new(image, _("Mouse Pointer"), cursor->width,
                         cursor->height, GIMP_RGBA_IMAGE, 100.0,
  ...
  iter = gegl_buffer_iterator_new(
      buffer,
      GEGL_RECTANGLE(0, 0, gimp_drawable_width(GIMP_DRAWABLE(layer)),
                     gimp_drawable_height(GIMP_DRAWABLE(layer))),
      0, babl_format("R'G'B'A u8"), GEGL_ACCESS_READWRITE, GEGL_ABYSS_NONE, 1);
  ...
  gimp_layer_set_offsets(layer, cursor->x - cursor->xhot,
...
gboolean screenshot_x11_available(void) { return TRUE; }
...
ScreenshotCapabilities screenshot_x11_get_capabilities(void) {
  ...
  capabilities |= SCREENSHOT_CAN_SHOOT_REGION | SCREENSHOT_CAN_SHOOT_WINDOW |
                  SCREENSHOT_CAN_PICK_WINDOW | SCREENSHOT_CAN_DELAY_WINDOW_SHOT;
...
GimpPDBStatusType screenshot_x11_shoot(ScreenshotValues *shootvals,
  ...
  if (shootvals->shoot_type == SHOOT_REGION) {
    ...
    monitor = gdk_display_get_monitor_at_point(display, rect.x + rect.width / 2,
    ...
    if (shootvals->shoot_type == SHOOT_ROOT) {
      ...
      window =
          gdk_x11_window_foreign_new_for_display(display, shootvals->window_id);
  ...
  screenshot =
      cairo_image_surface_create(CAIRO_FORMAT_RGB24, rect.width, rect.height);

  cr = cairo_create(screenshot);

  gdk_cairo_set_source_window(cr, window, -(rect.x - screen_rect.x),
