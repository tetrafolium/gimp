...
static GimpImage *ReadImage(FILE *fd, GFile *file, gint width, gint height,
                            guchar cmap[256][3], gint ncols, gint bpp,
                            gint compression, gint rowbytes, gboolean gray,
                            const BitmapChannel *masks, GError **error);

static void setMasksDefault(gushort biBitCnt, BitmapChannel *masks) {
  switch (biBitCnt) {
  case 32:
...
static gint32 ToL(const guchar *buffer) {
  return (buffer[0] | buffer[1] << 8 | buffer[2] << 16 | buffer[3] << 24);
}

static gint16 ToS(const guchar *buffer) { return (buffer[0] | buffer[1] << 8); }

static gboolean ReadColorMap(FILE *fd, guchar buffer[256][3], gint number,
                             gint size, gboolean *gray) {
  gint i;
...
static gboolean ReadChannelMasks(guint32 *tmp, BitmapChannel *masks,
                                 guint channels) {
  gint i;
  ...
  for (i = 0; i < channels; i++) {
    ...
    g_print("Channel %d mask %08x in %d max_val %d\n", i, masks[i].mask,
            masks[i].shiftin, (gint)masks[i].max_value);
#endif
  }
...
GimpImage *load_image(GFile *file, GError **error) {
  ...
  if (!ReadOK(fd, magick, 2) ||
      !(!strncmp(magick, "BA", 2) || !strncmp(magick, "BM", 2) ||
        !strncmp(magick, "IC", 2) || !strncmp(magick, "PT", 2) ||
        !strncmp(magick, "CI", 2) || !strncmp(magick, "CP", 2))) {
  ...
  } else if (bitmap_file_head.biSize == 40) /* Windows 3.x */
  {
    ...
    if (bitmap_head.biCompr == BI_BITFIELDS) {
      ...
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                  ...
                  bitmap_head.biCompr, gimp_file_get_utf8_name(file));
    }
  ...
  if (bitmap_head.biWidth < 0 || bitmap_head.biHeight == G_MININT32) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("'%s' is not a valid BMP file"),
                gimp_file_get_utf8_name(file));
    goto out;
  ...
  if (((guint64)bitmap_head.biWidth) > G_MAXINT32 / bitmap_head.biBitCnt ||
      ((guint64)bitmap_head.biWidth) >
          (G_MAXINT32 / ABS(bitmap_head.biHeight)) / 4) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("'%s' is not a valid BMP file"),
                gimp_file_get_utf8_name(file));
    goto out;
  ...
  image = ReadImage(fd, file, bitmap_head.biWidth, ABS(bitmap_head.biHeight),
                    ColorMap, bitmap_head.biClrUsed, bitmap_head.biBitCnt,
                    bitmap_head.biCompr, rowbytes, gray, masks, error);
...
static GimpImage *ReadImage(FILE *fd, GFile *file, gint width, gint height,
  ...
  layer = gimp_layer_new(image, _("Background"), width, height, image_type, 100,
  ...
  case 32: {
    while (ReadOK(fd, row_buf, rowbytes)) {
      ...
      for (xpos = 0; xpos < width; ++xpos) {
        ...
        *(temp++) = ((px32 & masks[0].mask) >> masks[0].shiftin) * 255.0 /
                        masks[0].max_value +
                    0.5;
        *(temp++) = ((px32 & masks[1].mask) >> masks[1].shiftin) * 255.0 /
                        masks[1].max_value +
                    0.5;
        *(temp++) = ((px32 & masks[2].mask) >> masks[2].shiftin) * 255.0 /
                        masks[2].max_value +
                    0.5;
        if (channels > 3)
          *(temp++) = ((px32 & masks[3].mask) >> masks[3].shiftin) * 255.0 /
                          masks[3].max_value +
                      0.5;
      }
  ...
  case 16: {
    while (ReadOK(fd, row_buf, rowbytes)) {
      ...
      for (xpos = 0; xpos < width; ++xpos) {
        ...
        *(temp++) = ((rgb & masks[0].mask) >> masks[0].shiftin) * 255.0 /
                        masks[0].max_value +
                    0.5;
        *(temp++) = ((rgb & masks[1].mask) >> masks[1].shiftin) * 255.0 /
                        masks[1].max_value +
                    0.5;
        *(temp++) = ((rgb & masks[2].mask) >> masks[2].shiftin) * 255.0 /
                        masks[2].max_value +
                    0.5;
        if (channels > 3)
          *(temp++) = ((rgb & masks[3].mask) >> masks[3].shiftin) * 255.0 /
                          masks[3].max_value +
                      0.5;
      }
    ...
    /* no compression */
    {
      ...
      while (ypos >= 0 && xpos <= width) {
        ...
        /* Count + Color - record */
        {
           ...
           */
          for (j = 0; ((guchar)j < row_buf[0]) && (xpos < width);) {
#ifdef DEBUG2
            ...
            for (i = 1; ((i <= (8 / bpp)) && (xpos < width) &&
                 ...
                 i++, xpos++, j++) {
              temp = dest + (ypos * rowstride) + (xpos * channels);
              *temp = (row_buf[1] & (((1 << bpp) - 1) << (8 - (i * bpp)))) >>
                      (8 - (i * bpp));
              if (gray)
                *temp = cmap[*temp][0];
            }
