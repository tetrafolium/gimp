...
typedef enum { VALUE_PAIR_INT, VALUE_PAIR_DOUBLE } ValuePairType;
...
static GimpValueArray *ifs_run(GimpProcedure *procedure, GimpRunMode run_mode,
...
static gint design_area_draw(GtkWidget *widget, cairo_t *cr);
static gint design_area_button_press(GtkWidget *widget, GdkEventButton *event);
static gint design_area_button_release(GtkWidget *widget,
                                       GdkEventButton *event);
static void design_area_select_all_callback(GtkWidget *widget, gpointer data);
...
static void recompute_center_cb(GtkWidget *widget, gpointer data);

static void ifs_compose(GimpDrawable *drawable);

static ColorMap *color_map_create(const gchar *name, GimpRGB *orig_color,
                                  GimpRGB *data, gboolean fixed_point);
static void color_map_color_changed_cb(GtkWidget *widget, ColorMap *color_map);
...
static void ifs_compose_new_callback(GtkAction *action, gpointer data);
static void ifs_compose_delete_callback(GtkAction *action, gpointer data);
static void ifs_compose_options_callback(GtkAction *action, gpointer data);
static void ifs_compose_load(GtkWidget *parent);
static void ifs_compose_save(GtkWidget *parent);
static void ifs_compose_response(GtkWidget *widget, gint response_id,
                                 gpointer data);
...
static void ifs_class_init(IfsClass *klass) {
...
static GimpProcedure *ifs_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, ifs_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(
        procedure, N_("Create an Iterated Function System (IFS) fractal"),
        ...
        name);
    gimp_procedure_set_attribution(procedure, "Owen Taylor", "Owen Taylor",
                                   "1997");
  }
...
static GimpValueArray *ifs_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_INTERACTIVE:
     ...
     */
    parasite = gimp_item_get_parasite(GIMP_ITEM(drawable), PLUG_IN_PARASITE);
    if (parasite) {
  ...
  if (run_mode == GIMP_RUN_INTERACTIVE) {
    ...
    parasite = gimp_parasite_new(
        PLUG_IN_PARASITE, GIMP_PARASITE_PERSISTENT | GIMP_PARASITE_UNDOABLE,
        strlen(str) + 1, str);
...
static GtkWidget *ifs_compose_color_page(void) {
  ...
  ifsD->hue_scale_pair = value_pair_create(&ifsD->current_vals.hue_scale, 0.0,
                                           1.0, TRUE, VALUE_PAIR_DOUBLE);
...
static gint ifs_compose_dialog(GimpDrawable *drawable) {
  ...
  dialog = gimp_dialog_new(_("IFS Fractal"), PLUG_IN_ROLE, NULL, 0,
                           gimp_standard_help_func, PLUG_IN_PROC,

                           _("_Open"), RESPONSE_OPEN, _("_Save"), RESPONSE_SAVE,
  ...
  gimp_dialog_set_alternative_button_order(
      GTK_DIALOG(dialog), RESPONSE_OPEN, RESPONSE_SAVE, RESPONSE_RESET,
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(ifs_compose_response), NULL);
  g_signal_connect(dialog, "destroy", G_CALLBACK(gtk_main_quit), NULL);

  design_area_create(dialog, design_width, design_height);

  toolbar =
      gtk_ui_manager_get_widget(ifsDesign->ui_manager, "/ifs-compose-toolbar");
  ...
  aspect_frame = gtk_aspect_frame_new(NULL, 0.5, 0.5,
  ...
  gtk_widget_set_size_request(ifsD->preview, ifsD->preview_width,
                              ifsD->preview_height);
  ...
  if (ifsvals.num_elements == 0) {
    ...
    if (ratio != ifsvals.aspect_ratio) {
      ...
      aff2_apply(&t3, ifsvals.center_x, ifsvals.center_y, &center_x, &center_y);
      ...
      for (i = 0; i < ifsvals.num_elements; i++) {
        ...
        aff_element_decompose_trans(elements[i], &elements[i]->trans, 1,
                                    ifsvals.aspect_ratio, center_x, center_y);
      }
...
static void design_area_create(GtkWidget *window, gint design_width,
  ...
  g_signal_connect(ifsDesign->area, "realize", G_CALLBACK(design_area_realize),
                   NULL);
  g_signal_connect(ifsDesign->area, "draw", G_CALLBACK(design_area_draw), NULL);
  g_signal_connect(ifsDesign->area, "button-press-event",
                   G_CALLBACK(design_area_button_press), NULL);
  g_signal_connect(ifsDesign->area, "button-release-event",
                   G_CALLBACK(design_area_button_release), NULL);
  g_signal_connect(ifsDesign->area, "motion-notify-event",
                   G_CALLBACK(design_area_motion), NULL);
  g_signal_connect(ifsDesign->area, "size-allocate",
                   G_CALLBACK(design_area_size_allocate), NULL);
...
static GtkUIManager *design_op_menu_create(GtkWidget *window) {
  static GtkActionEntry actions[] = {
      ...
      {"new", GIMP_ICON_DOCUMENT_NEW, N_("_New"), "<primary>N", NULL,
       G_CALLBACK(ifs_compose_new_callback)},

      {"delete", GIMP_ICON_EDIT_DELETE, N_("_Delete"), "<primary>D", NULL,
       G_CALLBACK(ifs_compose_delete_callback)},

      {"undo", GIMP_ICON_EDIT_UNDO, N_("_Undo"), "<primary>Z", NULL,
       G_CALLBACK(undo)},

      {"redo", GIMP_ICON_EDIT_REDO, N_("_Redo"), "<primary>Y", NULL,
       G_CALLBACK(redo)},

      {"select-all", GIMP_ICON_SELECTION_ALL, N_("Select _All"), "<primary>A",
       NULL, G_CALLBACK(design_area_select_all_callback)},

      {"center", GIMP_ICON_CENTER, N_("Re_center"), "<primary>C",
       N_("Recompute Center"), G_CALLBACK(recompute_center_cb)},

      {"options", GIMP_ICON_PREFERENCES_SYSTEM, N_("Render Options"), NULL,
       NULL, G_CALLBACK(ifs_compose_options_callback)}};
  static GtkRadioActionEntry radio_actions[] = {
      {"move", GIMP_ICON_TOOL_MOVE, N_("Move"), "M", NULL, OP_TRANSLATE},

      {"rotate", GIMP_ICON_TOOL_ROTATE, N_("Rotate"), "R", N_("Rotate / Scale"),
       OP_ROTATE},

      {"stretch", GIMP_ICON_TOOL_PERSPECTIVE, N_("Stretch"), "S", NULL,
       OP_STRETCH}};
  ...
  gtk_action_group_add_radio_actions(
      group, radio_actions, G_N_ELEMENTS(radio_actions), ifsDesign->op,
      G_CALLBACK(design_op_update_callback), window);
...
static void ifs_options_dialog(GtkWidget *parent) {
  if (!ifsOptD) {
    ...
    ifsOptD->dialog =
        gimp_dialog_new(_("IFS Fractal Render Options"), PLUG_IN_ROLE, parent,
    ...
    g_signal_connect(ifsOptD->dialog, "response", G_CALLBACK(gtk_widget_hide),
                     NULL);
    ...
    gtk_box_pack_start(
        GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(ifsOptD->dialog))), grid,
        FALSE, FALSE, 0);
    gtk_widget_show(grid);
    ...
    ifsOptD->memory_pair = value_pair_create(&ifsvals.max_memory, 1, 1000000,
                                             FALSE, VALUE_PAIR_INT);
    ...
    ifsOptD->iterations_pair = value_pair_create(
        &ifsvals.iterations, 1, 10000000, FALSE, VALUE_PAIR_INT);
    gtk_grid_attach(GTK_GRID(grid), ifsOptD->iterations_pair->spin, 1, 1, 1, 1);
    // GTK_FILL, GTK_FILL, 0, 0);
    ...
    ifsOptD->subdivide_pair =
        value_pair_create(&ifsvals.subdivide, 1, 10, FALSE, VALUE_PAIR_INT);
    gtk_grid_attach(GTK_GRID(grid), ifsOptD->subdivide_pair->spin, 1, 2, 1, 1);
    // GTK_FILL, GTK_FILL, 0, 0);
    ...
    ifsOptD->radius_pair =
        value_pair_create(&ifsvals.radius, 0, 5, TRUE, VALUE_PAIR_DOUBLE);
    gtk_grid_attach(GTK_GRID(grid), ifsOptD->radius_pair->scale, 1, 3, 1, 1);
    // GTK_FILL, GTK_FILL, 0, 0);
...
static void ifs_compose(GimpDrawable *drawable) {
  ...
  for (band_no = 0, band_y = 0; band_no < num_bands; band_no++) {
    ...
    gimp_progress_init_printf(_("Rendering IFS (%d/%d)"), band_no + 1,
                              num_bands);
    ...
    ifs_render(elements, ifsvals.num_elements, width, height,
               ifsvals.iterations, &ifsvals, band_y, band_height, data, mask,
               nhits, FALSE);

    /* transfer the image to the drawable */

    iter = gegl_buffer_iterator_new(
        buffer, GEGL_RECTANGLE(0, band_y, width, band_height), 0, format,
      ...
      for (j = roi->y; j < (roi->y + roi->height); j++) {
        ...
        for (i = roi->x; i < (roi->x + roi->width); i++) {
          ...
          for (jj = 0; jj < ifsvals.subdivide; jj++) {
            ...
            ptr = data + 3 * (((j - band_y) * ifsvals.subdivide + jj) *
...
static void update_values(void) {
  ...
  if (elements[ifsD->current_element]->v.simple_color)
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ifsD->simple_button), TRUE);
  else
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(ifsD->full_button), TRUE);
...
static void set_current_element(gint index) {
  gchar *frame_name =
      g_strdup_printf(_("Transformation %s"), elements[index]->name);
...
static void design_area_realize(GtkWidget *widget) {
  ...
  GdkCursor *cursor =
      gdk_cursor_new_for_display(display, cursors[ifsDesign->op]);
  gdk_window_set_cursor(gtk_widget_get_window(widget), cursor);
  g_object_unref(cursor);
}

static gboolean design_area_draw(GtkWidget *widget, cairo_t *cr) {
  cairo_t *design_cr;
  GtkAllocation allocation;
  PangoLayout *layout;
  ...
  for (i = 0; i < ifsvals.num_elements; i++) {
    aff_element_draw(elements[i], element_selected[i], allocation.width,
                     allocation.height, design_cr, &black, layout);
  }
...
static void design_area_size_allocate(GtkWidget *widget,
  ...
  for (i = 0; i < ifsvals.num_elements; i++)
    aff_element_compute_trans(elements[i], allocation->width,
                              allocation->height, ifsvals.center_x,
                              ifsvals.center_y);

  for (i = 0; i < ifsvals.num_elements; i++)
    aff_element_compute_boundary(elements[i], allocation->width,
                                 allocation->height, elements,
                                 ifsvals.num_elements);

  if (ifsDesign->surface)
    cairo_surface_destroy(ifsDesign->surface);

  ifsDesign->surface = cairo_image_surface_create(
...
static gint design_area_button_press(GtkWidget *widget, GdkEventButton *event) {
  ...
  if (gdk_event_triggers_context_menu((GdkEvent *)event)) {
    GtkWidget *menu = gtk_ui_manager_get_widget(
        ifsDesign->ui_manager, "/dummy-menubar/ifs-compose-menu");
...
static gint design_area_motion(GtkWidget *widget, GdkEventMotion *event) {
  ...
  case OP_ROTATE:
    ...
    aff2_scale(&t2, sqrt((SQR(xn) + SQR(yn)) / (SQR(xo) + SQR(yo))), 0);
    aff2_compose(&t3, &t2, &t1);
  ...
  case OP_TRANSLATE:
    aff2_translate(&trans, (xn - xo) * allocation.width,
...
static void undo_begin(void) {
  ...
  undo_ring[new_index].element_selected = g_new(gboolean, ifsvals.num_elements);
...
static void undo_update(gint el) {
  AffElement *elem;
  /* initialize */
  ...
  if (!undo_ring[(undo_start + undo_cur) % UNDO_LEVELS].elements[el])
    undo_ring[(undo_start + undo_cur) % UNDO_LEVELS].elements[el] = elem =
        g_new(AffElement, 1);
...
static void val_changed_update(void) {
  ...
  aff_element_compute_trans(cur, allocation.width, allocation.height,
                            ifsvals.center_x, ifsvals.center_y);
...
static ColorMap *color_map_create(const gchar *name, GimpRGB *orig_color,
  ...
  color_map->orig_preview = gimp_color_area_new(fixed_point ? data : orig_color,
                                                GIMP_COLOR_AREA_FLAT, 0);
  gtk_drag_dest_unset(color_map->orig_preview);
  gtk_widget_set_size_request(color_map->orig_preview, COLOR_SAMPLE_SIZE,
                              COLOR_SAMPLE_SIZE);
  ...
  color_map->button = gimp_color_button_new(
      name, COLOR_SAMPLE_SIZE, COLOR_SAMPLE_SIZE, data, GIMP_COLOR_AREA_FLAT);
  gtk_box_pack_start(GTK_BOX(color_map->hbox), color_map->button, FALSE, FALSE,
                     0);
  gtk_widget_show(color_map->button);

  g_signal_connect(color_map->button, "color-changed",
                   G_CALLBACK(gimp_color_button_get_color), data);

  g_signal_connect(color_map->button, "color-changed",
                   G_CALLBACK(color_map_color_changed_cb), color_map);
...
static ValuePair *value_pair_create(gpointer data, gdouble lower, gdouble upper,
  ...
  value_pair->adjustment = gtk_adjustment_new(
      1.0, lower, upper, (upper - lower) / 100, (upper - lower) / 10, 0.0);
  ...
  g_signal_connect(value_pair->adjustment, "value-changed",
                   G_CALLBACK(value_pair_scale_callback), value_pair);

  if (create_scale) {
    value_pair->scale =
        gtk_scale_new(GTK_ORIENTATION_HORIZONTAL, value_pair->adjustment);
...
static gboolean value_pair_scale_callback_real(gpointer data) {
...
static void value_pair_scale_callback(GtkAdjustment *adjustment,
  ...
  value_pair->timeout_id =
      g_timeout_add(500, /* update every half second */
                    value_pair_scale_callback_real, value_pair);
}
...
static void recompute_center(gboolean save_undo) {
  ...
  for (i = 0; i < ifsvals.num_elements; i++) {
    aff_element_decompose_trans(elements[i], &elements[i]->trans, 1,
                                ifsvals.aspect_ratio, ifsvals.center_x,
                                ifsvals.center_y);
  }
...
static void ifs_compose_set_defaults(void) {
  ...
  element_selected =
      g_realloc(element_selected, ifsvals.num_elements * sizeof(gboolean));
...
static void ifscompose_message_dialog(GtkMessageType type, GtkWindow *parent,
                                      const gchar *title,
                                      const gchar *message) {
  GtkWidget *dialog;

  dialog =
      gtk_message_dialog_new(parent, 0, type, GTK_BUTTONS_OK, "%s", message);
...
/* save an ifs file */
static void ifsfile_save_response(GtkWidget *dialog, gint response_id,
                                  gpointer data) {
...
static void ifsfile_replace_ifsvals(IfsComposeVals *new_ifsvals,
  ...
  for (i = 0; i < ifsvals.num_elements; i++) {
    aff_element_compute_trans(elements[i], allocation.width, allocation.height,
                              ifsvals.center_x, ifsvals.center_y);
    aff_element_compute_color_trans(elements[i]);
  }
...
/* load an ifs file */
static void ifsfile_load_response(GtkWidget *dialog, gint response_id,
                                  gpointer data) {
  if (response_id == GTK_RESPONSE_OK) {
    gchar *filename;
    gchar *buffer;
...
static void ifs_compose_save(GtkWidget *parent) {
  ...
  if (!dialog) {
    dialog = gtk_file_chooser_dialog_new(
        _("Save as IFS Fractal file"), GTK_WINDOW(parent),
    ...
    g_signal_connect(dialog, "destroy", G_CALLBACK(gtk_widget_destroyed),
                     &dialog);
    g_signal_connect(dialog, "response", G_CALLBACK(ifsfile_save_response),
                     NULL);
  }
...
static void ifs_compose_load(GtkWidget *parent) {
  ...
  if (!dialog) {
    dialog = gtk_file_chooser_dialog_new(
        _("Open IFS Fractal file"), GTK_WINDOW(parent),
    ...
    g_signal_connect(dialog, "destroy", G_CALLBACK(gtk_widget_destroyed),
                     &dialog);
    g_signal_connect(dialog, "response", G_CALLBACK(ifsfile_load_response),
                     NULL);
  }
...
static void ifs_compose_new_callback(GtkAction *action, gpointer data) {
  ...
  ifsD->selected_orig =
      g_realloc(ifsD->selected_orig, ifsvals.num_elements * sizeof(AffElement));
  aff_element_compute_trans(elem, allocation.width, allocation.height,
                            ifsvals.center_x, ifsvals.center_y);
...
static gint preview_idle_render(gpointer data) {
  ...
  ifs_render(elements, ifsvals.num_elements, allocation.width,
             allocation.height, iterations, &ifsvals, 0, allocation.height,
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(ifsD->preview), 0, 0,
                         allocation.width, allocation.height, GIMP_RGB_IMAGE,
                         ifsD->preview_data, allocation.width * 3);
