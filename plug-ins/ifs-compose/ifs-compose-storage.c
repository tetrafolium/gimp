...
} symbols[] = {{"iterations", TOKEN_ITERATIONS},
...
static GTokenType ifsvals_parse_element(GScanner *scanner,
...
static gboolean ifsvals_parse(GScanner *scanner, IfsComposeVals *vals,
                              AffElement ***elements) {
...
gchar *ifsvals_stringify(IfsComposeVals *vals, AffElement **elements) {
  ...
  for (i = 0; i < vals->num_elements; i++) {
    ...
    g_ascii_dtostr(cbuf[0], G_ASCII_DTOSTR_BUF_SIZE,
                   elements[i]->v.red_color.r);
    g_ascii_dtostr(cbuf[1], G_ASCII_DTOSTR_BUF_SIZE,
                   elements[i]->v.red_color.g);
    g_ascii_dtostr(cbuf[2], G_ASCII_DTOSTR_BUF_SIZE,
