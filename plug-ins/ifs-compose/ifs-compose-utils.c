...
static void aff_element_compute_click_boundary(AffElement *elem,
                                               ...
                                               gdouble *points_x,
                                               gdouble *points_y);
static guchar *create_brush(IfsComposeVals *ifsvals, gint *brush_size,
...
void aff2_scale(Aff2 *naff, gdouble s, gboolean flip) {
  if (flip)
    naff->a11 = -s;
  else
    naff->a11 = s;
...
void aff2_compute_stretch(Aff2 *naff, gdouble xo, gdouble yo, gdouble xn,
                          gdouble yn) {
...
void aff2_apply(Aff2 *aff, gdouble x, gdouble y, gdouble *xf, gdouble *yf) {
...
void aff3_apply(Aff3 *t, gdouble x, gdouble y, gdouble z, gdouble *xf,
                gdouble *yf, gdouble *zf) {
  gdouble xt = (t->vals[0][0] * x + t->vals[0][1] * y + t->vals[0][2] * z +
                t->vals[0][3]);
  gdouble yt = (t->vals[1][0] * x + t->vals[1][1] * y + t->vals[1][2] * z +
                t->vals[1][3]);
  gdouble zt = (t->vals[2][0] * x + t->vals[2][1] * y + t->vals[2][2] * z +
                t->vals[2][3]);
...
static int ipolygon_sort_func(const void *a, const void *b) {
...
void aff_element_compute_color_trans(AffElement *elem) {
  ...
  if (elem->v.simple_color) {
    ...
    if (mag2 == 0)
      ...
      /*  red  */
      for (j = 0; j < 3; j++) {
        elem->color_trans.vals[0][j] =
            elem->v.target_color.r / mag2 *
            (elem->v.value_scale - elem->v.hue_scale);
      }

      /*  green  */
      for (j = 0; j < 3; j++) {
        elem->color_trans.vals[1][j] =
            elem->v.target_color.g / mag2 *
            (elem->v.value_scale - elem->v.hue_scale);
      }

      /*  blue  */
      for (j = 0; j < 3; j++) {
        elem->color_trans.vals[2][j] =
            elem->v.target_color.g / mag2 *
            (elem->v.value_scale - elem->v.hue_scale);
      }
    ...
    aff3_apply(&elem->color_trans, 1.0, 0.0, 0.0, &elem->v.red_color.r,
               &elem->v.red_color.g, &elem->v.red_color.b);
    aff3_apply(&elem->color_trans, 0.0, 1.0, 0.0, &elem->v.green_color.r,
               &elem->v.green_color.g, &elem->v.green_color.b);
    aff3_apply(&elem->color_trans, 0.0, 0.0, 1.0, &elem->v.blue_color.r,
               &elem->v.blue_color.g, &elem->v.blue_color.b);
    aff3_apply(&elem->color_trans, 0.0, 0.0, 0.0, &elem->v.black_color.r,
               &elem->v.black_color.g, &elem->v.black_color.b);
  } else {
    elem->color_trans.vals[0][0] = elem->v.red_color.r - elem->v.black_color.r;
    elem->color_trans.vals[1][0] = elem->v.red_color.g - elem->v.black_color.g;
    elem->color_trans.vals[2][0] = elem->v.red_color.b - elem->v.black_color.b;
    ...
    elem->color_trans.vals[0][2] = elem->v.blue_color.r - elem->v.black_color.r;
    elem->color_trans.vals[1][2] = elem->v.blue_color.g - elem->v.black_color.g;
    elem->color_trans.vals[2][2] = elem->v.blue_color.b - elem->v.black_color.b;
...
void aff_element_compute_trans(AffElement *elem, gdouble width, gdouble height,
...
void aff_element_decompose_trans(AffElement *elem, Aff2 *aff, gdouble width,
...
void aff_element_compute_boundary(AffElement *elem, gint width, gint height,
                                  AffElement **elements, gint num_elements) {
  gint i;
  IPolygon tmp_poly;
  gdouble *points_x;
  gdouble *points_y;
...
void aff_element_draw(AffElement *elem, gboolean selected, gint width,
                      gint height, cairo_t *cr, GdkRGBA *color,
                      PangoLayout *layout) {
  PangoRectangle rect;
  gint i;
  ...
  cairo_move_to(cr, elem->v.x * width - rect.width / 2,
  ...
  if (elem->click_boundary != elem->draw_boundary) {
    cairo_move_to(cr, elem->click_boundary->points[0].x,
                  elem->click_boundary->points[0].y);

    for (i = 1; i < elem->click_boundary->npoints; i++)
      cairo_line_to(cr, elem->click_boundary->points[i].x,
  ...
  cairo_move_to(cr, elem->draw_boundary->points[0].x,
                elem->draw_boundary->points[0].y);

  for (i = 1; i < elem->draw_boundary->npoints; i++)
    cairo_line_to(cr, elem->draw_boundary->points[i].x,
...
void ifs_render(AffElement **elements, gint num_elements, gint width,
                gint height, gint nsteps, IfsComposeVals *vals, gint band_y,
                gint band_height, guchar *data, guchar *mask, guchar *nhits,
                gboolean preview) {
  gint i, k, n;
  gdouble x, y;
  gdouble r, g, b;
  ...
  for (i = 0; i < num_elements; i++) {
    aff_element_compute_trans(elements[i], width * subdivide,
                              height * subdivide, vals->center_x,
                              vals->center_y);
    fprob[i] = fabs(elements[i]->trans.a11 * elements[i]->trans.a22 -
  ...
  for (i = 0; i < nsteps; i++) {
    ...
    if ((ri < 0) || (ri > 255) || (gi < 0) || (gi > 255) || (bi < 0) ||
        (bi > 255))
      continue;

    if (preview) {
      if ((x < width) && (y < (band_y + band_height)) && (x >= 0) &&
          (y >= band_y)) {
      ...
      if ((x < width * subdivide) && (y < height * subdivide) && (x >= 0) &&
          (y >= 0)) {
        gint ii;
        gint jj;
