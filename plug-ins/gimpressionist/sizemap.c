...
static void updatesmpreviewprev(void) {
  ...
  for (y = 6; y < OMHEIGHT - 4; y += 10) {
    for (x = 6; x < OMWIDTH - 4; x += 10) {
      gdouble siz =
          5 * getsiz_from_gui(x / (double)OMWIDTH, y / (double)OMHEIGHT);
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(smpreviewprev), 0, 0, OMWIDTH,
                         OMHEIGHT, GIMP_RGB_IMAGE, nsbuffer.col, OMWIDTH * 3);
}
...
static void updatesmvectorprev(void) {
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(smvectorprev), 0, 0, OMWIDTH,
                         OMHEIGHT, GIMP_RGB_IMAGE,
                         update_vector_preview_sbuffer.col, OMWIDTH * 3);
...
static void updatesmsliders(void) {
  smadjignore = TRUE;
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(sizadjust),
                            smvector[selectedsmvector].siz);
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(smstradjust),
...
static void angsmadjmove(GtkWidget *w, gpointer data) {
  if (!smadjignore) {
    smvector[selectedsmvector].siz =
        gimp_label_spin_get_value(GIMP_LABEL_SPIN(sizadjust));
    updatesmvectorprev();
    updatesmpreviewprev();
  }
}
...
static void strsmadjmove(GtkWidget *w, gpointer data) {
  if (!smadjignore) {
    smvector[selectedsmvector].str =
        gimp_label_spin_get_value(GIMP_LABEL_SPIN(smstradjust));
    updatesmvectorprev();
    updatesmpreviewprev();
  }
}
...
static void smresponse(GtkWidget *widget, gint response_id, gpointer data) {
  switch (response_id) {
  case RESPONSE_APPLY:
...
void create_sizemap_dialog(GtkWidget *parent) {
  ...
  smwindow = gimp_dialog_new(_("Size Map Editor"), PLUG_IN_ROLE,
                             ...
                             _("_Apply"), RESPONSE_APPLY, _("_Cancel"),
                             GTK_RESPONSE_CANCEL, _("_OK"), GTK_RESPONSE_OK,

                             NULL);

  gimp_dialog_set_alternative_button_order(GTK_DIALOG(smwindow),
                                           GTK_RESPONSE_OK, RESPONSE_APPLY,
                                           GTK_RESPONSE_CANCEL, -1);

  g_signal_connect(smwindow, "response", G_CALLBACK(smresponse), NULL);
  g_signal_connect(smwindow, "destroy", G_CALLBACK(gtk_widget_destroyed),
                   &smwindow);
  ...
  gimp_help_set_help_data(
      tmpw,
      _("The smvector-field. Left-click to move selected smvector, Right-click "
        "to point it towards mouse, Middle-click to add a new smvector."),
      NULL);
  ...
  g_signal_connect(tmpw2, "button-press-event", G_CALLBACK(smmapclick), NULL);
  ...
  sizadjust = gimp_scale_entry_new(_("_Size:"), 50.0, 0.0, 100.0, 1);
  gimp_help_set_help_data(sizadjust,
                          _("Change the angle of the selected smvector"), NULL);
  g_signal_connect(sizadjust, "value-changed", G_CALLBACK(angsmadjmove), NULL);
  gtk_grid_attach(GTK_GRID(grid2), sizadjust, 0, 0, 3, 1);
  gtk_widget_show(sizadjust);

  smstradjust = gimp_scale_entry_new(_("S_trength:"), 1.0, 0.1, 5.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(smstradjust), 0.1, 0.5);
  gimp_help_set_help_data(
      smstradjust, _("Change the strength of the selected smvector"), NULL);
  g_signal_connect(smstradjust, "value-changed", G_CALLBACK(strsmadjmove),
                   NULL);
  ...
  g_signal_connect(tmpw, "clicked", G_CALLBACK(smstrexpsmadjmove), NULL);
  gimp_help_set_help_data(tmpw,
                          _("Voronoi-mode makes only the smvector closest to "
                            "the given point have any influence"),
                          NULL);
