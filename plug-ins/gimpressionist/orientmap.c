...
double get_direction(double x, double y, int from) {
  ...
  if (from == 0) {
    ...
    strexp =
        gimp_label_spin_get_value(GIMP_LABEL_SPIN(orient_map_str_exp_adjust));
...
static void update_orient_map_preview_prev(void) {
  ...
  for (y = 6; y < OMHEIGHT - 4; y += 10)
    for (x = 6; x < OMWIDTH - 4; x += 10) {
      double dir = gimp_deg_to_rad(
          get_direction(x / (double)OMWIDTH, y / (double)OMHEIGHT, 0));
      double xo = sin(dir) * 4.0;
      double yo = cos(dir) * 4.0;
      ppm_drawline(&update_om_preview_nbuffer, x - xo, y - yo, x + xo, y + yo,
                   gray);
      ppm_put_rgb(&update_om_preview_nbuffer, x - xo, y - yo, white);
    }

  gimp_preview_area_draw(GIMP_PREVIEW_AREA(orient_map_preview_prev), 0, 0,
                         OMWIDTH, OMHEIGHT, GIMP_RGB_IMAGE,
                         (guchar *)update_om_preview_nbuffer.col, OMWIDTH * 3);
...
static void update_vector_prev(void) {
  ...
  if (vector_preview_brightness_adjust)
    val = 1.0 -
          gtk_adjustment_get_value(vector_preview_brightness_adjust) / 100.0;
  else
    val = 0.5;
  ...
  gimp_preview_area_draw(
      GIMP_PREVIEW_AREA(vector_preview), 0, 0, OMWIDTH, OMHEIGHT,
      GIMP_RGB_IMAGE, (guchar *)update_vector_preview_buffer.col, OMWIDTH * 3);
}
...
static void update_slides(void) {
  ...
  adjignore = TRUE;
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(angle_adjust),
                            vector[selectedvector].dir);
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(strength_adjust),
...
static void angle_adjust_move_callback(GtkWidget *w, gpointer data) {
  if (adjignore)
    return;
  vector[selectedvector].dir =
      gimp_label_spin_get_value(GIMP_LABEL_SPIN(angle_adjust));
...
static void strength_adjust_move_callback(GtkWidget *w, gpointer data) {
  if (adjignore)
    return;
  vector[selectedvector].str =
      gimp_label_spin_get_value(GIMP_LABEL_SPIN(strength_adjust));
  update_vector_prev();
  update_orient_map_preview_prev();
}

static void strength_exponent_adjust_move_callback(GtkWidget *w,
                                                   gpointer data) {
  if (adjignore)
    return;
  update_vector_prev();
...
static void orient_map_response(GtkWidget *widget, gint response_id,
                                gpointer data) {
  switch (response_id) {
  ...
  case GTK_RESPONSE_OK: {
    ...
    pcvals.orient_angle_offset =
        gimp_label_spin_get_value(GIMP_LABEL_SPIN(angle_offset_adjust));
    pcvals.orient_voronoi =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(orient_voronoi));
  }
  };
...
void create_orientmap_dialog(GtkWidget *parent) {
  ...
  orient_map_window = gimp_dialog_new(
      _("Orientation Map Editor"), PLUG_IN_ROLE,
      gtk_widget_get_toplevel(parent), 0, gimp_standard_help_func, PLUG_IN_PROC,

      _("_Apply"), GTK_RESPONSE_APPLY, _("_Cancel"), GTK_RESPONSE_CANCEL,
  ...
  g_signal_connect(orient_map_window, "response",
                   G_CALLBACK(orient_map_response), orient_map_window);
  g_signal_connect(orient_map_window, "destroy",
                   G_CALLBACK(gtk_widget_destroyed), &orient_map_window);

  grid1 = gtk_grid_new();
  gtk_container_set_border_width(GTK_CONTAINER(grid1), 6);
  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(orient_map_window))),
      grid1, TRUE, TRUE, 0);
  ...
  g_signal_connect(ebox, "button-press-event", G_CALLBACK(map_click_callback),
                   NULL);
  ...
  tmpw =
      gtk_scale_new(GTK_ORIENTATION_VERTICAL, vector_preview_brightness_adjust);
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Type"), G_CALLBACK(vector_type_click_callback), &vector_type,
      NULL, 0,

      _("_Normal"), 0, &vector_types[0], _("Vorte_x"), 1, &vector_types[1],
      _("Vortex_2"), 2, &vector_types[2], _("Vortex_3"), 3, &vector_types[3],
  ...
  gimp_help_set_help_data(tmpw,
                          _("Voronoi-mode makes only the vector closest to the "
                            "given point have any influence"),
                          NULL);
  ...
  angle_adjust = gimp_scale_entry_new(_("A_ngle:"), 0.0, 0.0, 360.0, 1);
  gimp_help_set_help_data(angle_adjust,
                          _("Change the angle of the selected vector"), NULL);
  ...
  gimp_help_set_help_data(angle_offset_adjust,
                          _("Offset all vectors with a given angle"), NULL);
  ...
  strength_adjust = gimp_scale_entry_new(_("_Strength:"), 1.0, 0.1, 5.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(strength_adjust), 0.1, 1.0);
  gimp_help_set_help_data(
      strength_adjust, _("Change the strength of the selected vector"), NULL);
  ...
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(orient_map_str_exp_adjust),
                                 0.1, 1.0);
  gimp_help_set_help_data(orient_map_str_exp_adjust,
                          _("Change the exponent of the strength"), NULL);
