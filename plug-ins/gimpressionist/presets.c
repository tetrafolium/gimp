...
static gboolean can_delete_preset(const gchar *abs) {
  ...
  user_data_dir = g_strconcat(gimp_directory(), G_DIR_SEPARATOR_S, NULL);
...
static gchar *get_early_line_from_preset(gchar *full_path,
                                         const gchar *prefix) {
  FILE *f;
  gchar line[4096];
...
static gchar *get_object_name(const gchar *dir, gchar *filename,
                              void *context) {
...
static void preset_read_dir_into_list(void) {
  readdirintolist_extended("Presets", presetlist, NULL, TRUE, get_object_name,
                           NULL);
}

static gchar *preset_create_filename(const gchar *basename,
...
static void apply_preset(GtkWidget *w, GtkTreeSelection *selection) {
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, PRESETS_LIST_COLUMN_FILENAME, &preset, -1);
...
static void save_preset_response(GtkWidget *widget, gint response_id,
                                 gpointer data) {
...
static void create_save_preset(GtkWidget *parent) {
  ...
  window = gimp_dialog_new(
      _("Save Current"), PLUG_IN_ROLE, gtk_widget_get_toplevel(parent), 0,
  ...
  gimp_dialog_set_alternative_button_order(GTK_DIALOG(window), GTK_RESPONSE_OK,
                                           GTK_RESPONSE_CANCEL, -1);

  g_signal_connect(window, "response", G_CALLBACK(save_preset_response), NULL);
  ...
  g_signal_connect(buffer, "changed", G_CALLBACK(preset_desc_callback), NULL);
...
static void save_preset(void) {
   ...
   */
  presets_dir_path =
      g_build_filename((const gchar *)thispath->data, "Presets", NULL);

  if (!g_file_test(presets_dir_path, G_FILE_TEST_IS_DIR)) {
    if (g_mkdir(presets_dir_path, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP |
  ...
  if (selected_preset_orig_name &&
    ...
    fname = g_build_filename(presets_dir_path, selected_preset_filename, NULL);
  } else {
  ...
  for (i = 0; i < pcvals.num_orient_vectors; i++) {
    g_ascii_dtostr(vbuf[0], G_ASCII_DTOSTR_BUF_SIZE,
                   pcvals.orient_vectors[i].x);
    g_ascii_dtostr(vbuf[1], G_ASCII_DTOSTR_BUF_SIZE,
                   pcvals.orient_vectors[i].y);
    g_ascii_dtostr(vbuf[2], G_ASCII_DTOSTR_BUF_SIZE,
                   pcvals.orient_vectors[i].dir);
    g_ascii_dtostr(vbuf[3], G_ASCII_DTOSTR_BUF_SIZE,
                   pcvals.orient_vectors[i].dx);
    g_ascii_dtostr(vbuf[4], G_ASCII_DTOSTR_BUF_SIZE,
                   pcvals.orient_vectors[i].dy);
    g_ascii_dtostr(vbuf[5], G_ASCII_DTOSTR_BUF_SIZE,
                   pcvals.orient_vectors[i].str);

    fprintf(f, "orientvector=%d,%s,%s,%s,%s,%s,%s,%d\n", i, vbuf[0], vbuf[1],
            vbuf[2], vbuf[3], vbuf[4], vbuf[5], pcvals.orient_vectors[i].type);
  }

  fprintf(
      f, "orientangoff=%s\n",
      g_ascii_dtostr(buf, G_ASCII_DTOSTR_BUF_SIZE, pcvals.orient_angle_offset));
  ...
  for (i = 0; i < pcvals.num_size_vectors; i++) {
    ...
    g_ascii_dtostr(vbuf[2], G_ASCII_DTOSTR_BUF_SIZE,
                   pcvals.size_vectors[i].siz);
    g_ascii_dtostr(vbuf[3], G_ASCII_DTOSTR_BUF_SIZE,
                   pcvals.size_vectors[i].str);
    fprintf(f, "sizevector=%d,%s,%s,%s,%s\n", i, vbuf[0], vbuf[1], vbuf[2],
            vbuf[3]);
  }
...
static GtkWidget *create_presets_list(GtkWidget *parent) {
  GtkListStore *store;
  ...
  column = gtk_tree_view_column_new_with_attributes(
      "Preset", renderer, "text", PRESETS_LIST_COLUMN_OBJECT_NAME, NULL);
  ...
  g_signal_connect(selection, "changed", G_CALLBACK(presets_list_select_preset),
                   NULL);
...
void create_presetpage(GtkNotebook *notebook) {
  ...
  gimp_help_set_help_data(tmpw, _("Reads the selected Preset into memory"),
                          NULL);
  ...
  gimp_label_set_attributes(GTK_LABEL(tmpw), PANGO_ATTR_STYLE,
