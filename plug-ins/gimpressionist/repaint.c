...
static int choose_best_brush(ppm_t *p, ppm_t *a, int tx, int ty, ppm_t *brushes,
                             int num_brushes, double *brushes_sum, int start,
                             int step) {
  double dev, thissum;
  double bestdev = 0.0;
  double r, g, b;
...
static void apply_brush(ppm_t *brush, ppm_t *shadow, ppm_t *p, ppm_t *a, int tx,
                        int ty, int r, int g, int b) {
  ppm_t tmp;
  ppm_t atmp;
  double v, h;
  int x, y;
...
void repaint(ppm_t *p, ppm_t *a) {
  ...
  resize(&brushes[0], brushes[0].width,
  ...
  for (i = 0; i < runningvals.size_num; i++) {
    ...
    for (j = 0; j < runningvals.orient_num; j++) {
      ...
      rescale(&brushes[h], (sv * runningvals.size_first +
                            (1.0 - sv) * runningvals.size_last) /
  ...
  if (dropshadow) {
    for (i = 0; i < num_brushes; i++) {
      ...
      ppm_pad(&shadows[i], shadowblur * 2, shadowblur * 2, shadowblur * 2,
              shadowblur * 2, back);
  ...
  if (runningvals.general_background_type == BG_TYPE_SOLID) {
    ...
    dy =
        runningvals.general_paint_edges ? paper_ppm.height - maxbrushheight : 0;
    ...
    for (y = 0; y < tmp.height; y++) {
      ...
      for (x = 0; x < tmp.width; x += lx) {
        ...
        memcpy(&tmp.col[y * tmp.width * 3 + x * 3],
               &paper_ppm.col[ry * paper_ppm.width * 3 + rx * 3], 3 * lx);
      }
    }
  }
  ...
  case ORIENTATION_FLOWING:
    ...
    blur(&dirmap, 2, 2);
    if (runningvals.general_paint_edges)
      edgepad(&dirmap, maxbrushwidth, maxbrushheight, maxbrushwidth,
              maxbrushheight);
    break;
  ...
  case ORIENTATION_MANUAL:
    ppm_new(&dirmap, p->width - maxbrushwidth * 2,
            p->height - maxbrushheight * 2);
    for (y = 0; y < dirmap.height; y++) {
      guchar *dstrow = &dirmap.col[y * dirmap.width * 3];
      double tmpy = y / (double)dirmap.height;
      for (x = 0; x < dirmap.width; x++) {
        dstrow[x * 3] = get_pixel_value(
            90 - get_direction(x / (double)dirmap.width, tmpy, 1));
      }
    }
    edgepad(&dirmap, maxbrushwidth, maxbrushwidth, maxbrushheight,
            maxbrushheight);
    break;
  }
  ...
  if (runningvals.size_type == SIZE_TYPE_VALUE) {
    ...
    blur(&sizmap, 2, 2);
    if (runningvals.general_paint_edges)
      edgepad(&sizmap, maxbrushwidth, maxbrushheight, maxbrushwidth,
              maxbrushheight);
    ...
    ppm_new(&sizmap, p->width - maxbrushwidth * 2,
            p->height - maxbrushheight * 2);
    ...
    for (y = 0; y < sizmap.height; y++) {
      ...
      for (x = 0; x < sizmap.width; x++) {
        dstrow[x * 3] =
            255 * (1.0 - get_siz_from_pcvals(x / (double)sizmap.width, tmpy));
      }
    }
    edgepad(&sizmap, maxbrushwidth, maxbrushwidth, maxbrushheight,
            maxbrushheight);
  }
#if 0
  ...
  for (; i; i--) {
    ...
    if (i % progstep == 0) {
      if (runningvals.run) {
        ...
        g_snprintf(tmps, sizeof(tmps), "%.1f %%",
                   100 * (1.0 - ((double)i / max_progress)));
    ...
    case ORIENTATION_HUE:
    case ORIENTATION_MANUAL:
      on = runningvals.orient_num * dirmap.col[ty * dirmap.width * 3 + tx * 3] /
           256;
      break;
    ...
    case SIZE_TYPE_MANUAL:
      sn = runningvals.size_num * sizmap.col[ty * sizmap.width * 3 + tx * 3] /
           256;
      break;
    ...
    if (runningvals.orient_type == ORIENTATION_ADAPTIVE) {
      if (runningvals.size_type == SIZE_TYPE_ADAPTIVE)
        n = choose_best_brush(p, a, tx - maxbrushwidth / 2,
                              ty - maxbrushheight / 2, brushes, num_brushes,
                              brushes_sum, 0, 1);
      else {
      ...
      if (runningvals.size_type == SIZE_TYPE_ADAPTIVE)
        n = choose_best_brush(p, a, tx - maxbrushwidth / 2,
                              ty - maxbrushheight / 2, brushes, num_brushes,
                              brushes_sum, on, runningvals.orient_num);
      else
...
#undef MYASSIGN
    ...
    if (runningvals.general_tileable && runningvals.general_paint_edges) {
      ...
      if (doy) {
        if (dox < 0)
          apply_brush(brush, shadow, &tmp, &atmp, tx + orig_width,
                      ty + doy * orig_height, r, g, b);
        if (dox > 0)
          apply_brush(brush, shadow, &tmp, &atmp, tx - orig_width,
                      ty + doy * orig_height, r, g, b);
      }
  ...
  if (runningvals.general_paint_edges) {
    crop(&tmp, maxbrushwidth, maxbrushheight, tmp.width - maxbrushwidth,
         tmp.height - maxbrushheight);
    if (img_has_alpha)
      crop(&atmp, maxbrushwidth, maxbrushheight, atmp.width - maxbrushwidth,
           atmp.height - maxbrushheight);
  }
