...
void preview_set_button_label(const gchar *text) {
  g_object_set(previewbutton, "label", text, "use-underline", TRUE, NULL);
}
...
void updatepreview(GtkWidget *wg, gpointer d) {
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(preview), 0, 0, PREVIEWSIZE,
                         PREVIEWSIZE, GIMP_RGB_IMAGE, preview_ppm.col,
                         PREVIEWSIZE * 3);
...
static void preview_size_allocate(GtkWidget *preview) {
...
GtkWidget *create_preview(void) {
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(updatepreview), (gpointer)1);
  gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
  gtk_widget_show(button);
  gimp_help_set_help_data(button, _("Refresh the Preview window"), NULL);

  button = gtk_button_new_with_mnemonic(_("_Reset"));
  g_signal_connect(button, "clicked", G_CALLBACK(updatepreview), (gpointer)2);
  gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
  gtk_widget_show(button);
  gimp_help_set_help_data(button, _("Revert to the original image"), NULL);
