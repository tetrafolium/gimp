...
static int normalize_bg(int n) { return (!img_has_alpha && (n == 3)) ? 1 : n; }
...
void general_store(void) {
  pcvals.general_paint_edges =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(general_paint_edges));
  pcvals.general_dark_edge =
      gimp_label_spin_get_value(GIMP_LABEL_SPIN(general_dark_edge_scale));
  pcvals.general_tileable =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(general_tileable));
  pcvals.general_drop_shadow =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(general_drop_shadow));
  pcvals.general_shadow_darkness =
      gimp_label_spin_get_value(GIMP_LABEL_SPIN(general_shadow_scale));
  pcvals.general_shadow_depth =
      gimp_label_spin_get_value(GIMP_LABEL_SPIN(general_shadow_depth));
  pcvals.general_shadow_blur =
      gimp_label_spin_get_value(GIMP_LABEL_SPIN(general_shadow_blur));
  pcvals.devthresh =
      gimp_label_spin_get_value(GIMP_LABEL_SPIN(dev_thresh_scale));
}

int general_bg_type_input(int in) { return CLAMP_UP_TO(in, NUMGENERALBGRADIO); }

void general_restore(void) {
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(
          general_bg_radio[normalize_bg(pcvals.general_background_type)]),
      TRUE);
...
static void select_color(GtkWidget *widget, gpointer data) {
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(general_bg_radio[BG_TYPE_SOLID]), TRUE);
}
...
static GtkWidget *create_general_button(GtkWidget *box, int idx,
                                        ...
                                        GSList **radio_group) {
  return create_radio_button(box, idx, general_bg_callback, label, help_string,
                             radio_group, general_bg_radio);
}
...
void create_generalpage(GtkNotebook *notebook) {
  ...
  create_general_button(
      box3, BG_TYPE_FROM_PAPER, _("From paper"),
  ...
  create_general_button(box4, BG_TYPE_SOLID, _("Solid"),
                        _("Solid colored background"), &radio_group);

  general_color_button =
      gimp_color_button_new(_("Color"), COLORBUTTONWIDTH, COLORBUTTONHEIGHT,
                            &pcvals.color, GIMP_COLOR_AREA_FLAT);
  g_signal_connect(general_color_button, "clicked", G_CALLBACK(select_color),
                   NULL);
  g_signal_connect(general_color_button, "color-changed",
                   G_CALLBACK(gimp_color_button_get_color), &pcvals.color);
  gtk_box_pack_start(GTK_BOX(box4), general_color_button, FALSE, FALSE, 0);
  gtk_widget_show(general_color_button);

  tmpw = create_general_button(box3, BG_TYPE_TRANSPARENT, _("Transparent"),
                               _("Use a transparent background; Only the "
                                 "strokes painted will be visible"),
                               &radio_group);
  ...
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(general_bg_radio[pcvals.general_background_type]),
      TRUE);
  ...
  gimp_help_set_help_data(tmpw,
                          _("Selects if to place strokes all the way out to "
  ...
  gimp_help_set_help_data(
      tmpw, _("Selects if the resulting image should be seamlessly tileable"),
      NULL);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(tmpw),
                               pcvals.general_tileable);
  ...
  gtk_box_pack_start(GTK_BOX(box2), tmpw, FALSE, FALSE, 0);
  gtk_widget_show(tmpw);
  gimp_help_set_help_data(tmpw, _("Adds a shadow effect to each brush stroke"),
                          NULL);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(tmpw),
                               pcvals.general_drop_shadow);
  ...
  general_dark_edge_scale = gimp_scale_entry_new(
      _("Edge darken:"), pcvals.general_dark_edge, 0.0, 1.0, 2);
  gimp_help_set_help_data(
      general_dark_edge_scale,
      _("How much to \"darken\" the edges of each brush stroke"), NULL);
  gtk_grid_attach(GTK_GRID(grid), general_dark_edge_scale, 0, 0, 3, 1);
  gtk_widget_show(general_dark_edge_scale);

  general_shadow_scale = gimp_scale_entry_new(
      _("Shadow darken:"), pcvals.general_shadow_darkness, 0.0, 99.0, 2);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(general_shadow_scale), 0.1,
                                 1.0);
  gimp_help_set_help_data(general_shadow_scale,
                          _("How much to \"darken\" the drop shadow"), NULL);
  gtk_grid_attach(GTK_GRID(grid), general_shadow_scale, 0, 1, 3, 1);
  gtk_widget_show(general_shadow_scale);

  general_shadow_depth = gimp_scale_entry_new(
      _("Shadow depth:"), pcvals.general_shadow_depth, 0, 99, 0);
  gimp_help_set_help_data(general_shadow_depth,
                          _("The depth of the drop shadow, i.e. how far apart "
                            "from the object it should be"),
                          NULL);
  gtk_grid_attach(GTK_GRID(grid), general_shadow_depth, 0, 2, 3, 1);
  gtk_widget_show(general_shadow_depth);

  general_shadow_blur = gimp_scale_entry_new(
      _("Shadow blur:"), pcvals.general_shadow_blur, 0, 99, 0);
  gimp_help_set_help_data(general_shadow_blur,
                          _("How much to blur the drop shadow"), NULL);
  gtk_grid_attach(GTK_GRID(grid), general_shadow_blur, 0, 3, 3, 1);
  gtk_widget_show(general_shadow_blur);

  dev_thresh_scale = gimp_scale_entry_new(_("Deviation threshold:"),
                                          pcvals.devthresh, 0.0, 1.0, 2);
  gimp_help_set_help_data(dev_thresh_scale,
                          _("A bailout-value for adaptive selections"), NULL);
