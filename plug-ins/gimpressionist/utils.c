...
double getsiz_proto(double x, double y, int n, smvector_t *vec, double smstrexp,
                    int voronoi) {
  int i;
  double sum, ssum, dst;
  int first = 0, last;
...
void reselect(GtkWidget *view, gchar *fname) {
  GtkTreeModel *model;
  ...
  if (gtk_tree_model_get_iter_first(model, &iter)) {
    ...
    do {
      ...
      if (!strcmp(name, fname)) {
        ...
        gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(view), tree_path, NULL, TRUE,
                                     0.5, 0.5);
...
static void readdirintolist_real(const char *subdir, GtkWidget *view,
  ...
  while (flist) {
    ...
    /* Set the filename */
    gtk_list_store_set(store, &iter, PRESETS_LIST_COLUMN_FILENAME, flist->data,
                       -1);
    /* Set the object name */
...
void readdirintolist_extended(const char *subdir, GtkWidget *view,
                              char *selected, gboolean with_filename_column,
                              gchar *(*get_object_name_cb)(const gchar *dir,
                                                           gchar *filename,
                                                           void *context),
...
GtkWidget *create_radio_button(GtkWidget *box, int orient_type,
                               ...
                               const gchar *label, const gchar *help_string,
                               GSList **radio_group,
  ...
  g_signal_connect(tmpw, "clicked", G_CALLBACK(callback),
                   GINT_TO_POINTER(orient_type));
...
void gimpressionist_scale_entry_update_int(GimpLabelSpin *entry, gint *value) {
