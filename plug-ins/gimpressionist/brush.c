...
void brush_restore(void) {
  ...
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(brush_relief_scale),
                            pcvals.brush_relief);
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(brush_aspect_scale),
                            pcvals.brush_aspect);
}
...
void brush_free(void) { g_free(last_selected_brush); }
...
static void brushdmenuselect(GtkWidget *widget, gpointer data) {
  ...
  if (bpp == 3) { /* RGB */
    ...
    for (row = 0, y = y1; y < y2; row++, y++) {
      gegl_buffer_get(src_buffer, GEGL_RECTANGLE(x1, y, w, 1), 1.0, format,
...
static void savebrush_response(GtkWidget *dialog, gint response_id,
                               gpointer data) {
...
static void savebrush(GtkWidget *wg, gpointer data) {
  ...
  dialog = gtk_file_chooser_dialog_new(
      _("Save Brush"), GTK_WINDOW(gtk_widget_get_toplevel(wg)),
  ...
  g_signal_connect(dialog, "destroy", G_CALLBACK(gtk_widget_destroyed),
                   &dialog);
  g_signal_connect(dialog, "response", G_CALLBACK(savebrush_response), NULL);

  gtk_widget_show(dialog);
}

static gboolean validdrawable(GimpImage *image, GimpItem *item, gpointer data) {
...
static void padbrush(ppm_t *p, gint width, gint height) {
...
static void update_brush_preview(const gchar *fn) {
  gint i, j;
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(brush_preview), 0, 0, 100, 100,
                         GIMP_GRAY_IMAGE, preview_image, 100);
 ...
 * */
static void brush_select(GtkTreeSelection *selection, gboolean force) {
  GtkTreeIter iter;
  GtkTreeModel *model;
  gchar *fname = NULL;
  gchar *brush = NULL;
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    if (brush) {
      ...
      g_strlcpy(pcvals.selected_brush, fname, sizeof(pcvals.selected_brush));
...
static void brush_select_file(GtkTreeSelection *selection, gpointer data) {
  brush_from_file = 1;
  preset_save_button_set_sensitive(TRUE);
  brush_select(selection, FALSE);
}

static void brush_preview_size_allocate(GtkWidget *preview) {
...
static void brush_aspect_adjust_cb(GimpLabelSpin *scale, gdouble *value) {
...
void create_brushpage(GtkNotebook *notebook) {
  ...
  brush_gamma_adjust =
      gtk_adjustment_new(pcvals.brushgamma, 0.5, 3.0, 0.1, 0.1, 1.0);
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), -1,
                             G_CALLBACK(brushdmenuselect), NULL, NULL);
  ...
  brush_aspect_scale = gimp_scale_entry_new(_("Aspect ratio:"),
                                            pcvals.brush_aspect, -1.0, 1.0, 2);
  gimp_help_set_help_data(brush_aspect_scale,
                          _("Specifies the aspect ratio of the brush"), NULL);
  gtk_size_group_add_widget(
      group, gimp_labeled_get_label(GIMP_LABELED(brush_aspect_scale)));
  g_signal_connect(brush_aspect_scale, "value-changed",
                   G_CALLBACK(brush_aspect_adjust_cb), &pcvals.brush_aspect);
  ...
  gimp_help_set_help_data(
      ...
      NULL);
  gtk_size_group_add_widget(
      group, gimp_labeled_get_label(GIMP_LABELED(brush_relief_scale)));
