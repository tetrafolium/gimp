...
static void paper_update_preview(void) {
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(paper_preview), 0, 0, 100, 100,
                         GIMP_GRAY_IMAGE, paper_preview_buffer, 100);
...
static void paper_select(GtkTreeSelection *selection, gpointer data) {
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    if (paper) {
      ...
      g_strlcpy(pcvals.selected_paper, fname, sizeof(pcvals.selected_paper));
...
void paper_restore(void) {
  ...
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(paper_relief_adjust),
                            pcvals.paper_relief);
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(paper_scale_adjust),
                            pcvals.paper_scale);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(paper_invert),
                               pcvals.paper_invert);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(paper_overlay),
                               pcvals.paper_overlay);
}

void paper_store(void) {
  pcvals.paper_invert =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(paper_invert));
  pcvals.paper_overlay =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(paper_overlay));
}
...
void create_paperpage(GtkNotebook *notebook) {
  ...
  gtk_widget_show(tmpw);
  g_signal_connect_swapped(tmpw, "clicked", G_CALLBACK(paper_select),
                           selection);
