...
void color_restore(void) {
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(colorradio[pcvals.color_type]),
                               TRUE);
...
int color_type_input(int in) { return CLAMP_UP_TO(in, NUMCOLORRADIO); }
...
void create_colorpage(GtkNotebook *notebook) {
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Color"), G_CALLBACK(gimp_radio_button_update),
      &pcvals.color_type, NULL, 0,

      _("A_verage under brush"), COLOR_TYPE_AVERAGE,
      &colorradio[COLOR_TYPE_AVERAGE], _("C_enter of brush"), COLOR_TYPE_CENTER,
  ...
  colornoiseadjust = gimp_scale_entry_new(_("Color _noise:"),
                                          pcvals.color_noise, 0.0, 100.0, 0);
  gimp_help_set_help_data(colornoiseadjust, _("Adds random noise to the color"),
                          NULL);
