...
int orientation_type_input(int in) { return CLAMP_UP_TO(in, NUMORIENTRADIO); }

void orientation_restore(void) {
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(orient_radio[pcvals.orient_type]), TRUE);
...
static void create_orientmap_dialog_helper(GtkWidget *widget) {
...
static void create_orientradio_button(GtkWidget *box, int orient_type,
                                      ...
                                      GSList **radio_group) {
  create_radio_button(box, orient_type, orientation_store, label, help_string,
                      radio_group, orient_radio);
}
...
void create_orientationpage(GtkNotebook *notebook) {
  ...
  orient_first_adjust = gimp_scale_entry_new(
      _("Start angle:"), pcvals.orient_first, 0.0, 360.0, 0);
  ...
  create_orientradio_button(box3, ORIENTATION_RADIAL, _("Radial"),
                            _("Let the direction from the center determine the "
  ...
  create_orientradio_button(
      box3, ORIENTATION_HUE, _("Hue"),
  ...
  create_orientradio_button(box3, ORIENTATION_ADAPTIVE, _("Adaptive"),
                            _("The direction that matches the original image "
  ...
  gtk_widget_show(tmpw);
  g_signal_connect(tmpw, "clicked", G_CALLBACK(create_orientmap_dialog_helper),
                   NULL);
  gimp_help_set_help_data(tmpw, _("Opens up the Orientation Map Editor"), NULL);
