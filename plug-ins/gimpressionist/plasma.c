...
static void mkplasma_sub(ppm_t *p, int x1, int x2, int y1, int y2, float turb) {
  ...
  if (!PIXEL(y1, nx, r))
    PIXEL(y1, nx, r) =
        pfix((PIXEL(y1, x1, r) + PIXEL(y1, x2, r)) / 2.0 +
             turb * g_rand_double_range(random_generator, -xr / 2.0, xr / 2.0));
  if (!PIXEL(y2, nx, r))
    PIXEL(y2, nx, r) =
        pfix((PIXEL(y2, x1, r) + PIXEL(y2, x2, r)) / 2.0 +
             turb * g_rand_double_range(random_generator, -xr / 2.0, xr / 2.0));
  if (!PIXEL(ny, x1, r))
    PIXEL(ny, x1, r) =
        pfix((PIXEL(y1, x1, r) + PIXEL(y2, x1, r)) / 2.0 +
             turb * g_rand_double_range(random_generator, -yr / 2.0, yr / 2.0));
  if (!PIXEL(ny, x2, r))
    PIXEL(ny, x2, r) =
        pfix((PIXEL(y1, x2, r) + PIXEL(y2, x2, r)) / 2.0 +
             turb * g_rand_double_range(random_generator, -yr / 2.0, yr / 2.0));
  if (!PIXEL(ny, nx, r))
    PIXEL(ny, nx, r) =
        pfix((PIXEL(y1, x1, r) + PIXEL(y1, x2, r) + PIXEL(y2, x1, r) +
              PIXEL(y2, x2, r)) /
                 4.0 +
             turb * g_rand_double_range(random_generator, -(xr + yr) / 4.0,
                                        (xr + yr) / 4.0));
