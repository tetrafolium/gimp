...
int place_type_input(int in) { return CLAMP_UP_TO(in, NUM_PLACE_RADIO); }

void place_store(void) {
  pcvals.placement_center =
      gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(placement_center));
}
...
void create_placementpage(GtkNotebook *notebook) {
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Placement"), G_CALLBACK(gimp_radio_button_update),
  ...
  gimp_help_set_help_data(placement_radio[PLACEMENT_TYPE_RANDOM],
                          _("Place strokes randomly around the image"), NULL);
  gimp_help_set_help_data(
      placement_radio[PLACEMENT_TYPE_EVEN_DIST],
      _("The strokes are evenly distributed across the image"), NULL);
  ...
  brush_density_adjust = gimp_scale_entry_new(
      _("Stroke _density:"), pcvals.brush_density, 1.0, 50.0, 0);
  gimp_help_set_help_data(brush_density_adjust,
                          _("The relative density of the brush strokes"), NULL);
