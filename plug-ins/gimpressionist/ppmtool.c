...
void rescale(ppm_t *p, double sc) { resize(p, p->width * sc, p->height * sc); }
...
void ppm_save(ppm_t *p, const char *fn) {
  ...
  if (!f) {
     ...
     * */
    g_message(_("Failed to save PPM file '%s': %s"), gimp_filename_to_utf8(fn),
              g_strerror(errno));
    return;
  }
...
void edgepad(ppm_t *p, int left, int right, int top, int bottom) {
  ...
  for (; y - top < p->height; y++) {
    memcpy(&tmp.col[y * drowstride + left * 3], p->col + (y - top) * srowstride,
           srowstride);
  }
  for (; y < tmp.height; y++) {
    memcpy(&tmp.col[y * drowstride + left * 3],
           p->col + (p->height - 1) * srowstride, srowstride);
  }
  for (y = 0; y < tmp.height; y++) {
    guchar *col, *tmprow;
