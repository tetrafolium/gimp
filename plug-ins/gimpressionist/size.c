...
int size_type_input(int in) { return CLAMP_UP_TO(in, NUMSIZERADIO); }

static void size_type_restore(void) {
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(sizeradio[pcvals.size_type]),
                               TRUE);
}

void size_restore(void) {
  size_type_restore();
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(sizenumadjust), pcvals.size_num);
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(sizefirstadjust),
                            pcvals.size_first);
  gimp_label_spin_set_value(GIMP_LABEL_SPIN(sizelastadjust), pcvals.size_last);
}

static void create_sizemap_dialog_helper(GtkWidget *widget) {
...
static void create_size_radio_button(GtkWidget *box, int orient_type,
                                     ...
                                     GSList **radio_group) {
  create_radio_button(box, orient_type, size_store, label, help_string,
                      radio_group, sizeradio);
}
...
void create_sizepage(GtkNotebook *notebook) {
  ...
  gimp_help_set_help_data(sizenumadjust,
                          _("The number of sizes of brushes to use"), NULL);
  ...
  sizefirstadjust = gimp_scale_entry_new(_("Minimum size:"), pcvals.size_first,
                                         0.0, 360.0, 0);
  gimp_help_set_help_data(sizefirstadjust, _("The smallest brush to create"),
                          NULL);
  ...
  gimp_help_set_help_data(sizelastadjust, _("The largest brush to create"),
                          NULL);
  ...
  create_size_radio_button(
      box3, SIZE_TYPE_RADIAL, _("Radial"),
  ...
  create_size_radio_button(box4, SIZE_TYPE_MANUAL, _("Manual"),
                           _("Manually specify the stroke size"), &radio_group);
  ...
  gtk_widget_show(tmpw);
  g_signal_connect(tmpw, "clicked", G_CALLBACK(create_sizemap_dialog_helper),
                   NULL);
