...
static GimpValueArray *warp_run(GimpProcedure *procedure, GimpRunMode run_mode,
                                GimpImage *image, GimpDrawable *drawable,
                                const GimpValueArray *args, gpointer run_data);

static void blur16(GimpDrawable *drawable);

static void diff(GimpDrawable *drawable, GimpDrawable **xl, GimpDrawable **yl);

static void diff_prepare_row(GeglBuffer *buffer, const Babl *format,
                             guchar *data, gint x, gint y, gint w);

static void warp_one(GimpDrawable *draw, GimpDrawable *newid,
                     GimpDrawable *map_x, GimpDrawable *map_y,
                     GimpDrawable *mag_draw, gboolean first_time, gint step);

static void warp(GimpDrawable *drawable);

static gboolean warp_dialog(GimpDrawable *drawable);
static void warp_pixel(GeglBuffer *buffer, const Babl *format, gint width,
                       gint height, gint x1, gint y1, gint x2, gint y2, gint x,
                       gint y, guchar *pixel);

static gboolean warp_map_constrain(GimpImage *image, GimpItem *item,
                                   gpointer data);
static gdouble warp_map_mag_give_value(guchar *pt, gint alpha, gint bytes);
...
static void warp_class_init(WarpClass *klass) {
...
static void warp_init(Warp *warp) {}
...
static GimpProcedure *warp_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, warp_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "John P. Beale", "John P. Beale",
                                   "1997");

    GIMP_PROC_ARG_DOUBLE(procedure, "amount", "Amount",
                         "Pixel displacement multiplier", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 10.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "warp-map", "Warp map",
                           "Displacement control map", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "iter", "Iter", "Iteration count", 1, 100, 5,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "dither", "Dither", "Random dither amount",
                         0, 100, 0.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "angle", "Angle",
                         "Angle of gradient vector rotation", 0, 360, 90.0,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "wrap-type", "Wrap type",
                      "Edge behavior: { WRAP (0), SMEAR (1), BLACK (2), "
                      "COLOR (3) }",
                      0, 3, WRAP, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "mag-map", "Mag map",
                           "Magnitude control map", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "mag-use", "Mag use", "Use magnitude map",
                          FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "substeps", "Substeps",
                      "Substeps between image updates", 1, 100, 1,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "grad-map", "Grad map",
                           "Gradient control map", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "grad-scale", "Grad scale",
                         "Scaling factor for gradient map (0=don't use)",
                         -G_MAXDOUBLE, G_MAXDOUBLE, 0.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "vector-map", "Vector map",
                           "Fixed vector control map", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "vector-scale", "Vector scale",
                         "Scaling factor for fixed vector map (0=don't use)",
                         -G_MAXDOUBLE, G_MAXDOUBLE, 0.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "vector-angle", "Vector angle",
                         "Angle for fixed vector map", 0, 360, 0.0,
                         G_PARAM_READWRITE);
  }
...
static GimpValueArray *warp_run(GimpProcedure *procedure, GimpRunMode _run_mode,
                                GimpImage *image, GimpDrawable *drawable,
                                const GimpValueArray *args, gpointer run_data) {
  GimpRGB color;
  ...
  gimp_rgb_get_uchar(&color, &color_pixel[0], &color_pixel[1], &color_pixel[2]);
...
static gboolean warp_dialog(GimpDrawable *drawable) {
  ...
  dlg = gimp_dialog_new(
      _("Warp"), PLUG_IN_ROLE, NULL, 0, gimp_standard_help_func, PLUG_IN_PROC,
  ...
  /*  amount, iter */
  spinbutton = spin_button_new(&adj, dvals.amount, -1000, 1000, /* ??? */
                               1, 10, 0, 1, 2);
  gtk_size_group_add_widget(spin_group, spinbutton);
  g_object_unref(spin_group);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Step size:"), 0.0,
                                   0.5, spinbutton, 1);
  gtk_size_group_add_widget(label_group, label);
  g_object_unref(label_group);

  g_signal_connect(adj, "value-changed",
                   G_CALLBACK(gimp_double_adjustment_update), &dvals.amount);

  spinbutton = spin_button_new(&adj, dvals.iter, 1, 100, 1, 5, 0, 1, 0);
  gtk_size_group_add_widget(spin_group, spinbutton);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Iterations:"), 0.0,
                                   0.5, spinbutton, 1);
  gtk_size_group_add_widget(label_group, label);
  ...
  combo = gimp_drawable_combo_box_new(warp_map_constrain, drawable, NULL);
  gtk_widget_set_margin_start(combo, 12);
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), dvals.warp_map_id,
  ...
  g_object_set_data(G_OBJECT(toggle), "gimp-item-data", GINT_TO_POINTER(WRAP));

  g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_radio_button_update),
                   &dvals.wrap_type);
  ...
  g_object_set_data(G_OBJECT(toggle), "gimp-item-data", GINT_TO_POINTER(SMEAR));
  ...
  g_object_set_data(G_OBJECT(toggle), "gimp-item-data", GINT_TO_POINTER(BLACK));
  ...
  g_object_set_data(G_OBJECT(toggle), "gimp-item-data", GINT_TO_POINTER(COLOR));
  ...
  spinbutton = spin_button_new(&adj, dvals.dither, 0, 100, 1, 10, 0, 1, 2);
  gtk_size_group_add_widget(spin_group, spinbutton);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Dither size:"), 0.0,
                                   0.5, spinbutton, 1);
  gtk_size_group_add_widget(label_group, label);

  g_signal_connect(adj, "value-changed",
                   G_CALLBACK(gimp_double_adjustment_update), &dvals.dither);

  spinbutton = spin_button_new(&adj, dvals.angle, 0, 360, 1, 15, 0, 1, 1);
  gtk_size_group_add_widget(spin_group, spinbutton);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Rotation angle:"),
                                   0.0, 0.5, spinbutton, 1);
  gtk_size_group_add_widget(label_group, label);

  g_signal_connect(adj, "value-changed",
                   G_CALLBACK(gimp_double_adjustment_update), &dvals.angle);

  spinbutton = spin_button_new(&adj, dvals.substeps, 1, 100, 1, 5, 0, 1, 0);
  gtk_size_group_add_widget(spin_group, spinbutton);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Substeps:"), 0.0,
                                   0.5, spinbutton, 1);
  gtk_size_group_add_widget(label_group, label);
  ...
  combo = gimp_drawable_combo_box_new(warp_map_constrain, drawable, NULL);
  gtk_widget_set_margin_start(combo, 12);
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), dvals.mag_map_id,
                             G_CALLBACK(gimp_int_combo_box_get_active),
  ...
  spinbutton = spin_button_new(&adj, dvals.grad_scale, -1000, 1000, /* ??? */
                               0.01, 0.1, 0, 1, 3);
  gtk_size_group_add_widget(spin_group, spinbutton);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Gradient scale:"),
                                   0.0, 0.5, spinbutton, 1);
  gtk_size_group_add_widget(label_group, label);
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), dvals.grad_map_id,
                             G_CALLBACK(gimp_int_combo_box_get_active),
  ...
  spinbutton = spin_button_new(&adj, dvals.vector_scale, -1000, 1000, /* ??? */
                               0.01, 0.1, 0, 1, 3);
  gtk_size_group_add_widget(spin_group, spinbutton);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Vector mag:"), 0.0,
                                   0.5, spinbutton, 1);
  gtk_size_group_add_widget(label_group, label);
  ...
  spinbutton =
      spin_button_new(&adj, dvals.vector_angle, 0, 360, 1, 15, 0, 1, 1);
  gtk_size_group_add_widget(spin_group, spinbutton);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Angle:"), 0.0, 0.5,
                                   spinbutton, 1);
  gtk_size_group_add_widget(label_group, label);
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), dvals.vector_map_id,
                             G_CALLBACK(gimp_int_combo_box_get_active),
                             &dvals.vector_map_id, NULL);

  gimp_help_set_help_data(combo, _("Fixed-direction-vector map selection menu"),
                          NULL);
...
static const Babl *get_u8_format(GimpDrawable *drawable) {
...
static void blur16(GimpDrawable *drawable) {
  ...
  if (!gimp_drawable_mask_intersect(drawable, &x1, &y1, &width, &height))
    return;
  ...
  dest_bytes_inc =
      dest_bytes -
      2; /* this is most likely zero, but I guess it's more conservative... */
  ...
  pr =
      prev_row + src_bytes; /* row arrays are prepared for indexing to -1 (!) */
  ...
  for (row = y1; row < y2; row++) {
    ...
    for (col = 0; col < (x2 - x1); col++) /* over columns of pixels */
    {
      ...
      pval = (256.0 * pr[offb - src_bytes] + pr[off1 - src_bytes] +
              256.0 * pr[offb] + pr[off1] + 256.0 * pr[offb + src_bytes] +
              pr[off1 + src_bytes] + 256.0 * cr[offb - src_bytes] +
              cr[off1 - src_bytes] + 256.0 * cr[offb] + cr[off1] +
              256.0 * cr[offb + src_bytes] + cr[off1 + src_bytes] +
              256.0 * nr[offb - src_bytes] + nr[off1 - src_bytes] +
              256.0 * nr[offb] + nr[off1] + 256.0 * nr[offb + src_bytes]) +
             nr[off1 + src_bytes];
...
static void diff_prepare_row(GeglBuffer *buffer, const Babl *format,
  ...
  gegl_buffer_get(buffer, GEGL_RECTANGLE(x, y, w, 1), 1.0, format, data,
...
static void diff(GimpDrawable *drawable, GimpDrawable **xl, GimpDrawable **yl) {
  ...
  gint dest_bytes_inc;
  gint do_gradmap =
      FALSE; /* whether to add in gradient of gradmap to final diff. map */
  gint do_vecmap =
      FALSE; /* whether to add in a fixed vector scaled by the vector map */
  ...
  gint row, col, offb, off,
      bytes; /* relating to indexing into pixel row arrays */
  gint x1, y1, x2, y2;
  gint dvalx, dvaly; /* differential value at particular pixel */
  gdouble tx,
      ty; /* temporary x,y differential value increments from gradmap, etc. */
  ...
  gdouble r, theta, dtheta; /* rectangular<-> spherical coordinate transform for
                               vector rotation */
  gdouble scale_vec_x,
      scale_vec_y; /* fixed vector X,Y component scaling factors */
  ...
  if (dvals.vector_scale !=
      0.0) /* add in gradient of vectormap if scale != 0.000 */
    do_vecmap = TRUE;

  do_magmap =
      (dvals.mag_use == TRUE); /* multiply by magnitude map if so requested */
  ...
  xlayer =
      gimp_layer_new(new_image, "Warp_X_Vectors", width, height, GIMP_RGB_IMAGE,
                     100.0, gimp_image_get_default_new_layer_mode(new_image));

  ylayer =
      gimp_layer_new(new_image, "Warp_Y_Vectors", width, height, GIMP_RGB_IMAGE,
                     100.0, gimp_image_get_default_new_layer_mode(new_image));
  ...
  /* for a GRAYA drawable, I would expect this to be two bytes; any more would
   * be excess */
  ...
  for (row = y1; row < y2; row++) {
    ...
    for (col = 0; col < (x2 - x1); col++) /* over columns of pixels */
    {
      ...
      offb = col * src_bytes; /* base of byte pointer offset */
      for (bytes = 0; bytes < src_bytes;
           bytes++) /* add all channels together */
      {
        ...
        rdy += ((gint)-pr[off - src_bytes] - (gint)2 * pr[off] -
                (gint)pr[off + src_bytes] + (gint)nr[off - src_bytes] +
                (gint)2 * nr[off] + (gint)nr[off + src_bytes]);
      }

      rdx *=
          rscalefac; /* take average, then reduce. Assume max. rdx now 65535 */
      rdy *= rscalefac; /* take average, then reduce */

      theta =
          atan2(rdy, rdx); /* convert to polar, then back to rectang. coords */
      ...
      if (do_gradmap) {
        offb = col * gbytes; /* base of byte pointer offset into pixel values
                                (R,G,B,Alpha, etc.) */
        for (bytes = 0; bytes < src_bytes;
             bytes++) /* add all channels together */
        {
          ...
          ty += ((gint)-prg[off - gbytes] - (gint)2 * prg[off] -
                 (gint)prg[off + gbytes] + (gint)nrg[off - gbytes] +
                 (gint)2 * nrg[off] + (gint)nrg[off + gbytes]);
        }
        tx *= gscalefac;
        ty *= gscalefac;
      ...
      dvalx =
          rdx +
          (2 << 14); /* take zero point to be 2^15, since this is two bytes */
      dvaly = rdy + (2 << 14);
    ...
    /*  store the dest  */
    gegl_buffer_set(destx_buffer, GEGL_RECTANGLE(x1, row, (x2 - x1), 1), 0,
                    destx_format, destx, GEGL_AUTO_ROWSTRIDE);

    gegl_buffer_set(desty_buffer, GEGL_RECTANGLE(x1, row, (x2 - x1), 1), 0,
                    desty_format, desty, GEGL_AUTO_ROWSTRIDE);
...
static void warp(GimpDrawable *orig_draw) {
  ...
  /* Get selection area */
  if (!gimp_drawable_mask_intersect(orig_draw, &x1, &y1, &width, &height))
    return;
  ...
  for (warp_iter = 0; warp_iter < dvals.iter; warp_iter++) {
    ...
    warp_one(orig_draw, orig_draw, map_x, map_y, mag_draw, first_time,
             warp_iter);

    gimp_drawable_update(orig_draw, x1, y1, (x2 - x1), (y2 - y1));
...
static void warp_one(GimpDrawable *draw, GimpDrawable *new, GimpDrawable *map_x,
                     GimpDrawable *map_y, GimpDrawable *mag_draw,
                     gboolean first_time, gint step) {
  GeglBuffer *src_buffer;
  GeglBuffer *dest_buffer;
  ...
  if (!gimp_drawable_mask_intersect(draw, &x1, &y1, &width, &height))
    return;
  ...
  iter = gegl_buffer_iterator_new(
      src_buffer, GEGL_RECTANGLE(x1, y1, (x2 - x1), (y2 - y1)), 0, src_format,
  ...
  gegl_buffer_iterator_add(iter, dest_buffer,
                           ...
                           dest_format, GEGL_ACCESS_WRITE, GEGL_ABYSS_NONE);
  ...
  gegl_buffer_iterator_add(iter, map_x_buffer,
                           GEGL_RECTANGLE(x1, y1, (x2 - x1), (y2 - y1)), 0,
                           map_x_format, GEGL_ACCESS_READ, GEGL_ABYSS_NONE);
  ...
  gegl_buffer_iterator_add(iter, map_y_buffer,
                           GEGL_RECTANGLE(x1, y1, (x2 - x1), (y2 - y1)), 0,
                           map_y_format, GEGL_ACCESS_READ, GEGL_ABYSS_NONE);
  ...
  if (dvals.mag_use) {
    ...
    gegl_buffer_iterator_add(iter, mag_buffer,
                             GEGL_RECTANGLE(x1, y1, (x2 - x1), (y2 - y1)), 0,
                             mag_format, GEGL_ACCESS_READ, GEGL_ABYSS_NONE);
  }
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (y = roi.y; y < (roi.y + roi.height); y++) {
      ...
      for (x = roi.x; x < (roi.x + roi.width); x++) {
        ...
        if (dvals.mag_use) {
          scalefac =
              warp_map_mag_give_value(mmag, mag_alpha, mag_bytes) / 255.0;
          dx *= scalefac;
          dy *= scalefac;
        }

        if (dvals.dither !=
            0.0) { /* random dither is +/- dvals.dither pixels */
          dx += g_rand_double_range(gr, -dvals.dither, dvals.dither);
          dy += g_rand_double_range(gr, -dvals.dither, dvals.dither);
        }

        if (dvals.substeps !=
            1) { /* trace (substeps) iterations of displacement vector */
          for (substep = 1; substep < dvals.substeps; substep++) {
            /* In this (substep) loop, (x,y) remain fixed. (dx,dy) vary each
             * step. */
            needx = x + dx;
            needy = y + dy;
            ...
            /* get 4 neighboring DX values from DiffX drawable for linear
             * interpolation */
            warp_pixel(map_x_buffer, map_x_format, width, height, x1, y1, x2,
                       y2, xi, yi, pixel[0]);
            warp_pixel(map_x_buffer, map_x_format, width, height, x1, y1, x2,
                       y2, xi + 1, yi, pixel[1]);
            warp_pixel(map_x_buffer, map_x_format, width, height, x1, y1, x2,
            ...
            /* get 4 neighboring DY values from DiffY drawable for linear
             * interpolation */
            warp_pixel(map_y_buffer, map_y_format, width, height, x1, y1, x2,
                       y2, xi, yi, pixel[0]);
            warp_pixel(map_y_buffer, map_y_format, width, height, x1, y1, x2,
                       y2, xi + 1, yi, pixel[1]);
            warp_pixel(map_y_buffer, map_y_format, width, height, x1, y1, x2,
         ...
         */
        warp_pixel(src_buffer, src_format, width, height, x1, y1, x2, y2, xi,
                   yi, pixel[0]);
        warp_pixel(src_buffer, src_format, width, height, x1, y1, x2, y2,
                   xi + 1, yi, pixel[1]);
        warp_pixel(src_buffer, src_format, width, height, x1, y1, x2, y2, xi,
                   yi + 1, pixel[2]);
        warp_pixel(src_buffer, src_format, width, height, x1, y1, x2, y2,
...
static void warp_pixel(GeglBuffer *buffer, const Babl *format, gint width,
  ...
  if (x >= x1 && y >= y1 && x < x2 && y < y2) {
    gegl_buffer_sample(buffer, x, y, NULL, pixel, format, GEGL_SAMPLER_NEAREST,
                       GEGL_ABYSS_NONE);
  } else {
    gint bpp = babl_format_get_bytes_per_pixel(format);
    gint b;
