...
/*
   ...
   1.2.1 fixes some minor bugs -- spaces in #XXXXXX strings, small typos in
   code.
...
static GimpValueArray *xpm_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *xpm_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void parse_image(GimpImage *image, XpmImage *xpm_image, guchar *cmap);
...
static void xpm_class_init(XpmClass *klass) {
...
static GimpProcedure *xpm_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, xpm_load, NULL, NULL);
    ...
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-pixmap");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "xpm");
    gimp_file_procedure_set_magics(GIMP_FILE_PROCEDURE(procedure),
                                   "0, string,/*\\040XPM\\040*/");
  } else if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, xpm_save, NULL, NULL);
    ...
    GIMP_PROC_ARG_INT(procedure, "threshold", "Threshold", "Alpha threshold", 0,
                      255, 127, G_PARAM_READWRITE);
  }
...
static GimpValueArray *xpm_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "XPM",
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  case XpmOpenFailed:
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Error opening file '%s'"), gimp_file_get_utf8_name(file));
    g_free(filename);
    return NULL;
  ...
  image = gimp_image_new(xpm_image.width, xpm_image.height, GIMP_RGB);
...
static void parse_image(GimpImage *image, XpmImage *xpm_image, guchar *cmap) {
  ...
  layer = gimp_layer_new(image, _("Color"), xpm_image->width, xpm_image->height,
                         GIMP_RGBA_IMAGE, 100,
  ...
  for (i = 0; i < xpm_image->height; i += tile_height) {
    ...
    gegl_buffer_set(buffer, GEGL_RECTANGLE(0, i, xpm_image->width, scanlines),
                    0, NULL, buf, GEGL_AUTO_ROWSTRIDE);
  }
...
static guint rgbhash(rgbkey *c) {
  return ((guint)c->r) ^ ((guint)c->g) ^ ((guint)c->b);
}

static guint compare(rgbkey *c1, rgbkey *c2) {
  return (c1->r == c2->r) && (c1->g == c2->g) && (c1->b == c2->b);
}

static void set_XpmImage(XpmColor *array, guint index, gchar *colorstring) {
  gchar *p;
...
static void create_colormap_from_hash(gpointer gkey, gpointer value,
                                      gpointer user_data) {
  rgbkey *key = gkey;
...
static void decrement_hash_values(gpointer gkey, gpointer value,
                                  gpointer user_data) {
  --(*((guint *)value));
}
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  g_object_get(config, "threshold", &threshold, NULL);
  ...
  buf = g_new(guchar, gimp_tile_height() * width *
  ...
  for (i = 0; i < height; i += gimp_tile_height()) {
    ...
    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, i, width, scanlines), 1.0, format,
  ...
  if (indexed) {
    ...
    cpp = 1 + (gdouble)log(ncolors) / (gdouble)log(sizeof(linenoise) - 1.0);
...
static gboolean save_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  scale =
      gimp_prop_scale_entry_new(config, "threshold", NULL, 1.0, FALSE, 0, 0);
