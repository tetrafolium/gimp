...
typedef enum { GIMP_PLUGIN_HGT_LOAD_ARGUMENT_ERROR } GimpPluginHGTError;

static GQuark gimp_plugin_hgt_load_error_quark(void) {
...
typedef enum {
  ...
  RAW_BGR565_BE, /* RGB Image 16bit, 5,6,5 bits per channel, big-endian, red and
                    blue swapped */
  RAW_BGR565_LE, /* RGB Image 16bit, 5,6,5 bits per channel, little-endian, red
                    and blue swapped */
...
static GimpValueArray *raw_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *raw_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static gboolean raw_load_standard(RawGimpData *data, gint bpp);
static gboolean raw_load_gray(RawGimpData *data, gint bpp, gint bitspp);
static gboolean raw_load_rgb565(RawGimpData *data, RawType type);
static gboolean raw_load_planar(RawGimpData *data);
static gboolean raw_load_palette(RawGimpData *data, GFile *palette_file);

/* support functions */
static goffset get_file_info(GFile *file);
static void raw_read_row(FILE *fp, guchar *buf, gint32 offset, gint32 size);
static int mmap_read(gint fd, gpointer buf, gint32 len, gint32 pos,
                     gint rowstride);
static void rgb_565_to_888(guint16 *in, guchar *out, gint32 num_pixels,
                           RawType type);
...
static gboolean load_dialog(GFile *file, gboolean is_hgt);
static gboolean save_dialog(GimpImage *image, GimpProcedure *procedure,
                            GObject *config);
static void palette_callback(GtkFileChooser *button, GimpPreviewArea *preview);

static void file_raw_scale_entry_update_int(GimpLabelSpin *entry, gint *value);
...
static void raw_class_init(RawClass *klass) {
...
static GimpProcedure *raw_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, raw_load, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "information",
                                     name);
    gimp_procedure_set_attribution(procedure, "timecop, pg@futureware.at",
                                   "timecop, pg@futureware.at", "Aug 2004");

    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "data");
    ...
    gimp_procedure_set_documentation(procedure, "Load HGT data as images",
                                     ...
                                     "plug-in.",
                                     name);
    gimp_procedure_set_attribution(procedure, NULL, NULL, "2017-12-09");

    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "hgt");

    GIMP_PROC_ARG_INT(procedure, "sample-spacing", "Sample spacing",
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, raw_save, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "Dump images to disk in raw format", name);
    gimp_procedure_set_attribution(
        procedure, "Björn Kautler, Bjoern@Kautler.net",
        "Björn Kautler, Bjoern@Kautler.net", "April 2014");

    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure),
                                       "data,raw");

    GIMP_PROC_ARG_INT(procedure, "image-type", "Image type",
                      "The image type { RAW_RGB (0), RAW_PLANAR (6) }", RAW_RGB,
                      RAW_PLANAR, RAW_RGB, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "palette-type", "Palette type",
                      "The palette type "
...
static GimpValueArray *raw_load(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  } else if (is_hgt) /* HGT file in non-interactive mode. */
  {
    ...
    if (sample_spacing != 0 && sample_spacing != 1 && sample_spacing != 3) {
      g_set_error(&error, GIMP_PLUGIN_HGT_LOAD_ERROR,
      ...
      case 0:
         ...
         * it was successful.
         */
        if (runtime->image_width != 1201 && runtime->image_width != 3601) {
...
static GimpValueArray *raw_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  g_object_get(config, "image-type", &image_type, NULL);
  ...
  export = gimp_export_image(
      &image, &n_drawables, &drawables, "RAW",
 ...
 */
static gint mmap_read(gint fd, void *buf, gint32 len, gint32 pos,
                      gint rowstride) {
...
static gboolean raw_load_standard(RawGimpData *data, gint bpp) {
  ...
  gegl_buffer_set(
      data->buffer,
      GEGL_RECTANGLE(0, 0, runtime->image_width, runtime->image_height), 0,
...
static gboolean raw_load_gray16(RawGimpData *data, RawType type) {
  guint16 *in_raw = NULL;
  ...
  out_size =
      runtime->image_width * runtime->image_height * 3 * sizeof(*out_raw);
...
static gboolean raw_load_gray(RawGimpData *data, gint bpp, gint bitspp) {
  ...
  raw_read_row(data->fp, in_raw, runtime->file_offset, in_size);
...
static gboolean raw_load_planar(RawGimpData *data) {
  ...
  for (i = 0; i < runtime->image_height; i++) {
    ...
    gegl_buffer_set(data->buffer, GEGL_RECTANGLE(0, i, runtime->image_width, 1),
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  g_object_get(config, "image-type", &image_type, "palette-type", &palette_type,
               NULL);
  ...
  gegl_buffer_get(buffer, GEGL_RECTANGLE(0, 0, width, height), 1.0, format, buf,
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  if (runtime->image_type >= RAW_GRAY_16BPP_BE)
    data->image = gimp_image_new_with_precision(runtime->image_width,
                                                runtime->image_height, itype,
                                                GIMP_PRECISION_U16_NON_LINEAR);
  else
    data->image =
        gimp_image_new(runtime->image_width, runtime->image_height, itype);
  gimp_image_set_file(data->image, file);
  layer = gimp_layer_new(data->image, _("Background"), runtime->image_width,
                         runtime->image_height, ltype, 100,
...
static void preview_update_size(GimpPreviewArea *preview) {
  gtk_widget_set_size_request(GTK_WIDGET(preview), runtime->image_width,
...
static void preview_update(GimpPreviewArea *preview) {
  ...
  gimp_preview_area_fill(preview, 0, 0, preview_width, preview_height, 255, 255,
                         255);
  ...
  case RAW_RGB:
    /* standard RGB image */
    {
      ...
      for (y = 0; y < height; y++) {
        ...
        gimp_preview_area_draw(preview, 0, y, width, 1, GIMP_RGB_IMAGE, row,
                               width * 3);
      }
  ...
  case RAW_RGBA:
    /* RGB + alpha image */
    {
      ...
      for (y = 0; y < height; y++) {
        ...
        gimp_preview_area_draw(preview, 0, y, width, 1, GIMP_RGBA_IMAGE, row,
                               width * 4);
      }
  ...
  case RAW_GRAY_8BPP:
...

    {
      ...
      mmap_read(preview_fd, in_raw, in_size, runtime->file_offset, in_size);
      ...
      gimp_preview_area_draw(preview, 0, 0, width, height, GIMP_RGB_IMAGE,
                             out_raw, width * 3);
      g_free(in_raw);
      g_free(out_raw);
    }
  ...
  case RAW_GRAY_16BPP_SLE: {
    ...
    for (y = 0; y < height; y++) {
      ...
      gimp_preview_area_draw(preview, 0, y, width, 1, GIMP_GRAY_IMAGE, row,
                             width * 3);
    }
...
static gboolean load_dialog(GFile *file, gboolean is_hgt) {
  ...
  dialog = gimp_dialog_new(_("Load Image from Raw Data"), PLUG_IN_ROLE, NULL, 0,
                           gimp_standard_help_func,
                           is_hgt ? LOAD_HGT_PROC : LOAD_PROC, _("_Cancel"),
  ...
  gtk_widget_set_size_request(preview, runtime->image_width,
                              runtime->image_height);
  gtk_container_add(GTK_CONTAINER(viewport), preview);
  gtk_widget_show(preview);

  g_signal_connect_after(preview, "size-allocate", G_CALLBACK(preview_update),
                         NULL);
  ...
  combo = NULL;
  if (is_hgt && runtime->image_width != 1201 && runtime->image_width != 3601) {
     ...
     * https://dds.cr.usgs.gov/srtm/version1/Documentation/SRTM_Topo.txt "SRTM-1
     * data are sampled at one arc-second of latitude and longitude and each
     * file contains 3601 lines and 3601 samples.
     * [...]
     * SRTM-3 data are sampled at three arc-seconds and contain 1201 lines and
     * 1201 samples with similar overlapping rows and columns."
     */
    combo = gimp_int_combo_box_new(_("SRTM-1 (1 arc-second)"), 3601,
                                   _("SRTM-3 (3 arc-seconds)"), 1201, NULL);
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Sample Spacing:"), 0.0,
                             0.5, combo, 2);
    ...
    combo = gimp_int_combo_box_new(
        _("RGB"), RAW_RGB, _("RGB Alpha"), RAW_RGBA, _("RGB565 Big Endian"),
        RAW_RGB565_BE, _("RGB565 Little Endian"), RAW_RGB565_LE,
        _("BGR565 Big Endian"), RAW_BGR565_BE, _("BGR565 Little Endian"),
        RAW_BGR565_LE, _("Planar RGB"), RAW_PLANAR, _("B&W 1 bit"),
        RAW_GRAY_1BPP, _("Gray 2 bit"), RAW_GRAY_2BPP, _("Gray 4 bit"),
        RAW_GRAY_4BPP, _("Gray 8 bit"), RAW_GRAY_8BPP, _("Indexed"),
        RAW_INDEXED, _("Indexed Alpha"), RAW_INDEXEDA,
        ...
        _("Gray 16 bit Little Endian"), RAW_GRAY_16BPP_SLE, NULL);
    gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo),
                                  runtime->image_type);
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Image _Type:"), 0.0, 0.5,
                             combo, 2);
  ...
  if (combo)
    g_signal_connect_swapped(combo, "changed", G_CALLBACK(preview_update),
                             preview);

  scale = gimp_scale_entry_new(_("O_ffset:"), runtime->file_offset, 0,
                               file_size, 0);
  ...
  g_signal_connect(scale, "value-changed",
                   ...
                   &runtime->file_offset);
  g_signal_connect_swapped(scale, "value-changed", G_CALLBACK(preview_update),
                           preview);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 1, 3, 1);
  gtk_widget_show(scale);

  if (!is_hgt) {
    scale = gimp_scale_entry_new(_("_Width:"), runtime->image_width, 1,
                                 file_size, 0);
    ...
    g_signal_connect_swapped(scale, "value-changed",
                             G_CALLBACK(preview_update_size), preview);
    g_signal_connect_swapped(scale, "value-changed", G_CALLBACK(preview_update),
                             preview);
    gtk_grid_attach(GTK_GRID(grid), scale, 0, 2, 3, 1);
    gtk_widget_show(scale);

    scale = gimp_scale_entry_new(_("_Height:"), runtime->image_height, 1,
                                 file_size, 0);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Palette Type:"), 0.0, 0.5,
                           combo, 2);

  g_signal_connect(combo, "changed", G_CALLBACK(gimp_int_combo_box_get_active),
                   &runtime->palette_type);
  g_signal_connect_swapped(combo, "changed", G_CALLBACK(palette_update),
                           preview);

  scale = gimp_scale_entry_new(_("Off_set:"), runtime->palette_offset, 0,
                               1 << 24, 0);
  ...
  g_signal_connect_swapped(scale, "value-changed", G_CALLBACK(palette_update),
                           preview);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 1, 3, 1);
  gtk_widget_show(scale);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Pal_ette File:"), 0.0, 0.5,
                           button, 2);

  g_signal_connect(button, "selection-changed", G_CALLBACK(palette_callback),
                   preview);
...
static gboolean save_dialog(GimpImage *image, GimpProcedure *procedure,
  ...
  store = gimp_int_store_new(_("_Standard (R,G,B)"), RAW_RGB,
                             _("_Planar (RRR,GGG,BBB)"), RAW_PLANAR, NULL);
  frame = gimp_prop_int_radio_frame_new(config, "image-type", _("Image Type"),
  ...
  /* Palette type combo */
  store =
      gimp_int_store_new(_("_R, G, B (normal)"), RAW_PALETTE_RGB,
                         _("_B, G, R, X (BMP style)"), RAW_PALETTE_BGR, NULL);
  frame = gimp_prop_int_radio_frame_new(
      config, "palette-type", _("Palette Type"), GIMP_INT_STORE(store));
  g_object_unref(store);
  gtk_box_pack_start(GTK_BOX(main_vbox), frame, FALSE, FALSE, 0);
