...
static void retinex(GimpDrawable *drawable, GimpPreview *preview);
static void retinex_preview(GimpDrawable *drawable, GimpPreview *preview);

static void retinex_scales_distribution(gfloat *scales, gint nscales, gint mode,
                                        gint s);

static void compute_mean_var(gfloat *src, gfloat *mean, gfloat *var, gint size,
                             gint bytes);

static void compute_coefs3(gauss3_coefs *c, gfloat sigma);

static void gausssmooth(gfloat *in, gfloat *out, gint size, gint rowtride,
                        gauss3_coefs *c);
 ...
 */
static void MSRCR(guchar *src, gint width, gint height, gint bytes,
...
static void retinex_class_init(RetinexClass *klass) {
...
static void retinex_init(Retinex *retinex) {}
...
static GimpProcedure *retinex_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, retinex_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "rendition.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Fabien Pelisson",
                                   "Fabien Pelisson", "2003");

    GIMP_PROC_ARG_INT(procedure, "scale", "Scale", "Biggest scale value",
                      MIN_GAUSSIAN_SCALE, MAX_GAUSSIAN_SCALE, 240,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "nscales", "N scales", "Number of scales", 0,
                      MAX_RETINEX_SCALES, 3, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "scales-mode", "Scales mode",
                      "Retinex distribution through scales", RETINEX_UNIFORM,
                      RETINEX_HIGH, RETINEX_UNIFORM, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "cvar", "Cvar", "Variance value", 0, 4, 1.2,
                         G_PARAM_READWRITE);
  }
...
static GimpValueArray *retinex_run(GimpProcedure *procedure,
  ...
  if (!gimp_drawable_mask_intersect(drawable, &x, &y, &width, &height) ||
      width < MIN_GAUSSIAN_SCALE || height < MIN_GAUSSIAN_SCALE) {
...
static gboolean retinex_dialog(GimpDrawable *drawable) {
  ...
  dialog = gimp_dialog_new(_("Retinex Image Enhancement"), PLUG_IN_ROLE, NULL,
  ...
  g_signal_connect_swapped(preview, "invalidated", G_CALLBACK(retinex_preview),
                           drawable);
  ...
  combo = gimp_int_combo_box_new(_("Uniform"), filter_uniform, _("Low"),
                                 filter_low, _("High"), filter_high, NULL);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Level:"), 0.0, 0.5, combo,
                           2);
  gtk_widget_show(combo);

  scale = gimp_scale_entry_new(_("_Scale:"), rvals.scale, MIN_GAUSSIAN_SCALE,
                               MAX_GAUSSIAN_SCALE, 0);
  ...
  scale = gimp_scale_entry_new(_("Scale _division:"), rvals.nscales, 0,
                               MAX_RETINEX_SCALES, 0);
...
static void retinex(GimpDrawable *drawable, GimpPreview *preview) {
   ...
   */
  if (preview) {
    src = gimp_zoom_preview_get_source(GIMP_ZOOM_PREVIEW(preview), &width,
                                       &height, &bytes);
  } else {
    if (!gimp_drawable_mask_intersect(drawable, &x, &y, &width, &height))
      return;
...
static void retinex_scales_distribution(gfloat *scales, gint nscales, gint mode,
  ...
  if (nscales == 1) { /* For one filter we choose the median scale */
    scales[0] = (gint)s / 2;
             ...
             2) { /* For two filters we choose the median and maximum scale */
    scales[0] = (gint)s / 2;
    scales[1] = (gint)s;
  } else {
...
static void compute_coefs3(gauss3_coefs *c, gfloat sigma) {
   ...
   *          Ian T. Young , Lucas J. Van Vliet, Signal Processing 44, Elsevier
   * 1995. formula: 11b       computation of q 8c        computation of b0..b1
...
static void gausssmooth(gfloat *in, gfloat *out, gint size, gint rowstride,
                        gauss3_coefs *c) {
  /*
   * Papers:  "Recursive Implementation of the gaussian filter.",
   *          Ian T. Young , Lucas J. Van Vliet, Signal Processing 44, Elsevier
   * 1995. formula: 9a        forward filter 9b        backward filter fig7
   * algorithm
   */
  gint i, n, bufsize;
  gfloat *w1, *w2;
  ...
  for (i = 0, n = 3; i <= size; i++, n++) {
    w1[n] = (gfloat)(
        c->B * in[i * rowstride] +
        ((c->b[1] * w1[n - 1] + c->b[2] * w1[n - 2] + c->b[3] * w1[n - 3]) /
         c->b[0]));
  }
  ...
  for (i = size, n = i; i >= 0; i--, n--) {
    w2[n] = out[i * rowstride] = (gfloat)(
        c->B * w1[n + 3] +
        ((c->b[1] * w2[n + 1] + c->b[2] * w2[n + 2] + c->b[3] * w2[n + 3]) /
         c->b[0]));
  }
...
static void MSRCR(guchar *src, gint width, gint height, gint bytes,
  ...
  /*
      Adapt the dynamics of the colors according to the statistics of the first
     and second order. The use of the variance makes it possible to control the
     degree of saturation of the colors.
   */
  pdst = dst;
