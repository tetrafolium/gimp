 ...
 * (gimp_text_layer_get_font sometimes returns suffixes such as "semi-expanded"
 * to the font's name although the GIMP's font selection dialog shows the don'ts
 * name normally - This should be checked again in GIMP 2.7)
 * 3. Indexed layers can't be optimized yet (Since gimp_histogram won't work on
 * indexed layers)
 * 4. Rendering the pango layout requires multiplying the size in PANGO_SCALE.
 * This means I'll need to do some hacking on the markup returned from GIMP.
 * 5. When accessing the contents of layer groups is supported, we should do use
 * it (since this plugin should preserve layers).
 *
 ...
 *   - Added various optimizations (solid rectangles instead of bitmaps,
 * ignoring invisible layers, etc.) as a macro flag.
 *   - Added handling for layer masks, use CAIRO_FORMAT_A8 for grayscale
 * drawables.
 ...
 *   Also merged the header and c file into one file, Updated some of the
 * comments and documentation, and moved this into the main source repository.
 */
...
typedef enum { GIMP_PLUGIN_PDF_SAVE_ERROR_FAILED } GimpPluginPDFError;
...
enum { THUMB, PAGE_NUMBER, IMAGE_NAME, IMAGE };
...
static GimpValueArray *pdf_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void choose_file_call(GtkWidget *browse_button, gpointer file_entry);

static gboolean get_image_list(void);

static GtkTreeModel *create_model(void);

static void add_image_call(GtkWidget *widget, gpointer img_combo);
static void del_image_call(GtkWidget *widget, gpointer icon_view);
static void remove_call(GtkTreeModel *tree_model, GtkTreePath *path,
                        gpointer user_data);
static void recount_pages(void);

static cairo_surface_t *get_cairo_surface(GimpDrawable *drawable,
                                          gboolean as_mask, GError **error);

static GimpRGB get_layer_color(GimpLayer *layer, gboolean *single);

static void drawText(GimpLayer *layer, gdouble opacity, cairo_t *cr,
                     gdouble x_res, gdouble y_res);

static gboolean draw_layer(GimpLayer **layers, gint n_layers, gint j,
                           cairo_t *cr, gdouble x_res, gdouble y_res,
...
G_DEFINE_QUARK(gimp - plugin - pdf - save - error - quark,
               gimp_plugin_pdf_save_error)

static void pdf_class_init(PdfClass *klass) {
...
static GimpProcedure *pdf_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, pdf_save, NULL, NULL);

    gimp_procedure_set_image_types(procedure, "*");

    gimp_procedure_set_menu_label(procedure, N_("Portable Document Format"));

    gimp_procedure_set_documentation(procedure, "Save files in PDF format",
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Barak Itkin, Lionel N., Jehan",
                                   "Copyright Barak Itkin, Lionel N., Jehan",
                                   "August 2009, 2017");

    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "application/pdf");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "pdf");

    GIMP_PROC_ARG_BOOLEAN(procedure, "vectorize", "Vectorize",
                          "Convert bitmaps to vector graphics where possible.",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "ignore-hidden", "Ignore hidden",
                          "Omit hidden layers and layers with zero opacity.",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "apply-masks", "Apply masks",
                          "Apply layer masks before saving (Keeping them "
                          "will not change the output),",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "layers-as-pages", "Layers as pages",
                          "Layers as pages (bottom layers first).", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "reverse-order", "Reverse order",
                          "Reverse the pages order (top layers first).", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(
        procedure, "convert-text-layers", "Convert text layers to image",
        "Convert text layers to raster graphics", FALSE, G_PARAM_READWRITE);
    ...
    gimp_procedure_set_documentation(procedure, "Save files in PDF format",
                                     ...
                                     "distant cousin of PostScript.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Barak Itkin",
                                   "Copyright Barak Itkin", "August 2009");

    GIMP_PROC_ARG_ENUM(procedure, "run-mode", "Run mode", "The run mode",
                       GIMP_TYPE_RUN_MODE, GIMP_RUN_INTERACTIVE,
                       G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "count", "Count",
                      "The number of images entered (This will be the "
                      "number of pages).",
                      1, MAX_PAGE_COUNT, 1, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT32_ARRAY(procedure, "images", "Images",
    ...
    GIMP_PROC_ARG_STRING(procedure, "uri", "URI",
                         "The URI of the file to save to", NULL,
...
static GimpValueArray *pdf_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  if (run_mode == GIMP_RUN_NONINTERACTIVE) {
    ...
    optimize.layers_as_pages =
        GIMP_VALUES_GET_BOOLEAN(args, SA_LAYERS_AS_PAGES);
...
static cairo_status_t write_func(void *fp, const unsigned char *data,
                                 unsigned int size) {
  return fwrite(data, 1, size, fp) == size ? CAIRO_STATUS_SUCCESS
                                           : CAIRO_STATUS_WRITE_ERROR;
}

static GList *get_missing_fonts(GList *layers) {
  ...
  for (iter = layers; iter; iter = iter->next) {
    ...
    if (gimp_item_is_group(GIMP_ITEM(layer))) {
      ...
      for (iter2 = child_missing_fonts; iter2; iter2 = iter2->next) {
        ...
        if (g_list_find_custom(missing_fonts, missing,
      ...
      if (g_strcmp0(pango_font_description_get_family(font_description),
        ...
        if (g_list_find_custom(missing_fonts, missing,
                               (GCompareFunc)g_strcmp0) == NULL)
          missing_fonts = g_list_prepend(missing_fonts, g_strdup(missing));
      }
...
static GimpValueArray *pdf_save_image(GimpProcedure *procedure,
  ...
  for (i = 0; i < multi_page.image_count; i++) {
    ...
    if (!(gimp_export_image(&image, &temp_size, &temp, NULL, capabilities) ==
          GIMP_EXPORT_EXPORT)) {
    ...
    cairo_pdf_surface_set_size(pdf_file, gimp_image_width(image) * x_scale,
    ...
    if (gimp_drawable_has_alpha(GIMP_DRAWABLE(layers[n_layers - 1]))) {
      ...
      cairo_rectangle(cr, 0.0, 0.0, gimp_image_width(image),
    ...
    for (j = 0; j < n_layers; j++) {
      if (!draw_layer(layers, n_layers, j, cr, x_res, y_res,
                      gimp_procedure_get_name(procedure), &error)) {
        /* free the resources */
        g_free(layers);
...
static gboolean gui_single(void) {
  ...
  gtk_box_pack_start(GTK_BOX(gimp_export_dialog_get_content_area(window)), vbox,
                     TRUE, TRUE, 0);

  gtk_container_set_border_width(GTK_CONTAINER(window), 12);

  convert_text_c =
      gtk_check_button_new_with_mnemonic(_("_Convert text layers to image"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(convert_text_c),
                               optimize.convert_text);
  missing_fonts =
      get_missing_fonts(gimp_image_list_layers(multi_page.images[0]));
  if (missing_fonts != NULL) {
    ...
    text = g_strdup_printf(
        _("The following fonts cannot be found: %s.\n"
  ...
  ignore_hidden_c = gtk_check_button_new_with_mnemonic(
      _("_Omit hidden layers and layers with zero opacity"));
  ...
  vectorize_c = gtk_check_button_new_with_mnemonic(
  ...
  apply_c =
      gtk_check_button_new_with_mnemonic(_("_Apply layer masks before saving"));
  ...
  gimp_help_set_help_data(
      apply_c, _("Keeping the masks will not change the output"), NULL);
  ...
  text = g_strdup_printf(_("_Layers as pages (%s)"),
                         optimize.reverse_order ? _("top layers first")
                                                : _("bottom layers first"));
  ...
  reverse_order_c =
      gtk_check_button_new_with_mnemonic(_("_Reverse the pages order"));
  ...
  if (n_layers <= 1) {
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(layers_as_pages_c), FALSE);
  }

  g_object_bind_property(layers_as_pages_c, "active", reverse_order_c,
                         "sensitive", G_BINDING_SYNC_CREATE);
  g_signal_connect(G_OBJECT(reverse_order_c), "toggled",
                   G_CALLBACK(reverse_order_toggled), layers_as_pages_c);
...
static gboolean gui_multi(void) {
  ...
  file_choose = gtk_file_chooser_dialog_new(
      _("Multipage PDF export"), GTK_WINDOW(window),
      GTK_FILE_CHOOSER_ACTION_SAVE, _("_Save"), GTK_RESPONSE_OK, _("_Cancel"),
  ...
  g_signal_connect(G_OBJECT(file_browse), "clicked",
                   G_CALLBACK(choose_file_call), file_entry);

  g_signal_connect(G_OBJECT(add_image), "clicked", G_CALLBACK(add_image_call),
                   img_combo);

  g_signal_connect(G_OBJECT(del), "clicked", G_CALLBACK(del_image_call),
                   page_view);

  g_signal_connect(G_OBJECT(model), "row-deleted", G_CALLBACK(remove_call),
                   NULL);
...
static GtkTreeModel *create_model(void) {
   ...
   */
  model = gtk_list_store_new(4, GDK_TYPE_PIXBUF, /* THUMB */
  ...
  for (i = 0; i < multi_page.image_count && i < MAX_PAGE_COUNT; i++) {
    ...
    gtk_list_store_set(model, &iter, THUMB, pixbuf, PAGE_NUMBER,
                       g_strdup_printf(_("Page %d"), i + 1), IMAGE_NAME,
                       gimp_image_get_name(image), IMAGE, image, -1);
...
static gboolean get_image_list(void) {
  ...
  for (valid = gtk_tree_model_get_iter_first(model, &iter); valid;
    ...
    gtk_tree_model_get(model, &iter, IMAGE, &image, -1);
...
static void add_image_call(GtkWidget *widget, gpointer img_combo) {
  ...
  gtk_list_store_set(store, &iter, PAGE_NUMBER,
                     g_strdup_printf(_("Page %d"), multi_page.image_count + 1),
                     THUMB, pixbuf, IMAGE_NAME, gimp_image_get_name(image),
                     IMAGE, image, -1);
...
static void recount_pages(void) {
  ...
  for (valid = gtk_tree_model_get_iter_first(model, &iter); valid;
       valid = gtk_tree_model_iter_next(model, &iter)) {
    gtk_list_store_set(store, &iter, PAGE_NUMBER,
                       g_strdup_printf(_("Page %d"), i + 1), -1);
    i++;
  }
}
...
static cairo_surface_t *get_cairo_surface(GimpDrawable *drawable,
  ...
  if (status != CAIRO_STATUS_SUCCESS) {
    ...
    case CAIRO_STATUS_INVALID_SIZE:
      g_set_error_literal(
          error, GIMP_PLUGIN_PDF_SAVE_ERROR, GIMP_PLUGIN_PDF_SAVE_ERROR_FAILED,
          _("Cannot handle the size (either width or height) of the image."));
      break;
    default:
      g_set_error(error, GIMP_PLUGIN_PDF_SAVE_ERROR,
                  GIMP_PLUGIN_PDF_SAVE_ERROR_FAILED, "Cairo error: %s",
...
static GimpRGB get_layer_color(GimpLayer *layer, gboolean *single) {
  ...
  if (gimp_drawable_bpp(GIMP_DRAWABLE(layer)) >= 3) {
    ...
    gimp_drawable_histogram(GIMP_DRAWABLE(layer), GIMP_HISTOGRAM_RED, 0.0, 1.0,
                            &red, &dev, &median, &pixels, &count, &percentile);
    devSum += dev;

    gimp_drawable_histogram(GIMP_DRAWABLE(layer), GIMP_HISTOGRAM_GREEN, 0.0,
                            1.0, &green, &dev, &median, &pixels, &count,
                            &percentile);
    devSum += dev;

    gimp_drawable_histogram(GIMP_DRAWABLE(layer), GIMP_HISTOGRAM_BLUE, 0.0, 1.0,
                            &blue, &dev, &median, &pixels, &count, &percentile);
    devSum += dev;
  } else {
    /* We are in Grayscale mode (or Indexed) */

    gimp_drawable_histogram(GIMP_DRAWABLE(layer), GIMP_HISTOGRAM_VALUE, 0.0,
                            1.0, &red, &dev, &median, &pixels, &count,
                            &percentile);
    devSum += dev;
    green = red;
    blue = red;
  ...
  if (gimp_drawable_has_alpha(GIMP_DRAWABLE(layer)))
    gimp_drawable_histogram(GIMP_DRAWABLE(layer), GIMP_HISTOGRAM_ALPHA, 0.0,
                            1.0, &alpha, &dev, &median, &pixels, &count,
                            &percentile);
  else
    alpha = 255;
...
static void drawText(GimpLayer *layer, gdouble opacity, cairo_t *cr,
  ...
  if (language)
    pango_context_set_language(context, pango_language_from_string(language));
  ...
  if (!PANGO_GRAVITY_IS_VERTICAL(pango_context_get_base_gravity(context)))
    pango_layout_set_width(layout, gimp_drawable_width(GIMP_DRAWABLE(layer)) *
                                       PANGO_SCALE);
  else
    pango_layout_set_width(layout, gimp_drawable_height(GIMP_DRAWABLE(layer)) *
                                       PANGO_SCALE);
  ...
  letter_spacing_at =
      pango_attr_letter_spacing_new((int)(PANGO_SCALE * letter_spacing));
...
static gboolean draw_layer(GimpLayer **layers, gint n_layers, gint j,
  ...
  if (gimp_item_is_group(GIMP_ITEM(layer))) {
    ...
    for (i = 0; i < children_num; i++) {
      if (!draw_layer((GimpLayer **)children, children_num, i, cr, x_res, y_res,
                      name, error)) {
        g_free(children);
        return FALSE;
      }
    ...
    if (mask) {
      mask_image = get_cairo_surface(GIMP_DRAWABLE(mask), TRUE, error);
    ...
    if (!gimp_item_is_text_layer(GIMP_ITEM(layer)) || optimize.convert_text) {
      ...
      cairo_rectangle(cr, x, y, gimp_drawable_width(GIMP_DRAWABLE(layer)),
                      gimp_drawable_height(GIMP_DRAWABLE(layer)));

      if (optimize.vectorize && single_color) {
        cairo_set_source_rgba(cr, layer_color.r, layer_color.g, layer_color.b,
        ...
        layer_image = get_cairo_surface(GIMP_DRAWABLE(layer), FALSE, error);
