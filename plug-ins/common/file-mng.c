...
enum { CHUNKS_PNG_D, CHUNKS_JNG_D, CHUNKS_PNG, CHUNKS_JNG };

enum { DISPOSE_COMBINE, DISPOSE_REPLACE };
...
static GimpValueArray *mng_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void MNG_DECL myfree(mng_ptr ptr, mng_size_t size);
static mng_bool MNG_DECL myopenstream(mng_handle handle);
static mng_bool MNG_DECL myclosestream(mng_handle handle);
static mng_bool MNG_DECL mywritedata(mng_handle handle, mng_ptr buf,
                                     mng_uint32 size, mng_uint32 *written_size);
...
static gint32 parse_ms_tag_from_layer_name(const gchar *str,
                                           gint default_delay);
static gint find_unused_ia_color(guchar *pixels, gint numpixels, gint *colors);
static gboolean ia_has_transparent_pixels(guchar *pixels, gint numpixels);

static gboolean respin_cmap(png_structp png_ptr, png_infop png_info_ptr,
                            guchar *remap, GimpImage *image, GeglBuffer *buffer,
                            int *bit_depth);

static gboolean mng_save_image(GFile *file, GimpImage *image, gint n_drawables,
                               GimpDrawable **drawables, GObject *config,
                               GError **error);
static gboolean mng_save_dialog(GimpImage *image, GimpProcedure *procedure,
                                GObject *config);
...
static void mng_class_init(MngClass *klass) {
...
static GimpProcedure *mng_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, mng_save, NULL, NULL);
    ...
    gimp_procedure_set_attribution(
        procedure, "Mukund Sivaraman <muks@mukund.org>",
        "Mukund Sivaraman <muks@mukund.org>", "November 19, 2002");

    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-mng");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "mng");

    GIMP_PROC_ARG_BOOLEAN(procedure, "interlaced", "Interlaced",
                          "Use interlacing", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "png-compression", "PNG Compression",
                      _("PNG compression level, choose a high compression "
                        "level for small file size"),
                      0, 9, 9, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "jpeg-quality", "JPEG Quality",
                         "JPEG quality factor", 0, 1, 0.75, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "jpeg-smoothing", "JPEG Smoothing",
                         "JPEG smoothing factor", 0, 1, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "loop", "Loop",
                          "(ANIMATED MNG) Loop infinitely", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "default-delay", "Default delay",
                      "(ANIMATED MNG) Default delay between frames in "
                      "milliseconds",
                      1, G_MAXINT, 100, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "default-chunks", "Default chunks",
                      "(ANIMATED MNG) Default chunks type "
                      "(0 = PNG + Delta PNG; 1 = JNG + Delta PNG; "
                      "2 = All PNG; 3 = All JNG)",
                      0, 3, CHUNKS_PNG_D, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "default-dispose", "Default dispose",
                      "(ANIMATED MNG) Default dispose type "
                      "(0 = combine; 1 = replace)",
                      0, 1, DISPOSE_COMBINE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "bkgd", "bKGD",
                          "Write bKGd (background color) chunk", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "gama", "gAMA", "Write gAMA (gamma) chunk",
                          FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "phys", "pHYs",
                          "Write pHYs (image resolution) chunk", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "time", "tIME",
                          "Write tIME (creation time) chunk", TRUE,
                          G_PARAM_READWRITE);
  }
...
static GimpValueArray *mng_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  if (run_mode == GIMP_RUN_INTERACTIVE || run_mode == GIMP_RUN_WITH_LAST_VALS) {
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "MNG",
  ...
  if (status == GIMP_PDB_SUCCESS) {
    if (!mng_save_image(file, image, n_drawables, drawables, G_OBJECT(config),
                        &error)) {
...
static void MNG_DECL myfree(mng_ptr ptr, mng_size_t size) { g_free(ptr); }

static mng_bool MNG_DECL myopenstream(mng_handle handle) { return MNG_TRUE; }

static mng_bool MNG_DECL myclosestream(mng_handle handle) { return MNG_TRUE; }
...
static mng_bool MNG_DECL mywritedata(mng_handle handle, mng_ptr buf,
  ...
  *written_size =
      (mng_uint32)fwrite((void *)buf, 1, (size_t)size, userdata->fp);
...
static int get_bit_depth_for_palette(int num_palette) {
  if (num_palette <= 2)
    return 1;
...
static gboolean respin_cmap(png_structp pp, png_infop info, guchar *remap,
                            GimpImage *image, GeglBuffer *buffer,
                            int *bit_depth) {
  ...
  gegl_buffer_get(buffer, GEGL_RECTANGLE(0, 0, cols, rows), 1.0, NULL, pixels,
...
static mng_retcode mng_putchunk_plte_wrapper(mng_handle handle, gint numcolors,
...
static mng_retcode mng_putchunk_trns_wrapper(mng_handle handle, gint n_alphas,
                                             const guchar *buffer) {
  ...
  return mng_putchunk_trns(handle, !mng_empty, !mng_global,
                           MNG_COLORTYPE_INDEXED, n_alphas, alphas, 0, 0, 0, 0,
                           0, alphas);
}
...
static gboolean mng_save_image(GFile *file, GimpImage *image, gint n_drawables,
  ...
  g_object_get(config, "interlaced", &config_interlaced, "png-compression",
               &config_png_compression, "jpeg-quality", &config_jpeg_quality,
               "jpeg-smoothing", &config_jpeg_smoothing, "loop", &config_loop,
               "defaut-delay", &config_default_delay, "default-chunks",
               &config_default_chunks, "default-dispose",
               &config_default_dispose, "bkgd", &config_bkgd, "gama",
               &config_gama, "phys", &config_phys, "time", &config_time, NULL);
  ...
  mng_simplicity_profile |= (MNG_SIMPLICITY_JNG | MNG_SIMPLICITY_DELTAPNG);
  ...
  if (mng_putchunk_mhdr(handle, cols, rows, mng_ticks_per_second, 1, num_layers,
  ...
  if ((num_layers > 1) && (config_loop)) {
    gint32 ms = parse_ms_tag_from_layer_name(
        gimp_item_get_name(GIMP_ITEM(layers[0])), config_default_delay);

    if (mng_putchunk_term(handle, MNG_TERMACTION_REPEAT,
                          MNG_ITERACTION_LASTFRAME, ms,
  ...
  if (mng_putchunk_text(handle, strlen(MNG_TEXT_TITLE), MNG_TEXT_TITLE, 18,
  ...
  if (config_gama) {
    if (mng_putchunk_gama(handle, MNG_FALSE, (1.0 / 2.2 * 100000)) !=
        MNG_NOERROR) {
  ...
  if (gimp_image_base_type(image) == GIMP_INDEXED) {
    ...
    if ((numcolors != 0) && (mng_putchunk_plte_wrapper(
                                 handle, numcolors, palette) != MNG_NOERROR)) {
  ...
  for (i = (num_layers - 1); i >= 0; i--) {
    ...
    layer_chunks_type =
        parse_chunks_type_from_layer_name(layer_name, config_default_chunks);
    ...
    gimp_drawable_offsets(GIMP_DRAWABLE(layers[i]), &layer_offset_x,
                          &layer_offset_y);
    ...
    if ((i == (num_layers - 1)) ||
        (parse_disposal_type_from_layer_name(
             layer_name, config_default_dispose) != DISPOSE_COMBINE))
      frame_mode = MNG_FRAMINGMODE_3;
    else
      frame_mode = MNG_FRAMINGMODE_1;

    frame_delay =
        parse_ms_tag_from_layer_name(layer_name, config_default_delay);

    if (mng_putchunk_fram(
            handle, MNG_FALSE, frame_mode, 0, NULL, MNG_CHANGEDELAY_DEFAULT,
            MNG_CHANGETIMOUT_NO, MNG_CHANGECLIPPING_DEFAULT,
            MNG_CHANGESYNCID_NO, frame_delay, 0, 0, layer_offset_x,
            layer_offset_x + layer_cols, layer_offset_y,
            layer_offset_y + layer_rows, 0, NULL) != MNG_NOERROR) {
      g_warning("Unable to mng_putchunk_fram() in mng_save_image()");
      goto err3;
    }

    if ((layer_offset_x != 0) || (layer_offset_y != 0)) {
      if (mng_putchunk_defi(handle, 0, 0, 1, 1, layer_offset_x, layer_offset_y,
                            1, layer_offset_x, layer_offset_x + layer_cols,
                            layer_offset_y,
    ...
    if ((outfile = g_fopen(temp_file_name, "wb")) == NULL) {
      g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno),
                  _("Could not open '%s' for writing: %s"),
                  gimp_file_get_utf8_name(temp_file), g_strerror(errno));
    ...
    if (!info) {
      g_warning("Unable to png_create_info_struct() in mng_save_image()");
    ...
    case GIMP_INDEXED_IMAGE:
      ...
      mngg.has_plte = TRUE;
      mngg.palette =
          (png_colorp)gimp_image_get_colormap(image, &mngg.num_palette);
    ...
    case GIMP_INDEXEDA_IMAGE:
      ...
      layer_has_unique_palette =
          respin_cmap(pp, info, layer_remap, image, layer_buffer, &bit_depth);
      break;
     ...
     * png_set_*() functions.
     */
    png_set_IHDR(pp, info, layer_cols, layer_rows, bit_depth, color_type,
    ...
    for (pass = 0; pass < num_passes; pass++) {
      for (begin = 0, end = tile_height; begin < layer_rows;
        ...
        gegl_buffer_get(layer_buffer, GEGL_RECTANGLE(0, begin, layer_cols, num),
                        1.0, layer_format, layer_pixel, GEGL_AUTO_ROWSTRIDE,
                        GEGL_ABYSS_NONE);
        ...
        } else if (png_get_valid(pp, info, PNG_INFO_PLTE) && (layer_bpp == 2)) {
          for (j = 0; j < num; j++) {
            fixed = layer_pixels[j];
    ...
    while (!feof(infile)) {
      ...
      if (strncmp(chunkname, "IHDR", 4) == 0) {
        ...
        if (mng_putchunk_ihdr(handle, chunkwidth, chunkheight, chunkbitdepth,
                              chunkcolortype, chunkcompression, chunkfilter,
        ...
        if (mng_putchunk_idat(handle, chunksize, chunkbuffer) != MNG_NOERROR) {
         ...
         */
        if (mng_putchunk_plte_wrapper(
                handle, (layer_has_unique_palette ? (chunksize / 3) : 0),
        ...
        if (mng_putchunk_trns_wrapper(handle, chunksize, chunkbuffer) !=
            MNG_NOERROR) {
...
static gboolean mng_save_dialog(GimpImage *image, GimpProcedure *procedure,
  ...
  toggle = gimp_prop_check_button_new(config, "interlaced", _("_Interlace"));
  gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);

  toggle =
      gimp_prop_check_button_new(config, "bkgd", _("Save _background color"));
  gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);

  gtk_widget_set_sensitive(toggle, FALSE);

  toggle = gimp_prop_check_button_new(config, "gama", _("Save _gamma"));
  gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);

  toggle = gimp_prop_check_button_new(config, "phys", _("Save resolution"));
  gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);

  gtk_widget_set_sensitive(toggle, FALSE);

  toggle = gimp_prop_check_button_new(config, "time", _("Save creation _time"));
  gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);
  ...
  if (num_layers == 1)
    store = gimp_int_store_new(_("PNG"), CHUNKS_PNG_D, _("JNG"), CHUNKS_JNG_D,
                               NULL);
  else
    store = gimp_int_store_new(_("PNG + delta PNG"), CHUNKS_PNG_D,
                               _("JNG + delta PNG"), CHUNKS_JNG_D, _("All PNG"),
                               CHUNKS_PNG, _("All JNG"), CHUNKS_JNG, NULL);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Default chunks type:"), 0.0,
                           0.5, combo, 1);

  store = gimp_int_store_new(_("Combine"), DISPOSE_COMBINE, _("Replace"),
                             DISPOSE_REPLACE, NULL);
  combo = gimp_prop_int_combo_box_new(config, "default-dispose",
                                      GIMP_INT_STORE(store));
  g_object_unref(store);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Default _frame disposal:"),
                           0.0, 0.5, combo, 1);

  scale = gimp_prop_hscale_new(config, "png-compression", 1.0, 1.0, 0);
  gtk_widget_set_size_request(scale, SCALE_WIDTH, -1);
  gtk_scale_set_value_pos(GTK_SCALE(scale), GTK_POS_TOP);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("_PNG compression level:"),
                           0.0, 0.9, scale, 1);

  scale = gimp_prop_hscale_new(config, "jpeg-quality", 0.01, 0.01, 2);
  gtk_widget_set_size_request(scale, SCALE_WIDTH, -1);
  gtk_scale_set_value_pos(GTK_SCALE(scale), GTK_POS_TOP);
  gtk_widget_set_sensitive(scale, FALSE);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("JPEG compression quality:"),
                           0.0, 0.9, scale, 1);

  scale = gimp_prop_hscale_new(config, "jpeg-smoothing", 0.01, 0.01, 2);
  gtk_widget_set_size_request(scale, SCALE_WIDTH, -1);
  gtk_scale_set_value_pos(GTK_SCALE(scale), GTK_POS_TOP);
  gtk_widget_set_sensitive(scale, FALSE);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 4, _("JPEG smoothing factor:"),
                           0.0, 0.9, scale, 1);
  ...
  toggle = gimp_prop_check_button_new(config, "loop", _("_Loop"));
  gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);
  ...
  spinbutton = gimp_prop_spin_button_new(config, "default-delay", 10, 100, 0);
