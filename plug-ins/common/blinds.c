...
static void blinds_scale_entry_update_int(GimpLabelSpin *entry, gint *value);

static void dialog_update_preview(GimpDrawable *drawable, GimpPreview *preview);
...
static BlindVals bvals = {30, 3, GIMP_ORIENTATION_HORIZONTAL, FALSE};

static void blinds_class_init(BlindsClass *klass) {
...
static void blinds_init(Blinds *blinds) {}
...
static GimpProcedure *blinds_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, blinds_run, NULL, NULL);
    ...
    GIMP_PROC_ARG_INT(procedure, "angle-displacement", "Angle displacement",
                      "Angle of Displacement", 0, 360, 30, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "num-segments", "Num segments",
                      "Number of segments in blinds", 1, 1024, 3,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_ENUM(procedure, "orientation", "Orientation",
                       "The orientation", GIMP_TYPE_ORIENTATION_TYPE,
                       GIMP_ORIENTATION_HORIZONTAL, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "bg-transparent", "BG transparent",
                          "Background transparent", FALSE, G_PARAM_READWRITE);
  }
...
static gboolean blinds_dialog(GimpDrawable *drawable) {
  ...
  dialog = gimp_dialog_new(
      _("Blinds"), PLUG_IN_ROLE, NULL, 0, gimp_standard_help_func, PLUG_IN_PROC,
  ...
  g_signal_connect_swapped(preview, "invalidated",
                           G_CALLBACK(dialog_update_preview), drawable);
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Orientation"), G_CALLBACK(gimp_radio_button_update),
      &bvals.orientation, NULL, bvals.orientation,

      _("_Horizontal"), GIMP_ORIENTATION_HORIZONTAL, &horizontal,

      _("_Vertical"), GIMP_ORIENTATION_VERTICAL, &vertical,
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(blinds_scale_entry_update_int), &bvals.angledsp);
  g_signal_connect_swapped(scale, "value-changed",
                           G_CALLBACK(gimp_preview_invalidate), preview);
  gtk_box_pack_start(GTK_BOX(main_vbox), scale, FALSE, FALSE, 2);
  gtk_widget_show(scale);

  scale = gimp_scale_entry_new(_("_Number of segments:"), bvals.numsegs, 1,
                               MAX_FANS, 0);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(blinds_scale_entry_update_int), &bvals.numsegs);
  g_signal_connect_swapped(scale, "value-changed",
                           G_CALLBACK(gimp_preview_invalidate), preview);
  gtk_box_pack_start(GTK_BOX(main_vbox), scale, FALSE, FALSE, 2);
  gtk_widget_show(scale);
...
static void blindsapply(guchar *srow, guchar *drow, gint width, gint bpp,
                        guchar *bg) {
  guchar *src;
  guchar *dst;
  gint i, j, k;
  ...
  for (k = 0; k < bvals.numsegs; k++) {
    ...
    for (i = 0; i < (fanwidths[k] / 2); i++) {
      ...
      dst =
          &drow[(available + fanwidths[k] - j - (fanwidths[k] % 2) - dx) * bpp];
...
static void dialog_update_preview(GimpDrawable *drawable,
  ...
  cache = gimp_drawable_get_thumbnail_data(drawable, &width, &height, &bpp);
  p = cache;
  ...
  if (bvals.orientation == GIMP_ORIENTATION_VERTICAL) {
    for (y = 0; y < height; y++) {
      blindsapply(p, buffer + y * width * bpp, width, bpp, bg);
      p += width * bpp;
    }
  } else {
    /* Horizontal blinds */
    ...
    for (y = 0; y < height; y++) {
      if (dr[y] == 0) {
        ...
        /* Draw line from src */
        p = cache + (width * bpp * (dr[y] - 1));
      }
      memcpy(buffer + y * width * bpp, p, width * bpp);
    }
    g_free(sr);
    g_free(dr);
  }
  ...
  if (bvals.orientation == GIMP_ORIENTATION_VERTICAL) {
    for (y = 0; y < sel_height; y += STEP) {
      ...
      gegl_buffer_get(src_buffer,
                      GEGL_RECTANGLE(sel_x1, sel_y1 + y, sel_width, step), 1.0,
                      format, src_rows, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);

      /* OK I could make this better */
      for (rr = 0; rr < STEP; rr++)
        blindsapply(src_rows + (sel_width * rr * bytes),
                    des_rows + (sel_width * rr * bytes), sel_width, bytes, bg);

      gegl_buffer_set(dest_buffer,
                      GEGL_RECTANGLE(sel_x1, sel_y1 + y, sel_width, step), 0,
                      format, des_rows, GEGL_AUTO_ROWSTRIDE);
    ...
    for (x = 0; x < sel_width; x += STEP) {
      ...
      gegl_buffer_get(src_buffer,
                      GEGL_RECTANGLE(sel_x1 + x, sel_y1, step, sel_height), 1.0,
                      format, src_rows, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
      ...
      for (rr = 0; rr < sel_height; rr++) {
        ...
        memcpy(des_rows + (rr * step * bytes), p, step * bytes);
      }

      gegl_buffer_set(dest_buffer,
                      GEGL_RECTANGLE(sel_x1 + x, sel_y1, step, sel_height), 0,
                      format, des_rows, GEGL_AUTO_ROWSTRIDE);
