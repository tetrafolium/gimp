...
static TileItInterface tint = {NULL,       /* Preview */
                               {'4', 'u'}, /* Preview_row */
                               4,          /* bpp of drawable */
                               NULL};
...
static GimpValueArray *tile_run(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void tileit_scale_update(GimpLabelSpin *entry, gint *value);
...
static void tileit_radio_update(GtkWidget *widget, gpointer data);
static void tileit_hvtoggle_update(GtkWidget *widget, gpointer data);

static void do_tiles(GimpDrawable *drawable);
static gint tiles_xy(gint width, gint height, gint x, gint y, gint *nx,
                     gint *ny);
...
static gboolean tileit_preview_draw(GtkWidget *widget, cairo_t *cr);
static gboolean tileit_preview_events(GtkWidget *widget, GdkEvent *event);
...
static TileItVals itvals = {2};

/* Structures for call backs... */
/* The "explicit tile" & family */
typedef enum { ALL, ALT, EXPLICIT } AppliedTo;
...
static GimpProcedure *tile_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, tile_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "More here later", name);
    gimp_procedure_set_attribution(procedure, "Andy Thomas", "Andy Thomas",
                                   "1997");

    GIMP_PROC_ARG_INT(procedure, "num-tiles", "Num tiles",
                      "Number of tiles to make", 2, MAX_SEGS, 2,
                      G_PARAM_READWRITE);
  }
...
static GimpValueArray *tile_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  if (!gimp_drawable_mask_intersect(drawable, &sel_x1, &sel_y1, &sel_width,
                                    &sel_height)) {
...
static gboolean tileit_dialog(GimpDrawable *drawable) {
  ...
  dlg = gimp_dialog_new(_("Small Tiles"), PLUG_IN_ROLE, NULL, 0,
  ...
  g_signal_connect_after(tint.preview, "draw", G_CALLBACK(tileit_preview_draw),
                         NULL);
  g_signal_connect(tint.preview, "event", G_CALLBACK(tileit_preview_events),
                   NULL);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(tileit_hvtoggle_update),
                   &do_horz);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(tileit_reset), &res_call);
  ...
  toggle =
      gtk_radio_button_new_with_mnemonic(orientation_group, _("A_ll tiles"));
  orientation_group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(toggle));
  gtk_grid_attach(GTK_GRID(grid), toggle, 0, 0, 4, 1);
  gtk_widget_show(toggle);

  g_object_set_data(G_OBJECT(toggle), "gimp-item-data", GINT_TO_POINTER(ALL));

  g_signal_connect(toggle, "toggled", G_CALLBACK(tileit_radio_update),
                   &exp_call.type);
  ...
  g_object_set_data(G_OBJECT(toggle), "gimp-item-data", GINT_TO_POINTER(ALT));
  ...
  g_object_bind_property(toggle, "active", label, "sensitive",
  ...
  g_signal_connect(adj, "value-changed", G_CALLBACK(tileit_exp_update_f),
                   &exp_call);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(tileit_exp_update), &exp_call);
  ...
  g_signal_connect(scale, "value-changed", G_CALLBACK(tileit_scale_update),
                   &opacity);
...
static void exp_need_update(gint nx, gint ny) {
  ...
  if (nx != exp_call.x || ny != exp_call.y) {
    ...
    g_signal_handlers_block_by_func(exp_call.c_adj, tileit_exp_update_f,
                                    &exp_call);
    g_signal_handlers_block_by_func(exp_call.r_adj, tileit_exp_update_f,
                                    &exp_call);

    gtk_adjustment_set_value(exp_call.c_adj, nx);
    gtk_adjustment_set_value(exp_call.r_adj, ny);

    g_signal_handlers_unblock_by_func(exp_call.c_adj, tileit_exp_update_f,
                                      &exp_call);
    g_signal_handlers_unblock_by_func(exp_call.r_adj, tileit_exp_update_f,
                                      &exp_call);
  }
}
...
static void explicit_update(gboolean settile) {
  gint x, y;
...
static void tileit_scale_update(GimpLabelSpin *scale, gint *value) {
  *value = RINT(gimp_label_spin_get_value(scale));

  dialog_update_preview();
}

static void tileit_reset(GtkWidget *widget, gpointer data) {
  Reset_Call *r = (Reset_Call *)data;

  memset(tileactions, 0, sizeof(tileactions));

  g_signal_handlers_block_by_func(r->htoggle, tileit_hvtoggle_update, &do_horz);
  g_signal_handlers_block_by_func(r->vtoggle, tileit_hvtoggle_update, &do_vert);
  ...
  g_signal_handlers_unblock_by_func(r->htoggle, tileit_hvtoggle_update,
                                    &do_horz);
  g_signal_handlers_unblock_by_func(r->vtoggle, tileit_hvtoggle_update,
                                    &do_vert);
...
static void tileit_exp_update(GtkWidget *widget, gpointer applied) {
  explicit_update(TRUE);
  dialog_update_preview();
}

static void tileit_exp_update_f(GtkWidget *widget, gpointer applied) {
  explicit_update(FALSE);
  dialog_update_preview();
}
...
static void cache_preview(GimpDrawable *drawable) {
  ...
  gegl_buffer_get(buffer,
                  GEGL_RECTANGLE(scale * sel_x1, scale * sel_y1, preview_width,
                                 preview_height),
                  scale, format, tint.pv_cache, GEGL_AUTO_ROWSTRIDE,
                  GEGL_ABYSS_NONE);
...
static void do_tiles(GimpDrawable *drawable) {
  ...
  iter = gegl_buffer_iterator_new(
      dest_buffer, GEGL_RECTANGLE(sel_x1, sel_y1, sel_width, sel_height), 0,
      format, GEGL_ACCESS_WRITE, GEGL_ABYSS_NONE, 1);
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (row = dest_roi.y; row < (dest_roi.y + dest_roi.height); row++) {
      ...
      for (col = dest_roi.x; col < (dest_roi.x + dest_roi.width); col++) {
        ...
        gegl_buffer_sample(src_buffer, nc + sel_x1, nr + sel_y1, NULL, pixel,
  ...
  gimp_drawable_update(drawable, sel_x1, sel_y1, sel_width, sel_height);
}
...
static void do_tiles_preview(guchar *dest_row, guchar *src_rows, gint width,
                             gint dh, gint height, gint bpp) {
  gint x;
  gint i;
  gint px, py;
  gint rnum, cnum;
  ...
  for (x = 0; x < width; x++) {
    ...
    for (i = 0; i < bpp; i++)
      dest_row[x * tint.img_bpp + i] = src_rows[(px + (py * width)) * bpp + i];
...
static void dialog_update_preview(void) {
  ...
  for (y = 0; y < preview_height; y++) {
    do_tiles_preview(tint.preview_row, tint.pv_cache, preview_width, y,
                     preview_height, tint.img_bpp);

    memcpy(buffer + y * (preview_width * tint.img_bpp), tint.preview_row,
           preview_width * tint.img_bpp);
  }

  gimp_preview_area_draw(GIMP_PREVIEW_AREA(tint.preview), 0, 0, preview_width,
                         preview_height,
                         (tint.img_bpp > 3) ? GIMP_RGBA_IMAGE : GIMP_RGB_IMAGE,
                         buffer, preview_width * tint.img_bpp);
