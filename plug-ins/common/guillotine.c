...
#define GUILLOTINE                                                             \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), GUILLOTINE_TYPE, Guillotine))
...
static GimpValueArray *guillotine_run(GimpProcedure *procedure,
                                      ...
                                      gpointer run_data);
static GList *guillotine(GimpImage *image, gboolean interactive);

G_DEFINE_TYPE(Guillotine, guillotine, GIMP_TYPE_PLUG_IN)

GIMP_MAIN(GUILLOTINE_TYPE)

static void guillotine_class_init(GuillotineClass *klass) {
...
static void guillotine_init(Guillotine *film) {}
...
static GimpProcedure *guillotine_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, guillotine_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "not modified.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Adam D. Moss (adam@foxbox.org)",
                                   "Adam D. Moss (adam@foxbox.org)", "1998");

    GIMP_PROC_VAL_INT(procedure, "image-count", "Number of images created",
                      "Number of images created", 0, G_MAXINT, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_VAL_OBJECT_ARRAY(procedure, "images", "Output images",
                               "Output images", GIMP_TYPE_IMAGE,
                               G_PARAM_READWRITE);
  }
...
static gint guide_sort_func(gconstpointer a, gconstpointer b) {
...
static GList *guillotine(GimpImage *image, gboolean interactive) {
  ...
  for (guide = gimp_image_find_next_guide(image, 0); guide > 0;
  ...
  if (guides_found) {
    ...
    for (y = 0, hg = hguides; hg && hg->next; y++, hg = hg->next) {
      for (x = 0, vg = vguides; vg && vg->next; x++, vg = vg->next) {
        ...
        gimp_image_crop(
            new_image,
            GPOINTER_TO_INT(vg->next->data) - GPOINTER_TO_INT(vg->data),
            GPOINTER_TO_INT(hg->next->data) - GPOINTER_TO_INT(hg->data),
            GPOINTER_TO_INT(vg->data), GPOINTER_TO_INT(hg->data));
