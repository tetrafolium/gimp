...
static GimpValueArray *svg_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *svg_load_thumb(GimpProcedure *procedure, GFile *file,
                                      gint size, const GimpValueArray *args,
                                      gpointer run_data);

static GimpImage *load_image(GFile *file, GError **error);
static GdkPixbuf *load_rsvg_pixbuf(GFile *file, SvgLoadVals *vals,
                                   GError **error);
static gboolean load_rsvg_size(GFile *file, SvgLoadVals *vals, GError **error);
static gboolean load_dialog(GFile *file, GError **error);

G_DEFINE_TYPE(Svg, svg, GIMP_TYPE_PLUG_IN)

GIMP_MAIN(SVG_TYPE)

static SvgLoadVals load_vals = {SVG_DEFAULT_RESOLUTION, 0, 0, FALSE, FALSE};

static void svg_class_init(SvgClass *klass) {
...
static GimpProcedure *svg_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, svg_load, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "using librsvg.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Dom Lachowicz, Sven Neumann",
                                   "Dom Lachowicz <cinamod@hotmail.com>",
                                   SVG_VERSION);

    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/svg+xml");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "svg");
    ...
    GIMP_PROC_ARG_INT(procedure, "paths", "Paths",
                      "(0) don't import paths, (1) paths individually, "
                      "(2) paths merged",
                      0, 2, 0, GIMP_PARAM_READWRITE);
  } else if (!strcmp(name, LOAD_THUMB_PROC)) {
    procedure = gimp_thumbnail_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, svg_load_thumb, NULL, NULL);
...
static GimpValueArray *svg_load(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  if (load_vals.import) {
    ...
    gimp_vectors_import_from_file(image, file, load_vals.merge, TRUE,
                                  &num_vectors, &vectors);
    if (num_vectors > 0)
      g_free(vectors);
  }
...
static GimpImage *load_image(GFile *file, GError **load_error) {
  GimpImage *image;
  GimpLayer *layer;
  GdkPixbuf *pixbuf;
  gint width;
  ...
  layer = gimp_layer_new_from_pixbuf(
      image, _("Rendered SVG"), pixbuf, 100,
      gimp_image_get_default_new_layer_mode(image), 0.0, 1.0);
...
static gboolean load_dialog(GFile *file, GError **load_error) {
  GtkWidget *dialog;
  GtkWidget *frame;
  GtkWidget *hbox;
  GtkWidget *vbox;
  ...
  gboolean run;
  GError *error = NULL;
  SvgLoadVals vals = {SVG_DEFAULT_RESOLUTION, -SVG_PREVIEW_SIZE,
                      -SVG_PREVIEW_SIZE};
  ...
  dialog = gimp_dialog_new(_("Render Scalable Vector Graphics"), PLUG_IN_ROLE,
  ...
  xadj = gtk_adjustment_new(
      ratio_x, (gdouble)GIMP_MIN_IMAGE_SIZE / (gdouble)svg_width,
      (gdouble)GIMP_MAX_IMAGE_SIZE / (gdouble)svg_width, 0.01, 0.1, 0);
  ...
  yadj = gtk_adjustment_new(
      ratio_y, (gdouble)GIMP_MIN_IMAGE_SIZE / (gdouble)svg_height,
      (gdouble)GIMP_MAX_IMAGE_SIZE / (gdouble)svg_height, 0.01, 0.1, 0);
  ...
  g_signal_connect(toggle2, "toggled", G_CALLBACK(gimp_toggle_button_update),
                   &load_vals.merge);

  g_object_bind_property(toggle, "active", toggle2, "sensitive",
