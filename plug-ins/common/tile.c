...
static void tile(GimpImage *image, GimpDrawable *drawable,
                 GimpImage **new_image, GimpLayer **new_layer);

static void tile_gegl(GeglBuffer *src, gint src_width, gint src_height,
                      GeglBuffer *dst, gint dst_width, gint dst_height);

static gboolean tile_dialog(GimpImage *image, GimpDrawable *drawable);
...
static GimpProcedure *tile_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    ...
    GIMP_PROC_ARG_INT(procedure, "new-width", "New width",
                      "New (tiled) image width", 1, GIMP_MAX_IMAGE_SIZE, 1,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "new-height", "New height",
                      "New (tiled) image height", 1, GIMP_MAX_IMAGE_SIZE, 1,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "new-image", "New image",
                          "Create a new image", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_VAL_IMAGE(procedure, "new-image", "New image",
                        "Output image (NULL if new-image == FALSE)", TRUE,
                        G_PARAM_READWRITE);

    GIMP_PROC_VAL_LAYER(procedure, "new-layer", "New layer",
                        "Output layer (NULL if new-image == FALSE)", TRUE,
                        G_PARAM_READWRITE);
  }
...
static GimpValueArray *tile_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  tile(image, drawable, &new_image, &new_layer);
...
static void tile_gegl(GeglBuffer *src, gint src_width, gint src_height,
  ...
  buffer_src_node = gegl_node_new_child(node, "operation", "gegl:buffer-source",
                                        "buffer", src, NULL);

  crop_src_node = gegl_node_new_child(node, "operation", "gegl:crop", "width",
                                      (gdouble)src_width, "height",
                                      (gdouble)src_height, NULL);

  tile_node = gegl_node_new_child(node, "operation", "gegl:tile", NULL);

  crop_dst_node = gegl_node_new_child(node, "operation", "gegl:crop", "width",
                                      (gdouble)dst_width, "height",
                                      (gdouble)dst_height, NULL);

  buffer_dst_node = gegl_node_new_child(node, "operation", "gegl:write-buffer",
                                        "buffer", dst, NULL);

  gegl_node_link_many(buffer_src_node, crop_src_node, tile_node, crop_dst_node,
                      buffer_dst_node, NULL);
...
static void tile(GimpImage *image, GimpDrawable *drawable,
  ...
  if (tvals.new_image) {
    ...
    *new_image = gimp_image_new_with_precision(dst_width, dst_height,
    ...
    *new_layer =
        gimp_layer_new(*new_image, _("Background"), dst_width, dst_height,
                       gimp_drawable_type(drawable), 100,
  ...
  tile_gegl(src_buffer, src_width, src_height, dst_buffer, dst_width,
            dst_height);
...
static gboolean tile_dialog(GimpImage *image, GimpDrawable *drawable) {
  ...
  dlg = gimp_dialog_new(
      _("Tile"), PLUG_IN_ROLE, NULL, 0, gimp_standard_help_func, PLUG_IN_PROC,
  ...
  sizeentry = gimp_coordinates_new(
      unit, "%a", TRUE, TRUE, 8, GIMP_SIZE_ENTRY_UPDATE_SIZE,

      tvals.constrain, TRUE,

      _("_Width:"), width, xres, 1, GIMP_MAX_IMAGE_SIZE, 0, width,

      _("_Height:"), height, yres, 1, GIMP_MAX_IMAGE_SIZE, 0, height);
