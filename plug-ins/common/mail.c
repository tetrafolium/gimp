...
static GimpValueArray *mail_run(GimpProcedure *procedure, GimpRunMode run_mode,
                                GimpImage *image, GimpDrawable *drawable,
                                const GimpValueArray *args, gpointer run_data);

static GimpPDBStatusType send_image(const gchar *filename, GimpImage *image,
                                    GimpDrawable *drawable, gint32 run_mode);

static gboolean send_dialog(void);
static void mail_entry_callback(GtkWidget *widget, gchar *data);
static gboolean valid_file(GFile *file);
static gchar *find_extension(const gchar *filename);

#ifdef SENDMAIL
static void mesg_body_callback(GtkTextBuffer *buffer, gpointer data);

static gchar *sendmail_content_type(const gchar *filename);
static void sendmail_create_headers(FILE *mailpipe);
static gboolean sendmail_to64(const gchar *filename, FILE *outfile,
                              GError **error);
static FILE *sendmail_pipe(gchar **cmd, GPid *pid);
#endif
...
static void mail_class_init(MailClass *klass) {
...
static void mail_init(Mail *mail) {}
...
static GimpProcedure *mail_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, mail_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure, N_("Send the image by email"),
#ifdef SENDMAIL
...
#endif
                                     name);
    gimp_procedure_set_attribution(procedure, "Adrian Likins, Reagan Blundell",
                                   "Adrian Likins, Reagan Blundell, "
                                   "Daniel Risacher, "
                                   "Spencer Kimball and Peter Mattis",
                                   "1995-1997");

    GIMP_PROC_ARG_STRING(procedure, "filename", "Filename",
                         "The name of the file to save the image in", NULL,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "to-address", "To address",
                         "The email address to send to", NULL,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "from-address", "From address",
                         "The email address for the From: field", NULL,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "subject", "Subject", "The subject", NULL,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "comment", "Comment", "The comment", NULL,
                         G_PARAM_READWRITE);
  }
...
static GimpValueArray *mail_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_NONINTERACTIVE:
    g_strlcpy(mail_info.filename, GIMP_VALUES_GET_STRING(args, 0), BUFFER_SIZE);
    g_strlcpy(mail_info.receipt, GIMP_VALUES_GET_STRING(args, 1), BUFFER_SIZE);
    g_strlcpy(mail_info.from, GIMP_VALUES_GET_STRING(args, 2), BUFFER_SIZE);
    g_strlcpy(mail_info.subject, GIMP_VALUES_GET_STRING(args, 3), BUFFER_SIZE);
    g_strlcpy(mail_info.comment, GIMP_VALUES_GET_STRING(args, 4), BUFFER_SIZE);
    break;
  ...
  status = send_image(mail_info.filename, image, drawable, run_mode);
...
static GimpPDBStatusType send_image(const gchar *filename, GimpImage *image,
  ...
  if (!(gimp_file_save(run_mode, image, 1, (const GimpItem **)drawables,
                       tmpfile) &&
        valid_file(tmpfile))) {
    g_free(drawables);
    goto error;
  ...
  enumerator = g_file_enumerate_children(
      tmp_dir, G_FILE_ATTRIBUTE_STANDARD_TYPE,
  ...
  if (g_rename(tmpname, filepath) == -1) {
    ...
    if (!g_file_move(source, target, G_FILE_COPY_NONE, NULL, NULL, NULL,
                     &error)) {
  ...
  if (!g_spawn_async(NULL, mailcmd, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL, NULL,
                     &error)) {
    g_message("%s", error->message);
    g_error_free(error);
    goto error;
...
static gboolean send_dialog(void) {
  ...
  dlg = gimp_dialog_new(_("Send by Email"), PLUG_IN_ROLE, NULL, 0,
                        gimp_standard_help_func, PLUG_IN_PROC,

                        _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Send"),
                        GTK_RESPONSE_OK,
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("_Filename:"), 0.0, 0.5,
                           entry, 1);
  g_signal_connect(entry, "changed", G_CALLBACK(mail_entry_callback),
                   mail_info.filename);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++,
                           C_("email-address", "_To:"), 0.0, 0.5, entry, 1);
  g_signal_connect(entry, "changed", G_CALLBACK(mail_entry_callback),
                   mail_info.receipt);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++,
                           C_("email-address", "_From:"), 0.0, 0.5, entry, 1);
  g_signal_connect(entry, "changed", G_CALLBACK(mail_entry_callback),
                   mail_info.from);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("S_ubject:"), 0.0, 0.5,
                           entry, 1);
  g_signal_connect(entry, "changed", G_CALLBACK(mail_entry_callback),
                   mail_info.subject);
  ...
  g_signal_connect(text_buffer, "changed", G_CALLBACK(mesg_body_callback),
                   NULL);
...
static gchar *sendmail_content_type(const gchar *filename) {
  ...
  const gchar *type_mappings[20] = {"gif",  "image/gif",
                                    "jpg",  "image/jpeg",
                                    "jpeg", "image/jpeg",
                                    "tif",  "image/tiff",
                                    "tiff", "image/tiff",
