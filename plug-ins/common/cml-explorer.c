...
#define CANNONIZE(p, x)                                                        \
  (255 * (((p).range_h - (p).range_l) * (x) + (p).range_l))
#define HCANNONIZE(p, x)                                                       \
  (254 * (((p).range_h - (p).range_l) * (x) + (p).range_l))
#define POS_IN_TORUS(i, size)                                                  \
  ((i < 0) ? size + i : ((size <= i) ? i - size : i))
...
static const gchar *function_names[CML_NUM_VALUES] = {
    N_("Keep image's values"),    N_("Keep the first value"),
    N_("Fill with parameter k"),  N_("k{x(1-x)}^p"),
    N_("k{x(1-x)}^p stepped"),    N_("kx^p"),
    N_("kx^p stepped"),           N_("k(1-x^p)"),
    N_("k(1-x^p) stepped"),       N_("Delta function"),
    N_("Delta function stepped"), N_("sin^p-based function"),
    N_("sin^p, stepped")};
...
static ValueType VALS = {
    ...
    {CML_SIN_CURVE, COMP_NONE, STANDARD, 1, 0.5, 0.7, 2, 0.0, 1, 1.0, 1.0, 0, 1,
     0.0, 0.1},
    {CML_FILL, COMP_NONE, STANDARD, 0, 0.6, 0.1, 2, 0.0, 1, 1.4, 0.9, 0, 0.9,
     0.0, 0.1},
    {CML_FILL, COMP_NONE, STANDARD, 0, 0.5, 0.2, 2, 0.0, 1, 2.0, 1.0, 0, 0.9,
     0.0, 0.1},
    6, /* random value 1 */
    1, /* scale */
    0, /* start_offset */
...
static CML_PARAM *channel_params[] = {&VALS.hue, &VALS.sat, &VALS.val};

static const gchar *channel_names[] = {N_("Hue"), N_("Saturation"),
                                       N_("Value")};

static const gchar *load_channel_names[] = {N_("(None)"), N_("Hue"),
                                            N_("Saturation"), N_("Value")};
...
#define EXPLORER                                                               \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), EXPLORER_TYPE, Explorer))
...
static void CML_compute_next_step(gint size, gdouble **h, gdouble **s,
                                  gdouble **v, gdouble **hn, gdouble **sn,
                                  gdouble **vn, gdouble **haux, gdouble **saux,
                                  gdouble **vaux);
static gdouble CML_next_value(gdouble *vec, gint pos, gint size, gdouble c1,
                              gdouble c2, CML_PARAM *param, gdouble aux);
...
static void CML_explorer_int_entry_init(WidgetEntry *widget_entry,
                                        GtkWidget *scale, gpointer value_ptr);

static void CML_explorer_double_entry_init(WidgetEntry *widget_entry,
                                           GtkWidget *scale,
                                           gpointer value_ptr);

static void CML_explorer_menu_update(GtkWidget *widget, gpointer data);
...
static void CML_explorer_menu_entry_init(WidgetEntry *widget_entry,
                                         GtkWidget *widget, gpointer value_ptr);
...
static void CML_save_to_file_callback(GtkWidget *widget, gpointer data);
static void CML_save_to_file_response(GtkWidget *dialog, gint response_id,
                                      gpointer data);

static void CML_preview_update_callback(GtkWidget *widget, gpointer data);
static void CML_load_from_file_callback(GtkWidget *widget, gpointer data);
static gboolean CML_load_parameter_file(const gchar *filename,
                                        gboolean interactive_mode);
static void CML_load_from_file_response(GtkWidget *dialog, gint response_id,
                                        gpointer data);
static gint parse_line_to_gint(FILE *file, gboolean *flag);
static gdouble parse_line_to_gdouble(FILE *file, gboolean *flag);
...
static void explorer_init(Explorer *explorer) {}
...
static GimpProcedure *explorer_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, explorer_run, NULL, NULL);
    ...
    GIMP_PROC_ARG_STRING(procedure, "parameter-uri", "Parameter UTI",
...
static gboolean CML_main_function(gboolean preview_p) {
  ...
  if (!gimp_drawable_mask_intersect(drawable, &x, &y, &width_by_pixel,
                                    &height_by_pixel))
    return TRUE;
  ...
  for (index = 0; index < cell_num; index++) {
    ...
    case RAND_AND_P:
      haux[index] =
          ((index % (2 * VALS.hue.diffusion_dist) == 0) ? g_rand_double(gr)
                                                        : VALS.hue.power);
      break;
    default:
    ...
    case RAND_AND_P:
      saux[index] =
          ((index % (2 * VALS.sat.diffusion_dist) == 0) ? g_rand_double(gr)
                                                        : VALS.sat.power);
      break;
    default:
    ...
    case RAND_AND_P:
      vaux[index] =
          ((index % (2 * VALS.val.diffusion_dist) == 0) ? g_rand_double(gr)
                                                        : VALS.val.power);
      break;
    default:
  ...
  if (VALS.initial_value == 3) {
    ...
    for (index = 0; index < MIN(cell_num, width_by_pixel / VALS.scale);
         index++) {
      guchar buffer[4];
      GimpRGB rgb;
      GimpHSV hsv;

      gegl_buffer_sample(src_buffer, x + (index * VALS.scale), y, NULL, buffer,
                         src_format, GEGL_SAMPLER_NEAREST, GEGL_ABYSS_NONE);
  ...
  for (dy = 0; dy < height_by_pixel; dy += VALS.scale) {
    ...
    if ((VALS.hue.function == CML_KEEP_VALUES) ||
      ...
      gegl_buffer_get(
          src_buffer, GEGL_RECTANGLE(x, y + dy, width_by_pixel, keep_height),
          1.0, src_format, src_buf, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
    }
    ...
    for (dx = 0; dx < cell_num; dx++) {
      ...
      if (!dest_is_gray) {
        ...
        gimp_hsv_set(&hsv, (gdouble)h / 255.0, (gdouble)s / 255.0,
                     (gdouble)v / 255.0);
      ...
      for (offset_y = 0;
           ...
           offset_y++)
        for (offset_x = 0; (offset_x < VALS.scale) &&
                           (dx * VALS.scale + offset_x < width_by_pixel);
             offset_x++) {
          if ((VALS.hue.function == CML_KEEP_VALUES) ||
              (VALS.sat.function == CML_KEEP_VALUES) ||
              (VALS.val.function == CML_KEEP_VALUES)) {
          ...
          dest_offset =
              (offset_y * dest_bpl + (dx * VALS.scale + offset_x) * dest_bpp);
    ...
    if (preview_p) {
      gimp_preview_area_draw(GIMP_PREVIEW_AREA(preview), 0, dy, width_by_pixel,
                             keep_height, GIMP_RGB_IMAGE, dest_buf, dest_bpl);
    } else {
      gegl_buffer_set(dest_buffer,
                      GEGL_RECTANGLE(x, y + dy, width_by_pixel, keep_height), 0,
                      dest_format, dest_buf, GEGL_AUTO_ROWSTRIDE);
    }
  }
  ...
  if (preview_p) {
    ...
    gimp_drawable_update(drawable, x, y, width_by_pixel, height_by_pixel);
  }
...
static void CML_compute_next_step(gint size, gdouble **h, gdouble **s,
  ...
  for (index = 0; index < size; index++)
    (*hn)[index] =
        CML_next_value(*h, index, size, (*s)[POS_IN_TORUS(index, size)],
                       (*v)[POS_IN_TORUS(index, size)], &VALS.hue,
                       (*haux)[POS_IN_TORUS(index, size)]);
  for (index = 0; index < size; index++)
    (*sn)[index] =
        CML_next_value(*s, index, size, (*v)[POS_IN_TORUS(index, size)],
                       (*h)[POS_IN_TORUS(index, size)], &VALS.sat,
                       (*saux)[POS_IN_TORUS(index, size)]);
  for (index = 0; index < size; index++)
    (*vn)[index] =
        CML_next_value(*v, index, size, (*h)[POS_IN_TORUS(index, size)],
                       (*s)[POS_IN_TORUS(index, size)], &VALS.val,
...
static gdouble CML_next_value(gdouble *vec, gint pos, gint size, gdouble c1,
                              gdouble c2, CML_PARAM *param, gdouble power) {
  gdouble val = vec[pos];
  gdouble diff = 0;
  gdouble self_diff = 0;
  gdouble by_env = 0;
  ...
  case AVERAGE:
    ...
    for (index = 1; index <= param->diffusion_dist / 2; index++)
      env_factor += vec[POS_IN_TORUS(pos + index, size)] +
                    vec[POS_IN_TORUS(pos - index, size)];
    if ((param->diffusion_dist % 2) == 1)
      env_factor += (vec[POS_IN_TORUS(pos + index, size)] +
                     vec[POS_IN_TORUS(pos - index, size)]) /
                    2;
  ...
  if (g_rand_double(gr) < param->mutation_rate) {
    val += ((g_rand_double(gr) < 0.5) ? -1.0 : 1.0) * param->mutation_dist *
           g_rand_double(gr);
  }
...
static gdouble logistic_function(CML_PARAM *param, gdouble x, gdouble power) {
  ...
  case CML_SIN_CURVE:
    if (1.0 < power)
      result =
          0.5 *
          (sin(G_PI * ABS(x1 - 0.5) / power) / sin(G_PI * 0.5 / power) + 1);
    else
...
static gint CML_explorer_dialog(void) {
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(CML_preview_update_callback),
                   &VALS);
  ...
  g_signal_connect(button, "clicked",
                   G_CALLBACK(CML_set_or_randomize_seed_callback), &VALS);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(CML_load_from_file_callback),
                   &VALS);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(CML_save_to_file_callback),
                   &VALS);

  {
...

    {
      ...
      CML_explorer_menu_entry_init(&widget_pointers[3][0], combo,
                                   &VALS.initial_value);
      label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0,
                                       _("Initial value:"), 0.0, 0.5, combo, 2);
      gtk_size_group_add_widget(group, label);
      g_object_unref(group);

      scale = gimp_scale_entry_new(_("Zoom scale:"), VALS.scale, 1, 10, 0);
      gtk_size_group_add_widget(group,
                                gimp_labeled_get_label(GIMP_LABELED(scale)));
      CML_explorer_int_entry_init(&widget_pointers[3][1], scale, &VALS.scale);
      gtk_grid_attach(GTK_GRID(grid), scale, 0, 1, 3, 1);
      gtk_widget_show(scale);

      scale = gimp_scale_entry_new(_("Start offset:"), VALS.start_offset, 0,
                                   100, 0);
      gtk_size_group_add_widget(group,
                                gimp_labeled_get_label(GIMP_LABELED(scale)));
      CML_explorer_int_entry_init(&widget_pointers[3][2], scale,
                                  &VALS.start_offset);
      ...
      gtk_size_group_add_widget(group,
                                gimp_labeled_get_label(GIMP_LABELED(scale)));
      CML_explorer_int_entry_init(&widget_pointers[3][3], scale, &VALS.seed);
...

    {
      ...
      g_signal_connect(combo, "changed",
                       G_CALLBACK(gimp_int_combo_box_get_active), &copy_source);

      label = gimp_grid_attach_aligned(
          GTK_GRID(grid), 0, 0, _("Source channel:"), 0.0, 0.5, combo, 1);
      gtk_size_group_add_widget(group, label);
      g_object_unref(group);
      ...
      label = gimp_grid_attach_aligned(
          GTK_GRID(grid), 0, 1, _("Destination channel:"), 0.0, 0.5, combo, 1);
      gtk_size_group_add_widget(group, label);
      ...
      g_signal_connect(button, "clicked",
                       G_CALLBACK(CML_copy_parameters_callback), &VALS);
  ...
  buffer = cairo_image_surface_create_for_data(
      img, CAIRO_FORMAT_RGB24, GRAPHSIZE, GRAPHSIZE, img_stride);
...
static GtkWidget *CML_dialog_channel_panel_new(CML_PARAM *param,
  ...
  g_signal_connect(combo, "changed", G_CALLBACK(CML_explorer_menu_update),
                   &param->function);

  CML_explorer_menu_entry_init(&widget_pointers[channel_id][index], combo,
                               &param->function);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, index, _("Function type:"), 0.0,
                           0.5, combo, 2);
  index++;

  combo = gimp_int_combo_box_new_array(COMP_NUM_VALUES, composition_names);

  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), param->composition);
  ...
  CML_explorer_menu_entry_init(&widget_pointers[channel_id][index], combo,
                               &param->composition);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, index, _("Composition:"), 0.0,
                           0.5, combo, 2);
  index++;
  ...
  CML_explorer_menu_entry_init(&widget_pointers[channel_id][index], combo,
                               &param->arrange);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, index, _("Misc arrange:"), 0.0,
                           0.5, combo, 2);
  index++;

  toggle = gtk_check_button_new_with_label(_("Use cyclic range"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle), param->cyclic_range);
  gtk_grid_attach(GTK_GRID(grid), toggle, 0, index, 3, 1);
  CML_explorer_toggle_entry_init(&widget_pointers[channel_id][index], toggle,
                                 &param->cyclic_range);
  gtk_widget_show(toggle);
  index++;

  scale = gimp_scale_entry_new(_("Mod. rate:"), param->mod_rate, 0.0, 1.0, 2);
  CML_explorer_double_entry_init(&widget_pointers[channel_id][index], scale,
                                 &param->mod_rate);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, index, 3, 1);
  gtk_widget_show(scale);
  index++;

  scale = gimp_scale_entry_new(_("Env. sensitivity:"), param->env_sensitivity,
                               0.0, 1.0, 2);
  CML_explorer_double_entry_init(&widget_pointers[channel_id][index], scale,
  ...
  scale = gimp_scale_entry_new(_("Diffusion dist.:"), param->diffusion_dist, 2,
                               10, 0);
  CML_explorer_int_entry_init(&widget_pointers[channel_id][index], scale,
  ...
  scale =
      gimp_scale_entry_new(_("# of subranges:"), param->range_num, 1, 10, 0);
  CML_explorer_int_entry_init(&widget_pointers[channel_id][index], scale,
                              &param->range_num);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, index, 3, 1);
  gtk_widget_show(scale);
  ...
  scale =
      gimp_scale_entry_new(_("P(ower factor):"), param->power, 0.0, 10.0, 2);
  ...
  scale =
      gimp_scale_entry_new(_("Parameter k:"), param->parameter_k, 0.0, 10.0, 2);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 0.1, 1.0);
  CML_explorer_double_entry_init(&widget_pointers[channel_id][index], scale,
                                 &param->parameter_k);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, index, 3, 1);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(function_graph_new), chank);
  return grid;
}
...
static GtkWidget *CML_dialog_advanced_panel_new(void) {
  ...
  for (channel_id = 0; channel_id < 3; channel_id++) {
    ...
    scale = gimp_scale_entry_new(_("Ch. sensitivity:"), param->ch_sensitivity,
                                 0.0, 1.0, 2);
    CML_explorer_double_entry_init(
        &widget_pointers[channel_id][index + widget_offset], scale,
        &param->ch_sensitivity);
    gtk_grid_attach(GTK_GRID(grid), scale, 0, index, 3, 1);
    gtk_widget_show(scale);
    index++;

    scale = gimp_scale_entry_new(_("Mutation rate:"), param->mutation_rate, 0.0,
                                 1.0, 2);
    CML_explorer_double_entry_init(
        &widget_pointers[channel_id][index + widget_offset], scale,
        &param->mutation_rate);
    ...
    scale = gimp_scale_entry_new(_("Mutation dist.:"), param->mutation_dist,
                                 0.0, 1.0, 2);
    CML_explorer_double_entry_init(
        &widget_pointers[channel_id][index + widget_offset], scale,
        &param->mutation_dist);
...
static gboolean function_graph_draw(GtkWidget *widget, cairo_t *cr,
                                    gpointer *data) {
  gint x, y;
  gint rgbi[3];
  GimpHSV hsv;
  ...
  for (x = 0; x < GRAPHSIZE; x++) {
    ...
    for (y = 0; y < GRAPHSIZE; y++) {
      GIMP_CAIRO_RGB24_SET_PIXEL((img + (y * img_stride + x * 4)),
                                 ROUND(rgb.r * 255.0), ROUND(rgb.g * 255.0),
                                 ROUND(rgb.b * 255.0));
    }
  }
  ...
  y = 255 * CLAMP(logistic_function(param, 0, param->power), 0, 1.0);
  cairo_move_to(cr, 0, 255 - y);
  for (x = 0; x < GRAPHSIZE; x++) {
    /* curve */
    y = 255 *
        CLAMP(logistic_function(param, x / (gdouble)255, param->power), 0, 1.0);
...
static void function_graph_new(GtkWidget *widget, gpointer *data) {
  ...
  gtk_container_add(GTK_CONTAINER(frame), preview);
  gtk_widget_show(preview);
  g_signal_connect(preview, "draw", G_CALLBACK(function_graph_draw), data);
...
static void CML_copy_parameters_callback(GtkWidget *widget, gpointer data) {
  ...
  if (copy_source == copy_destination) {
    g_message(
        _("Warning: the source and the destination are the same channel."));
    return;
  }
...
static void CML_save_to_file_callback(GtkWidget *widget, gpointer data) {
  ...
  if (!dialog) {
    dialog = gtk_file_chooser_dialog_new(
        _("Save CML Explorer Parameters"),
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(CML_save_to_file_response),
                     NULL);
...
static void CML_save_to_file_response(GtkWidget *dialog, gint response_id,
  ...
  for (channel_id = 0; channel_id < 3; channel_id++) {
    ...
    fprintf(file, "Function_type    : %d (%s)\n", param.function,
            function_names[param.function]);
    fprintf(file, "Compostion_type  : %d (%s)\n", param.composition,
            composition_names[param.composition]);
    fprintf(file, "Arrange          : %d (%s)\n", param.arrange,
            arrange_names[param.arrange]);
    fprintf(file, "Cyclic_range     : %d (%s)\n", param.cyclic_range,
            (param.cyclic_range ? "TRUE" : "FALSE"));
  ...
  fprintf(file, "Initial value  : %d (%s)\n", VALS.initial_value,
          initial_value_names[VALS.initial_value]);
...
static void CML_load_from_file_callback(GtkWidget *widget, gpointer data) {
  ...
  if (!dialog) {
    dialog = gtk_file_chooser_dialog_new(
        _("Load CML Explorer Parameters"),
        GTK_WINDOW(gtk_widget_get_toplevel(widget)),
    ...
    g_signal_connect(dialog, "response",
                     G_CALLBACK(CML_load_from_file_response), NULL);
    g_signal_connect(dialog, "delete-event", G_CALLBACK(gtk_true), NULL);
  }
  ...
  if (!flag) {
    ...
    if ((selective_load_source == 0) || (selective_load_destination == 0)) {
      ...
      memcpy((CML_PARAM *)&VALS + (selective_load_destination - 1),
             (void *)&ch[selective_load_source - 1], sizeof(CML_PARAM));
    }
...
static void CML_explorer_toggle_update(GtkWidget *widget, gpointer data) {
  gimp_toggle_button_update(widget, data);

  preview_update();
}

static void CML_explorer_toggle_entry_change_value(WidgetEntry *widget_entry) {
...
static void CML_explorer_toggle_entry_init(WidgetEntry *widget_entry,
                                           GtkWidget *widget,
                                           gpointer value_ptr) {
  g_signal_connect(widget, "toggled", G_CALLBACK(CML_explorer_toggle_update),
                   value_ptr);
...
static void CML_explorer_int_adjustment_update(GimpLabelSpin *scale,
                                               gint *value) {
...
static void CML_explorer_int_entry_change_value(WidgetEntry *widget_entry) {
...
static void CML_explorer_int_entry_init(WidgetEntry *widget_entry,
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(CML_explorer_int_adjustment_update), value_ptr);
...
static void CML_explorer_double_adjustment_update(GimpLabelSpin *scale,
                                                  gdouble *value) {
...
static void CML_explorer_double_entry_change_value(WidgetEntry *widget_entry) {
  GimpLabelSpin *scale = GIMP_LABEL_SPIN(widget_entry->widget);
...
static void CML_explorer_menu_entry_change_value(WidgetEntry *widget_entry) {
