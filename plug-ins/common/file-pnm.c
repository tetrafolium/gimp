...
typedef void (*PNMLoaderFunc)(PNMScanner *scanner, PNMInfo *info,
                              GeglBuffer *buffer);
typedef gboolean (*PNMSaverowFunc)(PNMRowInfo *info, guchar *data,
                                   GError **error);
...
static GimpValueArray *pnm_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *pnm_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static gint save_image(GFile *file, GimpImage *image, GimpDrawable *drawable,
                       FileType file_type, GObject *config, GError **error);

static gboolean save_dialog(GimpProcedure *procedure, GObject *config);

static void pnm_load_ascii(PNMScanner *scan, PNMInfo *info, GeglBuffer *buffer);
static void pnm_load_raw(PNMScanner *scan, PNMInfo *info, GeglBuffer *buffer);
static void pnm_load_rawpbm(PNMScanner *scan, PNMInfo *info,
                            GeglBuffer *buffer);
static void pnm_load_rawpfm(PNMScanner *scan, PNMInfo *info,
                            GeglBuffer *buffer);

static gboolean pnmsaverow_ascii(PNMRowInfo *ri, guchar *data, GError **error);
static gboolean pnmsaverow_raw(PNMRowInfo *ri, guchar *data, GError **error);
static gboolean pnmsaverow_raw_pbm(PNMRowInfo *ri, guchar *data,
                                   GError **error);
static gboolean pnmsaverow_ascii_pbm(PNMRowInfo *ri, guchar *data,
                                     GError **error);
static gboolean pnmsaverow_ascii_indexed(PNMRowInfo *ri, guchar *data,
                                         GError **error);
static gboolean pnmsaverow_raw_indexed(PNMRowInfo *ri, guchar *data,
                                       GError **error);

static PNMScanner *pnmscanner_create(GInputStream *input);
static void pnmscanner_destroy(PNMScanner *s);
static void pnmscanner_createbuffer(PNMScanner *s, gint bufsize);
static void pnmscanner_getchar(PNMScanner *s);
static void pnmscanner_eatwhitespace(PNMScanner *s);
static void pnmscanner_gettoken(PNMScanner *s, gchar *buf, gint bufsize);
static void pnmscanner_getsmalltoken(PNMScanner *s, gchar *buf);
...
static void pnm_class_init(PnmClass *klass) {
...
static GimpProcedure *pnm_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, pnm_load, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Erik Nygren", "Erik Nygren",
                                   "1996");
    ...
    procedure =
        gimp_save_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                pnm_save, GINT_TO_POINTER(FILE_TYPE_PNM), NULL);
    ...
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-portable-anymap");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "pnm");

    GIMP_PROC_ARG_BOOLEAN(procedure, "raw", "Raw",
                          "TRUE for raw output, FALSE for ascii output", TRUE,
                          G_PARAM_READWRITE);
  } else if (!strcmp(name, PBM_SAVE_PROC)) {
    procedure =
        gimp_save_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                pnm_save, GINT_TO_POINTER(FILE_TYPE_PBM), NULL);
    ...
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "pbm");
    ...
    procedure =
        gimp_save_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                pnm_save, GINT_TO_POINTER(FILE_TYPE_PGM), NULL);
    ...
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "pgm");
    ...
    procedure =
        gimp_save_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                pnm_save, GINT_TO_POINTER(FILE_TYPE_PPM), NULL);
    ...
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "ppm");
    ...
    procedure =
        gimp_save_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                pnm_save, GINT_TO_POINTER(FILE_TYPE_PFM), NULL);
    ...
    gimp_procedure_set_attribution(procedure, "Mukund Sivaraman",
                                   "Mukund Sivaraman", "2015");

    gimp_file_procedure_set_handles_remote(GIMP_FILE_PROCEDURE(procedure),
                                           TRUE);
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-portable-floatmap");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "pfm");
  }
...
static GimpValueArray *pnm_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  if (n_drawables != 1) {
    g_set_error(&error, G_FILE_ERROR, 0,
                _("%s format does not support multiple layers."), format_name);

    return gimp_procedure_new_return_values(procedure, GIMP_PDB_CALLING_ERROR,
                                            error);
  }

  if (file_type != FILE_TYPE_PFM && run_mode == GIMP_RUN_INTERACTIVE) {
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  if (pnminfo->float_format) {
    ...
    CHECK_FOR_ERROR(!isnormal(pnminfo->scale_factor), pnminfo->jmpbuf,
                    _("Unsupported scale factor."));
   ...
   * with it.
   */
  image = gimp_image_new_with_precision(
      pnminfo->xres, pnminfo->yres, (pnminfo->np >= 3) ? GIMP_RGB : GIMP_GRAY,
      precision);
...
static void pnm_load_ascii(PNMScanner *scan, PNMInfo *info,
                           GeglBuffer *buffer) {
  gint bpc;
  guchar *data, *d;
  gushort *s;
      ...
      for (x = 0; x < info->xres; x++) {
        for (b = 0; b < np; b++) {
          ...
          if (info->maxval == 1) {
            ...
            s[b] =
                (65535.0 * (((gdouble)(g_ascii_isdigit(*buf) ? atoi(buf) : 0)) /
                            (gdouble)(info->maxval)));
          } else {
            d[b] =
                (255.0 * (((gdouble)(g_ascii_isdigit(*buf) ? atoi(buf) : 0)) /
                          (gdouble)(info->maxval)));
          }
        }
  ...
  for (y = 0; y < info->yres; y += scanlines) {
    ...
    for (i = 0; i < scanlines; i++) {
      ...
      if (g_input_stream_read_all(input, d, info->xres * info->np * bpc,
        ...
        CHECK_FOR_ERROR(info->xres * info->np * bpc != bytes_read, info->jmpbuf,
                        _("Premature end of file."));
      } else {
...
static void pnm_load_rawpbm(PNMScanner *scan, PNMInfo *info,
  ...
  for (y = 0; y < info->yres; y += scanlines) {
    ...
    for (i = 0; i < scanlines; i++) {
      ...
      if (g_input_stream_read_all(input, buf, rowlen, &bytes_read, NULL,
                                  &error)) {
        CHECK_FOR_ERROR(rowlen != bytes_read, info->jmpbuf,
                        _("Premature end of file."));
      } else {
        CHECK_FOR_ERROR(FALSE, info->jmpbuf, "%s", error->message);
      }
...
static void pnm_load_rawpfm(PNMScanner *scan, PNMInfo *info,
  ...
  for (y = info->yres - 1; y >= 0; y--) {
    ...
    if (g_input_stream_read_all(input, data,
                                ...
                                &bytes_read, NULL, &error)) {
      CHECK_FOR_ERROR(info->xres * info->np * sizeof(float) != bytes_read,
    ...
    gegl_buffer_set(buffer, GEGL_RECTANGLE(0, y, info->xres, 1), 0, NULL, data,
                    GEGL_AUTO_ROWSTRIDE);
...
static gboolean output_write(GOutputStream *output, gconstpointer buffer,
...
static gboolean pnmsaverow_float(PNMRowInfo *ri, const float *data,
                                 GError **error) {
  return output_write(ri->output, data, ri->xres * ri->np * sizeof(float),
                      error);
}
...
static gboolean pnmsaverow_ascii(PNMRowInfo *ri, guchar *data, GError **error) {
  ...
  return output_write(ri->output, ri->rowbuf, rbcur - ri->rowbuf, error);
}
...
static gboolean save_image(GFile *file, GimpImage *image,
                           GimpDrawable *drawable, FileType file_type,
  ...
  if (file_type != FILE_TYPE_PFM)
    g_object_get(config, "raw", &config_raw, NULL);
  ...
  if (drawable_type == GIMP_INDEXED_IMAGE) {
    ...
    if (file_type == FILE_TYPE_PBM) {
      ...
      switch (num_colors) {
      case 1:
        rowinfo.zero_is_black =
            (GIMP_RGB_LUMINANCE(cmap[0], cmap[1], cmap[2]) < 128);
        break;

      case 2:
        rowinfo.zero_is_black = (GIMP_RGB_LUMINANCE(cmap[0], cmap[1], cmap[2]) <
                                 GIMP_RGB_LUMINANCE(cmap[3], cmap[4], cmap[5]));
        break;
  ...
  if (file_type != FILE_TYPE_PFM) {
    ...
    data = g_new(guchar, gimp_tile_height() * xres *
    ...
    for (ypos = 0; ypos < yres; ypos++) {
      if ((ypos % gimp_tile_height()) == 0) {
        ...
        gegl_buffer_get(buffer, GEGL_RECTANGLE(0, ypos, xres, yend - ypos), 1.0,
    ...
    for (ypos = yres - 1; ypos >= 0; ypos--) {
      gegl_buffer_get(buffer, GEGL_RECTANGLE(0, ypos, xres, 1), 1.0, format,
...
static gboolean save_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  /*  file save type  */
  frame = gimp_prop_boolean_radio_frame_new(config, "raw", _("Data formatting"),
                                            _("_Raw"), _("_ASCII"));
...
static PNMScanner *pnmscanner_create(GInputStream *input) {
  ...
  if (!g_input_stream_read_all(input, &s->cur, 1, &bytes_read, NULL, NULL) ||
      bytes_read != 1) {
    s->eof = TRUE;
  }
...
static void pnmscanner_createbuffer(PNMScanner *s, gint bufsize) {
  ...
  g_input_stream_read_all(s->input, s->inbuf, bufsize, &bytes_read, NULL, NULL);
...
static void pnmscanner_gettoken(PNMScanner *s, gchar *buf, gint bufsize) {
  ...
  while (!s->eof && !g_ascii_isspace(s->cur) && (s->cur != '#') &&
         (ctr < bufsize)) {
    buf[ctr++] = s->cur;
  ...
  if (s->inbuf) {
    ...
    if (s->inbufpos >= s->inbufvalidsize) {
      if (s->inbufpos > s->inbufvalidsize) {
        ...
        g_input_stream_read_all(s->input, s->inbuf, s->inbufsize, &bytes_read,
                                NULL, NULL);
    ...
    if (!g_input_stream_read_all(s->input, &s->cur, 1, &bytes_read, NULL,
                                 NULL) ||
        bytes_read != 1) {
      s->eof = TRUE;
    }
