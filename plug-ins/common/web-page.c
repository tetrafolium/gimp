...
static void webpage_class_init(WebpageClass *klass) {
...
static void webpage_init(Webpage *webpage) {}
...
static GimpProcedure *webpage_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    ...
    gimp_procedure_set_attribution(
        procedure, "Mukund Sivaraman <muks@banu.com>", "2011", "2011");

    GIMP_PROC_ARG_ENUM(procedure, "run-mode", "Run mode", "The run mode",
                       GIMP_TYPE_RUN_MODE, GIMP_RUN_INTERACTIVE,
                       G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "url", "URL",
                         "URL of the webpage to screenshot",
                         "http://www.gimp.org/", G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "width", "Width",
                      "The width of the screenshot (in pixels)", 100,
                      GIMP_MAX_IMAGE_SIZE, 1024, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "font-size", "Font size",
                      "The font size to use in the page (in pt)", 1, 1000, 12,
                      G_PARAM_READWRITE);

    GIMP_PROC_VAL_IMAGE(procedure, "image", "Image", "The output image", FALSE,
                        G_PARAM_READWRITE);
  }
...
static gboolean webpage_dialog(void) {
  ...
  dialog = gimp_dialog_new(_("Create from webpage"), PLUG_IN_ROLE, NULL, 0,
                           gimp_standard_help_func, PLUG_IN_PROC,

                           _("_Cancel"), GTK_RESPONSE_CANCEL, _("Cre_ate"),
                           GTK_RESPONSE_OK,
  ...
  if (webpagevals.url)
    gtk_entry_set_text(GTK_ENTRY(entry), webpagevals.url);
  ...
  adjustment = gtk_adjustment_new(webpagevals.width, 1, 8192, 1, 10, 0);
  ...
  combo = gimp_int_combo_box_new(_("Huge"), 16, _("Large"), 14,
                                 C_("web-page", "Default"), 12, _("Small"), 10,
                                 _("Tiny"), 8, NULL);
...
static void notify_progress_cb(WebKitWebView *view, GParamSpec *pspec,
                               gpointer user_data) {
...
static void snapshot_ready(GObject *source_object, GAsyncResult *result,
                           gpointer user_data) {
  WebKitWebView *view = WEBKIT_WEB_VIEW(source_object);
  cairo_surface_t *surface;

  surface =
      webkit_web_view_get_snapshot_finish(view, result, &webpagevals.error);
...
static gboolean load_finished_idle(gpointer data) {
  static gint count = 0;
  ...
  webkit_web_view_get_snapshot(
      ...
      WEBKIT_SNAPSHOT_OPTIONS_NONE, NULL, snapshot_ready, NULL);
...
static void load_changed_cb(WebKitWebView *view, WebKitLoadEvent event,
                            gpointer user_data) {
...
static GimpImage *webpage_capture(void) {
  ...
  if (webpagevals.width < 32) {
    g_warning("Width '%d' is too small. Clamped to 32.", webpagevals.width);
    webpagevals.width = 32;
  } else if (webpagevals.width > 8192) {
    g_warning("Width '%d' is too large. Clamped to 8192.", webpagevals.width);
  ...
  ua = g_strdup_printf("%s GIMP/%s", webkit_settings_get_user_agent(settings),
                       GIMP_VERSION);
  ...
  g_signal_connect(view, "load-failed", G_CALLBACK(load_failed_cb), window);
  g_signal_connect(view, "load-changed", G_CALLBACK(load_changed_cb), window);

  gimp_progress_init_printf(_("Downloading webpage '%s'"), webpagevals.url);

  webkit_web_view_load_uri(WEBKIT_WEB_VIEW(view), webpagevals.url);
