...
static GimpValueArray *remap_run(GimpProcedure *procedure, GimpRunMode run_mode,
                                 GimpImage *image, GimpDrawable *drawable,
                                 const GimpValueArray *args, gpointer run_data);

static gboolean remap(GimpImage *image, gint num_colors, guchar *map);

static gboolean remap_dialog(GimpImage *image, guchar *map);

G_DEFINE_TYPE(Remap, remap, GIMP_TYPE_PLUG_IN)

GIMP_MAIN(REMAP_TYPE)

static void remap_class_init(RemapClass *klass) {
...
static void remap_init(Remap *remap) {}
...
static GimpProcedure *remap_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC_REMAP)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, remap_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure, N_("Rearrange the colormap"),
    ...
    gimp_procedure_set_attribution(
        ...
        "Mukund Sivaraman <muks@mukund.org>", "June 2006");

    GIMP_PROC_ARG_INT(procedure, "num-colors", "Num colors",
                      "Length of 'map' argument", 1, 256, 1, G_PARAM_READWRITE);

    GIMP_PROC_ARG_UINT8_ARRAY(procedure, "map", "Map",
    ...
    GIMP_PROC_ARG_UCHAR(procedure, "index1", "Index 1",
                        "First index in the colormap", 0, 255, 0,
                        G_PARAM_READWRITE);

    GIMP_PROC_ARG_UCHAR(procedure, "index2", "Index 2",
                        "Second (other) index in the colormap", 0, 255, 0,
                        G_PARAM_READWRITE);
  }
...
static GimpValueArray *remap_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  if (strcmp(gimp_procedure_get_name(procedure), PLUG_IN_PROC_REMAP) == 0) {
    gint n_cols;
    gint n_col_args;
    ...
    if (run_mode != GIMP_RUN_NONINTERACTIVE)
      gimp_displays_flush();
  } else if (strcmp(gimp_procedure_get_name(procedure), PLUG_IN_PROC_SWAP) ==
             0) {
...
static gboolean remap(GimpImage *image, gint num_colors, guchar *map) {
  ...
  for (list = layers; list; list = list->next) {
    ...
    iter =
        gegl_buffer_iterator_new(buffer, GEGL_RECTANGLE(0, 0, width, height), 0,
                                 format, GEGL_ACCESS_READ, GEGL_ABYSS_NONE, 2);
    src_roi = &iter->items[0].roi;

    gegl_buffer_iterator_add(iter, shadow, GEGL_RECTANGLE(0, 0, width, height),
                             0, format, GEGL_ACCESS_WRITE, GEGL_ABYSS_NONE);
...
static void remap_sort(GtkTreeSortable *store, gint column, GtkSortType order) {
  gtk_tree_sortable_set_sort_column_id(store, column, order);
  gtk_tree_sortable_set_sort_column_id(
      store, GTK_TREE_SORTABLE_UNSORTED_SORT_COLUMN_ID, 0);
}

static void remap_sort_callback(GtkAction *action, GtkTreeSortable *store) {
...
static void remap_reset_callback(GtkAction *action, GtkTreeSortable *store) {
  remap_sort(store, COLOR_INDEX, GTK_SORT_ASCENDING);
}

static void remap_reverse_callback(GtkAction *action, GtkListStore *store) {
  gtk_list_store_reorder(store, reverse_order);
}

static GtkUIManager *remap_ui_manager_new(GtkWidget *window,
                                          GtkListStore *store) {
...
static gboolean remap_popup_menu(GtkWidget *widget, GdkEventButton *event) {
...
static gboolean remap_button_press(GtkWidget *widget, GdkEventButton *event) {
...
static void remap_response(GtkWidget *dialog, gint response_id,
...
static gboolean remap_dialog(GimpImage *image, guchar *map) {
  ...
  dialog = gimp_dialog_new(_("Rearrange Colormap"), PLUG_IN_ROLE, NULL, 0,
  ...
  store = gtk_list_store_new(NUM_COLS, G_TYPE_INT, G_TYPE_STRING, GIMP_TYPE_RGB,
  ...
  for (i = 0; i < ncols; i++) {
    ...
    gimp_rgb_set_uchar(&rgb, cmap[index * 3], cmap[index * 3 + 1],
                       cmap[index * 3 + 2]);
    ...
    gtk_list_store_set(store, &iter, COLOR_INDEX, index, COLOR_INDEX_TEXT, text,
                       COLOR_RGB, &rgb, COLOR_H, hsv.h, COLOR_S, hsv.s, COLOR_V,
                       hsv.v, -1);
    g_free(text);
  }
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(iconview), renderer, "color",
                                 COLOR_RGB, NULL);
  g_object_set(renderer, "width", 24, NULL);

  renderer = gtk_cell_renderer_text_new();
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(iconview), renderer, TRUE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(iconview), renderer, "text",
                                 COLOR_INDEX_TEXT, NULL);
  g_object_set(renderer, "size-points", 6.0, "xalign", 0.5, "ypad", 0, NULL);

  g_signal_connect(iconview, "popup-menu", G_CALLBACK(remap_popup_menu), NULL);

  g_signal_connect(iconview, "button-press-event",
                   G_CALLBACK(remap_button_press), NULL);
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(remap_response), store);
  ...
  for (valid = gtk_tree_model_get_iter_first(GTK_TREE_MODEL(store), &iter);
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(store), &iter, COLOR_INDEX, &index, -1);
    map[i++] = index;
  }
