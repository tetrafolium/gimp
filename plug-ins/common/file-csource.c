...
static void csource_class_init(CsourceClass *klass) {
...
static void csource_init(Csource *csource) {}
...
static GimpProcedure *csource_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, csource_save, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Tim Janik", "Tim Janik", "1999");

    gimp_file_procedure_set_handles_remote(GIMP_FILE_PROCEDURE(procedure),
                                           TRUE);
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-csrc");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "c");

    GIMP_PROC_AUX_ARG_STRING(procedure, "prefixed-name", "Prefixed name",
                             "Prefixed name", "gimp_image",
                             GIMP_PARAM_READWRITE);

    GIMP_PROC_AUX_ARG_STRING(procedure, "gimp-comment", "Comment", "Comment",
                             gimp_get_default_comment(), GIMP_PARAM_READWRITE);
    ...
    GIMP_PROC_AUX_ARG_BOOLEAN(procedure, "save-comment", "Save comment",
                              "Save comment", gimp_export_comment(),
                              GIMP_PARAM_READWRITE);

    GIMP_PROC_AUX_ARG_BOOLEAN(procedure, "glib-types", "GLib types",
                              "Use GLib types", TRUE, GIMP_PARAM_READWRITE);

    GIMP_PROC_AUX_ARG_BOOLEAN(procedure, "save-alpha", "Save alpha",
                              "Save the alpha channel", FALSE,
                              GIMP_PARAM_READWRITE);

    GIMP_PROC_AUX_ARG_BOOLEAN(procedure, "rgb565", "RGB565",
                              "Use RGB565 encoding", FALSE,
                              GIMP_PARAM_READWRITE);

    GIMP_PROC_AUX_ARG_BOOLEAN(procedure, "use-macros", "Use macros",
    ...
    GIMP_PROC_AUX_ARG_DOUBLE(procedure, "opacity", "Opacity", "Opacity", 0.0,
                             100.0, 100.0, GIMP_PARAM_READWRITE);
  }
...
static GimpValueArray *csource_save(GimpProcedure *procedure,
  ...
  g_object_set(config, "save-alpha", gimp_drawable_has_alpha(drawables[0]),
               NULL);
  ...
  g_object_get(config, "prefixed-name", &prefixed_name, "gimp-comment",
               &comment, NULL);

  if (!prefixed_name || !prefixed_name[0])
    g_object_set(config, "prefixed-name", "tmp", NULL);

  if (comment && !comment[0])
    g_object_set(config, "gimp-comment", NULL, NULL);
...
static gboolean diff2_rgb565(guint8 *ip) {
...
static guint8 *rl_encode_rgbx(guint8 *bp, guint8 *ip, guint8 *limit,
                              guint bpp) {
  ...
  switch (bpp) {
  case 2:
    diff2_pix = diff2_rgb565;
    break;
  case 3:
    diff2_pix = diff2_rgb;
    break;
  case 4:
    diff2_pix = diff2_rgba;
    break;
  default:
...
static gboolean print(GOutputStream *stream, GError **error,
...
static gboolean print(GOutputStream *stream, GError **error,
  ...
  success = g_output_stream_vprintf(stream, NULL, NULL, error, format, args);
  va_end(args);

  return success;
}

static inline gboolean save_rle_decoder(GOutputStream *output,
                                        const gchar *macro_name,
                                        const gchar *s_uint,
                                        const gchar *s_uint_8, guint bpp,
                                        GError **error) {
  return print(output, error,
               "#define %s_RUN_LENGTH_DECODE(image_buf, rle_data, size, bpp) "
               "do \\\n",
               macro_name) &&
         print(output, error,
               ...
               s_uint_8, s_uint_8) &&
         print(output, error,
               "  __bpp = (bpp); __ip = (image_buf); __il = __ip + (size) * "
               "__bpp; \\\n"
         ...
         print(output, error,
               ...
               "        do { memcpy (__ip, __rd, 4); __ip += 4; } while "
               "(--__l); __rd += 4; \\\n"
         ...
         print(output, error,
               "      if (__l & 128) { __l = __l - 128; \\\n"
               "        do { memcpy (__ip, __rd, 3); __ip += 3; } while "
               "(--__l); __rd += 3; \\\n"
         ...
         print(output, error,
               "      if (__l & 128) { __l = __l - 128; \\\n"
               "        do { memcpy (__ip, __rd, 2); __ip += 2; } while "
               "(--__l); __rd += 2; \\\n"
...
static inline gboolean save_uchar(GOutputStream *output, guint *c, guint8 d,
                                  gboolean use_macros, GError **error) {
  static guint8 pad = 0;
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  g_object_get(config, "prefixed-name", &config_prefixed_name, "gimp-comment",
               &config_comment, "save-comment", &config_save_comment,
               "glib-types", &config_glib_types, "save-alpha",
               &config_save_alpha, "rgb565", &config_rgb565, "use-macros",
               &config_use_macros, "use-rle", &config_use_rle, "opacity",
               &config_opacity, NULL);
  ...
  for (y = 0; y < height; y++) {
    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, y, width, 1), 1.0,
                    drawable_format, data, GEGL_AUTO_ROWSTRIDE,
                    GEGL_ABYSS_NONE);
  ...
  if (config_use_rle) {
    img_buffer_end =
        rl_encode_rgbx(data, img_buffer, img_buffer + n_bytes, bpp);
    img_buffer = data;
  } else {
  ...
  if (!print(output, error, "/* GIMP %s C-Source image dump %s(%s) */\n\n",
             config_save_alpha ? "RGBA" : "RGB",
             config_use_rle ? "1-byte-run-length-encoded " : "", basename))
    goto fail;
  ...
  if (config_use_rle && !config_use_macros) {
    if (!save_rle_decoder(
            output, macro_name, config_glib_types ? "guint" : "unsigned int",
            config_glib_types ? "guint8" : "unsigned char", bpp, error))
      goto fail;
  }
  ...
  if (!config_use_macros) {
    ...
    if (!print(output, error, "  %s\t %spixel_data[", s_uint_8,
               config_use_rle ? "rle_" : ""))
      goto fail;

    if (config_use_rle) {
      if (!print(output, error, "%u + 1];\n",
                 (guint)(img_buffer_end - img_buffer)))
        goto fail;
    } else {
      if (!print(output, error, "%u * %u * %u + 1];\n", width, height, bpp))
        goto fail;
    }
  ...
  } else /* use macros */
  {
    if (!print(output, error,
               ...
               macro_name, width, macro_name, height, macro_name, bpp)) {
      goto fail;
    }
  }
  ...
  if (config_save_comment && !config_comment) {
    if (!config_use_macros) {
      ...
      if (!print(output, error, "#define %s_COMMENT (%s)\n", macro_name,
                 s_null))
        goto fail;
    }
  ...
  if (config_use_macros) {
    if (!print(output, error,
               ...
               config_use_rle ? "RLE_" : "", s_uint_8, macro_name,
               config_use_rle ? "rle_" : ""))
      goto fail;

    if (config_use_rle) {
      if (!save_rle_decoder(output, macro_name, s_uint, s_uint_8, bpp, error))
        goto fail;
    }

    if (!print(output, error, "static const %s %s_%spixel_data[", s_uint_8,
               macro_name, config_use_rle ? "rle_" : ""))
      goto fail;

    if (config_use_rle) {
      if (!print(output, error, "%u] =\n",
                 (guint)(img_buffer_end - img_buffer)))
        goto fail;
    } else {
      if (!print(output, error, "%u * %u * %u + 1] =\n", width, height, bpp))
  ...
  case GIMP_RGBA_IMAGE:
    do {
      if (!save_uchar(output, &c, *(img_buffer++), config_use_macros, error))
        goto fail;
...
static gboolean save_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Prefixed name:"), 0.0, 0.5,
                           entry, 1);

  /* Comment Entry
   */
  entry = gimp_prop_entry_new(config, "gimp-comment", -1);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Co_mment:"), 0.0, 0.5,
                           entry, 1);
  ...
  g_object_bind_property(config, "rgb565", toggle, "sensitive",
  ...
  /* Max Alpha Value
   */
  scale = gimp_prop_scale_entry_new(config, "opacity", _("Op_acity:"), 1.0,
                                    FALSE, 0, 0);
