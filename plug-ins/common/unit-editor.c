...
static GimpUnit new_unit_dialog(GtkWidget *main_dialog, GimpUnit template);
static void unit_editor_dialog(void);
static void unit_editor_response(GtkWidget *widget, gint response_id,
                                 gpointer data);
static void new_callback(GtkAction *action, GtkTreeView *tv);
static void duplicate_callback(GtkAction *action, GtkTreeView *tv);
...
static GtkActionEntry actions[] = {
    {"unit-editor-toolbar", NULL, "Unit Editor Toolbar", NULL, NULL, NULL},

    {"unit-editor-new", GIMP_ICON_DOCUMENT_NEW, NULL, "<control>N",
     N_("Create a new unit from scratch"), G_CALLBACK(new_callback)},

    {"unit-editor-duplicate", GIMP_ICON_OBJECT_DUPLICATE, NULL, "<control>D",
...
static GimpUnit new_unit_dialog(GtkWidget *main_dialog, GimpUnit template) {
  ...
  dialog = gimp_dialog_new(
      _("Add a New Unit"), PLUG_IN_ROLE, main_dialog, GTK_DIALOG_MODAL,
      gimp_standard_help_func, PLUG_IN_PROC,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Add"), GTK_RESPONSE_OK,
  ...
  if (template != GIMP_UNIT_PIXEL) {
    gtk_entry_set_text(GTK_ENTRY(entry), gimp_unit_get_identifier(template));
  }
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_ID:"), 0.0, 0.5, entry, 1);

  gimp_help_set_help_data(entry, gettext(columns[IDENTIFIER].help), NULL);

  factor_adj = gtk_adjustment_new(
      (template != GIMP_UNIT_PIXEL) ? gimp_unit_get_factor(template) : 1.0,
      GIMP_MIN_RESOLUTION, GIMP_MAX_RESOLUTION, 0.01, 0.1, 0.0);
  spinbutton = gimp_spin_button_new(factor_adj, 0.01, 5);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Factor:"), 0.0, 0.5,
                           spinbutton, 1);

  gimp_help_set_help_data(spinbutton, gettext(columns[FACTOR].help), NULL);

  digits_adj = gtk_adjustment_new(
      (template != GIMP_UNIT_PIXEL) ? gimp_unit_get_digits(template) : 2.0, 0,
      5, 1, 1, 0);
  spinbutton = gimp_spin_button_new(digits_adj, 0, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("_Digits:"), 0.0, 0.5,
                           spinbutton, 1);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("_Symbol:"), 0.0, 0.5, entry,
                           1);
  ...
  if (template != GIMP_UNIT_PIXEL) {
    gtk_entry_set_text(GTK_ENTRY(entry), gimp_unit_get_abbreviation(template));
  }
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 4, _("_Abbreviation:"), 0.0, 0.5,
                           entry, 1);
  ...
  if (template != GIMP_UNIT_PIXEL) {
    gtk_entry_set_text(GTK_ENTRY(entry), gimp_unit_get_singular(template));
  }
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 5, _("Si_ngular:"), 0.0, 0.5,
                           entry, 1);
  ...
  if (template != GIMP_UNIT_PIXEL) {
    gtk_entry_set_text(GTK_ENTRY(entry), gimp_unit_get_plural(template));
  }
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 6, _("_Plural:"), 0.0, 0.5, entry,
                           1);
  ...
  while (TRUE) {
    ...
    if (!strlen(identifier) || !strlen(symbol) || !strlen(abbreviation) ||
        !strlen(singular) || !strlen(plural)) {
      GtkWidget *msg =
          gtk_message_dialog_new(GTK_WINDOW(dialog), 0, GTK_MESSAGE_ERROR,
                                 GTK_BUTTONS_OK, _("Incomplete input"));

      gtk_message_dialog_format_secondary_text(
          GTK_MESSAGE_DIALOG(msg), _("Please fill in all text fields."));
...
static void unit_editor_dialog(void) {
  ...
  list_store = gtk_list_store_new(NUM_COLUMNS, G_TYPE_BOOLEAN, /*  SAVE */
  ...
  dialog = gimp_dialog_new(_("Unit Editor"), PLUG_IN_ROLE, NULL, 0,
                           gimp_standard_help_func, PLUG_IN_PROC,

                           _("_Refresh"), RESPONSE_REFRESH, _("_Close"),
                           GTK_RESPONSE_CLOSE,
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(unit_editor_response), tv);
  ...
  col = gtk_tree_view_column_new_with_attributes(
      gettext(columns[SAVE].title), rend, "active", SAVE, "activatable",
      USER_UNIT, "cell-background-gdk", BG_COLOR, NULL);
  ...
  if (col_widget) {
    ...
    if (button)
      gimp_help_set_help_data(button, gettext(columns[SAVE].help), NULL);
  }

  g_signal_connect(rend, "toggled", G_CALLBACK(saved_toggled_callback),
                   list_store);
  ...
  for (i = 0; i < G_N_ELEMENTS(columns); i++) {
    ...
    col = gtk_tree_view_column_new_with_attributes(
        gettext(columns[i].title), gtk_cell_renderer_text_new(), "text", i,
        "cell-background-gdk", BG_COLOR, NULL);
...
static void new_callback(GtkAction *action, GtkTreeView *tv) {
  ...
  unit =
      new_unit_dialog(gtk_widget_get_toplevel(GTK_WIDGET(tv)), GIMP_UNIT_PIXEL);
...
static void duplicate_callback(GtkAction *action, GtkTreeView *tv) {
  ...
  if (gtk_tree_selection_get_selected(sel, NULL, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, UNIT, &unit, -1);

    unit = new_unit_dialog(gtk_widget_get_toplevel(GTK_WIDGET(tv)), unit);
...
static void saved_toggled_callback(GtkCellRendererToggle *celltoggle,
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(list_store), &iter, SAVE, &saved, UNIT,
                     &unit, -1);

  if (unit >= gimp_unit_get_number_of_built_in_units()) {
    gimp_unit_set_deletion_flag(unit, saved);
    gtk_list_store_set(GTK_LIST_STORE(list_store), &iter, SAVE, !saved, -1);
  }
}
...
static void unit_list_init(GtkTreeView *tv) {
  ...
  for (unit = GIMP_UNIT_INCH; unit < num_units; unit++) {
    ...
    gtk_list_store_set(
        list_store, &iter, SAVE, !gimp_unit_get_deletion_flag(unit), IDENTIFIER,
        gimp_unit_get_identifier(unit), FACTOR, gimp_unit_get_factor(unit),
        DIGITS, gimp_unit_get_digits(unit), SYMBOL, gimp_unit_get_symbol(unit),
        ABBREVIATION, gimp_unit_get_abbreviation(unit), SINGULAR,
        gimp_unit_get_singular(unit), PLURAL, gimp_unit_get_plural(unit), UNIT,
        unit, USER_UNIT, user_unit,
