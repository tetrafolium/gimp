...
enum { H_NONE, H_COLLECT, LEFT2RIGHT, RIGHT2LEFT, SNAP2HGRID };

enum { H_BASE_LEFT, H_BASE_CENTER, H_BASE_RIGHT };

enum { V_NONE, V_COLLECT, TOP2BOTTOM, BOTTOM2TOP, SNAP2VGRID };

enum { V_BASE_TOP, V_BASE_CENTER, V_BASE_BOTTOM };
...
#define ALIGN_LAYERS                                                           \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), ALIGN_LAYERS_TYPE, AlignLayers))
...
static GimpLayer *align_layers_find_last_layer(GList *layers, gboolean *found);
static gint align_layers_spread_visibles_layers(GList *layers,
                                                GimpLayer **layers_array);
static GimpLayer **align_layers_spread_image(GimpImage *image, gint *layer_num);
static GimpLayer *align_layers_find_background(GimpImage *image);
static AlignData align_layers_gather_data(GimpLayer **layers, gint layer_num,
                                          GimpLayer *background);
static void align_layers_perform_alignment(GimpLayer **layers, gint layer_num,
                                           AlignData data);
static void align_layers_get_align_offsets(GimpDrawable *drawable, gint *x,
                                           gint *y);
...
static ValueType VALS = {H_NONE, H_BASE_LEFT, V_NONE, V_BASE_TOP,
                         TRUE,   FALSE,       10};

static void align_layers_class_init(AlignLayersClass *klass) {
...
static void align_layers_init(AlignLayers *film) {}
...
static GimpProcedure *align_layers_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, align_layers_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(
        procedure, N_("Align all visible layers of the image"),
        "Align visible layers", name);
...
static GimpValueArray *align_layers_run(GimpProcedure *procedure,
  ...
  case GIMP_RUN_INTERACTIVE:
    ...
    if (layer_num < 2) {
      error =
          g_error_new_literal(0, 0, _("There are not enough layers to align."));
      break;
    }
...
static GimpPDBStatusType align_layers(GimpImage *image) {
  ...
  data = align_layers_gather_data(layers, layer_num, background);

  gimp_image_undo_group_start(image);

  align_layers_perform_alignment(layers, layer_num, data);
...
static GimpLayer *align_layers_find_last_layer(GList *layers, gboolean *found) {
  ...
  for (; last; last = last->prev) {
    ...
    if (gimp_item_is_group(item)) {
      ...
      last_layer = align_layers_find_last_layer(children, found);
...
static GimpLayer **align_layers_spread_image(GimpImage *image,
  ...
  *layer_num = align_layers_count_visibles_layers(layers);
  ...
  align_layers_spread_visibles_layers(layers, layers_array);
  g_list_free(layers);
...
static AlignData align_layers_gather_data(GimpLayer **layers, gint layer_num,
  ...
  for (index = 0; index < layer_num; index++) {
    ...
    align_layers_get_align_offsets(GIMP_DRAWABLE(layers[index]), &offset_x,
                                   &offset_y);
    orig_x += offset_x;
    orig_y += offset_y;
  ...
  if (VALS.base_is_bottom_layer) {
    ...
    align_layers_get_align_offsets(GIMP_DRAWABLE(background), &offset_x,
                                   &offset_y);
    orig_x += offset_x;
    orig_y += offset_y;
...
static int align_layers_dialog(void) {
  GtkWidget *dialog;
  GtkWidget *grid;
  GtkWidget *combo;
  GtkWidget *toggle;
  ...
  dialog = gimp_dialog_new(_("Align Visible Layers"), PLUG_IN_ROLE, NULL, 0,
  ...
  combo = gimp_int_combo_box_new(
      C_("align-style", "None"), H_NONE, _("Collect"), H_COLLECT,
      _("Fill (left to right)"), LEFT2RIGHT, _("Fill (right to left)"),
      RIGHT2LEFT, _("Snap to grid"), SNAP2HGRID, NULL);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Horizontal style:"), 0.0,
                           0.5, combo, 2);

  combo = gimp_int_combo_box_new(_("Left edge"), H_BASE_LEFT, _("Center"),
                                 H_BASE_CENTER, _("Right edge"), H_BASE_RIGHT,
                                 NULL);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Ho_rizontal base:"), 0.0,
                           0.5, combo, 2);

  combo = gimp_int_combo_box_new(
      C_("align-style", "None"), V_NONE, _("Collect"), V_COLLECT,
      _("Fill (top to bottom)"), TOP2BOTTOM, _("Fill (bottom to top)"),
      BOTTOM2TOP, _("Snap to grid"), SNAP2VGRID, NULL);
  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), VALS.v_style);

  g_signal_connect(combo, "changed", G_CALLBACK(gimp_int_combo_box_get_active),
                   &VALS.v_style);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("_Vertical style:"), 0.0,
                           0.5, combo, 2);

  combo = gimp_int_combo_box_new(_("Top edge"), V_BASE_TOP, _("Center"),
                                 V_BASE_CENTER, _("Bottom edge"), V_BASE_BOTTOM,
                                 NULL);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("Ver_tical base:"), 0.0, 0.5,
                           combo, 2);
