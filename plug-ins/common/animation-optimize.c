...
#define OPTIMIZE                                                               \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), OPTIMIZE_TYPE, Optimize))
...
static GimpImage *do_optimizations(GimpRunMode run_mode, GimpImage *image,
                                   gboolean diff_only);
...
static void remove_disposal_tag(gchar *dest, gchar *src);
static void remove_ms_tag(gchar *dest, gchar *src);
...
static void optimize_class_init(OptimizeClass *klass) {
...
static void optimize_init(Optimize *optimize) {}
...
static GimpProcedure *optimize_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, OPTIMIZE_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, optimize_run, NULL, NULL);
  ...
  if (procedure) {
    ...
    GIMP_PROC_VAL_IMAGE(procedure, "result", "Result", "Resultimg image", FALSE,
                        G_PARAM_READWRITE);
  }
...
static void total_alpha(guchar *imdata, guint32 numpix, guchar bytespp) {
...
static const Babl *get_format(GimpDrawable *drawable) {
...
static void compose_row(gint frame_num, DisposeType dispose, gint row_num,
                        guchar *dest, gint dest_width, GimpDrawable *drawable,
                        gboolean cleanup) {
  ...
  if (row_num >= rawheight + rawy || row_num < rawy)
    return;
  ...
  gegl_buffer_get(src_buffer, GEGL_RECTANGLE(0, row_num - rawy, rawwidth, 1),
                  1.0, format, line_buf, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
...
static GimpImage *do_optimizations(GimpRunMode run_mode, GimpImage *image,
  ...
  drawabletype_alpha = (imagetype == GIMP_RGB)
                           ? GIMP_RGBA_IMAGE
                           : ((imagetype == GIMP_INDEXED) ? GIMP_INDEXEDA_IMAGE
                                                          : GIMP_GRAYA_IMAGE);
  ...
  if (opmode == OPBACKGROUND || opmode == OPFOREGROUND)
  ...
  if (opmode == OPBACKGROUND || opmode == OPFOREGROUND) {
    ...
    for (row = 0; row < height; row++) {
      ...
      for (this_frame_num = 0; this_frame_num < total_frames;
           this_frame_num++) {
        GimpDrawable *drawable =
        ...
        compose_row(this_frame_num, dispose, row, these_rows[this_frame_num],
                    width, drawable, FALSE);
      }
        ...
        for (i = 0; i < width; i++) {
          if (these_rows[this_frame_num][i * pixelstep + pixelstep - 1] >=
            ...
            for (j = 0; j < num_colors[i]; j++) {
              ...
              switch (pixelstep) {
              case 4:
                if (these_rows[this_frame_num][i * 4 + 0] == red[j][i] &&
                    these_rows[this_frame_num][i * 4 + 1] == green[j][i] &&
                    these_rows[this_frame_num][i * 4 + 2] == blue[j][i]) {
                  (count[j][i])++;
                  goto same;
                }
                break;
              case 2:
                if (these_rows[this_frame_num][i * 2 + 0] == red[j][i]) {
                  (count[j][i])++;
                  goto same;
                }
                break;
            ...
            red[num_colors[i]][i] = these_rows[this_frame_num][i * pixelstep];
            if (pixelstep == 4) {
              green[num_colors[i]][i] = these_rows[this_frame_num][i * 4 + 1];
              blue[num_colors[i]][i] = these_rows[this_frame_num][i * 4 + 2];
            }
            num_colors[i]++;
          }
        same:
      ...
      for (i = 0; i < width; i++) {
        ...
        if (pixelstep == 4) {
          back_frame[width * pixelstep * row + i * pixelstep + 1] = best_g;
          back_frame[width * pixelstep * row + i * pixelstep + 2] = best_b;
        }
  ...
  if (opmode == OPBACKGROUND) {
    ...
    new_layer = gimp_layer_new(
        new_image, "Backgroundx", width, height, drawabletype_alpha, 100.0,
    ...
    for (this_frame_num = 0; this_frame_num < total_frames; this_frame_num++) {
      ...
      for (row = 0; row < height; row++) {
        compose_row(this_frame_num, dispose, row,
                    &this_frame[pixelstep * width * row], width, drawable,
                    FALSE);
      }
          ...
          for (xit = 0; xit < width; xit++) {
            for (byteit = 0; byteit < pixelstep - 1; byteit++) {
              if (back_frame[yit * width * pixelstep + xit * pixelstep +
                             byteit] != this_frame[yit * width * pixelstep +
                                                   xit * pixelstep + byteit]) {
                goto enough;
              }
            }
            this_frame[yit * width * pixelstep + xit * pixelstep + pixelstep -
                       1] = 0;
          enough:
              /* nop */
              ;
      ...
      if ((this_frame_num != 0) /* Can't delta bottom frame! */
        ...
        for (yit = 0; yit < height; yit++) {
          ...
          decided:
            ...
            if (keep_pix) {
               ...
               */
              opti_frame[yit * width * pixelstep + xit * pixelstep + pixelstep -
                         1] = 0;
            }
          } /* xit */
        }   /* yit */
        ...
        if (can_combine && !diff_only) {
          ...
          for (yit = bbox_top; yit < bbox_bottom; yit++) {
            ...
            for (xit = bbox_left + 1; xit < bbox_right; xit++) {
              if (!(opti_frame[yit * width * pixelstep + xit * pixelstep +
                               pixelstep - 1] &
                    128) &&
                  (opti_frame[yit * width * pixelstep + (xit - 1) * pixelstep +
                              pixelstep - 1] &
                   128) &&
                  (last_frame[yit * width * pixelstep + xit * pixelstep +
                              pixelstep - 1] &
                ...
                for (byteit = 0; byteit < pixelstep; byteit++) {
                  opti_frame[yit * width * pixelstep + xit * pixelstep +
                             byteit] = last_frame[yit * width * pixelstep +
                                                  xit * pixelstep + byteit];
                }
              }
            skip_right:
        ...
        for (yit = bbox_top; yit < bbox_bottom; yit++) {
          for (xit = bbox_left; xit < bbox_right; xit++) {
            for (byteit = 0; byteit < pixelstep; byteit++) {
              *(destptr++) =
                  srcptr[yit * pixelstep * width + pixelstep * xit + byteit];
            }
          }
        }
      } /* !bot frame? */
      ...
      oldlayer_name = gimp_item_get_name(
          GIMP_ITEM(layers[total_frames - (this_frame_num + 1)]));
      ...
      g_snprintf(
          newlayer_name, buflen, "%s(%dms)%s", oldlayer_name, this_delay,
          (this_frame_num == 0) ? "" : can_combine ? "(combine)" : "(replace)");
      ...
      if (bbox_right <= bbox_left || bbox_bottom <= bbox_top) {
        ...
        g_snprintf(
            newlayer_name, buflen, "%s(%dms)%s", oldlayer_name, cumulated_delay,
            (this_frame_num == 0) ? ""
        ...
        last_true_frame = new_layer =
            gimp_layer_new(new_image, newlayer_name, bbox_right - bbox_left,
                           bbox_bottom - bbox_top, drawabletype_alpha, 100.0,
                           gimp_image_get_default_new_layer_mode(new_image));
        g_free(newlayer_name);
        ...
        gegl_buffer_set(buffer,
                        GEGL_RECTANGLE(0, 0, bbox_right - bbox_left,
                                       bbox_bottom - bbox_top),
                        0, format, opti_frame, GEGL_AUTO_ROWSTRIDE);

        g_object_unref(buffer);
        gimp_item_transform_translate(GIMP_ITEM(new_layer), bbox_left,
                                      bbox_top);
      }
...
static DisposeType get_frame_disposal(guint whichframe) {
  ...
  layer_name =
      gimp_item_get_name(GIMP_ITEM(layers[total_frames - (whichframe + 1)]));
...
static guint32 get_frame_duration(guint whichframe) {
  ...
  if (duration < 0)
    duration = 100; /* FIXME for default-if-not-said  */
  if (duration == 0)
    duration = 100; /* FIXME - 0-wait is nasty */
