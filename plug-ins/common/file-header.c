...
static GimpProcedure *header_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, header_save, NULL, NULL);
    ...
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-chdr");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "h");
  }
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  if (!print(output, error,
      ...
      !print(output, error, "static unsigned int width = %d;\n", width) ||
      !print(output, error, "static unsigned int height = %d;\n\n", height) ||
      !print(output, error,
             "/*  Call this macro repeatedly.  After each use, the pixel data "
             "can be extracted  */\n\n")) {
    goto fail;
  }
  ...
  case GIMP_RGB_IMAGE:
    if (!print(
            ...
            "pixel[1] = ((((data[1] - 33) & 0xF) << 4) | ((data[2] - 33) >> "
            "2)); \\\n"
    ...
    c = 0;
    for (y = 0; y < height; y++) {
      gegl_buffer_get(buffer, GEGL_RECTANGLE(0, y, width, 1), 1.0, format, data,
  ...
  case GIMP_INDEXED_IMAGE:
    ...
    if (!print(output, error,
        ...
        !print(output, error, "\n\t{%3d,%3d,%3d}", (gint)cmap[0], (gint)cmap[1],
               (gint)cmap[2])) {
      goto fail;
    }

    for (c = 1; c < colors; c++) {
      if (!print(output, error, ",\n\t{%3d,%3d,%3d}", (gint)cmap[3 * c],
                 (gint)cmap[3 * c + 1], (gint)cmap[3 * c + 2])) {
        goto fail;
      }
    }
    ...
    c = 0;
    for (y = 0; y < height; y++) {
      gegl_buffer_get(buffer, GEGL_RECTANGLE(0, y, width, 1), 1.0, NULL, data,
                      GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
