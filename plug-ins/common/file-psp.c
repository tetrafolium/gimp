...
typedef enum {
  ...
  PSP_ADJUSTMENT_EXTENSION_BLOCK, /* Adjustment Layer Extension Block (sub)
                                     (since PSP6)*/
  ...
  PSP_COMPOSITE_ATTRIBUTES_BLOCK, /* Composite Image Attributes (sub) (since
                                     PSP6) */
  ...
  PSP_RASTER_EXTENSION_BLOCK,  /* Assumed name based on usage, probably since
                                  PSP11 */
} PSPBlockID;
 ...
 */
typedef enum { keHorizontalGuide = 0, keVerticalGuide } PSPGuideOrientationType;
...
static GimpValueArray *psp_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *psp_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static gboolean save_image(GFile *file, GimpImage *image, gint n_drawables,
...
static void psp_class_init(PspClass *klass) {
...
static GimpProcedure *psp_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, psp_load, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Tor Lillqvist", "Tor Lillqvist",
                                   "1999");
    ...
    gimp_file_procedure_set_magics(
        GIMP_FILE_PROCEDURE(procedure),
        "0,string,Paint\\040Shop\\040Pro\\040Image\\040File\n\032");
  } else if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, psp_save, NULL, NULL);
    ...
    GIMP_PROC_ARG_INT(procedure, "compression", "Compression",
                      "Specify 0 for no compression, "
                      "1 for RLE, and 2 for LZ77",
                      0, 2, PSP_COMP_LZ77, G_PARAM_READWRITE);
  }
...
static GimpValueArray *psp_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "PSP",
  ...
  if (status == GIMP_PDB_SUCCESS) {
    if (!save_image(file, image, n_drawables, drawables, G_OBJECT(config),
                    &error)) {
...
static gboolean save_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  /*  file save type  */
  store = gimp_int_store_new(C_("compression", "None"), PSP_COMP_NONE, _("RLE"),
                             PSP_COMP_RLE, _("LZ77"), PSP_COMP_LZ77, NULL);
  frame = gimp_prop_int_radio_frame_new(
      config, "compression", _("Data Compression"), GIMP_INT_STORE(store));
 ...
 */
static const gchar *block_name(gint id) {
 ...
 */
static gint read_block_header(FILE *f, guint32 *init_len, guint32 *total_len,
                              GError **error) {
  guchar buf[4];
  guint16 id;
  long header_start;
  ...
  if (fread(buf, 4, 1, f) < 1 || fread(&id, 2, 1, f) < 1 ||
      fread(&len, 4, 1, f) < 1 ||
  ...
  if (memcmp(buf, "~BK\0", 4) != 0) {
    ...
    else g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
...
static gint try_fseek(FILE *f, glong pos, gint whence, GError **error) {
...
static gint read_general_image_attribute_block(FILE *f, guint init_len,
                                               guint total_len, PSPimage *ia,
                                               GError **error) {
  gchar buf[6];
  guint64 res;
  ...
  chunk_start = ftell(f);
  if ((psp_ver_major >= 4 && (fread(&init_len, 4, 1, f) < 1 ||
                              ((init_len = GUINT32_FROM_LE(init_len)) < 46))) ||
      fread(&ia->width, 4, 1, f) < 1 || fread(&ia->height, 4, 1, f) < 1 ||
      fread(&res, 8, 1, f) < 1 || fread(&ia->metric, 1, 1, f) < 1 ||
      fread(&ia->compression, 2, 1, f) < 1 || fread(&ia->depth, 2, 1, f) < 1 ||
...
static gint read_creator_block(FILE *f, GimpImage *image, guint total_len,
                               PSPimage *ia, GError **error) {
  long data_start;
  guchar buf[4];
  guint16 keyword;
  guint32 length;
  ...
  while (ftell(f) < data_start + total_len) {
    if (fread(buf, 4, 1, f) < 1 || fread(&keyword, 2, 1, f) < 1 ||
  ...
  if (comment->len > 0) {
    comment_parasite =
        gimp_parasite_new("gimp-comment", GIMP_PARASITE_PERSISTENT,
...
static gint read_color_block(FILE *f, GimpImage *image, guint total_len,
                             PSPimage *ia, GError **error) {
  long block_start;
  ...
  if (psp_ver_major >= 4) {
    if (fread(&chunk_len, 4, 1, f) < 1 || fread(&entry_count, 4, 1, f) < 1) {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
...
static void inline swab_rect(guint32 *rect) {
...
static GimpLayerMode gimp_layer_mode_from_psp_blend_mode(PSPBlendModes mode) {
  switch (mode) {
...
static const gchar *blend_mode_name(PSPBlendModes mode) {
  static const gchar *blend_mode_names[] = {
      "NORMAL", "DARKEN", "LIGHTEN", "HUE", "SATURATION", "COLOR", "LUMINOSITY",
      "MULTIPLY", "SCREEN", "DISSOLVE", "OVERLAY", "HARD_LIGHT", "SOFT_LIGHT",
      "DIFFERENCE", "DODGE", "BURN", "EXCLUSION", "TRUE HUE", "TRUE SATURATION",
      "TRUE COLOR", "TRUE LIGHTNESS",
...
static const gchar *layer_type_name(PSPLayerTypePSP6 type) {
  static const gchar *layer_type_names[] = {
      "Undefined", "Raster",     "Floating Raster Selection",
      "Vector",    "Adjustment", "Group",
      "Mask",      "Art Media",
  };
  static gchar *err_name = NULL;
...
static const gchar *bitmap_type_name(gint type) {
  static const gchar *bitmap_type_names[] = {"IMAGE",      "TRANS_MASK",
                                             "USER_MASK",  "SELECTION",
                                             "ALPHA_MASK", "THUMBNAIL"};
  static gchar *err_name = NULL;
...
static const gchar *channel_type_name(gint type) {
  static const gchar *channel_type_names[] = {"COMPOSITE", "RED", "GREEN",
                                              "BLUE"};
  static gchar *err_name = NULL;
...
static void *psp_zalloc(void *opaque, guint items, guint size) {
  return g_malloc(items * size);
}

static void psp_zfree(void *opaque, void *ptr) { g_free(ptr); }

static void upscale_indexed_sub_8(FILE *f, gint width, gint height, gint bpp,
                                  guchar *buf) {
...
static int read_channel_data(FILE *f, PSPimage *ia, guchar **pixels,
                             guint bytespp, guint offset, GeglBuffer *buffer,
                             guint32 compressed_len, GError **error) {
  gint i, y, line_width;
  ...
  case PSP_COMP_LZ77:
    ...
    if (inflateInit(&zstream) != Z_OK) {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, _("zlib error"));
      return -1;
    }
    if (bytespp == 1)
...
static gboolean read_raster_layer_info(FILE *f, long layer_extension_start,
                                       guint16 *bitmap_count,
                                       guint16 *channel_count, GError **error) {
  long block_start;
  ...
  if (psp_ver_major > 8 && memcmp(&layer_extension_len, "~BK\0", 4) == 0) {
    if (fread(&block_id, 2, 1, f) < 1 || fread(&block_len, 4, 1, f) < 1) {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
  ...
  if (fread(bitmap_count, 2, 1, f) < 1 || fread(channel_count, 2, 1, f) < 1) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Error reading layer extension information"));
    return FALSE;
  }
  if (try_fseek(f, layer_extension_start + layer_extension_len, SEEK_SET,
                error) < 0) {
    return FALSE;
  }
...
static GimpLayer *read_layer_block(FILE *f, GimpImage *image, guint total_len,
                                   PSPimage *ia, GError **error) {
  gint i;
  ...
  while (ftell(f) < block_start + total_len) {
    ...
    if (psp_ver_major >= 4) {
      if (fread(&chunk_len, 4, 1, f) < 1 ||
          ...
          /* A zero length layer name is apparently valid. To not get a warning
             for namelen < 0 always being false we use this more complicated
             comparison. */
          ||
          ((namelen = GUINT16_FROM_LE(namelen)) && (FALSE || namelen == 0)) ||
          (name = g_malloc(namelen + 1)) == NULL ||
          (namelen > 0 && fread(name, namelen, 1, f) < 1) ||
          fread(&type, 1, 1, f) < 1 || fread(&image_rect, 16, 1, f) < 1 ||
          fread(&saved_image_rect, 16, 1, f) < 1 ||
          fread(&opacity, 1, 1, f) < 1 || fread(&blend_mode, 1, 1, f) < 1 ||
      ...
      case keGLTFloatingRasterSelection:
        g_message("Floating selection restored as normal layer (%s)",
                  layer_name);
      case keGLTRaster:
        if (!read_raster_layer_info(f, layer_extension_start, &bitmap_count,
                                    &channel_count, error)) {
          g_free(layer_name);
          return NULL;
        }
      ...
      default:
        bitmap_count = 0;
        channel_count = 0;
        g_message("Unsupported layer type %s (%s)", layer_type_name(type),
                  layer_name);
        break;
      }
    } else {
      ...
      if (fread(name, 256, 1, f) < 1 || fread(&type, 1, 1, f) < 1 ||
          ...
          fread(&saved_mask_rect, 16, 1, f) < 1 ||
          fread(&mask_linked, 1, 1, f) < 1 ||
          fread(&mask_disabled, 1, 1, f) < 1 || fseek(f, 43, SEEK_CUR) < 0 ||
    ...
    {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                  _("Invalid layer dimensions: %dx%d"), width, height);
      g_free(layer_name);
      return NULL;
    }

    IFDBG(2)
    g_message("layer: %s %dx%d (%dx%d) @%d,%d opacity %d blend_mode %s "
              "%d bitmaps %d channels",
              layer_name, image_rect[2] - image_rect[0],
              image_rect[3] - image_rect[1], width, height,
              image_rect[0] + saved_image_rect[0],
              image_rect[1] + saved_image_rect[1], opacity,
              blend_mode_name(blend_mode), bitmap_count, channel_count);

    IFDBG(2)
    g_message("mask %dx%d (%dx%d) @%d,%d", mask_rect[2] - mask_rect[0],
              mask_rect[3] - mask_rect[1],
              saved_mask_rect[2] - saved_mask_rect[0],
              saved_mask_rect[3] - saved_mask_rect[1], saved_mask_rect[0],
              saved_mask_rect[1]);
    ...
    else if (bitmap_count == 1)
      drawable_type = GIMP_INDEXED_IMAGE, bytespp = 1;
    else
      drawable_type = GIMP_INDEXEDA_IMAGE, bytespp = 2;
    bytespp *= ia->bytes_per_sample;

    layer = gimp_layer_new(image, layer_name, width, height, drawable_type,
                           100.0 * opacity / 255.0, layer_mode);
    g_free(layer_name);
    if (!layer) {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
    ...
    if (image_rect[0] != 0 || image_rect[1] != 0 || saved_image_rect[0] != 0 ||
        saved_image_rect[1] != 0)
      gimp_layer_set_offsets(layer, image_rect[0] + saved_image_rect[0],
                             image_rect[1] + saved_image_rect[1]);
    ...
    if (can_handle_layer) {
      ...
      while (ftell(f) < sub_block_start + sub_total_len) {
        sub_id =
            read_block_header(f, &channel_init_len, &channel_total_len, error);
        if (sub_id == -1) {
          gimp_image_delete(image);
          return NULL;
        }
        ...
        chunk_len =
            channel_init_len; /* init chunk_len for psp_ver_major == 3 */
        if ((psp_ver_major >= 4 &&
        ...
        if (bitmap_type > PSP_DIB_USER_MASK) {
          g_message("Conversion of bitmap type %d is not supported.",
                    bitmap_type);
          ...
          IFDBG(2)
          g_message("channel: %s %s %d (%d) bytes %d bytespp",
                    bitmap_type_name(bitmap_type),
                    channel_type_name(channel_type), uncompressed_len,
                    compressed_len, bytespp);
          ...
          if (!null_layer) {
            ...
            if (read_channel_data(f, ia, pixels, bytespp, offset, buffer,
                                  compressed_len, error) == -1) {
              return NULL;
            }
          }
        }
        if (try_fseek(f, channel_start + channel_total_len, SEEK_SET, error) <
            0) {
          return NULL;
        }
      }

      gegl_buffer_set(buffer, GEGL_RECTANGLE(0, 0, width, height), 0, NULL,
      ...
      g_free(pixel);
      if (psp_ver_major >= 4) {
        if (try_fseek(f, sub_block_start + sub_total_len, SEEK_SET, error) <
            0) {
          return NULL;
        }
      }
    } else {
      /* Can't handle this type of layer, skip the data so we can read the next
       * layer. */
      if (psp_ver_major >= 4) {
        if (try_fseek(f, sub_block_start + sub_total_len, SEEK_SET, error) <
            0) {
...
static gint read_tube_block(FILE *f, GimpImage *image, guint total_len,
                            PSPimage *ia, GError **error) {
  guint16 version;
  guchar name[514];
  ...
  if (psp_ver_major >= 4) {
    name[0] = 0;
    if (fread(&chunk_len, 4, 1, f) < 1 || fread(&version, 2, 1, f) < 1 ||
        fread(&step_size, 4, 1, f) < 1 || fread(&column_count, 4, 1, f) < 1 ||
        fread(&row_count, 4, 1, f) < 1 || fread(&cell_count, 4, 1, f) < 1 ||
    ...
    chunk_len = 0;
    if (fread(&version, 2, 1, f) < 1 || fread(name, 513, 1, f) < 1 ||
        fread(&step_size, 4, 1, f) < 1 || fread(&column_count, 4, 1, f) < 1 ||
        fread(&row_count, 4, 1, f) < 1 || fread(&cell_count, 4, 1, f) < 1 ||
        fread(&placement_mode, 4, 1, f) < 1 ||
        fread(&selection_mode, 4, 1, f) < 1) {
  ...
  params.placement =
      (placement_mode == tpmRandom
           ? "random"
           : (placement_mode == tpmConstant ? "constant" : "default"));
  params.rank[0] = params.ncells;
  params.selection[0] =
      (selection_mode == tsmRandom
           ? "random"
           : (selection_mode == tsmIncremental
                  ? "incremental"
                  : (selection_mode == tsmAngular
                         ? "angular"
                         : (selection_mode == tsmPressure
                                ? "pressure"
                                : (selection_mode == tsmVelocity
                                       ? "velocity"
                                       : "default")))));
  ...
  pipe_parasite =
      gimp_parasite_new("gimp-brush-pipe-parameters", GIMP_PARASITE_PERSISTENT,
...
static const gchar *compression_name(gint compression) {
  switch (compression) {
  case PSP_COMP_NONE:
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  if (fread(buf, 32, 1, f) < 1 || fread(&psp_ver_major, 2, 1, f) < 1 ||
  ...
  if (psp_ver_major < 3) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Unsupported PSP file format version %d.%d."), psp_ver_major,
                psp_ver_minor);
    goto error;
  }
  ...
  while (ftell(f) != st.st_size &&
         (id = read_block_header(f, &block_init_len, &block_total_len,
                                 error)) != -1) {
    block_start = ftell(f);
    ...
    if (block_start + block_total_len > st.st_size) {
      g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                  _("Could not open '%s' for reading: %s"),
                  gimp_file_get_utf8_name(file), _("invalid block size"));
      goto error;
    }
    ...
    if (id == PSP_IMAGE_BLOCK) {
      ...
      if (read_general_image_attribute_block(f, block_init_len, block_total_len,
                                             &ia, error) == -1) {
        goto error;
      }

      IFDBG(2)
      g_message("%d dpi %dx%d %s", (int)ia.resolution, ia.width, ia.height,
                compression_name(ia.compression));

      image = gimp_image_new_with_precision(ia.width, ia.height, ia.base_type,
                                            ia.precision);
      if (!image) {
        goto error;
      }
...
static gint save_image(GFile *file, GimpImage *image, gint n_drawables,
                       GimpDrawable **drawables, GObject *config,
                       GError **error) {
  g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
