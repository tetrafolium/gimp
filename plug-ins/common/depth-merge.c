...
static GimpValueArray *merge_run(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void DepthMerge_executeRegion(DepthMerge *dm, guchar *source1Row,
                                     guchar *source2Row, guchar *depthMap1Row,
                                     guchar *depthMap2Row, guchar *resultRow,
                                     gint length);
...
static gboolean dm_constraint(GimpImage *image, GimpItem *item, gpointer data);
...
static void dialogValueScaleUpdateCallback(GimpLabelSpin *scale, gfloat *value);

static void util_fillReducedBuffer(guchar *dest, const Babl *dest_format,
                                   gint destWidth, gint destHeight,
                                   GimpDrawable *sourceDrawable, gint x0,
                                   gint y0, gint sourceWidth,
                                   gint sourceHeight);
...
static void merge_class_init(MergeClass *klass) {
...
static void merge_init(Merge *merge) {}
...
static GimpProcedure *merge_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, merge_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Sean Cier", "Sean Cier",
                                   PLUG_IN_VERSION);

    GIMP_PROC_ARG_DRAWABLE(procedure, "source-1", "Source 1", "Source 1", TRUE,
                           G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "source-2", "Source 2", "Source 2", TRUE,
                           G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "depth-map-1", "Depth map 1",
                           "Depth map 1", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "depth-map-2", "Depth map 2",
                           "Depth map 2", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "overlap", "Overlap", "Overlap", 0, 2, 0,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "offset", "Offset", "Depth relative offset",
                         -1, 1, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "scale-1", "Scale 1",
                         "Depth relative scale 1", -1, 1, 1, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "scale-2", "Scale 2",
                         "Depth relative scale 2", -1, 1, 1, G_PARAM_READWRITE);
  }
...
static GimpValueArray *merge_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_INTERACTIVE:
    ...
    if (!DepthMerge_dialog(&dm)) {
      gimp_procedure_new_return_values(procedure, GIMP_PDB_CANCEL, NULL);
    }
    break;
  ...
  if (!DepthMerge_execute(&dm)) {
    gimp_procedure_new_return_values(procedure, GIMP_PDB_EXECUTION_ERROR, NULL);
  } else {
...
static gboolean DepthMerge_construct(DepthMerge *dm) {
  ...
  if (!gimp_drawable_mask_intersect(dm->resultDrawable, &(dm->selectionX),
                                    &(dm->selectionY), &(dm->selectionWidth),
                                    &(dm->selectionHeight))) {
    return FALSE;
  }
...
static gint32 DepthMerge_execute(DepthMerge *dm) {
  ...
  for (y = dm->selectionY; y < (dm->selectionY + dm->selectionHeight); y++) {
    if (dm->source1Drawable) {
      gegl_buffer_get(source1_buffer,
                      GEGL_RECTANGLE(dm->selectionX, y, dm->selectionWidth, 1),
                      1.0, babl_format("R'G'B'A u8"), source1Row,
    ...
    if (dm->source2Drawable) {
      gegl_buffer_get(source2_buffer,
                      GEGL_RECTANGLE(dm->selectionX, y, dm->selectionWidth, 1),
                      1.0, babl_format("R'G'B'A u8"), source2Row,
                      GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
    }
    ...
    if (dm->depthMap1Drawable) {
      gegl_buffer_get(depthMap1_buffer,
                      GEGL_RECTANGLE(dm->selectionX, y, dm->selectionWidth, 1),
                      1.0, babl_format("Y' u8"), depthMap1Row,
                      GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
    }
    ...
    if (dm->depthMap2Drawable) {
      gegl_buffer_get(depthMap2_buffer,
                      GEGL_RECTANGLE(dm->selectionX, y, dm->selectionWidth, 1),
                      1.0, babl_format("Y' u8"), depthMap2Row,
                      GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
    }

    DepthMerge_executeRegion(dm, source1Row, source2Row, depthMap1Row,
                             depthMap2Row, resultRow, dm->selectionWidth);

    gegl_buffer_set(result_buffer,
                    GEGL_RECTANGLE(dm->selectionX, y, dm->selectionWidth, 1), 0,
                    babl_format("R'G'B'A u8"), resultRow, GEGL_AUTO_ROWSTRIDE);
  ...
  gimp_drawable_update(dm->resultDrawable, dm->selectionX, dm->selectionY,
...
static void DepthMerge_executeRegion(DepthMerge *dm, guchar *source1Row,
  ...
  for (i = 0; i < length; i++) {
    ...
    temp = DIV255(cR[0]);
    resultRow[4 * i] = MIN(temp, 255);
    temp = DIV255(cR[1]);
    resultRow[4 * i + 1] = MIN(temp, 255);
    temp = DIV255(cR[2]);
    resultRow[4 * i + 2] = MIN(temp, 255);
    temp = DIV255(cR[3]);
    resultRow[4 * i + 3] = MIN(temp, 255);
  }
}
...
static gboolean DepthMerge_dialog(DepthMerge *dm) {
  ...
  dm->interface->dialog = dialog = gimp_dialog_new(
      _("Depth Merge"), PLUG_IN_ROLE, NULL, 0, gimp_standard_help_func,
      PLUG_IN_PROC,
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), dm->params.source1_id,
                             G_CALLBACK(dialogSource1ChangedCallback), dm,
                             NULL);
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), dm->params.depthMap1_id,
                             G_CALLBACK(dialogDepthMap1ChangedCallback), dm,
                             NULL);
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), dm->params.source2_id,
                             G_CALLBACK(dialogSource2ChangedCallback), dm,
                             NULL);
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), dm->params.depthMap2_id,
                             G_CALLBACK(dialogDepthMap2ChangedCallback), dm,
                             NULL);
  ...
  run =
      (gimp_dialog_run(GIMP_DIALOG(dm->interface->dialog)) == GTK_RESPONSE_OK);
...
static void DepthMerge_buildPreviewSourceImage(DepthMerge *dm) {
  dm->interface->previewSource1 = g_new(
      guchar, dm->interface->previewWidth * dm->interface->previewHeight * 4);
  util_fillReducedBuffer(dm->interface->previewSource1,
                         babl_format("R'G'B'A u8"), dm->interface->previewWidth,
                         dm->interface->previewHeight, dm->source1Drawable,
                         dm->selectionX, dm->selectionY, dm->selectionWidth,
                         dm->selectionHeight);

  dm->interface->previewSource2 = g_new(
      guchar, dm->interface->previewWidth * dm->interface->previewHeight * 4);
  util_fillReducedBuffer(dm->interface->previewSource2,
                         babl_format("R'G'B'A u8"), dm->interface->previewWidth,
                         dm->interface->previewHeight, dm->source2Drawable,
                         dm->selectionX, dm->selectionY, dm->selectionWidth,
                         dm->selectionHeight);

  dm->interface->previewDepthMap1 = g_new(
      guchar, dm->interface->previewWidth * dm->interface->previewHeight * 1);
  util_fillReducedBuffer(dm->interface->previewDepthMap1, babl_format("Y' u8"),
                         dm->interface->previewWidth,
                         dm->interface->previewHeight, dm->depthMap1Drawable,
                         dm->selectionX, dm->selectionY, dm->selectionWidth,
                         dm->selectionHeight);

  dm->interface->previewDepthMap2 = g_new(
      guchar, dm->interface->previewWidth * dm->interface->previewHeight * 1);
  util_fillReducedBuffer(dm->interface->previewDepthMap2, babl_format("Y' u8"),
                         dm->interface->previewWidth,
                         dm->interface->previewHeight, dm->depthMap2Drawable,
                         dm->selectionX, dm->selectionY, dm->selectionWidth,
                         dm->selectionHeight);
}
...
static void DepthMerge_updatePreview(DepthMerge *dm) {
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(dm->interface->preview), 0, 0,
                         dm->interface->previewWidth,
                         dm->interface->previewHeight, GIMP_RGBA_IMAGE,
                         resultRGBA, dm->interface->previewWidth * 4);
  g_free(resultRGBA);
}
...
static void util_fillReducedBuffer(guchar *dest, const Babl *dest_format,
  ...
  for (x = 0; x < destWidth; x++)
    sourceRowOffsetLookup[x] =
        (x * (sourceWidth - 1) / (destWidth - 1)) * destBPP;

  gegl_buffer_get(buffer, GEGL_RECTANGLE(x0, y0, sourceWidth, sourceHeight),
                  1.0, dest_format, sourceBuffer, GEGL_AUTO_ROWSTRIDE,
                  GEGL_ABYSS_NONE);
