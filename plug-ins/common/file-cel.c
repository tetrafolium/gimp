...
static GimpValueArray *cel_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *cel_save(GimpProcedure *procedure, GimpRunMode run_mode,
                                GimpImage *image, gint n_drawables,
                                GimpDrawable **drawables, GFile *file,
                                const GimpValueArray *args, gpointer run_data);

static gint load_palette(GFile *file, FILE *fp, guchar palette[],
                         GError **error);
...
static gboolean need_palette(GFile *file, GError **error);
...
static void cel_class_init(CelClass *klass) {
...
static GimpProcedure *cel_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, cel_load, NULL, NULL);
    ...
    gimp_procedure_set_attribution(
        procedure, "Nick Lamb", "Nick Lamb <njl195@zepler.org.uk>", "May 1998");

    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "cel");
    gimp_file_procedure_set_magics(GIMP_FILE_PROCEDURE(procedure),
                                   "0,string,KiSS\\040");

    GIMP_PROC_ARG_STRING(procedure, "palette-filename", "Palette filename",
                         "Filename to load palette from", NULL,
                         G_PARAM_READWRITE | GIMP_PARAM_NO_VALIDATE);
  } else if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, cel_save, NULL, NULL);
    ...
    GIMP_PROC_ARG_STRING(procedure, "palette-filename", "Palette filename",
                         "Filename to save palette to", NULL,
                         G_PARAM_READWRITE | GIMP_PARAM_NO_VALIDATE);
  }
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  if (bpp == 32)
    layer =
        gimp_layer_new(image, _("Background"), width, height, GIMP_RGBA_IMAGE,
  ...
  for (i = 0; i < height && !feof(fp); ++i) {
    ...
    gegl_buffer_set(buffer, GEGL_RECTANGLE(0, i, width, 1), 0, NULL, line,
                    GEGL_AUTO_ROWSTRIDE);
  ...
  if (bpp != 32) {
    ...
    if (palette_file == NULL) {
      ...
      if (fp == NULL) {
        g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno),
                    _("Could not open '%s' for reading: %s"),
                    gimp_filename_to_utf8(palette_file), g_strerror(errno));
        return NULL;
      }
    }
    ...
    if (fp != NULL) {
      colors =
          load_palette(g_file_new_for_path(palette_file), fp, palette, error);
      fclose(fp);
      if (colors < 0 || *error)
        return NULL;
    } else {
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  if (!g_output_stream_write_all(output, header, 32, NULL, NULL, error))
    goto fail;
  ...
  for (i = 0; i < height; ++i) {
    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, i, width, 1), 1.0, format, line,
    ...
    if (bpp == 32) {
      ...
      if (!g_output_stream_write_all(output, buf, width * 4, NULL, NULL, error))
        goto fail;
    } else if (colors > 16) {
      for (j = 0, k = 0; j < width * 2; j += 2, ++k) {
        if (line[j + 1] > 127) {
      ...
      if (!g_output_stream_write_all(output, buf, width, NULL, NULL, error))
        goto fail;
    } else {
      for (j = 0, k = 0; j < width * 2; j += 4, ++k) {
        buf[k] = 0;
      ...
      if (!g_output_stream_write_all(output, buf, width + 1 / 2, NULL, NULL,
                                     error))
        goto fail;
    }
