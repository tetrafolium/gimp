...
static void browser_dialog_response(GtkWidget *widget, gint response_id,
...
static gboolean find_existing_mpath(GtkTreeModel *model, const gchar *mpath,
...
static GimpProcedure *browser_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(procedure_name, PLUG_IN_PROC)) {
    procedure =
        gimp_procedure_new(plug_in, procedure_name, GIMP_PDB_PROC_TYPE_PLUGIN,
...
static gboolean find_existing_mpath_helper(GtkTreeModel *model,
  ...
  do {
    ...
    gtk_tree_model_get(model, iter, TREE_COLUMN_MPATH, &picked_mpath, -1);
    ...
    if (gtk_tree_model_iter_children(model, &child, iter)) {
      ...
      if (find_existing_mpath_helper(model, &child, path, mpath, return_iter)) {
        g_free(picked_mpath);
        return TRUE;
      }
...
static void get_parent(PluginBrowser *browser, const gchar *mpath,
                       GtkTreeIter *parent) {
  ...
  if (str_ptr == NULL) {
    ...
    gtk_tree_store_set(tree_store, parent, TREE_COLUMN_MPATH, mpath,
                       TREE_COLUMN_PATH_NAME, mpath, -1);
  } else {
    gchar *leaf_ptr;
    ...
    *str_ptr = '\0';
    ...
    gtk_tree_store_set(tree_store, parent, TREE_COLUMN_MPATH, mpath,
                       TREE_COLUMN_PATH_NAME, leaf_ptr, -1);

    g_free(leaf_ptr);
  }
}

static void insert_into_tree_view(PluginBrowser *browser, const gchar *name,
                                  gint64 xtime, const gchar *xtimestr,
                                  const gchar *menu_path,
                                  const gchar *types_str, PInfo *pinfo) {
  GtkTreeStore *tree_store;
  ...
  gtk_tree_store_set(
      tree_store, &iter, TREE_COLUMN_MPATH, menu_path, TREE_COLUMN_PATH_NAME,
      name, TREE_COLUMN_IMAGE_TYPES, types_str, TREE_COLUMN_DATE, xtime,
      TREE_COLUMN_DATE_STRING, xtimestr, TREE_COLUMN_PINFO, pinfo, -1);
}

static void browser_search(GimpBrowser *gimp_browser, const gchar *search_text,
                           gint search_type, PluginBrowser *browser) {
  ...
  return_vals = gimp_pdb_run_procedure(gimp_get_pdb(), "gimp-plug-ins-query",
                                       G_TYPE_STRING, search_text, G_TYPE_NONE);
  ...
  if (!search_text || strlen(search_text) == 0) {
    str = g_strdup_printf(ngettext("%d plug-in", "%d plug-ins", num_plugins),
                          num_plugins);
  } else {
    switch (num_plugins) {
    case 0:
    ...
    default:
      str =
          g_strdup_printf(ngettext("%d plug-in matches your query",
                                   "%d plug-ins match your query", num_plugins),
                          num_plugins);
      break;
    }
  }
  ...
  if (num_plugins > 0) {
    ...
    for (i = 0; i < num_plugins; i++) {
      ...
      procedure = gimp_pdb_lookup_procedure(gimp_get_pdb(), procedure_strs[i]);
      ...
      gtk_list_store_set(list_store, &iter, LIST_COLUMN_NAME, menu_label,
                         LIST_COLUMN_DATE, (gint64)tx, LIST_COLUMN_DATE_STRING,
                         xtimestr, LIST_COLUMN_PATH, menu_path,
                         LIST_COLUMN_IMAGE_TYPES, types, LIST_COLUMN_PINFO,
                         pinfo, -1);

      /* Now do the tree view.... */
      insert_into_tree_view(browser, menu_label, (gint64)tx, xtimestr,
                            menu_path, types, pinfo);
    ...
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(list_store),
                                         LIST_COLUMN_NAME, GTK_SORT_ASCENDING);
    ...
    gimp_browser_show_message(GIMP_BROWSER(browser->browser), _("No matches"));
  }
...
static GtkWidget *browser_dialog_new(void) {
  ...
  browser->dialog = gimp_dialog_new(_("Plug-in Browser"), PLUG_IN_ROLE, NULL, 0,
  ...
  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(browser->dialog))),
      browser->browser, TRUE, TRUE, 0);
  gtk_widget_show(browser->browser);

  g_signal_connect(browser->browser, "search", G_CALLBACK(browser_search),
                   browser);
  ...
  gtk_box_pack_start(
      GTK_BOX(gimp_browser_get_left_vbox(GIMP_BROWSER(browser->browser))),
      notebook, TRUE, TRUE, 0);

  /* list : list in a scrolled_win */
  list_store = gtk_list_store_new(N_LIST_COLUMNS, G_TYPE_STRING, G_TYPE_INT64,
  ...
  column = gtk_tree_view_column_new_with_attributes(
      _("Menu Path"), renderer, "text", LIST_COLUMN_PATH, NULL);
  gtk_tree_view_column_set_sort_column_id(column, LIST_COLUMN_PATH);
  gtk_tree_view_append_column(GTK_TREE_VIEW(list_view), column);

  renderer = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      _("Image Types"), renderer, "text", LIST_COLUMN_IMAGE_TYPES, NULL);
  ...
  column = gtk_tree_view_column_new_with_attributes(
      _("Installation Date"), renderer, "text", LIST_COLUMN_DATE_STRING, NULL);
  ...
  g_signal_connect(selection, "changed",
                   G_CALLBACK(browser_list_selection_changed), browser);
  ...
  /* notebook->ctree */
  tree_store = gtk_tree_store_new(N_LIST_COLUMNS, G_TYPE_STRING, G_TYPE_INT64,
                                  G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
                                  G_TYPE_POINTER);
  ...
  renderer = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      _("Menu Path"), renderer, "text", TREE_COLUMN_PATH_NAME, NULL);
  gtk_tree_view_column_set_sort_column_id(column, TREE_COLUMN_PATH_NAME);
  gtk_tree_view_append_column(GTK_TREE_VIEW(tree_view), column);

  renderer = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      _("Image Types"), renderer, "text", TREE_COLUMN_IMAGE_TYPES, NULL);
  gtk_tree_view_column_set_sort_column_id(column, TREE_COLUMN_IMAGE_TYPES);
  gtk_tree_view_append_column(GTK_TREE_VIEW(tree_view), column);

  renderer = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      _("Installation Date"), renderer, "text", TREE_COLUMN_DATE_STRING, NULL);
  ...
  g_signal_connect(selection, "changed",
                   G_CALLBACK(browser_tree_selection_changed), browser);
  ...
  parent = gtk_widget_get_parent(
      gimp_browser_get_right_vbox(GIMP_BROWSER(browser->browser)));
  ...
  if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(list_store), &iter))
    gtk_tree_selection_select_iter(
        gtk_tree_view_get_selection(GTK_TREE_VIEW(list_view)), &iter);
...
static void browser_list_selection_changed(GtkTreeSelection *selection,
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    gtk_tree_model_get(model, &iter, LIST_COLUMN_PINFO, &pinfo,
                       LIST_COLUMN_PATH, &mpath, -1);
  }
  ...
  if (find_existing_mpath(model, mpath, &iter)) {
    ...
    g_signal_handlers_block_by_func(tree_selection,
                                    browser_tree_selection_changed, browser);
    gtk_tree_selection_select_iter(tree_selection, &iter);
    g_signal_handlers_unblock_by_func(tree_selection,
                                      browser_tree_selection_changed, browser);

    gtk_tree_view_scroll_to_cell(browser->tree_view, tree_path, NULL, TRUE, 0.5,
                                 0.0);
  } else {
...
static void browser_tree_selection_changed(GtkTreeSelection *selection,
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    gtk_tree_model_get(model, &iter, TREE_COLUMN_PINFO, &pinfo,
                       TREE_COLUMN_MPATH, &mpath, -1);
  }
  ...
  while (valid) {
    ...
    gtk_tree_model_get(model, &iter, LIST_COLUMN_PATH, &picked_mpath, -1);
  ...
  if (found) {
    ...
    g_signal_handlers_block_by_func(list_selection,
                                    browser_list_selection_changed, browser);
    gtk_tree_selection_select_iter(list_selection, &iter);
    g_signal_handlers_unblock_by_func(list_selection,
                                      browser_list_selection_changed, browser);

    gtk_tree_view_scroll_to_cell(browser->list_view, tree_path, NULL, TRUE, 0.5,
                                 0.0);
  } else {
