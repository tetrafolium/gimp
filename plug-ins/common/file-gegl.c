...
static GimpValueArray *goat_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                 GFile *file, const GimpValueArray *args,
                                 gpointer run_data);
static GimpValueArray *goat_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static const FileFormat file_formats[] = {
    {N_("Radiance RGBE"), "image/vnd.radiance", "hdr", "0,string,?#",

     "file-load-rgbe", "Load files in the RGBE file format",
     "This procedure loads images in the RGBE format, using gegl:load",

     "file-save-rgbe", "Saves files in the RGBE file format",
     "This procedure exports images in the RGBE format, using gegl:save"},
    {N_("OpenEXR image"), "image/x-exr", "exr", "0,lelong,20000630",

     /* no EXR loading (implemented in native GIMP plug-in) */
     NULL, NULL, NULL,

     "file-exr-save", "Saves files in the OpenEXR file format",
    ...
    if (!g_strcmp0(name, format->load_proc)) {
      procedure =
          gimp_load_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                  goat_load, (gpointer)format, NULL);

      gimp_procedure_set_menu_label(procedure, format->file_type);

      gimp_procedure_set_documentation(procedure, format->load_blurb,
                                       format->load_help, name);
      ...
      procedure =
          gimp_save_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                  goat_save, (gpointer)format, NULL);

      gimp_procedure_set_image_types(procedure, "*");

      gimp_procedure_set_menu_label(procedure, format->file_type);

      gimp_procedure_set_documentation(procedure, format->save_blurb,
                                       format->save_help, name);
...
static GimpValueArray *goat_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "GEGL",
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  source = gegl_node_new_child(graph, "operation", "gegl:load", "path",
                               filename, NULL);
  sink = gegl_node_new_child(graph, "operation", "gegl:buffer-sink", "buffer",
                             &src_buf, NULL);
  ...
  if (!src_buf) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Could not open '%s'"), gimp_file_get_utf8_name(file));
    return NULL;
  }
  ...
  if (babl_format_is_palette(format)) {
    ...
    if (model == babl_model("Y") || model == babl_model("Y'") ||
      ...
      if (model == babl_model("Y'") || model == babl_model("Y'A"))
        linear = FALSE;
    } else {
      base_type = GIMP_RGB;
      ...
      if (model == babl_model("R'G'B'") || model == babl_model("R'G'B'A"))
        linear = FALSE;
    }
  ...
  image = gimp_image_new_with_precision(width, height, base_type, precision);
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  source = gegl_node_new_child(graph, "operation", "gegl:buffer-source",
                               "buffer", src_buf, NULL);
  sink = gegl_node_new_child(graph, "operation", "gegl:save", "path", filename,
                             NULL);
