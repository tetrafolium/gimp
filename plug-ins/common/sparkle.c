...
static gint compute_luminosity(const guchar *pixel, gboolean gray,
                               gboolean has_alpha);
static gint compute_lum_threshold(GimpDrawable *drawable, gdouble percentile);
static void sparkle(GimpDrawable *drawable, GimpPreview *preview);
static void sparkle_preview(GimpDrawable *drawable, GimpPreview *preview);
static void fspike(GeglBuffer *src_buffer, GeglBuffer *dest_buffer,
                   const Babl *format, gint bytes, gint x1, gint y1, gint x2,
                   gint y2, gint xr, gint yr, gdouble inten, gdouble length,
                   gdouble angle, GRand *gr, guchar *dest_buf);
static void rpnt(GeglBuffer *dest_buffer, const Babl *format, gint x1, gint y1,
                 gint x2, gint y2, gdouble xr, gdouble yr, gint bytes,
                 gdouble inten, guchar color[MAX_CHANNELS], guchar *dest_buf);
...
static void sparkle_class_init(SparkleClass *klass) {
...
static void sparkle_init(Sparkle *sparkle) {}
...
static GimpProcedure *sparkle_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, sparkle_run, NULL, NULL);
    ...
    GIMP_PROC_ARG_DOUBLE(procedure, "lum-threshold", "Lum threshold",
                         "Luminosity threshold", 0.0, 1.0, 0.001,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "flare-inten", "Flare inten",
                         "Flare intensity", 0.0, 1.0, 0.5, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "spike-len", "Spike len",
                      "Spike length (in pixels)", 1, 1000, 20,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "spike-points", "Spike points",
                      "# of spike points", 1, 1000, 4, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "spike-angle", "Spike angle",
                      "Spike angle (-1: random)", -1, 360, 15,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "density", "Density", "Spike density", 0.0,
                         1.0, 1.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "transparency", "Transparency",
                         "Transparency", 0.0, 1.0, 0.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "random-hue", "Random hue", "Random hue",
                         0.0, 1.0, 0.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "random-saturation", "Random saturation",
                         "Random saturation", 0.0, 1.0, 0.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "preserve-luminosity",
                          "Preserve luminosity", "Preserve luminosity", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "inverse", "Inverse", "Inverse", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "border", "Border", "Add border", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "color-type", "Color type",
                      "Color of sparkles: { NATURAL (0), "
                      "FOREGROUND (1), BACKGROUND (2) }",
                      0, 2, NATURAL, G_PARAM_READWRITE);
  }
...
static gboolean sparkle_dialog(GimpDrawable *drawable) {
  ...
  dialog = gimp_dialog_new(_("Sparkle"), PLUG_IN_ROLE, NULL, 0,
  ...
  g_signal_connect_swapped(preview, "invalidated", G_CALLBACK(sparkle_preview),
                           drawable);
  ...
  scale = gimp_scale_entry_new(_("Luminosity _threshold:"), svals.lum_threshold,
                               0.0, 0.1, 3);
  ...
  scale = gimp_scale_entry_new(_("F_lare intensity:"), svals.flare_inten, 0.0,
                               1.0, 2);
  ...
  scale = gimp_scale_entry_new(_("Spi_ke angle (-1: random):"),
                               svals.spike_angle, -1, 360, 0);
  ...
  scale =
      gimp_scale_entry_new(_("Spik_e density:"), svals.density, 0.0, 1.0, 2);
  ...
  scale = gimp_scale_entry_new(_("Tr_ansparency:"), svals.transparency, 0.0,
                               1.0, 2);
  ...
  scale =
      gimp_scale_entry_new(_("_Random hue:"), svals.random_hue, 0.0, 1.0, 2);
  gimp_help_set_help_data(
      scale, _("Adjust how much the hue should be changed randomly"), NULL);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(sparkle_scale_entry_update_double),
                   &svals.random_hue);
  g_signal_connect_swapped(scale, "value-changed",
  ...
  scale = gimp_scale_entry_new(_("Rando_m saturation:"),
                               svals.random_saturation, 0.0, 1.0, 2);
  gimp_help_set_help_data(
      scale, _("Adjust how much the saturation should be changed randomly"),
      NULL);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(sparkle_scale_entry_update_double),
  ...
  gimp_help_set_help_data(toggle, _("Should the effect be inversed?"), NULL);
  ...
  gimp_help_set_help_data(toggle, _("Draw a border of spikes around the image"),
                          NULL);
  ...
  /*  colortype  */
  vbox = gimp_int_radio_group_new(
      FALSE, NULL, G_CALLBACK(gimp_radio_button_update), &svals.colortype, NULL,
      svals.colortype,

      _("_Natural color"), NATURAL, &r1, _("_Foreground color"), FOREGROUND,
      &r2, _("_Background color"), BACKGROUND, &r3,
  ...
  g_signal_connect_swapped(r1, "toggled", G_CALLBACK(gimp_preview_invalidate),
                           preview);
  g_signal_connect_swapped(r2, "toggled", G_CALLBACK(gimp_preview_invalidate),
                           preview);
  g_signal_connect_swapped(r3, "toggled", G_CALLBACK(gimp_preview_invalidate),
                           preview);
...
static gint compute_lum_threshold(GimpDrawable *drawable, gdouble percentile) {
  ...
  iter = gegl_buffer_iterator_new(src_buffer,
                                  ...
                                  format, GEGL_ACCESS_READ, GEGL_ABYSS_NONE, 1);
...
static void sparkle(GimpDrawable *drawable, GimpPreview *preview) {
  ...
  gegl_buffer_iterator_add(iter, dest_buffer,
                           GEGL_RECTANGLE(x1, y1, width, height), 0, format,
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (y = 0; y < roi.height; y++) {
      ...
      for (x = 0; x < roi.width; x++) {
        if (svals.border) {
          if (x + roi.x == 0 || y + roi.y == 0 || x + roi.x == d_width - 1 ||
              y + roi.y == d_height - 1) {
            lum = 255;
          } else {
            lum = 0;
        ...
        if (lum >= threshold) {
          nfrac =
              fabs((gdouble)(lum + 1 - threshold) / (gdouble)(256 - threshold));
          ...
          if (svals.spike_pts > 0) {
            ...
            if (g_rand_double(gr) <= svals.density) {
              fspike(src_buffer, dest_buffer, format, bytes, x1, y1, x2, y2,
                     x + roi.x, y + roi.y, inten, length, spike_angle, gr,
                     dest_buf);

              /* minor spikes */
              fspike(src_buffer, dest_buffer, format, bytes, x1, y1, x2, y2,
                     x + roi.x, y + roi.y, inten * 0.7, length * 0.7,
...
static inline void rpnt(GeglBuffer *dest_buffer, const Babl *format, gint x1,
                        gint y1, gint x2, gint y2, gdouble xr, gdouble yr,
                        gint bytes, gdouble inten, guchar color[MAX_CHANNELS],
                        guchar *dest_buf) {
  gint x, y, b;
  ...
  if (x >= x1 && y >= y1 && x < x2 && y < y2) {
    if (dest_buf) {
      ...
      gegl_buffer_sample(dest_buffer, x, y, NULL, pixel_buf, format,
    ...
    if (!dest_buf)
      gegl_buffer_set(dest_buffer, GEGL_RECTANGLE(x, y, 1, 1), 0, format,
                      pixel_buf, GEGL_AUTO_ROWSTRIDE);
  }
}
...
static void fspike(GeglBuffer *src_buffer, GeglBuffer *dest_buffer,
  ...
  for (i = 0; i < svals.spike_pts; i++) {
    ...
    if (svals.random_hue > 0.0 || svals.random_saturation > 0.0) {
      ...
      hsv.v += (svals.random_saturation * g_rand_double_range(gr, -1.0, 1.0));
    ...
    do {
      ...
      rpnt(dest_buffer, format, x1, y1, x2, y2, xrt, yrt, bytes, in, color,
           dest_buf);

      rpnt(dest_buffer, format, x1, y1, x2, y2, xrt + 1.0, yrt, bytes, in,
           color, dest_buf);

      rpnt(dest_buffer, format, x1, y1, x2, y2, xrt + 1.0, yrt + 1.0, bytes, in,
           color, dest_buf);

      rpnt(dest_buffer, format, x1, y1, x2, y2, xrt, yrt + 1.0, bytes, in,
           color, dest_buf);
