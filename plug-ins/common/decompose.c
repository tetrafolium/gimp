...
#define DECOMPOSE                                                              \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), DECOMPOSE_TYPE, Decompose))
...
static gint decompose(GimpImage *image, GimpDrawable *drawable, GObject *config,
                      GimpImage **image_dst, gint32 *num_layers,
                      GimpLayer **layer_dst);
static GimpImage *create_new_image(GFile *file, const gchar *layername,
                                   guint width, guint height,
                                   GimpImageBaseType type,
                                   GimpPrecision precision, gdouble xres,
                                   gdouble yres, GimpLayer **layer);
static GimpLayer *create_new_layer(GimpImage *image, gint position,
                                   const gchar *layername, guint width,
                                   guint height, GimpImageBaseType type);
static void transfer_registration_color(GeglBuffer *src, GeglBuffer **dst,
                                        gint count);
static void cpn_affine_transform_clamp(GeglBuffer *buffer, gdouble min,
                                       gdouble max, gboolean clamp);
static void copy_n_components(GeglBuffer *src, GeglBuffer **dst, Extract ext);
static void copy_one_component(GeglBuffer *src, GeglBuffer *dst,
                               const char *model, const Component component,
                               gboolean clamp);
static gboolean decompose_dialog(GimpProcedure *procedure, GObject *config);
static gchar *generate_filename(GimpImage *image, GObject *config,
                                guint colorspace, guint channel);
...
#define CPN_RGBA_R                                                             \
  { "R", N_("red"), 0.0, 1.0, FALSE }
#define CPN_RGBA_G                                                             \
  { "G", N_("green"), 0.0, 1.0, FALSE }
#define CPN_RGBA_B                                                             \
  { "B", N_("blue"), 0.0, 1.0, FALSE }
#define CPN_RGBA_A                                                             \
  { "A", N_("alpha"), 0.0, 1.0, TRUE }

#define CPN_HSV_H                                                              \
  { "hue", N_("hue"), 0.0, 1.0, TRUE }
#define CPN_HSV_S                                                              \
  { "saturation", N_("saturation"), 0.0, 1.0, TRUE }
#define CPN_HSV_V                                                              \
  { "value", N_("value"), 0.0, 1.0, TRUE }
...
#define CPN_HSL_S                                                              \
  { "saturation", N_("saturation"), 0.0, 1.0, TRUE }
#define CPN_HSL_L                                                              \
  { "lightness", N_("lightness"), 0.0, 1.0, TRUE }

#define CPN_CMYK_C                                                             \
  { "Cyan", N_("cyan"), 0.0, 1.0, TRUE }
#define CPN_CMYK_M                                                             \
  { "Magenta", N_("magenta"), 0.0, 1.0, TRUE }
#define CPN_CMYK_Y                                                             \
  { "Yellow", N_("yellow"), 0.0, 1.0, TRUE }
#define CPN_CMYK_K                                                             \
  { "Key", N_("black"), 0.0, 1.0, TRUE }

#define CPN_LAB_L                                                              \
  { "CIE L", N_("L"), 0.0, 100.0, TRUE }
#define CPN_LAB_A                                                              \
  { "CIE a", N_("A"), -127.5, 127.5, TRUE }
#define CPN_LAB_B                                                              \
  { "CIE b", N_("B"), -127.5, 127.5, TRUE }

#define CPN_LCH_L                                                              \
  { "CIE L", N_("L"), 0.0, 100.0, TRUE }
#define CPN_LCH_C                                                              \
  { "CIE C(ab)", N_("C"), 0.0, 200.0, TRUE }
#define CPN_LCH_H                                                              \
  { "CIE H(ab)", N_("H"), 0.0, 360.0, TRUE }

#define CPN_YCBCR_Y                                                            \
  { "Y'", N_("luma-y470"), 0.0, 1.0, TRUE }
#define CPN_YCBCR_CB                                                           \
  { "Cb", N_("blueness-cb470"), -0.5, 0.5, TRUE }
#define CPN_YCBCR_CR                                                           \
  { "Cr", N_("redness-cr470"), -0.5, 0.5, TRUE }

#define CPN_YCBCR709_Y                                                         \
  { "Y'", N_("luma-y709"), 0.0, 1.0, TRUE }
#define CPN_YCBCR709_CB                                                        \
  { "Cb", N_("blueness-cb709"), -0.5, 0.5, TRUE }
#define CPN_YCBCR709_CR                                                        \
  { "Cr", N_("redness-cr709"), -0.5, 0.5, TRUE }
...
static void decompose_class_init(DecomposeClass *klass) {
...
static void decompose_init(Decompose *decompose) {}
...
static GimpProcedure *decompose_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    ...
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, decompose_run, NULL, NULL);
    ...
    GIMP_PROC_ARG_STRING(procedure, "decompose-type", "Decompose type",
                         type_desc->str, "RGB", G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "layers-mode", "Layers mode",
                          "Create channels as layers in a single image", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "use-registration", "Use registration",
    ...
    GIMP_PROC_VAL_IMAGE(procedure, "new-image-1", "New image 1",
                        "Output gray image 1", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_VAL_IMAGE(procedure, "new-image-2", "New image 2",
                        "Output gray image 2 (N/A for single channel extract)",
                        TRUE, G_PARAM_READWRITE);

    GIMP_PROC_VAL_IMAGE(procedure, "new-image-3", "New image 3",
                        "Output gray image 3 (N/A for single channel extract)",
                        TRUE, G_PARAM_READWRITE);

    GIMP_PROC_VAL_IMAGE(procedure, "new-image-4", "New image 4",
...
static GimpValueArray *decompose_run(GimpProcedure *procedure,
  ...
  g_object_get(config, "decompose-type", &decompose_type, NULL);

  tmp = g_ascii_strup(decompose_type, -1);
  g_free(decompose_type);

  g_object_set(config, "decompose-type", tmp, NULL);
  ...
  num_images = decompose(image, drawable, G_OBJECT(config), image_extract,
                         &num_layers, layer_extract);
  ...
  g_string_printf(data, "source=%d type=%s ",
                  gimp_item_get_id(GIMP_ITEM(drawable)), decompose_type);
  ...
  for (j = 0; j < num_images; j++) {
    ...
    parasite = gimp_parasite_new("decompose-data", 0, data->len + 1, data->str);
 ...
 */
static gint decompose(GimpImage *image, GimpDrawable *drawable, GObject *config,
                      GimpImage **image_dst, gint *nlayers,
                      GimpLayer **layer_dst) {
  ...
  g_object_get(config, "decompose-type", &config_extract_type, "layers-mode",
               &config_as_layers, "use-registration", &config_use_registration,
               NULL);
  ...
  for (j = 0; j < num_layers; j++) {
    ...
    decomp_has_alpha |=
        !g_strcmp0("alpha", extract[extract_idx].component[j].babl_name);
    decomp_has_alpha |=
        !g_strcmp0("A", extract[extract_idx].component[j].babl_name);
  }

  requirements |= (gimp_drawable_is_rgb(drawable));
  requirements |= (gimp_drawable_is_indexed(drawable));
  requirements |=
      (gimp_drawable_is_gray(drawable) && gimp_drawable_has_alpha(drawable) &&
       (num_layers <= 2) && decomp_has_alpha);
    ...
    if (config_as_layers) {
      ...
      if (j == 0)
        image_dst[j] = create_new_image(g_file_new_for_path(filename),
                                        layername, width, height, GIMP_GRAY,
                                        precision, xres, yres, layer_dst + j);
      else
        layer_dst[j] = create_new_layer(image_dst[0], j, layername, width,
                                        height, GIMP_GRAY);
    } else {
      image_dst[j] =
          create_new_image(g_file_new_for_path(filename), NULL, width, height,
                           GIMP_GRAY, precision, xres, yres, layer_dst + j);
    }
  ...
  copy_n_components(src_buffer, dst_buffer, extract[extract_idx]);
...
static GimpImage *create_new_image(GFile *file, const gchar *layername,
  ...
  *layer = create_new_layer(image, 0, layername, width, height, type);
...
static GimpLayer *create_new_layer(GimpImage *image, gint position,
  ...
  layer = gimp_layer_new(image, layername, width, height, gdtype, 100,
...
static void transfer_registration_color(GeglBuffer *src, GeglBuffer **dst,
  ...
  gi = gegl_buffer_iterator_new(src, NULL, 0, NULL, GEGL_ACCESS_READ,
                                GEGL_ABYSS_NONE, 10);

  for (i = 0; i < count; i++) {
    gegl_buffer_iterator_add(gi, dst[i], NULL, 0, NULL, GEGL_ACCESS_READWRITE,
                             GEGL_ABYSS_NONE);
  }
    ...
    for (k = 0; k < gi->length; k++) {
      ...
      if (gimp_rgb_distance(&test, &color) < 1e-6) {
        for (j = 0; j < count; j++) {
          ...
          babl_process(babl_fish(babl_format("Y double"), dst_format), &white,
                       (guchar *)data + (k * dst_bpp), 1);
        }
      }
    }
  }
...
static void copy_one_component(GeglBuffer *src, GeglBuffer *dst,
                               const gchar *model, const Component component,
                               gboolean clamp) {
  ...
  component_format = babl_format_new(babl_model(model), babl_type("double"),
                                     babl_component(component.babl_name), NULL);
...
static gboolean decompose_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  g_object_get(config, "decompose-type", &config_extract_type, NULL);
  ...
  for (j = 0; j < G_N_ELEMENTS(extract); j++) {
    if (extract[j].dialog) {
      ...
      gtk_list_store_insert_with_values(store, NULL, G_N_ELEMENTS(extract), 0,
                                        extract[j].type, 1, label, -1);
  ...
  gimp_help_set_help_data(toggle,
                          _("Pixels in the foreground color will "
...
gchar *generate_filename(GimpImage *image, GObject *config, guint colorspace,
                         guint channel) {
  /* Build a filename like <imagename>-<channel>.<extension> */
  gchar *fname;
  gchar *filename;
  ...
  g_object_get(config, "layers-mode", &config_as_layers, NULL);
  ...
  if (fname) {
      ...
      *(extension++) = '\0';
      ...
      if (config_as_layers)
        filename = g_strdup_printf(
            "%s-%s.%s", fname, gettext(extract[colorspace].type), extension);
      else
        filename = g_strdup_printf(
            "%s-%s.%s", fname,
      ...
      if (config_as_layers)
        filename =
            g_strdup_printf("%s-%s", fname, gettext(extract[colorspace].type));
      else
        filename = g_strdup_printf(
            "%s-%s", fname,
            gettext(extract[colorspace].component[channel].channel_name));
    ...
    filename =
        g_strdup(gettext(extract[colorspace].component[channel].channel_name));
  }
