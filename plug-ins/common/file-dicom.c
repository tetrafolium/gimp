...
static void dicom_loader(guint8 *pix_buf, DicomInfo *info, GeglBuffer *buffer);
...
static void add_tag_pointer(GByteArray *group_stream, gint group, gint element,
                            const gchar *value_rep, const guint8 *data,
                            gint length);
static GSList *dicom_add_tags(FILE *dicom, GByteArray *group_stream,
                              GSList *elements);
static gboolean write_group_to_file(FILE *dicom, gint group,
...
static void dicom_class_init(DicomClass *klass) {
...
static void dicom_init(Dicom *dicom) {}
...
static GimpProcedure *dicom_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, dicom_load, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "sections.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Dov Grobgeld",
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, dicom_save, NULL, NULL);
 ...
 **/
static void add_parasites_to_image(gpointer data, gpointer user_data) {
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  if (g_ascii_strncasecmp(buf, "DICM", 4) != 0) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("'%s' is not a DICOM file."), gimp_file_get_utf8_name(file));
    g_free(dicominfo);
    fclose(dicom);
    return NULL;
  }
  ...
  while (!feof(dicom)) {
    ...
    if ((/* Always need lookup for implicit encoding */
        ...
        || ((value_rep[0] < 'A' || value_rep[0] > 'Z' || value_rep[1] < 'A' ||
             value_rep[1] > 'Z')
    ...
    if (group_word == 0x7fe0 && element_word == 0x0010) {
      ...
      g_snprintf(pname, sizeof(pname), "dcm/%04x-%04x-%s", group_word,
                 element_word, value_rep);
      if ((parasite = gimp_parasite_new(pname, GIMP_PARASITE_PERSISTENT,
...
static void dicom_loader(guint8 *pix_buffer, DicomInfo *info,
                         GeglBuffer *buffer) {
  guchar *data;
  gint row_idx;
 ...
 **/
static GSList *dicom_add_element(GSList *elements, guint16 group_word,
                                 guint16 element_word, const gchar *value_rep,
                                 guint32 element_length, guint8 *value) {
...
static GSList *dicom_add_element_copy(GSList *elements, guint16 group_word,
                                      guint16 element_word, gchar *value_rep,
                                      guint32 element_length,
                                      const guint8 *value) {
  elements = dicom_add_element(elements, group_word, element_word, value_rep,
                               element_length, g_memdup(value, element_length));
 ...
 * Returns: the new head of @elements
 */
static GSList *dicom_add_element_int(GSList *elements, guint16 group_word,
                                     guint16 element_word, gchar *value_rep,
                                     guint8 *value) {
  guint32 len;
  ...
  return dicom_add_element(elements, group_word, element_word, value_rep, len,
                           value);
}
 ...
 **/
static void dicom_element_done(gpointer data) {
  if (data) {
    DICOMELEMENT *e = data;
 ...
 **/
static void dicom_elements_destroy(GSList *elements) {
  if (elements)
 ...
 * Returns: the new head of @elements
 **/
static GSList *dicom_destroy_element(GSList *elements, DICOMELEMENT *ele) {
  if (ele) {
 ...
 **/
static gint dicom_elements_compare(gconstpointer a, gconstpointer b) {
 ...
 **/
static DICOMELEMENT *dicom_element_find_by_num(GSList *head, guint16 group_word,
                                               guint16 element_word) {
 ...
 **/
static GSList *dicom_get_elements_list(GimpImage *image) {
  GSList *elements = NULL;
  GimpParasite *parasite;
      ...
      if (strncmp(parasites[i], "dcm", 3) == 0) {
        ...
        if (parasite) {
          ...
          if (group_word > 0 && element_word > 0) {
            ...
            elements = dicom_add_element_copy(
                elements, group_word, element_word, value_rep, len, val);
          }
 ...
 *
 * Removes certain DICOMELEMENTs from the elements list which are specific to
 *the output of this plugin.
 *
 * Returns: the new head of @elements
 **/
static GSList *dicom_remove_gimp_specified_elements(GSList *elements,
                                                    gint samples_per_pixel) {
  DICOMELEMENT remove[] = {/* Image presentation group */
                           /* Samples per pixel */
  ...
  /*
   * Remove all Dicom elements which will be set as part of the writing of the
   * new file
   */
  for (i = 0; remove[i].group_word > 0; i++) {
    if ((ele = dicom_element_find_by_num(elements, remove[i].group_word,
                                         remove[i].element_word))) {
 ...
 * Returns: the new head of @elements
 **/
static GSList *dicom_ensure_required_elements_present(GSList *elements,
                                                      gchar *today_string) {
  const DICOMELEMENT defaults[] = {
      ...
      {0x0002, 0x0010, "UI", strlen("1.2.840.10008.1.2.1"),
       (guint8 *)"1.2.840.10008.1.2.1"},
      /* 0002, 0013 - Implementation version name */
      {0x0002, 0x0013, "SH", strlen("GIMP Dicom Plugin 1.0"),
       (guint8 *)"GIMP Dicom Plugin 1.0"},
      /* Identifying group */
      /* ImageType */
      {0x0008, 0x0008, "CS", strlen("ORIGINAL\\PRIMARY"),
       (guint8 *)"ORIGINAL\\PRIMARY"},
      {0x0008, 0x0016, "UI", strlen("1.2.840.10008.5.1.4.1.1.7"),
       (guint8 *)"1.2.840.10008.5.1.4.1.1.7"},
      /* Study date */
      ...
      /* Content Time */
      {0x0008, 0x0030, "TM", strlen("000000.000000"),
       (guint8 *)"000000.000000"},
      /* AccessionNumber */
      ...
      /* Patient ID */
      {0x0010, 0x0020, "LO", strlen("314159265"), (guint8 *)"314159265"},
      /* Patient Birth date */
  ...
  for (i = 0; defaults[i].group_word > 0; i++) {
    if (dicom_element_find_by_num(elements, defaults[i].group_word,
                                  defaults[i].element_word) == NULL) {
      elements = dicom_add_element(
          elements, defaults[i].group_word, defaults[i].element_word,
          defaults[i].value_rep, defaults[i].element_length, defaults[i].value);
    }
  }
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  g_snprintf(today_string, sizeof(today_string), "%04d%02d%02d", date->year,
             date->month, date->day);
  g_date_free(date);
  ...
  if (samples_per_pixel == 3)
    elements =
        dicom_add_element_int(elements, group, 0x0006, "US", (guint8 *)&zero);
  /* rows */
  elements =
      dicom_add_element_int(elements, group, 0x0010, "US", (guint8 *)&height);
  /* columns */
  elements =
      dicom_add_element_int(elements, group, 0x0011, "US", (guint8 *)&width);
  /* Bits allocated */
  elements =
      dicom_add_element_int(elements, group, 0x0100, "US", (guint8 *)&eight);
  /* Bits Stored */
  elements =
      dicom_add_element_int(elements, group, 0x0101, "US", (guint8 *)&eight);
  /* High bit */
  elements =
      dicom_add_element_int(elements, group, 0x0102, "US", (guint8 *)&seven);
  /* Pixel representation */
  elements =
      dicom_add_element_int(elements, group, 0x0103, "US", (guint8 *)&zero);
  ...
  if (src) {
    ...
    elements =
        dicom_add_element(elements, group, 0x0010, "OW",
                          width * height * samples_per_pixel, (guint8 *)src);
 ...
 */
static void dicom_print_tags(gpointer data, gpointer user_data) {
  struct {
    FILE *dicom;
    GByteArray *group_stream;
    gint last_group;
  ...
  add_tag_pointer(d->group_stream, e->group_word, e->element_word, e->value_rep,
                  e->value, e->element_length);
...
static gboolean write_group_to_file(FILE *dicom, gint group,
  ...
  if (fwrite(group_stream->data, 1, group_stream->len, dicom) !=
      group_stream->len)
    retval = FALSE;
