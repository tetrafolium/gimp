...
static gboolean save_aa(GFile *file, GimpDrawable *drawable, GObject *config,
                        GError **error);
static void gimp2aa(GimpDrawable *drawable, aa_context *context);
...
static void ascii_class_init(AsciiClass *klass) {
...
static void ascii_init(Ascii *ascii) {}
...
static GimpProcedure *ascii_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, SAVE_PROC)) {
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, ascii_save, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Tim Newsome <nuisance@cmu.edu>",
                                   "Tim Newsome <nuisance@cmu.edu>", "1997");
    ...
    GIMP_PROC_ARG_INT(procedure, "file-type", "File type", "File type to use",
                      0, i, 0, G_PARAM_READWRITE);
  }
...
static GimpValueArray *ascii_save(GimpProcedure *procedure,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "AA",
...
static gboolean save_aa(GFile *file, GimpDrawable *drawable, GObject *config,
  ...
  g_object_get(config, "file-type", &output_type, NULL);
...
static void gimp2aa(GimpDrawable *drawable, aa_context *context) {
  ...
  for (y = 0; y < height; y++) {
    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, y, width, 1), 1.0, format, buf,
    ...
    case 3: /* RGB */
      for (x = 0, p = buf; x < width; x++, p += 3)
        aa_putpixel(context, x, y, GIMP_RGB_LUMINANCE(p[0], p[1], p[2]) + 0.5);
      break;
...
static gboolean save_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  for (i = 0; aa_formats[i]; i++)
    gtk_list_store_insert_with_values(store, NULL, -1, GIMP_INT_STORE_VALUE, i,
                                      GIMP_INT_STORE_LABEL,
                                      aa_formats[i]->formatname, -1);

  combo =
      gimp_prop_int_combo_box_new(config, "file-type", GIMP_INT_STORE(store));
