...
#define BUSY_DIALOG                                                            \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), BUSY_DIALOG_TYPE, BusyDialog))
...
static GimpPDBStatusType busy_dialog(gint read_fd, gint write_fd,
                                     const gchar *message, gboolean cancelable);

static gboolean busy_dialog_read_channel_notify(GIOChannel *source,
                                                GIOCondition condition,
                                                Context *context);

static gboolean busy_dialog_delete_event(GtkDialog *dialog, GdkEvent *event,
                                         Context *context);
static void busy_dialog_response(GtkDialog *dialog, gint response_id,
                                 Context *context);
...
static void busy_dialog_class_init(BusyDialogClass *klass) {
...
static void busy_dialog_init(BusyDialog *busy_dialog) {}
...
static GimpProcedure *busy_dialog_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Ell", "Ell", "2018");

    GIMP_PROC_ARG_ENUM(procedure, "run-mode", "Run mode", "The run mode",
                       GIMP_TYPE_RUN_MODE, GIMP_RUN_INTERACTIVE,
                       G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "read-fd", "The read file descriptor",
                      "The read file descriptor", G_MININT, G_MAXINT, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "write-fd", "The write file descriptor",
                      "The write file descriptor", G_MININT, G_MAXINT, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "message", "The message", "The message",
                         NULL, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(
        procedure, "cancelable", "Whether the dialog is cancelable",
        "Whether the dialog is cancelable", FALSE, G_PARAM_READWRITE);
  }
...
static GimpValueArray *busy_dialog_run(GimpProcedure *procedure,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    if (gimp_value_array_length(args) != 5) {
      ...
      status = busy_dialog(
          GIMP_VALUES_GET_INT(args, 1), GIMP_VALUES_GET_INT(args, 2),
          GIMP_VALUES_GET_STRING(args, 3), GIMP_VALUES_GET_BOOLEAN(args, 4));
    }
    break;
...
static GimpPDBStatusType busy_dialog(gint read_fd, gint write_fd,
  ...
  g_io_add_watch(context.read_channel, G_IO_IN | G_IO_ERR | G_IO_HUP,
                 (GIOFunc)busy_dialog_read_channel_notify, &context);
  ...
  /* the main window */
  if (!cancelable) {
    window = g_object_new(GTK_TYPE_WINDOW, "title", _("Please Wait"),
                          "skip-taskbar-hint", TRUE, "deletable", FALSE,
                          "resizable", FALSE, "role", "gimp-busy-dialog",
    ...
    window = g_object_new(GTK_TYPE_DIALOG, "title", _("Please Wait"),
                          "skip-taskbar-hint", TRUE, "resizable", FALSE, "role",
                          "gimp-busy-dialog", "window-position",
                          GTK_WIN_POS_CENTER, NULL);

    gtk_dialog_add_button(GTK_DIALOG(window), _("_Cancel"),
                          GTK_RESPONSE_CANCEL);

    g_signal_connect(window, "delete-event",
                     G_CALLBACK(busy_dialog_delete_event), &context);

    g_signal_connect(window, "response", G_CALLBACK(busy_dialog_response),
                     &context);
