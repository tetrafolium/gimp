...
#define CHECKERBOARD                                                           \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), CHECKERBOARD_TYPE, Checkerboard))
...
static void do_checkerboard_preview(GimpDrawable *drawable,
                                    GimpPreview *preview);
static gint inblock(gint pos, gint size);

static gboolean checkerboard_dialog(GimpImage *image, GimpDrawable *drawable);
...
static void checkerboard_class_init(CheckerboardClass *klass) {
...
static void checkerboard_init(Checkerboard *checkerboard) {}
...
static GimpProcedure *checkerboard_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, checkerboard_run, NULL, NULL);

    gimp_procedure_set_image_types(procedure, "RGB*, GRAY*");

    gimp_procedure_set_menu_label(procedure, N_("_Checkerboard (legacy)..."));
    ...
    GIMP_PROC_ARG_BOOLEAN(procedure, "psychobily", "Psychobily",
                          "Render a psychobiliy checkerboard", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "check-size", "Check size",
                      "Size of the checks", 1, GIMP_MAX_IMAGE_SIZE, 10,
                      G_PARAM_READWRITE);
  }
...
static void checkerboard_func(gint x, gint y, guchar *dest, gint bpp,
                              gpointer data) {
...
static void do_checkerboard_pattern(GimpDrawable *drawable,
  ...
  if (gimp_drawable_is_gray(drawable)) {
    ...
    gimp_rgba_get_uchar(&bg, param.bg, param.bg + 1, param.bg + 2,
                        param.bg + 1);

    gimp_rgba_get_uchar(&fg, param.fg, param.fg + 1, param.fg + 2,
                        param.fg + 3);
  ...
  if (preview) {
    ...
    for (i = 0; i < width * height; i++) {
      checkerboard_func(x1 + i % width, y1 + i / width, buffer + i * bpp, bpp,
                        &param);
    }
    ...
    iter =
        gegl_buffer_iterator_new(buffer, GEGL_RECTANGLE(x, y, w, h), 0, format,
    ...
    while (gegl_buffer_iterator_next(iter)) {
      ...
      for (y1 = 0; y1 < roi.height; y1++) {
        for (x1 = 0; x1 < roi.width; x1++) {
          checkerboard_func(roi.x + x1, roi.y + y1, d + x1 * bpp, bpp, &param);
        }
      ...
      gimp_progress_update((gdouble)progress_done / (gdouble)progress_total);
    }
...
static gboolean checkerboard_dialog(GimpImage *image, GimpDrawable *drawable) {
  ...
  dialog = gimp_dialog_new(_("Checkerboard"), PLUG_IN_ROLE, NULL, 0,
  ...
  g_signal_connect_swapped(preview, "invalidated",
                           G_CALLBACK(do_checkerboard_preview), drawable);
  ...
  size_entry =
      gimp_size_entry_new(1, unit, "%a", TRUE, TRUE, FALSE, SPIN_BUTTON_WIDTH,
  ...
  /*  attach labels  */
  gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(size_entry), _("_Size:"), 1, 0,
                               0.0);

  g_signal_connect(size_entry, "value-changed",
                   G_CALLBACK(check_size_update_callback), &cvals.size);
