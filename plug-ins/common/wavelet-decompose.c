...
static void wavelet_blur(GimpDrawable *drawable, gint radius);

static gboolean wavelet_decompose_dialog(void);
static void wavelet_scale_entry_update_int(GimpLabelSpin *entry, gint *value);
...
static void wavelet_class_init(WaveletClass *klass) {
...
static void wavelet_init(Wavelet *wavelet) {}
...
static GimpProcedure *wavelet_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, wavelet_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure, N_("Wavelet decompose"),
                                     "Compute and render wavelet scales", name);
    gimp_procedure_set_attribution(
        procedure, "Miroslav Talasek <miroslav.talasek@seznam.cz>",
        "Miroslav Talasek <miroslav.talasek@seznam.cz>", "19january 2017");

    GIMP_PROC_ARG_INT(procedure, "scales", "Scales", "Number of scales", 1, 7,
                      5, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "create-group", "Create group",
                          "Create a layer group to store the "
                          "decomposition",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "create-masks", "Create masks",
                          "Add a layer mask to each scales layer", FALSE,
                          G_PARAM_READWRITE);
  }
...
static GimpValueArray *wavelet_run(GimpProcedure *procedure,
  ...
  if (wavelet_params.create_group) {
    ...
    gimp_image_insert_layer(
        image, parent, GIMP_LAYER(gimp_item_get_parent(GIMP_ITEM(drawable))),
  ...
  for (id = 0; id < wavelet_params.scales; id++) {
    ...
    gimp_image_insert_layer(
        image, tmp, parent,
        gimp_image_get_item_position(image, GIMP_ITEM(new_scale)));
    wavelet_blur(GIMP_DRAWABLE(tmp), pow(2.0, id));

    blur = gimp_layer_copy(tmp);
    gimp_image_insert_layer(
        image, blur, parent,
        gimp_image_get_item_position(image, GIMP_ITEM(tmp)));

    gimp_layer_set_mode(tmp, grain_extract_mode);
    new_scale = gimp_image_merge_down(image, tmp, GIMP_EXPAND_AS_NECESSARY);
    scale_layers[id] = new_scale;
  ...
  for (id = 0; id < wavelet_params.scales; id++) {
    gimp_image_reorder_item(
        image, GIMP_ITEM(scale_layers[id]), GIMP_ITEM(parent),
        gimp_image_get_item_position(image, GIMP_ITEM(new_scale)));
    gimp_layer_set_mode(scale_layers[id], grain_merge_mode);

    if (wavelet_params.create_masks) {
      GimpLayerMask *mask =
          gimp_layer_create_mask(scale_layers[id], GIMP_ADD_MASK_WHITE);
  ...
  if (run_mode == GIMP_RUN_INTERACTIVE)
    gimp_set_data(PLUG_IN_PROC, &wavelet_params,
                  sizeof(WaveletDecomposeParams));
...
static void wavelet_blur(GimpDrawable *drawable, gint radius) {
  ...
  if (gimp_drawable_mask_intersect(drawable, &x, &y, &width, &height)) {
    ...
    gegl_render_op(buffer, shadow, "gegl:wavelet-blur", "radius",
                   (gdouble)radius, NULL);
...
static gboolean wavelet_decompose_dialog(void) {
  ...
  dialog = gimp_dialog_new(_("Wavelet decompose"), PLUG_IN_ROLE, NULL, 0,
  ...
  scale =
      gimp_scale_entry_new(_("Scales:"), wavelet_params.scales, 1.0, 7.0, 0);
  ...
  button = gtk_check_button_new_with_mnemonic(
      _("Create a layer group to store the decomposition"));
  ...
  button = gtk_check_button_new_with_mnemonic(
      _("Add a layer mask to each scales layers"));
  gtk_box_pack_start(GTK_BOX(main_vbox), button, FALSE, FALSE, 0);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button),
