...
typedef struct {
  ...
  const gboolean
      is_perceptual; /* Take the componenent from an Y' or Y buffer */
} COMPONENT_DSC;
...
typedef struct {
  ...
  ComposeInput
      selected[MAX_COMPOSE_IMAGES]; /* Image Ids or mask values from menus */
...
static void cpn_affine_transform(GeglBuffer *buffer, gdouble min, gdouble max);

static void fill_buffer_from_components(GeglBuffer *temp[MAX_COMPOSE_IMAGES],
                                        GeglBuffer *dst, gint num_cpn,
                                        ComposeInput *inputs,
...
static void perform_composition(COMPOSE_DSC curr_compose_dsc,
                                ...
                                GeglBuffer *buffer_dst, ComposeInput *inputs,
                                gint num_images);

static GimpImage *compose(const gchar *compose_type, ComposeInput *inputs,
                          gboolean compose_by_drawable);

static GimpImage *create_new_image(GFile *file, guint width, guint height,
                                   GimpImageType gdtype,
                                   GimpPrecision precision, GimpLayer **layer,
                                   GeglBuffer **buffer);

static gboolean compose_dialog(const gchar *compose_type,
                               GimpDrawable *drawable);

static gboolean check_gray(GimpImage *image, GimpItem *drawable, gpointer data);

static void combo_callback(GimpIntComboBox *cbox, gpointer data);

static void scale_callback(GimpLabelSpin *scale, ComposeInput *input);

static void check_response(GtkWidget *dialog, gint response, gpointer data);

static void type_combo_callback(GimpIntComboBox *combo, gpointer data);
...
#define CPN_RGBA_R                                                             \
  { "R", N_("_Red:"), GIMP_ICON_CHANNEL_RED, 0.0, 1.0, FALSE }
#define CPN_RGBA_G                                                             \
  { "G", N_("_Green:"), GIMP_ICON_CHANNEL_GREEN, 0.0, 1.0, FALSE }
#define CPN_RGBA_B                                                             \
  { "B", N_("_Blue:"), GIMP_ICON_CHANNEL_BLUE, 0.0, 1.0, FALSE }
#define CPN_RGBA_A                                                             \
  { "A", N_("_Alpha:"), GIMP_ICON_CHANNEL_ALPHA, 0.0, 1.0, TRUE }

#define CPN_HSV_H                                                              \
  { "hue", N_("_Hue:"), NULL, 0.0, 1.0, TRUE }
#define CPN_HSV_S                                                              \
  { "saturation", N_("_Saturation:"), NULL, 0.0, 1.0, TRUE }
#define CPN_HSV_V                                                              \
  { "value", N_("_Value:"), NULL, 0.0, 1.0, TRUE }
...
#define CPN_HSL_S                                                              \
  { "saturation", N_("_Saturation:"), NULL, 0.0, 1.0, TRUE }
#define CPN_HSL_L                                                              \
  { "lightness", N_("_Lightness:"), NULL, 0.0, 1.0, TRUE }

#define CPN_CMYK_C                                                             \
  { "Cyan", N_("_Cyan:"), NULL, 0.0, 1.0, TRUE }
#define CPN_CMYK_M                                                             \
  { "Magenta", N_("_Magenta:"), NULL, 0.0, 1.0, TRUE }
#define CPN_CMYK_Y                                                             \
  { "Yellow", N_("_Yellow:"), NULL, 0.0, 1.0, TRUE }
#define CPN_CMYK_K                                                             \
  { "Key", N_("_Black:"), NULL, 0.0, 1.0, TRUE }

#define CPN_LAB_L                                                              \
  { "CIE L", N_("_L:"), NULL, 0.0, 100.0, TRUE }
#define CPN_LAB_A                                                              \
  { "CIE a", N_("_A:"), NULL, -127.5, 127.5, TRUE }
#define CPN_LAB_B                                                              \
  { "CIE b", N_("_B:"), NULL, -127.5, 127.5, TRUE }

#define CPN_LCH_L                                                              \
  { "CIE L", N_("_L"), NULL, 0.0, 100.0, TRUE }
#define CPN_LCH_C                                                              \
  { "CIE C(ab)", N_("_C"), NULL, 0.0, 200.0, TRUE }
#define CPN_LCH_H                                                              \
  { "CIE H(ab)", N_("_H"), NULL, 0.0, 360.0, TRUE }

#define CPN_YCBCR_Y                                                            \
  { "Y'", N_("_Luma y470:"), NULL, 0.0, 1.0, TRUE }
#define CPN_YCBCR_CB                                                           \
  { "Cb", N_("_Blueness cb470:"), NULL, -0.5, 0.5, TRUE }
#define CPN_YCBCR_CR                                                           \
  { "Cr", N_("_Redness cr470:"), NULL, -0.5, 0.5, TRUE }

#define CPN_YCBCR709_Y                                                         \
  { "Y'", N_("_Luma y709:"), NULL, 0.0, 1.0, TRUE }
#define CPN_YCBCR709_CB                                                        \
  { "Cb", N_("_Blueness cb709:"), NULL, -0.5, 0.5, TRUE }
#define CPN_YCBCR709_CR                                                        \
  { "Cr", N_("_Redness cr709:"), NULL, -0.5, 0.5, TRUE }

static COMPOSE_DSC compose_dsc[] = {
    {"RGB", N_("RGB"), 3, {CPN_RGBA_R, CPN_RGBA_G, CPN_RGBA_B}, "rgb-compose"},

    {"RGBA",
     N_("RGBA"),
     4,
     {CPN_RGBA_R, CPN_RGBA_G, CPN_RGBA_B, CPN_RGBA_A},
     "rgba-compose"},

    {"HSV", N_("HSV"), 3, {CPN_HSV_H, CPN_HSV_S, CPN_HSV_V}, "hsv-compose"},

    {"HSL", N_("HSL"), 3, {CPN_HSL_H, CPN_HSL_S, CPN_HSL_L}, "hsl-compose"},

    {"CMYK",
     N_("CMYK"),
     4,
     {CPN_CMYK_C, CPN_CMYK_M, CPN_CMYK_Y, CPN_CMYK_K},
     "cmyk-compose"},

    {"CIE Lab", N_("LAB"), 3, {CPN_LAB_L, CPN_LAB_A, CPN_LAB_B}, "lab-compose"},

    {"CIE LCH(ab)",
     N_("LCH"),
     3,
     {CPN_LCH_L, CPN_LCH_C, CPN_LCH_H},
     "lch-compose"},

    {"Y'CbCr",
     N_("YCbCr_ITU_R470"),
     3,
     {CPN_YCBCR_Y, CPN_YCBCR_CB, CPN_YCBCR_CR},
     "ycbcr470-compose"},

    {"Y'CbCr709",
     N_("YCbCr_ITU_R709"),
     3,
     {CPN_YCBCR709_Y, CPN_YCBCR709_CB, CPN_YCBCR709_CR},
     "ycbcr709-compose"},
...
static ComposeVals composevals = {
    {{
        0,
    }},    /* Image IDs of images to compose or mask values */
...
static ComposeInterface composeint = {
    ...
    {{
        0,
    }}, /* Image Ids or mask values from menus */
    0   /* Compose type */
};

static void compose_class_init(ComposeClass *klass) {
...
static void compose_init(Compose *compose) {}
...
static GimpProcedure *compose_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, COMPOSE_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, compose_run, NULL, NULL);
    ...
    GIMP_PROC_ARG_IMAGE(procedure, "image-2", "Image 2", "Second input image",
                        TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_IMAGE(procedure, "image-3", "Image 3", "Third input image",
                        TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_IMAGE(procedure, "image-4", "Image 4", "Fourth input image",
                        TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "compose-type", "Compose type",
                         type_desc->str, "RGB", G_PARAM_READWRITE);
    ...
    GIMP_PROC_ARG_DRAWABLE(procedure, "drawable-2", "Drawable 2",
                           "Second input drawable", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "drawable-3", "Drawable 3",
                           "Third input drawable", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "drawable-4", "Drawable 4",
                           "Fourth input drawable", TRUE, G_PARAM_READWRITE);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Bill Skaggs", "Bill Skaggs",
                                   "2004");
  }
...
static GimpValueArray *compose_run(GimpProcedure *procedure,
  ...
  if (!strcmp(name, RECOMPOSE_PROC)) {
    GimpParasite *parasite = gimp_image_get_parasite(image, "decompose-data");
    ...
    if (!parasite) {
      ...
      nret = sscanf(gimp_parasite_data(parasite),
                    "source=%d type=%31s %d %d %d %d", &source,
                    composevals.compose_type, input, input + 1, input + 2,
                    input + 3);
    ...
    case GIMP_RUN_NONINTERACTIVE:
      ...
      g_strlcpy(composevals.compose_type, GIMP_VALUES_GET_STRING(args, 3),
  ...
  image = compose(composevals.compose_type, composevals.inputs,
                  compose_by_drawable);
...
static void cpn_affine_transform(GeglBuffer *buffer, gdouble min, gdouble max) {
  ...
  gi = gegl_buffer_iterator_new(buffer, NULL, 0, NULL, GEGL_ACCESS_READWRITE,
                                GEGL_ABYSS_NONE, 1);
...
static void fill_buffer_from_components(GeglBuffer *temp[MAX_COMPOSE_IMAGES],
  ...
  gi = gegl_buffer_iterator_new(dst, NULL, 0, NULL, GEGL_ACCESS_WRITE,
                                GEGL_ABYSS_NONE, 10);

  for (j = 0; j < num_cpn; j++) {
    if (inputs[j].is_object)
      gegl_buffer_iterator_add(gi, temp[j], NULL, 0, NULL, GEGL_ACCESS_READ,
                               GEGL_ABYSS_NONE);
  }
...
static void perform_composition(COMPOSE_DSC curr_compose_dsc,
  ...
  for (i = 0; i < num_images; i++) {
    ...
    if (!inputs[i].is_object) {
      ...
      mask_vals[i] = mask_vals[i] * (cpn_dsc.range_max - cpn_dsc.range_min) +
                     cpn_dsc.range_min;
    } else {
  ...
  dst_format = babl_format_new(
      babl_model(curr_compose_dsc.babl_model), babl_type("double"),
      ...
      num_images > 3 ? babl_component(components[3].babl_name) : NULL, NULL);
...
static GimpImage *compose(const gchar *compose_type, ComposeInput *inputs,
  ...
  if (compose_by_drawable) {
    GimpImage *first_image =
        gimp_item_get_image(inputs[first_object].comp.object);
  ...
  if (composevals.do_recompose) {
    ...
    gdtype_dst =
        ((babl_model(compose_dsc[compose_idx].babl_model) == babl_model("RGBA"))
             ? GIMP_RGBA_IMAGE
             : GIMP_RGB_IMAGE);

    image_dst = create_new_image(
        g_file_new_for_path(compose_dsc[compose_idx].filename), width, height,
        gdtype_dst, precision, &layer_dst, &buffer_dst);
  }
  ...
  perform_composition(compose_dsc[compose_idx], buffer_src, buffer_dst, inputs,
                      num_images);
...
static gboolean compose_dialog(const gchar *compose_type,
  ...
  layer_list =
      gimp_image_get_layers(gimp_item_get_image(GIMP_ITEM(drawable)), &nlayers);

  dialog = gimp_dialog_new(_("Compose"), PLUG_IN_ROLE, NULL, 0,
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(check_response), NULL);
  ...
  for (j = 0; j < G_N_ELEMENTS(compose_dsc); j++) {
    ...
    gimp_int_combo_box_append(GIMP_INT_COMBO_BOX(combo), GIMP_INT_STORE_LABEL,
                              label, GIMP_INT_STORE_VALUE, j, -1);
    g_free(label);
  }
  ...
  for (j = 0; j < MAX_COMPOSE_IMAGES; j++) {
    ...
    gtk_list_store_set(GTK_LIST_STORE(model), &iter, GIMP_INT_STORE_VALUE, -1,
                       GIMP_INT_STORE_LABEL, _("Mask value"),
                       GIMP_INT_STORE_ICON_NAME, GIMP_ICON_CHANNEL_GRAY, -1);
    ...
    g_signal_connect(scale, "value-changed", G_CALLBACK(scale_callback),
    ...
    gimp_int_combo_box_connect(
        ...
        G_CALLBACK(combo_callback), GINT_TO_POINTER(j), NULL);
  }

  g_free(layer_list);

  /* Calls the combo callback and sets icons, labels and sensitivity */
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), composeint.compose_idx,
                             G_CALLBACK(type_combo_callback), NULL, NULL);
...
static void combo_callback(GimpIntComboBox *widget, gpointer data) {
  gint id;
  gint n;
