...
typedef enum { LIC_HUE, LIC_SATURATION, LIC_BRIGHTNESS } LICEffectChannel;
...
static GimpValueArray *lic_run(GimpProcedure *procedure, GimpRunMode run_mode,
                               GimpImage *image, GimpDrawable *drawable,
                               const GimpValueArray *args, gpointer run_data);
static void lic_scale_entry_update(GimpLabelSpin *entry, gdouble *value);

G_DEFINE_TYPE(Lic, lic, GIMP_TYPE_PLUG_IN)

GIMP_MAIN(LIC_TYPE)

static void lic_class_init(LicClass *klass) {
...
static void peek(GeglBuffer *buffer, gint x, gint y, GimpRGB *color) {
  gegl_buffer_sample(buffer, x, y, NULL, color, babl_format("R'G'B'A double"),
                     GEGL_SAMPLER_NEAREST, GEGL_ABYSS_NONE);
}

static void poke(GeglBuffer *buffer, gint x, gint y, GimpRGB *color) {
  gegl_buffer_set(buffer, GEGL_RECTANGLE(x, y, 1, 1), 0,
                  babl_format("R'G'B'A double"), color, GEGL_AUTO_ROWSTRIDE);
}

static gint peekmap(const guchar *image, gint x, gint y) {
  while (x < 0)
    x += effect_width;
  x %= effect_width;
...
static gint gradx(const guchar *image, gint x, gint y) {
  gint val = 0;
...
static gint grady(const guchar *image, gint x, gint y) {
  gint val = 0;
...
static gdouble cubic(gdouble t) {
  gdouble at = fabs(t);

  return (at < 1.0) ? at * at * (2.0 * at - 3.0) + 1.0 : 0.0;
}

static gdouble omega(gdouble u, gdouble v, gint i, gint j) {
  while (i < 0)
    i += numx;
...
static gdouble noise(gdouble x, gdouble y) {
  ...
  for (i = sti; i <= sti + 1; i++)
    for (j = stj; j <= stj + 1; j++)
      sum +=
          omega((x - (gdouble)i * dx) / dx, (y - (gdouble)j * dy) / dy, i, j);
...
static gdouble filter(gdouble u) {
...
static gdouble lic_noise(gint x, gint y, gdouble vx, gdouble vy) {
  gdouble i = 0.0;
...
static void getpixel(GeglBuffer *buffer, GimpRGB *p, gdouble u, gdouble v) {
...
static void lic_image(GeglBuffer *buffer, gint x, gint y, gdouble vx,
                      gdouble vy, GimpRGB *color) {
  gdouble u, step = 2.0 * l / isteps;
  gdouble xx = (gdouble)x, yy = (gdouble)y;
  gdouble c, s;
  GimpRGB col = {0, 0, 0, 0};
...
static guchar *rgb_to_hsl(GimpDrawable *drawable,
  ...
  for (y = 0; y < border_h; y++) {
    for (x = 0; x < border_w; x++) {
      ...
      gegl_buffer_sample(buffer, x, y, NULL, data, babl_format("R'G'B'A u8"),
                         GEGL_SAMPLER_NEAREST, GEGL_ABYSS_NONE);
...
static void compute_lic(GimpDrawable *drawable, const guchar *scalarfield,
                        gboolean rotate) {
...
static void compute_image(GimpDrawable *drawable) {
  GimpDrawable *effect_image;
  guchar *scalarfield = NULL;

  /* Get some useful info on the input drawable */
  /* ========================================== */
  if (!gimp_drawable_mask_intersect(drawable, &border_x, &border_y, &border_w,
                                    &border_h))
    return;
...
static gboolean effect_image_constrain(GimpImage *image, GimpItem *item,
                                       gpointer data) {
  return gimp_drawable_is_rgb(GIMP_DRAWABLE(item));
}

static gboolean create_main_dialog(void) {
  GtkWidget *vbox;
  GtkWidget *hbox;
  GtkWidget *frame;
  GtkWidget *grid;
  ...
  dialog = gimp_dialog_new(_("Van Gogh (LIC)"), PLUG_IN_ROLE, NULL, 0,
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Effect Channel"), G_CALLBACK(gimp_radio_button_update),
      &licvals.effect_channel, NULL, licvals.effect_channel,

      _("_Hue"), 0, NULL, _("_Saturation"), 1, NULL, _("_Brightness"), 2, NULL,
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Effect Operator"), G_CALLBACK(gimp_radio_button_update),
      &licvals.effect_operator, NULL, licvals.effect_operator,

      _("_Derivative"), 0, NULL, _("_Gradient"), 1, NULL,
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Convolve"), G_CALLBACK(gimp_radio_button_update),
      &licvals.effect_convolve, NULL, licvals.effect_convolve,

      _("_With white noise"), 0, NULL, _("W_ith source image"), 1, NULL,
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), licvals.effect_image_id,
                             G_CALLBACK(gimp_int_combo_box_get_active),
                             &licvals.effect_image_id, NULL);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Effect image:"), 0.0, 0.5,
                           combo, 2);
  ...
  scale =
      gimp_scale_entry_new(_("_Filter length:"), licvals.filtlen, 0.1, 64, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 1.0, 8.0);
  g_signal_connect(scale, "value-changed", G_CALLBACK(lic_scale_entry_update),
                   &licvals.filtlen);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, row++, 3, 1);
  gtk_widget_show(scale);

  scale =
      gimp_scale_entry_new(_("_Noise magnitude:"), licvals.noisemag, 1, 5, 1);
  ...
  scale = gimp_scale_entry_new(_("In_tegration steps:"), licvals.intsteps, 1,
                               40, 1);
  g_signal_connect(scale, "value-changed", G_CALLBACK(lic_scale_entry_update),
                   &licvals.intsteps);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, row++, 3, 1);
  ...
  scale = gimp_scale_entry_new(_("M_aximum value:"), licvals.maxv, 0, 100, 1);
  g_signal_connect(scale, "value-changed", G_CALLBACK(lic_scale_entry_update),
                   &licvals.maxv);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, row++, 3, 1);
  gtk_widget_show(scale);
...
static GimpProcedure *lic_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, lic_run, NULL, NULL);
