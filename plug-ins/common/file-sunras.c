...
static void set_color_table(GimpImage *image, L_SUNFILEHEADER *sunhdr,
                            const guchar *suncolmap);
static GimpImage *create_new_image(GFile *file, guint width, guint height,
                                   GimpImageBaseType type, GimpLayer **layer,
                                   GeglBuffer **buffer);

static GimpImage *load_sun_d1(GFile *file, FILE *ifp, L_SUNFILEHEADER *sunhdr,
                              guchar *suncolmap);
static GimpImage *load_sun_d8(GFile *file, FILE *ifp, L_SUNFILEHEADER *sunhdr,
                              guchar *suncolmap);
static GimpImage *load_sun_d24(GFile *file, FILE *ifp, L_SUNFILEHEADER *sunhdr,
                               guchar *suncolmap);
static GimpImage *load_sun_d32(GFile *file, FILE *ifp, L_SUNFILEHEADER *sunhdr,
                               guchar *suncolmap);

static L_CARD32 read_card32(FILE *ifp, int *err);

static void write_card32(FILE *ofp, L_CARD32 c);

static void byte2bit(guchar *byteline, int width, guchar *bitline,
                     gboolean invert);

static void rle_startread(FILE *ifp);
static int rle_fread(char *ptr, int sz, int nelem, FILE *ifp);
...
static int rle_fwrite(char *ptr, int sz, int nelem, FILE *ofp);
static int rle_fputc(int val, FILE *ofp);
static int rle_putrun(int n, int val, FILE *ofp);
static void rle_endwrite(FILE *ofp);
#define rle_putc rle_fputc

static void read_sun_header(FILE *ifp, L_SUNFILEHEADER *sunhdr);
static void write_sun_header(FILE *ofp, L_SUNFILEHEADER *sunhdr);
static void read_sun_cols(FILE *ifp, L_SUNFILEHEADER *sunhdr, guchar *colormap);
static void write_sun_cols(FILE *ofp, L_SUNFILEHEADER *sunhdr,
                           guchar *colormap);

static gint save_index(FILE *ofp, GimpImage *image, GimpDrawable *drawable,
                       gboolean grey, gboolean rle);
static gint save_rgb(FILE *ofp, GimpImage *image, GimpDrawable *drawable,
                     gboolean rle);
...
static void sunras_class_init(SunrasClass *klass) {
...
static void sunras_init(Sunras *sunras) {}
...
static GimpProcedure *sunras_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, sunras_load, NULL, NULL);

    gimp_procedure_set_menu_label(procedure, N_("SUN Rasterfile image"));

    gimp_procedure_set_documentation(
        procedure, "Load file of the SunRaster file format",
        "Load file of the SunRaster file format", name);
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, sunras_save, NULL, NULL);
    ...
    GIMP_PROC_ARG_BOOLEAN(procedure, "rle", "RLE", "Use RLE output", TRUE,
                          G_PARAM_READWRITE);
  }
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  if ((sunhdr.l_ras_maptype == 1) && (sunhdr.l_ras_maplength > 0)) {
...
#ifdef DEBUG
    {
      ...
      for (j = 0; j < ncols; j++)
        printf("Entry 0x%08x: 0x%04x,  0x%04x, 0x%04x\n", j, suncolmap[j],
               suncolmap[j + ncols], suncolmap[j + 2 * ncols]);
    }
#endif
    if (sunhdr.l_ras_magic != RAS_MAGIC) {
    ...
    fseek(ifp,
          (sizeof(L_SUNFILEHEADER) / sizeof(L_CARD32)) * 4 +
              sunhdr.l_ras_maplength,
          SEEK_SET);
  }
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  g_object_get(config, "rle", &rle, NULL);
...
static void byte2bit(guchar *byteline, gint width, guchar *bitline,
                     gboolean invert) {
  guchar bitval;
  guchar rest[8];
...
static gint rle_fread(gchar *ptr, gint sz, gint nelem, FILE *ifp) {
...
static gint rle_fgetc(FILE *ifp) {
...
static gint rle_fwrite(gchar *ptr, gint sz, gint nelem, FILE *ofp) {
...
static gint rle_fputc(gint val, FILE *ofp) {
  int retval;
...
static gint rle_putrun(gint n, gint val, FILE *ofp) {
...
static void read_sun_cols(FILE *ifp, L_SUNFILEHEADER *sunhdr,
                          guchar *colormap) {
  int ncols, err = 0;
...
static GimpImage *load_sun_d1(GFile *file, FILE *ifp, L_SUNFILEHEADER *sunhdr,
  ...
  image = create_new_image(file, width, height, GIMP_INDEXED, &layer, &buffer);
...
static GimpImage *load_sun_d8(GFile *file, FILE *ifp, L_SUNFILEHEADER *sunhdr,
  ...
  if ((ncols > 0) && (suncolmap != NULL)) {
    for (j = 0; j < ncols; j++) {
      if ((suncolmap[j] != j) || (suncolmap[j + ncols] != j) ||
  ...
  image =
      create_new_image(file, width, height,
                       grayscale ? GIMP_GRAY : GIMP_INDEXED, &layer, &buffer);
...
static GimpImage *load_sun_d24(GFile *file, FILE *ifp, L_SUNFILEHEADER *sunhdr,
  ...
  image = create_new_image(file, width, height, GIMP_RGB, &layer, &buffer);
...
static gboolean save_index(FILE *ofp, GimpImage *image, GimpDrawable *drawable,
                           gboolean grey, gboolean rle) {
  ...
  src = data =
      g_malloc(tile_height * width * babl_format_get_bytes_per_pixel(format));
...
static gboolean save_rgb(FILE *ofp, GimpImage *image, GimpDrawable *drawable,
                         gboolean rle) {
...
static gboolean save_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  frame = gimp_prop_boolean_radio_frame_new(config, "rle", _("Data Formatting"),
