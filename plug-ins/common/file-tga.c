...
typedef enum { ORIGIN_TOP_LEFT = 0, ORIGIN_BOTTOM_LEFT = 1 } TgaOrigin;
...
static GimpValueArray *tga_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *tga_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static GimpImage *ReadImage(FILE *fp, tga_info *info, GFile *file);
...
static guchar magic[18] = {0x54, 0x52, 0x55, 0x45, 0x56, 0x49, 0x53, 0x49, 0x4f,
                           0x4e, 0x2d, 0x58, 0x46, 0x49, 0x4c, 0x45, 0x2e, 0x0};

static void tga_class_init(TgaClass *klass) {
...
static GimpProcedure *tga_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, tga_load, NULL, NULL);

    gimp_procedure_set_menu_label(procedure, N_("TarGA image"));

    gimp_procedure_set_documentation(procedure,
                                     "Loads files of Targa file format",
                                     "FIXME: write help for tga_load", name);
    gimp_procedure_set_attribution(
        procedure, "Raphael FRANCOIS, Gordon Matzigkeit",
        "Raphael FRANCOIS, Gordon Matzigkeit", "1997,2000,2007");
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, tga_save, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "FIXME: write help for tga_save", name);
    gimp_procedure_set_attribution(
        procedure, "Raphael FRANCOIS, Gordon Matzigkeit",
        "Raphael FRANCOIS, Gordon Matzigkeit", "1997,2000");

    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-tga");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "tga");

    GIMP_PROC_ARG_BOOLEAN(procedure, "rle", "RLE", "Use RLE compression", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "origin", "Origin",
                      "Image origin (0 = top-left, 1 = bottom-left)", 0, 1,
                      ORIGIN_BOTTOM_LEFT, G_PARAM_READWRITE);
  }
...
static GimpValueArray *tga_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "TGA",
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  if (fseek(fp, 0, SEEK_SET) || fread(header, sizeof(header), 1, fp) != 1) {
    g_message(_("Cannot read header from '%s'"), gimp_file_get_utf8_name(file));
    fclose(fp);
    return NULL;
  }
  ...
  case TGA_TYPE_MAPPED:
    if (info.bpp != 8) {
      g_message("Unhandled sub-format in '%s' (type = %u, bpp = %u)",
                gimp_file_get_utf8_name(file), info.imageType, info.bpp);
      fclose(fp);
      return NULL;
    }
    break;
  case TGA_TYPE_COLOR:
    if ((info.bpp != 15 && info.bpp != 16 && info.bpp != 24 &&
         info.bpp != 32) ||
        ((info.bpp == 15 || info.bpp == 24) && info.alphaBits != 0) ||
        (info.bpp == 16 && info.alphaBits != 1 && info.alphaBits != 0) ||
        (info.bpp == 32 && info.alphaBits != 8)) {
      g_message(
          "Unhandled sub-format in '%s' (type = %u, bpp = %u, alpha = %u)",
          gimp_file_get_utf8_name(file), info.imageType, info.bpp,
          info.alphaBits);
      fclose(fp);
      return NULL;
  ...
  default:
    g_message("Unknown image type %u for '%s'", info.imageType,
              gimp_file_get_utf8_name(file));
    fclose(fp);
    return NULL;
  }
  ...
  if (info.imageType == TGA_TYPE_MAPPED && info.colorMapType != 1) {
    g_message("Indexed image has invalid color map type %u", info.colorMapType);
    fclose(fp);
    return NULL;
...
static void rle_write(FILE *fp, guchar *buf, guint width, guint bytes) {
  gint repeat = 0;
  gint direct = 0;
  guchar *from = buf;
  guint x;
...
static gint rle_read(FILE *fp, guchar *buf, tga_info *info) {
...
static void flip_line(guchar *buf, tga_info *info) {
  guchar temp;
  guchar *alt;
  gint x, s;
...
static void upsample(guchar *dest, const guchar *src, guint width, guint bytes,
                     guint alpha) {
  guint x;
...
static void bgr2rgb(guchar *dest, const guchar *src, guint width, guint bytes,
                    guint alpha) {
  guint x;
...
static void apply_colormap(guchar *dest, const guchar *src, guint width,
                           const guchar *cmap, gboolean alpha, guint16 index) {
  guint x;
...
static void apply_index(guchar *dest, const guchar *src, guint width,
                        guint16 index) {
  guint x;
...
static void read_line(FILE *fp, guchar *row, guchar *buf, tga_info *info,
                      gint bpp, const guchar *convert_cmap) {
...
static GimpImage *ReadImage(FILE *fp, tga_info *info, GFile *file) {
    ...
    if (cmap_bytes <= 4 &&
      ...
      if (convert_cmap) {
        if (info->colorMapSize == 32)
          ...
          upsample(convert_cmap, tga_cmap, info->colorMapLength, cmap_bytes,
                   info->alphaBits);
        else {
          g_message("Unsupported colormap depth: %u", info->colorMapSize);
          return NULL;
        }
      } else {
        if (info->colorMapSize == 24)
  ...
  layer =
      gimp_layer_new(image, _("Background"), info->width, info->height, dtype,
  ...
  if (info->flipVert) {
    for (i = 0; i < info->height; i += tileheight) {
      ...
      gimp_progress_update((gdouble)(i + tileheight) / (gdouble)info->height);
    }
  } else {
    for (i = 0; i < info->height; i += max_tileheight) {
      ...
      gegl_buffer_set(buffer, GEGL_RECTANGLE(0, i, info->width, tileheight), 0,
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  g_object_get(config, "rle", &rle, "origin", &origin, NULL);
  ...
  for (row = 0; row < height; ++row) {
    if (origin) {
      gegl_buffer_get(buffer, GEGL_RECTANGLE(0, height - (row + 1), width, 1),
                      1.0, format, pixels, GEGL_AUTO_ROWSTRIDE,
                      GEGL_ABYSS_NONE);
    } else {
      gegl_buffer_get(buffer, GEGL_RECTANGLE(0, row, width, 1), 1.0, format,
...
static gboolean save_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  /*  rle  */
  toggle = gimp_prop_check_button_new(config, "rle", _("_RLE compression"));
  ...
  store = gimp_int_store_new(_("Bottom left"), ORIGIN_BOTTOM_LEFT,
                             _("Top left"), ORIGIN_TOP_LEFT, NULL);
  combo = gimp_prop_int_combo_box_new(config, "origin", GIMP_INT_STORE(store));
