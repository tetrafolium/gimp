...
enum { TRIANGLE, DISC, PLANE, SPHERE, CYLINDER, LIGHT };
...
enum { PERSPECTIVE, ORTHOGONAL, FISHEYE };

enum { FOG };

enum { TYPE, TEXTURE, NUM_COLUMNS };
...
#define DESIGNER                                                               \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), DESIGNER_TYPE, Designer))
...
static void fileselect(GtkFileChooserAction action, GtkWidget *parent);
static gint traceray(ray *r, GimpVector4 *col, gint level, gdouble imp);
...
static struct textures_t textures[] = {
    {0, N_("Solid"), SOLID},   {1, N_("Checker"), CHECKER},
    {2, N_("Marble"), MARBLE}, {3, N_("Lizard"), LIZARD},
    {4, N_("Phong"), PHONG},   {5, N_("Noise"), PERLIN},
    {6, N_("Wood"), WOOD},     {7, N_("Spiral"), SPIRAL},
    {8, N_("Spots"), SPOTS},   {0, NULL, 0}};
...
static void designer_init(Designer *designer) {}
...
static GimpProcedure *designer_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, designer_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "good quality",
                                     name);
    gimp_procedure_set_attribution(procedure, "Vidar Madsen", "Vidar Madsen",
                                   "1999");
  }
...
static gdouble noise3(gdouble *vec) {
...
static double turbulence(gdouble *point, gdouble lofreq, gdouble hifreq) {
  gdouble freq, t, p[3];
...
static inline void vcopy(GimpVector4 *a, GimpVector4 *b) { *a = *b; }

static inline void vcross(GimpVector4 *r, GimpVector4 *a, GimpVector4 *b) {
  r->x = a->y * b->z - a->z * b->y;
  r->y = -(a->x * b->z - a->z * b->x);
  r->z = a->x * b->y - a->y * b->x;
}

static inline gdouble vdot(GimpVector4 *a, GimpVector4 *b) {
  return a->x * b->x + a->y * b->y + a->z * b->z;
}

static inline gdouble vdist(GimpVector4 *a, GimpVector4 *b) {
  gdouble x, y, z;
...
static inline gdouble vdist2(GimpVector4 *a, GimpVector4 *b) {
  gdouble x, y, z;
...
static inline gdouble vlen(GimpVector4 *a) {
  return sqrt(a->x * a->x + a->y * a->y + a->z * a->z);
}

static inline void vnorm(GimpVector4 *a, gdouble v) {
  gdouble d;
...
static inline void vrotate(GimpVector4 *axis, gdouble ang,
                           GimpVector4 *vector) {
...
static inline void vcset(GimpVector4 *v, gdouble a, gdouble b, gdouble c,
                         gdouble d) {
  v->x = a;
  v->y = b;
  v->z = c;
...
static inline void vvrotate(GimpVector4 *p, GimpVector4 *rot) {
  GimpVector4 axis;
...
static inline void vsub(GimpVector4 *a, GimpVector4 *b) {
  a->x -= b->x;
  a->y -= b->y;
  a->z -= b->z;
  a->w -= b->w;
...
static inline void vadd(GimpVector4 *a, GimpVector4 *b) {
  a->x += b->x;
  a->y += b->y;
  a->z += b->z;
  a->w += b->w;
...
static inline void vneg(GimpVector4 *a) {
  a->x = -a->x;
  a->y = -a->y;
  a->z = -a->z;
  a->w = -a->w;
...
static inline void vmul(GimpVector4 *v, gdouble a) {
  v->x *= a;
  v->y *= a;
  v->z *= a;
  v->w *= a;
...
static inline void vvmul(GimpVector4 *a, GimpVector4 *b) {
  a->x *= b->x;
  a->y *= b->y;
  a->z *= b->z;
  a->w *= b->w;
...
static inline void vvdiv(GimpVector4 *a, GimpVector4 *b) {
  a->x /= b->x;
  a->y /= b->y;
  a->z /= b->z;
}

static void vmix(GimpVector4 *r, GimpVector4 *a, GimpVector4 *b, gdouble v) {
  gdouble i = 1.0 - v;
...
static double vmax(GimpVector4 *a) {
...
static void trianglenormal(GimpVector4 *n, gdouble *t, triangle *tri) {
  triangle tmp;
  vcopy(&tmp.b, &tri->b);
  vcopy(&tmp.c, &tri->c);
  vsub(&tmp.b, &tri->a);
...
static gdouble checkdisc(ray *r, disc *disc) {
...
static gdouble checksphere(ray *r, sphere *sphere) {
...
static gdouble checkcylinder(ray *r, cylinder *cylinder) {
  /* FIXME */
  return 0.0;
}

static gdouble checkplane(ray *r, plane *plane) {
...
static gdouble checktri(ray *r, triangle *tri) {
  GimpVector4 ed1, ed2;
...
static void transformpoint(GimpVector4 *p, texture *t) {
  gdouble point[3], f;
...
static void checker(GimpVector4 *q, GimpVector4 *col, texture *t) {
  gint c = 0;
  GimpVector4 p;
...
static void gradcolor(GimpVector4 *col, gradient *t, gdouble val) {
  gint i;
  gdouble d;
  GimpVector4 tmpcol;
...
static void marble(GimpVector4 *q, GimpVector4 *col, texture *t) {
  gdouble f;
  GimpVector4 p;
...
static void lizard(GimpVector4 *q, GimpVector4 *col, texture *t) {
  gdouble f;
  GimpVector4 p;
...
static void wood(GimpVector4 *q, GimpVector4 *col, texture *t) {
  gdouble f;
  GimpVector4 p;
...
static void spiral(GimpVector4 *q, GimpVector4 *col, texture *t) {
  gdouble f;
  GimpVector4 p;
...
static void spots(GimpVector4 *q, GimpVector4 *col, texture *t) {
  gdouble f;
  GimpVector4 p, r;
...
static void perlin(GimpVector4 *q, GimpVector4 *col, texture *t) {
  gdouble f, point[3];
  GimpVector4 p;
...
static void imagepixel(GimpVector4 *q, GimpVector4 *col, texture *t) {
  GimpVector4 p;
  gint x, y;
  guchar *rgb;
...
static void objcolor(GimpVector4 *col, GimpVector4 *p, common *obj) {
  gint i;
  texture *t;
  GimpVector4 tmpcol;
...
static void objnormal(GimpVector4 *res, common *obj, GimpVector4 *p) {
  gint i;
...
static void calclight(GimpVector4 *col, GimpVector4 *point, common *obj) {
  gint i, j;
  ray r;
  gdouble b, a;
  GimpVector4 lcol;
...
static void calcphong(common *obj, ray *r2, GimpVector4 *col) {
  gint i, j;
  ray r;
  gdouble b;
  GimpVector4 lcol;
...
static int traceray(ray *r, GimpVector4 *col, gint level, gdouble imp) {
  gint i, b = -1;
  ...
  if (bobj) {
    ...
    for (i = 0; i < bobj->numtexture; i++) {
      ...
      if ((world.quality >= 4) && ((bobj->texture[i].type == REFLECTION) ||
...
static void setdefaults(texture *t) {
...
static gchar *mklabel(texture *t) {
  struct textures_t *l;
...
static texture *currenttexture(void) {
  ...
  if (gtk_tree_selection_get_selected(sel, NULL, &iter)) {
    gtk_tree_model_get(gtk_tree_view_get_model(texturelist), &iter, TEXTURE, &t,
                       -1);
  }
...
static void relabel(void) {
  ...
  if (gtk_tree_selection_get_selected(sel, NULL, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, TEXTURE, &t, -1);
    gtk_list_store_set(GTK_LIST_STORE(model), &iter, TYPE, mklabel(t), -1);
  }
}

static gboolean noupdate = FALSE;

static void setvals(texture *t) {
  struct textures_t *l;
  ...
  while (l->s) {
    if (l->n == t->type) {
      gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(texturemenu), l->index);
      break;
    }
    l++;
  }
...
static void selectitem(GtkTreeSelection *treeselection, gpointer data) {
  setvals(currenttexture());
}

static void addtexture(void) {
  ...
  gtk_list_store_set(list_store, &iter, TYPE, mklabel(&s.com.texture[n]),
                     TEXTURE, &s.com.texture[n], -1);
...
static void loadit(const gchar *fn) {
  FILE *f;
  ...
  snprintf(fmt_str, sizeof(fmt_str), "%%d %%d %%%" G_GSIZE_FORMAT "s",
           sizeof(endbuf) - 1);
...
static void loadpreset_response(GtkWidget *dialog, gint response_id,
                                gpointer data) {
...
static void saveit(const gchar *fn) {
  gint i;
  FILE *f;
...
static void savepreset_response(GtkWidget *dialog, gint response_id,
                                gpointer data) {
  if (response_id == GTK_RESPONSE_OK) {
...
static void loadpreset(GtkWidget *widget, GtkWidget *parent) {
  fileselect(GTK_FILE_CHOOSER_ACTION_OPEN, parent);
}

static void savepreset(GtkWidget *widget, GtkWidget *parent) {
...
static void fileselect(GtkFileChooserAction action, GtkWidget *parent) {
  ...
  if (!windows[action]) {
    GtkWidget *dialog = windows[action] = gtk_file_chooser_dialog_new(
        gettext(titles[action]), GTK_WINDOW(parent), action,

        _("_Cancel"), GTK_RESPONSE_CANCEL,

        action == GTK_FILE_CHOOSER_ACTION_OPEN ? _("_Open") : _("_Save"),
        GTK_RESPONSE_OK,
    ...
    g_signal_connect(dialog, "destroy", G_CALLBACK(gtk_widget_destroyed),
                     &windows[action]);
    g_signal_connect(dialog, "response", G_CALLBACK(handlers[action]), NULL);
  }
...
static void selecttexture(GtkWidget *widget, gpointer data) {
  texture *t;
...
static void selecttype(GtkWidget *widget, gpointer data) {
  texture *t;
...
static void getscales(GtkWidget *widget, gpointer data) {
  gdouble f;
  texture *t;
...
static void color1_changed(GimpColorButton *button) {
  texture *t = currenttexture();
...
static void color2_changed(GimpColorButton *button) {
  texture *t = currenttexture();
...
static void drawcolor1(GtkWidget *w) {
  ...
  gimp_rgba_set(&color, t->color1.x, t->color1.y, t->color1.z, t->color1.w);

  gimp_color_button_set_color(GIMP_COLOR_BUTTON(w), &color);
}

static void drawcolor2(GtkWidget *w) {
  static GtkWidget *lastw = NULL;
  ...
  gimp_rgba_set(&color, t->color2.x, t->color2.y, t->color2.z, t->color2.w);
...
static void sphere_response(GtkWidget *widget, gint response_id,
                            gpointer data) {
  switch (response_id) {
  case RESPONSE_RESET:
    s.com.numtexture = 3;
...
static GtkWidget *makewindow(void) {
  GtkListStore *store;
  ...
  window = gimp_dialog_new(_("Sphere Designer"), PLUG_IN_ROLE, NULL, 0,
  ...
  gimp_dialog_set_alternative_button_order(GTK_DIALOG(window), RESPONSE_RESET,
                                           GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL,
                                           -1);

  gimp_window_set_transient(GTK_WINDOW(window));

  g_signal_connect(window, "response", G_CALLBACK(sphere_response), NULL);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(loadpreset), window);

  button = gtk_button_new_with_mnemonic(_("_Save"));
  gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
  gtk_widget_show(button);

  g_signal_connect(button, "clicked", G_CALLBACK(savepreset), window);
  ...
  g_signal_connect(selection, "changed", G_CALLBACK(selectitem), NULL);
  ...
  col = gtk_tree_view_column_new_with_attributes(
      _("Layers"), gtk_cell_renderer_text_new(), "text", TYPE, NULL);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
  g_signal_connect_swapped(button, "clicked", G_CALLBACK(addtexture), NULL);
  gtk_widget_show(button);

  button = gtk_button_new_with_mnemonic(_("D_uplicate"));
  gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
  g_signal_connect_swapped(button, "clicked", G_CALLBACK(duptexture), NULL);
  gtk_widget_show(button);

  button = gtk_button_new_with_mnemonic(_("_Delete"));
  gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
  g_signal_connect_swapped(button, "clicked", G_CALLBACK(deltexture), NULL);
  gtk_widget_show(button);
  ...
  typemenu = gimp_int_combo_box_new(_("Texture"), 0, _("Bump"), 1, _("Light"),
                                    2, NULL);
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(typemenu), 0,
                             G_CALLBACK(selecttype), NULL, NULL);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Type:"), 0.0, 0.5, typemenu,
                           2);
  ...
  texturemenu = g_object_new(GIMP_TYPE_INT_COMBO_BOX, NULL);
  {
    ...
    for (t = textures; t->s; t++)
      gimp_int_combo_box_append(GIMP_INT_COMBO_BOX(texturemenu),
                                ...
                                GIMP_INT_STORE_LABEL, gettext(t->s), -1);
  }

  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(texturemenu), 0,
                             G_CALLBACK(selecttexture), NULL, NULL);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Texture:"), 0.0, 0.5,
                           texturemenu, 2);

  hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 12);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Colors:"), 0.0, 0.5, hbox,
                           2);

  button = gimp_color_button_new(_("Color Selection Dialog"), COLORBUTTONWIDTH,
                                 COLORBUTTONHEIGHT, &rgb, GIMP_COLOR_AREA_FLAT);
  gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
  gtk_widget_show(button);
  drawcolor1(button);

  g_signal_connect(button, "color-changed", G_CALLBACK(color1_changed), NULL);
  ...
  g_signal_connect(button, "color-changed", G_CALLBACK(color2_changed), NULL);

  scalescale = gimp_scale_entry_new(_("Scale:"), 1.0, 0.0, 10.0, 1);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scalescale), 0.1, 1.0);
  g_signal_connect(scalescale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  g_signal_connect(turbulencescale, "value-changed", G_CALLBACK(getscales),
                   NULL);
  ...
  g_signal_connect(amountscale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  g_signal_connect(expscale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  g_signal_connect(scalexscale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  g_signal_connect(scaleyscale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  gtk_widget_set_margin_bottom(
      gimp_labeled_get_label(GIMP_LABELED(scalezscale)), 6);
  gtk_widget_set_margin_bottom(
      gimp_scale_entry_get_range(GIMP_SCALE_ENTRY(scalezscale)), 6);
  gtk_widget_set_margin_bottom(
      gimp_label_spin_get_spin_button(GIMP_LABEL_SPIN(scalezscale)), 6);
  g_signal_connect(scalezscale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  g_signal_connect(rotxscale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  g_signal_connect(rotyscale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  gtk_widget_set_margin_bottom(gimp_labeled_get_label(GIMP_LABELED(rotzscale)),
                               6);
  gtk_widget_set_margin_bottom(
      gimp_scale_entry_get_range(GIMP_SCALE_ENTRY(rotzscale)), 6);
  gtk_widget_set_margin_bottom(
      gimp_label_spin_get_spin_button(GIMP_LABEL_SPIN(rotzscale)), 6);
  g_signal_connect(rotzscale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  g_signal_connect(posxscale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  g_signal_connect(posyscale, "value-changed", G_CALLBACK(getscales), NULL);
  ...
  g_signal_connect(poszscale, "value-changed", G_CALLBACK(getscales), NULL);
...
static guchar pixelval(gdouble v) {
  v += 0.5;
  if (v < 0.0)
    return 0;
  if (v > 255.0)
    ...
    for (y = 0; y < ty; y++) {
      ...
      for (x = 0; x < tx; x++) {
        ...
        GIMP_CAIRO_RGB24_SET_PIXEL(
            (dest_row + p), pixelval(255 * col.x) * col.w + g * (1.0 - col.w),
...
static void realrender(GimpDrawable *drawable) {
  ...
  for (y = 0; y < height; y++) {
    ...
    gegl_buffer_get(src_buffer, GEGL_RECTANGLE(x1, y1 + y, width, 1), 1.0,
                    format, ibuffer, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
    ...
    gegl_buffer_set(dest_buffer, GEGL_RECTANGLE(x1, y1 + y, width, 1), 0,
                    format, ibuffer, GEGL_AUTO_ROWSTRIDE);
...
static gboolean sphere_main(GimpDrawable *drawable) {
  gimp_ui_init(PLUG_IN_BINARY);

  img_stride = cairo_format_stride_for_width(CAIRO_FORMAT_RGB24, PREVIEWSIZE);
  img = g_malloc0(img_stride * PREVIEWSIZE);

  buffer = cairo_image_surface_create_for_data(
      img, CAIRO_FORMAT_RGB24, PREVIEWSIZE, PREVIEWSIZE, img_stride);
