...
#define LEVELS_DA_MASK                                                         \
  (GDK_EXPOSURE_MASK | GDK_ENTER_NOTIFY_MASK | GDK_BUTTON_PRESS_MASK |         \
...
typedef struct {
  ...
  gint from_sample;   /* TRUE: color found in sample, FALSE: interpolated color
                         added */
...
#define COLORIZE                                                               \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), COLORIZE_TYPE, Colorize))
...
static void refresh_dst_preview(GtkWidget *preview, guchar *src_buffer);
...
static gint level_in_events(GtkWidget *widget, GdkEvent *event);
static gint level_in_draw(GtkWidget *widget, cairo_t *cr);
static gint level_out_events(GtkWidget *widget, GdkEvent *event);
static gint level_out_draw(GtkWidget *widget, cairo_t *cr);
static void calculate_level_transfers(void);
static void get_pixel(t_GDRW *gdrw, gint32 x, gint32 y, guchar *pixel);
static void init_gdrw(t_GDRW *gdrw, GimpDrawable *drawable, gboolean shadow);
static void end_gdrw(t_GDRW *gdrw);
static void remap_pixel(guchar *pixel, const guchar *original, gint bpp2);
...
static guchar
    g_dst_preview_buffer[PREVIEW_SIZE_X * PREVIEW_SIZE_Y *
                         4]; /* color copy with mask of dst in previewsize */
...
static void colorize_class_init(ColorizeClass *klass) {
...
static void colorize_init(Colorize *colorize) {}
...
static GimpProcedure *colorize_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    static gchar *help_string =
        ...
        " sample colortable. If use_subcolors is TRUE, the remapping process "
        "uses"
        ...
        " The brightness intensity value is transformed at the remapping "
        "process"
        " according to the levels: out_lo, out_hi, in_lo, in_high and gamma"
        " The in_low / in_high levels specify an initial mapping of the "
        "intensity."
        ...
        " interpolation. Higher gamma values results in more high-level "
        "intensities"
    ...
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, colorize_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     N_("Colorize image using a sample "
                                        "image as a guide"),
                                     help_string, name);
    gimp_procedure_set_attribution(procedure, "Wolfgang Hofer",
                                   "hof@hotbot.com", "02/2000");

    GIMP_PROC_ARG_DRAWABLE(procedure, "sample-drawable", "Sample drawable",
                           "Sample drawable (should be of Type RGB or RGBA)",
                           TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "hold-inten", "Hold inten",
                          "Hold brightness intensity levels", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "orig-inten", "Orig inten",
                          "TRUE: hold brightness of original intensity "
                          "levels, "
                          "FALSE: Hold Intensity of input levels",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "rnd-subcolors", "Rnd subcolors",
                          "TRUE: Use all subcolors of same intensity, "
                          "FALSE: Use only one color per intensity",
                          FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "guess-missing", "Guess missing",
                          "TRUE: guess samplecolors for the missing "
                          "intensity values, "
                          "FALSE: use only colors found in the sample",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "in-low", "In low",
                      "Intensity of lowest input", 0, 254, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "in-high", "In high",
                      "Intensity of highest input", 1, 255, 255,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "gamma", "Gamma",
                         "Gamma adjustment factor, 1.0 is linear", 0.1, 1.0,
                         1.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "out-low", "out low",
                      "Lowest sample color intensity", 0, 254, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "ouz-high", "Out high",
                      "Highest sample color intensity", 1, 255, 255,
                      G_PARAM_READWRITE);
  }
...
static void smp_response_callback(GtkWidget *widget, gint response_id) {
  switch (response_id) {
  case RESPONSE_RESET:
    g_values.lvl_in_min = 0;
    g_values.lvl_in_max = 255;
  ...
  case GTK_RESPONSE_APPLY:
    ...
    gtk_dialog_set_response_sensitive(GTK_DIALOG(widget), GTK_RESPONSE_APPLY,
                                      FALSE);
    break;
...
static void smp_toggle_callback(GtkWidget *widget, gpointer data) {
...
static void smp_sample_combo_callback(GtkWidget *widget) {
  gint value;
...
static void smp_dest_combo_callback(GtkWidget *widget) {
  gint id;
...
static gint smp_constrain(GimpImage *image, GimpItem *item, gpointer data) {
...
static void smp_adj_lvl_in_max_upd_callback(GtkAdjustment *adjustment) {
  gint32 value;
  gint upd_flags;
...
static void smp_adj_lvl_in_min_upd_callback(GtkAdjustment *adjustment) {
  double value;
  gint upd_flags;
...
static void smp_text_gamma_upd_callback(GtkAdjustment *adjustment) {
  double value;
...
static void smp_adj_lvl_out_max_upd_callback(GtkAdjustment *adjustment) {
  gint32 value;
  gint upd_flags;
...
static void smp_adj_lvl_out_min_upd_callback(GtkAdjustment *adjustment) {
  double value;
  gint upd_flags;
...
static void refresh_dst_preview(GtkWidget *preview, guchar *src_buffer) {
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(preview), 0, 0, PREVIEW_SIZE_X,
                         PREVIEW_SIZE_Y, GIMP_RGB_IMAGE, allrowsbuf,
                         PREVIEW_SIZE_X * 3);
}

static void clear_preview(GtkWidget *preview) {
  gimp_preview_area_fill(GIMP_PREVIEW_AREA(preview), 0, 0, PREVIEW_SIZE_X,
                         PREVIEW_SIZE_Y, 170, 170, 170);
}

static void update_pv(GtkWidget *preview, gboolean show_selection, t_GDRW *gdrw,
                      guchar *dst_buffer, gboolean is_color) {
  ...
  if (show_selection) {
    ...
    if (sel_height > sel_width) {
      ...
      scale_x = scale_y;
      ofx = (gdrw->x1 + ((sel_width - (PREVIEW_SIZE_X * scale_x)) / 2));
      ofy = gdrw->y1;
    } else {
      scale_x = (gfloat)sel_width / PREVIEW_SIZE_X;
      scale_y = scale_x;
      ofx = gdrw->x1;
      ofy = (gdrw->y1 + ((sel_height - (PREVIEW_SIZE_Y * scale_y)) / 2));
    }
  } else {
  ...
  for (y = 0; y < PREVIEW_SIZE_Y; y++) {
    for (x = 0; x < PREVIEW_SIZE_X; x++) {
      if (gdrw->drawable) {
        ...
        if (gdrw->sel_gdrw) {
          get_pixel(gdrw->sel_gdrw, x2 + gdrw->seldeltax, y2 + gdrw->seldeltay,
                    &maskbytes[0]);
        } else {
          maskbytes[0] = 255;
        }
  ...
  if (dst_buffer == NULL) {
    gimp_preview_area_draw(GIMP_PREVIEW_AREA(preview), 0, 0, PREVIEW_SIZE_X,
                           PREVIEW_SIZE_Y, GIMP_RGBA_IMAGE, allrowsbuf,
                           PREVIEW_SIZE_X * 4);
...
static void update_preview(GimpDrawable *drawable) {
  ...
  if (gimp_item_get_id(GIMP_ITEM(drawable)) == g_values.sample_id) {
    ...
    update_pv(g_di.sample_preview, g_di.sample_show_selection, &gdrw, NULL,
...
static void levels_draw_slider(cairo_t *cr, GdkRGBA *border_color,
                               GdkRGBA *fill_color, gint xpos) {
...
static void smp_get_colors(GtkWidget *dialog) {
  ...
  if (dialog && main_colorize(MC_GET_SAMPLE_COLORS) >=
                    0) /* do not colorize, just analyze sample colors */
    gtk_dialog_set_response_sensitive(GTK_DIALOG(g_di.dialog),
                                      GTK_RESPONSE_APPLY, TRUE);
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(g_di.sample_colortab_preview), 0, 0,
                         DA_WIDTH, GRADIENT_HEIGHT, GIMP_RGB_IMAGE, buffer,
                         DA_WIDTH * 3);
}
...
static void levels_update(gint update) {
  ...
  if (update & LOW_INPUT)
    gtk_adjustment_set_value(g_di.adj_lvl_in_min, g_values.lvl_in_min);
  if (update & GAMMA)
    gtk_adjustment_set_value(g_di.adj_lvl_in_gamma, g_values.lvl_in_gamma);
  if (update & HIGH_INPUT)
    gtk_adjustment_set_value(g_di.adj_lvl_in_max, g_values.lvl_in_max);
  if (update & LOW_OUTPUT)
    gtk_adjustment_set_value(g_di.adj_lvl_out_min, g_values.lvl_out_min);
  if (update & HIGH_OUTPUT)
    gtk_adjustment_set_value(g_di.adj_lvl_out_max, g_values.lvl_out_max);
  if (update & INPUT_LEVELS) {
    guchar buffer[DA_WIDTH * GRADIENT_HEIGHT];
    for (i = 0; i < GRADIENT_HEIGHT; i++)
      memcpy(buffer + DA_WIDTH * i, g_lvl_trans_tab, DA_WIDTH);
    gimp_preview_area_draw(GIMP_PREVIEW_AREA(g_di.in_lvl_gray_preview), 0, 0,
                           DA_WIDTH, GRADIENT_HEIGHT, GIMP_GRAY_IMAGE, buffer,
                           DA_WIDTH);
  }
...
static gboolean level_in_events(GtkWidget *widget, GdkEvent *event) {
  ...
  if (update) {
    ...
    case 0: /*  low input  */
      ...
      g_values.lvl_in_min = CLAMP(g_values.lvl_in_min, 0, g_values.lvl_in_max);
      break;
    ...
    case 1: /*  gamma  */
      ...
      g_values.lvl_in_gamma = floor(g_values.lvl_in_gamma * 100 + 0.5) / 100.0;
      break;

    case 2: /*  high input  */
      g_values.lvl_in_max = ((double)x / (double)DA_WIDTH) * 255.0;
      g_values.lvl_in_max =
          CLAMP(g_values.lvl_in_max, g_values.lvl_in_min, 255);
      break;
    }
...
static gboolean level_in_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  gtk_render_background(style, cr, 0, 0, gtk_widget_get_allocated_width(widget),
  ...
  levels_draw_slider(cr, &black, &gray, g_di.slider_pos[1]);
  levels_draw_slider(cr, &black, &black, g_di.slider_pos[0]);
  levels_draw_slider(cr, &black, &white, g_di.slider_pos[2]);

  return FALSE;
}

static gboolean level_out_events(GtkWidget *widget, GdkEvent *event) {
  GdkEventButton *bevent;
  GdkEventMotion *mevent;
  gint x, distance;
  gint i;
  ...
  if (update) {
    ...
    case 3: /*  low output  */
      ...
      g_values.lvl_out_min =
          CLAMP(g_values.lvl_out_min, 0, g_values.lvl_out_max);
      break;

    case 4: /*  high output  */
      g_values.lvl_out_max = ((double)x / (double)DA_WIDTH) * 255.0;
      g_values.lvl_out_max =
          CLAMP(g_values.lvl_out_max, g_values.lvl_out_min, 255);
      break;
    }
...
static gboolean level_out_draw(GtkWidget *widget, cairo_t *cr) {
  GtkStyleContext *style = gtk_widget_get_style_context(widget);
  GdkRGBA black = {0.0, 0.0, 0.0, 1.0};
  ...
  levels_draw_slider(cr, &black, &black, g_di.slider_pos[3]);
  levels_draw_slider(cr, &black, &black, g_di.slider_pos[4]);
...
static void smp_dialog(void) {
  ...
  /* Main Dialog */
  g_di.dialog = dialog = gimp_dialog_new(
      _("Sample Colorize"), PLUG_IN_ROLE, NULL, 0, gimp_standard_help_func,
      PLUG_IN_PROC,

      _("_Reset"), RESPONSE_RESET, _("Get _Sample Colors"), RESPONSE_GET_COLORS,
      _("_Close"), GTK_RESPONSE_CLOSE, _("_Apply"), GTK_RESPONSE_APPLY,

      NULL);

  gimp_dialog_set_alternative_button_order(
      GTK_DIALOG(dialog), RESPONSE_GET_COLORS, RESPONSE_RESET,
      GTK_RESPONSE_APPLY, GTK_RESPONSE_CLOSE, -1);

  gimp_window_set_transient(GTK_WINDOW(dialog));

  g_signal_connect(dialog, "response", G_CALLBACK(smp_response_callback), NULL);
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo),
                             gimp_item_get_id(GIMP_ITEM(g_values.dst)),
                             G_CALLBACK(smp_dest_combo_callback), NULL, NULL);
  ...
  gimp_int_combo_box_prepend(GIMP_INT_COMBO_BOX(combo), GIMP_INT_STORE_VALUE,
                             SMP_INV_GRADIENT, GIMP_INT_STORE_LABEL,
                             _("From reverse gradient"),
                             GIMP_INT_STORE_ICON_NAME, GIMP_ICON_GRADIENT, -1);
  gimp_int_combo_box_prepend(GIMP_INT_COMBO_BOX(combo), GIMP_INT_STORE_VALUE,
                             SMP_GRADIENT, GIMP_INT_STORE_LABEL,
                             _("From gradient"), GIMP_INT_STORE_ICON_NAME,
                             GIMP_ICON_GRADIENT, -1);

  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), g_values.sample_id,
                             G_CALLBACK(smp_sample_combo_callback), NULL, NULL);
  ...
  g_signal_connect(check_button, "toggled", G_CALLBACK(smp_toggle_callback),
  ...
  gtk_widget_set_size_request(g_di.dst_preview, PREVIEW_SIZE_X, PREVIEW_SIZE_Y);
  ...
  gtk_widget_set_size_request(g_di.sample_preview, PREVIEW_SIZE_X,
                              PREVIEW_SIZE_Y);
  ...
  gtk_widget_set_size_request(g_di.in_lvl_gray_preview, DA_WIDTH,
                              GRADIENT_HEIGHT);
  ...
  g_signal_connect(g_di.in_lvl_gray_preview, "event",
                   G_CALLBACK(level_in_events), NULL);
  ...
  gtk_widget_set_size_request(g_di.in_lvl_drawarea, DA_WIDTH, CONTROL_HEIGHT);
  ...
  g_signal_connect(g_di.in_lvl_drawarea, "event", G_CALLBACK(level_in_events),
                   NULL);
  g_signal_connect(g_di.in_lvl_drawarea, "draw", G_CALLBACK(level_in_draw),
                   NULL);
  ...
  gtk_widget_set_size_request(g_di.sample_colortab_preview, DA_WIDTH,
                              GRADIENT_HEIGHT);
  gtk_box_pack_start(GTK_BOX(vbox2), g_di.sample_colortab_preview, FALSE, TRUE,
                     0);
  ...
  gtk_widget_set_size_request(g_di.sample_drawarea, DA_WIDTH, CONTROL_HEIGHT);
  ...
  g_signal_connect(g_di.sample_drawarea, "event", G_CALLBACK(level_out_events),
                   NULL);
  g_signal_connect(g_di.sample_drawarea, "draw", G_CALLBACK(level_out_draw),
                   NULL);
  ...
  g_signal_connect(g_di.adj_lvl_in_min, "value-changed",
                   G_CALLBACK(smp_adj_lvl_in_min_upd_callback), &g_di);

  /* input gamma spinbutton */
  data = gtk_adjustment_new((gfloat)g_values.lvl_in_gamma, 0.1, 10.0, 0.02, 0.2,
                            0);
  ...
  g_signal_connect(g_di.adj_lvl_in_gamma, "value-changed",
                   G_CALLBACK(smp_text_gamma_upd_callback), &g_di);
  ...
  g_signal_connect(g_di.adj_lvl_in_max, "value-changed",
                   G_CALLBACK(smp_adj_lvl_in_max_upd_callback), &g_di);
  ...
  g_signal_connect(g_di.adj_lvl_out_min, "value-changed",
                   G_CALLBACK(smp_adj_lvl_out_min_upd_callback), &g_di);
  ...
  g_signal_connect(g_di.adj_lvl_out_max, "value-changed",
                   G_CALLBACK(smp_adj_lvl_out_max_upd_callback), &g_di);
...
static void print_ppm(const gchar *ppm_name) {
  FILE *fp;
  gint idx;
  gint cnt;
  gint r;
...
static void print_color_list(FILE *fp, t_samp_color_elem *col_ptr) {
  if (fp == NULL)
    return;

  while (col_ptr) {
    fprintf(fp, "  RGBA: %03d %03d %03d %03d  sum: [%d]\n",
            (gint)col_ptr->color[0], (gint)col_ptr->color[1],
            (gint)col_ptr->color[2], (gint)col_ptr->color[3],
...
static void print_table(FILE *fp) {
  ...
  for (idx = 0; idx < 256; idx++) {
    fprintf(fp, "LUM [%03d]  pixcount:%d\n", idx,
            (int)g_lum_tab[idx].all_samples);
    print_color_list(fp, g_lum_tab[idx].col_ptr);
  }
}

static void print_transtable(FILE *fp) {
  gint idx;
  ...
  for (idx = 0; idx < 256; idx++) {
    fprintf(fp, "LVL_TRANS [%03d]  in_lvl: %3d  out_lvl: %3d\n", idx,
            (int)g_lvl_trans_tab[idx], (int)g_out_trans_tab[idx]);
  }
}

static void print_values(FILE *fp) {
  if (fp == NULL)
    return;
...
static gint32 color_error(guchar ref_red, guchar ref_green, guchar ref_blue,
...
static void free_colors(void) {
  ...
  for (lum = 0; lum < 256; lum++) {
    for (col_ptr = g_lum_tab[lum].col_ptr; col_ptr != NULL;
         col_ptr = next_ptr) {
...
/* setup lum transformer table according to input_levels, gamma and output
 * levels (uses sam algorithm as GIMP Level Tool)
 */
static void calculate_level_transfers(void) {
  double inten;
  gint i;
...
static t_samp_color_elem *new_samp_color(const guchar *color) {
  t_samp_color_elem *col_ptr;
...
static void add_color(const guchar *color) {
  gint32 lum;
  t_samp_color_elem *col_ptr;
  ...
  for (col_ptr = g_lum_tab[lum].col_ptr; col_ptr != NULL;
    ...
    if ((color[0] == col_ptr->color[0]) && (color[1] == col_ptr->color[1]) &&
 ...
 */
static void sort_color(gint32 lum) {
  t_samp_color_elem *col_ptr;
  t_samp_color_elem *next_ptr;
...
static void cnt_same_sample_colortones(t_samp_color_elem *ref_ptr,
                                       guchar *prev_color, guchar *color_tone,
                                       gint *csum) {
  ...
  if (prev_color != NULL) {
    ref_error =
        color_error(ref_ptr->color[0], ref_ptr->color[1], ref_ptr->color[2],
                    prev_color[0], prev_color[1], prev_color[2]);
  }

  /* collect colors that are (nearly) the same */
  for (col_ptr = ref_ptr->next; col_ptr != NULL;
       col_ptr = (t_samp_color_elem *)col_ptr->next) {
    ...
    if (col_error <= g_tol_col_err) {
      ...
      *csum += col_ptr->sum_color;
      ...
      if (prev_color != NULL) {
        col_error =
            color_error(col_ptr->color[0], col_ptr->color[1], col_ptr->color[2],
                        prev_color[0], prev_color[1], prev_color[2]);
 ...
 */
static void ideal_samples(void) {
  gint32 lum;
  t_samp_color_elem *col_ptr;
  guchar *color;
  ...
  for (lum = 0; lum < 256; lum++) {
    ...
    /* collect colors that are (nearly) the same */
    for (; col_ptr != NULL; col_ptr = (t_samp_color_elem *)col_ptr->next) {
      csum = 0;
      if (col_ptr->sum_color > 0) {
        memcpy(&color_tone[0], &col_ptr->color[0], 3);
        cnt_same_sample_colortones(col_ptr, color, &color_tone[0], &csum);
        if (csum > maxsum) {
          maxsum = csum;
    ...
    if ((g_lum_tab[lum].col_ptr == NULL) ||
      ...
      if (lum > 0) {
        for (idx = lum; idx < 256; idx++) {
          if ((g_lum_tab[idx].col_ptr != NULL) && g_lum_tab[idx].from_sample) {
            memcpy(&hi_color[0], &g_sample_color_tab[3 * idx], 3);
            break;
          }
          if (idx == 255) {
            hi_color[0] = 255;
        ...
        /*
         *          printf ("LO: %03d %03d %03d HI: %03d %03d %03d   NEW: %03d
         * %03d %03d\n", (int)lo_color[0],  (int)lo_color[1],  (int)lo_color[2],
         *                  (int)hi_color[0],  (int)hi_color[1],
         * (int)hi_color[2], (int)new_color[0], (int)new_color[1],
         * (int)new_color[2]);
         */
      }
  ...
  for (lum = 0; lum < 256; lum++) {
    if ((g_lum_tab[lum].col_ptr == NULL) ||
      ...
      if (lum > 0) {
        ...
        if ((lum > (lo_idx + ((idx - lo_idx) / 2))) || (lo_idx == 0)) {
...
static void get_gradient(gint mode) {
  ...
  gimp_gradient_get_uniform_samples(name, 256 /* n_samples */,
                                    mode == SMP_INV_GRADIENT, &n_f_samples,
                                    &f_samples);
  ...
  for (lum = 0; lum < 256; lum++) {
    ...
    g_lum_tab[lum].col_ptr = new_samp_color(&g_sample_color_tab[3 * lum]);
    g_lum_tab[lum].from_sample = TRUE;
...
static void init_gdrw(t_GDRW *gdrw, GimpDrawable *drawable, gboolean shadow) {
  ...
  if (!gimp_drawable_mask_intersect(gdrw->drawable, &gdrw->x1, &gdrw->y1, &w,
                                    &h))
    return;
  ...
  if (non_empty && sel_channel) {
    ...
    if (g_Sdebug) {
      g_printf("init_gdrw: SEL_BOUNDS x1: %d y1: %d x2:%d y2: %d\n",
               (int)sel_gdrw->x1, (int)sel_gdrw->y1, (int)sel_gdrw->x2,
               (int)sel_gdrw->y2);
...
static int sample_analyze(t_GDRW *sample_gdrw) {
   ...
   * all pixels of one tile in the sample drawable, the outer loops (row/col) do
   * step to the next tiles. (this was done to reduce tile swapping)
   */
      ...
      for (; x < x2; x++) {
        ...
        for (; y < y2; y++) {
          ...
          if (sample_gdrw->sel_gdrw) {
            get_pixel(sample_gdrw->sel_gdrw, (x + sample_gdrw->seldeltax),
                      (y + sample_gdrw->seldeltay), &color[0]);
...
static void rnd_remap(gint32 lum, guchar *mapped_color) {
  t_samp_color_elem *col_ptr;
  gint rnd;
  gint ct;
  gint idx;
...
static void remap_pixel(guchar *pixel, const guchar *original, gint bpp2) {
  ...
  if (g_values.hold_inten) {
    ...
    if (mapped_lum == 0) {
      ...
      if (mr > 0.0) {
        red = orig_lum / (30 + (59 * mg / mr) + (11 * mb / mr));
        grn = mg * red / mr;
        blu = mb * red / mr;
      } else if (mg > 0.0) {
        grn = orig_lum / ((30 * mr / mg) + 59 + (11 * mb / mg));
        red = mr * grn / mg;
        blu = mb * grn / mg;
      } else {
        blu = orig_lum / ((30 * mr / mb) + (59 * mg / mb) + 11);
        grn = mg * blu / mb;
        red = mr * blu / mb;
      }
...
static void colorize_func(const guchar *src, guchar *dest, gint bpp,
                          gboolean has_alpha) {
  if (has_alpha) {
    bpp--;
    dest[bpp] = src[bpp];
...
static void colorize_drawable(GimpDrawable *drawable) {
  ...
  iter = gegl_buffer_iterator_new(src_buffer, GEGL_RECTANGLE(x, y, w, h), 0,
                                  format, GEGL_ACCESS_READ, GEGL_ABYSS_NONE, 2);

  gegl_buffer_iterator_add(iter, dest_buffer, GEGL_RECTANGLE(x, y, w, h), 0,
                           format, GEGL_ACCESS_WRITE, GEGL_ABYSS_NONE);
...
static int main_colorize(gint mc_flags) {
  t_GDRW sample_gdrw;
  ...
  g_tol_col_err =
      (((float)max * (g_values.tol_col_err * g_values.tol_col_err)) /
       (100.0 * 100.0));
  g_max_col_err = max;
  ...
  if ((mc_flags & MC_DST_REMAP) && (rc == 0)) {
    if (gimp_drawable_is_gray(g_values.dst) && (mc_flags & MC_DST_REMAP)) {
