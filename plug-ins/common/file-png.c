...
static GimpValueArray *png_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *png_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static gboolean save_image(GFile *file, GimpImage *image,
                           ...
                           GObject *config, gint *bits_per_sample,
                           GError **error);

static int respin_cmap(png_structp pp, png_infop info, guchar *remap,
                       GimpImage *image, GimpDrawable *drawable);

static gboolean save_dialog(GimpImage *image, GimpProcedure *procedure,
                            GObject *config, gboolean alpha);

static gboolean offsets_dialog(gint offset_x, gint offset_y);

static gboolean ia_has_transparent_pixels(GeglBuffer *buffer);

static gint find_unused_ia_color(GeglBuffer *buffer, gint *colors);

G_DEFINE_TYPE(Png, png, GIMP_TYPE_PLUG_IN)

GIMP_MAIN(PNG_TYPE)

static void png_class_init(PngClass *klass) {
...
static GimpProcedure *png_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, png_load, NULL, NULL);
    ...
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/png");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "png");
    gimp_file_procedure_set_magics(GIMP_FILE_PROCEDURE(procedure),
                                   "0,string,\211PNG\r\n\032\n");
  } else if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, png_save, NULL, NULL);
    ...
    GIMP_PROC_ARG_BOOLEAN(procedure, "interlaced", "_Interlacing (Adam7)",
                          "Use Adam7 interlacing?", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "compression", "Co_mpression level",
                      "Deflate Compression factor (0..9)", 0, 9, 9,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "bkgd", "Save _background color",
                          "Write bKGD chunk (PNG metadata)", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "gama", "Save _gamma",
                          "Write gAMA chunk (PNG metadata)", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "offs", "Save layer o_ffset",
                          "Write oFFs chunk (PNG metadata)", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "phys", "Save resol_ution",
                          "Write pHYs chunk (PNG metadata)", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "time", "Save creation _time",
                          "Write tIME chunk (PNG metadata)", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "save-transparent",
                          "Save color _values from transparent pixels",
                          "Preserve color of transparent pixels?", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_AUX_ARG_INT(procedure, "format", "_Pixel format",
                          "PNG export format", PNG_FORMAT_AUTO,
                          PNG_FORMAT_GRAYA16, PNG_FORMAT_AUTO,
                          G_PARAM_READWRITE);
...
static GimpValueArray *png_load(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  image = load_image(file, interactive, &resolution_loaded, &profile_loaded,
                     &error);
  ...
  metadata = gimp_image_metadata_load_prepare(image, "image/png", file, NULL);
  ...
  if (metadata) {
    ...
    gimp_image_metadata_load_finish(image, "image/png", metadata, flags);
...
static GimpValueArray *png_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "PNG",
   ...
   */
  if (!alpha)
    g_object_set(config, "bkgd", FALSE, NULL);
...
static void on_read_error(png_structp png_ptr, png_const_charp error_msg) {
  ...
  gegl_buffer_set(
      error_data->buffer,
      GEGL_RECTANGLE(0, error_data->begin, error_data->width, error_data->num),
      0, error_data->file_format, error_data->pixel, GEGL_AUTO_ROWSTRIDE);
...
static GimpColorProfile *load_color_profile(png_structp pp, png_infop info,
                                            gchar **profile_name) {
  ...
  if (png_get_iCCP(pp, info, &profname, &profcomp, &prof, &proflen)) {
    profile =
        gimp_color_profile_new_from_icc_profile((guint8 *)prof, proflen, NULL);
    if (profile && profname) {
      *profile_name = g_convert(profname, strlen(profname), "ISO-8859-1",
...
static GimpImage *load_image(GFile *file, gboolean interactive,
  ...
  if (!info) {
    g_set_error(error, 0, 0,
                _("Error while reading '%s'. Could not create PNG header info "
                  "structure."),
                gimp_file_get_utf8_name(file));
    return NULL;
  }
  ...
  default:
    g_set_error(error, 0, 0, _("Unknown color model in PNG file '%s'."),
                gimp_file_get_utf8_name(file));
    return NULL;
  }
  ...
  image =
      gimp_image_new_with_precision(width, height, image_type, image_precision);
  if (!image) {
    g_set_error(error, 0, 0, _("Could not create new image for '%s': %s"),
                gimp_file_get_utf8_name(file),
  ...
  if (profile) {
    ...
    if (profile_name) {
      ...
      parasite = gimp_parasite_new(
          "icc-profile-name", GIMP_PARASITE_PERSISTENT | GIMP_PARASITE_UNDOABLE,
          strlen(profile_name), profile_name);
  ...
  layer = gimp_layer_new(
      (GimpImage *)image, _("Background"), width, height, layer_type, 100,
  ...
  if (png_get_valid(pp, info, PNG_INFO_gAMA)) {
    ...
    parasite = gimp_parasite_new("gamma", GIMP_PARASITE_PERSISTENT,
                                 strlen(buf) + 1, buf);
    gimp_image_attach_parasite((GimpImage *)image, parasite);
    gimp_parasite_free(parasite);
  }
  ...
  if (png_get_valid(pp, info, PNG_INFO_oFFs)) {
    ...
    if (offset_x != 0 || offset_y != 0) {
      if (!interactive) {
        ...
        if (abs(offset_x) > width || abs(offset_y) > height) {
  ...
  if (png_get_valid(pp, info, PNG_INFO_pHYs)) {
    ...
    if (png_get_pHYs(pp, info, &xres, &yres, &unit_type) && xres > 0 &&
        yres > 0) {
      switch (unit_type) {
      ...
      case PNG_RESOLUTION_METER:
        gimp_image_set_resolution((GimpImage *)image, (gdouble)xres * 0.0254,
                                  (gdouble)yres * 0.0254);
  ...
  for (pass = 0; pass < num_passes; pass++) {
    ...
    for (begin = 0; begin < height; begin += tile_height) {
      ...
      if (pass != 0) /* to handle interlaced PiNGs */
        gegl_buffer_get(buffer, GEGL_RECTANGLE(0, begin, width, num), 1.0,
                        file_format, pixel, GEGL_AUTO_ROWSTRIDE,
                        GEGL_ABYSS_NONE);
      ...
      gegl_buffer_set(buffer, GEGL_RECTANGLE(0, begin, width, num), 0,
                      file_format, pixel, GEGL_AUTO_ROWSTRIDE);

      gimp_progress_update(((gdouble)pass + (gdouble)end / (gdouble)height) /
                           (gdouble)num_passes);
    }
  }
  ...
  if (png_get_text(pp, info, &text, &num_texts)) {
    ...
    for (i = 0; i < num_texts && !comment; i++, text++) {
      ...
      {
        comment =
            g_convert(text->text, -1, "UTF-8", "ISO-8859-1", NULL, NULL, NULL);
      } else if (g_utf8_validate(text->text, -1, NULL)) { /*  iTXt  */
...
static gboolean offsets_dialog(gint offset_x, gint offset_y) {
  ...
  dialog = gimp_dialog_new(_("Apply PNG Offset"), PLUG_IN_ROLE, NULL, 0,
  ...
  gimp_dialog_set_alternative_button_order(GTK_DIALOG(dialog), GTK_RESPONSE_YES,
                                           GTK_RESPONSE_NO, -1);
...
static gboolean save_image(GFile *file, GimpImage *image,
...
#if !defined(PNG_iCCP_SUPPORTED)
  g_object_set(config, "save-color-profile", FALSE, NULL);
#endif

  g_object_get(config, "interlaced", &save_interlaced, "bkgd", &save_bkgd,
               "gama", &save_gama, "offs", &save_offs, "phys", &save_phys,
               "time", &save_time, "save-comment", &save_comment,
               "gimp-comment", &comment, "save-transparent",
               &save_transp_pixels, "compression", &compression_level, "format",
               &export_format, "save-color-profile", &save_profile, NULL);
  ...
  if (save_profile) {
    ...
    if (profile || export_format == PNG_FORMAT_AUTO ||
  ...
  if (!info) {
    g_set_error(error, 0, 0,
                _("Error while exporting '%s'. Could not create PNG header "
                  "info structure."),
                gimp_file_get_utf8_name(file));
    return FALSE;
  }
  ...
  if (export_format == PNG_FORMAT_AUTO) {
    ...
    case GIMP_INDEXED_IMAGE:
      ...
      pngg.has_plte = TRUE;
      pngg.palette =
          (png_colorp)gimp_image_get_colormap(image, &pngg.num_palette);
  ...
  if (save_profile) {
    ...
    parasite = gimp_image_get_parasite(orig_image, "icc-profile-name");
    if (parasite)
      profile_name = g_convert(gimp_parasite_data(parasite),
                               gimp_parasite_data_size(parasite), "UTF-8",
                               "ISO-8859-1", NULL, NULL, NULL);

    png_set_iCCP(pp, info, profile_name ? profile_name : "ICC profile", 0,
                 icc_data, icc_length);
  ...
  /* Small texts are not worth compressing and will be even bigger if
     compressed. Empirical length limit of a text being worth compressing. */
#define COMPRESSION_WORTHY_LENGTH 200
  ...
  if (save_comment && comment && strlen(comment)) {
    ...
    text[0].text =
        g_convert(comment, -1, "ISO-8859-1", "UTF-8", NULL, &text_length, NULL);
...
#endif /* PNG_iTXt_SUPPORTED */
    {
       ...
       */
      text[0].text = g_convert_with_fallback(comment, -1, "ISO-8859-1", "UTF-8",
                                             "", NULL, &text_length, NULL);
#endif
...
#undef COMPRESSION_WORTHY_LENGTH
  ...
  for (pass = 0; pass < num_passes; pass++) {
    ...
    for (begin = 0, end = tile_height; begin < height;
      ...
      gegl_buffer_get(buffer, GEGL_RECTANGLE(0, begin, width, num), 1.0,
                      file_format, pixel, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
      ...
      if (png_get_valid(pp, info, PNG_INFO_tRNS)) {
        ...
        for (i = 0; i < num; ++i) {
          ...
          for (k = 0; k < width; ++k) {
            fixed[k] =
                (fixed[k * 2 + 1] > 127) ? inverse_remap[fixed[k * 2]] : 0;
          }
        }
      }
...
static gboolean ia_has_transparent_pixels(GeglBuffer *buffer) {
  ...
  iter = gegl_buffer_iterator_new(buffer, NULL, 0, format, GEGL_ACCESS_READ,
                                  GEGL_ABYSS_NONE, 1);
...
static gboolean save_dialog(GimpImage *image, GimpProcedure *procedure,
  ...
  dialog = gimp_save_procedure_dialog_new(GIMP_SAVE_PROCEDURE(procedure),
                                          GIMP_PROCEDURE_CONFIG(config), image);

  gimp_procedure_dialog_get_widget(GIMP_PROCEDURE_DIALOG(dialog), "compression",
                                   GIMP_TYPE_SCALE_ENTRY);

  store = gimp_int_store_new(
      _("Automatic"), PNG_FORMAT_AUTO, _("8 bpc RGB"), PNG_FORMAT_RGB8,
      _("8 bpc GRAY"), PNG_FORMAT_GRAY8, _("8 bpc RGBA"), PNG_FORMAT_RGBA8,
      _("8 bpc GRAYA"), PNG_FORMAT_GRAYA8, _("16 bpc RGB"), PNG_FORMAT_RGB16,
      _("16 bpc GRAY"), PNG_FORMAT_GRAY16, _("16 bpc RGBA"), PNG_FORMAT_RGBA16,
      _("16 bpc GRAYA"), PNG_FORMAT_GRAYA16, NULL);
  gimp_procedure_dialog_get_int_combo(GIMP_PROCEDURE_DIALOG(dialog), "format",
                                      GIMP_INT_STORE(store));
  g_object_unref(store);

  gtk_widget_set_sensitive(
      gimp_procedure_dialog_get_widget(GIMP_PROCEDURE_DIALOG(dialog),
                                       "save-transparent", G_TYPE_NONE),
      alpha);

  gimp_save_procedure_dialog_add_metadata(GIMP_SAVE_PROCEDURE_DIALOG(dialog),
                                          "bkgd");
  gimp_save_procedure_dialog_add_metadata(GIMP_SAVE_PROCEDURE_DIALOG(dialog),
                                          "gama");
  gimp_save_procedure_dialog_add_metadata(GIMP_SAVE_PROCEDURE_DIALOG(dialog),
  ...
  gimp_save_procedure_dialog_add_metadata(GIMP_SAVE_PROCEDURE_DIALOG(dialog),
                                          "time");
  gimp_procedure_dialog_fill(GIMP_PROCEDURE_DIALOG(dialog), "format",
                             "compression", "interlaced", "save-transparent",
                             NULL);
