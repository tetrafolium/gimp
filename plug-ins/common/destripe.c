...
#define DESTRIPE                                                               \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), DESTRIPE_TYPE, Destripe))
...
static void destripe(GimpDrawable *drawable, GimpPreview *preview);
static void destripe_preview(GimpDrawable *drawable, GimpPreview *preview);

static gboolean destripe_dialog(GimpDrawable *drawable);
static void destripe_scale_entry_update_int(GimpLabelSpin *entry, gint *value);
...
static void destripe_class_init(DestripeClass *klass) {
...
static void destripe_init(Destripe *destripe) {}
...
static GimpProcedure *destripe_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, destripe_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Marc Lehmann <pcg@goof.com>",
                                   "Marc Lehmann <pcg@goof.com>",
                                   PLUG_IN_VERSION);

    GIMP_PROC_ARG_INT(procedure, "avg-width", "Avg width",
                      "Averaging filter width", 2, MAX_AVG, 36,
                      G_PARAM_READWRITE);
  }
...
static void destripe(GimpDrawable *drawable, GimpPreview *preview) {
  ...
  for (ox = x1; ox < x2; ox += tile_width) {
    ...
    gegl_buffer_get(src_buffer, GEGL_RECTANGLE(ox, y1, cols, height), 1.0,
                    format, rows, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
  ...
  for (ox = x1; ox < x2; ox += tile_width) {
    ...
    gegl_buffer_set(dest_buffer, GEGL_RECTANGLE(ox, y1, cols, height), 0,
                    format, src_rows, GEGL_AUTO_ROWSTRIDE);
...
static gboolean destripe_dialog(GimpDrawable *drawable) {
  ...
  dialog = gimp_dialog_new(_("Destripe"), PLUG_IN_ROLE, NULL, 0,
  ...
  g_signal_connect_swapped(preview, "invalidated", G_CALLBACK(destripe_preview),
                           drawable);
