...
static GimpValueArray *heif_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                 GFile *file, const GimpValueArray *args,
                                 gpointer run_data);
static GimpValueArray *heif_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static GimpValueArray *heif_av1_save(GimpProcedure *procedure,
                                     ...
                                     gint n_drawables, GimpDrawable **drawables,
                                     GFile *file, const GimpValueArray *args,
                                     gpointer run_data);
#endif

static GimpImage *load_image(GFile *file, gboolean interactive,
                             GimpPDBStatusType *status, GError **error);
static gboolean save_image(GFile *file, GimpImage *image,
                           GimpDrawable *drawable, GObject *config,
                           GError **error,
...
static gboolean save_dialog(GimpProcedure *procedure, GObject *config,
                            GimpImage *image);
...
static void heif_class_init(HeifClass *klass) {
...
static void heif_init(Heif *heif) {}
...
static GimpProcedure *heif_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, heif_load, NULL, NULL);

    gimp_procedure_set_menu_label(procedure, N_("HEIF/HEIC"));

    gimp_procedure_set_documentation(procedure, _("Loads HEIF images"),
                                     _("Load image stored in HEIF format (High "
                                       "Efficiency Image File Format). Typical "
                                       "suffices for HEIF files are .heif, "
                                       ".heic."),
                                     name);
    gimp_procedure_set_attribution(procedure, "Dirk Farin <farin@struktur.de>",
                                   "Dirk Farin <farin@struktur.de>", "2018");
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, heif_save, NULL, NULL);

    gimp_procedure_set_image_types(procedure, "RGB*");

    gimp_procedure_set_menu_label(procedure, N_("HEIF/HEIC"));

    gimp_procedure_set_documentation(procedure, _("Exports HEIF images"),
    ...
    GIMP_PROC_ARG_INT(procedure, "quality", "Quality",
                      "Quality factor (0 = worst, 100 = best)", 0, 100, 50,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "lossless", "Lossless",
                          "Use lossless compression", FALSE, G_PARAM_READWRITE);
    ...
    GIMP_PROC_ARG_INT(procedure, "save-bit-depth", "Bit depth",
                      "Bit depth of exported image", 8, 12, 8,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "pixel-format", "Pixel format",
                      "Format of color sub-sampling",
                      HEIFPLUGIN_EXPORT_FORMAT_RGB,
                      HEIFPLUGIN_EXPORT_FORMAT_YUV420,
                      HEIFPLUGIN_EXPORT_FORMAT_YUV420, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "encoder-speed", "Encoder speed",
                      "Tradeoff between speed and compression",
                      HEIFPLUGIN_ENCODER_SPEED_SLOW,
                      HEIFPLUGIN_ENCODER_SPEED_FASTER,
                      HEIFPLUGIN_ENCODER_SPEED_BALANCED, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "save-exif", "Save Exif",
                          "Toggle saving Exif data", gimp_export_exif(),
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "save-xmp", "Save XMP",
                          "Toggle saving XMP data", gimp_export_xmp(),
                          G_PARAM_READWRITE);
  }
#if LIBHEIF_HAVE_VERSION(1, 8, 0)
    ...
    gimp_procedure_set_documentation(
        procedure, _("Loads AVIF images"),
        _("Load image stored in AV1 Image File Format (AVIF)"), name);
    gimp_procedure_set_attribution(
        procedure, "Daniel Novomesky <dnovomesky@gmail.com>",
        "Daniel Novomesky <dnovomesky@gmail.com>", "2020");

    gimp_file_procedure_set_handles_remote(GIMP_FILE_PROCEDURE(procedure),
                                           TRUE);
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/avif");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "avif");
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, heif_av1_save, NULL, NULL);
    ...
    gimp_procedure_set_documentation(
        procedure, _("Exports AVIF images"),
        _("Save image in AV1 Image File Format (AVIF)"), name);
    gimp_procedure_set_attribution(
        procedure, "Daniel Novomesky <dnovomesky@gmail.com>",
        "Daniel Novomesky <dnovomesky@gmail.com>", "2020");
...
static GimpValueArray *heif_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  if (status == GIMP_PDB_SUCCESS) {
    if (!save_image(file, image, drawables[0], G_OBJECT(config), &error,
                    heif_compression_HEVC, metadata)) {
...
static goffset get_file_size(GFile *file, GError **error) {
  GFileInfo *info;
  goffset size = 1;
...
#if LIBHEIF_HAVE_VERSION(1, 8, 0)
static void heifplugin_color_profile_set_tag(cmsHPROFILE profile,
                                             cmsTagSignature sig,
                                             const gchar *tag) {
  cmsMLU *mlu;
...
nclx_to_gimp_profile(const struct heif_color_profile_nclx *nclx) {
  ...
  cmsFloat64Number rec709_parameters[5] = {2.2, 1.0 / 1.099, 0.099 / 1.099,
                                           1.0 / 4.5, 0.081};
  ...
  case 22:
    ...
    break;
  default:
    g_warning("%s: Unsupported color_primaries value %d.", G_STRFUNC,
  ...
  if (profile) {
    ...
    heifplugin_color_profile_set_tag(profile, cmsSigProfileDescriptionTag,
                                     description);
    heifplugin_color_profile_set_tag(profile, cmsSigDeviceMfgDescTag, "GIMP");
...
GimpImage *load_image(GFile *file, gboolean interactive,
                      GimpPDBStatusType *status, GError **error) {
  GInputStream *input;
  goffset file_size;
  guchar *file_buffer;
  ...
  gimp_progress_init_printf(_("Opening '%s'"), g_file_get_parse_name(file));
  ...
  *status = GIMP_PDB_EXECUTION_ERROR;
  ...
  if (!g_input_stream_read_all(input, file_buffer, file_size, &bytes_read, NULL,
                               error) &&
      bytes_read == 0) {
    g_free(file_buffer);
  ...
  if (err.code) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Loading HEIF image failed: %s"), err.message);
  ...
  case heif_color_profile_type_prof:
     ...
     */
    {
      ...
      if (err.code)
        g_warning("%s: ICC profile loading failed and discarded.", G_STRFUNC);
      else
        profile = gimp_color_profile_new_from_icc_profile(
            (guint8 *)profile_data, profile_size, NULL);
  ...
  case heif_color_profile_type_nclx: {
    ...
    if (err.code) {
      g_warning("%s: NCLX profile loading failed and discarded.", G_STRFUNC);
    } else {
...
#endif
  default:
    g_warning("%s: unknown color profile type has been discarded.", G_STRFUNC);
    break;
  }
  ...
  layer = gimp_layer_new(image, _("image content"), width, height,
                         has_alpha ? GIMP_RGBA_IMAGE : GIMP_RGB_IMAGE, 100.0,
  ...
  format = babl_format_with_space(encoding, gegl_buffer_get_format(buffer));

  if (bit_depth == 8) {
    gegl_buffer_set(buffer, GEGL_RECTANGLE(0, 0, width, height), 0, format,
                    data, stride);
  } else /* high bit depth */
  {
    uint16_t *data16;
    ...
    case 10:
      for (y = 0; y < height; y++) {
        ...
        for (x = 0; x < rowentries; x++) {
          tmp_pixelval =
              (int)(((float)(0x03ff & (*src16)) / 1023.0f) * 65535.0f + 0.5f);
    ...
    case 12:
      for (y = 0; y < height; y++) {
        ...
        for (x = 0; x < rowentries; x++) {
          tmp_pixelval =
              (int)(((float)(0x0fff & (*src16)) / 4095.0f) * 65535.0f + 0.5f);
          *dest16 = CLAMP(tmp_pixelval, 0, 65535);
          dest16++;
          src16++;
        }
...

  {
    ...
    if (exif_data || xmp_data) {
      ...
      if (exif_data) {
        ...
        {
          if (tiffheader[0] == tiffHeaderBE[0] &&
              tiffheader[1] == tiffHeaderBE[1] &&
              tiffheader[2] == tiffHeaderBE[2] &&
              tiffheader[3] == tiffHeaderBE[3]) {
            break;
          }
          if (tiffheader[0] == tiffHeaderLE[0] &&
              tiffheader[1] == tiffHeaderLE[1] &&
              tiffheader[2] == tiffHeaderLE[2] &&
              tiffheader[3] == tiffHeaderLE[3]) {
            break;
          }
          new_exif_size--;
          tiffheader++;
        ...
        {
          if (!gexiv2_metadata_open_buf(exif_metadata, tiffheader,
                                        new_exif_size, error)) {
      ...
      if (xmp_data) {
        if (!gimp_metadata_set_from_xmp(metadata, xmp_data, xmp_data_size,
                                        error)) {
      ...
      gimp_image_metadata_load_finish(image, "image/heif", metadata, flags);
    }
  }
...
static void heifplugin_image_metadata_copy_tag(GExiv2Metadata *src,
                                               GExiv2Metadata *dest,
                                               const gchar *tag) {
...
static struct heif_error write_callback(struct heif_context *ctx,
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  g_object_get(config, "lossless", &lossless, "quality", &quality,
#if LIBHEIF_HAVE_VERSION(1, 10, 0)
               "pixel-format", &pixel_format,
#endif
#if LIBHEIF_HAVE_VERSION(1, 8, 0)
               "save-bit-depth", &save_bit_depth, "encoder-speed",
               &encoder_speed,
#endif
               ...
               "save-exif", &save_exif,
#endif
               "save-xmp", &save_xmp, NULL);

  gimp_progress_init_printf(_("Exporting '%s'"), g_file_get_parse_name(file));
...
#if (G_BYTE_ORDER == G_LITTLE_ENDIAN)
    err = heif_image_create(width, height, heif_colorspace_RGB,
                            has_alpha ? heif_chroma_interleaved_RRGGBBAA_LE
                                      : heif_chroma_interleaved_RRGGBB_LE,
                            &h_image);
#else
    err = heif_image_create(width, height, heif_colorspace_RGB,
                            has_alpha ? heif_chroma_interleaved_RRGGBBAA_BE
  ...
  default:
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                "Unsupported bit depth: %d", save_bit_depth);
  ...
  if (err.code != 0) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Encoding HEIF image failed: %s"), err.message);
    heif_context_free(context);
    return FALSE;
  }
  ...
  if (save_profile) {
    ...
    if (!profile) {
      ...
      if (gimp_color_profile_is_linear(profile)) {
        if (gimp_image_get_precision(image) != GIMP_PRECISION_U8_LINEAR) {
          ...
          saved_profile =
              gimp_color_profile_new_srgb_trc_from_color_profile(profile);
    ...
    space = gimp_color_profile_get_space(
        profile, GIMP_COLOR_RENDERING_INTENT_RELATIVE_COLORIMETRIC, error);
    if (error && *error) {
      /* Don't make this a hard failure yet output the error. */
      g_printerr("%s: error getting the profile space: %s", G_STRFUNC,
                 (*error)->message);
      g_clear_error(error);
    }
  ...
  if (save_bit_depth > 8) {
    ...
    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, 0, width, height), 1.0, format,
                    data16, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);

    g_object_unref(buffer);

    heif_image_add_plane(h_image, heif_channel_interleaved, width, height,
                         save_bit_depth);
  ...
  {
#if LIBHEIF_HAVE_VERSION(1, 8, 0)
    heif_image_add_plane(h_image, heif_channel_interleaved, width, height, 8);
#else
    /* old style settings */
    heif_image_add_plane(h_image, heif_channel_interleaved, width, height,
                         has_alpha ? 32 : 24);
  ...
  /*  encode to HEIF file  */
  err = heif_context_get_encoder_for_format(context, compression, &encoder);
  ...
  if (err.code != 0) {
    g_printerr("Failed to set chroma %s for %s encoder: %s", parameter_value,
  ...
  if (compression == heif_compression_HEVC) {
    ...
    if (err.code != 0) {
      g_printerr("Failed to set preset %s for %s encoder: %s", parameter_value,
                 encoder_name, err.message);
    }
    ...
    err = heif_encoder_set_parameter_integer(encoder, "threads",
                                             parameter_number);
    if (err.code != 0) {
      g_printerr("Failed to set threads=%d for %s encoder: %s",
                 parameter_number, encoder_name, err.message);
    }

    if (g_ascii_strncasecmp(encoder_name, "AOM", 3) ==
        0) /* AOMedia AV1 encoder */
    {
      switch (encoder_speed) {
      case HEIFPLUGIN_ENCODER_SPEED_SLOW:
        parameter_number = 1;
      ...
      case HEIFPLUGIN_ENCODER_SPEED_FASTER:
        ...
        if (err.code != 0) {
          g_printerr("Failed to set realtime=1 for %s encoder: %s",
                     encoder_name, err.message);
        }
#endif
        break;
      ...
      err = heif_encoder_set_parameter_integer(encoder, "speed",
                                               parameter_number);
      if (err.code != 0) {
        g_printerr("Failed to set speed=%d for %s encoder: %s",
                   parameter_number, encoder_name, err.message);
      }

    } else if (g_ascii_strncasecmp(encoder_name, "Rav1e", 5) ==
               0) /* Rav1e encoder */
    {
      switch (encoder_speed) {
      case HEIFPLUGIN_ENCODER_SPEED_SLOW:
      ...
      g_printerr("Parameters not set, unsupported AV1 encoder: %s",
                 encoder_name);
    }
  }
#endif

  err = heif_context_encode_image(context, h_image, encoder, NULL, &handle);
  if (err.code != 0) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Encoding HEIF image failed: %s"), err.message);
  ...
  if (save_exif && metadata) {
    if (gexiv2_metadata_get_supports_exif(GEXIV2_METADATA(metadata)) &&
      ...
      gchar **exif_data =
          gexiv2_metadata_get_exif_tags(GEXIV2_METADATA(metadata));
      guint i;
      ...
      for (i = 0; exif_data[i] != NULL; i++) {
        if (!gexiv2_metadata_has_tag(new_gexiv2metadata, exif_data[i]) &&
          ...
          heifplugin_image_metadata_copy_tag(GEXIV2_METADATA(metadata),
                                             new_gexiv2metadata, exif_data[i]);
        }
      }

      g_strfreev(exif_data);

      raw_exif_data = gexiv2_metadata_get_exif_data(
          new_gexiv2metadata, GEXIV2_BYTE_ORDER_LITTLE, error);
      if (raw_exif_data) {
        gsize exif_size = 0;
        gconstpointer exif_buffer = g_bytes_get_data(raw_exif_data, &exif_size);

        if (exif_size >= 4) {
          err = heif_context_add_exif_metadata(context, handle, exif_buffer,
                                               exif_size);
          if (err.code != 0) {
        ...
        if (error && *error) {
          g_printerr("%s: error preparing EXIF metadata: %s", G_STRFUNC,
                     (*error)->message);
          g_clear_error(error);
        }
      }
  ...
  if (save_xmp && metadata) {
    if (gexiv2_metadata_get_supports_xmp(GEXIV2_METADATA(metadata)) &&
      ...
      timestamp_usec =
          ((gint64)timer_usec.tv_sec) * 1000000ll + (gint64)timer_usec.tv_usec;
      ...
      gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata),
                                     "Xmp.GIMP.TimeStamp", ts);

      gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata),
                                     "Xmp.xmp.CreatorTool", "GIMP");

      gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata),
                                     "Xmp.GIMP.Version", GIMP_VERSION);

      gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata), "Xmp.GIMP.API",
                                     GIMP_API_VERSION);
      gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata),
                                     "Xmp.GIMP.Platform",
      ...
      for (i = 0; xmp_data[i] != NULL; i++) {
        if (!gexiv2_metadata_has_tag(new_g2metadata, xmp_data[i]) &&
          ...
          heifplugin_image_metadata_copy_tag(GEXIV2_METADATA(metadata),
                                             new_g2metadata, xmp_data[i]);
        }
      }

      g_strfreev(xmp_data);

      xmp_packet = gexiv2_metadata_generate_xmp_packet(
          new_g2metadata,
          GEXIV2_USE_COMPACT_FORMAT | GEXIV2_OMIT_ALL_FORMATTING, 0);
      if (xmp_packet) {
        int xmp_size = strlen(xmp_packet);
        if (xmp_size > 0) {
          heif_context_add_XMP_metadata(context, handle, xmp_packet, xmp_size);
        }
        g_free(xmp_packet);
      }
  ...
  if (err.code != 0) {
    ...
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Writing HEIF image failed: %s"), err.message);
...
static gboolean load_thumbnails(struct heif_context *heif, HeifImage *images) {
  guint32 *IDs;
  gint n_images;
  gint i;
  ...
  for (i = 0; i < n_images; i++) {
    ...
    if (heif_image_handle_is_primary_image(handle)) {
      g_snprintf(images[i].caption, sizeof(images[i].caption), "%dx%d (%s)",
                 width, height, _("primary"));
    } else {
      g_snprintf(images[i].caption, sizeof(images[i].caption), "%dx%d", width,
                 height);
    }
    ...
    err =
        heif_decode_image(thumbnail_handle, &thumbnail_img, heif_colorspace_RGB,
    ...
    if (thumbnail_width > MAX_THUMBNAIL_SIZE ||
      ...
      err = heif_image_scale_image(thumbnail_img, &scaled_img, new_width,
                                   new_height, NULL);
      if (err.code) {
        gimp_message(err.message);
        continue;
...
static void load_dialog_item_activated(GtkIconView *icon_view,
                                       GtkTreePath *path, GtkDialog *dialog) {
...
static gboolean load_dialog(struct heif_context *heif,
  ...
  dialog = gimp_dialog_new(_("Load HEIF Image"), PLUG_IN_BINARY, NULL, 0,
  ...
  for (i = 0; i < n_images; i++) {
    ...
    pixbuf = gdk_pixbuf_new_from_data(
        data, GDK_COLORSPACE_RGB, FALSE, 8, heif_images[i].width,
        heif_images[i].height, stride, NULL, NULL);
  ...
  gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(scrolled_window),
                                      GTK_SHADOW_IN);
  gtk_widget_set_size_request(scrolled_window, 2 * MAX_THUMBNAIL_SIZE,
  ...
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(icon_view), renderer, FALSE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(icon_view), renderer, "text",
                                 0, NULL);
  g_object_set(renderer, "alignment", PANGO_ALIGN_CENTER, "wrap-mode",
               PANGO_WRAP_WORD_CHAR, "xalign", 0.5, "yalign", 0.0, NULL);

  g_signal_connect(icon_view, "item-activated",
                   G_CALLBACK(load_dialog_item_activated), dialog);
...
gboolean save_dialog(GimpProcedure *procedure, GObject *config,
                     GimpImage *image) {
  GtkWidget *dialog;
  GtkWidget *main_vbox;
  GtkWidget *grid;
  ...
  dialog = gimp_procedure_dialog_new(
      procedure, GIMP_PROCEDURE_CONFIG(config),
      g_strcmp0(gimp_procedure_get_name(procedure), SAVE_PROC_AV1) == 0
          ? _("Export Image as AVIF")
  ...
  button = gimp_prop_check_button_new(config, "lossless", _("_Lossless"));
  ...
  g_object_bind_property(config, "lossless", grid, "sensitive",
                         G_BINDING_SYNC_CREATE | G_BINDING_INVERT_BOOLEAN);

  scale = gimp_prop_scale_entry_new(config, "quality", NULL, 1.0, FALSE, 0, 0);
  gtk_widget_hide(gimp_labeled_get_label(GIMP_LABELED(scale)));
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Quality"), 0.0, 0.5, scale,
                           2);

#if LIBHEIF_HAVE_VERSION(1, 10, 0)
  store =
      gimp_int_store_new(_("RGB"), HEIFPLUGIN_EXPORT_FORMAT_RGB, _("YUV444"),
                         HEIFPLUGIN_EXPORT_FORMAT_YUV444, _("YUV420"),
                         HEIFPLUGIN_EXPORT_FORMAT_YUV420, NULL);

  combo = gimp_prop_int_combo_box_new(config, "pixel-format",
                                      GIMP_INT_STORE(store));
  g_object_unref(store);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Pixel format:"), 0.0, 0.5,
                           combo, 2);
#endif

#if LIBHEIF_HAVE_VERSION(1, 8, 0)
  g_object_get(config, "save-bit-depth", &save_bit_depth, NULL);
  ...
  case GIMP_PRECISION_U8_PERCEPTUAL:
    ...
    if (save_bit_depth > 8) {
      save_bit_depth = 8;
      g_object_set(config, "save-bit-depth", save_bit_depth, NULL);
    }
    break;
  default:
    /* high bit depth */
  ...
  store = gimp_int_store_new(_("8 bit/channel"), 8, _("10 bit/channel"), 10,
                             _("12 bit/channel"), 12, NULL);

  combo = gimp_prop_int_combo_box_new(config, "save-bit-depth",
                                      GIMP_INT_STORE(store));
  g_object_unref(store);
  gimp_grid_attach_aligned(GTK_GRID(grid2), 0, 1, _("Bit depth:"), 0.0, 0.5,
                           combo, 2);

  store = gimp_int_store_new(_("Slow"), HEIFPLUGIN_ENCODER_SPEED_SLOW,
                             _("Balanced"), HEIFPLUGIN_ENCODER_SPEED_BALANCED,
                             _("Fast"), HEIFPLUGIN_ENCODER_SPEED_FASTER, NULL);

  combo = gimp_prop_int_combo_box_new(config, "encoder-speed",
                                      GIMP_INT_STORE(store));
  g_object_unref(store);
  gimp_grid_attach_aligned(GTK_GRID(grid2), 0, 2, _("Speed:"), 0.0, 0.5, combo,
                           2);
#endif
...
#if GEXIV2_CHECK_VERSION(0, 12, 2)
  button =
      gimp_prop_check_button_new(config, "save-exif", _("_Save Exif data"));
  gtk_box_pack_start(GTK_BOX(main_vbox), button, FALSE, FALSE, 0);
#endif

  /* XMP metadata */
  button = gimp_prop_check_button_new(config, "save-xmp", _("Save _XMP data"));
  gtk_box_pack_start(GTK_BOX(main_vbox), button, FALSE, FALSE, 0);
