...
static GimpValueArray *ps_load(GimpProcedure *procedure, GimpRunMode run_mode,
                               GFile *file, const GimpValueArray *args,
                               gpointer run_data);
static GimpValueArray *ps_load_thumb(GimpProcedure *procedure, GFile *file,
                                     gint size, const GimpValueArray *args,
                                     gpointer run_data);
static GimpValueArray *ps_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void ps_set_save_size(PSSaveVals *vals, GimpImage *image);

static gboolean save_ps_header(GOutputStream *output, GFile *file,
                               GError **error);
static gboolean save_ps_setup(GOutputStream *output, GimpDrawable *drawable,
                              gint width, gint height, gint bpp,
                              GError **error);
static gboolean save_ps_trailer(GOutputStream *output, GError **error);

static gboolean save_ps_preview(GOutputStream *output, GimpDrawable *drawable,
                                GError **error);

static gboolean save_gray(GOutputStream *output, GimpImage *image,
                          GimpDrawable *drawable, GError **error);
static gboolean save_bw(GOutputStream *output, GimpImage *image,
                        GimpDrawable *drawable, GError **error);
static gboolean save_index(GOutputStream *output, GimpImage *image,
                           GimpDrawable *drawable, GError **error);
static gboolean save_rgb(GOutputStream *output, GimpImage *image,
                         GimpDrawable *drawable, GError **error);

static gboolean print(GOutputStream *output, GError **error,
                      const gchar *format, ...) G_GNUC_PRINTF(3, 4);

static GimpImage *create_new_image(GFile *file, guint pagenum, guint width,
                                   guint height, GimpImageBaseType type,
                                   GimpLayer **layer);

static void check_load_vals(void);
static void check_save_vals(void);

static gint page_in_list(gchar *list, guint pagenum);

static gint get_bbox(GFile *file, gint *x0, gint *y0, gint *x1, gint *y1);

static FILE *ps_open(GFile *file, const PSLoadVals *loadopt, gint *llx,
                     gint *lly, gint *urx, gint *ury, gboolean *is_epsf);

static void ps_close(FILE *ifp);

static gboolean skip_ps(FILE *ifp);

static GimpImage *load_ps(GFile *file, guint pagenum, FILE *ifp, gint llx,
                          gint lly, gint urx, gint ury);

static void dither_grey(const guchar *grey, guchar *bw, gint npix,
                        gint linecount);
...
static void load_pages_entry_callback(GtkWidget *widget, gpointer data);
...
static void save_unit_toggle_update(GtkWidget *widget, gpointer data);
...
static void ps_class_init(PostScriptClass *klass) {
...
static void ps_init(PostScript *ps) {}
...
static GimpProcedure *ps_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PS_PROC) || !strcmp(name, LOAD_EPS_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, ps_load, NULL, NULL);

    if (!strcmp(name, LOAD_PS_PROC)) {
      gimp_procedure_set_menu_label(procedure, N_("PostScript document"));

      gimp_procedure_set_documentation(procedure, "Load PostScript documents",
                                       "Load PostScript documents", name);

      gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                         "application/postscript");
      gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "ps");
      ...
      gimp_procedure_set_documentation(
          procedure, "load Encapsulated PostScript images",
          "load Encapsulated PostScript images", name);

      gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                         "image/x-eps");
      gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "eps");
      gimp_file_procedure_set_magics(GIMP_FILE_PROCEDURE(procedure),
                                     "0,string,%!,0,long,0xc5d0d3c6");
    }

    gimp_procedure_set_attribution(
        procedure, "Peter Kirchgessner <peter@kirchgessner.net>",
        "Peter Kirchgessner", dversion);

    gimp_load_procedure_set_thumbnail_loader(GIMP_LOAD_PROCEDURE(procedure),
                                             LOAD_PS_THUMB_PROC);

    GIMP_PROC_ARG_INT(procedure, "resolution", "Resolution",
                      "Resolution to interpret image (dpi)", MIN_RESOLUTION,
                      MAX_RESOLUTION, 100, GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "width", "Width", "Desired width", 1,
                      GIMP_MAX_IMAGE_SIZE, 826, GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "height", "Height", "Desired height", 1,
                      GIMP_MAX_IMAGE_SIZE, 1170, GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "check-bbox", "Check bbox",
                          "FALSE: Use width/height, TRUE: Use BoundingBox",
                          TRUE, GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "pages", "Pages",
                         "Pages to load (e.g.: 1,3,5-7)", "1",
                         GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "coloring", "Coloring",
                      "4: b/w, 5: grey, 6: color image, 7: automatic", 4, 7, 6,
                      GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "text-alpha-bits", "Text alpha bits",
                      "1, 2 or 4", 1, 4, 1, GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "graphoc-alpha-bits", "Graphic alpha bits",
                      "1, 2 or 4", 1, 4, 1, GIMP_PARAM_READWRITE);
  } else if (!strcmp(name, LOAD_PS_THUMB_PROC)) {
    procedure = gimp_thumbnail_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, ps_load_thumb, NULL, NULL);
    ...
    gimp_procedure_set_attribution(
        procedure, "Peter Kirchgessner <peter@kirchgessner.net>",
        "Peter Kirchgessner", dversion);
  } else if (!strcmp(name, SAVE_PS_PROC) || !strcmp(name, SAVE_EPS_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, ps_save, NULL, NULL);
    ...
    GIMP_PROC_ARG_DOUBLE(procedure, "width", "Width",
                         "Width of the image in PostScript file "
                         "(0: use input image size)",
                         0, GIMP_MAX_IMAGE_SIZE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "height", "Height",
                         "Height of the image in PostScript file "
                         "(0: use input image size)",
                         0, GIMP_MAX_IMAGE_SIZE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "x-offset", "X offset",
                         "X-offset to image from lower left corner",
                         -GIMP_MAX_IMAGE_SIZE, GIMP_MAX_IMAGE_SIZE, 0,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "y-offset", "Y offset",
                         "Y-offset to image from lower left corner",
                         -GIMP_MAX_IMAGE_SIZE, GIMP_MAX_IMAGE_SIZE, 0,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "unit", "Unit",
                      "Unit for width/height/offset. "
                      "0: inches, 1: millimeters",
                      0, 1, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "keep-ratio", "Keep ratio",
                          "FALSE: use width/height, TRUE: keep aspect ratio",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "rotation", "Rotation", "0, 90, 180, 270", 0,
                      270, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "eps-flag", "EPG flag",
                          "FALSE: PostScript, TRUE: Encapsulated PostScript",
                          FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "preview", "Preview",
                      "0: no preview, >0: max. size of preview", 0,
                      GIMP_MAX_IMAGE_SIZE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "level", "Level",
                      "1: PostScript Level 1, 2: PostScript Level 2", 1, 2, 2,
                      G_PARAM_READWRITE);
  }
...
static GimpValueArray *ps_load(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_NONINTERACTIVE:
    ...
    if (GIMP_VALUES_GET_STRING(args, 4))
      g_strlcpy(plvals.pages, GIMP_VALUES_GET_STRING(args, 4),
                sizeof(plvals.pages));
    else
      plvals.pages[0] = '\0';
...
static GimpValueArray *ps_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, psvals.eps ? "EPS" : "PostScript",
  ...
  if (status == GIMP_PDB_SUCCESS) {
    ...
    if (save_image(file, image, drawables[0], &error)) {
      gimp_set_data(gimp_procedure_get_name(procedure), &psvals,
                    sizeof(PSSaveVals));
    } else {
...
static void compress_packbits(int nin, unsigned char *src, int *nout,
                              unsigned char *dst);
...
static void ascii85_init(void) {
  ascii85_len = 0;
  ascii85_linewidth = 0;
}

static gboolean ascii85_flush(GOutputStream *output, GError **error) {
  gchar c[5];
  gint i;
  ...
  if (string->len > 0 &&
      !g_output_stream_write_all(output, string->str, string->len, NULL, NULL,
                                 error)) {
...
static inline gboolean ascii85_out(GOutputStream *output, guchar byte,
                                   GError **error) {
  if (ascii85_len == 4)
...
static gboolean ascii85_nout(GOutputStream *output, gint n, guchar *uptr,
                             GError **error) {
  while (n-- > 0) {
...
static gboolean ascii85_done(GOutputStream *output, GError **error) {
  if (ascii85_len) {
  ...
  for (;;) {
    ...
    if ((nin > 1) && (c == src[1])) {
      ...
      while ((nin > 0) && (c == *src)) {
        ...
        nin--;
        if (nrepeat == 127)
          break; /* Maximum repeat */
      }

      /* Add two-byte repeat to last literal run ? */
      if ((nrepeat == 1) && (last_literal != NULL) &&
          (((*last_literal) + 1) + 2 <= 128)) {
        *last_literal += 2;
        *(dst++) = c;
        *(dst++) = c;
        continue;
...
static gboolean ps_begin_data(GOutputStream *output, GError **error) {
...
static gboolean ps_end_data(GOutputStream *output, GError **error) {
  goffset end_data;
  gchar s[64];
  ...
  if ((ps_data_pos.begin_data > 0) && (ps_data_pos.eol > 0)) {
    ...
    if (end_data > 0) {
      g_snprintf(s, sizeof(s), "%" G_GOFFSET_FORMAT " ASCII Bytes",
                 end_data - ps_data_pos.begin_data);

      if (!g_seekable_seek(G_SEEKABLE(output), ps_data_pos.eol - strlen(s),
      ...
      if (!g_seekable_seek(G_SEEKABLE(output), end_data, G_SEEK_SET, NULL,
                           error))
        return FALSE;
    }
  }
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  for (page_count = 1; page_count <= max_pagenum; page_count++) {
    if (page_in_list(plvals.pages, page_count)) {
      ...
      gimp_image_set_resolution(image, (gdouble)plvals.resolution,
...
static void ps_set_save_size(PSSaveVals *vals, GimpImage *image) {
  ...
  if (factor == 0.0254 || factor == 0.254 || factor == 2.54 || factor == 25.4) {
    vals->unit_mm = TRUE;
  }
  ...
  while (*c1) /* Remove all whitespace and break on unsupported characters */
  {
    if ((*c1 >= '0') && (*c1 <= '9')) {
      *(c0++) = *c1;
               ...
               (*c1 == ',')) { /* Try to remove double occurrences of these
                                  characters */
      if (c0 == tmplist) {
        *(c0++) = *c1;
      } else {
...
static FILE *ps_open(GFile *file, const PSLoadVals *loadopt, gint *llx,
  ...
  *is_epsf = FALSE;
  ...
  if ((loadopt->pnm_type != 4) && (loadopt->textalpha != 1))
    g_ptr_array_add(cmdA,
                    g_strdup_printf("-dTextAlphaBits=%d", loadopt->textalpha));
...
static gint read_pnmraw_type(FILE *ifp, gint *width, gint *height,
                             gint *maxval) {
  int frst, scnd, thrd;
  gint pnmtype;
  gchar line[1024];
...
static GimpImage *create_new_image(GFile *file, guint pagenum, guint width,
  ...
  *layer = gimp_layer_new(image, tmp, width, height, gdtype, 100,
...
static GimpImage *load_ps(GFile *file, guint pagenum, FILE *ifp, gint llx,
  ...
  if ((width == urx + 1) &&
      (height == ury + 1)) /* gs respected BoundingBox ? */
  {
    skip_left = llx;
    skip_bottom = lly;
  ...
  image = create_new_image(file, pagenum, image_width, image_height, imagetype,
                           &layer);
  ...
  if (pnmtype == 4) /* Read bitimage ? Must be mapped to indexed */
  {
    ...
    for (i = 0; i < height; i++) {
      ...
      if ((scan_lines == tile_height) || ((i + 1) == image_height)) {
        gegl_buffer_set(
            buffer,
            GEGL_RECTANGLE(0, i - scan_lines + 1, image_width, scan_lines), 0,
...
static gboolean save_ps_header(GOutputStream *output, GFile *file,
  ...
  if (!print(output, error, "%%!PS-Adobe-3.0%s\n",
             psvals.eps ? " EPSF-3.0" : ""))
    goto fail;
...
static gboolean save_ps_setup(GOutputStream *output, GimpDrawable *drawable,
  ...
  i_urx = (gint)urx;
  i_ury = (gint)ury;
  if (urx != (gdouble)i_urx)
    i_urx++; /* Check for non-integer value */
  if (ury != (gdouble)i_ury)
    i_ury++;
  ...
  if (!print(output, error, "%s %s scale\n",
  ...
  if (psvals.level <= 1) {
    if (!print(output, error, "%% Variable to keep one line of raster data\n"))
      return FALSE;

    if (bpp == 1) {
      if (!print(output, error, "/scanline %d string def\n", (width + 7) / 8))
        return FALSE;
    } else {
      if (!print(output, error, "/scanline %d %d mul string def\n", width,
                 bpp / 8))
        return FALSE;
    }
  }
  ...
  if (psvals.height < 0.0) {
    height = -height;
    ytrans = -height;
  }

  if (!print(output, error, "[ %d 0 0 %d %d %d ]\n", width, height, xtrans,
             ytrans))
    return FALSE;
...
static void dither_grey(const guchar *grey, guchar *bw, gint npix,
  ...
  for (x = 0, fse = fs_error + 1; x < npix; x++, fse++) {
    greyval = limit[*(greyptr++) + fse_inline + 512]; /* 0 <= greyval <= 255 */
...
static gboolean save_ps_preview(GOutputStream *output, GimpDrawable *drawable,
  ...
  if ((width > psvals.preview_size) || (height > psvals.preview_size)) {
  ...
  if (!print(output, error, "%%%%BeginPreview: %d %d 1 %d\n", width, height,
  ...
  for (y = 0; y < height; y++) {
    ...
    gegl_buffer_get(buffer,
                    GEGL_RECTANGLE(0, src_y, gegl_buffer_get_width(buffer), 1),
                    1.0, format, src_row, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);

    greyptr = grey;
    if (bpp == 3) /* RGB-image */
    {
      for (x = 0; x < width; x++) { /* Convert to grey */
    ...
    } else /* Grey image */
    {
      for (x = 0; x < width; x++)
        *(greyptr++) =
            *(src_row + ((x * gegl_buffer_get_width(buffer)) / width));
    }
...
static gboolean save_gray(GOutputStream *output, GimpImage *image,
  ...
  if (!level2) {
    if (!print(output, error, "{ currentfile scanline readhexstring pop }\n"))
      goto fail;
  } else {
    if (!print(output, error,
...
#define GET_GRAY_TILE(begin)                                                   \
  {                                                                            \
    ...
    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, i, width, scan_lines), 1.0,      \
                    format, begin, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);      \
    src = begin;                                                               \
  }
  ...
  for (i = 0; i < height; i++) {
    ...
    if (!level2) {
      ...
      while (j > 0) {
        ...
        if (!g_output_stream_write_all(output, dst, nwrite, NULL, NULL, error))
          goto fail;
...
#define GET_INDEX_TILE(begin)                                                  \
  ...
  for (i = 0; i < height; i++) {
    ...
    if (!level2) {
      for (j = 0; j < width; j++) {
        if (!g_output_stream_write_all(output, coltab + (*(src++)) * 6, 6, NULL,
                                       NULL, error))
          goto fail;
...
#define GET_RGB_TILE(begin)                                                    \
  ...
  for (i = 0; i < height; i++) {
    ...
    if (!level2) {
      for (j = 0; j < width; j++) {
        if (!print(output, error, "%c", hex[(*src) >> 4])) /* Red */
          goto fail;

        if (!print(output, error, "%c", hex[(*(src++)) & 0x0f]))
          goto fail;

        if (!print(output, error, "%c", hex[(*src) >> 4])) /* Green */
          goto fail;

        if (!print(output, error, "%c", hex[(*(src++)) & 0x0f]))
          goto fail;

        if (!print(output, error, "%c", hex[(*src) >> 4])) /* Blue */
          goto fail;
...
static gboolean print(GOutputStream *output, GError **error,
  ...
  success = g_output_stream_vprintf(output, NULL, NULL, error, format, args);
  va_end(args);
...
static gboolean load_dialog(GFile *file) {
  ...
  dialog = gimp_dialog_new(_("Import from PostScript"), PLUG_IN_ROLE, NULL, 0,
  ...
  adj = gtk_adjustment_new(plvals.resolution, MIN_RESOLUTION, MAX_RESOLUTION, 1,
                           10, 0);
  spinbutton = gimp_spin_button_new(adj, 1.0, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Resolution:"), 0.0, 0.5,
                           spinbutton, 1);

  g_signal_connect(adj, "value-changed", G_CALLBACK(resolution_change_callback),
                   &plvals.resolution);
  g_signal_connect(adj, "value-changed", G_CALLBACK(gimp_int_adjustment_update),
                   &plvals.resolution);

  adj = gtk_adjustment_new(plvals.width, 1, GIMP_MAX_IMAGE_SIZE, 1, 10, 0);
  ps_width_spinbutton = gimp_spin_button_new(adj, 1.0, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Width:"), 0.0, 0.5,
                           ps_width_spinbutton, 1);

  g_signal_connect(adj, "value-changed", G_CALLBACK(gimp_int_adjustment_update),
                   &plvals.width);

  adj = gtk_adjustment_new(plvals.height, 1, GIMP_MAX_IMAGE_SIZE, 1, 10, 0);
  ps_height_spinbutton = gimp_spin_button_new(adj, 1.0, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("_Height:"), 0.0, 0.5,
  ...
  if (page_count == 0) {
    ...
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("Pages:"), 0.0, 0.5, entry,
                             1);

    g_signal_connect(entry, "changed", G_CALLBACK(load_pages_entry_callback),
                     NULL);
    ...
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 4, _("Open as"), 0.0, 0.5,
                             target, 1);
  }
  ...
  frame = gimp_int_radio_group_new(TRUE, _("Coloring"),
                                   ...
                                   _("B/W"), 4, NULL, _("Gray"), 5, NULL,
                                   _("Color"), 6, NULL, _("Automatic"), 7, NULL,
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Graphic antialiasing"), G_CALLBACK(gimp_radio_button_update),
      &plvals.graphicsalpha, NULL, plvals.graphicsalpha,

      C_("antialiasing", "None"), 1, NULL, _("Weak"), 2, NULL, _("Strong"), 4,
      NULL,
  ...
  if (selector) {
    ...
    if (strlen(range) < 1) {
      ...
      range =
          gimp_page_selector_get_selected_range(GIMP_PAGE_SELECTOR(selector));
    }
...
static gboolean save_dialog(void) {
  ...
  dialog =
      gimp_export_dialog_new(_("PostScript"), PLUG_IN_BINARY, SAVE_PS_PROC);

  /* Main hbox */
  hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 12);
  gtk_container_set_border_width(GTK_CONTAINER(hbox), 12);
  gtk_box_pack_start(GTK_BOX(gimp_export_dialog_get_content_area(dialog)), hbox,
                     FALSE, FALSE, 0);
  ...
  vals->adjustment[0] =
      gtk_adjustment_new(psvals.width, 1e-5, GIMP_MAX_IMAGE_SIZE, 1, 10, 0);
  spinbutton = gimp_spin_button_new(vals->adjustment[0], 1.0, 2);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Width:"), 0.0, 0.5,
                           spinbutton, 1);
  g_signal_connect(vals->adjustment[0], "value-changed",
                   G_CALLBACK(gimp_double_adjustment_update), &psvals.width);

  vals->adjustment[1] =
      gtk_adjustment_new(psvals.height, 1e-5, GIMP_MAX_IMAGE_SIZE, 1, 10, 0);
  spinbutton = gimp_spin_button_new(vals->adjustment[1], 1.0, 2);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Height:"), 0.0, 0.5,
                           spinbutton, 1);
  g_signal_connect(vals->adjustment[1], "value-changed",
                   G_CALLBACK(gimp_double_adjustment_update), &psvals.height);

  vals->adjustment[2] =
      gtk_adjustment_new(psvals.x_offset, 0.0, GIMP_MAX_IMAGE_SIZE, 1, 10, 0);
  spinbutton = gimp_spin_button_new(vals->adjustment[2], 1.0, 2);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("_X offset:"), 0.0, 0.5,
                           spinbutton, 1);
  g_signal_connect(vals->adjustment[2], "value-changed",
                   G_CALLBACK(gimp_double_adjustment_update), &psvals.x_offset);

  vals->adjustment[3] =
      gtk_adjustment_new(psvals.y_offset, 0.0, GIMP_MAX_IMAGE_SIZE, 1, 10, 0);
  spinbutton = gimp_spin_button_new(vals->adjustment[3], 1.0, 2);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("_Y offset:"), 0.0, 0.5,
                           spinbutton, 1);
  g_signal_connect(vals->adjustment[3], "value-changed",
                   G_CALLBACK(gimp_double_adjustment_update), &psvals.y_offset);
  ...
  /* Unit */
  uframe = gimp_int_radio_group_new(
      TRUE, _("Unit"), G_CALLBACK(save_unit_toggle_update), vals, NULL,
      psvals.unit_mm,

      _("_Inch"), FALSE, NULL, _("_Millimeter"), TRUE, NULL,
  ...
  /* Rotation */
  frame = gimp_int_radio_group_new(
      TRUE, _("Rotation"), G_CALLBACK(gimp_radio_button_update), &psvals.rotate,
      NULL, psvals.rotate,

      "_0", 0, NULL, "_90", 90, NULL, "_180", 180, NULL, "_270", 270, NULL,
  ...
  g_object_bind_property(toggle, "active", grid, "sensitive",
                         G_BINDING_SYNC_CREATE);

  adj = gtk_adjustment_new(psvals.preview_size, 0, 1024, 1, 10, 0);
  spinbutton = gimp_spin_button_new(adj, 1.0, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Preview _size:"), 1.0, 0.5,
                           spinbutton, 1);
