...
typedef enum { ACT_LREDUX, ACT_SREDUX, ACT_FLAG } hotAction;

typedef enum { MODE_NTSC, MODE_PAL } hotModes;
...
static GimpValueArray *hot_run(GimpProcedure *procedure, GimpRunMode run_mode,
                               GimpImage *image, GimpDrawable *drawable,
                               const GimpValueArray *args, gpointer run_data);

static gboolean pluginCore(GimpImage *image, GimpDrawable *drawable,
                           piArgs *argp);
static gboolean plugin_dialog(piArgs *argp);
static gboolean hotp(guint8 r, guint8 g, guint8 b);
...
static void hot_class_init(HotClass *klass) {
...
static GimpProcedure *hot_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, hot_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Eric L. Hernes, Alan Wm Paeth",
                                   "Eric L. Hernes", "1997");

    GIMP_PROC_ARG_INT(procedure, "mode", "Mode", "Mode { NTSC (0), PAL (1) }",
                      0, 1, MODE_NTSC, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "action", "Action",
                      "Action { (0) reduce luminance, "
                      "(1) reduce saturation, or (2) Blacken }",
                      0, 2, ACT_LREDUX, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "new-layer", "New layer",
                          "Create a new layer", TRUE, G_PARAM_READWRITE);
  }
...
static gboolean pluginCore(GimpImage *image, GimpDrawable *drawable,
  ...
  if (argp->new_layerp) {
    ...
    g_snprintf(name, sizeof(name), "hot mask (%s, %s)", mode_names[argp->mode],
               action_names[argp->action]);

    nl = gimp_layer_new(image, name, width, height, GIMP_RGBA_IMAGE, 100,
  ...
  gegl_buffer_get(src_buffer, GEGL_RECTANGLE(sel_x1, sel_y1, width, height),
                  1.0, src_format, src, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
        ...
        if (argp->action == ACT_FLAG) {
          ...
          if (r == prev_r && g == prev_g && b == prev_b) {
            ...
            *d++ = new_b;
            ...
            fc = hypot((double)I / (double)SCALE, (double)Q / (double)SCALE);
  ...
  gegl_buffer_set(dest_buffer, GEGL_RECTANGLE(sel_x1, sel_y1, width, height), 0,
                  dest_format, dst, GEGL_AUTO_ROWSTRIDE);
...
static gboolean plugin_dialog(piArgs *argp) {
  ...
  dlg = gimp_dialog_new(
      _("Hot"), PLUG_IN_ROLE, NULL, 0, gimp_standard_help_func, PLUG_IN_PROC,
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Mode"), G_CALLBACK(gimp_radio_button_update), &argp->mode, NULL,
      argp->mode,

      "N_TSC", MODE_NTSC, NULL, "_PAL", MODE_PAL, NULL,
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Action"), G_CALLBACK(gimp_radio_button_update), &argp->action,
      NULL, argp->action,

      _("Reduce _Luminance"), ACT_LREDUX, NULL, _("Reduce _Saturation"),
      ACT_SREDUX, NULL, _("_Blacken"), ACT_FLAG, NULL,
...
static void build_tab(int m) {
  ...
  compos_lim =
      ((double)COMPOS_LIM - mode[m].pedestal) / (100.0 - mode[m].pedestal);
