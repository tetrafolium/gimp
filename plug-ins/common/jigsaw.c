...
static void jigsaw(GimpDrawable *drawable, GimpPreview *preview);
static void jigsaw_preview(GimpDrawable *drawable, GimpPreview *preview);

static gboolean jigsaw_dialog(GimpDrawable *drawable);
static void jigsaw_scale_entry_update_double(GimpLabelSpin *entry,
                                             gdouble *value);
static void jigsaw_scale_entry_update_int(GimpLabelSpin *entry, gint *value);

static void draw_jigsaw(guchar *buffer, gint bufsize, gint width, gint height,
                        gint bytes, gboolean preview_mode);

static void draw_vertical_border(guchar *buffer, gint bufsize, gint width,
                                 gint height, gint bytes, gint x_offset,
                                 gint ytiles, gint blend_lines,
                                 gdouble blend_amount);
static void draw_horizontal_border(guchar *buffer, gint bufsize, gint width,
                                   gint bytes, gint y_offset, gint xtiles,
                                   gint blend_lines, gdouble blend_amount);
static void draw_vertical_line(guchar *buffer, gint bufsize, gint width,
                               gint bytes, gint px[2], gint py[2]);
static void draw_horizontal_line(guchar *buffer, gint bufsize, gint width,
                                 gint bytes, gint px[2], gint py[2]);
static void darken_vertical_line(guchar *buffer, gint bufsize, gint width,
                                 gint bytes, gint *px, gint *py, gdouble delta);
static void lighten_vertical_line(guchar *buffer, gint bufsize, gint width,
                                  gint bytes, gint *px, gint *py,
                                  gdouble delta);
static void darken_horizontal_line(guchar *buffer, gint bufsize, gint width,
                                   gint bytes, gint *px, gint *py,
                                   gdouble delta);
static void lighten_horizontal_line(guchar *buffer, gint bufsize, gint width,
                                    gint bytes, gint *px, gint *py,
                                    gdouble delta);
static void draw_right_bump(guchar *buffer, gint bufsize, gint width,
                            gint bytes, gint x_offset, gint curve_start_offset,
                            gint steps);
static void draw_left_bump(guchar *buffer, gint bufsize, gint width, gint bytes,
                           gint x_offset, gint curve_start_offset, gint steps);
static void draw_up_bump(guchar *buffer, gint bufsize, gint width, gint bytes,
                         gint y_offset, gint curve_start_offset, gint steps);
static void draw_down_bump(guchar *buffer, gint bufsize, gint width, gint bytes,
                           gint y_offset, gint curve_start_offset, gint steps);
static void darken_right_bump(guchar *buffer, gint bufsize, gint width,
                              gint bytes, gint x_offset,
                              gint curve_start_offset, gint steps,
                              gdouble delta, gint counter);
static void lighten_right_bump(guchar *buffer, gint bufsize, gint width,
                               gint bytes, gint x_offset,
                               gint curve_start_offset, gint steps,
                               gdouble delta, gint counter);
static void darken_left_bump(guchar *buffer, gint bufsize, gint width,
                             gint bytes, gint x_offset, gint curve_start_offset,
                             gint steps, gdouble delta, gint counter);
static void lighten_left_bump(guchar *buffer, gint bufsize, gint width,
                              gint bytes, gint x_offset,
                              gint curve_start_offset, gint steps,
                              gdouble delta, gint counter);
static void darken_up_bump(guchar *buffer, gint bufsize, gint width, gint bytes,
                           gint y_offset, gint curve_start_offest, gint steps,
                           gdouble delta, gint counter);
static void lighten_up_bump(guchar *buffer, gint bufsize, gint width,
                            gint bytes, gint y_offset, gint curve_start_offset,
                            gint steps, gdouble delta, gint counter);
static void darken_down_bump(guchar *buffer, gint bufsize, gint width,
                             gint bytes, gint y_offset, gint curve_start_offset,
                             gint steps, gdouble delta, gint counter);
static void lighten_down_bump(guchar *buffer, gint bufsize, gint width,
                              gint bytes, gint y_offset,
                              gint curve_start_offset, gint steps,
                              gdouble delta, gint counter);
static void generate_grid(gint width, gint height, gint xtiles, gint ytiles,
                          gint *x, gint *y);
static void generate_bezier(gint px[4], gint py[4], gint steps, gint *cachex,
                            gint *cachey);
...
static void draw_bezier_line(guchar *buffer, gint bufsize, gint width,
                             gint bytes, gint steps, gint *cx, gint *cy);
static void darken_bezier_line(guchar *buffer, gint bufsize, gint width,
                               gint bytes, gint x_offset, gint y_offset,
                               gint steps, gint *cx, gint *cy, gdouble delta);
static void lighten_bezier_line(guchar *buffer, gint bufsize, gint width,
                                gint bytes, gint x_offset, gint y_offset,
                                gint steps, gint *cx, gint *cy, gdouble delta);
static void draw_bezier_vertical_border(guchar *buffer, gint bufsize,
                                        gint width, gint height, gint bytes,
                                        gint x_offset, gint xtiles, gint ytiles,
                                        gint blend_lines, gdouble blend_amount,
                                        gint steps);
...
static config_t config = {5, 5, BEZIER_1, 3, 0.5};

static globals_t globals;

static void jigsaw_class_init(JigsawClass *klass) {
...
static void jigsaw_init(Jigsaw *jigsaw) {}
...
static GimpProcedure *jigsaw_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, jigsaw_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     N_("Add a jigsaw-puzzle pattern "
                                        "to the image"),
                                     "Jigsaw puzzle look", name);
    gimp_procedure_set_attribution(procedure, "Nigel Wetten", "Nigel Wetten",
                                   "May 2000");

    GIMP_PROC_ARG_INT(procedure, "x", "X", "Number of tiles across", 1,
                      GIMP_MAX_IMAGE_SIZE, 5, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "y", "Y", "Number of tiles down", 1,
                      GIMP_MAX_IMAGE_SIZE, 5, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "style", "Style",
                      "The style/shape of the jigsaw puzzle { 0, 1 }", 0, 1,
                      BEZIER_1, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "blend-lines", "Blend lines",
                      "Number of lines for shading bevels", 1,
                      GIMP_MAX_IMAGE_SIZE, 3, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "blend-amount", "Blend amount",
                         "The power of the light highlights", 0, 5, 0.5,
                         G_PARAM_READWRITE);
  }
...
static void jigsaw(GimpDrawable *drawable, GimpPreview *preview) {
  ...
  if (preview) {
    ...
    buffer =
        gimp_drawable_get_thumbnail_data(drawable, &width, &height, &bytes);
  ...
  if (preview) {
    ...
    gegl_buffer_set(gegl_buffer, GEGL_RECTANGLE(0, 0, width, height), 0, format,
...
static void generate_bezier(gint px[4], gint py[4], gint steps, gint *cachex,
  ...
  for (i = 0; i < steps; i++) {
    ...
    t3 = t2 * t;
    t_1 = 1 - t;
    x = t_1 * t_1 * t_1 * px[0] + 3 * t * t_1 * t_1 * px[1] +
        3 * t2 * t_1 * px[2] + t3 * px[3];
    y = t_1 * t_1 * t_1 * py[0] + 3 * t * t_1 * t_1 * py[1] +
        3 * t2 * t_1 * py[2] + t3 * py[3];
    cachex[i] = (gint)(x + 0.2);
    cachey[i] = (gint)(y + 0.2);
  } /* for */
}
...
static void draw_jigsaw(guchar *buffer, gint bufsize, gint width, gint height,
  ...
  if (style == BEZIER_1) {
    for (i = 0; i < xlines; i++) {
      draw_vertical_border(buffer, bufsize, width, height, bytes, x[i], ytiles,
    ...
    for (i = 0; i < xlines; i++) {
      draw_bezier_vertical_border(buffer, bufsize, width, height, bytes, x[i],
                                  xtiles, ytiles, blend_lines, blend_amount,
                                  steps);
      if (!preview_mode)
        gimp_progress_update((gdouble)i / (gdouble)progress_total);
    }
    for (i = 0; i < ylines; i++) {
      draw_bezier_horizontal_border(buffer, bufsize, width, height, bytes, y[i],
                                    xtiles, ytiles, blend_lines, blend_amount,
                                    steps);
      if (!preview_mode)
        gimp_progress_update((gdouble)(i + xlines) / (gdouble)progress_total);
  ...
  for (i = 0; i < ytiles; i++) {
    ...
    if (right) {
      draw_right_bump(buffer, bufsize, width, bytes, x_offset,
                      y_offset + curve_start_offset, globals.steps[RIGHT]);
      delta = blend_amount;
      for (j = 0; j < blend_lines; j++) {
        /* use to be -j -1 */
      ...
      draw_left_bump(buffer, bufsize, width, bytes, x_offset,
                     y_offset + curve_start_offset, globals.steps[LEFT]);
      delta = blend_amount;
      for (j = 0; j < blend_lines; j++) {
        /* use to be -j -1 */
...
static void draw_horizontal_border(guchar *buffer, gint bufsize, gint width,
  ...
  for (i = 0; i < xtiles; i++) {
    ...
    for (j = 0; j < blend_lines; j++) {
      ...
      lx[1]--;
      py[0] = y_offset - j - 1;
      darken_horizontal_line(buffer, bufsize, width, bytes, dx, py, delta);
      py[0] = y_offset + j + 1;
      lighten_horizontal_line(buffer, bufsize, width, bytes, lx, py, delta);
      delta -= sigma;
    }

    if (up) {
      draw_up_bump(buffer, bufsize, width, bytes, y_offset,
                   x_offset + curve_start_offset, globals.steps[UP]);
      delta = blend_amount;
      for (j = 0; j < blend_lines; j++) {
        /* use to be -j -1 */
        darken_up_bump(buffer, bufsize, width, bytes, y_offset,
                       x_offset + curve_start_offset, globals.steps[UP], delta,
                       j);
        /* use to be +j + 1 */
      ...
      draw_down_bump(buffer, bufsize, width, bytes, y_offset,
                     x_offset + curve_start_offset, globals.steps[DOWN]);
      delta = blend_amount;
      for (j = 0; j < blend_lines; j++) {
        /* use to be +j + 1 */
...
static void init_right_bump(gint width, gint height) {
  ...
  for (i = 0; i < blend_lines; i++) {
    ...
    py[2]--;
    px[3]++;
    generate_bezier(px, py, steps, globals.blend_outer_cachex1[RIGHT][i],
  ...
  for (i = 0; i < blend_lines; i++) {
    ...
    py[2]++;
    px[3]--;
    generate_bezier(px, py, steps, globals.blend_inner_cachex1[RIGHT][i],
  ...
  for (i = 0; i < blend_lines; i++) {
    ...
    py[3]++;
    px[0]++;
    generate_bezier(px, py, steps, globals.blend_outer_cachex2[RIGHT][i],
  ...
  for (i = 0; i < blend_lines; i++) {
    ...
    py[3]--;
    px[0]--;
    generate_bezier(px, py, steps, globals.blend_inner_cachex2[RIGHT][i],
...
static void init_left_bump(gint width, gint height) {
  ...
  py[2] = curve_start_offset - YFACTOR3 * tile_height;
  py[3] = curve_start_offset + YFACTOR4 * tile_height;
  generate_bezier(px, py, steps, globals.cachex1[LEFT], globals.cachey1[LEFT]);
  /* outer left bump */
  for (i = 0; i < blend_lines; i++) {
    py[0]--;
    py[1]--;
    py[2]--;
    px[3]--;
    generate_bezier(px, py, steps, globals.blend_outer_cachex1[LEFT][i],
  ...
  for (i = 0; i < blend_lines; i++) {
    ...
    py[2]++;
    px[3]++;
    generate_bezier(px, py, steps, globals.blend_inner_cachex1[LEFT][i],
  ...
  py[2] = curve_start_offset + YFACTOR7 * tile_height;
  py[3] = curve_end_offset;
  generate_bezier(px, py, steps, globals.cachex2[LEFT], globals.cachey2[LEFT]);
  /* outer left bump */
  for (i = 0; i < blend_lines; i++) {
    py[1]++;
    py[2]++;
    py[3]++;
    px[0]--;
    generate_bezier(px, py, steps, globals.blend_outer_cachex2[LEFT][i],
  ...
  for (i = 0; i < blend_lines; i++) {
    ...
    py[3]--;
    px[0]++;
    generate_bezier(px, py, steps, globals.blend_inner_cachex2[LEFT][i],
...
static void init_up_bump(gint width, gint height) {
  ...
  generate_bezier(px, py, steps, globals.cachex1[UP], globals.cachey1[UP]);
  /* outer up bump */
  for (i = 0; i < blend_lines; i++) {
    px[0]--;
    px[1]--;
    px[2]--;
    py[3]--;
    generate_bezier(px, py, steps, globals.blend_outer_cachex1[UP][i],
  ...
  for (i = 0; i < blend_lines; i++) {
    ...
    px[2]++;
    py[3]++;
    generate_bezier(px, py, steps, globals.blend_inner_cachex1[UP][i],
  ...
  py[3] = y_offset;
  generate_bezier(px, py, steps, globals.cachex2[UP], globals.cachey2[UP]);
  /* outer up bump */
  for (i = 0; i < blend_lines; i++) {
    px[1]++;
    px[2]++;
    px[3]++;
    py[0]--;
    generate_bezier(px, py, steps, globals.blend_outer_cachex2[UP][i],
  ...
  for (i = 0; i < blend_lines; i++) {
    ...
    px[3]--;
    py[0]++;
    generate_bezier(px, py, steps, globals.blend_inner_cachex2[UP][i],
...
static void init_down_bump(gint width, gint height) {
  ...
  py[2] = y_offset + XFACTOR3 * tile_height;
  py[3] = y_offset + XFACTOR4 * tile_height;
  generate_bezier(px, py, steps, globals.cachex1[DOWN], globals.cachey1[DOWN]);
  /* outer down bump */
  for (i = 0; i < blend_lines; i++) {
    px[0]--;
    px[1]--;
    px[2]--;
    py[3]++;
    generate_bezier(px, py, steps, globals.blend_outer_cachex1[DOWN][i],
  ...
  for (i = 0; i < blend_lines; i++) {
    ...
    px[2]++;
    py[3]--;
    generate_bezier(px, py, steps, globals.blend_inner_cachex1[DOWN][i],
  ...
  py[2] = y_offset + XFACTOR7 * tile_height;
  py[3] = y_offset;
  generate_bezier(px, py, steps, globals.cachex2[DOWN], globals.cachey2[DOWN]);
  /* outer down bump */
  for (i = 0; i < blend_lines; i++) {
    px[1]++;
    px[2]++;
    px[3]++;
    py[0]++;
    generate_bezier(px, py, steps, globals.blend_outer_cachex2[DOWN][i],
  ...
  for (i = 0; i < blend_lines; i++) {
    ...
    px[3]--;
    py[0]--;
    generate_bezier(px, py, steps, globals.blend_inner_cachex2[DOWN][i],
...
static void darken_right_bump(guchar *buffer, gint bufsize, gint width,
  ...
  for (i = 0; i < steps; i++) {
    x = x_offset + globals.blend_inner_cachex1[RIGHT][j][i];
    y = curve_start_offset + globals.blend_inner_cachey1[RIGHT][j][i];
    index = y * rowstride + x * bytes;
    ...
    x = x_offset + globals.blend_inner_cachex2[RIGHT][j][i];
    y = curve_start_offset + globals.blend_inner_cachey2[RIGHT][j][i];
    index = y * rowstride + x * bytes;
...
static void lighten_right_bump(guchar *buffer, gint bufsize, gint width,
  ...
  for (i = 0; i < steps; i++) {
    x = x_offset + globals.blend_outer_cachex1[RIGHT][j][i];
    y = curve_start_offset + globals.blend_outer_cachey1[RIGHT][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index1) {
      if (i < steps / 1.3) {
        DARKEN_POINT(buffer, bufsize, index, delta, temp);
    ...
    x = x_offset + globals.blend_outer_cachex2[RIGHT][j][i];
    y = curve_start_offset + globals.blend_outer_cachey2[RIGHT][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index2) {
      LIGHTEN_POINT(buffer, bufsize, index, delta, temp);
      last_index2 = index;
...
static void darken_left_bump(guchar *buffer, gint bufsize, gint width,
  ...
  for (i = 0; i < steps; i++) {
    x = x_offset + globals.blend_outer_cachex1[LEFT][j][i];
    y = curve_start_offset + globals.blend_outer_cachey1[LEFT][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index1) {
      DARKEN_POINT(buffer, bufsize, index, delta, temp);
      last_index1 = index;
    ...
    x = x_offset + globals.blend_outer_cachex2[LEFT][j][i];
    y = curve_start_offset + globals.blend_outer_cachey2[LEFT][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index2) {
      if (i < steps / 4) {
        DARKEN_POINT(buffer, bufsize, index, delta, temp);
...
static void lighten_left_bump(guchar *buffer, gint bufsize, gint width,
  ...
  for (i = 0; i < steps; i++) {
    x = x_offset + globals.blend_inner_cachex1[LEFT][j][i];
    y = curve_start_offset + globals.blend_inner_cachey1[LEFT][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index1) {
      LIGHTEN_POINT(buffer, bufsize, index, delta, temp);
      last_index1 = index;
    ...
    x = x_offset + globals.blend_inner_cachex2[LEFT][j][i];
    y = curve_start_offset + globals.blend_inner_cachey2[LEFT][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index2) {
      if (i < steps / 4) {
        LIGHTEN_POINT(buffer, bufsize, index, delta, temp);
...
static void darken_up_bump(guchar *buffer, gint bufsize, gint width, gint bytes,
  ...
  for (i = 0; i < steps; i++) {
    x = curve_start_offset + globals.blend_outer_cachex1[UP][j][i];
    y = y_offset + globals.blend_outer_cachey1[UP][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index1) {
      DARKEN_POINT(buffer, bufsize, index, delta, temp);
      last_index1 = index;
    ...
    x = curve_start_offset + globals.blend_outer_cachex2[UP][j][i];
    y = y_offset + globals.blend_outer_cachey2[UP][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index2) {
      if (i < steps / 4) {
        DARKEN_POINT(buffer, bufsize, index, delta, temp);
...
static void lighten_up_bump(guchar *buffer, gint bufsize, gint width,
  ...
  for (i = 0; i < steps; i++) {
    x = curve_start_offset + globals.blend_inner_cachex1[UP][j][i];
    y = y_offset + globals.blend_inner_cachey1[UP][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index1) {
      LIGHTEN_POINT(buffer, bufsize, index, delta, temp);
      last_index1 = index;
    ...
    x = curve_start_offset + globals.blend_inner_cachex2[UP][j][i];
    y = y_offset + globals.blend_inner_cachey2[UP][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index2) {
      if (i < steps / 4) {
        LIGHTEN_POINT(buffer, bufsize, index, delta, temp);
...
static void darken_down_bump(guchar *buffer, gint bufsize, gint width,
  ...
  for (i = 0; i < steps; i++) {
    x = curve_start_offset + globals.blend_inner_cachex1[DOWN][j][i];
    y = y_offset + globals.blend_inner_cachey1[DOWN][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index1) {
      if (i < steps / 1.2) {
        LIGHTEN_POINT(buffer, bufsize, index, delta, temp);
    ...
    x = curve_start_offset + globals.blend_inner_cachex2[DOWN][j][i];
    y = y_offset + globals.blend_inner_cachey2[DOWN][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index2) {
      DARKEN_POINT(buffer, bufsize, index, delta, temp);
      last_index2 = index;
...
static void lighten_down_bump(guchar *buffer, gint bufsize, gint width,
  ...
  for (i = 0; i < steps; i++) {
    x = curve_start_offset + globals.blend_outer_cachex1[DOWN][j][i];
    y = y_offset + globals.blend_outer_cachey1[DOWN][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index1) {
      if (i < steps / 1.2) {
        DARKEN_POINT(buffer, bufsize, index, delta, temp);
    ...
    x = curve_start_offset + globals.blend_outer_cachex2[DOWN][j][i];
    y = y_offset + globals.blend_outer_cachey2[DOWN][j][i];
    index = y * rowstride + x * bytes;
    if (index != last_index2) {
      LIGHTEN_POINT(buffer, bufsize, index, delta, temp);
      last_index2 = index;
...
static void draw_bezier_vertical_border(guchar *buffer, gint bufsize,
  ...
  for (i = 0; i < ytiles; i++) {
    ...
    for (j = 0; j < blend_lines; j++) {
      px[0] = -j - 1;
      darken_bezier_line(buffer, bufsize, width, bytes, px[0], 0, steps, cachex,
                         cachey, delta);
      px[0] = j + 1;
      lighten_bezier_line(buffer, bufsize, width, bytes, px[0], 0, steps,
                          cachex, cachey, delta);
      delta -= sigma;
    }
    if (right) {
...
static void draw_bezier_horizontal_border(guchar *buffer, gint bufsize,
  ...
  for (i = 0; i < xtiles; i++) {
    ...
    for (j = 0; j < blend_lines; j++) {
      py[0] = -j - 1;
      darken_bezier_line(buffer, bufsize, width, bytes, 0, py[0], steps, cachex,
                         cachey, delta);
      py[0] = j + 1;
      lighten_bezier_line(buffer, bufsize, width, bytes, 0, py[0], steps,
                          cachex, cachey, delta);
      delta -= sigma;
    }
    /* bumps */
...
static gboolean jigsaw_dialog(GimpDrawable *drawable) {
  ...
  dialog = gimp_dialog_new(
      _("Jigsaw"), PLUG_IN_ROLE, NULL, 0, gimp_standard_help_func, PLUG_IN_PROC,
  ...
  g_signal_connect_swapped(preview, "invalidated", G_CALLBACK(jigsaw_preview),
                           drawable);
  ...
  /* xtiles */
  scale = gimp_scale_entry_new(_("_Horizontal:"), config.x, MIN_XTILES,
                               MAX_XTILES, 0);
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(jigsaw_scale_entry_update_int), &config.x);
  g_signal_connect_swapped(scale, "value-changed",
                           G_CALLBACK(gimp_preview_invalidate), preview);

  /* ytiles */
  scale = gimp_scale_entry_new(_("_Vertical:"), config.y, MIN_YTILES,
                               MAX_YTILES, 0);
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(jigsaw_scale_entry_update_int), &config.y);
  g_signal_connect_swapped(scale, "value-changed",
                           G_CALLBACK(gimp_preview_invalidate), preview);
  ...
  scale = gimp_scale_entry_new(_("_Bevel width:"), config.blend_lines,
                               MIN_BLEND_LINES, MAX_BLEND_LINES, 0);
  ...
  /* blending amount */
  scale = gimp_scale_entry_new(_("H_ighlight:"), config.blend_amount,
                               MIN_BLEND_AMOUNT, MAX_BLEND_AMOUNT, 2);
  gimp_help_set_help_data(
      scale, _("The amount of highlighting on the edges of each piece"), NULL);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 1, 3, 1);
  gtk_widget_show(scale);
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Jigsaw Style"), G_CALLBACK(gimp_radio_button_update),
      &config.style, NULL, config.style,

      _("_Square"), BEZIER_1, &rbutton1, _("C_urved"), BEZIER_2, &rbutton2,
