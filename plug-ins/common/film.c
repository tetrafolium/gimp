...
static GimpValueArray *film_run(GimpProcedure *procedure, GimpRunMode run_mode,
                                GimpImage *image, GimpDrawable *drawable,
                                const GimpValueArray *args, gpointer run_data);

static GimpImage *create_new_image(guint width, guint height,
                                   GimpImageType gdtype, GimpLayer **layer);
...
static void set_pixels(gint numpix, guchar *dst, GimpRGB *color);

static guchar *create_hole_rgb(gint width, gint height);

static void draw_number(GimpLayer *layer, gint num, gint x, gint y,
                        gint height);

static void add_list_item_callback(GtkWidget *widget, GtkTreeSelection *sel);
static void del_list_item_callback(GtkWidget *widget, GtkTreeSelection *sel);

static GtkTreeModel *add_image_list(gboolean add_box_flag, GList *images,
                                    GtkWidget *hbox);

static gboolean film_dialog(GimpImage *image);
static void film_reset_callback(GtkWidget *widget, gpointer data);
static void film_font_select_callback(GimpFontSelectButton *button,
                                      const gchar *name, gboolean closing,
                                      gpointer data);
...
static FilmInterface filmint = {
    ...
    NULL,
    NULL /* image list widgets */
};

static void film_class_init(FilmClass *klass) {
...
static void film_init(Film *film) {}
...
static GimpProcedure *film_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, film_run, NULL, NULL);
    ...
    gimp_procedure_set_attribution(
        procedure, "Peter Kirchgessner",
        "Peter Kirchgessner (peter@kirchgessner.net)", "1997");

    GIMP_PROC_ARG_INT(procedure, "film-height", "Film height",
                      "Height of film (0: fit to images)", 0,
                      GIMP_MAX_IMAGE_SIZE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_RGB(procedure, "film-color", "Film color",
                      "Color of the film", TRUE, NULL, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "number-start", "Number start",
                      "Start index for numbering", G_MININT, G_MAXINT, 1,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "number-font", "Number font",
                         "Font for drawing numbers", NULL, G_PARAM_READWRITE);

    GIMP_PROC_ARG_RGB(procedure, "number-color", "Number color",
                      "Color for numbers", TRUE, NULL, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "at-top", "At top", "Draw numbers at top",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "at-bottom", "At bottom",
                          "Draw numbers at bottom", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "num-images", "Num images",
                      "Number of images to be used for film", 1,
                      MAX_FILM_PICTURES, 1, G_PARAM_READWRITE);

    GIMP_PROC_ARG_OBJECT_ARRAY(procedure, "images", "Images",
                               "num-images images to be used for film",
                               GIMP_TYPE_IMAGE, G_PARAM_READWRITE);

    GIMP_PROC_VAL_IMAGE(procedure, "new-image", "New image", "Output image",
...
static GimpValueArray *film_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_NONINTERACTIVE:
    ...
    g_strlcpy(filmvals.number_font, GIMP_VALUES_GET_STRING(args, 3), FONT_LEN);
...
static GimpImage *film(void) {
  ...
  for (i = 0; i < filmvals.num_images; i++) {
    images_src =
        g_list_append(images_src, gimp_image_get_by_id(filmvals.images[i]));
  }
...
#ifdef FILM_DEBUG
  g_printerr(
      "hole_x %d hole_offset %d hole_width %d hole_height %d hole_space %d\n",
  ...
  if (hole) {
    ...
    while (hole_x < film_width) {
      gegl_buffer_set(
          buffer, GEGL_RECTANGLE(hole_x, hole_offset, hole_width, hole_height),
          0, babl_format("R'G'B' u8"), hole, GEGL_AUTO_ROWSTRIDE);
  ...
  for (iter = images_src; iter; iter = iter->next) {
    ...
    for (iter2 = layers; iter2; iter2 = g_list_next(iter2)) {
      ...
      new_layer =
          gimp_layer_new_from_drawable(GIMP_DRAWABLE(layer_src), image_dst);
      ...
      if ((number_height > 0) &&
        ...
        if (filmvals.number_pos[0])
          draw_number(layer_dst, filmvals.number_start + picture_count,
...
static void draw_number(GimpLayer *layer, gint num, gint x, gint y,
  ...
  for (k = 0; k < max_delta * 2 + 1; k++) {
    ...
    success = gimp_text_get_extents_fontname(
        buf, height + delta, GIMP_PIXELS, fontname, &text_width, &text_height,
        &text_ascent, &descent);
  ...
  text_layer =
      gimp_text_fontname(image, GIMP_DRAWABLE(layer), x, y + descent / 2, buf,
                         1, FALSE, height, GIMP_PIXELS, fontname);
...
static GimpImage *create_new_image(guint width, guint height,
  ...
  *layer = gimp_layer_new(image, _("Background"), width, height, gdtype, 100,
...
static void add_list_item_callback(GtkWidget *widget, GtkTreeSelection *sel) {
  ...
  for (list = paths; list; list = g_list_next(list)) {
    ...
    if (gtk_tree_model_get_iter(model, &iter, list->data)) {
      ...
      gtk_tree_model_get(model, &iter, 0, &image, 1, &name, -1);

      gtk_list_store_append(GTK_LIST_STORE(filmint.image_list_film), &iter);

      gtk_list_store_set(GTK_LIST_STORE(filmint.image_list_film), &iter, 0,
                         image, 1, name, -1);
...
static GtkTreeModel *add_image_list(gboolean add_box_flag, GList *images,
  ...
  label = gtk_label_new(add_box_flag ? _("Available images:") : _("On film:"));
  ...
  for (list = images; list; list = list->next) {
    ...
    gtk_list_store_set(store, &iter, 0, gimp_image_get_id(list->data), 1, name,
                       -1);
  ...
  button =
      gtk_button_new_with_mnemonic(add_box_flag ? _("_Add") : _("_Remove"));
...
static void create_selection_tab(GtkWidget *notebook, GimpImage *image) {
  ...
  /* Film height */
  adj = gtk_adjustment_new(filmvals.film_height, 10, GIMP_MAX_IMAGE_SIZE, 1, 10,
                           0);
  spinbutton = gimp_spin_button_new(adj, 1, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Height:"), 0.0,
                                   0.5, spinbutton, 1);
  ...
  g_object_bind_property(toggle, "active", spinbutton, "sensitive",
                         G_BINDING_SYNC_CREATE | G_BINDING_INVERT_BOOLEAN);
  g_object_bind_property(
      toggle, "active",
      /* FIXME: eeeeeek */
      g_list_nth_data(gtk_container_get_children(GTK_CONTAINER(grid)), 1),
      "sensitive", G_BINDING_SYNC_CREATE | G_BINDING_INVERT_BOOLEAN);

  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle), filmvals.keep_height);

  /* Film color */
  button = gimp_color_button_new(_("Select Film Color"), COLOR_BUTTON_WIDTH,
                                 COLOR_BUTTON_HEIGHT, &filmvals.film_color,
                                 GIMP_COLOR_AREA_FLAT);
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Co_lor:"), 0.0, 0.5,
                                   button, 1);
  gtk_size_group_add_widget(group, label);
  ...
  /* Startindex */
  adj = gtk_adjustment_new(filmvals.number_start, 0, GIMP_MAX_IMAGE_SIZE, 1, 10,
                           0);
  spinbutton = gimp_spin_button_new(adj, 1, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);

  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Start _index:"),
  ...
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Font:"), 0.0, 0.5,
                                   font_button, 1);
  gtk_size_group_add_widget(group, label);

  /* Numbering color */
  button = gimp_color_button_new(_("Select Number Color"), COLOR_BUTTON_WIDTH,
                                 COLOR_BUTTON_HEIGHT, &filmvals.number_color,
                                 GIMP_COLOR_AREA_FLAT);
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Co_lor:"), 0.0, 0.5,
                                   button, 1);
  gtk_size_group_add_widget(group, label);
  ...
  for (j = 0; j < 2; j++) {
    toggle =
        gtk_check_button_new_with_mnemonic(j ? _("At _bottom") : _("At _top"));
    gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);
...
static void create_advanced_tab(GtkWidget *notebook) {
  GtkWidget *vbox;
  GtkWidget *hbox;
  GtkWidget *grid;
  GtkWidget *frame;
  ...
  filmint.scales[0] = scale = gimp_scale_entry_new(
      _("Image _height:"), filmvals.picture_height, 0.0, 1.0, 3);
  ...
  filmint.scales[1] = scale = gimp_scale_entry_new(
      _("Image spac_ing:"), filmvals.picture_space, 0.0, 1.0, 3);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 0.001, 0.01);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(film_scale_entry_update_double),
  ...
  gtk_widget_set_margin_bottom(
      gimp_scale_entry_get_range(GIMP_SCALE_ENTRY(scale)), 6);
  gtk_widget_set_margin_bottom(
      gimp_label_spin_get_spin_button(GIMP_LABEL_SPIN(scale)), 6);
  gtk_widget_show(scale);

  filmint.scales[2] = scale = gimp_scale_entry_new(
      _("_Hole offset:"), filmvals.hole_offset, 0.0, 1.0, 3);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 0.001, 0.01);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(film_scale_entry_update_double),
  ...
  filmint.scales[4] = scale = gimp_scale_entry_new(
      _("Hol_e height:"), filmvals.hole_height, 0.0, 1.0, 3);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 0.001, 0.01);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(film_scale_entry_update_double),
  ...
  filmint.scales[5] = scale = gimp_scale_entry_new(
      _("Hole sp_acing:"), filmvals.hole_space, 0.0, 1.0, 3);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 0.001, 0.01);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(film_scale_entry_update_double),
                   &filmvals.hole_space);
  ...
  filmint.scales[6] = scale = gimp_scale_entry_new(
      _("_Number height:"), filmvals.number_height, 0.0, 1.0, 3);
  gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scale), 0.001, 0.01);
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(film_scale_entry_update_double),
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(film_reset_callback), NULL);
}
...
static gboolean film_dialog(GimpImage *image) {
  ...
  dlg = gimp_dialog_new(_("Filmstrip"), PLUG_IN_ROLE, NULL, 0,
  ...
  if (run) {
    ...
    for (iter_valid =
             gtk_tree_model_get_iter_first(filmint.image_list_film, &iter);
         iter_valid; iter_valid = gtk_tree_model_iter_next(
                         filmint.image_list_film, &iter)) {
      gint image_id;

      gtk_tree_model_get(filmint.image_list_film, &iter, 0, &image_id, -1);
