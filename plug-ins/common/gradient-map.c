...
typedef enum { GRADIENT_MODE = 1, PALETTE_MODE } MapMode;
...
static void map(GeglBuffer *buffer, GeglBuffer *shadow_buffer,
                GimpDrawable *drawable, MapMode mode);
  ...
  if (!strcmp(name, GRADMAP_PROC)) {
    procedure =
        gimp_image_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                 map_run, GINT_TO_POINTER(GRADIENT_MODE), NULL);
    ...
    procedure =
        gimp_image_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN,
                                 map_run, GINT_TO_POINTER(PALETTE_MODE), NULL);
...
static GimpValueArray *map_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  gimp_drawable_update(drawable, 0, 0, gimp_drawable_width(drawable),
...
static void map(GeglBuffer *buffer, GeglBuffer *shadow_buffer,
  ...
  index_iter = gegl_buffer_iterator_add(gi, buffer, NULL, 0, format_buffer,
...
static gdouble *get_samples_palette(GimpDrawable *drawable) {
  ...
  for (i = 0; i < NSAMPLES; i++) {
    ...
    gimp_rgb_get_pixel(&color_sample, format, d_samp);
  }
