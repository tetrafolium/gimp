...
static GimpValueArray *gih_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static BrushInfo info = {"GIMP Brush Pipe", 20};
...
static const gchar *const selection_modes[] = {
    "incremental", "angular", "random", "velocity",
    "pressure",    "xtilt",   "ytilt"};

static void gih_class_init(GihClass *klass) {
...
static GimpProcedure *gih_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, gih_save, NULL, NULL);
    ...
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-gimp-gih");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "gih");
    gimp_file_procedure_set_handles_remote(GIMP_FILE_PROCEDURE(procedure),
                                           TRUE);

    GIMP_PROC_ARG_INT(procedure, "spacing", "Spacing", "Spacing of the brush",
                      1, 1000, 10, GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "description", "Description",
                         "Short description of the gihtern", "GIMP Gihtern",
                         GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "cell-width", "Cell width",
                      "Width of the brush cells", 1, 1000, 10,
                      GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "cell-height", "Cell height",
                      "Height of the brush cells", 1, 1000, 10,
                      GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "display-cols", "Display columns",
                      "Display column number", 1, 1000, 1,
                      GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "display-rows", "Display rows",
                      "Display row number", 1, 1000, 1, GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "dimension", "Dimension",
                      "Dimension of the brush pipe", 1, 4, 1,
                      GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_UINT8_ARRAY(procedure, "rank", "Rank",
                              "Ranks of the dimensions", GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "dimension-2", "Dimension 2",
                      "Dimension of the brush pipe (same as dimension)", 1, 4,
                      1, GIMP_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING_ARRAY(procedure, "sel", "Sel", "Selection modes",
                               GIMP_PARAM_READWRITE);
  }
...
static GimpValueArray *gih_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "GIH",
    ...
    parasite = gimp_image_get_parasite(orig_image, "gimp-brush-pipe-name");
    if (parasite) {
      g_strlcpy(
          info.description, gimp_parasite_data(parasite),
          MIN(sizeof(info.description), gimp_parasite_data_size(parasite)));
    ...
    parasite = gimp_image_get_parasite(orig_image, "gimp-brush-pipe-spacing");
    if (parasite) {
  ...
  case GIMP_RUN_INTERACTIVE:
    ...
    parasite =
        gimp_image_get_parasite(orig_image, "gimp-brush-pipe-parameters");
    if (parasite) {
      gimp_pixpipe_params_parse(gimp_parasite_data(parasite), &gihparams);
      gimp_parasite_free(parasite);
    }
  ...
  case GIMP_RUN_NONINTERACTIVE:
    ...
    g_strlcpy(info.description, GIMP_VALUES_GET_STRING(args, 1),
  ...
  if (status == GIMP_PDB_SUCCESS) {
    ...
    save_retvals = gimp_pdb_run_procedure(
        gimp_get_pdb(), "file-gih-save-internal", GIMP_TYPE_RUN_MODE,
        GIMP_RUN_NONINTERACTIVE, GIMP_TYPE_IMAGE, image, G_TYPE_INT,
        n_drawables, GIMP_TYPE_OBJECT_ARRAY, drawables, G_TYPE_FILE, file,
        G_TYPE_INT, info.spacing, G_TYPE_STRING, info.description,
        G_TYPE_STRING, paramstring, G_TYPE_NONE);

    if (GIMP_VALUES_GET_ENUM(save_retvals, 0) == GIMP_PDB_SUCCESS) {
      gimp_set_data(SAVE_PROC, &info, sizeof(info));

      parasite =
          gimp_parasite_new("gimp-brush-pipe-name", GIMP_PARASITE_PERSISTENT,
                            strlen(info.description) + 1, info.description);
      gimp_image_attach_parasite(orig_image, parasite);
      gimp_parasite_free(parasite);

      g_snprintf(spacing, sizeof(spacing), "%d", info.spacing);

      parasite =
          gimp_parasite_new("gimp-brush-pipe-spacing", GIMP_PARASITE_PERSISTENT,
...
static void size_adjustment_callback(GtkAdjustment *adjustment,
  ...
  *(adj->value) = gtk_adjustment_get_value(adjustment);
  ...
  if (adj->orientation == GIMP_ORIENTATION_VERTICAL) {
    ...
    for (i = 0; i < adj->nguides; i++)
      adj->guides[i] =
          gimp_image_add_vguide(adj->image, *(adj->value) * (i + 1));
  } else {
    ...
    for (i = 0; i < adj->nguides; i++)
      adj->guides[i] =
          gimp_image_add_hguide(adj->image, *(adj->value) * (i + 1));
  }
  gimp_displays_flush();
  ...
  *(adj->count) = newn;
  ...
  if (adj->ncells != NULL)
    gtk_adjustment_set_value(adj->ncells, *(adj->other_count) * *(adj->count));
  if (adj->rank0 != NULL)
    gtk_adjustment_set_value(adj->rank0, *(adj->other_count) * *(adj->count));
}
...
static void cb_callback(GtkWidget *widget, gpointer data) {
  gint index;
...
static void dim_callback(GtkAdjustment *adjustment, SizeAdjustmentData *data) {
  gint i;
...
static gboolean gih_save_dialog(GimpImage *image) {
  ...
  gtk_box_pack_start(GTK_BOX(gimp_export_dialog_get_content_area(dialog)), grid,
                     TRUE, TRUE, 0);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Description:"), 0.0, 0.5,
                           entry, 1);

  g_signal_connect(entry, "changed", G_CALLBACK(entry_callback),
                   info.description);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Spacing (percent):"), 0.0,
                           0.5, spinbutton, 1);

  g_signal_connect(adjustment, "value-changed",
                   G_CALLBACK(gimp_int_adjustment_update), &info.spacing);
  ...
  adjustment = gtk_adjustment_new(gihparams.cellwidth, 2,
                                  gimp_image_width(image), 1, 10, 0);
  spinbutton = gimp_spin_button_new(adjustment, 1.0, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  ...
  g_signal_connect(adjustment, "value-changed",
                   G_CALLBACK(size_adjustment_callback), &cellw_adjust);

  label = gtk_label_new("x");
  gtk_box_pack_start(GTK_BOX(box), label, FALSE, FALSE, 0);
  gtk_widget_show(label);

  adjustment = gtk_adjustment_new(gihparams.cellheight, 2,
                                  gimp_image_height(image), 1, 10, 0);
  spinbutton = gimp_spin_button_new(adjustment, 1.0, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gtk_box_pack_start(GTK_BOX(box), spinbutton, FALSE, FALSE, 0);
  gtk_widget_show(spinbutton);
  ...
  g_signal_connect(adjustment, "value-changed",
                   G_CALLBACK(size_adjustment_callback), &cellh_adjust);

  label = gtk_label_new(_("Pixels"));
  gtk_box_pack_start(GTK_BOX(box), label, FALSE, FALSE, 0);
  gtk_widget_show(label);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Ce_ll size:"), 0.0, 0.5,
                           box, 1);
  ...
  spinbutton = gimp_spin_button_new(adjustment, 1.0, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("_Number of cells:"), 0.0,
                           0.5, spinbutton, 1);

  g_signal_connect(adjustment, "value-changed",
                   G_CALLBACK(gimp_int_adjustment_update), &gihparams.ncells);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 4, _("Display as:"), 0.0, 0.5,
                           box, 1);
  ...
  spinbutton = gimp_spin_button_new(adjustment, 1.0, 0);
  gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(spinbutton), TRUE);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 5, _("Di_mension:"), 0.0, 0.5,
                           spinbutton, 1);

  g_signal_connect(adjustment, "value-changed", G_CALLBACK(dim_callback),
                   &cellw_adjust);
  ...
  for (i = 0; i < GIMP_PIXPIPE_MAXDIM; i++) {
    ...
    g_signal_connect(GTK_COMBO_BOX(cb), "changed", G_CALLBACK(cb_callback),
                     &gihparams.selection[i]);

    cellw_adjust.mode_entry[i] = cellh_adjust.mode_entry[i] = cb;
  }

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 6, _("Ranks:"), 0.0, 0.0, dimgrid,
                           1);
  ...
  if (run) {
    ...
    /* Fix up bogus values */
    gihparams.ncells =
        MIN(gihparams.ncells, num_layers * gihparams.rows * gihparams.cols);
  }
