...
static GimpValueArray *xwd_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *xwd_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static GimpImage *create_new_image(GFile *file, guint width, guint height,
                                   GimpImageBaseType type, GimpImageType gdtype,
                                   GimpLayer **layer, GeglBuffer **buffer);

static int set_pixelmap(gint ncols, L_XWDCOLOR *xwdcol, PIXEL_MAP *pixelmap);
static gboolean get_pixelmap(L_CARD32 pixelval, PIXEL_MAP *pixelmap,
                             guchar *red, guchar *green, guchar *glue);

static void set_bw_color_table(GimpImage *image);
static void set_color_table(GimpImage *image, L_XWDFILEHEADER *xwdhdr,
                            L_XWDCOLOR *xwdcolmap);

static GimpImage *load_xwd_f2_d1_b1(GFile *file, FILE *ifp,
                                    L_XWDFILEHEADER *xwdhdr,
                                    L_XWDCOLOR *xwdcolmap);
static GimpImage *load_xwd_f2_d8_b8(GFile *file, FILE *ifp,
                                    L_XWDFILEHEADER *xwdhdr,
                                    L_XWDCOLOR *xwdcolmap);
static GimpImage *load_xwd_f2_d16_b16(GFile *file, FILE *ifp,
                                      L_XWDFILEHEADER *xwdhdr,
                                      L_XWDCOLOR *xwdcolmap);
static GimpImage *load_xwd_f2_d24_b32(GFile *file, FILE *ifp,
                                      L_XWDFILEHEADER *xwdhdr,
                                      L_XWDCOLOR *xwdcolmap, GError **error);
static GimpImage *load_xwd_f2_d32_b32(GFile *file, FILE *ifp,
                                      L_XWDFILEHEADER *xwdhdr,
                                      L_XWDCOLOR *xwdcolmap);
static GimpImage *load_xwd_f1_d24_b1(GFile *file, FILE *ifp,
                                     L_XWDFILEHEADER *xwdhdr,
                                     L_XWDCOLOR *xwdcolmap, GError **error);

static L_CARD32 read_card32(FILE *ifp, gint *err);
static L_CARD16 read_card16(FILE *ifp, gint *err);
static L_CARD8 read_card8(FILE *ifp, gint *err);

static gboolean write_card32(GOutputStream *output, L_CARD32 c, GError **error);
static gboolean write_card16(GOutputStream *output, L_CARD32 c, GError **error);
static gboolean write_card8(GOutputStream *output, L_CARD32 c, GError **error);

static void read_xwd_header(FILE *ifp, L_XWDFILEHEADER *xwdhdr);

static gboolean write_xwd_header(GOutputStream *output, L_XWDFILEHEADER *xwdhdr,
                                 GError **error);

static void read_xwd_cols(FILE *ifp, L_XWDFILEHEADER *xwdhdr,
                          L_XWDCOLOR *xwdcolmap);

static gboolean write_xwd_cols(GOutputStream *output, L_XWDFILEHEADER *xwdhdr,
                               L_XWDCOLOR *colormap, GError **error);

static gint save_index(GOutputStream *output, GimpImage *image,
                       GimpDrawable *drawable, gboolean gray, GError **error);
static gint save_rgb(GOutputStream *output, GimpImage *image,
                     GimpDrawable *drawable, GError **error);
...
static void xwd_class_init(XwdClass *klass) {
...
static GimpProcedure *xwd_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, xwd_load, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Peter Kirchgessner",
                                   "Peter Kirchgessner", "1996");

    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-xwindowdump");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "xwd");
    gimp_file_procedure_set_magics(GIMP_FILE_PROCEDURE(procedure),
                                   "4,long,0x00000007");
  } else if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, xwd_save, NULL, NULL);
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  if (xwdhdr.l_colormap_entries > 256) {
    g_message(_("'%s':\nIllegal number of colormap entries: %ld"),
              gimp_file_get_utf8_name(file), (long)xwdhdr.l_colormap_entries);
    goto out;
  }
  ...
  case 0:                             /* Single plane bitmap */
    if ((depth == 1) && (bpp == 1)) { /* Can be performed by format 2 loader */
  ...
  if (!image && !(error && *error))
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                ...
                gimp_file_get_utf8_name(file), (gint)xwdhdr.l_pixmap_format,
                depth, bpp);
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  if (!output) {
    g_prefix_error(error, _("Could not open '%s' for writing: "),
                   gimp_file_get_utf8_name(file));
    return FALSE;
  }
  ...
  if (success && !g_output_stream_close(output, NULL, error)) {
    g_prefix_error(error, _("Error exporting '%s': "),
                   gimp_file_get_utf8_name(file));
    success = FALSE;
  } else if (!success) {
...
static L_CARD32 read_card32(FILE *ifp, int *err)
...
static L_CARD16 read_card16(FILE *ifp, int *err)
...
static L_CARD8 read_card8(FILE *ifp, int *err) {
  L_CARD8 c;
...
static gboolean write_card32(GOutputStream *output, L_CARD32 c,
                             GError **error) {
  guchar buffer[4];
  ...
  return g_output_stream_write_all(output, buffer, 4, NULL, NULL, error);
}

static gboolean write_card16(GOutputStream *output, L_CARD32 c,
                             GError **error) {
  guchar buffer[2];
  ...
  return g_output_stream_write_all(output, buffer, 2, NULL, NULL, error);
}
...
static gboolean write_card8(GOutputStream *output, L_CARD32 c, GError **error) {
  ...
  return g_output_stream_write_all(output, buffer, 1, NULL, NULL, error);
}
...
static void read_xwd_header(FILE *ifp, L_XWDFILEHEADER *xwdhdr) {
  ...
  for (j = 0; j < sizeof(L_XWDFILEHEADER) / sizeof(xwdhdr->l_file_version);
       j++) {
 ...
 */
static gint set_pixelmap(int ncols, L_XWDCOLOR *xwdcol, PIXEL_MAP *pixelmap)

{
  ...
  for (j = 0; j < pixelmap->npixel; j++)
    g_printf("Pixelvalue 0x%08lx, 0x%02x 0x%02x 0x%02x\n",
             pixelmap->pmap[j].pixel_val, pixelmap->pmap[j].red,
             pixelmap->pmap[j].green, pixelmap->pmap[j].blue);
...
static void init_color_table256(L_XWDFILEHEADER *xwdhdr, guchar *ColorMap)
...
static GimpImage *create_new_image(GFile *file, guint width, guint height,
  ...
  *layer = gimp_layer_new(image, "Background", width, height, gdtype, 100,
  ...
  for (i = 0; i < height; i++) {
    ...
    if ((scan_lines == tile_height) || ((i + 1) == height)) {
      gegl_buffer_set(buffer,
                      GEGL_RECTANGLE(0, i - scan_lines + 1, width, scan_lines),
                      0, NULL, data, GEGL_AUTO_ROWSTRIDE);
...
static GimpImage *load_xwd_f2_d8_b8(GFile *file, FILE *ifp,
  ...
  if ((xwdhdr->l_ncolors == 256) && (xwdhdr->l_colormap_entries == 256)) {
    for (j = 0; j < 256; j++) {
      if ((xwdcolmap[j].l_pixel != j) || ((xwdcolmap[j].l_red >> 8) != j) ||
  ...
  image = create_new_image(
      file, width, height, grayscale ? GIMP_GRAY : GIMP_INDEXED,
      grayscale ? GIMP_GRAY_IMAGE : GIMP_INDEXED_IMAGE, &layer, &buffer);
...
static GimpImage *load_xwd_f2_d16_b16(GFile *file, FILE *ifp,
  ...
  image = create_new_image(file, width, height, GIMP_RGB, GIMP_RGB_IMAGE,
                           &layer, &buffer);
  ...
  while (((1 << redshift) & redmask) == 0)
    redshift++;
  while (((1 << greenshift) & greenmask) == 0)
    greenshift++;
  while (((1 << blueshift) & bluemask) == 0)
    blueshift++;

  /* The bits_per_rgb may not be correct. Use redmask instead */
  maxred = 0;
  while (redmask >> (redshift + maxred))
    maxred++;
  maxred = (1 << maxred) - 1;

  maxgreen = 0;
  while (greenmask >> (greenshift + maxgreen))
    maxgreen++;
  maxgreen = (1 << maxgreen) - 1;

  maxblue = 0;
  while (bluemask >> (blueshift + maxblue))
    maxblue++;
  maxblue = (1 << maxblue) - 1;
...
static GimpImage *load_xwd_f2_d24_b32(GFile *file, FILE *ifp,
  ...
  if (maxred > sizeof(redmap) || maxgreen > sizeof(greenmap) ||
      maxblue > sizeof(bluemap)) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("XWD-file %s is corrupt."), gimp_file_get_utf8_name(file));
    return NULL;
  }
  ...
  if (xwdhdr->l_bits_per_pixel == 32) {
    for (i = 0; i < height; i++) {
      ...
      if ((scan_lines == tile_height) || ((i + 1) == height)) {
        gegl_buffer_set(
            buffer, GEGL_RECTANGLE(0, i - scan_lines + 1, width, scan_lines), 0,
...
static GimpImage *load_xwd_f2_d32_b32(GFile *file, FILE *ifp,
  ...
  image = create_new_image(file, width, height, GIMP_RGB, GIMP_RGBA_IMAGE,
                           &layer, &buffer);
  ...
  while (((1 << blueshift) & bluemask) == 0)
    blueshift++;
  while (((1 << alphashift) & alphamask) == 0)
    alphashift++;
  ...
  maxalpha = 0;
  while (alphamask >> (alphashift + maxalpha))
    maxalpha++;
...
static GimpImage *load_xwd_f1_d24_b1(GFile *file, FILE *ifp,
  ...
  image = create_new_image(
      file, width, height, indexed ? GIMP_INDEXED : GIMP_RGB,
      indexed ? GIMP_INDEXED_IMAGE : GIMP_RGB_IMAGE, &layer, &buffer);
  ...
  for (tile_start = 0; tile_start < height; tile_start += tile_height) {
    ...
    gegl_buffer_set(
        buffer, GEGL_RECTANGLE(0, tile_start, width, tile_end - tile_start + 1),
        0, NULL, data, GEGL_AUTO_ROWSTRIDE);
  }
...
static gboolean save_index(GOutputStream *output, GimpImage *image,
                           GimpDrawable *drawable, gboolean gray,
                           GError **error) {
  gint height, width;
  gint linepad;
  gint tile_height;
  ...
  src = data = g_new(guchar, tile_height * width *
  ...
  if (gray) {
    ...
    for (j = 0; j < ncolors; j++) {
      xwdcolmap[j].l_pixel = j;
      xwdcolmap[j].l_red = ((*cmap) << 8) | *cmap;
      cmap++;
      xwdcolmap[j].l_green = ((*cmap) << 8) | *cmap;
      cmap++;
      xwdcolmap[j].l_blue = ((*cmap) << 8) | *cmap;
      cmap++;
      xwdcolmap[j].l_flags = 7;
      xwdcolmap[j].l_pad = 0;
    }
  ...
  for (i = 0; i < height; i++) {
    ...
    {
      gint scan_lines =
          (i + tile_height - 1 < height) ? tile_height : (height - i);
    ...
    success = g_output_stream_write_all(output, src, width, NULL, NULL, error);
    if (!success)
      goto out;

    if (linepad) {
      success =
          g_output_stream_write_all(output, &tmp, linepad, NULL, NULL, error);
      if (!success)
        goto out;
    }
...
static gboolean save_rgb(GOutputStream *output, GimpImage *image,
  ...
  for (i = 0; i < height; i++) {
    ...
    success =
        g_output_stream_write_all(output, src, width * 3, NULL, NULL, error);
    if (!success)
      goto out;
