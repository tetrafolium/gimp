...
#define GIMP_RESOLUTION_ENTRY(obj)                                             \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), GIMP_TYPE_RESOLUTION_ENTRY,               \
                              GimpResolutionEntry))
#define GIMP_RESOLUTION_ENTRY_CLASS(klass)                                     \
  (G_TYPE_CHECK_CLASS_CAST((klass), GIMP_TYPE_RESOLUTION_ENTRY,                \
                           GimpResolutionEntryClass))
#define GIMP_IS_RESOLUTION_ENTRY(obj)                                          \
  (G_TYPE_CHECK_INSTANCE_TYPE(obj, GIMP_TYPE_RESOLUTION_ENTRY))
#define GIMP_IS_RESOLUTION_ENTRY_CLASS(klass)                                  \
  (G_TYPE_CHECK_CLASS_TYPE((klass), GIMP_TYPE_RESOLUTION_ENTRY))
#define GIMP_RESOLUTION_ENTRY_GET_CLASS(obj)                                   \
  (G_TYPE_INSTANCE_GET_CLASS((obj), GIMP_TYPE_RESOLUTION_ENTRY,                \
                             GimpResolutionEntryClass))
...
GtkWidget *gimp_resolution_entry_new(const gchar *width_label, gdouble width,
                                     const gchar *height_label, gdouble height,
                                     GimpUnit size_unit, const gchar *res_label,
                                     gdouble initial_res,
                                     GimpUnit initial_unit);

GtkWidget *gimp_resolution_entry_attach_label(GimpResolutionEntry *gre,
                                              const gchar *text, gint row,
                                              gint column, gfloat alignment);
...
static void gimp_resolution_entry_field_init(
    GimpResolutionEntry *gre, GimpResolutionEntryField *gref,
    GimpResolutionEntryField *corresponding, guint changed_signal,
    gdouble initial_val, GimpUnit initial_unit, gboolean size,
    gint spinbutton_width);

static void gimp_resolution_entry_format_label(GimpResolutionEntry *gre,
                                               GtkWidget *label, gdouble size);
...
static GQuark gimp_plugin_pdf_load_error_quark(void) {
...
static PdfLoadVals loadvals = {GIMP_PAGE_SELECTOR_TARGET_LAYERS,
                               100.00, /* 100 dpi   */
                               TRUE, NULL};
...
static GimpValueArray *pdf_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *pdf_load_thumb(GimpProcedure *procedure, GFile *file,
                                      gint size, const GimpValueArray *args,
                                      gpointer run_data);

static GimpImage *load_image(PopplerDocument *doc, GFile *file,
                             GimpRunMode run_mode,
                             GimpPageSelectorTarget target, guint32 resolution,
                             gboolean antialias, PdfSelectedPages *pages);

static GimpPDBStatusType load_dialog(PopplerDocument *doc,
                                     PdfSelectedPages *pages);

static PopplerDocument *open_document(GFile *file, const gchar *PDF_password,
                                      GimpRunMode run_mode, GError **error);

static cairo_surface_t *get_thumb_surface(PopplerDocument *doc, gint page,
                                          gint preferred_size);

static GdkPixbuf *get_thumb_pixbuf(PopplerDocument *doc, gint page,
                                   gint preferred_size);

static GimpLayer *layer_from_surface(GimpImage *image, const gchar *layer_name,
                                     gint position, cairo_surface_t *surface,
...
static GimpProcedure *pdf_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, pdf_load, NULL, NULL);

    gimp_procedure_set_menu_label(procedure, N_("Portable Document Format"));

    gimp_procedure_set_documentation(procedure, "Load file in PDF format",
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Nathan Summers, Lionel N.",
                                   "Nathan Summers, Lionel N.", "2005, 2017");
    ...
    GIMP_PROC_ARG_STRING(procedure, "pdf-password", "PDF password",
                         "The password to decrypt the encrypted PDF file", NULL,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "n-pages", "N pages",
                      "Number of pages to load (0 for all)", 0, G_MAXINT, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT32_ARRAY(procedure, "pages", "Pages",
                              "The pages to load in the expected order",
                              G_PARAM_READWRITE);
  } else if (!strcmp(name, LOAD_THUMB_PROC)) {
    procedure = gimp_thumbnail_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, pdf_load_thumb, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Nathan Summers",
                                   "Nathan Summers", "2005");
  }
...
static GimpValueArray *pdf_load(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_INTERACTIVE:
    ...
    doc = open_document(file, loadvals.PDF_password, run_mode, &error);
  ...
  case GIMP_RUN_NONINTERACTIVE:
    doc =
        open_document(file, GIMP_VALUES_GET_STRING(args, 0), run_mode, &error);
          ...
          for (i = 0; i < pages.n_pages; i++) {
            if (p[i] >= doc_n_pages) {
              ...
              g_set_error(&error, GIMP_PLUGIN_PDF_LOAD_ERROR, 0,
                           ...
                           */
                          ngettext("PDF document '%1$s' has %3$d page. Page "
                                   "%2$d is out of range.",
                                   "PDF document '%1$s' has %3$d pages. Page "
                                   "%2$d is out of range.",
                                   doc_n_pages),
                          gimp_file_get_utf8_name(file), p[i], doc_n_pages);
              break;
            } else {
              pages.pages[i] = p[i];
            }
  ...
  if (status == GIMP_PDB_SUCCESS) {
    image = load_image(doc, file, run_mode, loadvals.target,
                       loadvals.resolution, loadvals.antialias, &pages);
  }
...
static GimpValueArray *pdf_load_thumb(GimpProcedure *procedure, GFile *file,
  ...
  doc = open_document(file, NULL, GIMP_RUN_NONINTERACTIVE, &error);
  ...
  if (surface) {
    image = gimp_image_new(cairo_image_surface_get_width(surface),
                           cairo_image_surface_get_height(surface), GIMP_RGB);
...
static PopplerDocument *open_document(GFile *file, const gchar *PDF_password,
  ...
  if (run_mode == GIMP_RUN_INTERACTIVE) {
    ...
    label = gtk_label_new(
        _("PDF is password protected, please input the password:"));
    while (error && error->domain == POPPLER_ERROR &&
      ...
      dialog = gimp_dialog_new(_("Encrypted PDF"), PLUG_IN_ROLE, NULL, 0, NULL,
                               NULL, _("_Cancel"), GTK_RESPONSE_CANCEL,
                               _("_OK"), GTK_RESPONSE_OK, NULL);
      gimp_window_set_transient(GTK_WINDOW(dialog));
      vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 12);
      gtk_container_set_border_width(GTK_CONTAINER(vbox), 12);
      gtk_box_pack_start(
          GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), vbox, TRUE,
          TRUE, 0);
      entry = gtk_entry_new();
      ...
      if (run == GTK_RESPONSE_OK) {
        g_clear_error(&error);
        doc = poppler_document_new_from_gfile(
            file, gtk_entry_get_text(GTK_ENTRY(entry)), NULL, &error);
      }
  ...
  if (!doc) {
    g_set_error(load_error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
                _("Could not load '%s': %s"), gimp_file_get_utf8_name(file),
                error->message);
    g_error_free(error);
    return NULL;
  }
...
static GimpLayer *layer_from_surface(GimpImage *image, const gchar *layer_name,
  ...
  layer =
      gimp_layer_new_from_surface(image, layer_name, surface, progress_start,
...
static cairo_surface_t *render_page_to_surface(PopplerPage *page, int width,
...
static GimpImage *load_image(PopplerDocument *doc, GFile *file,
  ...
  for (i = 0; i < pages->n_pages; i++) {
    ...
    layer_from_surface(image, page_label, 0, surface, doc_progress,
                       1.0 / pages->n_pages);
...
static cairo_surface_t *get_thumb_surface(PopplerDocument *doc, gint page_num,
                                          gint preferred_size) {
  PopplerPage *page;
  cairo_surface_t *surface;
...
static GdkPixbuf *get_thumb_pixbuf(PopplerDocument *doc, gint page_num,
                                   gint preferred_size) {
  cairo_surface_t *surface;
  GdkPixbuf *pixbuf;
...
static gboolean idle_set_thumbnail(gpointer data) {
  ...
  gimp_page_selector_set_page_thumbnail(idle_data->selector, idle_data->page_no,
                                        idle_data->pixbuf);
...
static gpointer thumbnail_thread(gpointer data) {
  ...
  for (i = 0; i < n_pages; i++) {
    ...
    idle_data->pixbuf =
        get_thumb_pixbuf(thread_data->document, i, THUMBNAIL_SIZE);
...
static GimpPDBStatusType load_dialog(PopplerDocument *doc,
  ...
  dialog = gimp_dialog_new(_("Import from PDF"), PLUG_IN_ROLE, NULL, 0,
  ...
  resolution = gimp_resolution_entry_new(
      _("_Width (pixels):"), width, _("_Height (pixels):"), height,
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(antialias),
                               loadvals.antialias);
  gtk_box_pack_start(GTK_BOX(vbox), antialias, FALSE, FALSE, 0);

  g_signal_connect(antialias, "toggled", G_CALLBACK(gimp_toggle_button_update),
                   &loadvals.antialias);
  ...
  pages->pages = gimp_page_selector_get_selected_pages(
      GIMP_PAGE_SELECTOR(selector), &pages->n_pages);
...
GType gimp_resolution_entry_get_type(void) {
  ...
  if (!gre_type) {
    ...
    gre_type = g_type_register_static(GTK_TYPE_GRID, "GimpResolutionEntry",
                                      &gre_info, 0);
  }
...
static void gimp_resolution_entry_class_init(GimpResolutionEntryClass *klass) {
  ...
  gimp_resolution_entry_signals[HEIGHT_CHANGED] = g_signal_new(
      "height-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpResolutionEntryClass, value_changed), NULL, NULL,
      NULL, G_TYPE_NONE, 0);

  gimp_resolution_entry_signals[WIDTH_CHANGED] = g_signal_new(
      "width-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpResolutionEntryClass, value_changed), NULL, NULL,
      NULL, G_TYPE_NONE, 0);

  gimp_resolution_entry_signals[X_CHANGED] =
      g_signal_new("x-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpResolutionEntryClass, value_changed),
                   NULL, NULL, NULL, G_TYPE_NONE, 0);

  gimp_resolution_entry_signals[Y_CHANGED] =
      g_signal_new("y-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
...
static void gimp_resolution_entry_field_init(
  ...
  if (size) {
    gref->value = initial_val / gimp_unit_get_factor(initial_unit) *
                  corresponding->value * gimp_unit_get_factor(gre->unit);
  ...
  if (size) {
    gref->label =
        g_object_new(GTK_TYPE_LABEL, "xalign", 0.0, "yalign", 0.5, NULL);
    gimp_label_set_attributes(GTK_LABEL(gref->label), PANGO_ATTR_STYLE,
  ...
  gref->adjustment = gtk_adjustment_new(gref->value, gref->min_value,
                                        gref->max_value, 1.0, 10.0, 0.0);
  gref->spinbutton = gimp_spin_button_new(gref->adjustment, 1.0, digits);
...
GtkWidget *gimp_resolution_entry_new(const gchar *width_label, gdouble width,
  ...
  gimp_resolution_entry_field_init(gre, &gre->x, &gre->width, X_CHANGED,
                                   initial_res, initial_unit, FALSE, 0);

  gtk_grid_attach(GTK_GRID(gre), gre->x.spinbutton, 1, 3, 1, 1);

  g_signal_connect(gre->x.adjustment, "value-changed",
                   G_CALLBACK(gimp_resolution_entry_value_callback), &gre->x);
  ...
  g_object_set(model, "short-format", _("pixels/%a"), "long-format",
               _("pixels/%a"), NULL);
  ...
  g_signal_connect(gre->unitmenu, "changed",
                   G_CALLBACK(gimp_resolution_entry_unit_callback), gre);
  gtk_widget_show(gre->unitmenu);

  gimp_resolution_entry_field_init(gre, &gre->width, &gre->x, WIDTH_CHANGED,
                                   width, size_unit, TRUE, 0);
  ...
  gimp_resolution_entry_field_init(gre, &gre->height, &gre->x, HEIGHT_CHANGED,
                                   height, size_unit, TRUE, 0);
...
GtkWidget *gimp_resolution_entry_attach_label(GimpResolutionEntry *gre,
  ...
  if (column == 0) {
    ...
    for (list = children; list; list = g_list_next(list)) {
      ...
      gtk_container_child_get(GTK_CONTAINER(gre), child, "left-attach",
...
static void gimp_resolution_entry_update_value(GimpResolutionEntryField *gref,
  ...
  if (gref->size)
    gimp_resolution_entry_update_value(
        gref->corresponding,
        gref->value / gref->phy_size / gimp_unit_get_factor(gref->gre->unit));
  else {
    gdouble factor = gimp_unit_get_factor(gref->gre->unit);

    gimp_resolution_entry_update_value(
        &gref->gre->width, gref->value * gref->gre->width.phy_size * factor);

    gimp_resolution_entry_update_value(
        &gref->gre->height, gref->value * gref->gre->height.phy_size * factor);
  }
...
static void gimp_resolution_entry_update_unit(GimpResolutionEntry *gre,
  ...
  gimp_resolution_entry_format_label(gre, gre->width.label,
                                     gre->width.phy_size);
  gimp_resolution_entry_format_label(gre, gre->height.label,
                                     gre->height.phy_size);
...
static void gimp_resolution_entry_format_label(GimpResolutionEntry *gre,
                                               GtkWidget *label, gdouble size) {
  gchar *format =
      g_strdup_printf("%%.%df %%s", gimp_unit_get_digits(gre->unit));
  gchar *text = g_strdup_printf(format, size * gimp_unit_get_factor(gre->unit),
