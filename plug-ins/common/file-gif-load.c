...
static GimpValueArray *gif_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *gif_load_thumb(GimpProcedure *procedure, GFile *file,
                                      gint size, const GimpValueArray *args,
                                      gpointer run_data);

static GimpImage *load_image(GFile *file, gboolean thumbnail, GError **error);
...
static GimpProcedure *gif_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, gif_load, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "file format",
                                     "FIXME: write help for gif_load", name);
    ...
    procedure = gimp_thumbnail_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, gif_load_thumb, NULL, NULL);
...
static gboolean ReadImage(FILE *fd, GFile *file, gint len, gint height,
                          CMap cmap, gint ncols, gint format, gint interlace,
                          gint number, guint leftpos, guint toppos,
  ...
  while (TRUE) {
    ...
    if (!useGlobalColormap) {
      ...
      status = ReadImage(fd, file, LM_to_uint(buf[4], buf[5]),
                         ...
                         grayScale, BitSet(buf[8], INTERLACE), imageCount,
                         (guint)LM_to_uint(buf[0], buf[1]),
                         (guint)LM_to_uint(buf[2], buf[3]), GifScreen.Width,
                         GifScreen.Height, &image);
    } else {
      status = ReadImage(
          fd, file, LM_to_uint(buf[4], buf[5]), LM_to_uint(buf[6], buf[7]),
          GifScreen.ColorMap, GifScreen.BitPixel, GifScreen.GrayScale,
          BitSet(buf[8], INTERLACE), imageCount,
          (guint)LM_to_uint(buf[0], buf[1]), (guint)LM_to_uint(buf[2], buf[3]),
          GifScreen.Width, GifScreen.Height, &image);
    }
  ...
  */
  ...
  if (frame_number == 1) {
    ...
    *image = gimp_image_new(screenwidth, screenheight, GIMP_INDEXED);
    ...
    if (Gif89.delayTime < 0)
      ...
      framename =
          g_strdup_printf(_("Background (%d%s)"), 10 * Gif89.delayTime, "ms");

    previous_disposal = Gif89.disposal;

    if (Gif89.transparent == -1) {
      layer =
          gimp_layer_new(*image, framename, len, height, GIMP_INDEXED_IMAGE,
                         100, gimp_image_get_default_new_layer_mode(*image));
    } else {
      layer =
          gimp_layer_new(*image, framename, len, height, GIMP_INDEXEDA_IMAGE,
                         100, gimp_image_get_default_new_layer_mode(*image));
      alpha_frame = TRUE;
    }
  ...
  } else /* NOT FIRST FRAME */
  {
    gimp_progress_set_text_printf(_("Opening '%s' (frame %d)"),
                                  gimp_file_get_utf8_name(file), frame_number);
    gimp_progress_pulse();
    ...
    if (Gif89.delayTime < 0)
      ...
      framename = g_strdup_printf(_("Frame %d (%d%s)"), frame_number,
    ...
    layer =
        gimp_layer_new(*image, framename, len, height,
  ...
  if (len >
      (G_MAXSIZE / height / (alpha_frame ? (promote_to_rgb ? 4 : 2) : 1))) {
...
fini:
  ...
  gegl_buffer_set(buffer, GEGL_RECTANGLE(0, 0, len, height), 0, NULL, dest,
                  GEGL_AUTO_ROWSTRIDE);
