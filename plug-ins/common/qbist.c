...
static GimpValueArray *qbist_run(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void qbist(ExpInfo *info, gfloat *buffer, gint xp, gint yp, gint num,
                  gint width, gint height, gint oversampling);
...
static void qbist_class_init(QbistClass *klass) {
...
static void qbist_init(Qbist *qbist) {}
...
static GimpProcedure *qbist_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, qbist_run, NULL, NULL);
    ...
    gimp_procedure_set_attribution(
        procedure, "Jörn Loviscach, Jens Ch. Restemeier",
        "Jörn Loviscach, Jens Ch. Restemeier", PLUG_IN_VERSION);
  }
...
static GimpValueArray *qbist_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  iter = gegl_buffer_iterator_new(
      buffer, GEGL_RECTANGLE(0, 0, img_width, img_height), 0,
      babl_format("R'G'B'A float"), GEGL_ACCESS_READWRITE, GEGL_ABYSS_NONE, 1);
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (row = 0; row < roi.height; row++) {
      qbist(&qbist_info.info, data + row * roi.width * 4, roi.x, roi.y + row,
            roi.width, sel_width, sel_height, qbist_info.oversampling);
    }

    done_pixels += roi.width * roi.height;

    gimp_progress_update((gdouble)done_pixels / (gdouble)total_pixels);
  }
    ...
    for (yy = 0; yy < oversampling; yy++) {
      ...
      for (xx = 0; xx < oversampling; xx++) {
        for (i = 0; i < NUM_REGISTERS; i++) {
          if (used_reg_flag[i]) {
            reg[i][0] = ((gfloat)((gx + xp) * oversampling + xx)) /
                        ((gfloat)(width * oversampling));
            reg[i][1] = ((gfloat)(yp * oversampling + yy)) /
                        ((gfloat)(height * oversampling));
...
static void dialog_update_previews(GtkWidget *widget, gpointer data) {
  ...
  if (!fish)
    fish = babl_fish(babl_format("R'G'B'A float"), babl_format("R'G'B'A u8"));

  for (j = 0; j < 9; j++) {
    optimize(&info[(j + 5) % 9]);

    for (i = 0; i < PREVIEW_SIZE; i++) {
      qbist(&info[(j + 5) % 9], buf + i * PREVIEW_SIZE * 4, 0, i, PREVIEW_SIZE,
            PREVIEW_SIZE, PREVIEW_SIZE, 1);
    }

    babl_process(fish, buf, u8_buf, PREVIEW_SIZE * PREVIEW_SIZE);

    gimp_preview_area_draw(GIMP_PREVIEW_AREA(preview[j]), 0, 0, PREVIEW_SIZE,
                           PREVIEW_SIZE, GIMP_RGBA_IMAGE, u8_buf,
                           PREVIEW_SIZE * 4);
  }
}
...
static guint16 get_be16(guint8 *buf) { return (guint16)buf[0] << 8 | buf[1]; }
...
static void dialog_load(GtkWidget *widget, gpointer data) {
  ...
  dialog = gtk_file_chooser_dialog_new(
      _("Load QBE File"), GTK_WINDOW(parent), GTK_FILE_CHOOSER_ACTION_OPEN,
...
static void dialog_save(GtkWidget *widget, gpointer data) {
  ...
  dialog = gtk_file_chooser_dialog_new(
      _("Save as QBE File"), GTK_WINDOW(parent), GTK_FILE_CHOOSER_ACTION_SAVE,
...
static gboolean dialog_run(void) {
  ...
  dialog = gimp_dialog_new(_("G-Qbist"), PLUG_IN_ROLE, NULL, 0,
  ...
  for (i = 0; i < 9; i++) {
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(dialog_select_preview),
                     (gpointer) & (info[(i + 5) % 9]));
  ...
  g_signal_connect(button, "toggled", G_CALLBACK(dialog_toggle_antialaising),
                   NULL);
