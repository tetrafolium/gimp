...
static GimpImage *smooth_palette(GimpDrawable *drawable, GimpLayer **layer);
...
static struct {
  ...
  gint try_size;
  gboolean show_image;
} config = {256, 64, 50, 10000, TRUE};

static void palette_class_init(PaletteClass *klass) {
...
static void palette_init(Palette *palette) {}
...
static GimpProcedure *palette_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, palette_run, NULL, NULL);
    ...
    GIMP_PROC_ARG_INT(procedure, "width", "Widtg", "Widtg", 2,
                      GIMP_MAX_IMAGE_SIZE, 256, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "height", "Height", "Height", 2,
                      GIMP_MAX_IMAGE_SIZE, 64, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "n-tries", "N tries", "Search septh", 1, 1024,
                      50, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "show-image", "Show image", "Show image",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_VAL_IMAGE(procedure, "new-image", "New image", "Output image",
                        FALSE, G_PARAM_READWRITE);

    GIMP_PROC_VAL_LAYER(procedure, "new-layer", "New layer", "Output layer",
                        FALSE, G_PARAM_READWRITE);
  }
...
static gfloat pix_diff(gfloat *pal, guint bpp, gint i, gint j) {
  gfloat r = 0.f;
  guint k;
...
static void pix_swap(gfloat *pal, guint bpp, gint i, gint j) {
  guint k;
...
static GimpImage *smooth_palette(GimpDrawable *drawable, GimpLayer **layer) {
  ...
  new_image = gimp_image_new_with_precision(
      config.width, config.height, GIMP_RGB, GIMP_PRECISION_FLOAT_LINEAR);

  gimp_image_undo_disable(new_image);

  *layer = gimp_layer_new(new_image, _("Background"), config.width,
                          config.height, gimp_drawable_type(drawable), 100,
                          gimp_image_get_default_new_layer_mode(new_image));

  gimp_image_insert_layer(new_image, *layer, NULL, 0);

  if (!gimp_drawable_mask_intersect(drawable, &sel_x1, &sel_y1, &width,
                                    &height))
    return new_image;
  ...
  for (i = 0; i < psize; i++) {
    ...
    gegl_sampler_get(sampler, (gdouble)x, (gdouble)y, NULL, pal + i * bpp,
                     GEGL_ABYSS_NONE);
  }
  ...
  if (1) {
    ...
    for (try = 0; try < config.ntries; try ++) {
      ...
      for (i = 0; i < config.try_size; i++) {
        ...
        if (1 == (i0 - i1)) {
          as_is =
              (pix_diff(pal, bpp, i1 - 1, i1) + pix_diff(pal, bpp, i0, i0 + 1));
          swapd =
              (pix_diff(pal, bpp, i1 - 1, i0) + pix_diff(pal, bpp, i1, i0 + 1));
        } else if (1 == (i1 - i0)) {
          as_is =
              (pix_diff(pal, bpp, i0 - 1, i0) + pix_diff(pal, bpp, i1, i1 + 1));
          swapd =
              (pix_diff(pal, bpp, i0 - 1, i1) + pix_diff(pal, bpp, i0, i1 + 1));
        } else {
          as_is =
              (pix_diff(pal, bpp, i0, i0 + 1) + pix_diff(pal, bpp, i0, i0 - 1) +
               pix_diff(pal, bpp, i1, i1 + 1) + pix_diff(pal, bpp, i1, i1 - 1));
          swapd =
              (pix_diff(pal, bpp, i1, i0 + 1) + pix_diff(pal, bpp, i1, i0 - 1) +
               pix_diff(pal, bpp, i0, i1 + 1) + pix_diff(pal, bpp, i0, i1 - 1));
        }
        if (swapd < as_is) {
    ...
    for (i = 1; i < 4 * psize; i++) {
      ...
      as_is = (pix_diff(pal, bpp, i0 - 1, i0) + pix_diff(pal, bpp, i1, i1 + 1));
      swapd = (pix_diff(pal, bpp, i0 - 1, i1) + pix_diff(pal, bpp, i0, i1 + 1));
  ...
  gimp_drawable_update(GIMP_DRAWABLE(*layer), 0, 0, config.width,
                       config.height);
...
static gboolean dialog(GimpDrawable *drawable) {
  ...
  dlg = gimp_dialog_new(_("Smooth Palette"), PLUG_IN_ROLE, NULL, 0,
  ...
  sizeentry = gimp_coordinates_new(unit, "%a", TRUE, FALSE, 6,
                                   GIMP_SIZE_ENTRY_UPDATE_SIZE, FALSE, FALSE,

                                   _("_Width:"), config.width, xres, 2,
                                   GIMP_MAX_IMAGE_SIZE, 2, GIMP_MAX_IMAGE_SIZE,

                                   _("_Height:"), config.height, yres, 1,
                                   GIMP_MAX_IMAGE_SIZE, 1, GIMP_MAX_IMAGE_SIZE);
  ...
  gimp_grid_attach_aligned(GTK_GRID(sizeentry), 0, 2, _("_Search depth:"), 0.0,
                           0.5, spinbutton, 1);
  ...
  if (run) {
    config.width = gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(sizeentry), 0);
    config.height = gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(sizeentry), 1);
  }
