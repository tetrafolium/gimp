...
enum { DISPOSE_UNSPECIFIED, DISPOSE_COMBINE, DISPOSE_REPLACE };
...
static GimpValueArray *gif_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static GimpPDBStatusType sanity_check(GFile *file, GimpImage **image,
                                      GimpRunMode run_mode, GError **error);
static gboolean bad_bounds_dialog(void);

static gboolean save_dialog(GimpImage *image, GimpProcedure *procedure,
                            GObject *config);
...
static void gif_class_init(GifClass *klass) {
...
static GimpProcedure *gif_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, gif_save, NULL, NULL);
    ...
    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/gif");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "gif");

    GIMP_PROC_ARG_BOOLEAN(procedure, "interlace", "Interlace",
                          "Try to export as interlaced", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "loop", "Loop",
                          "(animated gif) loop infinitely", TRUE,
                          G_PARAM_READWRITE);
    ...
    GIMP_PROC_ARG_INT(procedure, "default-dispose", "Default dispoe",
    ...
    GIMP_PROC_ARG_BOOLEAN(procedure, "as-animation", "As animation",
                          "Export GIF as animation?", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "force-delay", "Force delay",
                          "(animated gif) Use specified delay for all frames",
                          FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(
        procedure, "force-dispose", "Force dispose",
        "(animated gif) Use specified disposal for all frames", FALSE,
        G_PARAM_READWRITE);

    GIMP_PROC_AUX_ARG_BOOLEAN(procedure, "save-comment", "Save comment",
    ...
    GIMP_PROC_AUX_ARG_STRING(procedure, "gimp-comment", "Comment",
                             _("Image comment"), gimp_get_default_comment(),
                             G_PARAM_READWRITE);
...
static GimpValueArray *gif_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  if (status == GIMP_PDB_SUCCESS) {
    ...
    case GIMP_RUN_WITH_LAST_VALS: {
      ...
      g_object_get(config, "as-animation", &as_animation, NULL);
 ...
 */
typedef gint (*ifunptr)(gint x, gint y);

static gint find_unused_ia_color(const guchar *pixels, gint numpixels,
                                 gint num_indices, gint *colors);

static void special_flatten_indexed_alpha(guchar *pixels, gint transparent,
                                          gint numpixels);

static gint colors_to_bpp(gint colors);
static gint bpp_to_colors(gint bpp);
static gint get_pixel(gint x, gint y);
static gint gif_next_pixel(ifunptr getpixel);
static void bump_pixel(void);

static gboolean gif_encode_header(GOutputStream *output, gboolean gif89,
                                  gint width, gint height, gint background,
                                  gint bpp, gint *red, gint *green, gint *blue,
                                  ifunptr get_pixel, GError **error);
static gboolean gif_encode_graphic_control_ext(GOutputStream *output,
                                               gint disposal, gint delay89,
                                               gint n_frames, gint width,
                                               gint height, gint transparent,
                                               gint bpp, ifunptr get_pixel,
                                               GError **error);
static gboolean gif_encode_image_data(GOutputStream *output, gint width,
                                      gint height, gint interlace, gint bpp,
                                      ifunptr get_pixel, gint offset_x,
                                      gint offset_y, GError **error);
static gboolean gif_encode_close(GOutputStream *output, GError **error);
static gboolean gif_encode_loop_ext(GOutputStream *output, guint n_loops,
                                    GError **error);
...
static gboolean compress(GOutputStream *output, gint init_bits,
                         ifunptr ReadValue, GError **error);
static gboolean no_compress(GOutputStream *output, gint init_bits,
                            ifunptr ReadValue, GError **error);
static gboolean rle_compress(GOutputStream *output, gint init_bits,
                             ifunptr ReadValue, GError **error);
static gboolean normal_compress(GOutputStream *output, gint init_bits,
                                ifunptr ReadValue, GError **error);

static gboolean put_byte(GOutputStream *output, guchar b, GError **error);
static gboolean put_word(GOutputStream *output, gint w, GError **error);
static gboolean put_string(GOutputStream *output, const gchar *s,
                           GError **error);
static gboolean output_code(GOutputStream *output, gint code, GError **error);
static gboolean cl_block(GOutputStream *output, GError **error);
static void cl_hash(glong hsize);

static void char_init(void);
static gboolean char_out(GOutputStream *output, gint c, GError **error);
static gboolean char_flush(GOutputStream *output, GError **error);
...
static gint parse_disposal_tag(const gchar *str, gint default_dispose) {
  gint offset = 0;
  gint length;
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  g_object_get(config, "interlace", &config_interlace, "loop", &config_loop,
               "default-delay", &config_default_delay, "default-dispose",
               &config_default_dispose, "force-delay",
               &config_use_default_delay, "force-dispose",
               &config_use_default_dispose, "as-animation",
               &config_as_animation, "save-comment", &config_save_comment,
               "gimp-comment", &config_comment, NULL);
  ...
  if (colors < 256) {
     ...
     * wasted indices.
     */
    liberalBPP = BitsPerPixel = colors_to_bpp(
        colors + ((drawable_type == GIMP_INDEXEDA_IMAGE) ? 1 : 0));
  } else {
    liberalBPP = BitsPerPixel = colors_to_bpp(256);
  ...
  if (!gif_encode_header(output, is_gif89, cols, rows, bgindex, BitsPerPixel,
                         Red, Green, Blue, get_pixel, error))
    return FALSE;
  ...
  for (list = layers, i = nlayers - 1; list && i >= 0;
    ...
    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, 0, cols, rows), 1.0, format,
    ...
    if ((drawable_type == GIMP_INDEXEDA_IMAGE) ||
      ...
      transparent = find_unused_ia_color(
          pixels, cols * rows, bpp_to_colors(colors_to_bpp(colors)), &colors);

      special_flatten_indexed_alpha(pixels, transparent, cols * rows);
    } else {
      transparent = -1;
    }
    ...
    if (is_gif89) {
      if (i > 0 && !config_use_default_dispose) {
        ...
        Disposal = parse_disposal_tag(layer_name, config_default_dispose);
        g_free(layer_name);
      } else {
      ...
      if (!gif_encode_graphic_control_ext(output, Disposal, Delay89, nlayers,
                                          cols, rows, transparent, useBPP,
                                          get_pixel, error))
        return FALSE;
    }

    if (!gif_encode_image_data(output, cols, rows,
                               (rows > 4) ? config_interlace : 0, useBPP,
                               get_pixel, offset_x, offset_y, error))
      return FALSE;
...
static gboolean save_dialog(GimpImage *image, GimpProcedure *procedure,
  ...
  /* Interlace toggle */
  toggle = gimp_prop_check_button_new(config, "interlace", _("_Interlace"));
  ...
  toggle =
      gimp_prop_check_button_new(config, "save-comment", _("Save c_omment"));
  ...
  g_object_bind_property(config, "save-comment", text_view, "sensitive",
                         G_BINDING_SYNC_CREATE);

#define MAX_COMMENT 240

  text_buffer = gimp_prop_text_buffer_new(config, "gimp-comment", MAX_COMMENT);
  ...
  if (animation_supported) {
    ...
    toggle =
        gimp_prop_check_button_new(config, "as-animation", _("As _animation"));
    gtk_frame_set_label_widget(GTK_FRAME(frame), toggle);
    ...
    g_object_bind_property(config, "as-animation", grid, "sensitive",
                           G_BINDING_SYNC_CREATE);

    /* Loop forever toggle */
    toggle = gimp_prop_check_button_new(config, "loop", _("_Loop forever"));
    gtk_grid_attach(GTK_GRID(grid), toggle, 0, 0, 2, 1);

    /* Default delay spin */
    hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1,
                             _("_Delay between frames where unspecified:"), 0.0,
                             0.5, hbox, 1);

    spin = gimp_prop_spin_button_new(config, "default-delay", 1, 10, 0);
    ...
    store = gimp_int_store_new(
        _("I don't care"), DISPOSE_UNSPECIFIED,
        _("Cumulative layers (combine)"), DISPOSE_COMBINE,
        _("One frame per layer (replace)"), DISPOSE_REPLACE, NULL);
    combo = gimp_prop_int_combo_box_new(config, "default-dispose",
                                        GIMP_INT_STORE(store));
    g_object_unref(store);

    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2,
                             _("_Frame disposal where unspecified"), 0.0, 0.5,
                             combo, 1);
...
static int colors_to_bpp(gint colors) {
  gint bpp;
...
static int bpp_to_colors(gint bpp) {
  gint colors;
...
static gboolean gif_encode_header(GOutputStream *output, gboolean gif89,
                                  gint GWidth, gint GHeight, gint Background,
                                  gint BitsPerPixel, gint Red[], gint Green[],
                                  gint Blue[], ifunptr get_pixel,
                                  GError **error) {
  gint B;
  gint RWidth, RHeight;
  gint Resolution;
   ...
   */
  if (!put_word(output, RWidth, error) || !put_word(output, RHeight, error))
    return FALSE;
  ...
  for (i = 0; i < ColorMapSize; i++) {
    if (!put_byte(output, Red[i], error) ||
        !put_byte(output, Green[i], error) || !put_byte(output, Blue[i], error))
      return FALSE;
  }
...
static gboolean
gif_encode_graphic_control_ext(GOutputStream *output, int Disposal, int Delay89,
                               int NumFramesInImage, int GWidth, int GHeight,
  ...
  if ((Transparent >= 0) || (NumFramesInImage > 1)) {
    ...
    if (!put_byte(output, Transparent, error) || !put_byte(output, 0, error))
      return FALSE;
  }
...
static gboolean gif_encode_image_data(GOutputStream *output, int GWidth,
                                      int GHeight, int GInterlace,
                                      int BitsPerPixel, ifunptr get_pixel,
  ...
  if (!put_word(output, LeftOfs, error) || !put_word(output, TopOfs, error) ||
      !put_word(output, Width, error) || !put_word(output, Height, error))
    return FALSE;
...
static gboolean gif_encode_loop_ext(GOutputStream *output, guint num_loops,
                                    GError **error) {
  return (put_byte(output, 0x21, error) && put_byte(output, 0xff, error) &&
          put_byte(output, 0x0b, error) &&
          put_string(output, "NETSCAPE2.0", error) &&
          put_byte(output, 0x03, error) && put_byte(output, 0x01, error) &&
          put_word(output, num_loops, error) && put_byte(output, 0x00, error));
...
static gboolean gif_encode_comment_ext(GOutputStream *output,
  ...
  return (put_byte(output, 0x21, error) && put_byte(output, 0xfe, error) &&
          put_byte(output, strlen(comment), error) &&
          put_string(output, comment, error) && put_byte(output, 0x00, error));
}
 ...
 */
static gboolean put_byte(GOutputStream *output, guchar b, GError **error) {
  return g_data_output_stream_put_byte(G_DATA_OUTPUT_STREAM(output), b, NULL,
                                       error);
}

static gboolean put_word(GOutputStream *output, gint w, GError **error) {
  return g_data_output_stream_put_int16(G_DATA_OUTPUT_STREAM(output), w, NULL,
                                        error);
}

static gboolean put_string(GOutputStream *output, const gchar *s,
                           GError **error) {
  return g_data_output_stream_put_string(G_DATA_OUTPUT_STREAM(output), s, NULL,
                                         error);
}
...
static gint maxmaxcode = (gint)1
                         << GIF_BITS; /* should NEVER generate this code */
...
static gulong masks[] = {0x0000, 0x0001, 0x0003, 0x0007, 0x000F, 0x001F,
...
static void cl_hash(glong hsize) /* reset code table */
{
 ...
 */
static void char_init(void) { a_count = 0; }
...
static gboolean char_flush(GOutputStream *output, GError **error) {
  if (a_count > 0) {
    ...
    if (!g_output_stream_write_all(output, accum, a_count, NULL, NULL, error))
      return FALSE;
