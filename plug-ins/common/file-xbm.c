...
static GimpValueArray *xbm_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *xbm_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static gboolean save_image(GFile *file, const gchar *prefix, gboolean save_mask,
                           GimpImage *image, GimpDrawable *drawable,
                           GObject *config, GError **error);
static gboolean save_dialog(GimpDrawable *drawable, GimpProcedure *procedure,
                            GObject *config);
...
static void xbm_class_init(XbmClass *klass) {
...
static GimpProcedure *xbm_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, xbm_load, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     name);
    gimp_procedure_set_attribution(procedure, "Gordon Matzigkeit",
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, xbm_save, NULL, NULL);
    ...
    GIMP_PROC_ARG_BOOLEAN(procedure, "save-comment", "Save comment",
                          _("Write a comment at the beginning of the file."),
                          FALSE, /* *NOT* gimp_export_comment() */
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "gimp-comment", "Comment",
                         "Image description (maximum 72 bytes)",
                         gimp_get_default_comment(), G_PARAM_READWRITE);

    gimp_procedure_set_argument_sync(procedure, "gimp-comment",
                                     GIMP_ARGUMENT_SYNC_PARASITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "x10-format", "X10 format",
                          "Export in X10 format", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "use-hot-spot", "Use hot spot",
                          "Write hotspot information", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "hot-spot-x", "Hot spot X",
                      "X coordinate of hotspot", 0, GIMP_MAX_IMAGE_SIZE, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "hot-spot-y", "Hot spot Y",
                      "Y coordinate of hotspot", 0, GIMP_MAX_IMAGE_SIZE, 0,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "prefix", "Prefix",
                         "Identifier prefix [determined from filename]",
                         "bitmap", G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "write-mask", "Write mask",
                          "Write extra mask file", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_STRING(procedure, "mask-suffix", "Mask suffix",
                         "Suffix of the mask file", "-mask", G_PARAM_READWRITE);
  }
...
static gchar *init_prefix(GFile *file, GObject *config) {
  gchar *filename;
  gchar *p, *prefix;
  gint len;
  ...
  g_object_set(config, "prefix", NULL, NULL);
  ...
  if (prefix) {
    ...
    g_object_set(config, "prefix", prefix, NULL);
  }
...
static GimpValueArray *xbm_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  if (run_mode == GIMP_RUN_INTERACTIVE) {
    ...
    if (parasite) {
      ...
      if (sscanf(gimp_parasite_data(parasite), "%i %i", &x, &y) == 2) {
        g_object_set(config, "use-hot-spot", TRUE, "hot-spot-x", x,
                     "hot-spot-y", y, NULL);
      }
  ...
  if (status == GIMP_PDB_SUCCESS) {
    ...
    g_object_get(config, "prefix", &prefix, "mask-suffix", &mask_ext,
                 "write-mask", &write_mask, NULL);
    ...
    if (!save_image(file, prefix, FALSE, image, drawables[0], G_OBJECT(config),
                    &error)

        ||

        (write_mask && !save_image(mask_file, mask_prefix, TRUE, image,
                                   drawables[0], G_OBJECT(config), &error))) {
...
static gint getval(gint c, gint base) {
...
/* Get a comment */
static gchar *fgetcomment(FILE *fp) {
  GString *str = NULL;
  gint comment, c;
...
static gint match(FILE *fp, const gchar *s) {
  gint c;
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  if (x_hot > 0 || y_hot > 0) {
    ...
    parasite = gimp_parasite_new("hot-spot", GIMP_PARASITE_PERSISTENT,
  ...
  layer =
      gimp_layer_new(image, _("Background"), width, height, GIMP_INDEXED_IMAGE,
  ...
  for (i = 0; i < height; i += tileheight) {
    ...
    gegl_buffer_set(buffer, GEGL_RECTANGLE(0, i, width, tileheight), 0, NULL,
...
static gboolean save_image(GFile *file, const gchar *prefix, gboolean save_mask,
  ...
  g_object_get(config, "save-comment", &config_save_comment, "gimp-comment",
               &config_comment, "x10-format", &config_x10_format,
               "use-hot-spot", &config_use_hot, "hot-spot-x", &config_x_hot,
               "hot-spot-y", &config_y_hot, NULL);
  ...
  if (config_use_hot) {
    if (!print(output, error, "#define %s_x_hot %d\n", prefix, config_x_hot) ||
        !print(output, error, "#define %s_y_hot %d\n", prefix, config_y_hot))
      goto fail;
  }
  ...
  if (!print(output, error, "static %s %s_bits[] = {\n  ",
  ...
  for (i = 0; i < height; i += tileheight) {
    ...
    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, i, width, tileheight), 1.0, NULL,
...
static gboolean save_dialog(GimpDrawable *drawable, GimpProcedure *procedure,
  ...
  /*  X10 format  */
  toggle =
      gimp_prop_check_button_new(config, "x10-format", _("_X10 format bitmap"));
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Identifier prefix:"), 0.0,
                           0.5, entry, 1);
  ...
  toggle =
      gimp_prop_check_button_new(config, "save-comment", _("_Write comment"));
  ...
  hint = g_object_new(GIMP_TYPE_HINT_BOX, "icon-name", GIMP_ICON_DIALOG_WARNING,
                      "hint",
                      _("Writing a comment will make the XBM "
  ...
  g_object_bind_property(config, "save-comment", grid, "sensitive",
                         G_BINDING_SYNC_CREATE);

  entry = gimp_prop_entry_new(config, "gimp-comment", MAX_COMMENT);
  gtk_widget_set_size_request(entry, 240, -1);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Comment:"), 0.0, 0.5, entry,
                           1);
  ...
  g_object_bind_property(config, "use-hot-spot", grid, "sensitive",
                         G_BINDING_SYNC_CREATE);

  spinbutton = gimp_prop_spin_button_new(config, "hot-spot-x", 1, 10, 0);
  gtk_spin_button_set_range(GTK_SPIN_BUTTON(spinbutton), 0,
                            gimp_drawable_width(drawable) - 1);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Hot spot _X:"), 0.0, 0.5,
                           spinbutton, 1);

  spinbutton = gimp_prop_spin_button_new(config, "hot-spot-y", 1, 10, 0);
  gtk_spin_button_set_range(GTK_SPIN_BUTTON(spinbutton), 0,
                            gimp_drawable_width(drawable) - 1);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Hot spot _Y:"), 0.0, 0.5,
                           spinbutton, 1);
  ...
  g_object_bind_property(config, "write-mask", grid, "sensitive",
                         G_BINDING_SYNC_CREATE);

  entry = gimp_prop_entry_new(config, "mask-suffix", MAX_MASK_EXT);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Mask file extension:"),
                           0.0, 0.5, entry, 1);
