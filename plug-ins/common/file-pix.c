...
static GimpValueArray *pix_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *pix_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static gboolean get_short(GInputStream *input, guint16 *value, GError **error);
static gboolean put_short(GOutputStream *output, guint16 value, GError **error);

G_DEFINE_TYPE(Pix, pix, GIMP_TYPE_PLUG_IN)

GIMP_MAIN(PIX_TYPE)

static void pix_class_init(PixClass *klass) {
...
static GimpProcedure *pix_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, pix_load, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "Pix file format",
                                     name);
    gimp_procedure_set_attribution(procedure, "Michael Taylor",
                                   "Michael Taylor", "1997");
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, pix_save, NULL, NULL);
...
static gboolean get_short(GInputStream *input, guint16 *value, GError **error) {
  ...
  if (!g_input_stream_read_all(input, buf, 2, &bytes_read, NULL, error) ||
      bytes_read != 2) {
    return FALSE;
  }
...
static gboolean put_short(GOutputStream *output, guint16 value,
                          GError **error) {
  guchar buf[2];
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  if (!get_short(input, &width, error) || !get_short(input, &height, error) ||
  ...
  if (depth == 24) {
    ...
    for (i = 0; i < height;) {
      for (dest = dest_base, row = 0; row < tile_height && i < height;
           i++, row++) {
        guchar record[4];
        gsize bytes_read;
        guchar count;
        ...
        j = 0;
        while (j < width) {
          if (!g_input_stream_read_all(input, record, 4, &bytes_read, NULL,
                                       error) ||
              bytes_read != 4)
            break;
      ...
      gegl_buffer_set(buffer, GEGL_RECTANGLE(0, i - row, width, row), 0, NULL,
                      dest_base, GEGL_AUTO_ROWSTRIDE);
    ...
    for (i = 0; i < height;) {
      for (dest = dest_base, row = 0; row < tile_height && i < height;
        ...
        j = 0;
        while (j < width) {
          if (!g_input_stream_read_all(input, record, 2, &bytes_read, NULL,
                                       error) ||
              bytes_read != 2)
            break;
...
static gboolean save_image(GFile *file, GimpImage *image,
  ...
  if (!put_short(output, width, error) || !put_short(output, height, error) ||
      !put_short(output, 0, error) || !put_short(output, 0, error)) {
  ...
  if (savingColor) {
    ...
    for (i = 0; i < height;) {
      rectHeight = (tile_height < (height - i)) ? tile_height : (height - i);

      gegl_buffer_get(buffer, GEGL_RECTANGLE(0, i, width, rectHeight), 1.0,
                      format, src_base, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);

      for (src = src_base, row = 0; row < tile_height && i < height;
           i += 1, row += 1) {
        ...
        src += depth;
        for (j = 1; j < width; ++j) {
          if ((record[3] != src[0]) || (record[2] != src[1]) ||
              (record[1] != src[2]) || (record[0] == 255)) {
            /* Write current RLE record and start a new one */

            if (!g_output_stream_write_all(output, record, 4, NULL, NULL,
                                           error)) {
              goto fail;
            }
        ...
        if (!g_output_stream_write_all(output, record, 4, NULL, NULL, error)) {
          goto fail;
        }
      }
      ...
      for (src = src_base, row = 0; row < tile_height && i < height;
        ...
        for (j = 1; j < width; ++j) {
          if ((record[1] != src[0]) || (record[0] == 255)) {
            ...
            if (!g_output_stream_write_all(output, record, 2, NULL, NULL,
                                           error)) {
              goto fail;
            }
        ...
        if (!g_output_stream_write_all(output, record, 2, NULL, NULL, error)) {
          goto fail;
        }
      }
