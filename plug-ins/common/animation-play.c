...
typedef enum { DISPOSE_COMBINE = 0x00, DISPOSE_REPLACE = 0x01 } DisposeType;
...
static GimpValueArray *play_run(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void da_size_callback(GtkWidget *widget, GtkAllocation *allocation,
                             gpointer data);

static void sda_realize_callback(GtkWidget *widget, gpointer data);
static void sda_size_callback(GtkWidget *widget, GtkAllocation *allocation,
                              gpointer data);
...
static void framecombo_changed(GtkWidget *combo, gpointer data);
static void speedcombo_changed(GtkWidget *combo, gpointer data);
static void fpscombo_changed(GtkWidget *combo, gpointer data);
static void zoomcombo_activated(GtkEntry *combo, gpointer data);
static void zoomcombo_changed(GtkWidget *combo, gpointer data);
static gboolean repaint_sda(GtkWidget *darea, cairo_t *cr, gpointer data);
static gboolean repaint_da(GtkWidget *darea, cairo_t *cr, gpointer data);
...
static gboolean is_disposal_tag(const gchar *str, DisposeType *disposal,
                                gint *taglength);
static gboolean is_ms_tag(const gchar *str, gint *duration, gint *taglength);
...
static AnimationSettings settings = {
    3, DISPOSE_COMBINE, 100 /* ms */
};

static GimpImage *frames_image = NULL;

static void play_class_init(PlayClass *klass) {
...
static void play_init(Play *play) {}
...
static GimpProcedure *play_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, play_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     N_("Preview a GIMP layer-based "
                                        "animation"),
                                     "", name);
    gimp_procedure_set_attribution(procedure, "Adam D. Moss <adam@gimp.org>",
...
static GimpValueArray *play_run(GimpProcedure *procedure, GimpRunMode run_mode,
                                GimpImage *_image, GimpDrawable *drawable,
                                const GimpValueArray *args, gpointer run_data) {
  INIT_I18N();
  gegl_init(NULL, NULL);
...
static void sda_realize_callback(GtkWidget *widget, gpointer data) {
  ...
  cursor =
      gdk_cursor_new_for_display(gtk_widget_get_display(widget), GDK_HAND2);
...
static gboolean shape_pressed(GtkWidget *widget, GdkEventButton *event) {
...
static gboolean shape_motion(GtkWidget *widget, GdkEventMotion *event) {
  GdkModifierType mask;
  gint xp, yp;

  gdk_window_get_device_position(
      root_win, gdk_event_get_device((GdkEvent *)event), &xp, &yp, &mask);
...
static void close_callback(GtkAction *action, gpointer data) {
  gtk_widget_destroy(GTK_WIDGET(data));
}

static void help_callback(GtkAction *action, gpointer data) {
...
static GtkUIManager *ui_manager_new(GtkWidget *window) {
  static GtkActionEntry actions[] = {
      {"step-back", "media-skip-backward", N_("Step _back"), "d",
       N_("Step back to previous frame"), G_CALLBACK(step_back_callback)},

      {"step", "media-skip-forward", N_("_Step"), "f", N_("Step to next frame"),
       G_CALLBACK(step_callback)},

      {"rewind", "media-seek-backward", NULL, NULL, N_("Rewind the animation"),
       G_CALLBACK(rewind_callback)},

      {"refresh", GIMP_ICON_VIEW_REFRESH, NULL, "<control>R",
       N_("Reload the image"), G_CALLBACK(refresh_callback)},

      {"help", "help-browser", NULL, NULL, NULL, G_CALLBACK(help_callback)},

      {"close", "window-close", NULL, "<control>W", NULL,
       G_CALLBACK(close_callback)},
      {"quit", "application-quit", NULL, "<control>Q", NULL,
       G_CALLBACK(close_callback)},
      {"speed-up", NULL, N_("Faster"), "<control>L",
       N_("Increase the speed of the animation"),
       G_CALLBACK(speed_up_callback)},
      {"speed-down", NULL, N_("Slower"), "<control>J",
       N_("Decrease the speed of the animation"),
       G_CALLBACK(speed_down_callback)},
      {"speed-reset", NULL, N_("Reset speed"), "<control>K",
       N_("Reset the speed of the animation"),
       G_CALLBACK(speed_reset_callback)}};

  static GtkToggleActionEntry toggle_actions[] = {
      {"play", "media-playback-start", NULL, "space", N_("Start playback"),
       G_CALLBACK(play_callback), FALSE},

      {"detach", GIMP_ICON_DETACH, N_("Detach"), NULL,
  ...
  gtk_action_group_add_toggle_actions(group, toggle_actions,
...
static void refresh_dialog(gchar *imagename) {
  ...
  if (window_width + 50 > workarea.width ||
    ...
    guint expected_drawing_area_width =
        MAX(1, width - window_width + workarea.width);
    guint expected_drawing_area_height =
        MAX(1, height - window_height + workarea.height);
    gdouble expected_scale =
        MIN((gdouble)expected_drawing_area_width / (gdouble)width,
            (gdouble)expected_drawing_area_height / (gdouble)height);

    update_scale(expected_scale);

    /* There is unfortunately no good way to know the size of the decorations,
     * taskbars, etc. So we take a wild guess by making the window slightly
     * smaller to fit into any case. */
    gtk_window_set_default_size(
        GTK_WINDOW(window),
        MIN(expected_drawing_area_width + 20, workarea.width - 60),
        MIN(expected_drawing_area_height + 90, workarea.height - 60));
...
static void build_dialog(gchar *imagename) {
  ...
  g_signal_connect(window, "popup-menu", G_CALLBACK(popup_menu), NULL);
  ...
  item = GTK_TOOL_ITEM(
      gtk_ui_manager_get_widget(ui_manager, "/anim-play-toolbar/space"));
  ...
  /* I add the drawing area inside an alignment box to prevent it from being
   * resized. */
  ...
  g_signal_connect(drawing_area, "size-allocate", G_CALLBACK(da_size_callback),
                   NULL);
  g_signal_connect(drawing_area, "button-press-event", G_CALLBACK(button_press),
                   NULL);
  ...
  g_signal_connect(GTK_ENTRY(gtk_bin_get_child(GTK_BIN(zoomcombo))), "activate",
                   G_CALLBACK(zoomcombo_activated), NULL);
  g_signal_connect(zoomcombo, "changed", G_CALLBACK(zoomcombo_changed), NULL);
  ...
  g_signal_connect(fpscombo, "changed", G_CALLBACK(fpscombo_changed), NULL);
  ...
  for (index = 0; index < 7; index++) {
    text =
        g_strdup_printf("%g\303\227", (100 / get_duration_factor(index)) / 100);
  ...
  g_signal_connect(speedcombo, "changed", G_CALLBACK(speedcombo_changed), NULL);

  gimp_help_set_help_data(speedcombo, _("Playback speed"), NULL);

  action =
      gtk_ui_manager_get_action(ui_manager, "/anim-play-popup/speed-reset");
  ...
  g_signal_connect(frame_disposal_combo, "changed",
                   G_CALLBACK(framecombo_changed), NULL);
  ...
  g_signal_connect(shape_window, "realize", G_CALLBACK(sda_realize_callback),
                   NULL);
  g_signal_connect(shape_window, "size-allocate", G_CALLBACK(sda_size_callback),
                   NULL);
  ...
  g_object_set_data(G_OBJECT(shape_window), "cursor-offset",
                    g_new0(CursorOffset, 1));

  g_signal_connect(drawing_area, "draw", G_CALLBACK(repaint_da), NULL);

  g_signal_connect(shape_window, "draw", G_CALLBACK(repaint_sda), NULL);
...
static void init_frames(void) {
  ...
  for (iter = layers, i = 0; iter; iter = g_list_next(iter), i++) {
    ...
    new_frame =
        gimp_image_merge_visible_layers(frames_image, GIMP_CLIP_TO_IMAGE);
    frames[i] = new_frame;
  ...
  action = gtk_ui_manager_get_action(ui_manager, "/ui/anim-play-toolbar/play");
  gtk_action_set_sensitive(action, animated);

  action =
      gtk_ui_manager_get_action(ui_manager, "/ui/anim-play-toolbar/step-back");
  gtk_action_set_sensitive(action, animated);

  action = gtk_ui_manager_get_action(ui_manager, "/ui/anim-play-toolbar/step");
  gtk_action_set_sensitive(action, animated);

  action =
      gtk_ui_manager_get_action(ui_manager, "/ui/anim-play-toolbar/rewind");
  gtk_action_set_sensitive(action, animated);
...
static void show_frame(void) {
  ...
  gtk_progress_bar_set_fraction(
      ...
      ((gfloat)frame_number / (gfloat)(total_frames - 0.999)));
...
static gint advance_frame_callback(gpointer data) {
  gdouble duration;
...
static void play_callback(GtkToggleAction *action) {
  ...
  g_object_set(action, "tooltip",
               playing ? _("Stop playback") : _("Start playback"), NULL);
}
...
static gdouble get_scale(gint index) {
  ...
  default: {
     ...
     * Try a text conversion, locale-aware in such a case, assuming people write
     * in percent. */
    gchar *active_text =
        gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(zoomcombo));
...
static void step_back_callback(GtkAction *action) {
  if (playing)
    gtk_action_activate(
        gtk_ui_manager_get_action(ui_manager, "/anim-play-toolbar/play"));
  do_back_step();
  show_frame();
}
...
static void refresh_callback(GtkAction *action) { initialize(); }
...
static void speed_up_callback(GtkAction *action) {
  ...
  action = gtk_ui_manager_get_action(ui_manager, "/anim-play-popup/speed-down");
...
static void speed_down_callback(GtkAction *action) {
  ...
  action = gtk_ui_manager_get_action(ui_manager, "/anim-play-popup/speed-up");
  gtk_action_set_sensitive(action, TRUE);
...
static void update_scale(gdouble scale) {
  ...
  gtk_widget_set_size_request(drawing_area, expected_drawing_area_width,
                              expected_drawing_area_height);
  gtk_widget_set_size_request(shape_window, expected_drawing_area_width,
                              expected_drawing_area_height);
...
static void zoomcombo_changed(GtkWidget *combo, gpointer data) {
  ...
  /* If no index, user is probably editing by hand. We wait for him to click
   * "Enter". */
  if (index != -1)
...
static gboolean is_ms_tag(const gchar *str, gint *duration, gint *taglength) {
  ...
  if (length - offset <= 2 || g_ascii_toupper(str[offset]) != 'M' ||
