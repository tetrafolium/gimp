...
static GimpValueArray *wmf_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *wmf_load_thumb(GimpProcedure *procedure, GFile *file,
                                      gint size, const GimpValueArray *args,
                                      gpointer run_data);

static GimpImage *load_image(GFile *file, GError **error);
static gboolean load_wmf_size(GFile *file, WmfLoadVals *vals);
static gboolean load_dialog(GFile *file);
static guchar *wmf_get_pixbuf(GFile *file, gint *width, gint *height);
static guchar *wmf_load_file(GFile *file, guint *width, guint *height,
                             GError **error);
...
static void wmf_class_init(WmfClass *klass) {
...
static GimpProcedure *wmf_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, wmf_load, NULL, NULL);

    gimp_procedure_set_menu_label(procedure, N_("Microsoft WMF file"));

    gimp_procedure_set_documentation(
        procedure, "Loads files in the WMF file format",
        "Loads files in the WMF file format", name);
    gimp_procedure_set_attribution(
        procedure, "Dom Lachowicz <cinamod@hotmail.com>",
        "Dom Lachowicz <cinamod@hotmail.com>", "(c) 2003 - Version 0.3.0");
    ...
    GIMP_PROC_ARG_DOUBLE(procedure, "resolution", "Resolution",
                         "Resolution to use for rendering the WMF",
                         GIMP_MIN_RESOLUTION, GIMP_MAX_RESOLUTION,
                         WMF_DEFAULT_RESOLUTION, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "width", "Width",
                      "Width (in pixels) to load the WMF in, "
                      "0 for original width",
                      0, GIMP_MAX_IMAGE_SIZE, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "height", "Height",
                      "Height (in pixels) to load the WMF in, "
                      "0 for original height",
                      0, GIMP_MAX_IMAGE_SIZE, 0, G_PARAM_READWRITE);
  } else if (!strcmp(name, LOAD_THUMB_PROC)) {
    procedure = gimp_thumbnail_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, wmf_load_thumb, NULL, NULL);

    gimp_procedure_set_documentation(
        procedure, "Loads a small preview from a WMF image", "", name);
    gimp_procedure_set_attribution(
        procedure, "Dom Lachowicz <cinamod@hotmail.com>",
        "Dom Lachowicz <cinamod@hotmail.com>", "(c) 2003 - Version 0.3.0");
  }
...
static GimpValueArray *wmf_load_thumb(GimpProcedure *procedure, GFile *file,
  ...
  if (load_wmf_size(file, &load_vals) && load_vals.width > 0 &&
      load_vals.height > 0) {
...
static gboolean load_wmf_size(GFile *file, WmfLoadVals *vals) {
  ...
  err = wmf_mem_open(API, (guchar *)g_mapped_file_get_contents(mapped),
  ...
  err = wmf_display_size(API, &width, &height, vals->resolution,
                         vals->resolution);
...
static void load_dialog_resolution_callback(GimpSizeEntry *res, GFile *file) {
...
static void wmf_preview_callback(GtkWidget *widget, GtkAllocation *allocation,
                                 guchar *pixels) {
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(widget), 0, 0, allocation->width,
                         allocation->height, GIMP_RGBA_IMAGE, pixels,
                         allocation->width * 4);
}
...
static gboolean load_dialog(GFile *file) {
  ...
  WmfLoadVals vals = {WMF_DEFAULT_RESOLUTION, -WMF_PREVIEW_SIZE,
                      -WMF_PREVIEW_SIZE};

  gimp_ui_init(PLUG_IN_BINARY);

  dialog = gimp_dialog_new(_("Render Windows Metafile"), PLUG_IN_ROLE, NULL, 0,
  ...
  g_signal_connect(image, "size-allocate", G_CALLBACK(wmf_preview_callback),
                   pixels);
  ...
  size = GIMP_SIZE_ENTRY(gimp_size_entry_new(1, GIMP_UNIT_PIXEL, "%a", TRUE,
                                             FALSE, FALSE, 10,
  ...
  gimp_size_entry_set_refval_boundaries(size, 0, GIMP_MIN_IMAGE_SIZE,
                                        GIMP_MAX_IMAGE_SIZE);
  gimp_size_entry_set_refval_boundaries(size, 1, GIMP_MIN_IMAGE_SIZE,
                                        GIMP_MAX_IMAGE_SIZE);
  ...
  g_signal_connect(size, "value-changed", G_CALLBACK(load_dialog_size_callback),
                   NULL);
  ...
  xadj = gtk_adjustment_new(
      ratio_x, (gdouble)GIMP_MIN_IMAGE_SIZE / (gdouble)wmf_width,
      (gdouble)GIMP_MAX_IMAGE_SIZE / (gdouble)wmf_width, 0.01, 0.1, 0);
  ...
  g_signal_connect(xadj, "value-changed",
                   G_CALLBACK(load_dialog_ratio_callback), NULL);
  ...
  yadj = gtk_adjustment_new(
      ratio_y, (gdouble)GIMP_MIN_IMAGE_SIZE / (gdouble)wmf_height,
      (gdouble)GIMP_MAX_IMAGE_SIZE / (gdouble)wmf_height, 0.01, 0.1, 0);
  ...
  gimp_help_set_help_data(
      gimp_chain_button_get_button(GIMP_CHAIN_BUTTON(constrain)),
  ...
  res = gimp_size_entry_new(1, GIMP_UNIT_INCH, _("pixels/%a"), FALSE, FALSE,
                            FALSE, 10, GIMP_SIZE_ENTRY_UPDATE_RESOLUTION);
  ...
  gimp_size_entry_set_refval_boundaries(GIMP_SIZE_ENTRY(res), 0, 5.0,
                                        GIMP_MAX_RESOLUTION);
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(res), 0, load_vals.resolution);

  g_signal_connect(res, "value-changed",
                   G_CALLBACK(load_dialog_resolution_callback), file);
...
static guchar *pixbuf_gd_convert(const gint *gd_pixels, gint width,
                                 gint height) {
...
static guchar *wmf_get_pixbuf(GFile *file, gint *width, gint *height) {
  ...
  err = wmf_display_size(API, &file_width, &file_height, WMF_DEFAULT_RESOLUTION,
                         WMF_DEFAULT_RESOLUTION);
...
static guchar *wmf_load_file(GFile *file, guint *width, guint *height,
  ...
  *width = *height = -1;
  ...
  err = wmf_display_size(API, width, height, load_vals.resolution,
                         load_vals.resolution);
  if (err != wmf_E_None || *width <= 0 || *height <= 0)
    goto _wmf_error;
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  layer =
      gimp_layer_new(image, _("Rendered WMF"), width, height, GIMP_RGBA_IMAGE,
                     100, gimp_image_get_default_new_layer_mode(image));

  buffer = gimp_drawable_get_buffer(GIMP_DRAWABLE(layer));

  gegl_buffer_set(buffer, GEGL_RECTANGLE(0, 0, width, height), 0,
                  babl_format("R'G'B'A u8"), pixels, GEGL_AUTO_ROWSTRIDE);
