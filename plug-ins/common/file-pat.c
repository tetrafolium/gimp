...
static GimpValueArray *pat_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void pat_class_init(PatClass *klass) {
...
static GimpProcedure *pat_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, pat_save, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     SAVE_PROC);
    gimp_procedure_set_attribution(procedure, "Tim Newsome", "Tim Newsome",
                                   "1997");

    gimp_file_procedure_set_mime_types(GIMP_FILE_PROCEDURE(procedure),
                                       "image/x-gimp-pat");
    gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "pat");
    gimp_file_procedure_set_handles_remote(GIMP_FILE_PROCEDURE(procedure),
                                           TRUE);

    GIMP_PROC_ARG_STRING(procedure, "description", "Description",
                         "Short description of the pattern", "GIMP Pattern",
...
static GimpValueArray *pat_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "PAT",
  ...
  if (status == GIMP_PDB_SUCCESS) {
    ...
    save_retvals = gimp_pdb_run_procedure(
        gimp_get_pdb(), "file-pat-save-internal", GIMP_TYPE_RUN_MODE,
