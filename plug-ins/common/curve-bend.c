...
#define GRAPH_MASK                                                             \
  GDK_EXPOSURE_MASK | GDK_POINTER_MOTION_MASK | GDK_POINTER_MOTION_HINT_MASK | \
      GDK_ENTER_NOTIFY_MASK | GDK_BUTTON_PRESS_MASK |                          \
...
struct _BenderValues {
  guchar curve[2]
              [256]; /* for curve_type freehand mode   0   <= curve  <= 255 */
  gdouble points[2][17]
                [2]; /* for curve_type smooth mode     0.0 <= points <= 1.0 */
...
static void bender_plot_curve(BenderDialog *, int, int, int, int, gint32,
                              gint32, gint);
...
static gint bender_graph_draw(GtkWidget *, cairo_t *, BenderDialog *);
...
static void p_init_gdrw(t_GDRW *gdrw, GimpDrawable *drawable, int shadow);
static void p_end_gdrw(t_GDRW *gdrw);
static GimpLayer *p_main_bend(BenderDialog *cd, GimpDrawable *, gint);
static GimpImage *p_create_pv_image(GimpDrawable *src_drawable,
                                    GimpLayer **layer);
static void p_render_preview(BenderDialog *cd, GimpLayer *layer);
...
static void p_bender_calculate_iter_curve(BenderDialog *cd, gint32 xmax,
                                          gint32 ymax);
static void p_delta_gint32(gint32 *val, gint32 val_from, gint32 val_to,
                           gint32 total_steps, gdouble current_step);
static void p_copy_points(BenderDialog *cd, int outline, int xy, int argc,
                          const gdouble *floatarray);
static void p_copy_yval(BenderDialog *cd, int outline, int argc,
...
static void bender_class_init(BenderClass *klass) {
...
static void bender_init(Bender *bender) {}
...
static GimpProcedure *bender_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, bender_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(
        procedure, N_("Bend the image using two control curves"),
    ...
    GIMP_PROC_ARG_BOOLEAN(procedure, "smoothing", "Smoothing", "Smoothing",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "antialias", "Antialias", "Antialias",
                          TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "work-on-copy", "Work on copy",
                          "Copy the drawable and bend the copy", FALSE,
                          G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "curve-type", "Curve type",
                      "{ 0 == smooth (use 17 points), "
                      "1 == freehand (use 256 val_y) }",
                      0, 1, 0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "argc-upper-point-x", "Argc upper point X",
                      "Argc upper point X", 2, 17, 2, G_PARAM_READWRITE);

    GIMP_PROC_ARG_FLOAT_ARRAY(procedure, "upper-point-x", "Upper point X",
                              "Array of 17 x point coords "
                              "{ 0.0 <= x <= 1.0 or -1 for unused point }",
                              G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "argc-upper-point-y", "Argc upper point Y",
                      "Argc upper point Y", 2, 17, 2, G_PARAM_READWRITE);

    GIMP_PROC_ARG_FLOAT_ARRAY(procedure, "upper-point-y", "Upper point Y",
                              "Array of 17 y point coords "
                              "{ 0.0 <= y <= 1.0 or -1 for unused point }",
                              G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "argc-lower-point-x", "Argc lower point X",
                      "Argc lower point X", 2, 17, 2, G_PARAM_READWRITE);

    GIMP_PROC_ARG_FLOAT_ARRAY(procedure, "lower-point-x", "Lower point X",
                              "Array of 17 x point coords "
                              "{ 0.0 <= x <= 1.0 or -1 for unused point }",
                              G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "argc-lower-point-y", "Argc lower point Y",
                      "Argc lower point Y", 2, 17, 2, G_PARAM_READWRITE);

    GIMP_PROC_ARG_FLOAT_ARRAY(procedure, "lower-point-y", "Lower point Y",
                              "Array of 17 y point coords "
                              "{ 0.0 <= y <= 1.0 or -1 for unused point }",
                              G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "argc-upper-val-y", "Argc upper val Y",
                      "Argc upper val Y", 256, 256, 256, G_PARAM_READWRITE);

    GIMP_PROC_ARG_UINT8_ARRAY(procedure, "upper-val-y", "Upper val Y",
                              "Array of 256 y freehand coords "
                              "{ 0 <= y <= 255 }",
                              G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "argc-lower-val-y", "Argc lower val Y",
                      "Argc lower val Y", 256, 256, 256, G_PARAM_READWRITE);

    GIMP_PROC_ARG_UINT8_ARRAY(procedure, "lower-val-y", "Lower val Y",
                              "Array of 256 y freehand coords "
                              "{ 0 <= y <= 255 }",
                              G_PARAM_READWRITE);

    GIMP_PROC_VAL_LAYER(procedure, "bent-layer", "Bent layer",
                        "The transformed layer", FALSE, G_PARAM_READWRITE);
  }
...
static GimpItem *p_gimp_rotate(GimpImage *image, GimpDrawable *drawable,
                               gint32 interpolation, gdouble angle_deg) {
  gdouble angle_rad;
  GimpItem *rc;
  ...
  rc = gimp_item_transform_rotate(GIMP_ITEM(drawable), angle_rad,
                                  TRUE /*auto_center*/, -1.0 /*center_x*/,
                                  -1.0 /*center_y*/);
  gimp_context_pop();
...
static GimpLayer *p_if_selection_float_it(GimpImage *image, GimpLayer *layer) {
...
static GimpValueArray *bender_run(GimpProcedure *procedure,
  ...
  case GIMP_RUN_NONINTERACTIVE:
    ...
    p_copy_points(cd, OUTLINE_UPPER, 0, GIMP_VALUES_GET_INT(args, 5),
                  GIMP_VALUES_GET_FLOAT_ARRAY(args, 6));
    p_copy_points(cd, OUTLINE_UPPER, 1, GIMP_VALUES_GET_INT(args, 7),
                  GIMP_VALUES_GET_FLOAT_ARRAY(args, 8));
    p_copy_points(cd, OUTLINE_LOWER, 0, GIMP_VALUES_GET_INT(args, 9),
                  GIMP_VALUES_GET_FLOAT_ARRAY(args, 10));
    p_copy_points(cd, OUTLINE_LOWER, 1, GIMP_VALUES_GET_INT(args, 11),
                  GIMP_VALUES_GET_FLOAT_ARRAY(args, 12));

    p_copy_yval(cd, OUTLINE_UPPER, GIMP_VALUES_GET_INT(args, 13),
                GIMP_VALUES_GET_UINT8_ARRAY(args, 14));
    p_copy_yval(cd, OUTLINE_LOWER, GIMP_VALUES_GET_INT(args, 15),
  ...
  if (cd->run) {
    ...
    bent_layer = p_main_bend(cd, cd->drawable, cd->work_on_copy);
...
static int p_save_pointfile(BenderDialog *cd, const gchar *filename) {
  ...
  for (j = 0; j < 256; j++) {
    fprintf(fp, "%s %3d  %3d\n", KEY_VAL_Y, (int)cd->curve[OUTLINE_UPPER][j],
  ...
  if (strncmp(buff, KEY_POINTFILE, strlen(KEY_POINTFILE)) == 0) {
    while (NULL != fgets(buff, 2000 - 1, fp)) {
      ...
      if (strncmp(buff, KEY_POINTS, len) == 0) {
        n = sscanf(&buff[len], "%f %f %f %f", &fux, &fuy, &flx, &fly);
...
static void p_delta_gint32(gint32 *val, gint32 val_from, gint32 val_to,
  ...
  delta = ((double)(val_to - val_from) / (double)total_steps) *
          ((double)total_steps - current_step);
  *val = val_from + delta;
}
...
static BenderDialog *do_dialog(GimpDrawable *drawable) {
  ...
  cd->preview_image = p_create_pv_image(drawable, &cd->preview_layer1);
...
static BenderDialog *bender_new_dialog(GimpDrawable *drawable) {
  ...
  cd->shell = gimp_dialog_new(_("Curve Bend"), PLUG_IN_ROLE, NULL, 0,
                              gimp_standard_help_func, PLUG_IN_PROC,

                              _("_Cancel"), GTK_RESPONSE_CANCEL, _("_OK"),
                              GTK_RESPONSE_OK,

                              NULL);

  gimp_dialog_set_alternative_button_order(
      GTK_DIALOG(cd->shell), GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL, -1);
  ...
  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(cd->shell))), main_hbox,
      TRUE, TRUE, 0);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(bender_preview_update_once),
                   cd);
  ...
  g_signal_connect(cd->rotate_data, "value-changed",
                   G_CALLBACK(bender_rotate_adj_callback), cd);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(bender_smoothing_callback),
                   cd);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(bender_antialias_callback),
                   cd);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(bender_work_on_copy_callback),
                   cd);
  ...
  gtk_widget_set_size_request(cd->graph, GRAPH_WIDTH + RADIUS * 2,
  ...
  g_signal_connect(cd->graph, "draw", G_CALLBACK(bender_graph_draw), cd);
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Curve for Border"), G_CALLBACK(bender_border_callback),
  ...
  frame = gimp_int_radio_group_new(
      TRUE, _("Curve Type"), G_CALLBACK(bender_type_callback), &cd->curve_type,
      NULL, cd->curve_type,

      _("Smoot_h"), SMOOTH, &smooth, _("_Free"), GFREE, &freew,
  ...
  gimp_help_set_help_data(
      button, _("Mirror the active curve to the other border"), NULL);
...
static void bender_update(BenderDialog *cd, int update) {
  if (update & UP_PREVIEW) {
    ...
    gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(cd->shell)), NULL);
  }
...
static void bender_plot_curve(BenderDialog *cd, int p1, int p2, int p3, int p4,
  ...
  if (fix255) {
    ...
    if (gb_debug)
      g_printf("bender_plot_curve xmax:%d ymax:%d\n", (int)xmax, (int)ymax);
  }
  ...
  for (i = 0; i < ntimes; i++) {
    ...
    if ((lastx != newx) || (lasty != newy)) {
      if (fix255) {
        ...
        if (gb_debug)
          g_printf("outline: %d  cX: %d cY: %d\n", (int)cd->outline, (int)newx,
                   (int)newy);
      }
    }
...
static void bender_calculate_curve(BenderDialog *cd, gint32 xmax, gint32 ymax,
                                   gint fix255) {
  int i;
  int points[17];
  int num_pts;
  ...
  case SMOOTH:
    ...
    if (num_pts != 0) {
      if (fix255) {
        ...
        for (i = (cd->points[cd->outline][points[num_pts - 1]][0] * 255);
             i < 256; i++)
          cd->curve[cd->outline][i] =
...
static void bender_rotate_adj_callback(GtkAdjustment *adjustment,
...
static void bender_type_callback(GtkWidget *widget, gpointer data) {
  ...
  if (cd->curve_type == SMOOTH) {
    ...
    for (i = 0; i <= 8; i++) {
      ...
      cd->points[cd->outline][i * 2][1] =
          (gdouble)cd->curve[cd->outline][index] / 255.0;
    }
...
#define SWAP_VALUE(a, b, h)                                                    \
  ...
  for (i = 0; i < 17; i++) {
    SWAP_VALUE(cd->points[other][i][0], cd->points[cd->outline][i][0],
               hd); /* x */
    SWAP_VALUE(cd->points[other][i][1], cd->points[cd->outline][i][1],
               hd); /* y */
  }
...
static void bender_response(GtkWidget *widget, gint response_id,
                            BenderDialog *cd) {
...
static void p_points_save_to_file_response(GtkWidget *dialog, gint response_id,
                                           BenderDialog *cd) {
  if (response_id == GTK_RESPONSE_OK) {
    gchar *filename;
...
static void p_points_load_from_file_response(GtkWidget *dialog,
                                             gint response_id,
                                             BenderDialog *cd) {
  if (response_id == GTK_RESPONSE_OK) {
    gchar *filename;
...
static void bender_load_callback(GtkWidget *w, BenderDialog *cd) {
  if (!cd->filechooser) {
    cd->filechooser = gtk_file_chooser_dialog_new(
        _("Load Curve Points from File"),
        GTK_WINDOW(gtk_widget_get_toplevel(w)), GTK_FILE_CHOOSER_ACTION_OPEN,
    ...
    gimp_dialog_set_alternative_button_order(
        GTK_DIALOG(cd->filechooser), GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL, -1);

    gtk_dialog_set_default_response(GTK_DIALOG(cd->filechooser),
                                    GTK_RESPONSE_OK);

    g_signal_connect(cd->filechooser, "response",
                     G_CALLBACK(p_points_load_from_file_response), cd);
    g_signal_connect(cd->filechooser, "destroy",
                     G_CALLBACK(gtk_widget_destroyed), &cd->filechooser);
  }
...
static void bender_save_callback(GtkWidget *w, BenderDialog *cd) {
  if (!cd->filechooser) {
    cd->filechooser = gtk_file_chooser_dialog_new(
        _("Save Curve Points to File"), GTK_WINDOW(gtk_widget_get_toplevel(w)),
    ...
    g_signal_connect(cd->filechooser, "response",
                     G_CALLBACK(p_points_save_to_file_response), cd);
    g_signal_connect(cd->filechooser, "destroy",
                     G_CALLBACK(gtk_widget_destroyed), &cd->filechooser);
...
static gboolean bender_graph_events(GtkWidget *widget, GdkEvent *event,
                                    BenderDialog *cd) {
  ...
  gdk_window_get_device_position(gtk_widget_get_window(cd->graph),
                                 gdk_event_get_device(event), &tx, &ty, NULL);
      ...
      /*  Else, drag the grabbed point  */
        ...
        if (x > cd->leftmost && x < cd->rightmost) {
          ...
          if (cd->points[cd->outline][closest_point][0] == -1)
            cd->grab_point = closest_point;
          cd->points[cd->outline][cd->grab_point][0] = (gdouble)x / 255.0;
          cd->points[cd->outline][cd->grab_point][1] =
              (gdouble)(255 - y) / 255.0;
        }
...
static gboolean bender_graph_draw(GtkWidget *widget, cairo_t *cr,
                                  BenderDialog *cd) {
  ...
  if (cd->curve_type == SMOOTH) {
    for (i = 0; i < 17; i++) {
      if (cd->points[cd->outline][i][0] != -1) {
        ...
        cairo_arc(cr, (cd->points[cd->outline][i][0] * 255.0) + RADIUS,
...
static void p_render_preview(BenderDialog *cd, GimpLayer *layer) {
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(cd->pv_widget), 0, 0, PREVIEW_SIZE_X,
                         PREVIEW_SIZE_Y, GIMP_RGBA_IMAGE, buf,
...
static void bender_init_min_max(BenderDialog *cd, gint32 xmax) {
   ...
   * rightmost point
   */
  cd->zero2[OUTLINE_UPPER] =
      MIN(cd->curve_ptr[OUTLINE_UPPER][0], cd->curve_ptr[OUTLINE_UPPER][xmax]);

  /* for LOWER outline : y-zero line is assumed at the min leftmost or
   * rightmost point
   */
  cd->zero2[OUTLINE_LOWER] =
      MAX(cd->curve_ptr[OUTLINE_LOWER][0], cd->curve_ptr[OUTLINE_LOWER][xmax]);
}
...
static gint32 p_upper_curve_extend(BenderDialog *cd, gint32 drawable_width,
...
static gint32 p_lower_curve_extend(BenderDialog *cd, gint32 drawable_width,
                                   gint32 drawable_height) {
  gint32 y1, y2;
...
static void p_end_gdrw(t_GDRW *gdrw) { g_object_unref(gdrw->buffer); }
...
static void p_put_pixel(t_GDRW *gdrw, gint32 x, gint32 y, guchar *pixel) {
  gegl_buffer_set(gdrw->buffer, GEGL_RECTANGLE(x, y, 1, 1), 0, gdrw->format,
...
static GimpImage *p_create_pv_image(GimpDrawable *src_drawable,
  ...
  *layer = gimp_layer_new(new_image, "preview_original", new_width, new_height,
                          type, 100.0, /* opacity */
                          0);          /* mode NORMAL */
 ...
 * ============================================================================
 */
static GimpLayer *p_add_layer(gint width, gint height,
  ...
  new_layer = gimp_layer_new(image, name, width, height, type, opacity, mode);
...
static void p_bender_calculate_iter_curve(BenderDialog *cd, gint32 xmax,
  ...
  if ((cd->bval_from == NULL) || (cd->bval_to == NULL) ||
      (cd->bval_curr == NULL)) {
    if (gb_debug)
    ...
    if (gb_debug)
      g_printf("p_bender_calculate_iter_curve ITERmode 1\n");
    ...
    for (x = 0; x <= xmax; x++) {
      p_delta_gint32(&cd->curve_ptr[OUTLINE_UPPER][x],
                     ...
                     cd->bval_curr->total_steps, cd->bval_curr->current_step);
...
static void p_vertical_bend(BenderDialog *cd, t_GDRW *src_gdrw,
                            t_GDRW *dst_gdrw) {
  gint32 row, col;
  ...
  topshift = p_upper_curve_extend(cd, src_gdrw->width, src_gdrw->height);
    ...
    for (col = first_col; col <= last_col; col++) {
      ...
      for (; x < x2; x++) {
        ...
        for (; y < y2; y++) {
          ...
          curvy = p_curve_get_dy(cd, x, (gint32)src_gdrw->width,
                                 (gint32)src_gdrw->height, (gdouble)y);
          desty = y + topshift + curvy;

          /* ----------- SMOOTHING ------------------ */
          if (cd->smoothing && (x > 0)) {
            nb_curvy = p_curve_get_dy(cd, x - 1, (gint32)src_gdrw->width,
                                      (gint32)src_gdrw->height, (gdouble)y);
            if ((nb_curvy == curvy) && (x > 1)) {
              nb2_curvy = p_curve_get_dy(cd, x - 2, (gint32)src_gdrw->width,
                                         (gint32)src_gdrw->height, (gdouble)y);
            } else {
              nb2_curvy = nb_curvy;
            }

            p_put_mix_pixel(dst_gdrw, x, desty, color, nb_curvy, nb2_curvy,
                            curvy);
          } else {
          ...
          if (cd->antialias) {
            ...
            {
              first_arr[x].y = curvy;
              memcpy(first_arr[x].color, color, dst_gdrw->bpp);

              if (x > 0) {
                memcpy(mixcolor, first_arr[x - 1].color, dst_gdrw->bpp);

                diff = abs(first_arr[x - 1].y - curvy) + 1;
                miny = MIN(first_arr[x - 1].y, curvy) - 1;
                maxy = MAX(first_arr[x - 1].y, curvy) + 1;

                othery = (src_gdrw->y2 - 1) + topshift +
                         p_curve_get_dy(cd, x, (gint32)src_gdrw->width,
            ...
            {
              if (x > 0) {
                memcpy(mixcolor, last_arr[x - 1].color, dst_gdrw->bpp);
            ...
            if (alias_dir != 0) {
              ...
              for (dy = 0; dy < diff; dy++) {
                ...
                mixcolor[dst_gdrw->index_alpha] = MIX_CHANNEL(
                    color[dst_gdrw->index_alpha], alpha_lo, mixmask);

                if (alias_dir > 0) {
                  p_put_pixel(dst_gdrw, x - 1, y + topshift + miny + dy,
                              mixcolor);
                } else {
                  p_put_pixel(dst_gdrw, x - 1, y + topshift + (maxy - dy),
                              mixcolor);
                }
              }
            }
            ...
            if (cd->smoothing) {
              ...
              if (mixcolor[3]) {
                mixcolor[0] = (alpha1 * mixmask * last_arr[x].color[0] +
                               alpha2 * (255 - mixmask) * color[0]) /
                              alpha;
                mixcolor[1] = (alpha1 * mixmask * last_arr[x].color[1] +
                               alpha2 * (255 - mixmask) * color[1]) /
                              alpha;
                mixcolor[2] = (alpha1 * mixmask * last_arr[x].color[2] +
                               alpha2 * (255 - mixmask) * color[2]) /
                              alpha;
                /*mixcolor[2] =  MIX_CHANNEL(last_arr[x].color[2], color[2],
                 * mixmask);*/
              }
            } else {
              ...
              if (dy < diff / 2) {
                ...
                memcpy(mixcolor, last_arr[x].color, dst_gdrw->bpp);
              }
            }

            if (cd->smoothing) {
              p_put_mix_pixel(dst_gdrw, x, desty + (dy * sign), mixcolor,
                              nb_curvy, nb2_curvy, curvy);
            } else {
              p_put_pixel(dst_gdrw, x, desty + (dy * sign), mixcolor);
            }
          }
...
static GimpLayer *p_main_bend(BenderDialog *cd, GimpDrawable *original_drawable,
                              gint work_on_copy) {
  t_GDRW src_gdrw;
  t_GDRW dst_gdrw;
  GimpDrawable *src_drawable;
  ...
  if (gb_debug)
    g_printf("p_main_bend  tmp_layer_id %d\n",
             (int)gimp_item_get_id(GIMP_ITEM(tmp_layer)));

  if (cd->rotation != 0.0) {
    if (gb_debug)
      g_printf("p_main_bend rotate: %f\n", (float)cd->rotation);
    p_gimp_rotate(image, GIMP_DRAWABLE(tmp_layer), interpolation, cd->rotation);
  }
  ...
  dst_height = src_height + p_upper_curve_extend(cd, src_width, src_height) +
               p_lower_curve_extend(cd, src_width, src_height);

  if (gb_debug)
    g_printf("p_main_bend: dst_height:%d\n", dst_height);

  if (work_on_copy) {
    dst_drawable =
        GIMP_DRAWABLE(p_add_layer(src_width, dst_height, src_drawable));
    if (gb_debug)
      g_printf("p_main_bend: DONE add layer\n");
  } else {
    /* work on the original */
    gimp_layer_resize(GIMP_LAYER(original_drawable), src_width, dst_height,
                      offset_x, offset_y);
    if (gb_debug)
      g_printf("p_main_bend: DONE layer resize\n");
  ...
  if (gb_debug)
    g_printf("p_main_bend: DONE vertical bend\n");

  p_end_gdrw(&src_gdrw);
  p_end_gdrw(&dst_gdrw);

  if (cd->rotation != 0.0) {
    p_gimp_rotate(image, dst_drawable, interpolation,
                  (gdouble)(360.0 - cd->rotation));

    /* TODO: here we should crop dst_drawable to cut off full transparent
     * borderpixels */
  }
  ...
  if (gb_debug)
    g_printf("p_main_bend: DONE bend main\n");
