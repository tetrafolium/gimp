...
#define BORDER_AVERAGE                                                         \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), BORDER_AVERAGE_TYPE, BorderAverage))
...
border_average_run(GimpProcedure *procedure, GimpRunMode run_mode,
                   GimpImage *image, GimpDrawable *drawable,
                   const GimpValueArray *args, gpointer run_data);

static void borderaverage(GeglBuffer *buffer, GimpDrawable *drawable,
                          GimpRGB *result);

static gboolean borderaverage_dialog(GimpImage *image, GimpDrawable *drawable);

static void add_new_color(const guchar *buffer, gint *cube, gint bucket_expo);

static void thickness_callback(GtkWidget *widget, gpointer data);
...
static borderaverage_data = {3, 4};

static void border_average_class_init(BorderAverageClass *klass) {
...
static void border_average_init(BorderAverage *film) {}
...
static GimpProcedure *border_average_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    ...
    gimp_procedure_set_documentation(
        ...
        name);
    gimp_procedure_set_attribution(procedure, "Philipp Klaus",
                                   "Internet Access AG", "1998");

    GIMP_PROC_ARG_INT(procedure, "thickness", "Border size to take in count",
                      "Border size to take in count", 0, G_MAXINT, 3,
                      G_PARAM_READWRITE);
    GIMP_PROC_ARG_INT(procedure, "bucket-exponent",
                      "Bits for bucket size (default=4: 16 Levels)",
                      "Bits for bucket size (default=4: 16 Levels)", 0,
                      G_MAXINT, 4, G_PARAM_READWRITE);

    GIMP_PROC_VAL_RGB(procedure, "borderaverage",
                      "The average color of the specified border.",
                      "The average color of the specified border.", TRUE, NULL,
                      G_PARAM_READWRITE);
  }
  ...
  if (status == GIMP_PDB_SUCCESS) {
    ...
    if (gimp_drawable_is_rgb(drawable)) {
      ...
      if (run_mode == GIMP_RUN_INTERACTIVE) {
        ...
        gimp_set_data(PLUG_IN_PROC, &borderaverage_data,
                      sizeof(borderaverage_data));
      }
    } else {
      status = GIMP_PDB_EXECUTION_ERROR;
    }
...
static void borderaverage(GeglBuffer *buffer, GimpDrawable *drawable,
  ...
  for (i = 0; i < 4; i++) {
    if (border[i].width > 0 && border[i].height > 0) {
      ...
      gi = gegl_buffer_iterator_new(buffer, &border[i], 0,
      ...
      while (gegl_buffer_iterator_next(gi)) {
        ...
        for (k = 0; k < gi->length; k++) {
          add_new_color(data + k * 3, cube, bucket_expo);
        }
      }
    }
  }
...
static gboolean borderaverage_dialog(GimpImage *image, GimpDrawable *drawable) {
  ...
  const gchar *labels[] = {"1", "2", "4", "8", "16", "32", "64", "128", "256"};

  gimp_ui_init(PLUG_IN_BINARY);

  dialog = gimp_dialog_new(_("Border Average"), PLUG_IN_ROLE, NULL, 0,
  ...
  if (buffer)
    gimp_size_entry_set_size(
        GIMP_SIZE_ENTRY(size_entry), 0, 0.0,
        MIN(gegl_buffer_get_width(buffer), gegl_buffer_get_height(buffer)));
  ...
  g_signal_connect(size_entry, "value-changed", G_CALLBACK(thickness_callback),
                   NULL);
