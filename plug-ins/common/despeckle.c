...
#define DESPECKLE                                                              \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), DESPECKLE_TYPE, Despeckle))
...
static void despeckle(GimpDrawable *drawable, GObject *config);
static void despeckle_median(GObject *config, guchar *src, guchar *dst,
                             gint width, gint height, gint bpp,
                             gboolean preview);

static gboolean despeckle_dialog(GimpProcedure *procedure, GObject *config,
                                 GimpDrawable *drawable);

static void dialog_adaptive_callback(GtkWidget *button, GObject *config);
static void dialog_adaptive_notify(GObject *config, const GParamSpec *pspec,
                                   GtkWidget *button);
static void dialog_recursive_callback(GtkWidget *button, GObject *config);
static void dialog_recursive_notify(GObject *config, const GParamSpec *pspec,
                                    GtkWidget *button);

static void preview_update(GtkWidget *preview, GObject *config);
...
static void despeckle_class_init(DespeckleClass *klass) {
...
static void despeckle_init(Despeckle *despeckle) {}
...
static GimpProcedure *despeckle_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, despeckle_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(procedure,
                                     ...
                                     "an image.",
                                     name);
    gimp_procedure_set_attribution(procedure, "Michael Sweet <mike@easysw.com>",
                                   "Copyright 1997-1998 by Michael Sweet",
                                   PLUG_IN_VERSION);

    GIMP_PROC_ARG_INT(procedure, "radius", "Radius", "Filter box radius", 1,
                      MAX_RADIUS, 3, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "type", "Type",
                      "Filter type { MEDIAN (0), ADAPTIVE (1), "
                      "RECURSIVE-MEDIAN (2), RECURSIVE-ADAPTIVE (3) }",
                      0, 3, FILTER_ADAPTIVE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "black", "Black", "Black level", -1, 255, 7,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "white", "White", "White level", 0, 256, 248,
                      G_PARAM_READWRITE);
  }
...
static inline guchar pixel_luminance(const guchar *p, gint bpp) {
  switch (bpp) {
  case 1:
  case 2:
    return p[0];
...
static inline void pixel_copy(guchar *dest, const guchar *src, gint bpp) {
  switch (bpp) {
  case 4:
    *dest++ = *src++;
  case 3:
...
static void despeckle(GimpDrawable *drawable, GObject *config) {
  ...
  gegl_buffer_get(src_buffer, GEGL_RECTANGLE(x, y, width, height), 1.0, format,
                  src, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);

  despeckle_median(config, src, dst, width, height, img_bpp, FALSE);

  gegl_buffer_set(dest_buffer, GEGL_RECTANGLE(x, y, width, height), 0, format,
                  dst, GEGL_AUTO_ROWSTRIDE);
...
static gboolean despeckle_dialog(GimpProcedure *procedure, GObject *config,
  ...
  g_signal_connect(button, "toggled", G_CALLBACK(dialog_adaptive_callback),
                   config);
  g_signal_connect(config, "notify::type", G_CALLBACK(dialog_adaptive_notify),
                   button);
  ...
  g_signal_connect(button, "toggled", G_CALLBACK(dialog_recursive_callback),
                   config);
  g_signal_connect(config, "notify::type", G_CALLBACK(dialog_recursive_notify),
                   button);
  ...
  scale = gimp_prop_scale_entry_new(config, "radius", _("_Radius:"), 1.0, FALSE,
                                    0, 0);
  ...
  scale = gimp_prop_scale_entry_new(config, "black", _("_Black level:"), 1.0,
                                    FALSE, 0, 0);
  ...
  scale = gimp_prop_scale_entry_new(config, "white", _("_White level:"), 1.0,
                                    FALSE, 0, 0);
...
static void preview_update(GtkWidget *widget, GObject *config) {
  ...
  gegl_buffer_get(src_buffer, GEGL_RECTANGLE(x1, y1, width, height), 1.0,
                  format, src, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);

  despeckle_median(config, src, dst, width, height, img_bpp, TRUE);
...
static void dialog_adaptive_callback(GtkWidget *button, GObject *config) {
  ...
  g_signal_handlers_block_by_func(config, dialog_adaptive_notify, button);

  g_object_set(config, "type", filter_type, NULL);

  g_signal_handlers_unblock_by_func(config, dialog_adaptive_notify, button);
}
...
static void dialog_adaptive_notify(GObject *config, const GParamSpec *pspec,
  ...
  g_signal_handlers_block_by_func(button, dialog_adaptive_callback, config);

  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button),
                               (filter_type & FILTER_ADAPTIVE) != 0);

  g_signal_handlers_unblock_by_func(button, dialog_adaptive_callback, config);
}
...
static void dialog_recursive_callback(GtkWidget *button, GObject *config) {
  ...
  g_signal_handlers_block_by_func(config, dialog_recursive_notify, button);

  g_object_set(config, "type", filter_type, NULL);

  g_signal_handlers_unblock_by_func(config, dialog_recursive_notify, button);
}
...
static void dialog_recursive_notify(GObject *config, const GParamSpec *pspec,
  ...
  g_signal_handlers_block_by_func(button, dialog_recursive_callback, config);

  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button),
                               (filter_type & FILTER_RECURSIVE) != 0);

  g_signal_handlers_unblock_by_func(button, dialog_recursive_callback, config);
}

static inline void list_add_elem(PixelsList *list, const guchar *elem) {
...
static inline const guchar *list_get_random_elem(PixelsList *list) {
...
static inline void histogram_add(DespeckleHistogram *hist, guchar val,
                                 const guchar *orig) {
  hist->elems[val]++;
  list_add_elem(&hist->origs[val], orig);
}

static inline void histogram_remove(DespeckleHistogram *hist, guchar val) {
  hist->elems[val]--;
...
static inline const guchar *histogram_get_median(DespeckleHistogram *hist,
...
static inline void add_val(DespeckleHistogram *hist, gint black_level,
                           gint white_level, const guchar *src, gint width,
                           gint bpp, gint x, gint y) {
...
static inline void del_val(DespeckleHistogram *hist, gint black_level,
                           gint white_level, const guchar *src, gint width,
                           gint bpp, gint x, gint y) {
  const gint pos = (x + (y * width)) * bpp;
  const gint value = pixel_luminance(src + pos, bpp);
...
static inline void add_vals(DespeckleHistogram *hist, gint black_level,
                            gint white_level, const guchar *src, gint width,
                            gint bpp, gint xmin, gint ymin, gint xmax,
                            gint ymax) {
  gint x;
  gint y;
  ...
  for (y = ymin; y <= ymax; y++) {
    for (x = xmin; x <= xmax; x++) {
      add_val(hist, black_level, white_level, src, width, bpp, x, y);
    }
  }
}

static inline void del_vals(DespeckleHistogram *hist, gint black_level,
                            gint white_level, const guchar *src, gint width,
                            gint bpp, gint xmin, gint ymin, gint xmax,
                            gint ymax) {
  gint x;
  ...
  for (y = ymin; y <= ymax; y++) {
    for (x = xmin; x <= xmax; x++) {
      del_val(hist, black_level, white_level, src, width, bpp, x, y);
    }
  }
}

static inline void update_histogram(DespeckleHistogram *hist, gint black_level,
                                    gint white_level, const guchar *src,
                                    gint width, gint bpp, gint xmin, gint ymin,
                                    gint xmax, gint ymax) {
  /* assuming that radious of the box can change no more than one
     pixel in each call */
  /* assuming that box is moving either right or down */

  del_vals(hist, black_level, white_level, src, width, bpp, hist->xmin,
           hist->ymin, xmin - 1, hist->ymax);
  del_vals(hist, black_level, white_level, src, width, bpp, xmin, hist->ymin,
           xmax, ymin - 1);
  del_vals(hist, black_level, white_level, src, width, bpp, xmin, ymax + 1,
           xmax, hist->ymax);

  add_vals(hist, black_level, white_level, src, width, bpp, hist->xmax + 1,
           ymin, xmax, ymax);
  add_vals(hist, black_level, white_level, src, width, bpp, xmin, ymin,
           hist->xmax, hist->ymin - 1);
  add_vals(hist, black_level, white_level, src, width, bpp, hist->xmin,
           hist->ymax + 1, hist->xmax, ymax);
...
static void despeckle_median(GObject *config, guchar *src, guchar *dst,
  ...
  g_object_get(config, "radius", &radius, "type", &filter_type, "black",
               &black_level, "white", &white_level, NULL);
  ...
  for (y = 0; y < height; y++) {
    ...
    histogram.xmax = xmax;
    histogram.ymax = ymax;
    add_vals(&histogram, black_level, white_level, src, width, bpp,
             histogram.xmin, histogram.ymin, histogram.xmax, histogram.ymax);

    for (x = 0; x < width; x++) {
      const guchar *pixel;

      ymin = MAX(0, y - adapt_radius); /* update ymin, ymax when adapt_radius
                                          changed (FILTER_ADAPTIVE) */
      ymax = MIN(height - 1, y + adapt_radius);
      xmin = MAX(0, x - adapt_radius);
      xmax = MIN(width - 1, x + adapt_radius);

      update_histogram(&histogram, black_level, white_level, src, width, bpp,
                       xmin, ymin, xmax, ymax);
      ...
      if (filter_type & FILTER_RECURSIVE) {
        del_val(&histogram, black_level, white_level, src, width, bpp, x, y);

        pixel_copy(src + pos, pixel, bpp);

        add_val(&histogram, black_level, white_level, src, width, bpp, x, y);
      }
