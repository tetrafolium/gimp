...
static FitsRecordList *fits_read_header(FILE *fp, gint *nrec);
static FitsHduList *fits_decode_header(FitsRecordList *hdr, glong hdr_offset,
                                       glong dat_offset);
static gint fits_eval_pixrange(FILE *fp, FitsHduList *hdu);
...
/* Macros */
#define FITS_RETURN(msg, val)                                                  \
  {                                                                            \
    fits_set_error(msg);                                                       \
    return (val);                                                              \
  }
#define FITS_VRETURN(msg)                                                      \
  {                                                                            \
    fits_set_error(msg);                                                       \
    return;                                                                    \
  }
...
#define FITS_GETBITPIXM32(p, val)                                              \
  {                                                                            \
    if (fits_ieee32_intel) {                                                   \
    ...
    } else if (fits_ieee32_motorola) {                                         \
      val = *(FitsBitpixM32 *)p;                                               \
    } else if (fits_ieee64_motorola) {                                         \
      FitsBitpixM64 m64;                                                       \
      ...
      uc[2] = p[2];                                                            \
      uc[3] = p[3];                                                            \
      uc[4] = uc[5] = uc[6] = uc[7] = 0;                                       \
      val = (FitsBitpixM32)m64;                                                \
    } else if (fits_ieee64_intel) {                                            \
      FitsBitpixM64 i64;                                                       \
...
#define FITS_WRITE_BOOLCARD(fp, key, value)                                    \
  {                                                                            \
    gchar card[81];                                                            \
    g_snprintf(card, sizeof(card), "%-8.8s= %20s%50s", key, value ? "T" : "F", \
               " ");                                                           \
    fwrite(card, 1, 80, fp);                                                   \
  }
...
#define FITS_WRITE_LONGCARD(fp, key, value)                                    \
  {                                                                            \
    gchar card[81];                                                            \
    g_snprintf(card, sizeof(card), "%-8.8s= %20ld%50s", key, (long)value,      \
               " ");                                                           \
    fwrite(card, 1, 80, fp);                                                   \
  }
...
#define FITS_WRITE_DOUBLECARD(fp, key, value)                                  \
  {                                                                            \
    ...
    if (istr)                                                                  \
      *istr = 'E';                                                             \
    g_snprintf(card, sizeof(card), "%-8.8s= %20.20s%50s", key, dbl, " ");      \
    fwrite(card, 1, 80, fp);                                                   \
  }
...
#define FITS_WRITE_STRINGCARD(fp, key, value)                                  \
  {                                                                            \
    gchar card[81];                                                            \
    int k;                                                                     \
    g_snprintf(card, sizeof(card), "%-8.8s= \'%s", key, value);                \
    for (k = strlen(card); k < 81; k++)                                        \
      card[k] = ' ';                                                           \
    k = strlen(key);                                                           \
    if (k < 8)                                                                 \
      card[19] = '\'';                                                         \
...
#define FITS_WRITE_CARD(fp, value)                                             \
  {                                                                            \
    gchar card[81];                                                            \
    g_snprintf(card, sizeof(card), "%-80.80s", value);                         \
    fwrite(card, 1, 80, fp);                                                   \
  }
...
static gboolean fits_scanfdouble(const gchar *buf, gdouble *value) {
...
static FitsFile *fits_new_filestruct(void) { return g_new0(FitsFile, 1); }
...
static FitsHduList *fits_new_hdulist(void) { return g_new0(FitsHduList, 1); }
...
static int fits_nan_32(guchar *v) {
  register gulong k;
...
static int fits_nan_64(guchar *v) {
  register gulong k;
...
static void fits_set_error(const gchar *errmsg) {
...
FitsFile *fits_open(const gchar *filename, const gchar *openmode)
...
gint fits_add_card(FitsHduList *hdulist, const gchar *card) {
  gint k;
...
gint fits_write_header(FitsFile *ff, FitsHduList *hdulist) {
  gint numcards;
  gint k;
...
#define FITS_TRY_CARD(mhdr, mhdu, mkey, mvar, mtyp, unionvar)                  \
  ...
  FITS_TRY_CARD(hdr, hdulist, "DATAMIN", datamin, FITS_DATA_TYPE_FDOUBLE,
                fdouble);
  FITS_TRY_CARD(hdr, hdulist, "DATAMAX", datamax, FITS_DATA_TYPE_FDOUBLE,
                fdouble);

  FITS_TRY_CARD(hdr, hdulist, "BZERO", bzero, FITS_DATA_TYPE_FDOUBLE, fdouble);
  FITS_TRY_CARD(hdr, hdulist, "BSCALE", bscale, FITS_DATA_TYPE_FDOUBLE,
                fdouble);
  ...
  bitpix_supported =
      ...
      ((hdulist->bitpix == -32) && (fits_ieee32_intel || fits_ieee32_motorola ||
                                    fits_ieee64_intel || fits_ieee64_motorola));
...
FitsData *fits_decode_card(const gchar *card, FitsDataType data_type) {
  ...
  case FITS_DATA_TYPE_BITPIX_16:
    if (sscanf(l_card + 10, "%ld", &l_long) != 1) {
      fits_set_error(
          "fits_decode_card: error decoding FITS_DATA_TYPE_BITPIX_16");
      ErrCount++;
      break;
    }
  ...
  case FITS_DATA_TYPE_BITPIX_32:
    if (sscanf(l_card + 10, "%ld", &l_long) != 1) {
      fits_set_error(
          "fits_decode_card: error decoding FITS_DATA_TYPE_BITPIX_32");
      ErrCount++;
      break;
    }
  ...
  case FITS_DATA_TYPE_BITPIX_M32:
    if (fits_scanfdouble(l_card + 10, &l_double) != 1) {
      fits_set_error(
          "fits_decode_card: error decoding FITS_DATA_TYPE_BITPIX_M32");
      ErrCount++;
      break;
    }
  ...
  case FITS_DATA_TYPE_BITPIX_M64:
    if (fits_scanfdouble(l_card + 10, &l_double) != 1) {
      fits_set_error(
          "fits_decode_card: error decoding FITS_DATA_TYPE_BITPIX_M64");
      ErrCount++;
      break;
    }
  ...
  case FITS_DATA_TYPE_FSTRING:
    ...
    if (*cp != '\'') {
      fits_set_error(
          "fits_decode_card: missing \' decoding FITS_DATA_TYPE_FSTRING");
      ErrCount++;
      break;
    }
...
gchar *fits_search_card(FitsRecordList *rl, gchar *keyword) {
  gint key_len, k;
  gchar key[9];
...
FitsHduList *fits_image_info(FitsFile *ff, gint picind, gint *hdupicind) {
  FitsHduList *hdulist;
...
FitsHduList *fits_seek_image(FitsFile *ff, gint picind) {
  FitsHduList *hdulist;
  gint hdupicind;
...
gint fits_read_pixel(FitsFile *ff, FitsHduList *hdulist, gint npix,
                     FitsPixTransform *trans, void *buf) {
  gdouble offs, scale;
...
gint fits_to_pgmraw(gchar *fitsfile, gchar *pgmfile) {
...
gint pgmraw_to_fits(gchar *pgmfile, gchar *fitsfile) {
