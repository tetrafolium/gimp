...
static GimpValueArray *fits_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                 GFile *file, const GimpValueArray *args,
                                 gpointer run_data);
static GimpValueArray *fits_save(GimpProcedure *procedure, GimpRunMode run_mode,
                                 GimpImage *image, gint n_drawables,
                                 GimpDrawable **drawables, GFile *file,
                                 const GimpValueArray *args, gpointer run_data);

static GimpImage *load_image(GFile *file, GObject *config, GimpRunMode run_mode,
                             GError **error);
static gint save_image(GFile *file, GimpImage *image, GimpDrawable *drawable,
                       GError **error);

static FitsHduList *create_fits_header(FitsFile *ofp, guint width, guint height,
                                       guint channels, guint bitpix);

static gint save_fits(FitsFile *ofp, GimpImage *image, GimpDrawable *drawable);

static GimpImage *create_new_image(GFile *file, guint pagenum, guint width,
                                   guint height, GimpImageBaseType itype,
                                   GimpImageType dtype,
                                   GimpPrecision iprecision, GimpLayer **layer,
                                   GeglBuffer **buffer);

static GimpImage *load_fits(GFile *file, FitsFile *ifp, GObject *config,
                            guint picnum, guint ncompose);

static gboolean load_dialog(GimpProcedure *procedure, GObject *config);
...
static void fits_class_init(FitsClass *klass) {
...
static void fits_init(Fits *fits) {}
...
static GimpProcedure *fits_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, fits_load, NULL, NULL);
    ...
    GIMP_PROC_AUX_ARG_INT(procedure, "replace", "Replace",
                          "Replacement for undefined pixels", 0, 255, 0,
                          G_PARAM_READWRITE);

    GIMP_PROC_AUX_ARG_BOOLEAN(procedure, "use-data-min-max", "Use data min max",
                              "Use DATAMIN/DATAMAX-scaling if possible", FALSE,
                              G_PARAM_READWRITE);

    GIMP_PROC_AUX_ARG_BOOLEAN(procedure, "compose", "Compose",
                              "Image composing", FALSE, G_PARAM_READWRITE);
  } else if (!strcmp(name, SAVE_PROC)) {
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, fits_save, NULL, NULL);
...
static GimpImage *load_image(GFile *file, GObject *config, GimpRunMode run_mode,
  ...
  g_object_get(config, "compose", &compose_arg, NULL);
  ...
  if (!ifp) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, "%s",
                _("Error during open of FITS file"));
    return NULL;
  }

  if (ifp->n_pic <= 0) {
    g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, "%s",
                _("FITS file keeps no displayable images"));
    fits_close(ifp);
    return NULL;
  }
  ...
  /* Display images in reverse order. The last will be displayed by GIMP
   * itself*/
...
static GimpImage *create_new_image(GFile *file, guint pagenum, guint width,
  ...
  *layer = gimp_layer_new(image, _("Background"), width, height, dtype, 100,
...
static GimpImage *load_fits(GFile *file, FitsFile *ifp, GObject *config,
  ...
  g_object_get(config, "replace", &replace, "use-data-min-max", &use_datamin,
               NULL);
  ...
  if (ncompose == 2) {
    ...
    format = babl_format_new(babl_model("R'G'B'"), type, babl_component("R'"),
                             babl_component("G'"), babl_component("B'"), NULL);
  } else if (ncompose == 4) {
    itype = GIMP_RGB;
    dtype = GIMP_RGBA_IMAGE;
    format = babl_format_new(babl_model("R'G'B'A"), type, babl_component("R'"),
  ...
  image = create_new_image(file, picnum, width, height, itype, dtype,
                           iprecision, &layer, &buffer);
   ...
   * specified, use it
   */
  if (use_datamin && hdulist->used.datamin && hdulist->used.datamax &&
    ...
    for (i = 0; i < height; i++) {
      ...
      if ((scan_lines == tile_height) || ((i + 1) == height)) {
        gegl_buffer_set(buffer,
                        GEGL_RECTANGLE(0, height - i - 1, width, scan_lines), 0,
  ...
  } else /* multiple images to compose */
  {
    ...
    for (channel = 0; channel < ncompose; channel++) {
      dest = data + tile_height * width * hdulist->bpp * ncompose +
             channel * hdulist->bpp;
      scan_lines = 0;
      ...
      for (i = 0; i < height; i++) {
        if ((channel > 0) && ((i % tile_height) == 0)) {
          ...
          gegl_buffer_get(
              buffer, GEGL_RECTANGLE(0, height - i - max_scan, width, max_scan),
              1.0, format,
              data_end - max_scan * width * hdulist->bpp * ncompose,
        ...
        if ((scan_lines == tile_height) || ((i + 1) == height)) {
          gegl_buffer_set(
              buffer, GEGL_RECTANGLE(0, height - i - 1, width, scan_lines), 0,
              format, dest - channel * hdulist->bpp, GEGL_AUTO_ROWSTRIDE);

          scan_lines = 0;
          dest = data + tile_height * width * ncompose * hdulist->bpp +
                 channel * hdulist->bpp;
        }
...
static FitsHduList *create_fits_header(FitsFile *ofp, guint width, guint height,
  ...
  static const char *ctype3_card[] = {
      NULL,
      NULL,
      NULL, /* bpp = 0: no additional card */
      "COMMENT Image type within GIMP: GIMP_GRAY_IMAGE",
      NULL,
      NULL,
      "COMMENT Image type within GIMP: GIMP_GRAYA_IMAGE (gray with alpha "
      "channel)",
      ...
      "COMMENT Image type within GIMP: GIMP_RGBA_IMAGE (rgb with alpha "
      "channel)",
  ...
  fits_add_card(hdulist, "");
  fits_add_card(hdulist, "COMMENT FitsRW is (C) Peter Kirchgessner "
                         "(peter@kirchgessner.net), but available");
  fits_add_card(hdulist, "COMMENT under the GNU general public licence.");
  fits_add_card(hdulist, "COMMENT For sources see http://www.kirchgessner.net");
  fits_add_card(hdulist, "");
  ...
  for (channel = 0; channel < channelnum; channel++) {
    for (i = 0; i < height; i++) {
      if ((i % tile_height) == 0) {
        ...
        gegl_buffer_get(
            buffer,
            GEGL_RECTANGLE(0, height - i - scan_lines, width, scan_lines), 1.0,
      ...
      if (channelnum == 1 &&
          bitpix == 8) /* One channel and 8 bit? Write the scanline */
      {
...
static gboolean load_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  store = gimp_int_store_new(_("_Black"), 0, _("_White"), 255, NULL);
  frame = gimp_prop_int_radio_frame_new(config, "replace",
                                        _("Replacement for undefined pixels"),
                                        GIMP_INT_STORE(store));
  gtk_box_pack_start(GTK_BOX(vbox), frame, FALSE, FALSE, 0);

  frame = gimp_prop_boolean_radio_frame_new(
      config, "use-data-min-max", _("Pixel value scaling"),
      _("By _DATAMIN/DATAMAX"), _("_Automatic"));
  gtk_box_pack_start(GTK_BOX(vbox), frame, FALSE, FALSE, 0);

  frame = gimp_prop_boolean_radio_frame_new(
      config, "compose", _("Image Composing"), "NA_XIS=3, NAXIS3=2,...,4",
      C_("composing", "_None"));
  gtk_box_pack_start(GTK_BOX(vbox), frame, FALSE, FALSE, 0);
