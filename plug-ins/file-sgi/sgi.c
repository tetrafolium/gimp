 ...
 *   save_ok_callback()          - Destroy the export dialog and export the
 * image. save_dialog()               - Pop up the export dialog.
 *
 */
...
static GimpValueArray *sgi_load(GimpProcedure *procedure, GimpRunMode run_mode,
                                GFile *file, const GimpValueArray *args,
                                gpointer run_data);
static GimpValueArray *sgi_save(GimpProcedure *procedure, GimpRunMode run_mode,
...
static void sgi_class_init(SgiClass *klass) {
...
static GimpProcedure *sgi_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, LOAD_PROC)) {
    procedure = gimp_load_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, sgi_load, NULL, NULL);

    gimp_procedure_set_menu_label(procedure, N_("Silicon Graphics IRIS image"));

    gimp_procedure_set_documentation(
        procedure, "Loads files in SGI image file format",
        "This plug-in loads SGI image files.", name);
    ...
    procedure = gimp_save_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, sgi_save, NULL, NULL);
    ...
    gimp_procedure_set_documentation(
        procedure, "Exports files in SGI image file format",
        "This plug-in exports SGI image files.", name);
    gimp_procedure_set_attribution(procedure, "Michael Sweet <mike@easysw.com>",
                                   "Copyright 1997-1998 by Michael Sweet",
                                   PLUG_IN_VERSION);
    ...
    GIMP_PROC_ARG_INT(procedure, "compression", "Compression",
                      "Compression level (0 = none, 1 = RLE, 2 = ARLE)", 0, 2,
                      SGI_COMP_RLE, GIMP_PARAM_READWRITE);
  }
...
static GimpValueArray *sgi_save(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_WITH_LAST_VALS:
    ...
    export = gimp_export_image(
        &image, &n_drawables, &drawables, "SGI",
...
static GimpImage *load_image(GFile *file, GError **error) {
  ...
  for (y = 0, count = 0; y < sgip->ysize; y++, count++) {
    if (count >= tile_height) {
      gegl_buffer_set(buffer, GEGL_RECTANGLE(0, y - count, sgip->xsize, count),
                      0, NULL, pixels[0], GEGL_AUTO_ROWSTRIDE);
  ...
  gegl_buffer_set(buffer, GEGL_RECTANGLE(0, y - count, sgip->xsize, count), 0,
...
static gint save_image(GFile *file, GimpImage *image, GimpDrawable *drawable,
  ...
  filename = g_file_get_path(file);
  sgip = sgiOpen(filename, SGI_WRITE, compression, 1, width, height, zsize);
  g_free(filename);
  ...
  for (y = 0; y < height; y += count) {
    ...
    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, y, width, count), 1.0, format,
                    pixels[0], GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
...
static gboolean save_dialog(GimpProcedure *procedure, GObject *config) {
  ...
  store = gimp_int_store_new(
      _("No compression"), SGI_COMP_NONE, _("RLE compression"), SGI_COMP_RLE,
      _("Aggressive RLE (not supported by SGI)"), SGI_COMP_ARLE, NULL);
  combo =
      gimp_prop_int_combo_box_new(config, "compression", GIMP_INT_STORE(store));
  g_object_unref(store);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Compression _type:"), 1.0,
                           0.5, combo, 1);
