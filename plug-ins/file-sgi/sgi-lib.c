...
int sgiClose(sgi_t *sgip) /* I - SGI image */
{
  int i;        /* Return status */
  ...
  if (sgip->mode == SGI_WRITE && sgip->comp != SGI_COMP_NONE) {
    ...
    for (i = sgip->ysize * sgip->zsize, offset = sgip->table[0]; i > 0;
         i--, offset++)
      if (putlong(offset[0], sgip) < 0)
        return (-1);

    for (i = sgip->ysize * sgip->zsize, offset = sgip->length[0]; i > 0;
         i--, offset++)
      if (putlong(offset[0], sgip) < 0)
        return (-1);
  };
...
int sgiGetRow(sgi_t *sgip,         /* I - SGI image */
              ...
              int z)               /* I - Channel to read */
{
  ...
  if (sgip == NULL || row == NULL || y < 0 || y >= sgip->ysize || z < 0 ||
      z >= sgip->zsize)
    return (-1);
...
sgi_t *sgiOpen(const char *filename, /* I - File to open */
...
sgi_t *sgiOpenFile(FILE *file, /* I - File to open */
                   int mode,   /* I - Open mode (SGI_READ or SGI_WRITE) */
                   int comp,   /* I - Type of compression */
                   int bpp,    /* I - Bytes per pixel */
                   int xsize,  /* I - Width of image in pixels */
                   ...
                   int zsize)  /* I - Number of channels */
{
  ...
  case SGI_WRITE:
    if (xsize < 1 || ysize < 1 || zsize < 1 || bpp < 1 || bpp > 2 ||
...
int sgiPutRow(sgi_t *sgip,         /* I - SGI image */
  ...
  case SGI_COMP_ARLE:
    ...
    if (sgip->bpp == 1) {
      do {
        ...
        if ((sgip->arle_length = read_rle8(sgip, sgip->arle_row, sgip->xsize)) <
            0) {
          x = 0;
          break;
        };
      ...
      do {
        sgip->arle_offset = ftell(sgip->file);
        if ((sgip->arle_length =
                 read_rle16(sgip, sgip->arle_row, sgip->xsize)) < 0) {
          x = 0;
          break;
        };
...
static int getlong(sgi_t *sgip) /* I - SGI image to read from */
{
  unsigned char b[4];
...
static int getshort(sgi_t *sgip) /* I - SGI image to read from */
{
  unsigned char b[2];
...
static int putlong(long n,      /* I - Long to write */
...
static int putshort(unsigned short n, /* I - Short to write */
                    sgi_t *sgip)      /* I - File to write to */
{
  if (putc(n >> 8, sgip->file) == EOF)
    return (EOF);
...
static int read_rle8(sgi_t *sgip,         /* I - SGI image to read from */
...
static int read_rle16(sgi_t *sgip,         /* I - SGI image to read from */
                      unsigned short *row, /* O - Data */
                      int xsize)           /* I - Width of data in pixels */
{
  int i,      /* Looping var */
...
static int write_rle8(sgi_t *sgip,         /* I - SGI image to write to */
...
static int write_rle16(sgi_t *sgip,         /* I - SGI image to write to */
                       unsigned short *row, /* I - Data */
                       int xsize)           /* I - Width of data in pixels */
{
  int length,            /* Length of output line */
