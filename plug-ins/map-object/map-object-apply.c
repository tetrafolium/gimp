...
void init_compute(void) {
  ...
  case MAP_PLANE:
    ...
    mapvals.normal =
        gimp_vector3_cross_product(&mapvals.firstaxis, &mapvals.secondaxis);
  ...
  case MAP_CYLINDER:
    ...
    for (i = 0; i < 2; i++) {
      cylinder_drawables[i] =
          gimp_drawable_get_by_id(mapvals.cylindermap_id[i]);
      ;
...
static void render(gdouble x, gdouble y, GimpRGB *col, gpointer data) {
  GimpVector3 pos;
...
static void show_progress(gint min, gint max, gint curr, gpointer data) {
...
void compute_image(void) {
  ...
  if (mapvals.create_new_image || mapvals.create_new_layer ||
      (mapvals.transparent_background &&
       !gimp_drawable_has_alpha(output_drawable))) {
    gchar *layername[] = {_("Map to plane"), _("Map to sphere"),
                          _("Map to box"), _("Map to cylinder"),
                          _("Background")};

    new_layer = gimp_layer_new(
        new_image, layername[mapvals.create_new_image ? 4 : mapvals.maptype],
        width, height,
        mapvals.transparent_background ? GIMP_RGBA_IMAGE : GIMP_RGB_IMAGE,
  ...
  if (!mapvals.antialiasing) {
    ...
    gimp_adaptive_supersample_area(0, 0, width - 1, height - 1, max_depth,
                                   mapvals.pixelthreshold, render, NULL, poke,
                                   NULL, show_progress, NULL);
  }
