...
static void compute_preview(gint x, gint y, gint w, gint h, gint pw, gint ph);
static void draw_light_marker(cairo_t *cr, gint xpos, gint ypos);
static void draw_line(cairo_t *cr, gint startx, gint starty, gint pw, gint ph,
                      gdouble cx1, gdouble cy1, gdouble cx2, gdouble cy2,
                      GimpVector3 a, GimpVector3 b);
static void draw_wireframe(cairo_t *cr, gint startx, gint starty, gint pw,
                           gint ph);
static void draw_preview_wireframe(cairo_t *cr);
static void draw_wireframe_plane(cairo_t *cr, gint startx, gint starty, gint pw,
                                 gint ph);
static void draw_wireframe_sphere(cairo_t *cr, gint startx, gint starty,
                                  gint pw, gint ph);
static void draw_wireframe_box(cairo_t *cr, gint startx, gint starty, gint pw,
                               gint ph);
static void draw_wireframe_cylinder(cairo_t *cr, gint startx, gint starty,
                                    gint pw, gint ph);
...
static void compute_preview(gint x, gint y, gint w, gint h, gint pw, gint ph) {
  ...
  gimp_rgba_set(&lightcheck, GIMP_CHECK_LIGHT, GIMP_CHECK_LIGHT,
                GIMP_CHECK_LIGHT, 1.0);
  gimp_rgba_set(&darkcheck, GIMP_CHECK_DARK, GIMP_CHECK_DARK, GIMP_CHECK_DARK,
                1.0);
      ...
      if (color.a < 1.0) {
        ...
        if (f1) {
          if (color.a == 0.0)
            ...
            gimp_rgb_composite(&color, &lightcheck, GIMP_RGB_COMPOSITE_BEHIND);
        } else {
          if (color.a == 0.0)
            color = darkcheck;
          else
            gimp_rgb_composite(&color, &darkcheck, GIMP_RGB_COMPOSITE_BEHIND);
        }
      }
...
static void draw_lights(cairo_t *cr, gint startx, gint starty, gint pw,
                        gint ph) {
  gdouble dxpos, dypos;
  gint xpos, ypos;
  ...
  if (xpos >= 0 && xpos <= PREVIEW_WIDTH && ypos >= 0 &&
      ypos <= PREVIEW_HEIGHT) {
...
void update_light(gint xpos, gint ypos) {
  ...
  gimp_vector_2d_to_3d(startx, starty, pw, ph, xpos, ypos, &mapvals.viewpoint,
...
gboolean preview_draw(GtkWidget *widget, cairo_t *cr) {
  gint startx, starty, pw, ph;
...
void draw_preview_wireframe(cairo_t *cr) {
  gint startx, starty, pw, ph;
...
void draw_wireframe(cairo_t *cr, gint startx, gint starty, gint pw, gint ph) {
...
static void draw_wireframe_plane(cairo_t *cr, gint startx, gint starty, gint pw,
  ...
  dir1 = v1;
  gimp_vector3_normalize(&dir1);
  dir2 = v2;
  gimp_vector3_normalize(&dir2);
  ...
  for (cnt = 0; cnt <= WIRESIZE; cnt++) {
    gimp_vector_3d_to_2d(startx, starty, pw, ph, &x1, &y1, &mapvals.viewpoint,
                         &a);
    gimp_vector_3d_to_2d(startx, starty, pw, ph, &x2, &y2, &mapvals.viewpoint,
                         &b);
...
static void draw_wireframe_cylinder(cairo_t *cr, gint startx, gint starty,
  ...
  for (i = 0; i < 7; i++) {
    ...
    draw_line(cr, startx, starty, pw, ph, cx1, cy1, cx2, cy2, p[i + 8],
              p[i + 9]);
