...
static GimpRGB phong_shade(GimpVector3 *pos, GimpVector3 *viewpoint,
                           GimpVector3 *normal, GimpRGB *diff_col,
                           GimpRGB *spec_col, LightType type) {
...
static gint plane_intersect(GimpVector3 *dir, GimpVector3 *viewp,
                            GimpVector3 *ipos, gdouble *u, gdouble *v) {
...
GimpRGB get_ray_color_plane(GimpVector3 *pos) {
  ...
  if (plane_intersect(&ray, &mapvals.viewpoint, &spos, &vx, &vy) == TRUE) {
    ...
    if (color.a != 0.0 && inside == TRUE &&
      ...
      color = phong_shade(&spos, &mapvals.viewpoint, &mapvals.normal, &color,
  ...
  if (mapvals.transparent_background == FALSE && color.a < 1.0) {
    gimp_rgb_composite(&color, &background, GIMP_RGB_COMPOSITE_BEHIND);
  }
...
static void sphere_to_image(GimpVector3 *normal, gdouble *u, gdouble *v) {
  ...
  *v = alpha / G_PI;
  ...
  if (*v == 0.0 || *v == 1.0) {
    ...
    *u = acos(fac) / (2.0 * G_PI);
    ...
    cross_prod =
        gimp_vector3_cross_product(&mapvals.secondaxis, &mapvals.firstaxis);
...
static gint sphere_intersect(GimpVector3 *dir, GimpVector3 *viewp,
                             GimpVector3 *spos1, GimpVector3 *spos2) {
...
GimpRGB get_ray_color_sphere(GimpVector3 *pos) {
  ...
  if (sphere_intersect(&ray, &mapvals.viewpoint, &spos1, &spos2) == TRUE) {
    ...
    if (color.a < 1.0) {
      ...
      color = phong_shade(&spos1, &mapvals.viewpoint, &normal, &color,
                          &mapvals.lightsource.color, mapvals.lightsource.type);
      ...
      color2 =
          phong_shade(&spos2, &mapvals.viewpoint, &normal, &color2,
                      &mapvals.lightsource.color, mapvals.lightsource.type);
      ...
      gimp_rgb_clamp(&color);
    } else if (color.a != 0.0 && inside == TRUE &&
               mapvals.lightsource.type != NO_LIGHT) {
      /* Compute shading at this point */
      /* ============================= */
...
static gboolean intersect_rect(gdouble u, gdouble v, gdouble w,
...
static gboolean intersect_box(GimpVector3 scale, GimpVector3 viewp,
                              GimpVector3 dir,
  ...
  if (intersect_rect(scale.x, scale.y, scale.z / 2.0, viewp, dir,
  ...
  if (i < 2) {
    /* Top: Rotate viewpoint and direction into rectangle's local coordinate
     * system */
    /* ============================================================================
     */
    ...
    if (intersect_rect(scale.x, scale.z, scale.y / 2.0, v, d,
                       &face_intersect[i]) == TRUE) {
  ...
  if (i < 2) {
    /* Bottom: Rotate viewpoint and direction into rectangle's local coordinate
     * system */
    /* ===============================================================================
     */
  ...
  if (i < 2) {
    /* Left side: Rotate viewpoint and direction into rectangle's local
     * coordinate system */
    /* ==================================================================================
     */
    ...
    if (intersect_rect(scale.z, scale.y, scale.x / 2.0, v, d,
                       &face_intersect[i]) == TRUE) {
  ...
  if (i < 2) {
    /* Right side: Rotate viewpoint and direction into rectangle's local
     * coordinate system */
    /* ===================================================================================
     */
...
GimpRGB get_ray_color_box(GimpVector3 *pos) {
  ...
  if (intersect_box(mapvals.scale, lvp, ldir, face_intersect) == TRUE) {
    ...
    color = get_box_image_color(face_intersect[0].face, face_intersect[0].u,
                                face_intersect[0].v);
    ...
    if (color.a < 1.0) {
      ...
      color = phong_shade(&face_intersect[0].s, &mapvals.viewpoint,
                          &face_intersect[0].n, &color,
                          &mapvals.lightsource.color, mapvals.lightsource.type);

      gimp_rgb_clamp(&color);

      color2 = get_box_image_color(face_intersect[1].face, face_intersect[1].u,
                                   face_intersect[1].v);
      ...
      color2 = phong_shade(
          &face_intersect[1].s, &mapvals.viewpoint, &face_intersect[1].n,
          &color2, &mapvals.lightsource.color, mapvals.lightsource.type);
...
static gboolean intersect_circle(GimpVector3 vp, GimpVector3 dir, gdouble w,
                                 FaceIntersectInfo *face_info) {
  gboolean result = FALSE;
  gdouble r, d;
...
static gboolean intersect_cylinder(GimpVector3 vp, GimpVector3 dir,
                                   FaceIntersectInfo *face_intersect) {
  ...
  if (d >= 0.0) {
    ...
    if (f != 0.0) {
      ...
      for (i = 0; i < 2; i++) {
        ...
        face_intersect[i].u =
            (atan2(face_intersect[i].s.x, face_intersect[i].s.z) + G_PI) /
            (2.0 * G_PI);
        face_intersect[i].v =
            (face_intersect[i].s.y + l) / mapvals.cylinder_length;
...
static GimpRGB get_cylinder_color(gint face, gdouble u, gdouble v) {
  GimpRGB color;
  gint inside;
...
GimpRGB get_ray_color_cylinder(GimpVector3 *pos) {
  ...
  if (intersect_cylinder(lvp, ldir, face_intersect) == TRUE) {
    ...
    color = get_cylinder_color(face_intersect[0].face, face_intersect[0].u,
                               face_intersect[0].v);
    ...
    if (color.a < 1.0) {
      ...
      color2 = get_cylinder_color(face_intersect[1].face, face_intersect[1].u,
                                  face_intersect[1].v);
