...
static void double_adjustment_update(GtkAdjustment *adjustment, gpointer data);

static void toggle_update(GtkWidget *widget, gpointer data);

static void lightmenu_callback(GtkWidget *widget, gpointer data);

static void preview_callback(GtkWidget *widget, gpointer data);

static gint box_constrain(GimpImage *image, GimpItem *item, gpointer data);
static gint cylinder_constrain(GimpImage *image, GimpItem *item, gpointer data);
...
static void scale_entry_update_double(GimpLabelSpin *entry, gdouble *value) {
...
static void update_light_pos_entries(void) {
  g_signal_handlers_block_by_func(xadj, double_adjustment_update,
                                  &mapvals.lightsource.position.x);
  gtk_adjustment_set_value(xadj, mapvals.lightsource.position.x);
  g_signal_handlers_unblock_by_func(xadj, double_adjustment_update,
                                    &mapvals.lightsource.position.x);

  g_signal_handlers_block_by_func(yadj, double_adjustment_update,
                                  &mapvals.lightsource.position.y);
  gtk_adjustment_set_value(yadj, mapvals.lightsource.position.y);
  g_signal_handlers_unblock_by_func(yadj, double_adjustment_update,
                                    &mapvals.lightsource.position.y);

  g_signal_handlers_block_by_func(zadj, double_adjustment_update,
                                  &mapvals.lightsource.position.z);
  gtk_adjustment_set_value(zadj, mapvals.lightsource.position.z);
  g_signal_handlers_unblock_by_func(zadj, double_adjustment_update,
                                    &mapvals.lightsource.position.z);
}
...
static void lightmenu_callback(GtkWidget *widget, gpointer data) {
  ...
  gimp_int_combo_box_get_active(GIMP_INT_COMBO_BOX(widget), &active);
...
static void mapmenu_callback(GtkWidget *widget, gpointer data) {
  int active;
...
static void zoomed_callback(GimpZoomModel *model) {
...
static gint cylinder_constrain(GimpImage *image, GimpItem *item,
                               gpointer data) {
  return (gimp_drawable_is_rgb(GIMP_DRAWABLE(item)) &&
          !gimp_drawable_is_indexed(GIMP_DRAWABLE(item)));
}
  ...
  case GDK_MOTION_NOTIFY:
    if (left_button_pressed == TRUE) {
      if (light_hit == TRUE) {
        ...
        pos.x =
            -(2.0 * (gdouble)event->motion.x / (gdouble)PREVIEW_WIDTH - 1.0);
        pos.y =
            (2.0 * (gdouble)event->motion.y / (gdouble)PREVIEW_HEIGHT - 1.0);
        /*                ArcBall_Mouse(pos);
                        ArcBall_Update();
...
static GtkWidget *create_options_page(void) {
  ...
  combo = gimp_int_combo_box_new(_("Plane"), MAP_PLANE, _("Sphere"), MAP_SPHERE,
                                 _("Box"), MAP_BOX, _("Cylinder"), MAP_CYLINDER,
                                 NULL);
  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), mapvals.maptype);
  gtk_box_pack_start(GTK_BOX(hbox), combo, TRUE, TRUE, 0);
  gtk_widget_show(combo);

  g_signal_connect(combo, "changed", G_CALLBACK(mapmenu_callback),
                   &mapvals.maptype);
  ...
  gimp_help_set_help_data(toggle, _("Make image transparent outside object"),
                          NULL);

  toggle = gtk_check_button_new_with_label(_("Tile source image"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle), mapvals.tiled);
  gtk_box_pack_start(GTK_BOX(vbox), toggle, FALSE, FALSE, 0);
  gtk_widget_show(toggle);
  ...
  gimp_help_set_help_data(
      toggle, _("Tile source image: useful for infinite planes"), NULL);
  ...
  gimp_help_set_help_data(toggle, _("Create a new layer when applying filter"),
                          NULL);
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle), mapvals.antialiasing);
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double), &mapvals.maxdepth);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 0, 3, 1);
  gtk_widget_show(scale);

  spinbutton = spin_button_new(&adj, mapvals.pixelthreshold, 0.001, 1000, 0.1,
                               1, 0, 0, 3);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Threshold:"), 0.0, 0.5,
                           spinbutton, 1);

  g_signal_connect(adj, "value-changed", G_CALLBACK(double_adjustment_update),
...
static GtkWidget *create_light_page(void) {
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Lightsource type:"), 0.0,
                           0.5, combo, 1);

  g_signal_connect(combo, "changed", G_CALLBACK(lightmenu_callback),
                   &mapvals.lightsource.type);

  gimp_help_set_help_data(combo, _("Type of light source to apply"), NULL);

  colorbutton =
      gimp_color_button_new(_("Select lightsource color"), 64, 16,
                            &mapvals.lightsource.color, GIMP_COLOR_AREA_FLAT);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Lightsource color:"), 0.0,
                           0.5, colorbutton, 1);
  ...
  spinbutton = spin_button_new(&xadj, mapvals.lightsource.position.x,
                               -G_MAXFLOAT, G_MAXFLOAT, 0.1, 1.0, 0.0, 0.0, 2);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("X:"), 0.0, 0.5, spinbutton,
                           1);

  g_signal_connect(xadj, "value-changed", G_CALLBACK(double_adjustment_update),
                   &mapvals.lightsource.position.x);

  gimp_help_set_help_data(spinbutton, _("Light source X position in XYZ space"),
                          NULL);

  spinbutton = spin_button_new(&yadj, mapvals.lightsource.position.y,
                               -G_MAXFLOAT, G_MAXFLOAT, 0.1, 1.0, 0.0, 0.0, 2);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Y:"), 0.0, 0.5, spinbutton,
                           1);

  g_signal_connect(yadj, "value-changed", G_CALLBACK(double_adjustment_update),
                   &mapvals.lightsource.position.y);

  gimp_help_set_help_data(spinbutton, _("Light source Y position in XYZ space"),
                          NULL);

  spinbutton = spin_button_new(&zadj, mapvals.lightsource.position.z,
                               -G_MAXFLOAT, G_MAXFLOAT, 0.1, 1.0, 0.0, 0.0, 2);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Z:"), 0.0, 0.5, spinbutton,
                           1);

  g_signal_connect(zadj, "value-changed", G_CALLBACK(double_adjustment_update),
                   &mapvals.lightsource.position.z);

  gimp_help_set_help_data(spinbutton, _("Light source Z position in XYZ space"),
                          NULL);
  ...
  spinbutton = spin_button_new(&adj, mapvals.lightsource.direction.x, -1.0, 1.0,
                               0.01, 0.1, 0.0, 0.0, 2);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("X:"), 0.0, 0.5, spinbutton,
                           1);
  ...
  spinbutton = spin_button_new(&adj, mapvals.lightsource.direction.y, -1.0, 1.0,
                               0.01, 0.1, 0.0, 0.0, 2);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Y:"), 0.0, 0.5, spinbutton,
                           1);
  ...
  spinbutton = spin_button_new(&adj, mapvals.lightsource.direction.z, -1.0, 1.0,
                               0.01, 0.1, 0.0, 0.0, 2);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Z:"), 0.0, 0.5, spinbutton,
                           1);
...
static GtkWidget *create_material_page(void) {
  ...
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Ambient:"), 0.0,
                                   0.5, image, 1);
  gtk_size_group_add_widget(group, label);

  spinbutton = spin_button_new(&adj, mapvals.material.ambient_int, 0,
                               G_MAXFLOAT, 0.1, 1.0, 0.0, 0.0, 2);
  ...
  image = gtk_image_new_from_icon_name(MAPOBJECT_INTENSITY_DIFFUSE_LOW,
                                       GTK_ICON_SIZE_BUTTON);
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Diffuse:"), 0.0,
                                   0.5, image, 1);
  gtk_size_group_add_widget(group, label);
  ...
  image = gtk_image_new_from_icon_name(MAPOBJECT_REFLECTIVITY_DIFFUSE_LOW,
                                       GTK_ICON_SIZE_BUTTON);
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Diffuse:"), 0.0,
                                   0.5, image, 1);
  gtk_size_group_add_widget(group, label);

  spinbutton = spin_button_new(&adj, mapvals.material.diffuse_ref, 0,
                               G_MAXFLOAT, 0.1, 1.0, 0.0, 0.0, 2);
  gtk_grid_attach(GTK_GRID(grid), spinbutton, 2, 0, 1, 1);
  gtk_widget_show(spinbutton);
  ...
  image = gtk_image_new_from_icon_name(MAPOBJECT_REFLECTIVITY_SPECULAR_LOW,
                                       GTK_ICON_SIZE_BUTTON);
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Specular:"), 0.0,
                                   0.5, image, 1);
  gtk_size_group_add_widget(group, label);
  ...
  image = gtk_image_new_from_icon_name(MAPOBJECT_REFLECTIVITY_HIGHLIGHT_LOW,
                                       GTK_ICON_SIZE_BUTTON);
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Highlight:"), 0.0,
                                   0.5, image, 1);
  gtk_size_group_add_widget(group, label);
...
static GtkWidget *create_orientation_page(void) {
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double), &mapvals.position.x);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 0, 3, 1);
  gtk_widget_show(scale);
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double), &mapvals.position.y);
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double), &mapvals.position.z);
  ...
  gtk_size_group_add_widget(
      group, gimp_label_spin_get_spin_button(GIMP_LABEL_SPIN(scale)));

  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double), &mapvals.alpha);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 0, 3, 1);
  gtk_widget_show(scale);
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double), &mapvals.beta);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 1, 3, 1);
  gtk_widget_show(scale);
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double), &mapvals.gamma);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 2, 3, 1);
  gtk_widget_show(scale);
...
static GtkWidget *create_box_page(void) {
  ...
  static gchar *labels[] = {N_("Front:"),  N_("Back:"), N_("Top:"),
                            N_("Bottom:"), N_("Left:"), N_("Right:")};
  ...
  for (i = 0; i < 6; i++) {
    ...
    gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), mapvals.boxmap_id[i],
                               G_CALLBACK(gimp_int_combo_box_get_active),
                               &mapvals.boxmap_id[i], NULL);

    gimp_grid_attach_aligned(GTK_GRID(grid), 0, i, gettext(labels[i]), 0.0, 0.5,
                             combo, 1);
  }
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double), &mapvals.scale.x);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 0, 3, 1);
  gtk_widget_show(scale);
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double), &mapvals.scale.y);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 1, 3, 1);
  gtk_widget_show(scale);
  ...
  g_signal_connect(scale, "value-changed",
                   G_CALLBACK(scale_entry_update_double), &mapvals.scale.z);
  gtk_grid_attach(GTK_GRID(grid), scale, 0, 2, 3, 1);
  gtk_widget_show(scale);
...
static GtkWidget *create_cylinder_page(void) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, i, gettext(labels[i]),
                                     0.0, 0.5, combo, 1);
    gtk_size_group_add_widget(group, label);
  }
  ...
  scale =
      gimp_scale_entry_new(_("R_adius:"), mapvals.cylinder_radius, 0.0, 2.0, 2);
  ...
  scale =
      gimp_scale_entry_new(_("L_ength:"), mapvals.cylinder_length, 0.0, 2.0, 2);
...
gboolean main_dialog(GimpDrawable *drawable) {
  ...
  appwin = gimp_dialog_new(_("Map to Object"), PLUG_IN_ROLE, NULL, 0,
  ...
  g_object_set(gtk_bin_get_child(GTK_BIN(button)), "margin-start", 2,
               "margin-end", 2, NULL);
  ...
  g_signal_connect(model, "zoomed", G_CALLBACK(zoomed_callback), NULL);
