...
static GimpValueArray *map_run(GimpProcedure *procedure, GimpRunMode run_mode,
...
static GimpProcedure *map_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    ...
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, map_run, NULL, NULL);
    ...
    gimp_procedure_set_attribution(
        procedure, "Tom Bech & Federico Mena Quintero",
        "Tom Bech & Federico Mena Quintero", "Version 1.2.0, July 16 1998");

    GIMP_PROC_ARG_INT(procedure, "map-type", "Map type",
                      "Type of mapping (0=plane, 1=sphere, 2=box, "
                      "3=cylinder)",
                      0, 2, MAP_PLANE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "viewpoint-x", "Viewpoint X",
                         "Position of viewpoint (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0.5, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "viewpoint-y", "Viewpoint Y",
                         "Position of viewpoint (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0.5, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "viewpoint-z", "Viewpoint Z",
                         "Position of viewpoint (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 2.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "position-x", "Position X",
                         "Object position (x,y,z)", -G_MAXDOUBLE, G_MAXDOUBLE,
                         0.5, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "position-y", "Position Y",
                         "Object position (x,y,z)", -G_MAXDOUBLE, G_MAXDOUBLE,
                         0.5, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "position-z", "Position Z",
                         "Object position (x,y,z)", -G_MAXDOUBLE, G_MAXDOUBLE,
                         0.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "first-axis-x", "First axis X",
                         "First axis of object (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 1.0, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "first-axis-y", "First axis y",
                         "First axis of object (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0.0, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "first-axis-z", "First axis Z",
                         "First axis of object (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "second-axis-x", "Second axis X",
                         "Second axis of object (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0.0, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "second-axis-y", "Second axis Y",
                         "Second axis of object (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 1.0, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "second-axis-z", "Second axis Z",
                         "Second axis of object (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "rotation-angle-x", "Rotation angle X",
                         "Rotation about X axis in degrees", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0.0, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "rotation-angle-y", "Rotation angle Y",
                         "Rotation about Y axis in degrees", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0.0, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "rotation-angle-z", "Rotation angle Z",
                         "Rotation about Z axis in degrees", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 0.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "light-type", "Light type",
                      "Type of lightsource (0=point, 1=directional, 2=none)", 0,
                      2, POINT_LIGHT, G_PARAM_READWRITE);

    GIMP_PROC_ARG_RGB(procedure, "light-color", "Light color",
                      "Light source color", TRUE, &white, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "light-position-x", "Light position X",
                         "Light source position (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, -0.5, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "light-position-y", "Light position Y",
                         "Light source position (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, -0.5, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "light-position-z", "Light position Z",
                         "Light source position (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 2.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "light-direction-x", "Light direction X",
                         "Light source direction (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, -1.0, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "light-direction-y", "Light direction Y",
                         "Light source direction (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, -1.0, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "light-direction-z", "Light direction Z",
                         "Light source direction (x,y,z)", -G_MAXDOUBLE,
                         G_MAXDOUBLE, 1.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "ambient-intensity", "Ambient intensity",
                         "Material ambient intensity", 0, 1, 0.3,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "diffuse-intensity", "Diffuse intensity",
                         "Material diffuse intensity", 0, 1, 1.0,
                         G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(
        procedure, "diffuse-reflectivity", "Diffuse reflectivity",
        "Material diffuse reflectivity", 0, 1, 0.5, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(
        procedure, "specular-reflectivity", "Specular reflectivity",
        "Material specular reflectivity", 0, 1, 0.5, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "highlight", "Highlight",
                         "Material highlight (note, it's exponential)", 0,
                         G_MAXDOUBLE, 27.0, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "antialiasing", "Antialiasing",
                          "Apply antialiasing", TRUE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "tiled", "Tiled", "Tile source image",
                          FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "new-image", "New image",
                          "Create a new image", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(
        procedure, "transparent-background", "Transparent background",
        "Make background transparent", FALSE, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "radius", "Radius",
                         "Sphere/cylinder radius (only used when "
                         "maptype=1 or 3)",
                         0, G_MAXDOUBLE, 0.25, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "x-scale", "X scale", "Box X size", 0,
                         G_MAXDOUBLE, 0.5, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "y-scale", "Y scale", "Box Y size", 0,
                         G_MAXDOUBLE, 0.5, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DOUBLE(procedure, "z-scale", "Z scale", "Box Z size", 0,
                         G_MAXDOUBLE, 0.5, G_PARAM_READWRITE);

    GIMP_PROC_ARG_DOUBLE(procedure, "cylinder-length", "Cylinder length",
                         "Cylinder length", 0, G_MAXDOUBLE, 0.25,
                         G_PARAM_READWRITE);
    ...
    GIMP_PROC_ARG_DRAWABLE(procedure, "box-back-drawable", "Box back drawable",
                           "Box back face", TRUE, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DRAWABLE(procedure, "box-top-drawable", "Box top drawable",
                           "Box top face", TRUE, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DRAWABLE(procedure, "box-bottom-drawable",
                           "Box bottom drawable", "Box bottom face", TRUE,
                           G_PARAM_READWRITE);
    GIMP_PROC_ARG_DRAWABLE(procedure, "box-left-drawable", "Box left drawable",
                           "Box left face", TRUE, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DRAWABLE(procedure, "box-right-drawable",
                           "Box right drawable", "Box right face", TRUE,
                           G_PARAM_READWRITE);

    GIMP_PROC_ARG_DRAWABLE(procedure, "cyl-top-drawable", "Cyl top drawable",
                           "Cylinder top face (set these to NULL if not used)",
                           TRUE, G_PARAM_READWRITE);
    GIMP_PROC_ARG_DRAWABLE(procedure, "cyl-bottom-drawable",
                           "Cyl bottom drawable", "Cylinder bottom face", TRUE,
                           G_PARAM_READWRITE);
  }
...
static GimpValueArray *map_run(GimpProcedure *procedure, GimpRunMode run_mode,
  ...
  case GIMP_RUN_NONINTERACTIVE:
    ...
    mapvals.radius = mapvals.cylinder_radius = GIMP_VALUES_GET_DOUBLE(args, 33);
