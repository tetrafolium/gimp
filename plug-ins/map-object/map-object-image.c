...
GimpRGB peek(gint x, gint y) {
  GimpRGB color;

  gegl_buffer_sample(source_buffer, x, y, NULL, &color,
                     babl_format("R'G'B'A double"), GEGL_SAMPLER_NEAREST,
                     GEGL_ABYSS_NONE);
...
static GimpRGB peek_box_image(gint image, gint x, gint y) {
  GimpRGB color;

  gegl_buffer_sample(box_buffers[image], x, y, NULL, &color,
                     babl_format("R'G'B'A double"), GEGL_SAMPLER_NEAREST,
                     GEGL_ABYSS_NONE);
...
static GimpRGB peek_cylinder_image(gint image, gint x, gint y) {
  GimpRGB color;

  gegl_buffer_sample(cylinder_buffers[image], x, y, NULL, &color,
                     babl_format("R'G'B'A double"), GEGL_SAMPLER_NEAREST,
                     GEGL_ABYSS_NONE);
...
void poke(gint x, gint y, GimpRGB *color, gpointer user_data) {
...
gint checkbounds(gint x, gint y) {
  if (x < border_x || y < border_y || x >= border_x + border_w ||
      y >= border_y + border_h)
    return FALSE;
  else
    return TRUE;
...
static gint checkbounds_box_image(gint image, gint x, gint y) {
  gint w, h;
...
static gint checkbounds_cylinder_image(gint image, gint x, gint y) {
  gint w, h;
...
GimpRGB get_image_color(gdouble u, gdouble v, gint *inside) {
  gint x1, y1, x2, y2;
  GimpRGB p[4];
...
GimpRGB get_box_image_color(gint image, gdouble u, gdouble v) {
  gint w, h;
  gint x1, y1, x2, y2;
  GimpRGB p[4];
...
GimpRGB get_cylinder_image_color(gint image, gdouble u, gdouble v) {
  gint w, h;
  gint x1, y1, x2, y2;
  GimpRGB p[4];
...
gint image_setup(GimpDrawable *drawable, gint interactive) {
  ...
  if (interactive == TRUE) {
    preview_rgb_stride =
        cairo_format_stride_for_width(CAIRO_FORMAT_RGB24, PREVIEW_WIDTH);
    preview_rgb_data = g_new0(guchar, preview_rgb_stride * PREVIEW_HEIGHT);
    preview_surface = cairo_image_surface_create_for_data(
        preview_rgb_data, CAIRO_FORMAT_RGB24, PREVIEW_WIDTH, PREVIEW_HEIGHT,
        preview_rgb_stride);
  }
