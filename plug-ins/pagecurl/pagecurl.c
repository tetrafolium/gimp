...
#define CURL_EDGE_LEFT(e)                                                      \
  ((e) == CURL_EDGE_LOWER_LEFT || (e) == CURL_EDGE_UPPER_LEFT)
#define CURL_EDGE_RIGHT(e)                                                     \
  ((e) == CURL_EDGE_LOWER_RIGHT || (e) == CURL_EDGE_UPPER_RIGHT)
#define CURL_EDGE_LOWER(e)                                                     \
  ((e) == CURL_EDGE_LOWER_RIGHT || (e) == CURL_EDGE_LOWER_LEFT)
#define CURL_EDGE_UPPER(e)                                                     \
  ((e) == CURL_EDGE_UPPER_LEFT || (e) == CURL_EDGE_UPPER_RIGHT)
...
#define PAGECURL                                                               \
  (obj)(G_TYPE_CHECK_INSTANCE_CAST((obj), PAGECURL_TYPE, Pagecurl))
...
static void dialog_scale_update(GimpLabelSpin *spin, gdouble *value);
...
static GimpRGB *get_gradient_samples(GimpDrawable *drawable, gboolean reverse);
...
static void pagecurl_class_init(PagecurlClass *klass) {
...
static void pagecurl_init(Pagecurl *pagecurl) {}
...
static GimpProcedure *pagecurl_create_procedure(GimpPlugIn *plug_in,
  ...
  if (!strcmp(name, PLUG_IN_PROC)) {
    procedure = gimp_image_procedure_new(
        plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, pagecurl_run, NULL, NULL);
    ...
    gimp_procedure_set_documentation(
        procedure, N_("Curl up one of the image corners"),
        "This plug-in creates a pagecurl-effect.", name);
    gimp_procedure_set_attribution(
        procedure, "Federico Mena Quintero and Simon Budig",
        "Federico Mena Quintero and Simon Budig", PLUG_IN_VERSION);

    GIMP_PROC_ARG_INT(procedure, "colors", "Colors",
                      "FG- and BG-Color (0), Current gradient (1), "
                      "Current gradient reversed (2)",
                      CURL_COLORS_FG_BG, CURL_COLORS_LAST, CURL_COLORS_FG_BG,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "edge", "Edge",
                      "Edge to curl (1-4, clockwise, starting in the "
                      "lower right edge)",
                      CURL_EDGE_LOWER_RIGHT, CURL_EDGE_UPPER_RIGHT,
                      CURL_EDGE_LOWER_RIGHT, G_PARAM_READWRITE);

    GIMP_PROC_ARG_INT(procedure, "orientation", "Orientation",
                      "Vertical (0), Horizontal (1)", CURL_ORIENTATION_VERTICAL,
                      CURL_ORIENTATION_HORIZONTAL, CURL_ORIENTATION_VERTICAL,
                      G_PARAM_READWRITE);

    GIMP_PROC_ARG_BOOLEAN(procedure, "shade", "Shade",
                          "Shade the region under the curl", TRUE,
                          G_PARAM_READWRITE);

    GIMP_PROC_VAL_LAYER(procedure, "curl-layer", "Curl layer",
                        "The new layer with the curl.", FALSE,
                        G_PARAM_READWRITE);
  }
...
static GimpValueArray *pagecurl_run(GimpProcedure *procedure,
  ...
  if (!gimp_drawable_mask_intersect(drawable, &sel_x, &sel_y, &true_sel_width,
                                    &true_sel_height)) {
...
static inline gboolean left_of_diagl(gdouble x, gdouble y) {
  return (x < (sel_width + (y - sel_height) * diagl_slope));
}

static inline gboolean right_of_diagr(gdouble x, gdouble y) {
  return (x > (sel_width + (y - sel_height) * diagr_slope));
}

static inline gboolean below_diagb(gdouble x, gdouble y) {
  return (y < (right_tangent.y + (x - right_tangent.x) * diagb_slope));
}

static inline gboolean right_of_diagm(gdouble x, gdouble y) {
  return (x > (sel_width + (y - sel_height) * diagm_slope));
}

static inline gboolean inside_circle(gdouble x, gdouble y) {
  x -= center.x;
  y -= center.y;
...
static void dialog_scale_update(GimpLabelSpin *scale, gdouble *value) {
  *value = ((gdouble)gimp_label_spin_get_value(scale)) / 100.0;
}

static void curl_pixbuf_update(void) {
  GdkPixbuf *pixbuf;
  gint index;
  gchar *resource;

  switch (curl.edge) {
  case CURL_EDGE_LOWER_RIGHT:
    index = 0;
    break;
  case CURL_EDGE_LOWER_LEFT:
    index = 1;
    break;
  case CURL_EDGE_UPPER_RIGHT:
    index = 2;
    break;
  case CURL_EDGE_UPPER_LEFT:
    index = 3;
    break;
  default:
    return;
  ...
  resource =
      g_strdup_printf("/org/gimp/pagecurl-icons/curl%c.png", '0' + index);
...
static gboolean dialog(void) {
  ...
  dialog = gimp_dialog_new(_("Pagecurl Effect"), PLUG_IN_ROLE, NULL, 0,
  ...
  button = gimp_brush_select_button_new("Brush Test", NULL, 1.0, 20,
                                        GIMP_LAYER_MODE_NORMAL);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, "Brush Select:", 0.0, 0.5,
                           button, 1);

  button = gimp_gradient_select_button_new("Gradient Test", NULL);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, "Gradient Select:", 0.0, 0.5,
                           button, 1);

  button = gimp_palette_select_button_new("Palette Test", NULL);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, "Palette Select:", 0.0, 0.5,
                           button, 1);

  button = gimp_font_select_button_new("Font Test", NULL);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, "Font Select:", 0.0, 0.5,
                           button, 1);

  button = gimp_pattern_select_button_new("Pattern Test", NULL);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 4, "Pattern Select:", 0.0, 0.5,
                           button, 1);
  ...
  {
    static const gchar *name[] = {N_("Lower right"), N_("Lower left"),
                                  N_("Upper left"), N_("Upper right")};
    gint i;
    ...
    for (i = CURL_EDGE_FIRST; i <= CURL_EDGE_LAST; i++) {
      ...
      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button), curl.edge == i);

      g_object_set_data(G_OBJECT(button), "gimp-item-data", GINT_TO_POINTER(i));

      gtk_grid_attach(GTK_GRID(grid), button, CURL_EDGE_LEFT(i) ? 0 : 1,
                      CURL_EDGE_UPPER(i) ? 0 : 2, 1, 1);
      // GTK_FILL | GTK_EXPAND, GTK_SHRINK, 0, 0);
      gtk_widget_show(button);

      g_signal_connect(button, "toggled", G_CALLBACK(gimp_radio_button_update),
                       &curl.edge);

      g_signal_connect(button, "toggled", G_CALLBACK(curl_pixbuf_update), NULL);
    }
  }
  ...
  {
    static const gchar *name[] = {N_("_Vertical"), N_("_Horizontal")};
    gint i;
  ...
  gimp_int_combo_box_prepend(GIMP_INT_COMBO_BOX(combo), GIMP_INT_STORE_VALUE,
                             CURL_COLORS_GRADIENT_REVERSE, GIMP_INT_STORE_LABEL,
                             _("Current gradient (reversed)"),
                             GIMP_INT_STORE_ICON_NAME, GIMP_ICON_GRADIENT, -1);
  gimp_int_combo_box_prepend(GIMP_INT_COMBO_BOX(combo), GIMP_INT_STORE_VALUE,
                             CURL_COLORS_GRADIENT, GIMP_INT_STORE_LABEL,
                             _("Current gradient"), GIMP_INT_STORE_ICON_NAME,
                             GIMP_ICON_GRADIENT, -1);
  gimp_int_combo_box_prepend(
      GIMP_INT_COMBO_BOX(combo), GIMP_INT_STORE_VALUE, CURL_COLORS_FG_BG,
      GIMP_INT_STORE_LABEL, _("Foreground / background colors"),
      GIMP_INT_STORE_ICON_NAME, GIMP_ICON_COLORS_DEFAULT, -1);

  gtk_box_pack_start(GTK_BOX(vbox), combo, FALSE, FALSE, 0);
  gtk_widget_show(combo);

  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), curl.colors,
                             G_CALLBACK(gimp_int_combo_box_get_active),
                             &curl.colors, NULL);

  scale = gimp_scale_entry_new(_("_Opacity:"), curl.opacity * 100.0, 0.0, 100.0,
                               0.0);
  g_signal_connect(scale, "value-changed", G_CALLBACK(dialog_scale_update),
                   &curl.opacity);
...
static void init_calculation(GimpDrawable *drawable) {
  ...
  gimp_vector2_set(&right_tangent, v1.x * cos(angle) + v1.y * -sin(angle),
  ...
  diagb_slope =
      ((right_tangent.y - left_tangent.y) / (right_tangent.x - left_tangent.x));
...
static GimpLayer *do_curl_effect(GimpDrawable *drawable) {
  ...
  curl_layer =
      gimp_layer_new(image, _("Curl Layer"), true_sel_width, true_sel_height,
                     color_image ? GIMP_RGBA_IMAGE : GIMP_GRAYA_IMAGE, 100,
  ...
  iter = gegl_buffer_iterator_new(
      curl_buffer, GEGL_RECTANGLE(0, 0, width, height), 0, format,
      GEGL_ACCESS_WRITE, GEGL_ABYSS_NONE, 1);

  /* Init shade_under */
  gimp_vector2_set(&dl, -sel_width, -sel_height);
  dl_mag = gimp_vector2_length(&dl);
  gimp_vector2_set(&dr, -(sel_width - right_tangent.x),
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (y1 = roi->y; y1 < roi->y + roi->height; y1++) {
      ...
      for (x1 = roi->x; x1 < roi->x + roi->width; x1++) {
        ...
        if (left_of_diagl(x, y)) { /* uncurled region: transparent black */
          gimp_rgba_set(&color, 0, 0, 0, 0);
        } else if (right_of_diagr(x, y) ||
                   (right_of_diagm(x, y) && below_diagb(x, y) &&
                    !inside_circle(x, y))) {
...
static void clear_curled_region(GimpDrawable *drawable) {
  ...
  iter = gegl_buffer_iterator_new(
      shadow_buf, GEGL_RECTANGLE(0, 0, width, height), 0, format,
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (y1 = roi->y; y1 < roi->y + roi->height; y1++) {
      for (x1 = roi->x; x1 < roi->x + roi->width; x1++) {
        ...
        switch (curl.orientation) {
        case CURL_ORIENTATION_VERTICAL:
          x = (CURL_EDGE_RIGHT(curl.edge) ? x1 - sel_x
                                          : sel_width - 1 - (x1 - sel_x));
          y = (CURL_EDGE_UPPER(curl.edge) ? y1 - sel_y
                                          : sel_height - 1 - (y1 - sel_y));
          break;

        case CURL_ORIENTATION_HORIZONTAL:
          x = (CURL_EDGE_LOWER(curl.edge) ? y1 - sel_y
                                          : sel_width - 1 - (y1 - sel_y));
          y = (CURL_EDGE_LEFT(curl.edge) ? x1 - sel_x
                                         : sel_height - 1 - (x1 - sel_x));
          break;
        }
  ...
  gimp_drawable_update(drawable, sel_x, sel_y, true_sel_width, true_sel_height);
}
...
static GimpRGB *get_gradient_samples(GimpDrawable *drawable, gboolean reverse) {
  ...
  for (i = 0; i < NGRADSAMPLES; i++) {
    gimp_rgba_set(rgba + i, d_samples[i * 4 + 0], d_samples[i * 4 + 1],
                  d_samples[i * 4 + 2], d_samples[i * 4 + 3]);
  }
