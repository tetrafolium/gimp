...
static gboolean test_clipboard_copy(GtkClipboard *clipboard,
                                    const gchar *target, const gchar *filename);
...
static const GOptionEntry main_entries[] = {
    {"selection-type", 's', 0, G_OPTION_ARG_CALLBACK,
     test_clipboard_parse_selection,
     "Selection type (primary|secondary|clipboard)", "<type>"},
    {"list-targets", 'l', 0, G_OPTION_ARG_NONE, &option_list_targets,
    ...
    {"copy", 'c', 0, G_OPTION_ARG_STRING, &option_copy_filename,
     "Copy <file> to clipboard", "<file>"},
    {"store", 'S', 0, G_OPTION_ARG_STRING, &option_store_filename,
     "Store <file> in the clipboard manager", "<file>"},
    {"paste", 'p', 0, G_OPTION_ARG_STRING, &option_paste_filename,
     "Paste clipboard into <file> ('-' pastes to STDOUT)", "<file>"},
    {"version", 'v', G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK,
     test_clipboard_show_version, "Show version information and exit", NULL},
    {NULL}};
...
gint main(gint argc, gchar *argv[]) {
  ...
  if (option_copy_filename) {
    ...
    if (!test_clipboard_copy(clipboard, option_target, option_copy_filename))
      return EXIT_FAILURE;
  }
  ...
  if (option_store_filename) {
    ...
    if (!test_clipboard_store(clipboard, option_target, option_store_filename))
      return EXIT_FAILURE;
  }
  ...
  if (option_paste_filename) {
    ...
    if (!test_clipboard_paste(clipboard, option_target, option_paste_filename))
      return EXIT_FAILURE;
  }
...
static void test_clipboard_show_version(void) {
...
static gboolean test_clipboard_copy(GtkClipboard *clipboard,
  ...
  if (!gtk_clipboard_set_with_data(clipboard, &entry, 1,
                                   test_clipboard_copy_callback, NULL, &data)) {
...
static gboolean test_clipboard_paste(GtkClipboard *clipboard,
  ...
  if (sel_data) {
    ...
    if (fd < 0) {
      g_printerr("%s: open() filed: %s", g_get_prgname(), g_strerror(errno));
      return FALSE;
    }
    ...
    if (write(fd, data, length) < length) {
      close(fd);
      g_printerr("%s: write() failed: %s", g_get_prgname(), g_strerror(errno));
      return FALSE;
    }

    if (close(fd) < 0) {
      g_printerr("%s: close() failed: %s", g_get_prgname(), g_strerror(errno));
      return FALSE;
    }
