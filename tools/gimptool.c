...
static gchar *win32_command(const gchar *command) {
...
static gboolean starts_with_dir(const gchar *string, const gchar *dir) {
  gchar *dirslash = g_strconcat(dir, "/", NULL);
  gboolean retval;

  retval = (g_str_has_prefix(string, dirslash) || g_strcmp0(string, dir) == 0);
  g_free(dirslash);

  return retval;
}

static gchar *one_line_output(const gchar *program, const gchar *args) {
...
static gchar *pkg_config(const gchar *args) {
#ifdef G_OS_WIN32
  if (msvc_syntax)
...
static gchar *get_runtime_prefix(gchar slash) {
#ifdef G_OS_WIN32
  ...
  g_printerr("Cannot determine GIMP " GIMP_APP_VERSION
             " installation location\n");

  exit(EXIT_FAILURE);
#else
  /* On Unix assume the executable package is in the same prefix as the
   * developer stuff */
  return pkg_config("--variable=prefix gimp-" GIMP_PKGCONFIG_VERSION);
#endif
}

static gchar *get_exec_prefix(gchar slash) {
#ifdef G_OS_WIN32
...
static gchar *expand_and_munge(const gchar *value) {
  gchar *retval;
...
static void find_out_env_flags(void) {
  gchar *p;
...
static gchar *get_cflags(void) {
  return pkg_config("--cflags gimpui-" GIMP_PKGCONFIG_VERSION);
}

static void do_cflags(void) {
...
static gchar *get_cflags_noui(void) {
  return pkg_config("--cflags gimp-" GIMP_PKGCONFIG_VERSION);
}

static void do_cflags_noui(void) {
...
static gchar *get_cflags_nogimpui(void) {
  return pkg_config("--cflags gimp-" GIMP_PKGCONFIG_VERSION " gtk+-3.0");
}

static void do_cflags_nogimpui(void) {
...
static gchar *get_libs_noui(void) {
  return pkg_config("--libs gimp-" GIMP_PKGCONFIG_VERSION);
}

static void do_libs_noui(void) {
...
static gchar *get_libs_nogimpui(void) {
  return pkg_config("--libs gimp-" GIMP_PKGCONFIG_VERSION " gtk+-3.0");
}

static void do_libs_nogimpui(void) {
...
static void maybe_run(gchar *cmd) {
  if (!silent)
    g_print("%s\n", cmd);
...
static void do_build_2(const gchar *cflags, const gchar *libs,
                       const gchar *install_dir, const gchar *what) {
  ...
  p = strrchr(dest_exe, '.');
  if (p == NULL || !(strcmp(p, ".c") == 0 || strcmp(p, ".cc") == 0 ||
                     strcmp(p, ".cpp") == 0)) {
    ...
    if (strcmp(env_cc, "g++") == 0 ||
      ...
      g_printerr(
          "The source file (%s) doesn't have a \"standard\" C or C++ suffix, "
  ...
  if (install_dir) {
    ...
    g_mkdir_with_parents(dest_dir, S_IRUSR | S_IXUSR | S_IWUSR | S_IRGRP |
  ...
  cmd = g_strdup_printf("%s %s%s %s %s%s %s%s %s%s %s %s", env_cc, lang_flag,
                        env_cflags, cflags, output_flag, dest_exe, source,
                        here_comes_linker_flags, env_ldflags,
                        windows_subsystem_flag, libs, env_libs);
...
static void do_build(const gchar *what) {
  gchar *cflags = get_cflags();
  gchar *libs = get_libs();
...
static void do_build_noui(const gchar *what) {
  gchar *cflags = get_cflags_noui();
  gchar *libs = get_libs_noui();
...
static void do_build_nogimpui(const gchar *what) { do_build(what); }

static gchar *get_user_plugin_dir(void) {
  return g_build_filename(gimp_directory(), "plug-ins", NULL);
}

static gchar *get_plugin_dir(const gchar *base_dir, const gchar *what) {
  ...
  plugin_dir = g_strconcat(base_dir, G_DIR_SEPARATOR_S, plugin_name, NULL);
...
static void do_install(const gchar *what) {
  gchar *cflags = get_cflags();
  gchar *libs = get_libs();
...
static void do_install_noui(const gchar *what) {
  gchar *cflags = get_cflags_noui();
  gchar *libs = get_libs_noui();
  gchar *dir = get_user_plugin_dir();
...
static void do_install_nogimpui(const gchar *what) { do_install(what); }

static gchar *get_sys_plugin_dir(gboolean forward_slashes) {
  gchar *dir;
  ...
  dir = g_build_path(forward_slashes ? "/" : G_DIR_SEPARATOR_S, rprefix, "lib",
                     "gimp", GIMP_PLUGIN_VERSION, "plug-ins", NULL);
  g_free(rprefix);
#else
  dir = g_build_path(forward_slashes ? "/" : G_DIR_SEPARATOR_S, LIBDIR, "gimp",
                     GIMP_PLUGIN_VERSION, "plug-ins", NULL);
#endif

  return dir;
}

static void do_install_admin(const gchar *what) {
  gchar *cflags = get_cflags();
  gchar *libs = get_libs();
...
static void do_install_admin_noui(const gchar *what) {
  gchar *cflags = get_cflags_noui();
  gchar *libs = get_libs_noui();
  gchar *dir = get_sys_plugin_dir(TRUE);
...
static void do_install_admin_nogimpui(const gchar *what) {
  gchar *cflags = get_cflags();
  gchar *libs = get_libs();
  gchar *dir = get_sys_plugin_dir(TRUE);
...
static void do_install_bin_2(const gchar *dir, const gchar *what) {
  gchar *cmd;
  gchar *quoted_src;
  gchar *quoted_dir;
...
static void do_install_bin(const gchar *what) {
  gchar *dir = get_user_plugin_dir();
...
static void do_install_admin_bin(const gchar *what) {
...
static void do_uninstall(const gchar *dir) {
  gchar *cmd;
  gchar *quoted_src;
...
static void do_uninstall_script_2(const gchar *dir, const gchar *what) {
  gchar *cmd;
  gchar *quoted_src;
  gchar *src;
...
static gchar *maybe_append_exe(const gchar *what) {
#ifdef G_OS_WIN32
...
static void do_uninstall_bin(const gchar *what) {
  gchar *dir = get_user_plugin_dir();
...
static void do_uninstall_admin_bin(const gchar *what) {
  gchar *dir = get_sys_plugin_dir(FALSE);
  gchar *exe = maybe_append_exe(what);
  gchar *plugin_dir = get_plugin_dir(dir, what);
...
static gchar *get_user_script_dir(void) {
  return g_build_filename(gimp_directory(), "scripts", NULL);
}

static void do_install_script(const gchar *what) {
...
static gchar *get_sys_script_dir(void) {
  gchar *dir;
  gchar *prefix = get_runtime_prefix(G_DIR_SEPARATOR);

  dir = g_build_filename(prefix, "share", "gimp", GIMP_PLUGIN_VERSION,
                         "scripts", NULL);
  g_free(prefix);
...
static void do_install_admin_script(const gchar *what) {
...
static void do_uninstall_script(const gchar *what) {
  gchar *dir = get_user_script_dir();
...
static void do_uninstall_admin_script(const gchar *what) {
  gchar *dir = get_sys_script_dir();
