...
int main(int argc, char **argv) {
  ...
  rsvg_handle_get_geometry_for_layer(handle, id, &viewport, &out_ink_rect,
  ...
  if (out_ink_rect.width != out_ink_rect.height) {
    ...
    fprintf(
        stderr,
        "WARNING: object \"%s\" has unexpected size %fx%f [pos: (%f, %f)].\n",
        id, out_ink_rect.width, out_ink_rect.height, out_ink_rect.x,
        out_ink_rect.y);
  }
  printf("viewBox=\"%f %f %f %f\"", out_ink_rect.x + prev_x,
         out_ink_rect.y + prev_y, out_ink_rect.width, out_ink_rect.height);
