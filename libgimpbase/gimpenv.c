...
void gimp_env_init(gboolean plug_in) {
  ...
  if (plug_in) {
    ...
    gchar *libdir =
        g_build_filename(gimp_installation_directory(), "lib", NULL);
  ...
  if (!g_file_test(data_home, G_FILE_TEST_IS_DIR)) {
    if (g_mkdir_with_parents(data_home, S_IRUSR | S_IWUSR | S_IXUSR) != 0) {
      g_warning("Failed to create the data directory '%s': %s", data_home,
                g_strerror(errno));
    }
  }
}
...
const gchar *gimp_directory(void) {
  ...
  if (gimp_dir) {
    ...
    gimp3_directory_changed =
        (env_gimp_dir == NULL && last_env_gimp_dir != NULL) ||
        (env_gimp_dir != NULL && last_env_gimp_dir == NULL) ||
        (env_gimp_dir != NULL && last_env_gimp_dir != NULL &&
  ...
  if (env_gimp_dir) {
    ...
    gimp_dir = g_build_filename([library_dir UTF8String], GIMPDIR,
                                GIMP_USER_VERSION, NULL);
    ...
    gimp_dir = g_build_filename(conf_dir, GIMPDIR, GIMP_USER_VERSION, NULL);
    g_free(conf_dir);
     ...
     */
    gimp_dir = g_build_filename(g_get_user_config_dir(), GIMPDIR,
                                GIMP_USER_VERSION, NULL);
 ...
 **/
const gchar *gimp_installation_directory(void) {
  ...
  toplevel =
      g_win32_get_package_installation_directory_of_module(libgimpbase_dll);
  if (!toplevel)
...
const gchar *gimp_data_directory(void) {
  ...
  if (!gimp_data_dir) {
    gchar *tmp =
        g_build_filename("share", GIMP_PACKAGE, GIMP_DATA_VERSION, NULL);
...
const gchar *gimp_locale_directory(void) {
  ...
  if (!gimp_locale_dir) {
    gchar *tmp = g_build_filename("share", "locale", NULL);
...
const gchar *gimp_sysconf_directory(void) {
  ...
  if (!gimp_sysconf_dir) {
    gchar *tmp =
        g_build_filename("etc", GIMP_PACKAGE, GIMP_SYSCONF_VERSION, NULL);

    gimp_sysconf_dir =
        gimp_env_get_dir("GIMP3_SYSCONFDIR", GIMPSYSCONFDIR, tmp);
    g_free(tmp);
  }
...
const gchar *gimp_plug_in_directory(void) {
  ...
  if (!gimp_plug_in_dir) {
    gchar *tmp =
        g_build_filename("lib", GIMP_PACKAGE, GIMP_PLUGIN_VERSION, NULL);
 ...
 **/
const gchar *gimp_cache_directory(void) {
  static gchar *gimp_cache_dir = NULL;

  if (!gimp_cache_dir) {
    gchar *tmp = g_build_filename(g_get_user_cache_dir(), GIMP_PACKAGE,
                                  GIMP_USER_VERSION, NULL);
 ...
 **/
const gchar *gimp_temp_directory(void) {
  static gchar *gimp_temp_dir = NULL;

  if (!gimp_temp_dir) {
    gchar *tmp = g_build_filename(g_get_tmp_dir(), GIMP_PACKAGE,
                                  GIMP_USER_VERSION, NULL);
...
static GFile *gimp_child_file(const gchar *parent, const gchar *element,
                              va_list args) {
 ...
 */
static void gimp_path_runtime_fix(gchar **path) {
  ...
  if (strncmp(*path, PREFIX G_DIR_SEPARATOR_S,
    ...
    *path = g_build_filename(gimp_installation_directory(),
                             *path + strlen(PREFIX G_DIR_SEPARATOR_S), NULL);
    g_free(p);
  }
#endif
}
 ...
 **/
GList *gimp_path_parse(const gchar *path, gint max_paths, gboolean check,
                       GList **check_failed) {
  gchar **patharray;
  GList *list = NULL;
  ...
  for (list = path; list; list = g_list_next(list)) {
    ...
    if (!err && S_ISDIR(filestat.st_mode) &&
        ...
        ((filestat.st_mode & S_IWUSR) ||
         ...
         ((filestat.st_mode & S_IWGRP) && (euid != filestat.st_uid)) ||

         ((filestat.st_mode & S_IWOTH) && (euid != filestat.st_uid) &&
...
static gchar *gimp_env_get_dir(const gchar *gimp_env_name,
  ...
  if (env) {
    ...
    return g_build_filename(gimp_installation_directory(), relative_subdir,
                            NULL);
  }
