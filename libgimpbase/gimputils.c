...
static gboolean gimp_utils_generic_available(const gchar *program, gint major,
                                             gint minor);
static gboolean gimp_utils_gdb_available(gint major, gint minor);
 ...
 **/
gchar *gimp_any_to_utf8(const gchar *str, gssize len,
  ...
  if (!utf8) {
    if (warning_format) {
      ...
      g_logv(G_LOG_DOMAIN, G_LOG_LEVEL_MESSAGE, warning_format, warning_args);
...
gboolean gimp_file_show_in_file_manager(GFile *file, GError **error) {
...

  {
    ...
    if (!filename) {
      g_set_error_literal(error, G_FILE_ERROR, 0, _("File path is NULL"));
      goto out;
    }

    n = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, filename, -1, NULL,
                            0);
    if (n == 0) {
    ...
    n = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, filename, -1,
...

  {
    ...
    retval = g_dbus_proxy_call_sync(proxy, "ShowItems",
 ...
 * Since: 2.2
 **/
const GimpEnumDesc *gimp_enum_get_desc(GEnumClass *enum_class, gint value) {
 ...
 * Since: 2.2
 **/
gboolean gimp_enum_get_value(GType enum_type, gint value,
                             const gchar **value_name, const gchar **value_nick,
  ...
  if (enum_value) {
    ...
    if (value_desc || value_help) {
      ...
      if (value_desc) {
        if (enum_desc && enum_desc->value_desc) {
          ...
          if (context) /*  the new way, using NC_()    */
            *value_desc =
                g_dpgettext2(gimp_type_get_translation_domain(enum_type),
                             context, enum_desc->value_desc);
          else /*  for backward compatibility  */
            *value_desc = g_strip_context(
      ...
      if (value_help) {
        *value_help =
            ((enum_desc && enum_desc->value_help)
                 ? dgettext(gimp_type_get_translation_domain(enum_type),
...
const gchar *gimp_enum_value_get_desc(GEnumClass *enum_class,
  ...
  if (enum_desc && enum_desc->value_desc) {
    ...
    if (context) /*  the new way, using NC_()    */
      return g_dpgettext2(gimp_type_get_translation_domain(type), context,
                          enum_desc->value_desc);
    else /*  for backward compatibility  */
...
const gchar *gimp_enum_value_get_abbrev(GEnumClass *enum_class,
  ...
  if (enum_desc && enum_desc[1].value == enum_desc->value &&
 ...
 * Since: 2.2
 **/
gboolean gimp_flags_get_first_value(GType flags_type, guint value,
  ...
  if (flags_value) {
    ...
    if (value_desc || value_help) {
      ...
      if (value_desc)
        *value_desc =
            ((flags_desc && flags_desc->value_desc)
...
const gchar *gimp_flags_value_get_abbrev(GFlagsClass *flags_class,
  ...
  if (flags_desc && flags_desc[1].value == flags_desc->value &&
 ...
 * Since: 2.10
 **/
gboolean gimp_stack_trace_print(const gchar *prog_name, gpointer stream,
                                gchar **trace) {
  ...
  if (fork_pid == 0) {
     ...
     * See bug 793514. */
    if (!gimp_utils_gdb_available(7, 0) || execvp(args[0], args) == -1) {
      /* LLDB as alternative if the GDB call failed or if it was in
       * a too-old version. */
      gchar *args_lldb[15] = {"lldb",
                              "--attach-pid",
                              NULL,
                              "--batch",
                              "--one-line",
      ...
      if (symbols) {
        for (i = 0; i < n_symbols; i++) {
          ...
          if (trace) {
            if (!gtrace)
              gtrace = g_string_new(NULL);
            g_string_append(gtrace, (const gchar *)symbols[i]);
...
retry:
  ...
  g_fprintf(stdout, "%s (pid:%u): %s: ", prog_name, (guint)getpid(),
 ...
 * function.
 */
void gimp_range_estimate_settings(gdouble lower, gdouble upper, gdouble *step,
                                  gdouble *page, gint *digits) {
  gdouble range;
