 ...
 */
static char *_br_find_exe(GimpBinrelocInitError *error) {
#if !defined(ENABLE_RELOCATABLE_RESOURCES) || defined(G_OS_WIN32)
  if (error)
 ...
 *               error report. See the #GimpBinrelocInitError for a list of
 * error codes.
 *
 * @returns TRUE on success, FALSE if BinReloc failed to initialize.
 */
gboolean _gimp_reloc_init(GError **error) {
 ...
 */
gboolean _gimp_reloc_init_lib(GError **error) {
  GimpBinrelocInitError errcode;
...
static void set_gerror(GError **error, GimpBinrelocInitError errcode) {
  ...
  g_set_error(error, g_quark_from_static_string("GBinReloc"), errcode, "%s",
              error_message);
}
 ...
 *         returned.
 */
gchar *_gimp_reloc_find_prefix(const gchar *default_prefix) {
  gchar *dir1, *dir2;
