...
void gimp_wire_register(guint32 type, GimpWireReadFunc read_func,
                        GimpWireWriteFunc write_func,
                        GimpWireDestroyFunc destroy_func) {
...
gboolean gimp_wire_read(GIOChannel *channel, guint8 *buf, gsize count,
                        gpointer user_data) {
  if (wire_read_func) {
    ...
    while (count > 0) {
      do {
        bytes = 0;
        status = g_io_channel_read_chars(channel, (gchar *)buf, count, &bytes,
                                         &error);
      } while (G_UNLIKELY(status == G_IO_STATUS_AGAIN));

      if (G_UNLIKELY(status != G_IO_STATUS_NORMAL)) {
        if (error) {
          g_warning("%s: gimp_wire_read(): error: %s", g_get_prgname(),
                    error->message);
          g_error_free(error);
        } else {
          g_warning("%s: gimp_wire_read(): error", g_get_prgname());
      ...
      if (G_UNLIKELY(bytes == 0)) {
        g_warning("%s: gimp_wire_read(): unexpected EOF", g_get_prgname());
        wire_error_val = TRUE;
        return FALSE;
      }
...
gboolean gimp_wire_write(GIOChannel *channel, const guint8 *buf, gsize count,
                         gpointer user_data) {
  if (wire_write_func) {
    ...
    while (count > 0) {
      do {
        bytes = 0;
        status = g_io_channel_write_chars(channel, (const gchar *)buf, count,
                                          &bytes, &error);
      } while (G_UNLIKELY(status == G_IO_STATUS_AGAIN));

      if (G_UNLIKELY(status != G_IO_STATUS_NORMAL)) {
        if (error) {
          g_warning("%s: gimp_wire_write(): error: %s", g_get_prgname(),
                    error->message);
          g_error_free(error);
        } else {
          g_warning("%s: gimp_wire_write(): error", g_get_prgname());
...
gboolean gimp_wire_flush(GIOChannel *channel, gpointer user_data) {
  if (wire_flush_func)
...
gboolean gimp_wire_error(void) { return wire_error_val; }

void gimp_wire_clear_error(void) { wire_error_val = FALSE; }

gboolean gimp_wire_read_msg(GIOChannel *channel, GimpWireMessage *msg,
                            gpointer user_data) {
  GimpWireHandler *handler;
...
gboolean gimp_wire_write_msg(GIOChannel *channel, GimpWireMessage *msg,
                             gpointer user_data) {
  GimpWireHandler *handler;
...
gboolean _gimp_wire_read_int64(GIOChannel *channel, guint64 *data, gint count,
                               gpointer user_data) {
  g_return_val_if_fail(count >= 0, FALSE);

  if (count > 0) {
    if (!_gimp_wire_read_int8(channel, (guint8 *)data, count * 8, user_data))
      return FALSE;
...
gboolean _gimp_wire_read_int32(GIOChannel *channel, guint32 *data, gint count,
                               gpointer user_data) {
  g_return_val_if_fail(count >= 0, FALSE);

  if (count > 0) {
    if (!_gimp_wire_read_int8(channel, (guint8 *)data, count * 4, user_data))
      return FALSE;
...
gboolean _gimp_wire_read_int16(GIOChannel *channel, guint16 *data, gint count,
                               gpointer user_data) {
  g_return_val_if_fail(count >= 0, FALSE);

  if (count > 0) {
    if (!_gimp_wire_read_int8(channel, (guint8 *)data, count * 2, user_data))
      return FALSE;
...
gboolean _gimp_wire_read_int8(GIOChannel *channel, guint8 *data, gint count,
                              gpointer user_data) {
  g_return_val_if_fail(count >= 0, FALSE);
...
gboolean _gimp_wire_read_double(GIOChannel *channel, gdouble *data, gint count,
                                gpointer user_data) {
  gdouble *t;
  guint8 tmp[8];
  gint i;
...
gboolean _gimp_wire_read_string(GIOChannel *channel, gchar **data, gint count,
                                gpointer user_data) {
  gint i;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (tmp > 0) {
      ...
      if (!_gimp_wire_read_int8(channel, (guint8 *)data[i], tmp, user_data)) {
        g_free(data[i]);
        return FALSE;
      }
...
gboolean _gimp_wire_read_color(GIOChannel *channel, GimpRGB *data, gint count,
                               gpointer user_data) {
  g_return_val_if_fail(count >= 0, FALSE);

  return _gimp_wire_read_double(channel, (gdouble *)data, 4 * count, user_data);
}

gboolean _gimp_wire_write_int64(GIOChannel *channel, const guint64 *data,
  ...
  if (count > 0) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (!_gimp_wire_write_int8(channel, (const guint8 *)&tmp, 8, user_data))
        return FALSE;
    }
  }
...
gboolean _gimp_wire_write_int32(GIOChannel *channel, const guint32 *data,
                                gint count, gpointer user_data) {
  g_return_val_if_fail(count >= 0, FALSE);
  ...
  if (count > 0) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (!_gimp_wire_write_int8(channel, (const guint8 *)&tmp, 4, user_data))
        return FALSE;
    }
  }
...
gboolean _gimp_wire_write_int16(GIOChannel *channel, const guint16 *data,
                                gint count, gpointer user_data) {
  g_return_val_if_fail(count >= 0, FALSE);
  ...
  if (count > 0) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (!_gimp_wire_write_int8(channel, (const guint8 *)&tmp, 2, user_data))
        return FALSE;
    }
  }
...
gboolean _gimp_wire_write_int8(GIOChannel *channel, const guint8 *data,
                               gint count, gpointer user_data) {
  g_return_val_if_fail(count >= 0, FALSE);
...
gboolean _gimp_wire_write_double(GIOChannel *channel, const gdouble *data,
                                 gint count, gpointer user_data) {
  gdouble *t;
  guint8 tmp[8];
  gint i;
...
gboolean _gimp_wire_write_string(GIOChannel *channel, gchar **data, gint count,
                                 gpointer user_data) {
  gint i;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (tmp > 0)
      if (!_gimp_wire_write_int8(channel, (const guint8 *)data[i], tmp,
                                 user_data))
        return FALSE;
  }
...
gboolean _gimp_wire_write_color(GIOChannel *channel, const GimpRGB *data,
                                gint count, gpointer user_data) {
  g_return_val_if_fail(count >= 0, FALSE);

  return _gimp_wire_write_double(channel, (gdouble *)data, 4 * count,
                                 user_data);
}

static guint gimp_wire_hash(const guint32 *key) { return *key; }

static gboolean gimp_wire_compare(const guint32 *a, const guint32 *b) {
  return (*a == *b);
}
