...
G_DEFINE_BOXED_TYPE(GimpParasite, gimp_parasite, gimp_parasite_copy,
                    gimp_parasite_free)
...
#define GIMP_PARAM_SPEC_PARASITE(pspec)                                        \
  (G_TYPE_CHECK_INSTANCE_CAST((pspec), GIMP_TYPE_PARAM_PARASITE,               \
...
static gboolean gimp_param_parasite_validate(GParamSpec *pspec, GValue *value);
...
GType gimp_param_parasite_get_type(void) {
  ...
  if (!type) {
    ...
    type = g_type_register_static(G_TYPE_PARAM_BOXED, "GimpParamParasite",
                                  &type_info, 0);
  }
...
static gboolean gimp_param_parasite_validate(GParamSpec *pspec, GValue *value) {
  ...
  if (!parasite) {
    return TRUE;
  } else if (parasite->name == NULL || *parasite->name == '\0' ||
...
GParamSpec *gimp_param_spec_parasite(const gchar *name, const gchar *nick,
  ...
  parasite_spec =
      g_param_spec_internal(GIMP_TYPE_PARAM_PARASITE, name, nick, blurb, flags);

  return G_PARAM_SPEC(parasite_spec);
}

#ifdef DEBUG
static void gimp_parasite_print(GimpParasite *parasite) {
  if (parasite == NULL) {
...
GimpParasite *gimp_parasite_copy(const GimpParasite *parasite) {
  ...
  return gimp_parasite_new(parasite->name, parasite->flags, parasite->size,
                           parasite->data);
}
