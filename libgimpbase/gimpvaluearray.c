...
G_DEFINE_BOXED_TYPE(GimpValueArray, gimp_value_array, gimp_value_array_ref,
...
static inline void value_array_grow(GimpValueArray *value_array, gint n_values,
                                    gboolean zero_init) {
  ...
  if (value_array->n_values > value_array->n_prealloced) {
    ...
    value_array->n_prealloced =
        (value_array->n_values + GROUP_N_VALUES - 1) & ~(GROUP_N_VALUES - 1);
    value_array->values =
...
static inline void value_array_shrink(GimpValueArray *value_array) {
...
GimpValueArray *gimp_value_array_new_from_types(gchar **error_msg,
  ...
  value_array =
      gimp_value_array_new_from_types_valist(error_msg, first_type, va_args);
...
GType gimp_param_value_array_get_type(void) {
  ...
  if (!type) {
    ...
    type = g_type_register_static(G_TYPE_PARAM_BOXED, "GimpParamValueArray",
                                  &info, 0);
  }
...
static inline guint gimp_value_array_ensure_size(GimpValueArray *value_array,
...
static gboolean gimp_param_value_array_validate(GParamSpec *pspec,
  ...
  if (value->data[0].v_pointer) {
    ...
    changed +=
        gimp_value_array_ensure_size(value_array, aspec->fixed_n_elements);
 ...
 * Since: 3.0
 **/
GParamSpec *gimp_param_spec_value_array(const gchar *name, const gchar *nick,
                                        const gchar *blurb,
  ...
  aspec = g_param_spec_internal(GIMP_TYPE_PARAM_VALUE_ARRAY, name, nick, blurb,
                                flags);
  if (element_spec) {
