...
static void gimp_metadata_copy_tag(GExiv2Metadata *src, GExiv2Metadata *dest,
                                   const gchar *tag);
static void gimp_metadata_copy_tags(GExiv2Metadata *src, GExiv2Metadata *dest,
                                    const gchar **tags);
static void gimp_metadata_add(GimpMetadata *src, GimpMetadata *dest);

static const gchar *tiff_tags[] = {"Xmp.tiff",
...
static const gchar *jpeg_tags[] = {"Exif.Image.JPEGProc",
...
static const gchar *unsupported_tags[] = {
    "Exif.Image.SubIFDs", "Exif.Image.ClipPath", "Exif.Image.XClipPathUnits",
    "Exif.Image.YClipPathUnits", "Exif.Image.XPTitle", "Exif.Image.XPComment",
    "Exif.Image.XPAuthor", "Exif.Image.XPKeywords", "Exif.Image.XPSubject",
    "Exif.Image.DNGVersion", "Exif.Image.DNGBackwardVersion", "Exif.Iop",
    /* FIXME Even though adding the tags below fixes the issue it's not very
       flexible. It might be better in the long run if there was a way for a
       user to configure which
       tags to block or a way for us to detect problems with tags before writing
       them. */
    /* Issues #1367, #2253. Offending tag is PreviewOffset but the other Preview
       tags (PreviewResolution, PreviewLength, PreviewImageBorders) also make no
       sense because we are not including a Pentax specific preview image. */
    "Exif.Pentax.Preview", "Exif.PentaxDng.Preview"};

static const guint8 minimal_exif[] = {
    0xff, 0xd8, 0xff, 0xe0, 0x00, 0x10, 0x4a, 0x46, 0x49, 0x46, 0x00,
    0x01, 0x01, 0x01, 0x00, 0x5a, 0x00, 0x5a, 0x00, 0x00, 0xff, 0xe1};
...
static void gimp_metadata_init(GimpMetadata *metadata) {}
...
void gimp_metadata_add_xmp_history(GimpMetadata *metadata,
  ...
  static const gchar *tags[] = {"Xmp.xmpMM.InstanceID", "Xmp.xmpMM.DocumentID",
                                "Xmp.xmpMM.OriginalDocumentID",
                                "Xmp.xmpMM.History"};

  static const gchar *history_tags[] = {"/stEvt:action", "/stEvt:instanceID",
                                        "/stEvt:when", "/stEvt:softwareAgent",
                                        "/stEvt:changed"};
  ...
  gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata), tags[0], iid_data);
  g_free(uuid);
  ...
  if (!did || !strlen(did)) {
    ...
    gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata), tags[1],
                                   did_data);
    g_free(uuid);
  }
  ...
  if (!odid || !strlen(odid)) {
    ...
    gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata), tags[2],
                                   did_data);
    g_free(uuid);
  }
  ...
  gexiv2_metadata_set_xmp_tag_struct(GEXIV2_METADATA(metadata), tags[3],
  ...
  for (count = 1; count < 65536; count++) {
    lastfound = 0;
    for (ii = 0; ii < 5; ii++) {
      g_snprintf(tagstr, sizeof(tagstr), "%s[%d]%s", tags[3], count,
                 history_tags[ii]);

      if (gexiv2_metadata_has_tag(GEXIV2_METADATA(metadata), tagstr)) {
        lastfound = 1;
      }
    }
  ...
  g_snprintf(tagstr, sizeof(tagstr), "%s[%d]%s", tags[3], id_count,
             history_tags[0]);

  gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata), tagstr, "saved");
  ...
  g_snprintf(tagstr, sizeof(tagstr), "%s[%d]%s", tags[3], id_count,
             history_tags[1]);
  g_snprintf(strdata, sizeof(strdata), "xmp.iid:%s", uuid);

  gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata), tagstr, strdata);
  g_free(uuid);
  ...
  gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata), tagstr, tmp);
  g_free(tmp);
  ...
  gexiv2_metadata_set_tag_string(GEXIV2_METADATA(metadata), tagstr,
                                 PACKAGE_STRING " "
...
GimpMetadata *gimp_metadata_new(void) {
  ...
  if (gexiv2_initialize()) {
    ...
    if (!gexiv2_metadata_open_buf(GEXIV2_METADATA(metadata), wilber_jpg,
                                  wilber_jpg_len, NULL)) {
...
static void gimp_metadata_deserialize_start_element(
  ...
  if (!strcmp(element_name, "tag")) {
    ...
    encoding = gimp_metadata_attribute_name_to_value(
        attribute_names, attribute_values, "encoding");
...
static void gimp_metadata_deserialize_text(GMarkupParseContext *context,
  ...
  if (!g_strcmp0(current_element, "tag")) {
    ...
    if (value) {
      ...
      values = gexiv2_metadata_get_tag_multiple(g2_metadata, parse_data->name);
      ...
      if (values) {
        ...
        gexiv2_metadata_set_tag_multiple(g2_metadata, parse_data->name,
                                         (const gchar **)values);
        g_strfreev(values);
      } else {
...
GimpMetadata *gimp_metadata_deserialize(const gchar *metadata_xml) {
  ...
  g_markup_parse_context_parse(context, metadata_xml, strlen(metadata_xml),
                               NULL);
...
static gchar *gimp_metadata_escape(const gchar *name, const gchar *value,
                                   gboolean *base64) {
...
static void gimp_metadata_append_tag(GString *string, const gchar *name,
                                     gchar *value, gboolean base64) {
  if (value) {
    if (base64) {
      g_string_append_printf(
          string, "  <tag name=\"%s\" encoding=\"base64\">%s</tag>\n", name,
          value);
    } else {
 ...
 * Since: 2.10
 */
gboolean gimp_metadata_save_to_file(GimpMetadata *metadata, GFile *file,
                                    GError **error) {
  gchar *path;
  gchar *filename;
  gboolean success;
  ...
  success =
      gexiv2_metadata_save_file(GEXIV2_METADATA(metadata), filename, error);
...
gboolean gimp_metadata_set_from_exif(GimpMetadata *metadata,
  ...
  if (exif_data_length < 0 || exif_data_length + 2 >= 65536) {
    g_set_error(error, GIMP_METADATA_ERROR, 0, _("Invalid Exif data size."));
    return FALSE;
  }
  ...
  exif_bytes =
      g_byte_array_append(exif_bytes, minimal_exif, G_N_ELEMENTS(minimal_exif));
  exif_bytes = g_byte_array_append(exif_bytes, data_size, 2);
  exif_bytes =
      g_byte_array_append(exif_bytes, (guint8 *)exif_data, exif_data_length);
  ...
  if (!gexiv2_metadata_has_exif(GEXIV2_METADATA(exif_metadata))) {
    g_set_error(error, GIMP_METADATA_ERROR, 0, _("Parsing Exif data failed."));
    g_object_unref(exif_metadata);
    g_byte_array_free(exif_bytes, TRUE);
    return FALSE;
  }
...
gboolean gimp_metadata_set_from_iptc(GimpMetadata *metadata,
  ...
  if (!gexiv2_metadata_open_buf(GEXIV2_METADATA(iptc_metadata), iptc_data,
                                iptc_data_length, error)) {
    g_object_unref(iptc_metadata);
    return FALSE;
  }

  if (!gexiv2_metadata_has_iptc(GEXIV2_METADATA(iptc_metadata))) {
    g_set_error(error, GIMP_METADATA_ERROR, 0, _("Parsing IPTC data failed."));
    g_object_unref(iptc_metadata);
    return FALSE;
  }
...
gboolean gimp_metadata_set_from_xmp(GimpMetadata *metadata,
                                    ...
                                    gint xmp_data_length, GError **error) {
  ...
  if (!gexiv2_metadata_open_buf(GEXIV2_METADATA(xmp_metadata), xmp_data,
                                xmp_data_length, error)) {
    g_object_unref(xmp_metadata);
    return FALSE;
  }

  if (!gexiv2_metadata_has_xmp(GEXIV2_METADATA(xmp_metadata))) {
    g_set_error(error, GIMP_METADATA_ERROR, 0, _("Parsing XMP data failed."));
    g_object_unref(xmp_metadata);
    return FALSE;
  }
 ...
 * Since: 2.10
 */
void gimp_metadata_set_pixel_size(GimpMetadata *metadata, gint width,
                                  gint height) {
  gchar buffer[32];
...
void gimp_metadata_set_bits_per_sample(GimpMetadata *metadata,
  ...
  g_snprintf(buffer, sizeof(buffer), "%d %d %d", bits_per_sample,
 ...
 * Since: 2.10
 */
gboolean gimp_metadata_get_resolution(GimpMetadata *metadata, gdouble *xres,
                                      gdouble *yres, GimpUnit *unit) {
  gint xnom, xdenom;
  gint ynom, ydenom;
  ...
  if (gexiv2_metadata_get_exif_tag_rational(GEXIV2_METADATA(metadata),
    ...
    if (xnom != 0 && xdenom != 0 && ynom != 0 && ydenom != 0) {
 ...
 * Since: 2.10
 */
void gimp_metadata_set_resolution(GimpMetadata *metadata, gdouble xres,
                                  gdouble yres, GimpUnit unit) {
  gchar buffer[32];
  gint exif_unit;
  gint factor;
...
GimpMetadataColorspace gimp_metadata_get_colorspace(GimpMetadata *metadata) {
  ...
  if (exif_cs == 0x01) {
    ...
    if (exif_cs == 0xffff) {
      ...
      iop_index = gexiv2_metadata_get_tag_string(
          GEXIV2_METADATA(metadata), "Exif.Iop.InteroperabilityIndex");
...
static void gimp_metadata_add(GimpMetadata *src, GimpMetadata *dest) {
  ...
  if (gexiv2_metadata_get_supports_exif(g2src) &&
    ...
    if (exif_tags) {
      gimp_metadata_copy_tags(g2src, g2dest, (const gchar **)exif_tags);
  ...
  if (gexiv2_metadata_get_supports_xmp(g2src) &&
    ...
    if (xmp_tags) {
      gimp_metadata_copy_tags(g2src, g2dest, (const gchar **)xmp_tags);
      g_strfreev(xmp_tags);
    }
  }
  ...
  if (gexiv2_metadata_get_supports_iptc(g2src) &&
    ...
    if (iptc_tags) {
      gimp_metadata_copy_tags(g2src, g2dest, (const gchar **)iptc_tags);
