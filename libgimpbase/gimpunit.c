...
GType gimp_unit_get_type(void) {
  ...
  if (!unit_type) {
    ...
    unit_type = g_type_register_static(G_TYPE_INT, "GimpUnit", &type_info, 0);
...
GimpUnit gimp_unit_new(gchar *identifier, gdouble factor, gint digits,
  ...
  return _gimp_unit_vtable.unit_new(identifier, factor, digits, symbol,
...
gint gimp_unit_get_scaled_digits(GimpUnit unit, gdouble resolution) {
  ...
  digits = ceil(log10(1.0 / gimp_pixels_to_units(1.0, unit, resolution)));
...
gchar *gimp_unit_format_string(const gchar *format, GimpUnit unit) {
  ...
  g_return_val_if_fail(
      unit == GIMP_UNIT_PERCENT ||
          (unit >= GIMP_UNIT_PIXEL && unit < gimp_unit_get_number_of_units()),
      NULL);
  ...
  while (i < (sizeof(buffer) - 1) && *format) {
    ...
    case '%':
      ...
      case 'f': /* factor (how many units make up an inch) */
        i += print(buffer, sizeof(buffer), i, "%f", gimp_unit_get_factor(unit));
        break;

      case 'y': /* symbol ("''" for inch) */
        i += print(buffer, sizeof(buffer), i, "%s", gimp_unit_get_symbol(unit));
        break;
      ...
      case 'p': /* plural */
        i += print(buffer, sizeof(buffer), i, "%s", gimp_unit_get_plural(unit));
        break;
...
GType gimp_param_unit_get_type(void) {
  ...
  if (!spec_type) {
    ...
    spec_type = g_type_register_static(G_TYPE_PARAM_INT, "GimpParamUnit",
                                       &type_info, 0);
  }
...
static gboolean gimp_param_unit_value_validate(GParamSpec *pspec,
  ...
  if (!(uspec->allow_percent && value->data[0].v_int == GIMP_UNIT_PERCENT)) {
    value->data[0].v_int = CLAMP(value->data[0].v_int, ispec->minimum,
...
GParamSpec *gimp_param_spec_unit(const gchar *name, const gchar *nick,
  ...
  pspec = g_param_spec_internal(GIMP_TYPE_PARAM_UNIT, name, nick, blurb, flags);
 ...
 * Since: 2.8
 **/
gdouble gimp_pixels_to_units(gdouble pixels, GimpUnit unit,
                             gdouble resolution) {
  if (unit == GIMP_UNIT_PIXEL)
    return pixels;
...
gdouble gimp_units_to_points(gdouble value, GimpUnit unit, gdouble resolution) {
  ...
  return (value * gimp_unit_get_factor(GIMP_UNIT_POINT) /
