...
static void gimp_thumbnail_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_thumbnail_get_property(GObject *object, guint property_id,
...
static gboolean gimp_thumbnail_save(GimpThumbnail *thumbnail,
                                    GimpThumbSize size, const gchar *filename,
                                    GdkPixbuf *pixbuf, const gchar *software,
                                    GError **error);
#ifdef GIMP_THUMB_DEBUG
static void gimp_thumbnail_debug_notify(GObject *object, GParamSpec *pspec);
#endif
...
static void gimp_thumbnail_class_init(GimpThumbnailClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_IMAGE_STATE,
      g_param_spec_enum("image-state", NULL,
                        "State of the image associated to the thumbnail object",
                        GIMP_TYPE_THUMB_STATE, GIMP_THUMB_STATE_UNKNOWN,
                        GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_IMAGE_URI,
      g_param_spec_string("image-uri", NULL, "URI of the image file", NULL,
                          GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_IMAGE_MTIME,
      g_param_spec_int64(
          "image-mtime", NULL,
          "Modification time of the image file in seconds since the Epoch",
          G_MININT64, G_MAXINT64, 0, GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_IMAGE_FILESIZE,
      g_param_spec_int64("image-filesize", NULL,
                         "Size of the image file in bytes", 0, G_MAXINT64, 0,
                         GIMP_PARAM_READWRITE));
  /**
   ...
   * Since: 2.2
   **/
  g_object_class_install_property(object_class, PROP_IMAGE_MIMETYPE,
                                  g_param_spec_string("image-mimetype", NULL,
                                                      "Image mimetype", NULL,
                                                      GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_IMAGE_WIDTH,
      g_param_spec_int("image-width", NULL, "Width of the image in pixels", 0,
                       G_MAXINT, 0, GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_IMAGE_HEIGHT,
      g_param_spec_int("image-height", NULL, "Height of the image in pixels", 0,
                       G_MAXINT, 0, GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_IMAGE_TYPE,
      g_param_spec_string("image-type", NULL,
                          "String describing the type of the image format",
                          NULL, GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_IMAGE_NUM_LAYERS,
      g_param_spec_int("image-num-layers", NULL,
                       "The number of layers in the image", 0, G_MAXINT, 0,
                       GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_THUMB_STATE,
      g_param_spec_enum("thumb-state", NULL, "State of the thumbnail file",
                        GIMP_TYPE_THUMB_STATE, GIMP_THUMB_STATE_UNKNOWN,
                        GIMP_PARAM_READWRITE));
}
...
#ifdef GIMP_THUMB_DEBUG
  g_signal_connect(thumbnail, "notify", G_CALLBACK(gimp_thumbnail_debug_notify),
                   NULL);
#endif
}
...
static void gimp_thumbnail_set_property(GObject *object, guint property_id,
  ...
  case PROP_IMAGE_STATE:
    ...
    break;
  case PROP_IMAGE_URI:
    gimp_thumbnail_set_uri(GIMP_THUMBNAIL(object), g_value_get_string(value));
    break;
...
void gimp_thumbnail_set_uri(GimpThumbnail *thumbnail, const gchar *uri) {
  ...
  g_object_set(thumbnail, "image-state", GIMP_THUMB_STATE_UNKNOWN,
               "image-filesize", (gint64)0, "image-mtime", (gint64)0,
               "image-mimetype", NULL, "image-width", 0, "image-height", 0,
               "image-type", NULL, "image-num-layers", 0, "thumb-state",
               GIMP_THUMB_STATE_UNKNOWN, NULL);
}
...
GimpThumbState gimp_thumbnail_peek_image(GimpThumbnail *thumbnail) {
  g_return_val_if_fail(GIMP_IS_THUMBNAIL(thumbnail), GIMP_THUMB_STATE_UNKNOWN);
...
static void gimp_thumbnail_update_image(GimpThumbnail *thumbnail) {
  ...
  default:
    switch (gimp_thumb_file_test(thumbnail->image_filename, &mtime, &filesize,
  ...
  if (state != thumbnail->image_state) {
    g_object_set(thumbnail, "image-state", state, NULL);
  }

  if (mtime != thumbnail->image_mtime ||
      filesize != thumbnail->image_filesize) {
    g_object_set(thumbnail, "image-mtime", mtime, "image-filesize", filesize,
                 NULL);

    if (thumbnail->thumb_state == GIMP_THUMB_STATE_OK)
      g_object_set(thumbnail, "thumb-state", GIMP_THUMB_STATE_OLD, NULL);
  }
}
...
static void gimp_thumbnail_update_thumb(GimpThumbnail *thumbnail,
  ...
  if (filename)
    state = (size > GIMP_THUMB_SIZE_FAIL ? GIMP_THUMB_STATE_EXISTS
...
static void gimp_thumbnail_reset_info(GimpThumbnail *thumbnail) {
  g_object_set(thumbnail, "image-width", 0, "image-height", 0, "image-type",
               NULL, "image-num-layers", 0, NULL);
}
...
static gboolean gimp_thumbnail_save(GimpThumbnail *thumbnail,
  ...
  tmpname = g_strdup_printf("%s%cgimp-thumb-%d-%.8s", dirname, G_DIR_SEPARATOR,
                            getpid(), basename);
  ...
  success =
      gdk_pixbuf_savev(pixbuf, tmpname, "png", (gchar **)keys, values, error);
  ...
  if (success) {
    ...
    if (success && thumbnail->thumb_state == GIMP_THUMB_STATE_EXISTS &&
...
GdkPixbuf *gimp_thumbnail_load_thumb(GimpThumbnail *thumbnail,
                                     GimpThumbSize size, GError **error) {
  GimpThumbState state;
  GdkPixbuf *pixbuf;
  const gchar *option;
  gint64 image_mtime;
 ...
 *               %FALSE otherwise
 **/
gboolean gimp_thumbnail_save_thumb(GimpThumbnail *thumbnail, GdkPixbuf *pixbuf,
  ...
  success = gimp_thumbnail_save(thumbnail, size, name, pixbuf, software, error);
  g_free(name);
...
gboolean gimp_thumbnail_save_failure(GimpThumbnail *thumbnail,
  ...
  success = gdk_pixbuf_save(pixbuf, name, "png", error, TAG_DESCRIPTION, desc,
                            TAG_SOFTWARE, software, TAG_THUMB_URI,
                            thumbnail->image_uri, TAG_THUMB_MTIME, time_str,
                            TAG_THUMB_FILESIZE, size_str, NULL);
  if (success) {
    ...
    if (success)
      ...
      g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno),
                  "Could not set permissions of thumbnail '%s': %s", name,
                  g_strerror(errno));
  }
...
void gimp_thumbnail_delete_failure(GimpThumbnail *thumbnail) {
  ...
  filename =
      gimp_thumb_name_from_uri(thumbnail->image_uri, GIMP_THUMB_SIZE_FAIL);
  if (filename) {
    g_unlink(filename);
    g_free(filename);
  }
