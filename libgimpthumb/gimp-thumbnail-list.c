...
static gboolean parse_option_state(const gchar *option_name, const gchar *value,
                                   gpointer data, GError **error);
static gboolean parse_option_path(const gchar *option_name, const gchar *value,
                                  gpointer data, GError **error);
...
static const GOptionEntry main_entries[] = {
    {"state", 's', 0, G_OPTION_ARG_CALLBACK, parse_option_state,
     "Filter by thumbnail state "
     "(unknown|remote|folder|special|not-found|exists|old|failed|ok|error)",
     "<state>"},
    {"path", 'p', 0, G_OPTION_ARG_CALLBACK, parse_option_path,
     "Filter by original file's path", "<path>"},
...
static void process_thumbnail(const gchar *filename) {
  ...
  if (!gimp_thumbnail_set_from_thumb(thumbnail, filename, &error)) {
    ...
    if ((option_state == STATE_NONE || state == option_state)
        ...
        (option_path == NULL || strstr(thumbnail->image_uri, option_path))) {
      if (option_verbose)
