...
static gboolean gimp_config_diff_property(GObject *a, GObject *b,
...
static GList *gimp_config_diff_same(GObject *a, GObject *b, GParamFlags flags) {
  ...
  param_specs =
      g_object_class_list_properties(G_OBJECT_GET_CLASS(a), &n_param_specs);
...
static GList *gimp_config_diff_other(GObject *a, GObject *b,
                                     GParamFlags flags) {
  GParamSpec **param_specs;
  guint n_param_specs;
  gint i;
  ...
  for (i = 0; i < n_param_specs; i++) {
    ...
    GParamSpec *b_spec =
        g_object_class_find_property(G_OBJECT_GET_CLASS(b), a_spec->name);

    if (b_spec && (a_spec->value_type == b_spec->value_type) &&
 ...
 * compared.
 *
 * Returns: (transfer container) (element-type GParamSpec): a GList of differing
 *GParamSpecs.
 *
 * Since: 2.4
 **/
    ...
    if ((prop_spec->flags & G_PARAM_WRITABLE) &&
      ...
      if (G_IS_PARAM_SPEC_OBJECT(prop_spec)) {
        ...
        if (g_param_values_cmp(prop_spec, &default_value, &value)) {
          g_object_set_property(object, prop_spec->name, &default_value);
        }
