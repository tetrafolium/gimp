...
#define COLOR_MANAGEMENT_MODE_BLURB _("How images are displayed on screen.")

#define DISPLAY_PROFILE_BLURB _("The color profile of your (primary) monitor.")
...
#define GRAY_PROFILE_BLURB                                                     \
  _("The preferred grayscale working space color profile. It will be offered " \
    "next to the built-in grayscale profile when a color profile can be "      \
    "chosen.")
...
static void gimp_color_config_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_color_config_get_property(GObject *object, guint property_id,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpColorConfig, gimp_color_config, G_TYPE_OBJECT,
    G_ADD_PRIVATE(GimpColorConfig) G_IMPLEMENT_INTERFACE(GIMP_TYPE_CONFIG, NULL)
...
static void gimp_color_config_class_init(GimpColorConfigClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(
      object_class, PROP_MODE, "mode", _("Mode of operation"),
      COLOR_MANAGEMENT_MODE_BLURB, GIMP_TYPE_COLOR_MANAGEMENT_MODE,
      GIMP_COLOR_MANAGEMENT_DISPLAY, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_PATH(object_class, PROP_RGB_PROFILE, "rgb-profile",
                        _("Preferred RGB profile"), RGB_PROFILE_BLURB,
                        GIMP_CONFIG_PATH_FILE, NULL, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_PATH(object_class, PROP_GRAY_PROFILE, "gray-profile",
                        _("Preferred grayscale profile"), GRAY_PROFILE_BLURB,
                        GIMP_CONFIG_PATH_FILE, NULL, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_PATH(object_class, PROP_CMYK_PROFILE, "cmyk-profile",
                        _("CMYK profile"), CMYK_PROFILE_BLURB,
                        GIMP_CONFIG_PATH_FILE, NULL, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_PATH(object_class, PROP_DISPLAY_PROFILE, "display-profile",
                        _("Monitor profile"), DISPLAY_PROFILE_BLURB,
                        GIMP_CONFIG_PATH_FILE, NULL, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_DISPLAY_PROFILE_FROM_GDK, "display-profile-from-gdk",
      _("Use the system monitor profile"), DISPLAY_PROFILE_FROM_GDK_BLURB,
      FALSE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_PATH(
      object_class, PROP_SIMULATION_PROFILE, "simulation-profile",
      _("Simulation profile for soft-proofing"), SIMULATION_PROFILE_BLURB,
      GIMP_CONFIG_PATH_FILE, NULL, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(
      object_class, PROP_DISPLAY_RENDERING_INTENT, "display-rendering-intent",
      _("Display rendering intent"), DISPLAY_RENDERING_INTENT_BLURB,
  ...
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_DISPLAY_USE_BPC,
                           ...
                           DISPLAY_USE_BPC_BLURB, TRUE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_DISPLAY_OPTIMIZE, "display-optimize",
      _("Optimize display color transformations"), DISPLAY_OPTIMIZE_BLURB, TRUE,
      GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(
      object_class, PROP_SIMULATION_RENDERING_INTENT,
      "simulation-rendering-intent", _("Soft-proofing rendering intent"),
      SIMULATION_RENDERING_INTENT_BLURB, GIMP_TYPE_COLOR_RENDERING_INTENT,
      GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL, GIMP_PARAM_STATIC_STRINGS);
  ...
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_SIMULATION_USE_BPC,
                           ...
                           SIMULATION_USE_BPC_BLURB, FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_SIMULATION_OPTIMIZE, "simulation-optimize",
      _("Optimize soft-proofing color transformations"),
      SIMULATION_OPTIMIZE_BLURB, TRUE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_SIMULATION_GAMUT_CHECK, "simulation-gamut-check",
      _("Mark out of gamut colors"), SIMULATION_GAMUT_CHECK_BLURB, FALSE,
      GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_RGB(object_class, PROP_OUT_OF_GAMUT_COLOR,
                       "out-of-gamut-color", _("Out of gamut warning color"),
                       OUT_OF_GAMUT_COLOR_BLURB, FALSE, &color,
                       GIMP_PARAM_STATIC_STRINGS);
}

static void gimp_color_config_init(GimpColorConfig *config) {
...
static void gimp_color_config_set_property(GObject *object, guint property_id,
  ...
  case PROP_RGB_PROFILE:
    gimp_color_config_set_rgb_profile(config, g_value_get_string(value),
                                      &error);
    break;
  case PROP_GRAY_PROFILE:
    gimp_color_config_set_gray_profile(config, g_value_get_string(value),
                                       &error);
    break;
  case PROP_CMYK_PROFILE:
    gimp_color_config_set_cmyk_profile(config, g_value_get_string(value),
                                       &error);
    break;
  case PROP_DISPLAY_PROFILE:
    gimp_color_config_set_display_profile(config, g_value_get_string(value),
                                          &error);
    break;
  ...
  case PROP_SIMULATION_PROFILE:
    gimp_color_config_set_simulation_profile(config, g_value_get_string(value),
                                             &error);
    break;
...
GimpColorManagementMode gimp_color_config_get_mode(GimpColorConfig *config) {
  g_return_val_if_fail(GIMP_IS_COLOR_CONFIG(config), GIMP_COLOR_MANAGEMENT_OFF);
...
gimp_color_config_get_rgb_color_profile(GimpColorConfig *config,
  ...
  if (GET_PRIVATE(config)->rgb_profile) {
    GFile *file =
        gimp_file_new_for_config_path(GET_PRIVATE(config)->rgb_profile, error);
...
gimp_color_config_get_gray_color_profile(GimpColorConfig *config,
  ...
  if (GET_PRIVATE(config)->gray_profile) {
    GFile *file =
        gimp_file_new_for_config_path(GET_PRIVATE(config)->gray_profile, error);
...
gimp_color_config_get_cmyk_color_profile(GimpColorConfig *config,
  ...
  if (GET_PRIVATE(config)->cmyk_profile) {
    GFile *file =
        gimp_file_new_for_config_path(GET_PRIVATE(config)->cmyk_profile, error);
...
gimp_color_config_get_display_color_profile(GimpColorConfig *config,
  ...
  if (GET_PRIVATE(config)->display_profile) {
    GFile *file = gimp_file_new_for_config_path(
        GET_PRIVATE(config)->display_profile, error);
...
gimp_color_config_get_simulation_color_profile(GimpColorConfig *config,
  ...
  if (GET_PRIVATE(config)->printer_profile) {
    GFile *file = gimp_file_new_for_config_path(
        GET_PRIVATE(config)->printer_profile, error);
