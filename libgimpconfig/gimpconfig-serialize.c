...
static gboolean gimp_config_serialize_rgb(const GValue *value, GString *str,
                                          gboolean has_alpha);
...
gboolean gimp_config_serialize_property(GimpConfig *config,
  ...
  if (config_iface && config_iface != parent_iface && /* see comment above */
      ...
      config_iface->serialize_property(config, param_spec->param_id,
                                       (const GValue *)&value, param_spec,
                                       writer)) {
    success = TRUE;
  }
    ...
    if (!success) {
      ...
      if (G_VALUE_HOLDS_STRING(&value)) {
        ...
        g_warning("couldn't serialize property %s::%s of type %s",
                  g_type_name(G_TYPE_FROM_INSTANCE(config)), param_spec->name,
...
gboolean gimp_config_serialize_value(const GValue *value, GString *str,
  ...
  if (GIMP_VALUE_HOLDS_MATRIX2(value)) {
    ...
    if (trafo) {
      ...
      for (i = 0, k = 0; i < 2; i++)
        for (j = 0; j < 2; j++, k++)
          g_ascii_dtostr(buf[k], G_ASCII_DTOSTR_BUF_SIZE, trafo->coeff[i][j]);

      g_string_append_printf(str, "(matrix %s %s %s %s)", buf[0], buf[1],
                             buf[2], buf[3]);
    } else {
  ...
  if (G_VALUE_TYPE(value) == GIMP_TYPE_VALUE_ARRAY) {
    ...
    if (array) {
      ...
      for (i = 0; i < length; i++) {
        ...
        if (!gimp_config_serialize_value(gimp_value_array_index(array, i), str,
                                         TRUE))
          return FALSE;
      }
    } else {
...
static gboolean gimp_config_serialize_rgb(const GValue *value, GString *str,
  ...
  if (rgb) {
    ...
    if (has_alpha) {
      ...
      g_string_append_printf(str, "(color-rgba %s %s %s %s)", buf[0], buf[1],
                             buf[2], buf[3]);
    } else {
      g_string_append_printf(str, "(color-rgb %s %s %s)", buf[0], buf[1],
                             buf[2]);
    }
