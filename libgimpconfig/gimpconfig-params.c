...
static gboolean gimp_gegl_param_spec_has_key(GParamSpec *pspec,
                                             const gchar *key,
                                             const gchar *value) {
...
GParamSpec *gimp_config_param_spec_duplicate(GParamSpec *pspec) {
  ...
  if (G_IS_PARAM_SPEC_STRING(pspec)) {
    ...
    if (GEGL_IS_PARAM_SPEC_FILE_PATH(pspec)) {
      copy = gimp_param_spec_config_path(
          name, nick, blurb, GIMP_CONFIG_PATH_FILE, spec->default_value, flags);
    } else if (GIMP_IS_PARAM_SPEC_CONFIG_PATH(pspec)) {
      copy = gimp_param_spec_config_path(
          name, nick, blurb, gimp_param_spec_config_path_type(pspec),
          spec->default_value, flags);
    } else {
      copy = g_param_spec_string(name, nick, blurb, spec->default_value, flags);
    }
  } else if (G_IS_PARAM_SPEC_BOOLEAN(pspec)) {
    GParamSpecBoolean *spec = G_PARAM_SPEC_BOOLEAN(pspec);

    copy = g_param_spec_boolean(name, nick, blurb, spec->default_value, flags);
    ...
    if (GEGL_IS_PARAM_SPEC_DOUBLE(pspec)) {
      ...
      copy = gegl_param_spec_double(
          name, nick, blurb, spec->minimum, spec->maximum, spec->default_value,
          gspec->ui_minimum, gspec->ui_maximum, gspec->ui_gamma, flags);
      ...
      copy = g_param_spec_double(name, nick, blurb, spec->minimum,
                                 spec->maximum, spec->default_value, flags);
    }
  } else if (G_IS_PARAM_SPEC_FLOAT(pspec)) {
    GParamSpecFloat *spec = G_PARAM_SPEC_FLOAT(pspec);

    copy = g_param_spec_float(name, nick, blurb, spec->minimum, spec->maximum,
                              spec->default_value, flags);
    ...
    if (GEGL_IS_PARAM_SPEC_INT(pspec)) {
      ...
      copy = gegl_param_spec_int(
          name, nick, blurb, spec->minimum, spec->maximum, spec->default_value,
          gspec->ui_minimum, gspec->ui_maximum, gspec->ui_gamma, flags);
      gegl_param_spec_int_set_steps(GEGL_PARAM_SPEC_INT(copy),
                                    gspec->ui_step_small, gspec->ui_step_big);
    } else if (GIMP_IS_PARAM_SPEC_UNIT(pspec)) {
      GimpParamSpecUnit *spec = GIMP_PARAM_SPEC_UNIT(pspec);
      GParamSpecInt *ispec = G_PARAM_SPEC_INT(pspec);

      copy = gimp_param_spec_unit(
          name, nick, blurb, ispec->minimum == GIMP_UNIT_PIXEL,
          spec->allow_percent, ispec->default_value, flags);
    } else {
      copy = g_param_spec_int(name, nick, blurb, spec->minimum, spec->maximum,
                              spec->default_value, flags);
    }
    ...
    if (GEGL_IS_PARAM_SPEC_SEED(pspec)) {
      ...
      copy = gegl_param_spec_seed(name, nick, blurb, flags);
      ...
      copy = g_param_spec_uint(name, nick, blurb, spec->minimum, spec->maximum,
                               spec->default_value, flags);
    }
    ...
    copy = gimp_param_spec_rgb(
        name, nick, blurb, gimp_param_spec_rgb_has_alpha(pspec), &color, flags);
    ...
    copy = gimp_param_spec_rgb(name, nick, blurb, TRUE, &gimp_color, flags);
  } else if (G_IS_PARAM_SPEC_PARAM(pspec)) {
    copy = g_param_spec_param(name, nick, blurb, G_PARAM_SPEC_VALUE_TYPE(pspec),
                              flags);
  } else if (GIMP_IS_PARAM_SPEC_PARASITE(pspec)) {
    copy = gimp_param_spec_parasite(name, nick, blurb, flags);
  } else if (GIMP_IS_PARAM_SPEC_ARRAY(pspec)) {
    if (GIMP_IS_PARAM_SPEC_UINT8_ARRAY(pspec)) {
      copy = gimp_param_spec_uint8_array(name, nick, blurb, flags);
    } else if (GIMP_IS_PARAM_SPEC_INT16_ARRAY(pspec)) {
      copy = gimp_param_spec_int16_array(name, nick, blurb, flags);
    } else if (GIMP_IS_PARAM_SPEC_INT32_ARRAY(pspec)) {
      copy = gimp_param_spec_int32_array(name, nick, blurb, flags);
    } else if (GIMP_IS_PARAM_SPEC_FLOAT_ARRAY(pspec)) {
      copy = gimp_param_spec_float_array(name, nick, blurb, flags);
    } else if (GIMP_IS_PARAM_SPEC_STRING_ARRAY(pspec)) {
      copy = gimp_param_spec_string_array(name, nick, blurb, flags);
    } else if (GIMP_IS_PARAM_SPEC_RGB_ARRAY(pspec)) {
      copy = gimp_param_spec_rgb_array(name, nick, blurb, flags);
    } else if (GIMP_IS_PARAM_SPEC_OBJECT_ARRAY(pspec)) {
      GimpParamSpecObjectArray *spec = GIMP_PARAM_SPEC_OBJECT_ARRAY(pspec);

      copy = gimp_param_spec_object_array(name, nick, blurb, spec->object_type,
                                          flags);
    }
    ...
    if (value_type == G_TYPE_FILE) {
      copy = g_param_spec_object(name, nick, blurb, value_type, flags);
    }
  }
