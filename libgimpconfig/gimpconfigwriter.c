...
static inline void gimp_config_writer_flush(GimpConfigWriter *writer) {
  ...
  if (!g_output_stream_write_all(writer->output, writer->buffer->str,
                                 writer->buffer->len, NULL, NULL, &error)) {
    g_set_error(&writer->error, GIMP_CONFIG_ERROR, GIMP_CONFIG_ERROR_WRITE,
                _("Error writing to '%s': %s"),
                writer->file ? gimp_file_get_utf8_name(writer->file)
                             : "output stream",
                error->message);
    g_clear_error(&error);
  }
 ...
 * Since: 2.10
 **/
GimpConfigWriter *gimp_config_writer_new_from_file(GFile *file, gboolean atomic,
                                                   const gchar *header,
                                                   GError **error) {
  ...
  if (dir && !g_file_query_exists(dir, NULL)) {
    if (!g_file_make_directory_with_parents(dir, NULL, error))
      g_prefix_error(error, _("Could not create directory '%s' for '%s': "),
  ...
  if (atomic) {
    ...
    if (!output)
      g_prefix_error(error, _("Could not create temporary file for '%s': "),
                     gimp_file_get_utf8_name(file));
  } else {
 ...
 * Since: 2.4
 **/
void gimp_config_writer_print(GimpConfigWriter *writer, const gchar *string,
                              gint len) {
  g_return_if_fail(writer != NULL);
  g_return_if_fail(writer->finished == FALSE);
 ...
 * Since: 2.4
 **/
void gimp_config_writer_data(GimpConfigWriter *writer, gint length,
                             const guint8 *data) {
  gint i;
...
#define LINE_LENGTH 75
  ...
  while (len > 0) {
    for (s = comment, i = 0, space = 0;
         *s != '\n' && (i <= LINE_LENGTH || space == 0) && i < len; s++, i++) {
