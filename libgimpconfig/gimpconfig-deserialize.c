...
static inline gboolean scanner_string_utf8_valid(GScanner *scanner,
  ...
  g_scanner_error(scanner, _("value for token %s is not a valid UTF-8 string"),
                  token_name);
...
GTokenType gimp_config_deserialize_property(GimpConfig *config,
  ...
  if (G_TYPE_IS_OBJECT(prop_spec->owner_type)) {
    ...
    if (config_iface) {
      ...
      parent_iface =
          g_type_interface_peek(owner_parent_class, GIMP_TYPE_CONFIG);
    }
  }

  if (config_iface && config_iface != parent_iface && /* see comment above */
      config_iface->deserialize_property &&
      config_iface->deserialize_property(config, prop_spec->param_id, &value,
                                         prop_spec, scanner, &token)) {
    /* nop */
  } else {
    if (G_VALUE_HOLDS_OBJECT(&value) && G_VALUE_TYPE(&value) != G_TYPE_FILE) {
      token = gimp_config_deserialize_object(&value, config, prop_spec, scanner,
                                             nest_level);
    } else {
      token = gimp_config_deserialize_value(&value, config, prop_spec, scanner);
    }
  }
  ...
  if (token == G_TOKEN_RIGHT_PAREN &&
    ...
    g_warning("%s: couldn't deserialize property %s::%s of type %s", G_STRFUNC,
              g_type_name(G_TYPE_FROM_INSTANCE(config)), prop_spec->name,
...
static GTokenType gimp_config_deserialize_value(GValue *value,
  ...
  if (G_TYPE_FUNDAMENTAL(prop_spec->value_type) == G_TYPE_ENUM) {
    ...
    return gimp_config_deserialize_value_array(value, config, prop_spec,
                                               scanner);
...
static GTokenType gimp_config_deserialize_fundamental(GValue *value,
  ...
  case G_TYPE_INT:
    g_value_set_int(value, (negate ? -(gint)scanner->value.v_int64
  ...
  case G_TYPE_LONG:
    g_value_set_long(value, (negate ? -(glong)scanner->value.v_int64
  ...
  case G_TYPE_INT64:
    g_value_set_int64(value, (negate ? -(gint64)scanner->value.v_int64
  ...
  case G_TYPE_FLOAT:
    if (next_token == G_TOKEN_FLOAT)
      g_value_set_float(value, negate ? -scanner->value.v_float
                                      : scanner->value.v_float);
    else
      g_value_set_float(value, negate ? -(gfloat)scanner->value.v_int
                                      : (gfloat)scanner->value.v_int);
    break;

  case G_TYPE_DOUBLE:
    if (next_token == G_TOKEN_FLOAT)
      g_value_set_double(value, negate ? -scanner->value.v_float
                                       : scanner->value.v_float);
    else
      g_value_set_double(value, negate ? -(gdouble)scanner->value.v_int
...
static GTokenType gimp_config_deserialize_enum(GValue *value,
  ...
  case G_TOKEN_IDENTIFIER:
    ...
    enum_value =
        g_enum_get_value_by_nick(enum_class, scanner->value.v_identifier);
    if (!enum_value)
      enum_value =
          g_enum_get_value_by_name(enum_class, scanner->value.v_identifier);
    if (!enum_value) {
      ...
      GType compat_type = (GType)g_type_get_qdata(G_VALUE_TYPE(value), quark);

      if (compat_type) {
        GEnumClass *compat_class = g_type_class_ref(compat_type);

        enum_value =
            g_enum_get_value_by_nick(compat_class, scanner->value.v_identifier);
        if (!enum_value)
    ...
    if (!enum_value) {
      g_scanner_error(scanner, _("invalid value '%s' for token %s"),
  ...
  case G_TOKEN_INT:
    ...
    if (!enum_value) {
      g_scanner_error(scanner, _("invalid value '%ld' for token %s"),
...
static GTokenType gimp_config_deserialize_path(GValue *value,
  ...
  if (scanner->value.v_string) {
     ...
     */
    gchar *expand =
        gimp_config_path_expand(scanner->value.v_string, TRUE, &error);

    if (!expand) {
      g_scanner_error(scanner, _("while parsing token '%s': %s"),
...
static GTokenType gimp_config_deserialize_value_array(GValue *value,
  ...
  for (i = 0; i < n_values; i++) {
    ...
    token = gimp_config_deserialize_value(&array_value, config,
                                          array_spec->element_spec, scanner);
...
static GTokenType gimp_config_deserialize_file_value(GValue *value,
  ...
  if (token == G_TOKEN_IDENTIFIER) {
    ...
    gchar *path = gimp_config_path_expand(scanner->value.v_string, TRUE, NULL);
...
static GTokenType gimp_config_deserialize_any(GValue *value,
  ...
  if (!g_value_type_transformable(G_TYPE_STRING, prop_spec->value_type)) {
    g_warning("%s: %s can not be transformed from a string", G_STRFUNC,
              g_type_name(prop_spec->value_type));
    return G_TOKEN_NONE;
  }
