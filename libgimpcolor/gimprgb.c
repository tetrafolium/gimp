...
void gimp_value_get_rgb(const GValue *value, GimpRGB *rgb) {
...
void gimp_value_set_rgb(GValue *value, const GimpRGB *rgb) {
  g_return_if_fail(GIMP_VALUE_HOLDS_RGB(value));
  g_return_if_fail(rgb != NULL);
 ...
 **/
void gimp_rgb_set(GimpRGB *rgb, gdouble r, gdouble g, gdouble b) {
  g_return_if_fail(rgb != NULL);
 ...
 **/
void gimp_rgb_set_alpha(GimpRGB *rgb, gdouble a) {
  g_return_if_fail(rgb != NULL);
 ...
 * Since: 2.10
 **/
void gimp_rgb_set_pixel(GimpRGB *rgb, const Babl *format, gconstpointer pixel) {
  g_return_if_fail(rgb != NULL);
  g_return_if_fail(format != NULL);
  g_return_if_fail(pixel != NULL);

  babl_process(babl_fish(format, babl_format("R'G'B' double")), pixel, rgb, 1);
}
 ...
 * Since: 2.10
 **/
void gimp_rgb_get_pixel(const GimpRGB *rgb, const Babl *format,
                        gpointer pixel) {
  g_return_if_fail(rgb != NULL);
  g_return_if_fail(format != NULL);
  g_return_if_fail(pixel != NULL);

  babl_process(babl_fish(babl_format("R'G'B' double"), format), rgb, pixel, 1);
}
 ...
 */
void gimp_rgb_get_uchar(const GimpRGB *rgb, guchar *red, guchar *green,
                        guchar *blue) {
  g_return_if_fail(rgb != NULL);

  if (red)
    *red = ROUND(CLAMP(rgb->r, 0.0, 1.0) * 255.0);
  if (green)
    *green = ROUND(CLAMP(rgb->g, 0.0, 1.0) * 255.0);
  if (blue)
    *blue = ROUND(CLAMP(rgb->b, 0.0, 1.0) * 255.0);
}

void gimp_rgb_add(GimpRGB *rgb1, const GimpRGB *rgb2) {
...
void gimp_rgb_subtract(GimpRGB *rgb1, const GimpRGB *rgb2) {
  g_return_if_fail(rgb1 != NULL);
  g_return_if_fail(rgb2 != NULL);
...
void gimp_rgb_multiply(GimpRGB *rgb, gdouble factor) {
  g_return_if_fail(rgb != NULL);
...
gdouble gimp_rgb_distance(const GimpRGB *rgb1, const GimpRGB *rgb2) {
  ...
  return (fabs(rgb1->r - rgb2->r) + fabs(rgb1->g - rgb2->g) +
          fabs(rgb1->b - rgb2->b));
}
...
void gimp_rgb_composite(GimpRGB *color1, const GimpRGB *color2,
 ...
 * Since: 2.10
 **/
void gimp_rgba_set_pixel(GimpRGB *rgba, const Babl *format,
                         gconstpointer pixel) {
  g_return_if_fail(rgba != NULL);
  g_return_if_fail(format != NULL);
  g_return_if_fail(pixel != NULL);

  babl_process(babl_fish(format, babl_format("R'G'B'A double")), pixel, rgba,
               1);
}
 ...
 * Since: 2.10
 **/
void gimp_rgba_get_pixel(const GimpRGB *rgba, const Babl *format,
                         gpointer pixel) {
  g_return_if_fail(rgba != NULL);
  g_return_if_fail(format != NULL);
  g_return_if_fail(pixel != NULL);

  babl_process(babl_fish(babl_format("R'G'B'A double"), format), rgba, pixel,
               1);
}
 ...
 * values (0 to 255).
 **/
void gimp_rgba_set_uchar(GimpRGB *rgba, guchar r, guchar g, guchar b,
                         guchar a) {
  g_return_if_fail(rgba != NULL);
 ...
 **/
void gimp_rgba_get_uchar(const GimpRGB *rgba, guchar *r, guchar *g, guchar *b,
                         guchar *a) {
  g_return_if_fail(rgba != NULL);

  if (r)
    *r = ROUND(CLAMP(rgba->r, 0.0, 1.0) * 255.0);
  if (g)
    *g = ROUND(CLAMP(rgba->g, 0.0, 1.0) * 255.0);
  if (b)
    *b = ROUND(CLAMP(rgba->b, 0.0, 1.0) * 255.0);
  if (a)
    *a = ROUND(CLAMP(rgba->a, 0.0, 1.0) * 255.0);
}

void gimp_rgba_add(GimpRGB *rgba1, const GimpRGB *rgba2) {
...
#define GIMP_PARAM_SPEC_RGB(pspec)                                             \
  (G_TYPE_CHECK_INSTANCE_CAST((pspec), GIMP_TYPE_PARAM_RGB, GimpParamSpecRGB))
...
static void gimp_param_rgb_set_default(GParamSpec *pspec, GValue *value);
static gboolean gimp_param_rgb_validate(GParamSpec *pspec, GValue *value);
static gint gimp_param_rgb_values_cmp(GParamSpec *pspec, const GValue *value1,
                                      const GValue *value2);
...
GType gimp_param_rgb_get_type(void) {
  ...
  if (!spec_type) {
    ...
    spec_type = g_type_register_static(G_TYPE_PARAM_BOXED, "GimpParamRGB",
                                       &type_info, 0);
  }
...
static gboolean gimp_param_rgb_validate(GParamSpec *pspec, GValue *value) {
  ...
  if (rgb_spec->validate && rgb) {
    ...
    return (oval.r != rgb->r || oval.g != rgb->g || oval.b != rgb->b ||
...
static gint gimp_param_rgb_values_cmp(GParamSpec *pspec, const GValue *value1,
  ...
  if (!rgb1) {
    ...
    if (GIMP_PARAM_SPEC_RGB(pspec)->has_alpha) {
      gimp_rgba_get_uchar(rgb1, ((guchar *)&int1) + 0, ((guchar *)&int1) + 1,
                          ((guchar *)&int1) + 2, ((guchar *)&int1) + 3);
      gimp_rgba_get_uchar(rgb2, ((guchar *)&int2) + 0, ((guchar *)&int2) + 1,
                          ((guchar *)&int2) + 2, ((guchar *)&int2) + 3);
    } else {
      gimp_rgb_get_uchar(rgb1, ((guchar *)&int1) + 0, ((guchar *)&int1) + 1,
                         ((guchar *)&int1) + 2);
      gimp_rgb_get_uchar(rgb2, ((guchar *)&int2) + 0, ((guchar *)&int2) + 1,
                         ((guchar *)&int2) + 2);
    }
 ...
 * Since: 2.4
 **/
GParamSpec *gimp_param_spec_rgb(const gchar *name, const gchar *nick,
                                const gchar *blurb, gboolean has_alpha,
                                const GimpRGB *default_value,
                                GParamFlags flags) {
  GimpParamSpecRGB *cspec;

  cspec = g_param_spec_internal(GIMP_TYPE_PARAM_RGB, name, nick, blurb, flags);
