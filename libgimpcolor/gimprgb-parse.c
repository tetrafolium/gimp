...
static gchar *gimp_rgb_parse_strip(const gchar *str, gint len);

static gboolean gimp_rgb_parse_name_internal(GimpRGB *rgb, const gchar *name);
static gboolean gimp_rgb_parse_hex_internal(GimpRGB *rgb, const gchar *hex);
static gboolean gimp_rgb_parse_css_internal(GimpRGB *rgb, const gchar *css);
static gboolean gimp_rgba_parse_css_internal(GimpRGB *rgb, const gchar *css);
...
gint gimp_rgb_list_names(const gchar ***names, GimpRGB **colors) {
  ...
  *colors = g_new(GimpRGB, G_N_ELEMENTS(named_colors));
  ...
  for (i = 0; i < G_N_ELEMENTS(named_colors); i++) {
    ...
    gimp_rgba_set_uchar((*colors) + i, named_colors[i].red,
                        named_colors[i].green, named_colors[i].blue, 0xFF);
  }
...
static gint gimp_rgb_color_entry_compare(gconstpointer a, gconstpointer b) {
  const gchar *name = a;
...
static gboolean gimp_rgb_parse_name_internal(GimpRGB *rgb, const gchar *name) {
  ColorEntry *entry = bsearch(name, named_colors, G_N_ELEMENTS(named_colors),
                              sizeof(ColorEntry), gimp_rgb_color_entry_compare);
...
static gboolean gimp_rgb_parse_hex_component(const gchar *hex, gint len,
                                             gdouble *value) {
  gint i;
  guint c = 0;
  ...
  switch (len) {
  case 1:
    *value = (gdouble)c / 15.0;
    break;
  case 2:
    *value = (gdouble)c / 255.0;
    break;
  case 3:
    *value = (gdouble)c / 4095.0;
    break;
  case 4:
    *value = (gdouble)c / 65535.0;
    break;
  default:
...
static gboolean gimp_rgb_parse_css_numeric(GimpRGB *rgb, const gchar *css) {
  gdouble values[4];
  gboolean alpha;
  gboolean hsl;
  gint i;
...
static gboolean gimp_rgb_parse_css_internal(GimpRGB *rgb, const gchar *css) {
  ...
  } else if (strncmp(css, "rgb(", 4) == 0 || strncmp(css, "hsl(", 4) == 0) {
...
static gboolean gimp_rgba_parse_css_internal(GimpRGB *rgba, const gchar *css) {
  if (strncmp(css, "rgba(", 5) != 0 && strncmp(css, "hsla(", 5) != 0)
    return FALSE;
