...
#define COLOR_AREA_EVENT_MASK                                                  \
  (GDK_EXPOSURE_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |       \
   GDK_BUTTON_MOTION_MASK | GDK_ENTER_NOTIFY_MASK)
...
typedef enum { DRAG_NONE, DRAG_XY, DRAG_Z } ColorSelectDragMode;
...
#define GIMP_COLOR_SELECT_CLASS(klass)                                         \
  (G_TYPE_CHECK_CLASS_CAST((klass), GIMP_TYPE_COLOR_SELECT,                    \
                           GimpColorSelectClass))
#define GIMP_IS_COLOR_SELECT_CLASS(klass)                                      \
  (G_TYPE_CHECK_CLASS_TYPE((klass), GIMP_TYPE_COLOR_SELECT))
#define GIMP_COLOR_SELECT_GET_CLASS(obj)                                       \
  (G_TYPE_INSTANCE_GET_CLASS((obj), GIMP_TYPE_COLOR_SELECT,                    \
                             GimpColorSelectClass))
...
static void gimp_color_select_xy_size_allocate(GtkWidget *widget,
                                               ...
                                               GimpColorSelect *select);
static gboolean gimp_color_select_xy_draw(GtkWidget *widget, cairo_t *cr,
                                          GimpColorSelect *select);
static gboolean gimp_color_select_xy_events(GtkWidget *widget, GdkEvent *event,
                                            GimpColorSelect *select);
static void gimp_color_select_z_size_allocate(GtkWidget *widget,
                                              GtkAllocation *allocation,
                                              GimpColorSelect *select);
static gboolean gimp_color_select_z_draw(GtkWidget *widget, cairo_t *cr,
                                         GimpColorSelect *select);
static gboolean gimp_color_select_z_events(GtkWidget *widget, GdkEvent *event,
                                           GimpColorSelect *select);

static void gimp_color_select_render(GtkWidget *widget, guchar *buf, gint width,
                                     gint height, gint rowstride,
                                     ColorSelectFillType fill_type,
                                     const GimpHSV *hsv, const GimpRGB *rgb,
...
static void gimp_color_select_class_init(GimpColorSelectClass *klass) {
  ...
  fish_lch_to_rgb_u8 =
      babl_fish(babl_format("CIE LCH(ab) double"), babl_format("R'G'B' u8"));
}
...
static void gimp_color_select_init(GimpColorSelect *select) {
  ...
  gtk_widget_set_size_request(select->xy_color, GIMP_COLOR_SELECTOR_SIZE,
  ...
  g_signal_connect(select->xy_color, "size-allocate",
                   G_CALLBACK(gimp_color_select_xy_size_allocate), select);
  g_signal_connect_after(select->xy_color, "draw",
                         G_CALLBACK(gimp_color_select_xy_draw), select);
  g_signal_connect(select->xy_color, "event",
                   G_CALLBACK(gimp_color_select_xy_events), select);
  ...
  gtk_widget_set_size_request(select->z_color, GIMP_COLOR_SELECTOR_BAR_SIZE,
                              -1);
  ...
  g_signal_connect(select->z_color, "size-allocate",
                   G_CALLBACK(gimp_color_select_z_size_allocate), select);
  g_signal_connect_after(select->z_color, "draw",
                         G_CALLBACK(gimp_color_select_z_draw), select);
  g_signal_connect(select->z_color, "event",
                   G_CALLBACK(gimp_color_select_z_events), select);
  ...
  for (model = GIMP_COLOR_SELECTOR_MODEL_RGB;
       model <= GIMP_COLOR_SELECTOR_MODEL_HSV; model++) {
    enum_desc = gimp_enum_get_desc(model_class, model);

    select->toggle_box[model] = gtk_box_new(GTK_ORIENTATION_VERTICAL, 2);
    gtk_box_pack_start(GTK_BOX(vbox), select->toggle_box[model], FALSE, FALSE,
                       0);
    ...
    /*  channel toggles  */
    {
      ...
      for (; channel < end_channel; channel++) {
        ...
        button = gtk_radio_button_new_with_mnemonic(
            group, gettext(enum_desc->value_desc));
        group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(button));
        gtk_toggle_button_set_mode(GTK_TOGGLE_BUTTON(button), FALSE);
        gtk_box_pack_start(GTK_BOX(select->toggle_box[model]), button, TRUE,
                           TRUE, 0);
        ...
        g_signal_connect(button, "toggled",
                         G_CALLBACK(gimp_color_select_channel_toggled), select);
      }
    }
  }
  ...
  if (config != select->config) {
    if (select->config) {
      g_signal_handlers_disconnect_by_func(
          select->config, gimp_color_select_notify_config, select);
    ...
    if (select->config) {
      g_signal_connect(select->config, "notify",
                       G_CALLBACK(gimp_color_select_notify_config), select);
...
static void gimp_color_select_channel_toggled(GtkWidget *widget,
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
    ...
    channel = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(widget), "channel"));
...
static gboolean gimp_color_select_xy_draw(GtkWidget *widget, cairo_t *cr,
  ...
  if (select->xy_needs_render) {
    ...
    gimp_color_select_render(select->xy_color, select->xy_buf, select->xy_width,
                             select->xy_height, select->xy_rowstride,
                             select->xy_color_fill, &selector->hsv,
                             &selector->rgb, select->oog_color);
  ...
  if (select->transform) {
    ...
    guchar *buf = g_new(guchar, select->xy_rowstride * select->xy_height);
    guchar *src = select->xy_buf;
    guchar *dest = buf;
    gint i;

    for (i = 0; i < select->xy_height; i++) {
      gimp_color_transform_process_pixels(select->transform, format, src,
                                          format, dest, select->xy_width);
    ...
    pixbuf = gdk_pixbuf_new_from_data(
        buf, GDK_COLORSPACE_RGB, FALSE, 8, select->xy_width, select->xy_height,
        select->xy_rowstride, (GdkPixbufDestroyNotify)g_free, NULL);
  } else {
    pixbuf = gdk_pixbuf_new_from_data(select->xy_buf, GDK_COLORSPACE_RGB, FALSE,
                                      8, select->xy_width, select->xy_height,
                                      select->xy_rowstride, NULL, NULL);
  }
...
static gboolean gimp_color_select_z_draw(GtkWidget *widget, cairo_t *cr,
  ...
  if (select->z_needs_render) {
    ...
    gimp_color_select_render(select->z_color, select->z_buf, select->z_width,
                             select->z_height, select->z_rowstride,
                             select->z_color_fill, &selector->hsv,
                             &selector->rgb, select->oog_color);
  ...
  if (select->transform) {
    const Babl *format = babl_format("R'G'B' u8");
    guchar *buf = g_new(guchar, select->z_rowstride * select->z_height);
    ...
    pixbuf = gdk_pixbuf_new_from_data(
        buf, GDK_COLORSPACE_RGB, FALSE, 8, select->z_width, select->z_height,
        select->z_rowstride, (GdkPixbufDestroyNotify)g_free, NULL);
  } else {
    pixbuf = gdk_pixbuf_new_from_data(select->z_buf, GDK_COLORSPACE_RGB, FALSE,
                                      8, select->z_width, select->z_height,
                                      select->z_rowstride, NULL, NULL);
  }
...
static void gimp_color_select_render(GtkWidget *preview, guchar *buf,
...
static void gimp_color_select_create_transform(GimpColorSelect *select) {
  if (select->config) {
    ...
    select->transform = gimp_widget_get_color_transform(
        GTK_WIDGET(select), select->config, profile, format, format);
  }
}
...
static void gimp_color_select_notify_config(GimpColorConfig *config,
  ...
  gimp_rgb_get_uchar(&color, select->oog_color, select->oog_color + 1,
                     select->oog_color + 2);
  select->xy_needs_render = TRUE;
  select->z_needs_render = TRUE;
}
