...
enum { PROP_0, PROP_ICON_NAME, PROP_ICON_SIZE, PROP_OVERRIDE_BACKGROUND };
...
gimp_cell_renderer_toggle_class_init(GimpCellRendererToggleClass *klass) {
  ...
  toggle_cell_signals[CLICKED] = g_signal_new(
      "clicked", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpCellRendererToggleClass, clicked), NULL, NULL,
      _gimp_widgets_marshal_VOID__STRING_FLAGS, G_TYPE_NONE, 2, G_TYPE_STRING,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("icon-name", "Icon Name", "The icon to display", NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_ICON_SIZE,
      g_param_spec_int("icon-size", "Icon Size",
                       "The desired icon size to use in pixel (before applying "
                       "scaling factor)",
                       0, G_MAXINT, DEFAULT_ICON_SIZE,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_OVERRIDE_BACKGROUND,
      g_param_spec_boolean("override-background", "Override Background",
                           "Draw the background if the row is selected", FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}
...
static void gimp_cell_renderer_toggle_render(
  ...
  if ((flags & GTK_CELL_RENDERER_SELECTED) && priv->override_background) {
    gboolean background_set;

    g_object_get(cell, "cell-background-set", &background_set, NULL);

    if (background_set) {
      GdkRGBA *color;

      g_object_get(cell, "cell-background-rgba", &color, NULL);
  ...
  gimp_cell_renderer_toggle_get_size(cell, widget, cell_area, &toggle_rect.x,
                                     &toggle_rect.y, &toggle_rect.width,
                                     &toggle_rect.height);
  gtk_style_context_save(context);
  ...
  if (state & GTK_STATE_FLAG_PRELIGHT)
    gtk_render_frame(context, cr, toggle_rect.x, toggle_rect.y,
  ...
  if (active) {
    ...
    gdk_cairo_set_source_pixbuf(cr, priv->pixbuf, toggle_rect.x, toggle_rect.y);
    cairo_paint(cr);

    g_object_get(cell, "inconsistent", &inconsistent, NULL);
    ...
    if (inconsistent) {
      ...
      cairo_move_to(cr, toggle_rect.x + toggle_rect.width - 1,
                    toggle_rect.y + 1);
      cairo_line_to(cr, toggle_rect.x + 1,
...
static gboolean gimp_cell_renderer_toggle_activate(
  ...
  if (gtk_cell_renderer_toggle_get_activatable(toggle)) {
    ...
    gimp_cell_renderer_toggle_clicked(GIMP_CELL_RENDERER_TOGGLE(cell), path,
                                      state);
...
gimp_cell_renderer_toggle_create_pixbuf(GimpCellRendererToggle *toggle,
  ...
  if (priv->icon_name) {
    ...
    icon_info = gtk_icon_theme_lookup_icon_for_scale(
        icon_theme, icon_name, priv->icon_size, scale_factor,
...
GtkCellRenderer *gimp_cell_renderer_toggle_new(const gchar *icon_name) {
  return g_object_new(GIMP_TYPE_CELL_RENDERER_TOGGLE, "icon-name", icon_name,
                      NULL);
}
