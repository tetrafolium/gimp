...
static void gimp_unit_store_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_unit_store_get_property(GObject *object, guint property_id,
...
G_DEFINE_TYPE_WITH_CODE(GimpUnitStore, gimp_unit_store, G_TYPE_OBJECT,
                        G_ADD_PRIVATE(GimpUnitStore) G_IMPLEMENT_INTERFACE(
                            GTK_TYPE_TREE_MODEL,
...
static GType column_types[GIMP_UNIT_STORE_UNIT_COLUMNS] = {
    G_TYPE_INVALID, G_TYPE_DOUBLE, G_TYPE_INT,    G_TYPE_STRING, G_TYPE_STRING,
...
static void gimp_unit_store_class_init(GimpUnitStoreClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_NUM_VALUES,
      g_param_spec_int("num-values", "Num Values",
                       "The number of values this store provides", 0, G_MAXINT,
                       0, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_HAS_PIXELS,
      g_param_spec_boolean("has-pixels", "Has Pixels",
                           "Whether the store has GIMP_UNIT_PIXELS", TRUE,
                           GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_HAS_PERCENT,
      g_param_spec_boolean("has-percent", "Has Percent",
                           "Whether the store has GIMP_UNIT_PERCENT", FALSE,
                           GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_SHORT_FORMAT,
      g_param_spec_string("short-format", "Short Format",
                          "Format string for a short label", "%a",
                          GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_LONG_FORMAT,
      g_param_spec_string("long-format", "Long Format",
                          "Format string for a long label", "%p",
                          GIMP_PARAM_READWRITE));
}

static void gimp_unit_store_init(GimpUnitStore *store) {
...
private
  ->has_percent = FALSE;
private
  ->short_format = g_strdup("%a");
private
  ->long_format = g_strdup("%p");
private
  ->synced_unit = gimp_unit_get_number_of_units() - 1;
}
...
static void gimp_unit_store_set_property(GObject *object, guint property_id,
  ...
  private
    ->num_values = g_value_get_int(value);
    if (private->num_values) {
    private
      ->values = g_new0(gdouble, private->num_values);
    private
      ->resolutions = g_new0(gdouble, private->num_values);
    }
    break;
  ...
  private
    ->short_format = g_value_dup_string(value);
  ...
  private
    ->long_format = g_value_dup_string(value);
    if (!private->long_format)
    private
    ->long_format = g_strdup("%a");
    break;
...
static void gimp_unit_store_tree_model_get_value(GtkTreeModel *tree_model,
  ...
  g_value_init(value, column < GIMP_UNIT_STORE_UNIT_COLUMNS
                          ? column_types[column]
                          : G_TYPE_DOUBLE);
  ...
  if ((unit >= 0 && unit < gimp_unit_get_number_of_units()) ||
    ...
    case GIMP_UNIT_STORE_UNIT_SHORT_FORMAT:
      g_value_take_string(value,
                          gimp_unit_format_string(private->short_format, unit));
      break;
    case GIMP_UNIT_STORE_UNIT_LONG_FORMAT:
      g_value_take_string(value,
                          gimp_unit_format_string(private->long_format, unit));
      break;
...
GimpUnitStore *gimp_unit_store_new(gint num_values) {
  return g_object_new(GIMP_TYPE_UNIT_STORE, "num-values", num_values, NULL);
}

void gimp_unit_store_set_has_pixels(GimpUnitStore *store, gboolean has_pixels) {
  GimpUnitStorePrivate *private;
...
void gimp_unit_store_set_pixel_value(GimpUnitStore *store, gint index,
                                     gdouble value) {
  GimpUnitStorePrivate *private;
...
void gimp_unit_store_set_pixel_values(GimpUnitStore *store, gdouble first_value,
                                      ...) {
  GimpUnitStorePrivate *private;
  va_list args;
  gint i;
...
void gimp_unit_store_set_resolution(GimpUnitStore *store, gint index,
                                    gdouble resolution) {
  GimpUnitStorePrivate *private;
...
private
  ...
  for (i = 0; i < private->num_values;) {
  private
    ->resolutions[i] = first_resolution;
...
gdouble gimp_unit_store_get_nth_value(GimpUnitStore *store, GimpUnit unit,
                                      gint index) {
  GimpUnitStorePrivate *private;
  GtkTreeIter iter;
...
private
  ...
  gimp_unit_store_tree_model_get_value(GTK_TREE_MODEL(store), &iter,
...
void gimp_unit_store_get_values(GimpUnitStore *store, GimpUnit unit,
                                gdouble *first_value, ...) {
  GimpUnitStorePrivate *private;
  va_list args;
  gint i;
...
private
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    gtk_tree_model_get(model, &iter, GIMP_UNIT_STORE_UNIT, &unit, -1);

    if (unit != GIMP_UNIT_PERCENT && unit > private->synced_unit) {
      GtkTreePath *path;
