...
static void gimp_pick_button_pick(GimpPickButton *button, GdkEvent *event);

static GdkCursor *make_cursor(GdkDisplay *display) {
  GdkPixbuf *pixbuf;
  GError *error = NULL;

  pixbuf = gdk_pixbuf_new_from_resource(
      "/org/gimp/color-picker-cursors/cursor-color-picker.png", &error);
...
static gboolean gimp_pick_button_mouse_press(GtkWidget *invisible,
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
    g_signal_connect(invisible, "motion-notify-event",
                     G_CALLBACK(gimp_pick_button_mouse_motion), button);
    g_signal_connect(invisible, "button-release-event",
                     G_CALLBACK(gimp_pick_button_mouse_release), button);

    g_signal_handlers_disconnect_by_func(invisible,
                                         gimp_pick_button_mouse_press, button);
    g_signal_handlers_disconnect_by_func(invisible, gimp_pick_button_key_press,
                                         button);
...
static gboolean gimp_pick_button_mouse_release(GtkWidget *invisible,
  ...
  g_signal_handlers_disconnect_by_func(invisible, gimp_pick_button_mouse_motion,
                                       button);
  g_signal_handlers_disconnect_by_func(invisible,
                                       gimp_pick_button_mouse_release, button);
...
static void gimp_pick_button_pick(GimpPickButton *button, GdkEvent *event) {
...

  {
    ...
    gimp_rgba_set_uchar(&rgb, GetRValue(win32_color), GetGValue(win32_color),
                        GetBValue(win32_color), 255);
  }
  ...
  if (monitor_profile) {
    ...
    transform =
        gimp_color_transform_new(monitor_profile, format, srgb_profile, format,
                                 GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL, flags);
    g_object_unref(srgb_profile);

    if (transform) {
      gimp_color_transform_process_pixels(transform, format, &rgb, format, &rgb,
                                          1);
      gimp_rgb_clamp(&rgb);
...
void _gimp_pick_button_default_pick(GimpPickButton *button) {
  ...
  if (gdk_seat_grab(gdk_display_get_default_seat(display),
                    ...
                    FALSE, button->priv->cursor, NULL, NULL,
  ...
  g_signal_connect(widget, "button-press-event",
                   G_CALLBACK(gimp_pick_button_mouse_press), button);
  g_signal_connect(widget, "key-press-event",
                   G_CALLBACK(gimp_pick_button_key_press), button);
}
