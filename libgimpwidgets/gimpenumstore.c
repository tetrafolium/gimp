...
static void gimp_enum_store_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_enum_store_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);

static void gimp_enum_store_add_value(GtkListStore *store, GEnumValue *value);
...
static void gimp_enum_store_class_init(GimpEnumStoreClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_ENUM_TYPE,
      g_param_spec_gtype("enum-type", "Enum Type", "The type of the enum",
                         G_TYPE_ENUM,
                         G_PARAM_CONSTRUCT_ONLY | GIMP_PARAM_READWRITE));
}

static void gimp_enum_store_init(GimpEnumStore *store) {
...
static void gimp_enum_store_get_property(GObject *object, guint property_id,
  ...
  case PROP_ENUM_TYPE:
    g_value_set_gtype(
        value,
        (priv->enum_class ? G_TYPE_FROM_CLASS(priv->enum_class) : G_TYPE_NONE));
    break;
...
GtkListStore *gimp_enum_store_new(GType enum_type) {
  ...
  store = gimp_enum_store_new_with_range(enum_type, enum_class->minimum,
                                         enum_class->maximum);
...
GtkListStore *gimp_enum_store_new_with_range(GType enum_type, gint minimum,
  ...
  for (value = priv->enum_class->values; value->value_name; value++) {
...
GtkListStore *gimp_enum_store_new_with_values_valist(GType enum_type,
  ...
  for (i = 0; i < n_values; i++) {
    value = g_enum_get_value(priv->enum_class, va_arg(args, gint));
...
void gimp_enum_store_set_icon_prefix(GimpEnumStore *store,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    if (icon_prefix) {
      ...
      if (enum_value) {
        icon_name = g_strconcat(icon_prefix, "-", enum_value->value_nick, NULL);
      }
    }

    gtk_list_store_set(GTK_LIST_STORE(store), &iter, GIMP_INT_STORE_ICON_NAME,
                       icon_name, -1);
