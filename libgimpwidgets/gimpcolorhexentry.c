...
enum { COLUMN_NAME, COLUMN_COLOR, NUM_COLUMNS };
...
static gboolean gimp_color_hex_entry_events(GtkWidget *widget, GdkEvent *event);
...
static void gimp_color_hex_entry_class_init(GimpColorHexEntryClass *klass) {
  ...
  entry_signals[COLOR_CHANGED] = g_signal_new(
      "color-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpColorHexEntryClass, color_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
...
private
  ...
  for (i = 0; i < num_colors; i++) {
    ...
    gtk_list_store_set(store, &iter, COLUMN_NAME, names[i], COLUMN_COLOR,
                       colors + i, -1);
  }
  ...
  completion = g_object_new(GTK_TYPE_ENTRY_COMPLETION, "model", store, NULL);
  g_object_unref(store);

  cell = gimp_cell_renderer_color_new();
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(completion), cell, FALSE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(completion), cell, "color",
                                 COLUMN_COLOR, NULL);
  ...
  g_signal_connect(entry, "focus-out-event",
                   G_CALLBACK(gimp_color_hex_entry_events), NULL);
  g_signal_connect(entry, "key-press-event",
                   G_CALLBACK(gimp_color_hex_entry_events), NULL);

  g_signal_connect(completion, "match-selected",
                   G_CALLBACK(gimp_color_hex_entry_matched), entry);
}
...
static gboolean gimp_color_hex_entry_events(GtkWidget *widget,
  ...
  case GDK_FOCUS_CHANGE: {
    ...
    if (g_ascii_strcasecmp(buffer, text) != 0) {
      ...
      if (len > 0 && (gimp_rgb_parse_hex(&color, text, len) ||
