...
GtkWidget *gimp_int_radio_group_new(
    ...
    GCallback radio_button_callback, gpointer radio_button_callback_data,
    GDestroyNotify radio_button_callback_destroy, gint initial, /* item_data */
  ...
  while (label) {
    ...
    g_signal_connect(button, "toggled", radio_button_callback,
                     radio_button_callback_data);
...
void gimp_int_radio_group_set_active(GtkRadioButton *radio_button,
  ...
  for (group = gtk_radio_button_get_group(radio_button); group;
...
static void gimp_random_seed_update(GtkWidget *widget, gpointer data) {
 ...
 **/
GtkWidget *gimp_random_seed_new(guint *seed, gboolean *random_seed) {
  GtkWidget *hbox;
  GtkWidget *toggle;
  ...
  g_signal_connect(adj, "value-changed",
                   G_CALLBACK(gimp_uint_adjustment_update), seed);
   ...
   */
  g_signal_connect(button, "clicked", G_CALLBACK(gimp_random_seed_update),
                   spinbutton);
  ...
  g_signal_connect(toggle, "toggled", G_CALLBACK(gimp_random_seed_update),
                   spinbutton);
  ...
  g_object_bind_property(toggle, "active", button, "sensitive",
                         G_BINDING_SYNC_CREATE | G_BINDING_INVERT_BOOLEAN);
...
static void gimp_coordinates_callback(GtkWidget *widget,
  ...
  if (gimp_chain_button_get_active(data->chainbutton)) {
    if (data->chain_constrains_ratio) {
      if ((data->orig_x != 0) && (data->orig_y != 0)) {
        g_signal_handlers_block_by_func(widget, gimp_coordinates_callback,
                                        data);
        ...
        if (ROUND(new_x) != ROUND(data->last_x)) {
          ...
          gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(widget), 1, new_y);
          data->last_y = gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(widget), 1);
        } else if (ROUND(new_y) != ROUND(data->last_y)) {
          data->last_y = new_y;
          new_x = (new_y * data->orig_x) / data->orig_y;

          gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(widget), 0, new_x);
          data->last_x = gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(widget), 0);
        }

        g_signal_handlers_unblock_by_func(widget, gimp_coordinates_callback,
                                          data);
      }
    } else {
...
static void gimp_coordinates_data_free(GimpCoordinatesData *data) {
  g_slice_free(GimpCoordinatesData, data);
}

static void gimp_coordinates_chainbutton_toggled(GimpChainButton *button,
                                                 GimpSizeEntry *entry) {
...
GtkWidget *gimp_coordinates_new(
    GimpUnit unit, const gchar *unit_format, gboolean menu_show_pixels,
    gboolean menu_show_percent, gint spinbutton_width,
    GimpSizeEntryUpdatePolicy update_policy,

    gboolean chainbutton_active, gboolean chain_constrains_ratio,

    const gchar *xlabel, gdouble x, gdouble xres, gdouble lower_boundary_x,
    gdouble upper_boundary_x, gdouble xsize_0, /* % */
    gdouble xsize_100,                         /* % */

    const gchar *ylabel, gdouble y, gdouble yres, gdouble lower_boundary_y,
    gdouble upper_boundary_y, gdouble ysize_0, /* % */
    gdouble ysize_100 /* % */) {
  GimpCoordinatesData *data;
  ...
  sizeentry = gimp_size_entry_new(1, unit, unit_format, menu_show_pixels,
                                  menu_show_percent, FALSE, spinbutton_width,
                                  update_policy);
  ...
  if (menu_show_percent) {
    gimp_size_entry_set_size(GIMP_SIZE_ENTRY(sizeentry), 0, xsize_0, xsize_100);
    gimp_size_entry_set_size(GIMP_SIZE_ENTRY(sizeentry), 1, ysize_0, ysize_100);
  }

  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(sizeentry), 0, x);
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(sizeentry), 1, y);

  gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(sizeentry), xlabel, 0, 0, 0.0);
  gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(sizeentry), ylabel, 1, 0, 0.0);
