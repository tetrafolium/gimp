...
enum { PROP_0, PROP_ID_COLUMN, PROP_LABEL_COLUMN, PROP_ELLIPSIZE };
...
static void gimp_string_combo_box_class_init(GimpStringComboBoxClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_ID_COLUMN,
      g_param_spec_int("id-column", "ID Column",
                       "The model column that holds the ID", 0, G_MAXINT, 0,
   ...
   */
  g_object_class_install_property(
      object_class, PROP_LABEL_COLUMN,
      g_param_spec_int("label-column", "Label Column",
                       "The model column that holds the label", 0, G_MAXINT, 0,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
...
static void gimp_string_combo_box_init(GimpStringComboBox *combo_box) {
...
static void gimp_string_combo_box_constructed(GObject *object) {
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(object), cell, "text",
                                 priv->label_column, NULL);
}
...
static void gimp_string_combo_box_get_property(GObject *object,
  ...
  case PROP_ELLIPSIZE:
    g_object_get_property(G_OBJECT(priv->text_renderer), pspec->name, value);
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID(object, property_id, pspec);
    break;
...
static gboolean gimp_string_model_lookup(GtkTreeModel *model, gint column,
                                         const gchar *id, GtkTreeIter *iter) {
 ...
 * Since: 2.4
 **/
GtkWidget *gimp_string_combo_box_new(GtkTreeModel *model, gint id_column,
                                     gint label_column) {
  g_return_val_if_fail(GTK_IS_TREE_MODEL(model), NULL);
  g_return_val_if_fail(
      gtk_tree_model_get_column_type(model, id_column) == G_TYPE_STRING, NULL);
  g_return_val_if_fail(gtk_tree_model_get_column_type(model, label_column) ==
                           G_TYPE_STRING,
                       NULL);

  return g_object_new(GIMP_TYPE_STRING_COMBO_BOX, "model", model, "id-column",
                      id_column, "label-column", label_column, NULL);
}
