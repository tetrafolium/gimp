...
#define GIMP_COLOR_SCALES_CLASS(klass)                                         \
  (G_TYPE_CHECK_CLASS_CAST((klass), GIMP_TYPE_COLOR_SCALES,                    \
                           GimpColorScalesClass))
#define GIMP_IS_COLOR_SCALES_CLASS(klass)                                      \
  (G_TYPE_CHECK_CLASS_TYPE((klass), GIMP_TYPE_COLOR_SCALES))
#define GIMP_COLOR_SCALES_GET_CLASS(obj)                                       \
  (G_TYPE_INSTANCE_GET_CLASS((obj), GIMP_TYPE_COLOR_SCALES,                    \
                             GimpColorScalesClass))
...
static void gimp_color_scales_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);
static void gimp_color_scales_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
...
static void gimp_color_scales_set_channel(GimpColorSelector *selector,
                                          GimpColorSelectorChannel channel);
static void gimp_color_scales_set_model_visible(GimpColorSelector *selector,
...
static void gimp_color_scales_update_scales(GimpColorScales *scales, gint skip);
...
static void gimp_color_scales_class_init(GimpColorScalesClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_SHOW_RGB_U8,
      g_param_spec_boolean("show-rgb-u8", "Show RGB 0..255",
                           "Show RGB 0..255 scales", FALSE,
...
static GtkWidget *create_group(GimpColorScales *scales, GSList **radio_group,
  ...
  for (i = first_channel, row = 0; i <= last_channel; i++, row++) {
    ...
    if (i == GIMP_COLOR_SELECTOR_ALPHA || i == GIMP_COLOR_SELECTOR_ALPHA_U8) {
      ...
      *radio_group =
      ...
      if (is_u8) {
        ...
        g_signal_connect(scales->toggles[i], "toggled",
                         G_CALLBACK(gimp_color_scales_toggle_changed), scales);
      }
    }
    ...
    gimp_help_set_help_data(scales->toggles[i], gettext(enum_desc->value_help),
                            NULL);

    gtk_size_group_add_widget(size_group0, scales->toggles[i]);

    scales->scales[i] = gimp_color_scale_entry_new(
        gettext(enum_desc->value_desc), scale_defs[i].default_value,
        scale_defs[i].spin_min_value, scale_defs[i].spin_max_value, 1);
    gtk_grid_attach(GTK_GRID(grid), scales->scales[i], 1, row, 3, 1);
    gimp_label_spin_set_increments(GIMP_LABEL_SPIN(scales->scales[i]), 1.0,
                                   scale_defs[i].scale_inc);
    gimp_help_set_help_data(scales->scales[i], gettext(enum_desc->value_help),
                            NULL);
    gtk_widget_show(scales->scales[i]);

    gimp_scale_entry_set_bounds(GIMP_SCALE_ENTRY(scales->scales[i]),
                                scale_defs[i].scale_min_value,
                                scale_defs[i].scale_max_value, TRUE);

    g_object_add_weak_pointer(G_OBJECT(scales->scales[i]),
                              (gpointer)&scales->scales[i]);

    gimp_color_scale_set_channel(GIMP_COLOR_SCALE(gimp_scale_entry_get_range(
                                     GIMP_SCALE_ENTRY(scales->scales[i]))),
                                 enum_value);
    ...
    g_signal_connect(scales->scales[i], "value-changed",
                     G_CALLBACK(gimp_color_scales_scale_changed), scales);
  }
...
static void gimp_color_scales_init(GimpColorScales *scales) {
  ...
  u8_group =
      gtk_radio_button_get_group(GTK_RADIO_BUTTON(scales->dummy_u8_toggle));
  ...
  scales->rgb_percent_group = grid =
      create_group(scales, &main_group, size_group0, size_group1, size_group2,
                   GIMP_COLOR_SELECTOR_RED, GIMP_COLOR_SELECTOR_BLUE);
  gtk_box_pack_start(GTK_BOX(scales), grid, FALSE, FALSE, 0);

  scales->rgb_u8_group = grid =
      create_group(scales, &u8_group, size_group0, size_group1, size_group2,
                   GIMP_COLOR_SELECTOR_RED_U8, GIMP_COLOR_SELECTOR_BLUE_U8);
  gtk_box_pack_start(GTK_BOX(scales), grid, FALSE, FALSE, 0);

  scales->lch_group = grid = create_group(
      scales, &main_group, size_group0, size_group1, size_group2,
      GIMP_COLOR_SELECTOR_LCH_LIGHTNESS, GIMP_COLOR_SELECTOR_LCH_HUE);
  gtk_box_pack_start(GTK_BOX(scales), grid, FALSE, FALSE, 0);

  scales->hsv_group = grid =
      create_group(scales, &main_group, size_group0, size_group1, size_group2,
                   GIMP_COLOR_SELECTOR_HUE, GIMP_COLOR_SELECTOR_VALUE);
  gtk_box_pack_start(GTK_BOX(scales), grid, FALSE, FALSE, 0);

  scales->alpha_percent_group = grid =
      create_group(scales, &main_group, size_group0, size_group1, size_group2,
                   GIMP_COLOR_SELECTOR_ALPHA, GIMP_COLOR_SELECTOR_ALPHA);
  gtk_box_pack_start(GTK_BOX(scales), grid, FALSE, FALSE, 0);

  scales->alpha_u8_group = grid =
      create_group(scales, &u8_group, size_group0, size_group1, size_group2,
                   GIMP_COLOR_SELECTOR_ALPHA_U8, GIMP_COLOR_SELECTOR_ALPHA_U8);
  gtk_box_pack_start(GTK_BOX(scales), grid, FALSE, FALSE, 0);
  ...
  g_object_bind_property(G_OBJECT(radio2), "active", G_OBJECT(scales),
                         "show-rgb-u8",
                         G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);
  ...
  g_signal_connect(radio1, "toggled",
                   G_CALLBACK(gimp_color_scales_toggle_lch_hsv), scales);
}
...
static void gimp_color_scales_set_channel(GimpColorSelector *selector,
  ...
  if (GTK_IS_RADIO_BUTTON(scales->toggles[channel])) {
    g_signal_handlers_block_by_func(scales->toggles[channel],
                                    gimp_color_scales_toggle_changed, scales);
  ...
  for (i = 0; i < G_N_ELEMENTS(scale_defs); i++) {
    if (scales->scales[i])
      gimp_color_scale_set_color_config(
          GIMP_COLOR_SCALE(
              gimp_scale_entry_get_range(GIMP_SCALE_ENTRY(scales->scales[i]))),
          config);
  }
}
...
static void gimp_color_scales_update_visible(GimpColorScales *scales) {
  ...
  rgb_visible = gimp_color_selector_get_model_visible(
      selector, GIMP_COLOR_SELECTOR_MODEL_RGB);
  lch_visible = gimp_color_selector_get_model_visible(
      selector, GIMP_COLOR_SELECTOR_MODEL_LCH);
  hsv_visible = gimp_color_selector_get_model_visible(
      selector, GIMP_COLOR_SELECTOR_MODEL_HSV);
  ...
  for (i = 0; i < G_N_ELEMENTS(scale_defs); i++) {
    if (i != skip) {
      g_signal_handlers_block_by_func(scales->scales[i],
                                      gimp_color_scales_scale_changed, scales);

      gimp_label_spin_set_value(GIMP_LABEL_SPIN(scales->scales[i]), values[i]);

      g_signal_handlers_unblock_by_func(
          scales->scales[i], gimp_color_scales_scale_changed, scales);
    }

    gimp_color_scale_set_color(GIMP_COLOR_SCALE(gimp_scale_entry_get_range(
                                   GIMP_SCALE_ENTRY(scales->scales[i]))),
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
    ...
    for (i = 0; i < G_N_ELEMENTS(scale_defs); i++) {
      if (widget == scales->toggles[i]) {
        ...
        if (i < GIMP_COLOR_SELECTOR_RED || i > GIMP_COLOR_SELECTOR_BLUE) {
          gtk_toggle_button_set_active(
              GTK_TOGGLE_BUTTON(scales->dummy_u8_toggle), TRUE);
        }
...
static void gimp_color_scales_toggle_lch_hsv(GtkToggleButton *toggle,
  ...
  if (gtk_toggle_button_get_active(toggle)) {
    gimp_color_selector_set_model_visible(selector,
                                          GIMP_COLOR_SELECTOR_MODEL_LCH, TRUE);
    gimp_color_selector_set_model_visible(selector,
                                          GIMP_COLOR_SELECTOR_MODEL_HSV, FALSE);
  } else {
    gimp_color_selector_set_model_visible(selector,
                                          GIMP_COLOR_SELECTOR_MODEL_LCH, FALSE);
    gimp_color_selector_set_model_visible(selector,
                                          GIMP_COLOR_SELECTOR_MODEL_HSV, TRUE);
  }
}
