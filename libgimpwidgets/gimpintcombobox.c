...
enum { PROP_0, PROP_ELLIPSIZE, PROP_LABEL, PROP_LAYOUT, PROP_VALUE };
...
static void gimp_int_combo_box_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_int_combo_box_get_property(GObject *object, guint property_id,
...
static void gimp_int_combo_box_class_init(GimpIntComboBoxClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("ellipsize", "Ellipsize",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("label", "Label", "An optional label to be displayed",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("layout", "Layout", "Combo box layout",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int("value", "Value", "Value of active item", G_MININT,
...
static void gimp_int_combo_box_init(GimpIntComboBox *combo_box) {
  ...
  g_signal_connect(combo_box, "changed", G_CALLBACK(gimp_int_combo_box_changed),
                   NULL);
}
...
static void gimp_int_combo_box_set_property(GObject *object, guint property_id,
  ...
  case PROP_ELLIPSIZE:
    ...
    if (priv->text_renderer) {
      g_object_set_property(G_OBJECT(priv->text_renderer), pspec->name, value);
    }
    break;
  case PROP_LABEL:
...
GtkWidget *gimp_int_combo_box_new_array(gint n_values, const gchar *labels[]) {
  ...
  for (i = 0; i < n_values; i++) {
    ...
    if (labels[i]) {
      ...
      gtk_list_store_set(store, &iter, GIMP_INT_STORE_VALUE, i,
                         GIMP_INT_STORE_LABEL, gettext(labels[i]), -1);
    }
  }
 ...
 * Since: 2.10
 **/
gboolean gimp_int_combo_box_set_active_by_user_data(GimpIntComboBox *combo_box,
                                                    gpointer user_data) {
  GtkTreeModel *model;
  GtkTreeIter iter;
 ...
 * Since: 2.10
 **/
gboolean gimp_int_combo_box_get_active_user_data(GimpIntComboBox *combo_box,
                                                 gpointer *user_data) {
  GtkTreeIter iter;
  ...
  if (gtk_combo_box_get_active_iter(GTK_COMBO_BOX(combo_box), &iter)) {
    gtk_tree_model_get(gtk_combo_box_get_model(GTK_COMBO_BOX(combo_box)), &iter,
                       GIMP_INT_STORE_USER_DATA, user_data, -1);
    return TRUE;
  }
 ...
 * Since: 2.2
 **/
gulong gimp_int_combo_box_connect(GimpIntComboBox *combo_box, gint value,
...
void gimp_int_combo_box_set_label(GimpIntComboBox *combo_box,
  ...
  if (priv->label) {
    ...
    g_signal_handlers_disconnect_by_func(combo_box,
                                         gimp_int_combo_box_create_cells, NULL);
  }
  ...
  if (label) {
    ...
    g_signal_connect(combo_box, "notify::popup-shown",
                     G_CALLBACK(gimp_int_combo_box_create_cells), NULL);
  }
...
static void gimp_int_combo_box_create_cells(GimpIntComboBox *combo_box) {
  ...
  if (popup_shown) {
    ...
    g_object_set(pixbuf_renderer, "xpad", 2, NULL);
    ...
    gtk_cell_layout_set_attributes(layout, pixbuf_renderer, "icon-name",
                                   GIMP_INT_STORE_ICON_NAME, "pixbuf",
                                   GIMP_INT_STORE_PIXBUF, NULL);
    gtk_cell_layout_set_attributes(layout, text_renderer, "text",
                                   GIMP_INT_STORE_LABEL, NULL);

    if (priv->sensitivity_func) {
      gtk_cell_layout_set_cell_data_func(
          layout, pixbuf_renderer, gimp_int_combo_box_data_func, priv, NULL);

      gtk_cell_layout_set_cell_data_func(
          layout, text_renderer, gimp_int_combo_box_data_func, priv, NULL);
    }
  } else {
    /*  combo box layout  */
    ...
    if (priv->layout != GIMP_INT_COMBO_BOX_LAYOUT_ICON_ONLY) {
      ...
      g_object_set(priv->text_renderer, "ellipsize", priv->ellipsize, NULL);
    } else {
      text_renderer = NULL;
    }
    ...
    if (priv->label) {
      ...
      g_object_set(label_renderer, "text", priv->label, NULL);
...
static void gimp_int_combo_box_data_func(GtkCellLayout *layout,
  ...
  if (priv->layout == GIMP_INT_COMBO_BOX_LAYOUT_ABBREVIATED &&
    ...
    gtk_tree_model_get(model, iter, GIMP_INT_STORE_ABBREV, &abbrev, -1);

    if (abbrev) {
      g_object_set(cell, "text", abbrev, NULL);
