...
static void gimp_zoom_model_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_zoom_model_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);

static guint zoom_model_signals[LAST_SIGNAL] = {
    0,
};
...
static void gimp_zoom_model_class_init(GimpZoomModelClass *klass) {
  ...
  zoom_model_signals[ZOOMED] =
      g_signal_new("zoomed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpZoomModelClass, zoomed), NULL, NULL,
                   _gimp_widgets_marshal_VOID__DOUBLE_DOUBLE, G_TYPE_NONE, 2,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("value", "Value", "Zoom factor", ZOOM_MIN, ZOOM_MAX,
                          1.0, GIMP_PARAM_READWRITE));
  /**
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("minimum", "Minimum",
                          "Lower limit for the zoom factor", ZOOM_MIN, ZOOM_MAX,
                          ZOOM_MIN, GIMP_PARAM_READWRITE));
  /**
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("maximum", "Maximum",
                          "Upper limit for the zoom factor", ZOOM_MIN, ZOOM_MAX,
                          ZOOM_MAX, GIMP_PARAM_READWRITE));
   ...
   */
  g_object_class_install_property(
      object_class, PROP_FRACTION,
      g_param_spec_string("fraction", "Fraction",
                          "The zoom factor expressed as a fraction", "1:1",
                          GIMP_PARAM_READABLE));
  /**
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("percentage", "Percentage",
                          "The zoom factor expressed as a percentage", "100%",
                          GIMP_PARAM_READABLE));
}

static void gimp_zoom_model_init(GimpZoomModel *model) {
...
static void gimp_zoom_model_set_property(GObject *object, guint property_id,
  ...
  if (priv->value != previous_value) {
    g_signal_emit(object, zoom_model_signals[ZOOMED], 0, previous_value,
                  priv->value);
  }
}
...
static void gimp_zoom_model_get_property(GObject *object, guint property_id,
  ...
  case PROP_FRACTION: {
    ...
    gimp_zoom_model_get_fraction(GIMP_ZOOM_MODEL(object), &numerator,
                                 &denominator);
...
static void gimp_zoom_model_zoom_in(GimpZoomModel *model) {
...
static void gimp_zoom_model_zoom_out(GimpZoomModel *model) {
  GimpZoomModelPrivate *priv = GET_PRIVATE(model);
 ...
 * Since GIMP 2.4
 **/
void gimp_zoom_model_zoom(GimpZoomModel *model, GimpZoomType zoom_type,
                          gdouble scale) {
  gdouble delta = 0.0;
  ...
  g_object_set(model, "value",
               gimp_zoom_model_zoom_step(zoom_type, scale, delta), NULL);
}
 ...
 * Since GIMP 2.4
 **/
void gimp_zoom_model_get_fraction(GimpZoomModel *model, gint *numerator,
                                  gint *denominator) {
  gint p0, p1, p2;
  gint q0, q1, q2;
  gdouble zoom_factor;
...
static GtkWidget *zoom_button_new(const gchar *icon_name,
                                  GtkIconSize icon_size) {
  GtkWidget *button;
  GtkWidget *image;

  image = gtk_image_new_from_icon_name(
      icon_name, icon_size > 0 ? icon_size : GTK_ICON_SIZE_BUTTON);
...
static void zoom_in_button_callback(GimpZoomModel *model, gdouble old,
                                    gdouble new, GtkWidget *button) {
  GimpZoomModelPrivate *priv = GET_PRIVATE(model);

  gtk_widget_set_sensitive(button, priv->value != priv->maximum);
}

static void zoom_out_button_callback(GimpZoomModel *model, gdouble old,
                                     gdouble new, GtkWidget *button) {
  GimpZoomModelPrivate *priv = GET_PRIVATE(model);
 ...
 * Since GIMP 2.4
 **/
GtkWidget *gimp_zoom_button_new(GimpZoomModel *model, GimpZoomType zoom_type,
                                GtkIconSize icon_size) {
  ...
  case GIMP_ZOOM_IN:
    ...
    g_signal_connect_swapped(button, "clicked",
                             G_CALLBACK(gimp_zoom_model_zoom_in), model);
    g_signal_connect_object(model, "zoomed",
                            G_CALLBACK(zoom_in_button_callback), button, 0);
    break;

  case GIMP_ZOOM_OUT:
    button = zoom_button_new("zoom-out", icon_size);
    g_signal_connect_swapped(button, "clicked",
                             G_CALLBACK(gimp_zoom_model_zoom_out), model);
    g_signal_connect_object(model, "zoomed",
                            G_CALLBACK(zoom_out_button_callback), button, 0);
    break;
  ...
  if (button) {
    ...
    if (icon_size > 0) {
      ...
      if (gimp_enum_get_value(GIMP_TYPE_ZOOM_TYPE, zoom_type, NULL, NULL, &desc,
                              NULL)) {
...
#define ZOOM_MIN_STEP 1.1
  ...
  const gdouble presets[] = {
      1.0 / 256, 1.0 / 180, 1.0 / 128, 1.0 / 90, 1.0 / 64, 1.0 / 45, 1.0 / 32,
      1.0 / 23,  1.0 / 16,  1.0 / 11,  1.0 / 8,  2.0 / 11, 1.0 / 4,  1.0 / 3,
      1.0 / 2,   2.0 / 3,   1.0,       3.0 / 2,  2.0,      3.0,      4.0,
      11.0 / 2,  8.0,       11.0,      16.0,     23.0,     32.0,     45.0,
      64.0,      90.0,      128.0,     180.0,    256.0,
  };
