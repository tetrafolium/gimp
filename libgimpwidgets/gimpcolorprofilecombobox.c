...
enum { PROP_0, PROP_DIALOG, PROP_MODEL };
...
gimp_color_profile_combo_box_class_init(GimpColorProfileComboBoxClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_DIALOG,
      g_param_spec_object(
          "dialog", "Dialog",
          "The dialog to present when selecting profiles from disk",
          GTK_TYPE_DIALOG, G_PARAM_CONSTRUCT_ONLY | GIMP_PARAM_READWRITE));
  /**
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("model", "Model",
                          ...
                          GIMP_TYPE_COLOR_PROFILE_STORE, GIMP_PARAM_READWRITE));
}
...
gimp_color_profile_combo_box_init(GimpColorProfileComboBox *combo_box) {
  ...
  combo_box->priv =
      gimp_color_profile_combo_box_get_instance_private(combo_box);

  cell = gtk_cell_renderer_text_new();

  g_object_set(cell, "width-chars", 42, "ellipsize", PANGO_ELLIPSIZE_END, NULL);

  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(combo_box), cell, TRUE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combo_box), cell, "text",
                                 GIMP_COLOR_PROFILE_STORE_LABEL, NULL);
...
static void gimp_color_profile_combo_box_set_property(GObject *object,
  ...
  private
    ->dialog = g_value_dup_object(value);
  ...
  case PROP_MODEL:
    gtk_combo_box_set_model(GTK_COMBO_BOX(object), g_value_get_object(value));
    break;
...
static void gimp_color_profile_combo_box_get_property(GObject *object,
  ...
  case PROP_MODEL:
    g_value_set_object(value, gtk_combo_box_get_model(GTK_COMBO_BOX(object)));
    break;
 ...
 * Since: 2.4
 **/
GtkWidget *gimp_color_profile_combo_box_new(GtkWidget *dialog, GFile *history) {
  GtkWidget *combo;
  GtkListStore *store;
...
GtkWidget *gimp_color_profile_combo_box_new_with_model(GtkWidget *dialog,
  ...
  return g_object_new(GIMP_TYPE_COLOR_PROFILE_COMBO_BOX, "dialog", dialog,
                      "model", model, NULL);
}
...
void gimp_color_profile_combo_box_add_file(GimpColorProfileComboBox *combo,
  ...
  gimp_color_profile_store_add_file(GIMP_COLOR_PROFILE_STORE(model), file,
                                    label);
}
...
void gimp_color_profile_combo_box_set_active_file(
    GimpColorProfileComboBox *combo, GFile *file, const gchar *label) {
...
gimp_color_profile_combo_box_get_active_file(GimpColorProfileComboBox *combo) {
  ...
  if (gtk_combo_box_get_active_iter(GTK_COMBO_BOX(combo), &iter)) {
    ...
    gtk_tree_model_get(model, &iter, GIMP_COLOR_PROFILE_STORE_ITEM_TYPE, &type,
                       GIMP_COLOR_PROFILE_STORE_FILE, &file, -1);
