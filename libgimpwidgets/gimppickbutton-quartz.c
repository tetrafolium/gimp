...
- (id)initWithButton:(GimpPickButton *)_button
          controller:(GimpPickWindowController *)controller;
@end
...
- (id)initWithButton:(GimpPickButton *)_button
          controller:(GimpPickWindowController *)_controller {
  self = [super init];
...
- (void)pickColor:(NSEvent *)event {
  ...
  rect.origin.y =
      [[[NSScreen screens] objectAtIndex:0] frame].size.height - rect.origin.y;
  ...
  if (color_space) {
    ...
    if (icc_data) {
      ...
      profile = gimp_color_profile_new_from_icc_profile(
          buffer, CFDataGetLength(icc_data), NULL);
      g_free(buffer);
      CFRelease(icc_data);
    }
  }
  ...
  if (profile) {
    ...
    transform =
        gimp_color_transform_new(profile, format, srgb_profile, format,
...
- (id)initWithButton:(GimpPickButton *)button
...
- (void)windowDidBecomeKey:(NSNotification *)aNotification {
  ...
  event =
      [NSEvent enterExitEventWithType:NSMouseEntered
...
+ (NSCursor *)makePickCursor {
  ...
  bytes = g_resources_lookup_data(
      "/org/gimp/color-picker-cursors-raw/cursor-color-picker.png",
  ...
  if (bytes) {
    ...
    NSCursor *cursor =
        [[NSCursor alloc] initWithImage:image hotSpot:NSMakePoint(1, 30)];
