...
GtkWidget *gimp_enum_radio_box_new(GType enum_type, GCallback callback,
  ...
  vbox = gimp_enum_radio_box_new_with_range(
      enum_type, enum_class->minimum, enum_class->maximum, callback,
      callback_data, callback_data_destroy, first_button);
...
gimp_enum_radio_box_new_with_range(GType enum_type, gint minimum, gint maximum,
  ...
  g_object_weak_ref(G_OBJECT(vbox), (GWeakNotify)g_type_class_unref,
                    enum_class);

  if (callback_data_destroy)
    g_object_weak_ref(G_OBJECT(vbox), (GWeakNotify)callback_data_destroy,
                      callback_data);
  ...
  for (value = enum_class->values; value->value_name; value++) {
    ...
    if (callback)
      g_signal_connect(button, "toggled", callback, callback_data);
  }
 ...
 * #GtkFrame.
 *
 * Returns: (transfer full): a new #GtkFrame holding a group of
 *#GtkRadioButtons.
 *
 * Since: 2.4
 **/
GtkWidget *gimp_enum_radio_frame_new(GType enum_type, GtkWidget *label_widget,
  ...
  radio_box = gimp_enum_radio_box_new(enum_type, callback, callback_data,
                                      callback_data_destroy, first_button);
 ...
 * @minimum:       the minimum enum value
 * @maximum:       the maximum enum value
 * @label_widget: (nullable): a widget to put into the frame that will hold the
 *radio box.
 * @callback: (nullable): a callback to connect to the "toggled" signal of each
 *                        #GtkRadioButton that is created.
 * @callback_data: data to pass to the @callback.
...
GtkWidget *gimp_enum_radio_frame_new_with_range(
  ...
  radio_box = gimp_enum_radio_box_new_with_range(
      enum_type, minimum, maximum, callback, callback_data,
      callback_data_destroy, first_button);
  gtk_container_add(GTK_CONTAINER(frame), radio_box);
  gtk_widget_show(radio_box);
 ...
 *
 * Returns: (transfer full): a new horizontal #GtkBox holding a group of
 *#GtkRadioButtons.
 *
 * Since: 2.10
 **/
GtkWidget *gimp_enum_icon_box_new(GType enum_type, const gchar *icon_prefix,
  ...
  box = gimp_enum_icon_box_new_with_range(
      enum_type, enum_class->minimum, enum_class->maximum, icon_prefix,
      icon_size, callback, callback_data, callback_data_destroy, first_button);
...
GtkWidget *gimp_enum_icon_box_new_with_range(
  ...
  g_object_weak_ref(G_OBJECT(hbox), (GWeakNotify)g_type_class_unref,
                    enum_class);

  if (callback_data_destroy)
    g_object_weak_ref(G_OBJECT(hbox), (GWeakNotify)callback_data_destroy,
                      callback_data);
  ...
  for (value = enum_class->values; value->value_name; value++) {
    ...
    gimp_help_set_help_data(button, gimp_enum_value_get_desc(enum_class, value),
                            NULL);
...
void gimp_enum_icon_box_set_child_padding(GtkWidget *icon_box, gint xpad,
  ...
  for (list = children; list; list = g_list_next(list)) {
    ...
    g_object_get(child, "margin-start", &start, "margin-end", &end,
                 "margin-top", &top, "margin-bottom", &bottom, NULL);

    g_object_set(child, "margin-start", xpad < 0 ? start : xpad, "margin-end",
                 xpad < 0 ? end : xpad, "margin-top", ypad < 0 ? top : ypad,
                 "margin-bottom", ypad < 0 ? bottom : ypad, NULL);
  }
