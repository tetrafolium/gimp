...
enum { CHANGED, ADDED, REMOVED, REORDERED, LAST_SIGNAL };
...
static void gimp_color_display_stack_display_enabled(
    GimpColorDisplay *display, GParamSpec *pspec, GimpColorDisplayStack *stack);
...
gimp_color_display_stack_class_init(GimpColorDisplayStackClass *klass) {
  ...
  stack_signals[CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpColorDisplayStackClass, changed), NULL,
                   NULL, NULL, G_TYPE_NONE, 0);

  stack_signals[ADDED] =
      g_signal_new("added", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpColorDisplayStackClass, added), NULL,
                   NULL, _gimp_widgets_marshal_VOID__OBJECT_INT, G_TYPE_NONE, 2,
                   GIMP_TYPE_COLOR_DISPLAY, G_TYPE_INT);

  stack_signals[REMOVED] =
      g_signal_new("removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpColorDisplayStackClass, removed), NULL,
                   NULL, _gimp_widgets_marshal_VOID__OBJECT, G_TYPE_NONE, 1,
                   GIMP_TYPE_COLOR_DISPLAY);

  stack_signals[REORDERED] =
      g_signal_new("reordered", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpColorDisplayStackClass, reordered), NULL,
                   NULL, _gimp_widgets_marshal_VOID__OBJECT_INT, G_TYPE_NONE, 2,
                   GIMP_TYPE_COLOR_DISPLAY, G_TYPE_INT);
...
private
  ->filters = g_list_append(private->filters, g_object_ref(display));
  ...
  g_signal_emit(stack, stack_signals[ADDED], 0, display,
                g_list_length(private->filters) - 1);
...
private
  ->filters = g_list_remove(private->filters, display);
...
private
  ...
  if (list->prev) {
    ...
    g_signal_emit(stack, stack_signals[REORDERED], 0, display,
                  g_list_position(private->filters, list->prev));
...
private
  ...
  if (list->next) {
    ...
    g_signal_emit(stack, stack_signals[REORDERED], 0, display,
                  g_list_position(private->filters, list->next));
...
static void gimp_color_display_stack_disconnect(GimpColorDisplayStack *stack,
  ...
  g_signal_handlers_disconnect_by_func(
      display, gimp_color_display_stack_display_changed, stack);
  g_signal_handlers_disconnect_by_func(
      display, gimp_color_display_stack_display_enabled, stack);
}
