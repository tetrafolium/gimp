...
static gboolean test_units(const gchar *ident, GimpEevlQuantity *factor,
                           gdouble *offset, gpointer data) {
  gboolean resolved = FALSE;
  gboolean default_unit = (ident == NULL);

  *offset = 0.0;

  if (default_unit || (ident && strcmp("in", ident) == 0)) {
    factor->dimension = 1;
    factor->value = 1.;
...
int main(void) {
  ...
  TestCase cases[] = {/* "Default" test case */
                      {"2in + 3in", {2 + 3, 1}, TRUE},
  ...
  for (i = 0; cases[i].string; i++) {
    ...
    gimp_eevl_evaluate(test, &options, &result, &error_pos, &error);
    ...
    if (error || error_pos) {
      ...
      failed++;
    } else if (should.value != result.value ||
               should.dimension != result.dimension) {
