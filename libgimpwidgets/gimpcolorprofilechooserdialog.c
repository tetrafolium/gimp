...
static void gimp_color_profile_chooser_dialog_add_shortcut(
...
static void gimp_color_profile_chooser_dialog_class_init(
    GimpColorProfileChooserDialogClass *klass) {
...
gimp_color_profile_chooser_dialog_init(GimpColorProfileChooserDialog *dialog) {
  dialog->priv = gimp_color_profile_chooser_dialog_get_instance_private(dialog);
}
...
GtkWidget *gimp_color_profile_chooser_dialog_new(const gchar *title,
  ...
  dialog = g_object_new(GIMP_TYPE_COLOR_PROFILE_CHOOSER_DIALOG, "title", title,
                        "action", action, NULL);
  ...
  if (gtk_file_chooser_get_action(GTK_FILE_CHOOSER(dialog)) ==
    ...
    gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("_Cancel"),
                           GTK_RESPONSE_CANCEL, _("_Save"), GTK_RESPONSE_ACCEPT,
                           NULL);
  ...
  gimp_color_profile_chooser_dialog_add_shortcut(
      GIMP_COLOR_PROFILE_CHOOSER_DIALOG(dialog));

  return dialog;
}

/* Add shortcuts for default ICC profile locations */
static gboolean add_shortcut(GimpColorProfileChooserDialog *dialog,
                             const gchar *folder) {
  return (g_file_test(folder, G_FILE_TEST_IS_DIR) &&
          gtk_file_chooser_add_shortcut_folder(GTK_FILE_CHOOSER(dialog), folder,
                                               NULL));
}
...
static void gimp_color_profile_chooser_dialog_add_shortcut(
...
#elif defined(PLATFORM_OSX)
  {
    ...
    if (save) {
      ...
      folder = g_build_filename([library_dir UTF8String], "ColorSync",
                                "Profiles", NULL);
...
#else
  {
    ...
    if (save) {
      gchar *folder =
          g_build_filename(g_get_user_data_dir(), "color", "icc", NULL);
      ...
      if (!folder_set) {
         ...
         * $XDG_DATA_HOME/icc/
         */
        folder = g_build_filename(g_get_user_data_dir(), "icc", NULL);
      ...
      if (!folder_set) {
        g_free(folder);
        folder = g_build_filename(g_get_home_dir(), ".color", "icc", NULL);
...
static void gimp_color_profile_chooser_dialog_update_preview(
  ...
  case G_FILE_TYPE_REGULAR:
    ...
    if (!profile) {
      ...
      gimp_color_profile_view_set_profile(dialog->priv->profile_view, profile);
      g_object_unref(profile);
    }
    break;

  case G_FILE_TYPE_DIRECTORY:
    gimp_color_profile_view_set_error(dialog->priv->profile_view, _("Folder"));
    break;
