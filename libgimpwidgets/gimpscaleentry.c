...
G_DEFINE_TYPE_WITH_PRIVATE(GimpScaleEntry, gimp_scale_entry,
                           GIMP_TYPE_LABEL_SPIN)

#define parent_class gimp_scale_entry_parent_class

static void gimp_scale_entry_class_init(GimpScaleEntryClass *klass) {
...
static void gimp_scale_entry_init(GimpScaleEntry *entry) {
...
static void gimp_scale_entry_constructed(GObject *object) {
  ...
  scale_adjustment =
      gtk_adjustment_new(gtk_adjustment_get_value(spin_adjustment),
  ...
  if (klass->new_range_widget) {
    ...
    g_return_if_fail(scale_adjustment ==
                     gtk_range_get_adjustment(GTK_RANGE(priv->scale)));
  } else {
  ...
  g_signal_connect_swapped(spin_adjustment, "changed",
                           G_CALLBACK(gimp_scale_entry_configure), entry);
...
static void gimp_scale_entry_configure(GimpScaleEntry *entry) {
  ...
  if (priv->logarithmic) {
    ...
    gtk_adjustment_configure(scale_adj, log_value, log_lower, log_upper,
                             log_step_increment, log_page_increment, 0.0);

    binding = g_object_bind_property_full(
        G_OBJECT(spin_adj), "value", G_OBJECT(scale_adj), "value",
        G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE,
        gimp_scale_entry_linear_to_log, gimp_scale_entry_log_to_linear, NULL,
        NULL);
  } else {
    gtk_adjustment_configure(scale_adj, gtk_adjustment_get_value(spin_adj),
                             scale_lower, scale_upper,
                             gtk_adjustment_get_step_increment(spin_adj),
                             gtk_adjustment_get_page_increment(spin_adj), 0.0);
...
GtkWidget *gimp_scale_entry_new(const gchar *text, gdouble value, gdouble lower,
  ...
  entry = g_object_new(GIMP_TYPE_SCALE_ENTRY, "label", text, "value", value,
...
void gimp_scale_entry_set_bounds(GimpScaleEntry *entry, gdouble lower,
  ...
  if (limit_scale) {
    ...
    g_object_set(entry, "lower", lower, "upper", upper, NULL);
  }
}
