...
enum { PROP_0, PROP_CHECK_SIZE, PROP_CHECK_TYPE };
...
#define CHECK_COLOR(priv, row, col)                                            \
  (((((priv)->offset_y + (row)) & size) ^ (((priv)->offset_x + (col)) & size)) \
       ? dark                                                                  \
       : light)
...
static void gimp_preview_area_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_preview_area_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);

static void gimp_preview_area_size_allocate(GtkWidget *widget,
                                            GtkAllocation *allocation);
static gboolean gimp_preview_area_widget_draw(GtkWidget *widget, cairo_t *cr);

static void gimp_preview_area_queue_draw(GimpPreviewArea *area, gint x, gint y,
...
static void gimp_preview_area_class_init(GimpPreviewAreaClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_CHECK_SIZE,
      g_param_spec_enum(
          "check-size", _("Check Size"),
          "The size of the checkerboard pattern indicating transparency",
          GIMP_TYPE_CHECK_SIZE, DEFAULT_CHECK_SIZE, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_CHECK_TYPE,
      g_param_spec_enum(
          "check-type", _("Check Style"),
          "The colors of the checkerboard pattern indicating transparency",
          GIMP_TYPE_CHECK_TYPE, DEFAULT_CHECK_TYPE, GIMP_PARAM_READWRITE));
}
...
static void gimp_preview_area_size_allocate(GtkWidget *widget,
  ...
  width = (priv->max_width > 0 ? MIN(allocation->width, priv->max_width)
                               : allocation->width);
  height = (priv->max_height > 0 ? MIN(allocation->height, priv->max_height)
                                 : allocation->height);
...
static gboolean gimp_preview_area_widget_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (priv->transform) {
    ...
    pixbuf = gdk_pixbuf_new_from_data(buf, GDK_COLORSPACE_RGB, FALSE, 8,
                                      rect.width, rect.height, rowstride,
                                      (GdkPixbufDestroyNotify)g_free, NULL);
  } else {
    pixbuf = gdk_pixbuf_new_from_data(priv->buf, GDK_COLORSPACE_RGB, FALSE, 8,
                                      rect.width, rect.height, priv->rowstride,
                                      NULL, NULL);
  }
 ...
 * Since GIMP 2.2
 **/
void gimp_preview_area_draw(GimpPreviewArea *area, gint x, gint y, gint width,
                            gint height, GimpImageType type, const guchar *buf,
                            gint rowstride) {
  GimpPreviewAreaPrivate *priv;
  const guchar *src;
  guchar *dest;
    ...
    for (row = y; row < y + height; row++) {
      ...
      for (col = x; col < x + width; col++, s += 2, d += 3) {
        ...
        default: {
          ...
          d[0] = d[1] = d[2] = ((check << 8) + (s[0] - check) * alpha) >> 8;
        } break;
        }
      }
 ...
 * Since GIMP 2.2
 **/
void gimp_preview_area_blend(GimpPreviewArea *area, gint x, gint y, gint width,
                             gint height, GimpImageType type,
                             const guchar *buf1, gint rowstride1,
                             const guchar *buf2, gint rowstride2,
                             guchar opacity) {
  GimpPreviewAreaPrivate *priv;
  const guchar *src1;
  const guchar *src2;
  ...
  switch (opacity) {
  case 0:
    gimp_preview_area_draw(area, x, y, width, height, type, buf1, rowstride1);
    return;

  case 255:
    gimp_preview_area_draw(area, x, y, width, height, type, buf2, rowstride2);
    return;
 ...
 * Since GIMP 2.2
 **/
void gimp_preview_area_mask(GimpPreviewArea *area, gint x, gint y, gint width,
                            gint height, GimpImageType type, const guchar *buf1,
                            gint rowstride1, const guchar *buf2,
                            gint rowstride2, const guchar *mask,
                            gint rowstride_mask) {
  GimpPreviewAreaPrivate *priv;
  const guchar *src1;
  const guchar *src2;
      ...
      for (col = x; col < x + width; col++, s1 += 2, s2 += 2, m++, d += 3) {
        ...
        case 0:
          ...
          default: {
            ...
            d[0] = d[1] = d[2] = ((check << 8) + (s1[0] - check) * alpha) >> 8;
          } break;
          }
          break;
        ...
        case 255:
          ...
          default: {
            ...
            d[0] = d[1] = d[2] = ((check << 8) + (s2[0] - check) * alpha) >> 8;
          } break;
          }
          break;
 ...
 * Since GIMP 2.2
 **/
void gimp_preview_area_fill(GimpPreviewArea *area, gint x, gint y, gint width,
                            gint height, guchar red, guchar green,
                            guchar blue) {
  GimpPreviewAreaPrivate *priv;
  guchar *dest;
  guchar *d;
  ...
  if (config != priv->config) {
    if (priv->config) {
      g_signal_handlers_disconnect_by_func(
          priv->config, gimp_preview_area_destroy_transform, area);
 ...
 */
void gimp_preview_area_get_size(GimpPreviewArea *area, gint *width,
                                gint *height) {
  GimpPreviewAreaPrivate *priv;
 ...
 * Since: 2.2
 **/
void gimp_preview_area_set_max_size(GimpPreviewArea *area, gint width,
                                    gint height) {
  GimpPreviewAreaPrivate *priv;
...
static void gimp_preview_area_menu_toggled(GtkWidget *item,
                                           GimpPreviewArea *area) {
  gboolean active = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(item));

  if (active) {
    const gchar *name =
        g_object_get_data(G_OBJECT(item), "gimp-preview-area-prop-name");
    if (name) {
      gint value = GPOINTER_TO_INT(
          g_object_get_data(G_OBJECT(item), "gimp-preview-area-prop-value"));
      g_object_set(area, name, value, NULL);
    }
  }
}

static GtkWidget *gimp_preview_area_menu_new(GimpPreviewArea *area,
  ...
  g_object_get(area, property, &value, NULL);
  ...
  for (enum_value = enum_class->values; enum_value->value_name; enum_value++) {
    ...
    g_object_set_data(G_OBJECT(item), "gimp-preview-area-prop-name",
                      (gpointer)property);

    g_object_set_data(G_OBJECT(item), "gimp-preview-area-prop-value",
    ...
    g_signal_connect(item, "toggled",
                     G_CALLBACK(gimp_preview_area_menu_toggled), area);
...
void gimp_preview_area_menu_popup(GimpPreviewArea *area,
  ...
  menu = gtk_menu_new();
  gtk_menu_set_screen(GTK_MENU(menu), gtk_widget_get_screen(GTK_WIDGET(area)));
