...
gimp_color_profile_store_history_insert(GimpColorProfileStore *store,
                                        GtkTreeIter *iter, GFile *file,
                                        const gchar *label, gint index);
...
gimp_color_profile_store_class_init(GimpColorProfileStoreClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_HISTORY,
      g_param_spec_object(
          "history", "History",
          "Filen of the color history used to populate the profile store",
          G_TYPE_FILE, G_PARAM_CONSTRUCT_ONLY | GIMP_PARAM_READWRITE));
}

static void gimp_color_profile_store_init(GimpColorProfileStore *store) {
  GType types[] = {
  ...
  gtk_list_store_set_column_types(GTK_LIST_STORE(store), G_N_ELEMENTS(types),
                                  types);
}
...
static void gimp_color_profile_store_constructed(GObject *object) {
  ...
  gtk_list_store_set(
      GTK_LIST_STORE(store), &iter, GIMP_COLOR_PROFILE_STORE_ITEM_TYPE,
      GIMP_COLOR_PROFILE_STORE_ITEM_DIALOG, GIMP_COLOR_PROFILE_STORE_LABEL,
...
GtkListStore *gimp_color_profile_store_new(GFile *history) {
  ...
  return g_object_new(GIMP_TYPE_COLOR_PROFILE_STORE, "history", history, NULL);
}
...
void gimp_color_profile_store_add_file(GimpColorProfileStore *store,
  ...
  gtk_list_store_set(
      GTK_LIST_STORE(store), &iter, GIMP_COLOR_PROFILE_STORE_ITEM_TYPE,
      GIMP_COLOR_PROFILE_STORE_ITEM_FILE, GIMP_COLOR_PROFILE_STORE_FILE, file,
      GIMP_COLOR_PROFILE_STORE_LABEL, label, GIMP_COLOR_PROFILE_STORE_INDEX, -1,
      -1);
}
 ...
 * Since: 2.4
 **/
gboolean _gimp_color_profile_store_history_add(GimpColorProfileStore *store,
                                               GFile *file, const gchar *label,
                                               GtkTreeIter *iter) {
  GtkTreeModel *model;
  gboolean iter_valid;
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, iter); iter_valid;
    ...
    gtk_tree_model_get(model, iter, GIMP_COLOR_PROFILE_STORE_ITEM_TYPE, &type,
                       GIMP_COLOR_PROFILE_STORE_INDEX, &index, -1);
    ...
    gtk_tree_model_get(model, iter, GIMP_COLOR_PROFILE_STORE_FILE, &this, -1);

    if ((this && file && g_file_equal(this, file)) || (!this && !file)) {
      /*  update the label  */
      if (label && *label)
        gtk_list_store_set(GTK_LIST_STORE(store), iter,
                           GIMP_COLOR_PROFILE_STORE_LABEL, label, -1);
  ...
  if (label && *label) {
    iter_valid = gimp_color_profile_store_history_insert(store, iter, file,
                                                         label, ++max);
  } else {
 ...
 * Since: 2.4
 **/
void _gimp_color_profile_store_history_reorder(GimpColorProfileStore *store,
                                               GtkTreeIter *iter) {
  GtkTreeModel *model;
  gint index;
  gboolean iter_valid;
  ...
  gtk_tree_model_get(model, iter, GIMP_COLOR_PROFILE_STORE_INDEX, &index, -1);
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, iter); iter_valid;
    ...
    gtk_tree_model_get(model, iter, GIMP_COLOR_PROFILE_STORE_ITEM_TYPE, &type,
                       GIMP_COLOR_PROFILE_STORE_INDEX, &this_index, -1);
...
gimp_color_profile_store_history_insert(GimpColorProfileStore *store,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &sibling); iter_valid;
       iter_valid = gtk_tree_model_iter_next(model, &sibling)) {
    gint type;
    gint this_index;

    gtk_tree_model_get(model, &sibling, GIMP_COLOR_PROFILE_STORE_ITEM_TYPE,
                       &type, GIMP_COLOR_PROFILE_STORE_INDEX, &this_index, -1);
    ...
    if (type == GIMP_COLOR_PROFILE_STORE_ITEM_FILE && this_index > -1) {
      ...
      gtk_tree_model_get(model, &sibling, GIMP_COLOR_PROFILE_STORE_LABEL,
                         &this_label, -1);
  ...
  if (iter_valid)
    gtk_list_store_set(
        GTK_LIST_STORE(store), iter, GIMP_COLOR_PROFILE_STORE_ITEM_TYPE,
        GIMP_COLOR_PROFILE_STORE_ITEM_FILE, GIMP_COLOR_PROFILE_STORE_FILE, file,
        GIMP_COLOR_PROFILE_STORE_LABEL, label, GIMP_COLOR_PROFILE_STORE_INDEX,
        index, -1);
...
gimp_color_profile_store_create_separator(GimpColorProfileStore *store,
  ...
  if (top) {
    ...
    for (iter_valid = gtk_tree_model_get_iter_first(model, &sibling);
         iter_valid; iter_valid = gtk_tree_model_iter_next(model, &sibling)) {
      gint type;
  ...
  gtk_list_store_set(GTK_LIST_STORE(store), iter,
                     ...
                     top ? GIMP_COLOR_PROFILE_STORE_ITEM_SEPARATOR_TOP
                         : GIMP_COLOR_PROFILE_STORE_ITEM_SEPARATOR_BOTTOM,
                     GIMP_COLOR_PROFILE_STORE_INDEX, -1, -1);
}
...
static void gimp_color_profile_store_get_separator(GimpColorProfileStore *store,
  ...
  needle = (top ? GIMP_COLOR_PROFILE_STORE_ITEM_SEPARATOR_TOP
                : GIMP_COLOR_PROFILE_STORE_ITEM_SEPARATOR_BOTTOM);
  ...
  if (gimp_scanner_parse_string(scanner, &label) &&
    ...
    if (file) {
      if (g_file_query_file_type(file, 0, NULL) == G_FILE_TYPE_REGULAR) {
        gimp_color_profile_store_history_insert(store, &iter, file, label,
                                                index);
      }
...
static gboolean gimp_color_profile_store_save(GimpColorProfileStore *store,
  ...
  GtkTreeModel *model;
  GtkTreeIter iter;
  gchar *labels[HISTORY_SIZE] = {
      NULL,
  };
  GFile *files[HISTORY_SIZE] = {
      NULL,
  };
  gboolean iter_valid;
  gint i;

  writer = gimp_config_writer_new_from_file(
      file, TRUE, "GIMP color profile history", error);
  if (!writer)
    return FALSE;
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    if (type == GIMP_COLOR_PROFILE_STORE_ITEM_FILE && index >= 0 &&
        index < HISTORY_SIZE) {
      if (labels[index] || files[index])
        g_warning("%s: double index %d", G_STRFUNC, index);

      gtk_tree_model_get(model, &iter, GIMP_COLOR_PROFILE_STORE_LABEL,
                         &labels[index], GIMP_COLOR_PROFILE_STORE_FILE,
                         &files[index], -1);
    }
  }
  ...
  return gimp_config_writer_finish(writer, "end of color profile history",
                                   error);
}
