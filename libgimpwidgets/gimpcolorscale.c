...
static void gimp_color_scale_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);
static void gimp_color_scale_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);

static void gimp_color_scale_size_allocate(GtkWidget *widget,
                                           GtkAllocation *allocation);
static gboolean gimp_color_scale_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_color_scale_class_init(GimpColorScaleClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("channel", "Channel",
                        "The channel which is edited by the color scale",
...
static void gimp_color_scale_init(GimpColorScale *scale) {
  ...
  gtk_style_context_add_provider(
      gtk_widget_get_style_context(GTK_WIDGET(scale)), GTK_STYLE_PROVIDER(css),
...
static void gimp_color_scale_size_allocate(GtkWidget *widget,
  ...
  if (range_rect.width != priv->width || range_rect.height != priv->height) {
...
static gboolean gimp_color_scale_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  case GTK_ORIENTATION_HORIZONTAL:
    cairo_set_source_surface(cr, buffer, range_rect.x, range_rect.y);
    break;
  ...
  case GTK_ORIENTATION_HORIZONTAL:
    ...
    cairo_line_to(cr, area.x + area.width / 2 + 0.5, area.y + slider_size);
    break;

  case GTK_ORIENTATION_VERTICAL:
    cairo_move_to(cr, area.x, area.y);
    cairo_line_to(cr, area.x, area.y + area.height);
    cairo_line_to(cr, area.x + slider_size, area.y + area.height / 2 + 0.5);
    break;
  }
  ...
  case GTK_ORIENTATION_HORIZONTAL:
    ...
    cairo_line_to(cr, area.x + area.width / 2 + 0.5,
  ...
  case GTK_ORIENTATION_VERTICAL:
    ...
    cairo_line_to(cr, area.x + area.width, area.y + area.height);
    cairo_line_to(cr, area.x + area.width - slider_size,
...
GtkWidget *gimp_color_scale_new(GtkOrientation orientation,
                                GimpColorSelectorChannel channel) {
  GimpColorScale *scale = g_object_new(GIMP_TYPE_COLOR_SCALE, "orientation",
                                       orientation, "channel", channel, NULL);
  ...
  if (config != priv->config) {
    if (priv->config) {
      g_signal_handlers_disconnect_by_func(
          priv->config, gimp_color_scale_notify_config, scale);
    ...
    if (priv->config) {
      g_signal_connect(priv->config, "notify",
                       G_CALLBACK(gimp_color_scale_notify_config), scale);
...
static void gimp_color_scale_render_alpha(GimpColorScale *scale) {
  ...
  case GTK_ORIENTATION_HORIZONTAL: {
    ...
    dark = (priv->height > GIMP_CHECK_SIZE_SM
                ? buf + GIMP_CHECK_SIZE_SM * priv->rowstride
                : light);
    ...
    for (x = 0, d = light, l = dark; x < priv->width; x++) {
      ...
      GIMP_CAIRO_RGB24_SET_PIXEL(
          l, (GIMP_CHECK_LIGHT + (rgb.r - GIMP_CHECK_LIGHT) * a) * 255.999,
          (GIMP_CHECK_LIGHT + (rgb.g - GIMP_CHECK_LIGHT) * a) * 255.999,
          (GIMP_CHECK_LIGHT + (rgb.b - GIMP_CHECK_LIGHT) * a) * 255.999);
      l += 4;

      GIMP_CAIRO_RGB24_SET_PIXEL(
          d, (GIMP_CHECK_DARK + (rgb.r - GIMP_CHECK_DARK) * a) * 255.999,
          (GIMP_CHECK_DARK + (rgb.g - GIMP_CHECK_DARK) * a) * 255.999,
          (GIMP_CHECK_DARK + (rgb.b - GIMP_CHECK_DARK) * a) * 255.999);
      d += 4;
    }
  ...
  case GTK_ORIENTATION_VERTICAL: {
    ...
    for (y = 0, d = buf; y < priv->height; y++, d += priv->rowstride) {
      ...
      GIMP_CAIRO_RGB24_SET_PIXEL(
          light, (GIMP_CHECK_LIGHT + (rgb.r - GIMP_CHECK_LIGHT) * a) * 255.999,
          (GIMP_CHECK_LIGHT + (rgb.g - GIMP_CHECK_LIGHT) * a) * 255.999,
          (GIMP_CHECK_LIGHT + (rgb.b - GIMP_CHECK_LIGHT) * a) * 255.999);

      GIMP_CAIRO_RGB24_SET_PIXEL(
          dark, (GIMP_CHECK_DARK + (rgb.r - GIMP_CHECK_DARK) * a) * 255.999,
          (GIMP_CHECK_DARK + (rgb.g - GIMP_CHECK_DARK) * a) * 255.999,
          (GIMP_CHECK_DARK + (rgb.b - GIMP_CHECK_DARK) * a) * 255.999);
...
static void gimp_color_scale_create_transform(GimpColorScale *scale) {
  ...
  if (priv->config) {
    ...
    priv->transform = gimp_widget_get_color_transform(
        GTK_WIDGET(scale), priv->config, profile, format, format);
  }
}
...
static void gimp_color_scale_notify_config(GimpColorConfig *config,
  ...
  gimp_rgb_get_uchar(&color, priv->oog_color, priv->oog_color + 1,
                     priv->oog_color + 2);
  priv->needs_render = TRUE;
}
