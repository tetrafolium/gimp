...
enum { PATH_CHANGED, WRITABLE_CHANGED, LAST_SIGNAL };

enum { COLUMN_UTF8, COLUMN_DIRECTORY, COLUMN_WRITABLE, NUM_COLUMNS };
...
static void gimp_path_editor_class_init(GimpPathEditorClass *klass) {
  ...
  gimp_path_editor_signals[PATH_CHANGED] =
      g_signal_new("path-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpPathEditorClass, path_changed), NULL,
   ...
   **/
  gimp_path_editor_signals[WRITABLE_CHANGED] = g_signal_new(
      "writable-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpPathEditorClass, writable_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
...
static void gimp_path_editor_init(GimpPathEditor *editor) {
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(gimp_path_editor_new_clicked),
                   editor);

  gimp_help_set_help_data(editor->new_button, _("Add a new folder"), NULL);
  ...
  image = gtk_image_new_from_icon_name(GIMP_ICON_GO_UP, GTK_ICON_SIZE_BUTTON);
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_widget_show(image);

  g_signal_connect(button, "clicked", G_CALLBACK(gimp_path_editor_move_clicked),
                   editor);

  gimp_help_set_help_data(editor->up_button, _("Move the selected folder up"),
                          NULL);
  ...
  image = gtk_image_new_from_icon_name(GIMP_ICON_GO_DOWN, GTK_ICON_SIZE_BUTTON);
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_widget_show(image);
  ...
  gimp_help_set_help_data(editor->down_button,
                          _("Move the selected folder down"), NULL);
  ...
  image =
      gtk_image_new_from_icon_name(GIMP_ICON_EDIT_DELETE, GTK_ICON_SIZE_BUTTON);
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_widget_show(image);

  g_signal_connect(button, "clicked",
                   G_CALLBACK(gimp_path_editor_delete_clicked), editor);

  gimp_help_set_help_data(editor->delete_button,
                          _("Remove the selected folder from the list"), NULL);
  ...
  editor->dir_list = gtk_list_store_new(NUM_COLUMNS, G_TYPE_STRING,
  ...
  g_signal_connect(renderer, "toggled",
                   G_CALLBACK(gimp_path_editor_writable_toggled), editor);
  ...
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(tv), -1, _("Folder"), gtk_cell_renderer_text_new(), "text",
      COLUMN_UTF8, NULL);
  ...
  g_signal_connect(editor->sel, "changed",
                   G_CALLBACK(gimp_path_editor_selection_changed), editor);
}
...
GtkWidget *gimp_path_editor_new(const gchar *title, const gchar *path) {
  ...
  gtk_box_pack_start(GTK_BOX(editor->upper_hbox), editor->file_entry, TRUE,
                     TRUE, 0);
  gtk_widget_show(editor->file_entry);

  g_signal_connect(editor->file_entry, "filename-changed",
                   G_CALLBACK(gimp_path_editor_file_entry_changed), editor);
...
gchar *gimp_path_editor_get_path(GimpPathEditor *editor) {
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    gtk_tree_model_get(model, &iter, COLUMN_DIRECTORY, &dir, -1);
...
void gimp_path_editor_set_path(GimpPathEditor *editor, const gchar *path) {
  ...
  for (list = path_list; list; list = g_list_next(list)) {
    ...
    gtk_list_store_set(editor->dir_list, &iter, COLUMN_UTF8, utf8,
                       COLUMN_DIRECTORY, directory, COLUMN_WRITABLE, FALSE, -1);
...
gchar *gimp_path_editor_get_writable_path(GimpPathEditor *editor) {
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    gtk_tree_model_get(model, &iter, COLUMN_DIRECTORY, &dir, COLUMN_WRITABLE,
                       &dir_writable, -1);
...
void gimp_path_editor_set_writable_path(GimpPathEditor *editor,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    if (dir_writable != new_writable) {
      gtk_list_store_set(editor->dir_list, &iter, COLUMN_WRITABLE, new_writable,
                         -1);
...
void gimp_path_editor_set_dir_writable(GimpPathEditor *editor,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    if (!strcmp(dir, directory) && dir_writable != writable) {
      gtk_list_store_set(editor->dir_list, &iter, COLUMN_WRITABLE,
                         writable ? TRUE : FALSE, -1);
...
static void gimp_path_editor_new_clicked(GtkWidget *widget,
  ...
  if (editor->sel_path) {
    g_signal_handlers_block_by_func(editor->sel,
                                    gimp_path_editor_selection_changed, editor);

    gtk_tree_selection_unselect_path(editor->sel, editor->sel_path);

    g_signal_handlers_unblock_by_func(
        editor->sel, gimp_path_editor_selection_changed, editor);
  ...
  gtk_editable_set_position(GTK_EDITABLE(gimp_file_entry_get_entry(
                                GIMP_FILE_ENTRY(editor->file_entry))),
                            -1);
...
static void gimp_path_editor_move_clicked(GtkWidget *widget,
  ...
  gtk_tree_model_get(model, &iter1, COLUMN_UTF8, &utf81, COLUMN_DIRECTORY,
                     &dir1, COLUMN_WRITABLE, &writable1, -1);
  gtk_tree_model_get(model, &iter2, COLUMN_UTF8, &utf82, COLUMN_DIRECTORY,
                     &dir2, COLUMN_WRITABLE, &writable2, -1);

  gtk_list_store_set(editor->dir_list, &iter1, COLUMN_UTF8, utf82,
                     COLUMN_DIRECTORY, dir2, COLUMN_WRITABLE, writable2, -1);
  gtk_list_store_set(editor->dir_list, &iter2, COLUMN_UTF8, utf81,
                     COLUMN_DIRECTORY, dir1, COLUMN_WRITABLE, writable1, -1);
...
static void gimp_path_editor_delete_clicked(GtkWidget *widget,
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(editor->dir_list), &iter, COLUMN_WRITABLE,
                     &dir_writable, -1);
  ...
  if (editor->num_items == 0) {
    ...
    g_signal_handlers_block_by_func(
        editor->file_entry, gimp_path_editor_file_entry_changed, editor);
...
static void gimp_path_editor_file_entry_changed(GtkWidget *widget,
  ...
  if (editor->sel_path == NULL) {
    gtk_list_store_append(editor->dir_list, &iter);
    gtk_list_store_set(editor->dir_list, &iter, COLUMN_UTF8, utf8,
                       COLUMN_DIRECTORY, dir, COLUMN_WRITABLE, FALSE, -1);
    editor->num_items++;
...
static void gimp_path_editor_selection_changed(GtkTreeSelection *sel,
  ...
  if (gtk_tree_selection_get_selected(sel, NULL, &iter)) {
    ...
    g_signal_handlers_block_by_func(sel, gimp_path_editor_selection_changed,
                                    editor);

    gtk_tree_selection_select_path(editor->sel, editor->sel_path);

    g_signal_handlers_unblock_by_func(sel, gimp_path_editor_selection_changed,
                                      editor);
  }
}
...
static void gimp_path_editor_writable_toggled(GtkCellRendererToggle *toggle,
  ...
  if (gtk_tree_model_get_iter(GTK_TREE_MODEL(editor->dir_list), &iter, path)) {
    ...
    gtk_list_store_set(editor->dir_list, &iter, COLUMN_WRITABLE, !dir_writable,
                       -1);
