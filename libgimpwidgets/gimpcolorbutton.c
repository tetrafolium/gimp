...
static void gimp_color_button_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);
static void gimp_color_button_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
...
static const GtkActionEntry actions[] = {
    {"color-button-popup", NULL, "Color Button Menu", NULL, NULL, NULL},

    {GIMP_COLOR_BUTTON_COLOR_FG, NULL, N_("_Foreground Color"), NULL, NULL,
     G_CALLBACK(gimp_color_button_use_color)},
    {GIMP_COLOR_BUTTON_COLOR_BG, NULL, N_("_Background Color"), NULL, NULL,
     G_CALLBACK(gimp_color_button_use_color)},
    {GIMP_COLOR_BUTTON_COLOR_BLACK, NULL, N_("Blac_k"), NULL, NULL,
     G_CALLBACK(gimp_color_button_use_color)},
    {GIMP_COLOR_BUTTON_COLOR_WHITE, NULL, N_("_White"), NULL, NULL,
     G_CALLBACK(gimp_color_button_use_color)}};
...
static void gimp_color_button_class_init(GimpColorButtonClass *klass) {
  ...
  gimp_color_button_signals[COLOR_CHANGED] = g_signal_new(
      "color-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpColorButtonClass, color_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("title", "Title",
                          "The title to be used for the color selection dialog",
                          NULL, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  /**
  ...
  g_object_class_install_property(
      ...
      gimp_param_spec_rgb(
          "color", "Color", "The color displayed in the button's color area",
          TRUE, &color, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  /**
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("type", "Type", "The type of the button's color area",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("continuous-update", "Contiguous Update",
                           "The update policy of the color button", FALSE,
   ...
   */
  g_object_class_install_property(
      object_class, PROP_AREA_WIDTH,
      g_param_spec_int("area-width", "Area Width",
                       "The minimum width of the button's GimpColorArea", 1,
                       G_MAXINT, 16, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT));
  /**
   ...
   */
  g_object_class_install_property(
      object_class, PROP_AREA_HEIGHT,
      g_param_spec_int("area-height", "Area Height",
                       "The minimum height of the button's GimpColorArea", 1,
                       G_MAXINT, 16, G_PARAM_WRITABLE | G_PARAM_CONSTRUCT));
  /**
   ...
   */
  g_object_class_install_property(
      object_class, PROP_COLOR_CONFIG,
      g_param_spec_object("color-config", "Color Config",
                          "The color config object used",
                          GIMP_TYPE_COLOR_CONFIG, G_PARAM_READWRITE));
}
...
static void gimp_color_button_init(GimpColorButton *button) {
  ...
  priv->color_area =
      g_object_new(GIMP_TYPE_COLOR_AREA, "drag-mask", GDK_BUTTON1_MASK, NULL);

  g_signal_connect(priv->color_area, "color-changed",
                   G_CALLBACK(gimp_color_button_area_changed), button);
...
static void gimp_color_button_constructed(GObject *object) {
  ...
  for (i = 0; i < G_N_ELEMENTS(actions); i++) {
    ...
    action = g_object_new(klass->get_action_type(button), "name",
                          actions[i].name, "label", label, "tooltip", tooltip,
                          "icon-name", actions[i].stock_id, NULL);

    if (actions[i].callback)
      g_signal_connect(action, "activate", actions[i].callback, button);
...
static void gimp_color_button_set_property(GObject *object, guint property_id,
  ...
  case PROP_AREA_WIDTH:
    ...
    gtk_widget_set_size_request(priv->color_area, g_value_get_int(value),
                                other);
    break;

  case PROP_AREA_HEIGHT:
    gtk_widget_get_size_request(priv->color_area, &other, NULL);
    gtk_widget_set_size_request(priv->color_area, other,
...
static gboolean gimp_color_button_button_press(GtkWidget *widget,
  ...
  if (gdk_event_triggers_context_menu((GdkEvent *)bevent)) {
    GtkWidget *menu =
        gtk_ui_manager_get_widget(priv->ui_manager, "/color-button-popup");
...
gimp_color_button_state_flags_changed(GtkWidget *widget,
  ...
  if (GTK_WIDGET_CLASS(parent_class)->state_flags_changed)
    GTK_WIDGET_CLASS(parent_class)->state_flags_changed(widget, previous_state);
}
...
static void gimp_color_button_clicked(GtkButton *button) {
  ...
  if (!priv->dialog) {
    ...
    gimp_color_selection_set_show_alpha(
        GIMP_COLOR_SELECTION(priv->selection),
    ...
    g_signal_connect(priv->selection, "color-changed",
                     G_CALLBACK(gimp_color_button_selection_changed), button);
  }

  gimp_color_button_get_color(color_button, &color);

  g_signal_handlers_block_by_func(priv->selection,
                                  gimp_color_button_selection_changed, button);
  ...
  g_signal_handlers_unblock_by_func(
      priv->selection, gimp_color_button_selection_changed, button);
 ...
 **/
GtkWidget *gimp_color_button_new(const gchar *title, gint width, gint height,
                                 const GimpRGB *color, GimpColorAreaType type) {
  g_return_val_if_fail(color != NULL, NULL);
  g_return_val_if_fail(width > 0, NULL);
  g_return_val_if_fail(height > 0, NULL);

  return g_object_new(GIMP_TYPE_COLOR_BUTTON, "title", title, "type", type,
                      "color", color, "area-width", width, "area-height",
                      height, NULL);
}
 ...
 * Since: 2.10
 **/
void gimp_color_button_set_title(GimpColorButton *button, const gchar *title) {
    ...
    if (priv->selection) {
      ...
      if (priv->continuous_update) {
        ...
        gimp_color_selection_get_old_color(
            GIMP_COLOR_SELECTION(priv->selection), &color);
        gimp_color_button_set_color(button, &color);
      }
    }
...
static void gimp_color_button_selection_changed(GtkWidget *selection,
  ...
  if (priv->continuous_update) {
    ...
    g_signal_handlers_block_by_func(priv->color_area,
                                    gimp_color_button_area_changed, button);
