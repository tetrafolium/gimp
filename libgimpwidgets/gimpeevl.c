...
static void gimp_eevl_init(GimpEevl *eva, const gchar *string,
 ...
 **/
gboolean gimp_eevl_evaluate(const gchar *string, const GimpEevlOptions *options,
                            GimpEevlQuantity *result, const gchar **error_pos,
                            GError **error) {
  GimpEevl eva;
  ...
  gimp_eevl_init(&eva, string, options);
  ...
  } else /* catch.. */
  {
    ...
    g_set_error_literal(error, GIMP_WIDGETS_ERROR, GIMP_WIDGETS_PARSE_ERROR,
                        eva.error_message);
...
static GimpEevlQuantity gimp_eevl_complete(GimpEevl *eva) {
  ...
  eva->options.unit_resolver_proc(NULL, &default_unit_factor,
                                  &default_unit_offset, eva->options.data);
...
static GimpEevlQuantity gimp_eevl_expression(GimpEevl *eva) {
  ...
  for (subtract = FALSE; gimp_eevl_accept(eva, '+', NULL) ||
...
static GimpEevlQuantity gimp_eevl_term(GimpEevl *eva) {
  ...
  for (division = FALSE; gimp_eevl_accept(eva, '*', NULL) ||
...
static GimpEevlQuantity gimp_eevl_ratio(GimpEevl *eva) {
  ...
  while (gimp_eevl_accept(eva, ':', NULL)) {
    ...
    evaluated_signed_factors.value *=
        eva->options.ratio_quantity.value / new_signed_factor.value;
    evaluated_signed_factors.dimension +=
        eva->options.ratio_quantity.dimension - new_signed_factor.dimension;
  }
...
static GimpEevlQuantity gimp_eevl_factor(GimpEevl *eva) {
  ...
  if (gimp_eevl_accept(eva, '^', NULL)) {
    ...
    evaluated_factor.value =
        pow(evaluated_factor.value, evaluated_exponent.value);
...
static GimpEevlQuantity gimp_eevl_quantity(GimpEevl *eva) {
  ...
  if (gimp_eevl_accept(eva, GIMP_EEVL_TOKEN_NUM, &consumed_token)) {
  ...
  if (eva->current_token.type == GIMP_EEVL_TOKEN_IDENTIFIER) {
    ...
    gimp_eevl_accept(eva, GIMP_EEVL_TOKEN_ANY, &consumed_token);
    ...
    if (eva->options.unit_resolver_proc(identifier, &factor, &offset,
                                        eva->options.data)) {
      if (gimp_eevl_accept(eva, '^', NULL)) {
        GimpEevlQuantity evaluated_exponent;
        ...
        if (offset != 0.0) {
          gimp_eevl_error(eva, "Invalid unit exponent");
        }
...
static void gimp_eevl_lex(GimpEevl *eva) {
  ...
  if (!s || s[0] == '\0') {
    ...
    if (endptr && endptr != s) {
      ...
      gimp_eevl_lex_accept_count(eva, eva->current_token.value.size,
...
static gboolean gimp_eevl_unit_identifier_start(gunichar c) {
  return (g_unichar_isalpha(c) || c == (gunichar)'%' || c == (gunichar)'\'');
}

static gboolean gimp_eevl_unit_identifier_continue(gunichar c) {
  return (gimp_eevl_unit_identifier_start(c) || g_unichar_isdigit(c));
}
 ...
 * terminator).
 **/
static gint gimp_eevl_unit_identifier_size(const gchar *string,
                                           gint start_offset) {
