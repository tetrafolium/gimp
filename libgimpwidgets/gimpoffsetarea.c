...
static gboolean gimp_offset_area_event(GtkWidget *widget, GdkEvent *event);
static gboolean gimp_offset_area_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_offset_area_class_init(GimpOffsetAreaClass *klass) {
  ...
  gimp_offset_area_signals[OFFSETS_CHANGED] = g_signal_new(
      "offsets-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpOffsetAreaClass, offsets_changed), NULL, NULL,
      _gimp_widgets_marshal_VOID__INT_INT, G_TYPE_NONE, 2, G_TYPE_INT,
      G_TYPE_INT);
...
private
  ->orig_width = private->width = orig_width;
private
  ->orig_height = private->height = orig_height;
 ...
 * Since: 2.2
 **/
void gimp_offset_area_set_pixbuf(GimpOffsetArea *area, GdkPixbuf *pixbuf) {
  g_return_if_fail(GIMP_IS_OFFSET_AREA(area));
  g_return_if_fail(GDK_IS_PIXBUF(pixbuf));

  g_object_set_data_full(G_OBJECT(area), "pixbuf", gdk_pixbuf_copy(pixbuf),
 ...
 * signal is emitted.
 **/
void gimp_offset_area_set_size(GimpOffsetArea *area, gint width, gint height) {
  GimpOffsetAreaPrivate *private;
...
private
  ...
  private
    ...
    if (private->orig_width <= private->width)
      offset_x =
          CLAMP(private->offset_x, 0, private->width - private->orig_width);
    else
      offset_x =
          CLAMP(private->offset_x, private->width - private->orig_width, 0);

    if (private->orig_height <= private->height)
      offset_y =
          CLAMP(private->offset_y, 0, private->height - private->orig_height);
    else
      offset_y =
          CLAMP(private->offset_y, private->height - private->orig_height, 0);
    ...
    private
      ...
      g_signal_emit(area, gimp_offset_area_signals[OFFSETS_CHANGED], 0,
                    offset_x, offset_y);
    }
 ...
 **/
void gimp_offset_area_set_offsets(GimpOffsetArea *area, gint offset_x,
                                  gint offset_y) {
  GimpOffsetAreaPrivate *private;
...
private
  ...
  if (private->offset_x != offset_x || private->offset_y != offset_y) {
    if (private->orig_width <= private->width)
    private
    ->offset_x = CLAMP(offset_x, 0, private->width - private->orig_width);
    else private->offset_x =
        CLAMP(offset_x, private->width - private->orig_width, 0);

    if (private->orig_height <= private->height)
    private
    ->offset_y = CLAMP(offset_y, 0, private->height - private->orig_height);
    else private->offset_y =
        CLAMP(offset_y, private->height - private->orig_height, 0);
...
private
  ->display_ratio_x = ((gdouble)allocation->width /
                       ((private->orig_width <= private->width)
                            ? private->width
                            : private->orig_width * 2 - private->width));

private
  ->display_ratio_y = ((gdouble)allocation->height /
  ...
  if (pixbuf) {
    ...
    if (copy && (pixbuf_width != gdk_pixbuf_get_width(copy) ||
    ...
    if (!copy) {
      ...
      g_object_set_data_full(G_OBJECT(area), "pixbuf-copy", copy,
                             (GDestroyNotify)g_object_unref);
    }
  }
}
...
static gboolean gimp_offset_area_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (private->orig_width > private->width ||
    ...
    cairo_rectangle(cr, x + line_width / 2.0, y + line_width / 2.0,
                    MAX(w - line_width, 1), MAX(h - line_width, 1));
