...
static void set_param_spec(GObject *object, GtkWidget *widget,
                           GParamSpec *param_spec);
static void set_radio_spec(GObject *object, GParamSpec *param_spec);
static GParamSpec *get_param_spec(GObject *object);

static GParamSpec *find_param_spec(GObject *object, const gchar *property_name,
                                   const gchar *strloc);
static GParamSpec *check_param_spec(GObject *object, const gchar *property_name,
                                    GType type, const gchar *strloc);
static GParamSpec *check_param_spec_w(GObject *object,
                                      const gchar *property_name, GType type,
                                      const gchar *strloc);

static gboolean get_numeric_values(GObject *object, GParamSpec *param_spec,
                                   gdouble *value, gdouble *lower,
                                   gdouble *upper, const gchar *strloc);

static void connect_notify(GObject *config, const gchar *property_name,
...
GtkWidget *gimp_prop_check_button_new(GObject *config,
  ...
  param_spec = check_param_spec_w(config, property_name, G_TYPE_PARAM_BOOLEAN,
                                  G_STRFUNC);
  if (!param_spec)
    return NULL;
...
GtkWidget *gimp_prop_enum_check_button_new(GObject *config,
                                           const gchar *property_name,
                                           const gchar *label, gint false_value,
                                           gint true_value) {
  GParamSpec *param_spec;
  GtkWidget *button;
  gint value;
  ...
  param_spec =
      check_param_spec_w(config, property_name, G_TYPE_PARAM_ENUM, G_STRFUNC);
  if (!param_spec)
    return NULL;
  ...
  g_signal_connect(button, "toggled",
                   G_CALLBACK(gimp_prop_enum_check_button_callback), config);

  connect_notify(config, property_name,
                 G_CALLBACK(gimp_prop_enum_check_button_notify), button);
...
static void gimp_prop_enum_check_button_callback(GtkWidget *widget,
  ...
  false_value =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(widget), "false-value"));
  true_value =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(widget), "true-value"));

  value =
      (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget)) ? true_value
                                                               : false_value);
...
static void gimp_prop_enum_check_button_notify(GObject *config,
  ...
  false_value =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "false-value"));
  true_value =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "true-value"));
  ...
  gtk_toggle_button_set_inconsistent(GTK_TOGGLE_BUTTON(button), inconsistent);

  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(button)) != active) {
    g_signal_handlers_block_by_func(
        button, gimp_prop_enum_check_button_callback, config);
 ...
 * Since: 3.0
 */
GtkWidget *gimp_prop_switch_new(GObject *config, const gchar *property_name,
                                const gchar *label, GtkWidget **label_out,
...
GtkWidget *gimp_prop_int_combo_box_new(GObject *config,
  ...
  param_spec =
      check_param_spec_w(config, property_name, G_TYPE_PARAM_INT, G_STRFUNC);
  if (!param_spec)
    return NULL;

  combo_box = g_object_new(GIMP_TYPE_INT_COMBO_BOX, "model", store, "visible",
                           TRUE, NULL);
  ...
  g_object_bind_property(config, property_name, combo_box, "value",
                         G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);
...
GtkWidget *gimp_prop_pointer_combo_box_new(GObject *config,
  ...
  param_spec =
      check_param_spec_w(config, property_name, G_TYPE_PARAM_GTYPE, G_STRFUNC);
  if (!param_spec) {
    param_spec = check_param_spec_w(config, property_name, G_TYPE_PARAM_POINTER,
                                    G_STRFUNC);
    if (!param_spec)
      return NULL;
  }

  g_object_get(config, property_name, &property_value, NULL);
   ...
   */
  combo_box = g_object_new(GIMP_TYPE_INT_COMBO_BOX, "model", store, NULL);

  gimp_int_combo_box_set_active_by_user_data(GIMP_INT_COMBO_BOX(combo_box),
                                             property_value);

  g_signal_connect(combo_box, "changed",
                   G_CALLBACK(gimp_prop_pointer_combo_box_callback), config);

  set_param_spec(G_OBJECT(combo_box), combo_box, param_spec);

  connect_notify(config, property_name,
                 G_CALLBACK(gimp_prop_pointer_combo_box_notify), combo_box);
...
GtkWidget *gimp_prop_enum_combo_box_new(GObject *config,
  ...
  if (minimum != maximum) {
    store = gimp_enum_store_new_with_range(param_spec->value_type, minimum,
                                           maximum);
     ...
     */
    store = gimp_enum_store_new_with_values(
        param_spec->value_type, 5, GIMP_DESATURATE_LUMINANCE,
        GIMP_DESATURATE_LUMA, GIMP_DESATURATE_LIGHTNESS,
        GIMP_DESATURATE_AVERAGE, GIMP_DESATURATE_VALUE);
  }

  if (store) {
    combo_box = g_object_new(GIMP_TYPE_ENUM_COMBO_BOX, "model", store, NULL);
  ...
  g_signal_connect(combo_box, "changed",
                   G_CALLBACK(gimp_prop_int_combo_box_callback), config);

  set_param_spec(G_OBJECT(combo_box), combo_box, param_spec);

  connect_notify(config, property_name,
                 G_CALLBACK(gimp_prop_int_combo_box_notify), combo_box);
...
static void gimp_prop_int_combo_box_notify(GObject *config,
  ...
  g_signal_handlers_block_by_func(combo_box, gimp_prop_int_combo_box_callback,
                                  config);

  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo_box), value);

  g_signal_handlers_unblock_by_func(combo_box, gimp_prop_int_combo_box_callback,
                                    config);
}
...
static void gimp_prop_pointer_combo_box_notify(GObject *config,
  ...
  g_signal_handlers_block_by_func(combo_box,
                                  gimp_prop_pointer_combo_box_callback, config);

  gimp_int_combo_box_set_active_by_user_data(GIMP_INT_COMBO_BOX(combo_box),
                                             value);

  g_signal_handlers_unblock_by_func(
      combo_box, gimp_prop_pointer_combo_box_callback, config);
}
...
GtkWidget *gimp_prop_boolean_combo_box_new(GObject *config,
  ...
  combo = gimp_int_combo_box_new(true_text, TRUE, false_text, FALSE, NULL);

  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), value);

  g_signal_connect(combo, "changed",
                   G_CALLBACK(gimp_prop_boolean_combo_box_callback), config);

  set_param_spec(G_OBJECT(combo), combo, param_spec);

  connect_notify(config, property_name,
                 G_CALLBACK(gimp_prop_boolean_combo_box_notify), combo);
...
static void gimp_prop_boolean_combo_box_notify(GObject *config,
  ...
  g_signal_handlers_block_by_func(combo, gimp_prop_boolean_combo_box_callback,
                                  config);

  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo), value);

  g_signal_handlers_unblock_by_func(combo, gimp_prop_boolean_combo_box_callback,
                                    config);
}
...
static void gimp_prop_radio_button_callback(GtkWidget *widget, GObject *config);
...
GtkWidget *gimp_prop_enum_radio_frame_new(GObject *config,
                                          const gchar *property_name,
                                          const gchar *title, gint minimum,
                                          gint maximum) {
  GParamSpec *param_spec;
  GtkWidget *frame;
  GtkWidget *button;
  ...
  if (minimum != maximum) {
    frame = gimp_enum_radio_frame_new_with_range(
        param_spec->value_type, minimum, maximum, gtk_label_new(title),
        G_CALLBACK(gimp_prop_radio_button_callback), config, NULL, &button);
  } else {
    frame = gimp_enum_radio_frame_new(
        param_spec->value_type, gtk_label_new(title),
        G_CALLBACK(gimp_prop_radio_button_callback), config, NULL, &button);
  }
  ...
  connect_notify(config, property_name,
                 G_CALLBACK(gimp_prop_radio_button_notify), button);
...
GtkWidget *gimp_prop_enum_radio_box_new(GObject *config,
  ...
  if (minimum != maximum) {
    vbox = gimp_enum_radio_box_new_with_range(
        param_spec->value_type, minimum, maximum,
        G_CALLBACK(gimp_prop_radio_button_callback), config, NULL, &button);
  } else {
...
GtkWidget *gimp_prop_int_radio_box_new(GObject *config,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    gtk_tree_model_get(model, &iter, GIMP_INT_STORE_LABEL, &label,
                       GIMP_INT_STORE_VALUE, &value, -1);
    ...
    g_signal_connect(button, "toggled",
                     G_CALLBACK(gimp_prop_radio_button_callback), config);
  }
...
static void gimp_prop_enum_label_notify(GObject *config, GParamSpec *param_spec,
                                        GtkWidget *label);
...
GtkWidget *gimp_prop_enum_label_new(GObject *config,
  ...
  param_spec =
      check_param_spec(config, property_name, G_TYPE_PARAM_ENUM, G_STRFUNC);
  if (!param_spec)
    return NULL;
  ...
  connect_notify(config, property_name, G_CALLBACK(gimp_prop_enum_label_notify),
                 label);
...
GtkWidget *gimp_prop_boolean_radio_frame_new(GObject *config,
  ...
  frame = gimp_int_radio_group_new(
      TRUE, title, G_CALLBACK(gimp_prop_radio_button_callback), config, NULL,
      value,

      false_text, FALSE, &button, true_text, TRUE, NULL,
 ...
 *
 * Returns: (transfer full): A #libgimpwidgets-gimpenumiconbox containing the
 * radio buttons.
 *
 * Since: 2.10
 */
GtkWidget *gimp_prop_enum_icon_box_new(GObject *config,
                                       const gchar *property_name,
                                       const gchar *icon_prefix, gint minimum,
                                       gint maximum) {
  GParamSpec *param_spec;
  GtkWidget *box;
  GtkWidget *button;
  ...
  if (minimum != maximum) {
    box = gimp_enum_icon_box_new_with_range(
        param_spec->value_type, minimum, maximum, icon_prefix,
        GTK_ICON_SIZE_MENU, G_CALLBACK(gimp_prop_radio_button_callback), config,
        NULL, &button);
  } else {
    box = gimp_enum_icon_box_new(
        param_spec->value_type, icon_prefix, GTK_ICON_SIZE_MENU,
        G_CALLBACK(gimp_prop_radio_button_callback), config, NULL, &button);
  }
...
static void gimp_prop_adjustment_callback(GtkAdjustment *adjustment,
                                          GObject *config);
static void gimp_prop_adjustment_notify(GObject *config, GParamSpec *param_spec,
...
GtkWidget *gimp_prop_spin_button_new(GObject *config,
                                     ...
                                     gdouble page_increment, gint digits) {
  GParamSpec *param_spec;
  ...
  if (!get_numeric_values(config, param_spec, &value, &lower, &upper,
                          G_STRFUNC))
    return NULL;
  ...
  adjustment = gtk_adjustment_new(value, lower, upper, step_increment,
                                  page_increment, 0);
  ...
  g_signal_connect(adjustment, "value-changed",
                   G_CALLBACK(gimp_prop_adjustment_callback), config);

  connect_notify(config, property_name, G_CALLBACK(gimp_prop_adjustment_notify),
                 adjustment);
 ...
 * Since: 2.4
 */
GtkWidget *gimp_prop_label_spin_new(GObject *config, const gchar *property_name,
                                    gint digits) {
  const gchar *label;
  GParamSpec *param_spec;
  GtkWidget *widget;
  ...
  g_object_bind_property(config, property_name, widget, "value",
                         G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);
 ...
 * Since: 2.4
 */
GtkWidget *gimp_prop_hscale_new(GObject *config, const gchar *property_name,
  ...
  scale =
      g_object_new(GTK_TYPE_SCALE, "orientation", GTK_ORIENTATION_HORIZONTAL,
                   "adjustment", adjustment, "digits", digits, NULL);
...
GtkWidget *gimp_prop_scale_entry_new(GObject *config,
                                     const gchar *property_name,
                                     const gchar *label, gdouble factor,
                                     gboolean limit_scale, gdouble lower_limit,
                                     gdouble upper_limit) {
  GtkWidget *widget;
  GParamSpec *param_spec;
  const gchar *tooltip;
  ...
  g_return_val_if_fail(G_IS_PARAM_SPEC_DOUBLE(param_spec) || factor == 1.0,
                       NULL);
  ...
  widget = gimp_scale_entry_new(label, value, lower * factor, upper * factor,
                                digits);
  if (limit_scale)
    gimp_scale_entry_set_bounds(GIMP_SCALE_ENTRY(widget), lower_limit,
                                upper_limit, FALSE);
  ...
  if (factor != 1.0) {
    ...
    *user_data = factor;
     ...
     */
    g_object_bind_property_full(config, property_name, widget, "value",
                                G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE,
                                gimp_prop_widget_double_to_factor,
                                gimp_prop_widget_double_from_factor, user_data,
...
static void gimp_prop_adjustment_callback(GtkAdjustment *adjustment,
  ...
  factor =
      g_object_get_data(G_OBJECT(adjustment), "gimp-prop-adjustment-factor");
  if (factor)
    value /= *factor;
...
static void gimp_prop_adjustment_notify(GObject *config, GParamSpec *param_spec,
  ...
  if (G_IS_PARAM_SPEC_INT(param_spec)) {
    ...
    g_warning("%s: unhandled param spec of type %s", G_STRFUNC,
              G_PARAM_SPEC_TYPE_NAME(param_spec));
    return;
  }
  ...
  if (gtk_adjustment_get_value(adjustment) != value) {
    g_signal_handlers_block_by_func(adjustment, gimp_prop_adjustment_callback,
                                    config);

    gtk_adjustment_set_value(adjustment, value);

    g_signal_handlers_unblock_by_func(adjustment, gimp_prop_adjustment_callback,
                                      config);
  }
}
...
static void gimp_prop_memsize_callback(GimpMemsizeEntry *entry,
                                       GObject *config);
static void gimp_prop_memsize_notify(GObject *config, GParamSpec *param_spec,
...
GtkWidget *gimp_prop_memsize_entry_new(GObject *config,
  ...
  entry =
      gimp_memsize_entry_new(value, uint64_spec->minimum, uint64_spec->maximum);
  ...
  g_signal_connect(entry, "value-changed",
                   G_CALLBACK(gimp_prop_memsize_callback), config);

  connect_notify(config, property_name, G_CALLBACK(gimp_prop_memsize_notify),
                 entry);
...
static void gimp_prop_memsize_notify(GObject *config, GParamSpec *param_spec,
  ...
  if (gimp_memsize_entry_get_value(entry) != value) {
    g_signal_handlers_block_by_func(entry, gimp_prop_memsize_callback, config);

    gimp_memsize_entry_set_value(entry, value);

    g_signal_handlers_unblock_by_func(entry, gimp_prop_memsize_callback,
                                      config);
  }
}
 ...
 * Since: 2.4
 */
GtkWidget *gimp_prop_label_new(GObject *config, const gchar *property_name) {
  GParamSpec *param_spec;
  GtkWidget *label;
  const gchar *blurb;
  ...
  g_object_bind_property(config, property_name, label, "label", flags);
...
static void gimp_prop_entry_callback(GtkWidget *entry, GObject *config);
static void gimp_prop_entry_notify(GObject *config, GParamSpec *param_spec,
                                   GtkWidget *entry);
 ...
 * Since: 2.4
 */
GtkWidget *gimp_prop_entry_new(GObject *config, const gchar *property_name,
                               gint max_len) {
  GParamSpec *param_spec;
  GtkWidget *entry;
  gchar *value;
  ...
  param_spec =
      check_param_spec(config, property_name, G_TYPE_PARAM_STRING, G_STRFUNC);
  if (!param_spec)
    return NULL;
  ...
  g_signal_connect(entry, "changed", G_CALLBACK(gimp_prop_entry_callback),
                   config);

  connect_notify(config, property_name, G_CALLBACK(gimp_prop_entry_notify),
                 entry);
...
static void gimp_prop_entry_callback(GtkWidget *entry, GObject *config) {
  ...
  if (g_strcmp0(v, value)) {
    g_signal_handlers_block_by_func(config, gimp_prop_entry_notify, entry);

    g_object_set(config, param_spec->name, value, NULL);

    g_signal_handlers_unblock_by_func(config, gimp_prop_entry_notify, entry);
  }
...
static void gimp_prop_entry_notify(GObject *config, GParamSpec *param_spec,
  ...
  g_signal_handlers_block_by_func(entry, gimp_prop_entry_callback, config);

  gtk_entry_set_text(GTK_ENTRY(entry), value ? value : "");

  g_signal_handlers_unblock_by_func(entry, gimp_prop_entry_callback, config);
...
GtkTextBuffer *gimp_prop_text_buffer_new(GObject *config,
  ...
  param_spec =
      check_param_spec_w(config, property_name, G_TYPE_PARAM_STRING, G_STRFUNC);
  if (!param_spec)
    return NULL;
  ...
  g_signal_connect(text_buffer, "changed",
                   G_CALLBACK(gimp_prop_text_buffer_callback), config);

  connect_notify(config, property_name,
                 G_CALLBACK(gimp_prop_text_buffer_notify), text_buffer);
...
static void gimp_prop_text_buffer_callback(GtkTextBuffer *text_buffer,
  ...
  max_len =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(text_buffer), "max-length"));
  ...
  if (max_len > 0 && g_utf8_strlen(text, -1) > max_len) {
    ...
    gtk_text_buffer_get_iter_at_offset(text_buffer, &start_iter, max_len - 1);
    ...
    if (g_strcmp0(v, text)) {
      g_signal_handlers_block_by_func(config, gimp_prop_text_buffer_notify,
                                      text_buffer);

      g_object_set(config, param_spec->name, text, NULL);

      g_signal_handlers_unblock_by_func(config, gimp_prop_text_buffer_notify,
                                        text_buffer);
    }
...
static void gimp_prop_text_buffer_notify(GObject *config,
  ...
  g_signal_handlers_block_by_func(text_buffer, gimp_prop_text_buffer_callback,
                                  config);

  gtk_text_buffer_set_text(text_buffer, value ? value : "", -1);

  g_signal_handlers_unblock_by_func(text_buffer, gimp_prop_text_buffer_callback,
                                    config);
...
GtkWidget *gimp_prop_string_combo_box_new(GObject *config,
                                          const gchar *property_name,
                                          GtkTreeModel *model, gint id_column,
                                          gint label_column) {
  GParamSpec *param_spec;
  GtkWidget *combo_box;
  gchar *value;
  ...
  g_signal_connect(combo_box, "changed",
                   G_CALLBACK(gimp_prop_string_combo_box_callback), config);

  set_param_spec(G_OBJECT(combo_box), combo_box, param_spec);

  connect_notify(config, property_name,
                 G_CALLBACK(gimp_prop_string_combo_box_notify), combo_box);
...
static void gimp_prop_string_combo_box_notify(GObject *config,
  ...
  g_signal_handlers_block_by_func(combo_box,
                                  gimp_prop_string_combo_box_callback, config);

  gimp_string_combo_box_set_active(GIMP_STRING_COMBO_BOX(combo_box), value);

  g_signal_handlers_unblock_by_func(
      combo_box, gimp_prop_string_combo_box_callback, config);
...
static GtkWidget *gimp_prop_file_chooser_button_setup(GtkWidget *button,
  ...
  g_signal_connect(button, "file-set",
                   G_CALLBACK(gimp_prop_file_chooser_button_callback), config);

  connect_notify(config, param_spec->name,
                 G_CALLBACK(gimp_prop_file_chooser_button_notify), button);
...
static void gimp_prop_file_chooser_button_callback(GtkFileChooser *button,
  ...
  if (g_strcmp0(v, value)) {
    g_signal_handlers_block_by_func(
        config, gimp_prop_file_chooser_button_notify, button);
...
static void gimp_prop_file_chooser_button_notify(GObject *config,
  ...
  g_signal_handlers_block_by_func(
      button, gimp_prop_file_chooser_button_callback, config);
...
GtkWidget *gimp_prop_path_editor_new(GObject *config,
  ...
  if (writable_property_name) {
    writable_param_spec = check_param_spec_w(
        config, writable_property_name, GIMP_TYPE_PARAM_CONFIG_PATH, G_STRFUNC);
    if (!writable_param_spec)
      return NULL;
  }

  g_object_get(config, path_property_name, &value, NULL);
  ...
  if (writable_property_name) {
    g_object_get(config, writable_property_name, &value, NULL);
  ...
  g_signal_connect(editor, "path-changed",
                   G_CALLBACK(gimp_prop_path_editor_path_callback), config);

  connect_notify(config, path_property_name,
                 G_CALLBACK(gimp_prop_path_editor_path_notify), editor);
  ...
  if (writable_property_name) {
    ...
    connect_notify(config, writable_property_name,
                   G_CALLBACK(gimp_prop_path_editor_writable_notify), editor);
  }
...
static void gimp_prop_path_editor_path_callback(GimpPathEditor *editor,
  ...
  path_param_spec =
      g_object_get_data(G_OBJECT(editor), "gimp-config-param-spec-path");
  writable_param_spec =
      g_object_get_data(G_OBJECT(editor), "gimp-config-param-spec-writable");
  if (!path_param_spec)
    return;
  ...
  g_signal_handlers_block_by_func(config, gimp_prop_path_editor_path_notify,
                                  editor);

  g_object_set(config, path_param_spec->name, utf8, NULL);

  g_signal_handlers_unblock_by_func(config, gimp_prop_path_editor_path_notify,
                                    editor);
  ...
  if (writable_param_spec) {
    ...
    g_signal_handlers_block_by_func(
        config, gimp_prop_path_editor_writable_notify, editor);

    g_object_set(config, writable_param_spec->name, utf8, NULL);
...
static void gimp_prop_path_editor_writable_callback(GimpPathEditor *editor,
  ...
  g_signal_handlers_block_by_func(config, gimp_prop_path_editor_writable_notify,
                                  editor);

  g_object_set(config, param_spec->name, utf8, NULL);
...
static void gimp_prop_path_editor_path_notify(GObject *config,
  ...
  g_signal_handlers_block_by_func(editor, gimp_prop_path_editor_path_callback,
                                  config);

  gimp_path_editor_set_path(editor, filename);

  g_signal_handlers_unblock_by_func(editor, gimp_prop_path_editor_path_callback,
                                    config);
...
static void gimp_prop_path_editor_writable_notify(GObject *config,
  ...
  g_signal_handlers_block_by_func(
      editor, gimp_prop_path_editor_writable_callback, config);
...
static void gimp_prop_size_entry_callback(GimpSizeEntry *entry,
                                          GObject *config);
static void gimp_prop_size_entry_notify(GObject *config, GParamSpec *param_spec,
                                        GimpSizeEntry *entry);
static void gimp_prop_size_entry_notify_unit(GObject *config,
                                             GParamSpec *param_spec,
                                             GimpSizeEntry *entry);
static gint gimp_prop_size_entry_num_chars(gdouble lower, gdouble upper);
 ...
 * Since: 2.4
 */
GtkWidget *gimp_prop_size_entry_new(GObject *config, const gchar *property_name,
  ...
  if (unit_property_name) {
    ...
    g_object_get(config, unit_property_name, &unit_value, NULL);
  } else {
  ...
  entry = gimp_size_entry_new(
      1, unit_value, unit_format, show_pixels, show_percent, FALSE,
  ...
  if (unit_param_spec)
    set_param_spec(NULL, gimp_size_entry_get_unit_combo(GIMP_SIZE_ENTRY(entry)),
                   unit_param_spec);

  gimp_size_entry_set_unit(GIMP_SIZE_ENTRY(entry), unit_value);

  if (update_policy == GIMP_SIZE_ENTRY_UPDATE_SIZE)
    gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(entry), 0, resolution,
                                   FALSE);

  gimp_size_entry_set_value_boundaries(GIMP_SIZE_ENTRY(entry), 0, lower, upper);
  ...
  g_signal_connect(entry, "refval-changed",
                   G_CALLBACK(gimp_prop_size_entry_callback), config);
  g_signal_connect(entry, "value-changed",
                   G_CALLBACK(gimp_prop_size_entry_callback), config);

  connect_notify(config, property_name, G_CALLBACK(gimp_prop_size_entry_notify),
                 entry);
  ...
  if (unit_property_name) {
    ...
    connect_notify(config, unit_property_name,
                   G_CALLBACK(gimp_prop_size_entry_notify_unit), entry);
  }
...
static void gimp_prop_size_entry_callback(GimpSizeEntry *entry,
  ...
  unit_param_spec =
      g_object_get_data(G_OBJECT(entry), "gimp-config-param-spec-unit");

  value_is_pixel =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(entry), "value-is-pixel"));
  ...
  if (unit_param_spec) {
    ...
    g_object_get(config, unit_param_spec->name, &old_unit, NULL);
  ...
  if (G_IS_PARAM_SPEC_INT(param_spec)) {
    g_object_set(config, param_spec->name, ROUND(value),

                 unit_param_spec ? unit_param_spec->name : NULL, unit_value,

                 NULL);
  } else if (G_IS_PARAM_SPEC_DOUBLE(param_spec)) {
    g_object_set(config, param_spec->name, value,
...
static void gimp_prop_size_entry_notify(GObject *config, GParamSpec *param_spec,
  ...
  if (value != entry_value) {
    g_signal_handlers_block_by_func(entry, gimp_prop_size_entry_callback,
                                    config);
    ...
    g_signal_handlers_unblock_by_func(entry, gimp_prop_size_entry_callback,
                                      config);
  }
}
...
GtkWidget *gimp_prop_coordinates_new(
    GObject *config, const gchar *x_property_name, const gchar *y_property_name,
    const gchar *unit_property_name, const gchar *unit_format,
    GimpSizeEntryUpdatePolicy update_policy, gdouble xresolution,
    gdouble yresolution, gboolean has_chainbutton) {
  GtkWidget *entry;
  GtkWidget *chainbutton = NULL;

  entry = gimp_size_entry_new(2, GIMP_UNIT_INCH, unit_format, FALSE, FALSE,
                              TRUE, 10, update_policy);
  ...
  if (!gimp_prop_coordinates_connect(config, x_property_name, y_property_name,
                                     unit_property_name, entry, chainbutton,
...
gboolean gimp_prop_coordinates_connect(
    GObject *config, const gchar *x_property_name, const gchar *y_property_name,
    const gchar *unit_property_name, GtkWidget *entry, GtkWidget *chainbutton,
  ...
  g_return_val_if_fail(
      gimp_size_entry_get_n_fields(GIMP_SIZE_ENTRY(entry)) == 2, FALSE);
  g_return_val_if_fail(chainbutton == NULL || GIMP_IS_CHAIN_BUTTON(chainbutton),
                       FALSE);
  ...
  if (!get_numeric_values(config, x_param_spec, &x_value, &x_lower, &x_upper,
                          G_STRFUNC) ||
      !get_numeric_values(config, y_param_spec, &y_value, &y_lower, &y_upper,
                          G_STRFUNC))
    return FALSE;
  ...
  case GIMP_SIZE_ENTRY_UPDATE_SIZE:
    gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(entry), 0, xresolution,
                                   FALSE);
    gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(entry), 1, yresolution,
                                   FALSE);
  ...
  gimp_size_entry_set_refval_boundaries(GIMP_SIZE_ENTRY(entry), 0, x_lower,
                                        x_upper);
  gimp_size_entry_set_refval_boundaries(GIMP_SIZE_ENTRY(entry), 1, y_lower,
                                        y_upper);

  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(entry), 0, x_value);
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(entry), 1, y_value);

  g_object_set_data(G_OBJECT(entry), "gimp-config-param-spec-x", x_param_spec);
  g_object_set_data(G_OBJECT(entry), "gimp-config-param-spec-y", y_param_spec);

  old_x_value = g_new0(gdouble, 1);
  *old_x_value = x_value;
  g_object_set_data_full(G_OBJECT(entry), "old-x-value", old_x_value,
                         (GDestroyNotify)g_free);

  old_y_value = g_new0(gdouble, 1);
  *old_y_value = y_value;
  g_object_set_data_full(G_OBJECT(entry), "old-y-value", old_y_value,
                         (GDestroyNotify)g_free);
  ...
  g_signal_connect(entry, "value-changed",
                   G_CALLBACK(gimp_prop_coordinates_callback), config);
  g_signal_connect(entry, "refval-changed",
                   G_CALLBACK(gimp_prop_coordinates_callback), config);

  connect_notify(config, x_property_name,
                 G_CALLBACK(gimp_prop_coordinates_notify_x), entry);
  connect_notify(config, y_property_name,
                 G_CALLBACK(gimp_prop_coordinates_notify_y), entry);
  ...
  if (unit_property_name) {
    ...
    *old_unit_value = unit_value;
    g_object_set_data_full(G_OBJECT(entry), "old-unit-value", old_unit_value,
                           (GDestroyNotify)g_free);
    ...
    connect_notify(config, unit_property_name,
                   G_CALLBACK(gimp_prop_coordinates_notify_unit), entry);
  }
...
static void gimp_prop_coordinates_callback(GimpSizeEntry *entry,
  ...
  x_param_spec = g_object_get_data(G_OBJECT(entry), "gimp-config-param-spec-x");
  y_param_spec = g_object_get_data(G_OBJECT(entry), "gimp-config-param-spec-y");
  ...
  *old_y_value = y_value;
  ...
  if (unit_param_spec)
    g_object_set(config, unit_param_spec->name, unit_value, NULL);

  if (G_IS_PARAM_SPEC_INT(x_param_spec) && G_IS_PARAM_SPEC_INT(y_param_spec)) {
    if (backwards)
      g_object_set(config, y_param_spec->name, ROUND(y_value),
                   x_param_spec->name, ROUND(x_value), NULL);
    else
      g_object_set(config, x_param_spec->name, ROUND(x_value),
                   y_param_spec->name, ROUND(y_value), NULL);

  } else if (G_IS_PARAM_SPEC_DOUBLE(x_param_spec) &&
             G_IS_PARAM_SPEC_DOUBLE(y_param_spec)) {
    if (backwards)
      g_object_set(config, y_param_spec->name, y_value, x_param_spec->name,
                   x_value, NULL);
    else
      g_object_set(config, x_param_spec->name, x_value, y_param_spec->name,
                   y_value, NULL);
  }
}
...
static void gimp_prop_coordinates_notify_x(GObject *config,
  ...
  if (value != gimp_size_entry_get_refval(entry, 0)) {
    gdouble *old_x_value = g_object_get_data(G_OBJECT(entry), "old-x-value");

    g_signal_handlers_block_by_func(entry, gimp_prop_coordinates_callback,
                                    config);
    ...
    g_signal_handlers_unblock_by_func(entry, gimp_prop_coordinates_callback,
                                      config);
  }
}
...
static void gimp_prop_coordinates_notify_y(GObject *config,
  ...
  if (value != gimp_size_entry_get_refval(entry, 1)) {
    gdouble *old_y_value = g_object_get_data(G_OBJECT(entry), "old-y-value");
...
static void gimp_prop_color_area_callback(GtkWidget *widget, GObject *config);
static void gimp_prop_color_area_notify(GObject *config, GParamSpec *param_spec,
                                        GtkWidget *area);
 ...
 * Since: 2.4
 */
GtkWidget *gimp_prop_color_area_new(GObject *config, const gchar *property_name,
  ...
  param_spec =
      check_param_spec_w(config, property_name, GIMP_TYPE_PARAM_RGB, G_STRFUNC);
  if (!param_spec)
    return NULL;

  g_object_get(config, property_name, &value, NULL);

  area = gimp_color_area_new(value, type, GDK_BUTTON1_MASK | GDK_BUTTON2_MASK);
  ...
  g_signal_connect(area, "color-changed",
                   G_CALLBACK(gimp_prop_color_area_callback), config);

  connect_notify(config, property_name, G_CALLBACK(gimp_prop_color_area_notify),
                 area);
...
static void gimp_prop_color_area_callback(GtkWidget *area, GObject *config) {
  GParamSpec *param_spec;
  GimpRGB value;
  ...
  g_signal_handlers_block_by_func(config, gimp_prop_color_area_notify, area);

  g_object_set(config, param_spec->name, &value, NULL);

  g_signal_handlers_unblock_by_func(config, gimp_prop_color_area_notify, area);
}
...
static void gimp_prop_color_area_notify(GObject *config, GParamSpec *param_spec,
  ...
  g_signal_handlers_block_by_func(area, gimp_prop_color_area_callback, config);

  gimp_color_area_set_color(GIMP_COLOR_AREA(area), value);

  g_free(value);

  g_signal_handlers_unblock_by_func(area, gimp_prop_color_area_callback,
                                    config);
}
...
GtkWidget *gimp_prop_unit_combo_box_new(GObject *config,
  ...
  param_spec = check_param_spec_w(config, property_name, GIMP_TYPE_PARAM_UNIT,
                                  G_STRFUNC);
  if (!param_spec)
    return NULL;
  ...
  g_object_get(config, property_name, &unit, NULL);
  ...
  g_signal_connect(combo, "changed",
                   G_CALLBACK(gimp_prop_unit_combo_box_callback), config);

  connect_notify(config, property_name,
                 G_CALLBACK(gimp_prop_unit_combo_box_notify), combo);
...
static void gimp_prop_unit_combo_box_callback(GtkWidget *combo,
  ...
  if (v != value) {
    ...
    g_signal_handlers_block_by_func(config, gimp_prop_unit_combo_box_notify,
                                    combo);

    g_object_set(config, param_spec->name, value, NULL);

    g_signal_handlers_unblock_by_func(config, gimp_prop_unit_combo_box_notify,
                                      combo);
  }
}
...
static void gimp_prop_unit_combo_box_notify(GObject *config,
  ...
  g_object_get(config, param_spec->name, &unit, NULL);

  g_signal_handlers_block_by_func(combo, gimp_prop_unit_combo_box_callback,
                                  config);
  ...
  g_signal_handlers_unblock_by_func(combo, gimp_prop_unit_combo_box_callback,
                                    config);
}
 ...
 * Since: 2.10
 */
GtkWidget *gimp_prop_icon_image_new(GObject *config, const gchar *property_name,
  ...
  g_object_get(config, property_name, &icon_name, NULL);
  ...
  g_object_bind_property(config, property_name, image, "icon-name",
...
static void gimp_prop_expander_notify(GObject *config, GParamSpec *param_spec,
 ...
 * Since: 2.4
 */
GtkWidget *gimp_prop_expander_new(GObject *config, const gchar *property_name,
                                  const gchar *label) {
  GParamSpec *param_spec;
  GtkWidget *expander;
  gboolean value;
  ...
  expander =
      g_object_new(GTK_TYPE_EXPANDER, "label", label, "expanded", value, NULL);

  set_param_spec(G_OBJECT(expander), expander, param_spec);

  g_signal_connect(expander, "notify::expanded",
                   G_CALLBACK(gimp_prop_expanded_notify), config);

  connect_notify(config, property_name, G_CALLBACK(gimp_prop_expander_notify),
                 expander);
...
static void gimp_prop_expanded_notify(GtkExpander *expander,
  ...
  g_object_set(config, param_spec->name, gtk_expander_get_expanded(expander),
               NULL);
}
...
static void gimp_prop_expander_notify(GObject *config, GParamSpec *param_spec,
  ...
  if (gtk_expander_get_expanded(expander) != value) {
    g_signal_handlers_block_by_func(expander, gimp_prop_expanded_notify,
                                    config);

    gtk_expander_set_expanded(expander, value);

    g_signal_handlers_unblock_by_func(expander, gimp_prop_expanded_notify,
                                      config);
  }
}
...
static GQuark gimp_prop_widgets_param_spec_quark(void) {
...
static void set_radio_spec(GObject *object, GParamSpec *param_spec) {
  ...
  for (list = gtk_radio_button_get_group(GTK_RADIO_BUTTON(object)); list;
...
static GParamSpec *find_param_spec(GObject *object, const gchar *property_name,
  ...
  if (!param_spec)
    g_warning("%s: %s has no property named '%s'", strloc,
              g_type_name(G_TYPE_FROM_INSTANCE(object)), property_name);
...
static GParamSpec *check_param_spec(GObject *object, const gchar *property_name,
  ...
  if (param_spec && !g_type_is_a(G_TYPE_FROM_INSTANCE(param_spec), type)) {
    g_warning("%s: property '%s' of %s is not a %s", strloc, param_spec->name,
              g_type_name(param_spec->owner_type), g_type_name(type));
    return NULL;
  }
...
static GParamSpec *check_param_spec_w(GObject *object,
  ...
  if (param_spec && (param_spec->flags & G_PARAM_WRITABLE) == 0) {
    g_warning("%s: property '%s' of %s is not writable", strloc,
              param_spec->name, g_type_name(param_spec->owner_type));
    return NULL;
  }
...
static gboolean get_numeric_values(GObject *object, GParamSpec *param_spec,
  ...
  if (G_IS_PARAM_SPEC_INT(param_spec)) {
    ...
    *upper = double_spec->maximum;
    ...
    g_warning("%s: property '%s' of %s is not numeric", strloc,
              param_spec->name, g_type_name(G_TYPE_FROM_INSTANCE(object)));
    return FALSE;
  }
