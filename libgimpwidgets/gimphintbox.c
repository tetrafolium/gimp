...
enum { PROP_0, PROP_ICON_NAME, PROP_HINT };
...
static void gimp_hint_box_set_property(GObject *object, guint property_id,
                                       const GValue *value, GParamSpec *pspec);
static void gimp_hint_box_get_property(GObject *object, guint property_id,
...
static void gimp_hint_box_class_init(GimpHintBoxClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_HINT,
      g_param_spec_string("hint", "Hint", "The hint to display", NULL,
                          G_PARAM_CONSTRUCT_ONLY | GIMP_PARAM_READWRITE));
}

static void gimp_hint_box_init(GimpHintBox *box) {
...
static void gimp_hint_box_constructed(GObject *object) {
  ...
  label = g_object_new(GTK_TYPE_LABEL, "label", box->priv->hint, "wrap", TRUE,
                       "justify", GTK_JUSTIFY_LEFT, "xalign", 0.0, "yalign",
                       0.5, NULL);
...
GtkWidget *gimp_hint_box_new(const gchar *hint) {
  ...
  return g_object_new(GIMP_TYPE_HINT_BOX, "hint", hint, NULL);
}
