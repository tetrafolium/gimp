...
enum { PROP_0, PROP_NAME, PROP_STATE };

enum { EVENT, LAST_SIGNAL };

static void gimp_controller_finalize(GObject *object);
static void gimp_controller_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_controller_get_property(GObject *object, guint property_id,
...
static void gimp_controller_class_init(GimpControllerClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("name", "Name", "The controller's name",
                          "Unnamed Controller",
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_STATE,
      g_param_spec_string("state", "State",
                          "The controller's state, as human-readable string",
                          "Unknown", GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  ...
  controller_signals[EVENT] =
      g_signal_new("event", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   ...
                   _gimp_widgets_marshal_BOOLEAN__POINTER, G_TYPE_BOOLEAN, 1,
                   G_TYPE_POINTER);
}

static void gimp_controller_init(GimpController *controller) {}
...
const gchar *gimp_controller_get_event_name(GimpController *controller,
  ...
  if (GIMP_CONTROLLER_GET_CLASS(controller)->get_event_name)
    name = GIMP_CONTROLLER_GET_CLASS(controller)
               ->get_event_name(controller, event_id);
...
const gchar *gimp_controller_get_event_blurb(GimpController *controller,
  ...
  if (GIMP_CONTROLLER_GET_CLASS(controller)->get_event_blurb)
    blurb = GIMP_CONTROLLER_GET_CLASS(controller)
                ->get_event_blurb(controller, event_id);
