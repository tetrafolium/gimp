...
static QueryBox *
create_query_box(const gchar *title, GtkWidget *parent, GimpHelpFunc help_func,
                 const gchar *help_id, GCallback response_callback,
                 const gchar *icon_name, const gchar *message,
                 const gchar *ok_button, const gchar *cancel_button,
                 GObject *object, const gchar *signal, GCallback callback,
...
static void string_query_box_response(GtkWidget *widget, gint response_id,
                                      QueryBox *query_box);
...
static void size_query_box_response(GtkWidget *widget, gint response_id,
                                    QueryBox *query_box);
static void boolean_query_box_response(GtkWidget *widget, gint response_id,
                                       QueryBox *query_box);
...
create_query_box(const gchar *title, GtkWidget *parent, GimpHelpFunc help_func,
  ...
  query_box->qbox = gimp_dialog_new(
      title, "gimp-query-box", parent, 0, help_func, help_id,

      cancel_button, GTK_RESPONSE_CANCEL, ok_button, GTK_RESPONSE_OK,

      NULL);

  gimp_dialog_set_alternative_button_order(
      GTK_DIALOG(query_box->qbox), GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL, -1);

  query_box->response_handler = g_signal_connect(
      query_box->qbox, "response", G_CALLBACK(response_callback), query_box);

  g_signal_connect(query_box->qbox, "destroy", G_CALLBACK(gtk_widget_destroyed),
                   &query_box->qbox);
  ...
  if (hbox) {
    ...
    gtk_box_pack_start(GTK_BOX(content_area), query_box->vbox, TRUE, TRUE, 0);
  }
 ...
 * @help_func:    (scope async): The help function to show this dialog's help
 *page.
 ...
 * @signal:       The object's signal which will cause the query box to be
 *closed.
...
GtkWidget *gimp_query_string_box(const gchar *title, GtkWidget *parent,
  ...
  query_box = create_query_box(
      title, parent, help_func, help_id, G_CALLBACK(string_query_box_response),
      "dialog-question", message, _("_OK"), _("_Cancel"), object, signal,
      G_CALLBACK(callback), data, data_destroy);
...
GtkWidget *gimp_query_int_box(const gchar *title, GtkWidget *parent,
  ...
  query_box = create_query_box(
      title, parent, help_func, help_id, G_CALLBACK(int_query_box_response),
      "dialog-question", message, _("_OK"), _("_Cancel"), object, signal,
      G_CALLBACK(callback), data, data_destroy);
...
GtkWidget *gimp_query_double_box(const gchar *title, GtkWidget *parent,
  ...
  query_box = create_query_box(
      title, parent, help_func, help_id, G_CALLBACK(double_query_box_response),
      "dialog-question", message, _("_OK"), _("_Cancel"), object, signal,
      G_CALLBACK(callback), data, data_destroy);
 ...
 * Returns: (transfer full): A pointer to the new #GtkDialog.
 **/
GtkWidget *gimp_query_size_box(const gchar *title, GtkWidget *parent,
                               GimpHelpFunc help_func, const gchar *help_id,
                               const gchar *message, gdouble initial,
                               gdouble lower, gdouble upper, gint digits,
                               GimpUnit unit, gdouble resolution,
                               gboolean dot_for_dot, GObject *object,
                               const gchar *signal,
                               GimpQuerySizeCallback callback, gpointer data,
                               GDestroyNotify data_destroy) {
  QueryBox *query_box;
  GtkWidget *sizeentry;
  GtkWidget *spinbutton;
  ...
  query_box = create_query_box(
      title, parent, help_func, help_id, G_CALLBACK(size_query_box_response),
      "dialog-question", message, _("_OK"), _("_Cancel"), object, signal,
      G_CALLBACK(callback), data, data_destroy);
  ...
  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(sizeentry), 0, resolution,
                                 FALSE);
  gimp_size_entry_set_refval_digits(GIMP_SIZE_ENTRY(sizeentry), 0, digits);
  gimp_size_entry_set_refval_boundaries(GIMP_SIZE_ENTRY(sizeentry), 0, lower,
                                        upper);
...
GtkWidget *gimp_query_boolean_box(const gchar *title, GtkWidget *parent,
  ...
  query_box = create_query_box(
      title, parent, help_func, help_id, G_CALLBACK(boolean_query_box_response),
      icon_name, message, true_button, false_button, object, signal,
      G_CALLBACK(callback), data, data_destroy);
...
static void query_box_disconnect(QueryBox *query_box) {
  ...
  if (query_box->response_handler) {
    g_signal_handler_disconnect(query_box->qbox, query_box->response_handler);
  ...
  if (query_box->object)
    g_signal_handlers_disconnect_by_func(query_box->object,
                                         query_box_cancel_callback, query_box);
}
...
static void string_query_box_response(GtkWidget *widget, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK)
    (*(GimpQueryStringCallback)query_box->callback)(query_box->qbox, string,
...
static void int_query_box_response(GtkWidget *widget, gint response_id,
  ...
  /*  Call the user defined callback  */
  if (response_id == GTK_RESPONSE_OK)
    (*(GimpQueryIntCallback)query_box->callback)(query_box->qbox, value,
                                                 query_box->callback_data);
...
static void double_query_box_response(GtkWidget *widget, gint response_id,
  ...
  /*  Call the user defined callback  */
  if (response_id == GTK_RESPONSE_OK)
    (*(GimpQueryDoubleCallback)query_box->callback)(query_box->qbox, value,
                                                    query_box->callback_data);
...
static void size_query_box_response(GtkWidget *widget, gint response_id,
  ...
  /*  Call the user defined callback  */
  if (response_id == GTK_RESPONSE_OK)
    (*(GimpQuerySizeCallback)query_box->callback)(query_box->qbox, size, unit,
                                                  query_box->callback_data);
...
static void boolean_query_box_response(GtkWidget *widget, gint response_id,
  ...
  /*  Call the user defined callback  */
  (*(GimpQueryBooleanCallback)query_box->callback)(
      query_box->qbox, (response_id == GTK_RESPONSE_OK),
      query_box->callback_data);
