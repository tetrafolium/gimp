...
enum { FILENAME_CHANGED, LAST_SIGNAL };
...
static void gimp_file_entry_entry_changed(GtkWidget *widget, GtkWidget *button);
static void gimp_file_entry_entry_activate(GtkWidget *widget,
                                           GimpFileEntry *entry);
static gint gimp_file_entry_entry_focus_out(GtkWidget *widget, GdkEvent *event,
                                            GimpFileEntry *entry);
...
static void gimp_file_entry_class_init(GimpFileEntryClass *klass) {
   ...
   **/
  gimp_file_entry_signals[FILENAME_CHANGED] = g_signal_new(
      "filename-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpFileEntryClass, filename_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
...
static void gimp_file_entry_init(GimpFileEntry *entry) {
  ...
  g_signal_connect(button, "clicked",
                   G_CALLBACK(gimp_file_entry_file_manager_clicked), entry);

  gimp_help_set_help_data(button, _("Show file location in the file manager"),
                          NULL);
  ...
  g_signal_connect(entry->priv->browse_button, "clicked",
                   G_CALLBACK(gimp_file_entry_browse_clicked), entry);
  ...
  g_signal_connect(entry->priv->entry, "changed",
                   G_CALLBACK(gimp_file_entry_entry_changed), button);
  g_signal_connect(entry->priv->entry, "activate",
                   G_CALLBACK(gimp_file_entry_entry_activate), entry);
  g_signal_connect(entry->priv->entry, "focus-out-event",
                   G_CALLBACK(gimp_file_entry_entry_focus_out), entry);
}
...
GtkWidget *gimp_file_entry_new(const gchar *title, const gchar *filename,
  ...
  if (check_valid) {
    entry->priv->file_exists =
        gtk_image_new_from_icon_name("gtk-no", GTK_ICON_SIZE_BUTTON);
    gtk_box_pack_start(GTK_BOX(entry), entry->priv->file_exists, FALSE, FALSE,
                       0);
    gtk_widget_show(entry->priv->file_exists);

    gimp_help_set_help_data(
        entry->priv->file_exists,
        entry->priv->dir_only ? _("Indicates whether or not the folder exists")
...
static void gimp_file_entry_entry_activate(GtkWidget *widget,
  ...
  while (((len = strlen(utf8)) > 1) && (utf8[len - 1] == G_DIR_SEPARATOR))
    utf8[len - 1] = '\0';

  filename = g_filename_from_utf8(utf8, -1, NULL, NULL, NULL);

  g_signal_handlers_block_by_func(entry->priv->entry,
                                  gimp_file_entry_entry_activate, entry);
  gtk_entry_set_text(GTK_ENTRY(entry->priv->entry), utf8);
...
static gboolean gimp_file_entry_entry_focus_out(GtkWidget *widget,
                                                GdkEvent *event,
                                                GimpFileEntry *entry) {
...
static void gimp_file_entry_chooser_response(GtkWidget *dialog,
                                             gint response_id,
                                             GimpFileEntry *entry) {
...
static void gimp_file_entry_file_manager_clicked(GtkWidget *widget,
  ...
  if (!gimp_file_show_in_file_manager(file, &error)) {
    g_message(_("Can't show file in file manager: %s"), error->message);
    g_clear_error(&error);
  }
...
static void gimp_file_entry_browse_clicked(GtkWidget *widget,
  ...
  if (!entry->priv->file_dialog) {
    ...
    entry->priv->file_dialog = gtk_file_chooser_dialog_new(
        title, NULL,
        entry->priv->dir_only ? GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER
    ...
    gimp_dialog_set_alternative_button_order(
        GTK_DIALOG(entry->priv->file_dialog), GTK_RESPONSE_OK,
        GTK_RESPONSE_CANCEL, -1);

    chooser = GTK_FILE_CHOOSER(entry->priv->file_dialog);

    gtk_window_set_position(GTK_WINDOW(chooser), GTK_WIN_POS_MOUSE);
    gtk_window_set_role(GTK_WINDOW(chooser), "gimp-file-entry-file-dialog");

    g_signal_connect(chooser, "response",
                     G_CALLBACK(gimp_file_entry_chooser_response), entry);
    g_signal_connect(chooser, "delete-event", G_CALLBACK(gtk_true), NULL);

    g_signal_connect_swapped(entry, "unmap", G_CALLBACK(gtk_widget_hide),
                             chooser);
  } else {
    chooser = GTK_FILE_CHOOSER(entry->priv->file_dialog);
  }
