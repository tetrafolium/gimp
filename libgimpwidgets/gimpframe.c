...
static gboolean gimp_frame_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_frame_class_init(GimpFrameClass *klass) {
  ...
  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_boolean("label-bold", "Label Bold",
                           "Whether the frame's label should be bold",
                           DEFAULT_LABEL_BOLD, G_PARAM_READABLE));
  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_int("label-spacing", "Label Spacing",
                       "The spacing between the label and the frame content", 0,
                       G_MAXINT, DEFAULT_LABEL_SPACING, G_PARAM_READABLE));
}

static void gimp_frame_init(GimpFrame *frame) {
  g_signal_connect(frame, "notify::label-widget",
                   G_CALLBACK(gimp_frame_label_widget_notify), NULL);
  g_signal_connect(frame, "notify::child", G_CALLBACK(gimp_frame_child_notify),
                   NULL);
}
...
static void gimp_frame_label_widget_notify(GimpFrame *frame) {
  ...
  if (label_widget) {
    ...
    if (label) {
      ...
      gimp_label_set_attributes(label, PANGO_ATTR_WEIGHT, PANGO_WEIGHT_BOLD,
                                -1);
    }
  }
}
...
static gint gimp_frame_get_indent(GimpFrame *frame) {
  gint width = GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(frame), GIMP_FRAME_INDENT_KEY));
  ...
  if (!width) {
    ...
    g_object_set_data(G_OBJECT(frame), GIMP_FRAME_INDENT_KEY,
...
static gint gimp_frame_get_label_spacing(GimpFrame *frame) {
  ...
  if ((label_widget && gtk_widget_get_visible(label_widget)) ||
    ...
    gtk_widget_style_get(GTK_WIDGET(frame), "label_spacing", &spacing, NULL);
  }
...
GtkWidget *gimp_frame_new(const gchar *label) {
  ...
  frame = g_object_new(GIMP_TYPE_FRAME, "label", label, NULL);

  if (expander)
    g_object_set_data(G_OBJECT(frame), GIMP_FRAME_IN_EXPANDER_KEY,
                      (gpointer)TRUE);
