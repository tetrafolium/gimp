...
static void gimp_unit_combo_box_class_init(GimpUnitComboBoxClass *klass) {
...
static void gimp_unit_combo_box_init(GimpUnitComboBox *combo) {
  ...
  gtk_cell_layout_set_attributes(layout, cell, "text",
                                 GIMP_UNIT_STORE_UNIT_LONG_FORMAT, NULL);

  g_signal_connect(combo, "notify::popup-shown",
                   G_CALLBACK(gimp_unit_combo_box_popup_shown), NULL);
}
...
static void gimp_unit_combo_box_style_updated(GtkWidget *widget) {
  ...
  gtk_cell_layout_pack_start(layout, cell, TRUE);
  gtk_cell_layout_set_attributes(layout, cell, "text",
                                 GIMP_UNIT_STORE_UNIT_SHORT_FORMAT, NULL);
...
static void gimp_unit_combo_box_popup_shown(GtkWidget *widget,
  ...
  g_object_get(widget, "model", &store, "popup-shown", &shown, NULL);
...
GtkWidget *gimp_unit_combo_box_new(void) {
  ...
  combo_box = g_object_new(GIMP_TYPE_UNIT_COMBO_BOX, "model", store, NULL);
...
GtkWidget *gimp_unit_combo_box_new_with_model(GimpUnitStore *model) {
  return g_object_new(GIMP_TYPE_UNIT_COMBO_BOX, "model", model, NULL);
}
 ...
 **/
void gimp_unit_combo_box_set_active(GimpUnitComboBox *combo, GimpUnit unit) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  gboolean iter_valid;
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    gtk_tree_model_get(model, &iter, GIMP_UNIT_STORE_UNIT, &iter_unit, -1);
