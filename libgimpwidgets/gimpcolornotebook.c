...
static void gimp_color_notebook_set_model_visible(GimpColorSelector *selector,
...
static void gimp_color_notebook_model_visible_changed(
    GimpColorSelector *page, GimpColorSelectorModel model, gboolean visible,
    GimpColorNotebook *notebook);
...
static void gimp_color_notebook_class_init(GimpColorNotebookClass *klass) {
  ...
  gtk_widget_class_install_style_property(
      widget_class, g_param_spec_enum("tab-icon-size", NULL,
                                      ...
                                      GTK_TYPE_ICON_SIZE, DEFAULT_TAB_ICON_SIZE,
                                      G_PARAM_READABLE));
...
private
  ...
  g_signal_connect(private->notebook, "switch-page",
                   G_CALLBACK(gimp_color_notebook_switch_page), notebook);
  g_signal_connect(private->notebook, "remove",
                   G_CALLBACK(gimp_color_notebook_remove_selector), notebook);
...
static void gimp_color_notebook_style_updated(GtkWidget *widget) {
  ...
  gtk_widget_style_get(widget, "tab-icon-size", &icon_size, NULL);
...
static void gimp_color_notebook_set_color(GimpColorSelector *selector,
  ...
  g_signal_handlers_block_by_func(private->cur_page,
                                  gimp_color_notebook_color_changed, selector);

  gimp_color_selector_set_color(private->cur_page, rgb, hsv);

  g_signal_handlers_unblock_by_func(
      private->cur_page, gimp_color_notebook_color_changed, selector);
}
...
static void gimp_color_notebook_set_channel(GimpColorSelector *selector,
  ...
  g_signal_handlers_block_by_func(
      private->cur_page, gimp_color_notebook_channel_changed, selector);
...
static void gimp_color_notebook_set_model_visible(GimpColorSelector *selector,
  ...
  g_signal_handlers_block_by_func(
      private->cur_page, gimp_color_notebook_model_visible_changed, selector);
...
private
  ...
  g_signal_handlers_block_by_func(
      private->cur_page, gimp_color_notebook_channel_changed, notebook);
  g_signal_handlers_block_by_func(
      private->cur_page, gimp_color_notebook_model_visible_changed, notebook);

  gimp_color_selector_set_color(private->cur_page, &selector->rgb,
                                &selector->hsv);
  ...
  g_signal_handlers_unblock_by_func(
      private->cur_page, gimp_color_notebook_color_changed, notebook);
  g_signal_handlers_unblock_by_func(
      private->cur_page, gimp_color_notebook_channel_changed, notebook);
  g_signal_handlers_unblock_by_func(
      private->cur_page, gimp_color_notebook_model_visible_changed, notebook);
...
static GtkWidget *gimp_color_notebook_add_page(GimpColorNotebook *notebook,
  ...
  page = gimp_color_selector_new(page_type, &selector->rgb, &selector->hsv,
                                 gimp_color_selector_get_channel(selector));
  ...
  show_alpha =
      gimp_color_selector_get_show_alpha(GIMP_COLOR_SELECTOR(notebook));
  ...
  gtk_notebook_append_page_menu(GTK_NOTEBOOK(private->notebook), page, image,
                                menu_widget);

  if (!private->cur_page)
  private
  ->cur_page = GIMP_COLOR_SELECTOR(page);

private
  ->selectors = g_list_append(private->selectors, page);

  gtk_widget_show(page);

  g_signal_connect(page, "color-changed",
                   G_CALLBACK(gimp_color_notebook_color_changed), notebook);
  g_signal_connect(page, "channel-changed",
                   G_CALLBACK(gimp_color_notebook_channel_changed), notebook);
...
static void gimp_color_notebook_remove_selector(GtkContainer *container,
                                                GtkWidget *widget,
                                                GimpColorNotebook *notebook) {
  notebook->priv->selectors = g_list_remove(notebook->priv->selectors, widget);
...
GtkWidget *gimp_color_notebook_set_has_page(GimpColorNotebook *notebook,
  ...
  g_return_val_if_fail(g_type_is_a(page_type, GIMP_TYPE_COLOR_SELECTOR), NULL);
  g_return_val_if_fail(!g_type_is_a(page_type, GIMP_TYPE_COLOR_NOTEBOOK), NULL);
