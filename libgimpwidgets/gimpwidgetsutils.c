...
static GtkWidget *find_mnemonic_widget(GtkWidget *widget, gint level) {
  gboolean can_focus;

  g_object_get(widget, "can-focus", &can_focus, NULL);

  if (GTK_WIDGET_GET_CLASS(widget)->activate_signal || can_focus ||
      GTK_WIDGET_GET_CLASS(widget)->mnemonic_activate !=
          GTK_WIDGET_CLASS(g_type_class_peek(GTK_TYPE_WIDGET))
              ->mnemonic_activate) {
    return widget;
  }
...
void gimp_label_set_attributes(GtkLabel *label, ...) {
  ...
  do {
    ...
    case PANGO_ATTR_FONT_DESC:
      attr =
          pango_attr_font_desc_new(va_arg(args, const PangoFontDescription *));
      break;
 ...
 * @widget: a #GtkWidget.
 *
 * Returns: (transfer none): the #GdkMonitor where @widget is current displayed
 * on.
 */
...
static gboolean track_monitor_configure_event(GtkWidget *toplevel,
                                              GdkEvent *event,
...
static void track_monitor_hierarchy_changed(GtkWidget *widget,
  ...
  if (previous_toplevel) {
    g_signal_handlers_disconnect_by_func(
        previous_toplevel, track_monitor_configure_event, track_data);
  }
...
void gimp_widget_track_monitor(GtkWidget *widget,
  ...
  g_signal_connect(widget, "hierarchy-changed",
                   G_CALLBACK(track_monitor_hierarchy_changed), track_data);
...
GimpColorProfile *gimp_monitor_get_color_profile(GdkMonitor *monitor) {
...
#if defined GDK_WINDOWING_X11
  {
    ...
    if (gdk_property_get(gdk_screen_get_root_window(screen),
                         ...
                         64 * 1024 * 1024, FALSE, &type, &format, &nitems,
                         &data) &&
        nitems > 0) {
      profile = gimp_color_profile_new_from_icc_profile(data, nitems, NULL);
      g_free(data);
    }
    ...
    if (space) {
      ...
      if (data) {
        ...
        profile = gimp_color_profile_new_from_icc_profile(
            buffer, CFDataGetLength(data), NULL);
...
static GimpColorProfile *get_display_profile(GtkWidget *widget,
...
static gboolean profiles_equal(GimpColorProfile *profile1,
...
transform_cache_get(GimpColorConfig *config, GimpColorProfile *src_profile,
                    const Babl *src_format, GimpColorProfile *dest_profile,
                    const Babl *dest_format, GimpColorProfile *proof_profile) {
  GList *list;

  for (list = transform_caches; list; list = g_list_next(list)) {
    TransformCache *cache = list->data;

    if (config == cache->config && src_format == cache->src_format &&
...
static void transform_cache_config_notify(GObject *config,
...
GimpColorTransform *gimp_widget_get_color_transform(
  ...
  case GIMP_COLOR_MANAGEMENT_SOFTPROOF:
    proof_profile =
        gimp_color_config_get_simulation_color_profile(config, NULL);
    /*  fallthru  */
  ...
  cache = transform_cache_get(config, src_profile, src_format, dest_profile,
                              dest_format, proof_profile);
  ...
  cache->notify_id =
      g_signal_connect(cache->config, "notify",
                       G_CALLBACK(transform_cache_config_notify), cache);
  ...
  if (cache->proof_profile) {
    ...
    cache->transform = gimp_color_transform_new_proofing(
        cache->src_profile, cache->src_format, cache->dest_profile,
        cache->dest_format, cache->proof_profile,
        gimp_color_config_get_simulation_intent(config),
        gimp_color_config_get_display_intent(config), flags);
  } else {
    GimpColorTransformFlags flags = 0;
    ...
    cache->transform = gimp_color_transform_new(
        cache->src_profile, cache->src_format, cache->dest_profile,
        cache->dest_format, gimp_color_config_get_display_intent(config),
        flags);
  }
