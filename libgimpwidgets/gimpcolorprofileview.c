...
static void gimp_color_profile_view_init(GimpColorProfileView *view) {
...
static void gimp_color_profile_view_constructed(GObject *object) {
  ...
  gtk_text_buffer_create_tag(buffer, "text", NULL);
  gtk_text_buffer_create_tag(buffer, "title", "weight", PANGO_WEIGHT_BOLD,
                             "scale", PANGO_SCALE_LARGE, NULL);
  gtk_text_buffer_create_tag(buffer, "header", "weight", PANGO_WEIGHT_BOLD,
                             NULL);
  gtk_text_buffer_create_tag(buffer, "error", "style", PANGO_STYLE_OBLIQUE,
                             NULL);
...
void gimp_color_profile_view_set_profile(GimpColorProfileView *view,
  ...
  if (g_set_object(&view->priv->profile, profile) && profile) {
    ...
    if (text && strlen(text)) {
      gtk_text_buffer_insert_with_tags_by_name(buffer, &iter, text, -1, "title",
                                               NULL);
    ...
    if (text && strlen(text)) {
      gtk_text_buffer_insert_with_tags_by_name(buffer, &iter, text, -1, "text",
                                               NULL);
      gtk_text_buffer_insert(buffer, &iter, "\n", 1);
    }
    ...
    if (text && strlen(text)) {
      gtk_text_buffer_insert_with_tags_by_name(
          buffer, &iter, _("Manufacturer: "), -1, "header", NULL);
      gtk_text_buffer_insert_with_tags_by_name(buffer, &iter, text, -1, "text",
                                               NULL);
      gtk_text_buffer_insert(buffer, &iter, "\n", 1);
    }
    ...
    if (text && strlen(text)) {
      gtk_text_buffer_insert_with_tags_by_name(buffer, &iter, _("Copyright: "),
                                               -1, "header", NULL);
      gtk_text_buffer_insert_with_tags_by_name(buffer, &iter, text, -1, "text",
                                               NULL);
      gtk_text_buffer_insert(buffer, &iter, "\n", 1);
...
void gimp_color_profile_view_set_error(GimpColorProfileView *view,
  ...
  gtk_text_buffer_insert_with_tags_by_name(buffer, &iter, message, -1, "error",
                                           NULL);
}
