...
static void gimp_preview_get_property(GObject *object, guint property_id,
                                      GValue *value, GParamSpec *pspec);
static void gimp_preview_set_property(GObject *object, guint property_id,
...
static void gimp_preview_area_realize(GtkWidget *widget, GimpPreview *preview);
...
static gboolean gimp_preview_area_event(GtkWidget *area, GdkEvent *event,
                                        GimpPreview *preview);
...
static void gimp_preview_real_transform(GimpPreview *preview, gint src_x,
                                        gint src_y, gint *dest_x, gint *dest_y);
static void gimp_preview_real_untransform(GimpPreview *preview, gint src_x,
...
static void gimp_preview_class_init(GimpPreviewClass *klass) {
  ...
  preview_signals[INVALIDATED] =
      g_signal_new("invalidated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpPreviewClass, invalidated), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("update", "Update",
                           ...
                           TRUE, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  gtk_widget_class_install_style_property(
      widget_class, g_param_spec_int("size", "Size", "The preview's size", 1,
                                     1024, DEFAULT_SIZE, GIMP_PARAM_READABLE));
}
...
static void gimp_preview_init(GimpPreview *preview) {
  ...
  g_signal_connect_swapped(priv->area, "notify::check-size",
                           G_CALLBACK(gimp_preview_notify_checks), preview);
  g_signal_connect_swapped(priv->area, "notify::check-type",
                           G_CALLBACK(gimp_preview_notify_checks), preview);

  gtk_widget_add_events(
      priv->area, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
                      GDK_SCROLL_MASK | GDK_SMOOTH_SCROLL_MASK |
                      GDK_POINTER_MOTION_HINT_MASK | GDK_BUTTON_MOTION_MASK);

  g_signal_connect(priv->area, "event", G_CALLBACK(gimp_preview_area_event),
                   preview);

  g_signal_connect(priv->area, "realize", G_CALLBACK(gimp_preview_area_realize),
                   preview);
  g_signal_connect(priv->area, "unrealize",
                   G_CALLBACK(gimp_preview_area_unrealize), preview);

  g_signal_connect_data(priv->area, "realize",
                        G_CALLBACK(gimp_preview_area_set_cursor), preview, NULL,
                        G_CONNECT_AFTER | G_CONNECT_SWAPPED);

  g_signal_connect(priv->area, "size-allocate",
                   G_CALLBACK(gimp_preview_area_size_allocate), preview);
  ...
  g_signal_connect(priv->toggle, "toggled",
                   G_CALLBACK(gimp_preview_toggle_callback), preview);
}
...
static void gimp_preview_direction_changed(GtkWidget *widget,
  ...
  gtk_aspect_frame_set(GTK_ASPECT_FRAME(priv->frame), xalign, 0.0, 1.0, TRUE);
}
...
static gboolean gimp_preview_invalidate_now(GimpPreview *preview) {
  ...
  if (toplevel && gtk_widget_get_realized(toplevel)) {
    gdk_window_set_cursor(gtk_widget_get_window(toplevel), priv->cursor_busy);
    gdk_window_set_cursor(gtk_widget_get_window(priv->area), priv->cursor_busy);
...
void gimp_preview_set_update(GimpPreview *preview, gboolean update) {
  ...
  g_object_set(preview, "update", update, NULL);
}
 ...
 * Since: 2.2
 **/
void gimp_preview_set_bounds(GimpPreview *preview, gint xmin, gint ymin,
                             gint xmax, gint ymax) {
  GimpPreviewPrivate *priv;
  ...
  gimp_preview_area_set_max_size(GIMP_PREVIEW_AREA(priv->area), xmax - xmin,
                                 ymax - ymin);
}
 ...
 */
void gimp_preview_get_bounds(GimpPreview *preview, gint *xmin, gint *ymin,
                             gint *xmax, gint *ymax) {
  GimpPreviewPrivate *priv;
...
void gimp_preview_set_size(GimpPreview *preview, gint width, gint height) {
  GimpPreviewPrivate *priv;
...
void gimp_preview_set_offsets(GimpPreview *preview, gint xoff, gint yoff) {
  GimpPreviewPrivate *priv;
 ...
 */
void gimp_preview_get_offsets(GimpPreview *preview, gint *xoff, gint *yoff) {
  GimpPreviewPrivate *priv;
...
void gimp_preview_get_position(GimpPreview *preview, gint *x, gint *y) {
  ...
  if (x)
    *x = priv->xoff + priv->xmin;
  if (y)
    *y = priv->yoff + priv->ymin;
}
 ...
 * Since: 2.4
 **/
void gimp_preview_transform(GimpPreview *preview, gint src_x, gint src_y,
                            gint *dest_x, gint *dest_y) {
  g_return_if_fail(GIMP_IS_PREVIEW(preview));
  g_return_if_fail(dest_x != NULL && dest_y != NULL);

  GIMP_PREVIEW_GET_CLASS(preview)->transform(preview, src_x, src_y, dest_x,
                                             dest_y);
}
 ...
 * Since: 2.4
 **/
void gimp_preview_untransform(GimpPreview *preview, gint src_x, gint src_y,
                              gint *dest_x, gint *dest_y) {
  g_return_if_fail(GIMP_IS_PREVIEW(preview));
  g_return_if_fail(dest_x != NULL && dest_y != NULL);

  GIMP_PREVIEW_GET_CLASS(preview)->untransform(preview, src_x, src_y, dest_x,
                                               dest_y);
}
 ...
 * @preview: a #GimpPreview widget
 *
 * Returns: (transfer none): a pointer to the #GtkAspectFrame used in the
 *@preview.
 *
 * Since: 3.0
 **/
 ...
 *
 * Returns: (transfer none): a pointer to the #GimpPreviewArea used in the
 *@preview.
 *
 * Since: 2.4
 **/
 ...
 * Since: 2.2
 **/
void gimp_preview_draw_buffer(GimpPreview *preview, const guchar *buffer,
                              gint rowstride) {
  GimpPreviewClass *class = GIMP_PREVIEW_GET_CLASS(preview);
...
void gimp_preview_invalidate(GimpPreview *preview) {
  ...
  if (priv->update_preview) {
    ...
    priv->timeout_id = g_timeout_add_full(
        G_PRIORITY_DEFAULT_IDLE, PREVIEW_TIMEOUT,
        (GSourceFunc)gimp_preview_invalidate_now, preview, NULL);
  }
}
