...
static void gimp_help_menu_item_set_tooltip(GtkWidget *widget,
                                            const gchar *tooltip,
                                            const gchar *help_id);
static gboolean gimp_help_menu_item_query_tooltip(GtkWidget *widget, gint x,
                                                  gint y,
 ...
 * @help_func:         The function which will be called if the user presses
 *"F1".
 * @help_id:           The @help_id which will be passed to @help_func.
 * @help_data:         The @help_data pointer which will be passed to
 *@help_func.
...
void gimp_help_connect(GtkWidget *widget, GimpHelpFunc help_func,
  ...
  if (!initialized) {
    ...
    gtk_binding_entry_add_signal(binding_set, GDK_KEY_F1, 0, "show-help", 1,
                                 GTK_TYPE_WIDGET_HELP_TYPE,
                                 GIMP_WIDGET_HELP_TYPE_HELP);
    gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_F1, 0, "show-help", 1,
                                 GTK_TYPE_WIDGET_HELP_TYPE,
                                 GIMP_WIDGET_HELP_TYPE_HELP);
  ...
  g_object_set_data_full(G_OBJECT(widget), "gimp-help-data", help_data,
                         help_data_destroy);

  g_signal_connect(widget, "show-help", G_CALLBACK(gimp_help_callback),
                   help_func);
 ...
 * Since: 2.2
 **/
GQuark gimp_help_id_quark(void) {
...
static void gimp_help_menu_item_set_tooltip(GtkWidget *widget,
  ...
  if (tooltip && help_id) {
    ...
    g_signal_connect(widget, "query-tooltip",
                     G_CALLBACK(gimp_help_menu_item_query_tooltip), NULL);
  } else if (!tooltip) {
    g_object_set(widget, "has-tooltip", FALSE, NULL);

    g_signal_handlers_disconnect_by_func(
        widget, gimp_help_menu_item_query_tooltip, NULL);
  }
}
...
static gboolean gimp_help_menu_item_query_tooltip(GtkWidget *widget, gint x,
  ...
  gimp_label_set_attributes(GTK_LABEL(label), PANGO_ATTR_STYLE,
                            PANGO_STYLE_ITALIC, PANGO_ATTR_SCALE,
                            PANGO_SCALE_SMALL, -1);
...
static gboolean gimp_context_help_idle_start(gpointer widget) {
  if (!gtk_grab_get_current()) {
    ...
    g_signal_connect(invisible, "button-press-event",
                     G_CALLBACK(gimp_context_help_button_press), NULL);
    g_signal_connect(invisible, "key-press-event",
                     G_CALLBACK(gimp_context_help_key_press), NULL);
  }
...
static gboolean gimp_context_help_idle_show_help(gpointer data) {
  ...
  help_id = gimp_help_get_help_data(GTK_WIDGET(data), &help_widget, &help_data);
