...
enum { PROP_0, PROP_HELP_FUNC, PROP_HELP_ID, PROP_PARENT };
...
static void gimp_dialog_set_property(GObject *object, guint property_id,
                                     const GValue *value, GParamSpec *pspec);
static void gimp_dialog_get_property(GObject *object, guint property_id,
                                     GValue *value, GParamSpec *pspec);

static void gimp_dialog_hide(GtkWidget *widget);
static gboolean gimp_dialog_delete_event(GtkWidget *widget, GdkEventAny *event);
...
static void gimp_dialog_class_init(GimpDialogClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_HELP_FUNC,
      g_param_spec_pointer("help-func", "Help Func",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("help-id", "Help ID",
                          "The help ID to pass to help-func", NULL,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("parent", "Parent", "The dialog's parent widget",
                          GTK_TYPE_WIDGET,
...
static void gimp_dialog_init(GimpDialog *dialog) {
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(gimp_dialog_response), NULL);
}
...
static void gimp_dialog_constructed(GObject *object) {
  ...
  if (private->help_func)
    gimp_help_connect(GTK_WIDGET(object), private->help_func, private->help_id,
                      object, NULL);

  if (show_help_button && private->help_func && private->help_id) {
  private
    ->help_button =
        gtk_dialog_add_button(GTK_DIALOG(object), ("_Help"), GTK_RESPONSE_HELP);
  }
}
...
static void gimp_dialog_set_property(GObject *object, guint property_id,
  ...
  case PROP_HELP_FUNC:
  private
    ->help_func = g_value_get_pointer(value);
    break;
  ...
  case PROP_PARENT: {
    ...
    if (parent) {
      if (GTK_IS_WINDOW(parent)) {
        gtk_window_set_transient_for(GTK_WINDOW(object), GTK_WINDOW(parent));
      } else {
        gtk_window_set_screen(GTK_WINDOW(object),
                              gtk_widget_get_screen(parent));
        gtk_window_set_position(GTK_WINDOW(object), GTK_WIN_POS_MOUSE);
      }
    }
  } break;
...
static void gimp_dialog_response(GtkDialog *dialog, gint response_id) {
  ...
  if (widget &&
      (!GTK_IS_BUTTON(widget) || gtk_widget_get_focus_on_click(widget))) {
   ...
   */
  if (response_id == GTK_RESPONSE_HELP && widget == private->help_button) {
    g_signal_stop_emission_by_name(dialog, "response");

    if (private->help_func)
    private
    ->help_func(private->help_id, dialog);
  }
}
...
GtkWidget *gimp_dialog_new(const gchar *title, const gchar *role,
  ...
  dialog = gimp_dialog_new_valist(title, role, parent, flags, help_func,
                                  help_id, args);
 ...
 * Returns: A #GimpDialog.
 **/
GtkWidget *gimp_dialog_new_valist(const gchar *title, const gchar *role,
                                  GtkWidget *parent, GtkDialogFlags flags,
                                  GimpHelpFunc help_func, const gchar *help_id,
                                  va_list args) {
  GtkWidget *dialog;
  ...
  dialog = g_object_new(GIMP_TYPE_DIALOG, "title", title, "role", role, "modal",
                        (flags & GTK_DIALOG_MODAL), "help-func", help_func,
                        "help-id", help_id, "parent", parent, "use-header-bar",
                        use_header_bar, NULL);

  if (parent) {
    if (flags & GTK_DIALOG_DESTROY_WITH_PARENT)
      g_signal_connect_object(parent, "destroy", G_CALLBACK(gimp_dialog_close),
 ...
 **/
GtkWidget *gimp_dialog_add_button(GimpDialog *dialog, const gchar *button_text,
                                  gint response_id) {
  GtkWidget *button;
  gboolean use_header_bar;
  ...
  g_object_get(dialog, "use-header-bar", &use_header_bar, NULL);

  if (use_header_bar &&
      (response_id == GTK_RESPONSE_OK || response_id == GTK_RESPONSE_CANCEL ||
    ...
    gtk_container_child_set(GTK_CONTAINER(header), button, "position", 0, NULL);
  }
...
static void run_shutdown_loop(RunInfo *ri) {
  if (g_main_loop_is_running(ri->loop))
    g_main_loop_quit(ri->loop);
}

static void run_unmap_handler(GtkDialog *dialog, RunInfo *ri) {
...
static void run_destroy_handler(GtkDialog *dialog, RunInfo *ri) {
 ...
 * Since: 3.0
 **/
void gimp_dialog_set_alternative_button_order_from_array(GimpDialog *dialog,
                                                         gint n_buttons,
                                                         gint *order) {
  ...
  gtk_dialog_set_alternative_button_order_from_array(GTK_DIALOG(dialog),
                                                     n_buttons, order);
