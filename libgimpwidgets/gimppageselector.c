...
enum { SELECTION_CHANGED, ACTIVATE, LAST_SIGNAL };

enum { PROP_0, PROP_N_PAGES, PROP_TARGET };

enum { COLUMN_PAGE_NO, COLUMN_THUMBNAIL, COLUMN_LABEL, COLUMN_LABEL_SET };
...
static void gimp_page_selector_get_property(GObject *object, guint property_id,
                                            GValue *value, GParamSpec *pspec);
static void gimp_page_selector_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
...
static void gimp_page_selector_range_activate(GtkEntry *entry,
                                              GimpPageSelector *selector);
static gint gimp_page_selector_int_compare(gconstpointer a, gconstpointer b);
static void gimp_page_selector_print_range(GString *string, gint start,
                                           gint end);
...
static void gimp_page_selector_class_init(GimpPageSelectorClass *klass) {
   ...
   * Since: 2.4
   **/
  selector_signals[SELECTION_CHANGED] = g_signal_new(
      "selection-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpPageSelectorClass, selection_changed), NULL, NULL,
      NULL, G_TYPE_NONE, 0);
  ...
  selector_signals[ACTIVATE] =
      g_signal_new("activate", G_OBJECT_CLASS_TYPE(object_class),
                   G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(GimpPageSelectorClass, activate), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      object_class, PROP_N_PAGES,
      g_param_spec_int("n-pages", "N Pages", "The number of pages to open", 0,
                       G_MAXINT, 0, GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("target", "Target", "the target to open to",
...
static void gimp_page_selector_init(GimpPageSelector *selector) {
  ...
  priv->store = gtk_list_store_new(4, G_TYPE_INT, GDK_TYPE_PIXBUF,
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(priv->view), renderer,
                                 "pixbuf", COLUMN_THUMBNAIL, NULL);

  renderer = gtk_cell_renderer_text_new();
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(priv->view), renderer, FALSE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(priv->view), renderer, "text",
                                 COLUMN_LABEL, NULL);
  ...
  g_signal_connect(priv->view, "selection-changed",
                   G_CALLBACK(gimp_page_selector_selection_changed), selector);
  g_signal_connect(priv->view, "item-activated",
                   G_CALLBACK(gimp_page_selector_item_activated), selector);

  /*  Count label  */

  priv->count_label = gtk_label_new(_("Nothing selected"));
  gtk_label_set_xalign(GTK_LABEL(priv->count_label), 0.0);
  gimp_label_set_attributes(GTK_LABEL(priv->count_label), PANGO_ATTR_STYLE,
  ...
  g_signal_connect_swapped(button, "clicked",
                           G_CALLBACK(gimp_page_selector_select_all), selector);
  ...
  g_signal_connect(priv->range_entry, "focus-out-event",
                   G_CALLBACK(gimp_page_selector_range_focus_out), selector);
  g_signal_connect(priv->range_entry, "activate",
                   G_CALLBACK(gimp_page_selector_range_activate), selector);
  ...
  priv->default_thumbnail = gtk_icon_theme_load_icon(
      gtk_icon_theme_get_default(), "text-x-generic", 32, 0, NULL);
}
  ...
  if (n_pages != priv->n_pages) {
    ...
    if (n_pages < priv->n_pages) {
      for (i = n_pages; i < priv->n_pages; i++) {
        gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(priv->store), &iter, NULL,
                                      n_pages);
      ...
      for (i = priv->n_pages; i < n_pages; i++) {
        ...
        gtk_list_store_set(priv->store, &iter, COLUMN_PAGE_NO, i,
                           COLUMN_THUMBNAIL, priv->default_thumbnail,
                           COLUMN_LABEL, text, COLUMN_LABEL_SET, FALSE, -1);
...
void gimp_page_selector_set_page_thumbnail(GimpPageSelector *selector,
  ...
  gtk_list_store_set(priv->store, &iter, COLUMN_THUMBNAIL, thumbnail, -1);
...
GdkPixbuf *gimp_page_selector_get_page_thumbnail(GimpPageSelector *selector,
  ...
  gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(priv->store), &iter, NULL,
                                page_no);
  gtk_tree_model_get(GTK_TREE_MODEL(priv->store), &iter, COLUMN_THUMBNAIL,
                     &thumbnail, -1);
 ...
 * Since: 2.4
 **/
void gimp_page_selector_set_page_label(GimpPageSelector *selector, gint page_no,
                                       const gchar *label) {
  GimpPageSelectorPrivate *priv;
  GtkTreeIter iter;
  gchar *tmp;
  ...
  gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(priv->store), &iter, NULL,
                                page_no);
  gtk_list_store_set(priv->store, &iter, COLUMN_LABEL, tmp, COLUMN_LABEL_SET,
                     label != NULL, -1);
...
gchar *gimp_page_selector_get_page_label(GimpPageSelector *selector,
  ...
  gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(priv->store), &iter, NULL,
                                page_no);
  gtk_tree_model_get(GTK_TREE_MODEL(priv->store), &iter, COLUMN_LABEL, &label,
                     COLUMN_LABEL_SET, &label_set, -1);
...
gboolean gimp_page_selector_page_is_selected(GimpPageSelector *selector,
  ...
  selected = gtk_icon_view_path_is_selected(GTK_ICON_VIEW(priv->view), path);
...
gint *gimp_page_selector_get_selected_pages(GimpPageSelector *selector,
  ...
  *n_selected_pages = g_list_length(selected);
  ...
  qsort(array, *n_selected_pages, sizeof(gint), gimp_page_selector_int_compare);
...
void gimp_page_selector_select_range(GimpPageSelector *selector,
  ...
  g_signal_handlers_block_by_func(
      priv->view, gimp_page_selector_selection_changed, selector);
    ...
    for (i = 0; ranges[i] != NULL; i++) {
      ...
      if (dash) {
        ...
        *dash = '\0';
        ...
        if (page_from > 0 && page_to > 0 && page_from <= page_to &&
        ...
        if (sscanf(range, "%i", &page_no) == 1 && page_no >= 1 &&
...
static void gimp_page_selector_selection_changed(GtkIconView *icon_view,
  ...
  if (n_selected == 0) {
    gtk_label_set_text(GTK_LABEL(priv->count_label), _("Nothing selected"));
  } else if (n_selected == 1) {
    gtk_label_set_text(GTK_LABEL(priv->count_label), _("One page selected"));
  } else {
    gchar *text;

    if (n_selected == priv->n_pages)
      text = g_strdup_printf(
          ngettext("%d page selected", "All %d pages selected", n_selected),
          n_selected);
    else
      text = g_strdup_printf(
          ngettext("%d page selected", "%d pages selected", n_selected),
          n_selected);
...
static void draw_frame_row(GdkPixbuf *frame_image, gint target_width,
                           gint source_width, gint source_v_position,
                           gint dest_v_position, GdkPixbuf *result_pixbuf,
...
static void draw_frame_column(GdkPixbuf *frame_image, gint target_height,
                              gint source_height, gint source_h_position,
                              gint dest_h_position, GdkPixbuf *result_pixbuf,
...
static GdkPixbuf *stretch_frame_image(GdkPixbuf *frame_image, gint left_offset,
                                      gint top_offset, gint right_offset,
                                      gint bottom_offset, gint dest_width,
                                      gint dest_height) {
  GdkPixbuf *pixbuf;
  ...
  pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, dest_width, dest_height);
  ...
  gdk_pixbuf_copy_area(frame_image, 0, 0, left_offset, top_offset, pixbuf, 0,
                       0);
  draw_frame_row(frame_image, target_width, target_frame_width, 0, 0, pixbuf,
  ...
  gdk_pixbuf_copy_area(frame_image, frame_width - right_offset, 0, right_offset,
                       ...
                       pixbuf, dest_width - right_offset, 0);
  ...
  gdk_pixbuf_copy_area(frame_image, frame_width - right_offset,
                       ...
                       bottom_offset, pixbuf, dest_width - right_offset,
  ...
  gdk_pixbuf_copy_area(frame_image, 0, frame_height - bottom_offset,
                       left_offset, bottom_offset, pixbuf, 0,
                       dest_height - bottom_offset);
...
static GdkPixbuf *gimp_page_selector_add_frame(GtkWidget *widget,
  ...
  frame = stretch_frame_image(frame, FRAME_LEFT, FRAME_TOP, FRAME_RIGHT,
                              FRAME_BOTTOM, width + FRAME_LEFT + FRAME_RIGHT,
                              height + FRAME_TOP + FRAME_BOTTOM);

  gdk_pixbuf_copy_area(pixbuf, 0, 0, width, height, frame, FRAME_LEFT,
                       FRAME_TOP);
