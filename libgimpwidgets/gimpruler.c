...
static const RulerMetric ruler_metric_decimal = {{1, 2, 5, 10, 25, 50, 100, 250,
...
static const RulerMetric ruler_metric_inches = {
    ...
    {1, 2, 6, 12, 36, 72, 100, 250, 500, 1000, 2500, 5000, 10000, 25000, 50000,
     100000},
...
static const RulerMetric ruler_metric_feet = {
    ...
    {1, 3, 6, 12, 36, 72, 100, 250, 500, 1000, 2500, 5000, 10000, 25000, 50000,
     100000},
...
static void gimp_ruler_set_property(GObject *object, guint prop_id,
                                    const GValue *value, GParamSpec *pspec);
static void gimp_ruler_get_property(GObject *object, guint prop_id,
...
static gboolean gimp_ruler_draw(GtkWidget *widget, cairo_t *cr);

static void gimp_ruler_draw_ticks(GimpRuler *ruler);
static GdkRectangle gimp_ruler_get_pos_rect(GimpRuler *ruler, gdouble position);
static gboolean gimp_ruler_idle_queue_pos_redraw(gpointer data);
static void gimp_ruler_queue_pos_redraw(GimpRuler *ruler);
static void gimp_ruler_draw_pos(GimpRuler *ruler, cairo_t *cr);
static void gimp_ruler_make_pixmap(GimpRuler *ruler);
static PangoLayout *gimp_ruler_get_layout(GtkWidget *widget, const gchar *text);
static const RulerMetric *gimp_ruler_get_metric(GimpUnit unit);

G_DEFINE_TYPE_WITH_PRIVATE(GimpRuler, gimp_ruler, GTK_TYPE_WIDGET)

#define parent_class gimp_ruler_parent_class

static void gimp_ruler_class_init(GimpRulerClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("orientation", "Orientation",
                        "The orientation of the ruler", GTK_TYPE_ORIENTATION,
                        GTK_ORIENTATION_HORIZONTAL, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_UNIT,
      gimp_param_spec_unit("unit", "Unit", "Unit of ruler", TRUE, TRUE,
                           GIMP_UNIT_PIXEL, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_LOWER,
      g_param_spec_double("lower", "Lower", "Lower limit of ruler",
                          -G_MAXDOUBLE, G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_UPPER,
      g_param_spec_double("upper", "Upper", "Upper limit of ruler",
                          -G_MAXDOUBLE, G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_POSITION,
      g_param_spec_double("position", "Position",
                          "Position of mark on the ruler", -G_MAXDOUBLE,
                          G_MAXDOUBLE, 0.0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_MAX_SIZE,
      g_param_spec_double("max-size", "Max Size", "Maximum size of the ruler",
                          -G_MAXDOUBLE, G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE));
...
static void gimp_ruler_init(GimpRuler *ruler) {
...
GtkWidget *gimp_ruler_new(GtkOrientation orientation) {
  return g_object_new(GIMP_TYPE_RULER, "orientation", orientation, NULL);
}

static void gimp_ruler_update_position(GimpRuler *ruler, gdouble x, gdouble y) {
  GimpRulerPrivate *priv = GET_PRIVATE(ruler);
  GtkAllocation allocation;
  gdouble lower;
  gdouble upper;
  ...
  if (priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
    gimp_ruler_set_position(ruler,
                            lower + (upper - lower) * x / allocation.width);
  } else {
    gimp_ruler_set_position(ruler,
                            lower + (upper - lower) * y / allocation.height);
  }
}
...
static gboolean gtk_widget_get_translation_to_window(GtkWidget *widget,
  ...
  for (w = window; w && w != widget_window;
...
static void gimp_ruler_event_to_widget_coords(GtkWidget *widget,
                                              GdkWindow *window,
...
static gboolean gimp_ruler_track_widget_motion_notify(GtkWidget *widget,
  ...
  gimp_ruler_event_to_widget_coords(widget, mevent->window, mevent->x,
                                    mevent->y, &widget_x, &widget_y);

  if (gtk_widget_translate_coordinates(widget, GTK_WIDGET(ruler), widget_x,
                                       widget_y, &ruler_x, &ruler_y)) {
...
void gimp_ruler_add_track_widget(GimpRuler *ruler, GtkWidget *widget) {
  ...
  g_signal_connect(widget, "motion-notify-event",
                   G_CALLBACK(gimp_ruler_track_widget_motion_notify), ruler);
  g_signal_connect_swapped(widget, "destroy",
                           G_CALLBACK(gimp_ruler_remove_track_widget), ruler);
}
...
void gimp_ruler_remove_track_widget(GimpRuler *ruler, GtkWidget *widget) {
  ...
  g_signal_handlers_disconnect_by_func(
      widget, gimp_ruler_track_widget_motion_notify, ruler);
  g_signal_handlers_disconnect_by_func(widget, gimp_ruler_remove_track_widget,
                                       ruler);
}
...
void gimp_ruler_set_position(GimpRuler *ruler, gdouble position) {
  ...
  if (priv->position != position) {
    ...
    if (priv->last_pos_rect.width != 0 && priv->last_pos_rect.height != 0 &&
      ...
      priv->pos_redraw_idle_id = g_idle_add_full(
          G_PRIORITY_LOW, gimp_ruler_idle_queue_pos_redraw, ruler, NULL);
    }
  }
}
...
static void gimp_ruler_size_allocate(GtkWidget *widget,
  ...
  if (gtk_widget_get_realized(widget)) {
    gdk_window_move_resize(priv->input_window, allocation->x, allocation->y,
...
static void gimp_ruler_draw_ticks(GimpRuler *ruler) {
  ...
  if (priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
    cairo_rectangle(cr, border.left, height + border.top,
                    allocation.width - (border.left + border.right), 1);
  } else {
    cairo_rectangle(cr, height + border.left, border.top, 1,
  ...
  for (i = G_N_ELEMENTS(ruler_metric->subdivide) - 1; i >= 0; i--) {
    ...
    for (cur = start; cur <= end; cur += subd_incr) {
      ...
      if (priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
        cairo_rectangle(cr, pos, height + border.top - length, 1, length);
      } else {
        cairo_rectangle(cr, height + border.left - length, pos, length, 1);
      }
      ...
      if (i == 0) {
        ...
        if (priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
          ...
          for (j = 0; j < (int)strlen(unit_str); j++) {
            ...
            cairo_move_to(cr, border.left + 1,
...
static GdkRectangle gimp_ruler_get_pos_rect(GimpRuler *ruler,
  ...
  if (priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
    ...
    rect.x = ROUND((position - lower) * increment) +
             (border.left + border.right - rect.width) / 2 - 1;
    rect.y = (height + rect.height) / 2 + border.top;
  } else {
    increment = (gdouble)height / (upper - lower);

    rect.x = (width + rect.width) / 2 + border.left;
    rect.y = ROUND((position - lower) * increment) +
             (border.top + border.bottom - rect.height) / 2 - 1;
  }
...
static void gimp_ruler_queue_pos_redraw(GimpRuler *ruler) {
  ...
  gtk_widget_queue_draw_area(GTK_WIDGET(ruler), rect.x + allocation.x,
                             rect.y + allocation.y, rect.width, rect.height);

  if (priv->last_pos_rect.width != 0 && priv->last_pos_rect.height != 0) {
    gtk_widget_queue_draw_area(
        GTK_WIDGET(ruler), priv->last_pos_rect.x + allocation.x,
        priv->last_pos_rect.y + allocation.y, priv->last_pos_rect.width,
...
static void gimp_ruler_draw_pos(GimpRuler *ruler, cairo_t *cr) {
  ...
  if ((pos_rect.width > 0) && (pos_rect.height > 0)) {
    ...
    if (priv->orientation == GTK_ORIENTATION_HORIZONTAL) {
      ...
      cairo_line_to(cr, pos_rect.x + pos_rect.width, pos_rect.y);
    } else {
      cairo_line_to(cr, pos_rect.x + pos_rect.width,
                    pos_rect.y + pos_rect.height / 2.0);
      cairo_line_to(cr, pos_rect.x, pos_rect.y + pos_rect.height);
    }
  ...
  if (priv->last_pos_rect.width != 0 && priv->last_pos_rect.height != 0) {
    gdk_rectangle_union(&priv->last_pos_rect, &pos_rect, &priv->last_pos_rect);
  } else {
...
static void gimp_ruler_make_pixmap(GimpRuler *ruler) {
  ...
  priv->backing_store = gdk_window_create_similar_surface(
