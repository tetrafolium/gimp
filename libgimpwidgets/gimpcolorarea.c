...
enum { PROP_0, PROP_COLOR, PROP_TYPE, PROP_DRAG_MASK, PROP_DRAW_BORDER };
...
static void gimp_color_area_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);
static void gimp_color_area_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);

static void gimp_color_area_size_allocate(GtkWidget *widget,
                                          GtkAllocation *allocation);
static gboolean gimp_color_area_draw(GtkWidget *widget, cairo_t *cr);
static void gimp_color_area_render_buf(GtkWidget *widget,
                                       GimpColorAreaType type, guchar *buf,
                                       guint width, guint height,
                                       guint rowstride, GimpRGB *color);
...
static void gimp_color_area_class_init(GimpColorAreaClass *klass) {
  ...
  gimp_color_area_signals[COLOR_CHANGED] = g_signal_new(
      "color-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpColorAreaClass, color_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      object_class, PROP_COLOR,
      gimp_param_spec_rgb("color", "Color", "The displayed color", TRUE, &color,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("type", "Type", "The type of the color area",
                        GIMP_TYPE_COLOR_AREA_TYPE, GIMP_COLOR_AREA_FLAT,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  /**
   ...
   */
  g_object_class_install_property(
      object_class, PROP_DRAG_MASK,
      g_param_spec_flags("drag-mask", "Drag Mask",
   ...
   */
  g_object_class_install_property(
      object_class, PROP_DRAW_BORDER,
      g_param_spec_boolean("draw-border", "Draw Border",
...
static void gimp_color_area_init(GimpColorArea *area) {
  ...
  gimp_widget_track_monitor(GTK_WIDGET(area),
                            G_CALLBACK(gimp_color_area_destroy_transform), NULL,
                            NULL);
}
...
static void gimp_color_area_set_property(GObject *object, guint property_id,
  ...
  case PROP_DRAG_MASK:
    drag_mask = g_value_get_flags(value) &
                (GDK_BUTTON1_MASK | GDK_BUTTON2_MASK | GDK_BUTTON3_MASK);
    if (drag_mask)
      gtk_drag_source_set(GTK_WIDGET(area), drag_mask, &target, 1,
...
static void gimp_color_area_size_allocate(GtkWidget *widget,
  ...
  if (allocation->width != priv->width || allocation->height != priv->height) {
...
static gboolean gimp_color_area_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (priv->transform) {
    ...
    for (i = 0; i < priv->height; i++) {
      gimp_color_transform_process_pixels(priv->transform, format, src, format,
                                          dest, priv->width);
    ...
    buffer = cairo_image_surface_create_for_data(
        buf, CAIRO_FORMAT_RGB24, priv->width, priv->height, priv->rowstride);
    cairo_surface_set_user_data(buffer, NULL, buf,
                                (cairo_destroy_func_t)g_free);
  } else {
    buffer = cairo_image_surface_create_for_data(priv->buf, CAIRO_FORMAT_RGB24,
  ...
  if (!gtk_widget_is_sensitive(widget)) {
    ...
    if (!pattern) {
      static const guchar stipple[] = {0, 255, 0, 0, 255, 0, 0, 0};
      ...
      surface = cairo_image_surface_create_for_data(
          (guchar *)stipple, CAIRO_FORMAT_A8, 2, 2, stride);
  ...
  if (priv->config &&
      ((priv->color.r < 0.0 || priv->color.r > 1.0 || priv->color.g < 0.0 ||
        priv->color.g > 1.0 || priv->color.b < 0.0 || priv->color.b > 1.0) ||
       priv->out_of_gamut)) {
    GimpRGB color;
 ...
 **/
GtkWidget *gimp_color_area_new(const GimpRGB *color, GimpColorAreaType type,
                               GdkModifierType drag_mask) {
  return g_object_new(GIMP_TYPE_COLOR_AREA, "color", color, "type", type,
                      "drag-mask", drag_mask, NULL);
}
  ...
  if (config != priv->config) {
    if (priv->config) {
      g_signal_handlers_disconnect_by_func(
          priv->config, gimp_color_area_destroy_transform, area);
...
static void gimp_color_area_render_buf(GtkWidget *widget,
  ...
  if (check_size == 0 || color->a == 1.0) {
    for (y = 0; y < height; y++) {
      ...
      for (x = 0; x < width; x++) {
        GIMP_CAIRO_RGB24_SET_PIXEL(p, opaque[0], opaque[1], opaque[2]);
  ...
  light[0] =
      (GIMP_CHECK_LIGHT + (color->r - GIMP_CHECK_LIGHT) * color->a) * 255.999;
  light[1] =
      (GIMP_CHECK_LIGHT + (color->g - GIMP_CHECK_LIGHT) * color->a) * 255.999;
  light[2] =
      (GIMP_CHECK_LIGHT + (color->b - GIMP_CHECK_LIGHT) * color->a) * 255.999;

  dark[0] =
      (GIMP_CHECK_DARK + (color->r - GIMP_CHECK_DARK) * color->a) * 255.999;
  dark[1] =
      (GIMP_CHECK_DARK + (color->g - GIMP_CHECK_DARK) * color->a) * 255.999;
  dark[2] =
      (GIMP_CHECK_DARK + (color->b - GIMP_CHECK_DARK) * color->a) * 255.999;
  ...
  for (y = 0; y < height; y++) {
    ...
    for (x = 0; x < width; x++) {
      ...
      if (((x / check_size) ^ (y / check_size)) & 1) {
        if ((gint)frac) {
          GIMP_CAIRO_RGB24_SET_PIXEL(p, light[0], light[1], light[2]);
        } else {
          GIMP_CAIRO_RGB24_SET_PIXEL(
              p, ((gdouble)light[0] * frac + (gdouble)opaque[0] * (1.0 - frac)),
              ((gdouble)light[1] * frac + (gdouble)opaque[1] * (1.0 - frac)),
              ((gdouble)light[2] * frac + (gdouble)opaque[2] * (1.0 - frac)));
        }
      } else {
        if ((gint)frac) {
          GIMP_CAIRO_RGB24_SET_PIXEL(p, dark[0], dark[1], dark[2]);
        } else {
          GIMP_CAIRO_RGB24_SET_PIXEL(
              p, ((gdouble)dark[0] * frac + (gdouble)opaque[0] * (1.0 - frac)),
              ((gdouble)dark[1] * frac + (gdouble)opaque[1] * (1.0 - frac)),
              ((gdouble)dark[2] * frac + (gdouble)opaque[2] * (1.0 - frac)));
        }
      }
...
static void gimp_color_area_render(GimpColorArea *area) {
  ...
  gimp_color_area_render_buf(GTK_WIDGET(area), priv->type, priv->buf,
...
static void gimp_color_area_drag_begin(GtkWidget *widget,
  ...
  g_object_set_data_full(G_OBJECT(widget), "gimp-color-area-drag-window",
...
static void gimp_color_area_drag_end(GtkWidget *widget,
                                     GdkDragContext *context) {
  g_object_set_data(G_OBJECT(widget), "gimp-color-area-drag-window", NULL);
}
...
static void gimp_color_area_drag_data_received(GtkWidget *widget,
  ...
  gimp_rgba_set(&color, (gdouble)vals[0] / 0xffff, (gdouble)vals[1] / 0xffff,
                (gdouble)vals[2] / 0xffff, (gdouble)vals[3] / 0xffff);
...
static void gimp_color_area_create_transform(GimpColorArea *area) {
  ...
  if (priv->config) {
    ...
    priv->transform = gimp_widget_get_color_transform(
        GTK_WIDGET(area), priv->config, profile, format, format);
  }
}
