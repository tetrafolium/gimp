...
enum { PROP_0, PROP_COLOR, PROP_OPAQUE, PROP_SIZE };
...
static void gimp_cell_renderer_color_render(GtkCellRenderer *cell, cairo_t *cr,
                                            GtkWidget *widget,
...
gimp_cell_renderer_color_class_init(GimpCellRendererColorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boxed("color", "Color", "The displayed color", GIMP_TYPE_RGB,
                         GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_OPAQUE,
      g_param_spec_boolean("opaque", "Opaque", "Whether to show transparency",
                           TRUE, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_SIZE,
      g_param_spec_int("icon-size", "Icon Size", "The cell's size", 0, G_MAXINT,
                       DEFAULT_ICON_SIZE,
...
static void gimp_cell_renderer_color_set_property(GObject *object,
  ...
  case PROP_OPAQUE:
  private
    ->opaque = g_value_get_boolean(value);
    break;
  case PROP_SIZE:
  private
...
static void gimp_cell_renderer_color_render(GtkCellRenderer *cell, cairo_t *cr,
  ...
  gimp_cell_renderer_color_get_size(cell, widget, cell_area, &rect.x, &rect.y,
  ...
  if (rect.width > 2 && rect.height > 2) {
    ...
    cairo_rectangle(cr, rect.x + 1, rect.y + 1, rect.width - 2,
                    rect.height - 2);
    ...
    if (!private->opaque && private->color.a < 1.0) {
      ...
      pattern =
          gimp_cairo_checkerboard_create(cr, GIMP_CHECK_SIZE_SM, NULL, NULL);
    ...
    /* draw border */
    cairo_rectangle(cr, rect.x + 0.5, rect.y + 0.5, rect.width - 1,
                    rect.height - 1);
