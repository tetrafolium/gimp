...
static void gimp_scrolled_preview_area_size_allocate(
    GtkWidget *widget, GtkAllocation *allocation, GimpScrolledPreview *preview);
...
static gboolean
gimp_scrolled_preview_nav_button_press(GtkWidget *widget, GdkEventButton *event,
                                       GimpScrolledPreview *preview);

static gboolean
gimp_scrolled_preview_nav_popup_event(GtkWidget *widget, GdkEvent *event,
                                      GimpScrolledPreview *preview);
static gboolean
gimp_scrolled_preview_nav_popup_draw(GtkWidget *widget, cairo_t *cr,
                                     GimpScrolledPreview *preview);
...
static void gimp_scrolled_preview_init(GimpScrolledPreview *preview) {
  ...
  g_signal_connect(adj, "value-changed",
                   G_CALLBACK(gimp_scrolled_preview_h_scroll), preview);
  ...
  g_signal_connect(adj, "value-changed",
                   G_CALLBACK(gimp_scrolled_preview_v_scroll), preview);
  ...
  g_signal_connect_after(area, "event",
                         G_CALLBACK(gimp_scrolled_preview_area_event), preview);

  g_signal_connect(area, "realize",
                   G_CALLBACK(gimp_scrolled_preview_area_realize), preview);
  g_signal_connect(area, "unrealize",
                   G_CALLBACK(gimp_scrolled_preview_area_unrealize), preview);
  ...
  g_signal_connect(priv->nav_icon, "button-press-event",
                   G_CALLBACK(gimp_scrolled_preview_nav_button_press), preview);
...
static void gimp_scrolled_preview_hscr_update(GimpScrolledPreview *preview) {
  GimpScrolledPreviewPrivate *priv = GET_PRIVATE(preview);
  GtkAdjustment *adj;
  gint xmin, xmax;
  gint width;
  ...
  gtk_adjustment_configure(adj, gtk_adjustment_get_value(adj), 0, xmax - xmin,
                           1.0, MAX(width / 2.0, 1.0), width);
}

static void gimp_scrolled_preview_vscr_update(GimpScrolledPreview *preview) {
  GimpScrolledPreviewPrivate *priv = GET_PRIVATE(preview);
  GtkAdjustment *adj;
  gint ymin, ymax;
  gint height;
  ...
  gtk_adjustment_configure(adj, gtk_adjustment_get_value(adj), 0, ymax - ymin,
                           1.0, MAX(height / 2.0, 1.0), height);
}
...
static gboolean gimp_scrolled_preview_area_event(GtkWidget *area,
  ...
  case GDK_BUTTON_PRESS:
    ...
    case 2:
      cursor =
          gdk_cursor_new_for_display(gtk_widget_get_display(area), GDK_FLEUR);

      if (gdk_seat_grab(seat, gtk_widget_get_window(area),
                        GDK_SEAT_CAPABILITY_ALL_POINTING, TRUE, cursor, event,
                        NULL, NULL) == GDK_GRAB_SUCCESS) {
        gdk_window_get_device_position(gtk_widget_get_window(area),
                                       gdk_event_get_device(event),
                                       &priv->drag_x, &priv->drag_y, NULL);
  ...
  case GDK_MOTION_NOTIFY:
    if (priv->in_drag) {
      ...
      x = CLAMP(x, gtk_adjustment_get_lower(hadj),
                gtk_adjustment_get_upper(hadj) -
                    gtk_adjustment_get_page_size(hadj));
      y = CLAMP(y, gtk_adjustment_get_lower(vadj),
  ...
  case GDK_SCROLL: {
    ...
    gimp_scroll_adjustment_values(sevent, adj_x, adj_y, &value_x, &value_y);
...
gimp_scrolled_preview_nav_button_press(GtkWidget *widget, GdkEventButton *event,
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
    ...
    g_object_get(gimp_preview_get_area(gimp_preview), "check-type", &check_type,
                 NULL);

    area = g_object_new(GIMP_TYPE_PREVIEW_AREA, "check-size",
                        GIMP_CHECK_SIZE_SMALL_CHECKS, "check-type", check_type,
                        NULL);
    ...
    GIMP_PREVIEW_GET_CLASS(preview)->draw_thumb(
        gimp_preview, GIMP_PREVIEW_AREA(area), POPUP_SIZE, POPUP_SIZE);
    ...
    adj = gtk_range_get_adjustment(GTK_RANGE(priv->hscr));
    h = ((gtk_adjustment_get_value(adj) / gtk_adjustment_get_upper(adj)) +
         (gtk_adjustment_get_page_size(adj) / gtk_adjustment_get_upper(adj)) /
             2.0);

    adj = gtk_range_get_adjustment(GTK_RANGE(priv->vscr));
    v = ((gtk_adjustment_get_value(adj) / gtk_adjustment_get_upper(adj)) +
         (gtk_adjustment_get_page_size(adj) / gtk_adjustment_get_upper(adj)) /
             2.0);

    gimp_preview_area_get_size(GIMP_PREVIEW_AREA(area), &area_width,
                               &area_height);
    ...
    cursor =
        gdk_cursor_new_for_display(gtk_widget_get_display(widget), GDK_FLEUR);

    gdk_seat_grab(gdk_display_get_default_seat(gtk_widget_get_display(widget)),
                  gtk_widget_get_window(area), GDK_SEAT_CAPABILITY_ALL_POINTING,
                  TRUE, cursor, (GdkEvent *)event, NULL, NULL);
...
gimp_scrolled_preview_nav_popup_event(GtkWidget *widget, GdkEvent *event,
  ...
  case GDK_MOTION_NOTIFY: {
    ...
    gdk_window_get_device_position(gtk_widget_get_window(widget),
                                   gdk_event_get_device(event), &cx, &cy, NULL);

    x = cx * (gtk_adjustment_get_upper(hadj) - gtk_adjustment_get_lower(hadj)) /
        allocation.width;
    y = cy * (gtk_adjustment_get_upper(vadj) - gtk_adjustment_get_lower(vadj)) /
        allocation.height;
    ...
    gtk_adjustment_set_value(hadj,
                             CLAMP(x, gtk_adjustment_get_lower(hadj),
                                   gtk_adjustment_get_upper(hadj) -
                                       gtk_adjustment_get_page_size(hadj)));
    gtk_adjustment_set_value(vadj,
                             CLAMP(y, gtk_adjustment_get_lower(vadj),
                                   gtk_adjustment_get_upper(vadj) -
                                       gtk_adjustment_get_page_size(vadj)));
...
static void gimp_scrolled_preview_set_cursor(GimpPreview *preview) {
  ...
  if (xmax - xmin > width || ymax - ymin > height) {
    gdk_window_set_cursor(gtk_widget_get_window(area), priv->cursor_move);
  } else {
 ...
 * Since: 2.4
 **/
void gimp_scrolled_preview_set_position(GimpScrolledPreview *preview, gint x,
                                        gint y) {
  GimpScrolledPreviewPrivate *priv;
  GtkAdjustment *adj;
  gint xmin, ymin;
 ...
 */
void gimp_scroll_adjustment_values(GdkEventScroll *sevent, GtkAdjustment *hadj,
                                   GtkAdjustment *vadj, gdouble *hvalue,
                                   gdouble *vvalue) {
  GtkAdjustment *adj_x;
  GtkAdjustment *adj_y;
  ...
  if (adj_x)
    value_x = CLAMP(value_x + gtk_adjustment_get_value(adj_x),
                    gtk_adjustment_get_lower(adj_x),
                    gtk_adjustment_get_upper(adj_x) -
                        gtk_adjustment_get_page_size(adj_x));

  if (adj_y)
    value_y = CLAMP(value_y + gtk_adjustment_get_value(adj_y),
