...
static void gimp_browser_class_init(GimpBrowserClass *klass) {
  ...
  browser_signals[SEARCH] =
      g_signal_new("search", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpBrowserClass, search), NULL, NULL,
                   _gimp_widgets_marshal_VOID__STRING_INT, G_TYPE_NONE, 2,
...
static void gimp_browser_init(GimpBrowser *browser) {
  ...
  g_signal_connect(priv->search_entry, "changed",
                   G_CALLBACK(gimp_browser_entry_changed), browser);
  ...
  g_signal_connect(priv->search_entry, "icon-press",
                   G_CALLBACK(gimp_browser_entry_icon_press), browser);
  ...
  gtk_box_pack_end(GTK_BOX(priv->left_vbox), priv->count_label, FALSE, FALSE,
                   0);
...
void gimp_browser_add_search_types(GimpBrowser *browser,
  ...
  if (!priv->search_type_combo) {
    ...
    combo =
        gimp_int_combo_box_new_valist(first_type_label, first_type_id, args);
    va_end(args);
    ...
    gtk_box_pack_end(GTK_BOX(gtk_widget_get_parent(priv->search_entry)), combo,
                     FALSE, FALSE, 0);
    gtk_widget_show(combo);

    gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), priv->search_type,
                               G_CALLBACK(gimp_int_combo_box_get_active),
                               &priv->search_type, NULL);

    g_signal_connect(combo, "changed", G_CALLBACK(gimp_browser_combo_changed),
                     browser);
  } else {
    gimp_int_combo_box_append(GIMP_INT_COMBO_BOX(priv->search_type_combo),
                              first_type_label, first_type_id, NULL);
  }
}
 ...
 * Since: 2.4
 **/
void gimp_browser_set_widget(GimpBrowser *browser, GtkWidget *widget) {
  GimpBrowserPrivate *priv;
  ...
  if (priv->right_widget)
    gtk_container_remove(GTK_CONTAINER(priv->right_vbox), priv->right_widget);

  priv->right_widget = widget;

  if (widget) {
    gtk_box_pack_start(GTK_BOX(priv->right_vbox), widget, FALSE, FALSE, 0);
 ...
 * Since: 2.4
 **/
void gimp_browser_show_message(GimpBrowser *browser, const gchar *message) {
  GimpBrowserPrivate *priv;
...
static gboolean gimp_browser_search_timeout(gpointer data) {
  ...
  g_signal_emit(data, browser_signals[SEARCH], 0, search_string,
                priv->search_type);
