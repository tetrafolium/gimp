...
enum { VALUE_CHANGED, REFVAL_CHANGED, UNIT_CHANGED, LAST_SIGNAL };
...
static void gimp_size_entry_refval_callback(GtkAdjustment *adjustment,
                                            gpointer data);
static void gimp_size_entry_update_unit(GimpSizeEntry *gse, GimpUnit unit);
...
static void gimp_size_entry_class_init(GimpSizeEntryClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  gimp_size_entry_signals[VALUE_CHANGED] = g_signal_new(
      "value-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpSizeEntryClass, value_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_size_entry_signals[REFVAL_CHANGED] = g_signal_new(
      "refval-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpSizeEntryClass, refval_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_size_entry_signals[UNIT_CHANGED] =
      g_signal_new("unit-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpSizeEntryClass, unit_changed), NULL,
 ...
 **/
GtkWidget *gimp_size_entry_new(gint number_of_fields, GimpUnit unit,
                               const gchar *unit_format,
                               gboolean menu_show_pixels,
                               gboolean menu_show_percent, gboolean show_refval,
  ...
  for (i = 0; i < number_of_fields; i++) {
    ...
    digits = ((unit == GIMP_UNIT_PIXEL) ? gsef->refval_digits
                                        : ((unit == GIMP_UNIT_PERCENT)
                                               ? 2
                                               : GIMP_SIZE_ENTRY_DIGITS(unit)));

    gsef->value_adjustment = gtk_adjustment_new(
        gsef->value, gsef->min_value, gsef->max_value, 1.0, 10.0, 0.0);
    gsef->value_spinbutton =
        gimp_spin_button_new(gsef->value_adjustment, 1.0, digits);
    gtk_spin_button_set_numeric(GTK_SPIN_BUTTON(gsef->value_spinbutton), TRUE);

    gimp_size_entry_attach_eevl(GTK_SPIN_BUTTON(gsef->value_spinbutton), gsef);
    ...
    if (spinbutton_width > 0) {
      if (spinbutton_width < 17)
        ...
        gtk_widget_set_size_request(gsef->value_spinbutton, spinbutton_width,
                                    -1);
    }

    gtk_grid_attach(GTK_GRID(gse), gsef->value_spinbutton, i + 1,
                    priv->show_refval + 1, 1, 1);
    g_signal_connect(gsef->value_adjustment, "value-changed",
                     G_CALLBACK(gimp_size_entry_value_callback), gsef);

    gtk_widget_show(gsef->value_spinbutton);

    if (priv->show_refval) {
      gsef->refval_adjustment = gtk_adjustment_new(
          gsef->refval, gsef->min_refval, gsef->max_refval, 1.0, 10.0, 0.0);
      ...
      gtk_widget_set_size_request(gsef->refval_spinbutton, spinbutton_width,
                                  -1);
      gtk_grid_attach(GTK_GRID(gse), gsef->refval_spinbutton, i + 1, 1, 1, 1);
      g_signal_connect(gsef->refval_adjustment, "value-changed",
                       G_CALLBACK(gimp_size_entry_refval_callback), gsef);
  ...
  if (unit_format) {
    ...
    g_object_set(store, "short-format", short_format, "long-format",
                 unit_format, NULL);
  ...
  gtk_grid_attach(GTK_GRID(gse), priv->unit_combo, i + 2, priv->show_refval + 1,
                  1, 1);
  g_signal_connect(priv->unit_combo, "changed",
                   G_CALLBACK(gimp_size_entry_unit_callback), gse);
...
void gimp_size_entry_add_field(GimpSizeEntry *gse,
  ...
  digits = ((priv->unit == GIMP_UNIT_PIXEL)
                ? gsef->refval_digits
                : (priv->unit == GIMP_UNIT_PERCENT)
                      ? 2
                      : GIMP_SIZE_ENTRY_DIGITS(priv->unit));

  gtk_spin_button_set_digits(GTK_SPIN_BUTTON(value_spinbutton), digits);

  if (priv->menu_show_pixels && !priv->show_refval &&
      (priv->unit == GIMP_UNIT_PIXEL)) {
    gtk_spin_button_set_digits(GTK_SPIN_BUTTON(gsef->value_spinbutton),
                               gsef->refval_digits);
  }
 ...
 **/
GtkWidget *gimp_size_entry_attach_label(GimpSizeEntry *gse, const gchar *text,
  ...
  if (column == 0) {
    ...
    for (list = children; list; list = g_list_next(list)) {
      ...
      gtk_container_child_get(GTK_CONTAINER(gse), child, "left-attach",
 ...
 **/
void gimp_size_entry_set_resolution(GimpSizeEntry *gse, gint field,
                                    gdouble resolution, gboolean keep_size) {
  GimpSizeEntryPrivate *priv;
  GimpSizeEntryField *gsef;
  gfloat val;
  ...
  gsef->stop_recursion = 0;
  gimp_size_entry_set_refval_boundaries(gse, field, gsef->min_refval,
                                        gsef->max_refval);
 ...
 * gimp_size_entry_new() doesn't equal to GIMP_SIZE_ENTRY_UPDATE_SIZE.
 **/
void gimp_size_entry_set_size(GimpSizeEntry *gse, gint field, gdouble lower,
                              gdouble upper) {
  GimpSizeEntryPrivate *priv;
  GimpSizeEntryField *gsef;
 ...
 **/
void gimp_size_entry_set_value_boundaries(GimpSizeEntry *gse, gint field,
  ...
  case GIMP_SIZE_ENTRY_UPDATE_SIZE:
    ...
    case GIMP_UNIT_PIXEL:
      gimp_size_entry_set_refval_boundaries(gse, field, gsef->min_value,
                                            gsef->max_value);
      break;
    case GIMP_UNIT_PERCENT:
      gimp_size_entry_set_refval_boundaries(
          gse, field,
          gsef->lower + (gsef->upper - gsef->lower) * gsef->min_value / 100,
          gsef->lower + (gsef->upper - gsef->lower) * gsef->max_value / 100);
      break;
    default:
      gimp_size_entry_set_refval_boundaries(
          gse, field,
          gsef->min_value * gsef->resolution / gimp_unit_get_factor(priv->unit),
          gsef->max_value * gsef->resolution /
  ...
  case GIMP_SIZE_ENTRY_UPDATE_RESOLUTION:
    gimp_size_entry_set_refval_boundaries(
        gse, field, gsef->min_value * gimp_unit_get_factor(priv->unit),
        gsef->max_value * gimp_unit_get_factor(priv->unit));
    break;
...
static void gimp_size_entry_update_value(GimpSizeEntryField *gsef,
  ...
  case GIMP_SIZE_ENTRY_UPDATE_SIZE:
    ...
    default:
      gsef->refval =
          CLAMP(value * gsef->resolution / gimp_unit_get_factor(priv->unit),
                gsef->min_refval, gsef->max_refval);
      break;
    }
    if (priv->show_refval)
  ...
  case GIMP_SIZE_ENTRY_UPDATE_RESOLUTION:
    gsef->refval = CLAMP(value * gimp_unit_get_factor(priv->unit),
                         gsef->min_refval, gsef->max_refval);
    if (priv->show_refval)
      gtk_adjustment_set_value(gsef->refval_adjustment, gsef->refval);
    break;
 ...
 **/
void gimp_size_entry_set_refval_boundaries(GimpSizeEntry *gse, gint field,
                                           gdouble lower, gdouble upper) {
  GimpSizeEntryPrivate *priv;
  GimpSizeEntryField *gsef;
  ...
  case GIMP_SIZE_ENTRY_UPDATE_SIZE:
    switch (priv->unit) {
    case GIMP_UNIT_PIXEL:
      gimp_size_entry_set_value_boundaries(gse, field, gsef->min_refval,
                                           gsef->max_refval);
      break;
    case GIMP_UNIT_PERCENT:
      gimp_size_entry_set_value_boundaries(
          gse, field,
          100 * (gsef->min_refval - gsef->lower) / (gsef->upper - gsef->lower),
          100 * (gsef->max_refval - gsef->lower) / (gsef->upper - gsef->lower));
      break;
    default:
      gimp_size_entry_set_value_boundaries(
          gse, field,
          gsef->min_refval * gimp_unit_get_factor(priv->unit) /
              gsef->resolution,
          gsef->max_refval * gimp_unit_get_factor(priv->unit) /
              gsef->resolution);
      break;
    }
    break;
  ...
  case GIMP_SIZE_ENTRY_UPDATE_RESOLUTION:
    gimp_size_entry_set_value_boundaries(
        gse, field, gsef->min_refval / gimp_unit_get_factor(priv->unit),
        gsef->max_refval / gimp_unit_get_factor(priv->unit));
    break;
 ...
 **/
void gimp_size_entry_set_refval_digits(GimpSizeEntry *gse, gint field,
                                       gint digits) {
  GimpSizeEntryPrivate *priv;
  GimpSizeEntryField *gsef;
...
static void gimp_size_entry_update_refval(GimpSizeEntryField *gsef,
  ...
  case GIMP_SIZE_ENTRY_UPDATE_SIZE:
    ...
    default:
      gsef->value =
          CLAMP(refval * gimp_unit_get_factor(priv->unit) / gsef->resolution,
                gsef->min_value, gsef->max_value);
      break;
    }
 ...
 * gimp_size_entry_new().
 **/
void gimp_size_entry_set_refval(GimpSizeEntry *gse, gint field,
                                gdouble refval) {
  GimpSizeEntryPrivate *priv;
  GimpSizeEntryField *gsef;
...
static void gimp_size_entry_update_unit(GimpSizeEntry *gse, GimpUnit unit) {
  ...
  digits =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(gse), "gimp-pixel-digits"));
  ...
  for (i = 0; i < priv->number_of_fields; i++) {
    ...
    if (priv->update_policy == GIMP_SIZE_ENTRY_UPDATE_SIZE) {
      ...
      digits =
          (gimp_unit_get_digits(GIMP_UNIT_INCH) - gimp_unit_get_digits(unit));
      gtk_spin_button_set_digits(GTK_SPIN_BUTTON(gsef->value_spinbutton),
                                 MAX(3 + digits, 3));
    }
    ...
    gimp_size_entry_set_refval_boundaries(gse, i, gsef->min_refval,
                                          gsef->max_refval);
  }
...
static void gimp_size_entry_attach_eevl(GtkSpinButton *spin_button,
  ...
  g_signal_connect_after(spin_button, "input",
                         G_CALLBACK(gimp_size_entry_eevl_input_callback), gsef);
}
...
static gint gimp_size_entry_eevl_input_callback(GtkSpinButton *spinner,
  ...
  if (priv->number_of_fields == 2) {
    ...
    options.unit_resolver_proc(NULL, &default_unit_factor, &default_unit_offset,
                               options.data);

    options.ratio_quantity.value =
        other_gsef->value / default_unit_factor.value;
    options.ratio_quantity.dimension = default_unit_factor.dimension;
  }

  success = gimp_eevl_evaluate(gtk_entry_get_text(GTK_ENTRY(spinner)), &options,
                               &result, &error_pos, &error);
  if (!success) {
    if (error && error_pos) {
    ...
    *return_val = result.value * ui_unit.value;
    ...
    val = CLAMP(*return_val, gtk_adjustment_get_lower(adj),
...
static gboolean gimp_size_entry_eevl_unit_resolver(const gchar *identifier,
                                                   GimpEevlQuantity *factor,
                                                   gdouble *offset,
                                                   gpointer data) {
  GimpSizeEntryField *gsef = (GimpSizeEntryField *)data;
  ...
  *offset = 0.0;
  ...
  for (unit = 0; unit <= gimp_unit_get_number_of_units(); unit++) {
 ...
 * it's impossible to implement gtk_widget_grab_focus() for a #GtkGrid (is this
 *actually true after the Table->Grid conversion?).
 **/
void gimp_size_entry_grab_focus(GimpSizeEntry *gse) {
  ...
  if (gsef)
    gtk_widget_grab_focus(priv->show_refval ? gsef->refval_spinbutton
                                            : gsef->value_spinbutton);
}
 ...
 *
 * Returns: (transfer none): a #GtkWidget pointer that you can attach a tooltip
 *to.
 **/
GtkWidget *gimp_size_entry_get_help_widget(GimpSizeEntry *gse, gint field) {
  ...
  return (gsef->refval_spinbutton ? gsef->refval_spinbutton
                                  : gsef->value_spinbutton);
}
