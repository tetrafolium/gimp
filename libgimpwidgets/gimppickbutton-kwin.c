...
gboolean _gimp_pick_button_kwin_available(void) {
  ...
  proxy = g_dbus_proxy_new_for_bus_sync(
      ...
      "org.kde.KWin", "/ColorPicker", "org.kde.kwin.ColorPicker", NULL, NULL);
...
void _gimp_pick_button_kwin_pick(GimpPickButton *button) {
  ...
  retval = g_dbus_proxy_call_sync(proxy, "pick", NULL, G_DBUS_CALL_FLAGS_NONE,
                                  -1, NULL, &error);
  if (retval) {
    GimpRGB rgb;
    guint32 color;
    ...
    gimp_rgba_set_uchar(&rgb, (color >> 16) & 0xff, /* Red */
                        (color >> 8) & 0xff, /* Green                         */
                        color & 0xff,        /* Blue: least significant byte. */
                        (color >> 24) &
                            0xff); /* Alpha: most significant byte. */
