...
static void gimp_icons_change_icon_theme(GFile *new_search_path) {
...
static void gimp_icons_notify_system_icon_theme(GObject *settings,
                                                GParamSpec *param,
                                                gpointer unused) {
  ...
  if (gdk_screen_get_setting(screen, "gtk-icon-theme-name", &value)) {
    ...
    g_object_get(settings, "gtk-fallback-icon-theme", &cur_system_icon_theme,
                 NULL);
    if (g_strcmp0(cur_system_icon_theme, new_system_icon_theme)) {
      g_object_set(settings, "gtk-fallback-icon-theme", new_system_icon_theme,
                   NULL);
...
static gboolean gimp_icons_sanity_check(GFile *path, const gchar *theme_name) {
  ...
  if (g_file_query_exists(child, NULL)) {
    ...
    if (g_file_query_exists(index, NULL))
      ...
      g_printerr("%s: Icon theme path has no '%s/index.theme': %s\n", G_STRFUNC,
                 theme_name, gimp_file_get_utf8_name(path));
...
void gimp_icons_init(void) {
  ...
  gtk_icon_theme_prepend_search_path(gtk_icon_theme_get_default(), icons_dir);
  g_free(icons_dir);
  ...
  g_object_set(settings, "gtk-fallback-icon-theme", system_icon_theme,
               "gtk-icon-theme-name", gimp_icon_theme, NULL);
  ...
  pixbuf = gdk_pixbuf_new_from_resource(
      "/org/gimp/icons/64/gimp-wilber-eek.png", &error);
