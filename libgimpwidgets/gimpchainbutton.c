...
enum { PROP_0, PROP_POSITION, PROP_ICON_SIZE, PROP_ACTIVE };
...
static void gimp_chain_button_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_chain_button_get_property(GObject *object, guint property_id,
...
static GtkWidget *gimp_chain_line_new(GimpChainPosition position, gint which);
...
static const gchar *const gimp_chain_icon_names[] = {
    GIMP_ICON_CHAIN_HORIZONTAL, GIMP_ICON_CHAIN_HORIZONTAL_BROKEN,
    GIMP_ICON_CHAIN_VERTICAL, GIMP_ICON_CHAIN_VERTICAL_BROKEN};
...
static void gimp_chain_button_class_init(GimpChainButtonClass *klass) {
  ...
  gimp_chain_button_signals[TOGGLED] =
      g_signal_new("toggled", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpChainButtonClass, toggled), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("position", "Position", "The chain's position",
                        GIMP_TYPE_CHAIN_POSITION, GIMP_CHAIN_TOP,
                        G_PARAM_CONSTRUCT_ONLY | GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("icon-size", "Icon Size", "The chain's icon size",
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("active", "Active", "The chain's toggled state",
...
private
  ...
  g_signal_connect(private->button, "clicked",
                   G_CALLBACK(gimp_chain_button_clicked_callback), button);
}
...
private
  ->line1 = gimp_chain_line_new(private->position, 1);
private
  ->line2 = gimp_chain_line_new(private->position, -1);
...
GtkWidget *gimp_chain_button_new(GimpChainPosition position) {
  return g_object_new(GIMP_TYPE_CHAIN_BUTTON, "position", position, NULL);
}
...
void gimp_chain_button_set_icon_size(GimpChainButton *button,
  ...
  g_object_set(button, "icon-size", size, NULL);
}
...
GtkIconSize gimp_chain_button_get_icon_size(GimpChainButton *button) {
  ...
  g_object_get(button, "icon-size", &size, NULL);
...
static gboolean gimp_chain_line_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_chain_line_class_init(GimpChainLineClass *klass) {
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

  widget_class->draw = gimp_chain_line_draw;
}

static void gimp_chain_line_init(GimpChainLine *line) {
