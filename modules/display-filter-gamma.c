...
#define CDISPLAY_GAMMA(obj)                                                    \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), CDISPLAY_TYPE_GAMMA, CdisplayGamma))
#define CDISPLAY_GAMMA_CLASS(klass)                                            \
  (G_TYPE_CHECK_CLASS_CAST((klass), CDISPLAY_TYPE_GAMMA, CdisplayGammaClass))
#define CDISPLAY_IS_GAMMA(obj)                                                 \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), CDISPLAY_TYPE_GAMMA))
#define CDISPLAY_IS_GAMMA_CLASS(klass)                                         \
  (G_TYPE_CHECK_CLASS_TYPE((klass), CDISPLAY_TYPE_GAMMA))
...
enum { PROP_0, PROP_GAMMA };

static GType cdisplay_gamma_get_type(void);

static void cdisplay_gamma_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void cdisplay_gamma_get_property(GObject *object, guint property_id,
                                        GValue *value, GParamSpec *pspec);

static void cdisplay_gamma_convert_buffer(GimpColorDisplay *display,
                                          GeglBuffer *buffer,
                                          GeglRectangle *area);
static void cdisplay_gamma_set_gamma(CdisplayGamma *gamma, gdouble value);
...
G_DEFINE_DYNAMIC_TYPE(CdisplayGamma, cdisplay_gamma, GIMP_TYPE_COLOR_DISPLAY)
...
static void cdisplay_gamma_class_init(CdisplayGammaClass *klass) {
  ...
  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_GAMMA, "gamma", _("Gamma"), NULL,
                          0.01, 10.0, DEFAULT_GAMMA, 0);
...
static void cdisplay_gamma_class_finalize(CdisplayGammaClass *klass) {}
