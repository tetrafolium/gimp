...
enum { COLUMN_GUID, COLUMN_LABEL, COLUMN_IDEVICE, NUM_COLUMNS };

enum { DEVICE_ADDED, DEVICE_REMOVED, LAST_SIGNAL };
...
struct _GimpInputDeviceStoreClass {
  ...
  void (*device_added)(GimpInputDeviceStore *store, const gchar *udi);
  void (*device_removed)(GimpInputDeviceStore *store, const gchar *udi);
};
...
gimp_input_device_store_class_init(GimpInputDeviceStoreClass *klass) {
  ...
  store_signals[DEVICE_REMOVED] = g_signal_new(
      "device-removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpInputDeviceStoreClass, device_removed), NULL, NULL,
...
static GdkFilterReturn aux_window_filter(GdkXEvent *xevent, GdkEvent *event,
                                         gpointer data) {
#if 0
...
static GdkWindow *create_aux_window(GimpInputDeviceStore *store) {
  GdkWindowAttr wa;
  GdkWindow *retval;
...
static BOOL CALLBACK enum_devices(const DIDEVICEINSTANCEW *di,
                                  void *user_data) {
...
static void gimp_input_device_store_init(GimpInputDeviceStore *store) {
  ...
  typedef HRESULT(WINAPI * t_DirectInput8Create)(HINSTANCE, DWORD, REFIID,
                                                 LPVOID *, LPUNKNOWN);
  ...
  if (!GetModuleHandleEx(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS,
                         (LPCTSTR)&gimp_input_device_store_init, &thismodule))
    return;
  ...
  if ((p_DirectInput8Create = (t_DirectInput8Create)GetProcAddress(
  ...
  if (FAILED((hresult = (*p_DirectInput8Create)(
                  thismodule, DIRECTINPUT_VERSION, &IID_IDirectInput8W,
                  (LPVOID *)&store->directinput8, NULL)))) {
  ...
  if (FAILED((hresult = IDirectInput8_EnumDevices(
                  store->directinput8, DI8DEVCLASS_GAMECTRL, enum_devices,
                  store, DIEDFL_ATTACHEDONLY)))) {
    g_set_error(&store->error, GIMP_MODULE_ERROR, GIMP_MODULE_FAILED,
...
static gboolean gimp_input_device_store_lookup(GimpInputDeviceStore *store,
                                               const gchar *guid,
                                               GtkTreeIter *iter) {
...
static void gimp_input_device_store_insert(GimpInputDeviceStore *store,
                                           const gchar *guid,
                                           const gchar *label,
  ...
  gtk_list_store_insert_with_values(GTK_LIST_STORE(store), &iter, pos,
                                    COLUMN_GUID, guid, COLUMN_LABEL, label,
                                    COLUMN_IDEVICE, didevice8, -1);
}
...
static gboolean gimp_input_device_store_add(GimpInputDeviceStore *store,
  ...
  if (FAILED((hresult = IDirectInput8_CreateDevice(store->directinput8, guid,
                                                   &didevice8, NULL)))) {
    g_free(guidstring);
    return FALSE;
  }

  if (FAILED((hresult = IDirectInputDevice8_SetCooperativeLevel(
                  didevice8, (HWND)gdk_win32_window_get_handle(store->window),
  ...
  if (FAILED((hresult = IDirectInputDevice8_GetDeviceInfo(didevice8, &di)))) {
...
gchar *gimp_input_device_store_get_device_file(GimpInputDeviceStore *store,
  ...
  if (gimp_input_device_store_lookup(store, udi, &iter)) {
    gtk_tree_model_get_value(GTK_TREE_MODEL(store), &iter, COLUMN_IDEVICE,
                             &value);
