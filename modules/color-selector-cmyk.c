...
#define COLORSEL_CMYK(obj)                                                     \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), COLORSEL_TYPE_CMYK, ColorselCmyk))
#define COLORSEL_CMYK_CLASS(klass)                                             \
  (G_TYPE_CHECK_CLASS_CAST((klass), COLORSEL_TYPE_CMYK, ColorselCmykClass))
#define COLORSEL_IS_CMYK(obj)                                                  \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), COLORSEL_TYPE_CMYK))
#define COLORSEL_IS_CMYK_CLASS(klass)                                          \
  (G_TYPE_CHECK_CLASS_TYPE((klass), COLORSEL_TYPE_CMYK))
...
G_DEFINE_DYNAMIC_TYPE(ColorselCmyk, colorsel_cmyk, GIMP_TYPE_COLOR_SELECTOR)
...
static void colorsel_cmyk_class_finalize(ColorselCmykClass *klass) {}

static void colorsel_cmyk_init(ColorselCmyk *module) {
  GtkWidget *grid;
  gint i;

  static const gchar *const cmyk_labels[] = {/* Cyan        */
                                             N_("_C"),
                                             /* Magenta     */
                                             N_("_M"),
                                             /* Yellow      */
                                             ...
                                             /* Key (Black) */
                                             N_("_K")};
  static const gchar *const cmyk_tips[] = {N_("Cyan"), N_("Magenta"),
                                           N_("Yellow"), N_("Black")};
  ...
  gimp_label_set_attributes(GTK_LABEL(module->name_label), PANGO_ATTR_SCALE,
                            PANGO_SCALE_SMALL, -1);
...
static void colorsel_cmyk_set_color(GimpColorSelector *selector,
  ...
  if (module->rgb2cmyk) {
    ...
    gimp_color_transform_process_pixels(
        module->rgb2cmyk, babl_format("R'G'B' double"), rgb_values,
  ...
  for (i = 0; i < 4; i++) {
    g_signal_handlers_block_by_func(module->scales[i],
                                    colorsel_cmyk_scale_update, module);
  ...
  if (config != module->config) {
    if (module->config)
      g_signal_handlers_disconnect_by_func(
          module->config, colorsel_cmyk_config_changed, module);
...
static void colorsel_cmyk_scale_update(GimpLabelSpin *scale,
  ...
  if (module->cmyk2rgb) {
    ...
    gimp_color_transform_process_pixels(
        module->cmyk2rgb, babl_format("CMYK double"), cmyk_values,
        babl_format("R'G'B' double"), rgb_values, 1);
...
static void colorsel_cmyk_config_changed(ColorselCmyk *module) {
  ...
  gimp_help_set_help_data(module->name_label,
                          gimp_color_profile_get_summary(cmyk_profile), NULL);

  flags |= GIMP_COLOR_TRANSFORM_FLAGS_NOOPTIMIZE;
  flags |= GIMP_COLOR_TRANSFORM_FLAGS_BLACK_POINT_COMPENSATION;

  module->rgb2cmyk =
      gimp_color_transform_new(rgb_profile, babl_format("R'G'B' double"),
                               cmyk_profile, babl_format("CMYK double"),
                               GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL, flags);
  module->cmyk2rgb =
      gimp_color_transform_new(cmyk_profile, babl_format("CMYK double"),
                               rgb_profile, babl_format("R'G'B' double"),
                               GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL, flags);
