...
#define COLORSEL_WATER(obj)                                                    \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), COLORSEL_TYPE_WATER, ColorselWater))
#define COLORSEL_WATER_CLASS(klass)                                            \
  (G_TYPE_CHECK_CLASS_CAST((klass), COLORSEL_TYPE_WATER, ColorselWaterClass))
#define COLORSEL_IS_WATER(obj)                                                 \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), COLORSEL_TYPE_WATER))
#define COLORSEL_IS_WATER_CLASS(klass)                                         \
  (G_TYPE_CHECK_CLASS_TYPE((klass), COLORSEL_TYPE_WATER))
...
static gboolean select_area_draw(GtkWidget *widget, cairo_t *cr,
                                 ColorselWater *water);
static gboolean button_press_event(GtkWidget *widget, GdkEventButton *event,
                                   ColorselWater *water);
static gboolean motion_notify_event(GtkWidget *widget, GdkEventMotion *event,
                                    ColorselWater *water);
static gboolean proximity_out_event(GtkWidget *widget, GdkEventProximity *event,
                                    ColorselWater *water);
static void pressure_adjust_update(GtkAdjustment *adj, ColorselWater *water);
...
G_DEFINE_DYNAMIC_TYPE(ColorselWater, colorsel_water, GIMP_TYPE_COLOR_SELECTOR)
...
static void colorsel_water_class_finalize(ColorselWaterClass *klass) {}
...
static void colorsel_water_init(ColorselWater *water) {
  ...
  g_signal_connect(water->area, "draw", G_CALLBACK(select_area_draw), water);

  /* Event signals */
  g_signal_connect(water->area, "motion-notify-event",
                   G_CALLBACK(motion_notify_event), water);
  g_signal_connect(water->area, "button-press-event",
                   G_CALLBACK(button_press_event), water);
  g_signal_connect(water->area, "proximity-out-event",
                   G_CALLBACK(proximity_out_event), water);

  gtk_widget_add_events(
      water->area, GDK_LEAVE_NOTIFY_MASK | GDK_BUTTON_PRESS_MASK |
                       GDK_KEY_PRESS_MASK | GDK_POINTER_MOTION_MASK |
                       GDK_POINTER_MOTION_HINT_MASK | GDK_PROXIMITY_OUT_MASK);

  gtk_widget_grab_focus(water->area);

  adj = gtk_adjustment_new(200.0 - water->pressure_adjust * 100.0, 0.0, 200.0,
                           1.0, 1.0, 0.0);
  g_signal_connect(adj, "value-changed", G_CALLBACK(pressure_adjust_update),
                   water);
  ...
  gimp_widget_track_monitor(GTK_WIDGET(water),
                            G_CALLBACK(colorsel_water_destroy_transform), NULL,
                            NULL);
}

static gdouble calc(gdouble x, gdouble y, gdouble angle) {
  ...
  if (config != water->config) {
    if (water->config) {
      g_signal_handlers_disconnect_by_func(
          water->config, colorsel_water_destroy_transform, water);
...
static void colorsel_water_create_transform(ColorselWater *water) {
  if (water->config) {
    ...
    water->transform = gimp_widget_get_color_transform(
        water->area, water->config, profile, format, format);
  }
}
...
static gboolean select_area_draw(GtkWidget *widget, cairo_t *cr,
  ...
  surface =
      cairo_image_surface_create(CAIRO_FORMAT_RGB24, area.width, area.height);
  ...
  for (j = 0, y = area.y / allocation.height; j < area.height; j++, y += dy) {
    guchar *d = dest;
    ...
    for (i = 0; i < area.width; i++) {
      GIMP_CAIRO_RGB24_SET_PIXEL(d, CLAMP((gint)r, 0, 255),
                                 CLAMP((gint)g, 0, 255),
                                 CLAMP((gint)b, 0, 255));
    ...
    if (water->transform)
      gimp_color_transform_process_pixels(
          water->transform, babl_format("cairo-RGB24"), dest,
          babl_format("cairo-RGB24"), dest, area.width);
...
static void add_pigment(ColorselWater *water, gboolean erase, gdouble x,
                        gdouble y, gdouble much) {
...
static void draw_brush(ColorselWater *water, GtkWidget *widget, gboolean erase,
...
static gboolean motion_notify_event(GtkWidget *widget, GdkEventMotion *event,
  ...
  if (event->state & (GDK_BUTTON1_MASK | GDK_BUTTON2_MASK | GDK_BUTTON3_MASK |
    ...
    if (gdk_device_get_history(event->device, event->window, last_motion_time,
                               event->time, &coords, &nevents)) {
      for (i = 0; i < nevents; i++) {
        gdouble x = 0.0;
        gdouble y = 0.0;
        gdouble pressure = 0.5;

        gdk_device_get_axis(event->device, coords[i]->axes, GDK_AXIS_X, &x);
        gdk_device_get_axis(event->device, coords[i]->axes, GDK_AXIS_Y, &y);
        gdk_device_get_axis(event->device, coords[i]->axes, GDK_AXIS_PRESSURE,
                            &pressure);

        draw_brush(water, widget, erase, x / allocation.width,
      ...
      draw_brush(water, widget, erase, event->x / allocation.width,
...
static void pressure_adjust_update(GtkAdjustment *adj, ColorselWater *water) {
  water->pressure_adjust =
      (gtk_adjustment_get_upper(adj) - gtk_adjustment_get_value(adj)) / 100.0;
}
