...
#define COLORSEL_WHEEL(obj)                                                    \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), COLORSEL_TYPE_WHEEL, ColorselWheel))
#define COLORSEL_WHEEL_CLASS(klass)                                            \
  (G_TYPE_CHECK_CLASS_CAST((klass), COLORSEL_TYPE_WHEEL, ColorselWheelClass))
#define COLORSEL_IS_WHEEL(obj)                                                 \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), COLORSEL_TYPE_WHEEL))
#define COLORSEL_IS_WHEEL_CLASS(klass)                                         \
  (G_TYPE_CHECK_CLASS_TYPE((klass), COLORSEL_TYPE_WHEEL))
...
G_DEFINE_DYNAMIC_TYPE(ColorselWheel, colorsel_wheel, GIMP_TYPE_COLOR_SELECTOR)
...
static void colorsel_wheel_class_init(ColorselWheelClass *klass) {
...
static void colorsel_wheel_class_finalize(ColorselWheelClass *klass) {}

static void colorsel_wheel_init(ColorselWheel *wheel) {
  wheel->hsv = gimp_color_wheel_new();
  g_object_add_weak_pointer(G_OBJECT(wheel->hsv), (gpointer)&wheel->hsv);
  gtk_box_pack_start(GTK_BOX(wheel), wheel->hsv, TRUE, TRUE, 0);
  gtk_widget_show(wheel->hsv);

  g_signal_connect(wheel->hsv, "changed", G_CALLBACK(colorsel_wheel_changed),
                   wheel);
}
...
static void colorsel_wheel_set_color(GimpColorSelector *selector,
  ...
  gimp_color_wheel_set_color(GIMP_COLOR_WHEEL(wheel->hsv), hsv->h, hsv->s,
                             hsv->v);
}
...
static void colorsel_wheel_changed(GimpColorWheel *hsv,
                                   GimpColorSelector *selector) {
  gimp_color_wheel_get_color(hsv, &selector->hsv.h, &selector->hsv.s,
                             &selector->hsv.v);
