...
#define CDISPLAY_ACES_RRT(obj)                                                 \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), CDISPLAY_TYPE_ACES_RRT, CdisplayAcesRRT))
#define CDISPLAY_ACES_RRT_CLASS(klass)                                         \
  (G_TYPE_CHECK_CLASS_CAST((klass), CDISPLAY_TYPE_ACES_RRT,                    \
                           CdisplayAcesRRTClass))
#define CDISPLAY_IS_ACES_RRT(obj)                                              \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), CDISPLAY_TYPE_ACES_RRT))
#define CDISPLAY_IS_ACES_RRT_CLASS(klass)                                      \
  (G_TYPE_CHECK_CLASS_TYPE((klass), CDISPLAY_TYPE_ACES_RRT))
...
enum { PROP_0, PROP_EXPOSURE };

static GType cdisplay_aces_rrt_get_type(void);

static void cdisplay_aces_rrt_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void cdisplay_aces_rrt_get_property(GObject *object, guint property_id,
...
static const GimpModuleInfo cdisplay_aces_rrt_info = {
    ...
    N_("ACES RRT (RRT = Reference Rendering Transform). An HDR to SDR proof "
       "color display filter, using a luminance-only approximation of the ACES "
       "RRT, a pre-defined filmic look to be used before ODT (display or "
       "output space ICC profile)"),
...
static void cdisplay_aces_rrt_class_init(CdisplayAcesRRTClass *klass) {
  ...
  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_EXPOSURE, "exposure",
                          _("Pre-transform change in stops"), NULL, -10.0, 10.0,
                          DEFAULT_EXPOSURE, 1);
...
static void cdisplay_aces_rrt_class_finalize(CdisplayAcesRRTClass *klass) {}

static void cdisplay_aces_rrt_init(CdisplayAcesRRT *aces_rrt) {}
...
static void cdisplay_aces_rrt_convert_buffer(GimpColorDisplay *display,
  ...
  iter = gegl_buffer_iterator_new(buffer, area, 0, babl_format("RGBA float"),
