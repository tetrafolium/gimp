...
#define CDISPLAY_CLIP_WARNING(obj)                                             \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), CDISPLAY_TYPE_CLIP_WARNING,               \
                              CdisplayClipWarning))
#define CDISPLAY_CLIP_WARNING_CLASS(klass)                                     \
  (G_TYPE_CHECK_CLASS_CAST((klass), CDISPLAY_TYPE_CLIP_WARNING,                \
                           CdisplayClipWarningClass))
#define CDISPLAY_IS_CLIP_WARNING(obj)                                          \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), CDISPLAY_TYPE_CLIP_WARNING))
#define CDISPLAY_IS_CLIP_WARNING_CLASS(klass)                                  \
  (G_TYPE_CHECK_CLASS_TYPE((klass), CDISPLAY_TYPE_CLIP_WARNING))
...
static void cdisplay_clip_warning_set_member(CdisplayClipWarning *clip_warning,
                                             ...
                                             gpointer member,
                                             gconstpointer value, gsize size);
static void
...
static void cdisplay_clip_warning_class_init(CdisplayClipWarningClass *klass) {
  ...
  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_SHOW_SHADOWS, "show-shadows", _("Show shadows"),
      _("Show warning for pixels with a negative component"), TRUE,
      GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_RGB(object_class, PROP_SHADOWS_COLOR, "shadows-color",
                       _("Shadows color"), _("Shadows warning color"), FALSE,
  ...
  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_SHOW_HIGHLIGHTS, "show-highlights",
      _("Show highlights"),
      _("Show warning for pixels with a component greater than one"), TRUE,
      GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_RGB(object_class, PROP_HIGHLIGHTS_COLOR, "highlights-color",
                       _("Highlights color"), _("Highlights warning color"),
                       FALSE, DEFAULT_HIGHLIGHTS_COLOR,
                       GIMP_PARAM_STATIC_STRINGS | GIMP_CONFIG_PARAM_DEFAULTS);
  ...
  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_SHOW_BOGUS, "show-bogus", _("Show bogus"),
      _("Show warning for pixels with an infinite or NaN component"), TRUE,
      GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_RGB(object_class, PROP_BOGUS_COLOR, "bogus-color",
                       _("Bogus color"), _("Bogus warning color"), FALSE,
                       DEFAULT_BOGUS_COLOR,
                       GIMP_PARAM_STATIC_STRINGS | GIMP_CONFIG_PARAM_DEFAULTS);
  ...
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_INCLUDE_ALPHA, "include-alpha",
                           _("Include alpha component"),
                           _("Include alpha component in the warning"), TRUE,
                           GIMP_PARAM_STATIC_STRINGS);
...
static void cdisplay_clip_warning_init(CdisplayClipWarning *clip_warning) {}
...
#define SET_MEMBER_PTR(member, value)                                          \
  cdisplay_clip_warning_set_member(clip_warning, pspec->name,                  \
                                   &clip_warning->member, (value),             \
...
static void cdisplay_clip_warning_convert_buffer(GimpColorDisplay *display,
  ...
  iter = gegl_buffer_iterator_new(buffer, area, 0, babl_format("R'G'B'A float"),
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    while (count--) {
      ...
      if (warning) {
        ...
        memcpy(data, clip_warning->colors[warning][alt], 4 * sizeof(gfloat));
      }
