...
enum { COLUMN_IDENTIFIER, COLUMN_LABEL, COLUMN_DEVICE_FILE, NUM_COLUMNS };

enum { PROP_0, PROP_CONSTRUCT_ERROR };
...
struct _GimpInputDeviceStoreClass {
  ...
  void (*device_added)(GimpInputDeviceStore *store, const gchar *identifier);
  void (*device_removed)(GimpInputDeviceStore *store, const gchar *identifier);
};
...
static void gimp_input_device_store_init(GimpInputDeviceStore *store) {
  ...
  g_signal_connect(store->client, "uevent",
                   G_CALLBACK(gimp_input_device_store_uevent), store);
}
...
static void gimp_input_device_store_insert(GimpInputDeviceStore *store,
  ...
  gtk_list_store_insert_with_values(GTK_LIST_STORE(store), &iter, pos,
                                    COLUMN_IDENTIFIER, identifier, COLUMN_LABEL,
                                    label, COLUMN_DEVICE_FILE, device_file, -1);
}
  ...
  if (device_file) {
    if (name) {
      ...
      if (!gimp_input_device_store_lookup(store, name, &unused)) {
        ...
        g_signal_emit(store, store_signals[DEVICE_ADDED], 0, name);
      ...
      if (parent) {
        ...
        if (parent_name) {
          ...
          if (!gimp_input_device_store_lookup(store, parent_name, &unused)) {
            ...
            g_signal_emit(store, store_signals[DEVICE_ADDED], 0, parent_name);
...
gchar *gimp_input_device_store_get_device_file(GimpInputDeviceStore *store,
  ...
  if (gimp_input_device_store_lookup(store, identifier, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, COLUMN_DEVICE_FILE, &device_file, -1);
...
GType gimp_input_device_store_get_type(void) { return G_TYPE_NONE; }

GimpInputDeviceStore *gimp_input_device_store_new(void) { return NULL; }
