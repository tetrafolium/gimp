...
static gboolean gimp_color_wheel_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_color_wheel_move(GimpColorWheel *wheel, GtkDirectionType dir);
...
static void gimp_color_wheel_class_init(GimpColorWheelClass *class) {
  ...
  wheel_signals[CHANGED] = g_signal_new(
      "changed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpColorWheelClass, changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  wheel_signals[MOVE] =
      g_signal_new("move", G_OBJECT_CLASS_TYPE(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(GimpColorWheelClass, move), NULL, NULL, NULL,
...
static void gimp_color_wheel_class_finalize(GimpColorWheelClass *klass) {}

static void gimp_color_wheel_init(GimpColorWheel *wheel) {
...
static void gimp_color_wheel_realize(GtkWidget *widget) {
  ...
  attr.event_mask =
      (gtk_widget_get_events(widget) | GDK_KEY_PRESS_MASK |
...
static void compute_triangle(GimpColorWheel *wheel, gint *hx, gint *hy,
                             gint *sx, gint *sy, gint *vx, gint *vy) {
  GimpColorWheelPrivate *priv = wheel->priv;
  GtkAllocation allocation;
  gdouble center_x;
  gdouble center_y;
...
static gboolean is_in_ring(GimpColorWheel *wheel, gdouble x, gdouble y) {
  GimpColorWheelPrivate *priv = wheel->priv;
  GtkAllocation allocation;
  gdouble dx, dy, dist;
  gdouble center_x;
...
static void compute_sv(GimpColorWheel *wheel, gdouble x, gdouble y, gdouble *s,
                       gdouble *v) {
  GtkAllocation allocation;
...
static gboolean is_in_triangle(GimpColorWheel *wheel, gdouble x, gdouble y) {
...
static double compute_v(GimpColorWheel *wheel, gdouble x, gdouble y) {
  GtkAllocation allocation;
  gdouble center_x;
  gdouble center_y;
  gdouble dx, dy;
...
static gboolean set_cross_grab(GimpColorWheel *wheel, GdkEvent *event) {
  GimpColorWheelPrivate *priv = wheel->priv;
  GdkDisplay *display;
  GdkSeat *seat;
  GdkCursor *cursor;
  ...
  success =
      (gdk_seat_grab(seat, priv->window, GDK_SEAT_CAPABILITY_ALL_POINTING,
                     FALSE, cursor, event, NULL, NULL) == GDK_GRAB_SUCCESS);
...
static gboolean gimp_color_wheel_button_press(GtkWidget *widget,
  ...
  if (is_in_ring(wheel, x, y)) {
    ...
    gimp_color_wheel_set_color(wheel, compute_v(wheel, x, y), priv->s, priv->v);
...
/* Paints the hue ring */
static void paint_ring(GimpColorWheel *wheel, cairo_t *cr) {
  ...
  for (yy = 0; yy < height; yy++) {
    ...
    for (xx = 0; xx < width; xx++) {
      ...
      *p++ = (((int)floor(r * 255 + 0.5) << 16) |
              ((int)floor(g * 255 + 0.5) << 8) | (int)floor(b * 255 + 0.5));
    }
  }
  ...
  if (priv->transform) {
    ...
    for (i = 0; i < height; i++) {
      gimp_color_transform_process_pixels(priv->transform, format, b, format, b,
                                          width);
  ...
  source = cairo_image_surface_create_for_data(
      (guchar *)buf, CAIRO_FORMAT_RGB24, width, height, stride);
  ...
  cairo_new_path(cr);
  cairo_arc(cr, center_x, center_y, priv->size / 2.0 - priv->ring_width / 2.0,
            0, 2 * G_PI);
  cairo_stroke(cr);
...
static void paint_triangle(GimpColorWheel *wheel, cairo_t *cr,
                           gboolean draw_focus) {
  GtkWidget *widget = GTK_WIDGET(wheel);
  GimpColorWheelPrivate *priv = wheel->priv;
 ...
 * Since: 2.10
 */
void gimp_color_wheel_set_color(GimpColorWheel *wheel, gdouble h, gdouble s,
                                gdouble v) {
 ...
 * Since: 2.10
 */
void gimp_color_wheel_set_ring_fraction(GimpColorWheel *hsv, gdouble fraction) {
  GimpColorWheelPrivate *priv;
  ...
  if (config != priv->config) {
    if (priv->config) {
      g_signal_handlers_disconnect_by_func(
          priv->config, gimp_color_wheel_destroy_transform, wheel);
...
static void gimp_color_wheel_create_transform(GimpColorWheel *wheel) {
  ...
  if (priv->config) {
    ...
    priv->transform = gimp_widget_get_color_transform(
        GTK_WIDGET(wheel), priv->config, profile, format, format);
  }
}
