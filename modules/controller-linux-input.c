...
static const LinuxInputEvent rel_events[] = {
    ...
    {REL_HWHEEL, "horizontal-wheel-turn-forward",
     N_("Horiz. Wheel Turn Forward")},
...
#define CONTROLLER_LINUX_INPUT(obj)                                            \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), CONTROLLER_TYPE_LINUX_INPUT,              \
                              ControllerLinuxInput))
#define CONTROLLER_LINUX_INPUT_CLASS(klass)                                    \
  (G_TYPE_CHECK_CLASS_CAST((klass), CONTROLLER_TYPE_LINUX_INPUT,               \
                           ControllerLinuxInputClass))
#define CONTROLLER_IS_LINUX_INPUT(obj)                                         \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), CONTROLLER_TYPE_LINUX_INPUT))
#define CONTROLLER_IS_LINUX_INPUT_CLASS(klass)                                 \
  (G_TYPE_CHECK_CLASS_TYPE((klass), CONTROLLER_TYPE_LINUX_INPUT))
...
static void linux_input_set_property(GObject *object, guint property_id,
                                     const GValue *value, GParamSpec *pspec);
static void linux_input_get_property(GObject *object, guint property_id,
...
static gboolean linux_input_set_device(ControllerLinuxInput *controller,
                                       const gchar *device);
static gboolean linux_input_read_event(GIOChannel *io, GIOCondition cond,
                                       gpointer data);
...
controller_linux_input_class_init(ControllerLinuxInputClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "device", _("Device:"),
          _("The name of the device to read Linux Input events from."), NULL,
...
static void controller_linux_input_init(ControllerLinuxInput *controller) {
...
static void linux_input_get_device_info(ControllerLinuxInput *controller,
                                        int fd) {
...
static gboolean linux_input_set_device(ControllerLinuxInput *controller,
  ...
  if (filename) {
    ...
    if (fd >= 0) {
      ...
      name[0] = '\0';
      if (ioctl(fd, EVIOCGNAME(sizeof(name)), name) >= 0 && strlen(name) > 0 &&
      ...
      controller->io_id = g_io_add_watch(controller->io, G_IO_IN,
                                         linux_input_read_event, controller);
      return TRUE;
    } else {
      state = g_strdup_printf(_("Device not available: %s"), g_strerror(errno));
      g_object_set(controller, "state", state, NULL);
      g_free(state);
    }
...
static gboolean linux_input_read_event(GIOChannel *io, GIOCondition cond,
  ...
  status = g_io_channel_read_chars(io, (gchar *)&ev, sizeof(struct input_event),
                                   &n_bytes, &error);
