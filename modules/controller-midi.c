...
enum { PROP_0, PROP_DEVICE, PROP_CHANNEL };

#define CONTROLLER_TYPE_MIDI (controller_midi_get_type())
#define CONTROLLER_MIDI(obj)                                                   \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), CONTROLLER_TYPE_MIDI, ControllerMidi))
#define CONTROLLER_MIDI_CLASS(klass)                                           \
  (G_TYPE_CHECK_CLASS_CAST((klass), CONTROLLER_TYPE_MIDI, ControllerMidiClass))
#define CONTROLLER_IS_MIDI(obj)                                                \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), CONTROLLER_TYPE_MIDI))
#define CONTROLLER_IS_MIDI_CLASS(klass)                                        \
  (G_TYPE_CHECK_CLASS_TYPE((klass), CONTROLLER_TYPE_MIDI))
...
static void midi_set_property(GObject *object, guint property_id,
                              const GValue *value, GParamSpec *pspec);
static void midi_get_property(GObject *object, guint property_id, GValue *value,
                              GParamSpec *pspec);
...
static gboolean midi_set_device(ControllerMidi *controller,
                                const gchar *device);
static void midi_event(ControllerMidi *midi, gint channel, gint event_id,
                       gdouble value);

static gboolean midi_read_event(GIOChannel *io, GIOCondition cond,
                                gpointer data);

#ifdef HAVE_ALSA
static gboolean midi_alsa_prepare(GSource *source, gint *timeout);
static gboolean midi_alsa_check(GSource *source);
static gboolean midi_alsa_dispatch(GSource *source, GSourceFunc callback,
                                   gpointer user_data);

static GSourceFuncs alsa_source_funcs = {midi_alsa_prepare, midi_alsa_check,
                                         midi_alsa_dispatch, NULL};
...
G_DEFINE_DYNAMIC_TYPE(ControllerMidi, controller_midi, GIMP_TYPE_CONTROLLER)
...
static void controller_midi_class_init(ControllerMidiClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int("channel", _("Channel:"),
                       _("The MIDI channel to read events from. Set to -1 for "
                         "reading from all MIDI channels."),
                       -1, 15, -1, GIMP_CONFIG_PARAM_FLAGS));
...
static void controller_midi_class_finalize(ControllerMidiClass *klass) {}
...
static const gchar *midi_get_event_name(GimpController *controller,
  ...
  if (event_id < (128 + 128 + 128)) {
    if (!midi_events[event_id].name) {
      if (event_id < 128)
        midi_events[event_id].name = g_strdup_printf("note-on-%02x", event_id);
      else if (event_id < (128 + 128))
        midi_events[event_id].name =
            g_strdup_printf("note-off-%02x", event_id - 128);
      else if (event_id < (128 + 128 + 128))
        midi_events[event_id].name =
            g_strdup_printf("controller-%03d", event_id - 256);
    }
...
static const gchar *midi_get_event_blurb(GimpController *controller,
  ...
  if (event_id <= 383) {
    if (!midi_events[event_id].blurb) {
      if (event_id <= 127)
        midi_events[event_id].blurb =
            g_strdup_printf(_("Note %02x on"), event_id);
      else if (event_id <= 255)
        midi_events[event_id].blurb =
            g_strdup_printf(_("Note %02x off"), event_id - 128);
      else if (event_id <= 383)
        midi_events[event_id].blurb =
            g_strdup_printf(_("Controller %03d"), event_id - 256);
    }
...
static gboolean midi_set_device(ControllerMidi *midi, const gchar *device) {
  midi->swallow = TRUE;
  midi->command = 0x0;
  midi->channel = 0x0;
  midi->key = -1;
  ...
  if (midi->device && strlen(midi->device)) {
    ...
    if (!g_ascii_strcasecmp(midi->device, "alsa")) {
      ...
      ret = snd_seq_open(&midi->sequencer, "default", SND_SEQ_OPEN_INPUT, 0);
      if (ret >= 0) {
        snd_seq_set_client_name(midi->sequencer, _("GIMP"));
        ret = snd_seq_create_simple_port(
            midi->sequencer, _("GIMP MIDI Input Controller"),
      ...
      if (ret < 0) {
        state =
            g_strdup_printf(_("Device not available: %s"), snd_strerror(ret));
      ...
      alsa = g_strdup_printf("ALSA (%d:%d)", snd_seq_client_id(midi->sequencer),
                             ret);
      ...
      event_source = g_source_new(&alsa_source_funcs, sizeof(GAlsaSource));
    ...
    if (fd >= 0) {
      ...
      midi->io_id =
          g_io_add_watch(midi->io, G_IO_IN | G_IO_ERR | G_IO_HUP | G_IO_NVAL,
                         midi_read_event, midi);
      return TRUE;
    } else {
      gchar *state =
          g_strdup_printf(_("Device not available: %s"), g_strerror(errno));
      g_object_set(midi, "state", state, NULL);
      g_free(state);
    }
  } else {
...
static void midi_event(ControllerMidi *midi, gint channel, gint event_id,
                       gdouble value) {
  if (channel == -1 || midi->midi_channel == -1 ||
...
gboolean midi_read_event(GIOChannel *io, GIOCondition cond, gpointer data) {
  ...
  status =
      g_io_channel_read_chars(io, (gchar *)buf, sizeof(buf), &size, &error);
  ...
  case G_IO_STATUS_EOF:
    ...
    if (error) {
      gchar *state =
          g_strdup_printf(_("Device not available: %s"), error->message);
      g_object_set(midi, "state", state, NULL);
      g_free(state);
...
static gboolean midi_alsa_dispatch(GSource *source, GSourceFunc callback,
  ...
  if (snd_seq_event_input_pending(midi->sequencer, 1) > 0) {
    ...
    case SND_SEQ_EVENT_NOTEON:
      midi_event(midi, event->data.note.channel, event->data.note.note,
                 (gdouble)event->data.note.velocity / 127.0);
      break;

    case SND_SEQ_EVENT_NOTEOFF:
      midi_event(midi, event->data.note.channel, event->data.note.note + 128,
                 (gdouble)event->data.note.velocity / 127.0);
      break;
    ...
    case SND_SEQ_EVENT_PORT_SUBSCRIBED:
      ...
      snd_seq_get_any_client_info(
          midi->sequencer, event->data.connect.sender.client, client_info);
      snd_seq_get_any_port_info(midi->sequencer,
                                event->data.connect.sender.client,
                                event->data.connect.sender.port, port_info);
      /*
