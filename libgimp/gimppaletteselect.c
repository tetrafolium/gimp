...
const gchar *gimp_palette_select_new(const gchar *title,
  ...
  procedure =
      gimp_procedure_new(plug_in, palette_callback,
                         GIMP_PDB_PROC_TYPE_TEMPORARY, gimp_temp_palette_run,
                         palette_data, (GDestroyNotify)gimp_palette_data_free);

  GIMP_PROC_ARG_STRING(procedure, "palette-name", "Palette name",
                       "The palette name", NULL, G_PARAM_READWRITE);

  GIMP_PROC_ARG_INT(procedure, "num-colors", "Num colors", "Number of colors",
...
static GimpValueArray *gimp_temp_palette_run(GimpProcedure *procedure,
  ...
  if (!data->idle_id)
    data->idle_id = g_idle_add((GSourceFunc)gimp_temp_palette_idle, data);
...
static gboolean gimp_temp_palette_idle(GimpPaletteData *data) {
  ...
  if (data->callback)
    data->callback(data->palette_name, data->closing, data->data);
