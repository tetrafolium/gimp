...
gimp_gradient_select_new(const gchar *title, const gchar *gradient_name,
  ...
  procedure = gimp_procedure_new(plug_in, gradient_callback,
                                 GIMP_PDB_PROC_TYPE_TEMPORARY,
                                 gimp_temp_gradient_run, gradient_data,
                                 (GDestroyNotify)gimp_gradient_data_free);

  GIMP_PROC_ARG_STRING(procedure, "gradient-name", "Gradient name",
                       "The gradient name", NULL, G_PARAM_READWRITE);

  GIMP_PROC_ARG_INT(procedure, "gradient-width", "Gradient width",
                    "The gradient width", 0, G_MAXINT, 0, G_PARAM_READWRITE);

  GIMP_PROC_ARG_FLOAT_ARRAY(procedure, "gradient-data", "Gradient data",
                            "The gradient data", G_PARAM_READWRITE);
...
static gboolean gimp_temp_gradient_idle(GimpGradientData *data) {
  ...
  if (data->callback)
    data->callback(data->gradient_name, data->width, data->gradient_data,
                   data->closing, data->data);
