...
#define GET_PRIVATE(obj)                                                       \
  (g_object_get_data(G_OBJECT(obj), "gimp-item-combo-box-private"))
...
static void gimp_item_combo_box_model_add(GimpIntComboBox *combo_box,
                                          GtkListStore *store, GimpImage *image,
                                          GList *items, gint tree_level);
...
static const GtkTargetEntry targets[] = {{"application/x-gimp-channel-id", 0},
...
static void gimp_drawable_combo_box_init(GimpDrawableComboBox *combo_box) {
...
GtkWidget *gimp_drawable_combo_box_new(GimpItemConstraintFunc constraint,
                                       gpointer data,
                                       GDestroyNotify data_destroy) {
  return gimp_item_combo_box_new(GIMP_TYPE_DRAWABLE_COMBO_BOX, constraint, data,
                                 data_destroy);
}
...
static void gimp_channel_combo_box_class_init(GimpChannelComboBoxClass *klass) {
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

  widget_class->drag_data_received = gimp_item_combo_box_drag_data_received;
}

static void gimp_channel_combo_box_init(GimpChannelComboBox *combo_box) {
  gtk_drag_dest_set(GTK_WIDGET(combo_box),
                    GTK_DEST_DEFAULT_HIGHLIGHT | GTK_DEST_DEFAULT_MOTION |
                        GTK_DEST_DEFAULT_DROP,
...
GtkWidget *gimp_channel_combo_box_new(GimpItemConstraintFunc constraint,
                                      gpointer data,
                                      GDestroyNotify data_destroy) {
  return gimp_item_combo_box_new(GIMP_TYPE_CHANNEL_COMBO_BOX, constraint, data,
                                 data_destroy);
}

G_DEFINE_TYPE(GimpLayerComboBox, gimp_layer_combo_box, GIMP_TYPE_INT_COMBO_BOX)

static void gimp_layer_combo_box_class_init(GimpLayerComboBoxClass *klass) {
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

  widget_class->drag_data_received = gimp_item_combo_box_drag_data_received;
}

static void gimp_layer_combo_box_init(GimpLayerComboBox *combo_box) {
  gtk_drag_dest_set(GTK_WIDGET(combo_box),
                    GTK_DEST_DEFAULT_HIGHLIGHT | GTK_DEST_DEFAULT_MOTION |
                        GTK_DEST_DEFAULT_DROP,
                    targets + 1, 1, GDK_ACTION_COPY);
...
GtkWidget *gimp_layer_combo_box_new(GimpItemConstraintFunc constraint,
                                    gpointer data,
                                    GDestroyNotify data_destroy) {
  return gimp_item_combo_box_new(GIMP_TYPE_LAYER_COMBO_BOX, constraint, data,
                                 data_destroy);
}
...
static void gimp_vectors_combo_box_class_init(GimpVectorsComboBoxClass *klass) {
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

  widget_class->drag_data_received = gimp_item_combo_box_drag_data_received;
}

static void gimp_vectors_combo_box_init(GimpVectorsComboBox *combo_box) {
  gtk_drag_dest_set(GTK_WIDGET(combo_box),
                    GTK_DEST_DEFAULT_HIGHLIGHT | GTK_DEST_DEFAULT_MOTION |
                        GTK_DEST_DEFAULT_DROP,
                    targets + 2, 1, GDK_ACTION_COPY);
...
GtkWidget *gimp_vectors_combo_box_new(GimpItemConstraintFunc constraint,
                                      gpointer data,
                                      GDestroyNotify data_destroy) {
  return gimp_item_combo_box_new(GIMP_TYPE_VECTORS_COMBO_BOX, constraint, data,
                                 data_destroy);
}
...
static GtkWidget *gimp_item_combo_box_new(GType type,
  ...
  combo_box = g_object_new(type, "width-request", WIDTH_REQUEST, "ellipsize",
                           PANGO_ELLIPSIZE_MIDDLE, NULL);

private
  = GET_PRIVATE(combo_box);
...
private
  ...
  g_signal_connect(combo_box, "changed",
                   G_CALLBACK(gimp_item_combo_box_changed), NULL);
...
static void gimp_item_combo_box_populate(GimpIntComboBox *combo_box) {
  ...
  for (list = images; list; list = g_list_next(list)) {
    ...
    if (GIMP_IS_DRAWABLE_COMBO_BOX(combo_box) ||
      ...
      gimp_item_combo_box_model_add(combo_box, GTK_LIST_STORE(model), image,
                                    items, 0);
      g_list_free(items);
    }
...
static void gimp_item_combo_box_model_add(GimpIntComboBox *combo_box,
  ...
  for (list = items; list; list = g_list_next(list)) {
    ...
    if (!private->constraint ||
      ...
      label = g_strdup_printf("%s%s-%d / %s-%d", indent, image_name,
                              gimp_image_get_id(image), item_name, item_id);
      ...
      if (GIMP_IS_VECTORS_COMBO_BOX(combo_box))
        ...
        thumb = gimp_drawable_get_thumbnail(GIMP_DRAWABLE(item), THUMBNAIL_SIZE,
                                            THUMBNAIL_SIZE,
                                            GIMP_PIXBUF_SMALL_CHECKS);

      gtk_list_store_append(store, &iter);
      gtk_list_store_set(store, &iter, GIMP_INT_STORE_VALUE, item_id,
                         GIMP_INT_STORE_LABEL, label, GIMP_INT_STORE_PIXBUF,
                         thumb, -1);
    ...
    if (gimp_item_is_group(item)) {
      ...
      gimp_item_combo_box_model_add(combo_box, store, image, children,
                                    tree_level + 1);
...
static void gimp_item_combo_box_drag_data_received(GtkWidget *widget,
  ...
  if (g_utf8_validate(str, -1, NULL)) {
    ...
    if (sscanf(str, "%i:%i", &pid, &ID) == 2 && pid == gimp_getpid()) {
...
static gboolean gimp_item_combo_box_remove_items(GtkTreeModel *model,
                                                 GtkTreePath *path,
                                                 GtkTreeIter *iter,
                                                 gpointer data) {
  gint item_ID;
  ...
  gtk_tree_model_get(model, iter, GIMP_INT_STORE_VALUE, &item_ID, -1);
...
static void gimp_item_combo_box_changed(GimpIntComboBox *combo_box) {
  ...
  if (gimp_int_combo_box_get_active(combo_box, &item_ID)) {
    if (item_ID > 0 && !gimp_item_get_by_id(item_ID)) {
      ...
      gtk_tree_model_foreach(model, gimp_item_combo_box_remove_items, &remove);
