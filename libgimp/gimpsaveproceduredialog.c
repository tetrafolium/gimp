...
static gboolean gimp_save_procedure_dialog_activate_edit_metadata(
    GtkLinkButton *link, GimpSaveProcedureDialog *dialog);

G_DEFINE_TYPE_WITH_PRIVATE(GimpSaveProcedureDialog, gimp_save_procedure_dialog,
...
static void gimp_save_procedure_dialog_init(GimpSaveProcedureDialog *dialog) {
...
static void gimp_save_procedure_dialog_fill_list(GimpProcedureDialog *dialog,
  ...
  GIMP_PROCEDURE_DIALOG_CLASS(parent_class)
      ->fill_list(dialog, procedure, config, properties2);
  ...
  if (gimp_save_procedure_get_support_exif(save_procedure) ||
    ...
    g_signal_connect(
        link, "activate-link",
        G_CALLBACK(gimp_save_procedure_dialog_activate_edit_metadata), dialog);
    ...
    if (gimp_save_procedure_get_support_exif(save_procedure)) {
      widget = gimp_prop_check_button_new(G_OBJECT(config), "save-exif", NULL);
      gtk_grid_attach(GTK_GRID(grid), widget, left, 0, 6 / n_metadata, 1);
      left += 6 / n_metadata;
      top = 1;
      gtk_widget_show(widget);
    }
    if (gimp_save_procedure_get_support_iptc(save_procedure)) {
      widget = gimp_prop_check_button_new(G_OBJECT(config), "save-iptc", NULL);
      gtk_grid_attach(GTK_GRID(grid), widget, left, 0, 6 / n_metadata, 1);
      left += 6 / n_metadata;
      top = 1;
      gtk_widget_show(widget);
    ...
    if (gimp_save_procedure_get_support_xmp(save_procedure)) {
      widget = gimp_prop_check_button_new(G_OBJECT(config), "save-xmp", NULL);
      gtk_grid_attach(GTK_GRID(grid), widget, left, 0, 6 / n_metadata, 1);
      left += 6 / n_metadata;
      top = 1;
      gtk_widget_show(widget);
    ...
    if (gimp_save_procedure_get_support_thumbnail(save_procedure)) {
      widget =
          gimp_prop_check_button_new(G_OBJECT(config), "save-thumbnail", NULL);
      gtk_grid_attach(GTK_GRID(grid), widget, left, top, 6 / n_metadata, 1);
      left += 6 / n_metadata;
      gtk_widget_show(widget);
    }
    ...
    left = 0;
    for (iter = save_dialog->priv->additional_metadata; iter;
         iter = iter->next) {
      widget =
          gimp_procedure_dialog_get_widget(dialog, iter->data, G_TYPE_NONE);
      g_object_ref(widget);
    ...
    if (gimp_save_procedure_get_support_comment(save_procedure)) {
      ...
      title =
          gimp_prop_check_button_new(G_OBJECT(config), "save-comment", NULL);
      gtk_frame_set_label_widget(GTK_FRAME(frame2), title);
      gtk_widget_show(title);

      buffer = gimp_prop_text_buffer_new(G_OBJECT(config), "gimp-comment", -1);
...
static gpointer gimp_save_procedure_dialog_edit_metadata_thread(gpointer data) {
  ...
  gimp_pdb_run_procedure(gimp_get_pdb(), "plug-in-metadata-editor",
                         ...
                         GIMP_TYPE_IMAGE, dialog->priv->image, G_TYPE_NONE);
...
static gboolean gimp_save_procedure_dialog_activate_edit_metadata(
  ...
  if (!dialog->priv->metadata_thread)
    ...
    dialog->priv->metadata_thread = g_thread_try_new(
        "Edit Metadata", gimp_save_procedure_dialog_edit_metadata_thread,
        dialog, NULL);
...
GtkWidget *gimp_save_procedure_dialog_new(GimpSaveProcedure *procedure,
  ...
  if (!format_name) {
    g_critical("%s: no format name set on file procedure '%s'. "
               ...
               G_STRFUNC, gimp_procedure_get_name(GIMP_PROCEDURE(procedure)));
    return NULL;
  }
  ...
  dialog = g_object_new(GIMP_TYPE_SAVE_PROCEDURE_DIALOG, "procedure", procedure,
                        "config", config, "title", title, "help-func",
                        gimp_standard_help_func, "help-id", help_id,
...
void gimp_save_procedure_dialog_add_metadata(GimpSaveProcedureDialog *dialog,
  ...
  if (!g_list_find(dialog->priv->additional_metadata, property))
    dialog->priv->additional_metadata =
        g_list_append(dialog->priv->additional_metadata, g_strdup(property));
}
