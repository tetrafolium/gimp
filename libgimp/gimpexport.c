...
typedef void (*ExportFunc)(GimpImage *image, GList **drawables);
...
static void export_merge(GimpImage *image, GList **drawables) {
  GList *layers;
  GList *iter;
  gint32 nvisible = 0;
  ...
  if (nvisible <= 1) {
    ...
    transp = gimp_layer_new(image, "-", gimp_image_width(image),
                            gimp_image_height(image), layer_type, 100.0,
  ...
  if (nvisible > 1) {
    ...
    *drawables = g_list_prepend(NULL, merged);
    ...
    if (gimp_drawable_width(GIMP_DRAWABLE(merged)) != gimp_image_width(image) ||
      ...
      gimp_layer_resize(merged, gimp_image_width(image),
                        gimp_image_height(image), off_x, off_y);
    }
  }
...
static void export_flatten(GimpImage *image, GList **drawables) {
  GimpLayer *flattened;
...
static void export_remove_alpha(GimpImage *image, GList **drawables) {
  GList *layers;
  GList *iter;
...
static void export_apply_masks(GimpImage *image, GList **drawables) {
  GList *layers;
  GList *iter;
...
static void export_convert_rgb(GimpImage *image, GList **drawables) {
  gimp_image_convert_rgb(image);
}

static void export_convert_grayscale(GimpImage *image, GList **drawables) {
  gimp_image_convert_grayscale(image);
}

static void export_convert_indexed(GimpImage *image, GList **drawables) {
  GList *layers;
  GList *iter;
  ...
  if (layers || has_alpha)
    gimp_image_convert_indexed(image, GIMP_CONVERT_DITHER_NONE,
                               GIMP_CONVERT_PALETTE_GENERATE, 255, FALSE, FALSE,
                               "");
  else
    gimp_image_convert_indexed(image, GIMP_CONVERT_DITHER_NONE,
                               GIMP_CONVERT_PALETTE_GENERATE, 256, FALSE, FALSE,
                               "");
  g_list_free(layers);
}

static void export_convert_bitmap(GimpImage *image, GList **drawables) {
  if (gimp_image_base_type(image) == GIMP_INDEXED)
    gimp_image_convert_rgb(image);

  gimp_image_convert_indexed(image, GIMP_CONVERT_DITHER_FS,
                             GIMP_CONVERT_PALETTE_GENERATE, 2, FALSE, FALSE,
                             "");
}

static void export_add_alpha(GimpImage *image, GList **drawables) {
  GList *layers;
  GList *iter;
...
static void export_crop_image(GimpImage *image, GList **drawables) {
  gimp_image_crop(image, gimp_image_width(image), gimp_image_height(image), 0,
                  0);
}

static void export_resize_image(GimpImage *image, GList **drawables) {
  gimp_image_resize_to_layers(image);
}

static void export_void(GimpImage *image, GList **drawables) {
  /* do nothing */
}

/* a set of predefined actions */

static ExportAction export_action_merge = {export_merge,
                                           NULL,
...
static ExportFunc export_action_get_func(const ExportAction *action) {
...
static void export_action_perform(const ExportAction *action, GimpImage *image,
                                  GList **drawables) {
...
static void export_toggle_callback(GtkWidget *widget, gpointer data) {
...
static GimpExportReturn confirm_save_dialog(const gchar *message,
  ...
  dialog = gimp_dialog_new(
      _("Confirm Save"), "gimp-export-image-confirm", NULL, 0,
      gimp_standard_help_func, "gimp-export-confirm-dialog",

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("C_onfirm"), GTK_RESPONSE_OK,
  ...
  gimp_label_set_attributes(GTK_LABEL(label), PANGO_ATTR_SCALE,
                            PANGO_SCALE_LARGE, PANGO_ATTR_WEIGHT,
                            PANGO_WEIGHT_BOLD, -1);
...
static GimpExportReturn export_dialog(GSList *actions,
                                      const gchar *format_name) {
  GtkWidget *dialog;
  GtkWidget *hbox;
  GtkWidget *image;
  ...
  dialog = gimp_dialog_new(_("Export File"), "gimp-export-image", NULL, 0,
                           gimp_standard_help_func, "gimp-export-dialog",

                           _("_Ignore"), GTK_RESPONSE_NO, _("_Cancel"),
                           GTK_RESPONSE_CANCEL, _("_Export"), GTK_RESPONSE_OK,

                           NULL);

  gimp_dialog_set_alternative_button_order(GTK_DIALOG(dialog), GTK_RESPONSE_NO,
  ...
  for (list = actions; list; list = g_slist_next(list)) {
    ...
    if (action->possibilities[0] && action->possibilities[1]) {
      ...
      button = gtk_radio_button_new_with_label(
          radio_group, gettext(action->possibilities[0]));
      gtk_label_set_justify(GTK_LABEL(gtk_bin_get_child(GTK_BIN(button))),
                            GTK_JUSTIFY_LEFT);
      radio_group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(button));
      gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 0);
      g_signal_connect(button, "toggled", G_CALLBACK(export_toggle_callback),
                       &action->choice);
      ...
      button = gtk_radio_button_new_with_label(
          radio_group, gettext(action->possibilities[1]));
      gtk_label_set_justify(GTK_LABEL(gtk_bin_get_child(GTK_BIN(button))),
                            GTK_JUSTIFY_LEFT);
      radio_group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(button));
      gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 0);
...
GimpExportReturn gimp_export_image(GimpImage **image, gint *n_drawables,
  ...
  for (i = 0; i < *n_drawables; i++) {
    ...
    if (drawables_type == G_TYPE_NONE ||
      ...
      g_return_val_if_fail(
          g_type_is_a(G_OBJECT_TYPE((*drawables)[i]), drawables_type), FALSE);
  }
  ...
  if (interactive && !g_type_is_a(drawables_type, GIMP_TYPE_LAYER) &&
      !(capabilities & GIMP_EXPORT_CAN_HANDLE_LAYERS)) {
  ...
  if (!added_flatten) {
    ...
    if ((capabilities & GIMP_EXPORT_CAN_HANDLE_LAYERS) &&
      ...
      for (iter = layers; iter; iter = iter->next) {
        ...
        gimp_drawable_offsets(drawable, &layer_bounds.x, &layer_bounds.y);
      ...
      if (needs_crop) {
        actions = g_slist_prepend(actions, &export_action_crop_or_resize);
      }
    }

    /* check if layer size != canvas size, opacity != 100%, or offsets != 0 */
    if (g_list_length(layers) == 1 && !children &&
      ...
      if ((gimp_layer_get_opacity(GIMP_LAYER(drawable)) < 100.0) ||
          (gimp_image_width(*image) != gimp_drawable_width(drawable)) ||
          (gimp_image_height(*image) != gimp_drawable_height(drawable)) ||
          offset_x || offset_y) {
        if (capabilities & GIMP_EXPORT_CAN_HANDLE_ALPHA) {
    ...
    /* check multiple layers */
      ...
      if (capabilities & GIMP_EXPORT_CAN_HANDLE_LAYERS_AS_ANIMATION) {
        ...
        if (capabilities & GIMP_EXPORT_NEEDS_ALPHA)
          ...
          actions = g_slist_prepend(actions, &export_action_merge_or_flatten);
      }
    }
    ...
    /* check layer masks */
    if (has_layer_masks && !(capabilities & GIMP_EXPORT_CAN_HANDLE_LAYER_MASKS))
  ...
  case GIMP_RGB:
    if (!(capabilities & GIMP_EXPORT_CAN_HANDLE_RGB)) {
      if ((capabilities & GIMP_EXPORT_CAN_HANDLE_INDEXED) &&
        ...
        actions = g_slist_prepend(actions, &export_action_convert_bitmap);
    }
    break;
  ...
  case GIMP_GRAY:
    if (!(capabilities & GIMP_EXPORT_CAN_HANDLE_GRAY)) {
      if ((capabilities & GIMP_EXPORT_CAN_HANDLE_RGB) &&
        ...
        actions =
            g_slist_prepend(actions, &export_action_convert_rgb_or_indexed);
  ...
  case GIMP_INDEXED:
    if (!(capabilities & GIMP_EXPORT_CAN_HANDLE_INDEXED)) {
      if ((capabilities & GIMP_EXPORT_CAN_HANDLE_RGB) &&
          (capabilities & GIMP_EXPORT_CAN_HANDLE_GRAY))
        actions =
            g_slist_prepend(actions, &export_action_convert_rgb_or_grayscale);
      else if (capabilities & GIMP_EXPORT_CAN_HANDLE_RGB)
        actions = g_slist_prepend(actions, &export_action_convert_rgb);
      else if (capabilities & GIMP_EXPORT_CAN_HANDLE_GRAY)
        actions = g_slist_prepend(actions, &export_action_convert_grayscale);
...
GtkWidget *gimp_export_dialog_new(const gchar *format_name, const gchar *role,
  ...
  dialog = gimp_dialog_new(
      title, role, NULL, 0, gimp_standard_help_func, help_id,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Export"), GTK_RESPONSE_OK,
