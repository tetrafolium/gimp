...
static void gimp_message_func(const gchar *log_domain, GLogLevelFlags log_level,
                              const gchar *message, gpointer data);
static void gimp_fatal_func(const gchar *log_domain, GLogLevelFlags flags,
                            const gchar *message, gpointer data);
...
void _gimp_debug_init(const gchar *basename) {
  ...
  if (is_debug_name_match_basename && debug_options) {
    _gimp_debug_flags = g_parse_debug_string(debug_options + 1, gimp_debug_keys,
...
guint _gimp_get_debug_flags(void) { return _gimp_debug_flags; }
...
void _gimp_debug_configure(GimpStackTraceMode stack_trace_mode) {
  ...
  for (i = 0; i < G_N_ELEMENTS(gimp_log_domains); i++)
    g_log_set_handler(gimp_log_domains[i], G_LOG_LEVEL_MESSAGE,
  ...
  g_log_set_handler(NULL, fatal_mask, gimp_fatal_func, NULL);

  for (i = 0; i < G_N_ELEMENTS(gimp_log_domains); i++)
    g_log_set_handler(gimp_log_domains[i], fatal_mask, gimp_fatal_func, NULL);
  for (i = 0; i < G_N_ELEMENTS(glib_log_domains); i++)
    g_log_set_handler(glib_log_domains[i], fatal_mask, gimp_fatal_func, NULL);
}
...
void _gimp_debug_stop(void) {
  ...
  g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "Debugging (restart externally): %ld",
        (long int)opid);
...
static void gimp_fatal_func(const gchar *log_domain, GLogLevelFlags flags,
   ...
   */
  g_printerr("Plugin %s: %s: %s: %s\n", g_get_prgname(), log_domain, level,
             message);
