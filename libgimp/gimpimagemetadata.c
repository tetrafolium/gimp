 ...
 * Since: 3.0
 */
void gimp_image_metadata_load_finish(GimpImage *image, const gchar *mime_type,
                                     GimpMetadata *metadata,
  ...
  if (flags & GIMP_METADATA_LOAD_COMMENT) {
    ...
    comment = gexiv2_metadata_get_tag_interpreted_string(
 ...
 * Since: 2.10
 */
GimpImage *gimp_image_metadata_load_thumbnail(GFile *file, GError **error) {
  GimpMetadata *metadata;
  ...
  input_stream = g_memory_input_stream_new_from_data(
      thumbnail_buffer, thumbnail_size, (GDestroyNotify)g_free);
  ...
  if (pixbuf) {
    ...
    image = gimp_image_new(gdk_pixbuf_get_width(pixbuf),
                           gdk_pixbuf_get_height(pixbuf), GIMP_RGB);
    gimp_image_undo_disable(image);

    layer = gimp_layer_new_from_pixbuf(
        image, _("Background"), pixbuf, 100.0,
        gimp_image_get_default_new_layer_mode(image), 0.0, 0.0);
    g_object_unref(pixbuf);

    gimp_image_insert_layer(image, layer, NULL, 0);

    gimp_image_metadata_rotate(
        image, gexiv2_metadata_get_orientation(GEXIV2_METADATA(metadata)));
  }
