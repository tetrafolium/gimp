...
enum { PATTERN_SET, LAST_SIGNAL };

enum { PROP_0, PROP_TITLE, PROP_PATTERN_NAME };
...
static void gimp_pattern_select_button_callback(
    const gchar *pattern_name, gint width, gint height, gint bytes,
    const guchar *mask_data, gboolean dialog_closing, gpointer user_data);

static void gimp_pattern_select_preview_resize(GimpPatternSelectButton *button);
static gboolean
gimp_pattern_select_preview_events(GtkWidget *widget, GdkEvent *event,
                                   GimpPatternSelectButton *button);
static void gimp_pattern_select_preview_update(GtkWidget *preview, gint width,
                                               gint height, gint bytes,
                                               const guchar *mask_data);

static void
gimp_pattern_select_button_open_popup(GimpPatternSelectButton *button, gint x,
                                      gint y);
static void
gimp_pattern_select_button_close_popup(GimpPatternSelectButton *button);

static void gimp_pattern_select_drag_data_received(
    GimpPatternSelectButton *button, GdkDragContext *context, gint x, gint y,
    GtkSelectionData *selection, guint info, guint time);

static GtkWidget *
gimp_pattern_select_button_create_inside(GimpPatternSelectButton *button);
...
gimp_pattern_select_button_class_init(GimpPatternSelectButtonClass *klass) {
   ...
   */
  g_object_class_install_property(
      object_class, PROP_PATTERN_NAME,
      g_param_spec_string("pattern-name", "Pattern name",
                          "The name of the currently selected pattern", NULL,
   ...
   * Since: 2.4
   */
  pattern_button_signals[PATTERN_SET] = g_signal_new(
      "pattern-set", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpPatternSelectButtonClass, pattern_set), NULL, NULL,
      _gimpui_marshal_VOID__STRING_INT_INT_INT_POINTER_BOOLEAN, G_TYPE_NONE, 6,
      G_TYPE_STRING, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_POINTER,
      G_TYPE_BOOLEAN);
}

static void gimp_pattern_select_button_init(GimpPatternSelectButton *button) {
  gint mask_data_size;

  button->priv = gimp_pattern_select_button_get_instance_private(button);

  button->priv->pattern_name = gimp_context_get_pattern();
  gimp_pattern_get_pixels(button->priv->pattern_name, &button->priv->width,
                          &button->priv->height, &button->priv->bytes,
                          &mask_data_size, &button->priv->mask_data);
...
GtkWidget *gimp_pattern_select_button_new(const gchar *title,
  ...
  if (title)
    button = g_object_new(GIMP_TYPE_PATTERN_SELECT_BUTTON, "title", title,
                          "pattern-name", pattern_name, NULL);
  else
    button = g_object_new(GIMP_TYPE_PATTERN_SELECT_BUTTON, "pattern-name",
                          pattern_name, NULL);
...
void gimp_pattern_select_button_set_pattern(GimpPatternSelectButton *button,
  ...
  if (select_button->temp_callback) {
    ...
    if (gimp_pattern_get_pixels(name, &width, &height, &bytes, &mask_data_size,
                                &mask_data)) {
      gimp_pattern_select_button_callback(name, width, height, bytes, mask_data,
                                          FALSE, button);
...
static void gimp_pattern_select_button_set_property(GObject *object,
  ...
  case PROP_PATTERN_NAME:
    gimp_pattern_select_button_set_pattern(button, g_value_get_string(value));
    break;
...
static void gimp_pattern_select_button_callback(
  ...
  gimp_pattern_select_preview_update(button->priv->preview, width, height,
                                     bytes, mask_data);

  if (dialog_closing)
    select_button->temp_callback = NULL;

  g_signal_emit(button, pattern_button_signals[PATTERN_SET], 0, pattern_name,
                width, height, bytes, dialog_closing);
...
gimp_pattern_select_button_clicked(GimpPatternSelectButton *button) {
  ...
  if (select_button->temp_callback) {
    ...
    select_button->temp_callback = gimp_pattern_select_new(
        button->priv->title, button->priv->pattern_name,
        gimp_pattern_select_button_callback, button, NULL);
  }
}
...
gimp_pattern_select_preview_resize(GimpPatternSelectButton *button) {
  if (button->priv->width > 0 && button->priv->height > 0)
    gimp_pattern_select_preview_update(
        button->priv->preview, button->priv->width, button->priv->height,
        button->priv->bytes, button->priv->mask_data);
}
...
gimp_pattern_select_preview_events(GtkWidget *widget, GdkEvent *event,
  ...
  if (button->priv->mask_data) {
    ...
    case GDK_BUTTON_PRESS:
      ...
      if (bevent->button == 1) {
        gtk_grab_add(widget);
        gimp_pattern_select_button_open_popup(button, bevent->x, bevent->y);
      }
      break;
...
static void gimp_pattern_select_preview_update(GtkWidget *preview, gint width,
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(preview), 0, 0, width, height, type,
                         mask_data, width * bytes);
}
...
gimp_pattern_select_button_open_popup(GimpPatternSelectButton *button, gint x,
  ...
  /*  Draw the pattern  */
  gimp_pattern_select_preview_update(preview, priv->width, priv->height,
                                     priv->bytes, priv->mask_data);
}
...
static GtkWidget *gimp_pattern_select_button_create_inside(
