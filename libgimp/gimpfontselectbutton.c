...
enum { PROP_0, PROP_TITLE, PROP_FONT_NAME };
...
gimp_font_select_button_class_init(GimpFontSelectButtonClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "title", "Title",
          "The title to be used for the font selection popup dialog",
          _("Font Selection"), GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
   ...
   */
  g_object_class_install_property(
      object_class, PROP_FONT_NAME,
      g_param_spec_string("font-name", "Font name",
                          "The name of the currently selected font",
                          "Sans-serif", GIMP_PARAM_READWRITE));
  ...
  font_button_signals[FONT_SET] =
      g_signal_new("font-set", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpFontSelectButtonClass, font_set), NULL,
                   NULL, _gimpui_marshal_VOID__STRING_BOOLEAN, G_TYPE_NONE, 2,
                   G_TYPE_STRING, G_TYPE_BOOLEAN);
}

static void gimp_font_select_button_init(GimpFontSelectButton *button) {
...
GtkWidget *gimp_font_select_button_new(const gchar *title,
  ...
  if (title)
    button = g_object_new(GIMP_TYPE_FONT_SELECT_BUTTON, "title", title,
                          "font-name", font_name, NULL);
  else
    button = g_object_new(GIMP_TYPE_FONT_SELECT_BUTTON, "font-name", font_name,
                          NULL);
...
static void gimp_font_select_button_set_property(GObject *object,
  ...
  case PROP_FONT_NAME:
    gimp_font_select_button_set_font(button, g_value_get_string(value));
    break;
...
static void gimp_font_select_button_callback(const gchar *font_name,
  ...
  g_signal_emit(button, font_button_signals[FONT_SET], 0, font_name,
                dialog_closing);
...
static void gimp_font_select_button_clicked(GimpFontSelectButton *button) {
  ...
  if (select_button->temp_callback) {
    ...
    gimp_fonts_set_popup(select_button->temp_callback, button->priv->font_name);
  } else {
    select_button->temp_callback =
        gimp_font_select_new(button->priv->title, button->priv->font_name,
                             gimp_font_select_button_callback, button, NULL);
  }
}
...
gimp_font_select_button_create_inside(GimpFontSelectButton *font_button) {
  ...
  image = gtk_image_new_from_icon_name(GIMP_ICON_FONT, GTK_ICON_SIZE_BUTTON);
