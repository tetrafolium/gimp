...
GParamSpec *_gimp_gp_param_def_to_param_spec(const GPParamDef *param_def) {
  ...
  case GP_PARAM_DEF_TYPE_INT:
    if (!strcmp(param_def->type_name, "GParamInt"))
      return g_param_spec_int(name, nick, blurb, param_def->meta.m_int.min_val,
                              param_def->meta.m_int.max_val,
                              param_def->meta.m_int.default_val, flags);

    if (!strcmp(param_def->type_name, "GParamUInt"))
      return g_param_spec_uint(name, nick, blurb, param_def->meta.m_int.min_val,
                               param_def->meta.m_int.max_val,
                               param_def->meta.m_int.default_val, flags);
  ...
  case GP_PARAM_DEF_TYPE_UNIT:
    if (!strcmp(param_def->type_name, "GimpParamUnit"))
      return gimp_param_spec_unit(name, nick, blurb,
                                  ...
                                  param_def->meta.m_unit.default_val, flags);
    break;
  ...
  case GP_PARAM_DEF_TYPE_ENUM:
    if (!strcmp(param_def->type_name, "GParamEnum"))
      return g_param_spec_enum(name, nick, blurb,
                               g_type_from_name(param_def->value_type_name),
                               param_def->meta.m_enum.default_val, flags);
  ...
  case GP_PARAM_DEF_TYPE_BOOLEAN:
    if (!strcmp(param_def->type_name, "GParamBoolean"))
      return g_param_spec_boolean(name, nick, blurb,
                                  param_def->meta.m_boolean.default_val, flags);
    break;
  ...
  case GP_PARAM_DEF_TYPE_COLOR:
    if (!strcmp(param_def->type_name, "GimpParamRGB"))
      return gimp_param_spec_rgb(name, nick, blurb,
                                 ...
                                 &param_def->meta.m_color.default_val, flags);
  ...
  case GP_PARAM_DEF_TYPE_ID_ARRAY:
    if (!strcmp(param_def->type_name, "GimpParamObjectArray"))
      return gimp_param_spec_object_array(
          name, nick, blurb,
          g_type_from_name(param_def->meta.m_id_array.type_name), flags);
    break;
  }
...
void _gimp_param_spec_to_gp_param_def(GParamSpec *pspec,
  ...
  if (pspec_type == G_TYPE_PARAM_INT) {
    ...
    param_def->meta.m_color.has_alpha = gimp_param_spec_rgb_has_alpha(pspec);
  ...
  } else if (pspec_type == G_TYPE_PARAM_OBJECT && value_type != G_TYPE_FILE) {
...
static GimpImage *get_image_by_id(gpointer gimp, gint id) {
...
static GimpItem *get_item_by_id(gpointer gimp, gint id) {
#ifdef LIBGIMP_COMPILATION
...
static GimpDisplay *get_display_by_id(gpointer gimp, gint id) {
#ifdef LIBGIMP_COMPILATION
...
static void gimp_gp_param_to_value(gpointer gimp, const GPParam *param,
  ...
  if (type == G_TYPE_INT || type == GIMP_TYPE_UNIT) {
    ...
    gimp_value_set_float_array(value, (const gdouble *)param->data.d_array.data,
    ...
    gimp_value_set_rgb_array(value, (GimpRGB *)param->data.d_array.data,
...
static void gimp_value_to_gp_param(const GValue *value, GPParam *param,
                                   gboolean full_copy) {
  GType type;
  ...
  if (type == G_TYPE_INT || type == GIMP_TYPE_UNIT) {
    ...
    GimpParasite *parasite =
        (full_copy ? g_value_dup_boxed(value) : g_value_get_boxed(value));
    ...
    if (array) {
      ...
      if (full_copy)
        param->data.d_array.data = g_memdup(array->data, array->length);
      else
    ...
    if (array) {
      ...
      if (full_copy)
        param->data.d_string_array.data = gimp_value_dup_string_array(value);
      else
...
void _gimp_gp_params_free(GPParam *params, gint n_params, gboolean full_copy) {
  ...
  for (i = 0; i < n_params; i++) {
    ...
    case GP_PARAM_TYPE_STRING_ARRAY:
      if (full_copy && params[i].data.d_string_array.size > 0 &&
