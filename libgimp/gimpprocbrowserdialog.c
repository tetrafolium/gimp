...
enum { SELECTION_CHANGED, ROW_ACTIVATED, LAST_SIGNAL };
...
enum { COLUMN_PROC_NAME, N_COLUMNS };
...
static void browser_row_activated(GtkTreeView *treeview, GtkTreePath *path,
                                  GtkTreeViewColumn *column,
                                  GimpProcBrowserDialog *dialog);
static void browser_search(GimpBrowser *browser, const gchar *query_text,
                           gint search_type, GimpProcBrowserDialog *dialog);
...
gimp_proc_browser_dialog_class_init(GimpProcBrowserDialogClass *klass) {
   ...
   */
  dialog_signals[SELECTION_CHANGED] = g_signal_new(
      "selection-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpProcBrowserDialogClass, selection_changed), NULL,
  ...
  dialog_signals[ROW_ACTIVATED] =
      g_signal_new("row-activated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpProcBrowserDialogClass, row_activated),
...
static void gimp_proc_browser_dialog_init(GimpProcBrowserDialog *dialog) {
  ...
  gimp_browser_add_search_types(
      GIMP_BROWSER(priv->browser), _("by name"), SEARCH_TYPE_NAME,
      _("by description"), SEARCH_TYPE_BLURB, _("by help"), SEARCH_TYPE_HELP,
      _("by authors"), SEARCH_TYPE_AUTHORS, _("by copyright"),
      SEARCH_TYPE_COPYRIGHT, _("by date"), SEARCH_TYPE_DATE, _("by type"),
      SEARCH_TYPE_PROC_TYPE, NULL);
  ...
  g_signal_connect(priv->browser, "search", G_CALLBACK(browser_search), dialog);
  ...
  gtk_box_pack_start(
      GTK_BOX(gimp_browser_get_left_vbox(GIMP_BROWSER(priv->browser))),
  ...
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(priv->tree_view), -1, NULL, renderer, "text", 0, NULL);
  gtk_tree_view_set_headers_visible(GTK_TREE_VIEW(priv->tree_view), FALSE);

  g_signal_connect(priv->tree_view, "row_activated",
                   G_CALLBACK(browser_row_activated), dialog);
  ...
  g_signal_connect(selection, "changed", G_CALLBACK(browser_selection_changed),
                   dialog);

  parent = gtk_widget_get_parent(
      gimp_browser_get_right_vbox(GIMP_BROWSER(priv->browser)));
  parent = gtk_widget_get_parent(parent);

  gtk_widget_set_size_request(parent, DBL_WIDTH - DBL_LIST_WIDTH, -1);

  /* first search (all procedures) */
  browser_search(GIMP_BROWSER(dialog->priv->browser), "", SEARCH_TYPE_ALL,
                 dialog);
}
 ...
 * Since: 2.4
 **/
GtkWidget *gimp_proc_browser_dialog_new(const gchar *title, const gchar *role,
  ...
  dialog = g_object_new(GIMP_TYPE_PROC_BROWSER_DIALOG, "title", title, "role",
                        role, "help-func", help_func, "help-id", help_id,
...
static void browser_search(GimpBrowser *browser, const gchar *query_text,
  ...
  case SEARCH_TYPE_NAME: {
    ...
    proc_list = gimp_pdb_query_procedures(pdb, query->str, ".*", ".*", ".*",
                                          ".*", ".*", ".*", ".*", &num_procs);
  ...
  case SEARCH_TYPE_BLURB:
    ...
    proc_list = gimp_pdb_query_procedures(pdb, ".*", query_text, ".*", ".*",
                                          ".*", ".*", ".*", ".*", &num_procs);
    break;
  ...
  case SEARCH_TYPE_AUTHORS:
    ...
    proc_list = gimp_pdb_query_procedures(
        pdb, ".*", ".*", ".*", ".*", query_text, ".*", ".*", ".*", &num_procs);
    break;
  ...
  if (!query_text || strlen(query_text) == 0) {
    str = g_strdup_printf(dngettext(GETTEXT_PACKAGE "-libgimp", "%d procedure",
                                    "%d procedures", num_procs),
                          num_procs);
  } else {
    switch (num_procs) {
    case 0:
  ...
  if (num_procs > 0) {
    ...
    for (i = 0; i < num_procs; i++) {
      ...
      gtk_list_store_set(priv->store, &iter, COLUMN_PROC_NAME, proc_list[i],
                         -1);
    }
    ...
    gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(priv->store),
                                         COLUMN_PROC_NAME, GTK_SORT_ASCENDING);
