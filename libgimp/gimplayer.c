...
GimpLayer *gimp_layer_new(GimpImage *image, const gchar *name, gint width,
                          ...
                          GimpLayerMode mode) {
  return _gimp_layer_new(image, width, height, type, name, opacity, mode);
}
...
GimpLayer *gimp_layer_new_from_pixbuf(GimpImage *image, const gchar *name,
  ...
  layer = gimp_layer_new(image, name, width, height,
                         bpp == 3 ? GIMP_RGB_IMAGE : GIMP_RGBA_IMAGE, opacity,
                         mode);
  ...
  gegl_buffer_set(dest_buffer, GEGL_RECTANGLE(0, 0, width, height), 0,
                  gimp_pixbuf_get_format(pixbuf), gdk_pixbuf_get_pixels(pixbuf),
...
GimpLayer *gimp_layer_new_from_surface(GimpImage *image, const gchar *name,
  ...
  layer = gimp_layer_new(image, name, width, height,
                         format == CAIRO_FORMAT_RGB24 ? GIMP_RGB_IMAGE
                                                      : GIMP_RGBA_IMAGE,
                         100.0, gimp_image_get_default_new_layer_mode(image));
