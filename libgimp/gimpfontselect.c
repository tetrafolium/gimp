...
const gchar *gimp_font_select_new(const gchar *title, const gchar *font_name,
  ...
  procedure = gimp_procedure_new(
      plug_in, font_callback, GIMP_PDB_PROC_TYPE_TEMPORARY, gimp_temp_font_run,
      font_data, (GDestroyNotify)gimp_font_data_free);

  GIMP_PROC_ARG_STRING(procedure, "font-name", "Font name", "The font name",
...
static GimpValueArray *gimp_temp_font_run(GimpProcedure *procedure,
  ...
  if (!data->idle_id)
    data->idle_id = g_idle_add((GSourceFunc)gimp_temp_font_idle, data);
...
static gboolean gimp_temp_font_idle(GimpFontData *data) {
  ...
  if (data->callback)
    data->callback(data->font_name, data->closing, data->data);
