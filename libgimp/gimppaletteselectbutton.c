...
enum { PALETTE_SET, LAST_SIGNAL };

enum { PROP_0, PROP_TITLE, PROP_PALETTE_NAME };
...
static void gimp_palette_select_drag_data_received(
    GimpPaletteSelectButton *button, GdkDragContext *context, gint x, gint y,
    GtkSelectionData *selection, guint info, guint time);

static GtkWidget *gimp_palette_select_button_create_inside(
...
gimp_palette_select_button_class_init(GimpPaletteSelectButtonClass *klass) {
   ...
   */
  g_object_class_install_property(
      object_class, PROP_PALETTE_NAME,
      g_param_spec_string("palette-name", "Palette name",
                          "The name of the currently selected palette", NULL,
  ...
  palette_button_signals[PALETTE_SET] =
      g_signal_new("palette-set", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpPaletteSelectButtonClass, palette_set),
                   NULL, NULL, _gimpui_marshal_VOID__STRING_BOOLEAN,
                   G_TYPE_NONE, 2, G_TYPE_STRING, G_TYPE_BOOLEAN);
}

static void gimp_palette_select_button_init(GimpPaletteSelectButton *button) {
...
GtkWidget *gimp_palette_select_button_new(const gchar *title,
  ...
  if (title)
    button = g_object_new(GIMP_TYPE_PALETTE_SELECT_BUTTON, "title", title,
                          "palette-name", palette_name, NULL);
  else
    button = g_object_new(GIMP_TYPE_PALETTE_SELECT_BUTTON, "palette-name",
                          palette_name, NULL);
...
static void gimp_palette_select_button_set_property(GObject *object,
  ...
  case PROP_PALETTE_NAME:
    gimp_palette_select_button_set_palette(button, g_value_get_string(value));
    break;
...
static void gimp_palette_select_button_callback(const gchar *palette_name,
  ...
  g_signal_emit(button, palette_button_signals[PALETTE_SET], 0, palette_name,
                dialog_closing);
...
gimp_palette_select_button_clicked(GimpPaletteSelectButton *button) {
  ...
  if (select_button->temp_callback) {
    ...
    select_button->temp_callback = gimp_palette_select_new(
        button->priv->title, button->priv->palette_name,
        gimp_palette_select_button_callback, button, NULL);
  }
}
...
static GtkWidget *gimp_palette_select_button_create_inside(
  ...
  image = gtk_image_new_from_icon_name(GIMP_ICON_PALETTE, GTK_ICON_SIZE_BUTTON);
  gtk_box_pack_start(GTK_BOX(hbox), image, FALSE, FALSE, 0);

  palette_button->priv->label =
      gtk_label_new(palette_button->priv->palette_name);
