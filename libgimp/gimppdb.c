...
static void gimp_pdb_set_error(GimpPDB *pdb, GimpValueArray *return_values);
 ...
 * Since: 3.0
 **/
gboolean gimp_pdb_procedure_exists(GimpPDB *pdb, const gchar *procedure_name) {
...
GimpProcedure *gimp_pdb_lookup_procedure(GimpPDB *pdb,
  ...
  if (!procedure) {
    ...
    if (procedure)
      g_hash_table_insert(pdb->priv->procedures, g_strdup(procedure_name),
                          procedure);
  }
...
GimpValueArray *gimp_pdb_run_procedure(GimpPDB *pdb,
  ...
  return_values =
      gimp_pdb_run_procedure_valist(pdb, procedure_name, first_type, args);
...
GimpValueArray *gimp_pdb_run_procedure_valist(GimpPDB *pdb,
  ...
  arguments =
      gimp_value_array_new_from_types_valist(&error_msg, first_type, args);
  ...
  if (!arguments) {
    ...
    return gimp_procedure_new_return_values(NULL, GIMP_PDB_CALLING_ERROR,
                                            error);
  }

  return_values = gimp_pdb_run_procedure_array(pdb, procedure_name, arguments);
...
GimpValueArray *gimp_pdb_run_procedure_array(GimpPDB *pdb,
  ...
  return_values = _gimp_gp_params_to_value_array(
      NULL, NULL, 0, proc_return->params, proc_return->n_params, TRUE);
 ...
 * Since: 3.0
 **/
gboolean gimp_pdb_dump_to_file(GimpPDB *pdb, GFile *file) {
  g_return_val_if_fail(GIMP_IS_PDB(pdb), FALSE);
 ...
 * @proc_type:   The regex for procedure type: { 'Internal GIMP procedure',
 *'GIMP Plug-in', 'GIMP Extension', 'Temporary Procedure' }.
 ...
 * Since: 3.0
 **/
gchar **gimp_pdb_query_procedures(GimpPDB *pdb, const gchar *name,
                                  const gchar *blurb, const gchar *help,
                                  const gchar *help_id, const gchar *authors,
                                  const gchar *copyright, const gchar *date,
                                  const gchar *proc_type, gint *num_matches) {
  gchar **matches;

  g_return_val_if_fail(GIMP_IS_PDB(pdb), NULL);

  _gimp_pdb_query(name, blurb, help, /* FIXME help_id */
                  authors, copyright, date, proc_type, num_matches, &matches);

  return matches;
}

GQuark _gimp_pdb_error_quark(void) {
 ...
 * the identifier
 */
gboolean gimp_pdb_get_data(const gchar *identifier, gpointer data) {
  gint size;
  guint8 *hack;
  gboolean success;
 ...
 * Returns: TRUE on success.
 */
gboolean gimp_pdb_set_data(const gchar *identifier, gconstpointer data,
                           guint32 bytes) {
