...
static void gimp_image_combo_box_model_add(GtkListStore *store, GList *images,
                                           GimpImageConstraintFunc constraint,
                                           gpointer data);
...
GtkWidget *gimp_image_combo_box_new(GimpImageConstraintFunc constraint,
  ...
  combo_box =
      g_object_new(GIMP_TYPE_IMAGE_COMBO_BOX, "width-request", WIDTH_REQUEST,
  ...
  g_signal_connect(combo_box, "changed",
                   G_CALLBACK(gimp_image_combo_box_changed), NULL);
...
static void gimp_image_combo_box_populate(GimpImageComboBox *combo_box) {
  ...
  gimp_image_combo_box_model_add(GTK_LIST_STORE(model), images,
                                 combo_box->constraint, combo_box->data);
...
static void gimp_image_combo_box_model_add(GtkListStore *store, GList *images,
  ...
  for (list = images; list; list = g_list_next(list)) {
    ...
    if (!constraint || constraint(image, data)) {
      ...
      thumb = gimp_image_get_thumbnail(image, THUMBNAIL_SIZE, THUMBNAIL_SIZE,
                                       GIMP_PIXBUF_SMALL_CHECKS);

      gtk_list_store_append(store, &iter);
      gtk_list_store_set(store, &iter, GIMP_INT_STORE_VALUE, image_id,
...
static void gimp_image_combo_box_changed(GimpImageComboBox *combo_box) {
  ...
  if (gimp_int_combo_box_get_active(GIMP_INT_COMBO_BOX(combo_box), &image_ID)) {
