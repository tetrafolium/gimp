...
enum { GRADIENT_SET, LAST_SIGNAL };

enum { PROP_0, PROP_TITLE, PROP_GRADIENT_NAME };
...
static gboolean
gimp_gradient_select_preview_draw(GtkWidget *preview, cairo_t *cr,
                                  GimpGradientSelectButton *button);

static void gimp_gradient_select_drag_data_received(
    GimpGradientSelectButton *button, GdkDragContext *context, gint x, gint y,
    GtkSelectionData *selection, guint info, guint time);

static GtkWidget *
gimp_gradient_select_button_create_inside(GimpGradientSelectButton *button);
...
G_DEFINE_TYPE_WITH_PRIVATE(GimpGradientSelectButton,
                           gimp_gradient_select_button, GIMP_TYPE_SELECT_BUTTON)
...
gimp_gradient_select_button_class_init(GimpGradientSelectButtonClass *klass) {
   ...
   */
  g_object_class_install_property(
      object_class, PROP_GRADIENT_NAME,
      g_param_spec_string("gradient-name", "Gradient name",
                          "The name of the currently selected gradient", NULL,
   ...
   * Since: 2.4
   */
  gradient_button_signals[GRADIENT_SET] = g_signal_new(
      "gradient-set", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpGradientSelectButtonClass, gradient_set), NULL, NULL,
      _gimpui_marshal_VOID__STRING_INT_POINTER_BOOLEAN, G_TYPE_NONE, 4,
      G_TYPE_STRING, G_TYPE_INT, G_TYPE_POINTER, G_TYPE_BOOLEAN);
}

static void gimp_gradient_select_button_init(GimpGradientSelectButton *button) {
...
GtkWidget *gimp_gradient_select_button_new(const gchar *title,
  ...
  if (title)
    button = g_object_new(GIMP_TYPE_GRADIENT_SELECT_BUTTON, "title", title,
                          "gradient-name", gradient_name, NULL);
  else
    button = g_object_new(GIMP_TYPE_GRADIENT_SELECT_BUTTON, "gradient-name",
                          gradient_name, NULL);
...
void gimp_gradient_select_button_set_gradient(GimpGradientSelectButton *button,
  ...
  if (select_button->temp_callback) {
    ...
    if (gimp_gradient_get_uniform_samples(name, button->priv->sample_size,
                                          button->priv->reverse, &n_samples,
                                          &samples)) {
      gimp_gradient_select_button_callback(name, n_samples, samples, FALSE,
                                           button);
...
static void gimp_gradient_select_button_set_property(GObject *object,
  ...
  case PROP_GRADIENT_NAME:
    gimp_gradient_select_button_set_gradient(button, g_value_get_string(value));
    break;
...
static void gimp_gradient_select_button_callback(const gchar *gradient_name,
  ...
  button->priv->gradient_data =
      g_memdup(gradient_data, n_samples * sizeof(gdouble));
  ...
  g_signal_emit(button, gradient_button_signals[GRADIENT_SET], 0, gradient_name,
                n_samples, gradient_data, dialog_closing);
...
gimp_gradient_select_button_clicked(GimpGradientSelectButton *button) {
  ...
  if (select_button->temp_callback) {
    ...
    select_button->temp_callback = gimp_gradient_select_new(
        button->priv->title, button->priv->gradient_name,
        button->priv->sample_size, gimp_gradient_select_button_callback, button,
        NULL);
  }
}
...
gimp_gradient_select_preview_size_allocate(GtkWidget *widget,
  ...
  if (gimp_gradient_get_uniform_samples(
          button->priv->gradient_name, allocation->width, button->priv->reverse,
          &n_samples, &samples)) {
    g_free(button->priv->gradient_data);
...
static gboolean
gimp_gradient_select_preview_draw(GtkWidget *widget, cairo_t *cr,
                                  GimpGradientSelectButton *button) {
  GtkAllocation allocation;
  ...
  for (x = 0, dest = cairo_image_surface_get_data(surface); x < width;
       x++, src += 4, dest += 4) {
    GimpRGB color;
    guchar r, g, b, a;
...
static GtkWidget *gimp_gradient_select_button_create_inside(
