...
static void gimp_aspect_preview_get_property(GObject *object, guint property_id,
                                             GValue *value, GParamSpec *pspec);
static void gimp_aspect_preview_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
...
static void gimp_aspect_preview_draw_buffer(GimpPreview *preview,
                                            const guchar *buffer,
                                            gint rowstride);
static void gimp_aspect_preview_transform(GimpPreview *preview, gint src_x,
                                          gint src_y, gint *dest_x,
                                          gint *dest_y);
static void gimp_aspect_preview_untransform(GimpPreview *preview, gint src_x,
                                            gint src_y, gint *dest_x,
                                            gint *dest_y);
...
static void gimp_aspect_preview_constructed(GObject *object) {
  ...
  g_object_set_data_full(object, "gimp-aspect-preview-data-name", data_name,
...
static void gimp_aspect_preview_dispose(GObject *object) {
  ...
  data_name =
      g_object_get_data(G_OBJECT(object), "gimp-aspect-preview-data-name");
  if (data_name) {
...
static void gimp_aspect_preview_draw(GimpPreview *preview) {
  ...
  gimp_preview_area_fill(GIMP_PREVIEW_AREA(area), 0, 0, width, height, 0, 0, 0);
}
...
static void gimp_aspect_preview_draw_buffer(GimpPreview *preview,
  ...
  if (gimp_selection_is_empty(image)) {
    ...
    src = gimp_drawable_get_thumbnail_data(priv->drawable, &w, &h, &bpp);
    sel = gimp_drawable_get_thumbnail_data(GIMP_DRAWABLE(selection), &w, &h,
                                           &bpp);
...
static void gimp_aspect_preview_set_drawable(GimpAspectPreview *preview,
  ...
  if (height > 0)
    g_object_set(gimp_preview_get_frame(GIMP_PREVIEW(preview)), "ratio",
                 (gdouble)d_width / (gdouble)d_height, NULL);
}
...
GtkWidget *gimp_aspect_preview_new_from_drawable(GimpDrawable *drawable) {
  ...
  return g_object_new(GIMP_TYPE_ASPECT_PREVIEW, "drawable", drawable, NULL);
}
