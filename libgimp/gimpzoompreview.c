...
enum { PROP_0, PROP_DRAWABLE, PROP_MODEL };
...
static void gimp_zoom_preview_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);
static void gimp_zoom_preview_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
...
static void gimp_zoom_preview_draw_thumb(GimpPreview *preview,
                                         GimpPreviewArea *area, gint width,
                                         gint height);
static void gimp_zoom_preview_set_cursor(GimpPreview *preview);
static void gimp_zoom_preview_transform(GimpPreview *preview, gint src_x,
                                        gint src_y, gint *dest_x, gint *dest_y);
static void gimp_zoom_preview_untransform(GimpPreview *preview, gint src_x,
...
static void gimp_zoom_preview_get_source_area(GimpPreview *preview, gint *x,
...
static void gimp_zoom_preview_class_init(GimpZoomPreviewClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_DRAWABLE,
      g_param_spec_object(
          "drawable", "Drawable", "The drawable this preview is attached to",
          GIMP_TYPE_DRAWABLE, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("model", "Model", "The zoom preview's GimpZoomModel",
                          GIMP_TYPE_ZOOM_MODEL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_zoom_preview_init(GimpZoomPreview *preview) {
  ...
  g_signal_connect(area, "size-allocate",
                   G_CALLBACK(gimp_zoom_preview_size_allocate), preview);
  g_signal_connect(area, "scroll-event",
                   G_CALLBACK(gimp_zoom_preview_scroll_event), preview);

  g_object_set(area, "check-size", gimp_check_size(), "check-type",
               gimp_check_type(), NULL);
...
static void gimp_zoom_preview_constructed(GObject *object) {
  ...
  g_object_set_data_full(object, "gimp-zoom-preview-data-name", data_name,
...
static void gimp_zoom_preview_dispose(GObject *object) {
  const gchar *data_name =
      g_object_get_data(G_OBJECT(object), "gimp-zoom-preview-data-name");
...
static void gimp_zoom_preview_set_adjustments(GimpZoomPreview *preview,
  ...
  gtk_adjustment_configure(
      hadj,
      (gtk_adjustment_get_value(vadj) + width / 2.0) * ratio - width / 2.0, 0,
      width * new_factor, new_factor, MAX(width / 2.0, new_factor), width);

  gtk_adjustment_configure(
      vadj,
      (gtk_adjustment_get_value(vadj) + height / 2.0) * ratio - height / 2.0, 0,
      height * new_factor, new_factor, MAX(height / 2.0, new_factor), height);
...
static void gimp_zoom_preview_style_updated(GtkWidget *widget) {
  ...
  if (area) {
    ...
    if (_gimp_drawable_preview_get_bounds(priv->drawable, &x1, &y1, &x2, &y2)) {
      width = x2 - x1;
      height = y2 - y1;
    } else {
...
static void gimp_zoom_preview_draw(GimpPreview *preview) {
  ...
  data = gimp_zoom_preview_get_source(GIMP_ZOOM_PREVIEW(preview), &width,
                                      &height, &bpp);

  if (data) {
    GtkWidget *area = gimp_preview_get_area(preview);

    gimp_preview_area_draw(GIMP_PREVIEW_AREA(area), 0, 0, width, height,
                           gimp_drawable_type(priv->drawable), data,
                           width * bpp);
    g_free(data);
  }
}
...
static void gimp_zoom_preview_draw_buffer(GimpPreview *preview,
  ...
  if (gimp_selection_is_empty(image)) {
    gimp_preview_area_draw(GIMP_PREVIEW_AREA(area), 0, 0, width, height,
                           gimp_drawable_type(priv->drawable), buffer,
                           rowstride);
  } else {
    guchar *sel;
    guchar *src;
    ...
    gimp_zoom_preview_get_source_area(preview, &src_x, &src_y, &src_width,
                                      &src_height);

    src = gimp_drawable_get_sub_thumbnail_data(
        priv->drawable, src_x, src_y, src_width, src_height, &w, &h, &bpp);
    gimp_drawable_offsets(priv->drawable, &offsx, &offsy);
    sel = gimp_drawable_get_sub_thumbnail_data(
        GIMP_DRAWABLE(selection), src_x + offsx, src_y + offsy, src_width,
        src_height, &width, &height, &bpp);

    gimp_preview_area_mask(GIMP_PREVIEW_AREA(area), 0, 0, width, height,
                           gimp_drawable_type(priv->drawable), src,
                           width * gimp_drawable_bpp(priv->drawable), buffer,
                           rowstride, sel, width);
...
static void gimp_zoom_preview_draw_thumb(GimpPreview *preview,
  ...
  if (priv->drawable != NULL)
    _gimp_drawable_preview_area_draw_thumb(area, priv->drawable, width, height);
}
...
static void gimp_zoom_preview_set_cursor(GimpPreview *preview) {
  ...
  if (gimp_zoom_preview_get_factor(GIMP_ZOOM_PREVIEW(preview)) > 1.0) {
    ...
    gdk_window_set_cursor(gtk_widget_get_window(area), cursor);
...
static void gimp_zoom_preview_transform(GimpPreview *preview, gint src_x,
  ...
  *dest_x = ((gdouble)(src_x - priv->extents.x) * width / priv->extents.width *
             zoom) -
            xoff;

  *dest_y = ((gdouble)(src_y - priv->extents.y) * height /
             priv->extents.height * zoom) -
            yoff;
}
...
static void gimp_zoom_preview_untransform(GimpPreview *preview, gint src_x,
  ...
  *dest_x = (priv->extents.x +
             ((gdouble)(src_x + xoff) * priv->extents.width / width / zoom));

  *dest_y = (priv->extents.y +
             ((gdouble)(src_y + yoff) * priv->extents.height / height / zoom));
}
...
static void gimp_zoom_preview_set_drawable(GimpZoomPreview *preview,
  ...
  gimp_preview_set_bounds(GIMP_PREVIEW(preview), 0, 0, max_width, max_height);

  g_object_set(gimp_preview_get_frame(GIMP_PREVIEW(preview)), "ratio",
               (gdouble)width / (gdouble)height, NULL);
}
...
static void gimp_zoom_preview_set_model(GimpZoomPreview *preview,
  ...
  /* zoom out */
  button = gimp_zoom_button_new(priv->model, GIMP_ZOOM_OUT,
                                GTK_ICON_SIZE_SMALL_TOOLBAR);
  gtk_box_pack_start(GTK_BOX(button_bar), button, FALSE, FALSE, 0);
  gtk_widget_show(button);

  /* zoom in */
  button = gimp_zoom_button_new(priv->model, GIMP_ZOOM_IN,
                                GTK_ICON_SIZE_SMALL_TOOLBAR);
  gtk_box_pack_start(GTK_BOX(button_bar), button, FALSE, FALSE, 0);
  gtk_widget_show(button);
}
...
GtkWidget *gimp_zoom_preview_new_from_drawable(GimpDrawable *drawable) {
  ...
  return g_object_new(GIMP_TYPE_ZOOM_PREVIEW, "drawable", drawable, NULL);
}
...
gimp_zoom_preview_new_with_model_from_drawable(GimpDrawable *drawable,
  ...
  return g_object_new(GIMP_TYPE_ZOOM_PREVIEW, "drawable", drawable, "model",
                      model, NULL);
}
 ...
 *         will be put.
 * @height: (out): a pointer to an int where the current width of the zoom
 * widget will be put.
 ...
 * Since: 2.4
 */
guchar *gimp_zoom_preview_get_source(GimpZoomPreview *preview, gint *width,
  ...
  if (drawable) {
    ...
    gimp_zoom_preview_get_source_area(gimp_preview, &src_x, &src_y, &src_width,
                                      &src_height);

    return gimp_drawable_get_sub_thumbnail_data(
        drawable, src_x, src_y, src_width, src_height, width, height, bpp);
  } else {
    *width = 0;
    *height = 0;
    *bpp = 0;
