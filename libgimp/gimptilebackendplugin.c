...
static gboolean gimp_tile_write(GimpTileBackendPlugin *backend_plugin, gint x,
                                gint y, GeglTile *tile);
static GeglTile *gimp_tile_read(GimpTileBackendPlugin *backend_plugin, gint x,
                                gint y);

static gboolean gimp_tile_init(GimpTileBackendPlugin *backend_plugin,
                               GimpTile *tile, gint row, gint col);
...
static void _gimp_tile_backend_plugin_init(GimpTileBackendPlugin *backend) {
...
static gpointer gimp_tile_backend_plugin_command(GeglTileSource *tile_store,
  ...
  default:
    result = gegl_tile_backend_command(GEGL_TILE_BACKEND(tile_store), command,
                                       x, y, z, data);
    break;
  }
...
GeglTileBackend *_gimp_tile_backend_plugin_new(GimpDrawable *drawable,
  ...
  backend =
      g_object_new(GIMP_TYPE_TILE_BACKEND_PLUGIN, "tile-width", TILE_WIDTH,
                   "tile-height", TILE_HEIGHT, "format", format, NULL);
  ...
  gegl_tile_backend_set_extent(backend, GEGL_RECTANGLE(0, 0, width, height));
  ...
  if (gimp_tile.ewidth * gimp_tile.eheight * priv->bpp == tile_size) {
    ...
    for (row = 0; row < gimp_tile.eheight; row++) {
      memcpy(tile_data + row * tile_stride,
             gimp_tile.data + row * gimp_tile_stride, gimp_tile_stride);
    }
  }
...
static gboolean gimp_tile_write(GimpTileBackendPlugin *backend_plugin, gint x,
  ...
  gimp_tile.data =
      g_new(guchar, gimp_tile.ewidth * gimp_tile.eheight * priv->bpp);
  ...
  if (gimp_tile.ewidth * gimp_tile.eheight * priv->bpp == tile_size) {
    ...
    for (row = 0; row < gimp_tile.eheight; row++) {
      memcpy(gimp_tile.data + row * gimp_tile_stride,
             tile_data + row * tile_stride, gimp_tile_stride);
    }
  }
...
static gboolean gimp_tile_init(GimpTileBackendPlugin *backend_plugin,
  ...
  if (row > priv->ntile_rows - 1 || col > priv->ntile_cols - 1) {
    return FALSE;
  }
...
static void gimp_tile_get(GimpTileBackendPlugin *backend_plugin,
  ...
  if (!gp_tile_req_write(_gimp_plug_in_get_write_channel(plug_in), &tile_req,
                         plug_in))
    gimp_quit();
  ...
  if (tile_data->drawable_id != priv->drawable_id ||
      ...
      tile_data->shadow != priv->shadow || tile_data->width != tile->ewidth ||
      tile_data->height != tile->eheight || tile_data->bpp != priv->bpp) {
#if 0
  ...
  if (tile_data->use_shm) {
    tile->data =
        g_memdup(_gimp_shm_addr(), tile->ewidth * tile->eheight * priv->bpp);
  } else {
    tile->data = tile_data->data;
    tile_data->data = NULL;
  }

  if (!gp_tile_ack_write(_gimp_plug_in_get_write_channel(plug_in), plug_in))
    gimp_quit();
...
static void gimp_tile_put(GimpTileBackendPlugin *backend_plugin,
  ...
  if (tile_info->use_shm) {
    memcpy(_gimp_shm_addr(), tile->data,
           tile->ewidth * tile->eheight * priv->bpp);
  } else {
    tile_data.data = tile->data;
  }

  if (!gp_tile_data_write(_gimp_plug_in_get_write_channel(plug_in), &tile_data,
                          plug_in))
    gimp_quit();
