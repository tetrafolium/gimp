...
const gchar *gimp_progress_install_vtable(const GimpProgressVtable *vtable,
  ...
  procedure = gimp_procedure_new(plug_in, progress_callback,
                                 GIMP_PDB_PROC_TYPE_TEMPORARY,
                                 gimp_temp_progress_run, progress_data,
                                 (GDestroyNotify)gimp_progress_data_free);

  GIMP_PROC_ARG_ENUM(procedure, "command", "Command", "The progress command",
                     GIMP_TYPE_PROGRESS_COMMAND, GIMP_PROGRESS_COMMAND_START,
                     G_PARAM_READWRITE);

  GIMP_PROC_ARG_STRING(procedure, "text", "Text", "The progress text", NULL,
                       G_PARAM_READWRITE);

  GIMP_PROC_ARG_DOUBLE(procedure, "value", "Value", "The progress value", 0.0,
                       1.0, 0.0, G_PARAM_READWRITE);

  GIMP_PROC_VAL_DOUBLE(procedure, "value", "Value", "The progress value", 0.0,
                       1.0, 0.0, G_PARAM_READWRITE);
...
gboolean gimp_progress_update(gdouble percentage) {
  ...
  if (percentage <= 0.0) {
    ...
    changed = (fabs(gimp_progress_current - percentage) > gimp_progress_step);
...
static GimpValueArray *gimp_temp_progress_run(GimpProcedure *procedure,
  ...
  case GIMP_PROGRESS_COMMAND_START:
    progress_data->vtable.start(text, value != 0.0, progress_data->data);
    break;
