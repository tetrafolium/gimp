...
enum { PROP_0, PROP_PLUG_IN, PROP_NAME, PROP_PROCEDURE_TYPE, N_PROPS };
...
static void gimp_procedure_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_procedure_get_property(GObject *object, guint property_id,
...
gimp_procedure_real_create_config(GimpProcedure *procedure, GParamSpec **args,
                                  gint n_args);

static gboolean
gimp_procedure_validate_args(GimpProcedure *procedure, GParamSpec **param_specs,
                             gint n_param_specs, GimpValueArray *args,
                             gboolean return_vals, GError **error);
...
static void gimp_procedure_class_init(GimpProcedureClass *klass) {
  ...
  props[PROP_PLUG_IN] = g_param_spec_object(
      "plug-in", "Plug-In", "The GimpPlugIn of this plug-in process",
      GIMP_TYPE_PLUG_IN, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  props[PROP_NAME] =
      g_param_spec_string("name", "Name", "The procedure's name", NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  props[PROP_PROCEDURE_TYPE] = g_param_spec_enum(
      "procedure-type", "Procedure type", "The procedure's type",
      GIMP_TYPE_PDB_PROC_TYPE, GIMP_PDB_PROC_TYPE_PLUGIN,
      GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  g_object_class_install_properties(object_class, N_PROPS, props);
}

static void gimp_procedure_init(GimpProcedure *procedure) {
...
static void gimp_procedure_install_icon(GimpProcedure *procedure) {
  GimpIconType icon_type;
  ...
  case GIMP_ICON_TYPE_PIXBUF: {
    ...
    if (pixbuf)
      gdk_pixbuf_save_to_buffer(pixbuf, (gchar **)&icon_data, &icon_data_length,
                                "png", NULL, NULL);
  } break;
  ...
  if (icon_data)
    _gimp_pdb_set_proc_icon(gimp_procedure_get_name(procedure), icon_type,
                            icon_data_length, icon_data);
...
static void gimp_procedure_real_install(GimpProcedure *procedure) {
  ...
  for (i = 0; i < n_args; i++) {
    _gimp_param_spec_to_gp_param_def(args[i], &proc_install.params[i]);
  }
  ...
  for (list = gimp_procedure_get_menu_paths(procedure); list;
       list = g_list_next(list)) {
    _gimp_pdb_add_proc_menu_path(gimp_procedure_get_name(procedure),
                                 list->data);
  }

  if (procedure->priv->blurb || procedure->priv->help ||
      procedure->priv->help_id) {
    _gimp_pdb_set_proc_documentation(
        gimp_procedure_get_name(procedure), procedure->priv->blurb,
        procedure->priv->help, procedure->priv->help_id);
  }

  if (procedure->priv->authors || procedure->priv->copyright ||
      procedure->priv->date) {
    _gimp_pdb_set_proc_attribution(
        gimp_procedure_get_name(procedure), procedure->priv->authors,
        procedure->priv->copyright, procedure->priv->date);
  }
...
static GimpValueArray *gimp_procedure_real_run(GimpProcedure *procedure,
                                               const GimpValueArray *args) {
  return procedure->priv->run_func(procedure, args, procedure->priv->run_data);
}
...
gimp_procedure_real_create_config(GimpProcedure *procedure, GParamSpec **args,
  ...
  if (!type) {
    ...
    memcpy(config_args, args, n_args * sizeof(GParamSpec *));
    memcpy(config_args + n_args, procedure->priv->aux_args,
           procedure->priv->n_aux_args * sizeof(GParamSpec *));

    type = gimp_config_type_register(GIMP_TYPE_PROCEDURE_CONFIG, type_name,
  ...
  return g_object_new(type, "procedure", procedure, NULL);
}
 ...
 * Since: 3.0
 **/
GimpProcedure *gimp_procedure_new(GimpPlugIn *plug_in, const gchar *name,
                                  GimpPDBProcType proc_type,
                                  GimpRunFunc run_func, gpointer run_data,
                                  GDestroyNotify run_data_destroy) {
  ...
  procedure = g_object_new(GIMP_TYPE_PROCEDURE, "plug-in", plug_in, "name",
...
GimpPDBProcType gimp_procedure_get_proc_type(GimpProcedure *procedure) {
  g_return_val_if_fail(GIMP_IS_PROCEDURE(procedure), GIMP_PDB_PROC_TYPE_PLUGIN);
...
void gimp_procedure_add_menu_path(GimpProcedure *procedure,
  ...
  procedure->priv->menu_paths =
      g_list_append(procedure->priv->menu_paths, g_strdup(menu_path));
...
void gimp_procedure_set_icon_name(GimpProcedure *procedure,
  ...
  gimp_procedure_set_icon(procedure, GIMP_ICON_TYPE_ICON_NAME, icon_name);
}
...
void gimp_procedure_set_icon_pixbuf(GimpProcedure *procedure,
  ...
  gimp_procedure_set_icon(procedure, GIMP_ICON_TYPE_PIXBUF, pixbuf);
}
 ...
 * Since: 3.0
 */
void gimp_procedure_set_icon_file(GimpProcedure *procedure, GFile *file) {
  g_return_if_fail(GIMP_IS_PROCEDURE(procedure));
  g_return_if_fail(file == NULL || G_IS_FILE(file));

  gimp_procedure_set_icon(procedure, GIMP_ICON_TYPE_IMAGE_FILE, file);
}
...
void gimp_procedure_set_documentation(GimpProcedure *procedure,
                                      const gchar *blurb, const gchar *help,
                                      const gchar *help_id) {
  g_return_if_fail(GIMP_IS_PROCEDURE(procedure));
...
GParamSpec *gimp_procedure_add_argument(GimpProcedure *procedure,
  ...
  if (gimp_procedure_find_argument(procedure, pspec->name)) {
    g_warning("Argument with name '%s' already exists on procedure '%s'",
              pspec->name, gimp_procedure_get_name(procedure));
    return pspec;
  }
  ...
  procedure->priv->args =
      g_renew(GParamSpec *, procedure->priv->args, procedure->priv->n_args + 1);
 ...
 * Since: 3.0
 */
GParamSpec *gimp_procedure_add_argument_from_property(GimpProcedure *procedure,
                                                      GObject *config,
...
GParamSpec *gimp_procedure_add_aux_argument_from_property(
    GimpProcedure *procedure, GObject *config, const gchar *prop_name) {
  GParamSpec *pspec;
...
GimpValueArray *gimp_procedure_run(GimpProcedure *procedure,
  ...
  if (!gimp_procedure_validate_args(procedure, procedure->priv->args,
                                    procedure->priv->n_args, args, FALSE,
                                    &error)) {
  ...
  if (gimp_value_array_length(args) < procedure->priv->n_args) {
    ...
    /*  call the procedure  */
    return_vals = GIMP_PROCEDURE_GET_CLASS(procedure)->run(procedure, complete);
  ...
  if (!return_vals) {
    ...
    error = g_error_new(0, 0, _("Procedure '%s' returned no return values"),
                        gimp_procedure_get_name(procedure));
 ...
 * Since: 3.0
 **/
GimpProcedureConfig *gimp_procedure_create_config(GimpProcedure *procedure) {
  g_return_val_if_fail(GIMP_IS_PROCEDURE(procedure), NULL);

  return GIMP_PROCEDURE_GET_CLASS(procedure)->create_config(
      procedure, procedure->priv->args, procedure->priv->n_args);
}
...
gimp_procedure_validate_args(GimpProcedure *procedure, GParamSpec **param_specs,
  ...
  for (i = 0; i < MIN(gimp_value_array_length(args), n_param_specs); i++) {
    ...
    if (arg_type != spec_type) {
      if (return_vals) {
        g_set_error(error, GIMP_PDB_ERROR, GIMP_PDB_ERROR_INVALID_RETURN_VALUE,
                    _("Procedure '%s' returned a wrong value type "
                      "for return value '%s' (#%d). "
                      "Expected %s, got %s."),
                    gimp_procedure_get_name(procedure),
                    g_param_spec_get_name(pspec), i + 1, g_type_name(spec_type),
      ...
      if (g_param_value_validate(pspec, arg)) {
        ...
        if (return_vals) {
          g_set_error(
              error, GIMP_PDB_ERROR, GIMP_PDB_ERROR_INVALID_RETURN_VALUE,
              _("Procedure '%s' returned "
                "'%s' as return value '%s' "
                "(#%d, type %s). "
                "This value is out of range."),
              gimp_procedure_get_name(procedure), value,
              g_param_spec_get_name(pspec), i + 1, g_type_name(spec_type));
        } else {
          g_set_error(error, GIMP_PDB_ERROR, GIMP_PDB_ERROR_INVALID_ARGUMENT,
                      _("Procedure '%s' has been called with "
                        ...
                        "This value is out of range."),
                      gimp_procedure_get_name(procedure), value,
                      g_param_spec_get_name(pspec), i + 1,
...
GimpDisplay *_gimp_procedure_get_display(GimpProcedure *procedure,
  ...
  if (G_UNLIKELY(!procedure->priv->displays))
    procedure->priv->displays = g_hash_table_new_full(
  ...
  if (!display) {
    display = _gimp_plug_in_get_display(procedure->priv->plug_in, display_id);

    if (display)
      g_hash_table_insert(procedure->priv->displays,
                          GINT_TO_POINTER(display_id), g_object_ref(display));
  }
...
GimpImage *_gimp_procedure_get_image(GimpProcedure *procedure,
  ...
  if (G_UNLIKELY(!procedure->priv->images))
    procedure->priv->images = g_hash_table_new_full(
        g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)g_object_unref);

  image =
      g_hash_table_lookup(procedure->priv->images, GINT_TO_POINTER(image_id));

  if (!image) {
    image = _gimp_plug_in_get_image(procedure->priv->plug_in, image_id);

    if (image)
      g_hash_table_insert(procedure->priv->images, GINT_TO_POINTER(image_id),
                          g_object_ref(image));
  }
...
GimpItem *_gimp_procedure_get_item(GimpProcedure *procedure, gint32 item_id) {
  ...
  if (G_UNLIKELY(!procedure->priv->items))
    procedure->priv->items = g_hash_table_new_full(
        g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)g_object_unref);

  item = g_hash_table_lookup(procedure->priv->items, GINT_TO_POINTER(item_id));

  if (!item) {
    item = _gimp_plug_in_get_item(procedure->priv->plug_in, item_id);

    if (item)
      g_hash_table_insert(procedure->priv->items, GINT_TO_POINTER(item_id),
                          g_object_ref(item));
  }
