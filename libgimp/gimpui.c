...
static void gimp_ui_help_func(const gchar *help_id, gpointer help_data);
static void gimp_ui_theme_changed(GFileMonitor *monitor, GFile *file,
                                  GFile *other_file,
...
void gimp_ui_init(const gchar *prog_name) {
  ...
  g_signal_connect(css_monitor, "changed", G_CALLBACK(gimp_ui_theme_changed),
                   css_provider);
  ...
  gimp_widgets_init(gimp_ui_help_func, gimp_context_get_foreground,
                    gimp_context_get_background, gimp_ensure_modules);
...
#ifdef GDK_WINDOWING_QUARTZ
static void gimp_window_transient_show(GtkWidget *window) {
  g_signal_handlers_disconnect_by_func(window, gimp_window_transient_show,
                                       NULL);
...
#ifdef GDK_WINDOWING_QUARTZ
    g_signal_connect(window, "show", G_CALLBACK(gimp_window_transient_show),
                     NULL);
#endif
  }
}
...
static void gimp_ui_theme_changed(GFileMonitor *monitor, GFile *file,
  ...
  if (g_file_load_contents(file, NULL, &contents, NULL, NULL, &error)) {
    ...
    g_object_set(gtk_settings_get_for_screen(gdk_screen_get_default()),
                 "gtk-application-prefer-dark-theme", prefer_dark_theme, NULL);
...
static gboolean gimp_window_set_transient_for(GtkWindow *window,
   ...
   */
#ifndef GDK_WINDOWING_WIN32
  g_signal_handlers_disconnect_matched(window, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                                       gimp_window_transient_realized, NULL);
