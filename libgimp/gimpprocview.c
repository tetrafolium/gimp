...
static gint gimp_proc_view_add_label(GtkGrid *grid, gint row,
...
GtkWidget *gimp_proc_view_new(const gchar *procedure_name) {
  ...
  if (blurb && strlen(blurb) < 2)
    blurb = NULL;
  if (help && strlen(help) < 2)
    help = NULL;
  if (help_id && strlen(help_id) < 2)
    help_id = NULL;
  if (authors && strlen(authors) < 2)
    authors = NULL;
  if (copyright && strlen(copyright) < 2)
    copyright = NULL;
  if (date && strlen(date) < 2)
    date = NULL;
  ...
  if (!gimp_enum_get_value(GIMP_TYPE_PDB_PROC_TYPE, type, NULL, NULL, &type_str,
                           NULL))
    type_str = "UNKNOWN";
  ...
  if (type != GIMP_PDB_PROC_TYPE_INTERNAL) {
    ...
    row = gimp_proc_view_add_label(GTK_GRID(grid), row, _("Image types:"),
                                   gimp_procedure_get_image_types(procedure));
    row = gimp_proc_view_add_label(GTK_GRID(grid), row, _("Menu label:"),
                                   gimp_procedure_get_menu_label(procedure));

    for (list = gimp_procedure_get_menu_paths(procedure); list;
         list = g_list_next(list)) {
      row = gimp_proc_view_add_label(GTK_GRID(grid), row, _("Menu path:"),
                                     list->data);
    }
  }
  ...
  /* in parameters */
  grid = gimp_proc_view_create_args(procedure, FALSE, name_group, type_group,
                                    desc_group);
  ...
  /* out parameters */
  grid = gimp_proc_view_create_args(procedure, TRUE, name_group, type_group,
                                    desc_group);
  ...
  if (authors || date || copyright) {
    ...
    row = gimp_proc_view_add_label(GTK_GRID(grid), row, _("Authors:"), authors);
    row = gimp_proc_view_add_label(GTK_GRID(grid), row, _("Date:"), date);
    row = gimp_proc_view_add_label(GTK_GRID(grid), row, _("Copyright:"),
                                   copyright);
  }
...
static gint gimp_proc_view_add_label(GtkGrid *grid, gint row,
  ...
  if (right_label && strlen(right_label)) {
    ...
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, left_label, 0.0, 0.0,
                             label, 3);
  }
