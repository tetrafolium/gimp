...
guchar *_gimp_shm_addr(void) { return _shm_addr; }
...
void _gimp_shm_open(gint shm_ID) {
  ...
  if (_shm_ID != -1) {
    ...
    if (_shm_addr == (guchar *)-1) {
      g_error("shmat() failed: %s\n" ERRMSG_SHM_FAILED, g_strerror(errno));
    }
    ...
    _shm_handle = OpenFileMapping(FILE_MAP_ALL_ACCESS, 0, fileMapName);
    if (_shm_handle) {
      /* Map the shared memory into our address space for use */
      _shm_addr = (guchar *)MapViewOfFile(_shm_handle, FILE_MAP_ALL_ACCESS, 0,
                                          0, TILE_MAP_SIZE);
    ...
    if (shm_fd != -1) {
      /* Map the shared memory into our address space for use */
      _shm_addr = (guchar *)mmap(NULL, TILE_MAP_SIZE, PROT_READ | PROT_WRITE,
                                 MAP_SHARED, shm_fd, 0);
