...
static GimpValueArray *gimp_thumbnail_procedure_run(GimpProcedure *procedure,
...
static void gimp_thumbnail_procedure_init(GimpThumbnailProcedure *procedure) {
...
static void gimp_thumbnail_procedure_constructed(GObject *object) {
  ...
  GIMP_PROC_ARG_INT(procedure, "thumb-size", "Thumb Size",
                    "Preferred thumbnail size", 16, 2014, 256,
                    GIMP_PARAM_READWRITE);

  GIMP_PROC_VAL_IMAGE(procedure, "image", "Image", "Thumbnail image", TRUE,
                      GIMP_PARAM_READWRITE);

  GIMP_PROC_VAL_INT(procedure, "image-width", "Image width",
                    "Width of the full-sized image (0 for unknown)", 0,
                    GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE);

  GIMP_PROC_VAL_INT(procedure, "image-height", "Image height",
                    "Height of the full-sized image (0 for unknown)", 0,
                    GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE);

  GIMP_PROC_VAL_ENUM(procedure, "image-type", "Image type", "Type of the image",
                     GIMP_TYPE_IMAGE_TYPE, GIMP_RGB_IMAGE,
                     GIMP_PARAM_READWRITE);

  GIMP_PROC_VAL_INT(procedure, "num-layers", "Num layers",
                    "Number of layers in the image", 1, G_MAXINT, 1,
                    GIMP_PARAM_READWRITE);
}
...
gimp_thumbnail_procedure_run(GimpProcedure *procedure,
  ...
  return_values = thumbnail_proc->priv->run_func(
      procedure, file, size, remaining, thumbnail_proc->priv->run_data);
...
GimpProcedure *gimp_thumbnail_procedure_new(GimpPlugIn *plug_in,
  ...
  procedure = g_object_new(GIMP_TYPE_THUMBNAIL_PROCEDURE, "plug-in", plug_in,
