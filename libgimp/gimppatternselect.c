...
const gchar *gimp_pattern_select_new(const gchar *title,
  ...
  procedure =
      gimp_procedure_new(plug_in, pattern_callback,
                         GIMP_PDB_PROC_TYPE_TEMPORARY, gimp_temp_pattern_run,
                         pattern_data, (GDestroyNotify)gimp_pattern_data_free);

  GIMP_PROC_ARG_STRING(procedure, "pattern-name", "Pattern name",
                       "The pattern name", NULL, G_PARAM_READWRITE);

  GIMP_PROC_ARG_INT(procedure, "mask-width", "Mask width", "Pattern width", 0,
                    10000, 0, G_PARAM_READWRITE);

  GIMP_PROC_ARG_INT(procedure, "mask-height", "Mask height", "Pattern height",
                    0, 10000, 0, G_PARAM_READWRITE);

  GIMP_PROC_ARG_INT(procedure, "mask-bpp", "Mask bpp",
                    "Pattern bytes per pixel", 0, 10000, 0, G_PARAM_READWRITE);

  GIMP_PROC_ARG_INT(procedure, "mask-len", "Mask length",
                    "Length of pattern mask data", 0, G_MAXINT, 0,
                    G_PARAM_READWRITE);

  GIMP_PROC_ARG_UINT8_ARRAY(procedure, "mask-data", "Mask data",
                            "The pattern mask data", G_PARAM_READWRITE);

  GIMP_PROC_ARG_BOOLEAN(procedure, "closing", "Closing",
                        "If the dialog was cloaing", FALSE, G_PARAM_READWRITE);
...
static gboolean gimp_temp_pattern_idle(GimpPatternData *data) {
  ...
  if (data->callback)
    data->callback(data->pattern_name, data->width, data->height, data->bytes,
                   data->pattern_mask_data, data->closing, data->data);
