...
enum { PROP_0, PROP_READ_CHANNEL, PROP_WRITE_CHANNEL, N_PROPS };
...
static void gimp_plug_in_set_property(GObject *object, guint property_id,
                                      const GValue *value, GParamSpec *pspec);
static void gimp_plug_in_get_property(GObject *object, guint property_id,
                                      GValue *value, GParamSpec *pspec);

static void gimp_plug_in_register(GimpPlugIn *plug_in, GList *procedures);

static gboolean gimp_plug_in_write(GIOChannel *channel, const guint8 *buf,
                                   gulong count, gpointer user_data);
static gboolean gimp_plug_in_flush(GIOChannel *channel, gpointer user_data);
...
static void gimp_plug_in_process_message(GimpPlugIn *plug_in,
                                         GimpWireMessage *msg);
static void gimp_plug_in_proc_run(GimpPlugIn *plug_in, GPProcRun *proc_run);
...
static void gimp_plug_in_class_init(GimpPlugInClass *klass) {
  ...
  props[PROP_READ_CHANNEL] = g_param_spec_boxed(
      "read-channel", "Read channel", "The GIOChanel to read from GIMP",
      G_TYPE_IO_CHANNEL, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  props[PROP_WRITE_CHANNEL] = g_param_spec_boxed(
      "write-channel", "Write channel", "The GIOChanel to write to GIMP",
      G_TYPE_IO_CHANNEL, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  g_object_class_install_properties(object_class, N_PROPS, props);
}

static void gimp_plug_in_init(GimpPlugIn *plug_in) {
 ...
 * Since: 3.0
 **/
void gimp_plug_in_set_help_domain(GimpPlugIn *plug_in, const gchar *domain_name,
                                  GFile *domain_uri) {
  g_return_if_fail(GIMP_IS_PLUG_IN(plug_in));
  g_return_if_fail(domain_name != NULL);
 ...
 * Since: 3.0
 **/
void gimp_plug_in_add_menu_branch(GimpPlugIn *plug_in, const gchar *menu_path,
  ...
  plug_in->priv->menu_branches =
      g_list_append(plug_in->priv->menu_branches, branch);
}
...
void gimp_plug_in_add_temp_procedure(GimpPlugIn *plug_in,
  ...
  plug_in->priv->temp_procedures =
      g_list_prepend(plug_in->priv->temp_procedures, g_object_ref(procedure));
...
void gimp_plug_in_remove_temp_procedure(GimpPlugIn *plug_in,
  ...
  if (procedure) {
    ...
    plug_in->priv->temp_procedures =
        g_list_remove(plug_in->priv->temp_procedures, procedure);
  ...
  if (!plug_in->priv->extension_source_id) {
    plug_in->priv->extension_source_id =
        g_io_add_watch(plug_in->priv->read_channel, G_IO_IN | G_IO_PRI,
                       gimp_plug_in_extension_read, plug_in);
  }
}
 ...
 * Since: 3.0
 **/
void gimp_plug_in_extension_process(GimpPlugIn *plug_in, guint timeout) {
#ifndef G_OS_WIN32
  ...
  do {
    ...
    FD_ZERO(&readfds);
    FD_SET(g_io_channel_unix_get_fd(plug_in->priv->read_channel), &readfds);
  ...
  g_io_channel_win32_make_pollfd(plug_in->priv->read_channel, G_IO_IN, &pollfd);
 ...
 * Since: 3.0
 **/
GimpPDBErrorHandler gimp_plug_in_get_pdb_error_handler(GimpPlugIn *plug_in) {
...
void _gimp_plug_in_run(GimpPlugIn *plug_in) {
  ...
  g_io_add_watch(plug_in->priv->read_channel, G_IO_ERR | G_IO_HUP,
                 gimp_plug_in_io_error_handler, NULL);
...
void _gimp_plug_in_read_expect_msg(GimpPlugIn *plug_in, GimpWireMessage *msg,
                                   gint type) {
  g_return_if_fail(GIMP_IS_PLUG_IN(plug_in));
...
static void gimp_plug_in_register(GimpPlugIn *plug_in, GList *procedures) {
  ...
  for (list = plug_in->priv->menu_branches; list; list = g_list_next(list)) {
    ...
    _gimp_plug_in_menu_branch_register(branch->menu_path, branch->menu_label);
  }
}
    ...
    while (count != plug_in->priv->write_buffer_index) {
      ...
      do {
        ...
        status = g_io_channel_write_chars(
            channel, &plug_in->priv->write_buffer[count],
            (plug_in->priv->write_buffer_index - count), &bytes, &error);
      } while (status == G_IO_STATUS_AGAIN);

      if (status != G_IO_STATUS_NORMAL) {
        if (error) {
          g_warning("%s: gimp_flush(): error: %s", g_get_prgname(),
                    error->message);
          g_error_free(error);
        } else {
...
static void gimp_plug_in_loop(GimpPlugIn *plug_in) {
  while (TRUE) {
    ...
    case GP_TEMP_PROC_RETURN:
      g_warning(
          "unexpected temp proc return message received (should not happen");
      break;
...
static void gimp_plug_in_proc_run(GimpPlugIn *plug_in, GPProcRun *proc_run) {
  ...
  if (procedure) {
    gimp_plug_in_proc_run_internal(plug_in, proc_run, procedure, &proc_return);
    g_object_unref(procedure);
  }

  if (!gp_proc_return_write(plug_in->priv->write_channel, &proc_return,
                            plug_in))
    gimp_quit();
...
static void gimp_plug_in_temp_proc_run(GimpPlugIn *plug_in,
  ...
  if (!gp_temp_proc_return_write(plug_in->priv->write_channel, &proc_return,
                                 plug_in))
    gimp_quit();
...
static void gimp_plug_in_proc_run_internal(GimpPlugIn *plug_in,
  ...
  arguments = _gimp_gp_params_to_value_array(NULL, NULL, 0, proc_run->params,
                                             proc_run->n_params, FALSE);
...
GimpDisplay *_gimp_plug_in_get_display(GimpPlugIn *plug_in, gint32 display_id) {
  GimpDisplay *display = NULL;

  g_return_val_if_fail(GIMP_IS_PLUG_IN(plug_in), NULL);

  if (G_UNLIKELY(!plug_in->priv->displays))
    plug_in->priv->displays = g_hash_table_new_full(
        g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)g_object_unref);

  display =
      g_hash_table_lookup(plug_in->priv->displays, GINT_TO_POINTER(display_id));

  if (!display) {
    display = g_object_new(GIMP_TYPE_DISPLAY, "id", display_id, NULL);

    g_hash_table_insert(plug_in->priv->displays, GINT_TO_POINTER(display_id),
...
GimpImage *_gimp_plug_in_get_image(GimpPlugIn *plug_in, gint32 image_id) {
  GimpImage *image = NULL;

  g_return_val_if_fail(GIMP_IS_PLUG_IN(plug_in), NULL);

  if (G_UNLIKELY(!plug_in->priv->images))
    plug_in->priv->images = g_hash_table_new_full(
        g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)g_object_unref);

  image = g_hash_table_lookup(plug_in->priv->images, GINT_TO_POINTER(image_id));

  if (!image) {
    image = g_object_new(GIMP_TYPE_IMAGE, "id", image_id, NULL);

    g_hash_table_insert(plug_in->priv->images, GINT_TO_POINTER(image_id),
...
GimpItem *_gimp_plug_in_get_item(GimpPlugIn *plug_in, gint32 item_id) {
  GimpItem *item = NULL;

  g_return_val_if_fail(GIMP_IS_PLUG_IN(plug_in), NULL);

  if (G_UNLIKELY(!plug_in->priv->items))
    plug_in->priv->items = g_hash_table_new_full(
        g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)g_object_unref);

  item = g_hash_table_lookup(plug_in->priv->items, GINT_TO_POINTER(item_id));

  if (!item) {
    if (gimp_item_id_is_layer(item_id)) {
      item = g_object_new(GIMP_TYPE_LAYER, "id", item_id, NULL);
    } else if (gimp_item_id_is_layer_mask(item_id)) {
      item = g_object_new(GIMP_TYPE_LAYER_MASK, "id", item_id, NULL);
    } else if (gimp_item_id_is_selection(item_id)) {
      item = g_object_new(GIMP_TYPE_SELECTION, "id", item_id, NULL);
    } else if (gimp_item_id_is_channel(item_id)) {
      item = g_object_new(GIMP_TYPE_CHANNEL, "id", item_id, NULL);
    } else if (gimp_item_id_is_vectors(item_id)) {
      item = g_object_new(GIMP_TYPE_VECTORS, "id", item_id, NULL);
    }

    if (item)
      g_hash_table_insert(plug_in->priv->items, GINT_TO_POINTER(item_id),
