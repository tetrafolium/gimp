...
static void gimp_brush_select_button_callback(
    const gchar *brush_name, gdouble opacity, gint spacing,
    GimpLayerMode paint_mode, gint width, gint height, const guchar *mask_data,
...
static void gimp_brush_select_preview_draw(GimpPreviewArea *area, gint x,
...
static GtkWidget *
gimp_brush_select_button_create_inside(GimpBrushSelectButton *button);
...
gimp_brush_select_button_class_init(GimpBrushSelectButtonClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "title", "Title",
          "The title to be used for the brush selection popup dialog",
          _("Brush Selection"), GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
   ...
   */
  g_object_class_install_property(
      object_class, PROP_BRUSH_NAME,
      g_param_spec_string("brush-name", "Brush name",
                          "The name of the currently selected brush", NULL,
   ...
   */
  g_object_class_install_property(
      object_class, PROP_BRUSH_OPACITY,
      g_param_spec_double("brush-opacity", "Brush opacity",
                          "The opacity of the currently selected brush", -1.0,
                          100.0, -1.0, GIMP_PARAM_READWRITE));
   ...
   */
  g_object_class_install_property(
      object_class, PROP_BRUSH_SPACING,
      g_param_spec_int("brush-spacing", "Brush spacing",
                       "The spacing of the currently selected brush", -G_MAXINT,
                       1000, -1, GIMP_PARAM_READWRITE));
   ...
   */
  g_object_class_install_property(
      object_class, PROP_BRUSH_PAINT_MODE,
      g_param_spec_int("brush-paint-mode", "Brush paint mode",
                       "The paint mode of the currently selected brush", -1,
                       GIMP_LAYER_MODE_LUMINANCE, -1, GIMP_PARAM_READWRITE));
   ...
   * Since: 2.4
   */
  brush_button_signals[BRUSH_SET] = g_signal_new(
      "brush-set", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpBrushSelectButtonClass, brush_set), NULL, NULL,
      _gimpui_marshal_VOID__STRING_DOUBLE_INT_INT_INT_INT_POINTER_BOOLEAN,
      G_TYPE_NONE, 8, G_TYPE_STRING, G_TYPE_DOUBLE, G_TYPE_INT, G_TYPE_INT,
      G_TYPE_INT, G_TYPE_INT, G_TYPE_POINTER, G_TYPE_BOOLEAN);
}

static void gimp_brush_select_button_init(GimpBrushSelectButton *button) {
  ...
  gimp_brush_get_pixels(priv->brush_name, &priv->width, &priv->height,
                        &mask_bpp, &mask_data_size, &priv->mask_data,
                        &color_bpp, &color_data_size, &color_data);
...
GtkWidget *gimp_brush_select_button_new(const gchar *title,
                                        ...
                                        gdouble opacity, gint spacing,
                                        GimpLayerMode paint_mode) {
  GtkWidget *button;

  if (title)
    button = g_object_new(GIMP_TYPE_BRUSH_SELECT_BUTTON, "title", title,
                          "brush-name", brush_name, "brush-opacity", opacity,
                          "brush-spacing", spacing, "brush-paint-mode",
                          paint_mode, NULL);
  else
    button = g_object_new(GIMP_TYPE_BRUSH_SELECT_BUTTON, "brush-name",
                          brush_name, "brush-opacity", opacity, "brush-spacing",
                          spacing, "brush-paint-mode", paint_mode, NULL);
...
void gimp_brush_select_button_set_brush(GimpBrushSelectButton *button,
  ...
  if (select_button->temp_callback) {
    gimp_brushes_set_popup(select_button->temp_callback, brush_name, opacity,
                           spacing, paint_mode);
  } else {
    gchar *name;
    gint width;
    ...
    if (gimp_brush_get_pixels(name, &width, &height, &bytes, &mask_data_size,
                              &mask_data, &color_bpp, &color_data_size,
                              &color_data)) {
      if (color_data)
        g_free(color_data);
      ...
      gimp_brush_select_button_callback(name, opacity, spacing, paint_mode,
                                        width, height, mask_data, FALSE,
                                        button);
...
static void gimp_brush_select_button_set_property(GObject *object,
  ...
  case PROP_BRUSH_NAME:
    gimp_brush_select_button_set_brush(button, g_value_get_string(value), -1.0,
                                       -1, -1);
    break;
...
static void gimp_brush_select_button_callback(
    const gchar *name, gdouble opacity, gint spacing, GimpLayerMode paint_mode,
    gint width, gint height, const guchar *mask_data, gboolean dialog_closing,
    gpointer data) {
  ...
  gimp_brush_select_preview_update(button->priv->preview, width, height,
                                   mask_data);

  if (dialog_closing)
    select_button->temp_callback = NULL;

  g_signal_emit(button, brush_button_signals[BRUSH_SET], 0, name, opacity,
                spacing, paint_mode, width, height, mask_data, dialog_closing);
...
static void gimp_brush_select_button_clicked(GimpBrushSelectButton *button) {
  ...
  if (select_button->temp_callback) {
    ...
    gimp_brushes_set_popup(select_button->temp_callback,
                           button->priv->brush_name, button->priv->opacity,
                           button->priv->spacing, button->priv->paint_mode);
  } else {
    select_button->temp_callback = gimp_brush_select_new(
        button->priv->title, button->priv->brush_name, button->priv->opacity,
        button->priv->spacing, button->priv->paint_mode,
        gimp_brush_select_button_callback, button, NULL);
  }
}

static void gimp_brush_select_preview_resize(GimpBrushSelectButton *button) {
  if (button->priv->width > 0 && button->priv->height > 0)
    gimp_brush_select_preview_update(button->priv->preview, button->priv->width,
                                     button->priv->height,
                                     button->priv->mask_data);
}

static gboolean
gimp_brush_select_preview_events(GtkWidget *widget, GdkEvent *event,
                                 GimpBrushSelectButton *button) {
  ...
  if (button->priv->mask_data) {
    ...
    case GDK_BUTTON_PRESS:
      ...
      if (bevent->button == 1) {
        gtk_grab_add(widget);
        gimp_brush_select_button_open_popup(button, bevent->x, bevent->y);
      }
      break;
...
static void gimp_brush_select_preview_draw(GimpPreviewArea *area, gint x,
  ...
  gimp_preview_area_draw(area, x, y, width, height, GIMP_GRAY_IMAGE, buf,
                         width);
...
static void gimp_brush_select_preview_update(GtkWidget *preview,
  ...
  if (x || y)
    gimp_preview_area_fill(area, 0, 0, allocation.width, allocation.height,
                           0xFF, 0xFF, 0xFF);
...
static void gimp_brush_select_button_open_popup(GimpBrushSelectButton *button,
  ...
  gdk_window_get_origin(gtk_widget_get_window(priv->preview), &x_org, &y_org);
  ...
  /*  Draw the brush  */
  gimp_brush_select_preview_draw(GIMP_PREVIEW_AREA(preview), 0, 0, priv->width,
                                 priv->height, priv->mask_data, priv->width);
}
...
static void gimp_brush_select_drag_data_received(GimpBrushSelectButton *button,
  ...
  str =
      g_strndup((const gchar *)gtk_selection_data_get_data(selection), length);
...
gimp_brush_select_button_create_inside(GimpBrushSelectButton *brush_button) {
  ...
  g_signal_connect(priv->preview, "event",
                   G_CALLBACK(gimp_brush_select_preview_events), brush_button);
