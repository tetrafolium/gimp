...
static void gimp_drawable_preview_init(GimpDrawablePreview *preview) {
  ...
  g_object_set(gimp_preview_get_area(GIMP_PREVIEW(preview)), "check-size",
               gimp_check_size(), "check-type", gimp_check_type(), NULL);
}
...
static void gimp_drawable_preview_constructed(GObject *object) {
  ...
  g_object_set_data_full(object, "gimp-drawable-preview-data-name", data_name,
...
static void gimp_drawable_preview_dispose(GObject *object) {
  ...
  data_name =
      g_object_get_data(G_OBJECT(object), "gimp-drawable-preview-data-name");
  if (data_name) {
...
static void gimp_drawable_preview_get_property(GObject *object,
  ...
  switch (property_id) {
  case PROP_DRAWABLE:
    g_value_set_object(value, gimp_drawable_preview_get_drawable(preview));
    break;
...
static void gimp_drawable_preview_set_property(GObject *object,
  ...
  switch (property_id) {
  case PROP_DRAWABLE:
    gimp_drawable_preview_set_drawable(preview, g_value_dup_object(value));
    break;
...
static void gimp_drawable_preview_style_updated(GtkWidget *widget) {
  ...
  if (area) {
    ...
    gtk_widget_set_size_request(area, MIN(xmax - xmin, size),
                                MIN(ymax - ymin, size));
  }
}
...
static void gimp_drawable_preview_draw_original(GimpPreview *preview) {
  ...
  buffer = gimp_drawable_get_sub_thumbnail_data(priv->drawable, xoff + xmin,
                                                yoff + ymin, width, height,
                                                &width, &height, &bpp);
  ...
  gimp_preview_area_draw(GIMP_PREVIEW_AREA(gimp_preview_get_area(preview)), 0,
                         0, width, height, type, buffer, width * bpp);
  g_free(buffer);
}
...
void _gimp_drawable_preview_area_draw_thumb(GimpPreviewArea *area,
                                            GimpDrawable *drawable, gint width,
                                            gint height) {
  guchar *buffer;
  gint x1, y1, x2, y2;
  gint bpp;
  ...
  if (_gimp_drawable_preview_get_bounds(drawable, &x1, &y1, &x2, &y2)) {
    buffer = gimp_drawable_get_sub_thumbnail_data(
        drawable, x1, y1, x2 - x1, y2 - y1, &nav_width, &nav_height, &bpp);
  } else {
    buffer = gimp_drawable_get_thumbnail_data(drawable, &nav_width, &nav_height,
                                              &bpp);
  }
  ...
  if (buffer) {
    ...
    gimp_preview_area_draw(area, 0, 0, nav_width, nav_height, type, buffer,
                           bpp * nav_width);
    g_free(buffer);
  }
}

static void gimp_drawable_preview_draw_area(GimpDrawablePreview *preview,
                                            gint x, gint y, gint width,
                                            gint height, const guchar *buf,
                                            gint rowstride) {
  GimpDrawablePreviewPrivate *priv = GET_PRIVATE(preview);
  ...
  if (gimp_selection_is_empty(image)) {
    gimp_preview_area_draw(GIMP_PREVIEW_AREA(area), x - xoff - xmin,
                           y - yoff - ymin, width, height,
                           gimp_drawable_type(priv->drawable), buf, rowstride);
  } else {
    ...
    if (gimp_drawable_mask_intersect(priv->drawable, &mask_x, &mask_y,
                                     &mask_width, &mask_height) &&
        gimp_rectangle_intersect(mask_x, mask_y, mask_width, mask_height, x, y,
                                 width, height, &draw_x, &draw_y, &draw_width,
                                 &draw_height)) {
      GimpImageType type;
      ...
      src = gimp_drawable_get_sub_thumbnail_data(priv->drawable, draw_x, draw_y,
                                                 draw_width, draw_height, &d_w,
                                                 &d_h, &d_bpp);

      sel = gimp_drawable_get_sub_thumbnail_data(
          GIMP_DRAWABLE(selection), draw_x + offset_x, draw_y + offset_y,
          draw_width, draw_height, &s_w, &s_h, &s_bpp);
      ...
      gimp_preview_area_mask(
          GIMP_PREVIEW_AREA(area), draw_x - xoff - xmin, draw_y - yoff - ymin,
          draw_width, draw_height, type, src, draw_width * d_bpp,
          (buf + (draw_x - x) * d_bpp + (draw_y - y) * d_w * d_bpp), rowstride,
          sel, s_w);
...
static void gimp_drawable_preview_draw_buffer(GimpPreview *preview,
  ...
  gimp_drawable_preview_draw_area(GIMP_DRAWABLE_PREVIEW(preview), x, y, width,
                                  height, buffer, rowstride);
}
...
gimp_drawable_preview_set_drawable(GimpDrawablePreview *drawable_preview,
  ...
  if (gimp_drawable_is_indexed(drawable)) {
    ...
    gimp_preview_area_set_colormap(GIMP_PREVIEW_AREA(area), cmap, num_colors);
    g_free(cmap);
  }
}
...
gboolean _gimp_drawable_preview_get_bounds(GimpDrawable *drawable, gint *xmin,
                                           gint *ymin, gint *xmax, gint *ymax) {
  gint width;
  gint height;
  gint offset_x;
  gint offset_y;
...
GtkWidget *gimp_drawable_preview_new_from_drawable(GimpDrawable *drawable) {
  ...
  return g_object_new(GIMP_TYPE_DRAWABLE_PREVIEW, "drawable", drawable, NULL);
}
