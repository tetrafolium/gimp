...
enum { ADD, REMOVE, MODULE_MODIFIED, LAST_SIGNAL };
...
static void gimp_module_db_load_directory(GimpModuleDB *db, GFile *directory);
static void gimp_module_db_load_module(GimpModuleDB *db, GFile *file);

static GimpModule *gimp_module_db_module_find_by_file(GimpModuleDB *db,
                                                      GFile *file);

static void gimp_module_db_module_dump_func(gpointer data, gpointer user_data);
...
static void gimp_module_db_class_init(GimpModuleDBClass *klass) {
  ...
  db_signals[ADD] =
      g_signal_new("add", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpModuleDBClass, add), NULL, NULL, NULL,
                   G_TYPE_NONE, 1, GIMP_TYPE_MODULE);

  db_signals[REMOVE] =
      g_signal_new("remove", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpModuleDBClass, remove), NULL, NULL, NULL,
                   G_TYPE_NONE, 1, GIMP_TYPE_MODULE);

  db_signals[MODULE_MODIFIED] = g_signal_new(
      "module-modified", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpModuleDBClass, module_modified), NULL, NULL, NULL,
      G_TYPE_NONE, 1, GIMP_TYPE_MODULE);
...
static void gimp_module_db_finalize(GObject *object) {
  ...
  for (list = db->priv->modules; list; list = g_list_next(list)) {
    g_signal_handlers_disconnect_by_func(
        list->data, gimp_module_db_module_modified, object);
  }
...
static gboolean is_in_inhibit_list(GFile *file, const gchar *inhibit_list) {
  gchar *filename;
  gchar *p;
  gint pathlen;
  const gchar *start;
...
void gimp_module_db_set_load_inhibit(GimpModuleDB *db,
  ...
  for (list = db->priv->modules; list; list = g_list_next(list)) {
    ...
    inhibit = is_in_inhibit_list(gimp_module_get_file(module), load_inhibit);
...
void gimp_module_db_refresh(GimpModuleDB *db, const gchar *module_path) {
  ...
  while (list) {
    ...
    if (!gimp_module_is_on_disk(module) && !gimp_module_is_loaded(module)) {
...
static void gimp_module_db_load_module(GimpModuleDB *db, GFile *file) {
  ...
  module = gimp_module_new(file, !load_inhibit, db->priv->verbose);

  g_signal_connect(module, "modified",
                   G_CALLBACK(gimp_module_db_module_modified), db);
...
static void gimp_module_db_module_dump_func(gpointer data, gpointer user_data) {
  static const gchar *const statenames[] = {
      N_("Module error"), N_("Loaded"), N_("Load failed"), N_("Not loaded")};
