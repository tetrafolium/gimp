...
let url =
    "https://gitlab.gnome.org/GNOME/gimp/blob/master/extensions/goat-exercises/goat-exercise-gjs.js";

var Goat = GObject.registerClass({
  GTypeName : 'Goat',
},
                                 class Goat extends Gimp.PlugIn {
  vfunc_query_procedures() { return [ "plug-in-goat-exercise-gjs" ]; }

  vfunc_create_procedure(name) {
    let procedure =
        Gimp.ImageProcedure.new(this, name, Gimp.PDBProcType.PLUGIN, this.run);
    ...
    procedure.set_documentation(
        "Exercise a goat in the JavaScript language (GJS)",
        "Takes a goat for a walk in Javascript with the GJS interpreter", name);
  ...
  run(procedure, run_mode, image, drawable, args, run_data) {
    ...
    if (run_mode == Gimp.RunMode.INTERACTIVE) {
      ...
      let box =
          new Gtk.Box({orientation : Gtk.Orientation.VERTICAL, spacing : 2});
       ...
       */
      let head_text =
          `This plug-in is an exercise in '${
              lang}' to demo plug-in creation.\n` +
      ...
      let contents = imports.byteArray.toString(
          GLib.file_get_contents(System.programInvocationName)[1]);
      if (contents) {
