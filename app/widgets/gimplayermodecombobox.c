...
enum { PROP_0, PROP_CONTEXT, PROP_LAYER_MODE, PROP_GROUP };
...
gimp_layer_mode_combo_box_class_init(GimpLayerModeComboBoxClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_flags("context", NULL, NULL, GIMP_TYPE_LAYER_MODE_CONTEXT,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("group", NULL, NULL, GIMP_TYPE_LAYER_MODE_GROUP,
                        GIMP_LAYER_MODE_GROUP_DEFAULT,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_layer_mode_combo_box_init(GimpLayerModeComboBox *combo) {
  combo->priv = gimp_layer_mode_combo_box_get_instance_private(combo);

  gtk_combo_box_set_row_separator_func(
      GTK_COMBO_BOX(combo), gimp_layer_mode_combo_box_separator_func,
      GINT_TO_POINTER(GIMP_LAYER_MODE_SEPARATOR), NULL);
}
...
static void gimp_layer_mode_combo_box_changed(GtkComboBox *gtk_combo) {
  ...
  if (gimp_int_combo_box_get_active(GIMP_INT_COMBO_BOX(combo), (gint *)&mode)) {
...
GtkWidget *gimp_layer_mode_combo_box_new(GimpLayerModeContext context) {
  return g_object_new(GIMP_TYPE_LAYER_MODE_COMBO_BOX, "context", context, NULL);
}
...
void gimp_layer_mode_combo_box_set_mode(GimpLayerModeComboBox *combo,
  ...
  g_return_if_fail(mode == -1 ||
                   (gimp_layer_mode_get_context(mode) & combo->priv->context));
...
static void gimp_enum_store_add_value(GtkListStore *store,
  ...
  gtk_list_store_set(store, &iter, GIMP_INT_STORE_VALUE, value->value,
                     GIMP_INT_STORE_LABEL, stripped, GIMP_INT_STORE_ABBREV,
                     abbrev, -1);

  g_free(stripped);
}

static void gimp_enum_store_add_separator(GtkListStore *store) {
  GtkTreeIter iter = {
      0,
  };

  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter, GIMP_INT_STORE_VALUE,
                     GIMP_LAYER_MODE_SEPARATOR, -1);
}

static GtkListStore *
gimp_enum_store_new_from_array(GType enum_type, gint n_values,
                               const gint *values,
                               GimpLayerModeContext context) {
  GtkListStore *store;
  GEnumClass *enum_class;
  ...
  store = g_object_new(GIMP_TYPE_ENUM_STORE, "enum-type", enum_type, NULL);
...
static void gimp_layer_mode_combo_box_update_model(GimpLayerModeComboBox *combo,
  ...
  store = gimp_enum_store_new_from_array(GIMP_TYPE_LAYER_MODE, n_modes,
                                         (gint *)modes, combo->priv->context);
