...
gimp_container_tree_view_insert_item(GimpContainerView *view,
                                     ...
                                     gpointer parent_insert_data, gint index);
...
static gboolean gimp_container_tree_view_edit_focus_out(GtkWidget *widget,
                                                        GdkEvent *event,
                                                        gpointer user_data);
static void gimp_container_tree_view_name_started(
    GtkCellRendererText *cell, GtkCellEditable *editable, const gchar *path_str,
...
static void
gimp_container_tree_view_selection_changed(GtkTreeSelection *sel,
                                           GimpContainerTreeView *tree_view);
static gboolean
gimp_container_tree_view_button(GtkWidget *widget, GdkEventButton *bevent,
                                GimpContainerTreeView *tree_view);
static gboolean
gimp_container_tree_view_tooltip(GtkWidget *widget, gint x, gint y,
                                 gboolean keyboard_tip, GtkTooltip *tooltip,
                                 GimpContainerTreeView *tree_view);
static GimpViewable *
gimp_container_tree_view_drag_viewable(GtkWidget *widget, GimpContext **context,
                                       gpointer data);
...
static gboolean gimp_container_tree_view_search_path_foreach(
...
G_DEFINE_TYPE_WITH_CODE(
    GimpContainerTreeView, gimp_container_tree_view, GIMP_TYPE_CONTAINER_BOX,
...
gimp_container_tree_view_class_init(GimpContainerTreeViewClass *klass) {
  ...
  tree_view_signals[EDIT_NAME] =
      g_signal_new("edit-name", G_TYPE_FROM_CLASS(klass),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(GimpContainerTreeViewClass, edit_name), NULL,
                   NULL, NULL, G_TYPE_NONE, 0);

  binding_set = gtk_binding_set_by_class(klass);

  gtk_binding_entry_add_signal(binding_set, GDK_KEY_F2, 0, "edit-name", 0);
}
...
static void gimp_container_tree_view_init(GimpContainerTreeView *tree_view) {
  ...
  gimp_widget_track_monitor(
      ...
      G_CALLBACK(gimp_container_tree_view_monitor_changed), NULL, NULL);
}
...
static void gimp_container_tree_view_constructed(GObject *object) {
  ...
  tree_view->model = gimp_container_tree_store_new(
      view, tree_view->n_model_columns, tree_view->model_columns);

  tree_view->view = g_object_new(
      GTK_TYPE_TREE_VIEW, "model", tree_view->model, "search-column",
      GIMP_CONTAINER_TREE_STORE_COLUMN_NAME, "enable-search", FALSE,
      "headers-visible", FALSE, "has-tooltip", TRUE, "show-expanders",
      GIMP_CONTAINER_VIEW_GET_IFACE(view)->model_is_tree, NULL);
  ...
  gtk_tree_view_column_pack_start(tree_view->main_column,
                                  tree_view->renderer_cell, FALSE);

  gtk_tree_view_column_set_attributes(
      tree_view->main_column, tree_view->renderer_cell, "renderer",
  ...
  gtk_tree_view_column_pack_end(tree_view->main_column,
                                tree_view->priv->name_cell, FALSE);

  gtk_tree_view_column_set_attributes(
      tree_view->main_column, tree_view->priv->name_cell, "text",
      GIMP_CONTAINER_TREE_STORE_COLUMN_NAME, "attributes",
      GIMP_CONTAINER_TREE_STORE_COLUMN_NAME_ATTRIBUTES, "sensitive",
      GIMP_CONTAINER_TREE_STORE_COLUMN_NAME_SENSITIVE, NULL);
  ...
  g_signal_connect(tree_view->view, "drag-failed",
                   G_CALLBACK(gimp_container_tree_view_drag_failed), tree_view);
  g_signal_connect(tree_view->view, "drag-leave",
                   G_CALLBACK(gimp_container_tree_view_drag_leave), tree_view);
  g_signal_connect(tree_view->view, "drag-motion",
                   G_CALLBACK(gimp_container_tree_view_drag_motion), tree_view);
  g_signal_connect(tree_view->view, "drag-drop",
                   G_CALLBACK(gimp_container_tree_view_drag_drop), tree_view);
...
static gboolean gimp_container_tree_view_style_updated_foreach(
    GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data) {
  GimpViewRenderer *renderer;

  gtk_tree_model_get(model, iter, GIMP_CONTAINER_TREE_STORE_COLUMN_RENDERER,
                     &renderer, -1);
...
static void gimp_container_tree_view_style_updated(GtkWidget *widget) {
  ...
  if (tree_view->model)
    gtk_tree_model_foreach(
        tree_view->model, gimp_container_tree_view_style_updated_foreach, NULL);
}
...
static void gimp_container_tree_view_menu_position(GtkMenu *menu, gint *x,
  ...
  if (gimp_container_tree_view_get_selected_single(tree_view, &selected_iter)) {
    ...
    gtk_tree_view_get_cell_area(tree_view->view, path, tree_view->main_column,
                                &cell_rect);
...
static gboolean gimp_container_tree_view_popup_menu(GtkWidget *widget) {
  return gimp_editor_popup_menu(GIMP_EDITOR(widget),
                                gimp_container_tree_view_menu_position, widget);
}
...
GtkWidget *gimp_container_tree_view_new(GimpContainer *container,
  ...
  g_return_val_if_fail(container == NULL || GIMP_IS_CONTAINER(container), NULL);
...
void gimp_container_tree_view_set_main_column_title(
    GimpContainerTreeView *tree_view, const gchar *title) {
  g_return_if_fail(GIMP_IS_CONTAINER_TREE_VIEW(tree_view));

  gtk_tree_view_column_set_title(tree_view->main_column, title);
}

void gimp_container_tree_view_add_toggle_cell(GimpContainerTreeView *tree_view,
  ...
  tree_view->priv->toggle_cells =
      g_list_prepend(tree_view->priv->toggle_cells, cell);
}
...
void gimp_container_tree_view_add_renderer_cell(
  ...
  tree_view->priv->renderer_cells =
      g_list_prepend(tree_view->priv->renderer_cells, cell);

  gimp_container_tree_store_add_renderer_cell(
      GIMP_CONTAINER_TREE_STORE(tree_view->model), cell);
}
...
void gimp_container_tree_view_connect_name_edited(
  ...
  g_object_set(tree_view->priv->name_cell, "mode",
               GTK_CELL_RENDERER_MODE_EDITABLE, "editable", TRUE, NULL);

  if (!g_list_find(tree_view->priv->editable_cells, tree_view->priv->name_cell))
    tree_view->priv->editable_cells = g_list_prepend(
        tree_view->priv->editable_cells, tree_view->priv->name_cell);

  g_signal_connect(tree_view->priv->name_cell, "edited", callback, data);
}

gboolean gimp_container_tree_view_name_edited(
    GtkCellRendererText *cell, const gchar *path_str, const gchar *new_name,
    GimpContainerTreeView *tree_view) {
  GtkTreePath *path;
  GtkTreeIter iter;
...
static void gimp_container_tree_view_set_container(GimpContainerView *view,
  ...
  if (old_container) {
    ...
    g_signal_handlers_disconnect_by_func(
        tree_view->view, gimp_container_tree_view_row_expanded, tree_view);
    if (!container) {
      if (gimp_dnd_viewable_list_source_remove(
              GTK_WIDGET(tree_view->view),
              gimp_container_get_children_type(old_container))) {
        if (GIMP_VIEWABLE_CLASS(
                g_type_class_peek(
                    gimp_container_get_children_type(old_container)))
                ->get_size)
      ...
      g_signal_handlers_disconnect_by_func(
          tree_view->view, gimp_container_tree_view_button, tree_view);
    }
  } else if (container) {
    if (gimp_dnd_drag_source_set_by_type(
            GTK_WIDGET(tree_view->view), GDK_BUTTON1_MASK | GDK_BUTTON2_MASK,
            gimp_container_get_children_type(container), GDK_ACTION_COPY)) {
      gimp_dnd_viewable_list_source_add(
          GTK_WIDGET(tree_view->view),
          gimp_container_get_children_type(container),
          gimp_container_tree_view_drag_viewable_list, tree_view);
      ...
      if (GIMP_VIEWABLE_CLASS(
              g_type_class_peek(gimp_container_get_children_type(container)))
              ->get_size)
    ...
    g_signal_connect(tree_view->view, "button-press-event",
                     G_CALLBACK(gimp_container_tree_view_button), tree_view);
  ...
  if (container) {
    gimp_container_tree_view_expand_rows(tree_view->model, tree_view->view,
                                         NULL);

    g_signal_connect(tree_view->view, "row-collapsed",
...
static void gimp_container_tree_view_set_context(GimpContainerView *view,
  ...
  if (tree_view->model)
    gimp_container_tree_store_set_context(
        GIMP_CONTAINER_TREE_STORE(tree_view->model), context);
...
gimp_container_tree_view_insert_item(GimpContainerView *view,
  ...
  iter = gimp_container_tree_store_insert_item(
      GIMP_CONTAINER_TREE_STORE(tree_view->model), viewable, parent_iter,
      index);
...
static void gimp_container_tree_view_remove_item(GimpContainerView *view,
  ...
  gimp_container_tree_store_remove_item(
      GIMP_CONTAINER_TREE_STORE(tree_view->model), viewable, iter);
...
static void gimp_container_tree_view_reorder_item(GimpContainerView *view,
  ...
  if (iter) {
    ...
    selected =
        gimp_container_tree_view_get_selected_single(tree_view, &selected_iter);
  ...
  gimp_container_tree_store_reorder_item(
      GIMP_CONTAINER_TREE_STORE(tree_view->model), viewable, new_index, iter);
...
static void gimp_container_tree_view_rename_item(GimpContainerView *view,
  ...
  if (gimp_container_tree_store_rename_item(
          GIMP_CONTAINER_TREE_STORE(tree_view->model), viewable, iter)) {
    gtk_tree_view_columns_autosize(tree_view->view);
  }
}
...
static void gimp_container_tree_view_expand_item(GimpContainerView *view,
  ...
  gtk_tree_model_get(tree_view->model, iter,
                     GIMP_CONTAINER_TREE_STORE_COLUMN_RENDERER, &renderer, -1);

  if (renderer) {
    GtkTreePath *path = gtk_tree_model_get_path(tree_view->model, iter);

    g_signal_handlers_block_by_func(
        tree_view, gimp_container_tree_view_row_expanded, view);
...
static gboolean gimp_container_tree_view_select_item(GimpContainerView *view,
  ...
  if (viewable && insert_data) {
    ...
    g_signal_handlers_unblock_by_func(
        tree_view->priv->selection, gimp_container_tree_view_selection_changed,
        tree_view);

    gtk_tree_view_scroll_to_cell(tree_view->view, path, NULL, FALSE, 0.0, 0.0);
...
static gboolean gimp_container_tree_view_select_items(GimpContainerView *view,
  ...
  gtk_tree_selection_unselect_all(tree_view->priv->selection);
  for (item = items, path = paths; item && path;
       item = item->next, path = path->next) {
    GtkTreePath *parent_path;
  ...
  if (paths) {
    ...
    if (scroll_to_first)
      gtk_tree_view_scroll_to_cell(tree_view->view, paths->data, NULL, FALSE,
                                   0.0, 0.0);
  }
...
static void gimp_container_tree_view_clear_items(GimpContainerView *view) {
  ...
  gimp_container_tree_store_clear_items(
      GIMP_CONTAINER_TREE_STORE(tree_view->model));
...
static GtkCellRenderer *gimp_container_tree_view_find_click_cell(
    GtkWidget *widget, GList *cells, GtkTreeViewColumn *column,
    GdkRectangle *column_area, gint tree_x, gint tree_y) {
  GList *list;
  ...
  for (list = cells; list; list = g_list_next(list)) {
    ...
    if (gtk_cell_renderer_get_visible(renderer) &&
        gtk_tree_view_column_cell_get_position(column, renderer, &start,
                                               &width)) {
      gint xpad, ypad;
      gint x;
      ...
      if (tree_x >= x + xpad && tree_x < x + width - xpad) {
        return renderer;
      }
    }
  }
...
gimp_container_tree_view_button(GtkWidget *widget, GdkEventButton *bevent,
  ...
  if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(widget), bevent->x, bevent->y,
    ...
    if (!modifiers || (modifiers & ~(gimp_get_extend_selection_mask() |
    ...
    gtk_tree_view_get_cell_area(tree_view->view, path, column, &column_area);
    ...
    if (bevent->button == 1 &&
      ...
      if (!gimp_container_tree_view_find_click_cell(
              widget, cells, column, &column_area, bevent->x, bevent->y)) {
        if (bevent->state & gimp_get_extend_selection_mask() &&
            ...
            !gtk_tree_view_is_blank_at_pos(GTK_TREE_VIEW(widget), bevent->x,
                                           bevent->y, NULL, NULL, NULL, NULL)) {
          ...
          do {
            GtkTreePath *path2 =
                gtk_tree_model_get_path(tree_view->model, &iter);
          ...
          gtk_tree_view_scroll_to_cell(tree_view->view, path, NULL, FALSE, 0.0,
                                       0.0);
    ...
    toggled_cell = gimp_container_tree_view_find_click_cell(
        widget, tree_view->priv->toggle_cells, column, &column_area, bevent->x,
        bevent->y);

    if (!toggled_cell)
      clicked_cell =
          (GimpCellRendererViewable *)gimp_container_tree_view_find_click_cell(
              widget, tree_view->priv->renderer_cells, column, &column_area,
              bevent->x, bevent->y);

    if (!toggled_cell && !clicked_cell) {
      edit_cell = gimp_container_tree_view_find_click_cell(
          widget, tree_view->priv->editable_cells, column, &column_area,
          bevent->x, bevent->y);
      ...
      if (bevent->type == GDK_BUTTON_PRESS ||
        ...
        if (!toggled_cell) {
          ...
          if (bevent->type == GDK_BUTTON_RELEASE && clicked_cell)
            handled = gimp_cell_renderer_viewable_pre_clicked(
                clicked_cell, path_str, bevent->state);

          if (!handled && !multisel_mode && !modifiers) {
            if (!tree_view->priv->editing_path &&
                (bevent->type == GDK_BUTTON_RELEASE ||
                 !gimp_container_view_is_item_selected(container_view,
                                                       renderer->viewable)))
          ...
          /* Multi selection will be handled by
           * gimp_container_tree_view_selection_changed() */
        ...
        if (gimp_container_view_get_container(container_view)) {
          ...
          gtk_tree_view_column_cell_set_cell_data(column, tree_view->model,
                                                  &iter, FALSE, FALSE);
            ...
            if (toggled_cell) {
              if (GIMP_IS_CELL_RENDERER_TOGGLE(toggled_cell)) {
                gimp_cell_renderer_toggle_clicked(
                    GIMP_CELL_RENDERER_TOGGLE(toggled_cell), path_str,
                    bevent->state);
              } else if (GIMP_IS_CELL_RENDERER_BUTTON(toggled_cell)) {
                gimp_cell_renderer_button_clicked(
                    GIMP_CELL_RENDERER_BUTTON(toggled_cell), path_str,
                    bevent->state);
              }
            } else if (clicked_cell) {
              gimp_cell_renderer_viewable_clicked(clicked_cell, path_str,
                                                  bevent->state);
            }
        ...
        if (success) {
          if (edit_cell) {
            if (gimp_viewable_is_name_editable(renderer->viewable)) {
              gtk_tree_view_set_cursor_on_cell(tree_view->view, path, column,
                                               edit_cell, TRUE);
            } else {
...
static GdkPixbuf *gimp_container_tree_view_drag_pixbuf(GtkWidget *widget,
  ...
  if (renderer && gimp_viewable_get_size(renderer->viewable, &width, &height))
    return gimp_viewable_get_new_pixbuf(renderer->viewable, renderer->context,
                                        width, height);
...
static gint gimp_container_tree_view_get_selected(GimpContainerView *view,
  ...
  *items = NULL;
  ...
  for (current_row = selected_paths; current_row;
  ...
  *items = g_list_reverse(*items);
  ...
  for (current_row = removed_paths; current_row;
...
static void gimp_container_tree_view_row_expanded(GtkTreeView *tree_view,
  ...
  if (renderer) {
    ...
    gimp_viewable_set_expanded(renderer->viewable, expanded);
    if (expanded) {
      g_signal_handlers_block_by_func(
          tree_view, gimp_container_tree_view_row_expanded, view);
...
static gboolean gimp_container_tree_view_monitor_changed_foreach(
    GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data) {
  GimpViewRenderer *renderer;
...
gimp_container_tree_view_monitor_changed(GimpContainerTreeView *view) {
  gtk_tree_model_foreach(
      view->model, gimp_container_tree_view_monitor_changed_foreach, NULL);
}
...
gimp_container_tree_view_get_path(GimpContainerTreeView *tree_view,
  ...
  gtk_tree_model_foreach(
      GTK_TREE_MODEL(tree_view->model),
