...
static void gimp_pdb_dialog_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);

static void gimp_pdb_dialog_response(GtkDialog *dialog, gint response_id);
...
static void gimp_pdb_dialog_class_init(GimpPdbDialogClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_SELECT_TYPE,
      g_param_spec_gtype("select-type", NULL, NULL, GIMP_TYPE_OBJECT,
                         GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_INITIAL_OBJECT,
      g_param_spec_object("initial-object", NULL, NULL, GIMP_TYPE_OBJECT,
                          GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
...
static void gimp_pdb_dialog_constructed(GObject *object) {
  ...
  dialog->context = gimp_context_new(dialog->caller_context->gimp,
                                     G_OBJECT_TYPE_NAME(object), NULL);
  ...
  g_signal_connect_object(dialog->context, signal_name,
                          G_CALLBACK(gimp_pdb_dialog_context_changed), dialog,
                          0);
  g_signal_connect_object(
      dialog->context->gimp->plug_in_manager, "plug-in-closed",
      G_CALLBACK(gimp_pdb_dialog_plug_in_closed), dialog, 0);
}
...
static void gimp_pdb_dialog_response(GtkDialog *gtk_dialog, gint response_id) {
...
void gimp_pdb_dialog_run_callback(GimpPdbDialog *dialog, gboolean closing) {
  ...
  if (object && klass->run_callback && dialog->callback_name &&
    ...
    if (gimp_pdb_lookup_procedure(dialog->pdb, dialog->callback_name)) {
      ...
      if (g_value_get_enum(gimp_value_array_index(return_vals, 0)) !=
        ...
        gimp_message(dialog->context->gimp, G_OBJECT(dialog),
                     GIMP_MESSAGE_ERROR, _("Unable to run %s callback.\n%s"),
                     g_type_name(G_TYPE_FROM_INSTANCE(dialog)), message);
      } else if (error) {
...
GimpPdbDialog *gimp_pdb_dialog_get_by_callback(GimpPdbDialogClass *klass,
  ...
  for (list = klass->dialogs; list; list = g_list_next(list)) {
    ...
    if (dialog->callback_name && !strcmp(callback_name, dialog->callback_name))
      return dialog;
  }
