...
static void gimp_symmetry_editor_class_init(GimpSymmetryEditorClass *klass) {
  GimpImageEditorClass *image_editor_class = GIMP_IMAGE_EDITOR_CLASS(klass);

  image_editor_class->set_image = gimp_symmetry_editor_set_image;
}

static void gimp_symmetry_editor_init(GimpSymmetryEditor *editor) {
...
static void gimp_symmetry_editor_set_image(GimpImageEditor *image_editor,
                                           GimpImage *image) {
  GimpSymmetryEditor *editor = GIMP_SYMMETRY_EDITOR(image_editor);

  if (image_editor->image) {
    g_signal_handlers_disconnect_by_func(
        image_editor->image, G_CALLBACK(gimp_symmetry_editor_symmetry_notify),
        editor);
  }
  ...
  if (image_editor->image) {
    ...
    for (sym_iter = syms; sym_iter; sym_iter = g_list_next(sym_iter)) {
      ...
      gtk_list_store_set(store, &iter, GIMP_INT_STORE_LABEL, klass->label,
                         GIMP_INT_STORE_USER_DATA, sym_iter->data, -1);
      g_type_class_unref(klass);
    }
    g_list_free(syms);

    gtk_list_store_prepend(store, &iter);
    gtk_list_store_set(store, &iter, GIMP_INT_STORE_LABEL, _("None"),
                       GIMP_INT_STORE_USER_DATA, GIMP_TYPE_SYMMETRY, -1);
    editor->p->menu = gimp_prop_pointer_combo_box_new(
        G_OBJECT(image_editor->image), "symmetry", GIMP_INT_STORE(store));
    ...
    gtk_box_pack_start(GTK_BOX(editor), editor->p->menu, FALSE, FALSE, 0);
    ...
    g_signal_connect(image_editor->image, "notify::symmetry",
                     G_CALLBACK(gimp_symmetry_editor_symmetry_notify), editor);
...
static void gimp_symmetry_editor_symmetry_updated(GimpSymmetry *symmetry,
  ...
  if (image != gimp_image_editor_get_image(GIMP_IMAGE_EDITOR(editor)) ||
      symmetry != gimp_image_get_active_symmetry(image)) {
    g_signal_handlers_disconnect_by_func(
        symmetry, gimp_symmetry_editor_symmetry_updated, editor);
    return;
  }
...
static void gimp_symmetry_editor_set_options(GimpSymmetryEditor *editor,
  ...
  if (symmetry && G_TYPE_FROM_INSTANCE(symmetry) != GIMP_TYPE_SYMMETRY) {
    ...
    gui = gimp_prop_gui_new(
        G_OBJECT(symmetry), GIMP_TYPE_SYMMETRY, GIMP_SYMMETRY_PARAM_GUI,
        GEGL_RECTANGLE(0, 0, gimp_image_get_width(image),
                       gimp_image_get_height(image)),
        GIMP_IMAGE_EDITOR(editor)->context, NULL, NULL, NULL);
    gtk_box_pack_start(GTK_BOX(editor->p->options_vbox), gui, FALSE, FALSE, 0);
  }
}
...
GtkWidget *gimp_symmetry_editor_new(GimpMenuFactory *menu_factory) {
  ...
  return g_object_new(GIMP_TYPE_SYMMETRY_EDITOR, "menu-factory", menu_factory,
                      NULL);
}
