...
enum { PROP_0, PROP_CONTAINER_VIEW, PROP_USE_NAME };
...
#define GET_PRIVATE(store)                                                     \
  ((GimpContainerTreeStorePrivate *)                                           \
       gimp_container_tree_store_get_instance_private(                         \
...
gimp_container_tree_store_class_init(GimpContainerTreeStoreClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("container-view", NULL, NULL,
  ...
  g_object_class_install_property(object_class, PROP_USE_NAME,
                                  g_param_spec_boolean("use-name", NULL, NULL,
                                                       FALSE,
                                                       GIMP_PARAM_READWRITE));
}

static void gimp_container_tree_store_init(GimpContainerTreeStore *store) {}
...
static void gimp_container_tree_store_set_property(GObject *object,
  ...
  private
    ->container_view = g_value_get_object(value); /* don't ref */
    break;
  case PROP_USE_NAME:
  private
    ->use_name = g_value_get_boolean(value);
    break;
...
GtkTreeModel *gimp_container_tree_store_new(GimpContainerView *container_view,
  ...
  store = g_object_new(GIMP_TYPE_CONTAINER_TREE_STORE, "container-view",
                       container_view, NULL);
...
private
  ->renderer_cells = g_list_prepend(private->renderer_cells, cell);
}
...
static gboolean gimp_container_tree_store_set_context_foreach(
...
void gimp_container_tree_store_reorder_item(GimpContainerTreeStore *store,
                                            GimpViewable *viewable,
                                            gint new_index, GtkTreeIter *iter) {
  GimpContainerTreeStorePrivate *private;
  GimpViewable *parent;
...
private
  ...
  if (new_index == -1 ||
    ...
    if (new_index != old_index) {
      ...
      if (new_index > old_index)
        gtk_tree_store_move_after(GTK_TREE_STORE(store), iter, &place_iter);
      else
        gtk_tree_store_move_before(GTK_TREE_STORE(store), iter, &place_iter);
    }
...
gboolean gimp_container_tree_store_rename_item(GimpContainerTreeStore *store,
  ...
  if (iter) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(store), iter,
                       GIMP_CONTAINER_TREE_STORE_COLUMN_NAME, &old_name, -1);
...
static gboolean gimp_container_tree_store_set_view_size_foreach(
    GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data) {
...
private
  ...
  size_data.view_size = gimp_container_view_get_view_size(
      private->container_view, &size_data.border_width);
...
void gimp_container_tree_store_columns_init(GType *types, gint *n_types) {
  ...
  gimp_assert(
      ...
      gimp_container_tree_store_columns_add(types, n_types, G_TYPE_STRING));
  ...
  gimp_assert(
      ...
      gimp_container_tree_store_columns_add(types, n_types, G_TYPE_BOOLEAN));

  gimp_assert(
      GIMP_CONTAINER_TREE_STORE_COLUMN_USER_DATA ==
      gimp_container_tree_store_columns_add(types, n_types, G_TYPE_POINTER));
}

gint gimp_container_tree_store_columns_add(GType *types, gint *n_types,
                                           GType type) {
...
static void gimp_container_tree_store_set(GimpContainerTreeStore *store,
  ...
  view_size =
      gimp_container_view_get_view_size(private->container_view, &border_width);

  renderer = gimp_view_renderer_new(context, G_TYPE_FROM_INSTANCE(viewable),
                                    view_size, border_width, FALSE);
  ...
  gtk_tree_store_set(GTK_TREE_STORE(store), iter,
                     ...
                     GIMP_CONTAINER_TREE_STORE_COLUMN_NAME_SENSITIVE, TRUE, -1);
...
gimp_container_tree_store_renderer_update(GimpViewRenderer *renderer,
  ...
  iter =
      gimp_container_view_lookup(private->container_view, renderer->viewable);
