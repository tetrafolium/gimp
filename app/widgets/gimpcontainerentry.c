...
G_DEFINE_TYPE_WITH_CODE(
    GimpContainerEntry, gimp_container_entry, GTK_TYPE_ENTRY,
...
static void gimp_container_entry_init(GimpContainerEntry *entry) {
  ...
  completion =
      g_object_new(GTK_TYPE_ENTRY_COMPLETION, "inline-completion", TRUE,
                   "popup-single-match", FALSE, "popup-set-width", FALSE, NULL);

  gimp_container_tree_store_columns_init(types, &n_types);

  model =
      gimp_container_tree_store_new(GIMP_CONTAINER_VIEW(entry), n_types, types);
  ...
  g_signal_connect(completion, "match-selected",
                   G_CALLBACK(gimp_container_entry_match_selected), entry);
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(completion), cell, "renderer",
  ...
  g_signal_connect(entry, "changed", G_CALLBACK(gimp_container_entry_changed),
                   entry);
}
...
static GtkTreeModel *gimp_container_entry_get_model(GimpContainerView *view) {
  GtkEntryCompletion *completion;
...
static gpointer gimp_container_entry_insert_item(GimpContainerView *view,
  ...
  return gimp_container_tree_store_insert_item(
...
static void gimp_container_entry_rename_item(GimpContainerView *view,
  ...
  if (viewable == container_entry->viewable) {
    g_signal_handlers_block_by_func(entry, gimp_container_entry_changed, view);

    gtk_entry_set_text(entry, gimp_object_get_name(viewable));

    g_signal_handlers_unblock_by_func(entry, gimp_container_entry_changed,
                                      view);
  }
...
static gboolean gimp_container_entry_select_item(GimpContainerView *view,
  ...
  g_signal_handlers_unblock_by_func(entry, gimp_container_entry_changed, view);
