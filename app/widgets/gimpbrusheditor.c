...
static void gimp_brush_editor_set_data(GimpDataEditor *editor, GimpData *data);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpBrushEditor, gimp_brush_editor, GIMP_TYPE_DATA_EDITOR,
...
static void gimp_brush_editor_docked_iface_init(GimpDockedInterface *iface) {
...
static void gimp_brush_editor_init(GimpBrushEditor *editor) {
  ...
  data_editor->view = gimp_view_new_full_by_types(
      NULL, GIMP_TYPE_VIEW, GIMP_TYPE_BRUSH, BRUSH_VIEW_SIZE, BRUSH_VIEW_SIZE,
      0, FALSE, FALSE, TRUE);
  ...
  box = gimp_enum_icon_box_new(GIMP_TYPE_BRUSH_GENERATED_SHAPE, "gimp-shape",
                               GTK_ICON_SIZE_MENU,
                               G_CALLBACK(gimp_brush_editor_update_shape),
                               editor, NULL, &editor->shape_group);
  ...
  g_signal_connect(editor->radius_data, "value-changed",
                   G_CALLBACK(gimp_brush_editor_update_brush), editor);
...
static void gimp_brush_editor_set_data(GimpDataEditor *editor, GimpData *data) {
  ...
  if (editor->data)
    g_signal_handlers_disconnect_by_func(
        editor->data, gimp_brush_editor_notify_brush, editor);

  GIMP_DATA_EDITOR_CLASS(parent_class)->set_data(editor, data);

  if (editor->data)
    g_signal_connect(editor->data, "notify",
                     G_CALLBACK(gimp_brush_editor_notify_brush), editor);
  ...
  gtk_widget_set_sensitive(brush_editor->options_box, editor->data_editable);
...
GtkWidget *gimp_brush_editor_new(GimpContext *context,
  ...
  return g_object_new(GIMP_TYPE_BRUSH_EDITOR, "menu-factory", menu_factory,
                      "menu-identifier", "<BrushEditor>", "ui-path",
                      "/brush-editor-popup", "data-factory",
                      context->gimp->brush_factory, "context", context, "data",
                      gimp_context_get_brush(context), NULL);
}
...
static void gimp_brush_editor_update_brush(GtkAdjustment *adjustment,
  ...
  g_signal_handlers_block_by_func(brush, gimp_brush_editor_notify_brush,
                                  editor);
  ...
  g_signal_handlers_unblock_by_func(brush, gimp_brush_editor_notify_brush,
                                    editor);
}
...
static void gimp_brush_editor_notify_brush(GimpBrushGenerated *brush,
  ...
  if (!strcmp(pspec->name, "shape")) {
    g_signal_handlers_block_by_func(editor->shape_group,
                                    gimp_brush_editor_update_shape, editor);
  ...
  if (adj) {
    g_signal_handlers_block_by_func(adj, gimp_brush_editor_update_brush,
                                    editor);

    gtk_adjustment_set_value(adj, value);

    g_signal_handlers_unblock_by_func(adj, gimp_brush_editor_update_brush,
                                      editor);
  }
}
