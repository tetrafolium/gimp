...
static void gimp_tag_popup_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_tag_popup_get_property(GObject *object, guint property_id,
                                        GValue *value, GParamSpec *pspec);

static void gimp_tag_popup_get_border(GtkWidget *widget, GtkBorder *border);
static gboolean gimp_tag_popup_border_draw(GtkWidget *widget, cairo_t *cr,
                                           GimpTagPopup *popup);
static void gimp_tag_popup_list_style_updated(GtkWidget *widget,
                                              GimpTagPopup *popup);
static gboolean gimp_tag_popup_list_draw(GtkWidget *widget, cairo_t *cr,
                                         GimpTagPopup *popup);
static gboolean gimp_tag_popup_border_event(GtkWidget *widget, GdkEvent *event);
static gboolean gimp_tag_popup_list_event(GtkWidget *widget, GdkEvent *event,
                                          GimpTagPopup *popup);
static gboolean gimp_tag_popup_is_in_tag(PopupTagData *tag_data, gint x,
                                         gint y);
...
static void gimp_tag_popup_check_can_toggle(GimpTagged *tagged,
                                            GimpTagPopup *popup);
static gint gimp_tag_popup_layout_tags(GimpTagPopup *popup, gint width);
...
static void gimp_tag_popup_scroll_by(GimpTagPopup *popup, gint step,
                                     const GdkEvent *event);
static void gimp_tag_popup_handle_scrolling(GimpTagPopup *popup, gint x, gint y,
                                            gboolean enter, gboolean motion);
...
static void get_arrows_visible_area(GimpTagPopup *combo_entry,
                                    GdkRectangle *border, GdkRectangle *upper,
                                    GdkRectangle *lower, gint *arrow_space);
static void get_arrows_sensitive_area(GimpTagPopup *popup, GdkRectangle *upper,
                                      GdkRectangle *lower);
...
static void gimp_tag_popup_class_init(GimpTagPopupClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("owner", NULL, NULL, GIMP_TYPE_COMBO_TAG_ENTRY,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_tag_popup_init(GimpTagPopup *popup) {
  ...
  gtk_widget_add_events(widget,
                        GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
                            GDK_POINTER_MOTION_MASK | GDK_KEY_RELEASE_MASK |
  ...
  gtk_widget_add_events(popup->tag_area, GDK_BUTTON_PRESS_MASK |
  ...
  g_signal_connect(popup->border_area, "draw",
                   G_CALLBACK(gimp_tag_popup_border_draw), popup);
  g_signal_connect(popup, "event", G_CALLBACK(gimp_tag_popup_border_event),
                   NULL);
  g_signal_connect(popup->tag_area, "style-updated",
                   G_CALLBACK(gimp_tag_popup_list_style_updated), popup);
  g_signal_connect(popup->tag_area, "draw",
                   G_CALLBACK(gimp_tag_popup_list_draw), popup);
  g_signal_connect(popup->tag_area, "event",
                   G_CALLBACK(gimp_tag_popup_list_event), popup);
}
...
static void gimp_tag_popup_constructed(GObject *object) {
  ...
  gtk_widget_style_get(GTK_WIDGET(popup), "scroll-arrow-vlength",
                       &popup->scroll_arrow_height, NULL);
  ...
  for (i = 0, tag_iterator = tag_list; i < popup->tag_count;
  ...
  width = (entry_allocation.width - popup_border.left - popup_border.right);
  height = (gimp_tag_popup_layout_tags(popup, width) + popup_border.top +
            popup_border.bottom);
  ...
  if (popup_height < height) {
    ...
    gtk_widget_set_margin_top(popup->tag_area, popup->scroll_arrow_height + 2);
...
GtkWidget *gimp_tag_popup_new(GimpComboTagEntry *combo_entry) {
  ...
  return g_object_new(GIMP_TYPE_TAG_POPUP, "type", GTK_WINDOW_POPUP, "owner",
                      combo_entry, NULL);
}
...
void gimp_tag_popup_show(GimpTagPopup *popup, GdkEvent *event) {
  ...
  if (gdk_seat_grab(gdk_event_get_seat(event), gtk_widget_get_window(widget),
                    GDK_SEAT_CAPABILITY_ALL, TRUE, NULL, event, NULL,
...
static gint gimp_tag_popup_layout_tags(GimpTagPopup *popup, gint width) {
  ...
  space_width =
      PANGO_PIXELS(pango_font_metrics_get_approximate_char_width(font_metrics));
  ...
  for (i = 0; i < popup->tag_count; i++) {
    ...
    pango_layout_set_text(popup->layout, gimp_tag_get_name(tag_data->tag), -1);
    ...
    if (x + space_width + tag_data->bounds.width + GIMP_TAG_POPUP_MARGIN - 1 >
        width) {
      x = GIMP_TAG_POPUP_MARGIN;
      y += line_height + 2 * GIMP_TAG_POPUP_PADDING +
           GIMP_TAG_POPUP_LINE_SPACING;
    }
  ...
  if (gtk_widget_get_direction(GTK_WIDGET(popup)) == GTK_TEXT_DIR_RTL) {
    for (i = 0; i < popup->tag_count; i++) {
      ...
      tag_data->bounds.x =
          (width - tag_data->bounds.x - tag_data->bounds.width);
    }
  }
...
static gboolean gimp_tag_popup_border_draw(GtkWidget *widget, cairo_t *cr,
  ...
  gtk_render_frame(style, cr, 0, 0, gtk_widget_get_allocated_width(widget),
                   gtk_widget_get_allocated_height(widget));
  ...
  if (popup->arrows_visible) {
    ...
    gtk_render_background(style, cr, upper.x, upper.y, upper.width,
                          upper.height);
    gtk_render_frame(style, cr, upper.x, upper.y, upper.width, upper.height);

    gtk_render_arrow(style, cr, 0, upper.x + (upper.width - arrow_size) / 2,
                     upper.y + popup_border.top +
                         (arrow_space - arrow_size) / 2,
                     arrow_size);
    ...
    gtk_render_background(style, cr, lower.x, lower.y, lower.width,
                          lower.height);
    gtk_render_frame(style, cr, lower.x, lower.y, lower.width, lower.height);

    gtk_render_arrow(style, cr, G_PI, lower.x + (lower.width - arrow_size) / 2,
                     lower.y + popup_border.top +
                         (arrow_space - arrow_size) / 2,
                     arrow_size);
...
static gboolean gimp_tag_popup_border_event(GtkWidget *widget,
  ...
  if (event->type == GDK_BUTTON_PRESS) {
    ...
    gdk_window_get_device_position(gtk_widget_get_window(widget),
                                   gdk_event_get_device(event), &x, &y, NULL);

    if (button_event->window != gtk_widget_get_window(popup->tag_area) &&
        (x < allocation.y || y < allocation.x ||
    ...
    gimp_tag_popup_handle_scrolling(
        popup, x, y, motion_event->window == gtk_widget_get_window(widget),
        TRUE);
...
static gboolean gimp_tag_popup_list_draw(GtkWidget *widget, cairo_t *cr,
  ...
  for (i = 0; i < popup->tag_count; i++) {
    ...
    if (tag_data->state_flags &
        (GTK_STATE_FLAG_SELECTED | GTK_STATE_FLAG_INSENSITIVE)) {
      gtk_render_background(style, cr, tag_data->bounds.x,
                            tag_data->bounds.y - popup->scroll_y,
                            tag_data->bounds.width, tag_data->bounds.height);
    }

    gtk_render_layout(
        style, cr, (tag_data->bounds.x + GIMP_TAG_POPUP_PADDING),
        (tag_data->bounds.y - popup->scroll_y + GIMP_TAG_POPUP_PADDING),
        popup->layout);

    if (tag_data == popup->prelight &&
        !(tag_data->state_flags & GTK_STATE_FLAG_INSENSITIVE) &&
        !popup->single_select_disabled) {
      gtk_render_focus(style, cr, tag_data->bounds.x,
                       tag_data->bounds.y - popup->scroll_y,
                       tag_data->bounds.width, tag_data->bounds.height);
    }
...
static gboolean gimp_tag_popup_is_in_tag(PopupTagData *tag_data, gint x,
                                         gint y) {
  if (x >= tag_data->bounds.x && y >= tag_data->bounds.y &&
...
static void gimp_tag_popup_queue_draw_tag(GimpTagPopup *popup,
                                          PopupTagData *tag_data) {
  gtk_widget_queue_draw_area(popup->tag_area, tag_data->bounds.x,
                             tag_data->bounds.y - popup->scroll_y,
                             tag_data->bounds.width, tag_data->bounds.height);
}
...
static int gimp_tag_popup_data_compare(const void *a, const void *b) {
...
static void gimp_tag_popup_check_can_toggle(GimpTagged *tagged,
  ...
  for (iterator = gimp_tagged_get_tags(tagged); iterator;
    ...
    search_result = (PopupTagData *)bsearch(
        &search_key, popup->tag_data, popup->tag_count, sizeof(PopupTagData),
...
static gboolean gimp_tag_popup_scroll_timeout(gpointer data) {
  ...
  g_object_get(gtk_widget_get_settings(GTK_WIDGET(popup)),
               "gtk-touchscreen-mode", &touchscreen_mode, NULL);
...
static gboolean gimp_tag_popup_scroll_timeout_initial(gpointer data) {
  ...
  g_object_get(gtk_widget_get_settings(GTK_WIDGET(popup)), "gtk-timeout-repeat",
               &timeout, "gtk-touchscreen-mode", &touchscreen_mode, NULL);
  ...
  popup->scroll_timeout_id =
      gdk_threads_add_timeout(timeout, gimp_tag_popup_scroll_timeout, popup);
...
static void gimp_tag_popup_start_scrolling(GimpTagPopup *popup) {
  ...
  popup->scroll_timeout_id = gdk_threads_add_timeout(
      timeout, gimp_tag_popup_scroll_timeout_initial, popup);
}
...
static void gimp_tag_popup_scroll_by(GimpTagPopup *popup, gint step,
  ...
  if (new_scroll_y < 0) {
    ...
    arrow_state =
        (popup->upper_arrow_prelight ? GTK_STATE_PRELIGHT : GTK_STATE_NORMAL);
  }
  ...
  if (new_scroll_y >= popup->scroll_height) {
    ...
    arrow_state =
        (popup->lower_arrow_prelight ? GTK_STATE_PRELIGHT : GTK_STATE_NORMAL);
  }
...
static void gimp_tag_popup_handle_scrolling(GimpTagPopup *popup, gint x, gint y,
  ...
  in_arrow = FALSE;
  if (popup->arrows_visible && x >= rect.x && x < rect.x + rect.width &&
    ...
    if (popup->arrows_visible) {
      ...
      } else /* !touchscreen_mode */
      {
        ...
        if (enter && in_arrow &&
          ...
          gimp_tag_popup_remove_scroll_timeout(popup);
          popup->scroll_step =
              (scroll_fast ? -MENU_SCROLL_STEP2 : -MENU_SCROLL_STEP1);

          popup->scroll_timeout_id = gdk_threads_add_timeout(
              scroll_fast ? MENU_SCROLL_TIMEOUT2 : MENU_SCROLL_TIMEOUT1,
...
static gboolean gimp_tag_popup_button_scroll(GimpTagPopup *popup,
  ...
  if (popup->upper_arrow_prelight || popup->lower_arrow_prelight) {
    ...
    if (touchscreen_mode)
      gimp_tag_popup_handle_scrolling(popup, event->x_root, event->y_root,
                                      event->type == GDK_BUTTON_PRESS, FALSE);
...
static void get_arrows_visible_area(GimpTagPopup *popup, GdkRectangle *border,
...
static void get_arrows_sensitive_area(GimpTagPopup *popup, GdkRectangle *upper,
  ...
  get_arrows_visible_area(popup, &tmp_border, &tmp_upper, &tmp_lower,
                          &tmp_arrow_space);
