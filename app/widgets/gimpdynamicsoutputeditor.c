...
#define GET_PRIVATE(editor)                                                    \
  ((GimpDynamicsOutputEditorPrivate *)                                         \
       gimp_dynamics_output_editor_get_instance_private(                       \
...
static void
gimp_dynamics_output_editor_input_toggled(GtkCellRenderer *cell, gchar *path,
                                          GimpDynamicsOutputEditor *editor);
...
gimp_dynamics_output_editor_class_init(GimpDynamicsOutputEditorClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_OUTPUT,
      g_param_spec_object("output", NULL, NULL, GIMP_TYPE_DYNAMICS_OUTPUT,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_dynamics_output_editor_init(GimpDynamicsOutputEditor *editor) {
...
private
  ->curve_view = gimp_curve_view_new();
  g_object_set(private->curve_view, "border-width", CURVE_BORDER, NULL);

  g_object_get(private->output, "type", &output_type, NULL);

  if (gimp_enum_get_value(GIMP_TYPE_DYNAMICS_OUTPUT_TYPE, output_type, NULL,
                          NULL, &type_desc, NULL))
    g_object_set(private->curve_view, "y-axis-label", type_desc, NULL);
  ...
  g_signal_connect(button, "clicked",
                   G_CALLBACK(gimp_dynamics_output_editor_curve_reset), editor);

private
  ->input_list = gtk_list_store_new(INPUT_N_COLUMNS, G_TYPE_INT, G_TYPE_BOOLEAN,
                                    G_TYPE_STRING, GIMP_TYPE_RGB);

  for (i = 0; i < G_N_ELEMENTS(inputs); i++) {
    gboolean use_input;

    g_object_get(private->output, inputs[i].use_property, &use_input, NULL);

    gtk_list_store_insert_with_values(
        private->input_list, &private->input_iters[i], -1, INPUT_COLUMN_INDEX,
        i, INPUT_COLUMN_USE_INPUT, use_input, INPUT_COLUMN_NAME,
        gettext(inputs[i].label), INPUT_COLUMN_COLOR, &inputs[i].color, -1);
  }
  ...
  g_object_set(cell, "mode", GTK_CELL_RENDERER_MODE_ACTIVATABLE, "activatable",
               TRUE, NULL);
  ...
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(view), -1, NULL, gimp_cell_renderer_color_new(), "color",
      INPUT_COLUMN_COLOR, NULL);

  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(view), -1, NULL,
                                              cell, "active",
                                              INPUT_COLUMN_USE_INPUT, NULL);

  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(view), -1, NULL,
                                              gtk_cell_renderer_text_new(),
                                              "text", INPUT_COLUMN_NAME, NULL);
...
static void gimp_dynamics_output_editor_set_property(GObject *object,
  ...
  case PROP_OUTPUT:
  private
    ->output = g_value_dup_object(value);
    break;
...
gimp_dynamics_output_editor_input_selected(GtkTreeSelection *selection,
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, INPUT_COLUMN_INDEX, &input, -1);
...
gimp_dynamics_output_editor_input_toggled(GtkCellRenderer *cell, gchar *path,
  ...
  if (gtk_tree_model_get_iter_from_string(model, &iter, path)) {
    ...
    gtk_tree_model_get(model, &iter, INPUT_COLUMN_INDEX, &input,
                       INPUT_COLUMN_USE_INPUT, &use, -1);

    g_object_set(private->output, inputs[input].use_property, !use, NULL);
  }
}
...
gimp_dynamics_output_editor_activate_input(GimpDynamicsOutputEditor *editor,
  ...
  for (i = 0; i < G_N_ELEMENTS(inputs); i++) {
    ...
    g_object_get(private->output, inputs[i].use_property, &use_input,
                 inputs[i].curve_property, &input_curve, NULL);

    if (input == i) {
      gimp_curve_view_set_curve(GIMP_CURVE_VIEW(private->curve_view),
                                input_curve, INPUT_COLOR(i));
    private
      ->active_curve = input_curve;
...
gimp_dynamics_output_editor_notify_output(GimpDynamicsOutput *output,
  ...
  for (i = 0; i < G_N_ELEMENTS(inputs); i++) {
    if (!strcmp(pspec->name, inputs[i].use_property)) {
      ...
      g_object_get(output, pspec->name, &use_input, inputs[i].curve_property,
                   &input_curve, NULL);

      gtk_list_store_set(private->input_list, &private->input_iters[i],
                         INPUT_COLUMN_USE_INPUT, use_input, -1);

      if (!gtk_tree_selection_iter_is_selected(sel, &private->input_iters[i])) {
        if (use_input) {
          gimp_curve_view_add_background(GIMP_CURVE_VIEW(private->curve_view),
                                         input_curve, INPUT_COLOR(i));
        } else {
          gimp_curve_view_remove_background(
              GIMP_CURVE_VIEW(private->curve_view), input_curve);
        }
...
GtkWidget *gimp_dynamics_output_editor_new(GimpDynamicsOutput *output) {
  ...
  return g_object_new(GIMP_TYPE_DYNAMICS_OUTPUT_EDITOR, "output", output, NULL);
}
