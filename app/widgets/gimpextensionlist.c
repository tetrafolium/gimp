...
enum { EXTENSION_ACTIVATED, LAST_SIGNAL };
...
static void gimp_extension_list_delete_clicked(GtkButton *delbutton,
                                               GimpExtensionList *list);
static void gimp_extension_switch_active(GObject *onoff, GParamSpec *spec,
                                         gpointer extension);
...
static void gimp_extension_list_class_init(GimpExtensionListClass *klass) {
  signals[EXTENSION_ACTIVATED] = g_signal_new(
      "extension-activated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpExtensionListClass, extension_activated), NULL, NULL,
      NULL, G_TYPE_NONE, 1, GIMP_TYPE_OBJECT);
}

static void gimp_extension_list_init(GimpExtensionList *list) {
...
static void gimp_extension_list_disable(GtkWidget *row, gchar *extension_id) {
  GimpExtension *extension;
...
GtkWidget *gimp_extension_list_new(GimpExtensionManager *manager) {
  ...
  g_signal_connect(manager, "extension-installed",
                   G_CALLBACK(gimp_extension_list_ext_installed), list);
  g_signal_connect(manager, "extension-removed",
                   G_CALLBACK(gimp_extension_list_ext_removed), list);
...
void gimp_extension_list_show_system(GimpExtensionList *list) {
  ...
  gimp_extension_list_set(
      list, gimp_extension_manager_get_system_extensions(list->p->manager),
      TRUE);
}
...
void gimp_extension_list_show_user(GimpExtensionList *list) {
  ...
  gimp_extension_list_set(
      list, gimp_extension_manager_get_user_extensions(list->p->manager),
      FALSE);
}
...
static void gimp_extension_list_set(GimpExtensionList *list,
  ...
  gtk_container_foreach(GTK_CONTAINER(list), (GtkCallback)gtk_widget_destroy,
                        NULL);

  for (; iter; iter = iter->next) {
    GimpExtension *extension = iter->data;

    gimp_extension_list_ext_installed(list->p->manager, extension, is_system,
                                      list);
  }
...
static void gimp_extension_list_ext_installed(GimpExtensionManager *manager,
  ...
  gtk_switch_set_active(GTK_SWITCH(onoff), gimp_extension_manager_is_running(
                                               list->p->manager, extension));
  gtk_widget_set_sensitive(
      onoff, gimp_extension_manager_can_run(list->p->manager, extension));
  ...
  if (gimp_extension_get_comment(extension)) {
    ...
    gtk_text_buffer_set_text(buffer, gimp_extension_get_comment(extension), -1);
    ...
    gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(desc), GTK_WRAP_WORD_CHAR);
  ...
  if (!is_system_ext) {
    ...
    g_signal_connect(delbutton, "clicked",
                     G_CALLBACK(gimp_extension_list_delete_clicked), list);
    gtk_button_set_relief(GTK_BUTTON(delbutton), GTK_RELIEF_NONE);
    image =
        gtk_image_new_from_icon_name(GIMP_ICON_EDIT_DELETE, GTK_ICON_SIZE_MENU);
    gtk_image_set_pixel_size(GTK_IMAGE(image), 12);
...
static void gimp_extension_list_ext_removed(GimpExtensionManager *manager,
                                            ...
                                            GimpExtensionList *list) {
  gtk_container_foreach(GTK_CONTAINER(list),
                        (GtkCallback)gimp_extension_list_disable, extension_id);
}
...
static void gimp_extension_switch_active(GObject *onoff, GParamSpec *pspec,
                                         gpointer extension) {
  ...
  if (gtk_switch_get_active(GTK_SWITCH(onoff))) {
    ...
    if (error) {
      g_signal_handlers_block_by_func(
          onoff, G_CALLBACK(gimp_extension_switch_active), extension);
      gtk_switch_set_active(GTK_SWITCH(onoff), FALSE);
      g_signal_handlers_unblock_by_func(
          onoff, G_CALLBACK(gimp_extension_switch_active), extension);
      g_printerr("Extension '%s' failed to run: %s\n",
                 gimp_object_get_name(ext), error->message);
      g_error_free(error);
    }
  } else
