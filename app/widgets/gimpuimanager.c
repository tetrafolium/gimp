...
enum { PROP_0, PROP_NAME, PROP_GIMP };

enum { UPDATE, SHOW_TOOLTIP, HIDE_TOOLTIP, LAST_SIGNAL };
...
static void gimp_ui_manager_set_property(GObject *object, guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_ui_manager_get_property(GObject *object, guint prop_id,
...
static GimpUIManagerUIEntry *gimp_ui_manager_entry_get(GimpUIManager *manager,
                                                       const gchar *ui_path);
static gboolean gimp_ui_manager_entry_load(GimpUIManager *manager,
                                           GimpUIManagerUIEntry *entry,
                                           GError **error);
static GimpUIManagerUIEntry *
gimp_ui_manager_entry_ensure(GimpUIManager *manager, const gchar *path);
static void gimp_ui_manager_menu_position(GtkMenu *menu, gint *x, gint *y,
                                          gpointer data);
static void gimp_ui_manager_menu_pos(GtkMenu *menu, gint *x, gint *y,
...
static gboolean gimp_ui_manager_item_key_press(GtkWidget *widget,
                                               GdkEventKey *kevent,
                                               GimpUIManager *manager);
static GtkWidget *find_widget_under_pointer(GdkWindow *window, gint *x,
                                            gint *y);
...
static void gimp_ui_manager_class_init(GimpUIManagerClass *klass) {
  ...
  manager_signals[UPDATE] =
      g_signal_new("update", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpUIManagerClass, update), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, G_TYPE_POINTER);

  manager_signals[SHOW_TOOLTIP] =
      g_signal_new("show-tooltip", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpUIManagerClass, show_tooltip), NULL,
                   NULL, NULL, G_TYPE_NONE, 1, G_TYPE_STRING);

  manager_signals[HIDE_TOOLTIP] =
      g_signal_new("hide-tooltip", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpUIManagerClass, hide_tooltip), NULL,
...
static void gimp_ui_manager_constructed(GObject *object) {
  ...
  if (manager->name) {
    ...
    g_hash_table_replace(manager_class->managers, g_strdup(manager->name),
                         list);
  }
}
...
static void gimp_ui_manager_connect_proxy(GtkUIManager *manager,
  ...
  if (GTK_IS_MENU_ITEM(proxy)) {
    g_signal_connect(proxy, "select",
                     G_CALLBACK(gimp_ui_manager_menu_item_select), manager);
    g_signal_connect(proxy, "deselect",
                     G_CALLBACK(gimp_ui_manager_menu_item_deselect), manager);

    g_signal_connect_after(proxy, "realize",
                           G_CALLBACK(gimp_ui_manager_item_realize), manager);
  }
}
...
static void gimp_ui_manager_real_update(GimpUIManager *manager,
  ...
  for (list = gimp_ui_manager_get_action_groups(manager); list;
...
GimpUIManager *gimp_ui_manager_new(Gimp *gimp, const gchar *name) {
  ...
  manager =
      g_object_new(GIMP_TYPE_UI_MANAGER, "name", name, "gimp", gimp, NULL);
...
void gimp_ui_manager_add_ui(GimpUIManager *manager, guint merge_id,
                            const gchar *path, const gchar *name,
                            const gchar *action, GtkUIManagerItemType type,
                            gboolean top) {
  gtk_ui_manager_add_ui((GtkUIManager *)manager, merge_id, path, name, action,
                        type, top);
}

void gimp_ui_manager_remove_ui(GimpUIManager *manager, guint merge_id) {
...
GimpAction *gimp_ui_manager_get_action(GimpUIManager *manager,
                                       const gchar *path) {
  return (GimpAction *)gtk_ui_manager_get_action((GtkUIManager *)manager, path);
}
...
void gimp_ui_manager_ui_register(GimpUIManager *manager, const gchar *ui_path,
...
static void menu_pos_free(MenuPos *pos) { g_slice_free(MenuPos, pos); }

void gimp_ui_manager_ui_popup(GimpUIManager *manager, const gchar *ui_path,
                              GtkWidget *parent,
  ...
  if (popdown_func && popdown_data) {
    g_object_set_data_full(G_OBJECT(manager), "popdown-data", popdown_data,
                           popdown_func);
    g_signal_connect(menu, "selection-done",
                     G_CALLBACK(gimp_ui_manager_delete_popdown_data), manager);
  }

  gtk_menu_popup(GTK_MENU(menu), NULL, NULL, gimp_ui_manager_menu_pos, menu_pos,
                 button, activate_time);
}

void gimp_ui_manager_ui_popup_at_widget(GimpUIManager *manager,
                                        const gchar *ui_path, GtkWidget *widget,
  ...
  gtk_menu_popup_at_widget(GTK_MENU(menu), widget, widget_anchor, menu_anchor,
                           trigger_event);
}

void gimp_ui_manager_ui_popup_at_pointer(GimpUIManager *manager,
                                         const gchar *ui_path,
                                         const GdkEvent *trigger_event,
                                         GDestroyNotify popdown_func,
                                         gpointer popdown_data) {
...
static gboolean gimp_ui_manager_entry_load(GimpUIManager *manager,
  ...
  if (menus_path_override) {
    ...
    for (list = path; list; list = g_list_next(list)) {
      ...
      if (!list->next || g_file_test(filename, G_FILE_TEST_EXISTS))
        break;
    ...
    filename =
        g_build_filename(gimp_data_directory(), "menus", entry->basename, NULL);
  }
  ...
  entry->merge_id =
      gtk_ui_manager_add_ui_from_file(GTK_UI_MANAGER(manager), filename, error);
...
gimp_ui_manager_entry_ensure(GimpUIManager *manager, const gchar *path) {
    ...
    if (!gimp_ui_manager_entry_load(manager, entry, &error)) {
      if (error->domain == G_FILE_ERROR && error->code == G_FILE_ERROR_EXIST) {
        gimp_message(manager->gimp, NULL, GIMP_MESSAGE_ERROR, "%s\n\n%s\n\n%s",
                     _("Your GIMP installation is incomplete:"), error->message,
        ...
        gimp_message(manager->gimp, NULL, GIMP_MESSAGE_ERROR,
                     _("There was an error parsing the menu definition "
                       "from %s: %s"),
                     gimp_filename_to_utf8(entry->basename), error->message);
      }
  ...
  if (!entry->widget) {
    ...
    entry->widget = GTK_UI_MANAGER_CLASS(parent_class)
                        ->get_widget(gtk_manager, entry->ui_path);
...
static void gimp_ui_manager_delete_popdown_data(GtkWidget *widget,
  ...
  g_signal_handlers_disconnect_by_func(
      widget, gimp_ui_manager_delete_popdown_data, manager);
...
static void gimp_ui_manager_item_realize(GtkWidget *widget,
  ...
  g_signal_handlers_disconnect_by_func(widget, gimp_ui_manager_item_realize,
                                       manager);
  ...
  if (GTK_IS_MENU_SHELL(menu)) {
    ...
    if (!GPOINTER_TO_INT(
            g_object_get_qdata(G_OBJECT(menu), quark_key_press_connected))) {
      g_signal_connect(menu, "key-press-event",
                       G_CALLBACK(gimp_ui_manager_item_key_press), manager);

      g_object_set_qdata(G_OBJECT(menu), quark_key_press_connected,
  ...
  if (submenu)
    g_object_set_qdata(G_OBJECT(submenu), GIMP_HELP_ID,
                       g_object_get_qdata(G_OBJECT(widget), GIMP_HELP_ID));
}
...
static gboolean gimp_ui_manager_item_key_press(GtkWidget *widget,
    ...
    if (!menu_item && GTK_IS_MENU(widget)) {
      ...
      if (window) {
        ...
        if (menu_item && !GTK_IS_MENU_ITEM(menu_item)) {
          menu_item = gtk_widget_get_ancestor(menu_item, GTK_TYPE_MENU_ITEM);
  ...
  if (kevent->keyval != GDK_KEY_F1) {
    if (help_id && gtk_accelerator_valid(kevent->keyval, 0) &&
...
/* FIXME: remove this crack as soon as a GTK+ widget_under_pointer() is
 * available */
...
static void child_location_foreach(GtkWidget *child, gpointer data) {
  gint x, y;
  ...
  if (!child_loc->child &&
    ...
    g_print("candidate: %s  alloc=[(%d,%d)  %dx%d]     (%d, %d)->(%d, %d)\n",
            gtk_widget_get_name(child), child_allocation.x, child_allocation.y,
            child_allocation.width, child_allocation.height, child_loc->x,
            child_loc->y, x, y);
#endif /* DEBUG_TOOLTIP */

    /* (x, y) relative to child's allocation. */
    if (x >= 0 && x < child_allocation.width && y >= 0 &&
      ...
      if (GTK_IS_CONTAINER(child)) {
        ...
        gtk_container_forall(GTK_CONTAINER(child), child_location_foreach,
                             &tmp);
 ...
 */
static void window_to_alloc(GtkWidget *dest_widget, gint src_x, gint src_y,
...
static GtkWidget *find_widget_under_pointer(GdkWindow *window, gint *x,
  ...
  g_print("event window %p (belonging to %p (%s))  (%d, %d)\n", window,
          event_widget, gtk_widget_get_name(event_widget), *x, *y);
#endif
   ...
   */
  window_to_alloc(event_widget, child_loc.x, child_loc.y, &child_loc.x,
                  &child_loc.y);
  ...
  if (GTK_IS_CONTAINER(event_widget)) {
    ...
    gtk_container_forall(GTK_CONTAINER(event_widget), child_location_foreach,
                         &child_loc);
    ...
    gtk_widget_translate_coordinates(container, event_widget, child_loc.x,
                                     child_loc.y, &child_loc.x, &child_loc.y);
  }
