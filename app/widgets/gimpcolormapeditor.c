...
G_DEFINE_TYPE_WITH_CODE(
    GimpColormapEditor, gimp_colormap_editor, GIMP_TYPE_IMAGE_EDITOR,
...
static void gimp_colormap_editor_init(GimpColormapEditor *editor) {}
...
static void gimp_colormap_editor_constructed(GObject *object) {
  ...
  gimp_editor_add_action_button(
      GIMP_EDITOR(editor), "colormap", "colormap-add-color-from-fg",
      "colormap-add-color-from-bg", gimp_get_toggle_behavior_mask(), NULL);

  gimp_editor_add_action_button(
      GIMP_EDITOR(editor), "colormap", "colormap-selection-replace",
      "colormap-selection-add", extend_mask, "colormap-selection-subtract",
      modify_mask, "colormap-selection-intersect", extend_mask | modify_mask,
      NULL);
}
...
GtkWidget *gimp_colormap_editor_new(GimpMenuFactory *menu_factory) {
  ...
  return g_object_new(GIMP_TYPE_COLORMAP_EDITOR, "menu-factory", menu_factory,
                      "menu-identifier", "<Colormap>", "ui-path",
                      "/colormap-popup", NULL);
}
...
void gimp_colormap_editor_edit_color(GimpColormapEditor *editor) {
  ...
  index = gimp_colormap_selection_get_index(
      GIMP_COLORMAP_SELECTION(editor->selection), NULL);
  ...
  gimp_rgba_set_uchar(&color, colormap[index * 3], colormap[index * 3 + 1],
                      colormap[index * 3 + 2], 255);

  desc = g_strdup_printf(_("Edit colormap entry #%d"), index);

  if (!editor->color_dialog) {
    editor->color_dialog = gimp_color_dialog_new(
        GIMP_VIEWABLE(image), GIMP_IMAGE_EDITOR(editor)->context, FALSE,
        _("Edit Colormap Entry"), GIMP_ICON_COLORMAP, desc, GTK_WIDGET(editor),
        gimp_dialog_factory_get_singleton(),
        "gimp-colormap-editor-color-dialog", (const GimpRGB *)&color, TRUE,
        FALSE);
    ...
    g_signal_connect(editor->color_dialog, "update",
                     G_CALLBACK(gimp_colormap_editor_color_update), editor);
  } else {
    gimp_viewable_dialog_set_viewables(
        GIMP_VIEWABLE_DIALOG(editor->color_dialog), g_list_prepend(NULL, image),
    ...
    if (!gtk_widget_get_visible(editor->color_dialog))
      gimp_dialog_factory_position_dialog(
          gimp_dialog_factory_get_singleton(),
          "gimp-colormap-editor-color-dialog", editor->color_dialog,
...
gint gimp_colormap_editor_get_index(GimpColormapEditor *editor,
  ...
  return gimp_colormap_selection_get_index(
      GIMP_COLORMAP_SELECTION(editor->selection), search);
}

gboolean gimp_colormap_editor_set_index(GimpColormapEditor *editor, gint index,
                                        GimpRGB *color) {
  g_return_val_if_fail(GIMP_IS_COLORMAP_EDITOR(editor), FALSE);

  return gimp_colormap_selection_set_index(
      GIMP_COLORMAP_SELECTION(editor->selection), index, color);
}

gint gimp_colormap_editor_max_index(GimpColormapEditor *editor) {
  g_return_val_if_fail(GIMP_IS_COLORMAP_EDITOR(editor), -1);

  return gimp_colormap_selection_max_index(
      GIMP_COLORMAP_SELECTION(editor->selection));
}
...
static void gimp_colormap_editor_color_update(GimpColorDialog *dialog,
  ...
  if (image) {
    ...
    col_index = gimp_colormap_selection_get_index(
        GIMP_COLORMAP_SELECTION(editor->selection), NULL);
    if (push_undo) {
      GimpRGB old_color;
