...
enum { PROP_0, PROP_ENABLED, PROP_DEBUG_EVENTS, PROP_CONTROLLER, PROP_MAPPING };

enum { EVENT_MAPPED, LAST_SIGNAL };
...
static gboolean
gimp_controller_info_serialize_property(GimpConfig *config, guint property_id,
                                        const GValue *value, GParamSpec *pspec,
                                        GimpConfigWriter *writer);
static gboolean gimp_controller_info_deserialize_property(
    GimpConfig *config, guint property_id, GValue *value, GParamSpec *pspec,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpControllerInfo, gimp_controller_info, GIMP_TYPE_VIEWABLE,
...
static void gimp_controller_info_class_init(GimpControllerInfoClass *klass) {
  ...
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_ENABLED, "enabled", _("Enabled"),
                           NULL, TRUE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_DEBUG_EVENTS, "debug-events",
                           _("Debug events"), NULL, FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_CONTROLLER, "controller",
                          "Controller", NULL, GIMP_TYPE_CONTROLLER,
                          GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOXED(object_class, PROP_MAPPING, "mapping", "Mapping", NULL,
                         G_TYPE_HASH_TABLE, GIMP_PARAM_STATIC_STRINGS);

  info_signals[EVENT_MAPPED] =
      g_signal_new("event-mapped", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpControllerInfoClass, event_mapped),
                   g_signal_accumulator_true_handled, NULL,
                   gimp_marshal_BOOLEAN__OBJECT_POINTER_STRING, G_TYPE_BOOLEAN,
                   3, G_TYPE_OBJECT, G_TYPE_POINTER, G_TYPE_STRING);
}

static void gimp_controller_info_init(GimpControllerInfo *info) {
  info->controller = NULL;
  info->mapping = g_hash_table_new_full(
  ...
  case PROP_CONTROLLER:
    if (info->controller) {
      g_signal_handlers_disconnect_by_func(info->controller,
                                           gimp_controller_info_event, info);
...
static void gimp_controller_info_serialize_mapping(gpointer key, gpointer value,
                                                   gpointer data) {
...
gimp_controller_info_serialize_property(GimpConfig *config, guint property_id,
  ...
  if (mapping) {
    ...
    g_hash_table_foreach(
        mapping, (GHFunc)gimp_controller_info_serialize_mapping, writer);
...
GimpControllerInfo *gimp_controller_info_new(GType type) {
  ...
  info = g_object_new(GIMP_TYPE_CONTROLLER_INFO, "name", controller_class->name,
...
static gboolean gimp_controller_info_event(GimpController *controller,
  ...
  event_blurb =
      gimp_controller_get_event_blurb(controller, event->any.event_id);
  ...
  if (action_name) {
    ...
    g_signal_emit(info, info_signals[EVENT_MAPPED], 0, controller, event,
                  action_name, &retval);
