...
enum { SRC_COLUMN_NAME, SRC_COLUMN_ICON, SRC_COLUMN_TYPE, N_SRC_COLUMNS };
...
static void gimp_color_display_editor_init(GimpColorDisplayEditor *editor) {
  ...
  editor->src = gtk_list_store_new(N_SRC_COLUMNS, G_TYPE_STRING, G_TYPE_STRING,
                                   G_TYPE_GTYPE);
  ...
  gtk_tree_view_column_set_attributes(column, rend, "icon-name",
                                      SRC_COLUMN_ICON, NULL);

  rend = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(column, rend, TRUE);
  gtk_tree_view_column_set_attributes(column, rend, "text", SRC_COLUMN_NAME,
                                      NULL);
  ...
  g_signal_connect(editor->src_sel, "changed",
                   G_CALLBACK(gimp_color_display_editor_src_changed), editor);
  ...
  image = gtk_image_new_from_icon_name(GIMP_ICON_GO_NEXT, GTK_ICON_SIZE_BUTTON);
  gtk_container_add(GTK_CONTAINER(editor->add_button), image);
  gtk_widget_show(image);

  g_signal_connect(editor->add_button, "clicked",
                   G_CALLBACK(gimp_color_display_editor_add_clicked), editor);
  ...
  image =
      gtk_image_new_from_icon_name(GIMP_ICON_GO_PREVIOUS, GTK_ICON_SIZE_BUTTON);
  ...
  editor->up_button = gimp_editor_add_button(
      GIMP_EDITOR(ed), GIMP_ICON_GO_UP, _("Move the selected filter up"), NULL,
      G_CALLBACK(gimp_color_display_editor_up_clicked), NULL, editor);

  editor->down_button = gimp_editor_add_button(
      GIMP_EDITOR(ed), GIMP_ICON_GO_DOWN, _("Move the selected filter down"),
      NULL, G_CALLBACK(gimp_color_display_editor_down_clicked), NULL, editor);
  ...
  editor->dest =
      gtk_list_store_new(N_DEST_COLUMNS, G_TYPE_BOOLEAN, G_TYPE_STRING,
  ...
  column = gtk_tree_view_column_new_with_attributes(NULL, rend, "active",
                                                    DEST_COLUMN_ENABLED, NULL);
  gtk_tree_view_insert_column(GTK_TREE_VIEW(tv), column, 0);

  image = gtk_image_new_from_icon_name(GIMP_ICON_VISIBLE, GTK_ICON_SIZE_MENU);
  ...
  gtk_tree_view_column_pack_start(column, rend, FALSE);
  gtk_tree_view_column_set_attributes(column, rend, "icon-name",
                                      DEST_COLUMN_ICON, NULL);

  rend = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(column, rend, TRUE);
  gtk_tree_view_column_set_attributes(column, rend, "text", DEST_COLUMN_NAME,
                                      NULL);
  ...
  g_signal_connect(editor->dest_sel, "changed",
                   G_CALLBACK(gimp_color_display_editor_dest_changed), editor);
  ...
  g_signal_connect(editor->reset_button, "clicked",
                   G_CALLBACK(gimp_color_display_editor_reset_clicked), editor);
...
GtkWidget *gimp_color_display_editor_new(Gimp *gimp,
  ...
  for (i = 0; i < n_display_types; i++) {
    ...
    gtk_list_store_set(editor->src, &iter, SRC_COLUMN_ICON,
                       display_class->icon_name, SRC_COLUMN_NAME,
                       display_class->name, SRC_COLUMN_TYPE, display_types[i],
                       -1);
  ...
  for (list = gimp_color_display_stack_get_filters(stack); list;
    ...
    gtk_list_store_set(editor->dest, &iter, DEST_COLUMN_ENABLED, enabled,
                       DEST_COLUMN_ICON, icon_name, DEST_COLUMN_NAME, name,
                       DEST_COLUMN_FILTER, display, -1);
...
gimp_color_display_editor_add_clicked(GtkWidget *widget,
  ...
  if (gtk_tree_selection_get_selected(editor->src_sel, &model, &iter)) {
    ...
    display = g_object_new(type, "color-config", editor->config,
                           "color-managed", editor->managed, NULL);
...
gimp_color_display_editor_dest_changed(GtkTreeSelection *sel,
  ...
  if (display) {
    ...
    g_object_add_weak_pointer(G_OBJECT(display), (gpointer)&editor->selected);

    editor->config_widget = gimp_color_display_configure(display);

    if (!editor->config_widget) {
      editor->config_widget = gimp_prop_gui_new(
          G_OBJECT(display), G_TYPE_FROM_INSTANCE(display), 0, NULL,
          gimp_get_user_context(editor->gimp), NULL, NULL, NULL);
    }
...
static void gimp_color_display_editor_removed(GimpColorDisplayStack *stack,
  ...
  for (iter_valid =
           gtk_tree_model_get_iter_first(GTK_TREE_MODEL(editor->dest), &iter);
       iter_valid; iter_valid = gtk_tree_model_iter_next(
                       GTK_TREE_MODEL(editor->dest), &iter)) {
    GimpColorDisplay *display2;

    gtk_tree_model_get(GTK_TREE_MODEL(editor->dest), &iter, DEST_COLUMN_FILTER,
                       &display2, -1);
    ...
    if (display == display2) {
      g_signal_handlers_disconnect_by_func(
          display, gimp_color_display_editor_enabled, editor);
...
gimp_color_display_editor_reordered(GimpColorDisplayStack *stack,
                                    GimpColorDisplay *display, gint position,
                                    GimpColorDisplayEditor *editor) {
  GtkTreeIter iter;
  gboolean iter_valid;
  ...
  for (iter_valid =
    ...
    if (display == display2) {
      ...
      if (position == -1 || position == g_list_length(filters) - 1) {
        ...
        gtk_tree_model_get_iter(GTK_TREE_MODEL(editor->dest), &place_iter,
                                path);
        gtk_tree_path_free(path);
...
static void gimp_color_display_editor_enabled(GimpColorDisplay *display,
  ...
  for (iter_valid =
    ...
    if (display == display2) {
      ...
      gtk_list_store_set(editor->dest, &iter, DEST_COLUMN_ENABLED, enabled, -1);
...
gimp_color_display_editor_enable_toggled(GtkCellRendererToggle *toggle,
  ...
  if (gtk_tree_model_get_iter(GTK_TREE_MODEL(editor->dest), &iter, path)) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(editor->dest), &iter, DEST_COLUMN_FILTER,
                       &display, DEST_COLUMN_ENABLED, &enabled, -1);
