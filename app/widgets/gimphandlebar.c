...
static void gimp_handle_bar_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_handle_bar_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);

static gboolean gimp_handle_bar_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_handle_bar_class_init(GimpHandleBarClass *klass) {
...
static void gimp_handle_bar_init(GimpHandleBar *bar) {
  gtk_widget_add_events(GTK_WIDGET(bar), GDK_BUTTON_PRESS_MASK |
...
static gboolean gimp_handle_bar_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (bar->slider_adj[i]) {
      bar->slider_pos[i] =
          ROUND((gdouble)(width - 1) *
                (gtk_adjustment_get_value(bar->slider_adj[i]) - bar->lower) /
                (bar->upper - bar->lower));

      cairo_set_source_rgb(cr, 0.5 * i, 0.5 * i, 0.5 * i);

      cairo_move_to(cr, x + bar->slider_pos[i], y);
      cairo_line_to(cr, x + bar->slider_pos[i] - (height - 1) / 2,
                    y + height - 1);
      cairo_line_to(cr, x + bar->slider_pos[i] + (height - 1) / 2,
                    y + height - 1);
      cairo_line_to(cr, x + bar->slider_pos[i], y);
...
static gboolean gimp_handle_bar_button_press(GtkWidget *widget,
  ...
  for (i = 0; i < 3; i++)
    if (bar->slider_pos[i] != -1) {
      ...
      if (fabs(dist) < min_dist || (fabs(dist) == min_dist && dist > 0)) {
        bar->active_slider = i;
        min_dist = fabs(dist);
      }
    }
  ...
  if (width == 1)
    ...
    value = ((gdouble)(bevent->x - border) / (gdouble)(width - 1) *
             (bar->upper - bar->lower));
...
static gboolean gimp_handle_bar_motion_notify(GtkWidget *widget,
  ...
  if (width == 1)
    ...
    value = ((gdouble)(mevent->x - border) / (gdouble)(width - 1) *
             (bar->upper - bar->lower));
...
GtkWidget *gimp_handle_bar_new(GtkOrientation orientation) {
  return g_object_new(GIMP_TYPE_HANDLE_BAR, "orientation", orientation, NULL);
}

void gimp_handle_bar_set_adjustment(GimpHandleBar *bar, gint handle_no,
  ...
  if (bar->slider_adj[handle_no]) {
    g_signal_handlers_disconnect_by_func(
        bar->slider_adj[handle_no], gimp_handle_bar_adjustment_changed, bar);
  ...
  if (bar->slider_adj[handle_no]) {
    ...
    g_signal_connect(bar->slider_adj[handle_no], "value-changed",
                     G_CALLBACK(gimp_handle_bar_adjustment_changed), bar);
...
void gimp_handle_bar_set_limits(GimpHandleBar *bar, gdouble lower,
                                gdouble upper) {
  g_return_if_fail(GIMP_IS_HANDLE_BAR(bar));
...
gboolean gimp_handle_bar_get_limits(GimpHandleBar *bar, gdouble *lower,
                                    gdouble *upper) {
...
void gimp_handle_bar_connect_events(GimpHandleBar *bar,
  ...
  g_signal_connect_object(event_source, "button-press-event",
                          G_CALLBACK(widget_class->button_press_event), bar,
                          G_CONNECT_SWAPPED);

  g_signal_connect_object(event_source, "button-release-event",
                          G_CALLBACK(widget_class->button_release_event), bar,
                          G_CONNECT_SWAPPED);

  g_signal_connect_object(event_source, "motion-notify-event",
                          G_CALLBACK(widget_class->motion_notify_event), bar,
                          G_CONNECT_SWAPPED);
}
