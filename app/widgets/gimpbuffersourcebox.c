...
static void gimp_buffer_source_box_class_init(GimpBufferSourceBoxClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_SOURCE_NODE,
      g_param_spec_object("source-node", NULL, NULL, GEGL_TYPE_NODE,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("enabled", NULL, NULL, TRUE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_buffer_source_box_init(GimpBufferSourceBox *box) {
...
static void gimp_buffer_source_box_constructed(GObject *object) {
  ...
  box->priv->button =
      gimp_pickable_button_new(box->priv->context, GIMP_VIEW_SIZE_LARGE, 1);
...
static void gimp_buffer_source_box_update_node(GimpBufferSourceBox *box) {
  ...
  if (box->priv->pickable) {
    ...
    if (box->priv->enabled) {
       ...
       */
      buffer =
          gimp_gegl_buffer_dup(gimp_pickable_get_buffer(box->priv->pickable));
    }

    desc =
        gimp_viewable_get_description(GIMP_VIEWABLE(box->priv->pickable), NULL);
  ...
  gegl_node_set(box->priv->source_node, "buffer", buffer, NULL);
...
GtkWidget *gimp_buffer_source_box_new(GimpContext *context,
  ...
  return g_object_new(GIMP_TYPE_BUFFER_SOURCE_BOX, "context", context,
                      "source-node", source_node, "name", name, NULL);
}
