...
enum { PROP_0, PROP_GIMP, PROP_ICON_NAME, PROP_ICON_PIXBUF };
...
#define GET_PRIVATE(picker)                                                    \
  ((GimpIconPickerPrivate *)gimp_icon_picker_get_instance_private(             \
...
static void gimp_icon_picker_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_icon_picker_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);

static void gimp_icon_picker_icon_changed(GimpContext *context,
                                          GimpTemplate *template,
                                          GimpIconPicker *picker);
static void gimp_icon_picker_clicked(GtkWidget *widget, GdkEventButton *event,
                                     gpointer data);
...
static void gimp_icon_picker_menu_copy(GtkWidget *widget, GdkEventButton *event,
                                       gpointer data);
...
static void gimp_icon_picker_class_init(GimpIconPickerClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("icon-name", NULL, NULL, "gimp-toilet-paper",
...
static void gimp_icon_picker_init(GimpIconPicker *picker) {
...
private
  ->preview = g_object_new(GIMP_TYPE_VIEWABLE, "icon-name", private->icon_name,
                           "icon-pixbuf", private->icon_pixbuf, NULL);
}
...
private
  ->icon_name_container = gimp_list_new(GIMP_TYPE_TEMPLATE, FALSE);
private
  ->icon_name_context = gimp_context_new(private->gimp, "foo", NULL);

  g_signal_connect(private->icon_name_context, "template-changed",
                   G_CALLBACK(gimp_icon_picker_icon_changed), picker);
  ...
  for (list = icon_list; list; list = g_list_next(list)) {
    GimpObject *object = g_object_new(GIMP_TYPE_TEMPLATE, "name", list->data,
                                      "icon-name", list->data, NULL);
   ...
   */
private
  ->null_template_object =
      g_object_new(GIMP_TYPE_TEMPLATE, "name", "", "icon-name", "", NULL);
  ...
  viewable_view = gimp_view_new(private->icon_name_context, private->preview,
                                GIMP_VIEW_SIZE_SMALL, 0, FALSE);
...
private
  ->menu_item_file_icon = gtk_menu_item_new_with_label(_("From File..."));
...
private
  ...
  g_signal_connect(private->menu_item_name_icon, "button-press-event",
                   G_CALLBACK(gimp_icon_picker_menu_from_name), object);

private
  ->menu_item_copy = gtk_menu_item_new_with_label(_("Copy Icon to Clipboard"));
  gtk_menu_shell_append(GTK_MENU_SHELL(private->right_click_menu),
...
GtkWidget *gimp_icon_picker_new(Gimp *gimp) {
  ...
  return g_object_new(GIMP_TYPE_ICON_PICKER, "gimp", gimp, NULL);
}
...
private
  ...
  g_object_set(private->preview, "icon-name", private->icon_name, NULL);
...
static void gimp_icon_picker_menu_from_file(GtkWidget *widget,
  ...
  dialog = gtk_file_chooser_dialog_new(
      _("Load Icon Image"), NULL, GTK_FILE_CHOOSER_ACTION_OPEN,
...
static void gimp_icon_picker_clicked(GtkWidget *widget, GdkEventButton *event,
  ...
  gtk_menu_popup_at_widget(GTK_MENU(private->right_click_menu), widget,
                           GDK_GRAVITY_EAST, GDK_GRAVITY_NORTH_WEST,
                           (GdkEvent *)event);
}
...
static void gimp_icon_picker_menu_from_name(GtkWidget *widget,
  ...
  popup = gimp_container_popup_new(
      private->icon_name_container, private->icon_name_context,
      GIMP_VIEW_TYPE_LIST, GIMP_VIEW_SIZE_SMALL, GIMP_VIEW_SIZE_SMALL, 0, NULL,
      NULL, NULL, NULL);
