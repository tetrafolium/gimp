...
static void gimp_view_renderer_gradient_fg_bg_changed(
    GimpContext *context, const GimpRGB *color, GimpViewRenderer *renderer) {
#if 0
...
static void gimp_view_renderer_gradient_set_context(GimpViewRenderer *renderer,
  ...
  if (renderer->context && rendergrad->has_fg_bg_segments) {
    g_signal_handlers_disconnect_by_func(
        renderer->context, gimp_view_renderer_gradient_fg_bg_changed, renderer);
  }
  ...
  if (renderer->context && rendergrad->has_fg_bg_segments) {
    ...
    gimp_view_renderer_gradient_fg_bg_changed(renderer->context, NULL,
                                              renderer);
  }
}
  ...
  if (rendergrad->has_fg_bg_segments != has_fg_bg_segments) {
    if (renderer->context) {
      if (rendergrad->has_fg_bg_segments) {
        g_signal_handlers_disconnect_by_func(
            renderer->context, gimp_view_renderer_gradient_fg_bg_changed,
            renderer);
      } else {
        g_signal_connect(renderer->context, "foreground-changed",
                         G_CALLBACK(gimp_view_renderer_gradient_fg_bg_changed),
...
static void gimp_view_renderer_gradient_render(GimpViewRenderer *renderer,
  ...
  for (x = 0, dest = buf; x < renderer->width; x++, dest += 4) {
    ...
    seg = gimp_gradient_get_color_at(gradient, renderer->context, seg, cur_x,
                                     rendergrad->reverse,
                                     rendergrad->blend_color_space, &color);
    cur_x += dx;
  ...
  if (!renderer->surface)
    renderer->surface = cairo_image_surface_create(
        CAIRO_FORMAT_ARGB32, renderer->width, renderer->height);
  ...
  transform = gimp_view_renderer_get_color_transform(
      renderer, widget, babl_format("cairo-ARGB32"),
      babl_format("cairo-ARGB32"));

  if (transform)
    gimp_color_transform_process_pixels(transform, babl_format("cairo-ARGB32"),
                                        buf, babl_format("cairo-ARGB32"), buf,
                                        renderer->width);
