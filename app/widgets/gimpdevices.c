...
void gimp_devices_init(Gimp *gimp) {
  ...
  g_object_set_data_full(G_OBJECT(gimp), GIMP_DEVICE_MANAGER_DATA_KEY, manager,
...
void gimp_devices_restore(Gimp *gimp) {
  ...
  for (list = GIMP_LIST(manager)->queue->head; list; list = g_list_next(list)) {
  ...
  if (!gimp_config_deserialize_file(GIMP_CONFIG(manager), file, gimp, &error)) {
  ...
  for (list = GIMP_LIST(manager)->queue->head; list; list = g_list_next(list)) {
    ...
    if (!GIMP_TOOL_PRESET(device_info)->tool_options) {
      ...
      g_printerr("%s: set default tool on loaded GimpDeviceInfo without tool "
                 "options: %s\n",
...
void gimp_devices_save(Gimp *gimp, gboolean always_save) {
  GimpDeviceManager *manager;
  GFile *file;
  GError *error = NULL;
  ...
  if (!gimp_config_serialize_to_file(GIMP_CONFIG(manager), file,
                                     "GIMP devicerc", "end of devicerc", NULL,
                                     &error)) {
    gimp_message_literal(gimp, NULL, GIMP_MESSAGE_ERROR, error->message);
    g_error_free(error);
  }
...
gboolean gimp_devices_clear(Gimp *gimp, GError **error) {
  GimpDeviceManager *manager;
  GFile *file;
...
GdkDevice *gimp_devices_get_from_event(Gimp *gimp, const GdkEvent *event,
...
void gimp_devices_add_widget(Gimp *gimp, GtkWidget *widget) {
  g_return_if_fail(GIMP_IS_GIMP(gimp));
  g_return_if_fail(GTK_IS_WIDGET(widget));

  g_signal_connect(widget, "motion-notify-event",
                   G_CALLBACK(gimp_devices_check_callback), gimp);
}

gboolean gimp_devices_check_callback(GtkWidget *widget, GdkEvent *event,
                                     Gimp *gimp) {
...
gboolean gimp_devices_check_change(Gimp *gimp, GdkDevice *device) {
  GimpDeviceManager *manager;
