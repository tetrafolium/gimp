...
gimp_text_style_editor_create_toggle(GimpTextStyleEditor *editor,
                                     GtkTextTag *tag, const gchar *icon_name,
                                     const gchar *tooltip);
...
G_DEFINE_TYPE(GimpTextStyleEditor, gimp_text_style_editor, GTK_TYPE_BOX)

#define parent_class gimp_text_style_editor_parent_class

static void gimp_text_style_editor_class_init(GimpTextStyleEditorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("text", NULL, NULL, GIMP_TYPE_TEXT,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_BUFFER,
      g_param_spec_object("buffer", NULL, NULL, GIMP_TYPE_TEXT_BUFFER,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_FONTS,
      g_param_spec_object("fonts", NULL, NULL, GIMP_TYPE_CONTAINER,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_RESOLUTION_X,
      g_param_spec_double("resolution-x", NULL, NULL, GIMP_MIN_RESOLUTION,
                          GIMP_MAX_RESOLUTION, 1.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_RESOLUTION_Y,
      g_param_spec_double("resolution-y", NULL, NULL, GIMP_MIN_RESOLUTION,
                          GIMP_MAX_RESOLUTION, 1.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
...
static void gimp_text_style_editor_init(GimpTextStyleEditor *editor) {
  GtkWidget *image;
  GimpRGB color;
  ...
  editor->font_entry =
      gimp_container_entry_new(NULL, NULL, GIMP_VIEW_SIZE_SMALL, 1);
  gtk_box_pack_start(GTK_BOX(editor->upper_hbox), editor->font_entry, FALSE,
                     FALSE, 0);
  gtk_widget_show(editor->font_entry);

  gimp_help_set_help_data(editor->font_entry, _("Change font of selected text"),
                          NULL);

  editor->size_entry = gimp_size_entry_new(1, 0, "%a", TRUE, FALSE, FALSE, 10,
                                           GIMP_SIZE_ENTRY_UPDATE_SIZE);
  gtk_box_pack_start(GTK_BOX(editor->upper_hbox), editor->size_entry, FALSE,
                     FALSE, 0);
  gtk_widget_show(editor->size_entry);

  gimp_help_set_help_data(editor->size_entry, _("Change size of selected text"),
                          NULL);

  g_signal_connect(editor->size_entry, "value-changed",
                   G_CALLBACK(gimp_text_style_editor_size_changed), editor);
  ...
  gtk_box_pack_start(GTK_BOX(editor->lower_hbox), editor->clear_button, FALSE,
                     FALSE, 0);
  ...
  g_signal_connect(editor->clear_button, "clicked",
                   G_CALLBACK(gimp_text_style_editor_clear_tags), editor);
  ...
  editor->color_button = gimp_color_panel_new(
      _("Change color of selected text"), &color, GIMP_COLOR_AREA_FLAT, 20, 20);
  gimp_widget_set_fully_opaque(editor->color_button, TRUE);

  gtk_box_pack_end(GTK_BOX(editor->lower_hbox), editor->color_button, FALSE,
                   FALSE, 0);
  ...
  g_signal_connect(editor->color_button, "color-changed",
                   G_CALLBACK(gimp_text_style_editor_color_changed), editor);

  editor->kerning_adjustment =
      gtk_adjustment_new(0.0, -1000.0, 1000.0, 1.0, 10.0, 0.0);
  ...
  g_signal_connect(editor->kerning_adjustment, "value-changed",
                   G_CALLBACK(gimp_text_style_editor_kerning_changed), editor);
  ...
  g_signal_connect(editor->baseline_adjustment, "value-changed",
                   G_CALLBACK(gimp_text_style_editor_baseline_changed), editor);
}
...
static void gimp_text_style_editor_constructed(GObject *object) {
  ...
  g_signal_connect(editor->context, "font-changed",
                   G_CALLBACK(gimp_text_style_editor_font_changed), editor);
  ...
  gimp_text_style_editor_create_toggle(editor, editor->buffer->underline_tag,
                                       ...
                                       _("Underline"));
  gimp_text_style_editor_create_toggle(
      editor, editor->buffer->strikethrough_tag,
      GIMP_ICON_FORMAT_TEXT_STRIKETHROUGH, _("Strikethrough"));

  g_signal_connect_swapped(editor->text, "notify::font",
                           G_CALLBACK(gimp_text_style_editor_update), editor);
  ...
  g_signal_connect_data(editor->buffer, "changed",
                        G_CALLBACK(gimp_text_style_editor_update), editor, 0,
...
GtkWidget *gimp_text_style_editor_new(Gimp *gimp, GimpText *text,
  ...
  return g_object_new(GIMP_TYPE_TEXT_STYLE_EDITOR, "gimp", gimp, "text", text,
                      "buffer", buffer, "fonts", fonts, "resolution-x",
  ...
  *remove_tags = NULL;
...

  {
    ...
    if (pixels != 0.0) {
      ...
      points =
          gimp_units_to_points(pixels, GIMP_UNIT_PIXEL, editor->resolution_y);
      tag = gimp_text_buffer_get_size_tag(editor->buffer, PANGO_SCALE * points);
      tags = g_list_prepend(tags, tag);
    }
  }
...
gimp_text_style_editor_create_toggle(GimpTextStyleEditor *editor,
  ...
  g_signal_connect(toggle, "toggled",
                   G_CALLBACK(gimp_text_style_editor_tag_toggled), editor);
...
static void gimp_text_style_editor_set_font(GimpTextStyleEditor *editor,
  ...
  g_signal_handlers_block_by_func(editor->context,
                                  gimp_text_style_editor_font_changed, editor);

  gimp_context_set_font_name(editor->context, font);

  g_signal_handlers_unblock_by_func(
      editor->context, gimp_text_style_editor_font_changed, editor);
...
static void gimp_text_style_editor_set_color(GimpTextStyleEditor *editor,
  ...
  g_signal_handlers_block_by_func(editor->color_button,
                                  gimp_text_style_editor_color_changed, editor);
...
static void gimp_text_style_editor_set_toggle(GimpTextStyleEditor *editor,
                                              GtkToggleButton *toggle,
                                              gboolean active) {
  g_signal_handlers_block_by_func(toggle, gimp_text_style_editor_tag_toggled,
                                  editor);

  gtk_toggle_button_set_active(toggle, active);

  g_signal_handlers_unblock_by_func(toggle, gimp_text_style_editor_tag_toggled,
                                    editor);
}
...
static void gimp_text_style_editor_set_size(GimpTextStyleEditor *editor,
  ...
  g_signal_handlers_block_by_func(editor->size_entry,
                                  gimp_text_style_editor_size_changed, editor);

  pixels = gimp_units_to_pixels((gdouble)size / PANGO_SCALE, GIMP_UNIT_POINT,
                                editor->resolution_y);
  ...
  if (size == 0) {
    ...
    spinbutton =
        gimp_size_entry_get_help_widget(GIMP_SIZE_ENTRY(editor->size_entry), 0);

    gtk_entry_set_text(GTK_ENTRY(spinbutton), "");
  }

  g_signal_handlers_unblock_by_func(
      editor->size_entry, gimp_text_style_editor_size_changed, editor);
}

static void
gimp_text_style_editor_set_default_size(GimpTextStyleEditor *editor) {
  gdouble pixels = gimp_units_to_pixels(
      editor->text->font_size, editor->text->unit, editor->resolution_y);
...
static void gimp_text_style_editor_update(GimpTextStyleEditor *editor) {
  ...
  editor->update_idle_id = gdk_threads_add_idle(
      (GSourceFunc)gimp_text_style_editor_update_idle, editor);
}
...
gimp_text_style_editor_update_idle(GimpTextStyleEditor *editor) {
  ...
  if (gtk_text_buffer_get_has_selection(buffer)) {
    ...
    color_tag = gimp_text_buffer_get_iter_color(editor->buffer, &start, NULL);
    size_tag = gimp_text_buffer_get_iter_size(editor->buffer, &start, NULL);
    baseline_tag =
        gimp_text_buffer_get_iter_baseline(editor->buffer, &start, NULL);
    kerning_tag =
        gimp_text_buffer_get_iter_kerning(editor->buffer, &start, NULL);

    for (iter = start; gtk_text_iter_in_range(&iter, &start, &end);
      ...
      if (!font_differs) {
        ...
        tag = gimp_text_buffer_get_iter_font(editor->buffer, &iter, NULL);
      ...
      if (!color_differs) {
        ...
        tag = gimp_text_buffer_get_iter_color(editor->buffer, &iter, NULL);
      ...
      if (!size_differs) {
        ...
        tag = gimp_text_buffer_get_iter_size(editor->buffer, &iter, NULL);
      ...
      if (!baseline_differs) {
        ...
        tag = gimp_text_buffer_get_iter_baseline(editor->buffer, &iter, NULL);
      ...
      if (!kerning_differs) {
        ...
        tag = gimp_text_buffer_get_iter_kerning(editor->buffer, &iter, NULL);

        if (tag != kerning_tag)
          kerning_differs = TRUE;
      }

      if (!any_toggle_active && color_differs && font_differs && size_differs &&
          baseline_differs && kerning_differs)
        break;
    }
  ...
  } else /* no selection */
  {
    ...
    for (list = editor->buffer->font_tags; list; list = g_list_next(list)) {
      ...
      if ((g_slist_find(tags, tag) && !g_slist_find(tags_on, tag)) ||
    ...
    for (list = editor->toggles; list; list = g_list_next(list)) {
      ...
      gimp_text_style_editor_set_toggle(
          editor, toggle,
          (g_slist_find(tags, tag) && !g_slist_find(tags_on, tag)) ||
              g_slist_find(tags_off, tag));
    }
