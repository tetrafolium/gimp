...
static gboolean gimp_drawable_tree_view_drop_possible(
...
static void gimp_drawable_tree_view_drop_viewables(
    GimpContainerTreeView *view, GList *src_viewables,
    GimpViewable *dest_viewable, GtkTreeViewDropPosition drop_pos);
static void gimp_drawable_tree_view_drop_color(
    GimpContainerTreeView *view, const GimpRGB *color,
    GimpViewable *dest_viewable, GtkTreeViewDropPosition drop_pos);
...
static void gimp_drawable_tree_view_new_pattern_dropped(GtkWidget *widget,
                                                        gint x, gint y,
                                                        GimpViewable *viewable,
                                                        gpointer data);
static void gimp_drawable_tree_view_new_color_dropped(GtkWidget *widget, gint x,
                                                      gint y,
                                                      const GimpRGB *color,
                                                      gpointer data);

G_DEFINE_TYPE_WITH_CODE(
    GimpDrawableTreeView, gimp_drawable_tree_view, GIMP_TYPE_ITEM_TREE_VIEW,
...
static void gimp_drawable_tree_view_constructed(GObject *object) {
  ...
  gimp_dnd_viewable_dest_add(
      gimp_item_tree_view_get_new_button(item_view), GIMP_TYPE_PATTERN,
      gimp_drawable_tree_view_new_pattern_dropped, item_view);
  gimp_dnd_color_dest_add(gimp_item_tree_view_get_new_button(item_view),
                          gimp_drawable_tree_view_new_color_dropped, item_view);

  gimp_dnd_color_dest_add(GTK_WIDGET(tree_view->view), NULL, tree_view);
...
static gboolean gimp_drawable_tree_view_select_item(GimpContainerView *view,
  ...
  if (image) {
    ...
    success = (item == NULL || floating_sel == NULL ||
...
static gboolean gimp_drawable_tree_view_drop_possible(
  ...
  if (GIMP_CONTAINER_TREE_VIEW_CLASS(parent_class)
          ->drop_possible(tree_view, src_type, src_viewables, dest_viewable,
                          ...
                          return_drag_action)) {
    if (src_type == GIMP_DND_TYPE_COLOR || src_type == GIMP_DND_TYPE_PATTERN) {
      if (!dest_viewable ||
...
static void gimp_drawable_tree_view_drop_viewables(
  ...
  for (iter = src_viewables; iter; iter = iter->next) {
    ...
    if (dest_viewable && GIMP_IS_PATTERN(src_viewable)) {
      ...
      GimpFillOptions *options =
          gimp_fill_options_new(image->gimp, NULL, FALSE);
      ...
      gimp_drawable_edit_fill(GIMP_DRAWABLE(dest_viewable), options,
  ...
  GIMP_CONTAINER_TREE_VIEW_CLASS(parent_class)
      ->drop_viewables(view, src_viewables, dest_viewable, drop_pos);
}
...
static void gimp_drawable_tree_view_set_image(GimpItemTreeView *view,
  ...
  if (gimp_item_tree_view_get_image(view))
    g_signal_handlers_disconnect_by_func(
        ...
        gimp_drawable_tree_view_floating_selection_changed, view);
  ...
  if (gimp_item_tree_view_get_image(view))
    g_signal_connect(
        ...
        G_CALLBACK(gimp_drawable_tree_view_floating_selection_changed), view);
}
...
static void gimp_drawable_tree_view_new_dropped(GimpItemTreeView *view,
                                                GimpFillOptions *options,
