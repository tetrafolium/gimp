...
enum { PROP_0, PROP_CHANNEL, PROP_SCALE, PROP_BORDER_WIDTH, PROP_SUBDIVISIONS };

static void gimp_histogram_view_dispose(GObject *object);
static void gimp_histogram_view_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_histogram_view_get_property(GObject *object, guint property_id,
...
static void gimp_histogram_view_get_preferred_height(GtkWidget *widget,
                                                     gint *minimum_height,
                                                     gint *natural_height);
static gboolean gimp_histogram_view_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_histogram_view_draw_spike(
    GimpHistogramView *view, GimpHistogramChannel channel, cairo_t *cr,
    const GdkRGBA *fg_color, cairo_operator_t fg_operator,
    const GdkRGBA *bg_color, gint x, gint i, gint j, gdouble max,
    gdouble bg_max, gint height, gint border);

G_DEFINE_TYPE(GimpHistogramView, gimp_histogram_view, GTK_TYPE_DRAWING_AREA)
...
static void gimp_histogram_view_class_init(GimpHistogramViewClass *klass) {
  ...
  histogram_view_signals[RANGE_CHANGED] = g_signal_new(
      "range-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpHistogramViewClass, range_changed), NULL, NULL,
      gimp_marshal_VOID__INT_INT, G_TYPE_NONE, 2, G_TYPE_INT, G_TYPE_INT);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("histogram-channel", NULL, NULL,
                        GIMP_TYPE_HISTOGRAM_CHANNEL, GIMP_HISTOGRAM_VALUE,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_SCALE,
      g_param_spec_enum("histogram-scale", NULL, NULL,
                        GIMP_TYPE_HISTOGRAM_SCALE, GIMP_HISTOGRAM_SCALE_LINEAR,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_BORDER_WIDTH,
      g_param_spec_int("border-width", NULL, NULL, 0, 32, 1,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_SUBDIVISIONS,
      g_param_spec_int("subdivisions", NULL, NULL, 1, 64, 5,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}
...
static gdouble gimp_histogram_view_get_maximum(GimpHistogramView *view,
...
static gboolean gimp_histogram_view_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (view->histogram)
    max = gimp_histogram_view_get_maximum(view, view->histogram, view->channel);
  ...
  for (x = 0; x < width; x++) {
    ...
    if (view->subdivisions > 1 && x >= (xstop * width / view->subdivisions)) {
      ...
      gtk_render_background(style, cr, x + border, border, 1, height - 1);
    ...
    if (view->channel == GIMP_HISTOGRAM_RGB) {
      ...
      for (c = 0; c < 3; c++)
        gimp_histogram_view_draw_spike(view, GIMP_HISTOGRAM_RED + c, cr, &black,
                                       CAIRO_OPERATOR_OVER, NULL, x, i, j, max,
                                       bg_max, height, border);

      for (c = 0; c < 3; c++)
        gimp_histogram_view_draw_spike(view, GIMP_HISTOGRAM_RED + c, cr,
                                       &rgb_color[c], CAIRO_OPERATOR_ADD, NULL,
                                       x, i, j, max, bg_max, height, border);

      gimp_histogram_view_draw_spike(
          view, view->channel, cr, in_selection ? &color_in : &color_out,
          CAIRO_OPERATOR_OVER, NULL, x, i, j, max, bg_max, height, border);
    } else {
      gimp_histogram_view_draw_spike(
          view, view->channel, cr, in_selection ? &color_in : &color_out,
          CAIRO_OPERATOR_OVER, in_selection ? &bg_color_in : &bg_color_out, x,
          i, j, max, bg_max, height, border);
    }
  }
...
static void gimp_histogram_view_draw_spike(
  ...
  if (view->histogram) {
    ...
    do {
      gdouble v = gimp_histogram_get_value(view->histogram, channel, ii++);
  ...
  if (bg_color && view->bg_histogram) {
    ...
    do {
      gdouble v = gimp_histogram_get_value(view->bg_histogram, channel, ii++);
...
static gboolean gimp_histogram_view_button_press(GtkWidget *widget,
  ...
  if (!view->grab_seat && bevent->type == GDK_BUTTON_PRESS &&
      bevent->button == 1) {
    ...
    if (gdk_seat_grab(seat, gtk_widget_get_window(widget),
                      GDK_SEAT_CAPABILITY_ALL, FALSE, NULL, (GdkEvent *)bevent,
    ...
    view->start =
        CLAMP(((bevent->x - view->border_width) * view->n_bins) / width, 0,
              view->n_bins - 1);
    view->end = view->start;
...
static gboolean gimp_histogram_view_button_release(GtkWidget *widget,
  ...
  if (gdk_event_get_seat((GdkEvent *)bevent) == view->grab_seat &&
    ...
    g_signal_emit(view, histogram_view_signals[RANGE_CHANGED], 0, view->start,
                  view->end);
  }
...
static gboolean gimp_histogram_view_motion_notify(GtkWidget *widget,
  ...
  if (gdk_event_get_seat((GdkEvent *)mevent) == view->grab_seat) {
    ...
    view->start =
        CLAMP(((mevent->x - view->border_width) * view->n_bins) / width, 0,
              view->n_bins - 1);
  ...
  if (view->histogram != histogram) {
    if (view->histogram) {
      g_signal_handlers_disconnect_by_func(view->histogram,
                                           gimp_histogram_view_notify, view);
    ...
    if (histogram) {
      ...
      g_signal_connect(histogram, "notify",
                       G_CALLBACK(gimp_histogram_view_notify), view);
...
void gimp_histogram_view_set_range(GimpHistogramView *view, gint start,
                                   gint end) {
  g_return_if_fail(GIMP_IS_HISTOGRAM_VIEW(view));

  if (view->start != MIN(start, end) || view->end != MAX(start, end)) {
    view->start = MIN(start, end);
    view->end = MAX(start, end);
...
void gimp_histogram_view_get_range(GimpHistogramView *view, gint *start,
                                   gint *end) {
  g_return_if_fail(GIMP_IS_HISTOGRAM_VIEW(view));
...
static void gimp_histogram_view_update_bins(GimpHistogramView *view) {
  ...
  if (new_bins > 0 && new_bins != view->n_bins) {
    view->start = MIN(ROUND((gdouble)view->start * new_bins / view->n_bins),
                      new_bins - 1);
    view->end =
        MAX(ROUND((gdouble)(view->end + 1) * new_bins / view->n_bins) - 1, 0);
