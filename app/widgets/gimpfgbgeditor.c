...
enum { PROP_0, PROP_CONTEXT, PROP_ACTIVE_COLOR };

enum { COLOR_CLICKED, TOOLTIP, LAST_SIGNAL };

static void gimp_fg_bg_editor_dispose(GObject *object);
static void gimp_fg_bg_editor_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_fg_bg_editor_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);

static GtkSizeRequestMode gimp_fg_bg_editor_get_request_mode(GtkWidget *widget);
static void gimp_fg_bg_editor_get_preferred_width_for_height(
...
static gboolean gimp_fg_bg_editor_draw(GtkWidget *widget, cairo_t *cr);
...
static gboolean gimp_fg_bg_editor_drag_motion(GtkWidget *widget,
                                              ...
                                              gint y, guint time);
static gboolean gimp_fg_bg_editor_query_tooltip(GtkWidget *widget, gint x,
                                                gint y, gboolean keyboard_mode,
                                                GtkTooltip *tooltip);

static void gimp_fg_bg_editor_drag_color(GtkWidget *widget, GimpRGB *color,
                                         gpointer data);
static void gimp_fg_bg_editor_drop_color(GtkWidget *widget, gint x, gint y,
...
static void gimp_fg_bg_editor_draw_color_frame(GimpFgBgEditor *editor,
                                               cairo_t *cr,
                                               const GimpRGB *color, gint x,
                                               gint y, gint width, gint height,
                                               gint corner_dx, gint corner_dy);
...
static void gimp_fg_bg_editor_class_init(GimpFgBgEditorClass *klass) {
  ...
  editor_signals[COLOR_CLICKED] = g_signal_new(
      "color-clicked", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpFgBgEditorClass, color_clicked), NULL, NULL, NULL,
      G_TYPE_NONE, 1, GIMP_TYPE_ACTIVE_COLOR);

  editor_signals[TOOLTIP] =
      g_signal_new("tooltip", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpFgBgEditorClass, tooltip), NULL, NULL,
                   gimp_marshal_VOID__INT_OBJECT, G_TYPE_NONE, 2, G_TYPE_INT,
                   GTK_TYPE_TOOLTIP);
  ...
  widget_class->get_preferred_width_for_height =
      gimp_fg_bg_editor_get_preferred_width_for_height;
  ...
  g_object_class_install_property(
      object_class, PROP_ACTIVE_COLOR,
      g_param_spec_enum("active-color", NULL, NULL, GIMP_TYPE_ACTIVE_COLOR,
                        GIMP_ACTIVE_COLOR_FOREGROUND, GIMP_PARAM_READWRITE));
...
static void gimp_fg_bg_editor_init(GimpFgBgEditor *editor) {
  ...
  gimp_dnd_color_source_add(GTK_WIDGET(editor), gimp_fg_bg_editor_drag_color,
                            NULL);
  gimp_dnd_color_dest_add(GTK_WIDGET(editor), gimp_fg_bg_editor_drop_color,
                          NULL);
...
static gboolean gimp_fg_bg_editor_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (!editor->default_icon)
    editor->default_icon =
        gimp_widget_load_icon(widget, GIMP_ICON_COLORS_DEFAULT, 12);
  ...
  if (default_w < width / 2 && default_h < height / 2) {
    ...
    surface = gdk_cairo_surface_create_from_pixbuf(editor->default_icon,
                                                   scale_factor, NULL);
    cairo_set_source_surface(cr, surface, border.left,
  ...
  if (!editor->swap_icon)
    editor->swap_icon =
        gimp_widget_load_icon(widget, GIMP_ICON_COLORS_SWAP, 12);
  ...
  if (swap_w < width / 2 && swap_h < height / 2) {
    ...
    surface = gdk_cairo_surface_create_from_pixbuf(editor->swap_icon,
                                                   scale_factor, NULL);
    cairo_set_source_surface(cr, surface, width - border.right - swap_w,
                             border.top);
    cairo_surface_destroy(surface);
  ...
  rect.height =
      height - MAX(default_h, swap_h) - 2 - border.left - border.right;

  if (rect.height > (height * 3 / 4))
    rect.width =
        MAX(rect.width - (rect.height - ((height * 3 / 4))), width * 2 / 3);
  ...
  if (editor->context) {
    ...
    gimp_fg_bg_editor_draw_color_frame(editor, cr, &color, rect.x, rect.y,
...
static GimpFgBgTarget gimp_fg_bg_editor_target(GimpFgBgEditor *editor, gint x,
                                               gint y) {
  ...
  if (x > border.left && x < border.left + rect_w && y > border.top &&
      y < border.top + rect_h) {
    return GIMP_FG_BG_TARGET_FOREGROUND;
  } else if (x > width - border.right - rect_w && x < width - border.right &&
             y > height - border.bottom - rect_h &&
             y < height - border.bottom) {
    return GIMP_FG_BG_TARGET_BACKGROUND;
  } else if (x > border.left && x < border.left + button_width &&
             y > border.top + rect_h && y < height - border.bottom) {
    return GIMP_FG_BG_TARGET_DEFAULT;
  } else if (x > border.left + rect_w && x < width - border.right &&
             y > border.top && y < border.top + button_height) {
...
static gboolean gimp_fg_bg_editor_button_press(GtkWidget *widget,
  ...
  if (bevent->button == 1 && bevent->type == GDK_BUTTON_PRESS) {
    GimpFgBgTarget target =
        gimp_fg_bg_editor_target(editor, bevent->x, bevent->y);
    ...
    case GIMP_FG_BG_TARGET_FOREGROUND:
      if (editor->active_color != GIMP_ACTIVE_COLOR_FOREGROUND)
        gimp_fg_bg_editor_set_active(editor, GIMP_ACTIVE_COLOR_FOREGROUND);
    ...
    case GIMP_FG_BG_TARGET_BACKGROUND:
      if (editor->active_color != GIMP_ACTIVE_COLOR_BACKGROUND)
        gimp_fg_bg_editor_set_active(editor, GIMP_ACTIVE_COLOR_BACKGROUND);
...
static gboolean gimp_fg_bg_editor_query_tooltip(GtkWidget *widget, gint x,
  ...
  if (!keyboard_mode) {
    ...
    if (target != GIMP_FG_BG_TARGET_INVALID) {
      g_signal_emit(widget, editor_signals[TOOLTIP], 0, target, tooltip);
...
GtkWidget *gimp_fg_bg_editor_new(GimpContext *context) {
  ...
  return g_object_new(GIMP_TYPE_FG_BG_EDITOR, "context", context, NULL);
}
  ...
  if (context != editor->context) {
    if (editor->context) {
      ...
      g_signal_handlers_disconnect_by_func(
          editor->context, G_CALLBACK(gimp_fg_bg_editor_image_changed), editor);
      g_object_unref(editor->context);

      g_signal_handlers_disconnect_by_func(
          editor->color_config, gimp_fg_bg_editor_destroy_transform, editor);
    ...
    if (context) {
      ...
      g_signal_connect_swapped(context, "foreground-changed",
                               G_CALLBACK(gtk_widget_queue_draw), editor);
      ...
      editor->color_config =
          g_object_ref(context->gimp->config->color_management);
...
static void gimp_fg_bg_editor_create_transform(GimpFgBgEditor *editor) {
  if (editor->color_config) {
    ...
    editor->transform = gimp_widget_get_color_transform(
        GTK_WIDGET(editor), editor->color_config, profile,
...
static void gimp_fg_bg_editor_draw_color_frame(GimpFgBgEditor *editor,
  ...
  if (editor->transform) {
    gimp_color_transform_process_pixels(
        editor->transform, babl_format("R'G'B'A double"), color,
        babl_format("R'G'B'A double"), &transformed_color, 1);
  } else {
