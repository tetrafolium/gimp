...
static void gimp_toolbox_set_property(GObject *object, guint property_id,
                                      const GValue *value, GParamSpec *pspec);
static void gimp_toolbox_get_property(GObject *object, guint property_id,
                                      GValue *value, GParamSpec *pspec);

static gboolean gimp_toolbox_button_press_event(GtkWidget *widget,
                                                GdkEventButton *event);
static void gimp_toolbox_drag_leave(GtkWidget *widget, GdkDragContext *context,
                                    guint time, GimpToolbox *toolbox);
...
static gboolean gimp_toolbox_drag_drop(GtkWidget *widget,
                                       ...
                                       guint time, GimpToolbox *toolbox);
static gchar *gimp_toolbox_get_description(GimpDock *dock, gboolean complete);
static void gimp_toolbox_set_host_geometry_hints(GimpDock *dock,
                                                 GtkWindow *window);
static void gimp_toolbox_book_added(GimpDock *dock, GimpDockbook *dockbook);
static void gimp_toolbox_book_removed(GimpDock *dock, GimpDockbook *dockbook);
static void gimp_toolbox_wilber_style_updated(GtkWidget *widget,
                                              GimpToolbox *toolbox);
static gboolean gimp_toolbox_draw_wilber(GtkWidget *widget, cairo_t *cr);
...
static GtkWidget *toolbox_create_image_area(GimpToolbox *toolbox,
                                            GimpContext *context);
static void toolbox_paste_received(GtkClipboard *clipboard, const gchar *text,
                                   gpointer data);
...
static void gimp_toolbox_constructed(GObject *object) {
   ...
   */
  g_signal_connect(toolbox, "drag-leave", G_CALLBACK(gimp_toolbox_drag_leave),
                   toolbox);
  g_signal_connect(toolbox, "drag-motion", G_CALLBACK(gimp_toolbox_drag_motion),
                   toolbox);
  g_signal_connect(toolbox, "drag-drop", G_CALLBACK(gimp_toolbox_drag_drop),
                   toolbox);
  g_signal_connect(toolbox->p->vbox, "drag-leave",
                   G_CALLBACK(gimp_toolbox_drag_leave), toolbox);
  g_signal_connect(toolbox->p->vbox, "drag-motion",
                   G_CALLBACK(gimp_toolbox_drag_motion), toolbox);
  g_signal_connect(toolbox->p->vbox, "drag-drop",
                   G_CALLBACK(gimp_toolbox_drag_drop), toolbox);

  toolbox->p->header = gtk_frame_new(NULL);
  gtk_frame_set_shadow_type(GTK_FRAME(toolbox->p->header), GTK_SHADOW_NONE);
  gtk_box_pack_start(GTK_BOX(toolbox->p->vbox), toolbox->p->header, FALSE,
                     FALSE, 0);

  g_object_bind_property(config, "toolbox-wilber", toolbox->p->header,
                         "visible", G_BINDING_SYNC_CREATE);

  g_signal_connect(toolbox->p->header, "style-updated",
                   G_CALLBACK(gimp_toolbox_wilber_style_updated), toolbox);
  g_signal_connect(toolbox->p->header, "draw",
                   G_CALLBACK(gimp_toolbox_draw_wilber), toolbox);
  ...
  gimp_tool_palette_set_toolbox(GIMP_TOOL_PALETTE(toolbox->p->tool_palette),
                                toolbox);
  gtk_box_pack_start(GTK_BOX(toolbox->p->vbox), toolbox->p->tool_palette, FALSE,
                     FALSE, 0);
  ...
  gtk_flow_box_set_selection_mode(GTK_FLOW_BOX(toolbox->p->area_box),
                                  GTK_SELECTION_NONE);
  gtk_box_pack_start(GTK_BOX(toolbox->p->vbox), toolbox->p->area_box, FALSE,
                     FALSE, 0);
  ...
  toolbox->p->color_area =
      toolbox_create_color_area(toolbox, toolbox->p->context);
  gtk_flow_box_insert(GTK_FLOW_BOX(toolbox->p->area_box),
                      toolbox->p->color_area, -1);

  g_object_bind_property(config, "toolbox-color-area", toolbox->p->color_area,
                         "visible", G_BINDING_SYNC_CREATE);

  toolbox->p->foo_area = toolbox_create_foo_area(toolbox, toolbox->p->context);
  gtk_flow_box_insert(GTK_FLOW_BOX(toolbox->p->area_box), toolbox->p->foo_area,
                      -1);

  g_object_bind_property(config, "toolbox-foo-area", toolbox->p->foo_area,
                         "visible", G_BINDING_SYNC_CREATE);

  toolbox->p->image_area =
      toolbox_create_image_area(toolbox, toolbox->p->context);
  gtk_flow_box_insert(GTK_FLOW_BOX(toolbox->p->area_box),
                      toolbox->p->image_area, -1);

  g_object_bind_property(config, "toolbox-image-area", toolbox->p->image_area,
                         "visible", G_BINDING_SYNC_CREATE);
...
static gboolean gimp_toolbox_button_press_event(GtkWidget *widget,
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 2) {
    ...
    gtk_clipboard_request_text(clipboard, toolbox_paste_received,
...
static gboolean gimp_toolbox_drag_motion(GtkWidget *widget,
  ...
  if (gimp_paned_box_will_handle_drag(toolbox->p->drag_handler, widget, context,
                                      x, y, time)) {
...
static gchar *gimp_toolbox_get_description(GimpDock *dock, gboolean complete) {
  ...
  gchar *dock_desc =
      GIMP_DOCK_CLASS(parent_class)->get_description(dock, complete);
...
static void gimp_toolbox_book_removed(GimpDock *dock, GimpDockbook *dockbook) {
  ...
  if (!gimp_dock_get_dockbooks(dock) && !toolbox->p->in_destruction) {
    gimp_dock_invalidate_geometry(dock);
  }
}
...
static void gimp_toolbox_set_host_geometry_hints(GimpDock *dock,
  ...
  if (gimp_tool_palette_get_button_size(
          GIMP_TOOL_PALETTE(toolbox->p->tool_palette), &button_width,
          &button_height)) {
    ...
    gtk_window_set_geometry_hints(window, NULL, &geometry,
                                  GDK_HINT_MIN_SIZE | GDK_HINT_BASE_SIZE |
                                      GDK_HINT_RESIZE_INC | GDK_HINT_USER_POS);

    gimp_dialog_factory_set_has_min_size(window, TRUE);
  }
}

GtkWidget *gimp_toolbox_new(GimpDialogFactory *factory, GimpContext *context,
                            GimpUIManager *ui_manager) {
  g_return_val_if_fail(GIMP_IS_DIALOG_FACTORY(factory), NULL);
  g_return_val_if_fail(GIMP_IS_CONTEXT(context), NULL);
  g_return_val_if_fail(GIMP_IS_UI_MANAGER(ui_manager), NULL);

  return g_object_new(GIMP_TYPE_TOOLBOX, "context", context, NULL);
}
...
static void gimp_toolbox_wilber_style_updated(GtkWidget *widget,
  ...
  if (gimp_tool_palette_get_button_size(
          GIMP_TOOL_PALETTE(toolbox->p->tool_palette), &button_width,
          &button_height)) {
    gtk_widget_set_size_request(widget, button_width * PANGO_SCALE_SMALL,
...
static GtkWidget *toolbox_create_color_area(GimpToolbox *toolbox,
  ...
  g_object_set(col_area, "halign", GTK_ALIGN_CENTER, "valign", GTK_ALIGN_CENTER,
...
static GtkWidget *toolbox_create_foo_area(GimpToolbox *toolbox,
  ...
  g_object_set(foo_area, "halign", GTK_ALIGN_CENTER, "valign", GTK_ALIGN_CENTER,
               "margin-left", 2, "margin-right", 2, "margin-top", 2,
               "margin-bottom", 2, NULL);
...
static GtkWidget *toolbox_create_image_area(GimpToolbox *toolbox,
  ...
  g_object_set(image_area, "halign", GTK_ALIGN_CENTER, "valign",
               GTK_ALIGN_CENTER, "margin-left", 2, "margin-right", 2,
