...
enum { PROP_0, PROP_CONTROLLER_INFO, PROP_CONTEXT };

enum { COLUMN_EVENT, COLUMN_BLURB, COLUMN_ICON_NAME, COLUMN_ACTION, N_COLUMNS };
...
gimp_controller_editor_class_init(GimpControllerEditorClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_CONTROLLER_INFO,
      g_param_spec_object("controller-info", NULL, NULL,
...
static void gimp_controller_editor_constructed(GObject *object) {
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("Name:"), 0.0, 0.5,
                           gimp_prop_label_new(G_OBJECT(controller), "name"),
                           1);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("State:"), 0.0, 0.5,
                           gimp_prop_label_new(G_OBJECT(controller), "state"),
                           1);

  property_specs = g_object_class_list_properties(
      G_OBJECT_CLASS(controller_class), &n_property_specs);
  ...
  for (i = 0; i < n_property_specs; i++) {
    ...
    if (G_IS_PARAM_SPEC_STRING(pspec)) {
      ...
      gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++,
                               g_param_spec_get_nick(pspec), 0.0, 0.5, widget,
                               1);
  ...
  g_signal_connect(tv, "row-activated",
                   G_CALLBACK(gimp_controller_editor_row_activated), editor);

  editor->sel = gtk_tree_view_get_selection(GTK_TREE_VIEW(tv));

  g_signal_connect(editor->sel, "changed",
                   G_CALLBACK(gimp_controller_editor_sel_changed), editor);
  ...
  for (i = 0; i < n_events; i++) {
    ...
    gtk_list_store_set(store, &iter, COLUMN_EVENT, event_name, COLUMN_BLURB,
                       event_blurb, COLUMN_ICON_NAME, icon_name, COLUMN_ACTION,
                       event_action, -1);
  }

  gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(tv), 0, _("Event"),
                                              gtk_cell_renderer_text_new(),
                                              "text", COLUMN_BLURB, NULL);
  ...
  gtk_tree_view_column_set_attributes(column, cell, "text", COLUMN_ACTION,
                                      NULL);
  ...
  g_signal_connect(editor->grab_button, "toggled",
                   G_CALLBACK(gimp_controller_editor_grab_toggled), editor);
  ...
  g_signal_connect(editor->edit_button, "clicked",
                   G_CALLBACK(gimp_controller_editor_edit_clicked), editor);
  ...
  g_signal_connect(editor->delete_button, "clicked",
                   G_CALLBACK(gimp_controller_editor_delete_clicked), editor);
...
static void gimp_controller_editor_unmap(GtkWidget *widget) {
  ...
  if (editor->edit_dialog)
    gtk_dialog_response(GTK_DIALOG(editor->edit_dialog), GTK_RESPONSE_CANCEL);
...
GtkWidget *gimp_controller_editor_new(GimpControllerInfo *info,
  ...
  return g_object_new(GIMP_TYPE_CONTROLLER_EDITOR, "controller-info", info,
...
static void gimp_controller_editor_sel_changed(GtkTreeSelection *sel,
  ...
  if (gtk_tree_selection_get_selected(sel, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, COLUMN_BLURB, &event, COLUMN_ACTION,
                       &action, -1);
...
static gboolean gimp_controller_editor_snooper(GimpControllerInfo *info,
                                               GimpController *controller,
  ...
  event_name =
      gimp_controller_get_event_name(info->controller, event->any.event_id);

  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
       iter_valid = gtk_tree_model_iter_next(model, &iter)) {
    gchar *list_name;

    gtk_tree_model_get(model, &iter, COLUMN_EVENT, &list_name, -1);
...
static void gimp_controller_editor_grab_toggled(GtkWidget *button,
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(button))) {
    gimp_controller_info_set_event_snooper(
        editor->info, gimp_controller_editor_snooper, editor);
  } else {
    gimp_controller_info_set_event_snooper(editor->info, NULL, NULL);
  }
}
...
static void gimp_controller_editor_edit_clicked(GtkWidget *button,
  ...
  if (gtk_tree_selection_get_selected(editor->sel, &model, &iter))
    gtk_tree_model_get(model, &iter, COLUMN_EVENT, &event_name, COLUMN_BLURB,
                       &event_blurb, COLUMN_ACTION, &action_name, -1);

  if (event_name) {
    GtkWidget *view;
    gchar *title;

    title = g_strdup_printf(_("Select Action for Event '%s'"), event_blurb);

    editor->edit_dialog = gimp_viewable_dialog_new(
        g_list_prepend(NULL, editor->info), editor->context,
        _("Select Controller Event Action"), "gimp-controller-action-dialog",
        GIMP_ICON_EDIT, title, gtk_widget_get_toplevel(GTK_WIDGET(editor)),
        gimp_standard_help_func, GIMP_HELP_PREFS_INPUT_CONTROLLERS,
    ...
    gimp_dialog_factory_add_foreign(
        gimp_dialog_factory_get_singleton(), "gimp-controller-action-dialog",
        editor->edit_dialog, gimp_widget_get_monitor(button));

    g_signal_connect(editor->edit_dialog, "response",
                     G_CALLBACK(gimp_controller_editor_edit_response), editor);

    view = gimp_action_editor_new(gimp_ui_managers_from_name("<Image>")->data,
                                  action_name, FALSE);
    gtk_container_set_border_width(GTK_CONTAINER(view), 12);
    gtk_box_pack_start(
        GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(editor->edit_dialog))),
        view, TRUE, TRUE, 0);
    gtk_widget_show(view);

    g_signal_connect(GIMP_ACTION_EDITOR(view)->view, "row-activated",
                     G_CALLBACK(gimp_controller_editor_edit_activated), editor);

    editor->edit_sel = gtk_tree_view_get_selection(
        GTK_TREE_VIEW(GIMP_ACTION_EDITOR(view)->view));
...
gimp_controller_editor_delete_clicked(GtkWidget *button,
  ...
  if (gtk_tree_selection_get_selected(editor->sel, &model, &iter))
    gtk_tree_model_get(model, &iter, COLUMN_EVENT, &event_name, -1);

  if (event_name) {
    g_hash_table_remove(editor->info->mapping, event_name);
    g_free(event_name);

    gtk_list_store_set(GTK_LIST_STORE(model), &iter, COLUMN_ICON_NAME, NULL,
                       COLUMN_ACTION, NULL, -1);
  }
}

static void
gimp_controller_editor_edit_activated(GtkTreeView *tv, GtkTreePath *path,
                                      GtkTreeViewColumn *column,
                                      GimpControllerEditor *editor) {
...
static void gimp_controller_editor_edit_response(GtkWidget *dialog,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    if (gtk_tree_selection_get_selected(editor->edit_sel, &model, &iter))
      gtk_tree_model_get(model, &iter, GIMP_ACTION_VIEW_COLUMN_ICON_NAME,
                         &icon_name, GIMP_ACTION_VIEW_COLUMN_NAME, &action_name,
                         -1);
    ...
    if (event_name && action_name) {
      g_hash_table_insert(editor->info->mapping, g_strdup(event_name),
                          g_strdup(action_name));

      gtk_list_store_set(GTK_LIST_STORE(model), &iter, COLUMN_ICON_NAME,
                         icon_name, COLUMN_ACTION, action_name, -1);
    }
...
static GtkWidget *gimp_controller_string_view_new(GimpController *controller,
  ...
  if (pspec->flags & G_PARAM_WRITABLE) {
    ...
    if (G_IS_PARAM_SPEC_OBJECT(model_spec) &&
      ...
      g_object_get(controller, model_name, &model, NULL);
    }

    g_free(model_name);

    if (model) {
      widget = gimp_prop_string_combo_box_new(G_OBJECT(controller), pspec->name,
                                              model, 0, 1);
...
static GtkWidget *gimp_controller_int_view_new(GimpController *controller,
  ...
  if (pspec->flags & G_PARAM_WRITABLE) {
    ...
    if (model) {
      widget =
          gimp_prop_int_combo_box_new(G_OBJECT(controller), pspec->name, model);
      g_object_unref(model);
    } else {
      widget =
          gimp_prop_spin_button_new(G_OBJECT(controller), pspec->name, 1, 8, 0);
    }
  } else {
    widget = gimp_prop_label_new(G_OBJECT(controller), pspec->name);
  }
