...
gboolean gimp_device_info_get_event_coords(GimpDeviceInfo *info,
  ...
  if (event && gdk_event_get_axis(event, GDK_AXIS_X, &x)) {
    *coords = default_coords;
     ...
     */
    if (event->any.window && event->any.window != window) {
      ...
      if (src_widget && dest_widget) {
        ...
        if (gtk_widget_translate_coordinates(src_widget, dest_widget, 0, 0,
                                             &offset_x, &offset_y)) {
          coords->x += offset_x;
          coords->y += offset_y;
        }
    ...
    if (gdk_event_get_axis(event, GDK_AXIS_PRESSURE, &coords->pressure)) {
      coords->pressure =
          gimp_device_info_map_axis(info, GDK_AXIS_PRESSURE, coords->pressure);
    }

    if (gdk_event_get_axis(event, GDK_AXIS_XTILT, &coords->xtilt)) {
      coords->xtilt =
          gimp_device_info_map_axis(info, GDK_AXIS_XTILT, coords->xtilt);
    }

    if (gdk_event_get_axis(event, GDK_AXIS_YTILT, &coords->ytilt)) {
      coords->ytilt =
          gimp_device_info_map_axis(info, GDK_AXIS_YTILT, coords->ytilt);
    }

    if (gdk_event_get_axis(event, GDK_AXIS_WHEEL, &coords->wheel)) {
      coords->wheel =
          gimp_device_info_map_axis(info, GDK_AXIS_WHEEL, coords->wheel);
    }

    if (gdk_event_get_axis(event, GDK_AXIS_DISTANCE, &coords->distance)) {
      coords->distance =
          gimp_device_info_map_axis(info, GDK_AXIS_DISTANCE, coords->distance);
    }

    if (gdk_event_get_axis(event, GDK_AXIS_ROTATION, &coords->rotation)) {
      coords->rotation =
          gimp_device_info_map_axis(info, GDK_AXIS_ROTATION, coords->rotation);
    }

    if (gdk_event_get_axis(event, GDK_AXIS_SLIDER, &coords->slider)) {
      coords->slider =
          gimp_device_info_map_axis(info, GDK_AXIS_SLIDER, coords->slider);
    }
...
void gimp_device_info_get_device_coords(GimpDeviceInfo *info, GdkWindow *window,
  ...
  *coords = default_coords;
  ...
  if (gdk_device_get_axis(device, axes, GDK_AXIS_DISTANCE, &coords->distance)) {
    coords->distance =
        gimp_device_info_map_axis(info, GDK_AXIS_DISTANCE, coords->distance);
  }

  if (gdk_device_get_axis(device, axes, GDK_AXIS_ROTATION, &coords->rotation)) {
    coords->rotation =
        gimp_device_info_map_axis(info, GDK_AXIS_ROTATION, coords->rotation);
  }

  if (gdk_device_get_axis(device, axes, GDK_AXIS_SLIDER, &coords->slider)) {
    coords->slider =
        gimp_device_info_map_axis(info, GDK_AXIS_SLIDER, coords->slider);
  }
}
...
void gimp_device_info_get_time_coords(GimpDeviceInfo *info, GdkTimeCoord *event,
  ...
  *coords = default_coords;
  ...
  if (gdk_device_get_axis(device, event->axes, GDK_AXIS_XTILT,
                          &coords->xtilt)) {
    coords->xtilt =
        gimp_device_info_map_axis(info, GDK_AXIS_XTILT, coords->xtilt);
  }

  if (gdk_device_get_axis(device, event->axes, GDK_AXIS_YTILT,
                          &coords->ytilt)) {
    coords->ytilt =
        gimp_device_info_map_axis(info, GDK_AXIS_YTILT, coords->ytilt);
  }

  if (gdk_device_get_axis(device, event->axes, GDK_AXIS_WHEEL,
                          &coords->wheel)) {
    coords->wheel =
        gimp_device_info_map_axis(info, GDK_AXIS_WHEEL, coords->wheel);
  }

  if (gdk_device_get_axis(device, event->axes, GDK_AXIS_DISTANCE,
                          &coords->distance)) {
    coords->distance =
        gimp_device_info_map_axis(info, GDK_AXIS_DISTANCE, coords->distance);
  }

  if (gdk_device_get_axis(device, event->axes, GDK_AXIS_ROTATION,
                          &coords->rotation)) {
    coords->rotation =
        gimp_device_info_map_axis(info, GDK_AXIS_ROTATION, coords->rotation);
  }

  if (gdk_device_get_axis(device, event->axes, GDK_AXIS_SLIDER,
                          &coords->slider)) {
    coords->slider =
        gimp_device_info_map_axis(info, GDK_AXIS_SLIDER, coords->slider);
  }
