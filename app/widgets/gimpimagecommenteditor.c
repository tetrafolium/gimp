...
G_DEFINE_TYPE(GimpImageCommentEditor, gimp_image_comment_editor,
...
static void gimp_image_comment_editor_init(GimpImageCommentEditor *editor) {
  GtkWidget *vbox;
...
GtkWidget *gimp_image_comment_editor_new(GimpImage *image) {
  ...
  return g_object_new(GIMP_TYPE_IMAGE_COMMENT_EDITOR, "image", image,
                      "parasite", GIMP_IMAGE_COMMENT_PARASITE, NULL);
}
...
static void gimp_image_comment_editor_update(GimpImageParasiteView *view) {
  ...
  g_signal_handlers_block_by_func(
      editor->buffer, gimp_image_comment_editor_buffer_changed, editor);
...
gimp_image_comment_editor_buffer_changed(GtkTextBuffer *buffer,
  ...
  if (len > 0) {
    ...
    parasite = gimp_parasite_new(GIMP_IMAGE_COMMENT_PARASITE,
                                 GIMP_PARASITE_PERSISTENT, len + 1, text);
