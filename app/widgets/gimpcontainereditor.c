...
G_DEFINE_TYPE_WITH_CODE(
    GimpContainerEditor, gimp_container_editor, GTK_TYPE_BOX,
...
static void gimp_container_editor_class_init(GimpContainerEditorClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_VIEW_TYPE,
      g_param_spec_enum("view-type", NULL, NULL, GIMP_TYPE_VIEW_TYPE,
                        GIMP_VIEW_TYPE_LIST,
...
static void gimp_container_editor_init(GimpContainerEditor *editor) {
...
static void gimp_container_editor_constructed(GObject *object) {
  ...
  case GIMP_VIEW_TYPE_GRID:
    editor->view = GIMP_CONTAINER_VIEW(gimp_container_icon_view_new(
        editor->priv->container, editor->priv->context, editor->priv->view_size,
        editor->priv->view_border_width));
    break;

  case GIMP_VIEW_TYPE_LIST:
    editor->view = GIMP_CONTAINER_VIEW(gimp_container_tree_view_new(
        editor->priv->container, editor->priv->context, editor->priv->view_size,
        editor->priv->view_border_width));
    break;
  ...
  if (GIMP_IS_LIST(editor->priv->container))
    gimp_container_view_set_reorderable(
        GIMP_CONTAINER_VIEW(editor->view),
  ...
  if (editor->priv->menu_factory && editor->priv->menu_identifier &&
    ...
    gimp_editor_create_menu(
        GIMP_EDITOR(editor->view), editor->priv->menu_factory,
        editor->priv->menu_identifier, editor->priv->ui_path, editor);
  }

  gtk_box_pack_start(GTK_BOX(editor), GTK_WIDGET(editor->view), TRUE, TRUE, 0);
  gtk_widget_show(GTK_WIDGET(editor->view));

  editor->priv->busy_box = gimp_busy_box_new(NULL);
  gtk_box_pack_start(GTK_BOX(editor), editor->priv->busy_box, TRUE, TRUE, 0);

  g_object_bind_property(editor->priv->busy_box, "visible", editor->view,
                         "visible",
  ...
  g_signal_connect_object(editor->view, "select-item",
                          G_CALLBACK(gimp_container_editor_select_item), editor,
                          G_CONNECT_AFTER);
...
gimp_container_editor_get_selection_mode(GimpContainerEditor *editor) {
  return gimp_container_view_get_selection_mode(
      GIMP_CONTAINER_VIEW(editor->view));
}
...
static gboolean gimp_container_editor_select_item(GtkWidget *widget,
  ...
  if (gimp_editor_get_ui_manager(GIMP_EDITOR(editor->view)))
    gimp_ui_manager_update(
        gimp_editor_get_ui_manager(GIMP_EDITOR(editor->view)),
...
static GtkWidget *gimp_container_editor_get_preview(GimpDocked *docked,
  ...
  return gimp_docked_get_preview(GIMP_DOCKED(editor->view), context, size);
}
...
void gimp_container_editor_bind_to_async_set(GimpContainerEditor *editor,
  ...
  if (async_set) {
    gimp_busy_box_set_message(GIMP_BUSY_BOX(editor->priv->busy_box), message);

    editor->priv->async_set_binding = g_object_bind_property(
        async_set, "empty", editor->priv->busy_box, "visible",
        G_BINDING_SYNC_CREATE | G_BINDING_INVERT_BOOLEAN);
  } else {
