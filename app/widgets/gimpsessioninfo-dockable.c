  ...
  while (g_scanner_peek_next_token(scanner) == token) {
    ...
    case G_TOKEN_SYMBOL:
      ...
      case SESSION_INFO_DOCKABLE_AUX:
        token = gimp_session_info_aux_deserialize(scanner, &info->aux_info);
...
GimpDockable *gimp_session_info_dockable_restore(GimpSessionInfoDockable *info,
  ...
  dockable =
      gimp_dialog_factory_dockable_new(gimp_dock_get_dialog_factory(dock), dock,
                                       info->identifier, info->view_size);
