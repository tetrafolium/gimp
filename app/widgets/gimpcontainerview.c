...
enum { SELECT_ITEM, SELECT_ITEMS, ACTIVATE_ITEM, CONTEXT_ITEM, LAST_SIGNAL };

#define GIMP_CONTAINER_VIEW_GET_PRIVATE(obj)                                   \
  (gimp_container_view_get_private((GimpContainerView *)(obj)))
...
static void gimp_container_view_reorder(GimpContainerView *view,
                                        GimpViewable *viewable, gint new_index,
                                        GimpContainer *container);
...
static void gimp_container_view_context_changed(GimpContext *context,
                                                GimpViewable *viewable,
                                                GimpContainerView *view);
static void gimp_container_view_viewable_dropped(GtkWidget *widget, gint x,
                                                 gint y, GimpViewable *viewable,
                                                 gpointer data);
...
gimp_container_view_default_init(GimpContainerViewInterface *iface) {
  view_signals[SELECT_ITEM] = g_signal_new(
      "select-item", G_TYPE_FROM_INTERFACE(iface), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpContainerViewInterface, select_item), NULL, NULL,
      gimp_marshal_BOOLEAN__OBJECT_POINTER, G_TYPE_BOOLEAN, 2, GIMP_TYPE_OBJECT,
      G_TYPE_POINTER);

  view_signals[SELECT_ITEMS] = g_signal_new(
      "select-items", G_TYPE_FROM_INTERFACE(iface), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpContainerViewInterface, select_items), NULL, NULL,
      NULL, G_TYPE_BOOLEAN, 2, G_TYPE_POINTER, G_TYPE_POINTER);

  view_signals[ACTIVATE_ITEM] = g_signal_new(
      "activate-item", G_TYPE_FROM_INTERFACE(iface), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpContainerViewInterface, activate_item), NULL, NULL,
      gimp_marshal_VOID__OBJECT_POINTER, G_TYPE_NONE, 2, GIMP_TYPE_OBJECT,
      G_TYPE_POINTER);

  view_signals[CONTEXT_ITEM] = g_signal_new(
      "context-item", G_TYPE_FROM_INTERFACE(iface), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpContainerViewInterface, context_item), NULL, NULL,
      gimp_marshal_VOID__OBJECT_POINTER, G_TYPE_NONE, 2, GIMP_TYPE_OBJECT,
      G_TYPE_POINTER);
  ...
  g_object_interface_install_property(
      iface, g_param_spec_object("container", NULL, NULL, GIMP_TYPE_CONTAINER,
                                 GIMP_PARAM_READWRITE));

  g_object_interface_install_property(
      iface, g_param_spec_object("context", NULL, NULL, GIMP_TYPE_CONTEXT,
                                 GIMP_PARAM_READWRITE));

  g_object_interface_install_property(
      iface,
      g_param_spec_enum("selection-mode", NULL, NULL, GTK_TYPE_SELECTION_MODE,
                        GTK_SELECTION_SINGLE, GIMP_PARAM_READWRITE));

  g_object_interface_install_property(
      iface, g_param_spec_boolean("reorderable", NULL, NULL, FALSE,
                                  GIMP_PARAM_READWRITE));

  g_object_interface_install_property(
      iface,
      g_param_spec_int("view-size", NULL, NULL, 1,
                       GIMP_VIEWABLE_MAX_PREVIEW_SIZE, GIMP_VIEW_SIZE_MEDIUM,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_interface_install_property(
      iface, g_param_spec_int("view-border-width", NULL, NULL, 0,
                              GIMP_VIEW_MAX_BORDER_WIDTH, 1,
                              GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}
...
gimp_container_view_private_finalize(GimpContainerViewPrivate *private) {
  ...
  g_clear_pointer(&private->name_changed_handler, gimp_tree_handler_disconnect);
...
private
  = g_object_get_qdata((GObject *)view, private_key);
  ...
  private
    = g_slice_new0(GimpContainerViewPrivate);

  private
    ->view_border_width = 1;

  private
    ->item_hash = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL,
                                        view_iface->insert_data_free);

    g_object_set_qdata_full(
        (GObject *)view, private_key, private,
        (GDestroyNotify)gimp_container_view_private_finalize);

    g_signal_connect(view, "destroy",
                     G_CALLBACK(gimp_container_view_private_dispose), private);
  }
...
void gimp_container_view_install_properties(GObjectClass *klass) {
  g_object_class_override_property(klass, GIMP_CONTAINER_VIEW_PROP_CONTAINER,
                                   "container");
  g_object_class_override_property(klass, GIMP_CONTAINER_VIEW_PROP_CONTEXT,
                                   "context");
  g_object_class_override_property(
      klass, GIMP_CONTAINER_VIEW_PROP_SELECTION_MODE, "selection-mode");
  g_object_class_override_property(klass, GIMP_CONTAINER_VIEW_PROP_REORDERABLE,
                                   "reorderable");
  g_object_class_override_property(klass, GIMP_CONTAINER_VIEW_PROP_VIEW_SIZE,
                                   "view-size");
  g_object_class_override_property(
      klass, GIMP_CONTAINER_VIEW_PROP_VIEW_BORDER_WIDTH, "view-border-width");
}
...
private
  = GIMP_CONTAINER_VIEW_GET_PRIVATE(view);
...
static void gimp_container_view_real_set_container(GimpContainerView *view,
  ...
  if (private->container) {
    ...
    g_signal_handlers_disconnect_by_func(private->container,
                                         gimp_container_view_freeze, view);
    g_signal_handlers_disconnect_by_func(private->container,
                                         gimp_container_view_thaw, view);
...
private
  ...
  if (private->container) {
    ...
    g_signal_connect_object(private->container, "freeze",
                            G_CALLBACK(gimp_container_view_freeze), view,
                            G_CONNECT_SWAPPED);
    g_signal_connect_object(private->container, "thaw",
                            G_CALLBACK(gimp_container_view_thaw), view,
                            G_CONNECT_SWAPPED);
...
void gimp_container_view_set_view_size(GimpContainerView *view, gint view_size,
                                       gint view_border_width) {
  GimpContainerViewPrivate *private;
...
private
  ...
  private
    ->view_size = view_size;
  private
    ->view_border_width = view_border_width;
...
void gimp_container_view_enable_dnd(GimpContainerView *view, GtkButton *button,
                                    GType children_type) {
  g_return_if_fail(GIMP_IS_CONTAINER_VIEW(view));
  g_return_if_fail(GTK_IS_BUTTON(button));

  gimp_dnd_viewable_list_dest_add(GTK_WIDGET(button), children_type,
                                  gimp_container_view_button_viewables_dropped,
                                  view);
  gimp_dnd_viewable_dest_add(GTK_WIDGET(button), children_type,
                             gimp_container_view_button_viewable_dropped, view);
}
...
private
  ...
  g_signal_emit(view, view_signals[SELECT_ITEMS], 0, viewables, NULL, &success);
...
private
  ...
  g_signal_emit(view, view_signals[ACTIVATE_ITEM], 0, viewable, insert_data);
}
...
private
  ...
  g_signal_emit(view, view_signals[CONTEXT_ITEM], 0, viewable, insert_data);
}
...
gboolean gimp_container_view_multi_selected(GimpContainerView *view,
  ...
  if (selected_count > 0) {
    g_signal_emit(view, view_signals[SELECT_ITEMS], 0, items, items_data,
                  &success);
  }
 ...
 */
gint gimp_container_view_get_selected(GimpContainerView *view, GList **items,
                                      GList **items_data) {
  g_return_val_if_fail(GIMP_IS_CONTAINER_VIEW(view), 0);
...
void gimp_container_view_set_property(GObject *object, guint property_id,
...
void gimp_container_view_get_property(GObject *object, guint property_id,
...
static void gimp_container_view_add_container(GimpContainerView *view,
  ...
  gimp_container_foreach(container, (GFunc)gimp_container_view_add_foreach,
                         view);
  ...
  private
    ->name_changed_handler = gimp_tree_handler_connect(
        container, viewable_class->name_changed_signal,
        G_CALLBACK(gimp_container_view_name_changed), view);

    if (GIMP_CONTAINER_VIEW_GET_IFACE(view)->expand_item) {
    private
      ->expanded_changed_handler = gimp_tree_handler_connect(
          container, "expanded-changed",
          G_CALLBACK(gimp_container_view_expanded_changed), view);
    }

    g_type_class_unref(viewable_class);
  }

  g_signal_connect_object(container, "add", G_CALLBACK(gimp_container_view_add),
                          view, G_CONNECT_SWAPPED);
  g_signal_connect_object(container, "remove",
                          G_CALLBACK(gimp_container_view_remove), view,
                          G_CONNECT_SWAPPED);
  g_signal_connect_object(container, "reorder",
                          G_CALLBACK(gimp_container_view_reorder), view,
                          G_CONNECT_SWAPPED);
}
...
private
  ...
  insert_data = view_iface->insert_item(view, viewable, parent_insert_data, -1);
...
private
  ...
  insert_data =
      view_iface->insert_item(view, viewable, parent_insert_data, index);
...
static void gimp_container_view_remove_container(GimpContainerView *view,
  ...
  g_signal_handlers_disconnect_by_func(container, gimp_container_view_add,
                                       view);
  g_signal_handlers_disconnect_by_func(container, gimp_container_view_remove,
                                       view);
  g_signal_handlers_disconnect_by_func(container, gimp_container_view_reorder,
                                       view);
  ...
  gimp_container_foreach(container, (GFunc)gimp_container_view_remove_foreach,
                         view);
...
static void gimp_container_view_reorder(GimpContainerView *view,
  ...
  if (insert_data) {
    GIMP_CONTAINER_VIEW_GET_IFACE(view)->reorder_item(view, viewable, new_index,
                                                      insert_data);
  }
}
...
static void gimp_container_view_name_changed(GimpViewable *viewable,
  ...
  if (insert_data) {
    GIMP_CONTAINER_VIEW_GET_IFACE(view)->rename_item(view, viewable,
                                                     insert_data);
  }
}
...
static void gimp_container_view_expanded_changed(GimpViewable *viewable,
  ...
  if (insert_data) {
    GIMP_CONTAINER_VIEW_GET_IFACE(view)->expand_item(view, viewable,
                                                     insert_data);
  }
}
...
static void gimp_container_view_connect_context(GimpContainerView *view) {
  ...
  if (signal_name) {
    ...
    if (private->dnd_widget)
      gimp_dnd_viewable_dest_add(private->dnd_widget, children_type,
                                 gimp_container_view_viewable_dropped, view);

    if (!gimp_container_frozen(private->container)) {
      GimpObject *object =
          gimp_context_get_by_type(private->context, children_type);
...
static void gimp_container_view_disconnect_context(GimpContainerView *view) {
  ...
  if (signal_name) {
    g_signal_handlers_disconnect_by_func(
        private->context, gimp_container_view_context_changed, view);

    if (private->dnd_widget) {
      gtk_drag_dest_unset(private->dnd_widget);
      gimp_dnd_viewable_dest_remove(private->dnd_widget, children_type);
    }
  }
}
