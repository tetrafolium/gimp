...
static void gimp_save_dialog_compression_toggled(GtkToggleButton *button,
...
G_DEFINE_TYPE(GimpSaveDialog, gimp_save_dialog, GIMP_TYPE_FILE_DIALOG)

#define parent_class gimp_save_dialog_parent_class

static void gimp_save_dialog_class_init(GimpSaveDialogClass *klass) {
...
GtkWidget *gimp_save_dialog_new(Gimp *gimp) {
  ...
  return g_object_new(GIMP_TYPE_SAVE_DIALOG, "gimp", gimp, "title",
                      _("Save Image"), "role", "gimp-file-save", "help-id",
                      GIMP_HELP_FILE_SAVE, "ok-button-label", _("_Save"),

                      "automatic-label", _("By Extension"), "automatic-help-id",
                      GIMP_HELP_FILE_SAVE_BY_EXTENSION,

                      "action", GTK_FILE_CHOOSER_ACTION_SAVE, "file-procs",
                      GIMP_FILE_PROCEDURE_GROUP_SAVE, "file-procs-all-images",
                      GIMP_FILE_PROCEDURE_GROUP_EXPORT, "file-filter-label",
                      _("All XCF images"), NULL);
}

void gimp_save_dialog_set_image(GimpSaveDialog *dialog, GimpImage *image,
  ...
  if (!dir_file)
    dir_file = g_object_get_data(G_OBJECT(image), "gimp-image-source-file");
  ...
  gimp_image_get_xcf_version(image, FALSE, &rle_version, &version_string, NULL);
  gimp_image_get_xcf_version(image, TRUE, &zlib_version, NULL, NULL);
  if (rle_version != zlib_version) {
    ...
    gtk_container_add(GTK_CONTAINER(dialog->compression_frame), label);
    gtk_widget_show(label);
    g_free(text);
  }

  compression_toggle =
      gtk_frame_get_label_widget(GTK_FRAME(dialog->compression_frame));
  ...
  if (g_file_query_file_type(dir_file, G_FILE_QUERY_INFO_NONE, NULL) ==
    ...
    gtk_file_chooser_set_current_folder_file(GTK_FILE_CHOOSER(dialog), dir_file,
                                             NULL);
  } else {
...
static void gimp_save_dialog_add_extra_widgets(GimpSaveDialog *dialog) {
  ...
  /* Compression toggle. */
  compression_toggle = gtk_check_button_new_with_mnemonic(
      _("Save this _XCF file with better but slower compression"));
  ...
  gtk_frame_set_label_widget(GTK_FRAME(dialog->compression_frame),
                             compression_toggle);
  gtk_widget_show(compression_toggle);
  gimp_file_dialog_add_extra_widget(GIMP_FILE_DIALOG(dialog),
                                    dialog->compression_frame, FALSE, FALSE, 0);
  ...
  gimp_file_dialog_add_extra_widget(GIMP_FILE_DIALOG(dialog),
                                    dialog->compat_info, FALSE, FALSE, 0);
  gtk_widget_show(dialog->compat_info);

  g_signal_connect(compression_toggle, "toggled",
                   G_CALLBACK(gimp_save_dialog_compression_toggled), dialog);
}
...
static void gimp_save_dialog_compression_toggled(GtkToggleButton *button,
  ...
  compat_hint = g_strdup_printf(_("The image uses features from %s and "
