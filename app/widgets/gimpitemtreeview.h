...
typedef void (*GimpSetItemFunc)(GimpImage *image, GimpItem *item);
typedef GList *(*GimpGetItemsFunc)(GimpImage *image);
typedef void (*GimpSetItemsFunc)(GimpImage *image, GList *items);
typedef void (*GimpAddItemFunc)(GimpImage *image, GimpItem *item,
                                GimpItem *parent, gint index,
                                gboolean push_undo);
typedef void (*GimpRemoveItemFunc)(GimpImage *image, GimpItem *item,
                                   gboolean push_undo, GimpItem *new_active);
typedef GimpItem *(*GimpNewItemFunc)(GimpImage *image);

#define GIMP_TYPE_ITEM_TREE_VIEW (gimp_item_tree_view_get_type())
#define GIMP_ITEM_TREE_VIEW(obj)                                               \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), GIMP_TYPE_ITEM_TREE_VIEW,                 \
                              GimpItemTreeView))
#define GIMP_ITEM_TREE_VIEW_CLASS(klass)                                       \
  (G_TYPE_CHECK_CLASS_CAST((klass), GIMP_TYPE_ITEM_TREE_VIEW,                  \
                           GimpItemTreeViewClass))
#define GIMP_IS_ITEM_TREE_VIEW(obj)                                            \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), GIMP_TYPE_ITEM_TREE_VIEW))
#define GIMP_IS_ITEM_TREE_VIEW_CLASS(klass)                                    \
  (G_TYPE_CHECK_CLASS_TYPE((klass), GIMP_TYPE_ITEM_TREE_VIEW))
#define GIMP_ITEM_TREE_VIEW_GET_CLASS(obj)                                     \
  (G_TYPE_INSTANCE_GET_CLASS((obj), GIMP_TYPE_ITEM_TREE_VIEW,                  \
                             GimpItemTreeViewClass))
...
struct _GimpItemTreeViewClass {
  ...
  /*  signals  */
  void (*set_image)(GimpItemTreeView *view, GimpImage *image);
...
GtkWidget *gimp_item_tree_view_new(GType view_type, gint view_size,
