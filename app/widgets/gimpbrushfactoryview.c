...
static void gimp_brush_factory_view_init(GimpBrushFactoryView *view) {
  view->spacing_adjustment =
      gtk_adjustment_new(0.0, 1.0, 5000.0, 1.0, 10.0, 0.0);

  view->spacing_scale =
      gimp_spin_scale_new(view->spacing_adjustment, _("Spacing"), 1);
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(view->spacing_scale), 1.0,
                                   200.0);
  gimp_help_set_help_data(view->spacing_scale,
                          _("Percentage of width of brush"), NULL);

  g_signal_connect(view->spacing_adjustment, "value-changed",
                   G_CALLBACK(gimp_brush_factory_view_spacing_update), view);
}
...
static void gimp_brush_factory_view_dispose(GObject *object) {
  ...
  if (view->spacing_changed_handler_id) {
    ...
    factory =
        gimp_data_factory_view_get_data_factory(GIMP_DATA_FACTORY_VIEW(editor));
    container = gimp_data_factory_get_container(factory);

    gimp_container_remove_handler(container, view->spacing_changed_handler_id);
...
GtkWidget *gimp_brush_factory_view_new(GimpViewType view_type,
  ...
  factory_view = g_object_new(
      GIMP_TYPE_BRUSH_FACTORY_VIEW, "view-type", view_type, "data-factory",
      factory, "context", context, "view-size", view_size, "view-border-width",
      view_border_width, "menu-factory", menu_factory, "menu-identifier",
      "<Brushes>", "ui-path", "/brushes-popup", "action-group", "brushes",
      NULL);
  ...
  gimp_editor_add_action_button(GIMP_EDITOR(editor->view), "brushes",
                                "brushes-open-as-image", NULL);

  gtk_box_pack_end(GTK_BOX(editor->view), factory_view->spacing_scale, FALSE,
                   FALSE, 0);
  gtk_widget_show(factory_view->spacing_scale);

  factory_view->spacing_changed_handler_id = gimp_container_add_handler(
      gimp_data_factory_get_container(factory), "spacing-changed",
      G_CALLBACK(gimp_brush_factory_view_spacing_changed), factory_view);
...
static void gimp_brush_factory_view_select_item(GimpContainerEditor *editor,
  ...
  if (viewable && gimp_container_have(container, GIMP_OBJECT(viewable))) {
    ...
    g_signal_handlers_block_by_func(
        view->spacing_adjustment, gimp_brush_factory_view_spacing_update, view);
...
static void gimp_brush_factory_view_spacing_update(GtkAdjustment *adjustment,
  ...
  if (brush && view->change_brush_spacing) {
    g_signal_handlers_block_by_func(
        brush, gimp_brush_factory_view_spacing_changed, view);
