...
static void color_area_color_clicked(GimpFgBgEditor *editor,
                                     ...
                                     GimpContext *context);
static void color_area_tooltip(GimpFgBgEditor *editor, GimpFgBgTarget target,
                               GtkTooltip *tooltip, GimpToolbox *toolbox);
...
GtkWidget *gimp_toolbox_color_area_create(GimpToolbox *toolbox, gint width,
                                          gint height) {
  GimpContext *context;
  ...
  gimp_help_set_help_data(color_area, NULL, GIMP_HELP_TOOLBOX_COLOR_AREA);
  g_object_set(color_area, "has-tooltip", TRUE, NULL);

  g_signal_connect(color_area, "color-clicked",
                   G_CALLBACK(color_area_color_clicked), context);

  g_signal_connect(color_area, "tooltip", G_CALLBACK(color_area_tooltip),
                   toolbox);
...
static void color_area_foreground_changed(GimpContext *context,
  ...
  if (edit_color == GIMP_ACTIVE_COLOR_FOREGROUND) {
    g_signal_handlers_block_by_func(dialog, color_area_dialog_update, context);

    /* FIXME this should use GimpColorDialog API */
    gimp_color_selection_set_color(GIMP_COLOR_SELECTION(dialog->selection),
                                   color);

    g_signal_handlers_unblock_by_func(dialog, color_area_dialog_update,
                                      context);
  }
}
...
static void color_area_dialog_update(GimpColorDialog *dialog,
  ...
  case GIMP_COLOR_DIALOG_UPDATE:
    if (edit_color == GIMP_ACTIVE_COLOR_FOREGROUND) {
      g_signal_handlers_block_by_func(context, color_area_foreground_changed,
                                      dialog);

      gimp_context_set_foreground(context, color);

      g_signal_handlers_unblock_by_func(context, color_area_foreground_changed,
                                        dialog);
    } else {
      g_signal_handlers_block_by_func(context, color_area_background_changed,
                                      dialog);

      gimp_context_set_background(context, color);

      g_signal_handlers_unblock_by_func(context, color_area_background_changed,
                                        dialog);
    }
    break;
...
static void color_area_color_clicked(GimpFgBgEditor *editor,
  ...
  if (!color_dialog) {
    color_dialog = gimp_color_dialog_new(
        NULL, context, TRUE, NULL, NULL, NULL, GTK_WIDGET(editor),
        gimp_dialog_factory_get_singleton(), "gimp-toolbox-color-dialog",
        &color, TRUE, FALSE);
    ...
    gimp_dialog_factory_position_dialog(
        gimp_dialog_factory_get_singleton(), "gimp-toolbox-color-dialog",
        color_dialog, gimp_widget_get_monitor(GTK_WIDGET(editor)));
  }
...
static void color_area_tooltip(GimpFgBgEditor *editor, GimpFgBgTarget target,
  ...
  case GIMP_FG_BG_TARGET_SWAP:
    action =
        gimp_ui_manager_find_action(manager, "context", "context-colors-swap");
  ...
  if (text) {
    ...
    if (action) {
      ...
      accel_key =
          gtk_accel_group_find(accel_group, accel_find_func, accel_closure);
