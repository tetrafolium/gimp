...
enum { PROP_0, PROP_GIMP, PROP_CONFIG, PROP_CONTAINER };
...
#define GET_PRIVATE(item)                                                      \
  ((GimpSettingsEditorPrivate *)gimp_settings_editor_get_instance_private(     \
...
static gboolean gimp_settings_editor_row_separator_func(GtkTreeModel *model,
                                                        GtkTreeIter *iter,
                                                        gpointer data);
...
static void gimp_settings_editor_class_init(GimpSettingsEditorClass *klass) {
...
static void gimp_settings_editor_init(GimpSettingsEditor *editor) {
...
private
  ->view = gimp_container_tree_view_new(
  ...
  g_signal_connect(tree_view, "select-item",
                   G_CALLBACK(gimp_settings_editor_select_item), editor);

  gimp_container_tree_view_connect_name_edited(
      tree_view, G_CALLBACK(gimp_settings_editor_name_edited), editor);

private
  ->import_button = gimp_editor_add_button(
      GIMP_EDITOR(tree_view), GIMP_ICON_DOCUMENT_OPEN,
      _("Import presets from a file"), NULL,
      G_CALLBACK(gimp_settings_editor_import_clicked), NULL, editor);

private
  ->export_button = gimp_editor_add_button(
      GIMP_EDITOR(tree_view), GIMP_ICON_DOCUMENT_SAVE,
      _("Export the selected presets to a file"), NULL,
      G_CALLBACK(gimp_settings_editor_export_clicked), NULL, editor);

private
  ->delete_button = gimp_editor_add_button(
      GIMP_EDITOR(tree_view), GIMP_ICON_EDIT_DELETE,
      _("Delete the selected preset"), NULL,
      G_CALLBACK(gimp_settings_editor_delete_clicked), NULL, editor);
...
private
  ->selected_setting = G_OBJECT(viewable);
...
static void gimp_settings_editor_delete_clicked(GtkWidget *widget,
  ...
  if (private->selected_setting) {
    ...
    new = gimp_container_get_neighbor_of(
        private->container, GIMP_OBJECT(private->selected_setting));
...
static void gimp_settings_editor_name_edited(GtkCellRendererText *cell,
  ...
  if (gtk_tree_model_get_iter(tree_view->model, &iter, path)) {
    ...
    if (strlen(name) && strcmp(old_name, name)) {
      ...
      if (t > 0)
        g_object_set(object, "time", (gint64)0, NULL);
...
GtkWidget *gimp_settings_editor_new(Gimp *gimp, GObject *config,
  ...
  return g_object_new(GIMP_TYPE_SETTINGS_EDITOR, "gimp", gimp, "config", config,
