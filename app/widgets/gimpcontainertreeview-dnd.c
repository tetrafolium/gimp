...
static gint
gimp_container_tree_view_viewable_sort(GimpViewable *v1, GimpViewable *v2,
 ...
 */
static gboolean gimp_container_tree_view_drop_status(
    GimpContainerTreeView *tree_view, GdkDragContext *context, gint x, gint y,
    guint time, GtkTreePath **return_path, GdkAtom *return_atom,
    GimpDndType *return_src_type, GList **return_src,
    GimpViewable **return_dest, GtkTreeViewDropPosition *return_pos) {
  ...
  target_atom = gtk_drag_dest_find_target(GTK_WIDGET(tree_view->view), context,
                                          target_list);
    ...
    /* Various GimpViewable list (GList) drag data. */
    {
      ...
      for (iter = src_viewables; iter; iter = iter->next)
        if (!GIMP_IS_VIEWABLE(iter->data)) {
          g_warning(
              "%s: contents of the viewable list has the wrong type '%s'.",
  ...
  if (gtk_tree_view_get_path_at_pos(tree_view->view, x, y, &drop_path, NULL,
                                    NULL, NULL)) {
    ...
    if (n_children > 0 && gtk_tree_model_iter_nth_child(tree_view->model, &iter,
                                                        NULL, n_children - 1)) {
      GimpViewRenderer *renderer;
  ...
  if (dest_viewable || tree_view->priv->dnd_drop_to_empty) {
    if (GIMP_CONTAINER_TREE_VIEW_GET_CLASS(tree_view)->drop_possible(
            tree_view, src_type, src_viewables, dest_viewable, drop_path,
            drop_pos, &drop_pos, &drag_action)) {
      ...
      if (return_src) {
        src_viewables = g_list_sort_with_data(
            src_viewables,
...
static gboolean gimp_container_tree_view_scroll_timeout(gpointer data) {
  ...
  new_value =
      CLAMP(new_value, gtk_adjustment_get_lower(adj),
  ...
  if (tree_view->priv->scroll_timeout_id) {
    ...
    tree_view->priv->scroll_timeout_id =
        g_timeout_add(tree_view->priv->scroll_timeout_interval,
                      gimp_container_tree_view_scroll_timeout, tree_view);
  }
...
gboolean
gimp_container_tree_view_drag_motion(GtkWidget *widget, GdkDragContext *context,
  ...
  if (gimp_container_tree_view_drop_status(tree_view, context, x, y, time,
...
gboolean gimp_container_tree_view_drag_drop(GtkWidget *widget,
  ...
  if (gimp_container_tree_view_drop_status(tree_view, context, x, y, time, NULL,
                                           &target, &src_type, &src_viewables,
    ...
    if (src_viewables) {
       ...
       * return success?
       */
      tree_view_class->drop_viewables(tree_view, src_viewables, dest_viewable,
                                      drop_pos);
  ...
  if (gimp_container_tree_view_drop_status(tree_view, context, x, y, time, NULL,
    ...
    case GIMP_DND_TYPE_NETSCAPE_URL:
      if (tree_view_class->drop_uri_list) {
        ...
        if (uri_list) {
          tree_view_class->drop_uri_list(tree_view, uri_list, dest_viewable,
                                         drop_pos);
    ...
    case GIMP_DND_TYPE_COLOR:
      if (tree_view_class->drop_color) {
        ...
        if (gimp_selection_data_get_color(selection_data, &color)) {
          tree_view_class->drop_color(tree_view, &color, dest_viewable,
                                      drop_pos);
    ...
    case GIMP_DND_TYPE_SVG_XML:
      if (tree_view_class->drop_svg) {
        ...
        stream = gimp_selection_data_get_stream(selection_data, &stream_length);

        if (stream) {
          tree_view_class->drop_svg(tree_view, (const gchar *)stream,
                                    stream_length, dest_viewable, drop_pos);
    ...
    case GIMP_DND_TYPE_COMPONENT:
      if (tree_view_class->drop_component) {
        ...
        if (tree_view->dnd_gimp)
          image = gimp_selection_data_get_component(
              selection_data, tree_view->dnd_gimp, &component);

        if (image) {
          tree_view_class->drop_component(tree_view, image, component,
                                          dest_viewable, drop_pos);
    ...
    case GIMP_DND_TYPE_PIXBUF:
      if (tree_view_class->drop_pixbuf) {
        ...
        if (pixbuf) {
          tree_view_class->drop_pixbuf(tree_view, pixbuf, dest_viewable,
                                       drop_pos);
...
void gimp_container_tree_view_real_drop_viewables(
  ...
  for (iter = src_viewables; iter; iter = iter->next) {
    ...
    if ((drop_pos == GTK_TREE_VIEW_DROP_INTO_OR_AFTER ||
      ...
      dest_container =
          gimp_viewable_get_children(gimp_viewable_get_parent(dest_viewable));
    } else {
    ...
    if (gimp_viewable_get_parent(src_viewable)) {
      src_container =
          gimp_viewable_get_children(gimp_viewable_get_parent(src_viewable));
    } else {
    ...
    if (src_container == dest_container) {
      ...
      gimp_container_reorder(src_container, GIMP_OBJECT(src_viewable),
                             dest_index);
    } else {
      switch (drop_pos) {
      case GTK_TREE_VIEW_DROP_AFTER:
