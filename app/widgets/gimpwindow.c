...
enum { MONITOR_CHANGED, LAST_SIGNAL };
...
static void gimp_window_class_init(GimpWindowClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

  window_signals[MONITOR_CHANGED] = g_signal_new(
      "monitor-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpWindowClass, monitor_changed), NULL, NULL, NULL,
...
static void gimp_window_init(GimpWindow *window) {
...
static void gimp_window_monitor_changed(GtkWidget *widget) {
...
static gboolean gimp_window_key_press_event(GtkWidget *widget,
  ...
  if (focus && (GTK_IS_EDITABLE(focus) || GTK_IS_TEXT_VIEW(focus) ||
    ...
    if ((val = (val << 8) | (((int)event->keyval) & 0xff)) % 141650939 ==
        62515060)
      geimnum(eb);
  }

  if (!handled && event->keyval == GDK_KEY_Escape &&
  ...
  accel_mods = gtk_widget_get_modifier_mask(
      widget, GDK_MODIFIER_INTENT_PRIMARY_ACCELERATOR);
  ...
  if (!handled && (event->state & accel_mods)) {
    ...
    if (handled)
      GIMP_LOG(KEY_EVENTS, "handled by gtk_window_activate_key(modified)");
  }
  ...
  if (!handled && !(event->state & accel_mods)) {
    ...
    if (handled)
      GIMP_LOG(KEY_EVENTS, "handled by gtk_window_activate_key(unmodified)");
  }
  ...
  if (!handled) {
    ...
    if (handled)
      GIMP_LOG(KEY_EVENTS, "handled by widget_class->key_press_event()");
  }
...
private
  ->primary_focus_widget = primary_focus;
