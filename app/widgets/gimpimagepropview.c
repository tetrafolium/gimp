...
static GtkWidget *gimp_image_prop_view_add_label(GtkGrid *grid, gint row,
                                                 const gchar *text);
static void gimp_image_prop_view_undo_event(GimpImage *image,
                                            GimpUndoEvent event, GimpUndo *undo,
...
static void gimp_image_prop_view_init(GimpImagePropView *view) {
  ...
  g_signal_connect(view, "realize", G_CALLBACK(gimp_image_prop_view_realize),
                   NULL);
}
...
static void gimp_image_prop_view_constructed(GObject *object) {
  ...
  g_signal_connect_object(view->image, "name-changed",
                          ...
                          G_OBJECT(view), G_CONNECT_SWAPPED);
...
GtkWidget *gimp_image_prop_view_new(GimpImage *image) {
  ...
  return g_object_new(GIMP_TYPE_IMAGE_PROP_VIEW, "image", image, NULL);
}
...
static GtkWidget *gimp_image_prop_view_add_label(GtkGrid *grid, gint row,
  ...
  desc = g_object_new(GTK_TYPE_LABEL, "label", text, "xalign", 1.0, "yalign",
                      0.0, NULL);
  gimp_label_set_attributes(GTK_LABEL(desc), PANGO_ATTR_WEIGHT,
                            PANGO_WEIGHT_BOLD, -1);
  gtk_grid_attach(grid, desc, 0, row, 1, 1);
  gtk_widget_show(desc);

  label = g_object_new(GTK_TYPE_LABEL, "xalign", 0.0, "yalign", 0.5,
...
static void gimp_image_prop_view_label_set_memsize(GtkWidget *label,
                                                   GimpObject *object) {
...
static void gimp_image_prop_view_label_set_filename(GtkWidget *label,
                                                    GimpImage *image) {
  GFile *file = gimp_image_get_any_file(image);

  if (file) {
    gtk_label_set_text(GTK_LABEL(label), gimp_file_get_utf8_name(file));
...
static void gimp_image_prop_view_label_set_filesize(GtkWidget *label,
                                                    GimpImage *image) {
  GFile *file = gimp_image_get_any_file(image);
...
static void gimp_image_prop_view_label_set_filetype(GtkWidget *label,
                                                    GimpImage *image) {
  ...
  if (!proc) {
    ...
    if (file)
      proc = gimp_plug_in_manager_file_procedure_find(
          manager, GIMP_FILE_PROCEDURE_GROUP_OPEN, file, NULL);
  }

  gtk_label_set_text(GTK_LABEL(label),
                     proc ? gimp_procedure_get_label(GIMP_PROCEDURE(proc))
                          : NULL);
}

static void gimp_image_prop_view_label_set_undo(GtkWidget *label,
                                                GimpUndoStack *stack) {
...
static void gimp_image_prop_view_update(GimpImagePropView *view) {
  ...
  g_snprintf(
      buf, sizeof(buf),
      ngettext("%d × %d pixel", "%d × %d pixels", gimp_image_get_height(image)),
  ...
  g_snprintf(buf, sizeof(buf), _("%g × %g %s"), xres / unit_factor,
             yres / unit_factor,
  ...
  gimp_enum_get_value(GIMP_TYPE_IMAGE_BASE_TYPE, type, NULL, NULL, &desc, NULL);
  ...
  gimp_enum_get_value(GIMP_TYPE_PRECISION, precision, NULL, NULL, &desc, NULL);
  ...
  /*  number of layers  */
  g_snprintf(buf, sizeof(buf), "%d", gimp_image_get_n_layers(image));
  gtk_label_set_text(GTK_LABEL(view->layers_label), buf);

  /*  number of channels  */
  g_snprintf(buf, sizeof(buf), "%d", gimp_image_get_n_channels(image));
  gtk_label_set_text(GTK_LABEL(view->channels_label), buf);

  /*  number of vectors  */
  g_snprintf(buf, sizeof(buf), "%d", gimp_image_get_n_vectors(image));
