...
enum { COLOR_APPLIED, LAST_SIGNAL };
...
static void gimp_text_buffer_class_init(GimpTextBufferClass *klass) {
  ...
  buffer_signals[COLOR_APPLIED] = g_signal_new(
      "color-applied", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpTextBufferClass, color_applied), NULL, NULL, NULL,
      G_TYPE_NONE, 1, GIMP_TYPE_RGB);
}

static void gimp_text_buffer_init(GimpTextBuffer *buffer) {
  buffer->markup_atom = gtk_text_buffer_register_serialize_format(
      GTK_TEXT_BUFFER(buffer), "application/x-gimp-pango-markup",
      gimp_text_buffer_serialize, NULL, NULL);

  gtk_text_buffer_register_deserialize_format(
      GTK_TEXT_BUFFER(buffer), "application/x-gimp-pango-markup",
      gimp_text_buffer_deserialize, NULL, NULL);
}
...
static void gimp_text_buffer_constructed(GObject *object) {
  ...
  buffer->bold_tag = gtk_text_buffer_create_tag(
      GTK_TEXT_BUFFER(buffer), "bold", "weight", PANGO_WEIGHT_BOLD, NULL);

  buffer->italic_tag = gtk_text_buffer_create_tag(
      GTK_TEXT_BUFFER(buffer), "italic", "style", PANGO_STYLE_ITALIC, NULL);

  buffer->underline_tag =
      gtk_text_buffer_create_tag(GTK_TEXT_BUFFER(buffer), "underline",
                                 "underline", PANGO_UNDERLINE_SINGLE, NULL);

  buffer->preedit_underline_tag =
      gtk_text_buffer_create_tag(GTK_TEXT_BUFFER(buffer), "preedit-underline",
                                 "underline", PANGO_UNDERLINE_SINGLE, NULL);

  buffer->strikethrough_tag = gtk_text_buffer_create_tag(
      GTK_TEXT_BUFFER(buffer), "strikethrough", "strikethrough", TRUE, NULL);
}
...
void gimp_text_buffer_set_markup(GimpTextBuffer *buffer, const gchar *markup) {
  ...
  if (markup) {
    ...
    if (!gtk_text_buffer_deserialize(GTK_TEXT_BUFFER(buffer), content,
                                     buffer->markup_atom, &insert,
                                     (const guint8 *)markup, -1, &error)) {
      g_printerr("EEK: %s\n", error->message);
      g_clear_error(&error);
    } else {
      GtkTextIter start, end;
      ...
      gtk_text_buffer_insert_range(GTK_TEXT_BUFFER(buffer), &insert, &start,
                                   &end);
    }
...
gchar *gimp_text_buffer_get_markup(GimpTextBuffer *buffer) {
  ...
  markup = (gchar *)gtk_text_buffer_serialize(GTK_TEXT_BUFFER(buffer), content,
                                              buffer->markup_atom, &start, &end,
                                              &length);
...
GtkTextTag *gimp_text_buffer_get_size_tag(GimpTextBuffer *buffer, gint size) {
  GList *list;
  GtkTextTag *tag;
  gchar name[32];
  ...
  tag = gtk_text_buffer_create_tag(GTK_TEXT_BUFFER(buffer), name,
                                   GIMP_TEXT_PROP_NAME_SIZE, size, NULL);
...
void gimp_text_buffer_set_size(GimpTextBuffer *buffer, const GtkTextIter *start,
                               const GtkTextIter *end, gint size) {
  GList *list;
...
void gimp_text_buffer_change_size(GimpTextBuffer *buffer,
                                  ...
                                  const GtkTextIter *end, gint count) {
  GtkTextIter iter;
  ...
  do {
    ...
    if (iter_size != span_size || gtk_text_iter_compare(&iter, end) >= 0) {
      if (span_size != 0) {
...
static GtkTextTag *gimp_text_buffer_get_baseline_tag(GimpTextBuffer *buffer,
                                                     gint baseline) {
  GList *list;
  GtkTextTag *tag;
  gchar name[32];
  ...
  tag =
      gtk_text_buffer_create_tag(GTK_TEXT_BUFFER(buffer), name,
                                 GIMP_TEXT_PROP_NAME_BASELINE, baseline, NULL);
...
void gimp_text_buffer_set_baseline(GimpTextBuffer *buffer,
                                   const GtkTextIter *start,
                                   const GtkTextIter *end, gint baseline) {
  GList *list;
...
void gimp_text_buffer_change_baseline(GimpTextBuffer *buffer,
  ...
  do {
    ...
    iter_tag =
        gimp_text_buffer_get_iter_baseline(buffer, &iter, &iter_baseline);
    ...
    if (iter_baseline != span_baseline ||
      ...
      if (span_baseline + count != 0) {
        span_tag =
            gimp_text_buffer_get_baseline_tag(buffer, span_baseline + count);
...
static GtkTextTag *gimp_text_buffer_get_kerning_tag(GimpTextBuffer *buffer,
                                                    gint kerning) {
  GList *list;
  GtkTextTag *tag;
  gchar name[32];
  ...
  tag = gtk_text_buffer_create_tag(GTK_TEXT_BUFFER(buffer), name,
                                   GIMP_TEXT_PROP_NAME_KERNING, kerning, NULL);
...
void gimp_text_buffer_set_kerning(GimpTextBuffer *buffer,
                                  const GtkTextIter *start,
                                  const GtkTextIter *end, gint kerning) {
  GList *list;
  ...
  do {
    ...
    if (iter_kerning != span_kerning ||
      ...
      if (span_kerning + count != 0) {
        span_tag =
            gimp_text_buffer_get_kerning_tag(buffer, span_kerning + count);
...
GtkTextTag *gimp_text_buffer_get_font_tag(GimpTextBuffer *buffer,
  ...
  tag = gtk_text_buffer_create_tag(GTK_TEXT_BUFFER(buffer), name, "font",
                                   description, NULL);
...
void gimp_text_buffer_set_font(GimpTextBuffer *buffer, const GtkTextIter *start,
                               const GtkTextIter *end, const gchar *font) {
  GList *list;
...
void gimp_text_buffer_set_color(GimpTextBuffer *buffer,
                                const GtkTextIter *start,
                                const GtkTextIter *end, const GimpRGB *color) {
  GList *list;
...
GtkTextTag *gimp_text_buffer_get_preedit_color_tag(GimpTextBuffer *buffer,
  ...
  g_snprintf(name, sizeof(name), "preedit-color-#%02x%02x%02x", r, g, b);
...
GtkTextTag *gimp_text_buffer_get_preedit_bg_color_tag(GimpTextBuffer *buffer,
  ...
  g_snprintf(name, sizeof(name), "bg-color-#%02x%02x%02x", r, g, b);

  tag = gtk_text_buffer_create_tag(GTK_TEXT_BUFFER(buffer), name,
                                   "background-rgba", (GdkRGBA *)color,
                                   "background-set", TRUE, NULL);

  buffer->preedit_bg_color_tags =
      g_list_prepend(buffer->preedit_bg_color_tags, tag);
...
void gimp_text_buffer_set_insert_tags(GimpTextBuffer *buffer,
                                      GList *insert_tags, GList *remove_tags) {
  g_return_if_fail(GIMP_IS_TEXT_BUFFER(buffer));
...
gint gimp_text_buffer_get_iter_index(GimpTextBuffer *buffer, GtkTextIter *iter,
  ...
  string =
      gtk_text_buffer_get_text(GTK_TEXT_BUFFER(buffer), &start, iter, TRUE);
  index = strlen(string);
  g_free(string);
...
gboolean gimp_text_buffer_load(GimpTextBuffer *buffer, GFile *file,
                               GError **error) {
  GInputStream *input;
  gchar buf[2048];
  gint to_read;
...
gboolean gimp_text_buffer_save(GimpTextBuffer *buffer, GFile *file,
                               gboolean selection_only, GError **error) {
  ...
  if (selection_only)
    gtk_text_buffer_get_selection_bounds(GTK_TEXT_BUFFER(buffer), &start_iter,
                                         &end_iter);
  else
    gtk_text_buffer_get_bounds(GTK_TEXT_BUFFER(buffer), &start_iter, &end_iter);

  text_contents = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(buffer), &start_iter,
                                           &end_iter, TRUE);

  if (text_contents) {
    gint text_length = strlen(text_contents);

    if (!g_output_stream_write_all(output, text_contents, text_length, NULL,
                                   NULL, &my_error)) {
