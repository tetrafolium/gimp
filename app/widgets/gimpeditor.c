...
static void gimp_editor_set_property(GObject *object, guint property_id,
                                     const GValue *value, GParamSpec *pspec);
static void gimp_editor_get_property(GObject *object, guint property_id,
...
static void gimp_editor_set_show_button_bar(GimpDocked *docked, gboolean show);
...
G_DEFINE_TYPE_WITH_CODE(GimpEditor, gimp_editor, GTK_TYPE_BOX,
                        G_ADD_PRIVATE(GimpEditor) G_IMPLEMENT_INTERFACE(
...
static void gimp_editor_class_init(GimpEditorClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_MENU_FACTORY,
      g_param_spec_object("menu-factory", NULL, NULL, GIMP_TYPE_MENU_FACTORY,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_MENU_IDENTIFIER,
      g_param_spec_string("menu-identifier", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_UI_PATH,
      g_param_spec_string("ui-path", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_POPUP_DATA,
      g_param_spec_pointer("popup-data", NULL, NULL,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(object_class, PROP_SHOW_NAME,
                                  g_param_spec_boolean("show-name", NULL, NULL,
                                                       FALSE,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("name", NULL, NULL, NULL,
                          GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT));

  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_int("content-spacing", NULL, NULL, 0, G_MAXINT,
                       DEFAULT_CONTENT_SPACING, GIMP_PARAM_READABLE));

  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_int("button-spacing", NULL, NULL, 0, G_MAXINT,
                       DEFAULT_BUTTON_SPACING, GIMP_PARAM_READABLE));

  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_enum("button-icon-size", NULL, NULL, GTK_TYPE_ICON_SIZE,
                        DEFAULT_BUTTON_ICON_SIZE, GIMP_PARAM_READABLE));

  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_enum("button-relief", NULL, NULL, GTK_TYPE_RELIEF_STYLE,
                        DEFAULT_BUTTON_RELIEF, GIMP_PARAM_READABLE));
}
...
static void gimp_editor_init(GimpEditor *editor) {
  ...
  editor->priv->name_label =
      g_object_new(GTK_TYPE_LABEL, "xalign", 0.0, "yalign", 0.5, "ellipsize",
                   PANGO_ELLIPSIZE_END, NULL);
  gimp_label_set_attributes(GTK_LABEL(editor->priv->name_label),
                            PANGO_ATTR_STYLE, PANGO_STYLE_ITALIC, -1);
  gtk_box_pack_start(GTK_BOX(editor), editor->priv->name_label, FALSE, FALSE,
                     0);
}
...
static void gimp_editor_constructed(GObject *object) {
  ...
  if (editor->priv->menu_factory && editor->priv->menu_identifier) {
    editor->priv->ui_manager = gimp_menu_factory_manager_new(
        editor->priv->menu_factory, editor->priv->menu_identifier,
...
static void gimp_editor_set_property(GObject *object, guint property_id,
  ...
  case PROP_SHOW_NAME:
    g_object_set_property(G_OBJECT(editor->priv->name_label), "visible", value);
    break;
...
static void gimp_editor_get_property(GObject *object, guint property_id,
  ...
  case PROP_SHOW_NAME:
    g_object_get_property(G_OBJECT(editor->priv->name_label), "visible", value);
    break;
...
void gimp_editor_create_menu(GimpEditor *editor, GimpMenuFactory *menu_factory,
  ...
  editor->priv->ui_manager =
      gimp_menu_factory_manager_new(menu_factory, menu_identifier, popup_data);
...
gboolean gimp_editor_popup_menu(GimpEditor *editor,
  ...
  if (editor->priv->ui_manager && editor->priv->ui_path) {
    ...
    gimp_ui_manager_ui_popup(editor->priv->ui_manager, editor->priv->ui_path,
                             GTK_WIDGET(editor), position_func, position_data,
                             NULL, NULL);
    return TRUE;
  }
...
GtkWidget *gimp_editor_add_button(GimpEditor *editor, const gchar *icon_name,
  ...
  if (callback)
    g_signal_connect(button, "clicked", callback, callback_data);

  if (extended_callback)
    g_signal_connect(button, "extended-clicked", extended_callback,
                     callback_data);
...
GtkWidget *gimp_editor_add_icon_box(GimpEditor *editor, GType enum_type,
  ...
  hbox = gimp_enum_icon_box_new(enum_type, icon_prefix, button_icon_size,
                                callback, callback_data, NULL, &first_button);
  ...
  for (list = children; list; list = g_list_next(list)) {
    ...
    gtk_box_pack_start(GTK_BOX(editor->priv->button_box), button, TRUE, TRUE,
                       0);
...
static void gimp_editor_button_extended_actions_free(GList *actions) {
  GList *list;
...
static void gimp_editor_button_extended_clicked(GtkWidget *button,
...
GtkWidget *gimp_editor_add_action_button(GimpEditor *editor,
  ...
  group =
      gimp_ui_manager_get_action_group(editor->priv->ui_manager, group_name);
  ...
  while (action_name) {
    ...
    if (action && mod_mask) {
      ...
      if (tooltip) {
        ...
        if (ext_tooltip) {
          gchar *tmp =
              g_strconcat(tooltip, "\n<b>", gimp_get_mod_string(ext->mod_mask),
                          "</b>  ", ext_tooltip, NULL);
          g_free(tooltip);
          tooltip = tmp;
        }
...
void gimp_editor_set_box_style(GimpEditor *editor, GtkBox *box) {
  ...
  gtk_widget_style_get(GTK_WIDGET(editor), "button-icon-size",
                       &button_icon_size, "button-spacing", &button_spacing,
                       "button-relief", &button_relief, NULL);
