...
static gboolean gimp_file_overwrite_dialog(GtkWidget *parent, GFile *file);
...
void gimp_dnd_xds_source_set(GdkDragContext *context, GimpImage *image) {
  ...
  if (image) {
    ...
    gdk_property_change(gdk_drag_context_get_source_window(context), property,
                        type, 8, GDK_PROP_MODE_REPLACE,
                        (const guchar *)basename,
                        basename ? strlen(basename) : 0);

    g_free(basename);
  } else {
    gdk_property_delete(gdk_drag_context_get_source_window(context), property);
  }
}
...
void gimp_dnd_xds_save_image(GdkDragContext *context, GimpImage *image,
  ...
  if (!gdk_property_get(gdk_drag_context_get_source_window(context), property,
                        type, 0, MAX_URI_LEN, FALSE, NULL, NULL, &length,
                        &data))
    return;
  ...
  proc = gimp_plug_in_manager_file_procedure_find(
      image->gimp->plug_in_manager, GIMP_FILE_PROCEDURE_GROUP_SAVE, file, NULL);
  if (!proc) {
    proc = gimp_plug_in_manager_file_procedure_find(
  ...
  if (proc) {
    if (!g_file_query_exists(file, NULL) ||
      ...
      if (file_save(image->gimp, image, NULL, file, proc, GIMP_RUN_INTERACTIVE,
                    !export, FALSE, export, &error) == GIMP_PDB_SUCCESS) {
...
static gboolean gimp_file_overwrite_dialog(GtkWidget *parent, GFile *file) {
  ...
  dialog = gimp_message_dialog_new(
      _("File Exists"), GIMP_ICON_DIALOG_WARNING, parent,
      GTK_DIALOG_DESTROY_WITH_PARENT, gimp_standard_help_func, NULL,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Replace"), GTK_RESPONSE_OK,
  ...
  if (GTK_IS_DIALOG(parent))
    gtk_dialog_set_response_sensitive(GTK_DIALOG(parent), GTK_RESPONSE_CANCEL,
                                      FALSE);
