...
enum { FILE_DIALOG_SETUP, IMPORT, EXPORT, SELECTED, LAST_SIGNAL };
...
#define GET_PRIVATE(item)                                                      \
  ((GimpSettingsBoxPrivate *)gimp_settings_box_get_instance_private(           \
...
static void gimp_settings_box_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_settings_box_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);

static GtkWidget *gimp_settings_box_menu_item_add(GimpSettingsBox *box,
                                                  const gchar *label,
                                                  GCallback callback);
static gboolean gimp_settings_box_row_separator_func(GtkTreeModel *model,
                                                     GtkTreeIter *iter,
                                                     gpointer data);
...
static void gimp_settings_box_file_dialog(GimpSettingsBox *box,
                                          const gchar *title, gboolean save);
static void gimp_settings_box_file_response(GtkWidget *dialog, gint response_id,
                                            GimpSettingsBox *box);
...
static void gimp_settings_box_class_init(GimpSettingsBoxClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  settings_box_signals[FILE_DIALOG_SETUP] = g_signal_new(
      "file-dialog-setup", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpSettingsBoxClass, file_dialog_setup), NULL, NULL,
      gimp_marshal_VOID__OBJECT_BOOLEAN, G_TYPE_NONE, 2,
      GTK_TYPE_FILE_CHOOSER_DIALOG, G_TYPE_BOOLEAN);

  settings_box_signals[IMPORT] = g_signal_new(
      "import", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpSettingsBoxClass, import), NULL, NULL,
      gimp_marshal_BOOLEAN__OBJECT, G_TYPE_BOOLEAN, 1, G_TYPE_FILE);

  settings_box_signals[EXPORT] = g_signal_new(
      "export", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpSettingsBoxClass, export), NULL, NULL,
      gimp_marshal_BOOLEAN__OBJECT, G_TYPE_BOOLEAN, 1, G_TYPE_FILE);

  settings_box_signals[SELECTED] =
      g_signal_new("selected", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpSettingsBoxClass, selected), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, GIMP_TYPE_CONFIG);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("config", NULL, NULL, GIMP_TYPE_CONFIG,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_CONTAINER,
      g_param_spec_object("container", NULL, NULL, GIMP_TYPE_CONTAINER,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_HELP_ID,
      g_param_spec_string("help-id", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_IMPORT_TITLE,
      g_param_spec_string("import-title", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_EXPORT_TITLE,
      g_param_spec_string("export-title", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_DEFAULT_FOLDER,
      g_param_spec_object("default-folder", NULL, NULL, G_TYPE_FILE,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_LAST_FILE,
      g_param_spec_object("last-file", NULL, NULL, G_TYPE_FILE,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_settings_box_init(GimpSettingsBox *box) {
...
private
  ->combo = gimp_container_combo_box_new(
      private->container, gimp_get_user_context(private->gimp), 16, 0);
  ...
  g_signal_connect_after(private->combo, "select-item",
                         G_CALLBACK(gimp_settings_box_setting_selected), box);
  ...
  image = gtk_image_new_from_icon_name(GIMP_ICON_LIST_ADD, GTK_ICON_SIZE_MENU);
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_widget_show(image);

  gimp_help_set_help_data(button,
                          _("Save the current settings as named preset"), NULL);

  g_signal_connect(button, "clicked",
                   G_CALLBACK(gimp_settings_box_favorite_activate), box);
  ...
  arrow = gtk_image_new_from_icon_name(GIMP_ICON_MENU_LEFT, GTK_ICON_SIZE_MENU);
  ...
  g_signal_connect(button, "button-press-event",
                   G_CALLBACK(gimp_settings_box_menu_press), box);
...
private
  ->import_item = gimp_settings_box_menu_item_add(
      box, _("_Import Current Settings from File..."),
      G_CALLBACK(gimp_settings_box_import_activate));

private
  ->export_item = gimp_settings_box_menu_item_add(
      box, _("_Export Current Settings to File..."),
      G_CALLBACK(gimp_settings_box_export_activate));

  gimp_settings_box_menu_item_add(box, NULL, NULL);

  gimp_settings_box_menu_item_add(
      box, _("_Manage Saved Presets..."),
...
static void gimp_settings_box_set_property(GObject *object, guint property_id,
  ...
  case PROP_GIMP:
  private
    ->gimp = g_value_get_object(value); /* don't dup */
    break;
  ...
  private
    ->config = g_value_dup_object(value);
    break;
  ...
  private
    ->container = g_value_dup_object(value);
    if (private->combo)
  ...
  case PROP_HELP_ID:
    g_free(private->help_id);
  private
    ->help_id = g_value_dup_string(value);
    break;

  case PROP_IMPORT_TITLE:
    g_free(private->import_title);
  private
    ->import_title = g_value_dup_string(value);
    break;

  case PROP_EXPORT_TITLE:
    g_free(private->export_title);
  private
    ->export_title = g_value_dup_string(value);
    break;
  ...
  private
    ->default_folder = g_value_dup_object(value);
    break;
  ...
  private
    ->last_file = g_value_dup_object(value);
    break;
...
static GtkWidget *gimp_settings_box_menu_item_add(GimpSettingsBox *box,
  ...
  if (label) {
    ...
    g_signal_connect(item, "activate", callback, box);
  } else {
...
static gboolean gimp_settings_box_row_separator_func(GtkTreeModel *model,
  ...
  gtk_tree_model_get(model, iter, GIMP_CONTAINER_TREE_STORE_COLUMN_NAME, &name,
                     -1);
  g_free(name);
...
static void gimp_settings_box_setting_selected(GimpContainerView *view,
                                               ...
                                               GimpSettingsBox *box) {
  if (object)
    g_signal_emit(box, settings_box_signals[SELECTED], 0, object);
}
...
static gboolean gimp_settings_box_menu_press(GtkWidget *widget,
  ...
  if (bevent->type == GDK_BUTTON_PRESS) {
    gtk_menu_popup_at_widget(GTK_MENU(private->menu), widget, GDK_GRAVITY_WEST,
...
static void gimp_settings_box_favorite_activate(GtkWidget *widget,
  ...
  dialog = gimp_query_string_box(
      _("Save Settings as Named Preset"), toplevel, gimp_standard_help_func,
      NULL, _("Enter a name for the preset"), _("Saved Settings"),
      G_OBJECT(toplevel), "hide", gimp_settings_box_favorite_callback, box,
      NULL);
...
private
  ->editor_dialog =
      gimp_dialog_new(_("Manage Saved Presets"), "gimp-settings-editor-dialog",
                      toplevel, 0, NULL, NULL,
  ...
  editor = gimp_settings_editor_new(private->gimp, private->config,
                                    private->container);
  gtk_container_set_border_width(GTK_CONTAINER(editor), 12);

  content_area =
      gtk_dialog_get_content_area(GTK_DIALOG(private->editor_dialog));
...
private
  ->file_dialog = dialog = gtk_file_chooser_dialog_new(
      title, GTK_WINDOW(toplevel),
      save ? GTK_FILE_CHOOSER_ACTION_SAVE : GTK_FILE_CHOOSER_ACTION_OPEN,

      _("_Cancel"), GTK_RESPONSE_CANCEL, save ? _("_Save") : _("_Open"),
      GTK_RESPONSE_OK,
  ...
  g_signal_connect_object(toplevel, "unmap",
                          G_CALLBACK(gimp_settings_box_toplevel_unmap), dialog,
                          0);
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(gimp_settings_box_file_response), box);
  g_signal_connect(dialog, "delete-event", G_CALLBACK(gtk_true), NULL);

  if (private->default_folder &&
      g_file_query_file_type(private->default_folder, G_FILE_QUERY_INFO_NONE,
                             NULL) == G_FILE_TYPE_DIRECTORY) {
    gchar *uri = g_file_get_uri(private->default_folder);
    gtk_file_chooser_add_shortcut_folder_uri(GTK_FILE_CHOOSER(dialog), uri,
                                             NULL);
    g_free(uri);
  ...
  if (private->last_file)
    gtk_file_chooser_set_file(GTK_FILE_CHOOSER(dialog), private->last_file,
                              NULL);
...
static void gimp_settings_box_file_response(GtkWidget *dialog, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    if (save)
      g_signal_emit(box, settings_box_signals[EXPORT], 0, file, &success);
    else
      g_signal_emit(box, settings_box_signals[IMPORT], 0, file, &success);
...
static void gimp_settings_box_truncate_list(GimpSettingsBox *box,
  ...
  while (list) {
    ...
    g_object_get(config, "time", &t, NULL);
...
GtkWidget *gimp_settings_box_new(Gimp *gimp, GObject *config,
                                 ...
                                 const gchar *help_id, GFile *default_folder,
                                 GFile *last_file) {
  ...
  return g_object_new(GIMP_TYPE_SETTINGS_BOX, "gimp", gimp, "config", config,
                      "container", container, "help-id", help_id,
                      "import-title", import_title, "export-title",
                      export_title, "default-folder", default_folder,
                      "last-file", last_file, NULL);
}
...
private
  ...
  for (list = GIMP_LIST(private->container)->queue->head; list;
       list = g_list_next(list)) {
    gint64 t;
    ...
    if (t > 0 &&
        gimp_config_is_equal_to(config, GIMP_CONFIG(private->config))) {
      GDateTime *now = g_date_time_new_now_utc();

      g_object_set(config, "time", g_date_time_to_unix(now), NULL);
