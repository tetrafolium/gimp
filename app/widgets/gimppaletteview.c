...
static gboolean gimp_palette_view_draw(GtkWidget *widget, cairo_t *cr);
...
static GimpPaletteEntry *gimp_palette_view_find_entry(GimpPaletteView *view,
                                                      gint x, gint y);
...
static void gimp_palette_view_drag_color(GtkWidget *widget, GimpRGB *color,
                                         gpointer data);
static void gimp_palette_view_drop_color(GtkWidget *widget, gint x, gint y,
...
static void gimp_palette_view_class_init(GimpPaletteViewClass *klass) {
  ...
  view_signals[ENTRY_CLICKED] = g_signal_new(
      "entry-clicked", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpPaletteViewClass, entry_clicked), NULL, NULL,
      gimp_marshal_VOID__POINTER_ENUM, G_TYPE_NONE, 2, G_TYPE_POINTER,
      GDK_TYPE_MODIFIER_TYPE);

  view_signals[ENTRY_SELECTED] = g_signal_new(
      "entry-selected", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpPaletteViewClass, entry_selected), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_POINTER);

  view_signals[ENTRY_ACTIVATED] = g_signal_new(
      "entry-activated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpPaletteViewClass, entry_activated), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_POINTER);

  view_signals[ENTRY_CONTEXT] = g_signal_new(
      "entry-context", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpPaletteViewClass, entry_context), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_POINTER);

  view_signals[COLOR_DROPPED] = g_signal_new(
      "color-dropped", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpPaletteViewClass, color_dropped), NULL, NULL,
      gimp_marshal_VOID__POINTER_BOXED, G_TYPE_NONE, 2, G_TYPE_POINTER,
      GIMP_TYPE_RGB);
...
static gboolean gimp_palette_view_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (view->renderer->viewable && pal_view->selected) {
    ...
    cairo_rectangle(cr, col * renderer->cell_width + 0.5,
                    row * renderer->cell_height + 0.5, renderer->cell_width,
...
static gboolean gimp_palette_view_button_press(GtkWidget *widget,
  ...
  if (bevent->type == GDK_BUTTON_PRESS)
    g_signal_emit(view, view_signals[ENTRY_CLICKED], 0, entry, bevent->state);
...
static gboolean gimp_palette_view_key_press(GtkWidget *widget,
  ...
  if (view->selected &&
      (kevent->keyval == GDK_KEY_space || kevent->keyval == GDK_KEY_KP_Space ||
       kevent->keyval == GDK_KEY_Return || kevent->keyval == GDK_KEY_KP_Enter ||
       kevent->keyval == GDK_KEY_ISO_Enter)) {
    g_signal_emit(view, view_signals[ENTRY_CLICKED], 0, view->selected,
                  kevent->state);
  }
...
static gboolean gimp_palette_view_focus(GtkWidget *widget,
  ...
  if (gtk_widget_get_can_focus(widget) && !gtk_widget_has_focus(widget)) {
    ...
    if (!view->selected && palette && gimp_palette_get_n_colors(palette) > 0) {
...
static void gimp_palette_view_set_viewable(GimpView *view,
  ...
  if (old_viewable) {
    g_signal_handlers_disconnect_by_func(old_viewable,
                                         gimp_palette_view_invalidate, view);
  ...
  GIMP_VIEW_CLASS(parent_class)->set_viewable(view, old_viewable, new_viewable);

  if (new_viewable) {
    g_signal_connect(new_viewable, "invalidate-preview",
                     G_CALLBACK(gimp_palette_view_invalidate), view);

    /*  unset the palette drag handler set by GimpView  */
    gimp_dnd_viewable_source_remove(GTK_WIDGET(view), GIMP_TYPE_PALETTE);

    if (!old_viewable) {
      gimp_dnd_color_source_add(GTK_WIDGET(view), gimp_palette_view_drag_color,
                                view);
      gimp_dnd_color_dest_add(GTK_WIDGET(view), gimp_palette_view_drop_color,
                              view);
    }
  }
}
...
static GimpPaletteEntry *gimp_palette_view_find_entry(GimpPaletteView *view,
  ...
  if (col >= 0 && col < renderer->columns && row >= 0 && row < renderer->rows) {
    entry = gimp_palette_get_entry(palette, row * renderer->columns + col);
  }
...
static void gimp_palette_view_expose_entry(GimpPaletteView *view,
  ...
  gtk_widget_queue_draw_area(
      GTK_WIDGET(view), allocation.x + col * renderer->cell_width,
      allocation.y + row * renderer->cell_height, renderer->cell_width + 1,
...
static void gimp_palette_view_drop_color(GtkWidget *widget, gint x, gint y,
  ...
  g_signal_emit(view, view_signals[COLOR_DROPPED], 0, entry, color);
}
