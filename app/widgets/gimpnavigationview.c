...
enum { MARKER_CHANGED, ZOOM, SCROLL, LAST_SIGNAL };
...
static gboolean gimp_navigation_view_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_navigation_view_draw_marker(GimpNavigationView *nav_view,
                                             cairo_t *cr);
static void gimp_navigation_view_move_to(GimpNavigationView *nav_view, gint tx,
                                         gint ty);
static void gimp_navigation_view_get_ratio(GimpNavigationView *nav_view,
                                           gdouble *ratiox, gdouble *ratioy);
static gboolean
gimp_navigation_view_point_in_marker(GimpNavigationView *nav_view, gint x,
                                     gint y);
...
static void gimp_navigation_view_class_init(GimpNavigationViewClass *klass) {
  ...
  view_signals[MARKER_CHANGED] = g_signal_new(
      "marker-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpNavigationViewClass, marker_changed), NULL, NULL,
      gimp_marshal_VOID__DOUBLE_DOUBLE_DOUBLE_DOUBLE, G_TYPE_NONE, 4,
      G_TYPE_DOUBLE, G_TYPE_DOUBLE, G_TYPE_DOUBLE, G_TYPE_DOUBLE);

  view_signals[ZOOM] =
      g_signal_new("zoom", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpNavigationViewClass, zoom), NULL, NULL,
                   gimp_marshal_VOID__ENUM_DOUBLE, G_TYPE_NONE, 2,
                   GIMP_TYPE_ZOOM_TYPE, G_TYPE_DOUBLE);

  view_signals[SCROLL] =
      g_signal_new("scroll", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpNavigationViewClass, scroll), NULL, NULL,
...
void gimp_navigation_view_grab_pointer(GimpNavigationView *nav_view,
                                       GdkEvent *event) {
  ...
  gdk_seat_grab(gdk_event_get_seat(event), window, GDK_SEAT_CAPABILITY_ALL,
...
static gboolean gimp_navigation_view_motion_notify(GtkWidget *widget,
  ...
  if (!nav_view->has_grab) {
    ...
    if (nav_view->p_center_x == view->renderer->width / 2 &&
      ...
      return FALSE;
    } else if (gimp_navigation_view_point_in_marker(nav_view, mevent->x,
                                                    mevent->y)) {
      cursor = gdk_cursor_new_for_display(display, GDK_FLEUR);
    } else {
  ...
  gimp_navigation_view_move_to(nav_view, mevent->x - nav_view->motion_offset_x,
...
static gboolean gimp_navigation_view_key_press(GtkWidget *widget,
  ...
  if (scroll_x || scroll_y) {
    gimp_navigation_view_move_to(nav_view, nav_view->p_center_x + scroll_x,
...
void gimp_navigation_view_set_marker(GimpNavigationView *nav_view,
...
void gimp_navigation_view_set_canvas(GimpNavigationView *nav_view,
                                     gboolean visible, gdouble x, gdouble y,
                                     gdouble width, gdouble height) {
  GimpView *view;
...
void gimp_navigation_view_set_motion_offset(GimpNavigationView *view,
                                            gint motion_offset_x,
                                            gint motion_offset_y) {
...
void gimp_navigation_view_get_local_marker(GimpNavigationView *view,
                                           gint *center_x, gint *center_y,
                                           gint *width, gint *height) {
  g_return_if_fail(GIMP_IS_NAVIGATION_VIEW(view));

  if (center_x)
    *center_x = view->p_center_x;
  if (center_y)
    *center_y = view->p_center_y;
  if (width)
    *width = view->p_width;
  if (height)
    *height = view->p_height;
}
...
static void gimp_navigation_view_draw_marker(GimpNavigationView *nav_view,
  ...
  if (view->renderer->viewable && nav_view->width && nav_view->height) {
    ...
    cairo_rectangle(cr, -p_width_2, -p_height_2, nav_view->p_width,
                    nav_view->p_height);
    ...
    if (nav_view->canvas_visible && nav_view->canvas_width &&
        nav_view->canvas_height) {
      cairo_save(cr);
      cairo_set_matrix(cr, &matrix);
      cairo_rectangle(
          cr, nav_view->p_canvas_x + 0.5, nav_view->p_canvas_y + 0.5,
          nav_view->p_canvas_width - 1.0, nav_view->p_canvas_height - 1.0);
...
static void gimp_navigation_view_move_to(GimpNavigationView *nav_view, gint tx,
  ...
  g_signal_emit(view, view_signals[MARKER_CHANGED], 0, x, y, nav_view->width,
                nav_view->height);
}
...
gimp_navigation_view_point_in_marker(GimpNavigationView *nav_view, gint x,
  ...
  return tx >= -p_width_2 && tx < p_width_2 && ty >= -p_height_2 &&
         ty < p_height_2;
}
