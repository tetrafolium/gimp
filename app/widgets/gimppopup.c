...
enum { CANCEL, CONFIRM, LAST_SIGNAL };

static gboolean gimp_popup_map_event(GtkWidget *widget, GdkEventAny *event);
static gboolean gimp_popup_button_press(GtkWidget *widget,
                                        GdkEventButton *bevent);
static gboolean gimp_popup_key_press(GtkWidget *widget, GdkEventKey *kevent);
...
static void gimp_popup_class_init(GimpPopupClass *klass) {
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);
  GtkBindingSet *binding_set;

  popup_signals[CANCEL] = g_signal_new("cancel", G_OBJECT_CLASS_TYPE(klass),
                                       G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                                       G_STRUCT_OFFSET(GimpPopupClass, cancel),
                                       NULL, NULL, NULL, G_TYPE_NONE, 0);

  popup_signals[CONFIRM] =
      g_signal_new("confirm", G_OBJECT_CLASS_TYPE(klass),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(GimpPopupClass, confirm), NULL, NULL, NULL,
                   G_TYPE_NONE, 0);
  ...
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Return, 0, "confirm", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Enter, 0, "confirm", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_ISO_Enter, 0, "confirm", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_space, 0, "confirm", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Space, 0, "confirm", 0);
}

static void gimp_popup_init(GimpPopup *popup) {}

static void gimp_popup_grab_notify(GtkWidget *widget, gboolean was_grabbed) {
  if (was_grabbed)
    return;
...
static gboolean gimp_popup_grab_broken_event(GtkWidget *widget,
...
static gboolean gimp_popup_map_event(GtkWidget *widget, GdkEventAny *event) {
  ...
  if (gdk_seat_grab(gdk_display_get_default_seat(display),
                    ...
                    TRUE, NULL, (GdkEvent *)event, NULL,
                    NULL) == GDK_GRAB_SUCCESS) {
    gtk_grab_add(widget);

    g_signal_connect(widget, "grab-notify", G_CALLBACK(gimp_popup_grab_notify),
                     widget);
    g_signal_connect(widget, "grab-broken-event",
                     G_CALLBACK(gimp_popup_grab_broken_event), widget);
...
static gboolean gimp_popup_button_press(GtkWidget *widget,
  ...
  if (event_widget == widget) {
     ...
     */
    if (bevent->x < 0 || bevent->y < 0 || bevent->x > allocation.width ||
...
static gboolean gimp_popup_key_press(GtkWidget *widget, GdkEventKey *kevent) {
  ...
  if (activate_binding) {
     ...
     *  it
     */
    if (gtk_binding_set_activate(binding_set, kevent->keyval, kevent->state,
                                 G_OBJECT(widget))) {
      return TRUE;
    }
  }
