...
enum { PROP_0, PROP_ELLIPSIZE = GIMP_CONTAINER_VIEW_PROP_LAST + 1 };
...
G_DEFINE_TYPE_WITH_CODE(
    GimpContainerComboBox, gimp_container_combo_box, GTK_TYPE_COMBO_BOX,
...
static void gimp_container_combo_box_init(GimpContainerComboBox *combo) {
  GtkTreeModel *model;
  ...
  model =
      gimp_container_tree_store_new(GIMP_CONTAINER_VIEW(combo), n_types, types);
  ...
  gtk_cell_layout_set_attributes(layout, cell, "renderer",
  ...
  gtk_cell_layout_set_attributes(layout, cell, "text",
                                 GIMP_CONTAINER_TREE_STORE_COLUMN_NAME, NULL);

  combo->text_renderer = cell;

  g_signal_connect(combo, "changed",
                   G_CALLBACK(gimp_container_combo_box_changed), combo);
...
static void gimp_container_combo_box_set_property(GObject *object,
  ...
  switch (property_id) {
  case PROP_ELLIPSIZE:
    g_object_set_property(G_OBJECT(combo->text_renderer), pspec->name, value);
    break;
...
static void gimp_container_combo_box_get_property(GObject *object,
  ...
  switch (property_id) {
  case PROP_ELLIPSIZE:
    g_object_get_property(G_OBJECT(combo->text_renderer), pspec->name, value);
    break;
...
gimp_container_combo_box_insert_item(GimpContainerView *view,
  ...
  if (model) {
    ...
    iter = gimp_container_tree_store_insert_item(
        GIMP_CONTAINER_TREE_STORE(model), viewable, parent_insert_data, index);
...
static gboolean gimp_container_combo_box_select_item(GimpContainerView *view,
  ...
  if (gtk_combo_box_get_model(GTK_COMBO_BOX(view))) {
    ...
    g_signal_handlers_block_by_func(combo_box, gimp_container_combo_box_changed,
                                    view);
    ...
    g_signal_handlers_unblock_by_func(combo_box,
                                      gimp_container_combo_box_changed, view);
  }
...
static void gimp_container_combo_box_changed(GtkComboBox *combo,
                                             GimpContainerView *view) {
  GtkTreeIter iter;
