...
enum { PROP_0, PROP_IMAGE, PROP_PARASITE };
...
gimp_image_parasite_view_class_init(GimpImageParasiteViewClass *klass) {
  ...
  view_signals[UPDATE] =
      g_signal_new("update", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpImageParasiteViewClass, update), NULL,
  ...
  g_object_class_install_property(
      object_class, PROP_PARASITE,
      g_param_spec_string("parasite", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_image_parasite_view_init(GimpImageParasiteView *view) {
...
GtkWidget *gimp_image_parasite_view_new(GimpImage *image,
  ...
  return g_object_new(GIMP_TYPE_IMAGE_PARASITE_VIEW, "image", image, "parasite",
                      parasite, NULL);
}
