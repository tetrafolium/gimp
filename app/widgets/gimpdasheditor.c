...
enum { PROP_0, PROP_STROKE_OPTIONS, PROP_N_SEGMENTS, PROP_LENGTH };

static void gimp_dash_editor_finalize(GObject *object);
static void gimp_dash_editor_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_dash_editor_get_property(GObject *object, guint property_id,
...
static void gimp_dash_editor_get_preferred_height(GtkWidget *widget,
                                                  gint *minimum_height,
                                                  gint *natural_height);
static gboolean gimp_dash_editor_draw(GtkWidget *widget, cairo_t *cr);
...
static gint dash_x_to_index(GimpDashEditor *editor, gint x);
...
static void gimp_dash_editor_class_init(GimpDashEditorClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_STROKE_OPTIONS,
      g_param_spec_object("stroke-options", NULL, NULL,
                          GIMP_TYPE_STROKE_OPTIONS,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_N_SEGMENTS,
      g_param_spec_int("n-segments", NULL, NULL, 2, 120, DEFAULT_N_SEGMENTS,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_LENGTH,
      g_param_spec_double("dash-length", NULL, NULL, 0.0, 2000.0,
...
static void gimp_dash_editor_init(GimpDashEditor *editor) {
  ...
  gtk_widget_add_events(GTK_WIDGET(editor), GDK_BUTTON_PRESS_MASK |
...
static void gimp_dash_editor_set_property(GObject *object, guint property_id,
  ...
  case PROP_STROKE_OPTIONS:
    ...
    g_signal_connect_object(editor->stroke_options, "notify::dash-info",
                            G_CALLBACK(update_segments_from_options), editor,
                            G_CONNECT_SWAPPED);
    break;
...
static void gimp_dash_editor_get_preferred_width(GtkWidget *widget,
  ...
  *minimum_width = *natural_width =
      MAX(editor->block_width * editor->n_segments + 20, MIN_WIDTH);
}
...
static void gimp_dash_editor_get_preferred_height(GtkWidget *widget,
  ...
  *minimum_height = *natural_height =
      MAX(editor->block_height + 10, MIN_HEIGHT);
}
...
static gboolean gimp_dash_editor_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  gtk_render_background(style, cr, allocation.x, allocation.y, allocation.width,
                        allocation.height);
...
GtkWidget *gimp_dash_editor_new(GimpStrokeOptions *stroke_options) {
  ...
  return g_object_new(GIMP_TYPE_DASH_EDITOR, "stroke-options", stroke_options,
                      NULL);
}
...
static void update_segments_from_options(GimpDashEditor *editor) {
  ...
  gimp_dash_pattern_fill_segments(dash_info, editor->segments,
                                  editor->n_segments);
}

static void update_options_from_segments(GimpDashEditor *editor) {
  GArray *pattern = gimp_dash_pattern_new_from_segments(
      editor->segments, editor->n_segments, editor->dash_length);
...
static void update_blocksize(GimpDashEditor *editor) {
  ...
  editor->block_width = MAX(
      ROUND(editor->dash_length / editor->n_segments * editor->block_height),
      4);
