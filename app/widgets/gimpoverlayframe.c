...
static gboolean gimp_overlay_frame_draw(GtkWidget *widget, cairo_t *cr);

G_DEFINE_TYPE(GimpOverlayFrame, gimp_overlay_frame, GTK_TYPE_BIN)

#define parent_class gimp_overlay_frame_parent_class

static void gimp_overlay_frame_class_init(GimpOverlayFrameClass *klass) {
...
static void gimp_overlay_frame_init(GimpOverlayFrame *frame) {
...
static gboolean gimp_overlay_frame_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (rgba) {
    gimp_cairo_rounded_rectangle(cr, 0.0, 0.0, allocation.width,
                                 allocation.height, border_radius);
  } else {
  ...
  gtk_render_frame(style, cr, 0, 0, allocation.width, allocation.height);
