...
static gboolean parse_iso_codes(GHashTable *base_lang_list, GError **error);
...
void gimp_language_store_parser_init(void) {
  ...
  l10n_lang_list = g_hash_table_new_full(
      g_str_hash, g_str_equal, (GDestroyNotify)g_free, (GDestroyNotify)g_free);
  all_lang_list = g_hash_table_new_full(
      g_str_hash, g_str_equal, (GDestroyNotify)g_free, (GDestroyNotify)g_free);
  base_lang_list = g_hash_table_new_full(
      g_str_hash, g_str_equal, (GDestroyNotify)g_free, (GDestroyNotify)g_free);
  ...
  if (locales_dir) {
    ...
    while ((locale = g_dir_read_name(locales_dir)) != NULL) {
      gchar *filename =
          g_build_filename(gimp_locale_directory(), locale, "LC_MESSAGES",
  ...
  while (g_hash_table_iter_next(&lang_iter, &key, NULL)) {
    ...
    if (english_name) {
      ...
      if (g_strcmp0(english_name, localized_name) == 0 &&
          g_strcmp0(base_code, "en") != 0 && g_strcmp0(code, base_code) != 0) {
    ...
    g_hash_table_replace(
        l10n_lang_list, g_strdup(code),
        g_strdup_printf("%s [%s]", localized_name ? localized_name : "???",
                        code));
    g_free(localized_name);
    g_free(base_code);
  }
...
static gboolean parse_iso_codes(GHashTable *base_lang_list, GError **error) {
  ...
  static const GMarkupParser markup_parser = {
      iso_codes_parser_start_element, iso_codes_parser_end_element,
      NULL, /*  characters   */
      NULL, /*  passthrough  */
      NULL  /*  error        */
  };
...
#else
  file =
      g_file_new_for_path(ISO_CODES_LOCATION G_DIR_SEPARATOR_S "iso_639.xml");
#endif
...
static void iso_codes_parser_entry(IsoCodesParser *parser, const gchar **names,
                                   const gchar **values) {
  ...
  if (lang && *lang && code && *code) {
    ...
    if (g_hash_table_contains(parser->base_lang_list, code))
      g_hash_table_replace(parser->base_lang_list, g_strdup(code),
                           g_strdup(lang));
