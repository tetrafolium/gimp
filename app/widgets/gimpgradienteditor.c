...
#define GRAD_MOVE_TIME                                                         \
  150 /* ms between mouse click and detection of movement in gradient control  \
       */

#define GRAD_VIEW_EVENT_MASK                                                   \
  (GDK_EXPOSURE_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_POINTER_MOTION_MASK |       \
   GDK_POINTER_MOTION_HINT_MASK | GDK_BUTTON_PRESS_MASK |                      \
   GDK_BUTTON_RELEASE_MASK | GDK_SCROLL_MASK | GDK_SMOOTH_SCROLL_MASK)

#define GRAD_CONTROL_EVENT_MASK                                                \
  (GDK_EXPOSURE_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_POINTER_MOTION_MASK |       \
   GDK_POINTER_MOTION_HINT_MASK | GDK_BUTTON_PRESS_MASK |                      \
   GDK_BUTTON_RELEASE_MASK | GDK_SCROLL_MASK | GDK_SMOOTH_SCROLL_MASK |        \
...
static void gradient_editor_drop_gradient(GtkWidget *widget, gint x, gint y,
                                          GimpViewable *viewable,
                                          gpointer data);
static void gradient_editor_drop_color(GtkWidget *widget, gint x, gint y,
                                       const GimpRGB *color, gpointer data);
static void gradient_editor_control_drop_color(GtkWidget *widget, gint x,
                                               gint y, const GimpRGB *color,
                                               gpointer data);
...
static gboolean view_events(GtkWidget *widget, GdkEvent *event,
                            GimpGradientEditor *editor);
static void view_set_hint(GimpGradientEditor *editor, gint x);

static void view_pick_color(GimpGradientEditor *editor,
                            GimpColorPickTarget pick_target,
                            GimpColorPickState pick_state, gint x);

/* Gradient control functions */

static gboolean control_events(GtkWidget *widget, GdkEvent *event,
                               GimpGradientEditor *editor);
static gboolean control_draw(GtkWidget *widget, cairo_t *cr,
                             GimpGradientEditor *editor);
static void control_do_hint(GimpGradientEditor *editor, gint x, gint y);
static void control_button_press(GimpGradientEditor *editor, gint x, gint y,
                                 guint button, guint state);
static gboolean control_point_in_handle(GimpGradientEditor *editor,
                                        GimpGradient *gradient, gint x, gint y,
...
static void control_extend_selection(GimpGradientEditor *editor,
                                     GimpGradientSegment *seg, gdouble pos);
static void control_motion(GimpGradientEditor *editor, GimpGradient *gradient,
                           gint x);

static void control_compress_left(GimpGradient *gradient,
                                  GimpGradientSegment *range_l,
                                  GimpGradientSegment *range_r,
                                  GimpGradientSegment *drag_seg, gdouble pos);

static double control_move(GimpGradientEditor *editor,
                           GimpGradientSegment *range_l,
                           GimpGradientSegment *range_r, gdouble delta);

/* Control update/redraw functions */

static void control_update(GimpGradientEditor *editor, GimpGradient *gradient,
                           gboolean recalculate);
static void control_draw_all(GimpGradientEditor *editor, GimpGradient *gradient,
                             cairo_t *cr, gint width, gint height, gdouble left,
                             gdouble right);
static void control_draw_handle(GimpGradientEditor *editor,
                                GtkStyleContext *style, cairo_t *cr,
                                gdouble pos, gint height, gboolean middle,
                                GtkStateFlags flags);

static gint control_calc_p_pos(GimpGradientEditor *editor, gdouble pos);
static gdouble control_calc_g_pos(GimpGradientEditor *editor, gint pos);

/* Segment functions */

static void seg_get_closest_handle(GimpGradient *grad, gdouble pos,
                                   GimpGradientSegment **seg,
                                   GradientEditorDragMode *handle);
static gboolean seg_in_selection(GimpGradient *grad, GimpGradientSegment *seg,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpGradientEditor, gimp_gradient_editor, GIMP_TYPE_DATA_EDITOR,
...
static void gimp_gradient_editor_docked_iface_init(GimpDockedInterface *iface) {
...
static void gimp_gradient_editor_init(GimpGradientEditor *editor) {
  ...
  data_editor->view = gimp_view_new_full_by_types(
      NULL, GIMP_TYPE_VIEW, GIMP_TYPE_GRADIENT, GRAD_VIEW_SIZE, GRAD_VIEW_SIZE,
      0, FALSE, FALSE, FALSE);
  ...
  g_signal_connect(data_editor->view, "event", G_CALLBACK(view_events), editor);

  gimp_dnd_viewable_dest_add(GTK_WIDGET(data_editor->view), GIMP_TYPE_GRADIENT,
                             gradient_editor_drop_gradient, editor);

  gimp_dnd_color_dest_add(GTK_WIDGET(data_editor->view),
                          gradient_editor_drop_color, editor);
  ...
  g_signal_connect(editor->control, "event", G_CALLBACK(control_events),
                   editor);

  g_signal_connect(editor->control, "draw", G_CALLBACK(control_draw), editor);

  gimp_dnd_color_dest_add(GTK_WIDGET(editor->control),
                          gradient_editor_control_drop_color, editor);

  /*  Scrollbar  */
  editor->zoom_factor = 1;

  editor->scroll_data = gtk_adjustment_new(
      0.0, 0.0, 1.0, GRAD_SCROLLBAR_STEP_SIZE, GRAD_SCROLLBAR_PAGE_SIZE, 1.0);

  g_signal_connect(editor->scroll_data, "value-changed",
                   G_CALLBACK(gradient_editor_scrollbar_update), editor);
  g_signal_connect(editor->scroll_data, "changed",
                   G_CALLBACK(gradient_editor_scrollbar_update), editor);

  editor->scrollbar =
      gtk_scrollbar_new(GTK_ORIENTATION_HORIZONTAL, editor->scroll_data);
  ...
  editor->current_color =
      gimp_color_area_new(&transp, GIMP_COLOR_AREA_SMALL_CHECKS,
                          GDK_BUTTON1_MASK | GDK_BUTTON2_MASK);
  gtk_container_add(GTK_CONTAINER(frame), editor->current_color);
  gtk_widget_set_size_request(editor->current_color, GRAD_CURRENT_COLOR_WIDTH,
                              -1);
  ...
  gimp_rgba_set(&editor->saved_colors[3], 0.0, 0.0, 0.0,
...
static void gimp_gradient_editor_dispose(GObject *object) {
  ...
  if (editor->color_dialog)
    gtk_dialog_response(GTK_DIALOG(editor->color_dialog), GTK_RESPONSE_CANCEL);
...
static void gimp_gradient_editor_set_data(GimpDataEditor *editor,
  ...
  if (old_data)
    g_signal_handlers_disconnect_by_func(
        old_data, gimp_gradient_editor_gradient_dirty, gradient_editor);
...
GtkWidget *gimp_gradient_editor_new(GimpContext *context,
  ...
  return g_object_new(GIMP_TYPE_GRADIENT_EDITOR, "menu-factory", menu_factory,
                      "menu-identifier", "<GradientEditor>", "ui-path",
                      "/gradient-editor-popup", "data-factory",
                      context->gimp->gradient_factory, "context", context,
                      "data", gimp_context_get_gradient(context), NULL);
}
...
void gimp_gradient_editor_edit_left_color(GimpGradientEditor *editor) {
  ...
  if (!gradient || !editor->control_sel_l ||
  ...
  editor->color_dialog = gimp_color_dialog_new(
      GIMP_VIEWABLE(gradient), GIMP_DATA_EDITOR(editor)->context, TRUE,
      _("Left Endpoint Color"), GIMP_ICON_GRADIENT,
      _("Gradient Segment's Left Endpoint Color"), GTK_WIDGET(editor),
      gimp_dialog_factory_get_singleton(), "gimp-gradient-editor-color-dialog",
      &editor->control_sel_l->left_color, TRUE, TRUE);

  g_signal_connect(editor->color_dialog, "destroy",
                   G_CALLBACK(gtk_widget_destroyed), &editor->color_dialog);

  g_signal_connect(editor->color_dialog, "update",
                   G_CALLBACK(gradient_editor_left_color_update), editor);
...
void gimp_gradient_editor_edit_right_color(GimpGradientEditor *editor) {
  ...
  if (!gradient || !editor->control_sel_r ||
  ...
  editor->color_dialog = gimp_color_dialog_new(
      GIMP_VIEWABLE(gradient), GIMP_DATA_EDITOR(editor)->context, TRUE,
      _("Right Endpoint Color"), GIMP_ICON_GRADIENT,
      _("Gradient Segment's Right Endpoint Color"), GTK_WIDGET(editor),
      gimp_dialog_factory_get_singleton(), "gimp-gradient-editor-color-dialog",
      &editor->control_sel_l->right_color, TRUE, TRUE);

  g_signal_connect(editor->color_dialog, "destroy",
                   G_CALLBACK(gtk_widget_destroyed), &editor->color_dialog);

  g_signal_connect(editor->color_dialog, "update",
                   G_CALLBACK(gradient_editor_right_color_update), editor);
...
void gimp_gradient_editor_zoom(GimpGradientEditor *editor,
                               GimpZoomType zoom_type, gdouble delta) {
  ...
  gtk_adjustment_configure(adjustment, value,
                           gtk_adjustment_get_lower(adjustment),
                           gtk_adjustment_get_upper(adjustment),
                           page_size * GRAD_SCROLLBAR_STEP_SIZE,
                           page_size * GRAD_SCROLLBAR_PAGE_SIZE, page_size);
}
...
static void gradient_editor_drop_color(GtkWidget *widget, gint x, gint y,
  ...
  gimp_gradient_segment_split_midpoint(gradient,
                                       GIMP_DATA_EDITOR(editor)->context, seg,
                                       editor->blend_color_space, &lseg, &rseg);
...
static void gradient_editor_scrollbar_update(GtkAdjustment *adjustment,
  ...
  str1 = g_strdup_printf(_("Zoom factor: %d:1"), editor->zoom_factor);
  ...
  renderer =
      GIMP_VIEW_RENDERER_GRADIENT(GIMP_VIEW(data_editor->view)->renderer);

  gimp_view_renderer_gradient_set_offsets(
      renderer, gtk_adjustment_get_value(adjustment),
      gtk_adjustment_get_value(adjustment) +
          gtk_adjustment_get_page_size(adjustment));
...
static void gradient_editor_left_color_update(GimpColorDialog *dialog,
  ...
  case GIMP_COLOR_DIALOG_UPDATE:
    gimp_gradient_segment_range_blend(
        gradient, editor->control_sel_l, editor->control_sel_r, color,
        &editor->control_sel_r->right_color, TRUE, TRUE);
    break;
...
static void gradient_editor_right_color_update(GimpColorDialog *dialog,
  ...
  case GIMP_COLOR_DIALOG_UPDATE:
    gimp_gradient_segment_range_blend(
        ...
        &editor->control_sel_l->left_color, color, TRUE, TRUE);
    break;
...
static gboolean view_events(GtkWidget *widget, GdkEvent *event,
    ...
    if (mevent->x != editor->view_last_x) {
      ...
      if (editor->view_button_down) {
        view_pick_color(editor,
                        ...
                        GIMP_COLOR_PICK_STATE_UPDATE, mevent->x);
      } else {
    ...
    if (gdk_event_triggers_context_menu((GdkEvent *)bevent)) {
      ...
      view_pick_color(editor,
                      (bevent->state & gimp_get_toggle_behavior_mask())
                          ? GIMP_COLOR_PICK_TARGET_BACKGROUND
                          : GIMP_COLOR_PICK_TARGET_FOREGROUND,
                      GIMP_COLOR_PICK_STATE_START, bevent->x);
    }
  } break;
  ...
  case GDK_SCROLL: {
    ...
    if (sevent->state & gimp_get_toggle_behavior_mask()) {
      ...
      gimp_scroll_adjustment_values(sevent, editor->scroll_data, NULL, &value,
                                    NULL);
    ...
    if (editor->view_button_down) {
      ...
      view_pick_color(editor,
                      (bevent->state & gimp_get_toggle_behavior_mask())
                          ? GIMP_COLOR_PICK_TARGET_BACKGROUND
                          : GIMP_COLOR_PICK_TARGET_FOREGROUND,
                      GIMP_COLOR_PICK_STATE_END, bevent->x);
      break;
    }
    break;
...
static void view_set_hint(GimpGradientEditor *editor, gint x) {
  ...
  gimp_gradient_get_color_at(GIMP_GRADIENT(data_editor->data),
                             data_editor->context, NULL, xpos, FALSE, FALSE,
                             &rgb);
  ...
  str2 = g_strdup_printf(_("RGB (%0.3f, %0.3f, %0.3f)"), rgb.r, rgb.g, rgb.b);
  str3 = g_strdup_printf(_("HSV (%0.1f, %0.1f, %0.1f)"), hsv.h * 360.0,
                         hsv.s * 100.0, hsv.v * 100.0);
...
static void view_pick_color(GimpGradientEditor *editor,
  ...
  str2 = g_strdup_printf(_("RGB (%d, %d, %d)"), (gint)(color.r * 255.0),
...
static gboolean control_events(GtkWidget *widget, GdkEvent *event,
  ...
  case GDK_BUTTON_PRESS:
    if (editor->control_drag_mode == GRAD_DRAG_NONE) {
      ...
      control_button_press(editor, bevent->x, bevent->y, bevent->button,
                           bevent->state);
      ...
      if (editor->control_drag_mode != GRAD_DRAG_NONE) {
        ...
        if (GIMP_DATA_EDITOR(editor)->data_editable) {
          g_signal_handlers_block_by_func(
              gradient, gimp_gradient_editor_gradient_dirty, editor);
        }
      }
    }
    break;
      ...
      if ((bevent->time - editor->control_click_time) >= GRAD_MOVE_TIME) {
        ...
        if ((editor->control_drag_mode == GRAD_DRAG_ALL) &&
          ...
          control_extend_selection(editor, seg,
                                   control_calc_g_pos(editor, bevent->x));
        } else {
...
static gboolean control_draw(GtkWidget *widget, cairo_t *cr,
  ...
  control_draw_all(
      editor, GIMP_GRADIENT(GIMP_DATA_EDITOR(editor)->data), cr,
      allocation.width, allocation.height, gtk_adjustment_get_value(adj),
      gtk_adjustment_get_value(adj) + gtk_adjustment_get_page_size(adj));
...
static void control_do_hint(GimpGradientEditor *editor, gint x, gint y) {
  ...
  if (in_handle) {
    ...
    case GRAD_DRAG_LEFT:
      if (seg != NULL) {
        if (seg->prev != NULL) {
          ...
          gradient_editor_set_hint(editor, NULL, _("Drag: move"), str, NULL);
          g_free(str);
        } else {
          str = g_strdup_printf(_("%s-Click: extend selection"),
                                gimp_get_mod_string(GDK_SHIFT_MASK));

          gradient_editor_set_hint(editor, NULL, _("Click: select"), str, NULL);
          g_free(str);
        }
      } else {
        str = g_strdup_printf(_("%s-Click: extend selection"),
    ...
    case GRAD_DRAG_MIDDLE:
      ...
      gradient_editor_set_hint(editor, NULL, _("Click: select    Drag: move"),
                               str, NULL);
      g_free(str);
      break;
    ...
    gradient_editor_set_hint(editor, _("Click: select    Drag: move"), str,
                             str2, NULL);
    g_free(str);
    g_free(str2);
  }
...
static void control_motion(GimpGradientEditor *editor, GimpGradient *gradient,
  ...
  case GRAD_DRAG_LEFT:
    ...
    if (!editor->control_compress)
      ...
      control_compress_left(gradient, editor->control_sel_l,
                            editor->control_sel_r, seg, pos);
  ...
  case GRAD_DRAG_ALL:
    ...
    if ((seg->left >= editor->control_sel_l->left) &&
      ...
      delta = control_move(editor, editor->control_sel_l, editor->control_sel_r,
                           delta);
    else
    ...
    str = g_strdup_printf(_("Distance: %0.4f"),
                          editor->control_last_gx - editor->control_orig_pos);
    break;

  default:
    g_warning("%s: Attempting to move bogus handle %d.", G_STRFUNC,
              editor->control_drag_mode);
    break;
  }
...
static void control_compress_left(GimpGradient *gradient,
  ...
  if (drag_seg == range_l)
    gimp_gradient_segment_range_compress(gradient, range_l->prev, range_l->prev,
                                         range_l->prev->left, pos);
  else
    gimp_gradient_segment_range_compress(gradient, range_l, drag_seg->prev,
                                         range_l->left, pos);

  /* Compress segments to the right of the handle */

  if (drag_seg != range_r->next)
    gimp_gradient_segment_range_compress(gradient, drag_seg, range_r, pos,
                                         range_r->right);
  else
    gimp_gradient_segment_range_compress(gradient, drag_seg, drag_seg, pos,
                                         drag_seg->right);
}

/*****/

static gdouble control_move(GimpGradientEditor *editor,
                            GimpGradientSegment *range_l,
                            GimpGradientSegment *range_r, gdouble delta) {
  GimpGradient *gradient = GIMP_GRADIENT(GIMP_DATA_EDITOR(editor)->data);

  return gimp_gradient_segment_range_move(gradient, range_l, range_r, delta,
                                          editor->control_compress);
}
...
static void control_draw_all(GimpGradientEditor *editor, GimpGradient *gradient,
  ...
  for (seg = gradient->segments; seg; seg = seg->next) {
    ...
    control_draw_handle(editor, style, cr, seg->left, height, FALSE, flags);
    control_draw_handle(editor, style, cr, seg->middle, height, TRUE, flags);

    /* Draw right handle only if this is the last segment */
    if (seg->next == NULL)
      control_draw_handle(editor, style, cr, seg->right, height, FALSE, flags);
  ...
  if (seg && seg_in_selection(gradient, seg, editor->control_sel_l,
...
static GtkWidget *gradient_hint_label_add(GtkBox *box) {
  GtkWidget *label = g_object_new(GTK_TYPE_LABEL, "xalign", 0.0, "yalign", 0.5,
