...
static const gchar *const axis_use_strings[] = {
    N_("X"), N_("Y"), N_("Pressure"), N_("X tilt"), N_("Y tilt"),
     ...
     * feature.
     */
    N_("Wheel"), N_("Distance"), N_("Rotation"), N_("Slider")};
...
static void gimp_device_info_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_device_info_get_property(GObject *object, guint property_id,
...
G_DEFINE_TYPE_WITH_PRIVATE(GimpDeviceInfo, gimp_device_info,
...
static void gimp_device_info_class_init(GimpDeviceInfoClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("device", NULL, NULL, GDK_TYPE_DEVICE,
                          GIMP_PARAM_STATIC_STRINGS | G_PARAM_READWRITE |
                              G_PARAM_CONSTRUCT_ONLY |
                              G_PARAM_EXPLICIT_NOTIFY));

  g_object_class_install_property(
      object_class, PROP_DISPLAY,
      g_param_spec_object("display", NULL, NULL, GDK_TYPE_DISPLAY,
                          GIMP_PARAM_STATIC_STRINGS | G_PARAM_READWRITE |
                              G_PARAM_CONSTRUCT_ONLY |
                              G_PARAM_EXPLICIT_NOTIFY));

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_MODE, "mode", _("Mode"), NULL,
                        GDK_TYPE_INPUT_MODE, GDK_MODE_DISABLED,
                        GIMP_PARAM_STATIC_STRINGS);

  g_object_class_install_property(
      object_class, PROP_SOURCE,
      g_param_spec_enum("source", NULL, NULL, GDK_TYPE_INPUT_SOURCE,
                        GDK_SOURCE_MOUSE,
                        GIMP_PARAM_STATIC_STRINGS | G_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_VENDOR_ID,
      g_param_spec_string("vendor-id", NULL, NULL, NULL,
                          GIMP_PARAM_STATIC_STRINGS | G_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_PRODUCT_ID,
      g_param_spec_string("product-id", NULL, NULL, NULL,
                          GIMP_PARAM_STATIC_STRINGS | G_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_TOOL_TYPE,
      g_param_spec_enum("tool-type", NULL, NULL, GDK_TYPE_DEVICE_TOOL_TYPE,
                        GDK_DEVICE_TOOL_TYPE_UNKNOWN,
                        GIMP_PARAM_STATIC_STRINGS | G_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_TOOL_SERIAL,
      g_param_spec_uint64("tool-serial", NULL, NULL, 0, G_MAXUINT64, 0,
                          GIMP_PARAM_STATIC_STRINGS | G_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_TOOL_HARDWARE_ID,
      g_param_spec_uint64("tool-hardware-id", NULL, NULL, 0, G_MAXUINT64, 0,
                          GIMP_PARAM_STATIC_STRINGS | G_PARAM_READABLE));

  param_spec = g_param_spec_enum("axis", NULL, NULL, GDK_TYPE_AXIS_USE,
                                 GDK_AXIS_IGNORE, GIMP_PARAM_READWRITE);
  g_object_class_install_property(
      object_class, PROP_AXES,
      gimp_param_spec_value_array("axes", NULL, NULL, param_spec,
                                  GIMP_PARAM_STATIC_STRINGS |
                                      GIMP_CONFIG_PARAM_FLAGS));

  param_spec =
      g_param_spec_string("key", NULL, NULL, NULL, GIMP_PARAM_READWRITE);
  g_object_class_install_property(
      object_class, PROP_KEYS,
      gimp_param_spec_value_array("keys", NULL, NULL, param_spec,
                                  GIMP_PARAM_STATIC_STRINGS |
                                      GIMP_CONFIG_PARAM_FLAGS));

  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_PRESSURE_CURVE, "pressure-curve",
                          _("Pressure curve"), NULL, GIMP_TYPE_CURVE,
                          GIMP_CONFIG_PARAM_AGGREGATE);
}

static void gimp_device_info_init(GimpDeviceInfo *info) {
  ...
  g_signal_connect(info, "notify::name",
                   G_CALLBACK(gimp_device_info_guess_icon), NULL);
}
...
static void gimp_device_info_set_property(GObject *object, guint property_id,
  ...
  case PROP_AXES: {
    ...
    if (array) {
      ...
      if (device) {
        ...
        n_device_values = MIN(n_device_values, gdk_device_get_n_axes(device));
  ...
  case PROP_KEYS: {
    ...
    if (array) {
      ...
      if (device)
        n_device_values = MIN(n_device_values, gdk_device_get_n_keys(device));

      info->priv->n_keys = n_device_values;
      info->priv->keys =
          g_renew(GimpDeviceKey, info->priv->keys, info->priv->n_keys);
  ...
  if (src_curve && dest_curve) {
    gimp_config_copy(GIMP_CONFIG(src_curve), GIMP_CONFIG(dest_curve),
...
static void gimp_device_info_get_property(GObject *object, guint property_id,
  ...
  case PROP_AXES: {
    ...
    for (i = 0; i < n_axes; i++) {
      g_value_set_enum(&enum_value, gimp_device_info_get_axis_use(info, i));
...
static void gimp_device_info_guess_icon(GimpDeviceInfo *info) {
  ...
  if (gimp_object_get_name(viewable) &&
    ...
    gchar *down = g_ascii_strdown(gimp_object_get_name(viewable), -1);
    ...
    } else if (strstr(down, "cursor") || strstr(down, "mouse") ||
               strstr(down, "pointer") || strstr(down, "touchpad") ||
...
static void gimp_device_info_updated(GimpDeviceInfo *info) {
  ...
  g_return_if_fail(
      (info->priv->device == NULL && info->priv->display == NULL) ||
      (GDK_IS_DEVICE(info->priv->device) &&
       GDK_IS_DISPLAY(info->priv->display)));
  ...
  if (info->priv->device) {
    ...
    g_object_set_data(G_OBJECT(info->priv->device), GIMP_DEVICE_INFO_DATA_KEY,
                      info);
    ...
    for (i = 0, iter = axes; i < info->priv->n_axes; i++, iter = iter->next) {
       ...
       */
      if (gdk_device_get_device_type(info->priv->device) ==
...
GimpDeviceInfo *gimp_device_info_new(Gimp *gimp, GdkDevice *device,
                                     GdkDisplay *display) {
  GimpContext *context;
  ...
  return g_object_new(GIMP_TYPE_DEVICE_INFO, "gimp", gimp, "device", device,
                      "display", display, "mode", gdk_device_get_mode(device),
                      "tool-options", tool_info->tool_options, NULL);
}
...
gboolean gimp_device_info_set_device(GimpDeviceInfo *info, GdkDevice *device,
                                     GdkDisplay *display) {
  GdkInputMode mode;
  ...
  g_return_val_if_fail(
      device == NULL ||
          strcmp(gdk_device_get_name(device), gimp_object_get_name(info)) == 0,
      FALSE);
  ...
  if (info->priv->device) {
    ...
    g_signal_handlers_disconnect_by_func(info->priv->device,
                                         gimp_device_info_tool_changed, info);
    g_signal_handlers_disconnect_by_func(info->priv->device,
                                         gimp_device_info_device_changed, info);
  }
...
void gimp_device_info_set_default_tool(GimpDeviceInfo *info) {
  ...
  tool_info = GIMP_TOOL_INFO(
      gimp_container_get_child_by_name(tools, "gimp-paintbrush-tool"));

  if (info->priv->device) {
    switch (gdk_device_get_source(info->priv->device)) {
    case GDK_SOURCE_ERASER:
      tool_info = GIMP_TOOL_INFO(
          gimp_container_get_child_by_name(tools, "gimp-eraser-tool"));
      break;
    case GDK_SOURCE_PEN:
      tool_info = GIMP_TOOL_INFO(
          gimp_container_get_child_by_name(tools, "gimp-paintbrush-tool"));
    ...
    case GDK_SOURCE_TOUCHSCREEN:
      tool_info = GIMP_TOOL_INFO(
          gimp_container_get_child_by_name(tools, "gimp-smudge-tool"));
      break;
    default:
      break;
    }
  ...
  if (tool_info) {
    g_object_set(info, "tool-options", tool_info->tool_options, NULL);
  }
}
...
void gimp_device_info_save_tool(GimpDeviceInfo *info) {
  ...
  g_object_set(
      ...
      "use-brush", (serialize_props & GIMP_CONTEXT_PROP_MASK_BRUSH) != 0,

      "use-dynamics", (serialize_props & GIMP_CONTEXT_PROP_MASK_DYNAMICS) != 0,

      "use-mypaint-brush",
      (serialize_props & GIMP_CONTEXT_PROP_MASK_MYBRUSH) != 0,

      "use-gradient", (serialize_props & GIMP_CONTEXT_PROP_MASK_GRADIENT) != 0,

      "use-pattern", (serialize_props & GIMP_CONTEXT_PROP_MASK_PATTERN) != 0,

      "use-palette", (serialize_props & GIMP_CONTEXT_PROP_MASK_PALETTE) != 0,

      "use-font", (serialize_props & GIMP_CONTEXT_PROP_MASK_FONT) != 0,
...
void gimp_device_info_set_mode(GimpDeviceInfo *info, GdkInputMode mode) {
  g_return_if_fail(GIMP_IS_DEVICE_INFO(info));
...
gboolean gimp_device_info_ignore_axis(GimpDeviceInfo *info, gint axis) {
...
const gchar *gimp_device_info_get_axis_name(GimpDeviceInfo *info, gint axis) {
  g_return_val_if_fail(GIMP_IS_DEVICE_INFO(info), NULL);
...
GdkAxisUse gimp_device_info_get_axis_use(GimpDeviceInfo *info, gint axis) {
...
void gimp_device_info_set_axis_use(GimpDeviceInfo *info, gint axis,
                                   GdkAxisUse use) {
  g_return_if_fail(GIMP_IS_DEVICE_INFO(info));
...
void gimp_device_info_get_key(GimpDeviceInfo *info, gint key, guint *keyval,
                              GdkModifierType *modifiers) {
  g_return_if_fail(GIMP_IS_DEVICE_INFO(info));
  ...
  if (info->priv->device) {
    ...
    *modifiers = 0;
    ...
    gdk_device_get_key(info->priv->device, key, keyval, modifiers);
  } else {
...
void gimp_device_info_set_key(GimpDeviceInfo *info, gint key, guint keyval,
                              GdkModifierType modifiers) {
  guint old_keyval;
  ...
  if (keyval != old_keyval || modifiers != old_modifiers) {
    if (info->priv->device)
...
GimpCurve *gimp_device_info_get_curve(GimpDeviceInfo *info, GdkAxisUse use) {
  g_return_val_if_fail(GIMP_IS_DEVICE_INFO(info), NULL);
...
gdouble gimp_device_info_map_axis(GimpDeviceInfo *info, GdkAxisUse use,
                                  gdouble value) {
...
gint gimp_device_info_compare(GimpDeviceInfo *a, GimpDeviceInfo *b) {
  GdkSeat *seat;
  ...
  if (a->priv->device && a->priv->display &&
    ...
    return gimp_object_name_collate((GimpObject *)a, (GimpObject *)b);
  }
}
