...
enum { PROP_0, PROP_CONTEXT, PROP_LAYER_MODE };
...
static void gimp_layer_mode_box_set_property(GObject *object, guint prop_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_layer_mode_box_get_property(GObject *object, guint prop_id,
...
static void gimp_layer_mode_box_class_init(GimpLayerModeBoxClass *klass) {
...
static void gimp_layer_mode_box_init(GimpLayerModeBox *box) {
...
static void gimp_layer_mode_box_constructed(GObject *object) {
  ...
  g_object_bind_property(object, "context", G_OBJECT(mode_combo), "context",
                         G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);

  g_object_bind_property(object, "layer-mode", G_OBJECT(mode_combo),
                         "layer-mode",
                         G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);

  box->priv->group_combo = group_combo =
      gimp_prop_enum_combo_box_new(G_OBJECT(mode_combo), "group", 0, 0);
...
GtkWidget *gimp_layer_mode_box_new(GimpLayerModeContext context) {
  return g_object_new(GIMP_TYPE_LAYER_MODE_BOX, "context", context, NULL);
}
...
GimpLayerMode gimp_layer_mode_box_get_mode(GimpLayerModeBox *box) {
  g_return_val_if_fail(GIMP_IS_LAYER_MODE_BOX(box), GIMP_LAYER_MODE_NORMAL);
...
void gimp_layer_mode_box_set_ellipsize(GimpLayerModeBox *box,
  ...
  g_object_set(box->priv->mode_combo, "ellipsize", mode, NULL);
}
