...
static void
gimp_tool_options_editor_docked_iface_init(GimpDockedInterface *iface);
...
gimp_tool_options_editor_delete_clicked(GtkWidget *widget,
                                        GimpToolOptionsEditor *editor);
static void gimp_tool_options_editor_drop_tool(GtkWidget *widget, gint x,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpToolOptionsEditor, gimp_tool_options_editor, GIMP_TYPE_EDITOR,
...
static void gimp_tool_options_editor_init(GimpToolOptionsEditor *editor) {
  ...
  gimp_dnd_viewable_dest_add(GTK_WIDGET(editor), GIMP_TYPE_TOOL_INFO,
                             gimp_tool_options_editor_drop_tool, editor);
  ...
  gtk_box_pack_start(GTK_BOX(editor), editor->p->title_label, FALSE, FALSE, 0);
  ...
  gtk_scrolled_window_set_policy(scrolled_window, GTK_POLICY_AUTOMATIC,
                                 GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_overlay_scrolling(scrolled_window, FALSE);

  gtk_box_pack_start(GTK_BOX(editor), editor->p->scrolled_window, TRUE, TRUE,
                     0);
...
static void gimp_tool_options_editor_constructed(GObject *object) {
  ...
  editor->p->save_button = gimp_editor_add_button(
      GIMP_EDITOR(editor), GIMP_ICON_DOCUMENT_SAVE, _("Save Tool Preset..."),
      GIMP_HELP_TOOL_OPTIONS_SAVE,
      G_CALLBACK(gimp_tool_options_editor_save_clicked), NULL, editor);

  editor->p->restore_button = gimp_editor_add_button(
      GIMP_EDITOR(editor), GIMP_ICON_DOCUMENT_REVERT,
      _("Restore Tool Preset..."), GIMP_HELP_TOOL_OPTIONS_RESTORE,
      G_CALLBACK(gimp_tool_options_editor_restore_clicked), NULL, editor);

  editor->p->delete_button = gimp_editor_add_button(
      GIMP_EDITOR(editor), GIMP_ICON_EDIT_DELETE, _("Delete Tool Preset..."),
      GIMP_HELP_TOOL_OPTIONS_DELETE,
      G_CALLBACK(gimp_tool_options_editor_delete_clicked), NULL, editor);

  editor->p->reset_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor), "tool-options", "tool-options-reset",
      "tool-options-reset-all", GDK_SHIFT_MASK, NULL);
  ...
  gimp_tool_options_editor_tool_changed(
      user_context, gimp_context_get_tool(user_context), editor);
}
...
static GtkWidget *gimp_tool_options_editor_get_preview(GimpDocked *docked,
  ...
  gimp_view_renderer_set_size_full(GIMP_VIEW(view)->renderer, width, height, 0);
...
GtkWidget *gimp_tool_options_editor_new(Gimp *gimp,
  ...
  return g_object_new(GIMP_TYPE_TOOL_OPTIONS_EDITOR, "gimp", gimp,
                      "menu-factory", menu_factory, "menu-identifier",
                      "<ToolOptions>", "ui-path", "/tool-options-popup", NULL);
}
...
static void gimp_tool_options_editor_menu_popup(GimpToolOptionsEditor *editor,
                                                GtkWidget *button,
                                                const gchar *path) {
  ...
  gimp_ui_manager_ui_popup_at_widget(gimp_editor_get_ui_manager(gimp_editor),
                                     path, button, GDK_GRAVITY_WEST,
                                     GDK_GRAVITY_NORTH_EAST, NULL, NULL, NULL);
}
  ...
  if (editor->p->visible_tool_options) {
    ...
    if (presets)
      g_signal_handlers_disconnect_by_func(
          presets, gimp_tool_options_editor_presets_update, editor);

    options_gui =
        gimp_tools_get_tool_options_gui(editor->p->visible_tool_options);
  ...
  if (tool_info && tool_info->tool_options) {
    ...
    if (presets) {
      g_signal_connect_object(
          presets, "add", G_CALLBACK(gimp_tool_options_editor_presets_update),
          G_OBJECT(editor), G_CONNECT_SWAPPED);
      g_signal_connect_object(
          presets, "remove",
          G_CALLBACK(gimp_tool_options_editor_presets_update), G_OBJECT(editor),
          G_CONNECT_SWAPPED);
      g_signal_connect_object(
          presets, "thaw", G_CALLBACK(gimp_tool_options_editor_presets_update),
          G_OBJECT(editor), G_CONNECT_SWAPPED);
    }
    ...
    if (!gtk_widget_get_parent(options_gui))
      gtk_box_pack_start(GTK_BOX(editor->p->options_vbox), options_gui, FALSE,
                         FALSE, 0);
