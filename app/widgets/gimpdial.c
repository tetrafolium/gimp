...
static void gimp_dial_set_property(GObject *object, guint property_id,
                                   const GValue *value, GParamSpec *pspec);
static void gimp_dial_get_property(GObject *object, guint property_id,
                                   GValue *value, GParamSpec *pspec);

static gboolean gimp_dial_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_dial_set_target(GimpDial *dial, DialTarget target);

static void gimp_dial_draw_arrows(cairo_t *cr, gint size, gdouble alpha,
                                  gdouble beta, gboolean clockwise_delta,
                                  DialTarget highlight, gboolean draw_beta);

static gdouble gimp_dial_normalize_angle(gdouble angle);
static gdouble gimp_dial_get_angle_distance(gdouble alpha, gdouble beta);

G_DEFINE_TYPE_WITH_PRIVATE(GimpDial, gimp_dial, GIMP_TYPE_CIRCLE)

#define parent_class gimp_dial_parent_class

static void gimp_dial_class_init(GimpDialClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("alpha", NULL, NULL, 0.0, 2 * G_PI, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_BETA,
      g_param_spec_double("beta", NULL, NULL, 0.0, 2 * G_PI, G_PI,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_CLOCKWISE_ANGLES,
      g_param_spec_boolean("clockwise-angles", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_CLOCKWISE_DELTA,
      g_param_spec_boolean("clockwise-delta", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_DRAW_BETA,
      g_param_spec_boolean("draw-beta", NULL, NULL, TRUE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_dial_init(GimpDial *dial) {
...
static gboolean gimp_dial_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  gimp_dial_draw_arrows(cr, size, alpha, beta, dial->priv->clockwise_delta,
                        dial->priv->target, dial->priv->draw_beta);
...
static gboolean gimp_dial_button_press_event(GtkWidget *widget,
  ...
  if (bevent->type == GDK_BUTTON_PRESS && bevent->button == 1 &&
    ...
    angle = _gimp_circle_get_angle_and_distance(GIMP_CIRCLE(dial), bevent->x,
                                                bevent->y, NULL);
...
static gboolean gimp_dial_motion_notify_event(GtkWidget *widget,
  ...
  angle = _gimp_circle_get_angle_and_distance(GIMP_CIRCLE(dial), mevent->x,
                                              mevent->y, &distance);
  ...
  if (_gimp_circle_has_grab(GIMP_CIRCLE(dial))) {
    ...
    if (delta != 0.0) {
      ...
      case DIAL_TARGET_BOTH:
        ...
        g_object_set(
            dial, "alpha", gimp_dial_normalize_angle(dial->priv->alpha + delta),
            "beta", gimp_dial_normalize_angle(dial->priv->beta + delta), NULL);
        break;
    ...
    if (dial->priv->draw_beta && distance > SEGMENT_FRACTION &&
...
GtkWidget *gimp_dial_new(void) { return g_object_new(GIMP_TYPE_DIAL, NULL); }
...
static void gimp_dial_draw_arrow(cairo_t *cr, gdouble radius, gdouble angle) {
#define REL 0.8
#define DEL 0.1

  cairo_move_to(cr, radius, radius);
  cairo_line_to(cr, radius + radius * cos(angle), radius - radius * sin(angle));

  cairo_move_to(cr, radius + radius * cos(angle), radius - radius * sin(angle));
  cairo_line_to(cr, radius + radius * REL * cos(angle - DEL),
...
static void gimp_dial_draw_segment(cairo_t *cr, gdouble radius, gdouble alpha,
                                   gdouble beta, gboolean clockwise_delta) {
  ...
  cairo_move_to(cr, radius + segment_dist * cos(beta),
                radius - segment_dist * sin(beta));
  cairo_line_to(
      cr, radius + segment_dist * cos(beta) + direction * tick * sin(beta),
      radius - segment_dist * sin(beta) + direction * tick * cos(beta));
  ...
  gimp_cairo_arc(cr, radius, radius, segment_dist, alpha, slice);
}
...
static void gimp_dial_draw_arrows(cairo_t *cr, gint size, gdouble alpha,
                                  ...
                                  DialTarget highlight, gboolean draw_beta) {
  gdouble radius =
      size / 2.0 - 2.0; /* half the broad line with and half a px */
