...
GList *gimp_session_info_aux_new_from_props(GObject *object, ...) {
  ...
  for (prop_name = va_arg(args, const gchar *); prop_name;
    ...
    if (pspec) {
      ...
      if (!g_param_value_defaults(pspec, &value) &&
        ...
        list = g_list_prepend(list,
                              gimp_session_info_aux_new(prop_name, str->str));
      }

      g_value_unset(&value);
      g_string_free(str, TRUE);
    } else {
      g_warning("%s: no property named '%s' for %s", G_STRFUNC, prop_name,
...
void gimp_session_info_aux_set_props(GObject *object, GList *auxs, ...) {
  const gchar *prop_name;
  va_list args;
        ...
        if (pspec) {
          ...
          if (G_VALUE_HOLDS_ENUM(&value)) {
            ...
            if (enum_value) {
              ...
              g_warning("%s: unknown enum value in '%s' for %s", G_STRFUNC,
            ...
            if (g_value_transform(&str_value, &value))
              ...
              g_warning("%s: cannot convert property '%s' for %s", G_STRFUNC,
                        prop_name, G_OBJECT_CLASS_NAME(class));
