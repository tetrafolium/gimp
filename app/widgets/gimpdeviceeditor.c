...
#define GIMP_DEVICE_EDITOR_GET_PRIVATE(editor)                                 \
  ((GimpDeviceEditorPrivate *)gimp_device_editor_get_instance_private(         \
...
static void gimp_device_editor_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_device_editor_get_property(GObject *object, guint property_id,
...
G_DEFINE_TYPE_WITH_PRIVATE(GimpDeviceEditor, gimp_device_editor, GTK_TYPE_PANED)

#define parent_class gimp_device_editor_parent_class

static void gimp_device_editor_class_init(GimpDeviceEditorClass *klass) {
...
static void gimp_device_editor_init(GimpDeviceEditor *editor) {
...
private
  ->treeview = gimp_container_tree_view_new(NULL, NULL, icon_height, 0);
...
private
  ->delete_button = gimp_editor_add_button(
      GIMP_EDITOR(private->treeview), "edit-delete",
      _("Delete the selected device"), NULL,
      G_CALLBACK(gimp_device_editor_delete_clicked), NULL, editor);
...
private
  ...
  gimp_label_set_attributes(GTK_LABEL(private->label), PANGO_ATTR_WEIGHT,
                            PANGO_WEIGHT_BOLD, -1);
...
static gboolean gimp_device_editor_filter(GimpObject *object,
                                          gpointer user_data) {
  ...
  return ((!device ||
           ...
           * device).
           */
          g_strcmp0(gimp_object_get_name(info), "Virtual core XTEST pointer") !=
              0);
}
...
static void gimp_device_editor_constructed(GObject *object) {
  ...
  filtered =
      gimp_filtered_container_new(devices, gimp_device_editor_filter, NULL);
  ...
  g_signal_connect(devices, "remove",
                   G_CALLBACK(gimp_device_editor_remove_device), editor);
  ...
  g_signal_connect(devices, "add", G_CALLBACK(gimp_device_editor_add_device),
                   editor);

private
  ->name_changed_handler = gimp_container_add_handler(
      devices, "name-changed", G_CALLBACK(gimp_device_editor_device_changed),
      editor);

  for (list = GIMP_LIST(devices)->queue->head; list; list = g_list_next(list)) {
...
static void gimp_device_editor_dispose(GObject *object) {
  ...
  g_signal_handlers_disconnect_by_func(devices, gimp_device_editor_add_device,
                                       object);

  g_signal_handlers_disconnect_by_func(
      devices, gimp_device_editor_remove_device, object);

  if (private->name_changed_handler) {
    gimp_container_remove_handler(devices, private->name_changed_handler);
  private
    ->name_changed_handler = 0;
  }
...
static void gimp_device_editor_add_device(GimpContainer *container,
  ...
  if (iter) {
    ...
    gtk_tree_store_set(GTK_TREE_STORE(treeview->model), iter,
                       ...
                       gimp_device_info_get_device(info, NULL) != NULL, -1);
  }
}
...
static void gimp_device_editor_remove_device(GimpContainer *container,
  ...
  if (iter) {
    ...
    gtk_tree_model_get(treeview->model, iter,
                       GIMP_CONTAINER_TREE_STORE_COLUMN_USER_DATA, &widget, -1);
...
static void gimp_device_editor_select_device(GimpContainerView *view,
  ...
  if (viewable && insert_data) {
    ...
    if (widget) {
      ...
      gtk_label_set_text(GTK_LABEL(private->label), gimp_object_get_name(info));
      gtk_image_set_from_icon_name(
          GTK_IMAGE(private->image),
...
static void gimp_device_editor_delete_response(GtkWidget *dialog,
                                               gint response_id,
                                               GimpDeviceEditor *editor) {
  GimpDeviceEditorPrivate *private = GIMP_DEVICE_EDITOR_GET_PRIVATE(editor);
...
static void gimp_device_editor_delete_clicked(GtkWidget *button,
  ...
  dialog = gimp_message_dialog_new(
      _("Delete Device Settings"), GIMP_ICON_DIALOG_QUESTION,
      gtk_widget_get_toplevel(GTK_WIDGET(editor)),
      GTK_DIALOG_DESTROY_WITH_PARENT, gimp_standard_help_func, NULL,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Delete"), GTK_RESPONSE_OK,
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(gimp_device_editor_delete_response), editor);
...
GtkWidget *gimp_device_editor_new(Gimp *gimp) {
  ...
  return g_object_new(GIMP_TYPE_DEVICE_EDITOR, "gimp", gimp, NULL);
}
