...
static void gimp_error_dialog_response(GtkDialog *dialog, gint response_id);
...
static void gimp_error_dialog_finalize(GObject *object) {
  ...
  g_list_free_full(dialog->messages, gimp_error_dialog_message_destroy);
...
GtkWidget *gimp_error_dialog_new(const gchar *title) {
  ...
  return g_object_new(GIMP_TYPE_ERROR_DIALOG, "title", title, "use-header-bar",
                      use_header_bar, NULL);
}
...
void gimp_error_dialog_add(GimpErrorDialog *dialog, const gchar *icon_name,
  ...
  item->box = g_object_new(GIMP_TYPE_MESSAGE_BOX, "icon-name", icon_name, NULL);
  item->domain = g_strdup(domain);
  item->message = g_strdup(message);

  if (overflow)
    gimp_message_box_set_primary_text(GIMP_MESSAGE_BOX(item->box), "%s",
                                      domain);
  else
