...
enum { PROP_0, PROP_SAMPLE_MERGED };
...
static void gimp_sample_point_editor_point_moved(GimpImage *image,
                                                 GimpSamplePoint *sample_point,
                                                 GimpSamplePointEditor *editor);
static void gimp_sample_point_editor_proj_update(GimpImage *image, gboolean now,
...
gimp_sample_point_editor_class_init(GimpSamplePointEditorClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_SAMPLE_MERGED,
      g_param_spec_boolean("sample-merged", NULL, NULL, TRUE,
...
static void gimp_sample_point_editor_init(GimpSamplePointEditor *editor) {
  ...
  gtk_widget_style_get(GTK_WIDGET(editor), "content-spacing", &content_spacing,
                       NULL);
  ...
  gimp_label_set_attributes(GTK_LABEL(editor->empty_label), PANGO_ATTR_STYLE,
...
static void gimp_sample_point_editor_set_image(GimpImageEditor *image_editor,
                                               GimpImage *image) {
  GimpSamplePointEditor *editor = GIMP_SAMPLE_POINT_EDITOR(image_editor);

  if (image_editor->image) {
    g_signal_handlers_disconnect_by_func(
        image_editor->image, gimp_sample_point_editor_point_added, editor);
    g_signal_handlers_disconnect_by_func(
        image_editor->image, gimp_sample_point_editor_point_removed, editor);
    g_signal_handlers_disconnect_by_func(
        image_editor->image, gimp_sample_point_editor_point_moved, editor);

    g_signal_handlers_disconnect_by_func(
        gimp_image_get_projection(image_editor->image),
        gimp_sample_point_editor_proj_update, editor);
  }
  ...
  if (image) {
    g_signal_connect(image, "sample-point-added",
                     G_CALLBACK(gimp_sample_point_editor_point_added), editor);
    ...
    g_signal_connect(image, "sample-point-moved",
                     G_CALLBACK(gimp_sample_point_editor_point_moved), editor);

    g_signal_connect(gimp_image_get_projection(image), "update",
                     G_CALLBACK(gimp_sample_point_editor_proj_update), editor);
  }

  gtk_widget_set_visible(editor->empty_icon, image_editor->image == NULL);
...
GtkWidget *gimp_sample_point_editor_new(GimpMenuFactory *menu_factory) {
  ...
  return g_object_new(GIMP_TYPE_SAMPLE_POINT_EDITOR, "menu-factory",
                      menu_factory, "menu-identifier", "<SamplePoints>",
                      "ui-path", "/sample-points-popup", NULL);
}
...
static void
gimp_sample_point_editor_proj_update(GimpImage *image, gboolean now, gint x,
  ...
  for (i = 0, list = sample_points; i < n_points;
    ...
    if (sp_x >= x && sp_x < (x + width) && sp_y >= y && sp_y < (y + height)) {
      gimp_sample_point_editor_dirty(editor, i);
    }
  }
}
...
#define RANDOM_MAGIC 16
  ...
  if (n_points < editor->n_color_frames && n_points < RANDOM_MAGIC &&
    ...
    editor->color_frames =
        g_renew(GtkWidget *, editor->color_frames, RANDOM_MAGIC);
    ...
    editor->color_frames = g_renew(GtkWidget *, editor->color_frames, n_points);

    for (i = editor->n_color_frames; i < n_points; i++) {
      gint row = i / 2;
      gint column = i % 2;

      editor->color_frames[i] = g_object_new(
          GIMP_TYPE_COLOR_FRAME, "mode", GIMP_COLOR_PICK_MODE_PIXEL,
          "has-number", TRUE, "number", i + 1, "has-color-area", TRUE,
          "has-coords", TRUE, "hexpand", TRUE, NULL);

      gimp_color_frame_set_color_config(
          GIMP_COLOR_FRAME(editor->color_frames[i]), config);

      gtk_grid_attach(GTK_GRID(editor->grid), editor->color_frames[i], column,
                      row, 1, 1);
      ...
      g_object_set_data(G_OBJECT(editor->color_frames[i]), "dirty",
...
static void gimp_sample_point_editor_dirty(GimpSamplePointEditor *editor,
                                           gint index) {
  if (index >= 0) {
    g_object_set_data(G_OBJECT(editor->color_frames[index]), "dirty",
                      GINT_TO_POINTER(TRUE));
  } else {
    gint i;
  ...
  editor->dirty_idle_id =
      g_idle_add((GSourceFunc)gimp_sample_point_editor_update, editor);
}
...
static gboolean gimp_sample_point_editor_update(GimpSamplePointEditor *editor) {
  ...
  for (i = 0, list = sample_points; i < n_points;
    ...
    if (GPOINTER_TO_INT(g_object_get_data(G_OBJECT(color_frame), "dirty"))) {
      GimpSamplePoint *sample_point = list->data;
      const Babl *format;
      gdouble pixel[4];
      GimpRGB color;
      ...
      g_object_set_data(G_OBJECT(color_frame), "dirty", GINT_TO_POINTER(FALSE));

      gimp_sample_point_get_position(sample_point, &x, &y);

      if (gimp_image_pick_color(image_editor->image, NULL, x, y, FALSE,
                                editor->sample_merged, FALSE, 0.0, &format,
                                pixel, &color)) {
        gimp_color_frame_set_color(color_frame, FALSE, format, pixel, &color, x,
                                   y);
      } else {
  ...
  for (i = 0, list = sample_points; i < n_points;
    ...
    if (GIMP_COLOR_FRAME(editor->color_frames[i]) == frame) {
      ...
      if (pick_mode != gimp_sample_point_get_pick_mode(sample_point))
        gimp_image_set_sample_point_pick_mode(image_editor->image, sample_point,
                                              pick_mode, TRUE);
      break;
    }
  }
