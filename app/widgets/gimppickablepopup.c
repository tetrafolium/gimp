...
static void gimp_pickable_popup_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_pickable_popup_get_property(GObject *object, guint property_id,
...
static void gimp_pickable_popup_class_init(GimpPickablePopupClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_VIEW_BORDER_WIDTH,
      g_param_spec_int("view-border-width", NULL, NULL, 0,
                       GIMP_VIEW_MAX_BORDER_WIDTH, 1,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_pickable_popup_init(GimpPickablePopup *popup) {
...
static void gimp_pickable_popup_constructed(GObject *object) {
  ...
  popup->priv->image_view = gimp_container_tree_view_new(
      popup->priv->context->gimp->images, popup->priv->context,
      popup->priv->view_size, popup->priv->view_border_width);
  gimp_container_box_set_size_request(
      GIMP_CONTAINER_BOX(popup->priv->image_view),
      4 * (popup->priv->view_size + 2 * popup->priv->view_border_width),
      4 * (popup->priv->view_size + 2 * popup->priv->view_border_width));
  ...
  popup->priv->layer_view = gimp_container_tree_view_new(
      NULL, popup->priv->context, popup->priv->view_size,
      popup->priv->view_border_width);
  gtk_tree_view_set_show_expanders(
      GTK_TREE_VIEW(GIMP_CONTAINER_TREE_VIEW(popup->priv->layer_view)->view),
      TRUE);
  gimp_container_box_set_size_request(
      GIMP_CONTAINER_BOX(popup->priv->layer_view),
      4 * (popup->priv->view_size + 2 * popup->priv->view_border_width),
      4 * (popup->priv->view_size + 2 * popup->priv->view_border_width));
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), popup->priv->layer_view,
  ...
  popup->priv->channel_view = gimp_container_tree_view_new(
      NULL, popup->priv->context, popup->priv->view_size,
      popup->priv->view_border_width);
  gimp_container_box_set_size_request(
      GIMP_CONTAINER_BOX(popup->priv->channel_view),
      4 * (popup->priv->view_size + 2 * popup->priv->view_border_width),
      4 * (popup->priv->view_size + 2 * popup->priv->view_border_width));
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), popup->priv->channel_view,
...
GtkWidget *gimp_pickable_popup_new(GimpContext *context, gint view_size,
  ...
  g_return_val_if_fail(
      view_size > 0 && view_size <= GIMP_VIEWABLE_MAX_POPUP_SIZE, NULL);
  g_return_val_if_fail(view_border_width >= 0 &&
                           view_border_width <= GIMP_VIEW_MAX_BORDER_WIDTH,
                       NULL);

  return g_object_new(GIMP_TYPE_PICKABLE_POPUP, "type", GTK_WINDOW_POPUP,
                      "context", context, "view-size", view_size,
...
GimpPickable *gimp_pickable_popup_get_pickable(GimpPickablePopup *popup) {
  ...
  if (focus && gtk_widget_is_ancestor(focus, popup->priv->image_view)) {
    ...
    if (gimp_container_view_get_selected(
            GIMP_CONTAINER_VIEW(popup->priv->layer_view), &selected, NULL)) {
      pickable = selected->data;
      g_list_free(selected);
    }
  } else if (focus &&
             gtk_widget_is_ancestor(focus, popup->priv->channel_view)) {
    GList *selected;

    if (gimp_container_view_get_selected(
            GIMP_CONTAINER_VIEW(popup->priv->channel_view), &selected, NULL)) {
      pickable = selected->data;
      g_list_free(selected);
    }
  }
...
static void gimp_pickable_popup_image_changed(GimpContext *context,
  ...
  gimp_container_view_set_container(
      GIMP_CONTAINER_VIEW(popup->priv->layer_view), layers);
  gimp_container_view_set_container(
      GIMP_CONTAINER_VIEW(popup->priv->channel_view), channels);
}
