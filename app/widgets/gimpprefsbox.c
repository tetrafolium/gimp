...
static void gimp_prefs_box_class_init(GimpPrefsBoxClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  object_class->finalize = gimp_prefs_box_finalize;
}

static void gimp_prefs_box_init(GimpPrefsBox *box) {
...
private
  ->tree_icon_size = GTK_ICON_SIZE_BUTTON;
private
  ->page_icon_size = GTK_ICON_SIZE_DIALOG;
  ...
  gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(frame),
                                      GTK_SHADOW_IN);
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(frame), GTK_POLICY_NEVER,
                                 GTK_POLICY_AUTOMATIC);
  gtk_box_pack_start(GTK_BOX(box), frame, FALSE, FALSE, 0);
  gtk_widget_show(frame);

private
  ->store = gtk_tree_store_new(7, G_TYPE_STRING, G_TYPE_INT, G_TYPE_STRING,
                               G_TYPE_STRING, G_TYPE_INT, G_TYPE_STRING,
                               G_TYPE_STRING);
private
  ->tree_view = gtk_tree_view_new_with_model(GTK_TREE_MODEL(private->store));
  ...
  gtk_tree_view_column_set_attributes(column, cell, "icon-name",
                                      COLUMN_TREE_ICON_NAME, "stock-size",
                                      COLUMN_TREE_ICON_SIZE, NULL);

  cell = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(column, cell, TRUE);
  gtk_tree_view_column_set_attributes(column, cell, "text", COLUMN_TREE_LABEL,
                                      NULL);
...
private
  ...
  gimp_label_set_attributes(GTK_LABEL(private->label), PANGO_ATTR_SCALE,
...
private
  ...
  g_signal_connect(sel, "changed",
                   G_CALLBACK(gimp_prefs_box_tree_select_callback), box);
}
...
static void gimp_prefs_box_tree_select_callback(GtkTreeSelection *sel,
  ...
  gtk_tree_model_get(model, &iter, COLUMN_PAGE_ICON_NAME, &page_icon_name,
                     COLUMN_PAGE_ICON_SIZE, &page_icon_size, COLUMN_PAGE_TITLE,
                     &page_title, COLUMN_PAGE_HELP_ID, &page_help_id, -1);

  gtk_label_set_text(GTK_LABEL(private->label), page_title);
  g_free(page_title);

  gtk_image_set_from_icon_name(GTK_IMAGE(private->image), page_icon_name,
                               page_icon_size);
  g_free(page_icon_name);

  gtk_stack_set_visible_child_name(GTK_STACK(private->stack), page_help_id);
...
private
  ...
  gtk_stack_add_named(GTK_STACK(private->stack), page_vbox, help_id /* EEK */);
  ...
  gtk_tree_store_set(private->store, iter, COLUMN_TREE_ICON_NAME, icon_name,
                     COLUMN_TREE_ICON_SIZE, private->tree_icon_size,
                     COLUMN_TREE_LABEL, tree_label, COLUMN_PAGE_ICON_NAME,
                     icon_name, COLUMN_PAGE_ICON_SIZE, private->page_icon_size,
                     COLUMN_PAGE_TITLE, page_title, COLUMN_PAGE_HELP_ID,
                     help_id, -1);
...
const gchar *gimp_prefs_box_get_current_icon_name(GimpPrefsBox *box) {
  ...
  if (gtk_tree_selection_get_selected(sel, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, COLUMN_PAGE_ICON_NAME,
                       &private->page_icon_name, -1);
...
const gchar *gimp_prefs_box_get_current_help_id(GimpPrefsBox *box) {
  ...
  if (gtk_tree_selection_get_selected(sel, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, COLUMN_PAGE_HELP_ID,
                       &private->page_help_id, -1);
...
private
  ...
  gtk_scrolled_window_set_policy(
      GTK_SCROLLED_WINDOW(scrolled_win), GTK_POLICY_NEVER,
      scrollable ? GTK_POLICY_AUTOMATIC : GTK_POLICY_NEVER);
}
