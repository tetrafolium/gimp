...
enum { PROP_0, PROP_USER_CONTEXT_AWARE };

static void gimp_color_dialog_constructed(GObject *object);
static void gimp_color_dialog_finalize(GObject *object);
static void gimp_color_dialog_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_color_dialog_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);

static void gimp_color_dialog_response(GtkDialog *dialog, gint response_id);
...
static void gimp_color_dialog_colormap_edit_activate(GimpColorDialog *dialog);
static void gimp_color_dialog_colormap_add_activate(GimpColorDialog *dialog);
...
static guint color_dialog_signals[LAST_SIGNAL] = {
    0,
};
...
static void gimp_color_dialog_class_init(GimpColorDialogClass *klass) {
  ...
  color_dialog_signals[UPDATE] =
      g_signal_new("update", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpColorDialogClass, update), NULL, NULL,
                   gimp_marshal_VOID__BOXED_ENUM, G_TYPE_NONE, 2, GIMP_TYPE_RGB,
                   GIMP_TYPE_COLOR_DIALOG_STATE);

  g_object_class_install_property(
      object_class, PROP_USER_CONTEXT_AWARE,
      g_param_spec_boolean("user-context-aware", NULL, NULL, FALSE,
...
static void gimp_color_dialog_constructed(GObject *object) {
  ...
  dialog->colormap_selection =
      gimp_colormap_selection_new(viewable_dialog->context->gimp->user_context);
  gtk_stack_add_named(GTK_STACK(dialog->stack), dialog->colormap_selection,
                      "colormap");
  ...
  image =
      gtk_image_new_from_icon_name(GIMP_ICON_EDIT, GTK_ICON_SIZE_LARGE_TOOLBAR);
  ...
  gtk_box_pack_start(
      GTK_BOX(GIMP_COLORMAP_SELECTION(dialog->colormap_selection)->right_vbox),
  ...
  gimp_help_set_help_data_with_markup(
      button, NC_("colormap-action", "Add current foreground color"),
  ...
  g_signal_connect(dialog->selection, "color-changed",
                   G_CALLBACK(gimp_color_dialog_color_changed), dialog);

  /* Color history box. */
  hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 4);
  gtk_box_pack_end(GTK_BOX(gimp_color_selection_get_right_vbox(
                       GIMP_COLOR_SELECTION(dialog->selection))),
                   hbox, FALSE, FALSE, 0);
  ...
  gimp_help_set_help_data(
      button, _("Add the current color to the color history"), NULL);
  gtk_widget_show(button);

  g_signal_connect(button, "clicked",
                   G_CALLBACK(gimp_color_history_add_clicked), dialog);
  ...
  g_signal_connect(history, "color-selected",
                   G_CALLBACK(gimp_color_dialog_history_selected), dialog);

  g_signal_connect(dialog, "show", G_CALLBACK(gimp_color_dialog_show), NULL);
  g_signal_connect(dialog, "hide", G_CALLBACK(gimp_color_dialog_hide), NULL);
...
static void gimp_color_dialog_finalize(GObject *object) {
  ...
  if (dialog->user_context_aware && viewable_dialog->context) {
    ...
    g_signal_handlers_disconnect_by_func(
        user_context, G_CALLBACK(gimp_color_dialog_image_changed), dialog);
  }
...
static void gimp_color_dialog_response(GtkDialog *gtk_dialog,
                                       gint response_id) {
  ...
  case GTK_RESPONSE_OK:
    ...
    if (dialog->colormap_editing && image) {
      ...
      g_signal_emit(dialog, color_dialog_signals[UPDATE], 0, &color,
...
GtkWidget *gimp_color_dialog_new(GimpViewable *viewable, GimpContext *context,
                                 gboolean user_context_aware,
                                 const gchar *title, const gchar *icon_name,
                                 const gchar *desc, GtkWidget *parent,
                                 GimpDialogFactory *dialog_factory,
                                 const gchar *dialog_identifier,
                                 const GimpRGB *color, gboolean wants_updates,
                                 gboolean show_alpha) {
  GimpColorDialog *dialog;
  const gchar *role;
  ...
  g_return_val_if_fail(
      dialog_factory == NULL || GIMP_IS_DIALOG_FACTORY(dialog_factory), NULL);
  ...
  dialog = g_object_new(
      GIMP_TYPE_COLOR_DIALOG, "title", title, "role", role, "help-func",
      gimp_color_dialog_help_func, "help-id", GIMP_HELP_COLOR_DIALOG,
      "icon-name", icon_name, "description", desc, "context", context,
      "user-context-aware", user_context_aware, "parent",
      gtk_widget_get_toplevel(parent), "use-header-bar", use_header_bar, NULL);
...
void gimp_color_dialog_set_color(GimpColorDialog *dialog,
  ...
  g_signal_handlers_block_by_func(dialog->selection,
                                  gimp_color_dialog_color_changed, dialog);
...
void gimp_color_dialog_get_color(GimpColorDialog *dialog, GimpRGB *color) {
  g_return_if_fail(GIMP_IS_COLOR_DIALOG(dialog));
  g_return_if_fail(color != NULL);
...
static void gimp_color_dialog_help_func(const gchar *help_id,
  ...
  notebook = GIMP_COLOR_NOTEBOOK(gimp_color_selection_get_notebook(
      GIMP_COLOR_SELECTION(dialog->selection)));
...
static void gimp_color_dialog_colormap_clicked(GimpColorDialog *dialog,
  ...
  if (dialog->wants_updates) {
    g_signal_emit(dialog, color_dialog_signals[UPDATE], 0, &entry->color,
                  GIMP_COLOR_DIALOG_UPDATE);
  }
}
...
static void gimp_color_dialog_history_selected(GimpColorHistory *history,
                                               const GimpRGB *rgb,
                                               GimpColorDialog *dialog) {
  gimp_color_selection_set_color(GIMP_COLOR_SELECTION(dialog->selection), rgb);
}
...
static void gimp_color_dialog_image_changed(GimpContext *context,
  ...
  if (dialog->active_image != image) {
    if (dialog->active_image) {
      ...
      g_signal_handlers_disconnect_by_func(
          dialog->active_image, G_CALLBACK(gimp_color_dialog_update), dialog);
    }
    ...
    if (image) {
      ...
      g_signal_connect_swapped(image, "notify::base-type",
                               G_CALLBACK(gimp_color_dialog_update), dialog);
    }
...
static void gimp_color_dialog_show(GimpColorDialog *dialog) {
  ...
  if (dialog->user_context_aware && viewable_dialog->context) {
    ...
    g_signal_connect(user_context, "image-changed",
                     G_CALLBACK(gimp_color_dialog_image_changed), dialog);

    gimp_color_dialog_image_changed(viewable_dialog->context, image, dialog);
    gimp_color_dialog_update(dialog);
  } else {
    gtk_stack_set_visible_child_name(GTK_STACK(dialog->stack), "color");
  }
