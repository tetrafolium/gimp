...
enum { PRE_CLICKED, CLICKED, LAST_SIGNAL };

enum { PROP_0, PROP_RENDERER };
...
gimp_cell_renderer_viewable_class_init(GimpCellRendererViewableClass *klass) {
   ...
   **/
  viewable_cell_signals[PRE_CLICKED] = g_signal_new(
      "pre-clicked", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpCellRendererViewableClass, pre_clicked),
      g_signal_accumulator_true_handled, NULL,
      gimp_marshal_BOOLEAN__STRING_FLAGS, G_TYPE_BOOLEAN, 2, G_TYPE_STRING,
   ...
   **/
  viewable_cell_signals[CLICKED] = g_signal_new(
      "clicked", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpCellRendererViewableClass, clicked), NULL, NULL,
      gimp_marshal_VOID__STRING_FLAGS, G_TYPE_NONE, 2, G_TYPE_STRING,
      GDK_TYPE_MODIFIER_TYPE);
...
gimp_cell_renderer_viewable_init(GimpCellRendererViewable *cellviewable) {
  g_object_set(cellviewable, "mode", GTK_CELL_RENDERER_MODE_ACTIVATABLE, NULL);
}
...
gboolean gimp_cell_renderer_viewable_pre_clicked(GimpCellRendererViewable *cell,
  ...
  g_signal_emit(cell, viewable_cell_signals[PRE_CLICKED], 0 /*detail*/, path,
                state, &handled);
  ...
  if (cell->renderer) {
    ...
    if (event) {
      ...
      if (bevent->type == GDK_BUTTON_PRESS &&
        ...
        gimp_view_popup_show(gtk_get_event_widget(event), bevent,
                             cell->renderer->context, cell->renderer->viewable,
                             cell->renderer->width, cell->renderer->height,
