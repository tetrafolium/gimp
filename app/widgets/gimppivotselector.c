...
enum { PROP_0, PROP_LEFT, PROP_TOP, PROP_RIGHT, PROP_BOTTOM, PROP_X, PROP_Y };
...
static void gimp_pivot_selector_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_pivot_selector_get_property(GObject *object, guint property_id,
...
static GtkWidget *
gimp_pivot_selector_position_to_button(GimpPivotSelector *selector, gdouble x,
                                       gdouble y);
...
G_DEFINE_TYPE_WITH_PRIVATE(GimpPivotSelector, gimp_pivot_selector,
                           GTK_TYPE_GRID)
...
static void gimp_pivot_selector_class_init(GimpPivotSelectorClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  pivot_selector_signals[CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpPivotSelectorClass, changed), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      object_class, PROP_LEFT,
      g_param_spec_double("left", NULL, NULL, -G_MAXDOUBLE, +G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_TOP,
      g_param_spec_double("top", NULL, NULL, -G_MAXDOUBLE, +G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_RIGHT,
      g_param_spec_double("right", NULL, NULL, -G_MAXDOUBLE, +G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_BOTTOM,
      g_param_spec_double("bottom", NULL, NULL, -G_MAXDOUBLE, +G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
...
static void gimp_pivot_selector_init(GimpPivotSelector *selector) {
  ...
  for (i = 0; i < 9; i++) {
    static const gchar *icon_names[9] = {
        GIMP_ICON_PIVOT_NORTH_WEST, GIMP_ICON_PIVOT_NORTH,
        GIMP_ICON_PIVOT_NORTH_EAST,

        GIMP_ICON_PIVOT_WEST,       GIMP_ICON_PIVOT_CENTER,
        GIMP_ICON_PIVOT_EAST,

        GIMP_ICON_PIVOT_SOUTH_WEST, GIMP_ICON_PIVOT_SOUTH,
    ...
    g_signal_connect(button, "toggled",
                     G_CALLBACK(gimp_pivot_selector_button_toggled), selector);
...
static void gimp_pivot_selector_set_property(GObject *object, guint property_id,
  ...
  switch (property_id) {
  case PROP_LEFT:
    gimp_pivot_selector_set_bounds(selector, g_value_get_double(value),
                                   selector->priv->top, selector->priv->right,
                                   selector->priv->bottom);
    break;
  case PROP_TOP:
    gimp_pivot_selector_set_bounds(
        selector, selector->priv->left, g_value_get_double(value),
        selector->priv->right, selector->priv->bottom);
    break;
  case PROP_RIGHT:
    gimp_pivot_selector_set_bounds(
        selector, selector->priv->left, selector->priv->top,
        g_value_get_double(value), selector->priv->bottom);
    break;
  case PROP_BOTTOM:
    gimp_pivot_selector_set_bounds(selector, selector->priv->left,
                                   selector->priv->top, selector->priv->right,
                                   g_value_get_double(value));
    break;

  case PROP_X:
    gimp_pivot_selector_set_position(selector, g_value_get_double(value),
                                     selector->priv->y);
    break;
  case PROP_Y:
    gimp_pivot_selector_set_position(selector, selector->priv->x,
                                     g_value_get_double(value));
    break;
...
static void gimp_pivot_selector_button_toggled(GtkToggleButton *button,
  ...
  if (GTK_WIDGET(button) == selector->priv->active_button) {
    ...
    gimp_pivot_selector_button_to_position(selector, GTK_WIDGET(button), &x,
                                           &y);
...
gimp_pivot_selector_update_active_button(GimpPivotSelector *selector) {
  ...
  button = gimp_pivot_selector_position_to_button(selector, selector->priv->x,
                                                  selector->priv->y);

  if (button != selector->priv->active_button) {
    if (selector->priv->active_button) {
      g_signal_handlers_block_by_func(selector->priv->active_button,
      ...
      g_signal_handlers_unblock_by_func(selector->priv->active_button,
                                        gimp_pivot_selector_button_toggled,
                                        selector);
    }
...
GtkWidget *gimp_pivot_selector_new(gdouble left, gdouble top, gdouble right,
                                   gdouble bottom) {
...
void gimp_pivot_selector_set_bounds(GimpPivotSelector *selector, gdouble left,
...
void gimp_pivot_selector_get_bounds(GimpPivotSelector *selector, gdouble *left,
                                    gdouble *top, gdouble *right,
                                    gdouble *bottom) {
  g_return_if_fail(GIMP_IS_PIVOT_SELECTOR(selector));

  if (left)
    *left = selector->priv->left;
  if (top)
    *top = selector->priv->top;
  if (right)
    *right = selector->priv->right;
  if (bottom)
    *bottom = selector->priv->bottom;
}

void gimp_pivot_selector_set_position(GimpPivotSelector *selector, gdouble x,
                                      gdouble y) {
  g_return_if_fail(GIMP_IS_PIVOT_SELECTOR(selector));
...
void gimp_pivot_selector_get_position(GimpPivotSelector *selector, gdouble *x,
                                      gdouble *y) {
  g_return_if_fail(GIMP_IS_PIVOT_SELECTOR(selector));
