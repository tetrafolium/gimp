...
void gimp_menu_item_set_image(GtkMenuItem *item, GtkWidget *image) {
  GtkWidget *hbox;
  GtkWidget *label;
  GtkWidget *old_image;
...
void gimp_grid_attach_icon(GtkGrid *grid, gint row, const gchar *icon_name,
                           GtkWidget *widget, gint columns) {
  GtkWidget *image;
...
void gimp_enum_radio_box_add(GtkBox *box, GtkWidget *widget, gint enum_value,
                             gboolean below) {
  GList *children;
  GList *list;
  gint pos;
  ...
  for (list = children, pos = 1; list; list = g_list_next(list), pos++) {
    if (GTK_IS_RADIO_BUTTON(list->data) &&
      ...
      if (below) {
        ...
        gtk_widget_set_size_request(spacer,
                                    indicator_size + 3 * indicator_spacing +
      ...
      g_object_bind_property(radio, "active", widget, "sensitive",
...
void gimp_enum_radio_frame_add(GtkFrame *frame, GtkWidget *widget,
                               gint enum_value, gboolean below) {
  GtkWidget *box;
 ...
 **/
GdkPixbuf *gimp_widget_load_icon(GtkWidget *widget, const gchar *icon_name,
                                 gint size) {
  ...
  icon_info = gtk_icon_theme_lookup_icon_for_scale(
      icon_theme, name, size, scale_factor, GTK_ICON_LOOKUP_GENERIC_FALLBACK);
  g_free(name);

  if (icon_info) {
    pixbuf = gtk_icon_info_load_symbolic_for_context(
        icon_info, gtk_widget_get_style_context(widget), NULL, NULL);
  ...
  if (!pixbuf) {
    icon_info = gtk_icon_theme_lookup_icon_for_scale(
        icon_theme, GIMP_ICON_WILBER_EEK "-symbolic", size, scale_factor,
  ...
  if (!pixbuf) {
    ...
    pixbuf = gdk_pixbuf_new_from_data(
        data, GDK_COLORSPACE_RGB, FALSE, 8, size * scale_factor,
        size * scale_factor, rowstride, (GdkPixbufDestroyNotify)g_free, NULL);
  }
...
const gchar *gimp_get_mod_string(GdkModifierType modifiers) {
  ...
  if (!label) {
    ...
    if (accel_label_class->mod_separator && *accel_label_class->mod_separator) {
    ...
    g_hash_table_insert(mod_labels,
                        g_memdup(&modifiers, sizeof(GdkModifierType)), label);
  }
 ...
 **/
gchar *gimp_suggest_modifiers(const gchar *message, GdkModifierType modifiers,
  ...
  if (modifiers & extend_mask) {
    if (extend_selection_format && *extend_selection_format) {
      ...
      g_strlcpy(msg_buf[num_msgs], gimp_get_mod_string(extend_mask), BUF_SIZE);
      try
        = TRUE;
    }
  ...
  if (modifiers & toggle_mask) {
    if (toggle_behavior_format && *toggle_behavior_format) {
      ...
      g_strlcpy(msg_buf[num_msgs], gimp_get_mod_string(toggle_mask), BUF_SIZE);
      try
        = TRUE;
    }
  ...
  if (modifiers & GDK_MOD1_MASK) {
    if (alt_format && *alt_format) {
      ...
      g_strlcpy(msg_buf[num_msgs], gimp_get_mod_string(GDK_MOD1_MASK),
                BUF_SIZE);
      try
        = TRUE;
    }
  ...
  switch (num_msgs) {
  case 1:
    return g_strdup_printf(try ? _("%s (try %s)") : _("%s (%s)"), message,
                           msg_buf[0]);

  case 2:
    return g_strdup_printf(_("%s (try %s, %s)"), message, msg_buf[0],
                           msg_buf[1]);

  case 3:
    return g_strdup_printf(_("%s (try %s, %s, %s)"), message, msg_buf[0],
                           msg_buf[1], msg_buf[2]);
  }
 ...
 **/
void gimp_get_monitor_resolution(GdkMonitor *monitor, gdouble *xres,
                                 gdouble *yres) {
...
gboolean gimp_get_style_color(GtkWidget *widget, const gchar *property_name,
                              GdkRGBA *color) {
  GdkRGBA *c = NULL;
  ...
  gtk_widget_style_get(widget, property_name, &c, NULL);
...
static GdkWindow *gimp_get_foreign_window(guint32 window) {
#ifdef GDK_WINDOWING_X11
...
void gimp_window_set_transient_for(GtkWindow *window, guint32 parent_ID) {
  ...
  g_signal_connect_object(
      window, "realize", G_CALLBACK(gimp_window_transient_realized), parent, 0);

  g_object_unref(parent);
#endif
}

static gboolean gimp_widget_accel_find_func(GtkAccelKey *key, GClosure *closure,
                                            gpointer data) {
  return (GClosure *)data == closure;
}

static void gimp_widget_accel_changed(GtkAccelGroup *accel_group, guint unused1,
  ...
  if (accel_closure == widget_closure) {
    ...
    accel_key = gtk_accel_group_find(accel_group, gimp_widget_accel_find_func,
                                     accel_closure);

    if (accel_key && accel_key->accel_key &&
...
gimp_accel_help_accel_group_weak_notify(gpointer widget,
  ...
  g_object_weak_unref(widget, gimp_accel_help_widget_weak_notify,
...
static void gimp_accel_help_widget_weak_notify(gpointer accel_group,
                                               GObject *where_widget_was) {
  g_object_weak_unref(accel_group, gimp_accel_help_accel_group_weak_notify,
                      where_widget_was);
}

void gimp_widget_set_accel_help(GtkWidget *widget, GimpAction *action) {
  ...
  if (accel_group) {
    g_signal_handlers_disconnect_by_func(accel_group, gimp_widget_accel_changed,
                                         widget);
    g_object_weak_unref(G_OBJECT(accel_group),
                        gimp_accel_help_accel_group_weak_notify, widget);
    g_object_weak_unref(G_OBJECT(widget), gimp_accel_help_widget_weak_notify,
                        accel_group);
  ...
  if (accel_closure) {
    ...
    g_object_set_data(G_OBJECT(widget), "gimp-accel-group", accel_group);
    g_object_weak_ref(G_OBJECT(accel_group),
                      gimp_accel_help_accel_group_weak_notify, widget);
    g_object_weak_ref(G_OBJECT(widget), gimp_accel_help_widget_weak_notify,
                      accel_group);

    g_object_set_data(G_OBJECT(widget), "gimp-accel-closure", accel_closure);
    g_object_set_data(G_OBJECT(widget), "gimp-accel-action", action);

    g_signal_connect_object(accel_group, "accel-changed",
                            G_CALLBACK(gimp_widget_accel_changed), widget, 0);

    gimp_widget_accel_changed(accel_group, 0, 0, accel_closure, widget);
  } else {
    gimp_help_set_help_data(widget, gimp_action_get_tooltip(action),
...
gboolean gimp_get_color_tag_color(GimpColorTag color_tag, GimpRGB *color,
                                  gboolean inherited) {
  static const struct {
    guchar r;
    guchar g;
  ...
  if (color_tag > GIMP_COLOR_TAG_NONE) {
    gimp_rgba_set_uchar(color, colors[color_tag].r, colors[color_tag].g,
                        colors[color_tag].b, 255);
...
void gimp_pango_layout_set_scale(PangoLayout *layout, gdouble scale) {
  PangoAttrList *attrs;
  PangoAttribute *attr;
...
static gboolean gimp_highlight_widget_draw(GtkWidget *widget, cairo_t *cr,
                                           gpointer data) {
 ...
 **/
void gimp_highlight_widget(GtkWidget *widget, gboolean highlight) {
  ...
  old_highlight = GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(widget), "gimp-widget-highlight"));

  if (highlight != old_highlight) {
    if (highlight) {
      g_signal_connect_after(widget, "draw",
                             G_CALLBACK(gimp_highlight_widget_draw), NULL);
    } else {
      g_signal_handlers_disconnect_by_func(widget, gimp_highlight_widget_draw,
                                           NULL);
    }

    g_object_set_data(G_OBJECT(widget), "gimp-widget-highlight",
...
static WidgetBlink *widget_blink_new(void) {
  WidgetBlink *blink;
...
static void widget_blink_free(WidgetBlink *blink) {
  if (blink->timeout_id) {
...
static gboolean gimp_widget_blink_timeout(GtkWidget *widget) {
  WidgetBlink *blink;
...
void gimp_widget_blink(GtkWidget *widget) {
  ...
  blink->timeout_id =
      g_timeout_add(150, (GSourceFunc)gimp_widget_blink_timeout, widget);
...
GtkWidget *gimp_dock_with_window_new(GimpDialogFactory *factory,
                                     GdkMonitor *monitor, gboolean toolbox) {
   ...
   */
  dock_window = gimp_dialog_factory_dialog_new(
      factory, monitor, NULL /*ui_manager*/, NULL,
      (toolbox ? "gimp-toolbox-window" : "gimp-dock-window"), -1 /*view_size*/,
      FALSE /*present*/);

  dock_container = GIMP_DOCK_CONTAINER(dock_window);
  ui_manager = gimp_dock_container_get_ui_manager(dock_container);
  dock =
      gimp_dialog_factory_dialog_new(factory, monitor, ui_manager, dock_window,
                                     (toolbox ? "gimp-toolbox" : "gimp-dock"),
                                     -1 /*view_size*/, FALSE /*present*/);

  if (dock)
    gimp_dock_window_add_dock(GIMP_DOCK_WINDOW(dock_window), GIMP_DOCK(dock),
                              -1);
...
GtkWidget *gimp_tools_get_tool_options_gui(GimpToolOptions *tool_options) {
  ...
  widget = g_object_get_data(G_OBJECT(tool_options), GIMP_TOOL_OPTIONS_GUI_KEY);
...
void gimp_tools_set_tool_options_gui(GimpToolOptions *tool_options,
  ...
  prev_widget =
      g_object_get_data(G_OBJECT(tool_options), GIMP_TOOL_OPTIONS_GUI_KEY);
  ...
  g_object_set_data_full(G_OBJECT(tool_options), GIMP_TOOL_OPTIONS_GUI_KEY,
                         widget ? g_object_ref_sink(widget) : NULL,
                         widget ? (GDestroyNotify)g_object_unref : NULL);
}

void gimp_tools_set_tool_options_gui_func(GimpToolOptions *tool_options,
                                          GimpToolOptionsGUIFunc func) {
  g_object_set_data(G_OBJECT(tool_options), GIMP_TOOL_OPTIONS_GUI_FUNC_KEY,
                    func);
}
...
gboolean gimp_widget_get_fully_opaque(GtkWidget *widget) {
  ...
  return g_object_get_data(G_OBJECT(widget), "gimp-widget-fully-opaque") !=
         NULL;
}

void gimp_widget_set_fully_opaque(GtkWidget *widget, gboolean fully_opaque) {
  g_return_if_fail(GTK_IS_WIDGET(widget));

  return g_object_set_data(G_OBJECT(widget), "gimp-widget-fully-opaque",
                           GINT_TO_POINTER(fully_opaque));
}

static void gimp_gtk_container_clear_callback(GtkWidget *widget,
...
void gimp_gtk_container_clear(GtkContainer *container) {
  gtk_container_foreach(
      container, (GtkCallback)gimp_gtk_container_clear_callback, container);
}

void gimp_button_set_suggested(GtkWidget *button, gboolean suggested,
...
void gimp_button_set_destructive(GtkWidget *button, gboolean destructive,
                                 GtkReliefStyle default_relief) {
  GtkStyleContext *style;
...
void gimp_gtk_adjustment_chain(GtkAdjustment *adjustment1,
  ...
  g_object_bind_property(adjustment1, "value", adjustment2, "lower",
                         G_BINDING_SYNC_CREATE);
  g_object_bind_property(adjustment2, "value", adjustment1, "upper",
                         G_BINDING_SYNC_CREATE);
}

static gboolean gimp_print_event_free(gpointer data) {
  g_free(data);
...
const gchar *gimp_print_event(const GdkEvent *event) {
  ...
  case GDK_ENTER_NOTIFY:
    str = g_strdup_printf("ENTER_NOTIFY (mode %d)", event->crossing.mode);
    break;

  case GDK_LEAVE_NOTIFY:
    str = g_strdup_printf("LEAVE_NOTIFY (mode %d)", event->crossing.mode);
    break;
  ...
  case GDK_BUTTON_PRESS:
    str =
        g_strdup_printf("BUTTON_PRESS (%d @ %0.0f:%0.0f)", event->button.button,
                        event->button.x, event->button.y);
    break;

  case GDK_2BUTTON_PRESS:
    str =
        g_strdup_printf("2BUTTON_PRESS (%d @ %0.0f:%0.0f)",
                        event->button.button, event->button.x, event->button.y);
    break;

  case GDK_3BUTTON_PRESS:
    str =
        g_strdup_printf("3BUTTON_PRESS (%d @ %0.0f:%0.0f)",
                        event->button.button, event->button.x, event->button.y);
    break;

  case GDK_BUTTON_RELEASE:
    str =
        g_strdup_printf("BUTTON_RELEASE (%d @ %0.0f:%0.0f)",
                        event->button.button, event->button.x, event->button.y);
    break;

  case GDK_SCROLL:
    str = g_strdup_printf("SCROLL (%d)", event->scroll.direction);
    break;

  case GDK_MOTION_NOTIFY:
    str = g_strdup_printf("MOTION_NOTIFY (%0.0f:%0.0f %d)", event->motion.x,
                          event->motion.y, event->motion.time);
    break;

  case GDK_KEY_PRESS:
    str = g_strdup_printf("KEY_PRESS (%d, %s)", event->key.keyval,
  ...
  case GDK_KEY_RELEASE:
    str = g_strdup_printf("KEY_RELEASE (%d, %s)", event->key.keyval,
                          gdk_keyval_name(event->key.keyval)
                              ? gdk_keyval_name(event->key.keyval)
                              : "<none>");
    break;
  ...
  default:
    str = g_strdup_printf("UNHANDLED (type %d)", event->type);
    break;
  }

  tmp =
      g_strdup_printf("%s (device '%s', source device '%s')", str,
...
gboolean gimp_color_profile_store_add_defaults(GimpColorProfileStore *store,
  ...
  profile =
      gimp_babl_get_builtin_color_profile(base_type, gimp_babl_trc(precision));
...
static void connect_path_show(GimpColorProfileChooserDialog *dialog) {
...
static void connect_path_response(GimpColorProfileChooserDialog *dialog,
                                  gint response) {
...
void gimp_color_profile_chooser_dialog_connect_path(
    GtkWidget *dialog, GObject *config, const gchar *property_name) {
  ...
  g_object_set_data_full(G_OBJECT(dialog), "profile-path-config",
                         g_object_ref(config), (GDestroyNotify)g_object_unref);
  g_object_set_data_full(G_OBJECT(dialog), "profile-path-property",
                         g_strdup(property_name), (GDestroyNotify)g_free);

  g_signal_connect(dialog, "show", G_CALLBACK(connect_path_show), NULL);
  g_signal_connect(dialog, "response", G_CALLBACK(connect_path_response), NULL);
}
