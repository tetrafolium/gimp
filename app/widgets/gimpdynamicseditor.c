...
static void gimp_dynamics_editor_class_init(GimpDynamicsEditorClass *klass) {
...
static void gimp_dynamics_editor_constructed(GObject *object) {
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(editor->notebook), vbox, NULL);
  ...
  gimp_dynamics_editor_add_icon_editor(dynamics, data_editor->context->gimp,
                                       vbox);
  ...
  gimp_dynamics_editor_init_output_editors(dynamics, editor->view_selector,
                                           editor->notebook, grid);
  ...
  gimp_int_combo_box_prepend(GIMP_INT_COMBO_BOX(editor->view_selector),
                             GIMP_INT_STORE_VALUE, -1, GIMP_INT_STORE_LABEL,
                             _("Mapping matrix"), GIMP_INT_STORE_USER_DATA,
                             vbox, -1);
...
static void gimp_dynamics_editor_set_data(GimpDataEditor *editor,
  ...
  if (editor->data)
    g_signal_handlers_disconnect_by_func(
        editor->data, gimp_dynamics_editor_notify_data, editor);

  GIMP_DATA_EDITOR_CLASS(parent_class)->set_data(editor, data);

  if (editor->data) {
    g_signal_handlers_block_by_func(dynamics_editor->dynamics_model,
                                    gimp_dynamics_editor_notify_model, editor);
    ...
    g_signal_connect(editor->data, "notify",
                     G_CALLBACK(gimp_dynamics_editor_notify_data), editor);
  }
...
GtkWidget *gimp_dynamics_editor_new(GimpContext *context,
  ...
  return g_object_new(GIMP_TYPE_DYNAMICS_EDITOR, "menu-factory", menu_factory,
                      "menu-identifier", "<DynamicsEditor>", "ui-path",
                      "/dynamics-editor-popup", "data-factory",
                      context->gimp->dynamics_factory, "context", context,
                      "data", gimp_context_get_dynamics(context), NULL);
}
...
static void gimp_dynamics_editor_notify_model(GimpDynamics *options,
  ...
  if (data_editor->data) {
    g_signal_handlers_block_by_func(data_editor->data,
                                    gimp_dynamics_editor_notify_data, editor);
...
static void gimp_dynamics_editor_add_output_row(GObject *config,
  ...
  dynamics_check_button_new(config, "use-pressure", grid, column, row);
  column++;

  dynamics_check_button_new(config, "use-velocity", grid, column, row);
  column++;

  dynamics_check_button_new(config, "use-direction", grid, column, row);
  column++;

  dynamics_check_button_new(config, "use-tilt", grid, column, row);
  column++;

  dynamics_check_button_new(config, "use-wheel", grid, column, row);
  column++;

  dynamics_check_button_new(config, "use-random", grid, column, row);
  column++;

  dynamics_check_button_new(config, "use-fade", grid, column, row);
  column++;
}
...
static void gimp_dynamics_editor_init_output_editors(GimpDynamics *dynamics,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter), i = 1;
       iter_valid; iter_valid = gtk_tree_model_iter_next(model, &iter), i++) {
    gint output_type;
    gchar *label;
    GimpDynamicsOutput *output;
    GtkWidget *output_editor;

    gtk_tree_model_get(GTK_TREE_MODEL(model), &iter, GIMP_INT_STORE_VALUE,
                       &output_type, GIMP_INT_STORE_LABEL, &label, -1);
    ...
    gtk_list_store_set(GTK_LIST_STORE(list), &iter, GIMP_INT_STORE_USER_DATA,
                       output_editor, -1);

    gimp_dynamics_editor_add_output_row(G_OBJECT(output), label,
                                        GTK_GRID(check_grid), i);
...
static void gimp_dynamics_editor_view_changed(GtkComboBox *combo,
  ...
  gtk_tree_model_get(model, &iter, GIMP_INT_STORE_USER_DATA, &widget, -1);
