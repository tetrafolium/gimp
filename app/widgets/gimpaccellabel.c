...
static void gimp_accel_label_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_accel_label_get_property(GObject *object, guint property_id,
...
static void gimp_accel_label_class_init(GimpAccelLabelClass *klass) {
...
static void gimp_accel_label_init(GimpAccelLabel *accel_label) {
...
static gboolean gimp_accel_label_update_accel_find_func(GtkAccelKey *key,
                                                        GClosure *closure,
                                                        gpointer data) {
...
static void gimp_accel_label_update(GimpAccelLabel *accel_label) {
  ...
  accel_key = gtk_accel_group_find(
      accel_group, gimp_accel_label_update_accel_find_func, accel_closure);
  ...
  if (accel_key && accel_key->accel_key &&
    ...
    label =
        gtk_accelerator_get_label(accel_key->accel_key, accel_key->accel_mods);
...
GtkWidget *gimp_accel_label_new(GimpAction *action) {
  ...
  return g_object_new(GIMP_TYPE_ACCEL_LABEL, "action", action, NULL);
}
    ...
    if (accel_label->priv->action) {
      ...
      if (accel_closure) {
        ...
        g_signal_handlers_disconnect_by_func(
            accel_group, gimp_accel_label_accel_changed, accel_label);
      }
    }
