...
static gboolean gimp_combo_tag_entry_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_combo_tag_entry_tag_count_changed(
    GimpTaggedContainer *container, gint tag_count, GimpComboTagEntry *entry);
...
static void gimp_combo_tag_entry_class_init(GimpComboTagEntryClass *klass) {
...
static void gimp_combo_tag_entry_init(GimpComboTagEntry *entry) {
  entry->popup = NULL;
  ...
  g_signal_connect(entry, "icon-press",
                   G_CALLBACK(gimp_combo_tag_entry_icon_press), NULL);
}
...
static void gimp_combo_tag_entry_constructed(GObject *object) {
  ...
  g_signal_connect_object(GIMP_TAG_ENTRY(entry)->container, "tag-count-changed",
...
GtkWidget *gimp_combo_tag_entry_new(GimpTaggedContainer *container,
  ...
  return g_object_new(GIMP_TYPE_COMBO_TAG_ENTRY, "container", container, "mode",
                      mode, NULL);
}
  ...
  if (!entry->popup) {
    ...
    if (tag_count > 0 && !GIMP_TAG_ENTRY(entry)->has_invalid_tags) {
      ...
      g_signal_connect(entry->popup, "destroy",
                       G_CALLBACK(gimp_combo_tag_entry_popup_destroy), entry);
