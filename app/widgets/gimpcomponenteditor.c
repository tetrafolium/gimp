...
static void gimp_component_editor_alpha_changed(GimpImage *image,
                                                GimpComponentEditor *editor);
static void gimp_component_editor_visibility_changed(
    GimpImage *image, GimpChannelType channel, GimpComponentEditor *editor);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpComponentEditor, gimp_component_editor, GIMP_TYPE_IMAGE_EDITOR,
...
static void gimp_component_editor_init(GimpComponentEditor *editor) {
  ...
  list = gtk_list_store_new(N_COLUMNS, G_TYPE_INT, G_TYPE_BOOLEAN,
                            GIMP_TYPE_VIEW_RENDERER, G_TYPE_STRING);
  ...
  gtk_tree_view_column_pack_start(editor->eye_column, editor->eye_cell, FALSE);
  ...
  g_signal_connect(editor->eye_cell, "clicked",
                   G_CALLBACK(gimp_component_editor_clicked), editor);

  editor->renderer_cell = gimp_cell_renderer_viewable_new();
  gtk_tree_view_insert_column_with_attributes(editor->view, -1, NULL,
                                              editor->renderer_cell, "renderer",
                                              COLUMN_RENDERER, NULL);
  ...
  g_signal_connect(editor->view, "button-press-event",
                   G_CALLBACK(gimp_component_editor_button_press), editor);
  ...
  gtk_tree_selection_set_select_function(
      editor->selection, gimp_component_editor_select, editor, NULL);

  gimp_dnd_component_source_add(GTK_WIDGET(editor->view),
                                gimp_component_editor_drag_component, editor);
}
...
static void gimp_component_editor_set_context(GimpDocked *docked,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(editor->model, &iter);
    ...
    gtk_tree_model_get(editor->model, &iter, COLUMN_RENDERER, &renderer, -1);
...
static void gimp_component_editor_set_image(GimpImageEditor *editor,
  ...
  if (editor->image) {
    ...
    g_signal_handlers_disconnect_by_func(
        editor->image, gimp_component_editor_mode_changed, component_editor);
    g_signal_handlers_disconnect_by_func(
        editor->image, gimp_component_editor_alpha_changed, component_editor);
    g_signal_handlers_disconnect_by_func(
        editor->image, gimp_component_editor_visibility_changed,
        component_editor);
    g_signal_handlers_disconnect_by_func(
        editor->image, gimp_component_editor_active_changed, component_editor);
  }
...
GtkWidget *gimp_component_editor_new(gint view_size,
  ...
  editor = g_object_new(GIMP_TYPE_COMPONENT_EDITOR, "menu-factory",
                        menu_factory, "menu-identifier", "<Channels>",
                        "ui-path", "/channels-popup", NULL);
...
gimp_component_editor_create_components(GimpComponentEditor *editor) {
  ...
  for (i = 0; i < n_components; i++) {
    ...
    g_signal_connect(renderer, "update",
                     G_CALLBACK(gimp_component_editor_renderer_update), editor);
    ...
    gtk_list_store_set(GTK_LIST_STORE(editor->model), &iter, COLUMN_CHANNEL,
                       components[i], COLUMN_VISIBLE, visible, COLUMN_RENDERER,
                       renderer, COLUMN_NAME, desc, -1);
...
gimp_component_editor_clicked(GtkCellRendererToggle *cellrenderertoggle,
  ...
  if (gtk_tree_model_get_iter(editor->model, &iter, path)) {
    ...
    gtk_tree_model_get(editor->model, &iter, COLUMN_CHANNEL, &channel, -1);
    g_object_get(cellrenderertoggle, "active", &active, NULL);
...
static gboolean
gimp_component_editor_button_press(GtkWidget *widget, GdkEventButton *bevent,
                                   GimpComponentEditor *editor) {
...
static gboolean gimp_component_editor_get_iter(GimpComponentEditor *editor,
                                               GimpChannelType channel,
                                               GtkTreeIter *iter) {
  gint index;

  index =
      gimp_image_get_component_index(GIMP_IMAGE_EDITOR(editor)->image, channel);
...
static void gimp_component_editor_visibility_changed(
  ...
  if (gimp_component_editor_get_iter(editor, channel, &iter)) {
    ...
    gtk_list_store_set(GTK_LIST_STORE(editor->model), &iter, COLUMN_VISIBLE,
                       visible, -1);
  }
}
...
static GimpImage *gimp_component_editor_drag_component(GtkWidget *widget,
  ...
  if (GIMP_IMAGE_EDITOR(editor)->image && editor->clicked_component != -1) {
    if (channel)
