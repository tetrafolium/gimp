...
static void gimp_file_proc_view_class_init(GimpFileProcViewClass *klass) {
  ...
  view_signals[CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpFileProcViewClass, changed), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);
}

static void gimp_file_proc_view_init(GimpFileProcView *view) {}
...
GtkWidget *gimp_file_proc_view_new(Gimp *gimp, GSList *procedures,
  ...
  view = g_object_new(GIMP_TYPE_FILE_PROC_VIEW, "model", store, "rules-hint",
                      TRUE, NULL);
  ...
  for (list = procedures; list; list = g_slist_next(list)) {
    ...
    if (!proc->prefixes_list) /*  skip URL loaders  */
    {
      ...
      if (label) {
        ...
        gtk_list_store_set(store, &iter, COLUMN_PROC, proc, COLUMN_LABEL, label,
                           COLUMN_EXTENSIONS, proc->extensions, COLUMN_HELP_ID,
                           help_id, COLUMN_FILTER, filter, -1);
        g_object_unref(filter);
      }

      for (list2 = proc->extensions_list; list2; list2 = g_slist_next(list2)) {
        ...
        if (dot && dot != ext)
          proc_view->meta_extensions =
              g_list_append(proc_view->meta_extensions, g_strdup(dot + 1));
      }
    }
  }
  ...
  if (automatic) {
    ...
    gtk_list_store_set(store, &iter, COLUMN_PROC, NULL, COLUMN_LABEL, automatic,
                       COLUMN_HELP_ID, automatic_help_id, COLUMN_FILTER,
                       all_filter, -1);
  }
  ...
  cell = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(column, cell, TRUE);
  gtk_tree_view_column_set_attributes(column, cell, "text", COLUMN_EXTENSIONS,
                                      NULL);

  gtk_tree_view_append_column(view, column);

  g_signal_connect(gtk_tree_view_get_selection(view), "changed",
                   G_CALLBACK(gimp_file_proc_view_selection_changed), view);
...
GimpPlugInProcedure *gimp_file_proc_view_get_proc(GimpFileProcView *view,
  ...
  gtk_tree_model_get(model, &iter, COLUMN_PROC, &proc, -1);
...
gboolean gimp_file_proc_view_set_proc(GimpFileProcView *view,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    gtk_tree_model_get(model, &iter, COLUMN_PROC, &this, -1);
...
gchar *gimp_file_proc_view_get_help_id(GimpFileProcView *view) {
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, COLUMN_HELP_ID, &help_id, -1);
...
gimp_file_proc_view_process_procedure(GimpPlugInProcedure *file_proc,
  ...
  for (list = file_proc->extensions_list, i = 0; list;
