...
static void gimp_action_impl_set_property(GObject *object, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_action_impl_get_property(GObject *object, guint prop_id,
                                          GValue *value, GParamSpec *pspec);

static void gimp_action_impl_activate(GtkAction *action);
static void gimp_action_impl_connect_proxy(GtkAction *action, GtkWidget *proxy);

static void gimp_action_impl_set_proxy(GimpActionImpl *impl, GtkWidget *proxy);
...
static void gimp_action_impl_class_init(GimpActionImplClass *klass) {
  ...
  g_object_class_install_property(object_class, PROP_COLOR,
                                  gimp_param_spec_rgb("color", NULL, NULL, TRUE,
                                                      &black,
                                                      GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(object_class, PROP_MAX_WIDTH_CHARS,
                                  g_param_spec_int("max-width-chars", NULL,
                                                   NULL, -1, G_MAXINT, -1,
                                                   GIMP_PARAM_READWRITE));
}

static void gimp_action_impl_init(GimpActionImpl *impl) {
...
static void gimp_action_impl_set_property(GObject *object, guint prop_id,
  ...
  if (set_proxy) {
    ...
    for (list = gimp_action_get_proxies(GIMP_ACTION(impl)); list;
...
GimpAction *gimp_action_impl_new(const gchar *name, const gchar *label,
                                 const gchar *tooltip, const gchar *icon_name,
                                 const gchar *help_id) {
  GimpAction *action;

  action = g_object_new(GIMP_TYPE_ACTION_IMPL, "name", name, "label", label,
...
static void gimp_action_impl_set_proxy(GimpActionImpl *impl, GtkWidget *proxy) {
  ...
  if (impl->color) {
    ...
    if (GIMP_IS_COLOR_AREA(area)) {
      ...
      area = gimp_color_area_new(impl->color, GIMP_COLOR_AREA_SMALL_CHECKS, 0);
      gimp_color_area_set_draw_border(GIMP_COLOR_AREA(area), TRUE);

      if (impl->context)
        gimp_color_area_set_color_config(
            GIMP_COLOR_AREA(area),
    ...
    if (GIMP_IS_VIEW(view) &&
      ...
      view = gimp_view_new_full(impl->context, impl->viewable, width, height,
                                border_width, FALSE, FALSE, FALSE);
