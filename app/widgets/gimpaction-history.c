...
gimp_action_history_item_new(const gchar *action_name, gint index, gint delta);
      ...
      if (scanner->value.v_symbol == GINT_TO_POINTER(HISTORY_ITEM)) {
        ...
        if (!gimp_action_history_is_excluded_action(action_name) &&
          ...
          item = gimp_action_history_item_new(
              action_name, g_queue_get_length(history.items), delta);

          g_queue_push_tail(history.items, item);

          g_hash_table_insert(history.links, item->action_name,
...
void gimp_action_history_exit(Gimp *gimp) {
  ...
  writer =
      gimp_config_writer_new_from_file(file, TRUE, "GIMP action-history", NULL);
  g_object_unref(file);
...
void gimp_action_history_action_activated(GimpAction *action) {
  ...
  if (!link) {
    ...
    item = gimp_action_history_item_new(action_name,
                                        g_queue_get_length(history.items), 0);
