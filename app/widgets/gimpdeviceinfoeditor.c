...
enum { INPUT_COLUMN_INDEX, INPUT_COLUMN_NAME, INPUT_N_COLUMNS };
...
#define GIMP_DEVICE_INFO_EDITOR_GET_PRIVATE(editor)                            \
  ((GimpDeviceInfoEditorPrivate *)                                             \
       gimp_device_info_editor_get_instance_private(                           \
...
gimp_device_info_editor_class_init(GimpDeviceInfoEditorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("info", NULL, NULL, GIMP_TYPE_DEVICE_INFO,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_device_info_editor_init(GimpDeviceInfoEditor *editor) {
  GimpDeviceInfoEditorPrivate *private;

private
  = GIMP_DEVICE_INFO_EDITOR_GET_PRIVATE(editor);
...
private
  ->axis_store = gtk_list_store_new(AXIS_N_COLUMNS,
                                    /* Axis index. */
                                    G_TYPE_INT,
                                    /* Axis name.  */
                                    G_TYPE_STRING,
...
private
  = GIMP_DEVICE_INFO_EDITOR_GET_PRIVATE(object);
  ...
  for (i = 0; i < n_axes; i++) {
    ...
    gtk_list_store_insert_with_values(
         ...
         * row otherwise.
         */
        (!has_axes || has_curve) ? &axis_iter : NULL, -1, AXIS_COLUMN_INDEX, i,
        AXIS_COLUMN_NAME, gettext(axis_name), -1);
    has_axes = TRUE;
  }
  ...
  if (has_axes) {
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(view), -1, NULL,
                                                ...
                                                "text", AXIS_COLUMN_NAME, NULL);

  private
    ->input_store = gtk_list_store_new(INPUT_N_COLUMNS,
                                       /* Axis real index.    */
                                       G_TYPE_INT,
                                       /* Axis printed index. */
                                       G_TYPE_STRING);
    ...
    g_object_set(cell, "mode", GTK_CELL_RENDERER_MODE_EDITABLE, "editable",
                 TRUE, "model", private->input_store, "text-column",
                 INPUT_COLUMN_NAME, "has-entry", FALSE, NULL);

    g_object_unref(private->input_store);

    g_signal_connect(cell, "changed",
                     G_CALLBACK(gimp_device_info_editor_axis_changed), editor);

    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(view), -1, NULL,
                                                cell, "text",
                                                AXIS_COLUMN_INPUT_NAME, NULL);
  ...
  combo = gimp_prop_enum_combo_box_new(G_OBJECT(private->info), "mode", 0, 0);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Mode:"), 0.0, 0.5, combo,
                           1);

  label = gimp_prop_enum_label_new(G_OBJECT(private->info), "source");
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Source:"), 0.0, 0.5, label,
                           1);

  label = gimp_prop_label_new(G_OBJECT(private->info), "vendor-id");
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Vendor ID:"), 0.0, 0.5,
                           label, 1);

  label = gimp_prop_label_new(G_OBJECT(private->info), "product-id");
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("Product ID:"), 0.0, 0.5,
                           label, 1);

  label = gimp_prop_enum_label_new(G_OBJECT(private->info), "tool-type");
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 4, _("Tool type:"), 0.0, 0.5,
                           label, 1);

  label = gimp_prop_label_new(G_OBJECT(private->info), "tool-serial");
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 5, _("Tool serial:"), 0.0, 0.5,
                           label, 1);

  label = gimp_prop_label_new(G_OBJECT(private->info), "tool-hardware-id");
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 6, _("Tool hardware ID:"), 0.0,
                           0.5, label, 1);
  ...
  private
    ...
    for (i = GDK_AXIS_X; i < GDK_AXIS_LAST; i++) {
      ...
      if (curve) {
        ...
        g_object_set(view, "gimp", GIMP_TOOL_PRESET(private->info)->gimp,
                     "border-width", CURVE_BORDER, NULL);
        gtk_widget_set_size_request(view, CURVE_SIZE + CURVE_BORDER * 2,
        ...
        combo =
            gimp_prop_enum_combo_box_new(G_OBJECT(curve), "curve-type", 0, 0);
    ...
    g_signal_connect(sel, "changed",
                     G_CALLBACK(gimp_device_info_editor_axis_selected), editor);
...
private
  ...
  if (gtk_tree_model_get_iter(GTK_TREE_MODEL(private->axis_store),
    ...
    if ((new_axis == -1 && (new_use == GDK_AXIS_X || new_use == GDK_AXIS_Y)) ||
...
private
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    ...
    gtk_notebook_set_current_page(GTK_NOTEBOOK(private->notebook), axis_index);
  }
}
...
static gboolean gimp_device_info_editor_foreach(GtkTreeModel *model,
  ...
  gtk_tree_model_get(model, iter, AXIS_COLUMN_INDEX, &axe_index, -1);
  ...
  gtk_list_store_set(private->axis_store, iter, AXIS_COLUMN_INPUT_NAME,
                     input_name, -1);
...
GtkWidget *gimp_device_info_editor_new(GimpDeviceInfo *info) {
  ...
  return g_object_new(GIMP_TYPE_DEVICE_INFO_EDITOR, "info", info, NULL);
}
