...
static void gimp_view_renderer_image_render(GimpViewRenderer *renderer,
  ...
  if (rendererimage->channel == -1 ||
    ...
    gimp_viewable_calc_preview_size(
        width, height, renderer->width, renderer->height, renderer->dot_for_dot,
        xres, yres, &view_width, &view_height, &scaling_up);

    if (scaling_up) {
      GimpTempBuf *temp_buf;

      temp_buf = gimp_viewable_get_new_preview(
          renderer->viewable, renderer->context, width, height);
    ...
    if (render_buf) {
      ...
      if (view_width > renderer->width || view_height > renderer->height) {
        ...
        temp_buf =
            gimp_temp_buf_scale(render_buf, renderer->width, renderer->height);
      ...
      gimp_view_renderer_render_temp_buf(
          renderer, widget, render_buf, render_buf_x, render_buf_y,
          component_index, GIMP_VIEW_BG_CHECKS, GIMP_VIEW_BG_WHITE);
      gimp_temp_buf_unref(render_buf);
  ...
  case GIMP_CHANNEL_RED:
    icon_name = GIMP_ICON_CHANNEL_RED;
    break;
  case GIMP_CHANNEL_GREEN:
    icon_name = GIMP_ICON_CHANNEL_GREEN;
    break;
  case GIMP_CHANNEL_BLUE:
    icon_name = GIMP_ICON_CHANNEL_BLUE;
    break;
  case GIMP_CHANNEL_GRAY:
    icon_name = GIMP_ICON_CHANNEL_GRAY;
    break;
  case GIMP_CHANNEL_INDEXED:
    icon_name = GIMP_ICON_CHANNEL_INDEXED;
    break;
  case GIMP_CHANNEL_ALPHA:
    icon_name = GIMP_ICON_CHANNEL_ALPHA;
    break;
