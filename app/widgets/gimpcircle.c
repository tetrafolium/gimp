...
enum { PROP_0, PROP_SIZE, PROP_BORDER_WIDTH, PROP_BACKGROUND };
...
static void gimp_circle_set_property(GObject *object, guint property_id,
                                     const GValue *value, GParamSpec *pspec);
static void gimp_circle_get_property(GObject *object, guint property_id,
...
static gboolean gimp_circle_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_circle_background_hsv(gdouble angle, gdouble distance,
                                       guchar *rgb);

static void gimp_circle_draw_background(GimpCircle *circle, cairo_t *cr,
                                        gint size,
                                        GimpCircleBackground background);
...
static void gimp_circle_class_init(GimpCircleClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int("size", NULL, NULL, 32, 1024, 96,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_BORDER_WIDTH,
      g_param_spec_int("border-width", NULL, NULL, 0, 64, 0,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_BACKGROUND,
      g_param_spec_enum("background", NULL, NULL, GIMP_TYPE_CIRCLE_BACKGROUND,
                        GIMP_CIRCLE_BACKGROUND_HSV,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_circle_init(GimpCircle *circle) {
...
static void gimp_circle_get_preferred_width(GtkWidget *widget,
  ...
  *minimum_width = *natural_width =
      2 * circle->priv->border_width + circle->priv->size;
}
...
static void gimp_circle_size_allocate(GtkWidget *widget,
  ...
  if (gtk_widget_get_realized(widget))
    gdk_window_move_resize(circle->priv->event_window, allocation->x,
...
static gboolean gimp_circle_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  cairo_translate(cr, (allocation.width - size) / 2,
...
static gdouble get_angle_and_distance(gdouble center_x, gdouble center_y,
                                      gdouble radius, gdouble x, gdouble y,
                                      gdouble *distance) {
  gdouble angle = atan2(center_y - y, x - center_x);

  if (angle < 0)
    angle += 2 * G_PI;

  if (distance)
    *distance = sqrt((SQR(x - center_x) + SQR(y - center_y)) / SQR(radius));
...
gdouble _gimp_circle_get_angle_and_distance(GimpCircle *circle, gdouble event_x,
                                            gdouble event_y,
                                            gdouble *distance) {
  GtkAllocation allocation;
  gdouble center_x;
...
static void gimp_circle_background_hsv(gdouble angle, gdouble distance,
  ...
  gimp_hsv_set(&hsv, angle / (2.0 * G_PI), distance,
    ...
    if (!circle->priv->surface) {
      ...
      for (y = 0; y < size; y++) {
        for (x = 0; x < size; x++) {
          ...
          angle = get_angle_and_distance(size / 2.0, size / 2.0, size / 2.0, x,
                                         y, &distance);
          ...
          GIMP_CAIRO_ARGB32_SET_PIXEL(data + y * stride + x * 4, rgb[0], rgb[1],
                                      rgb[2], 255);
        }
      }
