...
static GdkPixbuf *gimp_view_renderer_get_frame(GimpViewRenderer *renderer,
                                               gint width, gint height) {
  GimpViewRendererClass *class = GIMP_VIEW_RENDERER_GET_CLASS(renderer);

  return stretch_frame_image(class->frame, class->frame_left, class->frame_top,
                             class->frame_right, class->frame_bottom, width,
                             height);
}

static void gimp_view_renderer_ensure_frame(GimpViewRenderer *renderer,
                                            GtkWidget *widget) {
  GimpViewRendererClass *class = GIMP_VIEW_RENDERER_GET_CLASS(renderer);
...
GdkPixbuf *gimp_view_renderer_get_frame_pixbuf(GimpViewRenderer *renderer,
  ...
  if (w > 12 && h > 12) {
    pixbuf =
        gimp_viewable_get_pixbuf(renderer->viewable, renderer->context, w, h);
    if (!pixbuf)
      return NULL;
    ...
    frame = gimp_view_renderer_get_frame(renderer, w + x + class->frame_right,
 ...
 */
void gimp_view_renderer_get_frame_size(gint *horizontal, gint *vertical) {
  GimpViewRendererClass *class;
