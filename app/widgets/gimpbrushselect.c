...
enum { PROP_0, PROP_OPACITY, PROP_PAINT_MODE, PROP_SPACING };

static void gimp_brush_select_constructed(GObject *object);
static void gimp_brush_select_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
...
static void gimp_brush_select_class_init(GimpBrushSelectClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_PAINT_MODE,
      g_param_spec_enum("paint-mode", NULL, NULL, GIMP_TYPE_LAYER_MODE,
                        GIMP_LAYER_MODE_NORMAL,
                        GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_SPACING,
      g_param_spec_int("spacing", NULL, NULL, -G_MAXINT, 1000, -1,
                       GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT));
}

static void gimp_brush_select_init(GimpBrushSelect *select) {}
...
static void gimp_brush_select_constructed(GObject *object) {
  ...
  g_signal_connect(dialog->context, "opacity-changed",
                   G_CALLBACK(gimp_brush_select_opacity_changed), dialog);
  g_signal_connect(dialog->context, "paint-mode-changed",
                   G_CALLBACK(gimp_brush_select_mode_changed), dialog);

  dialog->view = gimp_brush_factory_view_new(
      GIMP_VIEW_TYPE_GRID, dialog->context->gimp->brush_factory,
      dialog->context, FALSE, GIMP_VIEW_SIZE_MEDIUM, 1, dialog->menu_factory);

  gimp_container_box_set_size_request(
      GIMP_CONTAINER_BOX(GIMP_CONTAINER_EDITOR(dialog->view)->view),
  ...
  select->opacity_data =
      gtk_adjustment_new(gimp_context_get_opacity(dialog->context) * 100.0, 0.0,
                         100.0, 1.0, 10.0, 0.0);

  scale = gimp_spin_scale_new(select->opacity_data, _("Opacity"), 1);
  ...
  g_signal_connect(select->opacity_data, "value-changed",
                   G_CALLBACK(gimp_brush_select_opacity_update), select);
  ...
  select->layer_mode_box =
      gimp_layer_mode_box_new(GIMP_LAYER_MODE_CONTEXT_PAINT);
  ...
  g_signal_connect(spacing_adj, "value-changed",
                   G_CALLBACK(gimp_brush_select_spacing_update), select);
}
    ...
    if (dialog->view) {
      if (g_value_get_int(value) >= 0)
        gtk_adjustment_set_value(
            GIMP_BRUSH_FACTORY_VIEW(dialog->view)->spacing_adjustment,
...
static GimpValueArray *gimp_brush_select_run_callback(GimpPdbDialog *dialog,
  ...
  return_vals = gimp_pdb_execute_procedure_by_name(
      dialog->pdb, dialog->caller_context, NULL, error, dialog->callback_name,
      G_TYPE_STRING, gimp_object_get_name(object), G_TYPE_DOUBLE,
      gimp_context_get_opacity(dialog->context) * 100.0, G_TYPE_INT,
      GIMP_BRUSH_SELECT(dialog)->spacing, GIMP_TYPE_LAYER_MODE,
      gimp_context_get_paint_mode(dialog->context), G_TYPE_INT,
      gimp_brush_get_width(brush), G_TYPE_INT, gimp_brush_get_height(brush),
      G_TYPE_INT, array->length, GIMP_TYPE_UINT8_ARRAY, array, G_TYPE_BOOLEAN,
      closing, G_TYPE_NONE);
...
static void gimp_brush_select_opacity_changed(GimpContext *context,
  ...
  g_signal_handlers_block_by_func(select->opacity_data,
                                  gimp_brush_select_opacity_update, select);
...
static void gimp_brush_select_opacity_update(GtkAdjustment *adjustment,
                                             GimpBrushSelect *select) {
  gimp_context_set_opacity(GIMP_PDB_DIALOG(select)->context,
                           gtk_adjustment_get_value(adjustment) / 100.0);
}

static void gimp_brush_select_spacing_update(GtkAdjustment *adjustment,
                                             GimpBrushSelect *select) {
