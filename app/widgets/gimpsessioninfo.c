...
G_DEFINE_TYPE_WITH_CODE(GimpSessionInfo, gimp_session_info, GIMP_TYPE_OBJECT,
                        G_ADD_PRIVATE(GimpSessionInfo) G_IMPLEMENT_INTERFACE(
                            GIMP_TYPE_CONFIG,
                            gimp_session_info_config_iface_init))

#define parent_class gimp_session_info_parent_class

static void gimp_session_info_class_init(GimpSessionInfoClass *klass) {
...
static void gimp_session_info_init(GimpSessionInfo *info) {
...
static gint monitor_number(GdkMonitor *monitor) {
 ...
 */
static gboolean gimp_session_info_parse_offset(GScanner *scanner, gint *dest,
                                               gboolean *negative) {
  ...
  while (g_scanner_peek_next_token(scanner) == token) {
    ...
    case G_TOKEN_SYMBOL:
      ...
      case SESSION_INFO_FACTORY_ENTRY: {
        ...
        entry = gimp_dialog_factory_find_entry(
            gimp_dialog_factory_get_singleton(), identifier);
        if (!entry)
          goto error;
      ...
      case SESSION_INFO_POSITION:
        token = G_TOKEN_INT;
        if (!gimp_session_info_parse_offset(scanner, &info->p->x,
                                            &info->p->right_align))
          goto error;
        if (!gimp_session_info_parse_offset(scanner, &info->p->y,
                                            &info->p->bottom_align))
          goto error;
        break;
      ...
      case SESSION_INFO_AUX:
        token = gimp_session_info_aux_deserialize(scanner, &info->p->aux_info);
      ...
      case SESSION_INFO_DOCK: {
        ...
        if (GPOINTER_TO_INT(scanner->value.v_symbol) == SESSION_INFO_DOCK &&
            info->p->factory_entry && info->p->factory_entry->identifier &&
            strcmp("gimp-toolbox-window", info->p->factory_entry->identifier) ==
                0) {
...
error:
   ...
   */
  if (!info->p->docks && info->p->factory_entry &&
      strcmp("gimp-toolbox-window", info->p->factory_entry->identifier) == 0) {
    info->p->docks = g_list_append(info->p->docks,
...
static gboolean gimp_session_info_is_for_dock_window(GimpSessionInfo *info) {
  ...
  gboolean widget_state_for_dock =
      (info->p->widget == NULL || GIMP_IS_DOCK_WINDOW(info->p->widget));
...
static void gimp_session_info_dialog_show(GtkWidget *widget,
                                          GimpSessionInfo *info) {
  gtk_window_move(GTK_WINDOW(widget), info->p->x, info->p->y);

  if (gimp_session_info_get_remember_size(info) && info->p->width > 0 &&
      info->p->height > 0) {
    gtk_window_resize(GTK_WINDOW(info->p->widget), info->p->width,
                      info->p->height);
  }
}
...
static gboolean gimp_session_info_restore_docks(GimpRestoreDocksData *data) {
  ...
  if (GIMP_IS_DOCK_CONTAINER(dialog)) {
    ...
    for (iter = info->p->docks; iter; iter = g_list_next(iter)) {
      ...
      dock = GTK_WIDGET(gimp_session_info_dock_restore(
          dock_info, factory, monitor, GIMP_DOCK_CONTAINER(dialog)));
...
void gimp_session_info_restore(GimpSessionInfo *info,
  ...
  if (info->p->factory_entry && info->p->factory_entry->restore_func) {
    dialog = info->p->factory_entry->restore_func(factory, monitor, info);
  } else
    g_printerr("EEEEK\n");
...
void gimp_session_info_apply_geometry(GimpSessionInfo *info,
  ...
  info->p->x =
      CLAMP(info->p->x, work_rect.x, work_rect.x + work_rect.width - width);
  info->p->y =
      CLAMP(info->p->y, work_rect.y, work_rect.y + work_rect.height - height);
  ...
  if (gimp_session_info_get_remember_size(info) && info->p->width > 0 &&
    ...
    gtk_window_resize(GTK_WINDOW(info->p->widget), info->p->width,
                      info->p->height);
#else
    gtk_window_set_default_size(GTK_WINDOW(info->p->widget), info->p->width,
                                info->p->height);
#endif
  }
  ...
  if (gravity == GDK_GRAVITY_SOUTH_EAST || gravity == GDK_GRAVITY_NORTH_EAST)
    info->p->x = work_rect.x + work_rect.width - width;

  if (gravity == GDK_GRAVITY_SOUTH_WEST || gravity == GDK_GRAVITY_SOUTH_EAST)
    info->p->y = work_rect.y + work_rect.height - height;

  gtk_window_set_gravity(GTK_WINDOW(info->p->widget), gravity);
  gtk_window_move(GTK_WINDOW(info->p->widget), info->p->x, info->p->y);

  hints = GDK_HINT_USER_POS;
  if (gimp_session_info_get_remember_size(info))
    hints |= GDK_HINT_USER_SIZE;

  gtk_window_set_geometry_hints(GTK_WINDOW(info->p->widget), NULL, NULL, hints);
  ...
  if (GTK_IS_WINDOW(info->p->widget))
    g_signal_connect(info->p->widget, "show",
                     G_CALLBACK(gimp_session_info_dialog_show), info);
}
...
void gimp_session_info_read_geometry(GimpSessionInfo *info,
  ...
  if (window) {
    ...
    monitor = gdk_display_get_monitor_at_point(display, info->p->x, info->p->y);
    ...
    if (gimp_session_info_get_remember_size(info)) {
      gtk_window_get_size(GTK_WINDOW(info->p->widget), &info->p->width,
                          &info->p->height);
    } else {
      info->p->width = 0;
      info->p->height = 0;
  ...
  if (gimp_session_info_get_remember_if_open(info)) {
    ...
    visibility = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(info->p->widget),
...
void gimp_session_info_get_info(GimpSessionInfo *info) {
  ...
  if (GIMP_IS_SESSION_MANAGED(info->p->widget))
    info->p->aux_info = gimp_session_managed_get_aux_info(
        GIMP_SESSION_MANAGED(info->p->widget));
...
gboolean gimp_session_info_is_singleton(GimpSessionInfo *info) {
  ...
  return (!gimp_session_info_is_for_dock_window(info) &&
          info->p->factory_entry && info->p->factory_entry->singleton);
}
...
gboolean gimp_session_info_is_session_managed(GimpSessionInfo *info) {
  ...
  return (gimp_session_info_is_for_dock_window(info) ||
          (info->p->factory_entry && info->p->factory_entry->session_managed));
}
...
gboolean gimp_session_info_get_remember_size(GimpSessionInfo *info) {
  ...
  return (gimp_session_info_is_for_dock_window(info) ||
          (info->p->factory_entry && info->p->factory_entry->remember_size));
}
...
gboolean gimp_session_info_get_remember_if_open(GimpSessionInfo *info) {
  ...
  return (gimp_session_info_is_for_dock_window(info) ||
          (info->p->factory_entry && info->p->factory_entry->remember_if_open));
}
...
void gimp_session_info_set_widget(GimpSessionInfo *info, GtkWidget *widget) {
  g_return_if_fail(GIMP_IS_SESSION_INFO(info));

  if (GTK_IS_WINDOW(info->p->widget))
    g_signal_handlers_disconnect_by_func(info->p->widget,
                                         gimp_session_info_dialog_show, info);
