...
enum { PROP_0, PROP_MANUAL_L18N, PROP_EMPTY_LABEL };
...
gimp_translation_store_class_init(GimpTranslationStoreClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_MANUAL_L18N,
      g_param_spec_boolean("manual-l18n", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      object_class, PROP_EMPTY_LABEL,
      g_param_spec_string("empty-label", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_translation_store_init(GimpTranslationStore *store) {}
...
static void gimp_translation_store_constructed(GObject *object) {
  ...
  if (GIMP_TRANSLATION_STORE(object)->manual_l18n &&
    ...
    GIMP_LANGUAGE_STORE_GET_CLASS(object)->add(
        GIMP_LANGUAGE_STORE(object),
        GIMP_TRANSLATION_STORE(object)->empty_label, "");
  }
...
GtkListStore *gimp_translation_store_new(gboolean manual_l18n,
  ...
  return g_object_new(GIMP_TYPE_TRANSLATION_STORE, "manual-l18n", manual_l18n,
                      "empty-label", empty_label, NULL);
}
