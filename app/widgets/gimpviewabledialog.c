...
enum { PROP_0, PROP_VIEWABLES, PROP_CONTEXT, PROP_ICON_NAME, PROP_DESC };
...
static void gimp_viewable_dialog_class_init(GimpViewableDialogClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_VIEWABLES,
      g_param_spec_pointer("viewables", NULL, NULL, GIMP_PARAM_READWRITE));
...
static void gimp_viewable_dialog_init(GimpViewableDialog *dialog) {
  ...
  gimp_label_set_attributes(GTK_LABEL(dialog->desc_label), PANGO_ATTR_SCALE,
  ...
  dialog->viewable_label =
      g_object_new(GTK_TYPE_LABEL, "xalign", 0.0, "yalign", 0.5, "ellipsize",
                   PANGO_ELLIPSIZE_END, NULL);
  gimp_label_set_attributes(GTK_LABEL(dialog->viewable_label), PANGO_ATTR_SCALE,
                            PANGO_SCALE_SMALL, -1);
...
static void gimp_viewable_dialog_set_property(GObject *object,
  ...
  switch (property_id) {
  case PROP_VIEWABLES:
    gimp_viewable_dialog_set_viewables(dialog, g_value_get_pointer(value),
                                       dialog->context);
    break;
...
GtkWidget *gimp_viewable_dialog_new(GList *viewables, GimpContext *context,
  ...
  if (!viewables)
    g_warning(
        "Use of GimpViewableDialog with an empty viewable list is deprecated!");

  g_object_get(gtk_settings_get_default(), "gtk-dialogs-use-header",
               &use_header_bar, NULL);

  dialog = g_object_new(GIMP_TYPE_VIEWABLE_DIALOG, "viewables", viewables,
                        "context", context, "title", title, "role", role,
                        "help-func", help_func, "help-id", help_id, "icon-name",
                        icon_name, "description", desc, "parent", parent,
...
void gimp_viewable_dialog_set_viewables(GimpViewableDialog *dialog,
  ...
  if (dialog->view) {
    ...
    if (old_viewable) {
      g_signal_handlers_disconnect_by_func(
          old_viewable, gimp_viewable_dialog_name_changed, dialog);

      g_signal_handlers_disconnect_by_func(old_viewable,
                                           gimp_viewable_dialog_close, dialog);
    }
  }
  ...
  if (g_list_length(viewables) == 1 && viewables->data) {
    ...
    g_signal_connect_object(
        viewable, GIMP_VIEWABLE_GET_CLASS(viewable)->name_changed_signal,
        G_CALLBACK(gimp_viewable_dialog_name_changed), dialog, 0);
    ...
    g_object_add_weak_pointer(G_OBJECT(dialog->view), (gpointer)&dialog->view);
    ...
    if (GIMP_IS_ITEM(viewable)) {
      g_signal_connect_object(viewable, "removed",
                              G_CALLBACK(gimp_viewable_dialog_close), dialog,
                              G_CONNECT_SWAPPED);
    } else {
...
static void gimp_viewable_dialog_name_changed(GimpObject *object,
  ...
  if (GIMP_IS_ITEM(object)) {
    ...
    tmp = name;
    name =
        g_strdup_printf("%s-%d (%s)", tmp, gimp_item_get_id(GIMP_ITEM(object)),
