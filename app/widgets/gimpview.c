...
enum { SET_VIEWABLE, CLICKED, DOUBLE_CLICKED, CONTEXT, LAST_SIGNAL };
...
static gboolean gimp_view_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_view_real_set_viewable(GimpView *view, GimpViewable *old,
...
static GimpViewable *gimp_view_drag_viewable(GtkWidget *widget,
                                             GimpContext **context,
                                             gpointer data);
static GdkPixbuf *gimp_view_drag_pixbuf(GtkWidget *widget, gpointer data);
...
static void gimp_view_class_init(GimpViewClass *klass) {
  ...
  view_signals[SET_VIEWABLE] =
      g_signal_new("set-viewable", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpViewClass, set_viewable), NULL, NULL,
                   gimp_marshal_VOID__OBJECT_OBJECT, G_TYPE_NONE, 2,
                   GIMP_TYPE_VIEWABLE, GIMP_TYPE_VIEWABLE);

  view_signals[CLICKED] =
      g_signal_new("clicked", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpViewClass, clicked), NULL, NULL, NULL,
                   G_TYPE_NONE, 1, GDK_TYPE_MODIFIER_TYPE);

  view_signals[DOUBLE_CLICKED] = g_signal_new(
      "double-clicked", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpViewClass, double_clicked), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  view_signals[CONTEXT] =
      g_signal_new("context", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpViewClass, context), NULL, NULL, NULL,
                   G_TYPE_NONE, 0);
...
static void gimp_view_init(GimpView *view) {
  ...
  gimp_widget_track_monitor(GTK_WIDGET(view),
                            G_CALLBACK(gimp_view_monitor_changed), NULL, NULL);
}
...
static void gimp_view_get_preferred_width(GtkWidget *widget,
  ...
  if (view->expand) {
    ...
    *minimum_width = *natural_width =
        (view->renderer->width + 2 * view->renderer->border_width);
  }
}
...
static void gimp_view_get_preferred_height(GtkWidget *widget,
  ...
  if (view->expand) {
    ...
    *minimum_height = *natural_height =
        (view->renderer->height + 2 * view->renderer->border_width);
  }
}
...
static void gimp_view_size_allocate(GtkWidget *widget,
  ...
  if (view->expand) {
    ...
    if (view->renderer->width != width || view->renderer->height != height) {
      ...
      if (view->renderer->size != -1 && view->renderer->viewable) {
        ...
        gimp_viewable_get_preview_size(
            view->renderer->viewable, GIMP_VIEWABLE_MAX_PREVIEW_SIZE,
            view->renderer->is_popup, view->renderer->dot_for_dot, &view_width,
            &view_height);

        gimp_viewable_calc_preview_size(view_width, view_height, width, height,
                                        TRUE, 1.0, 1.0, &scaled_width,
                                        &scaled_height, NULL);
        ...
        gimp_view_renderer_set_size(
            view->renderer, MAX(scaled_width, scaled_height), border_width);
      } else {
        gimp_view_renderer_set_size_full(view->renderer, width, height,
                                         border_width);
      }
  ...
  width = (view->renderer->width + 2 * view->renderer->border_width);
  height = (view->renderer->height + 2 * view->renderer->border_width);
  ...
  if (gtk_widget_get_realized(widget))
    gdk_window_move_resize(view->event_window, allocation->x, allocation->y,
...
static gboolean gimp_view_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  gimp_view_renderer_draw(GIMP_VIEW(widget)->renderer, widget, cr,
                          allocation.width, allocation.height);
...
static gboolean gimp_view_button_press_event(GtkWidget *widget,
  ...
  if (!view->clickable && !view->show_popup)
    return FALSE;
  ...
  if (bevent->type == GDK_BUTTON_PRESS) {
    if (gdk_event_triggers_context_menu((GdkEvent *)bevent)) {
      ...
      if (view->show_popup && view->viewable) {
        gimp_view_popup_show(widget, bevent, view->renderer->context,
                             view->viewable, view->renderer->width,
...
static void gimp_view_real_set_viewable(GimpView *view, GimpViewable *old,
  ...
  if (viewable) {
    ...
    g_return_if_fail(g_type_is_a(viewable_type, view->renderer->viewable_type));
  }
  ...
  if (view->viewable) {
    ...
    if (!viewable && !view->renderer->is_popup) {
      if (gimp_dnd_viewable_source_remove(
              GTK_WIDGET(view), G_TYPE_FROM_INSTANCE(view->viewable))) {
    ...
    if (gimp_dnd_drag_source_set_by_type(GTK_WIDGET(view),
                                         ...
                                         viewable_type, GDK_ACTION_COPY)) {
      gimp_dnd_viewable_source_add(GTK_WIDGET(view), viewable_type,
                                   gimp_view_drag_viewable, NULL);

      if (gimp_viewable_get_size(viewable, NULL, NULL))
        gimp_dnd_pixbuf_source_add(GTK_WIDGET(view), gimp_view_drag_pixbuf,
                                   NULL);
    }
  }
...
GtkWidget *gimp_view_new(GimpContext *context, GimpViewable *viewable,
  ...
  view = gimp_view_new_by_types(context, GIMP_TYPE_VIEW,
                                G_TYPE_FROM_INSTANCE(viewable), size,
...
GtkWidget *gimp_view_new_full(GimpContext *context, GimpViewable *viewable,
  ...
  view = gimp_view_new_full_by_types(
      context, GIMP_TYPE_VIEW, G_TYPE_FROM_INSTANCE(viewable), width, height,
      border_width, is_popup, clickable, show_popup);
...
GtkWidget *gimp_view_new_by_types(GimpContext *context, GType view_type,
  ...
  renderer = gimp_view_renderer_new(context, viewable_type, size, border_width,
                                    is_popup);
  ...
  g_signal_connect(renderer, "update", G_CALLBACK(gimp_view_update_callback),
                   view);
...
GtkWidget *gimp_view_new_full_by_types(GimpContext *context, GType view_type,
  ...
  renderer = gimp_view_renderer_new_full(context, viewable_type, width, height,
                                         border_width, is_popup);
...
static void gimp_view_update_callback(GimpViewRenderer *renderer,
  ...
  if (width != requisition.width || height != requisition.height) {
