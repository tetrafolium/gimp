...
enum { PROP_0, PROP_LAST_VERSION, PROP_RELEASE_DATE };
...
static void gimp_critical_dialog_set_property(GObject *object,
                                              ...
                                              const GValue *value,
                                              GParamSpec *pspec);
static void gimp_critical_dialog_response(GtkDialog *dialog, gint response_id);

static void gimp_critical_dialog_copy_info(GimpCriticalDialog *dialog);
static gboolean browser_open_url(GtkWindow *window, const gchar *url,
                                 GError **error);
...
static void gimp_critical_dialog_class_init(GimpCriticalDialogClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_LAST_VERSION,
      g_param_spec_string("last-version", NULL, NULL, NULL,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      object_class, PROP_RELEASE_DATE,
      g_param_spec_string("release-date", NULL, NULL, NULL,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_critical_dialog_init(GimpCriticalDialog *dialog) {
  PangoAttrList *attrs;
  PangoAttribute *attr;
  ...
  gtk_box_pack_start(GTK_BOX(dialog->main_vbox), dialog->top_label, FALSE,
                     FALSE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(dialog->main_vbox), dialog->center_label, FALSE,
                     FALSE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(dialog->main_vbox), dialog->bottom_label, FALSE,
                     FALSE, 0);
...
static void gimp_critical_dialog_constructed(GObject *object) {
  ...
  if (dialog->last_version) {
    ...
    g_signal_connect_swapped(
        button, "clicked", G_CALLBACK(gimp_critical_dialog_copy_info), dialog);
    gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 0);
    gtk_widget_show(button);

    gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("Go to _Download page"),
                           GIMP_CRITICAL_RESPONSE_DOWNLOAD, _("_Close"),
    ...
    gtk_dialog_add_buttons(GTK_DIALOG(dialog), BUTTON1_TEXT,
                           GIMP_CRITICAL_RESPONSE_CLIPBOARD, BUTTON2_TEXT,
                           GIMP_CRITICAL_RESPONSE_URL, _("_Close"),
                           GTK_RESPONSE_CLOSE, NULL);
    ...
    text = g_strdup_printf(
        ...
        BUTTON1_TEXT, _("Open our bug tracker in the browser by clicking: "),
        BUTTON2_TEXT, _("Create a login if you don't have one yet."),
...
static gboolean browser_open_url(GtkWindow *window, const gchar *url,
  ...
  HINSTANCE hinst =
      ShellExecute(GetDesktopWindow(), "open", url, NULL, NULL, SW_SHOW);
  ...
  if ((intptr_t)hinst <= 32) {
    ...
    case ERROR_BAD_FORMAT:
      err = _("The .exe file is invalid (non-Microsoft Win32 .exe or error in "
              ".exe image).");
      break;
    ...
    case SE_ERR_DLLNOTFOUND:
      ...
      break;
    case SE_ERR_NOASSOC:
      err = _("There is no application associated with the given file name "
              "extension.");
      break;
    case SE_ERR_OOM:
  ...
  return gtk_show_uri_on_window(window, url, GDK_CURRENT_TIME, error);
...
GtkWidget *gimp_critical_dialog_new(const gchar *title,
  ...
  dialog =
      g_object_new(GIMP_TYPE_CRITICAL_DIALOG, "title", title, "last-version",
                   last_version, "release-date", date, NULL);
  g_free(date);

  return dialog;
}

void gimp_critical_dialog_add(GtkWidget *dialog, const gchar *message,
                              const gchar *trace, gboolean is_fatal,
                              const gchar *program, gint pid) {
  ...
  if (is_fatal) {
    text = g_strdup_printf(_("GIMP crashed with a fatal error: %s"), message);
  } else if (!gtk_label_get_text(GTK_LABEL(critical->top_label)) ||
             strlen(gtk_label_get_text(GTK_LABEL(critical->top_label))) == 0) {
    /* First error. Let's just display it. */
    text = g_strdup_printf(_("GIMP encountered an error: %s"), message);
  } else {
  ...
  gtk_label_set_text(GTK_LABEL(critical->top_label), text);
  g_free(text);
   ...
   */
  if (is_fatal) {
    gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("_Restart GIMP"),
                           GIMP_CRITICAL_RESPONSE_RESTART, NULL);
