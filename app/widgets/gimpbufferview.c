...
G_DEFINE_TYPE_WITH_CODE(
    GimpBufferView, gimp_buffer_view, GIMP_TYPE_CONTAINER_EDITOR,
    G_IMPLEMENT_INTERFACE(GIMP_TYPE_DOCKED, gimp_buffer_view_docked_iface_init))
...
GtkWidget *gimp_buffer_view_new(GimpViewType view_type,
  ...
  g_return_val_if_fail(
      view_size > 0 && view_size <= GIMP_VIEWABLE_MAX_PREVIEW_SIZE, FALSE);
  g_return_val_if_fail(view_border_width >= 0 &&
                           view_border_width <= GIMP_VIEW_MAX_BORDER_WIDTH,
                       FALSE);

  buffer_view = g_object_new(
      GIMP_TYPE_BUFFER_VIEW, "view-type", view_type, "container", container,
      "context", context, "view-size", view_size, "view-border-width",
      view_border_width, "menu-factory", menu_factory, "menu-identifier",
      "<Buffers>", "ui-path", "/buffers-popup", NULL);
  ...
  if (GIMP_IS_CONTAINER_TREE_VIEW(editor->view)) {
    ...
    gimp_container_tree_view_connect_name_edited(
        tree_view, G_CALLBACK(gimp_container_tree_view_name_edited), tree_view);
  }
  ...
  buffer_view->clipboard_view = gimp_view_new_full_by_types(
      NULL, GIMP_TYPE_VIEW, GIMP_TYPE_BUFFER, view_size, view_size,
      view_border_width, FALSE, FALSE, TRUE);
  gtk_box_pack_start(GTK_BOX(hbox), buffer_view->clipboard_view, FALSE, FALSE,
                     0);
  gtk_widget_show(buffer_view->clipboard_view);

  g_signal_connect_object(editor->view, "notify::view-size",
                          G_CALLBACK(gimp_buffer_view_view_notify), buffer_view,
                          0);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), buffer_view->clipboard_label, FALSE, FALSE,
                     0);
  ...
  buffer_view->paste_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor->view), "buffers", "buffers-paste",
      "buffers-paste-in-place", gimp_get_extend_selection_mask(), NULL);

  buffer_view->paste_into_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor->view), "buffers", "buffers-paste-into",
      "buffers-paste-into-in-place", gimp_get_extend_selection_mask(), NULL);

  buffer_view->paste_as_new_layer_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor->view), "buffers", "buffers-paste-as-new-layer",
      "buffers-paste-as-new-layer-in-place", gimp_get_extend_selection_mask(),
      NULL);

  buffer_view->paste_as_new_image_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor->view), "buffers", "buffers-paste-as-new-image", NULL);

  buffer_view->delete_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor->view), "buffers", "buffers-delete", NULL);

  gimp_container_view_enable_dnd(
      editor->view, GTK_BUTTON(buffer_view->paste_button), GIMP_TYPE_BUFFER);
  gimp_container_view_enable_dnd(editor->view,
                                 GTK_BUTTON(buffer_view->paste_into_button),
                                 GIMP_TYPE_BUFFER);
  gimp_container_view_enable_dnd(
      editor->view, GTK_BUTTON(buffer_view->paste_as_new_layer_button),
      GIMP_TYPE_BUFFER);
  gimp_container_view_enable_dnd(
      editor->view, GTK_BUTTON(buffer_view->paste_as_new_image_button),
      GIMP_TYPE_BUFFER);
  gimp_container_view_enable_dnd(
      editor->view, GTK_BUTTON(buffer_view->delete_button), GIMP_TYPE_BUFFER);
...
static void gimp_buffer_view_clipboard_changed(Gimp *gimp,
  ...
  if (buffer) {
    gchar *desc = gimp_viewable_get_description(GIMP_VIEWABLE(buffer), NULL);
...
static void gimp_buffer_view_view_notify(GimpContainerView *container_view,
                                         GParamSpec *pspec,
                                         GimpBufferView *buffer_view) {
  ...
  view_size =
      gimp_container_view_get_view_size(container_view, &view_border_width);

  gimp_view_renderer_set_size_full(view->renderer, view_size, view_size,
                                   view_border_width);
}
