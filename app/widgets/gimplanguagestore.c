...
static void gimp_language_store_real_add(GimpLanguageStore *store,
                                         const gchar *label, const gchar *code);

static gint gimp_language_store_sort(GtkTreeModel *model, GtkTreeIter *a,
...
static void gimp_language_store_class_init(GimpLanguageStoreClass *klass) {
...
static void gimp_language_store_init(GimpLanguageStore *store) {
  ...
  gtk_tree_sortable_set_sort_column_id(
      GTK_TREE_SORTABLE(store), GIMP_LANGUAGE_STORE_LABEL, GTK_SORT_ASCENDING);
}
...
static void gimp_language_store_real_add(GimpLanguageStore *store,
  ...
  gtk_list_store_set(GTK_LIST_STORE(store), &iter, GIMP_LANGUAGE_STORE_LABEL,
                     label, GIMP_LANGUAGE_STORE_CODE, code, -1);
}
...
static gint gimp_language_store_sort(GtkTreeModel *model, GtkTreeIter *a,
  ...
  cmp =
      g_utf8_collate(g_value_get_string(&avalue), g_value_get_string(&bvalue));
...
gboolean gimp_language_store_lookup(GimpLanguageStore *store, const gchar *code,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, iter); iter_valid;
    ...
    gtk_tree_model_get(model, iter, GIMP_LANGUAGE_STORE_CODE, &value, -1);
