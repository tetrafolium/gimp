...
enum { COLOR_CONTEXT, COLOR_CLICKED, COLOR_ACTIVATED, LAST_SIGNAL };

#define BORDER 6
#define RGB_EPSILON 1e-6

#define HAVE_COLORMAP(image)                                                   \
  (image != NULL && gimp_image_get_base_type(image) == GIMP_INDEXED &&         \
...
static PangoLayout *gimp_colormap_selection_create_layout(GtkWidget *widget);

static void
gimp_colormap_selection_update_entries(GimpColormapSelection *selection);

static gboolean
gimp_colormap_selection_preview_draw(GtkWidget *widget, cairo_t *cr,
                                     GimpColormapSelection *selection);

static void gimp_colormap_selection_entry_clicked(
    GimpPaletteView *view, GimpPaletteEntry *entry, GdkModifierType state,
    GimpColormapSelection *selection);
static void
...
gimp_colormap_selection_entry_context(GimpPaletteView *view,
                                      GimpPaletteEntry *entry,
                                      GimpColormapSelection *selection);
static void gimp_colormap_selection_color_dropped(
    GimpPaletteView *view, GimpPaletteEntry *entry, const GimpRGB *color,
    GimpColormapSelection *selection);
...
gimp_colormap_selection_class_init(GimpColormapSelectionClass *klass) {
  ...
  signals[COLOR_CONTEXT] = g_signal_new(
      "color-context", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpColormapSelectionClass, color_context), NULL, NULL,
      NULL, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[COLOR_CLICKED] = g_signal_new(
      "color-clicked", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpColormapSelectionClass, color_clicked), NULL, NULL,
      gimp_marshal_VOID__POINTER_ENUM, G_TYPE_NONE, 2, G_TYPE_POINTER,
      GDK_TYPE_MODIFIER_TYPE);
  signals[COLOR_ACTIVATED] = g_signal_new(
      "color-activated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpColormapSelectionClass, color_activated), NULL, NULL,
      NULL, G_TYPE_NONE, 1, G_TYPE_POINTER);
...
static void gimp_colormap_selection_init(GimpColormapSelection *selection) {
  GtkWidget *frame;
  GtkWidget *grid;
  ...
  selection->view = gimp_view_new_full_by_types(NULL, GIMP_TYPE_PALETTE_VIEW,
                                                GIMP_TYPE_PALETTE, 1, 1, 0,
                                                FALSE, TRUE, FALSE);
  ...
  g_signal_connect(selection->view, "draw",
                   G_CALLBACK(gimp_colormap_selection_preview_draw), selection);
  ...
  gtk_box_pack_end(GTK_BOX(selection), selection->right_vbox, FALSE, FALSE, 0);
  ...
  selection->index_adjustment =
      (GtkAdjustment *)gtk_adjustment_new(0, 0, 0, 1, 10, 0);
  selection->index_spinbutton =
      gimp_spin_button_new(selection->index_adjustment, 1.0, 0);
...
static void gimp_colormap_selection_finalize(GObject *object) {
  ...
  if (selection->context) {
    ...
    g_signal_handlers_disconnect_by_func(
        selection->context, G_CALLBACK(gimp_colormap_selection_image_changed),
        selection);
  }
  if (selection->active_image) {
    g_signal_handlers_disconnect_by_func(
        selection->active_image, G_CALLBACK(gtk_widget_queue_draw), selection);
    g_signal_handlers_disconnect_by_func(
        selection->active_image,
        G_CALLBACK(gimp_colormap_selection_set_palette), selection);
  }
...
GtkWidget *gimp_colormap_selection_new(GimpContext *context) {
  ...
  return g_object_new(GIMP_TYPE_COLORMAP_SELECTION, "context", context,
...
static PangoLayout *gimp_colormap_selection_create_layout(GtkWidget *widget) {
  ...
  layout = gtk_widget_create_pango_layout(widget, _("Only indexed images have "
                                                    "a colormap."));
...
gimp_colormap_selection_update_entries(GimpColormapSelection *selection) {
  ...
  if (!HAVE_COLORMAP(image) || !gimp_image_get_colormap_size(image)) {
  ...
  if (context != selection->context) {
    ...
    if (context) {
      ...
      g_signal_connect_swapped(context, "foreground-changed",
                               G_CALLBACK(gtk_widget_queue_draw), selection);
      g_signal_connect_swapped(context, "background-changed",
                               G_CALLBACK(gtk_widget_queue_draw), selection);
      g_signal_connect_swapped(
          context, "image-changed",
          G_CALLBACK(gimp_colormap_selection_image_changed), selection);
      gimp_colormap_selection_image_changed(selection,
...
gimp_colormap_selection_image_changed(GimpColormapSelection *selection,
  ...
  if (selection->active_image) {
    ...
    if (gimp_image_get_base_type(selection->active_image) == GIMP_INDEXED) {
      ...
      g_signal_handlers_disconnect_by_func(
          palette, G_CALLBACK(gtk_widget_queue_draw), selection);
    }
    ...
    if (!HAVE_COLORMAP(image)) {
      ...
      if (gtk_widget_get_mapped(GTK_WIDGET(selection))) {
        if (selection->active_palette)
          g_signal_handlers_disconnect_by_func(
              selection->active_palette, G_CALLBACK(gtk_widget_queue_draw),
              selection);
