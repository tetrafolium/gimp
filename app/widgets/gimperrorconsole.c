...
static const gboolean default_highlight[] = {[GIMP_MESSAGE_ERROR] = TRUE,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpErrorConsole, gimp_error_console, GIMP_TYPE_EDITOR,
...
static void gimp_error_console_init(GimpErrorConsole *console) {
  GtkWidget *scrolled_window;

  console->text_buffer = GTK_TEXT_BUFFER(gimp_text_buffer_new());

  gtk_text_buffer_create_tag(console->text_buffer, "title", "scale",
                             PANGO_SCALE_LARGE, "weight", PANGO_WEIGHT_BOLD,
                             NULL);
  gtk_text_buffer_create_tag(console->text_buffer, "message", NULL);
  ...
  g_signal_connect(console->text_view, "button-press-event",
                   G_CALLBACK(gimp_error_console_button_press), console);
...
static void gimp_error_console_constructed(GObject *object) {
  ...
  console->clear_button = gimp_editor_add_action_button(
      GIMP_EDITOR(console), "error-console", "error-console-clear", NULL);

  console->save_button = gimp_editor_add_action_button(
      GIMP_EDITOR(console), "error-console", "error-console-save-all",
      "error-console-save-selection", GDK_SHIFT_MASK, NULL);
}
...
GtkWidget *gimp_error_console_new(Gimp *gimp, GimpMenuFactory *menu_factory) {
  ...
  console = g_object_new(GIMP_TYPE_ERROR_CONSOLE, "menu-factory", menu_factory,
                         "menu-identifier", "<ErrorConsole>", "ui-path",
                         "/error-console-popup", NULL);
...
void gimp_error_console_add(GimpErrorConsole *console,
  ...
  gtk_text_view_add_child_at_anchor(GTK_TEXT_VIEW(console->text_view), image,
                                    anchor);
  ...
  gtk_text_buffer_insert_with_tags_by_name(console->text_buffer, &end, str, -1,
                                           "title", NULL);
  g_free(str);

  gtk_text_buffer_insert(console->text_buffer, &end, "\n", -1);

  gtk_text_buffer_insert_with_tags_by_name(console->text_buffer, &end, message,
                                           -1, "message", NULL);

  gtk_text_buffer_insert(console->text_buffer, &end, "\n\n", -1);

  end_mark =
      gtk_text_buffer_create_mark(console->text_buffer, NULL, &end, TRUE);
