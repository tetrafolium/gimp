...
void gimp_controllers_init(Gimp *gimp) {
  ...
  g_object_set_data_full(G_OBJECT(gimp), GIMP_CONTROLLER_MANAGER_DATA_KEY,
                         manager, (GDestroyNotify)gimp_controller_manager_free);

  manager->controllers = gimp_list_new(GIMP_TYPE_CONTROLLER_INFO, TRUE);

  g_signal_connect(manager->controllers, "add",
                   G_CALLBACK(gimp_controllers_add), manager);
  g_signal_connect(manager->controllers, "remove",
                   G_CALLBACK(gimp_controllers_remove), manager);

  manager->event_mapped_id = gimp_container_add_handler(
      manager->controllers, "event-mapped",
      G_CALLBACK(gimp_controllers_event_mapped), manager);
...
void gimp_controllers_restore(Gimp *gimp, GimpUIManager *ui_manager) {
  GimpControllerManager *manager;
  GFile *file;
  GError *error = NULL;
  ...
  if (!gimp_config_deserialize_file(GIMP_CONFIG(manager->controllers), file,
                                    NULL, &error)) {
...
void gimp_controllers_save(Gimp *gimp) {
  ...
  const gchar *footer = "end of controllerrc";
  ...
  if (!gimp_config_serialize_to_file(GIMP_CONFIG(manager->controllers), file,
...
static void gimp_controller_manager_free(GimpControllerManager *manager) {
  gimp_container_remove_handler(manager->controllers, manager->event_mapped_id);
...
static gboolean gimp_controllers_event_mapped(GimpControllerInfo *info,
  ...
  for (list = gimp_ui_manager_get_action_groups(manager->ui_manager); list;
