...
enum { PROP_0, PROP_GRID, PROP_CONTEXT, PROP_XRESOLUTION, PROP_YRESOLUTION };

static void gimp_grid_editor_constructed(GObject *object);
static void gimp_grid_editor_finalize(GObject *object);
static void gimp_grid_editor_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_grid_editor_get_property(GObject *object, guint property_id,
...
static void gimp_grid_editor_class_init(GimpGridEditorClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_GRID,
      g_param_spec_object("grid", NULL, NULL, GIMP_TYPE_GRID,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("xresolution", NULL, NULL, GIMP_MIN_RESOLUTION,
                          GIMP_MAX_RESOLUTION, GRID_EDITOR_DEFAULT_RESOLUTION,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
...
static void gimp_grid_editor_init(GimpGridEditor *editor) {
...
static void gimp_grid_editor_constructed(GObject *object) {
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Line _style:"), 0.0, 0.5,
                           style, 1);

  color_button = gimp_prop_color_button_new(
      G_OBJECT(editor->grid), "fgcolor", _("Change grid foreground color"),
      GRID_EDITOR_COLOR_BUTTON_WIDTH, GRID_EDITOR_COLOR_BUTTON_HEIGHT,
      GIMP_COLOR_AREA_FLAT);
  gtk_widget_set_halign(color_button, GTK_ALIGN_START);
  gimp_color_panel_set_context(GIMP_COLOR_PANEL(color_button), editor->context);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Foreground color:"), 0.0,
                           0.5, color_button, 1);

  color_button = gimp_prop_color_button_new(
      G_OBJECT(editor->grid), "bgcolor", _("Change grid background color"),
      GRID_EDITOR_COLOR_BUTTON_WIDTH, GRID_EDITOR_COLOR_BUTTON_HEIGHT,
      GIMP_COLOR_AREA_FLAT);
  gtk_widget_set_halign(color_button, GTK_ALIGN_START);
  gimp_color_panel_set_context(GIMP_COLOR_PANEL(color_button), editor->context);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("_Background color:"), 0.0,
                           0.5, color_button, 1);
  ...
  sizeentry = gimp_prop_coordinates_new(
      G_OBJECT(editor->grid), "xspacing", "yspacing", "spacing-unit", "%a",
      GIMP_SIZE_ENTRY_UPDATE_SIZE, editor->xresolution, editor->yresolution,
      TRUE);

  gtk_grid_set_column_spacing(GTK_GRID(sizeentry), 2);
  gtk_grid_set_row_spacing(GTK_GRID(sizeentry), 2);

  gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(sizeentry), _("Horizontal"), 0,
                               1, 0.0);
  gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(sizeentry), _("Vertical"), 0, 2,
                               0.0);
  gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(sizeentry), _("Pixels"), 1, 4,
                               0.0);
  ...
  sizeentry = gimp_prop_coordinates_new(
      G_OBJECT(editor->grid), "xoffset", "yoffset", "offset-unit", "%a",
      GIMP_SIZE_ENTRY_UPDATE_SIZE, editor->xresolution, editor->yresolution,
      TRUE);
...
GtkWidget *gimp_grid_editor_new(GimpGrid *grid, GimpContext *context,
  ...
  return g_object_new(GIMP_TYPE_GRID_EDITOR, "grid", grid, "context", context,
