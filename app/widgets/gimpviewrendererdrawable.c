...
static void gimp_view_renderer_drawable_cancel_render(
    GimpViewRendererDrawable *renderdrawable);

G_DEFINE_TYPE_WITH_PRIVATE(GimpViewRendererDrawable,
                           gimp_view_renderer_drawable, GIMP_TYPE_VIEW_RENDERER)
...
static void gimp_view_renderer_drawable_dispose(GObject *object) {
  GimpViewRendererDrawable *renderdrawable =
      GIMP_VIEW_RENDERER_DRAWABLE(object);
...
static void gimp_view_renderer_drawable_invalidate(GimpViewRenderer *renderer) {
  GimpViewRendererDrawable *renderdrawable =
      GIMP_VIEW_RENDERER_DRAWABLE(renderer);
...
static void gimp_view_renderer_drawable_render_async_callback(
    GimpAsync *async, GimpViewRendererDrawable *renderdrawable) {
  GtkWidget *widget;
  ...
  if (gimp_async_is_finished(async)) {
    ...
    gimp_view_renderer_render_temp_buf(
        renderer, widget, render_buf, renderdrawable->priv->render_buf_x,
        renderdrawable->priv->render_buf_y, -1, GIMP_VIEW_BG_CHECKS,
        GIMP_VIEW_BG_CHECKS);
...
static void gimp_view_renderer_drawable_render(GimpViewRenderer *renderer,
  ...
  gimp_viewable_calc_preview_size(image_width, image_height, renderer->width,
                                  renderer->height, renderer->dot_for_dot, xres,
                                  yres, &view_width, &view_height, NULL);
  ...
  if (image) {
    ...
    if (gimp_rectangle_intersect(src_x, src_y, src_width, src_height, -offset_x,
                                 -offset_y, image_width, image_height, &src_x,
                                 &src_y, &src_width, &src_height)) {
      offset_x += src_x;
      offset_y += src_y;

      dst_x = ROUND(((gdouble)view_width / image_width) * offset_x);
      dst_y = ROUND(((gdouble)view_height / image_height) * offset_y);
      dst_width = ROUND(((gdouble)view_width / image_width) * src_width);
      dst_height = ROUND(((gdouble)view_height / image_height) * src_height);
    } else {
      dst_x = 0;
      dst_y = 0;
      dst_width = 1;
  ...
  if (!empty) {
    async = gimp_drawable_get_sub_preview_async(
        drawable, src_x, src_y, src_width, src_height, dst_width, dst_height);
  } else {
    ...
    gimp_async_finish_full(async, render_buf,
...
static void gimp_view_renderer_drawable_cancel_render(
   ...
   */
  if (renderdrawable->priv->render_async) {
    gimp_cancelable_cancel(GIMP_CANCELABLE(renderdrawable->priv->render_async));
