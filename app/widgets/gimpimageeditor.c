...
G_DEFINE_TYPE_WITH_CODE(
    GimpImageEditor, gimp_image_editor, GIMP_TYPE_EDITOR,
...
static void gimp_image_editor_set_context(GimpDocked *docked,
  ...
  if (editor->context) {
    g_signal_handlers_disconnect_by_func(editor->context,
                                         gimp_image_editor_set_image, editor);
  ...
  if (context) {
    ...
    g_signal_connect_swapped(context, "image-changed",
                             G_CALLBACK(gimp_image_editor_set_image), editor);
...
static void gimp_image_editor_real_set_image(GimpImageEditor *editor,
  ...
  if (editor->image)
    g_signal_handlers_disconnect_by_func(editor->image,
                                         gimp_image_editor_image_flush, editor);

  editor->image = image;

  if (editor->image)
    g_signal_connect(editor->image, "flush",
                     G_CALLBACK(gimp_image_editor_image_flush), editor);
