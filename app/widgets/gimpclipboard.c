...
void gimp_clipboard_init(Gimp *gimp) {
  ...
  g_object_set_data_full(G_OBJECT(gimp), GIMP_CLIPBOARD_KEY, gimp_clip,
                         (GDestroyNotify)gimp_clipboard_free);
}
  ...
  if (clipboard && gtk_clipboard_get_owner(clipboard) != G_OBJECT(gimp)) {
    ...
    if (atom != GDK_NONE) {
      ...
      if (data) {
        ...
        if (pixbuf) {
          buffer = gimp_buffer_new_from_pixbuf(pixbuf, _("Clipboard"), 0, 0);
 ...
 **/
void gimp_clipboard_set_image(Gimp *gimp, GimpImage *image) {
  GimpClipboard *gimp_clip;
  GtkClipboard *clipboard;
  ...
  if (image) {
    ...
    gtk_clipboard_set_with_owner(clipboard, gimp_clip->image_target_entries,
                                 gimp_clip->n_image_target_entries,
                                 (GtkClipboardGetFunc)gimp_clipboard_send_image,
                                 (GtkClipboardClearFunc)NULL, G_OBJECT(gimp));

    /*  mark the first two entries (image/x-xcf and image/png) as
     *  suitable for storing
     */
    gtk_clipboard_set_can_store(clipboard, gimp_clip->image_target_entries,
...
void gimp_clipboard_set_buffer(Gimp *gimp, GimpBuffer *buffer) {
  ...
  if (buffer) {
    ...
    gtk_clipboard_set_with_owner(
        clipboard, gimp_clip->buffer_target_entries,
    ...
    if (gimp_clip->n_buffer_target_entries > 0)
      gtk_clipboard_set_can_store(clipboard, gimp_clip->buffer_target_entries,
                                  1);
  } else if (gtk_clipboard_get_owner(clipboard) == G_OBJECT(gimp)) {
    gtk_clipboard_clear(clipboard);
  }
...
void gimp_clipboard_set_svg(Gimp *gimp, const gchar *svg) {
  ...
  if (svg) {
    ...
    gtk_clipboard_set_with_owner(clipboard, gimp_clip->svg_target_entries,
 ...
 **/
void gimp_clipboard_set_curve(Gimp *gimp, GimpCurve *curve) {
  GimpClipboard *gimp_clip;
  GtkClipboard *clipboard;
  ...
  if (curve) {
    ...
    gtk_clipboard_set_with_owner(clipboard, gimp_clip->curve_target_entries,
...
static GimpClipboard *gimp_clipboard_new(gboolean verbose) {
  ...
  gimp_clip->image_target_entries =
      g_new0(GtkTargetEntry, gimp_clip->n_image_target_entries);
  ...
  if (gimp_clip->n_buffer_target_entries > 0) {
    ...
    gimp_clip->buffer_target_entries =
        g_new0(GtkTargetEntry, gimp_clip->n_buffer_target_entries);
  ...
  gimp_clip->curve_target_entries[0].target =
      g_strdup("application/x-gimp-curve");
...
static GdkAtom gimp_clipboard_wait_for_curve(Gimp *gimp) {
  ...
  if (targets) {
    GdkAtom curve_atom =
        gdk_atom_intern_static_string("application/x-gimp-curve");
    gint i;
...
static void gimp_clipboard_send_svg(GtkClipboard *clipboard,
  ...
  if (gimp_clip->svg) {
    ...
    gimp_selection_data_set_stream(data, (const guchar *)gimp_clip->svg,
