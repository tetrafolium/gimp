...
static void gimp_view_renderer_brush_render(GimpViewRenderer *renderer,
  ...
  temp_buf = gimp_viewable_get_new_preview(
      renderer->viewable, renderer->context, renderer->width, renderer->height);
  ...
  if (renderer->is_popup) {
    gimp_view_renderer_render_temp_buf(renderer, widget, temp_buf, temp_buf_x,
                                       temp_buf_y, -1, GIMP_VIEW_BG_WHITE,
                                       GIMP_VIEW_BG_WHITE);
    ...
    if (GIMP_IS_BRUSH_PIPE(renderer->viewable)) {
      ...
      renderbrush->pipe_timeout_id = g_timeout_add(
          300, gimp_view_renderer_brush_render_timeout, renderbrush);
    }
...
static gboolean gimp_view_renderer_brush_render_timeout(gpointer data) {
  ...
  temp_buf =
      gimp_viewable_get_new_preview(GIMP_VIEWABLE(brush), renderer->context,
...
static void gimp_view_renderer_brush_draw(GimpViewRenderer *renderer,
                                          ...
                                          gint available_width,
                                          gint available_height) {
  GIMP_VIEW_RENDERER_CLASS(parent_class)
      ->draw(renderer, widget, cr, available_width, available_height);
...
#define INDICATOR_HEIGHT 7
  ...
  if (renderer->width > 2 * INDICATOR_WIDTH &&
    ...
    if (renderer->width < brush_width || renderer->height < brush_height) {
      cairo_move_to(cr, available_width - INDICATOR_WIDTH + 1,
                    available_height - INDICATOR_HEIGHT / 2.0);
      cairo_rel_line_to(cr, INDICATOR_WIDTH - 2, 0);

      cairo_move_to(cr, available_width - INDICATOR_WIDTH / 2.0,
