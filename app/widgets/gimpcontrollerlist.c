...
enum { PROP_0, PROP_GIMP };

enum { COLUMN_ICON, COLUMN_NAME, COLUMN_TYPE, N_COLUMNS };
...
static void gimp_controller_list_init(GimpControllerList *list) {
  ...
  list->src =
      gtk_list_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_GTYPE);
  ...
  gtk_tree_view_column_set_attributes(column, cell, "icon-name", COLUMN_ICON,
                                      NULL);
  ...
  controller_types = g_type_children(GIMP_TYPE_CONTROLLER, &n_controller_types);
  ...
  for (i = 0; i < n_controller_types; i++) {
    ...
    gtk_list_store_set(
        list->src, &iter, COLUMN_ICON, controller_class->icon_name, COLUMN_NAME,
        controller_class->name, COLUMN_TYPE, controller_types[i], -1);
  ...
  g_signal_connect(list->add_button, "clicked",
                   G_CALLBACK(gimp_controller_list_add_clicked), list);
  ...
  g_signal_connect(list->remove_button, "clicked",
                   G_CALLBACK(gimp_controller_list_remove_clicked), list);
  ...
  gimp_container_tree_view_set_main_column_title(
      GIMP_CONTAINER_TREE_VIEW(list->dest), _("Active Controllers"));
  ...
  list->edit_button = gimp_editor_add_button(
      GIMP_EDITOR(list->dest), GIMP_ICON_DOCUMENT_PROPERTIES,
      _("Configure the selected controller"), NULL,
      G_CALLBACK(gimp_controller_list_edit_clicked), NULL, list);
  list->up_button = gimp_editor_add_button(
      GIMP_EDITOR(list->dest), GIMP_ICON_GO_UP,
      _("Move the selected controller up"), NULL,
      G_CALLBACK(gimp_controller_list_up_clicked), NULL, list);
  list->down_button = gimp_editor_add_button(
      GIMP_EDITOR(list->dest), GIMP_ICON_GO_DOWN,
      _("Move the selected controller down"), NULL,
      G_CALLBACK(gimp_controller_list_down_clicked), NULL, list);
...
GtkWidget *gimp_controller_list_new(Gimp *gimp) {
  ...
  return g_object_new(GIMP_TYPE_CONTROLLER_LIST, "gimp", gimp, NULL);
}
...
static void gimp_controller_list_src_sel_changed(GtkTreeSelection *sel,
  ...
  if (gtk_tree_selection_get_selected(sel, &model, &iter)) {
    ...
    gtk_tree_model_get(model, &iter, COLUMN_NAME, &name, COLUMN_TYPE,
                       &list->src_gtype, -1);

    if (list->add_button) {
      tip = g_strdup_printf(_("Add '%s' to the list of active controllers"),
                            name);
...
static void gimp_controller_list_add_clicked(GtkWidget *button,
  ...
  if (list->src_gtype == GIMP_TYPE_CONTROLLER_KEYBOARD &&
    ...
    gimp_message_literal(list->gimp, G_OBJECT(button), GIMP_MESSAGE_WARNING,
...
#define RESPONSE_DISABLE 1
  ...
  dialog = gimp_message_dialog_new(
      _("Remove Controller?"), GIMP_ICON_DIALOG_WARNING, GTK_WIDGET(list),
      GTK_DIALOG_MODAL, NULL, NULL,

      _("_Disable Controller"), RESPONSE_DISABLE, _("_Cancel"),
      GTK_RESPONSE_CANCEL, _("_Remove Controller"), GTK_RESPONSE_OK,
  ...
  case GTK_RESPONSE_OK: {
    ...
    if (editor_dialog)
      gtk_dialog_response(GTK_DIALOG(editor_dialog), GTK_RESPONSE_DELETE_EVENT);
...
static void gimp_controller_list_edit_clicked(GtkWidget *button,
  ...
  dialog = gimp_dialog_new(
      _("Configure Input Controller"), "gimp-controller-editor-dialog",
      gtk_widget_get_toplevel(GTK_WIDGET(list)), GTK_DIALOG_DESTROY_WITH_PARENT,
  ...
  gimp_dialog_factory_add_foreign(gimp_dialog_factory_get_singleton(),
                                  "gimp-controller-editor-dialog", dialog,
  ...
  g_signal_connect_object(list, "destroy", G_CALLBACK(gtk_widget_destroy),
                          G_OBJECT(dialog), G_CONNECT_SWAPPED);
  g_signal_connect_object(list, "unmap", G_CALLBACK(gtk_widget_destroy),
                          G_OBJECT(dialog), G_CONNECT_SWAPPED);
...
static void gimp_controller_list_up_clicked(GtkWidget *button,
  ...
  index =
      gimp_container_get_child_index(container, GIMP_OBJECT(list->dest_info));

  if (index > 0)
    gimp_container_reorder(container, GIMP_OBJECT(list->dest_info), index - 1);
}
