...
static gint gimp_pixbuf_format_compare(GdkPixbufFormat *a, GdkPixbufFormat *b);
...
void gimp_pixbuf_targets_add(GtkTargetList *target_list, guint info,
  ...
  for (list = formats; list; list = g_slist_next(list)) {
    ...
    for (type = mime_types; *type; type++) {
      ...
      gtk_target_list_add(target_list, gdk_atom_intern(*type, FALSE), 0, info);
    }
...
void gimp_pixbuf_targets_remove(GtkTargetList *target_list) {
  ...
  for (list = formats; list; list = g_slist_next(list)) {
    ...
    for (type = mime_types; *type; type++) {
      gtk_target_list_remove(target_list, gdk_atom_intern(*type, FALSE));
    }
