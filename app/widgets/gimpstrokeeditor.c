...
enum { PROP_0, PROP_OPTIONS, PROP_RESOLUTION };

static void gimp_stroke_editor_constructed(GObject *object);
static void gimp_stroke_editor_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_stroke_editor_get_property(GObject *object, guint property_id,
                                            GValue *value, GParamSpec *pspec);

static gboolean gimp_stroke_editor_draw_button(GtkWidget *widget, cairo_t *cr,
                                               gpointer data);
...
static void gimp_stroke_editor_class_init(GimpStrokeEditorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("options", NULL, NULL, GIMP_TYPE_STROKE_OPTIONS,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_RESOLUTION,
      g_param_spec_double("resolution", NULL, NULL, GIMP_MIN_RESOLUTION,
                          GIMP_MAX_RESOLUTION, 72.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}
...
static void gimp_stroke_editor_constructed(GObject *object) {
  ...
  size =
      gimp_prop_size_entry_new(G_OBJECT(options), "width", FALSE, "unit", "%a",
                               GIMP_SIZE_ENTRY_UPDATE_SIZE, editor->resolution);
  ...
  box = gimp_prop_enum_icon_box_new(G_OBJECT(options), "cap-style", "gimp-cap",
                                    0, 0);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("_Cap style:"), 0.0, 0.5,
                           box, 2);
  ...
  scale = gimp_prop_scale_entry_new(G_OBJECT(options), "miter-limit", NULL, 1.0,
                                    FALSE, 0.0, 0.0);
  gtk_widget_hide(gimp_labeled_get_label(GIMP_LABELED(scale)));
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("_Miter limit:"), 0.0,
                           0.5, scale, 2);

  frame = gtk_frame_new(NULL);
  gtk_frame_set_shadow_type(GTK_FRAME(frame), GTK_SHADOW_IN);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("Dash pattern:"), 0.0,
                           0.5, frame, 2);
  ...
  button = g_object_new(GTK_TYPE_BUTTON, "width-request", 14, NULL);
  gtk_box_pack_start(GTK_BOX(box), button, FALSE, TRUE, 0);
  gtk_widget_show(button);

  g_signal_connect_object(button, "clicked",
                          G_CALLBACK(gimp_dash_editor_shift_left), dash_editor,
                          G_CONNECT_SWAPPED);
  g_signal_connect_after(button, "draw",
                         G_CALLBACK(gimp_stroke_editor_draw_button), button);
  ...
  g_signal_connect_object(button, "clicked",
                          G_CALLBACK(gimp_dash_editor_shift_right), dash_editor,
                          G_CONNECT_SWAPPED);
  g_signal_connect_after(button, "draw",
                         G_CALLBACK(gimp_stroke_editor_draw_button), NULL);

  store = g_object_new(GIMP_TYPE_ENUM_STORE, "enum-type", GIMP_TYPE_DASH_PRESET,
                       "user-data-type", GIMP_TYPE_DASH_PATTERN, NULL);
  ...
  for (value = enum_class->values; value->value_name; value++) {
    ...
    gtk_list_store_set(GTK_LIST_STORE(store), &iter, GIMP_INT_STORE_VALUE,
                       value->value, GIMP_INT_STORE_LABEL, desc, -1);
  }
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("Dash _preset:"), 0.0,
                           0.5, box, 2);

  cell = g_object_new(GIMP_TYPE_CELL_RENDERER_DASHES, "xpad", 2, NULL);
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(box), cell, FALSE);
  gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(box), cell, "pattern",
                                GIMP_INT_STORE_USER_DATA);

  gimp_stroke_editor_combo_fill(options, GTK_COMBO_BOX(box));

  g_signal_connect(box, "changed", G_CALLBACK(gimp_stroke_editor_dash_preset),
                   options);
  g_signal_connect_object(options, "dash-info-changed",
                          G_CALLBACK(gimp_int_combo_box_set_active), box,
                          G_CONNECT_SWAPPED);
}
...
GtkWidget *gimp_stroke_editor_new(GimpStrokeOptions *options,
                                  gdouble resolution, gboolean edit_context) {
  g_return_val_if_fail(GIMP_IS_STROKE_OPTIONS(options), NULL);

  return g_object_new(GIMP_TYPE_STROKE_EDITOR, "options", options, "resolution",
                      resolution, "edit-context", edit_context ? TRUE : FALSE,
                      NULL);
}
...
static gboolean gimp_stroke_editor_draw_button(GtkWidget *widget, cairo_t *cr,
  ...
  gtk_render_arrow(style, cr, data ? 3 * (G_PI / 2) : 1 * (G_PI / 2),
                   (allocation.width - w) / 2, (allocation.height - w) / 2, w);
...
static void gimp_stroke_editor_combo_update(GtkTreeModel *model,
                                            GParamSpec *pspec,
                                            GimpStrokeOptions *options) {
  GtkTreeIter iter;
  ...
  if (gimp_int_store_lookup_by_value(model, GIMP_DASH_CUSTOM, &iter)) {
    gtk_list_store_set(GTK_LIST_STORE(model), &iter, GIMP_INT_STORE_USER_DATA,
                       gimp_stroke_options_get_dash_info(options), -1);
  }
}
