...
static void gimp_container_popup_init(GimpContainerPopup *popup) {
...
static void gimp_container_popup_confirm(GimpPopup *popup) {
  ...
  object = gimp_context_get_by_type(
      c_popup->context, gimp_container_get_children_type(c_popup->container));
...
GtkWidget *gimp_container_popup_new(
  ...
  popup =
      g_object_new(GIMP_TYPE_CONTAINER_POPUP, "type", GTK_WINDOW_POPUP, NULL);
  ...
  g_signal_connect(popup->context,
                   ...
                   G_CALLBACK(gimp_container_popup_context_changed), popup);
...
void gimp_container_popup_set_view_size(GimpContainerPopup *popup,
  ...
  if (view_size != popup->view_size) {
    ...
    gimp_container_view_set_view_size(popup->editor->view, popup->view_size,
...
static void gimp_container_popup_create_view(GimpContainerPopup *popup) {
  ...
  popup->editor = g_object_new(
      GIMP_TYPE_CONTAINER_EDITOR, "view-type", popup->view_type, "container",
      popup->container, "context", popup->context, "view-size",
      popup->view_size, "view-border-width", popup->view_border_width, NULL);
  ...
  if (popup->view_type == GIMP_VIEW_TYPE_LIST) {
    ...
    gtk_box_pack_end(GTK_BOX(popup->editor->view), search_entry, FALSE, FALSE,
                     0);
    gtk_tree_view_set_search_entry(
        GTK_TREE_VIEW(
            GIMP_CONTAINER_TREE_VIEW(GIMP_CONTAINER_VIEW(popup->editor->view))
                ->view),
  ...
  gimp_container_box_set_size_request(
      GIMP_CONTAINER_BOX(popup->editor->view),
      columns * (popup->default_view_size + 2 * popup->view_border_width),
      rows * (popup->default_view_size + 2 * popup->view_border_width));
  ...
  gimp_editor_add_button(editor, "zoom-out", _("Smaller Previews"), NULL,
                         G_CALLBACK(gimp_container_popup_smaller_clicked), NULL,
                         popup);
  gimp_editor_add_button(editor, "zoom-in", _("Larger Previews"), NULL,
                         G_CALLBACK(gimp_container_popup_larger_clicked), NULL,
                         popup);

  button = gimp_editor_add_icon_box(
      editor, GIMP_TYPE_VIEW_TYPE, "gimp",
      G_CALLBACK(gimp_container_popup_view_type_toggled), popup);
  gimp_int_radio_group_set_active(GTK_RADIO_BUTTON(button), popup->view_type);

  if (popup->dialog_factory)
    gimp_editor_add_button(
        editor, popup->dialog_icon_name, popup->dialog_tooltip, NULL,
        G_CALLBACK(gimp_container_popup_dialog_clicked), NULL, popup);
...
static void gimp_container_popup_view_type_toggled(GtkWidget *button,
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(button))) {
    ...
    view_type =
        GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "gimp-item-data"));
...
static void gimp_container_popup_dialog_clicked(GtkWidget *button,
  ...
  gimp_window_strategy_show_dockable_dialog(
      GIMP_WINDOW_STRATEGY(gimp_get_window_strategy(popup->context->gimp)),
      popup->context->gimp, popup->dialog_factory,
      gimp_widget_get_monitor(button), popup->dialog_identifier);
  g_signal_emit_by_name(popup, "confirm");
}
