...
#define VALUE(i, j)                                                            \
  ((SAFE_CLAMP(SAMPLE(i)[j], meter->priv->range_min, meter->priv->range_max) - \
...
static void gimp_meter_set_property(GObject *object, guint property_id,
                                    const GValue *value, GParamSpec *pspec);
static void gimp_meter_get_property(GObject *object, guint property_id,
...
static void gimp_meter_get_preferred_height(GtkWidget *widget,
                                            gint *minimum_height,
                                            gint *natural_height);
static gboolean gimp_meter_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_meter_mask_sample(GimpMeter *meter, const gdouble *sample,
                                   gdouble *result);
...
static void gimp_meter_class_init(GimpMeterClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int("size", NULL, NULL, 32, 1024, 48,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_REFRESH_RATE,
      g_param_spec_double("refresh-rate", NULL, NULL, 0.001, 1000.0, 8.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_RANGE_MIN,
      g_param_spec_double("range-min", NULL, NULL, 0.0, G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_RANGE_MAX,
      g_param_spec_double("range-max", NULL, NULL, 0.0, G_MAXDOUBLE, 1.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_N_VALUES,
      g_param_spec_int("n-values", NULL, NULL, 0, 32, 0,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_HISTORY_VISIBLE,
      g_param_spec_boolean("history-visible", NULL, NULL, TRUE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_HISTORY_DURATION,
      g_param_spec_double("history-duration", NULL, NULL, 0.0, 3600.0, 60.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_HISTORY_RESOLUTION,
      g_param_spec_double("history-resolution", NULL, NULL, 0.1, 3600.0, 1.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_LED_ACTIVE,
      g_param_spec_boolean("led-active", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_LED_COLOR,
      gimp_param_spec_rgb("led-color", NULL, NULL, TRUE, &(GimpRGB){},
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_meter_init(GimpMeter *meter) {
...
static void gimp_meter_set_property(GObject *object, guint property_id,
  ...
  case PROP_RANGE_MIN:
    gimp_meter_set_range(meter, g_value_get_double(value),
                         gimp_meter_get_range_max(meter));
    break;

  case PROP_RANGE_MAX:
    gimp_meter_set_range(meter, gimp_meter_get_range_min(meter),
...
static void gimp_meter_map(GtkWidget *widget) {
  ...
  if (!meter->priv->timeout_id) {
    ...
    meter->priv->timeout_id =
        g_timeout_add(interval, (GSourceFunc)gimp_meter_timeout, meter);
  }
}
...
static void gimp_meter_get_preferred_height(GtkWidget *widget,
  ...
  *minimum_height = *natural_height =
      ceil(3.0 / 4.0 * vsize + 4.0 * BORDER_WIDTH);
}
...
static gboolean gimp_meter_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  cairo_translate(cr, 0.5 * BORDER_WIDTH + 0.5 * size,
                  1.5 * BORDER_WIDTH +
                      2.0 / 3.0 * (allocation.height - 4.0 * BORDER_WIDTH));

  cairo_save(cr);

  /* paint led */
  if (meter->priv->led_active) {
    cairo_arc(cr, 0.0, 0.0, 0.06 * size, 0.0 * REV, 1.0 * REV);
  ...
  cairo_arc(cr, 0.0, 0.0, 0.5 * size, 5.0 / 12.0 * REV, 1.0 / 12.0 * REV);
  cairo_arc_negative(cr, 0.0, 0.0, 0.1 * size, 1.0 / 12.0 * REV,
                     5.0 / 12.0 * REV);
  cairo_close_path(cr);
  cairo_clip(cr);
  ...
  if (meter->priv->range_min < meter->priv->range_max) {
    for (i = 0; i < meter->priv->n_values; i++) {
      ...
      cairo_arc(cr, 0.0, 0.0,
                ...
                0.5 * size, 5.0 / 12.0 * REV,
                (5.0 / 12.0 + 2.0 / 3.0 * v) * REV);
  ...
  if (meter->priv->history_visible) {
    ...
    cairo_line_to(cr, allocation.width - BORDER_WIDTH - 0.5 * size,
                  -0.50 * size);
    cairo_line_to(cr, allocation.width - BORDER_WIDTH - 0.5 * size,
                  0.25 * size);
    cairo_close_path(cr);

    cairo_path_extents(cr, &history_x1, &history_y1, &history_x2, &history_y2);
    ...
    if (meter->priv->range_min < meter->priv->range_max) {
      gdouble sample_width =
          (history_x2 - history_x1) / (meter->priv->n_samples - 4);
    ...
    for (i = 1; i < 6; i++) {
      cairo_move_to(cr, history_x1 + i / 6.0 * (history_x2 - history_x1),
                    history_y1);
...
static void gimp_meter_clear_history_unlocked(GimpMeter *meter) {
  ...
  meter->priv->last_sample_time =
      meter->priv->current_time / meter->priv->sample_duration;
...
static void gimp_meter_update_samples(GimpMeter *meter) {
  meter->priv->n_samples =
      ceil(meter->priv->history_duration / meter->priv->history_resolution) + 4;

  meter->priv->samples =
      g_renew(gdouble, meter->priv->samples,
              meter->priv->n_samples * meter->priv->n_values);

  meter->priv->sample_duration =
      ROUND(meter->priv->history_resolution * G_TIME_SPAN_SECOND);
...
static void gimp_meter_shift_samples(GimpMeter *meter) {
  ...
  n_new_samples =
      MIN(time - meter->priv->last_sample_time, meter->priv->n_samples - 1);
...
GtkWidget *gimp_meter_new(gint n_values) {
  return g_object_new(GIMP_TYPE_METER, "n-values", n_values, NULL);
}

void gimp_meter_set_size(GimpMeter *meter, gint size) {
...
void gimp_meter_set_refresh_rate(GimpMeter *meter, gdouble rate) {
  g_return_if_fail(GIMP_IS_METER(meter));
...
void gimp_meter_set_range(GimpMeter *meter, gdouble min, gdouble max) {
  g_return_if_fail(GIMP_IS_METER(meter));
...
void gimp_meter_set_n_values(GimpMeter *meter, gint n_values) {
  g_return_if_fail(GIMP_IS_METER(meter));
  ...
  if (n_values != meter->priv->n_values) {
    ...
    if (n_values > meter->priv->n_values) {
      gegl_memset_pattern(meter->priv->values,
                          ...
                          sizeof(Value), n_values - meter->priv->n_values);
    }
...
void gimp_meter_set_value_active(GimpMeter *meter, gint value,
                                 gboolean active) {
  g_return_if_fail(GIMP_IS_METER(meter));
...
gboolean gimp_meter_get_value_active(GimpMeter *meter, gint value) {
...
void gimp_meter_set_value_color(GimpMeter *meter, gint value,
                                const GimpRGB *color) {
  g_return_if_fail(GIMP_IS_METER(meter));
  g_return_if_fail(value >= 0 && value < meter->priv->n_values);
...
const GimpRGB *gimp_meter_get_value_color(GimpMeter *meter, gint value) {
...
void gimp_meter_set_value_interpolation(GimpMeter *meter, gint value,
                                        GimpInterpolationType interpolation) {
  g_return_if_fail(GIMP_IS_METER(meter));
  g_return_if_fail(value >= 0 && value < meter->priv->n_values);
...
void gimp_meter_set_value_show_in_gauge(GimpMeter *meter, gint value,
                                        gboolean show) {
  g_return_if_fail(GIMP_IS_METER(meter));
  g_return_if_fail(value >= 0 && value < meter->priv->n_values);
...
gboolean gimp_meter_get_value_show_in_gauge(GimpMeter *meter, gint value) {
  g_return_val_if_fail(GIMP_IS_METER(meter), FALSE);
  g_return_val_if_fail(value >= 0 && value < meter->priv->n_values, FALSE);
...
void gimp_meter_set_value_show_in_history(GimpMeter *meter, gint value,
                                          gboolean show) {
  g_return_if_fail(GIMP_IS_METER(meter));
  g_return_if_fail(value >= 0 && value < meter->priv->n_values);
...
gboolean gimp_meter_get_value_show_in_history(GimpMeter *meter, gint value) {
  g_return_val_if_fail(GIMP_IS_METER(meter), FALSE);
  g_return_val_if_fail(value >= 0 && value < meter->priv->n_values, FALSE);
...
void gimp_meter_set_history_visible(GimpMeter *meter, gboolean visible) {
  g_return_if_fail(GIMP_IS_METER(meter));
...
void gimp_meter_set_history_duration(GimpMeter *meter, gdouble duration) {
  g_return_if_fail(GIMP_IS_METER(meter));
...
void gimp_meter_set_history_resolution(GimpMeter *meter, gdouble resolution) {
  g_return_if_fail(GIMP_IS_METER(meter));
...
void gimp_meter_add_sample(GimpMeter *meter, const gdouble *sample) {
  g_return_if_fail(GIMP_IS_METER(meter));
...
void gimp_meter_set_led_active(GimpMeter *meter, gboolean active) {
  g_return_if_fail(GIMP_IS_METER(meter));
...
void gimp_meter_set_led_color(GimpMeter *meter, const GimpRGB *color) {
  g_return_if_fail(GIMP_IS_METER(meter));
  g_return_if_fail(color != NULL);
