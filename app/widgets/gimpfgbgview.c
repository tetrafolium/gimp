...
static void gimp_fg_bg_view_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_fg_bg_view_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);

static gboolean gimp_fg_bg_view_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_fg_bg_view_init(GimpFgBgView *view) {
  ...
  gimp_widget_track_monitor(GTK_WIDGET(view),
                            G_CALLBACK(gimp_fg_bg_view_destroy_transform), NULL,
                            NULL);
}
  ...
  if (view->context) {
    ...
    if (view->transform)
      gimp_color_transform_process_pixels(
          view->transform, babl_format("R'G'B'A double"), &color,
          babl_format("R'G'B'A double"), &color, 1);
...
static void gimp_fg_bg_view_create_transform(GimpFgBgView *view) {
  if (view->color_config) {
    ...
    view->transform = gimp_widget_get_color_transform(
        GTK_WIDGET(view), view->color_config, profile,
...
GtkWidget *gimp_fg_bg_view_new(GimpContext *context) {
  ...
  return g_object_new(GIMP_TYPE_FG_BG_VIEW, "context", context, NULL);
}
...
void gimp_fg_bg_view_set_context(GimpFgBgView *view, GimpContext *context) {
  ...
  if (context != view->context) {
    if (view->context) {
      g_signal_handlers_disconnect_by_func(view->context, gtk_widget_queue_draw,
                                           view);
      g_clear_object(&view->context);

      g_signal_handlers_disconnect_by_func(
          view->color_config, gimp_fg_bg_view_destroy_transform, view);
