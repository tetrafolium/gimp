...
enum { PROP_0, PROP_TYPE, PROP_ASPECT, PROP_ANGLE };

static void gimp_blob_editor_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_blob_editor_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);

static gboolean gimp_blob_editor_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_blob_editor_get_handle(GimpBlobEditor *editor,
                                        GdkRectangle *rect);
static void gimp_blob_editor_draw_blob(GimpBlobEditor *editor, cairo_t *cr,
                                       gdouble xc, gdouble yc, gdouble radius);
...
static void gimp_blob_editor_class_init(GimpBlobEditorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("blob-type", NULL, NULL, GIMP_TYPE_INK_BLOB_TYPE,
                        GIMP_INK_BLOB_TYPE_CIRCLE,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_ASPECT,
      g_param_spec_double("blob-aspect", NULL, NULL, 1.0, 10.0, 1.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_ANGLE,
      g_param_spec_double("blob-angle", NULL, NULL, -G_PI, G_PI, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}
...
GtkWidget *gimp_blob_editor_new(GimpInkBlobType type, gdouble aspect,
                                gdouble angle) {
  return g_object_new(GIMP_TYPE_BLOB_EDITOR, "blob-type", type, "blob-aspect",
                      aspect, "blob-angle", angle, NULL);
}
...
static gboolean gimp_blob_editor_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  gimp_blob_editor_draw_blob(editor, cr, allocation.width / 2.0,
                             allocation.height / 2.0, 0.9 * r0);
  ...
  gtk_style_context_set_state(style,
                              editor->in_handle ? GTK_STATE_FLAG_PRELIGHT : 0);
...
static gboolean gimp_blob_editor_motion_notify(GtkWidget *widget,
  ...
  if (editor->active) {
    ...
    if (rsquare > 0) {
      ...
      g_object_set(editor, "blob-angle", angle, "blob-aspect", aspect, NULL);
    }
  } else {
    GdkRectangle rect;
    gboolean in_handle;
...
static void gimp_blob_editor_draw_blob(GimpBlobEditor *editor, cairo_t *cr,
  ...
  radius *= 2.0;
  blob = function(2.0 * xc, 2.0 * yc, radius * cos(editor->angle),
