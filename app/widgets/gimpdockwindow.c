...
static void gimp_dock_window_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_dock_window_get_property(GObject *object, guint property_id,
...
static void gimp_dock_window_factory_image_changed(GimpContext *context,
                                                   GimpImage *image,
                                                   GimpDock *dock);
static void gimp_dock_window_auto_clicked(GtkWidget *widget, GimpDock *dock);

G_DEFINE_TYPE_WITH_CODE(
    GimpDockWindow, gimp_dock_window, GIMP_TYPE_WINDOW,
...
static void gimp_dock_window_class_init(GimpDockWindowClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("context", NULL, NULL, GIMP_TYPE_CONTEXT,
  ...
  g_object_class_install_property(
      object_class, PROP_UI_MANAGER_NAME,
      g_param_spec_string("ui-manager-name", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_IMAGE_CONTAINER,
      g_param_spec_object("image-container", NULL, NULL, GIMP_TYPE_CONTAINER,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_DISPLAY_CONTAINER,
      g_param_spec_object("display-container", NULL, NULL, GIMP_TYPE_CONTAINER,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_ALLOW_DOCKBOOK_ABSENCE,
      g_param_spec_boolean("allow-dockbook-absence", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  gtk_widget_class_install_style_property(
      widget_class, g_param_spec_int("default-height", NULL, NULL, -1, G_MAXINT,
                                     DEFAULT_DOCK_HEIGHT, GIMP_PARAM_READABLE));

  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_enum("menu-preview-size", NULL, NULL, GTK_TYPE_ICON_SIZE,
                        DEFAULT_MENU_VIEW_SIZE, GIMP_PARAM_READABLE));
}

static void gimp_dock_window_init(GimpDockWindow *dock_window) {
...
static void gimp_dock_window_session_managed_iface_init(
...
static void gimp_dock_window_constructed(GObject *object) {
   ...
   */
  dock_window->p->ui_manager = gimp_menu_factory_manager_new(
      menu_factory, dock_window->p->ui_manager_name, dock_window);
  ...
  g_signal_connect_object(dock_window->p->context, "display-changed",
                          ...
                          dock_window, G_CONNECT_SWAPPED);
  g_signal_connect_object(dock_window->p->context, "image-changed",
                          G_CALLBACK(gimp_dock_window_image_changed),
                          dock_window, G_CONNECT_SWAPPED);

  dock_window->p->image_flush_handler_id = gimp_container_add_handler(
      gimp->images, "flush", G_CALLBACK(gimp_dock_window_image_flush),
      dock_window);
  ...
  gimp_context_define_properties(
      ...
      FALSE);
  gimp_context_set_parent(dock_window->p->context, factory_context);
  ...
  /* Setup widget hierarchy */
  {
    ...
    /* Image selection menu */
    {
      ...
      dock_window->p->image_combo =
          gimp_container_combo_box_new(NULL, NULL, 16, 1);
      gtk_box_pack_start(GTK_BOX(hbox), dock_window->p->image_combo, TRUE, TRUE,
                         0);
      g_signal_connect(dock_window->p->image_combo, "destroy",
                       G_CALLBACK(gtk_widget_destroyed),
                       &dock_window->p->image_combo);
      gimp_help_set_help_data(dock_window->p->image_combo, NULL,
      ...
      gtk_toggle_button_set_active(
          GTK_TOGGLE_BUTTON(dock_window->p->auto_button),
          dock_window->p->auto_follow_active);
      gtk_box_pack_start(GTK_BOX(hbox), dock_window->p->auto_button, FALSE,
                         FALSE, 0);
      gtk_widget_show(dock_window->p->auto_button);

      g_signal_connect(dock_window->p->auto_button, "clicked",
                       G_CALLBACK(gimp_dock_window_auto_clicked), dock_window);
     ...
     */
    dock_window->p->dock_columns = GIMP_DOCK_COLUMNS(gimp_dock_columns_new(
  ...
  if (dock_window->p->auto_follow_active) {
    if (gimp_context_get_display(factory_context))
      gimp_context_copy_property(factory_context, dock_window->p->context,
  ...
  gtk_icon_size_lookup(DEFAULT_MENU_VIEW_SIZE, &menu_view_width,
                       &menu_view_height);

  g_object_set(dock_window->p->image_combo, "container",
               dock_window->p->image_container, "context",
...
static void gimp_dock_window_style_updated(GtkWidget *widget) {
  ...
  gtk_widget_style_get(widget, "default-height", &default_height,
                       "menu-preview-size", &menu_view_size, NULL);
  ...
  if (dock_window->p->image_combo) {
    ...
    gtk_icon_size_lookup(menu_view_size, &menu_view_width, &menu_view_height);

    gtk_widget_style_get(dock_window->p->auto_button, "focus-line-width",
                         &focus_line_width, "focus-padding", &focus_padding,
                         NULL);

    button_style = gtk_widget_get_style_context(widget);
    gtk_style_context_get_border(button_style,
                                 gtk_widget_get_state_flags(widget), &border);

    gimp_container_view_set_view_size(
        GIMP_CONTAINER_VIEW(dock_window->p->image_combo), menu_view_height, 1);

    gtk_widget_set_size_request(dock_window->p->auto_button, -1,
                                menu_view_height +
                                    2 * (1 /* CHILD_SPACING */ + border.top +
                                         border.bottom + focus_padding +
                                         focus_line_width));
  }
}
...
static gboolean gimp_dock_window_delete_event(GtkWidget *widget,
  ...
  entry_name =
      (gimp_dock_window_has_toolbox(dock_window) ? "gimp-toolbox-window"
                                                 : "gimp-dock-window");
...
gimp_dock_window_add_dock_from_session(GimpDockContainer *dock_container,
  ...
  gimp_dock_window_add_dock(dock_window, dock, -1 /*index*/);
}
...
gimp_dock_window_get_aux_info(GimpSessionManaged *session_managed) {
  ...
  aux = gimp_session_info_aux_new(AUX_INFO_SHOW_IMAGE_MENU,
                                  dock_window->p->show_image_menu ? "true"
                                                                  : "false");
...
gimp_dock_window_should_add_to_recent(GimpDockWindow *dock_window) {
  ...
  if (!docks) {
    ...
    if (GIMP_IS_TOOLBOX(dock) && gimp_dock_get_n_dockables(dock) == 0) {
      should_add = FALSE;
    } else if (!GIMP_IS_TOOLBOX(dock) && gimp_dock_get_n_dockables(dock) == 1) {
      should_add = FALSE;
    }
  }
...
static void gimp_dock_window_update_title(GimpDockWindow *dock_window) {
  ...
  dock_window->p->update_title_idle_id =
      g_idle_add((GSourceFunc)gimp_dock_window_update_title_idle, dock_window);
}

static gboolean
gimp_dock_window_update_title_idle(GimpDockWindow *dock_window) {
  gchar *desc =
      gimp_dock_window_get_description(dock_window, FALSE /*complete*/);
  if (desc) {
...
static void gimp_dock_window_image_changed(GimpDockWindow *dock_window,
  ...
  if (image == NULL && !gimp_container_is_empty(image_container)) {
    ...
    if (find_display) {
      ...
      for (list = GIMP_LIST(display_container)->queue->head; list;
...
static void gimp_dock_window_auto_clicked(GtkWidget *widget, GimpDock *dock) {
  ...
  if (dock_window->p->auto_follow_active) {
    ...
    gimp_context_copy_properties(context, dock_window->p->context,
                                 GIMP_CONTEXT_PROP_MASK_DISPLAY |
                                     GIMP_CONTEXT_PROP_MASK_IMAGE);
  }
}

void gimp_dock_window_add_dock(GimpDockWindow *dock_window, GimpDock *dock,
                               gint index) {
  g_return_if_fail(GIMP_IS_DOCK_WINDOW(dock_window));
  g_return_if_fail(GIMP_IS_DOCK(dock));

  gimp_dock_columns_add_dock(dock_window->p->dock_columns, GIMP_DOCK(dock),
                             index);
...
void gimp_dock_window_remove_dock(GimpDockWindow *dock_window, GimpDock *dock) {
  gimp_dock_columns_remove_dock(dock_window->p->dock_columns, GIMP_DOCK(dock));

  g_signal_handlers_disconnect_by_func(dock, gimp_dock_window_update_title,
                                       dock_window);
  g_signal_handlers_disconnect_by_func(dock, gimp_dock_set_host_geometry_hints,
                                       dock_window);
}

GtkWidget *gimp_dock_window_new(const gchar *role, const gchar *ui_manager_name,
                                gboolean allow_dockbook_absence,
                                GimpDialogFactory *dialog_factory,
                                GimpContext *context) {
  ...
  return g_object_new(GIMP_TYPE_DOCK_WINDOW, "role", role, "ui-manager-name",
                      ui_manager_name, "allow-dockbook-absence",
                      allow_dockbook_absence, "dialog-factory", dialog_factory,
...
GimpContext *gimp_dock_window_get_context(GimpDockWindow *dock_window) {
  g_return_val_if_fail(GIMP_IS_DOCK_WINDOW(dock_window), NULL);

  return dock_window->p->context;
}

gboolean gimp_dock_window_get_auto_follow_active(GimpDockWindow *dock_window) {
  g_return_val_if_fail(GIMP_IS_DOCK_WINDOW(dock_window), FALSE);

  return dock_window->p->auto_follow_active;
}

void gimp_dock_window_set_auto_follow_active(GimpDockWindow *dock_window,
                                             gboolean auto_follow_active) {
  g_return_if_fail(GIMP_IS_DOCK_WINDOW(dock_window));
...
gboolean gimp_dock_window_get_show_image_menu(GimpDockWindow *dock_window) {
  g_return_val_if_fail(GIMP_IS_DOCK_WINDOW(dock_window), FALSE);

  return dock_window->p->show_image_menu;
}

void gimp_dock_window_set_show_image_menu(GimpDockWindow *dock_window,
                                          gboolean show) {
  GtkWidget *parent;
...
gboolean gimp_dock_window_has_toolbox(GimpDockWindow *dock_window) {
  ...
  for (iter = gimp_dock_columns_get_docks(dock_window->p->dock_columns); iter;
