...
G_DEFINE_TYPE_WITH_CODE(
    GimpContainerBox, gimp_container_box, GIMP_TYPE_EDITOR,
...
static void gimp_container_box_init(GimpContainerBox *box) {
  ...
  sb = gtk_scrolled_window_get_vscrollbar(
...
void gimp_container_box_set_size_request(GimpContainerBox *box, gint width,
  ...
  if (sw_class->scrollbar_spacing >= 0)
    ...
    gtk_widget_style_get(GTK_WIDGET(box->scrolled_win), "scrollbar-spacing",
...
static GtkWidget *gimp_container_box_get_preview(GimpDocked *docked,
  ...
  preview = gimp_prop_view_new(G_OBJECT(context), prop_name, context, height);
  ...
  gimp_view_renderer_set_size_full(GIMP_VIEW(preview)->renderer, width, height,
                                   border_width);
