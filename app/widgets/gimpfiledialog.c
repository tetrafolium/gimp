...
static void gimp_file_dialog_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_file_dialog_get_property(GObject *object, guint property_id,
...
static GimpProgress *gimp_file_dialog_progress_start(GimpProgress *progress,
...
static GimpFileDialogState *gimp_file_dialog_get_state(GimpFileDialog *dialog);
...
static void gimp_file_dialog_class_init(GimpFileDialogClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_OK_BUTTON_LABEL,
      g_param_spec_string("ok-button-label", NULL, NULL, _("_OK"),
                          GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_AUTOMATIC_HELP_ID,
      g_param_spec_string("automatic-help-id", NULL, NULL, NULL,
                          GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_AUTOMATIC_LABEL,
      g_param_spec_string("automatic-label", NULL, NULL, NULL,
                          GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_FILE_FILTER_LABEL,
      g_param_spec_string("file-filter-label", NULL, NULL, NULL,
                          GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_FILE_PROCS,
      g_param_spec_enum("file-procs", NULL, NULL,
                        GIMP_TYPE_FILE_PROCEDURE_GROUP,
                        GIMP_FILE_PROCEDURE_GROUP_NONE,
                        GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_FILE_PROCS_ALL_IMAGES,
      g_param_spec_enum("file-procs-all-images", NULL, NULL,
                        GIMP_TYPE_FILE_PROCEDURE_GROUP,
                        GIMP_FILE_PROCEDURE_GROUP_NONE,
                        GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(object_class, PROP_SHOW_ALL_FILES,
                                  g_param_spec_boolean("show-all-files", NULL,
                                                       NULL, FALSE,
...
static void gimp_file_dialog_set_property(GObject *object, guint property_id,
  ...
  case PROP_FILE_PROCS:
    dialog->file_procs = gimp_plug_in_manager_get_file_procedures(
        dialog->gimp->plug_in_manager, g_value_get_enum(value));
    break;
  case PROP_FILE_PROCS_ALL_IMAGES:
    dialog->file_procs_all_images = gimp_plug_in_manager_get_file_procedures(
        dialog->gimp->plug_in_manager, g_value_get_enum(value));
    break;
...
static void gimp_file_dialog_constructed(GObject *object) {
  ...
  gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("_Cancel"), GTK_RESPONSE_CANCEL,
                         dialog->ok_button_label, GTK_RESPONSE_OK, NULL);
  ...
  if (dialog->help_id) {
    gimp_help_connect(GTK_WIDGET(dialog), gimp_file_dialog_help_func,
                      dialog->help_id, dialog, NULL);

    if (GIMP_GUI_CONFIG(dialog->gimp->config)->show_help_button) {
      gtk_dialog_add_buttons(GTK_DIALOG(dialog), _("_Help"), GTK_RESPONSE_HELP,
                             NULL);
    }
  }
...
static GFile *gimp_file_dialog_real_get_default_folder(GimpFileDialog *dialog) {
  ...
  if (dialog->gimp->default_folder) {
    ...
    file = g_object_get_data(G_OBJECT(dialog->gimp), "gimp-default-folder");
    ...
    if (!file) {
      ...
      if (!path)
        path = g_build_path(G_DIR_SEPARATOR_S, g_get_home_dir(),
                            G_DIR_SEPARATOR_S, NULL);

      file = g_file_new_for_path(path);
      g_free(path);

      g_object_set_data_full(G_OBJECT(dialog->gimp), "gimp-default-folder",
...
static GimpProgress *gimp_file_dialog_progress_start(GimpProgress *progress,
  ...
  if (dialog->progress) {
    retval = gimp_progress_start(GIMP_PROGRESS(dialog->progress), cancellable,
                                 "%s", message);
...
void gimp_file_dialog_add_extra_widget(GimpFileDialog *dialog,
  ...
  gtk_box_pack_start(GTK_BOX(dialog->extra_vbox), widget, expand, fill,
                     padding);
}
...
static void gimp_file_dialog_add_user_dir(GimpFileDialog *dialog,
  ...
  if (user_dir)
    gtk_file_chooser_add_shortcut_folder(GTK_FILE_CHOOSER(dialog), user_dir,
                                         NULL);
}
...
static void gimp_file_dialog_add_preview(GimpFileDialog *dialog) {
  ...
  g_signal_connect(dialog, "selection-changed",
                   G_CALLBACK(gimp_file_dialog_selection_changed), dialog);
  g_signal_connect(dialog, "update-preview",
                   G_CALLBACK(gimp_file_dialog_update_preview), dialog);
  ...
  GIMP_VIEW_RENDERER_IMAGEFILE(
      GIMP_VIEW(GIMP_THUMB_BOX(dialog->thumb_box)->preview)->renderer)
      ->file_system =
      _gtk_file_chooser_get_file_system(GTK_FILE_CHOOSER(dialog));
#endif
}
...
static void gimp_file_dialog_add_proc_selection(GimpFileDialog *dialog) {
  ...
  g_signal_connect(dialog->proc_view, "changed",
                   G_CALLBACK(gimp_file_dialog_proc_changed), dialog);

  gimp_file_proc_view_set_proc(GIMP_FILE_PROC_VIEW(dialog->proc_view), NULL);

  /* Checkbox to show all files. */
  checkbox = gimp_prop_check_button_new(G_OBJECT(dialog), "show-all-files",
                                        _("Show _All Files"));
    ...
    if (proc && proc->extensions_list) {
      ...
      if (uri && strlen(uri)) {
        ...
        if (last_dot && last_dot != uri) {
          ...
          for (list = view->meta_extensions; list; list = g_list_next(list)) {
