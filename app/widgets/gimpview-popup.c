...
static gboolean gimp_view_popup_button_release(GtkWidget *widget,
                                               ...
                                               GimpViewPopup *popup);
static void gimp_view_popup_unmap(GtkWidget *widget, GimpViewPopup *popup);
...
gboolean gimp_view_popup_show(GtkWidget *widget, GdkEventButton *bevent,
  ...
  if (!gimp_viewable_get_popup_size(viewable, view_width, view_height,
                                    dot_for_dot, &popup_width, &popup_height))
    return FALSE;
  ...
  g_signal_connect(widget, "button-release-event",
                   G_CALLBACK(gimp_view_popup_button_release), popup);
  g_signal_connect(widget, "unmap", G_CALLBACK(gimp_view_popup_unmap), popup);
  g_signal_connect(widget, "drag-begin", G_CALLBACK(gimp_view_popup_drag_begin),
                   popup);

  popup->timeout_id = g_timeout_add(
      VIEW_POPUP_DELAY, (GSourceFunc)gimp_view_popup_timeout, popup);
...
static void gimp_view_popup_hide(GimpViewPopup *popup) {
  ...
  g_signal_handlers_disconnect_by_func(popup->widget,
                                       gimp_view_popup_button_release, popup);
  g_signal_handlers_disconnect_by_func(popup->widget, gimp_view_popup_unmap,
                                       popup);
  g_signal_handlers_disconnect_by_func(popup->widget,
                                       gimp_view_popup_drag_begin, popup);
...
static gboolean gimp_view_popup_timeout(GimpViewPopup *popup) {
  ...
  gtk_window_set_transient_for(
      GTK_WINDOW(window), GTK_WINDOW(gtk_widget_get_toplevel(popup->widget)));
  ...
  view = gimp_view_new_full(popup->context, popup->viewable, popup->popup_width,
                            popup->popup_height, 0, TRUE, FALSE, FALSE);
  ...
  gdk_monitor_get_workarea(gdk_display_get_monitor_at_point(
                               display, popup->button_x, popup->button_y),
                           &workarea);
