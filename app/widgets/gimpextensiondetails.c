...
static void gimp_extension_details_init(GimpExtensionDetails *details) {
...
void gimp_extension_details_set(GimpExtensionDetails *details,
  ...
  gtk_container_foreach(GTK_CONTAINER(details), (GtkCallback)gtk_widget_destroy,
                        NULL);
  ...
  if (extension) {
    ...
    if (gtk_widget_get_realized(GTK_WIDGET(details))) {
      ...
      gtk_widget_get_allocated_size(GTK_WIDGET(details), &allocation,
                                    &baseline);
      gimp_extension_details_size_allocate(details, &allocation,
                                           GTK_GRID(grid));
    }
    g_signal_connect(details, "size-allocate",
                     G_CALLBACK(gimp_extension_details_size_allocate), grid);
...
static void gimp_extension_details_size_allocate(GimpExtensionDetails *details,
  ...
  if (allocation->width > 0 && allocation->height > 0 &&
    ...
    pixbuf = gimp_extension_get_screenshot(details->p->extension,
                                           details->p->screenshot_width,
                                           details->p->screenshot_height, NULL);
    if (pixbuf) {
