...
G_DEFINE_TYPE(GimpLanguageComboBox, gimp_language_combo_box, GTK_TYPE_COMBO_BOX)
...
static void gimp_language_combo_box_init(GimpLanguageComboBox *combo) {
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combo), renderer, "text",
                                 GIMP_LANGUAGE_STORE_LABEL, NULL);
}
...
GtkWidget *gimp_language_combo_box_new(gboolean manual_l18n,
  ...
  combo = g_object_new(GIMP_TYPE_LANGUAGE_COMBO_BOX, "model", store, NULL);
...
gchar *gimp_language_combo_box_get_code(GimpLanguageComboBox *combo) {
  ...
  gtk_tree_model_get(gtk_combo_box_get_model(GTK_COMBO_BOX(combo)), &iter,
                     GIMP_LANGUAGE_STORE_CODE, &code, -1);
