...
enum { ACTIVATE, CHANGE_STATE, LAST_SIGNAL };

static void gimp_action_set_proxy_tooltip(GimpAction *action, GtkWidget *proxy);
static void gimp_action_label_notify(GimpAction *action,
                                     const GParamSpec *pspec, gpointer data);
...
static void gimp_action_default_init(GimpActionInterface *iface) {
  action_signals[ACTIVATE] = g_signal_new(
      "gimp-activate", G_TYPE_FROM_INTERFACE(iface), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpActionInterface, activate), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_VARIANT);

  action_signals[CHANGE_STATE] = g_signal_new(
      "gimp-change-state", G_TYPE_FROM_INTERFACE(iface), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpActionInterface, change_state), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_VARIANT);
}
...
void gimp_action_init(GimpAction *action) {
  ...
  g_signal_connect(action, "notify::label",
                   G_CALLBACK(gimp_action_label_notify), NULL);
  g_signal_connect(action, "notify::tooltip",
                   G_CALLBACK(gimp_action_tooltip_notify), NULL);
}

/*  public functions  */

void gimp_action_emit_activate(GimpAction *action, GVariant *value) {
  g_return_if_fail(GIMP_IS_ACTION(action));
...
void gimp_action_emit_change_state(GimpAction *action, GVariant *value) {
  g_return_if_fail(GIMP_IS_ACTION(action));
...
void gimp_action_set_proxy(GimpAction *action, GtkWidget *proxy) {
  g_return_if_fail(GIMP_IS_ACTION(action));
...
void gimp_action_set_label(GimpAction *action, const gchar *label) {
...
void gimp_action_set_tooltip(GimpAction *action, const gchar *tooltip) {
...
void gimp_action_set_icon_name(GimpAction *action, const gchar *icon_name) {
...
void gimp_action_set_gicon(GimpAction *action, GIcon *icon) {
...
void gimp_action_set_help_id(GimpAction *action, const gchar *help_id) {
  g_return_if_fail(GIMP_IS_ACTION(action));

  g_object_set_qdata_full(G_OBJECT(action), GIMP_HELP_ID, g_strdup(help_id),
...
void gimp_action_set_visible(GimpAction *action, gboolean visible) {
...
void gimp_action_set_sensitive(GimpAction *action, gboolean sensitive) {
...
void gimp_action_set_accel_path(GimpAction *action, const gchar *accel_path) {
...
gint gimp_action_name_compare(GimpAction *action1, GimpAction *action2) {
  return strcmp(gimp_action_get_name(action1), gimp_action_get_name(action2));
}

gboolean gimp_action_is_gui_blacklisted(const gchar *action_name) {
  static const gchar *suffixes[] = {"-menu", "-popup"};
  ...
  static const gchar *actions[] = {"tools-brightness-contrast", "tools-curves",
                                   "tools-levels", "tools-offset",
                                   "tools-threshold"};
...
static void gimp_action_set_proxy_tooltip(GimpAction *action,
  ...
  if (tooltip)
    gimp_help_set_help_data(proxy, tooltip,
                            g_object_get_qdata(G_OBJECT(proxy), GIMP_HELP_ID));
}
...
static void gimp_action_label_notify(GimpAction *action,
  ...
  for (list = gimp_action_get_proxies(action); list;
       list = g_slist_next(list)) {
    if (GTK_IS_MENU_ITEM(list->data)) {
      GtkWidget *child = gtk_bin_get_child(GTK_BIN(list->data));

      if (GTK_IS_BOX(child)) {
        child = g_object_get_data(G_OBJECT(list->data), "gimp-menu-item-label");

        if (GTK_IS_LABEL(child))
          gtk_label_set_text(GTK_LABEL(child), gimp_action_get_label(action));
      }
    }
  }
}
