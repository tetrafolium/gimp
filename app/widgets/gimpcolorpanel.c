...
static guint color_panel_signals[LAST_SIGNAL] = {
    0,
};
...
static void gimp_color_panel_class_init(GimpColorPanelClass *klass) {
  ...
  color_panel_signals[RESPONSE] =
      g_signal_new("response", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpColorPanelClass, response), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, GIMP_TYPE_COLOR_DIALOG_STATE);
}
...
static gboolean gimp_color_panel_button_press(GtkWidget *widget,
  ...
  if (gdk_event_triggers_context_menu((GdkEvent *)bevent)) {
    ...
    action = gimp_action_group_get_action(group, "color-button-use-foreground");
    gimp_action_set_visible(action, color_panel->context != NULL);

    action = gimp_action_group_get_action(group, "color-button-use-background");
    gimp_action_set_visible(action, color_panel->context != NULL);

    if (color_panel->context) {
      action =
          gimp_action_group_get_action(group, "color-button-use-foreground");
      gimp_context_get_foreground(color_panel->context, &color);
      g_object_set(action, "color", &color, NULL);

      action =
          gimp_action_group_get_action(group, "color-button-use-background");
...
static void gimp_color_panel_clicked(GtkButton *button) {
  ...
  if (!panel->color_dialog) {
    ...
    panel->color_dialog = gimp_color_dialog_new(
        NULL, panel->context, TRUE, gimp_color_button_get_title(color_button),
        NULL, NULL, GTK_WIDGET(button), NULL, NULL, &color,
    ...
    g_signal_connect(panel->color_dialog, "destroy",
                     G_CALLBACK(gtk_widget_destroyed), &panel->color_dialog);

    g_signal_connect(panel->color_dialog, "update",
                     G_CALLBACK(gimp_color_panel_dialog_update), panel);
  } else {
...
GtkWidget *gimp_color_panel_new(const gchar *title, const GimpRGB *color,
  ...
  return g_object_new(GIMP_TYPE_COLOR_PANEL, "title", title, "type", type,
...
static void gimp_color_panel_dialog_update(GimpColorDialog *dialog,
  ...
  case GIMP_COLOR_DIALOG_CANCEL:
    ...
    g_signal_emit(panel, color_panel_signals[RESPONSE], 0, state);
    break;
  }
}
