...
enum { PROP_0, PROP_CONTEXT, PROP_PICKABLE };
...
static void gimp_pickable_button_drop_pickable(GtkWidget *widget, gint x,
...
static void gimp_pickable_button_class_init(GimpPickableButtonClass *klass) {
...
static void gimp_pickable_button_init(GimpPickableButton *button) {
  ...
  gimp_dnd_viewable_dest_add(GTK_WIDGET(button), GIMP_TYPE_LAYER,
                             gimp_pickable_button_drop_pickable, NULL);
...
static void gimp_pickable_button_constructed(GObject *object) {
  ...
  button->private->view = gimp_view_new_by_types(
      button->private->context, GIMP_TYPE_VIEW, GIMP_TYPE_VIEWABLE,
      button->private->view_size, button->private->view_border_width, FALSE);
...
static void gimp_pickable_button_clicked(GtkButton *button) {
  ...
  g_signal_connect(popup, "confirm",
                   G_CALLBACK(gimp_pickable_button_popup_confirm), button);
...
GtkWidget *gimp_pickable_button_new(GimpContext *context, gint view_size,
                                    gint view_border_width) {
  GimpPickableButton *button;
  ...
  button = g_object_new(GIMP_TYPE_PICKABLE_BUTTON, "context", context, NULL);
  ...
  if (pickable != button->private->pickable) {
    ...
    if (button->private->pickable)
      g_signal_connect(button->private->pickable, "notify::buffer",
                       G_CALLBACK(gimp_pickable_button_notify_buffer), button);
