...
static void gimp_action_view_accel_changed(GtkAccelGroup *accel_group,
                                           ...
                                           GimpActionView *view);
static void gimp_action_view_accel_edited(
    GtkCellRendererAccel *accel, const char *path_string, guint accel_key,
    GdkModifierType accel_mask, guint hardware_keycode, GimpActionView *view);
  ...
  if (view->manager) {
    if (view->show_shortcuts) {
      ...
      g_signal_handlers_disconnect_by_func(
          group, gimp_action_view_accel_changed, view);
    }
...
GtkWidget *gimp_action_view_new(GimpUIManager *manager,
  ...
  for (list = gimp_ui_manager_get_action_groups(manager); list;
    ...
    gtk_tree_store_set(store, &group_iter, GIMP_ACTION_VIEW_COLUMN_ICON_NAME,
                       group->icon_name, GIMP_ACTION_VIEW_COLUMN_LABEL,
                       group->label, -1);
    ...
    for (list2 = actions; list2; list2 = g_list_next(list2)) {
      ...
      if (show_shortcuts) {
        ...
        if (accel_closure) {
          ...
          key = gtk_accel_group_find(
              accel_group, gimp_action_view_accel_find_func, accel_closure);
      ...
      gtk_tree_store_set(
          store, &action_iter, GIMP_ACTION_VIEW_COLUMN_VISIBLE, TRUE,
          ...
          GIMP_ACTION_VIEW_COLUMN_NAME, name, GIMP_ACTION_VIEW_COLUMN_ACCEL_KEY,
          accel_key, GIMP_ACTION_VIEW_COLUMN_ACCEL_MASK, accel_mask,
          GIMP_ACTION_VIEW_COLUMN_ACCEL_CLOSURE, accel_closure, -1);
      ...
      if (select_action && !strcmp(select_action, name)) {
        select_path =
            gtk_tree_model_get_path(GTK_TREE_MODEL(store), &action_iter);
      }
    }
  ...
  view = g_object_new(GIMP_TYPE_ACTION_VIEW, "model", filter, "rules-hint",
                      TRUE, NULL);
  ...
  gtk_tree_view_column_set_attributes(column, cell, "icon-name",
                                      GIMP_ACTION_VIEW_COLUMN_ICON_NAME, NULL);

  cell = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(column, cell, TRUE);
  gtk_tree_view_column_set_attributes(column, cell, "text",
                                      GIMP_ACTION_VIEW_COLUMN_LABEL, NULL);

  gtk_tree_view_append_column(view, column);

  if (show_shortcuts) {
    g_signal_connect(accel_group, "accel-changed",
                     G_CALLBACK(gimp_action_view_accel_changed), view);

    column = gtk_tree_view_column_new();
    gtk_tree_view_column_set_title(column, _("Shortcut"));

    cell = gtk_cell_renderer_accel_new();
    g_object_set(cell, "mode", GTK_CELL_RENDERER_MODE_EDITABLE, "editable",
                 TRUE, NULL);
    gtk_tree_view_column_pack_start(column, cell, TRUE);
    gtk_tree_view_column_set_attributes(
        column, cell, "accel-key", GIMP_ACTION_VIEW_COLUMN_ACCEL_KEY,
        "accel-mods", GIMP_ACTION_VIEW_COLUMN_ACCEL_MASK, NULL);

    g_signal_connect(cell, "accel-edited",
                     G_CALLBACK(gimp_action_view_accel_edited), view);
    g_signal_connect(cell, "accel-cleared",
                     G_CALLBACK(gimp_action_view_accel_cleared), view);
  ...
  gtk_tree_view_column_pack_start(column, cell, TRUE);
  gtk_tree_view_column_set_attributes(column, cell, "text",
                                      GIMP_ACTION_VIEW_COLUMN_NAME, NULL);
...
void gimp_action_view_set_filter(GimpActionView *view, const gchar *filter) {
  ...
  model =
      gtk_tree_model_filter_get_model(GTK_TREE_MODEL_FILTER(filtered_model));
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    for (child_valid = gtk_tree_model_iter_children(model, &child_iter, &iter);
         child_valid;
      ...
      if (view->filter) {
        ...
        gtk_tree_model_get(model, &child_iter,
                           ...
                           GIMP_ACTION_VIEW_COLUMN_NAME, &name, -1);

        visible = label && name &&
                  (strstr(label, view->filter) != NULL ||
      ...
      gtk_tree_store_set(GTK_TREE_STORE(model), &child_iter,
                         GIMP_ACTION_VIEW_COLUMN_VISIBLE, visible, -1);
    ...
    gtk_tree_store_set(GTK_TREE_STORE(model), &iter,
                       GIMP_ACTION_VIEW_COLUMN_VISIBLE, n_children > 0, -1);
  }
...
static void gimp_action_view_accel_changed(GtkAccelGroup *accel_group,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    for (child_valid = gtk_tree_model_iter_children(model, &child_iter, &iter);
      ...
      gtk_tree_model_get(model, &child_iter,
                         GIMP_ACTION_VIEW_COLUMN_ACCEL_CLOSURE, &closure, -1);
      ...
      if (accel_closure == closure) {
        ...
        gtk_tree_store_set(GTK_TREE_STORE(model), &child_iter,
                           ...
                           GIMP_ACTION_VIEW_COLUMN_ACCEL_MASK, accel_mask, -1);
...
static void gimp_action_view_conflict_response(GtkWidget *dialog,
                                               gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    if (!gtk_accel_map_change_entry(confirm_data->accel_path,
                                    ...
                                    confirm_data->accel_mask, TRUE)) {
      gimp_message_literal(confirm_data->manager->gimp, G_OBJECT(dialog),
                           GIMP_MESSAGE_ERROR, _("Changing shortcut failed."));
    }
  }
...
static void gimp_action_view_conflict_confirm(GimpActionView *view,
                                              GimpAction *action,
                                              guint accel_key,
                                              GdkModifierType accel_mask,
  ...
  dialog = gimp_message_dialog_new(_("Conflicting Shortcuts"),
...
static const gchar *gimp_action_view_get_accel_action(
    GimpActionView *view, const gchar *path_string, GimpAction **action_return,
    guint *action_accel_key, GdkModifierType *action_accel_mask) {
  GtkTreeModel *model;
  GtkTreePath *path;
  GtkTreeIter iter;
  ...
  if (gtk_tree_model_get_iter(model, &iter, path)) {
    ...
    gtk_tree_model_get(model, &iter, GIMP_ACTION_VIEW_COLUMN_ACTION, &action,
...
static void gimp_action_view_accel_edited(
  ...
  accel_path = gimp_action_view_get_accel_action(
      view, path_string, &action, &action_accel_key, &action_accel_mask);

  if (!accel_path)
    return;

  if (accel_key == action_accel_key && accel_mask == action_accel_mask)
    return;
  ...
  if (!accel_key ||
       ...
       */
      accel_key == GDK_KEY_Left || accel_key == GDK_KEY_Right ||
      accel_key == GDK_KEY_Up || accel_key == GDK_KEY_Down ||
      accel_key == GDK_KEY_space || accel_key == GDK_KEY_KP_Space) {
    gimp_message_literal(view->manager->gimp, G_OBJECT(view),
                         GIMP_MESSAGE_ERROR, _("Invalid shortcut."));
  } else if (accel_key == GDK_KEY_F1 || action_accel_key == GDK_KEY_F1) {
    gimp_message_literal(view->manager->gimp, G_OBJECT(view),
                         GIMP_MESSAGE_ERROR, _("F1 cannot be remapped."));
  } else if (accel_key >= GDK_KEY_0 && accel_key <= GDK_KEY_9 &&
             accel_mask == GDK_MOD1_MASK) {
    gimp_message(view->manager->gimp, G_OBJECT(view), GIMP_MESSAGE_ERROR,
                 _("Alt+%d is used to switch to display %d and "
                   "cannot be remapped."),
                 accel_key - GDK_KEY_0,
                 accel_key == GDK_KEY_0 ? 10 : accel_key - GDK_KEY_0);
  } else if (!gtk_accel_map_change_entry(accel_path, accel_key, accel_mask,
                                         FALSE)) {
    GtkTreeModel *model;
    ...
    for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
      ...
      for (child_valid =
        ...
        gtk_tree_model_get(model, &child_iter,
                           GIMP_ACTION_VIEW_COLUMN_ACCEL_KEY, &child_accel_key,
                           GIMP_ACTION_VIEW_COLUMN_ACCEL_MASK,
                           &child_accel_mask, -1);

        if (accel_key == child_accel_key && accel_mask == child_accel_mask) {
          gtk_tree_model_get(model, &child_iter, GIMP_ACTION_VIEW_COLUMN_ACTION,
                             &conflict_action, -1);
          break;
        }
      }
    ...
    if (conflict_action != action) {
      if (conflict_action) {
        gimp_action_view_conflict_confirm(view, conflict_action, accel_key,
                                          accel_mask, accel_path);
...
static void gimp_action_view_accel_cleared(GtkCellRendererAccel *accel,
  ...
  if (!gtk_accel_map_change_entry(accel_path, 0, 0, FALSE)) {
    gimp_message_literal(view->manager->gimp, G_OBJECT(view),
                         GIMP_MESSAGE_ERROR, _("Removing shortcut failed."));
  }
}
