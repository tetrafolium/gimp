...
enum { PROP_0, PROP_TOOLBOX, PROP_TOOL_ITEM };
...
static void gimp_tool_button_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_tool_button_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);

static void gimp_tool_button_hierarchy_changed(GtkWidget *widget,
                                               GtkWidget *previous_toplevel);
static gboolean gimp_tool_button_draw(GtkWidget *widget, cairo_t *cr);
static gboolean gimp_tool_button_query_tooltip(GtkWidget *widget, gint x,
...
static void gimp_tool_button_add_menu_item(GimpToolButton *tool_button,
                                           GimpToolInfo *tool_info, gint index);
...
static void gimp_tool_button_class_init(GimpToolButtonClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_TOOLBOX,
      g_param_spec_object("toolbox", NULL, NULL, GIMP_TYPE_TOOLBOX,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(object_class, PROP_TOOL_ITEM,
                                  g_param_spec_object("tool-item", NULL, NULL,
                                                      GIMP_TYPE_TOOL_ITEM,
                                                      GIMP_PARAM_READWRITE));
}

static void gimp_tool_button_init(GimpToolButton *tool_button) {
...
static void gimp_tool_button_constructed(GObject *object) {
  ...
  g_signal_connect(gtk_bin_get_child(GTK_BIN(tool_button)),
                   "button-press-event",
                   G_CALLBACK(gimp_tool_button_button_press), tool_button);
  g_signal_connect(gtk_bin_get_child(GTK_BIN(tool_button)),
                   "button-release-event",
                   G_CALLBACK(gimp_tool_button_button_release), tool_button);
  g_signal_connect(gtk_bin_get_child(GTK_BIN(tool_button)), "scroll-event",
                   G_CALLBACK(gimp_tool_button_scroll), tool_button);
...
static void gimp_tool_button_hierarchy_changed(GtkWidget *widget,
  ...
  if (GTK_WIDGET_CLASS(parent_class)->hierarchy_changed) {
    GTK_WIDGET_CLASS(parent_class)
        ->hierarchy_changed(widget, previous_toplevel);
  }

  palette =
      gtk_widget_get_ancestor(GTK_WIDGET(tool_button), GTK_TYPE_TOOL_PALETTE);

  if (palette != tool_button->priv->palette) {
    if (tool_button->priv->palette) {
      g_signal_handlers_disconnect_by_func(tool_button->priv->palette,
    ...
    if (tool_button->priv->palette) {
      g_signal_connect(tool_button->priv->palette, "notify::icon-size",
...
static gboolean gimp_tool_button_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (GIMP_IS_TOOL_GROUP(tool_button->priv->tool_item)) {
    ...
    x1 = SIGNED_ROUND(allocation.width - (ARROW_BORDER + size * ARROW_SIZE));
    y1 = SIGNED_ROUND(allocation.height - (ARROW_BORDER + size * ARROW_SIZE));
...
static GtkWidget *gimp_tool_button_query_tooltip_add_tool(
    GimpToolButton *tool_button, GtkGrid *grid, gint row,
    GimpToolInfo *tool_info, const gchar *label_str, GtkIconSize icon_size) {
  GimpUIManager *ui_manager;
  GimpAction *action = NULL;
  GtkWidget *label;
  GtkWidget *image;

  ui_manager = gimp_dock_get_ui_manager(GIMP_DOCK(tool_button->priv->toolbox));
  ...
  image = gtk_image_new_from_icon_name(
      gimp_viewable_get_icon_name(GIMP_VIEWABLE(tool_info)), icon_size);
  ...
  if (action) {
    ...
    gtk_grid_attach(grid, accel_label, 2, row, 1, 1);
...
static gboolean gimp_tool_button_query_tooltip(GtkWidget *widget, gint x,
  ...
  if (!tool_button->priv->tooltip_widget) {
    ...
    label = gimp_tool_button_query_tooltip_add_tool(
        tool_button, GTK_GRID(grid), row++, tool_info, tooltip_labels[0],
        icon_size);
    ...
    if (GIMP_IS_TOOL_GROUP(tool_button->priv->tool_item)) {
      ...
      if (n_children > 1) {
        ...
        for (i = 0; i < n_children; i++) {
          ...
          other_tool_info =
              GIMP_TOOL_INFO(gimp_container_get_child_by_index(children, i));

          if (other_tool_info != tool_info) {
            gimp_tool_button_query_tooltip_add_tool(
                tool_button, GTK_GRID(grid), row++, other_tool_info,
                other_tool_info->label, icon_size);
          }
        }
      }
    }
...
static gboolean gimp_tool_button_button_press(GtkWidget *widget,
  ...
  if (GIMP_IS_TOOL_GROUP(tool_button->priv->tool_item)) {
    if (gdk_event_triggers_context_menu((GdkEvent *)event) &&
      ...
      timeout_event = (GdkEventButton *)gdk_event_copy((GdkEvent *)event);
      ...
      tool_button->priv->menu_timeout_id = g_timeout_add(
          MENU_TIMEOUT, (GSourceFunc)gimp_tool_button_menu_timeout,
          tool_button);
    }
  }
  ...
  if (event->type == GDK_2BUTTON_PRESS && event->button == 1) {
    ...
    gimp_window_strategy_show_dockable_dialog(
        ...
        context->gimp, gimp_dock_get_dialog_factory(dock),
        gimp_widget_get_monitor(widget), "gimp-tool-options");
...
static gboolean gimp_tool_button_scroll(GtkWidget *widget,
  ...
  if (tool_info && GIMP_IS_TOOL_GROUP(tool_button->priv->tool_item)) {
    ...
    children =
        gimp_viewable_get_children(GIMP_VIEWABLE(tool_button->priv->tool_item));

    n_children = gimp_container_get_n_children(children);

    index = gimp_container_get_child_index(children, GIMP_OBJECT(tool_info));
...
static gboolean
gimp_tool_button_menu_leave_notify(GtkMenu *menu, GdkEventCrossing *event,
                                   GimpToolButton *tool_button) {
...
static gboolean gimp_tool_button_menu_deactivate_idle(gpointer data) {
...
static void gimp_tool_button_update(GimpToolButton *tool_button) {
  ...
  gtk_tool_button_set_icon_name(
      ...
      tool_info ? gimp_viewable_get_icon_name(GIMP_VIEWABLE(tool_info)) : NULL);

  g_clear_object(&tool_button->priv->tooltip_widget);

  if (tool_info) {
    gimp_help_set_help_data(GTK_WIDGET(tool_button), tool_info->tooltip,
                            tool_info->help_id);
  } else {
...
static void gimp_tool_button_add_menu_item(GimpToolButton *tool_button,
  ...
  g_object_bind_property(tool_info, "visible", item, "visible",
  ...
  if (action) {
    gtk_accel_label_set_accel_closure(GTK_ACCEL_LABEL(label),
                                      gimp_action_get_accel_closure(action));

    if (ui_manager) {
      g_signal_emit_by_name(ui_manager, "connect-proxy", action, item);
    }
  }
...
static void gimp_tool_button_reconstruct_menu(GimpToolButton *tool_button) {
  ...
  if (GIMP_IS_TOOL_GROUP(tool_button->priv->tool_item)) {
    ...
    ui_manager =
        gimp_dock_get_ui_manager(GIMP_DOCK(tool_button->priv->toolbox));
    ...
    g_signal_connect(tool_button->priv->menu, "popped-up",
                     G_CALLBACK(gimp_tool_button_menu_popped_up), tool_button);
    g_signal_connect(tool_button->priv->menu, "deactivate",
                     G_CALLBACK(gimp_tool_button_menu_deactivate), tool_button);

    if (ui_manager) {
      gtk_menu_set_accel_group(GTK_MENU(tool_button->priv->menu),
                               gimp_ui_manager_get_accel_group(ui_manager));
    }

    tool_button->priv->menu_items =
        g_hash_table_new(g_direct_hash, g_direct_equal);

    gimp_container_foreach(
        children, (GFunc)gimp_tool_button_reconstruct_menu_add_menu_item,
        tool_button);
  }
}
...
static gboolean gimp_tool_button_show_menu(GimpToolButton *tool_button,
  ...
  gtk_menu_popup_at_widget(GTK_MENU(tool_button->priv->menu),
                           GTK_WIDGET(tool_button), GDK_GRAVITY_NORTH_EAST,
...
GtkToolItem *gimp_tool_button_new(GimpToolbox *toolbox,
  ...
  g_return_val_if_fail(tool_item == NULL || GIMP_IS_TOOL_ITEM(tool_item), NULL);

  return g_object_new(GIMP_TYPE_TOOL_BUTTON, "toolbox", toolbox, "tool-item",
                      tool_item, NULL);
}
...
void gimp_tool_button_set_tool_item(GimpToolButton *tool_button,
  ...
  if (tool_item != tool_button->priv->tool_item) {
    if (GIMP_IS_TOOL_GROUP(tool_button->priv->tool_item)) {
      ...
      g_signal_handlers_disconnect_by_func(tool_button->priv->tool_item,
                                           gimp_tool_button_active_tool_changed,
                                           tool_button);

      g_signal_handlers_disconnect_by_func(children, gimp_tool_button_tool_add,
                                           tool_button);
      g_signal_handlers_disconnect_by_func(
          children, gimp_tool_button_tool_remove, tool_button);
      g_signal_handlers_disconnect_by_func(
          children, gimp_tool_button_tool_reorder, tool_button);
    ...
    if (GIMP_IS_TOOL_GROUP(tool_button->priv->tool_item)) {
      ...
      g_signal_connect(tool_button->priv->tool_item, "active-tool-changed",
                       G_CALLBACK(gimp_tool_button_active_tool_changed),
                       tool_button);

      g_signal_connect(children, "add", G_CALLBACK(gimp_tool_button_tool_add),
                       tool_button);
      g_signal_connect(children, "remove",
                       G_CALLBACK(gimp_tool_button_tool_remove), tool_button);
      g_signal_connect(children, "reorder",
                       G_CALLBACK(gimp_tool_button_tool_reorder), tool_button);
