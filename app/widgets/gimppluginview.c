...
enum { COLUMN_FILE, COLUMN_PATH, N_COLUMNS };
...
static void gimp_plug_in_view_class_init(GimpPlugInViewClass *klass) {
  ...
  view_signals[CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpPlugInViewClass, changed), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);
}

static void gimp_plug_in_view_init(GimpPlugInView *view) {
  view->plug_in_hash = g_hash_table_new_full(
      g_file_hash, (GEqualFunc)g_file_equal, (GDestroyNotify)g_object_unref,
...
GtkWidget *gimp_plug_in_view_new(GSList *procedures) {
  ...
  store = gtk_list_store_new(N_COLUMNS, G_TYPE_FILE, /* COLUMN_FILE */
                             G_TYPE_STRING);         /* COLUMN_PATH */

  view = g_object_new(GIMP_TYPE_PLUG_IN_VIEW, "model", store, NULL);
  ...
  for (list = procedures; list; list = g_slist_next(list)) {
    ...
    if (!g_hash_table_lookup(GIMP_PLUG_IN_VIEW(view)->plug_in_hash, file)) {
      ...
      gtk_list_store_set(store, &iter, COLUMN_FILE, file, COLUMN_PATH, path,
                         -1);
  ...
  gtk_tree_view_column_set_attributes(column, cell, "text", COLUMN_PATH, NULL);

  gtk_tree_view_append_column(view, column);

  g_signal_connect(gtk_tree_view_get_selection(view), "changed",
                   G_CALLBACK(gimp_plug_in_view_selection_changed), view);
