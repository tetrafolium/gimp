...
enum { PROP_0, PROP_GIMP, PROP_CALLBACK, PROP_CALLBACK_DATA };
...
static void gimp_search_popup_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_search_popup_get_property(GObject *object, guint property_id,
...
static gboolean results_list_key_press_event(GtkWidget *widget,
                                             GdkEventKey *kevent,
                                             GimpSearchPopup *popup);
static void results_list_row_activated(GtkTreeView *treeview, GtkTreePath *path,
...
static void gimp_search_popup_class_init(GimpSearchPopupClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("gimp", NULL, NULL, G_TYPE_OBJECT,
   ...
   */
  g_object_class_install_property(
      object_class, PROP_CALLBACK_DATA,
      g_param_spec_pointer("callback-data", NULL, NULL,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_search_popup_init(GimpSearchPopup *search_popup) {
...
GtkWidget *gimp_search_popup_new(Gimp *gimp, const gchar *role,
  ...
  widget = g_object_new(GIMP_TYPE_SEARCH_POPUP, "type", GTK_WINDOW_TOPLEVEL,
                        "type-hint", GDK_WINDOW_TYPE_HINT_DIALOG, "decorated",
                        TRUE, "modal", TRUE, "role", role, "title", title,

                        "gimp", gimp, "callback", callback, "callback-data",
                        callback_data, NULL);
...
void gimp_search_popup_add_result(GimpSearchPopup *popup, GimpAction *action,
  ...
  markup = g_strdup_printf(
      "%s<small>%s%s%s<span weight='light'>%s</span></small>", escaped_label,
      has_shortcut ? " | " : "", has_shortcut ? escaped_accel : "",
  ...
  if (gtk_tree_model_get_iter_first(model, &next_section)) {
    while (TRUE) {
      ...
      gtk_tree_model_get(model, &next_section, COLUMN_SECTION, &iter_section,
                         -1);
  ...
  gtk_list_store_set(store, &iter, COLUMN_ICON, icon_name, COLUMN_MARKUP,
                     markup, COLUMN_TOOLTIP, action_name, COLUMN_ACTION, action,
                     COLUMN_SECTION, section, COLUMN_SENSITIVE,
                     gimp_action_is_sensitive(action), -1);
...
static void gimp_search_popup_constructed(GObject *object) {
  ...
  gtk_box_pack_start(GTK_BOX(main_vbox), popup->priv->keyword_entry, FALSE,
                     FALSE, 0);
  gtk_widget_show(popup->priv->keyword_entry);

  gimp_search_popup_setup_results(&popup->priv->results_list,
                                  &popup->priv->list_view);
  gtk_box_pack_start(GTK_BOX(main_vbox), popup->priv->list_view, TRUE, TRUE, 0);
  ...
  g_signal_connect(popup->priv->keyword_entry, "key-press-event",
                   G_CALLBACK(keyword_entry_key_press_event), popup);
  g_signal_connect(popup->priv->keyword_entry, "key-release-event",
                   G_CALLBACK(keyword_entry_key_release_event), popup);

  g_signal_connect(popup->priv->results_list, "key-press-event",
                   G_CALLBACK(results_list_key_press_event), popup);
  g_signal_connect(popup->priv->results_list, "row-activated",
                   G_CALLBACK(results_list_row_activated), popup);
}
...
static gboolean keyword_entry_key_release_event(GtkWidget *widget,
  ...
  if (strcmp(entry_text, "") != 0) {
    gtk_window_resize(GTK_WINDOW(popup), width, window_height);
    ...
    gtk_list_store_clear(GTK_LIST_STORE(gtk_tree_view_get_model(tree_view)));
    gtk_widget_show_all(popup->priv->list_view);
    popup->priv->build_results(popup, NULL, popup->priv->build_results_data);
    gtk_tree_selection_select_path(gtk_tree_view_get_selection(tree_view),
                                   gtk_tree_path_new_from_string("0"));
  } else {
...
static gboolean results_list_key_press_event(GtkWidget *widget,
  ...
  case GDK_KEY_Up: {
    ...
    selection =
        gtk_tree_view_get_selection(GTK_TREE_VIEW(popup->priv->results_list));
    gtk_tree_selection_set_mode(selection, GTK_SELECTION_SINGLE);
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
      ...
      if (strcmp(gtk_tree_path_to_string(path), "0") == 0) {
        ...
        gtk_editable_get_selection_bounds(
            GTK_EDITABLE(popup->priv->keyword_entry), &start_pos, &end_pos);
...
static void gimp_search_popup_setup_results(GtkWidget **results_list,
  ...
  *list_view = gtk_scrolled_window_new(NULL, NULL);
  store = gtk_list_store_new(N_COL, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
                             GIMP_TYPE_ACTION, G_TYPE_BOOLEAN, G_TYPE_INT);
  *results_list = gtk_tree_view_new_with_model(GTK_TREE_MODEL(store));
  ...
  column = gtk_tree_view_column_new_with_attributes(NULL, cell, "icon-name",
                                                    COLUMN_ICON, "sensitive",
                                                    COLUMN_SENSITIVE, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(*results_list), column);
  gtk_tree_view_column_set_min_width(column, 22);

  cell = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      NULL, cell, "markup", COLUMN_MARKUP, "sensitive", COLUMN_SENSITIVE, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(*results_list), column);
...
static gchar *gimp_search_popup_find_accel_label(GimpAction *action) {
  ...
  if (accel_closure) {
    ...
    key = gtk_accel_group_find(
        accel_group, gimp_search_popup_view_accel_find_func, accel_closure);
