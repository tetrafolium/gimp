...
enum { SESSION_INFO_SIDE, SESSION_INFO_POSITION, SESSION_INFO_BOOK };
...
GTokenType gimp_session_info_dock_deserialize(GScanner *scanner, gint scope,
                                              GimpSessionInfoDock **dock_info,
  ...
  while (g_scanner_peek_next_token(scanner) == token) {
    ...
    case G_TOKEN_SYMBOL:
      ...
      case SESSION_INFO_BOOK:
        ...
        token = gimp_session_info_book_deserialize(scanner, scope + 1, &book);
...
GimpSessionInfoDock *gimp_session_info_dock_from_widget(GimpDock *dock) {
  ...
  dock_info = gimp_session_info_dock_new(GIMP_IS_TOOLBOX(dock) ? "gimp-toolbox"
                                                               : "gimp-dock");
...
GimpDock *gimp_session_info_dock_restore(GimpSessionInfoDock *dock_info,
  ...
  dock = gimp_dialog_factory_dialog_new(factory, monitor, ui_manager, NULL,
                                        dock_info->dock_type, -1 /*view_size*/,
                                        FALSE /*present*/);
   ...
   */
  gimp_dock_container_add_dock(dock_container, GIMP_DOCK(dock), dock_info);

  /* Note that if it is a toolbox, we will get here even though we
   * don't have any books
   */
  for (iter = dock_info->books; iter; iter = g_list_next(iter)) {
    GimpSessionInfoBook *book_info = iter->data;
    GtkWidget *dockbook;

    dockbook =
        GTK_WIDGET(gimp_session_info_book_restore(book_info, GIMP_DOCK(dock)));
