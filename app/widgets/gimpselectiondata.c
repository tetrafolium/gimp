...
static gchar *gimp_unescape_uri_string(const char *escaped, int len,
...
void gimp_selection_data_set_uri_list(GtkSelectionData *selection,
  ...
  for (list = uri_list; list; list = g_list_next(list)) {
    if (vals) {
      gchar *tmp =
          g_strconcat(vals, list->data, list->next ? "\n" : NULL, NULL);
      g_free(vals);
      vals = tmp;
    } else {
      vals = g_strconcat(list->data, list->next ? "\n" : NULL, NULL);
    }
  }

  gtk_selection_data_set(selection, gtk_selection_data_get_target(selection), 8,
                         (guchar *)vals, strlen(vals));
    ...
    if (filename) {
      ...
      if (start != dnd_crap) {
        ...
        if (strstr(dnd_crap, "%")) {
          ...
          unescaped_filename = gimp_unescape_uri_string(start, -1, "/", FALSE);
           ...
           */
          local_filename =
              g_filename_from_utf8(unescaped_filename, -1, NULL, NULL, NULL);
        ...
        if (!uri) {
          gchar *escaped_filename = g_strescape(unescaped_filename, NULL);
...
gboolean gimp_selection_data_get_color(GtkSelectionData *selection,
  ...
  gimp_rgba_set_uchar(
      color, (guchar)(color_vals[0] >> 8), (guchar)(color_vals[1] >> 8),
      (guchar)(color_vals[2] >> 8), (guchar)(color_vals[3] >> 8));
...
void gimp_selection_data_set_xcf(GtkSelectionData *selection,
  ...
  xcf_save_stream(image->gimp, image, G_OUTPUT_STREAM(output), NULL, NULL,
                  NULL);
...
void gimp_selection_data_set_stream(GtkSelectionData *selection,
                                    const guchar *stream, gsize stream_length) {
  g_return_if_fail(selection != NULL);
  g_return_if_fail(stream != NULL);
  g_return_if_fail(stream_length > 0);

  gtk_selection_data_set(selection, gtk_selection_data_get_target(selection), 8,
                         (guchar *)stream, stream_length);
}
...
GimpCurve *gimp_selection_data_get_curve(GtkSelectionData *selection) {
  ...
  if (!gimp_config_deserialize_string(
          GIMP_CONFIG(curve),
          (const gchar *)gtk_selection_data_get_data(selection), length, NULL,
          &error)) {
...
GimpImage *gimp_selection_data_get_image(GtkSelectionData *selection,
  ...
  if (str) {
    ...
    if (sscanf(str, "%i:%i", &pid, &ID) == 2 && pid == gimp_get_pid()) {
...
GimpImage *gimp_selection_data_get_component(GtkSelectionData *selection,
  ...
  if (str) {
    ...
    if (sscanf(str, "%i:%i:%i", &pid, &ID, &ch) == 3 && pid == gimp_get_pid()) {
...
void gimp_selection_data_set_item(GtkSelectionData *selection, GimpItem *item) {
  gchar *str;
...
GimpBrush *gimp_selection_data_get_brush(GtkSelectionData *selection,
  ...
  return (GimpBrush *)gimp_selection_data_get_object(
      selection, gimp_data_factory_get_container(gimp->brush_factory),
...
GimpPattern *gimp_selection_data_get_pattern(GtkSelectionData *selection,
  ...
  return (GimpPattern *)gimp_selection_data_get_object(
      selection, gimp_data_factory_get_container(gimp->pattern_factory),
...
GimpGradient *gimp_selection_data_get_gradient(GtkSelectionData *selection,
  ...
  return (GimpGradient *)gimp_selection_data_get_object(
      selection, gimp_data_factory_get_container(gimp->gradient_factory),
...
GimpPalette *gimp_selection_data_get_palette(GtkSelectionData *selection,
  ...
  return (GimpPalette *)gimp_selection_data_get_object(
      selection, gimp_data_factory_get_container(gimp->palette_factory),
...
GimpFont *gimp_selection_data_get_font(GtkSelectionData *selection,
  ...
  return (GimpFont *)gimp_selection_data_get_object(
      selection, gimp_data_factory_get_container(gimp->font_factory),
...
GimpBuffer *gimp_selection_data_get_buffer(GtkSelectionData *selection,
  ...
  return (GimpBuffer *)gimp_selection_data_get_object(
      selection, gimp->named_buffers,
...
GimpToolItem *gimp_selection_data_get_tool_item(GtkSelectionData *selection,
  ...
  tool_item = (GimpToolItem *)gimp_selection_data_get_object(
      selection, gimp->tool_info_list,
      GIMP_OBJECT(gimp_tool_info_get_standard(gimp)));

  if (!tool_item) {
    tool_item = (GimpToolItem *)gimp_selection_data_get_object(
        selection, gimp->tool_item_list, NULL);
  }
...
static GimpObject *gimp_selection_data_get_object(GtkSelectionData *selection,
  ...
  if (str) {
    ...
    if (sscanf(str, "%i:%p:%n", &pid, &object_addr, &name_offset) >= 2 &&
      ...
      GIMP_LOG(DND, "pid = %d, addr = %p, name = '%s'", pid, object_addr, name);

      if (additional && strcmp(name, gimp_object_get_name(additional)) == 0 &&
