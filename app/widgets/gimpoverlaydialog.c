...
enum { PROP_0, PROP_TITLE, PROP_ICON_NAME };

enum { RESPONSE, DETACH, CLOSE, LAST_SIGNAL };
...
static void gimp_overlay_dialog_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_overlay_dialog_get_property(GObject *object, guint property_id,
...
G_DEFINE_TYPE(GimpOverlayDialog, gimp_overlay_dialog, GIMP_TYPE_OVERLAY_FRAME)
...
static void gimp_overlay_dialog_class_init(GimpOverlayDialogClass *klass) {
  ...
  widget_class->get_preferred_width_for_height =
      gimp_overlay_dialog_get_preferred_width_for_height;
  widget_class->get_preferred_height_for_width =
      gimp_overlay_dialog_get_preferred_height_for_width;
  ...
  signals[RESPONSE] =
      g_signal_new("response", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpOverlayDialogClass, response), NULL,
                   NULL, NULL, G_TYPE_NONE, 1, G_TYPE_INT);

  signals[DETACH] = g_signal_new(
      "detach", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(GimpOverlayDialogClass, detach), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  signals[CLOSE] = g_signal_new("close", G_OBJECT_CLASS_TYPE(klass),
                                G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                                G_STRUCT_OFFSET(GimpOverlayDialogClass, close),
                                NULL, NULL, NULL, G_TYPE_NONE, 0);

  gtk_binding_entry_add_signal(gtk_binding_set_by_class(klass), GDK_KEY_Escape,
                               0, "close", 0);

  gtk_widget_class_set_css_name(GTK_WIDGET_CLASS(klass), "GimpOverlayDialog");
}

static void gimp_overlay_dialog_init(GimpOverlayDialog *dialog) {
...
static void gimp_overlay_dialog_constructed(GObject *object) {
  ...
  dialog->icon_image = image =
      gtk_image_new_from_icon_name(dialog->icon_name, GTK_ICON_SIZE_MENU);
  ...
  gimp_help_set_help_data(dialog->detach_button, _("Detach dialog from canvas"),
                          NULL);

  image = gtk_image_new_from_icon_name(GIMP_ICON_DETACH, GTK_ICON_SIZE_MENU);
  gtk_image_set_pixel_size(GTK_IMAGE(image), 12);
  gtk_container_add(GTK_CONTAINER(button), image);
  gtk_widget_show(image);
...
static void gimp_overlay_dialog_get_preferred_width(GtkWidget *widget,
  ...
  gtk_widget_get_preferred_width(dialog->header, &header_minimum,
                                 &header_natural);
  gtk_widget_get_preferred_width(dialog->action_area, &action_minimum,
                                 &action_natural);
...
static void gimp_overlay_dialog_get_preferred_height(GtkWidget *widget,
  ...
  gtk_widget_get_preferred_height(dialog->header, &header_minimum,
                                  &header_natural);
  gtk_widget_get_preferred_height(dialog->action_area, &action_minimum,
                                  &action_natural);
...
static void gimp_overlay_dialog_get_preferred_width_for_height(
  ...
  GTK_WIDGET_CLASS(parent_class)
      ->get_preferred_width_for_height(widget, height, minimum_width,
                                       natural_width);
...
static void gimp_overlay_dialog_get_preferred_height_for_width(
  ...
  GTK_WIDGET_CLASS(parent_class)
      ->get_preferred_height_for_width(widget, width, minimum_height,
                                       natural_height);
...
static void gimp_overlay_dialog_size_allocate(GtkWidget *widget,
  ...
  if (child && gtk_widget_get_visible(child)) {
    ...
    child_allocation.y =
        (allocation->y + 2 * border_width + header_requisition.height);
    child_allocation.width = MAX(allocation->width - 2 * border_width, 0);
    child_allocation.height =
        MAX(allocation->height - 4 * border_width - header_requisition.height -
  ...
  action_allocation.y =
      (child_allocation.y + child_allocation.height + border_width);
...
static void gimp_overlay_dialog_forall(GtkContainer *container,
                                       ...
                                       GtkCallback callback,
                                       gpointer callback_data) {
  GTK_CONTAINER_CLASS(parent_class)
...
GtkWidget *gimp_overlay_dialog_new(GimpToolInfo *tool_info, const gchar *desc,
                                   ...) {
  ...
  dialog = g_object_new(GIMP_TYPE_OVERLAY_DIALOG, "title", tool_info->label,
...
void gimp_overlay_dialog_response(GimpOverlayDialog *dialog, gint response_id) {
  g_return_if_fail(GIMP_IS_OVERLAY_DIALOG(dialog));

  g_signal_emit(dialog, signals[RESPONSE], 0, response_id);
}

void gimp_overlay_dialog_add_buttons_valist(GimpOverlayDialog *dialog,
                                            va_list args) {
...
GtkWidget *gimp_overlay_dialog_add_button(GimpOverlayDialog *dialog,
                                          const gchar *button_text,
                                          gint response_id) {
  GtkWidget *button;
  ResponseData *ad;
...
void gimp_overlay_dialog_set_alternative_button_order(
    GimpOverlayDialog *overlay, gint n_ids, gint *ids) {
  /* TODO */
}
...
static ResponseData *get_response_data(GtkWidget *widget, gboolean create) {
  ResponseData *ad =
      g_object_get_data(G_OBJECT(widget), "gimp-overlay-dialog-response-data");

  if (!ad && create) {
    ad = g_slice_new(ResponseData);

    g_object_set_data_full(G_OBJECT(widget),
                           "gimp-overlay-dialog-response-data", ad,
                           response_data_free);
  }
