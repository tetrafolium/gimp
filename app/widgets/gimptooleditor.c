...
static gboolean gimp_tool_editor_drop_possible(
...
static void gimp_tool_editor_eye_clicked(GtkCellRendererToggle *toggle,
                                         gchar *path_str, GdkModifierType state,
                                         GimpToolEditor *tool_editor);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpToolEditor, gimp_tool_editor, GIMP_TYPE_CONTAINER_TREE_VIEW,
...
static void gimp_tool_editor_class_init(GimpToolEditorClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  GimpContainerTreeViewClass *tree_view_class =
      GIMP_CONTAINER_TREE_VIEW_CLASS(klass);
...
static void gimp_tool_editor_init(GimpToolEditor *tool_editor) {
...
static void gimp_tool_editor_constructed(GObject *object) {
  ...
  view_size = gimp_container_view_get_view_size(container_view, &border_width);
  ...
  gimp_dnd_viewable_dest_add(GTK_WIDGET(tree_view->view), GIMP_TYPE_TOOL_ITEM,
                             NULL, NULL);
  ...
  /* construct tree view */
  {
    ...
    gtk_tree_view_column_set_cell_data_func(
        column, eye_cell, gimp_tool_editor_eye_data_func, tree_view, NULL);

    gimp_container_tree_view_add_toggle_cell(tree_view, eye_cell);

    g_signal_connect(eye_cell, "clicked",
                     G_CALLBACK(gimp_tool_editor_eye_clicked), tool_editor);
  }

  /* buttons */
  tool_editor->priv->new_group_button = gimp_editor_add_button(
      GIMP_EDITOR(tool_editor), GIMP_ICON_FOLDER_NEW,
      _("Create a new tool group"), NULL,
      G_CALLBACK(gimp_tool_editor_new_group_clicked), NULL, tool_editor);

  tool_editor->priv->raise_button = gimp_editor_add_button(
      GIMP_EDITOR(tool_editor), GIMP_ICON_GO_UP, _("Raise this item"),
      _("Raise this item to the top"),
      G_CALLBACK(gimp_tool_editor_raise_clicked),
      G_CALLBACK(gimp_tool_editor_raise_extend_clicked), tool_editor);

  tool_editor->priv->lower_button = gimp_editor_add_button(
      GIMP_EDITOR(tool_editor), GIMP_ICON_GO_DOWN, _("Lower this item"),
      _("Lower this item to the bottom"),
      G_CALLBACK(gimp_tool_editor_lower_clicked),
      G_CALLBACK(gimp_tool_editor_lower_extend_clicked), tool_editor);

  tool_editor->priv->delete_button = gimp_editor_add_button(
      GIMP_EDITOR(tool_editor), GIMP_ICON_EDIT_DELETE,
      _("Delete this tool group"), NULL,
      G_CALLBACK(gimp_tool_editor_delete_clicked), NULL, tool_editor);

  tool_editor->priv->reset_button = gimp_editor_add_button(
      GIMP_EDITOR(tool_editor), GIMP_ICON_RESET,
      _("Reset tool order and visibility"), NULL,
      G_CALLBACK(gimp_tool_editor_reset_clicked), NULL, tool_editor);

  gimp_tool_editor_update_sensitivity(tool_editor);
}

static gboolean gimp_tool_editor_select_item(GimpContainerView *container_view,
                                             GimpViewable *viewable,
                                             gpointer insert_data) {
  ...
  result =
      parent_view_iface->select_item(container_view, viewable, insert_data);
...
static gboolean gimp_tool_editor_drop_possible(
    ...
    GtkTreeViewDropPosition drop_pos, GtkTreeViewDropPosition *return_drop_pos,
    GdkDragAction *return_drag_action) {
  if (GIMP_CONTAINER_TREE_VIEW_CLASS(parent_class)
...
static void gimp_tool_editor_drop_viewables(GimpContainerTreeView *tree_view,
  ...
  GIMP_CONTAINER_TREE_VIEW_CLASS(parent_class)
      ->drop_viewables(tree_view, src_viewables, dest_viewable, drop_pos);
...
static void gimp_tool_editor_new_group_clicked(GtkButton *button,
  ...
  if (tool_item) {
    ...
    container =
        gimp_tool_editor_get_tool_item_container(tool_editor, tool_item);

    index = gimp_container_get_child_index(container, GIMP_OBJECT(tool_item));
  } else {
...
static void gimp_tool_editor_raise_clicked(GtkButton *button,
  ...
  if (tool_item) {
    ...
    if (index > 0) {
      gimp_container_reorder(container, GIMP_OBJECT(tool_item), index - 1);
    }
  }
}
...
static void gimp_tool_editor_raise_extend_clicked(GtkButton *button,
  ...
  if (tool_item && (mask & GDK_SHIFT_MASK)) {
    ...
    if (index > 0) {
      gimp_container_reorder(container, GIMP_OBJECT(tool_item), 0);
    }
  }
}
...
static void gimp_tool_editor_lower_extend_clicked(GtkButton *button,
  ...
  if (tool_item && (mask & GDK_SHIFT_MASK)) {
    ...
    gimp_container_reorder(container, GIMP_OBJECT(tool_item), index);
  }
}
...
static void gimp_tool_editor_delete_clicked(GtkButton *button,
  ...
  if (tool_item) {
    ...
    index =
        gimp_container_get_child_index(dest_container, GIMP_OBJECT(tool_item));
    dest_index = index;
    ...
    gimp_container_view_select_item(
        container_view, GIMP_VIEWABLE(gimp_container_get_child_by_index(
                            dest_container, index)));
...
static void gimp_tool_editor_reset_clicked(GtkButton *button,
  ...
  gimp_tools_reset(tool_editor->priv->context->gimp,
                   tool_editor->priv->container, FALSE);
}
...
static void gimp_tool_editor_tool_item_notify(GimpToolItem *tool_item,
  ...
  iter = gimp_container_view_lookup(container_view, GIMP_VIEWABLE(tool_item));
...
static void gimp_tool_editor_eye_data_func(GtkTreeViewColumn *tree_column,
  ...
  g_object_set(cell, "active", gimp_tool_item_get_visible(tool_item),
               "inconsistent",
               gimp_tool_item_get_visible(tool_item) &&
...
static void gimp_tool_editor_update_container(GimpToolEditor *tool_editor) {
  ...
  if (container && context) {
    ...
    tool_editor->priv->tool_item_notify_handler = gimp_tree_handler_connect(
        container, "notify", G_CALLBACK(gimp_tool_editor_tool_item_notify),
        tool_editor);
...
static void gimp_tool_editor_update_sensitivity(GimpToolEditor *tool_editor) {
  ...
  if (tool_item) {
    ...
    gtk_widget_set_sensitive(tool_editor->priv->raise_button, index > 0);

    gtk_widget_set_sensitive(tool_editor->priv->lower_button,
                             index <
                                 gimp_container_get_n_children(container) - 1);
...
GtkWidget *gimp_tool_editor_new(GimpContainer *container, GimpContext *context,
  ...
  container_view = g_object_new(GIMP_TYPE_TOOL_EDITOR, "view-size", view_size,
                                "view-border-width", view_border_width, NULL);
...
void gimp_tool_editor_revert_changes(GimpToolEditor *tool_editor) {
  ...
  scanner =
      gimp_scanner_new_string(tool_editor->priv->initial_tool_state, -1, NULL);

  gimp_tools_deserialize(tool_editor->priv->context->gimp,
                         tool_editor->priv->container, scanner);
