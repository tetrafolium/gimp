...
static gboolean gimp_histogram_menu_sensitivity(gint value, gpointer data);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpHistogramEditor, gimp_histogram_editor, GIMP_TYPE_IMAGE_EDITOR,
...
static void gimp_histogram_editor_init(GimpHistogramEditor *editor) {
  ...
  const gchar *gimp_histogram_editor_labels[] = {
      N_("Mean:"),   N_("Std dev:"), N_("Median:"),
      N_("Pixels:"), N_("Count:"),   N_("Percentile:")};
  ...
  editor->menu = menu =
      gimp_prop_enum_combo_box_new(G_OBJECT(view), "histogram-channel", 0, 0);
  ...
  gimp_int_combo_box_set_sensitivity(GIMP_INT_COMBO_BOX(editor->menu),
                                     gimp_histogram_menu_sensitivity, editor,
                                     NULL);
  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(editor->menu),
                                view->channel);
  gtk_box_pack_start(GTK_BOX(hbox), menu, FALSE, FALSE, 0);

  gimp_help_set_help_data(editor->menu, _("Histogram channel"), NULL);

  menu = gimp_prop_enum_icon_box_new(G_OBJECT(view), "histogram-scale",
                                     "gimp-histogram", 0, 0);
  ...
  g_signal_connect_swapped(view, "draw",
                           G_CALLBACK(gimp_histogram_editor_view_draw), editor);
  ...
  for (i = 0; i < 6; i++) {
    ...
    gimp_label_set_attributes(GTK_LABEL(label), PANGO_ATTR_WEIGHT,
                              PANGO_WEIGHT_BOLD, PANGO_ATTR_SCALE,
                              PANGO_SCALE_SMALL, -1);
    ...
    editor->labels[i] = label =
        g_object_new(GTK_TYPE_LABEL, "xalign", 0.0, "yalign", 0.5,
                     "width-chars", i > 2 ? 9 : 5, NULL);
    gimp_label_set_attributes(GTK_LABEL(editor->labels[i]), PANGO_ATTR_SCALE,
                              PANGO_SCALE_SMALL, -1);
...
static void gimp_histogram_editor_set_aux_info(GimpDocked *docked,
  ...
  gimp_session_info_aux_set_props(G_OBJECT(view), aux_info, "histogram-channel",
...
static GList *gimp_histogram_editor_get_aux_info(GimpDocked *docked) {
  ...
  return g_list_concat(aux_info, gimp_session_info_aux_new_from_props(
                                     G_OBJECT(view), "histogram-channel",
                                     "histogram-scale", NULL));
}

static void gimp_histogram_editor_set_image(GimpImageEditor *image_editor,
                                            GimpImage *image) {
  ...
  if (image_editor->image) {
    ...
    g_signal_handlers_disconnect_by_func(
        image_editor->image, gimp_histogram_editor_layer_changed, editor);
    g_signal_handlers_disconnect_by_func(
        image_editor->image, gimp_histogram_editor_menu_update, editor);
  ...
  if (image) {
    ...
    g_signal_connect_object(image, "mask-changed",
                            G_CALLBACK(gimp_histogram_editor_update), editor,
                            G_CONNECT_SWAPPED);
  }
...
static void gimp_histogram_editor_layer_changed(GimpImage *image,
  ...
  if (editor->drawable) {
    ...
    g_signal_handlers_disconnect_by_func(
        editor->drawable, gimp_histogram_editor_name_update, editor);
    g_signal_handlers_disconnect_by_func(
        editor->drawable, gimp_histogram_editor_menu_update, editor);
    g_signal_handlers_disconnect_by_func(editor->drawable,
                                         gimp_histogram_editor_update, editor);
    g_signal_handlers_disconnect_by_func(
        editor->drawable, gimp_histogram_editor_buffer_update, editor);
    g_signal_handlers_disconnect_by_func(
        editor->drawable, gimp_histogram_editor_frozen_update, editor);
...
static gboolean gimp_histogram_editor_validate(GimpHistogramEditor *editor) {
    ...
    if (editor->drawable &&
      ...
      if (!editor->histogram) {
        ...
        gimp_histogram_clear_values(
            editor->histogram, babl_format_get_n_components(
...
static void gimp_histogram_editor_frozen_update(GimpHistogramEditor *editor,
  ...
  if (gimp_viewable_preview_is_frozen(GIMP_VIEWABLE(editor->drawable))) {
     ...
     */
    if (!editor->bg_histogram && gtk_widget_is_drawable(GTK_WIDGET(editor))) {
      if (editor->idle_id) {
        g_source_remove(editor->idle_id);
...
static void gimp_histogram_editor_buffer_update(GimpHistogramEditor *editor,
                                                const GParamSpec *pspec) {
  g_object_set(editor, "trc", gimp_drawable_get_trc(editor->drawable), NULL);
}
...
static void gimp_histogram_editor_update(GimpHistogramEditor *editor) {
  ...
  editor->idle_id = g_timeout_add_full(
      G_PRIORITY_LOW, 200, (GSourceFunc)gimp_histogram_editor_idle_update,
      editor, NULL);
}
...
static void gimp_histogram_editor_info_update(GimpHistogramEditor *editor) {
  ...
  if (hist) {
    ...
    count =
        gimp_histogram_get_count(hist, view->channel, view->start, view->end);

    g_snprintf(
        text, sizeof(text), "%.3f",
        gimp_histogram_get_mean(hist, view->channel, view->start, view->end));
    gtk_label_set_text(GTK_LABEL(editor->labels[0]), text);

    g_snprintf(text, sizeof(text), "%.3f",
               gimp_histogram_get_std_dev(hist, view->channel, view->start,
                                          view->end));
    gtk_label_set_text(GTK_LABEL(editor->labels[1]), text);

    g_snprintf(
        text, sizeof(text), "%.3f",
        gimp_histogram_get_median(hist, view->channel, view->start, view->end));
    ...
    g_snprintf(text, sizeof(text), "%.1f",
               (pixels > 0 ? (100.0 * count / pixels) : 0.0));
