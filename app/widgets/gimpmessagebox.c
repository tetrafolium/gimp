...
static void gimp_message_box_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_message_box_get_property(GObject *object, guint property_id,
...
static void gimp_message_box_set_label_text(GimpMessageBox *box, gint n,
                                            const gchar *format, va_list args)
    G_GNUC_PRINTF(3, 0);
static void gimp_message_box_set_label_markup(GimpMessageBox *box, gint n,
                                              const gchar *format, va_list args)
    G_GNUC_PRINTF(3, 0);
...
static void gimp_message_box_class_init(GimpMessageBoxClass *klass) {
  ...
  widget_class->get_preferred_height_for_width =
      gimp_message_box_get_preferred_height_for_width;
...
static void gimp_message_box_init(GimpMessageBox *box) {
  ...
  for (i = 0; i < 2; i++) {
    GtkWidget *label =
        g_object_new(GTK_TYPE_LABEL, "wrap", TRUE, "wrap-mode",
                     PANGO_WRAP_WORD_CHAR, "max-width-chars", 80, "selectable",
                     TRUE, "xalign", 0.0, "yalign", 0.5, NULL);
...
static void gimp_message_box_constructed(GObject *object) {
  ...
  if (box->icon_name) {
    box->image =
        gtk_image_new_from_icon_name(box->icon_name, GTK_ICON_SIZE_DIALOG);
...
static void gimp_message_box_get_preferred_width(GtkWidget *widget,
  ...
  if (box->image && gtk_widget_get_visible(box->image)) {
    ...
    gtk_widget_get_preferred_width(box->image, &image_minimum, &image_natural);
...
static void gimp_message_box_get_preferred_height(GtkWidget *widget,
  ...
  if (box->image && gtk_widget_get_visible(box->image)) {
    ...
    gtk_widget_get_preferred_height(box->image, &image_minimum, &image_natural);
...
static gboolean gimp_message_box_update(gpointer data) {
  ...
  message = g_strdup_printf(ngettext("Message repeated once.",
                                     "Message repeated %d times.", box->repeat),
                            box->repeat);
...
GtkWidget *gimp_message_box_new(const gchar *icon_name) {
  return g_object_new(GIMP_TYPE_MESSAGE_BOX, "icon-name", icon_name, NULL);
}
...
gint gimp_message_box_repeat(GimpMessageBox *box) {
  ...
  if (box->idle_id == 0) {
     ...
     */
    box->idle_id =
        g_idle_add_full(G_PRIORITY_LOW, gimp_message_box_update, box, NULL);
  }
