...
enum { TEXT_CHANGED, DIR_CHANGED, LAST_SIGNAL };
...
static void gimp_text_editor_class_init(GimpTextEditorClass *klass) {
  ...
  text_editor_signals[TEXT_CHANGED] =
      g_signal_new("text-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpTextEditorClass, text_changed), NULL,
                   NULL, NULL, G_TYPE_NONE, 0);

  text_editor_signals[DIR_CHANGED] =
      g_signal_new("dir-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpTextEditorClass, dir_changed), NULL,
                   NULL, NULL, G_TYPE_NONE, 0);
}
...
GtkWidget *gimp_text_editor_new(const gchar *title, GtkWindow *parent,
  ...
  editor = g_object_new(
      GIMP_TYPE_TEXT_EDITOR, "title", title, "role", "gimp-text-editor",
      "transient-for", parent, "help-func", gimp_standard_help_func, "help-id",
      GIMP_HELP_TEXT_EDITOR_DIALOG, "use-header-bar", use_header_bar, NULL);

  gtk_dialog_add_button(GTK_DIALOG(editor), _("_Close"), GTK_RESPONSE_CLOSE);

  g_signal_connect(editor, "response", G_CALLBACK(gtk_widget_destroy), NULL);

  g_signal_connect_object(text_buffer, "changed",
                          G_CALLBACK(gimp_text_editor_text_changed), editor, 0);

  editor->ui_manager =
      gimp_menu_factory_manager_new(menu_factory, "<TextEditor>", editor);

  content_area = gtk_dialog_get_content_area(GTK_DIALOG(editor));

  toolbar =
      gimp_ui_manager_get_widget(editor->ui_manager, "/text-editor-toolbar");
  ...
  style_editor = gimp_text_style_editor_new(
      gimp, text, text_buffer,
      gimp_data_factory_get_container(gimp->font_factory), xres, yres);
  ...
  gtk_box_pack_start(GTK_BOX(content_area), editor->font_toggle, FALSE, FALSE,
                     0);
  gtk_widget_show(editor->font_toggle);

  g_signal_connect(editor->font_toggle, "toggled",
                   G_CALLBACK(gimp_text_editor_font_toggled), editor);
