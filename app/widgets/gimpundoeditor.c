...
static void gimp_undo_editor_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
...
static void gimp_undo_editor_undo_event(GimpImage *image, GimpUndoEvent event,
                                        GimpUndo *undo, GimpUndoEditor *editor);

static void gimp_undo_editor_select_item(GimpContainerView *view,
                                         GimpUndo *undo, gpointer insert_data,
                                         GimpUndoEditor *editor);

G_DEFINE_TYPE_WITH_CODE(
    GimpUndoEditor, gimp_undo_editor, GIMP_TYPE_IMAGE_EDITOR,
    G_IMPLEMENT_INTERFACE(GIMP_TYPE_DOCKED, gimp_undo_editor_docked_iface_init))
...
static void gimp_undo_editor_class_init(GimpUndoEditorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("view-size", NULL, NULL, GIMP_TYPE_VIEW_SIZE,
...
static void gimp_undo_editor_constructed(GObject *object) {
  ...
  undo_editor->view =
      gimp_container_tree_view_new(NULL, NULL, undo_editor->view_size, 1);
  ...
  g_signal_connect(undo_editor->view, "select-item",
                   G_CALLBACK(gimp_undo_editor_select_item), undo_editor);

  undo_editor->undo_button = gimp_editor_add_action_button(
      GIMP_EDITOR(undo_editor), "edit", "edit-undo", NULL);

  undo_editor->redo_button = gimp_editor_add_action_button(
      GIMP_EDITOR(undo_editor), "edit", "edit-redo", NULL);

  undo_editor->clear_button = gimp_editor_add_action_button(
      GIMP_EDITOR(undo_editor), "edit", "edit-undo-clear", NULL);
}
...
static void gimp_undo_editor_set_image(GimpImageEditor *image_editor,
                                       GimpImage *image) {
  ...
  if (image_editor->image) {
    ...
    g_signal_handlers_disconnect_by_func(image_editor->image,
                                         gimp_undo_editor_undo_event, editor);
  }
  ...
  if (image_editor->image) {
    ...
    g_signal_connect(image_editor->image, "undo-event",
                     G_CALLBACK(gimp_undo_editor_undo_event), editor);
  }
}
...
static void gimp_undo_editor_set_context(GimpDocked *docked,
  ...
  gimp_container_view_set_context(GIMP_CONTAINER_VIEW(editor->view), context);
}
...
GtkWidget *gimp_undo_editor_new(GimpCoreConfig *config,
  ...
  return g_object_new(GIMP_TYPE_UNDO_EDITOR, "menu-factory", menu_factory,
                      "menu-identifier", "<Undo>", "ui-path", "/undo-popup",
                      "view-size", config->undo_preview_size, NULL);
}
...
static void gimp_undo_editor_fill(GimpUndoEditor *editor) {
  ...
  g_signal_handlers_block_by_func(editor->view, gimp_undo_editor_select_item,
                                  editor);
  ...
  g_signal_handlers_unblock_by_func(editor->view, gimp_undo_editor_select_item,
                                    editor);
}

static void gimp_undo_editor_clear(GimpUndoEditor *editor) {
  if (editor->container) {
    gimp_container_view_set_container(GIMP_CONTAINER_VIEW(editor->view), NULL);
...
static void gimp_undo_editor_undo_event(GimpImage *image, GimpUndoEvent event,
  ...
  case GIMP_UNDO_EVENT_UNDO_PUSHED:
    ...
    g_signal_handlers_unblock_by_func(editor->view,
                                      gimp_undo_editor_select_item, editor);
    break;
