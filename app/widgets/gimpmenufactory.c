...
G_DEFINE_TYPE_WITH_PRIVATE(GimpMenuFactory, gimp_menu_factory, GIMP_TYPE_OBJECT)
...
void gimp_menu_factory_manager_register(GimpMenuFactory *factory,
  ...
  factory->p->registered_menus =
      g_list_prepend(factory->p->registered_menus, entry);

  va_start(args, first_group);

  for (group = first_group; group; group = va_arg(args, const gchar *)) {
    entry->action_groups =
        g_list_prepend(entry->action_groups, g_strdup(group));
  }
...
static void gimp_menu_factory_manager_action_added(GimpActionGroup *group,
                                                   GimpAction *action,
    ...
    if (!strcmp(entry->identifier, identifier)) {
      ...
      for (list = entry->action_groups; list; list = g_list_next(list)) {
        ...
        g_signal_connect_object(
            group, "action-added",
            G_CALLBACK(gimp_menu_factory_manager_action_added), accel_group, 0);
      ...
      for (list = entry->managed_uis; list; list = g_list_next(list)) {
        ...
        gimp_ui_manager_ui_register(manager, ui_entry->ui_path,
                                    ui_entry->basename, ui_entry->setup_func);
      }
