...
static GimpProgress *gimp_progress_dialog_progress_start(GimpProgress *progress,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpProgressDialog, gimp_progress_dialog, GIMP_TYPE_DIALOG,
...
static void gimp_progress_dialog_init(GimpProgressDialog *dialog) {
  ...
  g_signal_connect(dialog->box, "destroy", G_CALLBACK(gtk_widget_destroyed),
                   &dialog->box);
...
static GimpProgress *gimp_progress_dialog_progress_start(GimpProgress *progress,
  ...
  if (gimp_progress_start(GIMP_PROGRESS(dialog->box), cancellable, "%s",
                          message)) {
...
GtkWidget *gimp_progress_dialog_new(void) {
  ...
  return g_object_new(GIMP_TYPE_PROGRESS_DIALOG, "title", _("Progress"), "role",
                      "progress", "skip-taskbar-hint", TRUE, "skip-pager-hint",
                      TRUE, "resizable", FALSE, "focus-on-map", FALSE,
                      "window-position", GTK_WIN_POS_CENTER, "use-header-bar",
                      use_header_bar, NULL);
}
