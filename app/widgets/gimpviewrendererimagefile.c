...
static void gimp_view_renderer_imagefile_render(GimpViewRenderer *renderer,
  ...
  pixbuf = gimp_view_renderer_get_frame_pixbuf(renderer, widget, width, height);

  if (!pixbuf) {
    GimpImagefile *imagefile = GIMP_IMAGEFILE(renderer->viewable);

    pixbuf = gimp_view_renderer_imagefile_get_icon(imagefile, widget,
                                                   MAX(width, height));
  }
...
static GdkPixbuf *get_icon_for_mime_type(const gchar *mime_type,
                                         gint pixel_size) {
  ...
  pixbuf =
      gtk_icon_theme_load_icon(icon_theme, icon_name->str, pixel_size, 0, NULL);
