...
enum { DOCK_WINDOW_ADDED, DOCK_WINDOW_REMOVED, LAST_SIGNAL };
...
static gboolean gimp_dialog_factory_set_user_pos(GtkWidget *dialog,
                                                 ...
                                                 gpointer data);
static gboolean gimp_dialog_factory_dialog_configure(
    GtkWidget *dialog, GdkEventConfigure *cevent, GimpDialogFactory *factory);
...
static void gimp_dialog_factory_class_init(GimpDialogFactoryClass *klass) {
  ...
  factory_signals[DOCK_WINDOW_ADDED] = g_signal_new(
      "dock-window-added", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpDialogFactoryClass, dock_window_added), NULL, NULL,
      NULL, G_TYPE_NONE, 1, GIMP_TYPE_DOCK_WINDOW);

  factory_signals[DOCK_WINDOW_REMOVED] = g_signal_new(
      "dock-window-removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpDialogFactoryClass, dock_window_removed), NULL, NULL,
      NULL, G_TYPE_NONE, 1, GIMP_TYPE_DOCK_WINDOW);
}
...
GimpDialogFactory *gimp_dialog_factory_new(const gchar *name,
  ...
  factory->p->dialog_state =
      (config->hide_docks ? GIMP_DIALOGS_HIDDEN_EXPLICITLY
                          : GIMP_DIALOGS_SHOWN);
...
void gimp_dialog_factory_register_entry(
    GimpDialogFactory *factory, const gchar *identifier, const gchar *name,
    const gchar *blurb, const gchar *icon_name, const gchar *help_id,
    GimpDialogNewFunc new_func, GimpDialogRestoreFunc restore_func,
    gint view_size, gboolean singleton, gboolean session_managed,
    gboolean remember_size, gboolean remember_if_open, gboolean hideable,
    gboolean image_window, gboolean dockable) {
  GimpDialogFactoryEntry *entry;
  ...
  factory->p->registered_dialogs =
      g_list_prepend(factory->p->registered_dialogs, entry);
}
 ...
 **/
static gboolean gimp_dialog_factory_dialog_sane(
    GimpDialogFactory *factory, GimpDialogFactory *widget_factory,
    GimpDialogFactoryEntry *widget_entry, GtkWidget *widget) {
  if (!widget_factory || !widget_entry) {
    g_warning("%s: dialog was not created by a GimpDialogFactory", G_STRFUNC);
    return FALSE;
  }
 ...
 **/
static GtkWidget *gimp_dialog_factory_dialog_new_internal(
    GimpDialogFactory *factory, GdkMonitor *monitor, GimpContext *context,
    GimpUIManager *ui_manager, GtkWidget *parent, const gchar *identifier,
    gint view_size, gboolean return_existing, gboolean present,
  ...
  if (!entry) {
    g_warning("%s: no entry registered for \"%s\"", G_STRFUNC, identifier);
    return NULL;
  }

  if (!entry->new_func) {
    g_warning("%s: entry for \"%s\" has no constructor", G_STRFUNC, identifier);
    return NULL;
  }
  ...
  if (!dialog) {
    ...
    if (create_containers) {
      if (entry->dockable) {
        ...
        dock_window = gimp_dialog_factory_dialog_new(
            factory, monitor, NULL /*ui_manager*/, parent,
            "gimp-toolbox-window", -1 /*view_size*/, FALSE /*present*/);
    ...
    if (context)
      dialog = gimp_dialog_factory_constructor(factory, entry, context,
                                               ui_manager, view_size);
    else if (dock)
      dialog = gimp_dialog_factory_constructor(
          factory, entry, gimp_dock_get_context(GIMP_DOCK(dock)),
          gimp_dock_get_ui_manager(GIMP_DOCK(dock)), view_size);
    else
      dialog = gimp_dialog_factory_constructor(
          factory, entry, factory->p->context, ui_manager, view_size);
    ...
    if (dialog) {
      ...
      if (dock) {
        if (GIMP_IS_DOCKABLE(dialog)) {
          gtk_notebook_append_page(GTK_NOTEBOOK(dockbook), dialog, NULL);
      ...
      g_warning("%s: constructor for \"%s\" returned NULL", G_STRFUNC,
                identifier);
  ...
  if (gtk_widget_is_toplevel(dialog)) {
    ...
    if (gimp_dockable_get_dockbook(dockable) &&
      ...
      GtkNotebook *notebook =
          GTK_NOTEBOOK(gimp_dockable_get_dockbook(dockable));
...
GtkWidget *gimp_dialog_factory_dialog_new(GimpDialogFactory *factory,
                                          ...
                                          GtkWidget *parent,
                                          const gchar *identifier,
                                          gint view_size, gboolean present) {
  g_return_val_if_fail(GIMP_IS_DIALOG_FACTORY(factory), NULL);
  g_return_val_if_fail(GDK_IS_MONITOR(monitor), NULL);
  g_return_val_if_fail(identifier != NULL, NULL);

  return gimp_dialog_factory_dialog_new_internal(
      factory, monitor, factory->p->context, ui_manager, parent, identifier,
      view_size, FALSE /*return_existing*/, present,
...
void gimp_dialog_factory_add_session_info(GimpDialogFactory *factory,
   ...
   */
  factory->p->session_infos =
      g_list_append(factory->p->session_infos, g_object_ref(info));
}
...
GtkWidget *gimp_dialog_factory_dialog_raise(GimpDialogFactory *factory,
  ...
  dialog = gimp_dialog_factory_dialog_new_internal(
      factory, monitor, NULL, NULL, parent, ids[i] ? ids[i] : ids[0], view_size,
      TRUE /*return_existing*/, TRUE /*present*/, TRUE /*create_containers*/);
  g_strfreev(ids);
...
GtkWidget *gimp_dialog_factory_dockable_new(GimpDialogFactory *factory,
  ...
  return gimp_dialog_factory_dialog_new_internal(
      factory, gimp_widget_get_monitor(GTK_WIDGET(dock)),
      gimp_dock_get_context(dock), gimp_dock_get_ui_manager(dock), NULL,
      identifier, view_size, FALSE /*return_existing*/, FALSE /*present*/,
      FALSE /*create_containers*/);
}

void gimp_dialog_factory_add_dialog(GimpDialogFactory *factory,
                                    GtkWidget *dialog, GdkMonitor *monitor) {
  ...
  if (entry) {
    ...
    GIMP_LOG(DIALOG_FACTORY, "adding %s \"%s\"",
             toplevel ? "toplevel" : "dockable", entry->identifier);
      ...
      if (gimp_session_info_get_factory_entry(current_info) == entry) {
        if (gimp_session_info_get_widget(current_info)) {
          if (gimp_session_info_is_singleton(current_info)) {
            g_warning("%s: singleton dialog \"%s\" created twice", G_STRFUNC,
                      entry->identifier);

            GIMP_LOG(DIALOG_FACTORY, "corrupt session info: %p (widget %p)",
        ...
        if (toplevel && gimp_session_info_is_session_managed(current_info) &&
            !gtk_widget_get_visible(dialog)) {
          GimpGuiConfig *gui_config;

          gui_config = GIMP_GUI_CONFIG(factory->p->context->gimp->config);

          gimp_session_info_apply_geometry(current_info, monitor,
    ...
    if (!info) {
      ...
      GIMP_LOG(DIALOG_FACTORY,
               "creating session info %p (widget %p) for %s \"%s\"", info,
      ...
      if (gimp_session_info_is_session_managed(info)) {
        ...
        g_signal_connect(dialog, "configure-event",
                         G_CALLBACK(gimp_dialog_factory_set_user_pos), NULL);
      }
...
void gimp_dialog_factory_add_foreign(GimpDialogFactory *factory,
                                     const gchar *identifier, GtkWidget *dialog,
                                     GdkMonitor *monitor) {
  ...
  if (dialog_factory || entry) {
    g_warning("%s: dialog was created by a GimpDialogFactory", G_STRFUNC);
    return;
  }
...
void gimp_dialog_factory_position_dialog(GimpDialogFactory *factory,
  ...
  if (!info) {
    g_warning("%s: no session info found for \"%s\"", G_STRFUNC, identifier);
    return;
  }
  ...
  gimp_session_info_apply_geometry(info, monitor, gui_config->restore_monitor);
}
...
void gimp_dialog_factory_remove_dialog(GimpDialogFactory *factory,
  ...
  GIMP_LOG(DIALOG_FACTORY, "removing \"%s\" (dialog = %p)", entry->identifier,
           dialog);
  ...
  for (list = factory->p->session_infos; list; list = g_list_next(list)) {
    ...
    if (gimp_session_info_get_widget(session_info) == dialog) {
      GIMP_LOG(DIALOG_FACTORY,
               "clearing session info %p (widget %p) for \"%s\"", session_info,
               gimp_session_info_get_widget(session_info), entry->identifier);
      ...
      g_signal_handlers_disconnect_by_func(
          dialog, gimp_dialog_factory_set_user_pos, NULL);
      g_signal_handlers_disconnect_by_func(
          dialog, gimp_dialog_factory_remove_dialog, factory);

      if (gimp_session_info_is_session_managed(session_info))
        g_signal_handlers_disconnect_by_func(
            dialog, gimp_dialog_factory_dialog_configure, factory);

      if (GIMP_IS_DOCK_WINDOW(dialog)) {
        /*  don't save session info for empty docks  */
        factory->p->session_infos =
            g_list_remove(factory->p->session_infos, session_info);
...
gimp_dialog_factory_from_widget(GtkWidget *dialog,
  ...
  if (entry)
    *entry =
        g_object_get_qdata(G_OBJECT(dialog), gimp_dialog_factory_entry_key);
...
gboolean gimp_dialog_factory_get_has_min_size(GtkWindow *window) {
  ...
  return GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(window), GIMP_DIALOG_FACTORY_MIN_SIZE_KEY));
}
...
static GtkWidget *gimp_dialog_factory_constructor(GimpDialogFactory *factory,
  ...
  if (widget && entry->dockable) {
    ...
    dockable = gimp_dockable_new(entry->name, entry->blurb, entry->icon_name,
                                 entry->help_id);
...
static void gimp_dialog_factory_set_widget_data(GtkWidget *dialog,
  ...
  if (entry)
    g_object_set_qdata(G_OBJECT(dialog), gimp_dialog_factory_entry_key, entry);
}
...
static gboolean gimp_dialog_factory_set_user_pos(GtkWidget *dialog,
  ...
  g_signal_handlers_disconnect_by_func(dialog, gimp_dialog_factory_set_user_pos,
                                       data);
  ...
  gtk_window_set_geometry_hints(GTK_WINDOW(dialog), NULL, NULL, geometry_mask);
  ...
  for (list = factory->p->session_infos; list; list = g_list_next(list)) {
    ...
    if (gimp_session_info_get_widget(session_info) == dialog) {
      ...
      GIMP_LOG(DIALOG_FACTORY,
               ...
               "(x=%d y=%d  %dx%d)",
               entry->identifier, gimp_session_info_get_x(session_info),
...
void gimp_dialog_factory_restore(GimpDialogFactory *factory,
  ...
  for (infos = factory->p->session_infos; infos; infos = g_list_next(infos)) {
    ...
    if (gimp_session_info_get_open(info)) {
      ...
      GIMP_LOG(DIALOG_FACTORY, "skipping to restore session info %p, not open",
               info);
    }
  }
}
...
static void gimp_dialog_factory_hide(GimpDialogFactory *factory) {
  ...
  for (list = factory->p->open_dialogs; list; list = g_list_next(list)) {
    ...
    if (GTK_IS_WIDGET(widget) && gtk_widget_is_toplevel(widget)) {
      ...
      g_object_set_data(G_OBJECT(widget), GIMP_DIALOG_VISIBILITY_KEY,
...
static void gimp_dialog_factory_show(GimpDialogFactory *factory) {
  ...
  for (list = factory->p->open_dialogs; list; list = g_list_next(list)) {
    ...
    if (GTK_IS_WIDGET(widget) && gtk_widget_is_toplevel(widget)) {
      ...
      visibility = GPOINTER_TO_INT(
          g_object_get_data(G_OBJECT(widget), GIMP_DIALOG_VISIBILITY_KEY));
...
void gimp_dialog_factory_set_busy(GimpDialogFactory *factory) {
  ...
  for (list = factory->p->open_dialogs; list; list = g_list_next(list)) {
    ...
    if (GTK_IS_WIDGET(widget) && gtk_widget_is_toplevel(widget)) {
      ...
      if (window) {
        GdkCursor *cursor = gimp_cursor_new(
            window, GIMP_HANDEDNESS_RIGHT, (GimpCursorType)GDK_WATCH,
            GIMP_TOOL_CURSOR_NONE, GIMP_CURSOR_MODIFIER_NONE);
...
void gimp_dialog_factory_set_singleton(GimpDialogFactory *factory) {
  g_return_if_fail(gimp_toplevel_factory == NULL || factory == NULL);
