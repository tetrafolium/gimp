...
static gboolean open_tag(GimpTextBuffer *buffer, GString *string,
                         GtkTextTag *tag) {
  const gchar *name;
  ...
  name =
      gimp_text_buffer_tag_to_name(buffer, tag, &attribute, &attribute_value);

  if (name) {
    if (attribute && attribute_value) {
      gchar *escaped = g_markup_escape_text(attribute_value, -1);

      g_string_append_printf(string, "<%s %s=\"%s\">", name, attribute,
                             escaped);
...
static gboolean close_tag(GimpTextBuffer *buffer, GString *string,
                          GtkTextTag *tag) {
...
typedef enum { STATE_START, STATE_MARKUP, STATE_TAG, STATE_UNKNOWN } ParseState;
...
static void set_error(GError **err, GMarkupParseContext *context,
                      int error_domain, int error_code, const char *format, ...)
    G_GNUC_PRINTF(5, 6);
...
static void parse_tag_element(GMarkupParseContext *context,
                              const gchar *element_name,
                              const gchar **attribute_names,
                              const gchar **attribute_values, ParseInfo *info,
                              GError **error) {
  GtkTextTag *tag;
...
static void start_element_handler(GMarkupParseContext *context,
  ...
  case STATE_TAG:
  case STATE_UNKNOWN:
    parse_tag_element(context, element_name, attribute_names, attribute_values,
                      info, error);
    break;
...
static void text_handler(GMarkupParseContext *context, const gchar *text,
  ...
  if (all_whitespace(text, text_len) && peek_state(info) != STATE_MARKUP &&
      peek_state(info) != STATE_TAG && peek_state(info) != STATE_UNKNOWN)
    return;
...
static void parse_info_init(ParseInfo *info, GtkTextBuffer *register_buffer,
                            GtkTextBuffer *content_buffer) {
...
static void insert_text(ParseInfo *info, GtkTextIter *iter) {
  ...
  mark = gtk_text_buffer_create_mark(
      info->content_buffer, "deserialize-insert-point", &start_iter, TRUE);
...
gboolean gimp_text_buffer_deserialize(GtkTextBuffer *register_buffer,
                                      GtkTextBuffer *content_buffer,
                                      GtkTextIter *iter, const guint8 *text,
  ...
  if (!g_markup_parse_context_parse(context, (const gchar *)text, length,
                                    error))
    goto out;
  ...
  do {
    ...
    for (list = tags; list; list = g_slist_next(list)) {
      ...
      if (g_list_find(buffer->kerning_tags, tag)) {
        ...
        gtk_text_buffer_insert_with_tags(content, &iter, WORD_JOINER, -1, tag,
                                         NULL);
