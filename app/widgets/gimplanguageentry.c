...
static void gimp_language_entry_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_language_entry_get_property(GObject *object, guint property_id,
...
static void gimp_language_entry_class_init(GimpLanguageEntryClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("model", NULL, NULL, GIMP_TYPE_LANGUAGE_STORE,
                          G_PARAM_CONSTRUCT_ONLY | GIMP_PARAM_READWRITE));
}

static void gimp_language_entry_init(GimpLanguageEntry *entry) {}
...
static void gimp_language_entry_constructed(GObject *object) {
  ...
  if (entry->store) {
    ...
    completion = g_object_new(GTK_TYPE_ENTRY_COMPLETION, "model", entry->store,
                              "inline-selection", TRUE, NULL);

    /* Note that we must use this function to set the text column,
     * otherwise we won't get a cell renderer for free.
     */
    gtk_entry_completion_set_text_column(completion, GIMP_LANGUAGE_STORE_LABEL);

    gtk_entry_set_completion(GTK_ENTRY(entry), completion);
    g_object_unref(completion);

    g_signal_connect(completion, "match-selected",
                     G_CALLBACK(gimp_language_entry_language_selected), entry);
  }
}
...
gimp_language_entry_language_selected(GtkEntryCompletion *completion,
  ...
  gtk_tree_model_get(model, iter, GIMP_LANGUAGE_STORE_CODE, &entry->code, -1);
...
GtkWidget *gimp_language_entry_new(void) {
  ...
  entry = g_object_new(GIMP_TYPE_LANGUAGE_ENTRY, "model", store, NULL);
...
gboolean gimp_language_entry_set_code(GimpLanguageEntry *entry,
  ...
  if (gimp_language_store_lookup(GIMP_LANGUAGE_STORE(entry->store), code,
                                 &iter)) {
    gchar *label;

    gtk_tree_model_get(GTK_TREE_MODEL(entry->store), &iter,
                       GIMP_LANGUAGE_STORE_LABEL, &label,
                       GIMP_LANGUAGE_STORE_CODE, &entry->code, -1);
