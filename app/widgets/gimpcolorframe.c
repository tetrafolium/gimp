...
static void gimp_color_frame_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);
static void gimp_color_frame_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);

static void gimp_color_frame_style_updated(GtkWidget *widget);
static gboolean gimp_color_frame_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_color_frame_class_init(GimpColorFrameClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("mode", NULL, NULL, GIMP_TYPE_COLOR_PICK_MODE,
                        GIMP_COLOR_PICK_MODE_PIXEL, GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_HAS_NUMBER,
                                  g_param_spec_boolean("has-number", NULL, NULL,
                                                       FALSE,
                                                       GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_NUMBER,
      g_param_spec_int("number", NULL, NULL, 0, 256, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_HAS_COLOR_AREA,
                                  g_param_spec_boolean("has-color-area", NULL,
                                                       NULL, FALSE,
                                                       GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_HAS_COORDS,
                                  g_param_spec_boolean("has-coords", NULL, NULL,
                                                       FALSE,
                                                       GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_ELLIPSIZE,
      g_param_spec_enum("ellipsize", NULL, NULL, PANGO_TYPE_ELLIPSIZE_MODE,
                        PANGO_ELLIPSIZE_NONE, GIMP_PARAM_READWRITE));
}
...
static void gimp_color_frame_init(GimpColorFrame *frame) {
  ...
  store = gimp_enum_store_new_with_values(
      GIMP_TYPE_COLOR_PICK_MODE, GIMP_COLOR_PICK_MODE_LAST + 1,
      GIMP_COLOR_PICK_MODE_PIXEL, GIMP_COLOR_PICK_MODE_RGB_PERCENT,
      GIMP_COLOR_PICK_MODE_RGB_U8, GIMP_COLOR_PICK_MODE_HSV,
      GIMP_COLOR_PICK_MODE_LCH, GIMP_COLOR_PICK_MODE_LAB,
      GIMP_COLOR_PICK_MODE_XYY, GIMP_COLOR_PICK_MODE_YUV,
  ...
  g_signal_connect(frame->combo, "changed",
                   G_CALLBACK(gimp_color_frame_combo_callback), frame);
  ...
  frame->color_area =
      g_object_new(GIMP_TYPE_COLOR_AREA, "color", &frame->color, "type",
                   GIMP_COLOR_AREA_SMALL_CHECKS, "drag-mask", GDK_BUTTON1_MASK,
                   "draw-border", TRUE, "height-request", 20, NULL);
  ...
  for (i = 0; i < GIMP_COLOR_FRAME_ROWS; i++) {
    ...
    gtk_box_pack_start(GTK_BOX(hbox), frame->name_labels[i], FALSE, FALSE, 0);
    ...
    gtk_box_pack_end(GTK_BOX(hbox), frame->value_labels[i], TRUE, TRUE, 0);
  ...
  gtk_box_pack_end(GTK_BOX(frame->coords_box_x), frame->coords_label_x, FALSE,
                   FALSE, 0);
  ...
  gtk_box_pack_end(GTK_BOX(frame->coords_box_y), frame->coords_label_y, FALSE,
                   FALSE, 0);
...
static gboolean gimp_color_frame_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (frame->has_number) {
    ...
    scale = ((gdouble)(allocation.height - combo_allocation.height -
    ...
    cairo_move_to(cr, (allocation.width / 2.0) / scale - w / 2.0,
                  (allocation.height / 2.0 + combo_allocation.height / 2.0 +
                   color_area_allocation.height / 2.0 +
                   coords_box_x_allocation.height / 2.0 +
                   coords_box_y_allocation.height / 2.0) /
                          scale -
                      h / 2.0);
 ...
 **/
void gimp_color_frame_set_mode(GimpColorFrame *frame, GimpColorPickMode mode) {
  ...
  if (ellipsize != frame->ellipsize) {
    ...
    for (i = 0; i < GIMP_COLOR_FRAME_ROWS; i++) {
      if (frame->value_labels[i])
        gtk_label_set_ellipsize(GTK_LABEL(frame->value_labels[i]), ellipsize);
    }
  }
}
...
void gimp_color_frame_set_number(GimpColorFrame *frame, gint number) {
  g_return_if_fail(GIMP_IS_COLOR_FRAME(frame));
 ...
 **/
void gimp_color_frame_set_color(GimpColorFrame *frame, gboolean sample_average,
                                const Babl *sample_format, gpointer pixel,
                                const GimpRGB *color, gint x, gint y) {
  g_return_if_fail(GIMP_IS_COLOR_FRAME(frame));
  g_return_if_fail(color != NULL);

  if (frame->sample_valid && frame->sample_average == sample_average &&
      frame->sample_format == sample_format && frame->x == x && frame->y == y &&
  ...
  if (config != frame->config) {
    if (frame->config) {
      g_signal_handlers_disconnect_by_func(
          frame->config, gimp_color_frame_destroy_transform, frame);
...
static void gimp_color_frame_update(GimpColorFrame *frame) {
  const gchar *names[GIMP_COLOR_FRAME_ROWS] = {
      NULL,
  };
  gchar **values = NULL;
  gboolean has_alpha;
  ...
  if (frame->sample_valid) {
    ...
    gtk_label_set_text(GTK_LABEL(frame->coords_label_x),
                       C_("Coordinates", "n/a"));
    /* TRANSLATORS: n/a for Not Available. */
    gtk_label_set_text(GTK_LABEL(frame->coords_label_y),
                       C_("Coordinates", "n/a"));
  }
    ...
    if (frame->sample_valid) {
      ...
      case GIMP_PRECISION_U8_NON_LINEAR:
        if (babl_format_is_palette(frame->sample_format)) {
          print_format = gimp_babl_format(
              GIMP_RGB, GIMP_PRECISION_U8_NON_LINEAR, has_alpha, space);
          break;
        }
        /* else fall thru */
      ...
      case GIMP_PRECISION_HALF_PERCEPTUAL:
        print_format = gimp_babl_format(
            base_type, gimp_babl_precision(GIMP_COMPONENT_TYPE_FLOAT, trc),
            has_alpha, space);
        break;
      }
    ...
    if (base_type == GIMP_GRAY) {
      ...
      if (babl_format_is_palette(frame->sample_format)) {
        ...
        if (frame->sample_valid) {
          ...
          if (!frame->sample_average) {
            values[4] = g_strdup_printf("%d", ((guint8 *)frame->pixel)[0]);
          }
        }
      }
    }
    ...
    if (frame->sample_valid) {
      ...
      if (frame->transform) {
        ...
        gimp_color_transform_process_pixels(
            frame->transform, babl_format("R'G'B' double"), rgb_values,
            babl_format("CMYK double"), cmyk_values, 1);
  ...
  if (frame->config) {
    ...
    if (cmyk_profile) {
      ...
      frame->transform = gimp_color_transform_new(
          rgb_profile, babl_format("R'G'B' double"), cmyk_profile,
          babl_format("CMYK double"), GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL,
