...
#define GET_PRIVATE(p)                                                         \
  ((GimpToolPalettePrivate *)gimp_tool_palette_get_instance_private(           \
      (GimpToolPalette *)(p)))

static void gimp_tool_palette_finalize(GObject *object);

static GtkSizeRequestMode gimp_tool_palette_get_request_mode(GtkWidget *widget);
...
static void gimp_tool_palette_get_preferred_height(GtkWidget *widget,
                                                   gint *min_width,
                                                   gint *pref_width);
static void gimp_tool_palette_height_for_width(GtkWidget *widget, gint width,
                                               gint *min_height,
                                               gint *pref_height);
...
static void gimp_tool_palette_tool_reorder(GimpContainer *container,
                                           GimpToolItem *tool_item, gint index,
                                           GimpToolPalette *palette);
...
static void gimp_tool_palette_class_init(GimpToolPaletteClass *klass) {
  ...
  widget_class->get_preferred_height_for_width =
      gimp_tool_palette_height_for_width;
  widget_class->style_updated = gimp_tool_palette_style_updated;

  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_enum("tool-icon-size", NULL, NULL, GTK_TYPE_ICON_SIZE,
                        DEFAULT_TOOL_ICON_SIZE, GIMP_PARAM_READABLE));
...
static void gimp_tool_palette_init(GimpToolPalette *palette) {
...
static gint gimp_tool_palette_get_n_tools(GimpToolPalette *palette,
                                          gint *button_width,
                                          gint *button_height,
                                          gint *min_columns, gint *min_rows) {
  GimpToolPalettePrivate *private = GET_PRIVATE(palette);
  GimpToolItem *tool_item;
  GHashTableIter iter;
  GdkMonitor *monitor;
  ...
  if (!gimp_tool_palette_get_button_size(palette, button_width,
                                         button_height)) {
...
static void gimp_tool_palette_get_preferred_width(GtkWidget *widget,
  ...
  gimp_tool_palette_get_n_tools(GIMP_TOOL_PALETTE(widget), &button_width,
                                &button_height, &min_columns, &min_rows);
...
static void gimp_tool_palette_style_updated(GtkWidget *widget) {
  ...
  gtk_widget_style_get(widget, "button-relief", &relief, "tool-icon-size",
                       &tool_icon_size, NULL);
...
private
  ...
  for (list = gimp_get_tool_item_ui_iter(context->gimp); list;
  ...
  g_signal_connect_object(context->gimp->tool_item_ui_list, "add",
                          G_CALLBACK(gimp_tool_palette_tool_add), palette, 0);
  g_signal_connect_object(context->gimp->tool_item_ui_list, "remove",
                          G_CALLBACK(gimp_tool_palette_tool_remove), palette,
                          0);
  g_signal_connect_object(context->gimp->tool_item_ui_list, "reorder",
                          G_CALLBACK(gimp_tool_palette_tool_reorder), palette,
                          0);
}
...
static void gimp_tool_palette_tool_reorder(GimpContainer *container,
  ...
  if (tool_button) {
    gtk_tool_item_group_set_item_position(GTK_TOOL_ITEM_GROUP(private->group),
...
static void gimp_tool_palette_add_button(GimpToolPalette *palette,
  ...
  gtk_tool_item_group_insert(GTK_TOOL_ITEM_GROUP(private->group), tool_button,
                             index);
  gtk_widget_show(GTK_WIDGET(tool_button));

  g_object_bind_property(tool_item, "shown", tool_button, "visible-horizontal",
                         G_BINDING_SYNC_CREATE);
  g_object_bind_property(tool_item, "shown", tool_button, "visible-vertical",
                         G_BINDING_SYNC_CREATE);

  button = gtk_bin_get_child(GTK_BIN(tool_button));

  gtk_widget_style_get(GTK_WIDGET(palette), "button-relief", &relief, NULL);
