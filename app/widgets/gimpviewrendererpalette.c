...
static void gimp_view_renderer_palette_render(GimpViewRenderer *renderer,
  ...
  if (renderpal->cell_size > 0) {
    ...
    if (n_columns > 0)
      cell_width =
          MAX((gdouble)renderpal->cell_size,
              (gdouble)(renderer->width - grid_width) / (gdouble)n_columns);
    else
  ...
  renderpal->cell_height =
      MAX(4, ((renderer->height - grid_width) / renderpal->rows));
  ...
  transform = gimp_view_renderer_get_color_transform(
      renderer, widget, babl_format("cairo-RGB24"), babl_format("cairo-RGB24"));
    ...
    if (renderpal->draw_grid && (y % renderpal->cell_height) == 0) {
      ...
      if (transform) {
        gimp_color_transform_process_pixels(
            transform, babl_format("cairo-RGB24"), row,
            babl_format("cairo-RGB24"), dest, renderer->width);
      } else {
