...
enum { PROP_0, PROP_DATA_FACTORY, PROP_ACTION_GROUP };
...
static GObject *
gimp_data_factory_view_constructor(GType type, guint n_construct_params,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpDataFactoryView, gimp_data_factory_view, GIMP_TYPE_CONTAINER_EDITOR,
...
static void gimp_data_factory_view_class_init(GimpDataFactoryViewClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_DATA_FACTORY,
      g_param_spec_object("data-factory", NULL, NULL, GIMP_TYPE_DATA_FACTORY,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_ACTION_GROUP,
      g_param_spec_string("action-group", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}
...
gimp_data_factory_view_constructor(GType type, guint n_construct_params,
  ...
  object = G_OBJECT_CLASS(parent_class)
  ...
  factory_view->priv->tagged_container = gimp_tagged_container_new(
      gimp_data_factory_get_container(factory_view->priv->factory));

  /* this must happen in constructor(), because doing it in
   * set_property() warns about wrong construct property usage
   */
  g_object_set(object, "container", factory_view->priv->tagged_container, NULL);
...
static void gimp_data_factory_view_constructed(GObject *object) {
  ...
  if (GIMP_IS_CONTAINER_TREE_VIEW(editor->view)) {
    ...
    gimp_container_tree_view_connect_name_edited(
        tree_view, G_CALLBACK(gimp_data_factory_view_tree_name_edited),
        factory_view);
  }
  ...
  if (gimp_ui_manager_find_action(manager, priv->action_group, str))
    priv->edit_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor->view), priv->action_group, str, NULL);
  g_free(str);

  if (gimp_data_factory_view_has_data_new_func(factory_view)) {
    str = g_strdup_printf("%s-new", priv->action_group);
    priv->new_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor->view), priv->action_group, str, NULL);
    g_free(str);
  }
  ...
  if (gimp_ui_manager_find_action(manager, priv->action_group, str))
    priv->duplicate_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor->view), priv->action_group, str, NULL);
  g_free(str);

  str = g_strdup_printf("%s-delete", priv->action_group);
  if (gimp_ui_manager_find_action(manager, priv->action_group, str))
    priv->delete_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor->view), priv->action_group, str, NULL);
  g_free(str);

  str = g_strdup_printf("%s-refresh", priv->action_group);
  if (gimp_ui_manager_find_action(manager, priv->action_group, str))
    priv->refresh_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor->view), priv->action_group, str, NULL);
  g_free(str);

  /* Query tag entry */
  priv->query_tag_entry = gimp_combo_tag_entry_new(
      GIMP_TAGGED_CONTAINER(priv->tagged_container), GIMP_TAG_ENTRY_MODE_QUERY);
  gtk_box_pack_start(GTK_BOX(editor->view), priv->query_tag_entry, FALSE, FALSE,
                     0);
  gtk_box_reorder_child(GTK_BOX(editor->view), priv->query_tag_entry, 0);
  ...
  gtk_box_pack_start(GTK_BOX(editor->view), priv->assign_tag_entry, FALSE,
                     FALSE, 0);
  gtk_widget_show(priv->assign_tag_entry);

  if (priv->edit_button)
    gimp_container_view_enable_dnd(
        editor->view, GTK_BUTTON(priv->edit_button),
        gimp_data_factory_get_data_type(priv->factory));

  if (priv->duplicate_button)
    gimp_container_view_enable_dnd(
        editor->view, GTK_BUTTON(priv->duplicate_button),
        gimp_data_factory_get_data_type(priv->factory));

  if (priv->delete_button)
    gimp_container_view_enable_dnd(
        editor->view, GTK_BUTTON(priv->delete_button),
        gimp_data_factory_get_data_type(priv->factory));
...
static void gimp_data_factory_view_set_aux_info(GimpDocked *docked,
  ...
  for (list = aux_info; list; list = g_list_next(list)) {
    ...
    if (!strcmp(aux->name, AUX_INFO_TAG_FILTER)) {
      gtk_entry_set_text(GTK_ENTRY(view->priv->query_tag_entry), aux->value);
    }
  }
}
...
GtkWidget *gimp_data_factory_view_new(
  ...
  return g_object_new(
      GIMP_TYPE_DATA_FACTORY_VIEW, "view-type", view_type, "data-factory",
      factory, "context", context, "view-size", view_size, "view-border-width",
      view_border_width, "menu-factory", menu_factory, "menu-identifier",
      menu_identifier, "ui-path", ui_path, "action-group", action_group, NULL);
}
...
gboolean gimp_data_factory_view_have(GimpDataFactoryView *factory_view,
  ...
  return gimp_container_have(
      gimp_data_factory_get_container(factory_view->priv->factory), object);
}
...
static void gimp_data_factory_view_select_item(GimpContainerEditor *editor,
  ...
  if (view->priv->assign_tag_entry) {
    ...
    gimp_tag_entry_set_selected_items(
        GIMP_TAG_ENTRY(view->priv->assign_tag_entry), active_items);
    g_list_free(active_items);
  } else if (viewable) {
    view->priv->selected_items =
        g_list_append(view->priv->selected_items, viewable);
  }
}
...
static void gimp_data_factory_view_tree_name_edited(GtkCellRendererText *cell,
  ...
  if (gtk_tree_model_get_iter(tree_view->model, &iter, path)) {
    ...
    g_signal_handlers_block_by_func(
        cell, gimp_data_factory_view_tree_name_edited, view);
