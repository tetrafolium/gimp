...
static void image_preview_clicked(GtkWidget *widget, GdkModifierType state,
                                  GimpToolbox *toolbox) {
  ...
  gimp_window_strategy_show_dockable_dialog(
      ...
      gimp_widget_get_monitor(widget), "gimp-image-list|gimp-image-grid");
}

static void image_preview_drop_image(GtkWidget *widget, gint x, gint y,
...
static void image_preview_set_viewable(GimpView *view,
...
GtkWidget *gimp_toolbox_image_area_create(GimpToolbox *toolbox, gint width,
  ...
  image_view =
      gimp_view_new_full_by_types(context, GIMP_TYPE_VIEW, GIMP_TYPE_IMAGE,
                                  width, height, 0, FALSE, TRUE, TRUE);

  g_signal_connect(image_view, "set-viewable",
                   G_CALLBACK(image_preview_set_viewable), NULL);
  ...
  gimp_help_set_help_data(image_view, tooltip, GIMP_HELP_TOOLBOX_IMAGE_AREA);
  g_free(tooltip);

  g_signal_connect_object(context, "image-changed",
                          G_CALLBACK(gimp_view_set_viewable), image_view,
                          G_CONNECT_SWAPPED);

  g_signal_connect(image_view, "clicked", G_CALLBACK(image_preview_clicked),
                   toolbox);

  gimp_dnd_viewable_dest_add(image_view, GIMP_TYPE_IMAGE,
                             image_preview_drop_image, context);
