...
static gboolean gimp_item_tree_view_drop_possible(
    GimpContainerTreeView *view, GimpDndType src_type, GList *src_viewables,
    GimpViewable *dest_viewable, GtkTreePath *drop_path,
    GtkTreeViewDropPosition drop_pos, GtkTreeViewDropPosition *return_drop_pos,
    GdkDragAction *return_drag_action);
static void gimp_item_tree_view_drop_viewables(
    GimpContainerTreeView *view, GList *src_viewables,
    GimpViewable *dest_viewable, GtkTreeViewDropPosition drop_pos);

static void gimp_item_tree_view_new_dropped(GtkWidget *widget, gint x, gint y,
                                            GimpViewable *viewable,
                                            gpointer data);
static void gimp_item_tree_view_new_list_dropped(GtkWidget *widget, gint x,
                                                 gint y, GList *viewables,
                                                 gpointer data);
...
static void gimp_item_tree_view_eye_clicked(GtkCellRendererToggle *toggle,
                                            gchar *path, GdkModifierType state,
                                            GimpItemTreeView *view);
static void gimp_item_tree_view_chain_clicked(GtkCellRendererToggle *toggle,
                                              gchar *path,
                                              GdkModifierType state,
                                              GimpItemTreeView *view);
static void gimp_item_tree_view_lock_content_toggled(GtkWidget *widget,
                                                     GimpItemTreeView *view);
static void gimp_item_tree_view_lock_position_toggled(GtkWidget *widget,
                                                      GimpItemTreeView *view);
static void gimp_item_tree_view_update_options(GimpItemTreeView *view,
                                               GList *items);

static gboolean gimp_item_tree_view_item_pre_clicked(
    GimpCellRendererViewable *cell, const gchar *path_str,
    GdkModifierType state, GimpItemTreeView *item_view);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpItemTreeView, gimp_item_tree_view, GIMP_TYPE_CONTAINER_TREE_VIEW,
...
static void gimp_item_tree_view_class_init(GimpItemTreeViewClass *klass) {
  ...
  view_signals[SET_IMAGE] =
      g_signal_new("set-image", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpItemTreeViewClass, set_image), NULL,
                   NULL, NULL, G_TYPE_NONE, 1, GIMP_TYPE_OBJECT);
...
static void gimp_item_tree_view_docked_iface_init(GimpDockedInterface *iface) {
...
static void gimp_item_tree_view_init(GimpItemTreeView *view) {
  ...
  view->priv->model_column_visible = gimp_container_tree_store_columns_add(
      tree_view->model_columns, &tree_view->n_model_columns, G_TYPE_BOOLEAN);

  view->priv->model_column_viewable = gimp_container_tree_store_columns_add(
      tree_view->model_columns, &tree_view->n_model_columns, G_TYPE_BOOLEAN);

  view->priv->model_column_linked = gimp_container_tree_store_columns_add(
      tree_view->model_columns, &tree_view->n_model_columns, G_TYPE_BOOLEAN);

  view->priv->model_column_color_tag = gimp_container_tree_store_columns_add(
      tree_view->model_columns, &tree_view->n_model_columns, GDK_TYPE_RGBA);
...
static void gimp_item_tree_view_constructed(GObject *object) {
  GimpItemTreeViewClass *item_view_class =
      GIMP_ITEM_TREE_VIEW_GET_CLASS(object);
  ...
  gimp_container_tree_view_connect_name_edited(
      tree_view, G_CALLBACK(gimp_item_tree_view_name_edited), item_view);

  g_signal_connect(tree_view->view, "row-expanded",
                   G_CALLBACK(gimp_item_tree_view_row_expanded), tree_view);

  g_signal_connect(tree_view->renderer_cell, "pre-clicked",
                   G_CALLBACK(gimp_item_tree_view_item_pre_clicked), item_view);

  column = gtk_tree_view_column_new();
  gtk_tree_view_insert_column(tree_view->view, column, 0);

  item_view->priv->eye_cell = gimp_cell_renderer_toggle_new(GIMP_ICON_VISIBLE);
  g_object_set(item_view->priv->eye_cell, "xpad", 0, "ypad", 0,
               "override-background", TRUE, NULL);
  gtk_tree_view_column_pack_start(column, item_view->priv->eye_cell, FALSE);
  gtk_tree_view_column_set_attributes(
      column, item_view->priv->eye_cell, "active",
      item_view->priv->model_column_visible, "inconsistent",
      item_view->priv->model_column_viewable, "cell-background-rgba",
      item_view->priv->model_column_color_tag, NULL);
  ...
  g_signal_connect(item_view->priv->eye_cell, "clicked",
                   G_CALLBACK(gimp_item_tree_view_eye_clicked), item_view);
  ...
  g_object_set(item_view->priv->chain_cell, "xpad", 0, "ypad", 0, NULL);
  gtk_tree_view_column_pack_start(column, item_view->priv->chain_cell, FALSE);
  gtk_tree_view_column_set_attributes(
      column, item_view->priv->chain_cell, "active",
      item_view->priv->model_column_linked, NULL);

  gimp_container_tree_view_add_toggle_cell(tree_view,
                                           item_view->priv->chain_cell);

  g_signal_connect(item_view->priv->chain_cell, "clicked",
                   G_CALLBACK(gimp_item_tree_view_chain_clicked), item_view);
  ...
  gimp_dnd_drag_dest_set_by_type(
      GTK_WIDGET(tree_view->view), GTK_DEST_DEFAULT_HIGHLIGHT,
      item_view_class->item_type, TRUE, GDK_ACTION_MOVE | GDK_ACTION_COPY);

  item_view->priv->new_button = gimp_editor_add_action_button(
      editor, item_view_class->action_group, item_view_class->new_action,
      item_view_class->new_default_action, GDK_SHIFT_MASK, NULL);
  /*  connect "drop to new" manually as it makes a difference whether
   *  it was clicked or dropped
   */
  gimp_dnd_viewable_list_dest_add(
      item_view->priv->new_button, item_view_class->item_type,
      gimp_item_tree_view_new_list_dropped, item_view);
  gimp_dnd_viewable_dest_add(item_view->priv->new_button,
                             item_view_class->item_type,
                             gimp_item_tree_view_new_dropped, item_view);

  item_view->priv->raise_button = gimp_editor_add_action_button(
      editor, item_view_class->action_group, item_view_class->raise_action,
      item_view_class->raise_top_action, GDK_SHIFT_MASK, NULL);

  item_view->priv->lower_button = gimp_editor_add_action_button(
      editor, item_view_class->action_group, item_view_class->lower_action,
      item_view_class->lower_bottom_action, GDK_SHIFT_MASK, NULL);
  ...
  gtk_container_child_set(GTK_CONTAINER(hbox),
                          item_view->priv->multi_selection_label, "expand",
                          TRUE, "fill", TRUE, NULL);

  /*  Lock content toggle  */
  item_view->priv->lock_content_toggle = gtk_toggle_button_new();
  gtk_box_pack_start(GTK_BOX(hbox), item_view->priv->lock_content_toggle, FALSE,
                     FALSE, 0);
  gtk_box_reorder_child(GTK_BOX(hbox), item_view->priv->lock_content_toggle, 0);
  ...
  gtk_widget_style_get(GTK_WIDGET(item_view), "button-icon-size", &icon_size,
                       NULL);

  image = gtk_image_new_from_icon_name(item_view_class->lock_content_icon_name,
                                       icon_size);
  gtk_container_add(GTK_CONTAINER(item_view->priv->lock_content_toggle), image);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), item_view->priv->lock_position_toggle,
                     FALSE, FALSE, 0);
  gtk_box_reorder_child(GTK_BOX(hbox), item_view->priv->lock_position_toggle,
                        1);
...
static void gimp_item_tree_view_style_updated(GtkWidget *widget) {
  ...
  gtk_widget_style_get(widget, "button-relief", &button_relief,
                       "button-icon-size", &button_icon_size, "content-spacing",
                       &content_spacing, "button-spacing", &button_spacing,
                       NULL);
  ...
  if (view->priv->lock_box) {
    ...
    children = gtk_container_get_children(GTK_CONTAINER(view->priv->lock_box));
    ...
    for (list = children; list; list = g_list_next(list)) {
      ...
      if (GTK_IS_BUTTON(child)) {
        ...
        if (GTK_IS_IMAGE(image)) {
          ...
          gtk_image_get_icon_name(GTK_IMAGE(image), &icon_name, &old_size);
  ...
  g_object_set(view->priv->eye_cell, "icon-name", GIMP_ICON_VISIBLE, NULL);
  g_object_set(view->priv->chain_cell, "icon-name", GIMP_ICON_LINKED, NULL);

  GTK_WIDGET_CLASS(parent_class)->style_updated(widget);
}

GtkWidget *
gimp_item_tree_view_new(GType view_type, gint view_size, gint view_border_width,
                        gboolean multiple_selection, GimpImage *image,
  ...
  g_return_val_if_fail(
      view_size > 0 && view_size <= GIMP_VIEWABLE_MAX_PREVIEW_SIZE, NULL);
  ...
  item_view = g_object_new(
      view_type, "reorderable", TRUE, "menu-factory", menu_factory,
      "menu-identifier", menu_identifier, "ui-path", ui_path, "selection-mode",
      multiple_selection ? GTK_SELECTION_MULTIPLE : GTK_SELECTION_SINGLE, NULL);

  gimp_container_view_set_view_size(GIMP_CONTAINER_VIEW(item_view), view_size,
                                    view_border_width);
...
void gimp_item_tree_view_set_image(GimpItemTreeView *view, GimpImage *image) {
...
void gimp_item_tree_view_add_options(GimpItemTreeView *view, const gchar *label,
                                     GtkWidget *options) {
  gint content_spacing;
  gint button_spacing;
  ...
  gtk_widget_style_get(GTK_WIDGET(view), "content-spacing", &content_spacing,
  ...
  if (!view->priv->options_box) {
    ...
    view->priv->options_box =
        gtk_box_new(GTK_ORIENTATION_VERTICAL, content_spacing);
  ...
  if (label) {
    ...
    gtk_box_pack_start(GTK_BOX(view->priv->options_box), options, FALSE, FALSE,
                       0);
    gtk_widget_show(options);
  }
}
...
GtkWidget *gimp_item_tree_view_get_lock_box(GimpItemTreeView *view) {
  ...
  if (!view->priv->lock_box) {
    ...
    gtk_widget_style_get(GTK_WIDGET(view), "button-spacing", &button_spacing,
                         NULL);

    view->priv->lock_box =
        gtk_box_new(GTK_ORIENTATION_HORIZONTAL, button_spacing);
...
gint gimp_item_tree_view_get_drop_index(GimpItemTreeView *view,
  ...
  g_return_val_if_fail(dest_viewable == NULL || GIMP_IS_VIEWABLE(dest_viewable),
                       -1);
...
static void gimp_item_tree_view_real_set_image(GimpItemTreeView *view,
  ...
  if (view->priv->image) {
    g_signal_handlers_disconnect_by_func(
        view->priv->image, gimp_item_tree_view_item_changed, view);
    g_signal_handlers_disconnect_by_func(
        view->priv->image, gimp_item_tree_view_size_changed, view);

    gimp_container_view_set_container(GIMP_CONTAINER_VIEW(view), NULL);

    g_signal_handlers_disconnect_by_func(view->priv->image,
                                         gimp_item_tree_view_image_flush, view);
  }
  ...
  if (view->priv->image) {
    ...
    g_signal_connect(view->priv->image,
                     ...
                     G_CALLBACK(gimp_item_tree_view_item_changed), view);
    g_signal_connect(view->priv->image, "size-changed",
                     G_CALLBACK(gimp_item_tree_view_size_changed), view);

    g_signal_connect(view->priv->image, "flush",
                     G_CALLBACK(gimp_item_tree_view_image_flush), view);
...
static void gimp_item_tree_view_set_container(GimpContainerView *view,
  ...
  if (container) {
    item_view->priv->visible_changed_handler = gimp_tree_handler_connect(
        container, "visibility-changed",
        G_CALLBACK(gimp_item_tree_view_visible_changed), view);

    item_view->priv->linked_changed_handler = gimp_tree_handler_connect(
        container, "linked-changed",
        G_CALLBACK(gimp_item_tree_view_linked_changed), view);

    item_view->priv->color_tag_changed_handler = gimp_tree_handler_connect(
        container, "color-tag-changed",
        G_CALLBACK(gimp_item_tree_view_color_tag_changed), view);

    item_view->priv->lock_content_changed_handler = gimp_tree_handler_connect(
        container, "lock-content-changed",
        G_CALLBACK(gimp_item_tree_view_lock_content_changed), view);

    item_view->priv->lock_position_changed_handler = gimp_tree_handler_connect(
        container, "lock-position-changed",
        G_CALLBACK(gimp_item_tree_view_lock_position_changed), view);
  }
}
...
static void gimp_item_tree_view_set_context(GimpContainerView *view,
  ...
  if (old_context) {
    g_signal_handlers_disconnect_by_func(
        old_context, gimp_item_tree_view_set_image, item_view);
  }
...
static gpointer gimp_item_tree_view_insert_item(GimpContainerView *view,
  ...
  iter =
      parent_view_iface->insert_item(view, viewable, parent_insert_data, index);

  item_view->priv->inserting_item = FALSE;

  has_color = gimp_get_color_tag_color(
      gimp_item_get_merged_color_tag(item), &color,
      gimp_item_get_color_tag(item) == GIMP_COLOR_TAG_NONE);

  gtk_tree_store_set(
      GTK_TREE_STORE(tree_view->model), iter,
      item_view->priv->model_column_visible, gimp_item_get_visible(item),
      item_view->priv->model_column_viewable,
      gimp_item_get_visible(item) && !gimp_item_is_visible(item),
      item_view->priv->model_column_linked, gimp_item_get_linked(item),
      item_view->priv->model_column_color_tag,
      has_color ? (GdkRGBA *)&color : NULL, -1);
...
static gboolean gimp_item_tree_view_select_item(GimpContainerView *view,
  ...
  if (item) {
    ...
    if (g_list_length(items) != 1 || items->data != (GimpItem *)item) {
      item_view_class->set_active_item(tree_view->priv->image, GIMP_ITEM(item));
  ...
  gimp_ui_manager_update(gimp_editor_get_ui_manager(GIMP_EDITOR(tree_view)),
                         tree_view);
...
static gboolean gimp_item_tree_view_select_items(GimpContainerView *view,
  ...
  g_signal_handlers_disconnect_by_func(
      ...
      G_CALLBACK(gimp_item_tree_view_selection_label_notify), tree_view);
  g_signal_handlers_disconnect_by_func(
      tree_view->priv->multi_selection_label,
      G_CALLBACK(gimp_item_tree_view_selection_label_notify), tree_view);
  if (g_list_length(items) > 1) {
    ...
    g_signal_connect(tree_view->priv->multi_selection_label, "notify::label",
                     ...
                     tree_view);
    g_signal_connect(
        tree_view->priv->multi_selection_label, "notify::selection-bound",
        G_CALLBACK(gimp_item_tree_view_selection_label_notify), tree_view);
    str = g_strdup_printf(
        ngettext("%d item selected", "%d items selected", g_list_length(items)),
        g_list_length(items));
...
static void gimp_item_tree_view_activate_item(GimpContainerView *view,
  ...
  if (item_view_class->activate_action) {
    gimp_ui_manager_activate_action(
        gimp_editor_get_ui_manager(GIMP_EDITOR(view)),
        item_view_class->action_group, item_view_class->activate_action);
  }
}
...
static gboolean gimp_item_tree_view_drop_possible(
    GimpContainerTreeView *tree_view, GimpDndType src_type,
    GList *src_viewables, GimpViewable *dest_viewable, GtkTreePath *drop_path,
    GtkTreeViewDropPosition drop_pos, GtkTreeViewDropPosition *return_drop_pos,
    GdkDragAction *return_drag_action) {
  GList *iter;
  ...
  return GIMP_CONTAINER_TREE_VIEW_CLASS(parent_class)
      ->drop_possible(tree_view, src_type, src_viewables, dest_viewable,
                      drop_path, drop_pos, return_drop_pos, return_drag_action);
}

static void gimp_item_tree_view_drop_viewables(
    GimpContainerTreeView *tree_view, GList *src_viewables,
    GimpViewable *dest_viewable, GtkTreeViewDropPosition drop_pos) {
  GimpItemTreeViewClass *item_view_class;
  ...
  for (iter = src_viewables; iter; iter = iter->next) {
    ...
    if (src_viewable_type == G_TYPE_NONE)
      ...
      if (g_type_is_a(src_viewable_type, G_TYPE_FROM_INSTANCE(src_viewable)))
      ...
      g_return_if_fail(
          g_type_is_a(G_TYPE_FROM_INSTANCE(src_viewable), src_viewable_type));
    }
  ...
  if (drop_pos == GTK_TREE_VIEW_DROP_AFTER ||
      (drop_pos == GTK_TREE_VIEW_DROP_INTO_OR_AFTER && dest_viewable &&
  ...
  if (item_view->priv->image != src_image ||
    ...
    gimp_image_undo_group_start(item_view->priv->image,
                                GIMP_UNDO_GROUP_LAYER_ADD, _("Drop layers"));
    ...
    for (iter = src_viewables; iter; iter = iter->next) {
      ...
      dest_index = gimp_item_tree_view_get_drop_index(
          item_view, dest_viewable, drop_pos, (GimpViewable **)&parent);
      ...
      item_view_class->add_item(item_view->priv->image, new_item, parent,
                                dest_index, TRUE);
    }
  } else if (dest_viewable) {
    gimp_image_undo_group_start(
        item_view->priv->image, GIMP_UNDO_GROUP_IMAGE_ITEM_REORDER,
    ...
    for (iter = src_viewables; iter; iter = iter->next) {
      ...
      dest_index = gimp_item_tree_view_get_drop_index(
          item_view, dest_viewable, drop_pos, (GimpViewable **)&dest_parent);
      ...
      gimp_image_reorder_item(item_view->priv->image, GIMP_ITEM(src_viewable),
...
static void gimp_item_tree_view_new_dropped(GtkWidget *widget, gint x, gint y,
  ...
  if (item_view_class->new_default_action && viewable &&
      gimp_container_view_lookup(view, viewable)) {
    GimpAction *action;

    action = gimp_ui_manager_find_action(
        gimp_editor_get_ui_manager(GIMP_EDITOR(view)),
        item_view_class->action_group, item_view_class->new_default_action);
...
static void gimp_item_tree_view_item_changed(GimpImage *image,
  ...
  items = GIMP_ITEM_TREE_VIEW_GET_CLASS(view)->get_selected_items(
      view->priv->image);
  ...
  if (gtk_tree_model_get_iter(tree_view->model, &iter, path)) {
    ...
    if (strcmp(old_name, new_name) &&
      ...
      gtk_tree_store_set(GTK_TREE_STORE(tree_view->model), &iter,
                         GIMP_CONTAINER_TREE_STORE_COLUMN_NAME, name, -1);
      g_free(name);
...
static void gimp_item_tree_view_visible_changed(GimpItem *item,
  ...
  iter = gimp_container_view_lookup(container_view, (GimpViewable *)item);
  ...
  if (iter) {
    ...
    gtk_tree_store_set(
        GTK_TREE_STORE(tree_view->model), iter,
        view->priv->model_column_visible, gimp_item_get_visible(item),
        view->priv->model_column_viewable,
        gimp_item_get_visible(item) && !gimp_item_is_visible(item), -1);

    children = gimp_viewable_get_children(GIMP_VIEWABLE(item));

    if (children)
      gimp_container_foreach(children,
                             (GFunc)gimp_item_tree_view_visible_changed, view);
  }
}
  ...
  if (iter) {
    ...
    if (children)
      gimp_container_foreach(
          children, (GFunc)gimp_item_tree_view_color_tag_changed, view);
  }
}
...
static void gimp_item_tree_view_lock_content_toggled(GtkWidget *widget,
  ...
  for (iter = locked_items; iter; iter = iter->next) {
    g_signal_handlers_block_by_func(
        iter->data, gimp_item_tree_view_lock_content_changed, view);
...
static void gimp_item_tree_view_lock_position_toggled(GtkWidget *widget,
  ...
  for (iter = items; iter; iter = iter->next) {
    if (gimp_item_get_lock_position(iter->data) != lock_position) {
      g_signal_handlers_block_by_func(
          iter->data, gimp_item_tree_view_lock_position_changed, view);
...
static gboolean gimp_item_tree_view_item_pre_clicked(
  ...
  if ((modifiers & GDK_MOD1_MASK) &&
    ...
    if (renderer) {
      ...
      gimp_item_to_selection(item, op, TRUE, FALSE, 0.0, 0.0);
      gimp_image_flush(image);
...
static void gimp_item_tree_view_update_options(GimpItemTreeView *view,
  ...
  inconsistent_lock_content =
      (!all_have_lock_content && inconsistent_lock_content);
  inconsistent_lock_position =
      (!all_have_lock_position && inconsistent_lock_position);

  if (all_have_lock_content != gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
                                   view->priv->lock_content_toggle))) {
    g_signal_handlers_block_by_func(view->priv->lock_content_toggle,
                                    gimp_item_tree_view_lock_content_toggled,
                                    view);

    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(view->priv->lock_content_toggle),
  ...
  if (all_have_lock_position != gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
                                    view->priv->lock_position_toggle))) {
    g_signal_handlers_block_by_func(view->priv->lock_position_toggle,
                                    gimp_item_tree_view_lock_position_toggled,
                                    view);

    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(view->priv->lock_position_toggle),
...
static void gimp_item_tree_view_toggle_clicked(GtkCellRendererToggle *toggle,
  ...
  void (*setter)(GimpItem * item, gboolean value, gboolean push_undo);
  void (*exclusive)(GimpItem * item, GimpContext * context);
  ...
  if (gtk_tree_model_get_iter(tree_view->model, &iter, path)) {
    ...
    if ((state & GDK_SHIFT_MASK) && exclusive) {
      ...
      undo =
          gimp_image_undo_can_compress(image, GIMP_TYPE_ITEM_UNDO, undo_type);
...
static void gimp_item_tree_view_row_expanded(GtkTreeView *tree_view,
  ...
  if (!item_view->priv->inserting_item) {
     ...
     */
    if (active_item && gimp_item_get_parent(active_item) == expanded_item) {
