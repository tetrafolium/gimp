...
enum { PROP_0, PROP_OPTIONS, PROP_EDIT_CONTEXT };

static void gimp_fill_editor_constructed(GObject *object);
static void gimp_fill_editor_finalize(GObject *object);
static void gimp_fill_editor_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_fill_editor_get_property(GObject *object, guint property_id,
...
static void gimp_fill_editor_class_init(GimpFillEditorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("options", NULL, NULL, GIMP_TYPE_FILL_OPTIONS,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_EDIT_CONTEXT,
      g_param_spec_boolean("edit-context", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_fill_editor_init(GimpFillEditor *editor) {
...
static void gimp_fill_editor_constructed(GObject *object) {
  ...
  if (editor->edit_context) {
    ...
    color_button = gimp_prop_color_button_new(G_OBJECT(editor->options),
                                              "foreground", _("Fill Color"), -1,
                                              24, GIMP_COLOR_AREA_SMALL_CHECKS);
    gimp_color_panel_set_context(GIMP_COLOR_PANEL(color_button),
                                 GIMP_CONTEXT(editor->options));
    gimp_enum_radio_box_add(GTK_BOX(box), color_button, GIMP_FILL_STYLE_SOLID,
                            FALSE);

    pattern_box =
        gimp_prop_pattern_box_new(NULL, GIMP_CONTEXT(editor->options), NULL, 2,
                                  "pattern-view-type", "pattern-view-size");
    gimp_enum_radio_box_add(GTK_BOX(box), pattern_box, GIMP_FILL_STYLE_PATTERN,
                            FALSE);
  }
...
GtkWidget *gimp_fill_editor_new(GimpFillOptions *options,
  ...
  return g_object_new(GIMP_TYPE_FILL_EDITOR, "options", options, "edit-context",
                      edit_context ? TRUE : FALSE, NULL);
}
