...
enum { PROP_0, PROP_GIMP, PROP_CURRENT_DEVICE };
...
static void gimp_device_manager_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_device_manager_get_property(GObject *object, guint property_id,
...
static void gimp_device_manager_device_added(GdkSeat *seat, GdkDevice *device,
                                             GimpDeviceManager *manager);
static void gimp_device_manager_device_removed(GdkSeat *seat, GdkDevice *device,
                                               GimpDeviceManager *manager);
...
static void gimp_device_manager_class_init(GimpDeviceManagerClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("current-device", NULL, NULL, GIMP_TYPE_DEVICE_INFO,
                          GIMP_PARAM_STATIC_STRINGS | G_PARAM_READABLE));
}

static void gimp_device_manager_init(GimpDeviceManager *manager) {
  manager->priv = gimp_device_manager_get_instance_private(manager);

  manager->priv->displays =
...
static void gimp_device_manager_constructed(GObject *object) {
  ...
  g_signal_connect(disp_manager, "display-opened",
                   G_CALLBACK(gimp_device_manager_display_opened), manager);
  ...
  g_signal_connect_object(user_context, "tool-changed",
                          G_CALLBACK(gimp_device_manager_tool_changed), manager,
                          0);
}
...
static void gimp_device_manager_dispose(GObject *object) {
  ...
  g_signal_handlers_disconnect_by_func(
      gdk_display_manager_get(), gimp_device_manager_display_opened, object);
...
GimpDeviceManager *gimp_device_manager_new(Gimp *gimp) {
  ...
  return g_object_new(GIMP_TYPE_DEVICE_MANAGER, "gimp", gimp, "children-type",
                      GIMP_TYPE_DEVICE_INFO, "policy",
                      GIMP_CONTAINER_POLICY_STRONG, "unique-names", TRUE,
                      "sort-func", gimp_device_info_compare, NULL);
}
...
private
  ...
  if (!config->devices_share_tool && private->current_device) {
    ...
    g_signal_handlers_block_by_func(user_context,
                                    gimp_device_manager_tool_changed, manager);

    gimp_device_info_restore_tool(private->current_device);

    g_signal_handlers_unblock_by_func(
        user_context, gimp_device_manager_tool_changed, manager);

  private
    ->active_tool = gimp_context_get_tool(user_context);
...
static void gimp_device_manager_display_opened(GdkDisplayManager *disp_manager,
  ...
  count = GPOINTER_TO_INT(g_hash_table_lookup(private->displays, display_name));
  ...
  g_signal_connect(seat, "device-added",
                   G_CALLBACK(gimp_device_manager_device_added), manager);
  g_signal_connect(seat, "device-removed",
                   G_CALLBACK(gimp_device_manager_device_removed), manager);

  g_signal_connect(display, "closed",
                   G_CALLBACK(gimp_device_manager_display_closed), manager);
}
...
static void gimp_device_manager_display_closed(GdkDisplay *display,
  ...
  g_signal_handlers_disconnect_by_func(seat, gimp_device_manager_device_added,
                                       manager);
  g_signal_handlers_disconnect_by_func(seat, gimp_device_manager_device_removed,
                                       manager);

  g_signal_handlers_disconnect_by_func(
      display, gimp_device_manager_display_closed, manager);
...
static void gimp_device_manager_device_added(GdkSeat *seat, GdkDevice *device,
  ...
  device_info = GIMP_DEVICE_INFO(gimp_container_get_child_by_name(
      GIMP_CONTAINER(manager), gdk_device_get_name(device)));
...
static void gimp_device_manager_connect_tool(GimpDeviceManager *manager) {
  ...
  if (!config->devices_share_tool && private->active_tool &&
    ...
    gimp_config_connect(G_OBJECT(private->active_tool->tool_options),
                        G_OBJECT(preset->tool_options), NULL);
  }
}
...
static void gimp_device_manager_device_defaults(GdkSeat *seat,
  ...
  } else /* slave or floating device */
  {
    ...
    if (gdk_device_set_mode(device, mode)) {
      g_printerr("set device '%s' to mode: %s\n", gdk_device_get_name(device),
                 g_enum_get_value(g_type_class_peek(GDK_TYPE_INPUT_MODE), mode)
                     ->value_nick);
    } else {
  ...
  info = GIMP_DEVICE_INFO(gimp_container_get_child_by_name(
      GIMP_CONTAINER(manager), gdk_device_get_name(device)));
  if (info) {
