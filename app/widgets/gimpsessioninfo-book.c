  ...
  while (g_scanner_peek_next_token(scanner) == token) {
    ...
    case G_TOKEN_SYMBOL:
      ...
      case SESSION_INFO_BOOK_DOCKABLE:
        ...
        token = gimp_session_info_dockable_deserialize(scanner, scope + 1,
                                                       &dockable);
...
gimp_session_info_book_from_widget(GimpDockbook *dockbook) {
  ...
  info->current_page = gtk_notebook_get_current_page(GTK_NOTEBOOK(dockbook));
...
GimpDockbook *gimp_session_info_book_restore(GimpSessionInfoBook *info,
  ...
  for (pages = info->dockables; pages; pages = g_list_next(pages)) {
    ...
    if (dockable) {
      gtk_notebook_append_page(GTK_NOTEBOOK(dockbook), GTK_WIDGET(dockable),
                               NULL);
      n_dockables++;
    }
  }

  if (info->current_page < gtk_notebook_get_n_pages(GTK_NOTEBOOK(dockbook))) {
    gtk_notebook_set_current_page(GTK_NOTEBOOK(dockbook), info->current_page);
