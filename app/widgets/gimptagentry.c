...
enum { PROP_0, PROP_CONTAINER, PROP_MODE };

static void gimp_tag_entry_dispose(GObject *object);
static void gimp_tag_entry_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_tag_entry_get_property(GObject *object, guint property_id,
                                        GValue *value, GParamSpec *pspec);
static void gimp_tag_entry_activate(GtkEntry *entry);
static void gimp_tag_entry_changed(GtkEntry *entry);
static void gimp_tag_entry_insert_text(GtkEditable *editable, gchar *new_text,
                                       gint text_length, gint *position);
static void gimp_tag_entry_delete_text(GtkEditable *editable, gint start_pos,
                                       gint end_pos);
...
static gboolean gimp_tag_entry_key_press(GtkWidget *widget, GdkEventKey *event);
static gboolean gimp_tag_entry_query_tag(GimpTagEntry *entry);

static void gimp_tag_entry_assign_tags(GimpTagEntry *entry);
static void gimp_tag_entry_load_selection(GimpTagEntry *entry, gboolean sort);
static void gimp_tag_entry_find_common_tags(gpointer key, gpointer value,
                                            gpointer user_data);
...
static void gimp_tag_entry_toggle_desc(GimpTagEntry *widget, gboolean show);
static gboolean gimp_tag_entry_draw(GtkWidget *widget, cairo_t *cr);
static void gimp_tag_entry_commit_region(GString *tags, GString *mask);
...
static void gimp_tag_entry_next_tag(GimpTagEntry *entry, gboolean select);
static void gimp_tag_entry_previous_tag(GimpTagEntry *entry, gboolean select);
...
static void gimp_tag_entry_class_init(GimpTagEntryClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_CONTAINER,
      g_param_spec_object("container", "Tagged container",
                          "The Tagged container", GIMP_TYPE_TAGGED_CONTAINER,
                          G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_MODE,
      g_param_spec_enum("mode", "Working mode", "Mode in which to work.",
                        GIMP_TYPE_TAG_ENTRY_MODE, GIMP_TAG_ENTRY_MODE_QUERY,
                        G_PARAM_CONSTRUCT_ONLY | G_PARAM_READWRITE));
}

static void gimp_tag_entry_init(GimpTagEntry *entry) {
  ...
  g_signal_connect(entry, "activate", G_CALLBACK(gimp_tag_entry_activate),
                   NULL);
  g_signal_connect(entry, "changed", G_CALLBACK(gimp_tag_entry_changed), NULL);
  g_signal_connect(entry, "insert-text", G_CALLBACK(gimp_tag_entry_insert_text),
                   NULL);
  g_signal_connect(entry, "delete-text", G_CALLBACK(gimp_tag_entry_delete_text),
                   NULL);
  g_signal_connect(entry, "key-press-event",
                   G_CALLBACK(gimp_tag_entry_key_press), NULL);
  g_signal_connect(entry, "focus-in-event", G_CALLBACK(gimp_tag_entry_focus_in),
                   NULL);
  g_signal_connect(entry, "focus-out-event",
                   G_CALLBACK(gimp_tag_entry_focus_out), NULL);
  g_signal_connect_after(entry, "draw", G_CALLBACK(gimp_tag_entry_draw), NULL);
}
...
static void gimp_tag_entry_dispose(GObject *object) {
  ...
  if (entry->container) {
    g_signal_handlers_disconnect_by_func(
        entry->container, gimp_tag_entry_container_changed, entry);
...
GtkWidget *gimp_tag_entry_new(GimpTaggedContainer *container,
  ...
  return g_object_new(GIMP_TYPE_TAG_ENTRY, "container", container, "mode", mode,
                      NULL);
}
...
static void gimp_tag_entry_activate(GtkEntry *entry) {
  ...
  gtk_editable_get_selection_bounds(GTK_EDITABLE(entry), &selection_start,
                                    &selection_end);
  if (selection_start != selection_end) {
    gtk_editable_select_region(GTK_EDITABLE(entry), selection_end,
                               selection_end);
  }
...
static void gimp_tag_entry_changed(GtkEntry *entry) {
  ...
  if (!gtk_widget_has_focus(GTK_WIDGET(entry)) && strlen(text) == 0) {
  ...
  if (tag_entry->mode == GIMP_TAG_ENTRY_MODE_QUERY &&
      !tag_entry->suppress_tag_query && !tag_entry->tag_query_idle_id) {
...
static void gimp_tag_entry_insert_text(GtkEditable *editable, gchar *new_text,
  ...
  if (is_tag[0] && is_tag[1]) {
    ...
    if (!entry->internal_operation && *position > 0 &&
        entry->mask->str[*position - 1] == 's' && !g_unichar_isspace(c)) {
      if (!entry->suppress_mask_update) {
        g_string_insert_c(entry->mask, *position, 'u');
      }

      g_signal_handlers_block_by_func(editable, gimp_tag_entry_insert_text,
                                      NULL);

      gtk_editable_insert_text(editable, " ", 1, position);
      gtk_editable_insert_text(editable, new_text, text_length, position);

      g_signal_handlers_unblock_by_func(editable, gimp_tag_entry_insert_text,
                                        NULL);

      g_signal_stop_emission_by_name(editable, "insert-text");
    } else if (!entry->internal_operation && num_chars == 1 &&
               *position < entry->mask->len &&
               entry->mask->str[*position] == 't' && !g_unichar_isspace(c)) {
      if (!entry->suppress_mask_update) {
        g_string_insert_c(entry->mask, *position, 'u');
      }
...
static void gimp_tag_entry_delete_text(GtkEditable *editable, gint start_pos,
  ...
  if (!entry->internal_operation) {
    g_signal_handlers_block_by_func(editable, gimp_tag_entry_delete_text, NULL);

    if (end_pos > start_pos && (entry->mask->str[end_pos - 1] == 't' ||
    ...
    g_signal_handlers_unblock_by_func(editable, gimp_tag_entry_delete_text,
                                      NULL);
...
static gboolean gimp_tag_entry_auto_complete(GimpTagEntry *tag_entry) {
  ...
  gtk_editable_get_selection_bounds(GTK_EDITABLE(tag_entry), &start_position,
                                    &end_position);
  ...
  completion_candidates = gimp_tag_entry_get_completion_candidates(
      GIMP_TAG_ENTRY(entry), tags, completion_prefix);
  completion_candidates =
      g_list_sort(completion_candidates, gimp_tag_compare_func);
  ...
  if (tag_entry->tab_completion_index >= 0 && completion_candidates) {
    ...
    the_chosen_one =
        g_list_nth_data(completion_candidates, tag_entry->tab_completion_index);
    g_list_free(completion_candidates);
    completion_candidates = NULL;
    completion_candidates =
        g_list_append(completion_candidates, the_chosen_one);
  }

  completion = gimp_tag_entry_get_completion_string(
      GIMP_TAG_ENTRY(entry), completion_candidates, completion_prefix);

  if (completion && strlen(completion) > 0) {
    start_position = gtk_editable_get_position(GTK_EDITABLE(entry));
    end_position = start_position;
    tag_entry->internal_operation++;
    gtk_editable_insert_text(GTK_EDITABLE(entry), completion,
                             strlen(completion), &end_position);
    tag_entry->internal_operation--;
    if (tag_entry->tab_completion_index >= 0 && candidate_count == 1) {
      gtk_editable_set_position(GTK_EDITABLE(entry), end_position);
    } else {
      gtk_editable_select_region(GTK_EDITABLE(entry), start_position,
                                 end_position);
    }
  }
...
static void gimp_tag_entry_assign_tags(GimpTagEntry *tag_entry) {
  ...
  for (tag_iter = tag_entry->common_tags; tag_iter;
       tag_iter = g_list_next(tag_iter)) {
    if (!g_list_find_custom(dont_remove_list, tag_iter->data,
                            gimp_tag_compare_func)) {
      remove_list = g_list_prepend(remove_list, g_object_ref(tag_iter->data));
    }
  }
  ...
  for (resource_iter = selected_items; resource_iter;
 ...
 **/
void gimp_tag_entry_set_selected_items(GimpTagEntry *tag_entry, GList *items) {
...
static void gimp_tag_entry_load_selection(GimpTagEntry *tag_entry,
  ...
  tag_entry->common_tags =
      g_list_sort(tag_entry->common_tags, gimp_tag_compare_func);
  ...
  for (list = tag_entry->common_tags; list; list = g_list_next(list)) {
    ...
    text = g_strdup_printf("%s%s ", gimp_tag_get_name(tag),
...
static void gimp_tag_entry_find_common_tags(gpointer key, gpointer value,
  ...
  if (ref_count == g_list_length(tag_entry->selected_items)) {
    tag_entry->common_tags =
        g_list_prepend(tag_entry->common_tags, g_object_ref(key));
  }
}
...
static gchar *gimp_tag_entry_get_completion_prefix(GimpTagEntry *entry) {
  ...
  if (position < 1 || entry->mask->str[position - 1] != 'u') {
    return g_strdup("");
  }
  ...
  do {
    ...
    for (i = 1; i < length; i++) {
      ...
      if (c != d) {
        ...
        completion_end =
            g_utf8_offset_to_pointer(candidate_string, num_chars_match);
...
static gboolean gimp_tag_entry_focus_out(GtkWidget *widget,
  ...
  gimp_tag_entry_add_to_recent(tag_entry, gtk_entry_get_text(GTK_ENTRY(widget)),
                               TRUE);
...
static void gimp_tag_entry_container_changed(GimpContainer *container,
  ...
  if (!gimp_container_have(GIMP_CONTAINER(tag_entry->container), object)) {
...
static void gimp_tag_entry_toggle_desc(GimpTagEntry *tag_entry, gboolean show) {
...
static gboolean gimp_tag_entry_key_press(GtkWidget *widget,
  ...
  extend_mask = gtk_widget_get_modifier_mask(
      widget, GDK_MODIFIER_INTENT_EXTEND_SELECTION);
  ...
  case GDK_KEY_Right:
    gimp_tag_entry_next_tag(entry, (event->state & extend_mask) ? TRUE : FALSE);
    return TRUE;
  ...
  case GDK_KEY_BackSpace: {
    ...
    gtk_editable_get_selection_bounds(GTK_EDITABLE(entry), &selection_start,
                                      &selection_end);
    if (gimp_tag_entry_select_jellybean(entry, selection_start, selection_end,
                                        TAG_SEARCH_LEFT)) {
      return TRUE;
    } else {
      gimp_tag_entry_select_for_deletion(entry, TAG_SEARCH_LEFT);
      /* FIXME: need to remove idle handler in dispose */
      g_idle_add((GSourceFunc)gimp_tag_entry_strip_extra_whitespace, entry);
    }
  } break;
...
static gboolean gimp_tag_entry_button_release(GtkWidget *widget,
  ...
  if (event->button == 1) {
    /* FIXME: need to remove idle handler in dispose */
    g_idle_add((GSourceFunc)gimp_tag_entry_try_select_jellybean, widget);
  }
...
static gboolean gimp_tag_entry_select_jellybean(GimpTagEntry *entry,
  ...
  case TAG_SEARCH_NONE:
    if (selection_start > 0 && entry->mask->str[selection_start] == 's') {
      selection_start--;
    }

    if (selection_start > 0 && (entry->mask->str[selection_start - 1] == 'w') &&
  ...
  case TAG_SEARCH_LEFT:
    if (selection_start == selection_end) {
      if (selection_start > 0 && entry->mask->str[selection_start] == 't' &&
  ...
  if (selection_start < entry->mask->len && selection_start == selection_end) {
    selection_end = selection_start + 1;
  }

  gtk_editable_get_selection_bounds(GTK_EDITABLE(entry), &prev_selection_start,
                                    &prev_selection_end);
  ...
  if (search_dir == TAG_SEARCH_NONE && selection_end - selection_start == 1 &&
  ...
  if ((selection_start != prev_selection_start ||
    ...
    if (search_dir == TAG_SEARCH_LEFT) {
      ...
      gtk_editable_select_region(GTK_EDITABLE(entry), selection_start,
                                 selection_end);
    }
...
static void gimp_tag_entry_commit_tags(GimpTagEntry *entry) {
  ...
  do {
    ...
    if (found_region) {
      ...
      tags_string =
          gtk_editable_get_chars(GTK_EDITABLE(entry), region_start, region_end);
      tags = g_string_new(tags_string);
      g_free(tags_string);
      length_before = region_end - region_start;

      mask = g_string_new_len(entry->mask->str + region_start,
                              region_end - region_start);

      gimp_tag_entry_commit_region(tags, mask);

      /* prepend space before if needed */
      if (region_start > 0 && entry->mask->str[region_start - 1] != 'w' &&
          mask->len > 0 && mask->str[0] != 'w') {
      ...
      if (region_end <= entry->mask->len &&
          entry->mask->str[region_end] != 'w' && mask->len > 0 &&
      ...
      gtk_editable_insert_text(GTK_EDITABLE(entry), tags->str, tags->len,
                               &position);
      entry->suppress_tag_query--;
      entry->suppress_mask_update--;
      entry->internal_operation--;
...
static void gimp_tag_entry_next_tag(GimpTagEntry *entry, gboolean select) {
  ...
  if (entry->mask->str[position] != 'u') {
    while (position < entry->mask->len && (entry->mask->str[position] != 'w')) {
      position++;
    }
...
static void gimp_tag_entry_previous_tag(GimpTagEntry *entry, gboolean select) {
  ...
  if (position >= 1 && entry->mask->str[position - 1] == 'w') {
    position--;
  }
  if (position < 1) {
    return;
  ...
  if (entry->mask->str[position - 1] != 'u') {
    while (position > 0 && (entry->mask->str[position - 1] != 'w')) {
      if (entry->mask->str[position - 1] == 'u') {
        break;
      }
...
static void gimp_tag_entry_select_for_deletion(GimpTagEntry *entry,
  ...
  while (start_pos > 0 && (entry->mask->str[start_pos - 1] == 't')) {
    start_pos--;
  }

  if (end_pos > start_pos && (entry->mask->str[end_pos - 1] == 't' ||
                              entry->mask->str[end_pos - 1] == 's')) {
    while (end_pos <= entry->mask->len && (entry->mask->str[end_pos] == 's')) {
      end_pos++;
    }
  }

  /* ensure there is no unnecessary whitespace selected */
  while (start_pos < end_pos && entry->mask->str[start_pos] == 'w') {
    start_pos++;
  }

  while (start_pos < end_pos && entry->mask->str[end_pos - 1] == 'w') {
    end_pos--;
  }
  ...
  } else if (end_pos > start_pos && search_dir == TAG_SEARCH_RIGHT &&
...
static gboolean gimp_tag_entry_strip_extra_whitespace(GimpTagEntry *entry) {
  ...
  while (entry->mask->len > 0 && entry->mask->str[0] == 'w') {
  ...
  while (entry->mask->len > 1 &&
    ...
    gtk_editable_delete_text(GTK_EDITABLE(entry), entry->mask->len - 1,
                             entry->mask->len);
  ...
  for (i = entry->mask->len - 1; i > 0; i--) {
    if (entry->mask->str[i] == 'w' && entry->mask->str[i - 1] == 'w') {
   ...
   */
  if (position == entry->mask->len - 1 && entry->mask->str[position] == 'w') {
    position++;
  }
