...
static void palette_editor_drop_palette(GtkWidget *widget, gint x, gint y,
                                        GimpViewable *viewable, gpointer data);
static void palette_editor_drop_color(GtkWidget *widget, gint x, gint y,
...
static void palette_editor_resize(GimpPaletteEditor *editor, gint width,
                                  gdouble zoom_factor);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpPaletteEditor, gimp_palette_editor, GIMP_TYPE_DATA_EDITOR,
...
static void gimp_palette_editor_docked_iface_init(GimpDockedInterface *iface) {
...
static void gimp_palette_editor_init(GimpPaletteEditor *editor) {
  ...
  editor->view = gimp_view_new_full_by_types(
      NULL, GIMP_TYPE_PALETTE_VIEW, GIMP_TYPE_PALETTE, PREVIEW_WIDTH,
      PREVIEW_HEIGHT, 0, FALSE, TRUE, FALSE);
  ...
  g_signal_connect(gtk_widget_get_parent(editor->view), "size-allocate",
                   G_CALLBACK(palette_editor_viewport_size_allocate), editor);

  g_signal_connect(editor->view, "entry-clicked",
                   G_CALLBACK(palette_editor_entry_clicked), editor);
  g_signal_connect(editor->view, "entry-selected",
                   G_CALLBACK(palette_editor_entry_selected), editor);
  g_signal_connect(editor->view, "entry-activated",
                   G_CALLBACK(palette_editor_entry_activated), editor);
  g_signal_connect(editor->view, "entry-context",
                   G_CALLBACK(palette_editor_entry_context), editor);
  g_signal_connect(editor->view, "color-dropped",
                   G_CALLBACK(palette_editor_color_dropped), editor);

  gimp_dnd_viewable_dest_add(editor->view, GIMP_TYPE_PALETTE,
                             palette_editor_drop_palette, editor);
  gimp_dnd_viewable_dest_add(gtk_widget_get_parent(editor->view),
                             GIMP_TYPE_PALETTE, palette_editor_drop_palette,
                             editor);

  gimp_dnd_color_dest_add(gtk_widget_get_parent(editor->view),
                          palette_editor_drop_color, editor);
  ...
  gimp_label_set_attributes(GTK_LABEL(editor->index_label), PANGO_ATTR_FAMILY,
                            "Monospace", -1);
  ...
  g_signal_connect(editor->color_name, "changed",
                   G_CALLBACK(palette_editor_color_name_changed), editor);
  ...
  g_signal_connect(editor->columns_adj, "value-changed",
                   G_CALLBACK(palette_editor_columns_changed), editor);
}
...
static void gimp_palette_editor_constructed(GObject *object) {
  ...
  gimp_editor_add_action_button(
      GIMP_EDITOR(editor), "palette-editor", "palette-editor-new-color-fg",
      "palette-editor-new-color-bg", gimp_get_toggle_behavior_mask(), NULL);
...
static void gimp_palette_editor_set_data(GimpDataEditor *editor,
  ...
  g_signal_handlers_block_by_func(palette_editor->columns_adj,
                                  palette_editor_columns_changed, editor);
  ...
  if (editor->data) {
    ...
    g_signal_handlers_disconnect_by_func(
        editor->data, palette_editor_invalidate_preview, editor);
  ...
  gimp_view_set_viewable(GIMP_VIEW(palette_editor->view), GIMP_VIEWABLE(data));

  if (editor->data) {
    GimpPalette *palette = GIMP_PALETTE(editor->data);

    g_signal_connect(editor->data, "invalidate-preview",
                     G_CALLBACK(palette_editor_invalidate_preview), editor);
...
GtkWidget *gimp_palette_editor_new(GimpContext *context,
  ...
  return g_object_new(GIMP_TYPE_PALETTE_EDITOR, "menu-factory", menu_factory,
                      "menu-identifier", "<PaletteEditor>", "ui-path",
                      "/palette-editor-popup", "data-factory",
                      context->gimp->palette_factory, "context", context,
                      "data", gimp_context_get_palette(context), NULL);
}
...
void gimp_palette_editor_edit_color(GimpPaletteEditor *editor) {
  ...
  if (!editor->color_dialog) {
    editor->color_dialog = gimp_color_dialog_new(
        GIMP_VIEWABLE(palette), data_editor->context, FALSE,
        _("Edit Palette Color"), GIMP_ICON_PALETTE,
        _("Edit Color Palette Entry"), GTK_WIDGET(editor),
        gimp_dialog_factory_get_singleton(), "gimp-palette-editor-color-dialog",
        &editor->color->color, FALSE, FALSE);

    g_signal_connect(editor->color_dialog, "destroy",
                     G_CALLBACK(gtk_widget_destroyed), &editor->color_dialog);

    g_signal_connect(editor->color_dialog, "update",
                     G_CALLBACK(palette_editor_edit_color_update), editor);
  } else {
    gimp_viewable_dialog_set_viewables(
        GIMP_VIEWABLE_DIALOG(editor->color_dialog),
        g_list_prepend(NULL, palette), data_editor->context);
    ...
    if (!gtk_widget_get_visible(editor->color_dialog))
      gimp_dialog_factory_position_dialog(
          ...
          "gimp-palette-editor-color-dialog", editor->color_dialog,
...
void gimp_palette_editor_pick_color(GimpPaletteEditor *editor,
  ...
  if (GIMP_DATA_EDITOR(editor)->data_editable) {
    ...
    case GIMP_COLOR_PICK_STATE_START:
      ...
      entry = gimp_palette_add_entry(GIMP_PALETTE(data), index, NULL, color);
    ...
    case GIMP_COLOR_PICK_STATE_END:
      gimp_palette_set_entry_color(GIMP_PALETTE(data), editor->color->position,
                                   color);
      break;
    }
  }
...
void gimp_palette_editor_zoom(GimpPaletteEditor *editor,
  ...
  case GIMP_ZOOM_TO: /* abused as ZOOM_ALL */
  {
    ...
    zoom_factor =
        (((gdouble)allocation.height - 2 * SPACING) / (gdouble)rows - SPACING) /
        ENTRY_HEIGHT;
  } break;
...
gboolean gimp_palette_editor_set_index(GimpPaletteEditor *editor, gint index,
                                       GimpRGB *color) {
  GimpPalette *palette;
...
static void palette_editor_viewport_size_allocate(GtkWidget *widget,
  ...
  if (allocation->width != editor->last_width) {
    palette_editor_resize(editor, allocation->width, editor->zoom_factor);
  }
}
...
static void palette_editor_entry_selected(GimpPaletteView *view,
  ...
  if (editor->color != entry) {
    ...
    g_signal_handlers_block_by_func(editor->color_name,
                                    palette_editor_color_name_changed, editor);

    gtk_entry_set_text(GTK_ENTRY(editor->color_name),
                       entry ? entry->name : _("Undefined"));

    g_signal_handlers_unblock_by_func(
        editor->color_name, palette_editor_color_name_changed, editor);
...
static void palette_editor_entry_activated(GimpPaletteView *view,
  ...
  if (GIMP_DATA_EDITOR(editor)->data_editable && entry == editor->color) {
    gimp_ui_manager_activate_action(
        gimp_editor_get_ui_manager(GIMP_EDITOR(editor)), "palette-editor",
...
static void palette_editor_columns_changed(GtkAdjustment *adj,
  ...
  if (GIMP_DATA_EDITOR(editor)->data) {
    ...
    gimp_palette_set_columns(palette, ROUND(gtk_adjustment_get_value(adj)));
  }
}
...
static void palette_editor_resize(GimpPaletteEditor *editor, gint width,
  ...
  preview_height =
      (rows * (SPACING + (gint)(ENTRY_HEIGHT * editor->zoom_factor)));
...
static void palette_editor_scroll_top_left(GimpPaletteEditor *palette_editor) {
  ...
  hadj = gtk_scrolled_window_get_hadjustment(
      GTK_SCROLLED_WINDOW(data_editor->view));
