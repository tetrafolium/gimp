...
static void gimp_tool_preset_editor_init(GimpToolPresetEditor *editor) {
...
static void gimp_tool_preset_editor_constructed(GObject *object) {
  ...
  preset = editor->priv->tool_preset_model = g_object_new(
      GIMP_TYPE_TOOL_PRESET, "gimp", data_editor->context->gimp, NULL);

  g_signal_connect(preset, "notify",
                   G_CALLBACK(gimp_tool_preset_editor_notify_model), editor);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), editor->priv->tool_icon, FALSE, FALSE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), editor->priv->tool_label, FALSE, FALSE, 0);
  ...
  button = editor->priv->opacity_paint_mode_toggle = gimp_prop_check_button_new(
      G_OBJECT(preset), "use-opacity-paint-mode", NULL);
  gtk_box_pack_start(GTK_BOX(data_editor), button, FALSE, FALSE, 0);
  ...
  button =
      gimp_editor_add_action_button(GIMP_EDITOR(editor), "tool-preset-editor",
...
static void gimp_tool_preset_editor_set_data(GimpDataEditor *editor,
  ...
  if (editor->data)
    g_signal_handlers_disconnect_by_func(
        editor->data, gimp_tool_preset_editor_notify_data, editor);

  GIMP_DATA_EDITOR_CLASS(parent_class)->set_data(editor, data);

  if (editor->data) {
    g_signal_connect(editor->data, "notify",
                     G_CALLBACK(gimp_tool_preset_editor_notify_data), editor);
...
GtkWidget *gimp_tool_preset_editor_new(GimpContext *context,
  ...
  return g_object_new(GIMP_TYPE_TOOL_PRESET_EDITOR, "menu-factory",
                      menu_factory, "menu-identifier", "<ToolPresetEditor>",
                      "ui-path", "/tool-preset-editor-popup", "data-factory",
                      context->gimp->tool_preset_factory, "context", context,
                      "data", gimp_context_get_tool_preset(context), NULL);
}
...
static void gimp_tool_preset_editor_sync_data(GimpToolPresetEditor *editor) {
  ...
  g_signal_handlers_block_by_func(priv->tool_preset_model,
                                  gimp_tool_preset_editor_notify_model, editor);
  ...
  g_signal_handlers_unblock_by_func(
      priv->tool_preset_model, gimp_tool_preset_editor_notify_model, editor);
  ...
  gtk_image_set_from_icon_name(GTK_IMAGE(priv->tool_icon), icon_name,
                               GTK_ICON_SIZE_MENU);
  ...
  gtk_widget_set_sensitive(
      priv->opacity_paint_mode_toggle,
      (serialize_props & (GIMP_CONTEXT_PROP_MASK_OPACITY |
  ...
  gtk_widget_set_sensitive(priv->palette_toggle,
                           (serialize_props & GIMP_CONTEXT_PROP_MASK_PALETTE) !=
                               0);
  gtk_widget_set_sensitive(
      priv->font_toggle, (serialize_props & GIMP_CONTEXT_PROP_MASK_FONT) != 0);
}
...
static void gimp_tool_preset_editor_notify_model(GimpToolPreset *options,
  ...
  if (data_editor->data) {
    g_signal_handlers_block_by_func(
        data_editor->data, gimp_tool_preset_editor_notify_data, editor);

    gimp_config_sync(G_OBJECT(editor->priv->tool_preset_model),
                     G_OBJECT(data_editor->data), GIMP_CONFIG_PARAM_SERIALIZE);
