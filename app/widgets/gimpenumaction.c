...
enum { PROP_0, PROP_VALUE, PROP_VALUE_VARIABLE };

static void gimp_enum_action_set_property(GObject *object, guint prop_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_enum_action_get_property(GObject *object, guint prop_id,
...
static void gimp_enum_action_class_init(GimpEnumActionClass *klass) {
  ...
  g_object_class_install_property(object_class, PROP_VALUE_VARIABLE,
                                  g_param_spec_boolean("value-variable", NULL,
                                                       NULL, FALSE,
                                                       GIMP_PARAM_READWRITE));
}
...
GimpEnumAction *gimp_enum_action_new(const gchar *name, const gchar *label,
  ...
  action = g_object_new(GIMP_TYPE_ENUM_ACTION, "name", name, "label", label,
                        "tooltip", tooltip, "icon-name", icon_name, "value",
                        value, "value-variable", value_variable, NULL);
