...
static gboolean gimp_help_browser(Gimp *gimp, GimpProgress *progress);
static void gimp_help_browser_error(Gimp *gimp, GimpProgress *progress,
                                    const gchar *title, const gchar *primary,
                                    const gchar *text);

static void gimp_help_call(Gimp *gimp, GimpProgress *progress,
                           const gchar *procedure_name,
                           const gchar *help_domain, const gchar *help_locales,
                           const gchar *help_id);

static gint gimp_help_get_help_domains(Gimp *gimp, gchar ***domain_names,
                                       gchar ***domain_uris);
...
gboolean gimp_help_user_manual_is_installed(Gimp *gimp) {
  ...
  if (g_file_query_file_type(basedir, G_FILE_QUERY_INFO_NONE, NULL) ==
    ...
    for (p = strchr(s, ':'); p && !found; p = strchr(s, ':')) {
      ...
      found = (g_file_query_file_type(file2, G_FILE_QUERY_INFO_NONE, NULL) ==
               G_FILE_TYPE_REGULAR);
      ...
      while ((info = g_file_enumerator_next_file(enumerator, NULL, NULL))) {
        if (g_file_info_get_file_type(info) == G_FILE_TYPE_DIRECTORY) {
          ...
          if (g_file_query_file_type(file, G_FILE_QUERY_INFO_NONE, NULL) ==
              G_FILE_TYPE_REGULAR) {
            manuals =
                g_list_prepend(manuals, g_strdup(g_file_info_get_name(info)));
          }
...
static gboolean gimp_idle_help(GimpIdleHelp *idle_help) {
  ...
  if (!idle_help->help_domain && !config->user_manual_online &&
  ...
  if (procedure_name)
    gimp_help_call(idle_help->gimp, idle_help->progress, procedure_name,
                   idle_help->help_domain, idle_help->help_locales,
                   idle_help->help_id);
...
static gboolean gimp_help_browser(Gimp *gimp, GimpProgress *progress) {
  ...
  procedure =
      gimp_pdb_lookup_procedure(gimp->pdb, "extension-gimp-help-browser-temp");
  ...
  if (!procedure) {
    ...
    procedure =
        gimp_pdb_lookup_procedure(gimp->pdb, "extension-gimp-help-browser");

    if (!procedure) {
      gimp_help_browser_error(gimp, progress, _("Help browser is missing"),
    ...
    g_value_set_int(gimp_value_array_index(args, 1), n_domains);
    gimp_value_take_string_array(gimp_value_array_index(args, 2), help_domains,
                                 n_domains);
    g_value_set_int(gimp_value_array_index(args, 3), n_domains);
    gimp_value_take_string_array(gimp_value_array_index(args, 4), help_uris,
                                 n_domains);
  ...
  if (!procedure) {
    gimp_help_browser_error(gimp, progress, _("Help browser doesn't start"),
...
static void gimp_help_browser_error(Gimp *gimp, GimpProgress *progress,
  ...
  dialog = gimp_message_dialog_new(
      title, GIMP_ICON_HELP_USER_MANUAL, NULL, 0, NULL, NULL,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("Use _Web Browser"), GTK_RESPONSE_OK,
  ...
  if (gimp_dialog_run(GIMP_DIALOG(dialog)) == GTK_RESPONSE_OK) {
    g_object_set(gimp->config, "help-browser", GIMP_HELP_BROWSER_WEB_BROWSER,
                 NULL);
  }
...
static void gimp_help_call(Gimp *gimp, GimpProgress *progress,
  ...
  if (!strcmp(procedure_name, "extension-gimp-help-browser-temp")) {
    ...
    GIMP_LOG(HELP, "Calling help via %s: %s %s %s", procedure_name,
             help_domain ? help_domain : "(null)",
             help_locales ? help_locales : "(null)",
             help_id ? help_id : "(null)");

    return_vals = gimp_pdb_execute_procedure_by_name(
        gimp->pdb, gimp_get_user_context(gimp), progress, &error,
        procedure_name, G_TYPE_STRING, help_domain, G_TYPE_STRING, help_locales,
        G_TYPE_STRING, help_id, G_TYPE_NONE);
  ...
  if (!procedure) {
    ...
    g_value_set_int(gimp_value_array_index(args, 0), n_domains);
    gimp_value_take_string_array(gimp_value_array_index(args, 1), help_domains,
                                 n_domains);
    g_value_set_int(gimp_value_array_index(args, 2), n_domains);
    gimp_value_take_string_array(gimp_value_array_index(args, 3), help_uris,
                                 n_domains);

    gimp_procedure_execute_async(procedure, gimp, gimp_get_user_context(gimp),
                                 progress, args, NULL, &error);
  ...
  if (procedure) {
    ...
    return_vals = gimp_pdb_execute_procedure_by_name(
        gimp->pdb, gimp_get_user_context(gimp), progress, &error,
        "extension-gimp-help-temp", G_TYPE_STRING, procedure_name,
        G_TYPE_STRING, help_domain, G_TYPE_STRING, help_locales, G_TYPE_STRING,
        help_id, G_TYPE_NONE);
...
static gint gimp_help_get_help_domains(Gimp *gimp, gchar ***domain_names,
  ...
  n_domains = gimp_plug_in_manager_get_help_domains(
      gimp->plug_in_manager, &plug_in_domains, &plug_in_uris);
...
static gchar *gimp_help_get_locales(Gimp *gimp) {
  ...
  for (i = 0; names[i]; i++) {
    ...
    if (locale && *locale &&
        !g_list_find_custom(locales_list, locale, (GCompareFunc)g_strcmp0)) {
      gchar *base;
      ...
      if (base && *base &&
          !g_list_find_custom(locales_list, base, (GCompareFunc)g_strcmp0)) {
...
static void gimp_help_query_online_response(GtkWidget *dialog, gint response,
                                            GimpIdleHelp *idle_help) {
  gtk_widget_destroy(dialog);

  if (response == GTK_RESPONSE_ACCEPT) {
    g_object_set(idle_help->gimp->config, "user-manual-online", TRUE, NULL);
  }
  if (response != GTK_RESPONSE_YES) {
    g_object_set(idle_help->gimp->config, "help-locales", "", NULL);
  }

  if (response == GTK_RESPONSE_ACCEPT || response == GTK_RESPONSE_YES) {
    gimp_help_show(idle_help->gimp, idle_help->progress, idle_help->help_domain,
                   idle_help->help_id);
  }
...
static void gimp_help_query_alt_user_manual(GimpIdleHelp *idle_help) {
  ...
  dialog = gimp_message_dialog_new(
      _("GIMP user manual is missing"), GIMP_ICON_HELP_USER_MANUAL, NULL, 0,
      NULL, NULL, _("_Cancel"), GTK_RESPONSE_CANCEL, NULL);
  ...
  if (manuals != NULL) {
    ...
    gtk_dialog_add_button(GTK_DIALOG(dialog), _("Read Selected _Language"),
                          GTK_RESPONSE_YES);
    /* And a dropdown list of available manuals. */
    lang_combo = gimp_language_combo_box_new(TRUE, _("Available manuals..."));
    gtk_combo_box_set_active(GTK_COMBO_BOX(lang_combo), 0);
    gtk_dialog_set_response_sensitive(idle_help->query_dialog, GTK_RESPONSE_YES,
                                      FALSE);
    g_signal_connect(lang_combo, "changed",
                     G_CALLBACK(gimp_help_language_combo_changed), idle_help);
  ...
  gtk_dialog_add_button(GTK_DIALOG(dialog), _("Read _Online"),
                        GTK_RESPONSE_ACCEPT);
  gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_ACCEPT);
  if (manuals != NULL) {
    gimp_dialog_set_alternative_button_order(
        GTK_DIALOG(dialog), GTK_RESPONSE_ACCEPT, GTK_RESPONSE_YES,
        GTK_RESPONSE_CANCEL, -1);
    g_list_free_full(manuals, g_free);
  } else {
    gimp_dialog_set_alternative_button_order(
        GTK_DIALOG(dialog), GTK_RESPONSE_ACCEPT, GTK_RESPONSE_CANCEL, -1);
  }
  g_signal_connect(dialog, "response",
                   G_CALLBACK(gimp_help_query_online_response), idle_help);
...
static void gimp_help_language_combo_changed(GtkComboBox *combo,
  ...
  g_object_set(idle_help->gimp->config, "help-locales",
               help_locales ? help_locales : "", NULL);
