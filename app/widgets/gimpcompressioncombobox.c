...
enum { COLUMN_ID, COLUMN_LABEL, N_COLUMNS };
...
static void gimp_compression_combo_box_constructed(GObject *object) {
  ...
  store = gtk_list_store_new(N_COLUMNS, G_TYPE_STRING, /* ID    */
                             G_TYPE_STRING);           /* LABEL */
  ...
  gtk_combo_box_set_row_separator_func(
      GTK_COMBO_BOX(combo_box), gimp_compression_combo_box_separator_func, NULL,
      NULL);

  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter, COLUMN_ID, "none", COLUMN_LABEL,
                     C_("compression", "None"), -1);

  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter, COLUMN_ID, NULL, COLUMN_LABEL, NULL, -1);

  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter, COLUMN_ID, "fast", COLUMN_LABEL,
                     C_("compression", "Best performance"), -1);

  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter, COLUMN_ID, "balanced", COLUMN_LABEL,
                     C_("compression", "Balanced"), -1);

  gtk_list_store_append(store, &iter);
  gtk_list_store_set(store, &iter, COLUMN_ID, "best", COLUMN_LABEL,
                     C_("compression", "Best compression"), -1);
...
GtkWidget *gimp_compression_combo_box_new(void) {
  return g_object_new(GIMP_TYPE_COMPRESSION_COMBO_BOX, "has-entry", TRUE,
                      "id-column", COLUMN_ID, "label-column", COLUMN_LABEL,
                      NULL);
}

void gimp_compression_combo_box_set_compression(
    GimpCompressionComboBox *combo_box, const gchar *compression) {
