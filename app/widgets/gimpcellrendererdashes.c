...
static void gimp_cell_renderer_dashes_render(
...
static void gimp_cell_renderer_dashes_init(GimpCellRendererDashes *dashes) {
...
static void gimp_cell_renderer_dashes_set_property(GObject *object,
  ...
  switch (param_id) {
  case PROP_PATTERN:
    gimp_dash_pattern_fill_segments(g_value_get_boxed(value), dashes->segments,
                                    N_SEGMENTS);
    break;
...
static void gimp_cell_renderer_dashes_get_size(GtkCellRenderer *cell,
  ...
  if (cell_area) {
    if (x_offset) {
      ...
      align =
          ((gtk_widget_get_direction(widget) == GTK_TEXT_DIR_RTL) ? 1.0 - xalign
                                                                  : xalign);
...
static void gimp_cell_renderer_dashes_render(
  ...
  for (x = 0; x < width + BLOCK_WIDTH; x += BLOCK_WIDTH) {
    ...
    if (dashes->segments[index]) {
      cairo_rectangle(cr, cell_area->x + xpad + x, y,
