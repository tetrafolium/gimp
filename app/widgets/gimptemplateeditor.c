...
enum { PROP_0, PROP_GIMP, PROP_TEMPLATE };
...
#define GET_PRIVATE(editor)                                                    \
  ((GimpTemplateEditorPrivate *)gimp_template_editor_get_instance_private(     \
...
static void gimp_template_editor_class_init(GimpTemplateEditorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("template", NULL, NULL, GIMP_TYPE_TEMPLATE,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}
...
private
  ->size_se =
      gimp_size_entry_new(0, gimp_template_get_unit(template), _("%p"), TRUE,
                          FALSE, FALSE, SB_WIDTH, GIMP_SIZE_ENTRY_UPDATE_SIZE);
  ...
  gimp_prop_coordinates_connect(G_OBJECT(template), "width", "height", "unit",
  ...
  aspect_box =
      gimp_enum_icon_box_new(GIMP_TYPE_ASPECT_TYPE, "gimp", GTK_ICON_SIZE_MENU,
                             G_CALLBACK(gimp_template_editor_aspect_callback),
                             editor, NULL, &private->aspect_button);
...
private
  ->pixel_label = gtk_label_new(NULL);
  gimp_label_set_attributes(GTK_LABEL(private->pixel_label), PANGO_ATTR_SCALE,
                            PANGO_SCALE_SMALL, -1);
...
private
  ->more_label = gtk_label_new(NULL);
  gimp_label_set_attributes(GTK_LABEL(private->more_label), PANGO_ATTR_SCALE,
                            PANGO_SCALE_SMALL, -1);
...
private
  ->memsize_label = gtk_label_new(NULL);
  gimp_label_set_attributes(GTK_LABEL(private->memsize_label), PANGO_ATTR_SCALE,
                            PANGO_SCALE_SMALL, PANGO_ATTR_WEIGHT,
                            PANGO_WEIGHT_BOLD, -1);
...
private
  ->expander = g_object_new(GTK_TYPE_EXPANDER, "label", text, "use-markup",
...
private
  ->resolution_se = gimp_size_entry_new(
      0, gimp_template_get_resolution_unit(template), _("pixels/%s"), FALSE,
      FALSE, FALSE, SB_WIDTH, GIMP_SIZE_ENTRY_UPDATE_RESOLUTION);
...
private
  ...
  gtk_grid_attach(GTK_GRID(private->resolution_se), private->chain_button, 1, 0,
                  1, 2);
  gtk_widget_show(private->chain_button);

  gimp_prop_coordinates_connect(
      G_OBJECT(template), "xresolution", "yresolution", "resolution-unit",
      private->resolution_se, private->chain_button, 1.0, 1.0);

  focus_chain = g_list_prepend(
      focus_chain,
  ...
  combo = gimp_prop_enum_combo_box_new(G_OBJECT(template), "image-type",
                                       GIMP_RGB, GIMP_GRAY);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("Color _space:"), 0.0,
                           0.5, combo, 1);
...
private
  ->precision_combo =
      g_object_new(GIMP_TYPE_ENUM_COMBO_BOX, "model", store, NULL);
  g_object_unref(store);

  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("_Precision:"), 0.0, 0.5,
  ...
  g_signal_connect(private->precision_combo, "changed",
                   G_CALLBACK(gimp_template_editor_precision_changed), editor);

  combo = gimp_prop_enum_combo_box_new(G_OBJECT(template), "trc",
                                       GIMP_TRC_LINEAR, GIMP_TRC_NON_LINEAR);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("_Gamma:"), 0.0, 0.5,
                           combo, 1);

private
  ->profile_combo = gimp_prop_profile_combo_box_new(
      G_OBJECT(template), "color-profile", NULL, _("Choose A Color Profile"),
      G_OBJECT(private->gimp->config), "color-profile-path");
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("Co_lor profile:"), 0.0,
                           0.5, private->profile_combo, 1);

  combo = gimp_prop_enum_combo_box_new(G_OBJECT(template), "fill-type", 0, 0);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("_Fill with:"), 0.0, 0.5,
                           combo, 1);
  ...
  label = gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("Comme_nt:"),
                                   0.0, 0.0, scrolled_window, 1);

  text_buffer = gimp_prop_text_buffer_new(G_OBJECT(template), "comment",
                                          MAX_COMMENT_LENGTH);
...
static void gimp_template_editor_set_property(GObject *object,
  ...
  case PROP_GIMP:
  private
    ->gimp = g_value_get_object(value); /* don't ref */
    break;

  case PROP_TEMPLATE:
  private
    ->template = g_value_dup_object(value);
    break;
...
GtkWidget *gimp_template_editor_new(GimpTemplate *template, Gimp *gimp,
  ...
  editor = g_object_new(GIMP_TYPE_TEMPLATE_EDITOR, "gimp", gimp, "template",
                        template, NULL);
...
private
  ...
  if (edit_template) {
    ...
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Name:"), 1.0, 0.5, entry,
                             1);

    icon_picker =
        gimp_prop_icon_picker_new(GIMP_VIEWABLE(private->template), gimp);
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("_Icon:"), 1.0, 0.5,
                             icon_picker, 1);
  }
...
static void gimp_template_editor_precision_changed(GtkWidget *widget,
  ...
  g_object_get(private->template, "trc", &trc, NULL);

  trc = gimp_suggest_trc_for_component_type(component_type, trc);

  g_object_set(private->template, "component-type", component_type, "trc", trc,
               NULL);
}

static void gimp_template_editor_set_pixels(GimpTemplateEditor *editor,
                                            GimpTemplate *template) {
  GimpTemplateEditorPrivate *private = GET_PRIVATE(editor);
  gchar *text;

  text = g_strdup_printf(ngettext("%d × %d pixel", "%d × %d pixels",
...
static void gimp_template_editor_aspect_callback(GtkWidget *widget,
  ...
  if (!private->block_aspect &&
    ...
    g_signal_handlers_block_by_func(
        template, gimp_template_editor_template_notify, editor);
    ...
    g_object_set(template, "width", height, "height", width, "xresolution",
                 yresolution, "yresolution", xresolution, NULL);
...
private
  ...
  gimp_enum_get_value(GIMP_TYPE_IMAGE_BASE_TYPE,
                      gimp_template_get_base_type(template), NULL, NULL, &desc,
                      NULL);
  ...
  if (!param_spec || !strcmp(param_spec->name, "image-type") ||
    ...
    gimp_color_profile_store_add_defaults(
        GIMP_COLOR_PROFILE_STORE(profile_store),
        private->gimp->config->color_management,
        gimp_template_get_base_type(template),
        gimp_template_get_precision(template), NULL);

    gtk_combo_box_set_model(GTK_COMBO_BOX(private->profile_combo),
                            GTK_TREE_MODEL(profile_store));
    g_object_unref(profile_store);

    g_object_get(template, "color-profile", &file, NULL);

    gimp_color_profile_combo_box_set_active_file(
        GIMP_COLOR_PROFILE_COMBO_BOX(private->profile_combo), file, NULL);
