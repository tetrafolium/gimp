...
static void gimp_device_status_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
...
static void gimp_device_status_init(GimpDeviceStatus *status) {
  status->gimp = NULL;
  ...
  status->save_button = gimp_editor_add_button(
      GIMP_EDITOR(status), GIMP_ICON_DOCUMENT_SAVE, _("Save device status"),
      NULL, G_CALLBACK(gimp_device_status_save_clicked), NULL, status);
}
...
static void gimp_device_status_constructed(GObject *object) {
  ...
  g_signal_connect_object(devices, "add",
                          G_CALLBACK(gimp_device_status_device_add), status, 0);
  g_signal_connect_object(devices, "remove",
                          G_CALLBACK(gimp_device_status_device_remove), status,
                          0);

  g_signal_connect(devices, "notify::current-device",
                   G_CALLBACK(gimp_device_status_notify_device), status);
  ...
  g_signal_connect_object(status->gimp->config, "notify::devices-share-tool",
                          G_CALLBACK(gimp_device_status_config_notify), status,
                          0);

  gimp_device_status_config_notify(GIMP_GUI_CONFIG(status->gimp->config), NULL,
                                   status);
}
  ...
  if (status->devices) {
    ...
    for (list = status->devices; list; list = list->next) {
      ...
      g_signal_handlers_disconnect_by_func(
          entry->device_info, gimp_device_status_notify_info, entry);
...
static void pack_prop_widget(GtkBox *hbox, GtkWidget *widget,
...
static void gimp_device_status_device_add(GimpContainer *devices,
  ...
  entry->context = gimp_context_new(GIMP_TOOL_PRESET(device_info)->gimp,
                                    gimp_object_get_name(device_info), NULL);

  gimp_context_define_properties(
      entry->context,
      GIMP_CONTEXT_PROP_MASK_TOOL | GIMP_CONTEXT_PROP_MASK_FOREGROUND |
          GIMP_CONTEXT_PROP_MASK_BACKGROUND | GIMP_CONTEXT_PROP_MASK_BRUSH |
          GIMP_CONTEXT_PROP_MASK_PATTERN | GIMP_CONTEXT_PROP_MASK_GRADIENT,
      FALSE);

  closure =
      g_cclosure_new(G_CALLBACK(gimp_device_status_notify_info), entry, NULL);
  g_object_watch_closure(G_OBJECT(status), closure);
  g_signal_connect_closure(device_info, "notify", closure, FALSE);

  entry->ebox = gtk_event_box_new();
  gtk_box_pack_start(GTK_BOX(status->vbox), entry->ebox, FALSE, FALSE, 0);
  ...
  if (display == NULL || display == gdk_display_get_default())
    ...
    name = g_strdup_printf("%s (%s)", gimp_object_get_name(device_info),
  ...
  entry->foreground =
      gimp_prop_color_area_new(G_OBJECT(entry->context), "foreground",
                               CELL_SIZE, CELL_SIZE, GIMP_COLOR_AREA_FLAT);
  ...
  entry->background =
      gimp_prop_color_area_new(G_OBJECT(entry->context), "background",
                               CELL_SIZE, CELL_SIZE, GIMP_COLOR_AREA_FLAT);
...
GtkWidget *gimp_device_status_new(Gimp *gimp) {
  ...
  return g_object_new(GIMP_TYPE_DEVICE_STATUS, "gimp", gimp, NULL);
}
...
static void toggle_prop_visible(GtkWidget *widget, GtkWidget *widget_none,
                                gboolean available) {
...
static void gimp_device_status_notify_info(GimpDeviceInfo *device_info,
  ...
  if (tool_options != entry->tool_options) {
    ...
    toggle_prop_visible(entry->foreground, entry->foreground_none,
                        serialize_props & GIMP_CONTEXT_PROP_MASK_FOREGROUND);

    toggle_prop_visible(entry->background, entry->background_none,
                        serialize_props & GIMP_CONTEXT_PROP_MASK_BACKGROUND);

    toggle_prop_visible(entry->brush, entry->brush_none,
    ...
    toggle_prop_visible(entry->gradient, entry->gradient_none,
...
static void gimp_device_status_view_clicked(GtkWidget *widget,
  ...
  status = GIMP_DEVICE_STATUS(
      gtk_widget_get_ancestor(widget, GIMP_TYPE_DEVICE_STATUS));
  dialog_factory = gimp_dialog_factory_get_singleton();

  gimp_window_strategy_show_dockable_dialog(
      GIMP_WINDOW_STRATEGY(gimp_get_window_strategy(status->gimp)),
      status->gimp, dialog_factory, gimp_widget_get_monitor(widget),
      identifier);
}
