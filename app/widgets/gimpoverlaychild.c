...
static void gimp_overlay_child_from_embedder(GdkWindow *child_window,
                                             ...
                                             gdouble *child_x, gdouble *child_y,
...
GimpOverlayChild *gimp_overlay_child_find(GimpOverlayBox *box,
  ...
  g_return_val_if_fail(GIMP_IS_OVERLAY_BOX(box), NULL);
  g_return_val_if_fail(GTK_IS_WIDGET(widget), NULL);
  g_return_val_if_fail(gtk_widget_get_parent(widget) == GTK_WIDGET(box), NULL);
...
void gimp_overlay_child_realize(GimpOverlayBox *box, GimpOverlayChild *child) {
  ...
  g_signal_connect(child->window, "from-embedder",
                   G_CALLBACK(gimp_overlay_child_from_embedder), child);
  g_signal_connect(child->window, "to-embedder",
                   G_CALLBACK(gimp_overlay_child_to_embedder), child);
...
void gimp_overlay_child_size_allocate(GimpOverlayBox *box,
  ...
  if (gtk_widget_get_realized(GTK_WIDGET(widget)))
    gdk_window_move_resize(child->window, child_allocation.x,
                           child_allocation.y, child_allocation.width,
...
static void gimp_overlay_child_clip_fully_opaque(GimpOverlayChild *child,
  ...
  for (list = children; list; list = g_list_next(list)) {
    ...
    if (gimp_widget_get_fully_opaque(widget)) {
      ...
      gtk_widget_get_allocation(widget, &allocation);
      gtk_widget_translate_coordinates(widget, child->widget, 0, 0, &x, &y);

      cairo_rectangle(cr, x, y, allocation.width, allocation.height);
    } else if (GTK_IS_CONTAINER(widget)) {
      gimp_overlay_child_clip_fully_opaque(child, GTK_CONTAINER(widget), cr);
    }
  }
...
gboolean gimp_overlay_child_draw(GimpOverlayBox *box, GimpOverlayChild *child,
  ...
  if (gtk_cairo_should_draw_window(cr, gtk_widget_get_window(widget))) {
    ...
    if (gtk_widget_get_visible(child->widget)) {
      ...
      gimp_overlay_child_clip_fully_opaque(child, GTK_CONTAINER(child->widget),
                                           cr);
      cairo_clip(cr);
      cairo_paint(cr);
...
gboolean gimp_overlay_child_damage(GimpOverlayBox *box, GimpOverlayChild *child,
  ...
  if (event->window == child->window) {
    ...
    for (i = 0; i < n_rects; i++) {
      ...
      gdk_window_invalidate_rect(gtk_widget_get_window(widget), &bounds, FALSE);
    }
...
gboolean gimp_overlay_child_pick(GimpOverlayBox *box, GimpOverlayChild *child,
  ...
  gimp_overlay_child_from_embedder(child->window, box_x, box_y, &child_x,
                                   &child_y, child);

  gtk_widget_get_allocation(child->widget, &child_allocation);

  if (child_x >= 0 && child_x < child_allocation.width && child_y >= 0 &&
