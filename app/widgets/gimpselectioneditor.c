...
static void gimp_selection_editor_drop_color(GtkWidget *widget, gint x, gint y,
                                             const GimpRGB *color,
                                             gpointer data);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpSelectionEditor, gimp_selection_editor, GIMP_TYPE_IMAGE_EDITOR,
...
static void gimp_selection_editor_init(GimpSelectionEditor *editor) {
  GtkWidget *frame;
  ...
  editor->view = gimp_view_new_by_types(
      NULL, GIMP_TYPE_VIEW, GIMP_TYPE_SELECTION, GIMP_VIEW_SIZE_HUGE, 0, TRUE);
  ...
  g_signal_connect(editor->view, "button-press-event",
                   G_CALLBACK(gimp_selection_view_button_press), editor);

  gimp_dnd_color_dest_add(editor->view, gimp_selection_editor_drop_color,
                          editor);
...
static void gimp_selection_editor_constructed(GObject *object) {
  ...
  editor->all_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor), "select", "select-all", NULL);

  editor->none_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor), "select", "select-none", NULL);

  editor->invert_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor), "select", "select-invert", NULL);

  editor->save_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor), "select", "select-save", NULL);

  editor->path_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor), "vectors", "vectors-selection-to-vectors",
      "vectors-selection-to-vectors-advanced", GDK_SHIFT_MASK, NULL);

  editor->stroke_button = gimp_editor_add_action_button(
      GIMP_EDITOR(editor), "select", "select-stroke",
      "select-stroke-last-values", GDK_SHIFT_MASK, NULL);
}

static void gimp_selection_editor_set_image(GimpImageEditor *image_editor,
                                            GimpImage *image) {
  GimpSelectionEditor *editor = GIMP_SELECTION_EDITOR(image_editor);

  if (image_editor->image) {
    g_signal_handlers_disconnect_by_func(
        image_editor->image, gimp_selection_editor_mask_changed, editor);
  }
  ...
  if (image) {
    g_signal_connect(image, "mask-changed",
                     G_CALLBACK(gimp_selection_editor_mask_changed), editor);
...
GtkWidget *gimp_selection_editor_new(GimpMenuFactory *menu_factory) {
  ...
  return g_object_new(GIMP_TYPE_SELECTION_EDITOR, "menu-factory", menu_factory,
                      "menu-identifier", "<Selection>", "ui-path",
...
static gboolean gimp_selection_view_button_press(GtkWidget *widget,
  ...
  if (gimp_image_pick_color(image_editor->image, drawables, x, y, FALSE,
                            options->sample_merged, FALSE, 0.0, NULL, NULL,
                            &color)) {
    gimp_channel_select_by_color(
        gimp_image_get_mask(image_editor->image), drawables,
        options->sample_merged, &color, options->threshold / 255.0,
        options->select_transparent, options->select_criterion, operation,
        sel_options->antialias, sel_options->feather,
...
static void gimp_selection_editor_drop_color(GtkWidget *widget, gint x, gint y,
  ...
  tool_info =
      gimp_get_tool_info(editor->image->gimp, "gimp-by-color-select-tool");
  if (!tool_info)
    return;
  ...
  gimp_channel_select_by_color(
      gimp_image_get_mask(editor->image), drawables, options->sample_merged,
      color, options->threshold / 255.0, options->select_transparent,
      options->select_criterion, sel_options->operation, sel_options->antialias,
      sel_options->feather, sel_options->feather_radius,
