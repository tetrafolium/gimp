...
enum { ACTION_ADDED, LAST_SIGNAL };

enum { PROP_0, PROP_GIMP, PROP_LABEL, PROP_ICON_NAME };
...
static void gimp_action_group_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_action_group_get_property(GObject *object, guint prop_id,
...
static void gimp_action_group_class_init(GimpActionGroupClass *klass) {
  ...
  signals[ACTION_ADDED] = g_signal_new(
      "action-added", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpActionGroupClass, action_added), NULL, NULL, NULL,
      G_TYPE_NONE, 1, GIMP_TYPE_ACTION);
}
 ...
 */
GimpActionGroup *gimp_action_group_new(Gimp *gimp, const gchar *name,
                                       const gchar *label,
  ...
  group = g_object_new(GIMP_TYPE_ACTION_GROUP, "gimp", gimp, "name", name,
                       "label", label, "icon-name", icon_name, NULL);
...
void gimp_action_group_add_actions(GimpActionGroup *group,
  ...
  for (i = 0; i < n_entries; i++) {
    ...
    action = gimp_action_impl_new(entries[i].name, label, tooltip,
                                  entries[i].icon_name, entries[i].help_id);

    if (entries[i].callback)
      g_signal_connect(action, "gimp-activate", G_CALLBACK(entries[i].callback),
                       group->user_data);
  ...
  for (i = 0; i < n_entries; i++) {
    ...
    if (entries[i].callback)
      g_signal_connect(action, "gimp-change-state",
                       G_CALLBACK(entries[i].callback), group->user_data);
...
GSList *gimp_action_group_add_radio_actions(GimpActionGroup *group,
  ...
  if (callback && first_action)
    g_signal_connect(first_action, "gimp-change-state", G_CALLBACK(callback),
                     group->user_data);
...
void gimp_action_group_add_enum_actions(GimpActionGroup *group,
  ...
  for (i = 0; i < n_entries; i++) {
    ...
    action = gimp_enum_action_new(entries[i].name, label, tooltip,
                                  entries[i].icon_name, entries[i].help_id,
                                  entries[i].value, entries[i].value_variable);

    if (callback)
      g_signal_connect(action, "gimp-activate", G_CALLBACK(callback),
                       group->user_data);
...
void gimp_action_group_add_procedure_actions(
    GimpActionGroup *group, const GimpProcedureActionEntry *entries,
    guint n_entries, GimpActionCallback callback) {
  gint i;
  ...
  for (i = 0; i < n_entries; i++) {
    ...
    action = gimp_procedure_action_new(
        entries[i].name, entries[i].label, entries[i].tooltip,
        entries[i].icon_name, entries[i].help_id, entries[i].procedure);
...
void gimp_action_group_remove_action_and_accel(GimpActionGroup *group,
  ...
  accel_path = g_strconcat("<Actions>/", group_name, "/", action_name, NULL);
...
void gimp_action_group_set_action_color(GimpActionGroup *group,
  ...
  if (set_label) {
    ...
    if (color)
      label = g_strdup_printf(_("RGBA (%0.3f, %0.3f, %0.3f, %0.3f)"), color->r,
                              color->g, color->b, color->a);
    else
      label = g_strdup(_("(none)"));

    g_object_set(action, "color", color, "label", label, NULL);
    g_free(label);
  } else {
...
void gimp_action_group_set_action_always_show_image(
    GimpActionGroup *group, const gchar *action_name,
