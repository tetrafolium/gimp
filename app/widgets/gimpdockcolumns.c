...
enum { PROP_0, PROP_CONTEXT, PROP_DIALOG_FACTORY, PROP_UI_MANAGER };

enum { DOCK_ADDED, DOCK_REMOVED, LAST_SIGNAL };
...
static void gimp_dock_columns_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_dock_columns_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);
static gboolean gimp_dock_columns_dropped_cb(GtkWidget *notebook,
                                             GtkWidget *child,
                                             gint insert_index, gpointer data);
...
static void gimp_dock_columns_class_init(GimpDockColumnsClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("ui-manager", NULL, NULL, GIMP_TYPE_UI_MANAGER,
                          GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  dock_columns_signals[DOCK_ADDED] =
      g_signal_new("dock-added", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpDockColumnsClass, dock_added), NULL,
                   NULL, NULL, G_TYPE_NONE, 1, GIMP_TYPE_DOCK);

  dock_columns_signals[DOCK_REMOVED] =
      g_signal_new("dock-removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpDockColumnsClass, dock_removed), NULL,
                   NULL, NULL, G_TYPE_NONE, 1, GIMP_TYPE_DOCK);
}

static void gimp_dock_columns_init(GimpDockColumns *dock_columns) {
  ...
  dock_columns->p->paned_hbox =
      gimp_paned_box_new(FALSE, 0, GTK_ORIENTATION_HORIZONTAL);
  gimp_paned_box_set_dropped_cb(GIMP_PANED_BOX(dock_columns->p->paned_hbox),
                                gimp_dock_columns_dropped_cb, dock_columns);
  gtk_box_pack_start(GTK_BOX(dock_columns), dock_columns->p->paned_hbox, TRUE,
                     TRUE, 0);
...
static gboolean gimp_dock_columns_dropped_cb(GtkWidget *notebook,
  ...
  gimp_dock_columns_prepare_dockbook(dock_columns, insert_index, &new_dockbook);
...
static void gimp_dock_columns_dock_book_removed(GimpDockColumns *dock_columns,
  ...
  if (gimp_dock_get_dockbooks(dock) == NULL && !GIMP_IS_TOOLBOX(dock) &&
...
GtkWidget *gimp_dock_columns_new(GimpContext *context,
  ...
  return g_object_new(GIMP_TYPE_DOCK_COLUMNS, "context", context,
                      "dialog-factory", dialog_factory, "ui-manager",
                      ui_manager, NULL);
}
 ...
 **/
void gimp_dock_columns_add_dock(GimpDockColumns *dock_columns, GimpDock *dock,
                                gint index) {
...
void gimp_dock_columns_prepare_dockbook(GimpDockColumns *dock_columns,
  ...
  menu_factory =
      gimp_dialog_factory_get_menu_factory(dock_columns->p->dialog_factory);
...
void gimp_dock_columns_remove_dock(GimpDockColumns *dock_columns,
  ...
  GIMP_LOG(DND, "Removing GimpDock %p from GimpDockColumns %p", dock,
           dock_columns);
  ...
  g_signal_handlers_disconnect_by_func(
      dock, gimp_dock_columns_dock_book_removed, dock_columns);
