...
enum { PROP_0, PROP_PROCEDURE };

static void gimp_procedure_action_finalize(GObject *object);
static void gimp_procedure_action_set_property(GObject *object, guint prop_id,
                                               const GValue *value,
                                               GParamSpec *pspec);
static void gimp_procedure_action_get_property(GObject *object, guint prop_id,
                                               GValue *value,
                                               GParamSpec *pspec);
...
G_DEFINE_TYPE(GimpProcedureAction, gimp_procedure_action, GIMP_TYPE_ACTION_IMPL)

#define parent_class gimp_procedure_action_parent_class

static void gimp_procedure_action_class_init(GimpProcedureActionClass *klass) {
...
static void gimp_procedure_action_init(GimpProcedureAction *action) {}
...
static void gimp_procedure_action_activate(GtkAction *action) {
  ...
  if (procedure_action->procedure) {
    ...
    gimp_action_emit_activate(GIMP_ACTION(action), g_variant_new_uint64(hack));
...
static void gimp_procedure_action_connect_proxy(GtkAction *action,
  ...
  if (GTK_IS_MENU_ITEM(proxy) && procedure_action->procedure) {
    ...
    g_object_get(procedure_action->procedure, "icon-pixbuf", &pixbuf, NULL);
...
gimp_procedure_action_new(const gchar *name, const gchar *label,
  ...
  action = g_object_new(GIMP_TYPE_PROCEDURE_ACTION, "name", name, "label",
                        label, "tooltip", tooltip, "icon-name", icon_name,
