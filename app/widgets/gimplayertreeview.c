...
static gboolean gimp_layer_tree_view_drop_possible(
...
static void
gimp_layer_tree_view_drop_uri_list(GimpContainerTreeView *view, GList *uri_list,
                                   GimpViewable *dest_viewable,
                                   GtkTreeViewDropPosition drop_pos);
static void
...
static void gimp_layer_tree_view_layer_mode_box_callback(
    GtkWidget *widget, const GParamSpec *pspec, GimpLayerTreeView *view);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpLayerTreeView, gimp_layer_tree_view, GIMP_TYPE_DRAWABLE_TREE_VIEW,
...
static void gimp_layer_tree_view_class_init(GimpLayerTreeViewClass *klass) {
  ...
  item_view_class->get_active_item =
      (GimpGetItemFunc)gimp_image_get_active_layer;
  item_view_class->set_active_item =
      (GimpSetItemFunc)gimp_image_set_active_layer;
  item_view_class->get_selected_items =
      (GimpGetItemsFunc)gimp_image_get_selected_layers;
  item_view_class->set_selected_items =
      (GimpSetItemsFunc)gimp_image_set_selected_layers;
...
static void gimp_layer_tree_view_init(GimpLayerTreeView *view) {
  ...
  view->priv->model_column_mask = gimp_container_tree_store_columns_add(
  ...
  view->priv->layer_mode_box =
      gimp_layer_mode_box_new(GIMP_LAYER_MODE_CONTEXT_LAYER);
  ...
  view->priv->opacity_adjustment =
      gtk_adjustment_new(100.0, 0.0, 100.0, 1.0, 10.0, 0.0);
  ...
  gimp_item_tree_view_add_options(GIMP_ITEM_TREE_VIEW(view), NULL, scale);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), view->priv->lock_alpha_toggle, FALSE, FALSE,
                     0);
  ...
  gimp_help_set_help_data(view->priv->lock_alpha_toggle,
                          _("Lock alpha channel"), GIMP_HELP_LAYER_LOCK_ALPHA);

  gtk_widget_style_get(GTK_WIDGET(view), "button-icon-size", &icon_size, NULL);
...
static void gimp_layer_tree_view_constructed(GObject *object) {
  ...
  gtk_tree_view_column_pack_start(tree_view->main_column,
                                  layer_view->priv->mask_cell, FALSE);
  gtk_tree_view_column_set_attributes(
      tree_view->main_column, layer_view->priv->mask_cell, "renderer",
      layer_view->priv->model_column_mask, "visible",
      layer_view->priv->model_column_mask_visible, NULL);
  ...
  g_signal_connect(tree_view->renderer_cell, "clicked",
                   G_CALLBACK(gimp_layer_tree_view_layer_clicked), layer_view);
  g_signal_connect(layer_view->priv->mask_cell, "clicked",
                   G_CALLBACK(gimp_layer_tree_view_mask_clicked), layer_view);

  gimp_dnd_component_dest_add(GTK_WIDGET(tree_view->view), NULL, tree_view);
  ...
  gimp_dnd_viewable_dest_add(GTK_WIDGET(tree_view->view), GIMP_TYPE_LAYER_MASK,
                             NULL, tree_view);
  gimp_dnd_uri_list_dest_add(GTK_WIDGET(tree_view->view), NULL, tree_view);
  gimp_dnd_pixbuf_dest_add(GTK_WIDGET(tree_view->view), NULL, tree_view);
  ...
  gimp_container_view_enable_dnd(GIMP_CONTAINER_VIEW(layer_view),
                                 GTK_BUTTON(button), GIMP_TYPE_LAYER);
  gtk_box_reorder_child(gimp_editor_get_button_box(GIMP_EDITOR(layer_view)),
                        button, 5);

  button = gimp_editor_add_action_button(
      GIMP_EDITOR(layer_view), "layers", "layers-merge-down-button",
      "layers-merge-group", GDK_SHIFT_MASK, "layers-merge-layers",
      GDK_CONTROL_MASK, "layers-merge-layers-last-values",
  ...
  button = gimp_editor_add_action_button(
      GIMP_EDITOR(layer_view), "layers", "layers-mask-add-button",
      "layers-mask-add-last-values", gimp_get_extend_selection_mask(),
      "layers-mask-delete", gimp_get_modify_selection_mask(),
      "layers-mask-apply",
      gimp_get_extend_selection_mask() | gimp_get_modify_selection_mask(),
      NULL);
  gimp_container_view_enable_dnd(GIMP_CONTAINER_VIEW(layer_view),
                                 GTK_BUTTON(button), GIMP_TYPE_LAYER);
  gtk_box_reorder_child(gimp_editor_get_button_box(GIMP_EDITOR(layer_view)),
...
static void gimp_layer_tree_view_set_container(GimpContainerView *view,
  ...
  if (container) {
    layer_view->priv->mode_changed_handler = gimp_tree_handler_connect(
        container, "mode-changed",
        G_CALLBACK(gimp_layer_tree_view_layer_signal_handler), view);

    layer_view->priv->opacity_changed_handler = gimp_tree_handler_connect(
        container, "opacity-changed",
        G_CALLBACK(gimp_layer_tree_view_layer_signal_handler), view);

    layer_view->priv->lock_alpha_changed_handler = gimp_tree_handler_connect(
        container, "lock-alpha-changed",
        G_CALLBACK(gimp_layer_tree_view_layer_signal_handler), view);

    layer_view->priv->mask_changed_handler = gimp_tree_handler_connect(
        container, "mask-changed",
        G_CALLBACK(gimp_layer_tree_view_mask_changed), view);

    layer_view->priv->alpha_changed_handler = gimp_tree_handler_connect(
        container, "alpha-changed",
        G_CALLBACK(gimp_layer_tree_view_alpha_changed), view);
  }
}
...
static gboolean gimp_layer_tree_view_set_context_foreach(GtkTreeModel *model,
                                                         GtkTreePath *path,
                                                         GtkTreeIter *iter,
                                                         gpointer data) {
  ...
  gtk_tree_model_get(model, iter, context_data->mask_column, &renderer, -1);
...
static gboolean gimp_layer_tree_view_select_items(GimpContainerView *view,
  ...
  if (layers) {
    if (success) {
      for (layers = items, path = paths; layers && path;
...
static gboolean gimp_layer_tree_view_set_view_size_foreach(GtkTreeModel *model,
                                                           GtkTreePath *path,
                                                           GtkTreeIter *iter,
                                                           gpointer data) {
  ...
  gtk_tree_model_get(model, iter, size_data->mask_column, &renderer, -1);

  if (renderer) {
    gimp_view_renderer_set_size(renderer, size_data->view_size,
...
static gboolean gimp_layer_tree_view_drop_possible(
  ...
  if (src_type == GIMP_DND_TYPE_URI_LIST ||
      ...
      src_type == GIMP_DND_TYPE_COMPONENT || src_type == GIMP_DND_TYPE_PIXBUF ||
      g_list_length(src_viewables) > 0) {
    GimpImage *dest_image =
        gimp_item_tree_view_get_image(GIMP_ITEM_TREE_VIEW(tree_view));
...
static void gimp_layer_tree_view_drop_color(GimpContainerTreeView *view,
  ...
  if (gimp_item_is_text_layer(GIMP_ITEM(dest_viewable))) {
    gimp_text_layer_set(GIMP_TEXT_LAYER(dest_viewable), NULL, "color", color,
                        NULL);
    gimp_image_flush(gimp_item_tree_view_get_image(GIMP_ITEM_TREE_VIEW(view)));
    return;
  }

  GIMP_CONTAINER_TREE_VIEW_CLASS(parent_class)
      ->drop_color(view, color, dest_viewable, drop_pos);
}
...
gimp_layer_tree_view_drop_uri_list(GimpContainerTreeView *view, GList *uri_list,
  ...
  index = gimp_item_tree_view_get_drop_index(item_view, dest_viewable, drop_pos,
                                             (GimpViewable **)&parent);
  ...
  for (list = uri_list; list; list = g_list_next(list)) {
    ...
    new_layers = file_open_layers(
        image->gimp, gimp_container_view_get_context(cont_view), NULL, image,
        FALSE, file, GIMP_RUN_INTERACTIVE, NULL, &status, &error);

    if (new_layers) {
      gimp_image_add_layers(image, new_layers, parent, index, 0, 0,
                            gimp_image_get_width(image),
                            gimp_image_get_height(image), _("Drop layers"));
...
static void gimp_layer_tree_view_drop_component(
    GimpContainerTreeView *tree_view, GimpImage *src_image,
    GimpChannelType component, GimpViewable *dest_viewable,
    GtkTreeViewDropPosition drop_pos) {
  ...
  new_item = gimp_item_convert(GIMP_ITEM(channel), image, GIMP_TYPE_LAYER);

  g_object_unref(channel);

  gimp_enum_get_value(GIMP_TYPE_CHANNEL_TYPE, component, NULL, NULL, &desc,
                      NULL);
...
static void gimp_layer_tree_view_drop_pixbuf(GimpContainerTreeView *tree_view,
  ...
  new_layer = gimp_layer_new_from_pixbuf(
      pixbuf, image, gimp_image_get_layer_format(image, TRUE),
      _("Dropped Buffer"), GIMP_OPACITY_OPAQUE,
...
static void gimp_layer_tree_view_set_image(GimpItemTreeView *view,
  ...
  if (gimp_item_tree_view_get_image(view)) {
    g_signal_handlers_disconnect_by_func(
        ...
        gimp_layer_tree_view_floating_selection_changed, view);
  }
  ...
  if (gimp_item_tree_view_get_image(view)) {
    g_signal_connect(
        gimp_item_tree_view_get_image(view), "floating-selection-changed",
        G_CALLBACK(gimp_layer_tree_view_floating_selection_changed), view);
    ...
    gimp_layer_tree_view_floating_selection_changed(
        gimp_item_tree_view_get_image(view), layer_view);
  }
...
static GimpItem *gimp_layer_tree_view_item_new(GimpImage *image) {
  ...
  new_layer = gimp_layer_new(
      ...
      gimp_image_get_layer_format(image, TRUE), NULL, GIMP_OPACITY_OPAQUE,
      gimp_image_get_default_new_layer_mode(image));

  gimp_image_add_layer(image, new_layer, GIMP_IMAGE_ACTIVE_PARENT, -1, TRUE);
  ...
  if (floating_sel) {
    ...
    if (iter)
      gtk_tree_store_set(GTK_TREE_STORE(tree_view->model), iter,
                         ...
                         layer_view->priv->italic_attrs, -1);
  } else {
    GList *all_layers;
    GList *list;
...
#define BLOCK()                                                                \
  g_signal_handlers_block_by_func(                                             \
      layer, gimp_layer_tree_view_layer_signal_handler, view)
...
static void gimp_layer_tree_view_layer_mode_box_callback(
  ...
  if (!push_undo)
    gimp_undo_refresh_preview(
        undo, gimp_container_view_get_context(GIMP_CONTAINER_VIEW(view)));
...
static void gimp_layer_tree_view_layer_signal_handler(GimpLayer *layer,
  ...
  selected_layers = GIMP_ITEM_TREE_VIEW_GET_CLASS(view)->get_selected_items(
...
static void gimp_layer_tree_view_update_options(GimpLayerTreeView *view,
  ...
  gimp_layer_mode_box_set_context(
      GIMP_LAYER_MODE_BOX(view->priv->layer_mode_box), context);
  ...
  if (all_have_lock_alpha != gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(
                                 view->priv->lock_alpha_toggle))) {
    BLOCK(view->priv->lock_alpha_toggle,
          gimp_layer_tree_view_lock_alpha_button_toggled);

    gtk_toggle_button_set_active(
        GTK_TOGGLE_BUTTON(view->priv->lock_alpha_toggle), all_have_lock_alpha);

    UNBLOCK(view->priv->lock_alpha_toggle,
            gimp_layer_tree_view_lock_alpha_button_toggled);
  }
  gtk_toggle_button_set_inconsistent(
      GTK_TOGGLE_BUTTON(view->priv->lock_alpha_toggle),
  ...
  if (opacity * 100.0 !=
    ...
    gtk_adjustment_set_value(view->priv->opacity_adjustment, opacity * 100.0);
...
static void gimp_layer_tree_view_update_menu(GimpLayerTreeView *layer_view,
                                             GList *layers) {
  GimpUIManager *ui_manager =
      gimp_editor_get_ui_manager(GIMP_EDITOR(layer_view));
  ...
  gimp_action_group_set_action_active(
      group, "layers-mask-edit",
      g_list_length(layers) == 1 && gimp_layer_get_mask(layers->data) &&
...
gimp_layer_tree_view_update_highlight(GimpLayerTreeView *layer_view) {
  ...
  gtk_widget_style_get(GTK_WIDGET(layer_view), "button-relief", &default_relief,
                       NULL);

  gimp_button_set_suggested(
      gimp_item_tree_view_get_new_button(item_view),
      floating_sel &&
  ...
  gimp_button_set_destructive(gimp_item_tree_view_get_delete_button(item_view),
                              floating_sel != NULL, default_relief);
...
static void gimp_layer_tree_view_mask_update(GimpLayerTreeView *layer_view,
                                             GtkTreeIter *iter,
                                             GimpLayer *layer) {
  GimpContainerView *view = GIMP_CONTAINER_VIEW(layer_view);
  GimpContainerTreeView *tree_view = GIMP_CONTAINER_TREE_VIEW(layer_view);
  ...
  if (mask) {
    ...
    renderer = gimp_view_renderer_new(gimp_container_view_get_context(view),
                                      G_TYPE_FROM_INSTANCE(mask), view_size,
                                      border_width, FALSE);
...
gimp_layer_tree_view_renderer_update(GimpViewRenderer *renderer,
  ...
  iter = gimp_container_view_lookup(
      view, (GimpViewable *)gimp_layer_mask_get_layer(mask));
...
static void gimp_layer_tree_view_update_borders(GimpLayerTreeView *layer_view,
                                                GtkTreeIter *iter) {
  GimpContainerTreeView *tree_view = GIMP_CONTAINER_TREE_VIEW(layer_view);
  ...
  gtk_tree_model_get(tree_view->model, iter,
                     ...
                     layer_view->priv->model_column_mask, &mask_renderer, -1);
...
static void gimp_layer_tree_view_mask_callback(GimpLayer *layer,
                                               GimpLayerTreeView *layer_view) {
  GimpContainerView *view = GIMP_CONTAINER_VIEW(layer_view);
  GtkTreeIter *iter;
  ...
  if (gtk_tree_model_get_iter(tree_view->model, &iter, path)) {
    ...
    if (renderer) {
      ...
      if (!modifiers && mask && gimp_layer_get_edit_mask(layer)) {
        ...
        if (mask)
          gimp_action_group_set_action_active(group, "layers-mask-edit", FALSE);
      }
    ...
    if (renderer) {
      ...
      if ((state & GDK_MOD1_MASK)) {
        ...
        if ((state & modifiers) == GDK_MOD1_MASK) {
          ...
          gimp_layer_set_show_mask(layer, !gimp_layer_get_show_mask(layer),
                                   TRUE);
          gimp_image_flush(image);
        }
        if ((state & modifiers) == (GDK_MOD1_MASK | GDK_CONTROL_MASK)) {
          /* Alt-Control-click enables/disables a layer mask */
          gimp_layer_set_apply_mask(layer, !gimp_layer_get_apply_mask(layer),
                                    TRUE);
          gimp_image_flush(image);
        }
      } else if (!gimp_layer_get_edit_mask(layer)) {
        /* Simple click selects the mask for edition. */
        gimp_action_group_set_action_active(group, "layers-mask-edit", TRUE);
      }
...
static void gimp_layer_tree_view_alpha_changed(GimpLayer *layer,
  ...
  if (iter) {
    ...
    if (g_list_find(gimp_image_get_selected_layers(
                        gimp_item_tree_view_get_image(item_view)),
                    layer))
      gimp_container_view_select_items(
          GIMP_CONTAINER_VIEW(view),
