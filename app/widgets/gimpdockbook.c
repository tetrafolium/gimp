...
enum { DOCKABLE_ADDED, DOCKABLE_REMOVED, DOCKABLE_REORDERED, LAST_SIGNAL };
...
static void gimp_dockbook_drag_end(GtkWidget *widget, GdkDragContext *context);
...
static GtkNotebook *gimp_dockbook_create_window(GtkNotebook *notebook,
                                                GtkWidget *page, gint x,
                                                gint y);
static void gimp_dockbook_page_added(GtkNotebook *notebook, GtkWidget *child,
                                     guint page_num);
static void gimp_dockbook_page_removed(GtkNotebook *notebook, GtkWidget *child,
                                       guint page_num);
...
static void gimp_dockbook_help_func(const gchar *help_id, gpointer help_data);
...
static void gimp_dockbook_class_init(GimpDockbookClass *klass) {
  ...
  dockbook_signals[DOCKABLE_ADDED] = g_signal_new(
      "dockable-added", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpDockbookClass, dockable_added), NULL, NULL, NULL,
      G_TYPE_NONE, 1, GIMP_TYPE_DOCKABLE);

  dockbook_signals[DOCKABLE_REMOVED] = g_signal_new(
      "dockable-removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpDockbookClass, dockable_removed), NULL, NULL, NULL,
      G_TYPE_NONE, 1, GIMP_TYPE_DOCKABLE);

  dockbook_signals[DOCKABLE_REORDERED] = g_signal_new(
      "dockable-reordered", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpDockbookClass, dockable_reordered), NULL, NULL, NULL,
      G_TYPE_NONE, 1, GIMP_TYPE_DOCKABLE);
  ...
  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_enum("tab-icon-size", NULL, NULL, GTK_TYPE_ICON_SIZE,
                        DEFAULT_TAB_ICON_SIZE, GIMP_PARAM_READABLE));
}
...
static void gimp_dockbook_init(GimpDockbook *dockbook) {
  ...
  gtk_drag_dest_set(GTK_WIDGET(dockbook), 0, dialog_target_table,
                    G_N_ELEMENTS(dialog_target_table), GDK_ACTION_MOVE);

  /* Menu button */
  dockbook->p->menu_button = gtk_button_new();
  gtk_widget_set_can_focus(dockbook->p->menu_button, FALSE);
  gtk_button_set_relief(GTK_BUTTON(dockbook->p->menu_button), GTK_RELIEF_NONE);
  gtk_notebook_set_action_widget(notebook, dockbook->p->menu_button,
                                 GTK_PACK_END);
  gtk_widget_show(dockbook->p->menu_button);

  image = gtk_image_new_from_icon_name(GIMP_ICON_MENU_LEFT, GTK_ICON_SIZE_MENU);
...
static gboolean gimp_dockbook_drag_motion(GtkWidget *widget,
  ...
  if (gimp_paned_box_will_handle_drag(dockbook->p->drag_handler, widget,
                                      context, x, y, time)) {
...
static gboolean gimp_dockbook_drag_drop(GtkWidget *widget,
  ...
  return GTK_WIDGET_CLASS(parent_class)->drag_drop(widget, context, x, y, time);
}
...
static GtkNotebook *gimp_dockbook_create_window(GtkNotebook *notebook,
  ...
  new_dock = gimp_dock_with_window_new(dialog_factory,
                                       gimp_widget_get_monitor(page), FALSE);
...
static void gimp_dockbook_page_added(GtkNotebook *notebook, GtkWidget *child,
  ...
  GIMP_LOG(DND, "GimpDockable %p added to GimpDockbook %p", dockable, dockbook);

  tab_widget = gimp_dockbook_create_tab_widget(dockbook, dockable);

  /* For the notebook right-click menu, always use the icon style */
  menu_widget = gimp_dockable_create_tab_widget(
      dockable, gimp_dock_get_context(dockbook->p->dock),
      GIMP_TAB_STYLE_ICON_BLURB, GTK_ICON_SIZE_MENU);
  ...
  g_signal_connect(dockable, "notify::locked",
                   G_CALLBACK(gimp_dockbook_tab_locked_notify), dockbook);
...
static void gimp_dockbook_page_removed(GtkNotebook *notebook, GtkWidget *child,
  ...
  GIMP_LOG(DND, "GimpDockable removed %p from GimpDockbook %p", dockable,
           dockbook);

  g_signal_handlers_disconnect_by_func(
      dockable, G_CALLBACK(gimp_dockbook_tab_locked_notify), dockbook);
...
static gboolean gimp_dockbook_show_menu(GimpDockbook *dockbook) {
  ...
  parent_menu_widget = gimp_ui_manager_get_widget(
      dockbook_ui_manager, "/dockable-popup/dockable-menu");
  parent_menu_action = gimp_ui_manager_get_action(
      dockbook_ui_manager, "/dockable-popup/dockable-menu");
  ...
  dialog_ui_manager =
      gimp_dockable_get_menu(dockable, &dialog_ui_path, &dialog_popup_data);
  ...
  if (dialog_ui_manager && dialog_ui_path) {
    ...
    if (!child_menu_widget) {
      g_warning("%s: UI manager '%s' has no widget at path '%s'", G_STRFUNC,
                dialog_ui_manager->name, dialog_ui_path);
      return FALSE;
    }

    child_menu_action =
        gimp_ui_manager_get_action(dialog_ui_manager, dialog_ui_path);

    if (!child_menu_action) {
      g_warning("%s: UI manager '%s' has no action at path '%s'", G_STRFUNC,
                dialog_ui_manager->name, dialog_ui_path);
      return FALSE;
    }

    g_object_get(child_menu_action, "label", &label, NULL);

    g_object_set(parent_menu_action, "label", label, "icon-name",
                 gimp_dockable_get_icon_name(dockable), "visible", TRUE, NULL);

    g_free(label);

    if (!GTK_IS_MENU(child_menu_widget)) {
      g_warning("%s: child_menu_widget (%p) is not a GtkMenu", G_STRFUNC,
                child_menu_widget);
      return FALSE;
    }

    {
      GtkWidget *image = gimp_dockable_get_icon(dockable, GTK_ICON_SIZE_MENU);
  ...
  g_object_set_data_full(G_OBJECT(dockable), GIMP_DOCKABLE_DETACH_REF_KEY,
                         g_object_ref(dockbook), g_object_unref);

  gimp_ui_manager_update(dockbook_ui_manager, dockable);

  gimp_ui_manager_ui_popup_at_widget(
      dockbook_ui_manager, "/dockable-popup", dockbook->p->menu_button,
      GDK_GRAVITY_WEST, GDK_GRAVITY_NORTH_EAST, NULL,
...
GtkWidget *gimp_dockbook_new(GimpMenuFactory *menu_factory) {
  ...
  dockbook->p->ui_manager =
      gimp_menu_factory_manager_new(menu_factory, "<Dockable>", dockbook);
...
GtkWidget *gimp_dockbook_add_from_dialog_factory(GimpDockbook *dockbook,
  ...
  dockable = gimp_dialog_factory_dockable_new(
      gimp_dock_get_dialog_factory(dock), dock, identifier, -1);
  ...
  if (dockable && !gimp_dockable_get_dockbook(GIMP_DOCKABLE(dockable)))
    gtk_notebook_append_page(GTK_NOTEBOOK(dockbook), dockable, NULL);
...
GtkWidget *gimp_dockbook_create_tab_widget(GimpDockbook *dockbook,
  ...
  gtk_widget_style_get(GTK_WIDGET(dockbook), "tab-icon-size", &tab_size, NULL);
  tab_widget = gimp_dockable_create_tab_widget(
      dockable, gimp_dock_get_context(dockbook->p->dock),
      gimp_dockable_get_tab_style(dockable), tab_size);
  ...
  if (dock_window &&
    ...
    dialog_id = g_object_get_data(G_OBJECT(dockable), "gimp-dialog-identifier");
      ...
      if (group) {
        ...
        for (list = actions; list; list = g_list_next(list)) {
          if (GIMP_IS_STRING_ACTION(list->data) &&
              strstr(GIMP_STRING_ACTION(list->data)->value, dialog_id)) {
            action = list->data;
            break;
          }
        }
  ...
  if (action)
    ...
    gimp_help_set_help_data(tab_widget, gimp_dockable_get_blurb(dockable),
...
void gimp_dockbook_remove_drag_callback(GimpDockbookDragCallback callback,
  ...
  while (iter) {
    ...
    if (callback_data->callback == callback && callback_data->data == data) {
...
static void gimp_dockbook_tab_locked_notify(GimpDockable *dockable,
  ...
  gtk_notebook_set_tab_reorderable(GTK_NOTEBOOK(dockbook), GTK_WIDGET(dockable),
                                   !locked);
  gtk_notebook_set_tab_detachable(GTK_NOTEBOOK(dockbook), GTK_WIDGET(dockable),
                                  !locked);
}
