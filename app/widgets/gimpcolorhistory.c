...
enum { COLOR_SELECTED, LAST_SIGNAL };
...
static void gimp_color_history_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_color_history_get_property(GObject *object, guint property_id,
                                            GValue *value, GParamSpec *pspec);

/* GtkWidget methods */
static GtkSizeRequestMode
gimp_color_history_get_request_mode(GtkWidget *widget);
static void gimp_color_history_get_preferred_width_for_height(
    GtkWidget *widget, gint height, gint *minimum_width, gint *natural_width);
static void gimp_color_history_get_preferred_height_for_width(
...
static void gimp_color_history_color_changed(GtkWidget *widget, gpointer data);
...
static void gimp_color_history_class_init(GimpColorHistoryClass *klass) {
  ...
  widget_class->get_preferred_width_for_height =
      gimp_color_history_get_preferred_width_for_height;
  widget_class->get_preferred_height_for_width =
      gimp_color_history_get_preferred_height_for_width;
  widget_class->size_allocate = gimp_color_history_size_allocate;

  history_signals[COLOR_SELECTED] = g_signal_new(
      "color-selected", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpColorHistoryClass, color_selected), NULL, NULL, NULL,
  ...
  g_object_class_install_property(
      object_class, PROP_HISTORY_SIZE,
      g_param_spec_int("history-size", NULL, NULL, 2, G_MAXINT,
...
static void gimp_color_history_init(GimpColorHistory *history) {
...
static void gimp_color_history_finalize(GObject *object) {
  ...
  if (history->context)
    g_signal_handlers_disconnect_by_func(
        history->context, gimp_color_history_image_changed, history);
...
static void gimp_color_history_set_property(GObject *object, guint property_id,
  ...
  case PROP_CONTEXT:
    ...
    if (history->context) {
      g_signal_connect(history->context, "image-changed",
                       G_CALLBACK(gimp_color_history_image_changed), history);
  ...
  case PROP_HISTORY_SIZE: {
    ...
    history->buttons = g_realloc_n(history->buttons, history->history_size,
                                   sizeof(GtkWidget *));
    history->color_areas = g_realloc_n(
        history->color_areas, history->history_size, sizeof(GtkWidget *));
...
static void gimp_color_history_get_preferred_height_for_width(
  ...
  if (button) {
    button_width = MAX(gtk_widget_get_allocated_width(button), COLOR_AREA_SIZE);
    button_height =
        MAX(gtk_widget_get_allocated_height(button), COLOR_AREA_SIZE);
  }

  if (width >
      button_width * history->history_size + 2 * (history->history_size - 1))
...
GtkWidget *gimp_color_history_new(GimpContext *context, gint history_size) {
  ...
  history = g_object_new(GIMP_TYPE_COLOR_HISTORY, "context", context,
                         "history-size", history_size, NULL);
...
static void gimp_color_history_palette_dirty(GimpColorHistory *history) {
  ...
  for (i = 0; i < history->history_size; i++) {
    ...
    if (/* Common out-of-gamut case */
        ...
        /* Grayscale images */
        (base_type == GIMP_GRAY && (ABS(color.r - color.g) > CHANNEL_EPSILON ||
                                    ABS(color.r - color.b) > CHANNEL_EPSILON ||
                                    ABS(color.g - color.b) > CHANNEL_EPSILON)))
      oog = TRUE;
    gimp_color_area_set_out_of_gamut(GIMP_COLOR_AREA(history->color_areas[i]),
                                     oog);
...
static void gimp_color_history_color_changed(GtkWidget *widget, gpointer data) {
  ...
  history = GIMP_COLOR_HISTORY(
      gtk_widget_get_ancestor(widget, GIMP_TYPE_COLOR_HISTORY));
