...
enum { PROP_0, PROP_DATA_FACTORY, PROP_CONTEXT, PROP_DATA };
...
static void gimp_data_editor_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_data_editor_get_property(GObject *object, guint property_id,
...
static void gimp_data_editor_set_context(GimpDocked *docked,
                                         GimpContext *context);
static void gimp_data_editor_set_aux_info(GimpDocked *docked, GList *aux_info);
...
static void gimp_data_editor_data_changed(GimpContext *context, GimpData *data,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpDataEditor, gimp_data_editor, GIMP_TYPE_EDITOR,
    G_IMPLEMENT_INTERFACE(GIMP_TYPE_DOCKED, gimp_data_editor_docked_iface_init))
...
static void gimp_data_editor_class_init(GimpDataEditorClass *klass) {
  ...
  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_int("minimal-height", NULL, NULL, 32, G_MAXINT,
                       DEFAULT_MINIMAL_HEIGHT, GIMP_PARAM_READABLE));
}
...
static void gimp_data_editor_init(GimpDataEditor *editor) {
  ...
  g_signal_connect(editor->name_entry, "key-press-event",
                   G_CALLBACK(gimp_data_editor_name_key_press), editor);
  g_signal_connect(editor->name_entry, "activate",
                   G_CALLBACK(gimp_data_editor_name_activate), editor);
  g_signal_connect(editor->name_entry, "focus-out-event",
                   G_CALLBACK(gimp_data_editor_name_focus_out), editor);
}
...
static void gimp_data_editor_style_updated(GtkWidget *widget) {
  ...
  gtk_widget_style_get(widget, "minimal-height", &minimal_height, NULL);
...
static void gimp_data_editor_set_context(GimpDocked *docked,
  ...
  if (editor->context) {
    g_signal_handlers_disconnect_by_func(editor->context,
                                         gimp_data_editor_data_changed, editor);
  ...
  if (editor->context) {
    ...
    g_signal_connect(editor->context,
                     ...
                     G_CALLBACK(gimp_data_editor_data_changed), editor);
  ...
  for (list = aux_info; list; list = g_list_next(list)) {
    ...
    if (!strcmp(aux->name, AUX_INFO_EDIT_ACTIVE)) {
      ...
      if (!editor->edit_active) {
        ...
        data = (GimpData *)gimp_container_get_child_by_name(
            gimp_data_factory_get_container(editor->data_factory), aux->value);
...
static void gimp_data_editor_real_set_data(GimpDataEditor *editor,
  ...
  if (editor->data) {
    ...
    g_signal_handlers_disconnect_by_func(
        editor->data, gimp_data_editor_data_name_changed, editor);
  ...
  if (editor->data) {
    ...
    g_signal_connect(editor->data, "name-changed",
                     G_CALLBACK(gimp_data_editor_data_name_changed), editor);
  ...
  gtk_editable_set_editable(GTK_EDITABLE(editor->name_entry),
                            editor->data && gimp_viewable_is_name_editable(
                                                GIMP_VIEWABLE(editor->data)));
...
void gimp_data_editor_set_data(GimpDataEditor *editor, GimpData *data) {
  ...
  g_return_if_fail(data == NULL || g_type_is_a(G_TYPE_FROM_INSTANCE(data),
                                               gimp_data_factory_get_data_type(
                                                   editor->data_factory)));
...
static void gimp_data_editor_name_activate(GtkWidget *widget,
  ...
  if (editor->data) {
    ...
    if (strlen(new_name) &&
      ...
      gtk_entry_set_text(GTK_ENTRY(widget), gimp_object_get_name(editor->data));
      g_free(new_name);
    }
  }
}
...
static void gimp_data_editor_save_dirty(GimpDataEditor *editor) {
  ...
  if (data && gimp_data_is_dirty(data) && gimp_data_is_writable(data)) {
    GError *error = NULL;
    ...
    if (!gimp_data_factory_data_save_single(editor->data_factory, data,
      ...
      gimp_message_literal(gimp_data_factory_get_gimp(editor->data_factory),
                           G_OBJECT(editor), GIMP_MESSAGE_ERROR,
                           error->message);
      g_clear_error(&error);
    }
  }
