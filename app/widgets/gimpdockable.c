...
static void gimp_dockable_set_property(GObject *object, guint property_id,
                                       const GValue *value, GParamSpec *pspec);
static void gimp_dockable_get_property(GObject *object, guint property_id,
                                       GValue *value, GParamSpec *pspec);

static void gimp_dockable_style_updated(GtkWidget *widget);

static void gimp_dockable_add(GtkContainer *container, GtkWidget *widget);
...
G_DEFINE_TYPE_WITH_CODE(GimpDockable, gimp_dockable, GTK_TYPE_BIN,
                        G_ADD_PRIVATE(GimpDockable) G_IMPLEMENT_INTERFACE(
...
static void gimp_dockable_class_init(GimpDockableClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("locked", NULL, NULL, FALSE, GIMP_PARAM_READWRITE));

  gtk_widget_class_install_style_property(
      widget_class, g_param_spec_int("content-border", NULL, NULL, 0, G_MAXINT,
                                     0, GIMP_PARAM_READABLE));
}
...
static void gimp_dockable_init(GimpDockable *dockable) {
  ...
  gtk_drag_dest_set(GTK_WIDGET(dockable), 0, dialog_target_table,
...
static void gimp_dockable_style_updated(GtkWidget *widget) {
  ...
  gtk_widget_style_get(widget, "content-border", &content_border, NULL);
...
static GtkWidget *gimp_dockable_new_tab_widget_internal(GimpDockable *dockable,
                                                        GimpContext *context,
                                                        GimpTabStyle tab_style,
                                                        GtkIconSize size,
                                                        gboolean dnd) {
  ...
  case GIMP_TAB_STYLE_PREVIEW_BLURB: {
    ...
    if (child)
      icon = gimp_docked_get_preview(GIMP_DOCKED(child), context, size);
...
GtkWidget *gimp_dockable_new(const gchar *name, const gchar *blurb,
...
void gimp_dockable_set_dockbook(GimpDockable *dockable,
  ...
  g_return_if_fail(dockbook == NULL || GIMP_IS_DOCKBOOK(dockbook));
...
void gimp_dockable_set_locked(GimpDockable *dockable, gboolean lock) {
  g_return_if_fail(GIMP_IS_DOCKABLE(dockable));
...
GtkWidget *gimp_dockable_get_icon(GimpDockable *dockable, GtkIconSize size) {
  g_return_val_if_fail(GIMP_IS_DOCKABLE(dockable), NULL);
...
GtkWidget *gimp_dockable_create_tab_widget(GimpDockable *dockable,
  ...
  return gimp_dockable_new_tab_widget_internal(dockable, context, tab_style,
                                               size, FALSE);
}

void gimp_dockable_set_context(GimpDockable *dockable, GimpContext *context) {
  g_return_if_fail(GIMP_IS_DOCKABLE(dockable));
...
void gimp_dockable_detach(GimpDockable *dockable) {
  ...
  dock = gimp_dock_with_window_new(
      dialog_factory, gimp_widget_get_monitor(GTK_WIDGET(dockable)),
      FALSE /*toolbox*/);
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(dockbook), GTK_WIDGET(dockable), NULL);
...
static GList *gimp_dockable_get_aux_info(GimpSessionManaged *session_managed) {
  GimpDockable *dockable;
  GtkWidget *child;
