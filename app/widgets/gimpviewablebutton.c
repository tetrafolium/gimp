...
enum { PROP_0, PROP_POPUP_VIEW_TYPE, PROP_POPUP_VIEW_SIZE };
...
static void gimp_viewable_button_class_init(GimpViewableButtonClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_POPUP_VIEW_TYPE,
      g_param_spec_enum("popup-view-type", NULL, NULL, GIMP_TYPE_VIEW_TYPE,
                        GIMP_VIEW_TYPE_LIST, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_POPUP_VIEW_SIZE,
      g_param_spec_int("popup-view-size", NULL, NULL, GIMP_VIEW_SIZE_TINY,
                       GIMP_VIEW_SIZE_GIGANTIC, GIMP_VIEW_SIZE_SMALL,
...
static gboolean gimp_viewable_button_scroll_event(GtkWidget *widget,
  ...
  object = gimp_context_get_by_type(
      button->context, gimp_container_get_children_type(button->container));
  ...
  if (index != -1) {
    ...
    if (new_index != index) {
      object = gimp_container_get_child_by_index(button->container, new_index);

      if (object)
        gimp_context_set_by_type(
            button->context,
            gimp_container_get_children_type(button->container), object);
    }
  }
...
static void gimp_viewable_button_clicked(GtkButton *button) {
  ...
  popup = gimp_container_popup_new(
      viewable_button->container, viewable_button->context,
      viewable_button->popup_view_type, viewable_button->button_view_size,
      viewable_button->popup_view_size, viewable_button->view_border_width,
      viewable_button->dialog_factory, viewable_button->dialog_identifier,
      viewable_button->dialog_icon_name, viewable_button->dialog_tooltip);

  g_signal_connect(popup, "cancel",
                   G_CALLBACK(gimp_viewable_button_popup_closed), button);
...
GtkWidget *gimp_viewable_button_new(
  ...
  g_return_val_if_fail(
      view_size > 0 && view_size <= GIMP_VIEWABLE_MAX_BUTTON_SIZE, NULL);
  ...
  button = g_object_new(GIMP_TYPE_VIEWABLE_BUTTON, "popup-view-type", view_type,
                        "popup-view-size", view_size, NULL);
  ...
  prop_name = gimp_context_type_to_prop_name(
      gimp_container_get_children_type(container));

  button->view = gimp_prop_view_new(G_OBJECT(context), prop_name, context,
