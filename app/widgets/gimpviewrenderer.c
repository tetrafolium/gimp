...
static void gimp_view_render_temp_buf_to_surface(
    GimpViewRenderer *renderer, GtkWidget *widget, GimpTempBuf *temp_buf,
    gint temp_buf_x, gint temp_buf_y, gint channel, GimpViewBG inside_bg,
    GimpViewBG outside_bg, cairo_surface_t *surface, gint dest_width,
    gint dest_height);
...
static void gimp_view_renderer_class_init(GimpViewRendererClass *klass) {
  ...
  renderer_signals[UPDATE] =
      g_signal_new("update", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpViewRendererClass, update), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);
...
static GimpViewRenderer *gimp_view_renderer_new_internal(GimpContext *context,
                                                         GType viewable_type,
                                                         gboolean is_popup) {
  GimpViewRenderer *renderer;

  renderer = g_object_new(
      gimp_view_renderer_type_from_viewable_type(viewable_type), NULL);
...
GimpViewRenderer *gimp_view_renderer_new(GimpContext *context,
  ...
  g_return_val_if_fail(size > 0 && size <= GIMP_VIEWABLE_MAX_PREVIEW_SIZE,
                       NULL);
  g_return_val_if_fail(
      border_width >= 0 && border_width <= GIMP_VIEW_MAX_BORDER_WIDTH, NULL);

  renderer = gimp_view_renderer_new_internal(context, viewable_type, is_popup);
...
GimpViewRenderer *gimp_view_renderer_new_full(GimpContext *context,
  ...
  g_return_val_if_fail(context == NULL || GIMP_IS_CONTEXT(context), NULL);
  g_return_val_if_fail(g_type_is_a(viewable_type, GIMP_TYPE_VIEWABLE), NULL);
  g_return_val_if_fail(width > 0 && width <= GIMP_VIEWABLE_MAX_PREVIEW_SIZE,
                       NULL);
  g_return_val_if_fail(height > 0 && height <= GIMP_VIEWABLE_MAX_PREVIEW_SIZE,
                       NULL);
  g_return_val_if_fail(
      border_width >= 0 && border_width <= GIMP_VIEW_MAX_BORDER_WIDTH, NULL);
...
static void gimp_view_renderer_weak_notify(GimpViewRenderer *renderer,
                                           GimpViewable *viewable) {
  renderer->viewable = NULL;
...
void gimp_view_renderer_set_viewable(GimpViewRenderer *renderer,
  ...
  if (viewable)
    g_return_if_fail(
        g_type_is_a(G_TYPE_FROM_INSTANCE(viewable), renderer->viewable_type));
  ...
  if (renderer->viewable) {
    g_object_weak_unref(G_OBJECT(renderer->viewable),
                        (GWeakNotify)gimp_view_renderer_weak_notify, renderer);

    g_signal_handlers_disconnect_by_func(
        renderer->viewable, G_CALLBACK(gimp_view_renderer_invalidate),
        renderer);

    g_signal_handlers_disconnect_by_func(
        renderer->viewable, G_CALLBACK(gimp_view_renderer_size_changed),
        renderer);

    if (GIMP_IS_COLOR_MANAGED(renderer->viewable))
      g_signal_handlers_disconnect_by_func(
          renderer->viewable, G_CALLBACK(gimp_view_renderer_profile_changed),
          renderer);
  }
  ...
  if (renderer->viewable) {
    ...
    g_signal_connect_swapped(renderer->viewable, "invalidate-preview",
                             G_CALLBACK(gimp_view_renderer_invalidate),
                             renderer);

    g_signal_connect_swapped(renderer->viewable, "size-changed",
                             G_CALLBACK(gimp_view_renderer_size_changed),
                             renderer);

    if (GIMP_IS_COLOR_MANAGED(renderer->viewable))
      g_signal_connect_swapped(renderer->viewable, "profile-changed",
...
void gimp_view_renderer_set_size(GimpViewRenderer *renderer, gint view_size,
                                 gint border_width) {
  gint width;
  gint height;
  ...
  if (renderer->viewable) {
    gimp_viewable_get_preview_size(renderer->viewable, view_size,
                                   renderer->is_popup, renderer->dot_for_dot,
                                   &width, &height);
  } else {
    width = view_size;
    height = view_size;
...
void gimp_view_renderer_set_size_full(GimpViewRenderer *renderer, gint width,
                                      gint height, gint border_width) {
  g_return_if_fail(GIMP_IS_VIEW_RENDERER(renderer));
  g_return_if_fail(width > 0 && width <= GIMP_VIEWABLE_MAX_PREVIEW_SIZE);
  g_return_if_fail(height > 0 && height <= GIMP_VIEWABLE_MAX_PREVIEW_SIZE);
  g_return_if_fail(border_width >= 0 &&
                   border_width <= GIMP_VIEW_MAX_BORDER_WIDTH);
  ...
  if (color_config != renderer->priv->color_config) {
    ...
    if (renderer->priv->color_config)
      g_signal_connect(renderer->priv->color_config, "notify",
                       G_CALLBACK(gimp_view_renderer_config_notify), renderer);
...
void gimp_view_renderer_invalidate(GimpViewRenderer *renderer) {
  ...
  renderer->priv->idle_id = g_idle_add_full(
      GIMP_PRIORITY_VIEWABLE_IDLE, (GSourceFunc)gimp_view_renderer_idle_update,
      renderer, NULL);
}
...
void gimp_view_renderer_draw(GimpViewRenderer *renderer, GtkWidget *widget,
                             cairo_t *cr, gint available_width,
                             gint available_height) {
  g_return_if_fail(GIMP_IS_VIEW_RENDERER(renderer));
  ...
  if (renderer->viewable) {
    ...
    GIMP_VIEW_RENDERER_GET_CLASS(renderer)->draw(
        renderer, widget, cr, available_width, available_height);
    ...
    gimp_view_renderer_real_draw(renderer, widget, cr, available_width,
                                 available_height);
  }
...
static void gimp_view_renderer_real_set_context(GimpViewRenderer *renderer,
  ...
  if (renderer->context && renderer->priv->color_config == NULL) {
    gimp_view_renderer_set_color_config(
        renderer, renderer->context->gimp->config->color_management);
  }
}
...
static void gimp_view_renderer_real_draw(GimpViewRenderer *renderer,
  ...
  if (renderer->priv->icon_surface) {
    ...
    if (content == CAIRO_CONTENT_COLOR_ALPHA) {
      if (!renderer->priv->pattern)
        renderer->priv->pattern = gimp_cairo_checkerboard_create(
            cr, GIMP_CHECK_SIZE_SM, gimp_render_light_check_color(),
...
static void gimp_view_renderer_real_render(GimpViewRenderer *renderer,
  ...
  pixbuf = gimp_viewable_get_pixbuf(renderer->viewable, renderer->context,
  ...
  temp_buf = gimp_viewable_get_preview(renderer->viewable, renderer->context,
...
void gimp_view_renderer_render_temp_buf_simple(GimpViewRenderer *renderer,
  ...
  gimp_view_renderer_render_temp_buf(renderer, widget, temp_buf, temp_buf_x,
                                     temp_buf_y, -1, GIMP_VIEW_BG_CHECKS,
                                     GIMP_VIEW_BG_WHITE);
}

void gimp_view_renderer_render_temp_buf(GimpViewRenderer *renderer,
                                        GtkWidget *widget,
                                        GimpTempBuf *temp_buf, gint temp_buf_x,
                                        gint temp_buf_y, gint channel,
  ...
  if (!renderer->surface)
    renderer->surface = cairo_image_surface_create(
        CAIRO_FORMAT_RGB24, renderer->width, renderer->height);

  gimp_view_render_temp_buf_to_surface(
      renderer, widget, temp_buf, temp_buf_x, temp_buf_y, channel, inside_bg,
      outside_bg, renderer->surface, renderer->width, renderer->height);
}
...
void gimp_view_renderer_render_pixbuf(GimpViewRenderer *renderer,
  ...
  transform =
      gimp_view_renderer_get_color_transform(renderer, widget, format, format);
  ...
  if (transform) {
    ...
    new = gdk_pixbuf_new(GDK_COLORSPACE_RGB, gdk_pixbuf_get_has_alpha(pixbuf),
                         8, width, height);
    ...
    for (i = 0; i < height; i++) {
      gimp_color_transform_process_pixels(transform, format, src, format, dest,
                                          width);
...
void gimp_view_renderer_render_icon(GimpViewRenderer *renderer,
  ...
  if (width > renderer->width * scale_factor ||
    ...
    gimp_viewable_calc_preview_size(
        width, height, renderer->width * scale_factor,
        renderer->height * scale_factor, TRUE, 1.0, 1.0, &width, &height, NULL);

    scaled_pixbuf =
        gdk_pixbuf_scale_simple(pixbuf, width * scale_factor,
                                height * scale_factor, GDK_INTERP_BILINEAR);
...
GimpColorTransform *gimp_view_renderer_get_color_transform(
    GimpViewRenderer *renderer, GtkWidget *widget, const Babl *src_format,
  ...
  renderer->priv->profile_transform = gimp_widget_get_color_transform(
      widget, renderer->priv->color_config, profile, src_format, dest_format);
...
static void gimp_view_render_temp_buf_to_surface(
    GimpViewRenderer *renderer, GtkWidget *widget, GimpTempBuf *temp_buf,
    gint temp_buf_x, gint temp_buf_y, gint channel, GimpViewBG inside_bg,
    GimpViewBG outside_bg, cairo_surface_t *surface, gint surface_width,
    gint surface_height) {
  cairo_t *cr;
  gint x, y;
  gint width, height;
  ...
  if (outside_bg == GIMP_VIEW_BG_CHECKS || inside_bg == GIMP_VIEW_BG_CHECKS) {
    if (!renderer->priv->pattern)
      renderer->priv->pattern = gimp_cairo_checkerboard_create(
          cr, GIMP_CHECK_SIZE_SM, gimp_render_light_check_color(),
          gimp_render_dark_check_color());
  ...
  if (!gimp_rectangle_intersect(0, 0, surface_width, surface_height, temp_buf_x,
                                temp_buf_y, temp_buf_width, temp_buf_height, &x,
                                &y, &width, &height)) {
    cairo_destroy(cr);
    return;
  }

  if (inside_bg != outside_bg && babl_format_has_alpha(temp_buf_format) &&
      channel == -1) {
    cairo_rectangle(cr, x, y, width, height);
  ...
  if (babl_format_has_alpha(temp_buf_format) && channel == -1) {
    ...
    transform = gimp_view_renderer_get_color_transform(
        renderer, widget, gegl_buffer_get_format(src_buffer),
        gegl_buffer_get_format(dest_buffer));

    if (transform) {
      gimp_color_transform_process_buffer(
          transform, src_buffer,
          GEGL_RECTANGLE(x - temp_buf_x, y - temp_buf_y, width, height),
          dest_buffer, GEGL_RECTANGLE(0, 0, 0, 0));
    } else {
      gimp_gegl_buffer_copy(
          src_buffer,
          GEGL_RECTANGLE(x - temp_buf_x, y - temp_buf_y, width, height),
          GEGL_ABYSS_NONE, dest_buffer, GEGL_RECTANGLE(0, 0, 0, 0));
    }
    ...
    if (transform) {
      gimp_color_transform_process_buffer(
          transform, src_buffer,
          GEGL_RECTANGLE(x - temp_buf_x, y - temp_buf_y, width, height),
          dest_buffer, GEGL_RECTANGLE(x, y, 0, 0));
    } else {
      gimp_gegl_buffer_copy(
          src_buffer,
          GEGL_RECTANGLE(x - temp_buf_x, y - temp_buf_y, width, height),
          GEGL_ABYSS_NONE, dest_buffer, GEGL_RECTANGLE(x, y, 0, 0));
    }
    ...
    src = gimp_temp_buf_get_data(temp_buf) +
          ((y - temp_buf_y) * rowstride + (x - temp_buf_x) * bytes);
    ...
    fish = babl_fish(temp_buf_format, babl_format("cairo-RGB24"));
...
gimp_view_renderer_create_background(GimpViewRenderer *renderer,
  ...
  if (renderer->priv->bg_icon_name) {
    ...
    pixbuf = gimp_widget_load_icon(widget, renderer->priv->bg_icon_name, 64);
