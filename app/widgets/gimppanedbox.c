...
static void gimp_paned_box_class_init(GimpPanedBoxClass *klass) {
...
static void gimp_paned_box_init(GimpPanedBox *paned_box) {
  paned_box->p = gimp_paned_box_get_instance_private(paned_box);

  /* Setup DND */
  gtk_drag_dest_set(GTK_WIDGET(paned_box), 0, dialog_target_table,
                    G_N_ELEMENTS(dialog_target_table), GDK_ACTION_MOVE);

  gimp_dockbook_add_drag_callback(
      (GimpDockbookDragCallback)gimp_paned_box_drag_callback, paned_box);
}
...
static void gimp_paned_box_set_widget_drag_handler(GtkWidget *widget,
  ...
  if (GIMP_IS_DOCKBOOK(widget)) {
    gimp_dockbook_set_drag_handler(GIMP_DOCKBOOK(widget), drag_handler);
  }
...
static gboolean gimp_paned_box_get_handle_drag(GimpPanedBox *paned_box,
                                               GdkDragContext *context, gint x,
                                               gint y, guint time,
                                               gint *insert_index,
                                               GeglRectangle *area) {
  ...
  if (gimp_paned_box_will_handle_drag(paned_box->p->drag_handler,
                                      GTK_WIDGET(paned_box), context, x, y,
                                      time)) {
    return FALSE;
  }
...
static gboolean gimp_paned_box_drop_indicator_draw(GtkWidget *widget,
  ...
  for (i = 0; i < G_N_ELEMENTS(paned_box->p->dnd_highlights); i++) {
    ...
    cairo_rectangle(cr, highlight->area.x, highlight->area.y,
                    highlight->area.width, highlight->area.height);
...
static void gimp_paned_box_position_drop_indicator(GimpPanedBox *paned_box,
                                                   gint index,
  ...
  if (!paned_box->p->dnd_highlights[index].active) {
    if (paned_box->p->dnd_n_highlights == 0) {
      g_signal_connect_after(
          widget, "draw", G_CALLBACK(gimp_paned_box_drop_indicator_draw), NULL);
    }
  ...
  if (paned_box->p->dnd_highlights[index].active) {
    ...
    if (paned_box->p->dnd_n_highlights == 0) {
      g_signal_handlers_disconnect_by_func(
          widget, gimp_paned_box_drop_indicator_draw, NULL);
    }
  }
...
static gboolean gimp_paned_box_drag_motion(GtkWidget *widget,
   ...
   */
  if (handle) {
    gimp_paned_box_position_drop_indicator(paned_box, 0, &area,
...
static gboolean gimp_paned_box_drag_drop(GtkWidget *widget,
  ...
  if (gimp_paned_box_will_handle_drag(paned_box->p->drag_handler, widget,
                                      context, x, y, time)) {
    return FALSE;
  }
...
static void gimp_paned_box_drag_data_received(GtkWidget *widget,
  ...
  if (paned_box->p->dropped_cb) {
    dropped =
        paned_box->p->dropped_cb(notebook, *child, paned_box->p->insert_index,
...
static gboolean gimp_paned_box_drag_callback_idle(GimpPanedBox *paned_box) {
  GtkAllocation allocation;
  GtkOrientation orientation;
  GeglRectangle area;
...
#define ADD_AREA(index, left, top)                                             \
  if (gimp_paned_box_get_handle_drag(paned_box, paned_box->p->dnd_context,     \
                                     (left), (top), 0, NULL, &area)) {         \
    gimp_paned_box_position_drop_indicator(paned_box, index, &area,            \
...
static void gimp_paned_box_drag_callback(GdkDragContext *context,
  ...
  paned = gtk_widget_get_ancestor(GTK_WIDGET(paned_box), GTK_TYPE_PANED);
  ...
  if (begin && !paned_box->p->dnd_context) {
    ...
    if (paned) {
      ...
      if (position < DROP_HIGHLIGHT_MIN_SIZE) {
        ...
        if (gtk_widget_is_ancestor(GTK_WIDGET(paned_box),
                                   gtk_paned_get_child2(GTK_PANED(paned)))) {
          position = allocation.width - position;
        }
    ...
    paned_box->p->dnd_idle_id =
        g_idle_add((GSourceFunc)gimp_paned_box_drag_callback_idle, paned_box);
...
GtkWidget *gimp_paned_box_new(gboolean homogeneous, gint spacing,
                              GtkOrientation orientation) {
  return g_object_new(GIMP_TYPE_PANED_BOX, "homogeneous", homogeneous,
                      "spacing", 0, "orientation", orientation, NULL);
}
 ...
 **/
void gimp_paned_box_add_widget(GimpPanedBox *paned_box, GtkWidget *widget,
                               gint index) {
  gint old_length = 0;
  ...
  if (old_length == 0) {
    ...
    if (index == 0) {
      ...
      gtk_paned_pack2(GTK_PANED(paned), old_widget, TRUE, FALSE);
    } else {
 ...
 **/
void gimp_paned_box_remove_widget(GimpPanedBox *paned_box, GtkWidget *widget) {
  gint old_length = 0;
  gint index = 0;
  ...
  GIMP_LOG(DND, "Removing GtkWidget %p from GimpPanedBox %p", widget,
           paned_box);
...
gboolean gimp_paned_box_will_handle_drag(GimpPanedBox *paned_box,
  ...
  g_return_val_if_fail(paned_box == NULL || GIMP_IS_PANED_BOX(paned_box),
                       FALSE);
  ...
  if (!gtk_widget_translate_coordinates(widget, GTK_WIDGET(paned_box), x, y,
