...
static void
gimp_container_icon_view_selection_changed(GtkIconView *view,
                                           GimpContainerIconView *icon_view);
static void
gimp_container_icon_view_item_activated(GtkIconView *view, GtkTreePath *path,
                                        GimpContainerIconView *icon_view);
static gboolean
gimp_container_icon_view_button_press(GtkWidget *widget, GdkEventButton *bevent,
                                      GimpContainerIconView *icon_view);
static gboolean
gimp_container_icon_view_tooltip(GtkWidget *widget, gint x, gint y,
                                 gboolean keyboard_tip, GtkTooltip *tooltip,
                                 GimpContainerIconView *icon_view);

static GimpViewable *
gimp_container_icon_view_drag_viewable(GtkWidget *widget, GimpContext **context,
                                       gpointer data);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpContainerIconView, gimp_container_icon_view, GIMP_TYPE_CONTAINER_BOX,
...
static void gimp_container_icon_view_init(GimpContainerIconView *icon_view) {
...
static void gimp_container_icon_view_constructed(GObject *object) {
  ...
  icon_view->model = gimp_container_tree_store_new(
      view, icon_view->n_model_columns, icon_view->model_columns);

  icon_view->view =
      g_object_new(GTK_TYPE_ICON_VIEW, "model", icon_view->model, "row-spacing",
                   0, "column-spacing", 0, "margin", 0, "item-padding", 1,
  ...
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(icon_view->view),
                             icon_view->renderer_cell, FALSE);
  gtk_cell_layout_set_attributes(
      GTK_CELL_LAYOUT(icon_view->view), icon_view->renderer_cell, "renderer",
      GIMP_CONTAINER_TREE_STORE_COLUMN_RENDERER, NULL);

  gimp_container_tree_store_add_renderer_cell(
      GIMP_CONTAINER_TREE_STORE(icon_view->model), icon_view->renderer_cell);
  ...
  g_signal_connect(icon_view->view, "query-tooltip",
                   G_CALLBACK(gimp_container_icon_view_tooltip), icon_view);
}
...
static void gimp_container_icon_view_menu_position(GtkMenu *menu, gint *x,
...
static gboolean gimp_container_icon_view_popup_menu(GtkWidget *widget) {
  return gimp_editor_popup_menu(GIMP_EDITOR(widget),
                                gimp_container_icon_view_menu_position, widget);
}
  ...
  if (old_container) {
    if (!container) {
      ...
      g_signal_handlers_disconnect_by_func(
          icon_view->view, gimp_container_icon_view_button_press, icon_view);
    }
  } else if (container) {
    if (gimp_dnd_drag_source_set_by_type(
            GTK_WIDGET(icon_view->view), GDK_BUTTON1_MASK | GDK_BUTTON2_MASK,
...
static void gimp_container_icon_view_set_context(GimpContainerView *view,
  ...
  if (icon_view->model)
    gimp_container_tree_store_set_context(
        GIMP_CONTAINER_TREE_STORE(icon_view->model), context);
}
...
gimp_container_icon_view_insert_item(GimpContainerView *view,
  ...
  iter = gimp_container_tree_store_insert_item(
      GIMP_CONTAINER_TREE_STORE(icon_view->model), viewable, parent_insert_data,
      index);
...
static void gimp_container_icon_view_remove_item(GimpContainerView *view,
  ...
  gimp_container_tree_store_remove_item(
      GIMP_CONTAINER_TREE_STORE(icon_view->model), viewable, insert_data);
}
...
static void gimp_container_icon_view_reorder_item(GimpContainerView *view,
  ...
  if (iter) {
    ...
    selected =
        gimp_container_icon_view_get_selected_single(icon_view, &selected_iter);
  ...
  gimp_container_tree_store_reorder_item(
      GIMP_CONTAINER_TREE_STORE(icon_view->model), viewable, new_index, iter);
...
static void gimp_container_icon_view_rename_item(GimpContainerView *view,
  ...
  gimp_container_tree_store_rename_item(
      GIMP_CONTAINER_TREE_STORE(icon_view->model), viewable, iter);
}
...
static gboolean gimp_container_icon_view_select_item(GimpContainerView *view,
  ...
  if (viewable && insert_data) {
    ...
    g_signal_handlers_block_by_func(
        icon_view->view, gimp_container_icon_view_selection_changed, icon_view);
...
static void gimp_container_icon_view_clear_items(GimpContainerView *view) {
  ...
  gimp_container_tree_store_clear_items(
      GIMP_CONTAINER_TREE_STORE(icon_view->model));
...
gimp_container_icon_view_button_press(GtkWidget *widget, GdkEventButton *bevent,
  ...
  path = gtk_icon_view_get_path_at_pos(GTK_ICON_VIEW(widget), bevent->x,
                                       bevent->y);
...
static gint gimp_container_icon_view_get_selected(GimpContainerView *view,
  ...
  if (items) {
    ...
    *items = NULL;
    ...
    for (list = selected_paths; list; list = g_list_next(list)) {
      GtkTreeIter iter;
      GimpViewRenderer *renderer;
