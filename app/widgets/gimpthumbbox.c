...
static GimpProgress *gimp_thumb_box_progress_start(GimpProgress *progress,
...
static void gimp_thumb_box_thumb_state_notify(GimpThumbnail *thumb,
                                              GParamSpec *pspec,
                                              GimpThumbBox *box);
static void gimp_thumb_box_create_thumbnails(GimpThumbBox *box, gboolean force);
static void gimp_thumb_box_create_thumbnail(GimpThumbBox *box, GFile *file,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpThumbBox, gimp_thumb_box, GTK_TYPE_FRAME,
...
static void gimp_header_class_init(GimpHeaderClass *klass) {
  gtk_widget_class_set_css_name(GTK_WIDGET_CLASS(klass), "header");
}

static void gimp_header_init(GimpHeader *header) {
...
GtkWidget *gimp_thumb_box_new(GimpContext *context) {
  ...
  g_signal_connect(ebox, "button-press-event",
                   G_CALLBACK(gimp_thumb_box_ebox_button_press), box);
  ...
  g_signal_connect(box->imagefile, "info-changed",
                   G_CALLBACK(gimp_thumb_box_imagefile_info_changed), box);

  g_signal_connect(gimp_imagefile_get_thumbnail(box->imagefile),
                   "notify::thumb-state",
                   G_CALLBACK(gimp_thumb_box_thumb_state_notify), box);

  gimp_view_renderer_get_frame_size(&h, &v);

  box->preview = gimp_view_new(
      context, GIMP_VIEWABLE(box->imagefile),
      /* add padding for the shadow frame */
      context->gimp->config->thumbnail_size + MAX(h, v), 0, FALSE);
  ...
  g_signal_connect(box->preview, "clicked",
                   G_CALLBACK(gimp_thumb_box_thumbnail_clicked), box);
  ...
  gimp_label_set_attributes(GTK_LABEL(box->filename), PANGO_ATTR_STYLE,
  ...
  gimp_label_set_attributes(GTK_LABEL(box->info), PANGO_ATTR_SCALE,
                            PANGO_SCALE_SMALL, -1);
...
void gimp_thumb_box_take_file(GimpThumbBox *box, GFile *file) {
...
void gimp_thumb_box_take_files(GimpThumbBox *box, GSList *files) {
  g_return_if_fail(GIMP_IS_THUMB_BOX(box));
...
static void gimp_thumb_box_thumbnail_clicked(GtkWidget *widget,
                                             GdkModifierType state,
                                             GimpThumbBox *box) {
  gimp_thumb_box_create_thumbnails(
      box, (state & gimp_get_toggle_behavior_mask()) ? TRUE : FALSE);
}
...
static void gimp_thumb_box_thumb_state_notify(GimpThumbnail *thumb,
  ...
  case GIMP_THUMB_STATE_OLD:
    box->idle_id = g_idle_add_full(
        G_PRIORITY_LOW, (GSourceFunc)gimp_thumb_box_auto_thumbnail, box, NULL);
    break;
...
static void gimp_thumb_box_create_thumbnails(GimpThumbBox *box,
  ...
  if (n_files > 1) {
    ...
    for (list = box->files->next, i = 1; list; list = g_slist_next(list), i++) {
      ...
      gimp_thumb_box_create_thumbnail(
          box, list->data, gimp->config->thumbnail_size, force, progress);
  ...
  if (box->files) {
    gimp_thumb_box_create_thumbnail(
        box, box->files->data, gimp->config->thumbnail_size, force, progress);
...
static void gimp_thumb_box_create_thumbnail(GimpThumbBox *box, GFile *file,
  ...
  if (force || (gimp_thumbnail_peek_thumb(thumb, (GimpThumbSize)size) <
                    GIMP_THUMB_STATE_FAILED &&
                !gimp_thumbnail_has_failed(thumb))) {
    GError *error = NULL;

    if (!gimp_imagefile_create_thumbnail(box->imagefile, box->context, progress,
                                         size, !force, &error)) {
      gimp_message_literal(box->context->gimp, G_OBJECT(progress),
...
static gboolean gimp_thumb_box_auto_thumbnail(GimpThumbBox *box) {
  ...
  case GIMP_THUMB_STATE_OLD:
    if (thumb->image_filesize < gimp->config->thumbnail_filesize_limit &&
        !gimp_thumbnail_has_failed(thumb) &&
        gimp_plug_in_manager_file_procedure_find_by_extension(
      ...
      if (thumb->image_filesize > 0) {
        ...
        gtk_label_set_text(GTK_LABEL(box->info), _("Creating preview..."));
      }

      gimp_imagefile_create_thumbnail_weak(box->imagefile, box->context,
                                           GIMP_PROGRESS(box),
                                           gimp->config->thumbnail_size, TRUE);
    }
    break;
