...
#define LOW_SWAP_SPACE_WARNING_ON  /* swap occupied is above */                \
  0.90                             /* of swap limit */
#define LOW_SWAP_SPACE_WARNING_OFF /* swap occupied is below */                \
  0.85                             /* of swap limit */
...
typedef void (*VariableFunc)(GimpDashboard *dashboard, Variable variable);
...
static void gimp_dashboard_set_aux_info(GimpDocked *docked, GList *aux_info);
static GList *gimp_dashboard_get_aux_info(GimpDocked *docked);

static gboolean gimp_dashboard_group_expander_button_press(
    GimpDashboard *dashboard, GdkEventButton *bevent, GtkWidget *widget);
...
static void gimp_dashboard_sample_object(GimpDashboard *dashboard,
                                         GObject *object, Variable variable);

static void gimp_dashboard_update_groups(GimpDashboard *dashboard);
static void gimp_dashboard_update_group(GimpDashboard *dashboard, Group group);
...
static void gimp_dashboard_label_set_text(GtkLabel *label, const gchar *text);
...
static const VariableInfo variables[] = {
    ...
    [VARIABLE_CACHE_OCCUPIED] = {.name = "cache-occupied",
    ...
    [VARIABLE_CACHE_HIT_MISS] = {.name = "cache-hit-miss",
    ...
    [VARIABLE_SWAP_OCCUPIED] = {.name = "swap-occupied",
                                .title = NC_("dashboard-variable", "Occupied"),
    ...
    [VARIABLE_SWAP_SIZE] = {.name = "swap-size",
    ...
    [VARIABLE_SWAP_COMPRESSION] = {.name = "swap-compression",
                                   .title =
                                       NC_("dashboard-variable", "Compression"),
    ...
    [VARIABLE_CPU_USAGE] = {.name = "cpu-usage",
    ...
    [VARIABLE_CPU_ACTIVE] = {.name = "cpu-active",
    ...
    [VARIABLE_MEMORY_USED] = {.name = "memory-used",
                              .title = NC_("dashboard-variable", "Used"),
                              .description =
                                  N_("Amount of memory used by the process"),
                              .type = VARIABLE_TYPE_SIZE,
    ...
    [VARIABLE_MEMORY_SIZE] = {.name = "memory-size",
                              .title = NC_("dashboard-variable", "Size"),
    ...
    [VARIABLE_MIPMAPED] = {.name = "mipmapped",
                           .title = NC_("dashboard-variable", "Mipmapped"),
                           .description =
                               N_("Total size of processed mipmapped data"),
                           .type = VARIABLE_TYPE_SIZE,
                           .sample_func = gimp_dashboard_sample_gegl_stats,
                           .data = "zoom-total"},
    ...
    [VARIABLE_ACTIVE_THREADS] = {.name = "active-threads",
                                 .title = NC_("dashboard-variable", "Active"),
                                 .description =
                                     N_("Number of active worker threads"),
                                 .type = VARIABLE_TYPE_INTEGER,
                                 .sample_func =
                                     gimp_dashboard_sample_gegl_stats,
                                 .data = "active-threads"},
    ...
    [VARIABLE_TILE_ALLOC_TOTAL] = {.name = "tile-alloc-total",
                                   .title = NC_("dashboard-variable", "Tile"),
                                   .description =
                                       N_("Total size of tile memory"),
                                   .type = VARIABLE_TYPE_SIZE,
    ...
    [VARIABLE_SCRATCH_TOTAL] = {.name = "scratch-total",
                                .title = NC_("dashboard-variable", "Scratch"),
                                .description =
                                    N_("Total size of scratch memory"),
                                .type = VARIABLE_TYPE_SIZE,
                                .sample_func = gimp_dashboard_sample_gegl_stats,
                                .data = "scratch-total"},
...
static const GroupInfo groups[] = {
         ...
         .fields =
             (const FieldInfo[]){
                 ...
                 {.variable = VARIABLE_CACHE_MAXIMUM,
                  ...
                  .meter_value = 1},
                 {.variable = VARIABLE_CACHE_LIMIT, .default_active = TRUE},

                 {VARIABLE_SEPARATOR},

                 {.variable = VARIABLE_CACHE_COMPRESSION,
                  .default_active = FALSE},
                 {.variable = VARIABLE_CACHE_HIT_MISS, .default_active = FALSE},

                 {}}},

    /* swap group */
    [GROUP_SWAP] = {.name = "swap",
                    ...
                    .meter_led =
                        (const Variable[]){VARIABLE_SWAP_QUEUE_FULL,
                                           VARIABLE_SWAP_READ_THROUGHPUT,
                                           VARIABLE_SWAP_WRITE_THROUGHPUT,
    ...
    [GROUP_CPU] = {.name = "cpu",
                   ...
                   .has_meter = TRUE,
                   .meter_led = (const Variable[]){VARIABLE_CPU_ACTIVE,
    ...
    [GROUP_MISC] =
        {.name = "misc",
         ...
         .description =
             N_("Miscellaneous information"),
         .default_active = FALSE,
         .default_expanded = FALSE,
         .has_meter = FALSE,
         .fields =
             (const FieldInfo[]){
                 {.variable = VARIABLE_MIPMAPED, .default_active = TRUE},
                 {.variable = VARIABLE_ASSIGNED_THREADS,
                  .default_active = TRUE},
                 {.variable = VARIABLE_ACTIVE_THREADS, .default_active = TRUE},
                 {.variable = VARIABLE_ASYNC_RUNNING, .default_active = TRUE},
                 {.variable = VARIABLE_TILE_ALLOC_TOTAL,
                  .default_active = TRUE},
                 {.variable = VARIABLE_SCRATCH_TOTAL, .default_active = TRUE},
                 {.variable = VARIABLE_TEMP_BUF_TOTAL, .default_active = TRUE},

                 {}}},
};

G_DEFINE_TYPE_WITH_CODE(GimpDashboard, gimp_dashboard, GIMP_TYPE_EDITOR,
                        G_ADD_PRIVATE(GimpDashboard) G_IMPLEMENT_INTERFACE(
...
static void gimp_dashboard_class_init(GimpDashboardClass *klass) {
...
static void gimp_dashboard_init(GimpDashboard *dashboard) {
  ...
  gtk_widget_style_get(GTK_WIDGET(dashboard), "content-spacing",
                       &content_spacing, NULL);
  ...
  for (group = FIRST_GROUP; group < N_GROUPS; group++) {
    ...
    for (field = 0; group_info->fields[field].variable; field++) {
      ...
      group_data->n_fields++;
      group_data->n_meter_values =
          MAX(group_data->n_meter_values, field_info->meter_value);
    }
    ...
    g_object_set_data(G_OBJECT(expander), "gimp-dashboard-group",
                      GINT_TO_POINTER(group));
    g_signal_connect_swapped(
        expander, "button-press-event",
        G_CALLBACK(gimp_dashboard_group_expander_button_press), dashboard);

    /* group expander label box */
    hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    gimp_help_set_help_data(hbox, g_dgettext(NULL, group_info->description),
                            NULL);
    ...
    label =
        gtk_label_new(g_dpgettext2(NULL, "dashboard-group", group_info->title));
    gtk_label_set_xalign(GTK_LABEL(label), 0.0);
    gimp_label_set_attributes(GTK_LABEL(label), PANGO_ATTR_WEIGHT,
                              PANGO_WEIGHT_BOLD, -1);
    ...
    g_object_bind_property(expander, "expanded", label, "visible",
                           G_BINDING_SYNC_CREATE | G_BINDING_INVERT_BOOLEAN);

    /* group expander menu button */
    button = gtk_button_new();
    group_data->menu_button = GTK_BUTTON(button);
    gimp_help_set_help_data(button, _("Select fields"), NULL);
    ...
    image =
        gtk_image_new_from_icon_name(GIMP_ICON_MENU_LEFT, GTK_ICON_SIZE_MENU);
    ...
    for (field = 0; field < group_data->n_fields; field++) {
      ...
      if (field_info->variable != VARIABLE_SEPARATOR) {
        ...
        item = gtk_check_menu_item_new_with_label(g_dpgettext2(
            NULL, "dashboard-variable",
            field_info->title ? field_info->title : variable_info->title));
        field_data->menu_item = GTK_CHECK_MENU_ITEM(item);
        gimp_help_set_help_data(
            item, g_dgettext(NULL, variable_info->description), NULL);

        g_object_set_data(G_OBJECT(item), "gimp-dashboard-group",
                          GINT_TO_POINTER(group));
        g_object_set_data(G_OBJECT(item), "gimp-dashboard-field",
                          GINT_TO_POINTER(field));
        g_signal_connect_swapped(
            item, "toggled", G_CALLBACK(gimp_dashboard_field_menu_item_toggled),
            dashboard);
    ...
    if (group_info->has_meter) {
      ...
      gimp_help_set_help_data(meter, g_dgettext(NULL, group_info->description),
                              NULL);
      ...
      for (field = 0; field < group_data->n_fields; field++) {
        ...
        if (field_info->meter_value) {
          ...
          if (gimp_dashboard_field_use_meter_underlay(group, field)) {
            gimp_meter_set_value_show_in_gauge(
                GIMP_METER(meter), field_info->meter_value - 1, FALSE);
   ...
   */
  priv->thread =
      g_thread_new("dashboard", (GThreadFunc)gimp_dashboard_sample, dashboard);
}
...
static void gimp_dashboard_constructed(GObject *object) {
  ...
  for (group = FIRST_GROUP; group < N_GROUPS; group++) {
    ...
    g_object_set_data(G_OBJECT(action), "gimp-dashboard-group",
                      GINT_TO_POINTER(group));
  ...
  button = gimp_editor_add_action_button(
      GIMP_EDITOR(dashboard), "dashboard", "dashboard-log-add-marker",
      "dashboard-log-add-empty-marker", gimp_get_extend_selection_mask(), NULL);

  action =
      gimp_action_group_get_action(action_group, "dashboard-log-add-marker");
  g_object_bind_property(action, "sensitive", button, "visible",
  ...
  action = gimp_action_group_get_action(action_group, "dashboard-reset");
  g_object_bind_property(action, "sensitive", button, "visible",
                         G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE);
        ...
        for (field = 0; field < group_data->n_fields; field++) {
          ...
          if (field_info->variable != VARIABLE_SEPARATOR) {
            const VariableInfo *variable_info =
                &variables[field_info->variable];

            name = g_strdup_printf("%s-%s-active", group_info->name,
                                   variable_info->name);

            if (!strcmp(aux->name, name)) {
              gboolean active = !strcmp(aux->value, "yes");

              gimp_dashboard_field_set_active(dashboard, group, field, active);
...
static gboolean gimp_dashboard_group_expander_button_press(
  ...
  group = GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(widget), "gimp-dashboard-group"));
  group_data = &priv->groups[group];

  gtk_widget_get_allocation(GTK_WIDGET(group_data->expander),
                            &expander_allocation);
  gtk_widget_get_allocation(GTK_WIDGET(group_data->menu_button), &allocation);

  allocation.x -= expander_allocation.x;
  allocation.y -= expander_allocation.y;

  if (bevent->button == 1 && bevent->x >= allocation.x &&
      bevent->x < allocation.x + allocation.width &&
      bevent->y >= allocation.y &&
      bevent->y < allocation.y + allocation.height) {
    gtk_menu_popup_at_widget(
        group_data->menu, GTK_WIDGET(group_data->menu_button), GDK_GRAVITY_WEST,
        GDK_GRAVITY_NORTH_EAST, (GdkEvent *)bevent);
...
static void gimp_dashboard_group_action_toggled(GimpDashboard *dashboard,
  ...
  group = GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(action), "gimp-dashboard-group"));
  group_data = &priv->groups[group];
...
static void gimp_dashboard_field_menu_item_toggled(GimpDashboard *dashboard,
  ...
  group = GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(item), "gimp-dashboard-group"));
  group_data = &priv->groups[group];

  field = GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(item), "gimp-dashboard-field"));
...
static gpointer gimp_dashboard_sample(GimpDashboard *dashboard) {
  ...
  while (!priv->quit) {
    ...
    if (priv->log_output) {
      sample_interval = G_TIME_SPAN_SECOND / priv->log_params.sample_frequency;
    } else {
    ...
    if (!g_cond_wait_until(&priv->cond, &priv->mutex, end_time) ||
      ...
      for (variable = FIRST_VARIABLE; variable < N_VARIABLES; variable++) {
        ...
        variables_changed =
            variables_changed ||
            memcmp(variable_data, &prev_variable_data, sizeof(VariableData));
      }
      ...
      /* update gui */
      if (priv->update_now || !priv->log_output ||
        ...
        for (group = FIRST_GROUP; group < N_GROUPS; group++) {
          ...
          for (field = 0; field < group_data->n_fields; field++) {
            ...
            if (field_info->meter_value) {
              ...
              value = gimp_dashboard_variable_to_double(dashboard, variable);

              if (value &&
                  gimp_dashboard_field_use_meter_underlay(group, field)) {
                value = G_MAXDOUBLE;
              }
        ...
        if (variables_changed) {
          ...
          if (!priv->update_idle_id &&
            ...
            priv->update_idle_id = g_idle_add_full(
                G_PRIORITY_DEFAULT, (GSourceFunc)gimp_dashboard_update,
                dashboard, NULL);
          }
          ...
          if (priv->low_swap_space_warning &&
            ...
            if (!seen_low_swap_space &&
              ...
              if (!priv->low_swap_space_idle_id) {
                priv->low_swap_space_idle_id = g_idle_add_full(
                    G_PRIORITY_HIGH, (GSourceFunc)gimp_dashboard_low_swap_space,
                    dashboard, NULL);
              }
              ...
              seen_low_swap_space = TRUE;
                       ...
                       swap_occupied <=
                           LOW_SWAP_SPACE_WARNING_OFF * swap_limit) {
              if (priv->low_swap_space_idle_id) {
                g_source_remove(priv->low_swap_space_idle_id);
  ...
  if (priv->gimp) {
    ...
    for (field = 0; field < priv->groups[GROUP_SWAP].n_fields; field++) {
      ...
      if (field_info->variable == VARIABLE_SWAP_OCCUPIED ||
        ...
        gimp_dashboard_field_set_active(dashboard, GROUP_SWAP, field, TRUE);
      }
    }
    ...
    gimp_window_strategy_show_dockable_dialog(
        GIMP_WINDOW_STRATEGY(gimp_get_window_strategy(priv->gimp)), priv->gimp,
        gimp_dialog_factory_get_singleton(), monitor, "gimp-dashboard");
...
#define CALL_FUNC(result_type) (((result_type(*)(void))variable_info->data)())
...
static void gimp_dashboard_sample_variable_changed(GimpDashboard *dashboard,
  ...
  gpointer prev_value = gimp_dashboard_variable_get_data(
      dashboard, variable, sizeof(var_data->value));

  if (var_data->available) {
    variable_data->available = TRUE;
    variable_data->value.boolean =
        memcmp(&var_data->value, prev_value, sizeof(var_data->value)) != 0;
...
gimp_dashboard_sample_variable_rate_of_change(GimpDashboard *dashboard,
  ...
  const VariableData *var_data = &priv->variables[var];
  Data *data =
      gimp_dashboard_variable_get_data(dashboard, variable, sizeof(Data));
  gint64 time;
...
static void gimp_dashboard_sample_swap_limit(GimpDashboard *dashboard,
  ...
  if (time - data->last_check_time >= G_TIME_SPAN_SECOND) {
    ...
    if (swap_dir) {
      ...
      if (info) {
        data->free_space = g_file_info_get_attribute_uint64(
...
static void gimp_dashboard_sample_cpu_active(GimpDashboard *dashboard,
  ...
  if (priv->variables[VARIABLE_CPU_USAGE].available) {
    if (!data->active) {
      active = priv->variables[VARIABLE_CPU_USAGE].value.percentage *
...
static void gimp_dashboard_sample_object(GimpDashboard *dashboard,
  ...
  case VARIABLE_TYPE_BOOLEAN:
    if (g_object_class_find_property(klass, variable_info->data)) {
      ...
      g_object_get(object, variable_info->data, &variable_data->value.boolean,
                   NULL);
    }
    break;
  ...
  case VARIABLE_TYPE_INTEGER:
    if (g_object_class_find_property(klass, variable_info->data)) {
      ...
      g_object_get(object, variable_info->data, &variable_data->value.integer,
                   NULL);
    }
    break;
  ...
  case VARIABLE_TYPE_SIZE:
    if (g_object_class_find_property(klass, variable_info->data)) {
      ...
      g_object_get(object, variable_info->data, &variable_data->value.size,
                   NULL);
    }
    break;
  ...
  case VARIABLE_TYPE_SIZE_RATIO: {
    ...
    if (g_object_class_find_property(klass, antecedent) &&
      ...
      g_object_get(object, antecedent,
                   &variable_data->value.size_ratio.antecedent, consequent,
                   &variable_data->value.size_ratio.consequent, NULL);
    }
  } break;
  ...
  case VARIABLE_TYPE_INT_RATIO: {
    ...
    if (g_object_class_find_property(klass, antecedent) &&
      ...
      g_object_get(object, antecedent,
                   &variable_data->value.int_ratio.antecedent, consequent,
                   &variable_data->value.int_ratio.consequent, NULL);
    }
  } break;
  ...
  case VARIABLE_TYPE_PERCENTAGE:
    if (g_object_class_find_property(klass, variable_info->data)) {
      ...
      g_object_get(object, variable_info->data,
                   &variable_data->value.percentage, NULL);
    }
    break;
  ...
  case VARIABLE_TYPE_DURATION:
    if (g_object_class_find_property(klass, variable_info->data)) {
      ...
      g_object_get(object, variable_info->data, &variable_data->value.duration,
                   NULL);
    }
    break;
  ...
  case VARIABLE_TYPE_RATE_OF_CHANGE:
    if (g_object_class_find_property(klass, variable_info->data)) {
      ...
      g_object_get(object, variable_info->data,
                   &variable_data->value.rate_of_change, NULL);
    }
    break;
  }
}
...
static void gimp_dashboard_update_group(GimpDashboard *dashboard, Group group) {
  ...
  gtk_widget_set_visible(GTK_WIDGET(group_data->expander), group_data->active);
  ...
  for (field = 0; field < group_data->n_fields; field++) {
    ...
    if (field_info->variable != VARIABLE_SEPARATOR) {
      if (group_info->has_meter && field_info->meter_value) {
        gimp_meter_set_value_active(
            group_data->meter, field_info->meter_value - 1, field_data->active);
      }
  ...
  for (field = 0; field < group_data->n_fields; field++) {
    ...
    if (field_info->variable != VARIABLE_SEPARATOR) {
      ...
      if (add_separator) {
        ...
        gtk_grid_attach(group_data->grid, separator, 0, n_rows, 3, 1);
      ...
      if (group_info->has_meter && field_info->meter_value) {
        color_area =
            gimp_color_area_new(&variable_info->color, GIMP_COLOR_AREA_FLAT, 0);
        gimp_help_set_help_data(color_area, description, NULL);
        gtk_widget_set_size_request(color_area, 5, 5);
        gtk_widget_set_valign(color_area, GTK_ALIGN_CENTER);
        gtk_grid_attach(group_data->grid, color_area, 0, n_rows, 1, 1);
        gtk_widget_show(color_area);
      }

      str = g_strdup_printf("%s:", g_dpgettext2(NULL, "dashboard-variable",
                                                field_info->title
                                                    ? field_info->title
                                                    : variable_info->title));

      label = gtk_label_new(str);
      gimp_help_set_help_data(label, description, NULL);
      gtk_label_set_xalign(GTK_LABEL(label), 0.0);
      gtk_grid_attach(group_data->grid, label, 1, n_rows, 1, 1);
      gtk_widget_show(label);
      ...
      gtk_grid_attach(group_data->grid, label, 2, n_rows, 1, 1);
      gtk_widget_show(label);
...
static void gimp_dashboard_update_group_values(GimpDashboard *dashboard,
  ...
  if (group_info->has_meter) {
    if (group_info->meter_limit) {
      const VariableData *variable_data =
          &priv->variables[group_info->meter_limit];
  ...
  for (field = 0; field < group_data->n_fields; field++) {
    ...
    if (field_data->active) {
      ...
      text = gimp_dashboard_field_to_string(dashboard, group, field, TRUE);
      ...
      if (field_info->show_in_header) {
        text = gimp_dashboard_field_to_string(dashboard, group, field, FALSE);
  ...
  if (active != group_data->active) {
    ...
    if (group_data->action) {
      g_signal_handlers_block_by_func(
          group_data->action, gimp_dashboard_group_action_toggled, dashboard);
...
static gpointer gimp_dashboard_variable_get_data(GimpDashboard *dashboard,
  ...
  if (variable_data->data_size != size) {
    ...
    if (variable_data->data_size < size) {
      memset((guint8 *)variable_data->data + variable_data->data_size, 0,
             size - variable_data->data_size);
    }
...
static gchar *gimp_dashboard_field_to_string(GimpDashboard *dashboard,
  ...
  if (variable_data->available) {
    ...
    case VARIABLE_TYPE_INT_RATIO: {
      ...
      if (max) {
        str = g_strdup_printf("%g:%g", RINT(100.0 * antecedent) / 100.0,
    ...
    case VARIABLE_TYPE_PERCENTAGE:
      str = g_strdup_printf(
          "%d%%", SIGNED_ROUND(100.0 * variable_data->value.percentage));
      static_str = FALSE;
      show_limit = FALSE;
      break;
    ...
    case VARIABLE_TYPE_RATE_OF_CHANGE:
       ...
       */
      str = g_strdup_printf(_("%g/s"), variable_data->value.rate_of_change);
      static_str = FALSE;
      break;
    }

    if (show_limit && variable_data->available && field_info->meter_value &&
        !field_info->meter_variable && group_data->limit) {
      gdouble value;
      gchar *tmp;

      value =
          gimp_dashboard_variable_to_double(dashboard, field_info->variable);

      if (full) {
        tmp = g_strdup_printf("%s (%d%%)", str,
                              SIGNED_ROUND(100.0 * value / group_data->limit));
      } else {
      ...
      str = tmp;
      static_str = FALSE;
    } else if (full && field_info->meter_variable &&
...
static gboolean gimp_dashboard_log_printf(GimpDashboard *dashboard,
  ...
  result = g_output_stream_vprintf(priv->log_output, NULL, NULL,
                                   &priv->log_error, format, args);
...
#define FLUSH()                                                                \
  G_STMT_START {                                                               \
    if (!g_output_stream_write_all(priv->log_output, buffer, i, NULL, NULL,    \
                                   &priv->log_error)) {                        \
      return FALSE;                                                            \
    }                                                                          \
                                                                               \
...
#define NONEMPTY()                                                             \
  ...
  if (priv->log_n_samples == 0 || variables_changed) {
    ...
    gimp_dashboard_log_printf(dashboard, "<vars>\n");
    ...
    for (variable = FIRST_VARIABLE; variable < N_VARIABLES; variable++) {
      ...
      if (priv->log_n_samples > 0 &&
          !memcmp(variable_data, log_variable_data, sizeof(VariableData))) {
        continue;
      }
...
#define LOG_VAR(format, ...)                                                   \
  gimp_dashboard_log_printf(dashboard, "<%s>" format "</%s>\n",                \
                            variable_info->name, __VA_ARGS__,                  \
                            variable_info->name)
...
#define LOG_VAR_FLOAT(value)                                                   \
  G_STMT_START {                                                               \
    gchar buffer[G_ASCII_DTOSTR_BUF_SIZE];                                     \
                                                                               \
    LOG_VAR("%s", g_ascii_dtostr(buffer, sizeof(buffer), value));              \
  }                                                                            \
  G_STMT_END

        switch (variable_info->type) {
        case VARIABLE_TYPE_BOOLEAN:
          LOG_VAR("%d", variable_data->value.boolean);
          break;

        case VARIABLE_TYPE_INTEGER:
          LOG_VAR("%d", variable_data->value.integer);
          break;

        case VARIABLE_TYPE_SIZE:
          LOG_VAR("%llu", (unsigned long long)variable_data->value.size);
          break;
        ...
        case VARIABLE_TYPE_INT_RATIO:
          LOG_VAR("%d:%d", variable_data->value.int_ratio.antecedent,
                  variable_data->value.int_ratio.consequent);
          break;

        case VARIABLE_TYPE_PERCENTAGE:
          LOG_VAR_FLOAT(variable_data->value.percentage);
          break;

        case VARIABLE_TYPE_DURATION:
          LOG_VAR_FLOAT(variable_data->value.duration);
          break;

        case VARIABLE_TYPE_RATE_OF_CHANGE:
          LOG_VAR_FLOAT(variable_data->value.rate_of_change);
          break;
        }
...
#undef LOG_VAR_FLOAT
        ...
        gimp_dashboard_log_printf(dashboard, "<%s />\n", variable_info->name);
      }
    }
...
#define BACKTRACE_NONEMPTY()                                                   \
  G_STMT_START {                                                               \
    if (backtrace_empty) {                                                     \
      NONEMPTY();                                                              \
                                                                               \
      gimp_dashboard_log_printf(dashboard, "<backtrace>\n");                   \
                                                                               \
  ...
  G_STMT_END
    ...
    if (priv->log_backtrace) {
      ...
      for (thread = 0; thread < n_threads; thread++) {
        ...
        thread_id = gimp_backtrace_get_thread_id(priv->log_backtrace, thread);

        if (gimp_backtrace_find_thread_by_id(backtrace, thread_id, thread) <
            0) {
          ...
          thread_name =
              gimp_backtrace_get_thread_name(priv->log_backtrace, thread);

          gimp_dashboard_log_printf(dashboard, "<thread id=\"%llu\"",
                                    (unsigned long long)thread_id);

          if (thread_name) {
            gimp_dashboard_log_printf(dashboard, " name=\"");
    ...
    for (thread = 0; thread < n_threads; thread++) {
      ...
      if (priv->log_backtrace) {
        ...
        if (other_thread >= 0) {
          ...
          last_running = gimp_backtrace_is_thread_running(priv->log_backtrace,
                                                          other_thread);
          last_n_frames =
              gimp_backtrace_get_n_frames(priv->log_backtrace, other_thread);

          n = MIN(n_frames, last_n_frames);

          for (i = 0; i < n; i++) {
            if (gimp_backtrace_get_frame_address(backtrace, thread, i) !=
          ...
          for (i = 0; i < n; i++) {
            if (gimp_backtrace_get_frame_address(backtrace, thread, -i - 1) !=
                gimp_backtrace_get_frame_address(priv->log_backtrace,
                                                 other_thread, -i - 1)) {
              break;
            }
      ...
      if (running == last_running && n_frames == last_n_frames &&
      ...
      gimp_dashboard_log_printf(dashboard, " running=\"%d\"", running);

      if (n_head > 0) {
        gimp_dashboard_log_printf(dashboard, " head=\"%d\"", n_head);
      }

      if (n_tail > 0) {
        gimp_dashboard_log_printf(dashboard, " tail=\"%d\"", n_tail);
      }
      ...
      if (n_frames == 0 || n_head + n_tail < n_frames) {
        ...
        for (frame = n_head; frame < n_frames - n_tail; frame++) {
          ...
          address = gimp_backtrace_get_frame_address(backtrace, thread, frame);

          gimp_dashboard_log_printf(dashboard, "<frame address=\"0x%llx\" />\n",
                                    (unsigned long long)address);

          if (g_hash_table_add(priv->log_addresses, (gpointer)address) &&
              priv->log_params.progressive) {
            if (!addresses) {
              addresses = g_array_new(FALSE, FALSE, sizeof(guintptr));
            }
        ...
        gimp_dashboard_log_printf(dashboard, "</thread>\n");
      } else {
        gimp_dashboard_log_printf(dashboard, " />\n");
      }
    }
...
#undef BACKTRACE_NONEMPTY
  ...
  if (empty) {
    ...
    gimp_dashboard_log_printf(dashboard, "</sample>\n");
  }

  if (addresses) {
    gimp_dashboard_log_write_address_map(dashboard, (guintptr *)addresses->data,
                                         addresses->len, NULL);
...
static void gimp_dashboard_log_update_highlight(GimpDashboard *dashboard) {
  ...
  gtk_widget_style_get(GTK_WIDGET(dashboard), "button-relief", &default_relief,
                       NULL);
...
static gint gimp_dashboard_log_compare_addresses(gconstpointer a1,
                                                 gconstpointer a2) {
  ...
  G_STMT_END
      ...
      if (n == 0 || strcmp(info->object_name, prev_info->object_name)) {
        ...
        if (info->object_name[0]) {
          gimp_dashboard_log_printf(dashboard, "<object>");
          gimp_dashboard_log_print_escaped(dashboard, info->object_name);
          gimp_dashboard_log_printf(dashboard, "</object>\n");
        } else {
          gimp_dashboard_log_printf(dashboard, "<object />\n");
        }
      ...
      if (n == 0 || strcmp(info->symbol_name, prev_info->symbol_name)) {
        ...
        if (info->symbol_name[0]) {
          gimp_dashboard_log_printf(dashboard, "<symbol>");
          gimp_dashboard_log_print_escaped(dashboard, info->symbol_name);
          gimp_dashboard_log_printf(dashboard, "</symbol>\n");
        } else {
          gimp_dashboard_log_printf(dashboard, "<symbol />\n");
        }
      ...
      if (n == 0 || info->symbol_address != prev_info->symbol_address) {
        ...
        if (info->symbol_address) {
          gimp_dashboard_log_printf(dashboard, "<base>0x%llx</base>\n",
                                    (unsigned long long)info->symbol_address);
        } else {
          gimp_dashboard_log_printf(dashboard, "<base />\n");
        }
      }
      ...
      if (n == 0 || strcmp(info->source_file, prev_info->source_file)) {
        ...
        if (info->source_file[0]) {
          gimp_dashboard_log_printf(dashboard, "<source>");
          gimp_dashboard_log_print_escaped(dashboard, info->source_file);
          gimp_dashboard_log_printf(dashboard, "</source>\n");
        } else {
          gimp_dashboard_log_printf(dashboard, "<source />\n");
        }
      ...
      if (n == 0 || info->source_line != prev_info->source_line) {
        ...
        if (info->source_line) {
          gimp_dashboard_log_printf(dashboard, "<line>%d</line>\n",
                                    info->source_line);
        } else {
          gimp_dashboard_log_printf(dashboard, "<line />\n");
        }
      }
      ...
      if (empty) {
        ...
        gimp_dashboard_log_printf(dashboard, "</address>\n");
      }
...
gimp_dashboard_log_write_global_address_map(GimpAsync *async,
  ...
  if (n_addresses > 0) {
    ...
    for (iter = g_hash_table_get_keys(priv->log_addresses), i = 0; iter;
         iter = g_list_next(iter), i++) {
      addresses[i] = (guintptr)iter->data;
    }

    gimp_dashboard_log_write_address_map(dashboard, addresses, n_addresses,
                                         async);
...
static gchar *gimp_dashboard_format_value(VariableType type, gdouble value) {
  ...
  case VARIABLE_TYPE_INT_RATIO:
    if (isfinite(value)) {
      ...
      return g_strdup_printf("%g:%g", RINT(100.0 * antecedent) / 100.0,
                             RINT(100.0 * consequent) / 100.0);
    } else if (isinf(value)) {
      return g_strdup("1:0");
    }
  ...
  case VARIABLE_TYPE_DURATION:
    return g_strdup_printf("%02d:%02d:%04.1f", (gint)floor(value / 3600.0),
...
GtkWidget *gimp_dashboard_new(Gimp *gimp, GimpMenuFactory *menu_factory) {
  GimpDashboard *dashboard;

  dashboard = g_object_new(GIMP_TYPE_DASHBOARD, "menu-factory", menu_factory,
                           "menu-identifier", "<Dashboard>", "ui-path",
                           "/dashboard-popup", NULL);
...
gboolean
gimp_dashboard_log_start_recording(GimpDashboard *dashboard, GFile *file,
  ...
  priv->log_params.sample_frequency =
      CLAMP(priv->log_params.sample_frequency, LOG_SAMPLE_FREQUENCY_MIN,
            LOG_SAMPLE_FREQUENCY_MAX);

  g_mutex_lock(&priv->mutex);

  if (priv->log_params.progressive && g_file_query_exists(file, NULL) &&
  ...
  priv->log_output = G_OUTPUT_STREAM(
  ...
  gimp_dashboard_log_printf(dashboard,
                            ...
                            "</params>\n",
                            priv->log_params.sample_frequency, has_backtrace,
                            priv->log_params.messages,
                            priv->log_params.progressive);
  ...
  gimp_dashboard_log_printf(dashboard, "</gimp-version>\n");
  ...
  for (env = envp; *env; env++) {
    if (g_str_has_prefix(*env, "BABL_") || g_str_has_prefix(*env, "GEGL_") ||
      ...
      for (s = *env;
           s != delim && (g_ascii_isalnum(*s) || *s == '_' || *s == '-'); s++)
        ;
      ...
      *delim = '\0';
      ...
      gimp_dashboard_log_printf(dashboard, "<%s>", *env);
  ...
  gimp_dashboard_log_printf(dashboard, "</env>\n");
  ...
  for (i = 0; i < n_pspecs; i++) {
    ...
    if (g_value_transform(&value, &str_value)) {
      gimp_dashboard_log_printf(dashboard, "<%s>", pspec->name);
  ...
  gimp_dashboard_log_printf(dashboard, "</gegl-config>\n");
  ...
  for (variable = FIRST_VARIABLE; variable < N_VARIABLES; variable++) {
    ...
    gimp_dashboard_log_printf(dashboard, "<var name=\"%s\" type=\"%s\" desc=\"",
  ...
  if (priv->log_params.messages) {
    priv->log_log_handler = gimp_log_set_handler(
        TRUE, G_LOG_LEVEL_MASK | G_LOG_FLAG_FATAL | G_LOG_FLAG_RECURSION,
        (GLogFunc)gimp_dashboard_log_log_func, dashboard);
  }
...
void gimp_dashboard_set_update_interval(
    GimpDashboard *dashboard, GimpDashboardUpdateInteval update_interval) {
  GimpDashboardPrivate *priv;
...
void gimp_dashboard_set_history_duration(
    GimpDashboard *dashboard, GimpDashboardHistoryDuration history_duration) {
  GimpDashboardPrivate *priv;
...
void gimp_dashboard_set_low_swap_space_warning(
    GimpDashboard *dashboard, gboolean low_swap_space_warning) {
  GimpDashboardPrivate *priv;
...
gboolean gimp_dashboard_get_low_swap_space_warning(GimpDashboard *dashboard) {
  g_return_val_if_fail(GIMP_IS_DASHBOARD(dashboard),
                       DEFAULT_LOW_SWAP_SPACE_WARNING);

  return dashboard->priv->low_swap_space_warning;
}

void gimp_dashboard_menu_setup(GimpUIManager *manager, const gchar *ui_path) {
  guint merge_id;
  Group group;
