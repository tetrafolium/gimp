...
static void gimp_curve_view_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_curve_view_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);

static void gimp_curve_view_style_updated(GtkWidget *widget);
static gboolean gimp_curve_view_draw(GtkWidget *widget, cairo_t *cr);
...
static void gimp_curve_view_curve_dirty(GimpCurve *curve, GimpCurveView *view);
static void gimp_curve_view_curve_notify_n_points(GimpCurve *curve,
                                                  GParamSpec *pspec,
                                                  GimpCurveView *view);

static void gimp_curve_view_set_cursor(GimpCurveView *view, gdouble x,
                                       gdouble y);
static void gimp_curve_view_unset_cursor(GimpCurveView *view);

G_DEFINE_TYPE(GimpCurveView, gimp_curve_view, GIMP_TYPE_HISTOGRAM_VIEW)

#define parent_class gimp_curve_view_parent_class

static guint curve_view_signals[LAST_SIGNAL] = {0};

static void gimp_curve_view_class_init(GimpCurveViewClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_BASE_LINE,
      g_param_spec_boolean("base-line", NULL, NULL, TRUE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_GRID_ROWS,
      g_param_spec_int("grid-rows", NULL, NULL, 0, 100, 8,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_GRID_COLUMNS,
      g_param_spec_int("grid-columns", NULL, NULL, 0, 100, 8,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(object_class, PROP_X_AXIS_LABEL,
                                  g_param_spec_string("x-axis-label", NULL,
                                                      NULL, NULL,
                                                      GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_Y_AXIS_LABEL,
                                  g_param_spec_string("y-axis-label", NULL,
                                                      NULL, NULL,
                                                      GIMP_PARAM_READWRITE));

  curve_view_signals[SELECTION_CHANGED] = g_signal_new(
      "selection-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpCurveViewClass, selection_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  curve_view_signals[CUT_CLIPBOARD] =
      g_signal_new("cut-clipboard", G_TYPE_FROM_CLASS(klass),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(GimpCurveViewClass, cut_clipboard), NULL,
                   NULL, NULL, G_TYPE_NONE, 0);

  curve_view_signals[COPY_CLIPBOARD] =
      g_signal_new("copy-clipboard", G_TYPE_FROM_CLASS(klass),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(GimpCurveViewClass, copy_clipboard), NULL,
                   NULL, NULL, G_TYPE_NONE, 0);

  curve_view_signals[PASTE_CLIPBOARD] =
      g_signal_new("paste-clipboard", G_TYPE_FROM_CLASS(klass),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(GimpCurveViewClass, paste_clipboard), NULL,
                   NULL, NULL, G_TYPE_NONE, 0);
...
static void gimp_curve_view_init(GimpCurveView *view) {
  view->curve = NULL;
  view->selected = -1;
  view->offset_x = 0.0;
  view->offset_y = 0.0;
...
static void gimp_curve_view_draw_grid(GimpCurveView *view, cairo_t *cr,
...
static void gimp_curve_view_draw_point(GimpCurveView *view, cairo_t *cr, gint i,
                                       gint width, gint height, gint border) {
  gdouble x, y;
...
#define DIAMOND_RADIUS (G_SQRT2 * CIRCLE_RADIUS)
  ...
  case GIMP_CURVE_POINT_SMOOTH:
    cairo_move_to(cr, border + (gdouble)(width - 1) * x + CIRCLE_RADIUS,
                  border + (gdouble)(height - 1) * y);
    cairo_arc(cr, border + (gdouble)(width - 1) * x,
              border + (gdouble)(height - 1) * y, CIRCLE_RADIUS, 0, 2 * G_PI);
    break;

  case GIMP_CURVE_POINT_CORNER:
    cairo_move_to(cr, border + (gdouble)(width - 1) * x,
                  border + (gdouble)(height - 1) * y - DIAMOND_RADIUS);
    cairo_line_to(cr, border + (gdouble)(width - 1) * x + DIAMOND_RADIUS,
                  border + (gdouble)(height - 1) * y);
    cairo_line_to(cr, border + (gdouble)(width - 1) * x,
                  border + (gdouble)(height - 1) * y + DIAMOND_RADIUS);
    cairo_line_to(cr, border + (gdouble)(width - 1) * x - DIAMOND_RADIUS,
                  border + (gdouble)(height - 1) * y);
    cairo_close_path(cr);
...
static void gimp_curve_view_draw_curve(GimpCurveView *view, cairo_t *cr,
                                       GimpCurve *curve, gint width,
                                       gint height, gint border) {
  gdouble x, y;
  gint i;
...
static gboolean gimp_curve_view_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (gtk_widget_has_focus(widget)) {
    gtk_render_focus(style, cr, border - 2, border - 2, width + 4, height + 4);
  }
  ...
  if (view->x_axis_label) {
    ...
    cairo_move_to(cr, width - border - layout_x, height - border - layout_y);
  ...
  if (view->y_axis_label) {
    ...
    cairo_move_to(cr, 2 * border, 2 * border + layout_x);
  ...
  for (list = view->bg_curves; list; list = g_list_next(list)) {
    ...
    if (bg->color_set) {
      cairo_set_source_rgba(cr, bg->color.r, bg->color.g, bg->color.b, 0.5);
    } else {
      cairo_set_source_rgba(cr, fg_color.red, fg_color.green, fg_color.blue,
                            0.5);
    }

    gimp_curve_view_draw_curve(view, cr, bg->curve, width, height, border);
  }
  ...
  gimp_curve_view_draw_curve(view, cr, view->curve, width, height, border);
  ...
  if (gimp_curve_get_curve_type(view->curve) == GIMP_CURVE_SMOOTH) {
    ...
    if (view->selected != -1) {
      gimp_curve_view_draw_point(view, cr, view->selected, width, height,
                                 border);
      cairo_fill(cr);
    }
  }
  ...
  if (view->xpos >= 0.0) {
    ...
    /* draw the color line */
    cairo_move_to(cr, border + ROUND((gdouble)(width - 1) * view->xpos),
                  border + 1);
    cairo_line_to(cr, border + ROUND((gdouble)(width - 1) * view->xpos),
                  border + height - 1);
    cairo_stroke(cr);
    ...
    if (view->range_x_max == 255.0) {
      ...
      g_snprintf(buf, sizeof(buf), "x:%3d",
                 (gint)(view->xpos * (view->range_x_max - view->range_x_min) +
                        view->range_x_min));
    } else if (view->range_x_max == 100.0) {
      /*  and for 0..100  */

      g_snprintf(buf, sizeof(buf), "x:%0.2f",
                 view->xpos * (view->range_x_max - view->range_x_min) +
                     view->range_x_min);
    } else {
    ...
    cairo_move_to(cr, border + (gdouble)width * view->xpos + layout_x,
                  border + height - border - layout_y);
    pango_cairo_show_layout(cr, view->layout);
  }

  if (view->cursor_x >= 0.0 && view->cursor_x <= 1.0 && view->cursor_y >= 0.0 &&
      view->cursor_y <= 1.0) {
    gchar buf[32];
    gint w, h;
    ...
    if (view->range_x_max == 255.0 && view->range_y_max == 255.0) {
      /*  stupid heuristic: special-case for 0..255  */

      g_snprintf(
          buf, sizeof(buf), "x:%3d y:%3d",
          (gint)round(view->cursor_x * (view->range_x_max - view->range_x_min) +
                      view->range_x_min),
          (gint)round((1.0 - view->cursor_y) *
                          (view->range_y_max - view->range_y_min) +
                      view->range_y_min));
    } else if (view->range_x_max == 100.0 && view->range_y_max == 100.0) {
      /*  and for 0..100  */

      g_snprintf(buf, sizeof(buf), "x:%0.2f y:%0.2f",
                 view->cursor_x * (view->range_x_max - view->range_x_min) +
                     view->range_x_min,
                 (1.0 - view->cursor_y) *
                         (view->range_y_max - view->range_y_min) +
                     view->range_y_min);
    ...
    pango_layout_get_pixel_extents(view->cursor_layout, NULL,
                                   &view->cursor_rect);
    ...
    if (view->x_axis_label)
      x += border +
           view->cursor_rect.height; /* coincidentially the right value */
...
static void set_cursor(GimpCurveView *view, GdkCursorType new_cursor) {
...
static gboolean gimp_curve_view_button_press(GtkWidget *widget,
  ...
  case GIMP_CURVE_SMOOTH:
    point = gimp_curve_get_closest_point(
        curve, x, 1.0 - y, POINT_MAX_DISTANCE / MAX(width, height));
  ...
  case GIMP_CURVE_SMOOTH:
    ...
    } else /*  Else, drag the grabbed point  */
    {
      ...
      if (x > view->leftmost && x < view->rightmost) {
        if (view->selected < 0) {
          gimp_curve_view_set_selected(view,
                                       gimp_curve_add_point(curve, x, 1.0 - y));

          gimp_curve_set_point_type(curve, view->selected, view->point_type);
        } else {
...
static gboolean gimp_curve_view_key_press(GtkWidget *widget,
  ...
  if (!view->grabbed && curve &&
    ...
    case GDK_KEY_Up:
      if (y < 1.0) {
        y = y +
            (kevent->state & GDK_SHIFT_MASK ? (16.0 / 255.0) : (1.0 / 255.0));
...
static void gimp_curve_view_paste_clipboard(GimpCurveView *view) {
  ...
  if (copy) {
    gimp_config_copy(GIMP_CONFIG(copy), GIMP_CONFIG(view->curve), 0);
    g_object_unref(copy);
  }
}
...
void gimp_curve_view_set_curve(GimpCurveView *view, GimpCurve *curve,
                               const GimpRGB *color) {
  ...
  if (view->curve) {
    g_signal_handlers_disconnect_by_func(view->curve,
                                         gimp_curve_view_curve_dirty, view);
    g_signal_handlers_disconnect_by_func(
        view->curve, gimp_curve_view_curve_notify_n_points, view);
  ...
  if (view->curve) {
    ...
    g_signal_connect(view->curve, "dirty",
                     G_CALLBACK(gimp_curve_view_curve_dirty), view);
    g_signal_connect(view->curve, "notify::n-points",
                     G_CALLBACK(gimp_curve_view_curve_notify_n_points), view);
  }
...
void gimp_curve_view_add_background(GimpCurveView *view, GimpCurve *curve,
                                    const GimpRGB *color) {
  GList *list;
  BGCurve *bg;
  ...
  g_signal_connect(bg->curve, "dirty", G_CALLBACK(gimp_curve_view_curve_dirty),
                   view);
...
void gimp_curve_view_remove_background(GimpCurveView *view, GimpCurve *curve) {
  GList *list;
...
void gimp_curve_view_remove_all_backgrounds(GimpCurveView *view) {
  ...
  while (view->bg_curves) {
    ...
    g_signal_handlers_disconnect_by_func(bg->curve, gimp_curve_view_curve_dirty,
                                         view);
    g_object_unref(bg->curve);
...
void gimp_curve_view_set_selected(GimpCurveView *view, gint selected) {
  g_return_if_fail(GIMP_IS_CURVE_VIEW(view));
...
void gimp_curve_view_set_range_x(GimpCurveView *view, gdouble min,
                                 gdouble max) {
  g_return_if_fail(GIMP_IS_CURVE_VIEW(view));
...
void gimp_curve_view_set_range_y(GimpCurveView *view, gdouble min,
                                 gdouble max) {
  g_return_if_fail(GIMP_IS_CURVE_VIEW(view));
...
void gimp_curve_view_set_xpos(GimpCurveView *view, gdouble x) {
  g_return_if_fail(GIMP_IS_CURVE_VIEW(view));
...
void gimp_curve_view_set_x_axis_label(GimpCurveView *view, const gchar *label) {
  g_return_if_fail(GIMP_IS_CURVE_VIEW(view));
...
void gimp_curve_view_set_y_axis_label(GimpCurveView *view, const gchar *label) {
  g_return_if_fail(GIMP_IS_CURVE_VIEW(view));
