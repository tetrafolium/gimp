...
static void gimp_color_editor_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_color_editor_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);

static void gimp_color_editor_style_updated(GtkWidget *widget);

static void gimp_color_editor_set_aux_info(GimpDocked *docked, GList *aux_info);
...
static void gimp_color_editor_tab_toggled(GtkWidget *widget,
                                          GimpColorEditor *editor);
static void gimp_color_editor_fg_bg_notify(GtkWidget *widget, GParamSpec *pspec,
                                           GimpColorEditor *editor);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpColorEditor, gimp_color_editor, GIMP_TYPE_EDITOR,
...
static void gimp_color_editor_init(GimpColorEditor *editor) {
  ...
  gtk_widget_style_get(GTK_WIDGET(editor), "content-spacing", &content_spacing,
                       "button-spacing", &button_spacing, "button-icon-size",
  ...
  editor->notebook = gimp_color_selector_new(GIMP_TYPE_COLOR_NOTEBOOK, &rgb,
                                             &hsv, GIMP_COLOR_SELECTOR_RED);
  gimp_color_selector_set_show_alpha(GIMP_COLOR_SELECTOR(editor->notebook),
                                     FALSE);
  gtk_box_pack_start(GTK_BOX(editor), editor->notebook, TRUE, TRUE,
                     content_spacing);
  gtk_widget_show(editor->notebook);

  g_signal_connect(editor->notebook, "color-changed",
                   G_CALLBACK(gimp_color_editor_color_changed), editor);

  notebook =
      gimp_color_notebook_get_notebook(GIMP_COLOR_NOTEBOOK(editor->notebook));
  ...
  for (list = gimp_color_notebook_get_selectors(
           GIMP_COLOR_NOTEBOOK(editor->notebook));
    ...
    gimp_help_set_help_data(button, selector_class->name,
                            selector_class->help_id);

    g_object_set_data(G_OBJECT(button), "selector", selector);
    g_object_set_data(G_OBJECT(selector), "button", button);

    g_signal_connect(button, "toggled",
                     G_CALLBACK(gimp_color_editor_tab_toggled), editor);
  }
  ...
  g_signal_connect(editor->fg_bg, "notify::active-color",
                   G_CALLBACK(gimp_color_editor_fg_bg_notify), editor);
  ...
  g_signal_connect(button, "color-picked",
                   G_CALLBACK(gimp_color_editor_color_picked), editor);
  ...
  g_signal_connect(editor->hex_entry, "color-changed",
                   G_CALLBACK(gimp_color_editor_entry_changed), editor);
}
...
static void gimp_color_editor_set_property(GObject *object, guint property_id,
  ...
  switch (property_id) {
  case PROP_CONTEXT:
    gimp_docked_set_context(GIMP_DOCKED(object), g_value_get_object(value));
    break;
  default:
...
static void gimp_color_editor_set_context(GimpDocked *docked,
  ...
  if (editor->context) {
    g_signal_handlers_disconnect_by_func(editor->context,
                                         gimp_color_editor_fg_changed, editor);
    g_signal_handlers_disconnect_by_func(editor->context,
  ...
  if (editor->context) {
    ...
    g_signal_connect(editor->context, "foreground-changed",
                     G_CALLBACK(gimp_color_editor_fg_changed), editor);
    g_signal_connect(editor->context, "background-changed",
                     G_CALLBACK(gimp_color_editor_bg_changed), editor);
...
GtkWidget *gimp_color_editor_new(GimpContext *context) {
  return g_object_new(GIMP_TYPE_COLOR_EDITOR, "context", context, NULL);
}
...
static void gimp_color_editor_set_color(GimpColorEditor *editor,
                                        const GimpRGB *rgb) {
  GimpHSV hsv;
  ...
  g_signal_handlers_block_by_func(editor->notebook,
                                  gimp_color_editor_color_changed, editor);

  gimp_color_selector_set_color(GIMP_COLOR_SELECTOR(editor->notebook), rgb,
                                &hsv);
  ...
  g_signal_handlers_block_by_func(editor->hex_entry,
                                  gimp_color_editor_entry_changed, editor);

  gimp_color_hex_entry_set_color(GIMP_COLOR_HEX_ENTRY(editor->hex_entry), rgb);
...
static void gimp_color_editor_tab_toggled(GtkWidget *widget,
  ...
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
    ...
    if (selector) {
      ...
      notebook = gimp_color_notebook_get_notebook(
          GIMP_COLOR_NOTEBOOK(editor->notebook));
...
static void gimp_color_editor_fg_bg_notify(GtkWidget *widget, GParamSpec *pspec,
  ...
  edit_bg =
      (GIMP_FG_BG_EDITOR(widget)->active_color == GIMP_ACTIVE_COLOR_BACKGROUND);
