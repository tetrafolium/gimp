...
static void gimp_font_select_init(GimpFontSelect *select) {}
...
static void gimp_font_select_constructed(GObject *object) {
  ...
  dialog->view = gimp_font_factory_view_new(
      GIMP_VIEW_TYPE_LIST, dialog->context->gimp->font_factory, dialog->context,
      GIMP_VIEW_SIZE_MEDIUM, 1, dialog->menu_factory);
...
static GimpValueArray *gimp_font_select_run_callback(GimpPdbDialog *dialog,
  ...
  return gimp_pdb_execute_procedure_by_name(
      ...
      G_TYPE_STRING, gimp_object_get_name(object), G_TYPE_BOOLEAN, closing,
      G_TYPE_NONE);
}
