...
static gint64 gimp_tag_cache_get_memsize(GimpObject *object, gint64 *gui_size);
static void gimp_tag_cache_object_initialize(GimpTagged *tagged,
                                             GimpTagCache *cache);
static void gimp_tag_cache_add_object(GimpTagCache *cache, GimpTagged *tagged);
...
static void gimp_tag_cache_class_init(GimpTagCacheClass *klass) {
...
static void gimp_tag_cache_init(GimpTagCache *cache) {
...
static void gimp_tag_cache_container_add_callback(GimpTagCache *cache,
                                                  GimpTagged *tagged,
...
void gimp_tag_cache_save(GimpTagCache *cache) {
  ...
  for (iterator = saved_records; iterator; iterator = g_list_next(iterator)) {
    ...
    identifier_string =
        g_markup_escape_text(g_quark_to_string(cache_rec->identifier), -1);
    g_string_append_printf(
        buf, "\n  <resource identifier=\"%s\" checksum=\"%s\">\n",
        identifier_string, g_quark_to_string(cache_rec->checksum));
  ...
  if (!output) {
    g_printerr("%s\n", error->message);
  } else if (!g_output_stream_write_all(output, buf->str, buf->len, NULL, NULL,
                                        &error)) {
...
void gimp_tag_cache_load(GimpTagCache *cache) {
  ...
  if (gimp_xml_parser_parse_gfile(xml_parser, file, &error)) {
    cache->priv->records =
        g_array_append_vals(cache->priv->records, parse_data.records->data,
...
static void gimp_tag_cache_load_start_element(GMarkupParseContext *context,
  ...
  if (!strcmp(element_name, "resource")) {
    ...
    identifier = gimp_tag_cache_attribute_name_to_value(
        attribute_names, attribute_values, "identifier");
    checksum = gimp_tag_cache_attribute_name_to_value(
        attribute_names, attribute_values, "checksum");

    if (!identifier) {
      g_set_error(
          error, gimp_tag_cache_get_error_domain(), 1001,
...
static void gimp_tag_cache_load_end_element(GMarkupParseContext *context,
  ...
  if (strcmp(element_name, "resource") == 0) {
    parse_data->records =
        g_array_append_val(parse_data->records, parse_data->current_record);
    memset(&parse_data->current_record, 0, sizeof(GimpTagCacheRecord));
  }
}
...
static void gimp_tag_cache_load_text(GMarkupParseContext *context,
  ...
  if (g_strcmp0(current_element, "tag") == 0) {
    ...
    if (tag) {
      parse_data->current_record.tags =
          g_list_append(parse_data->current_record.tags, tag);
    } else {
