...
static void gimp_parallel_set_n_threads(gint n_threads, gboolean finish_tasks);
...
static GimpParallelRunAsyncThread
    gimp_parallel_run_async_threads[GIMP_PARALLEL_RUN_ASYNC_MAX_THREADS];
...
void gimp_parallel_init(Gimp *gimp) {
  ...
  g_signal_connect(config, "notify::num-processors",
                   G_CALLBACK(gimp_parallel_notify_num_processors), NULL);
...
void gimp_parallel_exit(Gimp *gimp) {
  ...
  g_signal_handlers_disconnect_by_func(
      gimp->config, (gpointer)gimp_parallel_notify_num_processors, NULL);

  /* stop all threads */
  gimp_parallel_set_n_threads(0, /* finish_tasks = */ FALSE);
}

GimpAsync *gimp_parallel_run_async(GimpRunAsyncFunc func, gpointer user_data) {
  return gimp_parallel_run_async_full(0, func, user_data, NULL);
}

GimpAsync *gimp_parallel_run_async_full(gint priority, GimpRunAsyncFunc func,
                                        gpointer user_data,
                                        GDestroyNotify user_data_destroy_func) {
  GimpAsync *async;
  ...
  if (gimp_parallel_run_async_n_threads > 0) {
    g_signal_connect_after(async, "cancel",
                           G_CALLBACK(gimp_parallel_run_async_cancel), NULL);
    g_signal_connect_after(async, "waiting",
                           G_CALLBACK(gimp_parallel_run_async_waiting), NULL);
...
GimpAsync *gimp_parallel_run_async_independent_full(gint priority,
  ...
  gimp_async_add_callback(
      async,
      [](GimpAsync *async, gpointer thread) {
...
static void gimp_parallel_run_async_set_n_threads(gint n_threads,
  ...
  if (n_threads > gimp_parallel_run_async_n_threads) /* need more threads */
  {
    for (i = gimp_parallel_run_async_n_threads; i < n_threads; i++) {
      GimpParallelRunAsyncThread *thread = &gimp_parallel_run_async_threads[i];

      thread->quit = FALSE;

      thread->thread = g_thread_new(
          "async", (GThreadFunc)gimp_parallel_run_async_thread_func, thread);
    }
  } else if (n_threads <
             gimp_parallel_run_async_n_threads) /* need less threads */
  {
    g_mutex_lock(&gimp_parallel_run_async_mutex);
...
gimp_parallel_run_async_thread_func(GimpParallelRunAsyncThread *thread) {
  ...
  while (TRUE) {
    ...
    while (!thread->quit && (task = gimp_parallel_run_async_dequeue_task())) {
      gboolean resume;
      ...
      do {
        ...
        g_mutex_lock(&gimp_parallel_run_async_mutex);
               ...
               (g_queue_is_empty(&gimp_parallel_run_async_queue) ||
                task->priority < ((GimpParallelRunAsyncTask *)g_queue_peek_head(
    ...
    g_cond_wait(&gimp_parallel_run_async_cond, &gimp_parallel_run_async_mutex);
  }
...
gimp_parallel_run_async_enqueue_task(GimpParallelRunAsyncTask *task) {
  ...
  g_object_set_data(G_OBJECT(task->async), "gimp-parallel-run-async-link",
                    link);

  for (iter = g_queue_peek_tail_link(&gimp_parallel_run_async_queue); iter;
...
static void gimp_parallel_run_async_cancel(GimpAsync *async) {
  ...
  if (link) {
    g_object_set_data(G_OBJECT(async), "gimp-parallel-run-async-link", NULL);
