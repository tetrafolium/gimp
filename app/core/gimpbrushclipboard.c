...
enum { PROP_0, PROP_GIMP, PROP_MASK_ONLY };
...
static void gimp_brush_clipboard_changed(Gimp *gimp, GimpBrush *brush);
...
static void gimp_brush_clipboard_class_init(GimpBrushClipboardClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_MASK_ONLY,
      g_param_spec_boolean("mask-only", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_brush_clipboard_init(GimpBrushClipboard *brush) {}
...
static void gimp_brush_clipboard_constructed(GObject *object) {
  ...
  g_signal_connect_object(brush->gimp, "clipboard-changed",
                          G_CALLBACK(gimp_brush_clipboard_changed), brush, 0);
...
GimpData *gimp_brush_clipboard_new(Gimp *gimp, gboolean mask_only) {
  ...
  return g_object_new(GIMP_TYPE_BRUSH_CLIPBOARD, "name", name, "gimp", gimp,
                      "mask-only", mask_only, NULL);
}
...
static void gimp_brush_clipboard_changed(Gimp *gimp, GimpBrush *brush) {
  ...
  if (buffer) {
    ...
    if (GIMP_BRUSH_CLIPBOARD(brush)->mask_only) {
      ...
      brush->priv->pixmap =
          gimp_temp_buf_new(width, height, babl_format("R'G'B' u8"));
      ...
      if (babl_format_has_alpha(format)) {
        ...
        memset(gimp_temp_buf_get_data(brush->priv->mask), 255, width * height);
      }
