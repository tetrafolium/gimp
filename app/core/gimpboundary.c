...
static void gimp_boundary_add_seg(GimpBoundary *boundary, gint x1, gint y1,
                                  gint x2, gint y2, gboolean open);

static void find_empty_segs(const GeglRectangle *region,
                            const gfloat *line_data, gint scanline,
                            gint empty_segs[], gint max_empty, gint *num_empty,
                            GimpBoundaryType type, gint x1, gint y1, gint x2,
                            gint y2, gfloat threshold);
static void process_horiz_seg(GimpBoundary *boundary, gint x1, gint y1, gint x2,
                              gint y2, gboolean open);
static void make_horiz_segs(GimpBoundary *boundary, gint start, gint end,
                            gint scanline, gint empty[], gint num_empty,
                            gint top);
static GimpBoundary *generate_boundary(GeglBuffer *buffer,
                                       const GeglRectangle *region,
                                       const Babl *format,
                                       GimpBoundaryType type, gint x1, gint y1,
                                       gint x2, gint y2, gfloat threshold);
...
static const GimpBoundSeg *find_segment(const GimpBoundSeg **segs_by_xy1,
                                        ...
                                        gint num_segs, gint x, gint y);

static const GimpBoundSeg *
find_segment_with_func(const GimpBoundSeg **segs, gint num_segs,
                       const GimpBoundSeg *search_seg, GCompareFunc cmp_func);

static void simplify_subdivide(const GimpBoundSeg *segs, gint start_idx,
                               gint end_idx, GArray **ret_points);
...
GimpBoundSeg *gimp_boundary_find(GeglBuffer *buffer,
                                 const GeglRectangle *region,
                                 const Babl *format, GimpBoundaryType type,
                                 int x1, int y1, int x2, int y2,
                                 gfloat threshold, int *num_segs) {
  ...
  g_return_val_if_fail(
      babl_format_get_bytes_per_pixel(format) == sizeof(gfloat), NULL);
  ...
  boundary =
      generate_boundary(buffer, &rect, format, type, x1, y1, x2, y2, threshold);
 ...
 **/
GimpBoundSeg *gimp_boundary_sort(const GimpBoundSeg *segs, gint num_segs,
                                 gint *num_groups) {
  GimpBoundary *boundary;
  ...
  g_return_val_if_fail(
      (segs == NULL && num_segs == 0) || (segs != NULL && num_segs > 0), NULL);
  ...
  *num_groups = 0;
  ...
  for (index = 0; index < num_segs; index++) {
    ...
    gimp_boundary_add_seg(boundary, segs[index].x1, segs[index].y1,
                          segs[index].x2, segs[index].y2, segs[index].open);
    ...
    while ((cur_seg = find_segment(segs_ptrs_by_xy1, segs_ptrs_by_xy2, num_segs,
                                   x, y)) != NULL) {
      /*  make sure ordering is correct  */
      if (x == cur_seg->x1 && y == cur_seg->y1) {
        gimp_boundary_add_seg(boundary, cur_seg->x1, cur_seg->y1, cur_seg->x2,
                              cur_seg->y2, cur_seg->open);
        x = cur_seg->x2;
        y = cur_seg->y2;
      } else {
        gimp_boundary_add_seg(boundary, cur_seg->x2, cur_seg->y2, cur_seg->x1,
                              cur_seg->y1, cur_seg->open);
        x = cur_seg->x1;
        y = cur_seg->y1;
      }
 ...
 **/
GimpBoundSeg *gimp_boundary_simplify(GimpBoundSeg *sorted_segs, gint num_groups,
                                     gint *num_segs) {
  GArray *new_bounds;
  gint i, seg;

  g_return_val_if_fail((sorted_segs == NULL && num_groups == 0) ||
                           (sorted_segs != NULL && num_groups > 0),
                       NULL);
  g_return_val_if_fail(num_segs != NULL, NULL);
  ...
  for (i = 0; i < num_groups; i++) {
    ...
    while (sorted_segs[seg].x1 != -1 || sorted_segs[seg].x2 != -1 ||
           sorted_segs[seg].y1 != -1 || sorted_segs[seg].y2 != -1) {
      n_points++;
      seg++;
    }
    ...
    if (n_points > 0) {
      ...
      simplify_subdivide(sorted_segs, start, start + n_points, &tmp_points);
      sorted_segs[seg] = tmp_seg;

      for (j = 0; j < tmp_points->len; j++)
        g_array_append_val(new_bounds,
                           sorted_segs[g_array_index(tmp_points, gint, j)]);
...
void gimp_boundary_offset(GimpBoundSeg *segs, gint num_segs, gint off_x,
                          gint off_y) {
  gint i;
  ...
  for (i = 0; i < num_segs; i++) {
    ...
    if (!(segs[i].x1 == -1 && segs[i].y1 == -1 && segs[i].x2 == -1 &&
          segs[i].y2 == -1)) {
      segs[i].x1 += off_x;
      segs[i].y1 += off_y;
      segs[i].x2 += off_x;
...
static void make_horiz_segs(GimpBoundary *boundary, gint start, gint end,
  ...
  for (empty_index = 0; empty_index < num_empty; empty_index += 2) {
    ...
    if (e_s <= start && e_e >= end) {
      process_horiz_seg(boundary, start, scanline, end, scanline, top);
    } else if ((e_s > start && e_s < end) || (e_e < end && e_e > start)) {
      process_horiz_seg(boundary, MAX(e_s, start), scanline, MIN(e_e, end),
                        scanline, top);
    }
  }
}
...
static GimpBoundary *generate_boundary(GeglBuffer *buffer,
  ...
  find_empty_segs(region, NULL, start - 1, boundary->empty_segs_l,
                  boundary->max_empty_segs, &num_empty_l, type, x1, y1, x2, y2,
                  threshold);

  line_rect.y = start;
  gegl_buffer_get(buffer, &line_rect, 1.0, format, line_data,
                  GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);

  find_empty_segs(region, line_data, start, boundary->empty_segs_c,
                  boundary->max_empty_segs, &num_empty_c, type, x1, y1, x2, y2,
                  threshold);
  ...
  for (scanline = start; scanline < end; scanline++) {
    ...
    find_empty_segs(region, line_data, scanline + 1, boundary->empty_segs_n,
                    boundary->max_empty_segs, &num_empty_n, type, x1, y1, x2,
                    y2, threshold);

    /*  process the segments on the current scanline  */
    for (i = 1; i < num_empty_c - 1; i += 2) {
      make_horiz_segs(boundary, boundary->empty_segs_c[i],
                      boundary->empty_segs_c[i + 1], scanline,
                      boundary->empty_segs_l, num_empty_l, 1);
      make_horiz_segs(boundary, boundary->empty_segs_c[i],
                      boundary->empty_segs_c[i + 1], scanline + 1,
...
static inline gint cmp_xy(const gint ax, const gint ay, const gint bx,
                          const gint by) {
  if (ay < by) {
    return -1;
  } else if (ay > by) {
