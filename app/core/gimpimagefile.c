...
#define GET_PRIVATE(imagefile)                                                 \
  ((GimpImagefilePrivate *)gimp_imagefile_get_instance_private(                \
...
static void gimp_thumbnail_set_info(GimpThumbnail *thumbnail,
                                    const gchar *mime_type, gint width,
...
static void gimp_imagefile_class_init(GimpImagefileClass *klass) {
  ...
  gimp_imagefile_signals[INFO_CHANGED] =
      g_signal_new("info-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpImagefileClass, info_changed), NULL,
  ...
  creator =
      g_strdup_printf("gimp-%d.%d", GIMP_MAJOR_VERSION, GIMP_MINOR_VERSION);
...
static void gimp_imagefile_name_changed(GimpObject *object) {
  ...
  private
  ->file = g_file_new_for_uri(gimp_object_get_name(object));
}
...
static gchar *gimp_imagefile_get_description(GimpViewable *viewable,
  ...
  if (thumbnail->image_width > 0 && thumbnail->image_height > 0) {
    ...
    basename = g_strdup_printf("%s (%d × %d)", tmp, thumbnail->image_width,
...
GimpImagefile *gimp_imagefile_new(Gimp *gimp, GFile *file) {
  GimpImagefile *imagefile;
...
void gimp_imagefile_set_file(GimpImagefile *imagefile, GFile *file) {
...
private
  ...
  private
    ...
    g_file_query_info_async(private->file, "standard::icon",
                            ...
                            gimp_imagefile_icon_callback, imagefile);
  }
...
void gimp_imagefile_set_mime_type(GimpImagefile *imagefile,
  ...
  g_object_set(GET_PRIVATE(imagefile)->thumbnail, "image-mimetype", mime_type,
               NULL);
}
...
private
  ...
  g_object_get(private->thumbnail, "image-uri", &uri, NULL);

  if (uri) {
    GimpImagefile *documents_imagefile =
        (GimpImagefile *)gimp_container_get_child_by_name(
            private->gimp->documents, uri);
...
gboolean gimp_imagefile_create_thumbnail(GimpImagefile *imagefile,
                                         GimpContext *context,
                                         GimpProgress *progress, gint size,
                                         gboolean replace, GError **error) {
  GimpImagefilePrivate *private;
  GimpThumbnail *thumbnail;
...
private
  ...
  gimp_thumbnail_set_uri(thumbnail, gimp_object_get_name(imagefile));
  ...
  if (image_state == GIMP_THUMB_STATE_REMOTE ||
    ...
    if (g_file_is_native(private->file)) {
      ...
      readable = g_file_info_get_attribute_boolean(
          file_info, G_FILE_ATTRIBUTE_ACCESS_CAN_READ);
     ...
     */
    image = file_open_thumbnail(private->gimp, context, progress, private->file,
                                size, &mime_type, &width, &height, &format,
                                &num_layers, NULL);

    if (image) {
      gimp_thumbnail_set_info(private->thumbnail, mime_type, width, height,
                              format, num_layers);
    } else {
       ...
       */
      image = file_open_image(private->gimp, context, progress, private->file,
                              FALSE, NULL, GIMP_RUN_NONINTERACTIVE, &status,
                              &mime_type, NULL);

      if (image)
        gimp_thumbnail_set_info_from_image(private->thumbnail, mime_type,
                                           image);
    }

    if (image) {
      success =
          gimp_imagefile_save_thumb(imagefile, image, size, replace, error);

      g_object_unref(image);
    } else {
      success =
          gimp_thumbnail_save_failure(thumbnail, "GIMP " GIMP_VERSION, error);
...
private
  ...
  if (!gimp_imagefile_create_thumbnail(local, context, progress, size, replace,
     ...
     */
    g_object_set(private->thumbnail, "thumb-state", GIMP_THUMB_STATE_FAILED,
                 NULL);
  }
  ...
  if (imagefile) {
    ...
    g_object_remove_weak_pointer(G_OBJECT(imagefile), (gpointer)&imagefile);
  }
...
gboolean gimp_imagefile_save_thumbnail(GimpImagefile *imagefile,
                                       const gchar *mime_type, GimpImage *image,
                                       GError **error) {
  GimpImagefilePrivate *private;
  gint size;
...
private
  ...
  if (size > 0) {
    gimp_thumbnail_set_info_from_image(private->thumbnail, mime_type, image);

    success = gimp_imagefile_save_thumb(imagefile, image, size, FALSE, error);
  }
...
private
  ...
  if (file_info) {
  private
    ->icon = g_object_ref(g_file_info_get_icon(file_info));
    g_object_unref(file_info);
  }
...
private
  ...
  private
    ->description = (gchar *)_("Special File");
  private
    ->static_desc = TRUE;
    break;
  ...
  private
    ->description = (gchar *)g_strerror(thumbnail->image_not_found_errno);
  private
    ->static_desc = TRUE;
    break;
    ...
    case GIMP_THUMB_STATE_OK: {
      ...
      if (thumbnail->image_width > 0 && thumbnail->image_height > 0) {
        ...
        g_string_append_printf(str,
                               ngettext("%d × %d pixel", "%d × %d pixels",
                                        thumbnail->image_height),
                               thumbnail->image_width, thumbnail->image_height);
      }
      ...
      if (thumbnail->image_num_layers > 0) {
        ...
        g_string_append_printf(
            str, ngettext("%d layer", "%d layers", thumbnail->image_num_layers),
...
static GdkPixbuf *gimp_imagefile_load_thumb(GimpImagefile *imagefile,
  ...
  gimp_viewable_calc_preview_size(pixbuf_width, pixbuf_height, width, height,
                                  TRUE, 1.0, 1.0, &preview_width,
                                  &preview_height, NULL);

  if (preview_width < pixbuf_width || preview_height < pixbuf_height) {
    GdkPixbuf *scaled = gdk_pixbuf_scale_simple(
        pixbuf, preview_width, preview_height, GDK_INTERP_BILINEAR);
  ...
  if (gdk_pixbuf_get_n_channels(pixbuf) != 3) {
    GdkPixbuf *tmp = gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8, pixbuf_width,
                                    pixbuf_height);

    gdk_pixbuf_composite_color(pixbuf, tmp, 0, 0, pixbuf_width, pixbuf_height,
                               0.0, 0.0, 1.0, 1.0, GDK_INTERP_NEAREST, 255, 0,
                               0, GIMP_CHECK_SIZE_SM, 0x66666666, 0x99999999);
...
static gboolean gimp_imagefile_save_thumb(GimpImagefile *imagefile,
  ...
  success =
      gimp_thumbnail_save_thumb(thumbnail, pixbuf, "GIMP " GIMP_VERSION, error);
...
static void gimp_thumbnail_set_info_from_image(GimpThumbnail *thumbnail,
  ...
  format = gimp_image_get_layer_format(image, gimp_image_has_alpha(image));

  g_object_set(thumbnail, "image-mimetype", mime_type, "image-width",
               gimp_image_get_width(image), "image-height",
               gimp_image_get_height(image), "image-type",
               gimp_babl_format_get_description(format), "image-num-layers",
               gimp_image_get_n_layers(image), NULL);
}
...
static void gimp_thumbnail_set_info(GimpThumbnail *thumbnail,
  ...
  g_object_set(thumbnail, "image-mimetype", mime_type, "image-width", width,
               "image-height", height, NULL);

  if (format)
    g_object_set(thumbnail, "image-type",
                 gimp_babl_format_get_description(format), NULL);

  if (num_layers != -1)
    g_object_set(thumbnail, "image-num-layers", num_layers, NULL);
}
