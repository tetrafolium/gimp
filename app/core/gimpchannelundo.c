...
enum { PROP_0, PROP_PREV_PARENT, PROP_PREV_POSITION, PROP_PREV_CHANNELS };

static void gimp_channel_undo_constructed(GObject *object);
static void gimp_channel_undo_finalize(GObject *object);
static void gimp_channel_undo_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_channel_undo_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);

static gint64 gimp_channel_undo_get_memsize(GimpObject *object,
                                            gint64 *gui_size);

static void gimp_channel_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
...
static void gimp_channel_undo_class_init(GimpChannelUndoClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("prev-parent", NULL, NULL, GIMP_TYPE_CHANNEL,
  ...
  g_object_class_install_property(
      object_class, PROP_PREV_CHANNELS,
      g_param_spec_pointer("prev-channels", NULL, NULL,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_channel_undo_init(GimpChannelUndo *undo) {
...
static void gimp_channel_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
  ...
  if ((undo_mode == GIMP_UNDO_MODE_UNDO &&
    ...
    g_clear_pointer(&channel_undo->prev_channels, g_list_free);
    channel_undo->prev_channels =
        g_list_copy(gimp_image_get_selected_channels(undo->image));

    gimp_image_add_channel(undo->image, channel, channel_undo->prev_parent,
