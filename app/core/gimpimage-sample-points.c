...
GimpSamplePoint *gimp_image_add_sample_point_at_pos(GimpImage *image, gint x,
                                                    gint y,
                                                    gboolean push_undo) {
  ...
  if (push_undo)
    gimp_image_undo_push_sample_point(
        image, C_("undo-type", "Add Sample Point"), sample_point);
...
void gimp_image_add_sample_point(GimpImage *image,
                                 GimpSamplePoint *sample_point, gint x,
                                 gint y) {
...
private
  ->sample_points = g_list_append(private->sample_points, sample_point);
...
private
  ...
  if (push_undo)
    gimp_image_undo_push_sample_point(
        image, C_("undo-type", "Remove Sample Point"), sample_point);

private
  ->sample_points = g_list_remove(private->sample_points, sample_point);
...
void gimp_image_move_sample_point(GimpImage *image,
                                  GimpSamplePoint *sample_point, gint x, gint y,
                                  gboolean push_undo) {
  g_return_if_fail(GIMP_IS_IMAGE(image));
  g_return_if_fail(GIMP_IS_SAMPLE_POINT(sample_point));
  ...
  if (push_undo)
    gimp_image_undo_push_sample_point(
        image, C_("undo-type", "Move Sample Point"), sample_point);
...
void gimp_image_set_sample_point_pick_mode(GimpImage *image,
                                           GimpSamplePoint *sample_point,
  ...
  if (push_undo)
    gimp_image_undo_push_sample_point(
        image, C_("undo-type", "Set Sample Point Pick Mode"), sample_point);
...
GimpSamplePoint *gimp_image_get_sample_point(GimpImage *image, guint32 id) {
  GList *sample_points;
...
GimpSamplePoint *
gimp_image_get_next_sample_point(GimpImage *image, guint32 id,
  ...
  for (sample_points = GIMP_IMAGE_GET_PRIVATE(image)->sample_points;
    ...
    if (gimp_aux_item_get_id(GIMP_AUX_ITEM(sample_point)) ==
        id) /* found it, next one will be returned */
      *sample_point_found = TRUE;
  }
