...
enum { STROKES_UPDATED, GUI_PARAM_CHANGED, ACTIVE_CHANGED, LAST_SIGNAL };
...
static void gimp_symmetry_set_property(GObject *object, guint property_id,
                                       const GValue *value, GParamSpec *pspec);
static void gimp_symmetry_get_property(GObject *object, guint property_id,
                                       GValue *value, GParamSpec *pspec);

static void gimp_symmetry_real_update_strokes(GimpSymmetry *sym,
                                              GimpDrawable *drawable,
                                              GimpCoords *origin);
static void gimp_symmetry_real_get_transform(GimpSymmetry *sym, gint stroke,
...
static void gimp_symmetry_class_init(GimpSymmetryClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  /* This signal should likely be emitted at the end of
   * update_strokes() if stroke coordinates were changed.
   */
  gimp_symmetry_signals[STROKES_UPDATED] = g_signal_new(
      "strokes-updated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST, 0, NULL,
      NULL, NULL, G_TYPE_NONE, 1, GIMP_TYPE_IMAGE);
   ...
   */
  gimp_symmetry_signals[GUI_PARAM_CHANGED] = g_signal_new(
      "gui-param-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST, 0,
      NULL, NULL, NULL, G_TYPE_NONE, 1, GIMP_TYPE_IMAGE);

  gimp_symmetry_signals[ACTIVE_CHANGED] = g_signal_new(
      "active-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpSymmetryClass, active_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  ...
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_ACTIVE, "active", _("Active"),
                           _("Activate symmetry painting"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_INT(object_class, PROP_VERSION, "version",
                       "Symmetry version", "Version of the symmetry object", -1,
                       G_MAXINT, 0, GIMP_PARAM_STATIC_STRINGS);
}

static void gimp_symmetry_init(GimpSymmetry *sym) {}
...
static void gimp_symmetry_set_property(GObject *object, guint property_id,
  ...
  case PROP_ACTIVE:
    ...
    g_signal_emit(sym, gimp_symmetry_signals[ACTIVE_CHANGED], 0, sym->active);
    break;
  case PROP_VERSION:
...
static void gimp_symmetry_real_update_strokes(GimpSymmetry *sym,
  ...
  sym->strokes =
      g_list_prepend(sym->strokes, g_memdup(origin, sizeof(GimpCoords)));
}
...
static gboolean gimp_symmetry_real_update_version(GimpSymmetry *symmetry) {
  ...
  g_object_get(symmetry, "version", &version, NULL);
 ...
 **/
void gimp_symmetry_set_stateful(GimpSymmetry *symmetry, gboolean stateful) {
 ...
 **/
void gimp_symmetry_set_origin(GimpSymmetry *sym, GimpDrawable *drawable,
                              GimpCoords *origin) {
 ...
 **/
void gimp_symmetry_clear_origin(GimpSymmetry *sym) {
  g_return_if_fail(GIMP_IS_SYMMETRY(sym));
 ...
 * The returned value is owned by the #GimpSymmetry and must not be freed.
 **/
GimpCoords *gimp_symmetry_get_coords(GimpSymmetry *sym, gint stroke) {
  g_return_val_if_fail(GIMP_IS_SYMMETRY(sym), NULL);
 ...
 **/
void gimp_symmetry_get_transform(GimpSymmetry *sym, gint stroke, gdouble *angle,
                                 gboolean *reflect) {
  g_return_if_fail(GIMP_IS_SYMMETRY(sym));
  ...
  *reflect = FALSE;
  ...
  GIMP_SYMMETRY_GET_CLASS(sym)->get_transform(sym, stroke, angle, reflect);
}
 ...
 * number @stroke.
 **/
void gimp_symmetry_get_matrix(GimpSymmetry *sym, gint stroke,
                              GimpMatrix3 *matrix) {
  gdouble angle;
  gboolean reflect;
 ...
 **/
GeglNode *gimp_symmetry_get_operation(GimpSymmetry *sym, gint stroke) {
  GimpMatrix3 matrix;
...
GimpParasite *gimp_symmetry_to_parasite(const GimpSymmetry *sym) {
  ...
  parasite = gimp_config_serialize_to_parasite((GimpConfig *)sym, parasite_name,
...
GimpSymmetry *gimp_symmetry_from_parasite(const GimpParasite *parasite,
  ...
  g_return_val_if_fail(strcmp(gimp_parasite_name(parasite), parasite_name) == 0,
                       NULL);
  ...
  g_object_set(symmetry, "version", -1, NULL);

  if (!gimp_config_deserialize_parasite(GIMP_CONFIG(symmetry), parasite, NULL,
                                        &error)) {
