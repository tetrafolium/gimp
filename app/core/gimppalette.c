...
static gint64 gimp_palette_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_palette_get_preview_size(GimpViewable *viewable, gint size,
                                          gboolean popup, gboolean dot_for_dot,
                                          gint *width, gint *height);
static gboolean gimp_palette_get_popup_size(GimpViewable *viewable, gint width,
...
static void gimp_palette_copy(GimpData *data, GimpData *src_data);
...
static void gimp_palette_finalize(GObject *object) {
  ...
  if (palette->colors) {
    g_list_free_full(palette->colors, (GDestroyNotify)gimp_palette_entry_free);
    palette->colors = NULL;
  }
...
static gint64 gimp_palette_get_memsize(GimpObject *object, gint64 *gui_size) {
  ...
  memsize += gimp_g_list_get_memsize_foreach(
      palette->colors, (GimpMemsizeFunc)gimp_palette_entry_get_memsize,
      gui_size);
...
static GimpTempBuf *gimp_palette_get_new_preview(GimpViewable *viewable,
  ...
  for (y = 0; y < rows && list; y++) {
    ...
    for (x = 0; x < columns && list; x++) {
      ...
      gimp_rgb_get_uchar(&entry->color, &b[x * cell_size * 3 + 0],
                         &b[x * cell_size * 3 + 1], &b[x * cell_size * 3 + 2]);
...
static gchar *gimp_palette_get_description(GimpViewable *viewable,
  ...
  return g_strdup_printf("%s (%d)", gimp_object_get_name(palette),
                         palette->n_colors);
}
...
GimpData *gimp_palette_new(GimpContext *context, const gchar *name) {
  ...
  return g_object_new(GIMP_TYPE_PALETTE, "name", name, NULL);
}
...
static gchar *gimp_palette_get_checksum(GimpTagged *tagged) {
  ...
  if (palette->n_colors > 0) {
    ...
    g_checksum_update(checksum, (const guchar *)&palette->n_columns,
                      sizeof(palette->n_columns));
    ...
    while (color_iterator) {
      ...
      g_checksum_update(checksum, (const guchar *)&entry->color,
                        sizeof(entry->color));
      if (entry->name)
        g_checksum_update(checksum, (const guchar *)entry->name,
                          strlen(entry->name));
  ...
  if (g_list_find(palette->colors, entry)) {
    ...
    if (pos < position) {
      ...
      for (list = g_list_nth(palette->colors, position + 1);
           list && position < pos; list = g_list_next(list)) {
        entry = (GimpPaletteEntry *)list->data;
...
void gimp_palette_delete_entry(GimpPalette *palette, GimpPaletteEntry *entry) {
  ...
  if (g_list_find(palette->colors, entry)) {
    ...
    for (list = g_list_nth(palette->colors, pos); list;
         list = g_list_next(list)) {
      entry = (GimpPaletteEntry *)list->data;
