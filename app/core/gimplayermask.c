...
static gboolean gimp_layer_mask_rename(GimpItem *item, const gchar *new_name,
                                       const gchar *undo_desc, GError **error);

static void gimp_layer_mask_bounding_box_changed(GimpDrawable *drawable);
static void gimp_layer_mask_convert_type(
...
static gboolean gimp_layer_mask_is_attached(GimpItem *item) {
  ...
  return (GIMP_IS_IMAGE(gimp_item_get_image(item)) && GIMP_IS_LAYER(layer) &&
          gimp_layer_get_mask(layer) == mask &&
          gimp_item_is_attached(GIMP_ITEM(layer)));
}

static GimpItemTree *gimp_layer_mask_get_tree(GimpItem *item) { return NULL; }
...
static gboolean gimp_layer_mask_rename(GimpItem *item, const gchar *new_name,
  ...
  g_set_error(error, GIMP_ERROR, GIMP_FAILED, _("Cannot rename layer masks."));
...
GimpLayerMask *gimp_layer_mask_new(GimpImage *image, gint width, gint height,
  ...
  layer_mask = GIMP_LAYER_MASK(
      gimp_drawable_new(GIMP_TYPE_LAYER_MASK, image, name, 0, 0, width, height,
