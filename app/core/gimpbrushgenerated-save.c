...
gboolean gimp_brush_generated_save(GimpData *data, GOutputStream *output,
  ...
  g_string_append_printf(
      string, "%s\n",
      g_ascii_dtostr(buf, G_ASCII_DTOSTR_BUF_SIZE, brush->radius));
  ...
  g_string_append_printf(
      string, "%s\n",
      g_ascii_dtostr(buf, G_ASCII_DTOSTR_BUF_SIZE, brush->hardness));

  /* write brush aspect_ratio */
  g_string_append_printf(
      string, "%s\n",
      g_ascii_dtostr(buf, G_ASCII_DTOSTR_BUF_SIZE, brush->aspect_ratio));

  /* write brush angle */
  g_string_append_printf(
      string, "%s\n",
      g_ascii_dtostr(buf, G_ASCII_DTOSTR_BUF_SIZE, brush->angle));
