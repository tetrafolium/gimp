...
static void compute_offsets(GList *list, GimpAlignmentType alignment);
static void compute_offset(GObject *object, GimpAlignmentType alignment);
static gint offset_compare(gconstpointer a, gconstpointer b);

/**
 * gimp_image_arrange_objects:
 * @image:                The #GimpImage to which the objects belong.
 * @list:                 A #GList of objects to be aligned.
 * @alignment:            The point on each target object to bring into
 * alignment.
 * @reference:            The #GObject to align the targets with, or %NULL.
 * @reference_alignment:  The point on the reference object to align the target
 * item with..
...
void gimp_image_arrange_objects(GimpImage *image, GList *list,
  ...
  if (object_list) {
    ...
    if (reference_alignment == GIMP_ARRANGE_HFILL) {
      distr_width =
          GPOINTER_TO_INT(g_object_get_data(reference, "align-width"));
      /* The offset parameter works as an internal margin */
      fill_offset =
          (distr_width - 2 * offset) / (gint)g_list_length(object_list);
    }
    if (reference_alignment == GIMP_ARRANGE_VFILL) {
      distr_height =
          GPOINTER_TO_INT(g_object_get_data(reference, "align-height"));
      fill_offset =
          (distr_height - 2 * offset) / (gint)g_list_length(object_list);
    }
    ...
    for (list = object_list, n = 1; list; list = g_list_next(list), n++) {
      ...
      if (reference_alignment == GIMP_ARRANGE_HFILL) {
        gint width = GPOINTER_TO_INT(g_object_get_data(target, "align-width"));
        xtranslate =
            ROUND(z0 - z1 + (n - 0.5) * fill_offset - width / 2.0 + offset);
      } else if (reference_alignment == GIMP_ARRANGE_VFILL) {
        gint height =
            GPOINTER_TO_INT(g_object_get_data(target, "align-height"));
        ytranslate =
            ROUND(z0 - z1 + (n - 0.5) * fill_offset - height / 2.0 + offset);
      ...
      if (GIMP_IS_ITEM(target)) {
        gimp_item_translate(GIMP_ITEM(target), xtranslate, ytranslate, TRUE);
...
static GList *sort_by_offset(GList *list) {
  return g_list_sort(g_list_copy(list), offset_compare);
}

static gint offset_compare(gconstpointer a, gconstpointer b) {
  gint offset1 =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(a), "align-offset"));
  gint offset2 =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(b), "align-offset"));
...
static void compute_offset(GObject *object, GimpAlignmentType alignment) {
  ...
  if (GIMP_IS_IMAGE(object)) {
    ...
    gimp_item_bounds(item, &object_offset_x, &object_offset_y, &object_width,
                     &object_height);
  ...
  g_object_set_data(object, "align-offset", GINT_TO_POINTER(offset));
   ...
   */
  g_object_set_data(object, "align-width", GINT_TO_POINTER(object_width));

  g_object_set_data(object, "align-height", GINT_TO_POINTER(object_height));
}
