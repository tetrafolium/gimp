...
enum { PROP_0, PROP_TIME };

static void gimp_settings_get_property(GObject *object, guint property_id,
                                       GValue *value, GParamSpec *pspec);
static void gimp_settings_set_property(GObject *object, guint property_id,
...
static void gimp_settings_class_init(GimpSettingsClass *klass) {
  ...
  GIMP_CONFIG_PROP_INT64(object_class, PROP_TIME, "time", "Time",
                         "Time of settings creation", 0, G_MAXINT64, 0,
                         GIMP_CONFIG_PARAM_DONT_COMPARE);
}

static void gimp_settings_init(GimpSettings *settings) {}
...
static gchar *gimp_settings_get_description(GimpViewable *viewable,
  ...
  if (settings->time > 0) {
    ...
    return g_strdup_printf(_("Last used: %s"), gimp_object_get_name(settings));
  }
...
gint gimp_settings_compare(GimpSettings *a, GimpSettings *b) {
