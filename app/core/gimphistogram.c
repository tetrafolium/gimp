...
enum { PROP_0, PROP_N_COMPONENTS, PROP_N_BINS, PROP_VALUES };
...
static void gimp_histogram_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_histogram_get_property(GObject *object, guint property_id,
                                        GValue *value, GParamSpec *pspec);

static gint64 gimp_histogram_get_memsize(GimpObject *object, gint64 *gui_size);
...
static void gimp_histogram_class_init(GimpHistogramClass *klass) {
  ...
  g_object_class_install_property(object_class, PROP_N_BINS,
                                  g_param_spec_int("n-bins", NULL, NULL, 256,
                                                   1024, 1024,
                                                   GIMP_PARAM_READABLE));

  /* this is just for notifications */
  g_object_class_install_property(
      object_class, PROP_VALUES,
      g_param_spec_boolean("values", NULL, NULL, FALSE, G_PARAM_READABLE));
}

static void gimp_histogram_init(GimpHistogram *histogram) {
...
static gint64 gimp_histogram_get_memsize(GimpObject *object, gint64 *gui_size) {
  ...
  if (histogram->priv->values)
    memsize += (histogram->priv->n_channels * histogram->priv->n_bins *
                sizeof(gdouble));
...
GimpHistogram *gimp_histogram_duplicate(GimpHistogram *histogram) {
  ...
  dup->priv->values =
      g_memdup(histogram->priv->values,
               sizeof(gdouble) * dup->priv->n_channels * dup->priv->n_bins);

  return dup;
}

void gimp_histogram_calculate(GimpHistogram *histogram, GeglBuffer *buffer,
  ...
  gimp_histogram_set_values(histogram, context.n_components, context.n_bins,
                            context.values);
}
...
GimpAsync *gimp_histogram_calculate_async(GimpHistogram *histogram,
  ...
  context->histogram = histogram;
  context->buffer = gegl_buffer_new(&rect, gegl_buffer_get_format(buffer));
  context->buffer_rect = *buffer_rect;

  gimp_gegl_buffer_copy(buffer, &rect, GEGL_ABYSS_NONE, context->buffer, NULL);
  ...
  if (mask) {
    ...
    gimp_gegl_buffer_copy(mask, &rect, GEGL_ABYSS_NONE, context->mask, NULL);
  }

  histogram->priv->calculate_async = gimp_parallel_run_async(
      (GimpRunAsyncFunc)gimp_histogram_calculate_internal, context);

  gimp_async_add_callback(
      histogram->priv->calculate_async,
      (GimpAsyncCallback)gimp_histogram_calculate_async_callback, context);

  return histogram->priv->calculate_async;
}

void gimp_histogram_clear_values(GimpHistogram *histogram, gint n_components) {
  g_return_if_fail(GIMP_IS_HISTOGRAM(histogram));
...
gdouble gimp_histogram_get_maximum(GimpHistogram *histogram,
  ...
  if (!priv->values || !gimp_histogram_map_channel(histogram, &channel)) {
    return 0.0;
  }
...
gdouble gimp_histogram_get_value(GimpHistogram *histogram,
                                 GimpHistogramChannel channel, gint bin) {
  GimpHistogramPrivate *priv;
  ...
  if (!priv->values || (bin < 0 || bin >= priv->n_bins) ||
...
gdouble gimp_histogram_get_component(GimpHistogram *histogram, gint component,
                                     gint bin) {
  g_return_val_if_fail(GIMP_IS_HISTOGRAM(histogram), 0.0);
...
gdouble gimp_histogram_get_count(GimpHistogram *histogram,
                                 GimpHistogramChannel channel, gint start,
                                 gint end) {
  GimpHistogramPrivate *priv;
  gint i;
  gdouble count = 0.0;
  ...
  if (channel == GIMP_HISTOGRAM_RGB)
    return (
        gimp_histogram_get_count(histogram, GIMP_HISTOGRAM_RED, start, end) +
        gimp_histogram_get_count(histogram, GIMP_HISTOGRAM_GREEN, start, end) +
        gimp_histogram_get_count(histogram, GIMP_HISTOGRAM_BLUE, start, end));
...
static void gimp_histogram_calculate_internal(GimpAsync *async,
  ...
  case GIMP_INDEXED:
    format = gimp_babl_format(
        GIMP_RGB, gimp_babl_precision(GIMP_COMPONENT_TYPE_FLOAT, priv->trc),
        babl_format_has_alpha(format), space);
    break;

  case GIMP_GRAY:
    format = gimp_babl_format(
        GIMP_GRAY, gimp_babl_precision(GIMP_COMPONENT_TYPE_FLOAT, priv->trc),
        babl_format_has_alpha(format), space);
    break;
  ...
  gegl_parallel_distribute_area(
      ...
      (GeglParallelDistributeAreaFunc)gimp_histogram_calculate_area, &data);

  if (!async || !gimp_async_is_canceled(async)) {
    gdouble *total_values = NULL;
    gint n_values =
        (context->n_components + N_DERIVED_CHANNELS) * context->n_bins;
    GSList *iter;
...
static void gimp_histogram_calculate_area(const GeglRectangle *area,
  ...
  iter = gegl_buffer_iterator_new(context->buffer, area, 0, data->format,
...
#define VALUE(c, i)                                                            \
  (*(temp = (i)*n_bins_1f,                                                     \
     &values[(c)*n_bins + SIGNED_ROUND(SAFE_CLAMP(temp, 0.0f, n_bins_1f))]))
...
static void gimp_histogram_calculate_async_callback(GimpAsync *async,
  ...
  if (gimp_async_is_finished(async)) {
    gimp_histogram_set_values(context->histogram, context->n_components,
                              context->n_bins, context->values);
  }
