...
GimpTempBuf *gimp_drawable_get_new_preview(GimpViewable *viewable,
  ...
  return gimp_drawable_get_sub_preview(
      GIMP_DRAWABLE(viewable), 0, 0, gimp_item_get_width(item),
      gimp_item_get_height(item), width, height);
}
...
GdkPixbuf *gimp_drawable_get_new_pixbuf(GimpViewable *viewable,
  ...
  return gimp_drawable_get_sub_pixbuf(
      GIMP_DRAWABLE(viewable), 0, 0, gimp_item_get_width(item),
      gimp_item_get_height(item), width, height);
}
...
GimpTempBuf *gimp_drawable_get_sub_preview(GimpDrawable *drawable, gint src_x,
                                           gint src_y, gint src_width,
                                           gint src_height, gint dest_width,
                                           gint dest_height) {
  GimpItem *item;
  GimpImage *image;
  GeglBuffer *buffer;
  ...
  g_return_val_if_fail((src_y + src_height) <= gimp_item_get_height(item),
                       NULL);
  ...
  gegl_buffer_get(
      buffer, GEGL_RECTANGLE(scaled_x, scaled_y, dest_width, dest_height),
      scale, gimp_temp_buf_get_format(preview), gimp_temp_buf_get_data(preview),
      GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_CLAMP);

  return preview;
}

GdkPixbuf *gimp_drawable_get_sub_pixbuf(GimpDrawable *drawable, gint src_x,
                                        gint src_y, gint src_width,
                                        gint src_height, gint dest_width,
                                        gint dest_height) {
  GimpItem *item;
  ...
  if (transform) {
    ...
    gimp_color_transform_process_buffer(
        transform, src_buf, GEGL_RECTANGLE(0, 0, dest_width, dest_height),
    ...
    gegl_buffer_get(
        buffer, GEGL_RECTANGLE(scaled_x, scaled_y, dest_width, dest_height),
        scale, gimp_pixbuf_get_format(pixbuf), gdk_pixbuf_get_pixels(pixbuf),
...
static void gimp_drawable_get_sub_preview_async_func(GimpAsync *async,
                                                     SubPreviewData *data) {
  GimpTempBuf *preview;
  GimpTileHandlerValidate *validate;

  preview =
      gimp_temp_buf_new(data->rect.width, data->rect.height, data->format);
  ...
  if (validate) {
    if (!data->iter) {
      ...
      rect.width =
          ceil((data->rect.x + data->rect.width) / data->scale) - rect.x;
      rect.height =
          ceil((data->rect.x + data->rect.height) / data->scale) - rect.y;
    ...
    if (gimp_chunk_iterator_next(data->iter)) {
      ...
      while (gimp_chunk_iterator_get_rect(data->iter, &rect)) {
        gimp_tile_handler_validate_validate(validate, data->buffer, &rect,
                                            FALSE, FALSE);
      }
  ...
  gegl_buffer_get(
      data->buffer, &data->rect, data->scale, gimp_temp_buf_get_format(preview),
      gimp_temp_buf_get_data(preview), GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_CLAMP);

  sub_preview_data_free(data);

  gimp_async_finish_full(async, preview, (GDestroyNotify)gimp_temp_buf_unref);
}
...
GimpAsync *gimp_drawable_get_sub_preview_async(GimpDrawable *drawable,
  ...
  if (no_async_drawable_previews) {
    ...
    gimp_async_finish_full(
        async,
        gimp_drawable_get_sub_preview(drawable, src_x, src_y, src_width,
                                      src_height, dest_width, dest_height),
  ...
  data = sub_preview_data_new(
      gimp_drawable_get_preview_format(drawable), buffer,
      GEGL_RECTANGLE(scaled_x, scaled_y, dest_width, dest_height), scale);

  if (gimp_tile_handler_validate_get_assigned(buffer)) {
    return gimp_idle_run_async_full(
        GIMP_PRIORITY_VIEWABLE_IDLE,
        (GimpRunAsyncFunc)gimp_drawable_get_sub_preview_async_func, data,
        (GDestroyNotify)sub_preview_data_free);
  } else {
    return gimp_parallel_run_async_full(
        +1, (GimpRunAsyncFunc)gimp_drawable_get_sub_preview_async_func, data,
        (GDestroyNotify)sub_preview_data_free);
  }
}
