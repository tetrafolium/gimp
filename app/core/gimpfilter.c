...
enum { ACTIVE_CHANGED, LAST_SIGNAL };

enum { PROP_0, PROP_ACTIVE, PROP_IS_LAST_NODE };
...
#define GET_PRIVATE(filter)                                                    \
  ((GimpFilterPrivate *)gimp_filter_get_instance_private(                      \
      (GimpFilter *)(filter)))

/*  local function prototypes  */

static void gimp_filter_finalize(GObject *object);
static void gimp_filter_set_property(GObject *object, guint property_id,
                                     const GValue *value, GParamSpec *pspec);
static void gimp_filter_get_property(GObject *object, guint property_id,
                                     GValue *value, GParamSpec *pspec);

static gint64 gimp_filter_get_memsize(GimpObject *object, gint64 *gui_size);
...
static void gimp_filter_class_init(GimpFilterClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  GimpObjectClass *gimp_object_class = GIMP_OBJECT_CLASS(klass);

  gimp_filter_signals[ACTIVE_CHANGED] = g_signal_new(
      "active-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpFilterClass, active_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("active", NULL, NULL, TRUE, GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_IS_LAST_NODE,
                                  g_param_spec_boolean("is-last-node", NULL,
                                                       NULL, FALSE,
                                                       GIMP_PARAM_READWRITE));
}

static void gimp_filter_init(GimpFilter *filter) {
...
GimpFilter *gimp_filter_new(const gchar *name) {
  ...
  return g_object_new(GIMP_TYPE_FILTER, "name", name, NULL);
}
...
void gimp_filter_set_active(GimpFilter *filter, gboolean active) {
...
void gimp_filter_set_is_last_node(GimpFilter *filter, gboolean is_last_node) {
  g_return_if_fail(GIMP_IS_FILTER(filter));
