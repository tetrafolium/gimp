...
gint64 gimp_g_hash_table_get_memsize(GHashTable *hash, gint64 data_size) {
  if (hash)
    return (2 * sizeof(gint) + 5 * sizeof(gpointer) +
...
static void hash_memsize_foreach(gpointer key, gpointer value,
                                 HashMemsize *memsize) {
  gint64 gui_size = 0;
...
gint64 gimp_g_slist_get_memsize_foreach(GSList *slist, GimpMemsizeFunc func,
                                        gint64 *gui_size) {
  GSList *l;
  gint64 memsize = 0;
...
gint64 gimp_g_list_get_memsize_foreach(GList *list, GimpMemsizeFunc func,
                                       gint64 *gui_size) {
  GList *l;
  gint64 memsize = 0;
...
gint64 gimp_g_queue_get_memsize(GQueue *queue, gint64 data_size) {
  if (queue) {
...
gint64 gimp_g_queue_get_memsize_foreach(GQueue *queue, GimpMemsizeFunc func,
                                        gint64 *gui_size) {
  gint64 memsize = 0;
...
gint64 gimp_g_value_get_memsize(GValue *value) {
  ...
  if (G_VALUE_HOLDS_STRING(value)) {
    ...
    if (GIMP_VALUE_HOLDS_RGB(value)) {
      ...
      memsize += gimp_parasite_get_memsize(g_value_get_boxed(value), NULL);
...
gint64 gimp_parasite_get_memsize(GimpParasite *parasite, gint64 *gui_size) {
  if (parasite)
    return (sizeof(GimpParasite) + gimp_string_get_memsize(parasite->name) +
            parasite->size);
