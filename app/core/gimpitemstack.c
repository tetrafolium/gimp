...
static void gimp_item_stack_add(GimpContainer *container, GimpObject *object);
...
static void gimp_item_stack_class_init(GimpItemStackClass *klass) {
...
static void gimp_item_stack_init(GimpItemStack *stack) {}
...
static void gimp_item_stack_constructed(GObject *object) {
  ...
  gimp_assert(
      g_type_is_a(gimp_container_get_children_type(container), GIMP_TYPE_ITEM));
}
...
GimpContainer *gimp_item_stack_new(GType item_type) {
  ...
  return g_object_new(GIMP_TYPE_ITEM_STACK, "name", g_type_name(item_type),
                      "children-type", item_type, "policy",
...
GimpItem *gimp_item_stack_get_item_by_tattoo(GimpItemStack *stack,
  ...
  for (list = GIMP_LIST(stack)->queue->head; list; list = g_list_next(list)) {
    ...
    if (children) {
      item =
          gimp_item_stack_get_item_by_tattoo(GIMP_ITEM_STACK(children), tattoo);
...
GimpItem *gimp_item_stack_get_item_by_path(GimpItemStack *stack, GList *path) {
...
GimpItem *gimp_item_stack_get_parent_by_path(GimpItemStack *stack, GList *path,
                                             gint *index) {
...
static void gimp_item_stack_viewable_profile_changed(GimpViewable *viewable) {
  GimpContainer *children = gimp_viewable_get_children(viewable);
...
void gimp_item_stack_profile_changed(GimpItemStack *stack) {
  ...
  gimp_container_foreach(GIMP_CONTAINER(stack),
                         (GFunc)gimp_item_stack_viewable_profile_changed, NULL);
}
