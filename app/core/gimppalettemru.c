...
enum { COLOR_HISTORY = 1 };

G_DEFINE_TYPE(GimpPaletteMru, gimp_palette_mru, GIMP_TYPE_PALETTE)

#define parent_class gimp_palette_mru_parent_class

static void gimp_palette_mru_class_init(GimpPaletteMruClass *klass) {}

static void gimp_palette_mru_init(GimpPaletteMru *palette) {}
...
GimpData *gimp_palette_mru_new(const gchar *name) {
  ...
  palette = g_object_new(GIMP_TYPE_PALETTE_MRU, "name", name, "mime-type",
...
void gimp_palette_mru_save(GimpPaletteMru *mru, GFile *file) {
  ...
  writer = gimp_config_writer_new_from_file(file, TRUE,
                                            "GIMP colorrc\n\n"
  ...
  for (list = palette->colors; list; list = g_list_next(list)) {
    ...
    gimp_config_writer_printf(writer, "%s %s %s %s", buf[0], buf[1], buf[2],
                              buf[3]);
...
void gimp_palette_mru_add(GimpPaletteMru *mru, const GimpRGB *color) {
  ...
  for (list = gimp_palette_get_colors(palette); list;
  ...
  if (gimp_palette_get_n_colors(palette) == MAX_N_COLORS) {
    gimp_palette_delete_entry(
        palette, gimp_palette_get_entry(palette, MAX_N_COLORS - 1));
  }
