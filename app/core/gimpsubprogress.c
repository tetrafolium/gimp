...
static void gimp_sub_progress_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_sub_progress_get_property(GObject *object, guint property_id,
...
static gboolean gimp_sub_progress_message(GimpProgress *progress, Gimp *gimp,
...
static void gimp_sub_progress_class_init(GimpSubProgressClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_PROGRESS,
      g_param_spec_object("progress", NULL, NULL, GIMP_TYPE_PROGRESS,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_sub_progress_init(GimpSubProgress *sub) {
  sub->progress = NULL;
  sub->start = 0.0;
  sub->end = 1.0;
}
...
static void gimp_sub_progress_end(GimpProgress *progress) { /* does nothing */ }
...
static gboolean gimp_sub_progress_message(GimpProgress *progress, Gimp *gimp,
  ...
  if (sub->progress)
    return gimp_progress_message(sub->progress, gimp, severity, domain,
                                 message);
...
GimpProgress *gimp_sub_progress_new(GimpProgress *progress) {
  ...
  sub = g_object_new(GIMP_TYPE_SUB_PROGRESS, "progress", progress, NULL);
 ...
 * mapped to.
 */
void gimp_sub_progress_set_range(GimpSubProgress *progress, gdouble start,
                                 gdouble end) {
 ...
 */
void gimp_sub_progress_set_step(GimpSubProgress *progress, gint index,
                                gint num_steps) {
  g_return_if_fail(GIMP_IS_SUB_PROGRESS(progress));
