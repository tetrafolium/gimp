...
static gboolean gimp_channel_combine_start(GimpChannel *mask, GimpChannelOps op,
                                           const GeglRectangle *rect,
...
static void gimp_channel_combine_clear(GimpChannel *mask,
  ...
  if (rect) {
    ...
    if (mask->bounds_known) {
      if (!gegl_rectangle_intersect(&area,
                                    GEGL_RECTANGLE(mask->x1, mask->y1,
                                                   mask->x2 - mask->x1,
                                                   mask->y2 - mask->y1),
                                    rect)) {
        return;
  ...
  gimp_drawable_update(GIMP_DRAWABLE(mask), update_area.x, update_area.y,
...
static void gimp_channel_combine_clear_complement(GimpChannel *mask,
  ...
  gimp_channel_combine_clear(mask, GEGL_RECTANGLE(0, 0, width, rect->y));

  gimp_channel_combine_clear(mask,
                             GEGL_RECTANGLE(0, rect->y + rect->height, width,
                                            height - (rect->y + rect->height)));

  gimp_channel_combine_clear(mask,
                             GEGL_RECTANGLE(0, rect->y, rect->x, rect->height));

  gimp_channel_combine_clear(
      mask, GEGL_RECTANGLE(rect->x + rect->width, rect->y,
                           width - (rect->x + rect->width), rect->height));
}
...
static gboolean gimp_channel_combine_start(GimpChannel *mask, GimpChannelOps op,
  ...
  case GIMP_CHANNEL_OP_ADD:
    ...
    if (full_extent &&
        (mask->bounds_known || gegl_rectangle_equal(&data->rect, &extent))) {
      data->bounds_known = TRUE;
      data->empty = FALSE;

      if (mask->bounds_known && !mask->empty) {
        gegl_rectangle_bounding_box(&data->bounds, &data->bounds, &data->rect);
      } else {
        data->bounds = data->rect;
      }
    }
  ...
  case GIMP_CHANNEL_OP_SUBTRACT:
    if (intersects && mask->bounds_known) {
      if (mask->empty) {
        ...
        intersects =
            gegl_rectangle_intersect(&data->rect, &data->rect, &data->bounds);
      }
    }
    ...
    if (full_value &&
        gegl_rectangle_contains(&data->rect,
                                mask->bounds_known ? &data->bounds : &extent)) {
      gimp_channel_combine_clear(mask, NULL);
...
static void gimp_channel_combine_end(GimpChannel *mask,
  ...
  gimp_drawable_update(GIMP_DRAWABLE(mask), data->rect.x, data->rect.y,
...
void gimp_channel_combine_rect(GimpChannel *mask, GimpChannelOps op, gint x,
  ...
  if (gimp_channel_combine_start(mask, op, GEGL_RECTANGLE(x, y, w, h), TRUE,
                                 TRUE, &data)) {
    GeglBuffer *buffer = gimp_drawable_get_buffer(GIMP_DRAWABLE(mask));
...
void gimp_channel_combine_ellipse(GimpChannel *mask, GimpChannelOps op, gint x,
  ...
  gimp_channel_combine_ellipse_rect(mask, op, x, y, w, h, w / 2.0, h / 2.0,
                                    antialias);
}
...
void gimp_channel_combine_ellipse_rect(GimpChannel *mask, GimpChannelOps op,
  ...
  if (gimp_channel_combine_start(mask, op, GEGL_RECTANGLE(x, y, w, h), TRUE,
    ...
    gimp_gegl_mask_combine_ellipse_rect(buffer, op, x, y, w, h, rx, ry,
                                        antialias);
  }
...
void gimp_channel_combine_mask(GimpChannel *mask, GimpChannel *add_on,
  ...
  gimp_channel_combine_buffer(mask, add_on_buffer, op, off_x, off_y);
}
...
void gimp_channel_combine_buffer(GimpChannel *mask, GeglBuffer *add_on_buffer,
  ...
  if (gimp_channel_combine_start(
          mask, op,
          GEGL_RECTANGLE(off_x + gegl_buffer_get_x(add_on_buffer),
    ...
    gimp_gegl_mask_combine_buffer(buffer, add_on_buffer, op, off_x, off_y);
  }
...
void gimp_channel_combine_items(GimpChannel *mask, GList *items,
  ...
  for (iter = items; iter; iter = iter->next) {
    g_return_if_fail(GIMP_IS_LAYER(iter->data) || GIMP_IS_CHANNEL(iter->data) ||
  ...
  if (g_list_length(layers) > 1) {
    ...
    merged_layers = gimp_image_merge_visible_layers(
        temp_image, gimp_get_user_context(temp_image->gimp), GIMP_CLIP_TO_IMAGE,
        FALSE, TRUE, NULL);
    g_return_if_fail(g_list_length(merged_layers) == 1);
    gimp_item_to_selection(GIMP_ITEM(merged_layers->data),
                           GIMP_CHANNEL_OP_REPLACE, TRUE, FALSE, 0.0, 0.0);
    channel = gimp_image_get_mask(temp_image);
  }

  if (!channel) {
    channel = gimp_channel_new(image, gimp_image_get_width(image),
                               gimp_image_get_height(image), NULL, NULL);
    ...
    if (g_list_length(layers) == 1) {
      if (gimp_drawable_has_alpha(layers->data)) {
        ...
        alpha = gimp_channel_new_from_alpha(image, layers->data, NULL, NULL);
        gimp_item_get_offset(layers->data, &offset_x, &offset_y);
        gimp_channel_combine_mask(channel, alpha, GIMP_CHANNEL_OP_REPLACE,
                                  offset_x, offset_y);
  ...
  for (iter = items; iter; iter = iter->next) {
    if (!GIMP_IS_LAYER(iter->data)) {
      ...
      gimp_channel_combine_buffer(
          channel, gimp_drawable_get_buffer(GIMP_DRAWABLE(iter->data)),
          GIMP_CHANNEL_OP_ADD, offset_x, offset_y);
    }
  }

  gimp_channel_combine_buffer(
      mask, gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)), op, 0, 0);
