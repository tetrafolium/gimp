...
static gboolean gimp_param_string_validate(GParamSpec *pspec, GValue *value);
...
GType gimp_param_string_get_type(void) {
  ...
  if (!type) {
    ...
    type = g_type_register_static(G_TYPE_PARAM_STRING, "GimpParamString", &info,
                                  0);
  }
 ...
 * Since: 3.0
 **/
GParamSpec *gimp_param_spec_string(const gchar *name, const gchar *nick,
                                   const gchar *blurb, gboolean allow_non_utf8,
                                   gboolean null_ok, gboolean non_empty,
  ...
  sspec =
      g_param_spec_internal(GIMP_TYPE_PARAM_STRING, name, nick, blurb, flags);
...
static gboolean gimp_param_enum_validate(GParamSpec *pspec, GValue *value);
...
GType gimp_param_enum_get_type(void) {
  ...
  if (!type) {
    ...
    type = g_type_register_static(G_TYPE_PARAM_ENUM, "GimpParamEnum", &info, 0);
  }
...
GParamSpec *gimp_param_spec_enum(const gchar *name, const gchar *nick,
  ...
  espec = g_param_spec_internal(GIMP_TYPE_PARAM_ENUM, name, nick, blurb, flags);
...
void gimp_param_spec_enum_exclude_value(GimpParamSpecEnum *espec, gint value) {
  g_return_if_fail(GIMP_IS_PARAM_SPEC_ENUM(espec));
  g_return_if_fail(
      g_enum_get_value(G_PARAM_SPEC_ENUM(espec)->enum_class, value) != NULL);

  espec->excluded_values =
      g_slist_prepend(espec->excluded_values, GINT_TO_POINTER(value));
}
