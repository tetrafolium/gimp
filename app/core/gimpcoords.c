...
void gimp_coords_mix(const gdouble amul, const GimpCoords *a,
                     const gdouble bmul, const GimpCoords *b,
                     GimpCoords *ret_val) {
  if (b) {
...
void gimp_coords_average(const GimpCoords *a, const GimpCoords *b,
...
void gimp_coords_add(const GimpCoords *a, const GimpCoords *b,
                     GimpCoords *ret_add) {
...
void gimp_coords_difference(const GimpCoords *a, const GimpCoords *b,
...
void gimp_coords_scale(const gdouble f, const GimpCoords *a,
...
gdouble gimp_coords_scalarprod(const GimpCoords *a, const GimpCoords *b) {
  return (a->x * b->x + a->y * b->y + a->pressure * b->pressure +
          a->xtilt * b->xtilt + a->ytilt * b->ytilt + a->wheel * b->wheel +
          a->distance * b->distance + a->rotation * b->rotation +
          a->slider * b->slider + a->velocity * b->velocity +
...
gdouble gimp_coords_manhattan_dist(const GimpCoords *a, const GimpCoords *b) {
  gdouble dist = 0;
...
gboolean gimp_coords_equal(const GimpCoords *a, const GimpCoords *b) {
  return (a->x == b->x && a->y == b->y && a->pressure == b->pressure &&
          a->xtilt == b->xtilt && a->ytilt == b->ytilt &&
          a->wheel == b->wheel && a->distance == b->distance &&
          a->rotation == b->rotation && a->slider == b->slider &&
          a->velocity == b->velocity && a->direction == b->direction);
...
gdouble gimp_coords_direction(const GimpCoords *a, const GimpCoords *b) {
  gdouble direction;
