...
GimpGuide *gimp_image_add_hguide(GimpImage *image, gint position,
  ...
  g_return_val_if_fail(
      position >= 0 && position <= gimp_image_get_height(image), NULL);

  guide =
      gimp_guide_new(GIMP_ORIENTATION_HORIZONTAL, image->gimp->next_guide_id++);

  if (push_undo)
    gimp_image_undo_push_guide(image, C_("undo-type", "Add Horizontal Guide"),
                               guide);
...
GimpGuide *gimp_image_add_vguide(GimpImage *image, gint position,
  ...
  g_return_val_if_fail(GIMP_IS_IMAGE(image), NULL);
  g_return_val_if_fail(position >= 0 && position <= gimp_image_get_width(image),
                       NULL);

  guide =
      gimp_guide_new(GIMP_ORIENTATION_VERTICAL, image->gimp->next_guide_id++);

  if (push_undo)
    gimp_image_undo_push_guide(image, C_("undo-type", "Add Vertical Guide"),
                               guide);
...
private
  ->guides = g_list_prepend(private->guides, guide);
...
private
  ->guides = g_list_remove(private->guides, guide);
...
GimpGuide *gimp_image_get_guide(GimpImage *image, guint32 id) {
  ...
  for (guides = GIMP_IMAGE_GET_PRIVATE(image)->guides; guides;
...
GimpGuide *gimp_image_get_next_guide(GimpImage *image, guint32 id,
  ...
  for (guides = GIMP_IMAGE_GET_PRIVATE(image)->guides; guides;
    ...
    if (gimp_aux_item_get_id(GIMP_AUX_ITEM(guide)) ==
