...
static gboolean gimp_tool_info_filter_preset(GimpObject *object,
                                             gpointer user_data) {
...
GimpToolInfo *
gimp_tool_info_new(Gimp *gimp, GType tool_type, GType tool_options_type,
                   GimpContextPropMask context_props, const gchar *identifier,
                   const gchar *label, const gchar *tooltip,
                   const gchar *menu_label, const gchar *menu_accel,
                   const gchar *help_domain, const gchar *help_id,
                   const gchar *paint_core_name, const gchar *icon_name) {
  ...
  paint_info = (GimpPaintInfo *)gimp_container_get_child_by_name(
      gimp->paint_info_list, paint_core_name);

  g_return_val_if_fail(GIMP_IS_PAINT_INFO(paint_info), NULL);

  tool_info = g_object_new(GIMP_TYPE_TOOL_INFO, "name", identifier, "icon-name",
                           icon_name, NULL);
  ...
  if (tool_info->tool_options_type == paint_info->paint_options_type) {
    tool_info->tool_options =
        g_object_ref(GIMP_TOOL_OPTIONS(paint_info->paint_options));
  } else {
    tool_info->tool_options = g_object_new(tool_info->tool_options_type, "gimp",
                                           gimp, "name", identifier, NULL);
  }

  g_object_set(tool_info->tool_options, "tool", tool_info, "tool-info",
               tool_info, NULL);
  ...
  if (tool_info->tool_options_type != GIMP_TYPE_TOOL_OPTIONS) {
    ...
    tool_info->presets = gimp_filtered_container_new(
        presets, gimp_tool_info_filter_preset, tool_info);
  }
