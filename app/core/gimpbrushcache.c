...
static void gimp_brush_cache_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_brush_cache_get_property(GObject *object, guint property_id,
...
static void gimp_brush_cache_class_init(GimpBrushCacheClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_DATA_DESTROY,
      g_param_spec_pointer("data-destroy", NULL, NULL,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_brush_cache_init(GimpBrushCache *brush) {}
...
GimpBrushCache *gimp_brush_cache_new(GDestroyNotify data_destroy,
  ...
  cache =
      g_object_new(GIMP_TYPE_BRUSH_CACHE, "data-destroy", data_destroy, NULL);
...
gconstpointer gimp_brush_cache_get(GimpBrushCache *cache, gint width,
  ...
  for (iter = cache->cached_units; iter; iter = g_list_next(iter)) {
    ...
    if (unit->data && unit->width == width && unit->height == height &&
        unit->scale == scale && unit->aspect_ratio == aspect_ratio &&
        unit->angle == angle && unit->reflect == reflect &&
