...
void gimp_image_set_use_srgb_profile(GimpImage *image, gboolean use_srgb) {
  GimpImagePrivate *private;
...
void _gimp_image_set_hidden_profile(GimpImage *image, GimpColorProfile *profile,
                                    gboolean push_undo) {
  GimpImagePrivate *private;
...
gboolean gimp_image_validate_icc_parasite(GimpImage *image,
  ...
  if (gimp_parasite_flags(icc_parasite) !=
  ...
  return gimp_image_validate_icc_profile(
      image, gimp_parasite_data(icc_parasite),
      gimp_parasite_data_size(icc_parasite), is_builtin, error);
}
...
void gimp_image_set_icc_parasite(GimpImage *image,
  ...
  if (icc_parasite) {
    g_return_if_fail(gimp_image_validate_icc_parasite(image, icc_parasite, NULL,
                                                      NULL) == TRUE);
...
gboolean gimp_image_validate_icc_profile(GimpImage *image, const guint8 *data,
                                         gsize length, gboolean *is_builtin,
                                         GError **error) {
...
const guint8 *gimp_image_get_icc_profile(GimpImage *image, gsize *length) {
...
gboolean gimp_image_set_icc_profile(GimpImage *image, const guint8 *data,
  ...
  if (data) {
    ...
    parasite = gimp_parasite_new(
        ...
        GIMP_PARASITE_PERSISTENT | GIMP_PARASITE_UNDOABLE, length, data);
...
gboolean gimp_image_validate_color_profile(GimpImage *image,
  ...
  return gimp_image_validate_color_profile_by_format(format, profile,
                                                     is_builtin, error);
}
...
gboolean gimp_image_validate_color_profile_by_format(const Babl *format,
                                                     GimpColorProfile *profile,
                                                     gboolean *is_builtin,
                                                     GError **error) {
  ...
  if (is_builtin) {
    GimpColorProfile *builtin = gimp_babl_get_builtin_color_profile(
...
GimpColorProfile *gimp_image_get_builtin_color_profile(GimpImage *image) {
  const Babl *format;
  ...
  return gimp_babl_get_builtin_color_profile(
      gimp_babl_format_get_base_type(format), gimp_babl_format_get_trc(format));
}
...
gboolean gimp_image_assign_color_profile(GimpImage *image,
  ...
  g_return_val_if_fail(
      dest_profile == NULL || GIMP_IS_COLOR_PROFILE(dest_profile), FALSE);
  ...
  if (progress)
    gimp_progress_start(progress, FALSE,
                        dest_profile ? _("Assigning color profile")
                                     : _("Discarding color profile"));

  gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_IMAGE_CONVERT,
                              dest_profile ? _("Assign color profile")
...
gboolean gimp_image_convert_color_profile(GimpImage *image,
                                          GimpColorProfile *dest_profile,
                                          GimpColorRenderingIntent intent,
                                          gboolean bpc, GimpProgress *progress,
                                          GError **error) {
  GimpColorProfile *src_profile;
  ...
  if (progress)
    gimp_progress_start(progress, FALSE, _("Converting from '%s' to '%s'"),
  ...
  case GIMP_RGB:
  case GIMP_GRAY:
    gimp_image_convert_profile_layers(image, src_profile, dest_profile, intent,
                                      bpc, progress);
    break;

  case GIMP_INDEXED:
    gimp_image_convert_profile_colormap(image, src_profile, dest_profile,
                                        intent, bpc, progress);
    gimp_image_fix_layer_format_spaces(image, progress);
    break;
  }
...
void gimp_image_import_color_profile(GimpImage *image, GimpContext *context,
                                     GimpProgress *progress,
  ...
  if (gimp_image_get_color_profile(image)) {
    ...
    if (policy == GIMP_COLOR_PROFILE_POLICY_ASK) {
      if (interactive) {
        ...
        policy =
            gimp_query_profile_policy(image->gimp, image, context,
                                      &dest_profile, &intent, &bpc, &dont_ask);

        if (dont_ask) {
          g_object_set(G_OBJECT(image->gimp->config), "color-profile-policy",
                       policy, NULL);
        }
      } else {
...
void gimp_image_color_profile_pixel_to_srgb(GimpImage *image,
  ...
  if (transform) {
    gimp_color_transform_process_pixels(transform, pixel_format, pixel,
...
void gimp_image_color_profile_srgb_to_pixel(GimpImage *image,
                                            const GimpRGB *color,
                                            const Babl *pixel_format,
                                            gpointer pixel) {
  GimpColorTransform *transform;
...
private
  ->color_transforms_created = FALSE;
}
...
void _gimp_image_update_color_profile(GimpImage *image,
  ...
  private
    ->color_profile = gimp_color_profile_new_from_icc_profile(
        gimp_parasite_data(icc_parasite), gimp_parasite_data_size(icc_parasite),
        NULL);

  private
    ->layer_space = gimp_color_profile_get_space(
        private->color_profile,
        GIMP_COLOR_RENDERING_INTENT_RELATIVE_COLORIMETRIC, &error);
...
static void gimp_image_convert_profile_layers(GimpImage *image,
  ...
  while ((drawable = gimp_object_queue_pop(queue))) {
    ...
    gimp_gegl_convert_color_profile(gimp_drawable_get_buffer(drawable), NULL,
                                    src_profile, buffer, NULL, dest_profile,
                                    intent, bpc, progress);
...
static void gimp_image_convert_profile_colormap(GimpImage *image,
  ...
  src_format = gimp_color_profile_get_format(
      src_profile, babl_format("R'G'B' u8"),
      GIMP_COLOR_RENDERING_INTENT_RELATIVE_COLORIMETRIC, NULL);
  dest_format = gimp_color_profile_get_format(
      dest_profile, babl_format("R'G'B' u8"),
      GIMP_COLOR_RENDERING_INTENT_RELATIVE_COLORIMETRIC, NULL);

  transform = gimp_color_transform_new(src_profile, src_format, dest_profile,
                                       dest_format, intent, flags);

  if (transform) {
    gimp_color_transform_process_pixels(transform, babl_format("R'G'B' u8"),
                                        cmap, babl_format("R'G'B' u8"), cmap,
                                        n_colors);
...
static void gimp_image_create_color_transforms(GimpImage *image) {
  ...
  if (private->color_profile && !private->color_transforms_created) {
  ...
  private
    ->transform_to_srgb_u8 = gimp_color_transform_new(
        private->color_profile, gimp_image_get_layer_format(image, TRUE),
        srgb_profile, babl_format("R'G'B'A u8"),
        GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL, flags);

  private
    ->transform_from_srgb_u8 = gimp_color_transform_new(
        srgb_profile, babl_format("R'G'B'A u8"), private->color_profile,
        gimp_image_get_layer_format(image, TRUE),
        GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL, flags);

  private
    ->transform_to_srgb_double = gimp_color_transform_new(
        private->color_profile, gimp_image_get_layer_format(image, TRUE),
        srgb_profile, babl_format("R'G'B'A double"),
        GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL, flags);

  private
    ->transform_from_srgb_double = gimp_color_transform_new(
        srgb_profile, babl_format("R'G'B'A double"), private->color_profile,
        gimp_image_get_layer_format(image, TRUE),
        GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL, flags);
