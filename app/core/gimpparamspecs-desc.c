...
static inline const gchar *gimp_param_spec_get_blurb(GParamSpec *pspec) {
...
static gchar *gimp_param_spec_int_desc(GParamSpec *pspec) {
  ...
  if (ispec->minimum == G_MININT32)
    return g_strdup_printf("%s (%s <= %d)", blurb, g_param_spec_get_name(pspec),
                           ispec->maximum);
  ...
  return g_strdup_printf("%s (%d <= %s <= %d)", blurb, ispec->minimum,
...
static gchar *gimp_param_spec_double_desc(GParamSpec *pspec) {
  ...
  if (dspec->minimum == -G_MAXDOUBLE)
    return g_strdup_printf("%s (%s <= %g)", blurb, g_param_spec_get_name(pspec),
                           dspec->maximum);
  ...
  return g_strdup_printf("%s (%g <= %s <= %g)", blurb, dspec->minimum,
