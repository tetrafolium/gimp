...
G_DEFINE_BOXED_TYPE(GimpBezierDesc, gimp_bezier_desc, gimp_bezier_desc_copy,
                    gimp_bezier_desc_free)

GimpBezierDesc *gimp_bezier_desc_new(cairo_path_data_t *data, gint n_data) {
  GimpBezierDesc *desc;
...
static void add_polyline(GArray *path_data, const GimpVector2 *points,
...
GimpBezierDesc *gimp_bezier_desc_new_from_bound_segs(GimpBoundSeg *bound_segs,
  ...
  for (i = 0; i < n_bound_groups; i++) {
    while (bound_segs[seg].x1 != -1 || bound_segs[seg].x2 != -1 ||
           bound_segs[seg].y1 != -1 || bound_segs[seg].y2 != -1) {
  ...
  return gimp_bezier_desc_new(
      (cairo_path_data_t *)g_array_free(path_data, FALSE), path_data_len);
}

void gimp_bezier_desc_translate(GimpBezierDesc *desc, gdouble offset_x,
                                gdouble offset_y) {
  gint i, j;
...
GimpBezierDesc *gimp_bezier_desc_copy(const GimpBezierDesc *desc) {
  ...
  return gimp_bezier_desc_new(
      g_memdup(desc->data, desc->num_data * sizeof(cairo_path_data_t)),
      desc->num_data);
}
