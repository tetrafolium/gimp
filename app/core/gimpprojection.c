...
static void gimp_projection_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_projection_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);

static gint64 gimp_projection_get_memsize(GimpObject *object, gint64 *gui_size);
...
static gboolean gimp_projection_get_pixel_at(GimpPickable *pickable, gint x,
                                             gint y, const Babl *format,
                                             gpointer pixel);
static gdouble gimp_projection_get_opacity_at(GimpPickable *pickable, gint x,
                                              gint y);
...
static void gimp_projection_add_update_area(GimpProjection *proj, gint x,
                                            gint y, gint w, gint h);
static void gimp_projection_flush_whenever(GimpProjection *proj, gboolean now,
                                           gboolean direct);
...
gimp_projection_projectable_structure_changed(GimpProjectable *projectable,
                                              GimpProjection *proj);
static void gimp_projection_projectable_bounds_changed(
    GimpProjectable *projectable, gint old_x, gint old_y, GimpProjection *proj);

G_DEFINE_TYPE_WITH_CODE(GimpProjection, gimp_projection, GIMP_TYPE_OBJECT,
                        G_ADD_PRIVATE(GimpProjection) G_IMPLEMENT_INTERFACE(
                            GIMP_TYPE_PICKABLE,
...
static void gimp_projection_class_init(GimpProjectionClass *klass) {
  ...
  projection_signals[UPDATE] = g_signal_new(
      "update", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpProjectionClass, update), NULL, NULL,
      gimp_marshal_VOID__BOOLEAN_INT_INT_INT_INT, G_TYPE_NONE, 5,
      G_TYPE_BOOLEAN, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT);
...
gint64 gimp_projection_estimate_memsize(GimpImageBaseType type,
  ...
  format = gimp_babl_format(type, gimp_babl_precision(component_type, FALSE),
                            TRUE, NULL);
...
static GeglBuffer *gimp_projection_get_buffer(GimpPickable *pickable) {
  ...
  if (!proj->priv->buffer) {
     ...
     *  responsive.
     */
    gimp_projection_add_update_area(proj, bounding_box.x, bounding_box.y,
...
static gboolean gimp_projection_get_pixel_at(GimpPickable *pickable, gint x,
  ...
  if (x < bounding_box.x || y < bounding_box.y ||
...
GimpProjection *gimp_projection_new(GimpProjectable *projectable) {
  ...
  g_signal_connect_object(projectable, "flush",
                          G_CALLBACK(gimp_projection_projectable_flush), proj,
                          0);
  g_signal_connect_object(
      projectable, "structure-changed",
      G_CALLBACK(gimp_projection_projectable_structure_changed), proj, 0);
  g_signal_connect_object(
      projectable, "bounds-changed",
      G_CALLBACK(gimp_projection_projectable_bounds_changed), proj, 0);

  return proj;
}

void gimp_projection_set_priority(GimpProjection *proj, gint priority) {
  g_return_if_fail(GIMP_IS_PROJECTION(proj));

  proj->priv->priority = priority;
}

gint gimp_projection_get_priority(GimpProjection *proj) {
  g_return_val_if_fail(GIMP_IS_PROJECTION(proj), 0);

  return proj->priv->priority;
}

void gimp_projection_set_priority_rect(GimpProjection *proj, gint x, gint y,
                                       gint w, gint h) {
  g_return_if_fail(GIMP_IS_PROJECTION(proj));
...
void gimp_projection_flush_now(GimpProjection *proj, gboolean direct) {
  g_return_if_fail(GIMP_IS_PROJECTION(proj));
...
static void gimp_projection_allocate_buffer(GimpProjection *proj) {
  ...
  proj->priv->validate_handler = GIMP_TILE_HANDLER_VALIDATE(
...
static void gimp_projection_add_update_area(GimpProjection *proj, gint x,
   ...
   */
  w = ceil((gdouble)(x + w) / GIMP_PROJECTION_UPDATE_CHUNK_WIDTH) *
      GIMP_PROJECTION_UPDATE_CHUNK_WIDTH;
  h = ceil((gdouble)(y + h) / GIMP_PROJECTION_UPDATE_CHUNK_HEIGHT) *
      GIMP_PROJECTION_UPDATE_CHUNK_HEIGHT;
  x = floor((gdouble)x / GIMP_PROJECTION_UPDATE_CHUNK_WIDTH) *
      GIMP_PROJECTION_UPDATE_CHUNK_WIDTH;
  y = floor((gdouble)y / GIMP_PROJECTION_UPDATE_CHUNK_HEIGHT) *
      GIMP_PROJECTION_UPDATE_CHUNK_HEIGHT;
  ...
  if (proj->priv->update_region) {
    ...
    if (now) /* Synchronous */
    {
      ...
      for (i = 0; i < n_rects; i++) {
        ...
        cairo_region_get_rectangle(proj->priv->update_region, i, &rect);

        gimp_projection_paint_area(proj, direct, rect.x, rect.y, rect.width,
                                   rect.height);
      }
  ...
  if (region && !cairo_region_is_empty(region)) {
    ...
    if (!proj->priv->idle_id) {
      proj->priv->idle_id = g_idle_add_full(
          ...
          (GSourceFunc)gimp_projection_chunk_render_callback, proj, NULL);
    }
  } else {
    if (region)
...
static gboolean gimp_projection_chunk_render_iteration(GimpProjection *proj) {
  if (gimp_chunk_iterator_next(proj->priv->iter)) {
    ...
    while (gimp_chunk_iterator_get_rect(proj->priv->iter, &rect)) {
      gimp_projection_paint_area(proj, TRUE, rect.x, rect.y, rect.width,
                                 rect.height);
    }
...
static void gimp_projection_paint_area(GimpProjection *proj, gboolean now,
  ...
  if (gegl_rectangle_intersect(&rect, GEGL_RECTANGLE(x, y, w, h),
                               &bounding_box)) {
    if (now) {
      gimp_tile_handler_validate_validate(proj->priv->validate_handler,
                                          proj->priv->buffer, &rect, FALSE,
                                          FALSE);
    } else {
     ...
     */
    g_signal_emit(proj, projection_signals[UPDATE], 0, now, rect.x + off_x,
                  rect.y + off_y, rect.width, rect.height);
  }
}
...
gimp_projection_projectable_bounds_changed(GimpProjectable *projectable,
  ...
  if (dx == 0 && dy == 0) {
    ...
    gimp_tile_handler_validate_buffer_copy(
        old_buffer,
        GEGL_RECTANGLE(int_bounds.x - old_x, int_bounds.y - old_y,
                       int_bounds.width, int_bounds.height),
        proj->priv->buffer,
        GEGL_RECTANGLE(int_bounds.x - x, int_bounds.y - y, int_bounds.width,
                       int_bounds.height));
  ...
  if (int_bounds.x + int_bounds.width < bounding_box.x + bounding_box.width) {
    gimp_projection_add_update_area(
        proj, int_bounds.x + int_bounds.width, bounding_box.y,
        bounding_box.x + bounding_box.width - (int_bounds.x + int_bounds.width),
        bounding_box.height);
  }
  if (int_bounds.y + int_bounds.height < bounding_box.y + bounding_box.height) {
    gimp_projection_add_update_area(proj, bounding_box.x,
