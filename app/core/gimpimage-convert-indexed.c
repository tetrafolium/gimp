...
typedef void (*Pass2Func)(QuantizeObj *quantize_obj, GimpLayer *layer,
...
/*
 #define REF_FUNC(r,g,b) (((r)<<(PRECISION_G+PRECISION_B)) |
 ((g)<<(PRECISION_B)) | (b))
 */
 ...
 */
static inline ColorFreq *HIST_LIN(ColorFreq *hist_ptr, const gint r,
                                  const gint g, const gint b) {
...
static inline void rgb_to_unshifted_lin(const guchar r, const guchar g,
                                        const guchar b, gint *hr, gint *hg,
                                        gint *hb) {
  gint or, og, ob;
...
static inline void rgb_to_lin(const guchar r, const guchar g, const guchar b,
                              gint *hr, gint *hg, gint *hb) {
  gint or, og, ob;
  ...
  /*
     ...
     fprintf(stderr, " [L: %0.3f -> %0.3f / a: %0.3f -> %0.3f / b: %0.3f ->
     %0.3f]\t", low_l, high_l, low_a, high_a, low_b, high_b);
...
#if 0
#define RSDF(r)                                                                \
  ((r) >= ((HIST_R_ELEMS - 1) << R_SHIFT)                                      \
       ? HIST_R_ELEMS - 1                                                      \
       : ((r) + ((1 << R_SHIFT) >> 1)) >> R_SHIFT)
#define GSDF(g)                                                                \
  ((g) >= ((HIST_G_ELEMS - 1) << G_SHIFT)                                      \
       ? HIST_G_ELEMS - 1                                                      \
       : ((g) + ((1 << G_SHIFT) >> 1)) >> G_SHIFT)
#define BSDF(b)                                                                \
  ((b) >= ((HIST_B_ELEMS - 1) << B_SHIFT)                                      \
       ? HIST_B_ELEMS - 1                                                      \
...
static inline ColorFreq *HIST_RGB(ColorFreq *hist_ptr, const gint r,
                                  const gint g, const gint b) {
  gint hr, hg, hb;
...
static inline void lin_to_rgb(const gdouble hr, const gdouble hg,
                              const gdouble hb, guchar *r, guchar *g,
                              guchar *b) {
  gfloat rgb[3];
  gfloat lab[3];
  gdouble ir, ig, ib;
...
struct _QuantizeObj {
  ...
  Pass2InitFunc
      second_pass_init;  /* Initialize data which persists over invocations */
  Pass2Func second_pass; /* second pass maps from image data to colormap */
  CleanupFunc
      delete_func; /* function to clean up data associated with private */
...
static void generate_histogram_gray(CFHistogram hostogram, GimpLayer *layer,
                                    gboolean dither_alpha);
static void generate_histogram_rgb(CFHistogram histogram, GimpLayer *layer,
                                   gint col_limit, gboolean dither_alpha,
                                   GimpProgress *progress);

static QuantizeObj *initialize_median_cut(
    GimpImageBaseType old_type, gint max_colors,
    GimpConvertDitherType dither_type, GimpConvertPaletteType palette_type,
    GimpPalette *custom_palette, gboolean dither_alpha, GimpProgress *progress);

static void compute_color_lin8(QuantizeObj *quantobj, CFHistogram histogram,
                               boxptr boxp, const int icolor);
 ...
 * <adam@gimp.org>
 */
static void make_remap_table(const guchar old_palette[], guchar new_palette[],
                             const gulong index_used_count[],
                             guchar remap_table[], gint *num_entries) {
  gint i, j, k;
  ...
  for (i = 0; i < used; i++) {
    for (j = 0; j < i; j++) {
      if ((temppal[i * 3 + 1] == temppal[j * 3 + 1]) &&
          ...
          (temppal[i * 3 + 2] == temppal[j * 3 + 2]) && tempuse[j] &&
          tempuse[i]) {
...
static void remap_indexed_layer(GimpLayer *layer, const guchar *remap_table,
                                gint num_entries) {
  ...
  iter = gegl_buffer_iterator_new(
      gimp_drawable_get_buffer(GIMP_DRAWABLE(layer)), NULL, 0, NULL,
...
static gint color_quicksort(const void *c1, const void *c2) {
...
gboolean gimp_image_convert_indexed(
    GimpImage *image, GimpConvertPaletteType palette_type, gint max_colors,
    gboolean remove_duplicates, GimpConvertDitherType dither_type,
    gboolean dither_alpha, gboolean dither_text_layers,
    GimpPalette *custom_palette, GimpProgress *progress, GError **error) {
  ...
  g_return_val_if_fail(
      gimp_babl_is_valid(GIMP_INDEXED, gimp_image_get_precision(image)), FALSE);
  g_return_val_if_fail(
      custom_palette == NULL || GIMP_IS_PALETTE(custom_palette), FALSE);
  ...
  rgb_to_lab_fish =
      babl_fish(babl_format("R'G'B' float"), babl_format("CIE Lab float"));
  lab_to_rgb_fish =
      babl_fish(babl_format("CIE Lab float"), babl_format("R'G'B' float"));

  /* don't dither if the input is grayscale and we are simply mapping
   * every color
   */
  if (old_type == GIMP_GRAY && max_colors == 256 &&
  ...
  quantobj =
      initialize_median_cut(old_type, max_colors, dither_type, palette_type,
                            custom_palette, dither_alpha, sub_progress);
  ...
  if (palette_type == GIMP_CONVERT_PALETTE_GENERATE) {
    ...
    for (list = all_layers; list; list = g_list_next(list)) {
      ...
      if (old_type == GIMP_GRAY) {
        generate_histogram_gray(quantobj->histogram, layer, dither_alpha);
      } else {
        /* Note: generate_histogram_rgb may set needs_quantize
         * if the image contains more colors than the limit
         * specified by the user.
         */
        generate_histogram_rgb(quantobj->histogram, layer, max_colors,
                               dither_alpha, sub_progress);
      }
    }
  }
  ...
  if (old_type == GIMP_RGB && !needs_quantize &&
      palette_type == GIMP_CONVERT_PALETTE_GENERATE) {
    gint i;
    ...
    quantobj = initialize_median_cut(
        old_type, max_colors, GIMP_CONVERT_DITHER_NODESTRUCT, palette_type,
        custom_palette, dither_alpha, sub_progress);
   ...
   */
  {
    ...
    gimp_image_set_colormap(image, colormap, quantobj->actual_number_of_colors,
                            TRUE);
  }
  ...
  for (list = all_layers; list; list = g_list_next(list)) {
    ...
    if (quantize) {
      ...
      gimp_drawable_set_buffer(GIMP_DRAWABLE(layer), TRUE, NULL, new_buffer);
      g_object_unref(new_buffer);
    } else {
      gimp_drawable_convert_type(
          GIMP_DRAWABLE(layer), image, GIMP_INDEXED,
          gimp_drawable_get_precision(GIMP_DRAWABLE(layer)),
          gimp_drawable_has_alpha(GIMP_DRAWABLE(layer)), src_profile,
          dest_profile, GEGL_DITHER_NONE, GEGL_DITHER_NONE, TRUE, sub_progress);
    }
  }
  ...
  if (remove_duplicates && (palette_type != GIMP_CONVERT_PALETTE_GENERATE) &&
    ...
    make_remap_table(old_palette, new_palette, quantobj->index_used_count,
...
static void generate_histogram_gray(CFHistogram histogram, GimpLayer *layer,
                                    gboolean dither_alpha) {
  GeglBufferIterator *iter;
  const Babl *format;
  gint bpp;
  ...
  iter = gegl_buffer_iterator_new(
      gimp_drawable_get_buffer(GIMP_DRAWABLE(layer)), NULL, 0, format,
...
static void check_white_or_black(const guchar *data) {
    ...
    if (needs_quantize) {
      if (dither_alpha) {
        ...
        while (length--) {
          ...
          if (has_alpha &&
              data[ALPHA] < DM[col & DM_WIDTHMASK][row & DM_HEIGHTMASK])
            transparent = TRUE;

          if (!transparent) {
            colfreq = HIST_RGB(histogram, data[RED], data[GREEN], data[BLUE]);
        ...
        while (length--) {
          if ((has_alpha && ((data[ALPHA] > 127))) || (!has_alpha)) {
            colfreq = HIST_RGB(histogram, data[RED], data[GREEN], data[BLUE]);
            check_white_or_black(data);
            (*colfreq)++;
          }
      ...
      while (length--) {
        ...
        if (has_alpha) {
          if (dither_alpha) {
            if (data[ALPHA] < DM[col & DM_WIDTHMASK][row & DM_HEIGHTMASK])
              transparent = TRUE;
          } else {
            if (data[ALPHA] <= 127)
              transparent = TRUE;
        ...
        if (!transparent) {
          ...
          if (!needs_quantize) {
            for (nfc_iter = 0; nfc_iter < num_found_cols; nfc_iter++) {
...
static boxptr find_split_candidate(const boxptr boxlist, const gint numboxes,
                                   AxisType *which_axis,
...
#define BIAS_NUMBER 2 /* 0 */
    ...
    Lbias = (numboxes > BIAS_NUMBER)
                ? 1.0F
                : ((gdouble)(BIAS_NUMBER + 1) - ((gdouble)numboxes)) /
  ...
  for (i = 0, boxp = boxlist; i < numboxes; i++, boxp++) {
    if (boxp->volume > 0) {
      ...
      if (Lbias * rpe > maxc && boxp->Rmin < boxp->Rmax) {
        which = boxp;
        maxc = Lbias * rpe;
        *which_axis = AXIS_RED;
      }

      if (gpe > maxc && boxp->Gmin < boxp->Gmax) {
        which = boxp;
        maxc = gpe;
        *which_axis = AXIS_GREEN;
      }

      if (bpe > maxc && boxp->Bmin < boxp->Bmax) {
        which = boxp;
        maxc = bpe;
 ...
 */
static boxptr find_biggest_volume(const boxptr boxlist, const gint numboxes) {
  boxptr boxp;
  gint i;
  gint maxv = 0;
  boxptr which = NULL;
 ...
 */
static void update_box_gray(const CFHistogram histogram, boxptr boxp) {
  gint i, min, max, dist;
  ColorFreq ccount;
 ...
 */
static void update_box_rgb(const CFHistogram histogram, boxptr boxp,
...
finished_axesscan:
  ...
  if (dist0 && dist1 && dist2) {
    ...
    /* fprintf(stderr, " ratio:(%d/%d)=%d ", longest_length, longest_length2,
       ratio); fprintf(stderr, "C%d ", cells_remaining); */
 ...
 * boxes
 */
static gint median_cut_gray(CFHistogram histogram, boxptr boxlist,
                            gint numboxes, gint desired_colors) {
  gint lb;
  boxptr b1, b2;
 ...
 * boxes
 */
static gint median_cut_rgb(CFHistogram histogram, boxptr boxlist, gint numboxes,
                           gint desired_colors, GimpProgress *progress) {
  gint lb;
  boxptr b1, b2;
  AxisType which_axis;
 ...
 */
static void compute_color_gray(QuantizeObj *quantobj, CFHistogram histogram,
  ...
  if (total != 0) {
    quantobj->cmap[icolor].red = quantobj->cmap[icolor].green =
...
/* Compute representative color for a box, put it in colormap[icolor]
 */
static void compute_color_rgb(QuantizeObj *quantobj, CFHistogram histogram,
                              boxptr boxp, int icolor) {
  ...
  if (total > 0) {
    ...
    lin_to_rgb(/*(Rtotal + (total>>1)) / total,
               ...
               (double)Rtotal / (double)total, (double)Gtotal / (double)total,
               (double)Btotal / (double)total, &red, &green, &blue);
 ...
 */
static void select_colors_gray(QuantizeObj *quantobj, CFHistogram histogram) {
  boxptr boxlist;
  gint numboxes;
...
/* Master routine for color selection
 */
static void select_colors_rgb(QuantizeObj *quantobj, CFHistogram histogram) {
  boxptr boxlist;
  gint numboxes;
  gint desired = quantobj->desired_number_of_colors;
  gint i;
  ...
  /* Perform median-cut to produce final box list */
  numboxes =
      median_cut_rgb(histogram, boxlist, numboxes, desired, quantobj->progress);
 ...
 */
static gint find_nearby_colors(QuantizeObj *quantobj, int minR, int minG,
 ...
 * the box.
 */
static void find_best_colors(QuantizeObj *quantobj, gint minR, gint minG,
                             gint minB, gint numcolors, gint colorlist[],
                             gint bestcolor[]) {
  gint iR, iG, iB;
  gint i, icolor;
  ...
  gint bestdist[BOX_R_ELEMS * BOX_G_ELEMS * BOX_B_ELEMS] = {
      0,
  };
 ...
 */
static void fill_inverse_cmap_gray(QuantizeObj *quantobj, CFHistogram histogram,
                                   gint pixel) {
 ...
 * can fill as many others as we wish.)
 */
static void fill_inverse_cmap_rgb(QuantizeObj *quantobj, CFHistogram histogram,
  ...
  gint bestcolor[BOX_R_ELEMS * BOX_G_ELEMS * BOX_B_ELEMS] = {
      0,
  };
...
static void median_cut_pass1_gray(QuantizeObj *quantobj) {
  select_colors_gray(quantobj, quantobj->histogram);
}

static void snap_to_black_and_white(QuantizeObj *quantobj) {
...
#define POW2(a) ((a) * (a))
  ...
  for (i = 0; i < desired; i++) {
    ...
    dist = POW2(quantobj->cmap[i].red - 0) + POW2(quantobj->cmap[i].green - 0) +
  ...
  if (desired > 2 && had_white && white_dist < POW2(128)) {
    quantobj->cmap[whitest].red = quantobj->cmap[whitest].green =
        quantobj->cmap[whitest].blue = 255;
  }
  if (desired > 2 && had_black && black_dist < POW2(128)) {
    quantobj->cmap[blackest].red = quantobj->cmap[blackest].green =
        quantobj->cmap[blackest].blue = 0;
  }
#undef POW2
}

static void median_cut_pass1_rgb(QuantizeObj *quantobj) {
  select_colors_rgb(quantobj, quantobj->histogram);
  snap_to_black_and_white(quantobj);
}

static void monopal_pass1(QuantizeObj *quantobj) {
...
static void webpal_pass1(QuantizeObj *quantobj) {
  int i;
...
static void custompal_pass1(QuantizeObj *quantobj) {
  gint i;
  GList *list;
  ...
  for (i = 0, list = gimp_palette_get_colors(quantobj->custom_palette); list;
...
static void median_cut_pass2_no_dither_gray(QuantizeObj *quantobj,
                                            GimpLayer *layer,
                                            GeglBuffer *new_buffer) {
  GeglBufferIterator *iter;
  ...
  gegl_buffer_iterator_add(iter, new_buffer, NULL, 0, NULL, GEGL_ACCESS_WRITE,
                           GEGL_ABYSS_NONE);
...
static void median_cut_pass2_fixed_dither_gray(QuantizeObj *quantobj,
                                               GimpLayer *layer,
                                               GeglBuffer *new_buffer) {
  GeglBufferIterator *iter;
  CFHistogram histogram = quantobj->histogram;
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (row = 0; row < src_roi->height; row++) {
      ...
      for (col = 0; col < src_roi->width; col++) {
        gint pixel;
        const gint dmval = DM[(col + offsetx + src_roi->x) & DM_WIDTHMASK]
        ...
        if (quantobj->actual_number_of_colors > 2) {
          ...
          do {
            ...
            pixval2 = *cachep - 1;
            RV += re;
          } while ((pixval1 == pixval2) && (!(RV > 255 || RV < 0)) && re);
        } else {
          ...
          pixval2 = (pixval1 + 1) % (quantobj->actual_number_of_colors);
        }
...
static void median_cut_pass2_no_dither_rgb(QuantizeObj *quantobj,
                                           GimpLayer *layer,
                                           GeglBuffer *new_buffer) {
  GeglBufferIterator *iter;
  CFHistogram histogram = quantobj->histogram;
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (row = 0; row < src_roi->height; row++) {
      ...
      for (col = 0; col < src_roi->width; col++) {
        ...
        /* get pixel value and index into the cache */
        rgb_to_lin(src[red_pix], src[green_pix], src[blue_pix], &R, &G, &B);
...
static void median_cut_pass2_fixed_dither_rgb(QuantizeObj *quantobj,
                                              GimpLayer *layer,
                                              GeglBuffer *new_buffer) {
  GeglBufferIterator *iter;
  CFHistogram histogram = quantobj->histogram;
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (row = 0; row < src_roi->height; row++) {
      ...
      for (col = 0; col < src_roi->width; col++) {
        const int dmval = DM[(col + offsetx + src_roi->x) & DM_WIDTHMASK]
                            [(row + offsety + src_roi->y) & DM_HEIGHTMASK];
        ...
        if (quantobj->actual_number_of_colors > 2) {
          ...
          do {
            rgb_to_lin((CLAMP0255(RV)), (CLAMP0255(GV)), (CLAMP0255(BV)), &R,
                       &G, &B);
...
#define DISTP(R1, G1, B1, R2, G2, B2, D)                                       \
  do {                                                                         \
    D = sqrt(30 * SQR((R1) - (R2)) + 59 * SQR((G1) - (G2)) +                   \
             11 * SQR((B1) - (B2)));                                           \
  } while (0)
#define LIN_DISTP(R1, G1, B1, R2, G2, B2, D)                                   \
  do {                                                                         \
    ...
    D = sqrt(R_SCALE * SQR((spacer1) - (spacer2)) +                            \
        ...
        DISTP(color1->red, color1->green, color1->blue, src[red_pix],
              src[green_pix], src[blue_pix], err1);
        DISTP(color2->red, color2->green, color2->blue, src[red_pix],
              src[green_pix], src[blue_pix], err2);
...
static void median_cut_pass2_nodestruct_dither_rgb(QuantizeObj *quantobj,
                                                   GimpLayer *layer,
                                                   GeglBuffer *new_buffer) {
  GeglBufferIterator *iter;
  const Babl *src_format;
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (row = 0; row < src_roi->height; row++) {
      ...
      for (col = 0; col < src_roi->width; col++) {
        ...
        if (!transparent) {
          if ((lastred == src[red_pix]) && (lastgreen == src[green_pix]) &&
        ...
        } else { /*  have alpha, and transparent  */
          dest[ALPHA_I] = 0;
        }
...
static gint *init_error_limit(const int error_freedom)
...
static void median_cut_pass2_fs_dither_gray(QuantizeObj *quantobj,
                                            GimpLayer *layer,
                                            GeglBuffer *new_buffer) {
  ...
  for (row = 0; row < height; row++) {
    ...
    gegl_buffer_get(src_buffer, GEGL_RECTANGLE(0, row, width, 1), 1.0, NULL,
    ...
    *nr = 0;
    ...
    gegl_buffer_set(new_buffer, GEGL_RECTANGLE(0, row, width, 1), 0, NULL,
...
static void median_cut_pass2_rgb_init(QuantizeObj *quantobj) {
  int i;
  ...
  for (i = 0; i < quantobj->actual_number_of_colors; i++) {
    rgb_to_unshifted_lin(quantobj->cmap[i].red, quantobj->cmap[i].green,
                         quantobj->cmap[i].blue, &quantobj->clin[i].red,
                         &quantobj->clin[i].green, &quantobj->clin[i].blue);
  }
}

static void median_cut_pass2_gray_init(QuantizeObj *quantobj) {
  zero_histogram_gray(quantobj->histogram);
...
static void median_cut_pass2_fs_dither_rgb(QuantizeObj *quantobj,
                                           GimpLayer *layer,
                                           GeglBuffer *new_buffer) {
  GeglBuffer *src_buffer;
  CFHistogram histogram = quantobj->histogram;
  ...
  for (row = 0; row < height; row++) {
    ...
    *rnr = *gnr = *bnr = 0;
    ...
    for (col = 0; col < width; col++) {
...
#endif
      rgb_to_unshifted_lin(src[red_pix], src[green_pix], src[blue_pix], &re,
                           &ge, &be);
      ...
      cachep = HIST_LIN(histogram, RSDF(re), GSDF(ge), BSDF(be));
      /* If we have not seen this color before, find nearest
       * colormap entry and update the cache
       */
      if (*cachep == 0)
        fill_inverse_cmap_rgb(quantobj, histogram, RSDF(re), GSDF(ge),
                              BSDF(be));
...
static void delete_median_cut(QuantizeObj *quantobj) {
...
/**************************************************************/
static QuantizeObj *initialize_median_cut(
    GimpImageBaseType type, gint num_colors, GimpConvertDitherType dither_type,
    GimpConvertPaletteType palette_type, GimpPalette *custom_palette,
    gboolean want_dither_alpha, GimpProgress *progress) {
  ...
  if (type == GIMP_GRAY && palette_type == GIMP_CONVERT_PALETTE_GENERATE)
    ...
    quantobj->histogram =
        g_new(ColorFreq, HIST_R_ELEMS * HIST_G_ELEMS * HIST_B_ELEMS);
