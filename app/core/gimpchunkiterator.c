...
static void gimp_chunk_iterator_merge_current_rect(GimpChunkIterator *iter) {
  ...
  rect.width =
      iter->current_rect.x + iter->current_rect.width - iter->current_x;
    ...
    if (iter->current_y == iter->current_rect.y + iter->current_rect.height) {
      ...
      if (!iter->priority_region &&
        ...
        cairo_region_subtract_rectangle(
            iter->region, (const cairo_rectangle_int_t *)&iter->priority_rect);
      }
...
static gint compare_double(const gdouble *x, const gdouble *y) {
  return (*x > *y) - (*x < *y);
}
...
static void gimp_chunk_iterator_set_target_area(GimpChunkIterator *iter,
  ...
  iter->target_area_history_n =
      MAX(iter->target_area_history_n, iter->target_area_history_i);
...
static void gimp_chunk_iterator_calc_rect(GimpChunkIterator *iter,
  ...
  aspect_ratio =
      (gdouble)iter->tile_rect.height / (gdouble)iter->tile_rect.width;
  ...
  if (readjust_height) {
    ...
    rect->height = MIN(rect->height, iter->current_rect.y +
                                         iter->current_rect.height - rect->y);
    rect->height = MIN(rect->height, MAX_CHUNK_HEIGHT);
  } else {
    rect->height = iter->current_height;
  }

  rect->width = RINT((offset_x + (gdouble)target_area / (gdouble)rect->height) /
  ...
  rect->width = MIN(rect->width,
                    iter->current_rect.x + iter->current_rect.width - rect->x);
...
GimpChunkIterator *gimp_chunk_iterator_new(cairo_region_t *region) {
  ...
  g_object_get(gegl_config(), "tile-width", &iter->tile_rect.width,
               "tile-height", &iter->tile_rect.height, NULL);
...
gboolean gimp_chunk_iterator_get_rect(GimpChunkIterator *iter,
  ...
  if (iter->last_area >= MIN_AREA_PER_ITERATION) {
    ...
    gimp_chunk_iterator_set_target_area(iter, iter->last_area * iter->interval /
                                                  interval);
  ...
  if (rect->height != iter->current_height) {
    ...
    if (rect->x != iter->current_rect.x) {
      ...
      rem.x = rect->x;
      rem.y = rect->y;
      rem.width = iter->current_rect.x + iter->current_rect.width - rect->x;
