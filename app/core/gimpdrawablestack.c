...
static void gimp_drawable_stack_reorder(GimpContainer *container,
                                        GimpObject *object, gint new_index);

static void gimp_drawable_stack_drawable_update(GimpItem *item, gint x, gint y,
...
static void gimp_drawable_stack_class_init(GimpDrawableStackClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  GimpContainerClass *container_class = GIMP_CONTAINER_CLASS(klass);

  stack_signals[UPDATE] =
      g_signal_new("update", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpDrawableStackClass, update), NULL, NULL,
...
static void gimp_drawable_stack_init(GimpDrawableStack *stack) {}
...
GimpContainer *gimp_drawable_stack_new(GType drawable_type) {
  ...
  return g_object_new(GIMP_TYPE_DRAWABLE_STACK, "name",
                      g_type_name(drawable_type), "children-type",
                      drawable_type, "policy", GIMP_CONTAINER_POLICY_STRONG,
                      NULL);
}

/*  protected functions  */

void gimp_drawable_stack_update(GimpDrawableStack *stack, gint x, gint y,
                                gint width, gint height) {
  g_return_if_fail(GIMP_IS_DRAWABLE_STACK(stack));

  g_signal_emit(stack, stack_signals[UPDATE], 0, x, y, width, height);
}
...
static void gimp_drawable_stack_drawable_update(GimpItem *item, gint x, gint y,
  ...
  if (gimp_filter_get_active(GIMP_FILTER(item))) {
    ...
    gimp_drawable_stack_update(stack, x + offset_x, y + offset_y, width,
                               height);
  }
}
...
static void gimp_drawable_stack_drawable_active(GimpItem *item,
  ...
  gimp_drawable_stack_update(stack, bounding_box.x, bounding_box.y,
