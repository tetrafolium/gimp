...
void gimp_drawable_real_apply_buffer(GimpDrawable *drawable, GeglBuffer *buffer,
  ...
  if (!gimp_rectangle_intersect(
          base_x, base_y, buffer_region->width, buffer_region->height, 0, 0,
          gimp_item_get_width(item), gimp_item_get_height(item), &x, &y, &width,
          &height)) {
    return;
  }
  ...
  if (mask) {
     ...
     */
    if (!gimp_rectangle_intersect(x, y, width, height, -offset_x, -offset_y,
                                  gimp_item_get_width(mask_item),
                                  gimp_item_get_height(mask_item), &x, &y,
                                  &width, &height)) {
      return;
    }
  }
  ...
  if (push_undo) {
    gimp_drawable_push_undo(drawable, undo_desc, NULL, x, y, width, height);
  }
  ...
  gimp_applicator_set_apply_offset(applicator, base_x - buffer_region->x,
                                   base_y - buffer_region->y);

  gimp_applicator_set_opacity(applicator, opacity);
  gimp_applicator_set_mode(applicator, mode, blend_space, composite_space,
                           composite_mode);
