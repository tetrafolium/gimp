...
typedef GimpData *(*GimpDataNewFunc)(GimpContext *context, const gchar *name);
typedef GimpData *(*GimpDataGetStandardFunc)(GimpContext *context);

typedef void (*GimpDataForeachFunc)(GimpDataFactory *factory, GimpData *data,
                                    gpointer user_data);

#define GIMP_TYPE_DATA_FACTORY (gimp_data_factory_get_type())
#define GIMP_DATA_FACTORY(obj)                                                 \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), GIMP_TYPE_DATA_FACTORY, GimpDataFactory))
#define GIMP_DATA_FACTORY_CLASS(klass)                                         \
  (G_TYPE_CHECK_CLASS_CAST((klass), GIMP_TYPE_DATA_FACTORY,                    \
                           GimpDataFactoryClass))
#define GIMP_IS_DATA_FACTORY(obj)                                              \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), GIMP_TYPE_DATA_FACTORY))
#define GIMP_IS_DATA_FACTORY_CLASS(klass)                                      \
  (G_TYPE_CHECK_CLASS_TYPE((klass), GIMP_TYPE_DATA_FACTORY))
#define GIMP_DATA_FACTORY_GET_CLASS(obj)                                       \
  (G_TYPE_INSTANCE_GET_CLASS((obj), GIMP_TYPE_DATA_FACTORY,                    \
                             GimpDataFactoryClass))
...
struct _GimpDataFactoryClass {
  ...
  void (*data_init)(GimpDataFactory *factory, GimpContext *context);
  void (*data_refresh)(GimpDataFactory *factory, GimpContext *context);
  void (*data_save)(GimpDataFactory *factory);

  void (*data_cancel)(GimpDataFactory *factory);

  GimpData *(*data_duplicate)(GimpDataFactory *factory, GimpData *data);
  gboolean (*data_delete)(GimpDataFactory *factory, GimpData *data,
                          gboolean delete_from_disk, GError **error);
};
