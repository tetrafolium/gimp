...
void gimp_templates_load(Gimp *gimp) {
  ...
  if (!gimp_config_deserialize_file(GIMP_CONFIG(gimp->templates), file, NULL,
                                    &error)) {
    if (error->code == GIMP_CONFIG_ERROR_OPEN_ENOENT) {
      ...
      if (!gimp_config_deserialize_file(GIMP_CONFIG(gimp->templates), file,
                                        NULL, &error)) {
...
void gimp_templates_save(Gimp *gimp) {
  ...
  const gchar *footer = "end of templaterc";
  ...
  if (!gimp_config_serialize_to_file(GIMP_CONFIG(gimp->templates), file, header,
                                     footer, NULL, &error)) {
    gimp_message_literal(gimp, NULL, GIMP_MESSAGE_ERROR, error->message);
    g_error_free(error);
  }
...
void gimp_templates_migrate(const gchar *olddir) {
  ...
  if (gimp_config_deserialize_file(GIMP_CONFIG(templates), file, NULL, NULL)) {
    GFile *sysconf_file;
    ...
    if (olddir && (strstr(olddir, "2.0") || strstr(olddir, "2.2"))) {
      ...
      gimp_config_deserialize_file(GIMP_CONFIG(templates), sysconf_file, NULL,
                                   NULL);
    ...
    gimp_config_serialize_to_file(GIMP_CONFIG(templates), file, NULL, NULL,
                                  NULL, NULL);
  }
