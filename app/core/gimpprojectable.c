...
enum { INVALIDATE, FLUSH, STRUCTURE_CHANGED, BOUNDS_CHANGED, LAST_SIGNAL };
...
static void gimp_projectable_default_init(GimpProjectableInterface *iface) {
  projectable_signals[INVALIDATE] =
      g_signal_new("invalidate", G_TYPE_FROM_CLASS(iface), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpProjectableInterface, invalidate), NULL,
                   NULL, gimp_marshal_VOID__INT_INT_INT_INT, G_TYPE_NONE, 4,
                   G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT);

  projectable_signals[FLUSH] =
      g_signal_new("flush", G_TYPE_FROM_CLASS(iface), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpProjectableInterface, flush), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, G_TYPE_BOOLEAN);

  projectable_signals[STRUCTURE_CHANGED] = g_signal_new(
      "structure-changed", G_TYPE_FROM_CLASS(iface), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpProjectableInterface, structure_changed), NULL, NULL,
      NULL, G_TYPE_NONE, 0);

  projectable_signals[BOUNDS_CHANGED] = g_signal_new(
      "bounds-changed", G_TYPE_FROM_CLASS(iface), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpProjectableInterface, bounds_changed), NULL, NULL,
      gimp_marshal_VOID__INT_INT, G_TYPE_NONE, 2, G_TYPE_INT, G_TYPE_INT);
}

/*  public functions  */

void gimp_projectable_invalidate(GimpProjectable *projectable, gint x, gint y,
                                 gint width, gint height) {
  g_return_if_fail(GIMP_IS_PROJECTABLE(projectable));

  g_signal_emit(projectable, projectable_signals[INVALIDATE], 0, x, y, width,
                height);
}
...
void gimp_projectable_bounds_changed(GimpProjectable *projectable, gint old_x,
                                     gint old_y) {
  g_return_if_fail(GIMP_IS_PROJECTABLE(projectable));

  g_signal_emit(projectable, projectable_signals[BOUNDS_CHANGED], 0, old_x,
                old_y);
}
...
void gimp_projectable_get_offset(GimpProjectable *projectable, gint *x,
                                 gint *y) {
  GimpProjectableInterface *iface;
