...
enum { SPACING_CHANGED, LAST_SIGNAL };
...
static void gimp_brush_set_property(GObject *object, guint property_id,
                                    const GValue *value, GParamSpec *pspec);
static void gimp_brush_get_property(GObject *object, guint property_id,
                                    GValue *value, GParamSpec *pspec);

static gint64 gimp_brush_get_memsize(GimpObject *object, gint64 *gui_size);

static gboolean gimp_brush_get_size(GimpViewable *viewable, gint *width,
                                    gint *height);
...
static void gimp_brush_copy(GimpData *data, GimpData *src_data);

static void gimp_brush_real_begin_use(GimpBrush *brush);
static void gimp_brush_real_end_use(GimpBrush *brush);
static GimpBrush *
gimp_brush_real_select_brush(GimpBrush *brush, const GimpCoords *last_coords,
...
static void gimp_brush_class_init(GimpBrushClass *klass) {
  ...
  brush_signals[SPACING_CHANGED] = g_signal_new(
      "spacing-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpBrushClass, spacing_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("spacing", NULL, _("Brush Spacing"), 1.0, 5000.0,
                          20.0, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}
...
static GimpTempBuf *gimp_brush_get_new_preview(GimpViewable *viewable,
  ...
  if (mask_width > width || mask_height > height) {
    ...
    if (scale != 1.0) {
      ...
      if (GIMP_IS_BRUSH_GENERATED(brush)) {
        ...
        mask_buf = gimp_brush_transform_mask(
            brush, scale,
            (gimp_brush_generated_get_aspect_ratio(gen_brush) - 1.0) * 20.0 /
                19.0,
            gimp_brush_generated_get_angle(gen_brush) / 360.0, FALSE,
            gimp_brush_generated_get_hardness(gen_brush));
      } else
        mask_buf =
            gimp_brush_transform_mask(brush, scale, 0.0, 0.0, FALSE, 1.0);
      ...
      if (pixmap_buf)
        pixmap_buf =
            gimp_brush_transform_pixmap(brush, scale, 0.0, 0.0, FALSE, 1.0);
  ...
  return_buf =
      gimp_temp_buf_new(mask_width, mask_height, babl_format("R'G'B'A u8"));

  mask = mask_data =
      gimp_temp_buf_lock(mask_buf, babl_format("Y u8"), GEGL_ACCESS_READ);
  ...
  if (pixmap_buf) {
    ...
    pixmap = pixmap_data = gimp_temp_buf_lock(
        pixmap_buf, babl_format("R'G'B' u8"), GEGL_ACCESS_READ);
...
static gchar *gimp_brush_get_description(GimpViewable *viewable,
  ...
  return g_strdup_printf("%s (%d × %d)", gimp_object_get_name(brush),
...
static gchar *gimp_brush_get_checksum(GimpTagged *tagged) {
  ...
  if (brush->priv->mask) {
    ...
    g_checksum_update(checksum, gimp_temp_buf_get_data(brush->priv->mask),
                      gimp_temp_buf_get_data_size(brush->priv->mask));
    if (brush->priv->pixmap)
      g_checksum_update(checksum, gimp_temp_buf_get_data(brush->priv->pixmap),
                        gimp_temp_buf_get_data_size(brush->priv->pixmap));
    g_checksum_update(checksum, (const guchar *)&brush->priv->spacing,
                      sizeof(brush->priv->spacing));
    g_checksum_update(checksum, (const guchar *)&brush->priv->x_axis,
                      sizeof(brush->priv->x_axis));
    g_checksum_update(checksum, (const guchar *)&brush->priv->y_axis,
...
GimpData *gimp_brush_new(GimpContext *context, const gchar *name) {
  g_return_val_if_fail(name != NULL, NULL);

  return gimp_brush_generated_new(name, GIMP_BRUSH_GENERATED_CIRCLE, 5.0, 2,
                                  0.5, 1.0, 0.0);
}
...
GimpBrush *gimp_brush_select_brush(GimpBrush *brush,
  ...
  return GIMP_BRUSH_GET_CLASS(brush)->select_brush(brush, last_coords,
                                                   current_coords);
}
...
gboolean gimp_brush_want_null_motion(GimpBrush *brush,
  ...
  return GIMP_BRUSH_GET_CLASS(brush)->want_null_motion(brush, last_coords,
                                                       current_coords);
}

void gimp_brush_transform_size(GimpBrush *brush, gdouble scale,
                               gdouble aspect_ratio, gdouble angle,
                               gboolean reflect, gint *width, gint *height) {
  g_return_if_fail(GIMP_IS_BRUSH(brush));
  g_return_if_fail(scale > 0.0);
  g_return_if_fail(width != NULL);
  g_return_if_fail(height != NULL);

  if (scale == 1.0 && aspect_ratio == 0.0 && fmod(angle, 0.5) == 0.0) {
    *width = gimp_temp_buf_get_width(brush->priv->mask);
    *height = gimp_temp_buf_get_height(brush->priv->mask);
  ...
  GIMP_BRUSH_GET_CLASS(brush)->transform_size(brush, scale, aspect_ratio, angle,
                                              reflect, width, height);
}

const GimpTempBuf *gimp_brush_transform_mask(GimpBrush *brush, gdouble scale,
  ...
  gimp_brush_transform_size(brush, scale, aspect_ratio, angle, reflect, &width,
                            &height);

  mask = gimp_brush_cache_get(brush->priv->mask_cache, width, height, scale,
                              aspect_ratio, angle, reflect, hardness);
  ...
  if (!mask) {
    ...
    mask = GIMP_BRUSH_GET_CLASS(brush)->transform_mask(
        brush, scale, aspect_ratio, angle, reflect, effective_hardness);

    gimp_brush_cache_add(brush->priv->mask_cache, (gpointer)mask, width, height,
...
const GimpTempBuf *gimp_brush_transform_pixmap(GimpBrush *brush, gdouble scale,
                                               gdouble aspect_ratio,
                                               gdouble angle, gboolean reflect,
                                               gdouble hardness) {
  ...
  pixmap = gimp_brush_cache_get(brush->priv->pixmap_cache, width, height, scale,
                                aspect_ratio, angle, reflect, hardness);
  ...
  if (!pixmap) {
    ...
    pixmap = GIMP_BRUSH_GET_CLASS(brush)->transform_pixmap(
        brush, scale, aspect_ratio, angle, reflect, effective_hardness);

    gimp_brush_cache_add(brush->priv->pixmap_cache, (gpointer)pixmap, width,
                         height, scale, aspect_ratio, angle, reflect,
                         effective_hardness);
  }
...
const GimpBezierDesc *gimp_brush_transform_boundary(
  ...
  boundary =
      gimp_brush_cache_get(brush->priv->boundary_cache, *width, *height, scale,
                           aspect_ratio, angle, reflect, hardness);

  if (!boundary) {
    boundary = GIMP_BRUSH_GET_CLASS(brush)->transform_boundary(
        brush, scale, aspect_ratio, angle, reflect, hardness, width, height);
     ...
     */
    if (boundary)
      gimp_brush_cache_add(brush->priv->boundary_cache, (gpointer)boundary,
                           *width, *height, scale, aspect_ratio, angle, reflect,
                           hardness);
  }
