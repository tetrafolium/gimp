...
enum { PROP_0, PROP_VERBOSE };

static void gimp_constructed(GObject *object);
static void gimp_set_property(GObject *object, guint property_id,
                              const GValue *value, GParamSpec *pspec);
static void gimp_get_property(GObject *object, guint property_id, GValue *value,
                              GParamSpec *pspec);
static void gimp_dispose(GObject *object);
static void gimp_finalize(GObject *object);

static gint64 gimp_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_real_initialize(Gimp *gimp,
                                 GimpInitStatusFunc status_callback);
static void gimp_real_restore(Gimp *gimp, GimpInitStatusFunc status_callback);
static gboolean gimp_real_exit(Gimp *gimp, gboolean force);
...
static guint gimp_signals[LAST_SIGNAL] = {
    0,
};
...
static void gimp_class_init(GimpClass *klass) {
  ...
  gimp_signals[INITIALIZE] =
      g_signal_new("initialize", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpClass, initialize), NULL, NULL, NULL,
                   G_TYPE_NONE, 1, G_TYPE_POINTER);

  gimp_signals[RESTORE] =
      g_signal_new("restore", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpClass, restore), NULL, NULL, NULL,
                   G_TYPE_NONE, 1, G_TYPE_POINTER);

  gimp_signals[EXIT] = g_signal_new(
      "exit", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpClass, exit), g_signal_accumulator_true_handled, NULL,
      gimp_marshal_BOOLEAN__BOOLEAN, G_TYPE_BOOLEAN, 1, G_TYPE_BOOLEAN);

  gimp_signals[CLIPBOARD_CHANGED] = g_signal_new(
      "clipboard-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpClass, clipboard_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_signals[FILTER_HISTORY_CHANGED] = g_signal_new(
      "filter-history-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpClass, filter_history_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_signals[IMAGE_OPENED] =
      g_signal_new("image-opened", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpClass, image_opened), NULL, NULL, NULL,
...
static void gimp_init(Gimp *gimp) {
  ...
  gimp->displays =
      g_object_new(GIMP_TYPE_LIST, "children-type", GIMP_TYPE_OBJECT, "policy",
                   GIMP_CONTAINER_POLICY_WEAK, "append", TRUE, NULL);
  ...
  gimp->tool_info_list =
      g_object_new(GIMP_TYPE_LIST, "children-type", GIMP_TYPE_TOOL_INFO,
                   "append", TRUE, NULL);
  gimp_object_set_static_name(GIMP_OBJECT(gimp->tool_info_list), "tool infos");

  gimp->tool_item_list =
      g_object_new(GIMP_TYPE_LIST, "children-type", GIMP_TYPE_TOOL_ITEM,
                   "append", TRUE, NULL);
  gimp_object_set_static_name(GIMP_OBJECT(gimp->tool_item_list), "tool items");

  gimp->tool_item_ui_list =
      gimp_tree_proxy_new_for_container(gimp->tool_item_list);
...
static void gimp_finalize(GObject *object) {
  ...
  standards =
      g_list_prepend(standards, gimp_brush_get_standard(gimp->user_context));
  standards =
      g_list_prepend(standards, gimp_dynamics_get_standard(gimp->user_context));
  standards =
      g_list_prepend(standards, gimp_mybrush_get_standard(gimp->user_context));
  standards =
      g_list_prepend(standards, gimp_pattern_get_standard(gimp->user_context));
  standards =
      g_list_prepend(standards, gimp_gradient_get_standard(gimp->user_context));
  standards =
      g_list_prepend(standards, gimp_palette_get_standard(gimp->user_context));
  ...
  if (gimp->tool_info_list) {
    gimp_container_foreach(gimp->tool_info_list, (GFunc)g_object_run_dispose,
                           NULL);
...
static gint64 gimp_get_memsize(GimpObject *object, gint64 *gui_size) {
  ...
  memsize += gimp_object_get_memsize(GIMP_OBJECT(gimp->parasites), gui_size);

  memsize +=
      gimp_object_get_memsize(GIMP_OBJECT(gimp->paint_info_list), gui_size);

  memsize += gimp_g_object_get_memsize(G_OBJECT(gimp->module_db));
  memsize +=
      gimp_object_get_memsize(GIMP_OBJECT(gimp->plug_in_manager), gui_size);

  memsize += gimp_g_list_get_memsize_foreach(
      gimp->filter_history, (GimpMemsizeFunc)gimp_object_get_memsize, gui_size);
  ...
  memsize +=
      gimp_object_get_memsize(GIMP_OBJECT(gimp->clipboard_image), gui_size);
  memsize +=
      gimp_object_get_memsize(GIMP_OBJECT(gimp->clipboard_buffer), gui_size);
  memsize +=
      gimp_object_get_memsize(GIMP_OBJECT(gimp->named_buffers), gui_size);
  ...
  memsize +=
      gimp_object_get_memsize(GIMP_OBJECT(gimp->tool_info_list), gui_size);
  memsize +=
      gimp_object_get_memsize(GIMP_OBJECT(gimp->standard_tool_info), gui_size);
  memsize += gimp_object_get_memsize(GIMP_OBJECT(gimp->documents), gui_size);
  memsize += gimp_object_get_memsize(GIMP_OBJECT(gimp->templates), gui_size);
  ...
  memsize +=
      gimp_object_get_memsize(GIMP_OBJECT(gimp->default_context), gui_size);
  memsize += gimp_object_get_memsize(GIMP_OBJECT(gimp->user_context), gui_size);
...
Gimp *gimp_new(const gchar *name, const gchar *session_name,
  ...
  gimp = g_object_new(GIMP_TYPE_GIMP, "name", name, "verbose",
                      be_verbose ? TRUE : FALSE, NULL);
...
static void gimp_global_config_notify(GObject *global_config,
  ...
  if (g_param_values_cmp(param_spec, &global_value, &edit_value)) {
    g_signal_handlers_block_by_func(edit_config, gimp_edit_config_notify,
                                    global_config);

    g_object_set_property(edit_config, param_spec->name, &global_value);

    g_signal_handlers_unblock_by_func(edit_config, gimp_edit_config_notify,
                                      global_config);
  }
...
void gimp_load_config(Gimp *gimp, GFile *alternate_system_gimprc,
  ...
  g_return_if_fail(alternate_gimprc == NULL || G_IS_FILE(alternate_gimprc));
  ...
  gimprc = gimp_rc_new(G_OBJECT(gimp), alternate_system_gimprc,
  ...
  g_signal_connect_object(gimp->edit_config, "notify",
                          G_CALLBACK(gimp_edit_config_notify), gimp->config, 0);
  ...
  if (!gimp->show_playground) {
    ...
    g_object_get(gimp->edit_config, "use-opencl", &use_opencl,
...
void gimp_exit(Gimp *gimp, gboolean force) {
  ...
  g_signal_emit(gimp, gimp_signals[EXIT], 0, force ? TRUE : FALSE, &handled);
...
GimpToolInfo *gimp_get_tool_info(Gimp *gimp, const gchar *tool_id) {
  gpointer info;
