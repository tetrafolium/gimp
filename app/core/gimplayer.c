...
static void gimp_layer_set_property(GObject *object, guint property_id,
                                    const GValue *value, GParamSpec *pspec);
static void gimp_layer_get_property(GObject *object, guint property_id,
                                    GValue *value, GParamSpec *pspec);
static void gimp_layer_dispose(GObject *object);
static void gimp_layer_finalize(GObject *object);
static void gimp_layer_notify(GObject *object, GParamSpec *pspec);

static void gimp_layer_name_changed(GimpObject *object);
static gint64 gimp_layer_get_memsize(GimpObject *object, gint64 *gui_size);
...
static void gimp_layer_convert(GimpItem *item, GimpImage *dest_image,
                               GType old_type);
static gboolean gimp_layer_rename(GimpItem *item, const gchar *new_name,
                                  const gchar *undo_desc, GError **error);
static void gimp_layer_start_move(GimpItem *item, gboolean push_undo);
static void gimp_layer_end_move(GimpItem *item, gboolean push_undo);
static void gimp_layer_translate(GimpItem *item, gdouble offset_x,
                                 gdouble offset_y, gboolean push_undo);
static void gimp_layer_scale(GimpItem *item, gint new_width, gint new_height,
                             gint new_offset_x, gint new_offset_y,
                             GimpInterpolationType interp_type,
                             GimpProgress *progress);
static void gimp_layer_resize(GimpItem *item, GimpContext *context,
                              GimpFillType fill_type, gint new_width,
                              gint new_height, gint offset_x, gint offset_y);
static void gimp_layer_flip(GimpItem *item, GimpContext *context,
                            GimpOrientationType flip_type, gdouble axis,
                            gboolean clip_result);
static void gimp_layer_rotate(GimpItem *item, GimpContext *context,
                              GimpRotationType rotate_type, gdouble center_x,
                              gdouble center_y, gboolean clip_result);
static void gimp_layer_transform(GimpItem *item, GimpContext *context,
                                 const GimpMatrix3 *matrix,
                                 GimpTransformDirection direction,
                                 GimpInterpolationType interpolation_type,
                                 GimpTransformResize clip_result,
                                 GimpProgress *progress);
static void gimp_layer_to_selection(GimpItem *item, GimpChannelOps op,
                                    gboolean antialias, gboolean feather,
...
static void gimp_layer_convert_type(
    GimpDrawable *drawable, GimpImage *dest_image, const Babl *new_format,
    GimpColorProfile *src_profile, GimpColorProfile *dest_profile,
    GeglDitherMethod layer_dither_type, GeglDitherMethod mask_dither_type,
    gboolean push_undo, GimpProgress *progress);
static void gimp_layer_invalidate_boundary(GimpDrawable *drawable);
static void gimp_layer_get_active_components(GimpDrawable *drawable,
                                             gboolean *active);
static GimpComponentMask gimp_layer_get_active_mask(GimpDrawable *drawable);
static void gimp_layer_set_buffer(GimpDrawable *drawable, gboolean push_undo,
                                  const gchar *undo_desc, GeglBuffer *buffer,
                                  const GeglRectangle *bounds);
static GeglRectangle gimp_layer_get_bounding_box(GimpDrawable *drawable);

static GimpColorProfile *
gimp_layer_get_color_profile(GimpColorManaged *managed);

static gdouble gimp_layer_get_opacity_at(GimpPickable *pickable, gint x,
                                         gint y);
static void gimp_layer_pixel_to_srgb(GimpPickable *pickable, const Babl *format,
...
static void gimp_layer_real_translate(GimpLayer *layer, gint offset_x,
                                      gint offset_y);
static void gimp_layer_real_scale(GimpLayer *layer, gint new_width,
                                  gint new_height, gint new_offset_x,
                                  gint new_offset_y,
                                  GimpInterpolationType interp_type,
                                  GimpProgress *progress);
static void gimp_layer_real_resize(GimpLayer *layer, GimpContext *context,
                                   GimpFillType fill_type, gint new_width,
                                   gint new_height, gint offset_x,
                                   gint offset_y);
static void gimp_layer_real_flip(GimpLayer *layer, GimpContext *context,
                                 GimpOrientationType flip_type, gdouble axis,
                                 gboolean clip_result);
static void gimp_layer_real_rotate(GimpLayer *layer, GimpContext *context,
                                   GimpRotationType rotate_type,
                                   gdouble center_x, gdouble center_y,
                                   gboolean clip_result);
static void gimp_layer_real_transform(GimpLayer *layer, GimpContext *context,
                                      const GimpMatrix3 *matrix,
                                      GimpTransformDirection direction,
                                      GimpInterpolationType interpolation_type,
                                      GimpTransformResize clip_result,
...
static void gimp_layer_real_convert_type(
    GimpLayer *layer, GimpImage *dest_image, const Babl *new_format,
    GimpColorProfile *src_profile, GimpColorProfile *dest_profile,
    GeglDitherMethod layer_dither_type, GeglDitherMethod mask_dither_type,
    gboolean push_undo, GimpProgress *progress);
static GeglRectangle gimp_layer_real_get_bounding_box(GimpLayer *layer);
static void
gimp_layer_real_get_effective_mode(GimpLayer *layer, GimpLayerMode *mode,
                                   GimpLayerColorSpace *blend_space,
                                   GimpLayerColorSpace *composite_space,
                                   GimpLayerCompositeMode *composite_mode);
static gboolean gimp_layer_real_get_excludes_backdrop(GimpLayer *layer);

static void gimp_layer_layer_mask_update(GimpDrawable *layer_mask, gint x,
...
static void gimp_layer_class_init(GimpLayerClass *klass) {
  ...
  layer_signals[OPACITY_CHANGED] = g_signal_new(
      "opacity-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLayerClass, opacity_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  layer_signals[MODE_CHANGED] =
      g_signal_new("mode-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpLayerClass, mode_changed), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);

  layer_signals[BLEND_SPACE_CHANGED] = g_signal_new(
      "blend-space-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLayerClass, blend_space_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  layer_signals[COMPOSITE_SPACE_CHANGED] = g_signal_new(
      "composite-space-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLayerClass, composite_space_changed), NULL, NULL,
      NULL, G_TYPE_NONE, 0);

  layer_signals[COMPOSITE_MODE_CHANGED] = g_signal_new(
      "composite-mode-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLayerClass, composite_mode_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  layer_signals[EFFECTIVE_MODE_CHANGED] = g_signal_new(
      "effective-mode-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLayerClass, effective_mode_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  layer_signals[EXCLUDES_BACKDROP_CHANGED] = g_signal_new(
      "excludes-backdrop-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLayerClass, excludes_backdrop_changed), NULL, NULL,
      NULL, G_TYPE_NONE, 0);

  layer_signals[LOCK_ALPHA_CHANGED] = g_signal_new(
      "lock-alpha-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLayerClass, lock_alpha_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  layer_signals[MASK_CHANGED] =
      g_signal_new("mask-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpLayerClass, mask_changed), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);

  layer_signals[APPLY_MASK_CHANGED] = g_signal_new(
      "apply-mask-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLayerClass, apply_mask_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  layer_signals[EDIT_MASK_CHANGED] = g_signal_new(
      "edit-mask-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLayerClass, edit_mask_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  layer_signals[SHOW_MASK_CHANGED] = g_signal_new(
      "show-mask-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLayerClass, show_mask_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("opacity", NULL, NULL, GIMP_OPACITY_TRANSPARENT,
                          GIMP_OPACITY_OPAQUE, GIMP_OPACITY_OPAQUE,
                          GIMP_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_MODE,
      g_param_spec_enum("mode", NULL, NULL, GIMP_TYPE_LAYER_MODE,
                        GIMP_LAYER_MODE_NORMAL, GIMP_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_BLEND_SPACE,
      g_param_spec_enum("blend-space", NULL, NULL, GIMP_TYPE_LAYER_COLOR_SPACE,
                        GIMP_LAYER_COLOR_SPACE_AUTO, GIMP_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_COMPOSITE_SPACE,
      g_param_spec_enum("composite-space", NULL, NULL,
                        GIMP_TYPE_LAYER_COLOR_SPACE,
                        GIMP_LAYER_COLOR_SPACE_AUTO, GIMP_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_COMPOSITE_MODE,
      g_param_spec_enum("composite-mode", NULL, NULL,
                        GIMP_TYPE_LAYER_COMPOSITE_MODE,
                        GIMP_LAYER_COMPOSITE_AUTO, GIMP_PARAM_READABLE));
  ...
  g_object_class_install_property(object_class, PROP_LOCK_ALPHA,
                                  g_param_spec_boolean("lock-alpha", NULL, NULL,
                                                       FALSE,
                                                       GIMP_PARAM_READABLE));

  g_object_class_install_property(object_class, PROP_MASK,
                                  g_param_spec_object("mask", NULL, NULL,
...
static void gimp_layer_dispose(GObject *object) {
  ...
  if (layer->mask)
    g_signal_handlers_disconnect_by_func(layer->mask,
                                         gimp_layer_layer_mask_update, layer);
...
static void gimp_layer_update_mode_node(GimpLayer *layer) {
  GeglNode *mode_node;
  ...
  gimp_gegl_mode_node_set_mode(mode_node, visible_mode, visible_blend_space,
                               visible_composite_space, visible_composite_mode);
...
static void gimp_layer_name_changed(GimpObject *object) {
  ...
  if (layer->mask) {
    gchar *mask_name =
        g_strdup_printf(_("%s mask"), gimp_object_get_name(object));
...
static gchar *gimp_layer_get_description(GimpViewable *viewable,
  ...
  return GIMP_VIEWABLE_CLASS(parent_class)->get_description(viewable, tooltip);
}
...
static GeglNode *gimp_layer_get_node(GimpFilter *filter) {
  ...
  gegl_node_connect_to(input, "output", source, "input");
  ...
  layer->layer_offset_node =
      gegl_node_new_child(node, "operation", "gegl:translate", NULL);
  gimp_item_add_offset_node(GIMP_ITEM(layer), layer->layer_offset_node);

  /* the layer mask's offset node */
  layer->mask_offset_node =
      gegl_node_new_child(node, "operation", "gegl:translate", NULL);
  gimp_item_add_offset_node(GIMP_ITEM(layer), layer->mask_offset_node);

  if (!source_node_hijacked) {
    gegl_node_connect_to(source, "output", layer->layer_offset_node, "input");
  }

  if (!(layer->mask && gimp_layer_get_show_mask(layer))) {
    gegl_node_connect_to(layer->layer_offset_node, "output", mode_node, "aux");
  }
  ...
  if (layer->mask) {
    ...
    gegl_node_connect_to(mask, "output", layer->mask_offset_node, "input");

    if (gimp_layer_get_show_mask(layer)) {
      gegl_node_connect_to(layer->mask_offset_node, "output", mode_node, "aux");
    } else if (gimp_layer_get_apply_mask(layer)) {
      gegl_node_connect_to(layer->mask_offset_node, "output", mode_node,
                           "aux2");
    }
  }
...
static gboolean gimp_layer_is_attached(GimpItem *item) {
  ...
  return (GIMP_IS_IMAGE(image) &&
          gimp_container_have(gimp_image_get_layers(image), GIMP_OBJECT(item)));
}
...
static GimpItem *gimp_layer_duplicate(GimpItem *item, GType new_type) {
  ...
  if (GIMP_IS_LAYER(new_item)) {
    ...
    gimp_layer_set_blend_space(new_layer, gimp_layer_get_blend_space(layer),
                               FALSE);
    gimp_layer_set_composite_space(
        new_layer, gimp_layer_get_composite_space(layer), FALSE);
    ...
    if (gimp_layer_can_lock_alpha(new_layer))
      gimp_layer_set_lock_alpha(new_layer, gimp_layer_get_lock_alpha(layer),
                                FALSE);
...
static void gimp_layer_convert(GimpItem *item, GimpImage *dest_image,
  ...
  if (old_base_type != new_base_type || old_precision != new_precision ||
      dest_profile) {
    gimp_drawable_convert_type(drawable, dest_image, new_base_type,
                               new_precision, gimp_drawable_has_alpha(drawable),
                               src_profile, dest_profile, GEGL_DITHER_NONE,
                               GEGL_DITHER_NONE, FALSE, NULL);
  }
...
static void gimp_layer_translate(GimpItem *item, gdouble offset_x,
  ...
  GIMP_LAYER_GET_CLASS(layer)->translate(layer, SIGNED_ROUND(offset_x),
...
static void gimp_layer_scale(GimpItem *item, gint new_width, gint new_height,
  ...
  GIMP_LAYER_GET_CLASS(layer)->scale(layer, new_width, new_height, new_offset_x,
                                     new_offset_y, interpolation_type,
                                     progress);
  ...
  if (layer->mask) {
    ...
    gimp_item_scale(GIMP_ITEM(layer->mask), new_width, new_height, new_offset_x,
                    new_offset_y, interpolation_type, progress);
  }
...
static void gimp_layer_resize(GimpItem *item, GimpContext *context,
  ...
  GIMP_LAYER_GET_CLASS(layer)->resize(layer, context, fill_type, new_width,
                                      new_height, offset_x, offset_y);
...
static void gimp_layer_flip(GimpItem *item, GimpContext *context,
  ...
  if (layer->mask)
    gimp_item_flip(GIMP_ITEM(layer->mask), context, flip_type, axis,
                   clip_result);
}
...
static void gimp_layer_rotate(GimpItem *item, GimpContext *context,
  ...
  GIMP_LAYER_GET_CLASS(layer)->rotate(layer, context, rotate_type, center_x,
                                      center_y, clip_result);

  if (layer->mask)
    gimp_item_rotate(GIMP_ITEM(layer->mask), context, rotate_type, center_x,
                     center_y, clip_result);
}
...
static void gimp_layer_transform(GimpItem *item, GimpContext *context,
  ...
  if (layer->mask) {
    ...
    gimp_item_transform(GIMP_ITEM(layer->mask), context, matrix, direction,
                        interpolation_type, clip_result, progress);
  }
...
static void gimp_layer_to_selection(GimpItem *item, GimpChannelOps op,
  ...
  gimp_channel_select_alpha(gimp_image_get_mask(image), GIMP_DRAWABLE(layer),
                            op, feather, feather_radius_x, feather_radius_y);
}
...
static gint64 gimp_layer_estimate_memsize(GimpDrawable *drawable,
  ...
  if (layer->mask)
    memsize += gimp_drawable_estimate_memsize(GIMP_DRAWABLE(layer->mask),
                                              component_type, width, height);

  return memsize +
         GIMP_DRAWABLE_CLASS(parent_class)
             ->estimate_memsize(drawable, component_type, width, height);
}
...
static void gimp_layer_convert_type(
   ...
   */
  if (dest_profile) {
    dest_space = gimp_color_profile_get_space(
        dest_profile, GIMP_COLOR_RENDERING_INTENT_RELATIVE_COLORIMETRIC, NULL);
  } else {
  ...
  GIMP_LAYER_GET_CLASS(layer)->convert_type(
      layer, dest_image, space_format, src_profile, dest_profile,
      layer_dither_type, mask_dither_type, push_undo, progress);
  ...
  if (convert_mask) {
    ...
    gimp_drawable_convert_type(
        GIMP_DRAWABLE(layer->mask), dest_image, GIMP_GRAY,
        gimp_babl_format_get_precision(new_format),
        gimp_drawable_has_alpha(GIMP_DRAWABLE(layer->mask)), NULL, NULL,
        layer_dither_type, mask_dither_type, push_undo, progress);
  }
...
static void gimp_layer_set_buffer(GimpDrawable *drawable, gboolean push_undo,
  ...
  GIMP_DRAWABLE_CLASS(parent_class)
      ->set_buffer(drawable, push_undo, undo_desc, buffer, bounds);
...
static GeglRectangle gimp_layer_get_bounding_box(GimpDrawable *drawable) {
  ...
  if (mask && gimp_layer_get_show_mask(layer)) {
    ...
    if (mask && gimp_layer_get_apply_mask(layer)) {
      ...
      mask_bounding_box = gimp_drawable_get_bounding_box(GIMP_DRAWABLE(mask));

      gegl_rectangle_intersect(&bounding_box, &bounding_box,
                               &mask_bounding_box);
    }
  }
...
static gdouble gimp_layer_get_opacity_at(GimpPickable *pickable, gint x,
  ...
  if (x >= 0 && x < gimp_item_get_width(GIMP_ITEM(layer)) && y >= 0 &&
      y < gimp_item_get_height(GIMP_ITEM(layer)) &&
      gimp_item_is_visible(GIMP_ITEM(layer))) {
    if (!gimp_drawable_has_alpha(GIMP_DRAWABLE(layer))) {
      value = GIMP_OPACITY_OPAQUE;
    } else {
      gegl_buffer_sample(gimp_drawable_get_buffer(GIMP_DRAWABLE(layer)), x, y,
                         NULL, &value, babl_format("A double"),
    ...
    if (gimp_layer_get_mask(layer) && gimp_layer_get_apply_mask(layer)) {
      ...
      mask_value =
          gimp_pickable_get_opacity_at(GIMP_PICKABLE(layer->mask), x, y);
...
static void gimp_layer_real_translate(GimpLayer *layer, gint offset_x,
  ...
  GIMP_ITEM_CLASS(parent_class)
      ->translate(GIMP_ITEM(layer), offset_x, offset_y, FALSE);
...
static void gimp_layer_real_scale(GimpLayer *layer, gint new_width,
                                  ...
                                  GimpProgress *progress) {
  GIMP_ITEM_CLASS(parent_class)
      ->scale(GIMP_ITEM(layer), new_width, new_height, new_offset_x,
              new_offset_y, interpolation_type, progress);
}
...
static void gimp_layer_real_resize(GimpLayer *layer, GimpContext *context,
  ...
  GIMP_ITEM_CLASS(parent_class)
      ->resize(GIMP_ITEM(layer), context, fill_type, new_width, new_height,
               offset_x, offset_y);
}
...
static void gimp_layer_real_flip(GimpLayer *layer, GimpContext *context,
                                 ...
                                 gboolean clip_result) {
  GIMP_ITEM_CLASS(parent_class)
      ->flip(GIMP_ITEM(layer), context, flip_type, axis, clip_result);
}
...
static void gimp_layer_real_rotate(GimpLayer *layer, GimpContext *context,
                                   ...
                                   gboolean clip_result) {
  GIMP_ITEM_CLASS(parent_class)
      ->rotate(GIMP_ITEM(layer), context, rotate_type, center_x, center_y,
               clip_result);
}
...
static void gimp_layer_real_transform(GimpLayer *layer, GimpContext *context,
                                      ...
                                      GimpProgress *progress) {
  GIMP_ITEM_CLASS(parent_class)
      ->transform(GIMP_ITEM(layer), context, matrix, direction,
                  interpolation_type, clip_result, progress);
}
...
static void gimp_layer_real_convert_type(
  ...
  if (layer_dither_type == GEGL_DITHER_NONE) {
    ...
    src_buffer = gegl_buffer_new(
        GEGL_RECTANGLE(0, 0, gimp_item_get_width(GIMP_ITEM(layer)),
    ...
    gimp_gegl_apply_dither(gimp_drawable_get_buffer(drawable), NULL, NULL,
                           src_buffer, 1 << bits, layer_dither_type);
  }

  dest_buffer = gegl_buffer_new(
      GEGL_RECTANGLE(0, 0, gimp_item_get_width(GIMP_ITEM(layer)),
                     gimp_item_get_height(GIMP_ITEM(layer))),
      new_format);
  ...
  if (dest_profile) {
    ...
    gimp_gegl_convert_color_profile(
        src_buffer, NULL, src_profile, dest_buffer, NULL, dest_profile,
        GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL, TRUE, progress);
  } else {
...
static GeglRectangle gimp_layer_real_get_bounding_box(GimpLayer *layer) {
  return GIMP_DRAWABLE_CLASS(parent_class)
      ->get_bounding_box(GIMP_DRAWABLE(layer));
}
...
static gboolean gimp_layer_real_get_excludes_backdrop(GimpLayer *layer) {
  ...
  included_region = gimp_layer_mode_get_included_region(
      layer->mode, layer->effective_composite_mode);

  return !(included_region & GIMP_LAYER_COMPOSITE_REGION_DESTINATION);
}

static void gimp_layer_layer_mask_update(GimpDrawable *drawable, gint x, gint y,
                                         gint width, gint height,
                                         GimpLayer *layer) {
  if (gimp_layer_get_apply_mask(layer) || gimp_layer_get_show_mask(layer)) {
    gimp_drawable_update(GIMP_DRAWABLE(layer), x, y, width, height);
  }
}

/*  public functions  */

void gimp_layer_fix_format_space(GimpLayer *layer, gboolean copy_buffer,
                                 gboolean push_undo) {
...
GimpLayerMask *gimp_layer_add_mask(GimpLayer *layer, GimpLayerMask *mask,
  ...
  g_return_val_if_fail(gimp_item_get_image(GIMP_ITEM(layer)) ==
                           gimp_item_get_image(GIMP_ITEM(mask)),
                       NULL);
  ...
  if (push_undo)
    gimp_image_undo_push_layer_mask_add(
        image, C_("undo-type", "Add Layer Mask"), layer, mask);
  ...
  g_signal_connect(mask, "update", G_CALLBACK(gimp_layer_layer_mask_update),
                   layer);
...
GimpLayerMask *gimp_layer_create_mask(GimpLayer *layer,
  ...
  g_return_val_if_fail(
      add_mask_type != GIMP_ADD_MASK_CHANNEL || GIMP_IS_CHANNEL(channel), NULL);
  ...
  mask = gimp_layer_mask_new(image, gimp_item_get_width(item),
                             gimp_item_get_height(item), mask_name, &black);
  ...
  case GIMP_ADD_MASK_ALPHA_TRANSFER:
    if (gimp_drawable_has_alpha(drawable)) {
      ...
      gimp_gegl_buffer_copy(gimp_drawable_get_buffer(drawable), NULL,
                            GEGL_ABYSS_NONE, dest_buffer, NULL);
      gegl_buffer_set_format(dest_buffer, NULL);

      if (add_mask_type == GIMP_ADD_MASK_ALPHA_TRANSFER) {
        gimp_drawable_push_undo(
            drawable, C_("undo-type", "Transfer Alpha to Mask"), NULL, 0, 0,
            gimp_item_get_width(item), gimp_item_get_height(item));

        gimp_gegl_apply_set_alpha(gimp_drawable_get_buffer(drawable), NULL,
                                  NULL, gimp_drawable_get_buffer(drawable),
                                  1.0);
      }
    }
    break;
  ...
  case GIMP_ADD_MASK_CHANNEL: {
    ...
    gimp_rectangle_intersect(0, 0, gimp_image_get_width(image),
                             gimp_image_get_height(image), offset_x, offset_y,
                             gimp_item_get_width(item),
                             gimp_item_get_height(item), &copy_x, &copy_y,
                             &copy_width, &copy_height);

    if (copy_width < gimp_item_get_width(item) ||
        copy_height < gimp_item_get_height(item) || channel_empty)
      gimp_channel_clear(GIMP_CHANNEL(mask), NULL, FALSE);
    ...
    if ((copy_width || copy_height) && !channel_empty) {
      ...
      gimp_gegl_buffer_copy(
          ...
          GEGL_ABYSS_NONE, dest,
          GEGL_RECTANGLE(copy_x - offset_x, copy_y - offset_y, 0, 0));
  ...
  case GIMP_ADD_MASK_COPY: {
    ...
    if (!gimp_drawable_is_gray(drawable)) {
      const Babl *copy_format = gimp_image_get_format(
          image, GIMP_GRAY, gimp_drawable_get_precision(drawable),
          gimp_drawable_has_alpha(drawable), NULL);

      src_buffer =
          gegl_buffer_new(GEGL_RECTANGLE(0, 0, gimp_item_get_width(item),
                                         gimp_item_get_height(item)),
                          copy_format);

      gimp_gegl_buffer_copy(gimp_drawable_get_buffer(drawable), NULL,
                            GEGL_ABYSS_NONE, src_buffer, NULL);
    } else {
    ...
    if (gimp_drawable_has_alpha(drawable)) {
      ...
      gimp_gegl_apply_flatten(src_buffer, NULL, NULL, dest_buffer, &background,
                              NULL, GIMP_LAYER_COLOR_SPACE_RGB_LINEAR);
    } else {
      gimp_gegl_buffer_copy(src_buffer, NULL, GEGL_ABYSS_NONE, dest_buffer,
                            NULL);
    }
...
void gimp_layer_apply_mask(GimpLayer *layer, GimpMaskApplyMode mode,
                           gboolean push_undo) {
  GimpItem *item;
  GimpImage *image;
  GimpLayerMask *mask;
  ...
  if (mode == GIMP_MASK_APPLY) {
    ...
    if (push_undo)
      gimp_drawable_push_undo(GIMP_DRAWABLE(layer), NULL, NULL, 0, 0,
                              gimp_item_get_width(item),
                              gimp_item_get_height(item));
    ...
    gimp_gegl_apply_opacity(gimp_drawable_get_buffer(GIMP_DRAWABLE(layer)),
                            NULL, NULL, dest_buffer, mask_buffer, 0, 0, 1.0);
  }

  g_signal_handlers_disconnect_by_func(mask, gimp_layer_layer_mask_update,
                                       layer);
  ...
  if (gimp_filter_peek_node(GIMP_FILTER(layer))) {
    ...
    if (layer->show_mask) {
      gegl_node_connect_to(layer->layer_offset_node, "output", mode_node,
                           "aux");
    } else {
...
void gimp_layer_set_apply_mask(GimpLayer *layer, gboolean apply,
                               gboolean push_undo) {
  g_return_if_fail(GIMP_IS_LAYER(layer));
  ...
  if (layer->apply_mask != apply) {
    ...
    if (push_undo && gimp_item_is_attached(GIMP_ITEM(layer)))
      gimp_image_undo_push_layer_mask_apply(
          image,
          apply ? C_("undo-type", "Enable Layer Mask")
...
void gimp_layer_set_edit_mask(GimpLayer *layer, gboolean edit) {
  g_return_if_fail(GIMP_IS_LAYER(layer));
  g_return_if_fail(layer->mask != NULL);
...
void gimp_layer_set_show_mask(GimpLayer *layer, gboolean show,
                              gboolean push_undo) {
  g_return_if_fail(GIMP_IS_LAYER(layer));
  g_return_if_fail(layer->mask != NULL);
  ...
  if (layer->show_mask != show) {
    ...
    if (push_undo)
      gimp_image_undo_push_layer_mask_show(
          image, C_("undo-type", "Show Layer Mask"), layer);
...
void gimp_layer_add_alpha(GimpLayer *layer) {
  ...
  new_buffer = gegl_buffer_new(GEGL_RECTANGLE(0, 0, gimp_item_get_width(item),
                                              gimp_item_get_height(item)),
                               gimp_drawable_get_format_with_alpha(drawable));

  gimp_gegl_buffer_copy(gimp_drawable_get_buffer(drawable), NULL,
                        GEGL_ABYSS_NONE, new_buffer, NULL);

  gimp_drawable_set_buffer(GIMP_DRAWABLE(layer),
                           gimp_item_is_attached(GIMP_ITEM(layer)),
                           C_("undo-type", "Add Alpha Channel"), new_buffer);
  g_object_unref(new_buffer);
}

void gimp_layer_remove_alpha(GimpLayer *layer, GimpContext *context) {
  GeglBuffer *new_buffer;
  GimpRGB background;
  ...
  new_buffer = gegl_buffer_new(
      GEGL_RECTANGLE(0, 0, gimp_item_get_width(GIMP_ITEM(layer)),
                     gimp_item_get_height(GIMP_ITEM(layer))),
      gimp_drawable_get_format_without_alpha(GIMP_DRAWABLE(layer)));

  gimp_context_get_background(context, &background);
  gimp_pickable_srgb_to_image_color(GIMP_PICKABLE(layer), &background,
                                    &background);

  gimp_gegl_apply_flatten(gimp_drawable_get_buffer(GIMP_DRAWABLE(layer)), NULL,
                          NULL, new_buffer, &background,
                          gimp_drawable_get_space(GIMP_DRAWABLE(layer)),
                          gimp_layer_get_real_composite_space(layer));

  gimp_drawable_set_buffer(GIMP_DRAWABLE(layer),
                           gimp_item_is_attached(GIMP_ITEM(layer)),
                           C_("undo-type", "Remove Alpha Channel"), new_buffer);
  g_object_unref(new_buffer);
}

void gimp_layer_resize_to_image(GimpLayer *layer, GimpContext *context,
...
void gimp_layer_set_opacity(GimpLayer *layer, gdouble opacity,
                            gboolean push_undo) {
  g_return_if_fail(GIMP_IS_LAYER(layer));
...
void gimp_layer_set_mode(GimpLayer *layer, GimpLayerMode mode,
                         gboolean push_undo) {
  g_return_if_fail(GIMP_IS_LAYER(layer));
...
void gimp_layer_set_blend_space(GimpLayer *layer,
  ...
  if (layer->blend_space != blend_space) {
    if (push_undo && gimp_item_is_attached(GIMP_ITEM(layer))) {
      ...
      gimp_image_undo_push_layer_mode(image, _("Set layer's blend space"),
                                      layer);
    }
...
GimpLayerColorSpace gimp_layer_get_real_blend_space(GimpLayer *layer) {
...
void gimp_layer_set_composite_space(GimpLayer *layer,
  ...
  if (layer->composite_space != composite_space) {
    if (push_undo && gimp_item_is_attached(GIMP_ITEM(layer))) {
      ...
      gimp_image_undo_push_layer_mode(image, _("Set layer's composite space"),
                                      layer);
    }
...
GimpLayerColorSpace gimp_layer_get_real_composite_space(GimpLayer *layer) {
  g_return_val_if_fail(GIMP_IS_LAYER(layer), GIMP_LAYER_COLOR_SPACE_RGB_LINEAR);
...
void gimp_layer_set_composite_mode(GimpLayer *layer,
  ...
  if (layer->composite_mode != composite_mode) {
    if (push_undo && gimp_item_is_attached(GIMP_ITEM(layer))) {
      ...
      gimp_image_undo_push_layer_mode(image, _("Set layer's composite mode"),
                                      layer);
    }
...
GimpLayerCompositeMode gimp_layer_get_real_composite_mode(GimpLayer *layer) {
...
void gimp_layer_get_effective_mode(GimpLayer *layer, GimpLayerMode *mode,
                                   GimpLayerColorSpace *blend_space,
                                   GimpLayerColorSpace *composite_space,
                                   GimpLayerCompositeMode *composite_mode) {
  g_return_if_fail(GIMP_IS_LAYER(layer));
...
void gimp_layer_set_lock_alpha(GimpLayer *layer, gboolean lock_alpha,
                               gboolean push_undo) {
  g_return_if_fail(GIMP_IS_LAYER(layer));
...
void gimp_layer_update_effective_mode(GimpLayer *layer) {
  ...
  if (layer->mask && layer->show_mask) {
    ...
    GIMP_LAYER_GET_CLASS(layer)->get_effective_mode(
        layer, &mode, &blend_space, &composite_space, &composite_mode);
  }
...
void gimp_layer_update_excludes_backdrop(GimpLayer *layer) {
  ...
  excludes_backdrop = GIMP_LAYER_GET_CLASS(layer)->get_excludes_backdrop(layer);
