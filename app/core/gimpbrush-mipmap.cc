...
static void gimp_brush_mipmap_clear(GimpBrush *brush, GimpTempBuf ***mipmaps);
...
static void gimp_brush_mipmap_clear(GimpBrush *brush, GimpTempBuf ***mipmaps) {
  if (*mipmaps) {
    ...
    for (i = 0; i < brush->priv->n_horz_mipmaps * brush->priv->n_vert_mipmaps;
         i++) {
...
static const GimpTempBuf *gimp_brush_mipmap_get(GimpBrush *brush,
  ...
  x = floor(SAFE_CLAMP(log(1.0 / MAX(*scale_x, 0.0)) / M_LN2, 0,
                       brush->priv->n_horz_mipmaps - 1));
  y = floor(SAFE_CLAMP(log(1.0 / MAX(*scale_y, 0.0)) / M_LN2, 0,
                       brush->priv->n_vert_mipmaps - 1));
...
template <class T> struct MipmapTraits;

template <> struct MipmapTraits<guint8> {
  static guint8 mix(guint8 a, guint8 b) {
    return ((guint32)a + (guint32)b + 1) / 2;
  }

  static guint8 mix(guint8 a, guint8 b, guint8 c, guint8 d) {
    return ((guint32)a + (guint32)b + (guint32)c + (guint32)d + 2) / 4;
  }
};

template <> struct MipmapTraits<gfloat> {
  static gfloat mix(gfloat a, gfloat b) { return (a + b) / 2.0; }

  static gfloat mix(gfloat a, gfloat b, gfloat c, gfloat d) {
    return (a + b + c + d) / 4.0;
  }
};

template <class T, gint N> struct MipmapAlgorithms {
  static GimpTempBuf *downscale(const GimpTempBuf *source) {
    ...
    destination =
        gimp_temp_buf_new(width, height, gimp_temp_buf_get_format(source));
  ...
  static GimpTempBuf *downscale_horz(const GimpTempBuf *source) {
    GimpTempBuf *destination;
    gint width = gimp_temp_buf_get_width(source);
    gint height = gimp_temp_buf_get_height(source);
    ...
    gegl_parallel_distribute_range(
        height, PIXELS_PER_THREAD / width, [=](gint offset, gint size) {
          const T *src0 = (const T *)gimp_temp_buf_get_data(source);
          T *dest0 = (T *)gimp_temp_buf_get_data(destination);
          gint src_stride = N * gimp_temp_buf_get_width(source);
          gint dest_stride = N * gimp_temp_buf_get_width(destination);
  ...
  static GimpTempBuf *downscale_vert(const GimpTempBuf *source) {
    GimpTempBuf *destination;
    gint width = gimp_temp_buf_get_width(source);
    gint height = gimp_temp_buf_get_height(source);
    ...
    gegl_parallel_distribute_range(
        width, PIXELS_PER_THREAD / height, [=](gint offset, gint size) {
          const T *src0 = (const T *)gimp_temp_buf_get_data(source);
          T *dest0 = (T *)gimp_temp_buf_get_data(destination);
          gint src_stride = N * gimp_temp_buf_get_width(source);
          gint dest_stride = N * gimp_temp_buf_get_width(destination);
...
template <class Func>
static GimpTempBuf *gimp_brush_mipmap_dispatch(const GimpTempBuf *source,
                                               Func func) {
...
static GimpTempBuf *gimp_brush_mipmap_downscale(const GimpTempBuf *source) {
  return gimp_brush_mipmap_dispatch(
      source, [&](auto algorithms) { return algorithms.downscale(source); });
}

static GimpTempBuf *
gimp_brush_mipmap_downscale_horz(const GimpTempBuf *source) {
  return gimp_brush_mipmap_dispatch(source, [&](auto algorithms) {
...
const GimpTempBuf *gimp_brush_mipmap_get_mask(GimpBrush *brush,
                                              gdouble *scale_x,
                                              gdouble *scale_y) {
  return gimp_brush_mipmap_get(brush, brush->priv->mask,
                               &brush->priv->mask_mipmaps, scale_x, scale_y);
}

const GimpTempBuf *gimp_brush_mipmap_get_pixmap(GimpBrush *brush,
                                                gdouble *scale_x,
                                                gdouble *scale_y) {
  return gimp_brush_mipmap_get(brush, brush->priv->pixmap,
                               &brush->priv->pixmap_mipmaps, scale_x, scale_y);
}
...
gsize gimp_brush_mipmap_get_memsize(GimpBrush *brush) {
  ...
  if (brush->priv->mask_mipmaps) {
    ...
    for (i = 1; i < brush->priv->n_horz_mipmaps * brush->priv->n_vert_mipmaps;
         i++) {
