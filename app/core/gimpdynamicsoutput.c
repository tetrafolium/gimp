...
#define GET_PRIVATE(output)                                                    \
  ((GimpDynamicsOutputPrivate *)gimp_dynamics_output_get_instance_private(     \
...
static void gimp_dynamics_output_get_property(GObject *object,
                                              ...
                                              GParamSpec *pspec);
static void gimp_dynamics_output_copy_curve(GimpCurve *src, GimpCurve *dest);

static GimpCurve *gimp_dynamics_output_create_curve(GimpDynamicsOutput *output,
                                                    const gchar *name);
...
static void gimp_dynamics_output_class_init(GimpDynamicsOutputClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("type", NULL, _("Output type"),
                        GIMP_TYPE_DYNAMICS_OUTPUT_TYPE,
                        GIMP_DYNAMICS_OUTPUT_OPACITY,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_PRESSURE, "use-pressure",
                           NULL, NULL, DEFAULT_USE_PRESSURE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_VELOCITY, "use-velocity",
                           NULL, NULL, DEFAULT_USE_VELOCITY,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_DIRECTION, "use-direction",
                           NULL, NULL, DEFAULT_USE_DIRECTION,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_TILT, "use-tilt", NULL, NULL,
                           DEFAULT_USE_TILT, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_WHEEL, "use-wheel", NULL,
                           NULL, DEFAULT_USE_TILT, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_RANDOM, "use-random", NULL,
                           NULL, DEFAULT_USE_RANDOM, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_FADE, "use-fade", NULL, NULL,
                           DEFAULT_USE_FADE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_PRESSURE_CURVE, "pressure-curve",
                          NULL, NULL, GIMP_TYPE_CURVE,
                          GIMP_CONFIG_PARAM_AGGREGATE);

  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_VELOCITY_CURVE, "velocity-curve",
                          NULL, NULL, GIMP_TYPE_CURVE,
                          GIMP_CONFIG_PARAM_AGGREGATE);

  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_DIRECTION_CURVE, "direction-curve",
                          NULL, NULL, GIMP_TYPE_CURVE,
                          GIMP_CONFIG_PARAM_AGGREGATE);

  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_TILT_CURVE, "tilt-curve", NULL,
                          NULL, GIMP_TYPE_CURVE, GIMP_CONFIG_PARAM_AGGREGATE);

  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_WHEEL_CURVE, "wheel-curve", NULL,
                          NULL, GIMP_TYPE_CURVE, GIMP_CONFIG_PARAM_AGGREGATE);

  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_RANDOM_CURVE, "random-curve", NULL,
                          NULL, GIMP_TYPE_CURVE, GIMP_CONFIG_PARAM_AGGREGATE);

  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_FADE_CURVE, "fade-curve", NULL,
                          NULL, GIMP_TYPE_CURVE, GIMP_CONFIG_PARAM_AGGREGATE);
}

static void gimp_dynamics_output_init(GimpDynamicsOutput *output) {
  GimpDynamicsOutputPrivate *private = GET_PRIVATE(output);

private
  ->pressure_curve =
      gimp_dynamics_output_create_curve(output, "pressure-curve");
private
  ->velocity_curve =
      gimp_dynamics_output_create_curve(output, "velocity-curve");
private
  ->direction_curve =
      gimp_dynamics_output_create_curve(output, "direction-curve");
private
  ->tilt_curve = gimp_dynamics_output_create_curve(output, "tilt-curve");
private
  ->wheel_curve = gimp_dynamics_output_create_curve(output, "wheel-curve");
private
  ->random_curve = gimp_dynamics_output_create_curve(output, "random-curve");
private
  ->fade_curve = gimp_dynamics_output_create_curve(output, "fade-curve");
}
...
static void gimp_dynamics_output_set_property(GObject *object,
  ...
  private
    ->use_pressure = g_value_get_boolean(value);
    break;

  case PROP_USE_VELOCITY:
  private
    ->use_velocity = g_value_get_boolean(value);
    break;

  case PROP_USE_DIRECTION:
  private
    ->use_direction = g_value_get_boolean(value);
    break;

  case PROP_USE_TILT:
  private
    ->use_tilt = g_value_get_boolean(value);
    break;

  case PROP_USE_WHEEL:
  private
    ->use_wheel = g_value_get_boolean(value);
    break;

  case PROP_USE_RANDOM:
  private
    ->use_random = g_value_get_boolean(value);
    break;

  case PROP_USE_FADE:
  private
    ->use_fade = g_value_get_boolean(value);
    break;
...
GimpDynamicsOutput *gimp_dynamics_output_new(const gchar *name,
  ...
  return g_object_new(GIMP_TYPE_DYNAMICS_OUTPUT, "name", name, "type", type,
                      NULL);
}
...
gboolean gimp_dynamics_output_is_enabled(GimpDynamicsOutput *output) {
  ...
  return (private->use_pressure || private->use_velocity ||
          private->use_direction || private->use_tilt || private->use_wheel ||
          private->use_random || private->use_fade);
}
...
gdouble gimp_dynamics_output_get_linear_value(GimpDynamicsOutput *output,
  ...
  if (private->use_pressure) {
    total += gimp_curve_map_value(private->pressure_curve, coords->pressure);
    factors++;
  }

  if (private->use_velocity) {
    total +=
        gimp_curve_map_value(private->velocity_curve, (1.0 - coords->velocity));
    factors++;
  }
  ...
  if (private->use_tilt) {
    total += gimp_curve_map_value(
        private->tilt_curve,
        (1.0 - sqrt(SQR(coords->xtilt) + SQR(coords->ytilt))));
    factors++;
  }
...
gdouble gimp_dynamics_output_get_angular_value(GimpDynamicsOutput *output,
  ...
  gdouble total = 0.0;
  gdouble result =
      0.0; /* angles are additive, so we return zero for no change. */
  gint factors = 0;
  ...
  if (private->use_direction) {
    gdouble angle =
        gimp_curve_map_value(private->direction_curve, coords->direction);
  ...
  if (private->use_tilt) {
    ...
    if (tilt_x == 0.0) {
      ...
      tilt = atan((-1.0 * tilt_y) / tilt_x) / (2 * G_PI);
...
gdouble gimp_dynamics_output_get_aspect_value(GimpDynamicsOutput *output,
  ...
  if (private->use_velocity) {
    total += gimp_curve_map_value(private->velocity_curve, coords->velocity);
    factors++;
  }
  ...
  if (private->use_tilt) {
    ...
    tilt_value = gimp_curve_map_value(private->tilt_curve, tilt_value);
  ...
  if (private->use_wheel) {
    gdouble wheel = gimp_curve_map_value(private->wheel_curve, coords->wheel);
...
static void gimp_dynamics_output_copy_curve(GimpCurve *src, GimpCurve *dest) {
  if (src && dest) {
    gimp_config_copy(GIMP_CONFIG(src), GIMP_CONFIG(dest),
...
static GimpCurve *gimp_dynamics_output_create_curve(GimpDynamicsOutput *output,
  ...
  g_signal_connect_object(
      curve, "dirty", G_CALLBACK(gimp_dynamics_output_curve_dirty), output, 0);
