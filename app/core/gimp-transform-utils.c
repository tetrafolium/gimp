...
void gimp_transform_get_rotate_center(gint x, gint y, gint width, gint height,
...
void gimp_transform_get_flip_axis(gint x, gint y, gint width, gint height,
...
void gimp_transform_matrix_flip_free(GimpMatrix3 *matrix, gdouble x1,
...
void gimp_transform_matrix_rotate_rect(GimpMatrix3 *matrix, gint x, gint y,
                                       gint width, gint height, gdouble angle) {
  gdouble center_x;
  gdouble center_y;
...
void gimp_transform_matrix_rotate_center(GimpMatrix3 *matrix, gdouble center_x,
...
void gimp_transform_matrix_scale(GimpMatrix3 *matrix, gint x, gint y,
                                 gint width, gint height, gdouble t_x,
                                 gdouble t_y, gdouble t_width,
                                 gdouble t_height) {
  gdouble scale_x = 1.0;
  gdouble scale_y = 1.0;
...
void gimp_transform_matrix_shear(GimpMatrix3 *matrix, gint x, gint y,
...
void gimp_transform_matrix_perspective(GimpMatrix3 *matrix, gint x, gint y,
                                       gint width, gint height, gdouble t_x1,
                                       gdouble t_y1, gdouble t_x2, gdouble t_y2,
                                       gdouble t_x3, gdouble t_y3, gdouble t_x4,
                                       gdouble t_y4) {
  GimpMatrix3 trafo;
  gdouble scalex;
  gdouble scaley;
 ...
 */
static gboolean mod_gauss(gdouble matrix[], gdouble solution[], gint s) {
...
gboolean gimp_transform_matrix_generic(GimpMatrix3 *matrix,
  ...
  for (i = 0; i < 4; i++) {
    ...
    w = trafo.coeff[2][0] * input_points[i].x +
        trafo.coeff[2][1] * input_points[i].y + trafo.coeff[2][2];
...
gboolean gimp_transform_polygon_is_convex(gdouble x1, gdouble y1, gdouble x2,
...
void gimp_transform_polygon(const GimpMatrix3 *matrix,
                            const GimpVector2 *vertices, gint n_vertices,
                            gboolean closed, GimpVector2 *t_vertices,
                            gint *n_t_vertices) {
  GimpVector3 curr;
  ...
  *n_t_vertices = 0;
  ...
  curr.x = matrix->coeff[0][0] * vertices[0].x +
           matrix->coeff[0][1] * vertices[0].y + matrix->coeff[0][2];
  curr.y = matrix->coeff[1][0] * vertices[0].x +
           matrix->coeff[1][1] * vertices[0].y + matrix->coeff[1][2];
  curr.z = matrix->coeff[2][0] * vertices[0].x +
           matrix->coeff[2][1] * vertices[0].y + matrix->coeff[2][2];
  ...
  for (i = 0; i < n_vertices; i++) {
    if (curr_visible) {
      t_vertices[(*n_t_vertices)++] =
          (GimpVector2){curr.x / curr.z, curr.y / curr.z};
    }
    ...
    if (i < n_vertices - 1 || closed) {
      ...
      next.x = matrix->coeff[0][0] * vertices[j].x +
               matrix->coeff[0][1] * vertices[j].y + matrix->coeff[0][2];
      next.y = matrix->coeff[1][0] * vertices[j].x +
               matrix->coeff[1][1] * vertices[j].y + matrix->coeff[1][2];
      next.z = matrix->coeff[2][0] * vertices[j].x +
               matrix->coeff[2][1] * vertices[j].y + matrix->coeff[2][2];
      ...
      if (next_visible != curr_visible) {
        ...
        t_vertices[(*n_t_vertices)++] = (GimpVector2){
            (curr.x + (next.x - curr.x) * ratio) / GIMP_TRANSFORM_NEAR_Z,
...
void gimp_transform_polygon_coords(const GimpMatrix3 *matrix,
                                   const GimpCoords *vertices, gint n_vertices,
                                   gboolean closed, GimpCoords *t_vertices,
                                   gint *n_t_vertices) {
  GimpVector3 curr;
  gboolean curr_visible;
  gint i;
  ...
  for (i = 0; i < n_vertices; i++) {
    ...
    if (i < n_vertices - 1 || closed) {
      ...
      if (next_visible != curr_visible) {
        ...
        gimp_coords_mix(1.0 - ratio, &vertices[i], ratio, &vertices[j],
                        &t_vertices[*n_t_vertices]);

        t_vertices[*n_t_vertices].x =
            (curr.x + (next.x - curr.x) * ratio) / GIMP_TRANSFORM_NEAR_Z;
        t_vertices[*n_t_vertices].y =
            (curr.y + (next.y - curr.y) * ratio) / GIMP_TRANSFORM_NEAR_Z;
 ...
 */
static gdouble polynomial_eval(const gdouble *poly, gint degree, gdouble x) {
  gdouble y = poly[0];
  gint i;
 ...
 */
static void polynomial_derive(const gdouble *poly, gint degree,
                              gdouble *result) {
  while (degree)
    *result++ = *poly++ * degree--;
}
 ...
 */
static gboolean polynomial_odd_root(const gdouble *poly, gint degree,
                                    gdouble x1, gdouble x2, gdouble *root) {
  gdouble y1;
  gdouble y2;
  gint i;
 ...
 * 'n_roots'.
 */
static void polynomial_odd_roots(const gdouble *poly, gint degree, gdouble x1,
                                 gdouble x2, gdouble *roots, gint *n_roots) {
  *n_roots = 0;
...
#define ADD_ROOT(root)                                                         \
  ...
  default: {
    ...
    polynomial_odd_roots(deriv, degree - 1, x1, x2, deriv_roots,
                         &n_deriv_roots);
    ...
    for (i = 0; i <= n_deriv_roots; i++) {
      ...
      *n_roots += polynomial_odd_root(poly, degree, a, b, &roots[*n_roots]);
 ...
 */
static void clip_bezier(const GimpCoords bezier[4], gdouble a, gdouble b,
                        gdouble c, GimpCoords c_bezier[2][4], gint *n_c_bezier,
                        gboolean *start_in, gboolean *end_in) {
  gdouble dot[4];
  gdouble poly[4];
  gdouble roots[5];
  gint n_roots;
  ...
  *end_in = (*start_in + n_roots + 1) % 2;
  ...
  for (i = !*start_in; i < n_roots; i += 2) {
    ...
    gimp_coords_interpolate_bezier_at(bezier, t0, &c_bezier[*n_c_bezier][0],
                                      &c_bezier[*n_c_bezier][1]);
    gimp_coords_interpolate_bezier_at(bezier, t1, &c_bezier[*n_c_bezier][3],
                                      &c_bezier[*n_c_bezier][2]);

    gimp_coords_mix(1.0, &c_bezier[*n_c_bezier][0], (t1 - t0) / 3.0,
                    &c_bezier[*n_c_bezier][1], &c_bezier[*n_c_bezier][1]);
    gimp_coords_mix(1.0, &c_bezier[*n_c_bezier][3], (t0 - t1) / 3.0,
                    &c_bezier[*n_c_bezier][2], &c_bezier[*n_c_bezier][2]);
 ...
 */
static void transform_bezier_coords(const GimpMatrix3 *matrix,
                                    const GimpCoords bezier[4],
                                    GQueue *t_beziers, gint depth) {
  GimpCoords *t_bezier;
  gint n;
  ...
  if (depth > 0) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (fabs(gimp_vector2_cross_product(&c[i], &b).x) > 0.5) {
        ...
        gimp_coords_interpolate_bezier_at(bezier, 0.5, &mid_position,
                                          &mid_velocity);
        ...
        gimp_coords_mix(0.5, &sub[0], 0.5, &bezier[1], &sub[1]);
        gimp_coords_mix(1.0, &sub[3], -1.0 / 6.0, &mid_velocity, &sub[2]);
        ...
        gimp_coords_mix(1.0, &sub[0], +1.0 / 6.0, &mid_velocity, &sub[1]);
        gimp_coords_mix(0.5, &sub[3], 0.5, &bezier[2], &sub[2]);
   ...
   * camera.
   */
  gimp_transform_polygon_coords(matrix, bezier, 2, FALSE, t_bezier, &n);
  gimp_transform_polygon_coords(matrix, bezier + 2, 2, FALSE, t_bezier + 2, &n);
...
void gimp_transform_bezier_coords(const GimpMatrix3 *matrix,
                                  const GimpCoords bezier[4],
                                  GQueue *t_beziers[2], gint *n_t_beziers,
                                  gboolean *start_in, gboolean *end_in) {
  ...
  if (gimp_matrix3_is_affine(matrix)) {
    ...
    *n_t_beziers = 1;
    ...
    for (i = 0; i < 4; i++) {
      ...
      gimp_matrix3_transform_point(matrix, bezier[i].x, bezier[i].y,
  ...
  clip_bezier(bezier, matrix->coeff[2][0], matrix->coeff[2][1],
              matrix->coeff[2][2] - GIMP_TRANSFORM_NEAR_Z, c_bezier,
              n_t_beziers, start_in, end_in);
