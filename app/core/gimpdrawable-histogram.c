...
static GimpAsync *gimp_drawable_calculate_histogram_internal(
    GimpDrawable *drawable, GimpHistogram *histogram, gboolean with_filters,
    gboolean run_async);
...
static GimpAsync *gimp_drawable_calculate_histogram_internal(
  ...
  if (FALSE) {
    ...
    if (with_filters) {
      ...
      source = gimp_gegl_add_buffer_source(
          node, gimp_drawable_get_buffer(drawable), 0, 0);
    }

    histogram_sink = gegl_node_new_child(
        node, "operation", "gimp:histogram-sink", "histogram", histogram, NULL);

    gegl_node_connect_to(source, "output", histogram_sink, "input");
    ...
    if (!gimp_channel_is_empty(mask)) {
      ...
      mask_source = gimp_gegl_add_buffer_source(
          node, gimp_drawable_get_buffer(GIMP_DRAWABLE(mask)), -off_x, -off_y);

      gegl_node_connect_to(mask_source, "output", histogram_sink, "aux");
    }
    ...
    if (!gimp_channel_is_empty(mask)) {
      ...
      if (run_async) {
        async = gimp_histogram_calculate_async(
            histogram, buffer, GEGL_RECTANGLE(x, y, width, height),
      ...
      if (run_async) {
        async = gimp_histogram_calculate_async(
            histogram, buffer, GEGL_RECTANGLE(x, y, width, height), NULL, NULL);
      } else {
        gimp_histogram_calculate(
            histogram, buffer, GEGL_RECTANGLE(x, y, width, height), NULL, NULL);
      }
...
void gimp_drawable_calculate_histogram(GimpDrawable *drawable,
  ...
  gimp_drawable_calculate_histogram_internal(drawable, histogram, with_filters,
                                             FALSE);
}
...
GimpAsync *gimp_drawable_calculate_histogram_async(GimpDrawable *drawable,
  ...
  return gimp_drawable_calculate_histogram_internal(drawable, histogram,
                                                    with_filters, TRUE);
}
