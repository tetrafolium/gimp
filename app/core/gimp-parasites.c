...
static void list_func(const gchar *key, GimpParasite *parasite,
                      gchar ***current) {
...
void gimp_parasiterc_load(Gimp *gimp) {
  ...
  if (!gimp_config_deserialize_file(GIMP_CONFIG(gimp->parasites), file, NULL,
                                    &error)) {
...
void gimp_parasiterc_save(Gimp *gimp) {
  ...
  const gchar *footer = "end of parasiterc";
  ...
  if (!gimp_config_serialize_to_file(GIMP_CONFIG(gimp->parasites), file, header,
                                     footer, NULL, &error)) {
    gimp_message_literal(gimp, NULL, GIMP_MESSAGE_ERROR, error->message);
    g_error_free(error);
  }
