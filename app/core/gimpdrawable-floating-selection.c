...
static void gimp_drawable_fs_notify(GObject *object, const GParamSpec *pspec,
...
static void gimp_drawable_fs_bounding_box_changed(GimpLayer *fs,
                                                  GimpDrawable *drawable);
static void gimp_drawable_fs_update(GimpLayer *fs, gint x, gint y, gint width,
                                    gint height, GimpDrawable *drawable);
...
void gimp_drawable_attach_floating_sel(GimpDrawable *drawable, GimpLayer *fs) {
  GimpImage *image;
  ...
  g_signal_connect(fs, "visibility-changed",
                   G_CALLBACK(gimp_drawable_fs_visibility_changed), drawable);
  ...
  g_signal_connect(fs, "bounding-box-changed",
                   G_CALLBACK(gimp_drawable_fs_bounding_box_changed), drawable);
  g_signal_connect(fs, "update", G_CALLBACK(gimp_drawable_fs_update), drawable);

  gimp_drawable_fs_update(fs, 0, 0, gimp_item_get_width(GIMP_ITEM(fs)),
                          gimp_item_get_height(GIMP_ITEM(fs)), drawable);
}
...
void gimp_drawable_detach_floating_sel(GimpDrawable *drawable) {
  ...
  g_signal_handlers_disconnect_by_func(fs, gimp_drawable_fs_visibility_changed,
                                       drawable);
  g_signal_handlers_disconnect_by_func(
      fs, gimp_drawable_fs_excludes_backdrop_changed, drawable);
  g_signal_handlers_disconnect_by_func(
      fs, gimp_drawable_fs_bounding_box_changed, drawable);
  g_signal_handlers_disconnect_by_func(fs, gimp_drawable_fs_update, drawable);
...
private
  ->fs_filter = gimp_filter_new(_("Floating Selection"));
  ...
  if (fs->layer_offset_node) {
    ...
    gegl_node_remove_child(gimp_filter_get_node(GIMP_FILTER(fs)), fs_source);
  }

  gegl_node_add_child(node, fs_source);

private
  ->fs_applicator = gimp_applicator_new(node);
...
private
  ->fs_crop_node = gegl_node_new_child(node, "operation", "gegl:nop", NULL);

  gegl_node_connect_to(fs_source, "output", private->fs_crop_node, "input");
  gegl_node_connect_to(private->fs_crop_node, "output", node, "aux");
  ...
  g_signal_connect(drawable, "notify::offset-x",
                   G_CALLBACK(gimp_drawable_fs_notify), drawable);
  ...
  g_signal_connect(drawable, "format-changed",
                   G_CALLBACK(gimp_drawable_fs_format_changed), drawable);
  g_signal_connect(image, "component-active-changed",
                   G_CALLBACK(gimp_drawable_fs_affect_changed), drawable);
  g_signal_connect(image, "mask-changed",
                   G_CALLBACK(gimp_drawable_fs_mask_changed), drawable);
...
static void gimp_drawable_remove_fs_filter(GimpDrawable *drawable) {
  ...
  if (private->fs_filter) {
    ...
    g_signal_handlers_disconnect_by_func(fs, gimp_drawable_fs_notify, drawable);
    g_signal_handlers_disconnect_by_func(drawable, gimp_drawable_fs_notify,
                                         drawable);
    g_signal_handlers_disconnect_by_func(
        drawable, gimp_drawable_fs_lock_position_changed, drawable);
    g_signal_handlers_disconnect_by_func(
        drawable, gimp_drawable_fs_format_changed, drawable);
    g_signal_handlers_disconnect_by_func(image, gimp_drawable_fs_affect_changed,
                                         drawable);
    g_signal_handlers_disconnect_by_func(image, gimp_drawable_fs_mask_changed,
                                         drawable);
    ...
    if (fs->layer_offset_node) {
      gegl_node_add_child(gimp_filter_get_node(GIMP_FILTER(fs)), fs_source);
      gegl_node_connect_to(fs_source, "output", fs->layer_offset_node, "input");
    }
...
static void gimp_drawable_sync_fs_filter(GimpDrawable *drawable) {
  ...
  if (gimp_item_get_clip(GIMP_ITEM(drawable), GIMP_TRANSFORM_RESIZE_ADJUST) ==
    ...
    gegl_node_set(private->fs_crop_node, "operation", "gegl:crop", "x",
                  (gdouble)(off_x - fs_off_x), "y", (gdouble)(off_y - fs_off_y),
                  "width", (gdouble)gimp_item_get_width(GIMP_ITEM(drawable)),
                  "height", (gdouble)gimp_item_get_height(GIMP_ITEM(drawable)),
                  NULL);
  } else {
    gegl_node_set(private->fs_crop_node, "operation", "gegl:nop", NULL);
  }

  gimp_applicator_set_apply_offset(private->fs_applicator, fs_off_x - off_x,
                                   fs_off_y - off_y);
  ...
  if (gimp_channel_is_empty(mask)) {
    ...
    gimp_applicator_set_mask_offset(private->fs_applicator, -off_x, -off_y);
  }

  gimp_applicator_set_opacity(private->fs_applicator,
                              gimp_layer_get_opacity(fs));
  gimp_applicator_set_mode(private->fs_applicator, gimp_layer_get_mode(fs),
...
static void gimp_drawable_fs_notify(GObject *object, const GParamSpec *pspec,
                                    GimpDrawable *drawable) {
  if (!strcmp(pspec->name, "offset-x") || !strcmp(pspec->name, "offset-y") ||
      !strcmp(pspec->name, "visible") || !strcmp(pspec->name, "mode") ||
...
static void gimp_drawable_fs_update(GimpLayer *fs, gint x, gint y, gint width,
  ...
  if (gegl_rectangle_intersect(&rect, &rect, &bounding_box)) {
    gimp_drawable_update(drawable, rect.x - off_x, rect.y - off_y, rect.width,
                         rect.height);
  }
}
