...
void gimp_image_set_quick_mask_state(GimpImage *image, gboolean active) {
...
private
  ->quick_mask_state =
      (active ? TRUE | (channel_was_active ? CHANNEL_WAS_ACTIVE : 0) : FALSE);
  ...
  if (active) {
    if (!mask) {
      ...
      gimp_item_rename(GIMP_ITEM(mask), GIMP_IMAGE_QUICK_MASK_NAME, NULL);
    ...
    if (mask) {
      ...
      if (floating_sel && gimp_layer_get_floating_sel_drawable(floating_sel) ==
                              GIMP_DRAWABLE(mask))
        floating_sel_anchor(floating_sel);

      gimp_item_to_selection(GIMP_ITEM(mask), GIMP_CHANNEL_OP_REPLACE, TRUE,
                             FALSE, 0.0, 0.0);
...
void gimp_image_set_quick_mask_color(GimpImage *image, const GimpRGB *color) {
...
void gimp_image_get_quick_mask_color(GimpImage *image, GimpRGB *color) {
  g_return_if_fail(GIMP_IS_IMAGE(image));
  g_return_if_fail(color != NULL);
...
private
  ->quick_mask_inverted = !private->quick_mask_inverted;
}
