...
enum { PROP_0, PROP_UNIQUE_NAMES, PROP_SORT_FUNC, PROP_APPEND };

static void gimp_list_finalize(GObject *object);
static void gimp_list_set_property(GObject *object, guint property_id,
                                   const GValue *value, GParamSpec *pspec);
static void gimp_list_get_property(GObject *object, guint property_id,
                                   GValue *value, GParamSpec *pspec);

static gint64 gimp_list_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_list_add(GimpContainer *container, GimpObject *object);
static void gimp_list_remove(GimpContainer *container, GimpObject *object);
static void gimp_list_reorder(GimpContainer *container, GimpObject *object,
                              gint new_index);
static void gimp_list_clear(GimpContainer *container);
static gboolean gimp_list_have(GimpContainer *container, GimpObject *object);
static void gimp_list_foreach(GimpContainer *container, GFunc func,
                              gpointer user_data);
...
static void gimp_list_uniquefy_name(GimpList *gimp_list, GimpObject *object);
static void gimp_list_object_renamed(GimpObject *object, GimpList *list);
...
static void gimp_list_class_init(GimpListClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_UNIQUE_NAMES,
      g_param_spec_boolean("unique-names", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_SORT_FUNC,
      g_param_spec_pointer("sort-func", NULL, NULL,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_APPEND,
      g_param_spec_boolean("append", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}
...
static gint64 gimp_list_get_memsize(GimpObject *object, gint64 *gui_size) {
  ...
  if (gimp_container_get_policy(GIMP_CONTAINER(list)) ==
    ...
    memsize += gimp_g_queue_get_memsize_foreach(
        list->queue, (GimpMemsizeFunc)gimp_object_get_memsize, gui_size);
  } else {
...
static gint gimp_list_sort_func(gconstpointer a, gconstpointer b,
                                gpointer user_data) {
...
static void gimp_list_add(GimpContainer *container, GimpObject *object) {
  ...
  if (list->unique_names || list->sort_func)
    g_signal_connect(object, "name-changed",
                     G_CALLBACK(gimp_list_object_renamed), list);
...
static void gimp_list_remove(GimpContainer *container, GimpObject *object) {
  ...
  if (list->unique_names || list->sort_func)
    g_signal_handlers_disconnect_by_func(object, gimp_list_object_renamed,
                                         list);
...
GimpContainer *gimp_list_new(GType children_type, gboolean unique_names) {
  ...
  list = g_object_new(GIMP_TYPE_LIST, "children-type", children_type, "policy",
                      GIMP_CONTAINER_POLICY_STRONG, "unique-names",
                      unique_names ? TRUE : FALSE, NULL);
...
GimpContainer *gimp_list_new_weak(GType children_type, gboolean unique_names) {
  ...
  list = g_object_new(GIMP_TYPE_LIST, "children-type", children_type, "policy",
                      GIMP_CONTAINER_POLICY_WEAK, "unique-names",
                      unique_names ? TRUE : FALSE, NULL);
...
static void gimp_list_uniquefy_name(GimpList *gimp_list, GimpObject *object) {
  ...
  for (list = gimp_list->queue->head; list; list = g_list_next(list)) {
    ...
    if (object != object2 && name2 && !strcmp(name, name2))
      break;
  }
  ...
  if (list) {
    ...
    do {
      ...
      for (list = gimp_list->queue->head; list; list = g_list_next(list)) {
        ...
        if (object != object2 && name2 && !strcmp(new_name, name2))
          break;
      }
    } while (list);
...
static void gimp_list_object_renamed(GimpObject *object, GimpList *list) {
  if (list->unique_names) {
    g_signal_handlers_block_by_func(object, gimp_list_object_renamed, list);

    gimp_list_uniquefy_name(list, object);

    g_signal_handlers_unblock_by_func(object, gimp_list_object_renamed, list);
  }
