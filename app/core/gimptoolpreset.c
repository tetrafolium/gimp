...
static void gimp_tool_preset_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_tool_preset_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);
static void gimp_tool_preset_dispatch_properties_changed(GObject *object,
                                                         guint n_pspecs,
                                                         GParamSpec **pspecs);
...
static gboolean
gimp_tool_preset_deserialize_property(GimpConfig *config, guint property_id,
                                      GValue *value, GParamSpec *pspec,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpToolPreset, gimp_tool_preset, GIMP_TYPE_DATA,
    G_IMPLEMENT_INTERFACE(GIMP_TYPE_CONFIG, gimp_tool_preset_config_iface_init))

#define parent_class gimp_tool_preset_parent_class

static void gimp_tool_preset_class_init(GimpToolPresetClass *klass) {
  ...
  object_class->dispatch_properties_changed =
      gimp_tool_preset_dispatch_properties_changed;

  data_class->save = gimp_tool_preset_save;
  data_class->get_extension = gimp_tool_preset_get_extension;

  GIMP_CONFIG_PROP_STRING(object_class, PROP_NAME, "name", NULL, NULL,
                          "Unnamed", GIMP_PARAM_STATIC_STRINGS);
  ...
  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_TOOL_OPTIONS, "tool-options", NULL,
                          NULL, GIMP_TYPE_TOOL_OPTIONS,
                          GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_FG_BG, "use-fg-bg",
                           _("Apply stored FG/BG"), NULL, DEFAULT_USE_FG_BG,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_USE_OPACITY_PAINT_MODE, "use-opacity-paint-mode",
      _("Apply stored opacity/paint mode"), NULL,
      DEFAULT_USE_OPACITY_PAINT_MODE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_BRUSH, "use-brush",
                           _("Apply stored brush"), NULL, DEFAULT_USE_BRUSH,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_DYNAMICS, "use-dynamics",
                           _("Apply stored dynamics"), NULL,
                           DEFAULT_USE_DYNAMICS, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_MYBRUSH, "use-mypaint-brush",
                           _("Apply stored MyPaint brush"), NULL,
                           DEFAULT_USE_MYBRUSH, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_PATTERN, "use-pattern",
                           _("Apply stored pattern"), NULL, DEFAULT_USE_PATTERN,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_PALETTE, "use-palette",
                           _("Apply stored palette"), NULL, DEFAULT_USE_PALETTE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_GRADIENT, "use-gradient",
                           _("Apply stored gradient"), NULL,
                           DEFAULT_USE_GRADIENT, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_FONT, "use-font",
                           _("Apply stored font"), NULL, DEFAULT_USE_FONT,
                           GIMP_PARAM_STATIC_STRINGS);
}
...
static void gimp_tool_preset_init(GimpToolPreset *tool_preset) {}
...
gimp_tool_preset_deserialize_property(GimpConfig *config, guint property_id,
  ...
  case PROP_TOOL_OPTIONS: {
    ...
    if (!type) {
      g_scanner_error(scanner, "unable to determine type of '%s'", type_name);
      *expected = G_TOKEN_NONE;
      g_free(type_name);
      break;
    }
    ...
    if (!g_type_is_a(type, GIMP_TYPE_TOOL_OPTIONS)) {
      g_scanner_error(scanner, "'%s' is not a subclass of GimpToolOptions",
                      type_name);
      *expected = G_TOKEN_NONE;
      g_free(type_name);
      break;
    ...
    options = g_object_new(type, "gimp", tool_preset->gimp, NULL);
    ...
    gimp_context_copy_properties(
        gimp_get_user_context(tool_preset->gimp), GIMP_CONTEXT(options),
        GIMP_CONTEXT_PROP_MASK_BRUSH | GIMP_CONTEXT_PROP_MASK_DYNAMICS |
            GIMP_CONTEXT_PROP_MASK_MYBRUSH | GIMP_CONTEXT_PROP_MASK_PATTERN |
            GIMP_CONTEXT_PROP_MASK_GRADIENT | GIMP_CONTEXT_PROP_MASK_PALETTE |
    ...
    if (gimp_context_get_tool(GIMP_CONTEXT(options))) {
      g_object_set(options, "tool-info",
                   gimp_context_get_tool(GIMP_CONTEXT(options)), NULL);
    } else if (GIMP_TOOL_OPTIONS(options)->tool_info) {
      g_object_set(options, "tool", GIMP_TOOL_OPTIONS(options)->tool_info,
                   NULL);
    } else {
      /* if we have none, the options set_property() logic will
       * replace the NULL with its best guess
       */
      g_object_set(options, "tool", NULL, "tool-info", NULL, NULL);
    }
    ...
    gimp_context_set_serialize_properties(
        GIMP_CONTEXT(options), serialize_props | GIMP_CONTEXT_PROP_MASK_TOOL);
...
static void gimp_tool_preset_set_options(GimpToolPreset *preset,
  ...
  if (preset->tool_options) {
    g_signal_handlers_disconnect_by_func(
        preset->tool_options, gimp_tool_preset_options_notify, preset);

    g_signal_handlers_disconnect_by_func(
        preset->tool_options, gimp_tool_preset_options_prop_name_changed,
        preset);
  ...
  if (options) {
    ...
    serialize_props = gimp_context_get_serialize_properties(
    ...
    g_signal_connect(preset->tool_options, "notify",
                     G_CALLBACK(gimp_tool_preset_options_notify), preset);
...
GimpData *gimp_tool_preset_new(GimpContext *context, const gchar *unused) {
  ...
  return g_object_new(GIMP_TYPE_TOOL_PRESET, "name", tool_info->label,
                      "icon-name", icon_name, "gimp", context->gimp,
