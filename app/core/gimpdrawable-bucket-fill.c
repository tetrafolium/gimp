...
void gimp_drawable_bucket_fill(GimpDrawable *drawable, GimpFillOptions *options,
                               gboolean fill_transparent,
                               GimpSelectCriterion fill_criterion,
                               gdouble threshold, gboolean sample_merged,
                               gboolean diagonal_neighbors, gdouble seed_x,
                               gdouble seed_y) {
  GimpImage *image;
  GeglBuffer *buffer;
  gdouble mask_x;
  ...
  buffer = gimp_drawable_get_bucket_fill_buffer(
      drawable, options, fill_transparent, fill_criterion, threshold, FALSE,
      sample_merged, diagonal_neighbors, seed_x, seed_y, NULL, &mask_x, &mask_y,
      &width, &height);

  if (buffer) {
    /*  Apply it to the image  */
    gimp_drawable_apply_buffer(
        drawable, buffer, GEGL_RECTANGLE(0, 0, width, height), TRUE,
        C_("undo-type", "Bucket Fill"),
 ...
 */
GeglBuffer *gimp_drawable_get_bucket_fill_buffer(
    GimpDrawable *drawable, GimpFillOptions *options, gboolean fill_transparent,
    GimpSelectCriterion fill_criterion, gdouble threshold, gboolean show_all,
    gboolean sample_merged, gboolean diagonal_neighbors, gdouble seed_x,
    gdouble seed_y, GeglBuffer **mask_buffer, gdouble *mask_x, gdouble *mask_y,
  ...
  if (!gimp_item_mask_intersect(GIMP_ITEM(drawable), &sel_x, &sel_y, &sel_width,
                                &sel_height))
    return NULL;
  ...
  if (mask_buffer && *mask_buffer && threshold == 0.0) {
    ...
    gegl_buffer_sample(*mask_buffer, seed_x, seed_y, NULL, &pixel,
                       babl_format("Y float"), GEGL_SAMPLER_NEAREST,
                       GEGL_ABYSS_NONE);
   ...
   *  contiguous region.
   */
  new_mask = gimp_pickable_contiguous_region_by_seed(
      pickable, antialias, threshold, fill_transparent, fill_criterion,
      diagonal_neighbors, (gint)seed_x, (gint)seed_y);
  if (mask_buffer && *mask_buffer) {
    gimp_gegl_mask_combine_buffer(new_mask, *mask_buffer, GIMP_CHANNEL_OP_ADD,
                                  0, 0);
  ...
  if (!gimp_channel_is_empty(gimp_image_get_mask(image))) {
    ...
    if (!gimp_rectangle_intersect(x, y, width, height,
                                  ...
                                  sel_x + off_x, sel_y + off_y, sel_width,
                                  sel_height,
  ...
  if (sample_merged) {
    ...
    gimp_rectangle_intersect(x, y, width, height,
                             ...
                             off_x, off_y, gimp_item_get_width(item),
  ...
  buffer = gimp_fill_options_create_buffer(
      options, drawable, GEGL_RECTANGLE(0, 0, width, height), -x, -y);

  gimp_gegl_apply_opacity(buffer, NULL, NULL, buffer, new_mask, -mask_offset_x,
                          -mask_offset_y, 1.0);
 ...
 *          used in a drawable filter as preview.
 */
GeglBuffer *gimp_drawable_get_line_art_fill_buffer(
    GimpDrawable *drawable, GimpLineArt *line_art, GimpFillOptions *options,
    gboolean sample_merged, gdouble seed_x, gdouble seed_y,
    GeglBuffer **mask_buffer, gdouble *mask_x, gdouble *mask_y,
    gint *mask_width, gint *mask_height) {
  GimpImage *image;
  GeglBuffer *buffer;
  GeglBuffer *new_mask;
   ...
   *  contiguous region.
   */
  new_mask = gimp_pickable_contiguous_region_by_line_art(
      NULL, line_art, (gint)seed_x, (gint)seed_y);
  if (mask_buffer && *mask_buffer) {
    gimp_gegl_mask_combine_buffer(new_mask, *mask_buffer, GIMP_CHANNEL_OP_ADD,
                                  0, 0);
    g_object_unref(*mask_buffer);
  ...
  if (gimp_fill_options_get_feather(options, &feather_radius)) {
     ...
     */
    gimp_gegl_apply_feather(buffer, NULL, NULL, buffer, NULL, feather_radius,
                            feather_radius, TRUE);
  }
