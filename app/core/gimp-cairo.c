...
cairo_pattern_t *gimp_cairo_pattern_create_stipple(const GimpRGB *fg,
  ...
  surface = cairo_image_surface_create_for_data(data, CAIRO_FORMAT_ARGB32, 8, 8,
                                                8 * 4);
  cairo_surface_set_user_data(surface, &surface_data_key, data,
  ...
  if (offset_x != 0.0 || offset_y != 0.0) {
    ...
    cairo_matrix_init_translate(&matrix, fmod(offset_x, 8), fmod(offset_y, 8));
...
void gimp_cairo_arc(cairo_t *cr, gdouble center_x, gdouble center_y,
                    gdouble radius, gdouble start_angle, gdouble slice_angle) {
  g_return_if_fail(cr != NULL);

  if (slice_angle >= 0) {
    cairo_arc_negative(cr, center_x, center_y, radius, -start_angle,
                       -start_angle - slice_angle);
  } else {
    cairo_arc(cr, center_x, center_y, radius, -start_angle,
              -start_angle - slice_angle);
  }
}

void gimp_cairo_rounded_rectangle(cairo_t *cr, gdouble x, gdouble y,
  ...
  cairo_arc(cr, x + corner_radius, y + corner_radius, corner_radius, 0.50 * REV,
            0.75 * REV);
  ...
  cairo_arc(cr, x + width - corner_radius, y + height - corner_radius,
            corner_radius, 0.00 * REV, 0.25 * REV);
...
void gimp_cairo_segments(cairo_t *cr, GimpSegment *segs, gint n_segs) {
  gint i;
