...
static void gimp_pickable_default_init(GimpPickableInterface *iface) {
  iface->get_pixel_average = gimp_pickable_real_get_pixel_average;

  g_object_interface_install_property(
      iface, g_param_spec_object("buffer", NULL, NULL, GEGL_TYPE_BUFFER,
                                 GIMP_PARAM_READABLE));
}
...
static void gimp_pickable_real_get_pixel_average(GimpPickable *pickable,
  ...
  average_format =
      babl_format_with_space("RaGaBaA double", babl_format_get_space(format));

  for (y = rect->y; y < rect->y + rect->height; y++) {
    for (x = rect->x; x < rect->x + rect->width; x++) {
      gdouble sample[4];

      if (gimp_pickable_get_pixel_at(pickable, x, y, average_format, sample)) {
        for (c = 0; c < 4; c++)
          average[c] += sample[c];
...
gboolean gimp_pickable_get_pixel_at(GimpPickable *pickable, gint x, gint y,
...
gboolean gimp_pickable_get_color_at(GimpPickable *pickable, gint x, gint y,
                                    GimpRGB *color) {
  gdouble pixel[4];
...
gdouble gimp_pickable_get_opacity_at(GimpPickable *pickable, gint x, gint y) {
  GimpPickableInterface *pickable_iface;
...
void gimp_pickable_pixel_to_srgb(GimpPickable *pickable, const Babl *format,
...
void gimp_pickable_srgb_to_pixel(GimpPickable *pickable, const GimpRGB *color,
                                 const Babl *format, gpointer pixel) {
  GimpPickableInterface *pickable_iface;
...
void gimp_pickable_srgb_to_image_color(GimpPickable *pickable,
  ...
  gimp_pickable_srgb_to_pixel(pickable, color, babl_format("R'G'B'A double"),
                              image_color);
}

gboolean gimp_pickable_pick_color(GimpPickable *pickable, gint x, gint y,
                                  gboolean sample_average,
                                  gdouble average_radius, gpointer pixel,
                                  GimpRGB *color) {
  const Babl *format;
  gdouble sample[4];
  ...
  if (sample_average) {
    ...
    gimp_pickable_get_pixel_average(
        pickable,
        GEGL_RECTANGLE(x - radius, y - radius, 2 * radius + 1, 2 * radius + 1),
        format, sample);
  }
