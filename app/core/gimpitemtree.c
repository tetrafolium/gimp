...
#define GIMP_ITEM_TREE_GET_PRIVATE(object)                                     \
  ((GimpItemTreePrivate *)gimp_item_tree_get_instance_private(                 \
      (GimpItemTree *)(object)))
...
static void gimp_item_tree_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_item_tree_get_property(GObject *object, guint property_id,
                                        GValue *value, GParamSpec *pspec);

static gint64 gimp_item_tree_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_item_tree_uniquefy_name(GimpItemTree *tree, GimpItem *item,
...
static void gimp_item_tree_class_init(GimpItemTreeClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_CONTAINER_TYPE,
      g_param_spec_gtype("container-type", NULL, NULL, GIMP_TYPE_ITEM_STACK,
                         GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_ITEM_TYPE,
      g_param_spec_gtype("item-type", NULL, NULL, GIMP_TYPE_ITEM,
                         GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(object_class, PROP_ACTIVE_ITEM,
                                  g_param_spec_object("active-item", NULL, NULL,
                                                      GIMP_TYPE_ITEM,
                                                      GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_SELECTED_ITEMS,
      g_param_spec_pointer("selected-items", NULL, NULL, GIMP_PARAM_READWRITE));
}

static void gimp_item_tree_init(GimpItemTree *tree) {
...
static void gimp_item_tree_constructed(GObject *object) {
  ...
  tree->container = g_object_new(private->container_type, "name",
                                 g_type_name(private->item_type),
                                 "children-type", private->item_type, "policy",
                                 GIMP_CONTAINER_POLICY_STRONG, NULL);
}
...
static void gimp_item_tree_dispose(GObject *object) {
  ...
  gimp_container_foreach(tree->container, (GFunc)gimp_item_removed, NULL);
...
static void gimp_item_tree_set_property(GObject *object, guint property_id,
  ...
  case PROP_IMAGE:
  private
    ->image = g_value_get_object(value); /* don't ref */
    break;
  case PROP_CONTAINER_TYPE:
  private
    ->container_type = g_value_get_gtype(value);
    break;
  case PROP_ITEM_TYPE:
  private
    ->item_type = g_value_get_gtype(value);
    break;
  case PROP_ACTIVE_ITEM:
    /* Don't ref the item. */
  private
    ->selected_items = g_list_prepend(NULL, g_value_get_object(value));
    break;
  case PROP_SELECTED_ITEMS:
    /* Don't ref the item. */
  private
    ->selected_items = g_value_get_pointer(value);
    break;
...
static void gimp_item_tree_get_property(GObject *object, guint property_id,
  ...
  case PROP_ACTIVE_ITEM:
    g_value_set_object(value,
                       gimp_item_tree_get_active_item(GIMP_ITEM_TREE(object)));
    break;
  case PROP_SELECTED_ITEMS:
...
GimpItemTree *gimp_item_tree_new(GimpImage *image, GType container_type,
                                 GType item_type) {
  ...
  return g_object_new(GIMP_TYPE_ITEM_TREE, "image", image, "container-type",
                      container_type, "item-type", item_type, NULL);
}
...
void gimp_item_tree_set_active_item(GimpItemTree *tree, GimpItem *item) {
  gimp_item_tree_set_selected_items(tree, g_list_prepend(NULL, item));
}
 ...
 */
void gimp_item_tree_set_selected_items(GimpItemTree *tree, GList *items) {
...
private
  = GIMP_ITEM_TREE_GET_PRIVATE(tree);

  for (iter = items; iter; iter = iter->next) {
    g_return_if_fail(
        G_TYPE_CHECK_INSTANCE_TYPE(iter->data, private->item_type));
...
GimpItem *gimp_item_tree_get_item_by_name(GimpItemTree *tree,
  ...
  return g_hash_table_lookup(GIMP_ITEM_TREE_GET_PRIVATE(tree)->name_hash, name);
}

gboolean gimp_item_tree_get_insert_pos(GimpItemTree *tree, GimpItem *item,
...
private
  ...
  if (*position == -1) {
    ...
    for (iter = private->selected_items; iter; iter = iter->next) {
      selected_position = gimp_container_get_child_index(container, iter->data);
...
void gimp_item_tree_add_item(GimpItemTree *tree, GimpItem *item,
                             GimpItem *parent, gint position) {
  GimpItemTreePrivate *private;
  GimpContainer *container;
...
private
  ...
  if (parent)
    gimp_viewable_set_parent(GIMP_VIEWABLE(item), GIMP_VIEWABLE(parent));
...
GList *gimp_item_tree_remove_item(GimpItemTree *tree, GimpItem *item,
                                  GList *new_selected) {
  GimpItemTreePrivate *private;
  GimpItem *parent;
  GimpContainer *container;
...
private
  ...
  g_hash_table_remove(private->name_hash, gimp_object_get_name(item));
  ...
  if (children) {
    ...
    while (list) {
      g_hash_table_remove(private->name_hash, gimp_object_get_name(list->data));
  ...
  if (!new_selected) {
    ...
    if (n_children > 0) {
      ...
      selected = GIMP_ITEM(gimp_container_get_child_by_index(container, index));
    } else if (parent) {
      selected = parent;
    }
...
gboolean gimp_item_tree_reorder_item(GimpItemTree *tree, GimpItem *item,
...
private
  ...
  g_return_val_if_fail(
      new_parent == NULL ||
          G_TYPE_CHECK_INSTANCE_TYPE(new_parent, private->item_type),
      FALSE);
  g_return_val_if_fail(
      new_parent == NULL || gimp_item_get_tree(new_parent) == tree, FALSE);
  g_return_val_if_fail(new_parent == NULL || gimp_viewable_get_children(
  ...
  g_return_val_if_fail(new_parent == NULL ||
                           !gimp_viewable_is_ancestor(
                               GIMP_VIEWABLE(item), GIMP_VIEWABLE(new_parent)),
                       FALSE);
  ...
  if (new_container != container || new_index != gimp_item_get_index(item)) {
    ...
    if (new_container != container) {
      ...
      gimp_viewable_set_parent(GIMP_VIEWABLE(item), GIMP_VIEWABLE(new_parent));
...
void gimp_item_tree_rename_item(GimpItemTree *tree, GimpItem *item,
                                const gchar *new_name, gboolean push_undo,
                                const gchar *undo_desc) {
  GimpItemTreePrivate *private;
...
private
  ...
  if (strcmp(new_name, gimp_object_get_name(item))) {
    if (push_undo)
      gimp_image_undo_push_item_rename(gimp_item_get_image(item), undo_desc,
                                       item);
...
static void gimp_item_tree_uniquefy_name(GimpItemTree *tree, GimpItem *item,
  ...
  if (g_hash_table_lookup(private->name_hash, gimp_object_get_name(item))) {
    ...
    do {
      ...
      new_name = g_strdup_printf("%s #%.*d", name, precision, number);
  ...
  g_hash_table_insert(private->name_hash, (gpointer)gimp_object_get_name(item),
                      item);
}
