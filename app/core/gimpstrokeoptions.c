...
enum { DASH_INFO_CHANGED, LAST_SIGNAL };
...
#define GET_PRIVATE(options)                                                   \
  ((GimpStrokeOptionsPrivate *)gimp_stroke_options_get_instance_private(       \
...
static void gimp_stroke_options_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_stroke_options_get_property(GObject *object, guint property_id,
                                             GValue *value, GParamSpec *pspec);

static GimpConfig *gimp_stroke_options_duplicate(GimpConfig *config);

G_DEFINE_TYPE_WITH_CODE(
    GimpStrokeOptions, gimp_stroke_options, GIMP_TYPE_FILL_OPTIONS,
...
static void gimp_stroke_options_class_init(GimpStrokeOptionsClass *klass) {
  ...
  stroke_options_signals[DASH_INFO_CHANGED] = g_signal_new(
      "dash-info-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpStrokeOptionsClass, dash_info_changed), NULL, NULL,
      NULL, G_TYPE_NONE, 1, GIMP_TYPE_DASH_PRESET);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_METHOD, "method", _("Method"), NULL,
                        GIMP_TYPE_STROKE_METHOD, GIMP_STROKE_LINE,
                        GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_WIDTH, "width", _("Line width"),
                          NULL, 0.0, 2000.0, 6.0, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_UNIT(object_class, PROP_UNIT, "unit", _("Unit"), NULL, TRUE,
                        FALSE, GIMP_UNIT_PIXEL, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_CAP_STYLE, "cap-style",
                        _("Cap style"), NULL, GIMP_TYPE_CAP_STYLE,
                        GIMP_CAP_BUTT, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_JOIN_STYLE, "join-style",
                        _("Join style"), NULL, GIMP_TYPE_JOIN_STYLE,
                        GIMP_JOIN_MITER, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_MITER_LIMIT, "miter-limit",
                          _("Miter limit"),
                          ...
                          0.0, 100.0, 10.0, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_DASH_OFFSET, "dash-offset",
                          _("Dash offset"), NULL, 0.0, 2000.0, 0.0,
                          GIMP_PARAM_STATIC_STRINGS);

  array_spec = g_param_spec_double("dash-length", NULL, NULL, 0.0, 2000.0, 1.0,
                                   GIMP_PARAM_READWRITE);
  g_object_class_install_property(
      object_class, PROP_DASH_INFO,
      gimp_param_spec_value_array("dash-info", NULL, NULL, array_spec,
                                  GIMP_PARAM_STATIC_STRINGS |
                                      GIMP_CONFIG_PARAM_FLAGS));

  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_PAINT_OPTIONS, "paint-options",
                          NULL, NULL, GIMP_TYPE_PAINT_OPTIONS,
                          GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_EMULATE_DYNAMICS, "emulate-brush-dynamics",
      _("Emulate brush dynamics"), NULL, FALSE, GIMP_PARAM_STATIC_STRINGS);
}
...
static void gimp_stroke_options_set_property(GObject *object, guint property_id,
  ...
  private
    ->miter_limit = g_value_get_double(value);
    break;
  case PROP_DASH_OFFSET:
  private
    ->dash_offset = g_value_get_double(value);
    break;
  case PROP_DASH_INFO: {
  ...
  private
    ->paint_options = g_value_dup_object(value);
    break;
  case PROP_EMULATE_DYNAMICS:
  private
    ->emulate_dynamics = g_value_get_boolean(value);
    break;
...
GimpStrokeOptions *gimp_stroke_options_new(Gimp *gimp, GimpContext *context,
  ...
  options = g_object_new(GIMP_TYPE_STROKE_OPTIONS, "gimp", gimp, "paint-info",
                         paint_info, NULL);
...
GimpStrokeMethod gimp_stroke_options_get_method(GimpStrokeOptions *options) {
  g_return_val_if_fail(GIMP_IS_STROKE_OPTIONS(options), GIMP_STROKE_LINE);
...
private
  ...
  g_signal_emit(options, stroke_options_signals[DASH_INFO_CHANGED], 0, preset);
}
  ...
  case GIMP_STROKE_PAINT_METHOD: {
    ...
    if (paint_options) {
      ...
      gimp_context_define_properties(GIMP_CONTEXT(paint_options),
                                     GIMP_CONTEXT_PROP_MASK_PAINT, FALSE);
      ...
      gimp_context_copy_properties(context, GIMP_CONTEXT(paint_options),
                                   global_props);
    }
