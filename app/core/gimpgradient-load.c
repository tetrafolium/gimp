...
GList *gimp_gradient_load(GimpContext *context, GFile *file,
  ...
  gradient = g_object_new(GIMP_TYPE_GRADIENT, "mime-type",
                          "application/x-gimp-gradient", NULL);
  ...
  for (i = 0; i < num_segments; i++) {
    ...
    switch (sscanf(end, "%d %d %d %d", &type, &color, &left_color_type,
                   &right_color_type)) {
    case 4:
    ...
    if (seg->left > seg->middle || seg->middle > seg->right ||
        (prev && (prev->right != seg->left)) || (!prev && (0.0 != seg->left))) {
      g_set_error(error, GIMP_DATA_ERROR, GIMP_DATA_ERROR_READ,
...
static GimpGradientSegment *svg_parser_gradient_segments(GList *stops);
...
GList *gimp_gradient_load_svg(GimpContext *context, GFile *file,
                              GInputStream *input, GError **error) {
...
static void svg_parser_start_element(GMarkupParseContext *context,
  ...
  if (!parser->gradient && strcmp(element_name, "linearGradient") == 0) {
    ...
    parser->gradient = g_object_new(GIMP_TYPE_GRADIENT, "name", name,
                                    "mime-type", "image/svg+xml", NULL);
  } else if (parser->gradient && strcmp(element_name, "stop") == 0) {
    SvgStop *stop = svg_parse_gradient_stop(attribute_names, attribute_values);
    ...
    if (parser->stops)
      stop->offset =
          MAX(stop->offset, ((SvgStop *)parser->stops->data)->offset);
...
static void svg_parser_end_element(GMarkupParseContext *context,
  ...
  if (parser->gradient && strcmp(element_name, "linearGradient") == 0) {
    ...
    if (parser->gradient->segments)
      parser->gradients = g_list_prepend(parser->gradients, parser->gradient);
    else
      g_object_unref(parser->gradient);
...
static void svg_parse_gradient_stop_style_prop(SvgStop *stop, const gchar *name,
                                               const gchar *value) {
...
static void svg_parse_gradient_stop_style(SvgStop *stop, const gchar *style) {
  const gchar *end;
  const gchar *sep;
