...
GList *gimp_pattern_load(GimpContext *context, GFile *file, GInputStream *input,
                         GError **error) {
  ...
  if (header.magic_number != GIMP_PATTERN_MAGIC || header.version != 1 ||
    ...
    g_set_error(error, GIMP_DATA_ERROR, GIMP_DATA_ERROR_READ,
                _("Unknown pattern format version %d."), header.version);
    goto error;
  }
  ...
  if ((header.width == 0) || (header.width > GIMP_PATTERN_MAX_SIZE) ||
    ...
    g_set_error(error, GIMP_DATA_ERROR, GIMP_DATA_ERROR_READ,
                _("Invalid header data in '%s': width=%lu, height=%lu, "
                  "bytes=%lu"),
                gimp_file_get_utf8_name(file), (gulong)header.width,
                (gulong)header.height, (gulong)header.bytes);
    goto error;
  }
  ...
  if (!g_input_stream_read_all(input, gimp_temp_buf_get_data(pattern->mask),
                               size, &bytes_read, NULL, error) ||
      bytes_read != size) {
    g_prefix_error(error, _("File appears truncated."));
    goto error;
  }
...
GList *gimp_pattern_load_pixbuf(GimpContext *context, GFile *file,
