...
static void gimp_group_layer_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
                                      GimpUndoAccumulator *accum);
static void gimp_group_layer_undo_free(GimpUndo *undo, GimpUndoMode undo_mode);
...
static void gimp_group_layer_undo_class_init(GimpGroupLayerUndoClass *klass) {
...
static void gimp_group_layer_undo_init(GimpGroupLayerUndo *undo) {}
...
static void gimp_group_layer_undo_constructed(GObject *object) {
  ...
  case GIMP_UNDO_GROUP_LAYER_RESUME_MASK:
    group_layer_undo->mask_buffer = _gimp_group_layer_get_suspended_mask(
        group, &group_layer_undo->mask_bounds);
  ...
  case GIMP_UNDO_GROUP_LAYER_CONVERT:
    group_layer_undo->prev_type =
        gimp_drawable_get_base_type(GIMP_DRAWABLE(group));
    group_layer_undo->prev_precision =
        gimp_drawable_get_precision(GIMP_DRAWABLE(group));
    group_layer_undo->prev_has_alpha =
        gimp_drawable_has_alpha(GIMP_DRAWABLE(group));
    break;
...
static void gimp_group_layer_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
  ...
  case GIMP_UNDO_GROUP_LAYER_RESUME_RESIZE:
    if ((undo_mode == GIMP_UNDO_MODE_UNDO &&
      ...
      if (undo->undo_type == GIMP_UNDO_GROUP_LAYER_RESUME_RESIZE &&
        ...
        gimp_drawable_set_buffer_full(GIMP_DRAWABLE(mask), FALSE, NULL,
                                      group_layer_undo->mask_buffer,
                                      &group_layer_undo->mask_bounds, TRUE);
      }
    }
    break;
  ...
  case GIMP_UNDO_GROUP_LAYER_RESUME_MASK:
    if ((undo_mode == GIMP_UNDO_MODE_UNDO &&
      ...
      if (undo->undo_type == GIMP_UNDO_GROUP_LAYER_RESUME_MASK &&
          group_layer_undo->mask_buffer) {
        _gimp_group_layer_set_suspended_mask(group,
                                             group_layer_undo->mask_buffer,
  ...
  case GIMP_UNDO_GROUP_LAYER_CONVERT: {
    ...
    gimp_drawable_convert_type(
        GIMP_DRAWABLE(group), gimp_item_get_image(GIMP_ITEM(group)),
        group_layer_undo->prev_type, group_layer_undo->prev_precision,
        group_layer_undo->prev_has_alpha, NULL, NULL, 0, 0, FALSE, NULL);
