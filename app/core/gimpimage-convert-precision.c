...
void gimp_image_convert_precision(GimpImage *image, GimpPrecision precision,
  ...
  undo_desc =
      g_strdup_printf(C_("undo-type", "Convert Image to %s"), enum_desc);
  ...
  gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_IMAGE_CONVERT, undo_desc);
  g_free(undo_desc);
  ...
  if (gimp_babl_format_get_trc(old_format) !=
     ...
     */
    if (!new_profile) {
      new_profile = gimp_babl_get_builtin_color_profile(base_type, new_trc);
  ...
  while ((drawable = gimp_object_queue_pop(queue))) {
    if (drawable == GIMP_DRAWABLE(gimp_image_get_mask(image))) {
      ...
      gimp_image_undo_push_mask_precision(image, NULL, GIMP_CHANNEL(drawable));

      buffer = gegl_buffer_new(GEGL_RECTANGLE(0, 0, gimp_image_get_width(image),
      ...
      gimp_drawable_convert_type(
          drawable, image, gimp_drawable_get_base_type(drawable), precision,
          gimp_drawable_has_alpha(drawable), old_profile, new_profile,
          dither_type, mask_dither_type, TRUE, sub_progress);
    }
  }
...
void gimp_image_convert_dither_u8(GimpImage *image, GimpProgress *progress) {
  GeglNode *dither;

  g_return_if_fail(GIMP_IS_IMAGE(image));
  g_return_if_fail(progress == NULL || GIMP_IS_PROGRESS(progress));

  dither = gegl_node_new_child(NULL, "operation", "gegl:noise-rgb", "red",
                               1.0 / 256.0, "green", 1.0 / 256.0, "blue",
                               1.0 / 256.0, "linear", FALSE, "gaussian", FALSE,
                               NULL);
  ...
  if (dither) {
    ...
    while ((drawable = gimp_object_queue_pop(queue))) {
      gimp_drawable_apply_operation(drawable, sub_progress, _("Dithering"),
                                    dither);
    }
