...
GimpLayer *gimp_layer_new(GimpImage *image, gint width, gint height,
                          const Babl *format, const gchar *name,
  ...
  layer = GIMP_LAYER(gimp_drawable_new(GIMP_TYPE_LAYER, image, name, 0, 0,
                                       width, height, format));
 ...
 **/
GimpLayer *gimp_layer_new_from_buffer(GimpBuffer *buffer, GimpImage *dest_image,
                                      const Babl *format, const gchar *name,
                                      gdouble opacity, GimpLayerMode mode) {
  ...
  return gimp_layer_new_from_gegl_buffer(
      gimp_buffer_get_buffer(buffer), dest_image, format, name, opacity, mode,
...
GimpLayer *gimp_layer_new_from_gegl_buffer(GeglBuffer *buffer,
                                           ...
                                           const Babl *format,
                                           const gchar *name, gdouble opacity,
                                           GimpLayerMode mode,
  ...
  g_return_val_if_fail(
      buffer_profile == NULL || GIMP_IS_COLOR_PROFILE(buffer_profile), NULL);
   ...
   */
  layer = gimp_layer_new(dest_image, extent->width, extent->height, format,
                         name, opacity, mode);
 ...
 * Returns: The new layer.
 **/
GimpLayer *gimp_layer_new_from_pixbuf(GdkPixbuf *pixbuf, GimpImage *dest_image,
                                      const Babl *format, const gchar *name,
                                      gdouble opacity, GimpLayerMode mode) {
  GimpLayer *layer;
  GeglBuffer *buffer;
  ...
  layer = gimp_layer_new(dest_image, gdk_pixbuf_get_width(pixbuf),
                         gdk_pixbuf_get_height(pixbuf), format, name, opacity,
                         mode);
