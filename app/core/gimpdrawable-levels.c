...
void gimp_drawable_levels_stretch(GimpDrawable *drawable,
  ...
  gimp_levels_config_stretch(config, histogram, gimp_drawable_is_rgb(drawable));

  g_object_unref(histogram);

  levels = g_object_new(GEGL_TYPE_NODE, "operation", "gimp:levels", NULL);

  gegl_node_set(levels, "config", config, NULL);

  gimp_drawable_apply_operation(drawable, progress, _("Levels"), levels);
