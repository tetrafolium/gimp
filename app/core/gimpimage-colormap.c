...
private
  ...
  palette_name =
      g_strdup_printf(_("Colormap of Image #%d (%s)"), gimp_image_get_id(image),
...
private
  ->palette = GIMP_PALETTE(gimp_palette_new(NULL, palette_name));
...
private
  ...
  format_name =
      g_strdup_printf("-gimp-indexed-format-%d", gimp_image_get_id(image));

  babl_new_palette_with_space(format_name, space, &private->babl_palette_rgb,
  ...
  if (private->colormap && private->n_colors > 0) {
    babl_palette_set_palette(
        ...
        gimp_babl_format(GIMP_RGB, private->precision, FALSE, space),
        private->colormap, private->n_colors);
    babl_palette_set_palette(
        private->babl_palette_rgba,
        gimp_babl_format(GIMP_RGB, private->precision, FALSE, space),
        private->colormap, private->n_colors);
...
void gimp_image_set_colormap(GimpImage *image, const guchar *colormap,
                             gint n_colors, gboolean push_undo) {
  GimpImagePrivate *private;
...
void gimp_image_unset_colormap(GimpImage *image, gboolean push_undo) {
  GimpImagePrivate *private;
...
void gimp_image_get_colormap_entry(GimpImage *image, gint color_index,
                                   GimpRGB *color) {
  GimpImagePrivate *private;
...
private
  ...
  gimp_rgba_set_uchar(color, private->colormap[color_index * 3],
                      private->colormap[color_index * 3 + 1],
                      private->colormap[color_index * 3 + 2], 255);
}

void gimp_image_set_colormap_entry(GimpImage *image, gint color_index,
                                   const GimpRGB *color, gboolean push_undo) {
  GimpImagePrivate *private;
...
private
  ...
  if (push_undo)
    gimp_image_undo_push_image_colormap(
        image, C_("undo-type", "Change Colormap entry"));

  gimp_rgb_get_uchar(color, &private->colormap[color_index * 3],
                     &private->colormap[color_index * 3 + 1],
...
void gimp_image_add_colormap_entry(GimpImage *image, const GimpRGB *color) {
  GimpImagePrivate *private;
...
private
  ...
  gimp_rgb_get_uchar(color, &private->colormap[private->n_colors * 3],
...
static void gimp_image_colormap_set_palette_entry(GimpImage *image,
  ...
  if (c)
    ...
    gimp_rgba_set_uchar(&color, private->colormap[3 * index + 0],
                        private->colormap[3 * index + 1],
                        private->colormap[3 * index + 2], 255);
