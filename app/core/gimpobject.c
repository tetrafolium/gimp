...
enum { DISCONNECT, NAME_CHANGED, LAST_SIGNAL };

enum { PROP_0, PROP_NAME, N_PROPS };
...
static void gimp_object_set_property(GObject *object, guint property_id,
                                     const GValue *value, GParamSpec *pspec);
static void gimp_object_get_property(GObject *object, guint property_id,
...
static GParamSpec *object_props[N_PROPS] = {
    NULL,
};
...
static void gimp_object_class_init(GimpObjectClass *klass) {
  ...
  object_signals[DISCONNECT] =
      g_signal_new("disconnect", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpObjectClass, disconnect), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);

  object_signals[NAME_CHANGED] =
      g_signal_new("name-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpObjectClass, name_changed), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);
  ...
  object_props[PROP_NAME] = g_param_spec_string(
      "name", NULL, NULL, NULL, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT);
...
gint64 gimp_object_get_memsize(GimpObject *object, gint64 *gui_size) {
  ...
  if (gimp_debug_memsize) {
    ...
    memsize = GIMP_OBJECT_GET_CLASS(object)->get_memsize(object, &gui_memsize);
    ...
    object_size = g_strdup_printf(
        "%s%s \"%s\": "
        "%" G_GINT64_FORMAT "(%" G_GINT64_FORMAT ")\n",
        indent_buf, g_type_name(G_TYPE_FROM_INSTANCE(object)),
        object->p->name ? object->p->name : "anonymous", memsize, gui_memsize);
  ...
  my_size = GIMP_OBJECT_GET_CLASS(object)->get_memsize(object, &my_gui_size);
