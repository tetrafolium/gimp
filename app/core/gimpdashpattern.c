...
G_DEFINE_BOXED_TYPE(GimpDashPattern, gimp_dash_pattern, gimp_dash_pattern_copy,
...
GArray *gimp_dash_pattern_new_from_preset(GimpDashPreset preset) {
  ...
  case GIMP_DASH_LONG_DASH:
    dash = 9.0;
    g_array_append_val(pattern, dash);
    dash = 3.0;
    g_array_append_val(pattern, dash);
    break;
...
void gimp_dash_pattern_fill_segments(GArray *pattern, gboolean *segments,
                                     gint n_segments) {
  gdouble factor;
  gdouble sum;
  gint i, j;
...
gdouble *gimp_dash_pattern_to_double_array(GArray *pattern, gint *n_dashes) {
  if (pattern != NULL && pattern->len > 0) {
    gdouble *dashes;
    gint i;
