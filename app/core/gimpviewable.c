...
enum { PROP_0, PROP_ICON_NAME, PROP_ICON_PIXBUF, PROP_FROZEN, N_PROPS };
...
#define GET_PRIVATE(viewable)                                                  \
  ((GimpViewablePrivate *)gimp_viewable_get_instance_private(                  \
...
static void gimp_viewable_set_property(GObject *object, guint property_id,
                                       const GValue *value, GParamSpec *pspec);
static void gimp_viewable_get_property(GObject *object, guint property_id,
...
static gboolean
gimp_viewable_deserialize_property(GimpConfig *config, guint property_id,
                                   GValue *value, GParamSpec *pspec,
                                   GScanner *scanner, GTokenType *expected);

G_DEFINE_TYPE_WITH_CODE(GimpViewable, gimp_viewable, GIMP_TYPE_OBJECT,
                        G_ADD_PRIVATE(GimpViewable) G_IMPLEMENT_INTERFACE(
                            GIMP_TYPE_CONFIG, gimp_viewable_config_iface_init))
...
static void gimp_viewable_class_init(GimpViewableClass *klass) {
  ...
  viewable_signals[INVALIDATE_PREVIEW] = g_signal_new(
      "invalidate-preview", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpViewableClass, invalidate_preview), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  viewable_signals[SIZE_CHANGED] =
      g_signal_new("size-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpViewableClass, size_changed), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);

  viewable_signals[EXPANDED_CHANGED] = g_signal_new(
      "expanded-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpViewableClass, expanded_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  viewable_signals[ANCESTRY_CHANGED] = g_signal_new(
      "ancestry-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpViewableClass, ancestry_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  ...
  obj_props[PROP_ICON_NAME] = g_param_spec_string("icon-name", NULL, NULL, NULL,
                                                  GIMP_CONFIG_PARAM_FLAGS);

  obj_props[PROP_ICON_PIXBUF] = g_param_spec_object(
      "icon-pixbuf", NULL, NULL, GDK_TYPE_PIXBUF, GIMP_CONFIG_PARAM_FLAGS);

  obj_props[PROP_FROZEN] =
      g_param_spec_boolean("frozen", NULL, NULL, FALSE, GIMP_PARAM_READABLE);
...
static void gimp_viewable_set_property(GObject *object, guint property_id,
  ...
  private
    ->icon_pixbuf = g_value_dup_object(value);
...
static gint64 gimp_viewable_get_memsize(GimpObject *object, gint64 *gui_size) {
  ...
  *gui_size += gimp_temp_buf_get_memsize(private->preview_temp_buf);
  ...
  if (private->preview_pixbuf) {
    *gui_size += (gimp_g_object_get_memsize(G_OBJECT(private->preview_pixbuf)) +
...
private
  ->depth = gimp_viewable_get_depth(parent) + 1;
...
static GdkPixbuf *gimp_viewable_real_get_new_pixbuf(GimpViewable *viewable,
  ...
  if (temp_buf) {
    ...
    pixbuf = gdk_pixbuf_scale_simple(private->icon_pixbuf, width, height,
                                     GDK_INTERP_BILINEAR);
  }
...
static gboolean gimp_viewable_serialize_property(GimpConfig *config,
  ...
  case PROP_ICON_PIXBUF: {
    ...
    if (icon_pixbuf) {
      ...
      if (gdk_pixbuf_save_to_buffer(icon_pixbuf, &pixbuffer, &pixbuffer_size,
                                    "png", &error, NULL)) {
        gchar *pixbuffer_enc;

        pixbuffer_enc = g_base64_encode((guchar *)pixbuffer, pixbuffer_size);
  ...
  case PROP_ICON_PIXBUF: {
    ...
    if (encoded_image && strlen(encoded_image) > 0) {
      ...
      if (decoded_image) {
        ...
        stream =
            g_memory_input_stream_new_from_data(decoded_image, out_len, NULL);
 ...
 * counts.
 **/
void gimp_viewable_calc_preview_size(gint aspect_width, gint aspect_height,
                                     gint width, gint height,
                                     gboolean dot_for_dot, gdouble xresolution,
                                     gdouble yresolution, gint *return_width,
                                     gint *return_height,
                                     gboolean *scaling_up) {
  gdouble xratio;
  gdouble yratio;
...
gboolean gimp_viewable_get_size(GimpViewable *viewable, gint *width,
                                gint *height) {
 ...
 **/
void gimp_viewable_get_preview_size(GimpViewable *viewable, gint size,
  ...
  GIMP_VIEWABLE_GET_CLASS(viewable)->get_preview_size(viewable, size, popup,
                                                      dot_for_dot, &w, &h);
 ...
 *          original preview.
 **/
gboolean gimp_viewable_get_popup_size(GimpViewable *viewable, gint width,
  ...
  if (GIMP_VIEWABLE_GET_CLASS(viewable)->get_popup_size(viewable, width, height,
                                                        dot_for_dot, &w, &h)) {
    if (w < 1)
      w = 1;
    if (h < 1)
    ...
    if ((w > (2 * GIMP_VIEWABLE_MAX_POPUP_SIZE)) ||
      ...
      gimp_viewable_calc_preview_size(w, h, 2 * GIMP_VIEWABLE_MAX_POPUP_SIZE,
                                      2 * GIMP_VIEWABLE_MAX_POPUP_SIZE,
                                      dot_for_dot, 1.0, 1.0, &w, &h, NULL);
    }
    ...
    if ((w * h) > SQR(GIMP_VIEWABLE_MAX_POPUP_SIZE)) {
      ...
      factor =
          sqrt(((gdouble)(w * h) / (gdouble)SQR(GIMP_VIEWABLE_MAX_POPUP_SIZE)));
...
GimpTempBuf *gimp_viewable_get_preview(GimpViewable *viewable,
  ...
  if (viewable_class->get_new_preview)
    temp_buf =
        viewable_class->get_new_preview(viewable, context, width, height);

private
  ->preview_temp_buf = temp_buf;
 ...
 **/
GimpTempBuf *gimp_viewable_get_dummy_preview(GimpViewable *viewable, gint width,
 ...
 **/
GdkPixbuf *gimp_viewable_get_dummy_pixbuf(GimpViewable *viewable, gint width,
                                          gint height, gboolean with_alpha) {
  GdkPixbuf *icon;
  GdkPixbuf *pixbuf;
  GError *error = NULL;
  gdouble ratio;
  ...
  if (w && h)
    gdk_pixbuf_composite(icon, pixbuf, (width - w) / 2, (height - h) / 2, w, h,
...
gchar *gimp_viewable_get_description(GimpViewable *viewable, gchar **tooltip) {
  ...
  return GIMP_VIEWABLE_GET_CLASS(viewable)->get_description(viewable, tooltip);
}
...
void gimp_viewable_set_parent(GimpViewable *viewable, GimpViewable *parent) {
  ...
  private
    ->parent = parent;
  private
    ->depth = parent ? gimp_viewable_get_depth(parent) + 1 : 0;
