...
static void gimp_image_metadata_rotate(GimpImage *image, GimpContext *context,
                                       GExiv2Orientation orientation,
                                       GimpProgress *progress);

/* Public Functions */

void gimp_image_rotate(GimpImage *image, GimpContext *context,
                       GimpRotationType rotate_type, GimpProgress *progress) {
  ...
  while ((item = gimp_object_queue_pop(queue))) {
    ...
    if (GIMP_IS_LAYER(item)) {
      ...
      if (GIMP_IS_VECTORS(item)) {
        ...
        gimp_item_translate(
            item, (new_image_width - gimp_image_get_width(image)) / 2,
            (new_image_height - gimp_image_get_height(image)) / 2, FALSE);
      }
    }
  ...
  if (size_changed) {
    ...
    g_object_set(image, "width", new_image_width, "height", new_image_height,
                 NULL);
...
void gimp_image_import_rotation_metadata(GimpImage *image, GimpContext *context,
                                         GimpProgress *progress,
  ...
  if (metadata) {
    ...
    if (policy == GIMP_METADATA_ROTATION_POLICY_ASK) {
      if (interactive) {
        ...
        policy =
            gimp_query_rotation_policy(image->gimp, image, context, &dont_ask);

        if (dont_ask) {
          g_object_set(G_OBJECT(image->gimp->config),
                       "metadata-rotation-policy", policy, NULL);
        }
      } else {
    ...
    if (policy == GIMP_METADATA_ROTATION_POLICY_ROTATE)
      gimp_image_metadata_rotate(
          image, context,
          gexiv2_metadata_get_orientation(GEXIV2_METADATA(metadata)), progress);
  ...
  for (list = gimp_image_get_guides(image); list; list = g_list_next(list)) {
    ...
    case GIMP_ROTATE_90:
      ...
      case GIMP_ORIENTATION_HORIZONTAL:
        ...
        gimp_guide_set_position(guide, gimp_image_get_height(image) - position);
        break;
    ...
    case GIMP_ROTATE_180:
      ...
      case GIMP_ORIENTATION_HORIZONTAL:
        gimp_image_move_guide(image, guide,
                              gimp_image_get_height(image) - position, TRUE);
        break;

      case GIMP_ORIENTATION_VERTICAL:
        gimp_image_move_guide(image, guide,
                              gimp_image_get_width(image) - position, TRUE);
        break;
    ...
    case GIMP_ROTATE_270:
      ...
      case GIMP_ORIENTATION_VERTICAL:
        gimp_image_undo_push_guide(image, NULL, guide);
        gimp_guide_set_orientation(guide, GIMP_ORIENTATION_HORIZONTAL);
        gimp_guide_set_position(guide, gimp_image_get_width(image) - position);
        break;
...
static void gimp_image_rotate_sample_points(GimpImage *image,
  ...
  for (list = gimp_image_get_sample_points(image); list;
    ...
    switch (rotate_type) {
    case GIMP_ROTATE_90:
      gimp_sample_point_set_position(
          sample_point, gimp_image_get_height(image) - old_y, old_x);
      break;
    ...
    case GIMP_ROTATE_270:
      gimp_sample_point_set_position(sample_point, old_y,
                                     gimp_image_get_width(image) - old_x);
      break;
    }
  }
