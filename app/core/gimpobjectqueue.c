...
static void gimp_object_queue_class_init(GimpObjectQueueClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  object_class->dispose = gimp_object_queue_dispose;
}

static void gimp_object_queue_init(GimpObjectQueue *queue) {
...
void gimp_object_queue_push_container(GimpObjectQueue *queue,
  ...
  gimp_container_foreach(container, (GFunc)gimp_object_queue_push_swapped,
                         queue);
}
...
void gimp_object_queue_push_list(GimpObjectQueue *queue, GList *list) {
  ...
  g_list_foreach(list, (GFunc)gimp_object_queue_push_swapped, queue);
}
...
gpointer gimp_object_queue_pop(GimpObjectQueue *queue) {
  ...
  gimp_sub_progress_set_range(
      GIMP_SUB_PROGRESS(queue),
      (gdouble)queue->processed_memsize / (gdouble)queue->total_memsize,
      (gdouble)(queue->processed_memsize + item->memsize) /
