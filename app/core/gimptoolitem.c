...
enum { VISIBLE_CHANGED, SHOWN_CHANGED, LAST_SIGNAL };

enum { PROP_0, PROP_VISIBLE, PROP_SHOWN };
...
static void gimp_tool_item_get_property(GObject *object, guint property_id,
                                        GValue *value, GParamSpec *pspec);
static void gimp_tool_item_set_property(GObject *object, guint property_id,
...
static void gimp_tool_item_class_init(GimpToolItemClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  gimp_tool_item_signals[VISIBLE_CHANGED] = g_signal_new(
      "visible-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolItemClass, visible_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_tool_item_signals[SHOWN_CHANGED] = g_signal_new(
      "shown-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolItemClass, shown_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  object_class->get_property = gimp_tool_item_get_property;
  object_class->set_property = gimp_tool_item_set_property;

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_VISIBLE, "visible", NULL, NULL,
                           TRUE, GIMP_PARAM_STATIC_STRINGS);

  g_object_class_install_property(
      object_class, PROP_SHOWN,
      g_param_spec_boolean("shown", NULL, NULL, TRUE, GIMP_PARAM_READABLE));
}

static void gimp_tool_item_init(GimpToolItem *tool_item) {
...
gboolean gimp_tool_item_get_shown(GimpToolItem *tool_item) {
  ...
  parent = GIMP_TOOL_ITEM(gimp_viewable_get_parent(GIMP_VIEWABLE(tool_item)));
