...
GeglBuffer *gimp_drawable_transform_buffer_affine(
    GimpDrawable *drawable, GimpContext *context, GeglBuffer *orig_buffer,
    gint orig_offset_x, gint orig_offset_y, const GimpMatrix3 *matrix,
    GimpTransformDirection direction, GimpInterpolationType interpolation_type,
    GimpTransformResize clip_result, GimpColorProfile **buffer_profile,
    gint *new_offset_x, gint *new_offset_y, GimpProgress *progress) {
  ...
  *buffer_profile =
  ...
  gimp_transform_resize_boundary(&m, clip_result, u1, v1, u2, v2, &x1, &y1, &x2,
                                 &y2);
  ...
  gimp_gegl_apply_transform(orig_buffer, progress, NULL, new_buffer,
                            interpolation_type, &gegl_matrix);
...
GeglBuffer *gimp_drawable_transform_buffer_flip(
    GimpDrawable *drawable, GimpContext *context, GeglBuffer *orig_buffer,
    gint orig_offset_x, gint orig_offset_y, GimpOrientationType flip_type,
    gdouble axis, gboolean clip_result, GimpColorProfile **buffer_profile,
  ...
  *buffer_profile =
  ...
  case GIMP_ORIENTATION_HORIZONTAL:
    new_x = RINT(-((gdouble)orig_x + (gdouble)orig_width - axis) + axis);
    break;

  case GIMP_ORIENTATION_VERTICAL:
    new_y = RINT(-((gdouble)orig_y + (gdouble)orig_height - axis) + axis);
    break;
  ...
  new_buffer =
      gegl_buffer_new(GEGL_RECTANGLE(0, 0, new_width, new_height), format);
  ...
  if (clip_result && (new_x != orig_x || new_y != orig_y)) {
    ...
    *new_offset_y = orig_y;
    ...
    if (GIMP_IS_CHANNEL(drawable) || babl_format_has_alpha(format)) {
      ...
      gimp_pickable_srgb_to_image_color(GIMP_PICKABLE(drawable), &bg, &bg);
    }
    ...
    if (gimp_rectangle_intersect(orig_x, orig_y, orig_width, orig_height, new_x,
                                 new_y, new_width, new_height, &clip_x, &clip_y,
...
static void gimp_drawable_transform_rotate_point(gint x, gint y,
                                                 GimpRotationType rotate_type,
                                                 gdouble center_x,
                                                 gdouble center_y, gint *new_x,
                                                 gint *new_y) {
...
GeglBuffer *gimp_drawable_transform_buffer_rotate(
    GimpDrawable *drawable, GimpContext *context, GeglBuffer *orig_buffer,
    gint orig_offset_x, gint orig_offset_y, GimpRotationType rotate_type,
    gdouble center_x, gdouble center_y, gboolean clip_result,
    GimpColorProfile **buffer_profile, gint *new_offset_x, gint *new_offset_y) {
  const Babl *format;
  GeglBuffer *new_buffer;
  GeglRectangle src_rect;
  GeglRectangle dest_rect;
  ...
  *buffer_profile =
  ...
  orig_height = gegl_buffer_get_height(orig_buffer);
  orig_bpp =
      babl_format_get_bytes_per_pixel(gegl_buffer_get_format(orig_buffer));

  switch (rotate_type) {
  case GIMP_ROTATE_90:
    gimp_drawable_transform_rotate_point(orig_x, orig_y + orig_height,
  ...
  case GIMP_ROTATE_180:
    gimp_drawable_transform_rotate_point(orig_x + orig_width,
                                         orig_y + orig_height, rotate_type,
  ...
  case GIMP_ROTATE_270:
    gimp_drawable_transform_rotate_point(orig_x + orig_width, orig_y,
                                         rotate_type, center_x, center_y,
                                         &new_x, &new_y);
    new_width = orig_height;
    new_height = orig_width;
...

  {
    ...
    new_buffer =
        gegl_buffer_new(GEGL_RECTANGLE(0, 0, orig_width, orig_height), format);
    ...
    *new_offset_y = orig_y;
    ...
    if (gimp_rectangle_intersect(orig_x, orig_y, orig_width, orig_height, new_x,
      ...
      switch (rotate_type) {
      case GIMP_ROTATE_90:
        gimp_drawable_transform_rotate_point(clip_x + clip_width, clip_y,
                                             GIMP_ROTATE_270, center_x,
                                             center_y, &orig_x, &orig_y);
        orig_x -= saved_orig_x;
        orig_y -= saved_orig_y;
      ...
      case GIMP_ROTATE_270:
        gimp_drawable_transform_rotate_point(clip_x, clip_y + clip_height,
                                             GIMP_ROTATE_90, center_x, center_y,
                                             &orig_x, &orig_y);
        orig_x -= saved_orig_x;
        orig_y -= saved_orig_y;
        orig_width = clip_height;
...
GimpDrawable *gimp_drawable_transform_affine(
    GimpDrawable *drawable, GimpContext *context, const GimpMatrix3 *matrix,
    GimpTransformDirection direction, GimpInterpolationType interpolation_type,
  ...
  orig_buffer = gimp_drawable_transform_cut(drawables, context, &orig_offset_x,
                                            &orig_offset_y, &new_layer);
  g_free(drawables);
  ...
  if (orig_buffer) {
    ...
    if (GIMP_IS_LAYER(drawable) && gimp_layer_get_mask(GIMP_LAYER(drawable)) &&
        gimp_channel_is_empty(gimp_image_get_mask(image))) {
      GimpLayerMask *mask = gimp_layer_get_mask(GIMP_LAYER(drawable));

      gimp_item_transform(GIMP_ITEM(mask), context, matrix, direction,
                          interpolation_type, clip_result, progress);
    }

    /* transform the buffer */
    new_buffer = gimp_drawable_transform_buffer_affine(
        drawable, context, orig_buffer, orig_offset_x, orig_offset_y, matrix,
        direction, interpolation_type, clip_result, &profile, &new_offset_x,
        &new_offset_y, progress);

    /* Free the cut/copied buffer */
    g_object_unref(orig_buffer);

    if (new_buffer) {
      result = gimp_drawable_transform_paste(
          drawable, new_buffer, profile, new_offset_x, new_offset_y, new_layer);
...
GimpDrawable *gimp_drawable_transform_flip(GimpDrawable *drawable,
  ...
  if (orig_buffer) {
    ...
    if (GIMP_IS_LAYER(drawable) && gimp_layer_get_mask(GIMP_LAYER(drawable)) &&
      ...
      gimp_item_flip(GIMP_ITEM(mask), context, flip_type, axis, clip_result);
    }

    /* transform the buffer */
    new_buffer = gimp_drawable_transform_buffer_flip(
        drawable, context, orig_buffer, orig_offset_x, orig_offset_y, flip_type,
        axis, clip_result, &profile, &new_offset_x, &new_offset_y);
...
GimpDrawable *gimp_drawable_transform_rotate(GimpDrawable *drawable,
  ...
  if (orig_buffer) {
    ...
    if (GIMP_IS_LAYER(drawable) && gimp_layer_get_mask(GIMP_LAYER(drawable)) &&
      ...
      gimp_item_rotate(GIMP_ITEM(mask), context, rotate_type, center_x,
                       center_y, clip_result);
    }

    /* transform the buffer */
    new_buffer = gimp_drawable_transform_buffer_rotate(
        drawable, context, orig_buffer, orig_offset_x, orig_offset_y,
        rotate_type, center_x, center_y, clip_result, &profile, &new_offset_x,
        &new_offset_y);
...
GeglBuffer *gimp_drawable_transform_cut(GList *drawables, GimpContext *context,
  ...
  if (!gimp_channel_is_empty(gimp_image_get_mask(image))) {
    ...
    if (gimp_image_mask_intersect(image, drawables, &x, &y, &w, &h)) {
      buffer = gimp_selection_extract(
          GIMP_SELECTION(gimp_image_get_mask(image)), drawables, context, TRUE,
          FALSE, TRUE, offset_x, offset_y, NULL);
      /*  clear the selection  */
  ...
  {
    buffer = gimp_selection_extract(
        GIMP_SELECTION(gimp_image_get_mask(image)), drawables, context, FALSE,
        TRUE, drawables_are_layers, offset_x, offset_y, NULL);
...
GimpDrawable *gimp_drawable_transform_paste(GimpDrawable *drawable,
  ...
  if (new_layer) {
    layer = gimp_layer_new_from_gegl_buffer(
        buffer, image, gimp_drawable_get_format_with_alpha(drawable),
        _("Transformation"), GIMP_OPACITY_OPAQUE,
        gimp_image_get_default_new_layer_mode(image), buffer_profile);
    ...
    gimp_drawable_set_buffer_full(drawable, TRUE, NULL, buffer,
