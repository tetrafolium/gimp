...
enum { TAG_COUNT_CHANGED, LAST_SIGNAL };
...
static void
gimp_tagged_container_tag_added(GimpTagged *tagged, GimpTag *tag,
                                GimpTaggedContainer *tagged_container);
static void
gimp_tagged_container_tag_removed(GimpTagged *tagged, GimpTag *tag,
                                  GimpTaggedContainer *tagged_container);
...
gimp_tagged_container_unref_tag(GimpTaggedContainer *tagged_container,
                                GimpTag *tag);
static void
gimp_tagged_container_tag_count_changed(GimpTaggedContainer *tagged_container,
                                        gint tag_count);
...
static guint gimp_tagged_container_signals[LAST_SIGNAL] = {
    0,
};

static void gimp_tagged_container_class_init(GimpTaggedContainerClass *klass) {
  GObjectClass *g_object_class = G_OBJECT_CLASS(klass);
  GimpObjectClass *gimp_object_class = GIMP_OBJECT_CLASS(klass);
  GimpContainerClass *container_class = GIMP_CONTAINER_CLASS(klass);
  GimpFilteredContainerClass *filtered_class =
      GIMP_FILTERED_CONTAINER_CLASS(klass);
  ...
  gimp_tagged_container_signals[TAG_COUNT_CHANGED] = g_signal_new(
      "tag-count-changed", GIMP_TYPE_TAGGED_CONTAINER, G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpTaggedContainerClass, tag_count_changed), NULL, NULL,
      NULL, G_TYPE_NONE, 1, G_TYPE_INT);
}

static void gimp_tagged_container_init(GimpTaggedContainer *tagged_container) {
  tagged_container->tag_ref_counts = g_hash_table_new_full(
      (GHashFunc)gimp_tag_get_hash, (GEqualFunc)gimp_tag_equals,
...
static void gimp_tagged_container_clear(GimpContainer *container) {
  GimpFilteredContainer *filtered_container =
      GIMP_FILTERED_CONTAINER(container);
  GimpTaggedContainer *tagged_container = GIMP_TAGGED_CONTAINER(container);
  GList *list;

  for (list = GIMP_LIST(filtered_container->src_container)->queue->head; list;
       list = g_list_next(list)) {
    g_signal_handlers_disconnect_by_func(
        list->data, gimp_tagged_container_tag_added, tagged_container);
    g_signal_handlers_disconnect_by_func(
        list->data, gimp_tagged_container_tag_removed, tagged_container);
  }
...
gimp_tagged_container_src_add(GimpFilteredContainer *filtered_container,
                              GimpObject *object) {
  GimpTaggedContainer *tagged_container =
      GIMP_TAGGED_CONTAINER(filtered_container);
  GList *list;

  for (list = gimp_tagged_get_tags(GIMP_TAGGED(object)); list;
       list = g_list_next(list)) {
...
gimp_tagged_container_src_remove(GimpFilteredContainer *filtered_container,
  ...
  g_signal_handlers_disconnect_by_func(object, gimp_tagged_container_tag_added,
                                       tagged_container);
  g_signal_handlers_disconnect_by_func(
      object, gimp_tagged_container_tag_removed, tagged_container);
...
GimpContainer *gimp_tagged_container_new(GimpContainer *src_container) {
  ...
  tagged_container = g_object_new(
      GIMP_TYPE_TAGGED_CONTAINER, "sort-func", sort_func, "children-type",
      children_type, "policy", GIMP_CONTAINER_POLICY_WEAK, "unique-names",
      FALSE, "src-container", src_container, NULL);
...
void gimp_tagged_container_set_filter(GimpTaggedContainer *tagged_container,
  ...
  if (!gimp_container_frozen(
          GIMP_FILTERED_CONTAINER(tagged_container)->src_container)) {
...
gimp_tagged_container_object_matches(GimpTaggedContainer *tagged_container,
  ...
  for (filter_tags = tagged_container->filter; filter_tags;
    ...
    if (!gimp_tagged_has_tag(GIMP_TAGGED(object), filter_tags->data)) {
...
gimp_tagged_container_tag_added(GimpTagged *tagged, GimpTag *tag,
  ...
  if (gimp_tagged_container_object_matches(tagged_container,
      ...
      !gimp_container_have(GIMP_CONTAINER(tagged_container),
                           GIMP_OBJECT(tagged))) {
    gimp_container_add(GIMP_CONTAINER(tagged_container), GIMP_OBJECT(tagged));
  }
}
...
static void gimp_tagged_container_ref_tag(GimpTaggedContainer *tagged_container,
  ...
  ref_count = GPOINTER_TO_INT(
      g_hash_table_lookup(tagged_container->tag_ref_counts, tag));
  ref_count++;
  g_hash_table_insert(tagged_container->tag_ref_counts, g_object_ref(tag),
