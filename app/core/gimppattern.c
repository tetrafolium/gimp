...
static gint64 gimp_pattern_get_memsize(GimpObject *object, gint64 *gui_size);

static gboolean gimp_pattern_get_size(GimpViewable *viewable, gint *width,
                                      gint *height);
...
static void gimp_pattern_copy(GimpData *data, GimpData *src_data);
...
static void gimp_pattern_init(GimpPattern *pattern) { pattern->mask = NULL; }
...
static GimpTempBuf *gimp_pattern_get_new_preview(GimpViewable *viewable,
  ...
  gimp_gegl_buffer_copy(
      src_buffer, GEGL_RECTANGLE(0, 0, copy_width, copy_height),
...
static gchar *gimp_pattern_get_description(GimpViewable *viewable,
  ...
  return g_strdup_printf("%s (%d × %d)", gimp_object_get_name(pattern),
...
GimpData *gimp_pattern_new(GimpContext *context, const gchar *name) {
  ...
  pattern = g_object_new(GIMP_TYPE_PATTERN, "name", name, NULL);
