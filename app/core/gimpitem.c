...
#define GET_PRIVATE(item)                                                      \
  ((GimpItemPrivate *)gimp_item_get_instance_private((GimpItem *)(item)))

/*  local function prototypes  */

static void gimp_item_constructed(GObject *object);
static void gimp_item_finalize(GObject *object);
static void gimp_item_set_property(GObject *object, guint property_id,
                                   const GValue *value, GParamSpec *pspec);
static void gimp_item_get_property(GObject *object, guint property_id,
                                   GValue *value, GParamSpec *pspec);

static gint64 gimp_item_get_memsize(GimpObject *object, gint64 *gui_size);
...
static gboolean gimp_item_real_bounds(GimpItem *item, gdouble *x, gdouble *y,
                                      gdouble *width, gdouble *height);
static GimpItem *gimp_item_real_duplicate(GimpItem *item, GType new_type);
static void gimp_item_real_convert(GimpItem *item, GimpImage *dest_image,
                                   GType old_type);
static gboolean gimp_item_real_rename(GimpItem *item, const gchar *new_name,
                                      const gchar *undo_desc, GError **error);
static void gimp_item_real_start_transform(GimpItem *item, gboolean push_undo);
static void gimp_item_real_end_transform(GimpItem *item, gboolean push_undo);
static void gimp_item_real_translate(GimpItem *item, gdouble offset_x,
                                     gdouble offset_y, gboolean push_undo);
static void gimp_item_real_scale(GimpItem *item, gint new_width,
                                 gint new_height, gint new_offset_x,
                                 gint new_offset_y,
                                 GimpInterpolationType interpolation,
                                 GimpProgress *progress);
static void gimp_item_real_resize(GimpItem *item, GimpContext *context,
                                  GimpFillType fill_type, gint new_width,
                                  gint new_height, gint offset_x,
                                  gint offset_y);
static GimpTransformResize
gimp_item_real_get_clip(GimpItem *item, GimpTransformResize clip_result);
...
static void gimp_item_class_init(GimpItemClass *klass) {
  ...
  gimp_item_signals[REMOVED] =
      g_signal_new("removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpItemClass, removed), NULL, NULL, NULL,
                   G_TYPE_NONE, 0);

  gimp_item_signals[VISIBILITY_CHANGED] = g_signal_new(
      "visibility-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpItemClass, visibility_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_item_signals[LINKED_CHANGED] = g_signal_new(
      "linked-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpItemClass, linked_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_item_signals[COLOR_TAG_CHANGED] = g_signal_new(
      "color-tag-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpItemClass, color_tag_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_item_signals[LOCK_CONTENT_CHANGED] = g_signal_new(
      "lock-content-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpItemClass, lock_content_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_item_signals[LOCK_POSITION_CHANGED] = g_signal_new(
      "lock-position-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpItemClass, lock_position_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("image", NULL, NULL, GIMP_TYPE_IMAGE,
  ...
  g_object_class_install_property(object_class, PROP_HEIGHT,
                                  g_param_spec_int("height", NULL, NULL, 1,
                                                   GIMP_MAX_IMAGE_SIZE, 1,
                                                   GIMP_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_OFFSET_X,
      g_param_spec_int("offset-x", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READABLE));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("visible", NULL, NULL, TRUE, GIMP_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_LINKED,
      g_param_spec_boolean("linked", NULL, NULL, FALSE, GIMP_PARAM_READABLE));

  g_object_class_install_property(
      object_class, PROP_COLOR_TAG,
      g_param_spec_enum("color-tag", NULL, NULL, GIMP_TYPE_COLOR_TAG,
                        GIMP_COLOR_TAG_NONE, GIMP_PARAM_READABLE));

  g_object_class_install_property(object_class, PROP_LOCK_CONTENT,
                                  g_param_spec_boolean("lock-content", NULL,
                                                       NULL, FALSE,
                                                       GIMP_PARAM_READABLE));

  g_object_class_install_property(object_class, PROP_LOCK_POSITION,
                                  g_param_spec_boolean("lock-position", NULL,
                                                       NULL, FALSE,
                                                       GIMP_PARAM_READABLE));
}
...
static void gimp_item_finalize(GObject *object) {
  ...
  if (private->offset_nodes) {
    g_list_free_full(private->offset_nodes, (GDestroyNotify)g_object_unref);
  private
    ->offset_nodes = NULL;
  }
...
private
  ...
  new_item = gimp_item_new(
      new_type, gimp_item_get_image(item), new_name, private->offset_x,
      private->offset_y, gimp_item_get_width(item), gimp_item_get_height(item));
  ...
  GET_PRIVATE(new_item)->parasites =
      gimp_parasite_list_copy(private->parasites);
...
static gboolean gimp_item_real_rename(GimpItem *item, const gchar *new_name,
  ...
  if (gimp_item_is_attached(item))
    gimp_item_tree_rename_item(gimp_item_get_tree(item), item, new_name, TRUE,
                               undo_desc);
  else
...
static void gimp_item_real_translate(GimpItem *item, gdouble offset_x,
  ...
  gimp_item_set_offset(item, private->offset_x + SIGNED_ROUND(offset_x),
...
static void gimp_item_real_resize(GimpItem *item, GimpContext *context,
  ...
  gimp_item_set_offset(item, private->offset_x - offset_x,
 ...
 */
GimpItem *gimp_item_new(GType type, GimpImage *image, const gchar *name,
                        gint offset_x, gint offset_y, gint width, gint height) {
  GimpItem *item;
  GimpItemPrivate *private;
  ...
  item = g_object_new(type, "image", image, NULL);
...
gboolean gimp_item_bounds(GimpItem *item, gint *x, gint *y, gint *width,
                          gint *height) {
  ...
  retval = GIMP_ITEM_GET_CLASS(item)->bounds(item, &tmp_x, &tmp_y, &tmp_width,
                                             &tmp_height);
  ...
  if (y)
    *y = floor(tmp_y);
  if (width)
    *width = ceil(tmp_x + tmp_width) - floor(tmp_x);
  if (height)
    *height = ceil(tmp_y + tmp_height) - floor(tmp_y);

  return retval;
}

gboolean gimp_item_bounds_f(GimpItem *item, gdouble *x, gdouble *y,
                            gdouble *width, gdouble *height) {
  gdouble tmp_x, tmp_y, tmp_width, tmp_height;
  gboolean retval;
 ...
 */
GimpItem *gimp_item_convert(GimpItem *item, GimpImage *dest_image,
                            GType new_type) {
  GimpItem *new_item;
  GType old_type;
 ...
 */
gboolean gimp_item_rename(GimpItem *item, const gchar *new_name,
                          GError **error) {
...
void gimp_item_set_size(GimpItem *item, gint width, gint height) {
  GimpItemPrivate *private;
 ...
 */
void gimp_item_get_offset(GimpItem *item, gint *offset_x, gint *offset_y) {
  GimpItemPrivate *private;
...
private
  ...
  if (offset_x)
    *offset_x = private->offset_x;
  if (offset_y)
    *offset_y = private->offset_y;
}

void gimp_item_set_offset(GimpItem *item, gint offset_x, gint offset_y) {
  GimpItemPrivate *private;
  GList *list;
...
private
  ...
  for (list = private->offset_nodes; list; list = g_list_next(list)) {
    ...
    gegl_node_set(node, "x", (gdouble) private->offset_x, "y",
                  (gdouble) private->offset_y, NULL);
  }
...
void gimp_item_start_move(GimpItem *item, gboolean push_undo) {
  g_return_if_fail(GIMP_IS_ITEM(item));
...
void gimp_item_end_move(GimpItem *item, gboolean push_undo) {
  g_return_if_fail(GIMP_IS_ITEM(item));
...
void gimp_item_start_transform(GimpItem *item, gboolean push_undo) {
  g_return_if_fail(GIMP_IS_ITEM(item));
...
void gimp_item_end_transform(GimpItem *item, gboolean push_undo) {
  g_return_if_fail(GIMP_IS_ITEM(item));
 ...
 */
void gimp_item_translate(GimpItem *item, gdouble offset_x, gdouble offset_y,
                         gboolean push_undo) {
  GimpItemClass *item_class;
  GimpImage *image;
 ...
 **/
gboolean gimp_item_check_scaling(GimpItem *item, gint new_width,
                                 gint new_height) {
  GimpItemPrivate *private;
  GimpImage *image;
  gdouble img_scale_w;
...
private
  ...
  img_scale_w = ((gdouble)new_width / (gdouble)gimp_image_get_width(image));
  img_scale_h = ((gdouble)new_height / (gdouble)gimp_image_get_height(image));
...
void gimp_item_scale(GimpItem *item, gint new_width, gint new_height,
 ...
 **/
gboolean gimp_item_scale_by_factors(GimpItem *item, gdouble w_factor,
                                    gdouble h_factor,
                                    GimpInterpolationType interpolation,
                                    GimpProgress *progress) {
  return gimp_item_scale_by_factors_with_origin(item, w_factor, h_factor, 0, 0,
                                                0, 0, interpolation, progress);
}
 ...
 *          %FALSE if the scaled item has at least one zero dimension
 **/
gboolean gimp_item_scale_by_factors_with_origin(
    GimpItem *item, gdouble w_factor, gdouble h_factor, gint origin_x,
    gint origin_y, gint new_origin_x, gint new_origin_y,
    GimpInterpolationType interpolation, GimpProgress *progress) {
  GimpItemPrivate *private;
  GimpContainer *children;
...
private
  ...
  if (w_factor <= 0.0 || h_factor <= 0.0) {
    g_warning("%s: requested width or height scale is non-positive", G_STRFUNC);
    return FALSE;
  }
  ...
  if (new_width > 0 && new_height > 0) {
    gimp_item_scale(item, new_width, new_height, new_offset_x, new_offset_y,
                    interpolation, progress);
    return TRUE;
  }
 ...
 **/
void gimp_item_scale_by_origin(GimpItem *item, gint new_width, gint new_height,
                               GimpInterpolationType interpolation,
                               GimpProgress *progress, gboolean local_origin) {
  GimpItemPrivate *private;
  gint new_offset_x, new_offset_y;
...
private
  ...
  if (local_origin) {
    new_offset_x =
        (private->offset_x + ((gimp_item_get_width(item) - new_width) / 2.0));
    new_offset_y =
        (private->offset_y + ((gimp_item_get_height(item) - new_height) / 2.0));
  } else {
    new_offset_x = (gint)(((gdouble)new_width * (gdouble) private->offset_x /
                           (gdouble)gimp_item_get_width(item)));

    new_offset_y = (gint)(((gdouble)new_height * (gdouble) private->offset_y /
...
void gimp_item_resize(GimpItem *item, GimpContext *context,
                      GimpFillType fill_type, gint new_width, gint new_height,
  ...
  item_class->resize(item, context, fill_type, new_width, new_height, offset_x,
                     offset_y);
...
void gimp_item_flip(GimpItem *item, GimpContext *context,
...
void gimp_item_rotate(GimpItem *item, GimpContext *context,
...
void gimp_item_transform(GimpItem *item, GimpContext *context,
...
gboolean gimp_item_fill(GimpItem *item, GList *drawables,
                        GimpFillOptions *fill_options, gboolean push_undo,
  ...
  if (item_class->fill) {
    ...
    for (iter = drawables; iter; iter = iter->next) {
      retval = item_class->fill(item, iter->data, fill_options, push_undo,
                                progress, error);
      if (!retval)
        break;
    }
...
gboolean gimp_item_stroke(GimpItem *item, GList *drawables,
                          GimpContext *context,
                          GimpStrokeOptions *stroke_options,
                          GimpPaintOptions *paint_options, gboolean push_undo,
                          GimpProgress *progress, GError **error) {
  GimpItemClass *item_class;
  GList *iter;
  gboolean retval = FALSE;
  ...
  g_return_val_if_fail(
      paint_options == NULL || GIMP_IS_PAINT_OPTIONS(paint_options), FALSE);
  g_return_val_if_fail(progress == NULL || GIMP_IS_PROGRESS(progress), FALSE);
  g_return_val_if_fail(error == NULL || *error == NULL, FALSE);
...
void gimp_item_to_selection(GimpItem *item, GimpChannelOps op,
  ...
  if (item_class->to_selection)
    item_class->to_selection(item, op, antialias, feather, feather_radius_x,
                             feather_radius_y);
}

void gimp_item_add_offset_node(GimpItem *item, GeglNode *node) {
  GimpItemPrivate *private;
...
private
  ->offset_nodes = g_list_append(private->offset_nodes, g_object_ref(node));
}

void gimp_item_remove_offset_node(GimpItem *item, GeglNode *node) {
  GimpItemPrivate *private;
...
private
  ->offset_nodes = g_list_remove(private->offset_nodes, node);
  g_object_unref(node);
}
...
GimpItem *gimp_item_get_by_id(Gimp *gimp, gint item_id) {
...
void gimp_item_set_image(GimpItem *item, GimpImage *image) {
  GimpItemPrivate *private;
...
private
  ...
  if (private->ID == 0) {
  private
    ->ID = gimp_id_table_insert(image->gimp->item_table, item);
  ...
  private
    ->tattoo = gimp_image_get_new_tattoo(image);
  }
 ...
 **/
void gimp_item_replace_item(GimpItem *item, GimpItem *replace) {
  GimpItemPrivate *private;
  gint offset_x;
  gint offset_y;
...
private
  ->parasites = GET_PRIVATE(replace)->parasites;
  GET_PRIVATE(replace)->parasites = NULL;

  gimp_item_get_offset(replace, &offset_x, &offset_y);
  gimp_item_set_offset(item, offset_x, offset_y);

  gimp_item_set_size(item, gimp_item_get_width(replace),
  ...
  gimp_item_set_lock_position(item, gimp_item_get_lock_position(replace),
                              FALSE);
}
 ...
 * sources.
 **/
void gimp_item_set_parasites(GimpItem *item, GimpParasiteList *parasites) {
  GimpItemPrivate *private;
...
void gimp_item_parasite_attach(GimpItem *item, const GimpParasite *parasite,
                               gboolean push_undo) {
  GimpItemPrivate *private;
  GimpParasite copy;
...
private
    ...
    if (gimp_parasite_is_undoable(&copy)) {
      ...
      gimp_image_undo_push_item_parasite(private->image, NULL, item, &copy);
               ...
               !gimp_parasite_compare(
                   &copy,
                   gimp_item_parasite_find(item, gimp_parasite_name(&copy)))) {
  ...
  if (gimp_item_is_attached(item) && gimp_parasite_is_undoable(&copy)) {
    gimp_image_undo_group_end(private->image);
  }
}

void gimp_item_parasite_detach(GimpItem *item, const gchar *name,
                               gboolean push_undo) {
  GimpItemPrivate *private;
  const GimpParasite *parasite;
  ...
  if (push_undo) {
    if (gimp_parasite_is_undoable(parasite)) {
      gimp_image_undo_push_item_parasite_remove(
          private->image, C_("undo-type", "Remove Parasite from Item"), item,
          gimp_parasite_name(parasite));
    } else if (gimp_parasite_is_persistent(parasite)) {
      gimp_image_undo_push_cantundo(
          private->image, C_("undo-type", "Remove Parasite from Item"));
    }
  }
...
const GimpParasite *gimp_item_parasite_find(GimpItem *item, const gchar *name) {
  g_return_val_if_fail(GIMP_IS_ITEM(item), NULL);

  return gimp_parasite_list_find(GET_PRIVATE(item)->parasites, name);
}

static void gimp_item_parasite_list_foreach_func(gchar *name,
                                                 GimpParasite *parasite,
                                                 gchar ***cur) {
  *(*cur)++ = (gchar *)g_strdup(name);
}

gchar **gimp_item_parasite_list(GimpItem *item, gint *count) {
  GimpItemPrivate *private;
  gchar **list;
  gchar **cur;
...
private
  ...
  *count = gimp_parasite_list_length(private->parasites);
  ...
  gimp_parasite_list_foreach(
      private->parasites, (GHFunc)gimp_item_parasite_list_foreach_func, &cur);

  return list;
}

void gimp_item_set_visible(GimpItem *item, gboolean visible,
                           gboolean push_undo) {
  g_return_if_fail(GIMP_IS_ITEM(item));
...
void gimp_item_bind_visible_to_active(GimpItem *item, gboolean bind) {
  g_return_if_fail(GIMP_IS_ITEM(item));
...
void gimp_item_set_color_tag(GimpItem *item, GimpColorTag color_tag,
                             gboolean push_undo) {
  g_return_if_fail(GIMP_IS_ITEM(item));
...
void gimp_item_set_lock_content(GimpItem *item, gboolean lock_content,
                                gboolean push_undo) {
  g_return_if_fail(GIMP_IS_ITEM(item));
...
void gimp_item_set_lock_position(GimpItem *item, gboolean lock_position,
                                 gboolean push_undo) {
  g_return_if_fail(GIMP_IS_ITEM(item));
...
gboolean gimp_item_mask_bounds(GimpItem *item, gint *x1, gint *y1, gint *x2,
                               gint *y2) {
  GimpImage *image;
 ...
 * item.
 */
gboolean gimp_item_mask_intersect(GimpItem *item, gint *x, gint *y, gint *width,
                                  gint *height) {
  GimpImage *image;
  GimpChannel *selection;
  gint tmp_x, tmp_y;
   ...
   */
  if (GIMP_ITEM(selection) != item && !gimp_channel_is_empty(selection) &&
      gimp_item_bounds(GIMP_ITEM(selection), &tmp_x, &tmp_y, &tmp_width,
                       &tmp_height)) {
    gint off_x, off_y;

    gimp_item_get_offset(item, &off_x, &off_y);

    retval = gimp_rectangle_intersect(
        tmp_x - off_x, tmp_y - off_y, tmp_width, tmp_height, 0, 0,
...
gboolean gimp_item_is_in_set(GimpItem *item, GimpItemSet set) {
  GimpItemPrivate *private;
...
private
  ...
  case GIMP_ITEM_SET_IMAGE_SIZED:
    return (gimp_item_get_width(item) == gimp_image_get_width(private->image) &&
            gimp_item_get_height(item) ==
                gimp_image_get_height(private->image));
