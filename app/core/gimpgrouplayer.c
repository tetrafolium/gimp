...
#define GET_PRIVATE(item)                                                      \
  ((GimpGroupLayerPrivate *)gimp_group_layer_get_instance_private(             \
      (GimpGroupLayer *)(item)))
...
static void gimp_group_layer_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_group_layer_get_property(GObject *object, guint property_id,
...
static gboolean gimp_group_layer_get_size(GimpViewable *viewable, gint *width,
                                          gint *height);
...
static GimpItem *gimp_group_layer_duplicate(GimpItem *item, GType new_type);
static void gimp_group_layer_convert(GimpItem *item, GimpImage *dest_image,
                                     GType old_type);
static void gimp_group_layer_start_transform(GimpItem *item,
                                             gboolean push_undo);
static void gimp_group_layer_end_transform(GimpItem *item, gboolean push_undo);
static void gimp_group_layer_resize(GimpItem *item, GimpContext *context,
                                    GimpFillType fill_type, gint new_width,
                                    gint new_height, gint offset_x,
                                    gint offset_y);
static GimpTransformResize
gimp_group_layer_get_clip(GimpItem *item, GimpTransformResize clip_result);
...
static void gimp_group_layer_translate(GimpLayer *layer, gint offset_x,
                                       gint offset_y);
static void gimp_group_layer_scale(GimpLayer *layer, gint new_width,
                                   gint new_height, gint new_offset_x,
                                   gint new_offset_y,
                                   GimpInterpolationType interp_type,
                                   GimpProgress *progress);
static void gimp_group_layer_flip(GimpLayer *layer, GimpContext *context,
                                  GimpOrientationType flip_type, gdouble axis,
                                  gboolean clip_result);
static void gimp_group_layer_rotate(GimpLayer *layer, GimpContext *context,
                                    GimpRotationType rotate_type,
                                    gdouble center_x, gdouble center_y,
                                    gboolean clip_result);
static void gimp_group_layer_transform(GimpLayer *layer, GimpContext *context,
                                       const GimpMatrix3 *matrix,
                                       GimpTransformDirection direction,
                                       GimpInterpolationType interpolation_type,
                                       GimpTransformResize clip_result,
                                       GimpProgress *progress);
static void gimp_group_layer_convert_type(
...
static void gimp_group_layer_excludes_backdrop_changed(GimpLayer *layer);
static GeglRectangle gimp_group_layer_get_bounding_box(GimpLayer *layer);
static void
gimp_group_layer_get_effective_mode(GimpLayer *layer, GimpLayerMode *mode,
                                    GimpLayerColorSpace *blend_space,
                                    GimpLayerColorSpace *composite_space,
                                    GimpLayerCompositeMode *composite_mode);
static gboolean gimp_group_layer_get_excludes_backdrop(GimpLayer *layer);
...
static gdouble gimp_group_layer_get_opacity_at(GimpPickable *pickable, gint x,
                                               gint y);

static void gimp_group_layer_child_add(GimpContainer *container,
                                       GimpLayer *child, GimpGroupLayer *group);
static void gimp_group_layer_child_remove(GimpContainer *container,
                                          GimpLayer *child,
                                          GimpGroupLayer *group);
static void gimp_group_layer_child_move(GimpLayer *child, GParamSpec *pspec,
                                        GimpGroupLayer *group);
...
static void gimp_group_layer_stack_update(GimpDrawableStack *stack, gint x,
                                          gint y, gint width, gint height,
                                          GimpGroupLayer *group);
static void gimp_group_layer_proj_update(GimpProjection *proj, gboolean now,
                                         gint x, gint y, gint width,
                                         gint height, GimpGroupLayer *group);
...
static void gimp_group_layer_class_init(GimpGroupLayerClass *klass) {
  ...
  layer_class->excludes_backdrop_changed =
      gimp_group_layer_excludes_backdrop_changed;
...
static void gimp_projectable_iface_init(GimpProjectableInterface *iface) {
  ...
  iface->get_offset =
      (void (*)(GimpProjectable *, gint *, gint *))gimp_item_get_offset;
...
private
  ...
  gimp_container_add_handler(
      private->children, "effective-mode-changed",
      G_CALLBACK(gimp_group_layer_child_effective_mode_changed), group);
  gimp_container_add_handler(
      private->children, "excludes-backdrop-changed",
      G_CALLBACK(gimp_group_layer_child_excludes_backdrop_changed), group);

  g_signal_connect(private->children, "update",
                   G_CALLBACK(gimp_group_layer_stack_update), group);

private
  ->projection = gimp_projection_new(GIMP_PROJECTABLE(group));
  gimp_projection_set_priority(private->projection, 1);

  g_signal_connect(private->projection, "update",
                   G_CALLBACK(gimp_group_layer_proj_update), group);
}
...
static void gimp_group_layer_finalize(GObject *object) {
  ...
  if (private->children) {
    g_signal_handlers_disconnect_by_func(private->children,
                                         gimp_group_layer_child_add, object);
    g_signal_handlers_disconnect_by_func(private->children,
                                         gimp_group_layer_child_remove, object);
    g_signal_handlers_disconnect_by_func(private->children,
                                         gimp_group_layer_stack_update, object);
...
static GimpItem *gimp_group_layer_duplicate(GimpItem *item, GType new_type) {
  ...
  if (GIMP_IS_GROUP_LAYER(new_item)) {
    ...
    for (list =
             gimp_item_stack_get_item_iter(GIMP_ITEM_STACK(private->children));
         list; list = g_list_next(list)) {
      GimpItem *child = list->data;
      GimpItem *new_child;
      GimpLayerMask *mask;
      ...
      gimp_object_set_name(GIMP_OBJECT(new_child), gimp_object_get_name(child));
      ...
      if (mask) {
        ...
        gimp_object_set_name(GIMP_OBJECT(new_mask), gimp_object_get_name(mask));
      }

      gimp_viewable_set_parent(GIMP_VIEWABLE(new_child),
                               GIMP_VIEWABLE(new_group));

      gimp_container_insert(new_private->children, GIMP_OBJECT(new_child),
                            position++);
    }
...
static void gimp_group_layer_resize(GimpItem *item, GimpContext *context,
  ...
  while (list) {
    ...
    if (gimp_rectangle_intersect(
            x, y, new_width, new_height, gimp_item_get_offset_x(child),
            gimp_item_get_offset_y(child), gimp_item_get_width(child),
            gimp_item_get_height(child), &child_x, &child_y, &child_width,
            &child_height)) {
      gint child_offset_x = gimp_item_get_offset_x(child) - child_x;
      gint child_offset_y = gimp_item_get_offset_y(child) - child_y;

      gimp_item_resize(child, context, fill_type, child_width, child_height,
                       child_offset_x, child_offset_y);
    } else if (gimp_item_is_attached(item)) {
      gimp_image_remove_layer(gimp_item_get_image(item), GIMP_LAYER(child),
                              TRUE, NULL);
    } else {
...
gimp_group_layer_estimate_memsize(GimpDrawable *drawable,
  ...
  for (list = gimp_item_stack_get_item_iter(GIMP_ITEM_STACK(private->children));
    ...
    child_width = (gimp_item_get_width(GIMP_ITEM(child)) * width /
                   gimp_item_get_width(GIMP_ITEM(drawable)));
    child_height = (gimp_item_get_height(GIMP_ITEM(child)) * height /
                    gimp_item_get_height(GIMP_ITEM(drawable)));

    memsize += gimp_drawable_estimate_memsize(child, component_type,
                                              child_width, child_height);
  }

  base_type = gimp_drawable_get_base_type(drawable);

  memsize += gimp_projection_estimate_memsize(base_type, component_type, width,
                                              height);
...
private
  ...
  if (private->offset_node)
    gegl_node_set(private->offset_node, "x", (gdouble)-x, "y", (gdouble)-y,
                  NULL);
...
static void gimp_group_layer_scale(GimpLayer *layer, gint new_width,
  ...
  while (list) {
    ...
    if (!gimp_item_scale_by_factors_with_origin(
            child, width_factor, height_factor, old_offset_x, old_offset_y,
            new_offset_x, new_offset_y, interpolation_type, progress)) {
...
static void gimp_group_layer_flip(GimpLayer *layer, GimpContext *context,
  ...
  for (list = gimp_item_stack_get_item_iter(GIMP_ITEM_STACK(private->children));
    ...
    gimp_item_flip(child, context, flip_type, axis, clip_result);
  }
...
static void gimp_group_layer_rotate(GimpLayer *layer, GimpContext *context,
  ...
  for (list = gimp_item_stack_get_item_iter(GIMP_ITEM_STACK(private->children));
    ...
    gimp_item_rotate(child, context, rotate_type, center_x, center_y,
                     clip_result);
  }
...
static void gimp_group_layer_transform(GimpLayer *layer, GimpContext *context,
  ...
  for (list = gimp_item_stack_get_item_iter(GIMP_ITEM_STACK(private->children));
    ...
    gimp_item_transform(child, context, matrix, direction, interpolation_type,
...
static const Babl *get_projection_format(GimpProjectable *projectable,
                                         GimpImageBaseType base_type,
...
static void gimp_group_layer_convert_type(
   ...
   */
private
  ->convert_format = get_projection_format(
      GIMP_PROJECTABLE(group), gimp_babl_format_get_base_type(new_format),
  ...
  gimp_drawable_set_buffer_full(GIMP_DRAWABLE(group), FALSE, NULL, buffer, NULL,
                                TRUE);
...
private
  ...
  if (gegl_node_has_pad(private->parent_source_node, "input")) {
    gegl_node_connect_to(input, "output", private->parent_source_node, "input");
  }
...
gimp_group_layer_get_effective_mode(GimpLayer *layer, GimpLayerMode *mode,
  ...
  if (gimp_layer_get_mode(layer) == GIMP_LAYER_MODE_PASS_THROUGH &&
    ...
    *composite_mode = gimp_layer_get_real_composite_mode(layer);
    ...
    for (list =
      ...
      if (first) {
        gimp_layer_get_effective_mode(child, mode, blend_space, composite_space,
                                      composite_mode);
        ...
        gimp_layer_get_effective_mode(child, &other_mode, &other_blend_space,
        ...
        if (other_mode != *mode || other_blend_space != *blend_space ||
    ...
    if (reduce) {
      ...
      has_mask = gimp_layer_get_mask(layer) && gimp_layer_get_apply_mask(layer);

      if (first ||
          (gimp_layer_get_opacity(layer) == GIMP_OPACITY_OPAQUE && !has_mask) ||
          *composite_space == gimp_layer_get_real_composite_space(layer)) {
  ...
  GIMP_LAYER_CLASS(parent_class)
      ->get_effective_mode(layer, mode, blend_space, composite_space,
                           composite_mode);
}
...
private
  ...
  gegl_node_connect_to(input, "output", layers_node, "input");

  gimp_item_get_offset(GIMP_ITEM(group), &off_x, &off_y);

private
  ->offset_node =
      gegl_node_new_child(private->graph, "operation", "gegl:translate", "x",
                          (gdouble)-off_x, "y", (gdouble)-off_y, NULL);

  gegl_node_connect_to(layers_node, "output", private->offset_node, "input");

  output = gegl_node_get_output_proxy(private->graph, "output");

  gegl_node_connect_to(private->offset_node, "output", output, "input");
...
static void gimp_group_layer_end_render(GimpProjectable *projectable) {
  ...
  if (private->pass_through) {
    ...
    gegl_node_connect_to(input, "output", private->graph, "input");
  }
}
...
GimpLayer *gimp_group_layer_new(GimpImage *image) {
  ...
  group = GIMP_GROUP_LAYER(gimp_drawable_new(GIMP_TYPE_GROUP_LAYER, image, NULL,
                                             0, 0, 1, 1, format));

  gimp_layer_set_mode(GIMP_LAYER(group),
                      gimp_image_get_default_new_layer_mode(image), FALSE);
...
private
  ...
  if (push_undo) {
    undo = gimp_image_undo_push_group_layer_resume_resize(
        gimp_item_get_image(item), NULL, group);
...
private
  ...
  if (private->suspend_mask == 0) {
    ...
    private
      ->suspended_mask_buffer =
          g_object_ref(gimp_drawable_get_buffer(GIMP_DRAWABLE(mask)));

    private
      ->suspended_mask_bounds.x = gimp_item_get_offset_x(mask);
    private
      ->suspended_mask_bounds.y = gimp_item_get_offset_y(mask);
    private
      ->suspended_mask_bounds.width = gimp_item_get_width(mask);
    private
      ->suspended_mask_bounds.height = gimp_item_get_height(mask);
    } else {
    private
      ->suspended_mask_buffer = NULL;
    }
  }
...
private
  ->suspended_mask_buffer = buffer;
private
  ->suspended_mask_bounds = *bounds;
}
...
private
  ...
  if (mask && gimp_drawable_get_buffer(GIMP_DRAWABLE(mask)) !=
...
static void gimp_group_layer_update_size(GimpGroupLayer *group) {
  ...
  for (list = gimp_item_stack_get_item_iter(GIMP_ITEM_STACK(private->children));
    ...
    if (!gimp_viewable_get_size(GIMP_VIEWABLE(child), &child_bounds.width,
                                &child_bounds.height)) {
    ...
    child_bounding_box = gimp_drawable_get_bounding_box(GIMP_DRAWABLE(child));
    ...
    if (first) {
      ...
      gegl_rectangle_bounding_box(&bounds, &bounds, &child_bounds);
  ...
  if (resize_mask && gimp_layer_get_show_mask(layer)) {
    gimp_drawable_update(GIMP_DRAWABLE(group),
                         ...
                         gimp_item_get_width(mask), gimp_item_get_height(mask));
  }
  ...
  if (private->reallocate_projection || size_changed) {
     ...
     */
    if (private->offset_node)
      gegl_node_set(private->offset_node, "x", (gdouble)-bounds.x, "y",
                    (gdouble)-bounds.y, NULL);
  ...
  private
    ...
    private
       ...
       */
      gimp_projectable_bounds_changed(GIMP_PROJECTABLE(group), old_bounds.x,
                                      old_bounds.y);
    }
...
static void gimp_group_layer_update_mask_size(GimpGroupLayer *group) {
  ...
  intersect = gimp_rectangle_intersect(
      bounds.x, bounds.y, bounds.width, bounds.height, mask_bounds.x,
      mask_bounds.y, mask_bounds.width, mask_bounds.height, &copy_bounds.x,
      &copy_bounds.y, &copy_bounds.width, &copy_bounds.height);

  if (intersect) {
    gimp_gegl_buffer_copy(
        mask_buffer,
        GEGL_RECTANGLE(copy_bounds.x - mask_bounds.x,
                       copy_bounds.y - mask_bounds.y, copy_bounds.width,
                       copy_bounds.height),
        GEGL_ABYSS_NONE, buffer,
        GEGL_RECTANGLE(copy_bounds.x - bounds.x, copy_bounds.y - bounds.y,
                       copy_bounds.width, copy_bounds.height));
  }

  gimp_drawable_set_buffer_full(GIMP_DRAWABLE(mask), FALSE, NULL, buffer,
                                &bounds, TRUE);
...
static void gimp_group_layer_update_source_node(GimpGroupLayer *group) {
  ...
  if (private->pass_through) {
    gegl_node_connect_to(input, "output", private->graph, "input");
    gegl_node_connect_to(private->graph, "output", output, "input");
  } else {
    gegl_node_disconnect(private->graph, "input");

    gegl_node_connect_to(private->parent_source_node, "output", output,
                         "input");
  }
}
...
static void gimp_group_layer_update_mode_node(GimpGroupLayer *group) {
  ...
  if (private->pass_through &&
    ...
    gegl_node_connect_to(input, "output", mode_node, "input");
  }
}
...
static void gimp_group_layer_stack_update(GimpDrawableStack *stack, gint x,
  ...
  if (!private->direct_update) {
     ...
     */
    gimp_projectable_invalidate(GIMP_PROJECTABLE(group), x, y, width, height);
  ...
  if (private->direct_update || private->pass_through) {
    ...
    gimp_drawable_update(
        GIMP_DRAWABLE(group), x - gimp_item_get_offset_x(GIMP_ITEM(group)),
        y - gimp_item_get_offset_y(GIMP_ITEM(group)), width, height);
  }
}
