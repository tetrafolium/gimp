...
GList *gimp_brush_pipe_load(GimpContext *context, GFile *file,
  ...
  if (buffer->len > 0 && buffer->len < 1024) {
    gchar *utf8 = gimp_any_to_utf8(
        buffer->str, buffer->len, _("Invalid UTF-8 string in brush file '%s'."),
        gimp_file_get_utf8_name(file));

    pipe = g_object_new(GIMP_TYPE_BRUSH_PIPE, "name", utf8, "mime-type",
                        "image/x-gimp-gih", NULL);
  ...
  while (pipe->n_brushes < n_brushes) {
    pipe->brushes[pipe->n_brushes] =
        gimp_brush_load_brush(context, file, input, error);
