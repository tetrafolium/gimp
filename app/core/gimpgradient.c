...
static gint64 gimp_gradient_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_gradient_get_preview_size(GimpViewable *viewable, gint size,
                                           gboolean popup, gboolean dot_for_dot,
                                           gint *width, gint *height);
static gboolean gimp_gradient_get_popup_size(GimpViewable *viewable, gint width,
...
static void gimp_gradient_copy(GimpData *data, GimpData *src_data);
static gint gimp_gradient_compare(GimpData *data1, GimpData *data2);
...
gimp_gradient_calc_sphere_increasing_factor(gdouble middle, gdouble pos);
static inline gdouble
gimp_gradient_calc_sphere_decreasing_factor(gdouble middle, gdouble pos);
...
static void gimp_gradient_class_init(GimpGradientClass *klass) {
  ...
  fish_srgb_to_linear_rgb =
      babl_fish(babl_format("R'G'B' double"), babl_format("RGB double"));
  fish_linear_rgb_to_srgb =
      babl_fish(babl_format("RGB double"), babl_format("R'G'B' double"));
  fish_srgb_to_cie_lab =
      babl_fish(babl_format("R'G'B' double"), babl_format("CIE Lab double"));
  fish_cie_lab_to_srgb =
      babl_fish(babl_format("CIE Lab double"), babl_format("R'G'B' double"));
}
...
static GimpTempBuf *gimp_gradient_get_new_preview(GimpViewable *viewable,
  ...
  for (x = 0; x < width; x++) {
    seg =
        gimp_gradient_get_color_at(gradient, context, seg, cur_x, FALSE,
                                   GIMP_GRADIENT_BLEND_RGB_PERCEPTUAL, &color);
...
GimpData *gimp_gradient_new(GimpContext *context, const gchar *name) {
  GimpGradient *gradient;

  g_return_val_if_fail(name != NULL, NULL);
  g_return_val_if_fail(*name != '\0', NULL);

  gradient = g_object_new(GIMP_TYPE_GRADIENT, "name", name, NULL);
 ...
 **/
GimpGradientSegment *gimp_gradient_get_color_at(
    GimpGradient *gradient, GimpContext *context, GimpGradientSegment *seg,
    gdouble pos, gboolean reverse,
    GimpGradientBlendColorSpace blend_color_space, GimpRGB *color) {
  gdouble factor = 0.0;
  gdouble seg_len;
  gdouble middle;
  GimpRGB left_color;
  ...
  if (context) {
    gimp_gradient_segment_get_left_flat_color(gradient, context, seg,
                                              &left_color);

    gimp_gradient_segment_get_right_flat_color(gradient, context, seg,
                                               &right_color);
  } else {
  ...
  if (seg->color == GIMP_GRADIENT_SEGMENT_RGB) {
    ...
    case GIMP_GRADIENT_BLEND_CIE_LAB:
      babl_process(fish_srgb_to_cie_lab, &left_color, &left_color, 1);
      babl_process(fish_srgb_to_cie_lab, &right_color, &right_color, 1);
      break;

    case GIMP_GRADIENT_BLEND_RGB_LINEAR:
      babl_process(fish_srgb_to_linear_rgb, &left_color, &left_color, 1);
      babl_process(fish_srgb_to_linear_rgb, &right_color, &right_color, 1);
    ...
    switch (blend_color_space) {
    case GIMP_GRADIENT_BLEND_CIE_LAB:
      babl_process(fish_cie_lab_to_srgb, &rgb, &rgb, 1);
      break;

    case GIMP_GRADIENT_BLEND_RGB_LINEAR:
      babl_process(fish_linear_rgb_to_srgb, &rgb, &rgb, 1);
    ...
    default:
      g_warning("%s: Unknown coloring mode %d", G_STRFUNC, (gint)seg->color);
      break;
    }
...
GimpGradientSegment *gimp_gradient_get_segment_at(GimpGradient *gradient,
                                                  gdouble pos) {
  g_return_val_if_fail(GIMP_IS_GRADIENT(gradient), NULL);
...
void gimp_gradient_split_at(GimpGradient *gradient, GimpContext *context,
                            GimpGradientSegment *seg, gdouble pos,
  ...
  /* Get color at pos */
  gimp_gradient_get_color_at(gradient, context, seg, pos, FALSE,
                             blend_color_space, &color);
...
void gimp_gradient_segment_split_midpoint(
    GimpGradient *gradient, GimpContext *context, GimpGradientSegment *lseg,
    GimpGradientBlendColorSpace blend_color_space, GimpGradientSegment **newl,
    GimpGradientSegment **newr) {
  g_return_if_fail(GIMP_IS_GRADIENT(gradient));
  g_return_if_fail(GIMP_IS_CONTEXT(context));
...
void gimp_gradient_segment_split_uniform(
    GimpGradient *gradient, GimpContext *context, GimpGradientSegment *lseg,
    gint parts, GimpGradientBlendColorSpace blend_color_space,
    GimpGradientSegment **newl, GimpGradientSegment **newr) {
  ...
  for (i = 0; i < parts; i++) {
    ...
    gimp_gradient_get_color_at(gradient, context, lseg, seg->left, FALSE,
                               blend_color_space, &seg->left_color);
    gimp_gradient_get_color_at(gradient, context, lseg, seg->right, FALSE,
                               blend_color_space, &seg->right_color);
...
void gimp_gradient_segment_set_left_color(GimpGradient *gradient,
  ...
  gimp_gradient_segment_range_blend(gradient, seg, seg, color,
                                    &seg->right_color, TRUE, TRUE);
...
void gimp_gradient_segment_set_right_color(GimpGradient *gradient,
  ...
  gimp_gradient_segment_range_blend(gradient, seg, seg, &seg->left_color, color,
                                    TRUE, TRUE);
...
void gimp_gradient_segment_set_left_color_type(GimpGradient *gradient,
                                               GimpGradientSegment *seg,
                                               GimpGradientColor color_type) {
  g_return_if_fail(GIMP_IS_GRADIENT(gradient));
  g_return_if_fail(seg != NULL);
...
void gimp_gradient_segment_set_right_color_type(GimpGradient *gradient,
                                                GimpGradientSegment *seg,
                                                GimpGradientColor color_type) {
  g_return_if_fail(GIMP_IS_GRADIENT(gradient));
  g_return_if_fail(seg != NULL);
...
void gimp_gradient_segment_get_left_flat_color(GimpGradient *gradient,
                                               GimpContext *context,
                                               GimpGradientSegment *seg,
                                               GimpRGB *color) {
  g_return_if_fail(GIMP_IS_GRADIENT(gradient));
  ...
  gimp_gradient_get_flat_color(context, &seg->left_color, seg->left_color_type,
                               color);
}

void gimp_gradient_segment_get_right_flat_color(GimpGradient *gradient,
                                                GimpContext *context,
                                                GimpGradientSegment *seg,
                                                GimpRGB *color) {
  g_return_if_fail(GIMP_IS_GRADIENT(gradient));
  ...
  gimp_gradient_get_flat_color(context, &seg->right_color,
                               seg->right_color_type, color);
}
...
gdouble gimp_gradient_segment_set_left_pos(GimpGradient *gradient,
  ...
  if (seg->prev == NULL) {
    ...
    final_pos = seg->prev->right = seg->left =
        CLAMP(pos, seg->prev->middle + EPSILON, seg->middle - EPSILON);
...
gdouble gimp_gradient_segment_set_right_pos(GimpGradient *gradient,
  ...
  if (seg->next == NULL) {
    ...
    final_pos = seg->next->left = seg->right =
        CLAMP(pos, seg->middle + EPSILON, seg->next->middle - EPSILON);
...
gdouble gimp_gradient_segment_set_middle_pos(GimpGradient *gradient,
  ...
  final_pos = seg->middle =
      CLAMP(pos, seg->left + EPSILON, seg->right - EPSILON);
...
gint gimp_gradient_segment_range_get_n_segments(GimpGradient *gradient,
...
void gimp_gradient_segment_range_compress(GimpGradient *gradient,
  ...
  if (orig_r - orig_l > EPSILON) {
    ...
    n = gimp_gradient_segment_range_get_n_segments(gradient, range_l, range_r);
...
void gimp_gradient_segment_range_set_blending_function(
    GimpGradient *gradient, GimpGradientSegment *start_seg,
    GimpGradientSegment *end_seg, GimpGradientSegmentType new_type) {
  GimpGradientSegment *seg;
...
void gimp_gradient_segment_range_set_coloring_type(
    GimpGradient *gradient, GimpGradientSegment *start_seg,
    GimpGradientSegment *end_seg, GimpGradientSegmentColor new_color) {
  GimpGradientSegment *seg;
  gboolean reached_last_segment = FALSE;
...
void gimp_gradient_segment_range_replicate(
    GimpGradient *gradient, GimpGradientSegment *start_seg,
    GimpGradientSegment *end_seg, gint replicate_times,
    GimpGradientSegment **final_start_seg,
    GimpGradientSegment **final_end_seg) {
...
void gimp_gradient_segment_range_split_midpoint(
    GimpGradient *gradient, GimpContext *context,
    GimpGradientSegment *start_seg, GimpGradientSegment *end_seg,
    GimpGradientBlendColorSpace blend_color_space,
    GimpGradientSegment **final_start_seg,
    GimpGradientSegment **final_end_seg) {
  ...
  do {
    gimp_gradient_segment_split_midpoint(gradient, context, seg,
                                         blend_color_space, &lseg, &rseg);
    seg = rseg->next;
  } while (lseg != end_seg);
...
void gimp_gradient_segment_range_split_uniform(
    GimpGradient *gradient, GimpContext *context,
    GimpGradientSegment *start_seg, GimpGradientSegment *end_seg, gint parts,
    GimpGradientBlendColorSpace blend_color_space,
    GimpGradientSegment **final_start_seg,
    GimpGradientSegment **final_end_seg) {
  ...
  do {
    ...
    gimp_gradient_segment_split_uniform(gradient, context, seg, parts,
                                        blend_color_space, &lseg, &rseg);
...
void gimp_gradient_segment_range_delete(GimpGradient *gradient,
  ...
  join = (start_seg->left + end_seg->right) / 2.0;
  ...
  if (lseg != NULL)
    gimp_gradient_segment_range_compress(gradient, lseg, lseg, lseg->left,
                                         join);

  if (rseg != NULL)
    gimp_gradient_segment_range_compress(gradient, rseg, rseg, join,
                                         rseg->right);
...
gdouble gimp_gradient_segment_range_move(GimpGradient *gradient,
  ...
  if (!is_first) {
    if (!control_compress)
      ...
      gimp_gradient_segment_range_compress(gradient, range_l->prev,
                                           range_l->prev, range_l->prev->left,
                                           range_l->left);
  }
  ...
  if (!is_last) {
    if (!control_compress)
      ...
      gimp_gradient_segment_range_compress(gradient, range_r->next,
                                           range_r->next, range_r->right,
                                           range_r->next->right);
  }
