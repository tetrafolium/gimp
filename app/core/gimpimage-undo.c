...
private
  ...
  gimp_image_undo_pop_stack(image, private->undo_stack, private->redo_stack,
                            GIMP_UNDO_MODE_UNDO);
...
private
  ...
  gimp_image_undo_pop_stack(image, private->redo_stack, private->undo_stack,
                            GIMP_UNDO_MODE_REDO);
...
gboolean gimp_image_undo_group_start(GimpImage *image, GimpUndoType undo_type,
                                     const gchar *name) {
  GimpImagePrivate *private;
...
GimpUndo *gimp_image_undo_push(GimpImage *image, GType object_type,
                               GimpUndoType undo_type, const gchar *name,
                               GimpDirtyMask dirty_mask, ...) {
  GimpImagePrivate *private;
  gint n_properties = 0;
  gchar **names = NULL;
...
private
  ...
  names = gimp_properties_append(object_type, &n_properties, names, &values,
                                 "name", name, "image", image, "undo-type",
                                 undo_type, "dirty-mask", dirty_mask, NULL);
  ...
  undo = (GimpUndo *)g_object_new_with_properties(
      object_type, n_properties, (const gchar **)names, (const GValue *)values);
...
GimpUndo *gimp_image_undo_can_compress(GimpImage *image, GType object_type,
  ...
  if (undo) {
    ...
    if (accum.size_changed)
      gimp_image_size_changed_detailed(
          image, accum.previous_origin_x, accum.previous_origin_y,
          accum.previous_width, accum.previous_height);
...
static void gimp_image_undo_free_space(GimpImage *image) {
  ...
  while ((gimp_object_get_memsize(GIMP_OBJECT(container), NULL) > undo_size) ||
    ...
    GimpUndo *freed =
        gimp_undo_stack_free_bottom(private->undo_stack, GIMP_UNDO_MODE_UNDO);
...
static void gimp_image_undo_free_redo(GimpImage *image) {
  ...
  while (gimp_container_get_n_children(container) > 0) {
    GimpUndo *freed =
        gimp_undo_stack_free_bottom(private->redo_stack, GIMP_UNDO_MODE_REDO);
