...
static void gimp_tree_handler_class_init(GimpTreeHandlerClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  object_class->dispose = gimp_tree_handler_dispose;
}

static void gimp_tree_handler_init(GimpTreeHandler *handler) {}
...
static void gimp_tree_handler_dispose(GObject *object) {
  ...
  if (handler->container) {
    g_signal_handlers_disconnect_by_func(handler->container,
                                         gimp_tree_handler_freeze, handler);
    g_signal_handlers_disconnect_by_func(handler->container,
                                         gimp_tree_handler_thaw, handler);
...
GimpTreeHandler *gimp_tree_handler_connect(GimpContainer *container,
  ...
  g_signal_connect_object(container, "freeze",
                          G_CALLBACK(gimp_tree_handler_freeze), handler,
                          G_CONNECT_SWAPPED);
  g_signal_connect_object(container, "thaw", G_CALLBACK(gimp_tree_handler_thaw),
                          handler, G_CONNECT_SWAPPED);
...
static void gimp_tree_handler_add_container(GimpTreeHandler *handler,
                                            GimpContainer *container) {
  gimp_container_foreach(container, (GFunc)gimp_tree_handler_add_foreach,
                         handler);

  g_signal_connect_object(container, "add", G_CALLBACK(gimp_tree_handler_add),
                          handler, G_CONNECT_SWAPPED);
  g_signal_connect_object(container, "remove",
                          G_CALLBACK(gimp_tree_handler_remove), handler,
                          G_CONNECT_SWAPPED);
}
...
static void gimp_tree_handler_add(GimpTreeHandler *handler,
  ...
  g_signal_connect(viewable, handler->signal_name, handler->callback,
                   handler->user_data);
...
static void gimp_tree_handler_remove_container(GimpTreeHandler *handler,
                                               GimpContainer *container) {
  g_signal_handlers_disconnect_by_func(container, gimp_tree_handler_add,
                                       handler);
  g_signal_handlers_disconnect_by_func(container, gimp_tree_handler_remove,
                                       handler);

  gimp_container_foreach(container, (GFunc)gimp_tree_handler_remove_foreach,
                         handler);
}
...
static void gimp_tree_handler_remove(GimpTreeHandler *handler,
  ...
  g_signal_handlers_disconnect_by_func(viewable, handler->callback,
                                       handler->user_data);
}
