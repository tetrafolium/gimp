...
void gimp_drawable_fill(GimpDrawable *drawable, GimpContext *context,
  ...
  gimp_drawable_fill_buffer(drawable, gimp_drawable_get_buffer(drawable),
                            &color, pattern, 0, 0);

  gimp_drawable_update(drawable, 0, 0, -1, -1);
}

void gimp_drawable_fill_buffer(GimpDrawable *drawable, GeglBuffer *buffer,
                               const GimpRGB *color, GimpPattern *pattern,
                               gint pattern_offset_x, gint pattern_offset_y) {
  g_return_if_fail(GIMP_IS_DRAWABLE(drawable));
  ...
  if (pattern) {
    ...
    src_profile =
        gimp_babl_format_get_color_profile(gegl_buffer_get_format(src_buffer));
    dest_profile =
    ...
    gegl_buffer_set_pattern(buffer, NULL, dest_buffer, pattern_offset_x,
                            pattern_offset_y);
    ...
    gimp_pickable_srgb_to_image_color(GIMP_PICKABLE(drawable), color,
                                      &image_color);
    ...
    gegl_color =
        gimp_gegl_color_new(&image_color, gimp_drawable_get_space(drawable));
...
void gimp_drawable_fill_boundary(GimpDrawable *drawable,
  ...
  if (scan_convert) {
    gimp_drawable_fill_scan_convert(drawable, options, scan_convert, push_undo);
...
void gimp_drawable_fill_scan_convert(GimpDrawable *drawable,
   ...
   */
  mask_buffer =
      gegl_buffer_new(GEGL_RECTANGLE(0, 0, w, h), babl_format("Y u8"));

  /* render the stroke into it */
  gimp_item_get_offset(GIMP_ITEM(drawable), &off_x, &off_y);

  gimp_scan_convert_render(scan_convert, mask_buffer, x + off_x, y + off_y,
                           gimp_fill_options_get_antialias(options));

  buffer = gimp_fill_options_create_buffer(options, drawable,
                                           GEGL_RECTANGLE(0, 0, w, h), -x, -y);

  gimp_gegl_apply_opacity(buffer, NULL, NULL, buffer, mask_buffer, 0, 0, 1.0);
  g_object_unref(mask_buffer);

  /* Apply to drawable */
  gimp_drawable_apply_buffer(
      drawable, buffer, GEGL_RECTANGLE(0, 0, w, h), push_undo,
      C_("undo-type", "Render Stroke"), gimp_context_get_opacity(context),
      gimp_context_get_paint_mode(context), GIMP_LAYER_COLOR_SPACE_AUTO,
