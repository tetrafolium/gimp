...
GeglBuffer *gimp_drawable_get_shadow_buffer(GimpDrawable *drawable) {
  ...
  drawable->private->shadow =
      gegl_buffer_new(GEGL_RECTANGLE(0, 0, width, height), format);
...
void gimp_drawable_merge_shadow_buffer(GimpDrawable *drawable,
  ...
  if (gimp_item_mask_intersect(GIMP_ITEM(drawable), &x, &y, &width, &height)) {
    ...
    gimp_drawable_apply_buffer(
        drawable, buffer, GEGL_RECTANGLE(x, y, width, height), push_undo,
        undo_desc, GIMP_OPACITY_OPAQUE, GIMP_LAYER_MODE_REPLACE,
        GIMP_LAYER_COLOR_SPACE_AUTO, GIMP_LAYER_COLOR_SPACE_AUTO,
        GIMP_LAYER_COMPOSITE_AUTO, NULL, x, y);
