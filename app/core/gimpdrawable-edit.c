...
static gboolean gimp_drawable_edit_can_fill_direct(GimpDrawable *drawable,
  ...
  if (gimp_channel_is_empty(gimp_image_get_mask(image)) &&
      opacity == GIMP_OPACITY_OPAQUE && affect == GIMP_COMPONENT_MASK_ALL &&
...
static void gimp_drawable_edit_fill_direct(GimpDrawable *drawable,
  ...
  gimp_drawable_push_undo(drawable, undo_desc, NULL, 0, 0, width, height);
...
void gimp_drawable_edit_clear(GimpDrawable *drawable, GimpContext *context) {
  ...
  if (gimp_drawable_has_alpha(drawable))
    gimp_fill_options_set_by_fill_type(options, context, GIMP_FILL_TRANSPARENT,
                                       NULL);
  else
    gimp_fill_options_set_by_fill_type(options, context, GIMP_FILL_BACKGROUND,
                                       NULL);
...
void gimp_drawable_edit_fill(GimpDrawable *drawable, GimpFillOptions *options,
  ...
  if (gimp_drawable_edit_can_fill_direct(drawable, options)) {
    ...
    operation = gegl_node_new_child(
        NULL, "operation", "gimp:fill-source", "options", options, "drawable",
        drawable, "pattern-offset-x", -x, "pattern-offset-y", -y, NULL);

    filter = gimp_drawable_filter_new(drawable, undo_desc, operation, NULL);

    gimp_drawable_filter_set_opacity(filter, opacity);
    gimp_drawable_filter_set_mode(filter, mode, GIMP_LAYER_COLOR_SPACE_AUTO,
                                  GIMP_LAYER_COLOR_SPACE_AUTO, composite_mode);
