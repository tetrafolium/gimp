...
enum { PROP_0, PROP_PDB, PROP_CONTEXT, PROP_CALLBACK_NAME };
...
static void gimp_pdb_progress_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
...
GType gimp_pdb_progress_get_type(void) {
  ...
  if (!type) {
    ...
    type = g_type_register_static(G_TYPE_OBJECT, "GimpPdbProgress", &info, 0);

    g_type_add_interface_static(type, GIMP_TYPE_PROGRESS, &progress_iface_info);
  }
...
static void gimp_pdb_progress_class_init(GimpPdbProgressClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_PDB,
      g_param_spec_object("pdb", NULL, NULL, GIMP_TYPE_PDB,
  ...
  g_object_class_install_property(
      object_class, PROP_CALLBACK_NAME,
      g_param_spec_string("callback-name", NULL, NULL, NULL,
                          GIMP_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));
}
...
static gdouble gimp_pdb_progress_run_callback(GimpPdbProgress *progress,
  ...
  if (progress->callback_name && !progress->callback_busy) {
    ...
    return_vals = gimp_pdb_execute_procedure_by_name(
        progress->pdb, progress->context, NULL, NULL, progress->callback_name,
        GIMP_TYPE_PROGRESS_COMMAND, command, G_TYPE_STRING, text, G_TYPE_DOUBLE,
        value, G_TYPE_NONE);
...
static GimpProgress *gimp_pdb_progress_progress_start(GimpProgress *progress,
  ...
  if (!pdb_progress->active) {
    gimp_pdb_progress_run_callback(pdb_progress, GIMP_PROGRESS_COMMAND_START,
                                   message, 0.0);
...
static void gimp_pdb_progress_progress_end(GimpProgress *progress) {
  ...
  if (pdb_progress->active) {
    gimp_pdb_progress_run_callback(pdb_progress, GIMP_PROGRESS_COMMAND_END,
                                   NULL, 0.0);
...
static void gimp_pdb_progress_progress_set_text(GimpProgress *progress,
  ...
  if (pdb_progress->active)
    gimp_pdb_progress_run_callback(pdb_progress, GIMP_PROGRESS_COMMAND_SET_TEXT,
                                   message, 0.0);
}
...
static void gimp_pdb_progress_progress_set_value(GimpProgress *progress,
  ...
  if (pdb_progress->active) {
    gimp_pdb_progress_run_callback(
        pdb_progress, GIMP_PROGRESS_COMMAND_SET_VALUE, NULL, percentage);
...
static void gimp_pdb_progress_progress_pulse(GimpProgress *progress) {
  ...
  if (pdb_progress->active)
    gimp_pdb_progress_run_callback(pdb_progress, GIMP_PROGRESS_COMMAND_PULSE,
                                   NULL, 0.0);
}
...
gimp_pdb_progress_progress_get_window_id(GimpProgress *progress) {
  ...
  return (guint32)gimp_pdb_progress_run_callback(
      pdb_progress, GIMP_PROGRESS_COMMAND_GET_WINDOW, NULL, 0.0);
}
