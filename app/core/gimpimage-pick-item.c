...
GimpLayer *gimp_image_pick_layer(GimpImage *image, gint x, gint y,
  ...
  while (tries) {
    for (list = all_layers; list; list = g_list_next(list)) {
      ...
      if (gimp_pickable_get_opacity_at(GIMP_PICKABLE(layer), x - off_x,
                                       y - off_y) > 0.25) {
...
GimpLayer *gimp_image_pick_layer_by_bounds(GimpImage *image, gint x, gint y) {
  GList *all_layers;
  GList *list;
...
GimpTextLayer *gimp_image_pick_text_layer(GimpImage *image, gint x, gint y) {
  GList *all_layers;
  GList *list;
  ...
  for (list = all_layers; list; list = g_list_next(list)) {
    ...
    if (GIMP_IS_TEXT_LAYER(layer) && x >= off_x && y >= off_y &&
    ...
    } else if (gimp_pickable_get_opacity_at(GIMP_PICKABLE(layer), x - off_x,
                                            y - off_y) > 0.25) {
...
GimpVectors *gimp_image_pick_vectors(GimpImage *image, gdouble x, gdouble y,
                                     gdouble epsilon_x, gdouble epsilon_y) {
  ...
  for (list = all_vectors; list; list = g_list_next(list)) {
    ...
    if (gimp_item_is_visible(GIMP_ITEM(vectors))) {
      ...
      while ((stroke = gimp_vectors_stroke_get_next(vectors, stroke))) {
        ...
        dist = gimp_stroke_nearest_point_get(stroke, &coords, 1.0, NULL, NULL,
                                             NULL, NULL);

        if (dist >= 0.0 && dist < MIN(epsilon_y, mindist)) {
          mindist = dist;
          ret = vectors;
        }
      }
...
static GimpGuide *
gimp_image_pick_guide_internal(GimpImage *image, gdouble x, gdouble y,
                               gdouble epsilon_x, gdouble epsilon_y,
  ...
  for (list = GIMP_IMAGE_GET_PRIVATE(image)->guides; list;
...
GimpGuide *gimp_image_pick_guide(GimpImage *image, gdouble x, gdouble y,
                                 gdouble epsilon_x, gdouble epsilon_y) {
  g_return_val_if_fail(GIMP_IS_IMAGE(image), NULL);
...
GList *gimp_image_pick_guides(GimpImage *image, gdouble x, gdouble y,
                              gdouble epsilon_x, gdouble epsilon_y) {
  GimpGuide *guide;
  GList *result = NULL;
...
GimpSamplePoint *gimp_image_pick_sample_point(GimpImage *image, gdouble x,
                                              gdouble y, gdouble epsilon_x,
                                              gdouble epsilon_y) {
  GList *list;
  ...
  if (x < 0 || x >= gimp_image_get_width(image) || y < 0 ||
      y >= gimp_image_get_height(image)) {
    return NULL;
  }

  for (list = GIMP_IMAGE_GET_PRIVATE(image)->sample_points; list;
       list = g_list_next(list)) {
    ...
    dist = hypot((sp_x + 0.5) - x, (sp_y + 0.5) - y);
    if (dist < MIN(epsilon_y, mindist)) {
      mindist = dist;
      ret = sample_point;
    }
