...
gboolean gimp_spawn_async(gchar **argv, gchar **envp, GSpawnFlags flags,
                          GPid *child_pid, GError **error) {
  g_return_val_if_fail(argv != NULL, FALSE);
  g_return_val_if_fail(argv[0] != NULL, FALSE);

#ifdef HAVE_VFORK
  if (flags == (G_SPAWN_LEAVE_DESCRIPTORS_OPEN | G_SPAWN_DO_NOT_REAP_CHILD |
    ...
    if (pid < 0) {
      ...
      if (result) {
        ...
        g_set_error(error, G_SPAWN_ERROR, exec_err_to_g_error(status),
                    _("Failed to execute child process “%s” (%s)"), argv[0],
                    g_strerror(status));
