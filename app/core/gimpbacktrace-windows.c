...
static void gimp_backtrace_set_thread_name(DWORD tid, const gchar *name);
...
BOOL WINAPI (*gimp_backtrace_SymFromAddr)(HANDLE hProcess, DWORD64 Address,
...
#define EXCEPTION_SET_THREAD_NAME ((DWORD)0x406D1388)
  ...
  if (info->ExceptionRecord != NULL &&
      ...
      info->ExceptionRecord->NumberParameters * sizeof(ULONG_PTR) ==
...
#undef INIT_PROC
    ...
    options |= SYMOPT_OMAP_FIND_NEAREST | SYMOPT_DEFERRED_LOADS | SYMOPT_DEBUG;
...
GimpBacktrace *gimp_backtrace_new(gboolean include_current_thread) {
  ...
  for (i = 0; i < n_threads; i++) {
    ...
    hThread = OpenThread(THREAD_QUERY_INFORMATION | THREAD_GET_CONTEXT |
...
#error unsupported architecture
    ...
    while (thread->n_frames < MAX_N_FRAMES &&
           StackWalk64(machine_type, hProcess, hThread, &frame, &context, NULL,
...
gboolean gimp_backtrace_is_thread_running(GimpBacktrace *backtrace,
  ...
  return backtrace->threads[thread].time > backtrace->threads[thread].last_time;
}
...
guintptr gimp_backtrace_get_frame_address(GimpBacktrace *backtrace, gint thread,
  ...
  g_return_val_if_fail(
      frame >= 0 && frame < backtrace->threads[thread].n_frames, 0);
...
gboolean gimp_backtrace_get_address_info(guintptr address,
  ...
  if (hModule && GetModuleFileNameExA(hProcess, hModule, info->object_name,
  ...
  symbol_info = g_malloc(sizeof(SYMBOL_INFO) + sizeof(info->symbol_name) - 1);

  symbol_info->SizeOfStruct = sizeof(SYMBOL_INFO);
  symbol_info->MaxNameLen = sizeof(info->symbol_name);

  if (gimp_backtrace_SymFromAddr(hProcess, address, &offset, symbol_info)) {
    g_strlcpy(info->symbol_name, symbol_info->Name, sizeof(info->symbol_name));
  ...
  if (gimp_backtrace_SymGetLineFromAddr64(hProcess, address, &line_offset,
                                          &line)) {
    g_strlcpy(info->source_file, line.FileName, sizeof(info->source_file));
