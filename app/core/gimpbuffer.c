...
static gint64 gimp_buffer_get_memsize(GimpObject *object, gint64 *gui_size);

static gboolean gimp_buffer_get_size(GimpViewable *viewable, gint *width,
                                     gint *height);
static void gimp_buffer_get_preview_size(GimpViewable *viewable, gint size,
                                         gboolean is_popup,
                                         gboolean dot_for_dot,
                                         gint *popup_width, gint *popup_height);
static gboolean gimp_buffer_get_popup_size(GimpViewable *viewable, gint width,
...
static void gimp_buffer_get_preview_size(GimpViewable *viewable, gint size,
  ...
  gimp_viewable_calc_preview_size(gimp_buffer_get_width(buffer),
                                  gimp_buffer_get_height(buffer), size, size,
                                  dot_for_dot, 1.0, 1.0, width, height, NULL);
}
...
static gboolean gimp_buffer_get_popup_size(GimpViewable *viewable, gint width,
  ...
  if (buffer_width > width || buffer_height > height) {
    ...
    gimp_viewable_calc_preview_size(buffer_width, buffer_height, width * 2,
...
static GdkPixbuf *gimp_buffer_get_new_pixbuf(GimpViewable *viewable,
  ...
  if (buffer->color_profile) {
    ...
    temp_buf = gimp_temp_buf_new(width, height, gimp_buffer_get_format(buffer));

    gegl_buffer_get(buffer->buffer, GEGL_RECTANGLE(0, 0, width, height), scale,
    ...
    gimp_gegl_convert_color_profile(
        src_buf, GEGL_RECTANGLE(0, 0, width, height), buffer->color_profile,
        dest_buf, GEGL_RECTANGLE(0, 0, 0, 0), srgb_profile,
        GIMP_COLOR_RENDERING_INTENT_PERCEPTUAL, TRUE, NULL);
...
static gchar *gimp_buffer_get_description(GimpViewable *viewable,
  ...
  return g_strdup_printf("%s (%d × %d)", gimp_object_get_name(buffer),
...
GimpBuffer *gimp_buffer_new(GeglBuffer *buffer, const gchar *name,
  ...
  gimp_buffer = g_object_new(GIMP_TYPE_BUFFER, "name", name, NULL);
...
GimpBuffer *gimp_buffer_new_from_pixbuf(GdkPixbuf *pixbuf, const gchar *name,
                                        gint offset_x, gint offset_y) {
  GimpBuffer *gimp_buffer;
  GeglBuffer *buffer;
  guint8 *icc_data;
  ...
  gimp_buffer = gimp_buffer_new(buffer, name, offset_x, offset_y, FALSE);

  icc_data = gimp_pixbuf_get_icc_profile(pixbuf, &icc_len);
  if (icc_data) {
    profile = gimp_color_profile_new_from_icc_profile(icc_data, icc_len, NULL);
    g_free(icc_data);
  }
...
gboolean gimp_buffer_get_resolution(GimpBuffer *buffer, gdouble *resolution_x,
                                    gdouble *resolution_y) {
  g_return_val_if_fail(GIMP_IS_BUFFER(buffer), FALSE);

  if (buffer->resolution_x > 0.0 && buffer->resolution_y > 0.0) {
    if (resolution_x)
      *resolution_x = buffer->resolution_x;
    if (resolution_y)
      *resolution_y = buffer->resolution_y;
...
void gimp_buffer_set_unit(GimpBuffer *buffer, GimpUnit unit) {
  g_return_if_fail(GIMP_IS_BUFFER(buffer));
