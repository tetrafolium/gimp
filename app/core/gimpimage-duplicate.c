...
static void gimp_image_duplicate_floating_sel(GimpImage *image,
                                              GimpImage *new_image);
static void gimp_image_duplicate_mask(GimpImage *image, GimpImage *new_image);
static void gimp_image_duplicate_components(GimpImage *image,
                                            GimpImage *new_image);
static void gimp_image_duplicate_guides(GimpImage *image, GimpImage *new_image);
static void gimp_image_duplicate_sample_points(GimpImage *image,
                                               GimpImage *new_image);
static void gimp_image_duplicate_grid(GimpImage *image, GimpImage *new_image);
...
GimpImage *gimp_image_duplicate(GimpImage *image) {
  ...
  /*  Create a new image  */
  new_image = gimp_create_image(
      image->gimp, gimp_image_get_width(image), gimp_image_get_height(image),
      gimp_image_get_base_type(image), gimp_image_get_precision(image), FALSE);
...
static GimpItem *gimp_image_duplicate_item(GimpItem *item,
  ...
  new_item = gimp_item_convert(item, new_image, G_TYPE_FROM_INSTANCE(item));

  /*  Make sure the copied item doesn't say: "<old item> copy"  */
  gimp_object_set_name(GIMP_OBJECT(new_item), gimp_object_get_name(item));
...
static GList *gimp_image_duplicate_layers(GimpImage *image,
  ...
  for (list = selected_layers; list; list = list->next)
    selected_paths =
        g_list_prepend(selected_paths, gimp_item_get_path(list->data));

  for (list = gimp_image_get_layer_iter(image), count = 0; list;
    ...
    new_layer =
        GIMP_LAYER(gimp_image_duplicate_item(GIMP_ITEM(layer), new_image));
...
static GList *gimp_image_duplicate_channels(GimpImage *image,
  ...
  for (list = gimp_image_get_channel_iter(image), count = 0; list;
       list = g_list_next(list)) {
    GimpChannel *channel = list->data;
    GimpChannel *new_channel;

    new_channel =
        GIMP_CHANNEL(gimp_image_duplicate_item(GIMP_ITEM(channel), new_image));

    if (g_list_find(selected_channels, channel))
      new_selected_channels =
          g_list_prepend(new_selected_channels, new_channel);

    gimp_image_add_channel(new_image, new_channel, NULL, count++, FALSE);
  }
...
static GList *gimp_image_duplicate_vectors(GimpImage *image,
  ...
  for (list = gimp_image_get_vectors_iter(image), count = 0; list;
       list = g_list_next(list)) {
    GimpVectors *vectors = list->data;
    GimpVectors *new_vectors;

    new_vectors =
        GIMP_VECTORS(gimp_image_duplicate_item(GIMP_ITEM(vectors), new_image));

    if (g_list_find(selected_vectors, vectors))
      new_selected_vectors = g_list_prepend(new_selected_vectors, new_vectors);

    gimp_image_add_vectors(new_image, new_vectors, NULL, count++, FALSE);
  }
...
static void gimp_image_duplicate_floating_sel(GimpImage *image,
  ...
  if (GIMP_IS_LAYER(floating_sel_drawable)) {
    new_floating_sel = GIMP_LAYER(
        gimp_image_duplicate_item(GIMP_ITEM(floating_sel), new_image));
  } else {
     ...
     */
    new_floating_sel = GIMP_LAYER(gimp_item_duplicate(
        GIMP_ITEM(floating_sel), G_TYPE_FROM_INSTANCE(floating_sel)));
  ...
  new_floating_sel_drawable = GIMP_DRAWABLE(
      gimp_item_stack_get_item_by_path(new_item_stack, floating_sel_path));

  if (GIMP_IS_LAYER_MASK(floating_sel_drawable))
    new_floating_sel_drawable = GIMP_DRAWABLE(
        gimp_layer_get_mask(GIMP_LAYER(new_floating_sel_drawable)));
