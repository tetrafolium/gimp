...
static void gimp_curve_set_property(GObject *object, guint property_id,
                                    const GValue *value, GParamSpec *pspec);
static void gimp_curve_get_property(GObject *object, guint property_id,
                                    GValue *value, GParamSpec *pspec);

static gint64 gimp_curve_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_curve_get_preview_size(GimpViewable *viewable, gint size,
                                        gboolean popup, gboolean dot_for_dot,
                                        gint *width, gint *height);
static gboolean gimp_curve_get_popup_size(GimpViewable *viewable, gint width,
...
static void gimp_curve_data_copy(GimpData *data, GimpData *src_data);

static gboolean gimp_curve_serialize(GimpConfig *config,
                                     GimpConfigWriter *writer, gpointer data);
static gboolean gimp_curve_deserialize(GimpConfig *config, GScanner *scanner,
                                       gint nest_level, gpointer data);
static gboolean gimp_curve_equal(GimpConfig *a, GimpConfig *b);
static void _gimp_curve_reset(GimpConfig *config);
static gboolean gimp_curve_config_copy(GimpConfig *src, GimpConfig *dest,
                                       GParamFlags flags);

static void gimp_curve_calculate(GimpCurve *curve);
static void gimp_curve_plot(GimpCurve *curve, gint p1, gint p2, gint p3,
                            gint p4);
...
static void gimp_curve_class_init(GimpCurveClass *klass) {
  ...
  obj_props[PROP_CURVE_TYPE] = g_param_spec_enum(
      "curve-type", "Curve Type", "The curve type", GIMP_TYPE_CURVE_TYPE,
      GIMP_CURVE_SMOOTH, GIMP_CONFIG_PARAM_FLAGS);

  obj_props[PROP_N_POINTS] = g_param_spec_int(
      "n-points", "Number of Points", "The number of points", 0, G_MAXINT, 0,
      /* for backward compatibility */
      GIMP_CONFIG_PARAM_IGNORE | GIMP_CONFIG_PARAM_FLAGS);

  array_spec = g_param_spec_double("point", NULL, NULL, -1.0, 1.0, 0.0,
                                   GIMP_PARAM_READWRITE);
  obj_props[PROP_POINTS] = gimp_param_spec_value_array(
      "points", NULL, NULL, array_spec, GIMP_CONFIG_PARAM_FLAGS);

  array_spec =
      g_param_spec_enum("point-type", NULL, NULL, GIMP_TYPE_CURVE_POINT_TYPE,
                        GIMP_CURVE_POINT_SMOOTH, GIMP_PARAM_READWRITE);
  obj_props[PROP_POINT_TYPES] = gimp_param_spec_value_array(
      "point-types", NULL, NULL, array_spec, GIMP_CONFIG_PARAM_FLAGS);

  obj_props[PROP_N_SAMPLES] = g_param_spec_int(
      "n-samples", "Number of Samples", "The number of samples", 256, 256, 256,
      GIMP_CONFIG_PARAM_FLAGS);

  array_spec = g_param_spec_double("sample", NULL, NULL, 0.0, 1.0, 0.0,
                                   GIMP_PARAM_READWRITE);
  obj_props[PROP_SAMPLES] = gimp_param_spec_value_array(
      "samples", NULL, NULL, array_spec, GIMP_CONFIG_PARAM_FLAGS);
...
static void gimp_curve_set_property(GObject *object, guint property_id,
  ...
  case PROP_POINTS: {
    ...
    for (i = 0; i < length; i++) {
      ...
      if (n_points > 0) {
        points[n_points].x = MAX(points[n_points].x, points[n_points - 1].x);
      }
...
static void gimp_curve_data_copy(GimpData *data, GimpData *src_data) {
  ...
  gimp_config_copy(GIMP_CONFIG(src_data), GIMP_CONFIG(data), 0);
...
GimpData *gimp_curve_new(const gchar *name) {
  ...
  return g_object_new(GIMP_TYPE_CURVE, "name", name, NULL);
}
...
GimpData *gimp_curve_get_standard(void) {
  ...
  if (!standard_curve) {
    ...
    gimp_data_make_internal(standard_curve, "gimp-curve-standard");
...
void gimp_curve_reset(GimpCurve *curve, gboolean reset_type) {
  gint i;
...
void gimp_curve_set_curve_type(GimpCurve *curve, GimpCurveType curve_type) {
  g_return_if_fail(GIMP_IS_CURVE(curve));
  ...
  if (curve->curve_type != curve_type) {
    ...
    if (curve_type == GIMP_CURVE_SMOOTH) {
      ...
      for (i = 0; i < curve->n_points; i++) {
        ...
        curve->points[i].x = (gdouble)sample / (gdouble)(curve->n_samples - 1);
...
void gimp_curve_set_n_samples(GimpCurve *curve, gint n_samples) {
  g_return_if_fail(GIMP_IS_CURVE(curve));
...
gint gimp_curve_get_point_at(GimpCurve *curve, gdouble x) {
...
gint gimp_curve_get_closest_point(GimpCurve *curve, gdouble x, gdouble y,
  ...
  for (i = curve->n_points - 1; i >= 0; i--) {
    ...
    point_distance2 = SQR(x - curve->points[i].x) + SQR(y - curve->points[i].y);
...
gint gimp_curve_add_point(GimpCurve *curve, gdouble x, gdouble y) {
  GimpCurvePoint *points;
  gint point;
...
void gimp_curve_delete_point(GimpCurve *curve, gint point) {
  GimpCurvePoint *points;
...
void gimp_curve_set_point(GimpCurve *curve, gint point, gdouble x, gdouble y) {
  g_return_if_fail(GIMP_IS_CURVE(curve));
  g_return_if_fail(point >= 0 && point < curve->n_points);
...
void gimp_curve_move_point(GimpCurve *curve, gint point, gdouble y) {
  g_return_if_fail(GIMP_IS_CURVE(curve));
  g_return_if_fail(point >= 0 && point < curve->n_points);
...
void gimp_curve_get_point(GimpCurve *curve, gint point, gdouble *x,
                          gdouble *y) {
  g_return_if_fail(GIMP_IS_CURVE(curve));
  g_return_if_fail(point >= 0 && point < curve->n_points);

  if (x)
    *x = curve->points[point].x;
  if (y)
    *y = curve->points[point].y;
}

void gimp_curve_set_point_type(GimpCurve *curve, gint point,
...
GimpCurvePointType gimp_curve_get_point_type(GimpCurve *curve, gint point) {
  g_return_val_if_fail(GIMP_IS_CURVE(curve), GIMP_CURVE_POINT_SMOOTH);
  g_return_val_if_fail(point >= 0 && point < curve->n_points,
...
void gimp_curve_set_curve(GimpCurve *curve, gdouble x, gdouble y) {
  g_return_if_fail(GIMP_IS_CURVE(curve));
...
void gimp_curve_get_uchar(GimpCurve *curve, gint n_samples, guchar *samples) {
  gint i;
...
static void gimp_curve_plot(GimpCurve *curve, gint p1, gint p2, gint p3,
  ...
  for (i = 0; i <= ROUND(dx * (gdouble)(curve->n_samples - 1)); i++) {
    ...
    y = y0 * (1 - t) * (1 - t) * (1 - t) + 3 * y1 * (1 - t) * (1 - t) * t +
        3 * y2 * (1 - t) * t * t + y3 * t * t * t;
