...
static void gimp_image_undo_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_image_undo_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);

static gint64 gimp_image_undo_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_image_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
                                GimpUndoAccumulator *accum);
static void gimp_image_undo_free(GimpUndo *undo, GimpUndoMode undo_mode);

G_DEFINE_TYPE(GimpImageUndo, gimp_image_undo, GIMP_TYPE_UNDO)

#define parent_class gimp_image_undo_parent_class

static void gimp_image_undo_class_init(GimpImageUndoClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_PREVIOUS_ORIGIN_X,
      g_param_spec_int("previous-origin-x", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_PREVIOUS_ORIGIN_Y,
      g_param_spec_int("previous-origin-y", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_PREVIOUS_WIDTH,
      g_param_spec_int("previous-width", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_PREVIOUS_HEIGHT,
      g_param_spec_int("previous-height", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      object_class, PROP_PARASITE_NAME,
      g_param_spec_string("parasite-name", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_image_undo_init(GimpImageUndo *undo) {}
...
static void gimp_image_undo_constructed(GObject *object) {
  ...
  case GIMP_UNDO_IMAGE_RESOLUTION:
    gimp_image_get_resolution(image, &image_undo->xresolution,
  ...
  case GIMP_UNDO_IMAGE_COLORMAP:
    ...
    image_undo->colormap =
        g_memdup(gimp_image_get_colormap(image), GIMP_IMAGE_COLORMAP_SIZE);
    break;
...
static gint64 gimp_image_undo_get_memsize(GimpObject *object,
  ...
  memsize += gimp_object_get_memsize(GIMP_OBJECT(image_undo->grid), gui_size);
...
static void gimp_image_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
  ...
  case GIMP_UNDO_IMAGE_SIZE: {
    ...
    g_object_set(image, "width", width, "height", height, NULL);
  ...
  case GIMP_UNDO_IMAGE_RESOLUTION: {
    ...
    private
      ->xresolution = image_undo->xresolution;
    private
      ->yresolution = image_undo->yresolution;
    ...
    private
      ->resolution_unit = image_undo->resolution_unit;
  ...
  case GIMP_UNDO_IMAGE_COLORMAP: {
    ...
    if (image_undo->colormap)
      gimp_image_set_colormap(image, image_undo->colormap,
  ...
  case GIMP_UNDO_IMAGE_HIDDEN_PROFILE: {
    ...
    _gimp_image_set_hidden_profile(image, image_undo->hidden_profile, FALSE);
