...
static void gimp_brush_transform_blur(GimpTempBuf *buf, gint r);
static gint gimp_brush_transform_blur_radius(gint height, gint width,
                                             gdouble hardness);
...
void gimp_brush_real_transform_size(GimpBrush *brush, gdouble scale,
                                    gdouble aspect_ratio, gdouble angle,
                                    gboolean reflect, gint *width,
                                    gint *height) {
  ...
  gimp_brush_transform_get_scale(scale, aspect_ratio, &scale_x, &scale_y);

  source = gimp_brush_mipmap_get_mask(brush, &scale_x, &scale_y);

  gimp_brush_transform_matrix(gimp_temp_buf_get_width(source),
                              gimp_temp_buf_get_height(source), scale_x,
                              scale_y, angle, reflect, &matrix);
 ...
 */
GimpTempBuf *gimp_brush_real_transform_mask(GimpBrush *brush, gdouble scale,
                                            gdouble aspect_ratio, gdouble angle,
                                            gboolean reflect,
                                            gdouble hardness) {
  GimpTempBuf *result;
  ...
  gimp_brush_transform_matrix(src_width, src_height, scale_x, scale_y, angle,
                              reflect, &matrix);
  ...
  gimp_brush_transform_bounding_box(source, &matrix, &x, &y, &dest_width,
                                    &dest_height);
  ...
  if (hardness < 1.0) {
    ...
    gimp_brush_transform_matrix(src_width, src_height, scale_x, scale_y, 0.0,
                                FALSE, &unrotated_matrix);

    gimp_brush_transform_bounding_box(source, &unrotated_matrix, &unrotated_x,
                                      &unrotated_y, &unrotated_dest_width,
                                      &unrotated_dest_height);

    blur_radius = gimp_brush_transform_blur_radius(
        unrotated_dest_width, unrotated_dest_height, hardness);
  ...
  gimp_matrix3_transform_point(&matrix, 0.5, 0.5, &t_lx, &t_ly);
  gimp_matrix3_transform_point(&matrix, dest_width - 0.5, 0.5, &t_rx, &t_ry);
  gimp_matrix3_transform_point(&matrix, 0.5, dest_height - 0.5, &b_lx, &b_ly);
  gimp_matrix3_transform_point(&matrix, dest_width - 0.5, dest_height - 0.5,
                               &b_rx, &b_ry);
  ...
  src_walk_ux_i =
      (gint)((src_tl_to_tr_delta_x / MAX(dest_width - 1, 1)) * int_multiple);
  src_walk_uy_i =
      (gint)((src_tl_to_tr_delta_y / MAX(dest_width - 1, 1)) * int_multiple);
  src_walk_vx_i =
      (gint)((src_tl_to_bl_delta_x / MAX(dest_height - 1, 1)) * int_multiple);
  src_walk_vy_i =
      (gint)((src_tl_to_bl_delta_y / MAX(dest_height - 1, 1)) * int_multiple);
  ...
  gegl_parallel_distribute_area(
      ...
      [=](const GeglRectangle *area) {
        ...
        dest = gimp_temp_buf_get_data(result) + dest_width * area->y + area->x;

        /* initialize current position in source space to the start position
         * (tl) speed optimized, note conversion to int precision
         */
        ...
        for (v = 0; v < area->height; v++) {
          ...
          for (u = 0; u < area->width; u++) {
            ...
            } else /* reverse transformed point hits source pixel */
            {
              ...
              src_walker =
                  src + src_space_cur_pos_y * src_width + src_space_cur_pos_x;
              ...
              *dest = ((src_walker[0] * opposite_x +
                        pixel_next[0] * distance_from_true_x) *
                           opposite_y +
                       (pixel_below[0] * opposite_x +
                        pixel_below_next[0] * distance_from_true_x) *
                           distance_from_true_y) >>
                      recovery_bits;
            }
 ...
 * premultiplied by int_multiple
 */
GimpTempBuf *gimp_brush_real_transform_pixmap(GimpBrush *brush, gdouble scale,
        ...
        for (v = 0; v < area->height; v++) {
          ...
          for (u = 0; u < area->width; u++) {
            ...
            } else /* reverse transformed point hits source pixel */
            {
              ...
              src_walker = src + 3 * (src_space_cur_pos_y * src_width +
                                      src_space_cur_pos_x);
              ...
              dest[1] = ((src_walker[1] * opposite_x +
                          pixel_next[1] * distance_from_true_x) *
                             opposite_y +
                         (pixel_below[1] * opposite_x +
                          pixel_below_next[1] * distance_from_true_x) *
                             distance_from_true_y) >>
                        recovery_bits;

              dest[2] = ((src_walker[2] * opposite_x +
                          pixel_next[2] * distance_from_true_x) *
                             opposite_y +
                         (pixel_below[2] * opposite_x +
                          pixel_below_next[2] * distance_from_true_x) *
                             distance_from_true_y) >>
                        recovery_bits;
            }
...
void gimp_brush_transform_get_scale(gdouble scale, gdouble aspect_ratio,
...
void gimp_brush_transform_matrix(gdouble width, gdouble height, gdouble scale_x,
                                 gdouble scale_y, gdouble angle,
                                 gboolean reflect, GimpMatrix3 *matrix) {
...
static void gimp_brush_transform_blur(GimpTempBuf *buf, gint r) {
  ...
  gegl_parallel_distribute_range(
      height, PIXELS_PER_THREAD / width, [=](gint y0, gint height) {
        gint x;
        gint y;
        gint c;
        const guchar *d;
        ...
        for (y = 0; y < height; y++) {
          ...
          for (x = 0; x < width; x++) {
            for (c = 0; c < components; c++) {
              ...
              s->middle_sum =
                  2 * acc[c].leading_weighted_sum - acc[c].weighted_sum;
  ...
  gegl_parallel_distribute_range(
      width, PIXELS_PER_THREAD / height, [=](gint x0, gint width) {
        gint x;
        gint y;
        gint c;
        guchar *d0;
          ...
          for (y = 0; y < height; y++) {
            for (c = 0; c < components; c++) {
              if (y > 0) {
                acc[c].weighted_sum +=
                    s->weighted_sum +
                    n * (acc[c].leading_sum - acc[c].trailing_sum);
                acc[c].trailing_sum -= s->sum;

                if (y < height - r) {
                  acc[c].weighted_sum +=
                      n_r * s[stride_r].sum - s[stride_r].weighted_sum;
...
static void gimp_brush_transform_adjust_hardness_matrix(gdouble width,
  ...
  gimp_matrix3_translate(matrix, (1.0 - scale) * width / 2.0,
