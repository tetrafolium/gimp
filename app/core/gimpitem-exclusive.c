...
static void gimp_item_exclusive_get_lists(GimpItem *item, const gchar *property,
                                          GList **on, GList **off);
...
void gimp_item_toggle_exclusive_visible(GimpItem *item, GimpContext *context) {
  ...
  if (on || off || !gimp_item_is_visible(item)) {
    ...
    if (push_undo) {
      if (gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_ITEM_VISIBILITY,
...
void gimp_item_toggle_exclusive_linked(GimpItem *item, GimpContext *context) {
  ...
  for (list = gimp_item_get_container_iter(item); list;
  ...
  if (on || off || !gimp_item_get_linked(item)) {
    ...
    if (push_undo) {
      if (gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_ITEM_LINKED,
...
static GList *gimp_item_exclusive_get_ancestry(GimpItem *item) {
  ...
  for (parent = GIMP_VIEWABLE(item); parent;
