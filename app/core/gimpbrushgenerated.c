...
static void gimp_brush_generated_copy(GimpData *data, GimpData *src_data);
...
static GimpTempBuf *
gimp_brush_generated_transform_mask(GimpBrush *gbrush, gdouble scale,
                                    gdouble aspect_ratio, gdouble angle,
                                    gboolean reflect, gdouble hardness);

static GimpTempBuf *gimp_brush_generated_calc(
    GimpBrushGenerated *brush, GimpBrushGeneratedShape shape, gfloat radius,
    gint spikes, gfloat hardness, gfloat aspect_ratio, gfloat angle,
    gboolean reflect, GimpVector2 *xaxis, GimpVector2 *yaxis);
static void gimp_brush_generated_get_size(
    GimpBrushGenerated *gbrush, GimpBrushGeneratedShape shape, gfloat radius,
    gint spikes, gfloat hardness, gfloat aspect_ratio, gdouble angle_in_degrees,
    gboolean reflect, gint *width, gint *height, gdouble *_s, gdouble *_c,
    GimpVector2 *_x_axis, GimpVector2 *_y_axis);
...
static void gimp_brush_generated_class_init(GimpBrushGeneratedClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_SHAPE,
      g_param_spec_enum("shape", NULL, _("Brush Shape"),
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("radius", NULL, _("Brush Radius"), 0.1, 4000.0, 5.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_SPIKES,
      g_param_spec_int("spikes", NULL, _("Brush Spikes"), 2, 20, 2,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_HARDNESS,
      g_param_spec_double("hardness", NULL, _("Brush Hardness"), 0.0, 1.0, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_ASPECT_RATIO,
      g_param_spec_double("aspect-ratio", NULL, _("Brush Aspect Ratio"), 1.0,
                          20.0, 1.0, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_ANGLE,
      g_param_spec_double("angle", NULL, _("Brush Angle"), 0.0, 180.0, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}
...
static void gimp_brush_generated_dirty(GimpData *data) {
  ...
  gbrush->priv->mask = gimp_brush_generated_calc(
      brush, brush->shape, brush->radius, brush->spikes, brush->hardness,
      brush->aspect_ratio, brush->angle, FALSE, &gbrush->priv->x_axis,
      &gbrush->priv->y_axis);
...
static void gimp_brush_generated_transform_size(GimpBrush *gbrush,
  ...
  gimp_brush_generated_get_size(brush, brush->shape, brush->radius * scale,
                                brush->spikes, brush->hardness, ratio, angle,
                                reflect, width, height, NULL, NULL, NULL, NULL);
}
...
gimp_brush_generated_transform_mask(GimpBrush *gbrush, gdouble scale,
  ...
  if (scale == 1.0 && ratio == brush->aspect_ratio && angle == brush->angle &&
      reflect == FALSE && hardness == brush->hardness) {
    return gimp_temp_buf_copy(gimp_brush_get_mask(gbrush));
  }

  return gimp_brush_generated_calc(brush, brush->shape, brush->radius * scale,
                                   brush->spikes, hardness, ratio, angle,
                                   reflect, NULL, NULL);
}
...
/* set up lookup table */
static gfloat *gimp_brush_generated_calc_lut(gdouble radius, gdouble hardness) {
  gfloat *lookup;
  gint length;
  gint x;
  gdouble d;
...
static GimpTempBuf *gimp_brush_generated_calc(
  ...
  gimp_brush_generated_get_size(brush, shape, radius, spikes, hardness,
                                aspect_ratio, angle, reflect, &width, &height,
                                &s, &c, &x_axis, &y_axis);

  mask = gimp_temp_buf_new(width, height, babl_format("Y float"));

  half_width = width / 2;
  half_height = height / 2;

  centerp =
      (gfloat *)gimp_temp_buf_get_data(mask) + half_height * width + half_width;
...
static void gimp_brush_generated_get_size(
  ...
  if (spikes > 2) {
    ...
    half_width = half_height =
        sqrt(radius * radius + short_radius * short_radius);
    y_axis.x = s * radius;
    y_axis.y = c * radius;
  }
...
GimpData *gimp_brush_generated_new(const gchar *name,
                                   GimpBrushGeneratedShape shape, gfloat radius,
                                   gint spikes, gfloat hardness,
                                   gfloat aspect_ratio, gfloat angle) {
  GimpBrushGenerated *brush;

  g_return_val_if_fail(name != NULL, NULL);
  g_return_val_if_fail(*name != '\0', NULL);

  brush = g_object_new(GIMP_TYPE_BRUSH_GENERATED, "name", name, "mime-type",
                       "application/x-gimp-brush-generated", "spacing", 20.0,
                       "shape", shape, "radius", radius, "spikes", spikes,
                       "hardness", hardness, "aspect-ratio", aspect_ratio,
                       "angle", angle, NULL);
...
gint gimp_brush_generated_set_spikes(GimpBrushGenerated *brush, gint spikes) {
...
gfloat gimp_brush_generated_set_angle(GimpBrushGenerated *brush, gfloat angle) {
  g_return_val_if_fail(GIMP_IS_BRUSH_GENERATED(brush), -1.0);
