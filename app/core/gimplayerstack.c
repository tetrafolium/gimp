...
static void gimp_layer_stack_add(GimpContainer *container, GimpObject *object);
...
static void gimp_layer_stack_update_range(GimpLayerStack *stack, gint first,
                                          gint last);
...
static void gimp_layer_stack_class_init(GimpLayerStackClass *klass) {
...
static void gimp_layer_stack_init(GimpLayerStack *stack) {}
...
static void gimp_layer_stack_constructed(GObject *object) {
  ...
  gimp_container_add_handler(
      container, "excludes-backdrop-changed",
      G_CALLBACK(gimp_layer_stack_layer_excludes_backdrop), container);
}
...
GimpContainer *gimp_layer_stack_new(GType layer_type) {
  ...
  return g_object_new(GIMP_TYPE_LAYER_STACK, "name", g_type_name(layer_type),
                      "children-type", layer_type, "policy",
...
static void
gimp_layer_stack_update_backdrop(GimpLayerStack *stack, GimpLayer *layer,
                                 gboolean ignore_active,
                                 gboolean ignore_excludes_backdrop) {
...
static void gimp_layer_stack_update_range(GimpLayerStack *stack, gint first,
  ...
  for (iter = g_list_nth(iter, first); iter && first != last;
    ...
    if (gimp_filter_get_active(GIMP_FILTER(item))) {
      ...
      gimp_drawable_stack_update(GIMP_DRAWABLE_STACK(stack), bounding_box.x,
