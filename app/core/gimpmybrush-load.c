...
GList *gimp_mybrush_load(GimpContext *context, GFile *file, GInputStream *input,
  ...
  pixbuf = gdk_pixbuf_new_from_file_at_size(preview_filename, 48, 48, error);
  ...
  brush = g_object_new(GIMP_TYPE_MYBRUSH, "name", basename, "mime-type",
                       "image/x-gimp-myb", "icon-pixbuf", pixbuf, NULL);
  ...
  brush->priv->radius = mypaint_brush_get_base_value(
      mypaint_brush, MYPAINT_BRUSH_SETTING_RADIUS_LOGARITHMIC);

  brush->priv->opaque =
      mypaint_brush_get_base_value(mypaint_brush, MYPAINT_BRUSH_SETTING_OPAQUE);

  brush->priv->hardness = mypaint_brush_get_base_value(
      mypaint_brush, MYPAINT_BRUSH_SETTING_HARDNESS);

  brush->priv->eraser = mypaint_brush_get_base_value(
                            mypaint_brush, MYPAINT_BRUSH_SETTING_ERASER) > 0.5f;

  brush->priv->offset_by_random = mypaint_brush_get_base_value(
      mypaint_brush, MYPAINT_BRUSH_SETTING_OFFSET_BY_RANDOM);
