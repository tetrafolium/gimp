...
GimpTempBuf *gimp_temp_buf_new(gint width, gint height, const Babl *format) {
  GimpTempBuf *temp;
  gint bpp;
...
GimpTempBuf *gimp_temp_buf_copy(const GimpTempBuf *src) {
  GimpTempBuf *dest;
  ...
  memcpy(gimp_temp_buf_get_data(dest), gimp_temp_buf_get_data(src),
...
GimpTempBuf *gimp_temp_buf_scale(const GimpTempBuf *src, gint new_width,
                                 gint new_height) {
  GimpTempBuf *dest;
  ...
  dest = gimp_temp_buf_new(new_width, new_height, src->format);
  ...
  for (loop1 = 0; loop1 < new_height; loop1++) {
    for (loop2 = 0; loop2 < new_width; loop2++) {
      ...
      src_pixel = src_data + (gint)(loop2 * x_ratio) * bytes +
                  (gint)(loop1 * y_ratio) * bytes * src->width;

      dest_pixel = dest_data + (loop2 + loop1 * new_width) * bytes;
...
gint gimp_temp_buf_get_width(const GimpTempBuf *buf) { return buf->width; }

gint gimp_temp_buf_get_height(const GimpTempBuf *buf) { return buf->height; }

const Babl *gimp_temp_buf_get_format(const GimpTempBuf *buf) {
  return buf->format;
}

void gimp_temp_buf_set_format(GimpTempBuf *buf, const Babl *format) {
...
guchar *gimp_temp_buf_get_data(const GimpTempBuf *buf) { return buf->data; }

gsize gimp_temp_buf_get_data_size(const GimpTempBuf *buf) {
  return (gsize)babl_format_get_bytes_per_pixel(buf->format) * buf->width *
         buf->height;
}
...
gpointer gimp_temp_buf_lock(const GimpTempBuf *buf, const Babl *format,
                            GeglAccessMode access_mode) {
  guchar *data;
  LockData *lock_data;
  gint n_pixels;
  ...
  if (access_mode & GEGL_ACCESS_READ) {
    babl_process(babl_fish(buf->format, format), gimp_temp_buf_get_data(buf),
                 data, n_pixels);
  }
...
void gimp_temp_buf_unlock(const GimpTempBuf *buf, gconstpointer data) {
  LockData *lock_data;
  ...
  if (lock_data->access_mode & GEGL_ACCESS_WRITE) {
    babl_process(babl_fish(lock_data->format, buf->format), data,
                 gimp_temp_buf_get_data(buf), buf->width * buf->height);
  }
...
GeglBuffer *gimp_temp_buf_create_buffer(const GimpTempBuf *temp_buf) {
  GeglBuffer *buffer;

  g_return_val_if_fail(temp_buf != NULL, NULL);

  buffer = gegl_buffer_linear_new_from_data(
      gimp_temp_buf_get_data(temp_buf), temp_buf->format,
      GEGL_RECTANGLE(0, 0, temp_buf->width, temp_buf->height),
      GEGL_AUTO_ROWSTRIDE, (GDestroyNotify)gimp_temp_buf_unref,
      gimp_temp_buf_ref(temp_buf));

  g_object_set_data(G_OBJECT(buffer), "gimp-temp-buf", (GimpTempBuf *)temp_buf);

  return buffer;
}

GdkPixbuf *gimp_temp_buf_create_pixbuf(const GimpTempBuf *temp_buf) {
  GdkPixbuf *pixbuf;
  const Babl *format;
  const Babl *fish = NULL;
  const guchar *data;
  ...
  pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, babl_format_has_alpha(format), 8,
                          width, height);
