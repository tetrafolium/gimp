...
static GimpBuffer *gimp_edit_extract(GimpImage *image, GList *pickables,
                                     GimpContext *context, gboolean cut_pixels,
                                     GError **error);

/*  public functions  */

GimpObject *gimp_edit_cut(GimpImage *image, GList *drawables,
                          GimpContext *context, GError **error) {
  g_return_val_if_fail(GIMP_IS_IMAGE(image), NULL);
  g_return_val_if_fail(GIMP_IS_CONTEXT(context), NULL);
  g_return_val_if_fail(error == NULL || *error == NULL, NULL);

  if (g_list_length(drawables) == 1 && GIMP_IS_LAYER(drawables->data) &&
    ...
    g_object_set_data(G_OBJECT(clip_image), "offset-x", GINT_TO_POINTER(off_x));
    g_object_set_data(G_OBJECT(clip_image), "offset-y", GINT_TO_POINTER(off_y));
    ...
    gimp_image_remove_layer(image, GIMP_LAYER(drawables->data), TRUE, NULL);
...
GimpObject *gimp_edit_copy(GimpImage *image, GList *drawables,
                           GimpContext *context, GError **error) {
  GList *iter;
  ...
  g_return_val_if_fail(g_list_length(drawables) == 1 || drawables_are_layers,
                       NULL);
...
GimpBuffer *gimp_edit_copy_visible(GimpImage *image, GimpContext *context,
                                   GError **error) {
  GimpBuffer *buffer;
  GList *pickables;
...
static gboolean gimp_edit_paste_is_in_place(GimpPasteType paste_type) {
  switch (paste_type) {
...
static gboolean gimp_edit_paste_is_floating(GimpPasteType paste_type) {
  switch (paste_type) {
  case GIMP_PASTE_TYPE_FLOATING:
  case GIMP_PASTE_TYPE_FLOATING_INTO:
  case GIMP_PASTE_TYPE_FLOATING_IN_PLACE:
...
static GimpLayer *gimp_edit_paste_get_layer(GimpImage *image,
  ...
  if (GIMP_IS_IMAGE(paste)) {
    ...
    layer = GIMP_LAYER(gimp_item_convert(GIMP_ITEM(layer), image, layer_type));
    ...
    case GIMP_PASTE_TYPE_FLOATING_INTO_IN_PLACE:
      ...
      if (gimp_drawable_get_format(GIMP_DRAWABLE(layer)) != floating_format) {
        gimp_drawable_convert_type(
            GIMP_DRAWABLE(layer), image, gimp_drawable_get_base_type(drawable),
            gimp_drawable_get_precision(drawable), TRUE, NULL, NULL,
    ...
    layer = gimp_layer_new_from_buffer(
        GIMP_BUFFER(paste), image, floating_format, _("Pasted Layer"),
...
static void gimp_edit_paste_get_viewport_offset(
    GimpImage *image, GimpDrawable *drawable, GimpObject *paste,
    gint viewport_x, gint viewport_y, gint viewport_width, gint viewport_height,
  ...
  if (viewport_width == image_width && viewport_height == image_height) {
  ...
  if (drawable) {
    ...
    if (children && gimp_container_get_n_children(children) == 0) {
       ...
       * as target
       */
      gimp_item_bounds(GIMP_ITEM(gimp_image_get_mask(image)), &target_x,
                       &target_y, &target_width, &target_height);
    } else {
      gimp_item_mask_intersect(GIMP_ITEM(drawable), &target_x, &target_y,
    ...
    if (!have_mask &&         /* if we have no mask */
        ...
        gimp_rectangle_intersect(viewport_x, viewport_y, viewport_width,
                                 viewport_height, off_x,
                                 off_y, /* target_x,y are 0 */
                                 target_width, target_height, &paste_x,
...
static void gimp_edit_paste_get_paste_offset(GimpImage *image,
                                             GimpDrawable *drawable,
                                             GimpObject *paste, gint *offset_x,
                                             gint *offset_y) {
  g_return_if_fail(GIMP_IS_IMAGE(image));
  g_return_if_fail(drawable == NULL || GIMP_IS_DRAWABLE(drawable));
  g_return_if_fail(drawable == NULL ||
  ...
  if (GIMP_IS_IMAGE(paste)) {
    *offset_x = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(paste), "offset-x"));
    *offset_y = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(paste), "offset-y"));
  } else if (GIMP_IS_BUFFER(paste)) {
...
static GimpLayer *gimp_edit_paste_paste(GimpImage *image,
                                        GimpDrawable *drawable,
                                        GimpLayer *layer,
                                        GimpPasteType paste_type, gint offset_x,
                                        gint offset_y) {
...
GimpLayer *gimp_edit_paste(GimpImage *image, GimpDrawable *drawable,
                           GimpObject *paste, GimpPasteType paste_type,
                           gint viewport_x, gint viewport_y,
                           gint viewport_width, gint viewport_height) {
  GimpLayer *layer;
  gint offset_x = 0;
  gint offset_y = 0;
  ...
  g_return_val_if_fail(
      drawable == NULL || gimp_item_is_attached(GIMP_ITEM(drawable)), NULL);
  ...
  if (gimp_edit_paste_is_in_place(paste_type)) {
    gimp_edit_paste_get_paste_offset(image, drawable, paste, &offset_x,
                                     &offset_y);
  } else {
    gimp_edit_paste_get_viewport_offset(image, drawable, GIMP_OBJECT(layer),
                                        viewport_x, viewport_y, viewport_width,
                                        viewport_height, &offset_x, &offset_y);
  }

  return gimp_edit_paste_paste(image, drawable, layer, paste_type, offset_x,
                               offset_y);
}

GimpImage *gimp_edit_paste_as_new_image(Gimp *gimp, GimpObject *paste) {
...
const gchar *gimp_edit_named_cut(GimpImage *image, const gchar *name,
...
const gchar *gimp_edit_named_copy(GimpImage *image, const gchar *name,
                                  GList *drawables, GimpContext *context,
                                  GError **error) {
  GimpBuffer *buffer;
...
const gchar *gimp_edit_named_copy_visible(GimpImage *image, const gchar *name,
                                          GimpContext *context,
                                          GError **error) {
  GimpBuffer *buffer;
  GList *pickables;
...
static GimpBuffer *gimp_edit_extract(GimpImage *image, GList *pickables,
  ...
  if (g_list_length(pickables) > 1 || !GIMP_IS_DRAWABLE(pickables->data))
    cut_pixels = FALSE;
  ...
  buffer = gimp_selection_extract(GIMP_SELECTION(gimp_image_get_mask(image)),
                                  pickables, context, cut_pixels, FALSE, FALSE,
  ...
  if (buffer) {
    ...
    gimp_buffer =
        gimp_buffer_new(buffer, _("Global Buffer"), offset_x, offset_y, FALSE);
    g_object_unref(buffer);
    ...
    if (GIMP_IS_COLOR_MANAGED(pickables->data)) {
      GimpColorProfile *profile = gimp_color_managed_get_color_profile(
          GIMP_COLOR_MANAGED(pickables->data));
