...
static void gimp_pattern_clipboard_changed(Gimp *gimp, GimpPattern *pattern);
...
static void gimp_pattern_clipboard_init(GimpPatternClipboard *pattern) {}
...
static void gimp_pattern_clipboard_constructed(GObject *object) {
  ...
  g_signal_connect_object(pattern->gimp, "clipboard-changed",
                          G_CALLBACK(gimp_pattern_clipboard_changed), pattern,
                          0);
...
GimpData *gimp_pattern_clipboard_new(Gimp *gimp) {
  ...
  return g_object_new(GIMP_TYPE_PATTERN_CLIPBOARD, "name", _("Clipboard Image"),
                      "gimp", gimp, NULL);
}
...
static void gimp_pattern_clipboard_changed(Gimp *gimp, GimpPattern *pattern) {
  ...
  if (buffer) {
    ...
    pattern->mask =
        gimp_temp_buf_new(width, height, gegl_buffer_get_format(buffer));

    gegl_buffer_get(buffer, GEGL_RECTANGLE(0, 0, width, height), 1.0, NULL,
