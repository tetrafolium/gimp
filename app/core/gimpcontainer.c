...
enum { ADD, REMOVE, REORDER, FREEZE, THAW, LAST_SIGNAL };

enum { PROP_0, PROP_CHILDREN_TYPE, PROP_POLICY };
...
static void gimp_container_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_container_get_property(GObject *object, guint property_id,
                                        GValue *value, GParamSpec *pspec);

static gint64 gimp_container_get_memsize(GimpObject *object, gint64 *gui_size);
...
G_DEFINE_TYPE_WITH_CODE(GimpContainer, gimp_container, GIMP_TYPE_OBJECT,
                        G_ADD_PRIVATE(GimpContainer) G_IMPLEMENT_INTERFACE(
                            GIMP_TYPE_CONFIG, gimp_container_config_iface_init))

#define parent_class gimp_container_parent_class

static guint container_signals[LAST_SIGNAL] = {
    0,
};
...
static void gimp_container_class_init(GimpContainerClass *klass) {
  ...
  container_signals[ADD] =
      g_signal_new("add", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpContainerClass, add), NULL, NULL, NULL,
                   G_TYPE_NONE, 1, GIMP_TYPE_OBJECT);

  container_signals[REMOVE] =
      g_signal_new("remove", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpContainerClass, remove), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, GIMP_TYPE_OBJECT);

  container_signals[REORDER] =
      g_signal_new("reorder", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpContainerClass, reorder), NULL, NULL,
                   gimp_marshal_VOID__OBJECT_INT, G_TYPE_NONE, 2,
                   GIMP_TYPE_OBJECT, G_TYPE_INT);

  container_signals[FREEZE] =
      g_signal_new("freeze", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpContainerClass, freeze), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);

  container_signals[THAW] =
      g_signal_new("thaw", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpContainerClass, thaw), NULL, NULL, NULL,
                   G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      object_class, PROP_CHILDREN_TYPE,
      g_param_spec_gtype("children-type", NULL, NULL, GIMP_TYPE_OBJECT,
                         GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_POLICY,
      g_param_spec_enum("policy", NULL, NULL, GIMP_TYPE_CONTAINER_POLICY,
...
static void gimp_container_dispose(GObject *object) {
  ...
  while (container->priv->handlers)
    gimp_container_remove_handler(
        container,
        ((GimpContainerHandler *)container->priv->handlers->data)->quark);
...
static gint64 gimp_container_get_memsize(GimpObject *object, gint64 *gui_size) {
  ...
  for (list = container->priv->handlers; list; list = g_list_next(list)) {
    ...
    memsize += (sizeof(GList) + sizeof(GimpContainerHandler) +
...
static void gimp_container_serialize_foreach(GObject *object,
  ...
  serialize_data->success = config_iface->serialize(
      GIMP_CONFIG(object), serialize_data->writer, serialize_data->data);
...
static gboolean gimp_container_deserialize(GimpConfig *config,
  ...
  while (g_scanner_peek_next_token(scanner) == token) {
    ...
    case G_TOKEN_IDENTIFIER: {
      ...
      if (!type) {
        g_scanner_error(scanner, "unable to determine type of '%s'",
                        scanner->value.v_identifier);
        return FALSE;
      }

      if (!g_type_is_a(type, container->priv->children_type)) {
        g_scanner_error(scanner, "'%s' is not a subclass of '%s'",
                        scanner->value.v_identifier,
      ...
      if (!g_type_is_a(type, GIMP_TYPE_CONFIG)) {
        g_scanner_error(scanner, "'%s' does not implement GimpConfigInterface",
                        scanner->value.v_identifier);
        return FALSE;
      }
      ...
      if (!GIMP_CONFIG_GET_IFACE(child)->deserialize(
              GIMP_CONFIG(child), scanner, nest_level + 1, NULL)) {
        if (add_child)
...
static void gimp_container_free_handler(GimpContainer *container,
  ...
  gimp_container_foreach(
      container, (GFunc)gimp_container_free_handler_foreach_func, handler);
...
gboolean gimp_container_add(GimpContainer *container, GimpObject *object) {
  ...
  g_return_val_if_fail(
      G_TYPE_CHECK_INSTANCE_TYPE(object, container->priv->children_type),
      FALSE);

  if (gimp_container_have(container, object)) {
    g_warning("%s: container %p already contains object %p", G_STRFUNC,
              container, object);
    return FALSE;
  }
  ...
  for (list = container->priv->handlers; list; list = g_list_next(list)) {
    ...
    handler_id = g_signal_connect(object, handler->signame, handler->callback,
  ...
  case GIMP_CONTAINER_POLICY_WEAK:
    g_signal_connect(object, "disconnect",
                     G_CALLBACK(gimp_container_disconnect_callback), container);
    break;
  }
...
gboolean gimp_container_remove(GimpContainer *container, GimpObject *object) {
  ...
  if (!gimp_container_have(container, object)) {
    g_warning("%s: container %p does not contain object %p", G_STRFUNC,
              container, object);
    return FALSE;
  }
  ...
  case GIMP_CONTAINER_POLICY_WEAK:
    g_signal_handlers_disconnect_by_func(
        object, gimp_container_disconnect_callback, container);
    break;
  }
...
gboolean gimp_container_insert(GimpContainer *container, GimpObject *object,
                               gint index) {
  g_return_val_if_fail(GIMP_IS_CONTAINER(container), FALSE);
  g_return_val_if_fail(object != NULL, FALSE);
  g_return_val_if_fail(
  ...
  g_return_val_if_fail(index >= -1 && index <= container->priv->n_children,
                       FALSE);
...
gboolean gimp_container_reorder(GimpContainer *container, GimpObject *object,
                                gint new_index) {
  gint index;
  ...
  g_return_val_if_fail(
      new_index >= -1 && new_index < container->priv->n_children, FALSE);
  ...
  if (index != new_index)
    g_signal_emit(container, container_signals[REORDER], 0, object, new_index);
...
gboolean gimp_container_have(GimpContainer *container, GimpObject *object) {
  g_return_val_if_fail(GIMP_IS_CONTAINER(container), FALSE);
...
void gimp_container_foreach(GimpContainer *container, GFunc func,
                            gpointer user_data) {
  g_return_if_fail(GIMP_IS_CONTAINER(container));
...
GimpObject *gimp_container_search(GimpContainer *container,
  ...
  if (container->priv->n_children > 0) {
    return GIMP_CONTAINER_GET_CLASS(container)->search(container, func,
                                                       user_data);
  }
...
GimpObject *gimp_container_get_last_child(GimpContainer *container) {
  ...
  if (container->priv->n_children > 0)
    return GIMP_CONTAINER_GET_CLASS(container)->get_child_by_index(
        container, container->priv->n_children - 1);
...
gint gimp_container_get_child_index(GimpContainer *container,
  ...
  g_return_val_if_fail(
      G_TYPE_CHECK_INSTANCE_TYPE(object, container->priv->children_type), -1);
...
static void gimp_container_get_name_array_foreach_func(GimpObject *object,
                                                       gchar ***iter) {
  gchar **array = *iter;
...
gchar **gimp_container_get_name_array(GimpContainer *container, gint *length) {
  gchar **names;
  gchar **iter;
  ...
  *length = gimp_container_get_n_children(container);
  ...
  gimp_container_foreach(
      container, (GFunc)gimp_container_get_name_array_foreach_func, &iter);
...
GQuark gimp_container_add_handler(GimpContainer *container,
                                  const gchar *signame, GCallback callback,
                                  gpointer callback_data) {
  GimpContainerHandler *handler;
  gchar *key;
  ...
  if (!g_str_has_prefix(signame, "notify::"))
    g_return_val_if_fail(
        g_signal_lookup(signame, container->priv->children_type), 0);
  ...
  container->priv->handlers =
      g_list_prepend(container->priv->handlers, handler);

  gimp_container_foreach(
      container, (GFunc)gimp_container_add_handler_foreach_func, handler);
...
void gimp_container_remove_handler(GimpContainer *container, GQuark id) {
  ...
  if (!list) {
    g_warning("%s: tried to remove handler which unknown id %d", G_STRFUNC, id);
    return;
  }
...
void gimp_container_remove_handlers_by_func(GimpContainer *container,
                                            GCallback callback,
                                            gpointer callback_data) {
  GList *list;
...
void gimp_container_remove_handlers_by_data(GimpContainer *container,
                                            gpointer callback_data) {
  GList *list;
