...
static gboolean gimp_async_wait_until(GimpWaitable *waitable, gint64 end_time);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpAsync, gimp_async, G_TYPE_OBJECT,
...
static void gimp_async_class_init(GimpAsyncClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  async_signals[WAITING] =
      g_signal_new("waiting", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpAsyncClass, waiting), NULL, NULL, NULL,
                   G_TYPE_NONE, 0);
...
static void gimp_async_init(GimpAsync *async) {
...
static void gimp_async_finalize(GObject *object) {
  ...
  if (async->priv->finished && async->priv->result &&
...
static void gimp_async_stop(GimpAsync *async) {
  ...
  if (!g_queue_is_empty(&async->priv->callbacks)) {
    ...
    async->priv->idle_id = g_idle_add_full(
        G_PRIORITY_DEFAULT, (GSourceFunc)gimp_async_idle, async, NULL);
  }
 ...
 * "running" state.
 */
GimpAsync *gimp_async_new(void) { return g_object_new(GIMP_TYPE_ASYNC, NULL); }
 ...
 * may only be called on the main thread.
 */
void gimp_async_add_callback(GimpAsync *async, GimpAsyncCallback callback,
                             gpointer data) {
  GimpAsyncCallbackInfo *callback_info;
...
void gimp_async_add_callback_for_object(GimpAsync *async,
                                        GimpAsyncCallback callback,
                                        gpointer data, gpointer gobject) {
  GimpAsyncCallbackInfo *callback_info;
  ...
  g_object_weak_ref(gobject, (GWeakNotify)gimp_async_callback_weak_notify,
                    callback_info);
 ...
 * may only be called on the main thread.
 */
void gimp_async_remove_callback(GimpAsync *async, GimpAsyncCallback callback,
                                gpointer data) {
  GList *iter;
  gboolean unref_async = FALSE;
  ...
  while (iter) {
    ...
    if (callback_info->callback == callback && callback_info->data == data) {
      if (callback_info->gobject) {
        g_object_weak_unref(callback_info->gobject,
                            (GWeakNotify)gimp_async_callback_weak_notify,
                            callback_info);
 ...
 * may only be called on the async thread.
 */
void gimp_async_finish(GimpAsync *async, gpointer result) {
 ...
 */
void gimp_async_finish_full(GimpAsync *async, gpointer result,
                            GDestroyNotify result_destroy_func) {
  g_return_if_fail(GIMP_IS_ASYNC(async));
...
gint gimp_async_get_n_running(void) { return gimp_async_n_running; }
