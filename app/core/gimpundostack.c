...
static gint64 gimp_undo_stack_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_undo_stack_pop(GimpUndo *undo, GimpUndoMode undo_mode,
                                GimpUndoAccumulator *accum);
static void gimp_undo_stack_free(GimpUndo *undo, GimpUndoMode undo_mode);
...
static void gimp_undo_stack_pop(GimpUndo *undo, GimpUndoMode undo_mode,
  ...
  for (list = GIMP_LIST(stack->undos)->queue->head; list;
...
GimpUndoStack *gimp_undo_stack_new(GimpImage *image) {
  ...
  return g_object_new(GIMP_TYPE_UNDO_STACK, "image", image, NULL);
}
