...
#define GET_PRIVATE(options)                                                   \
  ((GimpFillOptionsPrivate *)gimp_fill_options_get_instance_private(           \
      (GimpFillOptions *)(options)))

static void gimp_fill_options_config_init(GimpConfigInterface *iface);

static void gimp_fill_options_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_fill_options_get_property(GObject *object, guint property_id,
...
G_DEFINE_TYPE_WITH_CODE(GimpFillOptions, gimp_fill_options, GIMP_TYPE_CONTEXT,
                        G_ADD_PRIVATE(GimpFillOptions) G_IMPLEMENT_INTERFACE(
                            GIMP_TYPE_CONFIG, gimp_fill_options_config_init))

static void gimp_fill_options_class_init(GimpFillOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(object_class, PROP_STYLE, "style", _("Style"), NULL,
                        GIMP_TYPE_FILL_STYLE, GIMP_FILL_STYLE_SOLID,
  ...
  g_object_class_install_property(
      object_class, PROP_PATTERN_VIEW_TYPE,
      g_param_spec_enum("pattern-view-type", NULL, NULL, GIMP_TYPE_VIEW_TYPE,
                        GIMP_VIEW_TYPE_GRID,
                        G_PARAM_CONSTRUCT | GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_PATTERN_VIEW_SIZE,
      g_param_spec_int("pattern-view-size", NULL, NULL, GIMP_VIEW_SIZE_TINY,
...
static void gimp_fill_options_init(GimpFillOptions *options) {}
...
static void gimp_fill_options_set_property(GObject *object, guint property_id,
  ...
  case PROP_FEATHER:
  private
    ->feather = g_value_get_boolean(value);
    break;
  case PROP_FEATHER_RADIUS:
  private
    ->feather_radius = g_value_get_double(value);
    break;

  case PROP_PATTERN_VIEW_TYPE:
  private
    ->pattern_view_type = g_value_get_enum(value);
    break;
  case PROP_PATTERN_VIEW_SIZE:
  private
    ->pattern_view_size = g_value_get_int(value);
    break;
...
GimpFillOptions *gimp_fill_options_new(Gimp *gimp, GimpContext *context,
  ...
  options = g_object_new(GIMP_TYPE_FILL_OPTIONS, "gimp", gimp, NULL);
...
void gimp_fill_options_set_feather(GimpFillOptions *options, gboolean feather,
                                   gdouble radius) {
  g_return_if_fail(GIMP_IS_FILL_OPTIONS(options));
...
private
  ...
  case GIMP_FILL_TRANSPARENT:
    gimp_context_get_background(context, &color);
    gimp_context_set_paint_mode(GIMP_CONTEXT(options), GIMP_LAYER_MODE_ERASE);
...
gboolean gimp_fill_options_set_by_fill_mode(GimpFillOptions *options,
  ...
  return gimp_fill_options_set_by_fill_type(options, context, fill_type, error);
}
...
GeglBuffer *gimp_fill_options_create_buffer(GimpFillOptions *options,
  ...
  buffer =
      gegl_buffer_new(rect, gimp_fill_options_get_format(options, drawable));

  gimp_fill_options_fill_buffer(options, drawable, buffer, pattern_offset_x,
                                pattern_offset_y);
...
void gimp_fill_options_fill_buffer(GimpFillOptions *options,
  ...
  case GIMP_FILL_STYLE_PATTERN: {
    ...
    gimp_drawable_fill_buffer(drawable, buffer, NULL, pattern, pattern_offset_x,
                              pattern_offset_y);
  } break;
  }
}
