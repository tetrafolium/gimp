...
static gboolean gimp_idle_unset_busy(gpointer data) {
  Gimp *gimp = data;
...
void gimp_set_busy_until_idle(Gimp *gimp) {
  ...
  if (!gimp->busy_idle_id) {
    ...
    gimp->busy_idle_id =
        g_idle_add_full(G_PRIORITY_HIGH, gimp_idle_unset_busy, gimp, NULL);
  }
}
...
void gimp_show_message(Gimp *gimp, GObject *handler,
  ...
  if (!gimp->console_messages) {
    if (gimp->gui.show_message) {
      gimp->gui.show_message(gimp, handler, severity, domain, message);
      return;
    } else if (GIMP_IS_PROGRESS(handler) &&
               gimp_progress_message(GIMP_PROGRESS(handler), gimp, severity,
                                     domain, message)) {
  ...
  gimp_enum_get_value(GIMP_TYPE_MESSAGE_SEVERITY, severity, NULL, NULL, &desc,
                      NULL);
  g_printerr("%s-%s: %s\n\n", domain, desc, message);
}
...
void gimp_wait(Gimp *gimp, GimpWaitable *waitable, const gchar *format, ...) {
  ...
  if (!gimp->console_messages && gimp->gui.wait &&
...
gchar *gimp_get_display_name(Gimp *gimp, gint display_id, GObject **monitor,
  ...
  if (gimp->gui.get_display_name)
    return gimp->gui.get_display_name(gimp, display_id, monitor,
                                      monitor_number);
...
gboolean gimp_pdb_dialog_new(Gimp *gimp, GimpContext *context,
  ...
  if (gimp->gui.pdb_dialog_new) {
    ...
    retval = gimp->gui.pdb_dialog_new(gimp, context, progress, container, title,
                                      callback_name, object_name, args);
...
gimp_query_profile_policy(Gimp *gimp, GimpImage *image, GimpContext *context,
  ...
  g_return_val_if_fail(GIMP_IS_CONTEXT(context),
  ...
  if (gimp->gui.query_profile_policy)
    return gimp->gui.query_profile_policy(gimp, image, context, dest_profile,
                                          intent, bpc, dont_ask);
...
GimpMetadataRotationPolicy gimp_query_rotation_policy(Gimp *gimp,
                                                      ...
                                                      GimpContext *context,
                                                      gboolean *dont_ask) {
  g_return_val_if_fail(GIMP_IS_GIMP(gimp),
