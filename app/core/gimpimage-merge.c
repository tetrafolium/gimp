...
static GimpLayer *
gimp_image_merge_layers(GimpImage *image, GimpContainer *container,
                        GSList *merge_list, GimpContext *context,
                        GimpMergeType merge_type, const gchar *undo_desc,
...
GList *gimp_image_merge_visible_layers(GimpImage *image, GimpContext *context,
  ...
  if (merge_active_group) {
    ...
    if (g_list_length(selected_layers) == 1 &&
        gimp_layer_is_floating_sel(selected_layers->data)) {
      ...
      if (GIMP_IS_LAYER(fs_drawable))
        containers = g_list_prepend(
            containers, gimp_item_get_container(GIMP_ITEM(fs_drawable)));
    } else {
      ...
      if (iter == NULL) {
        for (iter = selected_layers; iter; iter = iter->next) {
          ...
          if (iter2 == NULL &&
              !g_list_find(containers,
                           gimp_item_get_container(GIMP_ITEM(iter->data))))
            containers = g_list_prepend(
                containers, gimp_item_get_container(GIMP_ITEM(iter->data)));
        }
      }
    }
  }
  ...
  for (iter = containers; iter; iter = iter->next) {
    ...
    if (merge_list) {
      ...
      layer = gimp_image_merge_layers(image, container, merge_list, context,
                                      merge_type, undo_desc, progress);
...
GimpLayer *gimp_image_flatten(GimpImage *image, GimpContext *context,
  ...
  for (list = gimp_image_get_layer_iter(image); list;
  ...
  if (merge_list) {
    ...
    layer = gimp_image_merge_layers(image, gimp_image_get_layers(image),
                                    merge_list, context, GIMP_FLATTEN_IMAGE,
                                    undo_desc, progress);
    g_slist_free(merge_list);
...
GList *gimp_image_merge_down(GimpImage *image, GList *layers,
  ...
  while ((list = layers)) {
    ...
    for (list2 = gimp_item_get_container_iter(GIMP_ITEM(list->data)); list2;
    ...
    layer = NULL;
    for (list2 = g_list_next(list2); list2; list2 = g_list_next(list2)) {
      layer = list2->data;
  ...
  for (list = merge_lists; list; list = list->next) {
    ...
    layer = gimp_image_merge_layers(
        image, gimp_item_get_container(merge_list->data), merge_list, context,
        merge_type, undo_desc, progress);
...
GimpLayer *gimp_image_merge_group_layer(GimpImage *image,
  ...
  layer = GIMP_LAYER(gimp_item_duplicate(GIMP_ITEM(group), GIMP_TYPE_LAYER));
...
GimpVectors *gimp_image_merge_visible_vectors(GimpImage *image,
  ...
  if (merge_list && merge_list->next) {
    ...
    target_vectors = GIMP_VECTORS(
        gimp_item_duplicate(GIMP_ITEM(vectors), GIMP_TYPE_VECTORS));
    gimp_image_remove_vectors(image, vectors, TRUE, NULL);

    for (list = g_list_next(merge_list); list; list = g_list_next(list)) {
      vectors = list->data;
...
gimp_image_merge_layers(GimpImage *image, GimpContainer *container,
  ...
  if (merge_type == GIMP_FLATTEN_IMAGE ||
    ...
    merge_layer = gimp_layer_new(
        image, (x2 - x1), (y2 - y1), gimp_image_get_layer_format(image, FALSE),
        gimp_object_get_name(bottom_layer), GIMP_OPACITY_OPAQUE,
    ...
    gimp_pickable_srgb_to_image_color(GIMP_PICKABLE(layer), &bg, &bg);

    flatten_node = gimp_gegl_create_flatten_node(
        &bg, gimp_drawable_get_space(GIMP_DRAWABLE(layer)),
  ...
  offset_node = gegl_node_new_child(node, "operation", "gegl:translate", "x",
                                    (gdouble)-x1, "y", (gdouble)-y1, NULL);
  ...
  gimp_gegl_apply_operation(
      NULL, progress, undo_desc, offset_node,
      gimp_drawable_get_buffer(GIMP_DRAWABLE(merge_layer)), NULL, FALSE);
  ...
  if (merge_type == GIMP_FLATTEN_IMAGE) {
    ...
    gimp_image_add_layer(image, merge_layer, parent, position, TRUE);
  } else {
    /*  Add the layer to the image  */
    gimp_image_add_layer(
        image, merge_layer, parent,
        gimp_container_get_n_children(container) - position + 1, TRUE);
  }
