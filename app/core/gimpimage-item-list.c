...
gboolean gimp_image_item_list_bounds(GimpImage *image, GList *list, gint *x,
  ...
  for (l = list; l; l = g_list_next(l)) {
    ...
    if (gimp_item_bounds(item, &tmp_x, &tmp_y, &tmp_w, &tmp_h)) {
      ...
      if (bounds) {
        gimp_rectangle_union(*x, *y, *width, *height, tmp_x + off_x,
                             tmp_y + off_y, tmp_w, tmp_h, x, y, width, height);
      } else {
        *x = tmp_x + off_x;
        *y = tmp_y + off_y;
        *width = tmp_w;
...
void gimp_image_item_list_translate(GimpImage *image, GList *list,
  ...
  if (list) {
    ...
    for (l = list; l; l = g_list_next(l))
      gimp_item_translate(GIMP_ITEM(l->data), offset_x, offset_y, push_undo);
...
void gimp_image_item_list_flip(GimpImage *image, GList *list,
                               GimpContext *context,
  ...
  if (list) {
    ...
    for (l = list; l; l = g_list_next(l)) {
      ...
      gimp_item_flip(item, context, flip_type, axis,
...
void gimp_image_item_list_rotate(GimpImage *image, GList *list,
                                 GimpContext *context,
...
void gimp_image_item_list_transform(GimpImage *image, GList *list,
                                    GimpContext *context,
  ...
  if (list) {
    ...
    for (l = list; l; l = g_list_next(l)) {
      ...
      gimp_item_transform(item, context, matrix, direction, interpolation_type,
 ...
 **/
GList *gimp_image_item_list_get_list(GimpImage *image, GimpItemTypeMask type,
                                     GimpItemSet set) {
  GList *all_items;
  GList *list;
 ...
 */
GList *gimp_image_item_list_linked(GimpImage *image, GList *items) {
  ...
  if (linked) {
    new_items =
        gimp_image_item_list_get_list(image, type, GIMP_ITEM_SET_LINKED);
...
static GList *gimp_image_item_list_remove_children(GList *list,
  ...
  while (l) {
    ...
    if (gimp_viewable_is_ancestor(GIMP_VIEWABLE(parent), GIMP_VIEWABLE(item))) {
