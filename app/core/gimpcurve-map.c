...
gdouble gimp_curve_map_value(GimpCurve *curve, gdouble value) {
  g_return_val_if_fail(GIMP_IS_CURVE(curve), 0.0);

  return gimp_curve_map_value_inline(curve, value);
}

void gimp_curve_map_pixels(GimpCurve *curve_colors, GimpCurve *curve_red,
                           GimpCurve *curve_green, GimpCurve *curve_blue,
                           GimpCurve *curve_alpha, gfloat *src, gfloat *dest,
                           glong samples) {
  ...
  switch (gimp_curve_get_apply_mask(curve_colors, curve_red, curve_green,
                                    curve_blue, curve_alpha)) {
  case CURVE_NONE:
  ...
  default:
    while (samples--) {
      dest[0] = gimp_curve_map_value_inline(
          curve_colors, gimp_curve_map_value_inline(curve_red, src[0]));
      dest[1] = gimp_curve_map_value_inline(
          curve_colors, gimp_curve_map_value_inline(curve_green, src[1]));
      dest[2] = gimp_curve_map_value_inline(
          curve_colors, gimp_curve_map_value_inline(curve_blue, src[2]));
      /* don't apply the colors curve to the alpha channel */
      dest[3] = gimp_curve_map_value_inline(curve_alpha, src[3]);
