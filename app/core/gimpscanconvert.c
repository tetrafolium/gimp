...
gimp_scan_convert_new_from_boundary(const GimpBoundSeg *bound_segs,
  ...
  if (bound_segs) {
    ...
    stroke_segs = gimp_boundary_sort(bound_segs, n_bound_segs, &n_stroke_segs);

    if (stroke_segs) {
      GimpBezierDesc *bezier;

      bezier = gimp_bezier_desc_new_from_bound_segs(stroke_segs, n_bound_segs,
                                                    n_stroke_segs);
 ...
 */
void gimp_scan_convert_set_clip_rectangle(GimpScanConvert *sc, gint x, gint y,
 ...
 */
void gimp_scan_convert_add_polyline(GimpScanConvert *sc, guint n_points,
  ...
  for (i = 0; i < n_points; i++) {
    ...
    if (i == 0 || prev.x != points[i].x || prev.y != points[i].y) {
...
void gimp_scan_convert_add_bezier(GimpScanConvert *sc,
  ...
  sc->path_data =
      g_array_append_vals(sc->path_data, bezier->data, bezier->num_data);
}
 ...
 * units.
 */
void gimp_scan_convert_stroke(GimpScanConvert *sc, gdouble width,
                              GimpJoinStyle join, GimpCapStyle cap,
                              gdouble miter, gdouble dash_offset,
                              GArray *dash_info) {
  sc->do_stroke = TRUE;
  sc->width = width;
  sc->join = join;
  ...
  if (dash_info && dash_info->len >= 2) {
    ...
    if (n_dashes >= 2) {
      sc->dash_info =
          g_array_sized_new(FALSE, FALSE, sizeof(gdouble), n_dashes);
 ...
 * You cannot add additional polygons after this command.
 */
void gimp_scan_convert_render(GimpScanConvert *sc, GeglBuffer *buffer,
                              gint off_x, gint off_y, gboolean antialias) {
  gimp_scan_convert_render_full(sc, buffer, off_x, off_y, TRUE, antialias, 1.0);
}
 ...
 * You cannot add additional polygons after this command.
 */
void gimp_scan_convert_render_value(GimpScanConvert *sc, GeglBuffer *buffer,
                                    gint off_x, gint off_y, gdouble value) {
  gimp_scan_convert_render_full(sc, buffer, off_x, off_y, TRUE, FALSE, value);
}
 ...
 * You cannot add additional polygons after this command.
 */
void gimp_scan_convert_compose(GimpScanConvert *sc, GeglBuffer *buffer,
                               gint off_x, gint off_y) {
  gimp_scan_convert_render_full(sc, buffer, off_x, off_y, FALSE, FALSE, 1.0);
}
 ...
 * You cannot add additional polygons after this command.
 */
void gimp_scan_convert_compose_value(GimpScanConvert *sc, GeglBuffer *buffer,
                                     gint off_x, gint off_y, gdouble value) {
  gimp_scan_convert_render_full(sc, buffer, off_x, off_y, FALSE, FALSE, value);
}
 ...
 * You cannot add additional polygons after this command.
 */
void gimp_scan_convert_render_full(GimpScanConvert *sc, GeglBuffer *buffer,
                                   gint off_x, gint off_y, gboolean replace,
                                   gboolean antialias, gdouble value) {
  const Babl *format;
  ...
  if (sc->clip && !gimp_rectangle_intersect(x, y, width, height, sc->clip_x,
                                            sc->clip_y, sc->clip_w, sc->clip_h,
                                            &x, &y, &width, &height))
    return;
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    guchar *tmp_buf = NULL;
    const gint stride =
        cairo_format_stride_for_width(CAIRO_FORMAT_A8, roi->width);
    ...
    surface = cairo_image_surface_create_for_data(tmp_buf ? tmp_buf : data,
                                                  CAIRO_FORMAT_A8, roi->width,
                                                  roi->height, stride);

    cairo_surface_set_device_offset(surface, -off_x - roi->x, -off_y - roi->y);
    ...
    cairo_set_antialias(cr, antialias ? CAIRO_ANTIALIAS_GRAY
                                      : CAIRO_ANTIALIAS_NONE);
    cairo_set_miter_limit(cr, sc->miter);

    if (sc->do_stroke) {
      cairo_set_line_cap(cr, sc->cap == GIMP_CAP_BUTT
                                 ? CAIRO_LINE_CAP_BUTT
                                 : sc->cap == GIMP_CAP_ROUND
                                       ? CAIRO_LINE_CAP_ROUND
                                       : CAIRO_LINE_CAP_SQUARE);
      cairo_set_line_join(cr, sc->join == GIMP_JOIN_MITER
                                  ? CAIRO_LINE_JOIN_MITER
                                  : sc->join == GIMP_JOIN_ROUND
      ...
      if (sc->dash_info)
        cairo_set_dash(cr, (double *)sc->dash_info->data, sc->dash_info->len,
                       sc->dash_offset);
