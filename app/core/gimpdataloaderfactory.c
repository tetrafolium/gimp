...
gimp_data_loader_factory_get_loader(GimpDataFactory *factory, GFile *file);

static void gimp_data_loader_factory_load(GimpDataFactory *factory,
                                          GimpContext *context,
                                          GHashTable *cache);
static void gimp_data_loader_factory_load_directory(
    GimpDataFactory *factory, GimpContext *context, GHashTable *cache,
    gboolean dir_writable, GFile *directory, GFile *top_directory);
static void gimp_data_loader_factory_load_data(
    GimpDataFactory *factory, GimpContext *context, GHashTable *cache,
    gboolean dir_writable, GFile *file, GFileInfo *info, GFile *top_directory);
...
static void gimp_data_loader_factory_init(GimpDataLoaderFactory *factory) {
...
static void gimp_data_loader_factory_refresh_cache_add(GimpDataFactory *factory,
                                                       GimpData *data,
                                                       gpointer user_data) {
...
static gboolean
gimp_data_loader_factory_refresh_cache_remove(gpointer key, gpointer value,
                                              gpointer user_data) {
  GList *list;
...
static void gimp_data_loader_factory_data_refresh(GimpDataFactory *factory,
  ...
  gimp_data_factory_data_foreach(
      factory, TRUE, gimp_data_loader_factory_refresh_cache_add, cache);
  ...
  g_hash_table_foreach_remove(
      cache, gimp_data_loader_factory_refresh_cache_remove, NULL);
...
GimpDataFactory *gimp_data_loader_factory_new(
  ...
  return g_object_new(GIMP_TYPE_DATA_LOADER_FACTORY, "gimp", gimp, "data-type",
                      data_type, "path-property-name", path_property_name,
                      "writable-property-name", writable_property_name,
                      "ext-property-name", ext_property_name, "new-func",
                      new_func, "get-standard-func", get_standard_func, NULL);
}
...
static void gimp_data_loader_factory_load(GimpDataFactory *factory,
  ...
  for (list = (GList *)ext_path; list; list = g_list_next(list)) {
     ...
     */
    gimp_data_loader_factory_load_directory(factory, context, cache, FALSE,
                                            list->data, list->data);
  }
  ...
  for (list = path; list; list = g_list_next(list)) {
    ...
    gimp_data_loader_factory_load_directory(
        factory, context, cache, dir_writable, list->data, list->data);
  }
  ...
  if (enumerator) {
    ...
    while ((info = g_file_enumerator_next_file(enumerator, NULL, NULL))) {
      ...
      if (file_type == G_FILE_TYPE_DIRECTORY) {
        gimp_data_loader_factory_load_directory(
            factory, context, cache, dir_writable, child, top_directory);
      } else if (file_type == G_FILE_TYPE_REGULAR) {
        gimp_data_loader_factory_load_data(
            factory, context, cache, dir_writable, child, info, top_directory);
      }
...
static void gimp_data_loader_factory_load_data(
  ...
  if (cache) {
    ...
    if (cached_data && gimp_data_get_mtime(cached_data->data) != 0 &&
  ...
  if (input) {
    ...
    if (error) {
      g_prefix_error(&error, _("Error loading '%s': "),
                     gimp_file_get_utf8_name(file));
    } else if (!data_list) {
      g_set_error(&error, GIMP_DATA_ERROR, GIMP_DATA_ERROR_READ,
                  _("Error loading '%s'"), gimp_file_get_utf8_name(file));
    }
  ...
  if (G_LIKELY(data_list)) {
    ...
    for (list = data_list; list; list = g_list_next(list)) {
      ...
      if (obsolete) {
        gimp_container_add(container_obsolete, GIMP_OBJECT(data));
      } else {
        gimp_data_set_folder_tags(data, top_directory);

        gimp_container_add(container, GIMP_OBJECT(data));
      }
   ...
   */
  if (G_UNLIKELY(error)) {
    gimp_message(gimp_data_factory_get_gimp(factory), NULL, GIMP_MESSAGE_ERROR,
