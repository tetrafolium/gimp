...
enum { PROP_0, PROP_PATH, PROP_WRITABLE, PROP_RUNNING };
...
static void gimp_extension_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_extension_get_property(GObject *object, guint property_id,
                                        GValue *value, GParamSpec *pspec);

static void gimp_extension_clean(GimpExtension *extension);
static gint gimp_extension_file_cmp(GFile *a, GFile *b);
...
static void gimp_extension_class_init(GimpExtensionClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("writable", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
  g_object_class_install_property(
      object_class, PROP_RUNNING,
      g_param_spec_boolean("running", NULL, NULL, FALSE, GIMP_PARAM_READWRITE));
}

static void gimp_extension_init(GimpExtension *extension) {
...
GimpExtension *gimp_extension_new(const gchar *dir, gboolean writable) {
  g_return_val_if_fail(dir && g_file_test(dir, G_FILE_TEST_IS_DIR), NULL);

  return g_object_new(GIMP_TYPE_EXTENSION, "path", dir, "writable", writable,
                      NULL);
}
...
GdkPixbuf *gimp_extension_get_screenshot(GimpExtension *extension, gint width,
                                         gint height, const gchar **caption) {
  ...
  if (screenshot) {
    ...
    image = as_screenshot_get_image_for_locale(screenshot, g_getenv("LANGUAGE"),
                                               width, height);
    if (!image)
      image =
          as_screenshot_get_image_for_locale(screenshot, NULL, width, height);
    ...
    if (pixbuf) {
      ...
      if (istream) {
        pixbuf =
            gdk_pixbuf_new_from_stream(G_INPUT_STREAM(istream), NULL, &error);
...
gchar *gimp_extension_get_markup_description(GimpExtension *extension) {
  static const GMarkupParser appstream_text_parser = {
      appstream_text_start_element, appstream_text_end_element,
      appstream_text_characters, NULL, /*  passthrough */
      NULL                             /*  error       */
  };
...
gboolean gimp_extension_load(GimpExtension *extension, GError **error) {
  AsApp *app;
  GPtrArray *extends;
  GPtrArray *requires;
  AsRelease *release;
  ...
  app = as_app_new();
  success =
      as_app_parse_file(app, path, AS_APP_PARSE_FLAG_USE_HEURISTICS, error);
  g_free(path);
  if (success && as_app_get_kind(app) != AS_APP_KIND_ADDON) {
     ...
     */
    if (error && *error == NULL)
      *error = g_error_new(GIMP_EXTENSION_ERROR, GIMP_EXTENSION_BAD_APPDATA,
                           _("Extension AppData must be of type \"addon\", "
                             "found \"%s\" instead."),
  ...
  if (success && !g_ptr_array_find_with_equal_func(extends, "org.gimp.GIMP",
                                                   g_str_equal, NULL)) {
    ...
    if (error && *error == NULL)
      *error =
          g_error_new(GIMP_EXTENSION_ERROR, GIMP_EXTENSION_BAD_APPDATA,
                      _("Extension AppData must extend \"org.gimp.GIMP\"."));
    success = FALSE;
  }

  if (success &&
      g_strcmp0(as_app_get_id(app), gimp_object_get_name(extension)) != 0) {
    ...
    if (error && *error == NULL)
      *error = g_error_new(GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
                           _("Extension AppData id (\"%s\") and directory "
                             "(\"%s\") must be the same."),
  ...
  if (success && (!release || !as_release_get_version(release))) {
    ...
    if (error && *error == NULL)
      *error = g_error_new(
          GIMP_EXTENSION_ERROR, GIMP_EXTENSION_NO_VERSION,
  ...
  if (success && requires) {
    ...
    for (i = 0; i < requires->len; i++) {
      ...
      if (as_require_get_kind(require) == AS_REQUIRE_KIND_ID &&
         ...
         */
        *error = g_error_new(
            GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
  ...
  if (!has_require) {
    ...
    if (error && *error == NULL) {
      *error = g_error_new(GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
                           _("<requires><id>org.gimp.GIMP</id></requires> for "
                             "version comparison is mandatory."));
    }
  }
...
gboolean gimp_extension_run(GimpExtension *extension, GError **error) {
  GHashTable *metadata;
  gchar *value;
  ...
  g_object_set(extension, "running", TRUE, NULL);
...
void gimp_extension_stop(GimpExtension *extension) {
  gimp_extension_clean(extension);
  g_object_set(extension, "running", FALSE, NULL);
}
 ...
 */
gint gimp_extension_cmp(GimpExtension *extension1, GimpExtension *extension2) {
 ...
 */
gint gimp_extension_id_cmp(GimpExtension *extension, const gchar *id) {
...
static GList *gimp_extension_validate_paths(GimpExtension *extension,
  ...
  for (i = 0; patharray[i]; i++) {
    ...
    if (g_path_is_absolute(patharray[i])) {
      *error = g_error_new(GIMP_EXTENSION_ERROR, GIMP_EXTENSION_BAD_PATH,
                           _("'%s' is not a relative path."), patharray[i]);
      break;
    }
    ...
    if (!is_subpath) {
      *error =
          g_error_new(GIMP_EXTENSION_ERROR, GIMP_EXTENSION_BAD_PATH,
                      _("'%s' is not a child of the extension."), patharray[i]);
      g_object_unref(file);
      break;
    }
    ...
    if (as_directories) {
      if (g_file_query_file_type(file, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
        ...
        *error = g_error_new(GIMP_EXTENSION_ERROR, GIMP_EXTENSION_BAD_PATH,
                             _("'%s' is not a directory."), patharray[i]);
        g_object_unref(file);
        break;
      }
    } else {
      if (g_file_query_file_type(file, G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,
        ...
        *error = g_error_new(GIMP_EXTENSION_ERROR, GIMP_EXTENSION_BAD_PATH,
                             _("'%s' is not a valid file."), patharray[i]);
        g_object_unref(file);
        break;
      }
    }
...
static void appstream_text_start_element(GMarkupParseContext *context,
  ...
  if ((state->numbered_list || state->unnumbered_list) &&
    ...
    g_set_error(error, GIMP_ERROR, GIMP_FAILED, _("Unknown tag <%s>."),
                element_name);
  }
}
