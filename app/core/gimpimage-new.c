...
GimpTemplate *gimp_image_new_get_last_template(Gimp *gimp, GimpImage *image) {
  ...
  if (image) {
    gimp_config_sync(G_OBJECT(gimp->config->default_image), G_OBJECT(template),
                     0);
...
void gimp_image_new_set_last_template(Gimp *gimp, GimpTemplate *template) {
  g_return_if_fail(GIMP_IS_GIMP(gimp));
  g_return_if_fail(GIMP_IS_TEMPLATE(template));

  gimp_config_sync(G_OBJECT(template), G_OBJECT(gimp->image_new_last_template),
                   0);
}

GimpImage *gimp_image_new_from_template(Gimp *gimp, GimpTemplate *template,
                                        GimpContext *context) {
  GimpImage *image;
  GimpLayer *layer;
  ...
  image = gimp_create_image(gimp, gimp_template_get_width(template),
                            gimp_template_get_height(template),
                            gimp_template_get_base_type(template),
                            gimp_template_get_precision(template), FALSE);
  ...
  gimp_image_set_resolution(image, gimp_template_get_resolution_x(template),
  ...
  layer = gimp_layer_new(image, width, height,
                         ...
                         _("Background"), GIMP_OPACITY_OPAQUE,
                         gimp_image_get_default_new_layer_mode(image));

  gimp_drawable_fill(GIMP_DRAWABLE(layer), context,
                     gimp_template_get_fill_type(template));
...
GimpImage *gimp_image_new_from_drawable(Gimp *gimp, GimpDrawable *drawable) {
  GimpItem *item;
  GimpImage *image;
  GimpImage *new_image;
  GimpLayer *new_layer;
  ...
  new_image = gimp_create_image(gimp, gimp_item_get_width(item),
                                gimp_item_get_height(item), type,
                                gimp_drawable_get_precision(drawable), TRUE);
  gimp_image_undo_disable(new_image);

  if (type == GIMP_INDEXED)
    gimp_image_set_colormap(new_image, gimp_image_get_colormap(image),
                            gimp_image_get_colormap_size(image), FALSE);
  ...
  new_layer =
      GIMP_LAYER(gimp_item_convert(GIMP_ITEM(drawable), new_image, new_type));

  gimp_object_set_name(GIMP_OBJECT(new_layer), gimp_object_get_name(drawable));
  ...
  gimp_layer_set_mode(new_layer,
                      gimp_image_get_default_new_layer_mode(new_image), FALSE);
 ...
 */
static void gimp_image_new_copy_drawables(GimpImage *image, GList *drawables,
                                          GimpImage *new_image,
                                          GimpLayer *parent,
  ...
  for (iter = layers; iter; iter = iter->next) {
    if (g_list_find(drawables, iter->data)) {
      ...
      new_layer = GIMP_LAYER(
          gimp_item_convert(GIMP_ITEM(iter->data), new_image, new_type));
      ...
      gimp_item_set_visible(
          GIMP_ITEM(new_layer),
          n_drawables > 1 ? gimp_item_get_visible(iter->data) : TRUE, FALSE);
      gimp_layer_set_mode(
          new_layer,
      ...
      if (n_drawables > 1 && gimp_viewable_get_children(iter->data))
        gimp_image_new_copy_drawables(image, drawables, new_image, iter->data,
                                      new_layer);
    }
  }
...
GimpImage *gimp_image_new_from_drawables(Gimp *gimp, GList *drawables,
  ...
  new_image =
      gimp_create_image(gimp, gimp_image_get_width(image),
                        gimp_image_get_height(image), type, precision, TRUE);
  gimp_image_undo_disable(new_image);
  ...
  if (copy_selection) {
    ...
    if (!gimp_channel_is_empty(selection)) {
      ...
      gimp_drawable_set_buffer(GIMP_DRAWABLE(new_selection), FALSE, NULL,
                               buffer);
...
GimpImage *gimp_image_new_from_component(Gimp *gimp, GimpImage *image,
  ...
  new_image = gimp_create_image(gimp, gimp_image_get_width(image),
                                gimp_image_get_height(image), GIMP_GRAY,
  ...
  layer = GIMP_LAYER(
      gimp_item_convert(GIMP_ITEM(channel), new_image, GIMP_TYPE_LAYER));
...
GimpImage *gimp_image_new_from_buffer(Gimp *gimp, GimpBuffer *buffer) {
  GimpImage *image;
  GimpLayer *layer;
  const Babl *format;
  gboolean has_alpha;
  ...
  image = gimp_create_image(gimp, gimp_buffer_get_width(buffer),
                            gimp_buffer_get_height(buffer),
                            gimp_babl_format_get_base_type(format),
                            gimp_babl_format_get_precision(format), TRUE);
  gimp_image_undo_disable(image);
  ...
  layer = gimp_layer_new_from_buffer(
      buffer, image, gimp_image_get_layer_format(image, has_alpha),
      _("Pasted Layer"), GIMP_OPACITY_OPAQUE,
      gimp_image_get_default_new_layer_mode(image));
...
GimpImage *gimp_image_new_from_pixbuf(Gimp *gimp, GdkPixbuf *pixbuf,
  ...
  new_image = gimp_create_image(gimp, gdk_pixbuf_get_width(pixbuf),
                                gdk_pixbuf_get_height(pixbuf), base_type,
  ...
  layer = gimp_layer_new_from_pixbuf(
      pixbuf, new_image, gimp_image_get_layer_format(new_image, has_alpha),
      layer_name, GIMP_OPACITY_OPAQUE,
      gimp_image_get_default_new_layer_mode(new_image));
