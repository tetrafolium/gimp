...
enum { DIRTY, LAST_SIGNAL };

enum { PROP_0, PROP_FILE, PROP_WRITABLE, PROP_DELETABLE, PROP_MIME_TYPE };
...
static void gimp_data_set_property(GObject *object, guint property_id,
                                   const GValue *value, GParamSpec *pspec);
static void gimp_data_get_property(GObject *object, guint property_id,
                                   GValue *value, GParamSpec *pspec);

static void gimp_data_name_changed(GimpObject *object);
static gint64 gimp_data_get_memsize(GimpObject *object, gint64 *gui_size);
...
static gint gimp_data_real_compare(GimpData *data1, GimpData *data2);

static gboolean gimp_data_add_tag(GimpTagged *tagged, GimpTag *tag);
static gboolean gimp_data_remove_tag(GimpTagged *tagged, GimpTag *tag);
...
static void gimp_data_class_init(GimpDataClass *klass) {
  ...
  data_signals[DIRTY] = g_signal_new(
      "dirty", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpDataClass, dirty), NULL, NULL, NULL, G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      object_class, PROP_MIME_TYPE,
      g_param_spec_string("mime-type", NULL, NULL, NULL,
...
static void gimp_data_set_property(GObject *object, guint property_id,
  ...
  switch (property_id) {
  case PROP_FILE:
    gimp_data_set_file(data, g_value_get_object(value), private->writable,
                       private->deletable);
    break;
  ...
  case PROP_DELETABLE:
  private
    ->deletable = g_value_get_boolean(value);
    break;

  case PROP_MIME_TYPE:
    if (g_value_get_string(value))
    private
    ->mime_type = g_quark_from_string(g_value_get_string(value));
    else private->mime_type = 0;
    break;
...
static gint gimp_data_real_compare(GimpData *data1, GimpData *data2) {
  ...
  return gimp_object_name_collate((GimpObject *)data1, (GimpObject *)data2);
}
...
private
  ->tags = g_list_prepend(private->tags, g_object_ref(tag));
...
static gboolean gimp_data_remove_tag(GimpTagged *tagged, GimpTag *tag) {
  ...
  for (list = private->tags; list; list = g_list_next(list)) {
    ...
    if (gimp_tag_equals(tag, this)) {
    private
      ->tags = g_list_delete_link(private->tags, list);
      g_object_unref(this);
      return TRUE;
    }
  }
...
static gchar *gimp_data_get_identifier(GimpTagged *tagged) {
  ...
  if (private->file) {
    ...
    if (g_str_has_prefix(path, data_dir)) {
      tmp = g_strconcat("${gimp_data_dir}", path + strlen(data_dir), NULL);
      identifier = g_filename_to_utf8(tmp, -1, NULL, NULL, NULL);
      g_free(tmp);
    } else if (g_str_has_prefix(path, gimp_dir)) {
      tmp = g_strconcat("${gimp_dir}", path + strlen(gimp_dir), NULL);
      identifier = g_filename_to_utf8(tmp, -1, NULL, NULL, NULL);
      g_free(tmp);
    } else if (g_str_has_prefix(path, MYPAINT_BRUSHES_DIR)) {
      tmp = g_strconcat("${mypaint_brushes_dir}",
                        path + strlen(MYPAINT_BRUSHES_DIR), NULL);
      identifier = g_filename_to_utf8(tmp, -1, NULL, NULL, NULL);
      g_free(tmp);
    } else {
      identifier = g_filename_to_utf8(path, -1, NULL, NULL, NULL);
    }
...
static gchar *gimp_data_get_checksum(GimpTagged *tagged) { return NULL; }
 ...
 **/
gboolean gimp_data_save(GimpData *data, GError **error) {
...
private
  ...
  if (GIMP_DATA_GET_CLASS(data)->save) {
    ...
    output = G_OUTPUT_STREAM(g_file_replace(private->file, NULL, FALSE,
    ...
    if (output) {
      ...
      if (success) {
        if (!g_output_stream_close(output, NULL, error)) {
          g_prefix_error(error, _("Error saving '%s': "),
  ...
  if (success) {
    GFileInfo *info =
        g_file_query_info(private->file, G_FILE_ATTRIBUTE_TIME_MODIFIED,
 ...
 *          successful.
 **/
gboolean gimp_data_delete_from_disk(GimpData *data, GError **error) {
  GimpDataPrivate *private;
 ...
 **/
void gimp_data_set_file(GimpData *data, GFile *file, gboolean writable,
                        gboolean deletable) {
  GimpDataPrivate *private;
  gchar *path;
...
private
  ...
  if (writable || deletable) {
    ...
    {
      info = g_file_query_info(private->file, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE,
                               G_FILE_QUERY_INFO_NONE, NULL, NULL);
    ...
    } else /* OR it doesn't exist */
    {
      ...
      info = g_file_query_info(parent, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE,
                               G_FILE_QUERY_INFO_NONE, NULL, NULL);
 ...
 **/
void gimp_data_create_filename(GimpData *data, GFile *dest_dir) {
  GimpDataPrivate *private;
  gchar *safename;
  gchar *basename;
  GFile *file;
...
private
  ...
  while (g_file_query_exists(file, NULL)) {
    ...
    basename = g_strdup_printf("%s-%d%s", safename, unum++,
...
static const gchar *tag_blacklist[] = {"brushes",  "dynamics",  "patterns",
                                       "palettes", "gradients", "tool-presets"};
 ...
 **/
void gimp_data_set_folder_tags(GimpData *data, GFile *top_directory) {
  GimpDataPrivate *private;
  gchar *tmp;
  gchar *dirname;
  gchar *top_path;
...
void gimp_data_set_mtime(GimpData *data, gint64 mtime) {
  GimpDataPrivate *private;
 ...
 **/
void gimp_data_copy(GimpData *data, GimpData *src_data) {
  g_return_if_fail(GIMP_IS_DATA(data));
...
GimpData *gimp_data_duplicate(GimpData *data) {
  ...
  if (gimp_data_is_duplicatable(data)) {
    ...
    g_object_set(new, "name", NULL, "writable",
                 GIMP_DATA_GET_CLASS(new)->save != NULL, "deletable", TRUE,
                 NULL);
 ...
 **/
void gimp_data_make_internal(GimpData *data, const gchar *identifier) {
  GimpDataPrivate *private;
 ...
 **/
gint gimp_data_compare(GimpData *data1, GimpData *data2) {
  ...
  g_return_val_if_fail(GIMP_DATA_GET_CLASS(data1)->compare ==
                           GIMP_DATA_GET_CLASS(data2)->compare,
                       0);
