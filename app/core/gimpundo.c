...
enum { POP, FREE, LAST_SIGNAL };

enum { PROP_0, PROP_IMAGE, PROP_TIME, PROP_UNDO_TYPE, PROP_DIRTY_MASK };

static void gimp_undo_constructed(GObject *object);
static void gimp_undo_finalize(GObject *object);
static void gimp_undo_set_property(GObject *object, guint property_id,
                                   const GValue *value, GParamSpec *pspec);
static void gimp_undo_get_property(GObject *object, guint property_id,
                                   GValue *value, GParamSpec *pspec);

static gint64 gimp_undo_get_memsize(GimpObject *object, gint64 *gui_size);

static gboolean gimp_undo_get_popup_size(GimpViewable *viewable, gint width,
...
static void gimp_undo_real_pop(GimpUndo *undo, GimpUndoMode undo_mode,
                               GimpUndoAccumulator *accum);
static void gimp_undo_real_free(GimpUndo *undo, GimpUndoMode undo_mode);
...
static void gimp_undo_class_init(GimpUndoClass *klass) {
  ...
  undo_signals[POP] =
      g_signal_new("pop", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpUndoClass, pop), NULL, NULL,
                   gimp_marshal_VOID__ENUM_POINTER, G_TYPE_NONE, 2,
                   GIMP_TYPE_UNDO_MODE, G_TYPE_POINTER);

  undo_signals[FREE] =
      g_signal_new("free", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpUndoClass, free), NULL, NULL, NULL,
                   G_TYPE_NONE, 1, GIMP_TYPE_UNDO_MODE);
  ...
  g_object_class_install_property(object_class, PROP_TIME,
                                  g_param_spec_uint("time", NULL, NULL, 0,
                                                    G_MAXUINT, 0,
                                                    GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_UNDO_TYPE,
      g_param_spec_enum("undo-type", NULL, NULL, GIMP_TYPE_UNDO_TYPE,
                        GIMP_UNDO_GROUP_NONE,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_DIRTY_MASK,
      g_param_spec_flags("dirty-mask", NULL, NULL, GIMP_TYPE_DIRTY_MASK,
                         GIMP_DIRTY_NONE,
                         GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_undo_init(GimpUndo *undo) { undo->time = time(NULL); }
...
static gboolean gimp_undo_get_popup_size(GimpViewable *viewable, gint width,
  ...
  if (undo->preview && (gimp_temp_buf_get_width(undo->preview) > width ||
...
static GimpTempBuf *gimp_undo_get_new_preview(GimpViewable *viewable,
  ...
  if (undo->preview) {
    ...
    if (preview_width < gimp_temp_buf_get_width(undo->preview) &&
      ...
      return gimp_temp_buf_scale(undo->preview, preview_width, preview_height);
    }
...
void gimp_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
                   GimpUndoAccumulator *accum) {
...
void gimp_undo_free(GimpUndo *undo, GimpUndoMode undo_mode) {
  g_return_if_fail(GIMP_IS_UNDO(undo));
...
static void gimp_undo_idle_free(GimpUndoIdle *idle) {
  if (idle->context)
...
void gimp_undo_create_preview(GimpUndo *undo, GimpContext *context,
                              gboolean create_now) {
  g_return_if_fail(GIMP_IS_UNDO(undo));
  ...
  if (create_now) {
    ...
    undo->preview_idle_id = g_idle_add_full(
        GIMP_PRIORITY_VIEWABLE_IDLE, gimp_undo_create_preview_idle, idle,
...
static void gimp_undo_create_preview_private(GimpUndo *undo,
  ...
  undo->preview =
      gimp_viewable_get_new_preview(preview_viewable, context, width, height);
