...
gint gimp_get_pid(void) { return (gint)getpid(); }
...
GimpUnit gimp_get_default_unit(void) {
  ...
  ret =
      GetLocaleInfo(LOCALE_USER_DEFAULT, LOCALE_IMEASURE | LOCALE_RETURN_NUMBER,
                    (LPTSTR)&measurement, sizeof(measurement) / sizeof(TCHAR));
...
gchar **gimp_properties_append(GType object_type, gint *n_properties,
                               gchar **names, GValue **values, ...) {
  va_list args;
  ...
  names = gimp_properties_append_valist(object_type, n_properties, names,
                                        values, args);
  va_end(args);
...
gchar **gimp_properties_append_valist(GType object_type, gint *n_properties,
                                      gchar **names, GValue **values,
                                      va_list args) {
  ...
  while (param_name) {
    GValue *value;
    gchar *error = NULL;
    GParamSpec *pspec = g_object_class_find_property(object_class, param_name);

    if (!pspec) {
      g_warning("%s: object class `%s' has no property named `%s'", G_STRFUNC,
                g_type_name(object_type), param_name);
      break;
    }
...
void gimp_properties_free(gint n_properties, gchar **names, GValue *values) {
...
static gboolean unescape_gstring(GString *string) {
  const gchar *from;
  gchar *to;
    ...
    if (*from == '&') {
      ...
      if (*from == '#') {
        ...
        if (end == from || errno != 0) {
          ...
          if ((0 < l && l <= 0xD7FF) || (0xE000 <= l && l <= 0xFFFD) ||
 ...
 **/
const gchar *gimp_enum_get_value_name(GType enum_type, gint value) {
  const gchar *value_name = NULL;

  gimp_enum_get_value(enum_type, value, &value_name, NULL /*value_nick*/,
                      NULL /*value_desc*/, NULL /*value_help*/);

  return value_name;
}

gboolean gimp_get_fill_params(GimpContext *context, GimpFillType fill_type,
                              GimpRGB *color, GimpPattern **pattern,
                              GError **error)
 ...
 **/
void gimp_constrain_line(gdouble start_x, gdouble start_y, gdouble *end_x,
                         gdouble *end_y, gint n_snap_lines,
                         gdouble offset_angle, gdouble xres, gdouble yres) {
  GimpVector2 diff;
  GimpVector2 dir;
  gdouble angle;
...
gint gimp_file_compare(GFile *file1, GFile *file2) {
...
GFile *gimp_file_with_new_extension(GFile *file, GFile *ext_file) {
  gchar *uri;
  gchar *file_ext;
  gint file_ext_len = 0;
 ...
 */
gboolean gimp_file_delete_recursive(GFile *file, GError **error) {
...
gboolean gimp_ascii_strtoi(const gchar *nptr, gchar **endptr, gint base,
                           gint *result) {
  gchar *temp_endptr;
  gint64 temp_result;
  ...
  if (*endptr == nptr || errno == ERANGE || temp_result < G_MININT ||
      temp_result > G_MAXINT) {
    errno = 0;
...
gboolean gimp_ascii_strtod(const gchar *nptr, gchar **endptr, gdouble *result) {
  gchar *temp_endptr;
  gdouble temp_result;
...
gint gimp_g_list_compare(GList *list1, GList *list2) {
  while (list1 && list2) {
...
static GimpIdleRunAsyncData *gimp_idle_run_async_data_new(void) {
...
static void gimp_idle_run_async_data_inc_ref(GimpIdleRunAsyncData *data) {
  data->ref_count++;
}

static void gimp_idle_run_async_data_dec_ref(GimpIdleRunAsyncData *data) {
  data->ref_count--;
...
static void gimp_idle_run_async_cancel(GimpAsync *async,
                                       GimpIdleRunAsyncData *data) {
...
static void gimp_idle_run_async_waiting(GimpAsync *async,
                                        GimpIdleRunAsyncData *data) {
  gimp_idle_run_async_data_inc_ref(data);
  ...
  g_signal_handlers_block_by_func(data->async, gimp_idle_run_async_cancel,
                                  data);

  while (!gimp_async_is_stopped(data->async))
    data->func(data->async, data->user_data);

  g_signal_handlers_unblock_by_func(data->async, gimp_idle_run_async_cancel,
                                    data);
...
static gboolean gimp_idle_run_async_idle(GimpIdleRunAsyncData *data) {
  gimp_idle_run_async_data_inc_ref(data);
...
GimpAsync *gimp_idle_run_async(GimpRunAsyncFunc func, gpointer user_data) {
  return gimp_idle_run_async_full(G_PRIORITY_DEFAULT_IDLE, func, user_data,
                                  NULL);
}

GimpAsync *gimp_idle_run_async_full(gint priority, GimpRunAsyncFunc func,
                                    gpointer user_data,
                                    GDestroyNotify user_data_destroy_func) {
  GimpIdleRunAsyncData *data;
  ...
  g_signal_connect(data->async, "cancel",
                   G_CALLBACK(gimp_idle_run_async_cancel), data);
  g_signal_connect(data->async, "waiting",
                   G_CALLBACK(gimp_idle_run_async_waiting), data);

  data->idle_id =
      g_idle_add_full(priority, (GSourceFunc)gimp_idle_run_async_idle, data,
...
GimpImage *gimp_create_image_from_buffer(Gimp *gimp, GeglBuffer *buffer,
  ...
  image = gimp_create_image(gimp, gegl_buffer_get_width(buffer),
                            gegl_buffer_get_height(buffer),
                            gimp_babl_format_get_base_type(format),
                            gimp_babl_format_get_precision(format), FALSE);

  layer = gimp_layer_new_from_gegl_buffer(
      buffer, image, format, image_name, GIMP_OPACITY_OPAQUE,
      GIMP_LAYER_MODE_NORMAL, NULL /* same image */);
