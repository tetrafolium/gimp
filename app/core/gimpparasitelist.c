...
static void parasite_serialize(const gchar *key, GimpParasite *parasite,
                               GimpConfigWriter *writer);
static void parasite_copy(const gchar *key, GimpParasite *parasite,
                          GimpParasiteList *list);
static gboolean parasite_free(const gchar *key, GimpParasite *parasite,
                              gpointer unused);
static void parasite_count_if_persistent(const gchar *key,
                                         GimpParasite *parasite, gint *count);

G_DEFINE_TYPE_WITH_CODE(
    GimpParasiteList, gimp_parasite_list, GIMP_TYPE_OBJECT,
...
static void gimp_parasite_list_class_init(GimpParasiteListClass *klass) {
  ...
  parasite_list_signals[ADD] =
      g_signal_new("add", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpParasiteListClass, add), NULL, NULL,
  ...
  parasite_list_signals[REMOVE] =
      g_signal_new("remove", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpParasiteListClass, remove), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, G_TYPE_POINTER);
...
static gint64 gimp_parasite_list_get_memsize(GimpObject *object,
  ...
  memsize += gimp_g_hash_table_get_memsize_foreach(
      list->table, (GimpMemsizeFunc)gimp_parasite_get_memsize, gui_size);
...
static gboolean gimp_parasite_list_serialize(GimpConfig *list,
  ...
  if (GIMP_PARASITE_LIST(list)->table)
    g_hash_table_foreach(GIMP_PARASITE_LIST(list)->table,
                         (GHFunc)parasite_serialize, writer);
...
static gboolean gimp_parasite_list_deserialize(GimpConfig *list,
  ...
  g_scanner_scope_add_symbol(scanner, 0, parasite_symbol,
                             (gpointer)parasite_symbol);
  ...
  while (g_scanner_peek_next_token(scanner) == token) {
    ...
    case G_TOKEN_SYMBOL:
      if (scanner->value.v_symbol == parasite_symbol) {
        ...
        parasite = gimp_parasite_new(parasite_name, parasite_flags,
                                     parasite_data_size, parasite_data);
...
gint gimp_parasite_list_persistent_length(GimpParasiteList *list) {
  ...
  gimp_parasite_list_foreach(list, (GHFunc)parasite_count_if_persistent, &len);
...
static void parasite_serialize(const gchar *key, GimpParasite *parasite,
  ...
  gimp_config_writer_printf(
      writer, "\"%s\" %lu %lu", gimp_parasite_name(parasite),
      gimp_parasite_flags(parasite), gimp_parasite_data_size(parasite));

  gimp_config_writer_data(writer, gimp_parasite_data_size(parasite),
