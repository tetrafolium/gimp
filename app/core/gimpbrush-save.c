...
gboolean gimp_brush_save(GimpData *data, GOutputStream *output,
                         GError **error) {
  ...
  header.header_size = g_htonl(sizeof(GimpBrushHeader) + strlen(name) + 1);
  ...
  if (pixmap) {
    ...
    if (!g_output_stream_write_all(output, gimp_temp_buf_get_data(mask),
                                   gimp_temp_buf_get_data_size(mask), NULL,
                                   NULL, error)) {
      return FALSE;
    }
  }
