...
static void gimp_mirror_set_property(GObject *object, guint property_id,
                                     const GValue *value, GParamSpec *pspec);
static void gimp_mirror_get_property(GObject *object, guint property_id,
                                     GValue *value, GParamSpec *pspec);

static void gimp_mirror_update_strokes(GimpSymmetry *mirror,
                                       GimpDrawable *drawable,
                                       GimpCoords *origin);
static void gimp_mirror_get_transform(GimpSymmetry *mirror, gint stroke,
                                      gdouble *angle, gboolean *reflect);
...
static void gimp_mirror_remove_guide(GimpMirror *mirror,
                                     GimpOrientationType orientation);
static void gimp_mirror_guide_removed_cb(GObject *object, GimpMirror *mirror);
static void gimp_mirror_guide_position_cb(GObject *object, GParamSpec *pspec,
                                          GimpMirror *mirror);
...
static void gimp_mirror_set_vertical_symmetry(GimpMirror *mirror,
                                              gboolean active);
static void gimp_mirror_set_point_symmetry(GimpMirror *mirror, gboolean active);

static void
gimp_mirror_image_size_changed_cb(GimpImage *image, gint previous_origin_x,
                                  gint previous_origin_y, gint previous_width,
                                  gint previous_height, GimpSymmetry *sym);

G_DEFINE_TYPE(GimpMirror, gimp_mirror, GIMP_TYPE_SYMMETRY)

#define parent_class gimp_mirror_parent_class

static void gimp_mirror_class_init(GimpMirrorClass *klass) {
  ...
  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_HORIZONTAL_SYMMETRY, "horizontal-symmetry",
      _("Horizontal Symmetry"),
      _("Reflect the initial stroke across a horizontal axis"), FALSE,
      GIMP_PARAM_STATIC_STRINGS | GIMP_SYMMETRY_PARAM_GUI);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_VERTICAL_SYMMETRY, "vertical-symmetry",
      _("Vertical Symmetry"),
      _("Reflect the initial stroke across a vertical axis"), FALSE,
      GIMP_PARAM_STATIC_STRINGS | GIMP_SYMMETRY_PARAM_GUI);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_POINT_SYMMETRY, "point-symmetry",
      _("Central Symmetry"), _("Invert the initial stroke through a point"),
      FALSE, GIMP_PARAM_STATIC_STRINGS | GIMP_SYMMETRY_PARAM_GUI);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_DISABLE_TRANSFORMATION, "disable-transformation",
      _("Disable brush transform"), _("Disable brush reflection"), FALSE,
      GIMP_PARAM_STATIC_STRINGS | GIMP_SYMMETRY_PARAM_GUI);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_MIRROR_POSITION_X,
                          "mirror-position-x", _("Vertical axis position"),
                          NULL, 0.0, G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_STATIC_STRINGS | GIMP_SYMMETRY_PARAM_GUI);
  ...
  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_MIRROR_POSITION_Y,
                          "mirror-position-y", _("Horizontal axis position"),
                          NULL, 0.0, G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_STATIC_STRINGS | GIMP_SYMMETRY_PARAM_GUI);
...
static void gimp_mirror_init(GimpMirror *mirror) {}

static void gimp_mirror_constructed(GObject *object) {
  GimpSymmetry *sym = GIMP_SYMMETRY(object);

  g_signal_connect_object(sym->image, "size-changed-detailed",
                          G_CALLBACK(gimp_mirror_image_size_changed_cb), sym,
                          0);
}
...
static void gimp_mirror_set_property(GObject *object, guint property_id,
  ...
  case PROP_HORIZONTAL_SYMMETRY:
    gimp_mirror_set_horizontal_symmetry(mirror, g_value_get_boolean(value));
    break;

  case PROP_VERTICAL_SYMMETRY:
    gimp_mirror_set_vertical_symmetry(mirror, g_value_get_boolean(value));
    break;

  case PROP_POINT_SYMMETRY:
    gimp_mirror_set_point_symmetry(mirror, g_value_get_boolean(value));
    break;
    ...
    if (g_value_get_double(value) >= 0.0 &&
      ...
      if (mirror->vertical_guide) {
        g_signal_handlers_block_by_func(mirror->vertical_guide,
                                        gimp_mirror_guide_position_cb, mirror);
        gimp_image_move_guide(image, mirror->vertical_guide,
                              mirror->mirror_position_x, FALSE);
        g_signal_handlers_unblock_by_func(
            mirror->vertical_guide, gimp_mirror_guide_position_cb, mirror);
      }
    }
    break;
    ...
    if (g_value_get_double(value) >= 0.0 &&
      ...
      if (mirror->horizontal_guide) {
        ...
        gimp_image_move_guide(image, mirror->horizontal_guide,
                              mirror->mirror_position_y, FALSE);
        g_signal_handlers_unblock_by_func(
            mirror->horizontal_guide, gimp_mirror_guide_position_cb, mirror);
      }
    }
    break;
...
static void gimp_mirror_update_strokes(GimpSymmetry *sym,
                                       GimpDrawable *drawable,
                                       GimpCoords *origin) {
  ...
  strokes = g_list_prepend(strokes, g_memdup(origin, sizeof(GimpCoords)));
...
static void gimp_mirror_get_transform(GimpSymmetry *sym, gint stroke,
                                      gdouble *angle, gboolean *reflect) {
  GimpMirror *mirror = GIMP_MIRROR(sym);
...
static void gimp_mirror_reset(GimpMirror *mirror) {
  ...
  if (sym->origin) {
    gimp_symmetry_set_origin(sym, sym->drawable, sym->origin);
  }
}
...
static void gimp_mirror_add_guide(GimpMirror *mirror,
  ...
  guide = gimp_guide_custom_new(orientation, gimp->next_guide_id++,
  ...
  if (orientation == GIMP_ORIENTATION_HORIZONTAL) {
    ...
    g_object_set(mirror, "mirror-position-y", position, NULL);
    ...
    g_object_set(mirror, "mirror-position-x", position, NULL);

    mirror->vertical_guide = guide;
  }

  g_signal_connect(guide, "removed", G_CALLBACK(gimp_mirror_guide_removed_cb),
                   mirror);

  gimp_image_add_guide(image, guide, (gint)position);

  g_signal_connect(guide, "notify::position",
                   G_CALLBACK(gimp_mirror_guide_position_cb), mirror);
}
...
static void gimp_mirror_remove_guide(GimpMirror *mirror,
  ...
  if (guide) {
    g_signal_handlers_disconnect_by_func(G_OBJECT(guide),
                                         gimp_mirror_guide_removed_cb, mirror);
    g_signal_handlers_disconnect_by_func(G_OBJECT(guide),
                                         gimp_mirror_guide_position_cb, mirror);
...
static void gimp_mirror_guide_removed_cb(GObject *object, GimpMirror *mirror) {
  ...
  g_signal_handlers_disconnect_by_func(object, gimp_mirror_guide_removed_cb,
                                       mirror);
  g_signal_handlers_disconnect_by_func(object, gimp_mirror_guide_position_cb,
                                       mirror);
  ...
  if (GIMP_GUIDE(object) == mirror->horizontal_guide) {
    ...
    g_object_set(mirror, "horizontal-symmetry", FALSE, NULL);
    g_object_set(mirror, "point-symmetry", FALSE, NULL);
    g_object_set(mirror, "mirror-position-y", 0.0, NULL);

    if (mirror->vertical_guide && !mirror->vertical_mirror) {
      ...
      gimp_image_remove_guide(symmetry->image, mirror->vertical_guide, FALSE);
      g_clear_object(&mirror->vertical_guide);
    }
    ...
    g_object_set(mirror, "vertical-symmetry", FALSE, NULL);
    g_object_set(mirror, "point-symmetry", FALSE, NULL);
    g_object_set(mirror, "mirror-position-x", 0.0, NULL);

    if (mirror->horizontal_guide && !mirror->horizontal_mirror) {
      ...
      gimp_image_remove_guide(symmetry->image, mirror->horizontal_guide, FALSE);
      g_clear_object(&mirror->horizontal_guide);
    }
  }

  if (mirror->horizontal_guide == NULL && mirror->vertical_guide == NULL) {
    gimp_image_symmetry_remove(symmetry->image, GIMP_SYMMETRY(mirror));
  } else {
...
static void gimp_mirror_guide_position_cb(GObject *object, GParamSpec *pspec,
  ...
  if (guide == mirror->horizontal_guide) {
    g_object_set(mirror, "mirror-position-y",
                 (gdouble)gimp_guide_get_position(guide), NULL);
  } else if (guide == mirror->vertical_guide) {
    g_object_set(mirror, "mirror-position-x",
                 (gdouble)gimp_guide_get_position(guide), NULL);
  }
}
