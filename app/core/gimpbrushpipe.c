...
static gint64 gimp_brush_pipe_get_memsize(GimpObject *object, gint64 *gui_size);
...
static void gimp_brush_pipe_copy(GimpData *data, GimpData *src_data);

static void gimp_brush_pipe_begin_use(GimpBrush *brush);
static void gimp_brush_pipe_end_use(GimpBrush *brush);
static GimpBrush *
gimp_brush_pipe_select_brush(GimpBrush *brush, const GimpCoords *last_coords,
...
static gint64 gimp_brush_pipe_get_memsize(GimpObject *object,
  ...
  memsize +=
      pipe->dimension * (sizeof(gint) /* rank   */ + sizeof(gint) /* stride */ +
                         sizeof(PipeSelectModes));

  for (i = 0; i < pipe->n_brushes; i++)
    memsize += gimp_object_get_memsize(GIMP_OBJECT(pipe->brushes[i]), gui_size);
...
static void gimp_brush_pipe_copy(GimpData *data, GimpData *src_data) {
  ...
  g_clear_pointer(&pipe->rank, g_free);
  pipe->rank = g_memdup(src_pipe->rank, pipe->dimension * sizeof(gint));

  g_clear_pointer(&pipe->stride, g_free);
  pipe->stride = g_memdup(src_pipe->stride, pipe->dimension * sizeof(gint));

  g_clear_pointer(&pipe->select, g_free);
  pipe->select =
      g_memdup(src_pipe->select, pipe->dimension * sizeof(PipeSelectModes));

  g_clear_pointer(&pipe->index, g_free);
  pipe->index = g_memdup(src_pipe->index, pipe->dimension * sizeof(gint));
...
gimp_brush_pipe_select_brush(GimpBrush *brush, const GimpCoords *last_coords,
  ...
  for (i = 0; i < pipe->dimension; i++) {
    ...
    case PIPE_SELECT_ANGULAR:
      ...
      ix =
          (gint)RINT((1.0 - current_coords->direction + 0.25) * pipe->rank[i]) %
          pipe->rank[i];
      break;
    ...
    case PIPE_SELECT_TILT_X:
      ix =
          RINT(current_coords->xtilt / 2.0 * pipe->rank[i]) + pipe->rank[i] / 2;
      break;

    case PIPE_SELECT_TILT_Y:
      ix =
          RINT(current_coords->ytilt / 2.0 * pipe->rank[i]) + pipe->rank[i] / 2;
      break;
