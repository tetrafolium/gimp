...
enum { EXTENSION_INSTALLED, EXTENSION_REMOVED, LAST_SIGNAL };
...
gimp_extension_manager_search_directory(GimpExtensionManager *manager,
                                        GFile *directory, gboolean system_dir);

static void gimp_extension_manager_extension_running(
    GimpExtension *extension, GParamSpec *pspec, GimpExtensionManager *manager);

G_DEFINE_TYPE_WITH_CODE(
    GimpExtensionManager, gimp_extension_manager, GIMP_TYPE_OBJECT,
...
gimp_extension_manager_class_init(GimpExtensionManagerClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_BRUSH_PATHS,
      g_param_spec_pointer("brush-paths", NULL, NULL, GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_DYNAMICS_PATHS,
      g_param_spec_pointer("dynamics-paths", NULL, NULL, GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_pointer("pattern-paths", NULL, NULL, GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_GRADIENT_PATHS,
      g_param_spec_pointer("gradient-paths", NULL, NULL, GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_PALETTE_PATHS,
      g_param_spec_pointer("palette-paths", NULL, NULL, GIMP_PARAM_READWRITE));
  g_object_class_install_property(object_class, PROP_TOOL_PRESET_PATHS,
                                  g_param_spec_pointer("tool-preset-paths",
                                                       NULL, NULL,
                                                       GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_SPLASH_PATHS,
      g_param_spec_pointer("splash-paths", NULL, NULL, GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_THEME_PATHS,
      g_param_spec_pointer("theme-paths", NULL, NULL, GIMP_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_PLUG_IN_PATHS,
      g_param_spec_pointer("plug-in-paths", NULL, NULL, GIMP_PARAM_READWRITE));

  signals[EXTENSION_INSTALLED] = g_signal_new(
      "extension-installed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpExtensionManagerClass, extension_installed), NULL,
      NULL, gimp_marshal_VOID__OBJECT_BOOLEAN, G_TYPE_NONE, 2,
      GIMP_TYPE_EXTENSION, G_TYPE_BOOLEAN);
  signals[EXTENSION_REMOVED] = g_signal_new(
      "extension-removed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpExtensionManagerClass, extension_removed), NULL, NULL,
      NULL, G_TYPE_NONE, 1, G_TYPE_STRING);
}

static void gimp_extension_manager_init(GimpExtensionManager *manager) {
  ...
  while (g_scanner_peek_next_token(scanner) == token) {
    ...
    case G_TOKEN_IDENTIFIER: {
      ...
      if (!gimp_scanner_parse_token(scanner, G_TOKEN_LEFT_PAREN)) {
        g_scanner_error(scanner, "Left paren expected after extension ID.");
        g_free(name);
        return FALSE;
      }
          ...
          if (list) {
            ...
            if (gimp_extension_run(list->data, &error)) {
              ...
              g_printerr("Extension '%s' failed to run: %s\n",
                         gimp_object_get_name(list->data), error->message);
              g_error_free(error);
            }
          }
        }
...
GimpExtensionManager *gimp_extension_manager_new(Gimp *gimp) {
  ...
  manager = g_object_new(GIMP_TYPE_EXTENSION_MANAGER, "gimp", gimp, NULL);
...
void gimp_extension_manager_initialize(GimpExtensionManager *manager) {
  ...
  if (g_file_query_exists(file, NULL)) {
    ...
    gimp_config_deserialize_file(GIMP_CONFIG(manager), file, &processed_ids,
                                 &error);
    if (error) {
  ...
  for (list = manager->p->extensions; list; list = list->next) {
     ...
     * set in extensionrc.
     */
    if (!g_list_find_custom(processed_ids, gimp_object_get_name(list->data),
                            (GCompareFunc)g_strcmp0))
      processed_ids = g_list_prepend(
          processed_ids, g_strdup(gimp_object_get_name(list->data)));
...
void gimp_extension_manager_exit(GimpExtensionManager *manager) {
  ...
  if (!gimp_config_serialize_to_file(GIMP_CONFIG(manager), file,
                                     "GIMP extensionrc", "end of extensionrc",
                                     NULL, &error)) {
    gimp_message_literal(manager->p->gimp, NULL, GIMP_MESSAGE_ERROR,
                         error->message);
    g_error_free(error);
  }
...
gboolean gimp_extension_manager_install(GimpExtensionManager *manager,
  ...
  if ((success = gimp_extension_load(extension, error))) {
    manager->p->extensions = g_list_prepend(manager->p->extensions, extension);
...
gboolean gimp_extension_manager_remove(GimpExtensionManager *manager,
  ...
  manager->p->extensions = g_list_remove_link(manager->p->extensions, iter);
  manager->p->uninstalled_extensions =
      g_list_concat(manager->p->uninstalled_extensions, iter);
...
gboolean gimp_extension_manager_undo_remove(GimpExtensionManager *manager,
  ...
  manager->p->uninstalled_extensions =
      g_list_remove(manager->p->uninstalled_extensions, extension);
...
static void gimp_extension_manager_refresh(GimpExtensionManager *manager) {
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    new_paths =
        g_list_copy_deep(gimp_extension_get_mypaint_brush_paths(extension),
                         (GCopyFunc)g_object_ref, NULL);
    ...
    new_paths =
        g_list_copy_deep(gimp_extension_get_tool_preset_paths(extension),
                         (GCopyFunc)g_object_ref, NULL);
  ...
  g_object_set(manager, "brush-paths", brush_paths, "dynamics-paths",
               dynamics_paths, "mypaint-brush-paths", brush_paths,
               "pattern-paths", pattern_paths, "gradient-paths", gradient_paths,
               "palette-paths", palette_paths, "tool-preset-paths",
               tool_preset_paths, "plug-in-paths", plug_in_paths,
               "splash-paths", splash_paths, "theme-paths", theme_paths, NULL);
}
...
gimp_extension_manager_search_directory(GimpExtensionManager *manager,
  ...
  if (enumerator) {
    ...
    while ((info = g_file_enumerator_next_file(enumerator, NULL, NULL))) {
      ...
      if (g_file_query_file_type(subdir, G_FILE_QUERY_INFO_NONE, NULL) ==
          G_FILE_TYPE_DIRECTORY) {
        GimpExtension *extension;
        GError *error = NULL;

        extension = gimp_extension_new(g_file_peek_path(subdir), !system_dir);

        if (gimp_extension_load(extension, &error)) {
          if (system_dir)
            manager->p->sys_extensions =
                g_list_prepend(manager->p->sys_extensions, extension);
          else
            manager->p->extensions =
                g_list_prepend(manager->p->extensions, extension);
        } else {
...
gimp_extension_manager_extension_running(GimpExtension *extension,
  ...
  g_object_get(extension, "running", &running, NULL);
  if (running)
    g_hash_table_insert(manager->p->running_extensions,
                        (gpointer)gimp_object_get_name(extension), extension);
  else
