...
static void gimp_image_transform_guides(GimpImage *image,
  ...
  for (iter = gimp_image_get_guides(image); iter;) {
    ...
    gimp_transform_polygon(matrix, vertices, 2, FALSE, vertices, &n_vertices);
    ...
    if (new_orientation != old_orientation || new_position != old_position) {
...
static void
gimp_image_transform_sample_points(GimpImage *image, const GimpMatrix3 *matrix,
                                   const GeglRectangle *old_bounds) {
  GList *iter;
  ...
  for (iter = gimp_image_get_sample_points(image); iter;) {
    ...
    gimp_transform_polygon(matrix, vertices, 1, FALSE, vertices, &n_vertices);
    ...
    if (new_x < 0 || new_x >= gimp_image_get_width(image) || new_y < 0 ||
        new_y >= gimp_image_get_height(image)) {
      gimp_image_remove_sample_point(image, sample_point, TRUE);
...
void gimp_image_transform(GimpImage *image, GimpContext *context,
  ...
  gimp_transform_resize_boundary(
      &transform, clip_result,

      old_bounds.x, old_bounds.y, old_bounds.x + old_bounds.width,
  ...
  gimp_matrix3_translate(&transform, old_bounds.x - new_bounds.x,
  ...
  while ((item = gimp_object_queue_pop(queue))) {
    ...
    gimp_item_transform(item, context, &transform, direction,
                        interpolation_type, clip, progress);
  ...
  if (!gegl_rectangle_equal(&new_bounds, &old_bounds)) {
    gimp_image_undo_push_image_size(image, NULL, new_bounds.x, new_bounds.y,
                                    new_bounds.width, new_bounds.height);

    g_object_set(image, "width", new_bounds.width, "height", new_bounds.height,
                 NULL);
  }
  ...
  if (!gegl_rectangle_equal(&new_bounds, &old_bounds)) {
    gimp_image_size_changed_detailed(image, old_bounds.x - new_bounds.x,
                                     old_bounds.y - new_bounds.y,
