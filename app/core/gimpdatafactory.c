...
static void gimp_data_factory_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_data_factory_get_property(GObject *object, guint property_id,
...
static void gimp_data_factory_class_init(GimpDataFactoryClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_gtype("data-type", NULL, NULL, GIMP_TYPE_DATA,
                         GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_PATH_PROPERTY_NAME,
      g_param_spec_string("path-property-name", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_WRITABLE_PROPERTY_NAME,
      g_param_spec_string("writable-property-name", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_EXT_PROPERTY_NAME,
      g_param_spec_string("ext-property-name", NULL, NULL, NULL,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_NEW_FUNC,
      g_param_spec_pointer("new-func", NULL, NULL,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_GET_STANDARD_FUNC,
      g_param_spec_pointer("get-standard-func", NULL, NULL,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}
...
static gint64 gimp_data_factory_get_memsize(GimpObject *object,
  ...
  memsize += gimp_object_get_memsize(GIMP_OBJECT(priv->container), gui_size);
  memsize +=
      gimp_object_get_memsize(GIMP_OBJECT(priv->container_obsolete), gui_size);
...
static void gimp_data_factory_real_data_save(GimpDataFactory *factory) {
  ...
  for (list = GIMP_LIST(priv->container)->queue->head; list;
       list = g_list_next(list)) {
    GimpData *data = list->data;

    if (gimp_data_is_dirty(data) && gimp_data_is_writable(data)) {
...
void gimp_data_factory_data_init(GimpDataFactory *factory, GimpContext *context,
                                 gboolean no_data) {
  GimpDataFactoryPrivate *priv = GET_PRIVATE(factory);
  gchar *signal_name;
  ...
  g_signal_connect_object(priv->gimp->config, signal_name,
                          G_CALLBACK(gimp_data_factory_path_notify), factory,
                          0);
  g_free(signal_name);
...
static void gimp_data_factory_clean_cb(GimpDataFactory *factory, GimpData *data,
                                       gpointer user_data) {
...
void gimp_data_factory_data_clean(GimpDataFactory *factory) {
  ...
  gimp_data_factory_data_foreach(factory, TRUE, gimp_data_factory_clean_cb,
                                 NULL);
}
...
static void gimp_data_factory_data_free_foreach(GimpDataFactory *factory,
                                                GimpData *data,
                                                gpointer user_data) {
...
void gimp_data_factory_data_free(GimpDataFactory *factory) {
  ...
  if (!gimp_container_is_empty(factory->priv->container)) {
    ...
    gimp_data_factory_data_foreach(factory, TRUE,
                                   gimp_data_factory_data_free_foreach, NULL);
...
GimpData *gimp_data_factory_data_new(GimpDataFactory *factory,
  ...
  if (priv->data_new_func) {
    ...
    g_warning("%s: GimpDataFactory::data_new_func() returned NULL", G_STRFUNC);
  }
...
gboolean gimp_data_factory_data_delete(GimpDataFactory *factory, GimpData *data,
                                       gboolean delete_from_disk,
                                       GError **error) {
  GimpDataFactoryPrivate *priv;
  ...
  if (gimp_container_have(priv->container, GIMP_OBJECT(data))) {
    ...
    retval = GIMP_DATA_FACTORY_GET_CLASS(factory)->data_delete(
        factory, data, delete_from_disk, error);
...
gboolean gimp_data_factory_data_save_single(GimpDataFactory *factory,
  ...
  if (!gimp_data_get_file(data)) {
    ...
    if (!writable_dir) {
      g_set_error(error, GIMP_DATA_ERROR, 0, _("Failed to save data:\n\n%s"),
                  my_error->message);
...
GList *gimp_data_factory_get_data_path(GimpDataFactory *factory) {
  ...
  g_object_get(priv->gimp->config, priv->path_property_name, &path, NULL);
...
GList *gimp_data_factory_get_data_path_writable(GimpDataFactory *factory) {
  GimpDataFactoryPrivate *priv = GET_PRIVATE(factory);
  gchar *path = NULL;
  GList *list = NULL;
  ...
  g_object_get(priv->gimp->config, priv->writable_property_name, &path, NULL);
...
const GList *gimp_data_factory_get_data_path_ext(GimpDataFactory *factory) {
  ...
  g_object_get(priv->gimp->extension_manager, priv->ext_property_name, &list,
               NULL);
...
static GFile *gimp_data_factory_get_save_dir(GimpDataFactory *factory,
  ...
  if (writable_path) {
    ...
    for (list = writable_path; list; list = g_list_next(list)) {
      GList *found =
          g_list_find_custom(path, list->data, (GCompareFunc)gimp_file_compare);
      if (found) {
        GFile *dir = found->data;

        found_any = TRUE;

        if (g_file_query_file_type(dir, G_FILE_QUERY_INFO_NONE, NULL) !=
