...
void gimp_image_resize(GimpImage *image, GimpContext *context, gint new_width,
                       gint new_height, gint offset_x, gint offset_y,
                       GimpProgress *progress) {
  gimp_image_resize_with_layers(image, context, GIMP_FILL_TRANSPARENT,
                                new_width, new_height, offset_x, offset_y,
                                GIMP_ITEM_SET_NONE, TRUE, progress);
}

void gimp_image_resize_with_layers(GimpImage *image, GimpContext *context,
                                   GimpFillType fill_type, gint new_width,
                                   gint new_height, gint offset_x,
                                   gint offset_y, GimpItemSet layer_set,
  ...
  resize_layers =
      gimp_image_item_list_get_list(image, GIMP_ITEM_TYPE_LAYERS, layer_set);

  old_width = gimp_image_get_width(image);
  old_height = gimp_image_get_height(image);

  /*  Push the image size to the stack  */
  gimp_image_undo_push_image_size(image, NULL, -offset_x, -offset_y, new_width,
                                  new_height);

  /*  Set the new width and height  */
  g_object_set(image, "width", new_width, "height", new_height, NULL);
  ...
  while ((item = gimp_object_queue_pop(queue))) {
    if (GIMP_IS_LAYER(item)) {
      ...
      gimp_item_resize(item, context, fill_type, new_width, new_height,
                       old_offset_x, old_offset_y);

      gimp_item_end_move(item, TRUE);
    } else {
      gimp_item_resize(item, context, GIMP_FILL_TRANSPARENT, new_width,
  ...
  gimp_image_size_changed_detailed(image, offset_x, offset_y, old_width,
                                   old_height);
...
void gimp_image_resize_to_layers(GimpImage *image, GimpContext *context,
  ...
  for (list = g_list_next(list); list; list = g_list_next(list)) {
    ...
    gimp_rectangle_union(x, y, width, height, gimp_item_get_offset_x(item),
                         gimp_item_get_offset_y(item),
                         gimp_item_get_width(item), gimp_item_get_height(item),
                         &x, &y, &width, &height);
  }

  gimp_image_resize(image, context, width, height, -x, -y, progress);
  if (offset_x)
    *offset_x = -x;
  if (offset_y)
    *offset_y = -y;
...
void gimp_image_resize_to_selection(GimpImage *image, GimpContext *context,
                                    GimpProgress *progress) {
  GimpChannel *selection = gimp_image_get_mask(image);
  gint x, y, w, h;

  if (gimp_item_bounds(GIMP_ITEM(selection), &x, &y, &w, &h)) {
    gimp_image_resize(image, context, w, h, -x, -y, progress);
  }
}
