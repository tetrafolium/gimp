...
void floating_sel_attach(GimpLayer *layer, GimpDrawable *drawable) {
  GimpImage *image;
...
void floating_sel_anchor(GimpLayer *layer) {
  ...
  if (filter) {
    gimp_drawable_merge_filter(drawable, filter, NULL, NULL, NULL, FALSE, FALSE,
                               FALSE);
  }
...
gboolean floating_sel_to_layer(GimpLayer *layer, GError **error) {
  GimpItem *item;
  GimpImage *image;
  ...
  gimp_drawable_update(GIMP_DRAWABLE(layer), 0, 0, gimp_item_get_width(item),
...
const GimpBoundSeg *floating_sel_boundary(GimpLayer *layer, gint *n_segs) {
  ...
  if (layer->fs.boundary_known == FALSE) {
    ...
    if (gimp_drawable_has_alpha(GIMP_DRAWABLE(layer))) {
      ...
      layer->fs.segs = gimp_boundary_find(
          buffer, NULL, babl_format("A float"), GIMP_BOUNDARY_WITHIN_BOUNDS, 0,
          0, width, height, GIMP_BOUNDARY_HALF_WAY, &layer->fs.num_segs);
...
void floating_sel_invalidate(GimpLayer *layer) {
  ...
  gimp_viewable_invalidate_preview(
      GIMP_VIEWABLE(gimp_layer_get_floating_sel_drawable(layer)));
