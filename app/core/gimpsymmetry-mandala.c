...
static void gimp_mandala_set_property(GObject *object, guint property_id,
                                      const GValue *value, GParamSpec *pspec);
static void gimp_mandala_get_property(GObject *object, guint property_id,
...
static void gimp_mandala_guide_removed_cb(GObject *object,
                                          GimpMandala *mandala);
static void gimp_mandala_guide_position_cb(GObject *object, GParamSpec *pspec,
                                           GimpMandala *mandala);

static void gimp_mandala_update_strokes(GimpSymmetry *mandala,
                                        GimpDrawable *drawable,
                                        GimpCoords *origin);
static void gimp_mandala_get_transform(GimpSymmetry *mandala, gint stroke,
                                       gdouble *angle, gboolean *reflect);
static void
gimp_mandala_image_size_changed_cb(GimpImage *image, gint previous_origin_x,
...
static void gimp_mandala_class_init(GimpMandalaClass *klass) {
  ...
  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_CENTER_X, "center-x",
                          _("Center abscissa"), NULL, 0.0, G_MAXDOUBLE, 0.0,
  ...
  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_CENTER_Y, "center-y",
                          _("Center ordinate"), NULL, 0.0, G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_STATIC_STRINGS | GIMP_SYMMETRY_PARAM_GUI);
  ...
  GIMP_CONFIG_PROP_INT(object_class, PROP_SIZE, "size", _("Number of points"),
                       NULL, 1, 100, 6.0,
                       GIMP_PARAM_STATIC_STRINGS | GIMP_SYMMETRY_PARAM_GUI);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_DISABLE_TRANSFORMATION, "disable-transformation",
      _("Disable brush transform"), _("Disable brush rotation"), FALSE,
      GIMP_PARAM_STATIC_STRINGS | GIMP_SYMMETRY_PARAM_GUI);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_ENABLE_REFLECTION,
                           "enable-reflection", _("Kaleidoscope"),
                           _("Reflect consecutive strokes"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS | GIMP_SYMMETRY_PARAM_GUI);
}

static void gimp_mandala_init(GimpMandala *mandala) {}

static void gimp_mandala_constructed(GObject *object) {
  GimpSymmetry *sym = GIMP_SYMMETRY(object);

  g_signal_connect_object(sym->image, "size-changed-detailed",
                          G_CALLBACK(gimp_mandala_image_size_changed_cb), sym,
                          0);
}
    ...
    if (g_value_get_double(value) > 0.0 &&
      ...
      if (mandala->vertical_guide) {
        g_signal_handlers_block_by_func(
            mandala->vertical_guide, gimp_mandala_guide_position_cb, mandala);
        gimp_image_move_guide(image, mandala->vertical_guide, mandala->center_x,
                              FALSE);
  ...
  case PROP_CENTER_Y:
    if (g_value_get_double(value) > 0.0 &&
      ...
      if (mandala->horizontal_guide) {
        g_signal_handlers_block_by_func(
            mandala->horizontal_guide, gimp_mandala_guide_position_cb, mandala);
        gimp_image_move_guide(image, mandala->horizontal_guide,
                              mandala->center_y, FALSE);
        g_signal_handlers_unblock_by_func(
            mandala->horizontal_guide, gimp_mandala_guide_position_cb, mandala);
      }
    }
...
static void gimp_mandala_add_guide(GimpMandala *mandala,
  ...
  g_signal_connect(guide, "removed", G_CALLBACK(gimp_mandala_guide_removed_cb),
                   mandala);

  gimp_image_add_guide(image, guide, position);

  g_signal_connect(guide, "notify::position",
                   G_CALLBACK(gimp_mandala_guide_position_cb), mandala);
}
...
static void gimp_mandala_remove_guide(GimpMandala *mandala,
  ...
  guide = (orientation == GIMP_ORIENTATION_HORIZONTAL)
              ? mandala->horizontal_guide
              : mandala->vertical_guide;

  g_signal_handlers_disconnect_by_func(guide, gimp_mandala_guide_removed_cb,
                                       mandala);
  g_signal_handlers_disconnect_by_func(guide, gimp_mandala_guide_position_cb,
                                       mandala);
...
static void gimp_mandala_guide_removed_cb(GObject *object,
  ...
  g_signal_handlers_disconnect_by_func(object, gimp_mandala_guide_removed_cb,
                                       mandala);
  g_signal_handlers_disconnect_by_func(object, gimp_mandala_guide_position_cb,
                                       mandala);
...
static void gimp_mandala_guide_position_cb(GObject *object, GParamSpec *pspec,
  ...
  if (guide == mandala->horizontal_guide) {
    g_object_set(G_OBJECT(mandala), "center-y",
                 (gdouble)gimp_guide_get_position(guide), NULL);
  } else if (guide == mandala->vertical_guide) {
    g_object_set(G_OBJECT(mandala), "center-x",
                 (gdouble)gimp_guide_get_position(guide), NULL);
  }
}

static void gimp_mandala_update_strokes(GimpSymmetry *sym,
                                        GimpDrawable *drawable,
                                        GimpCoords *origin) {
  GimpMandala *mandala = GIMP_MANDALA(sym);
  GimpCoords *coords;
  ...
  if (mandala->enable_reflection) {
    ...
    gdouble angle = atan2(sym->origin->y - center_y, sym->origin->x - center_x);
  ...
  for (i = 1; i < mandala->size; i++) {
    ...
    if (mandala->enable_reflection && i % 2 == 1) {
      /* Reflecting over the mid_slice_angle axis, reflects slice without
       * changing position. */
    ...
    gimp_matrix3_translate(&matrix, +center_x, +center_y);
    gimp_matrix3_transform_point(&matrix, coords->x, coords->y, &new_x, &new_y);
    coords->x = new_x;
    coords->y = new_y;
    sym->strokes = g_list_prepend(sym->strokes, coords);
  }
...
static void gimp_mandala_get_transform(GimpSymmetry *sym, gint stroke,
                                       gdouble *angle, gboolean *reflect) {
  GimpMandala *mandala = GIMP_MANDALA(sym);
  gdouble slice_angle;
  ...
  if (mandala->enable_reflection && stroke % 2 == 1) {
    /* Find out in which slice the user is currently drawing. */
    gdouble origin_angle =
        gimp_rad_to_deg(atan2(sym->origin->y - mandala->center_y,
