...
void gimp_image_crop(GimpImage *image, GimpContext *context,
  ...
  gimp_image_undo_push_image_size(image, NULL, x, y, width, height);

  /*  Set the new width and height  */
  g_object_set(image, "width", width, "height", height, NULL);

  /*  Resize all channels  */
  for (list = gimp_image_get_channel_iter(image); list;
       list = g_list_next(list)) {
    GimpItem *item = list->data;

    gimp_item_resize(item, context, GIMP_FILL_TRANSPARENT, width, height, -x,
                     -y);
  }
  ...
  while (list) {
    ...
    if (crop_layers && !gimp_item_is_content_locked(item)) {
      ...
      lx2 = CLAMP(gimp_item_get_width(item) + off_x, 0,
                  gimp_image_get_width(image));
      ly2 = CLAMP(gimp_item_get_height(item) + off_y, 0,
      ...
      if (width > 0 && height > 0) {
        gimp_item_resize(item, context, fill_type, width, height,
                         -(lx1 - off_x), -(ly1 - off_y));
      } else {
  ...
  gimp_image_size_changed_detailed(image, -x, -y, previous_width,
                                   previous_height);
