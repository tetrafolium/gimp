...
enum { PROP_0, PROP_PREV_PARENT, PROP_PREV_POSITION, PROP_PREV_LAYERS };

static void gimp_layer_undo_constructed(GObject *object);
static void gimp_layer_undo_finalize(GObject *object);
static void gimp_layer_undo_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_layer_undo_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);

static gint64 gimp_layer_undo_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_layer_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
...
static void gimp_layer_undo_class_init(GimpLayerUndoClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_PREV_PARENT,
      g_param_spec_object("prev-parent", NULL, NULL, GIMP_TYPE_LAYER,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_PREV_POSITION,
      g_param_spec_int("prev-position", NULL, NULL, 0, G_MAXINT, 0,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_PREV_LAYERS,
      g_param_spec_pointer("prev-layers", NULL, NULL,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_layer_undo_init(GimpLayerUndo *undo) {
...
static gint64 gimp_layer_undo_get_memsize(GimpObject *object,
  ...
  if (!gimp_item_is_attached(item_undo->item))
    memsize += gimp_object_get_memsize(GIMP_OBJECT(item_undo->item), gui_size);
...
static void gimp_layer_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
  ...
  if ((undo_mode == GIMP_UNDO_MODE_UNDO &&
    ...
    gimp_image_remove_layer(undo->image, layer, FALSE, layer_undo->prev_layers);
  } else {
    /*  restore layer  */
    ...
    layer_undo->prev_layers =
        g_list_copy(gimp_image_get_selected_layers(undo->image));

    gimp_image_add_layer(undo->image, layer, layer_undo->prev_parent,
