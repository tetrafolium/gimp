...
static void gimp_coords_interpolate_bezier_internal(
    const GimpCoords bezier_pt[4], const gdouble start_t, const gdouble end_t,
    const gdouble precision, GArray *ret_coords, GArray *ret_params,
    gint depth);
...
void gimp_coords_interpolate_bezier(const GimpCoords bezier_pt[4],
                                    const gdouble precision, GArray *ret_coords,
                                    GArray *ret_params) {
  g_return_if_fail(bezier_pt != NULL);
  g_return_if_fail(precision > 0.0);
  g_return_if_fail(ret_coords != NULL);

  gimp_coords_interpolate_bezier_internal(bezier_pt, 0.0, 1.0, precision,
...
static void gimp_coords_interpolate_bezier_internal(
      ...
      gimp_coords_bezier_is_straight(subdivided, precision)) /* 1st half */
  {
    ...
    gimp_coords_interpolate_bezier_internal(subdivided, start_t,
                                            (start_t + end_t) / 2, precision,
      ...
      gimp_coords_bezier_is_straight(subdivided + 3, precision)) /* 2nd half */
  {
    ...
    gimp_coords_interpolate_bezier_internal(
        subdivided + 3, (start_t + end_t) / 2, end_t, precision, ret_coords,
        ret_params, depth - 1);
  }
}
...
void gimp_coords_interpolate_bezier_at(const GimpCoords bezier_pt[4], gdouble t,
                                       GimpCoords *position,
                                       GimpCoords *velocity) {
  gdouble u = 1.0 - t;
  ...
  if (position) {
    ...
    gimp_coords_mix(u * u * u, &bezier_pt[0], 3.0 * u * u * t, &bezier_pt[1],
                    &a);
    gimp_coords_mix(3.0 * u * t * t, &bezier_pt[2], t * t * t, &bezier_pt[3],
                    &b);
  ...
  if (velocity) {
    ...
    gimp_coords_mix(-3.0 * u * u, &bezier_pt[0], 3.0 * (u - 2.0 * t) * u,
                    &bezier_pt[1], &a);
    gimp_coords_mix(-3.0 * (t - 2.0 * u) * t, &bezier_pt[2], 3.0 * t * t,
                    &bezier_pt[3], &b);
...
gboolean gimp_coords_bezier_is_straight(const GimpCoords bezier_pt[4],
  ...
  gimp_coords_mix(2.0 / 3.0, &bezier_pt[0], 1.0 / 3.0, &bezier_pt[3], &pt1);
  gimp_coords_mix(1.0 / 3.0, &bezier_pt[0], 2.0 / 3.0, &bezier_pt[3], &pt2);
...
void gimp_coords_interpolate_catmull(const GimpCoords catmull_pt[4],
                                     gdouble precision, GArray *ret_coords,
                                     GArray *ret_params) {
  ...
  for (n = 1; n <= num_points; n++) {
    ...
    coords.x = gimp_coords_get_catmull_spline_point(
        p, past_coords.x, start_coords.x, end_coords.x, future_coords.x);

    coords.y = gimp_coords_get_catmull_spline_point(
        p, past_coords.y, start_coords.y, end_coords.y, future_coords.y);

    pressure = gimp_coords_get_catmull_spline_point(
        p, past_coords.pressure, start_coords.pressure, end_coords.pressure,
        future_coords.pressure);
    coords.pressure = CLAMP(pressure, 0.0, 1.0);

    coords.xtilt = gimp_coords_get_catmull_spline_point(
        p, past_coords.xtilt, start_coords.xtilt, end_coords.xtilt,
        future_coords.xtilt);
    coords.ytilt = gimp_coords_get_catmull_spline_point(
        p, past_coords.ytilt, start_coords.ytilt, end_coords.ytilt,
        future_coords.ytilt);

    coords.wheel = gimp_coords_get_catmull_spline_point(
        p, past_coords.wheel, start_coords.wheel, end_coords.wheel,
        future_coords.wheel);

    velocity = gimp_coords_get_catmull_spline_point(
        p, past_coords.velocity, start_coords.velocity, end_coords.velocity,
