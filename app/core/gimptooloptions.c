...
enum { PROP_0, PROP_TOOL, PROP_TOOL_INFO };
...
static void gimp_tool_options_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_tool_options_get_property(GObject *object, guint property_id,
...
G_DEFINE_TYPE_WITH_CODE(
    GimpToolOptions, gimp_tool_options, GIMP_TYPE_CONTEXT,
    G_IMPLEMENT_INTERFACE(GIMP_TYPE_CONFIG,
                          gimp_tool_options_config_iface_init))

#define parent_class gimp_tool_options_parent_class

static void gimp_tool_options_class_init(GimpToolOptionsClass *klass) {
  ...
  g_object_class_install_property(object_class, PROP_TOOL_INFO,
                                  g_param_spec_object("tool-info", NULL, NULL,
                                                      GIMP_TYPE_TOOL_INFO,
...
static void gimp_tool_options_init(GimpToolOptions *options) {
  ...
  g_signal_connect(options, "notify::tool",
                   G_CALLBACK(gimp_tool_options_tool_notify), NULL);
}
 ...
 */
static GimpToolInfo *gimp_tool_options_check_tool_info(GimpToolOptions *options,
  ...
  if (tool_info && G_OBJECT_TYPE(options) == tool_info->tool_options_type) {
    ...
    for (list = gimp_get_tool_info_iter(GIMP_CONTEXT(options)->gimp); list;
...
static void gimp_tool_options_set_property(GObject *object, guint property_id,
  ...
  case PROP_TOOL: {
    ...
    g_return_if_fail(context_tool == NULL || context_tool == tool_info);
...
static void gimp_tool_options_tool_notify(GimpToolOptions *options,
  ...
  if (tool_info && new_info != tool_info)
    g_warning("%s: 'tool' property on %s was set to bogus value "
              "'%s', it MUST be '%s'.",
              G_STRFUNC, g_type_name(G_OBJECT_TYPE(options)),
              gimp_object_get_name(tool_info), gimp_object_get_name(new_info));
}
...
gboolean gimp_tool_options_serialize(GimpToolOptions *tool_options,
  ...
  retval = gimp_config_serialize_to_file(GIMP_CONFIG(tool_options), file,
                                         header, footer, NULL, error);
...
gboolean gimp_tool_options_deserialize(GimpToolOptions *tool_options,
  ...
  retval = gimp_config_deserialize_file(GIMP_CONFIG(tool_options), file, NULL,
                                        error);
