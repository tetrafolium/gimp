...
static void gimp_backtrace_read_thread_name(pid_t tid, gchar *name, gint size);
...
static const gchar *const blacklisted_thread_names[] = {"gmain", "threaded-ml"};
...
static void gimp_backtrace_signal_handler(gint signum) {
  ...
  if (curr_backtrace) {
    ...
    for (i = 0; i < curr_backtrace->n_threads; i++) {
      ...
      if (thread->tid == tid) {
        thread->n_frames = backtrace((gpointer *)thread->frames, MAX_N_FRAMES);
...
gboolean gimp_backtrace_start(void) {
  ...
  if (n_initializations == 0) {
    ...
    if (sigaction(BACKTRACE_SIGNAL, &action, &orig_action) == 0) {
      ...
      n_threads =
          gimp_backtrace_enumerate_threads(TRUE, threads, MAX_N_THREADS);

      for (i = 0; i < n_threads; i++) {
        gchar name[MAX_THREAD_NAME_SIZE];
        gint j;

        gimp_backtrace_read_thread_name(threads[i], name, MAX_THREAD_NAME_SIZE);
...
GimpBacktrace *gimp_backtrace_new(gboolean include_current_thread) {
  ...
  n_threads = gimp_backtrace_enumerate_threads(include_current_thread, threads,
                                               MAX_N_THREADS);
  ...
  for (i = 0; i < n_threads; i++) {
    ...
    gimp_backtrace_read_thread_name(thread->tid, thread->name,
...
guintptr gimp_backtrace_get_frame_address(GimpBacktrace *backtrace, gint thread,
  ...
  g_return_val_if_fail(frame >= N_SKIPPED_FRAMES &&
                           frame < backtrace->threads[thread].n_frames,
                       0);
...
#ifdef HAVE_LIBBACKTRACE
static void gimp_backtrace_syminfo_callback(GimpBacktraceAddressInfo *info,
                                            guintptr pc, const gchar *symname,
                                            guintptr symval, guintptr symsize) {
  if (symname)
...
static gint gimp_backtrace_pcinfo_callback(GimpBacktraceAddressInfo *info,
                                           guintptr pc, const gchar *filename,
                                           gint lineno, const gchar *function) {
  if (function)
...
gboolean gimp_backtrace_get_address_info(guintptr address,
  ...
  if (backtrace_state) {
    backtrace_syminfo(
        ...
        (backtrace_syminfo_callback)gimp_backtrace_syminfo_callback, NULL,
        info);

    backtrace_pcinfo(backtrace_state, address,
...
#ifdef HAVE_LIBUNWIND
  /* we use libunwind to get the symbol name, when available, even if dladdr()
   * or libbacktrace already found one, since it provides more descriptive names
   * in some cases, and, in particular, full symbol names for C++ lambdas.
   *
...
#endif
  {
    ...
    if (unw_init_local(&cursor, &context) == 0 &&
        ...
        unw_get_proc_name(&cursor, info->symbol_name, sizeof(info->symbol_name),
                          &offset) == 0) {
