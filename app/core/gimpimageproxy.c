...
enum { PROP_0, PROP_IMAGE, PROP_SHOW_ALL, PROP_BUFFER };
...
static void gimp_image_proxy_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_image_proxy_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);

static gboolean gimp_image_proxy_get_size(GimpViewable *viewable, gint *width,
                                          gint *height);
static void gimp_image_proxy_get_preview_size(GimpViewable *viewable, gint size,
                                              gboolean is_popup,
                                              gboolean dot_for_dot, gint *width,
                                              gint *height);
...
static gboolean gimp_image_proxy_get_pixel_at(GimpPickable *pickable, gint x,
                                              gint y, const Babl *format,
                                              gpointer pixel);
static gdouble gimp_image_proxy_get_opacity_at(GimpPickable *pickable, gint x,
                                               gint y);
...
static void gimp_image_proxy_image_size_changed(GimpImage *image,
                                                GimpImageProxy *image_proxy);
static void gimp_image_proxy_image_bounds_changed(GimpImage *image, gint old_x,
                                                  gint old_y,
                                                  GimpImageProxy *image_proxy);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpImageProxy, gimp_image_proxy, GIMP_TYPE_VIEWABLE,
...
static void gimp_image_proxy_class_init(GimpImageProxyClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_SHOW_ALL,
      g_param_spec_boolean("show-all", NULL, NULL, FALSE,
...
static void gimp_image_proxy_init(GimpImageProxy *image_proxy) {
...
static void gimp_image_proxy_set_property(GObject *object, guint property_id,
  ...
  switch (property_id) {
  case PROP_IMAGE:
    gimp_image_proxy_set_image(image_proxy, g_value_get_object(value));
    break;

  case PROP_SHOW_ALL:
    gimp_image_proxy_set_show_all(image_proxy, g_value_get_boolean(value));
    break;
...
static void gimp_image_proxy_get_property(GObject *object, guint property_id,
  ...
  case PROP_SHOW_ALL:
    g_value_set_boolean(value, gimp_image_proxy_get_show_all(image_proxy));
    break;

  case PROP_BUFFER:
    g_value_set_object(value,
                       gimp_pickable_get_buffer(GIMP_PICKABLE(image_proxy)));
    break;
...
static void gimp_image_proxy_get_preview_size(GimpViewable *viewable, gint size,
  ...
  gimp_viewable_calc_preview_size(viewable_width, viewable_height, size, size,
                                  dot_for_dot, xres, yres, width, height, NULL);
}
...
static gboolean gimp_image_proxy_get_popup_size(GimpViewable *viewable,
  ...
  if (viewable_width > width || viewable_height > height) {
    ...
    gimp_viewable_calc_preview_size(viewable_width, viewable_height, width * 2,
                                    height * 2, dot_for_dot, 1.0, 1.0,
                                    popup_width, popup_height, &scaling_up);
...
static GimpTempBuf *gimp_image_proxy_get_new_preview(GimpViewable *viewable,
  ...
  gegl_buffer_get(gimp_pickable_get_buffer(pickable),
                  GEGL_RECTANGLE(bounding_box.x * scale, bounding_box.y * scale,
                                 width, height),
                  scale, gimp_temp_buf_get_format(buf),
                  gimp_temp_buf_get_data(buf), GEGL_AUTO_ROWSTRIDE,
                  GEGL_ABYSS_CLAMP);
...
static GdkPixbuf *gimp_image_proxy_get_new_pixbuf(GimpViewable *viewable,
  ...
  if (transform) {
    ...
    temp_buf =
        gimp_temp_buf_new(width, height, gimp_pickable_get_format(pickable));

    gegl_buffer_get(gimp_pickable_get_buffer(pickable),
                    GEGL_RECTANGLE(bounding_box.x * scale,
                                   bounding_box.y * scale, width, height),
    ...
    gimp_color_transform_process_buffer(transform, src_buf,
                                        GEGL_RECTANGLE(0, 0, width, height),
                                        dest_buf, GEGL_RECTANGLE(0, 0, 0, 0));
    ...
    gegl_buffer_get(gimp_pickable_get_buffer(pickable),
                    GEGL_RECTANGLE(bounding_box.x * scale,
                                   bounding_box.y * scale, width, height),
                    scale, gimp_pixbuf_get_format(pixbuf),
                    gdk_pixbuf_get_pixels(pixbuf),
                    gdk_pixbuf_get_rowstride(pixbuf), GEGL_ABYSS_CLAMP);
  }
...
static const guint8 *gimp_image_proxy_get_icc_profile(GimpColorManaged *managed,
  ...
  return gimp_color_managed_get_icc_profile(
      GIMP_COLOR_MANAGED(image_proxy->priv->image), len);
}
...
static void gimp_image_proxy_set_image(GimpImageProxy *image_proxy,
  ...
  if (image_proxy->priv->image) {
    g_signal_handlers_disconnect_by_func(image_proxy->priv->image,
                                         gimp_image_proxy_image_frozen_notify,
                                         image_proxy);
    g_signal_handlers_disconnect_by_func(
        image_proxy->priv->image, gimp_image_proxy_image_invalidate_preview,
        image_proxy);
    g_signal_handlers_disconnect_by_func(image_proxy->priv->image,
  ...
  if (image_proxy->priv->image) {
    ...
    g_signal_connect(image_proxy->priv->image, "notify::frozen",
                     G_CALLBACK(gimp_image_proxy_image_frozen_notify),
                     image_proxy);
    g_signal_connect(image_proxy->priv->image, "invalidate-preview",
                     G_CALLBACK(gimp_image_proxy_image_invalidate_preview),
                     image_proxy);
    g_signal_connect(image_proxy->priv->image, "size-changed",
                     G_CALLBACK(gimp_image_proxy_image_size_changed),
                     image_proxy);
    g_signal_connect(image_proxy->priv->image, "bounds-changed",
                     G_CALLBACK(gimp_image_proxy_image_bounds_changed),
                     image_proxy);
    g_signal_connect(image_proxy->priv->image, "profile-changed",
...
static void gimp_image_proxy_update_bounding_box(GimpImageProxy *image_proxy) {
  ...
  if (!gegl_rectangle_equal(&bounding_box, &image_proxy->priv->bounding_box)) {
...
static void gimp_image_proxy_update_frozen(GimpImageProxy *image_proxy) {
  ...
  frozen =
      gimp_viewable_preview_is_frozen(GIMP_VIEWABLE(image_proxy->priv->image));
...
GimpImageProxy *gimp_image_proxy_new(GimpImage *image) {
  ...
  return g_object_new(GIMP_TYPE_IMAGE_PROXY, "image", image, NULL);
}
