...
enum { PROP_0, PROP_CONVERT_FORMAT };

static void gimp_mask_undo_constructed(GObject *object);
static void gimp_mask_undo_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_mask_undo_get_property(GObject *object, guint property_id,
                                        GValue *value, GParamSpec *pspec);

static gint64 gimp_mask_undo_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_mask_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
                               GimpUndoAccumulator *accum);
static void gimp_mask_undo_free(GimpUndo *undo, GimpUndoMode undo_mode);

G_DEFINE_TYPE(GimpMaskUndo, gimp_mask_undo, GIMP_TYPE_ITEM_UNDO)

#define parent_class gimp_mask_undo_parent_class

static void gimp_mask_undo_class_init(GimpMaskUndoClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_CONVERT_FORMAT,
      g_param_spec_boolean("convert-format", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_mask_undo_init(GimpMaskUndo *undo) {}
...
static void gimp_mask_undo_constructed(GObject *object) {
  ...
  if (gimp_item_bounds(item, &mask_undo->bounds.x, &mask_undo->bounds.y,
                       &mask_undo->bounds.width, &mask_undo->bounds.height)) {
    ...
    mask_undo->buffer = gegl_buffer_new(
        GEGL_RECTANGLE(0, 0, rect.width, rect.height), mask_undo->format);

    gimp_gegl_buffer_copy(buffer, &rect, GEGL_ABYSS_NONE, mask_undo->buffer,
...
static void gimp_mask_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
  ...
  if (gimp_item_bounds(item, &bounds.x, &bounds.y, &bounds.width,
                       &bounds.height)) {
    GeglBuffer *buffer = gimp_drawable_get_buffer(drawable);
    ...
    gimp_gegl_buffer_copy(buffer, &rect, GEGL_ABYSS_NONE, new_buffer,
                          GEGL_RECTANGLE(0, 0, 0, 0));
  ...
  if (mask_undo->convert_format) {
    ...
    buffer =
        gegl_buffer_new(GEGL_RECTANGLE(0, 0, width, height), mask_undo->format);
  ...
  if (mask_undo->buffer) {
    gimp_gegl_buffer_copy(mask_undo->buffer, NULL, GEGL_ABYSS_NONE,
