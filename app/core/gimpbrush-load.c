...
static GimpBrush *gimp_brush_load_abr_brush_v12(GDataInputStream *input,
                                                AbrHeader *abr_hdr, gint index,
                                                GFile *file, GError **error);
static GimpBrush *gimp_brush_load_abr_brush_v6(GDataInputStream *input,
                                               AbrHeader *abr_hdr,
                                               gint32 max_offset, gint index,
                                               GFile *file, GError **error);

static gchar abr_read_char(GDataInputStream *input, GError **error);
static gint16 abr_read_short(GDataInputStream *input, GError **error);
static gint32 abr_read_long(GDataInputStream *input, GError **error);
static gchar *abr_read_ucs2_text(GDataInputStream *input, GError **error);
static gboolean abr_supported(AbrHeader *abr_hdr, GError **error);
static gboolean abr_reach_8bim_section(GDataInputStream *input,
                                       const gchar *name, GError **error);
static gboolean abr_rle_decode(GDataInputStream *input, gchar *buffer,
                               gsize buffer_size, gint32 height,
                               GError **error);
...
GimpBrush *gimp_brush_load_brush(GimpContext *context, GFile *file,
  ...
  case 1:
    ...
    if (!g_seekable_seek(G_SEEKABLE(input), -8, G_SEEK_CUR, NULL, error))
      return NULL;
  ...
  if ((bn_size = (header.header_size - sizeof(header)))) {
    ...
    if (bn_size > GIMP_BRUSH_MAX_NAME) {
      g_set_error(error, GIMP_DATA_ERROR, GIMP_DATA_ERROR_READ,
                  ...
                  gimp_file_get_utf8_name(file), (gulong)bn_size);
      return NULL;
    }

    name = g_new0(gchar, bn_size + 1);

    if (!g_input_stream_read_all(input, name, bn_size, &bytes_read, NULL,
                                 error) ||
        bytes_read != bn_size) {
      g_free(name);
      return NULL;
  ...
  brush = g_object_new(GIMP_TYPE_BRUSH, "name", name, "mime-type",
                       "image/x-gimp-gbr", NULL);
  g_free(name);

  brush->priv->mask =
      gimp_temp_buf_new(header.width, header.height, babl_format("Y u8"));
  ...
  case 1:
    success =
        (g_input_stream_read_all(input, mask, size, &bytes_read, NULL, error) &&
         bytes_read == size);
    ...
    if (success) {
      ...
      if (g_input_stream_read_all(input, &ph, sizeof(GimpPatternHeader),
        ...
        if (ph.magic_number == GIMP_PATTERN_MAGIC && ph.version == 1 &&
            ph.header_size > sizeof(GimpPatternHeader) && ph.bytes == 3 &&
            ph.width == header.width && ph.height == header.height &&
            g_input_stream_skip(
                input, ph.header_size - sizeof(GimpPatternHeader), NULL,
                NULL) == ph.header_size - sizeof(GimpPatternHeader)) {
          guchar *pixmap;
          gssize pixmap_size;
          ...
          success = (g_input_stream_read_all(input, pixmap, pixmap_size,
                                             &bytes_read, NULL, error) &&
                     bytes_read == pixmap_size);
        } else {
          /*  seek back if pattern wasn't found  */
          success = g_seekable_seek(G_SEEKABLE(input), rewind, G_SEEK_SET, NULL,
                                    error);
        }
      }
    }
  ...
  case 2: /*  cinepaint brush, 16 bit floats  */
  {
    ...
    for (i = 0; success && i < size;) {
      ...
      success = (g_input_stream_read_all(input, buf, bytes, &bytes_read, NULL,
                                         error) &&
                 bytes_read == bytes);
...
GList *gimp_brush_load_abr(GimpContext *context, GFile *file,
  ...
  if (abr_supported(&abr_hdr, &my_error)) {
    ...
    case 2:
      brush_list =
          gimp_brush_load_abr_v12(data_input, &abr_hdr, file, &my_error);
      break;

    case 10:
    case 6:
      brush_list =
          gimp_brush_load_abr_v6(data_input, &abr_hdr, file, &my_error);
      break;
    }
  }
...
static GList *gimp_brush_load_abr_v12(GDataInputStream *input,
  ...
  for (i = 0; i < abr_hdr->count; i++) {
    ...
    brush = gimp_brush_load_abr_brush_v12(input, abr_hdr, i, file, &my_error);
...
static GList *gimp_brush_load_abr_v6(GDataInputStream *input,
  ...
  sample_section_end =
      (sample_section_size + g_seekable_tell(G_SEEKABLE(input)));

  while (g_seekable_tell(G_SEEKABLE(input)) < sample_section_end) {
    GimpBrush *brush;
    GError *my_error = NULL;

    brush = gimp_brush_load_abr_brush_v6(input, abr_hdr, sample_section_end, i,
                                         file, &my_error);
...
static GimpBrush *gimp_brush_load_abr_brush_v12(GDataInputStream *input,
  ...
  case 1: /* computed brush */
    ...
    g_seekable_seek(G_SEEKABLE(input), abr_brush_hdr.size, G_SEEK_CUR, NULL,
                    NULL);
    break;
  ...
  case 2: /* sampled brush */
  {
    ...
    if (width < 1 || width > 10000 || height < 1 || height > 10000 ||
        bytes < 1 || bytes > 1 || G_MAXSIZE / width / height / bytes < 1) {
      g_set_error(error, GIMP_DATA_ERROR, GIMP_DATA_ERROR_READ,
                  _("Fatal parse error in brush file: "
    ...
    brush = g_object_new(GIMP_TYPE_BRUSH, "name", name,
                         /*  FIXME: MIME type!!  */
                         "mime-type", "application/x-photoshop-abr", NULL);
    ...
    brush->priv->mask = gimp_temp_buf_new(width, height, babl_format("Y u8"));
    ...
    if (!compress) {
      ...
      if (!g_input_stream_read_all(G_INPUT_STREAM(input), mask, size,
                                   &bytes_read, NULL, error) ||
          bytes_read != size) {
        g_object_unref(brush);
        brush = NULL;
...
static GimpBrush *gimp_brush_load_abr_brush_v6(GDataInputStream *input,
  ...
  if (abr_hdr->count == 1) {
    ...
    r = g_seekable_seek(G_SEEKABLE(input), 47, G_SEEK_CUR, NULL, error);
  } else {
    /* discard key and unknown bytes */
    r = g_seekable_seek(G_SEEKABLE(input), 301, G_SEEK_CUR, NULL, error);
  }

  if (!r) {
    g_prefix_error(error, _("Fatal parse error in brush file: "
                            "File appears truncated: "));
    return NULL;
  }

  top = abr_read_long(input, error);
  if (error && *error)
    return NULL;
  left = abr_read_long(input, error);
  if (error && *error)
    return NULL;
  bottom = abr_read_long(input, error);
  if (error && *error)
    return NULL;
  right = abr_read_long(input, error);
  if (error && *error)
    return NULL;
  depth = abr_read_short(input, error);
  if (error && *error)
    return NULL;
  compress = abr_read_char(input, error);
  if (error && *error)
  ...
  if (width < 1 || width > 10000 || height < 1 || height > 10000 || depth < 1 ||
      depth > 1 || G_MAXSIZE / width / height / depth < 1) {
    g_set_error(error, GIMP_DATA_ERROR, GIMP_DATA_ERROR_READ,
                _("Fatal parse error in brush file: "
                  "Brush dimensions out of range."));
    return NULL;
  ...
  if (!compress) {
    ...
    if (!g_input_stream_read_all(G_INPUT_STREAM(input), mask, size, &bytes_read,
                                 NULL, error) ||
        bytes_read != size) {
      g_object_unref(brush);
      return NULL;
  ...
  if (g_seekable_tell(G_SEEKABLE(input)) <= next_brush)
    g_seekable_seek(G_SEEKABLE(input), next_brush, G_SEEK_SET, NULL, NULL);
...
static gchar *abr_read_ucs2_text(GDataInputStream *input, GError **error) {
  ...
  name_utf8 = g_convert(name_ucs2, len, "UTF-8", "UCS-2BE", NULL, NULL, NULL);
...
static gboolean abr_reach_8bim_section(GDataInputStream *input,
  ...
  while (TRUE) {
    ...
    if (!g_input_stream_read_all(G_INPUT_STREAM(input), tag, 4, &bytes_read,
                                 NULL, error) ||
        bytes_read != 4)
      return FALSE;
    ...
    if (!g_input_stream_read_all(G_INPUT_STREAM(input), tagname, 4, &bytes_read,
                                 NULL, error) ||
        bytes_read != 4)
      return FALSE;
    ...
    if (!g_seekable_seek(G_SEEKABLE(input), section_size, G_SEEK_CUR, NULL,
                         error))
      return FALSE;
  }
...
static gboolean abr_rle_decode(GDataInputStream *input, gchar *buffer,
  ...
  for (i = 0; i < height; i++) {
    ...
    if (!g_input_stream_read_all(G_INPUT_STREAM(input), cdata, len, &bytes_read,
                                 NULL, error) ||
        bytes_read != len) {
      goto err;
    }
