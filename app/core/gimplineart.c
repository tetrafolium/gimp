...
static void gimp_line_art_set_property(GObject *object, guint property_id,
                                       const GValue *value, GParamSpec *pspec);
static void gimp_line_art_get_property(GObject *object, guint property_id,
                                       GValue *value, GParamSpec *pspec);

/* Functions for asynchronous computation. */

static void gimp_line_art_compute(GimpLineArt *line_art);
static void gimp_line_art_compute_cb(GimpAsync *async, GimpLineArt *line_art);
...
static GeglBuffer *gimp_line_art_close(
    GeglBuffer *buffer, gboolean select_transparent, gdouble stroke_threshold,
    gint spline_max_length, gint segment_max_length, gint minimal_lineart_area,
    gint normal_estimate_mask_size, gfloat end_point_rate,
    gfloat spline_max_angle, gint end_point_connectivity,
    gfloat spline_roundness, gboolean allow_self_intersections,
    gint created_regions_significant_area, gint created_regions_minimum_area,
    gboolean small_segments_from_spline_sources, gfloat **lineart_distmap,
    GimpAsync *async);

static void gimp_lineart_denoise(GeglBuffer *buffer, int size,
                                 GimpAsync *async);
static void gimp_lineart_compute_normals_curvatures(
    GeglBuffer *mask, gfloat *normals, gfloat *curvatures,
    gfloat *smoothed_curvatures, int normal_estimate_mask_size,
    GimpAsync *async);
...
static GList *gimp_lineart_find_spline_candidates(GArray *max_positions,
                                                  gfloat *normals, gint width,
                                                  gint distance_threshold,
                                                  gfloat max_angle_deg,
                                                  GimpAsync *async);

static GArray *gimp_lineart_discrete_spline(Pixel p0, GimpVector2 n0, Pixel p1,
                                            GimpVector2 n1);

static gint gimp_number_of_transitions(GArray *pixels, GeglBuffer *buffer);
static gboolean gimp_line_art_allow_closure(GeglBuffer *mask, GArray *pixels,
                                            GList **fill_pixels,
...
static gfloat *gimp_lineart_estimate_strokes_radii(GeglBuffer *mask,
                                                   GimpAsync *async);
static void gimp_line_art_simple_fill(GeglBuffer *buffer, gint x, gint y,
                                      gint *counter);

/* Some callback-type functions. */

static guint visited_hash_fun(Pixel *key);
static gboolean visited_equal_fun(Pixel *e1, Pixel *e2);

static inline gboolean border_in_direction(GeglBuffer *mask, Pixel p,
                                           int direction);
static inline GimpVector2 pair2normal(Pixel p, gfloat *normals, gint width);

/* Edgel */

static Edgel *gimp_edgel_new(int x, int y, Direction direction);
static void gimp_edgel_init(Edgel *edgel);
static void gimp_edgel_clear(Edgel **edgel);
static int gimp_edgel_cmp(const Edgel *e1, const Edgel *e2);
static guint edgel2index_hash_fun(Edgel *key);
static gboolean edgel2index_equal_fun(Edgel *e1, Edgel *e2);

static glong gimp_edgel_track_mark(GeglBuffer *mask, Edgel edgel,
                                   long size_limit);
static glong gimp_edgel_region_area(const GeglBuffer *mask, Edgel start_edgel);

/* Edgel set */

static GArray *gimp_edgelset_new(GeglBuffer *buffer, GimpAsync *async);
static void gimp_edgelset_add(GArray *set, int x, int y, Direction direction,
                              GHashTable *edgel2index);
static void gimp_edgelset_init_normals(GArray *set);
static void gimp_edgelset_smooth_normals(GArray *set, int mask_size,
                                         GimpAsync *async);
static void gimp_edgelset_compute_curvature(GArray *set, GimpAsync *async);

static void gimp_edgelset_build_graph(GArray *set, GeglBuffer *buffer,
                                      GHashTable *edgel2index,
                                      GimpAsync *async);
static void gimp_edgelset_next8(const GeglBuffer *buffer, Edgel *it, Edgel *n);
...
static void gimp_line_art_class_init(GimpLineArtClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  gimp_line_art_signals[COMPUTING_START] = g_signal_new(
      "computing-start", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLineArtClass, computing_start), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  gimp_line_art_signals[COMPUTING_END] = g_signal_new(
      "computing-end", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpLineArtClass, computing_end), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      object_class, PROP_SELECT_TRANSPARENT,
      g_param_spec_boolean("select-transparent",
                           _("Select transparent pixels instead of gray ones"),
                           _("Select transparent pixels instead of gray ones"),
                           TRUE, G_PARAM_CONSTRUCT | GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_THRESHOLD,
      g_param_spec_double("threshold", _("Line art detection threshold"),
  ...
  g_object_class_install_property(
      object_class, PROP_MAX_GROW,
      g_param_spec_int("max-grow", _("Maximum growing size"),
                       _("Maximum number of pixels grown under the line art"),
                       1, 100, 3, G_PARAM_CONSTRUCT | GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_SPLINE_MAX_LEN,
      g_param_spec_int(
          "spline-max-length", _("Maximum curved closing length"),
          _("Maximum curved length (in pixels) to close the line art"), 0, 1000,
          100, G_PARAM_CONSTRUCT | GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_SEGMENT_MAX_LEN,
      g_param_spec_int(
          "segment-max-length", _("Maximum straight closing length"),
          _("Maximum straight length (in pixels) to close the line art"), 0,
          1000, 100, G_PARAM_CONSTRUCT | GIMP_PARAM_READWRITE));
}

static void gimp_line_art_init(GimpLineArt *line_art) {
...
GimpLineArt *gimp_line_art_new(void) {
  return g_object_new(GIMP_TYPE_LINE_ART, NULL);
}
...
static void gimp_line_art_compute(GimpLineArt *line_art) {
  ...
  if (line_art->priv->input) {
    ...
    g_signal_handlers_block_by_func(
        line_art->priv->input,
        G_CALLBACK(gimp_line_art_input_invalidate_preview), line_art);
    ...
    gimp_async_add_callback_for_object(
        line_art->priv->async, (GimpAsyncCallback)gimp_line_art_compute_cb,
        line_art, line_art);
  }
}
...
static GimpAsync *gimp_line_art_prepare_async(GimpLineArt *line_art,
  ...
  buffer =
      gimp_gegl_buffer_dup(gimp_pickable_get_buffer(line_art->priv->input));
  ...
  async = gimp_parallel_run_async_full(
      priority, (GimpRunAsyncFunc)gimp_line_art_prepare_async_func, data,
...
static void gimp_line_art_prepare_async_func(GimpAsync *async,
  ...
  if (has_alpha) {
    if (data->select_transparent) {
      ...
      gi = gegl_buffer_iterator_new(data->buffer, NULL, 0, babl_format("A u8"),
  ...
  if (buffer_x != 0 || buffer_y != 0) {
    buffer = g_object_new(GEGL_TYPE_BUFFER, "source", buffer, "shift-x",
                          buffer_x, "shift-y", buffer_y, NULL);
  }
  ...
  closed = gimp_line_art_close(buffer, select_transparent, data->threshold,
                               data->spline_max_len, data->segment_max_len,
  ...
  if (!gimp_async_is_stopped(async)) {
    if (buffer_x != 0 || buffer_y != 0) {
      buffer = g_object_new(GEGL_TYPE_BUFFER, "source", closed, "shift-x",
                            -buffer_x, "shift-y", -buffer_y, NULL);
    ...
    gimp_async_finish_full(async, line_art_result_new(closed, distmap),
...
static LineArtResult *line_art_result_new(GeglBuffer *closed, gfloat *distmap) {
  LineArtResult *data;
...
static void line_art_result_free(LineArtResult *data) {
...
static void gimp_line_art_input_invalidate_preview(GimpViewable *viewable,
                                                   GimpLineArt *line_art) {
  if (!line_art->priv->idle_id) {
    line_art->priv->idle_id =
        g_idle_add_full(GIMP_PRIORITY_VIEWABLE_IDLE,
                        (GSourceFunc)gimp_line_art_idle, line_art, NULL);
  }
}
...
static GeglBuffer *gimp_line_art_close(
    ...
    gfloat spline_roundness, gboolean allow_self_intersections,
    gint created_regions_significant_area, gint created_regions_minimum_area,
    gboolean small_segments_from_spline_sources, gfloat **closed_distmap,
    GimpAsync *async) {
  ...
  strokes = gegl_buffer_new(gegl_buffer_get_extent(buffer), gray_format);
  ...
  if (!select_transparent) {
    ...
    gi = gegl_buffer_iterator_new(strokes, NULL, 0, NULL, GEGL_ACCESS_READ,
                                  GEGL_ABYSS_NONE, 1);
    while (gegl_buffer_iterator_next(gi)) {
  ...
  gi = gegl_buffer_iterator_new(strokes, NULL, 0, NULL, GEGL_ACCESS_READWRITE,
                                GEGL_ABYSS_NONE, 1);
  while (gegl_buffer_iterator_next(gi)) {
    guchar *data = (guchar *)gi->items[0].data;
    gint k;
  ...
  if (spline_max_length > 0 || segment_max_length > 0) {
    ...
    gimp_lineart_compute_normals_curvatures(strokes, normals, curvatures,
                                            smoothed_curvatures,
                                            normal_estimate_mask_size, async);
    if (gimp_async_is_stopped(async))
      goto end2;
    ...
    for (i = 0; i < width; i++) {
      ...
      for (j = 0; j < height; j++) {
        if (smoothed_curvatures[i + j * width] >=
                (threshold / MAX(1.0f, radii[i + j * width])) ||
    ...
    keypoints = gimp_lineart_curvature_extremums(
        curvatures, smoothed_curvatures, width, height, async);
    if (gimp_async_is_stopped(async))
      goto end2;
    ...
    if (spline_max_length > 0) {
      ...
      candidates = gimp_lineart_find_spline_candidates(keypoints, normals,
                                                       width, spline_max_length,
                                                       spline_max_angle, async);
      if (gimp_async_is_stopped(async))
        goto end3;
      ...
      while (candidates) {
        ...
        if ((!g_hash_table_contains(visited, p1) ||
          ...
          if (transitions == 2 &&
              gimp_line_art_allow_closure(closed, discrete_curve, &fill_pixels,
                                          created_regions_significant_area,
                                          created_regions_minimum_area)) {
            for (i = 0; i < discrete_curve->len; i++) {
              Pixel p = g_array_index(discrete_curve, Pixel, i);

              if (p.x >= 0 && p.x < gegl_buffer_get_width(closed) && p.y >= 0 &&
                  p.y < gegl_buffer_get_height(closed)) {
                guchar val = 2;

                gegl_buffer_set(closed,
                                GEGL_RECTANGLE((gint)p.x, (gint)p.y, 1, 1), 0,
            ...
            g_hash_table_replace(
                visited, p1,
                GINT_TO_POINTER(
                    GPOINTER_TO_INT(g_hash_table_lookup(visited, p1)) + 1));
            g_hash_table_replace(
                visited, p2,
                GINT_TO_POINTER(
                    GPOINTER_TO_INT(g_hash_table_lookup(visited, p2)) + 1));
            inserted = TRUE;
          }
      ...
      for (i = 0; i < keypoints->len; i++) {
        ...
        *p = *point;
        ...
        if (!g_hash_table_contains(visited, p) ||
            (small_segments_from_spline_sources &&
             GPOINTER_TO_INT(g_hash_table_lookup(visited, p)) <
                 end_point_connectivity)) {
          GArray *segment = gimp_lineart_line_segment_until_hit(
              closed, *point, pair2normal(*point, normals, width),
              segment_max_length);
          ...
          if (segment->len &&
            ...
            for (j = 0; j < segment->len; j++) {
              ...
              gegl_buffer_set(closed,
                              GEGL_RECTANGLE((gint)p2.x, (gint)p2.y, 1, 1), 0,
                              NULL, &val, GEGL_AUTO_ROWSTRIDE);
            }
            g_hash_table_replace(
                visited, p,
                GINT_TO_POINTER(
                    GPOINTER_TO_INT(g_hash_table_lookup(visited, p)) + 1));
            inserted = TRUE;
          }
  ...
  if (closed_distmap) {
    ...
    *closed_distmap = g_new(gfloat, width * height);
    ...
    graph = gegl_node_new();
    input = gegl_node_new_child(graph, "operation", "gegl:buffer-source",
                                "buffer", closed, NULL);
    op = gegl_node_new_child(graph, "operation", "gegl:distance-transform",
                             "metric", GEGL_DISTANCE_METRIC_EUCLIDEAN,
                             "normalize", FALSE, NULL);
    gegl_node_connect_to(input, "output", op, "input");
    gegl_node_blit(op, 1.0, gegl_buffer_get_extent(closed), NULL,
                   *closed_distmap, GEGL_AUTO_ROWSTRIDE, GEGL_BLIT_DEFAULT);
...
static void gimp_lineart_denoise(GeglBuffer *buffer, int minimum_area,
                                 GimpAsync *async) {
...
static void gimp_lineart_compute_normals_curvatures(
  ...
  while (*e) {
    ...
    curvatures[(*e)->x + (*e)->y * width] =
        MAX(curvature, curvatures[(*e)->x + (*e)->y * width]);
    e++;
  }
    ...
    for (int x = 0; x < width; ++x) {
      if ((curvatures[x + y * width] > 0.0) && !visited[x + y * width]) {
        ...
        if (max_smoothed_curvature > 0.0f) {
          curvatures[(gint)max_smoothed_curvature_pixel.x +
                     (gint)max_smoothed_curvature_pixel.y * width] =
              max_smoothed_curvature;
          g_array_append_val(max_positions, max_smoothed_curvature_pixel);
        } else {
          curvatures[(gint)max_raw_curvature_pixel.x +
                     (gint)max_raw_curvature_pixel.y * width] =
              max_raw_curvature;
    ...
    for (j = i + 1; j < max_positions->len; j++) {
      ...
      if (distance <= distance_threshold) {
        ...
        normalP1 = gimp_vector2_new(
            normals[((gint)p1.x + (gint)p1.y * width) * 2],
            normals[((gint)p1.x + (gint)p1.y * width) * 2 + 1]);
        normalP2 = gimp_vector2_new(
            normals[((gint)p2.x + (gint)p2.y * width) * 2],
        ...
        cosN = gimp_vector2_inner_product_val(normalP1,
                                              (gimp_vector2_neg_val(normalP2)));
        qualityA = MAX(0.0f, 1 - distance / distance_threshold);
        qualityB =
            MAX(0.0f, (float)(gimp_vector2_inner_product_val(normalP1, p1p2) -
        ...
        if (quality > 0) {
          ...
          candidates = g_list_insert_sorted_with_data(
              candidates, candidate,
              (GCompareDataFunc)gimp_spline_candidate_cmp, NULL);
        }
      }
    }
  }
...
static gboolean gimp_line_art_allow_closure(GeglBuffer *mask, GArray *pixels,
  ...
  for (i = 0; i < pixels->len; i++) {
    if (p->x >= 0 && p->x < gegl_buffer_get_width(mask) && p->y >= 0 &&
        p->y < gegl_buffer_get_height(mask)) {
      guchar val;

      gegl_buffer_sample(mask, (gint)p->x, (gint)p->y, NULL, &val, NULL,
                         GEGL_SAMPLER_NEAREST, GEGL_ABYSS_NONE);
      val = val ? 3 : 2;
      ...
      if (p.x >= 0 && p.x < gegl_buffer_get_width(mask) && p.y >= 0 &&
        ...
        if ((count != -1) && (count <= max_edgel_count)) {
          ...
          if (area >= significant_size && area < minimum_size) {
            ...
            np->x = direction == XPlusDirection
                        ? p.x + 1
                        : (direction == XMinusDirection ? p.x - 1 : p.x);
            np->y = direction == YPlusDirection
                        ? p.y + 1
                        : (direction == YMinusDirection ? p.y - 1 : p.y);
  ...
  for (i = 0; i < pixels->len; i++) {
    ...
    if (p.x >= 0 && p.x < gegl_buffer_get_width(mask) && p.y >= 0 &&
      ...
      gegl_buffer_sample(mask, (gint)p.x, (gint)p.y, NULL, &val, NULL,
                         GEGL_SAMPLER_NEAREST, GEGL_ABYSS_NONE);
      val &= 1;
      gegl_buffer_set(mask, GEGL_RECTANGLE((gint)p.x, (gint)p.y, 1, 1), 0, NULL,
                      &val, GEGL_AUTO_ROWSTRIDE);
    }
  }
  return TRUE;
}

static GArray *gimp_lineart_line_segment_until_hit(const GeglBuffer *mask,
                                                   Pixel start,
                                                   GimpVector2 direction,
                                                   int size) {
  ...
  for (int t = 0; t <= tmax; ++t) {
    GimpVector2 v = gimp_vector2_add_val(
        p0, gimp_vector2_mul_val(direction, (float)t / tmax));
    Pixel p;

    p.x = (gint)round(v.x);
    p.y = (gint)round(v.y);
    if (p.x >= 0 && p.x < gegl_buffer_get_width(buffer) && p.y >= 0 &&
        p.y < gegl_buffer_get_height(buffer)) {
      guchar val;
      gegl_buffer_sample(buffer, p.x, p.y, NULL, &val, NULL,
                         GEGL_SAMPLER_NEAREST, GEGL_ABYSS_NONE);
      if (out && val) {
        return points;
      }
...
static gfloat *gimp_lineart_estimate_strokes_radii(GeglBuffer *mask,
  ...
  op = gegl_node_new_child(graph, "operation", "gegl:distance-transform",
                           ...
                           "normalize", FALSE, NULL);
  gegl_node_connect_to(input, "output", op, "input");
  gegl_node_blit(op, 1.0, gegl_buffer_get_extent(mask), NULL, dist,
                 GEGL_AUTO_ROWSTRIDE, GEGL_BLIT_DEFAULT);
  g_object_unref(graph);

  thickness = g_new0(gfloat, width * height);
  gi = gegl_buffer_iterator_new(mask, NULL, 0, NULL, GEGL_ACCESS_READ,
                                GEGL_ABYSS_NONE, 1);
  while (gegl_buffer_iterator_next(gi)) {
...
static void gimp_line_art_simple_fill(GeglBuffer *buffer, gint x, gint y,
  ...
  if (x < 0 || x >= gegl_buffer_get_width(buffer) || y < 0 ||
      y >= gegl_buffer_get_height(buffer) || *counter <= 0)
    return;

  gegl_buffer_sample(buffer, x, y, NULL, &val, NULL, GEGL_SAMPLER_NEAREST,
                     GEGL_ABYSS_NONE);

  if (!val) {
    val = 1;
    gegl_buffer_set(buffer, GEGL_RECTANGLE(x, y, 1, 1), 0, NULL, &val,
                    GEGL_AUTO_ROWSTRIDE);
    (*counter)--;
...
static inline gboolean border_in_direction(GeglBuffer *mask, Pixel p,
  ...
  if (px >= 0 && px < gegl_buffer_get_width(mask) && py >= 0 &&
      py < gegl_buffer_get_height(mask)) {
    guchar val;

    gegl_buffer_sample(mask, px, py, NULL, &val, NULL, GEGL_SAMPLER_NEAREST,
                       GEGL_ABYSS_NONE);
    return !((gboolean)val);
  }
  return TRUE;
...
static void gimp_edgel_clear(Edgel **edgel) { g_clear_pointer(edgel, g_free); }

static int gimp_edgel_cmp(const Edgel *e1, const Edgel *e2) {
  gimp_assert(e1 && e2);

  if ((e1->x == e2->x) && (e1->y == e2->y) && (e1->direction == e2->direction))
    return 0;
...
static guint edgel2index_hash_fun(Edgel *key) {
   ...
   */
  return (key->x + key->y) * (key->x + key->y + 1) / 2 +
         key->y * key->direction;
}

static gboolean edgel2index_equal_fun(Edgel *e1, Edgel *e2) {
  return (e1->x == e2->x && e1->y == e2->y && e1->direction == e2->direction);
}
...
static glong gimp_edgel_track_mark(GeglBuffer *mask, Edgel edgel,
  ...
  do {
    ...
    gegl_buffer_sample(mask, edgel.x, edgel.y, NULL, &val, NULL,
                       GEGL_SAMPLER_NEAREST, GEGL_ABYSS_NONE);
    if (val & 2) {
      ...
      gegl_buffer_set(mask, GEGL_RECTANGLE(edgel.x, edgel.y, 1, 1), 0, NULL,
                      &val, GEGL_AUTO_ROWSTRIDE);
    }
...
static GArray *gimp_edgelset_new(GeglBuffer *buffer, GimpAsync *async) {
  ...
  gi = gegl_buffer_iterator_new(buffer, GEGL_RECTANGLE(0, 0, width, height), 0,
                                NULL, GEGL_ACCESS_READ, GEGL_ABYSS_NONE, 5);
  gegl_buffer_iterator_add(gi, buffer, GEGL_RECTANGLE(0, -1, width, height), 0,
                           NULL, GEGL_ACCESS_READ, GEGL_ABYSS_NONE);
  gegl_buffer_iterator_add(gi, buffer, GEGL_RECTANGLE(0, 1, width, height), 0,
                           NULL, GEGL_ACCESS_READ, GEGL_ABYSS_NONE);
  gegl_buffer_iterator_add(gi, buffer, GEGL_RECTANGLE(-1, 0, width, height), 0,
                           NULL, GEGL_ACCESS_READ, GEGL_ABYSS_NONE);
  gegl_buffer_iterator_add(gi, buffer, GEGL_RECTANGLE(1, 0, width, height), 0,
                           NULL, GEGL_ACCESS_READ, GEGL_ABYSS_NONE);
  while (gegl_buffer_iterator_next(gi)) {
...
static void gimp_edgelset_smooth_normals(GArray *set, int mask_size,
  ...
  for (i = 0; i < set->len; i++) {
    ...
    while (n-- && (edgel_after != edgel_before)) {
      smoothed_normal = gimp_vector2_add_val(
          smoothed_normal,
          gimp_vector2_mul_val(Direction2Normal[edgel_before->direction],
                               weights[i]));
      smoothed_normal = gimp_vector2_add_val(
          smoothed_normal,
          gimp_vector2_mul_val(Direction2Normal[edgel_after->direction],
                               weights[i]));
...
static void gimp_edgelset_compute_curvature(GArray *set, GimpAsync *async) {
  ...
  for (i = 0; i < set->len; i++) {
    ...
    GimpVector2 n_prev =
        gimp_vector2_new(previous->x_normal, previous->y_normal);
    GimpVector2 n_next = gimp_vector2_new(next->x_normal, next->y_normal);
    GimpVector2 diff =
        gimp_vector2_mul_val(gimp_vector2_sub_val(n_next, n_prev), 0.5);
...
static void gimp_edgelset_next8(const GeglBuffer *buffer, Edgel *it, Edgel *n) {
  ...
  gegl_buffer_get((GeglBuffer *)buffer,
                  GEGL_RECTANGLE(n->x - 1, n->y - 1, 3, 3), 1.0, NULL, pixels,
