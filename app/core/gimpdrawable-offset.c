...
void gimp_drawable_offset(GimpDrawable *drawable, GimpContext *context,
  ...
  if (!gimp_item_mask_intersect(GIMP_ITEM(drawable), NULL, NULL, &width,
                                &height)) {
    return;
  }
  ...
  node = gegl_node_new_child(NULL, "operation", "gimp:offset", "context",
                             context, "type", fill_type, "x", offset_x, "y",
                             offset_y, NULL);

  gimp_drawable_apply_operation(drawable, NULL,
                                C_("undo-type", "Offset Drawable"), node);
