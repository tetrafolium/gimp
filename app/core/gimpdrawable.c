...
static void gimp_drawable_set_property(GObject *object, guint property_id,
                                       const GValue *value, GParamSpec *pspec);
static void gimp_drawable_get_property(GObject *object, guint property_id,
                                       GValue *value, GParamSpec *pspec);

static gint64 gimp_drawable_get_memsize(GimpObject *object, gint64 *gui_size);

static gboolean gimp_drawable_get_size(GimpViewable *viewable, gint *width,
                                       gint *height);
...
static void gimp_drawable_scale(GimpItem *item, gint new_width, gint new_height,
                                gint new_offset_x, gint new_offset_y,
                                GimpInterpolationType interp_type,
                                GimpProgress *progress);
static void gimp_drawable_resize(GimpItem *item, GimpContext *context,
                                 GimpFillType fill_type, gint new_width,
                                 gint new_height, gint offset_x, gint offset_y);
static void gimp_drawable_flip(GimpItem *item, GimpContext *context,
                               GimpOrientationType flip_type, gdouble axis,
                               gboolean clip_result);
static void gimp_drawable_rotate(GimpItem *item, GimpContext *context,
                                 GimpRotationType rotate_type, gdouble center_x,
                                 gdouble center_y, gboolean clip_result);
static void gimp_drawable_transform(GimpItem *item, GimpContext *context,
...
static const guint8 *gimp_drawable_get_icc_profile(GimpColorManaged *managed,
                                                   gsize *len);
...
static gboolean gimp_drawable_get_pixel_at(GimpPickable *pickable, gint x,
                                           gint y, const Babl *format,
                                           gpointer pixel);
static void gimp_drawable_get_pixel_average(GimpPickable *pickable,
                                            const GeglRectangle *rect,
                                            const Babl *format, gpointer pixel);

static void gimp_drawable_real_update(GimpDrawable *drawable, gint x, gint y,
...
static gboolean gimp_drawable_real_supports_alpha(GimpDrawable *drawable);

static void gimp_drawable_real_convert_type(
...
static void gimp_drawable_real_push_undo(GimpDrawable *drawable,
                                         const gchar *undo_desc,
                                         GeglBuffer *buffer, gint x, gint y,
                                         gint width, gint height);
...
static void gimp_drawable_class_init(GimpDrawableClass *klass) {
  ...
  gimp_drawable_signals[UPDATE] =
      g_signal_new("update", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpDrawableClass, update), NULL, NULL,
                   gimp_marshal_VOID__INT_INT_INT_INT, G_TYPE_NONE, 4,
                   G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT);

  gimp_drawable_signals[FORMAT_CHANGED] = g_signal_new(
      "format-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpDrawableClass, format_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_drawable_signals[ALPHA_CHANGED] = g_signal_new(
      "alpha-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpDrawableClass, alpha_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  gimp_drawable_signals[BOUNDING_BOX_CHANGED] = g_signal_new(
      "bounding-box-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpDrawableClass, bounding_box_changed), NULL, NULL,
      NULL, G_TYPE_NONE, 0);
...
static void gimp_pickable_iface_init(GimpPickableInterface *iface) {
  iface->get_image =
      (GimpImage * (*)(GimpPickable * pickable)) gimp_item_get_image;
  iface->get_format =
      (const Babl *(*)(GimpPickable * pickable)) gimp_drawable_get_format;
  iface->get_format_with_alpha = (const Babl *(*)(GimpPickable * pickable))
      gimp_drawable_get_format_with_alpha;
  iface->get_buffer =
      (GeglBuffer * (*)(GimpPickable * pickable)) gimp_drawable_get_buffer;
...
static GeglNode *gimp_drawable_get_node(GimpFilter *filter) {
  ...
  drawable->private->mode_node =
      gegl_node_new_child(node, "operation", "gimp:normal", NULL);

  input = gegl_node_get_input_proxy(node, "input");
  output = gegl_node_get_output_proxy(node, "output");

  gegl_node_connect_to(input, "output", drawable->private->mode_node, "input");
  gegl_node_connect_to(drawable->private->mode_node, "output", output, "input");
...
static void gimp_drawable_scale(GimpItem *item, gint new_width, gint new_height,
  ...
  gimp_gegl_apply_scale(gimp_drawable_get_buffer(drawable), progress,
                        C_("undo-type", "Scale"), new_buffer,
                        interpolation_type,
                        ((gdouble)new_width / gimp_item_get_width(item)),
                        ((gdouble)new_height / gimp_item_get_height(item)));
...
static void gimp_drawable_resize(GimpItem *item, GimpContext *context,
  ...
  if (new_width == gimp_item_get_width(item) &&
      new_height == gimp_item_get_height(item) && offset_x == 0 &&
      offset_y == 0)
    return;
  ...
  intersect = gimp_rectangle_intersect(
      gimp_item_get_offset_x(item), gimp_item_get_offset_y(item),
      gimp_item_get_width(item), gimp_item_get_height(item), new_offset_x,
      new_offset_y, new_width, new_height, &copy_x, &copy_y, &copy_width,
      &copy_height);

  new_buffer = gegl_buffer_new(GEGL_RECTANGLE(0, 0, new_width, new_height),
                               gimp_drawable_get_format(drawable));

  if (!intersect || copy_width != new_width || copy_height != new_height) {
    ...
    gimp_drawable_fill_buffer(drawable, new_buffer, &color, pattern, 0, 0);
  }
  ...
  if (intersect && copy_width && copy_height) {
    ...
    gimp_gegl_buffer_copy(
        ...
        GEGL_RECTANGLE(copy_x - gimp_item_get_offset_x(item),
                       copy_y - gimp_item_get_offset_y(item), copy_width,
                       copy_height),
        GEGL_ABYSS_NONE, new_buffer,
        GEGL_RECTANGLE(copy_x - new_offset_x, copy_y - new_offset_y, 0, 0));
  }
...
static void gimp_drawable_flip(GimpItem *item, GimpContext *context,
  ...
  buffer = gimp_drawable_transform_buffer_flip(
      drawable, context, gimp_drawable_get_buffer(drawable), off_x, off_y,
      flip_type, axis, clip_result, &buffer_profile, &new_off_x, &new_off_y);

  if (buffer) {
    gimp_drawable_transform_paste(drawable, buffer, buffer_profile, new_off_x,
                                  new_off_y, FALSE);
...
static void gimp_drawable_rotate(GimpItem *item, GimpContext *context,
  ...
  buffer = gimp_drawable_transform_buffer_rotate(
      drawable, context, gimp_drawable_get_buffer(drawable), off_x, off_y,
      rotate_type, center_x, center_y, clip_result, &buffer_profile, &new_off_x,
      &new_off_y);
...
static void gimp_drawable_transform(GimpItem *item, GimpContext *context,
  ...
  buffer = gimp_drawable_transform_buffer_affine(
      drawable, context, gimp_drawable_get_buffer(drawable), off_x, off_y,
      matrix, direction, interpolation_type, clip_result, &buffer_profile,
      &new_off_x, &new_off_y, progress);
...
static gboolean gimp_drawable_get_pixel_at(GimpPickable *pickable, gint x,
  ...
  if (x < 0 || x >= gimp_item_get_width(GIMP_ITEM(drawable)) || y < 0 ||
      y >= gimp_item_get_height(GIMP_ITEM(drawable)))
    return FALSE;

  gegl_buffer_sample(gimp_drawable_get_buffer(drawable), x, y, NULL, pixel,
                     format, GEGL_SAMPLER_NEAREST, GEGL_ABYSS_NONE);
...
static void gimp_drawable_get_pixel_average(GimpPickable *pickable,
  ...
  return gimp_gegl_average_color(gimp_drawable_get_buffer(drawable), rect, TRUE,
                                 GEGL_ABYSS_NONE, format, pixel);
}
...
gimp_drawable_real_estimate_memsize(GimpDrawable *drawable,
  ...
  format = gimp_image_get_format(image, gimp_drawable_get_base_type(drawable),
...
static void gimp_drawable_real_set_buffer(GimpDrawable *drawable,
  ...
  if (drawable->private->buffer_source_node)
    gegl_node_set(drawable->private->buffer_source_node, "buffer",
                  gimp_drawable_get_buffer(drawable), NULL);

  gimp_item_set_offset(item, bounds->x, bounds->y);
  gimp_item_set_size(
      item, bounds->width ? bounds->width : gegl_buffer_get_width(buffer),
      bounds->height ? bounds->height : gegl_buffer_get_height(buffer));
...
static void gimp_drawable_real_push_undo(GimpDrawable *drawable,
  ...
  if (!buffer) {
    ...
    gegl_rectangle_align_to_buffer(
        &drawable_rect, GEGL_RECTANGLE(x, y, width, height), drawable_buffer,
    ...
    gimp_gegl_buffer_copy(drawable_buffer, &drawable_rect, GEGL_ABYSS_NONE,
                          buffer, GEGL_RECTANGLE(0, 0, 0, 0));
  } else {
    g_object_ref(buffer);
  }
  ...
  gimp_image_undo_push_drawable(image, undo_desc, drawable, buffer, x, y);
...
static void gimp_drawable_real_swap_pixels(GimpDrawable *drawable,
  ...
  gimp_gegl_buffer_copy(gimp_drawable_get_buffer(drawable),
                        ...
                        buffer, GEGL_RECTANGLE(0, 0, 0, 0));
  gimp_gegl_buffer_copy(tmp, GEGL_RECTANGLE(0, 0, width, height),
                        GEGL_ABYSS_NONE, gimp_drawable_get_buffer(drawable),
...
static GeglNode *gimp_drawable_real_get_source_node(GimpDrawable *drawable) {
  ...
  drawable->private->buffer_source_node =
      gegl_node_new_child(NULL, "operation", "gimp:buffer-source-validate",
                          "buffer", gimp_drawable_get_buffer(drawable), NULL);
...
GimpDrawable *gimp_drawable_new(GType type, GimpImage *image, const gchar *name,
                                gint offset_x, gint offset_y, gint width,
                                gint height, const Babl *format) {
  ...
  drawable = GIMP_DRAWABLE(
      gimp_item_new(type, image, name, offset_x, offset_y, width, height));
...
gint64 gimp_drawable_estimate_memsize(GimpDrawable *drawable,
  ...
  return GIMP_DRAWABLE_GET_CLASS(drawable)->estimate_memsize(
      drawable, component_type, width, height);
}

void gimp_drawable_update(GimpDrawable *drawable, gint x, gint y, gint width,
                          gint height) {
  ...
  if (drawable->private->paint_count == 0) {
    g_signal_emit(drawable, gimp_drawable_signals[UPDATE], 0, x, y, width,
                  height);
  } else {
    GeglRectangle rect;
    ...
    if (gegl_rectangle_intersect(
      ...
      if (drawable->private->paint_copy_region) {
        ...
        drawable->private->paint_copy_region = cairo_region_create_rectangle(
            (const cairo_rectangle_int_t *)&aligned_rect);
      }

      gegl_rectangle_align(&aligned_rect, &rect,
                           GEGL_RECTANGLE(0, 0, PAINT_UPDATE_CHUNK_WIDTH,
      ...
      if (drawable->private->paint_update_region) {
        ...
        drawable->private->paint_update_region = cairo_region_create_rectangle(
            (const cairo_rectangle_int_t *)&aligned_rect);
      }
    }
  }
...
void gimp_drawable_convert_type(GimpDrawable *drawable, GimpImage *dest_image,
  ...
  old_format = gimp_drawable_get_format(drawable);
  new_format =
      gimp_image_get_format(dest_image, new_base_type, new_precision,
                            new_has_alpha, NULL /* handled by layer */);
  ...
  GIMP_DRAWABLE_GET_CLASS(drawable)->convert_type(
      drawable, dest_image, new_format, src_profile, dest_profile,
...
void gimp_drawable_apply_buffer(GimpDrawable *drawable, GeglBuffer *buffer,
                                ...
                                GeglBuffer *base_buffer, gint base_x,
                                gint base_y) {
  g_return_if_fail(GIMP_IS_DRAWABLE(drawable));
  ...
  GIMP_DRAWABLE_GET_CLASS(drawable)->apply_buffer(
      drawable, buffer, buffer_region, push_undo, undo_desc, opacity, mode,
      blend_space, composite_space, composite_mode, base_buffer, base_x,
      base_y);
}
...
void gimp_drawable_set_buffer(GimpDrawable *drawable, gboolean push_undo,
...
void gimp_drawable_set_buffer_full(GimpDrawable *drawable, gboolean push_undo,
                                   const gchar *undo_desc, GeglBuffer *buffer,
                                   const GeglRectangle *bounds,
                                   gboolean update) {
  GimpItem *item;
  ...
  if (!bounds) {
    gimp_item_get_offset(GIMP_ITEM(drawable), &curr_bounds.x, &curr_bounds.y);
  ...
  GIMP_DRAWABLE_GET_CLASS(drawable)->set_buffer(drawable, push_undo, undo_desc,
                                                buffer, bounds);
...
void gimp_drawable_set_format(GimpDrawable *drawable, const Babl *format,
                              gboolean copy_buffer, gboolean push_undo) {
  GimpItem *item;
  GeglBuffer *buffer;
  ...
  if (push_undo)
    gimp_image_undo_push_drawable_format(gimp_item_get_image(item), NULL,
                                         drawable);

  buffer = gegl_buffer_new(GEGL_RECTANGLE(0, 0, gimp_item_get_width(item),
...
GeglNode *gimp_drawable_get_source_node(GimpDrawable *drawable) {
  ...
  filter = gimp_filter_stack_get_graph(
      GIMP_FILTER_STACK(drawable->private->filter_stack));

  gegl_node_add_child(drawable->private->source_node, filter);

  gegl_node_connect_to(source, "output", filter, "input");

  output = gegl_node_get_output_proxy(drawable->private->source_node, "output");

  gegl_node_connect_to(filter, "output", output, "input");
...
GeglRectangle gimp_drawable_get_bounding_box(GimpDrawable *drawable) {
  g_return_val_if_fail(GIMP_IS_DRAWABLE(drawable), *GEGL_RECTANGLE(0, 0, 0, 0));
...
gboolean gimp_drawable_update_bounding_box(GimpDrawable *drawable) {
  GeglRectangle bounding_box;

  g_return_val_if_fail(GIMP_IS_DRAWABLE(drawable), FALSE);

  bounding_box = GIMP_DRAWABLE_GET_CLASS(drawable)->get_bounding_box(drawable);
  ...
  if (!gegl_rectangle_equal(&bounding_box, &drawable->private->bounding_box)) {
    ...
    n_diff_rects =
        gegl_rectangle_subtract(diff_rects, &old_bounding_box, &bounding_box);

    for (i = 0; i < n_diff_rects; i++) {
      gimp_drawable_update(drawable, diff_rects[i].x, diff_rects[i].y,
    ...
    n_diff_rects =
        gegl_rectangle_subtract(diff_rects, &bounding_box, &old_bounding_box);
...
void gimp_drawable_swap_pixels(GimpDrawable *drawable, GeglBuffer *buffer,
                               gint x, gint y) {
  g_return_if_fail(GIMP_IS_DRAWABLE(drawable));
  g_return_if_fail(GEGL_IS_BUFFER(buffer));
...
void gimp_drawable_push_undo(GimpDrawable *drawable, const gchar *undo_desc,
                             GeglBuffer *buffer, gint x, gint y, gint width,
                             gint height) {
  GimpItem *item;
  ...
  if (!buffer && !gimp_rectangle_intersect(
                     x, y, width, height, 0, 0, gimp_item_get_width(item),
                     gimp_item_get_height(item), &x, &y, &width, &height)) {
    g_warning("%s: tried to push empty region", G_STRFUNC);
    return;
  }

  GIMP_DRAWABLE_GET_CLASS(drawable)->push_undo(drawable, undo_desc, buffer, x,
                                               y, width, height);
}
...
const Babl *gimp_drawable_get_format_with_alpha(GimpDrawable *drawable) {
  ...
  return gimp_image_get_format(gimp_item_get_image(GIMP_ITEM(drawable)),
                               gimp_drawable_get_base_type(drawable),
                               gimp_drawable_get_precision(drawable), TRUE,
                               gimp_drawable_get_space(drawable));
}

const Babl *gimp_drawable_get_format_without_alpha(GimpDrawable *drawable) {
  g_return_val_if_fail(GIMP_IS_DRAWABLE(drawable), NULL);

  return gimp_image_get_format(gimp_item_get_image(GIMP_ITEM(drawable)),
                               gimp_drawable_get_base_type(drawable),
                               gimp_drawable_get_precision(drawable), FALSE,
                               gimp_drawable_get_space(drawable));
}
...
const Babl *gimp_drawable_get_component_format(GimpDrawable *drawable,
  ...
  case GIMP_CHANNEL_RED:
    return gimp_babl_component_format(
        GIMP_RGB, gimp_drawable_get_precision(drawable), RED);

  case GIMP_CHANNEL_GREEN:
    return gimp_babl_component_format(
        GIMP_RGB, gimp_drawable_get_precision(drawable), GREEN);

  case GIMP_CHANNEL_BLUE:
    return gimp_babl_component_format(
        GIMP_RGB, gimp_drawable_get_precision(drawable), BLUE);

  case GIMP_CHANNEL_ALPHA:
    return gimp_babl_component_format(
        GIMP_RGB, gimp_drawable_get_precision(drawable), ALPHA);

  case GIMP_CHANNEL_GRAY:
    return gimp_babl_component_format(
        GIMP_GRAY, gimp_drawable_get_precision(drawable), GRAY);
...
gboolean gimp_drawable_flush_paint(GimpDrawable *drawable) {
  ...
  if (drawable->private->paint_copy_region) {
    ...
    n_rects = cairo_region_num_rectangles(drawable->private->paint_copy_region);

    for (i = 0; i < n_rects; i++) {
      GeglRectangle rect;

      cairo_region_get_rectangle(drawable->private->paint_copy_region, i,
                                 (cairo_rectangle_int_t *)&rect);

      gimp_gegl_buffer_copy(drawable->private->paint_buffer, &rect,
                            GEGL_ABYSS_NONE, buffer, NULL);
    }
    ...
    n_rects =
        cairo_region_num_rectangles(drawable->private->paint_update_region);

    for (i = 0; i < n_rects; i++) {
      GeglRectangle rect;

      cairo_region_get_rectangle(drawable->private->paint_update_region, i,
                                 (cairo_rectangle_int_t *)&rect);

      g_signal_emit(drawable, gimp_drawable_signals[UPDATE], 0, rect.x, rect.y,
