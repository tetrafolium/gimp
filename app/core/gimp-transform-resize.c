...
static void gimp_transform_resize_adjust(const GimpVector2 *points,
                                         gint n_points, gint *x1, gint *y1,
                                         gint *x2, gint *y2);
static void gimp_transform_resize_crop(const GimpVector2 *points, gint n_points,
                                       gdouble aspect, gint *x1, gint *y1,
                                       gint *x2, gint *y2);

static void add_rectangle(const GimpVector2 *points, gint n_points,
                          Rectangle *r, GimpVector2 a, GimpVector2 b,
                          GimpVector2 c, GimpVector2 d);
static gboolean intersect(GimpVector2 a, GimpVector2 b, GimpVector2 c,
                          GimpVector2 d, GimpVector2 *i);
static gboolean intersect_x(GimpVector2 a, GimpVector2 b, GimpVector2 c,
                            GimpVector2 *i);
static gboolean intersect_y(GimpVector2 a, GimpVector2 b, GimpVector2 c,
                            GimpVector2 *i);
static gboolean in_poly(const GimpVector2 *points, gint n_points,
                        GimpVector2 p);
static gboolean point_on_border(const GimpVector2 *points, gint n_points,
                                GimpVector2 p);

static void find_two_point_rectangle(Rectangle *r, const GimpVector2 *points,
                                     gint n_points, gint p);
static void find_three_point_rectangle_corner(Rectangle *r,
                                              const GimpVector2 *points,
                                              gint n_points, gint p);
static void find_three_point_rectangle(Rectangle *r, const GimpVector2 *points,
                                       gint n_points, gint p);
...
gboolean gimp_transform_resize_boundary(const GimpMatrix3 *inv,
                                        GimpTransformResize resize, gdouble u1,
                                        gdouble v1, gdouble u2, gdouble v2,
                                        gint *x1, gint *y1, gint *x2,
                                        gint *y2) {
  ...
  *y2 = ceil(v2);
  ...
  gimp_transform_polygon(inv, bounds, 4, TRUE, points, &n_points);
  ...
  case GIMP_TRANSFORM_RESIZE_ADJUST:
    ...
    gimp_transform_resize_adjust(points, n_points, x1, y1, x2, y2);
    break;

  case GIMP_TRANSFORM_RESIZE_CROP:
    gimp_transform_resize_crop(points, n_points, 0.0, x1, y1, x2, y2);
    break;

  case GIMP_TRANSFORM_RESIZE_CROP_WITH_ASPECT:
    gimp_transform_resize_crop(points, n_points, (u2 - u1) / (v2 - v1), x1, y1,
                               x2, y2);
    break;
...
static void gimp_transform_resize_crop(const GimpVector2 *orig_points,
                                       gint n_points, gdouble aspect, gint *x1,
  ...
  for (i = 1; i < n_points - 1; i++) {
    ...
    for (j = i; j < n_points; j++) {
      ...
      if ((theta < min_theta) || ((theta == min_theta) && (mag < min_mag))) {
        min_theta = theta;
        min_mag = mag;
        next = j;
      }
  ...
  if (r.area == 0) {
    ...
    gimp_transform_resize_adjust(orig_points, n_points, x1, y1, x2, y2);
...
static void find_three_point_rectangle(Rectangle *r, const GimpVector2 *points,
  ...
  if (intersect_x(b, c, a, &i1) && intersect_y(c, d, i1, &i2) &&
      intersect_x(d, a, i2, &i3))
    add_rectangle(points, n_points, r, i3, i3, i1, i1);

  if (intersect_y(b, c, a, &i1) && intersect_x(c, d, i1, &i2) &&
      intersect_y(d, a, i2, &i3))
    add_rectangle(points, n_points, r, i3, i3, i1, i1);

  if (intersect_x(d, c, a, &i1) && intersect_y(c, b, i1, &i2) &&
      intersect_x(b, a, i2, &i3))
    add_rectangle(points, n_points, r, i3, i3, i1, i1);

  if (intersect_y(d, c, a, &i1) && intersect_x(c, b, i1, &i2) &&
...
static void find_three_point_rectangle_corner(Rectangle *r,
  ...
  if (intersect_x(c, d, a, &i1) && intersect_y(b, c, i1, &i2))
    add_rectangle(points, n_points, r, a, a, i1, i2);

  if (intersect_y(c, d, a, &i1) && intersect_x(b, c, i1, &i2))
    add_rectangle(points, n_points, r, a, a, i1, i2);
}
...
static void find_two_point_rectangle(Rectangle *r, const GimpVector2 *points,
  ...
  if (intersect_x(c, b, a, &i1) && intersect_y(c, b, a, &i2)) {
    mid.x = (i1.x + i2.x) / 2.0;
    mid.y = (i1.y + i2.y) / 2.0;
...
static void find_three_point_rectangle_triangle(Rectangle *r,
  ...
  if (intersect_x(b, c, mid, &i1) && intersect_y(a, d, mid, &i2))
    add_rectangle(points, n_points, r, mid, mid, i1, i2);

  if (intersect_y(b, c, mid, &i1) && intersect_x(a, d, mid, &i2))
    add_rectangle(points, n_points, r, mid, mid, i1, i2);

  if (intersect_x(a, d, mid, &i1) && intersect_y(b, c, mid, &i2))
    add_rectangle(points, n_points, r, mid, mid, i1, i2);

  if (intersect_y(a, d, mid, &i1) && intersect_x(b, c, mid, &i2))
    add_rectangle(points, n_points, r, mid, mid, i1, i2);
}
...
static gboolean point_on_border(const GimpVector2 *points, gint n_points,
  ...
  for (i = 0; i <= n_points; i++) {
    ...
    if (ABS(c1 - c2) < EPSILON && MIN(a.x, b.x) <= p.x &&
        MAX(a.x, b.x) >= p.x && MIN(a.y, b.y) <= p.y && MAX(a.y, b.y) >= p.y)
      return TRUE;
  }
...
static void add_rectangle(const GimpVector2 *points, gint n_points,
  ...
  if (in_poly(points, n_points, a) && in_poly(points, n_points, b) &&
      in_poly(points, n_points, c) && in_poly(points, n_points, d)) {
