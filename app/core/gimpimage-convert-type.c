...
gboolean gimp_image_convert_type(GimpImage *image, GimpImageBaseType new_type,
  ...
  g_return_val_if_fail(
      gimp_babl_is_valid(new_type, gimp_image_get_precision(image)), FALSE);
  g_return_val_if_fail(
      dest_profile == NULL || GIMP_IS_COLOR_PROFILE(dest_profile), FALSE);
  g_return_val_if_fail(progress == NULL || GIMP_IS_PROGRESS(progress), FALSE);
  g_return_val_if_fail(error == NULL || *error == NULL, FALSE);

  new_layer_format = gimp_babl_format(new_type, gimp_image_get_precision(image),
                                      TRUE, gimp_image_get_layer_space(image));

  if (dest_profile && !gimp_image_validate_color_profile_by_format(
                          new_layer_format, dest_profile, NULL, error)) {
    return FALSE;
  }
   ...
   *  the same image
   */
  if (old_type == GIMP_GRAY || new_type == GIMP_GRAY) {
    if (!dest_profile)
  ...
  while ((drawable = gimp_object_queue_pop(queue))) {
    gimp_drawable_convert_type(
        drawable, image, new_type, gimp_drawable_get_precision(drawable),
        gimp_drawable_has_alpha(drawable), src_profile, dest_profile,
        GEGL_DITHER_NONE, GEGL_DITHER_NONE, TRUE, progress);
  }
