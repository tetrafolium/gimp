...
} gimp_user_install_items[] = {{"menurc", USER_INSTALL_COPY},
...
static void user_install_log(GimpUserInstall *install, const gchar *format, ...)
    G_GNUC_PRINTF(2, 3);
static void user_install_log_newline(GimpUserInstall *install);
static void user_install_log_error(GimpUserInstall *install, GError **error);
...
static gboolean user_install_dir_copy(GimpUserInstall *install, gint level,
                                      const gchar *source, const gchar *base,
...
GimpUserInstall *gimp_user_install_new(GObject *gimp, gboolean verbose) {
  ...
  if (!install->old_dir) {
    ...
    dir = g_build_filename([library_dir UTF8String], GIMPDIR, GIMP_USER_VERSION,
                           NULL);
...
static gchar *user_install_old_style_gimpdir(void) {
  ...
  if (home_dir) {
    ...
    gimp_dir = g_build_filename(gimp_data_directory(), subdir_name, NULL);
    g_free(user_name);
    g_free(subdir_name);
  }
...
static gboolean user_install_file_copy(GimpUserInstall *install,
  ...
  user_install_log(install, _("Copying file '%s' from '%s'..."),
                   gimp_filename_to_utf8(dest), gimp_filename_to_utf8(source));
...
static gboolean user_install_mkdir(GimpUserInstall *install,
  ...
  if (g_mkdir(dirname, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP |
                           S_IROTH | S_IXOTH) == -1) {
    GError *error = NULL;
...
static gboolean user_install_mkdir_with_parents(GimpUserInstall *install,
  ...
  if (g_mkdir_with_parents(dirname, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP |
 ...
 */
static gboolean user_update_menurc_over20(const GMatchInfo *matched_value,
                                          GString *new_value, gpointer data) {
  ...
  if (g_strcmp0(match, "\"<Actions>/buffers/buffers-paste-as-new\"") == 0) {
    g_string_append(new_value,
                    "\"<Actions>/buffers/buffers-paste-as-new-image\"");
...
static gboolean user_update_controllerrc(const GMatchInfo *matched_value,
                                         GString *new_value, gpointer data) {
  gchar *original;
  gchar *replacement;
  GRegex *regexp = NULL;
  ...
  replacement = g_regex_replace(regexp, original, -1, 0, "primary", 0, NULL);
...
static gboolean user_update_gimprc(const GMatchInfo *matched_value,
                                   GString *new_value, gpointer data) {
  /* Do not migrate paths and themes from GIMP < 2.10. */
  return FALSE;
}

#define GIMPRESSIONIST_UPDATE_PATTERN "selectedbrush=Brushes/paintbrush.pgm"

static gboolean user_update_gimpressionist(const GMatchInfo *matched_value,
                                           GString *new_value, gpointer data) {
  gchar *match = g_match_info_fetch(matched_value, 0);
...
static gboolean user_update_tool_presets(const GMatchInfo *matched_value,
                                         GString *new_value, gpointer data) {
  gchar *match = g_match_info_fetch(matched_value, 0);
...
static gboolean user_update_contextrc_over20(const GMatchInfo *matched_value,
                                             GString *new_value,
                                             gpointer data) {
  gchar *match = g_match_info_fetch(matched_value, 0);
...
static gboolean user_install_dir_copy(GimpUserInstall *install, gint level,
  ...
  while ((basename = g_dir_read_name(source_dir)) != NULL) {
    ...
    if (g_file_test(name, G_FILE_TEST_IS_REGULAR)) {
      g_snprintf(dest, sizeof(dest), "%s%c%s", dirname, G_DIR_SEPARATOR,
                 basename);

      success = user_install_file_copy(install, name, dest, update_pattern,
                                       update_callback);
      if (!success) {
        g_free(name);
        goto error;
      ...
      user_install_dir_copy(install, level + 1, name, dirname, update_pattern,
                            update_callback);
    }
...
static gboolean user_install_create_files(GimpUserInstall *install) {
  ...
  for (i = 0; i < G_N_ELEMENTS(gimp_user_install_items); i++) {
    g_snprintf(dest, sizeof(dest), "%s%c%s", gimp_directory(), G_DIR_SEPARATOR,
    ...
    case USER_INSTALL_COPY:
      g_snprintf(source, sizeof(source), "%s%c%s", gimp_sysconf_directory(),
                 G_DIR_SEPARATOR, gimp_user_install_items[i].name);
  ...
  while ((basename = g_dir_read_name(dir)) != NULL) {
    ...
    if (g_file_test(source, G_FILE_TEST_IS_REGULAR)) {
      ...
      if (strcmp(basename, "documents") == 0 ||
          ...
          strcmp(basename, "themerc") == 0 || strcmp(basename, "toolrc") == 0 ||
      ...
      g_snprintf(dest, sizeof(dest), "%s%c%s", gimp_directory(),
                 G_DIR_SEPARATOR, basename);

      user_install_file_copy(install, source, dest, update_pattern,
                             update_callback);
