...
enum { ACTIVE_TOOL_CHANGED, LAST_SIGNAL };

enum { PROP_0, PROP_ACTIVE_TOOL, PROP_CHILDREN };
...
static void gimp_tool_group_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);
static void gimp_tool_group_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);

static gint64 gimp_tool_group_get_memsize(GimpObject *object, gint64 *gui_size);
...
static void gimp_tool_group_class_init(GimpToolGroupClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  GimpObjectClass *gimp_object_class = GIMP_OBJECT_CLASS(klass);
  GimpViewableClass *viewable_class = GIMP_VIEWABLE_CLASS(klass);
  GimpToolItemClass *tool_item_class = GIMP_TOOL_ITEM_CLASS(klass);

  gimp_tool_group_signals[ACTIVE_TOOL_CHANGED] = g_signal_new(
      "active-tool-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolGroupClass, active_tool_changed), NULL, NULL,
      NULL, G_TYPE_NONE, 0);
...
static void gimp_tool_group_set_property_add_tool(GimpToolInfo *tool_info,
                                                  GimpToolGroup *tool_group) {
...
static void gimp_tool_group_set_property(GObject *object, guint property_id,
  ...
  case PROP_CHILDREN: {
    ...
    gimp_container_foreach(
        container, (GFunc)gimp_tool_group_set_property_add_tool, tool_group);
  } break;
...
static void gimp_tool_group_child_add(GimpContainer *container,
                                      GimpToolInfo *tool_info,
                                      GimpToolGroup *tool_group) {
  g_return_if_fail(gimp_viewable_get_parent(GIMP_VIEWABLE(tool_info)) == NULL);

  gimp_viewable_set_parent(GIMP_VIEWABLE(tool_info), GIMP_VIEWABLE(tool_group));
...
static void gimp_tool_group_shown_changed(GimpToolItem *tool_item) {
  ...
  for (iter = GIMP_LIST(tool_group->priv->children)->queue->head; iter;
...
void gimp_tool_group_set_active_tool(GimpToolGroup *tool_group,
  ...
  if (g_strcmp0(tool_group->priv->active_tool, tool_name)) {
    ...
    g_signal_emit(tool_group, gimp_tool_group_signals[ACTIVE_TOOL_CHANGED], 0);
...
GimpToolInfo *gimp_tool_group_get_active_tool_info(GimpToolGroup *tool_group) {
  ...
  return GIMP_TOOL_INFO(gimp_container_get_child_by_name(
      tool_group->priv->children, tool_group->priv->active_tool));
}
