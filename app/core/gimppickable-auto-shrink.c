...
typedef gboolean (*ColorsEqualFunc)(guchar *col1, guchar *col2);

/*  local function prototypes  */

static AutoShrinkType gimp_pickable_guess_bgcolor(GimpPickable *pickable,
                                                  guchar *color, gint x1,
                                                  gint x2, gint y1, gint y2);
static gboolean gimp_pickable_colors_equal(guchar *col1, guchar *col2);
static gboolean gimp_pickable_colors_alpha(guchar *col1, guchar *col2);

/*  public functions  */

GimpAutoShrink gimp_pickable_auto_shrink(GimpPickable *pickable, gint start_x,
                                         gint start_y, gint start_width,
                                         gint start_height, gint *shrunk_x,
                                         gint *shrunk_y, gint *shrunk_width,
                                         gint *shrunk_height) {
  GeglBuffer *buffer;
  GeglRectangle rect;
  ...
  *shrunk_height = y2 - y1;
  ...
  switch (
      gimp_pickable_guess_bgcolor(pickable, bgcolor, x1, x2 - 1, y1, y2 - 1)) {
  ...
  for (y = y1; y < y2 && !abort; y++) {
    rect.y = y;
    gegl_buffer_get(buffer, &rect, 1.0, format, buf, GEGL_AUTO_ROWSTRIDE,
                    GEGL_ABYSS_NONE);
  ...
  if (x1 != start_x || y1 != start_y || x2 - x1 != start_width ||
      y2 - y1 != start_height) {
    *shrunk_x = x1;
    *shrunk_y = y1;
    *shrunk_width = x2 - x1;
...
static gboolean gimp_pickable_colors_alpha(guchar *dummy, guchar *col) {
  return (col[ALPHA] == 0);
}
