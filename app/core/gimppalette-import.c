...
GimpPalette *
gimp_palette_import_from_gradient(GimpGradient *gradient, GimpContext *context,
                                  gboolean reverse,
                                  GimpGradientBlendColorSpace blend_color_space,
                                  const gchar *palette_name, gint n_colors) {
  GimpPalette *palette;
  ...
  for (i = 0, cur_x = 0; i < n_colors; i++, cur_x += dx) {
    seg = gimp_gradient_get_color_at(gradient, context, seg, cur_x, reverse,
...
static GHashTable *gimp_palette_import_store_colors(GHashTable *table,
                                                    guchar *colors,
                                                    guchar *colors_real,
                                                    gint n_colors) {
...
static void gimp_palette_import_create_list(gpointer key, gpointer value,
                                            gpointer user_data) {
...
static gint gimp_palette_import_sort_colors(gconstpointer a, gconstpointer b) {
...
static void gimp_palette_import_create_image_palette(gpointer data,
                                                     gpointer user_data) {
  ...
  n_colors =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(palette), "import-n-colors"));
  ...
  lab = g_strdup_printf(_("%s (occurs %u)"), _("Untitled"), color_tab->count);

  /* Adjust the colors to the mean of the the sample */
  gimp_rgba_set_uchar(
      &color, (guchar)color_tab->r + (color_tab->r_adj / color_tab->count),
      (guchar)color_tab->g + (color_tab->g_adj / color_tab->count),
      (guchar)color_tab->b + (color_tab->b_adj / color_tab->count), 255);
...
static GimpPalette *gimp_palette_import_make_palette(GHashTable *table,
...
static GHashTable *gimp_palette_import_extract(
    GimpImage *image, GHashTable *colors, GimpPickable *pickable,
    gint pickable_off_x, gint pickable_off_y, gboolean selection_only, gint x,
    gint y, gint width, gint height, gint n_colors, gint threshold) {
  GeglBuffer *buffer;
  ...
  iter = gegl_buffer_iterator_new(buffer, &rect, 0, format, GEGL_ACCESS_READ,
                                  GEGL_ABYSS_NONE, 2);
  bpp = babl_format_get_bytes_per_pixel(format);

  if (selection_only && !gimp_channel_is_empty(gimp_image_get_mask(image))) {
    ...
    gegl_buffer_iterator_add(iter, buffer, &rect, 0, format, GEGL_ACCESS_READ,
                             GEGL_ABYSS_NONE);
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    while (length--) {
      ...
      if ((!mask_data || *mask_data) && data[ALPHA]) {
        ...
        colors =
            gimp_palette_import_store_colors(colors, rgba, rgb_real, n_colors);
      }
...
GimpPalette *gimp_palette_import_from_image(GimpImage *image,
  ...
  colors = gimp_palette_import_extract(image, NULL, GIMP_PICKABLE(image), 0, 0,
                                       selection_only, x, y, width, height,
                                       n_colors, threshold);
...
GimpPalette *gimp_palette_import_from_indexed_image(GimpImage *image,
  ...
  for (count = 0; count < n_colors; ++count) {
    ...
    gimp_rgba_set_uchar(&color, colormap[count * 3 + 0],
                        colormap[count * 3 + 1], colormap[count * 3 + 2], 255);
...
GimpPalette *gimp_palette_import_from_drawables(GList *drawables,
  ...
  for (iter = drawables; iter; iter = iter->next) {
    ...
    if (selection_only) {
      if (!gimp_item_mask_intersect(GIMP_ITEM(drawable), &x, &y, &width,
                                    &height))
        return NULL;
    } else {
      x = 0;
    ...
    colors = gimp_palette_import_extract(
        gimp_item_get_image(GIMP_ITEM(drawable)), colors,
        GIMP_PICKABLE(drawable), off_x, off_y, selection_only, x, y, width,
        height, n_colors, threshold);
  }
...
GimpPalette *gimp_palette_import_from_file(GimpContext *context, GFile *file,
                                           const gchar *palette_name,
                                           GError **error) {
