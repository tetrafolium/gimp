...
static void gimp_selection_translate(GimpItem *item, gdouble offset_x,
                                     gdouble offset_y, gboolean push_undo);
static void gimp_selection_scale(GimpItem *item, gint new_width,
                                 gint new_height, gint new_offset_x,
                                 gint new_offset_y,
                                 GimpInterpolationType interp_type,
                                 GimpProgress *progress);
static void gimp_selection_resize(GimpItem *item, GimpContext *context,
                                  GimpFillType fill_type, gint new_width,
                                  gint new_height, gint offset_x,
                                  gint offset_y);
static void gimp_selection_flip(GimpItem *item, GimpContext *context,
                                GimpOrientationType flip_type, gdouble axis,
                                gboolean clip_result);
static void gimp_selection_rotate(GimpItem *item, GimpContext *context,
                                  GimpRotationType rotation_type,
                                  gdouble center_x, gdouble center_y,
                                  gboolean clip_result);
static gboolean gimp_selection_fill(GimpItem *item, GimpDrawable *drawable,
                                    GimpFillOptions *fill_options,
                                    gboolean push_undo, GimpProgress *progress,
                                    GError **error);
static gboolean gimp_selection_stroke(GimpItem *item, GimpDrawable *drawable,
                                      GimpStrokeOptions *stroke_options,
                                      gboolean push_undo,
                                      GimpProgress *progress, GError **error);
static void gimp_selection_convert_type(
...
static gboolean gimp_selection_boundary(GimpChannel *channel,
                                        ...
                                        gint *num_segs_in, gint *num_segs_out,
                                        gint x1, gint y1, gint x2, gint y2);
static gboolean gimp_selection_is_empty(GimpChannel *channel);
static void gimp_selection_feather(GimpChannel *channel, gdouble radius_x,
                                   gdouble radius_y, gboolean edge_lock,
                                   gboolean push_undo);
static void gimp_selection_sharpen(GimpChannel *channel, gboolean push_undo);
static void gimp_selection_clear(GimpChannel *channel, const gchar *undo_desc,
                                 gboolean push_undo);
static void gimp_selection_all(GimpChannel *channel, gboolean push_undo);
static void gimp_selection_invert(GimpChannel *channel, gboolean push_undo);
static void gimp_selection_border(GimpChannel *channel, gint radius_x,
                                  gint radius_y, GimpChannelBorderStyle style,
                                  gboolean edge_lock, gboolean push_undo);
static void gimp_selection_grow(GimpChannel *channel, gint radius_x,
                                gint radius_y, gboolean push_undo);
static void gimp_selection_shrink(GimpChannel *channel, gint radius_x,
                                  gint radius_y, gboolean edge_lock,
                                  gboolean push_undo);
static void gimp_selection_flood(GimpChannel *channel, gboolean push_undo);
...
static GimpItemTree *gimp_selection_get_tree(GimpItem *item) { return NULL; }
...
static void gimp_selection_rotate(GimpItem *item, GimpContext *context,
                                  ...
                                  gboolean clip_result) {
  GIMP_ITEM_CLASS(parent_class)
      ->rotate(item, context, rotation_type, center_x, center_y, clip_result);
}
...
static gboolean gimp_selection_fill(GimpItem *item, GimpDrawable *drawable,
  ...
  if (!gimp_channel_boundary(GIMP_CHANNEL(selection), &dummy_in, &dummy_out,
                             &num_dummy_in, &num_dummy_out, 0, 0, 0, 0)) {
  ...
  retval = GIMP_ITEM_CLASS(parent_class)
               ->fill(item, drawable, fill_options, push_undo, progress, error);
...
static gboolean gimp_selection_stroke(GimpItem *item, GimpDrawable *drawable,
  ...
  retval =
      GIMP_ITEM_CLASS(parent_class)
          ->stroke(item, drawable, stroke_options, push_undo, progress, error);
...
static void gimp_selection_convert_type(
  ...
  GIMP_DRAWABLE_CLASS(parent_class)
      ->convert_type(drawable, dest_image, new_format, src_profile,
                     dest_profile, layer_dither_type, mask_dither_type,
                     push_undo, progress);
}
...
static gboolean gimp_selection_boundary(GimpChannel *channel,
                                        ...
                                        const GimpBoundSeg **segs_out,
                                        gint *num_segs_in, gint *num_segs_out,
                                        gint unused1, gint unused2,
                                        gint unused3, gint unused4) {
  ...
  if ((floating_selection = gimp_image_get_floating_selection(image))) {
    ...
    GIMP_CHANNEL_CLASS(parent_class)
        ->boundary(channel, segs_in, segs_out, num_segs_in, num_segs_out, 0, 0,
                   0, 0);
    ...
    *segs_in = floating_sel_boundary(floating_selection, num_segs_in);
    ...
    return GIMP_CHANNEL_CLASS(parent_class)
        ->boundary(channel, segs_in, segs_out, num_segs_in, num_segs_out, 0, 0,
    ...
    return GIMP_CHANNEL_CLASS(parent_class)
        ->boundary(channel, segs_in, segs_out, num_segs_in, num_segs_out, x1,
                   y1, x2, y2);
  }
...
static void gimp_selection_feather(GimpChannel *channel, gdouble radius_x,
                                   ...
                                   gboolean push_undo) {
  GIMP_CHANNEL_CLASS(parent_class)
      ->feather(channel, radius_x, radius_y, edge_lock, push_undo);
}
...
static void gimp_selection_border(GimpChannel *channel, gint radius_x,
                                  ...
                                  gboolean edge_lock, gboolean push_undo) {
  GIMP_CHANNEL_CLASS(parent_class)
      ->border(channel, radius_x, radius_y, style, edge_lock, push_undo);
}

static void gimp_selection_grow(GimpChannel *channel, gint radius_x,
                                gint radius_y, gboolean push_undo) {
  GIMP_CHANNEL_CLASS(parent_class)
      ->grow(channel, radius_x, radius_y, push_undo);
}
...
static void gimp_selection_shrink(GimpChannel *channel, gint radius_x,
                                  ...
                                  gboolean push_undo) {
  GIMP_CHANNEL_CLASS(parent_class)
      ->shrink(channel, radius_x, radius_y, edge_lock, push_undo);
}
...
GimpChannel *gimp_selection_new(GimpImage *image, gint width, gint height) {
  ...
  channel = GIMP_CHANNEL(gimp_drawable_new(GIMP_TYPE_SELECTION, image, NULL, 0,
                                           0, width, height,
...
GeglBuffer *gimp_selection_extract(GimpSelection *selection, GList *pickables,
  ...
  if (GIMP_IS_DRAWABLE(pickable)) {
    non_empty = gimp_item_mask_bounds(GIMP_ITEM(pickable), &x1, &y1, &x2, &y2);

    gimp_item_get_offset(GIMP_ITEM(pickable), &off_x, &off_y);
  } else {
    non_empty = gimp_item_bounds(GIMP_ITEM(selection), &x1, &y1, &x2, &y2);
    x2 += x1;
    y2 += y1;
  ...
  if (babl_format_is_palette(src_format) && !keep_indexed) {
    dest_format =
        gimp_image_get_format(image, GIMP_RGB, gimp_image_get_precision(image),
                              add_alpha || babl_format_has_alpha(src_format),
  ...
  dest_buffer =
      gegl_buffer_new(GEGL_RECTANGLE(0, 0, x2 - x1, y2 - y1), dest_format);
  ...
  if (non_empty) {
    ...
    gimp_gegl_apply_opacity(dest_buffer, NULL, NULL, dest_buffer, mask_buffer,
                            -(off_x + x1), -(off_y + y1), 1.0);
    ...
    if (GIMP_IS_LAYER(pickable)) {
      gimp_image_remove_layer(image, GIMP_LAYER(pickable), TRUE, NULL);
    } else if (GIMP_IS_LAYER_MASK(pickable)) {
      gimp_layer_apply_mask(
          gimp_layer_mask_get_layer(GIMP_LAYER_MASK(pickable)),
          GIMP_MASK_DISCARD, TRUE);
    } else if (GIMP_IS_CHANNEL(pickable)) {
      gimp_image_remove_channel(image, GIMP_CHANNEL(pickable), TRUE, NULL);
    }
  }
...
GimpLayer *gimp_selection_float(GimpSelection *selection, GList *drawables,
  ...
  for (iter = drawables; iter; iter = iter->next) {
    ...
    if (!format)
      ...
      g_return_val_if_fail(
          format == gimp_drawable_get_format_with_alpha(iter->data), NULL);
  }
  ...
  for (iter = drawables; iter; iter = iter->next) {
    if (gimp_item_mask_bounds(iter->data, &x1, &y1, &x2, &y2) && x1 != x2 &&
        y1 != y2)
      break;
  }
  if (iter == NULL) {
  ...
  buffer = gimp_selection_extract(selection, drawables, context, cut_image,
                                  FALSE, TRUE, &x1, &y1, NULL);

  profile =
      gimp_color_managed_get_color_profile(GIMP_COLOR_MANAGED(drawables->data));
  ...
  layer = gimp_layer_new_from_gegl_buffer(
      buffer, image, format, _("Floated Layer"), GIMP_OPACITY_OPAQUE,
      gimp_image_get_default_new_layer_mode(image), profile);
