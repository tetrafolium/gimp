...
enum { PROP_0, PROP_CONTAINER, PROP_FLAT };
...
static void gimp_tree_proxy_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_tree_proxy_get_property(GObject *object, guint property_id,
...
static gint gimp_tree_proxy_add_container(GimpTreeProxy *tree_proxy,
                                          GimpContainer *container, gint index);
...
static void gimp_tree_proxy_class_init(GimpTreeProxyClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_FLAT,
      g_param_spec_boolean("flat", NULL, NULL, FALSE, GIMP_PARAM_READWRITE));
}

static void gimp_tree_proxy_init(GimpTreeProxy *tree_proxy) {
...
static void gimp_tree_proxy_add_container_func(GimpObject *object,
                                               AddContainerData *data) {
  data->index =
      gimp_tree_proxy_add_object(data->tree_proxy, object, data->index);
}
...
static gint gimp_tree_proxy_add_container(GimpTreeProxy *tree_proxy,
  ...
  g_signal_connect(container, "add", G_CALLBACK(gimp_tree_proxy_container_add),
                   tree_proxy);
  g_signal_connect(container, "remove",
                   G_CALLBACK(gimp_tree_proxy_container_remove), tree_proxy);
  g_signal_connect(container, "reorder",
                   G_CALLBACK(gimp_tree_proxy_container_reorder), tree_proxy);
  g_signal_connect(container, "freeze",
                   G_CALLBACK(gimp_tree_proxy_container_freeze), tree_proxy);
  g_signal_connect(container, "thaw",
                   G_CALLBACK(gimp_tree_proxy_container_thaw), tree_proxy);
  ...
  gimp_container_foreach(container, (GFunc)gimp_tree_proxy_add_container_func,
                         &data);
...
static void gimp_tree_proxy_remove_container_func(GimpObject *object,
                                                  GimpTreeProxy *tree_proxy) {
  gimp_tree_proxy_remove_object(tree_proxy, object);
}
...
static void gimp_tree_proxy_remove_container(GimpTreeProxy *tree_proxy,
  ...
  gimp_container_foreach(
      container, (GFunc)gimp_tree_proxy_remove_container_func, tree_proxy);

  gimp_container_thaw(GIMP_CONTAINER(tree_proxy));

  g_signal_handlers_disconnect_by_func(container, gimp_tree_proxy_container_add,
                                       tree_proxy);
  g_signal_handlers_disconnect_by_func(
      container, gimp_tree_proxy_container_remove, tree_proxy);
  g_signal_handlers_disconnect_by_func(
      container, gimp_tree_proxy_container_reorder, tree_proxy);
  g_signal_handlers_disconnect_by_func(
      container, gimp_tree_proxy_container_freeze, tree_proxy);
  g_signal_handlers_disconnect_by_func(
      container, gimp_tree_proxy_container_thaw, tree_proxy);
}
...
gimp_tree_proxy_find_container_search_func(GimpObject *object,
  ...
  if (children) {
    ...
    return gimp_container_search(children,
                                 (GimpContainerSearchFunc)
                                     gimp_tree_proxy_find_container_search_func,
                                 data) != NULL;
  }
...
static gboolean gimp_tree_proxy_find_object_search_func(GimpObject *object,
                                                        FindObjectData *data) {
  GimpContainer *children;
...
GimpContainer *gimp_tree_proxy_new(GType children_type) {
  ...
  g_return_val_if_fail(
      G_TYPE_CHECK_CLASS_TYPE(children_class, GIMP_TYPE_VIEWABLE), NULL);

  g_type_class_unref(children_class);

  return g_object_new(GIMP_TYPE_TREE_PROXY, "children-type", children_type,
                      "policy", GIMP_CONTAINER_POLICY_WEAK, "append", TRUE,
                      NULL);
}
...
void gimp_tree_proxy_set_container(GimpTreeProxy *tree_proxy,
  ...
  if (container) {
    ...
    children_class =
        g_type_class_ref(gimp_container_get_children_type(container));

    g_return_if_fail(G_TYPE_CHECK_CLASS_TYPE(
        children_class,
  ...
  if (container != tree_proxy->priv->container) {
    ...
    if (tree_proxy->priv->container) {
      gimp_tree_proxy_remove_container(tree_proxy, tree_proxy->priv->container);
    }

    g_set_object(&tree_proxy->priv->container, container);

    if (tree_proxy->priv->container) {
      gimp_tree_proxy_add_container(tree_proxy, tree_proxy->priv->container,
                                    -1);
    }
...
void gimp_tree_proxy_set_flat(GimpTreeProxy *tree_proxy, gboolean flat) {
  g_return_if_fail(GIMP_IS_TREE_PROXY(tree_proxy));
