...
gboolean gimp_drawable_has_filters(GimpDrawable *drawable) {
  ...
  for (list = GIMP_LIST(drawable->private->filter_stack)->queue->head; list;
...
void gimp_drawable_add_filter(GimpDrawable *drawable, GimpFilter *filter) {
  ...
  gimp_container_add(drawable->private->filter_stack, GIMP_OBJECT(filter));
}
...
void gimp_drawable_remove_filter(GimpDrawable *drawable, GimpFilter *filter) {
  ...
  gimp_container_remove(drawable->private->filter_stack, GIMP_OBJECT(filter));
}
...
gboolean gimp_drawable_merge_filter(GimpDrawable *drawable, GimpFilter *filter,
  ...
  if (!clip &&
      gegl_rectangle_equal(&rect, gegl_buffer_get_extent(dest_buffer))) {
    clip = TRUE;
  }

  if (clip) {
    if (!gimp_item_mask_intersect(GIMP_ITEM(drawable), &rect.x, &rect.y,
                                  &rect.width, &rect.height)) {
      return TRUE;
    }

    if (format != gimp_drawable_get_format(drawable)) {
      buffer = gegl_buffer_new(gegl_buffer_get_extent(dest_buffer), format);

      dest_buffer = buffer;
    }
  } else {
    buffer =
        gegl_buffer_new(GEGL_RECTANGLE(0, 0, rect.width, rect.height), format);

    dest_buffer = g_object_new(GEGL_TYPE_BUFFER, "source", buffer, "shift-x",
                               -rect.x, "shift-y", -rect.y, NULL);
  }
  ...
  if (applicator) {
     ...
     */
    cache = gimp_applicator_get_cache_buffer(applicator, &rects, &n_rects);
  ...
  if (!buffer) {
    gegl_rectangle_align_to_buffer(&undo_rect, &rect,
                                   gimp_drawable_get_buffer(drawable),
                                   GEGL_RECTANGLE_ALIGNMENT_SUPERSET);

    undo_buffer =
        gegl_buffer_new(GEGL_RECTANGLE(0, 0, undo_rect.width, undo_rect.height),
                        gimp_drawable_get_format(drawable));

    gimp_gegl_buffer_copy(gimp_drawable_get_buffer(drawable), &undo_rect,
                          GEGL_ABYSS_NONE, undo_buffer,
  ...
  if (gimp_gegl_apply_cached_operation(
          gimp_drawable_get_buffer(drawable), progress, undo_desc,
          gimp_filter_get_node(filter), FALSE, dest_buffer, &rect, FALSE, cache,
          rects, n_rects, cancellable)) {
    /*  finished successfully  */

    if (clip) {
      if (buffer) {
        gimp_drawable_set_buffer_full(drawable, TRUE, undo_desc, buffer, NULL,
                                      FALSE);
      } else {
        gimp_drawable_push_undo(drawable, undo_desc, undo_buffer, undo_rect.x,
                                undo_rect.y, undo_rect.width, undo_rect.height);
      }
    } else {
      ...
      gimp_drawable_set_buffer_full(
          ...
          GEGL_RECTANGLE(offset_x + rect.x, offset_y + rect.y, 0, 0), FALSE);

      if (mask) {
        gimp_item_resize(GIMP_ITEM(mask), gimp_get_default_context(image->gimp),
                         GIMP_FILL_TRANSPARENT, rect.width, rect.height,
                         -rect.x, -rect.y);
    ...
    if (clip) {
      gimp_gegl_buffer_copy(
          undo_buffer, GEGL_RECTANGLE(0, 0, undo_rect.width, undo_rect.height),
          GEGL_ABYSS_NONE, gimp_drawable_get_buffer(drawable), &undo_rect);
    }
