...
static gint64 gimp_channel_get_memsize(GimpObject *object, gint64 *gui_size);
...
static gboolean gimp_channel_bounds(GimpItem *item, gdouble *x, gdouble *y,
                                    gdouble *width, gdouble *height);
static GimpItem *gimp_channel_duplicate(GimpItem *item, GType new_type);
static void gimp_channel_convert(GimpItem *item, GimpImage *dest_image,
                                 GType old_type);
static void gimp_channel_translate(GimpItem *item, gdouble off_x, gdouble off_y,
                                   gboolean push_undo);
static void gimp_channel_scale(GimpItem *item, gint new_width, gint new_height,
                               gint new_offset_x, gint new_offset_y,
                               GimpInterpolationType interp_type,
                               GimpProgress *progress);
static void gimp_channel_resize(GimpItem *item, GimpContext *context,
                                GimpFillType fill_type, gint new_width,
                                gint new_height, gint offset_x, gint offset_y);
static GimpTransformResize
gimp_channel_get_clip(GimpItem *item, GimpTransformResize clip_result);
static gboolean gimp_channel_fill(GimpItem *item, GimpDrawable *drawable,
                                  GimpFillOptions *fill_options,
                                  gboolean push_undo, GimpProgress *progress,
                                  GError **error);
static gboolean gimp_channel_stroke(GimpItem *item, GimpDrawable *drawable,
                                    GimpStrokeOptions *stroke_options,
                                    gboolean push_undo, GimpProgress *progress,
                                    GError **error);
static void gimp_channel_to_selection(GimpItem *item, GimpChannelOps op,
                                      gboolean antialias, gboolean feather,
                                      gdouble feather_radius_x,
                                      gdouble feather_radius_y);

static void gimp_channel_convert_type(
...
static void gimp_channel_set_buffer(GimpDrawable *drawable, gboolean push_undo,
                                    const gchar *undo_desc, GeglBuffer *buffer,
                                    const GeglRectangle *bounds);

static gdouble gimp_channel_get_opacity_at(GimpPickable *pickable, gint x,
                                           gint y);
...
static gboolean gimp_channel_real_boundary(GimpChannel *channel,
                                           ...
                                           gint *num_segs_in,
                                           gint *num_segs_out, gint x1, gint y1,
                                           gint x2, gint y2);
static gboolean gimp_channel_real_is_empty(GimpChannel *channel);
static void gimp_channel_real_feather(GimpChannel *channel, gdouble radius_x,
                                      gdouble radius_y, gboolean edge_lock,
                                      gboolean push_undo);
static void gimp_channel_real_sharpen(GimpChannel *channel, gboolean push_undo);
static void gimp_channel_real_clear(GimpChannel *channel,
                                    const gchar *undo_desc, gboolean push_undo);
static void gimp_channel_real_all(GimpChannel *channel, gboolean push_undo);
static void gimp_channel_real_invert(GimpChannel *channel, gboolean push_undo);
static void gimp_channel_real_border(GimpChannel *channel, gint radius_x,
                                     gint radius_y,
                                     GimpChannelBorderStyle style,
                                     gboolean edge_lock, gboolean push_undo);
static void gimp_channel_real_grow(GimpChannel *channel, gint radius_x,
                                   gint radius_y, gboolean push_undo);
static void gimp_channel_real_shrink(GimpChannel *channel, gint radius_x,
                                     gint radius_y, gboolean edge_lock,
                                     gboolean push_undo);
static void gimp_channel_real_flood(GimpChannel *channel, gboolean push_undo);
...
static void gimp_channel_class_init(GimpChannelClass *klass) {
  ...
  channel_signals[COLOR_CHANGED] = g_signal_new(
      "color-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpChannelClass, color_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
...
static gchar *gimp_channel_get_description(GimpViewable *viewable,
                                           gchar **tooltip) {
  if (!strcmp(GIMP_IMAGE_QUICK_MASK_NAME, gimp_object_get_name(viewable))) {
...
static GeglNode *gimp_channel_get_node(GimpFilter *filter) {
  ...
  channel->color_node = gegl_node_new_child(node, "operation", "gegl:color",
                                            "format", color_format, NULL);
  gimp_gegl_node_set_color(channel->color_node, &channel->color, NULL);

  g_warn_if_fail(channel->mask_node == NULL);

  channel->mask_node =
      gegl_node_new_child(node, "operation", "gegl:opacity", NULL);
  gegl_node_connect_to(channel->color_node, "output", channel->mask_node,
                       "input");

  g_warn_if_fail(channel->invert_node == NULL);

  channel->invert_node =
      gegl_node_new_child(node, "operation", "gegl:invert-linear", NULL);

  if (channel->show_masked) {
    gegl_node_connect_to(source, "output", channel->invert_node, "input");
    gegl_node_connect_to(channel->invert_node, "output", channel->mask_node,
                         "aux");
  } else {
    gegl_node_connect_to(source, "output", channel->mask_node, "aux");
  }

  mode_node = gimp_drawable_get_mode_node(drawable);

  gegl_node_connect_to(channel->mask_node, "output", mode_node, "aux");
...
static gboolean gimp_channel_is_attached(GimpItem *item) {
  ...
  return (
      GIMP_IS_IMAGE(image) &&
      gimp_container_have(gimp_image_get_channels(image), GIMP_OBJECT(item)));
}
...
static gboolean gimp_channel_bounds(GimpItem *item, gdouble *x, gdouble *y,
  ...
  if (!channel->bounds_known) {
    ...
    channel->empty = !gimp_gegl_mask_bounds(buffer, &channel->x1, &channel->y1,
  ...
  if (GIMP_IS_CHANNEL(new_item)) {
    ...
    if (new_type == GIMP_TYPE_CHANNEL) {
      ...
      if (format != gimp_drawable_get_format(new_drawable)) {
        ...
        new_buffer =
            gegl_buffer_new(GEGL_RECTANGLE(0, 0, gimp_item_get_width(new_item),
                                           gimp_item_get_height(new_item)),
                            format);

        gegl_buffer_set_format(new_buffer,
                               gimp_drawable_get_format(new_drawable));
        gimp_gegl_buffer_copy(gimp_drawable_get_buffer(new_drawable), NULL,
...
static void gimp_channel_convert(GimpItem *item, GimpImage *dest_image,
  ...
  if (!gimp_drawable_is_gray(drawable)) {
    gimp_drawable_convert_type(drawable, dest_image, GIMP_GRAY,
                               ...
                               gimp_drawable_has_alpha(drawable), NULL, NULL,
                               GEGL_DITHER_NONE, GEGL_DITHER_NONE, FALSE, NULL);
  }
  ...
  if (gimp_drawable_has_alpha(drawable)) {
    ...
    gimp_gegl_apply_flatten(gimp_drawable_get_buffer(drawable), NULL, NULL,
                            new_buffer, &background, NULL,
                            GIMP_LAYER_COLOR_SPACE_RGB_LINEAR);

    gimp_drawable_set_buffer_full(drawable, FALSE, NULL, new_buffer,
                                  GEGL_RECTANGLE(gimp_item_get_offset_x(item),
  ...
  if (G_TYPE_FROM_INSTANCE(channel) == GIMP_TYPE_CHANNEL) {
    ...
    if (gimp_item_get_width(item) != width ||
      ...
      gimp_item_resize(item, gimp_get_user_context(dest_image->gimp),
                       GIMP_FILL_TRANSPARENT, width, height, 0, 0);
    }
  }
...
static void gimp_channel_translate(GimpItem *item, gdouble off_x, gdouble off_y,
  ...
  if (gimp_rectangle_intersect(
          x + SIGNED_ROUND(off_x), y + SIGNED_ROUND(off_y), width, height, 0, 0,
          gimp_item_get_width(GIMP_ITEM(channel)),
          gimp_item_get_height(GIMP_ITEM(channel)), &x, &y, &width, &height)) {
    ...
    gimp_gegl_buffer_copy(
        ...
        GEGL_RECTANGLE(x - SIGNED_ROUND(off_x), y - SIGNED_ROUND(off_y), width,
                       height),
        GEGL_ABYSS_NONE, tmp_buffer, GEGL_RECTANGLE(0, 0, 0, 0));

    /*  clear the mask  */
    gegl_buffer_clear(gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)), NULL);
  ...
  /*  update the new area  */
  gimp_drawable_update(GIMP_DRAWABLE(item), channel->x1, channel->y1,
                       channel->x2 - channel->x1, channel->y2 - channel->y1);
}
...
static void gimp_channel_scale(GimpItem *item, gint new_width, gint new_height,
  ...
  if (channel->bounds_known && channel->empty) {
    ...
    gimp_drawable_set_buffer_full(
        drawable, gimp_item_is_attached(item), NULL, new_buffer,
        GEGL_RECTANGLE(new_offset_x, new_offset_y, 0, 0), TRUE);
    g_object_unref(new_buffer);

    gimp_channel_clear(GIMP_CHANNEL(item), NULL, FALSE);
  } else {
    GIMP_ITEM_CLASS(parent_class)
        ->scale(item, new_width, new_height, new_offset_x, new_offset_y,
...
static void gimp_channel_resize(GimpItem *item, GimpContext *context,
                                ...
                                gint new_height, gint offset_x, gint offset_y) {
  GIMP_ITEM_CLASS(parent_class)
      ->resize(item, context, GIMP_FILL_TRANSPARENT, new_width, new_height,
               offset_x, offset_y);
...
static gboolean gimp_channel_fill(GimpItem *item, GimpDrawable *drawable,
  ...
  if (!gimp_channel_boundary(channel, &segs_in, &segs_out, &n_segs_in,
                             &n_segs_out, 0, 0, 0, 0)) {
  ...
  gimp_drawable_fill_boundary(drawable, fill_options, segs_in, n_segs_in,
...
static gboolean gimp_channel_stroke(GimpItem *item, GimpDrawable *drawable,
  ...
  case GIMP_STROKE_LINE:
    gimp_drawable_stroke_boundary(drawable, stroke_options, segs_in, n_segs_in,
                                  offset_x, offset_y, push_undo);
    retval = TRUE;
    break;
  ...
  case GIMP_STROKE_PAINT_METHOD: {
    ...
    retval = gimp_paint_core_stroke_boundary(
        core, drawable, paint_options, emulate_dynamics, segs_in, n_segs_in,
        offset_x, offset_y, push_undo, error);
...
static void gimp_channel_to_selection(GimpItem *item, GimpChannelOps op,
  ...
  gimp_channel_select_channel(
      gimp_image_get_mask(image), GIMP_ITEM_GET_CLASS(item)->to_selection_desc,
      channel, off_x, off_y, op, feather, feather_radius_x, feather_radius_x);
}
...
static void gimp_channel_convert_type(
  ...
  dest_buffer = gegl_buffer_new(
      GEGL_RECTANGLE(0, 0, gimp_item_get_width(GIMP_ITEM(drawable)),
...
static void gimp_channel_set_buffer(GimpDrawable *drawable, gboolean push_undo,
  ...
  if (old_buffer) {
    g_signal_handlers_disconnect_by_func(old_buffer,
                                         gimp_channel_buffer_changed, channel);
  }
  ...
  gegl_buffer_signal_connect(buffer, "changed",
                             G_CALLBACK(gimp_channel_buffer_changed), channel);
  ...
  if (gimp_filter_peek_node(GIMP_FILTER(channel))) {
    ...
    gegl_node_set(channel->color_node, "format", color_format, NULL);
  }
}
...
static gdouble gimp_channel_get_opacity_at(GimpPickable *pickable, gint x,
  ...
  if (x >= 0 && x < gimp_item_get_width(GIMP_ITEM(channel)) && y >= 0 &&
      y < gimp_item_get_height(GIMP_ITEM(channel))) {
    if (!channel->bounds_known ||
        (!channel->empty && x >= channel->x1 && x < channel->x2 &&
         y >= channel->y1 && y < channel->y2)) {
      gegl_buffer_sample(gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)), x, y,
                         NULL, &value, babl_format("Y double"),
...
static gboolean gimp_channel_real_boundary(GimpChannel *channel,
  ...
  if (!channel->boundary_known) {
    ...
    if (gimp_item_bounds(GIMP_ITEM(channel), &x3, &y3, &x4, &y4)) {
      ...
      channel->segs_out = gimp_boundary_find(
          buffer, &rect, babl_format("Y float"), GIMP_BOUNDARY_IGNORE_BOUNDS,
          x1, y1, x2, y2, GIMP_BOUNDARY_HALF_WAY, &channel->num_segs_out);
      x1 = MAX(x1, x3);
      y1 = MAX(y1, y3);
      x2 = MIN(x2, x4);
      y2 = MIN(y2, y4);
      ...
      if (x2 > x1 && y2 > y1) {
        channel->segs_in = gimp_boundary_find(
            buffer, NULL, babl_format("Y float"), GIMP_BOUNDARY_WITHIN_BOUNDS,
            x1, y1, x2, y2, GIMP_BOUNDARY_HALF_WAY, &channel->num_segs_in);
      } else {
        channel->segs_in = NULL;
        channel->num_segs_in = 0;
      }
...
static void gimp_channel_real_feather(GimpChannel *channel, gdouble radius_x,
  ...
  gimp_gegl_apply_feather(
      gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)), NULL, NULL,
      gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)),
      GEGL_RECTANGLE(x1, y1, x2 - x1, y2 - y1), radius_x, radius_y, edge_lock);
...
static void gimp_channel_real_sharpen(GimpChannel *channel,
  ...
  gimp_gegl_apply_threshold(gimp_drawable_get_buffer(drawable), NULL, NULL,
                            gimp_drawable_get_buffer(drawable), 0.5);
...
static void gimp_channel_real_clear(GimpChannel *channel,
  ...
  gimp_drawable_update(GIMP_DRAWABLE(channel), rect.x, rect.y, rect.width,
                       rect.height);
}
...
static void gimp_channel_real_all(GimpChannel *channel, gboolean push_undo) {
  ...
  if (push_undo)
    gimp_channel_push_undo(channel, GIMP_CHANNEL_GET_CLASS(channel)->all_desc);

  /*  clear the channel  */
  color = gegl_color_new("#fff");
  gegl_buffer_set_color(gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)), NULL,
                        color);
...
static void gimp_channel_real_invert(GimpChannel *channel, gboolean push_undo) {
  ...
  if (channel->bounds_known && channel->empty) {
    ...
    gimp_gegl_apply_invert_linear(gimp_drawable_get_buffer(drawable), NULL,
...
static void gimp_channel_real_border(GimpChannel *channel, gint radius_x,
  ...
  if (radius_x == 0 && radius_y == 0) {
     ...
     */
    gimp_channel_clear(channel, GIMP_CHANNEL_GET_CLASS(channel)->border_desc,
                       push_undo);
    return;
  ...
  gimp_gegl_apply_border(gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)), NULL,
                         NULL, gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)),
                         GEGL_RECTANGLE(x1, y1, x2 - x1, y2 - y1), radius_x,
                         radius_y, style, edge_lock);
...
static void gimp_channel_real_grow(GimpChannel *channel, gint radius_x,
  ...
  if (push_undo)
    gimp_channel_push_undo(channel, GIMP_CHANNEL_GET_CLASS(channel)->grow_desc);

  gimp_gegl_apply_grow(gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)), NULL,
                       NULL, gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)),
                       GEGL_RECTANGLE(x1, y1, x2 - x1, y2 - y1), radius_x,
                       radius_y);
...
static void gimp_channel_real_shrink(GimpChannel *channel, gint radius_x,
  ...
  gimp_gegl_apply_shrink(gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)), NULL,
                         NULL, gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)),
                         GEGL_RECTANGLE(x1, y1, x2 - x1, y2 - y1), radius_x,
                         radius_y, edge_lock);
...
static void gimp_channel_real_flood(GimpChannel *channel, gboolean push_undo) {
  ...
  gimp_gegl_apply_flood(gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)), NULL,
                        NULL, gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)),
...
GimpChannel *gimp_channel_new(GimpImage *image, gint width, gint height,
  ...
  channel = GIMP_CHANNEL(
      gimp_drawable_new(GIMP_TYPE_CHANNEL, image, name, 0, 0, width, height,
...
GimpChannel *gimp_channel_new_from_buffer(GimpImage *image, GeglBuffer *buffer,
                                          const gchar *name,
                                          const GimpRGB *color) {
  GimpChannel *channel;
  GeglBuffer *dest;
  ...
  channel = gimp_channel_new(image, gegl_buffer_get_width(buffer),
                             gegl_buffer_get_height(buffer), name, color);
...
void gimp_channel_set_color(GimpChannel *channel, const GimpRGB *color,
                            gboolean push_undo) {
  ...
  if (gimp_rgba_distance(&channel->color, color) > RGBA_EPSILON) {
    if (push_undo && gimp_item_is_attached(GIMP_ITEM(channel))) {
      ...
      gimp_image_undo_push_channel_color(
          image, C_("undo-type", "Set Channel Color"), channel);
    }
...
void gimp_channel_get_color(GimpChannel *channel, GimpRGB *color) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
  g_return_if_fail(color != NULL);
...
void gimp_channel_set_opacity(GimpChannel *channel, gdouble opacity,
                              gboolean push_undo) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
  ...
  if (channel->color.a != opacity) {
    if (push_undo && gimp_item_is_attached(GIMP_ITEM(channel))) {
      ...
      gimp_image_undo_push_channel_color(
          image, C_("undo-type", "Set Channel Opacity"), channel);
    }
...
void gimp_channel_push_undo(GimpChannel *channel, const gchar *undo_desc) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
  g_return_if_fail(gimp_item_is_attached(GIMP_ITEM(channel)));

  gimp_image_undo_push_mask(gimp_item_get_image(GIMP_ITEM(channel)), undo_desc,
                            channel);
}
...
GimpChannel *gimp_channel_new_mask(GimpImage *image, gint width, gint height) {
  GimpChannel *channel;

  g_return_val_if_fail(GIMP_IS_IMAGE(image), NULL);

  channel = GIMP_CHANNEL(
      gimp_drawable_new(GIMP_TYPE_CHANNEL, image, _("Selection Mask"), 0, 0,
                        width, height, gimp_image_get_mask_format(image)));
...
gboolean gimp_channel_boundary(GimpChannel *channel,
                               const GimpBoundSeg **segs_in,
                               const GimpBoundSeg **segs_out, gint *num_segs_in,
                               gint *num_segs_out, gint x1, gint y1, gint x2,
                               gint y2) {
  g_return_val_if_fail(GIMP_IS_CHANNEL(channel), FALSE);
  ...
  return GIMP_CHANNEL_GET_CLASS(channel)->boundary(
      channel, segs_in, segs_out, num_segs_in, num_segs_out, x1, y1, x2, y2);
}

gboolean gimp_channel_is_empty(GimpChannel *channel) {
  g_return_val_if_fail(GIMP_IS_CHANNEL(channel), FALSE);

  return GIMP_CHANNEL_GET_CLASS(channel)->is_empty(channel);
}

void gimp_channel_feather(GimpChannel *channel, gdouble radius_x,
                          gdouble radius_y, gboolean edge_lock,
                          gboolean push_undo) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
...
void gimp_channel_sharpen(GimpChannel *channel, gboolean push_undo) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
...
void gimp_channel_clear(GimpChannel *channel, const gchar *undo_desc,
                        gboolean push_undo) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
...
void gimp_channel_all(GimpChannel *channel, gboolean push_undo) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
...
void gimp_channel_invert(GimpChannel *channel, gboolean push_undo) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
...
void gimp_channel_border(GimpChannel *channel, gint radius_x, gint radius_y,
                         GimpChannelBorderStyle style, gboolean edge_lock,
                         gboolean push_undo) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
  ...
  GIMP_CHANNEL_GET_CLASS(channel)->border(channel, radius_x, radius_y, style,
                                          edge_lock, push_undo);
}

void gimp_channel_grow(GimpChannel *channel, gint radius_x, gint radius_y,
                       gboolean push_undo) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
  ...
  GIMP_CHANNEL_GET_CLASS(channel)->grow(channel, radius_x, radius_y, push_undo);
}

void gimp_channel_shrink(GimpChannel *channel, gint radius_x, gint radius_y,
                         gboolean edge_lock, gboolean push_undo) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
...
void gimp_channel_flood(GimpChannel *channel, gboolean push_undo) {
  g_return_if_fail(GIMP_IS_CHANNEL(channel));
