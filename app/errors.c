...
static void gimp_message_log_func(const gchar *log_domain, GLogLevelFlags flags,
                                  const gchar *message, gpointer data);
static void gimp_error_log_func(const gchar *domain, GLogLevelFlags flags,
                                const gchar *message,
                                gpointer data) G_GNUC_NORETURN;

static G_GNUC_NORETURN void gimp_eek(const gchar *reason, const gchar *message,
                                     gboolean use_handler);

/*  public functions  */

void errors_init(Gimp *gimp, const gchar *_full_prog_name,
  ...
  g_free(backup_path);
  backup_path =
      g_build_filename(gimp_directory(), "backups", "backup-XXX.xcf", NULL);

  backup_file = g_file_new_for_path(backup_path);

  log_domain_handler = gimp_log_set_handler(
      FALSE, G_LOG_LEVEL_WARNING | G_LOG_LEVEL_MESSAGE | G_LOG_LEVEL_CRITICAL,
      gimp_message_log_func, gimp);

  global_handler_id = g_log_set_handler(
      NULL, G_LOG_LEVEL_ERROR | G_LOG_FLAG_FATAL, gimp_error_log_func, gimp);
}
...
static void gimp_message_log_func(const gchar *log_domain, GLogLevelFlags flags,
   ...
   * messages.
   */
  if (!log_domain || (!g_str_has_prefix(log_domain, "Gimp") &&
   ...
   */
  g_object_get(G_OBJECT(config), "debug-policy", &debug_policy, NULL);
  ...
  if (gimp && gui_message) {
    ...
    gimp_enum_get_value(GIMP_TYPE_MESSAGE_SEVERITY, severity, NULL, NULL,
                        &reason, NULL);
...
static void gimp_eek(const gchar *reason, const gchar *message,
                     gboolean use_handler) {
  ...
  if (use_handler) {
    ...
    if (debug_policy != GIMP_DEBUG_POLICY_NEVER &&
        !the_errors_gimp->no_interface && backtrace_file) {
      FILE *fd;
...
#endif
      gchar *args[9] = {(gchar *)gimpdebug,
                        full_prog_name,
                        NULL,
                        (gchar *)reason,
                        (gchar *)message,
                        backtrace_file,
                        the_errors_gimp->config->last_known_release,
                        NULL,
                        NULL};
      gchar pid[16];
      gchar timestamp[16];
      ...
      g_snprintf(timestamp, 16, "%" G_GINT64_FORMAT,
                 the_errors_gimp->config->last_release_timestamp);
      args[7] = timestamp;
      ...
      has_backtrace =
          gimp_stack_trace_print((const gchar *)full_prog_name, fd, NULL);
      fclose(fd);
#endif
    ...
    if (!eek_handled) {
      ...
      case GIMP_STACK_TRACE_ALWAYS: {
        ...
        gimp_stack_trace_print((const gchar *)full_prog_name, stdout, NULL);
      } break;
  ...
  if (!eek_handled && !the_errors_gimp->no_interface)
    MessageBox(NULL, g_strdup_printf("%s: %s", reason, message), full_prog_name,
               MB_OK | MB_ICONERROR);
#endif
  ...
  if (backup_path) {
    ...
    for (; iter && i < 1000; iter = iter->next) {
      ...
      gimp_pdb_execute_procedure_by_name(
          the_errors_gimp->pdb, gimp_get_user_context(the_errors_gimp), NULL,
          NULL, "gimp-xcf-save", GIMP_TYPE_RUN_MODE, GIMP_RUN_NONINTERACTIVE,
          GIMP_TYPE_IMAGE, image, G_TYPE_INT, 0, GIMP_TYPE_OBJECT_ARRAY, NULL,
          G_TYPE_FILE, backup_file, G_TYPE_NONE);
