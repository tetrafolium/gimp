...
static gchar *gimp_library_version(const gchar *package, gint build_time_major,
                                   gint build_time_minor, gint build_time_micro,
                                   gint run_time_major, gint run_time_minor,
                                   gint run_time_micro, gboolean localized) {
  gchar *lib_version;
  gchar *build_time_version;
  gchar *run_time_version;

  build_time_version = g_strdup_printf("%d.%d.%d", build_time_major,
                                       build_time_minor, build_time_micro);
  run_time_version = g_strdup_printf("%d.%d.%d", run_time_major, run_time_minor,
                                     run_time_micro);

  /* show versions of libraries used by GIMP */
  lib_version = g_strdup_printf(
      localized ? _("using %s version %s (compiled against version %s)")
...
static gchar *gimp_library_versions(gboolean localized) {
  ...
  lib_versions = gimp_library_version(
      "babl", BABL_MAJOR_VERSION, BABL_MINOR_VERSION, BABL_MICRO_VERSION,
      babl_major_version, babl_minor_version, babl_micro_version, localized);
  ...
  lib_version = gimp_library_version(
      ...
      gegl_major_version, gegl_minor_version, gegl_micro_version, localized);
  ...
  lib_version = gimp_library_version(
      ...
      glib_major_version, glib_minor_version, glib_micro_version, localized);
  temp = g_strdup_printf("%s\n%s", lib_versions, lib_version);
  g_free(lib_versions);
  g_free(lib_version);
  lib_versions = temp;
  ...
  lib_version = gimp_library_version(
      "GdkPixbuf", GDK_PIXBUF_MAJOR, GDK_PIXBUF_MINOR, GDK_PIXBUF_MICRO,
      gdk_pixbuf_major_version, gdk_pixbuf_minor_version,
      gdk_pixbuf_micro_version, localized);
  temp = g_strdup_printf("%s\n%s", lib_versions, lib_version);
  g_free(lib_versions);
  g_free(lib_version);
  lib_versions = temp;
  ...
  lib_version = gimp_library_version(
      ...
      gtk_major_version, gtk_minor_version, gtk_micro_version, localized);
  temp = g_strdup_printf("%s\n%s", lib_versions, lib_version);
  g_free(lib_versions);
  g_free(lib_version);
  lib_versions = temp;
  ...
  lib_version = gimp_library_version(
      "Pango", PANGO_VERSION_MAJOR, PANGO_VERSION_MINOR, PANGO_VERSION_MICRO,
      pango_version() / 100 / 100, pango_version() / 100 % 100,
      pango_version() % 100, localized);
  temp = g_strdup_printf("%s\n%s", lib_versions, lib_version);
  g_free(lib_versions);
  g_free(lib_version);
  lib_versions = temp;
  ...
  lib_version = gimp_library_version(
      "Fontconfig", FC_MAJOR, FC_MINOR, FC_REVISION, FcGetVersion() / 100 / 100,
      FcGetVersion() / 100 % 100, FcGetVersion() % 100, localized);
  temp = g_strdup_printf("%s\n%s", lib_versions, lib_version);
  g_free(lib_versions);
  g_free(lib_version);
  lib_versions = temp;
...
gchar *gimp_version(gboolean be_verbose, gboolean localized) {
  gchar *version;
  gchar *temp;
  ...
  if (be_verbose) {
    ...
    verbose_info = g_strdup_printf(
        "git-describe: %s\n"
        "Build: %s rev %d for %s\n"
        "# C compiler #\n%s\n"
        "# Libraries #\n%s",
        GIMP_GIT_VERSION, GIMP_BUILD_ID, gimp_version_get_revision(),
        GIMP_BUILD_PLATFORM_FAMILY, CC_VERSION, lib_versions);
    g_free(lib_versions);
...
gint gimp_version_get_revision(void) {
   ...
   */
  if (g_key_file_load_from_file(key_file, gimp_release, G_KEY_FILE_NONE,
                                NULL)) {
