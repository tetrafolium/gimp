...
gimp_image_metadata_rotate_dialog(GimpImage *image, GimpContext *context,
...
metadata_rotation_import_dialog_run(GimpImage *image, GimpContext *context,
  ...
  return gimp_image_metadata_rotate_dialog(image, context, orientation,
                                           dont_ask);
}
...
gimp_image_metadata_rotate_dialog(GimpImage *image, GimpContext *context,
  ...
  dialog = gimp_dialog_new(
      title, "gimp-metadata-rotate-dialog", NULL, 0, NULL, NULL,

      _("_Keep Original"), GTK_RESPONSE_CANCEL, _("_Rotate"), GTK_RESPONSE_OK,
...
#define MAX_THUMBNAIL_SIZE (128 * scale_factor)
  ...
  pixbuf =
      gimp_viewable_get_pixbuf(GIMP_VIEWABLE(image), context, width, height);
  if (pixbuf) {
    GdkPixbuf *rotated;
    GtkWidget *hbox;
    GtkWidget *image;
  ...
  label = g_object_new(GTK_TYPE_LABEL, "label",
                       _("This image contains Exif orientation "
                         "metadata."),
                       "wrap", TRUE, "justify", GTK_JUSTIFY_LEFT, "xalign", 0.0,
                       "yalign", 0.5, NULL);
  ...
  label = g_object_new(
      GTK_TYPE_LABEL, "label", _("Would you like to rotate the image?"), "wrap",
      TRUE, "justify", GTK_JUSTIFY_LEFT, "xalign", 0.0, "yalign", 0.5, NULL);
  gtk_box_pack_start(GTK_BOX(main_vbox), label, FALSE, FALSE, 0);
  gtk_widget_show(label);
