...
static void item_options_dialog_response(GtkWidget *dialog, gint response_id,
...
GtkWidget *item_options_dialog_new(
    GimpImage *image, GimpItem *item, GimpContext *context, GtkWidget *parent,
    const gchar *title, const gchar *role, const gchar *icon_name,
    const gchar *desc, const gchar *help_id, const gchar *name_label,
    const gchar *lock_content_icon_name, const gchar *lock_content_label,
    const gchar *lock_position_label, const gchar *item_name,
    gboolean item_visible, gboolean item_linked, GimpColorTag item_color_tag,
    gboolean item_lock_content, gboolean item_lock_position,
    GimpItemOptionsCallback callback, gpointer user_data) {
  ItemOptionsDialog *private;
  GtkWidget *dialog;
...
private
  = g_slice_new0(ItemOptionsDialog);
...
private
  ->linked = item_linked;
private
  ->color_tag = item_color_tag;
private
  ->lock_content = item_lock_content;
private
  ->lock_position = item_lock_position;
private
  ->callback = callback;
private
  ->user_data = user_data;
  ...
  dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, viewable), context, title, role, icon_name, desc,
      parent, gimp_standard_help_func, help_id,
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(item_options_dialog_response),
                   private);

  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)item_options_dialog_free,
                    private);
...
private
  ...
  private
    ...
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, private->grid_row++, name_label,
                             0.0, 0.5, private->name_entry, 1);

    radio_box = gimp_enum_radio_box_new(GIMP_TYPE_COLOR_TAG,
                                        G_CALLBACK(gimp_radio_button_update),
                                        &private->color_tag, NULL, &radio);
    ...
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, private->grid_row++,
                             _("Color tag:"), 0.0, 0.5, radio_box, 1);
    ...
    for (list = children; list; list = g_list_next(list)) {
      ...
      color_tag =
          GPOINTER_TO_INT(g_object_get_data(G_OBJECT(radio), "gimp-item-data"));
      ...
      if (gimp_get_color_tag_color(color_tag, &color, FALSE)) {
        ...
        gimp_color_area_set_color_config(
            GIMP_COLOR_AREA(image), context->gimp->config->color_management);
        gtk_icon_size_lookup(GTK_ICON_SIZE_MENU, &w, &h);
        gtk_widget_set_size_request(image, w, h);
      } else {
        image =
            gtk_image_new_from_icon_name(GIMP_ICON_CLOSE, GTK_ICON_SIZE_MENU);
      }
...
private
  ->right_frame = gimp_frame_new(_("Switches"));
  gtk_box_pack_start(GTK_BOX(main_hbox), private->right_frame, FALSE, FALSE, 0);
  gtk_widget_show(private->right_frame);

private
  ->right_vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 6);
  gtk_container_add(GTK_CONTAINER(private->right_frame), private->right_vbox);
  gtk_widget_show(private->right_vbox);

  button = check_button_with_icon_new(_("_Visible"), GIMP_ICON_VISIBLE,
                                      GTK_BOX(private->right_vbox));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button), private->visible);
  g_signal_connect(button, "toggled", G_CALLBACK(gimp_toggle_button_update),
                   &private->visible);

  button = check_button_with_icon_new(_("_Linked"), GIMP_ICON_LINKED,
                                      GTK_BOX(private->right_vbox));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button), private->linked);
  g_signal_connect(button, "toggled", G_CALLBACK(gimp_toggle_button_update),
                   &private->linked);

  button = check_button_with_icon_new(
      lock_content_label, lock_content_icon_name, GTK_BOX(private->right_vbox));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button),
                               private->lock_content);
  g_signal_connect(button, "toggled", G_CALLBACK(gimp_toggle_button_update),
                   &private->lock_content);

  button = check_button_with_icon_new(lock_position_label, GIMP_ICON_TOOL_MOVE,
                                      GTK_BOX(private->right_vbox));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button),
...
private
  = g_object_get_data(G_OBJECT(dialog), "item-options-dialog-private");
...
GtkWidget *item_options_dialog_get_grid(GtkWidget *dialog, gint *next_row) {
  ItemOptionsDialog *private;
...
void item_options_dialog_add_widget(GtkWidget *dialog, const gchar *label,
                                    GtkWidget *widget) {
  ItemOptionsDialog *private;
...
private
  ...
  gimp_grid_attach_aligned(GTK_GRID(private->left_grid), 0, private->grid_row++,
...
static void item_options_dialog_response(GtkWidget *dialog, gint response_id,
  ...
  private
    ->callback(dialog, private->image, private->item, private->context, name,
               private->visible, private->linked, private->color_tag,
               private->lock_content, private->lock_position,
               private->user_data);
  } else {
