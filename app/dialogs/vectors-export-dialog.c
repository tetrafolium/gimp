...
static void vectors_export_dialog_response(GtkWidget *widget, gint response_id,
                                           VectorsExportDialog *private);

/*  public function  */

GtkWidget *vectors_export_dialog_new(GimpImage *image, GtkWidget *parent,
                                     GFile *export_folder, gboolean active_only,
                                     GimpVectorsExportCallback callback,
                                     gpointer user_data) {
  VectorsExportDialog *private;
  GtkWidget *dialog;
  ...
  g_return_val_if_fail(export_folder == NULL || G_IS_FILE(export_folder), NULL);
  g_return_val_if_fail(callback != NULL, NULL);

private
  = g_slice_new0(VectorsExportDialog);
...
private
  ...
  dialog = gtk_file_chooser_dialog_new(
      _("Export Path to SVG"), NULL, GTK_FILE_CHOOSER_ACTION_SAVE,
  ...
  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)vectors_export_dialog_free,
                    private);
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(vectors_export_dialog_response), private);
...
static void vectors_export_dialog_response(GtkWidget *dialog, gint response_id,
                                           VectorsExportDialog *private) {
  if (response_id == GTK_RESPONSE_OK) {
    ...
    private
      ->callback(dialog, private->image, file, folder, private->active_only,
                 private->user_data);
