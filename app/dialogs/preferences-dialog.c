...
static GtkWidget *prefs_dialog_new(Gimp *gimp, GimpConfig *config);
static void prefs_config_notify(GObject *config, GParamSpec *param_spec,
                                GObject *config_copy);
static void prefs_config_copy_notify(GObject *config_copy,
                                     GParamSpec *param_spec, GObject *config);
static void prefs_response(GtkWidget *widget, gint response_id,
                           GtkWidget *dialog);

static void prefs_message(GtkMessageType type, gboolean destroy,
                          const gchar *message);

static void prefs_color_management_reset(GtkWidget *widget, GObject *config);
static void prefs_dialog_defaults_reset(GtkWidget *widget, GObject *config);
static void prefs_folders_reset(GtkWidget *widget, GObject *config);
static void prefs_path_reset(GtkWidget *widget, GObject *config);
...
static void prefs_resolution_calibrate_callback(GtkWidget *widget,
                                                GtkWidget *entry);
static void prefs_input_devices_dialog(GtkWidget *widget, Gimp *gimp);
static void prefs_keyboard_shortcuts_dialog(GtkWidget *widget, Gimp *gimp);
static void prefs_menus_save_callback(GtkWidget *widget, Gimp *gimp);
static void prefs_menus_clear_callback(GtkWidget *widget, Gimp *gimp);
static void prefs_menus_remove_callback(GtkWidget *widget, Gimp *gimp);
static void prefs_session_save_callback(GtkWidget *widget, Gimp *gimp);
static void prefs_session_clear_callback(GtkWidget *widget, Gimp *gimp);
static void prefs_devices_save_callback(GtkWidget *widget, Gimp *gimp);
static void prefs_devices_clear_callback(GtkWidget *widget, Gimp *gimp);
static void prefs_search_clear_callback(GtkWidget *widget, Gimp *gimp);
static void prefs_tool_options_save_callback(GtkWidget *widget, Gimp *gimp);
static void prefs_tool_options_clear_callback(GtkWidget *widget, Gimp *gimp);
static void prefs_help_language_change_callback(GtkComboBox *combo, Gimp *gimp);
...
GtkWidget *preferences_dialog_create(Gimp *gimp) {
  ...
  g_signal_connect_object(config, "notify", G_CALLBACK(prefs_config_notify),
                          config_copy, 0);
  g_signal_connect_object(config_copy, "notify",
                          G_CALLBACK(prefs_config_copy_notify), config, 0);
...
static void prefs_config_notify(GObject *config, GParamSpec *param_spec,
  ...
  if (g_param_values_cmp(param_spec, &global_value, &copy_value)) {
    g_signal_handlers_block_by_func(config_copy, prefs_config_copy_notify,
                                    config);

    g_object_set_property(config_copy, param_spec->name, &global_value);

    g_signal_handlers_unblock_by_func(config_copy, prefs_config_copy_notify,
                                      config);
  }
  ...
  if (g_param_values_cmp(param_spec, &copy_value, &global_value)) {
    if (param_spec->flags & GIMP_CONFIG_PARAM_CONFIRM) {
      ...
      g_print("Applying prefs change of '%s' to edit_config\n",
              param_spec->name);
#endif
      g_signal_handlers_block_by_func(config, prefs_config_notify, config_copy);

      g_object_set_property(config, param_spec->name, &copy_value);

      g_signal_handlers_unblock_by_func(config, prefs_config_notify,
                                        config_copy);
    }
  }
...
static void prefs_response(GtkWidget *widget, gint response_id,
  ...
  case RESPONSE_RESET: {
    ...
    confirm = gimp_message_dialog_new(
        _("Reset All Preferences"), GIMP_ICON_DIALOG_QUESTION, dialog,
    ...
    if (gimp_dialog_run(GIMP_DIALOG(confirm)) == GTK_RESPONSE_OK) {
      ...
      if (!gimp_help_browser_is_installed(gimp)) {
        g_object_set(config_copy, "help-browser", GIMP_HELP_BROWSER_WEB_BROWSER,
                     NULL);
      }
    }
  ...
  case GTK_RESPONSE_OK: {
    ...
    for (list = confirm_diff; list; list = g_list_next(list)) {
      ...
      g_object_set_property(G_OBJECT(gimp->edit_config), param_spec->name,
                            &value);
     ...
     */
    restart_diff =
        gimp_config_diff(G_OBJECT(gimp->edit_config), G_OBJECT(gimp->config),
  ...
  default: {
    ...
    for (list = diff; list; list = g_list_next(list)) {
      ...
      g_object_get_property(config_orig, param_spec->name, &value);
      g_object_set_property(G_OBJECT(gimp->edit_config), param_spec->name,
                            &value);
...
static void prefs_dialog_defaults_reset(GtkWidget *widget, GObject *config) {
  ...
  pspecs =
      g_object_class_list_properties(G_OBJECT_GET_CLASS(config), &n_pspecs);
...
static void prefs_import_raw_procedure_callback(GtkWidget *widget,
  ...
  g_object_set(config, "import-raw-plug-in", raw_plug_in, NULL);
...
static void prefs_resolution_source_callback(GtkWidget *widget,
  ...
  if (from_gdk) {
    gimp_get_monitor_resolution(gimp_widget_get_monitor(widget), &xres, &yres);
  } else {
    GimpSizeEntry *entry =
  ...
  g_object_set(config, "monitor-xresolution", xres, "monitor-yresolution", yres,
               "monitor-resolution-from-windowing-system", from_gdk, NULL);
}
...
static void prefs_input_devices_dialog(GtkWidget *widget, Gimp *gimp) {
  gimp_dialog_factory_dialog_raise(gimp_dialog_factory_get_singleton(),
                                   gimp_widget_get_monitor(widget), widget,
...
static void prefs_menus_remove_callback(GtkWidget *widget, Gimp *gimp) {
  ...
  dialog = gimp_message_dialog_new(
      _("Remove all Keyboard Shortcuts"), GIMP_ICON_DIALOG_QUESTION,
      gtk_widget_get_toplevel(widget),
      GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
      gimp_standard_help_func, NULL,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("Cl_ear"), GTK_RESPONSE_OK,
...
static void prefs_help_language_change_callback(GtkComboBox *combo,
  ...
  g_object_set(gimp->config, "help-locales", help_locales ? help_locales : "",
               NULL);
  g_free(code);
  if (help_locales)
    g_free(help_locales);
...
static void prefs_help_language_change_callback2(GtkComboBox *combo,
  ...
  if (icon) {
    gtk_image_set_from_icon_name(icon, icon_name, GTK_ICON_SIZE_BUTTON);
  }

  g_list_free(children);
}

static void prefs_format_string_select_callback(GtkTreeSelection *sel,
                                                GtkEntry *entry) {
  GtkTreeModel *model;
  GtkTreeIter iter;
...
static void prefs_theme_select_callback(GtkTreeSelection *sel, Gimp *gimp) {
  GtkTreeModel *model;
  GtkTreeIter iter;
...
static void prefs_theme_reload_callback(GtkWidget *button, Gimp *gimp) {
  g_object_notify(G_OBJECT(gimp->config), "theme");
}

static void prefs_icon_theme_select_callback(GtkTreeSelection *sel,
                                             Gimp *gimp) {
  GtkTreeModel *model;
  GtkTreeIter iter;
...
static void prefs_canvas_padding_color_changed(GtkWidget *button,
                                               GtkWidget *combo) {
  gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(combo),
                                GIMP_CANVAS_PADDING_MODE_CUSTOM);
}

static void prefs_display_options_frame_add(Gimp *gimp, GObject *object,
                                            const gchar *label,
                                            GtkContainer *parent) {
  GtkWidget *vbox;
  GtkWidget *hbox;
  ...
  prefs_check_button_add(object, "show-selection", _("Show s_election"),
                         GTK_BOX(checks_vbox));
  prefs_check_button_add(object, "show-layer-boundary",
                         _("Show _layer boundary"), GTK_BOX(checks_vbox));
  prefs_check_button_add(object, "show-canvas-boundary",
                         _("Show can_vas boundary"), GTK_BOX(checks_vbox));
  prefs_check_button_add(object, "show-guides", _("Show _guides"),
                         GTK_BOX(checks_vbox));
  prefs_check_button_add(object, "show-grid", _("Show gri_d"),
                         GTK_BOX(checks_vbox));
  prefs_check_button_add(object, "show-sample-points", _("Show _sample points"),
                         GTK_BOX(checks_vbox));
  ...
  prefs_check_button_add(object, "show-menubar", _("Show _menubar"),
                         GTK_BOX(checks_vbox));
#endif /* !GDK_WINDOWING_QUARTZ */
  prefs_check_button_add(object, "show-rulers", _("Show _rulers"),
                         GTK_BOX(checks_vbox));
  prefs_check_button_add(object, "show-scrollbars", _("Show scroll_bars"),
                         GTK_BOX(checks_vbox));
  prefs_check_button_add(object, "show-statusbar", _("Show s_tatusbar"),
                         GTK_BOX(checks_vbox));

  grid = prefs_grid_new(GTK_CONTAINER(vbox));

  combo = prefs_enum_combo_box_add(object, "padding-mode", 0, 0,
                                   _("Canvas _padding mode:"), GTK_GRID(grid),
                                   0, NULL);

  button = prefs_color_button_add(
      object, "padding-color", _("Custom p_adding color:"),
      _("Select Custom Canvas Padding Color"), GTK_GRID(grid), 1, NULL,
      gimp_get_user_context(gimp));

  g_signal_connect(button, "color-changed",
                   G_CALLBACK(prefs_canvas_padding_color_changed), combo);
...
static void prefs_behavior_options_frame_add(Gimp *gimp, GObject *object,
                                             const gchar *label,
                                             GtkContainer *parent) {
  GtkWidget *vbox;
  GtkWidget *hbox;
  ...
  prefs_check_button_add(object, "snap-to-guides", _("Snap to _Guides"),
                         GTK_BOX(checks_vbox));
  prefs_check_button_add(object, "snap-to-grid", _("S_nap to Grid"),
                         GTK_BOX(checks_vbox));
  ...
  prefs_check_button_add(object, "snap-to-canvas", _("Snap to Canvas _Edges"),
                         GTK_BOX(checks_vbox));
  prefs_check_button_add(object, "snap-to-path", _("Snap to _Active Path"),
                         GTK_BOX(checks_vbox));
}

static void prefs_help_func(const gchar *help_id, gpointer help_data) {
  GtkWidget *prefs_box;
...
static void prefs_message(GtkMessageType type, gboolean destroy_with_parent,
                          const gchar *message) {
  GtkWidget *dialog;

  dialog = gtk_message_dialog_new(
      GTK_WINDOW(prefs_dialog),
      destroy_with_parent ? GTK_DIALOG_DESTROY_WITH_PARENT : 0, type,
...
static GtkWidget *prefs_dialog_new(Gimp *gimp, GimpConfig *config) {
  ...
  dialog = gimp_dialog_new(_("Preferences"), "gimp-preferences", NULL, 0,
                           prefs_help_func, GIMP_HELP_PREFS_DIALOG,
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(prefs_response), dialog);
  ...
  /*  System Resources  */
  /**********************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-system-resources",
      _("System Resources"), _("System Resources"),
      GIMP_HELP_PREFS_SYSTEM_RESOURCES, NULL, &top_iter);
  gimp_prefs_box_set_page_scrollable(GIMP_PREFS_BOX(prefs_box), vbox, TRUE);

  size_group = gtk_size_group_new(GTK_SIZE_GROUP_HORIZONTAL);

  vbox2 =
      prefs_frame_new(_("Resource Consumption"), GTK_CONTAINER(vbox), FALSE);

  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_spin_button_add(object, "undo-levels", 1.0, 5.0, 0,
                        _("Minimal number of _undo levels:"), GTK_GRID(grid), 0,
                        size_group);
  prefs_memsize_entry_add(object, "undo-size", _("Maximum undo _memory:"),
                          GTK_GRID(grid), 1, size_group);
  prefs_memsize_entry_add(object, "tile-cache-size", _("Tile cache _size:"),
                          GTK_GRID(grid), 2, size_group);
  prefs_memsize_entry_add(object, "max-new-image-size",
                          _("Maximum _new image size:"), GTK_GRID(grid), 3,
                          size_group);

  prefs_compression_combo_box_add(object, "swap-compression",
                                  _("S_wap compression:"), GTK_GRID(grid), 4,
                                  size_group);

#ifdef ENABLE_MP
  prefs_spin_button_add(object, "num-processors", 1.0, 4.0, 0,
                        _("Number of _threads to use:"), GTK_GRID(grid), 5,
                        size_group);
#endif /* ENABLE_MP */

  /*  Internet access  */
#ifdef CHECK_UPDATE
  vbox2 = prefs_frame_new(_("Network access"), GTK_CONTAINER(vbox), FALSE);

  prefs_switch_add(object, "check-updates",
                   _("Check for updates (requires internet)"), GTK_BOX(vbox2),
                   size_group);
#endif
  ...
  prefs_enum_combo_box_add(object, "thumbnail-size", 0, 0,
                           _("Size of _thumbnails:"), GTK_GRID(grid), 0,
                           size_group);
  ...
  vbox = gimp_prefs_box_add_page(GIMP_PREFS_BOX(prefs_box),
                                 ...
                                 GIMP_HELP_PREFS_DEBUGGING, NULL, &top_iter);

  hbox = g_object_new(GIMP_TYPE_HINT_BOX, "icon-name", GIMP_ICON_DIALOG_WARNING,
                      "hint",
                      _("We hope you will never need these "
  ...
  vbox2 = prefs_frame_new(_("Bug Reporting"), GTK_CONTAINER(vbox), FALSE);

  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  button =
      prefs_enum_combo_box_add(object, "debug-policy", 0, 0,
                               _("Debug _policy:"), GTK_GRID(grid), 0, NULL);
  ...
  if (!gimp_stack_trace_available(TRUE)) {
    ...
    hbox = prefs_hint_box_new(GIMP_ICON_DIALOG_WARNING,
                              _("This feature requires \"gdb\" or \"lldb\" "
                                "installed on your system."));
    gtk_widget_set_sensitive(button, FALSE);
#else
    hbox = prefs_hint_box_new(GIMP_ICON_DIALOG_WARNING,
                              _("This feature is more efficient with \"gdb\" "
                                "or \"lldb\" installed on your system."));
  ...
  /**********************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-color-management",
      _("Color Management"), _("Color Management"),
      GIMP_HELP_PREFS_COLOR_MANAGEMENT, NULL, &top_iter);

  gimp_prefs_box_set_page_scrollable(GIMP_PREFS_BOX(prefs_box), vbox, TRUE);

  button = gimp_prefs_box_set_page_resettable(GIMP_PREFS_BOX(prefs_box), vbox,
                                              _("R_eset Color Management"));
  g_signal_connect(button, "clicked", G_CALLBACK(prefs_color_management_reset),
                   config);

  {
    ...
    gimp_color_profile_store_add_file(GIMP_COLOR_PROFILE_STORE(store), NULL,
                                      NULL);
    ...
    prefs_enum_combo_box_add(color_config, "mode", 0, 0,
                             _("Image display _mode:"), GTK_GRID(grid), row++,
                             NULL);

    /*  Color Managed Display  */
    vbox2 =
        prefs_frame_new(_("Color Managed Display"), GTK_CONTAINER(vbox), FALSE);

    grid = prefs_grid_new(GTK_CONTAINER(vbox2));
    row = 0;

    prefs_profile_combo_box_add(color_config, "display-profile", store,
                                _("Select Monitor Color Profile"),
                                _("_Monitor profile:"), GTK_GRID(grid), row++,
                                size_group, object, "color-profile-path");

    button =
        gimp_prop_check_button_new(color_config, "display-profile-from-gdk",
    ...
    prefs_enum_combo_box_add(color_config, "display-rendering-intent", 0, 0,
                             _("_Rendering intent:"), GTK_GRID(grid), row++,
                             size_group);
    ...
    prefs_boolean_combo_box_add(color_config, "display-optimize", _("Speed"),
                                _("Precision / Color Fidelity"),
                                _("_Optimize image display for:"),
                                GTK_GRID(grid), row++, size_group);

    /*  Print Simulation (Soft-proofing)  */
    vbox2 = prefs_frame_new(_("Soft-Proofing"), GTK_CONTAINER(vbox), FALSE);

    grid = prefs_grid_new(GTK_CONTAINER(vbox2));
    row = 0;

    prefs_profile_combo_box_add(
        color_config, "simulation-profile", store,
        _("Select Soft-Proofing Color Profile"), _("_Soft-proofing profile:"),
        GTK_GRID(grid), row++, size_group, object, "color-profile-path");

    prefs_enum_combo_box_add(color_config, "simulation-rendering-intent", 0, 0,
                             _("Re_ndering intent:"), GTK_GRID(grid), row++,
                             size_group);

    button = gimp_prop_check_button_new(
        color_config, "simulation-use-black-point-compensation",
        _("Use black _point compensation"));
    gtk_grid_attach(GTK_GRID(grid), button, 1, row, 1, 1);
    row++;

    prefs_boolean_combo_box_add(color_config, "simulation-optimize", _("Speed"),
                                _("Precision / Color Fidelity"),
    ...
    button = gimp_prop_color_button_new(
        color_config, "out-of-gamut-color", _("Select Warning Color"),
        PREFS_COLOR_BUTTON_WIDTH, PREFS_COLOR_BUTTON_HEIGHT,
    ...
    /*  Preferred profiles  */
    vbox2 =
        prefs_frame_new(_("Preferred Profiles"), GTK_CONTAINER(vbox), FALSE);

    grid = prefs_grid_new(GTK_CONTAINER(vbox2));
    row = 0;

    prefs_profile_combo_box_add(color_config, "rgb-profile", store,
                                _("Select Preferred RGB Color Profile"),
                                _("_RGB profile:"), GTK_GRID(grid), row++,
                                size_group, object, "color-profile-path");

    prefs_profile_combo_box_add(color_config, "gray-profile", store,
                                _("Select Preferred Grayscale Color Profile"),
                                _("_Grayscale profile:"), GTK_GRID(grid), row++,
                                size_group, object, "color-profile-path");

    prefs_profile_combo_box_add(color_config, "cmyk-profile", store,
                                _("Select CMYK Color Profile"),
                                _("_CMYK profile:"), GTK_GRID(grid), row++,
                                size_group, object, "color-profile-path");

    /*  Policies  */
    vbox2 = prefs_frame_new(_("Policies"), GTK_CONTAINER(vbox), FALSE);
    grid = prefs_grid_new(GTK_CONTAINER(vbox2));
    ...
    /*  Filter Dialogs  */
    vbox2 = prefs_frame_new(_("Filter Dialogs"), GTK_CONTAINER(vbox), FALSE);
  ...
  /***************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-import-export",
      _("Image Import & Export"), _("Image Import & Export"),
      GIMP_HELP_PREFS_IMPORT_EXPORT, NULL, &top_iter);
  ...
  /*  Import Policies  */
  vbox2 = prefs_frame_new(_("Import Policies"), GTK_CONTAINER(vbox), FALSE);
  ...
  g_object_bind_property(button, "active", vbox3, "sensitive",
  ...
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));
  button = prefs_enum_combo_box_add(object, "color-profile-policy", 0, 0,
                                    _("Color _profile policy:"), GTK_GRID(grid),
                                    0, size_group);
  ...
  /*  Export Policies  */
  vbox2 = prefs_frame_new(_("Export Policies"), GTK_CONTAINER(vbox), FALSE);

  button = prefs_check_button_add(
      object, "export-color-profile",
      _("Export the i_mage's color profile by default"), GTK_BOX(vbox2));
  button = prefs_check_button_add(object, "export-comment",
                                  _("Export the image's comment by default"),
                                  GTK_BOX(vbox2));
  button = prefs_check_button_add(
      object, "export-metadata-exif",
      /* Translators: label for
       * configuration option (checkbox).
       * It determines how file export
       * plug-ins handle Exif by default.
       */
      _("Export _Exif metadata by default when available"), GTK_BOX(vbox2));
  button = prefs_check_button_add(
      object, "export-metadata-xmp",
      /* Translators: label for
       * configuration option (checkbox).
       * It determines how file export
       * plug-ins handle XMP by default.
       */
      _("Export _XMP metadata by default when available"), GTK_BOX(vbox2));
  button = prefs_check_button_add(
      object, "export-metadata-iptc",
      /* Translators: label for
       * configuration option (checkbox).
       * It determines how file export
       * plug-ins handle IPTC by default.
       */
      _("Export _IPTC metadata by default when available"), GTK_BOX(vbox2));
  hbox = prefs_hint_box_new(GIMP_ICON_DIALOG_WARNING,
  ...
  prefs_enum_combo_box_add(object, "export-file-type", 0, 0,
                           _("Default export file t_ype:"), GTK_GRID(grid), 0,
                           size_group);

  /*  Raw Image Importer  */
  vbox2 = prefs_frame_new(_("Raw Image Importer"), GTK_CONTAINER(vbox), TRUE);

  {
    ...
    g_signal_connect(view, "changed",
                     G_CALLBACK(prefs_import_raw_procedure_callback), config);
  }
  ...
  if (gimp->show_playground) {
    vbox = gimp_prefs_box_add_page(
        GIMP_PREFS_BOX(prefs_box), "gimp-prefs-playground",
        _("Experimental Playground"), _("Playground"),
        GIMP_HELP_PREFS_PLAYGROUND, NULL, &top_iter);

    hbox = g_object_new(GIMP_TYPE_HINT_BOX, "icon-name",
                        GIMP_ICON_DIALOG_WARNING, "hint",
                        _("These features are unfinished, buggy "
    ...
    vbox2 =
        prefs_frame_new(_("Hardware Acceleration"), GTK_CONTAINER(vbox), FALSE);
    ...
    prefs_switch_add(object, "use-opencl", _("Use O_penCL"), GTK_BOX(vbox2),
                     NULL);

    /*  Very unstable tools  */
    vbox2 = prefs_frame_new(_("Insane Options"), GTK_CONTAINER(vbox), FALSE);

    button =
        prefs_check_button_add(object, "playground-npd-tool",
                               _("_N-Point Deformation tool"), GTK_BOX(vbox2));
    button = prefs_check_button_add(object, "playground-seamless-clone-tool",
                                    _("_Seamless Clone tool"), GTK_BOX(vbox2));
    button = prefs_check_button_add(object, "playground-paint-select-tool",
                                    _("_Paint Select tool"), GTK_BOX(vbox2));
    if (!gegl_has_operation("gegl:paint-select")) {
      ...
      gimp_help_set_help_data(
          button, "Missing GEGL operation 'gegl:paint-select'.", NULL);
    }
  }
  ...
  /******************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-tool-options",
      C_("preferences", "Tool Options"), C_("preferences", "Tool Options"),
      GIMP_HELP_PREFS_TOOL_OPTIONS, NULL, &top_iter);
  gimp_prefs_box_set_page_scrollable(GIMP_PREFS_BOX(prefs_box), vbox, TRUE);
  ...
  prefs_check_button_add(object, "save-tool-options",
                         _("_Save tool options on exit"), GTK_BOX(vbox2));

  button = prefs_button_add(GIMP_ICON_DOCUMENT_SAVE,
                            _("Save Tool Options _Now"), GTK_BOX(vbox2));
  g_signal_connect(button, "clicked",
                   G_CALLBACK(prefs_tool_options_save_callback), gimp);
  ...
  g_signal_connect(button2, "clicked",
                   G_CALLBACK(prefs_tool_options_clear_callback), gimp);
  ...
  prefs_enum_combo_box_add(object, "interpolation-type", 0, 0,
                           _("Default _interpolation:"), GTK_GRID(grid), 0,
                           size_group);
  ...
  prefs_check_button_add_with_icon(object, "global-brush", _("_Brush"),
                                   GIMP_ICON_BRUSH, GTK_BOX(vbox2), size_group);
  prefs_check_button_add_with_icon(object, "global-dynamics", _("_Dynamics"),
                                   GIMP_ICON_DYNAMICS, GTK_BOX(vbox2),
                                   size_group);
  prefs_check_button_add_with_icon(object, "global-pattern", _("_Pattern"),
                                   GIMP_ICON_PATTERN, GTK_BOX(vbox2),
                                   size_group);
  prefs_check_button_add_with_icon(object, "global-gradient", _("_Gradient"),
                                   GIMP_ICON_GRADIENT, GTK_BOX(vbox2),
                                   size_group);

  /*  Move Tool */
  vbox2 = prefs_frame_new(_("Move Tool"), GTK_CONTAINER(vbox), FALSE);

  prefs_check_button_add_with_icon(
      object, "move-tool-changes-active", _("Set _layer or path as active"),
      GIMP_ICON_TOOL_MOVE, GTK_BOX(vbox2), size_group);
  ...
  /*******************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-new-image", _("Default New Image"),
      _("Default Image"), GIMP_HELP_PREFS_NEW_IMAGE, NULL, &top_iter);
...

  {
    ...
    combo = gimp_container_combo_box_new(gimp->templates,
                                         gimp_get_user_context(gimp), 16, 0);
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Template:"), 0.0, 0.5,
                             combo, 1);
  ...
  prefs_color_button_add(object, "quick-mask-color", _("Quick Mask color:"),
                         _("Set the default Quick Mask color"), GTK_GRID(grid),
                         0, NULL, gimp_get_user_context(gimp));

  /**********************************/
  /*  Default Image / Default Grid  */
  /**********************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-default-grid",
      _("Default Image Grid"), _("Default Grid"), GIMP_HELP_PREFS_DEFAULT_GRID,
      &top_iter, &child_iter);
  gimp_prefs_box_set_page_scrollable(GIMP_PREFS_BOX(prefs_box), vbox, TRUE);

  /*  Grid  */
  editor = gimp_grid_editor_new(
      core_config->default_grid, gimp_get_user_context(gimp),
  ...
  /***************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-interface", _("User Interface"),
      _("Interface"), GIMP_HELP_PREFS_INTERFACE, NULL, &top_iter);
  gimp_prefs_box_set_page_scrollable(GIMP_PREFS_BOX(prefs_box), vbox, TRUE);
  ...
  g_object_bind_property(button, "active", vbox3, "sensitive",
                         G_BINDING_SYNC_CREATE);
  button =
      prefs_check_button_add(object, "group-layer-previews",
                             _("Enable layer _group previews"), GTK_BOX(vbox3));
  ...
  prefs_enum_combo_box_add(object, "undo-preview-size", 0, 0,
                           _("_Undo preview size:"), GTK_GRID(grid), 1, NULL);
  prefs_enum_combo_box_add(object, "navigation-preview-size", 0, 0,
                           _("Na_vigation preview size:"), GTK_GRID(grid), 2,
                           NULL);
  ...
  prefs_check_button_add(object, "can-change-accels",
                         _("_Use dynamic keyboard shortcuts"), GTK_BOX(vbox2));

  button =
      prefs_button_add(GIMP_ICON_PREFERENCES_SYSTEM,
                       _("Configure _Keyboard Shortcuts..."), GTK_BOX(vbox2));
  g_signal_connect(button, "clicked",
                   G_CALLBACK(prefs_keyboard_shortcuts_dialog), gimp);

  prefs_check_button_add(object, "save-accels",
                         _("_Save keyboard shortcuts on exit"), GTK_BOX(vbox2));

  button = prefs_button_add(GIMP_ICON_DOCUMENT_SAVE,
                            _("Save Keyboard Shortcuts _Now"), GTK_BOX(vbox2));
  g_signal_connect(button, "clicked", G_CALLBACK(prefs_menus_save_callback),
                   gimp);

  button2 = prefs_button_add(GIMP_ICON_RESET,
                             _("_Reset Keyboard Shortcuts to Default Values"),
                             GTK_BOX(vbox2));
  g_signal_connect(button2, "clicked", G_CALLBACK(prefs_menus_clear_callback),
                   gimp);

  g_object_set_data(G_OBJECT(button), "clear-button", button2);

  button =
      prefs_button_add(GIMP_ICON_EDIT_CLEAR,
                       _("Remove _All Keyboard Shortcuts"), GTK_BOX(vbox2));
  g_signal_connect(button, "clicked", G_CALLBACK(prefs_menus_remove_callback),
                   gimp);

  /***********************/
  /*  Interface / Theme  */
  /***********************/
  vbox = gimp_prefs_box_add_page(GIMP_PREFS_BOX(prefs_box), "gimp-prefs-theme",
                                 _("Theme"), _("Theme"), GIMP_HELP_PREFS_THEME,
                                 &top_iter, &child_iter);
...

  {
    ...
    gtk_tree_view_insert_column_with_attributes(
        GTK_TREE_VIEW(view), 0, _("Theme"), gtk_cell_renderer_text_new(),
        "text", 0, NULL);
    gtk_tree_view_insert_column_with_attributes(
        GTK_TREE_VIEW(view), 1, _("Folder"), gtk_cell_renderer_text_new(),
        "text", 1, NULL);
    ...
    for (i = 0; i < n_themes; i++) {
      ...
      gtk_list_store_set(list_store, &iter, 0, themes[i], 1,
                         gimp_file_get_utf8_name(theme_dir), -1);
      ...
      if (GIMP_GUI_CONFIG(object)->theme &&
        ...
        gtk_tree_view_scroll_to_cell(GTK_TREE_VIEW(view), path, NULL, FALSE,
                                     0.0, 0.0);
    ...
    g_signal_connect(sel, "changed", G_CALLBACK(prefs_theme_select_callback),
                     gimp);
  }
  ...
  button = prefs_button_add(GIMP_ICON_VIEW_REFRESH, _("Reload C_urrent Theme"),
                            GTK_BOX(hbox));
  g_signal_connect(button, "clicked", G_CALLBACK(prefs_theme_reload_callback),
                   gimp);
  ...
  /****************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-icon-theme", _("Icon Theme"),
      _("Icon Theme"), GIMP_HELP_PREFS_ICON_THEME, &top_iter, &child_iter);
...

  {
    ...
    list_store = gtk_list_store_new(3, CAIRO_GOBJECT_TYPE_SURFACE,
    ...
    gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(view), 0, NULL,
                                                gtk_cell_renderer_pixbuf_new(),
                                                "surface", 0, NULL);
    gtk_tree_view_insert_column_with_attributes(
        GTK_TREE_VIEW(view), 1, _("Icon Theme"), gtk_cell_renderer_text_new(),
        "text", 1, NULL);
    gtk_tree_view_insert_column_with_attributes(
        GTK_TREE_VIEW(view), 2, _("Folder"), gtk_cell_renderer_text_new(),
        "text", 2, NULL);
    ...
    for (i = 0; i < n_icon_themes; i++) {
      ...
      gtk_icon_theme_prepend_search_path(
          theme, gimp_file_get_utf8_name(icon_theme_search_path));
      ...
      surface =
          gtk_icon_theme_load_surface(theme, example, 24, scale_factor, NULL,
                                      GTK_ICON_LOOKUP_GENERIC_FALLBACK, NULL);
      gtk_list_store_append(list_store, &iter);
      gtk_list_store_set(list_store, &iter, 0, surface, 1, icon_themes[i], 2,
                         gimp_file_get_utf8_name(icon_theme_dir), -1);
    ...
    g_signal_connect(sel, "changed",
                     G_CALLBACK(prefs_icon_theme_select_callback), gimp);
  ...
  /*************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-toolbox", _("Toolbox"),
      _("Toolbox"), GIMP_HELP_PREFS_TOOLBOX, &top_iter, &child_iter);
  ...
  prefs_check_button_add_with_icon(
      object, "toolbox-wilber", _("Show GIMP _logo (drag-and-drop target)"),
      GIMP_ICON_WILBER, GTK_BOX(vbox2), size_group);
  prefs_check_button_add_with_icon(
      object, "toolbox-color-area", _("Show _foreground & background color"),
      GIMP_ICON_COLORS_DEFAULT, GTK_BOX(vbox2), size_group);
  ...
  prefs_check_button_add_with_icon(object, "toolbox-image-area",
                                   _("Show active _image"), GIMP_ICON_IMAGE,
                                   GTK_BOX(vbox2), size_group);
  ...
  prefs_check_button_add_with_icon(object, "toolbox-groups",
                                   _("Use tool _groups"), NULL, GTK_BOX(vbox2),
                                   size_group);

  g_clear_object(&size_group);

  /* Tool Editor */
  vbox2 = prefs_frame_new(_("Tools Configuration"), GTK_CONTAINER(vbox), TRUE);
  ...
  /*********************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box),
      /* FIXME need an icon */
      "gimp-prefs-controllers", _("Dialog Defaults"), _("Dialog Defaults"),
      GIMP_HELP_PREFS_DIALOG_DEFAULTS, &top_iter, &child_iter);

  gimp_prefs_box_set_page_scrollable(GIMP_PREFS_BOX(prefs_box), vbox, TRUE);

  button = gimp_prefs_box_set_page_resettable(GIMP_PREFS_BOX(prefs_box), vbox,
                                              _("Reset Dialog _Defaults"));
  g_signal_connect(button, "clicked", G_CALLBACK(prefs_dialog_defaults_reset),
                   config);
  ...
  button = prefs_enum_combo_box_add(object, "color-profile-policy", 0, 0,
                                    _("Color profile policy:"), GTK_GRID(grid),
                                    0, size_group);
  ...
  prefs_enum_combo_box_add(object, "image-convert-profile-intent", 0, 0,
                           _("Rendering intent:"), GTK_GRID(grid), 0,
                           size_group);

  prefs_check_button_add(object,
                         "image-convert-profile-black-point-compensation",
                         _("Black point compensation"), GTK_BOX(vbox2));

  /*  Convert Precision Dialog  */
  vbox2 = prefs_frame_new(_("Precision Conversion Dialog"), GTK_CONTAINER(vbox),
                          FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_enum_combo_box_add(object,
                           "image-convert-precision-layer-dither-method", 0, 0,
                           _("Dither layers:"), GTK_GRID(grid), 0, size_group);
  prefs_enum_combo_box_add(
      object, "image-convert-precision-text-layer-dither-method", 0, 0,
      _("Dither text layers:"), GTK_GRID(grid), 1, size_group);
  prefs_enum_combo_box_add(
      object, "image-convert-precision-channel-dither-method", 0, 0,
      _("Dither channels/masks:"), GTK_GRID(grid), 2, size_group);

  /*  Convert Indexed Dialog  */
  vbox2 = prefs_frame_new(_("Indexed Conversion Dialog"), GTK_CONTAINER(vbox),
                          FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_enum_combo_box_add(object, "image-convert-indexed-palette-type", 0, 0,
                           _("Colormap:"), GTK_GRID(grid), 0, size_group);
  prefs_spin_button_add(object, "image-convert-indexed-max-colors", 1.0, 8.0, 0,
                        _("Maximum number of colors:"), GTK_GRID(grid), 1,
                        size_group);
  ...
  prefs_spin_button_add(object, "filter-tool-max-recent", 1.0, 8.0, 0,
                        _("Keep recent settings:"), GTK_GRID(grid), 1,
                        size_group);
  ...
  button = prefs_check_button_add(object, "filter-tool-use-last-settings",
                                  ...
                                  GTK_BOX(vbox2));
  button =
      prefs_check_button_add(object, "filter-tool-show-color-options",
                             _("Show advanced color options"), GTK_BOX(vbox2));

  /*  Canvas Size Dialog  */
  vbox2 = prefs_frame_new(_("Canvas Size Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_enum_combo_box_add(object, "image-resize-fill-type", 0, 0,
                           _("Fill with:"), GTK_GRID(grid), 0, size_group);
  prefs_enum_combo_box_add(object, "image-resize-layer-set", 0, 0,
                           _("Resize layers:"), GTK_GRID(grid), 1, size_group);

  prefs_check_button_add(object, "image-resize-resize-text-layers",
                         _("Resize text layers"), GTK_BOX(vbox2));

  /*  New Layer Dialog  */
  vbox2 = prefs_frame_new(_("New Layer Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_entry_add(object, "layer-new-name", _("Layer name:"), GTK_GRID(grid), 0,
                  size_group);

  prefs_enum_combo_box_add(object, "layer-new-fill-type", 0, 0, _("Fill type:"),
                           GTK_GRID(grid), 1, size_group);

  /*  Layer Boundary Size Dialog  */
  vbox2 = prefs_frame_new(_("Layer Boundary Size Dialog"), GTK_CONTAINER(vbox),
                          FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));
  ...
  vbox2 =
      prefs_frame_new(_("Add Layer Mask Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_enum_combo_box_add(object, "layer-add-mask-type", 0, 0,
                           _("Layer mask type:"), GTK_GRID(grid), 0,
                           size_group);

  prefs_check_button_add(object, "layer-add-mask-invert", _("Invert mask"),
                         GTK_BOX(vbox2));

  /*  Merge Layers Dialog  */
  vbox2 = prefs_frame_new(_("Merge Layers Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_enum_combo_box_add(object, "layer-merge-type", GIMP_EXPAND_AS_NECESSARY,
                           GIMP_CLIP_TO_BOTTOM_LAYER, _("Merged layer size:"),
                           GTK_GRID(grid), 0, size_group);

  prefs_check_button_add(object, "layer-merge-active-group-only",
                         _("Merge within active groups only"), GTK_BOX(vbox2));
  prefs_check_button_add(object, "layer-merge-discard-invisible",
                         _("Discard invisible layers"), GTK_BOX(vbox2));

  /*  New Channel Dialog  */
  vbox2 = prefs_frame_new(_("New Channel Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_entry_add(object, "channel-new-name", _("Channel name:"),
                  GTK_GRID(grid), 0, size_group);

  prefs_color_button_add(object, "channel-new-color", _("Color and opacity:"),
                         _("Default New Channel Color and Opacity"),
                         GTK_GRID(grid), 1, size_group,
                         gimp_get_user_context(gimp));

  /*  New Path Dialog  */
  vbox2 = prefs_frame_new(_("New Path Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_entry_add(object, "path-new-name", _("Path name:"), GTK_GRID(grid), 0,
                  size_group);

  /*  Export Path Dialog  */
  vbox2 = prefs_frame_new(_("Export Paths Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_file_chooser_button_add(object, "path-export-path", _("Export folder:"),
                                _("Select Default Folder for Exporting Paths"),
                                GTK_GRID(grid), 0, size_group);

  prefs_check_button_add(object, "path-export-active-only",
                         _("Export the active path only"), GTK_BOX(vbox2));

  /*  Import Path Dialog  */
  vbox2 = prefs_frame_new(_("Import Paths Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_file_chooser_button_add(object, "path-import-path", _("Import folder:"),
                                _("Select Default Folder for Importing Paths"),
                                GTK_GRID(grid), 0, size_group);

  prefs_check_button_add(object, "path-import-merge", _("Merge imported paths"),
                         GTK_BOX(vbox2));
  prefs_check_button_add(object, "path-import-scale", _("Scale imported paths"),
                         GTK_BOX(vbox2));

  /*  Feather Selection Dialog  */
  vbox2 = prefs_frame_new(_("Feather Selection Dialog"), GTK_CONTAINER(vbox),
                          FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_spin_button_add(object, "selection-feather-radius", 1.0, 10.0, 2,
                        _("Feather radius:"), GTK_GRID(grid), 0, size_group);
  ...
  vbox2 =
      prefs_frame_new(_("Grow Selection Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_spin_button_add(object, "selection-grow-radius", 1.0, 10.0, 0,
                        _("Grow radius:"), GTK_GRID(grid), 0, size_group);

  /*  Shrink Selection Dialog  */
  vbox2 =
      prefs_frame_new(_("Shrink Selection Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_spin_button_add(object, "selection-shrink-radius", 1.0, 10.0, 0,
                        _("Shrink radius:"), GTK_GRID(grid), 0, size_group);
  ...
  vbox2 =
      prefs_frame_new(_("Border Selection Dialog"), GTK_CONTAINER(vbox), FALSE);
  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_spin_button_add(object, "selection-border-radius", 1.0, 10.0, 0,
                        _("Border radius:"), GTK_GRID(grid), 0, size_group);

  prefs_enum_combo_box_add(object, "selection-border-style", 0, 0,
                           _("Border style:"), GTK_GRID(grid), 1, size_group);
  ...
  editor =
      gimp_fill_editor_new(GIMP_DIALOG_CONFIG(object)->fill_options, FALSE);
   ...
   */
  editor = gimp_stroke_editor_new(
      GIMP_DIALOG_CONFIG(object)->stroke_options,
      gimp_template_get_resolution_y(core_config->default_image), FALSE);
  gtk_box_pack_start(GTK_BOX(vbox2), editor, FALSE, FALSE, 0);
  gtk_widget_show(editor);
  ...
  /*****************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-help-system", _("Help System"),
      _("Help System"), GIMP_HELP_PREFS_HELP, &top_iter, &child_iter);
  ...
  prefs_check_button_add(object, "show-help-button", _("Show help _buttons"),
                         GTK_BOX(vbox2));

  grid = prefs_grid_new(GTK_CONTAINER(vbox2));
  button = prefs_boolean_combo_box_add(
      object, "user-manual-online", _("Use the online version"),
      _("Use a locally installed copy"), _("U_ser manual:"), GTK_GRID(grid), 0,
      size_group);
  ...
  if (manuals != NULL) {
    ...
    entry = gimp_language_combo_box_new(TRUE, _("User interface language"));
    ...
    g_signal_connect(entry, "changed",
                     G_CALLBACK(prefs_help_language_change_callback), gimp);
  ...
  if (manuals) {
    ...
    g_signal_connect(entry, "changed",
                     G_CALLBACK(prefs_help_language_change_callback2), hbox);
  ...
  if (gimp_help_browser_is_installed(gimp)) {
    ...
    g_object_set(config, "help-browser", GIMP_HELP_BROWSER_WEB_BROWSER, NULL);
  }
#else
  g_object_set(config, "help-browser", GIMP_HELP_BROWSER_WEB_BROWSER, NULL);
#endif /* HAVE_WEBKIT */
  ...
  prefs_spin_button_add(object, "action-history-size", 1.0, 10.0, 0,
                        _("_Maximum History Size:"), GTK_GRID(grid), 0,
                        size_group);

  button = prefs_button_add(GIMP_ICON_SHRED, _("C_lear Action History"),
                            GTK_BOX(vbox2));
  g_signal_connect(button, "clicked", G_CALLBACK(prefs_search_clear_callback),
                   gimp);
  ...
  /*************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-display", _("Display"),
      _("Display"), GIMP_HELP_PREFS_DISPLAY, &top_iter, &child_iter);
  gimp_prefs_box_set_page_scrollable(GIMP_PREFS_BOX(prefs_box), vbox, TRUE);
  ...
  prefs_enum_combo_box_add(object, "transparency-type", 0, 0,
                           _("_Check style:"), GTK_GRID(grid), 0, size_group);
  prefs_enum_combo_box_add(object, "transparency-size", 0, 0, _("Check _size:"),
                           GTK_GRID(grid), 1, size_group);
  ...
  prefs_enum_combo_box_add(object, "zoom-quality", 0, 0, _("_Zoom quality:"),
                           GTK_GRID(grid), 0, size_group);

  /*  Monitor Resolution  */
  vbox2 = prefs_frame_new(_("Monitor Resolution"), GTK_CONTAINER(vbox), FALSE);

  {
    gchar *pixels_per_unit = g_strconcat(_("Pixels"), "/%s", NULL);

    entry = gimp_prop_coordinates_new(
        object, "monitor-xresolution", "monitor-yresolution", NULL,
        pixels_per_unit, GIMP_SIZE_ENTRY_UPDATE_RESOLUTION, 0.0, 0.0, TRUE);
  ...
  gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(entry), _("Horizontal"), 0, 1,
                               0.0);
  gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(entry), _("Vertical"), 0, 2,
                               0.0);
  gimp_size_entry_attach_label(GIMP_SIZE_ENTRY(entry), _("ppi"), 1, 4, 0.0);
...

  {
    ...
    gimp_get_monitor_resolution(
        gdk_display_get_monitor(gdk_display_get_default(), 0), &xres, &yres);
  ...
  g_signal_connect(button, "toggled",
                   G_CALLBACK(prefs_resolution_source_callback), config);
  ...
  g_object_set(label, "margin-start", 4, "margin-end", 4, NULL);
  ...
  g_object_bind_property(button, "active", entry, "sensitive",
                         G_BINDING_SYNC_CREATE);
  g_object_bind_property(button, "active", calibrate_button, "sensitive",
                         G_BINDING_SYNC_CREATE);

  g_signal_connect(calibrate_button, "clicked",
                   G_CALLBACK(prefs_resolution_calibrate_callback), entry);
  ...
  /***********************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-window-management",
      _("Window Management"), _("Window Management"),
      GIMP_HELP_PREFS_WINDOW_MANAGEMENT, &top_iter, &child_iter);

  vbox2 =
      prefs_frame_new(_("Window Manager Hints"), GTK_CONTAINER(vbox), FALSE);

  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_enum_combo_box_add(object, "dock-window-hint", 0, 0,
                           _("Hint for _docks and toolbox:"), GTK_GRID(grid), 1,
                           NULL);

  vbox2 = prefs_frame_new(_("Focus"), GTK_CONTAINER(vbox), FALSE);

  prefs_check_button_add(object, "activate-on-focus",
                         _("Activate the _focused image"), GTK_BOX(vbox2));

  /* Window Positions */
  vbox2 = prefs_frame_new(_("Window Positions"), GTK_CONTAINER(vbox), FALSE);

  prefs_check_button_add(object, "save-session-info",
                         _("_Save window positions on exit"), GTK_BOX(vbox2));
  prefs_check_button_add(
      object, "restore-monitor",
      _("Open windows on the same _monitor they were open before"),
      GTK_BOX(vbox2));

  button = prefs_button_add(GIMP_ICON_DOCUMENT_SAVE,
                            _("Save Window Positions _Now"), GTK_BOX(vbox2));
  g_signal_connect(button, "clicked", G_CALLBACK(prefs_session_save_callback),
                   gimp);
  ...
  button2 = prefs_button_add(GIMP_ICON_RESET,
                             _("_Reset Saved Window Positions to "
                               "Default Values"),
                             GTK_BOX(vbox2));
  g_signal_connect(button2, "clicked", G_CALLBACK(prefs_session_clear_callback),
                   gimp);
  ...
  /*******************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-image-windows", _("Image Windows"),
      _("Image Windows"), GIMP_HELP_PREFS_IMAGE_WINDOW, NULL, &top_iter);
  gimp_prefs_box_set_page_scrollable(GIMP_PREFS_BOX(prefs_box), vbox, TRUE);
  ...
  prefs_check_button_add(object, "default-show-all",
                         _("Use \"Show _all\" by default"), GTK_BOX(vbox2));

  prefs_check_button_add(object, "default-dot-for-dot",
                         _("Use \"_Dot for dot\" by default"), GTK_BOX(vbox2));

  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_spin_button_add(object, "marching-ants-speed", 1.0, 10.0, 0,
                        _("Marching ants s_peed:"), GTK_GRID(grid), 0,
                        size_group);

  /*  Zoom & Resize Behavior  */
  vbox2 =
      prefs_frame_new(_("Zoom & Resize Behavior"), GTK_CONTAINER(vbox), FALSE);

  prefs_check_button_add(object, "resize-windows-on-zoom",
                         _("Resize window on _zoom"), GTK_BOX(vbox2));
  ...
  prefs_boolean_combo_box_add(
      object, "initial-zoom-to-fit", _("Show entire image"), "1:1",
      _("Initial zoom _ratio:"), GTK_GRID(grid), 0, size_group);

  /*  Space Bar  */
  vbox2 = prefs_frame_new(_("Space Bar"), GTK_CONTAINER(vbox), FALSE);

  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_enum_combo_box_add(object, "space-bar-action", 0, 0,
                           _("_While space bar is pressed:"), GTK_GRID(grid), 0,
                           size_group);

  /*  Mouse Pointers  */
  vbox2 = prefs_frame_new(_("Mouse Pointers"), GTK_CONTAINER(vbox), FALSE);

  button = prefs_check_button_add(object, "show-brush-outline",
                                  _("Show _brush outline"), GTK_BOX(vbox2));

  vbox3 = prefs_frame_new(NULL, GTK_CONTAINER(vbox2), FALSE);
  g_object_bind_property(button, "active", vbox3, "sensitive",
                         G_BINDING_SYNC_CREATE);
  prefs_check_button_add(object, "snap-brush-outline",
                         _("S_nap brush outline to stroke"), GTK_BOX(vbox3));

  prefs_check_button_add(object, "show-paint-tool-cursor",
                         _("Show pointer for paint _tools"), GTK_BOX(vbox2));

  grid = prefs_grid_new(GTK_CONTAINER(vbox2));

  prefs_enum_combo_box_add(object, "cursor-mode", 0, 0, _("Pointer _mode:"),
                           GTK_GRID(grid), 0, size_group);
  prefs_enum_combo_box_add(object, "cursor-handedness", 0, 0,
                           _("Pointer _handedness:"), GTK_GRID(grid), 1, NULL);
  ...
  /********************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-image-windows-appearance",
      _("Image Window Appearance"), _("Appearance"),
      GIMP_HELP_PREFS_IMAGE_WINDOW_APPEARANCE, &top_iter, &child_iter);

  gimp_prefs_box_set_page_scrollable(GIMP_PREFS_BOX(prefs_box), vbox, TRUE);

  prefs_display_options_frame_add(gimp, G_OBJECT(display_config->default_view),
                                  _("Default Appearance in Normal Mode"),
                                  GTK_CONTAINER(vbox));

  prefs_display_options_frame_add(
      gimp, G_OBJECT(display_config->default_fullscreen_view),
      _("Default Appearance in Fullscreen Mode"), GTK_CONTAINER(vbox));

  /****************************************************/
  /*  Image Windows / Image Title & Statusbar Format  */
  /****************************************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-image-title",
      _("Image Title & Statusbar Format"), _("Title & Status"),
      GIMP_HELP_PREFS_IMAGE_WINDOW_TITLE, &top_iter, &child_iter);

  {
    const gchar *format_strings[] = {NULL,
                                     NULL,
                                     "%f-%p.%i (%t) %z%%",
                                     "%f-%p.%i (%t) %d:%s",
                                     "%f-%p.%i (%t) %wx%h",
    ...
    const gchar *format_names[] = {
        N_("Current format"),       N_("Default format"),
        N_("Show zoom percentage"), N_("Show zoom ratio"),
        N_("Show image size"),      N_("Show drawable size")};
    ...
    } formats[] = {{NULL, GIMP_CONFIG_DEFAULT_IMAGE_TITLE_FORMAT,
    ...
    for (format = 0; format < G_N_ELEMENTS(formats); format++) {
      ...
      for (i = 0; i < G_N_ELEMENTS(format_strings); i++) {
        ...
        gtk_list_store_append(list_store, &iter);
        gtk_list_store_set(list_store, &iter, 0, gettext(format_names[i]), 1,
                           format_strings[i], -1);
      ...
      g_signal_connect(sel, "changed",
                       G_CALLBACK(prefs_format_string_select_callback), entry);
    }
  }
  ...
  /******************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-image-windows-snapping",
      _("Image Window Snapping Behavior"), _("Snapping"),
      GIMP_HELP_PREFS_IMAGE_WINDOW_SNAPPING, &top_iter, &child_iter);

  prefs_behavior_options_frame_add(gimp, G_OBJECT(display_config->default_view),
                                   _("Default Behavior in Normal Mode"),
                                   GTK_CONTAINER(vbox));
  prefs_behavior_options_frame_add(
      gimp, G_OBJECT(display_config->default_fullscreen_view),
      _("Default Behavior in Fullscreen Mode"), GTK_CONTAINER(vbox));
  ...
  prefs_spin_button_add(object, "snap-distance", 1.0, 5.0, 0,
                        _("_Snapping distance:"), GTK_GRID(grid), 0, NULL);

  /*******************/
  /*  Input Devices  */
  /*******************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-input-devices", _("Input Devices"),
      _("Input Devices"), GIMP_HELP_PREFS_INPUT_DEVICES, NULL, &top_iter);

  /*  Extended Input Devices  */
  vbox2 =
      prefs_frame_new(_("Extended Input Devices"), GTK_CONTAINER(vbox), FALSE);

  prefs_check_button_add(
      object, "devices-share-tool",
      _("S_hare tool and tool options between input devices"), GTK_BOX(vbox2));

  button = prefs_button_add(GIMP_ICON_PREFERENCES_SYSTEM,
                            _("Configure E_xtended Input Devices..."),
                            GTK_BOX(vbox2));
  g_signal_connect(button, "clicked", G_CALLBACK(prefs_input_devices_dialog),
                   gimp);
  ...
  button =
      prefs_button_add(GIMP_ICON_DOCUMENT_SAVE,
                       _("Save Input Device Settings _Now"), GTK_BOX(vbox2));
  g_signal_connect(button, "clicked", G_CALLBACK(prefs_devices_save_callback),
                   gimp);
  ...
  button2 = prefs_button_add(GIMP_ICON_RESET,
                             _("_Reset Saved Input Device Settings to "
                               "Default Values"),
                             GTK_BOX(vbox2));
  g_signal_connect(button2, "clicked", G_CALLBACK(prefs_devices_clear_callback),
                   gimp);
  ...
  /****************************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-controllers",
      _("Additional Input Controllers"), _("Input Controllers"),
      GIMP_HELP_PREFS_INPUT_CONTROLLERS, &top_iter, &child_iter);
  ...
  /*************/
  vbox = gimp_prefs_box_add_page(
      GIMP_PREFS_BOX(prefs_box), "gimp-prefs-folders", _("Folders"),
      _("Folders"), GIMP_HELP_PREFS_FOLDERS, NULL, &top_iter);

  button = gimp_prefs_box_set_page_resettable(GIMP_PREFS_BOX(prefs_box), vbox,
                                              _("Reset _Folders"));
  g_signal_connect(button, "clicked", G_CALLBACK(prefs_folders_reset), config);

  {
    ...
    } dirs[] = {{"temp-path", N_("_Temporary folder:"),
                 N_("Select Folder for Temporary Files")},
                {"swap-path", N_("_Swap folder:"), N_("Select Swap Folder")}};

    grid = prefs_grid_new(GTK_CONTAINER(vbox));

    for (i = 0; i < G_N_ELEMENTS(dirs); i++) {
      prefs_file_chooser_button_add(
          object, dirs[i].property_name, gettext(dirs[i].label),
          gettext(dirs[i].dialog_title), GTK_GRID(grid), i, NULL);
    }
  }
  ...
  /*********************/
  {
    static const struct {
      ...
      const gchar *writable_property_name;
        ...
        {N_("Brushes"), N_("Brush Folders"), "folders-brushes",
         GIMP_HELP_PREFS_FOLDERS_BRUSHES, N_("Reset Brush _Folders"),
         N_("Select Brush Folders"), "brush-path", "brush-path-writable"},
        {N_("Dynamics"), N_("Dynamics Folders"), "folders-dynamics",
         GIMP_HELP_PREFS_FOLDERS_DYNAMICS, N_("Reset Dynamics _Folders"),
         N_("Select Dynamics Folders"), "dynamics-path",
         "dynamics-path-writable"},
        {N_("Patterns"), N_("Pattern Folders"), "folders-patterns",
         GIMP_HELP_PREFS_FOLDERS_PATTERNS, N_("Reset Pattern _Folders"),
         N_("Select Pattern Folders"), "pattern-path", "pattern-path-writable"},
        {N_("Palettes"), N_("Palette Folders"), "folders-palettes",
         GIMP_HELP_PREFS_FOLDERS_PALETTES, N_("Reset Palette _Folders"),
         N_("Select Palette Folders"), "palette-path", "palette-path-writable"},
        {N_("Gradients"), N_("Gradient Folders"), "folders-gradients",
         GIMP_HELP_PREFS_FOLDERS_GRADIENTS, N_("Reset Gradient _Folders"),
         N_("Select Gradient Folders"), "gradient-path",
         "gradient-path-writable"},
        {N_("Fonts"), N_("Font Folders"), "folders-fonts",
         GIMP_HELP_PREFS_FOLDERS_FONTS, N_("Reset Font _Folders"),
         N_("Select Font Folders"), "font-path", NULL},
        {N_("Tool Presets"), N_("Tool Preset Folders"), "folders-tool-presets",
         GIMP_HELP_PREFS_FOLDERS_TOOL_PRESETS, N_("Reset Tool Preset _Folders"),
         N_("Select Tool Preset Folders"), "tool-preset-path",
         "tool-preset-path-writable"},
        {N_("MyPaint Brushes"), N_("MyPaint Brush Folders"),
         "folders-mypaint-brushes", GIMP_HELP_PREFS_FOLDERS_MYPAINT_BRUSHES,
         N_("Reset MyPaint Brush _Folders"), N_("Select MyPaint Brush Folders"),
         "mypaint-brush-path", "mypaint-brush-path-writable"},
        {N_("Plug-ins"), N_("Plug-in Folders"), "folders-plug-ins",
         GIMP_HELP_PREFS_FOLDERS_PLUG_INS, N_("Reset plug-in _Folders"),
         N_("Select plug-in Folders"), "plug-in-path", NULL},
        {N_("Scripts"), N_("Script-Fu Folders"), "folders-scripts",
         GIMP_HELP_PREFS_FOLDERS_SCRIPTS, N_("Reset Script-Fu _Folders"),
         N_("Select Script-Fu Folders"), "script-fu-path", NULL},
        {N_("Modules"), N_("Module Folders"), "folders-modules",
         GIMP_HELP_PREFS_FOLDERS_MODULES, N_("Reset Module _Folders"),
         N_("Select Module Folders"), "module-path", NULL},
        {N_("Interpreters"), N_("Interpreter Folders"), "folders-interp",
         GIMP_HELP_PREFS_FOLDERS_INTERPRETERS, N_("Reset Interpreter _Folders"),
         N_("Select Interpreter Folders"), "interpreter-path", NULL},
        {N_("Environment"), N_("Environment Folders"), "folders-environ",
         GIMP_HELP_PREFS_FOLDERS_ENVIRONMENT, N_("Reset Environment _Folders"),
         N_("Select Environment Folders"), "environ-path", NULL},
        {N_("Themes"), N_("Theme Folders"), "folders-themes",
         GIMP_HELP_PREFS_FOLDERS_THEMES, N_("Reset Theme _Folders"),
         N_("Select Theme Folders"), "theme-path", NULL},
        {N_("Icon Themes"), N_("Icon Theme Folders"), "folders-icon-themes",
         GIMP_HELP_PREFS_FOLDERS_ICON_THEMES, N_("Reset Icon Theme _Folders"),
         N_("Select Icon Theme Folders"), "icon-theme-path", NULL}};
    ...
    for (i = 0; i < G_N_ELEMENTS(paths); i++) {
      ...
      vbox = gimp_prefs_box_add_page(
          GIMP_PREFS_BOX(prefs_box), icon_name, gettext(paths[i].label),
          gettext(paths[i].tree_label), paths[i].help_data, &top_iter,
          &child_iter);
      g_free(icon_name);

      button = gimp_prefs_box_set_page_resettable(
          GIMP_PREFS_BOX(prefs_box), vbox, gettext(paths[i].reset_label));
      ...
      g_signal_connect(button, "clicked", G_CALLBACK(prefs_path_reset), config);

      editor = gimp_prop_path_editor_new(object, paths[i].path_property_name,
