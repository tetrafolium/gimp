...
static void fill_dialog_response(GtkWidget *dialog, gint response_id,
                                 FillDialog *private);
...
private
  ...
  dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, item), context, title, "gimp-fill-options",
      icon_name, _("Choose Fill Style"), parent, gimp_standard_help_func,
      help_id,
  ...
  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)fill_dialog_free, private);

  g_signal_connect(dialog, "response", G_CALLBACK(fill_dialog_response),
                   private);
