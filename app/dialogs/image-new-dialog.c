...
static void image_new_dialog_response(GtkWidget *widget, gint response_id,
...
private
  = g_slice_new0(ImageNewDialog);

private
  ->context = gimp_context_new(context->gimp, "image-new-dialog", context);
private
  ->template = g_object_new(GIMP_TYPE_TEMPLATE, NULL);

private
  ->dialog = dialog =
      gimp_dialog_new(_("Create a New Image"), "gimp-image-new", NULL, 0,
  ...
  g_object_set_data_full(G_OBJECT(dialog), "gimp-image-new-dialog", private,
                         (GDestroyNotify)image_new_dialog_free);

  g_signal_connect(dialog, "response", G_CALLBACK(image_new_dialog_response),
                   private);
...
private
  ->combo = g_object_new(GIMP_TYPE_CONTAINER_COMBO_BOX, "container",
                         context->gimp->templates, "context", private->context,
                         "view-size", 16, "view-border-width", 0, "ellipsize",
                         PANGO_ELLIPSIZE_NONE, "focus-on-click", FALSE, NULL);
  ...
  g_signal_connect(private->context, "template-changed",
                   G_CALLBACK(image_new_template_changed), private);

  /*  Template editor  */
private
  ->editor = gimp_template_editor_new(private->template, context->gimp, FALSE);
  gtk_box_pack_start(GTK_BOX(main_vbox), private->editor, FALSE, FALSE, 0);
  gtk_widget_show(private->editor);

  entry = GIMP_SIZE_ENTRY(
      gimp_template_editor_get_size_se(GIMP_TEMPLATE_EDITOR(private->editor)));
  gimp_size_entry_set_activates_default(entry, TRUE);
  gimp_size_entry_grab_focus(entry);

  image_new_template_changed(
      private->context, gimp_context_get_template(private->context), private);
...
private
  = g_object_get_data(G_OBJECT(dialog), "gimp-image-new-dialog");
  ...
  if (!template) {
    template = gimp_image_new_get_last_template(private->context->gimp, image);
...
static void image_new_dialog_response(GtkWidget *dialog, gint response_id,
                                      ImageNewDialog *private) {
  switch (response_id) {
  case RESPONSE_RESET:
...
static void image_new_template_changed(GimpContext *context,
  ...
  if (comment) {
    g_object_set(private->template, "comment", comment, NULL);
...
static void image_new_confirm_response(GtkWidget *dialog, gint response_id,
                                       ImageNewDialog *private) {
  gtk_widget_destroy(dialog);
...
private
  ->confirm_dialog = dialog = gimp_message_dialog_new(
      _("Confirm Image Size"), GIMP_ICON_DIALOG_WARNING, private->dialog,
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(image_new_confirm_response),
                   private);
