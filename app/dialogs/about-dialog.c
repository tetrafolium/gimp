 ...
 */
#define START_INDEX                                                            \
  (G_N_ELEMENTS(creators) - 1 /*NULL*/ + G_N_ELEMENTS(maintainers) - 1 /*NULL*/)
...
static void about_dialog_map(GtkWidget *widget, GimpAboutDialog *dialog);
static void about_dialog_unmap(GtkWidget *widget, GimpAboutDialog *dialog);
...
static void about_dialog_add_update(GimpAboutDialog *dialog,
                                    GimpCoreConfig *config);
static gboolean about_dialog_anim_draw(GtkWidget *widget, cairo_t *cr,
                                       GimpAboutDialog *dialog);
...
static void about_dialog_last_release_changed(GimpCoreConfig *config,
                                              const GParamSpec *pspec,
                                              GimpAboutDialog *dialog);
static void about_dialog_download_clicked(GtkButton *button, const gchar *link);
...
GtkWidget *about_dialog_create(GimpCoreConfig *config) {
  ...
  if (!dialog.dialog) {
    ...
    widget = g_object_new(
        GTK_TYPE_ABOUT_DIALOG, "role", "gimp-about", "window-position",
        GTK_WIN_POS_CENTER, "title", _("About GIMP"), "program-name",
        GIMP_ACRONYM, "version", version, "copyright", copyright, "comments",
        GIMP_NAME, "license", GIMP_LICENSE, "wrap-license", TRUE, "logo",
        pixbuf, "website", "https://www.gimp.org/", "website-label",
        _("Visit the GIMP website"), "authors", authors, "artists", artists,
    ...
    g_signal_connect(widget, "response", G_CALLBACK(gtk_widget_destroy), NULL);

    g_signal_connect(widget, "map", G_CALLBACK(about_dialog_map), &dialog);
    g_signal_connect(widget, "unmap", G_CALLBACK(about_dialog_unmap), &dialog);
...
static void about_dialog_add_animation(GtkWidget *vbox,
  ...
  g_object_weak_ref(G_OBJECT(dialog->anim_area), (GWeakNotify)g_object_unref,
                    dialog->layout);
  ...
  g_signal_connect(dialog->anim_area, "draw",
                   G_CALLBACK(about_dialog_anim_draw), dialog);
}
...
static void about_dialog_add_update(GimpAboutDialog *dialog,
  ...
  gtk_container_child_set(GTK_CONTAINER(box2), button_label, "expand", TRUE,
                          NULL);
  ...
  if (config->last_known_release != NULL) {
    ...
    if (config->last_revision > 0) {
      ...
      text = g_strdup_printf(
          _("Download GIMP %s revision %d (released on %s)\n"),
          config->last_known_release, config->last_revision, date);
    ...
    gtk_image_set_from_icon_name(GTK_IMAGE(button_image), "view-refresh",
                                 GTK_ICON_SIZE_MENU);
    gtk_label_set_text(GTK_LABEL(button_label), _("Check for updates"));
    g_signal_connect_swapped(button, "clicked", (GCallback)gimp_update_check,
                             config);
  }
  ...
  g_signal_connect(config, "notify::last-known-release",
                   (GCallback)about_dialog_last_release_changed, dialog);
}
...
static gchar *insert_spacers(const gchar *string) {
...
static void decorate_text(GimpAboutDialog *dialog, gint anim_type,
                          gdouble time) {
  const gchar *text;
  const gchar *ptr;
  gint letter_count = 0;
    ...
    while ((unichr = g_utf8_get_char(ptr))) {
      if (unichr == 0x200b) {
        ...
        attr = pango_attr_rise_new((1.0 - time) * 18000 *
                                   sin(4.0 * time + (float)letter_count * 0.7));
        attr->start_index = cluster_start;
        attr->end_index = cluster_end;
        pango_attr_list_change(attrlist, attr);
...
static void about_dialog_last_release_changed(GimpCoreConfig *config,
  ...
  g_signal_handlers_disconnect_by_func(
      config, (GCallback)about_dialog_last_release_changed, dialog);
  if (!dialog->dialog)
    return;
...
static void about_dialog_download_clicked(GtkButton *button,
                                          const gchar *link) {
  GtkWidget *window;
