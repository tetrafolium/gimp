...
static void convert_dialog_response(GtkWidget *widget, gint response_id,
...
GtkWidget *convert_indexed_dialog_new(
  ...
  g_return_val_if_fail(
      custom_palette == NULL || GIMP_IS_PALETTE(custom_palette), NULL);
  g_return_val_if_fail(callback != NULL, NULL);

private
  = g_slice_new0(IndexedDialog);
...
private
  ->dither_text_layers = dither_text_layers;
private
...
private
  ->dialog = dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, image), context, _("Indexed Color Conversion"),
      "gimp-image-convert-indexed", GIMP_ICON_CONVERT_INDEXED,
      _("Convert Image to Indexed Colors"), parent, gimp_standard_help_func,
  ...
  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)convert_dialog_free,
                    private);

  g_signal_connect(dialog, "response", G_CALLBACK(convert_dialog_response),
                   private);
  ...
  frame = gimp_enum_radio_frame_new_with_range(
      GIMP_TYPE_CONVERT_PALETTE_TYPE, GIMP_CONVERT_PALETTE_GENERATE,
      (palette_box ? GIMP_CONVERT_PALETTE_CUSTOM : GIMP_CONVERT_PALETTE_MONO),
      gtk_label_new(_("Colormap")), G_CALLBACK(convert_dialog_type_update),
      private, NULL, &button);
...
private
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), private->dither_type,
                             G_CALLBACK(gimp_int_combo_box_get_active),
                             &private->dither_type, NULL);

  toggle = gtk_check_button_new_with_mnemonic(
      _("Enable dithering of _transparency"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(toggle),
  ...
  gimp_help_set_help_data(
      toggle, _("Dithering text layers will make them uneditable"), NULL);
...
static void convert_dialog_response(GtkWidget *dialog, gint response_id,
                                    IndexedDialog *private) {
  if (response_id == GTK_RESPONSE_OK) {
  private
    ->callback(dialog, private->image, private->palette_type,
               private->max_colors, private->remove_duplicates,
               private->dither_type, private->dither_alpha,
               private->dither_text_layers, private->custom_palette,
               private->user_data);
  } else {
...
private
  ->container = gimp_container_filter(
      gimp_data_factory_get_container(gimp->palette_factory),
      convert_dialog_palette_filter, NULL);
...
private
  ->context = gimp_context_new(gimp, "convert-dialog", NULL);
  ...
  if (!custom_found) {
    if (web_palette)
    private
    ->custom_palette = web_palette;
    else private->custom_palette =
        GIMP_LIST(private->container)->queue->head->data;
  }

  gimp_context_set_palette(private->context, private->custom_palette);

  g_signal_connect(private->context, "palette-changed",
                   G_CALLBACK(convert_dialog_palette_changed), private);
...
static void convert_dialog_type_update(GtkWidget *widget,
  ...
  if (private->duplicates_toggle)
    gtk_widget_set_sensitive(
        private->duplicates_toggle,
        private->palette_type != GIMP_CONVERT_PALETTE_GENERATE &&
            private->palette_type != GIMP_CONVERT_PALETTE_MONO);
}
