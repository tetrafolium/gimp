...
GtkWidget *template_options_dialog_new(GimpTemplate *template,
                                       ...
                                       const gchar *desc, const gchar *help_id,
                                       GimpTemplateOptionsCallback callback,
                                       gpointer user_data) {
  TemplateOptionsDialog *private;
  GtkWidget *dialog;
...
private
  = g_slice_new0(TemplateOptionsDialog);
...
private
  ...
  if (template) {
    ...
    template = gimp_config_duplicate(
        GIMP_CONFIG(context->gimp->config->default_image));
    viewable = GIMP_VIEWABLE(template);
  ...
  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)template_options_dialog_free,
                    private);

  g_signal_connect(dialog, "response",
                   G_CALLBACK(template_options_dialog_response), private);
...
private
  ->editor = gimp_template_editor_new(template, context->gimp, TRUE);
...
static void template_options_dialog_response(GtkWidget *dialog,
  ...
  private
    ->callback(dialog, private->template,
               gimp_template_editor_get_template(editor), private->context,
               private->user_data);
  } else {
