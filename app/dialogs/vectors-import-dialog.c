...
static void vectors_import_dialog_response(GtkWidget *dialog, gint response_id,
                                           VectorsImportDialog *private);

/*  public function  */

GtkWidget *vectors_import_dialog_new(GimpImage *image, GtkWidget *parent,
                                     GFile *import_folder,
                                     gboolean merge_vectors,
                                     gboolean scale_vectors,
                                     GimpVectorsImportCallback callback,
  ...
  g_return_val_if_fail(import_folder == NULL || G_IS_FILE(import_folder), NULL);
  g_return_val_if_fail(callback != NULL, NULL);

private
  = g_slice_new0(VectorsImportDialog);
...
private
  ...
  dialog = gtk_file_chooser_dialog_new(
      _("Import Paths from SVG"), NULL, GTK_FILE_CHOOSER_ACTION_OPEN,
  ...
  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)vectors_import_dialog_free,
                    private);
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(vectors_import_dialog_response), private);
...
static void vectors_import_dialog_response(GtkWidget *dialog, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    private
      ->callback(dialog, private->image, file, folder, private->merge_vectors,
                 private->scale_vectors, private->user_data);
