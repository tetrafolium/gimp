...
static void color_profile_dialog_response(GtkWidget *dialog, gint response_id,
...
GtkWidget *color_profile_dialog_new(
    ColorProfileDialogType dialog_type, GimpImage *image, GimpContext *context,
    GtkWidget *parent, GimpColorProfile *current_profile,
    GimpColorProfile *default_profile, GimpColorRenderingIntent intent,
    gboolean bpc, GimpColorProfileCallback callback, gpointer user_data) {
  ProfileDialog *private;
  GtkWidget *dialog;
  GtkWidget *frame;
  GtkWidget *vbox;
  ...
  g_return_val_if_fail(
      current_profile == NULL || GIMP_IS_COLOR_PROFILE(current_profile), NULL);
  g_return_val_if_fail(
      default_profile == NULL || GIMP_IS_COLOR_PROFILE(default_profile), NULL);
  g_return_val_if_fail(callback != NULL, NULL);

private
  = g_slice_new0(ProfileDialog);
...
private
  ...
  case COLOR_PROFILE_DIALOG_ASSIGN_PROFILE:
    dialog = gimp_viewable_dialog_new(
        g_list_prepend(NULL, image), context, _("Assign ICC Color Profile"),
        "gimp-image-color-profile-assign", NULL,
        _("Assign a color profile to the image"), parent,
        gimp_standard_help_func, GIMP_HELP_IMAGE_COLOR_PROFILE_ASSIGN,

        _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Assign"), GTK_RESPONSE_OK,
  ...
  case COLOR_PROFILE_DIALOG_CONVERT_TO_PROFILE:
    dialog = gimp_viewable_dialog_new(
        g_list_prepend(NULL, image), context, _("Convert to ICC Color Profile"),
        "gimp-image-color-profile-convert", NULL,
        _("Convert the image to a color profile"), parent,
        gimp_standard_help_func, GIMP_HELP_IMAGE_COLOR_PROFILE_CONVERT,

        _("_Cancel"), GTK_RESPONSE_CANCEL, _("C_onvert"), GTK_RESPONSE_OK,
  ...
  case COLOR_PROFILE_DIALOG_CONVERT_TO_RGB:
    dialog = gimp_viewable_dialog_new(
        g_list_prepend(NULL, image), context, _("RGB Conversion"),
        "gimp-image-convert-rgb", GIMP_ICON_CONVERT_RGB,
        _("Convert Image to RGB"), parent, gimp_standard_help_func,
  ...
  case COLOR_PROFILE_DIALOG_CONVERT_TO_GRAY:
    dialog = gimp_viewable_dialog_new(
        g_list_prepend(NULL, image), context, _("Grayscale Conversion"),
        "gimp-image-convert-gray", GIMP_ICON_CONVERT_GRAYSCALE,
        _("Convert Image to Grayscale"), parent, gimp_standard_help_func,
  ...
  case COLOR_PROFILE_DIALOG_SELECT_SOFTPROOF_PROFILE:
    dialog = gimp_viewable_dialog_new(
        g_list_prepend(NULL, image), context, _("Soft-Proof Profile"),
        "gimp-select-softproof-profile", GIMP_ICON_DOCUMENT_PRINT,
        _("Select Soft-Proof Profile"), parent, gimp_standard_help_func,
        GIMP_HELP_VIEW_COLOR_MANAGEMENT,

        _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Select"), GTK_RESPONSE_OK,
...
private
  ...
  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)color_profile_dialog_free,
                    private);

  g_signal_connect(dialog, "response",
                   G_CALLBACK(color_profile_dialog_response), private);
...
private
  ->combo = color_profile_combo_box_new(private);
...
private
  ->dest_view = gimp_color_profile_view_new();
  ...
  g_signal_connect(private->combo, "changed",
                   G_CALLBACK(color_profile_dest_changed), private);
  ...
  if (dialog_type == COLOR_PROFILE_DIALOG_CONVERT_TO_PROFILE) {
    ...
    gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), private->intent,
                               G_CALLBACK(gimp_int_combo_box_get_active),
                               &private->intent, NULL);

    gtk_label_set_mnemonic_widget(GTK_LABEL(label), combo);

    toggle = gtk_check_button_new_with_mnemonic(_("_Black Point Compensation"));
...
static GtkWidget *color_profile_combo_box_new(ProfileDialog *private) {
  ...
  chooser = gimp_color_profile_chooser_dialog_new(
      _("Select Destination Profile"), NULL, GTK_FILE_CHOOSER_ACTION_OPEN);

  gimp_color_profile_chooser_dialog_connect_path(
      chooser, G_OBJECT(private->image->gimp->config), "color-profile-path");
  ...
  gimp_color_profile_combo_box_set_active_file(
      GIMP_COLOR_PROFILE_COMBO_BOX(combo), NULL, NULL);
...
static void color_profile_dialog_response(GtkWidget *dialog, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    file = gimp_color_profile_combo_box_get_active_file(
        GIMP_COLOR_PROFILE_COMBO_BOX(private->combo));
    ...
    if (file) {
      ...
      if (!profile) {
        gimp_message(private->image->gimp, G_OBJECT(dialog), GIMP_MESSAGE_ERROR,
                     "%s", error->message);
        g_clear_error(&error);
  ...
  private
    ->callback(dialog, private->image, profile, file, private->intent,
               private->bpc, private->user_data);
  ...
  if (file) {
    ...
    if (!dest_profile) {
      gimp_color_profile_view_set_error(
          GIMP_COLOR_PROFILE_VIEW(private->dest_view), error->message);
      g_clear_error(&error);
    }
  } else if (private->default_profile) {
    ...
    gimp_color_profile_view_set_error(
        GIMP_COLOR_PROFILE_VIEW(private->dest_view), C_("profile", "None"));
  }

  if (dest_profile) {
    gimp_color_profile_view_set_profile(
        GIMP_COLOR_PROFILE_VIEW(private->dest_view), dest_profile);
