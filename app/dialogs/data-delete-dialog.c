...
private
  ...
  dialog = gimp_message_dialog_new(
      _("Delete Object"), "edit-delete", gtk_widget_get_toplevel(parent), 0,
  ...
  g_signal_connect_object(data, "disconnect", G_CALLBACK(gtk_widget_destroy),
                          dialog, G_CONNECT_SWAPPED);

  g_signal_connect(dialog, "response", G_CALLBACK(data_delete_dialog_response),
                   private);
...
static void data_delete_dialog_response(GtkWidget *dialog, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    if (private->context &&
        GIMP_OBJECT(data) == gimp_context_get_by_type(
                                 private->context,
                                 gimp_container_get_children_type(container))) {
      new_active = gimp_container_get_neighbor_of(container, GIMP_OBJECT(data));
    }

    if (!gimp_data_factory_data_delete(factory, data, TRUE, &error)) {
      gimp_message(gimp_data_factory_get_gimp(factory),
                   G_OBJECT(private->parent), GIMP_MESSAGE_ERROR, "%s",
                   error->message);
      g_clear_error(&error);
    }
