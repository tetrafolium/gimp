...
typedef enum { GRADIENT_IMPORT, IMAGE_IMPORT, FILE_IMPORT } ImportType;
...
static void palette_import_response(GtkWidget *dialog, gint response_id,
                                    ImportDialog *private);
static void palette_import_gradient_changed(GimpContext *context,
                                            GimpGradient *gradient,
                                            ImportDialog *private);
static void palette_import_image_changed(GimpContext *context, GimpImage *image,
                                         ImportDialog *private);
...
static void palette_import_filename_changed(GtkFileChooser *button,
                                            ImportDialog *private);
static void import_dialog_drop_callback(GtkWidget *widget, gint x, gint y,
...
static void palette_import_columns_changed(GimpLabelSpin *columns,
                                           ImportDialog *private);
static void palette_import_image_add(GimpContainer *container, GimpImage *image,
                                     ImportDialog *private);
...
private
  = g_slice_new0(ImportDialog);

private
  ->import_type = GRADIENT_IMPORT;
private
  ->context = gimp_context_new(context->gimp, "Palette Import", context);

  dialog = private->dialog = gimp_dialog_new(
      _("Import a New Palette"), "gimp-palette-import", NULL, 0,
      gimp_standard_help_func, GIMP_HELP_PALETTE_IMPORT,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Import"), GTK_RESPONSE_OK,
  ...
  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)palette_import_free,
                    private);

  g_signal_connect(dialog, "response", G_CALLBACK(palette_import_response),
                   private);

  gimp_dnd_viewable_dest_add(dialog, GIMP_TYPE_GRADIENT,
                             import_dialog_drop_callback, private);
  gimp_dnd_viewable_dest_add(dialog, GIMP_TYPE_IMAGE,
                             import_dialog_drop_callback, private);
...
private
  ->gradient_radio = gtk_radio_button_new_with_mnemonic(group, _("_Gradient"));
  ...
  g_signal_connect(private->gradient_radio, "toggled",
                   G_CALLBACK(palette_import_grad_callback), private);

private
  ->image_radio = gtk_radio_button_new_with_mnemonic(group, _("I_mage"));
...
private
  ...
  g_signal_connect_swapped(private->sample_merged_toggle, "toggled",
                           G_CALLBACK(palette_import_make_palette), private);

private
  ->selection_only_toggle =
      gtk_check_button_new_with_mnemonic(_("_Selected Pixels only"));
  gtk_toggle_button_set_active(
      GTK_TOGGLE_BUTTON(private->selection_only_toggle), FALSE);
...
private
  ->file_radio = gtk_radio_button_new_with_mnemonic(group, _("Palette _file"));
  group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(private->image_radio));
  ...
  g_signal_connect(private->file_radio, "toggled",
                   G_CALLBACK(palette_import_file_callback), private);

  size_group = gtk_size_group_new(GTK_SIZE_GROUP_VERTICAL);

  /*  The gradient menu  */
private
  ->gradient_combo = gimp_container_combo_box_new(
      gimp_data_factory_get_container(context->gimp->gradient_factory),
      private->context, 24, 1);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, NULL, 0.0, 0.5,
                           private->gradient_combo, 1);
  gtk_size_group_add_widget(size_group, private->gradient_combo);

  /*  The image menu  */
private
  ->image_combo = gimp_container_combo_box_new(context->gimp->images,
                                               private->context, 24, 1);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, NULL, 0.0, 0.5,
                           private->image_combo, 1);
  gtk_size_group_add_widget(size_group, private->image_combo);

  /*  Palette file name entry  */
private
  ->file_chooser = gtk_file_chooser_button_new(_("Select Palette File"),
                                               GTK_FILE_CHOOSER_ACTION_OPEN);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 4, NULL, 0.0, 0.5,
                           private->file_chooser, 1);
...
private
  ...
  gtk_entry_set_text(GTK_ENTRY(private->entry),
                     gradient ? gimp_object_get_name(gradient)
                              : _("New import"));
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("Palette _name:"), 0.0, 0.5,
                           private->entry, 2);

  /*  The # of colors  */
private
  ->num_colors =
      gimp_scale_entry_new(_("N_umber of colors:"), 256, 2, 10000, 0);
  gimp_grid_attach_aligned(GTK_GRID(grid), -1, 1, NULL, 0.0, 0.5,
                           private->num_colors, 3);
  gimp_scale_entry_set_logarithmic(GIMP_SCALE_ENTRY(private->num_colors), TRUE);

  g_signal_connect_swapped(private->num_colors, "value-changed",
                           G_CALLBACK(palette_import_make_palette), private);

  /*  The columns  */
private
  ->columns = gimp_scale_entry_new(_("C_olumns:"), 16, 0, 64, 0);
  gimp_grid_attach_aligned(GTK_GRID(grid), -1, 2, NULL, 0.0, 0.5,
                           private->columns, 3);

  g_signal_connect(private->columns, "value-changed",
                   G_CALLBACK(palette_import_columns_changed), private);

  /*  The interval  */
private
  ->threshold = gimp_scale_entry_new(_("I_nterval:"), 1, 1, 128, 0);
  gimp_grid_attach_aligned(GTK_GRID(grid), -1, 3, NULL, 0.0, 0.5,
                           private->threshold, 3);
...
private
  ->preview = gimp_view_new_full_by_types(private->context, GIMP_TYPE_VIEW,
                                          GIMP_TYPE_PALETTE, 192, 192, 1, TRUE,
                                          FALSE, FALSE);
...
private
  ...
  g_signal_connect(context->gimp->images, "add",
                   G_CALLBACK(palette_import_image_add), private);
  g_signal_connect(context->gimp->images, "remove",
                   G_CALLBACK(palette_import_image_remove), private);

  g_signal_connect(private->context, "gradient-changed",
                   G_CALLBACK(palette_import_gradient_changed), private);
  g_signal_connect(private->context, "image-changed",
                   G_CALLBACK(palette_import_image_changed), private);
  g_signal_connect(private->file_chooser, "selection-changed",
                   G_CALLBACK(palette_import_filename_changed), private);
...
static void palette_import_free(ImportDialog *private) {
  ...
  g_signal_handlers_disconnect_by_func(gimp->images, palette_import_image_add,
                                       private);
  g_signal_handlers_disconnect_by_func(gimp->images,
                                       palette_import_image_remove, private);
...
static void palette_import_image_changed(GimpContext *context, GimpImage *image,
  ...
  if (private->image) {
    g_signal_handlers_disconnect_by_func(private->image,
                                         palette_import_layer_changed, private);
    g_signal_handlers_disconnect_by_func(private->image,
                                         palette_import_mask_changed, private);
  }
...
private
  ...
  if (private->import_type == IMAGE_IMPORT) {
    ...
    if (image) {
      ...
      label = g_strdup_printf("%s-%d", gimp_image_get_display_name(image),
  ...
  if (private->image) {
    g_signal_connect(private->image, "selected-layers-changed",
                     G_CALLBACK(palette_import_layer_changed), private);
    g_signal_connect(private->image, "mask-changed",
                     G_CALLBACK(palette_import_mask_changed), private);
  }
}
...
static void import_dialog_drop_callback(GtkWidget *widget, gint x, gint y,
  ...
  gimp_context_set_by_type(private->context, G_TYPE_FROM_INSTANCE(viewable),
                           GIMP_OBJECT(viewable));

  if (GIMP_IS_GRADIENT(viewable) && private->import_type != GRADIENT_IMPORT) {
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(private->gradient_radio),
                                 TRUE);
  } else if (GIMP_IS_IMAGE(viewable) && private->import_type != IMAGE_IMPORT) {
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(private->image_radio), TRUE);
  }
}

/*  the import source menu item callbacks  ***********************************/

static void palette_import_set_sensitive(ImportDialog *private) {
...
private
  ...
  gtk_entry_set_text(GTK_ENTRY(private->entry), gimp_object_get_name(gradient));
...
static void palette_import_make_palette(ImportDialog *private) {
  ...
  n_colors =
      ROUND(gimp_label_spin_get_value(GIMP_LABEL_SPIN(private->num_colors)));
  n_columns =
      ROUND(gimp_label_spin_get_value(GIMP_LABEL_SPIN(private->columns)));
  threshold =
      ROUND(gimp_label_spin_get_value(GIMP_LABEL_SPIN(private->threshold)));
  ...
  case GRADIENT_IMPORT: {
    ...
    palette = gimp_palette_import_from_gradient(
        gradient, private->context, FALSE, GIMP_GRADIENT_BLEND_RGB_PERCEPTUAL,
  ...
  case IMAGE_IMPORT: {
    ...
    sample_merged = gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(private->sample_merged_toggle));

    selection_only = gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(private->selection_only_toggle));

    if (gimp_image_get_base_type(image) == GIMP_INDEXED) {
      palette = gimp_palette_import_from_indexed_image(image, private->context,
                                                       palette_name);
    } else if (sample_merged) {
      palette =
          gimp_palette_import_from_image(image, private->context, palette_name,
                                         n_colors, threshold, selection_only);
    } else {
      GList *drawables;
      ...
      if (drawables)
        palette = gimp_palette_import_from_drawables(
            drawables, private->context, palette_name, n_colors, threshold,
            selection_only);
    }
  } break;
  ...
  case FILE_IMPORT: {
    ...
    palette = gimp_palette_import_from_file(private->context, file,
                                            palette_name, &error);
    g_object_unref(file);

    if (!palette) {
      gimp_message_literal(private->context->gimp, G_OBJECT(private->dialog),
...
private
  ...
  if (palette) {
    ...
    gimp_view_set_viewable(GIMP_VIEW(private->preview), GIMP_VIEWABLE(palette));
  }
