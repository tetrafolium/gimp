...
static void layer_options_dialog_callback(
    GtkWidget *dialog, GimpImage *image, GimpItem *item, GimpContext *context,
    const gchar *item_name, gboolean item_visible, gboolean item_linked,
    GimpColorTag item_color_tag, gboolean item_lock_content,
    gboolean item_lock_position, gpointer user_data);
static void
...
GtkWidget *layer_options_dialog_new(
    GimpImage *image, GimpLayer *layer, GimpContext *context, GtkWidget *parent,
    const gchar *title, const gchar *role, const gchar *icon_name,
    const gchar *desc, const gchar *help_id, const gchar *layer_name,
    ...
    GimpFillType layer_fill_type, gboolean layer_visible, gboolean layer_linked,
    GimpColorTag layer_color_tag, gboolean layer_lock_content,
    gboolean layer_lock_position, gboolean layer_lock_alpha,
    GimpLayerOptionsCallback callback, gpointer user_data) {
  LayerOptionsDialog *private;
  GtkWidget *dialog;
  GtkWidget *grid;
...
private
  = g_slice_new0(LayerOptionsDialog);
...
private
  ->mode = layer_mode;
private
  ->blend_space = layer_blend_space;
private
  ->composite_space = layer_composite_space;
private
  ->composite_mode = layer_composite_mode;
private
  ->opacity = layer_opacity * 100.0;
private
  ->fill_type = layer_fill_type;
private
  ->lock_alpha = layer_lock_alpha;
private
  ->rename_text_layers = FALSE;
private
  ->callback = callback;
private
  ->user_data = user_data;
  ...
  private
  ->rename_text_layers = GIMP_TEXT_LAYER(layer)->auto_rename;

  dialog = item_options_dialog_new(
      image, GIMP_ITEM(layer), context, parent, title, role, icon_name, desc,
      help_id, _("Layer _name:"), GIMP_ICON_TOOL_PAINTBRUSH, _("Lock _pixels"),
      _("Lock position and _size"), layer_name, layer_visible, layer_linked,
      layer_color_tag, layer_lock_content, layer_lock_position,
      layer_options_dialog_callback, private);

  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)layer_options_dialog_free,
                    private);
...
private
  ->mode_box = gimp_layer_mode_box_new(mode_context);
  ...
  g_signal_connect(private->mode_box, "notify::layer-mode",
                   G_CALLBACK(layer_options_dialog_mode_notify), private);

  space_model = gimp_enum_store_new_with_range(
      GIMP_TYPE_LAYER_COLOR_SPACE, GIMP_LAYER_COLOR_SPACE_AUTO,
      GIMP_LAYER_COLOR_SPACE_RGB_PERCEPTUAL);

private
  ->blend_space_combo = combo =
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), private->blend_space,
...
private
  ...
  gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), private->composite_mode,
                             G_CALLBACK(gimp_int_combo_box_get_active),
  ...
  adjustment = gtk_adjustment_new(private->opacity, 0.0, 100.0, 1.0, 10.0, 0.0);
  ...
  private
    ->size_se = gimp_size_entry_new(1, GIMP_UNIT_PIXEL, "%a", TRUE, TRUE, FALSE,
                                    10, GIMP_SIZE_ENTRY_UPDATE_SIZE);
    ...
    gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(private->size_se), 0, xres,
                                   FALSE);
    gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(private->size_se), 1, yres,
                                   FALSE);
    ...
    gimp_size_entry_set_size(GIMP_SIZE_ENTRY(private->size_se), 0, 0,
                             gimp_image_get_width(image));
    gimp_size_entry_set_size(GIMP_SIZE_ENTRY(private->size_se), 1, 0,
...
private
  ->offset_se = gimp_size_entry_new(1, GIMP_UNIT_PIXEL, "%a", TRUE, TRUE, FALSE,
                                    10, GIMP_SIZE_ENTRY_UPDATE_SIZE);
  ...
  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(private->offset_se), 0, xres,
                                 FALSE);
  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(private->offset_se), 1, yres,
                                 FALSE);
  ...
  gimp_size_entry_set_size(GIMP_SIZE_ENTRY(private->offset_se), 0, 0,
                           gimp_image_get_width(image));
  gimp_size_entry_set_size(GIMP_SIZE_ENTRY(private->offset_se), 1, 0,
                           gimp_image_get_height(image));
  ...
  if (!layer) {
    ...
    gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("_Fill with:"), 0.0,
                             0.5, combo, 1);
    gimp_int_combo_box_connect(GIMP_INT_COMBO_BOX(combo), private->fill_type,
                               G_CALLBACK(gimp_int_combo_box_get_active),
  ...
  if (layer) {
    ...
    view =
        gimp_container_tree_view_new(filters, context, GIMP_VIEW_SIZE_SMALL, 0);
  ...
  button = item_options_dialog_add_switch(dialog, GIMP_ICON_TRANSPARENCY,
                                          _("Lock _alpha"));
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(button), private->lock_alpha);
  g_signal_connect(button, "toggled", G_CALLBACK(gimp_toggle_button_update),
                   &private->lock_alpha);

  /*  For text layers add a toggle to control "auto-rename"  */
  if (layer && gimp_item_is_text_layer(GIMP_ITEM(layer))) {
    button = item_options_dialog_add_switch(dialog, GIMP_ICON_TOOL_TEXT,
    ...
    g_signal_connect(button, "toggled",
                     G_CALLBACK(layer_options_dialog_rename_toggled), private);
  }
...
static void layer_options_dialog_callback(
  ...
  if (private->size_se) {
    width =
        RINT(gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(private->size_se), 0));
    height =
        RINT(gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(private->size_se), 1));
  }
...
private
  ->callback(dialog, image, GIMP_LAYER(item), context, item_name, private->mode,
             private->blend_space, private->composite_space,
             private->composite_mode, private->opacity / 100.0,
             private->fill_type, width, height, offset_x, offset_y,
             item_visible, item_linked, item_color_tag, item_lock_content,
             item_lock_position, private->lock_alpha,
             private->rename_text_layers, private->user_data);
}
...
private
  ...
  gimp_int_combo_box_set_active(
      GIMP_INT_COMBO_BOX(private->composite_space_combo),
      GIMP_LAYER_COLOR_SPACE_AUTO);
  gimp_int_combo_box_set_active(
      GIMP_INT_COMBO_BOX(private->composite_mode_combo),
