...
static void file_open_dialog_response(GtkWidget *dialog, gint response_id,
                                      Gimp *gimp);
static GimpImage *file_open_dialog_open_image(GtkWidget *dialog, Gimp *gimp,
                                              GFile *file,
...
GtkWidget *file_open_dialog_new(Gimp *gimp) {
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(file_open_dialog_response),
                   gimp);
    ...
    if (open_dialog->open_as_layers) {
      if (!file_dialog->image) {
        gimp_open_dialog_set_image(
            open_dialog,
            file_open_dialog_open_image(dialog, gimp, file,
      ...
      } else if (file_open_dialog_open_layers(dialog, file_dialog->image, file,
                                              file_dialog->file_proc)) {
        success = TRUE;
      }
    } else {
      if (file_open_dialog_open_image(dialog, gimp, file,
                                      file_dialog->file_proc)) {
        success = TRUE;
...
static GimpImage *file_open_dialog_open_image(GtkWidget *dialog, Gimp *gimp,
  ...
  image = file_open_with_proc_and_display(
      gimp, gimp_get_user_context(gimp), GIMP_PROGRESS(dialog), file, FALSE,
      load_proc, G_OBJECT(gimp_widget_get_monitor(dialog)), &status, &error);
...
static gboolean file_open_dialog_open_layers(GtkWidget *dialog,
  ...
  new_layers = file_open_layers(
      image->gimp, gimp_get_user_context(image->gimp), GIMP_PROGRESS(dialog),
      image, FALSE, file, GIMP_RUN_INTERACTIVE, load_proc, &status, &error);

  if (new_layers) {
    gimp_image_add_layers(image, new_layers, GIMP_IMAGE_ACTIVE_PARENT, -1, 0, 0,
                          gimp_image_get_width(image),
                          gimp_image_get_height(image), _("Open layers"));
