...
private
  = g_slice_new0(ImageMergeLayersDialog);
...
private
  ->merge_active_group = merge_active_group;
private
  ->discard_invisible = discard_invisible;
private
  ->callback = callback;
private
  ->user_data = user_data;
  ...
  dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, image), context, _("Merge Layers"),
      "gimp-image-merge-layers", GIMP_ICON_LAYER_MERGE_DOWN,
      _("Layers Merge Options"), parent, gimp_standard_help_func,
      GIMP_HELP_IMAGE_MERGE_LAYERS,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Merge"), GTK_RESPONSE_OK,
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(image_merge_layers_dialog_response), private);
  ...
  frame = gimp_enum_radio_frame_new_with_range(
      GIMP_TYPE_MERGE_TYPE, GIMP_EXPAND_AS_NECESSARY, GIMP_CLIP_TO_BOTTOM_LAYER,
      gtk_label_new(_("Final, Merged Layer should be:")),
      G_CALLBACK(gimp_radio_button_update), &private->merge_type, NULL,
      &button);
  ...
  button =
      gtk_check_button_new_with_mnemonic(_("Merge within active _groups only"));
...
static void
image_merge_layers_dialog_response(GtkWidget *dialog, gint response_id,
                                   ImageMergeLayersDialog *private) {
  if (response_id == GTK_RESPONSE_OK) {
  private
    ->callback(dialog, private->image, private->context, private->merge_type,
               private->merge_active_group, private->discard_invisible,
               private->user_data);
  } else {
