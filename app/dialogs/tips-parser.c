...
static gchar *tips_parser_parse_help_id(TipsParser *parser, const gchar **names,
                                        const gchar **values);

static void tips_parser_parse_locale(TipsParser *parser, const gchar **names,
                                     const gchar **values);
static void tips_parser_set_by_locale(TipsParser *parser, gchar **dest);

static const GMarkupParser markup_parser = {
    tips_parser_start_element, tips_parser_end_element, tips_parser_characters,
    NULL, /*  passthrough  */
    NULL  /*  error        */
};
...
static void tips_parser_start_element(GMarkupParseContext *context,
  ...
  case TIPS_IN_TIPS:
    if (strcmp(element_name, "tip") == 0) {
      ...
      parser->current_tip->help_id =
          tips_parser_parse_help_id(parser, attribute_names, attribute_values);
    } else {
      tips_parser_start_unknown(parser);
    }
    break;
  ...
  case TIPS_IN_THETIP:
    if (strcmp(element_name, "b") == 0 || strcmp(element_name, "big") == 0 ||
...
static void tips_parser_parse_locale(TipsParser *parser, const gchar **names,
  ...
  while (*names && *values) {
    if (strcmp(*names, "xml:lang") == 0 && **values) {
      parser->locale_state =
          (parser->locale && strcmp(*values, parser->locale) == 0
               ? TIPS_LOCALE_MATCH
               : TIPS_LOCALE_MISMATCH);
    }
