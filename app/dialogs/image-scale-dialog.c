...
static void image_scale_callback(GtkWidget *widget, GimpViewable *viewable,
...
static void image_scale_confirm_large(ImageScaleDialog *private,
                                      gint64 new_memsize, gint64 max_memsize);
...
private
  ->dialog =
      scale_dialog_new(GIMP_VIEWABLE(image), context,
                       C_("dialog-title", "Scale Image"), "gimp-image-scale",
                       parent, gimp_standard_help_func, GIMP_HELP_IMAGE_SCALE,
                       unit, interpolation, image_scale_callback, private);
...
private
  ...
  scale_check =
      gimp_image_scale_check(image, width, height, max_memsize, &new_memsize);
  switch (scale_check) {
  ...
  private
    ->callback(private->dialog, GIMP_VIEWABLE(private->image), private->width,
               private->height, private->unit, private->interpolation,
               private->xresolution, private->yresolution,
               private->resolution_unit, private->user_data);
    break;
  }
}
...
static GtkWidget *image_scale_confirm_dialog(ImageScaleDialog *private) {
  ...
  widget = gimp_message_dialog_new(
      _("Confirm Scaling"), GIMP_ICON_DIALOG_WARNING, private->dialog,
      GTK_DIALOG_DESTROY_WITH_PARENT, gimp_standard_help_func,
      GIMP_HELP_IMAGE_SCALE_WARNING,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Scale"), GTK_RESPONSE_OK,

      NULL);

  gimp_dialog_set_alternative_button_order(GTK_DIALOG(widget), GTK_RESPONSE_OK,
                                           GTK_RESPONSE_CANCEL, -1);

  g_signal_connect(widget, "response", G_CALLBACK(image_scale_confirm_response),
                   private);
