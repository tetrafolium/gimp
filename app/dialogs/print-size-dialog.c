...
static void print_size_dialog_response(GtkWidget *dialog, gint response_id,
...
static void print_size_dialog_resolution_changed(GtkWidget *widget,
                                                 PrintSizeDialog *private);
static void print_size_dialog_set_size(PrintSizeDialog *private, gdouble width,
                                       gdouble height);
...
private
  ...
  dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, image), context, title, role,
      GIMP_ICON_DOCUMENT_PRINT_RESOLUTION, title, parent, help_func, help_id,
  ...
  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)print_size_dialog_free,
                    private);

  g_signal_connect(dialog, "response", G_CALLBACK(print_size_dialog_response),
                   private);
  ...
  entry = gimp_size_entry_new(0, gimp_get_default_unit(), "%p", FALSE, FALSE,
                              FALSE, SB_WIDTH, GIMP_SIZE_ENTRY_UPDATE_SIZE);
private
  ->size_entry = GIMP_SIZE_ENTRY(entry);
  ...
  gimp_size_entry_add_field(GIMP_SIZE_ENTRY(entry), GTK_SPIN_BUTTON(width),
                            NULL);
  gtk_grid_attach(GTK_GRID(entry), width, 0, 0, 1, 1);
  gtk_widget_show(width);

  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(entry), 0, private->xres,
                                 FALSE);
  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(entry), 1, private->yres,
                                 FALSE);
...
private
  ->resolution_entry = GIMP_SIZE_ENTRY(entry);
...
private
  ->chain = GIMP_CHAIN_BUTTON(chain);

  focus_chain = g_list_prepend(
      focus_chain, gimp_size_entry_get_unit_combo(GIMP_SIZE_ENTRY(entry)));
  ...
  g_signal_connect(private->size_entry, "value-changed",
                   G_CALLBACK(print_size_dialog_size_changed), private);
  g_signal_connect(private->resolution_entry, "value-changed",
                   G_CALLBACK(print_size_dialog_resolution_changed), private);
...
static void print_size_dialog_response(GtkWidget *dialog, gint response_id,
  ...
  private
    ->callback(dialog, private->image, gimp_size_entry_get_refval(entry, 0),
               gimp_size_entry_get_refval(entry, 1),
               gimp_size_entry_get_unit(entry), private->user_data);
    break;
...
static void print_size_dialog_reset(PrintSizeDialog *private) {
  ...
  gimp_size_entry_set_unit(private->resolution_entry, gimp_get_default_unit());
...
static void print_size_dialog_size_changed(GtkWidget *widget,
  ...
  print_size_dialog_set_resolution(private, xres, yres);
  print_size_dialog_set_size(private, gimp_image_get_width(image),
                             gimp_image_get_height(image));
}
...
static void print_size_dialog_set_size(PrintSizeDialog *private, gdouble width,
  ...
  g_signal_handlers_block_by_func(private->size_entry,
                                  print_size_dialog_size_changed, private);
...
private
  ...
  g_signal_handlers_block_by_func(
      private->resolution_entry, print_size_dialog_resolution_changed, private);
