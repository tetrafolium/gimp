...
GtkWidget *action_search_dialog_create(Gimp *gimp) {
  ...
  dialog = gimp_search_popup_new(gimp, "gimp-action-search-dialog",
                                 _("Search Actions"),
                                 action_search_history_and_actions, gimp);
  return dialog;
}
...
static void action_search_history_and_actions(GimpSearchPopup *popup,
  ...
  history_actions =
      gimp_action_history_search(gimp, action_search_match_keyword, keyword);
    ...
    for (list2 = actions; list2; list2 = g_list_next(list2)) {
      ...
      if (action_search_match_keyword(action, keyword, &section, gimp)) {
        ...
        for (list3 = history_actions; list3; list3 = g_list_next(list3)) {
          if (strcmp(gimp_action_get_name(list3->data), name) == 0) {
            is_redundant = TRUE;
            break;
          }
        }
...
static gboolean action_search_match_keyword(GimpAction *action,
  ...
  label_tokens =
      g_str_tokenize_and_fold(tmp, gimp->config->language, &label_alternates);
  g_free(tmp);
  ...
  if (!matched && g_strv_length(label_tokens) > 0) {
    ...
    if (matched && section) {
       ...
       * Finally we show at the end if the key tokens are found with a different
       * order. */
  ...
  if (!matched && key_tokens[0] && g_utf8_strlen(key_tokens[0], -1) > 2 &&
    ...
    tooltip_tokens =
        g_str_tokenize_and_fold(gimp_action_get_tooltip(action),
