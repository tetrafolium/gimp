...
static void file_open_location_response(GtkDialog *dialog, gint response_id,
                                        Gimp *gimp);
...
GtkWidget *file_open_location_dialog_new(Gimp *gimp) {
  ...
  dialog = gimp_dialog_new(
      _("Open Location"), "gimp-file-open-location", NULL, 0,
      gimp_standard_help_func, GIMP_HELP_FILE_OPEN_LOCATION,
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(file_open_location_response),
                   gimp);
  ...
  gtk_entry_completion_set_match_func(completion, file_open_location_completion,
                                      NULL, NULL);
  ...
  if (text && strlen(text)) {
    ...
    if (file) {
      ...
      image = file_open_with_proc_and_display(
          gimp, gimp_get_user_context(gimp), GIMP_PROGRESS(box), file, FALSE,
          NULL, G_OBJECT(gimp_widget_get_monitor(entry)), &status, &error);
