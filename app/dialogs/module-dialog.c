...
enum { COLUMN_NAME, COLUMN_ENABLED, COLUMN_MODULE, N_COLUMNS };
...
static void dialog_destroy_callback(GtkWidget *widget, ModuleDialog *private);
...
static void dialog_info_add(GimpModuleDB *db, GimpModule *module,
                            ModuleDialog *private);
static void dialog_info_remove(GimpModuleDB *db, GimpModule *module,
                               ModuleDialog *private);
static void dialog_info_update(GimpModuleDB *db, GimpModule *module,
                               ModuleDialog *private);
static void dialog_info_init(ModuleDialog *private, GtkWidget *grid);
...
private
  = g_slice_new0(ModuleDialog);

private
  ->gimp = gimp;

  dialog = gimp_dialog_new(_("Module Manager"), "gimp-modules", NULL, 0,
  ...
  gimp_dialog_set_alternative_button_order(
      GTK_DIALOG(dialog), GTK_RESPONSE_CLOSE, RESPONSE_REFRESH, -1);

  g_signal_connect(dialog, "response", G_CALLBACK(dialog_response), private);
...
private
  ->hint = gimp_hint_box_new(_("You will have to restart GIMP "
...
private
  ->list = gtk_list_store_new(N_COLUMNS, G_TYPE_STRING, G_TYPE_BOOLEAN,
                              GIMP_TYPE_MODULE);
  ...
  g_list_foreach(gimp_module_db_get_modules(gimp->module_db), make_list_item,
                 private);

  rend = gtk_cell_renderer_toggle_new();

  g_signal_connect(rend, "toggled", G_CALLBACK(dialog_enabled_toggled),
                   private);
  ...
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(view), 1, _("Module"), gtk_cell_renderer_text_new(), "text",
      COLUMN_NAME, NULL);
...
private
  ->error_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
...
private
  ...
  gtk_box_pack_start(GTK_BOX(private->error_box), private->error_label, TRUE,
                     TRUE, 0);
  ...
  g_signal_connect(sel, "changed", G_CALLBACK(dialog_select_callback), private);
   ...
   * appropriately.
   */
  g_signal_connect(gimp->module_db, "add", G_CALLBACK(dialog_info_add),
                   private);
  g_signal_connect(gimp->module_db, "remove", G_CALLBACK(dialog_info_remove),
                   private);
  g_signal_connect(gimp->module_db, "module-modified",
                   G_CALLBACK(dialog_info_update), private);

  g_signal_connect(dialog, "destroy", G_CALLBACK(dialog_destroy_callback),
                   private);
...
static void dialog_destroy_callback(GtkWidget *widget, ModuleDialog *private) {
  g_signal_handlers_disconnect_by_func(private->gimp->module_db,
                                       dialog_info_add, private);
  g_signal_handlers_disconnect_by_func(private->gimp->module_db,
                                       dialog_info_remove, private);
  g_signal_handlers_disconnect_by_func(private->gimp->module_db,
                                       dialog_info_update, private);
...
static void dialog_select_callback(GtkTreeSelection *sel,
  ...
  if (gtk_tree_selection_get_selected(sel, NULL, &iter)) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(private->list), &iter, COLUMN_MODULE,
                       &module, -1);
...
static void dialog_enabled_toggled(GtkCellRendererToggle *celltoggle,
  ...
  private
    ->gimp->write_modulerc = TRUE;
    gtk_widget_show(private->hint);
  }
}

static void dialog_list_item_update(ModuleDialog *private, GtkTreeIter *iter,
                                    GimpModule *module) {
  ...
  gtk_list_store_set(
      ...
      (info ? gettext(info->purpose) : gimp_file_get_utf8_name(file)),
      COLUMN_ENABLED, gimp_module_get_auto_load(module), COLUMN_MODULE, module,
      -1);
}
...
static void dialog_info_update(GimpModuleDB *db, GimpModule *module,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    gtk_tree_model_get(model, &iter, COLUMN_MODULE, &this, -1);
    if (this)
      g_object_unref(this);
  ...
  if (info) {
    ...
    text[INFO_LOCATION] =
        gimp_module_is_on_disk(module) ? location : _("Only in memory");
  } else {
    text[INFO_LOCATION] =
        gimp_module_is_on_disk(module) ? location : _("No longer available");
  }

  for (i = 0; i < N_INFOS; i++)
    gtk_label_set_text(GTK_LABEL(private->label[i]), text[i] ? text[i] : "--");
...
static void dialog_info_init(ModuleDialog *private, GtkWidget *grid) {
  ...
  const gchar *const text[] = {N_("Author:"), N_("Version:"), N_("Date:"),
                               N_("Copyright:"), N_("Location:")};
  ...
  private
    ...
    gtk_label_set_ellipsize(GTK_LABEL(private->label[i]), PANGO_ELLIPSIZE_END);
