...
file_save_dialog_confirm_overwrite(GtkWidget *dialog, Gimp *gimp);
static void file_save_dialog_response(GtkWidget *dialog, gint response_id,
                                      Gimp *gimp);
static CheckUriResult
file_save_dialog_check_file(GtkWidget *save_dialog, Gimp *gimp,
                            GFile **ret_file, gchar **ret_basename,
                            GimpPlugInProcedure **ret_save_proc);
static gboolean
file_save_dialog_no_overwrite_confirmation(GimpFileDialog *dialog, Gimp *gimp);
static GimpPlugInProcedure *
file_save_dialog_find_procedure(GimpFileDialog *dialog, GFile *file);
...
GtkWidget *file_save_dialog_new(Gimp *gimp, gboolean export) {
  ...
  g_signal_connect(dialog, "confirm-overwrite",
                   G_CALLBACK(file_save_dialog_confirm_overwrite), gimp);

  g_signal_connect(dialog, "response", G_CALLBACK(file_save_dialog_response),
                   gimp);
...
static void file_save_dialog_response(GtkWidget *dialog, gint response_id,
  ...
  switch (
      file_save_dialog_check_file(dialog, gimp, &file, &basename, &save_proc)) {
  case CHECK_URI_FAIL:
    break;
  ...
  case CHECK_URI_OK: {
    ...
    if (file_save_dialog_save_image(
            GIMP_PROGRESS(dialog), gimp, file_dialog->image, file, save_proc,
            GIMP_RUN_INTERACTIVE,
            GIMP_IS_SAVE_DIALOG(dialog) &&
                !GIMP_SAVE_DIALOG(dialog)->save_a_copy,
            FALSE, GIMP_IS_EXPORT_DIALOG(dialog), xcf_compression, FALSE)) {
      ...
      if (GIMP_IS_SAVE_DIALOG(dialog)) {
        ...
        g_object_set_data_full(G_OBJECT(file_dialog->image->gimp),
                               GIMP_FILE_SAVE_LAST_FILE_KEY, g_object_ref(file),
                               (GDestroyNotify)g_object_unref);
      } else {
        g_object_set_data_full(
            G_OBJECT(file_dialog->image->gimp), GIMP_FILE_EXPORT_LAST_FILE_KEY,
      ...
      if (GIMP_IS_SAVE_DIALOG(dialog) &&
        ...
        GimpDisplay *display =
            GIMP_DISPLAY(GIMP_SAVE_DIALOG(dialog)->display_to_close);
 ...
 */
static CheckUriResult
file_save_dialog_check_file(GtkWidget *dialog, Gimp *gimp, GFile **ret_file,
                            gchar **ret_basename,
                            GimpPlugInProcedure **ret_save_proc) {
  GimpFileDialog *file_dialog = GIMP_FILE_DIALOG(dialog);
  GFile *file;
  ...
  GIMP_LOG(SAVE_DIALOG, "selected save_proc: %s",
           save_proc ? gimp_procedure_get_label(GIMP_PROCEDURE(save_proc))
                     : "NULL");
  GIMP_LOG(SAVE_DIALOG, "URI save_proc: %s",
           uri_proc ? gimp_procedure_get_label(GIMP_PROCEDURE(uri_proc))
                    : "NULL");
  ...
  if (!basename_proc) {
    ...
    if (!strchr(basename, '.')) {
      ...
      if (!basename_proc) {
        GIMP_LOG(SAVE_DIALOG, "unable to figure save_proc, bailing out");
  ...
  if (!save_proc) {
    ...
    if (!basename_proc) {
      GIMP_LOG(SAVE_DIALOG, "basename has no useful extension, bailing out");
    ...
    if (save_proc != basename_proc) {
      ...
      if (uri_proc != basename_proc) {
        GIMP_LOG(SAVE_DIALOG, "that's impossible for remote URIs, bailing out");
        ...
        GIMP_LOG(SAVE_DIALOG, "ask the user if she really wants that filename");
...
file_save_dialog_no_overwrite_confirmation(GimpFileDialog *file_dialog,
  ...
  uri_will_change = (!basename_proc && !strchr(basename, '.') &&
                     (!save_proc || save_proc->extensions_list));

  unknown_ext = (!save_proc && !basename_proc);
...
static GimpPlugInProcedure *
file_save_dialog_find_procedure(GimpFileDialog *file_dialog, GFile *file) {
...
static gboolean file_save_other_dialog_activated(GtkWidget *label,
                                                 const gchar *uri,
                                                 GtkDialog *dialog) {
...
static gboolean file_save_dialog_switch_dialogs(GimpFileDialog *file_dialog,
  ...
  proc_in_other_group = gimp_plug_in_manager_file_procedure_find(
      gimp->plug_in_manager, other_group, file, NULL);
  ...
  if (proc_in_other_group) {
    ...
    dialog = gimp_message_dialog_new(
        _("Extension Mismatch"), GIMP_ICON_DIALOG_WARNING,
        GTK_WIDGET(file_dialog), GTK_DIALOG_DESTROY_WITH_PARENT,
    ...
    gimp_message_box_set_text(GIMP_MESSAGE_DIALOG(dialog)->box, "%s", message);
    ...
    if (GIMP_IS_EXPORT_DIALOG(file_dialog) ||
      ...
      g_signal_connect(label, "activate-link",
                       G_CALLBACK(file_save_other_dialog_activated), dialog);
    }

    gtk_dialog_set_response_sensitive(GTK_DIALOG(file_dialog),
                                      GTK_RESPONSE_CANCEL, FALSE);
    gtk_dialog_set_response_sensitive(GTK_DIALOG(file_dialog), GTK_RESPONSE_OK,
                                      FALSE);

    g_object_ref(dialog);

    if (gimp_dialog_run(GIMP_DIALOG(dialog)) ==
        FILE_SAVE_RESPONSE_OTHER_DIALOG) {
      switch_dialogs = TRUE;
    }
...
static gboolean file_save_dialog_use_extension(GtkWidget *save_dialog,
  ...
  dialog = gimp_message_dialog_new(
      _("Extension Mismatch"), GIMP_ICON_DIALOG_QUESTION, save_dialog,
      GTK_DIALOG_DESTROY_WITH_PARENT, gimp_standard_help_func, NULL,
  ...
  gtk_dialog_set_response_sensitive(GTK_DIALOG(save_dialog),
                                    GTK_RESPONSE_CANCEL, FALSE);
  gtk_dialog_set_response_sensitive(GTK_DIALOG(save_dialog), GTK_RESPONSE_OK,
                                    FALSE);
...
gboolean
file_save_dialog_save_image(GimpProgress *progress, Gimp *gimp,
  ...
  for (list = gimp_action_groups_from_name("file"); list;
  ...
  status =
      file_save(gimp, image, progress, file, save_proc, run_mode,
                change_saved_state, export_backward, export_forward, &error);
  ...
  case GIMP_PDB_CANCEL:
    if (verbose_cancel)
      gimp_message_literal(gimp, G_OBJECT(progress), GIMP_MESSAGE_INFO,
                           _("Saving canceled"));
    break;

  default: {
    gimp_message(gimp, G_OBJECT(progress), GIMP_MESSAGE_ERROR,
                 _("Saving '%s' failed:\n\n%s"), gimp_file_get_utf8_name(file),
