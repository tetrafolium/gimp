...
static void keyboard_shortcuts_dialog_response(GtkWidget *dialog, gint response,
                                               Gimp *gimp);
...
GtkWidget *keyboard_shortcuts_dialog_new(Gimp *gimp) {
  ...
  dialog = gimp_dialog_new(
      _("Configure Keyboard Shortcuts"), "gimp-keyboard-shortcuts-dialog", NULL,
      0, gimp_standard_help_func, GIMP_HELP_KEYBOARD_SHORTCUTS,

      _("_Save"), RESPONSE_SAVE, _("_Close"), GTK_RESPONSE_CLOSE,

      NULL);

  g_signal_connect(dialog, "response",
                   G_CALLBACK(keyboard_shortcuts_dialog_response), gimp);
