...
static void channel_options_dialog_callback(
...
private
  = g_slice_new0(ChannelOptionsDialog);
...
private
  ...
  dialog = item_options_dialog_new(
      image, GIMP_ITEM(channel), context, parent, title, role, icon_name, desc,
      help_id, channel_name ? _("Channel _name:") : NULL,
      GIMP_ICON_TOOL_PAINTBRUSH, _("Lock _pixels"),
      _("Lock position and _size"), channel_name, channel_visible,
      channel_linked, channel_color_tag, channel_lock_content,
      channel_lock_position, channel_options_dialog_callback, private);

  g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)channel_options_dialog_free,
                    private);

  opacity_adj =
      gtk_adjustment_new(channel_color->a * 100.0, 0.0, 100.0, 1.0, 10.0, 0);
  scale = gimp_spin_scale_new(opacity_adj, NULL, 1);
  gtk_widget_set_size_request(scale, 200, -1);
  item_options_dialog_add_widget(dialog, opacity_label, scale);

private
  ->color_panel = gimp_color_panel_new(color_label, channel_color,
                                       GIMP_COLOR_AREA_LARGE_CHECKS, 24, 24);
  gimp_color_panel_set_context(GIMP_COLOR_PANEL(private->color_panel), context);
  ...
  g_signal_connect(private->color_panel, "color-changed",
                   G_CALLBACK(channel_options_color_changed), opacity_adj);

  item_options_dialog_add_widget(dialog, NULL, private->color_panel);
  ...
  private
    ...
    item_options_dialog_add_widget(dialog, NULL, private->save_sel_toggle);
  }
...
static void channel_options_dialog_callback(
  ...
  gimp_color_button_get_color(GIMP_COLOR_BUTTON(private->color_panel), &color);

  if (private->save_sel_toggle)
    save_selection = gtk_toggle_button_get_active(
        GTK_TOGGLE_BUTTON(private->save_sel_toggle));

private
  ->callback(dialog, image, GIMP_CHANNEL(item), context, item_name, &color,
             save_selection, item_visible, item_linked, item_color_tag,
             item_lock_content, item_lock_position, private->user_data);
}
