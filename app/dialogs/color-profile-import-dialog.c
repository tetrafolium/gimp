...
GimpColorProfilePolicy color_profile_import_dialog_run(
  ...
  *dest_profile = gimp_image_get_builtin_color_profile(image);
  ...
  if (gimp_image_get_base_type(image) == GIMP_GRAY) {
    ...
    frame_title =
        _("Convert the image to the built-in grayscale color profile?");

    pref_profile = gimp_color_config_get_gray_color_profile(
        image->gimp->config->color_management, NULL);
    if (pref_profile &&
        gimp_color_profile_is_equal(pref_profile, *dest_profile))
    ...
    pref_profile = gimp_color_config_get_rgb_color_profile(
        image->gimp->config->color_management, NULL);
    if (pref_profile &&
        gimp_color_profile_is_equal(pref_profile, *dest_profile))
      g_clear_object(&pref_profile);
  ...
  dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, image), context, title,
      "gimp-image-color-profile-import", NULL,
      _("Import the image from a color profile"), parent,
      gimp_standard_help_func, GIMP_HELP_IMAGE_COLOR_PROFILE_IMPORT,

      _("_Keep"), GTK_RESPONSE_CANCEL, _("C_onvert"), GTK_RESPONSE_OK,
  ...
  if (pref_profile) {
    if (gimp_image_get_base_type(image) == GIMP_GRAY)
      frame_title =
          _("Convert the image to the preferred grayscale color profile?");
    else
  ...
  if (intent) {
    ...
    gimp_int_combo_box_set_active(GIMP_INT_COMBO_BOX(intent_combo), *intent);
  ...
  if (dont_ask)
    *dont_ask =
        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(dont_ask_toggle));
