...
static void phsh_unsea(GdkPixbuf *gp) {
...
static GdkPixbuf *get_phsh_frame(GdkPixbuf *pb, int frame) {
  GdkPixbuf *newpb;

  newpb = gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, PHSHWIDTH, PHSHHEIGHT);
  gdk_pixbuf_copy_area(pb, frame * PHSHWIDTH, 0, PHSHWIDTH, PHSHHEIGHT, newpb,
                       0, 0);
...
static void inv_show_status(void) {
  gchar *s, *t, *u, *v, *w;
  if (geginv == NULL)
    return;

  if (inv_game_over) {
    t = g_strdup_printf(_("<b>GAME OVER</b> at level %d!"), inv_level + 1);
    u = g_strdup_printf("<big>%s</big>", t);
    /* Translators: the first and third strings are similar to a
     * title, and the second string is a small information text.
     * The spaces are there only to separate all the strings, so
       try to keep them as is. */
    s = g_strdup_printf(_("%1$s   %2$s   %3$s"), u, _("Press 'q' to quit"), u);
    g_free(t);
    g_free(u);
    ...
    s = g_strdup_printf(_("%1$s\t%2$s"), t, _("Press 'p' to unpause"));
    g_free(t);
...
static gboolean inv_queue_draw_idle(gpointer data) {
  inv_draw_idle = 0;
...
static void inv_queue_draw(GtkWidget *window) {
  if (inv_draw_idle == 0)
    inv_draw_idle = g_idle_add(inv_queue_draw_idle, window);
}

static void inv_draw_explosion(int x, int y) {
  ...
  context =
      gdk_window_begin_draw_frame(gtk_widget_get_window(geginv_canvas), region);
  ...
  gdk_window_end_draw_frame(gtk_widget_get_window(geginv_canvas), context);

  inv_queue_draw(geginv);
}

static void inv_do_game_over(void) {
  GSList *li;
...
static GdkPixbuf *pb_scale(GdkPixbuf *pb, double scale) {
  int w, h;
...
static void refind_first_and_last(void) {
  int i, j;
...
static void whack_gegl(int i, int j) {
  if (!invs[i][j].live)
    return;
...
static gboolean geginv_timeout(gpointer data) {
  int i, j;
  int limitx1;
  int limitx2;
  int speed;
  ...
  if (geginv != data || inv_num <= 0 || inv_y > 700)
    return FALSE;
  ...
  for (i = 0; i < INV_COLS; i++) {
    for (j = 0; j < INV_ROWS; j++) {
      if (invs[i][j].live) {
        ...
        if (!inv_game_over && invs[i][j].y >= 570) {
          inv_do_game_over();
        } else if (!inv_game_over && invs[i][j].y >= 530 &&
...
static gboolean find_gegls(int x, int y) {
  int i, j;
  ...
  for (i = 0; i < INV_COLS; i++) {
    for (j = 0; j < INV_ROWS; j++) {
      ...
      if (y >= iy - 30 && y <= iy + 30 && x >= ix - 40 && x <= ix + 40) {
        whack_gegl(i, j);
        return TRUE;
      }
    }
...
static gboolean geginv_move_timeout(gpointer data) {
  GSList *li;
  ...
  while (li != NULL) {
    ...
    if (shot->good) {
      shot->y -= 30;
      if (find_gegls(shot->x, shot->y) || shot->y <= 0) {
        GSList *list = li;
        /* we were restarted */
        if (inv_shots == NULL)
          return TRUE;
      ...
      shot->y += 30;
      if (!inv_game_over && shot->y >= 535 && shot->y <= 565 &&
          shot->x >= inv_our_x - 25 && shot->x <= inv_our_x + 25) {
        inv_lives--;
        inv_draw_explosion(inv_our_x, 550);
        if (inv_lives <= 0) {
          inv_do_game_over();
...
static gboolean inv_key_press(GtkWidget *widget, GdkEventKey *event,
                              gpointer data) {
  switch (event->keyval) {
  case GDK_KEY_Left:
...
static gboolean inv_key_release(GtkWidget *widget, GdkEventKey *event,
                                gpointer data) {
  switch (event->keyval) {
  case GDK_KEY_Left:
  case GDK_KEY_KP_Left:
...
static gboolean ensure_creatures(void) {
  GdkPixbuf *pb, *pb1;

  if (inv_goat1 != NULL)
    return TRUE;

  pb = gdk_pixbuf_new_from_resource("/org/gimp/lebl-dialog/wanda.png", NULL);
  if (pb == NULL)
    return FALSE;
  ...
  pb = gdk_pixbuf_new_from_resource("/org/gimp/lebl-dialog/gegl-1.png", NULL);
  if (pb == NULL) {
  ...
  pb = gdk_pixbuf_new_from_resource("/org/gimp/lebl-dialog/gegl-2.png", NULL);
  if (pb == NULL) {
...
static void geginv_destroyed(GtkWidget *w, gpointer data) { geginv = NULL; }

static gboolean inv_draw(GtkWidget *widget, cairo_t *cr) {
  GdkPixbuf *goat;
  GSList *li;
  int i, j;
  ...
  for (i = 0; i < INV_COLS; i++) {
    for (j = 0; j < INV_ROWS; j++) {
      ...
      cairo_rectangle(cr, x, y, inv_goat_width, inv_goat_height);
      cairo_fill(cr);
    }
  }
  ...
  for (li = inv_shots; li != NULL; li = li->next) {
    ...
    cairo_rectangle(cr, (shot->x - 1) * inv_factor, (shot->y - 4) * inv_factor,
                    3, 8);
    cairo_fill(cr);
  }
  ...
  if (!inv_game_over) {
    ...
    cairo_rectangle(cr, inv_our_x * inv_factor - inv_phsh_width / 2,
                    550 * inv_factor - inv_phsh_height / 2, inv_phsh_width,
                    inv_phsh_height);
    cairo_fill(cr);
  }
...
gboolean gimp_lebl_dialog(void) {
  ...
  g_signal_connect(G_OBJECT(geginv), "destroy", G_CALLBACK(geginv_destroyed),
                   NULL);
  ...
  g_signal_connect(G_OBJECT(geginv_canvas), "draw", G_CALLBACK(inv_draw), NULL);
