...

{
  ...
  new_format =
      gimp_babl_format(GIMP_RGB, gimp_babl_precision(component_type, FALSE),
                       FALSE, babl_format_get_space(old_format));
...
private
  ->trc = trc;
private
  ->layer_dither_method = layer_dither_method;
private
  ->text_layer_dither_method = text_layer_dither_method;
private
  ->channel_dither_method = channel_dither_method;
private
  ->callback = callback;
private
  ->user_data = user_data;

  gimp_enum_get_value(GIMP_TYPE_COMPONENT_TYPE, component_type, NULL, NULL,
                      &enum_desc, NULL);

  blurb = g_strdup_printf(_("Convert Image to %s"), enum_desc);

  dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, image), context, _("Encoding Conversion"),
      "gimp-image-convert-precision", GIMP_ICON_CONVERT_PRECISION, blurb,
      parent, gimp_standard_help_func, GIMP_HELP_IMAGE_CONVERT_PRECISION,
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(convert_precision_dialog_response), private);
  ...
  vbox = gimp_int_radio_group_new(
      FALSE, NULL, G_CALLBACK(gimp_radio_button_update), &private->trc, NULL,
      trc,

      _("Linear light"), GIMP_TRC_LINEAR, NULL,

      _("Non-Linear"), GIMP_TRC_NON_LINEAR, NULL,

      _("Perceptual (sRGB)"), GIMP_TRC_PERCEPTUAL, &perceptual_radio,
...
static void convert_precision_dialog_response(GtkWidget *dialog,
                                              gint response_id,
                                              ConvertDialog *private) {
  if (response_id == GTK_RESPONSE_OK) {
    GimpPrecision precision =
        gimp_babl_precision(private->component_type, private->trc);

  private
    ->callback(dialog, private->image, precision, private->layer_dither_method,
               private->text_layer_dither_method,
               private->channel_dither_method, private->user_data);
  } else {
