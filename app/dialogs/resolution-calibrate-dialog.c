...
void resolution_calibrate_dialog(GtkWidget *resolution_entry,
  ...
  dialog = gimp_dialog_new(
      _("Calibrate Monitor Resolution"), "gimp-resolution-calibration",
      gtk_widget_get_toplevel(resolution_entry), GTK_DIALOG_DESTROY_WITH_PARENT,
      NULL, NULL,
  ...
  if (icon_name) {
    GtkWidget *image =
        gtk_image_new_from_icon_name(icon_name, GTK_ICON_SIZE_DIALOG);
  ...
  label = gtk_label_new(_("Measure the rulers and enter their lengths:"));
  ...
  calibrate_entry = gimp_coordinates_new(
      GIMP_UNIT_INCH, "%p", FALSE, FALSE, 10, GIMP_SIZE_ENTRY_UPDATE_SIZE,
      FALSE, FALSE, _("_Horizontal:"), ruler_width, calibrate_xres, 1,
      GIMP_MAX_IMAGE_SIZE, 0, 0, _("_Vertical:"), ruler_height, calibrate_yres,
      1, GIMP_MAX_IMAGE_SIZE, 0, 0);
  ...
  case GTK_RESPONSE_OK: {
    ...
    gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(resolution_entry), 0,
                               calibrate_xres);
    gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(resolution_entry), 1,
                               calibrate_yres);
  }
