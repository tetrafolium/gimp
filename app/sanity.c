...
static gboolean sanity_check_version(guint major_version, guint required_major,
...
#define GLIB_REQUIRED_MICRO 2
  ...
  const gchar *mismatch = glib_check_version(
  ...
  if (mismatch) {
    return g_strdup_printf(
        ...
        mismatch, GLIB_REQUIRED_MAJOR, GLIB_REQUIRED_MINOR, GLIB_REQUIRED_MICRO,
...
#define CAIRO_REQUIRED_MICRO 0
  ...
  if (cairo_version() < CAIRO_VERSION_ENCODE(CAIRO_REQUIRED_MAJOR,
    ...
    return g_strdup_printf("The Cairo version being used is too old!\n\n"
                           ...
                           CAIRO_REQUIRED_MAJOR, CAIRO_REQUIRED_MINOR,
                           CAIRO_REQUIRED_MICRO, cairo_version_string(),
                           CAIRO_REQUIRED_MAJOR, CAIRO_REQUIRED_MINOR,
                           CAIRO_REQUIRED_MICRO);
  }
...
#define PANGO_REQUIRED_MICRO 0
  ...
  const gchar *mismatch = pango_version_check(
  ...
  if (mismatch) {
    ...
    return g_strdup_printf("%s\n\n"
                           ...
                           mismatch, PANGO_REQUIRED_MAJOR, PANGO_REQUIRED_MINOR,
                           PANGO_REQUIRED_MICRO, pango_major_version,
                           pango_minor_version, pango_micro_version,
                           PANGO_REQUIRED_MAJOR, PANGO_REQUIRED_MINOR,
...
#define FC_REQUIRED_MICRO 0
  ...
  if (fc_version < ((FC_REQUIRED_MAJOR * 10000) + (FC_REQUIRED_MINOR * 100) +
                    (FC_REQUIRED_MICRO * 1))) {
...
#define FT_REQUIRED_MICRO 7
  ...
  FT_Library_Version(ft_library, &ft_major_version, &ft_minor_version,
                     &ft_micro_version);

  if (FT_Done_FreeType(ft_library) != 0)
    g_error("FT_Done_FreeType() failed");

  ft_version = (ft_major_version * 10000 + ft_minor_version * 100 +
                ft_micro_version * 1);

  if (ft_version < ((FT_REQUIRED_MAJOR * 10000) + (FT_REQUIRED_MINOR * 100) +
                    (FT_REQUIRED_MICRO * 1))) {
    return g_strdup_printf(
        ...
        ft_major_version, ft_minor_version, ft_micro_version, FT_REQUIRED_MAJOR,
        FT_REQUIRED_MINOR, FT_REQUIRED_MICRO);
  }
...
#define GDK_PIXBUF_REQUIRED_MICRO 8
  ...
  if (!sanity_check_version(gdk_pixbuf_major_version, GDK_PIXBUF_REQUIRED_MAJOR,
    ...
    return g_strdup_printf(
        ...
        GDK_PIXBUF_REQUIRED_MAJOR, GDK_PIXBUF_REQUIRED_MINOR,
        GDK_PIXBUF_REQUIRED_MICRO, gdk_pixbuf_major_version,
...
#define LCMS_REQUIRED_MINOR 8
  ...
  if (LCMS_VERSION > lcms_version) {
    return g_strdup_printf(
        ...
        LCMS_VERSION / 1000, LCMS_VERSION % 100 / 10, lcms_version / 1000,
        lcms_version % 100 / 10, LCMS_VERSION / 1000, LCMS_VERSION % 100 / 10);
  }

  if (lcms_version < (LCMS_REQUIRED_MAJOR * 1000 + LCMS_REQUIRED_MINOR * 10)) {
    ...
    return g_strdup_printf(
        ...
        LCMS_REQUIRED_MAJOR, LCMS_REQUIRED_MINOR, lcms_major_version,
        lcms_minor_version, LCMS_REQUIRED_MAJOR, LCMS_REQUIRED_MINOR);
  }
...
#define GEXIV2_REQUIRED_MICRO 6
  ...
  if (gexiv2_version < (GEXIV2_REQUIRED_MAJOR * 100 * 100 +
                        GEXIV2_REQUIRED_MINOR * 100 + GEXIV2_REQUIRED_MICRO)) {
...
#define GEGL_REQUIRED_MICRO 26
  ...
  gegl_get_version(&gegl_major_version, &gegl_minor_version,
                   &gegl_micro_version);
...
static gchar *sanity_check_gegl_ops(void) {
  static const gchar *required_ops[] = {"gegl:alien-map",
                                        "gegl:bayer-matrix",
                                        "gegl:bloom",
                                        "gegl:buffer-sink",
                                        "gegl:buffer-source",
  ...
  for (i = 0; i < G_N_ELEMENTS(required_ops); i++) {
    if (!gegl_has_operation(required_ops[i])) {
      return g_strdup_printf("GEGL operation missing!\n\n"
