...
static GString *gimp_vectors_export(GimpImage *image, GimpVectors *vectors);
static void gimp_vectors_export_image_size(GimpImage *image, GString *str);
static void gimp_vectors_export_path(GimpVectors *vectors, GString *str);
 ...
 **/
gboolean gimp_vectors_export_file(GimpImage *image, GimpVectors *vectors,
 ...
 **/
gchar *gimp_vectors_export_string(GimpImage *image, GimpVectors *vectors) {
...
static GString *gimp_vectors_export(GimpImage *image, GimpVectors *vectors) {
  ...
  if (vectors) {
    ...
    for (list = gimp_image_get_vectors_iter(image); list; list = list->next) {
...
static void gimp_vectors_export_image_size(GimpImage *image, GString *str) {
  ...
  g_string_append_printf(str, "width=\"%s%s\" height=\"%s%s\"", wbuf, abbrev,
                         hbuf, abbrev);
}
...
static gchar *gimp_vectors_export_path_data(GimpVectors *vectors) {
  ...
  for (strokes = vectors->strokes->head; strokes; strokes = strokes->next) {
    ...
    if (GIMP_IS_BEZIER_STROKE(stroke)) {
      if (control_points->len >= 3) {
        ...
        g_ascii_formatd(x_string, G_ASCII_DTOSTR_BUF_SIZE, "%.2f",
                        anchor->position.x);
        g_ascii_formatd(y_string, G_ASCII_DTOSTR_BUF_SIZE, "%.2f",
                        anchor->position.y);
      ...
      for (i = 2; i < (control_points->len + (closed ? 2 : -1)); i++) {
        ...
        anchor =
            &g_array_index(control_points, GimpAnchor, i % control_points->len);
        g_ascii_formatd(x_string, G_ASCII_DTOSTR_BUF_SIZE, "%.2f",
                        anchor->position.x);
        g_ascii_formatd(y_string, G_ASCII_DTOSTR_BUF_SIZE, "%.2f",
                        anchor->position.y);
