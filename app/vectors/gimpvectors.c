...
enum { FREEZE, THAW, LAST_SIGNAL };

static void gimp_vectors_finalize(GObject *object);

static gint64 gimp_vectors_get_memsize(GimpObject *object, gint64 *gui_size);

static gboolean gimp_vectors_is_attached(GimpItem *item);
static GimpItemTree *gimp_vectors_get_tree(GimpItem *item);
static gboolean gimp_vectors_bounds(GimpItem *item, gdouble *x, gdouble *y,
                                    gdouble *width, gdouble *height);
static GimpItem *gimp_vectors_duplicate(GimpItem *item, GType new_type);
static void gimp_vectors_convert(GimpItem *item, GimpImage *dest_image,
                                 GType old_type);
static void gimp_vectors_translate(GimpItem *item, gdouble offset_x,
                                   gdouble offset_y, gboolean push_undo);
static void gimp_vectors_scale(GimpItem *item, gint new_width, gint new_height,
                               gint new_offset_x, gint new_offset_y,
                               GimpInterpolationType interp_type,
                               GimpProgress *progress);
static void gimp_vectors_resize(GimpItem *item, GimpContext *context,
                                GimpFillType fill_type, gint new_width,
                                gint new_height, gint offset_x, gint offset_y);
static void gimp_vectors_flip(GimpItem *item, GimpContext *context,
                              GimpOrientationType flip_type, gdouble axis,
                              gboolean clip_result);
static void gimp_vectors_rotate(GimpItem *item, GimpContext *context,
                                GimpRotationType rotate_type, gdouble center_x,
                                gdouble center_y, gboolean clip_result);
static void gimp_vectors_transform(GimpItem *item, GimpContext *context,
...
gimp_vectors_get_clip(GimpItem *item, GimpTransformResize clip_result);
static gboolean gimp_vectors_fill(GimpItem *item, GimpDrawable *drawable,
                                  GimpFillOptions *fill_options,
                                  gboolean push_undo, GimpProgress *progress,
                                  GError **error);
static gboolean gimp_vectors_stroke(GimpItem *item, GimpDrawable *drawable,
                                    GimpStrokeOptions *stroke_options,
                                    gboolean push_undo, GimpProgress *progress,
                                    GError **error);
static void gimp_vectors_to_selection(GimpItem *item, GimpChannelOps op,
...
static gint gimp_vectors_real_interpolate(GimpVectors *vectors,
                                          GimpStroke *stroke, gdouble precision,
                                          gint max_points,
...
static void gimp_vectors_class_init(GimpVectorsClass *klass) {
  ...
  gimp_vectors_signals[FREEZE] =
      g_signal_new("freeze", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpVectorsClass, freeze), NULL, NULL, NULL,
                   G_TYPE_NONE, 0);

  gimp_vectors_signals[THAW] =
      g_signal_new("thaw", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpVectorsClass, thaw), NULL, NULL, NULL,
                   G_TYPE_NONE, 0);
...
static gboolean gimp_vectors_is_attached(GimpItem *item) {
  ...
  return (
      GIMP_IS_IMAGE(image) &&
      gimp_container_have(gimp_image_get_vectors(image), GIMP_OBJECT(item)));
}
...
static gboolean gimp_vectors_bounds(GimpItem *item, gdouble *x, gdouble *y,
  ...
  if (!vectors->bounds_valid) {
    ...
    for (stroke = gimp_vectors_stroke_get_next(vectors, NULL); stroke;
...
static void gimp_vectors_convert(GimpItem *item, GimpImage *dest_image,
                                 GType old_type) {
  gimp_item_set_size(item, gimp_image_get_width(dest_image),
...
static void gimp_vectors_translate(GimpItem *item, gdouble offset_x,
  ...
  if (push_undo)
    gimp_image_undo_push_vectors_mod(gimp_item_get_image(item), _("Move Path"),
                                     vectors);
...
static void gimp_vectors_scale(GimpItem *item, gint new_width, gint new_height,
  ...
  for (list = vectors->strokes->head; list; list = g_list_next(list)) {
    ...
    gimp_stroke_scale(
        stroke, (gdouble)new_width / (gdouble)gimp_item_get_width(item),
  ...
  GIMP_ITEM_CLASS(parent_class)
      ->scale(item, gimp_image_get_width(image), gimp_image_get_height(image),
...
static void gimp_vectors_resize(GimpItem *item, GimpContext *context,
  ...
  GIMP_ITEM_CLASS(parent_class)
      ->resize(item, context, fill_type, gimp_image_get_width(image),
               gimp_image_get_height(image), 0, 0);
...
static void gimp_vectors_flip(GimpItem *item, GimpContext *context,
  ...
  gimp_image_undo_push_vectors_mod(gimp_item_get_image(item), _("Flip Path"),
                                   vectors);
...
static void gimp_vectors_rotate(GimpItem *item, GimpContext *context,
  ...
  gimp_image_undo_push_vectors_mod(gimp_item_get_image(item), _("Rotate Path"),
                                   vectors);
...
static gboolean gimp_vectors_fill(GimpItem *item, GimpDrawable *drawable,
  ...
  return gimp_drawable_fill_vectors(drawable, fill_options, vectors, push_undo,
                                    error);
}
...
static gboolean gimp_vectors_stroke(GimpItem *item, GimpDrawable *drawable,
  ...
  case GIMP_STROKE_LINE:
    retval = gimp_drawable_stroke_vectors(drawable, stroke_options, vectors,
                                          push_undo, error);
    break;
  ...
  case GIMP_STROKE_PAINT_METHOD: {
    ...
    retval = gimp_paint_core_stroke_vectors(core, drawable, paint_options,
                                            emulate_dynamics, vectors,
                                            push_undo, error);
...
static void gimp_vectors_to_selection(GimpItem *item, GimpChannelOps op,
  ...
  gimp_channel_select_vectors(gimp_image_get_mask(image),
                              ...
                              vectors, op, antialias, feather, feather_radius_x,
                              feather_radius_x, TRUE);
}
...
GimpVectors *gimp_vectors_new(GimpImage *image, const gchar *name) {
  GimpVectors *vectors;

  g_return_val_if_fail(GIMP_IS_IMAGE(image), NULL);

  vectors = GIMP_VECTORS(gimp_item_new(GIMP_TYPE_VECTORS, image, name, 0, 0,
...
void gimp_vectors_add_strokes(GimpVectors *src_vectors,
  ...
  for (stroke = src_vectors->strokes->head; stroke != NULL;
    ...
    g_hash_table_insert(dest_vectors->stroke_to_list, newstroke,
                        g_queue_peek_tail_link(dest_vectors->strokes));

    dest_vectors->last_stroke_id++;
    gimp_stroke_set_id(newstroke, dest_vectors->last_stroke_id);
  }

  gimp_vectors_thaw(dest_vectors);
}

void gimp_vectors_stroke_add(GimpVectors *vectors, GimpStroke *stroke) {
  g_return_if_fail(GIMP_IS_VECTORS(vectors));
...
static void gimp_vectors_real_stroke_add(GimpVectors *vectors,
  ...
  /* Also add to {stroke: GList node} map */
  g_hash_table_insert(vectors->stroke_to_list, stroke,
...
void gimp_vectors_stroke_remove(GimpVectors *vectors, GimpStroke *stroke) {
  g_return_if_fail(GIMP_IS_VECTORS(vectors));
  g_return_if_fail(GIMP_IS_STROKE(stroke));
...
GimpStroke *gimp_vectors_stroke_get_by_id(GimpVectors *vectors, gint id) {
  GList *list;
...
void gimp_vectors_anchor_delete(GimpVectors *vectors, GimpAnchor *anchor) {
  g_return_if_fail(GIMP_IS_VECTORS(vectors));
...
void gimp_vectors_anchor_select(GimpVectors *vectors, GimpStroke *target_stroke,
                                GimpAnchor *anchor, gboolean selected,
                                gboolean exclusive) {
  GList *list;
  ...
  for (list = vectors->strokes->head; list; list = g_list_next(list)) {
    ...
    gimp_stroke_anchor_select(stroke, stroke == target_stroke ? anchor : NULL,
                              selected, exclusive);
  }
}

gdouble gimp_vectors_get_length(GimpVectors *vectors, const GimpAnchor *start) {
  g_return_val_if_fail(GIMP_IS_VECTORS(vectors), 0.0);
...
gint gimp_vectors_interpolate(GimpVectors *vectors, GimpStroke *stroke,
                              gdouble precision, gint max_points,
                              GimpCoords *ret_coords) {
  g_return_val_if_fail(GIMP_IS_VECTORS(vectors), 0);

  return GIMP_VECTORS_GET_CLASS(vectors)->interpolate(
      vectors, stroke, precision, max_points, ret_coords);
}
...
static GimpBezierDesc *gimp_vectors_real_make_bezier(GimpVectors *vectors) {
  ...
  for (stroke = gimp_vectors_stroke_get_next(vectors, NULL); stroke;
    ...
    if (bezdesc) {
      cmd_array =
          g_array_append_vals(cmd_array, bezdesc->data, bezdesc->num_data);
