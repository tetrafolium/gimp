...
static void gimp_stroke_warp_point(GimpStroke *stroke, gdouble x, gdouble y,
                                   GimpCoords *point_warped, gdouble y_offset,
                                   gdouble x_len);

static void gimp_vectors_warp_stroke(GimpVectors *vectors, GimpStroke *stroke,
                                     gdouble y_offset);

void gimp_vectors_warp_point(GimpVectors *vectors, GimpCoords *point,
                             GimpCoords *point_warped, gdouble y_offset) {
  gdouble x = point->x;
  gdouble y = point->y;
  gdouble len;
...
static void gimp_stroke_warp_point(GimpStroke *stroke, gdouble x, gdouble y,
  ...
  if (x + DX >= x_len) {
    ...
    if (!gimp_stroke_get_point_at_dist(stroke, x_len, EPSILON, &point_zero,
                                       &slope)) {
      point_warped->x = 0;
      point_warped->y = 0;
      return;
  ...
  if (!gimp_stroke_get_point_at_dist(stroke, x - DX, EPSILON, &point_minus,
                                     &slope))
    return;

  if (!gimp_stroke_get_point_at_dist(stroke, x + DX, EPSILON, &point_plus,
                                     &slope))
    return;
...
static void gimp_vectors_warp_stroke(GimpVectors *vectors, GimpStroke *stroke,
  ...
  for (list = stroke->anchors->head; list; list = g_list_next(list)) {
    ...
    gimp_vectors_warp_point(vectors, &anchor->position, &anchor->position,
                            y_offset);
  }
}

void gimp_vectors_warp_vectors(GimpVectors *vectors, GimpVectors *vectors_in,
                               gdouble y_offset) {
  GList *list;

  for (list = vectors_in->strokes->head; list; list = g_list_next(list)) {
