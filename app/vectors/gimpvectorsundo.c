...
enum { PROP_0, PROP_PREV_PARENT, PROP_PREV_POSITION, PROP_PREV_VECTORS };

static void gimp_vectors_undo_constructed(GObject *object);
static void gimp_vectors_undo_finalize(GObject *object);
static void gimp_vectors_undo_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_vectors_undo_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);

static gint64 gimp_vectors_undo_get_memsize(GimpObject *object,
                                            gint64 *gui_size);

static void gimp_vectors_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
...
static void gimp_vectors_undo_class_init(GimpVectorsUndoClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("prev-parent", NULL, NULL, GIMP_TYPE_VECTORS,
  ...
  g_object_class_install_property(
      object_class, PROP_PREV_VECTORS,
      g_param_spec_pointer("prev-vectors", NULL, NULL,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_vectors_undo_init(GimpVectorsUndo *undo) {
...
static void gimp_vectors_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
  ...
  if ((undo_mode == GIMP_UNDO_MODE_UNDO &&
    ...
    vectors_undo->prev_vectors =
        g_list_copy(gimp_image_get_selected_vectors(undo->image));

    gimp_image_add_vectors(undo->image, vectors, vectors_undo->prev_parent,
