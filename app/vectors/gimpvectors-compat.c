    ...
    if (points[i].type == GIMP_VECTORS_COMPAT_NEW_STROKE) {
      ...
      *curr_stroke = *(curr_coord - 1);
      ...
      stroke = gimp_bezier_stroke_new_from_coords(
          curr_stroke, curr_coord - curr_stroke - 1, TRUE);
...
gboolean gimp_vectors_compat_is_compatible(GimpImage *image) {
  ...
  for (list = gimp_image_get_vectors_iter(image); list;
    ...
    for (strokes = vectors->strokes->head; strokes;
...
GimpVectorsCompatPoint *gimp_vectors_compat_get_points(GimpVectors *vectors,
  ...
  *closed = TRUE;
  ...
  for (strokes = vectors->strokes->head; strokes || postponed;
       strokes = g_list_next(strokes)) {
    GimpStroke *stroke;
    GList *anchors;
    ...
    for (anchors = stroke->anchors->head; anchors;
