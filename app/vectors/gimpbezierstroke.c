...
static gdouble gimp_bezier_stroke_nearest_point_get(
    GimpStroke *stroke, const GimpCoords *coord, gdouble precision,
    GimpCoords *ret_point, GimpAnchor **ret_segment_start,
    GimpAnchor **ret_segment_end, gdouble *ret_pos);
static gdouble gimp_bezier_stroke_segment_nearest_point_get(
    const GimpCoords *beziercoords, const GimpCoords *coord, gdouble precision,
    GimpCoords *ret_point, gdouble *ret_pos, gint depth);
static gdouble gimp_bezier_stroke_nearest_tangent_get(
    GimpStroke *stroke, const GimpCoords *coord1, const GimpCoords *coord2,
    gdouble precision, GimpCoords *nearest, GimpAnchor **ret_segment_start,
    GimpAnchor **ret_segment_end, gdouble *ret_pos);
static gdouble gimp_bezier_stroke_segment_nearest_tangent_get(
    const GimpCoords *beziercoords, const GimpCoords *coord1,
    const GimpCoords *coord2, gdouble precision, GimpCoords *ret_point,
    gdouble *ret_pos);
static void
gimp_bezier_stroke_anchor_move_relative(GimpStroke *stroke, GimpAnchor *anchor,
                                        const GimpCoords *deltacoord,
                                        GimpAnchorFeatureType feature);
static void
gimp_bezier_stroke_anchor_move_absolute(GimpStroke *stroke, GimpAnchor *anchor,
...
static void gimp_bezier_stroke_anchor_delete(GimpStroke *stroke,
                                             GimpAnchor *anchor);
static gboolean gimp_bezier_stroke_point_is_movable(GimpStroke *stroke,
                                                    GimpAnchor *predec,
                                                    gdouble position);
static void gimp_bezier_stroke_point_move_relative(
    GimpStroke *stroke, GimpAnchor *predec, gdouble position,
    const GimpCoords *deltacoord, GimpAnchorFeatureType feature);
static void gimp_bezier_stroke_point_move_absolute(
    GimpStroke *stroke, GimpAnchor *predec, gdouble position,
    const GimpCoords *coord, GimpAnchorFeatureType feature);
...
static gboolean gimp_bezier_stroke_anchor_is_insertable(GimpStroke *stroke,
                                                        GimpAnchor *predec,
                                                        gdouble position);
...
static gboolean gimp_bezier_stroke_connect_stroke(GimpStroke *stroke,
                                                  ...
                                                  GimpAnchor *neighbor);
static gboolean gimp_bezier_stroke_reverse(GimpStroke *stroke);
static gboolean gimp_bezier_stroke_shift_start(GimpStroke *stroke,
                                               GimpAnchor *anchor);
static GArray *gimp_bezier_stroke_interpolate(GimpStroke *stroke,
                                              gdouble precision,
                                              gboolean *closed);
static GimpBezierDesc *gimp_bezier_stroke_make_bezier(GimpStroke *stroke);
...
static void gimp_bezier_stroke_init(GimpBezierStroke *stroke) {}
...
GimpStroke *gimp_bezier_stroke_new_from_coords(const GimpCoords *coords,
  ...
  for (count = 0; count < n_coords; count++)
    last_anchor = gimp_bezier_stroke_extend(stroke, &coords[count], last_anchor,
                                            EXTEND_SIMPLE);
...
static GimpAnchor *gimp_bezier_stroke_anchor_insert(GimpStroke *stroke,
  ...
  gimp_coords_mix(1 - position, &(beziercoords[0]), position,
                  &(beziercoords[1]), &(subdivided[1]));

  gimp_coords_mix(1 - position, &(beziercoords[1]), position,
                  &(beziercoords[2]), &(subdivided[7]));

  gimp_coords_mix(1 - position, &(beziercoords[2]), position,
                  &(beziercoords[3]), &(subdivided[5]));

  gimp_coords_mix(1 - position, &(subdivided[1]), position, &(subdivided[7]),
                  &(subdivided[2]));

  gimp_coords_mix(1 - position, &(subdivided[7]), position, &(subdivided[5]),
                  &(subdivided[4]));

  gimp_coords_mix(1 - position, &(subdivided[2]), position, &(subdivided[4]),
                  &(subdivided[3]));
  ...
  for (i = 0; i <= 6; i++) {
    if (i >= 2 && i <= 4) {
      list2 = g_list_append(
          NULL,
          gimp_anchor_new((i == 3 ? GIMP_ANCHOR_ANCHOR : GIMP_ANCHOR_CONTROL),
                          &(subdivided[i])));
...
static void gimp_bezier_stroke_point_move_relative(
  ...
  gimp_coords_scale((1 - feel_good) /
                        (3 * position * (1 - position) * (1 - position)),
                    deltacoord, &(offsetcoords[0]));
  gimp_coords_scale(feel_good / (3 * position * position * (1 - position)),
                    deltacoord, &(offsetcoords[1]));
...
static void gimp_bezier_stroke_point_move_absolute(
  ...
  gimp_coords_mix((1 - position) * (1 - position) * (1 - position),
                  &(beziercoords[0]),
                  3 * (1 - position) * (1 - position) * position,
                  &(beziercoords[1]), &tmp1);
  gimp_coords_mix(3 * (1 - position) * position * position, &(beziercoords[2]),
                  position * position * position, &(beziercoords[3]), &tmp2);
  gimp_coords_add(&tmp1, &tmp2, &abs_pos);

  gimp_coords_difference(coord, &abs_pos, &deltacoord);

  gimp_bezier_stroke_point_move_relative(stroke, predec, position, &deltacoord,
                                         feature);
}
...
static gdouble gimp_bezier_stroke_nearest_point_get(
  ...
  for (; anchorlist; anchorlist = g_list_next(anchorlist)) {
    ...
    if (count == 4) {
      ...
      dist = gimp_bezier_stroke_segment_nearest_point_get(
          segmentcoords, coord, precision, &point, &pos, 10);
...
static gdouble gimp_bezier_stroke_segment_nearest_point_get(
  ...
  if (!depth || (gimp_coords_bezier_is_straight(beziercoords, precision) &&
    ...
    gimp_coords_difference(&(beziercoords[3]), &(beziercoords[0]), &line);

    gimp_coords_difference(coord, &(beziercoords[0]), &dcoord);
    ...
    *ret_pos = pos1;
    ...
    gimp_coords_mix(1.0, &(beziercoords[0]), scalar, &line, ret_point);
  ...
  gimp_coords_average(&(beziercoords[0]), &(beziercoords[1]), &(subdivided[1]));

  gimp_coords_average(&(beziercoords[1]), &(beziercoords[2]), &(subdivided[7]));

  gimp_coords_average(&(beziercoords[2]), &(beziercoords[3]), &(subdivided[5]));
  ...
  dist1 = gimp_bezier_stroke_segment_nearest_point_get(
      &(subdivided[0]), coord, precision, &point1, &pos1, depth - 1);

  dist2 = gimp_bezier_stroke_segment_nearest_point_get(
      &(subdivided[3]), coord, precision, &point2, &pos2, depth - 1);
...
static gdouble gimp_bezier_stroke_nearest_tangent_get(
  ...
  for (; anchorlist; anchorlist = g_list_next(anchorlist)) {
    ...
    if (count == 4) {
      segment_end = anchorlist->data;
      dist = gimp_bezier_stroke_segment_nearest_tangent_get(
          segmentcoords, coord1, coord2, precision, &point, &pos);
...
static gdouble gimp_bezier_stroke_segment_nearest_tangent_get(
  ...
  g_printerr("(%.2f, %.2f)-(%.2f,%.2f): ", coord1->x, coord1->y, coord2->x,
             coord2->y);

  gimp_coords_interpolate_bezier(beziercoords, precision, ret_coords,
                                 ret_params);
  ...
  gimp_coords_difference(&g_array_index(ret_coords, GimpCoords, 1),
                         &g_array_index(ret_coords, GimpCoords, 0), &dir);
  ori = dir.x * line.y - dir.y * line.x;

  for (i = 2; i < ret_coords->len; i++) {
    gimp_coords_difference(&g_array_index(ret_coords, GimpCoords, i),
                           &g_array_index(ret_coords, GimpCoords, i - 1), &dir);
    ori2 = dir.x * line.y - dir.y * line.x;

    if (ori * ori2 <= 0) {
      gimp_coords_difference(&g_array_index(ret_coords, GimpCoords, i), coord1,
                             &dcoord);
      ...
      if (scalar >= 0 && scalar <= 1) {
        gimp_coords_mix(1.0, coord1, scalar, &line, &min_point);
        gimp_coords_difference(
            &min_point, &g_array_index(ret_coords, GimpCoords, i), &dcoord);
...
static gboolean gimp_bezier_stroke_is_extendable(GimpStroke *stroke,
  ...
  if (listneighbor->data == neighbor) {
    ...
    if (listneighbor->data == neighbor) {
      ...
      if (listneighbor && neighbor->type == GIMP_ANCHOR_CONTROL) {
        if (listneighbor->prev &&
          ...
          listneighbor = listneighbor->prev;
                   ...
                   GIMP_ANCHOR(listneighbor->next->data)->type ==
                       GIMP_ANCHOR_ANCHOR) {
       ...
       */
      {
        if (listneighbor->prev && listneighbor->prev->prev == NULL) {
          loose_end = -1;
        } else if (listneighbor->next && listneighbor->next->next == NULL) {
          loose_end = 1;
        }
      }
    }
...
GimpAnchor *gimp_bezier_stroke_extend(GimpStroke *stroke,
  ...
  if (g_queue_is_empty(stroke->anchors)) {
    ...
    case EXTEND_EDITABLE:
      anchor = gimp_bezier_stroke_extend(stroke, coords, anchor, EXTEND_SIMPLE);

      /* we return the GIMP_ANCHOR_ANCHOR */
      gimp_bezier_stroke_extend(stroke, coords, anchor, EXTEND_SIMPLE);
    ...
    if (listneighbor->data == neighbor) {
      ...
      if (listneighbor->data == neighbor) {
        ...
        if (listneighbor && neighbor->type == GIMP_ANCHOR_CONTROL) {
          if (listneighbor->prev &&
              GIMP_ANCHOR(listneighbor->prev->data)->type ==
                  GIMP_ANCHOR_ANCHOR) {
            listneighbor = listneighbor->prev;
          } else if (listneighbor->next &&
                     GIMP_ANCHOR(listneighbor->next->data)->type ==
         ...
         */
        {
          if (listneighbor->next && listneighbor->next->next == NULL) {
            loose_end = 1;
            listneighbor = listneighbor->next;
          } else if (listneighbor->prev && listneighbor->prev->prev == NULL) {
            loose_end = -1;
            listneighbor = listneighbor->prev;
          }
        }
    ...
    if (loose_end) {
      ...
      case EXTEND_EDITABLE:
        switch (control_count) {
        case 0:
          neighbor = gimp_bezier_stroke_extend(stroke, &(neighbor->position),
                                               neighbor, EXTEND_SIMPLE);
        case 1:
          neighbor = gimp_bezier_stroke_extend(stroke, coords, neighbor,
                                               EXTEND_SIMPLE);
        case 2:
          anchor = gimp_bezier_stroke_extend(stroke, coords, neighbor,
                                             EXTEND_SIMPLE);
    ...
    g_return_val_if_fail((list1->prev && list1->prev->prev == NULL &&
                          list2->next && list2->next->next == NULL) ||
                             (list1->next && list1->next->next == NULL &&
                              list2->prev && list2->prev->prev == NULL),
                         FALSE);
    gimp_stroke_close(stroke);
    return TRUE;
  }
  ...
  if (anchor->type == GIMP_ANCHOR_ANCHOR) {
    ...
    if (feature == GIMP_ANCHOR_FEATURE_SYMMETRIC) {
      ...
      if (opposite &&
        ...
        gimp_coords_add(&(GIMP_ANCHOR(neighbour->data)->position), &delta,
                        &coord1);
...
gimp_bezier_stroke_anchor_move_absolute(GimpStroke *stroke, GimpAnchor *anchor,
  ...
  gimp_bezier_stroke_anchor_move_relative(stroke, anchor, &deltacoord, feature);
}
...
static void gimp_bezier_stroke_anchor_convert(GimpStroke *stroke,
  ...
  case GIMP_ANCHOR_FEATURE_EDGE:
    if (anchor->type == GIMP_ANCHOR_ANCHOR) {
      ...
      if (g_list_previous(anchor_list) &&
          GIMP_ANCHOR(g_list_previous(anchor_list)->data)->type ==
              GIMP_ANCHOR_ANCHOR)
        anchor->position =
            GIMP_ANCHOR(g_list_previous(anchor_list)->data)->position;
      if (g_list_next(anchor_list) &&
          GIMP_ANCHOR(g_list_next(anchor_list)->data)->type ==
              GIMP_ANCHOR_ANCHOR)
        anchor->position =
            GIMP_ANCHOR(g_list_next(anchor_list)->data)->position;
    }
...
static GimpBezierDesc *gimp_bezier_stroke_make_bezier(GimpStroke *stroke) {
  ...
  cmd_array =
      g_array_sized_new(FALSE, FALSE, sizeof(cairo_path_data_t), num_cmds);
...
static GArray *gimp_bezier_stroke_interpolate(GimpStroke *stroke,
  ...
  for (; anchorlist; anchorlist = g_list_next(anchorlist)) {
    ...
    if (count == 4) {
      gimp_coords_interpolate_bezier(segmentcoords, precision, ret_coords,
                                     NULL);
      segmentcoords[0] = segmentcoords[3];
      count = 1;
      need_endpoint = TRUE;
  ...
  if (stroke->closed && !g_queue_is_empty(stroke->anchors)) {
    ...
    gimp_coords_interpolate_bezier(segmentcoords, precision, ret_coords, NULL);
    need_endpoint = TRUE;
  }
...
static void gimp_bezier_stroke_transform(GimpStroke *stroke,
  ...
  if (!ret_strokes || gimp_matrix3_is_affine(matrix) ||
  ...
  for (; anchorlist || stroke->closed; anchorlist = g_list_next(anchorlist)) {
    ...
    if (count == 4) {
      ...
      gimp_transform_bezier_coords(matrix, segmentcoords, transformed,
                                   &n_transformed, &start_in, &end_in);
      ...
      for (i = 0; i < n_transformed; i++) {
        ...
        if (i == 0 && start_in) {
          ...
          g_queue_push_tail(
              last_stroke->anchors,
              gimp_anchor_new(GIMP_ANCHOR_CONTROL, &anchor->position));
        }
        ...
        for (list = transformed[i]->head; list; list = g_list_next(list)) {
          ...
          if (!s) {
            ...
            g_queue_push_tail(
                s->anchors,
                gimp_anchor_new(GIMP_ANCHOR_CONTROL, &transformedcoords[0]));
          ...
          for (; j < 4; j++) {
            ...
            g_queue_push_tail(s->anchors,
                              gimp_anchor_new(type, &transformedcoords[j]));
          }
...
GimpStroke *gimp_bezier_stroke_new_moveto(const GimpCoords *start) {
  ...
  g_queue_push_tail(stroke->anchors,
                    gimp_anchor_new(GIMP_ANCHOR_CONTROL, start));
  g_queue_push_tail(stroke->anchors,
                    gimp_anchor_new(GIMP_ANCHOR_ANCHOR, start));
  g_queue_push_tail(stroke->anchors,
                    gimp_anchor_new(GIMP_ANCHOR_CONTROL, start));
  return stroke;
}

void gimp_bezier_stroke_lineto(GimpStroke *stroke, const GimpCoords *end) {
  ...
  g_queue_push_tail(stroke->anchors, gimp_anchor_new(GIMP_ANCHOR_CONTROL, end));
  g_queue_push_tail(stroke->anchors, gimp_anchor_new(GIMP_ANCHOR_ANCHOR, end));
  g_queue_push_tail(stroke->anchors, gimp_anchor_new(GIMP_ANCHOR_CONTROL, end));
}

void gimp_bezier_stroke_conicto(GimpStroke *stroke, const GimpCoords *control,
  ...
  g_queue_push_tail(stroke->anchors,
                    gimp_anchor_new(GIMP_ANCHOR_CONTROL, &coords));
  g_queue_push_tail(stroke->anchors, gimp_anchor_new(GIMP_ANCHOR_ANCHOR, end));
  g_queue_push_tail(stroke->anchors, gimp_anchor_new(GIMP_ANCHOR_CONTROL, end));
}

void gimp_bezier_stroke_cubicto(GimpStroke *stroke, const GimpCoords *control1,
  ...
  g_queue_push_tail(stroke->anchors,
                    gimp_anchor_new(GIMP_ANCHOR_CONTROL, control2));
  g_queue_push_tail(stroke->anchors, gimp_anchor_new(GIMP_ANCHOR_ANCHOR, end));
  g_queue_push_tail(stroke->anchors, gimp_anchor_new(GIMP_ANCHOR_CONTROL, end));
}

static gdouble arcto_circleparam(gdouble h, gdouble *y) {
  gdouble t0 = 0.5;
  gdouble dt = 0.25;
  gdouble pt0;
  ...
  while (dt >= 0.00001) {
    pt0 = (y[0] * (1 - t0) * (1 - t0) * (1 - t0) +
           3 * y[1] * (1 - t0) * (1 - t0) * t0 + 3 * y[2] * (1 - t0) * t0 * t0 +
           y[3] * t0 * t0 * t0);
  ...
  y[0] = y0123;
  y[1] = y123;
  y[2] = y23; /* y[3] unchanged */

  return t0;
}

static void arcto_subdivide(gdouble t, gint part, GimpCoords *p) {
...
static void arcto_ellipsesegment(gdouble radius_x, gdouble radius_y,
                                 gdouble phi0, gdouble phi1,
                                 GimpCoords *ellips) {
  ...
  ellips[0].x = cos(phi_s);
  ellips[0].y = sin(phi_s);
  ellips[3].x = cos(phi_e);
  ellips[3].y = sin(phi_e);
...
void gimp_bezier_stroke_arcto(GimpStroke *bez_stroke, gdouble radius_x,
                              gdouble radius_y, gdouble angle_rad,
  ...
  gimp_matrix3_transform_point(&anglerot, trans_delta.x, trans_delta.y, &tmpx,
                               &tmpy);
  trans_delta.x = tmpx;
  trans_delta.y = tmpy;

  lambda =
      (SQR(trans_delta.x) / SQR(radius_x) + SQR(trans_delta.y) / SQR(radius_y));
  ...
  gimp_matrix3_transform_point(&anglerot, tmp_center.x, tmp_center.y, &tmpx,
                               &tmpy);
  tmp_center.x = tmpx;
  tmp_center.y = tmpy;
  ...
  if (sweep) {
    ...
    while (phi0 < phi2) {
      arcto_ellipsesegment(radius_x, radius_y, MAX(phi0, phi1),
                           MIN(phi0 + G_PI_2, phi2), ellips);

      gimp_matrix3_transform_point(&anglerot, ellips[0].x, ellips[0].y, &tmpx,
                                   &tmpy);
      ellips[0].x = tmpx;
      ellips[0].y = tmpy;
      gimp_matrix3_transform_point(&anglerot, ellips[1].x, ellips[1].y, &tmpx,
                                   &tmpy);
      ellips[1].x = tmpx;
      ellips[1].y = tmpy;
      gimp_matrix3_transform_point(&anglerot, ellips[2].x, ellips[2].y, &tmpx,
                                   &tmpy);
      ellips[2].x = tmpx;
      ellips[2].y = tmpy;
      gimp_matrix3_transform_point(&anglerot, ellips[3].x, ellips[3].y, &tmpx,
                                   &tmpy);
      ellips[3].x = tmpx;
      ellips[3].y = tmpy;
      ...
      gimp_bezier_stroke_cubicto(bez_stroke, &(ctrl[1]), &(ctrl[2]),
                                 &(ctrl[3]));
      phi0 += G_PI_2;
    }
  } else {
    ...
    while (phi0 > phi2) {
      arcto_ellipsesegment(radius_x, radius_y, MIN(phi0, phi1),
                           MAX(phi0 - G_PI_2, phi2), ellips);
