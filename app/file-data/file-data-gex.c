...
static int file_gex_open_callback(struct archive *a, void *client_data);
static la_ssize_t file_gex_read_callback(struct archive *a, void *client_data,
                                         const void **buffer);
static int file_gex_close_callback(struct archive *a, void *client_data);

static gboolean file_gex_validate_path(const gchar *path,
                                       const gchar *file_name, gboolean first,
                                       gchar **plugin_id, GError **error);
static gboolean file_gex_validate(GFile *file, AsApp **appstream,
                                  GError **error);
static gchar *file_gex_decompress(GFile *file, gchar *plugin_id,
                                  GError **error);
...
static la_ssize_t file_gex_read_callback(struct archive *a, void *client_data,
  ...
  read_count =
      g_input_stream_read(data->input, data->buffer, 2048, NULL, &error);
...
static gboolean file_gex_validate_path(const gchar *path,
  ...
  if (g_path_is_absolute(path) || g_strcmp0(dirname, "/") == 0) {
    *error =
        g_error_new(GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
      ...
      if (r == ARCHIVE_OK) {
        ...
        while (archive_read_next_header(a, &entry) == ARCHIVE_OK &&
               file_gex_validate_path(archive_entry_pathname(entry),
                                      gimp_file_get_utf8_name(file), TRUE,
                                      &plugin_id, error)) {
          if (plugin_id && !appdata_path)
            appdata_path =
                g_strdup_printf("%s/%s.metainfo.xml", plugin_id, plugin_id);
          ...
          if (appdata_path) {
            if (g_strcmp0(appdata_path, archive_entry_pathname(entry)) == 0) {
              ...
              while (TRUE) {
                ...
                if (r == ARCHIVE_FATAL) {
                  *error = g_error_new(
                      ...
                      _("Fatal error when uncompressing GIMP extension '%s': "
                        "%s"),
                      gimp_file_get_utf8_name(file), archive_error_string(a));
                  break;
                ...
                appstring =
                    g_string_append_len(appstring, (const gchar *)buffer, size);
              }
              continue;
            }
          }
            ...
            *appstream = as_app_new();
            ...
            if (!as_app_parse_data(*appstream, appdata,
              ...
              *error = g_error_new(GIMP_EXTENSION_ERROR, GIMP_EXTENSION_FAILED,
                                   _("GIMP extension '%s' directory (%s) "
                                     "different from AppStream id: %s"),
                                   gimp_file_get_utf8_name(file), plugin_id,
                                   as_app_get_id(*appstream));
              g_clear_object(appstream);
            }
          } else {
            *error = g_error_new(
                ...
                gimp_file_get_utf8_name(file), appdata_path);
          }
        }
        if (appdata_path)
          g_free(appdata_path);
    ...
    if ((a = archive_read_new())) {
      ...
      if (r == ARCHIVE_OK) {
        while (archive_read_next_header(a, &entry) == ARCHIVE_OK &&
          ...
          path = g_build_filename(g_file_get_path(ext_dir),
          ...
          if (r < ARCHIVE_OK) {
            *error = g_error_new(
                ...
                gimp_file_get_utf8_name(file), archive_error_string(ext));
            break;
          }
...
GimpValueArray *file_gex_load_invoker(GimpProcedure *procedure, Gimp *gimp,
  ...
  if (ext_dir) {
    ...
    success = gimp_extension_manager_install(gimp->extension_manager, extension,
                                             error);
