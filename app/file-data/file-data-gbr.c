...
static GimpImage *file_gbr_brush_to_image(Gimp *gimp, GimpBrush *brush);
...
GimpLayer *file_gbr_brush_to_layer(GimpImage *image, GimpBrush *brush) {
  ...
  if (width > image_width || height > image_height) {
    ...
    gimp_image_resize(image, gimp_get_user_context(image->gimp), new_width,
                      new_height, (new_width - image_width) / 2,
                      (new_height - image_height) / 2, NULL);
  ...
  layer =
      gimp_layer_new(image, width, height, format, gimp_object_get_name(brush),
                     1.0, GIMP_LAYER_MODE_NORMAL);

  gimp_item_set_offset(GIMP_ITEM(layer), (image_width - width) / 2,
  ...
  if (pixmap) {
    ...
    gegl_buffer_set(buffer, GEGL_RECTANGLE(0, 0, width, height), 0,
                    babl_format("R'G'B' u8"), gimp_temp_buf_get_data(pixmap),
                    GEGL_AUTO_ROWSTRIDE);

    pixmap_data = gegl_buffer_linear_open(buffer, NULL, NULL, NULL);
    mask_data = gimp_temp_buf_get_data(mask);

    for (i = 0, p = pixmap_data, m = mask_data; i < width * height;
         i++, p += 4, m += 1) {
      p[3] = *m;
    }
    ...
    gegl_buffer_set(buffer, GEGL_RECTANGLE(0, 0, width, height), 0,
                    babl_format("Y' u8"), mask_data, GEGL_AUTO_ROWSTRIDE);
  }
...
GimpBrush *file_gbr_drawable_to_brush(GimpDrawable *drawable,
  ...
  brush = g_object_new(GIMP_TYPE_BRUSH, "name", name, "mime-type",
                       "image/x-gimp-gbr", "spacing", spacing, NULL);
  ...
  if (gimp_drawable_is_gray(drawable)) {
    ...
    if (gimp_drawable_has_alpha(drawable)) {
      ...
      iter = gegl_buffer_iterator_new(buffer, rect, 0, babl_format("Y'A u8"),
      ...
      while (gegl_buffer_iterator_next(iter)) {
        ...
        for (j = 0; j < iter->length; j++) {
          ...
          gimp_rgba_set_uchar(&gray, data[0], data[0], data[0], data[1]);
      ...
      gegl_buffer_get(buffer, rect, 1.0, babl_format("Y' u8"), m,
    ...
    gegl_buffer_get(buffer, rect, 1.0, babl_format("A u8"),
...
static GimpImage *file_gbr_brush_to_image(Gimp *gimp, GimpBrush *brush) {
  ...
  parasite =
      gimp_parasite_new("GimpProcedureConfig-file-gbr-save-last",
                        GIMP_PARASITE_PERSISTENT, string->len + 1, string->str);
...
static GimpBrush *file_gbr_image_to_brush(GimpImage *image,
  ...
  return file_gbr_drawable_to_brush(
      drawable, GEGL_RECTANGLE(0, 0, width, height), name, spacing);
}
