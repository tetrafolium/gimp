...
static GimpImage *file_gih_pipe_to_image(Gimp *gimp, GimpBrushPipe *pipe);
...
static GimpImage *file_gih_pipe_to_image(Gimp *gimp, GimpBrushPipe *pipe) {
  ...
  image = gimp_image_new(gimp, 1, 1, base_type, GIMP_PRECISION_U8_NON_LINEAR);

  parasite = gimp_parasite_new("gimp-brush-pipe-name", GIMP_PARASITE_PERSISTENT,
                               strlen(name) + 1, name);
...
static GimpBrushPipe *file_gih_image_to_pipe(GimpImage *image,
  ...
  pipe = g_object_new(GIMP_TYPE_BRUSH_PIPE, "name", name, "mime-type",
                      "image/x-gimp-gih", "spacing", spacing, NULL);
  ...
  for (list = layers; list; list = g_list_next(list)) {
    ...
    for (row = 0; row < params.rows; row++) {
      ...
      for (col = 0; col < params.cols; col++) {
        ...
        brush = file_gbr_drawable_to_brush(
            GIMP_DRAWABLE(layer), GEGL_RECTANGLE(thisx, thisy, thisw, thish),
            gimp_object_get_name(layer), spacing);
