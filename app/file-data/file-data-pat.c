...
static GimpImage *file_pat_pattern_to_image(Gimp *gimp, GimpPattern *pattern);
...
GimpValueArray *file_pat_load_invoker(GimpProcedure *procedure, Gimp *gimp,
                                      GimpContext *context,
...
GimpValueArray *file_pat_save_invoker(GimpProcedure *procedure, Gimp *gimp,
                                      GimpContext *context,
                                      GimpProgress *progress,
                                      const GimpValueArray *args,
                                      GError **error) {
...
static GimpImage *file_pat_pattern_to_image(Gimp *gimp, GimpPattern *pattern) {
  ...
  parasite =
      gimp_parasite_new("GimpProcedureConfig-file-pat-save-last",
  ...
  layer = gimp_layer_new(image, width, height, format,
                         gimp_object_get_name(pattern), 1.0,
...
static GimpPattern *file_pat_image_to_pattern(GimpImage *image,
  ...
  format = gimp_babl_format(
      gimp_drawable_is_gray(drawable) ? GIMP_GRAY : GIMP_RGB,
      GIMP_PRECISION_U8_NON_LINEAR, gimp_drawable_has_alpha(drawable), NULL);

  width = gimp_item_get_width(GIMP_ITEM(drawable));
  height = gimp_item_get_height(GIMP_ITEM(drawable));

  pattern = g_object_new(GIMP_TYPE_PATTERN, "name", name, "mime-type",
                         "image/x-gimp-pat", NULL);

  pattern->mask = gimp_temp_buf_new(width, height, format);

  gegl_buffer_get(gimp_drawable_get_buffer(drawable),
                  GEGL_RECTANGLE(0, 0, width, height), 1.0, format,
                  gimp_temp_buf_get_data(pattern->mask), GEGL_AUTO_ROWSTRIDE,
                  GEGL_ABYSS_NONE);
