...
static void gimp_config_connect_notify(GObject *src, GParamSpec *param_spec,
                                       GObject *dest) {
  if (param_spec->flags & G_PARAM_READABLE) {
    ...
    if (dest_spec && (dest_spec->value_type == param_spec->value_type) &&
 ...
 **/
void gimp_config_connect(GObject *a, GObject *b, const gchar *property_name) {
  gchar *signal_name;
  ...
  g_signal_connect_object(a, signal_name,
                          G_CALLBACK(gimp_config_connect_notify), b, 0);
  g_signal_connect_object(b, signal_name,
                          G_CALLBACK(gimp_config_connect_notify), a, 0);

  if (property_name)
    g_free(signal_name);
}

static void gimp_config_connect_full_notify(GObject *src,
  ...
  if (param_spec->flags & G_PARAM_READABLE) {
    ...
    if (dest_spec && (dest_spec->value_type == param_spec->value_type) &&
      ...
      g_signal_handlers_block_by_func(dest, gimp_config_connect_full_notify,
                                      src);
      g_object_set_property(dest, dest_prop_name, &value);
      g_signal_handlers_unblock_by_func(dest, gimp_config_connect_full_notify,
                                        src);
 ...
 * are of the same value_type are propagated.
 **/
void gimp_config_connect_full(GObject *a, GObject *b,
  ...
  g_signal_connect_object(a, signal_name,
                          G_CALLBACK(gimp_config_connect_full_notify), b, 0);
  ...
  g_signal_connect_object(b, signal_name,
                          G_CALLBACK(gimp_config_connect_full_notify), a, 0);
 ...
 **/
void gimp_config_disconnect(GObject *a, GObject *b) {
  g_return_if_fail(G_IS_OBJECT(a) && G_IS_OBJECT(b));

  g_signal_handlers_disconnect_by_func(
      b, G_CALLBACK(gimp_config_connect_notify), a);
  g_signal_handlers_disconnect_by_func(
      a, G_CALLBACK(gimp_config_connect_notify), b);
}
