...
gboolean gimp_config_file_copy(const gchar *source, const gchar *dest,
  ...
  while ((nbytes = fread(buffer + unwritten_len, 1,
                         sizeof(buffer) - unwritten_len, sfile)) > 0 ||
         unwritten_len) {
    ...
    if (old_options_regexp && update_callback) {
      ...
      if (eol) {
        ...
        *eol++ = '\n';
        ...
        g_set_error(error, GIMP_CONFIG_ERROR, GIMP_CONFIG_ERROR_PARSE, format,
      ...
      write_bytes = g_regex_replace_eval(old_options_regexp, buffer, read_len,
                                         0, 0, update_callback, NULL, error);
      if (write_bytes == NULL) {
        /* error already set. */
        fclose(sfile);
        fclose(dfile);
    ...
    if (fwrite(write_bytes, 1, write_len, dfile) < write_len) {
      g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno),
                  _("Error writing '%s': %s"), gimp_filename_to_utf8(dest),
                  g_strerror(errno));
      if (old_options_regexp && update_callback) {
        g_free(write_bytes);
        g_regex_unref(old_options_regexp);
  ...
  if (ferror(sfile)) {
    g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(errno),
                _("Error reading '%s': %s"), gimp_filename_to_utf8(source),
                g_strerror(errno));
    fclose(sfile);
    fclose(dfile);
    if (old_options_regexp)
