...
static gboolean parse_encoding(const gchar *text, gint text_len,
                               gchar **encodind);
...
gboolean gimp_xml_parser_parse_buffer(GimpXmlParser *parser,
  ...
  if (parse_encoding(buffer, len, &encoding) && encoding) {
    if (g_ascii_strcasecmp(encoding, "UTF-8") &&
      ...
      conv = g_convert(buffer, len, "UTF-8", encoding, NULL, &written, error);
      if (!conv) {
        g_free(encoding);
        return FALSE;
      }
  ...
  success = g_markup_parse_context_parse(parser->context, conv ? conv : buffer,
                                         len, error);
