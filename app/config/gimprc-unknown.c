 ...
 **/
void gimp_rc_add_unknown_token(GimpConfig *config, const gchar *key,
                               const gchar *value) {
  ...
  unknown_tokens =
      (GSList *)g_object_get_data(G_OBJECT(config), GIMP_RC_UNKNOWN_TOKENS);

  for (last = NULL, list = unknown_tokens; list;
    ...
    if (strcmp(token->key, key) == 0) {
      ...
      if (value) {
        ...
        g_object_set_data_full(G_OBJECT(config), GIMP_RC_UNKNOWN_TOKENS,
                               unknown_tokens,
...
const gchar *gimp_rc_lookup_unknown_token(GimpConfig *config,
  ...
  unknown_tokens = g_object_get_data(G_OBJECT(config), GIMP_RC_UNKNOWN_TOKENS);
