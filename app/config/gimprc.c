...
enum { PROP_0, PROP_VERBOSE, PROP_SYSTEM_GIMPRC, PROP_USER_GIMPRC };
...
static void gimp_rc_set_property(GObject *object, guint property_id,
                                 const GValue *value, GParamSpec *pspec);
static void gimp_rc_get_property(GObject *object, guint property_id,
                                 GValue *value, GParamSpec *pspec);

static GimpConfig *gimp_rc_duplicate(GimpConfig *object);
static gboolean gimp_rc_idle_save(GimpRc *rc);
static void gimp_rc_notify(GimpRc *rc, GParamSpec *param, gpointer data);
...
static void gimp_rc_duplicate_unknown_token(const gchar *key,
                                            const gchar *value,
                                            gpointer user_data) {
  gimp_rc_add_unknown_token(GIMP_CONFIG(user_data), key, value);
}

static GimpConfig *gimp_rc_duplicate(GimpConfig *config) {
  GObject *gimp;
  GimpConfig *dup;

  g_object_get(config, "gimp", &gimp, NULL);

  dup = g_object_new(GIMP_TYPE_RC, "gimp", gimp, NULL);
  ...
  gimp_rc_foreach_unknown_token(config, gimp_rc_duplicate_unknown_token, dup);
 ...
 */
GimpRc *gimp_rc_new(GObject *gimp, GFile *system_gimprc, GFile *user_gimprc,
                    gboolean verbose) {
  GimpRc *rc;
  ...
  rc = g_object_new(GIMP_TYPE_RC, "gimp", gimp, "verbose", verbose,
                    "system-gimprc", system_gimprc, "user-gimprc", user_gimprc,
                    NULL);
...
void gimp_rc_set_autosave(GimpRc *rc, gboolean autosave) {
  g_return_if_fail(GIMP_IS_RC(rc));
  ...
  if (autosave)
    g_signal_connect(rc, "notify", G_CALLBACK(gimp_rc_notify), NULL);
  else
 ...
 **/
gchar *gimp_rc_query(GimpRc *rc, const gchar *key) {
  GObjectClass *klass;
  GObject *rc_object;
  ...
  if (!retval) {
    const gchar *const path_tokens[] = {"gimp_dir", "gimp_data_dir",
                                        "gimp_plug_in_dir", "gimp_plugin_dir",
                                        "gimp_sysconf_dir"};
 ...
 **/
void gimp_rc_set_unknown_token(GimpRc *rc, const gchar *token,
                               const gchar *value) {
  g_return_if_fail(GIMP_IS_RC(rc));
...
void gimp_rc_save(GimpRc *rc) {
  ...
  const gchar *footer = "end of gimprc";

  g_return_if_fail(GIMP_IS_RC(rc));

  g_object_get(rc, "gimp", &gimp, NULL);

  global = g_object_new(GIMP_TYPE_RC, "gimp", gimp, NULL);

  if (gimp)
    g_object_unref(gimp);

  gimp_config_deserialize_file(GIMP_CONFIG(global), rc->system_gimprc, NULL,
                               NULL);

  header = g_strconcat(top, gimp_file_get_utf8_name(rc->system_gimprc), bottom,
                       NULL);

  if (rc->verbose)
    g_print("Writing '%s'\n", gimp_file_get_utf8_name(rc->user_gimprc));

  if (!gimp_config_serialize_to_file(GIMP_CONFIG(rc), rc->user_gimprc, header,
                                     footer, global, &error)) {
    g_message("%s", error->message);
    g_error_free(error);
  }
