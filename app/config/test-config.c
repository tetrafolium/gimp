...
static void notify_callback(GObject *object, GParamSpec *pspec);
static void output_unknown_token(const gchar *key, const gchar *value,
                                 gpointer data);
...
int main(int argc, char *argv[]) {
  ...
  if (!gimp_config_serialize_to_file(grid, file, "foorc", "end of foorc", NULL,
                                     &error)) {
    g_print("%s\n", error->message);
    return EXIT_FAILURE;
  }
  ...
  g_signal_connect(grid, "notify", G_CALLBACK(notify_callback), NULL);
  ...
  g_signal_connect(grid2, "notify", G_CALLBACK(notify_callback), NULL);

  g_print(" Changing a property in the duplicate ...");
  g_object_set(grid2, "xspacing", 20.0, NULL);

  g_print(" Creating a diff between the two ...");
  for (list = gimp_config_diff(G_OBJECT(grid), G_OBJECT(grid2), 0); list;
       list = list->next) {
  ...
  if (!gimp_config_deserialize_file(grid, g_file_new_for_path("gimpconfig.c"),
                                    NULL, &error)) {
...
static const gchar *unit_get_identifier(GimpUnit unit) {
  switch (unit) {
...
static gint unit_get_number_of_units(void) { return GIMP_UNIT_END; }
