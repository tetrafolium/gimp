...
static void gimp_gegl_config_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_gegl_config_get_property(GObject *object, guint property_id,
...
static void gimp_gegl_config_class_init(GimpGeglConfigClass *klass) {
  ...
  GIMP_CONFIG_PROP_PATH(object_class, PROP_TEMP_PATH, "temp-path", "Temp path",
                        TEMP_PATH_BLURB, GIMP_CONFIG_PATH_DIR,
                        "${gimp_temp_dir}",
                        GIMP_PARAM_STATIC_STRINGS | GIMP_CONFIG_PARAM_RESTART);

  GIMP_CONFIG_PROP_PATH(object_class, PROP_SWAP_PATH, "swap-path", "Swap path",
                        SWAP_PATH_BLURB, GIMP_CONFIG_PATH_DIR,
                        "${gimp_cache_dir}",
                        GIMP_PARAM_STATIC_STRINGS | GIMP_CONFIG_PARAM_RESTART);

  GIMP_CONFIG_PROP_STRING(object_class, PROP_SWAP_COMPRESSION,
                          "swap-compression", "Swap compression",
                          SWAP_COMPRESSION_BLURB, GIMP_DEFAULT_SWAP_COMPRESSION,
                          GIMP_PARAM_STATIC_STRINGS);

  n_threads = g_get_num_processors();

  max_n_threads =
      G_PARAM_SPEC_INT(g_object_class_find_property(
                           G_OBJECT_GET_CLASS(gegl_config()), "threads"))
          ->maximum;

  n_threads = MIN(n_threads, max_n_threads);

  GIMP_CONFIG_PROP_INT(object_class, PROP_NUM_PROCESSORS, "num-processors",
                       "Number of threads to use", NUM_PROCESSORS_BLURB, 1,
                       max_n_threads, n_threads, GIMP_PARAM_STATIC_STRINGS);
  ...
  GIMP_CONFIG_PROP_MEMSIZE(
      object_class, PROP_TILE_CACHE_SIZE, "tile-cache-size", "Tile cache size",
      TILE_CACHE_SIZE_BLURB, 0, GIMP_MAX_MEM_PROCESS, memory_size,
      GIMP_PARAM_STATIC_STRINGS | GIMP_CONFIG_PARAM_CONFIRM);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_OPENCL, "use-opencl",
                           "Use OpenCL", USE_OPENCL_BLURB, FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  /*  only for backward compatibility:  */
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_STINGY_MEMORY_USE,
                           "stingy-memory-use", NULL, NULL, FALSE,
