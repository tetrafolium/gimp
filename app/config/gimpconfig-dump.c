...
static void dump_gimprc_system(GimpConfig *rc, GimpConfigWriter *writer,
                               GOutputStream *output);
static void dump_gimprc_manpage(GimpConfig *rc, GimpConfigWriter *writer,
                                GOutputStream *output);
static gchar *dump_describe_param(GParamSpec *param_spec);
static void dump_with_linebreaks(GOutputStream *output, const gchar *text);
...
gboolean gimp_config_dump(GObject *gimp, GimpConfigDumpFormat format) {
  ...
  rc = g_object_new(GIMP_TYPE_RC, "gimp", gimp, NULL);
...
static const gchar *man_page_path =
    ...
    ".TP\n"
    ".I gimp_plug_in_dir\n"
    "Base to paths for architecture-specific plug-ins and modules, which is "
    "set\n"
    ...
    "variable GIMP3_CACHEDIR or to the system default for per-user cached "
    "files.\n"
    ".TP\n"
    ".I gimp_temp_dir\n"
...
static void dump_gimprc_manpage(GimpConfig *rc, GimpConfigWriter *writer,
  ...
  g_output_stream_printf(output, NULL, NULL, NULL, "%s", man_page_header);
...
static const gchar display_format_description[] =
    ...
    "%B  expands to (modified) if the image is dirty, the empty string "
    "otherwise\n"
...
static gchar *dump_describe_param(GParamSpec *param_spec) {
  ...
  if (GIMP_IS_PARAM_SPEC_RGB(param_spec)) {
    if (gimp_param_spec_rgb_has_alpha(param_spec))
      ...
      values = "The color is specified in the form (color-rgb red green blue) "
...
static void dump_with_linebreaks(GOutputStream *output, const gchar *text) {
  ...
  while (len > 0) {
    ...
    for (t = text, i = 0, space = 0;
         *t != '\n' && (i <= LINE_LENGTH || space == 0) && i < len; t++, i++) {
