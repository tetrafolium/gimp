...
static void gimp_plugin_config_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_plugin_config_get_property(GObject *object, guint property_id,
...
static void gimp_plugin_config_class_init(GimpPluginConfigClass *klass) {
  ...
  GIMP_CONFIG_PROP_PATH(object_class, PROP_FRACTALEXPLORER_PATH,
                        "fractalexplorer-path", "Fractal Explorer path",
                        FRACTALEXPLORER_PATH_BLURB, GIMP_CONFIG_PATH_DIR_LIST,
                        path, GIMP_PARAM_STATIC_STRINGS);
  g_free(path);

  path = gimp_config_build_data_path("gfig");
  GIMP_CONFIG_PROP_PATH(object_class, PROP_GFIG_PATH, "gfig-path", "GFig path",
                        GFIG_PATH_BLURB, GIMP_CONFIG_PATH_DIR_LIST, path,
                        GIMP_PARAM_STATIC_STRINGS);
  g_free(path);

  path = gimp_config_build_data_path("gflare");
  GIMP_CONFIG_PROP_PATH(object_class, PROP_GFLARE_PATH, "gflare-path",
  ...
  GIMP_CONFIG_PROP_PATH(object_class, PROP_GIMPRESSIONIST_PATH,
                        "gimpressionist-path", "GIMPressionist path",
                        GIMPRESSIONIST_PATH_BLURB, GIMP_CONFIG_PATH_DIR_LIST,
                        path, GIMP_PARAM_STATIC_STRINGS);
  g_free(path);

  path = gimp_config_build_data_path("scripts");
  GIMP_CONFIG_PROP_PATH(object_class, PROP_SCRIPT_FU_PATH, "script-fu-path",
                        "Script-Fu path", SCRIPT_FU_PATH_BLURB,
                        GIMP_CONFIG_PATH_DIR_LIST, path,
                        GIMP_PARAM_STATIC_STRINGS);
  g_free(path);
}

static void gimp_plugin_config_init(GimpPluginConfig *config) {}
