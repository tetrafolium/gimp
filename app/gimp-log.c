...
static const gchar *const log_domains[] = {"Gimp",
                                           "Gimp-Actions",
                                           "Gimp-Base",
                                           "Gimp-Composite",
                                           "Gimp-Config",
...
void gimp_log_init(void) {
  ...
  if (env_log_val) {
    ...
    if (g_ascii_strcasecmp(env_log_val, "list-all") == 0)
      gimp_log_flags =
          g_parse_debug_string("help", log_keys, G_N_ELEMENTS(log_keys));
    else if (g_ascii_strcasecmp(env_log_val, "help") == 0)
      gimp_log_flags = GIMP_LOG_HELP;
    else
      gimp_log_flags =
          g_parse_debug_string(env_log_val, log_keys, G_N_ELEMENTS(log_keys));
...
void gimp_log(GimpLogFlags flags, const gchar *function, gint line,
              const gchar *format, ...) {
  va_list args;
...
void gimp_logv(GimpLogFlags flags, const gchar *function, gint line,
  ...
  g_log(domain, G_LOG_LEVEL_DEBUG, "%s(%d): %s", function, line, message);

  g_free(message);
}

GimpLogHandler gimp_log_set_handler(gboolean global, GLogLevelFlags log_levels,
  ...
  for (i = 0; i < n; i++) {
    handler[i + 1] =
        g_log_set_handler(log_domains[i], log_levels, log_func, user_data);
  }
