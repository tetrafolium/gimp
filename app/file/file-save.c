...
GimpPDBStatusType file_save(Gimp *gimp, GimpImage *image,
  ...
  g_return_val_if_fail((export_backward && export_forward) == FALSE,
                       GIMP_PDB_CALLING_ERROR);
  g_return_val_if_fail(error == NULL || *error == NULL, GIMP_PDB_CALLING_ERROR);
  ...
  if (!file_proc->handles_remote || !mounted) {
    ...
    if (!my_path) {
      local_file =
          file_remote_upload_image_prepare(gimp, file, progress, &my_error);
  ...
  return_vals = gimp_pdb_execute_procedure_by_name(
      image->gimp->pdb, gimp_get_user_context(gimp), progress, error,
      gimp_object_get_name(file_proc), GIMP_TYPE_RUN_MODE, run_mode,
      GIMP_TYPE_IMAGE, image, G_TYPE_INT, n_drawables, GIMP_TYPE_OBJECT_ARRAY,
      drawables, G_TYPE_FILE, file, G_TYPE_NONE);
  ...
  if (status == GIMP_PDB_SUCCESS) {
    ...
    imagefile = gimp_document_list_add_file(
        documents, orig_file, g_slist_nth_data(file_proc->mime_types_list, 0));
    ...
    if (GIMP_PROCEDURE(file_proc)->proc_type == GIMP_PDB_PROC_TYPE_INTERNAL)
      gimp_imagefile_save_thumbnail(
          imagefile, g_slist_nth_data(file_proc->mime_types_list, 0), image,
          NULL);
