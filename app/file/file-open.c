...
static void file_open_sanitize_image(GimpImage *image, gboolean as_new);
static void file_open_convert_items(GimpImage *dest_image,
                                    const gchar *basename, GList *items);
static GList *file_open_get_layers(GimpImage *image, gboolean merge_visible,
                                   gint *n_visible);
static gboolean file_open_file_proc_is_import(GimpPlugInProcedure *file_proc);

/*  public functions  */

GimpImage *file_open_image(Gimp *gimp, GimpContext *context,
                           GimpProgress *progress, GFile *file, gboolean as_new,
                           GimpPlugInProcedure *file_proc, GimpRunMode run_mode,
                           GimpPDBStatusType *status, const gchar **mime_type,
                           GError **error) {
  ...
  *status = GIMP_PDB_EXECUTION_ERROR;
  ...
  if (!file_proc)
    file_proc = gimp_plug_in_manager_file_procedure_find(
        gimp->plug_in_manager, GIMP_FILE_PROCEDURE_GROUP_OPEN, file, error);
  ...
  if (!file_proc || !file_proc->handles_remote || !mounted) {
    ...
    if (!my_path) {
      ...
      local_file = file_remote_download_image(gimp, file, progress, &my_error);
      ...
      if (!file_proc)
        file_proc = gimp_plug_in_manager_file_procedure_find(
            gimp->plug_in_manager, GIMP_FILE_PROCEDURE_GROUP_OPEN, local_file,
            error);
  ...
  return_vals = gimp_pdb_execute_procedure_by_name(
      gimp->pdb, context, progress, error, gimp_object_get_name(file_proc),
      GIMP_TYPE_RUN_MODE, run_mode, G_TYPE_FILE, file, G_TYPE_NONE);
  ...
  if (image) {
    ...
    if (file_open_file_proc_is_import(file_proc)) {
      file_import_image(image, context, orig_file,
                        run_mode == GIMP_RUN_INTERACTIVE, progress);
    }
 ...
 */
GimpImage *file_open_thumbnail(Gimp *gimp, GimpContext *context,
                               GimpProgress *progress, GFile *file, gint size,
                               const gchar **mime_type, gint *image_width,
                               gint *image_height, const Babl **format,
                               gint *num_layers, GError **error) {
  ...
  *num_layers = -1;
  ...
  file_proc = gimp_plug_in_manager_file_procedure_find(
      gimp->plug_in_manager, GIMP_FILE_PROCEDURE_GROUP_OPEN, file, NULL);
  ...
  if (procedure && procedure->num_args >= 2 && procedure->num_values >= 1) {
    ...
    return_vals = gimp_pdb_execute_procedure_by_name(
        gimp->pdb, context, progress, error, gimp_object_get_name(procedure),
        G_TYPE_STRING, uri, G_TYPE_INT, size, G_TYPE_NONE);
      ...
      if (gimp_value_array_length(return_vals) >= 3 &&
        ...
        *image_height =
        ...
        if (gimp_value_array_length(return_vals) >= 5 &&
          ...
          switch (value) {
          case GIMP_RGB_IMAGE:
            *format = gimp_babl_format(GIMP_RGB, GIMP_PRECISION_U8_NON_LINEAR,
                                       FALSE, NULL);
            break;
          ...
          case GIMP_GRAY_IMAGE:
            *format = gimp_babl_format(GIMP_GRAY, GIMP_PRECISION_U8_NON_LINEAR,
                                       FALSE, NULL);
            break;
          ...
          case GIMP_INDEXEDA_IMAGE: {
            ...
            babl_new_palette("-gimp-indexed-format-dummy", &rgb, &rgba);
...
GimpImage *file_open_with_display(Gimp *gimp, GimpContext *context,
                                  GimpProgress *progress, GFile *file,
                                  gboolean as_new, GObject *monitor,
                                  GimpPDBStatusType *status, GError **error) {
  return file_open_with_proc_and_display(gimp, context, progress, file, as_new,
                                         NULL, monitor, status, error);
}

GimpImage *file_open_with_proc_and_display(
    Gimp *gimp, GimpContext *context, GimpProgress *progress, GFile *file,
    gboolean as_new, GimpPlugInProcedure *file_proc, GObject *monitor,
    GimpPDBStatusType *status, GError **error) {
  GimpImage *image;
  ...
  image = file_open_image(gimp, context, progress, file, as_new, file_proc,
                          GIMP_RUN_INTERACTIVE, status, &mime_type, error);
    ...
    if (!as_new) {
      ...
      if (any_file && g_file_equal(file, any_file)) {
        ...
        if (!gimp_imagefile_check_thumbnail(imagefile)) {
          gimp_imagefile_save_thumbnail(imagefile, mime_type, image, NULL);
        }
      }
    }
...
GList *file_open_layers(Gimp *gimp, GimpContext *context,
                        GimpProgress *progress, GimpImage *dest_image,
                        gboolean merge_visible, GFile *file,
                        GimpRunMode run_mode, GimpPlugInProcedure *file_proc,
                        GimpPDBStatusType *status, GError **error) {
  GimpImage *new_image;
  GList *layers = NULL;
  const gchar *mime_type = NULL;
  ...
  new_image = file_open_image(gimp, context, progress, file, FALSE, file_proc,
                              run_mode, status, &mime_type, error);
  ...
  if (new_image) {
    ...
    if (merge_visible && n_visible > 1) {
      ...
      layers = gimp_image_merge_visible_layers(
          new_image, context, GIMP_CLIP_TO_IMAGE, FALSE, FALSE, NULL);
    ...
    if (layers) {
      ...
      gimp_document_list_add_file(GIMP_DOCUMENT_LIST(gimp->documents), file,
                                  mime_type);
    } else {
      g_set_error_literal(error, G_FILE_ERROR, G_FILE_ERROR_FAILED,
 ...
 */
gboolean file_open_from_command_line(Gimp *gimp, GFile *file, gboolean as_new,
                                     GObject *monitor)

{
  ...
  image = file_open_with_display(gimp, gimp_get_user_context(gimp),
                                 GIMP_PROGRESS(display), file, as_new, monitor,
                                 &status, &error);
  ...
  if (image) {
    ...
    gimp_message(gimp, G_OBJECT(display), GIMP_MESSAGE_ERROR,
                 _("Opening '%s' failed: %s"), gimp_file_get_utf8_name(file),
                 error->message);
    g_clear_error(&error);
  }
...
static void file_open_convert_items(GimpImage *dest_image,
  ...
  for (list = items; list; list = g_list_next(list)) {
    ...
    if (g_list_length(items) == 1) {
      ...
      gimp_object_set_name(GIMP_OBJECT(item), gimp_object_get_name(src));
    }
...
static gboolean file_open_file_proc_is_import(GimpPlugInProcedure *file_proc) {
  return !(file_proc && file_proc->mime_types &&
