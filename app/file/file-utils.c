...
static gboolean file_utils_filename_is_uri(const gchar *filename,
                                           GError **error) {
  ...
  if (strstr(filename, "://")) {
    ...
    if (strcmp(scheme, canon) || !g_ascii_isgraph(canon[0])) {
      g_set_error(error, G_FILE_ERROR, 0, _("'%s:' is not a valid URI scheme"),
                  scheme);
...
GFile *file_utils_filename_to_file(Gimp *gimp, const gchar *filename,
                                   GError **error) {
  GFile *file;
  gchar *absolute;
...
GdkPixbuf *file_utils_load_thumbnail(GFile *file) {
  ...
  pixbuf = gimp_thumbnail_load_thumb(
      thumbnail, (GimpThumbSize)GIMP_THUMBNAIL_SIZE_NORMAL, NULL);
  ...
  if (pixbuf) {
    ...
    if (gdk_pixbuf_get_n_channels(pixbuf) != 3) {
      ...
      gdk_pixbuf_composite_color(pixbuf, tmp, 0, 0, width, height, 0, 0, 1.0,
                                 1.0, GDK_INTERP_NEAREST, 255, 0, 0,
                                 GIMP_CHECK_SIZE_SM, 0x66666666, 0x99999999);
...
gboolean file_utils_save_thumbnail(GimpImage *image, GFile *file) {
  GFile *image_file;
  ...
  if (image_file) {
    ...
    if (uri && image_uri && !strcmp(uri, image_uri)) {
      ...
      success = gimp_imagefile_save_thumbnail(imagefile, NULL, image, NULL);
