...
typedef enum { DOWNLOAD, UPLOAD } RemoteCopyMode;
...
static void file_remote_mount_volume_ready(GFile *file, GAsyncResult *result,
                                           RemoteMount *mount);
static void file_remote_mount_file_cancel(GimpProgress *progress,
                                          RemoteMount *mount);

static GFile *file_remote_get_temp_file(Gimp *gimp, GFile *file);
static gboolean file_remote_copy_file(Gimp *gimp, GFile *src_file,
                                      GFile *dest_file, RemoteCopyMode mode,
...
gboolean file_remote_mount_file(Gimp *gimp, GFile *file, GimpProgress *progress,
                                GError **error) {
  ...
  if (progress) {
    ...
    g_signal_connect(progress, "cancel",
                     G_CALLBACK(file_remote_mount_file_cancel), &mount);
  }

  g_file_mount_enclosing_volume(
      file, G_MOUNT_MOUNT_NONE, operation, mount.cancellable,
      (GAsyncReadyCallback)file_remote_mount_volume_ready, &mount);
  ...
  if (progress) {
    g_signal_handlers_disconnect_by_func(progress,
                                         file_remote_mount_file_cancel, &mount);
...
GFile *file_remote_download_image(Gimp *gimp, GFile *file,
                                  GimpProgress *progress, GError **error) {
  GFile *local_file;
  ...
  if (!file_remote_copy_file(gimp, file, local_file, DOWNLOAD, progress,
                             error)) {
...
GFile *file_remote_upload_image_prepare(Gimp *gimp, GFile *file,
                                        GimpProgress *progress,
                                        GError **error) {
  GFile *local_file;
...
gboolean file_remote_upload_image_finish(Gimp *gimp, GFile *file,
                                         GFile *local_file,
                                         GimpProgress *progress,
                                         GError **error) {
  g_return_val_if_fail(GIMP_IS_GIMP(gimp), FALSE);
  ...
  if (!file_remote_copy_file(gimp, local_file, file, UPLOAD, progress, error)) {
    return FALSE;
  }
...
static gboolean file_remote_copy_file(Gimp *gimp, GFile *src_file,
  ...
  if (progress) {
    ...
    success = g_file_copy(
        src_file, dest_file, G_FILE_COPY_OVERWRITE, remote_progress.cancellable,
        file_remote_progress_callback, &remote_progress, &my_error);

    g_signal_handlers_disconnect_by_func(progress, file_remote_copy_file_cancel,
                                         &remote_progress);
...
static void file_remote_progress_callback(goffset current_num_bytes,
  ...
  if (total_num_bytes > 0) {
    ...
    gimp_progress_set_value(progress->progress, (gdouble)current_num_bytes /
