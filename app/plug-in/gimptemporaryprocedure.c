...
gimp_temporary_procedure_execute(GimpProcedure *procedure, Gimp *gimp,
                                 GimpContext *context, GimpProgress *progress,
                                 GimpValueArray *args, GError **error);
static void gimp_temporary_procedure_execute_async(
...
static void gimp_temporary_procedure_init(GimpTemporaryProcedure *proc) {
...
gimp_temporary_procedure_execute(GimpProcedure *procedure, Gimp *gimp,
                                 GimpContext *context, GimpProgress *progress,
                                 GimpValueArray *args, GError **error) {
  return gimp_plug_in_manager_call_run_temp(
      gimp->plug_in_manager, context, progress,
      GIMP_TEMPORARY_PROCEDURE(procedure), args);
}
...
static void gimp_temporary_procedure_execute_async(
  ...
  return_vals = gimp_plug_in_manager_call_run_temp(
      gimp->plug_in_manager, context, progress, temp_procedure, args);

  if (return_vals) {
    GimpPlugInProcedure *proc = GIMP_PLUG_IN_PROCEDURE(procedure);

    gimp_plug_in_procedure_handle_return_values(proc, gimp, progress,
                                                return_vals);
