...
static gboolean match_string(GRegex *regex, gchar *string) {
...
gint gimp_plug_in_manager_query(GimpPlugInManager *manager,
  ...
  *time_ints = NULL;
  ...
  if (search_str) {
    sregex =
        g_regex_new(search_str, G_REGEX_CASELESS | G_REGEX_OPTIMIZE, 0, NULL);
    if (!sregex)
      return 0;
  }
  ...
  *time_ints = g_new(gint, num_plugins);
  ...
  for (list = matched, i = 0; list; list = g_slist_next(list), i++) {
    GimpPlugInProcedure *proc = list->data;
