...
static GimpPlugInProcedure *file_proc_find(GSList *procs, GFile *file,
                                           GError **error);
static GimpPlugInProcedure *file_proc_find_by_prefix(GSList *procs, GFile *file,
                                                     gboolean skip_magic);
static GimpPlugInProcedure *
file_proc_find_by_extension(GSList *procs, GFile *file, gboolean skip_magic);
static GimpPlugInProcedure *file_proc_find_by_mime_type(GSList *procs,
                                                        const gchar *mime_type);
static GimpPlugInProcedure *file_proc_find_by_name(GSList *procs, GFile *file,
                                                   gboolean skip_magic);

static void file_convert_string(const gchar *instr, gchar *outmem, gint maxmem,
                                gint *nmem);
static FileMatchType
file_check_single_magic(const gchar *offset, const gchar *type,
                        const gchar *value, const guchar *file_head,
                        gint headsize, GFile *file, GInputStream *input);
static FileMatchType file_check_magic_list(GSList *magics_list,
                                           const guchar *head, gint headsize,
...
GimpPlugInProcedure *gimp_plug_in_manager_file_procedure_find_by_prefix(
    GimpPlugInManager *manager, GimpFileProcedureGroup group, GFile *file) {
  g_return_val_if_fail(GIMP_IS_PLUG_IN_MANAGER(manager), NULL);
  g_return_val_if_fail(G_IS_FILE(file), NULL);
...
GimpPlugInProcedure *gimp_plug_in_manager_file_procedure_find_by_extension(
    GimpPlugInManager *manager, GimpFileProcedureGroup group, GFile *file) {
  g_return_val_if_fail(GIMP_IS_PLUG_IN_MANAGER(manager), NULL);
  g_return_val_if_fail(G_IS_FILE(file), NULL);
...
GimpPlugInProcedure *gimp_plug_in_manager_file_procedure_find_by_mime_type(
    GimpPlugInManager *manager, GimpFileProcedureGroup group,
    const gchar *mime_type) {
  g_return_val_if_fail(GIMP_IS_PLUG_IN_MANAGER(manager), NULL);
...
static gboolean file_proc_in_group(GimpPlugInProcedure *file_proc,
  ...
  is_filter =
      (strcmp(name, "file-gz-save") == 0 ||
       strcmp(name, "file-bz2-save") == 0 || strcmp(name, "file-xz-save") == 0);
...
static GimpPlugInProcedure *file_proc_find(GSList *procs, GFile *file,
  ...
  if (g_file_is_native(file)) {
    ...
    for (list = procs; list; list = g_slist_next(list)) {
      ...
      if (file_proc->magics_list) {
        if (G_UNLIKELY(!opened)) {
          ...
          if (input) {
            g_input_stream_read_all(input, head, sizeof(head), &head_size, NULL,
                                    error);
        ...
        if (head_size >= 4) {
          ...
          match_val = file_check_magic_list(file_proc->magics_list, head,
                                            head_size, file, input);
          ...
          if (match_val == FILE_MATCH_SIZE) {
            ...
            GIMP_LOG(MAGIC_MATCH, "magic match %d on %s\n", match_val,
    ...
    if (best_file_proc) {
      GIMP_LOG(MAGIC_MATCH, "best magic match on %s\n",
...
static GimpPlugInProcedure *
file_proc_find_by_mime_type(GSList *procs, const gchar *mime_type) {
  GSList *list;
...
static GimpPlugInProcedure *file_proc_find_by_prefix(GSList *procs, GFile *file,
  ...
  for (p = procs; p; p = g_slist_next(p)) {
    ...
    for (prefixes = proc->prefixes_list; prefixes;
...
file_proc_find_by_extension(GSList *procs, GFile *file, gboolean skip_magic) {
  ...
  if (ext) {
    ...
    for (p = procs; p; p = g_slist_next(p)) {
      ...
      if (g_slist_find_custom(proc->extensions_list, ext + 1,
  ...
  while ((*uin != '\0') && ((((gchar *)uout) - outmem) < maxmem)) {
    ...
    case '3': /* octal */
      for (tmpptr = tmp; (tmpptr - tmp) <= 3;) {
        *(tmpptr++) = *(uin++);
        if ((*uin == '\0') || (!g_ascii_isdigit(*uin)) || (*uin == '8') ||
            (*uin == '9'))
          break;
      }
  ...
  if (numbytes > 0) {
    ...
    if (numbytes == 5) {
      ...
      if (!g_seekable_seek(G_SEEKABLE(input), offs,
                           (offs >= 0) ? G_SEEK_SET : G_SEEK_END, NULL, NULL))
        return FILE_MATCH_NONE;
      ...
      for (k = 0; k < numbytes; k++) {
        ...
        byte = g_data_input_stream_read_byte(G_DATA_INPUT_STREAM(input), NULL,
                                             &error);
        if (error) {
          g_clear_error(&error);
          return FILE_MATCH_NONE;
    ...
    file_convert_string(value, mem_testval, sizeof(mem_testval), &numbytes);

    if (numbytes <= 0)
      return FILE_MATCH_NONE;

    if (offs >= 0 && (offs + numbytes <= headsize)) {
      /* We have it in memory */
...

{
  ...
  for (; magics_list; magics_list = magics_list->next) {
    ...
    if ((offset = magics_list->data) == NULL)
      return FILE_MATCH_NONE;
    if ((magics_list = magics_list->next) == NULL)
      return FILE_MATCH_NONE;
    if ((type = magics_list->data) == NULL)
      return FILE_MATCH_NONE;
    if ((magics_list = magics_list->next) == NULL)
      return FILE_MATCH_NONE;
    if ((value = magics_list->data) == NULL)
      return FILE_MATCH_NONE;

    single_match_val = file_check_single_magic(offset, type, value, head,
                                               headsize, file, input);
