...
static void gimp_interpreter_db_add_program(GimpInterpreterDB *db, GFile *file,
                                            gchar *buffer);
static void gimp_interpreter_db_add_binfmt_misc(GimpInterpreterDB *db,
                                                GFile *file, gchar *buffer);
...
void gimp_interpreter_db_load(GimpInterpreterDB *db, GList *path) {
  ...
  db->programs = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
...
static void gimp_interpreter_db_load_interp_file(GimpInterpreterDB *db,
  ...
  while ((buffer = g_data_input_stream_read_line(data_input, &buffer_len, NULL,
                                                 &error))) {
    /* Skip comments */
    if (buffer[0] == '#') {
      g_free(buffer);
  ...
  if (error) {
    g_message(_("Error reading '%s': %s"), gimp_file_get_utf8_name(file),
              error->message);
    g_clear_error(&error);
  }
  ...
  if (!g_file_test(program, G_FILE_TEST_IS_EXECUTABLE)) {
    ...
    if (!prog || !g_file_test(prog, G_FILE_TEST_IS_EXECUTABLE)) {
      g_message(_("Bad interpreter referenced in interpreter file %s: %s"),
                gimp_file_get_utf8_name(file), gimp_filename_to_utf8(program));
      if (prog)
        g_free(prog);
      return;
    }
...
static void gimp_interpreter_db_add_binfmt_misc(GimpInterpreterDB *db,
  ...
  if ((name[0] == '\0') || (program[0] == '\0') || (type[0] == '\0') ||
      (type[1] != '\0'))
    goto bail;
...
static gboolean scanarg(const gchar *s) {
  gchar c;
...
static guint unquote(gchar *from) {
  gchar *s = from;
  gchar *p = from;
  gchar c;
...
#ifdef INTERP_DEBUG
static void print_kv(gpointer key, gpointer value, gpointer user_data) {
  g_print("%s: %s\n", (gchar *)key, (gchar *)value);
}

static gchar *quote(gchar *s, guint size) {
  GString *d;
  guint i;
...
static gboolean resolve_program(gpointer key, gpointer value,
                                gpointer user_data) {
...
static void gimp_interpreter_db_resolve_programs(GimpInterpreterDB *db) {
  ...
  while (list) {
    ...
    g_print("program: %s, offset: %lu, magic: %s, mask: %s\n", magic->program,
            magic->offset, quote(magic->magic, magic->size),
...
static gchar *resolve_extension(GimpInterpreterDB *db,
...
static gchar *resolve_sh_bang(GimpInterpreterDB *db, const gchar *program_path,
                              gchar *buffer, gssize len, gchar **interp_arg) {
  gchar *cp;
  gchar *name;
  gchar *program;
...
static gchar *resolve_magic(GimpInterpreterDB *db, const gchar *program_path,
                            gchar *buffer) {
  GSList *list;
  GimpInterpreterMagic *magic;
  gchar *s;
  ...
  if (input) {
    ...
    if (bytes_read) {
      if (bytes_read > 3 && buffer[0] == '#' && buffer[1] == '!')
        program =
            resolve_sh_bang(db, program_path, buffer, bytes_read, interp_arg);
...
static void collect_extensions(const gchar *ext,
