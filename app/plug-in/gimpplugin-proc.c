...
gboolean gimp_plug_in_set_proc_image_types(GimpPlugIn *plug_in,
  ...
  if (!proc) {
    g_set_error(error, GIMP_PDB_ERROR, GIMP_PDB_ERROR_PROCEDURE_NOT_FOUND,
                ...
                gimp_file_get_utf8_name(plug_in->file), proc_name);
...
gboolean gimp_plug_in_set_proc_menu_label(GimpPlugIn *plug_in,
  ...
  if (!proc) {
    g_set_error(error, GIMP_PDB_ERROR, GIMP_PDB_ERROR_PROCEDURE_NOT_FOUND,
                ...
                "This is not allowed.",
                gimp_object_get_name(plug_in),
                gimp_file_get_utf8_name(plug_in->file), menu_label, proc_name);
...
gboolean gimp_plug_in_add_proc_menu_path(GimpPlugIn *plug_in,
  ...
  if (!proc) {
    g_set_error(error, GIMP_PDB_ERROR, GIMP_PDB_ERROR_PROCEDURE_NOT_FOUND,
                ...
                "This is not allowed.",
                gimp_object_get_name(plug_in),
                gimp_file_get_utf8_name(plug_in->file), menu_path, proc_name);
...
gboolean gimp_plug_in_set_proc_icon(GimpPlugIn *plug_in, const gchar *proc_name,
                                    GimpIconType type, const guint8 *data,
                                    gint data_length, GError **error) {
  GimpPlugInProcedure *proc;
  ...
  return gimp_plug_in_procedure_set_icon(proc, type, data, data_length, error);
}

gboolean gimp_plug_in_set_proc_help(GimpPlugIn *plug_in, const gchar *proc_name,
                                    const gchar *blurb, const gchar *help,
                                    const gchar *help_id, GError **error) {
  GimpPlugInProcedure *proc;
  ...
  gimp_procedure_set_help(GIMP_PROCEDURE(proc), blurb, help, help_id);
...
gboolean gimp_plug_in_set_proc_attribution(GimpPlugIn *plug_in,
  ...
  gimp_procedure_set_attribution(GIMP_PROCEDURE(proc), authors, copyright,
                                 date);
...
gboolean gimp_plug_in_set_file_proc_load_handler(
    GimpPlugIn *plug_in, const gchar *proc_name, const gchar *extensions,
    const gchar *prefixes, const gchar *magics, GError **error) {
  GimpPlugInProcedure *proc;
  ...
  if (((procedure->num_args < 2) || (procedure->num_values < 1) ||
...
gboolean gimp_plug_in_set_file_proc_save_handler(GimpPlugIn *plug_in,
  ...
  if ((procedure->num_args < 5) ||
    ...
    g_set_error(
        error, GIMP_PDB_ERROR, GIMP_PDB_ERROR_FAILED,
        "Plug-in \"%s\"\n(%s)\n"
        "attempted to register procedure \"%s\" "
        ...
        gimp_object_get_name(plug_in), gimp_file_get_utf8_name(plug_in->file),
        proc_name);
...
gboolean gimp_plug_in_set_file_proc_thumb_loader(GimpPlugIn *plug_in,
  ...
  if (!thumb_proc) {
    g_set_error(error, GIMP_PDB_ERROR, GIMP_PDB_ERROR_PROCEDURE_NOT_FOUND,
                ...
                "This is not allowed.",
                gimp_object_get_name(plug_in),
                gimp_file_get_utf8_name(plug_in->file), thumb_proc_name);
