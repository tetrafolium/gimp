...
enum { MENU_PATH_ADDED, LAST_SIGNAL };
...
gimp_plug_in_procedure_execute(GimpProcedure *procedure, Gimp *gimp,
...
static void gimp_plug_in_procedure_execute_async(
    ...
    GimpProgress *progress, GimpValueArray *args, GimpDisplay *display);
...
G_DEFINE_TYPE(GimpPlugInProcedure, gimp_plug_in_procedure, GIMP_TYPE_PROCEDURE)

#define parent_class gimp_plug_in_procedure_parent_class

static guint gimp_plug_in_procedure_signals[LAST_SIGNAL] = {0};

static void gimp_plug_in_procedure_class_init(GimpPlugInProcedureClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  GimpObjectClass *gimp_object_class = GIMP_OBJECT_CLASS(klass);
  GimpViewableClass *viewable_class = GIMP_VIEWABLE_CLASS(klass);
  GimpProcedureClass *proc_class = GIMP_PROCEDURE_CLASS(klass);

  gimp_plug_in_procedure_signals[MENU_PATH_ADDED] = g_signal_new(
      "menu-path-added", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpPlugInProcedureClass, menu_path_added), NULL, NULL,
...
static void gimp_plug_in_procedure_init(GimpPlugInProcedure *proc) {
...
gimp_plug_in_procedure_execute(GimpProcedure *procedure, Gimp *gimp,
  ...
  if (!gimp_plug_in_procedure_validate_args(plug_in_procedure, gimp, args,
                                            &pdb_error)) {
  ...
  if (procedure->proc_type == GIMP_PDB_PROC_TYPE_INTERNAL)
    return GIMP_PROCEDURE_CLASS(parent_class)
        ->execute(procedure, gimp, context, progress, args, error);

  return gimp_plug_in_manager_call_run(gimp->plug_in_manager, context, progress,
                                       GIMP_PLUG_IN_PROCEDURE(procedure), args,
                                       TRUE, NULL);
}
...
static void gimp_plug_in_procedure_execute_async(
  ...
  if (gimp_plug_in_procedure_validate_args(plug_in_procedure, gimp, args,
    ...
    return_vals =
        gimp_plug_in_manager_call_run(gimp->plug_in_manager, context, progress,
                                      plug_in_procedure, args, FALSE, display);

    if (return_vals) {
      gimp_plug_in_procedure_handle_return_values(plug_in_procedure, gimp,
                                                  progress, return_vals);
...
static inline gboolean GIMP_IS_PARAM_SPEC_FILE(GParamSpec *pspec) {
  return (G_IS_PARAM_SPEC_OBJECT(pspec) && pspec->value_type == G_TYPE_FILE);
}
...
gboolean gimp_plug_in_procedure_set_menu_label(GimpPlugInProcedure *proc,
  ...
  if (menu_label[0] == '<') {
    ...
    g_set_error(error, GIMP_PLUG_IN_ERROR, GIMP_PLUG_IN_FAILED,
                ...
                gimp_object_get_name(proc), menu_label);
...
gboolean gimp_plug_in_procedure_add_menu_path(GimpPlugInProcedure *proc,
  ...
  if (g_str_has_prefix(menu_path, "<Image>")) {
    ...
    if ((procedure->num_args < 3) ||
        !GIMP_IS_PARAM_SPEC_RUN_MODE(procedure->args[0]) ||
        !GIMP_IS_PARAM_SPEC_IMAGE(procedure->args[1]) ||
        !(G_TYPE_FROM_INSTANCE(procedure->args[2]) == GIMP_TYPE_PARAM_LAYER ||
    ...
    if ((procedure->num_args < 3) ||
        !GIMP_IS_PARAM_SPEC_RUN_MODE(procedure->args[0]) ||
        !GIMP_IS_PARAM_SPEC_IMAGE(procedure->args[1]) ||
        !(G_TYPE_FROM_INSTANCE(procedure->args[2]) == GIMP_TYPE_PARAM_CHANNEL ||
          G_TYPE_FROM_INSTANCE(procedure->args[2]) ==
              GIMP_TYPE_PARAM_DRAWABLE)) {
  ...
  if (required) {
    ...
    *p = '\0';
    ...
    g_set_error(error, GIMP_PLUG_IN_ERROR, GIMP_PLUG_IN_FAILED,
                ...
                "arguments: (%s).",
                basename, gimp_file_get_utf8_name(proc->file), prefix,
                gimp_object_get_name(proc), prefix, required);
...
gboolean gimp_plug_in_procedure_take_icon(GimpPlugInProcedure *proc,
  ...
  case GIMP_ICON_TYPE_PIXBUF:
    ...
    if (!gdk_pixbuf_loader_write(loader, proc->icon_data,
                                 proc->icon_data_length, error)) {
  ...
  case GIMP_ICON_TYPE_IMAGE_FILE:
    ...
    icon_pixbuf = gdk_pixbuf_new_from_file((gchar *)proc->icon_data, error);
...
static GimpPlugInImageType image_types_parse(const gchar *name,
  ...
  while (*image_types) {
    while (*image_types && ((*image_types == ' ') || (*image_types == '\t') ||
                            (*image_types == ',')))
      image_types++;
...
void gimp_plug_in_procedure_set_image_types(GimpPlugInProcedure *proc,
  ...
  proc->image_types_val =
      image_types_parse(gimp_object_get_name(proc), proc->image_types);
...
static GSList *extensions_parse(gchar *extensions) {
  GSList *list = NULL;
...
void gimp_plug_in_procedure_set_generic_file_proc(
    GimpPlugInProcedure *proc, gboolean is_generic_file_proc) {
