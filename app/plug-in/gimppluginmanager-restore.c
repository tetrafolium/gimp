...
static void
gimp_plug_in_manager_read_pluginrc(GimpPlugInManager *manager, GFile *file,
                                   GimpInitStatusFunc status_callback);
...
void gimp_plug_in_manager_restore(GimpPlugInManager *manager,
  ...
  for (list = manager->plug_in_defs; list; list = list->next) {
    ...
    if (plug_in_def->locale_domain_name)
      gimp_plug_in_manager_add_locale_domain(manager, plug_in_def->file,
      ...
      gimp_plug_in_def_set_locale_domain(plug_in_def,
                                         gimp_plug_in_manager_get_locale_domain(
                                             manager, plug_in_def->file, NULL),
                                         NULL);

    if (plug_in_def->help_domain_name)
      gimp_plug_in_manager_add_help_domain(manager, plug_in_def->file,
...
static void gimp_plug_in_manager_search(GimpPlugInManager *manager,
  ...
  g_object_get(manager->gimp->extension_manager, "plug-in-paths", &path, NULL);
    ...
    while ((info = g_file_enumerator_next_file(enumerator, NULL, NULL))) {
      ...
      if (g_file_query_file_type(child, G_FILE_QUERY_INFO_NONE, NULL) ==
        ...
        if (enumerator2) {
          ...
          while (
              (info2 = g_file_enumerator_next_file(enumerator2, NULL, NULL))) {
            GFile *child2;
            gchar *file_name;
            char *ext;
            ...
            if (g_strcmp0(file_name, g_file_info_get_name(info)) == 0 &&
              ...
              mtime = g_file_info_get_attribute_uint64(
                  info2, G_FILE_ATTRIBUTE_TIME_MODIFIED);
...
static GFile *gimp_plug_in_manager_get_pluginrc(GimpPlugInManager *manager) {
  ...
  if (gimp->config->plug_in_rc_path) {
    gchar *path =
        gimp_config_path_expand(gimp->config->plug_in_rc_path, TRUE, NULL);
...
/* read the pluginrc file for cached data */
static void
gimp_plug_in_manager_read_pluginrc(GimpPlugInManager *manager, GFile *pluginrc,
                                   GimpInitStatusFunc status_callback) {
  GSList *rc_defs;
  GError *error = NULL;
  ...
  if (rc_defs) {
    ...
    for (list = rc_defs; list; list = g_slist_next(list))
      gimp_plug_in_manager_add_from_rc(manager,
                                       list->data); /* consumes list->data */
...
gimp_plug_in_manager_run_extensions(GimpPlugInManager *manager,
  ...
  if (extensions) {
    ...
    for (list = extensions, nth = 0; list; list = g_list_next(list), nth++) {
      ...
      gimp_procedure_execute_async(GIMP_PROCEDURE(proc), gimp, context, NULL,
                                   args, NULL, &error);
...
static void gimp_plug_in_manager_bind_text_domains(GimpPlugInManager *manager) {
  ...
  n_domains = gimp_plug_in_manager_get_locale_domains(manager, &locale_domains,
                                                      &locale_paths);
...
gimp_plug_in_manager_ignore_plugin_basename(const gchar *plugin_basename) {
  ...
  ignore_basenames_string =
      g_getenv("GIMP_TESTING_PLUGINDIRS_BASENAME_IGNORES");
  ignore_basenames = gimp_path_parse(ignore_basenames_string, 256 /*max_paths*/,
                                     FALSE /*check*/, NULL /*check_failed*/);
...
static void gimp_plug_in_manager_add_from_rc(GimpPlugInManager *manager,
  ...
  for (list = manager->plug_in_defs; list; list = list->next) {
    ...
    if (!strcmp(basename1, basename2)) {
      if (g_file_equal(plug_in_def->file, ondisk_plug_in_def->file) &&
...
static void gimp_plug_in_manager_add_to_db(GimpPlugInManager *manager,
  ...
  if (proc->file_proc) {
    if (proc->image_types) {
      gimp_plug_in_procedure_set_file_proc(proc, proc->extensions,
                                           proc->prefixes, NULL);

      gimp_plug_in_manager_add_save_procedure(manager, proc);
    } else {
      gimp_plug_in_procedure_set_file_proc(proc, proc->extensions,
                                           proc->prefixes, proc->magics);
...
static void gimp_plug_in_manager_sort_file_procs(GimpPlugInManager *manager) {
  ...
  manager->load_procs = g_slist_sort_with_data(
      manager->load_procs, gimp_plug_in_manager_file_proc_compare,
      GINT_TO_POINTER(FALSE));
  manager->save_procs = g_slist_sort_with_data(
      manager->save_procs, gimp_plug_in_manager_file_proc_compare,
      GINT_TO_POINTER(FALSE));
  manager->export_procs = g_slist_sort_with_data(
      manager->export_procs, gimp_plug_in_manager_file_proc_compare,
      GINT_TO_POINTER(FALSE));
  ...
  manager->display_load_procs = g_slist_sort_with_data(
      manager->display_load_procs, gimp_plug_in_manager_file_proc_compare,
      GINT_TO_POINTER(TRUE));
  manager->display_save_procs = g_slist_sort_with_data(
      manager->display_save_procs, gimp_plug_in_manager_file_proc_compare,
      GINT_TO_POINTER(TRUE));
  manager->display_export_procs = g_slist_sort_with_data(
      manager->display_export_procs, gimp_plug_in_manager_file_proc_compare,
      GINT_TO_POINTER(TRUE));
  ...
  if (config->import_raw_plug_in) {
    ...
    if (!strstr(config->import_raw_plug_in, "file-raw-placeholder"))
      config_plug_in =
          gimp_file_new_for_config_path(config->import_raw_plug_in, NULL);
  }
  ...
  for (list = manager->load_procs; list; list = g_slist_next(list)) {
    ...
    if (file_proc->handles_raw) {
      ...
      manager->raw_load_procs =
          g_slist_prepend(manager->raw_load_procs, file_proc);

      file = gimp_plug_in_procedure_get_file(file_proc);

      if (!raw_plug_in && config_plug_in &&
  ...
  manager->display_raw_load_procs = g_slist_sort_with_data(
      manager->display_raw_load_procs, gimp_plug_in_manager_file_proc_compare,
      GINT_TO_POINTER(TRUE));
  ...
  if (!raw_plug_in && manager->raw_load_procs) {
    ...
    g_object_set(config, "import-raw-plug-in", path, NULL);
  ...
  while (list) {
    ...
    if (file_proc->handles_raw &&
        !g_file_equal(gimp_plug_in_procedure_get_file(file_proc),
                      raw_plug_in)) {
      manager->load_procs = g_slist_remove(manager->load_procs, file_proc);
