...
static void gimp_plug_in_def_class_init(GimpPlugInDefClass *klass) {
...
static void gimp_plug_in_def_init(GimpPlugInDef *def) {}
...
void gimp_plug_in_def_add_procedure(GimpPlugInDef *plug_in_def,
  ...
  gimp_plug_in_procedure_set_help_domain(proc, plug_in_def->help_domain_name);

  plug_in_def->procedures =
      g_slist_append(plug_in_def->procedures, g_object_ref(proc));
}
