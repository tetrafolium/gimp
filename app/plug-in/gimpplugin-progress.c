...
gint gimp_plug_in_progress_attach(GimpProgress *progress) {
  ...
  attach_count = GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(progress), "plug-in-progress-attach-count"));
...
void gimp_plug_in_progress_start(GimpPlugIn *plug_in, const gchar *message,
                                 GimpDisplay *display) {
  ...
  if (!proc_frame->progress) {
    proc_frame->progress = gimp_new_progress(plug_in->manager->gimp, display);
  ...
  if (proc_frame->progress) {
    if (!proc_frame->progress_cancel_id) {
      ...
      proc_frame->progress_cancel_id = g_signal_connect(
          proc_frame->progress, "cancel",
          G_CALLBACK(gimp_plug_in_progress_cancel_callback), plug_in);
    }
    ...
    if (gimp_progress_is_active(proc_frame->progress)) {
      ...
      gimp_progress_start(proc_frame->progress, TRUE, "%s",
                          message ? message : "");
    }
  }
}
...
void gimp_plug_in_progress_set_text(GimpPlugIn *plug_in, const gchar *message) {
  GimpPlugInProcFrame *proc_frame;
...
void gimp_plug_in_progress_set_value(GimpPlugIn *plug_in, gdouble percentage) {
  GimpPlugInProcFrame *proc_frame;
...
gboolean gimp_plug_in_progress_install(GimpPlugIn *plug_in,
  ...
  procedure =
      gimp_pdb_lookup_procedure(plug_in->manager->gimp->pdb, progress_callback);

  if (!GIMP_IS_TEMPORARY_PROCEDURE(procedure) ||
      GIMP_TEMPORARY_PROCEDURE(procedure)->plug_in != plug_in ||
      procedure->num_args != 3 || !G_IS_PARAM_SPEC_INT(procedure->args[0]) ||
  ...
  proc_frame->progress = g_object_new(
      GIMP_TYPE_PDB_PROGRESS, "pdb", plug_in->manager->gimp->pdb, "context",
      proc_frame->main_context, "callback-name", progress_callback, NULL);
...
static GimpValueArray *get_cancel_return_values(GimpProcedure *procedure) {
...
static void gimp_plug_in_progress_cancel_callback(GimpProgress *progress,
  ...
  if (proc_frame->main_loop) {
    proc_frame->return_vals = get_cancel_return_values(proc_frame->procedure);
  }
