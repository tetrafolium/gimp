...
static gboolean gimp_plug_in_write(GIOChannel *channel, const guint8 *buf,
                                   gulong count, gpointer data);
static gboolean gimp_plug_in_flush(GIOChannel *channel, gpointer data);
...
static gboolean gimp_plug_in_recv_message(GIOChannel *channel,
  ...
  if (!got_message) {
    ...
    gimp_message(plug_in->manager->gimp, G_OBJECT(progress), GIMP_MESSAGE_ERROR,
                 _("Plug-in crashed: \"%s\"\n(%s)\n\n"
...
static gboolean gimp_plug_in_write(GIOChannel *channel, const guint8 *buf,
  ...
  while (count > 0) {
    if ((plug_in->write_buffer_index + count) >= WRITE_BUFFER_SIZE) {
      ...
      memcpy(&plug_in->write_buffer[plug_in->write_buffer_index], buf, bytes);
    ...
    while (count != plug_in->write_buffer_index) {
      do {
        ...
        status = g_io_channel_write_chars(
            channel, &plug_in->write_buffer[count],
            (plug_in->write_buffer_index - count), &bytes, &error);
      } while (status == G_IO_STATUS_AGAIN);

      if (status != G_IO_STATUS_NORMAL) {
        if (error) {
          g_warning("%s: plug_in_flush(): error: %s",
                    gimp_filename_to_utf8(g_get_prgname()), error->message);
          g_error_free(error);
        } else {
...
static void gimp_plug_in_set_dll_directory(const gchar *path) {
  ...
  if (path && GetBinaryTypeA(path, &BinaryType) &&
      BinaryType == SCS_32BIT_BINARY)
    bin_dir = g_build_filename(install_dir, WIN32_32BIT_DLL_FOLDER, NULL);
  else
    bin_dir = g_build_filename(install_dir, "bin", NULL);

  n = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, bin_dir, -1, NULL, 0);
  if (n == 0)
    goto out;

  w_bin_dir = g_malloc_n(n + 1, sizeof(wchar_t));
  n = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, bin_dir, -1, w_bin_dir,
                          (n + 1) * sizeof(wchar_t));
  if (n == 0)
    goto out;
...
GimpPlugIn *gimp_plug_in_new(GimpPlugInManager *manager, GimpContext *context,
                             GimpProgress *progress,
                             GimpPlugInProcedure *procedure, GFile *file) {
  GimpPlugIn *plug_in;
  ...
  g_return_val_if_fail(
      procedure == NULL || GIMP_IS_PLUG_IN_PROCEDURE(procedure), NULL);
  g_return_val_if_fail(file == NULL || G_IS_FILE(file), NULL);
  g_return_val_if_fail((procedure != NULL || file != NULL) &&
                           !(procedure != NULL && file != NULL),
                       NULL);
  ...
  gimp_plug_in_proc_frame_init(&plug_in->main_proc_frame, context, progress,
                               procedure);

  return plug_in;
}

gboolean gimp_plug_in_open(GimpPlugIn *plug_in, GimpPlugInCallMode call_mode,
  ...
  if ((pipe(my_read) == -1) || (pipe(my_write) == -1)) {
    gimp_message(plug_in->manager->gimp, NULL, GIMP_MESSAGE_ERROR,
                 ...
                 gimp_object_get_name(plug_in),
                 gimp_file_get_utf8_name(plug_in->file), g_strerror(errno));
    return FALSE;
  }
  ...
  g_snprintf(protocol_version, sizeof(protocol_version), "%d",
  ...
  if (!gimp_spawn_async(argv, envp, spawn_flags, &plug_in->pid, &error)) {
    gimp_message(plug_in->manager->gimp, NULL, GIMP_MESSAGE_ERROR,
                 "Unable to run plug-in \"%s\"\n(%s)\n\n%s",
                 gimp_object_get_name(plug_in),
                 gimp_file_get_utf8_name(plug_in->file), error->message);
    g_clear_error(&error);
    goto cleanup;
  }
  ...
  if (!synchronous) {
    ...
    g_source_set_callback(source, (GSourceFunc)gimp_plug_in_recv_message,
                          plug_in, NULL);
...
void gimp_plug_in_close(GimpPlugIn *plug_in, gboolean kill_it) {
...
GimpPlugInProcFrame *
gimp_plug_in_proc_frame_push(GimpPlugIn *plug_in, GimpContext *context,
                             GimpProgress *progress,
  ...
  plug_in->temp_proc_frames =
      g_list_prepend(plug_in->temp_proc_frames, proc_frame);
...
void gimp_plug_in_proc_frame_pop(GimpPlugIn *plug_in) {
  ...
  plug_in->temp_proc_frames =
      g_list_remove(plug_in->temp_proc_frames, proc_frame);
}
...
void gimp_plug_in_add_temp_proc(GimpPlugIn *plug_in,
  ...
  plug_in->temp_procedures =
      g_slist_prepend(plug_in->temp_procedures, g_object_ref(proc));
