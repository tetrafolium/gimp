...
static GTokenType plug_in_def_deserialize(Gimp *gimp, GScanner *scanner,
                                          GSList **plug_in_defs);
static GTokenType plug_in_procedure_deserialize(GScanner *scanner, Gimp *gimp,
                                                GFile *file,
...
static GTokenType plug_in_file_proc_deserialize(GScanner *scanner,
                                                GimpPlugInProcedure *proc);
static GTokenType plug_in_proc_arg_deserialize(GScanner *scanner, Gimp *gimp,
...
GSList *plug_in_rc_parse(Gimp *gimp, GFile *file, GError **error) {
  ...
  g_scanner_scope_add_symbol(scanner, 0, "protocol-version",
                             GINT_TO_POINTER(PROTOCOL_VERSION));
  g_scanner_scope_add_symbol(scanner, 0, "file-version",
                             GINT_TO_POINTER(FILE_VERSION));
  g_scanner_scope_add_symbol(scanner, 0, "plug-in-def",
                             GINT_TO_POINTER(PLUG_IN_DEF));

  g_scanner_scope_add_symbol(scanner, PLUG_IN_DEF, "proc-def",
                             GINT_TO_POINTER(PROC_DEF));
  g_scanner_scope_add_symbol(scanner, PLUG_IN_DEF, "locale-def",
                             GINT_TO_POINTER(LOCALE_DEF));
  g_scanner_scope_add_symbol(scanner, PLUG_IN_DEF, "help-def",
                             GINT_TO_POINTER(HELP_DEF));
  g_scanner_scope_add_symbol(scanner, PLUG_IN_DEF, "has-init",
                             GINT_TO_POINTER(HAS_INIT));
  g_scanner_scope_add_symbol(scanner, PLUG_IN_DEF, "proc-arg",
                             GINT_TO_POINTER(PROC_ARG));
  g_scanner_scope_add_symbol(scanner, PLUG_IN_DEF, "menu-path",
                             GINT_TO_POINTER(MENU_PATH));
  g_scanner_scope_add_symbol(scanner, PLUG_IN_DEF, "icon",
                             GINT_TO_POINTER(ICON));
  g_scanner_scope_add_symbol(scanner, PLUG_IN_DEF, "load-proc",
                             GINT_TO_POINTER(LOAD_PROC));
  g_scanner_scope_add_symbol(scanner, PLUG_IN_DEF, "save-proc",
                             GINT_TO_POINTER(SAVE_PROC));

  g_scanner_scope_add_symbol(scanner, LOAD_PROC, "extensions",
                             GINT_TO_POINTER(EXTENSIONS));
  g_scanner_scope_add_symbol(scanner, LOAD_PROC, "prefixes",
                             GINT_TO_POINTER(PREFIXES));
  g_scanner_scope_add_symbol(scanner, LOAD_PROC, "magics",
                             GINT_TO_POINTER(MAGICS));
  g_scanner_scope_add_symbol(scanner, LOAD_PROC, "priority",
                             GINT_TO_POINTER(PRIORITY));
  g_scanner_scope_add_symbol(scanner, LOAD_PROC, "mime-types",
                             GINT_TO_POINTER(MIME_TYPES));
  g_scanner_scope_add_symbol(scanner, LOAD_PROC, "handles-remote",
                             GINT_TO_POINTER(HANDLES_REMOTE));
  g_scanner_scope_add_symbol(scanner, LOAD_PROC, "handles-raw",
                             GINT_TO_POINTER(HANDLES_RAW));
  g_scanner_scope_add_symbol(scanner, LOAD_PROC, "thumb-loader",
                             GINT_TO_POINTER(THUMB_LOADER));

  g_scanner_scope_add_symbol(scanner, SAVE_PROC, "extensions",
                             GINT_TO_POINTER(EXTENSIONS));
  g_scanner_scope_add_symbol(scanner, SAVE_PROC, "prefixes",
                             GINT_TO_POINTER(PREFIXES));
  g_scanner_scope_add_symbol(scanner, SAVE_PROC, "priority",
                             GINT_TO_POINTER(PRIORITY));
  g_scanner_scope_add_symbol(scanner, SAVE_PROC, "mime-types",
                             GINT_TO_POINTER(MIME_TYPES));
  g_scanner_scope_add_symbol(scanner, SAVE_PROC, "handles-remote",
                             GINT_TO_POINTER(HANDLES_REMOTE));
  ...
  if (protocol_version != GIMP_PROTOCOL_VERSION ||
      file_version != PLUG_IN_RC_FILE_VERSION || token != G_TOKEN_LEFT_PAREN) {
    if (protocol_version != GIMP_PROTOCOL_VERSION) {
      g_set_error(error, GIMP_CONFIG_ERROR, GIMP_CONFIG_ERROR_VERSION,
...
static GTokenType plug_in_def_deserialize(Gimp *gimp, GScanner *scanner,
  ...
  if (!file) {
    g_scanner_error(scanner, "unable to parse plug-in filename: %s",
                    error->message);
    g_clear_error(&error);
    return G_TOKEN_ERROR;
  }
  ...
  while (g_scanner_peek_next_token(scanner) == token) {
    ...
    case G_TOKEN_SYMBOL:
      switch (GPOINTER_TO_INT(scanner->value.v_symbol)) {
      case PROC_DEF:
        token = plug_in_procedure_deserialize(scanner, gimp, plug_in_def->file,
                                              &proc);
...
static GTokenType plug_in_procedure_deserialize(GScanner *scanner, Gimp *gimp,
  ...
  if (proc_type != GIMP_PDB_PROC_TYPE_PLUGIN &&
    ...
    g_free(str);
    g_scanner_error(scanner, "procedure type %d is out of range", proc_type);
    return G_TOKEN_ERROR;
  }
...
static GTokenType plug_in_icon_deserialize(GScanner *scanner,
  ...
  case G_TOKEN_IDENTIFIER:
    ...
    if (!enum_value) {
      g_scanner_error(scanner, _("invalid value '%s' for icon type"),
                      scanner->value.v_identifier);
      return G_TOKEN_NONE;
    }
    break;
  ...
  case G_TOKEN_INT:
    ...
    enum_value = g_enum_get_value(enum_class, (gint)scanner->value.v_int64);

    if (!enum_value) {
      g_scanner_error(scanner, _("invalid value '%ld' for icon type"),
  ...
  gimp_plug_in_procedure_take_icon(proc, icon_type, icon_data, icon_data_length,
                                   NULL);
...
static GTokenType plug_in_proc_arg_deserialize(GScanner *scanner, Gimp *gimp,
  ...
  case GP_PARAM_DEF_TYPE_INT:
    if (!gimp_scanner_parse_int64(scanner, &param_def.meta.m_int.min_val) ||
        !gimp_scanner_parse_int64(scanner, &param_def.meta.m_int.max_val) ||
        !gimp_scanner_parse_int64(scanner, &param_def.meta.m_int.default_val)) {
      token = G_TOKEN_INT;
      goto error;
    }
    break;
  ...
  case GP_PARAM_DEF_TYPE_UNIT:
    if (!gimp_scanner_parse_int(scanner, &param_def.meta.m_unit.allow_pixels) ||
        !gimp_scanner_parse_int(scanner,
                                &param_def.meta.m_unit.allow_percent) ||
        !gimp_scanner_parse_int(scanner, &param_def.meta.m_unit.default_val)) {
      token = G_TOKEN_INT;
      goto error;
    }
    break;
  ...
  case GP_PARAM_DEF_TYPE_ENUM:
    if (!gimp_scanner_parse_int(scanner, &param_def.meta.m_enum.default_val)) {
      token = G_TOKEN_STRING;
      goto error;
    }
    break;
  ...
  case GP_PARAM_DEF_TYPE_FLOAT:
    if (!gimp_scanner_parse_float(scanner, &param_def.meta.m_float.min_val) ||
        !gimp_scanner_parse_float(scanner, &param_def.meta.m_float.max_val) ||
  ...
  case GP_PARAM_DEF_TYPE_COLOR:
    if (!gimp_scanner_parse_int(scanner, &param_def.meta.m_color.has_alpha)) {
      token = G_TOKEN_INT;
      goto error;
    }
  ...
  case GP_PARAM_DEF_TYPE_ID:
    if (!gimp_scanner_parse_int(scanner, &param_def.meta.m_id.none_ok)) {
      token = G_TOKEN_INT;
      goto error;
    }
    break;
...
static void plug_in_rc_write_proc_arg(GimpConfigWriter *writer,
                                      GParamSpec *pspec) {
  GPParamDef param_def = {
      0,
  };
  ...
  case GP_PARAM_DEF_TYPE_INT:
    gimp_config_writer_printf(
        writer, "%" G_GINT64_FORMAT " %" G_GINT64_FORMAT " %" G_GINT64_FORMAT,
        param_def.meta.m_int.min_val, param_def.meta.m_int.max_val,
        param_def.meta.m_int.default_val);
    break;

  case GP_PARAM_DEF_TYPE_UNIT:
    gimp_config_writer_printf(
        writer, "%d %d %d", param_def.meta.m_unit.allow_pixels,
        param_def.meta.m_unit.allow_percent, param_def.meta.m_unit.default_val);
    break;

  case GP_PARAM_DEF_TYPE_ENUM:
    gimp_config_writer_printf(writer, "%d", param_def.meta.m_enum.default_val);
    break;
  ...
  case GP_PARAM_DEF_TYPE_FLOAT:
    g_ascii_dtostr(buf[0], sizeof(buf[0]), param_def.meta.m_float.min_val);
    g_ascii_dtostr(buf[1], sizeof(buf[1]), param_def.meta.m_float.max_val),
        g_ascii_dtostr(buf[2], sizeof(buf[2]),
                       param_def.meta.m_float.default_val);
    gimp_config_writer_printf(writer, "%s %s %s", buf[0], buf[1], buf[2]);
    break;

  case GP_PARAM_DEF_TYPE_STRING:
    gimp_config_writer_string(writer, param_def.meta.m_string.default_val);
    break;
  ...
  case GP_PARAM_DEF_TYPE_COLOR:
    ...
    gimp_config_writer_printf(writer, "%d %s %s %s %s",
                              param_def.meta.m_color.has_alpha, buf[0], buf[1],
                              buf[2], buf[3]);
    break;

  case GP_PARAM_DEF_TYPE_ID:
    gimp_config_writer_printf(writer, "%d", param_def.meta.m_id.none_ok);
    break;

  case GP_PARAM_DEF_TYPE_ID_ARRAY:
    gimp_config_writer_string(writer, param_def.meta.m_id_array.type_name);
    break;
  }
...
gboolean plug_in_rc_write(GSList *plug_in_defs, GFile *file, GError **error) {
  ...
  writer =
      gimp_config_writer_new_from_file(file, FALSE,
                                       "GIMP pluginrc\n\n"
  ...
  for (list = plug_in_defs; list; list = list->next) {
    ...
    if (plug_in_def->procedures) {
      ...
      for (list2 = plug_in_def->procedures; list2; list2 = list2->next) {
        ...
        gimp_config_writer_printf(writer, "%d", proc->icon_data_length);
        ...
        if (proc->file_proc) {
          gimp_config_writer_open(writer, proc->image_types ? "save-proc"
                                                            : "load-proc");
        ...
        gimp_config_writer_printf(writer, "%d %d", procedure->num_args,
      ...
      if (plug_in_def->locale_domain_name) {
        ...
        gimp_config_writer_string(writer, plug_in_def->locale_domain_name);
      ...
      if (plug_in_def->help_domain_name) {
        ...
        gimp_config_writer_string(writer, plug_in_def->help_domain_name);

        if (plug_in_def->help_domain_uri)
          gimp_config_writer_string(writer, plug_in_def->help_domain_uri);
