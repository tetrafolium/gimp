...
GimpPlugInShm *gimp_plug_in_shm_new(void) {
  ...
  /* Use SysV shared memory mechanisms for transferring tile data. */
  {
    ...
    if (shm->shm_id != -1) {
      ...
      g_printerr("shmget() failed: %s\n" ERRMSG_SHM_DISABLE, g_strerror(errno));
    }
  }
  ...
  /* Use Win32 shared memory mechanisms for transferring tile data. */
  {
    ...
    shm->shm_handle =
        CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0,
                          TILE_MAP_SIZE, fileMapName);

    if (shm->shm_handle) {
      /* Map the shared memory into our address space for use */
      shm->shm_addr = (guchar *)MapViewOfFile(
          shm->shm_handle, FILE_MAP_ALL_ACCESS, 0, 0, TILE_MAP_SIZE);
  ...
  /* Use POSIX shared memory mechanisms for transferring tile data. */
  {
    ...
    if (shm_fd != -1) {
      if (ftruncate(shm_fd, TILE_MAP_SIZE) != -1) {
        /* Map the shared memory into our address space for use */
        shm->shm_addr = (guchar *)mmap(
            NULL, TILE_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
...
void gimp_plug_in_shm_free(GimpPlugInShm *shm) {
  ...
  if (shm->shm_id != -1) {
    ...
    g_snprintf(shm_handle, sizeof(shm_handle), "/gimp-shm-%d", shm->shm_id);
