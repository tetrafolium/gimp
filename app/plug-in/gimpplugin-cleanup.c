...
gimp_plug_in_cleanup_item_new(GimpPlugInProcFrame *proc_frame, GimpItem *item);
...
static GimpPlugInCleanupItem *
gimp_plug_in_cleanup_item_get(GimpPlugInProcFrame *proc_frame, GimpItem *item);
...
void gimp_plug_in_cleanup(GimpPlugIn *plug_in,
  ...
  while (proc_frame->image_cleanups) {
    ...
    if (gimp_image_get_by_id(plug_in->manager->gimp, cleanup->image_id) ==
        cleanup->image) {
  ...
  while (proc_frame->item_cleanups) {
    ...
    if (gimp_item_get_by_id(plug_in->manager->gimp, cleanup->item_id) ==
        cleanup->item) {
...
gimp_plug_in_cleanup_image_new(GimpPlugInProcFrame *proc_frame,
  ...
  proc_frame->image_cleanups =
      g_list_prepend(proc_frame->image_cleanups, cleanup);
...
static void gimp_plug_in_cleanup_image_free(GimpPlugInProcFrame *proc_frame,
                                            GimpPlugInCleanupImage *cleanup) {
  proc_frame->image_cleanups =
      g_list_remove(proc_frame->image_cleanups, cleanup);
...
gimp_plug_in_cleanup_item_new(GimpPlugInProcFrame *proc_frame, GimpItem *item) {
  ...
  proc_frame->item_cleanups = g_list_remove(proc_frame->item_cleanups, cleanup);
