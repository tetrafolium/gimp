...
static void gimp_allow_set_foreground_window(GimpPlugIn *plug_in) {
#ifdef G_OS_WIN32
...
void gimp_plug_in_manager_call_query(GimpPlugInManager *manager,
  ...
  plug_in = gimp_plug_in_new(manager, context, NULL, NULL, plug_in_def->file);
...
GimpValueArray *gimp_plug_in_manager_call_run(
    GimpPlugInManager *manager, GimpContext *context, GimpProgress *progress,
    GimpPlugInProcedure *procedure, GimpValueArray *args, gboolean synchronous,
    GimpDisplay *display) {
  ...
  if (plug_in) {
    ...
    if (!gimp_plug_in_open(plug_in, GIMP_PLUG_IN_CALL_RUN, FALSE)) {
      ...
      GError *error =
          g_error_new(GIMP_PLUG_IN_ERROR, GIMP_PLUG_IN_EXECUTION_FAILED,
                      _("Failed to run plug-in \"%s\""), name);
    ...
    config.shm_id = (manager->shm ? gimp_plug_in_shm_get_id(manager->shm) : -1);
    ...
    config.show_help_button =
        (gui_config->use_help && gui_config->show_help_button);
    ...
    config.display_name = gimp_get_display_name(
        manager->gimp, display_id, &monitor, &config.monitor_number);
    config.timestamp = gimp_get_user_time(manager->gimp);
    config.icon_theme_dir =
        (icon_theme_dir ? g_file_get_path(icon_theme_dir) : NULL);
...
GimpValueArray *gimp_plug_in_manager_call_run_temp(
    GimpPlugInManager *manager, GimpContext *context, GimpProgress *progress,
    GimpTemporaryProcedure *procedure, GimpValueArray *args) {
  GimpValueArray *return_vals = NULL;
  GimpPlugIn *plug_in;
  ...
  if (plug_in) {
    ...
    proc_frame =
        gimp_plug_in_proc_frame_push(plug_in, context, progress, procedure);
