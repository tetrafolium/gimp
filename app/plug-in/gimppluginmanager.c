...
enum { PLUG_IN_OPENED, PLUG_IN_CLOSED, MENU_BRANCH_ADDED, LAST_SIGNAL };
...
static void gimp_plug_in_manager_class_init(GimpPlugInManagerClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  GimpObjectClass *gimp_object_class = GIMP_OBJECT_CLASS(klass);

  manager_signals[PLUG_IN_OPENED] = g_signal_new(
      "plug-in-opened", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpPlugInManagerClass, plug_in_opened), NULL, NULL, NULL,
      G_TYPE_NONE, 1, GIMP_TYPE_PLUG_IN);

  manager_signals[PLUG_IN_CLOSED] = g_signal_new(
      "plug-in-closed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpPlugInManagerClass, plug_in_closed), NULL, NULL, NULL,
      G_TYPE_NONE, 1, GIMP_TYPE_PLUG_IN);

  manager_signals[MENU_BRANCH_ADDED] = g_signal_new(
      "menu-branch-added", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpPlugInManagerClass, menu_branch_added), NULL, NULL,
      gimp_marshal_VOID__OBJECT_STRING_STRING, G_TYPE_NONE, 1, G_TYPE_FILE,
...
static void gimp_plug_in_manager_init(GimpPlugInManager *manager) {}
...
static gint64 gimp_plug_in_manager_get_memsize(GimpObject *object,
  ...
  memsize += gimp_g_slist_get_memsize_foreach(
      manager->plug_in_defs, (GimpMemsizeFunc)gimp_object_get_memsize,
      gui_size);
  ...
  memsize += gimp_g_slist_get_memsize_foreach(
      manager->open_plug_ins, (GimpMemsizeFunc)gimp_object_get_memsize,
      gui_size);
  memsize += gimp_g_slist_get_memsize(manager->plug_in_stack, 0);

  memsize += 0; /* FIXME manager->shm */
  memsize +=
      /* FIXME */ gimp_g_object_get_memsize(G_OBJECT(manager->interpreter_db));
  memsize +=
      /* FIXME */ gimp_g_object_get_memsize(G_OBJECT(manager->environ_table));
...
void gimp_plug_in_manager_add_procedure(GimpPlugInManager *manager,
  ...
  for (list = manager->plug_in_procedures; list; list = list->next) {
    ...
    if (strcmp(gimp_object_get_name(procedure),
      ...
      manager->raw_load_procs =
          g_slist_remove(manager->raw_load_procs, tmp_proc);
      manager->display_load_procs =
          g_slist_remove(manager->display_load_procs, tmp_proc);
      manager->display_save_procs =
          g_slist_remove(manager->display_save_procs, tmp_proc);
      manager->display_export_procs =
          g_slist_remove(manager->display_export_procs, tmp_proc);
      manager->display_raw_load_procs =
          g_slist_remove(manager->display_raw_load_procs, tmp_proc);
  ...
  manager->plug_in_procedures =
      g_slist_prepend(manager->plug_in_procedures, g_object_ref(procedure));
}
...
void gimp_plug_in_manager_remove_temp_proc(GimpPlugInManager *manager,
  ...
  manager->plug_in_procedures =
      g_slist_remove(manager->plug_in_procedures, procedure);

  gimp_filter_history_remove(manager->gimp, GIMP_PROCEDURE(procedure));
...
void gimp_plug_in_manager_add_open_plug_in(GimpPlugInManager *manager,
  ...
  manager->open_plug_ins =
      g_slist_prepend(manager->open_plug_ins, g_object_ref(plug_in));

  g_signal_emit(manager, manager_signals[PLUG_IN_OPENED], 0, plug_in);
}
...
void gimp_plug_in_manager_remove_open_plug_in(GimpPlugInManager *manager,
  ...
  g_signal_emit(manager, manager_signals[PLUG_IN_CLOSED], 0, plug_in);
...
void gimp_plug_in_manager_plug_in_push(GimpPlugInManager *manager,
  ...
  manager->plug_in_stack =
      g_slist_prepend(manager->plug_in_stack, manager->current_plug_in);
}
...
void gimp_plug_in_manager_plug_in_pop(GimpPlugInManager *manager) {
  ...
  if (manager->current_plug_in)
    manager->plug_in_stack =
        g_slist_remove(manager->plug_in_stack, manager->plug_in_stack->data);
