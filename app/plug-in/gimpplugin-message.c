...
static void gimp_plug_in_handle_tile_put(GimpPlugIn *plug_in,
  ...
  if (!gp_tile_data_write(plug_in->my_write, &tile_data, plug_in)) {
    gimp_message(plug_in->manager->gimp, NULL, GIMP_MESSAGE_ERROR, "%s: ERROR",
                 G_STRFUNC);
    gimp_plug_in_close(plug_in, TRUE);
    return;
  }
  ...
  if (!gimp_gegl_buffer_get_tile_rect(buffer, GIMP_PLUG_IN_TILE_WIDTH,
                                      GIMP_PLUG_IN_TILE_HEIGHT,
                                      tile_info->tile_num, &tile_rect)) {
    gimp_message(plug_in->manager->gimp, NULL, GIMP_MESSAGE_ERROR,
                 "Plug-in \"%s\"\n(%s)\n\n"
                 "requested invalid tile #%d for writing (killing)",
                 gimp_object_get_name(plug_in),
                 gimp_file_get_utf8_name(plug_in->file), tile_info->tile_num);
    gimp_plug_in_close(plug_in, TRUE);
    return;
  }
  ...
  if (tile_data.use_shm) {
    ...
    gegl_buffer_set(buffer, &tile_rect, 0, format, tile_info->data,
                    GEGL_AUTO_ROWSTRIDE);
  }
...
static void gimp_plug_in_handle_tile_get(GimpPlugIn *plug_in,
  ...
  if (!GIMP_IS_DRAWABLE(drawable)) {
    gimp_message(plug_in->manager->gimp, NULL, GIMP_MESSAGE_ERROR,
                 ...
                 gimp_object_get_name(plug_in),
                 gimp_file_get_utf8_name(plug_in->file), request->drawable_id);
    gimp_plug_in_close(plug_in, TRUE);
    return;
  } else if (gimp_item_is_removed(GIMP_ITEM(drawable))) {
    gimp_message(plug_in->manager->gimp, NULL, GIMP_MESSAGE_ERROR,
  ...
  if (!gimp_gegl_buffer_get_tile_rect(buffer, GIMP_PLUG_IN_TILE_WIDTH,
                                      GIMP_PLUG_IN_TILE_HEIGHT,
                                      request->tile_num, &tile_rect)) {
    gimp_message(plug_in->manager->gimp, NULL, GIMP_MESSAGE_ERROR,
                 "Plug-in \"%s\"\n(%s)\n\n"
                 "requested invalid tile #%d for reading (killing)",
                 gimp_object_get_name(plug_in),
                 gimp_file_get_utf8_name(plug_in->file), request->tile_num);
    gimp_plug_in_close(plug_in, TRUE);
    return;
  }
  ...
  tile_size = (babl_format_get_bytes_per_pixel(format) * tile_rect.width *
               tile_rect.height);
  ...
  if (tile_data.use_shm) {
    ...
    gegl_buffer_get(buffer, &tile_rect, 1.0, format, tile_data.data,
                    GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
  }
...
static void gimp_plug_in_handle_proc_error(GimpPlugIn *plug_in,
...
static void gimp_plug_in_handle_proc_run(GimpPlugIn *plug_in,
  ...
  procedure = gimp_pdb_lookup_procedure(plug_in->manager->gimp->pdb, canonical);
  ...
  if (!procedure) {
    ...
    if (proc_name) {
      procedure =
          gimp_pdb_lookup_procedure(plug_in->manager->gimp->pdb, proc_name);
      ...
      if (plug_in->manager->gimp->pdb_compat_mode == GIMP_PDB_COMPAT_WARN) {
        gimp_message(plug_in->manager->gimp, NULL, GIMP_MESSAGE_WARNING,
                     ...
                     gimp_object_get_name(plug_in),
                     gimp_file_get_utf8_name(plug_in->file), canonical,
                     proc_name);
      }
    }
  ...
  args = _gimp_gp_params_to_value_array(
      plug_in->manager->gimp, procedure ? procedure->args : NULL,
      procedure ? procedure->num_args : 0, proc_run->params, proc_run->n_params,
      FALSE);
  ...
  return_vals = gimp_pdb_execute_procedure_by_name_args(
      plug_in->manager->gimp->pdb,
      proc_frame->context_stack ? proc_frame->context_stack->data
                                : proc_frame->main_context,
      proc_frame->progress, &error, proc_name, args);
  gimp_plug_in_manager_plug_in_pop(plug_in->manager);

  gimp_value_array_unref(args);

  if (error) {
    gimp_plug_in_handle_proc_error(plug_in, proc_frame, canonical, error);
    g_error_free(error);
  }
...
static void gimp_plug_in_handle_proc_return(GimpPlugIn *plug_in,
  ...
  proc_frame->return_vals = _gimp_gp_params_to_value_array(
      plug_in->manager->gimp, proc_frame->procedure->values,
      proc_frame->procedure->num_values, proc_return->params,
      proc_return->n_params, TRUE);
  ...
  if (proc_frame->main_loop) {
    ...
    gimp_plug_in_procedure_handle_return_values(
        GIMP_PLUG_IN_PROCEDURE(proc_frame->procedure), plug_in->manager->gimp,
        proc_frame->progress, proc_frame->return_vals);
  }
...
static void gimp_plug_in_handle_proc_install(GimpPlugIn *plug_in,
  ...
  if (!gimp_is_canonical_identifier(proc_install->name)) {
    gimp_message(plug_in->manager->gimp, NULL, GIMP_MESSAGE_ERROR,
                 ...
                 "non-canonical name.",
                 gimp_object_get_name(plug_in),
                 gimp_file_get_utf8_name(plug_in->file), proc_install->name);
    return;
  }
...
#define VALIDATE(str) ((str) == NULL || g_utf8_validate((str), -1, NULL))
  ...
  for (i = 0; i < proc_install->n_return_vals && valid_utf8 && !null_name;
       i++) {
...
#undef VALIDATE
  ...
  case GIMP_PDB_PROC_TYPE_EXTENSION:
    procedure = gimp_plug_in_procedure_new(proc_install->type, plug_in->file);
    break;
...
gimp_plug_in_handle_proc_uninstall(GimpPlugIn *plug_in,
  ...
  if (!gimp_is_canonical_identifier(proc_uninstall->name)) {
    gimp_message(plug_in->manager->gimp, NULL, GIMP_MESSAGE_ERROR,
                 ...
                 "non-canonical name.",
                 gimp_object_get_name(plug_in),
                 gimp_file_get_utf8_name(plug_in->file), proc_uninstall->name);
    return;
  }
