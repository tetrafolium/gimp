...
static gboolean gui_message_error_console(Gimp *gimp,
                                          ...
                                          const gchar *domain,
                                          const gchar *message);
static gboolean gui_message_error_dialog(Gimp *gimp, GObject *handler,
                                         GimpMessageSeverity severity,
                                         const gchar *domain,
                                         const gchar *message,
                                         const gchar *trace);
...
static void gui_message_reset_errors(GObject *object, gpointer user_data);
...
void gui_message(Gimp *gimp, GObject *handler, GimpMessageSeverity severity,
  ...
  case GIMP_MESSAGE_BOX:
    ...
    if (g_strcmp0(GIMP_ACRONYM, domain) != 0) {
      ...
      gdk_threads_add_idle_full(G_PRIORITY_DEFAULT_IDLE, gui_message_idle, data,
                                g_free);
      return;
    }
...
static gboolean gui_message_idle(gpointer user_data) {
  ...
  if (!gui_message_error_dialog(data->gimp, data->handler, data->severity,
                                data->domain, data->message, data->trace)) {
    gui_message_console(data->severity, data->domain, data->message);
  }
  g_free(data->domain);
  g_free(data->message);
  if (data->trace)
...
static gboolean gui_message_error_console(Gimp *gimp,
  ...
  dockable = gimp_dialog_factory_find_widget(
      gimp_dialog_factory_get_singleton(), "gimp-error-console");
  ...
  if (!dockable) {
    ...
    dockable = gimp_window_strategy_show_dockable_dialog(
        ...
        gimp_dialog_factory_get_singleton(), monitor, "gimp-error-console");
  }

  if (dockable) {
    GtkWidget *child = gtk_bin_get_child(GTK_BIN(dockable));

    gimp_error_console_add(GIMP_ERROR_CONSOLE(child), severity, domain,
                           message);
...
static void progress_error_dialog_unset(GimpProgress *progress) {
  g_object_set_data(G_OBJECT(progress), "gimp-error-dialog", NULL);
}

static GtkWidget *progress_error_dialog(GimpProgress *progress) {
  GtkWidget *dialog;
  ...
  if (!dialog) {
    ...
    g_signal_connect_object(dialog, "destroy",
                            G_CALLBACK(progress_error_dialog_unset), progress,
                            G_CONNECT_SWAPPED);
...
static gboolean gui_message_error_dialog(Gimp *gimp, GObject *handler,
  ...
  if (severity >= GIMP_MESSAGE_BUG_WARNING) {
    ...
    if (!g_object_get_data(handler, "gimp-error-dialog") &&
        gimp_progress_message(GIMP_PROGRESS(handler), gimp, severity, domain,
                              message)) {
      return TRUE;
    }
  ...
  if (dialog) {
    ...
    gimp_error_dialog_add(GIMP_ERROR_DIALOG(dialog),
                          gimp_get_message_icon_name(severity), domain,
                          message);
...
static GtkWidget *global_error_dialog(void) {
  ...
  return gimp_dialog_factory_dialog_new(gimp_dialog_factory_get_singleton(),
                                        monitor, NULL /*ui_manager*/, NULL,
                                        "gimp-error-dialog", -1, FALSE);
}
...
static GtkWidget *global_critical_dialog(void) {
  ...
  dialog = gimp_dialog_factory_dialog_new(gimp_dialog_factory_get_singleton(),
                                          monitor, NULL /*ui_manager*/, NULL,
                                          "gimp-critical-dialog", -1, FALSE);
  g_signal_handlers_disconnect_by_func(dialog, gui_message_reset_errors, NULL);
  g_signal_connect(dialog, "destroy", G_CALLBACK(gui_message_reset_errors),
                   NULL);
  return dialog;
}
