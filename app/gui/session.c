  ...
  while (g_scanner_peek_next_token(scanner) == token) {
    ...
    case G_TOKEN_SYMBOL:
      if (scanner->value.v_symbol == GINT_TO_POINTER(SESSION_INFO)) {
        ...
        if (entry_name) {
          ...
          if (strcmp(entry_name, "dock") == 0) {
            entry = gimp_dialog_factory_find_entry(
                factory,
                (strcmp(factory_name, "toolbox") == 0 ? "gimp-toolbox-window"
                                                      : "gimp-dock-window"));
          } else {
            entry = gimp_dialog_factory_find_entry(factory, entry_name);
          }
        }
        ...
        if (gimp_config_deserialize(GIMP_CONFIG(info), scanner, 1, NULL)) {
          ...
          if (gimp_session_info_get_factory_entry(info)) {
            GIMP_LOG(DIALOG_FACTORY,
                     "successfully parsed and added session info %p", info);

            gimp_dialog_factory_add_session_info(factory, info);
          } else {
            GIMP_LOG(DIALOG_FACTORY,
                     "failed to parse session info %p, not adding", info);
          }
        ...
        g_object_set(gimp->config, "hide-docks", hide_docks, NULL);
      } else if (scanner->value.v_symbol ==
                 GINT_TO_POINTER(SINGLE_WINDOW_MODE)) {
        ...
        g_object_set(gimp->config, "single-window-mode", single_window_mode,
                     NULL);
        ...
        g_object_set(gimp->config, "show-tabs", show_tabs, NULL);
        ...
        g_object_set(gimp->config, "tabs-position", tabs_position, NULL);
        ...
        g_object_set(gimp->config, "last-tip-shown", last_tip_shown, NULL);
      }
...
void session_exit(Gimp *gimp) { g_return_if_fail(GIMP_IS_GIMP(gimp)); }

void session_restore(Gimp *gimp, GdkMonitor *monitor) {
  g_return_if_fail(GIMP_IS_GIMP(gimp));
  g_return_if_fail(GDK_IS_MONITOR(monitor));

  gimp_dialog_factory_restore(gimp_dialog_factory_get_singleton(), monitor);
...
void session_save(Gimp *gimp, gboolean always_save) {
  ...
  gimp_config_writer_identifier(
      writer, GIMP_GUI_CONFIG(gimp->config)->hide_docks ? "yes" : "no");
  gimp_config_writer_close(writer);

  gimp_config_writer_open(writer, "single-window-mode");
  gimp_config_writer_identifier(
      writer, GIMP_GUI_CONFIG(gimp->config)->single_window_mode ? "yes" : "no");
  gimp_config_writer_close(writer);

  gimp_config_writer_open(writer, "show-tabs");
  gimp_config_writer_printf(
      writer, GIMP_GUI_CONFIG(gimp->config)->show_tabs ? "yes" : "no");
  gimp_config_writer_close(writer);
...
gboolean session_clear(Gimp *gimp, GError **error) {
  GFile *file;
  ...
  if (!g_file_delete(file, NULL, &my_error) &&
    ...
    g_set_error(error, GIMP_ERROR, GIMP_FAILED, _("Deleting \"%s\" failed: %s"),
