...
static void gui_help_func(const gchar *help_id, gpointer help_data);
...
static void gui_restore_callback(Gimp *gimp, GimpInitStatusFunc callback);
static void gui_restore_after_callback(Gimp *gimp, GimpInitStatusFunc callback);

static gboolean gui_exit_callback(Gimp *gimp, gboolean force);
static gboolean gui_exit_after_callback(Gimp *gimp, gboolean force);

static void gui_show_help_button_notify(GimpGuiConfig *gui_config,
                                        GParamSpec *pspec, Gimp *gimp);
static void gui_user_manual_notify(GimpGuiConfig *gui_config, GParamSpec *pspec,
                                   Gimp *gimp);
...
static void gui_menu_show_tooltip(GimpUIManager *manager, const gchar *tooltip,
                                  Gimp *gimp);
static void gui_menu_hide_tooltip(GimpUIManager *manager, Gimp *gimp);

static void gui_display_changed(GimpContext *context, GimpDisplay *display,
                                Gimp *gimp);

static void gui_compare_accelerator(gpointer data, const gchar *accel_path,
                                    guint accel_key, GdkModifierType accel_mods,
                                    gboolean changed);
static void gui_check_unique_accelerator(gpointer data, const gchar *accel_path,
                                         guint accel_key,
...
void gui_abort(const gchar *abort_message) {
  ...
  dialog = gimp_dialog_new(_("GIMP Message"), "gimp-abort", NULL,
  ...
  box = g_object_new(GIMP_TYPE_MESSAGE_BOX, "icon-name", GIMP_ICON_WILBER_EEK,
...
GimpInitStatusFunc gui_init(Gimp *gimp, gboolean no_splash) {
  ...
  g_signal_connect_after(gimp, "initialize",
                         G_CALLBACK(gui_initialize_after_callback), NULL);

  g_signal_connect(gimp, "restore", G_CALLBACK(gui_restore_callback), NULL);
  g_signal_connect_after(gimp, "restore",
                         G_CALLBACK(gui_restore_after_callback), NULL);

  g_signal_connect(gimp, "exit", G_CALLBACK(gui_exit_callback), NULL);
  g_signal_connect_after(gimp, "exit", G_CALLBACK(gui_exit_after_callback),
                         NULL);
...
gboolean gui_recover(gint n_recoveries) {
  ...
  dialog = gimp_dialog_new(
      _("Image Recovery"), "gimp-recovery", NULL, GTK_DIALOG_MODAL, NULL, NULL,
      _("_Discard"), GTK_RESPONSE_CANCEL, _("_Recover"), GTK_RESPONSE_OK, NULL);
...
#define GTK_REQUIRED_MICRO 29
  ...
  const gchar *mismatch = gtk_check_version(
  ...
  if (mismatch) {
    return g_strdup_printf(
        ...
        mismatch, GTK_REQUIRED_MAJOR, GTK_REQUIRED_MINOR, GTK_REQUIRED_MICRO,
...
static void gui_initialize_after_callback(Gimp *gimp,
  ...
  if (name) {
    ...
    gimp_environ_table_add(gimp->plug_in_manager->environ_table, name, display,
                           NULL);
  }
...
static void gui_restore_callback(Gimp *gimp,
  ...
  if (display_config->monitor_res_from_gdk ||
    ...
    g_object_set(gimp->config, "monitor-xresolution", xres,
                 "monitor-yresolution", yres,
                 "monitor-resolution-from-windowing-system", TRUE, NULL);
  }
  ...
  g_signal_connect(gimp, "clipboard-changed", G_CALLBACK(gui_clipboard_changed),
                   NULL);
...
#ifdef GDK_WINDOWING_QUARTZ
static void gui_add_to_app_menu(GimpUIManager *ui_manager,
                                GtkosxApplication *osx_app,
                                const gchar *action_path, gint index) {
  GtkWidget *item;
...
static gboolean gui_quartz_quit_callback(GtkosxApplication *osx_app,
...
static void gui_restore_after_callback(Gimp *gimp,
  ...
  if (g_getenv("GIMP_ICONS_LIKE_A_BOSS")) {
    ...
    g_object_set(G_OBJECT(gtk_settings_get_for_screen(screen)),
                 "gtk-button-images", TRUE, "gtk-menu-images", TRUE, NULL);
  }

  if (gui_config->restore_accels)
    menus_restore(gimp);

  ui_configurer = g_object_new(GIMP_TYPE_UI_CONFIGURER, "gimp", gimp, NULL);

  image_ui_manager =
      gimp_menu_factory_manager_new(global_menu_factory, "<Image>", gimp);
  gimp_ui_manager_update(image_ui_manager, gimp);

  /* Check that every accelerator is unique. */
  gtk_accel_map_foreach_unfiltered(NULL, gui_check_unique_accelerator);
  ...
  g_signal_connect(image_ui_manager, "show-tooltip",
                   G_CALLBACK(gui_menu_show_tooltip), gimp);
  g_signal_connect(image_ui_manager, "hide-tooltip",
                   G_CALLBACK(gui_menu_hide_tooltip), gimp);
  ...
  if (gimp_get_show_gui(gimp)) {
    ...
    display = GIMP_DISPLAY(gimp_create_display(gimp, NULL, GIMP_UNIT_PIXEL, 1.0,
...
#ifdef GDK_WINDOWING_QUARTZ
    {
      ...
      menu = gimp_ui_manager_get_widget(image_ui_manager, "/image-menubar");
...
#undef PREFERENCES
      ...
      g_signal_connect(osx_app, "NSApplicationBlockTermination",
                       G_CALLBACK(gui_quartz_quit_callback), image_ui_manager);
...
static gboolean gui_exit_after_callback(Gimp *gimp, gboolean force) {
  ...
  g_signal_handlers_disconnect_by_func(gimp->config,
                                       gui_show_help_button_notify, gimp);
  g_signal_handlers_disconnect_by_func(gimp->config, gui_user_manual_notify,
                                       gimp);
   ...
   */
  g_signal_handlers_disconnect_by_func(gimp, G_CALLBACK(gui_clipboard_changed),
                                       NULL);
...
static void gui_single_window_mode_notify(GimpGuiConfig *gui_config,
                                          GParamSpec *pspec,
                                          GimpUIConfigurer *ui_configurer) {
  gimp_ui_configurer_configure(ui_configurer, gui_config->single_window_mode);
}
...
static void gui_display_changed(GimpContext *context, GimpDisplay *display,
  ...
  if (!display) {
    ...
    if (image) {
      ...
      for (list = gimp_get_display_iter(gimp); list; list = g_list_next(list)) {
...
static void gui_check_unique_accelerator(gpointer data, const gchar *accel_path,
  ...
  if (gtk_accelerator_valid(accel_key, accel_mods) &&
    ...
    gtk_accel_map_foreach_unfiltered(&accel, gui_compare_accelerator);
  }
}
