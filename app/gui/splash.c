...
static void splash_position_layouts(GimpSplash *splash, const gchar *text1,
                                    const gchar *text2, GdkRectangle *area);
static gboolean splash_area_draw(GtkWidget *widget, cairo_t *cr,
                                 GimpSplash *splash);
static void splash_rectangle_union(GdkRectangle *dest,
                                   PangoRectangle *pango_rect, gint offset_x,
                                   gint offset_y);
static void splash_average_text_area(GimpSplash *splash, GdkPixbuf *pixbuf,
                                     GdkRGBA *rgba);

static GdkPixbufAnimation *splash_image_load(Gimp *gimp, gint max_width,
                                             gint max_height,
                                             gboolean be_verbose);
static GdkPixbufAnimation *splash_image_load_from_file(GFile *file,
                                                       gint max_width,
                                                       gint max_height,
                                                       gboolean be_verbose);
static GdkPixbufAnimation *splash_image_pick_from_dirs(GList *dirs,
                                                       gint max_width,
                                                       gint max_height,
                                                       gboolean be_verbose);
...
void splash_create(Gimp *gimp, gboolean be_verbose, GdkMonitor *monitor) {
  GtkWidget *frame;
  GtkWidget *vbox;
  ...
  splash->window =
      g_object_new(GTK_TYPE_WINDOW, "type", GTK_WINDOW_TOPLEVEL, "type-hint",
                   GDK_WINDOW_TYPE_HINT_SPLASHSCREEN, "title",
                   _("GIMP Startup"), "role", "gimp-startup", "window-position",
                   GTK_WIN_POS_CENTER, "resizable", FALSE, NULL);

  g_signal_connect_swapped(splash->window, "delete-event", G_CALLBACK(exit),
                           GINT_TO_POINTER(0));

  splash->width = MIN(gdk_pixbuf_animation_get_width(pixbuf), workarea.width);
  splash->height =
      MIN(gdk_pixbuf_animation_get_height(pixbuf), workarea.height);
  ...
  splash->upper = gtk_widget_create_pango_layout(splash->area, MEASURE_UPPER);
  ...
  splash->lower = gtk_widget_create_pango_layout(splash->area, MEASURE_LOWER);
  ...
  splash_average_text_area(
      splash, gdk_pixbuf_animation_get_static_image(pixbuf), &splash->color);

  g_object_unref(pixbuf);

  g_signal_connect_after(splash->area, "draw", G_CALLBACK(splash_area_draw),
                         splash);
...
void splash_update(const gchar *text1, const gchar *text2, gdouble percentage) {
  ...
  if (expose.width > 0 && expose.height > 0)
    gtk_widget_queue_draw_area(splash->area, expose.x, expose.y, expose.width,
                               expose.height);
...
static void splash_position_layouts(GimpSplash *splash, const gchar *text1,
  ...
  if (text1) {
    ...
    if (area)
      splash_rectangle_union(area, &upper_ink, splash->upper_x,
                             splash->upper_y);
  ...
  if (text2) {
    ...
    if (area)
      splash_rectangle_union(area, &lower_ink, splash->lower_x,
                             splash->lower_y);
  ...
  if (text1) {
    splash->upper_y = MIN(splash->height - text_height,
                          splash->height * 13 / 16 - upper_ink.height / 2);
  ...
  if (text2) {
    splash->lower_y =
        ((splash->height + splash->upper_y) / 2 - lower_ink.height / 2);
...
static void splash_average_text_area(GimpSplash *splash, GdkPixbuf *pixbuf,
  ...
  if (gdk_rectangle_intersect(&image, &area, &area)) {
    ...
    luminance =
        GIMP_RGB_LUMINANCE(sum[0] / count, sum[1] / count, sum[2] / count);
...
static GdkPixbufAnimation *splash_image_load(Gimp *gimp, gint max_width,
  ...
  g_object_get(gimp->extension_manager, "splash-paths", &list, NULL);
  animation =
      splash_image_pick_from_dirs(list, max_width, max_height, be_verbose);
  if (animation)
    return animation;
  ...
  animation =
      splash_image_load_from_file(file, max_width, max_height, be_verbose);
  g_object_unref(file);
  if (animation)
    return animation;
...
static GdkPixbufAnimation *splash_image_load_from_file(GFile *file,
  ...
  if (input) {
    animation =
        gdk_pixbuf_animation_new_from_stream(G_INPUT_STREAM(input), NULL, NULL);
  ...
  if (animation && gdk_pixbuf_animation_is_static_image(animation) &&
      (gdk_pixbuf_animation_get_width(animation) > max_width ||
       gdk_pixbuf_animation_get_height(animation) > max_height || is_svg)) {
    GdkPixbuf *pixbuf;

    input = g_file_read(file, NULL, NULL);
    pixbuf = gdk_pixbuf_new_from_stream_at_scale(
        G_INPUT_STREAM(input), max_width, max_height, TRUE, NULL, NULL);
    g_object_unref(input);
    if (pixbuf) {
      GdkPixbufSimpleAnim *simple_anim = NULL;

      simple_anim = gdk_pixbuf_simple_anim_new(
          gdk_pixbuf_get_width(pixbuf), gdk_pixbuf_get_height(pixbuf), 1.0);
      if (simple_anim) {
...
static GdkPixbufAnimation *splash_image_pick_from_dirs(GList *dirs,
  ...
  if (splashes) {
    ...
    animation = splash_image_load_from_file(g_list_nth_data(splashes, i),
                                            max_width, max_height, be_verbose);
...
static void splash_timer_elapsed(void) {
  ...
  g_printerr("%8g  %8g  -  %s %g%%  -  %s\n", elapsed,
             elapsed - splash->last_time, splash->text1 ? splash->text1 : "",
             splash->percentage * 100.0, splash->text2 ? splash->text2 : "");
