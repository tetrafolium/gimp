...
static IdleOpenData *idle_open_data_new(GFile *file, gboolean as_new) {
...
static void idle_open_data_free(IdleOpenData *data) {
  g_object_unref(data->file);
  g_slice_free(IdleOpenData, data);
}

static gboolean gui_unique_win32_idle_open(IdleOpenData *data) {
  ...
  if (data->file) {
    file_open_from_command_line(unique_gimp, data->file, data->as_new, NULL);
  } else {
...
static LRESULT CALLBACK gui_unique_win32_message_handler(HWND hWnd, UINT uMsg,
                                                         WPARAM wParam,
                                                         LPARAM lParam) {
  switch (uMsg) {
  case WM_COPYDATA:
    if (unique_gimp) {
      ...
      if (copydata->cbData > 0) {
        ...
        data = idle_open_data_new(file, copydata->dwData != 0);

        g_object_unref(file);

        closure = g_cclosure_new(G_CALLBACK(gui_unique_win32_idle_open), data,
      ...
      if (display)
        gimp_display_shell_present(
            gimp_display_get_shell(GIMP_DISPLAY(display)));
    }
    return TRUE;
...
static void gui_unique_win32_init(Gimp *gimp) {
  ...
  proxy_window = CreateWindowExW(0, GIMP_UNIQUE_WIN32_WINDOW_CLASS,
                                 GIMP_UNIQUE_WIN32_WINDOW_NAME, WS_POPUP, 0, 0,
                                 1, 1, NULL, NULL, wc.hInstance, NULL);
}
...
static gboolean gui_unique_quartz_idle_open(GFile *file) {
  /*  We want to be called again later in case that GIMP is not fully
   *  started yet.
   */
  if (!gimp_is_restored(unique_gimp))
...
static gboolean
gui_unique_quartz_nsopen_file_callback(GtkosxApplication *osx_app, gchar *path,
                                       gpointer user_data) {
  GSource *source;
  GClosure *closure;

  closure =
      g_cclosure_new(G_CALLBACK(gui_unique_quartz_idle_open),
...
static void gui_unique_quartz_init(Gimp *gimp) {
  ...
  g_signal_connect(osx_app, "NSApplicationOpenFile",
                   G_CALLBACK(gui_unique_quartz_nsopen_file_callback), gimp);
...
static void gui_dbus_bus_acquired(GDBusConnection *connection,
                                  const gchar *name, Gimp *gimp) {
...
static void gui_dbus_name_acquired(GDBusConnection *connection,
                                   const gchar *name, Gimp *gimp) {}

static void gui_dbus_name_lost(GDBusConnection *connection, const gchar *name,
                               Gimp *gimp) {
  if (connection == NULL)
...
static void gui_dbus_service_init(Gimp *gimp) {
  ...
  dbus_name_id = g_bus_own_name(
      G_BUS_TYPE_SESSION, GIMP_DBUS_SERVICE_NAME, G_BUS_NAME_OWNER_FLAGS_NONE,
      (GBusAcquiredCallback)gui_dbus_bus_acquired,
      (GBusNameAcquiredCallback)gui_dbus_name_acquired,
      (GBusNameLostCallback)gui_dbus_name_lost, gimp, NULL);
}
