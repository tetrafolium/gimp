...
static void gimp_ui_configurer_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_ui_configurer_get_property(GObject *object, guint property_id,
...
static void gimp_ui_configurer_class_init(GimpUIConfigurerClass *klass) {
...
static void gimp_ui_configurer_init(GimpUIConfigurer *ui_configurer) {
...
static void gimp_ui_configurer_get_window_center_pos(GtkWindow *window,
...
gimp_ui_configurer_move_docks_to_columns(GimpUIConfigurer *ui_configurer,
  ...
  dialogs = g_list_copy(gimp_dialog_factory_get_open_dialogs(
  ...
  for (dialog_iter = dialogs; dialog_iter; dialog_iter = dialog_iter->next) {
     ...
     * window.
     */
    if (gimp_ui_configurer_get_relative_window_pos(
            GTK_WINDOW(uber_image_window), GTK_WINDOW(dock_window)) ==
        GIMP_ALIGN_LEFT)
    ...
    g_object_add_weak_pointer(G_OBJECT(dock_window), (gpointer)&dock_window);
...
gimp_ui_configurer_move_docks_to_window(GimpUIConfigurer *ui_configurer,
  ...
  dock_window = gimp_dialog_factory_dialog_new(
      gimp_dialog_factory_get_singleton(), monitor, NULL /*ui_manager*/,
      GTK_WIDGET(dock_columns),
      (contains_toolbox ? "gimp-toolbox-window" : "gimp-dock-window"),
  ...
  gtk_window_set_default_size(GTK_WINDOW(dock_window), original_size.width,
...
gimp_ui_configurer_separate_shells(GimpUIConfigurer *ui_configurer,
  ...
  GimpDisplayShell *active_shell =
      gimp_image_window_get_active_shell(source_image_window);
  ...
  while (gimp_image_window_get_n_shells(source_image_window) > 1) {
    ...
    new_image_window = gimp_image_window_new(
        ui_configurer->p->gimp, NULL, gimp_dialog_factory_get_singleton(),
 ...
 **/
static void gimp_ui_configurer_configure_for_single_window(
  ...
  gimp_ui_configurer_move_docks_to_columns(ui_configurer, uber_image_window);
  ...
  for (iter = windows; iter; iter = g_list_next(iter)) {
     ...
     */
    gimp_ui_configurer_move_shells(ui_configurer, image_window,
                                   uber_image_window);
    /* Destroy the window */
