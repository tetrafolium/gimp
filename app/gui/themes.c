...
static void themes_apply_theme(Gimp *gimp, GimpGuiConfig *config);
static void themes_list_themes_foreach(gpointer key, gpointer value,
                                       gpointer data);
static gint themes_name_compare(const void *p1, const void *p2);
static void themes_theme_change_notify(GimpGuiConfig *config, GParamSpec *pspec,
                                       Gimp *gimp);
...
void themes_init(Gimp *gimp) {
  ...
  g_signal_connect(gimp->extension_manager, "notify::theme-paths",
                   G_CALLBACK(themes_theme_paths_notify), gimp);
  ...
  gtk_style_context_add_provider_for_screen(
      gdk_screen_get_default(), themes_style_provider,
      GTK_STYLE_PROVIDER_PRIORITY_APPLICATION + 1);

  g_object_unref(themes_style_provider);

  g_signal_connect(config, "notify::theme",
                   G_CALLBACK(themes_theme_change_notify), gimp);
...
void themes_exit(Gimp *gimp) {
  ...
  if (themes_hash) {
    g_signal_handlers_disconnect_by_func(gimp->config,
                                         themes_theme_change_notify, gimp);
...
gchar **themes_list_themes(Gimp *gimp, gint *n_themes) {
...
GFile *themes_get_theme_dir(Gimp *gimp, const gchar *theme_name) {
  g_return_val_if_fail(GIMP_IS_GIMP(gimp), NULL);
...
GFile *themes_get_theme_file(Gimp *gimp, const gchar *first_component, ...) {
...
static void themes_apply_theme(Gimp *gimp, GimpGuiConfig *config) {
  ...
  output = G_OUTPUT_STREAM(
      g_file_replace(theme_css, NULL, FALSE, G_FILE_CREATE_NONE, NULL, &error));
  if (!output) {
    ...
    if (theme_dir) {
      css_files =
          g_slist_prepend(css_files, g_file_get_child(theme_dir, "gimp.css"));
    } else {
      gchar *tmp;

      tmp = g_build_filename(gimp_data_directory(), "themes", "System",
                             "gimp.css", NULL);
      css_files = g_slist_prepend(css_files, g_file_new_for_path(tmp));
      g_free(tmp);
    }

    css_files = g_slist_prepend(css_files,
                                gimp_sysconf_directory_file("gimp.css", NULL));

    css_user = gimp_directory_file("gimp.css", NULL);
    css_files = g_slist_prepend(css_files, css_user);
    ...
    if (error) {
      ...
      gimp_message(gimp, NULL, GIMP_MESSAGE_ERROR, _("Error writing '%s': %s"),
      ...
      gimp_message(gimp, NULL, GIMP_MESSAGE_ERROR, _("Error closing '%s': %s"),
                   gimp_file_get_utf8_name(theme_css), error->message);
      g_clear_error(&error);
    }
...
static void themes_theme_paths_notify(GimpExtensionManager *manager,
  ...
  if (config->theme_path) {
    ...
    g_object_get(gimp->extension_manager, "theme-paths", &path, NULL);
