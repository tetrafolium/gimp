...
static void gimp_dbus_service_gimp_opened(Gimp *gimp, GFile *file,
                                          GimpDBusService *service);

static gboolean gimp_dbus_service_queue_open(GimpDBusService *service,
                                             const gchar *uri, gboolean as_new);
...
GObject *gimp_dbus_service_new(Gimp *gimp) {
  ...
  g_signal_connect_object(gimp, "image-opened",
                          G_CALLBACK(gimp_dbus_service_gimp_opened), service,
                          0);
...
gboolean gimp_dbus_service_activate(GimpDBusServiceUI *service,
                                    GDBusMethodInvocation *invocation) {
...
gboolean gimp_dbus_service_open(GimpDBusServiceUI *service,
                                GDBusMethodInvocation *invocation,
                                const gchar *uri) {
  gboolean success;

  success =
      gimp_dbus_service_queue_open(GIMP_DBUS_SERVICE(service), uri, FALSE);
...
gboolean gimp_dbus_service_open_as_new(GimpDBusServiceUI *service,
                                       GDBusMethodInvocation *invocation,
                                       const gchar *uri) {
  gboolean success;

  success = gimp_dbus_service_queue_open(GIMP_DBUS_SERVICE(service), uri, TRUE);
...
gboolean gimp_dbus_service_batch_run(GimpDBusServiceUI *service,
                                     GDBusMethodInvocation *invocation,
                                     const gchar *batch_interpreter,
                                     const gchar *batch_command) {
  gboolean success;
  ...
  success = gimp_dbus_service_queue_batch(GIMP_DBUS_SERVICE(service),
                                          batch_interpreter, batch_command);
...
static gboolean gimp_dbus_service_queue_open(GimpDBusService *service,
  ...
  if (!service->source) {
    ...
    g_source_set_callback(service->source,
                          (GSourceFunc)gimp_dbus_service_process_idle, service,
                          NULL);
...
static gboolean gimp_dbus_service_queue_batch(GimpDBusService *service,
                                              const gchar *interpreter,
                                              const gchar *command) {
  g_queue_push_tail(service->queue, gimp_dbus_service_batch_data_new(
                                        service, interpreter, command));
...
static gboolean gimp_dbus_service_process_idle(GimpDBusService *service) {
  ...
  if (data) {
    ...
    if (data->command) {
      ...
      gimp_batch_run(service->gimp, data->interpreter, commands);
    }
