...
static void gui_help(Gimp *gimp, GimpProgress *progress,
                     const gchar *help_domain, const gchar *help_id);
static const gchar *gui_get_program_class(Gimp *gimp);
static gchar *gui_get_display_name(Gimp *gimp, gint display_id,
                                   GObject **monitor, gint *monitor_number);
...
static GimpDisplay *gui_display_create(Gimp *gimp, GimpImage *image,
                                       GimpUnit unit, gdouble scale,
                                       GObject *monitor);
static void gui_display_delete(GimpDisplay *display);
static void gui_displays_reconnect(Gimp *gimp, GimpImage *old_image,
                                   GimpImage *new_image);
static gboolean gui_wait(Gimp *gimp, GimpWaitable *waitable,
                         const gchar *message);
static GimpProgress *gui_new_progress(Gimp *gimp, GimpDisplay *display);
static void gui_free_progress(Gimp *gimp, GimpProgress *progress);
static gboolean gui_pdb_dialog_new(Gimp *gimp, GimpContext *context,
                                   GimpProgress *progress,
                                   GimpContainer *container, const gchar *title,
                                   const gchar *callback_name,
                                   const gchar *object_name, va_list args);
static gboolean gui_pdb_dialog_set(Gimp *gimp, GimpContainer *container,
                                   const gchar *callback_name,
                                   const gchar *object_name, va_list args);
static gboolean gui_pdb_dialog_close(Gimp *gimp, GimpContainer *container,
                                     const gchar *callback_name);
static gboolean gui_recent_list_add_file(Gimp *gimp, GFile *file,
...
gui_query_profile_policy(Gimp *gimp, GimpImage *image, GimpContext *context,
                         GimpColorProfile **dest_profile,
                         GimpColorRenderingIntent *intent, gboolean *bpc,
                         gboolean *dont_ask);
static GimpMetadataRotationPolicy
gui_query_rotation_policy(Gimp *gimp, GimpImage *image, GimpContext *context,
                          gboolean *dont_ask);
...
static gint get_monitor_number(GdkMonitor *monitor) {
...
static GFile *gui_get_icon_theme_dir(Gimp *gimp) {
  return icon_themes_get_theme_dir(gimp,
                                   GIMP_GUI_CONFIG(gimp->config)->icon_theme);
}
...
static GimpDisplay *gui_display_create(Gimp *gimp, GimpImage *image,
  ...
  if (display) {
    ...
    display = gimp_display_new(gimp, image, unit, scale, image_managers->data,
...
static void gui_wait_input_async(GimpAsync *async, const gint input_pipe[2]) {
  guint8 buffer[1];

  while (read(input_pipe[0], buffer, sizeof(buffer)) == -1 && errno == EINTR)
    ;
...
static gboolean gui_wait(Gimp *gimp, GimpWaitable *waitable,
  ...
  g_value_set_int(gimp_value_array_index(args, 1), output_pipe[0]);
  g_value_set_int(gimp_value_array_index(args, 2), input_pipe[1]);
  g_value_set_string(gimp_value_array_index(args, 3), message);
  ...
  if (GIMP_IS_CANCELABLE(waitable)) {
    ...
    input_async = gimp_parallel_run_async_independent(
        (GimpRunAsyncFunc)gui_wait_input_async, input_pipe);
...
static gboolean gui_pdb_dialog_present(GtkWindow *window) {
...
static gboolean gui_pdb_dialog_new(Gimp *gimp, GimpContext *context,
  ...
  } else if (gimp_container_get_children_type(container) ==
             GIMP_TYPE_GRADIENT) {
  ...
  if (dialog_type != G_TYPE_NONE) {
    ...
    if (!object)
      object = gimp_context_get_by_type(
    ...
    if (object) {
      ...
      names = gimp_properties_append(
          dialog_type, &n_properties, names, &values, "title", title, "role",
          dialog_role, "help-func", gimp_standard_help_func, "help-id", help_id,
          "pdb", gimp->pdb, "context", context, "select-type",
          gimp_container_get_children_type(container), "initial-object", object,
          "callback-name", callback_name, "menu-factory", global_menu_factory,
          NULL);

      names = gimp_properties_append_valist(dialog_type, &n_properties, names,
                                            &values, args);

      dialog = (GtkWidget *)g_object_new_with_properties(
          dialog_type, n_properties, (const gchar **)names,
          (const GValue *)values);
...
static gboolean gui_recent_list_add_file(Gimp *gimp, GFile *file,
  ...
  recent.mime_type =
      (mime_type ? (gchar *)mime_type : "application/octet-stream");
  ...
  success = gtk_recent_manager_add_full(gtk_recent_manager_get_default(), uri,
                                        &recent);
...
static gint gui_recent_list_compare(gconstpointer a, gconstpointer b) {
...
static void gui_recent_list_load(Gimp *gimp) {
  ...
  for (list = items; list; list = list->next) {
    ...
    if (gtk_recent_info_has_application(info,
      ...
      if (mime_type && gimp_plug_in_manager_file_procedure_find_by_mime_type(
                           gimp->plug_in_manager,
                           GIMP_FILE_PROCEDURE_GROUP_OPEN, mime_type)) {
...
gui_query_profile_policy(Gimp *gimp, GimpImage *image, GimpContext *context,
                         ...
                         GimpColorRenderingIntent *intent, gboolean *bpc,
                         gboolean *dont_ask) {
  return color_profile_import_dialog_run(image, context, NULL, dest_profile,
                                         intent, bpc, dont_ask);
}
...
gui_query_rotation_policy(Gimp *gimp, GimpImage *image, GimpContext *context,
                          gboolean *dont_ask) {
  return metadata_rotation_import_dialog_run(image, context, NULL, dont_ask);
}
