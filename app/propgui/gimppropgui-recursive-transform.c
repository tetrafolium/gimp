...
static void add_transform(GtkButton *button, GObject *config) {
  gchar *transform;
  gchar *new_transform;

  g_object_get(config, "transform", &transform, NULL);

  new_transform =
      g_strdup_printf("%s;matrix (1, 0, 0, 0, 1, 0, 0, 0, 1)", transform);

  g_object_set(config, "transform", new_transform, NULL);

  g_free(transform);
  g_free(new_transform);
}

static void duplicate_transform(GtkButton *button, GObject *config) {
  gchar *transform;
  gchar *new_transform;
  gchar *delim;
...
static void remove_transform(GtkButton *button, GObject *config) {
  gchar *transform;
  gchar *delim;
  ...
  if (delim) {
    *delim = '\0';
    ...
    g_object_set(config, "transform", transform, NULL);
  }

  g_free(transform);
}

static void transform_grids_callback(GObject *config, GeglRectangle *area,
  ...
  g_object_set(config, "transform", transform_str, NULL);
...
static void config_notify(GObject *config, const GParamSpec *pspec,
  ...
  duplicate_button =
      g_object_get_data(G_OBJECT(config), "duplicate-transform-button");
  remove_button =
      g_object_get_data(G_OBJECT(config), "remove-transform-button");
  set_func = g_object_get_data(G_OBJECT(config), "set-func");
  area = g_object_get_data(G_OBJECT(config), "area");

  g_object_get(config, "transform", &transform_str, NULL);
  ...
  for (i = 0; i < n_transforms; i++) {
    gegl_matrix3_parse_string((GeglMatrix3 *)&transforms[i], transform_strs[i]);
  }
...
GtkWidget *_gimp_prop_gui_new_recursive_transform(
  ...
  if (create_controller_func) {
    ...
    gimp_help_set_help_data(button, _("Add transform"), NULL);
    gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
    gtk_widget_show(button);

    image =
        gtk_image_new_from_icon_name(GIMP_ICON_LIST_ADD, GTK_ICON_SIZE_MENU);
    gtk_container_add(GTK_CONTAINER(button), image);
    gtk_widget_show(image);

    g_signal_connect(button, "clicked", G_CALLBACK(add_transform), config);

    g_object_set_data(config, "add-transform-button", button);

    button = gtk_button_new();
    gimp_help_set_help_data(button, _("Duplicate transform"), NULL);
    gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
    gtk_widget_show(button);
    ...
    g_signal_connect(button, "clicked", G_CALLBACK(duplicate_transform),
                     config);

    g_object_set_data(config, "duplicate-transform-button", button);

    button = gtk_button_new();
    gimp_help_set_help_data(button, _("Remove transform"), NULL);
    gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
    gtk_widget_show(button);

    image =
        gtk_image_new_from_icon_name(GIMP_ICON_LIST_REMOVE, GTK_ICON_SIZE_MENU);
    gtk_container_add(GTK_CONTAINER(button), image);
    gtk_widget_show(image);

    g_signal_connect(button, "clicked", G_CALLBACK(remove_transform), config);
    ...
    set_func = create_controller_func(
        creator, GIMP_CONTROLLER_TYPE_TRANSFORM_GRIDS,
        _("Recursive Transform: "), (GCallback)transform_grids_callback, config,
        &set_data);
