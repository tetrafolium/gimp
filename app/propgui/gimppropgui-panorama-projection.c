...
static void gyroscope_callback(GObject *config, GeglRectangle *area,
                               gdouble yaw, gdouble pitch, gdouble roll,
                               gdouble zoom, gboolean invert) {
  g_object_set_data_full(G_OBJECT(config), "area",
                         g_memdup(area, sizeof(GeglRectangle)),
                         (GDestroyNotify)g_free);

  g_object_set(config, "pan", -yaw, "tilt", -pitch, "spin", -roll, "zoom",
               CLAMP(100.0 * zoom, 0.01, 1000.0), "inverse", invert, NULL);
}
...
static void config_notify(GObject *config, const GParamSpec *pspec,
  ...
  g_object_get(config, "pan", &pan, "tilt", &tilt, "spin", &spin, "zoom", &zoom,
               "inverse", &inverse, NULL);

  set_func(set_data, area, -pan, -tilt, -spin, zoom / 100.0, inverse);
}

GtkWidget *_gimp_prop_gui_new_panorama_projection(
  ...
  if (create_controller_func) {
    ...
    set_func = create_controller_func(
        creator, GIMP_CONTROLLER_TYPE_GYROSCOPE, _("Panorama Projection: "),
        (GCallback)gyroscope_callback, config, &set_data);
