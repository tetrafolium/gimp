...
static void create_levels_scale(GObject *config, const gchar *property_name,
                                const gchar *left, const gchar *right,
                                GtkWidget *grid, gint col) {
  GtkWidget *label;
  GtkWidget *scale;
  ...
  scale = gimp_prop_spin_scale_new(config, property_name, NULL, 0.01, 0.1, 0);
...
GtkWidget *_gimp_prop_gui_new_color_balance(
  ...
  create_levels_scale(config, "cyan-red", _("Cyan"), _("Red"), grid, 0);

  create_levels_scale(config, "magenta-green", _("Magenta"), _("Green"), grid,
                      1);

  create_levels_scale(config, "yellow-blue", _("Yellow"), _("Blue"), grid, 2);
  ...
  button = gimp_prop_check_button_new(config, "preserve-luminosity",
