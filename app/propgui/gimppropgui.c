...
static gboolean gimp_prop_string_to_boolean(GBinding *binding,
                                            ...
                                            GValue *to_value,
                                            gpointer user_data);
static void gimp_prop_config_notify(GObject *config, GParamSpec *pspec,
                                    GtkWidget *widget);
static void gimp_prop_widget_show(GtkWidget *widget, GObject *config);
...
GtkWidget *gimp_prop_widget_new(GObject *config, const gchar *property_name,
  ...
  pspec =
      g_object_class_find_property(G_OBJECT_GET_CLASS(config), property_name);

  return gimp_prop_widget_new_from_pspec(
      config, pspec, area, context, create_picker_func, create_controller_func,
      creator, label);
}
...
gimp_prop_widget_new_from_pspec(GObject *config, GParamSpec *pspec,
  ...
  *label = NULL;
  ...
  if (GEGL_IS_PARAM_SPEC_SEED(pspec)) {
    ...
    *label = g_param_spec_get_nick(pspec);
  } else if (G_IS_PARAM_SPEC_INT(pspec) || G_IS_PARAM_SPEC_UINT(pspec) ||
    ...
    if (GEGL_IS_PARAM_SPEC_DOUBLE(pspec)) {
      ...
      _gimp_prop_widgets_get_numeric_values(config, pspec, &value, &lower,
                                            &upper, G_STRFUNC);
      ...
      if ((upper - lower <= 1.0) &&
        ...
        page = 0.1;
        digits = 4;
      } else if ((upper - lower <= 10.0) && (G_IS_PARAM_SPEC_FLOAT(pspec) ||
        ...
        step = 1.0;
        page = 10.0;
        digits = (G_IS_PARAM_SPEC_FLOAT(pspec) || G_IS_PARAM_SPEC_DOUBLE(pspec))
                     ? 2
                     : 0;
      }
    }
    ...
    widget =
        gimp_prop_spin_scale_new(config, pspec->name, NULL, step, page, digits);

    if (HAS_KEY(pspec, "unit", "degree") && (upper - lower) == 360.0) {
      GtkWidget *hbox;
      GtkWidget *dial;
      ...
      g_object_set(dial, "clockwise-angles", HAS_KEY(pspec, "direction", "cw"),
                   NULL);
      ...
      if (area &&
          ...
          (HAS_KEY(pspec, "axis", "x") || HAS_KEY(pspec, "axis", "y"))) {
        gdouble min = lower;
        gdouble max = upper;
        ...
        gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(widget), min, max);
      }
    }
    ...
    *label = g_param_spec_get_nick(pspec);
    ...
    if (GIMP_IS_PARAM_SPEC_CONFIG_PATH(pspec)) {
      widget = gimp_prop_file_chooser_button_new(config, pspec->name,
      ...
      gimp_message_box_set_primary_text(GIMP_MESSAGE_BOX(widget), "%s", *label);
      gimp_message_box_set_text(GIMP_MESSAGE_BOX(widget), "%s", "");

      l = GIMP_MESSAGE_BOX(widget)->label[1];

      g_object_bind_property(config, pspec->name, l, "label",
                             G_BINDING_SYNC_CREATE);
      g_object_bind_property_full(
          config, pspec->name, widget, "visible", G_BINDING_SYNC_CREATE,
          gimp_prop_string_to_boolean, NULL, NULL, NULL);
      *label = NULL;
    } else {
    ...
    button = gimp_prop_color_button_new(
        config, pspec->name, g_param_spec_get_nick(pspec), 128, 24,
        has_alpha ? GIMP_COLOR_AREA_SMALL_CHECKS : GIMP_COLOR_AREA_FLAT);
    ...
    if (create_picker_func) {
      button =
          create_picker_func(creator, pspec->name, GIMP_ICON_COLOR_PICKER_GRAY,
  ...
  if (gegl_param_spec_get_property_key(pspec, "sensitive") ||
    ...
    g_signal_connect_object(config, "notify",
                            G_CALLBACK(gimp_prop_config_notify), widget, 0);
    ...
    if (gegl_param_spec_get_property_key(pspec, "visible")) {
       ...
       * necessary.
       */
      g_signal_connect(widget, "show", G_CALLBACK(gimp_prop_widget_show),
                       config);
    }
...
typedef GtkWidget *(*GimpPropGuiNewFunc)(
...
static const struct {
  ...
  GimpPropGuiNewFunc gui_new_func;
    ...
    {"GimpColorBalanceConfig", _gimp_prop_gui_new_color_balance},
    {"GimpHueSaturationConfig", _gimp_prop_gui_new_hue_saturation},
    {"GimpGegl-gegl-color-rotate-config", _gimp_prop_gui_new_color_rotate},
    {"GimpGegl-gegl-color-to-alpha-config", _gimp_prop_gui_new_color_to_alpha},
    {"GimpGegl-gegl-convolution-matrix-config",
     _gimp_prop_gui_new_convolution_matrix},
    {"GimpGegl-gegl-channel-mixer-config", _gimp_prop_gui_new_channel_mixer},
    {"GimpGegl-gegl-diffraction-patterns-config",
     _gimp_prop_gui_new_diffraction_patterns},
    {"GimpGegl-gegl-focus-blur-config", _gimp_prop_gui_new_focus_blur},
    ...
    {"GimpGegl-gegl-newsprint-config", _gimp_prop_gui_new_newsprint},
    ...
    {"GimpGegl-gegl-spiral-config", _gimp_prop_gui_new_spiral},
    {"GimpGegl-gegl-supernova-config", _gimp_prop_gui_new_supernova},
    {"GimpGegl-gegl-vignette-config", _gimp_prop_gui_new_vignette},
...
GtkWidget *gimp_prop_gui_new(GObject *config, GType owner_type,
  ...
  param_specs = gimp_operation_config_list_properties(config, owner_type, flags,
                                                      &n_param_specs);
  ...
  if (param_specs) {
    ...
    for (i = 0; i < G_N_ELEMENTS(gui_new_funcs); i++) {
      if (!gui_new_funcs[i].config_type ||
        ...
        g_printerr("GUI new func match: %s\n",
                   gui_new_funcs[i].config_type ? gui_new_funcs[i].config_type
                                                : "generic fallback");

        gui = gui_new_funcs[i].gui_new_func(config, param_specs, n_param_specs,
                                            area, context, create_picker_func,
    ...
    gimp_label_set_attributes(GTK_LABEL(gui), PANGO_ATTR_STYLE,
                              PANGO_STYLE_ITALIC, -1);
    g_object_set(gui, "margin-top", 4, "margin-bottom", 4, NULL);
  }
...
void gimp_prop_gui_bind_container(GtkWidget *source, GtkWidget *target) {
  g_object_bind_property(source, "sensitive", target, "sensitive",
                         G_BINDING_SYNC_CREATE);
  g_object_bind_property(source, "visible", target, "visible",
                         G_BINDING_SYNC_CREATE);
}
...
void gimp_prop_gui_bind_label(GtkWidget *source, GtkWidget *target) {
   ...
   */
  g_return_if_fail(g_object_get_data(G_OBJECT(source), "gimp-prop-label-ref") ==
                   NULL);
  ...
  g_object_set_data_full(G_OBJECT(source), "gimp-prop-label-ref", label_ref,
...
void gimp_prop_gui_bind_tooltip(GtkWidget *source, GtkWidget *target) {
  g_object_bind_property(source, "tooltip-text", target, "tooltip-text",
                         G_BINDING_SYNC_CREATE);
}
...
static void gimp_prop_config_notify(GObject *config, GParamSpec *pspec,
  ...
  g_signal_handlers_block_by_func(widget, gimp_prop_widget_show, config);

  gtk_widget_set_sensitive(widget, sensitive);
  gtk_widget_set_visible(widget, visible);
  if (label_widget)
    g_object_set(label_widget, "label", label, NULL);
  gimp_help_set_help_data(widget, description, NULL);

  g_signal_handlers_unblock_by_func(widget, gimp_prop_widget_show, config);
