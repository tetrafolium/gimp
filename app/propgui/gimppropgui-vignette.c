...
static void focus_callback(GObject *config, GeglRectangle *area,
  ...
  g_object_get(config, "proportion", &proportion, NULL);
  ...
  g_object_set(config, "shape", type, "x", x / area->width, "y",
               y / area->height, "radius", 2.0 * radius / area->width,
               "proportion", proportion, "squeeze", squeeze, "rotation",
               fmod(fmod(angle * 180.0 / G_PI, 360.0) + 360.0, 360.0),
               "softness", 1.0 - inner_limit, "gamma",
               midpoint < 1.0 ? MIN(log(0.5) / log(midpoint), MAX_GAMMA)
                              : MAX_GAMMA,
               NULL);
}
...
static void config_notify(GObject *config, const GParamSpec *pspec,
  ...
  g_object_get(config, "shape", &shape, "x", &x, "y", &y, "radius", &radius,
               "proportion", &proportion, "squeeze", &squeeze, "rotation",
               &rotation, "softness", &softness, "gamma", &gamma, NULL);

  aspect_ratio = 1.0 + ((gdouble)area->height / area->width - 1.0) * proportion;
  ...
  set_func(set_data, area, shape, x * area->width, y * area->height,
           ...
           1.0 - softness, pow(0.5, 1.0 / gamma));
}

GtkWidget *_gimp_prop_gui_new_vignette(
  ...
  if (create_controller_func) {
    ...
    set_func = create_controller_func(
        creator, GIMP_CONTROLLER_TYPE_FOCUS, _("Vignette: "),
