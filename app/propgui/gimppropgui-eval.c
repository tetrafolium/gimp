...
static gboolean gimp_prop_eval_boolean_impl(GObject *config, GParamSpec *pspec,
                                            const gchar *key,
                                            gint default_value, GError **error,
                                            gint depth);
static gboolean gimp_prop_eval_boolean_or(GObject *config, GParamSpec *pspec,
                                          const gchar **expr, gchar **t,
                                          GError **error, gint depth);
static gboolean gimp_prop_eval_boolean_and(GObject *config, GParamSpec *pspec,
                                           const gchar **expr, gchar **t,
                                           GError **error, gint depth);
static gboolean gimp_prop_eval_boolean_not(GObject *config, GParamSpec *pspec,
                                           const gchar **expr, gchar **t,
                                           GError **error, gint depth);
static gboolean gimp_prop_eval_boolean_group(GObject *config, GParamSpec *pspec,
                                             const gchar **expr, gchar **t,
                                             GError **error, gint depth);
...
static gchar *gimp_prop_eval_string_impl(GObject *config, GParamSpec *pspec,
                                         const gchar *key,
...
static gchar *gimp_prop_eval_string_selection(GObject *config,
                                              ...
                                              const gchar **expr, gchar **t,
                                              GError **error, gint depth);
static gchar *gimp_prop_eval_string_simple(GObject *config, GParamSpec *pspec,
                                           const gchar **expr, gchar **t,
                                           GError **error, gint depth);

static gboolean
gimp_prop_eval_parse_reference(GObject *config, GParamSpec *pspec,
                               const gchar **expr, gchar **t, GError **error,
                               GParamSpec **ref_pspec, gchar **ref_key);

static gboolean gimp_prop_eval_depth_test(gint depth, GError **error);

static gchar *gimp_prop_eval_read_token(const gchar **expr, gchar **t,
                                        GError **error);
...
gboolean gimp_prop_eval_boolean(GObject *config, GParamSpec *pspec,
  ...
  result =
      gimp_prop_eval_boolean_impl(config, pspec, key, default_value, &error, 0);

  if (error) {
    g_warning("in object of type '%s': %s", G_OBJECT_TYPE_NAME(config),
              error->message);
...
gchar *gimp_prop_eval_string(GObject *config, GParamSpec *pspec,
  ...
  result =
      gimp_prop_eval_string_impl(config, pspec, key, default_value, &error, 0);
...
static gboolean gimp_prop_eval_boolean_impl(GObject *config, GParamSpec *pspec,
  ...
  if (!expr) {
    ...
    if (default_value < 0) {
      g_set_error(error, GIMP_PROP_EVAL_ERROR, GIMP_PROP_EVAL_FAILED,
                  "key '%s' of property '%s' not found", key,
  ...
  if (!*error) {
    result = gimp_prop_eval_boolean_or(config, pspec, &expr, &t, error, depth);
  }
  ...
  if (*error) {
    g_prefix_error(error, "in key '%s' of property '%s': ", key,
                   g_param_spec_get_name(pspec));
...
static gboolean gimp_prop_eval_boolean_or(GObject *config, GParamSpec *pspec,
  ...
  result = gimp_prop_eval_boolean_and(config, pspec, expr, t, error, depth);
...
static gboolean gimp_prop_eval_boolean_and(GObject *config, GParamSpec *pspec,
  ...
  result = gimp_prop_eval_boolean_not(config, pspec, expr, t, error, depth);
...
static gboolean gimp_prop_eval_boolean_not(GObject *config, GParamSpec *pspec,
  ...
  if (!g_strcmp0(*t, "!")) {
    ...
    return !gimp_prop_eval_boolean_not(config, pspec, expr, t, error,
                                       depth + 1);
  }

  return gimp_prop_eval_boolean_group(config, pspec, expr, t, error, depth);
}
...
static gboolean gimp_prop_eval_boolean_group(GObject *config, GParamSpec *pspec,
  ...
  if (!g_strcmp0(*t, "(")) {
    ...
    result =
        gimp_prop_eval_boolean_or(config, pspec, expr, t, error, depth + 1);
  ...
  return gimp_prop_eval_boolean_simple(config, pspec, expr, t, error, depth);
}
...
static gboolean gimp_prop_eval_boolean_simple(GObject *config,
  ...
  /* reference */
    ...
    if (!gimp_prop_eval_parse_reference(config, pspec, expr, t, error, &pspec,
                                        &key))
      return FALSE;

    result =
        gimp_prop_eval_boolean_impl(config, pspec, key, -1, error, depth + 1);
...
static gchar *gimp_prop_eval_string_impl(GObject *config, GParamSpec *pspec,
  ...
  if (!*error) {
    result =
        gimp_prop_eval_string_selection(config, pspec, &expr, &t, error, depth);
  }
...
static gchar *gimp_prop_eval_string_selection(GObject *config,
  ...
  if (!g_strcmp0(*t, "[")) {
    ...
    while (!*error) {
      ...
      value = gimp_prop_eval_string_selection(config, pspec, expr, t, error,
                                              depth + 1);
  ...
  return gimp_prop_eval_string_simple(config, pspec, expr, t, error, depth);
}
...
static gchar *gimp_prop_eval_string_simple(GObject *config, GParamSpec *pspec,
  ...
  /* reference */
    ...
    result =
        gimp_prop_eval_string_impl(config, pspec, key, NULL, error, depth + 1);
  ...
  /* deferred literal */
    ...
    if (!gimp_prop_eval_parse_reference(config, pspec, expr, t, error,
                                        &str_pspec, &str_key))
      return NULL;
    ...
    if (!str) {
      g_set_error(error, GIMP_PROP_EVAL_ERROR, GIMP_PROP_EVAL_FAILED,
                  "key '%s' of property '%s' not found", str_key,
...
static gchar *gimp_prop_eval_read_token(const gchar **expr, gchar **t,
  ...
  *t = NULL;
  ...
  if (gimp_prop_eval_is_name(token)) {
    do {
      ++*expr;
    } while (g_ascii_isalnum(**expr) || **expr == '_' || **expr == '-');
  }
  /* string literal */
  else if (token[0] == '\'') {
