...
GtkWidget *_gimp_prop_gui_new_generic(
  ...
  for (i = 0; i < n_param_specs; i++) {
    ...
    if (next_pspec && HAS_KEY(pspec, "axis", "x") &&
      ...
      widget_x = gimp_prop_widget_new_from_pspec(
          config, pspec, area, context, create_picker_func,
          create_controller_func, creator, &label_x);
      widget_y = gimp_prop_widget_new_from_pspec(
          config, next_pspec, area, context, create_picker_func,
          create_controller_func, creator, &label_y);
      ...
      if (!HAS_KEY(pspec, "unit", "pixel-coordinate") &&
          ...
          gtk_adjustment_get_value(adj_x) == gtk_adjustment_get_value(adj_y)) {
        GBinding *binding;

        gimp_chain_button_set_active(GIMP_CHAIN_BUTTON(chain), TRUE);

        binding = g_object_bind_property(adj_x, "value", adj_y, "value",
      ...
      g_signal_connect(chain, "toggled",
                       G_CALLBACK(gimp_prop_gui_chain_toggled), adj_x);
      ...
      if (create_picker_func &&
        ...
        pspec_name = g_strconcat(pspec->name, ":", next_pspec->name, NULL);

        button = create_picker_func(creator, pspec_name, GIMP_ICON_CURSOR,
                                    _("Pick coordinates from the image"),
                                    /* pick_abyss = */ TRUE, NULL, NULL);
        gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, FALSE, 0);
        gtk_widget_show(button);

        g_object_weak_ref(G_OBJECT(button), (GWeakNotify)g_free, pspec_name);
      }
    } else if (next_pspec && HAS_KEY(pspec, "role", "range-start") &&
      ...
      spin_scale = gimp_prop_widget_new_from_pspec(
          config, pspec, area, context, create_picker_func,
          create_controller_func, creator, &label_str);
      gtk_widget_show(spin_scale);

      g_object_set_data_full(G_OBJECT(vbox), "gimp-underlying-widget",
                             g_object_ref_sink(spin_scale), g_object_unref);

      range_label_str = gegl_param_spec_get_property_key(pspec, "range-label");
      ...
      gimp_spin_scale_get_scale_limits(GIMP_SPIN_SCALE(spin_scale), &ui_lower,
                                       &ui_upper);
      ...
      if (!range_label_str) {
        g_object_bind_property(spin_scale, "label", label, "label",
      ...
      range = gimp_prop_range_new(
          config, pspec->name, next_pspec->name, step_increment, page_increment,
          gtk_spin_button_get_digits(GTK_SPIN_BUTTON(spin_scale)),
          !HAS_KEY(pspec, "range-sorted", "false"));
      ...
      widget = gimp_prop_widget_new_from_pspec(
          config, pspec, area, context, create_picker_func,
          create_controller_func, creator, &label);
      ...
      if (widget && label) {
        ...
        if (GTK_IS_SCROLLED_WINDOW(widget)) {
          ...
          hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 4);
          gtk_box_pack_start(GTK_BOX(main_vbox), hbox, expand, expand, 0);
          gtk_widget_show(hbox);
        ...
        gtk_box_pack_start(GTK_BOX(main_vbox), widget, expand, expand, 0);
        gtk_widget_show(widget);
      }
    }
  }
...
static void gimp_prop_gui_chain_toggled(GimpChainButton *chain,
  ...
  if (gimp_chain_button_get_active(chain)) {
    if (!binding)
      binding = g_object_bind_property(x_adj, "value", y_adj, "value",
                                       G_BINDING_BIDIRECTIONAL);
  } else {
