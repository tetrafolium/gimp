...
GtkWidget *gimp_prop_kelvin_presets_new(GObject *config,
  ...
  g_signal_connect(button, "button-press-event",
                   G_CALLBACK(gimp_prop_kelvin_presets_button_press), menu);
  ...
  for (i = 0; i < G_N_ELEMENTS(kelvin_presets); i++) {
    ...
    *kelvin = kelvin_presets[i].kelvin;
    ...
    g_object_set_data_full(G_OBJECT(item), "kelvin", kelvin,
                           (GDestroyNotify)g_free);

    g_signal_connect(item, "activate",
                     G_CALLBACK(gimp_prop_kelvin_presets_activate), config);
  }
...
GtkWidget *gimp_prop_random_seed_new(GObject *config,
  ...
  spin = gimp_prop_spin_button_new(config, property_name, 1.0, 10.0, 0);
  ...
  g_signal_connect(button, "clicked",
                   G_CALLBACK(gimp_prop_random_seed_new_clicked), adj);
...
static gboolean gimp_prop_kelvin_presets_button_press(GtkWidget *widget,
  ...
  if (bevent->type == GDK_BUTTON_PRESS) {
    gtk_menu_popup_at_widget(menu, widget, GDK_GRAVITY_WEST,
