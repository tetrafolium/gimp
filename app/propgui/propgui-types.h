...
#define GIMP_CONTROLLER_SLIDER_DEFAULT                                         \
  ((const GimpControllerSlider){.value = 0.0,                                  \
                                .min = 0.0,                                    \
                                .max = 1.0,                                    \
                                                                               \
                                .visible = TRUE,                               \
...
typedef void (*GimpPickerCallback)(gpointer data, gpointer identifier,
                                   gdouble x, gdouble y,
                                   const Babl *sample_format,
                                   const GimpRGB *color);

typedef void (*GimpControllerLineCallback)(gpointer data, GeglRectangle *area,
                                           gdouble x1, gdouble y1, gdouble x2,
                                           gdouble y2);
typedef void (*GimpControllerSliderLineCallback)(
    gpointer data, GeglRectangle *area, gdouble x1, gdouble y1, gdouble x2,
    gdouble y2, const GimpControllerSlider *sliders, gint n_sliders);
typedef void (*GimpControllerTransformGridCallback)(
    gpointer data, GeglRectangle *area, const GimpMatrix3 *transform);
typedef void (*GimpControllerTransformGridsCallback)(
    gpointer data, GeglRectangle *area, const GimpMatrix3 *transforms,
    gint n_transforms);
typedef void (*GimpControllerGyroscopeCallback)(gpointer data,
                                                GeglRectangle *area,
                                                gdouble yaw, gdouble pitch,
                                                gdouble roll, gdouble zoom,
                                                gboolean invert);
typedef void (*GimpControllerFocusCallback)(gpointer data, GeglRectangle *area,
...
typedef GtkWidget *(*GimpCreatePickerFunc)(
    gpointer creator, const gchar *property_name, const gchar *icon_name,
    const gchar *tooltip, gboolean pick_abyss, GimpPickerCallback callback,
    gpointer callback_data);

typedef GCallback (*GimpCreateControllerFunc)(
    gpointer creator, GimpControllerType controller_type,
