...
static void hue_saturation_range_callback(GtkWidget *widget, GObject *config) {
...
static void hue_saturation_range_notify(GObject *object,
                                        const GParamSpec *pspec,
                                        GtkWidget *range_radio) {
  GimpHueSaturationConfig *config = GIMP_HUE_SATURATION_CONFIG(object);

  gimp_int_radio_group_set_active(GTK_RADIO_BUTTON(range_radio), config->range);
}

GtkWidget *_gimp_prop_gui_new_hue_saturation(
  ...
  const struct {
    ...
    gint frame_col;
    gint frame_row;
  } hue_range_grid[] = {{N_("M_aster"), N_("Adjust all colors"), 2, 3, 0, 0},
  ...
  for (i = 0; i < G_N_ELEMENTS(hue_range_grid); i++) {
    button = gtk_radio_button_new_with_mnemonic(
        group, gettext(hue_range_grid[i].label));
    group = gtk_radio_button_get_group(GTK_RADIO_BUTTON(button));
    g_object_set_data(G_OBJECT(button), "gimp-item-data", GINT_TO_POINTER(i));

    gimp_help_set_help_data(button, gettext(hue_range_grid[i].tooltip), NULL);
    ...
    gtk_grid_attach(GTK_GRID(grid), button, hue_range_grid[i].label_col,
                    hue_range_grid[i].label_row, 1, 1);
    ...
    if (i > 0) {
      ...
      gtk_grid_attach(GTK_GRID(grid), frame, hue_range_grid[i].frame_col,
                      hue_range_grid[i].frame_row, 1, 1);
      gtk_widget_show(frame);
      ...
      g_object_set_data(G_OBJECT(color_area), "hue-range", GINT_TO_POINTER(i));
    ...
    g_signal_connect(button, "toggled",
                     G_CALLBACK(hue_saturation_range_callback), config);
  ...
  scale =
      gimp_prop_spin_scale_new(config, "overlap", _("_Overlap"), 0.01, 0.1, 0);
  ...
  scale = gimp_prop_spin_scale_new(config, "hue", _("_Hue"), 1.0 / 180.0,
                                   15.0 / 180.0, 0);
  gimp_prop_widget_set_factor(scale, 180.0, 0.0, 0.0, 1);
  gtk_box_pack_start(GTK_BOX(vbox), scale, FALSE, FALSE, 0);

  /*  Create the lightness scale widget  */
  scale = gimp_prop_spin_scale_new(config, "lightness", _("_Lightness"), 0.01,
                                   0.1, 0);
  gimp_prop_widget_set_factor(scale, 100.0, 0.0, 0.0, 1);
  gtk_box_pack_start(GTK_BOX(vbox), scale, FALSE, FALSE, 0);

  /*  Create the saturation scale widget  */
  scale = gimp_prop_spin_scale_new(config, "saturation", _("_Saturation"), 0.01,
                                   0.1, 0);
  gimp_prop_widget_set_factor(scale, 100.0, 0.0, 0.0, 1);
  gtk_box_pack_start(GTK_BOX(vbox), scale, FALSE, FALSE, 0);
  ...
  g_signal_connect_object(config, "notify::range",
                          G_CALLBACK(hue_saturation_range_notify), range_radio,
                          0);
