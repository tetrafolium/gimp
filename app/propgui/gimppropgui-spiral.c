...
static void slider_line_callback(GObject *config, GeglRectangle *area,
  ...
  g_object_get(config, "type", &type, "base", &base, "balance", &balance, NULL);
  ...
  g_object_set(config, "x", x, "y", y, "radius", radius, "base", base,
               "rotation", rotation, "balance", balance, NULL);
}
...
static void config_notify(GObject *config, const GParamSpec *pspec,
  ...
  g_object_get(config, "type", &type, "x", &x, "y", &y, "radius", &radius,
               "rotation", &rotation, "base", &base, "balance", &balance, NULL);
...
GtkWidget *_gimp_prop_gui_new_spiral(
  ...
  if (create_controller_func) {
    ...
    set_func = create_controller_func(
        creator, GIMP_CONTROLLER_TYPE_SLIDER_LINE, _("Spiral: "),
        (GCallback)slider_line_callback, config, &set_data);
