...
static void app_init_update_noop(const gchar *text1, const gchar *text2,
                                 gdouble percentage);
static void app_restore_after_callback(Gimp *gimp,
                                       GimpInitStatusFunc status_callback);
static gboolean app_exit_after_callback(Gimp *gimp, gboolean kill_it,
                                        GMainLoop **loop);
...
void app_libs_init(GOptionContext *context, gboolean no_interface) {
#if 0
	GQuark quark;
#endif
   ...
   */
  g_object_set(gegl_config(), "use-opencl", FALSE, "application-license",
               "GPL3", NULL);
...
void app_abort(gboolean no_interface, const gchar *abort_message) {
#ifndef GIMP_CONSOLE_COMPILATION
  if (no_interface)
#endif
  {
...
void app_exit(gint status) { exit(status); }

void app_run(const gchar *full_prog_name, const gchar **filenames,
             GFile *alternate_system_gimprc, GFile *alternate_gimprc,
             const gchar *session_name, const gchar *batch_interpreter,
             const gchar **batch_commands, gboolean as_new,
             gboolean no_interface, gboolean no_data, gboolean no_fonts,
             gboolean no_splash, gboolean be_verbose, gboolean use_shm,
             gboolean use_cpu_accel, gboolean console_messages,
             gboolean use_debug_handler, gboolean show_playground,
             gboolean show_debug_menu, GimpStackTraceMode stack_trace_mode,
             GimpPDBCompatMode pdb_compat_mode, const gchar *backtrace_file) {
  ...
  if (filenames && filenames[0] && !filenames[1] &&
    ...
    if (g_path_is_absolute(filenames[0])) {
      ...
      gchar *absolute = g_build_path(G_DIR_SEPARATOR_S, g_get_current_dir(),
                                     filenames[0], NULL);
   ...
   */
  temprc =
      gimp_lang_rc_new(alternate_system_gimprc, alternate_gimprc, be_verbose);
   ...
   *  core object system
   */
  gimp = gimp_new(full_prog_name, session_name, default_folder, be_verbose,
                  no_data, no_fonts, no_interface, use_shm, use_cpu_accel,
                  console_messages, show_playground, show_debug_menu,
                  stack_trace_mode, pdb_compat_mode);
  ...
  if (g_file_query_file_type(gimpdir, G_FILE_QUERY_INFO_NONE, NULL) !=
    ...
    GimpUserInstall *install =
        gimp_user_install_new(G_OBJECT(gimp), be_verbose);

#ifdef GIMP_CONSOLE_COMPILATION
    gimp_user_install_run(install);
#else
    if (!(no_interface ? gimp_user_install_run(install)
  ...
  g_signal_connect_after(gimp, "restore",
                         G_CALLBACK(app_restore_after_callback), NULL);
  ...
  g_signal_connect_after(gimp, "exit", G_CALLBACK(app_exit_after_callback),
                         &run_loop);
  ...
  if (run_loop && !no_interface) {
    ...
    if (recovered_files && gui_recover(g_list_length(recovered_files))) {
      for (iter = recovered_files; iter; iter = iter->next) {
        ...
        image = file_open_with_display(gimp, gimp_get_user_context(gimp), NULL,
                                       file, as_new, initial_monitor, &status,
                                       &error);
        if (image) {
  ...
  if (filenames) {
    ...
    for (i = 0; filenames[i] != NULL; i++) {
      if (run_loop) {
        ...
        file_open_from_command_line(gimp, file, as_new, initial_monitor);
  ...
  if (font_error) {
    gimp_message_literal(gimp, NULL, GIMP_MESSAGE_INFO, font_error->message);
