...
#define GIMP_NPD_MAXIMUM_DEFORMATION_DELAY                                     \
  100000                          /* 100000 microseconds == 10 FPS */
...
static void gimp_n_point_deformation_tool_set_options(
    GimpNPointDeformationTool *npd_tool,
    GimpNPointDeformationOptions *npd_options);
static void gimp_n_point_deformation_tool_options_notify(
...
static void gimp_n_point_deformation_tool_modifier_key(GimpTool *tool,
                                                       ...
                                                       GimpDisplay *display);
static void gimp_n_point_deformation_tool_cursor_update(
    GimpTool *tool, const GimpCoords *coords, GdkModifierType state,
    GimpDisplay *display);
...
static void gimp_n_point_deformation_tool_clear_selected_points_list(
    GimpNPointDeformationTool *npd_tool);
static gboolean gimp_n_point_deformation_tool_add_cp_to_selection(
    GimpNPointDeformationTool *npd_tool, NPDControlPoint *cp);
static gboolean gimp_n_point_deformation_tool_is_cp_in_area(
    NPDControlPoint *cp, gfloat x0, gfloat y0, gfloat x1, gfloat y1,
    gfloat offset_x, gfloat offset_y, gfloat cp_radius);
...
static gboolean gimp_n_point_deformation_tool_canvas_update_timeout(
    GimpNPointDeformationTool *npd_tool);
static void gimp_n_point_deformation_tool_perform_deformation(
    GimpNPointDeformationTool *npd_tool);
static void
gimp_n_point_deformation_tool_halt_threads(GimpNPointDeformationTool *npd_tool);
static void gimp_n_point_deformation_tool_apply_deformation(
    GimpNPointDeformationTool *npd_tool);
...
void gimp_n_point_deformation_tool_register(GimpToolRegisterCallback callback,
  ...
  if (GIMP_GUI_CONFIG(GIMP(data)->config)->playground_npd_tool)
    (*callback)(GIMP_TYPE_N_POINT_DEFORMATION_TOOL,
                ...
                gimp_n_point_deformation_options_gui, 0,
                "gimp-n-point-deformation-tool", _("N-Point Deformation"),
                _("N-Point Deformation Tool: Rubber-like deformation of "
                  "image using points"),
                N_("_N-Point Deformation"), "<shift>N", NULL,
                GIMP_HELP_TOOL_N_POINT_DEFORMATION,
                GIMP_ICON_TOOL_N_POINT_DEFORMATION, data);
}

static void gimp_n_point_deformation_tool_class_init(
    GimpNPointDeformationToolClass *klass) {
...
gimp_n_point_deformation_tool_start(GimpNPointDeformationTool *npd_tool,
  ...
  npd_tool->source =
      gegl_node_new_child(npd_tool->graph, "operation", "gegl:buffer-source",
                          "buffer", source_buffer, NULL);
  npd_tool->npd_node =
      gegl_node_new_child(npd_tool->graph, "operation", "gegl:npd", NULL);
  npd_tool->sink =
      gegl_node_new_child(npd_tool->graph, "operation", "gegl:write-buffer",
                          "buffer", preview_buffer, NULL);

  gegl_node_link_many(npd_tool->source, npd_tool->npd_node, npd_tool->sink,
                      NULL);
  ...
  npd_tool->lattice_points =
      g_new(GimpVector2, 5 * model->hidden_model->num_of_bones);

  gimp_item_get_offset(GIMP_ITEM(tool->drawables->data), &npd_tool->offset_x,
                       &npd_tool->offset_y);
  ...
  npd_tool->deform_thread = g_thread_new(
      "deform thread",
      (GThreadFunc)gimp_n_point_deformation_tool_deform_thread_func, npd_tool);

  /* create and start canvas update timeout */
  npd_tool->draw_timeout_id = gdk_threads_add_timeout_full(
      G_PRIORITY_DEFAULT_IDLE, GIMP_NPD_DRAW_INTERVAL,
...
static void gimp_n_point_deformation_tool_set_options(
    ...
    GimpNPointDeformationOptions *npd_options) {
  gegl_node_set(
      npd_tool->npd_node, "square-size", (gint)npd_options->square_size,
      "rigidity", (gint)npd_options->rigidity, "asap-deformation",
      npd_options->asap_deformation, "mls-weights", npd_options->mls_weights,
      "mls-weights-alpha", npd_options->mls_weights_alpha, NULL);
}
...
static void gimp_n_point_deformation_tool_options_notify(
  ...
  GimpNPointDeformationTool *npd_tool = GIMP_N_POINT_DEFORMATION_TOOL(tool);
  GimpNPointDeformationOptions *npd_options =
      GIMP_N_POINT_DEFORMATION_OPTIONS(options);
...
static gboolean gimp_n_point_deformation_tool_key_press(GimpTool *tool,
  ...
  case GDK_KEY_BackSpace:
     ...
     *  control point
     */
    if (npd_tool->model && npd_tool->model->control_points &&
        npd_tool->model->control_points->len > 0) {
      GArray *cps = npd_tool->model->control_points;
      NPDControlPoint *cp = &g_array_index(cps, NPDControlPoint, cps->len - 1);
  ...
  case GDK_KEY_Delete:
    if (npd_tool->model && npd_tool->selected_cps) {
  ...
  if (release_type == GIMP_BUTTON_RELEASE_CLICK) {
    ...
    if (npd_tool->rubber_band) {
      ...
      for (i = 0; i < cps->len; i++) {
        ...
        if (gimp_n_point_deformation_tool_is_cp_in_area(
                cp, x0, y0, x1, y1, npd_tool->offset_x, npd_tool->offset_y,
...
static void gimp_n_point_deformation_tool_oper_update(GimpTool *tool,
  ...
  if (npd_tool->active) {
    ...
    npd_tool->hovering_cp = npd_get_control_point_with_radius_at(
        model, &p, npd_tool->cp_scaled_radius);
  }
...
static void gimp_n_point_deformation_tool_prepare_lattice(
    GimpNPointDeformationTool *npd_tool) {
  ...
  for (i = 0; i < hm->num_of_bones; i++) {
    ...
    for (j = 0; j < 4; j++)
      gimp_vector2_set(&points[5 * i + j], bone->points[j].x,
                       bone->points[j].y);
...
static void gimp_n_point_deformation_tool_draw_lattice(
  ...
  for (i = 0; i < n_squares; i++)
    gimp_draw_tool_add_lines(GIMP_DRAW_TOOL(npd_tool), &points[5 * i], 5, NULL,
                             FALSE);
}
...
static void gimp_n_point_deformation_tool_draw(GimpDrawTool *draw_tool) {
  ...
  for (i = 0; i < model->control_points->len; i++) {
    NPDControlPoint *cp =
        &g_array_index(model->control_points, NPDControlPoint, i);
    NPDPoint p = cp->point;
    ...
    if (cp == npd_tool->hovering_cp ||
        (npd_tool->rubber_band &&
         gimp_n_point_deformation_tool_is_cp_in_area(
             cp, x0, y0, x1, y1, npd_tool->offset_x, npd_tool->offset_y,
             npd_tool->cp_scaled_radius))) {
    ...
    gimp_draw_tool_add_handle(
        draw_tool, handle_type, p.x, p.y, GIMP_TOOL_HANDLE_SIZE_CIRCLE,
        GIMP_TOOL_HANDLE_SIZE_CIRCLE, GIMP_HANDLE_ANCHOR_CENTER);

    if (g_list_find(npd_tool->selected_cps, cp)) {
      gimp_draw_tool_add_handle(
          draw_tool, GIMP_HANDLE_SQUARE, p.x, p.y, GIMP_TOOL_HANDLE_SIZE_CIRCLE,
          GIMP_TOOL_HANDLE_SIZE_CIRCLE, GIMP_HANDLE_ANCHOR_CENTER);
    }
  }
  ...
  if (npd_tool->rubber_band) {
    /* draw a rubber band */
    gimp_draw_tool_add_rectangle(draw_tool, FALSE, x0, y0, x1 - x0, y1 - y0);
  }

  if (npd_tool->preview_buffer) {
    GimpCanvasItem *item;

    item = gimp_canvas_buffer_preview_new(
        gimp_display_get_shell(draw_tool->display), npd_tool->preview_buffer);
...
static void gimp_n_point_deformation_tool_perform_deformation(
  ...
  g_object_get(npd_tool->npd_node, "gegl-operation", &operation, NULL);
...
static void gimp_n_point_deformation_tool_halt_threads(
    GimpNPointDeformationTool *npd_tool) {
...
static void gimp_n_point_deformation_tool_apply_deformation(
  ...
  gimp_gegl_apply_operation(
      NULL, NULL, _("N-Point Deformation"), npd_tool->npd_node,
      gimp_drawable_get_buffer(tool->drawables->data), NULL, FALSE);

  gimp_drawable_update(tool->drawables->data, 0, 0, width, height);
