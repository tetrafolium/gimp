...
static void gimp_selection_tool_control(GimpTool *tool, GimpToolAction action,
                                        GimpDisplay *display);
...
static gboolean gimp_selection_tool_check(GimpSelectionTool *sel_tool,
                                          GimpDisplay *display, GError **error);
...
static void gimp_selection_tool_modifier_key(GimpTool *tool,
  ...
  if (key == extend_mask || key == modify_mask || key == GDK_MOD1_MASK) {
...
static void gimp_selection_tool_oper_update(GimpTool *tool,
  ...
  if (drawables) {
    if (floating_sel) {
      ...
      for (iter = drawables; iter; iter = iter->next) {
        if (gimp_item_mask_intersect(GIMP_ITEM(iter->data), NULL, NULL, NULL,
                                     NULL)) {
          move_layer = TRUE;
          break;
        }
  ...
  if (selection_tool->allow_move && (state & GDK_MOD1_MASK) &&
      (state & modify_mask) && move_layer) {
    /* move the selection */
    selection_tool->function = SELECTION_MOVE;
  } else if (selection_tool->allow_move && (state & GDK_MOD1_MASK) &&
             (state & extend_mask) && move_layer) {
    ...
    selection_tool->function = SELECTION_MOVE_MASK;
             ...
             !(state & (extend_mask | modify_mask)) && move_floating_sel) {
    /* move the selection */
    selection_tool->function = SELECTION_MOVE;
      ...
      case GIMP_CHANNEL_OP_REPLACE:
        if (have_selection) {
          status = gimp_suggest_modifiers(_("Click-Drag to replace the "
                                            "current selection"),
                                          modifiers & ~state, NULL, NULL, NULL);
          free_status = TRUE;
        } else {
...
static void gimp_selection_tool_cursor_update(GimpTool *tool,
  ...
  gimp_tool_set_cursor(tool, display,
                       gimp_tool_control_get_cursor(tool->control), tool_cursor,
                       modifier);
}
...
static gboolean gimp_selection_tool_check(GimpSelectionTool *sel_tool,
  ...
  case SELECTION_SELECT:
    ...
    case GIMP_CHANNEL_OP_SUBTRACT:
      if (!gimp_item_bounds(GIMP_ITEM(gimp_image_get_mask(image)), NULL, NULL,
                            NULL, NULL)) {
...
static void gimp_selection_tool_set_undo_ptr(GimpUndo **undo_ptr,
                                             GimpUndo *undo) {
  if (*undo_ptr) {
    g_object_remove_weak_pointer(G_OBJECT(*undo_ptr), (gpointer *)undo_ptr);
  }

  *undo_ptr = undo;

  if (*undo_ptr) {
    g_object_add_weak_pointer(G_OBJECT(*undo_ptr), (gpointer *)undo_ptr);
  }
}
...
gboolean gimp_selection_tool_start_edit(GimpSelectionTool *sel_tool,
  ...
  case SELECTION_MOVE_COPY: {
    ...
    gimp_edit_selection_tool_start(tool, display, coords, edit_mode, FALSE);
...
static gboolean gimp_selection_tool_idle(GimpSelectionTool *sel_tool) {
  GimpTool *tool = GIMP_TOOL(sel_tool);
...
void gimp_selection_tool_start_change(GimpSelectionTool *sel_tool,
  ...
  sel_tool->saved_show_selection = gimp_display_shell_get_show_selection(shell);
  ...
  if (create) {
    ...
    if (undo && undo == sel_tool->undo) {
      ...
      gimp_selection_tool_set_undo_ptr(&sel_tool->redo,
    ...
    if (operation == GIMP_CHANNEL_OP_REPLACE) {
       ...
       */
      sel_tool->idle_id = g_idle_add_full(G_PRIORITY_HIGH_IDLE,
                                          (GSourceFunc)gimp_selection_tool_idle,
                                          sel_tool, NULL);
    }
  }

  gimp_selection_tool_set_undo_ptr(&sel_tool->undo,
