...
static void gimp_crop_tool_control(GimpTool *tool, GimpToolAction action,
                                   GimpDisplay *display);
...
static void gimp_crop_tool_button_release(GimpTool *tool,
                                          ...
                                          GimpDisplay *display);
static void gimp_crop_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                  guint32 time, GdkModifierType state,
                                  GimpDisplay *display);
...
static void gimp_crop_tool_start(GimpCropTool *crop_tool, GimpDisplay *display);
...
void gimp_crop_tool_register(GimpToolRegisterCallback callback, gpointer data) {
  (*callback)(GIMP_TYPE_CROP_TOOL, GIMP_TYPE_CROP_OPTIONS,
              gimp_crop_options_gui, 0, "gimp-crop-tool", _("Crop"),
              _("Crop Tool: Remove edge areas from image or layer"),
              N_("_Crop"), "<shift>C", NULL, GIMP_HELP_TOOL_CROP,
              GIMP_ICON_TOOL_CROP, data);
}
...
static void gimp_crop_tool_constructed(GObject *object) {
  ...
  g_signal_connect_object(context, "image-changed",
                          G_CALLBACK(gimp_crop_tool_image_changed), crop_tool,
                          G_CONNECT_SWAPPED);

  /* Make sure we are connected to "size-changed" for the initial
   * image.
   */
  gimp_crop_tool_image_changed(crop_tool, gimp_context_get_image(context),
                               context);
}
...
static void gimp_crop_tool_button_release(GimpTool *tool,
  ...
  if (crop_tool->grab_widget) {
    gimp_tool_widget_button_release(crop_tool->grab_widget, coords, time, state,
                                    release_type);
  ...
  if (!strcmp(pspec->name, "layer-only") ||
    ...
    if (crop_tool->widget) {
      gimp_tool_rectangle_set_constraint(
          GIMP_TOOL_RECTANGLE(crop_tool->widget),
...
static void gimp_crop_tool_start(GimpCropTool *crop_tool,
  ...
  g_object_set(widget, "status-title", _("Crop to: "), NULL);
  ...
  gimp_rectangle_options_connect(
      ...
      G_CALLBACK(gimp_crop_tool_auto_shrink), crop_tool);

  gimp_tool_rectangle_set_constraint(GIMP_TOOL_RECTANGLE(widget),
                                     gimp_crop_tool_get_constraint(crop_tool));

  g_signal_connect(widget, "changed",
                   G_CALLBACK(gimp_crop_tool_rectangle_changed), crop_tool);
  g_signal_connect(widget, "response",
                   G_CALLBACK(gimp_crop_tool_rectangle_response), crop_tool);
      ...
      if (options->layer_only) {
        ...
        if (iter == NULL) {
          ...
          gimp_tools_blink_lock_box(tool->display->gimp,
                                    GIMP_ITEM(layers->data));
          return;
        }

        undo_text =
            ngettext("Resize Layer", "Resize %d layers", g_list_length(layers));
        ...
        for (iter = layers; iter; iter = iter->next) {
          ...
          gimp_item_resize(GIMP_ITEM(iter->data), GIMP_CONTEXT(options),
                           options->fill_type, w, h, off_x, off_y);
        }
        gimp_image_undo_group_end(image);
      } else {
        gimp_image_crop(image, GIMP_CONTEXT(options), GIMP_FILL_TRANSPARENT, x,
                        y, w, h, options->delete_pixels);
      }
...
static void gimp_crop_tool_update_option_defaults(GimpCropTool *crop_tool,
  ...
  if (rectangle && !ignore_pending) {
    ...
    gimp_tool_rectangle_pending_size_set(rectangle, G_OBJECT(options),
                                         "default-aspect-numerator",
                                         "default-aspect-denominator");

    g_object_set(G_OBJECT(options), "use-string-current", TRUE, NULL);
  } else {
    ...
    if (rectangle) {
      gimp_tool_rectangle_constraint_size_set(rectangle, G_OBJECT(options),
                                              "default-aspect-numerator",
                                              "default-aspect-denominator");
    ...
    g_object_set(G_OBJECT(options), "use-string-current", FALSE, NULL);
  }
}
...
static void gimp_crop_tool_image_changed(GimpCropTool *crop_tool,
  ...
  if (crop_tool->current_image) {
    g_signal_handlers_disconnect_by_func(
        crop_tool->current_image, gimp_crop_tool_image_size_changed, NULL);
    g_signal_handlers_disconnect_by_func(
        crop_tool->current_image, gimp_crop_tool_image_selected_layers_changed,
        NULL);
  ...
  if (crop_tool->current_image) {
    ...
    g_signal_connect_object(crop_tool->current_image, "size-changed",
                            ...
                            crop_tool, G_CONNECT_SWAPPED);
    g_signal_connect_object(
        crop_tool->current_image, "selected-layers-changed",
        G_CALLBACK(gimp_crop_tool_image_selected_layers_changed), crop_tool,
        G_CONNECT_SWAPPED);
  }
...
gimp_crop_tool_image_selected_layers_changed(GimpCropTool *crop_tool) {
  ...
  if (crop_tool->current_layers) {
    for (iter = crop_tool->current_layers; iter; iter = iter->next) {
      if (iter->data) {
        g_signal_handlers_disconnect_by_func(
            iter->data, gimp_crop_tool_layer_size_changed, NULL);
  ...
  if (crop_tool->current_image) {
    crop_tool->current_layers =
        gimp_image_get_selected_layers(crop_tool->current_image);
...
static void gimp_crop_tool_auto_shrink(GimpCropTool *crop_tool) {
  ...
  g_object_get(gimp_tool_get_options(GIMP_TOOL(crop_tool)), "shrink-merged",
               &shrink_merged, NULL);
