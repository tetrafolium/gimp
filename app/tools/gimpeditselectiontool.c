...
static void gimp_edit_selection_tool_button_release(
...
gimp_edit_selection_tool_calc_coords(GimpEditSelectionTool *edit_select,
                                     GimpImage *image, gdouble x, gdouble y);
static void
...
gimp_edit_selection_tool_class_init(GimpEditSelectionToolClass *klass) {
  ...
  tool_class->active_modifier_key =
      gimp_edit_selection_tool_active_modifier_key;

  draw_class->draw = gimp_edit_selection_tool_draw;
}

static void gimp_edit_selection_tool_init(GimpEditSelectionTool *edit_select) {
...
void gimp_edit_selection_tool_start(GimpTool *parent_tool, GimpDisplay *display,
  ...
  edit_select = g_object_new(GIMP_TYPE_EDIT_SELECTION_TOOL, "tool-info",
                             parent_tool->tool_info, NULL);
  ...
  selected_items =
      gimp_edit_selection_tool_get_selected_items(edit_select, image);
  ...
  /* Find the active item's selection bounds */
  {
    ...
    gimp_channel_boundary(channel, &segs_in, &segs_out,
                          &edit_select->num_segs_in, &edit_select->num_segs_out,
                          0, 0, 0, 0);

    edit_select->segs_in =
        g_memdup(segs_in, edit_select->num_segs_in * sizeof(GimpBoundSeg));

    edit_select->segs_out =
        g_memdup(segs_out, edit_select->num_segs_out * sizeof(GimpBoundSeg));
    ...
    if (edit_select->edit_mode == GIMP_TRANSLATE_MODE_VECTORS) {
       ...
       */
      gimp_item_mask_intersect(selected_items->data, &edit_select->sel_x,
                               &edit_select->sel_y, &edit_select->sel_width,
                               &edit_select->sel_height);
    }
  }

  gimp_edit_selection_tool_calc_coords(edit_select, image, coords->x,
                                       coords->y);

  {
    ...
    case GIMP_TRANSLATE_MODE_LAYER_MASK:
      edit_select->delayed_items =
          gimp_image_item_list_linked(image, selected_items);
      gimp_image_item_list_bounds(image, edit_select->delayed_items, &x, &y, &w,
                                  &h);
      x += off_x;
      y += off_y;
    ...
    case GIMP_TRANSLATE_MODE_VECTORS:
      edit_select->live_items =
          gimp_image_item_list_linked(image, selected_items);
      gimp_image_item_list_bounds(image, edit_select->live_items, &x, &y, &w,
                                  &h);
      break;
    }

    gimp_tool_control_set_snap_offsets(tool->control, x - coords->x,
                                       y - coords->y, w, h);
...
static void gimp_edit_selection_tool_button_release(
   ...
   */
  gimp_image_item_list_translate(image, edit_select->delayed_items,
                                 edit_select->cuml_x, edit_select->cuml_y,
                                 TRUE);
  ...
  if (edit_select->propagate_release &&
    ...
    tool_manager_button_release_active(display->gimp, coords, time, state,
                                       display);
  }
...
gimp_edit_selection_tool_update_motion(GimpEditSelectionTool *edit_select,
  ...
  if (edit_select->constrain) {
    gimp_constrain_line(edit_select->start_x, edit_select->start_y, &new_x,
                        &new_y, GIMP_CONSTRAIN_LINE_45_DEGREES, 0.0, 1.0, 1.0);
  }

  gimp_edit_selection_tool_calc_coords(edit_select, image, new_x, new_y);
  ...
  if (dx != 0 || dy != 0) {
    ...
    case GIMP_TRANSLATE_MODE_LAYER:
      gimp_image_item_list_translate(image, edit_select->live_items, dx, dy,
                                     edit_select->first_move);
      break;

    case GIMP_TRANSLATE_MODE_MASK_TO_LAYER:
    case GIMP_TRANSLATE_MODE_MASK_COPY_TO_LAYER:
      if (!gimp_selection_float(
              GIMP_SELECTION(gimp_image_get_mask(image)), selected_items,
              gimp_get_user_context(display->gimp),
              edit_select->edit_mode == GIMP_TRANSLATE_MODE_MASK_TO_LAYER, 0, 0,
              &error)) {
  ...
  gimp_tool_push_status_coords(
      tool, display, gimp_tool_control_get_precision(tool->control),
      _("Move: "), edit_select->cuml_x, ", ", edit_select->cuml_y, NULL);
...
static void gimp_edit_selection_tool_motion(GimpTool *tool,
  ...
  gimp_edit_selection_tool_update_motion(edit_select, coords->x, coords->y,
                                         display);
}
...
static void gimp_edit_selection_tool_active_modifier_key(GimpTool *tool,
  ...
  if (state & GDK_BUTTON1_MASK) {
    gimp_edit_selection_tool_update_motion(edit_select,
                                           ...
                                           edit_select->last_motion_y, display);
  }
}

static void gimp_edit_selection_tool_draw(GimpDrawTool *draw_tool) {
  ...
  case GIMP_TRANSLATE_MODE_MASK: {
    ...
    if (!floating_sel && edit_select->segs_in) {
      gimp_draw_tool_add_boundary(
          draw_tool, edit_select->segs_in, edit_select->num_segs_in, NULL,
          edit_select->cuml_x + off_x, edit_select->cuml_y + off_y);
    }

    if (edit_select->segs_out) {
      gimp_draw_tool_add_boundary(
          draw_tool, edit_select->segs_out, edit_select->num_segs_out, NULL,
          edit_select->cuml_x + off_x, edit_select->cuml_y + off_y);
  ...
  case GIMP_TRANSLATE_MODE_MASK_TO_LAYER:
  case GIMP_TRANSLATE_MODE_MASK_COPY_TO_LAYER:
    gimp_draw_tool_add_rectangle(draw_tool, FALSE, edit_select->sel_x + off_x,
  ...
  case GIMP_TRANSLATE_MODE_FLOATING_SEL:
    if (edit_select->segs_in) {
      gimp_draw_tool_add_boundary(draw_tool, edit_select->segs_in,
                                  edit_select->num_segs_in, NULL,
                                  edit_select->cuml_x, edit_select->cuml_y);
    }
    break;
  }
  ...
  gimp_draw_tool_add_handle(
      draw_tool, GIMP_HANDLE_CROSS, edit_select->center_x + edit_select->cuml_x,
      edit_select->center_y + edit_select->cuml_y, GIMP_TOOL_HANDLE_SIZE_SMALL,
      GIMP_TOOL_HANDLE_SIZE_SMALL, GIMP_HANDLE_ANCHOR_CENTER);
...
gimp_edit_selection_tool_calc_coords(GimpEditSelectionTool *edit_select,
  ...
  edit_select->current_x =
      ((gint)floor(x1) - (edit_select->sel_x - edit_select->last_x));
  edit_select->current_y =
      ((gint)floor(y1) - (edit_select->sel_y - edit_select->last_y));
}
...
gimp_edit_selection_tool_start_undo_group(GimpEditSelectionTool *edit_select,
  ...
  gimp_image_undo_group_start(image,
                              edit_select->edit_mode == GIMP_TRANSLATE_MODE_MASK
...
gboolean gimp_edit_selection_tool_translate(GimpTool *tool, GdkEventKey *kevent,
  ...
  if (kevent->keyval != GDK_KEY_Left && kevent->keyval != GDK_KEY_Right &&
      kevent->keyval != GDK_KEY_Up && kevent->keyval != GDK_KEY_Down)
    return FALSE;

  /*  adapt arrow velocity to the zoom factor when holding <shift>  */
  velocity = (ARROW_VELOCITY / gimp_zoom_model_get_factor(
                                   gimp_display_get_shell(display)->zoom));
   ...
   *  them.
   */
  inc_x = process_event_queue_keys(
      kevent, GDK_KEY_Left, kevent->state | extend_mask, -1 * velocity,

      GDK_KEY_Left, kevent->state & ~extend_mask, -1,

      GDK_KEY_Right, kevent->state | extend_mask, 1 * velocity,

      GDK_KEY_Right, kevent->state & ~extend_mask, 1,

      0);

  inc_y = process_event_queue_keys(
      kevent, GDK_KEY_Up, kevent->state | extend_mask, -1 * velocity,

      GDK_KEY_Up, kevent->state & ~extend_mask, -1,

      GDK_KEY_Down, kevent->state | extend_mask, 1 * velocity,

      GDK_KEY_Down, kevent->state & ~extend_mask, 1,
  ...
  case GIMP_TRANSFORM_TYPE_LAYER:
    ...
    if (!selected_items) {
      ...
      if (n_items == 0)
        locked_message =
            _("All selected channels' positions or pixels are locked.");
  ...
  if (undo &&
      g_object_get_data(G_OBJECT(undo), "edit-selection-tool") ==
          (gpointer)tool &&
  ...
  if (push_undo) {
    if (gimp_image_undo_group_start(image, undo_type, undo_desc)) {
      undo =
          gimp_image_undo_can_compress(image, GIMP_TYPE_UNDO_STACK, undo_type);

      if (undo) {
        g_object_set_data(G_OBJECT(undo), "edit-selection-tool", tool);
  ...
  if (push_undo)
    ...
    gimp_undo_refresh_preview(undo, gimp_get_user_context(display->gimp));
