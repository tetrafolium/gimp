...
static void gimp_paint_select_tool_button_press(GimpTool *tool,
                                                ...
                                                GimpDisplay *display);
static void
gimp_paint_select_tool_button_release(GimpTool *tool, const GimpCoords *coords,
...
static void gimp_paint_select_tool_update_image_mask(
    GimpPaintSelectTool *ps_tool, GeglBuffer *buffer, gint offset_x,
    gint offset_y, GimpPaintSelectMode mode);
...
static void gimp_paint_select_tool_toggle_scribbles_visibility(
    GimpPaintSelectTool *ps_tool);

static gfloat euclidean_distance(gint x1, gint y1, gint x2, gint y2);
...
void gimp_paint_select_tool_register(GimpToolRegisterCallback callback,
  ...
  if (gegl_has_operation("gegl:paint-select") &&
    ...
    (*callback)(
        GIMP_TYPE_PAINT_SELECT_TOOL, GIMP_TYPE_PAINT_SELECT_OPTIONS,
        gimp_paint_select_options_gui, 0, "gimp-paint-select-tool",
        _("Paint Select"), _("Paint Select Tool: Scult selection by painting"),
        N_("P_aint Select"), NULL, NULL, GIMP_HELP_TOOL_FOREGROUND_SELECT,
        GIMP_ICON_TOOL_PAINT_SELECT, data);
}

static void gimp_paint_select_tool_class_init(GimpPaintSelectToolClass *klass) {
...
static void gimp_paint_select_tool_init(GimpPaintSelectTool *ps_tool) {
  ...
  gimp_tool_control_set_dirty_mask(
      tool->control, GIMP_DIRTY_IMAGE | GIMP_DIRTY_ACTIVE_DRAWABLE);
  gimp_tool_control_set_dirty_action(tool->control, GIMP_TOOL_ACTION_HALT);
...
static void gimp_paint_select_tool_button_press(GimpTool *tool,
  ...
  if (gimp_paint_select_tool_paint_scribble(ps_tool)) {
    GimpPaintSelectOptions *options =
        GIMP_PAINT_SELECT_TOOL_GET_OPTIONS(ps_tool);

    gint offset_x = ps_tool->last_pos.x - options->stroke_width / 2;
    gint offset_y = ps_tool->last_pos.y - options->stroke_width / 2;

    gimp_paint_select_tool_update_image_mask(ps_tool, ps_tool->scribble,
                                             offset_x, offset_y, options->mode);
  }
...
static void gimp_paint_select_tool_motion(GimpTool *tool,
  ...
  if (state & GDK_BUTTON1_MASK) {
    gfloat distance = euclidean_distance(
        coords->x, coords->y, ps_tool->last_pos.x, ps_tool->last_pos.y);
    ...
    if (distance >= 2.f) {
      ...
      if (gimp_paint_select_tool_paint_scribble(ps_tool)) {
        ...
        gimp_paint_select_tool_update_image_mask(
            ps_tool, result, ps_tool->drawable_off_x, ps_tool->drawable_off_y,
            options->mode);
...
static void gimp_paint_select_tool_draw(GimpDrawTool *draw_tool) {
  ...
  GimpPaintSelectOptions *options =
      GIMP_PAINT_SELECT_TOOL_GET_OPTIONS(paint_select);
  gint size = options->stroke_width;

  gimp_draw_tool_add_arc(
      draw_tool, FALSE, paint_select->last_pos.x - (size / 2.0),
      paint_select->last_pos.y - (size / 2.0), size, size, 0.0, (2.0 * G_PI));
}
...
static void gimp_paint_select_tool_update_image_mask(
  ...
  if (tool->display) {
    ...
    gimp_channel_select_buffer(gimp_image_get_mask(image),
                               C_("command", "Paint Select"), buffer, offset_x,
                               offset_y, op, FALSE, 0, 0);
...
static void gimp_paint_select_tool_init_buffers(GimpPaintSelectTool *ps_tool,
  ...
  gimp_item_get_offset(GIMP_ITEM(drawable), &ps_tool->drawable_off_x,
...
static void gimp_paint_select_tool_init_scribble(GimpPaintSelectTool *ps_tool) {
  ...
  gimp_scan_convert_stroke(scan_convert, size, GIMP_JOIN_ROUND, GIMP_CAP_ROUND,
                           10.0, 0.0, NULL);
...
gimp_paint_select_tool_paint_scribble(GimpPaintSelectTool *ps_tool) {
  ...
  iter = gegl_buffer_iterator_new(ps_tool->scribble, NULL, 0,
                                  babl_format("Y float"), GEGL_ACCESS_READ,
                                  GEGL_ABYSS_NONE, 3);
...
static void gimp_paint_select_tool_create_graph(GimpPaintSelectTool *ps_tool) {
  ...
  m = gegl_node_new_child(ps_tool->graph, "operation", "gegl:buffer-source",
                          "buffer", ps_tool->image_mask, NULL);

  crop = gegl_node_new_child(
      ps_tool->graph, "operation", "gegl:crop", "x",
      (gdouble)ps_tool->drawable_off_x, "y", (gdouble)ps_tool->drawable_off_y,
      "width", (gdouble)gegl_buffer_get_width(ps_tool->drawable), "height",
      (gdouble)gegl_buffer_get_height(ps_tool->drawable), NULL);

  if (ps_tool->drawable_off_x || ps_tool->drawable_off_y) {
    translate =
        gegl_node_new_child(ps_tool->graph, "operation", "gegl:translate", "x",
                            -1.0 * ps_tool->drawable_off_x, "y",
                            -1.0 * ps_tool->drawable_off_y, NULL);
  }

  d = gegl_node_new_child(ps_tool->graph, "operation", "gegl:buffer-source",
                          "buffer", ps_tool->drawable, NULL);

  t = gegl_node_new_child(ps_tool->graph, "operation", "gegl:buffer-source",
                          "buffer", ps_tool->trimap, NULL);

  ps_tool->ps_node = gegl_node_new_child(ps_tool->graph, "operation",
                                         "gegl:paint-select", NULL);

  ps_tool->render_node = gegl_node_new_child(ps_tool->graph, "operation",
                                             "gegl:buffer-sink", NULL);

  gegl_node_link(m, crop);

  if (translate)
    gegl_node_link_many(crop, translate, ps_tool->ps_node, ps_tool->render_node,
                        NULL);
  else
...
static void gimp_paint_select_tool_toggle_scribbles_visibility(
  ...
  if (options->show_scribbles) {
    ...
    gimp_display_shell_set_mask(gimp_display_get_shell(tool->display),
                                ps_tool->trimap, ps_tool->drawable_off_x,
                                ps_tool->drawable_off_y, &black, TRUE);
  } else {
    gimp_display_shell_set_mask(gimp_display_get_shell(tool->display), NULL, 0,
                                0, NULL, FALSE);
  }
