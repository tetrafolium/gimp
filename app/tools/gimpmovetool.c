...
static void gimp_move_tool_button_release(GimpTool *tool,
                                          ...
                                          GimpDisplay *display);
static gboolean gimp_move_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
                                         GimpDisplay *display);
static void gimp_move_tool_modifier_key(GimpTool *tool, GdkModifierType key,
                                        gboolean press, GdkModifierType state,
                                        GimpDisplay *display);
static void gimp_move_tool_oper_update(GimpTool *tool, const GimpCoords *coords,
                                       GdkModifierType state,
                                       gboolean proximity,
                                       GimpDisplay *display);
...
void gimp_move_tool_register(GimpToolRegisterCallback callback, gpointer data) {
  (*callback)(GIMP_TYPE_MOVE_TOOL, GIMP_TYPE_MOVE_OPTIONS,
              gimp_move_options_gui, 0, "gimp-move-tool", C_("tool", "Move"),
              _("Move Tool: Move layers, selections, and other objects"),
              N_("_Move"), "M", NULL, GIMP_HELP_TOOL_MOVE, GIMP_ICON_TOOL_MOVE,
              data);
}
...
static void gimp_move_tool_button_press(GimpTool *tool,
  ...
  if (!options->move_current) {
    ...
    if (options->move_type == GIMP_TRANSFORM_TYPE_PATH) {
      ...
      vectors = gimp_image_pick_vectors(image, coords->x, coords->y,
      ...
      if (gimp_display_shell_get_show_guides(shell) &&
        ...
        if (gimp_image_get_floating_selection(image) &&
           ...
           */
          move->floating_layer = gimp_image_get_floating_selection(image);
          ...
          move->old_selected_layers =
              g_list_copy(gimp_image_get_selected_layers(image));
  ...
  gimp_edit_selection_tool_start(tool, display, coords, translate_mode, TRUE);
  g_list_free(selected_items);
}
...
static gboolean gimp_move_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
  ...
  return gimp_edit_selection_tool_translate(tool, kevent, options->move_type,
                                            display, options->type_box);
}
...
static void gimp_move_tool_modifier_key(GimpTool *tool, GdkModifierType key,
  ...
  } else if (key == GDK_MOD1_MASK || key == gimp_get_toggle_behavior_mask()) {
    GimpTransformType button_type;

    button_type = options->move_type;

    if (press) {
      if (key == (state & (GDK_MOD1_MASK | gimp_get_toggle_behavior_mask()))) {
        /*  first modifier pressed  */

        move->saved_type = options->move_type;
      }
    } else {
      if (!(state & (GDK_MOD1_MASK | gimp_get_toggle_behavior_mask()))) {
...
static void gimp_move_tool_oper_update(GimpTool *tool, const GimpCoords *coords,
  ...
  if (options->move_type == GIMP_TRANSFORM_TYPE_LAYER &&
      !options->move_current && gimp_display_shell_get_show_guides(shell) &&
      proximity) {
  ...
  if (options->move_type == GIMP_TRANSFORM_TYPE_PATH) {
    ...
    if (gimp_display_shell_get_show_guides(shell) &&
      ...
      modifier = GIMP_CURSOR_MODIFIER_MOVE;
                    ...
                    gimp_image_pick_layer(image, coords->x, coords->y, NULL))) {
      /*  if there is a floating selection, and this aint it...  */
      if (gimp_image_get_floating_selection(image) &&
          !gimp_layer_is_floating_sel(layer)) {
...
static void gimp_move_tool_draw(GimpDrawTool *draw_tool) {
  ...
  for (iter = move->guides; iter; iter = g_list_next(iter)) {
    ...
    item =
        gimp_draw_tool_add_guide(draw_tool, gimp_guide_get_orientation(guide),
