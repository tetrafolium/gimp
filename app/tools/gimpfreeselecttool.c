...
static void gimp_free_select_tool_control(GimpTool *tool, GimpToolAction action,
                                          GimpDisplay *display);
static void gimp_free_select_tool_button_press(GimpTool *tool,
                                               ...
                                               GimpDisplay *display);
static void
gimp_free_select_tool_button_release(GimpTool *tool, const GimpCoords *coords,
...
void gimp_free_select_tool_register(GimpToolRegisterCallback callback,
                                    gpointer data) {
  (*callback)(GIMP_TYPE_FREE_SELECT_TOOL, GIMP_TYPE_SELECTION_OPTIONS,
              gimp_selection_options_gui, 0, "gimp-free-select-tool",
              _("Free Select"),
              _("Free Select Tool: Select a hand-drawn region with free "
                "and polygonal segments"),
              N_("_Free Select"), "F", NULL, GIMP_HELP_TOOL_FREE_SELECT,
              GIMP_ICON_TOOL_FREE_SELECT, data);
}
...
static void gimp_free_select_tool_class_init(GimpFreeSelectToolClass *klass) {
  ...
  GimpPolygonSelectToolClass *poly_sel_class =
      GIMP_POLYGON_SELECT_TOOL_CLASS(klass);
...
static void gimp_free_select_tool_init(GimpFreeSelectTool *free_sel) {
  ...
  gimp_tool_control_set_dirty_mask(tool->control, GIMP_DIRTY_SELECTION);
...
static void gimp_free_select_tool_button_press(GimpTool *tool,
  ...
  if (press_type == GIMP_BUTTON_PRESS_NORMAL &&
      gimp_selection_tool_start_edit(GIMP_SELECTION_TOOL(poly_sel), display,
                                     coords))
    return;
...
gimp_free_select_tool_button_release(GimpTool *tool, const GimpCoords *coords,
  ...
  if (gimp_polygon_select_tool_is_grabbed(poly_sel)) {
    gimp_selection_tool_end_change(GIMP_SELECTION_TOOL(tool), !priv->changed);
...
static void gimp_free_select_tool_options_notify(GimpTool *tool,
  ...
  if (!strcmp(pspec->name, "antialias") || !strcmp(pspec->name, "feather") ||
    ...
    if (tool->display) {
      gimp_free_select_tool_change_complete(GIMP_POLYGON_SELECT_TOOL(tool),
                                            tool->display);
    }
  }
...
gimp_free_select_tool_have_selection(GimpSelectionTool *sel_tool,
  ...
  return GIMP_SELECTION_TOOL_CLASS(parent_class)
      ->have_selection(sel_tool, display);
}
...
gimp_free_select_tool_change_complete(GimpPolygonSelectTool *poly_sel,
  ...
  gimp_selection_tool_start_change(GIMP_SELECTION_TOOL(free_sel), FALSE,
                                   priv->operation_at_start);

  if (gimp_polygon_select_tool_is_closed(poly_sel))
    gimp_free_select_tool_select(free_sel, display);

  gimp_selection_tool_end_change(GIMP_SELECTION_TOOL(free_sel), FALSE);
}
...
static gboolean gimp_free_select_tool_select(GimpFreeSelectTool *free_sel,
  ...
  if (n_points > 2) {
    ...
    gimp_channel_select_polygon(
        gimp_image_get_mask(image), C_("command", "Free Select"), n_points,
        points, priv->operation_at_start, options->antialias, options->feather,
        options->feather_radius, options->feather_radius, TRUE);
