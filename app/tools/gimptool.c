...
static void gimp_tool_set_property(GObject *object, guint property_id,
                                   const GValue *value, GParamSpec *pspec);
static void gimp_tool_get_property(GObject *object, guint property_id,
                                   GValue *value, GParamSpec *pspec);

static gboolean gimp_tool_real_has_display(GimpTool *tool,
                                           GimpDisplay *display);
static GimpDisplay *gimp_tool_real_has_image(GimpTool *tool, GimpImage *image);
static gboolean gimp_tool_real_initialize(GimpTool *tool, GimpDisplay *display,
                                          GError **error);
static void gimp_tool_real_control(GimpTool *tool, GimpToolAction action,
                                   GimpDisplay *display);
...
static void gimp_tool_real_button_release(GimpTool *tool,
                                          ...
                                          GimpDisplay *display);
static void gimp_tool_real_motion(GimpTool *tool, const GimpCoords *coords,
                                  guint32 time, GdkModifierType state,
                                  GimpDisplay *display);
static gboolean gimp_tool_real_key_press(GimpTool *tool, GdkEventKey *kevent,
                                         GimpDisplay *display);
static gboolean gimp_tool_real_key_release(GimpTool *tool, GdkEventKey *kevent,
                                           GimpDisplay *display);
static void gimp_tool_real_modifier_key(GimpTool *tool, GdkModifierType key,
                                        gboolean press, GdkModifierType state,
                                        GimpDisplay *display);
static void gimp_tool_real_active_modifier_key(GimpTool *tool,
                                               ...
                                               GdkModifierType state,
                                               GimpDisplay *display);
static void gimp_tool_real_oper_update(GimpTool *tool, const GimpCoords *coords,
                                       GdkModifierType state,
                                       gboolean proximity,
                                       GimpDisplay *display);
...
static const gchar *gimp_tool_real_can_redo(GimpTool *tool,
                                            GimpDisplay *display);
static gboolean gimp_tool_real_undo(GimpTool *tool, GimpDisplay *display);
static gboolean gimp_tool_real_redo(GimpTool *tool, GimpDisplay *display);
...
static void gimp_tool_options_notify(GimpToolOptions *options,
                                     const GParamSpec *pspec, GimpTool *tool);
...
static void gimp_tool_class_init(GimpToolClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_TOOL_INFO,
      g_param_spec_object("tool-info", NULL, NULL, GIMP_TYPE_TOOL_INFO,
...
static void gimp_tool_constructed(GObject *object) {
  ...
  g_signal_connect_object(gimp_tool_get_options(tool), "notify",
                          G_CALLBACK(gimp_tool_options_notify), tool, 0);
}
...
void gimp_tool_set_label(GimpTool *tool, const gchar *label) {
...
void gimp_tool_set_undo_desc(GimpTool *tool, const gchar *undo_desc) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
...
void gimp_tool_set_icon_name(GimpTool *tool, const gchar *icon_name) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
...
void gimp_tool_set_help_id(GimpTool *tool, const gchar *help_id) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
...
gboolean gimp_tool_has_display(GimpTool *tool, GimpDisplay *display) {
...
GimpDisplay *gimp_tool_has_image(GimpTool *tool, GimpImage *image) {
  GimpDisplay *display;
...
gboolean gimp_tool_initialize(GimpTool *tool, GimpDisplay *display) {
  GError *error = NULL;
...
void gimp_tool_control(GimpTool *tool, GimpToolAction action,
                       GimpDisplay *display) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
...
void gimp_tool_button_press(GimpTool *tool, const GimpCoords *coords,
                            guint32 time, GdkModifierType state,
                            GimpButtonPressType press_type,
                            GimpDisplay *display) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
  ...
  GIMP_TOOL_GET_CLASS(tool)->button_press(tool, coords, time, state, press_type,
                                          display);
...
static gboolean gimp_tool_check_click_distance(GimpTool *tool,
                                               const GimpCoords *coords,
                                               guint32 time,
                                               GimpDisplay *display) {
...
void gimp_tool_button_release(GimpTool *tool, const GimpCoords *coords,
                              guint32 time, GdkModifierType state,
                              GimpDisplay *display) {
    ...
    if (gimp_tool_check_click_distance(tool, coords, time, display)) {
      ...
      if (tool->got_motion_event) {
        ...
        GIMP_TOOL_GET_CLASS(tool)->motion(tool, &my_coords, time,
                                          state & GDK_BUTTON1_MASK, display);
      }
...
void gimp_tool_motion(GimpTool *tool, const GimpCoords *coords, guint32 time,
...
void gimp_tool_set_focus_display(GimpTool *tool, GimpDisplay *display) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
...
gboolean gimp_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
                             GimpDisplay *display) {
  g_return_val_if_fail(GIMP_IS_TOOL(tool), FALSE);
  g_return_val_if_fail(GIMP_IS_DISPLAY(display), FALSE);
...
gboolean gimp_tool_key_release(GimpTool *tool, GdkEventKey *kevent,
                               GimpDisplay *display) {
  g_return_val_if_fail(GIMP_IS_TOOL(tool), FALSE);
  g_return_val_if_fail(GIMP_IS_DISPLAY(display), FALSE);
  g_return_val_if_fail(display == tool->focus_display, FALSE);
...
static void gimp_tool_modifier_key(GimpTool *tool, GdkModifierType key,
                                   gboolean press, GdkModifierType state,
                                   GimpDisplay *display) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
  g_return_if_fail(GIMP_IS_DISPLAY(display));
...
static void gimp_tool_active_modifier_key(GimpTool *tool, GdkModifierType key,
                                          gboolean press, GdkModifierType state,
                                          GimpDisplay *display) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
  g_return_if_fail(GIMP_IS_DISPLAY(display));
...
static gboolean state_changed(GdkModifierType old_state,
                              GdkModifierType new_state,
                              GdkModifierType modifier, gboolean *press) {
...
void gimp_tool_set_modifier_state(GimpTool *tool, GdkModifierType state,
                                  GimpDisplay *display) {
  gboolean press;
  ...
  GIMP_LOG(TOOL_FOCUS, "tool: %p  display: %p  tool->focus_display: %p", tool,
           display, tool->focus_display);

  g_return_if_fail(display == tool->focus_display);

  if (state_changed(tool->modifier_state, state, GDK_SHIFT_MASK, &press)) {
    gimp_tool_modifier_key(tool, GDK_SHIFT_MASK, press, state, display);
  }

  if (state_changed(tool->modifier_state, state, GDK_CONTROL_MASK, &press)) {
    gimp_tool_modifier_key(tool, GDK_CONTROL_MASK, press, state, display);
  }

  if (state_changed(tool->modifier_state, state, GDK_MOD1_MASK, &press)) {
    gimp_tool_modifier_key(tool, GDK_MOD1_MASK, press, state, display);
  }

  if (state_changed(tool->modifier_state, state, GDK_MOD2_MASK, &press)) {
    gimp_tool_modifier_key(tool, GDK_MOD2_MASK, press, state, display);
  }

  tool->modifier_state = state;
}

void gimp_tool_set_active_modifier_state(GimpTool *tool, GdkModifierType state,
                                         GimpDisplay *display) {
  ...
  if (state_changed(tool->active_modifier_state, state, GDK_SHIFT_MASK,
    ...
    case GIMP_TOOL_ACTIVE_MODIFIERS_SEPARATE:
      if (!press && (tool->button_press_state & GDK_SHIFT_MASK)) {
        ...
        gimp_tool_active_modifier_key(tool, GDK_SHIFT_MASK, press, state,
                                      display);
      }
      break;
    }
  ...
  if (state_changed(tool->active_modifier_state, state, GDK_CONTROL_MASK,
    ...
    case GIMP_TOOL_ACTIVE_MODIFIERS_SEPARATE:
      if (!press && (tool->button_press_state & GDK_CONTROL_MASK)) {
        ...
        gimp_tool_active_modifier_key(tool, GDK_CONTROL_MASK, press, state,
                                      display);
      }
      break;
    }
  ...
  if (state_changed(tool->active_modifier_state, state, GDK_MOD1_MASK,
    ...
    case GIMP_TOOL_ACTIVE_MODIFIERS_SEPARATE:
      if (!press && (tool->button_press_state & GDK_MOD1_MASK)) {
        ...
        gimp_tool_active_modifier_key(tool, GDK_MOD1_MASK, press, state,
                                      display);
      }
      break;
    }
  ...
  if (state_changed(tool->active_modifier_state, state, GDK_MOD2_MASK,
    ...
    case GIMP_TOOL_ACTIVE_MODIFIERS_SEPARATE:
      if (!press && (tool->button_press_state & GDK_MOD2_MASK)) {
        ...
        gimp_tool_active_modifier_key(tool, GDK_MOD2_MASK, press, state,
                                      display);
      }
      break;
    }
...
void gimp_tool_oper_update(GimpTool *tool, const GimpCoords *coords,
...
void gimp_tool_cursor_update(GimpTool *tool, const GimpCoords *coords,
                             GdkModifierType state, GimpDisplay *display) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
  g_return_if_fail(coords != NULL);
  g_return_if_fail(GIMP_IS_DISPLAY(display));
...
const gchar *gimp_tool_can_undo(GimpTool *tool, GimpDisplay *display) {
  g_return_val_if_fail(GIMP_IS_TOOL(tool), NULL);
...
const gchar *gimp_tool_can_redo(GimpTool *tool, GimpDisplay *display) {
  g_return_val_if_fail(GIMP_IS_TOOL(tool), NULL);
  g_return_val_if_fail(GIMP_IS_DISPLAY(display), NULL);
...
gboolean gimp_tool_undo(GimpTool *tool, GimpDisplay *display) {
  g_return_val_if_fail(GIMP_IS_TOOL(tool), FALSE);
  g_return_val_if_fail(GIMP_IS_DISPLAY(display), FALSE);
...
gboolean gimp_tool_redo(GimpTool *tool, GimpDisplay *display) {
  g_return_val_if_fail(GIMP_IS_TOOL(tool), FALSE);
  g_return_val_if_fail(GIMP_IS_DISPLAY(display), FALSE);
...
GimpUIManager *gimp_tool_get_popup(GimpTool *tool, const GimpCoords *coords,
                                   GdkModifierType state, GimpDisplay *display,
                                   const gchar **ui_path) {
  g_return_val_if_fail(GIMP_IS_TOOL(tool), NULL);
...
void gimp_tool_push_status(GimpTool *tool, GimpDisplay *display,
                           const gchar *format, ...) {
  GimpDisplayShell *shell;
  ...
  gimp_statusbar_push_valist(gimp_display_shell_get_statusbar(shell),
                             G_OBJECT_TYPE_NAME(tool), icon_name, format, args);
...
void gimp_tool_push_status_coords(GimpTool *tool, GimpDisplay *display,
  ...
  gimp_statusbar_push_coords(gimp_display_shell_get_statusbar(shell),
                             G_OBJECT_TYPE_NAME(tool), icon_name, precision,
...
void gimp_tool_push_status_length(GimpTool *tool, GimpDisplay *display,
                                  const gchar *title, GimpOrientationType axis,
                                  gdouble value, const gchar *help) {
  GimpDisplayShell *shell;
  const gchar *icon_name;
  ...
  gimp_statusbar_push_length(gimp_display_shell_get_statusbar(shell),
                             G_OBJECT_TYPE_NAME(tool), icon_name, title, axis,
                             value, help);
...
void gimp_tool_replace_status(GimpTool *tool, GimpDisplay *display,
                              const gchar *format, ...) {
  GimpDisplayShell *shell;
  const gchar *icon_name;
  va_list args;
  ...
  gimp_statusbar_replace_valist(gimp_display_shell_get_statusbar(shell),
                                G_OBJECT_TYPE_NAME(tool), icon_name, format,
                                args);
...
void gimp_tool_pop_status(GimpTool *tool, GimpDisplay *display) {
  GimpDisplayShell *shell;
...
void gimp_tool_message(GimpTool *tool, GimpDisplay *display,
                       const gchar *format, ...) {
  va_list args;
  ...
  gimp_message_valist(display->gimp, G_OBJECT(display), GIMP_MESSAGE_WARNING,
                      format, args);

  va_end(args);
}

void gimp_tool_message_literal(GimpTool *tool, GimpDisplay *display,
                               const gchar *message) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
  g_return_if_fail(GIMP_IS_DISPLAY(display));
  g_return_if_fail(message != NULL);

  gimp_message_literal(display->gimp, G_OBJECT(display), GIMP_MESSAGE_WARNING,
                       message);
}

void gimp_tool_set_cursor(GimpTool *tool, GimpDisplay *display,
                          GimpCursorType cursor, GimpToolCursorType tool_cursor,
                          GimpCursorModifier modifier) {
  g_return_if_fail(GIMP_IS_TOOL(tool));
  g_return_if_fail(GIMP_IS_DISPLAY(display));

  gimp_display_shell_set_cursor(gimp_display_get_shell(display), cursor,
...
static void gimp_tool_release(GimpTool *tool) {
  if (tool->last_pointer_state && gimp_tool_control_is_active(tool->control)) {
    gimp_tool_button_release(tool, &tool->last_pointer_coords,
                             tool->last_pointer_time +
                                 g_get_monotonic_time() / 1000,
                             tool->last_pointer_state, tool->display);
  }
}
