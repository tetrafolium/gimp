...
static void gimp_magnify_tool_button_release(GimpTool *tool,
                                             ...
                                             GimpDisplay *display);
static void gimp_magnify_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                     guint32 time, GdkModifierType state,
                                     GimpDisplay *display);
static void gimp_magnify_tool_modifier_key(GimpTool *tool, GdkModifierType key,
                                           gboolean press,
                                           GdkModifierType state,
                                           GimpDisplay *display);
...
void gimp_magnify_tool_register(GimpToolRegisterCallback callback,
                                gpointer data) {
  (*callback)(GIMP_TYPE_MAGNIFY_TOOL, GIMP_TYPE_MAGNIFY_OPTIONS,
              gimp_magnify_options_gui, 0, "gimp-zoom-tool", _("Zoom"),
              _("Zoom Tool: Adjust the zoom level"), N_("_Zoom"), "Z", NULL,
              GIMP_HELP_TOOL_ZOOM, GIMP_ICON_TOOL_ZOOM, data);
}
...
static void gimp_magnify_tool_init(GimpMagnifyTool *magnify_tool) {
  ...
  gimp_tool_control_set_tool_cursor(tool->control, GIMP_TOOL_CURSOR_ZOOM);
...
static void gimp_magnify_tool_button_release(GimpTool *tool,
  ...
  case GIMP_BUTTON_RELEASE_NO_MOTION:
    gimp_display_shell_scale(shell, options->zoom_type, 0.0,
  ...
  case GIMP_BUTTON_RELEASE_NORMAL: {
    ...
    gimp_display_shell_scale_to_rectangle(shell, options->zoom_type, x, y,
...
static void gimp_magnify_tool_draw(GimpDrawTool *draw_tool) {
  ...
  magnify->rectangle = gimp_draw_tool_add_rectangle(
      draw_tool, FALSE, magnify->x, magnify->y, magnify->w, magnify->h);
}

static void gimp_magnify_tool_update_items(GimpMagnifyTool *magnify) {
  if (gimp_draw_tool_is_active(GIMP_DRAW_TOOL(magnify))) {
    gimp_canvas_rectangle_set(magnify->rectangle, magnify->x, magnify->y,
                              magnify->w, magnify->h);
  }
}
