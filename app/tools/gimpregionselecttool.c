...
static void gimp_region_select_tool_button_press(GimpTool *tool,
                                                 ...
                                                 GimpDisplay *display);
static void
gimp_region_select_tool_button_release(GimpTool *tool, const GimpCoords *coords,
...
static void gimp_region_select_tool_init(GimpRegionSelectTool *region_select) {
...
static void gimp_region_select_tool_button_press(GimpTool *tool,
  ...
  if (gimp_selection_tool_start_edit(GIMP_SELECTION_TOOL(region_sel), display,
                                     coords)) {
    return;
  }
...
gimp_region_select_tool_button_release(GimpTool *tool, const GimpCoords *coords,
  ...
  if (options->draw_mask)
    gimp_display_shell_set_mask(gimp_display_get_shell(display), NULL, 0, 0,
                                NULL, FALSE);
  ...
  if (release_type != GIMP_BUTTON_RELEASE_CANCEL) {
    if (GIMP_SELECTION_TOOL(tool)->function == SELECTION_ANCHOR) {
      ...
      gimp_channel_select_buffer(
          gimp_image_get_mask(image),
          GIMP_REGION_SELECT_TOOL_GET_CLASS(tool)->undo_desc,
          region_sel->region_mask, off_x, off_y, sel_options->operation,
  ...
  g_object_set(options, "threshold", region_sel->saved_threshold, NULL);
}
...
static void gimp_region_select_tool_motion(GimpTool *tool,
  ...
  g_object_set(options, "threshold",
               CLAMP(region_sel->saved_threshold + diff, 0, 255), NULL);
...
static void gimp_region_select_tool_cursor_update(GimpTool *tool,
  ...
  if (!gimp_image_coords_in_active_pickable(image, coords, FALSE,
...
static void gimp_region_select_tool_draw(GimpDrawTool *draw_tool) {
  ...
  GimpRegionSelectOptions *options =
      GIMP_REGION_SELECT_TOOL_GET_OPTIONS(draw_tool);
  ...
  if (!options->draw_mask && region_sel->region_mask) {
    if (!region_sel->segs) {
       ...
       */
      region_sel->segs = gimp_boundary_find(
          region_sel->region_mask, NULL, babl_format("Y float"),
          GIMP_BOUNDARY_WITHIN_BOUNDS, 0, 0,
          gegl_buffer_get_width(region_sel->region_mask),
          gegl_buffer_get_height(region_sel->region_mask),
          GIMP_BOUNDARY_HALF_WAY, &region_sel->n_segs);
    }
    ...
    if (region_sel->segs) {
      ...
      gimp_draw_tool_add_boundary(draw_tool, region_sel->segs,
                                  region_sel->n_segs, NULL, off_x, off_y);
    }
  }
}
...
static void gimp_region_select_tool_get_mask(GimpRegionSelectTool *region_sel,
                                             GimpDisplay *display) {
  GimpRegionSelectOptions *options =
      GIMP_REGION_SELECT_TOOL_GET_OPTIONS(region_sel);
  ...
  region_sel->region_mask = GIMP_REGION_SELECT_TOOL_GET_CLASS(region_sel)
                                ->get_mask(region_sel, display);
  ...
  if (options->draw_mask) {
    if (region_sel->region_mask) {
      ...
      gimp_display_shell_set_mask(shell, region_sel->region_mask, off_x, off_y,
                                  &color, FALSE);
    } else {
