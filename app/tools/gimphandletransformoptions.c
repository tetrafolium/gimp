...
static void gimp_handle_transform_options_class_init(
    GimpHandleTransformOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(
      object_class, PROP_HANDLE_MODE, "handle-mode", _("Handle mode"),
      _("Handle mode"), GIMP_TYPE_TRANSFORM_HANDLE_MODE,
      GIMP_HANDLE_MODE_ADD_TRANSFORM, GIMP_PARAM_STATIC_STRINGS);
}
...
GtkWidget *gimp_handle_transform_options_gui(GimpToolOptions *tool_options) {
  ...
  frame = gimp_prop_enum_radio_frame_new(config, "handle-mode", NULL, 0, 0);
  ...
  if (GTK_IS_RADIO_BUTTON(button)) {
    ...
    for (i = g_slist_length(list) - 1; list; list = list->next, i--) {
      ...
      if (modifier) {
        ...
        tip =
            g_strdup_printf("%s  (%s)", tooltip, gimp_get_mod_string(modifier));
