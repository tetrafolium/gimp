...
static void gimp_paint_tool_control(GimpTool *tool, GimpToolAction action,
                                    GimpDisplay *display);
...
static void gimp_paint_tool_button_release(GimpTool *tool,
                                           ...
                                           GimpDisplay *display);
static void gimp_paint_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                   guint32 time, GdkModifierType state,
                                   GimpDisplay *display);
static void gimp_paint_tool_modifier_key(GimpTool *tool, GdkModifierType key,
...
static void gimp_paint_tool_real_paint_prepare(GimpPaintTool *paint_tool,
                                               GimpDisplay *display);

static GimpCanvasItem *gimp_paint_tool_get_outline(GimpPaintTool *paint_tool,
                                                   GimpDisplay *display,
                                                   gdouble x, gdouble y);
...
static void gimp_paint_tool_constructed(GObject *object) {
  ...
  paint_tool->core = g_object_new(paint_info->paint_type, "undo-desc",
                                  paint_info->blurb, NULL);

  g_signal_connect_object(options, "notify::hard",
                          G_CALLBACK(gimp_paint_tool_hard_notify), paint_tool,
                          0);
  ...
  g_signal_connect_object(display_config, "notify::show-paint-tool-cursor",
                          G_CALLBACK(gimp_paint_tool_cursor_notify), paint_tool,
                          0);
...
static void gimp_paint_tool_button_press(GimpTool *tool,
  ...
  if (!gimp_paint_tool_paint_start(paint_tool, display, coords, time, constrain,
                                   &error)) {
    gimp_tool_message_literal(tool, display, error->message);
    g_clear_error(&error);
    return;
...
static void gimp_paint_tool_cursor_update(GimpTool *tool,
  ...
  if (!gimp_color_tool_is_enabled(GIMP_COLOR_TOOL(tool))) {
    ...
    if (!paint_tool->show_cursor && modifier != GIMP_CURSOR_MODIFIER_BAD) {
      gimp_tool_set_cursor(tool, display, GIMP_CURSOR_NONE,
   ...
   */
  gimp_tool_control_set_cursor_modifier(tool->control, old_modifier);
  gimp_tool_control_set_toggle_cursor_modifier(tool->control,
                                               old_toggle_modifier);
}
...
static void gimp_paint_tool_oper_update(GimpTool *tool,
  ...
  if (g_list_length(drawables) == 1 && proximity) {
    ...
    if (display == tool->display && (state & GIMP_PAINT_TOOL_LINE_MASK)) {
      ...
      gimp_display_shell_get_constrained_line_params(shell, &offset_angle,
                                                     &xres, &yres);

      gimp_paint_core_round_line(core, paint_options,
                                 (state & constrain_mask) != 0, offset_angle,
                                 xres, yres);

      status_help = gimp_suggest_modifiers(
          paint_tool->status_line, constrain_mask & ~state, NULL,
          _("%s for constrained angles"), NULL);

      status = gimp_display_shell_get_line_status(
          shell, status_help, ". ", core->last_coords.x, core->last_coords.y,
      ...
      status = gimp_suggest_modifiers(paint_tool->status, modifiers & ~state,
...
static void gimp_paint_tool_draw(GimpDrawTool *draw_tool) {
  ...
  if (paint_tool->active &&
    ...
    if (gimp_paint_tool_paint_is_active(paint_tool) && paint_tool->snap_brush) {
      ...
      if (paint_tool->draw_line &&
        ...
        gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_CIRCLE, last_x, last_y,
                                  GIMP_TOOL_HANDLE_SIZE_CIRCLE,
                                  GIMP_TOOL_HANDLE_SIZE_CIRCLE,
                                  GIMP_HANDLE_ANCHOR_CENTER);

        gimp_draw_tool_add_line(draw_tool, last_x, last_y, cur_x, cur_y);

        gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_CIRCLE, cur_x, cur_y,
                                  GIMP_TOOL_HANDLE_SIZE_CIRCLE,
                                  GIMP_TOOL_HANDLE_SIZE_CIRCLE,
                                  GIMP_HANDLE_ANCHOR_CENTER);
    ...
    if (paint_tool->draw_brush)
      outline = gimp_paint_tool_get_outline(paint_tool, draw_tool->display,
                                            cur_x, cur_y);
...
#define ROTATION_ANGLE G_PI / 4
      ...
      gimp_draw_tool_add_arc(
          draw_tool, FALSE, cur_x - (size / 2.0), cur_y - (size / 2.0), size,
          size, ROTATION_ANGLE - (2.0 * G_PI) / (TICKMARK_ANGLE * 2),
          (2.0 * G_PI) / TICKMARK_ANGLE);

      gimp_draw_tool_add_arc(
          draw_tool, FALSE, cur_x - (size / 2.0), cur_y - (size / 2.0), size,
          size, ROTATION_ANGLE + G_PI / 2 - (2.0 * G_PI) / (TICKMARK_ANGLE * 2),
          (2.0 * G_PI) / TICKMARK_ANGLE);

      gimp_draw_tool_add_arc(
          draw_tool, FALSE, cur_x - (size / 2.0), cur_y - (size / 2.0), size,
          size, ROTATION_ANGLE + G_PI - (2.0 * G_PI) / (TICKMARK_ANGLE * 2),
          (2.0 * G_PI) / TICKMARK_ANGLE);

      gimp_draw_tool_add_arc(draw_tool, FALSE, cur_x - (size / 2.0),
                             cur_y - (size / 2.0), size, size,
                             ROTATION_ANGLE + 3 * G_PI / 2 -
                                 (2.0 * G_PI) / (TICKMARK_ANGLE * 2),
                             (2.0 * G_PI) / TICKMARK_ANGLE);
      ...
      gimp_draw_tool_add_arc(draw_tool, FALSE, cur_x - (size / 2.0),
                             cur_y - (size / 2.0), size, size, 0.0,
                             (2.0 * G_PI));
    }

    if (!outline && !line_drawn && !paint_tool->show_cursor &&
        !paint_tool->draw_circle) {
      /*  don't leave the user without any indication and draw
       *  a fallback crosshair
       */
      gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_CROSSHAIR, cur_x, cur_y,
...
static GimpCanvasItem *gimp_paint_tool_get_outline(GimpPaintTool *paint_tool,
  ...
  if (GIMP_PAINT_TOOL_GET_CLASS(paint_tool)->get_outline)
    return GIMP_PAINT_TOOL_GET_CLASS(paint_tool)
        ->get_outline(paint_tool, display, x, y);
...
static void gimp_paint_tool_hard_notify(GimpPaintOptions *options,
  ...
  if (paint_tool->active) {
    ...
    gimp_tool_control_set_precision(
        tool->control, options->hard ? GIMP_CURSOR_PRECISION_PIXEL_CENTER
