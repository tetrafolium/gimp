...
static void
gimp_sample_point_tool_button_release(GimpTool *tool, const GimpCoords *coords,
...
static void gimp_sample_point_tool_class_init(GimpSamplePointToolClass *klass) {
...
static void gimp_sample_point_tool_init(GimpSamplePointTool *sp_tool) {
...
gimp_sample_point_tool_button_release(GimpTool *tool, const GimpCoords *coords,
  ...
  if (release_type != GIMP_BUTTON_RELEASE_CANCEL) {
    ...
    if (sp_tool->sample_point_x == GIMP_SAMPLE_POINT_POSITION_UNDEFINED ||
        sp_tool->sample_point_x < 0 || sp_tool->sample_point_x >= width ||
        sp_tool->sample_point_y == GIMP_SAMPLE_POINT_POSITION_UNDEFINED ||
        sp_tool->sample_point_y < 0 || sp_tool->sample_point_y >= height) {
      if (sp_tool->sample_point) {
        gimp_image_remove_sample_point(image, sp_tool->sample_point, TRUE);
        sp_tool->sample_point = NULL;
      }
    } else {
      if (sp_tool->sample_point) {
        gimp_image_move_sample_point(image, sp_tool->sample_point,
                                     sp_tool->sample_point_x,
                                     sp_tool->sample_point_y, TRUE);
      } else {
        sp_tool->sample_point = gimp_image_add_sample_point_at_pos(
            image, sp_tool->sample_point_x, sp_tool->sample_point_y, TRUE);
      }
    }
...

{
  ...
  if (tx < 0 || tx >= shell->disp_width || ty < 0 || ty >= shell->disp_height) {
    sp_tool->sample_point_x = GIMP_SAMPLE_POINT_POSITION_UNDEFINED;
    sp_tool->sample_point_y = GIMP_SAMPLE_POINT_POSITION_UNDEFINED;
    ...
    if (sp_tool->sample_point_x < 0 || sp_tool->sample_point_x >= height ||
        sp_tool->sample_point_y < 0 || sp_tool->sample_point_y >= width) {
      delete_point = TRUE;
    }
  }
  ...
  if (delete_point) {
    gimp_tool_push_status(tool, display,
                          sp_tool->sample_point ? _("Remove Sample Point")
    ...
    gimp_tool_push_status_coords(
        ...
        sp_tool->sample_point_y - sp_tool->sample_point_old_y, NULL);
  } else {
    gimp_tool_push_status_coords(
        tool, display, gimp_tool_control_get_precision(tool->control),
        _("Add Sample Point: "), sp_tool->sample_point_x, ", ",
        sp_tool->sample_point_y, NULL);
  }
}
...
static void gimp_sample_point_tool_draw(GimpDrawTool *draw_tool) {
  ...
  if (sp_tool->sample_point_x != GIMP_SAMPLE_POINT_POSITION_UNDEFINED &&
      sp_tool->sample_point_y != GIMP_SAMPLE_POINT_POSITION_UNDEFINED) {
    gimp_draw_tool_add_crosshair(draw_tool, sp_tool->sample_point_x,
...
static void gimp_sample_point_tool_start(GimpTool *parent_tool,
  ...
  sp_tool = g_object_new(GIMP_TYPE_SAMPLE_POINT_TOOL, "tool-info",
  ...
  if (sample_point) {
    ...
    gimp_sample_point_get_position(sample_point, &sp_tool->sample_point_old_x,
