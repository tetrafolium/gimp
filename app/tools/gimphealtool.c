...
void gimp_heal_tool_register(GimpToolRegisterCallback callback, gpointer data) {
  (*callback)(GIMP_TYPE_HEAL_TOOL, GIMP_TYPE_SOURCE_OPTIONS,
              gimp_heal_options_gui, GIMP_PAINT_OPTIONS_CONTEXT_MASK,
              "gimp-heal-tool", _("Healing"),
              _("Healing Tool: Heal image irregularities"), N_("_Heal"), "H",
              NULL, GIMP_HELP_TOOL_HEAL, GIMP_ICON_TOOL_HEAL, data);
}

static void gimp_heal_tool_class_init(GimpHealToolClass *klass) {}

static void gimp_heal_tool_init(GimpHealTool *heal) {
...
static GtkWidget *gimp_heal_options_gui(GimpToolOptions *tool_options) {
  ...
  button =
      gimp_prop_check_button_new(config, "sample-merged", _("Sample merged"));
