...
GtkWidget *gimp_filter_tool_get_settings_box(GimpFilterTool *filter_tool) {
  ...
  box = gimp_settings_box_new(tool_info->gimp, filter_tool->config,
                              filter_tool->settings, import_title, export_title,
  ...
  g_signal_connect(box, "import", G_CALLBACK(gimp_filter_tool_settings_import),
                   filter_tool);

  g_signal_connect(box, "export", G_CALLBACK(gimp_filter_tool_settings_export),
                   filter_tool);

  g_signal_connect_swapped(
      box, "selected", G_CALLBACK(gimp_filter_tool_set_config), filter_tool);
...
gboolean gimp_filter_tool_real_settings_import(GimpFilterTool *filter_tool,
                                               GInputStream *input,
                                               GError **error) {
  return gimp_config_deserialize_stream(GIMP_CONFIG(filter_tool->config), input,
                                        NULL, error);
}
...
gboolean gimp_filter_tool_real_settings_export(GimpFilterTool *filter_tool,
  ...
  header = g_strdup_printf("GIMP '%s' settings", gimp_tool_get_label(tool));
  footer = g_strdup_printf("end of '%s' settings", gimp_tool_get_label(tool));

  success = gimp_config_serialize_to_stream(
      GIMP_CONFIG(filter_tool->config), output, header, footer, NULL, error);
...
static gboolean gimp_filter_tool_settings_import(GimpSettingsBox *box,
  ...
  if (!tool_class->settings_import(filter_tool, input, &error)) {
    gimp_message(GIMP_TOOL(filter_tool)->tool_info->gimp,
                 G_OBJECT(gimp_tool_gui_get_dialog(filter_tool->gui)),
                 GIMP_MESSAGE_ERROR, _("Error reading '%s': %s"),
                 gimp_file_get_utf8_name(file), error->message);
    g_clear_error(&error);
...
static gboolean gimp_filter_tool_settings_export(GimpSettingsBox *box,
  ...
  if (!tool_class->settings_export(filter_tool, output, &error)) {
    ...
    gimp_message(GIMP_TOOL(filter_tool)->tool_info->gimp,
                 G_OBJECT(gimp_tool_gui_get_dialog(filter_tool->gui)),
                 GIMP_MESSAGE_ERROR, _("Error writing '%s': %s"),
                 gimp_file_get_utf8_name(file), error->message);
    g_clear_error(&error);
  ...
  gimp_message(GIMP_TOOL(filter_tool)->tool_info->gimp,
               G_OBJECT(GIMP_TOOL(filter_tool)->display), GIMP_MESSAGE_INFO,
               _("Settings saved to '%s'"), gimp_file_get_utf8_name(file));
