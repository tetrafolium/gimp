...
static gboolean gimp_brightness_contrast_tool_initialize(GimpTool *tool,
                                                         GimpDisplay *display,
                                                         GError **error);
static void gimp_brightness_contrast_tool_button_press(
    GimpTool *tool, const GimpCoords *coords, guint32 time,
    GdkModifierType state, GimpButtonPressType press_type,
    GimpDisplay *display);
static void gimp_brightness_contrast_tool_button_release(
    GimpTool *tool, const GimpCoords *coords, guint32 time,
...
void gimp_brightness_contrast_tool_register(GimpToolRegisterCallback callback,
                                            gpointer data) {
  (*callback)(GIMP_TYPE_BRIGHTNESS_CONTRAST_TOOL, GIMP_TYPE_FILTER_OPTIONS,
              NULL, 0, "gimp-brightness-contrast-tool",
              _("Brightness-Contrast"), _("Adjust brightness and contrast"),
              N_("B_rightness-Contrast..."), NULL, NULL,
              GIMP_HELP_TOOL_BRIGHTNESS_CONTRAST,
              GIMP_ICON_TOOL_BRIGHTNESS_CONTRAST, data);
}

static void gimp_brightness_contrast_tool_class_init(
    GimpBrightnessContrastToolClass *klass) {
  ...
  filter_tool_class->get_operation =
      gimp_brightness_contrast_tool_get_operation;
...
static gboolean gimp_brightness_contrast_tool_initialize(GimpTool *tool,
  ...
  if (gimp_drawable_get_component_type(drawables->data) ==
      GIMP_COMPONENT_TYPE_U8) {
    gimp_prop_widget_set_factor(bc_tool->brightness_scale, 127.0, 1.0, 8.0, 0);
    gimp_prop_widget_set_factor(bc_tool->contrast_scale, 127.0, 1.0, 8.0, 0);
  } else {
    gimp_prop_widget_set_factor(bc_tool->brightness_scale, 0.5, 0.01, 0.1, 3);
    gimp_prop_widget_set_factor(bc_tool->contrast_scale, 0.5, 0.01, 0.1, 3);
  }
...
static void gimp_brightness_contrast_tool_button_press(
  ...
  if (!bc_tool->dragging) {
    ...
    g_object_get(GIMP_FILTER_TOOL(tool)->config, "brightness", &brightness,
...
static void gimp_brightness_contrast_tool_motion(GimpTool *tool,
  ...
  if (!bc_tool->dragging) {
    ...
    g_object_set(GIMP_FILTER_TOOL(tool)->config, "brightness",
                 CLAMP(bc_tool->dy, -127.0, 127.0) / 127.0, "contrast",
                 CLAMP(bc_tool->dx, -127.0, 127.0) / 127.0, NULL);
  }
}
...
static void gimp_brightness_contrast_tool_dialog(GimpFilterTool *filter_tool) {
  GimpBrightnessContrastTool *bc_tool =
      GIMP_BRIGHTNESS_CONTRAST_TOOL(filter_tool);
  GtkWidget *main_vbox;
  GtkWidget *scale;
  GtkWidget *button;
...
brightness_contrast_to_levels_callback(GtkWidget *widget,
  ...
  levels = gimp_brightness_contrast_config_to_levels_config(
      GIMP_BRIGHTNESS_CONTRAST_CONFIG(filter_tool->config));

  gimp_filter_tool_edit_as(filter_tool, "gimp-levels-tool",
                           GIMP_CONFIG(levels));
