...
static void gimp_filter_options_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_filter_options_get_property(GObject *object, guint property_id,
                                             GValue *value, GParamSpec *pspec);

G_DEFINE_TYPE(GimpFilterOptions, gimp_filter_options, GIMP_TYPE_COLOR_OPTIONS)

#define parent_class gimp_filter_options_parent_class

static void gimp_filter_options_class_init(GimpFilterOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_PREVIEW, "preview", _("_Preview"),
                           NULL, TRUE, GIMP_PARAM_STATIC_STRINGS);

  g_object_class_install_property(
      object_class, PROP_PREVIEW_SPLIT,
      g_param_spec_boolean("preview-split", _("Split _view"), NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_PREVIEW_SPLIT_ALIGNMENT,
      g_param_spec_enum("preview-split-alignment", NULL, NULL,
                        GIMP_TYPE_ALIGNMENT_TYPE, GIMP_ALIGN_LEFT,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_PREVIEW_SPLIT_POSITION,
      g_param_spec_int("preview-split-position", NULL, NULL, G_MININT, G_MAXINT,
                       0, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_CONTROLLER, "controller", _("On-canvas con_trols"),
      _("Show on-canvas filter controls"), TRUE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_BLENDING_OPTIONS_EXPANDED,
                           "blending-options-expanded", NULL, NULL, FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_COLOR_OPTIONS_EXPANDED,
                           "color-options-expanded", NULL, NULL, FALSE,
                           GIMP_PARAM_STATIC_STRINGS);
}

static void gimp_filter_options_init(GimpFilterOptions *options) {}
...
void gimp_filter_options_switch_preview_orientation(GimpFilterOptions *options,
  ...
  if (alignment == GIMP_ALIGN_LEFT || alignment == GIMP_ALIGN_RIGHT) {
    position = position_x;
  } else {
    position = position_y;
  }

  g_object_set(options, "preview-split-alignment", alignment,
               "preview-split-position", position, NULL);
}
