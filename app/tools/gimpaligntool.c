...
static void gimp_align_tool_control(GimpTool *tool, GimpToolAction action,
                                    GimpDisplay *display);
...
static void gimp_align_tool_button_release(GimpTool *tool,
                                           ...
                                           GimpDisplay *display);
static void gimp_align_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                   guint32 time, GdkModifierType state,
                                   GimpDisplay *display);
static gboolean gimp_align_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
                                          GimpDisplay *display);
static void gimp_align_tool_oper_update(GimpTool *tool,
                                        ...
                                        gboolean proximity,
                                        GimpDisplay *display);
static void gimp_align_tool_status_update(GimpTool *tool, GimpDisplay *display,
                                          GdkModifierType state,
                                          gboolean proximity);
...
void gimp_align_tool_register(GimpToolRegisterCallback callback,
                              gpointer data) {
  (*callback)(GIMP_TYPE_ALIGN_TOOL, GIMP_TYPE_ALIGN_OPTIONS,
              gimp_align_options_gui, 0, "gimp-align-tool", _("Alignment"),
              _("Alignment Tool: Align or arrange layers and other objects"),
              N_("_Align"), "Q", NULL, GIMP_HELP_TOOL_ALIGN,
              GIMP_ICON_TOOL_ALIGN, data);
}
...
static void gimp_align_tool_constructed(GObject *object) {
  ...
  g_signal_connect_object(options, "align-button-clicked",
                          G_CALLBACK(gimp_align_tool_align), align_tool,
                          G_CONNECT_SWAPPED);
}
...
static void gimp_align_tool_button_release(GimpTool *tool,
   ...
   */
  if (hypot(coords->x - align_tool->x1, coords->y - align_tool->y1) < EPSILON) {
    GimpVectors *vectors;
    GimpGuide *guide;
    GimpLayer *layer;
    gint snap_distance = display->config->snap_distance;

    if ((vectors = gimp_image_pick_vectors(image, coords->x, coords->y,
    ...
    } else if ((layer = gimp_image_pick_layer_by_bounds(image, coords->x,
                                                        coords->y))) {
  ...
  } else /* FIXME: look for vectors too */
  {
    ...
    for (list = all_layers; list; list = g_list_next(list)) {
      ...
      g_signal_connect(layer, "removed",
                       G_CALLBACK(gimp_align_tool_object_removed), align_tool);
    }
...
static void gimp_align_tool_oper_update(GimpTool *tool,
  ...
  if (gimp_image_pick_vectors(image, coords->x, coords->y,
                              FUNSCALEX(shell, snap_distance),
                              FUNSCALEY(shell, snap_distance))) {
    if (add)
...
static void gimp_align_tool_draw(GimpDrawTool *draw_tool) {
  ...
  for (list = align_tool->selected_objects; list; list = g_list_next(list)) {
    if (GIMP_IS_ITEM(list->data)) {
      ...
      gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_FILLED_SQUARE, x, y,
                                GIMP_TOOL_HANDLE_SIZE_SMALL,
                                GIMP_TOOL_HANDLE_SIZE_SMALL,
                                GIMP_HANDLE_ANCHOR_NORTH_WEST);
      gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_FILLED_SQUARE, x + w, y,
                                GIMP_TOOL_HANDLE_SIZE_SMALL,
                                GIMP_TOOL_HANDLE_SIZE_SMALL,
                                GIMP_HANDLE_ANCHOR_NORTH_EAST);
      gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_FILLED_SQUARE, x, y + h,
                                GIMP_TOOL_HANDLE_SIZE_SMALL,
                                GIMP_TOOL_HANDLE_SIZE_SMALL,
                                GIMP_HANDLE_ANCHOR_SOUTH_WEST);
      gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_FILLED_SQUARE, x + w,
                                y + h, GIMP_TOOL_HANDLE_SIZE_SMALL,
                                GIMP_TOOL_HANDLE_SIZE_SMALL,
      ...
      case GIMP_ORIENTATION_VERTICAL:
        ...
        gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_FILLED_SQUARE, x, h,
                                  GIMP_TOOL_HANDLE_SIZE_SMALL,
                                  GIMP_TOOL_HANDLE_SIZE_SMALL,
                                  GIMP_HANDLE_ANCHOR_SOUTH);
        gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_FILLED_SQUARE, x, 0,
                                  GIMP_TOOL_HANDLE_SIZE_SMALL,
                                  GIMP_TOOL_HANDLE_SIZE_SMALL,
      ...
      case GIMP_ORIENTATION_HORIZONTAL:
        ...
        gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_FILLED_SQUARE, w, y,
                                  GIMP_TOOL_HANDLE_SIZE_SMALL,
                                  GIMP_TOOL_HANDLE_SIZE_SMALL,
                                  GIMP_HANDLE_ANCHOR_EAST);
        gimp_draw_tool_add_handle(draw_tool, GIMP_HANDLE_FILLED_SQUARE, 0, y,
                                  GIMP_TOOL_HANDLE_SIZE_SMALL,
                                  GIMP_TOOL_HANDLE_SIZE_SMALL,
...
static void gimp_align_tool_align(GimpAlignTool *align_tool,
  ...
  gimp_image_arrange_objects(image, list, align_type, reference_object,
                             align_type, offset);
...
static void gimp_align_tool_object_removed(GObject *object,
  ...
  if (align_tool->selected_objects)
    g_signal_handlers_disconnect_by_func(object, gimp_align_tool_object_removed,
                                         align_tool);

  align_tool->selected_objects =
      g_list_remove(align_tool->selected_objects, object);
