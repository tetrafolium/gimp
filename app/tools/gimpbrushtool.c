...
static GimpCanvasItem *gimp_brush_tool_get_outline(GimpPaintTool *paint_tool,
                                                   GimpDisplay *display,
                                                   gdouble x, gdouble y);
...
gimp_brush_tool_get_boundary(GimpBrushTool *brush_tool, gint *width,
                             gint *height);
...
static void gimp_brush_tool_class_init(GimpBrushToolClass *klass) {
...
static void gimp_brush_tool_init(GimpBrushTool *brush_tool) {
  GimpTool *tool = GIMP_TOOL(brush_tool);

  gimp_tool_control_set_action_size(tool->control,
                                    "tools/tools-paintbrush-size-set");
  gimp_tool_control_set_action_aspect(
      tool->control, "tools/tools-paintbrush-aspect-ratio-set");
...
static void gimp_brush_tool_constructed(GObject *object) {
  ...
  g_signal_connect_object(gimp_tool_get_options(tool), "brush-changed",
                          G_CALLBACK(gimp_brush_tool_brush_changed), paint_tool,
                          0);

  g_signal_connect_object(paint_tool->core, "set-brush",
                          G_CALLBACK(gimp_brush_tool_set_brush), paint_tool, 0);
}
...
static void gimp_brush_tool_oper_update(GimpTool *tool,
  ...
  drawables =
      gimp_image_get_selected_drawables(gimp_display_get_image(display));
  ...
  if (!gimp_color_tool_is_enabled(GIMP_COLOR_TOOL(tool)) && drawable &&
      proximity) {
    GimpContext *context = GIMP_CONTEXT(paint_options);
    GimpPaintTool *paint_tool = GIMP_PAINT_TOOL(tool);
    GimpBrushCore *brush_core = GIMP_BRUSH_CORE(paint_tool->core);

    gimp_brush_core_set_brush(brush_core, gimp_context_get_brush(context));
    ...
    if (GIMP_BRUSH_CORE_GET_CLASS(brush_core)->handles_transforming_brush) {
      gimp_brush_core_eval_transform_dynamics(brush_core, drawable,
...
static void gimp_brush_tool_cursor_update(GimpTool *tool,
  ...
  GimpBrushCore *brush_core =
      GIMP_BRUSH_CORE(GIMP_PAINT_TOOL(brush_tool)->core);
...
static void gimp_brush_tool_options_notify(GimpTool *tool,
  ...
  if (!strcmp(pspec->name, "brush-size") ||
    ...
    g_signal_emit_by_name(brush_core, "set-brush", brush_core->main_brush);
  }
}
...
static void gimp_brush_tool_paint_start(GimpPaintTool *paint_tool) {
  ...
  boundary = gimp_brush_tool_get_boundary(
      brush_tool, &brush_tool->boundary_width, &brush_tool->boundary_height);
...
static GimpCanvasItem *gimp_brush_tool_get_outline(GimpPaintTool *paint_tool,
  ...
  if (!item) {
    ...
    if (brush_core->main_brush && brush_core->dynamics) {
      ...
      gimp_paint_tool_set_draw_fallback(paint_tool, TRUE, options->brush_size);
    }
  }
...
static void gimp_brush_tool_set_brush(GimpBrushCore *brush_core,
  ...
  if (GIMP_BRUSH_CORE_GET_CLASS(brush_core)->handles_transforming_brush) {
    ...
    gimp_brush_core_eval_transform_dynamics(
        brush_core, NULL, GIMP_PAINT_TOOL_GET_OPTIONS(brush_tool),
...
gimp_brush_tool_get_boundary(GimpBrushTool *brush_tool, gint *width,
  ...
  if (paint_tool->draw_brush && brush_core->main_brush &&
      brush_core->dynamics && brush_core->scale > 0.0) {
    return gimp_brush_transform_boundary(
        brush_core->main_brush, brush_core->scale, brush_core->aspect_ratio,
        brush_core->angle, brush_core->reflect, brush_core->hardness, width,
        height);
  }
