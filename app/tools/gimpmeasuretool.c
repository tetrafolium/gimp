...
static void gimp_measure_tool_control(GimpTool *tool, GimpToolAction action,
                                      GimpDisplay *display);
static void gimp_measure_tool_modifier_key(GimpTool *tool, GdkModifierType key,
                                           gboolean press,
...
static void gimp_measure_tool_compass_status(GimpToolWidget *widget,
                                             const gchar *status,
                                             GimpMeasureTool *measure);
static void gimp_measure_tool_compass_create_guides(GimpToolWidget *widget,
                                                    gint x, gint y,
                                                    gboolean horizontal,
                                                    gboolean vertical,
                                                    GimpMeasureTool *measure);
...
void gimp_measure_tool_register(GimpToolRegisterCallback callback,
                                gpointer data) {
  (*callback)(GIMP_TYPE_MEASURE_TOOL, GIMP_TYPE_MEASURE_OPTIONS,
              gimp_measure_options_gui, 0, "gimp-measure-tool", _("Measure"),
              _("Measure Tool: Measure distances and angles"), N_("_Measure"),
              "<shift>M", NULL, GIMP_HELP_TOOL_MEASURE, GIMP_ICON_TOOL_MEASURE,
              data);
}
...
static void gimp_measure_tool_init(GimpMeasureTool *measure) {
...
static void gimp_measure_tool_modifier_key(GimpTool *tool, GdkModifierType key,
  ...
  if (key == gimp_get_toggle_behavior_mask()) {
    ...
    case GIMP_COMPASS_ORIENTATION_HORIZONTAL:
      g_object_set(options, "orientation", GIMP_COMPASS_ORIENTATION_VERTICAL,
                   NULL);
      break;

    case GIMP_COMPASS_ORIENTATION_VERTICAL:
      g_object_set(options, "orientation", GIMP_COMPASS_ORIENTATION_HORIZONTAL,
                   NULL);
      break;
...
static void gimp_measure_tool_button_release(GimpTool *tool,
  ...
  if (measure->grab_widget) {
    gimp_tool_widget_button_release(measure->grab_widget, coords, time, state,
                                    release_type);
...
static void gimp_measure_tool_recalc_matrix(GimpTransformTool *tr_tool) {
  ...
  g_object_get(measure->widget, "pixel-angle", &angle, NULL);

  gimp_matrix3_identity(&tr_tool->transform);
  gimp_transform_matrix_rotate_center(&tr_tool->transform, measure->x[0],
                                      measure->y[0], angle);
...
static gchar *gimp_measure_tool_get_undo_desc(GimpTransformTool *tr_tool) {
  ...
  g_object_get(measure->widget, "effective-orientation", &orientation,
               "pixel-angle", &angle, NULL);

  angle = gimp_rad_to_deg(fabs(angle));

  switch (orientation) {
  case GIMP_COMPASS_ORIENTATION_AUTO:
    return g_strdup_printf(C_("undo-type", "Straighten by %-3.3g°"), angle);

  case GIMP_COMPASS_ORIENTATION_HORIZONTAL:
    return g_strdup_printf(
        C_("undo-type", "Straighten Horizontally by %-3.3g°"), angle);

  case GIMP_COMPASS_ORIENTATION_VERTICAL:
    return g_strdup_printf(C_("undo-type", "Straighten Vertically by %-3.3g°"),
                           angle);
  }
...
static void gimp_measure_tool_compass_changed(GimpToolWidget *widget,
  ...
  g_object_get(widget, "n-points", &measure->n_points, "x1", &measure->x[0],
               "y1", &measure->y[0], "x2", &measure->x[1], "y2", &measure->y[1],
               "x3", &measure->x[2], "y3", &measure->y[2], NULL);
...
static void gimp_measure_tool_start(GimpMeasureTool *measure,
  ...
  measure->widget =
      gimp_tool_compass_new(shell, options->orientation, measure->n_points,
                            measure->x[0], measure->y[0], measure->x[1],
                            measure->y[1], measure->x[2], measure->y[2]);

  gimp_draw_tool_set_widget(GIMP_DRAW_TOOL(tool), measure->widget);

  g_object_bind_property(options, "orientation", measure->widget, "orientation",
                         G_BINDING_DEFAULT);

  g_signal_connect(measure->widget, "changed",
                   G_CALLBACK(gimp_measure_tool_compass_changed), measure);
  g_signal_connect(measure->widget, "response",
                   G_CALLBACK(gimp_measure_tool_compass_response), measure);
  g_signal_connect(measure->widget, "status",
                   G_CALLBACK(gimp_measure_tool_compass_status), measure);
...
static void gimp_measure_tool_halt(GimpMeasureTool *measure) {
  ...
  if (options->straighten_button) {
    ...
    g_signal_handlers_disconnect_by_func(
        ...
        G_CALLBACK(gimp_measure_tool_straighten_button_clicked), measure);
  }
...
static void gimp_measure_tool_dialog_update(GimpMeasureTool *measure,
  ...
  inch_distance =
      sqrt(SQR((gdouble)(ax - bx) / xres) + SQR((gdouble)(ay - by) / yres));
  unit_distance = gimp_unit_get_factor(shell->unit) * inch_distance;

  g_object_get(measure->widget, "pixel-angle", &pixel_angle, "unit-angle",
               &unit_angle, NULL);
   ...
   */
  if (inch_distance)
    unit_distance_digits = gimp_unit_get_scaled_digits(
        shell->unit, pixel_distance / inch_distance);
  ...
  if (shell->unit == GIMP_UNIT_PIXEL) {
    gimp_tool_replace_status(
        GIMP_TOOL(measure), display, "%.1f %s, %.2f\302\260 (%d × %d)",
        pixel_distance, _("pixels"), pixel_angle, pixel_width, pixel_height);
  } else {
    g_snprintf(format, sizeof(format),
               "%%.%df %s, %%.2f\302\260 (%%.%df × %%.%df)",
               unit_distance_digits, gimp_unit_get_plural(shell->unit),
               unit_width_digits, unit_height_digits);

    gimp_tool_replace_status(GIMP_TOOL(measure), display, format, unit_distance,
                             unit_angle, unit_width, unit_height);
  }
  ...
  if (measure->gui) {
    ...
    if (shell->unit != GIMP_UNIT_PIXEL) {
      g_snprintf(format, sizeof(format), "%%.%df", unit_distance_digits);
    ...
    if (shell->unit != GIMP_UNIT_PIXEL) {
      g_snprintf(format, sizeof(format), "%%.%df", unit_width_digits);
    ...
    /* Height */
    if (shell->unit != GIMP_UNIT_PIXEL) {
      g_snprintf(format, sizeof(format), "%%.%df", unit_height_digits);
...
static GimpToolGui *gimp_measure_tool_dialog_new(GimpMeasureTool *measure) {
  ...
  gui = gimp_tool_gui_new(tool->tool_info, NULL,
                          _("Measure Distances and Angles"), NULL, NULL,
  ...
  g_signal_connect(gui, "response", G_CALLBACK(g_object_unref), NULL);

  grid = gtk_grid_new();
  gtk_grid_set_column_spacing(GTK_GRID(grid), 6);
  gtk_grid_set_row_spacing(GTK_GRID(grid), 6);
  gtk_box_pack_start(GTK_BOX(gimp_tool_gui_get_vbox(gui)), grid, FALSE, FALSE,
                     0);
