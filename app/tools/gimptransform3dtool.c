...
G_DEFINE_TYPE(GimpTransform3DTool, gimp_transform_3d_tool,
...
void gimp_transform_3d_tool_register(GimpToolRegisterCallback callback,
                                     gpointer data) {
  (*callback)(GIMP_TYPE_TRANSFORM_3D_TOOL, GIMP_TYPE_TRANSFORM_3D_OPTIONS,
              gimp_transform_3d_options_gui, GIMP_CONTEXT_PROP_MASK_BACKGROUND,
              "gimp-transform-3d-tool", _("3D Transform"),
              _("3D Transform Tool: Apply a 3D transformation to the layer, "
                "selection or path"),
              N_("_3D Transform"), "<shift>W", NULL,
              GIMP_ICON_TOOL_TRANSFORM_3D, GIMP_ICON_TOOL_TRANSFORM_3D, data);
}

static void gimp_transform_3d_tool_class_init(GimpTransform3DToolClass *klass) {
...
static void gimp_transform_3d_tool_init(GimpTransform3DTool *t3d) {}
...
static void gimp_transform_3d_tool_modifier_key(GimpTool *tool,
  ...
  if (key == gimp_get_extend_selection_mask()) {
    g_object_set(options, "constrain-axis", !options->constrain_axis, NULL);
  } else if (key == gimp_get_constrain_behavior_mask()) {
    g_object_set(options, "z-axis", !options->z_axis, NULL);
  } else if (key == GDK_MOD1_MASK) {
    g_object_set(options, "local-frame", !options->local_frame, NULL);
  }
}
...
gimp_transform_3d_tool_info_to_matrix(GimpTransformGridTool *tg_tool,
  ...
  gimp_transform_3d_matrix(
      ...
      tg_tool->trans_info[OFFSET_X], tg_tool->trans_info[OFFSET_Y],
      tg_tool->trans_info[OFFSET_Z],

      tg_tool->trans_info[ROTATION_ORDER], tg_tool->trans_info[ANGLE_X],
      tg_tool->trans_info[ANGLE_Y], tg_tool->trans_info[ANGLE_Z],

      tg_tool->trans_info[PIVOT_X], tg_tool->trans_info[PIVOT_Y],
...
static void gimp_transform_3d_tool_dialog(GimpTransformGridTool *tg_tool) {
  ...
  gtk_box_pack_start(GTK_BOX(gimp_tool_gui_get_vbox(tg_tool->gui)), notebook,
                     FALSE, FALSE, 0);
  ...
  g_signal_connect(se, "value-changed",
                   G_CALLBACK(gimp_transform_3d_tool_dialog_changed), t3d);
  ...
  g_signal_connect(combo, "changed",
                   G_CALLBACK(gimp_transform_3d_tool_lens_mode_changed), t3d);
  ...
  gimp_size_entry_set_value_boundaries(GIMP_SIZE_ENTRY(se), 0, 0.0,
                                       G_MAXDOUBLE);
  ...
  t3d->angle_of_view_adj =
      (GtkAdjustment *)gtk_adjustment_new(0, 0, 180, 1, 10, 0);
  scale = gimp_spin_scale_new(t3d->angle_of_view_adj, _("Angle"), 2);
  ...
  gimp_size_entry_set_refval_boundaries(
      GIMP_SIZE_ENTRY(se), 2, -GIMP_MAX_IMAGE_SIZE, GIMP_MAX_IMAGE_SIZE);
  ...
  for (i = 0; i < 3; i++) {
    ...
    /* angle spin scale */
    t3d->angle_adj[i] =
        (GtkAdjustment *)gtk_adjustment_new(0, -180, 180, 1, 10, 0);
    scale = gimp_spin_scale_new(t3d->angle_adj[i], labels[i], 2);
  ...
  g_signal_connect(selector, "changed",
                   G_CALLBACK(gimp_transform_3d_tool_pivot_changed), t3d);

  g_object_bind_property(options, "mode", t3d->notebook, "page",
...
gimp_transform_3d_tool_dialog_update(GimpTransformGridTool *tg_tool) {
  ...
  /* vanishing-point size entry */
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(t3d->vanishing_point_se), 0,
                             tg_tool->trans_info[VANISHING_POINT_X]);
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(t3d->vanishing_point_se), 1,
                             tg_tool->trans_info[VANISHING_POINT_Y]);
  ...
  gtk_widget_set_visible(t3d->focal_length_se,
                         lens_mode == GIMP_TRANSFORM_3D_LENS_MODE_FOCAL_LENGTH);
  gtk_widget_set_visible(t3d->angle_of_view_scale,
                         lens_mode != GIMP_TRANSFORM_3D_LENS_MODE_FOCAL_LENGTH);

  switch (lens_mode) {
  case GIMP_TRANSFORM_3D_LENS_MODE_FOCAL_LENGTH:
    gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(t3d->focal_length_se), 0,
                               tg_tool->trans_info[LENS_VALUE]);
    break;

  case GIMP_TRANSFORM_3D_LENS_MODE_FOV_IMAGE:
  case GIMP_TRANSFORM_3D_LENS_MODE_FOV_ITEM:
    gtk_adjustment_set_value(t3d->angle_of_view_adj,
                             gimp_rad_to_deg(tg_tool->trans_info[LENS_VALUE]));
    break;
  }

  /* offset size entry */
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(t3d->offset_se), 0,
                             tg_tool->trans_info[OFFSET_X]);
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(t3d->offset_se), 1,
                             tg_tool->trans_info[OFFSET_Y]);
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(t3d->offset_se), 2,
                             tg_tool->trans_info[OFFSET_Z]);
...
static void gimp_transform_3d_tool_prepare(GimpTransformGridTool *tg_tool) {
  ...
  gimp_size_entry_set_refval_boundaries(se, 0, -GIMP_MAX_IMAGE_SIZE,
                                        GIMP_MAX_IMAGE_SIZE);
  gimp_size_entry_set_refval_boundaries(se, 1, -GIMP_MAX_IMAGE_SIZE,
                                        GIMP_MAX_IMAGE_SIZE);
  ...
  gimp_pivot_selector_set_bounds(GIMP_PIVOT_SELECTOR(t3d->pivot_selector),
                                 tr_tool->x1, tr_tool->y1, tr_tool->x2,
                                 tr_tool->y2);
...
gimp_transform_3d_tool_get_widget(GimpTransformGridTool *tg_tool) {
  ...
  static const gchar *bound_properties[] = {
      "mode", "unified", "constrain-axis", "z-axis", "local-frame",
  };

  widget = gimp_tool_transform_3d_grid_new(
      shell, tr_tool->x1, tr_tool->y1, tr_tool->x2, tr_tool->y2,
      tg_tool->trans_info[VANISHING_POINT_X],
      tg_tool->trans_info[VANISHING_POINT_Y],
      -gimp_transform_3d_tool_get_focal_length(t3d));

  for (i = 0; i < G_N_ELEMENTS(bound_properties); i++) {
    g_object_bind_property(options, bound_properties[i], widget,
                           bound_properties[i],
                           G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);
  }
...
gimp_transform_3d_tool_update_widget(GimpTransformGridTool *tg_tool) {
  ...
  g_object_set(
      ...
      "x1", (gdouble)tr_tool->x1, "y1", (gdouble)tr_tool->y1, "x2",
      (gdouble)tr_tool->x2, "y2", (gdouble)tr_tool->y2,

      "camera-x", tg_tool->trans_info[VANISHING_POINT_X], "camera-y",
      tg_tool->trans_info[VANISHING_POINT_Y], "camera-z",
      -gimp_transform_3d_tool_get_focal_length(t3d),

      "offset-x", tg_tool->trans_info[OFFSET_X], "offset-y",
      tg_tool->trans_info[OFFSET_Y], "offset-z", tg_tool->trans_info[OFFSET_Z],

      "rotation-order", (gint)tg_tool->trans_info[ROTATION_ORDER], "angle-x",
      tg_tool->trans_info[ANGLE_X], "angle-y", tg_tool->trans_info[ANGLE_Y],
      "angle-z", tg_tool->trans_info[ANGLE_Z],

      "pivot-3d-x", tg_tool->trans_info[PIVOT_X], "pivot-3d-y",
      tg_tool->trans_info[PIVOT_Y], "pivot-3d-z", tg_tool->trans_info[PIVOT_Z],
...
gimp_transform_3d_tool_widget_changed(GimpTransformGridTool *tg_tool) {
  g_object_get(tg_tool->widget,
               ...
               "offset-x", &tg_tool->trans_info[OFFSET_X], "offset-y",
               &tg_tool->trans_info[OFFSET_Y], "offset-z",
               &tg_tool->trans_info[OFFSET_Z],

               "angle-x", &tg_tool->trans_info[ANGLE_X], "angle-y",
               &tg_tool->trans_info[ANGLE_Y], "angle-z",
...
static void gimp_transform_3d_tool_dialog_changed(GObject *object,
  ...
  /* vanishing-point size entry */
  tg_tool->trans_info[VANISHING_POINT_X] =
      gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(t3d->vanishing_point_se), 0);
  tg_tool->trans_info[VANISHING_POINT_Y] =
      gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(t3d->vanishing_point_se), 1);

  /* focal-length size entry / angle-of-view spin scale */
  switch ((gint)tg_tool->trans_info[LENS_MODE]) {
  case GIMP_TRANSFORM_3D_LENS_MODE_FOCAL_LENGTH:
    tg_tool->trans_info[LENS_VALUE] =
        gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(t3d->focal_length_se), 0);
    break;

  case GIMP_TRANSFORM_3D_LENS_MODE_FOV_IMAGE:
  case GIMP_TRANSFORM_3D_LENS_MODE_FOV_ITEM:
    tg_tool->trans_info[LENS_VALUE] =
        gimp_deg_to_rad(gtk_adjustment_get_value(t3d->angle_of_view_adj));
    break;
  }

  /* offset size entry */
  tg_tool->trans_info[OFFSET_X] =
      gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(t3d->offset_se), 0);
  tg_tool->trans_info[OFFSET_Y] =
      gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(t3d->offset_se), 1);
  tg_tool->trans_info[OFFSET_Z] =
      gimp_size_entry_get_refval(GIMP_SIZE_ENTRY(t3d->offset_se), 2);

  /* angle spin scales */
  tg_tool->trans_info[ANGLE_X] =
      gimp_deg_to_rad(gtk_adjustment_get_value(t3d->angle_adj[0]));
  tg_tool->trans_info[ANGLE_Y] =
      gimp_deg_to_rad(gtk_adjustment_get_value(t3d->angle_adj[1]));
  tg_tool->trans_info[ANGLE_Z] =
      gimp_deg_to_rad(gtk_adjustment_get_value(t3d->angle_adj[2]));
...
static void gimp_transform_3d_tool_lens_mode_changed(GtkComboBox *combo,
  ...
  case GIMP_TRANSFORM_3D_LENS_MODE_FOV_ITEM:
    tg_tool->trans_info[LENS_VALUE] =
        gimp_transform_3d_focal_length_to_angle_of_view(focal_length, x2 - x1,
                                                        y2 - y1);
    break;
  }

  /* vanishing-point size entry */
  gimp_size_entry_set_size(GIMP_SIZE_ENTRY(t3d->vanishing_point_se), 0, x1, x2);
  gimp_size_entry_set_size(GIMP_SIZE_ENTRY(t3d->vanishing_point_se), 1, y1, y2);
...
gimp_transform_3d_tool_rotation_order_clicked(GtkButton *button,
  ...
  gimp_transform_3d_matrix4_rotate_euler(
      &matrix, tg_tool->trans_info[ROTATION_ORDER],
      tg_tool->trans_info[ANGLE_X], tg_tool->trans_info[ANGLE_Y],
      tg_tool->trans_info[ANGLE_Z], 0.0, 0.0, 0.0);
...
static void gimp_transform_3d_tool_pivot_changed(GimpPivotSelector *selector,
  ...
  gimp_transform_3d_matrix4_rotate_euler(
      &matrix, tg_tool->trans_info[ROTATION_ORDER],
      tg_tool->trans_info[ANGLE_X], tg_tool->trans_info[ANGLE_Y],
      tg_tool->trans_info[ANGLE_Z], tg_tool->trans_info[PIVOT_X],
      tg_tool->trans_info[PIVOT_Y], tg_tool->trans_info[PIVOT_Z]);
  ...
  gimp_matrix4_transform_point(
      &matrix, tg_tool->trans_info[PIVOT_X], tg_tool->trans_info[PIVOT_Y],
      tg_tool->trans_info[PIVOT_Z], &offset_x, &offset_y, &offset_z);
