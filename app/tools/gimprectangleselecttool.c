...
static void gimp_rectangle_select_tool_button_press(
    ...
    GimpDisplay *display);
static void gimp_rectangle_select_tool_button_release(
    GimpTool *tool, const GimpCoords *coords, guint32 time,
...
static gboolean
gimp_rectangle_select_tool_select(GimpRectangleSelectTool *rect_tool, gint x,
                                  gint y, gint w, gint h);
static void
gimp_rectangle_select_tool_real_select(GimpRectangleSelectTool *rect_tool,
                                       GimpChannelOps operation, gint x, gint y,
                                       gint w, gint h);
...
static void gimp_rectangle_select_tool_rectangle_change_complete(
    GimpToolWidget *widget, GimpRectangleSelectTool *rect_tool);
...
static void gimp_rectangle_select_tool_update_option_defaults(
    GimpRectangleSelectTool *rect_tool, gboolean ignore_pending);
static void
...
void gimp_rectangle_select_tool_register(GimpToolRegisterCallback callback,
  ...
  (*callback)(GIMP_TYPE_RECTANGLE_SELECT_TOOL,
              ...
              gimp_rectangle_select_options_gui, 0, "gimp-rect-select-tool",
              _("Rectangle Select"),
              _("Rectangle Select Tool: Select a rectangular region"),
              N_("_Rectangle Select"), "R", NULL, GIMP_HELP_TOOL_RECT_SELECT,
              GIMP_ICON_TOOL_RECT_SELECT, data);
}
...
static void gimp_rectangle_select_tool_button_press(
  ...
  if (gimp_selection_tool_start_edit(GIMP_SELECTION_TOOL(tool), display,
                                     coords)) {
...
private
  ...
  gimp_selection_tool_start_change(
      GIMP_SELECTION_TOOL(tool), function == GIMP_TOOL_RECTANGLE_CREATING,
...
static void gimp_rectangle_select_tool_button_release(
  ...
  if (priv->grab_widget) {
    gimp_tool_widget_button_release(priv->grab_widget, coords, time, state,
                                    release_type);
...
static void gimp_rectangle_select_tool_cursor_update(GimpTool *tool,
  ...
  if (private->widget && display == tool->display) {
    gimp_tool_widget_get_cursor(private->widget, coords, state, &cursor, NULL,
                                &modifier);
  }
...
static void gimp_rectangle_select_tool_options_notify(GimpTool *tool,
                                                      GimpToolOptions *options,
                                                      const GParamSpec *pspec) {
  if (!strcmp(pspec->name, "antialias") || !strcmp(pspec->name, "feather") ||
...
gimp_rectangle_select_tool_select(GimpRectangleSelectTool *rect_tool, gint x,
  ...
  rectangle_exists =
      (x <= gimp_image_get_width(image) && y <= gimp_image_get_height(image) &&
       x + w >= 0 && y + h >= 0 && w > 0 && h > 0);
  ...
  if (rectangle_exists) {
    gimp_selection_tool_start_change(GIMP_SELECTION_TOOL(rect_tool), FALSE,
                                     operation);

    GIMP_RECTANGLE_SELECT_TOOL_GET_CLASS(rect_tool)->select(
        rect_tool, operation, x, y, w, h);

    gimp_selection_tool_end_change(GIMP_SELECTION_TOOL(rect_tool), FALSE);
  }
...
gimp_rectangle_select_tool_real_select(GimpRectangleSelectTool *rect_tool,
  ...
  if (rect_options->round_corners) {
    ...
    gimp_channel_select_round_rect(channel, x, y, w, h, radius, radius,
                                   operation, options->antialias,
                                   options->feather, options->feather_radius,
                                   options->feather_radius, TRUE);
  } else {
    gimp_channel_select_rectangle(channel, x, y, w, h, operation,
                                  options->feather, options->feather_radius,
                                  options->feather_radius, TRUE);
  }
}
...
static void gimp_rectangle_select_tool_rectangle_response(
  ...
  case GIMP_TOOL_WIDGET_RESPONSE_CONFIRM: {
    ...
    gimp_tool_rectangle_get_public_rect(GIMP_TOOL_RECTANGLE(widget), &x1, &y1,
                                        &x2, &y2);
    if (x1 == x2 && y1 == y2) {
...
static void gimp_rectangle_select_tool_start(GimpRectangleSelectTool *rect_tool,
                                             GimpDisplay *display) {
  static const gchar *properties[] = {"highlight",
                                      "highlight-opacity",
                                      "guide",
                                      "round-corners",
                                      "corner-radius",
...
private
  ->widget = widget = gimp_tool_rectangle_new(shell);

  draw_ellipse = GIMP_RECTANGLE_SELECT_TOOL_GET_CLASS(rect_tool)->draw_ellipse;

  g_object_set(widget, "draw-ellipse", draw_ellipse, "status-title",
               draw_ellipse ? _("Ellipse: ") : _("Rectangle: "), NULL);

  gimp_draw_tool_set_widget(GIMP_DRAW_TOOL(tool), widget);

  for (i = 0; i < G_N_ELEMENTS(properties); i++) {
    GBinding *binding = g_object_bind_property(
        G_OBJECT(options), properties[i], G_OBJECT(widget), properties[i],
        G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);

  private
    ->bindings = g_list_prepend(private->bindings, binding);
  }

  gimp_rectangle_options_connect(
      GIMP_RECTANGLE_OPTIONS(options), gimp_display_get_image(shell->display),
      G_CALLBACK(gimp_rectangle_select_tool_auto_shrink), rect_tool);

  g_signal_connect(widget, "response",
                   G_CALLBACK(gimp_rectangle_select_tool_rectangle_response),
                   rect_tool);
  g_signal_connect(
      widget, "change-complete",
...
gimp_rectangle_select_tool_commit(GimpRectangleSelectTool *rect_tool) {
  ...
  if (priv->widget) {
    ...
    gimp_tool_rectangle_get_public_rect(GIMP_TOOL_RECTANGLE(priv->widget), &x1,
                                        &y1, &x2, &y2);
    w = x2 - x1;
    h = y2 - y1;
    ...
    if (w == 0 && h == 0) {
      ...
      if (gimp_pickable_get_opacity_at(GIMP_PICKABLE(selection), press_x,
                                       press_y) > 0.5) {
        gint x, y, w, h;

        if (gimp_item_bounds(GIMP_ITEM(selection), &x, &y, &w, &h)) {
          g_object_set(priv->widget, "x1", (gdouble)x, "y1", (gdouble)y, "x2",
                       (gdouble)(x + w), "y2", (gdouble)(y + h), NULL);
        }
...
gimp_rectangle_select_tool_halt(GimpRectangleSelectTool *rect_tool) {
  ...
  if (tool->display) {
    ...
    gimp_rectangle_options_disconnect(
        GIMP_RECTANGLE_OPTIONS(options),
        G_CALLBACK(gimp_rectangle_select_tool_auto_shrink), rect_tool);
  }
...
static void gimp_rectangle_select_tool_update_option_defaults(
  ...
  if (priv->widget && !ignore_pending) {
    ...
    gimp_tool_rectangle_pending_size_set(
        GIMP_TOOL_RECTANGLE(priv->widget), G_OBJECT(rect_options),
        "default-aspect-numerator", "default-aspect-denominator");

    g_object_set(G_OBJECT(rect_options), "use-string-current", TRUE, NULL);
  } else {
    g_object_set(G_OBJECT(rect_options), "default-aspect-numerator", 1.0,
                 "default-aspect-denominator", 1.0, NULL);

    g_object_set(G_OBJECT(rect_options), "use-string-current", FALSE, NULL);
  }
}
...
gimp_rectangle_select_tool_update(GimpRectangleSelectTool *rect_tool) {
  ...
  if (tool->display && !gimp_tool_control_is_active(tool->control)) {
    ...
    gimp_rectangle_select_tool_select(rect_tool, x1, y1, x2 - x1, y2 - y1);
...
gimp_rectangle_select_tool_auto_shrink(GimpRectangleSelectTool *rect_tool) {
  ...
  g_object_get(gimp_tool_get_options(GIMP_TOOL(rect_tool)), "shrink-merged",
               &shrink_merged, NULL);
