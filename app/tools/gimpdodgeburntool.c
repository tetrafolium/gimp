...
void gimp_dodge_burn_tool_register(GimpToolRegisterCallback callback,
                                   gpointer data) {
  (*callback)(
      GIMP_TYPE_DODGE_BURN_TOOL, GIMP_TYPE_DODGE_BURN_OPTIONS,
      gimp_dodge_burn_options_gui, GIMP_PAINT_OPTIONS_CONTEXT_MASK,
      "gimp-dodge-burn-tool", _("Dodge / Burn"),
      _("Dodge / Burn Tool: Selectively lighten or darken using a brush"),
      N_("Dod_ge / Burn"), "<shift>D", NULL, GIMP_HELP_TOOL_DODGE_BURN,
      GIMP_ICON_TOOL_DODGE, data);
}
...
static void gimp_dodge_burn_tool_init(GimpDodgeBurnTool *dodgeburn) {
  ...
  gimp_tool_control_set_tool_cursor(tool->control, GIMP_TOOL_CURSOR_DODGE);
...
static void gimp_dodge_burn_tool_modifier_key(GimpTool *tool,
  ...
  if ((key == toggle_mask &&
      ...
      (key == line_mask && /* toggle back after keypresses CTRL(hold)->  */
       ...
       dodgeburn->toggled && !(state & toggle_mask))) {
...
static GtkWidget *gimp_dodge_burn_options_gui(GimpToolOptions *tool_options) {
  ...
  str = g_strdup_printf(_("Type  (%s)"), gimp_get_mod_string(toggle_mask));
  ...
  /*  the exposure scale  */
  scale = gimp_prop_spin_scale_new(config, "exposure", NULL, 1.0, 10.0, 1);
