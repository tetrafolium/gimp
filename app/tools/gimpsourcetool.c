...
static GimpDisplay *gimp_source_tool_has_image(GimpTool *tool,
                                               GimpImage *image);
static void gimp_source_tool_control(GimpTool *tool, GimpToolAction action,
                                     GimpDisplay *display);
static void gimp_source_tool_button_press(GimpTool *tool,
                                          ...
                                          GimpDisplay *display);
static void gimp_source_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                    guint32 time, GdkModifierType state,
                                    GimpDisplay *display);
static void gimp_source_tool_cursor_update(GimpTool *tool,
                                           ...
                                           GimpDisplay *display);
static void gimp_source_tool_modifier_key(GimpTool *tool, GdkModifierType key,
...
static void gimp_source_tool_class_init(GimpSourceToolClass *klass) {
...
static void gimp_source_tool_init(GimpSourceTool *source) {
...
static void gimp_source_tool_draw(GimpDrawTool *draw_tool) {
  ...
  if (gimp_source_core_use_source(source, options) && source->src_drawable &&
    ...
    if (source_tool->src_outline) {
      gimp_display_shell_remove_tool_item(src_shell, source_tool->src_outline);
      source_tool->src_outline = NULL;
    }

    if (source_tool->show_source_outline) {
      source_tool->src_outline = gimp_brush_tool_create_outline(
          GIMP_BRUSH_TOOL(source_tool), source_tool->src_display, src_x, src_y);

      if (source_tool->src_outline) {
        gimp_display_shell_add_tool_item(src_shell, source_tool->src_outline);
    ...
    if (source_tool->src_outline) {
      if (source_tool->src_handle) {
        gimp_display_shell_remove_tool_item(src_shell, source_tool->src_handle);
        source_tool->src_handle = NULL;
      }
    } else {
      if (!source_tool->src_handle) {
        source_tool->src_handle = gimp_canvas_handle_new(
            src_shell, GIMP_HANDLE_CROSS, GIMP_HANDLE_ANCHOR_CENTER, src_x,
            src_y, GIMP_TOOL_HANDLE_SIZE_CROSS, GIMP_TOOL_HANDLE_SIZE_CROSS);
        gimp_display_shell_add_tool_item(src_shell, source_tool->src_handle);
        g_object_unref(source_tool->src_handle);
      } else {
        gimp_canvas_handle_set_position(source_tool->src_handle, src_x, src_y);
      }
    }
  }
}
