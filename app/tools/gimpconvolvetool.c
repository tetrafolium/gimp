...
static void gimp_convolve_tool_modifier_key(GimpTool *tool, GdkModifierType key,
                                            gboolean press,
...
void gimp_convolve_tool_register(GimpToolRegisterCallback callback,
                                 gpointer data) {
  (*callback)(
      GIMP_TYPE_CONVOLVE_TOOL, GIMP_TYPE_CONVOLVE_OPTIONS,
      gimp_convolve_options_gui, GIMP_PAINT_OPTIONS_CONTEXT_MASK,
      "gimp-convolve-tool", _("Blur / Sharpen"),
      _("Blur / Sharpen Tool: Selective blurring or unblurring using a brush"),
      N_("Bl_ur / Sharpen"), "<shift>U", NULL, GIMP_HELP_TOOL_CONVOLVE,
      GIMP_ICON_TOOL_BLUR, data);
}
...
static void gimp_convolve_tool_init(GimpConvolveTool *convolve) {
  GimpTool *tool = GIMP_TOOL(convolve);

  gimp_tool_control_set_tool_cursor(tool->control, GIMP_TOOL_CURSOR_BLUR);
...
static void gimp_convolve_tool_modifier_key(GimpTool *tool, GdkModifierType key,
  ...
  if (((key == toggle_mask) &&
      ...
      (key == line_mask && /* toggle back after keypresses CTRL(hold)->  */
       ...
       convolve->toggled && !(state & toggle_mask))) {
...
static GtkWidget *gimp_convolve_options_gui(GimpToolOptions *tool_options) {
  ...
  frame = gimp_prop_enum_radio_frame_new(config, "type", str, 0, 0);
