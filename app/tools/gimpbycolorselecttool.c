...
void gimp_by_color_select_tool_register(GimpToolRegisterCallback callback,
                                        gpointer data) {
  (*callback)(GIMP_TYPE_BY_COLOR_SELECT_TOOL, GIMP_TYPE_REGION_SELECT_OPTIONS,
              gimp_region_select_options_gui, 0, "gimp-by-color-select-tool",
              _("Select by Color"),
              _("Select by Color Tool: Select regions with similar colors"),
              N_("_By Color Select"), "<shift>O", NULL,
              GIMP_HELP_TOOL_BY_COLOR_SELECT, GIMP_ICON_TOOL_BY_COLOR_SELECT,
              data);
}
...
gimp_by_color_select_tool_get_mask(GimpRegionSelectTool *region_select,
  ...
  if (!options->sample_merged) {
    if (g_list_length(drawables) == 1) {
      ...
      select_image =
          gimp_image_new_from_drawables(image->gimp, drawables, FALSE);
  ...
  if (gimp_pickable_get_color_at(pickable, x, y, &srgb)) {
    ...
    mask = gimp_pickable_contiguous_region_by_color(
        pickable, sel_options->antialias, options->threshold / 255.0,
        options->select_transparent, options->select_criterion, &color);
  }
