...
static void gimp_tile_handler_iscissors_validate(
...
gimp_tile_handler_iscissors_class_init(GimpTileHandlerIscissorsClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_PICKABLE,
      g_param_spec_object("pickable", NULL, NULL, GIMP_TYPE_PICKABLE,
...
static void gimp_tile_handler_iscissors_validate(
  ...
  temp0 = gegl_buffer_new(GEGL_RECTANGLE(0, 0, rect->width, rect->height),
                          babl_format("R'G'B'A u8"));

  temp1 = gegl_buffer_new(GEGL_RECTANGLE(0, 0, rect->width, rect->height),
                          babl_format("R'G'B'A u8"));

  temp2 = gegl_buffer_new(GEGL_RECTANGLE(0, 0, rect->width, rect->height),
                          babl_format("R'G'B'A u8"));

  /* XXX tile edges? */

  /*  Blur the source to get rid of noise  */
  gimp_gegl_convolve(src, rect, temp0,
                     GEGL_RECTANGLE(0, 0, rect->width, rect->height), blur_32,
                     3, 32, GIMP_NORMAL_CONVOL, FALSE);
  ...
  maxgrad_conv1 = (guchar *)gegl_buffer_linear_open(
      temp1, GEGL_RECTANGLE(0, 0, rect->width, rect->height), &stride1, NULL);

  maxgrad_conv2 = (guchar *)gegl_buffer_linear_open(
      temp2, GEGL_RECTANGLE(0, 0, rect->width, rect->height), &stride2, NULL);
...
GeglTileHandler *gimp_tile_handler_iscissors_new(GimpPickable *pickable) {
  ...
  return g_object_new(GIMP_TYPE_TILE_HANDLER_ISCISSORS, "whole-tile", TRUE,
