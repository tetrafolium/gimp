...
static void gimp_warp_options_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_warp_options_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);

G_DEFINE_TYPE(GimpWarpOptions, gimp_warp_options, GIMP_TYPE_TOOL_OPTIONS)

#define parent_class gimp_warp_options_parent_class

static void gimp_warp_options_class_init(GimpWarpOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(object_class, PROP_BEHAVIOR, "behavior", _("Behavior"),
                        _("Behavior"), GIMP_TYPE_WARP_BEHAVIOR,
                        GIMP_WARP_BEHAVIOR_MOVE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_EFFECT_SIZE, "effect-size",
                          _("Size"), _("Effect Size"), 1.0, 10000.0, 40.0,
                          GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_EFFECT_HARDNESS, "effect-hardness",
                          _("Hardness"), _("Effect Hardness"), 0.0, 100.0, 50.0,
                          GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_EFFECT_STRENGTH, "effect-strength",
                          _("Strength"), _("Effect Strength"), 1.0, 100.0, 50.0,
                          GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_STROKE_SPACING, "stroke-spacing",
                          _("Spacing"), _("Stroke Spacing"), 1.0, 100.0, 10.0,
                          GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_INTERPOLATION, "interpolation",
                        _("Interpolation"), _("Interpolation method"),
                        GIMP_TYPE_INTERPOLATION_TYPE, GIMP_INTERPOLATION_CUBIC,
                        GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_ABYSS_POLICY, "abyss-policy",
                        _("Abyss policy"), _("Out-of-bounds sampling behavior"),
                        GEGL_TYPE_ABYSS_POLICY, GEGL_ABYSS_NONE,
                        GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_HIGH_QUALITY_PREVIEW,
                           "high-quality-preview", _("High quality preview"),
                           _("Use an accurate but slower preview"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_REAL_TIME_PREVIEW,
                           "real-time-preview", _("Real-time preview"),
                           _("Render preview in real time (slower)"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_STROKE_DURING_MOTION,
                           "stroke-during-motion", _("During motion"),
                           _("Apply effect during motion"), TRUE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_STROKE_PERIODICALLY,
                           "stroke-periodically", _("Periodically"),
                           _("Apply effect periodically"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_STROKE_PERIODICALLY_RATE,
                          "stroke-periodically-rate", _("Rate"),
                          _("Periodic stroke rate"), 0.0, 100.0, 50.0,
                          GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_INT(
      object_class, PROP_N_ANIMATION_FRAMES, "n-animation-frames", _("Frames"),
      _("Number of animation frames"), 3, 1000, 10, GIMP_PARAM_STATIC_STRINGS);
}

static void gimp_warp_options_init(GimpWarpOptions *options) {}
...
GtkWidget *gimp_warp_options_gui(GimpToolOptions *tool_options) {
  ...
  scale = gimp_prop_spin_scale_new(config, "effect-size", NULL, 0.01, 1.0, 2);
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(scale), 1.0, 1000.0);
  gtk_box_pack_start(GTK_BOX(vbox), scale, FALSE, FALSE, 0);

  scale = gimp_prop_spin_scale_new(config, "effect-hardness", NULL, 1, 10, 1);
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(scale), 0.0, 100.0);
  gtk_box_pack_start(GTK_BOX(vbox), scale, FALSE, FALSE, 0);

  scale = gimp_prop_spin_scale_new(config, "effect-strength", NULL, 1, 10, 1);
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(scale), 1.0, 100.0);
  gtk_box_pack_start(GTK_BOX(vbox), scale, FALSE, FALSE, 0);

  scale = gimp_prop_spin_scale_new(config, "stroke-spacing", NULL, 1, 10, 1);
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(scale), 1.0, 100.0);
  gtk_box_pack_start(GTK_BOX(vbox), scale, FALSE, FALSE, 0);
  ...
  combo = gimp_prop_enum_combo_box_new(config, "abyss-policy", GEGL_ABYSS_NONE,
                                       GEGL_ABYSS_LOOP);
  ...
  scale = gimp_prop_spin_scale_new(config, "stroke-periodically-rate", NULL, 1,
                                   10, 1);
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(scale), 0.0, 100.0);
  ...
  scale = gimp_prop_spin_scale_new(config, "n-animation-frames", NULL, 1.0,
                                   10.0, 0);
  ...
  gtk_box_pack_start(GTK_BOX(vbox2), options->animate_button, FALSE, FALSE, 0);
