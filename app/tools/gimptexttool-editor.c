...
static void gimp_text_tool_select_all(GimpTextTool *text_tool, gboolean select);
static void gimp_text_tool_change_size(GimpTextTool *text_tool, gdouble amount);
...
static void gimp_text_tool_enter_text(GimpTextTool *text_tool,
                                      const gchar *str);
static void gimp_text_tool_xy_to_iter(GimpTextTool *text_tool, gdouble x,
                                      gdouble y, GtkTextIter *iter);
...
static void gimp_text_tool_im_preedit_changed(GtkIMContext *context,
                                              GimpTextTool *text_tool);
static void gimp_text_tool_im_commit(GtkIMContext *context, const gchar *str,
                                     GimpTextTool *text_tool);
static gboolean gimp_text_tool_im_retrieve_surrounding(GtkIMContext *context,
                                                       GimpTextTool *text_tool);
static gboolean gimp_text_tool_im_delete_surrounding(GtkIMContext *context,
...
static void gimp_text_tool_fix_position(GimpTextTool *text_tool, gdouble *x,
                                        gdouble *y);
...
void gimp_text_tool_editor_init(GimpTextTool *text_tool) {
  ...
  g_signal_connect(text_tool->im_context, "preedit-start",
                   G_CALLBACK(gimp_text_tool_im_preedit_start), text_tool);
  g_signal_connect(text_tool->im_context, "preedit-end",
                   G_CALLBACK(gimp_text_tool_im_preedit_end), text_tool);
  g_signal_connect(text_tool->im_context, "preedit-changed",
                   G_CALLBACK(gimp_text_tool_im_preedit_changed), text_tool);
  g_signal_connect(text_tool->im_context, "commit",
                   G_CALLBACK(gimp_text_tool_im_commit), text_tool);
  ...
  g_signal_connect(text_tool->im_context, "delete-surrounding",
                   G_CALLBACK(gimp_text_tool_im_delete_surrounding), text_tool);
}
...
void gimp_text_tool_editor_start(GimpTextTool *text_tool) {
  ...
  g_signal_connect(options, "notify::use-editor",
                   G_CALLBACK(gimp_text_tool_options_notify), text_tool);
  ...
  if (!text_tool->style_overlay) {
    ...
    gtk_container_set_border_width(GTK_CONTAINER(text_tool->style_overlay), 4);
    gimp_display_shell_add_overlay(shell, text_tool->style_overlay, 0, 0,
    ...
    text_tool->style_editor = gimp_text_style_editor_new(
        gimp, text_tool->proxy, text_tool->buffer, fonts, xres, yres);
...
void gimp_text_tool_editor_position(GimpTextTool *text_tool) {
  if (text_tool->style_overlay) {
    ...
    gtk_widget_get_preferred_size(text_tool->style_overlay, &requisition, NULL);

    g_object_get(text_tool->widget, "x1", &x, "y1", &y, NULL);

    gimp_display_shell_move_overlay(shell, text_tool->style_overlay, x, y,
    ...
    if (text_tool->image) {
      ...
      g_object_set(text_tool->style_editor, "resolution-x", xres,
                   "resolution-y", yres, NULL);
    }
  }
}
...
void gimp_text_tool_editor_halt(GimpTextTool *text_tool) {
  ...
  g_signal_handlers_disconnect_by_func(options, gimp_text_tool_options_notify,
                                       text_tool);

  if (text_tool->editor_dialog) {
    g_signal_handlers_disconnect_by_func(
        text_tool->editor_dialog, gimp_text_tool_editor_destroy, text_tool);
...
void gimp_text_tool_editor_button_press(GimpTextTool *text_tool, gdouble x,
                                        gdouble y,
...
void gimp_text_tool_editor_motion(GimpTextTool *text_tool, gdouble x,
                                  gdouble y) {
  GtkTextBuffer *buffer = GTK_TEXT_BUFFER(text_tool->buffer);
  ...
  if (text_tool->select_words || text_tool->select_lines) {
    GtkTextIter start;
    GtkTextIter end;
...
void gimp_text_tool_editor_get_cursor_rect(GimpTextTool *text_tool,
  ...
  gtk_text_buffer_get_iter_at_mark(buffer, &cursor,
                                   gtk_text_buffer_get_insert(buffer));
  cursor_index =
      gimp_text_buffer_get_iter_index(text_tool->buffer, &cursor, TRUE);
...
void gimp_text_tool_editor_update_im_cursor(GimpTextTool *text_tool) {
  ...
  g_object_get(text_tool->widget, "x1", &off_x, "y1", &off_y, NULL);
...
static void gimp_text_tool_ensure_proxy(GimpTextTool *text_tool) {
  ...
  if (text_tool->offscreen_window &&
    ...
    g_signal_connect_swapped(text_tool->proxy_text_view, "move-cursor",
                             G_CALLBACK(gimp_text_tool_move_cursor), text_tool);
    ...
    g_signal_connect_swapped(text_tool->proxy_text_view, "backspace",
                             G_CALLBACK(gimp_text_tool_backspace), text_tool);
    ...
    g_signal_connect_swapped(text_tool->proxy_text_view, "select-all",
                             G_CALLBACK(gimp_text_tool_select_all), text_tool);
    g_signal_connect_swapped(text_tool->proxy_text_view, "change-size",
                             G_CALLBACK(gimp_text_tool_change_size), text_tool);
...
static void gimp_text_tool_move_cursor(GimpTextTool *text_tool,
  ...
  GIMP_LOG(TEXT_EDITING, "%s count = %d, select = %s",
           g_enum_get_value(g_type_class_ref(GTK_TYPE_MOVEMENT_STEP), step)
               ->value_name,
           count, extend_selection ? "TRUE" : "FALSE");
    ...
    if (!cancel_selection) {
      ...
      while (count != 0) {
        ...
        if (count > 0) {
          ...
          pango_layout_move_cursor_visually(layout, TRUE, new_index, trailing,
                                            1, &new_index, &trailing);
          count--;
        } else {
          pango_layout_move_cursor_visually(layout, TRUE, index, 0, -1,
                                            &new_index, &trailing);
        ...
        gimp_text_buffer_get_iter_at_index(text_tool->buffer, &cursor, index,
                                           TRUE);
  ...
  case GTK_MOVEMENT_DISPLAY_LINES: {
    ...
    pango_layout_index_to_line_x(layout, cursor_index, FALSE, &line, &x_pos);
    ...
    if (text_tool->x_pos != -1 &&
        (x_pos <= logical.x || x_pos >= logical.x + logical.width))
    ...
    pango_layout_line_x_to_index(layout_line, x_pos - logical.x, &cursor_index,
                                 &trailing);

    gimp_text_buffer_get_iter_at_index(text_tool->buffer, &cursor, cursor_index,
                                       TRUE);
...
static gboolean is_whitespace(gunichar ch, gpointer user_data) {
  return (ch == ' ' || ch == '\t');
}

static gboolean is_not_whitespace(gunichar ch, gpointer user_data) {
...
static void gimp_text_tool_delete_from_cursor(GimpTextTool *text_tool,
  ...
  GIMP_LOG(TEXT_EDITING, "%s count = %d",
           g_enum_get_value(g_type_class_ref(GTK_TYPE_DELETE_TYPE), type)
               ->value_name,
           count);
...
static void gimp_text_tool_editor_dialog(GimpTextTool *text_tool) {
  ...
  dialog_factory =
      gimp_dock_container_get_dialog_factory(GIMP_DOCK_CONTAINER(image_window));

  if (text_tool->image)
    gimp_image_get_resolution(text_tool->image, &xres, &yres);

  text_tool->editor_dialog = gimp_text_options_editor_new(
      parent, tool->tool_info->gimp, options,
      gimp_dialog_factory_get_menu_factory(dialog_factory),
      _("GIMP Text Editor"), text_tool->proxy, text_tool->buffer, xres, yres);
  ...
  gimp_dialog_factory_add_foreign(
      dialog_factory, "gimp-text-tool-dialog", text_tool->editor_dialog,
      gimp_widget_get_monitor(GTK_WIDGET(image_window)));

  g_signal_connect(text_tool->editor_dialog, "destroy",
                   G_CALLBACK(gimp_text_tool_editor_destroy), text_tool);
...
static void gimp_text_tool_editor_destroy(GtkWidget *dialog,
  ...
  g_object_set(options, "use-editor", FALSE, NULL);
}
...
static void gimp_text_tool_enter_text(GimpTextTool *text_tool,
  ...
  if (had_selection && text_tool->style_editor)
    insert_tags = gimp_text_style_editor_list_tags(
        GIMP_TEXT_STYLE_EDITOR(text_tool->style_editor), &remove_tags);
  ...
  if (had_selection && text_tool->style_editor)
    gimp_text_buffer_set_insert_tags(text_tool->buffer, insert_tags,
                                     remove_tags);
...
static void gimp_text_tool_xy_to_iter(GimpTextTool *text_tool, gdouble x,
  ...
  pango_layout_xy_to_index(layout, x * PANGO_SCALE, y * PANGO_SCALE, &index,
                           &trailing);
...
static void gimp_text_tool_im_preedit_changed(GtkIMContext *context,
  ...
  gtk_im_context_get_preedit_string(context, &text_tool->preedit_string, &attrs,
  ...
  if (text_tool->preedit_string && *text_tool->preedit_string) {
    ...
    text_tool->preedit_start =
        gtk_text_buffer_create_mark(buffer, "preedit-start", &iter, TRUE);
    ...
    do {
      ...
      if (attr_start < strlen(text_tool->preedit_string)) {
        ...
        if (!g_utf8_validate(text_tool->preedit_string + attr_start,
                             attr_end - attr_start, &valid_preedit_end))
          g_warning("%s: preedit string is not valid UTF-8.", G_STRFUNC);

        /* Insert the preedit chunk at current cursor position. */
        gtk_text_buffer_insert_at_cursor(
            GTK_TEXT_BUFFER(text_tool->buffer),
          ...
          if (attr) {
            ...
            case PANGO_ATTR_UNDERLINE:
              gtk_text_buffer_apply_tag(
                  buffer, text_tool->buffer->preedit_underline_tag, &start,
                  &end);
              break;
            ...
            case PANGO_ATTR_FOREGROUND: {
              ...
              if (attr->klass->type == PANGO_ATTR_BACKGROUND) {
                gimp_text_buffer_set_preedit_bg_color(text_tool->buffer, &start,
                                                      &end, &color);
              } else {
                gimp_text_buffer_set_preedit_color(text_tool->buffer, &start,
                                                   &end, &color);
              }
            } break;
            default:
    ...
    text_tool->preedit_end =
        gtk_text_buffer_create_mark(buffer, "preedit-end", &iter, FALSE);
...
static void gimp_text_tool_im_delete_preedit(GimpTextTool *text_tool) {
  if (text_tool->preedit_string) {
    if (*text_tool->preedit_string) {
      ...
      gtk_text_buffer_get_iter_at_mark(buffer, &start,
                                       text_tool->preedit_start);
      gtk_text_buffer_get_iter_at_mark(buffer, &end, text_tool->preedit_end);
...
gimp_text_tool_editor_copy_selection_to_clipboard(GimpTextTool *text_tool) {
  ...
  if (!text_tool->editor_dialog && gtk_text_buffer_get_has_selection(buffer)) {
    GimpTool *tool = GIMP_TOOL(text_tool);
    GimpDisplayShell *shell = gimp_display_get_shell(tool->display);
    GtkClipboard *clipboard;

    clipboard =
        gtk_widget_get_clipboard(GTK_WIDGET(shell), GDK_SELECTION_PRIMARY);
