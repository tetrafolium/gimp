...
static void gimp_foreground_select_tool_control(GimpTool *tool,
                                                ...
                                                GimpDisplay *display);
static void gimp_foreground_select_tool_button_press(
    GimpTool *tool, const GimpCoords *coords, guint32 time,
    GdkModifierType state, GimpButtonPressType press_type,
    GimpDisplay *display);
static void gimp_foreground_select_tool_button_release(
    GimpTool *tool, const GimpCoords *coords, guint32 time,
...
static void gimp_foreground_select_tool_active_modifier_key(
    GimpTool *tool, GdkModifierType key, gboolean press, GdkModifierType state,
    GimpDisplay *display);
...
static void gimp_foreground_select_tool_cursor_update(GimpTool *tool,
                                                      ...
                                                      GdkModifierType state,
                                                      GimpDisplay *display);
static const gchar *gimp_foreground_select_tool_can_undo(GimpTool *tool,
                                                         GimpDisplay *display);
static const gchar *gimp_foreground_select_tool_can_redo(GimpTool *tool,
                                                         GimpDisplay *display);
...
static void
gimp_foreground_select_tool_response(GimpToolGui *gui, gint response_id,
                                     GimpForegroundSelectTool *fg_select);
static void gimp_foreground_select_tool_preview_toggled(
    GtkToggleButton *button, GimpForegroundSelectTool *fg_select);

static void
gimp_foreground_select_tool_update_gui(GimpForegroundSelectTool *fg_select);

static StrokeUndo *
gimp_foreground_select_undo_new(GeglBuffer *trimap, GArray *stroke,
                                GimpMattingDrawMode draw_mode,
                                gint stroke_width);
...
void gimp_foreground_select_tool_register(GimpToolRegisterCallback callback,
                                          gpointer data) {
  (*callback)(
      GIMP_TYPE_FOREGROUND_SELECT_TOOL, GIMP_TYPE_FOREGROUND_SELECT_OPTIONS,
      gimp_foreground_select_options_gui,
      GIMP_CONTEXT_PROP_MASK_FOREGROUND | GIMP_CONTEXT_PROP_MASK_BACKGROUND,
      "gimp-foreground-select-tool", _("Foreground Select"),
      _("Foreground Select Tool: Select a region containing foreground "
        "objects"),
      N_("F_oreground Select"), NULL, NULL, GIMP_HELP_TOOL_FOREGROUND_SELECT,
      GIMP_ICON_TOOL_FOREGROUND_SELECT, data);
}
...
gimp_foreground_select_tool_class_init(GimpForegroundSelectToolClass *klass) {
  ...
  tool_class->active_modifier_key =
      gimp_foreground_select_tool_active_modifier_key;
...
gimp_foreground_select_tool_init(GimpForegroundSelectTool *fg_select) {
  ...
  gimp_tool_control_set_action_size(
      tool->control, "tools/tools-foreground-select-brush-size-set");
...
static gboolean gimp_foreground_select_tool_initialize(GimpTool *tool,
  ...
  if (!fg_select->gui) {
    fg_select->gui = gimp_tool_gui_new(
        tool->tool_info, NULL, _("Dialog for foreground select"), NULL, NULL,
        gimp_widget_get_monitor(GTK_WIDGET(shell)), TRUE,

        _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Select"), GTK_RESPONSE_APPLY,
...
static void gimp_foreground_select_tool_motion(GimpTool *tool,
  ...
  if (fg_select->state == MATTING_STATE_FREE_SELECT) {
    ...
    GimpVector2 *last = &g_array_index(fg_select->stroke, GimpVector2,
  ...
  if (fg_select->state == MATTING_STATE_FREE_SELECT) {
    ...
    case GDK_KEY_ISO_Enter:
      if (fg_select->state == MATTING_STATE_PAINT_TRIMAP)
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(fg_select->preview_toggle), TRUE);
      else
        gimp_foreground_select_tool_response(fg_select->gui, GTK_RESPONSE_APPLY,
                                             fg_select);
      return TRUE;
    ...
    case GDK_KEY_Escape:
      if (fg_select->state == MATTING_STATE_PAINT_TRIMAP)
        ...
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON(fg_select->preview_toggle), FALSE);
      return TRUE;
...
static void gimp_foreground_select_tool_oper_update(GimpTool *tool,
  ...
  if (fg_select->state == MATTING_STATE_FREE_SELECT) {
    if (GIMP_SELECTION_TOOL(tool)->function == SELECTION_SELECT) {
      ...
      gimp_polygon_select_tool_get_points(GIMP_POLYGON_SELECT_TOOL(tool), NULL,
                                          &n_points);
  ...
  if (proximity && status_stage) {
    if (status_mode)
      gimp_tool_replace_status(tool, display, "%s, %s", status_mode,
                               status_stage);
    else
...
static const gchar *gimp_foreground_select_tool_can_undo(GimpTool *tool,
  ...
  if (fg_select->undo_stack) {
    ...
    if (gimp_enum_get_value(GIMP_TYPE_MATTING_DRAW_MODE, undo->draw_mode, NULL,
                            NULL, &desc, NULL)) {
      return desc;
    }
  }
...
static void gimp_foreground_select_tool_options_notify(
    GimpTool *tool, GimpToolOptions *options, const GParamSpec *pspec) {
  GimpForegroundSelectTool *fg_select = GIMP_FOREGROUND_SELECT_TOOL(tool);
...
static void gimp_foreground_select_tool_get_area(GeglBuffer *mask, gint *x1,
  ...
  if (fg_select->state == MATTING_STATE_FREE_SELECT) {
    ...
    if (fg_select->stroke) {
      ...
      gimp_draw_tool_add_pen(
          draw_tool, (const GimpVector2 *)fg_select->stroke->data,
          fg_select->stroke->len, GIMP_CONTEXT(options),
          GIMP_ACTIVE_COLOR_FOREGROUND, options->stroke_width * shell->scale_y);
    }
    ...
    if (FALSE) {
      ...
      gimp_foreground_select_tool_get_area(fg_select->mask, &x1, &y1, &x2, &y2);

      if (x < x1 + radius || x > x2 - radius || y < y1 + radius ||
          y > y2 - radius) {
        gimp_draw_tool_add_rectangle(draw_tool, FALSE, x1, y1, x2 - x1,
                                     y2 - y1);
      }
    }

    if (x > FAR_OUTSIDE && y > FAR_OUTSIDE)
      gimp_draw_tool_add_arc(draw_tool, FALSE, x - radius, y - radius,
                             2 * radius, 2 * radius, 0.0, 2.0 * G_PI);
...
static void gimp_foreground_select_tool_confirm(GimpPolygonSelectTool *poly_sel,
  ...
  if (drawable && fg_select->state == MATTING_STATE_FREE_SELECT) {
    ...
    fg_select->trimap = gegl_buffer_new(
        GEGL_RECTANGLE(gimp_item_get_offset_x(item),
                       gimp_item_get_offset_y(item), gimp_item_get_width(item),
                       gimp_item_get_height(item)),
        gimp_image_get_mask_format(image));

    gimp_scan_convert_render_value(scan_convert, fg_select->trimap, 0, 0, 0.5);
    gimp_scan_convert_free(scan_convert);

    fg_select->grayscale_preview = gimp_canvas_buffer_preview_new(
        gimp_display_get_shell(display), fg_select->trimap);
...
gimp_foreground_select_tool_halt(GimpForegroundSelectTool *fg_select) {
  ...
  if (tool->display)
    gimp_display_shell_set_mask(gimp_display_get_shell(tool->display), NULL, 0,
                                0, NULL, FALSE);
...
gimp_foreground_select_tool_commit(GimpForegroundSelectTool *fg_select) {
  ...
  if (tool->display && fg_select->state != MATTING_STATE_FREE_SELECT) {
    ...
    gimp_channel_select_buffer(
        gimp_image_get_mask(image), C_("command", "Foreground Select"),
        fg_select->mask, 0, /* x offset */
        0,                  /* y offset */
        options->operation, options->feather, options->feather_radius,
...
gimp_foreground_select_tool_set_trimap(GimpForegroundSelectTool *fg_select) {
  ...
  if (options->preview_mode == GIMP_MATTING_PREVIEW_MODE_ON_COLOR) {
    ...
    gimp_display_shell_set_mask(gimp_display_get_shell(tool->display),
                                fg_select->trimap, 0, 0, &options->mask_color,
                                TRUE);
  } else {
    gimp_display_shell_set_mask(gimp_display_get_shell(tool->display), NULL, 0,
                                0, NULL, FALSE);
    ...
    if (fg_select->grayscale_preview) {
      g_object_set(fg_select->grayscale_preview, "buffer", fg_select->trimap,
                   NULL);
...
gimp_foreground_select_tool_set_preview(GimpForegroundSelectTool *fg_select) {
  ...
  if (options->preview_mode == GIMP_MATTING_PREVIEW_MODE_ON_COLOR) {
    ...
    gimp_display_shell_set_mask(gimp_display_get_shell(tool->display),
                                fg_select->mask, 0, 0, &options->mask_color,
                                TRUE);
  } else {
    gimp_display_shell_set_mask(gimp_display_get_shell(tool->display), NULL, 0,
                                0, NULL, FALSE);
    ...
    if (fg_select->grayscale_preview) {
      g_object_set(fg_select->grayscale_preview, "buffer", fg_select->mask,
                   NULL);
      gimp_canvas_item_set_visible(fg_select->grayscale_preview, TRUE);
    }
  }
...
gimp_foreground_select_tool_preview(GimpForegroundSelectTool *fg_select) {
  ...
  fg_select->mask = gimp_drawable_foreground_extract(
      drawable, options->engine, options->iterations, options->levels,
      options->active_levels, fg_select->trimap, GIMP_PROGRESS(fg_select));
...
gimp_foreground_select_tool_stroke_paint(GimpForegroundSelectTool *fg_select) {
  ...
  undo = gimp_foreground_select_undo_new(fg_select->trimap, fg_select->stroke,
  ...
  if (fg_select->stroke->len == 1) {
    ...
    gimp_scan_convert_add_polyline(scan_convert, fg_select->stroke->len,
                                   (GimpVector2 *)fg_select->stroke->data,
                                   FALSE);
  }

  gimp_scan_convert_stroke(scan_convert, width, GIMP_JOIN_ROUND, GIMP_CAP_ROUND,
                           10.0, 0.0, NULL);
  ...
  gimp_scan_convert_compose_value(scan_convert, fg_select->trimap, 0, 0,
                                  opacity);
...

{
  ...
  gimp_gegl_buffer_copy(trimap, GEGL_RECTANGLE(x1, y1, width, height),
                        GEGL_ABYSS_NONE, undo->saved_trimap, NULL);
...
static void gimp_foreground_select_undo_pop(StrokeUndo *undo,
  ...
  gimp_gegl_buffer_copy(
      trimap, GEGL_RECTANGLE(undo->trimap_x, undo->trimap_y, width, height),
      GEGL_ABYSS_NONE, undo->saved_trimap, NULL);

  gimp_gegl_buffer_copy(
      buffer, GEGL_RECTANGLE(undo->trimap_x, undo->trimap_y, width, height),
      GEGL_ABYSS_NONE, trimap, NULL);
