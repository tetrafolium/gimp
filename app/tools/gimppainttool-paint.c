...
static gboolean gimp_paint_tool_paint_use_thread(GimpPaintTool *paint_tool) {
  if (!paint_tool->draw_line) {
    if (!paint_thread) {
      ...
      if (use_paint_thread) {
        paint_thread =
            g_thread_new("paint", gimp_paint_tool_paint_thread, NULL);
      }
    }
...
static void gimp_paint_tool_paint_interpolate(GimpPaintTool *paint_tool,
  ...
  gimp_paint_core_interpolate(core, drawable, paint_options, &data->coords,
                              data->time);
...
gboolean gimp_paint_tool_paint_start(GimpPaintTool *paint_tool,
                                     GimpDisplay *display,
  ...
  if ((display != tool->display) || !paint_tool->draw_line) {
     ...
     */
    gimp_paint_core_round_line(core, paint_options, constrain, offset_angle,
                               xres, yres);
  }
  ...
  if (gimp_paint_tool_paint_use_thread(paint_tool)) {
    paint_timeout_id = g_timeout_add_full(
        G_PRIORITY_HIGH_IDLE, DISPLAY_UPDATE_INTERVAL / 1000,
        (GSourceFunc)gimp_paint_tool_paint_timeout, paint_tool, NULL);
  }

  return TRUE;
}

void gimp_paint_tool_paint_end(GimpPaintTool *paint_tool, guint32 time,
                               gboolean cancel) {
  GimpPaintOptions *paint_options;
  GimpPaintCore *core;
  GimpDrawable *drawable;
  ...
  if (gimp_paint_tool_paint_use_thread(paint_tool)) {
    ...
    while (!finished) {
      if (!g_cond_wait_until(&paint_queue_cond, &paint_queue_mutex, end_time)) {
        g_mutex_unlock(&paint_queue_mutex);
  ...
  gimp_paint_core_paint(core, drawable, paint_options, GIMP_PAINT_STATE_FINISH,
                        time);
...
gboolean gimp_paint_tool_paint_is_active(GimpPaintTool *paint_tool) {
  g_return_val_if_fail(GIMP_IS_PAINT_TOOL(paint_tool), FALSE);
...
void gimp_paint_tool_paint_push(GimpPaintTool *paint_tool,
...
void gimp_paint_tool_paint_motion(GimpPaintTool *paint_tool,
                                  const GimpCoords *coords, guint32 time) {
  GimpPaintOptions *paint_options;
  GimpPaintCore *core;
  GimpDrawable *drawable;
  ...
  gimp_paint_tool_paint_push(
      paint_tool, (GimpPaintToolPaintFunc)gimp_paint_tool_paint_interpolate,
      data);
}
