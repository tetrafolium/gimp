...
static void gimp_filter_tool_set_line(Controller *controller,
                                      GeglRectangle *area, gdouble x1,
                                      gdouble y1, gdouble x2, gdouble y2);
static void gimp_filter_tool_line_changed(GimpToolWidget *widget,
                                          Controller *controller);

static void
gimp_filter_tool_set_slider_line(Controller *controller, GeglRectangle *area,
...
static void gimp_filter_tool_set_gyroscope(Controller *controller,
                                           GeglRectangle *area, gdouble yaw,
                                           gdouble pitch, gdouble roll,
                                           gdouble zoom, gboolean invert);
...
static void gimp_filter_tool_set_focus(Controller *controller,
                                       GeglRectangle *area, GimpLimitType type,
                                       gdouble x, gdouble y, gdouble radius,
                                       gdouble aspect_ratio, gdouble angle,
                                       gdouble inner_limit, gdouble midpoint);
...
GimpToolWidget *gimp_filter_tool_create_widget(
  ...
  case GIMP_CONTROLLER_TYPE_LINE:
    ...
    g_object_set(controller->widget, "status-title", status_title, NULL);

    g_signal_connect(controller->widget, "changed",
                     G_CALLBACK(gimp_filter_tool_line_changed), controller);
  ...
  case GIMP_CONTROLLER_TYPE_TRANSFORM_GRID: {
    ...
    controller->widget =
        gimp_tool_transform_grid_new(shell, &transform, x1, y1, x2, y2);

    g_object_set(controller->widget, "pivot-x", (x1 + x2) / 2.0, "pivot-y",
                 (y1 + y2) / 2.0, "inside-function",
                 GIMP_TRANSFORM_FUNCTION_MOVE, "outside-function",
                 GIMP_TRANSFORM_FUNCTION_ROTATE, "use-corner-handles", TRUE,
                 "use-perspective-handles", TRUE, "use-side-handles", TRUE,
                 "use-shear-handles", TRUE, "use-pivot-handle", TRUE, NULL);
  ...
  case GIMP_CONTROLLER_TYPE_GYROSCOPE: {
    ...
    g_object_set(controller->widget, "speed",
                 1.0 / MAX(area.width, area.height), "pivot-x",
                 off_x + area.x + area.width / 2.0, "pivot-y",
                 off_y + area.y + area.height / 2.0, NULL);
  ...
  case GIMP_CONTROLLER_TYPE_FOCUS:
    ...
    g_signal_connect(controller->widget, "changed",
                     G_CALLBACK(gimp_filter_tool_focus_changed), controller);
  ...
  g_object_set_data_full(filter_tool->config, "gimp-filter-tool-controller",
                         controller,
...
static void gimp_filter_tool_reset_transform_grid(GimpToolWidget *widget,
                                                  GimpFilterTool *filter_tool) {
  GimpMatrix3 *transform;
  gint off_x, off_y;
  GeglRectangle area;
  ...
  g_object_get(widget, "transform", &transform, NULL);
  ...
  gimp_matrix3_transform_point(transform, (x1 + x2) / 2.0, (y1 + y2) / 2.0,
                               &pivot_x, &pivot_y);

  g_object_set(widget, "pivot-x", pivot_x, "pivot-y", pivot_y, NULL);
...
void gimp_filter_tool_reset_widget(GimpFilterTool *filter_tool,
  ...
  controller =
      g_object_get_data(filter_tool->config, "gimp-filter-tool-controller");
  ...
  case GIMP_CONTROLLER_TYPE_TRANSFORM_GRIDS: {
    ...
    gimp_container_foreach(gimp_tool_widget_group_get_children(
...
static void gimp_filter_tool_set_line(Controller *controller,
  ...
  g_signal_handlers_block_by_func(controller->widget,
                                  gimp_filter_tool_line_changed, controller);

  g_object_set(controller->widget, "x1", x1, "y1", y1, "x2", x2, "y2", y2,
               NULL);
...
static void gimp_filter_tool_line_changed(GimpToolWidget *widget,
  ...
  g_object_get(widget, "x1", &x1, "y1", &y1, "x2", &x2, "y2", &y2, NULL);
  ...
  line_callback(controller->creator_data, &area, x1, y1, x2, y2);
}
...
gimp_filter_tool_set_slider_line(Controller *controller, GeglRectangle *area,
  ...
  g_signal_handlers_block_by_func(
      controller->widget, gimp_filter_tool_slider_line_changed, controller);

  g_object_set(controller->widget, "x1", x1, "y1", y1, "x2", x2, "y2", y2,
               NULL);

  gimp_tool_line_set_sliders(GIMP_TOOL_LINE(controller->widget), sliders,
                             n_sliders);
...
static void gimp_filter_tool_slider_line_changed(GimpToolWidget *widget,
  ...
  slider_line_callback(controller->creator_data, &area, x1, y1, x2, y2, sliders,
                       n_sliders);
}
...
static void gimp_filter_tool_set_transform_grid(Controller *controller,
  ...
  g_signal_handlers_block_by_func(
      controller->widget, gimp_filter_tool_transform_grid_changed, controller);

  g_object_set(controller->widget, "transform", &matrix, "x1", x1, "y1", y1,
               "x2", x2, "y2", y2, NULL);
...
static void gimp_filter_tool_transform_grid_changed(GimpToolWidget *widget,
  ...
  transform_grid_callback(controller->creator_data, &area, &matrix);
...
static void gimp_filter_tool_set_transform_grids(Controller *controller,
  ...
  g_signal_handlers_block_by_func(
      controller->widget, gimp_filter_tool_transform_grids_changed, controller);
  ...
  if (n_transforms > gimp_container_get_n_children(grids)) {
    ...
    for (i = gimp_container_get_n_children(grids); i < n_transforms; i++) {
      ...
      if (i > 0 &&
          !memcmp(&transforms[i], &transforms[i - 1], sizeof(GimpMatrix3))) {
        g_object_get(gimp_container_get_last_child(grids), "pivot-x", &pivot_x,
                     "pivot-y", &pivot_y, NULL);
      } else {
        pivot_x = (x1 + x2) / 2.0;
        pivot_y = (y1 + y2) / 2.0;

        gimp_matrix3_transform_point(&transforms[i], pivot_x, pivot_y, &pivot_x,
                                     &pivot_y);
      }

      g_object_set(grid, "pivot-x", pivot_x, "pivot-y", pivot_y,
                   "inside-function", GIMP_TRANSFORM_FUNCTION_MOVE,
                   "outside-function", GIMP_TRANSFORM_FUNCTION_ROTATE,
                   "use-corner-handles", TRUE, "use-perspective-handles", TRUE,
                   "use-side-handles", TRUE, "use-shear-handles", TRUE,
  ...
  for (i = 0; i < n_transforms; i++) {
    ...
    g_object_set(gimp_container_get_child_by_index(grids, i), "transform",
                 &matrix, "x1", x1, "y1", y1, "x2", x2, "y2", y2, NULL);
  }
...
static void gimp_filter_tool_transform_grids_changed(GimpToolWidget *widget,
  ...
  for (i = 0; i < n_transforms; i++) {
    ...
    g_object_get(gimp_container_get_child_by_index(grids, i), "transform",
                 &transform, NULL);

    gimp_matrix3_identity(&transforms[i]);
    gimp_matrix3_translate(&transforms[i], +(off_x + area.x),
                           +(off_y + area.y));
  ...
  transform_grids_callback(controller->creator_data, &area, transforms,
                           n_transforms);
...
static void gimp_filter_tool_set_gyroscope(Controller *controller,
  ...
  g_object_set(controller->widget, "yaw", yaw, "pitch", pitch, "roll", roll,
               "zoom", zoom, "invert", invert, NULL);
...
static void gimp_filter_tool_gyroscope_changed(GimpToolWidget *widget,
  ...
  g_object_get(widget, "yaw", &yaw, "pitch", &pitch, "roll", &roll, "zoom",
               &zoom, "invert", &invert, NULL);

  gyroscope_callback(controller->creator_data, &area, yaw, pitch, roll, zoom,
                     invert);
}
...
static void gimp_filter_tool_set_focus(Controller *controller,
  ...
  g_signal_handlers_block_by_func(controller->widget,
                                  gimp_filter_tool_focus_changed, controller);

  g_object_set(controller->widget, "type", type, "x", x, "y", y, "radius",
               radius, "aspect-ratio", aspect_ratio, "angle", angle,
               "inner-limit", inner_limit, "midpoint", midpoint, NULL);
...
static void gimp_filter_tool_focus_changed(GimpToolWidget *widget,
  ...
  g_object_get(widget, "type", &type, "x", &x, "y", &y, "radius", &radius,
               "aspect-ratio", &aspect_ratio, "angle", &angle, "inner-limit",
               &inner_limit, "midpoint", &midpoint, NULL);
  ...
  focus_callback(controller->creator_data, &area, type, x, y, radius,
                 aspect_ratio, angle, inner_limit, midpoint);
}
