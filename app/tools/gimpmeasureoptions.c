...
enum { PROP_0, PROP_ORIENTATION, PROP_USE_INFO_WINDOW };
...
static void gimp_measure_options_class_init(GimpMeasureOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(
      object_class, PROP_ORIENTATION, "orientation", _("Orientation"),
      _("Orientation against which the angle is measured"),
      GIMP_TYPE_COMPASS_ORIENTATION, GIMP_COMPASS_ORIENTATION_AUTO,
...
GtkWidget *gimp_measure_options_gui(GimpToolOptions *tool_options) {
  ...
  str =
      g_strdup_printf(_("Orientation  (%s)"), gimp_get_mod_string(toggle_mask));
