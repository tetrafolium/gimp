...
enum { COLUMN_NAME, COLUMN_LABEL, COLUMN_ICON_NAME, N_COLUMNS };

/*  local function prototypes  */

static void gimp_gegl_tool_control(GimpTool *tool, GimpToolAction action,
                                   GimpDisplay *display);
...
void gimp_gegl_tool_register(GimpToolRegisterCallback callback, gpointer data) {
  (*callback)(GIMP_TYPE_GEGL_TOOL, GIMP_TYPE_FILTER_OPTIONS,
              gimp_color_options_gui, 0, "gimp-gegl-tool", _("GEGL Operation"),
              _("GEGL Tool: Use an arbitrary GEGL operation"),
              N_("_GEGL Operation..."), NULL, NULL, GIMP_HELP_TOOL_GEGL,
              GIMP_ICON_GEGL, data);
}

static void gimp_gegl_tool_class_init(GimpGeglToolClass *klass) {
...
static void gimp_gegl_tool_init(GimpGeglTool *tool) {}
...
gimp_gegl_tool_operation_blacklisted(const gchar *name,
  ...
  static const gchar *const category_blacklist[] = {
      "compositors", "core",   "debug",       "display",   "hidden",
      "input",       "output", "programming", "transform", "video"};
 ...
 */
static GList *gimp_get_subtype_classes(GType type, GList *classes) {
...
static gint gimp_gegl_tool_compare_operation_names(GeglOperationClass *a,
...
static GList *gimp_get_geglopclasses(void) {
  ...
  opclasses = g_list_sort(opclasses,
                          (GCompareFunc)gimp_gegl_tool_compare_operation_names);
...
static void gimp_gegl_tool_dialog(GimpFilterTool *filter_tool) {
  ...
  for (iter = opclasses; iter; iter = iter->next) {
    ...
    gtk_list_store_insert_with_values(store, NULL, -1, COLUMN_NAME,
                                      opclass->name, COLUMN_LABEL, label,
  ...
  gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(combo), cell, "icon-name",
                                COLUMN_ICON_NAME);
  ...
  g_signal_connect(combo, "changed",
                   G_CALLBACK(gimp_gegl_tool_operation_changed), tool);
  ...
  gtk_box_pack_start(GTK_BOX(main_vbox), tool->description_label, FALSE, FALSE,
                     0);
  ...
  gimp_label_set_attributes(GTK_LABEL(options_gui), PANGO_ATTR_STYLE,
                            PANGO_STYLE_ITALIC, -1);
  g_object_set(options_gui, "margin-top", 4, "margin-bottom", 4, NULL);
...
static void gimp_gegl_tool_halt(GimpGeglTool *gegl_tool) {
  ...
  gimp_operation_tool_set_operation(op_tool, NULL, NULL, NULL, NULL, NULL,
                                    NULL);
}
...
static void gimp_gegl_tool_operation_changed(GtkWidget *widget,
  ...
  gtk_tree_model_get(model, &iter, COLUMN_NAME, &operation, -1);
  ...
  if (operation) {
    ...
    gimp_operation_tool_set_operation(
        GIMP_OPERATION_TOOL(tool), operation, _("GEGL Operation"),
        _("GEGL Operation"), NULL, GIMP_ICON_GEGL, GIMP_HELP_TOOL_GEGL);
    g_free(operation);
  }
}
