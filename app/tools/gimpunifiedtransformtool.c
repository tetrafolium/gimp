...
void gimp_unified_transform_tool_register(GimpToolRegisterCallback callback,
                                          gpointer data) {
  (*callback)(
      GIMP_TYPE_UNIFIED_TRANSFORM_TOOL, GIMP_TYPE_TRANSFORM_GRID_OPTIONS,
      gimp_transform_grid_options_gui, GIMP_CONTEXT_PROP_MASK_BACKGROUND,
      "gimp-unified-transform-tool", _("Unified Transform"),
      _("Unified Transform Tool: "
        "Transform the layer, selection or path"),
      N_("_Unified Transform"), "<shift>T", NULL,
      GIMP_HELP_TOOL_UNIFIED_TRANSFORM, GIMP_ICON_TOOL_UNIFIED_TRANSFORM, data);
}
...
gimp_unified_transform_tool_matrix_to_info(GimpTransformGridTool *tg_tool,
  ...
  gimp_matrix3_transform_point(transform, tr_tool->x1, tr_tool->y1,
                               &tg_tool->trans_info[X0],
                               &tg_tool->trans_info[Y0]);
  gimp_matrix3_transform_point(transform, tr_tool->x2, tr_tool->y1,
                               &tg_tool->trans_info[X1],
                               &tg_tool->trans_info[Y1]);
  gimp_matrix3_transform_point(transform, tr_tool->x1, tr_tool->y2,
                               &tg_tool->trans_info[X2],
                               &tg_tool->trans_info[Y2]);
  gimp_matrix3_transform_point(transform, tr_tool->x2, tr_tool->y2,
...
gimp_unified_transform_tool_readjust(GimpTransformGridTool *tg_tool) {
  ...
  r = MAX(MIN(x, y) / G_SQRT2 - GIMP_TOOL_TRANSFORM_GRID_MAX_HANDLE_SIZE / 2.0,
          GIMP_TOOL_TRANSFORM_GRID_MAX_HANDLE_SIZE / 2.0);

  if (!tg_options->fixedpivot) {
    gimp_display_shell_untransform_xy_f(shell, x, y,
                                        &tg_tool->trans_info[PIVOT_X],
                                        &tg_tool->trans_info[PIVOT_Y]);
  }

  gimp_display_shell_untransform_xy_f(
      shell, x - r, y - r, &tg_tool->trans_info[X0], &tg_tool->trans_info[Y0]);
  gimp_display_shell_untransform_xy_f(
      shell, x + r, y - r, &tg_tool->trans_info[X1], &tg_tool->trans_info[Y1]);
  gimp_display_shell_untransform_xy_f(
      shell, x - r, y + r, &tg_tool->trans_info[X2], &tg_tool->trans_info[Y2]);
  gimp_display_shell_untransform_xy_f(
      shell, x + r, y + r, &tg_tool->trans_info[X3], &tg_tool->trans_info[Y3]);
}
...
gimp_unified_transform_tool_get_widget(GimpTransformGridTool *tg_tool) {
  ...
  widget = gimp_tool_transform_grid_new(shell, &tr_tool->transform, tr_tool->x1,
                                        tr_tool->y1, tr_tool->x2, tr_tool->y2);

  g_object_set(widget, "pivot-x", (tr_tool->x1 + tr_tool->x2) / 2.0, "pivot-y",
               (tr_tool->y1 + tr_tool->y2) / 2.0, "inside-function",
...
gimp_unified_transform_tool_update_widget(GimpTransformGridTool *tg_tool) {
  ...
  g_object_set(tg_tool->widget, "x1", (gdouble)tr_tool->x1, "y1",
               (gdouble)tr_tool->y1, "x2", (gdouble)tr_tool->x2, "y2",
               (gdouble)tr_tool->y2, "pivot-x", tg_tool->trans_info[PIVOT_X],
               "pivot-y", tg_tool->trans_info[PIVOT_Y], NULL);
}
...
gimp_unified_transform_tool_widget_changed(GimpTransformGridTool *tg_tool) {
  ...
  g_object_get(tg_tool->widget, "transform", &transform, "pivot-x",
