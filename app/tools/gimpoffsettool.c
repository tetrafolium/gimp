...
static gboolean gimp_offset_tool_initialize(GimpTool *tool,
                                            GimpDisplay *display,
                                            GError **error);
static void gimp_offset_tool_control(GimpTool *tool, GimpToolAction action,
                                     GimpDisplay *display);
...
static void gimp_offset_tool_button_release(GimpTool *tool,
                                            ...
                                            GimpDisplay *display);
static void gimp_offset_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                    guint32 time, GdkModifierType state,
                                    GimpDisplay *display);
...
G_DEFINE_TYPE(GimpOffsetTool, gimp_offset_tool, GIMP_TYPE_FILTER_TOOL)

#define parent_class gimp_offset_tool_parent_class

void gimp_offset_tool_register(GimpToolRegisterCallback callback,
                               gpointer data) {
  (*callback)(GIMP_TYPE_OFFSET_TOOL, GIMP_TYPE_FILTER_OPTIONS, NULL,
              GIMP_CONTEXT_PROP_MASK_BACKGROUND, "gimp-offset-tool",
              _("Offset"),
              _("Shift the pixels, optionally wrapping them at the borders"),
              N_("_Offset..."), NULL, NULL, GIMP_HELP_TOOL_OFFSET,
              GIMP_ICON_TOOL_OFFSET, data);
}

static void gimp_offset_tool_class_init(GimpOffsetToolClass *klass) {
...
static void gimp_offset_tool_init(GimpOffsetTool *offset_tool) {
...
static gboolean gimp_offset_tool_initialize(GimpTool *tool,
  ...
  g_signal_handlers_block_by_func(offset_tool->offset_se,
                                  gimp_offset_tool_offset_changed, offset_tool);

  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(offset_tool->offset_se), 0,
                                 xres, FALSE);
  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(offset_tool->offset_se), 1,
                                 yres, FALSE);
  ...
  g_signal_handlers_unblock_by_func(
      offset_tool->offset_se, gimp_offset_tool_offset_changed, offset_tool);

  gegl_node_set(filter_tool->operation, "context", context, NULL);
...
static void gimp_offset_tool_button_press(GimpTool *tool,
  ...
  offset_tool->dragging =
      !gimp_filter_tool_on_guide(GIMP_FILTER_TOOL(tool), coords, display);
  ...
  if (!offset_tool->dragging) {
    ...
    g_object_get(GIMP_FILTER_TOOL(tool)->config, "x", &offset_tool->offset_x,
                 "y", &offset_tool->offset_y, NULL);
...
static void gimp_offset_tool_button_release(GimpTool *tool,
  ...
  if (!offset_tool->dragging) {
    ...
    if (release_type == GIMP_BUTTON_RELEASE_CANCEL) {
      g_object_set(GIMP_FILTER_TOOL(tool)->config, "x", offset_tool->offset_x,
                   "y", offset_tool->offset_y, NULL);
    }
  }
}
...
static void gimp_offset_tool_motion(GimpTool *tool, const GimpCoords *coords,
  ...
  if (!offset_tool->dragging) {
    ...
    g_object_get(filter_tool->config, "type", &type, NULL);
    ...
    g_object_set(filter_tool->config, "x", x, "y", y, NULL);

    gimp_tool_pop_status(tool, display);

    gimp_tool_push_status_coords(tool, display,
                                 GIMP_CURSOR_PRECISION_PIXEL_CENTER,
                                 _("Offset: "), offset_x, ", ", offset_y, NULL);
  }
}
...
static void gimp_offset_tool_cursor_update(GimpTool *tool,
  ...
  if (!tool->drawables ||
    ...
    gimp_tool_set_cursor(tool, display, GIMP_CURSOR_MOUSE,
                         GIMP_TOOL_CURSOR_MOVE, GIMP_CURSOR_MODIFIER_NONE);
  }
}
...
static void gimp_offset_tool_dialog(GimpFilterTool *filter_tool) {
  ...
  offset_tool->offset_se =
      gimp_size_entry_new(1, GIMP_UNIT_PIXEL, "%a", TRUE, TRUE, FALSE, 10,
  ...
  g_signal_connect(offset_tool->offset_se, "refval-changed",
                   G_CALLBACK(gimp_offset_tool_offset_changed), offset_tool);
  ...
  g_signal_connect(button, "clicked",
                   G_CALLBACK(gimp_offset_tool_half_xy_clicked), offset_tool);
  ...
  g_signal_connect(button, "clicked",
                   G_CALLBACK(gimp_offset_tool_half_x_clicked), offset_tool);
  ...
  g_signal_connect(button, "clicked",
                   G_CALLBACK(gimp_offset_tool_half_y_clicked), offset_tool);

  /*  The edge behavior frame  */
  frame = gimp_int_radio_group_new(
      TRUE, _("Edge Behavior"),

      G_CALLBACK(gimp_offset_tool_edge_behavior_toggled), offset_tool, NULL,

      GIMP_OFFSET_WRAP_AROUND,

      _("W_rap around"), GIMP_OFFSET_WRAP_AROUND, NULL,

      _("Fill with _background color"), GIMP_OFFSET_BACKGROUND, NULL,

      _("Make _transparent"), GIMP_OFFSET_TRANSPARENT,
...
static void gimp_offset_tool_offset_changed(GimpSizeEntry *se,
                                            GimpOffsetTool *offset_tool) {
  g_object_set(GIMP_FILTER_TOOL(offset_tool)->config, "x",
               (gint)gimp_size_entry_get_refval(se, 0), "y",
               (gint)gimp_size_entry_get_refval(se, 1), NULL);
}

static void gimp_offset_tool_half_xy_clicked(GtkButton *button,
                                             GimpOffsetTool *offset_tool) {
  g_object_set(GIMP_FILTER_TOOL(offset_tool)->config, "x",
               gimp_offset_tool_get_width(offset_tool) / 2, "y",
               gimp_offset_tool_get_height(offset_tool) / 2, NULL);
}

static void gimp_offset_tool_half_x_clicked(GtkButton *button,
                                            GimpOffsetTool *offset_tool) {
  g_object_set(GIMP_FILTER_TOOL(offset_tool)->config, "x",
               gimp_offset_tool_get_width(offset_tool) / 2, NULL);
}

static void gimp_offset_tool_half_y_clicked(GtkButton *button,
                                            GimpOffsetTool *offset_tool) {
  g_object_set(GIMP_FILTER_TOOL(offset_tool)->config, "y",
               gimp_offset_tool_get_height(offset_tool) / 2, NULL);
}
...
gimp_offset_tool_edge_behavior_toggled(GtkToggleButton *toggle,
  ...
  if (gtk_toggle_button_get_active(toggle)) {
    ...
    type =
        GPOINTER_TO_INT(g_object_get_data(G_OBJECT(toggle), "gimp-item-data"));

    g_object_set(GIMP_FILTER_TOOL(offset_tool)->config, "type", type, NULL);
  }
}
...
static void gimp_offset_tool_update(GimpOffsetTool *offset_tool) {
  ...
  g_object_get(filter_tool->config, "type", &orig_type, "x", &orig_x, "y",
               &orig_y, NULL);
  ...
  if (tool->drawables && !gimp_drawable_has_alpha(tool->drawables->data) &&
      type == GIMP_OFFSET_TRANSPARENT) {
    type = GIMP_OFFSET_BACKGROUND;
  }

  if (x != orig_x || y != orig_y || type != orig_type) {
    g_object_set(filter_tool->config, "type", type, "x", x, "y", y, NULL);
  }
  ...
  if (offset_tool->offset_se) {
    ...
    gimp_size_entry_set_refval_boundaries(
        GIMP_SIZE_ENTRY(offset_tool->offset_se), 0, -width, +width);
    gimp_size_entry_set_refval_boundaries(
        GIMP_SIZE_ENTRY(offset_tool->offset_se), 1, -height, +height);

    gimp_size_entry_set_size(GIMP_SIZE_ENTRY(offset_tool->offset_se), 0, 0,
                             width);
    gimp_size_entry_set_size(GIMP_SIZE_ENTRY(offset_tool->offset_se), 1, 0,
                             height);

    gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(offset_tool->offset_se), 0, x);
    gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(offset_tool->offset_se), 1, y);
  ...
  if (offset_tool->transparent_radio) {
    gimp_int_radio_group_set_active(
        GTK_RADIO_BUTTON(offset_tool->transparent_radio), type);
  }
}
...
static void gimp_offset_tool_halt(GimpOffsetTool *offset_tool) {
  ...
  g_signal_handlers_disconnect_by_func(
      context, gimp_offset_tool_background_changed, offset_tool);
}
