...
static void gimp_crop_options_rectangle_options_iface_init(
    GimpRectangleOptionsInterface *iface);

static void gimp_crop_options_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_crop_options_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);

G_DEFINE_TYPE_WITH_CODE(
    GimpCropOptions, gimp_crop_options, GIMP_TYPE_TOOL_OPTIONS,
...
static void gimp_crop_options_class_init(GimpCropOptionsClass *klass) {
   ...
   */
  GIMP_CONFIG_PROP_BOOLEAN(object_class, GIMP_RECTANGLE_OPTIONS_PROP_HIGHLIGHT,
                           "highlight", _("Highlight"),
                           _("Dim everything outside selection"), TRUE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class,
                          GIMP_RECTANGLE_OPTIONS_PROP_HIGHLIGHT_OPACITY,
                          "highlight-opacity", _("Highlight opacity"),
                          _("How much to dim everything outside selection"),
                          0.0, 1.0, 0.5, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_LAYER_ONLY, "layer-only",
                           _("Current layers only"),
                           _("Crop only currently selected layers"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_DELETE_PIXELS, "delete-pixels",
      _("Delete cropped pixels"),
      _("Discard non-locked layer data that falls out of the crop region"),
      FALSE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_ALLOW_GROWING, "allow-growing",
                           _("Allow growing"),
  ...
  GIMP_CONFIG_PROP_ENUM(
      object_class, PROP_FILL_TYPE, "fill-type", _("Fill with"),
...
GtkWidget *gimp_crop_options_gui(GimpToolOptions *tool_options) {
  ...
  g_object_bind_property(G_OBJECT(config), "layer-only", G_OBJECT(button2),
                         "sensitive",
  ...
  frame =
      gimp_prop_expanding_frame_new(config, "allow-growing", NULL, combo, NULL);
