...
enum { PROP_0, PROP_AUTO_RESIZE, PROP_ZOOM_TYPE };
...
G_DEFINE_TYPE_WITH_CODE(
    GimpMagnifyOptions, gimp_magnify_options, GIMP_TYPE_TOOL_OPTIONS,
...
static void gimp_magnify_options_class_init(GimpMagnifyOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_AUTO_RESIZE, "auto-resize",
                           _("Auto-resize window"),
                           _("Resize image window to accommodate "
                             "new zoom level"),
                           FALSE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_ZOOM_TYPE, "zoom-type",
                        _("Direction"), _("Direction of magnification"),
                        GIMP_TYPE_ZOOM_TYPE, GIMP_ZOOM_IN,
...
static void gimp_magnify_options_reset(GimpConfig *config) {
  ...
  pspec =
      g_object_class_find_property(G_OBJECT_GET_CLASS(config), "auto-resize");

  if (pspec)
    G_PARAM_SPEC_BOOLEAN(pspec)->default_value =
        GIMP_DISPLAY_CONFIG(tool_options->tool_info->gimp->config)
...
GtkWidget *gimp_magnify_options_gui(GimpToolOptions *tool_options) {
  ...
  /*  tool toggle  */
  str = g_strdup_printf(_("Direction  (%s)"), gimp_get_mod_string(toggle_mask));
  frame = gimp_prop_enum_radio_frame_new(config, "zoom-type", str, 0, 0);
