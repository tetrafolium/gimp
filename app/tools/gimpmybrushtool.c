...
void gimp_mybrush_tool_register(GimpToolRegisterCallback callback,
                                gpointer data) {
  (*callback)(
      GIMP_TYPE_MYBRUSH_TOOL, GIMP_TYPE_MYBRUSH_OPTIONS,
      ...
      "gimp-mypaint-brush-tool", _("MyPaint Brush"),
      _("MyPaint Brush Tool: Use MyPaint brushes in GIMP"),
      N_("M_yPaint Brush"), "Y", NULL, GIMP_HELP_TOOL_MYPAINT_BRUSH,
      GIMP_ICON_TOOL_MYPAINT_BRUSH, data);
}

static void gimp_mybrush_tool_class_init(GimpMybrushToolClass *klass) {
...
static void gimp_mybrush_tool_init(GimpMybrushTool *mybrush_tool) {
  ...
  gimp_tool_control_set_action_hardness(
      tool->control, "tools/tools-mypaint-brush-hardness-set");
...
GimpCanvasItem *gimp_mybrush_tool_create_cursor(GimpPaintTool *paint_tool,
  ...
  if (SCALEX(shell, radius) > 4 && SCALEY(shell, radius) > 4) {
    return gimp_canvas_arc_new(shell, x, y, radius, radius, 0.0, 2 * G_PI,
                               FALSE);
  }
