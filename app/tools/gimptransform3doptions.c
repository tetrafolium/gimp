...
gimp_transform_3d_options_class_init(GimpTransform3DOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(object_class, PROP_MODE, "mode", _("Mode"),
                        _("Transform mode"), GIMP_TYPE_TRANSFORM_3D_MODE,
                        GIMP_TRANSFORM_3D_MODE_ROTATE,
                        GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_UNIFIED, "unified", _("Unified interaction"),
      _("Combine all interaction modes"), FALSE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_CONSTRAIN_AXIS, "constrain-axis",
                           NULL, _("Constrain transformation to a single axis"),
                           FALSE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_Z_AXIS, "z-axis", NULL,
                           _("Transform along the Z axis"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_LOCAL_FRAME, "local-frame", NULL,
                           _("Transform in the local frame of reference"),
                           FALSE, GIMP_PARAM_STATIC_STRINGS);
}

static void gimp_transform_3d_options_init(GimpTransform3DOptions *options) {}
...
GtkWidget *gimp_transform_3d_options_gui(GimpToolOptions *tool_options) {
  ...
  label =
      g_strdup_printf(_("Z axis (%s)"), gimp_get_mod_string(constrain_mask));
