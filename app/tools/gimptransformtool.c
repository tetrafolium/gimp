...
static void gimp_transform_tool_control(GimpTool *tool, GimpToolAction action,
                                        GimpDisplay *display);
...
static GeglBuffer *gimp_transform_tool_real_transform(
...
static GeglBuffer *gimp_transform_tool_real_transform(
  ...
  progress = gimp_progress_start(GIMP_PROGRESS(tool), FALSE, "%s",
                                 klass->progress_text);
  ...
  if (orig_buffer) {
    ...
    ret = gimp_drawable_transform_buffer_affine(
        objects->data, context, orig_buffer, orig_offset_x, orig_offset_y,
        &tr_tool->transform, direction, options->interpolation, clip,
        buffer_profile, new_offset_x, new_offset_y, progress);
  } else if (g_list_length(objects) == 1 && GIMP_IS_IMAGE(objects->data)) {
    /*  this happens for images  */

    gimp_image_transform(objects->data, context, &tr_tool->transform, direction,
                         options->interpolation, clip, progress);
  } else {
    GList *items;
    ...
    items = gimp_image_item_list_linked(gimp_item_get_image(objects->data),
                                        objects);

    gimp_image_item_list_transform(gimp_item_get_image(objects->data), items,
                                   context, &tr_tool->transform, direction,
                                   options->interpolation, clip, progress);
    g_list_free(items);
  }
...
static void gimp_transform_tool_halt(GimpTransformTool *tr_tool) {
  ...
  if (tr_tool->restore_type) {
    g_object_set(options, "type", tr_tool->saved_type, NULL);
...
static gboolean gimp_transform_tool_confirm(GimpTransformTool *tr_tool,
  ...
  if (GIMP_TRANSFORM_TOOL_GET_CLASS(tr_tool)->recalc_matrix) {
    ...
    if (selection_empty && selected_objects &&
    ...
    if (options->type == GIMP_TRANSFORM_TYPE_IMAGE) {
      objects = g_list_concat(
          objects, gimp_image_item_list_get_list(image, GIMP_ITEM_TYPE_ALL,
                                                 GIMP_ITEM_SET_ALL));
    }
    ...
    for (iter = objects; iter; iter = g_list_next(iter)) {
      ...
      if (GIMP_IS_DRAWABLE(object)) {
        ...
        gimp_item_bounds(item, &orig_bounds.x, &orig_bounds.y,
      ...
      gimp_transform_resize_boundary(
          ...
          orig_bounds.x, orig_bounds.y, orig_bounds.x + orig_bounds.width,
      ...
      if (new_bounds.width > orig_bounds.width) {
        ratio = MAX(ratio, (gdouble)new_bounds.width /
                               (gdouble)gimp_image_get_width(image));
      }

      if (new_bounds.height > orig_bounds.height) {
        ratio = MAX(ratio, (gdouble)new_bounds.height /
  ...
  if (max_ratio > MIN_CONFIRMATION_RATIO) {
    ...
    dialog = gimp_message_dialog_new(
        _("Confirm Transformation"), GIMP_ICON_DIALOG_WARNING,
        GTK_WIDGET(shell), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
        gimp_standard_help_func, NULL,

        _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Transform"), GTK_RESPONSE_OK,
...
gboolean gimp_transform_tool_bounds(GimpTransformTool *tr_tool,
  ...
  case GIMP_TRANSFORM_TYPE_LAYER: {
    ...
    non_empty = gimp_item_mask_intersect(GIMP_ITEM(drawables->data), &x, &y,
                                         &width, &height);
  ...
  case GIMP_TRANSFORM_TYPE_SELECTION: {
    gimp_item_bounds(GIMP_ITEM(gimp_image_get_mask(image)), &tr_tool->x1,
  ...
  case GIMP_TRANSFORM_TYPE_PATH: {
    ...
    if (!gimp_channel_is_empty(selection)) {
      gimp_item_bounds(GIMP_ITEM(selection), &tr_tool->x1, &tr_tool->y1,
...
GList *gimp_transform_tool_check_selected_objects(GimpTransformTool *tr_tool,
  ...
  case GIMP_TRANSFORM_TYPE_LAYER:
    ...
    for (iter = objects; iter; iter = iter->next) {
      ...
      if (!gimp_item_is_visible(item) && !config->edit_non_visible &&
...
gboolean gimp_transform_tool_transform(GimpTransformTool *tr_tool,
  ...
  selected_objects =
      gimp_transform_tool_check_selected_objects(tr_tool, display, &error);
  ...
  case GIMP_TRANSFORM_TYPE_LAYER:
    if (!gimp_channel_is_empty(gimp_image_get_mask(image))) {
      orig_buffer = gimp_drawable_transform_cut(
          selected_objects, GIMP_CONTEXT(options), &orig_offset_x,
  ...
  new_buffer = GIMP_TRANSFORM_TOOL_GET_CLASS(tr_tool)->transform(
      tr_tool, selected_objects, orig_buffer, orig_offset_x, orig_offset_y,
  ...
  case GIMP_TRANSFORM_TYPE_LAYER:
    if (new_buffer) {
      ...
      gimp_drawable_transform_paste(GIMP_DRAWABLE(selected_objects->data),
                                    new_buffer, buffer_profile, new_offset_x,
                                    new_offset_y, new_layer);
...
void gimp_transform_tool_set_type(GimpTransformTool *tr_tool,
  ...
  g_object_set(options, "type", type, NULL);
