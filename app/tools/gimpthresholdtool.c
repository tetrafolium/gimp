...
static gboolean gimp_threshold_tool_channel_sensitive(gint value,
                                                      gpointer data);
...
G_DEFINE_TYPE(GimpThresholdTool, gimp_threshold_tool, GIMP_TYPE_FILTER_TOOL)

#define parent_class gimp_threshold_tool_parent_class

void gimp_threshold_tool_register(GimpToolRegisterCallback callback,
                                  gpointer data) {
  (*callback)(GIMP_TYPE_THRESHOLD_TOOL, GIMP_TYPE_HISTOGRAM_OPTIONS, NULL, 0,
              "gimp-threshold-tool", _("Threshold"),
              _("Reduce image to two colors using a threshold"),
              N_("_Threshold..."), NULL, NULL, GIMP_HELP_TOOL_THRESHOLD,
              GIMP_ICON_TOOL_THRESHOLD, data);
}
...
static void gimp_threshold_tool_init(GimpThresholdTool *t_tool) {
...
static gboolean gimp_threshold_tool_initialize(GimpTool *tool,
  ...
  g_object_get(filter_tool->config, "low", &low, "high", &high, NULL);
   ...
   */
  {
    ...
    gimp_histogram_calculate(t_tool->histogram, temp,
                             GEGL_RECTANGLE(0, 0, 1, 1), NULL, NULL);
...
static void gimp_threshold_tool_dialog(GimpFilterTool *filter_tool) {
  ...
  t_tool->channel_menu =
      gimp_prop_enum_combo_box_new(filter_tool->config, "channel", -1, -1);
  ...
  g_signal_connect(t_tool->histogram_box->view, "range-changed",
                   G_CALLBACK(gimp_threshold_tool_histogram_range), t_tool);

  g_object_bind_property(G_OBJECT(tool_options), "histogram-scale",
                         G_OBJECT(t_tool->histogram_box->view),
                         "histogram-scale",
  ...
  gimp_help_set_help_data(button,
                          _("Automatically adjust to optimal "
                            "binarization threshold"),
                          NULL);
  gtk_widget_show(button);

  g_signal_connect(button, "clicked",
                   G_CALLBACK(gimp_threshold_tool_auto_clicked), t_tool);
}
...
static void gimp_threshold_tool_config_notify(GimpFilterTool *filter_tool,
  ...
  if (!strcmp(pspec->name, "channel")) {
    ...
    g_object_get(config, "channel", &channel, NULL);

    gimp_histogram_view_set_channel(t_tool->histogram_box->view, channel);
  } else if (!strcmp(pspec->name, "low") || !strcmp(pspec->name, "high")) {
    gdouble low;
    gdouble high;
    gint n_bins;

    g_object_get(config, "low", &low, "high", &high, NULL);
...
static void gimp_threshold_tool_histogram_range(GimpHistogramView *widget,
                                                gint start, gint end,
                                                GimpThresholdTool *t_tool) {
  ...
  g_object_get(filter_tool->config, "low", &config_low, "high", &config_high,
               NULL);

  if (low != config_low || high != config_high) {
    g_object_set(filter_tool->config, "low", low, "high", high, NULL);
  }
}
...
static void gimp_threshold_tool_auto_clicked(GtkWidget *button,
  ...
  if (gimp_async_is_synced(t_tool->histogram_async) &&
    ...
    g_object_get(GIMP_FILTER_TOOL(t_tool)->config, "channel", &channel, NULL);

    n_bins = gimp_histogram_n_bins(t_tool->histogram);

    low =
        gimp_histogram_get_threshold(t_tool->histogram, channel, 0, n_bins - 1);

    gimp_histogram_view_set_range(t_tool->histogram_box->view, low, n_bins - 1);
  }
}
