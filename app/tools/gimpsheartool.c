...
enum { ORIENTATION, SHEAR_X, SHEAR_Y };
...
void gimp_shear_tool_register(GimpToolRegisterCallback callback,
                              gpointer data) {
  (*callback)(GIMP_TYPE_SHEAR_TOOL, GIMP_TYPE_TRANSFORM_GRID_OPTIONS,
              gimp_transform_grid_options_gui, 0, "gimp-shear-tool", _("Shear"),
              _("Shear Tool: Shear the layer, selection or path"), N_("S_hear"),
              "<shift>H", NULL, GIMP_HELP_TOOL_SHEAR, GIMP_ICON_TOOL_SHEAR,
              data);
}
...
static gboolean gimp_shear_tool_info_to_matrix(GimpTransformGridTool *tg_tool,
  ...
  gimp_transform_matrix_shear(
      ...
      tr_tool->y2 - tr_tool->y1, tg_tool->trans_info[ORIENTATION], amount);
...
static gchar *gimp_shear_tool_get_undo_desc(GimpTransformGridTool *tg_tool) {
  ...
  case GIMP_ORIENTATION_HORIZONTAL:
    return g_strdup_printf(C_("undo-type", "Shear horizontally by %-3.3g"), x);

  case GIMP_ORIENTATION_VERTICAL:
    return g_strdup_printf(C_("undo-type", "Shear vertically by %-3.3g"), y);

  default:
    /* e.g. user entered numbers but no notification callback */
    return g_strdup_printf(
        C_("undo-type", "Shear horizontally by %-3.3g, vertically by %-3.3g"),
        x, y);
  }
}
...
static void gimp_shear_tool_dialog(GimpTransformGridTool *tg_tool) {
  ...
  gtk_box_pack_start(GTK_BOX(gimp_tool_gui_get_vbox(tg_tool->gui)), vbox, FALSE,
                     FALSE, 0);
  ...
  g_signal_connect(shear->x_adj, "value-changed",
                   G_CALLBACK(shear_x_mag_changed), tg_tool);
  ...
  g_signal_connect(shear->y_adj, "value-changed",
                   G_CALLBACK(shear_y_mag_changed), tg_tool);
}
...
gimp_shear_tool_get_widget(GimpTransformGridTool *tg_tool) {
  ...
  widget = gimp_tool_shear_grid_new(
      shell, tr_tool->x1, tr_tool->y1, tr_tool->x2, tr_tool->y2,
      tg_tool->trans_info[ORIENTATION], tg_tool->trans_info[SHEAR_X],
      tg_tool->trans_info[SHEAR_Y]);

  g_object_set(widget, "inside-function", GIMP_TRANSFORM_FUNCTION_SHEAR,
               "outside-function", GIMP_TRANSFORM_FUNCTION_SHEAR,
               "frompivot-shear", TRUE, NULL);
...
static void gimp_shear_tool_update_widget(GimpTransformGridTool *tg_tool) {
  ...
  g_object_set(tg_tool->widget, "x1", (gdouble)tr_tool->x1, "y1",
               ...
               (gdouble)tr_tool->y2, "orientation",
               (gint)tg_tool->trans_info[ORIENTATION], "shear-x",
               tg_tool->trans_info[SHEAR_X], "shear-y",
               tg_tool->trans_info[SHEAR_Y], NULL);
}

static void gimp_shear_tool_widget_changed(GimpTransformGridTool *tg_tool) {
  GimpOrientationType orientation;

  g_object_get(tg_tool->widget, "orientation", &orientation, "shear-x",
               &tg_tool->trans_info[SHEAR_X], "shear-y",
               &tg_tool->trans_info[SHEAR_Y], NULL);
