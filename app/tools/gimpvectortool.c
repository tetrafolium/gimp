...
static void gimp_vector_tool_control(GimpTool *tool, GimpToolAction action,
                                     GimpDisplay *display);
...
static void gimp_vector_tool_button_release(GimpTool *tool,
                                            ...
                                            GimpDisplay *display);
static void gimp_vector_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                    guint32 time, GdkModifierType state,
                                    GimpDisplay *display);
static void gimp_vector_tool_modifier_key(GimpTool *tool, GdkModifierType key,
...
static void gimp_vector_tool_path_begin_change(GimpToolWidget *path,
                                               const gchar *desc,
                                               GimpVectorTool *vector_tool);
...
static void gimp_vector_tool_to_selection_extended(GimpVectorTool *vector_tool,
                                                   GdkModifierType state);

static void gimp_vector_tool_fill_vectors(GimpVectorTool *vector_tool,
                                          GtkWidget *button);
static void gimp_vector_tool_fill_callback(GtkWidget *dialog, GimpItem *item,
                                           GList *drawables,
                                           GimpContext *context,
...
static void gimp_vector_tool_stroke_vectors(GimpVectorTool *vector_tool,
                                            GtkWidget *button);
static void gimp_vector_tool_stroke_callback(GtkWidget *dialog, GimpItem *item,
                                             GList *drawables,
                                             GimpContext *context,
...
void gimp_vector_tool_register(GimpToolRegisterCallback callback,
                               gpointer data) {
  (*callback)(
      GIMP_TYPE_VECTOR_TOOL, GIMP_TYPE_VECTOR_OPTIONS, gimp_vector_options_gui,
      GIMP_PAINT_OPTIONS_CONTEXT_MASK | GIMP_CONTEXT_PROP_MASK_PATTERN |
          GIMP_CONTEXT_PROP_MASK_GRADIENT, /* for stroking */
      "gimp-vector-tool", _("Paths"), _("Paths Tool: Create and edit paths"),
      N_("Pat_hs"), "b", NULL, GIMP_HELP_TOOL_PATH, GIMP_ICON_TOOL_PATH, data);
}
...
static void gimp_vector_tool_init(GimpVectorTool *vector_tool) {
...
static void gimp_vector_tool_button_release(GimpTool *tool,
  ...
  if (vector_tool->grab_widget) {
    gimp_tool_widget_button_release(vector_tool->grab_widget, coords, time,
                                    state, release_type);
...
static void gimp_vector_tool_cursor_update(GimpTool *tool,
  ...
  if (display != tool->display || !vector_tool->widget) {
    ...
    if (gimp_image_pick_vectors(
            gimp_display_get_image(display), coords->x, coords->y,
            FUNSCALEX(shell, GIMP_TOOL_HANDLE_SIZE_CIRCLE / 2),
            FUNSCALEY(shell, GIMP_TOOL_HANDLE_SIZE_CIRCLE / 2))) {
...
static void gimp_vector_tool_start(GimpVectorTool *vector_tool,
  ...
  gimp_tool_path_set_vectors(GIMP_TOOL_PATH(widget), vector_tool->vectors);

  g_signal_connect(widget, "changed", G_CALLBACK(gimp_vector_tool_path_changed),
                   vector_tool);
  g_signal_connect(widget, "begin-change",
                   G_CALLBACK(gimp_vector_tool_path_begin_change), vector_tool);
  g_signal_connect(widget, "end-change",
                   G_CALLBACK(gimp_vector_tool_path_end_change), vector_tool);
  g_signal_connect(widget, "activate",
                   G_CALLBACK(gimp_vector_tool_path_activate), vector_tool);
...
static void gimp_vector_tool_path_changed(GimpToolWidget *path,
  ...
  g_object_get(path, "vectors", &vectors, NULL);

  if (vectors != vector_tool->vectors) {
    if (vectors && !gimp_item_is_attached(GIMP_ITEM(vectors))) {
      gimp_image_add_vectors(image, vectors, GIMP_IMAGE_ACTIVE_PARENT, -1,
                             TRUE);
...
void gimp_vector_tool_set_vectors(GimpVectorTool *vector_tool,
  ...
  if (vector_tool->vectors) {
    ...
    g_signal_handlers_disconnect_by_func(
        old_image, gimp_vector_tool_vectors_changed, vector_tool);
    g_signal_handlers_disconnect_by_func(
        vector_tool->vectors, gimp_vector_tool_vectors_removed, vector_tool);
    ...
    if (options->to_selection_button) {
      ...
      g_signal_handlers_disconnect_by_func(options->to_selection_button,
                                           gimp_vector_tool_to_selection, tool);
      g_signal_handlers_disconnect_by_func(
          options->to_selection_button, gimp_vector_tool_to_selection_extended,
          tool);
    }
    ...
    if (options->fill_button) {
      ...
      g_signal_handlers_disconnect_by_func(options->fill_button,
                                           gimp_vector_tool_fill_vectors, tool);
    }

    if (options->stroke_button) {
      gtk_widget_set_sensitive(options->stroke_button, FALSE);
      g_signal_handlers_disconnect_by_func(
          options->stroke_button, gimp_vector_tool_stroke_vectors, tool);
    }
  }

  if (!vectors || (tool->display && gimp_display_get_image(tool->display) !=
  ...
  if (options->to_selection_button) {
    g_signal_connect_swapped(options->to_selection_button, "clicked",
                             G_CALLBACK(gimp_vector_tool_to_selection), tool);
  ...
  if (tool->display) {
    ...
    if (!display ||
      ...
      for (list = gimp_get_display_iter(gimp_item_get_image(item)->gimp); list;
  ...
  if (options->edit_mode != GIMP_VECTOR_MODE_DESIGN)
    g_object_set(options, "vectors-edit-mode", GIMP_VECTOR_MODE_DESIGN, NULL);
}
...
static void gimp_vector_tool_to_selection_extended(GimpVectorTool *vector_tool,
  ...
  gimp_item_to_selection(GIMP_ITEM(vector_tool->vectors),
                         gimp_modifiers_to_channel_op(state), TRUE, FALSE, 0,
                         0);
  gimp_image_flush(image);
}
...
static void gimp_vector_tool_fill_vectors(GimpVectorTool *vector_tool,
  ...
  if (!drawables) {
    gimp_tool_message(GIMP_TOOL(vector_tool), GIMP_TOOL(vector_tool)->display,
                      _("There are no selected layers or channels to fill."));
    return;
  }

  dialog = fill_dialog_new(GIMP_ITEM(vector_tool->vectors), drawables,
                           GIMP_CONTEXT(GIMP_TOOL_GET_OPTIONS(vector_tool)),
                           _("Fill Path"), GIMP_ICON_TOOL_BUCKET_FILL,
                           GIMP_HELP_PATH_FILL, button, config->fill_options,
                           gimp_vector_tool_fill_callback, vector_tool);
...
static void gimp_vector_tool_fill_callback(GtkWidget *dialog, GimpItem *item,
  ...
  gimp_config_sync(G_OBJECT(options), G_OBJECT(config->fill_options), 0);

  if (!gimp_item_fill(item, drawables, options, TRUE, NULL, &error)) {
    gimp_message_literal(context->gimp, G_OBJECT(dialog), GIMP_MESSAGE_WARNING,
...
static void gimp_vector_tool_stroke_vectors(GimpVectorTool *vector_tool,
  ...
  if (!drawables) {
    gimp_tool_message(
        GIMP_TOOL(vector_tool), GIMP_TOOL(vector_tool)->display,
        _("There are no selected layers or channels to stroke to."));
    return;
  }

  dialog = stroke_dialog_new(
      GIMP_ITEM(vector_tool->vectors), drawables,
      GIMP_CONTEXT(GIMP_TOOL_GET_OPTIONS(vector_tool)), _("Stroke Path"),
      GIMP_ICON_PATH_STROKE, GIMP_HELP_PATH_STROKE, button,
      config->stroke_options, gimp_vector_tool_stroke_callback, vector_tool);
  gtk_widget_show(dialog);
  g_list_free(drawables);
}
...
static void gimp_vector_tool_stroke_callback(GtkWidget *dialog, GimpItem *item,
  ...
  gimp_config_sync(G_OBJECT(options), G_OBJECT(config->stroke_options), 0);

  if (!gimp_item_stroke(item, drawables, context, options, NULL, TRUE, NULL,
                        &error)) {
    gimp_message_literal(context->gimp, G_OBJECT(dialog), GIMP_MESSAGE_WARNING,
                         error ? error->message : "NULL");
