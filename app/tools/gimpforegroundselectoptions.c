...
static void gimp_foreground_select_options_class_init(
    GimpForegroundSelectOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(
      object_class, PROP_DRAW_MODE, "draw-mode", _("Draw Mode"),
      _("Paint over areas to mark color values for "
        "inclusion or exclusion from selection"),
      GIMP_TYPE_MATTING_DRAW_MODE, GIMP_MATTING_DRAW_MODE_FOREGROUND,
      GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(
      object_class, PROP_PREVIEW_MODE, "preview-mode", _("Preview Mode"),
      _("Preview Mode"), GIMP_TYPE_MATTING_PREVIEW_MODE,
      GIMP_MATTING_PREVIEW_MODE_ON_COLOR, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_INT(object_class, PROP_STROKE_WIDTH, "stroke-width",
                       _("Stroke width"),
                       _("Size of the brush used for refinements"), 1, 6000, 10,
                       GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_RGB(object_class, PROP_MASK_COLOR, "mask-color",
                       _("Preview color"), _("Color of selection preview mask"),
                       GIMP_TYPE_RGB, &blue, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_ENGINE, "engine", _("Engine"),
                        _("Matting engine to use"), GIMP_TYPE_MATTING_ENGINE,
                        GIMP_MATTING_ENGINE_LEVIN, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_INT(object_class, PROP_LEVELS, "levels", _("Levels"),
                       _("Number of downsampled levels to use"), 1, 10, 2,
                       GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_INT(object_class, PROP_ACTIVE_LEVELS, "active-levels",
                       _("Active levels"),
                       _("Number of levels to perform solving"), 1, 10, 2,
                       GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_INT(object_class, PROP_ITERATIONS, "iterations",
                       _("Iterations"), _("Number of iterations to perform"), 1,
                       10, 2, GIMP_PARAM_STATIC_STRINGS);
}
...
static void gimp_foreground_select_options_reset_stroke_width(
    GtkWidget *button, GimpToolOptions *tool_options) {
  g_object_set(tool_options, "stroke-width", 10, NULL);
}

static gboolean gimp_foreground_select_options_sync_engine(
    GBinding *binding, const GValue *source_value, GValue *target_value,
    gpointer user_data) {
  gint type = g_value_get_enum(source_value);

  g_value_set_boolean(target_value, type == GPOINTER_TO_INT(user_data));
...
GtkWidget *gimp_foreground_select_options_gui(GimpToolOptions *tool_options) {
  ...
  frame = gimp_prop_enum_radio_frame_new(config, "draw-mode", NULL, 0, 0);
  ...
  frame = gimp_prop_enum_radio_frame_new(config, "preview-mode", NULL, 0, 0);
  gtk_box_pack_start(GTK_BOX(vbox), frame, TRUE, TRUE, 0);

  /*  mask color */
  button = gimp_prop_color_button_new(config, "mask-color", NULL, 128, 24,
  ...
  /*  engine parameters  */
  scale = gimp_prop_spin_scale_new(config, "levels", NULL, 1.0, 1.0, 0);
  gtk_box_pack_start(GTK_BOX(inner_vbox), scale, FALSE, FALSE, 0);

  g_object_bind_property_full(config, "engine", scale, "visible",
                              G_BINDING_SYNC_CREATE,
                              gimp_foreground_select_options_sync_engine, NULL,
                              GINT_TO_POINTER(GIMP_MATTING_ENGINE_LEVIN), NULL);

  scale = gimp_prop_spin_scale_new(config, "active-levels", NULL, 1.0, 1.0, 0);
  gtk_box_pack_start(GTK_BOX(inner_vbox), scale, FALSE, FALSE, 0);
  ...
  scale = gimp_prop_spin_scale_new(config, "iterations", NULL, 1.0, 1.0, 0);
  gtk_box_pack_start(GTK_BOX(inner_vbox), scale, FALSE, FALSE, 0);

  g_object_bind_property_full(
      config, "engine", scale, "visible", G_BINDING_SYNC_CREATE,
      gimp_foreground_select_options_sync_engine, NULL,
      GINT_TO_POINTER(GIMP_MATTING_ENGINE_GLOBAL), NULL);
