...
static void gimp_n_point_deformation_options_class_init(
  ...
  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_SQUARE_SIZE, "square-size",
                          _("Density"), _("Density"), 5.0, 1000.0, 20.0,
                          GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_RIGIDITY, "rigidity",
                          _("Rigidity"), _("Rigidity"), 1.0, 10000.0, 100.0,
                          GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_ASAP_DEFORMATION,
                           "asap-deformation", _("Deformation mode"),
                           _("Deformation mode"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_MLS_WEIGHTS, "mls-weights",
                           _("Use weights"), _("Use weights"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_MLS_WEIGHTS_ALPHA,
                          "mls-weights-alpha", _("Control points influence"),
                          _("Amount of control points' influence"), 0.1, 2.0,
                          1.0, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_MESH_VISIBLE, "mesh-visible",
                           _("Show lattice"), _("Show lattice"), TRUE,
                           GIMP_PARAM_STATIC_STRINGS);
}
...
GtkWidget *gimp_n_point_deformation_options_gui(GimpToolOptions *tool_options) {
  ...
  widget = gimp_prop_spin_scale_new(config, "square-size", NULL, 1.0, 10.0, 0);
  npd_options->scale_square_size = widget;
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(widget), 10.0, 100.0);
  gtk_box_pack_start(GTK_BOX(vbox), widget, FALSE, FALSE, 0);
  gtk_widget_set_can_focus(widget, FALSE);

  widget = gimp_prop_spin_scale_new(config, "rigidity", NULL, 1.0, 10.0, 0);
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(widget), 1.0, 2000.0);
  gtk_box_pack_start(GTK_BOX(vbox), widget, FALSE, FALSE, 0);
  gtk_widget_set_can_focus(widget, FALSE);

  widget = gimp_prop_boolean_radio_frame_new(config, "asap-deformation", NULL,
  ...
  widget =
      gimp_prop_spin_scale_new(config, "mls-weights-alpha", NULL, 0.1, 0.1, 1);
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(widget), 0.1, 2.0);
  gtk_box_pack_start(GTK_BOX(vbox), widget, FALSE, FALSE, 0);
  gtk_widget_set_can_focus(widget, FALSE);

  g_object_bind_property(config, "mls-weights", widget, "sensitive",
...
void gimp_n_point_deformation_options_set_sensitivity(
    GimpNPointDeformationOptions *npd_options, gboolean tool_active) {
