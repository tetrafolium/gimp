...
static gboolean gimp_draw_tool_has_display(GimpTool *tool,
                                           GimpDisplay *display);
static GimpDisplay *gimp_draw_tool_has_image(GimpTool *tool, GimpImage *image);
static void gimp_draw_tool_control(GimpTool *tool, GimpToolAction action,
                                   GimpDisplay *display);
static gboolean gimp_draw_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
                                         GimpDisplay *display);
static gboolean gimp_draw_tool_key_release(GimpTool *tool, GdkEventKey *kevent,
                                           GimpDisplay *display);
static void gimp_draw_tool_modifier_key(GimpTool *tool, GdkModifierType key,
                                        gboolean press, GdkModifierType state,
                                        GimpDisplay *display);
static void gimp_draw_tool_active_modifier_key(GimpTool *tool,
                                               GdkModifierType key,
                                               gboolean press,
                                               GdkModifierType state,
                                               GimpDisplay *display);
static void gimp_draw_tool_oper_update(GimpTool *tool, const GimpCoords *coords,
                                       GdkModifierType state,
                                       gboolean proximity,
                                       GimpDisplay *display);
...
static void gimp_draw_tool_widget_status(GimpToolWidget *widget,
                                         const gchar *status, GimpTool *tool);
static void gimp_draw_tool_widget_status_coords(GimpToolWidget *widget,
                                                const gchar *title, gdouble x,
                                                const gchar *separator,
                                                gdouble y, const gchar *help,
                                                GimpTool *tool);
static void gimp_draw_tool_widget_message(GimpToolWidget *widget,
                                          const gchar *message, GimpTool *tool);
static void gimp_draw_tool_widget_snap_offsets(GimpToolWidget *widget,
...
static void gimp_draw_tool_active_modifier_key(GimpTool *tool,
  ...
  GIMP_TOOL_CLASS(parent_class)
      ->active_modifier_key(tool, key, press, state, display);
}
...
static void gimp_draw_tool_cursor_update(GimpTool *tool,
  ...
  if (draw_tool->widget && display == draw_tool->display) {
    ...
    if (gimp_tool_widget_get_cursor(draw_tool->widget, coords, state, &cursor,
                                    &tool_cursor, &modifier)) {
      gimp_tool_set_cursor(tool, display, cursor, tool_cursor, modifier);
      return;
    }
  }
...
static GimpUIManager *gimp_draw_tool_get_popup(GimpTool *tool,
  ...
  if (draw_tool->widget && display == draw_tool->display) {
    ...
    ui_manager =
        gimp_tool_widget_get_popup(draw_tool->widget, coords, state, ui_path);
  ...
  return GIMP_TOOL_CLASS(parent_class)
      ->get_popup(tool, coords, state, display, ui_path);
}
...
static void gimp_draw_tool_widget_snap_offsets(GimpToolWidget *widget,
                                               ...
                                               gint width, gint height,
                                               GimpTool *tool) {
  gimp_tool_control_set_snap_offsets(tool->control, offset_x, offset_y, width,
                                     height);
}

#ifdef USE_TIMEOUT
static gboolean gimp_draw_tool_draw_timeout(GimpDrawTool *draw_tool) {
  ...
  if (draw_tool->display && draw_tool->paused_count == 0 &&
    ...
    if (draw_tool->group_stack) {
      g_warning("%s: draw_tool->group_stack not empty after calling "
                ...
                G_STRFUNC, g_type_name(G_TYPE_FROM_INSTANCE(draw_tool)));
...
void gimp_draw_tool_resume(GimpDrawTool *draw_tool) {
  ...
  if (draw_tool->paused_count == 0) {
    ...
    if (gimp_draw_tool_is_active(draw_tool) && !draw_tool->draw_timeout) {
      draw_tool->draw_timeout = gdk_threads_add_timeout_full(
          G_PRIORITY_HIGH_IDLE, DRAW_TIMEOUT,
          (GSourceFunc)gimp_draw_tool_draw_timeout, draw_tool, NULL);
    }
#endif
...
gdouble gimp_draw_tool_calc_distance(GimpDrawTool *draw_tool,
                                     GimpDisplay *display, gdouble x1,
                                     gdouble y1, gdouble x2, gdouble y2) {
  return sqrt(
      gimp_draw_tool_calc_distance_square(draw_tool, display, x1, y1, x2, y2));
}
...
void gimp_draw_tool_set_widget(GimpDrawTool *draw_tool,
  ...
  if (draw_tool->widget) {
    ...
    g_signal_handlers_disconnect_by_func(
        draw_tool->widget, gimp_draw_tool_widget_status, draw_tool);
    g_signal_handlers_disconnect_by_func(
        draw_tool->widget, gimp_draw_tool_widget_status_coords, draw_tool);
    g_signal_handlers_disconnect_by_func(
        draw_tool->widget, gimp_draw_tool_widget_message, draw_tool);
    g_signal_handlers_disconnect_by_func(
        draw_tool->widget, gimp_draw_tool_widget_snap_offsets, draw_tool);
  ...
  if (draw_tool->widget) {
    ...
    g_signal_connect(draw_tool->widget, "status",
                     G_CALLBACK(gimp_draw_tool_widget_status), draw_tool);
    ...
    g_signal_connect(draw_tool->widget, "message",
                     G_CALLBACK(gimp_draw_tool_widget_message), draw_tool);
    g_signal_connect(draw_tool->widget, "snap-offsets",
                     G_CALLBACK(gimp_draw_tool_widget_snap_offsets), draw_tool);
...
void gimp_draw_tool_add_preview(GimpDrawTool *draw_tool, GimpCanvasItem *item) {
  g_return_if_fail(GIMP_IS_DRAW_TOOL(draw_tool));
...
void gimp_draw_tool_add_item(GimpDrawTool *draw_tool, GimpCanvasItem *item) {
...
void gimp_draw_tool_remove_item(GimpDrawTool *draw_tool, GimpCanvasItem *item) {
  g_return_if_fail(GIMP_IS_DRAW_TOOL(draw_tool));
  g_return_if_fail(GIMP_IS_CANVAS_ITEM(item));
...
void gimp_draw_tool_pop_group(GimpDrawTool *draw_tool) {
  ...
  draw_tool->group_stack =
      g_list_remove(draw_tool->group_stack, draw_tool->group_stack->data);
}
 ...
 * coordinates.
 **/
GimpCanvasItem *gimp_draw_tool_add_line(GimpDrawTool *draw_tool, gdouble x1,
                                        gdouble y1, gdouble x2, gdouble y2) {
  GimpCanvasItem *item;

  g_return_val_if_fail(GIMP_IS_DRAW_TOOL(draw_tool), NULL);

  item = gimp_canvas_line_new(gimp_display_get_shell(draw_tool->display), x1,
                              y1, x2, y2);
...
GimpCanvasItem *gimp_draw_tool_add_guide(GimpDrawTool *draw_tool,
                                         ...
                                         gint position, GimpGuideStyle style) {
  GimpCanvasItem *item;
...
GimpCanvasItem *gimp_draw_tool_add_crosshair(GimpDrawTool *draw_tool,
                                             gint position_x, gint position_y) {
  GimpCanvasGroup *group;

  group = gimp_draw_tool_add_stroke_group(draw_tool);

  gimp_draw_tool_push_group(draw_tool, group);
  gimp_draw_tool_add_guide(draw_tool, GIMP_ORIENTATION_VERTICAL, position_x,
                           GIMP_GUIDE_STYLE_NONE);
  gimp_draw_tool_add_guide(draw_tool, GIMP_ORIENTATION_HORIZONTAL, position_y,
                           GIMP_GUIDE_STYLE_NONE);
  gimp_draw_tool_pop_group(draw_tool);
 ...
 **/
GimpCanvasItem *gimp_draw_tool_add_sample_point(GimpDrawTool *draw_tool, gint x,
                                                gint y, gint index) {
  GimpCanvasItem *item;

  g_return_val_if_fail(GIMP_IS_DRAW_TOOL(draw_tool), NULL);

  item = gimp_canvas_sample_point_new(
      gimp_display_get_shell(draw_tool->display), x, y, index, TRUE);
...
GimpCanvasItem *gimp_draw_tool_add_arc(GimpDrawTool *draw_tool, gboolean filled,
                                       gdouble x, gdouble y, gdouble width,
                                       gdouble height, gdouble start_angle,
                                       gdouble slice_angle) {
  GimpCanvasItem *item;
  ...
  item = gimp_canvas_arc_new(gimp_display_get_shell(draw_tool->display),
                             ...
                             height / 2.0, start_angle, slice_angle, filled);
...
GimpCanvasItem *gimp_draw_tool_add_lines(GimpDrawTool *draw_tool,
                                         ...
                                         gint n_points, GimpMatrix3 *transform,
                                         gboolean filled) {
  GimpCanvasItem *item;
...
GimpCanvasItem *gimp_draw_tool_add_strokes(GimpDrawTool *draw_tool,
  ...
  item = gimp_canvas_polygon_new_from_coords(
      gimp_display_get_shell(draw_tool->display), points, n_points, transform,
      filled);
...
GimpCanvasItem *gimp_draw_tool_add_pen(GimpDrawTool *draw_tool,
                                       ...
                                       GimpContext *context,
                                       GimpActiveColor color, gint width) {
  GimpCanvasItem *item;
  ...
  item = gimp_canvas_pen_new(gimp_display_get_shell(draw_tool->display), points,
                             n_points, context, color, width);
 ...
 */
GimpCanvasItem *gimp_draw_tool_add_boundary(
    GimpDrawTool *draw_tool, const GimpBoundSeg *bound_segs, gint n_bound_segs,
    GimpMatrix3 *transform, gdouble offset_x, gdouble offset_y) {
  GimpCanvasItem *item;
  ...
  item = gimp_canvas_boundary_new(gimp_display_get_shell(draw_tool->display),
                                  bound_segs, n_bound_segs, transform, offset_x,
                                  offset_y);
...
gimp_draw_tool_add_transform_preview(GimpDrawTool *draw_tool,
                                     ...
                                     const GimpMatrix3 *transform, gdouble x1,
                                     gdouble y1, gdouble x2, gdouble y2) {
  GimpCanvasItem *item;
  ...
  item = gimp_canvas_transform_preview_new(
      gimp_display_get_shell(draw_tool->display), pickable, transform, x1, y1,
      x2, y2);
...
gboolean gimp_draw_tool_on_handle(GimpDrawTool *draw_tool, GimpDisplay *display,
                                  gdouble x, gdouble y, GimpHandleType type,
                                  gdouble handle_x, gdouble handle_y,
                                  gint width, gint height,
                                  GimpHandleAnchor anchor) {
  GimpDisplayShell *shell;
  gdouble tx, ty;
  ...
  gimp_display_shell_zoom_xy_f(shell, x, y, &tx, &ty);
  gimp_display_shell_zoom_xy_f(shell, handle_x, handle_y, &handle_tx,
                               &handle_ty);
  ...
  case GIMP_HANDLE_CROSSHAIR:
    gimp_canvas_item_shift_to_north_west(anchor, handle_tx, handle_ty, width,
                                         height, &handle_tx, &handle_ty);
  ...
  case GIMP_HANDLE_FILLED_CIRCLE:
    gimp_canvas_item_shift_to_center(anchor, handle_tx, handle_ty, width,
                                     height, &handle_tx, &handle_ty);
