...
static gboolean gimp_rotate_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
                                           GimpDisplay *display);
...
void gimp_rotate_tool_register(GimpToolRegisterCallback callback,
                               gpointer data) {
  (*callback)(
      GIMP_TYPE_ROTATE_TOOL, GIMP_TYPE_TRANSFORM_GRID_OPTIONS,
      gimp_transform_grid_options_gui, GIMP_CONTEXT_PROP_MASK_BACKGROUND,
      "gimp-rotate-tool", _("Rotate"),
      _("Rotate Tool: Rotate the layer, selection or path"), N_("_Rotate"),
      "<shift>R", NULL, GIMP_HELP_TOOL_ROTATE, GIMP_ICON_TOOL_ROTATE, data);
}
...
static gboolean gimp_rotate_tool_info_to_matrix(GimpTransformGridTool *tg_tool,
  ...
  gimp_transform_matrix_rotate_center(transform, tg_tool->trans_info[PIVOT_X],
...
static void gimp_rotate_tool_matrix_to_info(GimpTransformGridTool *tg_tool,
  ...
  if (fabs(q) > EPSILON) {
    ...
    gimp_matrix3_transform_point(
        &transfer, tg_tool->trans_info[PIVOT_X], tg_tool->trans_info[PIVOT_Y],
...
static gchar *gimp_rotate_tool_get_undo_desc(GimpTransformGridTool *tg_tool) {
  ...
  if (fabs(tg_tool->trans_info[PIVOT_X] - center_x) <= EPSILON &&
    ...
    return g_strdup_printf(C_("undo-type", "Rotate by %-3.3g°"),
                           gimp_rad_to_deg(tg_tool->trans_info[ANGLE]));
  } else {
    return g_strdup_printf(C_("undo-type", "Rotate by %-3.3g° around (%g, %g)"),
                           gimp_rad_to_deg(tg_tool->trans_info[ANGLE]),
...
static void gimp_rotate_tool_dialog(GimpTransformGridTool *tg_tool) {
  ...
  gtk_box_pack_start(GTK_BOX(gimp_tool_gui_get_vbox(tg_tool->gui)), grid, FALSE,
                     FALSE, 0);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("_Angle:"), 0.0, 0.5, button,
                           1);
  rotate->angle_spin_button = button;

  g_signal_connect(rotate->angle_adj, "value-changed",
                   G_CALLBACK(rotate_angle_changed), tg_tool);
  ...
  gtk_entry_set_width_chars(GTK_ENTRY(button), SB_WIDTH);
  gtk_widget_set_halign(button, GTK_ALIGN_START);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 2, _("Center _X:"), 0.0, 0.5,
                           button, 1);

  rotate->sizeentry =
      gimp_size_entry_new(1, GIMP_UNIT_PIXEL, "%a", TRUE, TRUE, FALSE, SB_WIDTH,
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 3, _("Center _Y:"), 0.0, 0.5,
                           rotate->sizeentry, 1);

  g_signal_connect(rotate->sizeentry, "value-changed",
                   G_CALLBACK(rotate_center_changed), tg_tool);
  ...
  g_signal_connect(rotate->pivot_selector, "changed",
                   G_CALLBACK(rotate_pivot_changed), tg_tool);
}
...
static void gimp_rotate_tool_dialog_update(GimpTransformGridTool *tg_tool) {
  ...
  g_signal_handlers_block_by_func(rotate->sizeentry, rotate_center_changed,
                                  tg_tool);
  ...
  g_signal_handlers_unblock_by_func(rotate->sizeentry, rotate_center_changed,
                                    tg_tool);

  g_signal_handlers_block_by_func(rotate->pivot_selector, rotate_pivot_changed,
                                  tg_tool);

  gimp_pivot_selector_set_position(GIMP_PIVOT_SELECTOR(rotate->pivot_selector),
                                   tg_tool->trans_info[PIVOT_X],
                                   tg_tool->trans_info[PIVOT_Y]);

  g_signal_handlers_unblock_by_func(rotate->pivot_selector,
                                    rotate_pivot_changed, tg_tool);
}
...
static void gimp_rotate_tool_prepare(GimpTransformGridTool *tg_tool) {
  ...
  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(rotate->sizeentry), 0, xres,
                                 FALSE);
  gimp_size_entry_set_resolution(GIMP_SIZE_ENTRY(rotate->sizeentry), 1, yres,
                                 FALSE);
  ...
  gimp_size_entry_set_size(GIMP_SIZE_ENTRY(rotate->sizeentry), 0, tr_tool->x1,
                           tr_tool->x2);
  gimp_size_entry_set_size(GIMP_SIZE_ENTRY(rotate->sizeentry), 1, tr_tool->y1,
                           tr_tool->y2);
...
static void gimp_rotate_tool_readjust(GimpTransformGridTool *tg_tool) {
  ...
  gimp_display_shell_untransform_xy_f(
      shell, shell->disp_width / 2.0, shell->disp_height / 2.0,
      &tg_tool->trans_info[PIVOT_X], &tg_tool->trans_info[PIVOT_Y]);
}
...
gimp_rotate_tool_get_widget(GimpTransformGridTool *tg_tool) {
  ...
  widget = gimp_tool_rotate_grid_new(
      shell, tr_tool->x1, tr_tool->y1, tr_tool->x2, tr_tool->y2,
      tg_tool->trans_info[PIVOT_X], tg_tool->trans_info[PIVOT_Y],
      tg_tool->trans_info[ANGLE]);

  g_object_set(widget, "inside-function", GIMP_TRANSFORM_FUNCTION_ROTATE,
...
static void gimp_rotate_tool_update_widget(GimpTransformGridTool *tg_tool) {
  ...
  g_object_set(tg_tool->widget, "angle", tg_tool->trans_info[ANGLE], "pivot-x",
               tg_tool->trans_info[PIVOT_X], "pivot-y",
               tg_tool->trans_info[PIVOT_Y], NULL);
}

static void gimp_rotate_tool_widget_changed(GimpTransformGridTool *tg_tool) {
  g_object_get(tg_tool->widget, "angle", &tg_tool->trans_info[ANGLE], "pivot-x",
               &tg_tool->trans_info[PIVOT_X], "pivot-y",
               &tg_tool->trans_info[PIVOT_Y], NULL);
...
static void rotate_center_changed(GtkWidget *widget,
                                  GimpTransformGridTool *tg_tool) {
...
static void rotate_pivot_changed(GimpPivotSelector *selector,
  ...
  gimp_pivot_selector_get_position(selector, &tg_tool->trans_info[PIVOT_X],
                                   &tg_tool->trans_info[PIVOT_Y]);
