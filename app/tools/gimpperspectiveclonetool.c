...
enum { X0, Y0, X1, Y1, X2, Y2, X3, Y3, PIVOT_X, PIVOT_Y };
...
static void gimp_perspective_clone_tool_button_press(
    ...
    GimpDisplay *display);
static void gimp_perspective_clone_tool_button_release(
    GimpTool *tool, const GimpCoords *coords, guint32 time,
...
static void gimp_perspective_clone_tool_widget_changed(
    GimpToolWidget *widget, GimpPerspectiveCloneTool *clone_tool);
static void
...
void gimp_perspective_clone_tool_register(GimpToolRegisterCallback callback,
                                          gpointer data) {
  (*callback)(
      GIMP_TYPE_PERSPECTIVE_CLONE_TOOL, GIMP_TYPE_PERSPECTIVE_CLONE_OPTIONS,
      gimp_perspective_clone_options_gui,
      GIMP_PAINT_OPTIONS_CONTEXT_MASK | GIMP_CONTEXT_PROP_MASK_PATTERN,
      "gimp-perspective-clone-tool", _("Perspective Clone"),
      _("Perspective Clone Tool: Clone from an image source "
        "after applying a perspective transformation"),
      N_("_Perspective Clone"), NULL, NULL, GIMP_HELP_TOOL_PERSPECTIVE_CLONE,
      GIMP_ICON_TOOL_PERSPECTIVE_CLONE, data);
}
...
static gboolean gimp_perspective_clone_tool_initialize(GimpTool *tool,
  ...
  if (display != tool->display) {
    ...
    clone_tool->widget = gimp_tool_transform_grid_new(
        shell, &clone_tool->transform, clone_tool->x1, clone_tool->y1,
        clone_tool->x2, clone_tool->y2);

    g_object_set(clone_tool->widget, "pivot-x",
                 (clone_tool->x1 + clone_tool->x2) / 2.0, "pivot-y",
                 (clone_tool->y1 + clone_tool->y2) / 2.0, "inside-function",
...
static void gimp_perspective_clone_tool_button_press(
  ...
  if (options->clone_mode == GIMP_PERSPECTIVE_CLONE_MODE_ADJUST) {
    if (clone_tool->widget) {
      ...
      if (gimp_tool_widget_button_press(clone_tool->widget, coords, time, state,
                                        press_type)) {
    ...
    gimp_perspective_clone_get_source_point(clone, coords->x, coords->y, &nnx,
                                            &nny);
...
static void gimp_perspective_clone_tool_button_release(
  ...
  case GIMP_PERSPECTIVE_CLONE_MODE_ADJUST:
    ...
    if (clone_tool->grab_widget) {
      gimp_tool_widget_button_release(clone_tool->grab_widget, coords, time,
                                      state, release_type);
...
static void gimp_perspective_clone_tool_cursor_update(GimpTool *tool,
  ...
  if (gimp_image_coords_in_active_pickable(image, coords, FALSE, FALSE, TRUE)) {
  ...
  if (options->clone_mode == GIMP_PERSPECTIVE_CLONE_MODE_ADJUST) {
    if (clone_tool->widget) {
      if (display == tool->display) {
        gimp_tool_widget_get_cursor(clone_tool->widget, coords, state, &cursor,
...
static void gimp_perspective_clone_tool_oper_update(GimpTool *tool,
  ...
  if (options->clone_mode == GIMP_PERSPECTIVE_CLONE_MODE_ADJUST) {
    if (clone_tool->widget) {
      if (display == tool->display) {
        gimp_tool_widget_hover(clone_tool->widget, coords, state, proximity);
      }
    }
  } else {
    GIMP_TOOL_CLASS(parent_class)
      ...
      if (source_core->src_drawable == NULL) {
        ...
        if (!source_core->first_stroke) {
          if (GIMP_SOURCE_OPTIONS(options)->align_mode ==
            ...
            /* Set the coordinates for the reference cross */
            gimp_perspective_clone_get_source_point(clone, coords->x, coords->y,
                                                    &nnx, &nny);
...
static void gimp_perspective_clone_tool_options_notify(
...
static void gimp_perspective_clone_tool_draw(GimpDrawTool *draw_tool) {
  ...
  GimpPerspectiveClone *clone =
      GIMP_PERSPECTIVE_CLONE(GIMP_PAINT_TOOL(tool)->core);
  GimpSourceCore *source_core = GIMP_SOURCE_CORE(clone);
  GimpPerspectiveCloneOptions *options;
  ...
  if (options->clone_mode == GIMP_PERSPECTIVE_CLONE_MODE_ADJUST) {
    ...
    gimp_draw_tool_add_line(
        draw_tool, clone_tool->trans_info[X0], clone_tool->trans_info[Y0],
        clone_tool->trans_info[X1], clone_tool->trans_info[Y1]);
    gimp_draw_tool_add_line(
        draw_tool, clone_tool->trans_info[X1], clone_tool->trans_info[Y1],
        clone_tool->trans_info[X3], clone_tool->trans_info[Y3]);
    gimp_draw_tool_add_line(
        draw_tool, clone_tool->trans_info[X2], clone_tool->trans_info[Y2],
        clone_tool->trans_info[X3], clone_tool->trans_info[Y3]);
    gimp_draw_tool_add_line(
        draw_tool, clone_tool->trans_info[X2], clone_tool->trans_info[Y2],
        clone_tool->trans_info[X0], clone_tool->trans_info[Y0]);
  ...
  if (source_core->src_drawable && clone_tool->src_display) {
    ...
    gimp_draw_tool_add_handle(
        draw_tool, GIMP_HANDLE_CROSS, clone_tool->src_x + 0.5,
        clone_tool->src_y + 0.5, GIMP_TOOL_HANDLE_SIZE_CROSS,
        GIMP_TOOL_HANDLE_SIZE_CROSS, GIMP_HANDLE_ANCHOR_CENTER);
...
gimp_perspective_clone_tool_halt(GimpPerspectiveCloneTool *clone_tool) {
  ...
  g_object_set(GIMP_PAINT_TOOL(tool)->core, "src-drawable", NULL, NULL);
...
static void
gimp_perspective_clone_tool_prepare(GimpPerspectiveCloneTool *clone_tool) {
  clone_tool->trans_info[PIVOT_X] =
      (gdouble)(clone_tool->x1 + clone_tool->x2) / 2.0;
  clone_tool->trans_info[PIVOT_Y] =
      (gdouble)(clone_tool->y1 + clone_tool->y2) / 2.0;
...
gimp_perspective_clone_tool_recalc_matrix(GimpPerspectiveCloneTool *clone_tool,
  ...
  gimp_transform_matrix_perspective(
      &clone_tool->transform, clone_tool->x1, clone_tool->y1,
      clone_tool->x2 - clone_tool->x1, clone_tool->y2 - clone_tool->y1,
      clone_tool->trans_info[X0], clone_tool->trans_info[Y0],
      clone_tool->trans_info[X1], clone_tool->trans_info[Y1],
      clone_tool->trans_info[X2], clone_tool->trans_info[Y2],
      clone_tool->trans_info[X3], clone_tool->trans_info[Y3]);

  if (widget)
    g_object_set(widget, "transform", &clone_tool->transform, "x1",
                 (gdouble)clone_tool->x1, "y1", (gdouble)clone_tool->y1, "x2",
                 (gdouble)clone_tool->x2, "y2", (gdouble)clone_tool->y2,
                 "pivot-x", clone_tool->trans_info[PIVOT_X], "pivot-y",
                 clone_tool->trans_info[PIVOT_Y], NULL);
}
...
static void gimp_perspective_clone_tool_widget_changed(
  ...
  g_object_get(widget, "transform", &transform, "pivot-x",
               &clone_tool->trans_info[PIVOT_X], "pivot-y",
               &clone_tool->trans_info[PIVOT_Y], NULL);

  gimp_matrix3_transform_point(transform, clone_tool->x1, clone_tool->y1,
                               &clone_tool->trans_info[X0],
                               &clone_tool->trans_info[Y0]);
  gimp_matrix3_transform_point(transform, clone_tool->x2, clone_tool->y1,
                               &clone_tool->trans_info[X1],
                               &clone_tool->trans_info[Y1]);
  gimp_matrix3_transform_point(transform, clone_tool->x1, clone_tool->y2,
                               &clone_tool->trans_info[X2],
                               &clone_tool->trans_info[Y2]);
  gimp_matrix3_transform_point(transform, clone_tool->x2, clone_tool->y2,
...
static void gimp_perspective_clone_tool_widget_status(
    GimpToolWidget *widget, const gchar *status,
    GimpPerspectiveCloneTool *clone_tool) {
  GimpTool *tool = GIMP_TOOL(clone_tool);
