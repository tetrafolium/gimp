...
static void gimp_warp_tool_control(GimpTool *tool, GimpToolAction action,
                                   GimpDisplay *display);
...
static void gimp_warp_tool_button_release(GimpTool *tool,
                                          ...
                                          GimpDisplay *display);
static void gimp_warp_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                  guint32 time, GdkModifierType state,
                                  GimpDisplay *display);
static gboolean gimp_warp_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
                                         GimpDisplay *display);
static void gimp_warp_tool_oper_update(GimpTool *tool, const GimpCoords *coords,
                                       GdkModifierType state,
                                       gboolean proximity,
                                       GimpDisplay *display);
static void gimp_warp_tool_cursor_update(GimpTool *tool,
                                         ...
                                         GdkModifierType state,
                                         GimpDisplay *display);
const gchar *gimp_warp_tool_can_undo(GimpTool *tool, GimpDisplay *display);
const gchar *gimp_warp_tool_can_redo(GimpTool *tool, GimpDisplay *display);
static gboolean gimp_warp_tool_undo(GimpTool *tool, GimpDisplay *display);
static gboolean gimp_warp_tool_redo(GimpTool *tool, GimpDisplay *display);
...
static void gimp_warp_tool_cursor_notify(GimpDisplayConfig *config,
                                         GParamSpec *pspec, GimpWarpTool *wt);

static gboolean gimp_warp_tool_can_stroke(GimpWarpTool *wt,
                                          GimpDisplay *display,
                                          gboolean show_message);

static gboolean gimp_warp_tool_start(GimpWarpTool *wt, GimpDisplay *display);
...
static void gimp_warp_tool_set_sampler(GimpWarpTool *wt, gboolean commit);
static GeglRectangle gimp_warp_tool_get_stroke_bounds(GeglNode *node);
...
static void gimp_warp_tool_update_stroke(GimpWarpTool *wt, GeglNode *node);
static void gimp_warp_tool_stroke_append(GimpWarpTool *wt, gchar type,
                                         gdouble x, gdouble y);
static void gimp_warp_tool_filter_flush(GimpDrawableFilter *filter,
                                        GimpTool *tool);
static void gimp_warp_tool_add_op(GimpWarpTool *wt, GeglNode *op);
...
void gimp_warp_tool_register(GimpToolRegisterCallback callback, gpointer data) {
  (*callback)(GIMP_TYPE_WARP_TOOL, GIMP_TYPE_WARP_OPTIONS,
              gimp_warp_options_gui, 0, "gimp-warp-tool", _("Warp Transform"),
              _("Warp Transform: Deform with different tools"),
              N_("_Warp Transform"), "W", NULL, GIMP_HELP_TOOL_WARP,
              GIMP_ICON_TOOL_WARP, data);
}

static void gimp_warp_tool_class_init(GimpWarpToolClass *klass) {
...
static void gimp_warp_tool_init(GimpWarpTool *self) {
  ...
  gimp_tool_control_set_dirty_mask(
      tool->control, GIMP_DIRTY_IMAGE | GIMP_DIRTY_DRAWABLE |
                         GIMP_DIRTY_SELECTION | GIMP_DIRTY_ACTIVE_DRAWABLE);
  gimp_tool_control_set_dirty_action(tool->control, GIMP_TOOL_ACTION_COMMIT);
  gimp_tool_control_set_wants_click(tool->control, TRUE);
  gimp_tool_control_set_precision(tool->control,
                                  GIMP_CURSOR_PRECISION_SUBPIXEL);
  gimp_tool_control_set_tool_cursor(tool->control, GIMP_TOOL_CURSOR_WARP);
...
static void gimp_warp_tool_constructed(GObject *object) {
  ...
  g_signal_connect_object(display_config, "notify::show-paint-tool-cursor",
                          G_CALLBACK(gimp_warp_tool_cursor_notify), wt, 0);
...
static void gimp_warp_tool_button_press(GimpTool *tool,
  ...
  new_op = gegl_node_new_child(
      NULL, "operation", "gegl:warp", "behavior", options->behavior, "size",
      options->effect_size, "hardness", options->effect_hardness / 100.0,
      "strength", options->effect_strength,
      /* we implement spacing manually.
       * anything > 1 will do.
       */
      "spacing", 10.0, "stroke", wt->current_stroke, NULL);
  ...
  gimp_warp_tool_stroke_append(wt, 'M', wt->last_pos.x - off_x,
                               wt->last_pos.y - off_y);
...
void gimp_warp_tool_button_release(GimpTool *tool, const GimpCoords *coords,
                                   guint32 time, GdkModifierType state,
                                   GimpButtonReleaseType release_type,
                                   GimpDisplay *display) {
  GimpWarpTool *wt = GIMP_WARP_TOOL(tool);
  ...
  if (release_type == GIMP_BUTTON_RELEASE_CANCEL) {
    ...
    if (wt->redo_stack) {
      ...
      g_list_free_full(wt->redo_stack, (GDestroyNotify)gimp_warp_tool_free_op);
      wt->redo_stack = NULL;
    }
...
static void gimp_warp_tool_cursor_update(GimpTool *tool,
  ...
  if (!wt->show_cursor && modifier != GIMP_CURSOR_MODIFIER_BAD) {
    gimp_tool_set_cursor(tool, display, GIMP_CURSOR_NONE, GIMP_TOOL_CURSOR_NONE,
...
static gboolean gimp_warp_tool_undo(GimpTool *tool, GimpDisplay *display) {
  ...
  gegl_node_connect_to(prev_node, "output", wt->render_node, "aux");
...
static gboolean gimp_warp_tool_redo(GimpTool *tool, GimpDisplay *display) {
  ...
  gegl_node_connect_to(to_add, "output", wt->render_node, "aux");
...
static void gimp_warp_tool_options_notify(GimpTool *tool,
  ...
  if (!strcmp(pspec->name, "effect-size")) {
    ...
    if (wt->render_node) {
      gegl_node_set(wt->render_node, "abyss-policy", wt_options->abyss_policy,
                    NULL);
...
static void gimp_warp_tool_draw(GimpDrawTool *draw_tool) {
  ...
  if (wt->draw_brush) {
    gimp_draw_tool_add_arc(draw_tool, FALSE, x - options->effect_size * 0.5,
                           y - options->effect_size * 0.5, options->effect_size,
                           options->effect_size, 0.0, 2.0 * G_PI);
  } else if (!wt->show_cursor) {
    /*  don't leave the user without any indication and draw
     *  a fallback crosshair
     */
    gimp_draw_tool_add_handle(
        draw_tool, GIMP_HANDLE_CROSSHAIR, x, y, GIMP_TOOL_HANDLE_SIZE_CROSSHAIR,
...
static gboolean gimp_warp_tool_can_stroke(GimpWarpTool *wt,
  ...
  if (gimp_viewable_get_children(GIMP_VIEWABLE(drawable))) {
    if (show_message) {
      gimp_tool_message_literal(tool, display, _("Cannot warp layer groups."));
    }
  ...
  if (!options->stroke_during_motion && !options->stroke_periodically) {
    if (show_message) {
      gimp_tool_message_literal(tool, display, _("No stroke events selected."));
...
static gboolean gimp_warp_tool_start(GimpWarpTool *wt, GimpDisplay *display) {
  ...
  gimp_item_mask_intersect(GIMP_ITEM(drawable), &bbox.x, &bbox.y, &bbox.width,
                           &bbox.height);

#ifdef WARP_DEBUG
  g_printerr("Initialize coordinate buffer (%d,%d) at %d,%d\n", bbox.width,
             bbox.height, bbox.x, bbox.y);
#endif
  ...
  if (options->animate_button) {
    g_signal_connect_swapped(options->animate_button, "clicked",
                             G_CALLBACK(gimp_warp_tool_animate), wt);
...
static void gimp_warp_tool_start_stroke_timer(GimpWarpTool *wt) {
  ...
  if (options->stroke_periodically && options->stroke_periodically_rate > 0.0 &&
    ...
    wt->stroke_timer = g_timeout_add(
        1000.0 / fps, (GSourceFunc)gimp_warp_tool_stroke_timer, wt);
  }
}
...
static void gimp_warp_tool_create_graph(GimpWarpTool *wt) {
  ...
  coords = gegl_node_new_child(graph, "operation", "gegl:buffer-source",
                               "buffer", wt->coords_buffer, NULL);

  render = gegl_node_new_child(graph, "operation", "gegl:map-relative",
                               "abyss-policy", options->abyss_policy, NULL);

  gegl_node_connect_to(input, "output", render, "input");

  gegl_node_connect_to(coords, "output", render, "aux");

  gegl_node_connect_to(render, "output", output, "input");
...
static void gimp_warp_tool_create_filter(GimpWarpTool *wt,
  ...
  wt->filter = gimp_drawable_filter_new(drawable, _("Warp transform"),
                                        wt->graph, GIMP_ICON_TOOL_WARP);
  ...
  g_signal_connect(wt->filter, "flush", G_CALLBACK(gimp_warp_tool_filter_flush),
                   wt);
}
...
static void gimp_warp_tool_set_sampler(GimpWarpTool *wt, gboolean commit) {
  ...
  gegl_node_get(wt->render_node, "sampler-type", &old_sampler, NULL);

  if (sampler != old_sampler) {
    gegl_node_set(wt->render_node, "sampler-type", sampler, NULL);
...
static GeglRectangle gimp_warp_tool_get_stroke_bounds(GeglNode *node) {
  ...
  gegl_node_get(node, "stroke", &stroke, "size", &size, NULL);
...
static GeglRectangle gimp_warp_tool_get_node_bounds(GeglNode *node) {
  ...
  if (!bounds) {
    ...
    gegl_rectangle_bounding_box(&input_bounds, &input_bounds, &stroke_bounds);

    bounds = gegl_rectangle_dup(&input_bounds);

    g_object_set_data_full(G_OBJECT(node), "gimp-warp-tool-bounds", bounds,
                           g_free);
  }
...
gimp_warp_tool_get_invalidated_by_change(GimpWarpTool *wt,
  ...
  if (wt->render_node) {
    ...
    result = gegl_operation_get_invalidated_by_change(operation, "aux", area);
  }
...
static void gimp_warp_tool_update_area(GimpWarpTool *wt,
  ...
  if (synchronous) {
    ...
    g_signal_handlers_block_by_func(wt->filter, gimp_warp_tool_filter_flush,
                                    wt);
    ...
    g_signal_handlers_unblock_by_func(wt->filter, gimp_warp_tool_filter_flush,
                                      wt);
  } else {
    gimp_drawable_filter_apply(wt->filter, &rect);
  }
...
static void gimp_warp_tool_add_op(GimpWarpTool *wt, GeglNode *op) {
  ...
  gegl_node_connect_to(last_op, "output", op, "input");
  gegl_node_connect_to(op, "output", wt->render_node, "aux");
}
...
static void gimp_warp_tool_animate(GimpWarpTool *wt) {
  ...
  gimp_progress_start(GIMP_PROGRESS(tool), FALSE, _("Rendering Frame %d"), 1);

  orig_image = gimp_item_get_image(GIMP_ITEM(tool->drawables->data));

  image = gimp_create_image(
      orig_image->gimp, gimp_item_get_width(GIMP_ITEM(tool->drawables->data)),
      gimp_item_get_height(GIMP_ITEM(tool->drawables->data)),
      gimp_drawable_get_base_type(tool->drawables->data),
      gimp_drawable_get_precision(tool->drawables->data), TRUE);

  /*  the first frame is always the unwarped image  */
  layer = GIMP_LAYER(gimp_item_convert(GIMP_ITEM(tool->drawables->data), image,
                                       GIMP_TYPE_LAYER));
  gimp_object_take_name(GIMP_OBJECT(layer), g_strdup_printf(_("Frame %d"), 1));
  ...
  scale_node = gegl_node_new_child(NULL, "operation", "gimp:scalar-multiply",
                                   "n-components", 2, NULL);
  ...
  for (i = 1; i < options->n_animation_frames; i++) {
    gimp_progress_set_text(GIMP_PROGRESS(tool), _("Rendering Frame %d"), i + 1);

    gimp_sub_progress_set_step(GIMP_SUB_PROGRESS(progress), i,
                               options->n_animation_frames);

    layer = GIMP_LAYER(
        gimp_item_duplicate(GIMP_ITEM(first_layer), GIMP_TYPE_LAYER));
    gimp_object_take_name(GIMP_OBJECT(layer),
                          g_strdup_printf(_("Frame %d"), i + 1));

    gegl_node_set(scale_node, "factor",
                  (gdouble)i / (gdouble)(options->n_animation_frames - 1),
                  NULL);

    gimp_gegl_apply_operation(
        gimp_drawable_get_buffer(GIMP_DRAWABLE(first_layer)), progress,
        _("Frame"), wt->graph, gimp_drawable_get_buffer(GIMP_DRAWABLE(layer)),
        NULL, FALSE);
