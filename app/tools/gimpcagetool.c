...
static gboolean gimp_cage_tool_initialize(GimpTool *tool, GimpDisplay *display,
                                          GError **error);
static void gimp_cage_tool_control(GimpTool *tool, GimpToolAction action,
                                   GimpDisplay *display);
...
static void gimp_cage_tool_button_release(GimpTool *tool,
                                          ...
                                          GimpDisplay *display);
static void gimp_cage_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                  guint32 time, GdkModifierType state,
                                  GimpDisplay *display);
static gboolean gimp_cage_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
                                         GimpDisplay *display);
static void gimp_cage_tool_cursor_update(GimpTool *tool,
                                         const GimpCoords *coords,
                                         GdkModifierType state,
                                         GimpDisplay *display);
static void gimp_cage_tool_oper_update(GimpTool *tool, const GimpCoords *coords,
                                       GdkModifierType state,
                                       gboolean proximity,
                                       GimpDisplay *display);
...
static void gimp_cage_tool_start(GimpCageTool *ct, GimpDisplay *display);
...
static gint gimp_cage_tool_is_on_handle(GimpCageTool *ct,
                                        ...
                                        gdouble y, gint handle_size);
static gint gimp_cage_tool_is_on_edge(GimpCageTool *ct, gdouble x, gdouble y,
                                      gint handle_size);
...
void gimp_cage_tool_register(GimpToolRegisterCallback callback, gpointer data) {
  (*callback)(GIMP_TYPE_CAGE_TOOL, GIMP_TYPE_CAGE_OPTIONS,
              gimp_cage_options_gui, 0, "gimp-cage-tool", _("Cage Transform"),
              _("Cage Transform: Deform a selection with a cage"),
              N_("_Cage Transform"), "<shift>G", NULL, GIMP_HELP_TOOL_CAGE,
              GIMP_ICON_TOOL_CAGE, data);
}

static void gimp_cage_tool_class_init(GimpCageToolClass *klass) {
...
static void gimp_cage_tool_init(GimpCageTool *self) {
...
static void gimp_cage_tool_button_press(GimpTool *tool,
  ...
  if (ct->config) {
    handle =
        gimp_cage_tool_is_on_handle(ct, draw_tool, display, coords->x,
                                    coords->y, GIMP_TOOL_HANDLE_SIZE_CIRCLE);
    edge = gimp_cage_tool_is_on_edge(ct, coords->x, coords->y,
  ...
  case CAGE_STATE_INIT:
     ...
     * moving handle state.
     */
    gimp_cage_config_add_cage_point(ct->config, coords->x - ct->offset_x,
  ...
  case CAGE_STATE_WAIT:
    if (handle == -1 && edge <= 0) {
      ...
      gimp_cage_config_select_point(
          ct->config, gimp_cage_config_get_n_points(ct->config) - 1);
      ct->tool_state = CAGE_STATE_MOVE_HANDLE;
      ...
      gimp_cage_config_insert_cage_point(ct->config, edge, coords->x,
                                         coords->y);
...
void gimp_cage_tool_button_release(GimpTool *tool, const GimpCoords *coords,
                                   guint32 time, GdkModifierType state,
                                   GimpButtonReleaseType release_type,
                                   GimpDisplay *display) {
  GimpCageTool *ct = GIMP_CAGE_TOOL(tool);
  ...
  if (release_type == GIMP_BUTTON_RELEASE_CANCEL) {
    ...
    case CAGE_STATE_SELECTING: {
      ...
      if (state & gimp_get_extend_selection_mask()) {
        gimp_cage_config_select_add_area(ct->config, GIMP_CAGE_MODE_CAGE_CHANGE,
                                         area);
      } else {
        gimp_cage_config_select_area(ct->config, GIMP_CAGE_MODE_CAGE_CHANGE,
                                     area);
      }
    ...
    case DEFORM_STATE_SELECTING: {
      ...
      if (state & gimp_get_extend_selection_mask()) {
        gimp_cage_config_select_add_area(ct->config, GIMP_CAGE_MODE_DEFORM,
                                         area);
      } else {
...
static void gimp_cage_tool_motion(GimpTool *tool, const GimpCoords *coords,
  ...
  case CAGE_STATE_CLOSING:
  case DEFORM_STATE_MOVE_HANDLE:
    gimp_cage_config_add_displacement(ct->config, options->cage_mode,
...
static gboolean gimp_cage_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
  ...
  case GDK_KEY_ISO_Enter:
    if (!gimp_cage_tool_is_complete(ct) &&
      ...
      g_object_set(gimp_tool_get_options(tool), "cage-mode",
                   GIMP_CAGE_MODE_DEFORM, NULL);
    } else if (ct->tool_state == DEFORM_STATE_WAIT) {
...
static void gimp_cage_tool_oper_update(GimpTool *tool, const GimpCoords *coords,
  ...
  if (ct->config) {
    ...
    ct->hovering_edge = gimp_cage_tool_is_on_edge(ct, coords->x, coords->y,
                                                  GIMP_TOOL_HANDLE_SIZE_CIRCLE);
  }
...
static void gimp_cage_tool_options_notify(GimpTool *tool,
  ...
  if (strcmp(pspec->name, "cage-mode") == 0) {
    ...
    g_object_get(options, "cage-mode", &mode, NULL);
    ...
    if (mode == GIMP_CAGE_MODE_DEFORM) {
      ...
      if (gimp_cage_config_get_n_points(ct->config) > 2) {
        ...
        g_object_set(options, "cage-mode", GIMP_CAGE_MODE_CAGE_CHANGE, NULL);
      }
    } else {
      /* switch to edit mode */
      if (ct->filter) {
...
static void gimp_cage_tool_draw(GimpDrawTool *draw_tool) {
  ...
  if (!gimp_cage_tool_is_complete(ct)) {
    ...
    last_point = gimp_cage_config_get_point_coordinate(
        ct->config, options->cage_mode, n_vertices - 1);

    gimp_draw_tool_add_line(draw_tool, last_point.x + ct->offset_x,
                            last_point.y + ct->offset_y, ct->cursor_x,
                            ct->cursor_y);
  }
  ...
  for (i = 0; i < n_vertices; i++) {
    ...
    if (i > 0 || gimp_cage_tool_is_complete(ct)) {
      ...
      point2 = gimp_cage_config_get_point_coordinate(
          ct->config, options->cage_mode, index_point2);
      point2.x += ct->offset_x;
      point2.y += ct->offset_y;

      if (i != ct->hovering_edge || gimp_cage_tool_is_complete(ct)) {
        gimp_draw_tool_push_group(draw_tool, stroke_group);
      }

      item = gimp_draw_tool_add_line(draw_tool, point1.x, point1.y, point2.x,
                                     point2.y);
    ...
    item = gimp_draw_tool_add_handle(
        draw_tool, handle, point1.x, point1.y, GIMP_TOOL_HANDLE_SIZE_CIRCLE,
...
static void gimp_cage_tool_start(GimpCageTool *ct, GimpDisplay *display) {
   ...
   * drawable coords
   */
  gimp_item_get_offset(GIMP_ITEM(tool->drawables->data), &ct->offset_x,
                       &ct->offset_y);
...
static void gimp_cage_tool_halt(GimpCageTool *ct) {
  ...
  g_object_set(gimp_tool_get_options(tool), "cage-mode",
               GIMP_CAGE_MODE_CAGE_CHANGE, NULL);
}
...
static gint gimp_cage_tool_is_on_handle(GimpCageTool *ct,
  ...
  for (i = 0; i < n_cage_vertices; i++) {
    cage_point =
        gimp_cage_config_get_point_coordinate(config, options->cage_mode, i);
    cage_point.x += ct->offset_x;
    cage_point.y += ct->offset_y;

    dist = gimp_draw_tool_calc_distance_square(
        GIMP_DRAW_TOOL(draw_tool), display, x, y, cage_point.x, cage_point.y);
...
static gint gimp_cage_tool_is_on_edge(GimpCageTool *ct, gdouble x, gdouble y,
  ...
  A = gimp_cage_config_get_point_coordinate(config, options->cage_mode,
                                            n_cage_vertices - 1);
  B = gimp_cage_config_get_point_coordinate(config, options->cage_mode, 0);
  C.x = x;
  C.y = y;
  ...
  for (i = 0; i < n_cage_vertices; i++) {
    ...
    A = B;
    B = gimp_cage_config_get_point_coordinate(config, options->cage_mode,
                                              (i + 1) % n_cage_vertices);
  }
...
static void gimp_cage_tool_compute_coef(GimpCageTool *ct) {
  ...
  input = gegl_node_new_child(gegl, "operation", "gimp:cage-coef-calc",
                              "config", ct->config, NULL);

  output = gegl_node_new_child(gegl, "operation", "gegl:buffer-sink", "buffer",
                               &buffer, "format", format, NULL);

  gegl_node_connect_to(input, "output", output, "input");
...
static void gimp_cage_tool_create_render_node(GimpCageTool *ct) {
  ...
  ct->coef_node =
      gegl_node_new_child(ct->render_node, "operation", "gegl:buffer-source",
                          "buffer", ct->coef, NULL);

  ct->cage_node = gegl_node_new_child(
      ct->render_node, "operation", "gimp:cage-transform", "config", ct->config,
      "fill-plain-color", options->fill_plain_color, NULL);

  render = gegl_node_new_child(ct->render_node, "operation",
                               "gegl:map-absolute", NULL);

  gegl_node_connect_to(input, "output", ct->cage_node, "input");

  gegl_node_connect_to(ct->coef_node, "output", ct->cage_node, "aux");

  gegl_node_connect_to(input, "output", render, "input");

  gegl_node_connect_to(ct->cage_node, "output", render, "aux");
...
static void gimp_cage_tool_render_node_update(GimpCageTool *ct) {
  ...
  gegl_node_get(ct->cage_node, "fill-plain-color", &fill, NULL);

  if (fill != options->fill_plain_color) {
    gegl_node_set(ct->cage_node, "fill-plain-color", options->fill_plain_color,
                  NULL);
  }

  gegl_node_get(ct->coef_node, "buffer", &buffer, NULL);

  if (buffer != ct->coef) {
    gegl_node_set(ct->coef_node, "buffer", ct->coef, NULL);
  }
...
static void gimp_cage_tool_create_filter(GimpCageTool *ct) {
  ...
  ct->filter = gimp_drawable_filter_new(GIMP_TOOL(ct)->drawables->data,
                                        _("Cage transform"), ct->render_node,
                                        GIMP_ICON_TOOL_CAGE);
  gimp_drawable_filter_set_region(ct->filter, GIMP_FILTER_REGION_DRAWABLE);

  g_signal_connect(ct->filter, "flush", G_CALLBACK(gimp_cage_tool_filter_flush),
                   ct);
}
