...
enum { PROP_0, PROP_SAMPLE_MERGED, PROP_SAMPLE_AVERAGE, PROP_AVERAGE_RADIUS };

static void gimp_color_options_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_color_options_get_property(GObject *object, guint property_id,
                                            GValue *value, GParamSpec *pspec);

G_DEFINE_TYPE(GimpColorOptions, gimp_color_options, GIMP_TYPE_TOOL_OPTIONS)
...
static void gimp_color_options_class_init(GimpColorOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_SAMPLE_AVERAGE, "sample-average",
                           _("Sample average"),
                           _("Use averaged color value from "
                             "nearby pixels"),
                           TRUE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_AVERAGE_RADIUS, "average-radius",
                          _("Radius"), _("Color Picker Average Radius"), 1.0,
                          300.0, 3.0, GIMP_PARAM_STATIC_STRINGS);
}
...
GtkWidget *gimp_color_options_gui(GimpToolOptions *tool_options) {
  ...
  scale =
      gimp_prop_spin_scale_new(config, "average-radius", NULL, 1.0, 10.0, 0);

  frame = gimp_prop_expanding_frame_new(config, "sample-average", NULL, scale,
                                        NULL);
