...
static void gimp_iscissors_tool_control(GimpTool *tool, GimpToolAction action,
                                        GimpDisplay *display);
static void gimp_iscissors_tool_button_press(GimpTool *tool,
                                             ...
                                             GimpDisplay *display);
static void
gimp_iscissors_tool_button_release(GimpTool *tool, const GimpCoords *coords,
                                   guint32 time, GdkModifierType state,
                                   GimpButtonReleaseType release_type,
                                   GimpDisplay *display);
static void gimp_iscissors_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                       guint32 time, GdkModifierType state,
                                       GimpDisplay *display);
...
static const gchar *gimp_iscissors_tool_can_redo(GimpTool *tool,
                                                 GimpDisplay *display);
static gboolean gimp_iscissors_tool_undo(GimpTool *tool, GimpDisplay *display);
static gboolean gimp_iscissors_tool_redo(GimpTool *tool, GimpDisplay *display);
...
static void find_optimal_path(GeglBuffer *gradient_map, GimpTempBuf *dp_buf,
                              gint x1, gint y1, gint x2, gint y2, gint xs,
                              gint ys);
static void find_max_gradient(GimpIscissorsTool *iscissors,
                              GimpPickable *pickable, gint *x, gint *y);
static void calculate_segment(GimpIscissorsTool *iscissors, ISegment *segment);
static GimpCanvasItem *iscissors_draw_segment(GimpDrawTool *draw_tool,
                                              ISegment *segment);

static gint mouse_over_vertex(GimpIscissorsTool *iscissors, gdouble x,
                              gdouble y);
static gboolean clicked_on_vertex(GimpIscissorsTool *iscissors, gdouble x,
                                  gdouble y);
static GList *mouse_over_segment(GimpIscissorsTool *iscissors, gdouble x,
                                 gdouble y);
static gboolean clicked_on_segment(GimpIscissorsTool *iscissors, gdouble x,
                                   gdouble y);

static GPtrArray *plot_pixels(GimpTempBuf *dp_buf, gint x1, gint y1, gint xs,
                              gint ys, gint xe, gint ye);

static ISegment *isegment_new(gint x1, gint y1, gint x2, gint y2);
...
static ISegment *icurve_append_segment(ICurve *curve, gint x1, gint y1, gint x2,
                                       gint y2);
static ISegment *icurve_insert_segment(ICurve *curve, GList *sibling, gint x1,
                                       gint y1, gint x2, gint y2);
static void icurve_delete_segment(ICurve *curve, ISegment *segment);

static void icurve_close(ICurve *curve);

static GimpScanConvert *icurve_create_scan_convert(ICurve *curve);
...
G_DEFINE_TYPE(GimpIscissorsTool, gimp_iscissors_tool, GIMP_TYPE_SELECTION_TOOL)
...
void gimp_iscissors_tool_register(GimpToolRegisterCallback callback,
                                  gpointer data) {
  (*callback)(
      GIMP_TYPE_ISCISSORS_TOOL, GIMP_TYPE_ISCISSORS_OPTIONS,
      gimp_iscissors_options_gui, 0, "gimp-iscissors-tool",
      _("Scissors Select"),
      _("Scissors Select Tool: Select shapes using intelligent edge-fitting"),
      N_("Intelligent _Scissors"), "I", NULL, GIMP_HELP_TOOL_ISCISSORS,
      GIMP_ICON_TOOL_ISCISSORS, data);
}
...
static void gimp_iscissors_tool_init(GimpIscissorsTool *iscissors) {
  ...
  gimp_tool_control_set_dirty_mask(
      tool->control, GIMP_DIRTY_IMAGE_SIZE | GIMP_DIRTY_ACTIVE_DRAWABLE);
...
static void gimp_iscissors_tool_button_press(GimpTool *tool,
  ...
  case NO_ACTION:
    ...
    if (!(state & gimp_get_extend_selection_mask()))
      find_max_gradient(iscissors, GIMP_PICKABLE(image), &iscissors->x,
                        &iscissors->y);
    ...
    segment = icurve_append_segment(iscissors->curve, iscissors->x,
                                    iscissors->y, iscissors->x, iscissors->y);
  ...
  default:
    ...
    /*  If the iscissors is closed, check if the click was inside  */
      ...
      if (last->x1 == last->x2 && last->y1 == last->y2) {
        ...
        segment = icurve_append_segment(iscissors->curve, last->x2, last->y2,
                                        iscissors->x, iscissors->y);
...
static void iscissors_convert(GimpIscissorsTool *iscissors,
  ...
  iscissors->mask = gimp_channel_new_mask(image, gimp_image_get_width(image),
                                          gimp_image_get_height(image));
  gimp_scan_convert_render(
      sc, gimp_drawable_get_buffer(GIMP_DRAWABLE(iscissors->mask)), 0, 0,
      options->antialias);
  ...
  if (release_type != GIMP_BUTTON_RELEASE_CANCEL) {
    ...
    case SEED_PLACEMENT:
      ...
      if (!iscissors->curve->first_point) {
        ...
        if (gimp_draw_tool_on_handle(
                GIMP_DRAW_TOOL(tool), display, iscissors->x, iscissors->y,
                GIMP_HANDLE_CIRCLE, segment->x1, segment->y1,
    ...
    case SEED_ADJUSTMENT:
      if (state & gimp_get_modify_selection_mask()) {
        if (iscissors->segment1 && iscissors->segment2) {
          icurve_delete_segment(iscissors->curve, iscissors->segment2);
...
static void gimp_iscissors_tool_draw(GimpDrawTool *draw_tool) {
  ...
  if (!iscissors->curve->first_point) {
    for (list = g_queue_peek_head_link(iscissors->curve->segments); list;
      ...
      if ((iscissors->state == SEED_PLACEMENT && !list->next) ||
          (iscissors->state == SEED_ADJUSTMENT &&
           (segment == iscissors->segment1 ||
            segment == iscissors->segment2))) {
        if (!options->interactive)
          item = gimp_draw_tool_add_line(draw_tool, segment->x1, segment->y1,
  ...
  for (list = g_queue_peek_head_link(iscissors->curve->segments); list;
    ...
    if (!iscissors->curve->first_point) {
      ...
      item = gimp_draw_tool_add_handle(
          draw_tool, adjustment ? GIMP_HANDLE_CROSS : GIMP_HANDLE_FILLED_CIRCLE,
          segment->x1, segment->y1, GIMP_TOOL_HANDLE_SIZE_CIRCLE,
          GIMP_TOOL_HANDLE_SIZE_CIRCLE, GIMP_HANDLE_ANCHOR_CENTER);
    ...
    if (!list->next && !iscissors->curve->closed) {
      ...
      item = gimp_draw_tool_add_handle(
          draw_tool, placement ? GIMP_HANDLE_CROSS : GIMP_HANDLE_FILLED_CIRCLE,
          segment->x2, segment->y2, GIMP_TOOL_HANDLE_SIZE_CIRCLE,
          GIMP_TOOL_HANDLE_SIZE_CIRCLE, GIMP_HANDLE_ANCHOR_CENTER);
...
static void gimp_iscissors_tool_oper_update(GimpTool *tool,
  ...
  if (mouse_over_vertex(iscissors, coords->x, coords->y) > 1) {
    ...
    if (state & remove_mask) {
      gimp_tool_replace_status(tool, display, _("Click to remove this point"));
      iscissors->op = ISCISSORS_OP_REMOVE_POINT;
    } else {
      gchar *status = gimp_suggest_modifiers(_("Click-Drag to move this point"),
                                             (snap_mask | remove_mask) & ~state,
                                             _("%s: disable auto-snap"),
                                             _("%s: remove this point"), NULL);
    ...
    if (gimp_draw_tool_on_handle(
            GIMP_DRAW_TOOL(tool), display, RINT(coords->x), RINT(coords->y),
            GIMP_HANDLE_CIRCLE, segment->x1, segment->y1,
            GIMP_TOOL_HANDLE_SIZE_CIRCLE, GIMP_TOOL_HANDLE_SIZE_CIRCLE,
            GIMP_HANDLE_ANCHOR_CENTER)) {
      gimp_tool_replace_status(tool, display, _("Click to close the curve"));
...
static void gimp_iscissors_tool_cursor_update(GimpTool *tool,
  ...
  if (modifier != GIMP_CURSOR_MODIFIER_NONE) {
    gimp_tool_set_cursor(tool, display, GIMP_CURSOR_MOUSE,
                         GIMP_TOOL_CURSOR_ISCISSORS, modifier);
  } else {
  ...
  case GDK_KEY_BackSpace:
    if (!iscissors->curve->closed &&
      ...
      if (g_queue_get_length(iscissors->curve->segments) > 1) {
        ...
        g_ptr_array_remove_range(segment->points, 0, segment->points->len);
        gimp_iscissors_tool_free_redo(iscissors);
...
static gboolean gimp_iscissors_tool_undo(GimpTool *tool, GimpDisplay *display) {
  ...
  iscissors->redo_stack =
      g_list_prepend(iscissors->redo_stack, iscissors->curve);

  iscissors->curve = iscissors->undo_stack->data;

  iscissors->undo_stack =
      g_list_remove(iscissors->undo_stack, iscissors->curve);
...
static gboolean gimp_iscissors_tool_redo(GimpTool *tool, GimpDisplay *display) {
  ...
  iscissors->undo_stack =
      g_list_prepend(iscissors->undo_stack, iscissors->curve);

  iscissors->curve = iscissors->redo_stack->data;

  iscissors->redo_stack =
      g_list_remove(iscissors->redo_stack, iscissors->curve);
...
static void gimp_iscissors_tool_push_undo(GimpIscissorsTool *iscissors) {
  iscissors->undo_stack =
      g_list_prepend(iscissors->undo_stack, icurve_copy(iscissors->curve));
}
...
static void gimp_iscissors_tool_commit(GimpIscissorsTool *iscissors,
  ...
  if (iscissors->curve->closed && iscissors->mask) {
    gimp_channel_select_channel(
        gimp_image_get_mask(image), gimp_tool_get_undo_desc(tool),
        iscissors->mask, 0, 0, options->operation, options->feather,
        options->feather_radius, options->feather_radius);
...
static gint mouse_over_vertex(GimpIscissorsTool *iscissors, gdouble x,
  ...
  for (list = g_queue_peek_head_link(iscissors->curve->segments); list;
    ...
    if (gimp_draw_tool_on_handle(
            GIMP_DRAW_TOOL(iscissors), GIMP_TOOL(iscissors)->display, x, y,
            GIMP_HANDLE_CIRCLE, segment->x1, segment->y1,
            GIMP_TOOL_HANDLE_SIZE_CIRCLE, GIMP_TOOL_HANDLE_SIZE_CIRCLE,
            GIMP_HANDLE_ANCHOR_CENTER)) {
    ...
    } else if (gimp_draw_tool_on_handle(
                   GIMP_DRAW_TOOL(iscissors), GIMP_TOOL(iscissors)->display, x,
                   y, GIMP_HANDLE_CIRCLE, segment->x2, segment->y2,
                   GIMP_TOOL_HANDLE_SIZE_CIRCLE, GIMP_TOOL_HANDLE_SIZE_CIRCLE,
                   GIMP_HANDLE_ANCHOR_CENTER)) {
...
static GList *mouse_over_segment(GimpIscissorsTool *iscissors, gdouble x,
  ...
  for (list = g_queue_peek_head_link(iscissors->curve->segments); list;
    ...
    while (len--) {
      ...
      if (gimp_draw_tool_calc_distance_square(
              GIMP_DRAW_TOOL(iscissors), GIMP_TOOL(iscissors)->display, tx, ty,
...
static gboolean clicked_on_segment(GimpIscissorsTool *iscissors, gdouble x,
  ...
  if (list) {
    ...
    new_segment = icurve_insert_segment(iscissors->curve, list, iscissors->x,
                                        iscissors->y, segment->x2, segment->y2);
...
static void calculate_segment(GimpIscissorsTool *iscissors, ISegment *segment) {
  ...
  y1 = MIN(ys, ye);
  x2 = MAX(xs, xe) +
       1; /*  +1 because if xe = 199 & xs = 0, x2 - x1, width = 200  */
  y2 = MAX(ys, ye) + 1;
  ...
  if ((x2 - x1) && (y2 - y1)) {
    ...
    dp_buf = gimp_temp_buf_new(dp_width, dp_height, babl_format("Y u32"));

    /*  find the optimal path of pixels from (x1, y1) to (x2, y2)  */
    find_optimal_path(iscissors->gradient_map, dp_buf, x1, y1, x2, y2, xs, ys);
...
static gboolean gradient_map_value(GeglSampler *map_sampler,
                                   const GeglRectangle *map_extent, gint x,
                                   gint y, guint8 *grad, guint8 *dir) {
  if (x >= map_extent->x && y >= map_extent->y && x < map_extent->width &&
      y < map_extent->height) {
    guint8 sample[2];
...
static gint calculate_link(GeglSampler *map_sampler,
                           const GeglRectangle *map_extent, gint x, gint y,
                           guint32 pixel, gint link) {
  gint value = 0;
...
static GPtrArray *plot_pixels(GimpTempBuf *dp_buf, gint x1, gint y1, gint xs,
  ...
  data =
      (guint *)gimp_temp_buf_get_data(dp_buf) + (ye - y1) * width + (xe - x1);
...
static void find_optimal_path(GeglBuffer *gradient_map, GimpTempBuf *dp_buf,
  ...
  map_sampler = gegl_buffer_sampler_new(
      gradient_map, gegl_buffer_get_format(gradient_map), GEGL_SAMPLER_NEAREST);
  ...
  for (i = 0; i < dp_buf_height; i++) {
    ...
    for (j = 0; j < dp_buf_width; j++) {
      ...
      for (k = 0; k < 8; k++)
        if (pixel[k]) {
          link_cost[k] =
              calculate_link(map_sampler, map_extent, xs + j * dirx,
                             ys + i * diry, pixel[k], ((k > 3) ? k - 4 : k));
...
static GeglBuffer *gradient_map_new(GimpPickable *pickable) {
  ...
  buffer = gegl_buffer_new(GEGL_RECTANGLE(0, 0, gegl_buffer_get_width(buffer),
  ...
  gimp_tile_handler_validate_invalidate(
      GIMP_TILE_HANDLER_VALIDATE(handler),
...
static void find_max_gradient(GimpIscissorsTool *iscissors,
  ...
  *y = cy;
  ...
  iter = gegl_buffer_iterator_new(iscissors->gradient_map,
                                  ...
                                  NULL, GEGL_ACCESS_READ, GEGL_ABYSS_NONE, 1);
...
static ISegment *isegment_copy(ISegment *segment) {
  ISegment *copy =
      isegment_new(segment->x1, segment->y1, segment->x2, segment->y2);
...
static ICurve *icurve_copy(ICurve *curve) {
  ...
  for (link = g_queue_peek_head_link(curve->segments); link;
...
static GimpScanConvert *icurve_create_scan_convert(ICurve *curve) {
  ...
  for (list = g_queue_peek_tail_link(curve->segments); list;
  ...
  for (list = g_queue_peek_tail_link(curve->segments); list;
    ...
    for (i = 0; i < n_points; i++) {
      guint32 packed = GPOINTER_TO_INT(g_ptr_array_index(segment->points, i));
