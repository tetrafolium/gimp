...
static void gimp_scale_tool_size_notify(GtkWidget *box, GParamSpec *pspec,
...
void gimp_scale_tool_register(GimpToolRegisterCallback callback,
                              gpointer data) {
  (*callback)(GIMP_TYPE_SCALE_TOOL, GIMP_TYPE_TRANSFORM_GRID_OPTIONS,
              gimp_transform_grid_options_gui,
              GIMP_CONTEXT_PROP_MASK_BACKGROUND, "gimp-scale-tool", _("Scale"),
              _("Scale Tool: Scale the layer, selection or path"), N_("_Scale"),
              "<shift>S", NULL, GIMP_HELP_TOOL_SCALE, GIMP_ICON_TOOL_SCALE,
              data);
}
...
static void gimp_scale_tool_init(GimpScaleTool *scale_tool) {
...
static gboolean gimp_scale_tool_info_to_matrix(GimpTransformGridTool *tg_tool,
  ...
  gimp_transform_matrix_scale(
      transform, tr_tool->x1, tr_tool->y1, tr_tool->x2 - tr_tool->x1,
      tr_tool->y2 - tr_tool->y1, tg_tool->trans_info[X0],
...
static void gimp_scale_tool_matrix_to_info(GimpTransformGridTool *tg_tool,
  ...
  tg_tool->trans_info[X1] =
      tg_tool->trans_info[X0] + w * (tr_tool->x2 - tr_tool->x1);
  tg_tool->trans_info[Y1] =
      tg_tool->trans_info[Y0] + h * (tr_tool->y2 - tr_tool->y1);
}
...
static gchar *gimp_scale_tool_get_undo_desc(GimpTransformGridTool *tg_tool) {
  ...
  return g_strdup_printf(C_("undo-type", "Scale to %d x %d"), width, height);
}
...
static void gimp_scale_tool_dialog_update(GimpTransformGridTool *tg_tool) {
  ...
  g_object_set(GIMP_SCALE_TOOL(tg_tool)->box, "width", width, "height", height,
               "keep-aspect", options->constrain_scale, NULL);
}
...
static void gimp_scale_tool_prepare(GimpTransformGridTool *tg_tool) {
  ...
  gimp_image_get_resolution(gimp_display_get_image(display), &xres, &yres);

  if (scale->box) {
    g_signal_handlers_disconnect_by_func(scale->box,
                                         gimp_scale_tool_size_notify, tg_tool);
   ...
   */
  scale->box = g_object_new(
      GIMP_TYPE_SIZE_BOX, "width", tr_tool->x2 - tr_tool->x1, "height",
      tr_tool->y2 - tr_tool->y1, "keep-aspect", options->constrain_scale,
      "unit", gimp_display_get_shell(display)->unit, "xresolution", xres,
      "yresolution", yres, NULL);

  gtk_box_pack_start(GTK_BOX(gimp_tool_gui_get_vbox(tg_tool->gui)), scale->box,
                     FALSE, FALSE, 0);
  gtk_widget_show(scale->box);

  g_signal_connect(scale->box, "notify",
                   G_CALLBACK(gimp_scale_tool_size_notify), tg_tool);
}
...
static void gimp_scale_tool_readjust(GimpTransformGridTool *tg_tool) {
  ...
  gimp_display_shell_untransform_xy_f(shell, x, y, &x, &y);
...
gimp_scale_tool_get_widget(GimpTransformGridTool *tg_tool) {
  ...
  g_object_set(widget, "inside-function", GIMP_TRANSFORM_FUNCTION_SCALE,
               "outside-function", GIMP_TRANSFORM_FUNCTION_SCALE,
               "use-corner-handles", TRUE, "use-side-handles", TRUE,
               "use-center-handle", TRUE, NULL);
...
static void gimp_scale_tool_widget_changed(GimpTransformGridTool *tg_tool) {
  ...
  g_object_get(tg_tool->widget, "transform", &transform, NULL);

  gimp_matrix3_transform_point(transform, tr_tool->x1, tr_tool->y1, &x0, &y0);
  gimp_matrix3_transform_point(transform, tr_tool->x2, tr_tool->y2, &x1, &y1);
...
static void gimp_scale_tool_size_notify(GtkWidget *box, GParamSpec *pspec,
  ...
  if (!strcmp(pspec->name, "width") || !strcmp(pspec->name, "height")) {
    gint width;
    gint height;
    gint old_width;
    gint old_height;

    g_object_get(box, "width", &width, "height", &height, NULL);
    ...
    if ((width != old_width) || (height != old_height)) {
      ...
      if (options->frompivot_scale) {
        ...
        center_x = (tg_tool->trans_info[X0] + tg_tool->trans_info[X1]) / 2.0;
        center_y = (tg_tool->trans_info[Y0] + tg_tool->trans_info[Y1]) / 2.0;
    ...
    g_object_get(box, "keep-aspect", &constrain, NULL);
