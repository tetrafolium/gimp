...
static gboolean gimp_gradient_tool_initialize(GimpTool *tool,
                                              GimpDisplay *display,
                                              GError **error);
static void gimp_gradient_tool_control(GimpTool *tool, GimpToolAction action,
                                       GimpDisplay *display);
static void gimp_gradient_tool_button_press(GimpTool *tool,
                                            ...
                                            GimpDisplay *display);
static void
gimp_gradient_tool_button_release(GimpTool *tool, const GimpCoords *coords,
                                  guint32 time, GdkModifierType state,
                                  GimpButtonReleaseType release_type,
                                  GimpDisplay *display);
static void gimp_gradient_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                      guint32 time, GdkModifierType state,
                                      GimpDisplay *display);
static gboolean gimp_gradient_tool_key_press(GimpTool *tool,
                                             GdkEventKey *kevent,
                                             GimpDisplay *display);
static void gimp_gradient_tool_modifier_key(GimpTool *tool, GdkModifierType key,
                                            gboolean press,
...
static const gchar *gimp_gradient_tool_can_redo(GimpTool *tool,
                                                GimpDisplay *display);
static gboolean gimp_gradient_tool_undo(GimpTool *tool, GimpDisplay *display);
static gboolean gimp_gradient_tool_redo(GimpTool *tool, GimpDisplay *display);
...
void gimp_gradient_tool_register(GimpToolRegisterCallback callback,
                                 gpointer data) {
  (*callback)(
      GIMP_TYPE_GRADIENT_TOOL, GIMP_TYPE_GRADIENT_OPTIONS,
      gimp_gradient_options_gui,
      GIMP_CONTEXT_PROP_MASK_FOREGROUND | GIMP_CONTEXT_PROP_MASK_BACKGROUND |
          GIMP_CONTEXT_PROP_MASK_OPACITY | GIMP_CONTEXT_PROP_MASK_PAINT_MODE |
          GIMP_CONTEXT_PROP_MASK_GRADIENT,
      "gimp-gradient-tool", _("Gradient"),
      _("Gradient Tool: Fill selected area with a color gradient"),
      N_("Gra_dient"), "G", NULL, GIMP_HELP_TOOL_GRADIENT,
      GIMP_ICON_TOOL_GRADIENT, data);
}

static void gimp_gradient_tool_class_init(GimpGradientToolClass *klass) {
...
static void gimp_gradient_tool_init(GimpGradientTool *gradient_tool) {
  ...
  gimp_tool_control_set_tool_cursor(tool->control, GIMP_TOOL_CURSOR_GRADIENT);
...
gimp_gradient_tool_button_release(GimpTool *tool, const GimpCoords *coords,
  ...
  if (gradient_tool->grab_widget) {
    gimp_tool_widget_button_release(gradient_tool->grab_widget, coords, time,
                                    state, release_type);
  ...
  if (!options->instant) {
    gimp_gradient_tool_editor_end_edit(
        gradient_tool, release_type == GIMP_BUTTON_RELEASE_CANCEL);
  }
}
...
static void gimp_gradient_tool_modifier_key(GimpTool *tool, GdkModifierType key,
  ...
  if (key == gimp_get_extend_selection_mask()) {
    if (options->instant_toggle &&
      ...
      g_object_set(options, "instant", !options->instant, NULL);
    }
  }
}
...
static void gimp_gradient_tool_cursor_update(GimpTool *tool,
  ...
  if (g_list_length(drawables) != 1 ||
      ...
      !(gimp_item_is_visible(drawables->data) || config->edit_non_visible)) {
...
static void gimp_gradient_tool_options_notify(GimpTool *tool,
  ...
  if (!strcmp(pspec->name, "gradient")) {
    ...
    if (!strcmp(pspec->name, "gradient-type")) {
      ...
      gegl_node_get(gradient_tool->render_node, "gradient-repeat", &node_repeat,
                    NULL);
      ...
      if (gradient_type >= GIMP_GRADIENT_SHAPEBURST_ANGULAR) {
        ...
        if (node_repeat != GIMP_REPEAT_NONE)
          gegl_node_set(gradient_tool->render_node, "gradient-repeat",
                        GIMP_REPEAT_NONE, NULL);
      } else if (node_repeat != gradient_repeat) {
        gegl_node_set(gradient_tool->render_node, "gradient-repeat",
                      gradient_repeat, NULL);
      }
    ...
    gimp_gradient_tool_update_graph(gradient_tool);
    gimp_drawable_filter_apply(gradient_tool->filter, NULL);
  } else if (gradient_tool->filter && !strcmp(pspec->name, "opacity")) {
    gimp_drawable_filter_set_opacity(gradient_tool->filter,
                                     gimp_context_get_opacity(context));
  } else if (gradient_tool->filter && !strcmp(pspec->name, "paint-mode")) {
    gimp_drawable_filter_set_mode(
        gradient_tool->filter, gimp_context_get_paint_mode(context),
...
static void gimp_gradient_tool_start(GimpGradientTool *gradient_tool,
  ...
  gradient_tool->widget =
      gimp_tool_line_new(shell, gradient_tool->start_x, gradient_tool->start_y,
                         gradient_tool->end_x, gradient_tool->end_y);

  g_object_set(gradient_tool->widget, "status-title", _("Gradient: "), NULL);

  gimp_draw_tool_set_widget(GIMP_DRAW_TOOL(tool), gradient_tool->widget);

  g_signal_connect(gradient_tool->widget, "changed",
                   G_CALLBACK(gimp_gradient_tool_line_changed), gradient_tool);
  g_signal_connect(gradient_tool->widget, "response",
                   G_CALLBACK(gimp_gradient_tool_line_response), gradient_tool);
...
static void gimp_gradient_tool_halt(GimpGradientTool *gradient_tool) {
  ...
  g_signal_handlers_disconnect_by_func(
      context, G_CALLBACK(gimp_gradient_tool_fg_bg_changed), gradient_tool);
...
static void gimp_gradient_tool_commit(GimpGradientTool *gradient_tool) {
  ...
  if (gradient_tool->filter) {
    ...
    gimp_drawable_filter_commit(gradient_tool->filter, GIMP_PROGRESS(tool),
                                FALSE);
...
static void gimp_gradient_tool_line_changed(GimpToolWidget *widget,
  ...
  g_object_get(widget, "x1", &start_x, "y1", &start_y, "x2", &end_x, "y2",
               &end_y, NULL);

  if (start_x != gradient_tool->start_x || start_y != gradient_tool->start_y ||
      end_x != gradient_tool->end_x || end_y != gradient_tool->end_y) {
...
gimp_gradient_tool_precalc_shapeburst(GimpGradientTool *gradient_tool) {
  ...
  if (!gimp_item_mask_intersect(GIMP_ITEM(tool->drawables->data), &x, &y,
                                &width, &height))
    return;

  gradient_tool->dist_buffer = gimp_drawable_gradient_shapeburst_distmap(
      tool->drawables->data, options->distance_metric,
      GEGL_RECTANGLE(x, y, width, height), GIMP_PROGRESS(gradient_tool));

  if (gradient_tool->dist_node)
    gegl_node_set(gradient_tool->dist_node, "buffer",
                  gradient_tool->dist_buffer, NULL);
...
static void gimp_gradient_tool_create_graph(GimpGradientTool *gradient_tool) {
  ...
  gradient_tool->dist_node = gegl_node_new_child(
      gradient_tool->graph, "operation", "gegl:buffer-source", "buffer",
      gradient_tool->dist_buffer, NULL);
  ...
  gradient_tool->render_node =
      gegl_node_new_child(gradient_tool->graph, "operation", "gimp:gradient",
  ...
  gegl_node_link_many(gradient_tool->dist_node,
	                     ...
	                     gradient_tool->divide_node,
#endif
                      gradient_tool->render_node, output, NULL);
...
static void gimp_gradient_tool_update_graph(GimpGradientTool *gradient_tool) {
...
#endif
  {
    ...
    gimp_item_mask_intersect(GIMP_ITEM(tool->drawables->data), &roi.x, &roi.y,
    ...
    gimp_drawable_gradient_adjust_coords(tool->drawables->data,
                                         options->gradient_type, &roi, &start_x,
                                         &start_y, &end_x, &end_y);

    gegl_node_set(gradient_tool->render_node, "start-x", start_x, "start-y",
                  start_y, "end-x", end_x, "end-y", end_y, NULL);
  }
}
...
static void gimp_gradient_tool_set_gradient(GimpGradientTool *gradient_tool,
  ...
  if (gradient_tool->gradient)
    g_signal_handlers_disconnect_by_func(
        gradient_tool->gradient, G_CALLBACK(gimp_gradient_tool_gradient_dirty),
        gradient_tool);
...
static void gimp_gradient_tool_create_filter(GimpGradientTool *gradient_tool,
  ...
  gradient_tool->filter =
      gimp_drawable_filter_new(drawable, C_("undo-type", "Gradient"),
                               gradient_tool->graph, GIMP_ICON_TOOL_GRADIENT);
  ...
  g_signal_connect(gradient_tool->filter, "flush",
                   G_CALLBACK(gimp_gradient_tool_filter_flush), gradient_tool);
}
...
void gimp_gradient_tool_set_tentative_gradient(GimpGradientTool *gradient_tool,
  ...
  if (g_set_object(&gradient_tool->tentative_gradient, gradient)) {
    if (gradient_tool->render_node) {
      gegl_node_set(gradient_tool->render_node, "gradient",
                    gradient ? gradient : gradient_tool->gradient, NULL);
