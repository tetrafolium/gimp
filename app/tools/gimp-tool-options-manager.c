...
static void tool_options_manager_paint_options_notify(GimpPaintOptions *src,
                                                      const GParamSpec *pspec,
...
void gimp_tool_options_manager_init(Gimp *gimp) {
  ...
  manager->global_paint_options =
      g_object_new(GIMP_TYPE_PAINT_OPTIONS, "gimp", gimp, "name",
                   "tool-options-manager-global-paint-options", NULL);
  ...
  for (list = gimp_get_tool_info_iter(gimp); list; list = g_list_next(list)) {
    ...
    gimp_context_define_properties(
        ...
        manager->global_props & tool_info->context_props, FALSE);
    ...
    if (GIMP_IS_PAINT_OPTIONS(tool_info->tool_options)) {
      ...
      tool_options_manager_copy_paint_props(
          manager->global_paint_options,
          GIMP_PAINT_OPTIONS(tool_info->tool_options),
          tool_info->context_props & manager->global_props);
    }
  }

  g_signal_connect(gimp->config, "notify::global-brush",
                   G_CALLBACK(tool_options_manager_global_notify), manager);
  ...
  g_signal_connect(user_context, "tool-changed",
                   G_CALLBACK(tool_options_manager_tool_changed), manager);

  tool_options_manager_tool_changed(
      user_context, gimp_context_get_tool(user_context), manager);
}
...
void gimp_tool_options_manager_exit(Gimp *gimp) {
  ...
  g_signal_handlers_disconnect_by_func(
      user_context, tool_options_manager_tool_changed, manager);

  g_signal_handlers_disconnect_by_func(
      gimp->config, tool_options_manager_global_notify, manager);
  ...
  for (list = gimp_get_tool_info_iter(gimp); list; list = g_list_next(list)) {
    ...
    if (GIMP_IS_PAINT_OPTIONS(tool_info->tool_options)) {
      g_signal_handlers_disconnect_by_func(
          tool_info->tool_options, tool_options_manager_paint_options_notify,
          manager->global_paint_options);

      g_signal_handlers_disconnect_by_func(
          manager->global_paint_options,
          tool_options_manager_paint_options_notify, tool_info->tool_options);
    }
  }
...
tool_options_manager_global_notify(GimpCoreConfig *config,
   ...
   */
  for (list = gimp_get_tool_info_iter(manager->gimp); list;
    ...
    gimp_context_define_properties(
        GIMP_CONTEXT(tool_info->tool_options),
        tool_info->context_props & disabled_global_props, TRUE);
    ...
    gimp_context_define_properties(
        GIMP_CONTEXT(tool_info->tool_options),
        tool_info->context_props & enabled_global_props, FALSE);

    if (GIMP_IS_PAINT_OPTIONS(tool_info->tool_options))
      tool_options_manager_copy_paint_props(
          manager->global_paint_options,
          GIMP_PAINT_OPTIONS(tool_info->tool_options),
          tool_info->context_props & enabled_global_props);
  }
...
static void tool_options_manager_paint_options_notify(GimpPaintOptions *src,
  ...
  if (gimp_paint_options_is_prop(pspec->name, prop_mask)) {
    ...
    g_signal_handlers_block_by_func(
        dest, tool_options_manager_paint_options_notify, src);
...
static void tool_options_manager_tool_changed(GimpContext *user_context,
  ...
  if (manager->active_tool) {
     ...
     */
    gimp_context_define_properties(
        GIMP_CONTEXT(active->tool_options),
        active->context_props & ~manager->global_props, TRUE);
  }
  ...
  if (manager->active_tool) {
    ...
    if (GIMP_IS_PAINT_OPTIONS(active->tool_options))
      tool_options_manager_copy_paint_props(
          GIMP_PAINT_OPTIONS(active->tool_options),
          manager->global_paint_options,
          active->context_props & ~manager->global_props);
