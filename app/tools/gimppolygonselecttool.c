...
static void gimp_polygon_select_tool_control(GimpTool *tool,
                                             ...
                                             GimpDisplay *display);
static void
gimp_polygon_select_tool_button_press(GimpTool *tool, const GimpCoords *coords,
                                      guint32 time, GdkModifierType state,
                                      GimpButtonPressType press_type,
                                      GimpDisplay *display);
static void gimp_polygon_select_tool_button_release(
...
static void gimp_polygon_select_tool_polygon_change_complete(
    GimpToolWidget *polygon, GimpPolygonSelectTool *poly_sel);
static void gimp_polygon_select_tool_polygon_response(
    GimpToolWidget *polygon, gint response_id, GimpPolygonSelectTool *poly_sel);
...
static void gimp_polygon_select_tool_init(GimpPolygonSelectTool *poly_sel) {
...
gimp_polygon_select_tool_button_press(GimpTool *tool, const GimpCoords *coords,
  ...
  if (!priv->widget) /* not tool->display, we have a subclass */
  {
     ...
     * if appropriate.
     */
    if (gimp_selection_tool_start_edit(GIMP_SELECTION_TOOL(poly_sel), display,
                                       coords)) {
      return;
    }
...
static void gimp_polygon_select_tool_button_release(
  ...
  if (priv->pending_response) {
    gimp_polygon_select_tool_polygon_response(
        priv->widget, priv->pending_response_id, poly_sel);
...
static void gimp_polygon_select_tool_cursor_update(GimpTool *tool,
  ...
  if (tool->display) {
    if (priv->widget && display == tool->display) {
      gimp_tool_widget_get_cursor(priv->widget, coords, state, NULL, NULL,
                                  &modifier);
    ...
    gimp_tool_set_cursor(
        tool, display, gimp_tool_control_get_cursor(tool->control),
        gimp_tool_control_get_tool_cursor(tool->control), modifier);
  }
...
gimp_polygon_select_tool_polygon_response(GimpToolWidget *polygon,
  ...
  case GIMP_TOOL_WIDGET_RESPONSE_CONFIRM:
    ...
    if (GIMP_POLYGON_SELECT_TOOL_GET_CLASS(poly_sel)->confirm) {
      GIMP_POLYGON_SELECT_TOOL_GET_CLASS(poly_sel)->confirm(poly_sel,
                                                            tool->display);
    }
    break;
...
void gimp_polygon_select_tool_get_points(GimpPolygonSelectTool *poly_sel,
  ...
  if (priv->widget) {
    gimp_tool_polygon_get_points(GIMP_TOOL_POLYGON(priv->widget), points,
                                 n_points);
  } else {
    if (points)
      *points = NULL;
