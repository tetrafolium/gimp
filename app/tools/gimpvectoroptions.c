...
enum { PROP_0, PROP_VECTORS_EDIT_MODE, PROP_VECTORS_POLYGONAL };

static void gimp_vector_options_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_vector_options_get_property(GObject *object, guint property_id,
                                             GValue *value, GParamSpec *pspec);

G_DEFINE_TYPE(GimpVectorOptions, gimp_vector_options, GIMP_TYPE_TOOL_OPTIONS)

static void gimp_vector_options_class_init(GimpVectorOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(object_class, PROP_VECTORS_EDIT_MODE,
                        "vectors-edit-mode", _("Edit Mode"), NULL,
                        GIMP_TYPE_VECTOR_MODE, GIMP_VECTOR_MODE_DESIGN,
                        GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_VECTORS_POLYGONAL, "vectors-polygonal", _("Polygonal"),
      _("Restrict editing to polygons"), FALSE, GIMP_PARAM_STATIC_STRINGS);
}
...
static void button_append_modifier(GtkWidget *button,
                                   GdkModifierType modifiers) {
  gchar *str =
      g_strdup_printf("%s (%s)", gtk_button_get_label(GTK_BUTTON(button)),
...
GtkWidget *gimp_vector_options_gui(GimpToolOptions *tool_options) {
  ...
  /*  tool toggle  */
  frame =
      gimp_prop_enum_radio_frame_new(config, "vectors-edit-mode", NULL, 0, 0);
  ...
  if (GTK_IS_RADIO_BUTTON(button)) {
    ...
    if (list->next) /* GIMP_VECTOR_MODE_EDIT  */
      button_append_modifier(list->next->data, gimp_get_toggle_behavior_mask());
  }
