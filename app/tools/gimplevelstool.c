...
static gboolean levels_menu_sensitivity(gint value, gpointer data);

static void levels_stretch_callback(GtkWidget *widget, GimpLevelsTool *tool);
...
void gimp_levels_tool_register(GimpToolRegisterCallback callback,
                               gpointer data) {
  (*callback)(GIMP_TYPE_LEVELS_TOOL, GIMP_TYPE_HISTOGRAM_OPTIONS,
              gimp_color_options_gui, 0, "gimp-levels-tool", _("Levels"),
              _("Adjust color levels"), N_("_Levels..."), NULL, NULL,
              GIMP_HELP_TOOL_LEVELS, GIMP_ICON_TOOL_LEVELS, data);
}
...
static gboolean gimp_levels_tool_initialize(GimpTool *tool,
  ...
  gimp_prop_widget_set_factor(l_tool->low_input_spinbutton, scale_factor,
                              step_increment, page_increment, digits);
  gimp_prop_widget_set_factor(l_tool->high_input_spinbutton, scale_factor,
                              step_increment, page_increment, digits);
  gimp_prop_widget_set_factor(l_tool->low_output_spinbutton, scale_factor,
                              step_increment, page_increment, digits);
  gimp_prop_widget_set_factor(l_tool->high_output_spinbutton, scale_factor,
                              step_increment, page_increment, digits);

  gtk_adjustment_configure(l_tool->gamma_linear, scale_factor / 2.0, 0,
                           scale_factor, 0.1, 1.0, 0);
...
static GtkWidget *gimp_levels_tool_color_picker_new(GimpLevelsTool *tool,
                                                    guint value) {
  ...
  return gimp_filter_tool_add_color_picker(
      GIMP_FILTER_TOOL(tool), GUINT_TO_POINTER(value), icon_name, help,
      /* pick_abyss = */ FALSE, NULL, NULL);
}
...
static void gimp_levels_tool_dialog(GimpFilterTool *filter_tool) {
  ...
  g_signal_connect(filter_tool->settings_box, "file-dialog-setup",
                   G_CALLBACK(gimp_levels_tool_export_setup), filter_tool);
  ...
  gimp_int_combo_box_set_sensitivity(GIMP_INT_COMBO_BOX(tool->channel_menu),
                                     levels_menu_sensitivity, filter_tool,
                                     NULL);
  ...
  g_signal_connect(tool->channel_menu, "changed",
                   G_CALLBACK(levels_channel_callback), tool);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(levels_channel_reset_callback),
                   tool);
  ...
  gimp_handle_bar_connect_events(GIMP_HANDLE_BAR(handle_bar), tool->input_bar);
  gimp_handle_bar_connect_events(GIMP_HANDLE_BAR(handle_bar), bar);
  ...
  tool->low_input_spinbutton = spinbutton =
      gimp_prop_spin_button_new(filter_tool->config, "low-input", 0.01, 0.1, 1);
  ...
  /*  input gamma spin  */
  spinbutton =
      gimp_prop_spin_button_new(filter_tool->config, "gamma", 0.01, 0.1, 2);
  gtk_box_pack_start(GTK_BOX(hbox2), spinbutton, FALSE, FALSE, 0);
  ...
  g_signal_connect(tool->gamma_linear, "value-changed",
                   G_CALLBACK(levels_linear_gamma_changed), tool);
  ...
  gimp_handle_bar_connect_events(GIMP_HANDLE_BAR(handle_bar), tool->output_bar);
  ...
  /*  low output spin  */
  tool->low_output_spinbutton = spinbutton = gimp_prop_spin_button_new(
      filter_tool->config, "low-output", 0.01, 0.1, 1);
  ...
  /*  high output spin  */
  tool->high_output_spinbutton = spinbutton = gimp_prop_spin_button_new(
      filter_tool->config, "high-output", 0.01, 0.1, 1);
  ...
  gimp_help_set_help_data(
      button, _("Adjust levels for all channels automatically"), NULL);
  gtk_widget_show(button);

  g_signal_connect(button, "clicked", G_CALLBACK(levels_stretch_callback),
                   tool);
  ...
  button =
      gimp_levels_tool_color_picker_new(tool, PICK_GAMMA | PICK_ALL_CHANNELS);
  gtk_box_pack_end(GTK_BOX(hbox), button, FALSE, FALSE, 0);
  gtk_widget_show(button);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(levels_to_curves_callback),
                   tool);
...
static void gimp_levels_tool_config_notify(GimpFilterTool *filter_tool,
  ...
  if (!levels_tool->channel_menu || !levels_tool->histogram_view)
    return;
  ...
  if (!strcmp(pspec->name, "trc")) {
    ...
    gimp_histogram_view_set_histogram(
        GIMP_HISTOGRAM_VIEW(levels_tool->histogram_view),
...
static void levels_input_adjust_by_color(GimpLevelsConfig *config, guint value,
                                         GimpHistogramChannel channel,
                                         const GimpRGB *color) {
  switch (value & 0xF) {
  case PICK_LOW_INPUT:
...
static void gimp_levels_tool_color_picked(GimpFilterTool *color_tool,
                                          gpointer identifier, gdouble x,
                                          gdouble y, const Babl *sample_format,
                                          const GimpRGB *color) {
  ...
  if (value & PICK_ALL_CHANNELS &&
    ...
    for (channel = GIMP_HISTOGRAM_RED; channel <= GIMP_HISTOGRAM_BLUE;
         channel++) {
...
static void levels_update_input_bar(GimpLevelsTool *tool) {
  ...
  case GIMP_HISTOGRAM_LUMINANCE: {
    ...
    for (i = 0; i < 256; i++) {
      value =
          gimp_operation_levels_map_input(config, config->channel, i / 255.0);
      v[i] = CLAMP(value, 0.0, 1.0) * 255.999;
    }

    gimp_color_bar_set_buffers(GIMP_COLOR_BAR(tool->input_bar), v, v, v);
  } break;
  ...
  case GIMP_HISTOGRAM_BLUE: {
    ...
    for (i = 0; i < 256; i++) {
      value = gimp_operation_levels_map_input(config, GIMP_HISTOGRAM_RED,
                                              i / 255.0);
      r[i] = CLAMP(value, 0.0, 1.0) * 255.999;

      value = gimp_operation_levels_map_input(config, GIMP_HISTOGRAM_GREEN,
                                              i / 255.0);
      g[i] = CLAMP(value, 0.0, 1.0) * 255.999;

      value = gimp_operation_levels_map_input(config, GIMP_HISTOGRAM_BLUE,
                                              i / 255.0);
      b[i] = CLAMP(value, 0.0, 1.0) * 255.999;
    }

    gimp_color_bar_set_buffers(GIMP_COLOR_BAR(tool->input_bar), r, g, b);
  } break;
  }
}
...
static void levels_to_curves_callback(GtkWidget *widget,
  ...
  gimp_filter_tool_edit_as(filter_tool, "gimp-curves-tool",
                           GIMP_CONFIG(curves));
