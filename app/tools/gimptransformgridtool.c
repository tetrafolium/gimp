...
static void gimp_transform_grid_tool_control(GimpTool *tool,
                                             ...
                                             GimpDisplay *display);
static void
gimp_transform_grid_tool_button_press(GimpTool *tool, const GimpCoords *coords,
                                      guint32 time, GdkModifierType state,
                                      GimpButtonPressType press_type,
                                      GimpDisplay *display);
static void gimp_transform_grid_tool_button_release(
    GimpTool *tool, const GimpCoords *coords, guint32 time,
...
static GeglBuffer *gimp_transform_grid_tool_transform(
    GimpTransformTool *tr_tool, GList *objects, GeglBuffer *orig_buffer,
    gint orig_offset_x, gint orig_offset_y, GimpColorProfile **buffer_profile,
...
static GeglBuffer *gimp_transform_grid_tool_real_transform(
    GimpTransformGridTool *tg_tool, GList *objects, GeglBuffer *orig_buffer,
    gint orig_offset_x, gint orig_offset_y, GimpColorProfile **buffer_profile,
    gint *new_offset_x, gint *new_offset_y);
...
gimp_transform_grid_tool_widget_changed(GimpToolWidget *widget,
                                        GimpTransformGridTool *tg_tool);
static void gimp_transform_grid_tool_widget_response(
    GimpToolWidget *widget, gint response_id, GimpTransformGridTool *tg_tool);
...
static void gimp_transform_grid_tool_image_linked_items_changed(
    GimpImage *image, GimpTransformGridTool *tg_tool);
...
static Filter *filter_new(GimpTransformGridTool *tg_tool,
                          GimpDrawable *drawable, GimpDrawable *root_drawable,
                          gboolean add_filter);
...
G_DEFINE_TYPE(GimpTransformGridTool, gimp_transform_grid_tool,
...
static void gimp_transform_grid_tool_init(GimpTransformGridTool *tg_tool) {
  GimpTool *tool = GIMP_TOOL(tg_tool);

  gimp_tool_control_set_scroll_lock(tool->control, TRUE);
  gimp_tool_control_set_preserve(tool->control, FALSE);
  gimp_tool_control_set_dirty_mask(
      tool->control, GIMP_DIRTY_IMAGE_SIZE | GIMP_DIRTY_IMAGE_STRUCTURE |
                         GIMP_DIRTY_DRAWABLE | GIMP_DIRTY_SELECTION |
  ...
  gimp_tool_control_set_action_opacity(
      tool->control, "tools/tools-transform-preview-opacity-set");
...
static gboolean gimp_transform_grid_tool_initialize(GimpTool *tool,
  ...
  GimpTransformGridOptions *tg_options =
      GIMP_TRANSFORM_GRID_TOOL_GET_OPTIONS(tool);
  ...
  g_signal_connect(
      ...
      G_CALLBACK(gimp_transform_grid_tool_image_linked_items_changed), tg_tool);
...
static void gimp_transform_grid_tool_button_release(
  ...
  if (tg_tool->grab_widget) {
    gimp_tool_widget_button_release(tg_tool->grab_widget, coords, time, state,
                                    release_type);
...
static void gimp_transform_grid_tool_modifier_key(GimpTool *tool,
  ...
  if (tg_tool->widget) {
    ...
    GimpTransformGridOptions *options =
        GIMP_TRANSFORM_GRID_TOOL_GET_OPTIONS(tool);

    if (key == gimp_get_constrain_behavior_mask()) {
      g_object_set(options, "frompivot-scale", !options->frompivot_scale,
      ...
      g_object_set(options, "cornersnap", !options->cornersnap,
...
static gboolean gimp_transform_grid_tool_undo(GimpTool *tool,
  ...
  if (direction != tr_options->direction) {
    g_object_set(tr_options, "direction", direction, NULL);
  }
...
static void gimp_transform_grid_tool_options_notify(GimpTool *tool,
  ...
  } else if (!strcmp(pspec->name, "preview-linked") && tg_tool->filters) {
...
static void gimp_transform_grid_tool_draw(GimpDrawTool *draw_tool) {
  ...
  if (tr_options->type == GIMP_TRANSFORM_TYPE_LAYER ||
    ...
    tg_tool->preview = gimp_draw_tool_add_transform_preview(
        draw_tool, pickable, &matrix, tr_tool->x1, tr_tool->y1, tr_tool->x2,
        tr_tool->y2);
  ...
  if (tr_options->type == GIMP_TRANSFORM_TYPE_SELECTION) {
    ...
    gimp_channel_boundary(gimp_image_get_mask(image), &segs_in, &segs_out,
                          &n_segs_in, &n_segs_out, 0, 0, 0, 0);

    if (segs_in) {
      tg_tool->boundary_in = gimp_draw_tool_add_boundary(
          draw_tool, segs_in, n_segs_in, &matrix, 0, 0);
    ...
    if (segs_out) {
      tg_tool->boundary_out = gimp_draw_tool_add_boundary(
          draw_tool, segs_out, n_segs_out, &matrix, 0, 0);
    ...
    if (vectors) {
      ...
      while ((stroke = gimp_vectors_stroke_get_next(vectors, stroke))) {
        ...
        if (coords && coords->len) {
          item = gimp_draw_tool_add_strokes(
              draw_tool, &g_array_index(coords, GimpCoords, 0), coords->len,
              &matrix, FALSE);

          g_ptr_array_add(tg_tool->strokes, item);
          g_object_weak_ref(G_OBJECT(item), (GWeakNotify)g_ptr_array_remove,
                            tg_tool->strokes);
...
static void gimp_transform_grid_tool_recalc_matrix(GimpTransformTool *tr_tool) {
  ...
  GimpTransformGridOptions *tg_options =
      GIMP_TRANSFORM_GRID_TOOL_GET_OPTIONS(tr_tool);
  ...
  if (GIMP_TRANSFORM_GRID_TOOL_GET_CLASS(tg_tool)->info_to_matrix) {
    ...
    forward_transform_valid =
        gimp_transform_grid_tool_info_to_matrix(tg_tool, &forward_transform);

    tg_tool->trans_info = tg_tool->trans_infos[GIMP_TRANSFORM_BACKWARD];
    backward_transform_valid =
        gimp_transform_grid_tool_info_to_matrix(tg_tool, &backward_transform);
    ...
    if (GIMP_TRANSFORM_GRID_TOOL_GET_CLASS(tg_tool)->matrix_to_info &&
      ...
      case GIMP_TRANSFORM_FORWARD:
        if (forward_transform_valid) {
          ...
          tg_tool->trans_info = tg_tool->trans_infos[GIMP_TRANSFORM_BACKWARD];
          gimp_transform_grid_tool_matrix_to_info(tg_tool, &backward_transform);
      ...
      case GIMP_TRANSFORM_BACKWARD:
        if (backward_transform_valid) {
          ...
          tg_tool->trans_info = tg_tool->trans_infos[GIMP_TRANSFORM_FORWARD];
          gimp_transform_grid_tool_matrix_to_info(tg_tool, &forward_transform);
    ...
    tr_tool->transform_valid =
        forward_transform_valid && backward_transform_valid;
  }
...
gimp_transform_grid_tool_get_undo_desc(GimpTransformTool *tr_tool) {
  ...
  if (GIMP_TRANSFORM_GRID_TOOL_GET_CLASS(tg_tool)->matrix_to_info) {
    ...
    result =
        GIMP_TRANSFORM_GRID_TOOL_GET_CLASS(tg_tool)->get_undo_desc(tg_tool);
    ...
    result = GIMP_TRANSFORM_TOOL_CLASS(parent_class)->get_undo_desc(tr_tool);
  }
...
static GeglBuffer *gimp_transform_grid_tool_transform(
   ...
   */
  new_buffer = GIMP_TRANSFORM_GRID_TOOL_GET_CLASS(tg_tool)->transform(
      tg_tool, objects, orig_buffer, orig_offset_x, orig_offset_y,
      buffer_profile, new_offset_x, new_offset_y);

  gimp_image_undo_push(image, GIMP_TYPE_TRANSFORM_GRID_TOOL_UNDO,
                       GIMP_UNDO_TRANSFORM_GRID, NULL, 0, "transform-tool",
                       tg_tool, NULL);
...
gimp_transform_grid_tool_real_update_widget(GimpTransformGridTool *tg_tool) {
  if (GIMP_TRANSFORM_GRID_TOOL_GET_CLASS(tg_tool)->info_to_matrix) {
    ...
    g_object_set(tg_tool->widget, "transform", &transform, NULL);
  }
}
...
static GeglBuffer *gimp_transform_grid_tool_real_transform(
  ...
  return GIMP_TRANSFORM_TOOL_CLASS(parent_class)
      ->transform(tr_tool, objects, orig_buffer, orig_offset_x, orig_offset_y,
                  buffer_profile, new_offset_x, new_offset_y);
}
...
static void gimp_transform_grid_tool_halt(GimpTransformGridTool *tg_tool) {
  ...
  GimpTransformTool *tr_tool = GIMP_TRANSFORM_TOOL(tg_tool);
  GimpTransformGridOptions *tg_options =
      GIMP_TRANSFORM_GRID_TOOL_GET_OPTIONS(tg_tool);

  if (tool->display) {
    GimpImage *image = gimp_display_get_image(tool->display);

    g_signal_handlers_disconnect_by_func(
        image, gimp_transform_grid_tool_image_linked_items_changed, tg_tool);
  }
  ...
  if (tg_options->direction_chain_button) {
    g_object_set(tg_options, "direction-linked", FALSE, NULL);
...
static void gimp_transform_grid_tool_dialog(GimpTransformGridTool *tg_tool) {
  ...
  ok_button_label =
      GIMP_TRANSFORM_GRID_TOOL_GET_CLASS(tg_tool)->ok_button_label;

  tg_tool->gui =
      gimp_tool_gui_new(tool_info, NULL, NULL, NULL, NULL,
                        gimp_widget_get_monitor(GTK_WIDGET(shell)), TRUE, NULL);
  ...
  gimp_tool_gui_set_alternative_button_order(tg_tool->gui, RESPONSE_RESET,
                                             RESPONSE_READJUST, GTK_RESPONSE_OK,
                                             GTK_RESPONSE_CANCEL, -1);

  g_signal_connect(tg_tool->gui, "response",
                   G_CALLBACK(gimp_transform_grid_tool_response), tg_tool);
...
gimp_transform_grid_tool_get_widget(GimpTransformGridTool *tg_tool) {
  static const gchar *properties[] = {
      "constrain-move",  "constrain-scale",       "constrain-rotate",
      "constrain-shear", "constrain-perspective", "frompivot-scale",
      "frompivot-shear", "frompivot-perspective", "cornersnap",
      "fixedpivot"};
  ...
  if (GIMP_TRANSFORM_GRID_TOOL_GET_CLASS(tg_tool)->get_widget) {
    ...
    g_object_bind_property(G_OBJECT(options), "grid-type", G_OBJECT(widget),
                           "guide-type",
                           G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);
    g_object_bind_property(G_OBJECT(options), "grid-size", G_OBJECT(widget),
                           "n-guides",
                           G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);

    for (i = 0; i < G_N_ELEMENTS(properties); i++)
      g_object_bind_property(G_OBJECT(options), properties[i], G_OBJECT(widget),
                             properties[i],
                             G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);
...
gimp_transform_grid_tool_update_widget(GimpTransformGridTool *tg_tool) {
  if (tg_tool->widget &&
    ...
    g_signal_handlers_block_by_func(
        tg_tool->widget, G_CALLBACK(gimp_transform_grid_tool_widget_changed),
        tg_tool);
...
static void gimp_transform_grid_tool_response(GimpToolGui *gui,
  ...
  case RESPONSE_READJUST:
    if (GIMP_TRANSFORM_GRID_TOOL_GET_CLASS(tg_tool)->readjust &&
      ...
      /*  save the current transformation info  */
      memcpy(old_trans_infos, tg_tool->trans_infos, sizeof(old_trans_infos));
      ...
      if (transform_valid) {
        ...
        memcpy(tg_tool->trans_infos, old_trans_infos, sizeof(old_trans_infos));
...
gimp_transform_grid_tool_update_sensitivity(GimpTransformGridTool *tg_tool) {
  ...
  gimp_tool_gui_set_response_sensitive(tg_tool->gui, RESPONSE_READJUST,
                                       tr_tool->transform_valid);
}
...
gimp_transform_grid_tool_update_preview(GimpTransformGridTool *tg_tool) {
  ...
  if (tg_options->show_preview &&
    ...
    if (!tg_tool->filters) {
      tg_tool->filters = g_hash_table_new_full(
          g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)filter_free);
    ...
    while (g_hash_table_iter_next(&iter, (gpointer *)&drawable,
                                  (gpointer *)&filter)) {
      GimpMatrix3 transform;
      GeglRectangle bounds;
      gint offset_x;
      ...
      gimp_transform_resize_boundary(
          &tr_tool->transform,
          gimp_item_get_clip(GIMP_ITEM(filter->root_drawable),
                             tr_options->clip),
          offset_x, offset_y, offset_x + width, offset_y + height, &x1, &y1,
          &x2, &y2);
      ...
      if (!gegl_rectangle_equal(&bounds, &filter->bounds)) {
        ...
        gegl_node_set(filter->crop_node, "x", (gdouble)bounds.x, "y",
                      (gdouble)bounds.y, "width", (gdouble)bounds.width,
                      "height", (gdouble)bounds.height, NULL);

        update = TRUE;
      }

      if (GIMP_IS_LAYER(drawable)) {
        gimp_drawable_filter_set_add_alpha(filter->filter,
                                           tr_options->interpolation !=
      ...
      if (update) {
        if (tg_options->synchronous_preview) {
          g_signal_handlers_block_by_func(
              filter->filter, G_CALLBACK(gimp_transform_grid_tool_filter_flush),
              tg_tool);
        }
  ...
  if (tg_tool->preview) {
    if (tg_options->show_preview &&
      ...
      g_object_set(tg_tool->preview, "transform", &tr_tool->transform, "clip",
                   gimp_item_get_clip(GIMP_ITEM(tool->drawables->data),
                                      tr_options->clip),
                   "opacity", tg_options->preview_opacity, NULL);
  ...
  if (tg_tool->boundary_in) {
    ...
    gimp_canvas_item_set_visible(tg_tool->boundary_in,
                                 tr_tool->transform_valid);
    g_object_set(tg_tool->boundary_in, "transform", &tr_tool->transform, NULL);
  ...
  if (tg_tool->boundary_out) {
    ...
    gimp_canvas_item_set_visible(tg_tool->boundary_out,
                                 tr_tool->transform_valid);
    g_object_set(tg_tool->boundary_out, "transform", &tr_tool->transform, NULL);
  ...
  for (i = 0; i < tg_tool->strokes->len; i++) {
    ...
    gimp_canvas_item_set_visible(item, tr_tool->transform_valid);
    g_object_set(item, "transform", &tr_tool->transform, NULL);
...
gimp_transform_grid_tool_hide_selected_objects(GimpTransformGridTool *tg_tool,
  ...
  if (options->show_preview) {
    if (tr_options->type == GIMP_TRANSFORM_TYPE_IMAGE) {
      ...
      gimp_display_shell_set_show_image(gimp_display_get_shell(display), FALSE);
    } else {
      ...
      for (iter = objects; iter; iter = iter->next) {
        if (tr_options->type == GIMP_TRANSFORM_TYPE_LAYER &&
            !options->composited_preview && GIMP_IS_DRAWABLE(iter->data) &&
            !GIMP_IS_LAYER_MASK(iter->data) &&
            gimp_item_get_visible(iter->data) &&
            gimp_channel_is_empty(gimp_image_get_mask(image))) {
          tg_tool->hidden_objects =
              g_list_prepend(tg_tool->hidden_objects, iter->data);
  ...
  if (tg_tool->hidden_objects) {
    ...
    for (iter = tg_tool->hidden_objects; iter; iter = iter->next) {
      if (GIMP_IS_ITEM(iter->data)) {
        gimp_item_set_visible(GIMP_ITEM(iter->data), TRUE, FALSE);
      } else {
...
static void gimp_transform_grid_tool_add_filter(GimpDrawable *drawable,
  ...
  if (GIMP_IS_LAYER(drawable)) {
    gimp_layer_get_effective_mode(GIMP_LAYER(drawable), &mode, NULL, NULL,
                                  NULL);
  }
  ...
  if (mode != GIMP_LAYER_MODE_PASS_THROUGH) {
    ...
    gimp_container_foreach(container,
                           (GFunc)gimp_transform_grid_tool_add_filter, data);
  }
...
gimp_transform_grid_tool_remove_filter(GimpDrawable *drawable,
  ...
  if (!filter->filter) {
    ...
    gimp_container_foreach(
        container, (GFunc)gimp_transform_grid_tool_remove_filter, tg_tool);
  }
...
static void gimp_transform_grid_tool_effective_mode_changed(
    GimpLayer *layer, GimpTransformGridTool *tg_tool) {
...
static Filter *filter_new(GimpTransformGridTool *tg_tool,
  ...
  if (add_filter) {
    ...
    filter->transform_node = gegl_node_new_child(
        node, "operation", "gegl:transform", "near-z", GIMP_TRANSFORM_NEAR_Z,
        "sampler", GEGL_SAMPLER_NEAREST, NULL);

    filter->crop_node =
        gegl_node_new_child(node, "operation", "gegl:crop", NULL);

    gegl_node_link_many(input_node, filter->transform_node, filter->crop_node,
                        output_node, NULL);

    gimp_gegl_node_set_underlying_operation(node, filter->transform_node);

    filter->filter = gimp_drawable_filter_new(
        drawable, GIMP_TRANSFORM_TOOL_GET_CLASS(tg_tool)->undo_desc, node,
    ...
    g_signal_connect(filter->filter, "flush",
  ...
  if (GIMP_IS_GROUP_LAYER(drawable)) {
    g_signal_connect(
        ...
        G_CALLBACK(gimp_transform_grid_tool_effective_mode_changed), tg_tool);
  }
...
static void filter_free(Filter *filter) {
  ...
  if (GIMP_IS_GROUP_LAYER(filter->drawable)) {
    g_signal_handlers_disconnect_by_func(
        filter->drawable, gimp_transform_grid_tool_effective_mode_changed,
        filter->tg_tool);
  }
...
static UndoInfo *undo_info_new(void) { return g_slice_new0(UndoInfo); }

static void undo_info_free(UndoInfo *info) { g_slice_free(UndoInfo, info); }
