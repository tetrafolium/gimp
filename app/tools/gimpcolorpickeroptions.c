...
gimp_color_picker_options_class_init(GimpColorPickerOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(
      object_class, PROP_PICK_TARGET, "pick-target", _("Pick Target"),
      _("Choose what the color picker will do"), GIMP_TYPE_COLOR_PICK_TARGET,
      GIMP_COLOR_PICK_TARGET_FOREGROUND, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_USE_INFO_WINDOW,
                           "use-info-window", _("Use info window"),
                           _("Open a floating dialog to view picked "
                             "color values in various color models"),
                           FALSE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_FRAME1_MODE, "frame1-mode",
                        "Frame 1 Mode", NULL, GIMP_TYPE_COLOR_PICK_MODE,
                        GIMP_COLOR_PICK_MODE_PIXEL, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_FRAME2_MODE, "frame2-mode",
                        "Frame 2 Mode", NULL, GIMP_TYPE_COLOR_PICK_MODE,
...
static void gimp_color_picker_options_get_property(GObject *object,
  ...
  switch (property_id) {
  case PROP_SAMPLE_AVERAGE:
    g_value_set_boolean(value, GIMP_COLOR_OPTIONS(options)->sample_average);
    break;
  case PROP_PICK_TARGET:
...
GtkWidget *gimp_color_picker_options_gui(GimpToolOptions *tool_options) {
  ...
  str =
      g_strdup_printf(_("Pick Target  (%s)"), gimp_get_mod_string(toggle_mask));
