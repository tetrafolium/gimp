...
static void gimp_bucket_fill_tool_motion(GimpTool *tool,
                                         ...
                                         GimpDisplay *display);
static void
gimp_bucket_fill_tool_button_release(GimpTool *tool, const GimpCoords *coords,
...
static gboolean gimp_bucket_fill_tool_coords_in_active_pickable(
    GimpBucketFillTool *tool, GimpDisplay *display, const GimpCoords *coords);
...
void gimp_bucket_fill_tool_register(GimpToolRegisterCallback callback,
                                    gpointer data) {
  (*callback)(
      GIMP_TYPE_BUCKET_FILL_TOOL, GIMP_TYPE_BUCKET_FILL_OPTIONS,
      ...
      "gimp-bucket-fill-tool", _("Bucket Fill"),
      _("Bucket Fill Tool: Fill selected area with a color or pattern"),
      N_("_Bucket Fill"), "<shift>B", NULL, GIMP_HELP_TOOL_BUCKET_FILL,
      GIMP_ICON_TOOL_BUCKET_FILL, data);
}
...
static void gimp_bucket_fill_tool_constructed(GObject *object) {
  ...
  gimp_tool_control_set_motion_mode(
      tool->control, options->fill_area == GIMP_BUCKET_FILL_LINE_ART
                         ? GIMP_MOTION_MODE_EXACT
                         : GIMP_MOTION_MODE_COMPRESS);

  line_art = gimp_line_art_new();
  g_object_bind_property(options, "fill-transparent", line_art,
                         "select-transparent",
                         G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);
  g_object_bind_property(options, "line-art-threshold", line_art, "threshold",
                         G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);
  g_object_bind_property(options, "line-art-max-grow", line_art, "max-grow",
                         G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);
  g_object_bind_property(options, "line-art-max-gap-length", line_art,
                         "spline-max-length",
  ...
  g_signal_connect_swapped(
      line_art, "computing-start",
      G_CALLBACK(gimp_bucket_fill_tool_line_art_computing_start), tool);
  g_signal_connect_swapped(
      line_art, "computing-end",
      G_CALLBACK(gimp_bucket_fill_tool_line_art_computing_end), tool);
...
static void gimp_bucket_fill_tool_finalize(GObject *object) {
  ...
  if (tool->priv->line_art_image) {
    g_signal_handlers_disconnect_by_data(
        gimp_image_get_layers(tool->priv->line_art_image), tool);
  ...
  if (tool->priv->line_art_shell) {
    g_signal_handlers_disconnect_by_func(
        tool->priv->line_art_shell, gimp_bucket_fill_tool_reset_line_art, tool);
...
static gboolean gimp_bucket_fill_tool_coords_in_active_pickable(
  ...
  return gimp_image_coords_in_active_pickable(image, coords, shell->show_all,
                                              sample_merged, TRUE);
}
...
static void gimp_bucket_fill_tool_start(GimpBucketFillTool *tool,
  ...
  tool->priv->filter =
      gimp_drawable_filter_new(drawables->data, _("Bucket fill"),
                               tool->priv->graph, GIMP_ICON_TOOL_BUCKET_FILL);
  ...
  gimp_drawable_filter_set_mode(
      tool->priv->filter, gimp_context_get_paint_mode(context),
  ...
  g_signal_connect(tool->priv->filter, "flush",
                   G_CALLBACK(gimp_bucket_fill_tool_filter_flush), tool);
}
  ...
  if (tool->priv->filter) {
    ...
    if (options->fill_area == GIMP_BUCKET_FILL_SIMILAR_COLORS) {
      ...
      fill = gimp_drawable_get_bucket_fill_buffer(
          drawable, fill_options, options->fill_transparent,
          options->fill_criterion, options->threshold / 255.0, shell->show_all,
          options->sample_merged, options->diagonal_neighbors, x, y,
          &tool->priv->fill_mask, &x, &y, NULL, NULL);
    } else {
      gint source_off_x = 0;
      gint source_off_y = 0;
      ...
      fill = gimp_drawable_get_line_art_fill_buffer(
          drawable, tool->priv->line_art, fill_options,
          options->line_art_source == GIMP_LINE_ART_SOURCE_SAMPLE_MERGED, x, y,
          &tool->priv->fill_mask, &x, &y, NULL, NULL);
      if (options->line_art_source != GIMP_LINE_ART_SOURCE_SAMPLE_MERGED) {
        gint off_x, off_y;
    ...
    if (fill) {
      gegl_node_set(tool->priv->fill_node, "buffer", fill, NULL);
      gegl_node_set(tool->priv->offset_node, "x", x, "y", y, NULL);
...
static void gimp_bucket_fill_tool_commit(GimpBucketFillTool *tool) {
  if (tool->priv->filter) {
    gimp_drawable_filter_commit(tool->priv->filter, GIMP_PROGRESS(tool), FALSE);
...
static void gimp_bucket_fill_tool_create_graph(GimpBucketFillTool *tool) {
  ...
  g_return_if_fail(!tool->priv->graph && !tool->priv->fill_node &&
                   !tool->priv->offset_node);

  graph = gegl_node_new();

  fill_node =
      gegl_node_new_child(graph, "operation", "gegl:buffer-source", NULL);
  offset_node = gegl_node_new_child(graph, "operation", "gegl:translate", NULL);
...
static void gimp_bucket_fill_tool_button_press(GimpTool *tool,
  ...
  if (press_type == GIMP_BUTTON_PRESS_NORMAL &&
      gimp_bucket_fill_tool_coords_in_active_pickable(bucket_tool, display,
                                                      coords)) {
    GimpContext *context = GIMP_CONTEXT(options);
    GimpFillOptions *fill_options;
    GError *error = NULL;
...
static void gimp_bucket_fill_tool_motion(GimpTool *tool,
  ...
  if (gimp_bucket_fill_tool_coords_in_active_pickable(bucket_tool, display,
                                                      coords) &&
  ...
  if (key == GDK_MOD1_MASK) {
    ...
    } else /* release */
    {
      g_object_set(options, "fill-mode",
                   GIMP_BUCKET_FILL_TOOL(tool)->priv->fill_mode, NULL);
    }
  } else if (key == gimp_get_toggle_behavior_mask()) {
    GimpToolInfo *info =
        gimp_get_tool_info(display->gimp, "gimp-color-picker-tool");
    ...
    if (press) {
      ...
      default:
        ...
        g_object_set(options, "fill-area", GIMP_BUCKET_FILL_SIMILAR_COLORS,
                     NULL);
        break;
      }
    } else /* release */
    {
      g_object_set(options, "fill-area",
                   GIMP_BUCKET_FILL_TOOL(tool)->priv->fill_area, NULL);
    }
  }
}
...
static void gimp_bucket_fill_tool_options_notify(GimpTool *tool,
  ...
  if (!strcmp(pspec->name, "fill-area")) {
     ...
     */
    gimp_tool_control_set_motion_mode(
        tool->control, bucket_options->fill_area == GIMP_BUCKET_FILL_LINE_ART
                           ? GIMP_MOTION_MODE_EXACT
                           : GIMP_MOTION_MODE_COMPRESS);
...
static void gimp_bucket_fill_tool_reset_line_art(GimpBucketFillTool *tool) {
  ...
  if (shell != tool->priv->line_art_shell) {
    if (tool->priv->line_art_shell) {
      g_signal_handlers_disconnect_by_func(tool->priv->line_art_shell,
