...
static gboolean gimp_operation_tool_initialize(GimpTool *tool,
                                               GimpDisplay *display,
                                               GError **error);
static void gimp_operation_tool_control(GimpTool *tool, GimpToolAction action,
                                        GimpDisplay *display);
...
G_DEFINE_TYPE(GimpOperationTool, gimp_operation_tool, GIMP_TYPE_FILTER_TOOL)

#define parent_class gimp_operation_tool_parent_class

void gimp_operation_tool_register(GimpToolRegisterCallback callback,
                                  gpointer data) {
  (*callback)(GIMP_TYPE_OPERATION_TOOL, GIMP_TYPE_FILTER_OPTIONS,
              gimp_color_options_gui,
              GIMP_CONTEXT_PROP_MASK_FOREGROUND |
                  GIMP_CONTEXT_PROP_MASK_BACKGROUND,
              "gimp-operation-tool", _("GEGL Operation"),
              _("Operation Tool: Use an arbitrary GEGL operation"), NULL, NULL,
              NULL, GIMP_HELP_TOOL_GEGL, GIMP_ICON_GEGL, data);
}

static void gimp_operation_tool_class_init(GimpOperationToolClass *klass) {
...
static void gimp_operation_tool_init(GimpOperationTool *op_tool) {}
...
static void gimp_operation_tool_dialog(GimpFilterTool *filter_tool) {
  ...
  gtk_box_pack_start(GTK_BOX(main_vbox), options_sw, TRUE, TRUE, 0);
...
#define HAS_KEY(p, k, v) gimp_gegl_param_spec_has_key(p, k, v)
      ...
      if (G_IS_PARAM_SPEC_INT(pspec_x)) {
        ...
        g_object_set(filter_tool->config, pspecs[0], g_value_get_int(&value_x),
                     pspecs[1], g_value_get_int(&value_y), NULL);
        ...
        g_object_set(filter_tool->config, pspecs[0],
                     g_value_get_double(&value_x), pspecs[1],
                     g_value_get_double(&value_y), NULL);
      } else {
        g_warning("%s: unhandled param spec of type %s", G_STRFUNC,
                  G_PARAM_SPEC_TYPE_NAME(pspec_x));
      }
    ...
    g_object_set(filter_tool->config, pspecs[0], color, NULL);
  }
...
static void gimp_operation_tool_sync_op(GimpOperationTool *op_tool,
  ...
  pspecs = g_object_class_list_properties(
      G_OBJECT_GET_CLASS(filter_tool->config), &n_pspecs);
...
static void gimp_operation_tool_create_gui(GimpOperationTool *op_tool) {
  GimpFilterTool *filter_tool = GIMP_FILTER_TOOL(op_tool);
  GtkWidget *options_gui;
  gint off_x, off_y;
  GeglRectangle area;
  ...
  options_gui = gimp_prop_gui_new(
      G_OBJECT(filter_tool->config), G_TYPE_FROM_INSTANCE(filter_tool->config),
      0, &area, GIMP_CONTEXT(GIMP_TOOL_GET_OPTIONS(op_tool)),
      (GimpCreatePickerFunc)gimp_filter_tool_add_color_picker,
      (GimpCreateControllerFunc)gimp_filter_tool_add_controller, filter_tool);
  ...
  if (input_pads) {
    ...
    for (i = 0; input_pads[i]; i++) {
      ...
      label = g_regex_replace(regex, input_pads[i], -1, 0, _("Aux\\1 Input"), 0,
                              NULL);

      input = gimp_operation_tool_aux_input_new(op_tool, filter_tool->operation,
                                                input_pads[i], label);
...
static void gimp_operation_tool_add_gui(GimpOperationTool *op_tool) {
  ...
  for (list = op_tool->aux_inputs; list; list = g_list_next(list)) {
    ...
    gtk_box_pack_start(GTK_BOX(options_box), input->box, FALSE, FALSE, 0);
  ...
  gtk_scrolled_window_set_policy(
      GTK_SCROLLED_WINDOW(options_sw), GTK_POLICY_NEVER,
      scrolling ? GTK_POLICY_AUTOMATIC : GTK_POLICY_NEVER);
...
static void gimp_operation_tool_aux_input_notify(GimpBufferSourceBox *box,
   ...
   */
  if (filter_tool->config)
    g_signal_emit_by_name(filter_tool->config, "notify", NULL);
}

static AuxInput *gimp_operation_tool_aux_input_new(GimpOperationTool *op_tool,
                                                   GeglNode *operation,
                                                   const gchar *input_pad,
                                                   const gchar *label) {
  ...
  input->node =
      gegl_node_new_child(NULL, "operation", "gegl:buffer-source", NULL);

  gegl_node_connect_to(input->node, "output", operation, input_pad);
  ...
  g_signal_connect(input->box, "notify::pickable",
                   G_CALLBACK(gimp_operation_tool_aux_input_notify), input);
...
static void gimp_operation_tool_aux_input_clear(AuxInput *input) {
  ...
  g_object_set(input->box, "pickable", NULL, NULL);
}
...
static void gimp_operation_tool_unlink_chains(GimpOperationTool *op_tool) {
  ...
  while (chains) {
    ...
    g_object_set_data(G_OBJECT(chain), "was-active", GINT_TO_POINTER(active));
  ...
  while (chains) {
    ...
    if (g_object_get_data(G_OBJECT(chain), "was-active")) {
      ...
      if (g_value_transform(&values[0], &double_x) &&
          ...
          g_value_get_double(&double_x) == g_value_get_double(&double_y)) {
