...
typedef enum { DIRECTION_NONE, DIRECTION_LEFT, DIRECTION_RIGHT } Direction;
...
typedef struct {
  ...
  /* copy of the gradient at the beginning of the operation, owned by the
   * gradient info, or NULL, if the gradient isn't affected
   */
...
static gint
gimp_gradient_tool_editor_line_add_slider(GimpToolLine *line, gdouble value,
                                          GimpGradientTool *gradient_tool);
static void gimp_gradient_tool_editor_line_prepare_to_remove_slider(
    GimpToolLine *line, gint slider, gboolean remove,
    GimpGradientTool *gradient_tool);
static void
gimp_gradient_tool_editor_line_remove_slider(GimpToolLine *line, gint slider,
                                             GimpGradientTool *gradient_tool);
static void gimp_gradient_tool_editor_line_selection_changed(
    GimpToolLine *line, GimpGradientTool *gradient_tool);
static gboolean gimp_gradient_tool_editor_line_handle_clicked(
    GimpToolLine *line, gint handle, GdkModifierType state,
    GimpButtonPressType press_type, GimpGradientTool *gradient_tool);

static void
gimp_gradient_tool_editor_gui_response(GimpToolGui *gui, gint response_id,
                                       GimpGradientTool *gradient_tool);

static void gimp_gradient_tool_editor_color_entry_color_clicked(
    GimpColorButton *button, GimpGradientTool *gradient_tool);
static void gimp_gradient_tool_editor_color_entry_color_changed(
    GimpColorButton *button, GimpGradientTool *gradient_tool);
static void gimp_gradient_tool_editor_color_entry_color_response(
    GimpColorButton *button, GimpColorDialogState state,
    GimpGradientTool *gradient_tool);

static void gimp_gradient_tool_editor_color_entry_type_changed(
    GtkComboBox *combo, GimpGradientTool *gradient_tool);

static void gimp_gradient_tool_editor_endpoint_se_value_changed(
    GimpSizeEntry *se, GimpGradientTool *gradient_tool);

static void gimp_gradient_tool_editor_stop_se_value_changed(
    GimpSizeEntry *se, GimpGradientTool *gradient_tool);
...
static void gimp_gradient_tool_editor_midpoint_se_value_changed(
    GimpSizeEntry *se, GimpGradientTool *gradient_tool);

static void gimp_gradient_tool_editor_midpoint_type_changed(
    GtkComboBox *combo, GimpGradientTool *gradient_tool);

static void gimp_gradient_tool_editor_midpoint_color_changed(
    GtkComboBox *combo, GimpGradientTool *gradient_tool);

static void gimp_gradient_tool_editor_midpoint_new_stop_clicked(
    GtkWidget *button, GimpGradientTool *gradient_tool);
static void gimp_gradient_tool_editor_midpoint_center_clicked(
    GtkWidget *button, GimpGradientTool *gradient_tool);
...
static GtkWidget *gimp_gradient_tool_editor_color_entry_new(
    GimpGradientTool *gradient_tool, const gchar *title, Direction direction,
    GtkWidget *chain_button, GtkWidget **color_panel, GtkWidget **type_combo);
static void
...
static gboolean gimp_gradient_tool_editor_gradient_info_is_trivial(
    GimpGradientTool *gradient_tool, const GradientInfo *info);
...
static void gimp_gradient_tool_editor_line_prepare_to_remove_slider(
  ...
  if (remove) {
    ...
    if (info->added_handle == slider) {
      ...
      tentative_gradient = GIMP_GRADIENT(
          gimp_data_duplicate(GIMP_DATA(gradient_tool->gradient)));

      seg = gimp_gradient_tool_editor_handle_get_segment(gradient_tool, slider);

      i = gimp_gradient_segment_range_get_n_segments(
              gradient_tool->gradient, gradient_tool->gradient->segments, seg) -
          1;

      seg = gimp_gradient_segment_get_nth(tentative_gradient->segments, i);

      gimp_gradient_segment_range_merge(tentative_gradient, seg, seg->next,
                                        NULL, NULL);
    }

    gimp_gradient_tool_set_tentative_gradient(gradient_tool,
                                              tentative_gradient);
...
static void gimp_gradient_tool_editor_color_entry_color_changed(
  ...
  direction = GPOINTER_TO_INT(g_object_get_data(
      G_OBJECT(button), "gimp-gradient-tool-editor-direction"));
...
static void gimp_gradient_tool_editor_color_entry_type_changed(
  ...
  direction = GPOINTER_TO_INT(g_object_get_data(
      G_OBJECT(combo), "gimp-gradient-tool-editor-direction"));
...
static void gimp_gradient_tool_editor_endpoint_se_value_changed(
  ...
  switch (selection) {
  case GIMP_TOOL_LINE_HANDLE_START:
    g_object_set(gradient_tool->widget, "x1", x, "y1", y, NULL);
    break;

  case GIMP_TOOL_LINE_HANDLE_END:
    g_object_set(gradient_tool->widget, "x2", x, "y2", y, NULL);
    break;
...
static void gimp_gradient_tool_editor_stop_se_value_changed(
  ...
  gimp_gradient_segment_range_compress(gradient_tool->gradient, seg, seg,
                                       seg->left, value);
  gimp_gradient_segment_range_compress(gradient_tool->gradient, seg->next,
                                       seg->next, value, seg->next->right);
...
static void gimp_gradient_tool_editor_midpoint_center_clicked(
  ...
  gimp_gradient_segment_range_recenter_handles(gradient_tool->gradient, seg,
                                               seg);
...
static gboolean gimp_gradient_tool_editor_is_gradient_editable(
    GimpGradientTool *gradient_tool) {
  GimpGradientOptions *options = GIMP_GRADIENT_TOOL_GET_OPTIONS(gradient_tool);
...
static gboolean gimp_gradient_tool_editor_are_handlers_blocked(
    GimpGradientTool *gradient_tool) {
...
static gint gimp_gradient_tool_editor_add_stop(GimpGradientTool *gradient_tool,
  ...
  gimp_gradient_split_at(
      gradient_tool->gradient, GIMP_CONTEXT(options), NULL, value,
      paint_options->gradient_options->gradient_blend_color_space, &seg, NULL);

  stop = gimp_gradient_segment_range_get_n_segments(
             gradient_tool->gradient, gradient_tool->gradient->segments, seg) -
         1;
...
gimp_gradient_tool_editor_delete_stop(GimpGradientTool *gradient_tool,
  ...
  if (info->added_handle == slider) {
    ...
    gimp_gradient_segment_range_merge(gradient_tool->gradient, seg, seg->next,
                                      NULL, NULL);
...
gimp_gradient_tool_editor_midpoint_to_stop(GimpGradientTool *gradient_tool,
  ...
  gimp_assert(
      gimp_gradient_tool_editor_handle_is_midpoint(gradient_tool, slider));

  sliders =
      gimp_tool_line_get_sliders(GIMP_TOOL_LINE(gradient_tool->widget), NULL);
...
gimp_gradient_tool_editor_update_sliders(GimpGradientTool *gradient_tool) {
  ...
  for (seg = gradient_tool->gradient->segments, i = 0; seg->next;
       seg = seg->next, i++) {
  ...
  for (seg = gradient_tool->gradient->segments, i = 0; seg;
       seg = seg->next, i++) {
    *slider = GIMP_CONTROLLER_SLIDER_DEFAULT;
  ...
  gimp_tool_line_set_sliders(GIMP_TOOL_LINE(gradient_tool->widget), sliders,
                             n_sliders);
...
static GtkWidget *gimp_gradient_tool_editor_color_entry_new(
    ...
    GtkWidget *chain_button, GtkWidget **color_panel, GtkWidget **type_combo) {
  GimpContext *context =
      GIMP_CONTEXT(GIMP_GRADIENT_TOOL_GET_OPTIONS(gradient_tool));
  GtkWidget *hbox;
  GtkWidget *button;
  GtkWidget *combo;
  GimpRGB color = {};
  ...
  /* the color panel */
  *color_panel = button =
      gimp_color_panel_new(title, &color, GIMP_COLOR_AREA_SMALL_CHECKS, 24, 24);
  gimp_color_button_set_update(GIMP_COLOR_BUTTON(button), TRUE);
  gimp_color_panel_set_context(GIMP_COLOR_PANEL(button), context);
  gtk_box_pack_start(GTK_BOX(hbox), button, TRUE, TRUE, 0);
  gtk_widget_show(button);

  g_object_set_data(G_OBJECT(button), "gimp-gradient-tool-editor-direction",
                    GINT_TO_POINTER(direction));
  g_object_set_data(G_OBJECT(button), "gimp-gradient-tool-editor-chain-button",
                    chain_button);
  ...
  *type_combo = combo = gimp_enum_combo_box_new(GIMP_TYPE_GRADIENT_COLOR);
  ...
  g_object_set_data(G_OBJECT(combo), "gimp-gradient-tool-editor-direction",
                    GINT_TO_POINTER(direction));
  g_object_set_data(G_OBJECT(combo), "gimp-gradient-tool-editor-chain-button",
                    chain_button);
...
gimp_gradient_tool_editor_init_endpoint_gui(GimpGradientTool *gradient_tool) {
  ...
  /* the endpoint editor */
  gradient_tool->endpoint_editor = editor = gimp_editor_new();
  ...
  gradient_tool->endpoint_se = se =
      gimp_size_entry_new(1, GIMP_UNIT_PIXEL, "%a", TRUE, TRUE, FALSE, 6,
                          GIMP_SIZE_ENTRY_UPDATE_SIZE);
  gtk_grid_set_row_spacing(GTK_GRID(se), 4);
  gtk_grid_set_column_spacing(GTK_GRID(se), 2);

  gimp_size_entry_add_field(GIMP_SIZE_ENTRY(se), GTK_SPIN_BUTTON(spinbutton),
                            NULL);
  ...
  gimp_size_entry_set_refval_boundaries(
      GIMP_SIZE_ENTRY(se), 0, -GIMP_MAX_IMAGE_SIZE, GIMP_MAX_IMAGE_SIZE);
  gimp_size_entry_set_refval_boundaries(
      GIMP_SIZE_ENTRY(se), 1, -GIMP_MAX_IMAGE_SIZE, GIMP_MAX_IMAGE_SIZE);

  gimp_size_entry_set_size(GIMP_SIZE_ENTRY(se), 0, 0,
                           gimp_image_get_width(image));
  gimp_size_entry_set_size(GIMP_SIZE_ENTRY(se), 1, 0,
  ...
  hbox = gimp_gradient_tool_editor_color_entry_new(
      ...
      &gradient_tool->endpoint_color_panel,
...
gimp_gradient_tool_editor_init_stop_gui(GimpGradientTool *gradient_tool) {
  ...
  /* the stop editor */
  gradient_tool->stop_editor = editor = gimp_editor_new();
  gtk_box_pack_start(GTK_BOX(gimp_tool_gui_get_vbox(gradient_tool->gui)),
                     editor, FALSE, TRUE, 0);
  ...
  /* the position size entry */
  gradient_tool->stop_se = se =
      gimp_size_entry_new(1, GIMP_UNIT_PERCENT, "%a", FALSE, TRUE, FALSE, 6,
                          GIMP_SIZE_ENTRY_UPDATE_SIZE);
  ...
  gradient_tool->stop_chain_button = button =
      gimp_chain_button_new(GIMP_CHAIN_RIGHT);
  ...
  hbox = gimp_gradient_tool_editor_color_entry_new(
      ...
      &gradient_tool->stop_left_color_panel,
      &gradient_tool->stop_left_type_combo);
  gtk_grid_attach(GTK_GRID(grid2), hbox, 0, 0, 1, 1);
  gtk_widget_show(hbox);

  hbox = gimp_gradient_tool_editor_color_entry_new(
      gradient_tool, _("Change Stop Color"), DIRECTION_RIGHT, button,
      &gradient_tool->stop_right_color_panel,
  ...
  gimp_editor_add_button(
      GIMP_EDITOR(editor), GIMP_ICON_EDIT_DELETE, _("Delete stop"), NULL,
      G_CALLBACK(gimp_gradient_tool_editor_stop_delete_clicked), NULL,
      gradient_tool);
}
...
gimp_gradient_tool_editor_init_midpoint_gui(GimpGradientTool *gradient_tool) {
  ...
  /* the stop editor */
  gradient_tool->midpoint_editor = editor = gimp_editor_new();
  gtk_box_pack_start(GTK_BOX(gimp_tool_gui_get_vbox(gradient_tool->gui)),
                     editor, FALSE, TRUE, 0);
  ...
  /* the type combo */
  gradient_tool->midpoint_type_combo = combo =
      gimp_enum_combo_box_new(GIMP_TYPE_GRADIENT_SEGMENT_TYPE);
  ...
  /* the color combo */
  gradient_tool->midpoint_color_combo = combo =
      gimp_enum_combo_box_new(GIMP_TYPE_GRADIENT_SEGMENT_COLOR);
  gtk_grid_attach(GTK_GRID(grid), combo, 1, row, 1, 1);
  gtk_widget_show(combo);
  ...
  /* the new stop button */
  gradient_tool->midpoint_new_stop_button = gimp_editor_add_button(
      GIMP_EDITOR(editor), GIMP_ICON_DOCUMENT_NEW, _("New stop at midpoint"),
      NULL, G_CALLBACK(gimp_gradient_tool_editor_midpoint_new_stop_clicked),
      NULL, gradient_tool);

  /* the center button */
  gradient_tool->midpoint_center_button = gimp_editor_add_button(
      GIMP_EDITOR(editor), GIMP_ICON_CENTER_HORIZONTAL, _("Center midpoint"),
      NULL, G_CALLBACK(gimp_gradient_tool_editor_midpoint_center_clicked), NULL,
      gradient_tool);
}
...
gimp_gradient_tool_editor_update_endpoint_gui(GimpGradientTool *gradient_tool,
  ...
  switch (selection) {
  case GIMP_TOOL_LINE_HANDLE_START:
    g_object_get(gradient_tool->widget, "x1", &x, "y1", &y, NULL);
    break;

  case GIMP_TOOL_LINE_HANDLE_END:
    g_object_get(gradient_tool->widget, "x2", &x, "y2", &y, NULL);
    break;
...
gimp_gradient_tool_editor_update_stop_gui(GimpGradientTool *gradient_tool,
  ...
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(gradient_tool->stop_se), 0,
                             100.0 * value);
  ...
  gimp_int_combo_box_set_active(
      GIMP_INT_COMBO_BOX(gradient_tool->stop_right_type_combo),
      right_color_type);
  ...
  gtk_widget_set_sensitive(GTK_WIDGET(gimp_editor_get_button_box(
                               GIMP_EDITOR(gradient_tool->stop_editor))),
                           editable);
...
gimp_gradient_tool_editor_update_midpoint_gui(GimpGradientTool *gradient_tool,
  ...
  gimp_size_entry_set_refval_boundaries(
      GIMP_SIZE_ENTRY(gradient_tool->midpoint_se), 0, 100.0 * min, 100.0 * max);
  gimp_size_entry_set_refval(GIMP_SIZE_ENTRY(gradient_tool->midpoint_se), 0,
                             100.0 * value);
  ...
  gtk_widget_set_sensitive(GTK_WIDGET(gimp_editor_get_button_box(
                               GIMP_EDITOR(gradient_tool->midpoint_editor))),
                           editable);
  ...
  if (gradient_tool->gradient && gradient_tool->widget && !options->instant) {
    ...
    if (selection != GIMP_TOOL_LINE_HANDLE_NONE) {
      if (!gradient_tool->gui) {
        ...
        gradient_tool->gui = gimp_tool_gui_new(
            GIMP_TOOL(gradient_tool)->tool_info, NULL, NULL, NULL, NULL,
            gimp_widget_get_monitor(GTK_WIDGET(shell)), TRUE,
      ...
      if (gimp_gradient_tool_editor_handle_is_endpoint(gradient_tool,
                                                       selection))
      ...
      if (gimp_gradient_tool_editor_handle_is_midpoint(gradient_tool,
                                                       selection))
...
gimp_gradient_tool_editor_gradient_info_apply(GimpGradientTool *gradient_tool,
  ...
  if (info->gradient) {
    if (info->removed_handle != GIMP_TOOL_LINE_HANDLE_NONE) {
      ...
      /* otherwise, if the selected handle is a midpoint... */
      else if (gimp_gradient_tool_editor_handle_is_midpoint(gradient_tool,
                                                            selection)) {
        const GimpControllerSlider *sliders;
        gint seg_i;

        sliders = gimp_tool_line_get_sliders(
            GIMP_TOOL_LINE(gradient_tool->widget), NULL);
         ...
         */
        if (seg_i == info->added_handle || seg_i == info->added_handle + 1) {
          selection = GIMP_TOOL_LINE_HANDLE_NONE;
        }
  ...
  g_object_set(gradient_tool->widget, "x1", info->start_x, "y1", info->start_y,
               "x2", info->end_x, "y2", info->end_y, NULL);
...
static gboolean gimp_gradient_tool_editor_gradient_info_is_trivial(
  ...
  if (info->gradient) {
    for (seg1 = info->gradient->segments,
        seg2 = gradient_tool->gradient->segments;
         seg1 && seg2; seg1 = seg1->next, seg2 = seg2->next) {
...
void gimp_gradient_tool_editor_start(GimpGradientTool *gradient_tool) {
  ...
  g_signal_connect(gradient_tool->widget, "add-slider",
                   ...
                   gradient_tool);
  g_signal_connect(
      gradient_tool->widget, "prepare-to-remove-slider",
...
gimp_gradient_tool_editor_line_changed(GimpGradientTool *gradient_tool) {
   ...
   */
  for (seg = gradient_tool->gradient->segments, i = n_sliders / 2; seg;
       seg = seg->next, i++) {
    gdouble value;
...
void gimp_gradient_tool_editor_gradient_changed(
    GimpGradientTool *gradient_tool) {
  GimpGradientOptions *options = GIMP_GRADIENT_TOOL_GET_OPTIONS(gradient_tool);
  GimpContext *context = GIMP_CONTEXT(options);
...
gboolean gimp_gradient_tool_editor_undo(GimpGradientTool *gradient_tool) {
  ...
  gradient_tool->redo_stack =
      g_slist_prepend(gradient_tool->redo_stack, new_info);
...
gboolean gimp_gradient_tool_editor_redo(GimpGradientTool *gradient_tool) {
  ...
  gradient_tool->undo_stack =
      g_slist_prepend(gradient_tool->undo_stack, new_info);
...
void gimp_gradient_tool_editor_start_edit(GimpGradientTool *gradient_tool) {
  if (gradient_tool->edit_count++ == 0) {
    ...
    gradient_tool->undo_stack =
        g_slist_prepend(gradient_tool->undo_stack, info);

    /*  update the undo actions / menu items  */
    if (!gradient_tool->flush_idle_id) {
      gradient_tool->flush_idle_id = g_idle_add(
          (GSourceFunc)gimp_gradient_tool_editor_flush_idle, gradient_tool);
    }
  }
}
...
void gimp_gradient_tool_editor_end_edit(GimpGradientTool *gradient_tool,
  ...
  if (--gradient_tool->edit_count == 0) {
    ...
    if (cancel || gimp_gradient_tool_editor_gradient_info_is_trivial(
                      gradient_tool, info)) {
      ...
      gradient_tool->undo_stack =
          g_slist_remove(gradient_tool->undo_stack, info);
      gimp_gradient_tool_editor_gradient_info_free(info);
    } else {
