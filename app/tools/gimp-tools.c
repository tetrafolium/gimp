...
static void gimp_tools_register(
    GType tool_type, GType tool_options_type,
    ...
    const gchar *menu_label, const gchar *menu_accel, const gchar *help_domain,
    const gchar *help_data, const gchar *icon_name, gpointer data);

static void gimp_tools_copy_structure(Gimp *gimp, GimpContainer *src_container,
...
void gimp_tools_init(Gimp *gimp) {
  GimpToolRegisterFunc register_funcs[] = {
      ...
      gimp_rectangle_select_tool_register, gimp_ellipse_select_tool_register,
      gimp_free_select_tool_register, gimp_fuzzy_select_tool_register,
      gimp_by_color_select_tool_register, gimp_iscissors_tool_register,
      gimp_foreground_select_tool_register, gimp_paint_select_tool_register,
      ...
      gimp_color_picker_tool_register, gimp_magnify_tool_register,
      gimp_measure_tool_register,

      /*  transform tools  */

      gimp_move_tool_register, gimp_align_tool_register,
      gimp_crop_tool_register, gimp_unified_transform_tool_register,
      gimp_rotate_tool_register, gimp_scale_tool_register,
      gimp_shear_tool_register, gimp_handle_transform_tool_register,
      gimp_perspective_tool_register, gimp_transform_3d_tool_register,
      gimp_flip_tool_register, gimp_cage_tool_register, gimp_warp_tool_register,
      gimp_n_point_deformation_tool_register,

      /*  paint tools  */

      gimp_seamless_clone_tool_register, gimp_text_tool_register,
      gimp_bucket_fill_tool_register, gimp_gradient_tool_register,
      gimp_pencil_tool_register, gimp_paintbrush_tool_register,
      gimp_eraser_tool_register, gimp_airbrush_tool_register,
      gimp_ink_tool_register, gimp_mybrush_tool_register,
      gimp_clone_tool_register, gimp_heal_tool_register,
      gimp_perspective_clone_tool_register, gimp_convolve_tool_register,
      gimp_smudge_tool_register, gimp_dodge_burn_tool_register,

      /*  filter tools  */

      gimp_brightness_contrast_tool_register, gimp_threshold_tool_register,
      gimp_levels_tool_register, gimp_curves_tool_register,
      gimp_offset_tool_register, gimp_gegl_tool_register,
  ...
  toolbox_groups_binding = g_object_bind_property(
      gimp->config, "toolbox-groups", gimp->tool_item_ui_list, "flat",
...
void gimp_tools_exit(Gimp *gimp) {
  ...
  for (list = gimp_get_tool_info_iter(gimp); list; list = g_list_next(list)) {
...
void gimp_tools_restore(Gimp *gimp) {
  ...
  for (list = gimp_get_tool_info_iter(gimp); list; list = g_list_next(list)) {
    ...
    gimp_context_copy_properties(
        gimp_get_user_context(gimp), GIMP_CONTEXT(tool_info->tool_options),
        GIMP_CONTEXT_PROP_MASK_ALL &
            ~(tool_info->context_props | GIMP_CONTEXT_PROP_MASK_TOOL |
              GIMP_CONTEXT_PROP_MASK_PAINT_INFO));

    gimp_tool_options_deserialize(tool_info->tool_options, NULL);

    options_gui_func =
        g_object_get_data(G_OBJECT(tool_info), "gimp-tool-options-gui-func");
...
gboolean gimp_tools_clear(Gimp *gimp, GError **error) {
  ...
  for (list = gimp_get_tool_info_iter(gimp); list && success;
...
gboolean gimp_tools_serialize(Gimp *gimp, GimpContainer *container,
                              GimpConfigWriter *writer) {
  g_return_val_if_fail(GIMP_IS_GIMP(gimp), FALSE);
...
gboolean gimp_tools_deserialize(Gimp *gimp, GimpContainer *container,
                                GScanner *scanner) {
  enum { FILE_VERSION = 1 };
  ...
  g_scanner_scope_add_symbol(scanner, scope_id, "file-version",
                             GINT_TO_POINTER(FILE_VERSION));

  token = G_TOKEN_LEFT_PAREN;

  while (g_scanner_peek_next_token(scanner) == token &&
         (token != G_TOKEN_LEFT_PAREN || !file_version)) {
  ...
  src_container = g_object_new(GIMP_TYPE_LIST, "children-type",
                               GIMP_TYPE_TOOL_ITEM, "append", TRUE, NULL);

  if (gimp_config_deserialize(GIMP_CONFIG(src_container), scanner, 0, NULL)) {
    GHashTable *tools;
    GList *list;
...
void gimp_tools_reset(Gimp *gimp, GimpContainer *container,
...
static void gimp_tools_register(
  ...
  tool_info =
      gimp_tool_info_new(gimp, tool_type, tool_options_type, context_props,
                         identifier, label, tooltip, menu_label, menu_accel,
...
static void gimp_tools_copy_structure(Gimp *gimp, GimpContainer *src_container,
  ...
  for (list = GIMP_LIST(src_container)->queue->head; list;
       list = g_list_next(list)) {
    ...
    if (GIMP_IS_TOOL_GROUP(src_tool_item)) {
      ...
      gimp_tools_copy_structure(
          gimp, gimp_viewable_get_children(GIMP_VIEWABLE(src_tool_item)),
          gimp_viewable_get_children(GIMP_VIEWABLE(dest_tool_item)), tools);
    ...
    if (dest_tool_item) {
      gimp_tool_item_set_visible(dest_tool_item,
                                 gimp_tool_item_get_visible(src_tool_item));

      gimp_container_add(dest_container, GIMP_OBJECT(dest_tool_item));
