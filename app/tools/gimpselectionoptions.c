...
gimp_selection_options_class_init(GimpSelectionOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(object_class, PROP_OPERATION, "operation", NULL, NULL,
                        GIMP_TYPE_CHANNEL_OPS, GIMP_CHANNEL_OP_REPLACE,
  ...
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_FEATHER, "feather",
                           _("Feather edges"),
                           _("Enable feathering of selection edges"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);
...
GtkWidget *gimp_selection_options_gui(GimpToolOptions *tool_options) {
  ...
  /*  the selection operation radio buttons  */
  {
    ...
    box = gimp_prop_enum_icon_box_new(config, "operation", "gimp-selection", 0,
                                      0);
    ...
    gtk_box_reorder_child(GTK_BOX(box), GTK_WIDGET(children->next->next->data),
                          0);
