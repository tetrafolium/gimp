...
static void gimp_color_tool_button_release(GimpTool *tool,
                                           ...
                                           GimpDisplay *display);
static void gimp_color_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                   guint32 time, GdkModifierType state,
                                   GimpDisplay *display);
...
static gboolean gimp_color_tool_real_can_pick(GimpColorTool *color_tool,
                                              const GimpCoords *coords,
                                              GimpDisplay *display);
...
static gboolean gimp_color_tool_can_pick(GimpColorTool *tool,
                                         const GimpCoords *coords,
                                         GimpDisplay *display);
static void gimp_color_tool_pick(GimpColorTool *tool, const GimpCoords *coords,
                                 GimpDisplay *display,
                                 GimpColorPickState pick_state);
...
static void gimp_color_tool_class_init(GimpColorToolClass *klass) {
  ...
  gimp_color_tool_signals[PICKED] =
      g_signal_new("picked", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpColorToolClass, picked), NULL, NULL,
                   gimp_marshal_VOID__POINTER_OBJECT_ENUM_POINTER_POINTER_BOXED,
                   G_TYPE_NONE, 6, G_TYPE_POINTER, GIMP_TYPE_DISPLAY,
                   GIMP_TYPE_COLOR_PICK_STATE, G_TYPE_POINTER, G_TYPE_POINTER,
...
static void gimp_color_tool_oper_update(GimpTool *tool,
  ...
  if (color_tool->enabled) {
    ...
    if (!draw_tool->widget &&
        ...
        (draw_tool->display != display || !proximity)) {
      gimp_draw_tool_stop(draw_tool);
    }

    if (gimp_display_shell_get_show_sample_points(shell) && proximity) {
      GimpImage *image = gimp_display_get_image(display);
      gint snap_distance = display->config->snap_distance;

      sample_point = gimp_image_pick_sample_point(
          image, coords->x, coords->y, FUNSCALEX(shell, snap_distance),
    ...
    if (!draw_tool->widget &&
        ...
        !gimp_draw_tool_is_active(draw_tool) && proximity) {
...
static void gimp_color_tool_cursor_update(GimpTool *tool,
  ...
  if (color_tool->enabled) {
    if (color_tool->sample_point) {
      ...
      gimp_tool_set_cursor(tool, display, GIMP_CURSOR_COLOR_PICKER,
                           GIMP_TOOL_CURSOR_COLOR_PICKER, modifier);
    }
  } else {
  ...
  if (color_tool->enabled) {
    if (color_tool->sample_point) {
      ...
      gimp_draw_tool_add_rectangle(
          draw_tool, FALSE, color_tool->center_x - radius,
          color_tool->center_y - radius, 2 * radius + 1, 2 * radius + 1);
    }
  }
}
...
static gboolean gimp_color_tool_real_can_pick(GimpColorTool *color_tool,
  ...
  return gimp_image_coords_in_active_pickable(
      image, coords, shell->show_all, color_tool->options->sample_merged,
      FALSE);
}
...
static gboolean gimp_color_tool_real_pick(GimpColorTool *color_tool,
  ...
  return gimp_image_pick_color(
      image, drawables, coords->x, coords->y, shell->show_all,
      color_tool->options->sample_merged, color_tool->options->sample_average,
      color_tool->options->average_radius, sample_format, pixel, color);
}
...
static void gimp_color_tool_real_picked(GimpColorTool *color_tool,
  ...
  if (color_tool->pick_target == GIMP_COLOR_PICK_TARGET_FOREGROUND ||
    ...
    widget =
        gimp_dialog_factory_find_widget(dialog_factory, "gimp-indexed-palette");
    if (widget) {
      ...
      if (babl_format_is_palette(sample_format)) {
        ...
        gimp_colormap_editor_set_index(GIMP_COLORMAP_EDITOR(editor), *index,
                                       NULL);
         ...
         */
        gint index =
            gimp_colormap_editor_get_index(GIMP_COLORMAP_EDITOR(editor), color);
        if (index > -1)
          gimp_colormap_editor_set_index(GIMP_COLORMAP_EDITOR(editor), index,
                                         NULL);
      }
    ...
    widget =
        gimp_dialog_factory_find_widget(dialog_factory, "gimp-palette-editor");
    if (widget) {
      GtkWidget *editor = gtk_bin_get_child(GTK_BIN(widget));
      gint index;

      index = gimp_palette_editor_get_index(GIMP_PALETTE_EDITOR(editor), color);
      if (index != -1)
        gimp_palette_editor_set_index(GIMP_PALETTE_EDITOR(editor), index, NULL);
    }
  }
  ...
  case GIMP_COLOR_PICK_TARGET_PALETTE: {
    ...
    dockable = gimp_window_strategy_show_dockable_dialog(
        GIMP_WINDOW_STRATEGY(gimp_get_window_strategy(display->gimp)),
        display->gimp, dialog_factory, monitor, "gimp-palette-editor");
    ...
    if (dockable) {
      ...
      gimp_palette_editor_pick_color(GIMP_PALETTE_EDITOR(palette_editor), color,
                                     pick_state);
    }
  } break;
  }
...
static void gimp_color_tool_pick(GimpColorTool *tool, const GimpCoords *coords,
  ...
  if (klass->pick &&
    ...
    g_signal_emit(tool, gimp_color_tool_signals[PICKED], 0, coords, display,
                  pick_state, sample_format, pixel, &color);
  }
}
