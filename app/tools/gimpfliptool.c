...
static void gimp_flip_tool_button_press(GimpTool *tool,
                                        ...
                                        GimpDisplay *display);
static void gimp_flip_tool_modifier_key(GimpTool *tool, GdkModifierType key,
                                        gboolean press, GdkModifierType state,
                                        GimpDisplay *display);
static void gimp_flip_tool_oper_update(GimpTool *tool, const GimpCoords *coords,
                                       GdkModifierType state,
                                       gboolean proximity,
                                       GimpDisplay *display);
...
static GeglBuffer *
gimp_flip_tool_transform(GimpTransformTool *tr_tool, GList *objects,
                         GeglBuffer *orig_buffer, gint orig_offset_x,
                         gint orig_offset_y, GimpColorProfile **buffer_profile,
...
void gimp_flip_tool_register(GimpToolRegisterCallback callback, gpointer data) {
  (*callback)(
      GIMP_TYPE_FLIP_TOOL, GIMP_TYPE_FLIP_OPTIONS, gimp_flip_options_gui,
      GIMP_CONTEXT_PROP_MASK_BACKGROUND, "gimp-flip-tool", _("Flip"),
      _("Flip Tool: "
        "Reverse the layer, selection or path horizontally or vertically"),
      N_("_Flip"), "<shift>F", NULL, GIMP_HELP_TOOL_FLIP, GIMP_ICON_TOOL_FLIP,
      data);
}
...
static void gimp_flip_tool_modifier_key(GimpTool *tool, GdkModifierType key,
  ...
  if (key == gimp_get_toggle_behavior_mask()) {
    ...
    case GIMP_ORIENTATION_HORIZONTAL:
      g_object_set(options, "flip-type", GIMP_ORIENTATION_VERTICAL, NULL);
      break;

    case GIMP_ORIENTATION_VERTICAL:
      g_object_set(options, "flip-type", GIMP_ORIENTATION_HORIZONTAL, NULL);
      break;
...
static void gimp_flip_tool_oper_update(GimpTool *tool, const GimpCoords *coords,
  ...
  if (gimp_display_shell_get_show_guides(shell) && proximity) {
  ...
  if (flip->guide != guide || (guide && !gimp_draw_tool_is_active(draw_tool))) {
...
static void gimp_flip_tool_cursor_update(GimpTool *tool,
  ...
  selected_objects =
      gimp_transform_tool_check_selected_objects(tr_tool, display, NULL);
...
static void gimp_flip_tool_draw(GimpDrawTool *draw_tool) {
  ...
  if (flip->guide) {
    ...
    item = gimp_draw_tool_add_guide(
        draw_tool, gimp_guide_get_orientation(flip->guide),
        gimp_guide_get_position(flip->guide), style);
...
gimp_flip_tool_transform(GimpTransformTool *tr_tool, GList *objects,
  ...
  if (flip->guide) {
    ...
    case GIMP_ORIENTATION_HORIZONTAL:
      axis =
          ((gdouble)tr_tool->x1 + (gdouble)(tr_tool->x2 - tr_tool->x1) / 2.0);
      break;

    case GIMP_ORIENTATION_VERTICAL:
      axis =
          ((gdouble)tr_tool->y1 + (gdouble)(tr_tool->y2 - tr_tool->y1) / 2.0);
      break;
  ...
  if (orig_buffer) {
    ...
    ret = gimp_drawable_transform_buffer_flip(
        GIMP_DRAWABLE(objects->data), context, orig_buffer, orig_offset_x,
        orig_offset_y, flip_type, axis, clip_result, buffer_profile,
    ...
    progress = gimp_progress_start(GIMP_PROGRESS(tr_tool), FALSE, "%s",
                                   tr_class->progress_text);

    gimp_image_flip_full(GIMP_IMAGE(objects->data), context, flip_type, axis,
    ...
    for (iter = objects; iter; iter = iter->next) {
      ...
      if (gimp_item_get_linked(item)) {
        gimp_item_linked_flip(item, context, flip_type, axis, clip_result);
      } else {
        clip_result = gimp_item_get_clip(item, clip_result);

        gimp_item_flip(item, context, flip_type, axis, clip_result);
      }
    }
  }
