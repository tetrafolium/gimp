...
gimp_transform_grid_options_class_init(GimpTransformGridOptionsClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_DIRECTION_LINKED,
      g_param_spec_boolean("direction-linked", NULL, NULL, FALSE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_SHOW_PREVIEW, "show-preview",
                           _("Show image preview"),
                           _("Show a preview of the transformed image"), TRUE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_COMPOSITED_PREVIEW,
                           "composited-preview", _("Composited preview"),
                           _("Show preview as part of the image composition"),
                           FALSE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_PREVIEW_LINKED, "preview-linked",
                           _("Preview linked items"),
                           _("Include linked items in the preview"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_SYNCHRONOUS_PREVIEW,
                           "synchronous-preview", _("Synchronous preview"),
                           _("Render the preview synchronously"), FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_PREVIEW_OPACITY, "preview-opacity",
                          _("Image opacity"), _("Opacity of the preview image"),
                          0.0, 1.0, 1.0, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_GRID_TYPE, "grid-type", _("Guides"),
                        _("Composition guides such as rule of thirds"),
                        GIMP_TYPE_GUIDES_TYPE, GIMP_GUIDES_NONE,
                        GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_INT(object_class, PROP_GRID_SIZE, "grid-size", NULL,
                       _("Size of a grid cell for variable number "
                         "of composition guides"),
                       1, 128, 15, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_CONSTRAIN_MOVE, "constrain-move",
                           NULL, NULL, FALSE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_CONSTRAIN_SCALE,
                           "constrain-scale", NULL, NULL, TRUE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_CONSTRAIN_ROTATE,
                           "constrain-rotate", NULL, NULL, FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_CONSTRAIN_SHEAR,
                           "constrain-shear", NULL, NULL, TRUE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_CONSTRAIN_PERSPECTIVE,
                           "constrain-perspective", NULL, NULL, FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_FROMPIVOT_SCALE,
                           "frompivot-scale", NULL, NULL, FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_FROMPIVOT_SHEAR,
                           "frompivot-shear", NULL, NULL, FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_FROMPIVOT_PERSPECTIVE,
                           "frompivot-perspective", NULL, NULL, FALSE,
                           GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_CORNERSNAP, "cornersnap", NULL,
                           NULL, FALSE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_FIXEDPIVOT, "fixedpivot", NULL,
                           NULL, FALSE, GIMP_PARAM_STATIC_STRINGS);
}
...
static void gimp_transform_grid_options_set_property(GObject *object,
  ...
  case PROP_DIRECTION:
     ...
     * original image only.
     */
    g_object_set(options, "show-preview",
                 transform_options->direction != GIMP_TRANSFORM_BACKWARD, NULL);
    break;
...
GtkWidget *gimp_transform_grid_options_gui(GimpToolOptions *tool_options) {
  ...
  if (tg_class->matrix_to_info) {
    ...
    g_object_bind_property(config, "direction-linked", button, "active",
  ...
  scale =
      gimp_prop_spin_scale_new(config, "preview-opacity", NULL, 0.01, 0.1, 0);
  gimp_prop_widget_set_factor(scale, 100.0, 0.0, 0.0, 1);
  gtk_box_pack_start(GTK_BOX(vbox2), scale, FALSE, FALSE, 0);
  gtk_widget_show(scale);

  g_object_bind_property(config, "composited-preview", scale, "sensitive",
                         G_BINDING_SYNC_CREATE | G_BINDING_INVERT_BOOLEAN);

  frame =
      gimp_prop_expanding_frame_new(config, "show-preview", NULL, vbox2, NULL);
  ...
  scale = gimp_prop_spin_scale_new(config, "grid-size", NULL, 1.8, 8.0, 0);
  gimp_spin_scale_set_label(GIMP_SPIN_SCALE(scale), NULL);
  gtk_container_add(GTK_CONTAINER(frame), scale);

  g_object_bind_property_full(
      config, "grid-type", scale, "visible", G_BINDING_SYNC_CREATE,
      gimp_transform_grid_options_sync_grid, NULL, NULL, NULL);

  if (tool_options->tool_info->tool_type == GIMP_TYPE_ROTATE_TOOL) {
    gchar *label;

    label =
        g_strdup_printf(_("15 degrees (%s)"), gimp_get_mod_string(extend_mask));
    ...
    gimp_help_set_help_data(button, _("Keep the original aspect ratio"), NULL);
    ...
    gimp_help_set_help_data(button, _("Scale around the center point"), NULL);
    ...
    gimp_help_set_help_data(button, _("Transform around the center point"),
                            NULL);

    g_free(label);
  } else if (tool_options->tool_info->tool_type ==
             GIMP_TYPE_UNIFIED_TRANSFORM_TOOL) {
    struct {
      GdkModifierType mod;
      gchar *name;
      gchar *desc;
      gchar *tip;
        ...
        {extend_mask, "constrain-perspective", N_("Perspective"),
         N_("Constrain perspective handles to move along edges and diagonal "
            "(%s)")},

        {constrain_mask, NULL, N_("From pivot  (%s)")},
        ...
        {0, "fixedpivot", N_("Lock"), N_("Lock pivot position to canvas")},
    };
    ...
    for (i = 0; i < G_N_ELEMENTS(opt_list); i++) {
      ...
      if (opt_list[i].name) {
        ...
        gtk_box_pack_start(GTK_BOX(frame ? grid_box : vbox), button, FALSE,
                           FALSE, 0);
...
static gboolean gimp_transform_grid_options_sync_grid(
    GBinding *binding, const GValue *source_value, GValue *target_value,
    gpointer user_data) {
  GimpGuidesType type = g_value_get_enum(source_value);

  g_value_set_boolean(target_value, type == GIMP_GUIDES_N_LINES ||
