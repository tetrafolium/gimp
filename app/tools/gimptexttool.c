...
static void gimp_text_tool_control(GimpTool *tool, GimpToolAction action,
                                   GimpDisplay *display);
...
static void gimp_text_tool_button_release(GimpTool *tool,
                                          ...
                                          GimpDisplay *display);
static void gimp_text_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                  guint32 time, GdkModifierType state,
                                  GimpDisplay *display);
static gboolean gimp_text_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
                                         GimpDisplay *display);
static gboolean gimp_text_tool_key_release(GimpTool *tool, GdkEventKey *kevent,
                                           GimpDisplay *display);
static void gimp_text_tool_oper_update(GimpTool *tool, const GimpCoords *coords,
                                       GdkModifierType state,
                                       gboolean proximity,
                                       GimpDisplay *display);
...
static void gimp_text_tool_layer_notify(GimpTextLayer *layer,
                                        ...
                                        GimpTextTool *text_tool);
static void gimp_text_tool_proxy_notify(GimpText *text, const GParamSpec *pspec,
                                        GimpTextTool *text_tool);

static void gimp_text_tool_text_notify(GimpText *text, const GParamSpec *pspec,
                                       GimpTextTool *text_tool);
...
static void gimp_text_tool_apply_list(GimpTextTool *text_tool, GList *pspecs);
...
static void gimp_text_tool_layer_changed(GimpImage *image,
                                         GimpTextTool *text_tool);
static void gimp_text_tool_set_image(GimpTextTool *text_tool, GimpImage *image);
...
static void gimp_text_tool_buffer_color_applied(GimpTextBuffer *buffer,
                                                const GimpRGB *color,
                                                GimpTextTool *text_tool);
...
void gimp_text_tool_register(GimpToolRegisterCallback callback, gpointer data) {
  (*callback)(
      GIMP_TYPE_TEXT_TOOL, GIMP_TYPE_TEXT_OPTIONS, gimp_text_options_gui,
      GIMP_CONTEXT_PROP_MASK_FOREGROUND | GIMP_CONTEXT_PROP_MASK_FONT |
          GIMP_CONTEXT_PROP_MASK_PALETTE /* for the color popup's palette tab */
      ,
      "gimp-text-tool", _("Text"), _("Text Tool: Create or edit text layers"),
      N_("Te_xt"), "T", NULL, GIMP_HELP_TOOL_TEXT, GIMP_ICON_TOOL_TEXT, data);
}
...
static void gimp_text_tool_init(GimpTextTool *text_tool) {
  GimpTool *tool = GIMP_TOOL(text_tool);

  text_tool->buffer = gimp_text_buffer_new();

  g_signal_connect(text_tool->buffer, "begin-user-action",
                   G_CALLBACK(gimp_text_tool_buffer_begin_edit), text_tool);
  g_signal_connect(text_tool->buffer, "end-user-action",
                   G_CALLBACK(gimp_text_tool_buffer_end_edit), text_tool);
  g_signal_connect(text_tool->buffer, "color-applied",
                   G_CALLBACK(gimp_text_tool_buffer_color_applied), text_tool);
...
static void gimp_text_tool_constructed(GObject *object) {
  ...
  g_signal_connect_object(text_tool->proxy, "notify",
                          G_CALLBACK(gimp_text_tool_proxy_notify), text_tool,
                          0);
  ...
  g_signal_connect_object(
      async_set, "notify::empty",
      G_CALLBACK(gimp_text_tool_fonts_async_set_empty_notify), text_tool, 0);
}
...
static void gimp_text_tool_remove_empty_text_layer(GimpTextTool *text_tool) {
  ...
  if (text_layer && text_layer->auto_rename) {
    ...
    if (text && text->box_mode == GIMP_TEXT_BOX_DYNAMIC &&
      ...
      if (text_tool->image == image)
        g_signal_handlers_block_by_func(image, gimp_text_tool_layer_changed,
                                        text_tool);
      ...
      if (text_tool->image == image)
        g_signal_handlers_unblock_by_func(image, gimp_text_tool_layer_changed,
                                          text_tool);
    }
  }
}
...
static void gimp_text_tool_button_press(GimpTool *tool,
  ...
  if (press_type == GIMP_BUTTON_PRESS_NORMAL) {
    ...
    if (gimp_tool_rectangle_point_in_rectangle(rectangle, coords->x,
                                               coords->y) &&
        !text_tool->moving) {
      gimp_tool_rectangle_set_function(rectangle, GIMP_TOOL_RECTANGLE_DEAD);
    } else if (gimp_tool_widget_button_press(text_tool->widget, coords, time,
                                             state, press_type)) {
     ...
     */
    if (!text_tool->moving && gimp_tool_rectangle_get_function(rectangle) ==
                                  GIMP_TOOL_RECTANGLE_CREATING) {
...
static void gimp_text_tool_button_release(GimpTool *tool,
  ...
  if (text_tool->selecting) {
    ...
    g_signal_handlers_block_by_func(
        text_tool->buffer, gimp_text_tool_buffer_begin_edit, text_tool);
    g_signal_handlers_block_by_func(text_tool->buffer,
                                    gimp_text_tool_buffer_end_edit, text_tool);

    gimp_text_tool_editor_button_release(text_tool);

    g_signal_handlers_unblock_by_func(
        text_tool->buffer, gimp_text_tool_buffer_end_edit, text_tool);
    g_signal_handlers_unblock_by_func(
        text_tool->buffer, gimp_text_tool_buffer_begin_edit, text_tool);
    ...
    g_object_get(rectangle, "x1", &x1, "y1", &y1, "x2", &x2, "y2", &y2, NULL);
    ...
    if (release_type == GIMP_BUTTON_RELEASE_CLICK || (x2 - x1) < 3 ||
      ...
      g_object_set(text_tool->proxy, "box-mode", GIMP_TEXT_BOX_DYNAMIC, NULL);
  ...
  if (text_tool->grab_widget) {
    gimp_tool_widget_button_release(text_tool->grab_widget, coords, time, state,
                                    release_type);
...
static void gimp_text_tool_motion(GimpTool *tool, const GimpCoords *coords,
  ...
  if (!text_tool->selecting) {
    if (text_tool->grab_widget) {
      gimp_tool_widget_motion(text_tool->grab_widget, coords, time, state);
    }
  } else {
    GimpItem *item = GIMP_ITEM(text_tool->layer);
    gdouble x = coords->x - gimp_item_get_offset_x(item);
...
static void gimp_text_tool_cursor_update(GimpTool *tool,
  ...
  if (rectangle && tool->display == display) {
    if (gimp_tool_rectangle_point_in_rectangle(rectangle, coords->x,
                                               coords->y) &&
        !text_tool->moving) {
      gimp_tool_set_cursor(tool, display, (GimpCursorType)GDK_XTERM,
...
static GimpUIManager *gimp_text_tool_get_popup(GimpTool *tool,
  ...
  if (rectangle &&
      gimp_tool_rectangle_point_in_rectangle(rectangle, coords->x, coords->y)) {
    if (!text_tool->ui_manager) {
      ...
      text_tool->ui_manager = gimp_menu_factory_manager_new(
          gimp_dialog_factory_get_menu_factory(dialog_factory), "<TextTool>",
          text_tool);
    }
...
static void gimp_text_tool_draw(GimpDrawTool *draw_tool) {
  ...
  if (gtk_text_buffer_get_has_selection(GTK_TEXT_BUFFER(text_tool->buffer))) {
    ...
    gimp_text_tool_editor_get_cursor_rect(text_tool, text_tool->overwrite_mode,
                                          &cursor_rect);
    ...
    item = gimp_draw_tool_add_text_cursor(draw_tool, &cursor_rect, overwrite,
                                          direction);
  ...
  do {
    ...
    if (i >= min && i < max) {
      ...
      case GIMP_TEXT_DIRECTION_RTL:
        rect.x += offset_x;
        rect.y += offset_y;
        gimp_draw_tool_add_rectangle(draw_tool, FALSE, rect.x, rect.y,
      ...
      case GIMP_TEXT_DIRECTION_TTB_RTL_UPRIGHT:
        ...
        gimp_draw_tool_add_rectangle(draw_tool, FALSE, rect.y, rect.x,
...
static gboolean gimp_text_tool_start(GimpTextTool *text_tool,
  ...
  g_object_set(widget, "force-narrow-mode", TRUE, "status-title",
               _("Text box: "), NULL);

  gimp_draw_tool_set_widget(GIMP_DRAW_TOOL(tool), widget);

  g_signal_connect(widget, "response",
                   G_CALLBACK(gimp_text_tool_rectangle_response), text_tool);
...
gimp_text_tool_rectangle_change_complete(GimpToolRectangle *rectangle,
  ...
  if (text_tool->handle_rectangle_change_complete) {
    ...
    if ((x2 - x1) != gimp_item_get_width(item) ||
      ...
      undo = gimp_image_undo_can_compress(
          text_tool->image, GIMP_TYPE_UNDO_STACK, GIMP_UNDO_GROUP_TEXT);
      ...
      gimp_item_translate(item, x1 - gimp_item_get_offset_x(item),
                          y1 - gimp_item_get_offset_y(item), TRUE);
...
static void gimp_text_tool_connect(GimpTextTool *text_tool,
  ...
  if (text_tool->text != text) {
    ...
    if (text_tool->text) {
      g_signal_handlers_disconnect_by_func(
          text_tool->text, gimp_text_tool_text_notify, text_tool);
      g_signal_handlers_disconnect_by_func(
          text_tool->text, gimp_text_tool_text_changed, text_tool);
    ...
    if (text) {
      ...
      g_signal_connect(text, "notify", G_CALLBACK(gimp_text_tool_text_notify),
                       text_tool);
      g_signal_connect(text, "changed", G_CALLBACK(gimp_text_tool_text_changed),
                       text_tool);
    }
  ...
  if (text_tool->layer != layer) {
    if (text_tool->layer) {
      g_signal_handlers_disconnect_by_func(
          text_tool->layer, gimp_text_tool_layer_notify, text_tool);
...
static gboolean gimp_text_tool_apply_idle(GimpTextTool *text_tool) {
  text_tool->idle_id = 0;
  ...
  if ((pspec->flags & G_PARAM_READWRITE) == G_PARAM_READWRITE &&
    ...
    if (text_tool->preedit_active) {
      ...
      if (!text_tool->idle_id) {
        ...
        text_tool->idle_id = g_idle_add_full(
            G_PRIORITY_LOW, (GSourceFunc)gimp_text_tool_apply_idle, text_tool,
            NULL);
      }
    }
  }
...
static void gimp_text_tool_text_notify(GimpText *text, const GParamSpec *pspec,
  ...
  if ((pspec->flags & G_PARAM_READWRITE) == G_PARAM_READWRITE) {
    ...
    g_signal_handlers_block_by_func(text_tool->proxy,
                                    gimp_text_tool_proxy_notify, text_tool);
   ...
   */
  if (strcmp(pspec->name, "text") == 0 || strcmp(pspec->name, "markup") == 0) {
    g_signal_handlers_block_by_func(
        text_tool->buffer, gimp_text_tool_buffer_begin_edit, text_tool);
    g_signal_handlers_block_by_func(text_tool->buffer,
                                    gimp_text_tool_buffer_end_edit, text_tool);
...
static void gimp_text_tool_apply_list(GimpTextTool *text_tool, GList *pspecs) {
  ...
  g_signal_handlers_block_by_func(dest, gimp_text_tool_text_notify, text_tool);
  g_signal_handlers_block_by_func(dest, gimp_text_tool_text_changed, text_tool);
  ...
  g_signal_handlers_unblock_by_func(dest, gimp_text_tool_text_notify,
                                    text_tool);
  g_signal_handlers_unblock_by_func(dest, gimp_text_tool_text_changed,
                                    text_tool);
}
...
static void gimp_text_tool_create_layer(GimpTextTool *text_tool,
  ...
  gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_TEXT, _("Add Text Layer"));
  ...
  g_object_get(text_tool->widget, "x1", &x1, "y1", &y1, "x2", &x2, "y2", &y2,
               NULL);
  ...
  if (text_tool->text_box_fixed) {
    ...
    g_object_set(text_tool->proxy, "box-mode", GIMP_TEXT_BOX_FIXED, "box-width",
                 gimp_pixels_to_units(x2 - x1, box_unit, xres), "box-height",
                 gimp_pixels_to_units(y2 - y1, box_unit, yres), NULL);
...
static void gimp_text_tool_confirm_response(GtkWidget *widget, gint response_id,
                                            GimpTextTool *text_tool) {
...
static void gimp_text_tool_confirm_dialog(GimpTextTool *text_tool) {
  GimpTool *tool = GIMP_TOOL(text_tool);
  GimpDisplayShell *shell = gimp_display_get_shell(tool->display);
  GtkWidget *dialog;
  GtkWidget *vbox;
  ...
  dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, text_tool->layer),
      GIMP_CONTEXT(gimp_tool_get_options(tool)), _("Confirm Text Editing"),
      "gimp-text-tool-confirm", GIMP_ICON_LAYER_TEXT_LAYER,
      _("Confirm Text Editing"), GTK_WIDGET(shell), gimp_standard_help_func,
      NULL,

      _("Create _New Layer"), RESPONSE_NEW, _("_Cancel"), GTK_RESPONSE_CANCEL,
      _("_Edit"), GTK_RESPONSE_ACCEPT,

      NULL);

  gimp_dialog_set_alternative_button_order(GTK_DIALOG(dialog), RESPONSE_NEW,
                                           GTK_RESPONSE_ACCEPT,
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(gimp_text_tool_confirm_response), text_tool);
...
static void gimp_text_tool_set_image(GimpTextTool *text_tool,
  ...
  if (text_tool->image) {
    g_signal_handlers_disconnect_by_func(
        text_tool->image, gimp_text_tool_layer_changed, text_tool);
  ...
  if (image) {
    ...
    g_signal_connect_object(text_tool->image, "selected-layers-changed",
                            G_CALLBACK(gimp_text_tool_layer_changed), text_tool,
                            0);
...
gboolean gimp_text_tool_ensure_layout(GimpTextTool *text_tool) {
  if (!text_tool->layout && text_tool->text) {
    ...
    text_tool->layout =
        gimp_text_layout_new(text_tool->layer->text, xres, yres, &error);
    if (error) {
...
void gimp_text_tool_apply(GimpTextTool *text_tool, gboolean push_undo) {
  ...
  for (list = text_tool->pending;
       list && list->next && list->next->data == list->data; list = list->next)
    /* do nothing */;
...
gboolean gimp_text_tool_set_layer(GimpTextTool *text_tool, GimpLayer *layer) {
...
void gimp_text_tool_cut_clipboard(GimpTextTool *text_tool) {
  ...
  clipboard =
      gtk_widget_get_clipboard(GTK_WIDGET(shell), GDK_SELECTION_CLIPBOARD);
...
void gimp_text_tool_copy_clipboard(GimpTextTool *text_tool) {
   ...
   */
  g_signal_handlers_block_by_func(text_tool->buffer,
                                  gimp_text_tool_buffer_begin_edit, text_tool);
  g_signal_handlers_block_by_func(text_tool->buffer,
                                  gimp_text_tool_buffer_end_edit, text_tool);

  gtk_text_buffer_copy_clipboard(GTK_TEXT_BUFFER(text_tool->buffer), clipboard);
...
void gimp_text_tool_paste_clipboard(GimpTextTool *text_tool) {
  ...
  gtk_text_buffer_paste_clipboard(GTK_TEXT_BUFFER(text_tool->buffer), clipboard,
                                  NULL, TRUE);
}
