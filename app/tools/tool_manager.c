...
void tool_manager_init(Gimp *gimp) {
  ...
  tool_manager->image_clean_handler_id = gimp_container_add_handler(
      gimp->images, "clean", G_CALLBACK(tool_manager_image_clean_dirty),
      tool_manager);

  tool_manager->image_dirty_handler_id = gimp_container_add_handler(
      gimp->images, "dirty", G_CALLBACK(tool_manager_image_clean_dirty),
      tool_manager);

  tool_manager->image_saving_handler_id = gimp_container_add_handler(
      gimp->images, "saving", G_CALLBACK(tool_manager_image_saving),
      tool_manager);
  ...
  g_signal_connect(user_context, "tool-preset-changed",
                   G_CALLBACK(tool_manager_preset_changed), tool_manager);

  tool_manager_tool_changed(user_context, gimp_context_get_tool(user_context),
                            tool_manager);
}
...
void tool_manager_exit(Gimp *gimp) {
  ...
  g_signal_handlers_disconnect_by_func(user_context, tool_manager_tool_changed,
                                       tool_manager);
  g_signal_handlers_disconnect_by_func(
      user_context, tool_manager_preset_changed, tool_manager);
...
void tool_manager_push_tool(Gimp *gimp, GimpTool *tool) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool) {
    ...
    tool_manager->tool_stack =
        g_slist_prepend(tool_manager->tool_stack, tool_manager->active_tool);
...
void tool_manager_pop_tool(Gimp *gimp) {
  ...
  if (tool_manager->tool_stack) {
    ...
    tool_manager->tool_stack = g_slist_remove(tool_manager->tool_stack, tool);
...
gboolean tool_manager_initialize_active(Gimp *gimp, GimpDisplay *display) {
  GimpToolManager *tool_manager;
...
void tool_manager_control_active(Gimp *gimp, GimpToolAction action,
                                 GimpDisplay *display) {
  GimpToolManager *tool_manager;
...
void tool_manager_button_press_active(Gimp *gimp, const GimpCoords *coords,
  ...
  if (tool_manager->active_tool) {
    gimp_tool_button_press(tool_manager->active_tool, coords, time, state,
                           press_type, display);
  }
}

void tool_manager_button_release_active(Gimp *gimp, const GimpCoords *coords,
                                        guint32 time, GdkModifierType state,
                                        GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool) {
    gimp_tool_button_release(tool_manager->active_tool, coords, time, state,
                             display);
  }
}

void tool_manager_motion_active(Gimp *gimp, const GimpCoords *coords,
                                guint32 time, GdkModifierType state,
                                GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool) {
    gimp_tool_motion(tool_manager->active_tool, coords, time, state, display);
  }
}

gboolean tool_manager_key_press_active(Gimp *gimp, GdkEventKey *kevent,
                                       GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool) {
    return gimp_tool_key_press(tool_manager->active_tool, kevent, display);
  }

  return FALSE;
}

gboolean tool_manager_key_release_active(Gimp *gimp, GdkEventKey *kevent,
                                         GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool) {
    return gimp_tool_key_release(tool_manager->active_tool, kevent, display);
  }

  return FALSE;
}

void tool_manager_focus_display_active(Gimp *gimp, GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool &&
    ...
    gimp_tool_set_focus_display(tool_manager->active_tool, display);
  }
}

void tool_manager_modifier_state_active(Gimp *gimp, GdkModifierType state,
                                        GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool &&
      !gimp_tool_control_is_active(tool_manager->active_tool->control)) {
    gimp_tool_set_modifier_state(tool_manager->active_tool, state, display);
  }
}
...
void tool_manager_active_modifier_state_active(Gimp *gimp,
  ...
  if (tool_manager->active_tool) {
    gimp_tool_set_active_modifier_state(tool_manager->active_tool, state,
                                        display);
  }
}

void tool_manager_oper_update_active(Gimp *gimp, const GimpCoords *coords,
  ...
  if (tool_manager->active_tool &&
      !gimp_tool_control_is_active(tool_manager->active_tool->control)) {
    gimp_tool_oper_update(tool_manager->active_tool, coords, state, proximity,
                          display);
  }
}

void tool_manager_cursor_update_active(Gimp *gimp, const GimpCoords *coords,
                                       GdkModifierType state,
                                       GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool &&
      !gimp_tool_control_is_active(tool_manager->active_tool->control)) {
    gimp_tool_cursor_update(tool_manager->active_tool, coords, state, display);
  }
}

const gchar *tool_manager_can_undo_active(Gimp *gimp, GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool) {
    return gimp_tool_can_undo(tool_manager->active_tool, display);
  }

  return NULL;
}

const gchar *tool_manager_can_redo_active(Gimp *gimp, GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool) {
    return gimp_tool_can_redo(tool_manager->active_tool, display);
  }

  return NULL;
}

gboolean tool_manager_undo_active(Gimp *gimp, GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool) {
    return gimp_tool_undo(tool_manager->active_tool, display);
  }

  return FALSE;
}

gboolean tool_manager_redo_active(Gimp *gimp, GimpDisplay *display) {
  GimpToolManager *tool_manager;
  ...
  if (tool_manager->active_tool) {
    return gimp_tool_redo(tool_manager->active_tool, display);
  }
...
GimpUIManager *tool_manager_get_popup_active(Gimp *gimp,
  ...
  if (tool_manager->active_tool) {
    return gimp_tool_get_popup(tool_manager->active_tool, coords, state,
                               display, ui_path);
  }
  ...
  if (tool_group != tool_manager->active_tool_group) {
    if (tool_manager->active_tool_group) {
      g_signal_handlers_disconnect_by_func(
          ...
          tool_manager_group_active_tool_changed, tool_manager);
    }

    g_set_weak_pointer(&tool_manager->active_tool_group, tool_group);

    if (tool_manager->active_tool_group) {
      g_signal_connect(tool_manager->active_tool_group, "active-tool-changed",
...
static void tool_manager_tool_changed(GimpContext *user_context,
  ...
  if (!g_type_is_a(tool_info->tool_type, GIMP_TYPE_TOOL)) {
    g_warning("%s: tool_info->tool_type is no GimpTool subclass", G_STRFUNC);
    return;
  }
  ...
  if (user_context->gimp->busy) {
    ...
    if (G_TYPE_FROM_INSTANCE(tool_manager->active_tool) !=
        tool_info->tool_type) {
      g_signal_handlers_block_by_func(user_context, tool_manager_tool_changed,
                                      tool_manager);

      /*  explicitly set the current tool  */
      gimp_context_set_tool(user_context, tool_manager->active_tool->tool_info);

      g_signal_handlers_unblock_by_func(user_context, tool_manager_tool_changed,
                                        tool_manager);
    }
  ...
  new_tool = g_object_new(tool_info->tool_type, "tool-info", tool_info, NULL);
...
static void tool_manager_copy_tool_options(GObject *src, GObject *dest) {
  GList *diff;
...
static void tool_manager_preset_changed(GimpContext *user_context,
   ...
   */
  gimp_context_copy_properties(GIMP_CONTEXT(preset->tool_options), user_context,
...
static void tool_manager_image_clean_dirty(GimpImage *image,
  ...
  if (tool && !gimp_tool_control_get_preserve(tool->control) &&
    ...
    if (display) {
      tool_manager_control_active(
          image->gimp, gimp_tool_control_get_dirty_action(tool->control),
          display);
    }
  }
}
...
static void tool_manager_tool_ancestry_changed(GimpToolInfo *tool_info,
  ...
  if (parent) {
    gimp_tool_group_set_active_tool_info(GIMP_TOOL_GROUP(parent), tool_info);
  }
