...
void gimp_airbrush_tool_register(GimpToolRegisterCallback callback,
                                 gpointer data) {
  (*callback)(GIMP_TYPE_AIRBRUSH_TOOL, GIMP_TYPE_AIRBRUSH_OPTIONS,
              gimp_airbrush_options_gui,
              GIMP_PAINT_OPTIONS_CONTEXT_MASK | GIMP_CONTEXT_PROP_MASK_GRADIENT,
              "gimp-airbrush-tool", _("Airbrush"),
              _("Airbrush Tool: Paint using a brush, with variable pressure"),
              N_("_Airbrush"), "A", NULL, GIMP_HELP_TOOL_AIRBRUSH,
              GIMP_ICON_TOOL_AIRBRUSH, data);
}
...
static void gimp_airbrush_tool_constructed(GObject *object) {
  ...
  g_signal_connect_object(paint_tool->core, "stamp",
                          G_CALLBACK(gimp_airbrush_tool_airbrush_stamp), object,
                          0);
}
...
static void gimp_airbrush_tool_airbrush_stamp(GimpAirbrush *airbrush,
  ...
  gimp_paint_tool_paint_push(
      paint_tool, (GimpPaintToolPaintFunc)gimp_airbrush_tool_stamp, NULL);
}
