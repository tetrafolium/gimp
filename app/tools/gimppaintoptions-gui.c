...
static GtkWidget *gimp_paint_options_gui_scale_with_buttons(
    GObject *config, gchar *prop_name, gchar *link_prop_name,
    gchar *reset_tooltip, gdouble step_increment, gdouble page_increment,
    gint digits, gdouble scale_min, gdouble scale_max, gdouble factor,
    gdouble gamma, GCallback reset_callback, GtkSizeGroup *link_group);
...
GtkWidget *gimp_paint_options_gui(GimpToolOptions *tool_options) {
  ...
  g_object_set_data(G_OBJECT(vbox), "gimp-paint-options-gui-paint-mode-box",
                    menu);

  if (tool_type == GIMP_TYPE_ERASER_TOOL ||
      tool_type == GIMP_TYPE_CONVOLVE_TOOL ||
      tool_type == GIMP_TYPE_DODGE_BURN_TOOL ||
      tool_type == GIMP_TYPE_HEAL_TOOL || tool_type == GIMP_TYPE_MYBRUSH_TOOL ||
      tool_type == GIMP_TYPE_SMUDGE_TOOL) {
    gtk_widget_set_sensitive(menu, FALSE);
  }

  /*  the opacity scale  */
  scale = gimp_prop_spin_scale_new(config, "opacity", NULL, 0.01, 0.1, 0);
  gimp_spin_scale_set_constrain_drag(GIMP_SPIN_SCALE(scale), TRUE);
  gimp_prop_widget_set_factor(scale, 100.0, 0.0, 0.0, 1);
  gtk_box_pack_start(GTK_BOX(vbox), scale, FALSE, FALSE, 0);

  /*  temp debug foo, disabled in stable  */
  if (FALSE && g_type_is_a(tool_type, GIMP_TYPE_PAINT_TOOL) &&
  ...
  if (g_type_is_a(tool_type, GIMP_TYPE_BRUSH_TOOL)) {
    ...
    button = gimp_prop_brush_box_new(
        NULL, GIMP_CONTEXT(tool_options), _("Brush"), 2, "brush-view-type",
        "brush-view-size", "gimp-brush-editor", _("Edit this brush"));
    gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 0);
    ...
    hbox = gimp_paint_options_gui_scale_with_buttons(
        ...
        _("Reset size to brush's native size"), 1.0, 10.0, 2, 1.0, 1000.0, 1.0,
        1.7, G_CALLBACK(gimp_paint_options_gui_reset_size), link_group);
    gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE, FALSE, 0);
    gtk_widget_show(hbox);

    hbox = gimp_paint_options_gui_scale_with_buttons(
        config, "brush-aspect-ratio", "brush-link-aspect-ratio",
        _("Reset aspect ratio to brush's native aspect ratio"), 0.1, 1.0, 2,
        -20.0, 20.0, 1.0, 1.0,
    ...
    hbox = gimp_paint_options_gui_scale_with_buttons(
        ...
        _("Reset angle to brush's native angle"), 0.1, 1.0, 2, -180.0, 180.0,
        1.0, 1.0, G_CALLBACK(gimp_paint_options_gui_reset_angle), link_group);
    gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE, FALSE, 0);
    gtk_widget_show(hbox);

    hbox = gimp_paint_options_gui_scale_with_buttons(
        config, "brush-spacing", "brush-link-spacing",
        _("Reset spacing to brush's native spacing"), 0.1, 1.0, 1, 1.0, 200.0,
        100.0, 1.7, G_CALLBACK(gimp_paint_options_gui_reset_spacing),
        link_group);
    gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE, FALSE, 0);
    gtk_widget_show(hbox);
    ...
    hbox = gimp_paint_options_gui_scale_with_buttons(
        ...
        _("Reset hardness to brush's native hardness"), 0.1, 1.0, 1, 0.0, 100.0,
        100.0, 1.0, G_CALLBACK(gimp_paint_options_gui_reset_hardness),
        link_group);
    gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE, FALSE, 0);
    gtk_widget_show(hbox);

    hbox = gimp_paint_options_gui_scale_with_buttons(
        config, "brush-force", NULL, _("Reset force to default"), 0.1, 1.0, 1,
        0.0, 100.0, 100.0, 1.0, G_CALLBACK(gimp_paint_options_gui_reset_force),
        link_group);
    gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE, FALSE, 0);
    gtk_widget_show(hbox);
    ...
    button = gimp_prop_dynamics_box_new(
        NULL, GIMP_CONTEXT(tool_options), _("Dynamics"), 2,
        "dynamics-view-type", "dynamics-view-size", "gimp-dynamics-editor",
  ...
  if (tool_type == GIMP_TYPE_PENCIL_TOOL ||
    ...
    button = gimp_prop_enum_check_button_new(config, "application-mode", NULL,
                                             GIMP_PAINT_CONSTANT,
  ...
  if (tool_type == GIMP_TYPE_ERASER_TOOL || tool_type == GIMP_TYPE_CLONE_TOOL ||
...
static GtkWidget *dynamics_options_gui(GimpPaintOptions *paint_options,
  ...
  scale = gimp_prop_spin_scale_new(config, "fade-length", NULL, 1.0, 50.0, 0);
  ...
  if (g_type_is_a(tool_type, GIMP_TYPE_PAINTBRUSH_TOOL) ||
    ...
    box = gimp_prop_gradient_box_new(
        NULL, GIMP_CONTEXT(config), _("Gradient"), 2, "gradient-view-type",
        "gradient-view-size", "gradient-reverse", "gradient-blend-color-space",
        "gimp-gradient-editor", _("Edit this gradient"));
...
static GtkWidget *jitter_options_gui(GimpPaintOptions *paint_options,
  ...
  scale = gimp_prop_spin_scale_new(config, "jitter-amount", NULL, 0.01, 1.0, 2);
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(scale), 0.0, 5.0);

  frame =
      gimp_prop_expanding_frame_new(config, "use-jitter", NULL, scale, NULL);
...
static GtkWidget *smoothing_options_gui(GimpPaintOptions *paint_options,
  ...
  frame =
      gimp_prop_expanding_frame_new(config, "use-smoothing", NULL, vbox, NULL);

  scale = gimp_prop_spin_scale_new(config, "smoothing-quality", NULL, 1, 10, 1);
  gtk_box_pack_start(GTK_BOX(vbox), scale, FALSE, FALSE, 0);

  scale = gimp_prop_spin_scale_new(config, "smoothing-factor", NULL, 1, 10, 1);
  gtk_box_pack_start(GTK_BOX(vbox), scale, FALSE, FALSE, 0);
...
gimp_paint_options_gui_reset_force(GtkWidget *button,
                                   GimpPaintOptions *paint_options) {
  g_object_set(paint_options, "brush-force", 0.5, NULL);
}
...
static GtkWidget *gimp_paint_options_gui_scale_with_buttons(
  ...
  scale = gimp_prop_spin_scale_new(config, prop_name, NULL, step_increment,
                                   page_increment, digits);
  gimp_spin_scale_set_constrain_drag(GIMP_SPIN_SCALE(scale), TRUE);

  gimp_prop_widget_set_factor(scale, factor, step_increment, page_increment,
                              digits);
  gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(scale), scale_min,
                                   scale_max);
  ...
  g_signal_connect(button, "clicked", reset_callback, config);
  ...
  if (link_prop_name) {
    ...
    image = gtk_image_new_from_icon_name(GIMP_ICON_LINKED, GTK_ICON_SIZE_MENU);
    gtk_container_add(GTK_CONTAINER(button), image);
    gtk_widget_show(image);

    g_object_bind_property(config, link_prop_name, button, "active",
  ...
  gimp_help_set_help_data(button, _("Link to brush default"), NULL);
