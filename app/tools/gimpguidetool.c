...
#define SWAP_ORIENT(orient)                                                    \
  ((orient) == GIMP_ORIENTATION_HORIZONTAL ? GIMP_ORIENTATION_VERTICAL         \
...
static void gimp_guide_tool_button_release(GimpTool *tool,
                                           ...
                                           GimpDisplay *display);
static void gimp_guide_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                   guint32 time, GdkModifierType state,
                                   GimpDisplay *display);

static void gimp_guide_tool_draw(GimpDrawTool *draw_tool);

static void gimp_guide_tool_start(GimpTool *parent_tool, GimpDisplay *display,
                                  GList *guides,
...
static void gimp_guide_tool_class_init(GimpGuideToolClass *klass) {
...
static void gimp_guide_tool_button_release(GimpTool *tool,
  ...
  if (release_type == GIMP_BUTTON_RELEASE_CANCEL) {
    for (i = 0; i < guide_tool->n_guides; i++) {
      ...
      if (guide->custom) {
        gimp_image_move_guide(image, guide->guide, guide->old_position, TRUE);
      }
    }
  } else {
    ...
    for (i = 0; i < guide_tool->n_guides; i++) {
      ...
      if (guide->position == GIMP_GUIDE_POSITION_UNDEFINED ||
          guide->position < 0 || guide->position > max_position) {
        remove_guides = TRUE;
      }
    }
    ...
    for (i = 0; i < guide_tool->n_guides; i++) {
      ...
      if (remove_guides) {
        ...
        if (guide->guide) {
          /* custom guides are moved live */
          if (!guide->custom) {
            gimp_image_move_guide(image, guide->guide, guide->position, TRUE);
          }
        } else {
          switch (guide->orientation) {
          case GIMP_ORIENTATION_HORIZONTAL:
            gimp_image_add_hguide(image, guide->position, TRUE);
            break;

          case GIMP_ORIENTATION_VERTICAL:
            gimp_image_add_vguide(image, guide->position, TRUE);
            break;
...

  {
    ...
    tool_manager_oper_update_active(display->gimp, coords, state, TRUE,
                                    display);
    tool_manager_cursor_update_active(display->gimp, coords, state, display);
...

{
  ...
  gimp_display_shell_transform_xy(shell, coords->x, coords->y, &tx, &ty);
  ...
  for (i = 0; i < guide_tool->n_guides; i++) {
    ...
    if (tx < 0 || tx >= shell->disp_width || ty < 0 ||
...
static void gimp_guide_tool_draw(GimpDrawTool *draw_tool) {
  ...
  for (i = 0; i < guide_tool->n_guides; i++) {
    ...
    if (guide->position != GIMP_GUIDE_POSITION_UNDEFINED) {
      /* custom guides are moved live */
      if (!guide->custom) {
        gimp_draw_tool_add_guide(draw_tool, guide->orientation, guide->position,
...
static void gimp_guide_tool_start(GimpTool *parent_tool, GimpDisplay *display,
  ...
  guide_tool = g_object_new(GIMP_TYPE_GUIDE_TOOL, "tool-info",
  ...
  gimp_tool_set_cursor(tool, display, GIMP_CURSOR_MOUSE, GIMP_TOOL_CURSOR_HAND,
  ...
  if (remove_guides) {
    ...
    if (n_guides == 1) {
      gimp_tool_push_status_length(
          tool, display, _("Move Guide: "), SWAP_ORIENT(guides[0]->orientation),
          guides[0]->position - guides[0]->old_position, NULL);
    } else if (n_guides == 2) {
      gimp_tool_push_status_coords(
          tool, display, GIMP_CURSOR_PRECISION_PIXEL_BORDER, _("Move Guides: "),
          guides[0]->position - guides[0]->old_position, ", ",
          guides[1]->position - guides[1]->old_position, NULL);
    } else {
      gimp_tool_push_status_length(
          tool, display, _("Add Guide: "),
          SWAP_ORIENT(guide_tool->guides[0].orientation),
          guide_tool->guides[0].position, NULL);
    }
  }
}
...
void gimp_guide_tool_start_new(GimpTool *parent_tool, GimpDisplay *display,
                               GimpOrientationType orientation) {
  g_return_if_fail(GIMP_IS_TOOL(parent_tool));
  g_return_if_fail(GIMP_IS_DISPLAY(display));
  g_return_if_fail(orientation != GIMP_ORIENTATION_UNKNOWN);

  gimp_guide_tool_start(parent_tool, display, NULL, orientation);
}

void gimp_guide_tool_start_edit(GimpTool *parent_tool, GimpDisplay *display,
                                GimpGuide *guide) {
  GList *guides = NULL;
  ...
  gimp_guide_tool_start(parent_tool, display, guides, GIMP_ORIENTATION_UNKNOWN);

  g_list_free(guides);
}

void gimp_guide_tool_start_edit_many(GimpTool *parent_tool,
                                     GimpDisplay *display, GList *guides) {
  g_return_if_fail(GIMP_IS_TOOL(parent_tool));
  g_return_if_fail(GIMP_IS_DISPLAY(display));
