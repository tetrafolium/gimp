...
static gboolean gimp_filter_tool_initialize(GimpTool *tool,
                                            GimpDisplay *display,
                                            GError **error);
static void gimp_filter_tool_control(GimpTool *tool, GimpToolAction action,
                                     GimpDisplay *display);
...
static void gimp_filter_tool_button_release(GimpTool *tool,
                                            ...
                                            GimpDisplay *display);
static void gimp_filter_tool_motion(GimpTool *tool, const GimpCoords *coords,
                                    guint32 time, GdkModifierType state,
                                    GimpDisplay *display);
static gboolean gimp_filter_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
                                           GimpDisplay *display);
...
static void gimp_filter_tool_color_picked(GimpColorTool *color_tool,
                                          ...
                                          const Babl *sample_format,
                                          gpointer pixel, const GimpRGB *color);
...
static void gimp_filter_tool_real_set_config(GimpFilterTool *filter_tool,
                                             GimpConfig *config);
static void gimp_filter_tool_real_config_notify(GimpFilterTool *filter_tool,
                                                GimpConfig *config,
                                                const GParamSpec *pspec);
...
static void gimp_filter_tool_unset_setting(GObject *object,
                                           const GParamSpec *pspec,
                                           GimpFilterTool *filter_tool);
static void gimp_filter_tool_lock_position_changed(GimpDrawable *drawable,
                                                   GimpFilterTool *filter_tool);
...
static void gimp_filter_tool_response(GimpToolGui *gui, gint response_id,
                                      GimpFilterTool *filter_tool);
...
static void gimp_filter_tool_class_init(GimpFilterToolClass *klass) {
...
static void gimp_filter_tool_init(GimpFilterTool *filter_tool) {
  GimpTool *tool = GIMP_TOOL(filter_tool);

  gimp_tool_control_set_scroll_lock(tool->control, TRUE);
  gimp_tool_control_set_preserve(tool->control, FALSE);
  gimp_tool_control_set_dirty_mask(
      tool->control, GIMP_DIRTY_IMAGE | GIMP_DIRTY_IMAGE_STRUCTURE |
                         GIMP_DIRTY_DRAWABLE | GIMP_DIRTY_ACTIVE_DRAWABLE);
...
static gboolean gimp_filter_tool_initialize(GimpTool *tool,
  ...
  if (!filter_tool->gui) {
    ...
    filter_tool->gui = gimp_tool_gui_new(
        tool_info, gimp_tool_get_label(tool), filter_tool->description,
        gimp_tool_get_icon_name(tool), gimp_tool_get_help_id(tool),
        gimp_widget_get_monitor(GTK_WIDGET(shell)), filter_tool->overlay,
    ...
    gimp_tool_gui_set_alternative_button_order(filter_tool->gui, RESPONSE_RESET,
                                               GTK_RESPONSE_OK,
    ...
    if (filter_tool->config) {
      ...
      gtk_box_pack_start(GTK_BOX(vbox), filter_tool->settings_box, FALSE, FALSE,
                         0);
    ...
    g_object_bind_property(G_OBJECT(tool_info->tool_options), "preview", toggle,
                           "sensitive", G_BINDING_SYNC_CREATE);

    /*  The show-controller toggle  */
    filter_tool->controller_toggle = gimp_prop_check_button_new(
        G_OBJECT(tool_info->tool_options), "controller", NULL);
    gtk_box_pack_end(GTK_BOX(vbox), filter_tool->controller_toggle, FALSE,
                     FALSE, 0);
    if (!filter_tool->widget)
      gtk_widget_hide(filter_tool->controller_toggle);

    /*  The operation-settings box  */
    filter_tool->operation_settings_box =
        gtk_box_new(GTK_ORIENTATION_VERTICAL, 2);
    gtk_box_pack_end(GTK_BOX(vbox), filter_tool->operation_settings_box, FALSE,
                     FALSE, 0);
    ...
    gimp_tool_gui_set_title(filter_tool->gui, gimp_tool_get_label(tool));
    ...
    gimp_tool_gui_set_help_id(filter_tool->gui, gimp_tool_get_help_id(tool));
  }
...
static void gimp_filter_tool_button_press(GimpTool *tool,
  ...
  if (gimp_filter_tool_on_guide(filter_tool, coords, display)) {
    ...
    if (state & gimp_get_extend_selection_mask()) {
      ...
      pos_x = CLAMP(RINT(coords->x) - gimp_item_get_offset_x(item), 0,
                    gimp_item_get_width(item));
      pos_y = CLAMP(RINT(coords->y) - gimp_item_get_offset_y(item), 0,
                    gimp_item_get_height(item));

      gimp_filter_options_switch_preview_orientation(options, pos_x, pos_y);
    } else {
      gimp_guide_tool_start_edit(tool, display, filter_tool->preview_guide);
    }
    ...
    if (gimp_tool_widget_button_press(filter_tool->widget, coords, time, state,
                                      press_type)) {
...
static void gimp_filter_tool_button_release(GimpTool *tool,
  ...
  if (filter_tool->grab_widget) {
    ...
    gimp_tool_widget_button_release(filter_tool->grab_widget, coords, time,
                                    state, release_type);
...
static gboolean gimp_filter_tool_key_press(GimpTool *tool, GdkEventKey *kevent,
  ...
  if (filter_tool->gui && display == tool->display) {
    ...
    case GDK_KEY_ISO_Enter:
      gimp_filter_tool_response(filter_tool->gui, GTK_RESPONSE_OK, filter_tool);
      return TRUE;

    case GDK_KEY_BackSpace:
      gimp_filter_tool_response(filter_tool->gui, RESPONSE_RESET, filter_tool);
      return TRUE;

    case GDK_KEY_Escape:
      gimp_filter_tool_response(filter_tool->gui, GTK_RESPONSE_CANCEL,
                                filter_tool);
      return TRUE;
    }
  }
...
static void gimp_filter_tool_oper_update(GimpTool *tool,
  ...
  if (gimp_filter_tool_on_guide(filter_tool, coords, display)) {
    ...
    if (state & extend_mask) {
      ...
      status =
          gimp_suggest_modifiers(_("Click to move the split guide"),
                                 (extend_mask | toggle_mask) & ~state,
                                 _("%s: switch original and filtered"),
                                 _("%s: switch horizontal and vertical"), NULL);
    }
    ...
    GIMP_TOOL_CLASS(parent_class)
        ->oper_update(tool, coords, state, proximity, display);
  }
}
...
static void gimp_filter_tool_cursor_update(GimpTool *tool,
  ...
  if (gimp_filter_tool_on_guide(filter_tool, coords, display)) {
    gimp_tool_set_cursor(tool, display, GIMP_CURSOR_MOUSE,
                         GIMP_TOOL_CURSOR_HAND, GIMP_CURSOR_MODIFIER_MOVE);
  } else {
...
static void gimp_filter_tool_options_notify(GimpTool *tool,
  ...
  if (!strcmp(pspec->name, "preview") && filter_tool->filter) {
  ...
  } else if (!strcmp(pspec->name, "preview-split") && filter_tool->filter) {
    if (filter_options->preview_split) {
      ...
      gimp_display_shell_untransform_viewport(shell, TRUE, &x, &y, &width,
                                              &height);

      if (!gimp_rectangle_intersect(
              gimp_item_get_offset_x(item), gimp_item_get_offset_y(item),
              gimp_item_get_width(item), gimp_item_get_height(item), x, y,
      ...
      g_object_set(options, "preview-split-position", position, NULL);
    }
  ...
  } else if (!strcmp(pspec->name, "controller") && filter_tool->widget) {
...
static gboolean gimp_filter_tool_can_pick_color(GimpColorTool *color_tool,
  ...
  return filter_tool->pick_abyss || GIMP_COLOR_TOOL_CLASS(parent_class)
                                        ->can_pick(color_tool, coords, display);
}
...
static gboolean gimp_filter_tool_pick_color(GimpColorTool *color_tool,
  ...
  picked = GIMP_COLOR_TOOL_CLASS(parent_class)
               ->pick(color_tool, coords, display, sample_format, pixel, color);
...
static void gimp_filter_tool_color_picked(
    GimpColorTool *color_tool, const GimpCoords *coords, GimpDisplay *display,
    GimpColorPickState pick_state, const Babl *sample_format, gpointer pixel,
    const GimpRGB *color) {
  GimpFilterTool *filter_tool = GIMP_FILTER_TOOL(color_tool);
  ...
  if (filter_tool->active_picker) {
    ...
    if (callback) {
      callback(callback_data, filter_tool->pick_identifier, coords->x,
               coords->y, sample_format, color);

      return;
    }
  }

  GIMP_FILTER_TOOL_GET_CLASS(filter_tool)
      ->color_picked(filter_tool, filter_tool->pick_identifier, coords->x,
                     coords->y, sample_format, color);
}
...
static void gimp_filter_tool_real_set_config(GimpFilterTool *filter_tool,
   ...
   */
  g_object_get(config, "gimp-region", &region, NULL);
  g_object_set(filter_tool->config, "gimp-region", region, NULL);

  gimp_config_copy(GIMP_CONFIG(config), GIMP_CONFIG(filter_tool->config), 0);
   ...
   *  used settings
   */
  g_object_set(filter_tool->config, "time", (gint64)0, NULL);
}
...
static void gimp_filter_tool_halt(GimpFilterTool *filter_tool) {
  ...
  if (tool->display) {
    ...
    for (iter = tool->drawables; iter; iter = iter->next)
      g_signal_handlers_disconnect_by_func(
          iter->data, gimp_filter_tool_lock_position_changed, filter_tool);

    g_signal_handlers_disconnect_by_func(image, gimp_filter_tool_mask_changed,
                                         filter_tool);
  }
  ...
  if (filter_tool->config) {
    g_signal_handlers_disconnect_by_func(
        filter_tool->config, gimp_filter_tool_config_notify, filter_tool);
    g_signal_handlers_disconnect_by_func(
...
static void gimp_filter_tool_commit(GimpFilterTool *filter_tool) {
  ...
  if (filter_tool->filter) {
    ...
    gimp_drawable_filter_commit(filter_tool->filter, GIMP_PROGRESS(tool), TRUE);
    g_clear_object(&filter_tool->filter);
    ...
    if (filter_tool->config && filter_tool->has_settings) {
      ...
      gimp_settings_box_add_current(
          GIMP_SETTINGS_BOX(filter_tool->settings_box),
...
gimp_filter_tool_update_dialog_operation_settings(GimpFilterTool *filter_tool) {
  ...
  if (filter_tool->operation_settings_box) {
    ...
    if (filter_tool->config) {
      ...
      /*  The clipping combo  */
      filter_tool->clip_combo = gimp_prop_enum_combo_box_new(
          filter_tool->config, "gimp-clip", GIMP_TRANSFORM_RESIZE_ADJUST,
          GIMP_TRANSFORM_RESIZE_CLIP);
      gimp_int_combo_box_set_label(GIMP_INT_COMBO_BOX(filter_tool->clip_combo),
                                   _("Clipping"));
      gtk_box_pack_start(GTK_BOX(vbox), filter_tool->clip_combo, FALSE, FALSE,
                         0);

      /*  The region combo  */
      filter_tool->region_combo = gimp_prop_enum_combo_box_new(
          filter_tool->config, "gimp-region", 0, 0);
      gtk_box_pack_start(GTK_BOX(vbox), filter_tool->region_combo, FALSE, FALSE,
                         0);
      ...
      g_object_bind_property(options, "blending-options-expanded", expander,
                             "expanded",
      ...
      /*  The mode box  */
      mode_box = gimp_prop_layer_mode_box_new(filter_tool->config, "gimp-mode",
                                              GIMP_LAYER_MODE_CONTEXT_FILTER);
      gimp_layer_mode_box_set_label(GIMP_LAYER_MODE_BOX(mode_box), _("Mode"));
      gtk_box_pack_start(GTK_BOX(vbox2), mode_box, FALSE, FALSE, 0);
      gtk_widget_show(mode_box);

      /*  The opacity scale  */
      scale = gimp_prop_spin_scale_new(filter_tool->config, "gimp-opacity",
                                       NULL, 1.0, 10.0, 1);
      ...
      g_object_bind_property(image->gimp->config,
                             "filter-tool-show-color-options", expander,
                             "visible", G_BINDING_SYNC_CREATE);
      g_object_bind_property(options, "color-options-expanded", expander,
                             "expanded",
                             G_BINDING_SYNC_CREATE | G_BINDING_BIDIRECTIONAL);
...
static void gimp_filter_tool_create_filter(GimpFilterTool *filter_tool) {
  ...
  filter_tool->filter = gimp_drawable_filter_new(
      tool->drawables->data, gimp_tool_get_undo_desc(tool),
      filter_tool->operation, gimp_tool_get_icon_name(tool));

  gimp_filter_tool_update_filter(filter_tool);

  g_signal_connect(filter_tool->filter, "flush",
                   G_CALLBACK(gimp_filter_tool_flush), filter_tool);

  gimp_gegl_progress_connect(filter_tool->operation, GIMP_PROGRESS(filter_tool),
  ...
  if (filter_tool->gui) {
    ...
    if (gimp_channel_is_empty(mask)) {
      gtk_widget_set_visible(
          ...
          gimp_item_get_clip(GIMP_ITEM(tool->drawables->data), FALSE) ==
                  FALSE &&
...
static void gimp_filter_tool_config_notify(GObject *object,
                                           ...
                                           GimpFilterTool *filter_tool) {
  GIMP_FILTER_TOOL_GET_CLASS(filter_tool)
      ->config_notify(filter_tool, GIMP_CONFIG(object), pspec);
}
...
static void gimp_filter_tool_add_guide(GimpFilterTool *filter_tool) {
  ...
  if (options->preview_split_alignment == GIMP_ALIGN_LEFT ||
    ...
    position = gimp_item_get_offset_x(item) + options->preview_split_position;
  } else {
    orientation = GIMP_ORIENTATION_HORIZONTAL;
    position = gimp_item_get_offset_y(item) + options->preview_split_position;
  }

  filter_tool->preview_guide = gimp_guide_custom_new(
      orientation, image->gimp->next_guide_id++, GIMP_GUIDE_STYLE_SPLIT_VIEW);

  gimp_image_add_guide(image, filter_tool->preview_guide, position);

  g_signal_connect(filter_tool->preview_guide, "removed",
                   G_CALLBACK(gimp_filter_tool_guide_removed), filter_tool);
  g_signal_connect(filter_tool->preview_guide, "notify::position",
                   G_CALLBACK(gimp_filter_tool_guide_moved), filter_tool);
}
...
static void gimp_filter_tool_move_guide(GimpFilterTool *filter_tool) {
  ...
  if (orientation != gimp_guide_get_orientation(filter_tool->preview_guide) ||
    ...
    gimp_image_move_guide(gimp_item_get_image(item), filter_tool->preview_guide,
                          position, FALSE);
  }
}
...
static void gimp_filter_tool_guide_removed(GimpGuide *guide,
  ...
  g_object_set(options, "preview-split", FALSE, NULL);
}
...
static void gimp_filter_tool_guide_moved(GimpGuide *guide,
  ...
  if (options->preview_split_alignment == GIMP_ALIGN_LEFT ||
      options->preview_split_alignment == GIMP_ALIGN_RIGHT) {
    position =
        CLAMP(gimp_guide_get_position(guide) - gimp_item_get_offset_x(item), 0,
              gimp_item_get_width(item));
  } else {
    position =
        CLAMP(gimp_guide_get_position(guide) - gimp_item_get_offset_y(item), 0,
              gimp_item_get_height(item));
  }
...
static void gimp_filter_tool_update_filter(GimpFilterTool *filter_tool) {
  ...
  gimp_drawable_filter_set_preview_split(
      filter_tool->filter, options->preview_split,
      options->preview_split_alignment, options->preview_split_position);
...
static void gimp_filter_tool_set_has_settings(GimpFilterTool *filter_tool,
  ...
  if (filter_tool->has_settings) {
    ...
    import_title =
        g_strdup_printf(_("Import '%s' Settings"), gimp_tool_get_label(tool));
    export_title =
        g_strdup_printf(_("Export '%s' Settings"), gimp_tool_get_label(tool));

    g_object_set(filter_tool->settings_box, "visible", TRUE, "config",
                 filter_tool->config, "container", filter_tool->settings,
                 "help-id", gimp_tool_get_help_id(tool), "import-title",
                 import_title, "export-title", export_title, "default-folder",
                 settings_folder, "last-file", NULL, NULL);

    g_free(import_title);
    g_free(export_title);
  } else {
    g_object_set(filter_tool->settings_box, "visible", FALSE, "config", NULL,
                 "container", NULL, "help-id", NULL, "import-title", NULL,
                 "export-title", NULL, "default-folder", NULL, "last-file",
                 NULL, NULL);
  }
}
...
void gimp_filter_tool_get_operation(GimpFilterTool *filter_tool) {
  ...
  operation_name = klass->get_operation(filter_tool, &filter_tool->description);
  ...
  filter_tool->operation =
      gegl_node_new_child(NULL, "operation", operation_name, NULL);

  filter_tool->config = g_object_new(
      gimp_operation_config_get_type(tool->tool_info->gimp, operation_name,
                                     gimp_tool_get_icon_name(tool),
                                     GIMP_TYPE_OPERATION_SETTINGS),
      NULL);

  gimp_operation_config_sync_node(filter_tool->config, filter_tool->operation);
  gimp_operation_config_connect_node(filter_tool->config,
                                     filter_tool->operation);

  filter_tool->settings = gimp_operation_config_get_container(
      tool->tool_info->gimp, G_TYPE_FROM_INSTANCE(filter_tool->config),
      (GCompareFunc)gimp_settings_compare);
  g_object_ref(filter_tool->settings);

  pspecs = gimp_operation_config_list_properties(
      filter_tool->config, G_TYPE_FROM_INSTANCE(filter_tool->config), 0, NULL);
  ...
  g_object_set(GIMP_FILTER_TOOL_GET_OPTIONS(filter_tool), "preview-split",
               FALSE, NULL);
...
void gimp_filter_tool_edit_as(GimpFilterTool *filter_tool,
                              const gchar *new_tool_id, GimpConfig *config) {
  GimpDisplay *display;
  ...
  tool_info = (GimpToolInfo *)gimp_container_get_child_by_name(
      display->gimp->tool_info_list, new_tool_id);
...
gboolean gimp_filter_tool_on_guide(GimpFilterTool *filter_tool,
  ...
  if (filter_tool->filter && filter_tool->preview_guide &&
...
static void gimp_filter_tool_color_picker_toggled(GtkWidget *widget,
                                                  GimpFilterTool *filter_tool) {
  if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget))) {
    ...
    identifier = g_object_get_data(G_OBJECT(widget), "picker-identifier");
    pick_abyss = GPOINTER_TO_INT(
        g_object_get_data(G_OBJECT(widget), "picker-pick-abyss"));

    gimp_filter_tool_enable_color_picking(filter_tool, identifier, pick_abyss);
...
GtkWidget *gimp_filter_tool_add_color_picker(
    GimpFilterTool *filter_tool, gpointer identifier, const gchar *icon_name,
    const gchar *tooltip, gboolean pick_abyss, GimpPickerCallback callback,
    gpointer callback_data) {
  GtkWidget *button;
  GtkWidget *image;
  ...
  button = g_object_new(GTK_TYPE_TOGGLE_BUTTON, "draw-indicator", FALSE, NULL);

  image = gtk_image_new_from_icon_name(icon_name, GTK_ICON_SIZE_BUTTON);
  g_object_set(image, "margin-start", 2, "margin-end", 2, "margin-top", 2,
  ...
  g_object_set_data(G_OBJECT(button), "picker-pick-abyss",
                    GINT_TO_POINTER(pick_abyss));
  g_object_set_data(G_OBJECT(button), "picker-callback", callback);
  g_object_set_data(G_OBJECT(button), "picker-callback-data", callback_data);
...
GCallback gimp_filter_tool_add_controller(GimpFilterTool *filter_tool,
  ...
  widget = gimp_filter_tool_create_widget(filter_tool, controller_type,
                                          status_title, callback, callback_data,
                                          &set_func, set_func_data);
...
void gimp_filter_tool_set_widget(GimpFilterTool *filter_tool,
  ...
  if (filter_tool->widget) {
    ...
    gimp_tool_widget_set_visible(filter_tool->widget, options->controller);
  ...
  *drawable_offset_y = 0;
  ...
  if (drawable && settings) {
    ...
    case GIMP_FILTER_REGION_SELECTION:
      if (!gimp_item_mask_intersect(GIMP_ITEM(drawable), &drawable_area->x,
                                    &drawable_area->y, &drawable_area->width,
