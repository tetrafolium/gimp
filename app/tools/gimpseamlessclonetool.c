...
static void gimp_seamless_clone_tool_control(GimpTool *tool,
                                             ...
                                             GimpDisplay *display);
static void
gimp_seamless_clone_tool_button_press(GimpTool *tool, const GimpCoords *coords,
                                      guint32 time, GdkModifierType state,
                                      GimpButtonPressType press_type,
                                      GimpDisplay *display);

static void gimp_seamless_clone_tool_button_release(
...
void gimp_seamless_clone_tool_register(GimpToolRegisterCallback callback,
  ...
  if (GIMP_GUI_CONFIG(GIMP(data)->config)->playground_seamless_clone_tool)
    (*callback)(GIMP_TYPE_SEAMLESS_CLONE_TOOL, GIMP_TYPE_SEAMLESS_CLONE_OPTIONS,
                gimp_seamless_clone_options_gui, 0, "gimp-seamless-clone-tool",
                _("Seamless Clone"),
                _("Seamless Clone: Seamlessly paste one image into another"),
                N_("_Seamless Clone"), NULL, NULL,
                GIMP_HELP_TOOL_SEAMLESS_CLONE, GIMP_ICON_TOOL_SEAMLESS_CLONE,
                data);
}
...
static void gimp_seamless_clone_tool_init(GimpSeamlessCloneTool *self) {
...
void gimp_seamless_clone_tool_button_release(GimpTool *tool,
                                             const GimpCoords *coords,
                                             guint32 time,
                                             GdkModifierType state,
...
static void gimp_seamless_clone_tool_draw(GimpDrawTool *draw_tool) {
  ...
  if (sc->tool_state == SC_STATE_RENDER_WAIT ||
      sc->tool_state == SC_STATE_RENDER_MOTION) {
    gimp_draw_tool_add_rectangle(draw_tool, FALSE, sc->xoff, sc->yoff,
                                 sc->width, sc->height);
  }
}
...
gimp_seamless_clone_tool_create_render_node(GimpSeamlessCloneTool *sc) {
  ...
  paste = gegl_node_new_child(node, "operation", "gegl:buffer-source", "buffer",
                              sc->paste, NULL);

  op = gegl_node_new_child(node, "operation", "gegl:seamless-clone",
                           "max-refine-scale", options->max_refine_scale, NULL);

  overlay = gegl_node_new_child(node, "operation", "svg:dst-over", NULL);

  gegl_node_connect_to(input, "output", op, "input");

  gegl_node_connect_to(paste, "output", op, "aux");

  gegl_node_connect_to(op, "output", overlay, "input");

  gegl_node_connect_to(input, "output", overlay, "aux");
...
gimp_seamless_clone_tool_render_node_update(GimpSeamlessCloneTool *sc) {
  ...
  if (rendered__max_refine_scale == options->max_refine_scale &&
      rendered_xoff == sc->xoff && rendered_yoff == sc->yoff)
    return FALSE;

  gimp_item_get_offset(GIMP_ITEM(bg), &off_x, &off_y);

  gegl_node_set(sc->sc_node, "xoff", (gint)sc->xoff - off_x, "yoff",
                (gint)sc->yoff - off_y, "max-refine-scale",
                (gint)options->max_refine_scale, NULL);
...
static void gimp_seamless_clone_tool_create_filter(GimpSeamlessCloneTool *sc,
  ...
  sc->filter =
      gimp_drawable_filter_new(drawable, _("Seamless Clone"), sc->render_node,
  ...
  g_signal_connect(sc->filter, "flush",
                   G_CALLBACK(gimp_seamless_clone_tool_filter_flush), sc);
}
...
static void gimp_seamless_clone_tool_filter_update(GimpSeamlessCloneTool *sc) {
   ...
   * displayed part */
  gimp_display_shell_untransform_viewport(shell, !shell->show_all, &x, &y, &w,
                                          &h);
  ...
  gimp_rectangle_intersect(x, y, w, h, off_x, off_y, gimp_item_get_width(item),
                           gimp_item_get_height(item), &visible.x, &visible.y,
