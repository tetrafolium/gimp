...
static void gimp_color_picker_tool_info_create(GimpColorPickerTool *picker_tool,
                                               GimpDisplay *display);
static void
gimp_color_picker_tool_info_response(GimpToolGui *gui, gint response_id,
                                     GimpColorPickerTool *picker_tool);
static void gimp_color_picker_tool_info_update(
    GimpColorPickerTool *picker_tool, GimpDisplay *display,
    gboolean sample_average, const Babl *sample_format, gpointer pixel,
    const GimpRGB *color, gint x, gint y);

G_DEFINE_TYPE(GimpColorPickerTool, gimp_color_picker_tool, GIMP_TYPE_COLOR_TOOL)
...
void gimp_color_picker_tool_register(GimpToolRegisterCallback callback,
                                     gpointer data) {
  (*callback)(GIMP_TYPE_COLOR_PICKER_TOOL, GIMP_TYPE_COLOR_PICKER_OPTIONS,
              gimp_color_picker_options_gui,
              GIMP_CONTEXT_PROP_MASK_FOREGROUND |
                  GIMP_CONTEXT_PROP_MASK_BACKGROUND,
              "gimp-color-picker-tool", _("Color Picker"),
              _("Color Picker Tool: Set colors from image pixels"),
              N_("C_olor Picker"), "O", NULL, GIMP_HELP_TOOL_COLOR_PICKER,
              GIMP_ICON_TOOL_COLOR_PICKER, data);
}
...
static void gimp_color_picker_tool_init(GimpColorPickerTool *picker_tool) {
...
static void gimp_color_picker_tool_modifier_key(GimpTool *tool,
  ...
  if (key == gimp_get_extend_selection_mask()) {
    g_object_set(options, "use-info-window", !options->use_info_window, NULL);
  } else if (key == gimp_get_toggle_behavior_mask()) {
    switch (options->pick_target) {
    case GIMP_COLOR_PICK_TARGET_FOREGROUND:
      g_object_set(options, "pick-target", GIMP_COLOR_PICK_TARGET_BACKGROUND,
                   NULL);
      break;

    case GIMP_COLOR_PICK_TARGET_BACKGROUND:
      g_object_set(options, "pick-target", GIMP_COLOR_PICK_TARGET_FOREGROUND,
                   NULL);
      break;
...
static void gimp_color_picker_tool_oper_update(GimpTool *tool,
  ...
  if (proximity) {
    ...
    case GIMP_COLOR_PICK_TARGET_NONE:
      status_help =
          gimp_suggest_modifiers(_("Click in any image to view"
                                   " its color"),
    ...
    case GIMP_COLOR_PICK_TARGET_PALETTE:
      status_help =
          gimp_suggest_modifiers(_("Click in any image to add"
...
static void gimp_color_picker_tool_picked(
  ...
  if (picker_tool->gui && (options->use_info_window ||
                           gimp_tool_gui_get_visible(picker_tool->gui))) {
    gimp_color_picker_tool_info_update(
        picker_tool, display, GIMP_COLOR_OPTIONS(options)->sample_average,
        sample_format, pixel, color, (gint)floor(coords->x),
        (gint)floor(coords->y));
  }

  GIMP_COLOR_TOOL_CLASS(parent_class)
      ->picked(color_tool, coords, display, pick_state, sample_format, pixel,
               color);
}
...
static void gimp_color_picker_tool_info_create(GimpColorPickerTool *picker_tool,
  ...
  picker_tool->gui = gimp_tool_gui_new(
      tool->tool_info, NULL, _("Color Picker Information"), NULL, NULL,
  ...
  gtk_box_pack_start(GTK_BOX(gimp_tool_gui_get_vbox(picker_tool->gui)), hbox,
                     FALSE, FALSE, 0);
  ...
  gimp_color_frame_set_has_coords(GIMP_COLOR_FRAME(picker_tool->color_frame1),
                                  TRUE);
  g_object_bind_property(options, "frame1-mode", picker_tool->color_frame1,
                         "mode",
                         G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);
  gtk_box_pack_start(GTK_BOX(hbox), picker_tool->color_frame1, FALSE, FALSE, 0);
  ...
  gimp_color_frame_set_color_config(GIMP_COLOR_FRAME(picker_tool->color_frame2),
                                    context->gimp->config->color_management);
  g_object_bind_property(options, "frame2-mode", picker_tool->color_frame2,
                         "mode",
                         G_BINDING_BIDIRECTIONAL | G_BINDING_SYNC_CREATE);
  gtk_box_pack_start(GTK_BOX(hbox), picker_tool->color_frame2, FALSE, FALSE, 0);
...
static void gimp_color_picker_tool_info_update(
  ...
  gimp_tool_gui_set_shell(picker_tool->gui, gimp_display_get_shell(display));
  gimp_tool_gui_set_viewables(picker_tool->gui, drawables);
  g_list_free(drawables);
  ...
  gimp_color_frame_set_color(GIMP_COLOR_FRAME(picker_tool->color_frame1),
                             sample_average, sample_format, pixel, color, x, y);
