...
static void gimp_rectangle_select_options_class_init(
    GimpRectangleSelectOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_BOOLEAN(object_class, GIMP_RECTANGLE_OPTIONS_PROP_HIGHLIGHT,
                           ...
                           _("Dim everything outside selection"), FALSE,
  ...
  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_ROUND_CORNERS, "round-corners", _("Rounded corners"),
      _("Round corners of selection"), FALSE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_CORNER_RADIUS, "corner-radius",
                          _("Radius"), _("Radius of rounding in pixels"), 0.0,
                          10000.0, 10.0, GIMP_PARAM_STATIC_STRINGS);
...
GtkWidget *gimp_rectangle_select_options_gui(GimpToolOptions *tool_options) {
  ...
  if (tool_options->tool_info->tool_type == GIMP_TYPE_RECTANGLE_SELECT_TOOL) {
    ...
    scale =
        gimp_prop_spin_scale_new(config, "corner-radius", NULL, 1.0, 10.0, 1);
    gimp_spin_scale_set_scale_limits(GIMP_SPIN_SCALE(scale), 0.0, 1000.0);
    gimp_spin_scale_set_gamma(GIMP_SPIN_SCALE(scale), 1.7);

    frame = gimp_prop_expanding_frame_new(config, "round-corners", NULL, scale,
                                          NULL);
    gtk_box_pack_start(GTK_BOX(vbox), frame, FALSE, FALSE, 0);

    toggle = GIMP_SELECTION_OPTIONS(tool_options)->antialias_toggle;

    g_object_bind_property(config, "round-corners", toggle, "sensitive",
