...
static void gimp_eraser_tool_modifier_key(GimpTool *tool, GdkModifierType key,
...
void gimp_eraser_tool_register(GimpToolRegisterCallback callback,
                               gpointer data) {
  (*callback)(
      GIMP_TYPE_ERASER_TOOL, GIMP_TYPE_ERASER_OPTIONS, gimp_eraser_options_gui,
      GIMP_PAINT_OPTIONS_CONTEXT_MASK, "gimp-eraser-tool", _("Eraser"),
      _("Eraser Tool: Erase to background or transparency using a brush"),
      N_("_Eraser"), "<shift>E", NULL, GIMP_HELP_TOOL_ERASER,
      GIMP_ICON_TOOL_ERASER, data);
}
...
static void gimp_eraser_tool_init(GimpEraserTool *eraser) {
  ...
  gimp_tool_control_set_tool_cursor(tool->control, GIMP_TOOL_CURSOR_ERASER);
...
static void gimp_eraser_tool_modifier_key(GimpTool *tool, GdkModifierType key,
  ...
  if (key == GDK_MOD1_MASK) {
    ...
    g_object_set(options, "anti-erase", !options->anti_erase, NULL);
  }
