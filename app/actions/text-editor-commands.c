...
static void text_editor_load_response(GtkWidget *dialog, gint response_id,
...
void text_editor_load_cmd_callback(GimpAction *action, GVariant *value,
  ...
  if (!editor->file_dialog) {
    ...
    dialog = editor->file_dialog = gtk_file_chooser_dialog_new(
        _("Open Text File (UTF-8)"), GTK_WINDOW(editor),
    ...
    g_object_add_weak_pointer(G_OBJECT(dialog), (gpointer)&editor->file_dialog);

    g_signal_connect(dialog, "response", G_CALLBACK(text_editor_load_response),
                     editor);
