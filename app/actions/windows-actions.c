...
static void windows_actions_display_reorder(GimpContainer *container,
                                            GimpDisplay *display, gint position,
                                            GimpActionGroup *group);
...
static const GimpActionEntry windows_actions[] = {
    {"windows-menu", NULL, NC_("windows-action", "_Windows")},
    {"windows-docks-menu", NULL,
     NC_("windows-action", "_Recently Closed Docks")},
    {"windows-dialogs-menu", NULL, NC_("windows-action", "_Dockable Dialogs")},

    {"windows-show-display-next", NULL, NC_("windows-action", "Next Image"),
     "<alt>Tab", NC_("windows-action", "Switch to the next image"),
     windows_show_display_next_cmd_callback, NULL},
    ...
    {"windows-show-display-previous", NULL,
     ...
     windows_show_display_previous_cmd_callback, NULL},

    {"windows-tab-position", NULL, NC_("windows-action", "_Tabs Position")},
};

static const GimpToggleActionEntry windows_toggle_actions[] = {
    {"windows-hide-docks", NULL, NC_("windows-action", "_Hide Docks"), "Tab",
     NC_("windows-action", "When enabled, docks and other dialogs are hidden, "
                           "leaving only image windows."),
     windows_hide_docks_cmd_callback, FALSE, GIMP_HELP_WINDOWS_HIDE_DOCKS},

    {"windows-show-tabs", NULL, NC_("windows-action", "_Show Tabs"), NULL,
     NC_("windows-action", "When enabled, the image tabs bar is shown."),
     windows_show_tabs_cmd_callback, FALSE, GIMP_HELP_WINDOWS_SHOW_TABS},
    ...
    {"windows-use-single-window-mode", NULL,
     ...
     windows_use_single_window_mode_cmd_callback, FALSE,
...
void windows_actions_setup(GimpActionGroup *group) {
  ...
  gimp_action_group_add_actions(group, "windows-action", windows_actions,
  ...
  gimp_action_group_add_radio_actions(
      group, "windows-tabs-position-action", windows_tabs_position_actions,
      G_N_ELEMENTS(windows_tabs_position_actions), NULL, 0,
  ...
  g_signal_connect_object(group->gimp->displays, "add",
                          G_CALLBACK(windows_actions_display_add), group, 0);
  g_signal_connect_object(group->gimp->displays, "remove",
                          G_CALLBACK(windows_actions_display_remove), group, 0);
  g_signal_connect_object(group->gimp->displays, "reorder",
                          G_CALLBACK(windows_actions_display_reorder), group,
                          0);

  for (list = gimp_get_display_iter(group->gimp); list;
  ...
  g_signal_connect_object(
      gimp_dialog_factory_get_singleton(), "dock-window-added",
      G_CALLBACK(windows_actions_dock_window_added), group, 0);
  g_signal_connect_object(
      gimp_dialog_factory_get_singleton(), "dock-window-removed",
      G_CALLBACK(windows_actions_dock_window_removed), group, 0);
  ...
  g_signal_connect_object(global_recent_docks, "add",
                          G_CALLBACK(windows_actions_recent_add), group, 0);
  g_signal_connect_object(global_recent_docks, "remove",
                          G_CALLBACK(windows_actions_recent_remove), group, 0);

  for (list = GIMP_LIST(global_recent_docks)->queue->head; list;
       list = g_list_next(list)) {
...
void windows_actions_update(GimpActionGroup *group, gpointer data) {
...
#define SET_ACTIVE(action, condition)                                          \
  ...
  gimp_action_group_set_action_sensitive(group, "windows-tab-position",
                                         config->single_window_mode);
  gimp_action_group_set_action_sensitive(group, "windows-show-tabs",
                                         config->single_window_mode);
...
static void windows_actions_display_add(GimpContainer *container,
  ...
  g_signal_connect_object(display, "notify::image",
                          G_CALLBACK(windows_actions_image_notify), group, 0);

  g_signal_connect_object(shell, "notify::title",
                          G_CALLBACK(windows_actions_title_notify), group, 0);
...
static void windows_actions_display_remove(GimpContainer *container,
  ...
  if (shell)
    g_signal_handlers_disconnect_by_func(shell, windows_actions_title_notify,
                                         group);
...
static void windows_actions_image_notify(GimpDisplay *display,
  ...
  if (image) {
    ...
    title = g_strdup_printf("%s-%d.%d", escaped, gimp_image_get_id(image),
                            gimp_display_get_instance(display));
    g_free(escaped);

    g_object_set(action, "visible", TRUE, "label", title, "tooltip",
                 gimp_image_get_display_path(image), "viewable", image,
                 "context", gimp_get_user_context(group->gimp), NULL);

    g_free(title);

    windows_actions_update_display_accels(group);
  } else {
    g_object_set(action, "visible", FALSE, "viewable", NULL, NULL);
  }
}
...
static void windows_actions_update_display_accels(GimpActionGroup *group) {
  ...
  for (list = gimp_get_display_iter(group->gimp), i = 0; list && i < 10;
    ...
    if (action) {
      ...
      gtk_accel_map_change_entry(accel_path, accel_key, GDK_MOD1_MASK, TRUE);
    }
  }
}
...
static void windows_actions_dock_window_added(GimpDialogFactory *factory,
  ...
  g_object_set(action, "ellipsize", PANGO_ELLIPSIZE_END, NULL);
  ...
  g_signal_connect_object(dock_window, "notify::title",
                          G_CALLBACK(windows_actions_dock_window_notify), group,
                          0);
...
static void windows_actions_dock_window_notify(GimpDockWindow *dock_window,
                                               const GParamSpec *pspec,
                                               GimpActionGroup *group) {
  GimpAction *action;
  gchar *action_name;
  ...
  if (action)
    g_object_set(action, "label", gtk_window_get_title(GTK_WINDOW(dock_window)),
...
static void windows_actions_recent_add(GimpContainer *container,
  ...
  info_id =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(info), "recent-action-id"));
  ...
  g_object_set(action, "ellipsize", PANGO_ELLIPSIZE_END, "max-width-chars", 30,
               NULL);
...
static void windows_actions_single_window_mode_notify(GimpDisplayConfig *config,
  ...
  gimp_action_group_set_action_active(
      group, "windows-use-single-window-mode",
