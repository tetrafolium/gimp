...
void windows_hide_docks_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  Gimp *gimp;
  gboolean active;
  ...
  if (active != GIMP_GUI_CONFIG(gimp->config)->hide_docks)
    g_object_set(gimp->config, "hide-docks", active, NULL);
}
...
void windows_use_single_window_mode_cmd_callback(GimpAction *action,
  ...
  if (active != GIMP_GUI_CONFIG(gimp->config)->single_window_mode)
    g_object_set(gimp->config, "single-window-mode", active, NULL);
}

void windows_show_tabs_cmd_callback(GimpAction *action, GVariant *value,
                                    gpointer data) {
  Gimp *gimp;
  gboolean active;
  return_if_no_gimp(gimp, data);
  ...
  if (active != GIMP_GUI_CONFIG(gimp->config)->show_tabs)
    g_object_set(gimp->config, "show-tabs", active, NULL);
}

void windows_set_tabs_position_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  Gimp *gimp;
  ...
  if (position != GIMP_GUI_CONFIG(gimp->config)->tabs_position)
    g_object_set(gimp->config, "tabs-position", position, NULL);
}

void windows_show_display_next_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  GimpDisplay *display;
  Gimp *gimp;
  gint index;
  ...
  index = gimp_container_get_child_index(gimp->displays, GIMP_OBJECT(display));
  index++;
  ...
  display =
      GIMP_DISPLAY(gimp_container_get_child_by_index(gimp->displays, index));
...
void windows_show_display_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
...
void windows_show_dock_cmd_callback(GimpAction *action, GVariant *value,
                                    gpointer data) {
...
void windows_open_recent_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  ...
  if (entry && strcmp("gimp-toolbox-window", entry->identifier) == 0 &&
    ...
    gimp_message(gimp, G_OBJECT(action_data_get_widget(data)),
