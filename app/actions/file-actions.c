...
static void file_actions_close_all_update(GimpContainer *images,
                                          GimpObject *unused,
                                          GimpActionGroup *group);
static gchar *file_actions_create_label(const gchar *format, GFile *file);
...
static const GimpActionEntry file_actions[] = {
    ...
    {"file-open", GIMP_ICON_IMAGE_OPEN, NC_("file-action", "_Open..."),
     "<primary>O", NC_("file-action", "Open an image file"),
     file_open_cmd_callback, GIMP_HELP_FILE_OPEN},
    ...
    {"file-open-as-layers", GIMP_ICON_LAYER,
     ...
     file_open_as_layers_cmd_callback, GIMP_HELP_FILE_OPEN_AS_LAYER},
    ...
    {"file-open-location", GIMP_ICON_WEB,
     ...
     file_open_location_cmd_callback, GIMP_HELP_FILE_OPEN_LOCATION},

    {"file-create-template", NULL, NC_("file-action", "Create _Template..."),
     NULL, NC_("file-action", "Create a new template from this image"),
     file_create_template_cmd_callback, GIMP_HELP_FILE_CREATE_TEMPLATE},

    {"file-revert", GIMP_ICON_IMAGE_RELOAD, NC_("file-action", "Re_vert"), NULL,
     NC_("file-action", "Reload the image file from disk"),
     file_revert_cmd_callback, GIMP_HELP_FILE_REVERT},

    {"file-close-all", GIMP_ICON_CLOSE_ALL, NC_("file-action", "C_lose All"),
     "<primary><shift>W", NC_("file-action", "Close all opened images"),
     file_close_all_cmd_callback, GIMP_HELP_FILE_CLOSE_ALL},
    ...
    {"file-copy-location", GIMP_ICON_EDIT_COPY,
     ...
     file_copy_location_cmd_callback, GIMP_HELP_FILE_COPY_LOCATION},
    ...
    {"file-quit", GIMP_ICON_APPLICATION_EXIT, NC_("file-action", "_Quit"),
     "<primary>Q",
     NC_("file-action", "Quit the GNU Image Manipulation Program"),
     file_quit_cmd_callback, GIMP_HELP_FILE_QUIT}};

static const GimpEnumActionEntry file_save_actions[] = {
    {"file-save", GIMP_ICON_DOCUMENT_SAVE, NC_("file-action", "_Save"),
     "<primary>S", NC_("file-action", "Save this image"), GIMP_SAVE_MODE_SAVE,
     FALSE, GIMP_HELP_FILE_SAVE},
    ...
    {"file-save-as", GIMP_ICON_DOCUMENT_SAVE_AS,
     ...
     GIMP_SAVE_MODE_SAVE_AS, FALSE, GIMP_HELP_FILE_SAVE_AS},

    {"file-save-a-copy", NULL, NC_("file-action", "Save a Cop_y..."), NULL,
     NC_("file-action",
         "Save a copy of this image, without affecting the source file "
         "(if any) or the current state of the image"),
     GIMP_SAVE_MODE_SAVE_A_COPY, FALSE, GIMP_HELP_FILE_SAVE_A_COPY},

    {"file-save-and-close", NULL, NC_("file-action", "Save and Close..."), NULL,
     NC_("file-action", "Save this image and close its window"),
     GIMP_SAVE_MODE_SAVE_AND_CLOSE, FALSE, GIMP_HELP_FILE_SAVE},

    {"file-export", NULL, NC_("file-action", "E_xport..."), "<primary>E",
     NC_("file-action", "Export the image"), GIMP_SAVE_MODE_EXPORT, FALSE,
     GIMP_HELP_FILE_EXPORT},

    {"file-overwrite", NULL, NC_("file-action", "Over_write"), "",
     NC_("file-action",
         "Export the image back to the imported file in the import format"),
     GIMP_SAVE_MODE_OVERWRITE, FALSE, GIMP_HELP_FILE_OVERWRITE},

    {"file-export-as", NULL, NC_("file-action", "E_xport As..."),
     "<primary><shift>E",
     NC_("file-action",
         "Export the image to various file formats such as PNG or JPEG"),
     GIMP_SAVE_MODE_EXPORT_AS, FALSE, GIMP_HELP_FILE_EXPORT_AS}};
...
void file_actions_setup(GimpActionGroup *group) {
  ...
  gimp_action_group_add_actions(group, "file-action", file_actions,
                                G_N_ELEMENTS(file_actions));

  gimp_action_group_add_enum_actions(group, "file-action", file_save_actions,
  ...
  g_signal_connect_object(group->gimp->images, "add",
                          G_CALLBACK(file_actions_close_all_update), group, 0);
...
void file_actions_update(GimpActionGroup *group, gpointer data) {
  ...
  show_overwrite =
      (source && gimp_plug_in_manager_file_procedure_find(
                     gimp->plug_in_manager, GIMP_FILE_PROCEDURE_GROUP_EXPORT,
                     source, NULL));
...
#define SET_SENSITIVE(action, condition)                                       \
  ...
  if (file) {
    gimp_action_group_set_action_label(group, "file-save",
  ...
  if (export) {
    ...
    gimp_action_group_set_action_label(group, "file-export",
  ...
  for (i = 0; i < n; i++) {
    ...
    if (i < num_documents) {
      GimpImagefile *imagefile =
          (GimpImagefile *)gimp_container_get_child_by_index(container, i);
      ...
      if (GIMP_ACTION_IMPL(action)->viewable != (GimpViewable *)imagefile) {
        ...
        g_object_set(action, "label", escaped, "tooltip", name, "visible", TRUE,
                     "viewable", imagefile, NULL);

        g_free(escaped);
      }
    } else {
      g_object_set(action, "label", name, "tooltip", NULL, "visible", FALSE,
                   "viewable", NULL, NULL);
    }
