...
void error_console_clear_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
...
void error_console_select_all_cmd_callback(GimpAction *action, GVariant *value,
                                           gpointer data) {
  GimpErrorConsole *console = GIMP_ERROR_CONSOLE(data);
  GtkTextIter start_iter;
  GtkTextIter end_iter;
...
void error_console_save_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpErrorConsole *console = GIMP_ERROR_CONSOLE(data);
  gboolean selection = (gboolean)g_variant_get_int32(value);

  if (selection &&
      !gtk_text_buffer_get_selection_bounds(console->text_buffer, NULL, NULL)) {
    gimp_message_literal(console->gimp, G_OBJECT(console), GIMP_MESSAGE_WARNING,
  ...
  if (!console->file_dialog) {
    ...
    dialog = console->file_dialog = gtk_file_chooser_dialog_new(
        _("Save Error Log to File"), NULL, GTK_FILE_CHOOSER_ACTION_SAVE,
    ...
    g_signal_connect(dialog, "response",
                     G_CALLBACK(error_console_save_response), console);
    g_signal_connect(dialog, "delete-event", G_CALLBACK(gtk_true), NULL);

    gimp_help_connect(dialog, gimp_standard_help_func, GIMP_HELP_ERRORS_DIALOG,
                      NULL, NULL);
  }
...
static void error_console_save_response(GtkWidget *dialog, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    if (!gimp_text_buffer_save(GIMP_TEXT_BUFFER(console->text_buffer), file,
