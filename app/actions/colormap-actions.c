...
static const GimpActionEntry colormap_actions[] = {
    ...
    {"colormap-edit-color", GIMP_ICON_EDIT,
     ...
     colormap_edit_color_cmd_callback, GIMP_HELP_INDEXED_PALETTE_EDIT}};
...
static const GimpEnumActionEntry colormap_add_color_actions[] = {
    {"colormap-add-color-from-fg", GIMP_ICON_LIST_ADD,
     ...
     NC_("colormap-action", "Add current foreground color"), FALSE, FALSE,
     GIMP_HELP_INDEXED_PALETTE_ADD},

    {"colormap-add-color-from-bg", GIMP_ICON_LIST_ADD,
     NC_("colormap-action", "_Add Color from BG"), "",
     NC_("colormap-action", "Add current background color"), TRUE, FALSE,
     GIMP_HELP_INDEXED_PALETTE_ADD}};
...
static const GimpEnumActionEntry colormap_to_selection_actions[] = {
    ...
    {"colormap-selection-add", GIMP_ICON_SELECTION_ADD,
     ...
     NC_("colormap-action",
         "Add all pixels with this color to the current selection"),
     GIMP_CHANNEL_OP_ADD, FALSE, GIMP_HELP_INDEXED_PALETTE_SELECTION_ADD},
...
void colormap_actions_setup(GimpActionGroup *group) {
  gimp_action_group_add_actions(group, "colormap-action", colormap_actions,
  ...
  gimp_action_group_add_enum_actions(
      group, "colormap-action", colormap_to_selection_actions,
...
#define SET_COLOR(action, color)                                               \
  ...
  SET_SENSITIVE("colormap-edit-color", indexed && num_colors > 0);

  SET_SENSITIVE("colormap-add-color-from-fg", indexed && num_colors < 256);
  SET_SENSITIVE("colormap-add-color-from-bg", indexed && num_colors < 256);
  ...
  SET_SENSITIVE("colormap-selection-add", drawable_indexed && num_colors > 0);
