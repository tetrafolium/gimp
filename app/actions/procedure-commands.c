    ...
    if (image) {
      ...
      if (gimp_value_array_length(args) > n_args &&
        ...
        if (item &&
          ...
          g_value_set_object(gimp_value_array_index(args, n_args), item);
          n_args++;
        } else {
  ...
  if (gimp_value_array_length(args) > n_args &&
    ...
    if (image) {
      ...
      if (gimp_value_array_length(args) > n_args &&
        ...
        if (drawable) {
          g_value_set_object(gimp_value_array_index(args, n_args), drawable);
          n_args++;
        } else {
...
gboolean procedure_commands_run_procedure(GimpProcedure *procedure, Gimp *gimp,
  ...
  if (gimp_value_array_length(args) > 0 &&
    ...
    g_value_set_enum(gimp_value_array_index(args, 0), GIMP_RUN_NONINTERACTIVE);

  return_vals = gimp_procedure_execute(
      procedure, gimp, gimp_get_user_context(gimp), progress, args, &error);
