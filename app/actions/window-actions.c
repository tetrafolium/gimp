...
static void window_actions_display_opened(GdkDisplayManager *manager,
  ...
  count = GPOINTER_TO_INT(g_hash_table_lookup(displays, display_name));
  ...
  radio_group =
      g_object_get_data(G_OBJECT(group), "change-to-screen-radio-group");
  radio_group = gimp_action_group_add_radio_actions(
      group, NULL, entries, n_screens, radio_group, 0,
