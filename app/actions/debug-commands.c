...
static void debug_dump_menus_recurse_menu(GtkWidget *menu, gint depth,
                                          gchar *path);

static void debug_print_qdata(GimpObject *object);
static void debug_print_qdata_foreach(GQuark key_id, gpointer data,
                                      gpointer user_data);

static gboolean debug_accel_find_func(GtkAccelKey *key, GClosure *closure,
                                      gpointer data);

/*  public functions  */

void debug_gtk_inspector_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  gtk_window_set_interactive_debugging(TRUE);
}

void debug_mem_profile_cmd_callback(GimpAction *action, GVariant *value,
                                    gpointer data) {
...
void debug_show_image_graph_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
...
void debug_dump_menus_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GList *list;

  for (list = gimp_menu_factory_get_registered_menus(global_menu_factory); list;
    ...
    if (managers) {
      ...
      for (list = manager->registered_uis; list; list = g_list_next(list)) {
        ...
        if (GTK_IS_MENU_SHELL(ui_entry->widget)) {
          ...
          debug_dump_menus_recurse_menu(ui_entry->widget, 1, entry->identifier);
          g_print("\n");
        }
      }
    }
...
void debug_dump_managers_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GList *list;
...
void debug_dump_keyboard_shortcuts_cmd_callback(GimpAction *action,
  ...
  for (group_it = gimp_ui_manager_get_action_groups(manager); group_it;
    ...
    for (action_it = actions; action_it; action_it = g_list_next(action_it)) {
      ...
      if (strstr(name, "-menu") || strstr(name, "-popup") || name[0] == '<')
        continue;

      accel_closure = gimp_action_get_accel_closure(action);

      if (accel_closure) {
        GtkAccelKey *key = gtk_accel_group_find(
            accel_group, debug_accel_find_func, accel_closure);
        if (key && key->accel_key && key->accel_flags & GTK_ACCEL_VISIBLE) {
          ...
          strings = g_list_prepend(
              strings, g_strdup_printf("%-20s %s", key_string, label));
...
void debug_dump_attached_data_cmd_callback(GimpAction *action, GVariant *value,
                                           gpointer data) {
...
static gboolean debug_show_image_graph(GimpImage *source_image) {
  ...
  introspect = gegl_node_new_child(NULL, "operation", "gegl:introspect", "node",
                                   output_node, NULL);
  sink = gegl_node_new_child(NULL, "operation", "gegl:buffer-sink", "buffer",
                             &buffer, NULL);
  ...
  if (buffer) {
    ...
    new_image =
        gimp_create_image_from_buffer(source_image->gimp, buffer, new_name);
...
static void debug_dump_menus_recurse_menu(GtkWidget *menu, gint depth,
  ...
  for (list = children; list; list = g_list_next(list)) {
    ...
    if (GTK_IS_LABEL(child)) {
      ...
      format_str =
          g_strdup_printf("%%%ds%%%ds %%-20s %%s\n", depth * 2, depth * 2 - 40);
      g_print(format_str, "", label, "", help_page ? help_page : "");
      g_free(format_str);
      g_free(help_page);
...
static void debug_print_qdata(GimpObject *object) {
  ...
  g_datalist_foreach(&G_OBJECT(object)->qdata, debug_print_qdata_foreach, NULL);
  g_print("\n");
}
