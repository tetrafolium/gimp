...
static const GimpActionEntry plug_in_actions[] = {
    {"plug-in-reset-all", GIMP_ICON_RESET,
     ...
     plug_in_reset_all_cmd_callback, GIMP_HELP_FILTER_RESET_ALL}};
...
void plug_in_actions_setup(GimpActionGroup *group) {
  ...
  gimp_action_group_add_actions(group, "plug-in-action", plug_in_actions,
                                G_N_ELEMENTS(plug_in_actions));

  for (list = gimp_plug_in_manager_get_menu_branches(manager); list;
       list = g_slist_next(list)) {
    GimpPlugInMenuBranch *branch = list->data;

    plug_in_actions_menu_branch_added(manager, branch->file, branch->menu_path,
                                      branch->menu_label, group);
  }

  g_signal_connect_object(manager, "menu-branch-added",
                          G_CALLBACK(plug_in_actions_menu_branch_added), group,
                          0);
  ...
  for (list = manager->plug_in_procedures; list; list = g_slist_next(list)) {
    ...
    if (plug_in_proc->file)
      plug_in_actions_register_procedure(group->gimp->pdb,
                                         GIMP_PROCEDURE(plug_in_proc), group);
  }

  g_signal_connect_object(group->gimp->pdb, "register-procedure",
                          G_CALLBACK(plug_in_actions_register_procedure), group,
                          0);
...
void plug_in_actions_update(GimpActionGroup *group, gpointer data) {
  ...
  for (list = manager->plug_in_procedures; list; list = g_slist_next(list)) {
    ...
    if (proc->menu_label && !proc->file_proc && proc->image_types_val) {
      GimpProcedure *procedure = GIMP_PROCEDURE(proc);
      gboolean sensitive;
      const gchar *tooltip;

      sensitive = gimp_procedure_get_sensitive(procedure, GIMP_OBJECT(drawable),
                                               &tooltip);

      gimp_action_group_set_action_sensitive(group, gimp_object_get_name(proc),
                                             sensitive);

      if (sensitive || !drawable || !tooltip)
        tooltip = gimp_procedure_get_blurb(procedure);

      gimp_action_group_set_action_tooltip(group, gimp_object_get_name(proc),
                                           tooltip);
    }
  }
}
...
static void plug_in_actions_register_procedure(GimpPDB *pdb,
  ...
  if (GIMP_IS_PLUG_IN_PROCEDURE(procedure)) {
    ...
    g_signal_connect_object(plug_in_proc, "menu-path-added",
                            G_CALLBACK(plug_in_actions_menu_path_added), group,
                            0);
...
static void plug_in_actions_unregister_procedure(GimpPDB *pdb,
  ...
  if (GIMP_IS_PLUG_IN_PROCEDURE(procedure)) {
    ...
    g_signal_handlers_disconnect_by_func(
        plug_in_proc, plug_in_actions_menu_path_added, group);
    ...
    if (plug_in_proc->menu_label && !plug_in_proc->file_proc) {
      ...
      action =
          gimp_action_group_get_action(group, gimp_object_get_name(procedure));
...
static void plug_in_actions_menu_path_added(GimpPlugInProcedure *plug_in_proc,
                                            const gchar *menu_path,
                                            GimpActionGroup *group) {
  const gchar *locale_domain;
  const gchar *path_translated;
...
static gboolean plug_in_actions_check_translation(const gchar *original,
  ...
  if (!p1 || !p2 || (p1 - original) != (p2 - translated) ||
...
static void plug_in_actions_build_path(GimpActionGroup *group,
  ...
  if (!path_table) {
    path_table = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);

    g_object_set_data_full(G_OBJECT(group), "plug-in-path-table", path_table,
