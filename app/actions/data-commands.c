...
void data_open_as_image_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer user_data) {
  ...
  data = (GimpData *)gimp_context_get_by_type(
      context, gimp_data_factory_view_get_children_type(view));
  ...
  if (data && gimp_data_get_file(data)) {
    ...
    if (!image && status != GIMP_PDB_CANCEL) {
      gimp_message(context->gimp, G_OBJECT(view), GIMP_MESSAGE_ERROR,
                   _("Opening '%s' failed:\n\n%s"),
...
void data_new_cmd_callback(GimpAction *action, GVariant *value,
                           gpointer user_data) {
  GimpDataFactoryView *view = GIMP_DATA_FACTORY_VIEW(user_data);
  ...
  if (gimp_data_factory_view_has_data_new_func(view)) {
    ...
    if (data) {
      ...
      gtk_button_clicked(
          GTK_BUTTON(gimp_data_factory_view_get_edit_button(view)));
    }
  }
}
...
void data_duplicate_cmd_callback(GimpAction *action, GVariant *value,
  ...
  if (data && gimp_data_factory_view_have(view, GIMP_OBJECT(data))) {
    ...
    new_data = gimp_data_factory_data_duplicate(
        gimp_data_factory_view_get_data_factory(view), data);
...
void data_copy_location_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer user_data) {
  GimpDataFactoryView *view = GIMP_DATA_FACTORY_VIEW(user_data);
  GimpContext *context;
  GimpData *data;
...
void data_show_in_file_manager_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer user_data) {
  GimpDataFactoryView *view = GIMP_DATA_FACTORY_VIEW(user_data);
  GimpContext *context;
  GimpData *data;
...
void data_delete_cmd_callback(GimpAction *action, GVariant *value,
  ...
  if (data && gimp_data_is_deletable(data) &&
    ...
    dialog = data_delete_dialog_new(factory, data, context, GTK_WIDGET(view));
    gtk_widget_show(dialog);
  }
}

void data_refresh_cmd_callback(GimpAction *action, GVariant *value,
                               gpointer user_data) {
  GimpDataFactoryView *view = GIMP_DATA_FACTORY_VIEW(user_data);
  Gimp *gimp;
...
void data_edit_cmd_callback(GimpAction *action, GVariant *value,
                            gpointer user_data) {
  GimpDataFactoryView *view = GIMP_DATA_FACTORY_VIEW(user_data);
  GimpContext *context;
  GimpData *data;
  ...
  if (data && gimp_data_factory_view_have(view, GIMP_OBJECT(data))) {
    ...
    dockable = gimp_window_strategy_show_dockable_dialog(
        ...
        context->gimp, gimp_dialog_factory_get_singleton(), monitor,
        g_variant_get_string(value, NULL));

    gimp_data_editor_set_data(
        GIMP_DATA_EDITOR(gtk_bin_get_child(GTK_BIN(dockable))), data);
  }
}
