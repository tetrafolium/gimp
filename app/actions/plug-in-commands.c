...
static void plug_in_reset_all_response(GtkWidget *dialog, gint response_id,
                                       Gimp *gimp);
...
void plug_in_run_cmd_callback(GimpAction *action, GVariant *value,
  ...
  case GIMP_PDB_PROC_TYPE_TEMPORARY:
    if (GIMP_IS_DATA_FACTORY_VIEW(data) || GIMP_IS_BUFFER_VIEW(data)) {
  ...
  if (args) {
    if (procedure_commands_run_procedure_async(
            procedure, gimp, GIMP_PROGRESS(display), GIMP_RUN_INTERACTIVE, args,
            display)) {
...
#define RESET_FILTERS_DIALOG_KEY "gimp-reset-all-filters-dialog"
  ...
  if (!dialog) {
    dialog = gimp_message_dialog_new(
        _("Reset all Filters"), GIMP_ICON_DIALOG_QUESTION, NULL, 0,
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(plug_in_reset_all_response),
                     gimp);
