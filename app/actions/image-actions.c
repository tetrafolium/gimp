...
static const GimpActionEntry image_actions[] = {
    {"image-menubar", NULL, NC_("image-action", "Image Menu"), NULL, NULL, NULL,
     GIMP_HELP_IMAGE_WINDOW},

    {"image-popup", NULL, NC_("image-action", "Image Menu"), NULL, NULL, NULL,
     GIMP_HELP_IMAGE_WINDOW},
    ...
    {"image-color-management-menu", NULL,
     NC_("image-action", "Color Ma_nagement")},
    ...
    {"image-new", GIMP_ICON_DOCUMENT_NEW, NC_("image-action", "_New..."),
     "<primary>N", NC_("image-action", "Create a new image"),
     image_new_cmd_callback, GIMP_HELP_FILE_NEW},
    ...
    {"image-duplicate", GIMP_ICON_OBJECT_DUPLICATE,
     ...
     image_duplicate_cmd_callback, GIMP_HELP_IMAGE_DUPLICATE},
    ...
    {"image-color-profile-save", NULL,
     ...
     image_color_profile_save_cmd_callback, GIMP_HELP_IMAGE_COLOR_PROFILE_SAVE},

    {"image-resize", GIMP_ICON_OBJECT_RESIZE,
     NC_("image-action", "Can_vas Size..."), NULL,
     NC_("image-action", "Adjust the image dimensions"),
     image_resize_cmd_callback, GIMP_HELP_IMAGE_RESIZE},
    ...
    {"image-resize-to-layers", NULL,
     ...
     image_resize_to_layers_cmd_callback, GIMP_HELP_IMAGE_RESIZE_TO_LAYERS},
    ...
    {"image-print-size", GIMP_ICON_DOCUMENT_PRINT_RESOLUTION,
     ...
     image_print_size_cmd_callback, GIMP_HELP_IMAGE_PRINT_SIZE},
    ...
    {"image-scale", GIMP_ICON_OBJECT_SCALE,
     ...
     image_scale_cmd_callback, GIMP_HELP_IMAGE_SCALE},
    ...
    {"image-crop-to-content", GIMP_ICON_TOOL_CROP,
     ...
     NC_("image-action", "Crop the image to the extents of its content (remove "
                         "empty borders from the image)"),
     image_crop_to_content_cmd_callback, GIMP_HELP_IMAGE_CROP},
    ...
    {"image-flatten", NULL, NC_("image-action", "_Flatten Image"), NULL,
    ...
    {"image-configure-grid", GIMP_ICON_GRID,
     ...
     image_configure_grid_cmd_callback, GIMP_HELP_IMAGE_GRID},
    ...
    {"image-properties", "dialog-information",
     ...
     image_properties_cmd_callback, GIMP_HELP_IMAGE_PROPERTIES}};
...
static const GimpRadioActionEntry image_convert_base_type_actions[] = {
    ...
    {"image-convert-grayscale", GIMP_ICON_CONVERT_GRAYSCALE,
     ...
     NC_("image-convert-action", "Convert the image to grayscale"), GIMP_GRAY,
...
static const GimpRadioActionEntry image_convert_precision_actions[] = {
    {"image-convert-u8", NULL, NC_("image-convert-action", "8 bit integer"),
     NULL, NC_("image-convert-action", "Convert the image to 8 bit integer"),
     GIMP_COMPONENT_TYPE_U8, GIMP_HELP_IMAGE_CONVERT_U8},

    {"image-convert-u16", NULL, NC_("image-convert-action", "16 bit integer"),
     NULL, NC_("image-convert-action", "Convert the image to 16 bit integer"),
     GIMP_COMPONENT_TYPE_U16, GIMP_HELP_IMAGE_CONVERT_U16},

    {"image-convert-u32", NULL, NC_("image-convert-action", "32 bit integer"),
     NULL, NC_("image-convert-action", "Convert the image to 32 bit integer"),
    ...
    {"image-convert-double", NULL,
     ...
     NC_("image-convert-action", "Convert the image to 64 bit floating point"),
     GIMP_COMPONENT_TYPE_DOUBLE, GIMP_HELP_IMAGE_CONVERT_DOUBLE}};

static const GimpRadioActionEntry image_convert_trc_actions[] = {
    {"image-convert-linear", NULL, NC_("image-convert-action", "Linear light"),
     NULL, NC_("image-convert-action", "Convert the image to linear light"),
...
static const GimpEnumActionEntry image_flip_actions[] = {
    {"image-flip-horizontal", GIMP_ICON_OBJECT_FLIP_HORIZONTAL,
     ...
     GIMP_ORIENTATION_HORIZONTAL, FALSE, GIMP_HELP_IMAGE_FLIP_HORIZONTAL},

    {"image-flip-vertical", GIMP_ICON_OBJECT_FLIP_VERTICAL,
     NC_("image-action", "Flip _Vertically"), NULL,
     NC_("image-action", "Flip image vertically"), GIMP_ORIENTATION_VERTICAL,
     FALSE, GIMP_HELP_IMAGE_FLIP_VERTICAL}};
...
static const GimpEnumActionEntry image_rotate_actions[] = {
    {"image-rotate-90", GIMP_ICON_OBJECT_ROTATE_90,
     ...
     GIMP_ROTATE_90, FALSE, GIMP_HELP_IMAGE_ROTATE_90},

    {"image-rotate-180", GIMP_ICON_OBJECT_ROTATE_180,
     NC_("image-action", "Rotate _180°"), NULL,
     NC_("image-action", "Turn the image upside-down"), GIMP_ROTATE_180, FALSE,
    ...
    {"image-rotate-270", GIMP_ICON_OBJECT_ROTATE_270,
     ...
     GIMP_ROTATE_270, FALSE, GIMP_HELP_IMAGE_ROTATE_270}};

void image_actions_setup(GimpActionGroup *group) {
  gimp_action_group_add_actions(group, "image-action", image_actions,
  ...
  gimp_action_group_add_radio_actions(
      group, "image-convert-action", image_convert_base_type_actions,
      G_N_ELEMENTS(image_convert_base_type_actions), NULL, 0,
      image_convert_base_type_cmd_callback);

  gimp_action_group_add_radio_actions(
      group, "image-convert-action", image_convert_precision_actions,
      G_N_ELEMENTS(image_convert_precision_actions), NULL, 0,
  ...
  gimp_action_group_add_radio_actions(group, "image-convert-action",
                                      ...
                                      NULL, 0, image_convert_trc_cmd_callback);

  gimp_action_group_add_enum_actions(group, "image-action", image_flip_actions,
                                     G_N_ELEMENTS(image_flip_actions),
                                     image_flip_cmd_callback);

  gimp_action_group_add_enum_actions(
      group, "image-action", image_rotate_actions,
      G_N_ELEMENTS(image_rotate_actions), image_rotate_cmd_callback);
...
void image_actions_update(GimpActionGroup *group, gpointer data) {
...
#define SET_VISIBLE(action, condition)                                         \
  ...
  if (image) {
    ...
    trc = gimp_babl_format_get_trc(gimp_image_get_layer_format(image, FALSE));
  ...
  if (profile) {
    SET_LABEL("image-convert-rgb", C_("image-convert-action", "_RGB..."));
  ...
  SET_SENSITIVE("image-color-profile-use-srgb",
                image && (profile || profile_hidden));
