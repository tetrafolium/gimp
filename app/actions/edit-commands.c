...
static gboolean check_drawable_alpha(GimpDrawable *drawable, gpointer data);
static void edit_paste(GimpDisplay *display, GimpPasteType paste_type,
                       gboolean try_svg);
static void cut_named_buffer_callback(GtkWidget *widget, const gchar *name,
                                      gpointer data);
static void copy_named_buffer_callback(GtkWidget *widget, const gchar *name,
                                       gpointer data);
...
void edit_undo_cmd_callback(GimpAction *action, GVariant *value,
                            gpointer data) {
  GimpImage *image;
  GimpDisplay *display;
...
void edit_redo_cmd_callback(GimpAction *action, GVariant *value,
                            gpointer data) {
  GimpImage *image;
  GimpDisplay *display;
  return_if_no_image(image, data);
...
void edit_undo_clear_cmd_callback(GimpAction *action, GVariant *value,
  ...
  dialog = gimp_message_dialog_new(
      _("Clear Undo History"), GIMP_ICON_DIALOG_WARNING, widget,
      GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
      gimp_standard_help_func, GIMP_HELP_EDIT_UNDO_CLEAR,
...
void edit_cut_cmd_callback(GimpAction *action, GVariant *value, gpointer data) {
  ...
  if (cut) {
    ...
    gimp_message_literal(image->gimp, G_OBJECT(action_data_get_display(data)),
...
void edit_copy_cmd_callback(GimpAction *action, GVariant *value,
                            gpointer data) {
  GimpImage *image;
  GList *drawables;
  GimpObject *copy;
  ...
  copy =
      gimp_edit_copy(image, drawables, action_data_get_context(data), &error);
...
void edit_named_cut_cmd_callback(GimpAction *action, GVariant *value,
  ...
  dialog = gimp_query_string_box(
      _("Cut Named"), widget, gimp_standard_help_func, GIMP_HELP_BUFFER_CUT,
      _("Enter a name for this buffer"), NULL, G_OBJECT(image), "disconnect",
      cut_named_buffer_callback, image, NULL);
...
void edit_named_copy_cmd_callback(GimpAction *action, GVariant *value,
  ...
  dialog = gimp_query_string_box(
      _("Copy Named"), widget, gimp_standard_help_func, GIMP_HELP_BUFFER_COPY,
      _("Enter a name for this buffer"), NULL, G_OBJECT(image), "disconnect",
      copy_named_buffer_callback, image, NULL);
  gtk_widget_show(dialog);
}
...
void edit_named_copy_visible_cmd_callback(GimpAction *action, GVariant *value,
  ...
  dialog = gimp_query_string_box(
      _("Copy Visible Named "), widget, gimp_standard_help_func,
      GIMP_HELP_BUFFER_COPY, _("Enter a name for this buffer"), NULL,
      G_OBJECT(image), "disconnect", copy_named_visible_buffer_callback, image,
      NULL);
  gtk_widget_show(dialog);
}
...
void edit_clear_cmd_callback(GimpAction *action, GVariant *value,
  ...
  gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_PAINT, _("Clear"));
...
void edit_fill_cmd_callback(GimpAction *action, GVariant *value,
                            gpointer data) {
  GimpImage *image;
  GList *drawables;
  GList *iter;
  ...
  if (gimp_fill_options_set_by_fill_type(options, action_data_get_context(data),
                                         fill_type, &error)) {
...
static gboolean check_drawable_alpha(GimpDrawable *drawable, gpointer data) {
  if (gimp_drawable_has_alpha(drawable) && GIMP_IS_LAYER(drawable) &&
...
static void edit_paste(GimpDisplay *display, GimpPasteType paste_type,
  ...
  if (paste) {
    ...
    if (drawable && paste_type != GIMP_PASTE_TYPE_NEW_LAYER &&
    ...
    if (gimp_edit_paste(image, drawable, paste, paste_type, x, y, width,
                        height)) {
      gimp_image_flush(image);
    }
...
static void cut_named_buffer_callback(GtkWidget *widget, const gchar *name,
  ...
  if (!drawables) {
    gimp_message_literal(
        image->gimp, NULL, GIMP_MESSAGE_WARNING,
...
static void copy_named_visible_buffer_callback(GtkWidget *widget,
  ...
  if (gimp_edit_named_copy_visible(
          image, name, gimp_get_user_context(image->gimp), &error)) {
    gimp_image_flush(image);
  } else {
    gimp_message_literal(image->gimp, NULL, GIMP_MESSAGE_WARNING,
                         error->message);
