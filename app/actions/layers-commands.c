...
static void layers_new_callback(
    GtkWidget *dialog, GimpImage *image, GimpLayer *layer, GimpContext *context,
    const gchar *layer_name, GimpLayerMode layer_mode,
    GimpLayerColorSpace layer_blend_space,
    GimpLayerColorSpace layer_composite_space,
    GimpLayerCompositeMode layer_composite_mode, gdouble layer_opacity,
    GimpFillType layer_fill_type, gint layer_width, gint layer_height,
    gint layer_offset_x, gint layer_offset_y, gboolean layer_visible,
    gboolean layer_linked, GimpColorTag layer_color_tag,
    gboolean layer_lock_pixels, gboolean layer_lock_position,
    gboolean layer_lock_alpha, gboolean rename_text_layer, gpointer user_data);
static void layers_edit_attributes_callback(
    GtkWidget *dialog, GimpImage *image, GimpLayer *layer, GimpContext *context,
    const gchar *layer_name, GimpLayerMode layer_mode,
    GimpLayerColorSpace layer_blend_space,
    GimpLayerColorSpace layer_composite_space,
    ...
    gboolean layer_lock_pixels, gboolean layer_lock_position,
    gboolean layer_lock_alpha, gboolean rename_text_layer, gpointer user_data);
static void layers_add_mask_callback(GtkWidget *dialog, GList *layers,
                                     GimpAddMaskType add_mask_type,
                                     GimpChannel *channel, gboolean invert,
                                     gpointer user_data);
static void layers_scale_callback(GtkWidget *dialog, GimpViewable *viewable,
                                  gint width, gint height, GimpUnit unit,
                                  GimpInterpolationType interpolation,
                                  gdouble xresolution, gdouble yresolution,
                                  GimpUnit resolution_unit, gpointer user_data);
static void layers_resize_callback(GtkWidget *dialog, GimpViewable *viewable,
                                   GimpContext *context, gint width,
                                   gint height, GimpUnit unit, gint offset_x,
                                   gint offset_y, GimpFillType fill_type,
                                   GimpItemSet unused, gboolean unused2,
                                   gpointer data);

static gint layers_mode_index(GimpLayerMode layer_mode,
                              const GimpLayerMode *modes, gint n_modes);
...
void layers_edit_cmd_callback(GimpAction *action, GVariant *value,
                              gpointer data) {
  GimpImage *image;
  GimpLayer *layer;
  GtkWidget *widget;
...
void layers_edit_text_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpImage *image;
  GimpLayer *layer;
  GtkWidget *widget;
  ...
  if (!GIMP_IS_TEXT_TOOL(active_tool)) {
    GimpToolInfo *tool_info = gimp_get_tool_info(image->gimp, "gimp-text-tool");
...
void layers_edit_attributes_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpImage *image;
  GimpLayer *layer;
  GtkWidget *widget;
...
#define EDIT_DIALOG_KEY "gimp-layer-edit-attributes-dialog"
  ...
  if (!dialog) {
    ...
    dialog = layer_options_dialog_new(
        gimp_item_get_image(GIMP_ITEM(layer)), layer,
        action_data_get_context(data), widget, _("Layer Attributes"),
        "gimp-layer-edit", GIMP_ICON_EDIT, _("Edit Layer Attributes"),
        GIMP_HELP_LAYER_EDIT, gimp_object_get_name(layer),
        gimp_layer_get_mode(layer), gimp_layer_get_blend_space(layer),
        gimp_layer_get_composite_space(layer),
        gimp_layer_get_composite_mode(layer), gimp_layer_get_opacity(layer),
        0 /* unused */, gimp_item_get_visible(item), gimp_item_get_linked(item),
        gimp_item_get_color_tag(item), gimp_item_get_lock_content(item),
        gimp_item_get_lock_position(item), gimp_layer_get_lock_alpha(layer),
        layers_edit_attributes_callback, NULL);
...
void layers_new_cmd_callback(GimpAction *action, GVariant *value,
                             gpointer data) {
  GimpImage *image;
  GtkWidget *widget;
...
#define NEW_DIALOG_KEY "gimp-layer-new-dialog"
  ...
  if (!dialog) {
    ...
    desc = ngettext("Create a New Layer", "Create %d New Layers",
    ...
    dialog = layer_options_dialog_new(
        image, NULL, action_data_get_context(data), widget, title,
        "gimp-layer-new", GIMP_ICON_LAYER, desc, GIMP_HELP_LAYER_NEW,
        config->layer_new_name, layer_mode, config->layer_new_blend_space,
        config->layer_new_composite_space, config->layer_new_composite_mode,
        config->layer_new_opacity, config->layer_new_fill_type, TRUE, FALSE,
        GIMP_COLOR_TAG_NONE, FALSE, FALSE, FALSE, layers_new_callback, NULL);
    g_free(desc);
...
void layers_new_last_vals_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpImage *image;
  GtkWidget *widget;
  GimpLayer *layer;
  ...
  gimp_image_undo_group_start(
      image, GIMP_UNDO_GROUP_LAYER_ADD,
      ngettext("New layer", "New layers", n_layers > 0 ? n_layers : 1));
  for (iter = layers; iter || run_once; iter = iter ? iter->next : NULL) {
    ...
    layer = gimp_layer_new(
        image, gimp_image_get_width(image), gimp_image_get_height(image),
        gimp_image_get_layer_format(image, TRUE), config->layer_new_name,
        config->layer_new_opacity, layer_mode);

    gimp_drawable_fill(GIMP_DRAWABLE(layer), action_data_get_context(data),
                       config->layer_new_fill_type);
    gimp_layer_set_blend_space(layer, config->layer_new_blend_space, FALSE);
    gimp_layer_set_composite_space(layer, config->layer_new_composite_space,
                                   FALSE);
    gimp_layer_set_composite_mode(layer, config->layer_new_composite_mode,
                                  FALSE);
...
void layers_new_from_visible_cmd_callback(GimpAction *action, GVariant *value,
                                          gpointer data) {
  GimpImage *image;
  GimpDisplayShell *shell;
  GimpLayer *layer;
  ...
  layer = gimp_layer_new_from_gegl_buffer(
      gimp_pickable_get_buffer(pickable), image,
      gimp_image_get_layer_format(image, TRUE), _("Visible"),
      GIMP_OPACITY_OPAQUE, gimp_image_get_default_new_layer_mode(image),
      profile);
...
void layers_new_group_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpImage *image;
  GList *new_layers = NULL;
  GList *layers;
  ...
  gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_LAYER_ADD,
                              ngettext("New layer group", "New layer groups",
                                       n_layers > 0 ? n_layers : 1));
...
void layers_select_cmd_callback(GimpAction *action, GVariant *value,
                                gpointer data) {
  GimpImage *image;
  GList *new_layers = NULL;
  GList *layers;
  ...
  for (iter = layers; iter || run_once; iter = iter ? iter->next : NULL) {
    ...
    new_layer = (GimpLayer *)action_select_object(select_type, container,
...
void layers_raise_cmd_callback(GimpAction *action, GVariant *value,
                               gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  gimp_image_undo_group_start(
      ...
      ngettext("Raise Layer", "Raise Layers", g_list_length(raised_layers)));
...
void layers_raise_to_top_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
...
void layers_lower_cmd_callback(GimpAction *action, GVariant *value,
                               gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  gimp_image_undo_group_start(
      image, GIMP_UNDO_GROUP_ITEM_DISPLACE,
      ngettext("Lower Layer", "Lower Layers", g_list_length(lowered_layers)));
...
void layers_lower_to_bottom_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
...
void layers_duplicate_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *new_layers = NULL;
  ...
  for (iter = layers; iter; iter = iter->next) {
    ...
    new_layer = GIMP_LAYER(gimp_item_duplicate(
        GIMP_ITEM(iter->data), G_TYPE_FROM_INSTANCE(iter->data)));

    /*  use the actual parent here, not GIMP_IMAGE_ACTIVE_PARENT because
     *  the latter would add a duplicated group inside itself instead of
     *  above it
     */
    gimp_image_add_layer(image, new_layer, gimp_layer_get_parent(iter->data),
                         gimp_item_get_index(iter->data), TRUE);
    new_layers = g_list_prepend(new_layers, new_layer);
  }
...
void layers_anchor_cmd_callback(GimpAction *action, GVariant *value,
                                gpointer data) {
  GimpImage *image;
  GList *layers;
  return_if_no_layers(image, layers, data);
...
void layers_merge_down_cmd_callback(GimpAction *action, GVariant *value,
                                    gpointer data) {
  GimpImage *image;
  GList *layers;
  GimpDisplay *display;
  ...
  if (error) {
    gimp_message_literal(image->gimp, G_OBJECT(display), GIMP_MESSAGE_WARNING,
                         error->message);
    g_clear_error(&error);
    return;
  }
...
void layers_merge_group_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpImage *image;
  GList *layers;
...
void layers_delete_cmd_callback(GimpAction *action, GVariant *value,
                                gpointer data) {
  GimpImage *image;
  GList *layers;
...
void layers_text_discard_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpImage *image;
  GimpLayer *layer;
  return_if_no_layer(image, layer, data);
...
void layers_text_to_vectors_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpImage *image;
  GimpLayer *layer;
  return_if_no_layer(image, layer, data);
  ...
  if (GIMP_IS_TEXT_LAYER(layer)) {
    ...
    gimp_image_add_vectors(image, vectors, GIMP_IMAGE_ACTIVE_PARENT, -1, TRUE);
    gimp_image_flush(image);
  }
}

void layers_text_along_vectors_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  GimpImage *image;
  GimpLayer *layer;
  GimpVectors *vectors;
  ...
  if (GIMP_IS_TEXT_LAYER(layer)) {
    ...
    gimp_image_add_vectors(image, new_vectors, GIMP_IMAGE_ACTIVE_PARENT, -1,
                           TRUE);
    gimp_image_flush(image);
  }
}

void layers_resize_cmd_callback(GimpAction *action, GVariant *value,
                                gpointer data) {
  GimpImage *image;
  GimpLayer *layer;
  GList *layers;
...
#define RESIZE_DIALOG_KEY "gimp-resize-dialog"
  ...
  if (!dialog) {
    ...
    dialog = resize_dialog_new(
        GIMP_VIEWABLE(layer), action_data_get_context(data),
        _("Set Layer Boundary Size"), "gimp-layer-resize", widget,
        gimp_standard_help_func, GIMP_HELP_LAYER_RESIZE, layer_resize_unit,
        config->layer_resize_fill_type, GIMP_ITEM_SET_NONE, FALSE,
        layers_resize_callback, NULL);
...
void layers_resize_to_image_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  for (iter = layers; iter; iter = iter->next)
    gimp_layer_resize_to_image(iter->data, action_data_get_context(data),
...
void layers_scale_cmd_callback(GimpAction *action, GVariant *value,
                               gpointer data) {
  GimpImage *image;
  GList *layers;
  GimpLayer *layer;
...
#define SCALE_DIALOG_KEY "gimp-scale-dialog"
  ...
  if (!dialog) {
    ...
    dialog = scale_dialog_new(
        GIMP_VIEWABLE(layer), action_data_get_context(data), _("Scale Layer"),
        "gimp-layer-scale", widget, gimp_standard_help_func,
        GIMP_HELP_LAYER_SCALE, layer_scale_unit, layer_scale_interp,
        layers_scale_callback, display);
...
void layers_crop_to_selection_cmd_callback(GimpAction *action, GVariant *value,
                                           gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  if (!gimp_item_bounds(GIMP_ITEM(gimp_image_get_mask(image)), &x, &y, &width,
                        &height)) {
    gimp_message_literal(image->gimp, G_OBJECT(widget), GIMP_MESSAGE_WARNING,
  ...
  for (iter = layers; iter; iter = iter->next) {
    ...
    gimp_item_resize(GIMP_ITEM(iter->data), action_data_get_context(data),
                     GIMP_FILL_TRANSPARENT, width, height, off_x, off_y);
  }
...
void layers_crop_to_content_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  for (iter = layers; iter; iter = iter->next) {
    switch (gimp_pickable_auto_shrink(
        GIMP_PICKABLE(iter->data), 0, 0, gimp_item_get_width(iter->data),
        gimp_item_get_height(iter->data), &x, &y, &width, &height)) {
  ...
  if (n_croppable == 0) {
    gimp_message_literal(image->gimp, G_OBJECT(widget), GIMP_MESSAGE_INFO,
  ...
  desc = g_strdup_printf(ngettext("Crop Layer to Content",
                                  "Crop %d Layers to Content", n_croppable),
                         n_croppable);
  gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_ITEM_RESIZE, desc);
  g_free(desc);
  ...
  for (iter = layers; iter; iter = iter->next) {
    switch (gimp_pickable_auto_shrink(
        ...
        gimp_item_get_height(iter->data), &x, &y, &width, &height)) {
    case GIMP_AUTO_SHRINK_SHRINK:
      gimp_item_resize(iter->data, action_data_get_context(data),
                       GIMP_FILL_TRANSPARENT, width, height, -x, -y);
...
void layers_mask_add_cmd_callback(GimpAction *action, GVariant *value,
                                  gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
...
#define ADD_MASK_DIALOG_KEY "gimp-add-mask-dialog"
  ...
  if (!dialog) {
    ...
    dialog = layer_add_mask_dialog_new(
        update_layers, action_data_get_context(data), widget,
        config->layer_add_mask_type, config->layer_add_mask_invert,
        layers_add_mask_callback, NULL);
...
void layers_mask_add_last_vals_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  for (iter = layers; iter; iter = iter->next) {
    ...
    mask = gimp_layer_create_mask(iter->data, config->layer_add_mask_type,
                                  channel);
...
void layers_mask_apply_cmd_callback(GimpAction *action, GVariant *value,
                                    gpointer data) {
...
void layers_mask_edit_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
...
void layers_mask_show_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
...
void layers_mask_disable_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
...
void layers_mask_to_selection_cmd_callback(GimpAction *action, GVariant *value,
                                           gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  if (masks) {
    ...
    gimp_channel_combine_items(gimp_image_get_mask(image), masks, operation);
    gimp_image_flush(image);
    g_list_free(masks);
  }
}

void layers_alpha_add_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
...
void layers_alpha_remove_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
...
void layers_alpha_to_selection_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  GimpImage *image;
  GimpDisplay *display;
  GList *layers;
  ...
  gimp_channel_combine_items(gimp_image_get_mask(image), layers, operation);
  gimp_image_flush(image);
...
void layers_opacity_cmd_callback(GimpAction *action, GVariant *value,
                                 gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  for (iter = layers; iter; iter = iter->next) {
    opacity =
        action_select_value(select_type, gimp_layer_get_opacity(iter->data),
                            0.0, 1.0, 1.0, 1.0 / 255.0, 0.01, 0.1, 0.0, FALSE);
...
void layers_mode_cmd_callback(GimpAction *action, GVariant *value,
                              gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  for (iter = layers; iter; iter = iter->next) {
    ...
    modes = gimp_layer_mode_get_context_array(
        layer_mode, GIMP_LAYER_MODE_CONTEXT_LAYER, &n_modes);
    index = layers_mode_index(layer_mode, modes, n_modes);
    index = action_select_value(select_type, index, 0, n_modes - 1, 0, 0.0, 1.0,
                                1.0, 0.0, FALSE);
...
void layers_blend_space_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *update_layers = NULL;
...
void layers_composite_space_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *update_layers = NULL;
...
void layers_composite_mode_cmd_callback(GimpAction *action, GVariant *value,
                                        gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *update_layers = NULL;
...
void layers_visible_cmd_callback(GimpAction *action, GVariant *value,
                                 gpointer data) {
  GimpImage *image;
  GimpLayer *layer;
  return_if_no_layer(image, layer, data);
...
void layers_linked_cmd_callback(GimpAction *action, GVariant *value,
                                gpointer data) {
  GimpImage *image;
  GimpLayer *layer;
  return_if_no_layer(image, layer, data);
...
void layers_lock_content_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_ITEM_LOCK_CONTENTS,
                              undo_label);
...
void layers_lock_position_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_ITEM_LOCK_POSITION,
                              undo_label);
...
void layers_lock_alpha_cmd_callback(GimpAction *action, GVariant *value,
                                    gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
  ...
  gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_LAYER_LOCK_ALPHA,
                              lock_alpha ? _("Lock alpha channels")
                                         : _("Unlock alpha channels"));
  for (iter = layers; iter; iter = iter->next) {
    if (gimp_layer_can_lock_alpha(iter->data)) {
      if (lock_alpha != gimp_layer_get_lock_alpha(iter->data))
...
void layers_color_tag_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpImage *image;
  GList *layers;
  GList *iter;
...
static void layers_new_callback(
    ...
    gboolean layer_linked, GimpColorTag layer_color_tag,
    gboolean layer_lock_pixels, gboolean layer_lock_position,
    gboolean layer_lock_alpha, gboolean rename_text_layer, /* unused */
    gpointer user_data) {
  GimpDialogConfig *config = GIMP_DIALOG_CONFIG(image->gimp->config);
  ...
  g_object_set(config, "layer-new-name", layer_name, "layer-new-mode",
               layer_mode, "layer-new-blend-space", layer_blend_space,
               "layer-new-composite-space", layer_composite_space,
               "layer-new-composite-mode", layer_composite_mode,
               "layer-new-opacity", layer_opacity, "layer-new-fill-type",
               layer_fill_type, NULL);
  ...
  for (iter = layers; iter || run_once; iter = iter ? iter->next : NULL) {
    ...
    layer = gimp_layer_new(image, layer_width, layer_height,
                           ...
                           config->layer_new_name, config->layer_new_opacity,
    ...
    if (layer) {
      ...
      gimp_item_set_lock_content(GIMP_ITEM(layer), layer_lock_pixels, FALSE);
      gimp_item_set_lock_position(GIMP_ITEM(layer), layer_lock_position, FALSE);
...
static void layers_edit_attributes_callback(
    ...
    GimpLayerColorSpace layer_composite_space,
    GimpLayerCompositeMode layer_composite_mode, gdouble layer_opacity,
    GimpFillType unused1, gint unused2, gint unused3, gint layer_offset_x,
    gint layer_offset_y, gboolean layer_visible, gboolean layer_linked,
    GimpColorTag layer_color_tag, gboolean layer_lock_pixels,
    gboolean layer_lock_position, gboolean layer_lock_alpha,
    gboolean rename_text_layer, gpointer user_data) {
  GimpItem *item = GIMP_ITEM(layer);
  ...
  if (strcmp(layer_name, gimp_object_get_name(layer)) ||
    ...
    if (strcmp(layer_name, gimp_object_get_name(layer))) {
      ...
      if (!gimp_item_rename(GIMP_ITEM(layer), layer_name, &error)) {
        gimp_message_literal(image->gimp, G_OBJECT(dialog),
    ...
    if (layer_offset_x != gimp_item_get_offset_x(item) ||
        layer_offset_y != gimp_item_get_offset_y(item)) {
      gimp_item_translate(item, layer_offset_x - gimp_item_get_offset_x(item),
                          layer_offset_y - gimp_item_get_offset_y(item), TRUE);
    }
  ...
  if (gimp_item_is_text_layer(GIMP_ITEM(layer))) {
    g_object_set(layer, "auto-rename", rename_text_layer, NULL);
  }
...
static void layers_add_mask_callback(GtkWidget *dialog, GList *layers,
  ...
  g_object_set(config, "layer-add-mask-type", add_mask_type,
               "layer-add-mask-invert", invert, NULL);
  ...
  for (iter = layers; iter; iter = iter->next) {
    ...
    if (!gimp_layer_add_mask(iter->data, mask, TRUE, &error)) {
      gimp_message_literal(image->gimp, G_OBJECT(dialog), GIMP_MESSAGE_WARNING,
                           error->message);
      g_object_unref(mask);
      g_clear_error(&error);
      return;
...
static void layers_scale_callback(GtkWidget *dialog, GimpViewable *viewable,
  ...
  if (width > 0 && height > 0) {
    ...
    gimp_item_scale_by_origin(item, width, height, interpolation, progress,
                              TRUE);
...
static void layers_resize_callback(GtkWidget *dialog, GimpViewable *viewable,
  ...
  if (width > 0 && height > 0) {
    ...
    g_object_set(config, "layer-resize-fill-type", fill_type, NULL);
    ...
    gimp_item_resize(item, context, fill_type, width, height, offset_x,
                     offset_y);
    gimp_image_flush(gimp_item_get_image(item));
  } else {
