...
void gradient_editor_left_color_type_cmd_callback(GimpAction *action,
  ...
  if (gradient && color_type >= 0 &&
      color_type != gimp_gradient_segment_get_left_color_type(gradient, left)) {
    GimpRGB color;

    gimp_gradient_segment_get_left_flat_color(
        gradient, GIMP_DATA_EDITOR(editor)->context, left, &color);
...
void gradient_editor_load_left_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  GimpGradientEditor *editor = GIMP_GRADIENT_EDITOR(data);
...
void gradient_editor_save_left_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  GimpGradientEditor *editor = GIMP_GRADIENT_EDITOR(data);
  GimpGradient *gradient;
  GimpGradientSegment *left;
...
void gradient_editor_right_color_type_cmd_callback(GimpAction *action,
  ...
  if (gradient && color_type >= 0 &&
    ...
    gimp_gradient_segment_get_right_flat_color(
        gradient, GIMP_DATA_EDITOR(editor)->context, right, &color);
...
void gradient_editor_blending_func_cmd_callback(GimpAction *action,
  ...
  if (gradient && g_enum_get_value(enum_class, type)) {
    gimp_gradient_segment_range_set_blending_function(gradient, left, right,
                                                      type);
  }
...
void gradient_editor_coloring_type_cmd_callback(GimpAction *action,
  ...
  if (gradient && g_enum_get_value(enum_class, color)) {
    gimp_gradient_segment_range_set_coloring_type(gradient, left, right, color);
  }

  g_type_class_unref(enum_class);
}

void gradient_editor_flip_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpGradientEditor *editor = GIMP_GRADIENT_EDITOR(data);
  GimpGradient *gradient;
  GimpGradientSegment *left;
  ...
  gimp_gradient_segment_range_flip(gradient, left, right, &left, &right);

  gimp_gradient_editor_set_selection(editor, left, right);
}

void gradient_editor_replicate_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  GimpGradientEditor *editor = GIMP_GRADIENT_EDITOR(data);
  GimpDataEditor *data_editor = GIMP_DATA_EDITOR(data);
  GimpGradient *gradient;
  ...
  dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, gradient), data_editor->context, title,
      "gimp-gradient-segment-replicate", GIMP_ICON_GRADIENT, desc,
      GTK_WIDGET(editor), gimp_standard_help_func,
      GIMP_HELP_GRADIENT_EDITOR_REPLICATE,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Replicate"), GTK_RESPONSE_OK,
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(gradient_editor_replicate_response), editor);
...
void gradient_editor_split_midpoint_cmd_callback(GimpAction *action,
  ...
  gimp_gradient_segment_range_split_midpoint(
      gradient, data_editor->context, left, right, editor->blend_color_space,
      &left, &right);
...
void gradient_editor_split_uniformly_cmd_callback(GimpAction *action,
  ...
  dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, gradient), data_editor->context, title,
      "gimp-gradient-segment-split-uniformly", GIMP_ICON_GRADIENT, desc,
      GTK_WIDGET(editor), gimp_standard_help_func,
      GIMP_HELP_GRADIENT_EDITOR_SPLIT_UNIFORM,

      _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Split"), GTK_RESPONSE_OK,
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(gradient_editor_split_uniform_response), editor);
  ...
  if (left == right)
    ...
    label =
        gtk_label_new(_("Select the number of uniform parts\n"
...
void gradient_editor_delete_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpGradientEditor *editor = GIMP_GRADIENT_EDITOR(data);
  GimpGradient *gradient;
  GimpGradientSegment *left;
  ...
  gimp_gradient_segment_range_delete(gradient, left, right, &left, &right);

  gimp_gradient_editor_set_selection(editor, left, right);
}

void gradient_editor_recenter_cmd_callback(GimpAction *action, GVariant *value,
                                           gpointer data) {
  GimpGradientEditor *editor = GIMP_GRADIENT_EDITOR(data);
  GimpGradient *gradient;
  GimpGradientSegment *left;
...
void gradient_editor_blend_opacity_cmd_callback(GimpAction *action,
  ...
  gimp_gradient_segment_range_blend(gradient, left, right, &left->left_color,
                                    &right->right_color, FALSE, TRUE);
}

void gradient_editor_zoom_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpGradientEditor *editor = GIMP_GRADIENT_EDITOR(data);
...
static void gradient_editor_split_uniform_response(GtkWidget *widget,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    gimp_gradient_segment_range_split_uniform(
        gradient, data_editor->context, left, right, split_parts,
        editor->blend_color_space, &left, &right);
...
static void gradient_editor_replicate_response(GtkWidget *widget,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    gimp_gradient_segment_range_replicate(gradient, left, right,
                                          replicate_times, &left, &right);
