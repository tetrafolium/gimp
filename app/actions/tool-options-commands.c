...
static void tool_options_show_preset_editor(Gimp *gimp, GimpEditor *editor,
...
void tool_options_save_new_preset_cmd_callback(GimpAction *action,
  ...
  data = gimp_data_factory_data_new(context->gimp->tool_preset_factory, context,
                                    _("Untitled"));

  tool_options_show_preset_editor(gimp, editor, GIMP_TOOL_PRESET(data));
}

void tool_options_save_preset_cmd_callback(GimpAction *action, GVariant *value,
                                           gpointer data) {
  ...
  preset = (GimpToolPreset *)gimp_container_get_child_by_index(
      tool_info->presets, index);
...
void tool_options_edit_preset_cmd_callback(GimpAction *action, GVariant *value,
                                           gpointer data) {
  GimpEditor *editor = GIMP_EDITOR(data);
  Gimp *gimp = gimp_editor_get_ui_manager(editor)->gimp;
  GimpContext *context = gimp_get_user_context(gimp);
...
void tool_options_delete_preset_cmd_callback(GimpAction *action,
  ...
  GimpEditor *editor = GIMP_EDITOR(data);
  GimpContext *context =
      gimp_get_user_context(gimp_editor_get_ui_manager(editor)->gimp);
  GimpToolInfo *tool_info = gimp_context_get_tool(context);
  GimpToolPreset *preset;
  gint index;
  ...
  if (preset && gimp_data_is_deletable(GIMP_DATA(preset))) {
...
void tool_options_reset_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpEditor *editor = GIMP_EDITOR(data);
  GimpContext *context =
      gimp_get_user_context(gimp_editor_get_ui_manager(editor)->gimp);
...
void tool_options_reset_all_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpEditor *editor = GIMP_EDITOR(data);
  GtkWidget *dialog;

  dialog = gimp_message_dialog_new(
      _("Reset All Tool Options"), GIMP_ICON_DIALOG_QUESTION,
      GTK_WIDGET(editor), GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT,
...
static void tool_options_show_preset_editor(Gimp *gimp, GimpEditor *editor,
  ...
  dockable = gimp_window_strategy_show_dockable_dialog(
      ...
      gimp_widget_get_monitor(GTK_WIDGET(editor)), "gimp-tool-preset-editor");

  gimp_data_editor_set_data(
      GIMP_DATA_EDITOR(gtk_bin_get_child(GTK_BIN(dockable))),
      GIMP_DATA(preset));
}
