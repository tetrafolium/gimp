...
void palettes_import_cmd_callback(GimpAction *action, GVariant *value,
  ...
  gimp_dialog_factory_dialog_new(
      ...
      NULL /*ui_manager*/, widget, "gimp-palette-import-dialog", -1, TRUE);
}
...
#define MERGE_DIALOG_KEY "gimp-palettes-merge-dialog"
  ...
  if (!dialog) {
    dialog = gimp_query_string_box(
        _("Merge Palettes"), GTK_WIDGET(editor), gimp_standard_help_func,
        GIMP_HELP_PALETTE_MERGE, _("Enter a name for the merged palette"), NULL,
        G_OBJECT(editor), "destroy", palettes_merge_callback, editor, NULL);
...
static void palettes_merge_callback(GtkWidget *widget,
  ...
  if (g_list_length(selected) < 2) {
    gimp_message_literal(context->gimp, G_OBJECT(editor), GIMP_MESSAGE_WARNING,
  ...
  new_palette =
      GIMP_PALETTE(gimp_data_factory_data_new(factory, context, palette_name));

  for (list = selected; list; list = g_list_next(list)) {
    GimpPalette *palette = list->data;
    GList *cols;

    for (cols = gimp_palette_get_colors(palette); cols;
         cols = g_list_next(cols)) {
      GimpPaletteEntry *entry = cols->data;

      gimp_palette_add_entry(new_palette, -1, entry->name, &entry->color);
    }
  }
