...
static void documents_open_image(GtkWidget *editor, GimpContext *context,
...
void documents_open_cmd_callback(GimpAction *action, GVariant *value,
                                 gpointer data) {
...
void documents_raise_or_open_cmd_callback(GimpAction *action, GVariant *value,
                                          gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
  GimpContext *context;
  GimpContainer *container;
  ...
  if (imagefile && gimp_container_have(container, GIMP_OBJECT(imagefile))) {
    ...
    gimp_container_foreach(context->gimp->displays,
                           (GFunc)documents_raise_display, &closure);
...
void documents_file_open_dialog_cmd_callback(GimpAction *action,
  ...
  if (imagefile && gimp_container_have(container, GIMP_OBJECT(imagefile))) {
    file_file_open_dialog(context->gimp, gimp_imagefile_get_file(imagefile),
                          GTK_WIDGET(editor));
  }
}

void documents_copy_location_cmd_callback(GimpAction *action, GVariant *value,
                                          gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
  GimpContext *context;
  GimpImagefile *imagefile;
  ...
  if (imagefile)
    gimp_clipboard_set_text(context->gimp, gimp_object_get_name(imagefile));
}
...
void documents_remove_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
...
void documents_clear_cmd_callback(GimpAction *action, GVariant *value,
                                  gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
  GimpContext *context = gimp_container_view_get_context(editor->view);
  ...
  dialog = gimp_message_dialog_new(
      _("Clear Document History"), GIMP_ICON_SHRED, GTK_WIDGET(editor),
  ...
  g_signal_connect_object(gtk_widget_get_toplevel(GTK_WIDGET(editor)), "unmap",
  ...
  if (gimp_dialog_run(GIMP_DIALOG(dialog)) == GTK_RESPONSE_OK) {
    ...
    for (list = items; list; list = list->next) {
      ...
      if (gtk_recent_info_has_application(info,
        ...
        gtk_recent_manager_remove_item(manager, gtk_recent_info_get_uri(info),
                                       NULL);
      }
...
void documents_recreate_preview_cmd_callback(GimpAction *action,
  ...
  if (imagefile && gimp_container_have(container, GIMP_OBJECT(imagefile))) {
    ...
    if (!gimp_imagefile_create_thumbnail(imagefile, context, NULL,
                                         context->gimp->config->thumbnail_size,
                                         FALSE, &error)) {
      gimp_message_literal(context->gimp, NULL, GIMP_MESSAGE_ERROR,
                           error->message);
      g_clear_error(&error);
    }
  }
...
void documents_reload_previews_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
  GimpContainer *container;
  ...
  gimp_container_foreach(container, (GFunc)gimp_imagefile_update, editor->view);
}

static void documents_remove_dangling_foreach(GimpImagefile *imagefile,
                                              GimpContainer *container) {
...
void documents_remove_dangling_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
  GimpContainer *container;
  ...
  gimp_container_foreach(container, (GFunc)documents_remove_dangling_foreach,
                         container);
}
