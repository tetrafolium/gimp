...
#define SAVE_AS_POV_DIALOG_KEY "gimp-save-as-pov-ray-dialog"
  ...
  if (!dialog) {
    ...
    dialog = gtk_file_chooser_dialog_new(
        title, NULL, GTK_FILE_CHOOSER_ACTION_SAVE,
    ...
    g_signal_connect(dialog, "response",
                     G_CALLBACK(gradients_save_as_pov_ray_response), gradient);
    g_signal_connect(dialog, "delete-event", G_CALLBACK(gtk_true), NULL);

    g_signal_connect_object(dialog, "destroy", G_CALLBACK(g_object_unref),
                            g_object_ref(gradient), G_CONNECT_SWAPPED);

    gimp_help_connect(dialog, gimp_standard_help_func,
                      GIMP_HELP_GRADIENT_SAVE_AS_POV, NULL, NULL);

    dialogs_attach_dialog(G_OBJECT(gradient), SAVE_AS_POV_DIALOG_KEY, dialog);
  }
