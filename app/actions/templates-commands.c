...
static void templates_new_callback(GtkWidget *dialog, GimpTemplate *template,
                                   GimpTemplate *edit_template,
                                   GimpContext *context, gpointer user_data);
static void templates_edit_callback(GtkWidget *dialog, GimpTemplate *template,
                                    GimpTemplate *edit_template,
                                    GimpContext *context, gpointer user_data);
static void templates_delete_response(GtkWidget *dialog, gint response_id,
                                      TemplateDeleteData *delete_data);
static void templates_delete_data_free(TemplateDeleteData *delete_data);

/*  public functions */

void templates_create_image_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  Gimp *gimp;
...
void templates_new_cmd_callback(GimpAction *action, GVariant *value,
                                gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
  GimpContext *context;
  GtkWidget *dialog;
...
#define NEW_DIALOG_KEY "gimp-template-new-dialog"
  ...
  if (!dialog) {
    dialog = template_options_dialog_new(
        NULL, context, GTK_WIDGET(editor), _("New Template"),
        "gimp-template-new", GIMP_ICON_TEMPLATE, _("Create a New Template"),
        GIMP_HELP_TEMPLATE_NEW, templates_new_callback, NULL);
...
void templates_duplicate_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
  GimpContainer *container;
  GimpContext *context;
...
void templates_edit_cmd_callback(GimpAction *action, GVariant *value,
                                 gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
  GimpContainer *container;
  GimpContext *context;
...
#define EDIT_DIALOG_KEY "gimp-template-edit-dialog"
    ...
    if (!dialog) {
      dialog = template_options_dialog_new(
          template, context, GTK_WIDGET(editor), _("Edit Template"),
          "gimp-template-edit", GIMP_ICON_EDIT, _("Edit Template"),
          GIMP_HELP_TEMPLATE_EDIT, templates_edit_callback, NULL);
...
void templates_delete_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
  GimpContainer *container;
  GimpContext *context;
  ...
  if (template && gimp_container_have(container, GIMP_OBJECT(template))) {
    ...
    dialog = gimp_message_dialog_new(
        _("Delete Template"), "edit-delete", GTK_WIDGET(editor), 0,
    ...
    g_object_weak_ref(G_OBJECT(dialog), (GWeakNotify)templates_delete_data_free,
                      delete_data);
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(templates_delete_response),
                     delete_data);
...
static void templates_edit_callback(GtkWidget *dialog, GimpTemplate *template,
                                    GimpTemplate *edit_template,
                                    GimpContext *context, gpointer user_data) {
  gimp_config_sync(G_OBJECT(edit_template), G_OBJECT(template), 0);
...
static void templates_delete_response(GtkWidget *dialog, gint response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    if (delete_data->template ==
      ...
      new_active = gimp_container_get_neighbor_of(
          delete_data->container, GIMP_OBJECT(delete_data->template));
    }
