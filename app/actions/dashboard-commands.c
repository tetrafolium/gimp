...
static void dashboard_log_record_response(GtkWidget *dialog, int response_id,
...
void dashboard_update_interval_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
...
void dashboard_log_record_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpDashboard *dashboard = GIMP_DASHBOARD(data);
...
#define LOG_RECORD_KEY "gimp-dashboard-log-record-dialog"
    ...
    if (!dialog) {
      ...
      dialog = gtk_file_chooser_dialog_new(
          ...
          _("_Cancel"), GTK_RESPONSE_CANCEL, _("_Record"), GTK_RESPONSE_OK,
      ...
      gtk_window_set_role(GTK_WINDOW(dialog), "gimp-dashboard-log-record");
      ...
      gimp_help_set_help_data(toggle, _("Include backtraces in log"), NULL);
      ...
      gimp_help_set_help_data(toggle, _("Include diagnostic messages in log"),
                              NULL);
      gtk_box_pack_start(GTK_BOX(hbox), toggle, FALSE, FALSE, 0);
      gtk_widget_show(toggle);
      ...
      g_signal_connect(dialog, "response",
                       G_CALLBACK(dashboard_log_record_response), dashboard);
...
#undef LOG_RECORD_KEY
    ...
    if (!gimp_dashboard_log_stop_recording(dashboard, &error)) {
      gimp_message_literal(
          gimp_editor_get_ui_manager(GIMP_EDITOR(dashboard))->gimp, NULL,
          GIMP_MESSAGE_ERROR, error->message);
    }
  }
}

void dashboard_log_add_marker_cmd_callback(GimpAction *action, GVariant *value,
                                           gpointer data) {
  GimpDashboard *dashboard = GIMP_DASHBOARD(data);
  GtkWidget *dialog;
...
#define LOG_ADD_MARKER_KEY "gimp-dashboard-log-add-marker-dialog"
  ...
  if (!dialog) {
    dialog = gimp_query_string_box(
        _("Add Marker"), GTK_WIDGET(dashboard), gimp_standard_help_func,
        GIMP_HELP_DASHBOARD_LOG_ADD_MARKER,
        _("Enter a description for the marker"), NULL, G_OBJECT(dashboard),
        "destroy", (GimpQueryStringCallback)dashboard_log_add_marker_response,
        dashboard, NULL);
...
void dashboard_reset_cmd_callback(GimpAction *action, GVariant *value,
                                  gpointer data) {
  GimpDashboard *dashboard = GIMP_DASHBOARD(data);
...
static void dashboard_log_record_response(GtkWidget *dialog, int response_id,
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    if (!gimp_dashboard_log_start_recording(dashboard, file, &info->params,
                                            &error)) {
      gimp_message_literal(
          gimp_editor_get_ui_manager(GIMP_EDITOR(dashboard))->gimp, NULL,
          GIMP_MESSAGE_ERROR, error->message);
