...
static void view_softproof_profile_callback(GtkWidget *dialog, GimpImage *image,
...
void view_new_cmd_callback(GimpAction *action, GVariant *value, gpointer data) {
  GimpDisplay *display;
  GimpDisplayShell *shell;
  ...
  gimp_create_display(display->gimp, gimp_display_get_image(display),
                      shell->unit, gimp_zoom_model_get_factor(shell->zoom),
                      G_OBJECT(gimp_widget_get_monitor(GTK_WIDGET(shell))));
}

void view_close_cmd_callback(GimpAction *action, GVariant *value,
                             gpointer data) {
  GimpDisplay *display;
  GimpDisplayShell *shell;
  GimpImage *image;
...
void view_scroll_center_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpDisplay *display;
  return_if_no_display(display, data);

  gimp_display_shell_scroll_center_image(gimp_display_get_shell(display), TRUE,
                                         TRUE);
}

void view_zoom_fit_in_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpDisplay *display;
  return_if_no_display(display, data);
...
void view_zoom_fill_cmd_callback(GimpAction *action, GVariant *value,
                                 gpointer data) {
  GimpDisplay *display;
  return_if_no_display(display, data);
...
void view_zoom_selection_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpDisplay *display;
  GimpImage *image;
  ...
  gimp_item_bounds(GIMP_ITEM(gimp_image_get_mask(image)), &x, &y, &width,
                   &height);

  gimp_display_shell_scale_to_rectangle(gimp_display_get_shell(display),
                                        GIMP_ZOOM_IN, x, y, width, height,
                                        FALSE);
}

void view_zoom_revert_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpDisplay *display;
  return_if_no_display(display, data);
...
void view_zoom_cmd_callback(GimpAction *action, GVariant *value,
                            gpointer data) {
  GimpDisplayShell *shell;
  ...
  default: {
    ...
    scale = action_select_value(select_type, scale, 0.0, 512.0, 1.0, 1.0 / 8.0,
                                1.0, 16.0, 0.0, FALSE);
...
void view_zoom_explicit_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpDisplayShell *shell;
  gint factor;
  return_if_no_shell(shell, data);
  ...
  if (factor != 0 /* not Other... */) {
    if (fabs(factor - gimp_zoom_model_get_factor(shell->zoom)) > 0.0001)
      gimp_display_shell_scale(
          shell, GIMP_ZOOM_TO, (gdouble)factor / 10000,
          GIMP_ZOOM_FOCUS_RETAIN_CENTERING_ELSE_BEST_GUESS);
  }
}

/* not a GimpActionCallback */
void view_zoom_other_cmd_callback(GimpAction *action, gpointer data) {
  GimpDisplayShell *shell;
  return_if_no_shell(shell, data);
...
void view_show_all_cmd_callback(GimpAction *action, GVariant *value,
                                gpointer data) {
  GimpDisplay *display;
  GimpDisplayShell *shell;
  gboolean active;
  ...
  if (active != shell->show_all) {
    ...
    if (window)
      SET_ACTIVE(gimp_image_window_get_ui_manager(window), "view-show-all",
                 shell->show_all);

    if (IS_ACTIVE_DISPLAY(display))
      SET_ACTIVE(shell->popup_manager, "view-show-all", shell->show_all);
  }
}

void view_dot_for_dot_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpDisplay *display;
  GimpDisplayShell *shell;
  gboolean active;
  ...
  if (active != shell->dot_for_dot) {
    ...
    if (window)
      SET_ACTIVE(gimp_image_window_get_ui_manager(window), "view-dot-for-dot",
                 shell->dot_for_dot);

    if (IS_ACTIVE_DISPLAY(display))
      SET_ACTIVE(shell->popup_manager, "view-dot-for-dot", shell->dot_for_dot);
  }
}

void view_flip_horizontally_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpDisplay *display;
  GimpDisplayShell *shell;
  gboolean active;
...
void view_flip_vertically_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpDisplay *display;
  GimpDisplayShell *shell;
  gboolean active;
...
void view_rotate_absolute_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpDisplay *display;
  GimpDisplayShell *shell;
  GimpActionSelectType select_type;
  ...
  angle = action_select_value(select_type, 0.0, -180.0, 180.0, 0.0, 1.0, 15.0,
                              90.0, 0.0, TRUE);
...
void view_rotate_relative_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpDisplay *display;
  GimpDisplayShell *shell;
  GimpActionSelectType select_type;
  ...
  delta = action_select_value(select_type, 0.0, -180.0, 180.0, 0.0, 1.0, 15.0,
                              90.0, 0.0, TRUE);

  gimp_display_shell_rotate(shell, delta);
}

void view_rotate_other_cmd_callback(GimpAction *action, GVariant *value,
                                    gpointer data) {
  GimpDisplay *display;
  GimpDisplayShell *shell;
  return_if_no_display(display, data);
...
void view_scroll_horizontal_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpDisplayShell *shell;
  GtkAdjustment *adj;
  GimpActionSelectType select_type;
  ...
  offset = action_select_value(
      select_type, gtk_adjustment_get_value(adj), gtk_adjustment_get_lower(adj),
      gtk_adjustment_get_upper(adj) - gtk_adjustment_get_page_size(adj),
      gtk_adjustment_get_lower(adj), 1, gtk_adjustment_get_step_increment(adj),
      gtk_adjustment_get_page_increment(adj), 0, FALSE);

  gtk_adjustment_set_value(shell->hsbdata, offset);
}

void view_scroll_vertical_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpDisplayShell *shell;
  GtkAdjustment *adj;
  GimpActionSelectType select_type;
...
void view_navigation_window_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  Gimp *gimp;
  GimpDisplayShell *shell;
  ...
  gimp_window_strategy_show_dockable_dialog(
      ...
      gimp_widget_get_monitor(GTK_WIDGET(shell)), "gimp-navigation-view");
}

void view_display_filters_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpDisplayShell *shell;
  GtkWidget *dialog;
  return_if_no_shell(shell, data);
...
void view_color_management_reset_cmd_callback(GimpAction *action,
  ...
  gimp_config_copy(GIMP_CONFIG(global_config), GIMP_CONFIG(shell_config), 0);
...
void view_color_management_enable_cmd_callback(GimpAction *action,
  ...
  if (mode != gimp_color_config_get_mode(color_config)) {
    g_object_set(color_config, "mode", mode, NULL);
...
void view_display_intent_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpDisplayShell *shell;
  GimpColorConfig *color_config;
  GimpColorRenderingIntent intent;
  ...
  if (intent != gimp_color_config_get_display_intent(color_config)) {
    g_object_set(color_config, "display-rendering-intent", intent, NULL);
    shell->color_config_set = TRUE;
  }
}

void view_display_bpc_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpDisplayShell *shell;
  GimpColorConfig *color_config;
  gboolean active;
  ...
  if (active != gimp_color_config_get_display_bpc(color_config)) {
    g_object_set(color_config, "display-use-black-point-compensation", active,
                 NULL);
    shell->color_config_set = TRUE;
  }
}

void view_softproof_profile_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpImage *image;
  GimpDisplayShell *shell;
  GimpColorConfig *color_config;
...
#define SOFTPROOF_PROFILE_DIALOG_KEY "gimp-softproof-profile-dialog"
  ...
  if (!dialog) {
    ...
    current_profile =
        gimp_color_config_get_simulation_color_profile(color_config, NULL);

    dialog = color_profile_dialog_new(
        COLOR_PROFILE_DIALOG_SELECT_SOFTPROOF_PROFILE, image,
        action_data_get_context(data), GTK_WIDGET(shell), current_profile, NULL,
        0, 0, view_softproof_profile_callback, shell);

    dialogs_attach_dialog(G_OBJECT(shell), SOFTPROOF_PROFILE_DIALOG_KEY,
                          dialog);
  }
...
void view_softproof_intent_cmd_callback(GimpAction *action, GVariant *value,
                                        gpointer data) {
  GimpDisplayShell *shell;
  GimpColorConfig *color_config;
  GimpColorRenderingIntent intent;
  ...
  if (intent != gimp_color_config_get_simulation_intent(color_config)) {
    g_object_set(color_config, "simulation-rendering-intent", intent, NULL);
    shell->color_config_set = TRUE;
  }
}

void view_softproof_bpc_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpDisplayShell *shell;
  GimpColorConfig *color_config;
  gboolean active;
  ...
  if (active != gimp_color_config_get_simulation_bpc(color_config)) {
    g_object_set(color_config, "simulation-use-black-point-compensation",
                 active, NULL);
    shell->color_config_set = TRUE;
  }
}
...
void view_softproof_gamut_check_cmd_callback(GimpAction *action,
  ...
  if (active != gimp_color_config_get_simulation_gamut_check(color_config)) {
    g_object_set(color_config, "simulation-gamut-check", active, NULL);
    shell->color_config_set = TRUE;
  }
}

void view_toggle_selection_cmd_callback(GimpAction *action, GVariant *value,
                                        gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_toggle_menubar_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_toggle_rulers_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_toggle_scrollbars_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_toggle_statusbar_cmd_callback(GimpAction *action, GVariant *value,
                                        gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_toggle_guides_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_toggle_grid_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_toggle_sample_points_cmd_callback(GimpAction *action, GVariant *value,
                                            gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_snap_to_guides_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_snap_to_grid_cmd_callback(GimpAction *action, GVariant *value,
                                    gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_snap_to_canvas_cmd_callback(GimpAction *action, GVariant *value,
                                      gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_snap_to_vectors_cmd_callback(GimpAction *action, GVariant *value,
                                       gpointer data) {
  GimpDisplayShell *shell;
  gboolean active;
  return_if_no_shell(shell, data);
...
void view_padding_color_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpDisplay *display;
...
#define PADDING_COLOR_DIALOG_KEY "gimp-padding-color-dialog"
  ...
  case GIMP_CANVAS_PADDING_MODE_CUSTOM: {
    ...
    if (!dialog) {
      ...
      dialog = gimp_color_dialog_new(
          GIMP_VIEWABLE(image), action_data_get_context(data), FALSE,
          _("Set Canvas Padding Color"), GIMP_ICON_FONT,
          _("Set Custom Canvas Padding Color"), GTK_WIDGET(shell), NULL, NULL,
          &options->padding_color, TRUE, FALSE);

      g_signal_connect(dialog, "update",
                       G_CALLBACK(view_padding_color_dialog_update), shell);

      dialogs_attach_dialog(G_OBJECT(shell), PADDING_COLOR_DIALOG_KEY, dialog);
    }
    *old_color = options->padding_color;
    old_padding_mode = options->padding_mode;
    g_object_set_data_full(G_OBJECT(dialog), "old-color", old_color, g_free);
  ...
  case GIMP_CANVAS_PADDING_MODE_RESET:
...

    {
      ...
      gimp_display_shell_set_padding(shell, default_options->padding_mode,
                                     &default_options->padding_color);
      gimp_display_shell_set_padding_in_show_all(
          shell, default_options->padding_in_show_all);
    }
    break;
  }
}
...
void view_shrink_wrap_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpDisplayShell *shell;
  return_if_no_shell(shell, data);
...
void view_fullscreen_cmd_callback(GimpAction *action, GVariant *value,
                                  gpointer data) {
  GimpDisplay *display;
  GimpDisplayShell *shell;
  GimpImageWindow *window;
...
static void view_softproof_profile_callback(GtkWidget *dialog, GimpImage *image,
  ...
  g_object_set(color_config, "simulation-profile", path, NULL);
  shell->color_config_set = TRUE;
...
static void view_padding_color_dialog_update(GimpColorDialog *dialog,
  ...
  old_padding_mode =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(dialog), "old-padding-mode"));
  ...
  case GIMP_COLOR_DIALOG_CANCEL:
    gimp_display_shell_set_padding(shell, old_padding_mode, old_color);
    gtk_widget_destroy(GTK_WIDGET(dialog));
    break;
