...
gimp_gegl_procedure_execute(GimpProcedure *procedure, Gimp *gimp,
...
G_DEFINE_TYPE(GimpGeglProcedure, gimp_gegl_procedure, GIMP_TYPE_PROCEDURE)

#define parent_class gimp_gegl_procedure_parent_class

static void gimp_gegl_procedure_class_init(GimpGeglProcedureClass *klass) {
...
static void gimp_gegl_procedure_init(GimpGeglProcedure *proc) {}
...
gimp_gegl_procedure_execute(GimpProcedure *procedure, Gimp *gimp,
  ...
  node = gegl_node_new_child(NULL, "operation",
                             GIMP_GEGL_PROCEDURE(procedure)->operation, NULL);

  gimp_drawable_apply_operation_with_config(
      drawable, progress, gimp_procedure_get_label(procedure), node, config);
...
static void gimp_gegl_procedure_execute_async(GimpProcedure *procedure,
  ...
  if (!settings &&
    ...
    container = gimp_operation_config_get_container(
        gimp, config_type, (GCompareFunc)gimp_settings_compare);
  ...
  if (run_mode == GIMP_RUN_NONINTERACTIVE ||
    ...
    if (settings || run_mode == GIMP_RUN_NONINTERACTIVE) {
      ...
      gimp_procedure_execute(procedure, gimp, context, progress, args, NULL);
      return;
    }
  ...
  if (!strcmp(gimp_object_get_name(active_tool->tool_info), tool_name)) {
    ...
    if (!strcmp(tool_name, "gimp-operation-tool")) {
      gimp_operation_tool_set_operation(
          GIMP_OPERATION_TOOL(active_tool), gegl_procedure->operation,
          gimp_procedure_get_label(procedure),
          gimp_procedure_get_label(procedure),
          gimp_procedure_get_label(procedure),
...
GimpProcedure *gimp_gegl_procedure_new(
  ...
  gimp_procedure_set_help(procedure, tooltip, tooltip, help_id);
  gimp_procedure_set_static_attribution(procedure, "author", "copyright",
                                        "date");

  gimp_procedure_add_argument(
      procedure, gimp_param_spec_enum("run-mode", "Run mode", "Run mode",
  ...
  gimp_procedure_add_argument(procedure,
                              gimp_param_spec_drawable("drawable", "Drawable",
                                                       "Input drawable", TRUE,
                                                       GIMP_PARAM_READWRITE));
  gimp_procedure_add_argument(
      procedure, g_param_spec_object("settings", "Settings", "Settings",
                                     config_type, GIMP_PARAM_READWRITE));
