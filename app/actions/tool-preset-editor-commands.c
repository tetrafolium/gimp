...
void tool_preset_editor_save_cmd_callback(GimpAction *action, GVariant *value,
                                          gpointer data) {
  ...
  if (tool_info && preset) {
    ...
    if (tool_info != preset_tool) {
      gimp_message(context->gimp, G_OBJECT(editor), GIMP_MESSAGE_WARNING,
                   _("Can't save '%s' tool options to an "
                     "existing '%s' tool preset."),
                   tool_info->label, preset_tool->label);
      return;
    }
