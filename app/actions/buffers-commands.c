...
void buffers_paste_cmd_callback(GimpAction *action, GVariant *value,
                                gpointer data) {
  ...
  if (buffer && gimp_container_have(container, GIMP_OBJECT(buffer))) {
    ...
    if (image) {
      ...
      gimp_edit_paste(image,
                      ...
                      GIMP_OBJECT(buffer), paste_type, x, y, width, height);
...
void buffers_paste_as_new_image_cmd_callback(GimpAction *action,
  ...
  if (buffer && gimp_container_have(container, GIMP_OBJECT(buffer))) {
    ...
    new_image =
        gimp_edit_paste_as_new_image(context->gimp, GIMP_OBJECT(buffer));
...
void buffers_delete_cmd_callback(GimpAction *action, GVariant *value,
                                 gpointer data) {
  GimpContainerEditor *editor = GIMP_CONTAINER_EDITOR(data);
