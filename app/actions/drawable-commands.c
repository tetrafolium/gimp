...
void drawable_equalize_cmd_callback(GimpAction *action, GVariant *value,
                                    gpointer data) {
  GimpImage *image;
...
void drawable_levels_stretch_cmd_callback(GimpAction *action, GVariant *value,
                                          gpointer data) {
  GimpImage *image;
  GimpDrawable *drawable;
  GimpDisplay *display;
...
void drawable_linked_cmd_callback(GimpAction *action, GVariant *value,
                                  gpointer data) {
  GimpImage *image;
  GList *drawables;
  GList *iter;
...
void drawable_visible_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpImage *image;
  GList *drawables;
  GList *iter;
  ...
  if (g_list_length(drawables) == 1) {
    ...
    /* TODO: undo groups cannot be compressed so far. */
    gimp_image_undo_group_start(image, GIMP_UNDO_GROUP_ITEM_VISIBILITY,
                                "Item visibility");
  }
...
void drawable_lock_content_cmd_callback(GimpAction *action, GVariant *value,
                                        gpointer data) {
  GimpImage *image;
  GimpDrawable *drawable;
  gboolean locked;
...
void drawable_lock_position_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpImage *image;
  GimpDrawable *drawable;
  gboolean locked;
...
void drawable_flip_cmd_callback(GimpAction *action, GVariant *value,
                                gpointer data) {
  GimpImage *image;
  GimpDrawable *drawable;
  GimpItem *item;
...
void drawable_rotate_cmd_callback(GimpAction *action, GVariant *value,
                                  gpointer data) {
  GimpImage *image;
  GimpDrawable *drawable;
  GimpContext *context;
  ...
  if (gimp_item_get_linked(item)) {
    gimp_item_linked_rotate(item, context, rotation_type, center_x, center_y,
                            FALSE);
  } else {
    gimp_item_rotate(item, context, rotation_type, center_x, center_y,
                     gimp_item_get_clip(item, FALSE));
  }
