...
static void
channels_new_callback(GtkWidget *dialog, GimpImage *image, GimpChannel *channel,
...
static void channels_edit_attributes_callback(
...
void channels_edit_attributes_cmd_callback(GimpAction *action, GVariant *value,
                                           gpointer data) {
  GimpImage *image;
  GimpChannel *channel;
  GtkWidget *widget;
...
#define EDIT_DIALOG_KEY "gimp-channel-edit-attributes-dialog"
  ...
  if (!dialog) {
    ...
    dialog = channel_options_dialog_new(
        image, channel, action_data_get_context(data), widget,
        _("Channel Attributes"), "gimp-channel-edit", GIMP_ICON_EDIT,
        _("Edit Channel Attributes"), GIMP_HELP_CHANNEL_EDIT,
        _("Edit Channel Color"), _("_Fill opacity:"), FALSE,
        gimp_object_get_name(channel), &channel->color,
        gimp_item_get_visible(item), gimp_item_get_linked(item),
        gimp_item_get_color_tag(item), gimp_item_get_lock_content(item),
        gimp_item_get_lock_position(item), channels_edit_attributes_callback,
        NULL);
...
void channels_new_cmd_callback(GimpAction *action, GVariant *value,
                               gpointer data) {
  GimpImage *image;
  GtkWidget *widget;
  GtkWidget *dialog;
...
#define NEW_DIALOG_KEY "gimp-channel-new-dialog"
  ...
  if (!dialog) {
    ...
    dialog = channel_options_dialog_new(
        image, NULL, action_data_get_context(data), widget, _("New Channel"),
        "gimp-channel-new", GIMP_ICON_CHANNEL, _("Create a New Channel"),
        GIMP_HELP_CHANNEL_NEW, _("New Channel Color"), _("_Fill opacity:"),
        TRUE, config->channel_new_name, &config->channel_new_color, TRUE, FALSE,
        GIMP_COLOR_TAG_NONE, FALSE, FALSE, channels_new_callback, NULL);
...
void channels_new_last_vals_cmd_callback(GimpAction *action, GVariant *value,
  ...
  channel = gimp_channel_new(
      image, gimp_image_get_width(image), gimp_image_get_height(image),
      config->channel_new_name, &config->channel_new_color);

  gimp_drawable_fill(GIMP_DRAWABLE(channel), action_data_get_context(data),
...
void channels_raise_cmd_callback(GimpAction *action, GVariant *value,
                                 gpointer data) {
  GimpImage *image;
  GimpChannel *channel;
  return_if_no_channel(image, channel, data);
...
void channels_raise_to_top_cmd_callback(GimpAction *action, GVariant *value,
                                        gpointer data) {
  GimpImage *image;
  GimpChannel *channel;
  return_if_no_channel(image, channel, data);
...
void channels_lower_to_bottom_cmd_callback(GimpAction *action, GVariant *value,
                                           gpointer data) {
  GimpImage *image;
  GimpChannel *channel;
  return_if_no_channel(image, channel, data);
...
void channels_duplicate_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpImage *image;
  ...
  if (GIMP_IS_COMPONENT_EDITOR(data)) {
    ...
    new_channel = gimp_channel_new_from_component(image, component, name, NULL);
    ...
    new_channel = GIMP_CHANNEL(
        gimp_item_duplicate(GIMP_ITEM(channel), G_TYPE_FROM_INSTANCE(channel)));
...
void channels_delete_cmd_callback(GimpAction *action, GVariant *value,
                                  gpointer data) {
  GimpImage *image;
  GimpChannel *channel;
  return_if_no_channel(image, channel, data);
...
void channels_to_selection_cmd_callback(GimpAction *action, GVariant *value,
  ...
  if (GIMP_IS_COMPONENT_EDITOR(data)) {
    ...
    gimp_channel_select_component(gimp_image_get_mask(image), component, op,
                                  FALSE, 0.0, 0.0);
  } else {
    GimpChannel *channel;
    return_if_no_channel(image, channel, data);

    gimp_item_to_selection(GIMP_ITEM(channel), op, TRUE, FALSE, 0.0, 0.0);
  }

  gimp_image_flush(image);
}

void channels_visible_cmd_callback(GimpAction *action, GVariant *value,
                                   gpointer data) {
  GimpImage *image;
  GimpChannel *channel;
  return_if_no_channel(image, channel, data);
...
void channels_linked_cmd_callback(GimpAction *action, GVariant *value,
                                  gpointer data) {
  GimpImage *image;
  GimpChannel *channel;
  return_if_no_channel(image, channel, data);
...
void channels_lock_position_cmd_callback(GimpAction *action, GVariant *value,
                                         gpointer data) {
  GimpImage *image;
  GimpChannel *channel;
  return_if_no_channel(image, channel, data);
...
void channels_color_tag_cmd_callback(GimpAction *action, GVariant *value,
                                     gpointer data) {
  GimpImage *image;
  GimpChannel *channel;
  ...
  items_color_tag_cmd_callback(action, image, GIMP_ITEM(channel), color_tag);
}

void channels_select_cmd_callback(GimpAction *action, GVariant *value,
                                  gpointer data) {
  GimpImage *image;
  GimpChannel *channel;
...
channels_new_callback(GtkWidget *dialog, GimpImage *image, GimpChannel *channel,
  ...
  g_object_set(config, "channel-new-name", channel_name, "channel-new-color",
               channel_color, NULL);
  ...
  if (save_selection) {
    ...
    channel = GIMP_CHANNEL(
        gimp_item_duplicate(GIMP_ITEM(selection), GIMP_TYPE_CHANNEL));

    gimp_object_set_name(GIMP_OBJECT(channel), config->channel_new_name);
    ...
    gimp_drawable_fill(GIMP_DRAWABLE(channel), context, GIMP_FILL_TRANSPARENT);
  }
