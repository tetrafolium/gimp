...
static gchar *gimp_text_get_xlfd_field(const gchar *fontname, gint field_num,
                                       gchar *buffer);
...
gchar *gimp_text_font_name_from_xlfd(const gchar *xlfd) {
  ...
  return launder_font_name(g_strconcat(fields[0], " ", fields[1], " ",
 ...
 **/
gboolean gimp_text_font_size_from_xlfd(const gchar *xlfd, gdouble *size,
                                       GimpUnit *size_unit) {
 ...
 **/
void gimp_text_set_font_from_xlfd(GimpText *text, const gchar *xlfd) {
  gchar *font;
  gdouble size;
  GimpUnit size_unit;
  ...
  if (gimp_text_font_size_from_xlfd(xlfd, &size, &size_unit)) {
    g_object_set(text, "font-size", size, "font-size-unit", size_unit,
                 font ? "font" : NULL, font, NULL);
  } else if (font) {
    g_object_set(text, "font", font, NULL);
  }
