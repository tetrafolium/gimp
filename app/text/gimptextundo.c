...
enum { PROP_0, PROP_PARAM };

static void gimp_text_undo_constructed(GObject *object);
static void gimp_text_undo_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_text_undo_get_property(GObject *object, guint property_id,
                                        GValue *value, GParamSpec *pspec);

static gint64 gimp_text_undo_get_memsize(GimpObject *object, gint64 *gui_size);

static void gimp_text_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
                               GimpUndoAccumulator *accum);
static void gimp_text_undo_free(GimpUndo *undo, GimpUndoMode undo_mode);
...
static void gimp_text_undo_class_init(GimpTextUndoClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_PARAM,
      g_param_spec_param("param", NULL, NULL, G_TYPE_PARAM,
...
static void gimp_text_undo_constructed(GObject *object) {
  ...
  case GIMP_UNDO_TEXT_LAYER:
    if (text_undo->pspec) {
      ...
      g_object_get_property(G_OBJECT(layer->text), text_undo->pspec->name,
                            text_undo->value);
    } else if (layer->text) {
...
static void gimp_text_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
  ...
  case GIMP_UNDO_TEXT_LAYER:
    if (text_undo->pspec) {
      ...
      g_object_set_property(G_OBJECT(layer->text), text_undo->pspec->name,
                            text_undo->value);
      ...
      text = (layer->text ? gimp_config_duplicate(GIMP_CONFIG(layer->text))
                          : NULL);

      if (layer->text && text_undo->text)
        gimp_config_sync(G_OBJECT(text_undo->text), G_OBJECT(layer->text), 0);
      else
  ...
  case GIMP_UNDO_TEXT_LAYER_CONVERT: {
    ...
    gimp_drawable_convert_type(
        GIMP_DRAWABLE(layer), gimp_item_get_image(GIMP_ITEM(layer)),
        gimp_babl_format_get_base_type(text_undo->format),
        gimp_babl_format_get_precision(text_undo->format),
        babl_format_has_alpha(text_undo->format), NULL, NULL, GEGL_DITHER_NONE,
