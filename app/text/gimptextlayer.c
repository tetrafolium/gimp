...
enum { PROP_0, PROP_TEXT, PROP_AUTO_RENAME, PROP_MODIFIED };
...
static void gimp_text_layer_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);
static void gimp_text_layer_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);

static gint64 gimp_text_layer_get_memsize(GimpObject *object, gint64 *gui_size);

static GimpItem *gimp_text_layer_duplicate(GimpItem *item, GType new_type);
static gboolean gimp_text_layer_rename(GimpItem *item, const gchar *new_name,
...
static void gimp_text_layer_convert_type(
...
static void gimp_text_layer_class_init(GimpTextLayerClass *klass) {
  ...
  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_TEXT, "text", NULL, NULL,
                          GIMP_TYPE_TEXT, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_AUTO_RENAME, "auto-rename", NULL,
                           NULL, TRUE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_MODIFIED, "modified", NULL, NULL,
...
static gint64 gimp_text_layer_get_memsize(GimpObject *object,
  ...
  memsize += gimp_object_get_memsize(GIMP_OBJECT(text_layer->text), gui_size);
...
static void gimp_text_layer_push_undo(GimpDrawable *drawable,
  ...
  GIMP_DRAWABLE_CLASS(parent_class)
      ->push_undo(drawable, undo_desc, buffer, x, y, width, height);
...
static void gimp_text_layer_convert_type(
  ...
  if (!text_layer->text || text_layer->modified ||
      layer_dither_type != GEGL_DITHER_NONE) {
    GIMP_LAYER_CLASS(parent_class)
        ->convert_type(layer, dest_image, new_format, src_profile, dest_profile,
                       layer_dither_type, mask_dither_type, push_undo,
                       progress);
  } else {
    if (push_undo)
...
GimpLayer *gimp_text_layer_new(GimpImage *image, GimpText *text) {
  ...
  layer = GIMP_TEXT_LAYER(
      gimp_drawable_new(GIMP_TYPE_TEXT_LAYER, image, NULL, 0, 0, 1, 1,
...
void gimp_text_layer_set_text(GimpTextLayer *layer, GimpText *text) {
  ...
  if (text) {
    ...
    g_signal_connect_object(text, "changed",
                            G_CALLBACK(gimp_text_layer_text_changed), layer,
                            G_CONNECT_SWAPPED);
  }
...
void gimp_text_layer_discard(GimpTextLayer *layer) {
  ...
  gimp_image_undo_push_text_layer(gimp_item_get_image(GIMP_ITEM(layer)),
                                  _("Discard Text Information"), layer, NULL);

  gimp_text_layer_set_text(layer, NULL);
}

gboolean gimp_item_is_text_layer(GimpItem *item) {
  return (GIMP_IS_TEXT_LAYER(item) && GIMP_TEXT_LAYER(item)->text &&
          GIMP_TEXT_LAYER(item)->modified == FALSE);
}

/*  private functions  */

static const Babl *gimp_text_layer_get_format(GimpTextLayer *layer) {
...
static void gimp_text_layer_text_changed(GimpTextLayer *layer) {
  ...
  if (layer->text_parasite) {
     ...
     */
    gimp_item_parasite_detach(GIMP_ITEM(layer), layer->text_parasite, FALSE);
    layer->text_parasite = NULL;
  }
...
static gboolean gimp_text_layer_render(GimpTextLayer *layer) {
  ...
  if (gimp_text_layout_get_size(layout, &width, &height) &&
    ...
    if (gimp_layer_get_mask(GIMP_LAYER(layer))) {
      ...
      gimp_item_resize(GIMP_ITEM(mask), unused_eek, GIMP_FILL_TRANSPARENT,
                       width, height, 0, 0);
    }
  }
  ...
  if (layer->auto_rename) {
    ...
    if (gimp_item_is_attached(item)) {
      gimp_item_tree_rename_item(gimp_item_get_tree(item), item, name, FALSE,
                                 NULL);
      g_free(name);
    } else {
...
static void gimp_text_layer_render_layout(GimpTextLayer *layer,
  ...
  if (status != CAIRO_STATUS_SUCCESS) {
    ...
    gimp_message_literal(
        image->gimp, NULL, GIMP_MESSAGE_ERROR,
  ...
  if (transform) {
    gimp_color_transform_process_buffer(
        transform, buffer, NULL, gimp_drawable_get_buffer(drawable), NULL);
  } else {
