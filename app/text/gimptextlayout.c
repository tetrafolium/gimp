...
static void gimp_text_layout_set_markup(GimpTextLayout *layout, GError **error);

static PangoContext *gimp_text_get_pango_context(GimpText *text, gdouble xres,
                                                 gdouble yres);
...
GimpTextLayout *gimp_text_layout_new(GimpText *text, gdouble xres, gdouble yres,
                                     GError **error) {
  ...
  size = pango_units_from_double(
      gimp_units_to_points(text->font_size, text->unit, yres));
  ...
  case GIMP_TEXT_BOX_FIXED:
    if (!PANGO_GRAVITY_IS_VERTICAL(pango_context_get_base_gravity(context)))
      pango_layout_set_width(layout->layout,
                             pango_units_from_double(gimp_units_to_pixels(
                                 text->box_width, text->box_unit, xres)));
    else
      pango_layout_set_width(layout->layout,
                             pango_units_from_double(gimp_units_to_pixels(
                                 text->box_height, text->box_unit, yres)));
    break;
  }

  pango_layout_set_indent(layout->layout,
                          pango_units_from_double(gimp_units_to_pixels(
                              text->indent, text->unit, xres)));
  pango_layout_set_spacing(layout->layout,
                           pango_units_from_double(gimp_units_to_pixels(
                               text->line_spacing, text->unit, yres)));
  ...
  case GIMP_TEXT_BOX_FIXED:
    layout->extents.width =
        ceil(gimp_units_to_pixels(text->box_width, text->box_unit, xres));
    layout->extents.height =
        ceil(gimp_units_to_pixels(text->box_height, text->box_unit, yres));
...
gboolean gimp_text_layout_get_size(GimpTextLayout *layout, gint *width,
                                   gint *height) {
...
void gimp_text_layout_get_offsets(GimpTextLayout *layout, gint *x, gint *y) {
...
void gimp_text_layout_get_resolution(GimpTextLayout *layout, gdouble *xres,
                                     gdouble *yres) {
  g_return_if_fail(GIMP_IS_TEXT_LAYOUT(layout));
...
void gimp_text_layout_transform_point(GimpTextLayout *layout, gdouble *x,
                                      gdouble *y) {
  cairo_matrix_t matrix;
  gdouble _x = 0.0;
  gdouble _y = 0.0;
...
void gimp_text_layout_transform_distance(GimpTextLayout *layout, gdouble *x,
                                         gdouble *y) {
  cairo_matrix_t matrix;
  gdouble _x = 0.0;
  gdouble _y = 0.0;
...
void gimp_text_layout_untransform_point(GimpTextLayout *layout, gdouble *x,
                                        gdouble *y) {
  cairo_matrix_t matrix;
  gdouble _x = 0.0;
  gdouble _y = 0.0;
...
void gimp_text_layout_untransform_distance(GimpTextLayout *layout, gdouble *x,
                                           gdouble *y) {
  cairo_matrix_t matrix;
  gdouble _x = 0.0;
  gdouble _y = 0.0;
...
static gboolean gimp_text_layout_split_markup(const gchar *markup,
                                              gchar **open_tag, gchar **content,
                                              gchar **close_tag) {
  gchar *p_open;
  gchar *p_close;
...
static gchar *gimp_text_layout_apply_tags(GimpTextLayout *layout,
                                          const gchar *markup) {
...

  {
    ...
    result = g_strdup_printf("<span color=\"#%02x%02x%02x\">%s</span>", r, g, b,
                             markup);
  }

  if (fabs(text->letter_spacing) > 0.1) {
    gchar *tmp =
        g_strdup_printf("<span letter_spacing=\"%d\">%s</span>",
                        (gint)(text->letter_spacing * PANGO_SCALE), result);
    g_free(result);
    result = tmp;
  }
...
static void gimp_text_layout_set_markup(GimpTextLayout *layout,
  ...
  if (text->markup) {
    if (!gimp_text_layout_split_markup(text->markup, &open_tag, &content,
                                       &close_tag)) {
  ...
  if (pango_parse_markup(markup, -1, 0, NULL, NULL, NULL, error) == FALSE) {
    if (error && *error && (*error)->domain == G_MARKUP_ERROR &&
...
static void gimp_text_layout_position(GimpTextLayout *layout) {
  ...
  if (pango_layout_get_width(layout->layout) > 0) {
    ...
    if ((base_dir == GIMP_TEXT_DIRECTION_LTR && align == PANGO_ALIGN_RIGHT) ||
        ...
        (base_dir == GIMP_TEXT_DIRECTION_TTB_RTL &&
         align == PANGO_ALIGN_RIGHT) ||
        (base_dir == GIMP_TEXT_DIRECTION_TTB_RTL_UPRIGHT &&
         align == PANGO_ALIGN_RIGHT) ||
        (base_dir == GIMP_TEXT_DIRECTION_TTB_LTR &&
  ...
  g_printerr("layout extents: %d x %d @ %d, %d\n", layout->extents.width,
             layout->extents.height, layout->extents.x, layout->extents.y);
#endif
}

static cairo_font_options_t *gimp_text_get_font_options(GimpText *text) {
