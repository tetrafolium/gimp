...
static GimpLayer *gimp_text_layer_from_layer(GimpLayer *layer, GimpText *text);
  ...
  if (parasite) {
    ...
    if (error) {
      gimp_message(gimp_item_get_image(GIMP_ITEM(*layer))->gimp, NULL,
                   ...
                   gimp_object_get_name(*layer), error->message);
      g_clear_error(&error);
    }
  } else {
...
void gimp_text_layer_set_xcf_flags(GimpTextLayer *text_layer, guint32 flags) {
  ...
  g_object_set(text_layer, "auto-rename",
               (flags & TEXT_LAYER_XCF_DONT_AUTO_RENAME) == 0, "modified",
               (flags & TEXT_LAYER_XCF_MODIFIED) != 0, NULL);
}
...
static GimpLayer *gimp_text_layer_from_layer(GimpLayer *layer, GimpText *text) {
  ...
  text_layer = g_object_new(GIMP_TYPE_TEXT_LAYER, "image",
                            gimp_item_get_image(GIMP_ITEM(layer)), NULL);
  ...
  gimp_layer_set_opacity(GIMP_LAYER(text_layer), gimp_layer_get_opacity(layer),
                         FALSE);
  gimp_layer_set_mode(GIMP_LAYER(text_layer), gimp_layer_get_mode(layer),
                      FALSE);
