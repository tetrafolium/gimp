...
static void gimp_font_factory_load(GimpFontFactory *factory, GError **error);
static gboolean gimp_font_factory_load_fonts_conf(FcConfig *config,
                                                  GFile *fonts_conf);
static void gimp_font_factory_add_directories(FcConfig *config, GList *path,
                                              GError **error);
static void gimp_font_factory_recursive_add_fontdir(FcConfig *config,
                                                    GFile *file,
                                                    GError **error);
...
static void gimp_font_factory_class_init(GimpFontFactoryClass *klass) {
...
static void gimp_font_factory_init(GimpFontFactory *factory) {
...
static void gimp_font_factory_data_init(GimpDataFactory *factory,
  ...
  if (error) {
    gimp_message_literal(gimp_data_factory_get_gimp(factory), NULL,
                         GIMP_MESSAGE_INFO, error->message);
    g_error_free(error);
  }
}
...
GimpDataFactory *gimp_font_factory_new(Gimp *gimp,
  ...
  return g_object_new(GIMP_TYPE_FONT_FACTORY, "gimp", gimp, "data-type",
                      GIMP_TYPE_FONT, "path-property-name", path_property_name,
                      "get-standard-func", gimp_font_get_standard, NULL);
}

/*  private functions  */

static void gimp_font_factory_load_async(GimpAsync *async, FcConfig *config) {
...
static void gimp_font_factory_load_async_callback(GimpAsync *async,
...
static void gimp_font_factory_load(GimpFontFactory *factory, GError **error) {
  ...
  async = gimp_parallel_run_async_independent_full(
      +10, (GimpRunAsyncFunc)gimp_font_factory_load_async, config);

  gimp_async_add_callback_for_object(
      async, (GimpAsyncCallback)gimp_font_factory_load_async_callback, factory,
      factory);
  ...
  if (enumerator) {
    ...
    if (error) {
      ...
      if (*error) {
        ...
        g_clear_error(error);
        g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, "%s\n- %s%s",
                    current_message, path, G_DIR_SEPARATOR_S);
        g_free(current_message);
      } else {
        g_set_error(error, G_FILE_ERROR, G_FILE_ERROR_FAILED, "- %s%s", path,
                    G_DIR_SEPARATOR_S);
      }
...
static void gimp_font_factory_add_font(GimpContainer *container,
                                       PangoContext *context,
   ...
   */
  if (name && strlen(name) > 0 && g_utf8_validate(name, -1, NULL)) {
    GimpFont *font;

    font = g_object_new(GIMP_TYPE_FONT, "name", name, "pango-context", context,
                        NULL);
 ...
 */
static void gimp_font_factory_make_alias(GimpContainer *container,
                                         PangoContext *context,
                                         const gchar *family, gboolean bold,
                                         gboolean italic) {
...
static void gimp_font_factory_load_aliases(GimpContainer *container,
  ...
  for (i = 0; i < 3; i++) {
    gimp_font_factory_make_alias(container, context, families[i], FALSE, FALSE);
    gimp_font_factory_make_alias(container, context, families[i], TRUE, FALSE);
    gimp_font_factory_make_alias(container, context, families[i], FALSE, TRUE);
    gimp_font_factory_make_alias(container, context, families[i], TRUE, TRUE);
  }
}
...
static void gimp_font_factory_load_names(GimpContainer *container,
  ...
  os = FcObjectSetBuild(FC_FAMILY, FC_STYLE, FC_SLANT, FC_WEIGHT, FC_WIDTH,
                        NULL);
  g_return_if_fail(os);
