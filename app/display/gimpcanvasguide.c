...
enum { PROP_0, PROP_ORIENTATION, PROP_POSITION, PROP_STYLE };
...
#define GET_PRIVATE(guide)                                                     \
  ((GimpCanvasGuidePrivate *)gimp_canvas_guide_get_instance_private(           \
      (GimpCanvasGuide *)(guide)))

/*  local function prototypes  */

static void gimp_canvas_guide_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_canvas_guide_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);
static void gimp_canvas_guide_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *gimp_canvas_guide_get_extents(GimpCanvasItem *item);
static void gimp_canvas_guide_stroke(GimpCanvasItem *item, cairo_t *cr);
...
static void gimp_canvas_guide_class_init(GimpCanvasGuideClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("orientation", NULL, NULL, GIMP_TYPE_ORIENTATION_TYPE,
                        GIMP_ORIENTATION_HORIZONTAL, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_POSITION,
      g_param_spec_int("position", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_STYLE,
      g_param_spec_enum("style", NULL, NULL, GIMP_TYPE_GUIDE_STYLE,
                        GIMP_GUIDE_STYLE_NONE, GIMP_PARAM_READWRITE));
}

static void gimp_canvas_guide_init(GimpCanvasGuide *guide) {}
...
static void gimp_canvas_guide_transform(GimpCanvasItem *item, gdouble *x1,
...
static void gimp_canvas_guide_stroke(GimpCanvasItem *item, cairo_t *cr) {
  ...
  if (private->style != GIMP_GUIDE_STYLE_NONE) {
    ...
    gimp_canvas_set_guide_style(
        gimp_canvas_item_get_canvas(item), cr, private->style,
        gimp_canvas_item_get_highlight(item), shell->offset_x, shell->offset_y);
    cairo_stroke(cr);
  } else {
...
GimpCanvasItem *gimp_canvas_guide_new(GimpDisplayShell *shell,
  ...
  return g_object_new(GIMP_TYPE_CANVAS_GUIDE, "shell", shell, "orientation",
                      orientation, "position", position, "style", style, NULL);
}

void gimp_canvas_guide_set(GimpCanvasItem *guide,
                           GimpOrientationType orientation, gint position) {
  g_return_if_fail(GIMP_IS_CANVAS_GUIDE(guide));

  gimp_canvas_item_begin_change(guide);

  g_object_set(guide, "orientation", orientation, "position", position, NULL);
