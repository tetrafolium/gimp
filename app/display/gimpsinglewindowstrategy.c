...
static void gimp_single_window_strategy_window_strategy_iface_init(
    GimpWindowStrategyInterface *iface);
static GtkWidget *gimp_single_window_strategy_show_dockable_dialog(
    GimpWindowStrategy *strategy, Gimp *gimp, GimpDialogFactory *factory,
    GdkMonitor *monitor, const gchar *identifiers);

G_DEFINE_TYPE_WITH_CODE(
    GimpSingleWindowStrategy, gimp_single_window_strategy, GIMP_TYPE_OBJECT,
...
static void gimp_single_window_strategy_window_strategy_iface_init(
    GimpWindowStrategyInterface *iface) {
  iface->show_dockable_dialog =
      gimp_single_window_strategy_show_dockable_dialog;
}
...
static GtkWidget *gimp_single_window_strategy_show_dockable_dialog(
  ...
  if (strcmp("gimp-toolbox", identifiers) == 0) {
    ...
    if (!gimp_image_window_has_toolbox(window)) {
      ...
      widget = gimp_dialog_factory_dialog_new(
          factory, monitor, ui_manager, GTK_WIDGET(window), "gimp-toolbox",
          -1 /*view_size*/, FALSE /*present*/);
      gtk_widget_show(widget);

      columns = gimp_image_window_get_left_docks(window);
      gimp_dock_columns_add_dock(columns, GIMP_DOCK(widget), -1 /*index*/);
    }
  } else if (gimp_dialog_factory_find_widget(factory, identifiers)) {
    /* if the dialog is already open, simply raise it */
    return gimp_dialog_factory_dialog_raise(
        factory, monitor, GTK_WIDGET(window), identifiers, -1);
  } else {
    GtkWidget *dockbook;
    ...
    if (!dockbook) {
      ...
      gimp_dock_columns_prepare_dockbook(dock_columns, -1 /*index*/, &dockbook);
    }
