...
void gimp_display_shell_scroll(GimpDisplayShell *shell, gint x_offset,
  ...
  if (x_offset || y_offset) {
    ...
    gimp_overlay_box_scroll(GIMP_OVERLAY_BOX(shell->canvas), -x_offset,
                            -y_offset);
    ...
    if (shell->render_cache) {
      ...
      surface = cairo_surface_create_similar_image(
          shell->render_cache, CAIRO_FORMAT_ARGB32,
      ...
      cairo_set_operator(cr, CAIRO_OPERATOR_SOURCE);
      cairo_set_source_surface(cr, surface, -x_offset * shell->render_scale,
    ...
    if (shell->render_cache_valid) {
      ...
      cairo_region_translate(shell->render_cache_valid, -x_offset, -y_offset);
...
void gimp_display_shell_scroll_set_offset(GimpDisplayShell *shell,
  ...
  if (shell->offset_x == offset_x && shell->offset_y == offset_y)
    return;
...
void gimp_display_shell_scroll_clamp_and_update(GimpDisplayShell *shell) {
  ...
  if (image) {
    if (!shell->show_all) {
      ...
      gimp_display_shell_scale_get_image_bounds(shell, &bounds_x, &bounds_y,
                                                &bounds_width, &bounds_height);

      if (shell->disp_width < bounds_width) {
        min_offset_x = bounds_x - shell->disp_width * OVERPAN_FACTOR;
        ...
        overpan_amount =
            shell->disp_width - bounds_width * (1.0 - OVERPAN_FACTOR);

        min_offset_x = bounds_x - overpan_amount;
        max_offset_x =
            bounds_x + bounds_width - shell->disp_width + overpan_amount;
      }

      if (shell->disp_height < bounds_height) {
        min_offset_y = bounds_y - shell->disp_height * OVERPAN_FACTOR;
        ...
        overpan_amount =
            shell->disp_height - bounds_height * (1.0 - OVERPAN_FACTOR);

        min_offset_y = bounds_y - overpan_amount;
        max_offset_y =
            bounds_y + bounds_height + overpan_amount - shell->disp_height;
      }
      ...
      gimp_display_shell_scrollbars_update_steppers(
          shell, min_offset_x, max_offset_x, min_offset_y, max_offset_y);
    } else {
      /* Set scrollbar stepper sensitiity */

      gimp_display_shell_scrollbars_update_steppers(shell, G_MININT, G_MAXINT,
                                                    G_MININT, G_MAXINT);
    }
  } else {
    shell->offset_x = 0;
...
void gimp_display_shell_scroll_unoverscrollify(GimpDisplayShell *shell,
  ...
  if (!out_offset_x)
    out_offset_x = &out_offset_x_dummy;
  if (!out_offset_y)
    out_offset_y = &out_offset_y_dummy;
  ...
  *out_offset_y = in_offset_y;
  ...
  if (!shell->show_all) {
    ...
    if (in_offset_x < 0) {
      *out_offset_x = MAX(in_offset_x, MIN(0, 0 - shell->offset_x));
    } else if (in_offset_x > 0) {
      gint min_offset = sw - shell->disp_width;

      *out_offset_x = MIN(in_offset_x, MAX(0, min_offset - shell->offset_x));
    }

    if (in_offset_y < 0) {
      *out_offset_y = MAX(in_offset_y, MIN(0, 0 - shell->offset_y));
    } else if (in_offset_y > 0) {
      gint min_offset = sh - shell->disp_height;

      *out_offset_y = MIN(in_offset_y, MAX(0, min_offset - shell->offset_y));
    }
  }
}
...
void gimp_display_shell_scroll_center_image_xy(GimpDisplayShell *shell,
  ...
  gimp_display_shell_transform_xy(shell, image_x, image_y, &viewport_x,
                                  &viewport_y);

  gimp_display_shell_scroll(shell, viewport_x - shell->disp_width / 2,
...
void gimp_display_shell_scroll_center_image(GimpDisplayShell *shell,
  ...
  gimp_display_shell_scale_get_image_bounds(shell, &image_x, &image_y,
...
void gimp_display_shell_scroll_center_content(GimpDisplayShell *shell,
  ...
  if (!gimp_display_shell_get_infinite_canvas(shell)) {
    gimp_display_shell_scale_get_image_bounds(shell, &content_x, &content_y,
                                              &content_width, &content_height);
  } else {
    gimp_display_shell_scale_get_image_bounding_box(
        shell, &content_x, &content_y, &content_width, &content_height);
  }
