...
void gimp_display_shell_draw_selection_out(GimpDisplayShell *shell, cairo_t *cr,
                                           GimpSegment *segs, gint n_segs) {
  g_return_if_fail(GIMP_IS_DISPLAY_SHELL(shell));
  g_return_if_fail(cr != NULL);
  g_return_if_fail(segs != NULL && n_segs > 0);

  gimp_canvas_set_selection_out_style(shell->canvas, cr, shell->offset_x,
                                      shell->offset_y);
...
void gimp_display_shell_draw_selection_in(GimpDisplayShell *shell, cairo_t *cr,
                                          cairo_pattern_t *mask, gint index) {
  g_return_if_fail(GIMP_IS_DISPLAY_SHELL(shell));
  g_return_if_fail(cr != NULL);
  g_return_if_fail(mask != NULL);

  gimp_canvas_set_selection_in_style(shell->canvas, cr, index, shell->offset_x,
                                     shell->offset_y);

  cairo_mask(cr, mask);
}

void gimp_display_shell_draw_background(GimpDisplayShell *shell, cairo_t *cr) {
  GimpCanvas *canvas;
...
void gimp_display_shell_draw_checkerboard(GimpDisplayShell *shell,
  ...
  if (G_UNLIKELY(!shell->checkerboard)) {
    ...
    g_object_get(shell->display->config, "transparency-size", &check_size,
                 "transparency-type", &check_type, NULL);
    ...
    shell->checkerboard = gimp_cairo_checkerboard_create(
        cr, 1 << (check_size + 2), &light, &dark);
  }
...
void gimp_display_shell_draw_image(GimpDisplayShell *shell, cairo_t *cr, gint x,
  ...
  if (shell->rotate_untransform) {
    ...
    chunk_width = chunk_height =
        (MIN(chunk_width, chunk_height) - 1.0) / (fabs(sin(a)) + fabs(cos(a)));
  }
  ...
  for (r = 0; r < n_rows; r++) {
    ...
    for (c = 0; c < n_cols; c++) {
      ...
      if (!gimp_display_shell_render_is_valid(shell, x1, y1, x2 - x1,
                                              y2 - y1)) {
        /* render image to the render cache */
        gimp_display_shell_render(shell, cr, x1, y1, x2 - x1, y2 - y1, scale);

        gimp_display_shell_render_validate_area(shell, x1, y1, x2 - x1,
                                                y2 - y1);
      }
       ...
       */
      cairo_scale(cr, 1.0 / shell->render_scale, 1.0 / shell->render_scale);
