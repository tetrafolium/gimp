...
enum { PROP_0, PROP_X, PROP_Y, PROP_INDEX, PROP_SAMPLE_POINT_STYLE };
...
#define GET_PRIVATE(sample_point)                                              \
  ((GimpCanvasSamplePointPrivate *)                                            \
       gimp_canvas_sample_point_get_instance_private(                          \
...
static void gimp_canvas_sample_point_get_property(GObject *object,
                                                  ...
                                                  GValue *value,
                                                  GParamSpec *pspec);
static void gimp_canvas_sample_point_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *
gimp_canvas_sample_point_get_extents(GimpCanvasItem *item);
static void gimp_canvas_sample_point_stroke(GimpCanvasItem *item, cairo_t *cr);
static void gimp_canvas_sample_point_fill(GimpCanvasItem *item, cairo_t *cr);
...
gimp_canvas_sample_point_class_init(GimpCanvasSamplePointClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_X,
      g_param_spec_int("x", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_Y,
      g_param_spec_int("y", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_INDEX,
                                  g_param_spec_int("index", NULL, NULL, 0,
                                                   G_MAXINT, 0,
...
static void gimp_canvas_sample_point_init(GimpCanvasSamplePoint *sample_point) {
}
...
static void gimp_canvas_sample_point_set_property(GObject *object,
  ...
  private
    ->sample_point_style = g_value_get_boolean(value);
    break;
...
static void gimp_canvas_sample_point_transform(GimpCanvasItem *item, gdouble *x,
                                               gdouble *y) {
  GimpCanvasSamplePointPrivate *private = GET_PRIVATE(item);

  gimp_canvas_item_transform_xy_f(item, private->x + 0.5, private->y + 0.5, x,
                                  y);
...
static void gimp_canvas_sample_point_draw(GimpCanvasItem *item, cairo_t *cr) {
  ...
  layout = gimp_canvas_get_layout(GIMP_CANVAS(canvas), "%d", private->index);
...
GimpCanvasItem *gimp_canvas_sample_point_new(GimpDisplayShell *shell, gint x,
                                             gint y, gint index,
                                             gboolean sample_point_style) {
  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), NULL);

  return g_object_new(GIMP_TYPE_CANVAS_SAMPLE_POINT, "shell", shell, "x", x,
                      "y", y, "index", index, "sample-point-style",
                      sample_point_style, NULL);
}

void gimp_canvas_sample_point_set(GimpCanvasItem *sample_point, gint x,
                                  gint y) {
  g_return_if_fail(GIMP_IS_CANVAS_SAMPLE_POINT(sample_point));

  gimp_canvas_item_begin_change(sample_point);

  g_object_set(sample_point, "x", x, "y", y, NULL);
