...
void gimp_display_shell_icon_update(GimpDisplayShell *shell) {
  ...
  if (gimp_display_get_image(shell->display))
    shell->icon_idle_id =
        g_timeout_add_full(G_PRIORITY_LOW, GIMP_DISPLAY_UPDATE_ICON_TIMEOUT,
                           gimp_display_shell_icon_update_idle, shell, NULL);
  else
...
static gboolean gimp_display_shell_icon_update_idle(gpointer data) {
  ...
  if (image) {
    ...
    width *= scale_factor;
    height *= scale_factor;
    pixbuf = gimp_viewable_get_pixbuf(
        GIMP_VIEWABLE(image), gimp_get_user_context(gimp), width, height);
    ...
    memset(gdk_pixbuf_get_pixels(icon), 0,
           gdk_pixbuf_get_height(icon) * gdk_pixbuf_get_rowstride(icon));

    gdk_pixbuf_copy_area(pixbuf, 0, 0, width, height, icon, 0,
                         shell->icon_size * scale_factor - height);
    ...
    gdk_pixbuf_composite(pixbuf, icon, shell->icon_size * scale_factor - width,
                         0, width, height,
                         shell->icon_size * scale_factor - width, 0.0, 1.0, 1.0,
