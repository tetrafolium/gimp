...
enum { PROP_0, PROP_SEGS, PROP_TRANSFORM, PROP_OFFSET_X, PROP_OFFSET_Y };
...
#define GET_PRIVATE(boundary)                                                  \
  ((GimpCanvasBoundaryPrivate *)gimp_canvas_boundary_get_instance_private(     \
      (GimpCanvasBoundary *)(boundary)))
...
static void gimp_canvas_boundary_get_property(GObject *object,
                                              ...
                                              GParamSpec *pspec);
static void gimp_canvas_boundary_draw(GimpCanvasItem *item, cairo_t *cr);
...
static void gimp_canvas_boundary_class_init(GimpCanvasBoundaryClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_SEGS,
      gimp_param_spec_array("segs", NULL, NULL, GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("offset-x", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
...
static void gimp_canvas_boundary_init(GimpCanvasBoundary *boundary) {
...
static void gimp_canvas_boundary_set_property(GObject *object,
  ...
  case PROP_OFFSET_X:
  private
    ->offset_x = g_value_get_double(value);
    break;
  case PROP_OFFSET_Y:
  private
    ->offset_y = g_value_get_double(value);
    break;
...
static void gimp_canvas_boundary_transform(GimpCanvasItem *item,
                                           GimpSegment *segs, gint *n_segs) {
  ...
  if (private->transform) {
    ...
    for (i = 0; i < private->n_segs; i++) {
      ...
      gimp_transform_polygon(private->transform, vertices, 2, FALSE, t_vertices,
                             &n_t_vertices);

      if (n_t_vertices == 2) {
        gimp_canvas_item_transform_xy(item, t_vertices[0].x + private->offset_x,
                                      t_vertices[0].y + private->offset_y,
                                      &segs[n].x1, &segs[n].y1);
        gimp_canvas_item_transform_xy(item, t_vertices[1].x + private->offset_x,
    ...
    *n_segs = n;
    ...
    for (i = 0; i < private->n_segs; i++) {
      gimp_canvas_item_transform_xy(
          item, private->segs[i].x1 + private->offset_x,
          private->segs[i].y1 + private->offset_y, &segs[i].x1, &segs[i].y1);
      gimp_canvas_item_transform_xy(
          item, private->segs[i].x2 + private->offset_x,
          private->segs[i].y2 + private->offset_y, &segs[i].x2, &segs[i].y2);
...
GimpCanvasItem *gimp_canvas_boundary_new(GimpDisplayShell *shell,
                                         const GimpBoundSeg *segs, gint n_segs,
                                         GimpMatrix3 *transform,
  ...
  item =
      g_object_new(GIMP_TYPE_CANVAS_BOUNDARY, "shell", shell, "transform",
                   transform, "offset-x", offset_x, "offset-y", offset_y, NULL);
private
  = GET_PRIVATE(item);

  /* puke */
private
  ->segs = g_memdup(segs, n_segs * sizeof(GimpBoundSeg));
private
  ->n_segs = n_segs;
