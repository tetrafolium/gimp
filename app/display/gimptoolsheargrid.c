...
enum { PROP_0, PROP_ORIENTATION, PROP_SHEAR_X, PROP_SHEAR_Y };
...
static void gimp_tool_shear_grid_class_init(GimpToolShearGridClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_SHEAR_X,
      g_param_spec_double("shear-x", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                          GIMP_MAX_IMAGE_SIZE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_SHEAR_Y,
      g_param_spec_double("shear-y", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                          GIMP_MAX_IMAGE_SIZE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_tool_shear_grid_init(GimpToolShearGrid *grid) {
  grid->private = gimp_tool_shear_grid_get_instance_private(grid);

  g_object_set(grid, "inside-function", GIMP_TRANSFORM_FUNCTION_SHEAR,
               "outside-function", GIMP_TRANSFORM_FUNCTION_SHEAR,
               "use-corner-handles", FALSE, "use-perspective-handles", FALSE,
               "use-side-handles", FALSE, "use-shear-handles", FALSE,
               "use-center-handle", FALSE, "use-pivot-handle", FALSE, NULL);
}
...
static void gimp_tool_shear_grid_set_property(GObject *object,
  ...
  case PROP_SHEAR_X:
  private
    ->shear_x = g_value_get_double(value);
    break;
  case PROP_SHEAR_Y:
  private
    ->shear_y = g_value_get_double(value);
    break;
...
void gimp_tool_shear_grid_motion(GimpToolWidget *widget,
                                 const GimpCoords *coords, guint32 time,
                                 GdkModifierType state) {
  GimpToolShearGrid *grid = GIMP_TOOL_SHEAR_GRID(widget);
  GimpToolShearGridPrivate *private = grid->private;
  ...
  g_object_get(widget, "transform", &t, "x1", &x1, "y1", &y1, "x2", &x2, "y2",
               &y2, NULL);
...
#define MIN_MOVE 5
  ...
  if (private->orientation == GIMP_ORIENTATION_HORIZONTAL) {
    ...
    private
    ->shear_y += diffy;
    else private->shear_y -= diffy;

    amount = private->shear_y;
  }

  gimp_matrix3_identity(&transform);
  gimp_transform_matrix_shear(&transform, x1, y1, x2 - x1, y2 - y1,
                              private->orientation, amount);

  g_object_set(widget, "transform", &transform, "orientation",
               private->orientation, "shear-x", private->shear_x, "shear_y",
               private->shear_y, NULL);
...
GimpToolWidget *gimp_tool_shear_grid_new(GimpDisplayShell *shell, gdouble x1,
  ...
  gimp_matrix3_identity(&transform);
  gimp_transform_matrix_shear(&transform, x1, y1, x2 - x1, y2 - y1, orientation,
                              amount);

  return g_object_new(GIMP_TYPE_TOOL_SHEAR_GRID, "shell", shell, "transform",
                      &transform, "x1", x1, "y1", y1, "x2", x2, "y2", y2,
                      "orientation", orientation, "shear-x", shear_x, "shear-y",
                      shear_y, NULL);
}
