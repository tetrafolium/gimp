...
static void gimp_canvas_set_property(GObject *object, guint property_id,
                                     const GValue *value, GParamSpec *pspec);
static void gimp_canvas_get_property(GObject *object, guint property_id,
...
static void gimp_canvas_init(GimpCanvas *canvas) {
...
GtkWidget *gimp_canvas_new(GimpDisplayConfig *config) {
  ...
  return g_object_new(GIMP_TYPE_CANVAS, "name", "gimp-canvas", "config", config,
                      NULL);
}
...
PangoLayout *gimp_canvas_get_layout(GimpCanvas *canvas, const gchar *format,
  ...
  if (!canvas->layout)
    canvas->layout = gtk_widget_create_pango_layout(GTK_WIDGET(canvas), NULL);
