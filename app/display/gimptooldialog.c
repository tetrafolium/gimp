...
#define GET_PRIVATE(dialog)                                                    \
  ((GimpToolDialogPrivate *)gimp_tool_dialog_get_instance_private(             \
...
GtkWidget *gimp_tool_dialog_new(GimpToolInfo *tool_info, GdkMonitor *monitor,
  ...
  dialog =
      g_object_new(GIMP_TYPE_TOOL_DIALOG, "title", title, "role",
                   gimp_object_get_name(tool_info), "description", description,
                   "icon-name", icon_name, "help-func", gimp_standard_help_func,
...
void gimp_tool_dialog_set_shell(GimpToolDialog *tool_dialog,
  ...
  if (private->shell) {
    ...
    g_signal_handlers_disconnect_by_func(
        private->shell, gimp_tool_dialog_shell_unmap, tool_dialog);
...
private
  ...
  if (private->shell) {
    ...
    gtk_window_set_transient_for(GTK_WINDOW(tool_dialog), GTK_WINDOW(toplevel));
