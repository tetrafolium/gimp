...
enum { PROP_0, PROP_GIMP, PROP_DIALOG_FACTORY, PROP_INITIAL_MONITOR };
...
#define GIMP_IMAGE_WINDOW_GET_PRIVATE(window)                                  \
  ((GimpImageWindowPrivate *)gimp_image_window_get_instance_private(           \
...
static void gimp_image_window_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_image_window_get_property(GObject *object, guint property_id,
...
static GimpDialogFactory *gimp_image_window_dock_container_get_dialog_factory(
    GimpDockContainer *dock_container);
static GimpUIManager *gimp_image_window_dock_container_get_ui_manager(
    GimpDockContainer *dock_container);
...
static void gimp_image_window_session_update(GimpImageWindow *window,
                                             ...
                                             GdkMonitor *monitor);
static const gchar *gimp_image_window_config_to_entry_id(GimpGuiConfig *config);
...
static void gimp_image_window_switch_page(GtkNotebook *notebook, gpointer page,
                                          gint page_num,
                                          GimpImageWindow *window);
static void gimp_image_window_page_removed(GtkNotebook *notebook,
                                           GtkWidget *widget, gint page_num,
                                           GimpImageWindow *window);
...
static void gimp_image_window_shell_icon_notify(GimpDisplayShell *shell,
                                                const GParamSpec *pspec,
                                                GimpImageWindow *window);
static GtkWidget *gimp_image_window_create_tab_label(GimpImageWindow *window,
...
static void gimp_image_window_class_init(GimpImageWindowClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_DIALOG_FACTORY,
      g_param_spec_object("dialog-factory", NULL, NULL,
                          GIMP_TYPE_DIALOG_FACTORY,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(
      object_class, PROP_INITIAL_MONITOR,
      g_param_spec_object("initial-monitor", NULL, NULL, GDK_TYPE_MONITOR,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_image_window_init(GimpImageWindow *window) {
...
private
  ->menubar_manager =
      gimp_menu_factory_manager_new(menu_factory, "<Image>", window);
  ...
  g_signal_connect(private->menubar_manager, "show-tooltip",
                   G_CALLBACK(gimp_image_window_show_tooltip), window);
  g_signal_connect(private->menubar_manager, "hide-tooltip",
                   G_CALLBACK(gimp_image_window_hide_tooltip), window);
...
private
  ->menubar =
      gimp_ui_manager_get_widget(private->menubar_manager, "/image-menubar");
#endif /* !GDK_WINDOWING_QUARTZ */
  if (private->menubar) {
    gtk_box_pack_start(GTK_BOX(private->main_vbox), private->menubar, FALSE,
                       FALSE, 0);
    ...
    g_signal_connect(private->menubar, "button-press-event",
                     G_CALLBACK(gimp_image_window_shell_events), window);
...
private
  ...
  gtk_box_pack_start(GTK_BOX(private->main_vbox), private->hbox, TRUE, TRUE, 0);
  gtk_widget_show(private->hbox);

  /* Create the left pane */
private
  ->left_hpane = gtk_paned_new(GTK_ORIENTATION_HORIZONTAL);
  gtk_paned_set_wide_handle(GTK_PANED(private->left_hpane), TRUE);
  gtk_box_pack_start(GTK_BOX(private->hbox), private->left_hpane, TRUE, TRUE,
                     0);
...
private
  ...
  gtk_paned_pack1(GTK_PANED(private->left_hpane), private->left_docks, FALSE,
                  FALSE);
  gtk_widget_set_visible(private->left_docks, config->single_window_mode);

  /* Create the right pane */
private
  ->right_hpane = gtk_paned_new(GTK_ORIENTATION_HORIZONTAL);
  gtk_paned_set_wide_handle(GTK_PANED(private->right_hpane), TRUE);
  gtk_paned_pack2(GTK_PANED(private->left_hpane), private->right_hpane, TRUE,
                  FALSE);
...
private
  ...
  gtk_paned_pack1(GTK_PANED(private->right_hpane), private->notebook, TRUE,
                  TRUE);
  g_signal_connect(private->notebook, "switch-page",
                   G_CALLBACK(gimp_image_window_switch_page), window);
  g_signal_connect(private->notebook, "page-removed",
                   G_CALLBACK(gimp_image_window_page_removed), window);
  g_signal_connect(private->notebook, "page-reordered",
                   G_CALLBACK(gimp_image_window_page_reordered), window);
...
private
  ->right_docks =
      gimp_dock_columns_new(gimp_get_user_context(private->gimp),
                            private->dialog_factory, private->menubar_manager);
  gtk_paned_pack2(GTK_PANED(private->right_hpane), private->right_docks, FALSE,
                  FALSE);
  ...
  g_signal_connect_object(config, "notify::single-window-mode",
                          G_CALLBACK(gimp_image_window_config_notify), window,
                          G_CONNECT_SWAPPED);
  ...
  gimp_image_window_session_update(window, NULL /*new_display*/,
...
static void gimp_image_window_dispose(GObject *object) {
  ...
  if (private->dialog_factory) {
    g_signal_handlers_disconnect_by_func(
        private->dialog_factory, gimp_image_window_update_ui_manager, object);
  private
    ->dialog_factory = NULL;
  }
...
static void gimp_image_window_set_property(GObject *object, guint property_id,
  ...
  private
    ->dialog_factory = g_value_get_object(value);
    break;
  case PROP_INITIAL_MONITOR:
  private
    ->initial_monitor = g_value_get_object(value);
    break;
...
static gboolean gimp_image_window_configure_event(GtkWidget *widget,
   ...
   */
  if (event->width != current_width || event->height != current_height) {
...
private
  ...
  if (event->changed_mask & GDK_WINDOW_STATE_FULLSCREEN) {
    ...
    GIMP_LOG(WM, "Image window '%s' [%p] set fullscreen %s",
             gtk_window_get_title(GTK_WINDOW(widget)), widget,
...
static void gimp_image_window_style_updated(GtkWidget *widget) {
  ...
  gtk_window_set_geometry_hints(GTK_WINDOW(widget), NULL, &geometry,
                                geometry_mask);
...
private
  = GIMP_IMAGE_WINDOW_GET_PRIVATE(dock_container);

  for (iter =
           gimp_dock_columns_get_docks(GIMP_DOCK_COLUMNS(private->left_docks));
       iter; iter = g_list_next(iter)) {
    all_docks = g_list_append(all_docks, GIMP_DOCK(iter->data));
  }

  for (iter =
           gimp_dock_columns_get_docks(GIMP_DOCK_COLUMNS(private->right_docks));
       iter; iter = g_list_next(iter)) {
    all_docks = g_list_append(all_docks, GIMP_DOCK(iter->data));
  }
...
void gimp_image_window_add_dock(GimpDockContainer *dock_container,
                                GimpDock *dock,
                                GimpSessionInfoDock *dock_info) {
  GimpImageWindow *window;
  GimpDisplayShell *active_shell;
...
private
  = GIMP_IMAGE_WINDOW_GET_PRIVATE(window);

  if (dock_info->side == GIMP_ALIGN_LEFT) {
    gimp_dock_columns_add_dock(GIMP_DOCK_COLUMNS(private->left_docks), dock,
                               -1 /*index*/);
  } else {
    gimp_dock_columns_add_dock(GIMP_DOCK_COLUMNS(private->right_docks), dock,
                               -1 /*index*/);
  }
...
private
  ...
  for (iter =
           gimp_dock_columns_get_docks(GIMP_DOCK_COLUMNS(private->left_docks));
       iter && side == -1; iter = g_list_next(iter)) {
...
private
  = GIMP_IMAGE_WINDOW_GET_PRIVATE(session_managed);
  config = GIMP_GUI_CONFIG(private->gimp->config);
  ...
  if (config->single_window_mode) {
    ...
    aux =
        gimp_session_info_aux_new(GIMP_IMAGE_WINDOW_LEFT_DOCKS_WIDTH, widthbuf);
    ...
    aux = gimp_session_info_aux_new(
        GIMP_IMAGE_WINDOW_MAXIMIZED,
...
static void gimp_image_window_set_right_docks_width(GtkPaned *paned,
                                                    GtkAllocation *allocation,
                                                    void *data) {
  ...
  g_signal_handlers_disconnect_by_func(
      paned, gimp_image_window_set_right_docks_width, data);
}
...
GimpImageWindow *gimp_image_window_new(Gimp *gimp, GimpImage *image,
                                       GimpDialogFactory *dialog_factory,
                                       GdkMonitor *monitor) {
  GimpImageWindow *window;
  GimpImageWindowPrivate *private;
  ...
  window =
      g_object_new(GIMP_TYPE_IMAGE_WINDOW, "gimp", gimp, "dialog-factory",
                   dialog_factory, "initial-monitor", monitor,
                   /* The window position will be overridden by the
                    * dialog factory, it is only really used on first
                    * startup.
                    */
                   image ? NULL : "window-position", GTK_WIN_POS_CENTER, NULL);
...
private
  ...
  if (!GIMP_GUI_CONFIG(private->gimp->config)->single_window_mode) {
    ...
    if (pointer_monitor != monitor) {
      ...
      gtk_window_move(GTK_WINDOW(window), rect.x + 300, rect.y + 30);
...
private
  ->gimp->image_windows = g_list_remove(private->gimp->image_windows, window);
...
private
  ->shells = g_list_append(private->shells, shell);

  tab_label = gimp_image_window_create_tab_label(window, shell);

  gtk_notebook_append_page(GTK_NOTEBOOK(private->notebook), GTK_WIDGET(shell),
                           tab_label);
...
private
  ->shells = g_list_remove(private->shells, shell);

  gtk_container_remove(GTK_CONTAINER(private->notebook), GTK_WIDGET(shell));
}
...
private
  ...
  page_num =
      gtk_notebook_page_num(GTK_NOTEBOOK(private->notebook), GTK_WIDGET(shell));
...
void gimp_image_window_shrink_wrap(GimpImageWindow *window,
  ...
  if (!gimp_display_shell_get_infinite_canvas(active_shell)) {
    gimp_display_shell_scale_get_image_size(active_shell, &width, &height);
  } else {
    gimp_display_shell_scale_get_image_bounding_box(active_shell, NULL, NULL,
                                                    &width, &height);
  }
  ...
  if (((width + border_width) < rect.width ||
    ...
    height =
        ((height + border_height) < rect.height) ? height : max_auto_height;
  ...
  if (resize) {
    ...
    gtk_widget_get_size_request(GTK_WIDGET(statusbar), &statusbar_width, NULL);
...
gimp_image_window_get_first_dockbook(GimpDockColumns *columns) {
  ...
  for (dock_iter = gimp_dock_columns_get_docks(columns); dock_iter;
...
private
  ...
  if (gtk_widget_translate_coordinates(GTK_WIDGET(shell->canvas),
                                       GTK_WIDGET(window), canvas_x, canvas_y,
                                       &window_x, &window_y)) {
    ...
    g_signal_connect_data(shell, "size-allocate",
                          ...
                          data, (GClosureNotify)g_free, G_CONNECT_AFTER);
    /*
...
private
  ...
  gtk_notebook_set_show_tabs(
      GTK_NOTEBOOK(private->notebook),
      config->single_window_mode && config->show_tabs && !config->hide_docks &&
          ((private->active_shell && private->active_shell->display &&
...
static void gimp_image_window_show_tooltip(GimpUIManager *manager,
  ...
  gimp_statusbar_push(statusbar, "menu-tooltip", NULL, "%s", tooltip);
}
...
static void gimp_image_window_config_notify(GimpImageWindow *window,
  ...
  if (strcmp(pspec->name, "single-window-mode") == 0 ||
    ...
    if (strcmp(pspec->name, "single-window-mode") == 0 ||
        strcmp(pspec->name, "hide-docks") == 0) {
      gboolean show_docks = (config->single_window_mode && !config->hide_docks);
      ...
      if (strcmp(pspec->name, "hide-docks") == 0 &&
          !config->single_window_mode && !config->hide_docks) {
        GimpDisplayShell *shell;
...
static void gimp_image_window_update_ui_manager(GimpImageWindow *window) {
  ...
  if (!private->update_ui_manager_idle_id) {
  private
    ->update_ui_manager_idle_id = g_idle_add_full(
        GIMP_PRIORITY_IMAGE_WINDOW_UPDATE_UI_MANAGER_IDLE,
        (GSourceFunc)gimp_image_window_update_ui_manager_idle, window, NULL);
  }
}
...
static void gimp_image_window_shell_size_allocate(GimpDisplayShell *shell,
  ...
  if (gtk_widget_translate_coordinates(
          GTK_WIDGET(shell->canvas), GTK_WIDGET(window), data->canvas_x,
          data->canvas_y, &new_window_x, &new_window_y)) {
  ...
  g_signal_handlers_disconnect_by_func(
      shell, gimp_image_window_shell_size_allocate, data);
}
...
private
  ...
  gimp_window_set_primary_focus_widget(GIMP_WINDOW(window), shell->canvas);

  active_display = private->active_shell->display;

  g_signal_connect(active_display, "notify::image",
                   G_CALLBACK(gimp_image_window_image_notify), window);

  g_signal_connect(private->active_shell, "scaled",
                   G_CALLBACK(gimp_image_window_shell_scaled), window);
  g_signal_connect(private->active_shell, "rotated",
                   G_CALLBACK(gimp_image_window_shell_rotated), window);
  g_signal_connect(private->active_shell, "notify::title",
                   G_CALLBACK(gimp_image_window_shell_title_notify), window);
  g_signal_connect(private->active_shell, "notify::icon",
                   G_CALLBACK(gimp_image_window_shell_icon_notify), window);
  ...
  if (gtk_widget_get_window(GTK_WIDGET(window))) {
     ...
     */
    gimp_image_window_session_update(
        window, active_display, NULL /*new_entry_id*/,
        gimp_widget_get_monitor(GTK_WIDGET(window)));
  } else {
...
static void gimp_image_window_page_reordered(GtkNotebook *notebook,
  ...
  if (index != page_num) {
  private
    ->shells = g_list_remove(private->shells, widget);
  private
    ->shells = g_list_insert(private->shells, widget, page_num);
  }
  ...
  gimp_container_reorder(
      displays, GIMP_OBJECT(GIMP_DISPLAY_SHELL(widget)->display), page_num);
...
gimp_image_window_disconnect_from_active_shell(GimpImageWindow *window) {
  ...
  if (active_display)
    g_signal_handlers_disconnect_by_func(
        active_display, gimp_image_window_image_notify, window);

  g_signal_handlers_disconnect_by_func(private->active_shell,
                                       gimp_image_window_shell_scaled, window);
  g_signal_handlers_disconnect_by_func(private->active_shell,
                                       gimp_image_window_shell_rotated, window);
  g_signal_handlers_disconnect_by_func(
      private->active_shell, gimp_image_window_shell_title_notify, window);
  g_signal_handlers_disconnect_by_func(
      private->active_shell, gimp_image_window_shell_icon_notify, window);
...
static void gimp_image_window_image_notify(GimpDisplay *display,
  ...
  gimp_image_window_session_update(window, display, NULL /*new_entry_id*/,
                                   gimp_widget_get_monitor(GTK_WIDGET(window)));

  tab_label =
      gtk_notebook_get_tab_label(GTK_NOTEBOOK(private->notebook),
...
static void gimp_image_window_session_clear(GimpImageWindow *window) {
  ...
  if (gimp_dialog_factory_from_widget(widget, NULL))
    gimp_dialog_factory_remove_dialog(private->dialog_factory, widget);
}
...
static void gimp_image_window_session_apply(GimpImageWindow *window,
  ...
  gimp_dialog_factory_add_foreign(private->dialog_factory, entry_id,
                                  GTK_WIDGET(window), monitor);
...
static void gimp_image_window_session_update(GimpImageWindow *window,
  ...
  if (new_entry_id) {
    if (!private->entry_id) {
      ...
      if (strcmp(new_entry_id, GIMP_EMPTY_IMAGE_WINDOW_ENTRY_ID) == 0) {
         ...
         */
        if (private->active_shell && private->active_shell->display &&
            !gimp_display_get_image(private->active_shell->display) &&
            g_list_length(private->shells) <= 1) {
          gimp_image_window_session_apply(window, new_entry_id, monitor);
        }
  ...
  if (new_display &&
    ...
    if (gimp_display_get_image(new_display)) {
       ...
       */
      gimp_image_window_session_apply(window, private->entry_id, monitor);
    }
  }
}
...
gimp_image_window_config_to_entry_id(GimpGuiConfig *config) {
  return (config->single_window_mode ? GIMP_SINGLE_IMAGE_WINDOW_ENTRY_ID
...
static GtkWidget *gimp_image_window_create_tab_label(GimpImageWindow *window,
  ...
  gimp_view_renderer_set_color_config(
      GIMP_VIEW(view)->renderer, gimp_display_shell_get_color_config(shell));
  ...
  gtk_image = gtk_image_new_from_icon_name(GIMP_ICON_CLOSE, GTK_ICON_SIZE_MENU);
  gtk_container_add(GTK_CONTAINER(button), gtk_image);
  gtk_widget_show(gtk_image);

  g_signal_connect_swapped(
      button, "clicked",
      G_CALLBACK(gimp_image_window_shell_close_button_callback), shell);
...
static void gimp_image_window_update_tab_labels(GimpImageWindow *window) {
  ...
  for (list = children; list; list = g_list_next(list)) {
    ...
    tab_widget =
        gtk_notebook_get_tab_label(GTK_NOTEBOOK(private->notebook), shell);
