...
enum { PROP_0, PROP_GROUP_STROKING, PROP_GROUP_FILLING };
...
static void gimp_canvas_group_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_canvas_group_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);
static void gimp_canvas_group_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *gimp_canvas_group_get_extents(GimpCanvasItem *item);
static gboolean gimp_canvas_group_hit(GimpCanvasItem *item, gdouble x,
                                      gdouble y);
...
static void gimp_canvas_group_class_init(GimpCanvasGroupClass *klass) {
  ...
  g_object_class_install_property(object_class, PROP_GROUP_STROKING,
                                  g_param_spec_boolean("group-stroking", NULL,
                                                       NULL, FALSE,
                                                       GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_GROUP_FILLING,
                                  g_param_spec_boolean("group-filling", NULL,
                                                       NULL, FALSE,
                                                       GIMP_PARAM_READWRITE));
}

static void gimp_canvas_group_init(GimpCanvasGroup *group) {
...
GimpCanvasItem *gimp_canvas_group_new(GimpDisplayShell *shell) {
  ...
  return g_object_new(GIMP_TYPE_CANVAS_GROUP, "shell", shell, NULL);
}
...
void gimp_canvas_group_add_item(GimpCanvasGroup *group, GimpCanvasItem *item) {
  ...
  g_signal_connect(item, "update", G_CALLBACK(gimp_canvas_group_child_update),
                   group);
}
...
void gimp_canvas_group_remove_item(GimpCanvasGroup *group,
  ...
  g_signal_handlers_disconnect_by_func(item, gimp_canvas_group_child_update,
                                       group);
...
void gimp_canvas_group_set_group_stroking(GimpCanvasGroup *group,
  ...
  if (group->priv->group_stroking != group_stroking) {
    ...
    g_object_set(group, "group-stroking", group_stroking ? TRUE : FALSE, NULL);
...
void gimp_canvas_group_set_group_filling(GimpCanvasGroup *group,
  ...
  if (group->priv->group_filling != group_filling) {
    ...
    g_object_set(group, "group-filling", group_filling ? TRUE : FALSE, NULL);
