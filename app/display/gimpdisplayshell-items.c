...
void gimp_display_shell_items_init(GimpDisplayShell *shell) {
  ...
  g_signal_connect(shell->canvas_item, "update",
                   G_CALLBACK(gimp_display_shell_item_update), shell);
  ...
  g_signal_connect(shell->unrotated_item, "update",
                   G_CALLBACK(gimp_display_shell_unrotated_item_update), shell);
}
...
void gimp_display_shell_items_free(GimpDisplayShell *shell) {
  ...
  if (shell->canvas_item) {
    g_signal_handlers_disconnect_by_func(shell->canvas_item,
                                         gimp_display_shell_item_update, shell);
  ...
  if (shell->unrotated_item) {
    g_signal_handlers_disconnect_by_func(
        shell->unrotated_item, gimp_display_shell_unrotated_item_update, shell);
  ...
  if (shell->rotate_transform) {
    ...
    for (i = 0; i < n_rects; i++) {
      ...
      gimp_display_shell_rotate_bounds(
          shell, rect.x, rect.y, rect.x + rect.width, rect.y + rect.height,
          &tx1, &ty1, &tx2, &ty2);
