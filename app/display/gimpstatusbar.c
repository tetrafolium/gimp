...
static GimpProgress *gimp_statusbar_progress_start(GimpProgress *progress,
...
static gboolean gimp_statusbar_label_draw(GtkWidget *widget, cairo_t *cr,
                                          GimpStatusbar *statusbar);
...
static void gimp_statusbar_add_message(GimpStatusbar *statusbar,
                                       guint context_id, const gchar *icon_name,
...
static gchar *gimp_statusbar_vprintf(const gchar *format, va_list args)
    G_GNUC_PRINTF(1, 0);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpStatusbar, gimp_statusbar, GTK_TYPE_FRAME,
...
static void gimp_statusbar_init(GimpStatusbar *statusbar) {
  ...
  g_signal_connect(statusbar->unit_combo, "changed",
                   G_CALLBACK(gimp_statusbar_unit_changed), statusbar);
  ...
  g_signal_connect(statusbar->scale_combo, "changed",
                   G_CALLBACK(gimp_statusbar_scale_changed), statusbar);

  g_signal_connect(statusbar->scale_combo, "entry-activated",
                   G_CALLBACK(gimp_statusbar_scale_activated), statusbar);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), statusbar->rotate_widget, FALSE, FALSE, 1);
  ...
  g_signal_connect(statusbar->horizontal_flip_icon, "button-press-event",
                   G_CALLBACK(gimp_statusbar_horiz_flip_pressed), statusbar);

  statusbar->vertical_flip_icon = gtk_event_box_new();
  gimp_statusbar_add_size_widget(statusbar, statusbar->vertical_flip_icon);
  gtk_box_pack_start(GTK_BOX(hbox), statusbar->vertical_flip_icon, FALSE, FALSE,
                     1);
  ...
  g_signal_connect(statusbar->vertical_flip_icon, "button-press-event",
                   G_CALLBACK(gimp_statusbar_vert_flip_pressed), statusbar);
  ...
  g_signal_connect_after(statusbar->label, "draw",
                         G_CALLBACK(gimp_statusbar_label_draw), statusbar);

  statusbar->progressbar =
      g_object_new(GTK_TYPE_PROGRESS_BAR, "show-text", TRUE, "ellipsize",
  ...
  gtk_button_set_relief(GTK_BUTTON(statusbar->cancel_button), GTK_RELIEF_NONE);
  gtk_widget_set_sensitive(statusbar->cancel_button, FALSE);
  gimp_statusbar_add_size_widget(statusbar, statusbar->cancel_button);
  gtk_box_pack_end(GTK_BOX(hbox), statusbar->cancel_button, FALSE, FALSE, 0);
  ...
  g_signal_connect(statusbar->cancel_button, "clicked",
                   G_CALLBACK(gimp_statusbar_progress_canceled), statusbar);
  ...
  if (!statusbar->progress_active) {
    ...
    if (cancellable) {
      if (message) {
        ...
        gimp_help_set_help_data_with_markup(statusbar->cancel_button, tooltip,
                                            NULL);
        g_free(tooltip);
      }
...
static void gimp_statusbar_set_text(GimpStatusbar *statusbar,
...
void gimp_statusbar_set_shell(GimpStatusbar *statusbar,
  ...
  if (statusbar->shell) {
    g_signal_handlers_disconnect_by_func(
        statusbar->shell, gimp_statusbar_shell_scaled, statusbar);
    g_signal_handlers_disconnect_by_func(
        statusbar->shell, gimp_statusbar_shell_rotated, statusbar);
    g_signal_handlers_disconnect_by_func(
        statusbar->shell, gimp_statusbar_shell_status_notify, statusbar);
  }

  statusbar->shell = shell;

  g_signal_connect_object(statusbar->shell, "scaled",
                          G_CALLBACK(gimp_statusbar_shell_scaled), statusbar,
                          0);
  g_signal_connect_object(statusbar->shell, "rotated",
                          G_CALLBACK(gimp_statusbar_shell_rotated), statusbar,
                          0);
...
void gimp_statusbar_set_visible(GimpStatusbar *statusbar, gboolean visible) {
  g_return_if_fail(GIMP_IS_STATUSBAR(statusbar));
...
void gimp_statusbar_push(GimpStatusbar *statusbar, const gchar *context,
                         const gchar *icon_name, const gchar *format, ...) {
  va_list args;
...
void gimp_statusbar_push_valist(GimpStatusbar *statusbar, const gchar *context,
                                const gchar *icon_name, const gchar *format,
                                va_list args) {
  guint context_id;
  ...
  gimp_statusbar_add_message(statusbar, context_id, icon_name, format, args,
                             /*  move_to_front =  */ TRUE);
}

void gimp_statusbar_push_coords(GimpStatusbar *statusbar, const gchar *context,
                                const gchar *icon_name,
  ...
  if (shell->unit == GIMP_UNIT_PIXEL) {
    if (precision == GIMP_CURSOR_PRECISION_SUBPIXEL) {
      gimp_statusbar_push(statusbar, context, icon_name,
                          statusbar->cursor_format_str_f, title, x, separator,
                          y, help);
    } else {
      gimp_statusbar_push(statusbar, context, icon_name,
                          statusbar->cursor_format_str, title, (gint)RINT(x),
                          separator, (gint)RINT(y), help);
    }
  } else /* show real world units */
  {
    ...
    gimp_image_get_resolution(gimp_display_get_image(shell->display), &xres,
                              &yres);

    gimp_statusbar_push(statusbar, context, icon_name,
                        statusbar->cursor_format_str, title,
                        gimp_pixels_to_units(x, shell->unit, xres), separator,
                        gimp_pixels_to_units(y, shell->unit, yres), help);
  }
}

void gimp_statusbar_push_length(GimpStatusbar *statusbar, const gchar *context,
                                const gchar *icon_name, const gchar *title,
                                GimpOrientationType axis, gdouble value,
                                const gchar *help) {
  GimpDisplayShell *shell;
  ...
  if (shell->unit == GIMP_UNIT_PIXEL) {
    gimp_statusbar_push(statusbar, context, icon_name,
                        statusbar->length_format_str, title, (gint)RINT(value),
                        help);
  } else /* show real world units */
  {
    gdouble xres;
    ...
    gimp_statusbar_push(
        statusbar, context, icon_name, statusbar->length_format_str, title,
        gimp_pixels_to_units(value, shell->unit, resolution), help);
  }
}

void gimp_statusbar_replace(GimpStatusbar *statusbar, const gchar *context,
                            const gchar *icon_name, const gchar *format, ...) {
  va_list args;
...
void gimp_statusbar_replace_valist(GimpStatusbar *statusbar,
                                   const gchar *context, const gchar *icon_name,
                                   const gchar *format, va_list args) {
  guint context_id;
...
void gimp_statusbar_pop(GimpStatusbar *statusbar, const gchar *context) {
  guint context_id;
  ...
  gimp_statusbar_remove_message(statusbar, context_id);
}
...
void gimp_statusbar_push_temp_valist(GimpStatusbar *statusbar,
  ...
  statusbar->temp_timeout_id = g_timeout_add(
      MESSAGE_TIMEOUT, (GSourceFunc)gimp_statusbar_temp_timeout, statusbar);

  statusbar->temp_severity = severity;

  gimp_statusbar_add_message(statusbar, statusbar->temp_context_id, icon_name,
                             format, args,
                             /*  move_to_front =  */ TRUE);
...
void gimp_statusbar_pop_temp(GimpStatusbar *statusbar) {
  ...
  if (statusbar->temp_timeout_id) {
    ...
    gimp_statusbar_remove_message(statusbar, statusbar->temp_context_id);
  }
}

void gimp_statusbar_update_cursor(GimpStatusbar *statusbar,
                                  GimpCursorPrecision precision, gdouble x,
                                  gdouble y) {
  GimpDisplayShell *shell;
  GimpImage *image;
  ...
  if (!image || x < 0 || y < 0 || x >= gimp_image_get_width(image) ||
  ...
  if (shell->unit == GIMP_UNIT_PIXEL) {
    if (precision == GIMP_CURSOR_PRECISION_SUBPIXEL) {
      g_snprintf(buffer, sizeof(buffer), statusbar->cursor_format_str_f, "", x,
                 ", ", y, "");
    } else {
  ...
  } else /* show real world units */
  {
    ...
    g_snprintf(buffer, sizeof(buffer), statusbar->cursor_format_str, "", x,
               ", ", y, "");
  }
...
static gboolean gimp_statusbar_label_draw(GtkWidget *widget, cairo_t *cr,
  ...
  if (statusbar->icon) {
    ...
    x += PANGO_PIXELS(rect.x) + (rect.width < 0 ? PANGO_PIXELS(rect.width) : 0);
...
static void gimp_statusbar_shell_scaled(GimpDisplayShell *shell,
  ...
  model = gtk_combo_box_get_model(GTK_COMBO_BOX(statusbar->unit_combo));
  gimp_unit_store_set_resolutions(GIMP_UNIT_STORE(model), image_xres,
                                  image_yres);
  ...
  if (shell->unit == GIMP_UNIT_PIXEL) {
    g_snprintf(statusbar->cursor_format_str,
               sizeof(statusbar->cursor_format_str), "%%s%%d%%s%%d%%s");
    g_snprintf(statusbar->cursor_format_str_f,
               sizeof(statusbar->cursor_format_str_f), "%%s%%.1f%%s%%.1f%%s");
    g_snprintf(statusbar->length_format_str,
               sizeof(statusbar->length_format_str), "%%s%%d%%s");
  } else /* show real world units */
  {
    gint w_digits;
    gint h_digits;
    ...
    g_snprintf(statusbar->cursor_format_str,
               sizeof(statusbar->cursor_format_str), "%%s%%.%df%%s%%.%df%%s",
               w_digits, h_digits);
    strcpy(statusbar->cursor_format_str_f, statusbar->cursor_format_str);
    g_snprintf(statusbar->length_format_str,
               sizeof(statusbar->length_format_str), "%%s%%.%df%%s",
               MAX(w_digits, h_digits));
  }
...
static void gimp_statusbar_shell_status_notify(GimpDisplayShell *shell,
                                               const GParamSpec *pspec,
                                               GimpStatusbar *statusbar) {
  gimp_statusbar_replace(statusbar, "title", NULL, "%s", shell->status);
}
...
static void gimp_statusbar_scale_changed(GimpScaleComboBox *combo,
                                         GimpStatusbar *statusbar) {
  gimp_display_shell_scale(statusbar->shell, GIMP_ZOOM_TO,
...
static guint gimp_statusbar_get_context_id(GimpStatusbar *statusbar,
                                           const gchar *context) {
  guint id =
      GPOINTER_TO_UINT(g_hash_table_lookup(statusbar->context_ids, context));

  if (!id) {
    id = statusbar->seq_context_id++;

    g_hash_table_insert(statusbar->context_ids, g_strdup(context),
                        GUINT_TO_POINTER(id));
  }
...
static void gimp_statusbar_add_message(GimpStatusbar *statusbar,
  ...
  if (!(statusbar->progress_active &&
    ...
    if (statusbar->temp_timeout_id && context_id != statusbar->temp_context_id)
      position++;
  }
...
static GdkPixbuf *gimp_statusbar_load_icon(GimpStatusbar *statusbar,
  ...
  g_hash_table_insert(statusbar->icon_hash, g_strdup(icon_name),
                      g_object_ref(icon));
