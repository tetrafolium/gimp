...
typedef struct _GimpCanvasTransformGuidesPrivate
...
#define GET_PRIVATE(transform)                                                 \
  ((GimpCanvasTransformGuidesPrivate *)                                        \
       gimp_canvas_transform_guides_get_instance_private(                      \
...
gimp_canvas_transform_guides_class_init(GimpCanvasTransformGuidesClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("type", NULL, NULL, GIMP_TYPE_GUIDES_TYPE,
                        GIMP_GUIDES_NONE, GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_N_GUIDES,
                                  g_param_spec_int("n-guides", NULL, NULL, 1,
                                                   128, 4,
                                                   GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_CLIP,
      g_param_spec_boolean("clip", NULL, NULL, FALSE, GIMP_PARAM_READWRITE));
}
...
static void gimp_canvas_transform_guides_set_property(GObject *object,
  ...
  case PROP_CLIP:
  private
    ->clip = g_value_get_boolean(value);
    break;
...
static gboolean gimp_canvas_transform_guides_transform(GimpCanvasItem *item,
  ...
  if (private->clip) {
    gimp_transform_polygon(&private->transform, vertices, 4, TRUE, t_vertices,
                           n_t_vertices);
    ...
    for (i = 0; i < 4; i++) {
      gimp_matrix3_transform_point(&private->transform, vertices[i].x,
                                   vertices[i].y, &t_vertices[i].x,
                                   &t_vertices[i].y);
    }
    ...
    *n_t_vertices = 4;
    ...
    return gimp_transform_polygon_is_convex(
        t_vertices[0].x, t_vertices[0].y, t_vertices[1].x, t_vertices[1].y,
        t_vertices[3].x, t_vertices[3].y, t_vertices[2].x, t_vertices[2].y);
  }
}

static void draw_line(cairo_t *cr, GimpCanvasItem *item, GimpMatrix3 *transform,
  ...
  if (private->clip) {
    gimp_transform_polygon(transform, vertices, 2, FALSE, t_vertices,
                           &n_t_vertices);
  } else {
    gint i;

    for (i = 0; i < 2; i++) {
      gimp_matrix3_transform_point(transform, vertices[i].x, vertices[i].y,
...
static void draw_hline(cairo_t *cr, GimpCanvasItem *item,
                       GimpMatrix3 *transform, gdouble x1, gdouble x2,
                       gdouble y) {
  draw_line(cr, item, transform, x1, y, x2, y);
}

static void draw_vline(cairo_t *cr, GimpCanvasItem *item,
                       GimpMatrix3 *transform, gdouble y1, gdouble y2,
                       gdouble x) {
...
static void gimp_canvas_transform_guides_draw(GimpCanvasItem *item,
  ...
  case GIMP_GUIDES_CENTER_LINES:
    draw_hline(cr, item, &private->transform, private->x1, private->x2,
               (private->y1 + private->y2) / 2);
    draw_vline(cr, item, &private->transform, private->y1, private->y2,
               (private->x1 + private->x2) / 2);
    break;

  case GIMP_GUIDES_THIRDS:
    draw_hline(cr, item, &private->transform, private->x1, private->x2,
               (2 * private->y1 + private->y2) / 3);
    draw_hline(cr, item, &private->transform, private->x1, private->x2,
               (private->y1 + 2 * private->y2) / 3);

    draw_vline(cr, item, &private->transform, private->y1, private->y2,
               (2 * private->x1 + private->x2) / 3);
    draw_vline(cr, item, &private->transform, private->y1, private->y2,
               (private->x1 + 2 * private->x2) / 3);
    break;
  ...
  case GIMP_GUIDES_DIAGONALS: {
     ...
     */
    const gdouble square_side =
        MIN(private->x2 - private->x1, private->y2 - private->y1);

    /* diagonal from the top-left edge */
    draw_line(cr, item, &private->transform, private->x1, private->y1,
              private->x1 + square_side, private->y1 + square_side);

    /* diagonal from the top-right edge */
    draw_line(cr, item, &private->transform, private->x2, private->y1,
              private->x2 - square_side, private->y1 + square_side);

    /* diagonal from the bottom-left edge */
    draw_line(cr, item, &private->transform, private->x1, private->y2,
              private->x1 + square_side, private->y2 - square_side);

    /* diagonal from the bottom-right edge */
    draw_line(cr, item, &private->transform, private->x2, private->y2,
              private->x2 - square_side, private->y2 - square_side);
  } break;
  ...
  case GIMP_GUIDES_SPACING: {
    ...
    for (i = 1; i <= ngx; i++) {
      gdouble x = private->x1 +
                  (((gdouble)i) / (ngx + 1) * (private->x2 - private->x1));

      draw_line(cr, item, &private->transform, x, private->y1, x, private->y2);
    }

    for (i = 1; i <= ngy; i++) {
      gdouble y = private->y1 +
                  (((gdouble)i) / (ngy + 1) * (private->y2 - private->y1));

      draw_line(cr, item, &private->transform, private->x1, y, private->x2, y);
    }
  }
  }
...
GimpCanvasItem *gimp_canvas_transform_guides_new(GimpDisplayShell *shell,
                                                 ...
                                                 GimpGuidesType type,
                                                 gint n_guides, gboolean clip) {
  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), NULL);

  return g_object_new(GIMP_TYPE_CANVAS_TRANSFORM_GUIDES, "shell", shell,
                      "transform", transform, "x1", x1, "y1", y1, "x2", x2,
                      "y2", y2, "type", type, "n-guides", n_guides, "clip",
                      clip, NULL);
}
...
void gimp_canvas_transform_guides_set(GimpCanvasItem *guides,
                                      ...
                                      GimpGuidesType type, gint n_guides,
                                      gboolean clip) {
  g_return_if_fail(GIMP_IS_CANVAS_TRANSFORM_GUIDES(guides));

  gimp_canvas_item_begin_change(guides);

  g_object_set(guides, "transform", transform, "x1", x1, "y1", y1, "x2", x2,
               "y2", y2, "type", type, "n-guides", n_guides, "clip", clip,
               NULL);
