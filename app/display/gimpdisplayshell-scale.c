...
static void gimp_display_shell_scale_get_image_size_for_scale(
    GimpDisplayShell *shell, gdouble scale, gint *w, gint *h);
static void gimp_display_shell_calculate_scale_x_and_y(GimpDisplayShell *shell,
                                                       gdouble scale,
                                                       gdouble *scale_x,
                                                       gdouble *scale_y);

static void gimp_display_shell_scale_to(GimpDisplayShell *shell, gdouble scale,
...
static gboolean gimp_display_shell_scale_image_starts_to_fit(
    GimpDisplayShell *shell, gdouble new_scale, gdouble current_scale,
    gboolean *vertically, gboolean *horizontally);
static gboolean gimp_display_shell_scale_viewport_coord_almost_centered(
    GimpDisplayShell *shell, gint x, gint y, gboolean *horizontally,
    gboolean *vertically);

static void gimp_display_shell_scale_get_image_center_viewport(
    GimpDisplayShell *shell, gint *image_center_x, gint *image_center_y);
...
gboolean gimp_display_shell_scale_revert(GimpDisplayShell *shell) {
  ...
  gimp_display_shell_scale_by_values(shell, shell->last_scale,
                                     shell->last_offset_x, shell->last_offset_y,
 ...
 *
 **/
void gimp_display_shell_scale_get_image_size(GimpDisplayShell *shell, gint *w,
                                             gint *h) {
  g_return_if_fail(GIMP_IS_DISPLAY_SHELL(shell));

  gimp_display_shell_scale_get_image_size_for_scale(
      shell, gimp_zoom_model_get_factor(shell->zoom), w, h);
}
 ...
 **/
void gimp_display_shell_scale_get_image_bounds(GimpDisplayShell *shell, gint *x,
  ...
  gimp_display_shell_transform_bounds(shell, 0, 0, gimp_image_get_width(image),
                                      gimp_image_get_height(image), &x1, &y1,
                                      &x2, &y2);
...
void gimp_display_shell_scale_get_image_unrotated_bounds(
  ...
  if (y)
    *y = -shell->offset_y;
  if (w)
    *w = floor(gimp_image_get_width(image) * shell->scale_x);
  if (h)
    *h = floor(gimp_image_get_height(image) * shell->scale_y);
}
 ...
 * been transformed (i.e., scaled, rotated, and scrolled).
 **/
void gimp_display_shell_scale_get_image_bounding_box(GimpDisplayShell *shell,
  ...
  gimp_display_shell_transform_bounds(shell, bounding_box.x, bounding_box.y,
                                      bounding_box.x + bounding_box.width,
                                      bounding_box.y + bounding_box.height, &x1,
                                      &y1, &x2, &y2);
...
void gimp_display_shell_scale_get_image_unrotated_bounding_box(
  ...
  x1 = bounding_box.x * shell->scale_x - shell->offset_x;
  y1 = bounding_box.y * shell->scale_y - shell->offset_y;

  x2 = (bounding_box.x + bounding_box.width) * shell->scale_x - shell->offset_x;
  y2 =
      (bounding_box.y + bounding_box.height) * shell->scale_y - shell->offset_y;
...
gboolean gimp_display_shell_scale_image_is_within_viewport(
    GimpDisplayShell *shell, gboolean *horizontally, gboolean *vertically) {
  gboolean horizontally_dummy, vertically_dummy;

  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), FALSE);

  if (!horizontally)
    horizontally = &horizontally_dummy;
  if (!vertically)
  ...
  if (!gimp_display_shell_get_infinite_canvas(shell)) {
    ...
    gimp_display_shell_scale_get_image_bounding_box(shell, &sx, &sy, &sw, &sh);
 ...
 *
 **/
void gimp_display_shell_scale(GimpDisplayShell *shell, GimpZoomType zoom_type,
                              gdouble new_scale, GimpZoomFocus zoom_focus) {
  ...
  if (resize_window) {
    ...
    gimp_display_shell_scale_get_zoom_focus(shell, new_scale, current_scale, &x,
                                            &y, zoom_focus);
    gimp_display_shell_scale_get_image_center_viewport(shell, &image_center_x,
                                                       &image_center_y);
    ...
    if (zoom_focus != GIMP_ZOOM_FOCUS_POINTER) {
      ...
      gimp_display_shell_scale_image_starts_to_fit(
          shell, new_scale, current_scale, &starts_fitting_horiz,
          &starts_fitting_vert);

      gimp_display_shell_scale_viewport_coord_almost_centered(
          shell, x, y, &zoom_focus_almost_centered_horiz,
          &zoom_focus_almost_centered_vert);
      gimp_display_shell_scale_viewport_coord_almost_centered(
          shell, image_center_x, image_center_y,
          &image_center_almost_centered_horiz,
          &image_center_almost_centered_vert);

      gimp_display_shell_scroll_center_image(
          shell,
          starts_fitting_horiz || (zoom_focus_almost_centered_horiz &&
                                   image_center_almost_centered_horiz),
          starts_fitting_vert || (zoom_focus_almost_centered_vert &&
                                  image_center_almost_centered_vert));
    }
  }
}
...
void gimp_display_shell_scale_to_rectangle(GimpDisplayShell *shell,
                                           GimpZoomType zoom_type, gdouble x,
  ...
  gimp_display_shell_transform_bounds(shell, x, y, x + width, y + height, &x,
                                      &y, &width, &height);
  ...
  switch (zoom_type) {
  case GIMP_ZOOM_IN:
    factor = MIN((shell->disp_width / width), (shell->disp_height / height));
    break;

  case GIMP_ZOOM_OUT:
    factor = MAX((width / shell->disp_width), (height / shell->disp_height));
    break;
  ...
  if (new_scale != current_scale || offset_x != shell->offset_x ||
      offset_y != shell->offset_y) {
    gimp_display_shell_scale_by_values(shell, new_scale, offset_x, offset_y,
                                       resize_window);
  }
}
 ...
 **/
void gimp_display_shell_scale_by_values(GimpDisplayShell *shell, gdouble scale,
  ...
  if (SCALE_EQUALS(gimp_zoom_model_get_factor(shell->zoom), scale) &&
      shell->offset_x == offset_x && shell->offset_y == offset_y)
    return;
...
void gimp_display_shell_scale_drag(GimpDisplayShell *shell, gdouble start_x,
                                   gdouble start_y, gdouble delta_x,
                                   gdouble delta_y) {
  gdouble scale;
  ...
  if (delta_y > 0) {
    gimp_display_shell_scale(shell, GIMP_ZOOM_TO, scale * 1.1,
                             GIMP_ZOOM_FOCUS_POINTER);
  } else if (delta_y < 0) {
    gimp_display_shell_scale(shell, GIMP_ZOOM_TO, scale * 0.9,
                             GIMP_ZOOM_FOCUS_POINTER);
  }
}
...
void gimp_display_shell_set_initial_scale(GimpDisplayShell *shell,
                                          gdouble scale, gint *display_width,
                                          gint *display_height) {
  GimpImage *image;
  ...
  if (shell->display->config->initial_zoom_to_fit) {
    ...
    if (shell_width > monitor_width || shell_height > monitor_height) {
      ...
      gimp_zoom_model_zoom(
          shell->zoom, GIMP_ZOOM_TO,
          gimp_zoom_model_zoom_step(GIMP_ZOOM_IN, new_scale, 0.0));
...
void gimp_display_shell_get_rotated_scale(GimpDisplayShell *shell,
  ...
  if (shell->rotate_angle == 0.0 || shell->scale_x == shell->scale_y) {
    if (scale_x)
      *scale_x = shell->scale_x;
    if (scale_y)
      *scale_y = shell->scale_y;
  } else {
    ...
    if (scale_x)
      *scale_x =
          1.0 / sqrt(SQR(cos_a / shell->scale_x) + SQR(sin_a / shell->scale_y));

    if (scale_y)
      *scale_y =
          1.0 / sqrt(SQR(cos_a / shell->scale_y) + SQR(sin_a / shell->scale_x));
  }
}
...
void gimp_display_shell_push_zoom_focus_pointer_pos(GimpDisplayShell *shell,
  ...
  g_queue_push_head(shell->zoom_focus_pointer_queue, point);
}
...
gimp_display_shell_scale_get_screen_resolution(GimpDisplayShell *shell,
  ...
  if (shell->dot_for_dot) {
    gimp_image_get_resolution(gimp_display_get_image(shell->display), &x, &y);
  } else {
...
static void gimp_display_shell_scale_get_image_size_for_scale(
  ...
  if (w)
    *w = scale_x * gimp_image_get_width(image);
  if (h)
    *h = scale_y * gimp_image_get_height(image);
}
...
static void gimp_display_shell_calculate_scale_x_and_y(GimpDisplayShell *shell,
  ...
  gimp_display_shell_scale_get_screen_resolution(shell, &screen_xres,
                                                 &screen_yres);

  if (scale_x)
    *scale_x = scale * screen_xres / xres;
  if (scale_y)
    *scale_y = scale * screen_yres / yres;
}
...
static void gimp_display_shell_scale_to(GimpDisplayShell *shell, gdouble scale,
  ...
  gimp_display_shell_untransform_xy_f(shell, viewport_x, viewport_y, &image_x,
                                      &image_y);

  /* Note that we never come here if we need to resize_windows_on_zoom
   */
  gimp_display_shell_scale_by_values(shell, scale, shell->offset_x,
                                     shell->offset_y, FALSE);

  gimp_display_shell_transform_xy_f(shell, image_x, image_y, &new_viewport_x,
                                    &new_viewport_y);

  gimp_display_shell_scroll(shell, new_viewport_x - viewport_x,
...
static void gimp_display_shell_scale_fit_or_fill(GimpDisplayShell *shell,
  ...
  gimp_display_shell_transform_bounds(
      shell, bounding_box.x, bounding_box.y,
  ...
  if (fill) {
    zoom_factor =
        MAX(shell->disp_width / image_width, shell->disp_height / image_height);
  } else {
    zoom_factor =
        MIN(shell->disp_width / image_width, shell->disp_height / image_height);
  }

  gimp_display_shell_scale(shell, GIMP_ZOOM_TO, zoom_factor * current_scale,
...
static gboolean gimp_display_shell_scale_image_starts_to_fit(
  ...
  if (new_scale > current_scale ||
    ...
    *horizontally = FALSE;
    ...
    gimp_display_shell_scale_get_image_size_for_scale(
        shell, current_scale, &current_scale_width, &current_scale_height);

    gimp_display_shell_scale_get_image_size_for_scale(
        shell, new_scale, &new_scale_width, &new_scale_height);
...
static gboolean gimp_display_shell_scale_image_stops_to_fit(
    GimpDisplayShell *shell, gdouble new_scale, gdouble current_scale,
    gboolean *vertically, gboolean *horizontally) {
  return gimp_display_shell_scale_image_starts_to_fit(
      shell, current_scale, new_scale, vertically, horizontally);
}
...
static gboolean gimp_display_shell_scale_viewport_coord_almost_centered(
  ...
  if (horizontally)
    *horizontally = local_horizontally;
  if (vertically)
    *vertically = local_vertically;
...
static void gimp_display_shell_scale_get_zoom_focus(GimpDisplayShell *shell,
  ...
  if (!event || gtk_get_event_widget(event) == shell->canvas ||
    ...
    if (point) {
      ...
      gdk_window_get_device_position(
          gtk_widget_get_window(shell->canvas), gdk_seat_get_pointer(seat),
          &canvas_pointer_x, &canvas_pointer_y, NULL);
    }

    if (canvas_pointer_x >= 0 && canvas_pointer_y >= 0 &&
  ...
  if (zoom_focus == GIMP_ZOOM_FOCUS_RETAIN_CENTERING_ELSE_BEST_GUESS) {
    if (gimp_display_shell_scale_viewport_coord_almost_centered(
            shell, image_center_x, image_center_y, NULL, NULL)) {
  ...
  default: {
    ...
    gimp_display_shell_scale_image_is_within_viewport(
        shell, &within_horizontally, &within_vertically);

    gimp_display_shell_scale_image_stops_to_fit(shell, new_scale, current_scale,
                                                &stops_horizontally,
                                                &stops_vertically);
