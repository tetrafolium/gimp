...
static void gimp_tool_rotate_grid_class_init(GimpToolRotateGridClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("angle", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                          GIMP_MAX_IMAGE_SIZE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_tool_rotate_grid_init(GimpToolRotateGrid *grid) {
...
static gint gimp_tool_rotate_grid_button_press(GimpToolWidget *widget,
  ...
  handle = GIMP_TOOL_WIDGET_CLASS(parent_class)
...
void gimp_tool_rotate_grid_motion(GimpToolWidget *widget,
                                  const GimpCoords *coords, guint32 time,
                                  GdkModifierType state) {
  GimpToolRotateGrid *grid = GIMP_TOOL_ROTATE_GRID(widget);
  GimpToolRotateGridPrivate *private = grid->private;
  ...
  if (!private->rotate_grab) {
    ...
    g_object_get(widget, "pivot-x", &old_pivot_x, "pivot-y", &old_pivot_y,
                 NULL);

    g_object_freeze_notify(G_OBJECT(widget));

    GIMP_TOOL_WIDGET_CLASS(parent_class)->motion(widget, coords, time, state);

    g_object_get(widget, "pivot-x", &pivot_x, "pivot-y", &pivot_y, NULL);

    if (old_pivot_x != pivot_x || old_pivot_y != pivot_y) {
      gimp_matrix3_identity(&transform);
      gimp_transform_matrix_rotate_center(&transform, pivot_x, pivot_y,
                                          private->angle);
  ...
  g_object_get(widget, "pivot-x", &pivot_x, "pivot-y", &pivot_y,
               "constrain-rotate", &constrain, NULL);
...
#define FIFTEEN_DEG (G_PI / 12.0)
  ...
  if (constrain) {
    angle = FIFTEEN_DEG *
            (gint)((private->real_angle + FIFTEEN_DEG / 2.0) / FIFTEEN_DEG);
  } else {
  ...
  g_object_set(widget, "transform", &transform, "angle", angle, NULL);
...
GimpToolWidget *gimp_tool_rotate_grid_new(GimpDisplayShell *shell, gdouble x1,
  ...
  return g_object_new(GIMP_TYPE_TOOL_ROTATE_GRID, "shell", shell, "transform",
                      &transform, "x1", x1, "y1", y1, "x2", x2, "y2", y2,
                      "pivot-x", pivot_x, "pivot-y", pivot_y, "angle", angle,
                      NULL);
}
