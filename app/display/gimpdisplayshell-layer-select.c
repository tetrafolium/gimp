...
static LayerSelect *layer_select_new(GimpDisplayShell *shell, GimpImage *image,
                                     GList *layers, gint view_size);
static void layer_select_destroy(LayerSelect *layer_select, GdkEvent *event);
static void layer_select_advance(LayerSelect *layer_select, gint move);
static gboolean layer_select_events(GtkWidget *widget, GdkEvent *event,
                                    LayerSelect *layer_select);

/*  public functions  */

void gimp_display_shell_layer_select_init(GimpDisplayShell *shell,
                                          GdkEvent *event, gint move) {
  LayerSelect *layer_select;
  GimpImage *image;
  GList *layers;
  ...
  status = gdk_seat_grab(
      gdk_event_get_seat(event), gtk_widget_get_window(layer_select->window),
      GDK_SEAT_CAPABILITY_KEYBOARD, FALSE, NULL, event, NULL, NULL);
...
static LayerSelect *layer_select_new(GimpDisplayShell *shell, GimpImage *image,
  ...
  gtk_widget_set_events(layer_select->window, GDK_KEY_PRESS_MASK |
                                                  GDK_KEY_RELEASE_MASK |
                                                  GDK_BUTTON_PRESS_MASK);

  g_signal_connect(layer_select->window, "event",
                   G_CALLBACK(layer_select_events), layer_select);
  ...
  /*  the view  */
  layer_select->view =
      gimp_view_new_by_types(gimp_get_user_context(image->gimp), GIMP_TYPE_VIEW,
                             GIMP_TYPE_LAYER, view_size, 1, FALSE);
  gimp_view_renderer_set_color_config(
      GIMP_VIEW(layer_select->view)->renderer,
      gimp_display_shell_get_color_config(shell));
  gimp_view_set_viewable(
      GIMP_VIEW(layer_select->view),
