...
static GdkModifierType gimp_display_shell_key_to_state(gint key);
static GdkModifierType gimp_display_shell_button_to_state(gint button);
...
static void gimp_display_shell_get_event_coords(GimpDisplayShell *shell,
                                                ...
                                                GdkModifierType *state,
                                                guint32 *time);
static void gimp_display_shell_untransform_event_coords(
    GimpDisplayShell *shell, const GimpCoords *display_coords,
    GimpCoords *image_coords, gboolean *update_software_cursor);
  ...
  case GDK_KEY_RELEASE: {
    ...
    if (kevent->state & GDK_BUTTON1_MASK) {
      if (kevent->keyval == GDK_KEY_Shift_L ||
          ...
          kevent->keyval == GDK_KEY_Alt_L || kevent->keyval == GDK_KEY_Alt_R ||
          kevent->keyval == GDK_KEY_Meta_L ||
          kevent->keyval == GDK_KEY_Meta_R || kevent->keyval == GDK_KEY_space ||
    ...
    default:
      if (shell->space_release_pending || shell->button1_release_pending ||
          shell->scrolling)
        return TRUE;
      break;
    }
...
static gboolean
gimp_display_shell_canvas_no_image_events(GtkWidget *canvas, GdkEvent *event,
                                          GimpDisplayShell *shell) {
  switch (event->type) {
  ...
  case GDK_KEY_PRESS: {
    ...
    if (kevent->keyval == GDK_KEY_Tab || kevent->keyval == GDK_KEY_KP_Tab ||
...
gboolean gimp_display_shell_canvas_tool_events(GtkWidget *canvas,
                                               GdkEvent *event,
                                               GimpDisplayShell *shell) {
  GimpDisplay *display;
  GimpImage *image;
  ...
  GIMP_LOG(TOOL_EVENTS, "event (display %p): %s", display,
           gimp_print_event(event));

  if (gimp_display_shell_check_device(shell, event, &device_changed))
    return TRUE;

  gimp_display_shell_get_event_coords(shell, event, &display_coords, &state,
                                      &time);
  gimp_display_shell_untransform_event_coords(shell, &display_coords,
                                              &image_coords, &update_sw_cursor);
  ...
  case GDK_ENTER_NOTIFY: {
    ...
    tool_manager_oper_update_active(gimp, &image_coords, state,
                                    shell->proximity, display);
  } break;
  ...
  case GDK_FOCUS_CHANGE: {
    ...
    if (fevent->in) {
      ...
      gimp_display_shell_update_focus(shell, FALSE, &image_coords, 0);
    }
  } break;
  ...
  case GDK_BUTTON_PRESS: {
    ...
    if (gdk_event_triggers_context_menu(event)) {
      ...
      ui_manager = tool_manager_get_popup_active(gimp, &image_coords, state,
                                                 display, &ui_path);
      ...
      gimp_ui_manager_ui_popup_at_pointer(ui_manager, ui_path, event, NULL,
                                          NULL);
    } else if (bevent->button == 1) {
      if (!gimp_display_shell_pointer_grab(
              shell, event, GDK_POINTER_MOTION_MASK | GDK_BUTTON_RELEASE_MASK))
        return TRUE;
      ...
      if (gimp_display_shell_initialize_tool(shell, &image_coords, state)) {
        ...
        if (motion_mode == GIMP_MOTION_MODE_EXACT) {
           ...
           */
          gdk_window_set_event_compression(gtk_widget_get_window(canvas),
                                           FALSE);
        }
        ...
        tool_manager_button_press_active(gimp, &image_coords, time, state,
                                         GIMP_BUTTON_PRESS_NORMAL, display);
      }
    } else if (bevent->button == 2) {
      gimp_display_shell_start_scrolling(shell, event, state, bevent->x,
                                         bevent->y);
    }
  ...
  case GDK_3BUTTON_PRESS: {
    ...
    if (bevent->button == 1 && active_tool &&
      ...
      tool_manager_button_press_active(gimp, &image_coords, time, state,
                                       GIMP_BUTTON_PRESS_TRIPLE, display);
    }
  ...
  case GDK_BUTTON_RELEASE: {
    ...
    if (bevent->button == 1) {
      ...
      if (active_tool &&
        ...
        gdk_window_set_event_compression(gtk_widget_get_window(canvas), TRUE);

        if (gimp_tool_control_is_active(active_tool->control)) {
          tool_manager_button_release_active(gimp, &image_coords, time, state,
                                             display);
        }
      }
      ...
      if (!(shell->scrolling || shell->grab_pointer) && mouse &&
          gimp_controller_mouse_button(GIMP_CONTROLLER_MOUSE(mouse), bevent)) {
        return TRUE;
      }
    }
  ...
  case GDK_SCROLL: {
    ...
    if (!wheel ||
        !gimp_controller_wheel_scroll(GIMP_CONTROLLER_WHEEL(wheel), sevent)) {
      if (state & gimp_get_toggle_behavior_mask()) {
        ...
        case GDK_SCROLL_SMOOTH:
          ...
          gimp_display_shell_scale(shell, GIMP_ZOOM_SMOOTH, delta,
                                   GIMP_ZOOM_FOCUS_POINTER);
          break;
        ...
        gimp_scroll_adjustment_values(sevent, shell->hsbdata, shell->vsbdata,
                                      &value_x, &value_y);
    ...
    gimp_display_shell_untransform_event_coords(
        shell, &display_coords, &image_coords, &update_sw_cursor);
  ...
  case GDK_MOTION_NOTIFY: {
    ...
    if (shell->scrolling) {
      gimp_display_shell_handle_scrolling(shell, state, mevent->x, mevent->y);
      ...
      if (active_tool && gimp_tool_control_is_active(active_tool->control) &&
          (!gimp_image_is_empty(image) ||
           gimp_tool_control_get_handle_empty_image(active_tool->control))) {
         ...
         *  scrolling...
         */
        if ((mevent->x < 0 || mevent->y < 0 || mevent->x > shell->disp_width ||
        ...
        if (motion_mode == GIMP_MOTION_MODE_EXACT &&
            ...
            gdk_device_get_history(mevent->device, mevent->window,
                                   last_motion_time + 1, mevent->time - 1,
                                   &history_events, &n_history_events)) {
          ...
          for (i = 0; i < n_history_events; i++) {
            gimp_device_info_get_time_coords(device, history_events[i],
                                             &display_coords);
             ...
             */
            if (gimp_motion_buffer_motion_event(
                    shell->motion_buffer, &image_coords,
                    history_events[i]->time, TRUE)) {
              gimp_motion_buffer_request_stroke(shell->motion_buffer, state,
           ...
           */
          if (gimp_motion_buffer_motion_event(
                  shell->motion_buffer, &image_coords, time, event_fill)) {
            gimp_motion_buffer_request_stroke(shell->motion_buffer, state,
                                              time);
          }
        }
    ...
    if (!(state & (GDK_BUTTON1_MASK | GDK_BUTTON2_MASK | GDK_BUTTON3_MASK))) {
       ...
       */
      if (gimp_motion_buffer_motion_event(shell->motion_buffer, &image_coords,
                                          time, FALSE)) {
        gimp_motion_buffer_request_hover(shell->motion_buffer, state,
                                         shell->proximity);
      }
    }
  ...
  case GDK_KEY_PRESS: {
    ...
    if (state & GDK_BUTTON1_MASK) {
      if (kevent->keyval == GDK_KEY_Alt_L || kevent->keyval == GDK_KEY_Alt_R ||
        ...
        if (active_tool && gimp_tool_control_is_active(active_tool->control) &&
            !gimp_image_is_empty(image)) {
          tool_manager_active_modifier_state_active(gimp, state, display);
        }
      }
    } else {
      ...
      case GDK_KEY_Escape:
        if (!gimp_image_is_empty(image))
          return_val = tool_manager_key_press_active(gimp, kevent, display);

        if (!return_val) {
          GimpController *keyboard = gimp_controllers_get_keyboard(gimp);

          if (keyboard)
            return_val = gimp_controller_keyboard_key_press(
                GIMP_CONTROLLER_KEYBOARD(keyboard), kevent);
        }
  ...
  case GDK_KEY_RELEASE: {
    ...
    if ((state & GDK_BUTTON1_MASK) && (!shell->space_release_pending ||
  ...
  /*  cursor update   */
  gimp_display_shell_update_cursor(shell, &display_coords, &image_coords, state,
                                   update_sw_cursor);

  return return_val;
}

void gimp_display_shell_canvas_grab_notify(GtkWidget *canvas,
  ...
  GIMP_LOG(TOOL_EVENTS, "grab_notify (display %p): was_grabbed = %s", display,
...
void gimp_display_shell_buffer_stroke(GimpMotionBuffer *buffer,
  ...
  if (active_tool && gimp_tool_control_is_active(active_tool->control)) {
    tool_manager_motion_active(gimp, coords, time, state, display);
  }
}
...
void gimp_display_shell_buffer_hover(GimpMotionBuffer *buffer,
  ...
  if (active_tool && !gimp_tool_control_is_active(active_tool->control)) {
    tool_manager_oper_update_active(gimp, coords, state, proximity, display);
  }
}

static gboolean
gimp_display_shell_ruler_button_press(GtkWidget *widget, GdkEventButton *event,
                                      GimpDisplayShell *shell,
  ...
  if (event->type == GDK_BUTTON_PRESS && event->button == 1) {
    ...
    if (active_tool) {
      gimp_display_shell_update_focus(shell, TRUE, NULL, event->state);
      ...
      if (gimp_display_shell_pointer_grab(shell, (GdkEvent *)event,
        ...
        if (gimp_display_shell_keyboard_grab(shell, (GdkEvent *)event)) {
          if (event->state & gimp_get_toggle_behavior_mask()) {
            ...
            gimp_guide_tool_start_new(active_tool, display, orientation);
          }
  ...
  if (device) {
    ...
    if (event->type == GDK_KEY_PRESS || event->type == GDK_KEY_RELEASE ||
      ...
      if ((shell->grab_keyboard && (shell->grab_keyboard != grab_device)) ||
          (shell->grab_keyboard_source &&
           (shell->grab_keyboard_source != device))) {
        GIMP_LOG(
            TOOL_EVENTS,
      ...
      if ((shell->grab_pointer && (shell->grab_pointer != grab_device)) ||
          (shell->grab_pointer_source &&
           (shell->grab_pointer_source != device))) {
        GIMP_LOG(TOOL_EVENTS,
                 "ignoring pointer event from '%s' while waiting for event "
                 "from '%s'\n",
                 gdk_device_get_name(device),
...
static void gimp_display_shell_start_scrolling(GimpDisplayShell *shell,
  ...
  if (shell->rotating) {
    gimp_display_shell_set_override_cursor(shell, (GimpCursorType)GDK_EXCHANGE);
    ...
    gimp_display_shell_set_override_cursor(
        shell, (GimpCursorType)GIMP_CURSOR_CROSSHAIR);

    gimp_display_shell_get_event_coords(shell, event, &display_coords, &state,
                                        &time);
    gimp_display_shell_untransform_event_coords(shell, &display_coords,
                                                &image_coords, NULL);
    layer = gimp_image_pick_layer(image, (gint)image_coords.x,
                                  (gint)image_coords.y, shell->picked_layer);
    ...
    if (layer && !gimp_image_get_floating_selection(image)) {
      ...
      if (g_list_length(layers) != 1 || layer != layers->data) {
        ...
        statusbar = gimp_display_shell_get_statusbar(shell);
        gimp_statusbar_push_temp(statusbar, GIMP_MESSAGE_INFO, GIMP_ICON_LAYER,
                                 _("Layer picked: '%s'"),
    ...
    gimp_display_shell_set_override_cursor(shell, (GimpCursorType)GDK_FLEUR);
}
...
static void gimp_display_shell_handle_scrolling(GimpDisplayShell *shell,
  ...
  if (shell->rotating) {
    ...
    gimp_display_shell_rotate_drag(shell, shell->scroll_last_x,
                                   shell->scroll_last_y, x, y, constrain);
  } else if (shell->scaling) {
    gimp_display_shell_scale_drag(
        shell, shell->scroll_start_x, shell->scroll_start_y,
        shell->scroll_last_x - x, shell->scroll_last_y - y);
  } else if (shell->layer_picking) {
    /* Do nothing. We only pick the layer on click. */
  } else {
    gimp_display_shell_scroll(shell, shell->scroll_last_x - x,
                              shell->scroll_last_y - y);
  }
...
static void gimp_display_shell_space_pressed(GimpDisplayShell *shell,
  ...
  case GIMP_SPACE_BAR_ACTION_PAN: {
    ...
    gimp_device_info_get_device_coords(
        current_device, gtk_widget_get_window(shell->canvas), &coords);
    gdk_event_get_state(event, &state);

    gimp_display_shell_start_scrolling(shell, event, state, coords.x, coords.y);
  } break;
  ...
  case GIMP_SPACE_BAR_ACTION_MOVE: {
    ...
    if (active_tool || !GIMP_IS_MOVE_TOOL(active_tool)) {
      ...
      shell->space_shaded_tool = gimp_object_get_name(active_tool->tool_info);
      ...
      gimp_display_shell_update_focus(shell, TRUE, NULL, state);
    }
  } break;
  }
...
static void gimp_display_shell_released(GimpDisplayShell *shell,
  ...
  case GIMP_SPACE_BAR_ACTION_MOVE:
    if (shell->space_shaded_tool) {
      gimp_context_set_tool(gimp_get_user_context(gimp),
                            gimp_get_tool_info(gimp, shell->space_shaded_tool));
...
static gboolean gimp_display_shell_tab_pressed(GimpDisplayShell *shell,
  ...
  if (kevent->state & GDK_CONTROL_MASK) {
    if (image && !gimp_image_is_empty(image)) {
      if (kevent->keyval == GDK_KEY_Tab || kevent->keyval == GDK_KEY_KP_Tab)
        gimp_display_shell_layer_select_init(shell, (GdkEvent *)kevent, 1);
      else
        gimp_display_shell_layer_select_init(shell, (GdkEvent *)kevent, -1);
    ...
    gimp_ui_manager_activate_action(manager, "windows", "windows-hide-docks");
...
static void gimp_display_shell_update_focus(GimpDisplayShell *shell,
  ...
  if (image_coords)
    tool_manager_oper_update_active(gimp, image_coords, state, shell->proximity,
                                    shell->display);
}
...
static void gimp_display_shell_update_cursor(GimpDisplayShell *shell,
  ...
  if (update_software_cursor) {
    ...
    gimp_display_shell_update_software_cursor(
        shell, precision, (gint)display_coords->x, (gint)display_coords->y,
...
gimp_display_shell_initialize_tool(GimpDisplayShell *shell,
  ...
  if (active_tool &&
    ...
    if (!active_tool->drawables) {
    ...
    } else if (!gimp_image_equal_selected_drawables(image,
                                                    active_tool->drawables) &&
               (!gimp_tool_control_get_preserve(active_tool->control) &&
                (gimp_tool_control_get_dirty_mask(active_tool->control) &
                 GIMP_DIRTY_ACTIVE_DRAWABLE))) {
      GimpProcedure *procedure =
          g_object_get_data(G_OBJECT(active_tool), "gimp-gegl-procedure");

      if (image ==
          gimp_item_get_image(GIMP_ITEM(active_tool->drawables->data))) {
        ...
        tool_manager_control_active(
            gimp, gimp_tool_control_get_dirty_action(active_tool->control),
            active_tool->display);
      }
      ...
      if (procedure) {
        ...
        gimp_ui_manager_activate_action(manager, "filters", "filters-reshow");
...
static void gimp_display_shell_get_event_coords(GimpDisplayShell *shell,
  ...
  gimp_device_info_get_event_coords(current_device,
                                    gtk_widget_get_window(shell->canvas), event,
                                    display_coords);

  gimp_device_info_get_event_state(
      current_device, gtk_widget_get_window(shell->canvas), event, state);
...
static void gimp_display_shell_untransform_event_coords(
  ...
  if (active_tool && gimp_tool_control_get_snap_to(active_tool->control)) {
    ...
    gimp_tool_control_get_snap_offsets(active_tool->control, &x, &y, &width,
                                       &height);

    if (gimp_display_shell_snap_coords(shell, image_coords, x, y, width,
                                       height)) {
      if (update_software_cursor)
