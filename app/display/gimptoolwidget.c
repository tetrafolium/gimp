...
enum { PROP_0, PROP_SHELL, PROP_ITEM };
...
static void gimp_tool_widget_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_tool_widget_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);
static void gimp_tool_widget_properties_changed(GObject *object, guint n_pspecs,
                                                GParamSpec **pspecs);
...
static void gimp_tool_widget_class_init(GimpToolWidgetClass *klass) {
  ...
  object_class->dispatch_properties_changed =
      gimp_tool_widget_properties_changed;
  ...
  widget_signals[CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpToolWidgetClass, changed), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);

  widget_signals[RESPONSE] =
      g_signal_new("response", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpToolWidgetClass, response), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, G_TYPE_INT);

  widget_signals[SNAP_OFFSETS] =
      g_signal_new("snap-offsets", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpToolWidgetClass, snap_offsets), NULL,
                   NULL, gimp_marshal_VOID__INT_INT_INT_INT, G_TYPE_NONE, 4,
                   G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT);

  widget_signals[STATUS] =
      g_signal_new("status", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpToolWidgetClass, status), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, G_TYPE_STRING);

  widget_signals[STATUS_COORDS] = g_signal_new(
      "status-coords", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolWidgetClass, status_coords), NULL, NULL,
      gimp_marshal_VOID__STRING_DOUBLE_STRING_DOUBLE_STRING, G_TYPE_NONE, 5,
      G_TYPE_STRING, G_TYPE_DOUBLE, G_TYPE_STRING, G_TYPE_DOUBLE,
      G_TYPE_STRING);

  widget_signals[MESSAGE] =
      g_signal_new("message", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpToolWidgetClass, message), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, G_TYPE_STRING);

  widget_signals[FOCUS_CHANGED] = g_signal_new(
      "focus-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolWidgetClass, focus_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  g_object_class_install_property(
      object_class, PROP_SHELL,
      g_param_spec_object("shell", NULL, NULL, GIMP_TYPE_DISPLAY_SHELL,
...
static void gimp_tool_widget_init(GimpToolWidget *widget) {
...
private
  ->item = gimp_canvas_group_new(private->shell);
  ...
  if (klass->changed) {
    if (klass->update_on_scale) {
      g_signal_connect_object(private->shell, "scaled",
                              G_CALLBACK(klass->changed), widget,
                              G_CONNECT_SWAPPED);
    }
...
static void gimp_tool_widget_set_property(GObject *object, guint property_id,
  ...
  case PROP_SHELL:
  private
    ->shell = g_value_get_object(value); /* don't ref */
    break;
...
void gimp_tool_widget_set_visible(GimpToolWidget *widget, gboolean visible) {
...
void gimp_tool_widget_set_focus(GimpToolWidget *widget, gboolean focus) {
  g_return_if_fail(GIMP_IS_TOOL_WIDGET(widget));
...
void gimp_tool_widget_response(GimpToolWidget *widget, gint response_id) {
  g_return_if_fail(GIMP_IS_TOOL_WIDGET(widget));

  g_signal_emit(widget, widget_signals[RESPONSE], 0, response_id);
}

void gimp_tool_widget_set_snap_offsets(GimpToolWidget *widget, gint offset_x,
                                       gint offset_y, gint width, gint height) {
...
private
  ...
  if (offset_x != private->snap_offset_x ||
      offset_y != private->snap_offset_y || width != private->snap_width ||
      height != private->snap_height) {
  private
    ->snap_offset_x = offset_x;
  private
    ->snap_offset_y = offset_y;
  private
    ->snap_width = width;
  private
    ->snap_height = height;

    g_signal_emit(widget, widget_signals[SNAP_OFFSETS], 0, offset_x, offset_y,
                  width, height);
  }
}

void gimp_tool_widget_get_snap_offsets(GimpToolWidget *widget, gint *offset_x,
                                       gint *offset_y, gint *width,
                                       gint *height) {
  GimpToolWidgetPrivate *private;
...
private
  ...
  if (offset_x)
    *offset_x = private->snap_offset_x;
  if (offset_y)
...
void gimp_tool_widget_set_status(GimpToolWidget *widget, const gchar *status) {
  g_return_if_fail(GIMP_IS_TOOL_WIDGET(widget));

  g_signal_emit(widget, widget_signals[STATUS], 0, status);
}

void gimp_tool_widget_set_status_coords(GimpToolWidget *widget,
                                        const gchar *title, gdouble x,
                                        const gchar *separator, gdouble y,
                                        const gchar *help) {
  g_return_if_fail(GIMP_IS_TOOL_WIDGET(widget));

  g_signal_emit(widget, widget_signals[STATUS_COORDS], 0, title, x, separator,
                y, help);
}

void gimp_tool_widget_message(GimpToolWidget *widget, const gchar *format,
                              ...) {
  va_list args;
  gchar *message;
...
void gimp_tool_widget_message_literal(GimpToolWidget *widget,
  ...
  g_signal_emit(widget, widget_signals[MESSAGE], 0, message);
}

void gimp_tool_widget_add_item(GimpToolWidget *widget, GimpCanvasItem *item) {
...
void gimp_tool_widget_remove_item(GimpToolWidget *widget,
  ...
  gimp_canvas_group_remove_item(GIMP_CANVAS_GROUP(widget->private->item), item);
}
...
void gimp_tool_widget_push_group(GimpToolWidget *widget,
  ...
  widget->private->group_stack =
      g_list_prepend(widget->private->group_stack, group);
}
...
void gimp_tool_widget_pop_group(GimpToolWidget *widget) {
  ...
  widget->private->group_stack = g_list_remove(
      widget->private->group_stack, widget->private->group_stack->data);
}
 ...
 **/
GimpCanvasItem *gimp_tool_widget_add_line(GimpToolWidget *widget, gdouble x1,
                                          gdouble y1, gdouble x2, gdouble y2) {
  GimpCanvasItem *item;

  g_return_val_if_fail(GIMP_IS_TOOL_WIDGET(widget), NULL);

  item = gimp_canvas_line_new(widget->private->shell, x1, y1, x2, y2);
...
GimpCanvasItem *gimp_tool_widget_add_rectangle(GimpToolWidget *widget,
  ...
  item = gimp_canvas_rectangle_new(widget->private->shell, x, y, width, height,
                                   filled);
...
GimpCanvasItem *gimp_tool_widget_add_arc(GimpToolWidget *widget,
                                         ...
                                         gdouble start_angle,
                                         gdouble slice_angle, gboolean filled) {
  GimpCanvasItem *item;

  g_return_val_if_fail(GIMP_IS_TOOL_WIDGET(widget), NULL);

  item =
      gimp_canvas_arc_new(widget->private->shell, center_x, center_y, radius_x,
                          radius_y, start_angle, slice_angle, filled);
...
GimpCanvasItem *gimp_tool_widget_add_limit(GimpToolWidget *widget,
                                           GimpLimitType type, gdouble x,
                                           gdouble y, gdouble radius,
  ...
  item = gimp_canvas_limit_new(widget->private->shell, type, x, y, radius,
                               aspect_ratio, angle, dashed);
...
GimpCanvasItem *gimp_tool_widget_add_polygon(GimpToolWidget *widget,
                                             ...
                                             gint n_points, gboolean filled) {
  GimpCanvasItem *item;

  g_return_val_if_fail(GIMP_IS_TOOL_WIDGET(widget), NULL);
  g_return_val_if_fail(points == NULL || n_points > 0, NULL);

  item = gimp_canvas_polygon_new(widget->private->shell, points, n_points,
                                 transform, filled);
...
GimpCanvasItem *gimp_tool_widget_add_polygon_from_coords(
    GimpToolWidget *widget, GimpMatrix3 *transform, const GimpCoords *points,
    gint n_points, gboolean filled) {
  GimpCanvasItem *item;
  ...
  item = gimp_canvas_polygon_new_from_coords(widget->private->shell, points,
                                             n_points, transform, filled);
...
GimpCanvasItem *gimp_tool_widget_add_path(GimpToolWidget *widget,
  ...
  item = gimp_canvas_path_new(widget->private->shell, desc, 0, 0, FALSE,
...
GimpCanvasItem *gimp_tool_widget_add_handle(GimpToolWidget *widget,
  ...
  item = gimp_canvas_handle_new(widget->private->shell, type, anchor, x, y,
                                width, height);
...
GimpCanvasItem *
gimp_tool_widget_add_corner(GimpToolWidget *widget, gdouble x, gdouble y,
                            gdouble width, gdouble height,
                            GimpHandleAnchor anchor, gint corner_width,
                            gint corner_height, gboolean outside) {
  GimpCanvasItem *item;

  g_return_val_if_fail(GIMP_IS_TOOL_WIDGET(widget), NULL);

  item = gimp_canvas_corner_new(widget->private->shell, x, y, width, height,
                                anchor, corner_width, corner_height, outside);
...
GimpCanvasItem *gimp_tool_widget_add_rectangle_guides(GimpToolWidget *widget,
                                                      gdouble x, gdouble y,
                                                      gdouble width,
                                                      gdouble height,
                                                      GimpGuidesType type) {
  ...
  item = gimp_canvas_rectangle_guides_new(widget->private->shell, x, y, width,
                                          height, type, 4);
...
GimpCanvasItem *gimp_tool_widget_add_transform_guides(
    GimpToolWidget *widget, const GimpMatrix3 *transform, gdouble x1,
    gdouble y1, gdouble x2, gdouble y2, GimpGuidesType type, gint n_guides,
    gboolean clip) {
  GimpCanvasItem *item;

  g_return_val_if_fail(GIMP_IS_TOOL_WIDGET(widget), NULL);

  item = gimp_canvas_transform_guides_new(widget->private->shell, transform, x1,
                                          y1, x2, y2, type, n_guides, clip);
...
gint gimp_tool_widget_button_press(GimpToolWidget *widget,
  ...
  if (widget->private->visible &&
    ...
    return GIMP_TOOL_WIDGET_GET_CLASS(widget)->button_press(
...
void gimp_tool_widget_button_release(GimpToolWidget *widget,
  ...
  if (widget->private->visible &&
    ...
    GIMP_TOOL_WIDGET_GET_CLASS(widget)->button_release(widget, coords, time,
                                                       state, release_type);
  }
}

void gimp_tool_widget_motion(GimpToolWidget *widget, const GimpCoords *coords,
                             guint32 time, GdkModifierType state) {
  g_return_if_fail(GIMP_IS_TOOL_WIDGET(widget));
  g_return_if_fail(coords != NULL);

  if (widget->private->visible && GIMP_TOOL_WIDGET_GET_CLASS(widget)->motion) {
    GIMP_TOOL_WIDGET_GET_CLASS(widget)->motion(widget, coords, time, state);
  }
}

GimpHit gimp_tool_widget_hit(GimpToolWidget *widget, const GimpCoords *coords,
  ...
  if (widget->private->visible && GIMP_TOOL_WIDGET_GET_CLASS(widget)->hit) {
    return GIMP_TOOL_WIDGET_GET_CLASS(widget)->hit(widget, coords, state,
                                                   proximity);
  }
...
void gimp_tool_widget_hover(GimpToolWidget *widget, const GimpCoords *coords,
                            GdkModifierType state, gboolean proximity) {
  g_return_if_fail(GIMP_IS_TOOL_WIDGET(widget));
  g_return_if_fail(coords != NULL);

  if (widget->private->visible && GIMP_TOOL_WIDGET_GET_CLASS(widget)->hover) {
    GIMP_TOOL_WIDGET_GET_CLASS(widget)->hover(widget, coords, state, proximity);
  }
}
...
void gimp_tool_widget_motion_modifier(GimpToolWidget *widget,
  ...
  if (widget->private->visible &&
    ...
    GIMP_TOOL_WIDGET_GET_CLASS(widget)->motion_modifier(widget, key, press,
                                                        state);
  }
}
...
void gimp_tool_widget_hover_modifier(GimpToolWidget *widget,
  ...
  if (widget->private->visible &&
    ...
    GIMP_TOOL_WIDGET_GET_CLASS(widget)->hover_modifier(widget, key, press,
                                                       state);
  }
}

gboolean
gimp_tool_widget_get_cursor(GimpToolWidget *widget, const GimpCoords *coords,
                            GdkModifierType state, GimpCursorType *cursor,
...

{
  ...
  if (widget->private->visible &&
    ...
    if (cursor)
      my_cursor = *cursor;
    if (tool_cursor)
      my_tool_cursor = *tool_cursor;
    if (modifier)
      my_modifier = *modifier;

    if (GIMP_TOOL_WIDGET_GET_CLASS(widget)->get_cursor(
            widget, coords, state, &my_cursor, &my_tool_cursor, &my_modifier)) {
      if (cursor)
        *cursor = my_cursor;
      if (tool_cursor)
        *tool_cursor = my_tool_cursor;
      if (modifier)
...
GimpUIManager *gimp_tool_widget_get_popup(GimpToolWidget *widget,
  ...
  if (widget->private->visible &&
    ...
    return GIMP_TOOL_WIDGET_GET_CLASS(widget)->get_popup(widget, coords, state,
                                                         ui_path);
  }
