...
#define GET_PRIVATE(limit)                                                     \
  ((GimpCanvasLimitPrivate *)gimp_canvas_limit_get_instance_private(           \
      (GimpCanvasLimit *)(limit)))

/*  local function prototypes  */

static void gimp_canvas_limit_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_canvas_limit_get_property(GObject *object, guint property_id,
                                           GValue *value, GParamSpec *pspec);

static void gimp_canvas_limit_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *gimp_canvas_limit_get_extents(GimpCanvasItem *item);
static gboolean gimp_canvas_limit_hit(GimpCanvasItem *item, gdouble x,
                                      gdouble y);
...
static void gimp_canvas_limit_class_init(GimpCanvasLimitClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("type", NULL, NULL, GIMP_TYPE_LIMIT_TYPE,
                        GIMP_LIMIT_CIRCLE, GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(object_class, PROP_RADIUS,
                                  g_param_spec_double("radius", NULL, NULL, 0.0,
                                                      +G_MAXDOUBLE, 0.0,
                                                      GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_ASPECT_RATIO,
                                  g_param_spec_double("aspect-ratio", NULL,
                                                      NULL, -1.0, +1.0, 0.0,
                                                      GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      object_class, PROP_DASHED,
      g_param_spec_boolean("dashed", NULL, NULL, FALSE, GIMP_PARAM_READWRITE));
}

static void gimp_canvas_limit_init(GimpCanvasLimit *limit) {}
...
static void gimp_canvas_limit_transform(GimpCanvasItem *item, gdouble *x,
                                        gdouble *y, gdouble *rx, gdouble *ry) {
  ...
  gimp_canvas_item_transform_xy_f(item, priv->x - *rx, priv->y - *ry, &x1, &y1);
  gimp_canvas_item_transform_xy_f(item, priv->x + *rx, priv->y + *ry, &x2, &y2);
...
static void gimp_canvas_limit_paint(GimpCanvasItem *item, cairo_t *cr) {
  GimpCanvasLimitPrivate *priv = GET_PRIVATE(item);
  ...
  gimp_canvas_limit_transform(item, &x, &y, &rx, &ry);
  ...
  inf = MAX(x, shell->disp_width - x) + MAX(y, shell->disp_height - y);
...
static gboolean gimp_canvas_limit_hit(GimpCanvasItem *item, gdouble x,
  ...
  gimp_canvas_limit_boundary_point(limit, x, y, &bx, &by);

  return gimp_canvas_item_transform_distance(item, x, y, bx, by) <=
         HIT_DISTANCE;
}
...
GimpCanvasItem *gimp_canvas_limit_new(GimpDisplayShell *shell,
                                      GimpLimitType type, gdouble x, gdouble y,
                                      gdouble radius, gdouble aspect_ratio,
                                      gdouble angle, gboolean dashed) {
  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), NULL);

  return g_object_new(GIMP_TYPE_CANVAS_LIMIT, "shell", shell, "type", type, "x",
                      x, "y", y, "radius", radius, "aspect-ratio", aspect_ratio,
                      "angle", angle, "dashed", dashed, NULL);
}

void gimp_canvas_limit_get_radii(GimpCanvasLimit *limit, gdouble *rx,
                                 gdouble *ry) {
  ...
  if (priv->aspect_ratio >= 0.0) {
    if (rx)
      *rx = priv->radius;
    if (ry)
      *ry = priv->radius * (1.0 - priv->aspect_ratio);
  } else {
    if (rx)
      *rx = priv->radius * (1.0 + priv->aspect_ratio);
    if (ry)
      *ry = priv->radius;
  }
}

gboolean gimp_canvas_limit_is_inside(GimpCanvasLimit *limit, gdouble x,
                                     gdouble y) {
  GimpCanvasLimitPrivate *priv;
  GimpVector2 p;
  gdouble rx, ry;
...
void gimp_canvas_limit_boundary_point(GimpCanvasLimit *limit, gdouble x,
                                      gdouble y, gdouble *bx, gdouble *by) {
  GimpCanvasLimitPrivate *priv;
  GimpVector2 p;
  gdouble rx, ry;
  ...
  case GIMP_LIMIT_DIAMOND: {
    ...
    t = gimp_vector2_inner_product(&r, &l) / gimp_vector2_inner_product(&l, &l);
    t = CLAMP(t, 0.0, 1.0);
...
gdouble gimp_canvas_limit_boundary_radius(GimpCanvasLimit *limit, gdouble x,
                                          gdouble y) {
  GimpCanvasLimitPrivate *priv;
  GimpVector2 p;
...
void gimp_canvas_limit_center_point(GimpCanvasLimit *limit, gdouble x,
                                    gdouble y, gdouble *cx, gdouble *cy) {
  GimpCanvasLimitPrivate *priv;
  GimpVector2 p;
