...
static void gimp_tool_widget_group_button_release(
...
static void gimp_tool_widget_group_hover_modifier(GimpToolWidget *widget,
                                                  ...
                                                  gboolean press,
                                                  GdkModifierType state);
static gboolean gimp_tool_widget_group_get_cursor(
...
static void gimp_tool_widget_group_child_response(GimpToolWidget *child,
                                                  gint response_id,
                                                  GimpToolWidgetGroup *group);
static void gimp_tool_widget_group_child_snap_offsets(
    GimpToolWidget *child, gint offset_x, gint offset_y, gint width,
    gint height, GimpToolWidgetGroup *group);
static void gimp_tool_widget_group_child_status(GimpToolWidget *child,
                                                const gchar *status,
                                                GimpToolWidgetGroup *group);
static void gimp_tool_widget_group_child_status_coords(
    GimpToolWidget *child, const gchar *title, gdouble x,
...
static GimpToolWidget *gimp_tool_widget_group_get_hover_widget(
    GimpToolWidgetGroup *group, const GimpCoords *coords, GdkModifierType state,
    gboolean proximity, GimpHit *hit);
...
static void gimp_tool_widget_group_class_init(GimpToolWidgetGroupClass *klass) {
...
static void gimp_tool_widget_group_init(GimpToolWidgetGroup *group) {
  GimpToolWidgetGroupPrivate *priv;

  priv = group->priv = gimp_tool_widget_group_get_instance_private(group);

  priv->children = g_object_new(GIMP_TYPE_LIST, "children-type",
                                GIMP_TYPE_TOOL_WIDGET, "append", TRUE, NULL);

  g_signal_connect(priv->children, "add",
                   G_CALLBACK(gimp_tool_widget_group_children_add), group);
  g_signal_connect(priv->children, "remove",
                   G_CALLBACK(gimp_tool_widget_group_children_remove), group);
  g_signal_connect(priv->children, "reorder",
                   G_CALLBACK(gimp_tool_widget_group_children_reorder), group);
  ...
  gimp_container_add_handler(
      priv->children, "snap-offsets",
      G_CALLBACK(gimp_tool_widget_group_child_snap_offsets), group);
  gimp_container_add_handler(priv->children, "status",
                             G_CALLBACK(gimp_tool_widget_group_child_status),
                             group);
  gimp_container_add_handler(
      priv->children, "status-coords",
      G_CALLBACK(gimp_tool_widget_group_child_status_coords), group);
  gimp_container_add_handler(priv->children, "message",
                             G_CALLBACK(gimp_tool_widget_group_child_message),
                             group);
  gimp_container_add_handler(
      priv->children, "focus-changed",
      G_CALLBACK(gimp_tool_widget_group_child_focus_changed), group);
}
...
static gint gimp_tool_widget_group_button_press(
    GimpToolWidget *widget, const GimpCoords *coords, guint32 time,
  ...
  if (priv->hover_widget) {
    if (priv->auto_raise) {
      gimp_container_reorder(priv->children, GIMP_OBJECT(priv->hover_widget),
                             -1);
    }

    return gimp_tool_widget_button_press(priv->hover_widget, coords, time,
                                         state, press_type);
  }
...
static void gimp_tool_widget_group_button_release(
  ...
  if (priv->hover_widget) {
    gimp_tool_widget_button_release(priv->hover_widget, coords, time, state,
                                    release_type);
  }
}
...
static GimpHit gimp_tool_widget_group_hit(GimpToolWidget *widget,
  ...
  gimp_tool_widget_group_get_hover_widget(group, coords, state, proximity,
                                          &hit);
...
static void gimp_tool_widget_group_hover(GimpToolWidget *widget,
  ...
  hover_widget = gimp_tool_widget_group_get_hover_widget(group, coords, state,
                                                         proximity, NULL);
...
static void gimp_tool_widget_group_hover_modifier(GimpToolWidget *widget,
  ...
  for (iter = g_queue_peek_head_link(GIMP_LIST(priv->children)->queue); iter;
...
static gboolean gimp_tool_widget_group_get_cursor(
  ...
  if (priv->hover_widget) {
    return gimp_tool_widget_get_cursor(priv->hover_widget, coords, state,
...
static void gimp_tool_widget_group_children_add(GimpContainer *container,
  ...
  gimp_canvas_group_add_item(canvas_group, gimp_tool_widget_get_item(child));
...
static void gimp_tool_widget_group_children_remove(GimpContainer *container,
  ...
  gimp_canvas_group_remove_item(canvas_group, gimp_tool_widget_get_item(child));
...
gimp_tool_widget_group_children_reorder(GimpContainer *container,
                                        GimpToolWidget *child, gint new_index,
                                        GimpToolWidgetGroup *group) {
  GimpToolWidget *widget = GIMP_TOOL_WIDGET(group);
  GimpCanvasGroup *canvas_group;
  GList *iter;
  ...
  for (iter = g_queue_peek_head_link(GIMP_LIST(container)->queue); iter;
       iter = g_list_next(iter)) {
...
static void gimp_tool_widget_group_child_snap_offsets(
  ...
  if (priv->hover_widget == child) {
    gimp_tool_widget_set_snap_offsets(widget, offset_x, offset_y, width,
                                      height);
  }
}
...
static GimpToolWidget *gimp_tool_widget_group_get_hover_widget(
  ...
  for (iter = g_queue_peek_tail_link(GIMP_LIST(priv->children)->queue); iter;
  ...
  if (hit)
    *hit = indirect_child ? GIMP_HIT_INDIRECT : GIMP_HIT_NONE;
...
GimpToolWidget *gimp_tool_widget_group_new(GimpDisplayShell *shell) {
  ...
  return g_object_new(GIMP_TYPE_TOOL_WIDGET_GROUP, "shell", shell, NULL);
}
