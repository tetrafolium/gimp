...
static void gimp_display_shell_drop_drawable(GtkWidget *widget, gint x, gint y,
                                             GimpViewable *viewable,
                                             gpointer data);
static void gimp_display_shell_drop_vectors(GtkWidget *widget, gint x, gint y,
                                            GimpViewable *viewable,
                                            gpointer data);
static void gimp_display_shell_drop_svg(GtkWidget *widget, gint x, gint y,
                                        const guchar *svg_data,
                                        gsize svg_data_length, gpointer data);
static void gimp_display_shell_drop_pattern(GtkWidget *widget, gint x, gint y,
                                            GimpViewable *viewable,
                                            gpointer data);
static void gimp_display_shell_drop_color(GtkWidget *widget, gint x, gint y,
                                          const GimpRGB *color, gpointer data);
static void gimp_display_shell_drop_buffer(GtkWidget *widget, gint x, gint y,
                                           GimpViewable *viewable,
                                           gpointer data);
static void gimp_display_shell_drop_uri_list(GtkWidget *widget, gint x, gint y,
                                             GList *uri_list, gpointer data);
static void gimp_display_shell_drop_component(GtkWidget *widget, gint x, gint y,
                                              GimpImage *image,
                                              GimpChannelType component,
                                              gpointer data);
static void gimp_display_shell_drop_pixbuf(GtkWidget *widget, gint x, gint y,
...
void gimp_display_shell_dnd_init(GimpDisplayShell *shell) {
  ...
  gimp_dnd_viewable_dest_add(shell->canvas, GIMP_TYPE_LAYER,
                             gimp_display_shell_drop_drawable, shell);
  ...
  gimp_dnd_viewable_dest_add(shell->canvas, GIMP_TYPE_VECTORS,
                             gimp_display_shell_drop_vectors, shell);
  ...
  gimp_dnd_viewable_dest_add(shell->canvas, GIMP_TYPE_BUFFER,
                             gimp_display_shell_drop_buffer, shell);
  gimp_dnd_color_dest_add(shell->canvas, gimp_display_shell_drop_color, shell);
  gimp_dnd_component_dest_add(shell->canvas, gimp_display_shell_drop_component,
                              shell);
  gimp_dnd_uri_list_dest_add(shell->canvas, gimp_display_shell_drop_uri_list,
                             shell);
  gimp_dnd_svg_dest_add(shell->canvas, gimp_display_shell_drop_svg, shell);
  gimp_dnd_pixbuf_dest_add(shell->canvas, gimp_display_shell_drop_pixbuf,
                           shell);
}
 ...
 */
static void gimp_display_shell_dnd_position_item(GimpDisplayShell *shell,
                                                 GimpImage *image,
                                                 GimpItem *item) {
  ...
  if (item_width >= gimp_image_get_width(image) &&
    ...
    gimp_display_shell_untransform_viewport(
        shell, !gimp_display_shell_get_infinite_canvas(shell), &x, &y, &width,
        &height);
  ...
  gimp_item_translate(item, off_x - gimp_item_get_offset_x(item),
                      off_y - gimp_item_get_offset_y(item), FALSE);
}

static void gimp_display_shell_dnd_flush(GimpDisplayShell *shell,
                                         GimpImage *image) {
...
static void gimp_display_shell_drop_svg(GtkWidget *widget, gint x, gint y,
                                        const guchar *svg_data,
                                        gsize svg_data_len, gpointer data) {
  GimpDisplayShell *shell = GIMP_DISPLAY_SHELL(data);
  GimpImage *image = gimp_display_get_image(shell->display);
  GError *error = NULL;
  ...
  if (!gimp_vectors_import_buffer(image, (const gchar *)svg_data, svg_data_len,
                                  TRUE, FALSE, GIMP_IMAGE_ACTIVE_PARENT, -1,
                                  NULL, &error)) {
...
static void gimp_display_shell_dnd_fill(GimpDisplayShell *shell,
...
static void gimp_display_shell_drop_pattern(GtkWidget *widget, gint x, gint y,
  ...
  GimpDisplayShell *shell = GIMP_DISPLAY_SHELL(data);
  GimpFillOptions *options =
      gimp_fill_options_new(shell->display->gimp, NULL, FALSE);
...
static void gimp_display_shell_drop_buffer(GtkWidget *widget, gint drop_x,
                                           gint drop_y, GimpViewable *viewable,
                                           gpointer data) {
  GimpDisplayShell *shell = GIMP_DISPLAY_SHELL(data);
  GimpImage *image = gimp_display_get_image(shell->display);
  ...
  if (!image) {
    image =
        gimp_image_new_from_buffer(shell->display->gimp, GIMP_BUFFER(viewable));
  ...
  gimp_edit_paste(image, drawable, GIMP_OBJECT(buffer), paste_type, x, y, width,
                  height);
  ...
  for (list = uri_list; list; list = g_list_next(list)) {
    ...
    if (open_as_layers) {
      ...
      new_layers = file_open_layers(
          shell->display->gimp, context, GIMP_PROGRESS(shell->display), image,
      ...
      if (new_layers) {
        ...
        if (gimp_display_get_image(shell->display)) {
          gimp_display_shell_untransform_viewport(
              shell, !gimp_display_shell_get_infinite_canvas(shell), &x, &y,
              &width, &height);
        }

        gimp_image_add_layers(image, new_layers, GIMP_IMAGE_ACTIVE_PARENT, -1,
                              x, y, width, height, _("Drop layers"));
      ...
      new_image = file_open_with_display(
          shell->display->gimp, context, NULL, file, FALSE,
      ...
      image = file_open_with_display(
          shell->display->gimp, context, GIMP_PROGRESS(shell->display), file,
          FALSE, G_OBJECT(gimp_widget_get_monitor(widget)), &status, &error);
...
static void gimp_display_shell_drop_component(GtkWidget *widget, gint x, gint y,
  ...
  if (!dest_image) {
    dest_image = gimp_image_new_from_component(image->gimp, image, component);
  ...
  new_item = gimp_item_convert(GIMP_ITEM(channel), dest_image, GIMP_TYPE_LAYER);
  ...
  if (new_item) {
    ...
    gimp_image_add_layer(dest_image, new_layer, GIMP_IMAGE_ACTIVE_PARENT, -1,
                         TRUE);
...
static void gimp_display_shell_drop_pixbuf(GtkWidget *widget, gint x, gint y,
  ...
  new_layer = gimp_layer_new_from_pixbuf(
      pixbuf, image, gimp_image_get_layer_format(image, has_alpha),
