...
GtkWidget *gimp_display_shell_filter_dialog_new(GimpDisplayShell *shell) {
  ...
  cdd->shell = shell;
  cdd->dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, image), gimp_get_user_context(shell->display->gimp),
      _("Color Display Filters"), "gimp-display-filters",
      GIMP_ICON_DISPLAY_FILTER, _("Configure Color Display Filters"),
      GTK_WIDGET(cdd->shell), gimp_standard_help_func,
  ...
  gimp_dialog_set_alternative_button_order(
      GTK_DIALOG(cdd->dialog), GTK_RESPONSE_OK, GTK_RESPONSE_CANCEL, -1);
  ...
  g_signal_connect(cdd->dialog, "response",
                   G_CALLBACK(gimp_display_shell_filter_dialog_response), cdd);

  if (shell->filter_stack) {
    cdd->old_stack = gimp_color_display_stack_clone(shell->filter_stack);

    g_object_weak_ref(G_OBJECT(cdd->dialog), (GWeakNotify)g_object_unref,
                      cdd->old_stack);
  } else {
  ...
  editor = gimp_color_display_editor_new(
      shell->display->gimp, shell->filter_stack,
      gimp_display_shell_get_color_config(shell), GIMP_COLOR_MANAGED(shell));
  gtk_container_set_border_width(GTK_CONTAINER(editor), 12);
  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(cdd->dialog))), editor,
      TRUE, TRUE, 0);
