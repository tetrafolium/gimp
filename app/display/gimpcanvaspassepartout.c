...
#define GET_PRIVATE(item)                                                      \
  ((GimpCanvasPassePartoutPrivate *)                                           \
       gimp_canvas_passe_partout_get_instance_private(                         \
...
static void gimp_canvas_passe_partout_get_property(GObject *object,
                                                   ...
                                                   GValue *value,
                                                   GParamSpec *pspec);
static void gimp_canvas_passe_partout_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *
gimp_canvas_passe_partout_get_extents(GimpCanvasItem *item);
static void gimp_canvas_passe_partout_fill(GimpCanvasItem *item, cairo_t *cr);
...
gimp_canvas_passe_partout_get_extents(GimpCanvasItem *item) {
  ...
  if (!gimp_display_shell_get_infinite_canvas(shell)) {
    ...
    gimp_display_shell_scale_get_image_size(shell, &rectangle.width,
                                            &rectangle.height);
...
GimpCanvasItem *gimp_canvas_passe_partout_new(GimpDisplayShell *shell,
  ...
  return g_object_new(GIMP_TYPE_CANVAS_PASSE_PARTOUT, "shell", shell, "x", x,
                      "y", y, "width", width, "height", height, "filled", TRUE,
                      NULL);
}
