...
enum { COLUMN_SCALE, COLUMN_LABEL, COLUMN_PERSISTENT, N_COLUMNS };

enum { ENTRY_ACTIVATED, LAST_SIGNAL };
...
static gboolean
gimp_scale_combo_box_entry_key_press(GtkWidget *entry, GdkEventKey *event,
                                     GimpScaleComboBox *combo_box);
...
static void gimp_scale_combo_box_class_init(GimpScaleComboBoxClass *klass) {
  ...
  scale_combo_box_signals[ENTRY_ACTIVATED] = g_signal_new(
      "entry-activated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpScaleComboBoxClass, entry_activated), NULL, NULL,
      NULL, G_TYPE_NONE, 0);
...
static void gimp_scale_combo_box_constructed(GObject *object) {
  ...
  store = gtk_list_store_new(N_COLUMNS, G_TYPE_DOUBLE, /* SCALE       */
                             G_TYPE_STRING,            /* LABEL       */
  ...
  g_object_set(entry, "xalign", 1.0, "width-chars", 5, "truncate-multiline",
               TRUE, "inner-border", &border, NULL);

  layout = GTK_CELL_LAYOUT(combo_box);

  cell = g_object_new(GTK_TYPE_CELL_RENDERER_TEXT, "xalign", 1.0, NULL);

  gtk_cell_layout_clear(layout);
  gtk_cell_layout_pack_start(layout, cell, TRUE);
  gtk_cell_layout_set_attributes(layout, cell, "text", COLUMN_LABEL, NULL);
  ...
  g_signal_connect(combo_box, "changed",
                   G_CALLBACK(gimp_scale_combo_box_changed), NULL);

  g_signal_connect(entry, "activate",
                   G_CALLBACK(gimp_scale_combo_box_entry_activate), combo_box);
  g_signal_connect(entry, "key-press-event",
                   G_CALLBACK(gimp_scale_combo_box_entry_key_press), combo_box);
}
...
static void gimp_scale_combo_box_changed(GimpScaleComboBox *combo_box) {
  ...
  if (gtk_combo_box_get_active_iter(GTK_COMBO_BOX(combo_box), &iter)) {
    ...
    gtk_tree_model_get(model, &iter, COLUMN_SCALE, &scale, -1);
    if (scale > 0.0) {
...
static gboolean gimp_scale_combo_box_parse_text(const gchar *text,
                                                gdouble *scale) {
  gchar *end;
  gdouble left_number;
...
static void gimp_scale_combo_box_entry_activate(GtkWidget *entry,
  ...
  if (gimp_scale_combo_box_parse_text(text, &scale) && scale >= 1.0 / 256.0 &&
      scale <= 256.0) {
...
#define PERCENT_SPACE "\342\200\211"
  ...
  if (scale > 1.0)
    g_snprintf(label, sizeof(label), "%d" PERCENT_SPACE "%%",
               (gint)ROUND(100.0 * scale));
  else
    g_snprintf(label, sizeof(label), "%.3g" PERCENT_SPACE "%%", 100.0 * scale);

  gtk_list_store_set(store, iter, COLUMN_SCALE, scale, COLUMN_LABEL, label,
                     COLUMN_PERSISTENT, persistent, -1);
}

static void gimp_scale_combo_box_mru_add(GimpScaleComboBox *combo_box,
                                         GtkTreeIter *iter) {
  GtkTreeModel *model = gtk_combo_box_get_model(GTK_COMBO_BOX(combo_box));
  ...
  for (list = combo_box->mru, found = FALSE; list && !found;
       list = list->next) {
  ...
  if (!found)
    combo_box->mru =
        g_list_prepend(combo_box->mru, gtk_tree_row_reference_new(model, path));

  gtk_tree_path_free(path);
}

static void gimp_scale_combo_box_mru_remove_last(GimpScaleComboBox *combo_box) {
  GtkTreeModel *model;
  GtkTreePath *path;
  GList *last;
  GtkTreeIter iter;
...
GtkWidget *gimp_scale_combo_box_new(void) {
  return g_object_new(GIMP_TYPE_SCALE_COMBO_BOX, "has-entry", TRUE, NULL);
}
...
void gimp_scale_combo_box_set_scale(GimpScaleComboBox *combo_box,
  ...
  for (iter_valid = gtk_tree_model_get_iter_first(model, &iter); iter_valid;
    ...
    gtk_tree_model_get(model, &iter, COLUMN_SCALE, &this, -1);
  ...
  if (!iter_valid) {
    ...
    for (iter_valid = gtk_tree_model_get_iter_first(model, &sibling);
      ...
      gtk_tree_model_get(model, &sibling, COLUMN_SCALE, &this, -1);
  ...
  gtk_tree_model_get(model, &iter, COLUMN_PERSISTENT, &persistent, -1);
  if (!persistent) {
  ...
  g_object_set(entry, "width-chars", MAX(5, n_digits + 4), NULL);
}
