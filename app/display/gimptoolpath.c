...
enum { PROP_0, PROP_VECTORS, PROP_EDIT_MODE, PROP_POLYGONAL };

enum { BEGIN_CHANGE, END_CHANGE, ACTIVATE, LAST_SIGNAL };
...
static void gimp_tool_path_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_tool_path_get_property(GObject *object, guint property_id,
...
static GimpVectorFunction gimp_tool_path_get_function(GimpToolPath *path,
                                                      const GimpCoords *coords,
                                                      GdkModifierType state);
...
static void gimp_tool_path_begin_change(GimpToolPath *path, const gchar *desc);
static void gimp_tool_path_end_change(GimpToolPath *path, gboolean success);
...
static void gimp_tool_path_move_selected_anchors(GimpToolPath *path, gdouble x,
                                                 gdouble y);
static void gimp_tool_path_delete_selected_anchors(GimpToolPath *path);
...
static void gimp_tool_path_class_init(GimpToolPathClass *klass) {
  ...
  path_signals[BEGIN_CHANGE] =
      g_signal_new("begin-change", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpToolPathClass, begin_change), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, G_TYPE_STRING);

  path_signals[END_CHANGE] =
      g_signal_new("end-change", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpToolPathClass, end_change), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, G_TYPE_BOOLEAN);

  path_signals[ACTIVATE] =
      g_signal_new("activate", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpToolPathClass, activate), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, GDK_TYPE_MODIFIER_TYPE);

  g_object_class_install_property(
      object_class, PROP_VECTORS,
      g_param_spec_object("vectors", NULL, NULL, GIMP_TYPE_VECTORS,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_EDIT_MODE,
      g_param_spec_enum("edit-mode", _("Edit Mode"), NULL,
                        GIMP_TYPE_VECTOR_MODE, GIMP_VECTOR_MODE_DESIGN,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_POLYGONAL,
      g_param_spec_boolean("polygonal", _("Polygonal"),
                           _("Restrict editing to polygons"), FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_tool_path_init(GimpToolPath *path) {
...
private
  ->path = gimp_tool_widget_add_path(widget, NULL);
...
static void gimp_tool_path_set_property(GObject *object, guint property_id,
  ...
  case PROP_EDIT_MODE:
  private
    ->edit_mode = g_value_get_enum(value);
    break;
  case PROP_POLYGONAL:
  private
    ->polygonal = g_value_get_boolean(value);
    break;
...
static void item_remove_func(GimpCanvasItem *item, GimpToolWidget *widget) {
...
static void gimp_tool_path_changed(GimpToolWidget *widget) {
  ...
  if (vectors && gimp_vectors_get_bezier(vectors)) {
    ...
    gimp_canvas_path_set(private->path, gimp_vectors_get_bezier(vectors));
    ...
    for (cur_stroke = gimp_vectors_stroke_get_next(vectors, NULL); cur_stroke;
      ...
      for (list = draw_anchors; list; list = g_list_next(list)) {
        ...
        if (cur_anchor->type == GIMP_ANCHOR_ANCHOR) {
          item = gimp_tool_widget_add_handle(
              widget,
              cur_anchor->selected ? GIMP_HANDLE_CIRCLE
                                   : GIMP_HANDLE_FILLED_CIRCLE,
              cur_anchor->position.x, cur_anchor->position.y,
              GIMP_CANVAS_HANDLE_SIZE_CIRCLE, GIMP_CANVAS_HANDLE_SIZE_CIRCLE,
          ...
          if (first) {
            ...
            for (next = gimp_stroke_anchor_get_next(cur_stroke, cur_anchor);
                 next; next = gimp_stroke_anchor_get_next(cur_stroke, next)) {
            ...
            if (next) {
              angle = atan2(next->position.y - cur_anchor->position.y,
                            next->position.x - cur_anchor->position.x);
              g_object_set(item, "type",
                           (cur_anchor->selected ? GIMP_HANDLE_DROP
                                                 : GIMP_HANDLE_FILLED_DROP),
                           "start-angle", angle, NULL);
            }
          }

        private
          ->items = g_list_prepend(private->items, item);
      ...
      if (private->sel_count <= 2) {
          ...
          if (coords->len % 2 == 0) {
            ...
            for (i = 0; i < coords->len; i += 2) {
              item = gimp_tool_widget_add_line(
                  widget, g_array_index(coords, GimpCoords, i).x,
        ...
        for (list = draw_anchors; list; list = g_list_next(list)) {
          ...
          item = gimp_tool_widget_add_handle(
              widget, GIMP_HANDLE_SQUARE, cur_anchor->position.x,
              cur_anchor->position.y, GIMP_CANVAS_HANDLE_SIZE_CIRCLE - 3,
              GIMP_CANVAS_HANDLE_SIZE_CIRCLE - 3, GIMP_HANDLE_ANCHOR_CENTER);
...
static gboolean gimp_tool_path_check_writable(GimpToolPath *path) {
  GimpToolPathPrivate *private = path->private;
...
gboolean gimp_tool_path_button_press(GimpToolWidget *widget,
                                     const GimpCoords *coords, guint32 time,
                                     GdkModifierType state,
                                     GimpButtonPressType press_type) {
  GimpToolPath *path = GIMP_TOOL_PATH(widget);
...
private
  ...
  if (private->function == VECTORS_SELECT_VECTOR) {
    ...
    if (gimp_canvas_item_on_vectors(private->path, coords,
                                    GIMP_CANVAS_HANDLE_SIZE_CIRCLE,
                                    GIMP_CANVAS_HANDLE_SIZE_CIRCLE, NULL, NULL,
                                    NULL, NULL, NULL, &vectors)) {
  ...
  private
    ->cur_anchor = gimp_bezier_stroke_extend(
        private->sel_stroke, &position, private->sel_anchor, EXTEND_EDITABLE);

  private
    ->restriction = GIMP_ANCHOR_FEATURE_SYMMETRIC;

    if (!private->polygonal)
    private
    ->function = VECTORS_MOVE_HANDLE;
    else private->function = VECTORS_MOVE_ANCHOR;

  private
    ->cur_stroke = private->sel_stroke;
  }
  ...
  private
    ->cur_anchor = gimp_stroke_anchor_insert(
        private->cur_stroke, private->cur_anchor, private->cur_position);
    if (private->cur_anchor) {
      if (private->polygonal) {
        gimp_stroke_anchor_convert(private->cur_stroke, private->cur_anchor,
  ...
  if (private->function == VECTORS_MOVE_HANDLE &&
    ...
    if (private->cur_anchor->type == GIMP_ANCHOR_ANCHOR) {
      if (!private->cur_anchor->selected) {
        gimp_vectors_anchor_select(private->vectors, private->cur_stroke,
                                   private->cur_anchor, TRUE, TRUE);
      private
        ->undo_motion = TRUE;
      }

      gimp_canvas_item_on_vectors_handle(
          private->path, private->vectors, coords,
          GIMP_CANVAS_HANDLE_SIZE_CIRCLE, GIMP_CANVAS_HANDLE_SIZE_CIRCLE,
          GIMP_ANCHOR_CONTROL, TRUE, &private->cur_anchor,
          &private->cur_stroke);
      if (!private->cur_anchor)
      private
      ->function = VECTORS_FINISHED;
  ...
  if (private->function == VECTORS_MOVE_ANCHORSET &&
    ...
    if (state & TOGGLE_MASK) {
      gimp_vectors_anchor_select(private->vectors, private->cur_stroke,
                                 private->cur_anchor,
                                 !private->cur_anchor->selected, FALSE);
    private
      ->undo_motion = TRUE;
  ...
  private
    ...
    gimp_stroke_connect_stroke(private->sel_stroke, private->sel_anchor,
                               private->cur_stroke, private->cur_anchor);

    if (private->cur_stroke != private->sel_stroke &&
        gimp_stroke_is_empty(private->cur_stroke)) {
      gimp_vectors_stroke_remove(private->vectors, private->cur_stroke);
    }

  private
    ->sel_anchor = private->cur_anchor;
  private
    ->cur_stroke = private->sel_stroke;

    gimp_vectors_anchor_select(private->vectors, private->sel_stroke,
  ...
  private
    ...
    new_stroke = gimp_stroke_open(private->cur_stroke, private->cur_anchor);
    if (new_stroke) {
...
void gimp_tool_path_button_release(GimpToolWidget *widget,
                                   const GimpCoords *coords, guint32 time,
                                   GdkModifierType state,
                                   GimpButtonReleaseType release_type) {
  GimpToolPath *path = GIMP_TOOL_PATH(widget);
...
private
  ...
  if (private->have_undo) {
    if (!private->undo_motion || release_type == GIMP_BUTTON_RELEASE_CANCEL) {
...
void gimp_tool_path_motion(GimpToolWidget *widget, const GimpCoords *coords,
                           guint32 time, GdkModifierType state) {
  GimpToolPath *path = GIMP_TOOL_PATH(widget);
  GimpToolPathPrivate *private = path->private;
  GimpCoords position = GIMP_COORDS_DEFAULT_VALUES;
  ...
  if (!private->modifier_lock) {
    ...
    private
      ->restriction = GIMP_ANCHOR_FEATURE_NONE;
    }
  }
  ...
  case VECTORS_MOVE_HANDLE:
    ...
    if (anchor) {
      gimp_stroke_anchor_move_absolute(private->cur_stroke, private->cur_anchor,
                                       &position, private->restriction);
    private
      ->undo_motion = TRUE;
    }
    break;
  ...
  case VECTORS_MOVE_CURVE:
    if (private->polygonal) {
      gimp_tool_path_move_selected_anchors(path, coords->x - private->last_x,
                                           coords->y - private->last_y);
    private
      ->undo_motion = TRUE;
    } else {
      gimp_stroke_point_move_absolute(private->cur_stroke, private->cur_anchor,
  ...
  case VECTORS_MOVE_STROKE:
    if (private->cur_stroke) {
      gimp_stroke_translate(private->cur_stroke, coords->x - private->last_x,
                            coords->y - private->last_y);
    private
      ->undo_motion = TRUE;
    } else if (private->sel_stroke) {
      gimp_stroke_translate(private->sel_stroke, coords->x - private->last_x,
                            coords->y - private->last_y);
    private
      ->undo_motion = TRUE;
    }
...
GimpHit gimp_tool_path_hit(GimpToolWidget *widget, const GimpCoords *coords,
                           GdkModifierType state, gboolean proximity) {
  GimpToolPath *path = GIMP_TOOL_PATH(widget);
...
void gimp_tool_path_hover(GimpToolWidget *widget, const GimpCoords *coords,
                          GdkModifierType state, gboolean proximity) {
  GimpToolPath *path = GIMP_TOOL_PATH(widget);
  GimpToolPathPrivate *private = path->private;

private
  ->function = gimp_tool_path_get_function(path, coords, state);
...
static gboolean gimp_tool_path_key_press(GimpToolWidget *widget,
  ...
  case GDK_KEY_ISO_Enter:
    g_signal_emit(path, path_signals[ACTIVATE], 0, kevent->state);
    break;
  ...
  case GDK_KEY_Escape:
    if (private->edit_mode != GIMP_VECTOR_MODE_DESIGN)
      g_object_set(private, "vectors-edit-mode", GIMP_VECTOR_MODE_DESIGN, NULL);
    break;
...
static GimpUIManager *gimp_tool_path_get_popup(GimpToolWidget *widget,
  ...
  if (!private->ui_manager) {
    ...
    dialog_factory = gimp_dock_container_get_dialog_factory(
        GIMP_DOCK_CONTAINER(image_window));

  private
    ->ui_manager = gimp_menu_factory_manager_new(
...
private
  ...
  if (private->vectors) {
    on_handle = gimp_canvas_item_on_vectors_handle(
        private->path, private->vectors, coords, GIMP_CANVAS_HANDLE_SIZE_CIRCLE,
        GIMP_CANVAS_HANDLE_SIZE_CIRCLE, GIMP_ANCHOR_ANCHOR,
        private->sel_count > 2, &anchor, &stroke);

    if (!on_handle)
      on_curve = gimp_canvas_item_on_vectors_curve(
          private->path, private->vectors, coords,
          GIMP_CANVAS_HANDLE_SIZE_CIRCLE, GIMP_CANVAS_HANDLE_SIZE_CIRCLE, NULL,
          &position, &anchor, &anchor2, &stroke);
  }

  if (!on_handle && !on_curve) {
    on_vectors = gimp_canvas_item_on_vectors(
        private->path, coords, GIMP_CANVAS_HANDLE_SIZE_CIRCLE,
        GIMP_CANVAS_HANDLE_SIZE_CIRCLE, NULL, NULL, NULL, NULL, NULL, NULL);
  }
...
private
  ...
  case GIMP_VECTOR_MODE_DESIGN:
    if (!private->vectors) {
      ...
      if (anchor->type == GIMP_ANCHOR_ANCHOR) {
        ...
        private
        ->restriction = GIMP_ANCHOR_FEATURE_SYMMETRIC;
        else private->restriction = GIMP_ANCHOR_FEATURE_NONE;
      }
    } else if (on_curve) {
      ...
      if (private->sel_stroke && private->sel_anchor &&
          gimp_stroke_is_extendable(private->sel_stroke, private->sel_anchor) &&
          !(state & TOGGLE_MASK))
        function = VECTORS_ADD_ANCHOR;
      else
        function = VECTORS_CREATE_STROKE;
  ...
  case GIMP_VECTOR_MODE_EDIT:
    if (!private->vectors) {
      ...
      if (anchor->type == GIMP_ANCHOR_ANCHOR) {
        if (!(state & TOGGLE_MASK) && private->sel_anchor &&
...
static void gimp_tool_path_update_status(GimpToolPath *path,
  ...
  case VECTORS_ADD_ANCHOR:
    status = gimp_suggest_modifiers(_("Click or Click-Drag to create "
                                      "a new anchor"),
                                    extend_mask & ~state, NULL, NULL, NULL);
    free_status = TRUE;
    break;
  ...
  case VECTORS_MOVE_ANCHOR:
    if (private->edit_mode != GIMP_VECTOR_MODE_EDIT) {
      status = gimp_suggest_modifiers(_("Click-Drag to move the "
                                        "anchor around"),
                                      toggle_mask & ~state, NULL, NULL, NULL);
      free_status = TRUE;
    } else
...
static void gimp_tool_path_begin_change(GimpToolPath *path, const gchar *desc) {
  ...
  g_signal_emit(path, path_signals[BEGIN_CHANGE], 0, desc);
...
private
  ...
  g_signal_emit(path, path_signals[END_CHANGE], 0, success);
}
...
private
  ...
  while ((cur_stroke =
              gimp_vectors_stroke_get_next(private->vectors, cur_stroke))) {
    GList *anchors;
    GList *list;
    ...
    for (list = anchors; list; list = g_list_next(list)) {
      ...
      if (cur_anchor->type == GIMP_ANCHOR_ANCHOR && cur_anchor->selected) {
      private
        ->sel_count++;
  ...
  while ((cur_stroke =
    ...
    for (list = anchors; list; list = g_list_next(list)) {
      ...
      if (cur_anchor->selected)
        gimp_stroke_anchor_move_relative(cur_stroke, cur_anchor, &offset,
...
GimpToolWidget *gimp_tool_path_new(GimpDisplayShell *shell) {
  ...
  return g_object_new(GIMP_TYPE_TOOL_PATH, "shell", shell, NULL);
}

void gimp_tool_path_set_vectors(GimpToolPath *path, GimpVectors *vectors) {
...
private
  ...
  if (private->vectors) {
    g_signal_handlers_disconnect_by_func(private->vectors,
                                         gimp_tool_path_vectors_visible, path);
    g_signal_handlers_disconnect_by_func(private->vectors,
                                         gimp_tool_path_vectors_freeze, path);
    g_signal_handlers_disconnect_by_func(private->vectors,
                                         gimp_tool_path_vectors_thaw, path);
...
private
  ...
  if (private->vectors) {
    ...
    g_signal_connect_object(private->vectors, "visibility-changed",
                            G_CALLBACK(gimp_tool_path_vectors_visible), path,
                            0);
    g_signal_connect_object(private->vectors, "freeze",
                            G_CALLBACK(gimp_tool_path_vectors_freeze), path, 0);
    g_signal_connect_object(private->vectors, "thaw",
                            G_CALLBACK(gimp_tool_path_vectors_thaw), path, 0);
  }

  g_object_notify(G_OBJECT(path), "vectors");
}

void gimp_tool_path_get_popup_state(GimpToolPath *path, gboolean *on_handle,
                                    gboolean *on_curve) {
  GimpToolPathPrivate *private = path->private;
