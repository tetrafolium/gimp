...
void gimp_display_shell_filter_set(GimpDisplayShell *shell,
  ...
  if (shell->filter_stack) {
    g_signal_handlers_disconnect_by_func(
        shell->filter_stack, gimp_display_shell_filter_changed, shell);
  }
  ...
  if (shell->filter_stack) {
    g_signal_connect(shell->filter_stack, "changed",
                     G_CALLBACK(gimp_display_shell_filter_changed), shell);
  }
...
static gboolean gimp_display_shell_filter_changed_idle(gpointer data) {
...
static void gimp_display_shell_filter_changed(GimpColorDisplayStack *stack,
  ...
  shell->filter_idle_id = g_idle_add_full(
      G_PRIORITY_LOW, gimp_display_shell_filter_changed_idle, shell, NULL);
}
