...
enum { PROP_0, PROP_GRID, PROP_GRID_STYLE };
...
#define GET_PRIVATE(grid)                                                      \
  ((GimpCanvasGridPrivate *)gimp_canvas_grid_get_instance_private(             \
      (GimpCanvasGrid *)(grid)))

/*  local function prototypes  */

static void gimp_canvas_grid_finalize(GObject *object);
static void gimp_canvas_grid_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_canvas_grid_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);
static void gimp_canvas_grid_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *gimp_canvas_grid_get_extents(GimpCanvasItem *item);
static void gimp_canvas_grid_stroke(GimpCanvasItem *item, cairo_t *cr);
...
static void gimp_canvas_grid_class_init(GimpCanvasGridClass *klass) {
  ...
  g_object_class_install_property(object_class, PROP_GRID_STYLE,
                                  g_param_spec_boolean("grid-style", NULL, NULL,
                                                       FALSE,
                                                       GIMP_PARAM_READWRITE));
}

static void gimp_canvas_grid_init(GimpCanvasGrid *grid) {
...
static void gimp_canvas_grid_set_property(GObject *object, guint property_id,
  ...
  private
    ->grid_style = g_value_get_boolean(value);
    break;
...
#define CROSSHAIR 2
  ...
  g_return_if_fail(xspacing >= 0.0 && yspacing >= 0.0);
  ...
  if (!gimp_display_shell_get_infinite_canvas(shell)) {
    ...
    gimp_display_shell_scale_get_image_unrotated_bounds(
        shell, &bounds.x, &bounds.y, &bounds.width, &bounds.height);

    if (!gegl_rectangle_intersect(&bounds, &bounds,
...
static cairo_region_t *gimp_canvas_grid_get_extents(GimpCanvasItem *item) {
  ...
  if (!gimp_display_shell_get_infinite_canvas(shell)) {
    ...
    gimp_canvas_item_untransform_viewport(item, &rectangle.x, &rectangle.y,
...
static void gimp_canvas_grid_stroke(GimpCanvasItem *item, cairo_t *cr) {
  ...
  if (private->grid_style) {
    ...
    gimp_canvas_set_grid_style(gimp_canvas_item_get_canvas(item), cr,
                               private->grid, shell->offset_x, shell->offset_y);
    cairo_stroke(cr);
  } else {
...
GimpCanvasItem *gimp_canvas_grid_new(GimpDisplayShell *shell, GimpGrid *grid) {
  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), NULL);
  g_return_val_if_fail(grid == NULL || GIMP_IS_GRID(grid), NULL);

  return g_object_new(GIMP_TYPE_CANVAS_GRID, "shell", shell, "grid", grid,
                      NULL);
}
