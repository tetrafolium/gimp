...
void gimp_display_shell_profile_update(GimpDisplayShell *shell) {
  ...
  if (!gimp_color_transform_can_gegl_copy(src_profile, filter_profile)) {
    shell->filter_transform = gimp_color_transform_new(
        src_profile, src_format, filter_profile, filter_format,
        GIMP_COLOR_RENDERING_INTENT_RELATIVE_COLORIMETRIC,
        GIMP_COLOR_TRANSFORM_FLAGS_BLACK_POINT_COMPENSATION |
            GIMP_COLOR_TRANSFORM_FLAGS_NOOPTIMIZE);
  }

  shell->profile_transform = gimp_widget_get_color_transform(
      gtk_widget_get_toplevel(GTK_WIDGET(shell)),
      gimp_display_shell_get_color_config(shell), filter_profile, filter_format,
      dest_format);
  ...
  if (shell->filter_transform || shell->profile_transform) {
    ...
    shell->profile_stride = w * babl_format_get_bytes_per_pixel(src_format);

    shell->profile_buffer = gegl_buffer_linear_new_from_data(
        shell->profile_data, src_format, GEGL_RECTANGLE(0, 0, w, h),
        GEGL_AUTO_ROWSTRIDE, (GDestroyNotify)gegl_free, shell->profile_data);
  }
}
...
gboolean gimp_display_shell_profile_can_convert_to_u8(GimpDisplayShell *shell) {
  ...
  if (image) {
    ...
    if (!gimp_display_shell_has_filter(shell))
      ...
      component_type =
          gimp_babl_format_get_component_type(shell->filter_format);
