...
void gimp_display_shell_scrollbars_update(GimpDisplayShell *shell) {
  ...
  g_object_set(shell->hsbdata, "value", (gdouble)shell->offset_x, "page-size",
               (gdouble)shell->disp_width, "page-increment",
               (gdouble)shell->disp_width / 2, NULL);
  ...
  g_object_set(shell->vsbdata, "value", (gdouble)shell->offset_y, "page-size",
               (gdouble)shell->disp_height, "page-increment",
               (gdouble)shell->disp_height / 2, NULL);
...
void gimp_display_shell_scrollbars_setup_horizontal(GimpDisplayShell *shell,
  ...
  gimp_display_shell_scale_get_image_bounds(shell, &bounds_x, NULL,
                                            &bounds_width, NULL);
  ...
  if (!gimp_display_shell_get_infinite_canvas(shell)) {
    ...
    gimp_display_shell_scale_get_image_bounding_box(
        shell, &bounding_box_x, NULL, &bounding_box_width, NULL);
  }
  ...
  g_object_set(shell->hsbdata, "lower", lower, "upper", upper, "step-increment",
               (gdouble)MAX(scale_x, MINIMUM_STEP_AMOUNT), NULL);
}
...
void gimp_display_shell_scrollbars_setup_vertical(GimpDisplayShell *shell,
  ...
  gimp_display_shell_scale_get_image_bounds(shell, NULL, &bounds_y, NULL,
                                            &bounds_height);
  ...
  g_object_set(shell->vsbdata, "lower", lower, "upper", upper, "step-increment",
               (gdouble)MAX(scale_y, MINIMUM_STEP_AMOUNT), NULL);
}
