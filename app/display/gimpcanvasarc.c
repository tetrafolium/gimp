...
static void gimp_canvas_arc_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_canvas_arc_get_property(GObject *object, guint property_id,
                                         GValue *value, GParamSpec *pspec);
static void gimp_canvas_arc_draw(GimpCanvasItem *item, cairo_t *cr);
...
static void gimp_canvas_arc_class_init(GimpCanvasArcClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_CENTER_X,
      g_param_spec_double("center-x", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                          GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_CENTER_Y,
      g_param_spec_double("center-y", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                          GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_RADIUS_X,
                                  g_param_spec_double("radius-x", NULL, NULL, 0,
                                                      GIMP_MAX_IMAGE_SIZE, 0,
                                                      GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_RADIUS_Y,
                                  g_param_spec_double("radius-y", NULL, NULL, 0,
                                                      GIMP_MAX_IMAGE_SIZE, 0,
                                                      GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      object_class, PROP_FILLED,
      g_param_spec_boolean("filled", NULL, NULL, FALSE, GIMP_PARAM_READWRITE));
}

static void gimp_canvas_arc_init(GimpCanvasArc *arc) {}
...
static void gimp_canvas_arc_set_property(GObject *object, guint property_id,
  ...
  case PROP_CENTER_X:
  private
    ->center_x = g_value_get_double(value);
    break;
  case PROP_CENTER_Y:
  private
    ->center_y = g_value_get_double(value);
    break;
  case PROP_RADIUS_X:
  private
    ->radius_x = g_value_get_double(value);
    break;
  case PROP_RADIUS_Y:
  private
    ->radius_y = g_value_get_double(value);
    break;
...
static void gimp_canvas_arc_transform(GimpCanvasItem *item, gdouble *center_x,
                                      gdouble *center_y, gdouble *radius_x,
                                      gdouble *radius_y) {
  GimpCanvasArcPrivate *private = GET_PRIVATE(item);
  gdouble x1, y1;
  gdouble x2, y2;

  gimp_canvas_item_transform_xy_f(item, private->center_x - private->radius_x,
                                  private->center_y - private->radius_y, &x1,
                                  &y1);
  gimp_canvas_item_transform_xy_f(item, private->center_x + private->radius_x,
                                  private->center_y + private->radius_y, &x2,
                                  &y2);
...
static void gimp_canvas_arc_draw(GimpCanvasItem *item, cairo_t *cr) {
  ...
  gimp_canvas_arc_transform(item, &center_x, &center_y, &radius_x, &radius_y);

  cairo_save(cr);
  cairo_translate(cr, center_x, center_y);
  cairo_scale(cr, radius_x, radius_y);
  gimp_cairo_arc(cr, 0.0, 0.0, 1.0, private->start_angle, private->slice_angle);
  cairo_restore(cr);
...
static cairo_region_t *gimp_canvas_arc_get_extents(GimpCanvasItem *item) {
  ...
  if (!private->filled && rectangle.width > 64 * 1.43 &&
...
GimpCanvasItem *gimp_canvas_arc_new(GimpDisplayShell *shell, gdouble center_x,
                                    gdouble center_y, gdouble radius_x,
                                    gdouble radius_y, gdouble start_angle,
                                    gdouble slice_angle, gboolean filled) {
  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), NULL);

  return g_object_new(GIMP_TYPE_CANVAS_ARC, "shell", shell, "center-x",
                      center_x, "center-y", center_y, "radius-x", radius_x,
                      "radius-y", radius_y, "start-angle", start_angle,
                      "slice-angle", slice_angle, "filled", filled, NULL);
}

void gimp_canvas_arc_set(GimpCanvasItem *arc, gdouble center_x,
                         gdouble center_y, gdouble radius_x, gdouble radius_y,
                         gdouble start_angle, gdouble slice_angle) {
  g_return_if_fail(GIMP_IS_CANVAS_ARC(arc));

  gimp_canvas_item_begin_change(arc);
  g_object_set(arc, "center-x", center_x, "center-y", center_y, "radius-x",
               radius_x, "radius-y", radius_y, "start-angle", start_angle,
               "slice-angle", slice_angle, NULL);
