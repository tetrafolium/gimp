...
void gimp_display_shell_title_update(GimpDisplayShell *shell) {
  ...
  shell->title_idle_id =
      g_idle_add(gimp_display_shell_update_title_idle, shell);
}
...
static gboolean gimp_display_shell_update_title_idle(gpointer data) {
  ...
  if (gimp_display_get_image(shell->display)) {
    ...
    g_object_set(shell, "title", title, "status", status, NULL);
  } else {
    g_object_set(shell, "title", GIMP_NAME, "status", " ", NULL);
  }

  return FALSE;
}

static const gchar *gimp_display_shell_title_image_type(GimpImage *image) {
  const gchar *name = "";
...
static const gchar *gimp_display_shell_title_image_precision(GimpImage *image) {
  const gchar *name = "";

  gimp_enum_get_value(GIMP_TYPE_PRECISION, gimp_image_get_precision(image),
                      NULL, NULL, &name, NULL);
...
static gint gimp_display_shell_format_title(GimpDisplayShell *shell,
                                            gchar *title, gint title_len,
                                            const gchar *format) {
  GimpImage *image;
  ...
  while (i < title_len && *format) {
    ...
    case '%':
      ...
      case 'z': /* user zoom factor (percentage) */
      {
        ...
        i += print(title, title_len, i, scale >= 0.15 ? "%.0f" : "%.2f",
                   100.0 * scale);
      } break;
      ...
      case 'l': /* number of layers */
        i += print(title, title_len, i, "%d", gimp_image_get_n_layers(image));
        break;
      ...
      case 'L': /* number of layers (long) */
      {
        ...
        i += print(title, title_len, i, ngettext("%d layer", "%d layers", num),
                   num);
      } break;
      ...
      case 'w': /* width in pixels */
        i += print(title, title_len, i, "%d", gimp_image_get_width(image));
        break;
      ...
      case 'h': /* height in pixels */
        i += print(title, title_len, i, "%d", gimp_image_get_height(image));
        break;

      case 'u': /* unit symbol */
        i +=
            print(title, title_len, i, "%s", gimp_unit_get_symbol(shell->unit));
        break;
      ...
      case 'X': /* drawable width in real world units */
        if (drawable && shell->unit != GIMP_UNIT_PIXEL) {
          ...
          g_snprintf(unit_format, sizeof(unit_format), "%%.%df",
                     gimp_unit_get_scaled_digits(shell->unit, xres));
          i += print(
              title, title_len, i, unit_format,
              gimp_pixels_to_units(gimp_item_get_width(GIMP_ITEM(drawable)),
                                   shell->unit, xres));
          break;
        }
        /* else fallthru */
      ...
      case 'Y': /* drawable height in real world units */
        if (drawable && shell->unit != GIMP_UNIT_PIXEL) {
          ...
          i += print(
              title, title_len, i, unit_format,
              gimp_pixels_to_units(gimp_item_get_height(GIMP_ITEM(drawable)),
                                   shell->unit, yres));
          break;
        }
        /* else fallthru */
      ...
      case 'e': /* display's offsets in pixels */
      {
        ...
        i += print(title, title_len, i,
                   scale >= 0.15 ? "%.0fx%.0f" : "%.2fx%.2f", offset_x,
                   offset_y);
      } break;
