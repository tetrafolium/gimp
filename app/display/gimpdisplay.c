...
enum { PROP_0, PROP_IMAGE, PROP_SHELL };
...
static void gimp_display_set_property(GObject *object, guint property_id,
                                      const GValue *value, GParamSpec *pspec);
static void gimp_display_get_property(GObject *object, guint property_id,
...
static void gimp_display_paint_area(GimpDisplay *display, gint x, gint y,
                                    gint w, gint h);

G_DEFINE_TYPE_WITH_CODE(GimpDisplayImpl, gimp_display_impl, GIMP_TYPE_DISPLAY,
                        G_ADD_PRIVATE(GimpDisplayImpl) G_IMPLEMENT_INTERFACE(
                            GIMP_TYPE_PROGRESS,
                            gimp_display_progress_iface_init))

#define parent_class gimp_display_impl_parent_class

static void gimp_display_impl_class_init(GimpDisplayImplClass *klass) {
  ...
  g_object_class_install_property(object_class, PROP_SHELL,
                                  g_param_spec_object("shell", NULL, NULL,
                                                      GIMP_TYPE_DISPLAY_SHELL,
                                                      GIMP_PARAM_READABLE));
}

static void gimp_display_impl_init(GimpDisplayImpl *display) {
...
static GimpProgress *gimp_display_progress_start(GimpProgress *progress,
  ...
  if (display->priv->shell)
    return gimp_progress_start(GIMP_PROGRESS(display->priv->shell), cancellable,
                               "%s", message);
...
static void gimp_display_progress_set_value(GimpProgress *progress,
  ...
  if (display->priv->shell)
    gimp_progress_set_value(GIMP_PROGRESS(display->priv->shell), percentage);
}
...
GimpDisplay *gimp_display_new(Gimp *gimp, GimpImage *image, GimpUnit unit,
  ...
  display = g_object_new(GIMP_TYPE_DISPLAY_IMPL, "gimp", gimp, NULL);

private
  = GIMP_DISPLAY_IMPL(display)->priv;
  ...
  if (!window) {
    window =
        gimp_image_window_new(gimp, private->image, dialog_factory, monitor);
  }

  /*  create the shell for the image  */
private
  ->shell =
      gimp_display_shell_new(display, unit, scale, popup_manager, monitor);
  ...
  g_signal_connect(gimp_display_shell_get_statusbar(shell), "cancel",
                   G_CALLBACK(gimp_display_progress_canceled), display);
...
private
  ...
  private
    ->instance = gimp_image_get_instance_count(image);
...
void gimp_display_fill(GimpDisplay *display, GimpImage *image, GimpUnit unit,
                       gdouble scale) {
  GimpDisplayImplPrivate *private;
...
void gimp_display_update_bounding_box(GimpDisplay *display) {
  GimpDisplayImplPrivate *private;
  GimpDisplayShell *shell;
  GeglRectangle bounding_box = {};
...
private
  ...
  if (shell) {
    ...
    if (!gegl_rectangle_equal(&bounding_box, &private->bounding_box)) {
      ...
      n_diff_rects = gegl_rectangle_subtract(diff_rects, &private->bounding_box,
                                             &bounding_box);

      for (i = 0; i < n_diff_rects; i++) {
        gimp_display_paint_area(display, diff_rects[i].x, diff_rects[i].y,
...
void gimp_display_update_area(GimpDisplay *display, gboolean now, gint x,
...
private
  ...
  if (now) {
    ...
    private
    ->update_region = cairo_region_create_rectangle(&rect);
  }
}
...
static void gimp_display_flush_update_region(GimpDisplay *display) {
  ...
  if (private->update_region) {
    ...
    for (i = 0; i < n_rects; i++) {
      ...
      cairo_region_get_rectangle(private->update_region, i, &rect);

      gimp_display_paint_area(display, rect.x, rect.y, rect.width, rect.height);
    }
...
static void gimp_display_paint_area(GimpDisplay *display, gint x, gint y,
  ...
  if (!gegl_rectangle_intersect(&rect, &private->bounding_box,
                                GEGL_RECTANGLE(x, y, w, h))) {
    return;
  }

  /*  display the area  */
  gimp_display_shell_transform_bounds(shell, rect.x, rect.y,
