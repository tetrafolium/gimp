...
enum { PROP_0, PROP_ANCHOR, PROP_X, PROP_Y };
...
#define GET_PRIVATE(progress)                                                  \
  ((GimpCanvasProgressPrivate *)gimp_canvas_progress_get_instance_private(     \
      (GimpCanvasProgress *)(progress)))
...
static void gimp_canvas_progress_get_property(GObject *object,
                                              ...
                                              GParamSpec *pspec);
static void gimp_canvas_progress_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *gimp_canvas_progress_get_extents(GimpCanvasItem *item);
static gboolean gimp_canvas_progress_hit(GimpCanvasItem *item, gdouble x,
                                         gdouble y);
...
static gboolean gimp_canvas_progress_message(GimpProgress *progress, Gimp *gimp,
                                             GimpMessageSeverity severity,
                                             const gchar *domain,
                                             const gchar *message);

G_DEFINE_TYPE_WITH_CODE(
    GimpCanvasProgress, gimp_canvas_progress, GIMP_TYPE_CANVAS_ITEM,
...
static void gimp_canvas_progress_class_init(GimpCanvasProgressClass *klass) {
...
static void gimp_canvas_progress_init(GimpCanvasProgress *progress) {}
...
static PangoLayout *gimp_canvas_progress_transform(GimpCanvasItem *item,
                                                   gdouble *x, gdouble *y,
  ...
  layout = gimp_canvas_get_layout(GIMP_CANVAS(canvas), "%s", private->text);
  ...
  *height += 3 * BORDER + 2 * RADIUS;
  ...
  gimp_canvas_item_shift_to_north_west(private->anchor, *x, *y, *width, *height,
                                       x, y);
...
static void gimp_canvas_progress_draw(GimpCanvasItem *item, cairo_t *cr) {
  ...
  cairo_line_to(cr, x + 2 * BORDER + 2 * RADIUS,
                y + height - BORDER - 2 * RADIUS);
  ...
  pango_cairo_show_layout(
      cr, gimp_canvas_get_layout(GIMP_CANVAS(canvas), "%s", private->text));

  gimp_canvas_set_tool_bg_style(gimp_canvas_item_get_canvas(item), cr);
  cairo_arc(cr, x + BORDER + RADIUS, y + height - BORDER - RADIUS, RADIUS,
            -G_PI / 2.0, 2 * G_PI - G_PI / 2.0);
  cairo_fill(cr);

  cairo_set_source_rgba(cr, 0.0, 1.0, 0.0, 1.0);
  cairo_move_to(cr, x + BORDER + RADIUS, y + height - BORDER - RADIUS);
  cairo_arc(cr, x + BORDER + RADIUS, y + height - BORDER - RADIUS, RADIUS,
            -G_PI / 2.0, 2 * G_PI * private->value - G_PI / 2.0);
  cairo_fill(cr);
}
...
static gboolean gimp_canvas_progress_hit(GimpCanvasItem *item, gdouble x,
  ...
  return (tx >= px && tx < (px + pwidth) && ty >= py && ty < (py + pheight));
}
...
private
  ->last_update_time = g_get_monotonic_time();
...
GimpCanvasItem *gimp_canvas_progress_new(GimpDisplayShell *shell,
  ...
  return g_object_new(GIMP_TYPE_CANVAS_PROGRESS, "shell", shell, "anchor",
                      anchor, "x", x, "y", y, NULL);
}
