...
void gimp_display_shell_canvas_realize(GtkWidget *canvas,
                                       GimpDisplayShell *shell) {
...
static gboolean gimp_display_shell_canvas_tick(GtkWidget *widget,
  ...
  if ((tick->prev_width != allocation.width) ||
    ...
    if (shell->zoom_on_resize && tick->prev_width > 64 &&
        tick->prev_height > 64 && allocation.width > 64 &&
       ...
       */
      scale *= (sqrt(SQR(allocation.width) + SQR(allocation.height)) /
                sqrt(SQR(tick->prev_width) + SQR(tick->prev_height)));
     ...
     */
    if (shell->size_allocate_from_configure_event && !shell->zoom_on_resize) {
      ...
      if (!gimp_display_shell_get_infinite_canvas(shell)) {
        gimp_display_shell_scroll_center_image(shell, center_horizontally,
                                               center_vertically);
      } else {
        gimp_display_shell_scroll_center_content(shell, center_horizontally,
                                                 center_vertically);
      }
      ...
      gimp_display_shell_scroll_set_offset(shell, target_offset_x,
                                           target_offset_y);
    }
...
void gimp_display_shell_canvas_size_allocate(GtkWidget *widget,
  ...
  gtk_widget_add_tick_callback(
      widget, (GtkTickCallback)gimp_display_shell_canvas_tick, tick, NULL);
}
  ...
  if (gdk_event_triggers_context_menu((GdkEvent *)bevent)) {
    ...
    if (window) {
      ...
      gimp_ui_manager_ui_popup_at_widget(
          manager, "/quick-mask-popup", widget, GDK_GRAVITY_EAST,
          GDK_GRAVITY_SOUTH_WEST, (GdkEvent *)bevent, NULL, NULL);
    }
...
void gimp_display_shell_quick_mask_toggled(GtkWidget *widget,
  ...
  if (active != gimp_image_get_quick_mask_state(image)) {
    ...
    if (window) {
      ...
      gimp_ui_manager_toggle_action(manager, "quick-mask", "quick-mask-toggle",
                                    active);
    }
  }
}
...
gboolean gimp_display_shell_navigation_button_press(GtkWidget *widget,
  ...
  if (bevent->type == GDK_BUTTON_PRESS && bevent->button == 1) {
    gimp_navigation_editor_popup(shell, widget, (GdkEvent *)bevent, bevent->x,
                                 bevent->y);
  }
...
static void gimp_display_shell_vadjustment_changed(GtkAdjustment *adjustment,
  ...
  if (!shell->scrolling)
    gimp_display_shell_scroll(
        shell, 0, gtk_adjustment_get_value(adjustment) - shell->offset_y);
}
...
static void gimp_display_shell_hadjustment_changed(GtkAdjustment *adjustment,
  ...
  if (!shell->scrolling)
    gimp_display_shell_scroll(
        shell, gtk_adjustment_get_value(adjustment) - shell->offset_x, 0);
}
...
gimp_display_shell_hscrollbar_change_value(GtkRange *range,
  ...
  if ((scroll == GTK_SCROLL_JUMP) || (scroll == GTK_SCROLL_PAGE_BACKWARD) ||
...
static void gimp_display_shell_canvas_draw_image(GimpDisplayShell *shell,
  ...
  gimp_display_shell_scale_get_image_unrotated_bounding_box(
      shell, &image_rect.x, &image_rect.y, &image_rect.width,
      &image_rect.height);

  gimp_display_shell_scale_get_image_unrotated_bounds(
      shell, &canvas_rect.x, &canvas_rect.y, &canvas_rect.width,
      &canvas_rect.height);
  ...
  if (shell->show_all) {
    ...
    if (gimp_display_shell_get_padding_in_show_all(shell)) {
      cairo_rectangle(cr, canvas_rect.x, canvas_rect.y, canvas_rect.width,
                      canvas_rect.height);
      cairo_clip(cr);
    }
  ...
  cairo_rectangle(cr, image_rect.x, image_rect.y, image_rect.width,
                  image_rect.height);
  cairo_clip(cr);

  gimp_display_shell_rotate_bounds(
      shell, image_rect.x, image_rect.y, image_rect.x + image_rect.width,
      image_rect.y + image_rect.height, &x1, &y1, &x2, &y2);
    ...
    if (shell->show_image) {
      ...
      for (i = 0; i < clip_rectangles->num_rectangles; i++) {
        ...
        if (gegl_rectangle_intersect(&rect, &rect, &rotated_image_rect)) {
          gimp_display_shell_draw_image(shell, cr, rect.x, rect.y, rect.width,
                                        rect.height);
        }
      }
    }
