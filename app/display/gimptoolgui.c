...
#define GET_PRIVATE(gui)                                                       \
  ((GimpToolGuiPrivate *)gimp_tool_gui_get_instance_private(                   \
      (GimpToolGui *)(gui)))

static void gimp_tool_gui_dispose(GObject *object);
static void gimp_tool_gui_finalize(GObject *object);

static void gimp_tool_gui_create_dialog(GimpToolGui *gui, GdkMonitor *monitor);
...
static void gimp_tool_gui_dialog_response(GtkWidget *dialog, gint response_id,
                                          GimpToolGui *gui);
...
static ResponseEntry *response_entry_find(GList *entries, gint response_id);
...
static void gimp_tool_gui_class_init(GimpToolGuiClass *klass) {
  ...
  signals[RESPONSE] =
      g_signal_new("response", G_OBJECT_CLASS_TYPE(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpToolGuiClass, response), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, G_TYPE_INT);
}

static void gimp_tool_gui_init(GimpToolGui *gui) {
 ...
 **/
GimpToolGui *gimp_tool_gui_new(GimpToolInfo *tool_info, const gchar *title,
                               const gchar *description, const gchar *icon_name,
                               const gchar *help_id, GdkMonitor *monitor,
                               gboolean overlay, ...) {
  GimpToolGui *gui;
...
private
  ->tool_info = g_object_ref(tool_info);
private
...
void gimp_tool_gui_set_title(GimpToolGui *gui, const gchar *title) {
  GimpToolGuiPrivate *private;
...
void gimp_tool_gui_set_description(GimpToolGui *gui, const gchar *description) {
  GimpToolGuiPrivate *private;
...
void gimp_tool_gui_set_icon_name(GimpToolGui *gui, const gchar *icon_name) {
  GimpToolGuiPrivate *private;
...
void gimp_tool_gui_set_help_id(GimpToolGui *gui, const gchar *help_id) {
  GimpToolGuiPrivate *private;
...
void gimp_tool_gui_set_shell(GimpToolGui *gui, GimpDisplayShell *shell) {
  GimpToolGuiPrivate *private;
...
private
  ...
  if (private->shell) {
    ...
    g_signal_handlers_disconnect_by_func(private->shell->canvas,
                                         gimp_tool_gui_canvas_resized, gui);
  }
...
private
  ...
  if (private->shell) {
    g_signal_connect(private->shell->canvas, "size-allocate",
                     G_CALLBACK(gimp_tool_gui_canvas_resized), gui);
...
void gimp_tool_gui_set_viewables(GimpToolGui *gui, GList *viewables) {
  GimpToolGuiPrivate *private;
  GList *iter;
...
private
  ->viewables = g_list_copy(viewables);

  if (private->viewables) {
    for (iter = private->viewables; iter; iter = iter->next)
      g_object_add_weak_pointer(G_OBJECT(iter->data), (gpointer)&iter->data);
  }

  gimp_tool_gui_update_viewable(gui);
}

void gimp_tool_gui_set_viewable(GimpToolGui *gui, GimpViewable *viewable) {
  ...
  if (private->overlay) {
    if (gtk_widget_get_parent(private->dialog)) {
      gtk_container_remove(
          GTK_CONTAINER(gtk_widget_get_parent(private->dialog)),
          private->dialog);
...
void gimp_tool_gui_set_overlay(GimpToolGui *gui, GdkMonitor *monitor,
                               gboolean overlay) {
  GimpToolGuiPrivate *private;
  gboolean visible;
...
void gimp_tool_gui_set_auto_overlay(GimpToolGui *gui, gboolean auto_overlay) {
  GimpToolGuiPrivate *private;
...
void gimp_tool_gui_set_focus_on_map(GimpToolGui *gui, gboolean focus_on_map) {
  GimpToolGuiPrivate *private;
...
private
  ->focus_on_map = focus_on_map ? TRUE : FALSE;
...
void gimp_tool_gui_add_buttons_valist(GimpToolGui *gui, va_list args) {
  const gchar *button_text;
  gint response_id;
...
void gimp_tool_gui_add_button(GimpToolGui *gui, const gchar *button_text,
                              gint response_id) {
  GimpToolGuiPrivate *private;
  ResponseEntry *entry;
...
private
  ->response_entries = g_list_append(private->response_entries, entry);
...
private
  ...
  g_return_if_fail(
      response_entry_find(private->response_entries, response_id) != NULL);

private
  ->default_response = response_id;

  if (private->overlay) {
    gimp_overlay_dialog_set_default_response(
        GIMP_OVERLAY_DIALOG(private->dialog), response_id);
  } else {
    gtk_dialog_set_default_response(GTK_DIALOG(private->dialog), response_id);
  }
}
...
private
  ...
  if (private->overlay) {
    gimp_overlay_dialog_set_response_sensitive(
        GIMP_OVERLAY_DIALOG(private->dialog), response_id, sensitive);
  } else {
    gtk_dialog_set_response_sensitive(GTK_DIALOG(private->dialog), response_id,
                                      sensitive);
  }
}
...
private
  ...
  for (response_id = va_arg(args, gint), i = 0; response_id != -1;
       response_id = va_arg(args, gint), i++) {
    ResponseEntry *entry =
        response_entry_find(private->response_entries, response_id);
...
static void gimp_tool_gui_create_dialog(GimpToolGui *gui, GdkMonitor *monitor) {
  ...
  private
    ->dialog =
        gimp_overlay_dialog_new(private->tool_info, private->description, NULL);
    ...
    if (private->default_response != -1)
      gimp_overlay_dialog_set_default_response(
          GIMP_OVERLAY_DIALOG(private->dialog), private->default_response);
  ...
  private
    ->dialog = gimp_tool_dialog_new(private->tool_info, monitor, private->title,
                                    private->description, private->icon_name,
    ...
    gtk_box_pack_start(
        GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(private->dialog))),
...
static void gimp_tool_gui_add_dialog_button(GimpToolGui *gui,
  ...
  if (private->overlay) {
    gimp_overlay_dialog_add_button(GIMP_OVERLAY_DIALOG(private->dialog),
                                   entry->button_text, entry->response_id);

    if (!entry->sensitive) {
      gimp_overlay_dialog_set_response_sensitive(
          GIMP_OVERLAY_DIALOG(private->dialog), entry->response_id, FALSE);
    }
  } else {
    gimp_dialog_add_button(GIMP_DIALOG(private->dialog), entry->button_text,
                           entry->response_id);
...
static void gimp_tool_gui_update_buttons(GimpToolGui *gui) {
  ...
  for (list = private->response_entries, i = 0; list;
  ...
  if (n_ids == n_alternatives) {
    if (private->overlay) {
      gimp_overlay_dialog_set_alternative_button_order(
          GIMP_OVERLAY_DIALOG(private->dialog), n_ids, ids);
    } else {
      gimp_dialog_set_alternative_button_order_from_array(
          GIMP_DIALOG(private->dialog), n_ids, ids);
    }
  }
...
static void gimp_tool_gui_dialog_response(GtkWidget *dialog, gint response_id,
  ...
  if (response_id == GIMP_RESPONSE_DETACH) {
    ...
    gimp_tool_gui_set_overlay(gui, gimp_widget_get_monitor(dialog), FALSE);
  } else {
    g_signal_emit(gui, signals[RESPONSE], 0, response_id);
  }
}
...
static void gimp_tool_gui_canvas_resized(GtkWidget *canvas,
  ...
  if (private->auto_overlay) {
    ...
    gimp_tool_gui_set_overlay(gui, gimp_widget_get_monitor(private->dialog),
                              overlay);
  }
}
