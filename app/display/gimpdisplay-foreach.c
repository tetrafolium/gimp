...
static void gimp_displays_image_dirty_callback(GimpImage *image,
                                               GimpDirtyMask dirty_mask,
                                               GimpContainer *container) {
  if (gimp_image_is_dirty(image) && gimp_image_get_display_count(image) > 0 &&
...
gimp_displays_dirty_images_disconnect(GimpContainer *dirty_container,
  ...
  handler = GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(dirty_container), "clean-handler"));
  gimp_container_remove_handler(global_container, handler);

  handler = GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(dirty_container), "dirty-handler"));
  gimp_container_remove_handler(global_container, handler);
}

static void gimp_displays_image_clean_callback(GimpImage *image,
                                               GimpDirtyMask dirty_mask,
                                               GimpContainer *container) {
...
GimpContainer *gimp_displays_get_dirty_images(Gimp *gimp) {
  ...
  if (gimp_displays_dirty(gimp)) {
    ...
    handler = gimp_container_add_handler(
        gimp->images, "clean", G_CALLBACK(gimp_displays_image_dirty_callback),
        container);
    g_object_set_data(G_OBJECT(container), "clean-handler",
                      GINT_TO_POINTER(handler));

    handler = gimp_container_add_handler(
        gimp->images, "dirty", G_CALLBACK(gimp_displays_image_dirty_callback),
        container);
    ...
    for (list = gimp_get_image_iter(gimp); list; list = g_list_next(list)) {
...
void gimp_displays_reconnect(Gimp *gimp, GimpImage *old, GimpImage *new) {
  GList *contexts = NULL;
  GList *list;
  ...
  for (list = gimp_get_display_iter(gimp); list; list = g_list_next(list)) {
    ...
    if (gtk_widget_is_drawable(GTK_WIDGET(shell))) {
      ...
      if (GTK_IS_WINDOW(toplevel)) {
        ...
        if ((state &
             (GDK_WINDOW_STATE_WITHDRAWN | GDK_WINDOW_STATE_ICONIFIED)) == 0) {
          visible++;
        }
      }
    }
...
void gimp_displays_set_busy(Gimp *gimp) {
  ...
  for (list = gimp_get_display_iter(gimp); list; list = g_list_next(list)) {
    GimpDisplayShell *shell = gimp_display_get_shell(GIMP_DISPLAY(list->data));
