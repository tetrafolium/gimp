...
enum { STROKE, HOVER, LAST_SIGNAL };

/*  local function prototypes  */

static void gimp_motion_buffer_dispose(GObject *object);
static void gimp_motion_buffer_finalize(GObject *object);
static void gimp_motion_buffer_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_motion_buffer_get_property(GObject *object, guint property_id,
...
static void gimp_motion_buffer_class_init(GimpMotionBufferClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  motion_buffer_signals[STROKE] =
      g_signal_new("stroke", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpMotionBufferClass, stroke), NULL, NULL,
                   gimp_marshal_VOID__POINTER_UINT_FLAGS, G_TYPE_NONE, 3,
                   G_TYPE_POINTER, G_TYPE_UINT, GDK_TYPE_MODIFIER_TYPE);

  motion_buffer_signals[HOVER] =
      g_signal_new("hover", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpMotionBufferClass, hover), NULL, NULL,
                   gimp_marshal_VOID__POINTER_FLAGS_BOOLEAN, G_TYPE_NONE, 3,
                   G_TYPE_POINTER, GDK_TYPE_MODIFIER_TYPE, G_TYPE_BOOLEAN);
...
static void gimp_motion_buffer_init(GimpMotionBuffer *buffer) {
...
GimpMotionBuffer *gimp_motion_buffer_new(void) {
  return g_object_new(GIMP_TYPE_MOTION_BUFFER, NULL);
}

void gimp_motion_buffer_begin_stroke(GimpMotionBuffer *buffer, guint32 time,
...
gboolean gimp_motion_buffer_motion_event(GimpMotionBuffer *buffer,
                                         GimpCoords *coords, guint32 time,
                                         gboolean event_fill) {
  ...
  if (buffer->last_motion_time == 0) {
    ...
    if (delta_time == 0) {
      ...
      coords->velocity =
          (buffer->last_coords.velocity *
           ...
           coords->velocity * MIN(SMOOTH_FACTOR, coords->velocity));
    ...
    if (((fabs(delta_x) > DIRECTION_RADIUS) &&
      ...
      while (((fabs(dir_delta_x) < DIRECTION_RADIUS) ||
             ...
             (x >= 0)) {
        last_dir_event = g_array_index(buffer->event_history, GimpCoords, x);
    ...
    if (delta_dir < -0.5) {
      ...
      coords->direction =
          (0.5 * coords->direction + 0.5 * buffer->last_coords.direction);
    }

    coords->direction = coords->direction - floor(coords->direction);

    /* do event fill for devices that do not provide enough events */
    if (distance >= EVENT_FILL_PRECISION && event_fill &&
...
#ifdef EVENT_VERBOSE
    g_printerr(
        "DIST: %f, DT:%f, Vel:%f, Press:%f,smooth_dd:%f, POS: (%f, %f)\n",
        distance, delta_time, buffer->last_coords.velocity, coords->pressure,
        distance - dist, coords->x, coords->y);
#endif
  }
...
void gimp_motion_buffer_request_stroke(GimpMotionBuffer *buffer,
  ...
  while (buffer->event_queue->len > keep) {
    ...
    g_signal_emit(buffer, motion_buffer_signals[STROKE], 0, &buf_coords, time,
                  event_state);
  }

  if (buffer->event_delay) {
    buffer->event_delay_timeout = g_timeout_add(
        50, (GSourceFunc)gimp_motion_buffer_event_queue_timeout, buffer);
  }
}
...
void gimp_motion_buffer_request_hover(GimpMotionBuffer *buffer,
  ...
  if (buffer->event_queue->len > 0) {
    GimpCoords buf_coords = g_array_index(buffer->event_queue, GimpCoords,
                                          buffer->event_queue->len - 1);

    g_signal_emit(buffer, motion_buffer_signals[HOVER], 0, &buf_coords, state,
                  proximity);
...
static void gimp_motion_buffer_interpolate_stroke(GimpMotionBuffer *buffer,
  ...
  gimp_coords_interpolate_catmull(catmull, EVENT_FILL_PRECISION / 2, ret_coords,
                                  NULL);

  /* Push the last actual event in history */
  gimp_motion_buffer_push_event_history(
      buffer, &g_array_index(buffer->event_queue, GimpCoords, 0));
...
gimp_motion_buffer_event_queue_timeout(GimpMotionBuffer *buffer) {
  ...
  if (buffer->event_queue->len > 0) {
    GimpCoords last_coords = g_array_index(buffer->event_queue, GimpCoords,
                                           buffer->event_queue->len - 1);

    gimp_motion_buffer_push_event_history(buffer, &last_coords);

    gimp_motion_buffer_request_stroke(buffer, buffer->last_active_state,
