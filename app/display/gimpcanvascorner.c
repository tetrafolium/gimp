...
#define GET_PRIVATE(corner)                                                    \
  ((GimpCanvasCornerPrivate *)gimp_canvas_corner_get_instance_private(         \
      (GimpCanvasCorner *)(corner)))

/*  local function prototypes  */

static void gimp_canvas_corner_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_canvas_corner_get_property(GObject *object, guint property_id,
                                            GValue *value, GParamSpec *pspec);
static void gimp_canvas_corner_draw(GimpCanvasItem *item, cairo_t *cr);
...
static void gimp_canvas_corner_class_init(GimpCanvasCornerClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("width", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                          GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_HEIGHT,
      g_param_spec_double("height", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                          GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      object_class, PROP_OUTSIDE,
      g_param_spec_boolean("outside", NULL, NULL, FALSE, GIMP_PARAM_READWRITE));
}

static void gimp_canvas_corner_init(GimpCanvasCorner *corner) {}
...
static void gimp_canvas_corner_set_property(GObject *object, guint property_id,
  ...
  private
    ->corner_width = g_value_get_int(value);
    break;
  case PROP_CORNER_HEIGHT:
  private
    ->corner_height = g_value_get_int(value);
    break;
  case PROP_OUTSIDE:
  private
    ->outside = g_value_get_boolean(value);
    break;
...
static void gimp_canvas_corner_transform(GimpCanvasItem *item, gdouble *x,
  ...
  gimp_canvas_item_transform_xy_f(
      item, MIN(private->x, private->x + private->width),
      MIN(private->y, private->y + private->height), &rx, &ry);
  gimp_canvas_item_transform_xy_f(
      item, MAX(private->x, private->x + private->width),
      MAX(private->y, private->y + private->height), &rw, &rh);
...
GimpCanvasItem *gimp_canvas_corner_new(GimpDisplayShell *shell, gdouble x,
  ...
  return g_object_new(GIMP_TYPE_CANVAS_CORNER, "shell", shell, "x", x, "y", y,
                      "width", width, "height", height, "anchor", anchor,
                      "corner-width", corner_width, "corner-height",
                      corner_height, "outside", outside, NULL);
}
...
void gimp_canvas_corner_set(GimpCanvasItem *corner, gdouble x, gdouble y,
  ...
  g_object_set(corner, "x", x, "y", y, "width", width, "height", height,
               "corner-width", corner_width, "corner-height", corner_height,
