...
enum { PROP_0, PROP_LAYERS, PROP_EDIT_MASK };
...
#define GET_PRIVATE(layer_boundary)                                            \
  ((GimpCanvasLayerBoundaryPrivate *)                                          \
       gimp_canvas_layer_boundary_get_instance_private(                        \
...
static void gimp_canvas_layer_boundary_draw(GimpCanvasItem *item, cairo_t *cr);
...
gimp_canvas_layer_boundary_class_init(GimpCanvasLayerBoundaryClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_LAYERS,
      g_param_spec_pointer("layers", NULL, NULL, GIMP_PARAM_READWRITE));
...
static void gimp_canvas_layer_boundary_set_property(GObject *object,
  ...
  private
    ->layers = g_list_copy(g_value_get_pointer(value));
    if (private->layers) {
      GList *iter;
      for (iter = private->layers; iter; iter = iter->next)
  ...
  case PROP_EDIT_MASK:
  private
    ->edit_mask = g_value_get_boolean(value);
    break;
...
GimpCanvasItem *gimp_canvas_layer_boundary_new(GimpDisplayShell *shell) {
  ...
  return g_object_new(GIMP_TYPE_CANVAS_LAYER_BOUNDARY, "shell", shell, NULL);
}
  ...
  if (changed) {
    ...
    for (iter = layers; iter; iter = iter->next) {
      if (iter->data) {
        ...
        x2 = MAX(x2, gimp_item_get_offset_x(item) + gimp_item_get_width(item));
        y2 = MAX(y2, gimp_item_get_offset_y(item) + gimp_item_get_height(item));
    ...
    if (layers)
      g_object_set(boundary, "x", (gdouble)x1, "y", (gdouble)y1, "width",
                   (gdouble)(x2 - x1), "height", (gdouble)(y2 - y1), NULL);
    g_object_set(boundary, "layers", layers, "edit-mask", edit_mask, NULL);
    ...
    g_object_get(boundary, "x", &x, "y", &y, "width", &w, "height", &h, NULL);
    ...
    if (x1 != (gint)x || x1 != (gint)y || (x2 - x1) != (gint)w ||
        (y2 - y1) != (gint)h || edit_mask != private->edit_mask) {
      gimp_canvas_item_begin_change(GIMP_CANVAS_ITEM(boundary));

      g_object_set(boundary, "x", (gdouble)x1, "y", (gdouble)y1, "width",
                   (gdouble)(x2 - x1), "height", (gdouble)(y2 - y1),
                   "edit-mask", edit_mask, NULL);
