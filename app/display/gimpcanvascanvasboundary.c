...
#define GET_PRIVATE(canvas_boundary)                                           \
  ((GimpCanvasCanvasBoundaryPrivate *)                                         \
       gimp_canvas_canvas_boundary_get_instance_private(                       \
...
static void gimp_canvas_canvas_boundary_draw(GimpCanvasItem *item, cairo_t *cr);
...
G_DEFINE_TYPE_WITH_PRIVATE(GimpCanvasCanvasBoundary,
...
GimpCanvasItem *gimp_canvas_canvas_boundary_new(GimpDisplayShell *shell) {
  ...
  return g_object_new(GIMP_TYPE_CANVAS_CANVAS_BOUNDARY, "shell", shell, NULL);
}
...
private
  ...
  if (image != private->image) {
    ...
    if (image) {
      g_object_set(boundary, "x", (gdouble)0, "y", (gdouble)0, "width",
                   (gdouble)gimp_image_get_width(image), "height",
                   (gdouble)gimp_image_get_height(image), NULL);
    }

    g_object_set(boundary, "image", image, NULL);
    ...
    if (lx != (gint)x || ly != (gint)y || lw != (gint)w || lh != (gint)h) {
      gimp_canvas_item_begin_change(GIMP_CANVAS_ITEM(boundary));

      g_object_set(boundary, "x", (gdouble)lx, "y", (gdouble)ly, "width",
                   (gdouble)lw, "height", (gdouble)lh, NULL);
