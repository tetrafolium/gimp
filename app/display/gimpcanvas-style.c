...
void gimp_canvas_set_guide_style(GtkWidget *canvas, cairo_t *cr,
                                 GimpGuideStyle style, gboolean active,
...
void gimp_canvas_set_sample_point_style(GtkWidget *canvas, cairo_t *cr,
                                        gboolean active) {
  g_return_if_fail(GTK_IS_WIDGET(canvas));
  g_return_if_fail(cr != NULL);
...
void gimp_canvas_set_grid_style(GtkWidget *canvas, cairo_t *cr, GimpGrid *grid,
                                gdouble offset_x, gdouble offset_y) {
  GimpRGB fg;
  GimpRGB bg;
  ...
  case GIMP_GRID_DOUBLE_DASH:
    if (grid->style == GIMP_GRID_DOUBLE_DASH) {
      ...
      pattern =
          gimp_cairo_pattern_create_stipple(&fg, &bg, 0, offset_x, offset_y);
    } else {
...
void gimp_canvas_set_pen_style(GtkWidget *canvas, cairo_t *cr,
                               const GimpRGB *color, gint width) {
  g_return_if_fail(GTK_IS_WIDGET(canvas));
  g_return_if_fail(cr != NULL);
...
void gimp_canvas_set_layer_style(GtkWidget *canvas, cairo_t *cr,
                                 GimpLayer *layer, gdouble offset_x,
                                 gdouble offset_y) {
  cairo_pattern_t *pattern;
  ...
  if (gimp_layer_get_mask(layer) && gimp_layer_get_edit_mask(layer)) {
    pattern = gimp_cairo_pattern_create_stipple(&layer_mask_fg, &layer_mask_bg,
                                                0, offset_x, offset_y);
  } else if (gimp_viewable_get_children(GIMP_VIEWABLE(layer))) {
    pattern = gimp_cairo_pattern_create_stipple(
        &layer_group_fg, &layer_group_bg, 0, offset_x, offset_y);
  } else {
...
void gimp_canvas_set_canvas_style(GtkWidget *canvas, cairo_t *cr,
                                  gdouble offset_x, gdouble offset_y) {
  cairo_pattern_t *pattern;
...
void gimp_canvas_set_selection_out_style(GtkWidget *canvas, cairo_t *cr,
                                         gdouble offset_x, gdouble offset_y) {
  cairo_pattern_t *pattern;
  ...
  pattern = gimp_cairo_pattern_create_stipple(
      &selection_out_fg, &selection_out_bg, 0, offset_x, offset_y);
  cairo_set_source(cr, pattern);
  cairo_pattern_destroy(pattern);
}

void gimp_canvas_set_selection_in_style(GtkWidget *canvas, cairo_t *cr,
                                        gint index, gdouble offset_x,
                                        gdouble offset_y) {
  cairo_pattern_t *pattern;
  ...
  pattern = gimp_cairo_pattern_create_stipple(
      &selection_in_fg, &selection_in_bg, index, offset_x, offset_y);
  cairo_set_source(cr, pattern);
  cairo_pattern_destroy(pattern);
}

void gimp_canvas_set_vectors_bg_style(GtkWidget *canvas, cairo_t *cr,
                                      gboolean active) {
  g_return_if_fail(GTK_IS_WIDGET(canvas));
  g_return_if_fail(cr != NULL);
...
void gimp_canvas_set_vectors_fg_style(GtkWidget *canvas, cairo_t *cr,
                                      gboolean active) {
  g_return_if_fail(GTK_IS_WIDGET(canvas));
  g_return_if_fail(cr != NULL);
...
void gimp_canvas_set_outline_bg_style(GtkWidget *canvas, cairo_t *cr) {
  g_return_if_fail(GTK_IS_WIDGET(canvas));
  g_return_if_fail(cr != NULL);
...
void gimp_canvas_set_outline_fg_style(GtkWidget *canvas, cairo_t *cr) {
...
void gimp_canvas_set_passe_partout_style(GtkWidget *canvas, cairo_t *cr) {
  g_return_if_fail(GTK_IS_WIDGET(canvas));
  g_return_if_fail(cr != NULL);
...
void gimp_canvas_set_tool_bg_style(GtkWidget *canvas, cairo_t *cr) {
  g_return_if_fail(GTK_IS_WIDGET(canvas));
  g_return_if_fail(cr != NULL);
...
void gimp_canvas_set_tool_fg_style(GtkWidget *canvas, cairo_t *cr,
                                   gboolean highlight) {
  g_return_if_fail(cr != NULL);
