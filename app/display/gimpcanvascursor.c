...
#define GET_PRIVATE(cursor)                                                    \
  ((GimpCanvasCursorPrivate *)gimp_canvas_cursor_get_instance_private(         \
      (GimpCanvasCursor *)(cursor)))

/*  local function prototypes  */

static void gimp_canvas_cursor_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_canvas_cursor_get_property(GObject *object, guint property_id,
                                            GValue *value, GParamSpec *pspec);
static void gimp_canvas_cursor_draw(GimpCanvasItem *item, cairo_t *cr);
...
static void gimp_canvas_cursor_class_init(GimpCanvasCursorClass *klass) {
...
static void gimp_canvas_cursor_init(GimpCanvasCursor *cursor) {
...
GimpCanvasItem *gimp_canvas_cursor_new(GimpDisplayShell *shell) {
  ...
  return g_object_new(GIMP_TYPE_CANVAS_CURSOR, "shell", shell, NULL);
}

void gimp_canvas_cursor_set(GimpCanvasItem *cursor, gdouble x, gdouble y) {
...
private
  ...
  if (private->x != x || private->y != y) {
    ...
    g_object_set(cursor, "x", x, "y", y, NULL);
