...
enum { PROP_0, PROP_SHELL, PROP_VISIBLE, PROP_LINE_CAP, PROP_HIGHLIGHT };
...
static void gimp_canvas_item_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_canvas_item_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);
static void gimp_canvas_item_dispatch_properties_changed(GObject *object,
                                                         guint n_pspecs,
                                                         GParamSpec **pspecs);

static void gimp_canvas_item_real_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *gimp_canvas_item_real_get_extents(GimpCanvasItem *item);
static void gimp_canvas_item_real_stroke(GimpCanvasItem *item, cairo_t *cr);
static void gimp_canvas_item_real_fill(GimpCanvasItem *item, cairo_t *cr);
static gboolean gimp_canvas_item_real_hit(GimpCanvasItem *item, gdouble x,
                                          gdouble y);
...
static void gimp_canvas_item_class_init(GimpCanvasItemClass *klass) {
  ...
  object_class->dispatch_properties_changed =
      gimp_canvas_item_dispatch_properties_changed;
  ...
  item_signals[UPDATE] =
      g_signal_new("update", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpCanvasItemClass, update), NULL, NULL,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean("visible", NULL, NULL, TRUE, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_LINE_CAP,
      g_param_spec_int("line-cap", NULL, NULL, CAIRO_LINE_CAP_BUTT,
...
static void gimp_canvas_item_init(GimpCanvasItem *item) {
...
private
  ->suspend_filling = 0;
private
  ->change_count = 1; /* avoid emissions during construction */
private
  ->change_region = NULL;
}
...
static void gimp_canvas_item_set_property(GObject *object, guint property_id,
  ...
  case PROP_LINE_CAP:
  private
    ->line_cap = g_value_get_int(value);
    break;
  case PROP_HIGHLIGHT:
  private
...
static void gimp_canvas_item_dispatch_properties_changed(GObject *object,
  ...
  if (_gimp_canvas_item_needs_update(item)) {
    ...
    if (region) {
      g_signal_emit(object, item_signals[UPDATE], 0, region);
...
void gimp_canvas_item_draw(GimpCanvasItem *item, cairo_t *cr) {
...
gboolean gimp_canvas_item_hit(GimpCanvasItem *item, gdouble x, gdouble y) {
...
void gimp_canvas_item_set_visible(GimpCanvasItem *item, gboolean visible) {
  g_return_if_fail(GIMP_IS_CANVAS_ITEM(item));

  if (item->private->visible != visible) {
    gimp_canvas_item_begin_change(item);
    g_object_set(G_OBJECT(item), "visible", visible, NULL);
...
void gimp_canvas_item_set_line_cap(GimpCanvasItem *item,
  ...
  if (item->private->line_cap != line_cap) {
    gimp_canvas_item_begin_change(item);
    g_object_set(G_OBJECT(item), "line-cap", line_cap, NULL);
    gimp_canvas_item_end_change(item);
  }
}

void gimp_canvas_item_set_highlight(GimpCanvasItem *item, gboolean highlight) {
  g_return_if_fail(GIMP_IS_CANVAS_ITEM(item));

  if (item->private->highlight != highlight) {
    g_object_set(G_OBJECT(item), "highlight", highlight, NULL);
  }
}
...
private
  ...
  private
    ->change_region = gimp_canvas_item_get_extents(item);
  }
}
...
private
  ...
  if (private->change_count == 0) {
    ...
    private
      ...
      if (region) {
        g_signal_emit(item, item_signals[UPDATE], 0, region);
        cairo_region_destroy(region);
      }
    } else {
...
void gimp_canvas_item_transform(GimpCanvasItem *item, cairo_t *cr) {
  GimpCanvasItemPrivate *private;
...
void gimp_canvas_item_transform_xy(GimpCanvasItem *item, gdouble x, gdouble y,
                                   gint *tx, gint *ty) {
  g_return_if_fail(GIMP_IS_CANVAS_ITEM(item));
...
void gimp_canvas_item_transform_xy_f(GimpCanvasItem *item, gdouble x, gdouble y,
 ...
 **/
gdouble gimp_canvas_item_transform_distance(GimpCanvasItem *item, gdouble x1,
                                            gdouble y1, gdouble x2,
                                            gdouble y2) {
  return sqrt(gimp_canvas_item_transform_distance_square(item, x1, y1, x2, y2));
}
 ...
 *          display coordinates
 **/
gdouble gimp_canvas_item_transform_distance_square(GimpCanvasItem *item,
...
void gimp_canvas_item_untransform_viewport(GimpCanvasItem *item, gint *x,
  ...
  gimp_display_shell_unrotate_bounds(shell, 0.0, 0.0, shell->disp_width,
                                     shell->disp_height, &x1, &y1, &x2, &y2);
...
void _gimp_canvas_item_update(GimpCanvasItem *item, cairo_region_t *region) {
  g_signal_emit(item, item_signals[UPDATE], 0, region);
}
...
void _gimp_canvas_item_stroke(GimpCanvasItem *item, cairo_t *cr) {
  if (item->private->suspend_filling > 0)
    g_warning(
        "_gimp_canvas_item_stroke() on an item that is in a filling group");
...
void _gimp_canvas_item_fill(GimpCanvasItem *item, cairo_t *cr) {
  if (item->private->suspend_stroking > 0)
    g_warning(
        "_gimp_canvas_item_fill() on an item that is in a stroking group");
