...
static void gimp_tool_polygon_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_tool_polygon_get_property(GObject *object, guint property_id,
...
static void gimp_tool_polygon_button_release(
...
static guint polygon_signals[LAST_SIGNAL] = {
    0,
};

static const GimpVector2 vector2_zero = {0.0, 0.0};

static void gimp_tool_polygon_class_init(GimpToolPolygonClass *klass) {
  ...
  polygon_signals[CHANGE_COMPLETE] = g_signal_new(
      "change-complete", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolPolygonClass, change_complete), NULL, NULL, NULL,
      G_TYPE_NONE, 0);
}

static void gimp_tool_polygon_init(GimpToolPolygon *polygon) {
...
private
  ->polygon = gimp_tool_widget_add_polygon(widget, NULL, NULL, 0, FALSE);

private
  ->pending_line = gimp_tool_widget_add_line(widget, 0, 0, 0, 0);
private
  ->closing_line = gimp_tool_widget_add_line(widget, 0, 0, 0, 0);
...
static void gimp_tool_polygon_get_segment(GimpToolPolygon *polygon,
                                          GimpVector2 **points, gint *n_points,
                                          gint segment_start,
                                          gint segment_end) {
  ...
  *n_points = priv->segment_indices[segment_end] -
              priv->segment_indices[segment_start] + 1;
}

static void gimp_tool_polygon_get_segment_point(GimpToolPolygon *polygon,
...
static gboolean gimp_tool_polygon_should_close(GimpToolPolygon *polygon,
                                               guint32 time,
                                               const GimpCoords *coords) {
  ...
  if (priv->polygon_modified || priv->n_segment_indices < 1 ||
      priv->n_points < 3 || priv->polygon_closed)
    return FALSE;

  dist = gimp_canvas_item_transform_distance_square(
      priv->polygon, coords->x, coords->y, priv->points[0].x,
      priv->points[0].y);
  ...
  if (time != NO_CLICK_TIME_AVAILABLE) {
    ...
    dist_from_last_point = gimp_canvas_item_transform_distance_square(
        priv->polygon, coords->x, coords->y, priv->last_click_coord.x,
...
static void gimp_tool_polygon_revert_to_last_segment(GimpToolPolygon *polygon) {
  GimpToolPolygonPrivate *priv = polygon->private;

  priv->n_points = priv->segment_indices[priv->n_segment_indices - 1] + 1;
}

static void gimp_tool_polygon_remove_last_segment(GimpToolPolygon *polygon) {
  GimpToolPolygonPrivate *priv = polygon->private;
  ...
  if (priv->n_segment_indices > 0) {
    ...
    handle = g_ptr_array_index(priv->handles, priv->n_segment_indices);
...
static void gimp_tool_polygon_add_point(GimpToolPolygon *polygon, gdouble x,
                                        gdouble y) {
  GimpToolPolygonPrivate *priv = polygon->private;
  ...
  if (priv->n_points >= priv->max_n_points) {
    ...
    priv->points =
        g_realloc(priv->points, sizeof(GimpVector2) * priv->max_n_points);
  }
...
static void gimp_tool_polygon_add_segment_index(GimpToolPolygon *polygon,
                                                gint index) {
  GimpToolPolygonPrivate *priv = polygon->private;
  ...
  if (priv->n_segment_indices >= priv->max_n_segment_indices) {
    ...
    priv->segment_indices =
        g_realloc(priv->segment_indices,
...
static gboolean gimp_tool_polygon_is_point_grabbed(GimpToolPolygon *polygon) {
  GimpToolPolygonPrivate *priv = polygon->private;

  return priv->grabbed_segment_index != INVALID_INDEX;
}

static void gimp_tool_polygon_fit_segment(GimpToolPolygon *polygon,
  ...
  /* Transform the destination end point */
  {
    ...
    /* Transate to origin */
    gimp_vector2_sub(&origo_translation_offset, &vector2_zero, &dest_points[0]);
    gimp_vector2_add(dest_end, dest_end, &origo_translation_offset);

    /* Calculate origo_translated_end_target */
    gimp_vector2_sub(&origo_translated_end_target, &dest_end_target,
                     &dest_start_target);
    ...
    current_rotation =
        atan2(vector2_zero.y - dest_end->y, vector2_zero.x - dest_end->x);
    ...
    /* Untranslate */
    gimp_vector2_sub(&untranslation_offset, &dest_end_target, dest_end);
    gimp_vector2_add(dest_end, dest_end, &untranslation_offset);
  }
  ...
  /* Do the same transformation for the rest of the points */
  {
    ...
    for (i = 0; i < n_points - 1; i++) {
      /* Translate */
      gimp_vector2_add(&dest_points[i], &origo_translation_offset,
                       &dest_points[i]);

      /* Rotate */
      gimp_vector2_rotate(&dest_points[i], rotation);

      /* Scale */
      gimp_vector2_mul(&dest_points[i], scale);

      /* Untranslate */
      gimp_vector2_add(&dest_points[i], &dest_points[i], &untranslation_offset);
    }
  }
}

static void gimp_tool_polygon_move_segment_vertex_to(GimpToolPolygon *polygon,
                                                     gint segment_index,
                                                     gdouble new_x,
                                                     gdouble new_y) {
  GimpToolPolygonPrivate *priv = polygon->private;
  ...
  if (segment_index > 0) {
    gimp_tool_polygon_get_segment(polygon, &dest, &n_points,
    ...
    gimp_tool_polygon_fit_segment(polygon, dest, *dest_start_target,
                                  *dest_end_target,
                                  priv->saved_points_lower_segment, n_points);
  }
  ...
  if (segment_index < priv->n_segment_indices - 1) {
    ...
    gimp_tool_polygon_fit_segment(polygon, dest, *dest_start_target,
                                  *dest_end_target,
                                  priv->saved_points_higher_segment, n_points);
  }

  /* Handle when there only is one point */
  if (segment_index == 0 && priv->n_segment_indices == 1) {
    priv->points[0].x = new_x;
    priv->points[0].y = new_y;
  }
}

static void gimp_tool_polygon_revert_to_saved_state(GimpToolPolygon *polygon) {
  GimpToolPolygonPrivate *priv = polygon->private;
  GimpVector2 *dest;
  gint n_points;
  ...
  if (priv->grabbed_segment_index > 0) {
    ...
    memcpy(dest, priv->saved_points_lower_segment,
  ...
  if (priv->grabbed_segment_index < priv->n_segment_indices - 1) {
    ...
    memcpy(dest, priv->saved_points_higher_segment,
           sizeof(GimpVector2) * n_points);
  }

  if (priv->grabbed_segment_index == 0 && priv->n_segment_indices == 1) {
    priv->points[0] = *priv->saved_points_lower_segment;
  }
}

static void gimp_tool_polygon_prepare_for_move(GimpToolPolygon *polygon) {
  GimpToolPolygonPrivate *priv = polygon->private;
  GimpVector2 *source;
  gint n_points;

  if (priv->grabbed_segment_index > 0) {
    gimp_tool_polygon_get_segment(polygon, &source, &n_points,
                                  priv->grabbed_segment_index - 1,
                                  priv->grabbed_segment_index);
    ...
    if (n_points > priv->max_n_saved_points_lower_segment) {
      ...
      priv->saved_points_lower_segment = g_realloc(
          priv->saved_points_lower_segment, sizeof(GimpVector2) * n_points);
    }

    memcpy(priv->saved_points_lower_segment, source,
           sizeof(GimpVector2) * n_points);
  }
  ...
  if (priv->grabbed_segment_index < priv->n_segment_indices - 1) {
    ...
    if (n_points > priv->max_n_saved_points_higher_segment) {
      ...
      priv->saved_points_higher_segment = g_realloc(
          priv->saved_points_higher_segment, sizeof(GimpVector2) * n_points);
    }

    memcpy(priv->saved_points_higher_segment, source,
           sizeof(GimpVector2) * n_points);
  }
...
static void gimp_tool_polygon_update_motion(GimpToolPolygon *polygon,
  ...
  if (gimp_tool_polygon_is_point_grabbed(polygon)) {
    ...
    if (priv->constrain_angle && priv->n_segment_indices > 1) {
      gdouble start_point_x;
      gdouble start_point_y;
      gint segment_index;
      ...
      gimp_tool_polygon_get_segment_point(polygon, &start_point_x,
                                          &start_point_y, segment_index);

      gimp_display_shell_constrain_line(
          gimp_tool_widget_get_shell(GIMP_TOOL_WIDGET(polygon)), start_point_x,
          start_point_y, &new_x, &new_y, GIMP_CONSTRAIN_LINE_15_DEGREES);
    }

    gimp_tool_polygon_move_segment_vertex_to(
        polygon, priv->grabbed_segment_index, new_x, new_y);
...
static void gimp_tool_polygon_status_update(GimpToolPolygon *polygon,
                                            const GimpCoords *coords,
                                            gboolean proximity) {
  GimpToolWidget *widget = GIMP_TOOL_WIDGET(polygon);
  GimpToolPolygonPrivate *priv = polygon->private;
  ...
  if (proximity) {
    ...
    if (gimp_tool_polygon_is_point_grabbed(polygon)) {
      if (gimp_tool_polygon_should_close(polygon, NO_CLICK_TIME_AVAILABLE,
                                         coords)) {
      ...
      status_text =
          _("Return commits, Escape cancels, Backspace re-opens shape");
    } else if (priv->n_segment_indices >= 3) {
      status_text =
          _("Return commits, Escape cancels, Backspace removes last segment");
    } else {
      status_text =
          _("Click-Drag adds a free segment, Click adds a polygonal segment");
    }
...
static void gimp_tool_polygon_changed(GimpToolWidget *widget) {
  ...
  GimpCoords coords = {
      private->last_coords.x, private->last_coords.y,
      /* pad with 0 */
  };
  gint i;

  gimp_canvas_polygon_set_points(private->polygon, private->points,
                                 private->n_points);

  if (private->show_pending_point) {
    GimpVector2 last = private->points[private->n_points - 1];

    gimp_canvas_line_set(private->pending_line, last.x, last.y,
                         private->pending_point.x, private->pending_point.y);
  }
  ...
  if (private->polygon_closed) {
    ...
    gimp_canvas_line_set(private->closing_line, first.x, first.y, last.x,
                         last.y);
  }

  gimp_canvas_item_set_visible(private->closing_line, private->polygon_closed);

  hovering_first_point =
      gimp_tool_polygon_should_close(polygon, NO_CLICK_TIME_AVAILABLE, &coords);
  ...
  for (i = 0; i < private->n_segment_indices; i++) {
    ...
    if (private->hover && handles_wants_to_show && !private->suppress_handles &&
         ...
         */
        (i == 0 || !(private->button_down || hovering_first_point))) {
      gdouble dist;
      GimpHandleType handle_type = -1;

      dist = gimp_canvas_item_transform_distance_square(
          handle, private->last_coords.x, private->last_coords.y, point->x,
          point->y);
      ...
      if (handle_type != -1) {
        ...
        g_object_set(handle, "type", handle_type, NULL);

        size = gimp_canvas_handle_calc_size(
            handle, private->last_coords.x, private->last_coords.y,
            GIMP_CANVAS_HANDLE_SIZE_CIRCLE, 2 * GIMP_CANVAS_HANDLE_SIZE_CIRCLE);
        if (FALSE)
...
static gboolean gimp_tool_polygon_button_press(GimpToolWidget *widget,
                                               const GimpCoords *coords,
                                               guint32 time,
                                               GdkModifierType state,
                                               GimpButtonPressType press_type) {
  ...
  if (gimp_tool_polygon_is_point_grabbed(polygon)) {
     ...
     * of the free segment.
     */
    gimp_tool_polygon_add_point(polygon, point_to_add.x, point_to_add.y);
...
static void gimp_tool_polygon_motion(GimpToolWidget *widget,
  ...
  gimp_tool_polygon_update_motion(polygon, coords->x, coords->y);
...
static void gimp_tool_polygon_hover(GimpToolWidget *widget,
  ...
  priv->grabbed_segment_index =
      gimp_tool_polygon_get_segment_index(polygon, coords);
  priv->hover = TRUE;
  ...
  if (priv->n_points == 0 || priv->polygon_closed ||
      (gimp_tool_polygon_is_point_grabbed(polygon) && !hovering_first_point) ||
      !proximity) {
    priv->show_pending_point = FALSE;
  } else {
    ...
    if (hovering_first_point) {
      ...
      if (priv->constrain_angle && priv->n_points > 0) {
        ...
        gimp_display_shell_constrain_line(
            gimp_tool_widget_get_shell(widget), start_point_x, start_point_y,
...
static void gimp_tool_polygon_motion_modifier(GimpToolWidget *widget,
  ...
  priv->constrain_angle =
      ((state & gimp_get_constrain_behavior_mask()) ? TRUE : FALSE);

  /* If we didn't came here due to a mouse release, immediately update
   * the position of the thing we move.
   */
  if (state & GDK_BUTTON1_MASK) {
    gimp_tool_polygon_update_motion(polygon, priv->last_coords.x,
                                    priv->last_coords.y);
  }
...
static void gimp_tool_polygon_hover_modifier(GimpToolWidget *widget,
  ...
  priv->suppress_handles =
      ((state & gimp_get_extend_selection_mask()) ? TRUE : FALSE);
...
static gboolean gimp_tool_polygon_get_cursor(GimpToolWidget *widget,
  ...
  if (gimp_tool_polygon_is_point_grabbed(polygon) &&
      !gimp_tool_polygon_should_close(polygon, NO_CLICK_TIME_AVAILABLE,
                                      coords)) {
...
static gint gimp_tool_polygon_get_segment_index(GimpToolPolygon *polygon,
  ...
  if (!priv->suppress_handles) {
    ...
    for (i = 0; i < priv->n_segment_indices; i++) {
      ...
      dist = gimp_canvas_item_transform_distance_square(
          priv->polygon, coords->x, coords->y, point->x, point->y);
...
GimpToolWidget *gimp_tool_polygon_new(GimpDisplayShell *shell) {
  ...
  return g_object_new(GIMP_TYPE_TOOL_POLYGON, "shell", shell, NULL);
}
