...
gboolean gimp_canvas_item_on_handle(GimpCanvasItem *item, gdouble x, gdouble y,
                                    GimpHandleType type, gdouble handle_x,
                                    gdouble handle_y, gint width, gint height,
...
gboolean gimp_canvas_item_on_vectors_handle(
    GimpCanvasItem *item, GimpVectors *vectors, const GimpCoords *coord,
    gint width, gint height, GimpAnchorType preferred, gboolean exclusive,
    GimpAnchor **ret_anchor, GimpStroke **ret_stroke) {
  ...
  if (ret_anchor && *ret_anchor &&
      gimp_canvas_item_on_handle(item, coord->x, coord->y, GIMP_HANDLE_CIRCLE,
                                 (*ret_anchor)->position.x,
                                 (*ret_anchor)->position.y, width, height,
  ...
  if (pref_anchor &&
      gimp_canvas_item_on_handle(
          item, coord->x, coord->y, GIMP_HANDLE_CIRCLE, pref_anchor->position.x,
          pref_anchor->position.y, width, height, GIMP_HANDLE_ANCHOR_CENTER)) {
    if (ret_anchor)
    ...
    return TRUE;
             ...
             gimp_canvas_item_on_handle(item, coord->x, coord->y,
                                        GIMP_HANDLE_CIRCLE, anchor->position.x,
                                        anchor->position.y, width, height,
                                        GIMP_HANDLE_ANCHOR_CENTER)) {
    if (ret_anchor)
      *ret_anchor = anchor;
...
gboolean gimp_canvas_item_on_vectors_curve(
    GimpCanvasItem *item, GimpVectors *vectors, const GimpCoords *coord,
    gint width, gint height, GimpCoords *ret_coords, gdouble *ret_pos,
    GimpAnchor **ret_segment_start, GimpAnchor **ret_segment_end,
  ...
  while ((stroke = gimp_vectors_stroke_get_next(vectors, stroke))) {
    cur_dist =
        gimp_stroke_nearest_point_get(stroke, coord, 1.0, &cur_coords,
                                      &segment_start, &segment_end, &cur_pos);
  ...
  if (min_dist >= 0 &&
      gimp_canvas_item_on_handle(item, coord->x, coord->y, GIMP_HANDLE_CIRCLE,
                                 min_coords.x, min_coords.y, width, height,
                                 GIMP_HANDLE_ANCHOR_CENTER)) {
    return TRUE;
  }
...
gboolean gimp_canvas_item_on_vectors(
    GimpCanvasItem *item, const GimpCoords *coords, gint width, gint height,
    GimpCoords *ret_coords, gdouble *ret_pos, GimpAnchor **ret_segment_start,
    GimpAnchor **ret_segment_end, GimpStroke **ret_stroke,
  ...
  for (list = all_vectors; list; list = g_list_next(list)) {
    ...
    if (gimp_canvas_item_on_vectors_curve(
            item, vectors, coords, width, height, ret_coords, ret_pos,
            ret_segment_start, ret_segment_end, ret_stroke)) {
      if (ret_vectors)
        *ret_vectors = vectors;
...
void gimp_canvas_item_shift_to_north_west(GimpHandleAnchor anchor, gdouble x,
...
void gimp_canvas_item_shift_to_center(GimpHandleAnchor anchor, gdouble x,
                                      gdouble y, gint width, gint height,
                                      gdouble *shifted_x, gdouble *shifted_y) {
  switch (anchor) {
  case GIMP_HANDLE_ANCHOR_CENTER:
    /*  nothing, this is the default  */
    break;
