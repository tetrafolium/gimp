...
enum { CREATE_GUIDES, LAST_SIGNAL };
...
static void gimp_tool_compass_set_property(GObject *object, guint property_id,
                                           const GValue *value,
                                           GParamSpec *pspec);
static void gimp_tool_compass_get_property(GObject *object, guint property_id,
...
static void gimp_tool_compass_button_release(
...
static void gimp_tool_compass_class_init(GimpToolCompassClass *klass) {
  ...
  compass_signals[CREATE_GUIDES] = g_signal_new(
      "create-guides", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolCompassClass, create_guides), NULL, NULL,
      gimp_marshal_VOID__INT_INT_BOOLEAN_BOOLEAN, G_TYPE_NONE, 4, G_TYPE_INT,
  ...
  g_object_class_install_property(
      object_class, PROP_N_POINTS,
      g_param_spec_int("n-points", NULL, NULL, 1, 3, 1,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_X1,
      g_param_spec_int("x1", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_Y1,
      g_param_spec_int("y1", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_X2,
      g_param_spec_int("x2", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_Y2,
      g_param_spec_int("y2", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_X3,
      g_param_spec_int("x3", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_Y3,
      g_param_spec_int("y3", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  ...
  g_object_class_install_property(
      object_class, PROP_EFFECTIVE_ORIENTATION,
      g_param_spec_enum("effective-orientation", NULL, NULL,
                        GIMP_TYPE_COMPASS_ORIENTATION,
                        GIMP_COMPASS_ORIENTATION_AUTO, GIMP_PARAM_READABLE));
}

static void gimp_tool_compass_init(GimpToolCompass *compass) {
...
private
  ->line1 = gimp_tool_widget_add_line(widget, private->x[0], private->y[0],
                                      private->x[1], private->y[1]);

private
  ->line2 = gimp_tool_widget_add_line(widget, private->x[0], private->y[0],
                                      private->x[2], private->y[2]);

private
  ->arc = gimp_tool_widget_add_handle(
      widget, GIMP_HANDLE_CIRCLE, private->x[0], private->y[0],
      ARC_RADIUS * 2 + 1, ARC_RADIUS * 2 + 1, GIMP_HANDLE_ANCHOR_CENTER);

private
  ->arc_line = gimp_tool_widget_add_line(widget, private->x[0], private->y[0],
                                         private->x[0] + 10, private->y[0]);

  gimp_tool_widget_pop_group(widget);

  for (i = 0; i < 3; i++) {
  private
    ->handles[i] = gimp_tool_widget_add_handle(
        widget, i == 0 ? GIMP_HANDLE_CIRCLE : GIMP_HANDLE_CROSS, private->x[i],
        private->y[i], GIMP_CANVAS_HANDLE_SIZE_CROSS,
        GIMP_CANVAS_HANDLE_SIZE_CROSS, GIMP_HANDLE_ANCHOR_CENTER);
  }
...
static void gimp_tool_compass_set_property(GObject *object, guint property_id,
  ...
  case PROP_N_POINTS:
  private
    ->n_points = g_value_get_int(value);
    break;
  case PROP_X1:
  private
    ->x[0] = g_value_get_int(value);
    break;
  case PROP_Y1:
  private
    ->y[0] = g_value_get_int(value);
    break;
  case PROP_X2:
  private
    ->x[1] = g_value_get_int(value);
    break;
  case PROP_Y2:
  private
    ->y[1] = g_value_get_int(value);
    break;
  case PROP_X3:
  private
    ->x[2] = g_value_get_int(value);
    break;
  case PROP_Y3:
  private
    ->y[2] = g_value_get_int(value);
    break;
...
static void gimp_tool_compass_changed(GimpToolWidget *widget) {
  ...
  gimp_canvas_line_set(private->line1, private->x[0], private->y[0],
                       private->x[1], private->y[1]);
  gimp_canvas_item_set_visible(private->line1, private->n_points > 1);
  if (private->n_points > 1 && gimp_canvas_item_transform_distance(
                                   private->line1, private->x[0], private->y[0],
                                   private->x[1], private->y[1]) > ARC_RADIUS) {
    draw_arc++;
  }

  arc_line_display_length =
      ARC_RADIUS + (GIMP_CANVAS_HANDLE_SIZE_CROSS >> 1) + ARC_GAP;
  arc_line_length =
      arc_line_display_length / hypot(private->radius2.x * shell->scale_x,
                                      private->radius2.y * shell->scale_y);

  if (private->n_points > 2) {
    gdouble length = gimp_canvas_item_transform_distance(
        private->line2, private->x[0], private->y[0], private->x[2],
        private->y[2]);
    ...
    if (length > ARC_RADIUS) {
      ...
      if (length > arc_line_display_length) {
        gimp_canvas_line_set(
            ...
            private->y[0] + private->radius2.y * arc_line_length, private->x[2],
            private->y[2]);
      ...
      gimp_canvas_line_set(private->line2, private->x[0], private->y[0],
                           private->x[2], private->y[2]);
      gimp_canvas_item_set_visible(private->line2, TRUE);
    }
  } else {
  ...
  gimp_canvas_handle_set_position(private->arc, private->x[0], private->y[0]);
  ...
  gimp_canvas_line_set(private->arc_line, private->x[0], private->y[0],
                       private->x[0] + private->radius2.x * arc_line_length,
  ...
  gimp_canvas_handle_set_position(private->handles[0], private->x[0],
                                  private->y[0]);
  gimp_canvas_item_set_visible(private->handles[0], private->n_points > 0);

  gimp_canvas_handle_set_position(private->handles[1], private->x[1],
                                  private->y[1]);
  gimp_canvas_item_set_visible(private->handles[1], private->n_points > 1);

  gimp_canvas_handle_set_position(private->handles[2], private->x[2],
                                  private->y[2]);
  gimp_canvas_item_set_visible(private->handles[2], private->n_points > 2);

  gimp_tool_compass_update_hilight(compass);
}

gint gimp_tool_compass_button_press(GimpToolWidget *widget,
                                    const GimpCoords *coords, guint32 time,
                                    GdkModifierType state,
                                    GimpButtonPressType press_type) {
  GimpToolCompass *compass = GIMP_TOOL_COMPASS(widget);
...
private
  ...
  if (private->point != -1) {
    ...
    if (state & (toggle_mask | GDK_MOD1_MASK)) {
      ...
      g_signal_emit(compass, compass_signals[CREATE_GUIDES], 0,
                    private->x[private->point], private->y[private->point],
                    create_hguide, create_vguide);
    ...
    private
      ...
      private
      ->function = ADDING;
      else private->function = MOVING;
    }
  }

  /*  adding to the middle point makes no sense  */
  if (private->point == 0 && private->function == ADDING &&
      private->n_points == 3) {
  private
    ->function = MOVING;
  }
  ...
  if (private->function == CREATING) {
    ...
    g_object_set(compass, "n-points", 1, "x1", (gint)(coords->x + 0.5), "y1",
                 (gint)(coords->y + 0.5), "x2", 0, "y2", 0, "x3", 0, "y3", 0,
                 NULL);
...
void gimp_tool_compass_button_release(GimpToolWidget *widget,
                                      const GimpCoords *coords, guint32 time,
                                      GdkModifierType state,
...
void gimp_tool_compass_motion(GimpToolWidget *widget, const GimpCoords *coords,
                              guint32 time, GdkModifierType state) {
  GimpToolCompass *compass = GIMP_TOOL_COMPASS(widget);
  GimpToolCompassPrivate *private = compass->private;
  gint new_n_points;
...
private
  ...
  case MOVING:
    ...
    g_object_set(compass, "n-points", new_n_points, "x1", new_x[0], "y1",
                 new_y[0], "x2", new_x[1], "y2", new_y[1], "x3", new_x[2], "y3",
                 new_y[2], NULL);
    break;
  ...
  case MOVING_ALL:
    ...
    g_object_set(compass, "x1", new_x[0] + dx, "y1", new_y[0] + dy, "x2",
                 new_x[1] + dx, "y2", new_y[1] + dy, "x3", new_x[2] + dx, "y3",
                 new_y[2] + dy, NULL);

  private
    ->last_x = ROUND(coords->x);
  private
    ->last_y = ROUND(coords->y);
    break;
...
GimpHit gimp_tool_compass_hit(GimpToolWidget *widget, const GimpCoords *coords,
                              GdkModifierType state, gboolean proximity) {
  GimpToolCompass *compass = GIMP_TOOL_COMPASS(widget);
...
void gimp_tool_compass_hover(GimpToolWidget *widget, const GimpCoords *coords,
                             GdkModifierType state, gboolean proximity) {
  GimpToolCompass *compass = GIMP_TOOL_COMPASS(widget);
  GimpToolCompassPrivate *private = compass->private;
  gint point;
  ...
  if (point >= 0) {
    ...
    if (state & toggle_mask) {
      if (state & GDK_MOD1_MASK) {
        ...
        status =
            gimp_suggest_modifiers(_("Click to place a "
                                     "horizontal guide"),
                                   GDK_MOD1_MASK & ~state, NULL, NULL, NULL);
      }
      ...
      status = gimp_suggest_modifiers(_("Click-Drag to add a "
                                        "new point"),
                                      (toggle_mask | GDK_MOD1_MASK) & ~state,
                                      NULL, NULL, NULL);
    } else {
      ...
      status = gimp_suggest_modifiers(
          _("Click-Drag to move this "
            "point"),
          (extend_mask | toggle_mask | GDK_MOD1_MASK) & ~state, NULL, NULL,
          NULL);
    }
    ...
    if ((private->n_points > 1) && (state & GDK_MOD1_MASK)) {
      gimp_tool_widget_set_status(widget, _("Click-Drag to move all points"));
    } else {
...
void gimp_tool_compass_leave_notify(GimpToolWidget *widget) {
  GimpToolCompass *compass = GIMP_TOOL_COMPASS(widget);
  GimpToolCompassPrivate *private = compass->private;
...
static void gimp_tool_compass_motion_modifier(GimpToolWidget *widget,
  ...
  if (key == gimp_get_constrain_behavior_mask() &&
    ...
    g_object_set(compass, "x1", new_x[0], "y1", new_y[0], "x2", new_x[1], "y2",
                 new_y[1], "x3", new_x[2], "y3", new_y[2], NULL);
  }
}
    ...
    if (state & toggle_mask) {
      ...
      *cursor = GIMP_CURSOR_SIDE_RIGHT;
      return TRUE;
               ...
               !((private->point == 0) && (private->n_points == 3))) {
...
static gint gimp_tool_compass_get_point(GimpToolCompass *compass,
  ...
  for (i = 0; i < private->n_points; i++) {
    if (gimp_canvas_item_hit(private->handles[i], coords->x, coords->y)) {
      return i;
    }
  }
...
static void gimp_tool_compass_update_hilight(GimpToolCompass *compass) {
  ...
  for (i = 0; i < private->n_points; i++) {
    if (private->handles[i]) {
      gimp_canvas_item_set_highlight(private->handles[i], private->point == i);
    }
  }
}
...
private
  ->radius1.x = private->x[1] - private->x[0];
private
  ->radius1.y = private->y[1] - private->y[0];

  if (private->n_points == 3) {
    orientation = GIMP_COMPASS_ORIENTATION_AUTO;

  private
    ->radius2.x = private->x[2] - private->x[0];
  private
    ->radius2.y = private->y[2] - private->y[0];
  } else {
  ...
  if (shell->dot_for_dot)
  private
  ->display_angle = pixel_angle;
  else private->display_angle = unit_angle;
  ...
  if (private->n_points == 2) {
    if (!flip && fabs(private->display_angle) > G_PI / 2.0 + EPSILON) {
      ...
      if (fabs(private->display_angle) <= G_PI / 4.0 + EPSILON) {
        ...
        gimp_tool_compass_update_angle(
            compass, GIMP_COMPASS_ORIENTATION_VERTICAL, FALSE);
...
GimpToolWidget *gimp_tool_compass_new(GimpDisplayShell *shell,
                                      ...
                                      gint y2, gint x3, gint y3) {
  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), NULL);

  return g_object_new(GIMP_TYPE_TOOL_COMPASS, "shell", shell, "orientation",
                      orientation, "n-points", n_points, "x1", x1, "y1", y1,
                      "x2", x2, "y2", y2, NULL);
}
