...
#define GET_PRIVATE(proxy_group)                                               \
  ((GimpCanvasProxyGroupPrivate *)                                             \
       gimp_canvas_proxy_group_get_instance_private(                           \
           (GimpCanvasProxyGroup *)(proxy_group)))
...
static void gimp_canvas_proxy_group_init(GimpCanvasProxyGroup *proxy_group) {
...
GimpCanvasItem *gimp_canvas_proxy_group_new(GimpDisplayShell *shell) {
  ...
  return g_object_new(GIMP_TYPE_CANVAS_PROXY_GROUP, "shell", shell, NULL);
}
...
private
  ...
  g_return_if_fail(g_hash_table_lookup(private->proxy_hash, object) == NULL);
