...
typedef enum { MODE_NONE, MODE_PAN, MODE_ROTATE, MODE_ZOOM } Mode;
...
static void gimp_tool_gyroscope_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_tool_gyroscope_get_property(GObject *object, guint property_id,
...
static void gimp_tool_gyroscope_button_release(
...
static void gimp_tool_gyroscope_class_init(GimpToolGyroscopeClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_YAW,
      g_param_spec_double("yaw", NULL, NULL, -G_MAXDOUBLE, +G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_PITCH,
      g_param_spec_double("pitch", NULL, NULL, -G_MAXDOUBLE, +G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_ROLL,
      g_param_spec_double("roll", NULL, NULL, -G_MAXDOUBLE, +G_MAXDOUBLE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_ZOOM,
      g_param_spec_double("zoom", NULL, NULL, 0.0, +G_MAXDOUBLE, 1.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_INVERT,
      g_param_spec_boolean("invert", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_SPEED,
      g_param_spec_double("speed", NULL, NULL, -G_MAXDOUBLE, +G_MAXDOUBLE, 1.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_PIVOT_X,
      g_param_spec_double("pivot-x", NULL, NULL, -G_MAXDOUBLE, +G_MAXDOUBLE,
                          0.0, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_PIVOT_Y,
      g_param_spec_double("pivot-y", NULL, NULL, -G_MAXDOUBLE, +G_MAXDOUBLE,
                          0.0, GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_tool_gyroscope_init(GimpToolGyroscope *gyroscope) {
...
static void gimp_tool_gyroscope_set_property(GObject *object, guint property_id,
  ...
  case PROP_YAW:
  private
    ->yaw = g_value_get_double(value);
    break;
  case PROP_PITCH:
  private
    ->pitch = g_value_get_double(value);
    break;
  case PROP_ROLL:
  private
    ->roll = g_value_get_double(value);
    break;
  case PROP_ZOOM:
  private
  ...
  case PROP_SPEED:
  private
    ->speed = g_value_get_double(value);
    break;
  case PROP_PIVOT_X:
  private
...
static gint gimp_tool_gyroscope_button_press(GimpToolWidget *widget,
  ...
  private
    ->last_angle =
        atan2(coords->y - private->pivot_y, coords->x - private->pivot_x);
  private
    ->curr_angle = private->last_angle;
  } else {
  private
    ->mode = MODE_PAN;
    ...
    private
    ->constraint = CONSTRAINT_UNKNOWN;
    else private->constraint = CONSTRAINT_NONE;
  }
...
static void gimp_tool_gyroscope_motion(GimpToolWidget *widget,
  ...
  case MODE_PAN: {
    ...
    if (private->constraint != CONSTRAINT_NONE) {
      ...
      if (private->constraint == CONSTRAINT_UNKNOWN) {
        ...
        private
        ->constraint = CONSTRAINT_HORIZONTAL;
        else if (fabs(y2 - y1) > fabs(x2 - x1)) private->constraint =
            CONSTRAINT_VERTICAL;
      }
  ...
  case MODE_ROTATE: {
    ...
    angle = atan2(coords->y - private->pivot_y, coords->x - private->pivot_x);
  ...
  case MODE_ZOOM: {
    ...
    gimp_display_shell_transform_xy_f(shell, private->last_x, private->last_y,
                                      &x1, &y1);
    gimp_display_shell_transform_xy_f(shell, coords->x, coords->y, &x2, &y2);
  ...
  private
    ...
    g_object_set(gyroscope, "zoom", private->zoom * pow(2.0, zoom), NULL);
  } break;
...
static gboolean gimp_tool_gyroscope_key_press(GimpToolWidget *widget,
  ...
  if (kevent->state & GDK_MOD1_MASK) {
    ...
    gimp_vector3_set(&axis, (y - y0) * DEG_TO_RAD, (x - x0) * DEG_TO_RAD, 0.0);
...
static void gimp_tool_gyroscope_update_status(GimpToolGyroscope *gyroscope,
  ...
  if (private->mode == MODE_ZOOM ||
      (private->mode == MODE_NONE && state & GDK_MOD1_MASK)) {
    status = gimp_suggest_modifiers(_("Click-Drag to zoom"),
                                    gimp_get_toggle_behavior_mask() & ~state,
                                    NULL, _("%s for constrained steps"), NULL);
    ...
    status = gimp_suggest_modifiers(_("Click-Drag to rotate"),
                                    gimp_get_toggle_behavior_mask() & ~state,
                                    NULL, _("%s for constrained angles"), NULL);
  } else {
    status = gimp_suggest_modifiers(
        _("Click-Drag to pan"),
        (((gimp_get_extend_selection_mask() | GDK_MOD1_MASK) *
          (private->mode == MODE_NONE)) |
         gimp_get_toggle_behavior_mask()) &
            ~state,
        _("%s to rotate"), _("%s for a constrained axis"), _("%s to zoom"));
  }
...
private
  ->orig_yaw = private->yaw;
private
  ->orig_pitch = private->pitch;
private
  ->orig_roll = private->roll;
private
  ->orig_zoom = private->zoom;
}

static void gimp_tool_gyroscope_restore(GimpToolGyroscope *gyroscope) {
  GimpToolGyroscopePrivate *private = gyroscope->private;

  g_object_set(gyroscope, "yaw", private->orig_yaw, "pitch",
               private->orig_pitch, "roll", private->orig_roll, "zoom",
               private->orig_zoom, NULL);
}
...
static void gimp_tool_gyroscope_rotate(GimpToolGyroscope *gyroscope,
  ...
  for (i = 0; i < 2; i++) {
    gimp_tool_gyroscope_rotate_vector(&basis[i],
                                      &(GimpVector3){0.0, 0.0, -roll});
  }
  ...
  for (i = 0; i < 1; i++) {
    gimp_tool_gyroscope_rotate_vector(&basis[i],
                                      &(GimpVector3){-pitch, 0.0, 0.0});
  }

  yaw = atan2(basis[0].z, basis[0].x);

  g_object_set(gyroscope, "yaw", yaw / DEG_TO_RAD, "pitch", pitch / DEG_TO_RAD,
               "roll", roll / DEG_TO_RAD, NULL);
}
...
static void gimp_tool_gyroscope_rotate_vector(GimpVector3 *vector,
  ...
  projection = gimp_vector3_mul_val(
      normalized_axis, gimp_vector3_inner_product(vector, &normalized_axis));
...
GimpToolWidget *gimp_tool_gyroscope_new(GimpDisplayShell *shell) {
  ...
  return g_object_new(GIMP_TYPE_TOOL_GYROSCOPE, "shell", shell, NULL);
}
