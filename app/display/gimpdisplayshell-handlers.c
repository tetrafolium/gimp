...
static void gimp_display_shell_component_visibility_changed_handler(
    GimpImage *image, GimpChannelType channel, GimpDisplayShell *shell);
static void gimp_display_shell_size_changed_detailed_handler(
    GimpImage *image, gint previous_origin_x, gint previous_origin_y,
    gint previous_width, gint previous_height, GimpDisplayShell *shell);
static void
...
static void gimp_display_shell_guide_move_handler(GimpImage *image,
                                                  GimpGuide *guide,
                                                  GimpDisplayShell *shell);
static void gimp_display_shell_sample_point_add_handler(
    GimpImage *image, GimpSamplePoint *sample_point, GimpDisplayShell *shell);
...
static void gimp_display_shell_profile_changed_handler(GimpColorManaged *image,
                                                       GimpDisplayShell *shell);
static void gimp_display_shell_saved_handler(GimpImage *image, GFile *file,
                                             GimpDisplayShell *shell);
static void gimp_display_shell_exported_handler(GimpImage *image, GFile *file,
                                                GimpDisplayShell *shell);
...
static void gimp_display_shell_nav_size_notify_handler(GObject *config,
                                                       GParamSpec *param_spec,
                                                       GimpDisplayShell *shell);
static void gimp_display_shell_monitor_res_notify_handler(
    GObject *config, GParamSpec *param_spec, GimpDisplayShell *shell);
static void gimp_display_shell_padding_notify_handler(GObject *config,
                                                      GParamSpec *param_spec,
                                                      GimpDisplayShell *shell);
static void gimp_display_shell_ants_speed_notify_handler(
    GObject *config, GParamSpec *param_spec, GimpDisplayShell *shell);
static void gimp_display_shell_quality_notify_handler(GObject *config,
                                                      GParamSpec *param_spec,
                                                      GimpDisplayShell *shell);
static void gimp_display_shell_color_config_notify_handler(
    GObject *config, GParamSpec *param_spec, GimpDisplayShell *shell);
...
void gimp_display_shell_connect(GimpDisplayShell *shell) {
  ...
  g_signal_connect(image, "guide-added",
                   G_CALLBACK(gimp_display_shell_guide_add_handler), shell);
  g_signal_connect(image, "guide-removed",
                   G_CALLBACK(gimp_display_shell_guide_remove_handler), shell);
  g_signal_connect(image, "guide-moved",
                   G_CALLBACK(gimp_display_shell_guide_move_handler), shell);
  ...
  g_signal_connect(image, "sample-point-moved",
                   ...
                   shell);
  for (list = gimp_image_get_sample_points(image); list;
  ...
  g_signal_connect(image, "mode-changed",
                   G_CALLBACK(gimp_display_shell_mode_changed_handler), shell);
  ...
  g_signal_connect(image, "profile-changed",
                   ...
                   shell);
  g_signal_connect(image, "saved", G_CALLBACK(gimp_display_shell_saved_handler),
                   shell);
  g_signal_connect(image, "exported",
                   G_CALLBACK(gimp_display_shell_exported_handler), shell);

  g_signal_connect(image, "selected-vectors-changed",
                   G_CALLBACK(gimp_display_shell_active_vectors_handler),
                   shell);

  shell->vectors_freeze_handler = gimp_tree_handler_connect(
      vectors, "freeze", G_CALLBACK(gimp_display_shell_vectors_freeze_handler),
      shell);
  shell->vectors_thaw_handler = gimp_tree_handler_connect(
      vectors, "thaw", G_CALLBACK(gimp_display_shell_vectors_thaw_handler),
      shell);
  shell->vectors_visible_handler = gimp_tree_handler_connect(
      vectors, "visibility-changed",
      G_CALLBACK(gimp_display_shell_vectors_visible_handler), shell);

  g_signal_connect(vectors, "add",
                   G_CALLBACK(gimp_display_shell_vectors_add_handler), shell);
  g_signal_connect(vectors, "remove",
                   G_CALLBACK(gimp_display_shell_vectors_remove_handler),
                   shell);
  for (list = gimp_item_stack_get_item_iter(GIMP_ITEM_STACK(vectors)); list;
       list = g_list_next(list)) {
    gimp_display_shell_vectors_add_handler(vectors, list->data, shell);
  }

  g_signal_connect(config, "notify::transparency-size",
                   G_CALLBACK(gimp_display_shell_check_notify_handler), shell);
  g_signal_connect(config, "notify::transparency-type",
                   G_CALLBACK(gimp_display_shell_check_notify_handler), shell);

  g_signal_connect(config, "notify::image-title-format",
                   G_CALLBACK(gimp_display_shell_title_notify_handler), shell);
  g_signal_connect(config, "notify::image-status-format",
                   G_CALLBACK(gimp_display_shell_title_notify_handler), shell);
  g_signal_connect(config, "notify::navigation-preview-size",
                   G_CALLBACK(gimp_display_shell_nav_size_notify_handler),
                   shell);
  g_signal_connect(config, "notify::monitor-resolution-from-windowing-system",
                   G_CALLBACK(gimp_display_shell_monitor_res_notify_handler),
                   shell);
  g_signal_connect(config, "notify::monitor-xresolution",
                   G_CALLBACK(gimp_display_shell_monitor_res_notify_handler),
                   shell);
  g_signal_connect(config, "notify::monitor-yresolution",
                   G_CALLBACK(gimp_display_shell_monitor_res_notify_handler),
                   shell);

  g_signal_connect(config->default_view, "notify::padding-mode",
                   G_CALLBACK(gimp_display_shell_padding_notify_handler),
                   shell);
  g_signal_connect(config->default_view, "notify::padding-color",
                   G_CALLBACK(gimp_display_shell_padding_notify_handler),
                   shell);
  g_signal_connect(config->default_fullscreen_view, "notify::padding-mode",
                   G_CALLBACK(gimp_display_shell_padding_notify_handler),
                   shell);
  g_signal_connect(config->default_fullscreen_view, "notify::padding-color",
                   G_CALLBACK(gimp_display_shell_padding_notify_handler),
                   shell);

  g_signal_connect(config, "notify::marching-ants-speed",
                   G_CALLBACK(gimp_display_shell_ants_speed_notify_handler),
                   shell);

  g_signal_connect(config, "notify::zoom-quality",
  ...
  gimp_display_shell_profile_changed_handler(GIMP_COLOR_MANAGED(image), shell);
  ...
  gimp_canvas_canvas_boundary_set_image(
      GIMP_CANVAS_CANVAS_BOUNDARY(shell->canvas_boundary), image);
...
void gimp_display_shell_disconnect(GimpDisplayShell *shell) {
  ...
  gimp_canvas_layer_boundary_set_layers(
      GIMP_CANVAS_LAYER_BOUNDARY(shell->layer_boundary), NULL);

  gimp_canvas_canvas_boundary_set_image(
      GIMP_CANVAS_CANVAS_BOUNDARY(shell->canvas_boundary), NULL);

  g_signal_handlers_disconnect_by_func(
      user_context, gimp_display_shell_display_changed_handler, shell);

  g_signal_handlers_disconnect_by_func(
      color_config, gimp_display_shell_color_config_notify_handler, shell);
  shell->color_config_set = FALSE;

  g_signal_handlers_disconnect_by_func(
      config, gimp_display_shell_quality_notify_handler, shell);
  g_signal_handlers_disconnect_by_func(
      config, gimp_display_shell_ants_speed_notify_handler, shell);
  g_signal_handlers_disconnect_by_func(
      config->default_fullscreen_view,
      gimp_display_shell_padding_notify_handler, shell);
  g_signal_handlers_disconnect_by_func(
      config->default_view, gimp_display_shell_padding_notify_handler, shell);
  g_signal_handlers_disconnect_by_func(
      config, gimp_display_shell_monitor_res_notify_handler, shell);
  g_signal_handlers_disconnect_by_func(
      config, gimp_display_shell_nav_size_notify_handler, shell);
  g_signal_handlers_disconnect_by_func(
      config, gimp_display_shell_title_notify_handler, shell);
  g_signal_handlers_disconnect_by_func(
      config, gimp_display_shell_check_notify_handler, shell);

  g_signal_handlers_disconnect_by_func(
      vectors, gimp_display_shell_vectors_remove_handler, shell);
  g_signal_handlers_disconnect_by_func(
      vectors, gimp_display_shell_vectors_add_handler, shell);
  ...
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_active_vectors_handler, shell);
  ...
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_exported_handler, shell);
  g_signal_handlers_disconnect_by_func(image, gimp_display_shell_saved_handler,
                                       shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_profile_changed_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_precision_changed_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_mode_changed_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_invalidate_preview_handler, shell);

  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_guide_add_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_guide_remove_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_guide_move_handler, shell);
  for (list = gimp_image_get_guides(image); list; list = g_list_next(list)) {
  ...
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_sample_point_add_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_sample_point_remove_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_sample_point_move_handler, shell);
  for (list = gimp_image_get_sample_points(image); list;
       list = g_list_next(list)) {
    gimp_canvas_proxy_group_remove_item(
        GIMP_CANVAS_PROXY_GROUP(shell->sample_points), list->data);
  }

  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_quick_mask_changed_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_resolution_changed_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_component_visibility_changed_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_size_changed_detailed_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_selection_invalidate_handler, shell);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_shell_name_changed_handler, shell);
...
gimp_display_shell_quick_mask_changed_handler(GimpImage *image,
  ...
  g_signal_handlers_block_by_func(shell->quick_mask_button,
                                  gimp_display_shell_quick_mask_toggled, shell);
  ...
  g_signal_handlers_unblock_by_func(
      shell->quick_mask_button, gimp_display_shell_quick_mask_toggled, shell);
}
...
static void gimp_display_shell_guide_add_handler(GimpImage *image,
  ...
  item = gimp_canvas_guide_new(shell, gimp_guide_get_orientation(guide),
                               gimp_guide_get_position(guide), style);
...
static void gimp_display_shell_guide_move_handler(GimpImage *image,
  ...
  gimp_canvas_guide_set(item, gimp_guide_get_orientation(guide),
...
static void gimp_display_shell_sample_point_add_handler(
  ...
  for (list = gimp_image_get_sample_points(image), i = 1; list;
    ...
    if (item)
      g_object_set(item, "index", i, NULL);
  }
}
...
static void gimp_display_shell_size_changed_detailed_handler(
  ...
  if (resize_window) {
    ...
    gimp_display_shell_scroll_set_offset(
        shell, shell->offset_x + scaled_previous_origin_x,
...
static void gimp_display_shell_saved_handler(GimpImage *image, GFile *file,
  ...
  gimp_statusbar_push_temp(statusbar, GIMP_MESSAGE_INFO,
                           GIMP_ICON_DOCUMENT_SAVE, _("Image saved to '%s'"),
...
static void gimp_display_shell_exported_handler(GimpImage *image, GFile *file,
  ...
  gimp_statusbar_push_temp(statusbar, GIMP_MESSAGE_INFO,
                           GIMP_ICON_DOCUMENT_SAVE, _("Image exported to '%s'"),
                           gimp_file_get_utf8_name(file));
}
...
static void gimp_display_shell_active_vectors_handler(GimpImage *image,
  ...
  for (list = gimp_image_get_vectors_iter(image); list;
       list = g_list_next(list)) {
...
gimp_display_shell_vectors_visible_handler(GimpVectors *vectors,
  ...
  gimp_canvas_item_set_visible(item, gimp_item_get_visible(GIMP_ITEM(vectors)));
}
...
static void gimp_display_shell_vectors_add_handler(GimpContainer *container,
  ...
  item = gimp_canvas_path_new(shell, gimp_vectors_get_bezier(vectors), 0, 0,
                              FALSE, GIMP_PATH_STYLE_VECTORS);
  gimp_canvas_item_set_visible(item, gimp_item_get_visible(GIMP_ITEM(vectors)));
...
static void gimp_display_shell_nav_size_notify_handler(
    GObject *config, GParamSpec *param_spec, GimpDisplayShell *shell) {
...
static void gimp_display_shell_color_config_notify_handler(
  ...
  if (param_spec) {
    ...
    if (copy) {
      ...
      g_object_set_property(G_OBJECT(shell->color_config), param_spec->name,
                            &value);

      g_value_unset(&value);
    }
  } else {
    gimp_config_copy(GIMP_CONFIG(config), GIMP_CONFIG(shell->color_config), 0);
    shell->color_config_set = FALSE;
  }
}

static void gimp_display_shell_display_changed_handler(
    GimpContext *context, GimpDisplay *display, GimpDisplayShell *shell) {
