...
static void update_zoom_values(GtkAdjustment *adj, ScaleDialogData *dialog);
...
void gimp_display_shell_scale_dialog(GimpDisplayShell *shell) {
  ...
  data->shell = shell;
  data->model = g_object_new(GIMP_TYPE_ZOOM_MODEL, "value",
                             fabs(shell->other_scale), NULL);

  shell->scale_dialog = gimp_viewable_dialog_new(
      g_list_prepend(NULL, image), gimp_get_user_context(shell->display->gimp),
      _("Zoom Ratio"), "display_scale", "zoom-original", _("Select Zoom Ratio"),
      GTK_WIDGET(shell), gimp_standard_help_func, GIMP_HELP_VIEW_ZOOM_OTHER,
  ...
  g_object_weak_ref(G_OBJECT(shell->scale_dialog), (GWeakNotify)g_object_unref,
                    data->model);
  ...
  g_signal_connect(shell->scale_dialog, "response",
                   G_CALLBACK(gimp_display_shell_scale_dialog_response), data);
  ...
  gtk_box_pack_start(
      GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(shell->scale_dialog))),
      grid, TRUE, TRUE, 0);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("Zoom ratio:"), 0.0, 0.5,
                           hbox, 1);
  ...
  hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, row++, _("Zoom:"), 0.0, 0.5, hbox,
                           1);

  data->scale_adj = gtk_adjustment_new(fabs(shell->other_scale) * 100,
                                       100.0 / 256.0, 25600.0, 10, 50, 0);
...
static void update_zoom_values(GtkAdjustment *adj, ScaleDialogData *dialog) {
  ...
  g_signal_handlers_block_by_func(dialog->scale_adj, update_zoom_values,
                                  dialog);
  g_signal_handlers_block_by_func(dialog->num_adj, update_zoom_values, dialog);
  g_signal_handlers_block_by_func(dialog->denom_adj, update_zoom_values,
                                  dialog);
  ...
  g_signal_handlers_unblock_by_func(dialog->scale_adj, update_zoom_values,
                                    dialog);
  g_signal_handlers_unblock_by_func(dialog->num_adj, update_zoom_values,
                                    dialog);
  g_signal_handlers_unblock_by_func(dialog->denom_adj, update_zoom_values,
                                    dialog);
}
