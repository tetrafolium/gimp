...
static void gimp_cursor_view_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_cursor_view_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);

static void gimp_cursor_view_style_updated(GtkWidget *widget);

static void gimp_cursor_view_set_aux_info(GimpDocked *docked, GList *aux_info);
...
static void gimp_cursor_view_shell_unit_changed(GimpCursorView *view,
                                                GParamSpec *pspec,
                                                GimpDisplayShell *shell);
static void gimp_cursor_view_format_as_unit(GimpUnit unit, gchar *output_buf,
...
G_DEFINE_TYPE_WITH_CODE(GimpCursorView, gimp_cursor_view, GIMP_TYPE_EDITOR,
                        G_ADD_PRIVATE(GimpCursorView) G_IMPLEMENT_INTERFACE(
                            GIMP_TYPE_DOCKED,
...
static void gimp_cursor_view_init(GimpCursorView *view) {
  ...
  view->priv->coord_hbox =
      gtk_box_new(GTK_ORIENTATION_HORIZONTAL, content_spacing);
  ...
  view->priv->selection_hbox =
      gtk_box_new(GTK_ORIENTATION_HORIZONTAL, content_spacing);
  gtk_box_set_homogeneous(GTK_BOX(view->priv->selection_hbox), TRUE);
  gtk_box_pack_start(GTK_BOX(view), view->priv->selection_hbox, FALSE, FALSE,
                     0);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0, _("X"), 0.5, 0.5,
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1, _("Y"), 0.5, 0.5,
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 0,
                           /* Width */
                           _("W"), 0.5, 0.5, view->priv->selection_width_label,
                           1);
  ...
  gimp_grid_attach_aligned(GTK_GRID(grid), 0, 1,
                           /* Height */
                           _("H"), 0.5, 0.5, view->priv->selection_height_label,
                           1);

  /* color information */

  view->priv->color_hbox =
      gtk_box_new(GTK_ORIENTATION_HORIZONTAL, content_spacing);
...
static void gimp_cursor_view_set_aux_info(GimpDocked *docked, GList *aux_info) {
  ...
  for (list = aux_info; list; list = g_list_next(list)) {
    ...
    if (frame) {
      ...
      if (enum_value)
        gimp_color_frame_set_mode(GIMP_COLOR_FRAME(frame), enum_value->value);
    }
  }
}
...
static GList *gimp_cursor_view_get_aux_info(GimpDocked *docked) {
  ...
  if (gimp_enum_get_value(
          GIMP_TYPE_COLOR_PICK_MODE,
          GIMP_COLOR_FRAME(view->priv->color_frame_1)->pick_mode, NULL, &nick,
          NULL, NULL)) {
  ...
  if (gimp_enum_get_value(
          GIMP_TYPE_COLOR_PICK_MODE,
          GIMP_COLOR_FRAME(view->priv->color_frame_2)->pick_mode, NULL, &nick,
          NULL, NULL)) {
...
static void gimp_cursor_view_format_as_unit(GimpUnit unit, gchar *output_buf,
  ...
  g_snprintf(format_buf, sizeof(format_buf), "%%.%df %s", unit_digits,
             unit_str);
...
static void gimp_cursor_view_set_label_italic(GtkWidget *label,
  ...
  gimp_label_set_attributes(GTK_LABEL(label), PANGO_ATTR_STYLE, attribute, -1);
}
...
static void gimp_cursor_view_set_context(GimpDocked *docked,
  ...
  if (view->priv->context) {
    ...
    g_signal_connect_swapped(view->priv->context, "display-changed",
                             G_CALLBACK(gimp_cursor_view_diplay_changed), view);

    g_signal_connect_swapped(view->priv->context, "image-changed",
                             G_CALLBACK(gimp_cursor_view_image_changed), view);
...
static void gimp_cursor_view_image_changed(GimpCursorView *view,
  ...
  if (view->priv->image) {
    g_signal_handlers_disconnect_by_func(view->priv->image,
                                         gimp_cursor_view_mask_changed, view);
  }
  ...
  if (view->priv->image) {
    g_signal_connect_swapped(view->priv->image, "mask-changed",
                             G_CALLBACK(gimp_cursor_view_mask_changed), view);
  }
...
static void gimp_cursor_view_mask_changed(GimpCursorView *view,
                                          GimpImage *image) {
  gimp_cursor_view_update_selection_info(view, view->priv->image,
                                         view->priv->unit);
}
...
static void gimp_cursor_view_diplay_changed(GimpCursorView *view,
  ...
  if (view->priv->shell) {
    g_signal_handlers_disconnect_by_func(
        view->priv->shell, gimp_cursor_view_shell_unit_changed, view);
  }
  ...
  gimp_cursor_view_shell_unit_changed(view, NULL, view->priv->shell);
}
...
static void gimp_cursor_view_update_selection_info(GimpCursorView *view,
  ...
  if (image && gimp_item_bounds(GIMP_ITEM(gimp_image_get_mask(image)), &x, &y,
                                &width, &height)) {
    gdouble xres, yres;
    gchar buf[32];
...
static gboolean gimp_cursor_view_cursor_idle(GimpCursorView *view) {
  ...
  if (view->priv->cursor_image) {
    ...
    in_image = (x >= 0.0 && x < gimp_image_get_width(image) && y >= 0.0 &&
    ...
    if (gimp_image_pick_color(image, NULL, int_x, int_y,
                              view->priv->shell->show_all,
                              view->priv->sample_merged, FALSE, 0.0,
                              &sample_format, pixel, &color)) {
      gimp_color_frame_set_color(GIMP_COLOR_FRAME(view->priv->color_frame_1),
                                 FALSE, sample_format, pixel, &color, int_x,
                                 int_y);
    ...
    gimp_cursor_view_update_selection_info(view, image,
...
GtkWidget *gimp_cursor_view_new(GimpMenuFactory *menu_factory) {
  ...
  return g_object_new(GIMP_TYPE_CURSOR_VIEW, "menu-factory", menu_factory,
                      "menu-identifier", "<CursorInfo>", "ui-path",
                      "/cursor-info-popup", NULL);
}
