...
enum { PROP_0, PROP_POINTS, PROP_TRANSFORM, PROP_FILLED };
...
#define GET_PRIVATE(polygon)                                                   \
  ((GimpCanvasPolygonPrivate *)gimp_canvas_polygon_get_instance_private(       \
      (GimpCanvasPolygon *)(polygon)))

/*  local function prototypes  */

static void gimp_canvas_polygon_finalize(GObject *object);
static void gimp_canvas_polygon_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_canvas_polygon_get_property(GObject *object, guint property_id,
                                             GValue *value, GParamSpec *pspec);
static void gimp_canvas_polygon_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *gimp_canvas_polygon_get_extents(GimpCanvasItem *item);
static gboolean gimp_canvas_polygon_hit(GimpCanvasItem *item, gdouble x,
                                        gdouble y);
...
static void gimp_canvas_polygon_class_init(GimpCanvasPolygonClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_POINTS,
      gimp_param_spec_array("points", NULL, NULL, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_TRANSFORM,
      g_param_spec_pointer("transform", NULL, NULL, GIMP_PARAM_READWRITE));
...
static void gimp_canvas_polygon_init(GimpCanvasPolygon *polygon) {}
...
static void gimp_canvas_polygon_set_property(GObject *object, guint property_id,
  ...
  private
    ...
    if (array) {
    private
      ->points = g_memdup(array->data, array->length);
    private
      ->n_points = array->length / sizeof(GimpVector2);
    }
  } break;
  ...
  case PROP_TRANSFORM: {
    ...
    if (transform)
    private
    ->transform = g_memdup(transform, sizeof(GimpMatrix3));
    else private->transform = NULL;
  } break;
  ...
  case PROP_POINTS:
    if (private->points) {
      ...
      array = gimp_array_new((const guint8 *)private->points,
                             private->n_points * sizeof(GimpVector2), FALSE);
...
static void gimp_canvas_polygon_transform(GimpCanvasItem *item,
                                          GimpVector2 *points, gint *n_points) {
  GimpCanvasPolygonPrivate *private = GET_PRIVATE(item);
  gint i;

  if (private->transform) {
    gimp_transform_polygon(private->transform, private->points,
                           private->n_points, FALSE, points, n_points);

    for (i = 0; i < *n_points; i++) {
      gimp_canvas_item_transform_xy_f(item, points[i].x, points[i].y,
    ...
    for (i = 0; i < private->n_points; i++) {
      gimp_canvas_item_transform_xy_f(item, private->points[i].x,
                                      private->points[i].y, &points[i].x,
                                      &points[i].y);
...
GimpCanvasItem *gimp_canvas_polygon_new(GimpDisplayShell *shell,
  ...
  item = g_object_new(GIMP_TYPE_CANVAS_POLYGON, "shell", shell, "transform",
                      transform, "filled", filled, "points", array, NULL);
...
GimpCanvasItem *gimp_canvas_polygon_new_from_coords(GimpDisplayShell *shell,
  ...
  array = gimp_array_new((const guint8 *)points, n_coords * sizeof(GimpVector2),
                         TRUE);
...
void gimp_canvas_polygon_set_points(GimpCanvasItem *polygon,
  ...
  g_object_set(polygon, "points", array, NULL);
