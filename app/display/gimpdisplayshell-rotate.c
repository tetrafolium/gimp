...
void gimp_display_shell_rotate_drag(GimpDisplayShell *shell, gdouble last_x,
  ...
  gimp_display_shell_rotate_to(
      shell, constrain ? RINT(shell->rotate_drag_angle / 15.0) * 15.0
...
void gimp_display_shell_rotate_update_transform(GimpDisplayShell *shell) {
  ...
  if ((shell->rotate_angle != 0.0 || shell->flip_horizontally ||
    ...
    gimp_display_shell_scale_get_image_size(shell, &image_width, &image_height);
...
static void gimp_display_shell_save_viewport_center(GimpDisplayShell *shell,
                                                    gdouble *x, gdouble *y) {
  gimp_display_shell_unrotate_xy_f(shell, shell->disp_width / 2,
