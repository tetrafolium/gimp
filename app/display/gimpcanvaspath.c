...
enum { PROP_0, PROP_PATH, PROP_X, PROP_Y, PROP_FILLED, PROP_PATH_STYLE };
...
#define GET_PRIVATE(path)                                                      \
  ((GimpCanvasPathPrivate *)gimp_canvas_path_get_instance_private(             \
      (GimpCanvasPath *)(path)))

/*  local function prototypes  */

static void gimp_canvas_path_finalize(GObject *object);
static void gimp_canvas_path_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_canvas_path_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);
static void gimp_canvas_path_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *gimp_canvas_path_get_extents(GimpCanvasItem *item);
static void gimp_canvas_path_stroke(GimpCanvasItem *item, cairo_t *cr);
...
static void gimp_canvas_path_class_init(GimpCanvasPathClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_PATH_STYLE,
      g_param_spec_enum("path-style", NULL, NULL, GIMP_TYPE_PATH_STYLE,
                        GIMP_PATH_STYLE_DEFAULT, GIMP_PARAM_READWRITE));
}

static void gimp_canvas_path_init(GimpCanvasPath *path) {}
...
static void gimp_canvas_path_set_property(GObject *object, guint property_id,
  ...
  private
    ->path_style = g_value_get_enum(value);
    break;
...
GimpCanvasItem *gimp_canvas_path_new(GimpDisplayShell *shell,
                                     const GimpBezierDesc *bezier, gdouble x,
                                     gdouble y, gboolean filled,
                                     GimpPathStyle style) {
  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), NULL);

  return g_object_new(GIMP_TYPE_CANVAS_PATH, "shell", shell, "path", bezier,
                      "x", x, "y", y, "filled", filled, "path-style", style,
                      NULL);
}

void gimp_canvas_path_set(GimpCanvasItem *path, const GimpBezierDesc *bezier) {
  g_return_if_fail(GIMP_IS_CANVAS_PATH(path));

  gimp_canvas_item_begin_change(path);

  g_object_set(path, "path", bezier, NULL);
