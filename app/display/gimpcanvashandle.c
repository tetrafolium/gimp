...
#define GET_PRIVATE(handle)                                                    \
  ((GimpCanvasHandlePrivate *)gimp_canvas_handle_get_instance_private(         \
      (GimpCanvasHandle *)(handle)))

/*  local function prototypes  */

static void gimp_canvas_handle_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_canvas_handle_get_property(GObject *object, guint property_id,
                                            GValue *value, GParamSpec *pspec);
static void gimp_canvas_handle_draw(GimpCanvasItem *item, cairo_t *cr);
static cairo_region_t *gimp_canvas_handle_get_extents(GimpCanvasItem *item);
static gboolean gimp_canvas_handle_hit(GimpCanvasItem *item, gdouble x,
                                       gdouble y);
...
static void gimp_canvas_handle_class_init(GimpCanvasHandleClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("type", NULL, NULL, GIMP_TYPE_HANDLE_TYPE,
                        GIMP_HANDLE_CROSS, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_ANCHOR,
      g_param_spec_enum("anchor", NULL, NULL, GIMP_TYPE_HANDLE_ANCHOR,
                        GIMP_HANDLE_ANCHOR_CENTER, GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int("width", NULL, NULL, 3, 1001, 7, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_HEIGHT,
      g_param_spec_int("height", NULL, NULL, 3, 1001, 7, GIMP_PARAM_READWRITE));
...
static void gimp_canvas_handle_init(GimpCanvasHandle *handle) {
...
static void gimp_canvas_handle_set_property(GObject *object, guint property_id,
  ...
  private
    ->start_angle = g_value_get_double(value);
    break;
  case PROP_SLICE_ANGLE:
  private
    ->slice_angle = g_value_get_double(value);
    break;
...
static void gimp_canvas_handle_transform(GimpCanvasItem *item, gdouble *x,
                                         gdouble *y) {
  GimpCanvasHandlePrivate *private = GET_PRIVATE(item);

  gimp_canvas_item_transform_xy_f(item, private->x, private->y, x, y);

  switch (private->type) {
  case GIMP_HANDLE_SQUARE:
  case GIMP_HANDLE_DASHED_SQUARE:
  case GIMP_HANDLE_FILLED_SQUARE:
    gimp_canvas_item_shift_to_north_west(private->anchor, *x, *y,
                                         private->width, private->height, x, y);
    break;
  ...
  case GIMP_HANDLE_FILLED_DROP:
    gimp_canvas_item_shift_to_center(private->anchor, *x, *y, private->width,
                                     private->height, x, y);
    break;
...
static void gimp_canvas_handle_draw(GimpCanvasItem *item, cairo_t *cr) {
  ...
  gimp_canvas_item_transform_xy_f(item, private->x, private->y, &tx, &ty);
  ...
  case GIMP_HANDLE_FILLED_DROP:
    ...
    case GIMP_HANDLE_FILLED_DROP:
      ...
      gimp_cairo_arc(cr, x, y, (gdouble) private->width / 2.0, G_PI / 3,
                     G_PI * 4 / 3.);
      cairo_close_path(cr);
  ...
  case GIMP_HANDLE_DASHED_CIRCLE:
    ...
    gimp_cairo_arc(cr, x, y, private->width / 2.0, private->start_angle,
                   private->slice_angle);
...
static gboolean gimp_canvas_handle_hit(GimpCanvasItem *item, gdouble x,
  ...
  gimp_canvas_item_transform_xy_f(item, x, y, &mx, &my);
...
GimpCanvasItem *gimp_canvas_handle_new(GimpDisplayShell *shell,
  ...
  return g_object_new(GIMP_TYPE_CANVAS_HANDLE, "shell", shell, "type", type,
                      "anchor", anchor, "x", x, "y", y, "width", width,
                      "height", height, NULL);
}
...
void gimp_canvas_handle_set_position(GimpCanvasItem *handle, gdouble x,
  ...
  g_object_set(handle, "x", x, "y", y, NULL);
...
gint gimp_canvas_handle_calc_size(GimpCanvasItem *item, gdouble mouse_x,
  ...
  distance =
      gimp_canvas_item_transform_distance_square(item, mouse_x, mouse_y, x, y);
  ...
  return (gint)CLAMP((size * hover_size), normal_size, hover_size);
}
...
void gimp_canvas_handle_get_size(GimpCanvasItem *handle, gint *width,
  ...
  g_object_get(handle, "width", width, "height", height, NULL);
}
...
void gimp_canvas_handle_set_size(GimpCanvasItem *handle, gint width,
  ...
  g_object_set(handle, "width", width, "height", height, NULL);

  gimp_canvas_item_end_change(handle);
}

void gimp_canvas_handle_set_angles(GimpCanvasItem *handle, gdouble start_angle,
                                   gdouble slice_angle) {
  g_return_if_fail(GIMP_IS_CANVAS_HANDLE(handle));

  gimp_canvas_item_begin_change(handle);

  g_object_set(handle, "start-angle", start_angle, "slice-angle", slice_angle,
               NULL);
