...
void gimp_display_shell_autoscroll_start(GimpDisplayShell *shell,
  ...
  info->timeout_id = g_timeout_add(
      AUTOSCROLL_DT, gimp_display_shell_autoscroll_timeout, shell);
...
static gboolean gimp_display_shell_autoscroll_timeout(gpointer data) {
  ...
  gimp_device_info_get_device_coords(
      info->device, gtk_widget_get_window(shell->canvas), &device_coords);
  ...
  if (dx || dy) {
    ...
    gimp_display_shell_scroll_unoverscrollify(shell, scroll_amount_x,
                                              scroll_amount_y, &scroll_amount_x,
                                              &scroll_amount_y);

    gimp_display_shell_scroll(shell, scroll_amount_x, scroll_amount_y);

    gimp_display_shell_untransform_coords(shell, &device_coords, &image_coords);
    ...
    if (gimp_tool_control_get_snap_to(active_tool->control)) {
      ...
      gimp_display_shell_snap_coords(shell, &image_coords, x, y, width, height);
    }

    tool_manager_motion_active(display->gimp, &image_coords, info->time,
                               info->state, display);
