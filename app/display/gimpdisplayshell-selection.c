...
static void selection_draw(Selection *selection, cairo_t *cr);
...
void gimp_display_shell_selection_init(GimpDisplayShell *shell) {
  ...
  g_signal_connect(shell, "window-state-event",
                   G_CALLBACK(selection_window_state_event), selection);
  g_signal_connect(shell, "visibility-notify-event",
                   G_CALLBACK(selection_visibility_notify_event), selection);
}
...
void gimp_display_shell_selection_free(GimpDisplayShell *shell) {
  ...
  g_signal_handlers_disconnect_by_func(shell, selection_window_state_event,
                                       selection);
  g_signal_handlers_disconnect_by_func(shell, selection_visibility_notify_event,
                                       selection);
...
static void selection_start(Selection *selection) {
  ...
  if (selection->paused == 0) {
    selection->timeout =
        g_idle_add((GSourceFunc)selection_start_timeout, selection);
  }
}
...
static void selection_draw(Selection *selection, cairo_t *cr) {
  if (selection->segs_in) {
    gimp_display_shell_draw_selection_in(
        selection->shell, cr, selection->segs_in_mask, selection->index % 8);
  }
}
...
static void selection_render_mask(Selection *selection) {
  ...
  gimp_cairo_segments(cr, selection->segs_in, selection->n_segs_in);
  cairo_stroke(cr);
...
static void selection_zoom_segs(Selection *selection,
  ...
  gimp_display_shell_zoom_segments(selection->shell, src_segs, dest_segs,
                                   n_segs, 0.0, 0.0);
...
static void selection_generate_segs(Selection *selection) {
  ...
  gimp_channel_boundary(gimp_image_get_mask(image), &segs_in, &segs_out,
                        &selection->n_segs_in, &selection->n_segs_out, 0, 0, 0,
                        0);

  if (selection->n_segs_in) {
    selection->segs_in = g_new(GimpSegment, selection->n_segs_in);
    selection_zoom_segs(selection, segs_in, selection->segs_in,
                        selection->n_segs_in);
  ...
  if (selection->n_segs_out) {
    ...
    selection_zoom_segs(selection, segs_out, selection->segs_out,
  ...
  if (selection->show_selection) {
    ...
    if (selection->segs_out) {
      ...
      gimp_display_shell_draw_selection_out(
          selection->shell, cr, selection->segs_out, selection->n_segs_out);
    }
    ...
    if (selection->segs_in && selection->shell_visible) {
      ...
      selection->timeout = g_timeout_add_full(
          G_PRIORITY_DEFAULT_IDLE, config->marching_ants_speed,
          (GSourceFunc)selection_timeout, selection, NULL);
    }
  }
...
static void selection_set_shell_visible(Selection *selection,
...
static gboolean selection_window_state_event(GtkWidget *shell,
                                             GdkEventWindowState *event,
                                             Selection *selection) {
  selection_set_shell_visible(
      selection, (event->new_window_state & (GDK_WINDOW_STATE_WITHDRAWN |
