...
typedef enum { GRAB_NONE, GRAB_SELECTION, GRAB_LINE } GimpToolLineGrab;
...
static void gimp_tool_line_set_property(GObject *object, guint property_id,
                                        const GValue *value, GParamSpec *pspec);
static void gimp_tool_line_get_property(GObject *object, guint property_id,
...
static void gimp_tool_line_motion_modifier(GimpToolWidget *widget,
                                           GdkModifierType key, gboolean press,
                                           GdkModifierType state);
...
static gint gimp_tool_line_get_hover(GimpToolLine *line,
                                     const GimpCoords *coords,
                                     GdkModifierType state);
static GimpControllerSlider *gimp_tool_line_get_slider(GimpToolLine *line,
                                                       gint slider);
static GimpCanvasItem *gimp_tool_line_get_handle(GimpToolLine *line,
                                                 gint handle);
static gdouble gimp_tool_line_project_point(GimpToolLine *line, gdouble x,
                                            gdouble y, gboolean constrain,
                                            gdouble *dist);

static gboolean gimp_tool_line_selection_motion(GimpToolLine *line,
                                                gboolean constrain);
...
static gboolean gimp_tool_line_handle_hit(GimpCanvasItem *handle, gdouble x,
                                          gdouble y, gdouble *min_dist);
...
static void gimp_tool_line_class_init(GimpToolLineClass *klass) {
  ...
  line_signals[CAN_ADD_SLIDER] = g_signal_new(
      "can-add-slider", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpToolLineClass, can_add_slider), NULL, NULL,
      gimp_marshal_BOOLEAN__DOUBLE, G_TYPE_BOOLEAN, 1, G_TYPE_DOUBLE);

  line_signals[ADD_SLIDER] =
      g_signal_new("add-slider", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpToolLineClass, add_slider), NULL, NULL,
                   gimp_marshal_INT__DOUBLE, G_TYPE_INT, 1, G_TYPE_DOUBLE);

  line_signals[PREPARE_TO_REMOVE_SLIDER] = g_signal_new(
      "prepare-to-remove-slider", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolLineClass, prepare_to_remove_slider), NULL, NULL,
      gimp_marshal_VOID__INT_BOOLEAN, G_TYPE_NONE, 2, G_TYPE_INT,
      G_TYPE_BOOLEAN);

  line_signals[REMOVE_SLIDER] = g_signal_new(
      "remove-slider", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolLineClass, remove_slider), NULL, NULL, NULL,
      G_TYPE_NONE, 1, G_TYPE_INT);

  line_signals[SELECTION_CHANGED] = g_signal_new(
      "selection-changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolLineClass, selection_changed), NULL, NULL, NULL,
      G_TYPE_NONE, 0);

  line_signals[HANDLE_CLICKED] = g_signal_new(
      "handle-clicked", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(GimpToolLineClass, handle_clicked), NULL, NULL,
      gimp_marshal_BOOLEAN__INT_UINT_ENUM, G_TYPE_BOOLEAN, 3, G_TYPE_INT,
      G_TYPE_UINT, GIMP_TYPE_BUTTON_PRESS_TYPE);
  ...
  g_object_class_install_property(
      object_class, PROP_SELECTION,
      g_param_spec_int("selection", NULL, NULL, GIMP_TOOL_LINE_HANDLE_NONE,
                       G_MAXINT, GIMP_TOOL_LINE_HANDLE_NONE,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_STATUS_TITLE,
      g_param_spec_string("status-title", NULL, NULL, _("Line: "),
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_tool_line_init(GimpToolLine *line) {
  GimpToolLinePrivate *private;

private
  = line->private = gimp_tool_line_get_instance_private(line);

private
  ->sliders = g_array_new(FALSE, FALSE, sizeof(GimpControllerSlider));

private
  ->selection = GIMP_TOOL_LINE_HANDLE_NONE;
private
  ->hover = GIMP_TOOL_LINE_HANDLE_NONE;
private
  ->grab = GRAB_NONE;

private
  ->slider_handles = g_array_new(FALSE, TRUE, sizeof(GimpCanvasItem *));
}
...
private
  ->line = gimp_tool_widget_add_line(widget, private->x1, private->y1,
                                     private->x2, private->y2);

  gimp_canvas_item_set_visible(private->line, SHOW_LINE);

private
  ->start_handle = gimp_tool_widget_add_handle(
      widget, ENDPOINT_HANDLE_TYPE, private->x1, private->y1,
      ENDPOINT_HANDLE_SIZE, ENDPOINT_HANDLE_SIZE, GIMP_HANDLE_ANCHOR_CENTER);

private
  ->end_handle = gimp_tool_widget_add_handle(
      widget, ENDPOINT_HANDLE_TYPE, private->x2, private->y2,
      ENDPOINT_HANDLE_SIZE, ENDPOINT_HANDLE_SIZE, GIMP_HANDLE_ANCHOR_CENTER);

private
  ->slider_group = gimp_canvas_group_new(gimp_tool_widget_get_shell(widget));
  gimp_tool_widget_add_item(widget, private->slider_group);
  g_object_unref(private->slider_group);

private
  ->handle_circle = gimp_tool_widget_add_handle(
      widget, GIMP_HANDLE_CIRCLE, private->x1, private->y1,
      ENDPOINT_HANDLE_SIZE * HANDLE_CIRCLE_SCALE,
      ENDPOINT_HANDLE_SIZE * HANDLE_CIRCLE_SCALE, GIMP_HANDLE_ANCHOR_CENTER);
...
static void gimp_tool_line_set_property(GObject *object, guint property_id,
  ...
  case PROP_SELECTION: {
    ...
    g_return_if_fail(selection < 0 ||
                     gimp_tool_line_get_slider(line, selection)->selectable);
...
static void gimp_tool_line_changed(GimpToolWidget *widget) {
  ...
  gimp_canvas_line_set(private->line, private->x1, private->y1, private->x2,
                       private->y2);

  gimp_canvas_handle_set_position(private->start_handle, private->x1,
                                  private->y1);

  gimp_canvas_handle_set_position(private->end_handle, private->x2,
                                  private->y2);
  ...
  for (i = 0; i < private->sliders->len; i++) {
    ...
    if (*handle) {
      ...
      *handle =
          gimp_canvas_handle_new(gimp_tool_widget_get_shell(widget),
                                 SLIDER_HANDLE_TYPE, GIMP_HANDLE_ANCHOR_CENTER,
                                 x, y, SLIDER_HANDLE_SIZE, SLIDER_HANDLE_SIZE);
...
gboolean gimp_tool_line_button_press(GimpToolWidget *widget,
                                     const GimpCoords *coords, guint32 time,
                                     GdkModifierType state,
                                     GimpButtonPressType press_type) {
  GimpToolLine *line = GIMP_TOOL_LINE(widget);
...
private
  ...
  if (press_type != GIMP_BUTTON_PRESS_NORMAL &&
    ...
    g_signal_emit(line, line_signals[HANDLE_CLICKED], 0, private->selection,
                  state, press_type, &result);
...
void gimp_tool_line_button_release(GimpToolWidget *widget,
                                   const GimpCoords *coords, guint32 time,
                                   GdkModifierType state,
                                   GimpButtonReleaseType release_type) {
  GimpToolLine *line = GIMP_TOOL_LINE(widget);
...
private
  ...
  if (release_type == GIMP_BUTTON_RELEASE_CANCEL) {
    if (grab != GRAB_NONE) {
      ...
      g_object_set(line, "x1", private->saved_x1, "y1", private->saved_y1, "x2",
                   private->saved_x2, "y2", private->saved_y2, NULL);
    }
    ...
    private
      ...
      g_signal_emit(line, line_signals[REMOVE_SLIDER], 0, private->selection);
    } else if (release_type == GIMP_BUTTON_RELEASE_CLICK) {
      gboolean result;

      g_signal_emit(line, line_signals[HANDLE_CLICKED], 0, private->selection,
                    state, GIMP_BUTTON_PRESS_NORMAL, &result);
    }
  }
}

void gimp_tool_line_motion(GimpToolWidget *widget, const GimpCoords *coords,
                           guint32 time, GdkModifierType state) {
  GimpToolLine *line = GIMP_TOOL_LINE(widget);
  GimpToolLinePrivate *private = line->private;
...
private
  ...
  if (private->grab == GRAB_LINE) {
    g_object_set(line, "x1", private->x1 + diff_x, "y1", private->y1 + diff_y,
                 "x2", private->x2 + diff_x, "y2", private->y2 + diff_y, NULL);
  } else {
...
GimpHit gimp_tool_line_hit(GimpToolWidget *widget, const GimpCoords *coords,
                           GdkModifierType state, gboolean proximity) {
  GimpToolLine *line = GIMP_TOOL_LINE(widget);
...
void gimp_tool_line_hover(GimpToolWidget *widget, const GimpCoords *coords,
                          GdkModifierType state, gboolean proximity) {
  GimpToolLine *line = GIMP_TOOL_LINE(widget);
  GimpToolLinePrivate *private = line->private;
...
private
  ...
  if (!(state & GRAB_LINE_MASK))
  private
  ->hover = gimp_tool_line_get_hover(line, coords, state);
  else private->hover = GIMP_TOOL_LINE_HANDLE_NONE;
  ...
  case GDK_KEY_Down:
    ...
    if (private->selection < 0 || move_line) {
      ...
      if (private->selection == GIMP_TOOL_LINE_HANDLE_START || move_line) {
        g_object_set(line, "x1", private->x1 + dx, "y1", private->y1 + dy,
                     NULL);
      }

      if (private->selection == GIMP_TOOL_LINE_HANDLE_END || move_line) {
        g_object_set(line, "x2", private->x2 + dx, "y2", private->y2 + dy,
                     NULL);
      }
    }
    /* move a slider */
      ...
      dist = gimp_canvas_item_transform_distance(
          private->line, private->x1, private->y1, private->x2, private->y2);
      ...
      case GDK_KEY_Up:
        if (private->y1 < private->y2)
          dvalue = -dist;
        else if (private->y1 > private->y2)
          dvalue = +dist;
        break;
      ...
      if (dvalue != 0.0) {
        ...
        g_object_set(line, "sliders", private->sliders, NULL);
      }
    }
    return TRUE;
...
static gint gimp_tool_line_get_hover(GimpToolLine *line,
  ...
  for (i = first_handle; i > GIMP_TOOL_LINE_HANDLE_NONE; i--) {
    ...
    if (gimp_tool_line_handle_hit(handle, private->mouse_x, private->mouse_y,
                                  &min_dist)) {
      hover = i;
    }
  }
  ...
  if (hover == GIMP_TOOL_LINE_HANDLE_NONE) {
    ...
    value = gimp_tool_line_project_point(line, private->mouse_x,
                                         private->mouse_y, constrain, &dist);

    if (value >= 0.0 && value <= 1.0 && dist <= LINE_VICINITY) {
      gboolean can_add;

      g_signal_emit(line, line_signals[CAN_ADD_SLIDER], 0, value, &can_add);
...
static GimpCanvasItem *gimp_tool_line_get_handle(GimpToolLine *line,
  ...
  default:
    gimp_assert(handle >= 0 && handle < (gint) private->slider_handles->len);

    return g_array_index(private->slider_handles, GimpCanvasItem *, handle);
  }
}
...
static gdouble gimp_tool_line_project_point(GimpToolLine *line, gdouble x,
  ...
  length_sqr = SQR(private->x2 - private->x1) + SQR(private->y2 - private->y1);
  ...
  if (length_sqr > 0.0) {
    ...
    if (dist) {
      ...
      *dist = gimp_canvas_item_transform_distance(private->line, x, y, px, py);
    }
    ...
    if (dist) {
      *dist = gimp_canvas_item_transform_distance(private->line, x, y,
  ...
  case GIMP_TOOL_LINE_HANDLE_START:
    if (constrain) {
      gimp_display_shell_constrain_line(
          ...
          private->y2, &x, &y, GIMP_CONSTRAIN_LINE_15_DEGREES);
    }

    g_object_set(line, "x1", x, "y1", y, NULL);
    return TRUE;

  case GIMP_TOOL_LINE_HANDLE_END:
    if (constrain) {
      gimp_display_shell_constrain_line(
          gimp_tool_widget_get_shell(GIMP_TOOL_WIDGET(line)), private->x1,
          private->y1, &x, &y, GIMP_CONSTRAIN_LINE_15_DEGREES);
    }

    g_object_set(line, "x2", x, "y2", y, NULL);
    return TRUE;
  ...
  default: {
    ...
    private
       ...
       */
      {
        ...
        gimp_tool_line_get_cursor(GIMP_TOOL_WIDGET(line), NULL, 0, &cursor,
                                  &tool_cursor, &modifier);
      ...
      gimp_tool_line_update_handles(line);
      gimp_tool_line_update_circle(line);
      gimp_tool_line_update_status(
          line, constrain ? gimp_get_constrain_behavior_mask() : 0, TRUE);
    }
...
static void gimp_tool_line_update_handles(GimpToolLine *line) {
  ...
  value = gimp_tool_line_project_point(line, private->mouse_x, private->mouse_y,
                                       FALSE, &dist);
  ...
  for (i = 0; i < private->sliders->len; i++) {
    ...
    show_autohidden =
        ...
        (private->grab == GRAB_NONE &&
         (private->hover <= GIMP_TOOL_LINE_HANDLE_NONE ||
          private->hover == i) &&
         dist <= hit_radius && value >= slider->min && value <= slider->max);
    ...
    if (visible) {
      g_object_set(handle, "type", slider->type, "width", size, "height", size,
                   NULL);
    }
...
static void gimp_tool_line_update_circle(GimpToolLine *line) {
  ...
  visible = (private->grab == GRAB_NONE &&
             private->hover != GIMP_TOOL_LINE_HANDLE_NONE) ||
            (private->grab == GRAB_SELECTION && private->remove_slider);
  ...
  if (visible) {
    ...
    if (private->grab == GRAB_NONE && private->hover == HOVER_NEW_SLIDER) {
      /* new slider */
      x = private->x1 + (private->x2 - private->x1) * private->new_slider_value;
    ...
    g_object_set(private->handle_circle, "type",
                 dashed ? GIMP_HANDLE_DASHED_CIRCLE : GIMP_HANDLE_CIRCLE, NULL);
  }
...
static void gimp_tool_line_update_hilight(GimpToolLine *line) {
  ...
  for (i = GIMP_TOOL_LINE_HANDLE_NONE + 1; i < (gint) private->sliders->len;
       i++) {
    GimpCanvasItem *handle;
...
static void gimp_tool_line_update_status(GimpToolLine *line,
  ...
  if (proximity) {
    ...
    if (handle == GIMP_TOOL_LINE_HANDLE_START ||
      ...
      line_status = gimp_display_shell_get_line_status(
          shell, _("Click-Drag to move the endpoint"), ". ", private->x1,
          private->y1, private->x2, private->y2);
    ...
    status = gimp_suggest_modifiers(
        ...
        ((toggle_behavior_format ? gimp_get_constrain_behavior_mask() : 0) |
         (private->grab == GRAB_NONE ? GDK_MOD1_MASK : 0)) &
            ~state,
        NULL, toggle_behavior_format, _("%s to move the whole line"));

    if (message || line_status) {
      gimp_tool_widget_set_status(GIMP_TOOL_WIDGET(line), status);
    } else {
      line_status = gimp_display_shell_get_line_status(
          shell, private->status_title, ". ", private->x1, private->y1,
          private->x2, private->y2);
      gimp_tool_widget_set_status_coords(GIMP_TOOL_WIDGET(line), line_status,
                                         private->x2 - private->x1, ", ",
                                         private->y2 - private->y1, status);
    }
...
static gboolean gimp_tool_line_handle_hit(GimpCanvasItem *handle, gdouble x,
  ...
  dist = gimp_canvas_item_transform_distance(handle, x, y, handle_x, handle_y);
...
GimpToolWidget *gimp_tool_line_new(GimpDisplayShell *shell, gdouble x1,
  ...
  return g_object_new(GIMP_TYPE_TOOL_LINE, "shell", shell, "x1", x1, "y1", y1,
                      "x2", x2, "y2", y2, NULL);
}
...
private
  ...
  g_object_set(line, "selection", handle, NULL);
}
