...
static GdkDevice *get_associated_pointer(GdkDevice *device) {
...
static GdkDevice *get_associated_keyboard(GdkDevice *device) {
  switch (gdk_device_get_device_type(device)) {
  case GDK_DEVICE_TYPE_SLAVE:
    device = gdk_device_get_associated_device(device);
    break;
...
gboolean gimp_display_shell_pointer_grab(GimpDisplayShell *shell,
  ...
  source_device =
      gimp_devices_get_from_event(shell->display->gimp, event, &device);
  ...
  status = gdk_device_grab(device, gtk_widget_get_window(shell->canvas),
                           GDK_OWNERSHIP_APPLICATION, FALSE, event_mask, NULL,
  ...
  g_printerr("%s: gdk_device_grab(%s) failed with status %d\n", G_STRFUNC,
             gdk_device_get_name(device), status);
...
gboolean gimp_display_shell_keyboard_grab(GimpDisplayShell *shell,
  ...
  status = gdk_device_grab(
      ...
      FALSE, GDK_KEY_PRESS_MASK | GDK_KEY_RELEASE_MASK | GDK_FOCUS_CHANGE_MASK,
