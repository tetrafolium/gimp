...
static void gimp_display_update_handler(GimpProjection *projection,
                                        gboolean now, gint x, gint y, gint w,
                                        gint h, GimpDisplay *display);

static void gimp_display_bounds_changed_handler(GimpImage *image, gint old_x,
                                                gint old_y,
                                                GimpDisplay *display);
...
void gimp_display_connect(GimpDisplay *display) {
  ...
  g_signal_connect(gimp_image_get_projection(image), "update",
                   G_CALLBACK(gimp_display_update_handler), display);

  g_signal_connect(image, "bounds-changed",
                   G_CALLBACK(gimp_display_bounds_changed_handler), display);
  g_signal_connect(image, "flush", G_CALLBACK(gimp_display_flush_handler),
                   display);
}
...
void gimp_display_disconnect(GimpDisplay *display) {
  ...
  g_signal_handlers_disconnect_by_func(image, gimp_display_flush_handler,
                                       display);
  g_signal_handlers_disconnect_by_func(
      image, gimp_display_bounds_changed_handler, display);

  g_signal_handlers_disconnect_by_func(gimp_image_get_projection(image),
                                       gimp_display_update_handler, display);
}
