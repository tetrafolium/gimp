...
static void gimp_display_shell_close_name_changed(GimpImage *image,
                                                  GimpMessageBox *box);
static void gimp_display_shell_close_exported(GimpImage *image, GFile *file,
                                              GimpMessageBox *box);
static gboolean gimp_display_shell_close_time_changed(GimpMessageBox *box);
static void gimp_display_shell_close_response(GtkWidget *widget, gboolean close,
...
static void gimp_time_since(gint64 then, gint *hours, gint *minutes);
...
void gimp_display_shell_close(GimpDisplayShell *shell, gboolean kill_it) {
   ...
   */
  if (!kill_it && image && gimp_image_get_display_count(image) == 1 &&
      gimp_image_is_dirty(image)) {
    /*  If there's a save dialog active for this image, then raise it.
     *  (see bug #511965)
     */
    GtkWidget *dialog =
        g_object_get_data(G_OBJECT(image), "gimp-file-save-dialog");
    if (dialog) {
...
static void gimp_display_shell_close_dialog(GimpDisplayShell *shell,
  ...
  shell->close_dialog = dialog = gimp_message_dialog_new(
      title, GIMP_ICON_DOCUMENT_SAVE, GTK_WIDGET(shell),
      GTK_DIALOG_DESTROY_WITH_PARENT, gimp_standard_help_func, NULL,

      file ? _("_Save") : _("Save _As"), RESPONSE_SAVE, _("_Cancel"),
      GTK_RESPONSE_CANCEL, _("_Discard Changes"), GTK_RESPONSE_CLOSE, NULL);

  g_free(title);

  gtk_dialog_set_default_response(GTK_DIALOG(dialog), GTK_RESPONSE_CANCEL);
  gimp_dialog_set_alternative_button_order(GTK_DIALOG(dialog), RESPONSE_SAVE,
                                           GTK_RESPONSE_CLOSE,
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(gimp_display_shell_close_response), shell);
  ...
  closure =
      g_closure_new_object(sizeof(GClosure), G_OBJECT(shell->close_dialog));
  ...
  hint = g_strdup_printf(
      _("Press %s to discard all changes and close the image."), accel_string);
  ...
  g_signal_connect_object(
      image, "exported", G_CALLBACK(gimp_display_shell_close_exported), box, 0);

  gimp_display_shell_close_name_changed(image, box);

  closure = g_cclosure_new_object(
      G_CALLBACK(gimp_display_shell_close_time_changed), G_OBJECT(box));
...
static void gimp_display_shell_close_name_changed(GimpImage *image,
  ...
  if (GTK_IS_WINDOW(window)) {
    gchar *title =
        g_strdup_printf(_("Close %s"), gimp_image_get_display_name(image));
  ...
  if (dirty_time) {
    ...
    if (hours > 0) {
      if (hours > 1 || minutes == 0) {
        time_text = g_strdup_printf(ngettext("If you don't save the image, "
...
static void gimp_display_shell_close_response(GtkWidget *widget,
  ...
  case RESPONSE_SAVE: {
    ...
    if (window) {
      ...
      gimp_ui_manager_activate_action(manager, "file", "file-save-and-close");
    }
  } break;
