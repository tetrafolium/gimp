...
#define GET_PRIVATE(text_cursor)                                               \
  ((GimpCanvasTextCursorPrivate *)                                             \
       gimp_canvas_text_cursor_get_instance_private(                           \
           (GimpCanvasTextCursor *)(text_cursor)))
...
static void gimp_canvas_text_cursor_get_property(GObject *object,
                                                 ...
                                                 GValue *value,
                                                 GParamSpec *pspec);
static void gimp_canvas_text_cursor_draw(GimpCanvasItem *item, cairo_t *cr);
...
gimp_canvas_text_cursor_class_init(GimpCanvasTextCursorClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int("width", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_HEIGHT,
      g_param_spec_int("height", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                       GIMP_MAX_IMAGE_SIZE, 0, GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("direction", NULL, NULL, gimp_text_direction_get_type(),
                        GIMP_TEXT_DIRECTION_LTR, GIMP_PARAM_READWRITE));
}

static void gimp_canvas_text_cursor_init(GimpCanvasTextCursor *text_cursor) {}
...
static void gimp_canvas_text_cursor_set_property(GObject *object,
  ...
  case PROP_OVERWRITE:
  private
    ->overwrite = g_value_get_boolean(value);
    break;
  case PROP_DIRECTION:
  private
...
static void gimp_canvas_text_cursor_transform(GimpCanvasItem *item, gdouble *x,
                                              gdouble *y, gdouble *w,
                                              gdouble *h) {
  ...
  gimp_canvas_item_transform_xy_f(
      ...
      MIN(private->y, private->y + private->height), x, y);
  gimp_canvas_item_transform_xy_f(
      item, MAX(private->x, private->x + private->width),
      MAX(private->y, private->y + private->height), w, h);
...
GimpCanvasItem *gimp_canvas_text_cursor_new(GimpDisplayShell *shell,
  ...
  return g_object_new(GIMP_TYPE_CANVAS_TEXT_CURSOR, "shell", shell, "x",
                      cursor->x, "y", cursor->y, "width", cursor->width,
                      "height", cursor->height, "overwrite", overwrite,
