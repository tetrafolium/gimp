...
typedef struct _GimpCanvasTransformPreviewPrivate
...
#define GET_PRIVATE(transform_preview)                                         \
  ((GimpCanvasTransformPreviewPrivate *)                                       \
       gimp_canvas_transform_preview_get_instance_private(                     \
...
static void gimp_canvas_transform_preview_layer_changed(
    GimpLayer *layer, GimpCanvasTransformPreview *transform_preview);

static void gimp_canvas_transform_preview_set_pickable(
    GimpCanvasTransformPreview *transform_preview, GimpPickable *pickable);
static void gimp_canvas_transform_preview_sync_node(
    GimpCanvasTransformPreview *transform_preview);

G_DEFINE_TYPE_WITH_PRIVATE(GimpCanvasTransformPreview,
                           gimp_canvas_transform_preview, GIMP_TYPE_CANVAS_ITEM)

#define parent_class gimp_canvas_transform_preview_parent_class

/* private functions */

static void gimp_canvas_transform_preview_class_init(
    GimpCanvasTransformPreviewClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_CLIP,
      g_param_spec_enum("clip", NULL, NULL, GIMP_TYPE_TRANSFORM_RESIZE,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("x1", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                          GIMP_MAX_IMAGE_SIZE, 0.0, GIMP_PARAM_READWRITE));
...
static void gimp_canvas_transform_preview_init(
    GimpCanvasTransformPreview *transform_preview) {
...
static void gimp_canvas_transform_preview_dispose(GObject *object) {
  GimpCanvasTransformPreview *transform_preview =
      GIMP_CANVAS_TRANSFORM_PREVIEW(object);
...
gimp_canvas_transform_preview_transform(GimpCanvasItem *item,
  ...
  if (!gimp_transform_resize_boundary(&private->transform, private->clip,
                                      private->x1, private->y1, private->x2,
                                      private->y2, &x1, &y1, &x2, &y2)) {
    return FALSE;
  }
...
static void gimp_canvas_transform_preview_draw(GimpCanvasItem *item,
                                               cairo_t *cr) {
  GimpCanvasTransformPreview *transform_preview =
      GIMP_CANVAS_TRANSFORM_PREVIEW(item);
  GimpCanvasTransformPreviewPrivate *private = GET_PRIVATE(item);
  ...
  if (!gegl_rectangle_intersect(
          &bounds,
          GEGL_RECTANGLE(extents.x, extents.y, extents.width, extents.height),
          GEGL_RECTANGLE(clip_x1, clip_y1, clip_x2 - clip_x1,
                         clip_y2 - clip_y1))) {
    return;
  }

  surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, bounds.width,
                                       bounds.height);
  ...
  gegl_node_blit(private->node_output, 1.0,
                 GEGL_RECTANGLE(bounds.x + shell->offset_x,
                                bounds.y + shell->offset_y, bounds.width,
                                bounds.height),
...
static void gimp_canvas_transform_preview_set_pickable(
  ...
  if (private->pickable && GIMP_IS_LAYER(private->pickable)) {
    g_signal_handlers_disconnect_by_func(
        private->pickable, gimp_canvas_transform_preview_layer_changed,
        transform_preview);
  }
...
static void gimp_canvas_transform_preview_sync_node(
  ...
  private
    ->source_node = gegl_node_new_child(private->node, "operation",
                                        "gimp:buffer-source-validate", NULL);

  private
    ->convert_format_node = gegl_node_new_child(private->node, "operation",
                                                "gegl:convert-format", NULL);

  private
    ->layer_mask_source_node = gegl_node_new_child(
        private->node, "operation", "gimp:buffer-source-validate", NULL);

  private
    ->layer_mask_opacity_node =
        gegl_node_new_child(private->node, "operation", "gegl:opacity", NULL);

  private
    ->mask_source_node = gegl_node_new_child(
        private->node, "operation", "gimp:buffer-source-validate", NULL);

  private
    ->mask_translate_node =
        gegl_node_new_child(private->node, "operation", "gegl:translate", NULL);

  private
    ->mask_crop_node =
        gegl_node_new_child(private->node, "operation", "gegl:crop", "width",
                            0.0, "height", 0.0, NULL);
  ...
  private
    ->cache_node =
        gegl_node_new_child(private->node, "operation", "gegl:cache", NULL);

  private
    ->transform_node = gegl_node_new_child(
        private->node, "operation", "gegl:transform", "near-z",
        GIMP_TRANSFORM_NEAR_Z, "sampler", GIMP_INTERPOLATION_NONE, NULL);

    gegl_node_link_many(private->source_node, private->convert_format_node,
                        private->transform_node, NULL);

    gegl_node_connect_to(private->layer_mask_source_node, "output",
                         private->layer_mask_opacity_node, "aux");

    gegl_node_link_many(private->mask_source_node, private->mask_translate_node,
                        private->mask_crop_node, NULL);

  private
    ->node_pickable = NULL;
  private
    ->node_layer_mask = NULL;
  private
    ->node_mask = NULL;
  private
    ->node_rect = *GEGL_RECTANGLE(0, 0, 0, 0);
  private
    ->node_opacity = 1.0;
    gimp_matrix3_identity(&private->node_matrix);
  private
    ->node_output = private->transform_node;
  }

  if (GIMP_IS_ITEM(pickable)) {
    gimp_item_get_offset(GIMP_ITEM(private->pickable), &offset_x, &offset_y);

    if (gimp_item_mask_bounds(GIMP_ITEM(pickable), NULL, NULL, NULL, NULL)) {
  ...
  if (pickable != private->node_pickable) {
    ...
    gegl_node_set(private->source_node, "buffer", buffer, NULL);
    gegl_node_set(private->convert_format_node, "format",
                  gimp_pickable_get_format_with_alpha(pickable), NULL);

    g_object_unref(buffer);
  }

  if (layer_mask != private->node_layer_mask) {
    gegl_node_set(private->layer_mask_source_node, "buffer",
                  layer_mask ? gimp_drawable_get_buffer(layer_mask) : NULL,
                  NULL);
  }
  ...
  if (mask) {
    ...
    if (mask != private->node_mask) {
      gegl_node_set(private->mask_source_node, "buffer",
                    gimp_drawable_get_buffer(mask), NULL);
    }
    ...
    private
      ...
      gegl_node_set(private->mask_translate_node, "x", (gdouble)-rect.x, "y",
                    (gdouble)-rect.y, NULL);

      gegl_node_set(private->mask_crop_node, "width", (gdouble)rect.width,
                    "height", (gdouble)rect.height, NULL);
    }
  ...
  if (opacity != private->node_opacity) {
    gegl_node_set(private->opacity_node, "value", opacity, NULL);
  }

  if (layer_mask != private->node_layer_mask || mask != private->node_mask ||
...
private
  ->node_pickable = pickable;
private
  ->node_layer_mask = layer_mask;
private
  ->node_mask = mask;
private
...
GimpCanvasItem *gimp_canvas_transform_preview_new(GimpDisplayShell *shell,
  ...
  return g_object_new(GIMP_TYPE_CANVAS_TRANSFORM_PREVIEW, "shell", shell,
                      "pickable", pickable, "transform", transform, "x1", x1,
                      "y1", y1, "x2", x2, "y2", y2, NULL);
}
