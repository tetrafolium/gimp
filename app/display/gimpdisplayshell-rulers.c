...
void gimp_display_shell_rulers_update(GimpDisplayShell *shell) {
  ...
  if (image) {
    horizontal_upper = gimp_pixels_to_units(shell->disp_width / scale_x,
                                            shell->unit, resolution_x);
    horizontal_max_size = gimp_pixels_to_units(MAX(image_width, image_height),
                                               shell->unit, resolution_x);

    vertical_upper = gimp_pixels_to_units(shell->disp_height / scale_y,
                                          shell->unit, resolution_y);
    vertical_max_size = gimp_pixels_to_units(MAX(image_width, image_height),
                                             shell->unit, resolution_y);
  } else {
  ...
  gimp_ruler_set_range(GIMP_RULER(shell->hrule), horizontal_lower,
                       horizontal_upper, horizontal_max_size);

  gimp_ruler_set_unit(GIMP_RULER(shell->hrule), shell->unit);

  gimp_ruler_set_range(GIMP_RULER(shell->vrule), vertical_lower, vertical_upper,
                       vertical_max_size);
