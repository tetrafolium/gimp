...
enum { PROP_0, PROP_X, PROP_Y, PROP_WIDTH, PROP_HEIGHT, PROP_FILLED };
...
#define GET_PRIVATE(rectangle)                                                 \
  ((GimpCanvasRectanglePrivate *)gimp_canvas_rectangle_get_instance_private(   \
      (GimpCanvasRectangle *)(rectangle)))
...
static void gimp_canvas_rectangle_get_property(GObject *object,
                                               ...
                                               GParamSpec *pspec);
static void gimp_canvas_rectangle_draw(GimpCanvasItem *item, cairo_t *cr);
...
static void gimp_canvas_rectangle_class_init(GimpCanvasRectangleClass *klass) {
...
static void gimp_canvas_rectangle_init(GimpCanvasRectangle *rectangle) {}
...
static void gimp_canvas_rectangle_transform(GimpCanvasItem *item, gdouble *x,
                                            gdouble *y, gdouble *w,
                                            gdouble *h) {
  ...
  gimp_canvas_item_transform_xy_f(
      ...
      MIN(private->y, private->y + private->height), &x1, &y1);
  gimp_canvas_item_transform_xy_f(
      item, MAX(private->x, private->x + private->width),
      MAX(private->y, private->y + private->height), &x2, &y2);
...
GimpCanvasItem *gimp_canvas_rectangle_new(GimpDisplayShell *shell, gdouble x,
                                          gdouble y, gdouble width,
                                          gdouble height, gboolean filled) {
  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), NULL);

  return g_object_new(GIMP_TYPE_CANVAS_RECTANGLE, "shell", shell, "x", x, "y",
                      y, "width", width, "height", height, "filled", filled,
                      NULL);
}

void gimp_canvas_rectangle_set(GimpCanvasItem *rectangle, gdouble x, gdouble y,
                               gdouble width, gdouble height) {
  g_return_if_fail(GIMP_IS_CANVAS_RECTANGLE(rectangle));

  gimp_canvas_item_begin_change(rectangle);

  g_object_set(rectangle, "x", x, "y", y, "width", width, "height", height,
               NULL);
