...
void gimp_display_shell_render_set_scale(GimpDisplayShell *shell, gint scale) {
...
void gimp_display_shell_render_invalidate_area(GimpDisplayShell *shell, gint x,
                                               gint y, gint width,
                                               gint height) {
  g_return_if_fail(GIMP_IS_DISPLAY_SHELL(shell));
...
void gimp_display_shell_render_validate_area(GimpDisplayShell *shell, gint x,
...
gboolean gimp_display_shell_render_is_valid(GimpDisplayShell *shell, gint x,
                                            gint y, gint width, gint height) {
  if (shell->render_cache_valid) {
    cairo_rectangle_int_t rect;
    ...
    overlap = cairo_region_contains_rectangle(shell->render_cache_valid, &rect);
...
void gimp_display_shell_render(GimpDisplayShell *shell, cairo_t *cr, gint tx,
                               gint ty, gint twidth, gint theight,
                               gdouble scale) {
  ...
  gimp_display_shell_untransform_bounds_with_scale(
      shell, scale, tx, ty, tx + twidth, ty + theight, &x1, &y1, &x2, &y2);
  ...
  buffer = gimp_pickable_get_buffer(gimp_display_shell_get_pickable(shell));
#ifdef USE_NODE_BLIT
  ...
  if (!shell->render_surface) {
    shell->render_surface = cairo_surface_create_similar_image(
        cairo_get_target(cr), CAIRO_FORMAT_ARGB32, shell->render_buf_width,
  ...
  if (!shell->render_cache) {
    shell->render_cache = cairo_surface_create_similar_image(
        cairo_get_target(cr), CAIRO_FORMAT_ARGB32,
  ...
  if (shell->profile_transform || gimp_display_shell_has_filter(shell)) {
    ...
    if ((gimp_display_shell_has_filter(shell) || !can_convert_to_u8) &&
      ...
      shell->filter_data = gegl_malloc(
          fw * fh * babl_format_get_bytes_per_pixel(shell->filter_format));

      shell->filter_stride =
          fw * babl_format_get_bytes_per_pixel(shell->filter_format);

      shell->filter_buffer = gegl_buffer_linear_new_from_data(
          shell->filter_data, shell->filter_format,
          GEGL_RECTANGLE(0, 0, fw, fh), GEGL_AUTO_ROWSTRIDE,
          (GDestroyNotify)gegl_free, shell->filter_data);
    }
    ...
    if (!gimp_display_shell_has_filter(shell) || shell->filter_transform) {
      ...
      gegl_buffer_get(buffer, GEGL_RECTANGLE(x, y, width, height), scale,
                      gimp_projectable_get_format(GIMP_PROJECTABLE(image)),
                      shell->profile_data, shell->profile_stride,
                      abyss_policy | filter);
#else
      gegl_node_blit(node, scale, GEGL_RECTANGLE(x, y, width, height),
                     gimp_projectable_get_format(GIMP_PROJECTABLE(image)),
                     shell->profile_data, shell->profile_stride,
...
#ifndef USE_NODE_BLIT
      gegl_buffer_get(buffer, GEGL_RECTANGLE(x, y, width, height), scale,
                      shell->filter_format, shell->filter_data,
                      shell->filter_stride, abyss_policy | filter);
#else
      gegl_node_blit(node, scale, GEGL_RECTANGLE(x, y, width, height),
                     shell->filter_format, shell->filter_data,
                     shell->filter_stride, GEGL_BLIT_CACHE | filter);
#endif
    }
    ...
    if (shell->filter_transform) {
      gimp_color_transform_process_buffer(
          shell->filter_transform, shell->profile_buffer,
          GEGL_RECTANGLE(0, 0, width, height), shell->filter_buffer,
    ...
    if (gimp_display_shell_has_filter(shell)) {
       ...
       */
      filter_buffer =
          g_object_new(GEGL_TYPE_BUFFER, "source", shell->filter_buffer,
                       "shift-x", -x, "shift-y", -y, NULL);

      /*  convert the filter_buffer in place
       */
      gimp_color_display_stack_convert_buffer(
          shell->filter_stack, filter_buffer,
    ...
    if (shell->profile_transform) {
      if (gimp_display_shell_has_filter(shell)) {
         ...
         */
        gimp_color_transform_process_buffer(
            shell->profile_transform, shell->filter_buffer,
            GEGL_RECTANGLE(0, 0, width, height), shell->filter_buffer,
            GEGL_RECTANGLE(0, 0, width, height));
         ...
         */
        gimp_color_transform_process_buffer(
            shell->profile_transform, shell->profile_buffer,
            GEGL_RECTANGLE(0, 0, width, height), shell->filter_buffer,
            GEGL_RECTANGLE(0, 0, width, height));
      } else {
        GeglBuffer *buffer = gegl_buffer_linear_new_from_data(
            cairo_data, babl_format("cairo-ARGB32"),
            GEGL_RECTANGLE(0, 0, width, height), cairo_stride, NULL, NULL);
        ...
        gimp_color_transform_process_buffer(
            shell->profile_transform, shell->profile_buffer,
            GEGL_RECTANGLE(0, 0, width, height), buffer,
            GEGL_RECTANGLE(0, 0, width, height));
     ...
     */
    if (gimp_display_shell_has_filter(shell) || !can_convert_to_u8) {
      gegl_buffer_get(shell->filter_buffer, GEGL_RECTANGLE(0, 0, width, height),
                      1.0, babl_format("cairo-ARGB32"), cairo_data,
                      cairo_stride, GEGL_ABYSS_NONE);
    }
  } else {
...
#ifndef USE_NODE_BLIT
    gegl_buffer_get(buffer, GEGL_RECTANGLE(x, y, width, height), scale,
                    babl_format("cairo-ARGB32"), cairo_data, cairo_stride,
                    abyss_policy | filter);
#else
    gegl_node_blit(node, scale, GEGL_RECTANGLE(x, y, width, height),
                   babl_format("cairo-ARGB32"), cairo_data, cairo_stride,
  ...
  if (shell->mask) {
    if (!shell->mask_surface) {
      shell->mask_surface = cairo_image_surface_create(
          CAIRO_FORMAT_A8, shell->render_buf_width, shell->render_buf_height);
    }
    ...
    gegl_buffer_get(shell->mask,
                    GEGL_RECTANGLE(x - floor(shell->mask_offset_x * scale),
                                   ...
                                   width, height),
                    scale, babl_format("Y u8"), cairo_data, cairo_stride,
