...
enum { SCALED, SCROLLED, ROTATED, RECONNECT, LAST_SIGNAL };
...
static void gimp_display_shell_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void gimp_display_shell_get_property(GObject *object, guint property_id,
...
static const guint8 *
gimp_display_shell_get_icc_profile(GimpColorManaged *managed, gsize *len);
...
static void gimp_display_shell_zoom_button_callback(GimpDisplayShell *shell,
                                                    GtkWidget *zoom_button);
static void gimp_display_shell_sync_config(GimpDisplayShell *shell,
                                           GimpDisplayConfig *config);

static void
gimp_display_shell_overlay_allocate(GtkWidget *child, GtkAllocation *allocation,
...
static void gimp_display_shell_transform_overlay(GimpDisplayShell *shell,
                                                 GtkWidget *child, gdouble *x,
                                                 gdouble *y);

G_DEFINE_TYPE_WITH_CODE(
    GimpDisplayShell, gimp_display_shell, GTK_TYPE_EVENT_BOX,
...
static void gimp_display_shell_class_init(GimpDisplayShellClass *klass) {
  ...
  display_shell_signals[SCALED] =
      g_signal_new("scaled", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpDisplayShellClass, scaled), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);

  display_shell_signals[SCROLLED] =
      g_signal_new("scrolled", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpDisplayShellClass, scrolled), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);

  display_shell_signals[ROTATED] =
      g_signal_new("rotated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpDisplayShellClass, rotated), NULL, NULL,
                   NULL, G_TYPE_NONE, 0);

  display_shell_signals[RECONNECT] =
      g_signal_new("reconnect", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpDisplayShellClass, reconnect), NULL,
  ...
  g_object_class_install_property(
      object_class, PROP_POPUP_MANAGER,
      g_param_spec_object("popup-manager", NULL, NULL, GIMP_TYPE_UI_MANAGER,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("display", NULL, NULL, GIMP_TYPE_DISPLAY,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));

  g_object_class_install_property(object_class, PROP_UNIT,
                                  gimp_param_spec_unit("unit", NULL, NULL, TRUE,
                                                       FALSE, GIMP_UNIT_PIXEL,
                                                       GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_TITLE,
      g_param_spec_string("title", NULL, NULL, GIMP_NAME,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_STATUS,
      g_param_spec_string("status", NULL, NULL, NULL, GIMP_PARAM_READWRITE));
  ...
  g_object_class_install_property(object_class, PROP_INFINITE_CANVAS,
                                  g_param_spec_boolean("infinite-canvas", NULL,
                                                       NULL, FALSE,
                                                       GIMP_PARAM_READABLE));
...
static void gimp_display_shell_init(GimpDisplayShell *shell) {
  ...
  shell->no_image_options =
      g_object_new(GIMP_TYPE_DISPLAY_OPTIONS_NO_IMAGE, NULL);
  ...
  shell->filter_profile =
      gimp_babl_get_builtin_color_profile(GIMP_RGB, GIMP_TRC_NON_LINEAR);
  ...
  g_signal_connect(shell->motion_buffer, "stroke",
                   G_CALLBACK(gimp_display_shell_buffer_stroke), shell);
  g_signal_connect(shell->motion_buffer, "hover",
                   G_CALLBACK(gimp_display_shell_buffer_hover), shell);

  shell->zoom_focus_pointer_queue = g_queue_new();

  gtk_widget_set_events(GTK_WIDGET(shell),
  ...
  g_signal_connect_swapped(shell->zoom, "zoomed",
                           G_CALLBACK(gimp_display_shell_scale_update), shell);

  /*  active display callback  */
  g_signal_connect(shell, "button-press-event",
                   G_CALLBACK(gimp_display_shell_events), shell);
...
static void gimp_display_shell_constructed(GObject *object) {
  ...
  if (config->monitor_res_from_gdk) {
    gimp_get_monitor_resolution(shell->initial_monitor, &shell->monitor_xres,
                                &shell->monitor_yres);
  } else {
  ...
  shell->hsbdata = gtk_adjustment_new(0, 0, image_width, 1, 1, image_width);
  shell->hsb = gtk_scrollbar_new(GTK_ORIENTATION_HORIZONTAL, shell->hsbdata);
  gtk_widget_set_can_focus(shell->hsb, FALSE);

  /*  the vertical scrollbar  */
  shell->vsbdata = gtk_adjustment_new(0, 0, image_height, 1, 1, image_height);
  ...
  gtk_image =
      gtk_image_new_from_icon_name(GIMP_ICON_MENU_RIGHT, GTK_ICON_SIZE_MENU);
  gtk_container_add(GTK_CONTAINER(shell->origin), gtk_image);
  gtk_widget_show(gtk_image);

  g_signal_connect(shell->origin, "button-press-event",
                   G_CALLBACK(gimp_display_shell_origin_button_press), shell);

  gimp_help_set_help_data(shell->origin, _("Access the image menu"),
  ...
  g_signal_connect(shell->canvas, "remove",
                   G_CALLBACK(gimp_display_shell_remove_overlay), shell);
  ...
  g_signal_connect(shell->hrule, "button-press-event",
                   G_CALLBACK(gimp_display_shell_hruler_button_press), shell);
  ...
  g_signal_connect(shell->vrule, "button-press-event",
                   G_CALLBACK(gimp_display_shell_vruler_button_press), shell);
  ...
  g_signal_connect(shell->canvas, "grab-notify",
                   G_CALLBACK(gimp_display_shell_canvas_grab_notify), shell);

  g_signal_connect(shell->canvas, "realize",
                   G_CALLBACK(gimp_display_shell_canvas_realize), shell);
  g_signal_connect(shell->canvas, "size-allocate",
                   G_CALLBACK(gimp_display_shell_canvas_size_allocate), shell);
  g_signal_connect(shell->canvas, "draw",
                   G_CALLBACK(gimp_display_shell_canvas_draw), shell);
  ...
  /*  the zoom button  */
  shell->zoom_button = g_object_new(
      GTK_TYPE_CHECK_BUTTON, "draw-indicator", FALSE, "relief", GTK_RELIEF_NONE,
      "width-request", 18, "height-request", 18, NULL);
  ...
  shell->quick_mask_button = g_object_new(
      GTK_TYPE_CHECK_BUTTON, "draw-indicator", FALSE, "relief", GTK_RELIEF_NONE,
      "width-request", 18, "height-request", 18, NULL);
  ...
  g_signal_connect(shell->quick_mask_button, "toggled",
                   G_CALLBACK(gimp_display_shell_quick_mask_toggled), shell);
  g_signal_connect(shell->quick_mask_button, "button-press-event",
                   G_CALLBACK(gimp_display_shell_quick_mask_button_press),
                   shell);

  action = gimp_ui_manager_find_action(shell->popup_manager, "quick-mask",
                                       "quick-mask-toggle");
  if (action)
    gimp_widget_set_accel_help(shell->quick_mask_button, action);
  else
    gimp_help_set_help_data(shell->quick_mask_button, _("Toggle Quick Mask"),
  ...
  gimp_help_set_help_data(shell->nav_ebox, _("Navigate the image display"),
...
static void gimp_display_shell_get_property(GObject *object, guint property_id,
  ...
  case PROP_INFINITE_CANVAS:
    g_value_set_boolean(value, gimp_display_shell_get_infinite_canvas(shell));
    break;
...
static gboolean gimp_display_shell_popup_menu(GtkWidget *widget) {
  ...
  gimp_ui_manager_ui_popup_at_widget(
      shell->popup_manager, "/dummy-menubar/image-popup", shell->origin,
      GDK_GRAVITY_EAST, GDK_GRAVITY_NORTH_WEST, NULL, NULL, NULL);
...
static void gimp_display_shell_sync_config(GimpDisplayShell *shell,
                                           GimpDisplayConfig *config) {
  gimp_config_sync(G_OBJECT(config->default_view), G_OBJECT(shell->options), 0);
...
static void gimp_display_shell_remove_overlay(GtkWidget *canvas,
  ...
  g_signal_handlers_disconnect_by_func(
      child, gimp_display_shell_overlay_allocate, overlay);
...
static void gimp_display_shell_transform_overlay(GimpDisplayShell *shell,
  ...
  gimp_display_shell_transform_xy_f(shell, overlay->image_x, overlay->image_y,
                                    x, y);
...
GtkWidget *gimp_display_shell_new(GimpDisplay *display, GimpUnit unit,
                                  gdouble scale, GimpUIManager *popup_manager,
                                  GdkMonitor *monitor) {
  g_return_val_if_fail(GIMP_IS_DISPLAY(display), NULL);
  g_return_val_if_fail(GIMP_IS_UI_MANAGER(popup_manager), NULL);
  g_return_val_if_fail(GDK_IS_MONITOR(monitor), NULL);

  return g_object_new(GIMP_TYPE_DISPLAY_SHELL, "popup-manager", popup_manager,
                      "initial-monitor", monitor, "display", display, "unit",
                      unit, NULL);
}

void gimp_display_shell_add_overlay(GimpDisplayShell *shell, GtkWidget *child,
                                    gdouble image_x, gdouble image_y,
                                    GimpHandleAnchor anchor, gint spacing_x,
                                    gint spacing_y) {
  GimpDisplayShellOverlay *overlay;
  gdouble x, y;
  ...
  g_signal_connect(child, "size-allocate",
                   G_CALLBACK(gimp_display_shell_overlay_allocate), overlay);
  ...
  gimp_overlay_box_set_child_position(GIMP_OVERLAY_BOX(shell->canvas), child, x,
                                      y);
}

void gimp_display_shell_move_overlay(GimpDisplayShell *shell, GtkWidget *child,
                                     gdouble image_x, gdouble image_y,
                                     GimpHandleAnchor anchor, gint spacing_x,
                                     gint spacing_y) {
  GimpDisplayShellOverlay *overlay;
...
GimpImageWindow *gimp_display_shell_get_window(GimpDisplayShell *shell) {
  ...
  return GIMP_IMAGE_WINDOW(
      gtk_widget_get_ancestor(GTK_WIDGET(shell), GIMP_TYPE_IMAGE_WINDOW));
}
...
GimpColorConfig *gimp_display_shell_get_color_config(GimpDisplayShell *shell) {
  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), NULL);
...
static gboolean gimp_display_shell_blink(GimpDisplayShell *shell) {
  shell->blink_timeout_id = 0;
...
static gboolean gimp_display_shell_fill_idle(GimpDisplayShell *shell) {
...
void gimp_display_shell_fill(GimpDisplayShell *shell, GimpImage *image,
                             GimpUnit unit, gdouble scale) {
  GimpDisplayConfig *config;
  GimpImageWindow *window;
  ...
  shell->fill_idle_id =
      g_idle_add_full(GIMP_PRIORITY_DISPLAY_SHELL_FILL_IDLE,
                      (GSourceFunc)gimp_display_shell_fill_idle, shell, NULL);
...
void gimp_display_shell_scaled(GimpDisplayShell *shell) {
  ...
  for (list = shell->children; list; list = g_list_next(list)) {
    ...
    gimp_overlay_box_set_child_position(GIMP_OVERLAY_BOX(shell->canvas), child,
                                        x, y);
  }
...
void gimp_display_shell_set_unit(GimpDisplayShell *shell, GimpUnit unit) {
  g_return_if_fail(GIMP_IS_DISPLAY_SHELL(shell));
...
gboolean gimp_display_shell_snap_coords(GimpDisplayShell *shell,
                                        GimpCoords *coords, gint snap_offset_x,
                                        gint snap_offset_y, gint snap_width,
                                        gint snap_height) {
  GimpImage *image;
  ...
  if (snap_to_guides || snap_to_grid || snap_to_canvas || snap_to_vectors) {
    ...
    if (snap_width > 0 && snap_height > 0) {
      snapped = gimp_image_snap_rectangle(
          image, coords->x + snap_offset_x, coords->y + snap_offset_y,
          coords->x + snap_offset_x + snap_width,
          coords->y + snap_offset_y + snap_height, &tx, &ty,
          FUNSCALEX(shell, snap_distance), FUNSCALEY(shell, snap_distance),
          snap_to_guides, snap_to_grid, snap_to_canvas, snap_to_vectors);
    } else {
      snapped = gimp_image_snap_point(
          image, coords->x + snap_offset_x, coords->y + snap_offset_y, &tx, &ty,
          FUNSCALEX(shell, snap_distance), FUNSCALEY(shell, snap_distance),
          snap_to_guides, snap_to_grid, snap_to_canvas, snap_to_vectors);
    }
...
gboolean gimp_display_shell_mask_bounds(GimpDisplayShell *shell, gint *x,
  ...
  if ((layer = gimp_image_get_floating_selection(image))) {
    ...
    if (!gimp_item_bounds(GIMP_ITEM(gimp_image_get_mask(image)), x, y, width,
      ...
      *height = fs_height;
      ...
      gimp_rectangle_union(*x, *y, *width, *height, fs_x, fs_y, fs_width,
                           fs_height, x, y, width, height);
    }
  } else if (!gimp_item_bounds(GIMP_ITEM(gimp_image_get_mask(image)), x, y,
                               width, height)) {
    return FALSE;
  }
  ...
  gimp_display_shell_transform_bounds(shell, x1, y1, x2, y2, &x1_f, &y1_f,
                                      &x2_f, &y2_f);
...
void gimp_display_shell_set_show_image(GimpDisplayShell *shell,
                                       gboolean show_image) {
  g_return_if_fail(GIMP_IS_DISPLAY_SHELL(shell));
...
GimpPickable *gimp_display_shell_get_canvas_pickable(GimpDisplayShell *shell) {
  GimpImage *image;
...
GeglRectangle gimp_display_shell_get_bounding_box(GimpDisplayShell *shell) {
  ...
  if (image) {
    if (!shell->show_all) {
      ...
      bounding_box = gimp_projectable_get_bounding_box(GIMP_PROJECTABLE(image));
    }
  }
...
gboolean gimp_display_shell_get_infinite_canvas(GimpDisplayShell *shell) {
  g_return_val_if_fail(GIMP_IS_DISPLAY_SHELL(shell), FALSE);

  return shell->show_all && !gimp_display_shell_get_padding_in_show_all(shell);
}

void gimp_display_shell_update_priority_rect(GimpDisplayShell *shell) {
  GimpImage *image;
  ...
  if (image) {
    ...
    gimp_display_shell_untransform_viewport(shell, !shell->show_all, &x, &y,
                                            &width, &height);
...
void gimp_display_shell_flush(GimpDisplayShell *shell) {
  ...
  gimp_canvas_layer_boundary_set_layers(
      GIMP_CANVAS_LAYER_BOUNDARY(shell->layer_boundary),
      gimp_image_get_selected_layers(gimp_display_get_image(shell->display)));

  gimp_canvas_canvas_boundary_set_image(
      GIMP_CANVAS_CANVAS_BOUNDARY(shell->canvas_boundary),
...
void gimp_display_shell_pause(GimpDisplayShell *shell) {
  ...
  if (shell->paused_count == 1) {
    ...
    tool_manager_control_active(shell->display->gimp, GIMP_TOOL_ACTION_PAUSE,
                                shell->display);
  }
}
...
void gimp_display_shell_resume(GimpDisplayShell *shell) {
  ...
  if (shell->paused_count == 0) {
    ...
    tool_manager_control_active(shell->display->gimp, GIMP_TOOL_ACTION_RESUME,
                                shell->display);
  }
}
...
void gimp_display_shell_set_highlight(GimpDisplayShell *shell,
  ...
  if (highlight) {
    ...
    gimp_canvas_rectangle_set(shell->passe_partout, highlight->x, highlight->y,
 ...
 * the given color.
 **/
void gimp_display_shell_set_mask(GimpDisplayShell *shell, GeglBuffer *mask,
                                 gint offset_x, gint offset_y,
                                 const GimpRGB *color, gboolean inverted) {
  g_return_if_fail(GIMP_IS_DISPLAY_SHELL(shell));
