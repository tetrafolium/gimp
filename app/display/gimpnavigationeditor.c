...
static void gimp_navigation_editor_zoom(GimpNavigationView *view,
                                        GimpZoomType direction, gdouble delta,
                                        GimpNavigationEditor *editor);
...
static void gimp_navigation_editor_shell_infinite_canvas_notify(
    GimpDisplayShell *shell, const GParamSpec *pspec,
    GimpNavigationEditor *editor);
...
G_DEFINE_TYPE_WITH_CODE(
    GimpNavigationEditor, gimp_navigation_editor, GIMP_TYPE_EDITOR,
...
static void gimp_navigation_editor_init(GimpNavigationEditor *editor) {
  ...
  editor->view = gimp_view_new_by_types(NULL, GIMP_TYPE_NAVIGATION_VIEW,
  ...
  g_signal_connect(editor->view, "marker-changed",
                   G_CALLBACK(gimp_navigation_editor_marker_changed), editor);
  g_signal_connect(editor->view, "zoom",
                   G_CALLBACK(gimp_navigation_editor_zoom), editor);
  g_signal_connect(editor->view, "scroll",
                   G_CALLBACK(gimp_navigation_editor_scroll), editor);
...
static void gimp_navigation_editor_display_changed(
    GimpContext *context, GimpDisplay *display, GimpNavigationEditor *editor) {
...
static void gimp_navigation_editor_image_chaged(GimpContext *context,
                                                GimpImage *image,
                                                GimpNavigationEditor *editor) {
...
static void gimp_navigation_editor_set_context(GimpDocked *docked,
  ...
  if (editor->context) {
    g_signal_handlers_disconnect_by_func(
        editor->context, gimp_navigation_editor_display_changed, editor);
    g_signal_handlers_disconnect_by_func(
        editor->context, gimp_navigation_editor_image_chaged, editor);
  }
  ...
  if (editor->context) {
    ...
    g_signal_connect(context, "image-changed",
                     G_CALLBACK(gimp_navigation_editor_image_chaged), editor);

    display = gimp_context_get_display(context);
  }

  gimp_view_renderer_set_context(GIMP_VIEW(editor->view)->renderer, context);

  gimp_navigation_editor_display_changed(editor->context, display, editor);
}
...
void gimp_navigation_editor_popup(GimpDisplayShell *shell, GtkWidget *widget,
  ...
  if (!shell->nav_popup) {
    ...
    editor =
        GIMP_NAVIGATION_EDITOR(gimp_navigation_editor_new_private(NULL, shell));
    gtk_container_add(GTK_CONTAINER(frame), GTK_WIDGET(editor));
    gtk_widget_show(GTK_WIDGET(editor));

    g_signal_connect(editor->view, "button-release-event",
                     G_CALLBACK(gimp_navigation_editor_button_release), shell);
  } else {
  ...
  gimp_navigation_view_get_local_marker(
      view, &view_marker_center_x, &view_marker_center_y, &view_marker_width,
      &view_marker_height);
  /* Position the popup */
  {
    GdkMonitor *monitor;
    ...
    gdk_window_get_origin(gtk_widget_get_window(widget), &x_origin, &y_origin);
    ...
    x = screen_click_x - border_width - view_marker_center_x;

    y = screen_click_y - border_height - view_marker_center_y;
...
gimp_navigation_editor_new_private(GimpMenuFactory *menu_factory,
  ...
  g_return_val_if_fail(
      menu_factory == NULL || GIMP_IS_MENU_FACTORY(menu_factory), NULL);
  ...
  if (shell) {
    ...
    gimp_view_renderer_set_size(view->renderer, config->nav_preview_size * 3,
                                view->renderer->border_width);
    gimp_view_renderer_set_context(view->renderer, gimp_get_user_context(gimp));
    gimp_view_renderer_set_color_config(
        view->renderer, gimp_display_shell_get_color_config(shell));
    ...
    editor =
        g_object_new(GIMP_TYPE_NAVIGATION_EDITOR, "menu-factory", menu_factory,
                     "menu-identifier", "<NavigationEditor>", NULL);

    gtk_widget_set_size_request(editor->view, GIMP_VIEW_SIZE_HUGE,
                                GIMP_VIEW_SIZE_HUGE);
    gimp_view_set_expand(GIMP_VIEW(editor->view), TRUE);

    /* the editor buttons */

    editor->zoom_out_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor), "view", "view-zoom-out", NULL);

    editor->zoom_in_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor), "view", "view-zoom-in", NULL);

    editor->zoom_100_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor), "view", "view-zoom-1-1", NULL);

    editor->zoom_fit_in_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor), "view", "view-zoom-fit-in", NULL);

    editor->zoom_fill_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor), "view", "view-zoom-fill", NULL);

    editor->shrink_wrap_button = gimp_editor_add_action_button(
        GIMP_EDITOR(editor), "view", "view-shrink-wrap", NULL);
    ...
    editor->zoom_adjustment = gtk_adjustment_new(0.0, -8.0, 8.0, 0.5, 1.0, 0.0);

    g_signal_connect(editor->zoom_adjustment, "value-changed",
                     G_CALLBACK(gimp_navigation_editor_zoom_adj_changed),
                     editor);

    hscale = gtk_scale_new(GTK_ORIENTATION_HORIZONTAL, editor->zoom_adjustment);
...
static void gimp_navigation_editor_set_shell(GimpNavigationEditor *editor,
  ...
  if (editor->shell) {
    g_signal_handlers_disconnect_by_func(
        ...
        editor);
    g_signal_handlers_disconnect_by_func(
        editor->shell, gimp_navigation_editor_shell_scaled, editor);
    g_signal_handlers_disconnect_by_func(
        editor->shell, gimp_navigation_editor_shell_scrolled, editor);
    g_signal_handlers_disconnect_by_func(
        editor->shell, gimp_navigation_editor_shell_rotated, editor);
    g_signal_handlers_disconnect_by_func(
        editor->shell, gimp_navigation_editor_shell_reconnect, editor);

    g_signal_handlers_disconnect_by_func(
        editor->shell->options,
        gimp_navigation_editor_options_show_canvas_notify, editor);
    g_signal_handlers_disconnect_by_func(
        editor->shell->fullscreen_options,
        gimp_navigation_editor_options_show_canvas_notify, editor);
  } else if (shell) {
  ...
  if (editor->shell) {
    ...
    if (image) {
      ...
      g_signal_connect(editor->image_proxy, "size-changed",
    ...
    g_signal_connect(
        editor->shell, "notify::infinite-canvas",
        G_CALLBACK(gimp_navigation_editor_shell_infinite_canvas_notify),
        editor);
    g_signal_connect(editor->shell, "scaled",
                     G_CALLBACK(gimp_navigation_editor_shell_scaled), editor);
    g_signal_connect(editor->shell, "scrolled",
                     G_CALLBACK(gimp_navigation_editor_shell_scrolled), editor);
    g_signal_connect(editor->shell, "rotated",
                     G_CALLBACK(gimp_navigation_editor_shell_rotated), editor);
    g_signal_connect(editor->shell, "reconnect",
                     G_CALLBACK(gimp_navigation_editor_shell_reconnect),
                     editor);

    g_signal_connect(
        editor->shell->options, "notify::show-canvas-boundary",
        G_CALLBACK(gimp_navigation_editor_options_show_canvas_notify), editor);
    g_signal_connect(
        editor->shell->fullscreen_options, "notify::show-canvas-boundary",
        G_CALLBACK(gimp_navigation_editor_options_show_canvas_notify), editor);
...
static void gimp_navigation_editor_marker_changed(
    GimpNavigationView *view, gdouble center_x, gdouble center_y, gdouble width,
    gdouble height, GimpNavigationEditor *editor) {
  ...
  if (editor->shell) {
    if (gimp_display_get_image(editor->shell->display)) {
      ...
      gimp_display_shell_scroll_center_image_xy(editor->shell, center_x,
                                                center_y);
    }
  }
}
...
static void gimp_navigation_editor_zoom(GimpNavigationView *view,
  ...
  if (editor->shell) {
    if (gimp_display_get_image(editor->shell->display))
      gimp_display_shell_scale(editor->shell, direction, delta,
...
static void gimp_navigation_editor_scroll(GimpNavigationView *view,
  ...
  if (editor->shell) {
    ...
    gimp_scroll_adjustment_values(sevent, editor->shell->hsbdata,
                                  editor->shell->vsbdata, &value_x, &value_y);
...
static gboolean gimp_navigation_editor_zoom_adj_changed_timeout(gpointer data) {
...
gimp_navigation_editor_zoom_adj_changed(GtkAdjustment *adj,
  ...
  editor->scale_timeout = g_timeout_add(
      UPDATE_DELAY, gimp_navigation_editor_zoom_adj_changed_timeout, editor);
}
...
static void gimp_navigation_editor_options_show_canvas_notify(
    GimpDisplayOptions *options, const GParamSpec *pspec,
    GimpNavigationEditor *editor) {
  gimp_navigation_editor_update_marker(editor);
}
...
static void gimp_navigation_editor_update_marker(GimpNavigationEditor *editor) {
  ...
  if (renderer->viewable) {
    ...
    image = gimp_image_proxy_get_image(GIMP_IMAGE_PROXY(renderer->viewable));
    ...
    bounding_box =
        gimp_image_proxy_get_bounding_box(GIMP_IMAGE_PROXY(renderer->viewable));

    gimp_display_shell_scroll_get_viewport(shell, &x, &y, &w, &h);
    gimp_display_shell_untransform_xy_f(shell, shell->disp_width / 2,
                                        shell->disp_height / 2, &x, &y);

    x -= bounding_box.x;
    y -= bounding_box.y;

    gimp_navigation_view_set_marker(view, x, y, w, h, shell->flip_horizontally,
    ...
    gimp_navigation_view_set_canvas(
        ...
        -bounding_box.x, -bounding_box.y, gimp_image_get_width(image),
