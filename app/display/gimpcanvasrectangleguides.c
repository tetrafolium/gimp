...
typedef struct _GimpCanvasRectangleGuidesPrivate
...
#define GET_PRIVATE(rectangle)                                                 \
  ((GimpCanvasRectangleGuidesPrivate *)                                        \
       gimp_canvas_rectangle_guides_get_instance_private(                      \
...
static void gimp_canvas_rectangle_guides_transform(GimpCanvasItem *item,
...
static void draw_hline(cairo_t *cr, gdouble x1, gdouble x2, gdouble y) {
  y = floor(y) + 0.5;
...
static void draw_vline(cairo_t *cr, gdouble y1, gdouble y2, gdouble x) {
  x = floor(x) + 0.5;
...
GimpCanvasItem *gimp_canvas_rectangle_guides_new(GimpDisplayShell *shell,
  ...
  return g_object_new(GIMP_TYPE_CANVAS_RECTANGLE_GUIDES, "shell", shell, "x", x,
                      "y", y, "width", width, "height", height, "type", type,
                      "n-guides", n_guides, NULL);
}

void gimp_canvas_rectangle_guides_set(GimpCanvasItem *rectangle, gdouble x,
  ...
  g_object_set(rectangle, "x", x, "y", y, "width", width, "height", height,
               "type", type, "n-guides", n_guides, NULL);
