...
enum { CHANGE_COMPLETE, LAST_SIGNAL };
...
static void gimp_tool_rectangle_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_tool_rectangle_get_property(GObject *object, guint property_id,
                                             GValue *value, GParamSpec *pspec);
static void gimp_tool_rectangle_notify(GObject *object, GParamSpec *pspec);
...
static void gimp_tool_rectangle_button_release(
    GimpToolWidget *widget, const GimpCoords *coords, guint32 time,
    GdkModifierType state, GimpButtonReleaseType release_type);
...
static void gimp_tool_rectangle_synthesize_motion(GimpToolRectangle *rectangle,
                                                  gint function, gdouble new_x,
                                                  gdouble new_y);

static GimpRectangleFunction
gimp_tool_rectangle_calc_function(GimpToolRectangle *rectangle,
                                  const GimpCoords *coords, gboolean proximity);
...
static void gimp_tool_rectangle_get_other_side_coord(
    GimpToolRectangle *rectangle, gdouble *other_side_x, gdouble *other_side_y);
...
static void gimp_tool_rectangle_apply_coord(GimpToolRectangle *rectangle,
                                            gdouble coord_x, gdouble coord_y);
static void gimp_tool_rectangle_setup_snap_offsets(GimpToolRectangle *rectangle,
                                                   const GimpCoords *coords);
...
static void gimp_tool_rectangle_keep_inside_horizontally(
    GimpToolRectangle *rectangle, GimpRectangleConstraint constraint);
static void
...
static void gimp_tool_rectangle_update_with_coord(GimpToolRectangle *rectangle,
                                                  gdouble new_x, gdouble new_y);
static void gimp_tool_rectangle_apply_fixed_rule(GimpToolRectangle *rectangle);

static void
gimp_tool_rectangle_get_constraints(GimpToolRectangle *rectangle, gint *min_x,
                                    gint *min_y, gint *max_x, gint *max_y,
                                    GimpRectangleConstraint constraint);
...
static guint rectangle_signals[LAST_SIGNAL] = {
    0,
};

static void gimp_tool_rectangle_class_init(GimpToolRectangleClass *klass) {
  ...
  rectangle_signals[CHANGE_COMPLETE] = g_signal_new(
      "change-complete", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpToolRectangleClass, change_complete), NULL, NULL,
      NULL, G_TYPE_NONE, 0);
  ...
  g_object_class_install_property(
      object_class, PROP_CONSTRAINT,
      g_param_spec_enum("constraint", NULL, NULL,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("precision", NULL, NULL, GIMP_TYPE_RECTANGLE_PRECISION,
                        GIMP_RECTANGLE_PRECISION_INT,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_NARROW_MODE,
      g_param_spec_boolean("narrow-mode", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_FORCE_NARROW_MODE,
      g_param_spec_boolean("force-narrow-mode", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_DRAW_ELLIPSE,
      g_param_spec_boolean("draw-ellipse", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_ROUND_CORNERS,
      g_param_spec_boolean("round-corners", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_CORNER_RADIUS,
      g_param_spec_double("corner-radius", NULL, NULL, 0.0, 10000.0, 10.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_STATUS_TITLE,
      g_param_spec_string("status-title", NULL, NULL, _("Rectangle: "),
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_HIGHLIGHT,
      g_param_spec_boolean("highlight", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_HIGHLIGHT_OPACITY,
      g_param_spec_double("highlight-opacity", NULL, NULL, 0.0, 1.0, 0.5,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_GUIDE,
      g_param_spec_enum("guide", NULL, NULL, GIMP_TYPE_GUIDES_TYPE,
                        GIMP_GUIDES_NONE,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_X,
      g_param_spec_double("x", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                          GIMP_MAX_IMAGE_SIZE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_Y,
      g_param_spec_double("y", NULL, NULL, -GIMP_MAX_IMAGE_SIZE,
                          GIMP_MAX_IMAGE_SIZE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_WIDTH,
      g_param_spec_double("width", NULL, NULL, 0.0, GIMP_MAX_IMAGE_SIZE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_HEIGHT,
      g_param_spec_double("height", NULL, NULL, 0.0, GIMP_MAX_IMAGE_SIZE, 0.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_FIXED_RULE_ACTIVE,
      g_param_spec_boolean("fixed-rule-active", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_FIXED_RULE,
      g_param_spec_enum("fixed-rule", NULL, NULL,
                        GIMP_TYPE_RECTANGLE_FIXED_RULE,
                        GIMP_RECTANGLE_FIXED_ASPECT,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_DESIRED_FIXED_WIDTH,
      g_param_spec_double("desired-fixed-width", NULL, NULL, 0.0,
                          GIMP_MAX_IMAGE_SIZE, 100.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_DESIRED_FIXED_HEIGHT,
      g_param_spec_double("desired-fixed-height", NULL, NULL, 0.0,
                          GIMP_MAX_IMAGE_SIZE, 100.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_DESIRED_FIXED_SIZE_WIDTH,
      g_param_spec_double("desired-fixed-size-width", NULL, NULL, 0.0,
                          GIMP_MAX_IMAGE_SIZE, 100.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_DESIRED_FIXED_SIZE_HEIGHT,
      g_param_spec_double("desired-fixed-size-height", NULL, NULL, 0.0,
                          GIMP_MAX_IMAGE_SIZE, 100.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_ASPECT_NUMERATOR,
      g_param_spec_double("aspect-numerator", NULL, NULL, 0.0,
                          GIMP_MAX_IMAGE_SIZE, 1.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_ASPECT_DENOMINATOR,
      g_param_spec_double("aspect-denominator", NULL, NULL, 0.0,
                          GIMP_MAX_IMAGE_SIZE, 1.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_FIXED_CENTER,
      g_param_spec_boolean("fixed-center", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_tool_rectangle_init(GimpToolRectangle *rectangle) {
...
private
  ->guides = gimp_tool_widget_add_rectangle_guides(widget, 0, 0, 10, 10,
                                                   GIMP_GUIDES_NONE);

private
  ->rectangle = gimp_tool_widget_add_rectangle(widget, 0, 0, 10, 10, FALSE);

private
  ->ellipse =
      gimp_tool_widget_add_arc(widget, 0, 0, 10, 10, 0.0, 2 * G_PI, FALSE);
...
private
  ->center = gimp_tool_widget_add_handle(
      widget, GIMP_HANDLE_CROSS, 0, 0, GIMP_CANVAS_HANDLE_SIZE_SMALL,
      GIMP_CANVAS_HANDLE_SIZE_SMALL, GIMP_HANDLE_ANCHOR_CENTER);

  gimp_tool_widget_push_group(widget, stroke_group);

private
  ->creating_corners[0] = gimp_tool_widget_add_corner(
      widget, 0, 0, 10, 10, GIMP_HANDLE_ANCHOR_NORTH_WEST, 10, 10, FALSE);

private
  ->creating_corners[1] = gimp_tool_widget_add_corner(
      widget, 0, 0, 10, 10, GIMP_HANDLE_ANCHOR_NORTH_EAST, 10, 10, FALSE);

private
  ->creating_corners[2] = gimp_tool_widget_add_corner(
      widget, 0, 0, 10, 10, GIMP_HANDLE_ANCHOR_SOUTH_WEST, 10, 10, FALSE);

private
  ->creating_corners[3] = gimp_tool_widget_add_corner(
      widget, 0, 0, 10, 10, GIMP_HANDLE_ANCHOR_SOUTH_EAST, 10, 10, FALSE);

  gimp_tool_widget_pop_group(widget);

  for (i = GIMP_TOOL_RECTANGLE_RESIZING_UPPER_LEFT;
       i <= GIMP_TOOL_RECTANGLE_RESIZING_BOTTOM; i++) {
    GimpHandleAnchor anchor;
  ...
  private
    ->handles[i] = gimp_tool_widget_add_corner(widget, 0, 0, 10, 10, anchor, 10,
                                               10, FALSE);

    gimp_tool_widget_pop_group(widget);

  private
    ->highlight_handles[i] = gimp_tool_widget_add_corner(widget, 0, 0, 10, 10,
                                                         anchor, 10, 10, FALSE);
...
static void gimp_tool_rectangle_set_property(GObject *object, guint property_id,
  ...
  private
    ->constraint = g_value_get_enum(value);
    break;
  case PROP_PRECISION:
  private
  ...
  private
    ->narrow_mode = g_value_get_boolean(value);
    break;
  case PROP_FORCE_NARROW_MODE:
  private
    ->force_narrow_mode = g_value_get_boolean(value);
    break;
  case PROP_DRAW_ELLIPSE:
  private
    ->draw_ellipse = g_value_get_boolean(value);
    break;
  case PROP_ROUND_CORNERS:
  private
    ->round_corners = g_value_get_boolean(value);
    break;
  case PROP_CORNER_RADIUS:
  private
    ->corner_radius = g_value_get_double(value);
    break;

  case PROP_STATUS_TITLE:
    g_free(private->status_title);
  private
    ->status_title = g_value_dup_string(value);
    if (!private->status_title)
    private
    ->status_title = g_strdup(_("Rectangle: "));
    break;

  case PROP_HIGHLIGHT:
  private
    ->highlight = g_value_get_boolean(value);
    break;
  case PROP_HIGHLIGHT_OPACITY:
  private
    ->highlight_opacity = g_value_get_double(value);
    break;
  case PROP_GUIDE:
  private
    ->guide = g_value_get_enum(value);
    break;
  ...
  private
    ->fixed_rule_active = g_value_get_boolean(value);
    break;
  case PROP_FIXED_RULE:
  private
    ->fixed_rule = g_value_get_enum(value);
    break;
  case PROP_DESIRED_FIXED_WIDTH:
  private
    ->desired_fixed_width = g_value_get_double(value);
    break;
  case PROP_DESIRED_FIXED_HEIGHT:
  private
    ->desired_fixed_height = g_value_get_double(value);
    break;
  case PROP_DESIRED_FIXED_SIZE_WIDTH:
  private
    ->desired_fixed_size_width = g_value_get_double(value);
    break;
  case PROP_DESIRED_FIXED_SIZE_HEIGHT:
  private
    ->desired_fixed_size_height = g_value_get_double(value);
    break;
  case PROP_ASPECT_NUMERATOR:
  private
    ->aspect_numerator = g_value_get_double(value);
    break;
  case PROP_ASPECT_DENOMINATOR:
  private
    ->aspect_denominator = g_value_get_double(value);
    break;

  case PROP_FIXED_CENTER:
  private
    ->fixed_center = g_value_get_boolean(value);
    break;
...
static void gimp_tool_rectangle_notify(GObject *object, GParamSpec *pspec) {
  ...
  if (!strcmp(pspec->name, "x1") || !strcmp(pspec->name, "y1") ||
      !strcmp(pspec->name, "x2") || !strcmp(pspec->name, "y2")) {
    ...
    gimp_tool_rectangle_synthesize_motion(rectangle, GIMP_TOOL_RECTANGLE_MOVING,
                                          private->x, private->y1);
  } else if (!strcmp(pspec->name, "y") &&
             !PIXEL_FEQUAL(private->y1, private->y)) {
    gimp_tool_rectangle_synthesize_motion(rectangle, GIMP_TOOL_RECTANGLE_MOVING,
                                          private->x1, private->y);
    ...
    if (private->fixed_center) {
      x2 = private->center_x_on_fixed_center + private->width / 2;
    } else {
      x2 = private->x1 + private->width;
    }

    gimp_tool_rectangle_synthesize_motion(
        rectangle, GIMP_TOOL_RECTANGLE_RESIZING_RIGHT, x2, private->y2);
    ...
    if (private->fixed_center) {
      y2 = private->center_y_on_fixed_center + private->height / 2;
    } else {
      y2 = private->y1 + private->height;
    }

    gimp_tool_rectangle_synthesize_motion(
        rectangle, GIMP_TOOL_RECTANGLE_RESIZING_BOTTOM, private->x2, y2);
    ...
    if (private->fixed_rule_active &&
      ...
      gimp_tool_rectangle_synthesize_motion(
          rectangle, GIMP_TOOL_RECTANGLE_RESIZING_LOWER_RIGHT, private->x2,
          private->y2);

      /* For some reason these needs to be set separately... */
      g_object_set(rectangle, "x", x, NULL);
      g_object_set(rectangle, "y", y, NULL);
    }
    ...
    gdouble new_inverse_ratio =
        private->aspect_denominator / private->aspect_numerator;
    gdouble lower_ratio;
    ...
    if (private->fixed_rule_active &&
		    ...
		    tool->active_modifier_state == 0                 &&
#endif
        lower_ratio < new_inverse_ratio && higher_ratio > new_inverse_ratio) {
      gdouble new_x2 = private->x1 + private->y2 - private->y1;
      gdouble new_y2 = private->y1 + private->x2 - private->x1;

      gimp_tool_rectangle_synthesize_motion(
          rectangle, GIMP_TOOL_RECTANGLE_RESIZING_LOWER_RIGHT, new_x2, new_y2);
    }
  }
}
...
static void gimp_tool_rectangle_changed(GimpToolWidget *widget) {
  ...
  gimp_canvas_rectangle_guides_set(private->guides, x1, y1, x2 - x1, y2 - y1,
                                   private->guide, 4);

  gimp_canvas_rectangle_set(private->rectangle, x1, y1, x2 - x1, y2 - y1);

  if (private->draw_ellipse) {
    gimp_canvas_arc_set(private->ellipse, (x1 + x2) / 2.0, (y1 + y2) / 2.0,
                        (x2 - x1) / 2.0, (y2 - y1) / 2.0, 0.0, 2 * G_PI);
  ...
  if (private->round_corners && private->corner_radius > 0.0) {
    ...
    radius = MIN(private->corner_radius, MIN((x2 - x1) / 2.0, (y2 - y1) / 2.0));

    gimp_canvas_arc_set(private->corners[0], x1 + radius, y1 + radius, radius,
                        radius, G_PI / 2.0, G_PI / 2.0);

    gimp_canvas_arc_set(private->corners[1], x2 - radius, y1 + radius, radius,
                        radius, 0.0, G_PI / 2.0);

    gimp_canvas_arc_set(private->corners[2], x1 + radius, y2 - radius, radius,
                        radius, G_PI, G_PI / 2.0);

    gimp_canvas_arc_set(private->corners[3], x2 - radius, y2 - radius, radius,
                        radius, G_PI * 1.5, G_PI / 2.0);
  ...
  case GIMP_TOOL_RECTANGLE_MOVING:
    if (private->rect_adjusting) {
      ...
      gimp_canvas_handle_set_position(private->center, (x1 + x2) / 2.0,
                                      (y1 + y2) / 2.0);
  ...
  case GIMP_TOOL_RECTANGLE_AUTO_SHRINK:
    for (i = 0; i < 4; i++) {
      gimp_canvas_corner_set(private->creating_corners[i], x1, y1, x2 - x1,
                             y2 - y1, handle_width, handle_height,
                             private->narrow_mode);
  ...
  if (handle_width >= 3 && handle_height >= 3 &&
    ...
    gimp_canvas_corner_set(corner, x1, y1, x2 - x1, y2 - y1, handle_width,
                           handle_height, private->narrow_mode);
...
gint gimp_tool_rectangle_button_press(GimpToolWidget *widget,
                                      const GimpCoords *coords, guint32 time,
                                      GdkModifierType state,
                                      GimpButtonPressType press_type) {
  GimpToolRectangle *rectangle = GIMP_TOOL_RECTANGLE(widget);
...
private
  ...
  private
    ->x1 = private->x2 = snapped_x;
  private
    ->y1 = private->y2 = snapped_y;
    ...
    private
    ->narrow_mode = TRUE;
    else private->narrow_mode = FALSE;
     ...
     */
  private
    ->center_x_on_fixed_center = snapped_x;
  private
    ->center_y_on_fixed_center = snapped_y;
     ...
     */
  private
    ->other_side_x = snapped_x;
  private
    ->other_side_y = snapped_y;
  } else {
  ...
  private
    ->center_x_on_fixed_center = (private->x1 + private->x2) / 2;
  private
    ->center_y_on_fixed_center = (private->y1 + private->y2) / 2;

    gimp_tool_rectangle_get_other_side_coord(rectangle, &private->other_side_x,
...
private
  ->rect_adjusting = gimp_tool_rectangle_rect_adjusting_func(rectangle);
...
void gimp_tool_rectangle_button_release(GimpToolWidget *widget,
                                        const GimpCoords *coords, guint32 time,
                                        GdkModifierType state,
...
void gimp_tool_rectangle_motion(GimpToolWidget *widget,
                                const GimpCoords *coords, guint32 time,
                                GdkModifierType state) {
  GimpToolRectangle *rectangle = GIMP_TOOL_RECTANGLE(widget);
  GimpToolRectanglePrivate *private = rectangle->private;
  ...
  if (private->function == GIMP_TOOL_RECTANGLE_CREATING) {
     ...
     */
    if (dx < 0) {
      function = (dy < 0 ? GIMP_TOOL_RECTANGLE_RESIZING_UPPER_LEFT
                         : GIMP_TOOL_RECTANGLE_RESIZING_LOWER_LEFT);
    } else if (dx > 0) {
      function = (dy < 0 ? GIMP_TOOL_RECTANGLE_RESIZING_UPPER_RIGHT
                         : GIMP_TOOL_RECTANGLE_RESIZING_LOWER_RIGHT);
    } else if (dy < 0) {
      function = (dx < 0 ? GIMP_TOOL_RECTANGLE_RESIZING_UPPER_LEFT
                         : GIMP_TOOL_RECTANGLE_RESIZING_UPPER_RIGHT);
    } else if (dy > 0) {
      function = (dx < 0 ? GIMP_TOOL_RECTANGLE_RESIZING_LOWER_LEFT
                         : GIMP_TOOL_RECTANGLE_RESIZING_LOWER_RIGHT);
    }
    ...
    if (private->fixed_rule_active &&
      ...
      gimp_tool_widget_set_snap_offsets(
          widget, -(private->x2 - private->x1) / 2,
          -(private->y2 - private->y1) / 2, private->x2 - private->x1,
          private->y2 - private->y1);

      gimp_tool_rectangle_set_function(rectangle, GIMP_TOOL_RECTANGLE_MOVING);
    }
  }
...
GimpHit gimp_tool_rectangle_hit(GimpToolWidget *widget,
                                const GimpCoords *coords, GdkModifierType state,
                                gboolean proximity) {
  GimpToolRectangle *rectangle = GIMP_TOOL_RECTANGLE(widget);
  GimpToolRectanglePrivate *private = rectangle->private;
  GimpRectangleFunction function;
...
void gimp_tool_rectangle_hover(GimpToolWidget *widget, const GimpCoords *coords,
...
static void gimp_tool_rectangle_motion_modifier(GimpToolWidget *widget,
  ...
  if (key == gimp_get_extend_selection_mask()) {
    ...
    g_object_set(rectangle, "fixed-rule-active", !private->fixed_rule_active,
                 NULL);
    ...
    if (button1_down) {
      if (!private->fixed_rule_active) {
        /* Reset anchor point */
        gimp_tool_rectangle_set_other_side_coord(
            rectangle, private->other_side_x, private->other_side_y);
      }

      gimp_tool_rectangle_update_with_coord(rectangle, private->lastx,
                                            private->lasty);
    }
  }

  if (key == gimp_get_toggle_behavior_mask()) {
    g_object_set(rectangle, "fixed-center", !private->fixed_center, NULL);
    ...
    if (private->fixed_center) {
       ...
       * afterwards.
       */
      gimp_tool_rectangle_set_other_side_coord(rectangle, private->other_side_x,
                                               private->other_side_y);
    }
  }
...
gimp_tool_rectangle_update_handle_sizes(GimpToolRectangle *rectangle) {
  ...
  gimp_tool_rectangle_get_public_rect(rectangle, &pub_x1, &pub_y1, &pub_x2,
                                      &pub_y2);
  {
     ...
     */
    gimp_rectangle_intersect(x1, y1, w1, h1, x2, y2, w2, h2, NULL, NULL,
    ...
    private
    ->narrow_mode = TRUE;
    else private->narrow_mode =
        (visible_rectangle_width < NARROW_MODE_THRESHOLD ||
  ...
  if (private->narrow_mode) {
     ...
     */
  private
    ->corner_handle_w = NARROW_MODE_HANDLE_SIZE;
  private
    ->corner_handle_h = NARROW_MODE_HANDLE_SIZE;

  private
    ->top_and_bottom_handle_w =
        CLAMP(rectangle_width,
              MIN(rectangle_width - 2, NARROW_MODE_HANDLE_SIZE), G_MAXINT);
  private
    ->left_and_right_handle_h =
        CLAMP(rectangle_height,
              MIN(rectangle_height - 2, NARROW_MODE_HANDLE_SIZE), G_MAXINT);
  } else {
    /* Calculate and clamp corner handle size. */

  private
    ->corner_handle_w = visible_rectangle_width / 4;
  private
    ->corner_handle_h = visible_rectangle_height / 4;

  private
    ->corner_handle_w =
        CLAMP(private->corner_handle_w, MIN_HANDLE_SIZE, MAX_HANDLE_SIZE);
  private
    ->corner_handle_h =
        CLAMP(private->corner_handle_h, MIN_HANDLE_SIZE, MAX_HANDLE_SIZE);

    /* Calculate and clamp side handle size. */

  private
    ->top_and_bottom_handle_w = rectangle_width - 3 * private->corner_handle_w;
  private
    ->left_and_right_handle_h = rectangle_height - 3 * private->corner_handle_h;
...
static void gimp_tool_rectangle_update_status(GimpToolRectangle *rectangle) {
  ...
  if (private->function == GIMP_TOOL_RECTANGLE_MOVING) {
    ...
    if (width > 0.0 && height > 0.0) {
      aspect_text =
          g_strdup_printf("  (%.2f:1)", (gdouble)width / (gdouble)height);
    }

    gimp_tool_widget_set_status_coords(GIMP_TOOL_WIDGET(rectangle),
                                       private->status_title, width, " × ",
                                       height, aspect_text);
    g_free(aspect_text);
  }
}
...
static void swap_doubles(gdouble *i, gdouble *j) {
  gdouble tmp;
...
gimp_tool_rectangle_calc_function(GimpToolRectangle *rectangle,
  ...
  } else if (gimp_tool_rectangle_coord_on_handle(
                 rectangle, coords, GIMP_HANDLE_ANCHOR_NORTH_WEST)) {
    return GIMP_TOOL_RECTANGLE_RESIZING_UPPER_LEFT;
  } else if (gimp_tool_rectangle_coord_on_handle(
                 rectangle, coords, GIMP_HANDLE_ANCHOR_SOUTH_EAST)) {
    return GIMP_TOOL_RECTANGLE_RESIZING_LOWER_RIGHT;
  } else if (gimp_tool_rectangle_coord_on_handle(
                 rectangle, coords, GIMP_HANDLE_ANCHOR_NORTH_EAST)) {
    return GIMP_TOOL_RECTANGLE_RESIZING_UPPER_RIGHT;
  } else if (gimp_tool_rectangle_coord_on_handle(
                 rectangle, coords, GIMP_HANDLE_ANCHOR_SOUTH_WEST)) {
    return GIMP_TOOL_RECTANGLE_RESIZING_LOWER_LEFT;
  } else if (gimp_tool_rectangle_coord_on_handle(rectangle, coords,
...
static gboolean gimp_tool_rectangle_coord_outside(GimpToolRectangle *rectangle,
  ...
  return (coord->x < x1_b || coord->x > x2_b || coord->y < y1_b ||
          coord->y > y2_b);
}
...
gimp_tool_rectangle_coord_on_handle(GimpToolRectangle *rectangle,
  ...
  return gimp_canvas_item_on_handle(private->rectangle, coords->x, coords->y,
                                    GIMP_HANDLE_SQUARE, handle_x, handle_y,
                                    handle_width, handle_height, anchor);
}
...
static void gimp_tool_rectangle_apply_coord(GimpToolRectangle *rectangle,
  ...
  private
    ...
    if (private->fixed_center)
    private
    ->x2 = 2 * private->center_x_on_fixed_center - private->x1;
  ...
  private
    ...
    if (private->fixed_center)
    private
    ->x1 = 2 * private->center_x_on_fixed_center - private->x2;
  ...
  private
    ...
    if (private->fixed_center)
    private
    ->y2 = 2 * private->center_y_on_fixed_center - private->y1;
  ...
  private
    ...
    if (private->fixed_center)
    private
    ->y1 = 2 * private->center_y_on_fixed_center - private->y2;
...
static void gimp_tool_rectangle_setup_snap_offsets(GimpToolRectangle *rectangle,
  ...
  gimp_tool_rectangle_get_public_rect(rectangle, &x1, &y1, &x2, &y2);
  gimp_tool_rectangle_adjust_coord(rectangle, coords->x, coords->y, &coord_x,
                                   &coord_y);
  ...
  case GIMP_TOOL_RECTANGLE_RESIZING_UPPER_LEFT:
    gimp_tool_widget_set_snap_offsets(widget, x1 - coord_x, y1 - coord_y, 0, 0);
    break;

  case GIMP_TOOL_RECTANGLE_RESIZING_UPPER_RIGHT:
    gimp_tool_widget_set_snap_offsets(widget, x2 - coord_x, y1 - coord_y, 0, 0);
    break;

  case GIMP_TOOL_RECTANGLE_RESIZING_LOWER_LEFT:
    gimp_tool_widget_set_snap_offsets(widget, x1 - coord_x, y2 - coord_y, 0, 0);
    break;

  case GIMP_TOOL_RECTANGLE_RESIZING_LOWER_RIGHT:
    gimp_tool_widget_set_snap_offsets(widget, x2 - coord_x, y2 - coord_y, 0, 0);
    break;

  case GIMP_TOOL_RECTANGLE_RESIZING_LEFT:
    gimp_tool_widget_set_snap_offsets(widget, x1 - coord_x, 0, 0, 0);
    break;

  case GIMP_TOOL_RECTANGLE_RESIZING_RIGHT:
    gimp_tool_widget_set_snap_offsets(widget, x2 - coord_x, 0, 0, 0);
    break;

  case GIMP_TOOL_RECTANGLE_RESIZING_TOP:
    gimp_tool_widget_set_snap_offsets(widget, 0, y1 - coord_y, 0, 0);
    break;

  case GIMP_TOOL_RECTANGLE_RESIZING_BOTTOM:
    gimp_tool_widget_set_snap_offsets(widget, 0, y2 - coord_y, 0, 0);
    break;

  case GIMP_TOOL_RECTANGLE_MOVING:
    gimp_tool_widget_set_snap_offsets(widget, x1 - coord_x, y1 - coord_y,
                                      x2 - x1, y2 - y1);
    break;
...
static void gimp_tool_rectangle_clamp(GimpToolRectangle *rectangle,
                                      ...
                                      GimpRectangleConstraint constraint,
                                      gboolean symmetrically) {
  gimp_tool_rectangle_clamp_width(rectangle, clamped_sides, constraint,
                                  symmetrically);

  gimp_tool_rectangle_clamp_height(rectangle, clamped_sides, constraint,
                                   symmetrically);
}
...
static void gimp_tool_rectangle_clamp_width(GimpToolRectangle *rectangle,
  ...
  gimp_tool_rectangle_get_constraints(rectangle, &min_x, NULL, &max_x, NULL,
                                      constraint);
  if (private->x1 < min_x) {
...
static void gimp_tool_rectangle_clamp_height(GimpToolRectangle *rectangle,
  ...
  gimp_tool_rectangle_get_constraints(rectangle, NULL, &min_y, NULL, &max_y,
                                      constraint);
  if (private->y1 < min_y) {
 ...
 * @rectangle: A #GimpToolRectangle.
 *
 * If the rectangle is outside of the canvas, move it into it. If the rectangle
 * is larger than the canvas in any direction, make it fill the canvas in that
 * direction.
 */
static void
...
gimp_tool_rectangle_apply_fixed_width(GimpToolRectangle *rectangle,
  ...
  case GIMP_TOOL_RECTANGLE_RESIZING_LEFT:
     ...
     */
  private
    ->x1 = private->center_x_on_fixed_center - width / 2;
  private
    ->x2 = private->x1 + width;
    break;
...

{
  ...
  case GIMP_TOOL_RECTANGLE_RESIZING_TOP:
     ...
     */
  private
    ->y1 = private->center_y_on_fixed_center - height / 2;
  private
    ->y2 = private->y1 + height;
    break;
...
static void gimp_tool_rectangle_apply_aspect(GimpToolRectangle *rectangle,
  ...
  private
    ->x1 = private->x2 - aspect * current_h;
    break;

  case SIDE_TO_RESIZE_RIGHT:
  private
    ->x2 = private->x1 + aspect * current_h;
    break;

  case SIDE_TO_RESIZE_TOP:
  private
    ->y1 = private->y2 - current_w / aspect;
    break;

  case SIDE_TO_RESIZE_BOTTOM:
  private
    ->y2 = private->y1 + current_w / aspect;
    break;
  ...
  private
    ->y1 = private->center_y_on_fixed_center - correct_h / 2;
  private
    ->y2 = private->y1 + correct_h;
  } break;
  ...
  private
    ->x1 = private->center_x_on_fixed_center - correct_w / 2;
  private
    ->x2 = private->x1 + correct_w;
  } break;
  }
}
...
static void gimp_tool_rectangle_apply_fixed_rule(GimpToolRectangle *rectangle) {
  ...
  if (private->fixed_rule_active &&
    ...
    aspect =
        CLAMP(private->aspect_numerator / private->aspect_denominator,
              1.0 / gimp_image_get_height(image), gimp_image_get_width(image));

    if (constraint_to_use == GIMP_RECTANGLE_CONSTRAIN_NONE) {
      gimp_tool_rectangle_apply_aspect(rectangle, aspect, CLAMPED_NONE);
    } else {
      if (private->function != GIMP_TOOL_RECTANGLE_MOVING) {
        ClampedSide clamped_sides = CLAMPED_NONE;

        gimp_tool_rectangle_apply_aspect(rectangle, aspect, clamped_sides);
         ...
         */
        gimp_tool_rectangle_clamp(rectangle, &clamped_sides, constraint_to_use,
        ...
        gimp_tool_rectangle_keep_inside(rectangle, constraint_to_use);
      }
    }
  } else if (private->fixed_rule_active &&
             private->fixed_rule == GIMP_RECTANGLE_FIXED_SIZE) {
    gimp_tool_rectangle_apply_fixed_width(rectangle, constraint_to_use,
                                          private->desired_fixed_size_width);
    gimp_tool_rectangle_apply_fixed_height(rectangle, constraint_to_use,
...
gimp_tool_rectangle_handle_general_clamping(GimpToolRectangle *rectangle) {
  ...
  if (private->function != GIMP_TOOL_RECTANGLE_MOVING) {
    gimp_tool_rectangle_clamp(rectangle, NULL, constraint,
                              private->fixed_center);
  } else {
...
private
  ->x1_int = SIGNED_ROUND(private->x1);
private
  ->y1_int = SIGNED_ROUND(private->y1);

  if (gimp_tool_rectangle_rect_rubber_banding_func(rectangle)) {
  private
    ->width_int = (gint)SIGNED_ROUND(private->x2) - private->x1_int;
  private
    ->height_int = (gint)SIGNED_ROUND(private->y2) - private->y1_int;
  }
}
...
GimpToolWidget *gimp_tool_rectangle_new(GimpDisplayShell *shell) {
  ...
  return g_object_new(GIMP_TYPE_TOOL_RECTANGLE, "shell", shell, NULL);
}
...
private
  ...
  g_object_set(object, width_property, MAX(private->x2 - private->x1, 1.0),
               height_property, MAX(private->y2 - private->y1, 1.0), NULL);
}
 ...
 */
void gimp_tool_rectangle_constraint_size_set(GimpToolRectangle *rectangle,
                                             GObject *object,
                                             const gchar *width_property,
                                             const gchar *height_property) {
  GimpDisplayShell *shell;
  ...
  g_object_set(object, width_property, width, height_property, height, NULL);
}
 ...
 * _button_release.
 */
gboolean gimp_tool_rectangle_rectangle_is_first(GimpToolRectangle *rectangle) {
 ...
 **/
gboolean gimp_tool_rectangle_point_in_rectangle(GimpToolRectangle *rectangle,
                                                gdouble x, gdouble y) {
  gdouble x1, y1, x2, y2;
...
void gimp_tool_rectangle_frame_item(GimpToolRectangle *rectangle,
  ...
  g_object_set(rectangle, "x1", (gdouble)offset_x, "y1", (gdouble)offset_y,
               "x2", (gdouble)(offset_x + width), "y2",
               (gdouble)(offset_y + height), NULL);
...
void gimp_tool_rectangle_auto_shrink(GimpToolRectangle *rectangle,
...
private
  ...
  for (iter = pickables; iter; iter = iter->next) {
    ...
    switch (gimp_pickable_auto_shrink(iter->data, x1, y1, x2 - x1, y2 - y1,
                                      &shrunk_x, &shrunk_y, &shrunk_width,
                                      &shrunk_height)) {
  ...
  if (new_x1 != G_MAXINT && new_y1 != G_MAXINT && new_x2 != G_MININT &&
      new_y2 != G_MININT) {
    GimpRectangleFunction original_function = private->function;

  private
    ->function = GIMP_TOOL_RECTANGLE_AUTO_SHRINK;
