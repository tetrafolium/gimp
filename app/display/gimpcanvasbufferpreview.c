...
#define GET_PRIVATE(transform_preview)                                         \
  ((GimpCanvasBufferPreviewPrivate *)                                          \
       gimp_canvas_buffer_preview_get_instance_private(                        \
...
static void gimp_canvas_buffer_preview_draw(GimpCanvasItem *item, cairo_t *cr);
...
static void gimp_canvas_buffer_preview_draw(GimpCanvasItem *item, cairo_t *cr) {
  ...
  area = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, rectangle.width,
                                    rectangle.height);

  data = cairo_image_surface_get_data(area);
  gegl_buffer_get(buffer,
                  GEGL_RECTANGLE(rectangle.x + shell->offset_x,
                                 rectangle.y + shell->offset_y, rectangle.width,
                                 rectangle.height),
                  shell->scale_x, babl_format("cairo-ARGB32"), data,
                  cairo_image_surface_get_stride(area), GEGL_ABYSS_NONE);

  cairo_surface_flush(area);
  cairo_surface_mark_dirty(area);

  cairo_set_source_surface(cr, area, rectangle.x, rectangle.y);
  cairo_rectangle(cr, rectangle.x, rectangle.y, rectangle.width,
                  rectangle.height);
  cairo_fill(cr);
...
gimp_canvas_buffer_preview_compute_bounds(GimpCanvasItem *item,
  ...
  gimp_canvas_item_transform_xy_f(item, extent.x, extent.y, &x1, &y1);
  gimp_canvas_item_transform_xy_f(item, extent.x + extent.width,
                                  extent.y + extent.height, &x2, &y2);
  ...
  gegl_rectangle_intersect(
      &extent, &extent,
      GEGL_RECTANGLE(0, 0, shell->disp_width, shell->disp_height));
...
GimpCanvasItem *gimp_canvas_buffer_preview_new(GimpDisplayShell *shell,
  ...
  return g_object_new(GIMP_TYPE_CANVAS_BUFFER_PREVIEW, "shell", shell, "buffer",
                      buffer, NULL);
}
