...
gboolean gimp_gegl_mask_combine_rect(GeglBuffer *mask, GimpChannelOps op,
  ...
  gegl_buffer_set_color_from_pixel(mask, &rect, &value, babl_format("Y float"));
...
gboolean gimp_gegl_mask_combine_ellipse(GeglBuffer *mask, GimpChannelOps op,
                                        gint x, gint y, gint w, gint h,
                                        gboolean antialias) {
  return gimp_gegl_mask_combine_ellipse_rect(mask, op, x, y, w, h, w / 2.0,
                                             h / 2.0, antialias);
}
...
gboolean gimp_gegl_mask_combine_ellipse_rect(GeglBuffer *mask,
  ...
  if (antialias) {
    format = gimp_babl_format_change_component_type(format,
  ...
  auto u_to_x_left = [=](gdouble u) { return (left + rx) - u; };

  auto u_to_x_right = [=](gdouble u) { return (right - rx) + u; };
   ...
   */
  auto ellipse_distance = [=](gdouble u, gdouble v) {
    gdouble du;
    gdouble dv;
    gdouble t;
    gdouble a, b, c;
  ...
  auto pixel_value = [=](gint x, gint y) {
  ...
  auto ellipse_range = [=](gdouble y, gdouble *x0, gdouble *x1) {
  ...
  auto fill0 = [=](gpointer dest, gint n) {
    switch (op) {
    case GIMP_CHANNEL_OP_REPLACE:
    case GIMP_CHANNEL_OP_INTERSECT:
      memset(dest, 0, bpp * n);
  ...
  auto fill1 = [=](gpointer dest, gint n) {
    switch (op) {
    case GIMP_CHANNEL_OP_REPLACE:
    case GIMP_CHANNEL_OP_ADD:
  ...
  auto set = [=](gpointer dest, gfloat value) {
  ...
  gegl_parallel_distribute_area(
      &rect, PIXELS_PER_THREAD, [=](const GeglRectangle *area) {
        ...
        iter = gegl_buffer_iterator_new(mask, area, 0, format,
...
gboolean gimp_gegl_mask_combine_buffer(GeglBuffer *mask, GeglBuffer *add_on,
  ...
  if (!gegl_rectangle_intersect(
          &mask_rect,
          GEGL_RECTANGLE(off_x + gegl_buffer_get_x(add_on),
  ...
  if (op == GIMP_CHANNEL_OP_REPLACE &&
    ...
    gegl_buffer_set_format(
        add_on, gimp_babl_format_change_trc(
                    add_on_format, gimp_babl_format_get_trc(mask_format)));

    gimp_gegl_buffer_copy(add_on, &add_on_rect, GEGL_ABYSS_NONE, mask,
                          &mask_rect);
  ...
  gegl_parallel_distribute_area(
      &mask_rect, PIXELS_PER_THREAD, [=](const GeglRectangle *mask_area) {
        ...
        iter = gegl_buffer_iterator_new(mask, mask_area, 0, mask_format,
                                        op == GIMP_CHANNEL_OP_REPLACE
                                            ? GEGL_ACCESS_WRITE
                                            : GEGL_ACCESS_READWRITE,
                                        GEGL_ABYSS_NONE, 2);

        gegl_buffer_iterator_add(iter, add_on, &add_on_area, 0, add_on_format,
        ...
        case GIMP_CHANNEL_OP_REPLACE:
          process(
              [](const gfloat *mask, const gfloat *add_on) { return *add_on; });
          break;

        case GIMP_CHANNEL_OP_ADD:
          process([](const gfloat *mask, const gfloat *add_on) {
            return *mask + *add_on;
          });
          break;
