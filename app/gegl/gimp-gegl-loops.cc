...
#define SHIFTED_AREA(dest, src)                                                \
  const GeglRectangle dest##_area_ = {                                         \
      ...
      src##_area->y + (dest##_rect->y - src##_rect->y), src##_area->width,     \
      src##_area->height};                                                     \
...
void gimp_gegl_buffer_copy(GeglBuffer *src_buffer,
  ...
  if (gegl_buffer_get_format(src_buffer) ==
    ...
    gegl_buffer_copy(src_buffer, src_rect, abyss_policy, dest_buffer,
                     dest_rect);
    ...
    gegl_parallel_distribute_area(
        src_rect, PIXELS_PER_THREAD, [=](const GeglRectangle *src_area) {
          SHIFTED_AREA(dest, src);

          gegl_buffer_copy(src_buffer, src_area, abyss_policy, dest_buffer,
                           dest_area);
        });
  }
}

void gimp_gegl_clear(GeglBuffer *buffer, const GeglRectangle *rect) {
  const Babl *format;
  gint bpp;
  gint n_components;
  gint bpc;
  ...
  gegl_parallel_distribute_area(
      rect, PIXELS_PER_THREAD, [=](const GeglRectangle *area) {
        GeglBufferIterator *iter;

        iter = gegl_buffer_iterator_new(
            buffer, area, 0, format, GEGL_ACCESS_READWRITE, GEGL_ABYSS_NONE, 1);
...
void gimp_gegl_convolve(GeglBuffer *src_buffer, const GeglRectangle *src_rect,
                        GeglBuffer *dest_buffer, const GeglRectangle *dest_rect,
                        const gfloat *kernel, gint kernel_size, gdouble divisor,
                        GimpConvolutionType mode, gboolean alpha_weighting) {
  gfloat *src;
  gint src_rowstride;
  ...
  if (babl_format_is_palette(src_format))
    src_format = gimp_babl_format(GIMP_RGB, GIMP_PRECISION_FLOAT_LINEAR,
                                  babl_format_has_alpha(src_format),
                                  babl_format_get_space(src_format));
  else
    src_format = gimp_babl_format(
        gimp_babl_format_get_base_type(src_format), GIMP_PRECISION_FLOAT_LINEAR,
        babl_format_has_alpha(src_format), babl_format_get_space(src_format));

  dest_format = gegl_buffer_get_format(dest_buffer);

  if (babl_format_is_palette(dest_format))
    dest_format = gimp_babl_format(GIMP_RGB, GIMP_PRECISION_FLOAT_LINEAR,
  ...
  gegl_parallel_distribute_area(
      dest_rect, PIXELS_PER_THREAD, [=](const GeglRectangle *dest_area) {
        ...
        dest_iter =
            gegl_buffer_iterator_new(dest_buffer, dest_area, 0, dest_format,
        ...
        while (gegl_buffer_iterator_next(dest_iter)) {
          ...
          const gint dest_x2 =
              dest_iter->items[0].roi.x + dest_iter->items[0].roi.width;
          const gint dest_y2 =
              dest_iter->items[0].roi.y + dest_iter->items[0].roi.height;
          gint x, y;
              ...
              for (x = dest_x1; x < dest_x2; x++) {
                ...
                for (j = y - margin; j <= y + margin; j++) {
                  for (i = x - margin; i <= x + margin; i++, m++) {
                    ...
                    const gfloat *s =
                        src + yy * src_rowstride + xx * components;
...
static inline gfloat odd_powf(gfloat x, gfloat y) {
  if (x >= 0.0f)
    return powf(x, y);
  else
    return -powf(-x, y);
...
void gimp_gegl_dodgeburn(GeglBuffer *src_buffer, const GeglRectangle *src_rect,
                         GeglBuffer *dest_buffer,
                         const GeglRectangle *dest_rect, gdouble exposure,
                         GimpDodgeBurnType type, GimpTransferMode mode) {
...
static void gimp_gegl_smudge_with_paint_process(
  ...
  while (count--) {
    ...
    gimp_gegl_smudge_with_paint_blend(accum, rate, canvas, 1 - rate, accum,
                                      no_erasing);
    ...
    if (brush_a == 0) /* pure smudge */
    {
      ...
      gimp_gegl_smudge_with_paint_blend(src1, flow, accum, 1 - flow, paint,
                                        no_erasing);
    }
...
void gimp_gegl_smudge_with_paint(GeglBuffer *accum_buffer,
                                 ...
                                 GeglBuffer *paint_buffer, gboolean no_erasing,
                                 gdouble flow, gdouble rate) {
  gfloat brush_color_float[4];
  gfloat brush_a = flow;
  GeglAccessMode paint_buffer_access_mode =
      (brush_color ? GEGL_ACCESS_WRITE : GEGL_ACCESS_READWRITE);
#if COMPILE_SSE2_INTRINISICS
  gboolean sse2 = (gimp_cpu_accel_get_support() & GIMP_CPU_ACCEL_X86_SSE2);
#endif
  ...
  gegl_parallel_distribute_area(
      accum_rect, PIXELS_PER_THREAD, [=](const GeglRectangle *accum_area) {
        GeglBufferIterator *iter;

        SHIFTED_AREA(canvas, accum);

        iter = gegl_buffer_iterator_new(
            accum_buffer, accum_area, 0, babl_format("RGBA float"),
            GEGL_ACCESS_READWRITE, GEGL_ABYSS_NONE, 3);

        gegl_buffer_iterator_add(iter, canvas_buffer, canvas_area, 0,
                                 babl_format("RGBA float"), GEGL_ACCESS_READ,
                                 GEGL_ABYSS_NONE);
...
#if COMPILE_SSE2_INTRINISICS
          if (sse2 && ((guintptr)accum | (guintptr)canvas |
                       (guintptr)(brush_color ? brush_color_float : paint) |
                       (guintptr)paint) %
                              16 ==
                          0) {
            gimp_gegl_smudge_with_paint_process_sse2(
                accum, canvas, paint, count,
                brush_color ? brush_color_float : NULL, brush_a, no_erasing,
                flow, rate);
          } else
#endif
          {
            gimp_gegl_smudge_with_paint_process(
                accum, canvas, paint, count,
                brush_color ? brush_color_float : NULL, brush_a, no_erasing,
                flow, rate);
          }
...
void gimp_gegl_apply_mask(GeglBuffer *mask_buffer,
                          ...
                          GeglBuffer *dest_buffer,
                          const GeglRectangle *dest_rect, gdouble opacity) {
  if (!mask_rect)
  ...
  gegl_parallel_distribute_area(
      mask_rect, PIXELS_PER_THREAD, [=](const GeglRectangle *mask_area) {
        GeglBufferIterator *iter;
...
void gimp_gegl_combine_mask(GeglBuffer *mask_buffer,
  ...
  gegl_parallel_distribute_area(
      mask_rect, PIXELS_PER_THREAD, [=](const GeglRectangle *mask_area) {
        ...
        gegl_buffer_iterator_add(iter, dest_buffer, dest_area, 0,
                                 babl_format("Y float"), GEGL_ACCESS_READWRITE,
                                 GEGL_ABYSS_NONE);
...
void gimp_gegl_combine_mask_weird(GeglBuffer *mask_buffer,
                                  ...
                                  GeglBuffer *dest_buffer,
                                  const GeglRectangle *dest_rect,
                                  gdouble opacity, gboolean stipple) {
  if (!mask_rect)
    mask_rect = gegl_buffer_get_extent(mask_buffer);
...
void gimp_gegl_index_to_mask(GeglBuffer *indexed_buffer,
                             ...
                             const GeglRectangle *mask_rect, gint index) {
  if (!indexed_rect)
  ...
  gegl_parallel_distribute_area(
      indexed_rect, PIXELS_PER_THREAD, [=](const GeglRectangle *indexed_area) {
        GeglBufferIterator *iter;

        SHIFTED_AREA(mask, indexed);

        iter = gegl_buffer_iterator_new(indexed_buffer, indexed_area, 0,
                                        indexed_format, GEGL_ACCESS_READ,
                                        GEGL_ABYSS_NONE, 2);
...
static void gimp_gegl_convert_color_profile_progress(GimpProgress *progress,
                                                     gdouble value) {
  if (gegl_is_main_thread())
    gimp_progress_set_value(progress, value);
}

void gimp_gegl_convert_color_profile(
    GeglBuffer *src_buffer, const GeglRectangle *src_rect,
    GimpColorProfile *src_profile, GeglBuffer *dest_buffer,
    const GeglRectangle *dest_rect, GimpColorProfile *dest_profile,
    GimpColorRenderingIntent intent, gboolean bpc, GimpProgress *progress) {
  ...
  if (transform) {
    if (progress) {
      g_signal_connect_swapped(
          transform, "progress",
          G_CALLBACK(gimp_gegl_convert_color_profile_progress), progress);
    }
    ...
    gegl_parallel_distribute_area(
        src_rect, PIXELS_PER_THREAD, [=](const GeglRectangle *src_area) {
          ...
          gimp_color_transform_process_buffer(transform, src_buffer, src_area,
    ...
    gimp_gegl_buffer_copy(src_buffer, src_rect, GEGL_ABYSS_NONE, dest_buffer,
                          dest_rect);
...
void gimp_gegl_average_color(GeglBuffer *buffer, const GeglRectangle *rect,
                             gboolean clip_to_buffer,
                             GeglAbyssPolicy abyss_policy, const Babl *format,
                             gpointer color) {
  typedef struct {
    gfloat color[4];
    gint n;
  ...
  average_format =
      babl_format_with_space("RaGaBaA float", babl_format_get_space(format));
