...
void gimp_gegl_init(Gimp *gimp) {
  ...
  g_object_set(gegl_config(), "swap-compression", config->swap_compression,
               "tile-cache-size", (guint64)config->tile_cache_size, "threads",
               config->num_processors, "use-opencl", config->use_opencl, NULL);

  gimp_parallel_init(gimp);

  g_signal_connect(config, "notify::temp-path",
                   G_CALLBACK(gimp_gegl_notify_temp_path), NULL);
  g_signal_connect(config, "notify::swap-path",
                   G_CALLBACK(gimp_gegl_notify_swap_path), NULL);
  g_signal_connect(config, "notify::swap-compression",
                   G_CALLBACK(gimp_gegl_notify_swap_compression), NULL);
  g_signal_connect(config, "notify::num-processors",
                   G_CALLBACK(gimp_gegl_notify_num_processors), NULL);
  g_signal_connect(config, "notify::tile-cache-size",
                   G_CALLBACK(gimp_gegl_notify_tile_cache_size), NULL);
  g_signal_connect(config, "notify::num-processors",
                   G_CALLBACK(gimp_gegl_notify_num_processors), NULL);
  g_signal_connect(config, "notify::use-opencl",
                   G_CALLBACK(gimp_gegl_notify_use_opencl), NULL);
...
static void gimp_gegl_notify_swap_path(GimpGeglConfig *config) {
  ...
  g_object_set(gegl_config(), "swap", path, NULL);
...
static void gimp_gegl_notify_tile_cache_size(GimpGeglConfig *config) {
  g_object_set(gegl_config(), "tile-cache-size",
               (guint64)config->tile_cache_size, NULL);
}

static void gimp_gegl_notify_num_processors(GimpGeglConfig *config) {
  g_object_set(gegl_config(), "threads", config->num_processors, NULL);
}

static void gimp_gegl_notify_use_opencl(GimpGeglConfig *config) {
  g_object_set(gegl_config(), "use-opencl", config->use_opencl, NULL);
}
