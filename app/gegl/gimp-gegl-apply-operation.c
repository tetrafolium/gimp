...
void gimp_gegl_apply_operation(GeglBuffer *src_buffer, GimpProgress *progress,
                               ...
                               gboolean crop_input) {
  gimp_gegl_apply_cached_operation(src_buffer, progress, undo_desc, operation,
                                   src_buffer != NULL, dest_buffer, dest_rect,
                                   crop_input, NULL, NULL, 0, FALSE);
}

static void gimp_gegl_apply_operation_cancel(GimpProgress *progress,
                                             gboolean *cancel) {
  *cancel = TRUE;
}
...
gboolean gimp_gegl_apply_cached_operation(
    ...
    if (gimp_progress_is_active(progress)) {
      ...
      if (cancelable)
        g_signal_connect(progress, "cancel",
                         G_CALLBACK(gimp_gegl_apply_operation_cancel), &cancel);
  ...
  if (result_buffer == src_buffer &&
    ...
    if (cache) {
      ...
      result_buffer =
          gegl_buffer_new(dest_rect, gegl_buffer_get_format(dest_buffer));
    }
  }
  ...
  if (n_valid_rects > 0) {
    ...
    for (i = 0; i < n_valid_rects; i++) {
      ...
      if (!gegl_rectangle_intersect(&valid_rect, &valid_rects[i], dest_rect)) {
        continue;
      }
      ...
      cairo_region_subtract_rectangle(region,
                                      (cairo_rectangle_int_t *)&valid_rect);

      done_pixels += (gint64)valid_rect.width * (gint64)valid_rect.height;

      if (progress) {
        gimp_progress_set_value(progress,
                                (gdouble)done_pixels / (gdouble)all_pixels);
      }
    }
  }
  ...
  if (connect_src_buffer || crop_input) {
    ...
    if (connect_src_buffer) {
      src_node = gegl_node_new_child(gegl, "operation", "gegl:buffer-source",
    ...
    if (crop_input) {
      ...
      crop_node = gegl_node_new_child(
          gegl, "operation", "gegl:crop", "x", (gdouble)dest_rect->x, "y",
          (gdouble)dest_rect->y, "width", (gdouble)dest_rect->width, "height",
          (gdouble)dest_rect->height, NULL);

      gegl_node_connect_to(src_node, "output", crop_node, "input");

      src_node = crop_node;
    }

    if (!gegl_node_has_pad(operation, "input")) {
      effect = gegl_node_new_child(gegl, "operation", "gimp:normal", NULL);

      gegl_node_connect_to(operation, "output", effect, "aux");
    }

    gegl_node_connect_to(src_node, "output", effect, "input");
  }

  dest_node = gegl_node_new_child(gegl, "operation", "gegl:write-buffer",
                                  "buffer", result_buffer, NULL);

  gegl_node_connect_to(effect, "output", dest_node, "input");
  ...
  if (progress &&
     ...
     */
    gimp_chunk_iterator_set_interval(iter,
  ...
  while (gimp_chunk_iterator_next(iter)) {
    ...
    while (gimp_chunk_iterator_get_rect(iter, &render_rect)) {
      ...
      done_pixels += (gint64)render_rect.width * (gint64)render_rect.height;
    }
  ...
  if (operation_src_node) {
    gegl_node_connect_to(operation_src_node, "output", operation, "input");
  }
  ...
  if (progress_started) {
    ...
    if (cancelable)
      g_signal_handlers_disconnect_by_func(
          progress, gimp_gegl_apply_operation_cancel, &cancel);
  }
...
void gimp_gegl_apply_dither(GeglBuffer *src_buffer, GimpProgress *progress,
  ...
  node = gegl_node_new_child(NULL, "operation", "gegl:dither", "red-levels",
                             levels, "green-levels", levels, "blue-levels",
                             levels, "alpha-bits", levels, "dither-method",
                             dither_type, NULL);

  gimp_gegl_apply_operation(src_buffer, progress, undo_desc, node, dest_buffer,
                            NULL, FALSE);
  g_object_unref(node);
}
...
void gimp_gegl_apply_feather(GeglBuffer *src_buffer, GimpProgress *progress,
   ...
   */
  gimp_gegl_apply_gaussian_blur(src_buffer, progress, undo_desc, dest_buffer,
                                dest_rect, radius_x / 3.5, radius_y / 3.5,
                                abyss_policy);
}
...
void gimp_gegl_apply_border(GeglBuffer *src_buffer, GimpProgress *progress,
  ...
  case GIMP_CHANNEL_BORDER_STYLE_FEATHERED: {
    ...
    node = gegl_node_new_child(NULL, "operation", "gimp:border", "radius-x",
                               radius_x, "radius-y", radius_y, "feather",
                               feather, "edge-lock", edge_lock, NULL);
  } break;
  ...
  case GIMP_CHANNEL_BORDER_STYLE_SMOOTH: {
    ...
    if (radius_x == 1 && radius_y == 1) {
      grow = gegl_node_new_child(node, "operation", "gegl:nop", NULL);
      shrink =
          gegl_node_new_child(node, "operation", "gimp:shrink", "radius-x", 1,
                              "radius-y", 1, "edge-lock", edge_lock, NULL);
    } else {
      grow = gegl_node_new_child(node, "operation", "gimp:grow", "radius-x",
                                 radius_x, "radius-y", radius_y, NULL);
      shrink = gegl_node_new_child(node, "operation", "gimp:shrink", "radius-x",
                                   radius_x + 1, "radius-y", radius_y + 1,
                                   "edge-lock", edge_lock, NULL);
    }

    subtract = gegl_node_new_child(node, "operation", "gegl:subtract", NULL);
...
void gimp_gegl_apply_grow(GeglBuffer *src_buffer, GimpProgress *progress,
  ...
  node = gegl_node_new_child(NULL, "operation", "gimp:grow", "radius-x",
                             radius_x, "radius-y", radius_y, NULL);
...
void gimp_gegl_apply_shrink(GeglBuffer *src_buffer, GimpProgress *progress,
  ...
  node = gegl_node_new_child(NULL, "operation", "gimp:shrink", "radius-x",
                             radius_x, "radius-y", radius_y, "edge-lock",
                             edge_lock, NULL);
...
void gimp_gegl_apply_flood(GeglBuffer *src_buffer, GimpProgress *progress,
  ...
  node = gegl_node_new_child(NULL, "operation", "gimp:flood", NULL);
...
void gimp_gegl_apply_gaussian_blur(
  ...
  node = gegl_node_new_child(NULL, "operation", "gegl:gaussian-blur",
                             "std-dev-x", std_dev_x, "std-dev-y", std_dev_y,
                             "abyss-policy", abyss_policy, NULL);
...
void gimp_gegl_apply_invert_gamma(GeglBuffer *src_buffer,
  ...
  node = gegl_node_new_child(NULL, "operation", "gegl:invert-gamma", NULL);
...
void gimp_gegl_apply_invert_linear(GeglBuffer *src_buffer,
  ...
  node = gegl_node_new_child(NULL, "operation", "gegl:invert-linear", NULL);
...
void gimp_gegl_apply_opacity(GeglBuffer *src_buffer, GimpProgress *progress,
  ...
  node = gimp_gegl_create_apply_opacity_node(mask, mask_offset_x, mask_offset_y,
                                             opacity);
...
void gimp_gegl_apply_scale(GeglBuffer *src_buffer, GimpProgress *progress,
  ...
  node = gegl_node_new_child(NULL, "operation", "gegl:scale-ratio", "origin-x",
                             0.0, "origin-y", 0.0, "sampler",
                             interpolation_type, "abyss-policy",
                             GEGL_ABYSS_CLAMP, "x", x, "y", y, NULL);
...
void gimp_gegl_apply_set_alpha(GeglBuffer *src_buffer, GimpProgress *progress,
  ...
  node = gegl_node_new_child(NULL, "operation", "gimp:set-alpha", "value",
                             value, NULL);
...
void gimp_gegl_apply_threshold(GeglBuffer *src_buffer, GimpProgress *progress,
  ...
  node = gegl_node_new_child(NULL, "operation", "gegl:threshold", "value",
                             value, NULL);
...
void gimp_gegl_apply_transform(GeglBuffer *src_buffer, GimpProgress *progress,
  ...
  node = gegl_node_new_child(NULL, "operation", "gegl:transform", "near-z",
                             GIMP_TRANSFORM_NEAR_Z, "sampler",
