 ...
 */
static inline void
gimp_gegl_smudge_with_paint_blend_sse2(const gfloat *src1, gfloat src1_rate,
  ...
  *v_dest =
      (v_src1 * _mm_set1_ps(src1_alpha) + v_src2 * _mm_set1_ps(src2_alpha)) /
...
void gimp_gegl_smudge_with_paint_process_sse2(
  ...
  while (count--) {
    ...
    gimp_gegl_smudge_with_paint_blend_sse2(accum, rate, canvas, 1 - rate, accum,
                                           no_erasing);
    ...
    if (brush_a == 0) /* pure smudge */
    {
      ...
      gimp_gegl_smudge_with_paint_blend_sse2(src1, flow, accum, 1 - flow, paint,
                                             no_erasing);
    }
