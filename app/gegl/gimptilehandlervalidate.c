...
static void gimp_tile_handler_validate_real_begin_validate(
    GimpTileHandlerValidate *validate);
static void
gimp_tile_handler_validate_real_end_validate(GimpTileHandlerValidate *validate);
static void gimp_tile_handler_validate_real_validate(
    GimpTileHandlerValidate *validate, const GeglRectangle *rect,
    const Babl *format, gpointer dest_buf, gint dest_stride);
static void gimp_tile_handler_validate_real_validate_buffer(
    GimpTileHandlerValidate *validate, const GeglRectangle *rect,
    GeglBuffer *buffer);
...
gimp_tile_handler_validate_class_init(GimpTileHandlerValidateClass *klass) {
  ...
  gimp_tile_handler_validate_signals[INVALIDATED] = g_signal_new(
      "invalidated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(GimpTileHandlerValidateClass, invalidated), NULL, NULL,
      g_cclosure_marshal_VOID__BOXED, G_TYPE_NONE, 1, GEGL_TYPE_RECTANGLE);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_pointer("format", NULL, NULL, GIMP_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_TILE_WIDTH,
      g_param_spec_int("tile-width", NULL, NULL, 1, G_MAXINT, 1,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_TILE_HEIGHT,
      g_param_spec_int("tile-height", NULL, NULL, 1, G_MAXINT, 1,
                       GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_WHOLE_TILE,
      g_param_spec_boolean("whole-tile", NULL, NULL, FALSE,
                           GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_tile_handler_validate_init(GimpTileHandlerValidate *validate) {
...
static void gimp_tile_handler_validate_real_end_validate(
    GimpTileHandlerValidate *validate) {
  validate->suspend_validate--;
}
...
static void gimp_tile_handler_validate_real_validate(
  ...
  gegl_node_blit(validate->graph, 1.0, rect, format, dest_buf, dest_stride,
                 GEGL_BLIT_DEFAULT);
}
...
static void gimp_tile_handler_validate_real_validate_buffer(
  ...
  if (klass->validate == gimp_tile_handler_validate_real_validate) {
    gegl_node_blit_buffer(validate->graph, buffer, rect, 0, GEGL_ABYSS_NONE);
  } else {
...
static GeglTile *
gimp_tile_handler_validate_validate_tile(GeglTileSource *source, gint x,
                                         gint y) {
  ...
  if (validate->suspend_validate ||
    ...
    return gegl_tile_handler_source_command(source, GEGL_TILE_GET, x, y, 0,
                                            NULL);
  }
  ...
  overlap = cairo_region_contains_rectangle(validate->dirty_region, &tile_rect);
  ...
  if (overlap == CAIRO_REGION_OVERLAP_IN || validate->whole_tile) {
    ...
    tile = gegl_tile_handler_get_source_tile(GEGL_TILE_HANDLER(source), x, y, 0,
                                             FALSE);
    ...
    GIMP_TILE_HANDLER_VALIDATE_GET_CLASS(validate)->validate(
        validate,
        GEGL_RECTANGLE(tile_rect.x, tile_rect.y, tile_rect.width,
                       tile_rect.height),
        validate->format, gegl_tile_get_data(tile), tile_stride);
    ...
    tile =
        gegl_tile_handler_source_command(source, GEGL_TILE_GET, x, y, 0, NULL);

    if (!tile) {
      tile = gegl_tile_handler_create_tile(GEGL_TILE_HANDLER(source), x, y, 0);

      memset(gegl_tile_get_data(tile), 0, tile_stride * validate->tile_height);
    }
    ...
    for (i = 0; i < n_rects; i++) {
      ...
      if (tile_x < 0)
        tile_x += validate->tile_width;

      tile_y = blit_rect.y % validate->tile_height;
      if (tile_y < 0)
        tile_y += validate->tile_height;

      GIMP_TILE_HANDLER_VALIDATE_GET_CLASS(validate)->validate(
          validate,
          GEGL_RECTANGLE(blit_rect.x, blit_rect.y, blit_rect.width,
                         blit_rect.height),
          validate->format,
          gegl_tile_get_data(tile) + tile_y * tile_stride + tile_x * tile_bpp,
          tile_stride);
    }
...
void gimp_tile_handler_validate_assign(GimpTileHandlerValidate *validate,
  ...
  g_object_get(buffer, "format", &validate->format, "tile-width",
               &validate->tile_width, "tile-height", &validate->tile_height,
               NULL);

  g_object_set_data(G_OBJECT(buffer), "gimp-tile-handler-validate", validate);
}
...
void gimp_tile_handler_validate_unassign(GimpTileHandlerValidate *validate,
  ...
  g_object_set_data(G_OBJECT(buffer), "gimp-tile-handler-validate", NULL);
...
gimp_tile_handler_validate_get_assigned(GeglBuffer *buffer) {
  ...
  return g_object_get_data(G_OBJECT(buffer), "gimp-tile-handler-validate");
}
...
void gimp_tile_handler_validate_invalidate(GimpTileHandlerValidate *validate,
  ...
  g_signal_emit(validate, gimp_tile_handler_validate_signals[INVALIDATED], 0,
                rect, NULL);
}

void gimp_tile_handler_validate_undo_invalidate(
    GimpTileHandlerValidate *validate, const GeglRectangle *rect) {
  g_return_if_fail(GIMP_IS_TILE_HANDLER_VALIDATE(validate));
  g_return_if_fail(rect != NULL);
...
void gimp_tile_handler_validate_begin_validate(
    GimpTileHandlerValidate *validate) {
  g_return_if_fail(GIMP_IS_TILE_HANDLER_VALIDATE(validate));
...
void gimp_tile_handler_validate_end_validate(
    GimpTileHandlerValidate *validate) {
  g_return_if_fail(GIMP_IS_TILE_HANDLER_VALIDATE(validate));
...
void gimp_tile_handler_validate_validate(GimpTileHandlerValidate *validate,
                                         GeglBuffer *buffer,
                                         const GeglRectangle *rect,
                                         gboolean intersect, gboolean chunked) {
  GimpTileHandlerValidateClass *klass;
  ...
  if (intersect) {
    ...
    region = cairo_region_create_rectangle((const cairo_rectangle_int_t *)rect);
  }
    ...
    if (!cairo_region_is_empty(region)) {
      ...
      if (chunked) {
        ...
        for (i = 0; i < n_rects; i++) {
          ...
          klass->validate_buffer(validate, (const GeglRectangle *)&blit_rect,
                                 buffer);
        }
      }
...
void gimp_tile_handler_validate_buffer_copy(GeglBuffer *src_buffer,
  ...
  gegl_rectangle_intersect(&real_dst_rect, dst_rect,
                           gegl_buffer_get_extent(dst_buffer));
  ...
  if (src_validate) {
    if (real_src_rect.x == real_dst_rect.x &&
      ...
      cairo_region_translate(region, real_dst_rect.x - real_src_rect.x,
