...
static void gimp_applicator_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_applicator_get_property(GObject *object, guint property_id,
...
static void gimp_applicator_class_init(GimpApplicatorClass *klass) {
...
static void gimp_applicator_init(GimpApplicator *applicator) {
...
GimpApplicator *gimp_applicator_new(GeglNode *parent) {
  ...
  applicator->input_node = gegl_node_get_input_proxy(applicator->node, "input");

  applicator->aux_node = gegl_node_get_input_proxy(applicator->node, "aux");

  applicator->output_node =
      gegl_node_get_output_proxy(applicator->node, "output");

  applicator->mode_node =
      gegl_node_new_child(applicator->node, "operation", "gimp:normal", NULL);

  gimp_gegl_mode_node_set_mode(
      applicator->mode_node, applicator->paint_mode, applicator->blend_space,
      applicator->composite_space, applicator->composite_mode);
  gimp_gegl_mode_node_set_opacity(applicator->mode_node, applicator->opacity);

  gegl_node_connect_to(applicator->input_node, "output", applicator->mode_node,
                       "input");

  applicator->apply_offset_node = gegl_node_new_child(
      applicator->node, "operation", "gegl:translate", NULL);

  gegl_node_link_many(applicator->aux_node, applicator->apply_offset_node,
                      NULL);

  gegl_node_connect_to(applicator->apply_offset_node, "output",
                       applicator->mode_node, "aux");

  applicator->mask_node = gegl_node_new_child(applicator->node, "operation",
                                              "gegl:buffer-source", NULL);

  applicator->mask_offset_node = gegl_node_new_child(
      applicator->node, "operation", "gegl:translate", NULL);
  ...
  applicator->affect_node =
      gegl_node_new_child(applicator->node, "operation", "gimp:mask-components",
                          "mask", applicator->affect, NULL);

  applicator->convert_format_node =
      gegl_node_new_child(applicator->node, "operation", "gegl:nop", NULL);
  ...
  gegl_node_link_many(applicator->input_node, applicator->affect_node,
                      applicator->convert_format_node, applicator->cache_node,
                      applicator->crop_node, applicator->output_node, NULL);

  gegl_node_connect_to(applicator->mode_node, "output", applicator->affect_node,
                       "aux");

  return applicator;
}

void gimp_applicator_set_active(GimpApplicator *applicator, gboolean active) {
...
void gimp_applicator_set_src_buffer(GimpApplicator *applicator,
  ...
  if (src_buffer) {
    if (!applicator->src_node) {
      applicator->src_node =
          gegl_node_new_child(applicator->node, "operation",
                              "gegl:buffer-source", "buffer", src_buffer, NULL);
    } else {
      gegl_node_set(applicator->src_node, "buffer", src_buffer, NULL);
    }
    ...
    gegl_node_set(applicator->src_node, "buffer", NULL, NULL);
  }
  ...
  if (dest_buffer) {
    if (!applicator->dest_node) {
      applicator->dest_node =
          gegl_node_new_child(applicator->node, "operation",
                              "gegl:write-buffer", "buffer", dest_buffer, NULL);
    } else {
      gegl_node_set(applicator->dest_node, "buffer", dest_buffer, NULL);
    }
    ...
    gegl_node_set(applicator->dest_node, "buffer", NULL, NULL);
  }
...
void gimp_applicator_set_mask_buffer(GimpApplicator *applicator,
  ...
  gegl_node_set(applicator->mask_node, "buffer", mask_buffer, NULL);
...
void gimp_applicator_set_mask_offset(GimpApplicator *applicator,
                                     gint mask_offset_x, gint mask_offset_y) {
  g_return_if_fail(GIMP_IS_APPLICATOR(applicator));
  ...
  if (applicator->mask_offset_x != mask_offset_x ||
    ...
    gegl_node_set(applicator->mask_offset_node, "x", (gdouble)mask_offset_x,
                  "y", (gdouble)mask_offset_y, NULL);
  }
}
...
void gimp_applicator_set_apply_buffer(GimpApplicator *applicator,
  ...
  if (apply_buffer) {
    if (!applicator->apply_src_node) {
      applicator->apply_src_node = gegl_node_new_child(
          applicator->node, "operation", "gegl:buffer-source", "buffer",
          apply_buffer, NULL);
    } else {
      gegl_node_set(applicator->apply_src_node, "buffer", apply_buffer, NULL);
    }
...
void gimp_applicator_set_apply_offset(GimpApplicator *applicator,
  ...
  if (applicator->apply_offset_x != apply_offset_x ||
    ...
    gegl_node_set(applicator->apply_offset_node, "x", (gdouble)apply_offset_x,
                  "y", (gdouble)apply_offset_y, NULL);
  }
}

void gimp_applicator_set_opacity(GimpApplicator *applicator, gdouble opacity) {
  g_return_if_fail(GIMP_IS_APPLICATOR(applicator));

  if (applicator->opacity != opacity) {
    applicator->opacity = opacity;

    gimp_gegl_mode_node_set_opacity(applicator->mode_node, opacity);
  }
}
...
void gimp_applicator_set_mode(GimpApplicator *applicator,
  ...
  if (applicator->paint_mode != paint_mode ||
    ...
    gimp_gegl_mode_node_set_mode(applicator->mode_node, paint_mode, blend_space,
...
void gimp_applicator_set_affect(GimpApplicator *applicator,
  ...
  if (applicator->affect != affect) {
    ...
    gegl_node_set(applicator->affect_node, "mask", affect, NULL);
  }
}
  ...
  if (applicator->output_format != format) {
    if (format) {
      if (!applicator->output_format) {
        gegl_node_set(applicator->convert_format_node, "operation",
                      "gegl:convert-format", "format", format, NULL);
      } else {
        gegl_node_set(applicator->convert_format_node, "format", format, NULL);
      }
    } else {
      gegl_node_set(applicator->convert_format_node, "operation", "gegl:nop",
                    NULL);
    }
...
void gimp_applicator_set_cache(GimpApplicator *applicator, gboolean enable) {
  g_return_if_fail(GIMP_IS_APPLICATOR(applicator));

  if (applicator->cache_enabled != enable) {
    if (enable) {
      gegl_node_set(applicator->cache_node, "operation", "gegl:cache", NULL);
    } else {
      gegl_node_set(applicator->cache_node, "operation", "gegl:nop", NULL);
    }
...
GeglBuffer *gimp_applicator_get_cache_buffer(GimpApplicator *applicator,
  ...
  if (applicator->cache_enabled) {
    ...
    gegl_node_get(applicator->cache_node, "cache", &cache, NULL);
  ...
  if (applicator->crop_enabled != (rect != NULL) ||
    ...
    if (rect) {
      if (!applicator->crop_enabled) {
        gegl_node_set(applicator->crop_node, "operation", "gimp:compose-crop",
        ...
        gegl_node_set(applicator->crop_node, "x", rect->x, "y", rect->y,
      ...
      gegl_node_set(applicator->crop_node, "operation", "gegl:nop", NULL);
...
void gimp_applicator_blit(GimpApplicator *applicator,
  ...
  gegl_node_blit(applicator->dest_node, 1.0, rect, NULL, NULL, 0,
                 GEGL_BLIT_DEFAULT);
}
