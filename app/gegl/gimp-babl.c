...
void gimp_babl_init(void) {
  static const gchar *babl_types[] = {"u8",   "u16",   "u32",
                                      "half", "float", "double"};
  ...
  for (i = 0; i < G_N_ELEMENTS(babl_types); i++) {
    ...
    babl_format_new("name", name, babl_model("RGBA"), babl_type(babl_types[i]),
                    babl_component("R"), NULL);
    g_snprintf(name, sizeof(name), "R' %s", babl_types[i]);
    babl_format_new("name", name, babl_model("R'G'B'A"),
                    babl_type(babl_types[i]), babl_component("R'"), NULL);
    g_snprintf(name, sizeof(name), "R~ %s", babl_types[i]);
    babl_format_new("name", name, babl_model("R~G~B~A"),
                    babl_type(babl_types[i]), babl_component("R~"), NULL);
    ...
    g_snprintf(name, sizeof(name), "G' %s", babl_types[i]);
    babl_format_new("name", name, babl_model("R'G'B'A"),
                    babl_type(babl_types[i]), babl_component("G'"), NULL);
    g_snprintf(name, sizeof(name), "G~ %s", babl_types[i]);
    babl_format_new("name", name, babl_model("R~G~B~A"),
                    babl_type(babl_types[i]), babl_component("G~"), NULL);
    ...
    g_snprintf(name, sizeof(name), "B' %s", babl_types[i]);
    babl_format_new("name", name, babl_model("R'G'B'A"),
                    babl_type(babl_types[i]), babl_component("B'"), NULL);
    g_snprintf(name, sizeof(name), "B~ %s", babl_types[i]);
    babl_format_new("name", name, babl_model("R~G~B~A"),
                    babl_type(babl_types[i]), babl_component("B~"), NULL);
...
void gimp_babl_init_fishes(GimpInitStatusFunc status_callback) {
  ...
  for (i = 0; i < G_N_ELEMENTS(fishes); i++) {
    status_callback(NULL, NULL,
                    (gdouble)(i + 1) / (gdouble)G_N_ELEMENTS(fishes) * 0.8);
...
} babl_descriptions[] = {{"RGB u8", N_("RGB")},
                         {"R'G'B' u8", N_("RGB")},
                         {"R~G~B~ u8", N_("RGB")},
                         {"RGB u16", N_("RGB")},
                         {"R'G'B' u16", N_("RGB")},
...
const gchar *gimp_babl_format_get_description(const Babl *babl) {
  const gchar *description;
  ...
  if (G_UNLIKELY(!babl_description_hash)) {
    ...
    babl_description_hash = g_hash_table_new(g_str_hash, g_str_equal);
...
GimpColorProfile *gimp_babl_format_get_color_profile(const Babl *format) {
  ...
  if (gimp_babl_format_get_trc(format) == GIMP_TRC_NON_LINEAR) {
    ...
    non_linear_format = gimp_babl_format(
        base_type, gimp_babl_precision(component_type, GIMP_TRC_NON_LINEAR),
        babl_format_has_alpha(format), babl_format_get_space(format));
  }

  icc = babl_space_get_icc(babl_format_get_space(non_linear_format), &icc_len);
...
GimpImageBaseType gimp_babl_format_get_base_type(const Babl *format) {
  ...
  if (!strcmp(name, "Y") || !strcmp(name, "Y'") || !strcmp(name, "Y~") ||
      !strcmp(name, "YA") || !strcmp(name, "Y'A") || !strcmp(name, "Y~A")) {
    return GIMP_GRAY;
  } else if (!strcmp(name, "RGB") || !strcmp(name, "R'G'B'") ||
             !strcmp(name, "R~G~B~") || !strcmp(name, "RGBA") ||
             !strcmp(name, "R'G'B'A") || !strcmp(name, "R~G~B~A") ||
             !strcmp(name, "RaGaBaA") || !strcmp(name, "R'aG'aB'aA") ||
...
GimpTRCType gimp_babl_format_get_trc(const Babl *format) {
  ...
  if (!strcmp(name, "Y") || !strcmp(name, "YA") || !strcmp(name, "RGB") ||
      !strcmp(name, "RGBA") || !strcmp(name, "RaGaBaA")) {
    return GIMP_TRC_LINEAR;
  } else if (!strcmp(name, "Y'") || !strcmp(name, "Y'A") ||
             !strcmp(name, "R'G'B'") || !strcmp(name, "R'G'B'A") ||
             !strcmp(name, "R'aG'aB'aA")) {
...
const Babl *gimp_babl_component_format(GimpImageBaseType base_type,
                                       GimpPrecision precision, gint index) {
  switch (base_type) {
  case GIMP_RGB:
    switch (precision) {
    case GIMP_PRECISION_U8_LINEAR:
      switch (index) {
      case 0:
        return babl_format("R u8");
      case 1:
        return babl_format("G u8");
      case 2:
        return babl_format("B u8");
      case 3:
        return babl_format("A u8");
      default:
        break;
      }
      break;
    ...
    case GIMP_PRECISION_U16_LINEAR:
      switch (index) {
      case 0:
        return babl_format("R u16");
      case 1:
        return babl_format("G u16");
      case 2:
        return babl_format("B u16");
      case 3:
        return babl_format("A u16");
      default:
        break;
      }
      break;
    ...
    case GIMP_PRECISION_U32_LINEAR:
      switch (index) {
      case 0:
        return babl_format("R u32");
      case 1:
        return babl_format("G u32");
      case 2:
        return babl_format("B u32");
      case 3:
        return babl_format("A u32");
      default:
        break;
      }
      break;
    ...
    case GIMP_PRECISION_HALF_LINEAR:
      switch (index) {
      case 0:
        return babl_format("R half");
      case 1:
        return babl_format("G half");
      case 2:
        return babl_format("B half");
      case 3:
        return babl_format("A half");
      default:
        break;
      }
      break;
    ...
    case GIMP_PRECISION_FLOAT_LINEAR:
      switch (index) {
      case 0:
        return babl_format("R float");
      case 1:
        return babl_format("G float");
      case 2:
        return babl_format("B float");
      case 3:
        return babl_format("A float");
      default:
        break;
      }
      break;
    ...
    case GIMP_PRECISION_DOUBLE_LINEAR:
      switch (index) {
      case 0:
        return babl_format("R double");
      case 1:
        return babl_format("G double");
      case 2:
        return babl_format("B double");
      case 3:
        return babl_format("A double");
      default:
        break;
      }
      break;
...
gimp_babl_format_change_component_type(const Babl *format,
  ...
  return gimp_babl_format(
      gimp_babl_format_get_base_type(format),
      gimp_babl_precision(component, gimp_babl_format_get_trc(format)),
      babl_format_has_alpha(format), babl_format_get_space(format));
}

const Babl *gimp_babl_format_change_trc(const Babl *format, GimpTRCType trc) {
  g_return_val_if_fail(format != NULL, NULL);

  return gimp_babl_format(
      gimp_babl_format_get_base_type(format),
      gimp_babl_precision(gimp_babl_format_get_component_type(format), trc),
      babl_format_has_alpha(format), babl_format_get_space(format));
}

gchar **gimp_babl_print_pixel(const Babl *format, gpointer pixel) {
  GimpPrecision precision;
  gint n_components;
  guchar tmp_pixel[32];
  gchar **strings;
  ...
  if (babl_format_is_palette(format)) {
    const Babl *f =
        gimp_babl_format(GIMP_RGB, precision, babl_format_has_alpha(format),
  ...
  case GIMP_COMPONENT_TYPE_HALF: {
    ...
    f = gimp_babl_format(gimp_babl_format_get_base_type(format), p,
