...
GType gimp_gegl_get_op_enum_type(const gchar *operation,
  ...
  node = g_object_new(GEGL_TYPE_NODE, "operation", operation, NULL);
...
GeglColor *gimp_gegl_color_new(const GimpRGB *rgb, const Babl *space) {
  GeglColor *color;

  g_return_val_if_fail(rgb != NULL, NULL);

  color = gegl_color_new(NULL);
  gegl_color_set_pixel(color, babl_format_with_space("R'G'B'A double", space),
                       rgb);
...
void gimp_gegl_progress_connect(GeglNode *node, GimpProgress *progress,
                                const gchar *text) {
  g_return_if_fail(GEGL_IS_NODE(node));
  g_return_if_fail(GIMP_IS_PROGRESS(progress));
  g_return_if_fail(text != NULL);

  g_signal_connect(node, "progress", G_CALLBACK(gimp_gegl_progress_callback),
                   progress);
...
const gchar *gimp_gegl_node_get_key(GeglNode *node, const gchar *key) {
...
gboolean gimp_gegl_node_has_key(GeglNode *node, const gchar *key) {
  return gimp_gegl_node_get_key(node, key) != NULL;
}

const Babl *gimp_gegl_node_get_format(GeglNode *node, const gchar *pad_name) {
  GeglOperation *op;
...
void gimp_gegl_node_set_underlying_operation(GeglNode *node,
  ...
  g_object_set_data(G_OBJECT(node), "gimp-gegl-node-underlying-operation",
                    operation);
}
...
GeglNode *gimp_gegl_node_get_underlying_operation(GeglNode *node) {
  ...
  operation =
      g_object_get_data(G_OBJECT(node), "gimp-gegl-node-underlying-operation");
...
gboolean gimp_gegl_param_spec_has_key(GParamSpec *pspec, const gchar *key,
                                      const gchar *value) {
...
GeglBuffer *gimp_gegl_buffer_dup(GeglBuffer *buffer) {
  ...
  g_object_get(buffer, "shift-x", &shift_x, "shift-y", &shift_y, "tile-width",
               &tile_width, "tile-height", &tile_height, NULL);

  new_buffer =
      g_object_new(GEGL_TYPE_BUFFER, "format", gegl_buffer_get_format(buffer),
                   "x", extent->x, "y", extent->y, "width", extent->width,
                   "height", extent->height, "abyss-x", abyss->x, "abyss-y",
                   abyss->y, "abyss-width", abyss->width, "abyss-height",
                   abyss->height, "shift-x", shift_x, "shift-y", shift_y,
                   "tile-width", tile_width, "tile-height", tile_height, NULL);

  gegl_rectangle_align_to_buffer(&rect, extent, buffer,
                                 GEGL_RECTANGLE_ALIGNMENT_SUPERSET);

  gimp_gegl_buffer_copy(buffer, &rect, GEGL_ABYSS_NONE, new_buffer, &rect);
...
gboolean gimp_gegl_buffer_set_extent(GeglBuffer *buffer,
  ...
  gegl_rectangle_align_to_buffer(&aligned_old_extent,
                                 ...
                                 GEGL_RECTANGLE_ALIGNMENT_SUPERSET);
  gegl_rectangle_align_to_buffer(&aligned_extent, extent, buffer,
                                 GEGL_RECTANGLE_ALIGNMENT_SUPERSET);

  n_diff_rects =
      gegl_rectangle_subtract(diff_rects, &aligned_old_extent, &aligned_extent);

  for (i = 0; i < n_diff_rects; i++)
    gegl_buffer_clear(buffer, &diff_rects[i]);

  if (gegl_rectangle_intersect(&old_extent_rem, gegl_buffer_get_extent(buffer),
                               &aligned_extent)) {
    n_diff_rects = gegl_rectangle_subtract(diff_rects, &old_extent_rem, extent);
