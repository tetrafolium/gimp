...
void gimp_debug_enable_instances(void) {
  ...
  class_hash = g_hash_table_new_full(g_str_hash, g_str_equal, NULL,
...
void gimp_debug_add_instance(GObject *instance, GObjectClass *klass) {
  if (class_hash) {
    ...
    if (!instance_hash) {
      instance_hash = g_hash_table_new(g_direct_hash, g_direct_equal);
...
static void gimp_debug_instance_foreach(GObject *instance) {
  g_printerr("  \'%s\': ref_count = %d\n",
             GIMP_IS_OBJECT(instance) ? gimp_object_get_name(instance)
                                      : "GObject",
             instance->ref_count);
}

static void gimp_debug_class_foreach(const gchar *type_name,
                                     GHashTable *instance_hash) {
  g_printerr("Leaked %s instances: %d\n", type_name,
             g_hash_table_size(instance_hash));

  g_hash_table_foreach(instance_hash, (GHFunc)gimp_debug_instance_foreach,
                       NULL);
}

void gimp_debug_instances(void) {
  if (class_hash) {
    g_hash_table_foreach(class_hash, (GHFunc)gimp_debug_class_foreach, NULL);
  }
}
