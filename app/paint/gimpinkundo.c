...
static void gimp_ink_undo_pop(GimpUndo *undo, GimpUndoMode undo_mode,
                              GimpUndoAccumulator *accum);
static void gimp_ink_undo_free(GimpUndo *undo, GimpUndoMode undo_mode);

G_DEFINE_TYPE(GimpInkUndo, gimp_ink_undo, GIMP_TYPE_PAINT_CORE_UNDO)

#define parent_class gimp_ink_undo_parent_class

static void gimp_ink_undo_class_init(GimpInkUndoClass *klass) {
...
static void gimp_ink_undo_init(GimpInkUndo *undo) { undo->last_blobs = NULL; }
...
static void gimp_ink_undo_constructed(GObject *object) {
  ...
  if (ink->start_blobs) {
    ...
    for (i = 0; i < g_list_length(ink->start_blobs); i++) {
      ...
      ink_undo->last_blobs =
          g_list_prepend(ink_undo->last_blobs, gimp_blob_duplicate(blob));
    }
