...
static void
gimp_perspective_clone_paint(GimpPaintCore *paint_core, GimpDrawable *drawable,
...
G_DEFINE_TYPE(GimpPerspectiveClone, gimp_perspective_clone, GIMP_TYPE_CLONE)

#define parent_class gimp_perspective_clone_parent_class

void gimp_perspective_clone_register(Gimp *gimp,
                                     GimpPaintRegisterCallback callback) {
  (*callback)(gimp, GIMP_TYPE_PERSPECTIVE_CLONE,
              GIMP_TYPE_PERSPECTIVE_CLONE_OPTIONS, "gimp-perspective-clone",
              _("Perspective Clone"), "gimp-tool-perspective-clone");
}
...
static void gimp_perspective_clone_init(GimpPerspectiveClone *clone) {
...
gimp_perspective_clone_paint(GimpPaintCore *paint_core, GimpDrawable *drawable,
  ...
  case GIMP_PAINT_STATE_INIT:
    if (source_core->set_source) {
      ...
      gimp_matrix3_transform_point(&clone->transform_inv, source_core->src_x,
                                   source_core->src_y, &clone->src_x_fv,
                                   &clone->src_y_fv);
      ...
      g_object_set(clone->node, "cache-policy", GEGL_CACHE_POLICY_NEVER, NULL);
      ...
      case GIMP_CLONE_IMAGE: {
        ...
        if ((options->sample_merged && (src_image != dest_image)) ||
      ...
      case GIMP_CLONE_PATTERN: {
        ...
        tile = gegl_node_new_child(clone->node, "operation", "gegl:tile", NULL);
        clone->crop =
            gegl_node_new_child(clone->node, "operation", "gegl:crop", NULL);
      } break;
      }

      src_node =
          gegl_node_new_child(clone->node, "operation", "gegl:buffer-source",
                              "buffer", orig_buffer, NULL);

      clone->transform_node =
          gegl_node_new_child(clone->node, "operation", "gegl:transform",
                              "sampler", GIMP_INTERPOLATION_LINEAR, NULL);

      clone->dest_node = gegl_node_new_child(clone->node, "operation",
                                             "gegl:write-buffer", NULL);

      if (tile) {
        gegl_node_link_many(src_node, tile, clone->crop, clone->transform_node,
                            clone->dest_node, NULL);

        g_object_unref(orig_buffer);
      } else {
        gegl_node_link_many(src_node, clone->transform_node, clone->dest_node,
                            NULL);
      }
    }
    break;
    ...
    if (source_core->set_source) {
      ...
      for (i = 0; i < n_strokes; i++) {
        ...
        if (options->align_mode == GIMP_SOURCE_ALIGN_REGISTERED) {
          ...
          gimp_matrix3_transform_point(&clone->transform_inv, dest_x, dest_y,
...
static GeglBuffer *gimp_perspective_clone_get_source(
  ...
  case GIMP_CLONE_IMAGE:
    if (!gimp_rectangle_intersect(
            xmin, ymin, xmax - xmin, ymax - ymin, gegl_buffer_get_x(src_buffer),
            gegl_buffer_get_y(src_buffer), gegl_buffer_get_width(src_buffer),
            gegl_buffer_get_height(src_buffer), NULL, NULL, NULL, NULL)) {
  ...
  case GIMP_CLONE_PATTERN:
    gegl_node_set(clone->crop, "x", (gdouble)xmin, "y", (gdouble)ymin, "width",
                  (gdouble)xmax - xmin, "height", (gdouble)ymax - ymin, NULL);
    break;
  }
  ...
  gegl_node_set(clone->dest_node, "buffer", dest_buffer, NULL);

  gegl_node_blit(clone->dest_node, 1.0,
                 GEGL_RECTANGLE(0, 0, x2d - x1d, y2d - y1d), NULL, NULL, 0,
                 GEGL_BLIT_DEFAULT);
...
void gimp_perspective_clone_get_source_point(GimpPerspectiveClone *clone,
  ...
  gimp_matrix3_transform_point(&clone->transform_inv, x, y, &temp_x, &temp_y);
  ...
  gimp_matrix3_transform_point(&clone->transform, temp_x, temp_y, newx, newy);
}
...
static void gimp_perspective_clone_get_matrix(GimpPerspectiveClone *clone,
  ...
  gimp_matrix3_translate(&temp, clone->dest_x_fv - clone->src_x_fv,
