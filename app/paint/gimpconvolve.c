...
static void gimp_convolve_calculate_matrix(GimpConvolve *convolve,
                                           GimpConvolveType type, gint radius_x,
                                           gint radius_y, gdouble rate);
...
void gimp_convolve_register(Gimp *gimp, GimpPaintRegisterCallback callback) {
  (*callback)(gimp, GIMP_TYPE_CONVOLVE, GIMP_TYPE_CONVOLVE_OPTIONS,
              "gimp-convolve", _("Convolve"), "gimp-tool-blur");
}
...
static void gimp_convolve_motion(GimpPaintCore *paint_core,
  ...
  gimp_brush_core_eval_transform_dynamics(GIMP_BRUSH_CORE(paint_core), drawable,
  ...
  for (i = 0; i < n_strokes; i++) {
    ...
    gimp_convolve_calculate_matrix(
        convolve, options->type, gimp_brush_get_width(brush_core->brush) / 2,
        gimp_brush_get_height(brush_core->brush) / 2, rate);
    ...
    gimp_gegl_buffer_copy(gimp_drawable_get_buffer(drawable),
                          GEGL_RECTANGLE(paint_buffer_x, paint_buffer_y,
                                         ...
                                         gegl_buffer_get_height(paint_buffer)),
                          GEGL_ABYSS_NONE, convolve_buffer,
    ...
    gimp_gegl_convolve(convolve_buffer,
                       ...
                       paint_buffer,
                       GEGL_RECTANGLE(0, 0, gegl_buffer_get_width(paint_buffer),
                                      gegl_buffer_get_height(paint_buffer)),
    ...
    gimp_brush_core_replace_canvas(
        ...
        gimp_paint_options_get_brush_mode(paint_options), 1.0,
