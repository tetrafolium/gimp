...
enum { PROP_0, PROP_SRC_DRAWABLE, PROP_SRC_X, PROP_SRC_Y };

static void gimp_source_core_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void gimp_source_core_get_property(GObject *object, guint property_id,
...
static GeglBuffer *gimp_source_core_real_get_source(
    GimpSourceCore *source_core, GimpDrawable *drawable,
    GimpPaintOptions *paint_options, GimpPickable *src_pickable,
    gint src_offset_x, gint src_offset_y, GeglBuffer *paint_buffer,
    gint paint_buffer_x, gint paint_buffer_y, gint *paint_area_offset_x,
    gint *paint_area_offset_y, gint *paint_area_width, gint *paint_area_height,
...
static void gimp_source_core_class_init(GimpSourceCoreClass *klass) {
  ...
  g_object_class_install_property(object_class, PROP_SRC_DRAWABLE,
                                  g_param_spec_object("src-drawable", NULL,
                                                      NULL, GIMP_TYPE_DRAWABLE,
                                                      GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_SRC_X,
                                  g_param_spec_int("src-x", NULL, NULL, 0,
                                                   GIMP_MAX_IMAGE_SIZE, 0,
                                                   GIMP_PARAM_READWRITE));

  g_object_class_install_property(object_class, PROP_SRC_Y,
                                  g_param_spec_int("src-y", NULL, NULL, 0,
                                                   GIMP_MAX_IMAGE_SIZE, 0,
                                                   GIMP_PARAM_READWRITE));
}

static void gimp_source_core_init(GimpSourceCore *source_core) {
...
static void gimp_source_core_set_property(GObject *object, guint property_id,
  ...
  case PROP_SRC_DRAWABLE:
    gimp_source_core_set_src_drawable(source_core, g_value_get_object(value));
    break;
  case PROP_SRC_X:
...

{
  ...
  opacity =
      gimp_dynamics_get_linear_value(dynamics, GIMP_DYNAMICS_OUTPUT_OPACITY,
                                     origin, paint_options, fade_point);
  if (opacity == 0.0)
    return;
  ...
  if (gimp_source_core_use_source(source_core, options)) {
    ...
    if (options->sample_merged) {
      ...
      gimp_item_get_offset(GIMP_ITEM(source_core->src_drawable), &off_x,
                           &off_y);
  ...
  gimp_brush_core_eval_transform_dynamics(brush_core, drawable, paint_options,
                                          origin);
  ...
  for (i = 0; i < n_strokes; i++) {
    ...
    paint_buffer = gimp_paint_core_get_paint_buffer(
        paint_core, drawable, paint_options, paint_mode, coords,
        &paint_buffer_x, &paint_buffer_y, NULL, NULL);
    if (!paint_buffer)
      continue;
    ...
    if (gimp_source_core_use_source(source_core, options)) {
      ...
      src_buffer =
          GIMP_SOURCE_CORE_GET_CLASS(source_core)
              ->get_source(source_core, drawable, paint_options, src_pickable,
                           src_offset_x, src_offset_y, paint_buffer,
                           paint_buffer_x, paint_buffer_y, &paint_area_offset_x,
                           &paint_area_offset_y, &paint_area_width,
                           &paint_area_height, &src_rect);
    ...
    if (op) {
      ...
      translate_before = gegl_node_new_child(
          node, "operation", "gegl:translate", "x", -(source_core->src_x + 0.5),
          "y", -(source_core->src_y + 0.5), NULL);

      gegl_node_add_child(node, op);

      translate_after = gegl_node_new_child(
          node, "operation", "gegl:translate", "x",
          (source_core->src_x + 0.5) + (paint_area_offset_x - src_rect.x), "y",
          (source_core->src_y + 0.5) + (paint_area_offset_y - src_rect.y),
          NULL);

      output = gegl_node_get_output_proxy(node, "output");

      gegl_node_link_many(input, translate_before, op, translate_after, output,
                          NULL);
    ...
    GIMP_SOURCE_CORE_GET_CLASS(source_core)
        ->motion(source_core, drawable, paint_options, coords, op, opacity,
                 src_pickable, src_buffer, &src_rect, src_offset_x,
                 src_offset_y, paint_buffer, paint_buffer_x, paint_buffer_y,
                 paint_area_offset_x, paint_area_offset_y, paint_area_width,
                 paint_area_height);
...
gboolean gimp_source_core_use_source(GimpSourceCore *source_core,
                                     GimpSourceOptions *options) {
  return GIMP_SOURCE_CORE_GET_CLASS(source_core)
      ->use_source(source_core, options);
}
...
static GeglBuffer *gimp_source_core_real_get_source(
  ...
  if (!gimp_rectangle_intersect(
          paint_buffer_x + src_offset_x, paint_buffer_y + src_offset_y,
          gegl_buffer_get_width(paint_buffer),
          gegl_buffer_get_height(paint_buffer), gegl_buffer_get_x(src_buffer),
          gegl_buffer_get_y(src_buffer), gegl_buffer_get_width(src_buffer),
          gegl_buffer_get_height(src_buffer), &x, &y, &width, &height)) {
    return FALSE;
  }
...
static void gimp_source_core_src_drawable_removed(GimpDrawable *drawable,
  ...
  g_signal_handlers_disconnect_by_func(
      drawable, gimp_source_core_src_drawable_removed, source_core);
}
