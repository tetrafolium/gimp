...
static void gimp_clone_motion(
    GimpSourceCore *source_core, GimpDrawable *drawable,
    GimpPaintOptions *paint_options, const GimpCoords *coords, GeglNode *op,
    gdouble opacity, GimpPickable *src_pickable, GeglBuffer *src_buffer,
    GeglRectangle *src_rect, gint src_offset_x, gint src_offset_y,
    GeglBuffer *paint_buffer, gint paint_buffer_x, gint paint_buffer_y,
    gint paint_area_offset_x, gint paint_area_offset_y, gint paint_area_width,
...
void gimp_clone_register(Gimp *gimp, GimpPaintRegisterCallback callback) {
  (*callback)(gimp, GIMP_TYPE_CLONE, GIMP_TYPE_CLONE_OPTIONS, "gimp-clone",
              _("Clone"), "gimp-tool-clone");
}
...
static void gimp_clone_motion(
  ...
  if (gimp_source_core_use_source(source_core, source_options)) {
    if (!op) {
      gimp_gegl_buffer_copy(
          src_buffer,
          GEGL_RECTANGLE(src_rect->x, src_rect->y, paint_area_width,
                         paint_area_height),
          GEGL_ABYSS_NONE, paint_buffer,
          GEGL_RECTANGLE(paint_area_offset_x, paint_area_offset_y, 0, 0));
    } else {
      gimp_gegl_apply_operation(
          src_buffer, NULL, NULL, op, paint_buffer,
    ...
    gegl_buffer_set_pattern(paint_buffer,
                            GEGL_RECTANGLE(paint_area_offset_x,
                                           paint_area_offset_y,
                                           paint_area_width, paint_area_height),
                            src_buffer, -paint_buffer_x - src_offset_x,
  ...
  gimp_brush_core_paste_canvas(
       ...
       */
      source_options->align_mode == GIMP_SOURCE_ALIGN_FIXED
