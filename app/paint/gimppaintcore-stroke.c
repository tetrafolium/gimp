...
gboolean gimp_paint_core_stroke(GimpPaintCore *core, GimpDrawable *drawable,
                                GimpPaintOptions *paint_options,
                                GimpCoords *strokes, gint n_strokes,
  ...
  if (gimp_paint_core_start(core, drawable, paint_options, &strokes[0],
    ...
    gimp_paint_core_paint(core, drawable, paint_options, GIMP_PAINT_STATE_INIT,
                          0);
    ...
    for (i = 1; i < n_strokes; i++) {
      gimp_paint_core_interpolate(core, drawable, paint_options, &strokes[i],
                                  0);
    }
...
gboolean gimp_paint_core_stroke_boundary(
    GimpPaintCore *core, GimpDrawable *drawable,
    GimpPaintOptions *paint_options, gboolean emulate_dynamics,
    const GimpBoundSeg *bound_segs, gint n_bound_segs, gint offset_x,
    gint offset_y, gboolean push_undo, GError **error) {
  ...
  for (s = 0; s < n_stroke_segs; s++) {
    while (stroke_segs[seg].x1 != -1 || stroke_segs[seg].x2 != -1 ||
           stroke_segs[seg].y1 != -1 || stroke_segs[seg].y2 != -1) {
      coords[n_coords] = default_coords;
    ...
    if (initialized || gimp_paint_core_start(core, drawable, paint_options,
                                             &coords[0], error)) {
      gint i;
      ...
      for (i = 1; i < n_coords; i++) {
        gimp_paint_core_interpolate(core, drawable, paint_options, &coords[i],
                                    0);
      }
...
gboolean gimp_paint_core_stroke_vectors(GimpPaintCore *core,
  ...
  for (stroke = vectors->strokes->head; stroke; stroke = stroke->next) {
    GArray *coords;
    gboolean closed;

    coords = gimp_stroke_interpolate(GIMP_STROKE(stroke->data), 1.0, &closed);
    ...
    if (coords && coords->len) {
      ...
      if (initialized ||
          gimp_paint_core_start(core, drawable, paint_options,
                                &g_array_index(coords, GimpCoords, 0), error)) {
        initialized = TRUE;
        ...
        for (i = 1; i < coords->len; i++) {
          gimp_paint_core_interpolate(core, drawable, paint_options,
                                      &g_array_index(coords, GimpCoords, i), 0);
        }
