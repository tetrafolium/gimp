...
static void clear_edges(GimpTempBuf *buf, gint top, gint bottom, gint left,
                        gint right) {
  guchar *data;
  ...
  if (left + right) {
    ...
    data = gimp_temp_buf_get_data(buf) + ((top + 1) * width - right) * bpp;
  ...
  data =
      gimp_temp_buf_get_data(buf) + ((height - bottom) * width - right) * bpp;

  memset(data, 0, (bottom * width + right) * bpp);
}

template <class T> static inline void rotate_pointers(T **p, gint n) {
  T *tmp;
  gint i;
...
template <class T>
static void
gimp_brush_core_subsample_mask_impl(const GimpTempBuf *mask, GimpTempBuf *dest,
                                    gint dest_offset_x, gint dest_offset_y,
  ...
  gegl_parallel_distribute_range(
      mask_height, PIXELS_PER_THREAD / mask_width, [=](gint y, gint height) {
        const value_type *m;
        value_type *d;
        const kernel_type *k;
        gint y0;
        ...
        m = (const value_type *)gimp_temp_buf_get_data(mask) + y0 * mask_width;
        ...
        for (i = y; i < y + height; i++) {
          ...
          memset(accum[KERNEL_HEIGHT - 1], 0, sizeof(accum_type) * dest_width);
        }
...
const GimpTempBuf *gimp_brush_core_subsample_mask(GimpBrushCore *core,
  ...
  dest = gimp_temp_buf_new(mask_width + 2, mask_height + 2, mask_format);
  clear_edges(dest, dest_offset_y, 0, 0, 0);

  core->subsample_brushes[index2][index1] = dest;

  if (mask_format == babl_format("Y u8")) {
    gimp_brush_core_subsample_mask_impl<guchar>(mask, dest, dest_offset_x,
                                                dest_offset_y, index1, index2);
  } else if (mask_format == babl_format("Y float")) {
    gimp_brush_core_subsample_mask_impl<gfloat>(mask, dest, dest_offset_x,
                                                dest_offset_y, index1, index2);
  } else {
    g_warn_if_reached();
  }
...
public:
  SimplePressure(gdouble pressure) { scale = 2.0 * pressure; }

  guchar operator()(guchar x) const {
    gint v = RINT(scale * x);

    return MIN(v, 255);
  }

  gfloat operator()(gfloat x) const {
    gfloat v = scale * x;

    return MIN(v, 1.0f);
  }

  template <class T> T operator()(T x) const = delete;
};
...
public:
  ...
  guchar operator()(guchar x) const { return RINT(255.0f * map[x]); }
...
template <class T> class CachedPressure {
  T map[T(~0) + 1];

public:
  template <class Pressure> CachedPressure(Pressure pressure) {
    gint i;
  ...
  T operator()(T x) const { return map[x]; }

  template <class U> U operator()(U x) const = delete;
};

template <class T, class Pressure>
void gimp_brush_core_pressurize_mask_impl(const GimpTempBuf *mask,
                                          GimpTempBuf *dest,
                                          Pressure pressure) {
  gegl_parallel_distribute_range(
      gimp_temp_buf_get_width(mask) * gimp_temp_buf_get_height(mask),
      PIXELS_PER_THREAD, [=](gint offset, gint size) {
        const T *m;
        T *d;
        gint i;
...
gimp_brush_core_pressurize_mask(GimpBrushCore *core,
  ...
  subsample_mask = gimp_brush_core_subsample_mask(core, brush_mask, x, y);
  ...
  core->pressure_brush = gimp_temp_buf_new(
      gimp_temp_buf_get_width(brush_mask) + 2,
      gimp_temp_buf_get_height(brush_mask) + 2, subsample_mask_format);
  ...
  if (subsample_mask_format == babl_format("Y u8")) {
    gimp_brush_core_pressurize_mask_impl<guchar>(
        subsample_mask, core->pressure_brush,
        CachedPressure<guchar>(Pressure(pressure)));
  } else if (subsample_mask_format == babl_format("Y float")) {
    gimp_brush_core_pressurize_mask_impl<gfloat>(
        subsample_mask, core->pressure_brush, Pressure(pressure));
  } else {
    g_warn_if_reached();
  }
...
template <class T>
static void
gimp_brush_core_solidify_mask_impl(const GimpTempBuf *mask, GimpTempBuf *dest,
                                   gint dest_offset_x, gint dest_offset_y) {
  gint mask_width = gimp_temp_buf_get_width(mask);
  gint mask_height = gimp_temp_buf_get_height(mask);
  gint dest_width = gimp_temp_buf_get_width(dest);
  ...
  gegl_parallel_distribute_area(
      GEGL_RECTANGLE(0, 0, mask_width, mask_height), PIXELS_PER_THREAD,
      [=](const GeglRectangle *area) {
        const T *m;
        gfloat *d;
        gint i, j;

        m = (const T *)gimp_temp_buf_get_data(mask) + area->y * mask_width +
            area->x;
...
const GimpTempBuf *gimp_brush_core_solidify_mask(GimpBrushCore *core,
  ...
  if (!core->solid_cache_invalid && brush_mask == core->last_solid_brush_mask) {
  ...
  dest = gimp_temp_buf_new(brush_mask_width + 2, brush_mask_height + 2,
                           babl_format("Y float"));
  clear_edges(dest, 1 + dest_offset_y, 1 - dest_offset_y, 1 + dest_offset_x,
              1 - dest_offset_x);

  core->solid_brushes[dest_offset_y][dest_offset_x] = dest;

  if (brush_mask_format == babl_format("Y u8")) {
    gimp_brush_core_solidify_mask_impl<guchar>(brush_mask, dest, dest_offset_x,
                                               dest_offset_y);
  } else if (brush_mask_format == babl_format("Y float")) {
    gimp_brush_core_solidify_mask_impl<gfloat>(brush_mask, dest, dest_offset_x,
                                               dest_offset_y);
  } else {
    g_warn_if_reached();
  }
