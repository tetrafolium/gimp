...
void gimp_airbrush_register(Gimp *gimp, GimpPaintRegisterCallback callback) {
  (*callback)(gimp, GIMP_TYPE_AIRBRUSH, GIMP_TYPE_AIRBRUSH_OPTIONS,
              "gimp-airbrush", _("Airbrush"), "gimp-tool-airbrush");
}
...
static void gimp_airbrush_class_init(GimpAirbrushClass *klass) {
  ...
  airbrush_signals[STAMP] =
      g_signal_new("stamp", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST,
                   G_STRUCT_OFFSET(GimpAirbrushClass, stamp), NULL, NULL, NULL,
                   G_TYPE_NONE, 0);
}
...
static void gimp_airbrush_paint(GimpPaintCore *paint_core,
  ...
  case GIMP_PAINT_STATE_INIT:
    GIMP_PAINT_CORE_CLASS(parent_class)
        ->paint(paint_core, drawable, paint_options, sym, paint_state, time);
    break;
  ...
  case GIMP_PAINT_STATE_MOTION:
    ...
    if ((options->rate != 0.0) && !options->motion_only) {
      ...
      timeout =
          (1000.0 / STAMP_MAX_FPS) / ((options->rate / 100.0) * dynamic_rate);

      airbrush->timeout_id = g_timeout_add_full(
          G_PRIORITY_HIGH, timeout, gimp_airbrush_timeout, airbrush, NULL);
    }
    break;
...
static void
gimp_airbrush_motion(GimpPaintCore *paint_core, GimpDrawable *drawable,
...

{
  ...
  _gimp_paintbrush_motion(paint_core, drawable, paint_options, sym, opacity);
}
...
void gimp_airbrush_stamp(GimpAirbrush *airbrush) {
  ...
  gimp_symmetry_set_origin(airbrush->sym, airbrush->drawable,
                           &airbrush->coords);

  gimp_airbrush_paint(GIMP_PAINT_CORE(airbrush), airbrush->drawable,
                      airbrush->paint_options, airbrush->sym,
