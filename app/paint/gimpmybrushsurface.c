...
static inline float calculate_rr(int xp, int yp, float x, float y,
...
static inline void closest_point_to_line(float lx, float ly, float px, float py,
                                         float *ox, float *oy) {
 ...
 */
static inline float calculate_rr_antialiased(int xp, int yp, float x, float y,
                                             float aspect_ratio, float sn,
                                             float cs, float one_over_radius2,
                                             float r_aa_start) {
  ...
  float farthest_x,
      farthest_y; /* farthest from origin, but still inside pixel */
  ...
  if (pixel_left < 0 && pixel_right > 0 && pixel_top < 0 && pixel_bottom > 0) {
    ...
    closest_point_to_line(cs, sn, pixel_center_x, pixel_center_y, &nearest_x,
                          &nearest_y);
...
static inline float calculate_alpha_for_rr(float rr, float hardness,
...
static GeglRectangle calculate_dab_roi(float x, float y, float radius) {
...
static void gimp_mypaint_surface_get_color(MyPaintSurface *base_surface,
  ...
  *color_a = 0.0f;
  ...
  if (dabRect.width > 0 || dabRect.height > 0) {
    ...
    GeglBufferIterator *iter = gegl_buffer_iterator_new(
        surface->buffer, &dabRect, 0, babl_format("R'aG'aB'aA float"),
        GEGL_BUFFER_READ, GEGL_ABYSS_CLAMP, 2);
    ...
    while (gegl_buffer_iterator_next(iter)) {
      ...
      for (iy = iter->items[0].roi.y;
           iy < iter->items[0].roi.y + iter->items[0].roi.height; iy++) {
        float yy = (iy + 0.5f - y);
        for (ix = iter->items[0].roi.x;
             ix < iter->items[0].roi.x + iter->items[0].roi.width; ix++) {
          /* pixel_weight == a standard dab with hardness = 0.5, aspect_ratio
           * = 1.0, and angle = 0.0 */
          float xx = (ix + 0.5f - x);
...
static int gimp_mypaint_surface_draw_dab(
    MyPaintSurface *base_surface, float x, float y, float radius, float color_r,
    float color_g, float color_b, float opaque, float hardness, float color_a,
  ...
  dabRect = calculate_dab_roi(x, y, radius);
  gegl_rectangle_intersect(&dabRect, &dabRect,
                           gegl_buffer_get_extent(surface->buffer));
  ...
  iter = gegl_buffer_iterator_new(surface->buffer, &dabRect, 0,
                                  ...
                                  GEGL_BUFFER_READWRITE, GEGL_ABYSS_NONE, 2);
  if (surface->paint_mask) {
    GeglRectangle mask_roi = dabRect;
    mask_roi.x -= surface->paint_mask_x;
    mask_roi.y -= surface->paint_mask_y;
  ...
  while (gegl_buffer_iterator_next(iter)) {
    ...
    for (iy = iter->items[0].roi.y;
      ...
      for (ix = iter->items[0].roi.x;
        ...
        if (radius < 3.0f)
          rr = calculate_rr_antialiased(ix, iy, x, y, aspect_ratio, sn, cs,
                                        one_over_radius2, r_aa_start);
        else
          rr = calculate_rr(ix, iy, x, y, aspect_ratio, sn, cs,
                            one_over_radius2);
        base_alpha = calculate_alpha_for_rr(rr, hardness, segment1_slope,
                                            segment2_slope);
...
static void gimp_mypaint_surface_begin_atomic(MyPaintSurface *base_surface) {}

static void gimp_mypaint_surface_end_atomic(MyPaintSurface *base_surface,
...
static void gimp_mypaint_surface_destroy(MyPaintSurface *base_surface) {
  GimpMybrushSurface *surface = (GimpMybrushSurface *)base_surface;
