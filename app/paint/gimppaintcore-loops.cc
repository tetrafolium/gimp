...
template <class Visitor, class Algorithm>
static inline void dispatch(Visitor visitor,
...
template <class Algorithm, class Dispatch,
          gboolean = (Algorithm::filter & Dispatch::mask) == Dispatch::mask>
struct dispatch_impl {
  template <class Visitor, class... DispatchRest>
  static void apply(Visitor visitor, const GimpPaintCoreLoopsParams *params,
                    GimpPaintCoreLoopsAlgorithm algorithms,
                    identity<Algorithm> algorithm, Dispatch disp,
...
template <class Visitor, class Algorithm, class Dispatch, class... DispatchRest>
static inline void
dispatch(Visitor visitor, const GimpPaintCoreLoopsParams *params,
         GimpPaintCoreLoopsAlgorithm algorithms, identity<Algorithm> algorithm,
         Dispatch disp, DispatchRest... disp_rest) {
  dispatch_impl<Algorithm, Dispatch>::apply(visitor, params, algorithms,
                                            algorithm, disp, disp_rest...);
}
...
static inline gfloat value_to_float(guint8 value) { return value / 255.0f; }

static inline gfloat value_to_float(gfloat value) { return value; }

template <class T> static inline gfloat value_to_float(T value) = delete;
...
struct AlgorithmBase {
   ...
   */
  explicit AlgorithmBase(const GimpPaintCoreLoopsParams *params) {}
   ...
   */
  template <class Derived> struct State {};
  ...
  void init(const GimpPaintCoreLoopsParams *params, State<Derived> *state,
            GeglBufferIterator *iter, const GeglRectangle *roi,
   ...
   */
  template <class Derived>
  void init_step(const GimpPaintCoreLoopsParams *params, State<Derived> *state,
                 GeglBufferIterator *iter, const GeglRectangle *roi,
                 const GeglRectangle *area, const GeglRectangle *rect) const {}
   ...
   */
  template <class Derived>
  void process_row(const GimpPaintCoreLoopsParams *params,
                   State<Derived> *state, GeglBufferIterator *iter,
                   const GeglRectangle *roi, const GeglRectangle *area,
                   const GeglRectangle *rect, gint y) const {}
   ...
   */
  template <class Derived>
  void finalize_step(const GimpPaintCoreLoopsParams *params,
   ...
   */
  template <class Derived>
  void finalize(const GimpPaintCoreLoopsParams *params,
                State<Derived> *state) const {}
};
...
template <template <class Base> class AlgorithmTemplate, guint Mask,
          class... Dependencies>
struct BasicDispatch {
  static constexpr guint mask = Mask;

  template <class Visitor, class Algorithm>
  void operator()(Visitor visitor, const GimpPaintCoreLoopsParams *params,
                  GimpPaintCoreLoopsAlgorithm algorithms,
...
struct MandatoryAlgorithmDispatch {
  ...
  void operator()(Visitor visitor, const GimpPaintCoreLoopsParams *params,
    ...
    BasicDispatch<AlgorithmTemplate, Mask, Dependencies...>()(
        visitor, params, algorithms, algorithm);
  }
};
...
template <class Base> struct PaintBuf : Base {
  ...
  explicit PaintBuf(const GimpPaintCoreLoopsParams *params) : Base(params) {
...
template <class Base, class MaskType> struct PaintMask : Base {
  ...
  explicit PaintMask(const GimpPaintCoreLoopsParams *params) : Base(params) {
    mask_stride = gimp_temp_buf_get_width(params->paint_mask);
    mask_data = (const mask_type *)gimp_temp_buf_get_data(params->paint_mask) +
...
template <class Base, gboolean StippleFlag> struct Stipple : Base {
...
template <class Base> struct CompMask : Base {
  ...
  template <class Derived> struct State : Base::template State<Derived> {
...
template <class Base>
static constexpr gboolean has_comp_mask(const CompMask<Base> *algorithm) {
  return TRUE;
}

static constexpr gboolean has_comp_mask(const AlgorithmBase *algorithm) {
  return FALSE;
}

template <class Base, class State>
static gfloat *comp_mask_data(const CompMask<Base> *algorithm, State *state) {
  return state->comp_mask_data;
}

template <class State>
static gfloat *comp_mask_data(const AlgorithmBase *algorithm, State *state) {
  return NULL;
}
...
template <class Base> struct TempCompMask : Base {
  static constexpr guint filter = Base::filter | ALGORITHM_TEMP_COMP_MASK;

  using Base::Base;

  template <class Derived> using State = typename Base::template State<Derived>;
...
static BasicDispatch<TempCompMask, ALGORITHM_TEMP_COMP_MASK,
...
template <class Base> struct CompBuffer : Base {
...
template <class Base>
static constexpr gboolean has_comp_buffer(const CompBuffer<Base> *algorithm) {
  return TRUE;
}

static constexpr gboolean has_comp_buffer(const AlgorithmBase *algorithm) {
  return FALSE;
}

template <class Base, class State>
static gfloat *comp_buffer_data(const CompBuffer<Base> *algorithm,
                                State *state) {
  return state->comp_buffer_data;
}

template <class State>
static gfloat *comp_buffer_data(const AlgorithmBase *algorithm, State *state) {
  return NULL;
}
...
template <class Base> struct TempCompBuffer : Base {
...
static BasicDispatch<TempCompBuffer, ALGORITHM_TEMP_COMP_BUFFER,
...
template <class Base, guint Access, gboolean First> struct CanvasBufferIterator;

template <class Base, guint Access>
static constexpr gboolean canvas_buffer_iterator_is_first(
    CanvasBufferIterator<Base, Access, TRUE> *algorithm) {
  return FALSE;
}
...
struct CanvasBufferIterator<Base, Access, TRUE> : Base {
   ...
   * including, the current class.
   */
  static constexpr GeglAccessMode canvas_buffer_access = (GeglAccessMode)Access;
...
template <guint Access> struct DispatchCanvasBufferIteratorHelper {
  template <class Base>
...
template <class Base> struct MaskBufferIterator : Base {
...
template <class Base, class State>
static gint mask_buffer_iterator(const MaskBufferIterator<Base> *algorithm,
                                 State *state) {
  return state->mask_buffer_iterator;
}

template <class State>
static gint mask_buffer_iterator(const AlgorithmBase *algorithm, State *state) {
  return -1;
}
...
struct CombinePaintMaskToCanvasBufferToPaintBufAlpha : Base {
  ...
  void process_row(const GimpPaintCoreLoopsParams *params,
    ...
    gint mask_offset = (y - roi->y) * this->mask_stride + (rect->x - roi->x);
    const mask_type *mask_pixel = &this->mask_data[mask_offset];
    gint paint_offset =
        (y - roi->y) * this->paint_stride + (rect->x - roi->x) * 4;
    ...
    for (x = 0; x < rect->width; x++) {
      if (Base::stipple) {
        ...
        if (params->paint_opacity > state->canvas_pixel[0]) {
          state->canvas_pixel[0] +=
              (params->paint_opacity - state->canvas_pixel[0]) *
              value_to_float(*mask_pixel) * params->paint_opacity;
        }
      }
...
static SuppressedAlgorithmDispatch<
    ...
    GIMP_PAINT_CORE_LOOPS_ALGORITHM_COMBINE_PAINT_MASK_TO_CANVAS_BUFFER |
        GIMP_PAINT_CORE_LOOPS_ALGORITHM_CANVAS_BUFFER_TO_PAINT_BUF_ALPHA,
    decltype(dispatch_paint_buf), decltype(dispatch_paint_mask),
...
template <class Base> struct CombinePaintMaskToCanvasBuffer : Base {
  using mask_type = typename Base::mask_type;
...
static AlgorithmDispatch<
    CombinePaintMaskToCanvasBuffer,
    GIMP_PAINT_CORE_LOOPS_ALGORITHM_COMBINE_PAINT_MASK_TO_CANVAS_BUFFER,
    decltype(dispatch_paint_mask), decltype(dispatch_stipple),
    DispatchCanvasBufferIterator<GEGL_BUFFER_READWRITE>>
    dispatch_combine_paint_mask_to_canvas_buffer;
...
template <class Base> struct CanvasBufferToPaintBufAlpha : Base {
  static constexpr guint filter =
      Base::filter |
      GIMP_PAINT_CORE_LOOPS_ALGORITHM_CANVAS_BUFFER_TO_PAINT_BUF_ALPHA |
      GIMP_PAINT_CORE_LOOPS_ALGORITHM_PAINT_MASK_TO_PAINT_BUF_ALPHA |
...
template <class Base> struct PaintMaskToPaintBufAlpha : Base {
  using mask_type = typename Base::mask_type;
  ...
  explicit PaintMaskToPaintBufAlpha(const GimpPaintCoreLoopsParams *params)
      : Base(params) {
...
template <class Base, gboolean Direct> struct CanvasBufferToCompMask : Base {
...
template <class Base> struct CanvasBufferToCompMask<Base, TRUE> : Base {
  static constexpr guint filter =
      Base::filter |
      GIMP_PAINT_CORE_LOOPS_ALGORITHM_CANVAS_BUFFER_TO_COMP_MASK |
      GIMP_PAINT_CORE_LOOPS_ALGORITHM_PAINT_MASK_TO_COMP_MASK;
...
struct DispatchCanvasBufferToCompMask {
  ...
  using DispatchDirect =
      BasicDispatch<AlgorithmDirect,
                    GIMP_PAINT_CORE_LOOPS_ALGORITHM_CANVAS_BUFFER_TO_COMP_MASK,
                    decltype(dispatch_comp_mask)>;
  using DispatchIndirect =
      BasicDispatch<AlgorithmIndirect,
                    GIMP_PAINT_CORE_LOOPS_ALGORITHM_CANVAS_BUFFER_TO_COMP_MASK,
                    decltype(dispatch_temp_comp_mask)>;

  template <class Algorithm, gboolean HasMaskBufferIterator =
                                 has_mask_buffer_iterator((Algorithm *)NULL)>
...
template <class Base, gboolean Direct> struct PaintMaskToCompMask : Base {
  using mask_type = typename Base::mask_type;
  using comp_mask_type = typename Base::comp_mask_type;

  static constexpr guint filter =
      Base::filter | GIMP_PAINT_CORE_LOOPS_ALGORITHM_PAINT_MASK_TO_COMP_MASK;
    ...
    if (has_mask_buffer_iterator(this)) {
      for (x = 0; x < rect->width; x++) {
        comp_mask_pixel[0] = value_to_float(mask_pixel[0]) *
                             state->mask_pixel[0] * params->paint_opacity;
      ...
      for (x = 0; x < rect->width; x++) {
        comp_mask_pixel[0] =
            value_to_float(mask_pixel[0]) * params->paint_opacity;
...
template <class Base> struct PaintMaskToCompMask<Base, TRUE> : Base {
  using mask_type = typename Base::mask_type;
  ...
  void init_step(const GimpPaintCoreLoopsParams *params, State<Derived> *state,
    ...
    gint mask_offset =
        (rect->y - roi->y) * this->mask_stride + (rect->x - roi->x);

    state->comp_mask_data =
        (mask_type *)&this->mask_data[mask_offset] - this->mask_stride;
  }
...
struct DispatchPaintMaskToCompMask {
  ...
  template <class Base> using AlgorithmDirect = PaintMaskToCompMask<Base, TRUE>;
  template <class Base>
  ...
  template <class Algorithm, class MaskType = typename Algorithm::mask_type,
            gboolean HasMaskBufferIterator =
                has_mask_buffer_iterator((Algorithm *)NULL)>
  struct Dispatch : DispatchIndirect {};
  ...
  void operator()(Visitor visitor, const GimpPaintCoreLoopsParams *params,
    ...
    if ((algorithms & mask) == mask) {
      dispatch(
          ...
          params, algorithms, algorithm, dispatch_paint_mask,
          dispatch_mask_buffer_iterator);
    } else {
      visitor(algorithm);
    }
...
template <class Base> struct DoLayerBlend : Base {
  static constexpr guint filter =
      Base::filter | GIMP_PAINT_CORE_LOOPS_ALGORITHM_DO_LAYER_BLEND;
  ...
  explicit DoLayerBlend(const GimpPaintCoreLoopsParams *params) : Base(params) {
    layer_mode = GIMP_OPERATION_LAYER_MODE(
        gimp_layer_mode_get_operation(params->paint_mode));
    layer_mode->opacity = params->image_opacity;

    iterator_format = gimp_layer_mode_get_format(
        params->paint_mode, layer_mode->blend_space,
        layer_mode->composite_space, layer_mode->composite_mode,
        gimp_temp_buf_get_format(params->paint_buf));

    g_return_if_fail(gimp_temp_buf_get_format(params->paint_buf) ==
                     iterator_format);
  }
  ...
  void init(const GimpPaintCoreLoopsParams *params, State<Derived> *state,
    ...
    state->iterator_base = gegl_buffer_iterator_add(
        iter, params->src_buffer, area, 0, iterator_format, GEGL_ACCESS_READ,
        GEGL_ABYSS_NONE);

    if (!has_comp_buffer((const Derived *)this)) {
      gegl_buffer_iterator_add(iter, params->dest_buffer, area, 0,
                               iterator_format, GEGL_ACCESS_WRITE,
                               GEGL_ABYSS_NONE);
    }
  ...
  void process_row(const GimpPaintCoreLoopsParams *params,
    ...
    if (!has_comp_buffer((const Derived *)this)) {
      ...
      out_pixel =
          comp_buffer_data((const Derived *)this,
                           (typename Derived::template State<Derived> *)state);
    }

    state->process_roi.y = y;

    layer_mode->function((GeglOperation *)layer_mode, state->in_pixel,
                         state->paint_pixel, mask_pixel, out_pixel, rect->width,
                         &state->process_roi, 0);
...
static MandatoryAlgorithmDispatch<
    DoLayerBlend, GIMP_PAINT_CORE_LOOPS_ALGORITHM_DO_LAYER_BLEND,
    decltype(dispatch_paint_buf), decltype(dispatch_mask_buffer_iterator)>
...
template <class Base> struct MaskComponents : Base {
  static constexpr guint filter =
      Base::filter | GIMP_PAINT_CORE_LOOPS_ALGORITHM_MASK_COMPONENTS;
  ...
  explicit MaskComponents(const GimpPaintCoreLoopsParams *params)
      : Base(params) {
  ...
  void init(const GimpPaintCoreLoopsParams *params, State<Derived> *state,
            GeglBufferIterator *iter, const GeglRectangle *roi,
            const GeglRectangle *area) const {
    state->dest_buffer_iterator =
        gegl_buffer_iterator_add(iter, params->dest_buffer, area, 0, format,
  ...
  void init_step(const GimpPaintCoreLoopsParams *params, State<Derived> *state,
    ...
    state->dest_pixel = (guint8 *)iter->items[state->dest_buffer_iterator].data;
  ...
  void process_row(const GimpPaintCoreLoopsParams *params,
    ...
    if (comp_fish) {
      babl_process(comp_fish, state->comp_buffer_data, state->comp_pixel,
                   rect->width);
    ...
    gimp_operation_mask_components_process(format, state->dest_pixel,
                                           comp_pixel, state->dest_pixel,
...
static AlgorithmDispatch<MaskComponents,
  ...
  dispatch(
      [&](auto algorithm_type) {
        ...
        gegl_parallel_distribute_area(
            &roi, PIXELS_PER_THREAD, [=](const GeglRectangle *area) {
              State state;
              gint y;
              ...
              if (Algorithm::max_n_iterators > 0) {
                ...
                iter =
                    gegl_buffer_iterator_empty_new(Algorithm::max_n_iterators);
                ...
                while (gegl_buffer_iterator_next(iter)) {
                  ...
                  for (y = 0; y < rect->height; y++) {
                    algorithm.process_row(params, &state, iter, &roi, area,
                                          rect, rect->y + y);
                  }
                ...
                for (y = 0; y < area->height; y++) {
                  algorithm.process_row(params, &state, NULL, &roi, area, area,
                                        area->y + y);
                }
      ...
      dispatch_canvas_buffer_to_paint_buf_alpha,
      dispatch_paint_mask_to_paint_buf_alpha,
      dispatch_canvas_buffer_to_comp_mask, dispatch_paint_mask_to_comp_mask,
      dispatch_do_layer_blend, dispatch_mask_components);
}
