...
static GeglBuffer *
gimp_heal_get_paint_buffer(GimpPaintCore *core, GimpDrawable *drawable,
                           GimpPaintOptions *paint_options,
                           GimpLayerMode paint_mode, const GimpCoords *coords,
                           gint *paint_buffer_x, gint *paint_buffer_y,
                           gint *paint_width, gint *paint_height);

static void
gimp_heal_motion(GimpSourceCore *source_core, GimpDrawable *drawable,
...
void gimp_heal_register(Gimp *gimp, GimpPaintRegisterCallback callback) {
  (*callback)(gimp, GIMP_TYPE_HEAL, GIMP_TYPE_SOURCE_OPTIONS, "gimp-heal",
              _("Healing"), "gimp-tool-heal");
}
...
static gboolean gimp_heal_start(GimpPaintCore *paint_core,
  ...
  if (!GIMP_PAINT_CORE_CLASS(parent_class)
           ->start(paint_core, drawable, paint_options, coords, error)) {
    return FALSE;
  }
...
gimp_heal_get_paint_buffer(GimpPaintCore *core, GimpDrawable *drawable,
                           ...
                           gint *paint_buffer_x, gint *paint_buffer_y,
                           gint *paint_width, gint *paint_height) {
  return GIMP_PAINT_CORE_CLASS(parent_class)
      ->get_paint_buffer(core, drawable, paint_options, GIMP_LAYER_MODE_NORMAL,
                         coords, paint_buffer_x, paint_buffer_y, paint_width,
                         paint_height);
}

/* Subtract bottom from top and store in result as a float
 */
static void gimp_heal_sub(GeglBuffer *top_buffer, const GeglRectangle *top_rect,
 ...
 */
static void
gimp_heal_add(GeglBuffer *first_buffer, const GeglRectangle *first_rect,
              GeglBuffer *second_buffer, const GeglRectangle *second_rect,
              GeglBuffer *result_buffer, const GeglRectangle *result_rect) {
  GeglBufferIterator *iter;
  ...
  iter =
      gegl_buffer_iterator_new(first_buffer, first_rect, 0,
                               babl_format_n(babl_type("float"), n_components),
                               GEGL_ACCESS_READ, GEGL_ABYSS_NONE, 3);
...
static float gimp_heal_laplace_iteration_sse(gfloat *pixels, gfloat *Adiag,
                                             gint *Aidx, gfloat w, gint nmask) {
 ...
 */
static float gimp_heal_laplace_iteration(gfloat *pixels, gfloat *Adiag,
                                         gint *Aidx, gfloat w, gint nmask,
                                         gint depth) {
  gint i, k;
  gfloat err = 0;
  ...
  for (i = 0; i < nmask; i++) {
    ...
    for (k = 0; k < depth; k++) {
      gfloat diff =
          (a * pixels[j0 + k] - w * (pixels[j1 + k] + pixels[j2 + k] +
                                     pixels[j3 + k] + pixels[j4 + k]));
 ...
 */
static void gimp_heal_laplace_loop(gfloat *pixels, gint height, gint depth,
                                   gint width, guchar *mask) {
...
#define MAX_ITER 500
   ...
   * array results updating all of the red cells and then all of the black
   * cells.
   */
  nmask = 0;
...
#define A_NEIGHBOR(o, di, dj)                                                  \
  if ((dj < 0 && j == 0) || (dj > 0 && j == width - 1) ||                      \
      (di < 0 && i == 0) || (di > 0 && i == height - 1))                       \
    Aidx[o + nmask * 5] = zero;                                                \
  else                                                                         \
    Aidx[o + nmask * 5] = ((i + di) * width + (j + dj)) * depth;
           ...
           */
          Adiag[nmask] =
              4 - (i == 0) - (j == 0) - (i == height - 1) - (j == width - 1);
          A_NEIGHBOR(0, 0, 0);
          A_NEIGHBOR(1, 0, 1);
          A_NEIGHBOR(2, 1, 0);
          A_NEIGHBOR(3, 0, -1);
  ...
  for (iter = 0; iter < MAX_ITER; iter++) {
    gfloat err =
        gimp_heal_laplace_iteration(pixels, Adiag, Aidx, w, nmask, depth);
 ...
 */
static void gimp_heal(GeglBuffer *src_buffer, const GeglRectangle *src_rect,
                      GeglBuffer *dest_buffer, const GeglRectangle *dest_rect,
                      GeglBuffer *mask_buffer, const GeglRectangle *mask_rect) {
  ...
  diff_buffer = gegl_buffer_linear_new_from_data(
      diff, babl_format_n(babl_type("float"), src_components),
  ...
  gimp_heal_sub(dest_buffer, dest_rect, src_buffer, src_rect, diff_buffer,
                GEGL_RECTANGLE(0, 0, width, height));

  mask = g_new(guchar, mask_rect->width * mask_rect->height);

  gegl_buffer_get(mask_buffer, mask_rect, 1.0, babl_format("Y u8"), mask,
  ...
  gimp_heal_add(diff_buffer, GEGL_RECTANGLE(0, 0, width, height), src_buffer,
                src_rect, dest_buffer, dest_rect);
...
gimp_heal_motion(GimpSourceCore *source_core, GimpDrawable *drawable,
  ...
  if (gimp_dynamics_is_output_enabled(dynamics, GIMP_DYNAMICS_OUTPUT_FORCE))
    force = gimp_dynamics_get_linear_value(dynamics, GIMP_DYNAMICS_OUTPUT_FORCE,
                                           coords, paint_options, fade_point);
  else
    force = paint_options->brush_force;

  mask_buf = gimp_brush_core_get_brush_mask(GIMP_BRUSH_CORE(source_core),
                                            coords, GIMP_BRUSH_HARD, force);
  ...
  src_copy =
      gegl_buffer_new(GEGL_RECTANGLE(paint_area_offset_x, paint_area_offset_y,
                                     src_rect->width, src_rect->height),
                      babl_format("R'G'B'A float"));

  if (!op) {
    gimp_gegl_buffer_copy(src_buffer, src_rect, GEGL_ABYSS_NONE, src_copy,
                          gegl_buffer_get_extent(src_copy));
  } else {
    gimp_gegl_apply_operation(src_buffer, NULL, NULL, op, src_copy,
                              gegl_buffer_get_extent(src_copy), FALSE);
  }

  if (src_options->sample_merged) {
    dest_pickable = GIMP_PICKABLE(image);

    gimp_item_get_offset(GIMP_ITEM(drawable), &dest_pickable_off_x,
                         &dest_pickable_off_y);
  } else {
  ...
  gimp_gegl_buffer_copy(gimp_pickable_get_buffer(dest_pickable),
                        ...
                        GEGL_ABYSS_NONE, paint_buffer,
                        GEGL_RECTANGLE(paint_area_offset_x, paint_area_offset_y,
  ...
  /* find the offset of the brush mask's rect */
  {
    ...
    gint y =
        (gint)floor(coords->y) - (gegl_buffer_get_height(mask_buffer) >> 1);

    mask_off_x = (x < 0) ? -x : 0;
    mask_off_y = (y < 0) ? -y : 0;
  }

  gimp_heal(src_copy, gegl_buffer_get_extent(src_copy), paint_buffer,
            GEGL_RECTANGLE(paint_area_offset_x, paint_area_offset_y,
                           paint_area_width, paint_area_height),
            mask_buffer,
            GEGL_RECTANGLE(mask_off_x, mask_off_y, paint_area_width,
                           paint_area_height));
  ...
  gimp_brush_core_replace_canvas(
      GIMP_BRUSH_CORE(paint_core), drawable, coords,
      MIN(opacity, GIMP_OPACITY_OPAQUE), gimp_context_get_opacity(context),
      gimp_paint_options_get_brush_mode(paint_options), force,
