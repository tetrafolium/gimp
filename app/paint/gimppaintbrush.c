...
static gboolean gimp_paintbrush_real_get_color_history_color(
    GimpPaintbrush *paintbrush, GimpDrawable *drawable,
    GimpPaintOptions *paint_options, GimpRGB *color);
static void gimp_paintbrush_real_get_paint_params(
    GimpPaintbrush *paintbrush, GimpDrawable *drawable,
    GimpPaintOptions *paint_options, GimpSymmetry *sym, gdouble grad_point,
    GimpLayerMode *paint_mode, GimpPaintApplicationMode *paint_appl_mode,
    const GimpTempBuf **paint_pixmap, GimpRGB *paint_color);

G_DEFINE_TYPE(GimpPaintbrush, gimp_paintbrush, GIMP_TYPE_BRUSH_CORE)

void gimp_paintbrush_register(Gimp *gimp, GimpPaintRegisterCallback callback) {
...
static void gimp_paintbrush_paint(GimpPaintCore *paint_core,
  ...
  case GIMP_PAINT_STATE_INIT: {
    ...
    if (GIMP_PAINTBRUSH_GET_CLASS(paintbrush)->get_color_history_color &&
        GIMP_PAINTBRUSH_GET_CLASS(paintbrush)
            ->get_color_history_color(paintbrush, drawable, paint_options,
                                      &color)) {
  ...
  case GIMP_PAINT_STATE_MOTION:
    _gimp_paintbrush_motion(paint_core, drawable, paint_options, sym,
                            GIMP_OPACITY_OPAQUE);
    break;
...
static void gimp_paintbrush_real_get_paint_params(
  ...
  *paint_mode = gimp_context_get_paint_mode(context);
  ...
  if (gimp_paint_options_get_gradient_color(paint_options, image, grad_point,
    ...
    gimp_pickable_srgb_to_image_color(GIMP_PICKABLE(drawable), paint_color,
                                      paint_color);
...
void _gimp_paintbrush_motion(GimpPaintCore *paint_core, GimpDrawable *drawable,
  ...
  for (i = 0; i < n_strokes; i++) {
    ...
    GIMP_PAINTBRUSH_GET_CLASS(paintbrush)
        ->get_paint_params(paintbrush, drawable, paint_options, sym, grad_point,
                           &paint_mode, &paint_appl_mode, &paint_pixmap,
                           &paint_color);
     ...
     * in-place).
     */
    if (paint_core->applicator || paint_buffer != paintbrush->paint_buffer ||
        paint_pixmap != paintbrush->paint_pixmap ||
        (!paint_pixmap &&
         (gimp_rgba_distance(&paint_color, &paintbrush->paint_color)))) {
    ...
    gimp_brush_core_paste_canvas(
        brush_core, drawable, coords, MIN(opacity, GIMP_OPACITY_OPAQUE),
        gimp_context_get_opacity(context), paint_mode,
