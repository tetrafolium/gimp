...
static void gimp_smudge_paint(GimpPaintCore *paint_core, GimpDrawable *drawable,
                              GimpPaintOptions *paint_options,
                              GimpSymmetry *sym, GimpPaintState paint_state,
                              guint32 time);
...
void gimp_smudge_register(Gimp *gimp, GimpPaintRegisterCallback callback) {
  (*callback)(gimp, GIMP_TYPE_SMUDGE, GIMP_TYPE_SMUDGE_OPTIONS, "gimp-smudge",
              _("Smudge"), "gimp-tool-smudge");
}
...
static void gimp_smudge_paint(GimpPaintCore *paint_core, GimpDrawable *drawable,
  ...
  case GIMP_PAINT_STATE_INIT: {
     ...
     */
    if (options->flow > 0.0 &&
        !gimp_dynamics_is_output_enabled(dynamics,
  ...
  case GIMP_PAINT_STATE_MOTION:
    ...
    if (!smudge->initialized)
      smudge->initialized =
          gimp_smudge_start(paint_core, drawable, paint_options, sym);
...
static gboolean gimp_smudge_start(GimpPaintCore *paint_core,
  ...
  for (i = 0; i < n_strokes; i++) {
    ...
    accum_buffer = gegl_buffer_new(GEGL_RECTANGLE(0, 0, accum_size, accum_size),
                                   babl_format("RGBA float"));
    smudge->accum_buffers = g_list_prepend(smudge->accum_buffers, accum_buffer);

    /*  adjust the x and y coordinates to the upper left corner of the
     *  accumulator
     */
    paint_buffer = gimp_paint_core_get_paint_buffer(
        paint_core, drawable, paint_options, GIMP_LAYER_MODE_NORMAL, coords,
     ...
     *  brush position.
     */
    if (x != paint_buffer_x || y != paint_buffer_y ||
      ...
      pick_x = CLAMP((gint)coords->x + dest_pickable_off_x, 0,
                     gegl_buffer_get_width(pickable_buffer) - 1);
      pick_y = CLAMP((gint)coords->y + dest_pickable_off_y, 0,
                     gegl_buffer_get_height(pickable_buffer) - 1);

      gimp_pickable_get_pixel_at(dest_pickable, pick_x, pick_y,
                                 babl_format("RGBA float"), pixel);
    ...
    gimp_gegl_buffer_copy(
        ...
        GEGL_ABYSS_NONE, accum_buffer,
        GEGL_RECTANGLE(paint_buffer_x - x, paint_buffer_y - y, 0, 0));
  }
...
static void gimp_smudge_motion(GimpPaintCore *paint_core,
  ...
  rate = ((options->rate / 100.0) *
          gimp_dynamics_get_linear_value(dynamics, GIMP_DYNAMICS_OUTPUT_RATE,
                                         coords, paint_options, fade_point));

  flow = ((options->flow / 100.0) *
          gimp_dynamics_get_linear_value(dynamics, GIMP_DYNAMICS_OUTPUT_FLOW,
                                         coords, paint_options, fade_point));

  grad_point = gimp_dynamics_get_linear_value(
      dynamics, GIMP_DYNAMICS_OUTPUT_COLOR, coords, paint_options, fade_point);

  /* Get current gradient color, brush pixmap, or foreground color */
  brush_color_ptr = &brush_color;
  if (gimp_paint_options_get_gradient_color(paint_options, image, grad_point,
  ...
  if (brush_color_ptr)
    gimp_pickable_srgb_to_pixel(dest_pickable, &brush_color,
                                babl_format("RGBA double"), &brush_color);
  ...
  for (i = 0; i < n_strokes; i++) {
    ...
    paint_buffer = gimp_paint_core_get_paint_buffer(
        paint_core, drawable, paint_options, GIMP_LAYER_MODE_NORMAL, coords,
        &paint_buffer_x, &paint_buffer_y, &paint_width, &paint_height);
    if (!paint_buffer)
      continue;
    ...
    if (!brush_color_ptr && flow > 0.0) {
      gimp_brush_core_color_area_with_pixmap(brush_core, drawable, coords,
                                             paint_buffer, paint_buffer_x,
                                             paint_buffer_y, TRUE);
    }

    gimp_gegl_smudge_with_paint(
        accum_buffer,
        GEGL_RECTANGLE(paint_buffer_x - x, paint_buffer_y - y,
                       paint_buffer_width, paint_buffer_height),
        gimp_pickable_get_buffer(dest_pickable),
        GEGL_RECTANGLE(paint_buffer_x + dest_pickable_off_x,
                       paint_buffer_y + dest_pickable_off_y, paint_buffer_width,
                       paint_buffer_height),
        brush_color_ptr, paint_buffer, options->no_erasing, flow, rate);

    if (gimp_dynamics_is_output_enabled(dynamics, GIMP_DYNAMICS_OUTPUT_FORCE))
      force =
          gimp_dynamics_get_linear_value(dynamics, GIMP_DYNAMICS_OUTPUT_FORCE,
                                         coords, paint_options, fade_point);
    else
...
static void gimp_smudge_accumulator_size(GimpPaintOptions *paint_options,
  ...
  max_brush_size =
      MIN(paint_options->brush_size / max_view_scale, GIMP_BRUSH_MAX_SIZE);
