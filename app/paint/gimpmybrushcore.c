...
void gimp_mybrush_core_register(Gimp *gimp,
  ...
  (*callback)(gimp, GIMP_TYPE_MYBRUSH_CORE, GIMP_TYPE_MYBRUSH_OPTIONS,
              "gimp-mybrush", _("Mybrush"), "gimp-tool-mypaint-brush");
}

static void gimp_mybrush_core_class_init(GimpMybrushCoreClass *klass) {
...
static void gimp_mybrush_core_init(GimpMybrushCore *mybrush) {
...
static void gimp_mybrush_core_paint(GimpPaintCore *paint_core,
  ...
  case GIMP_PAINT_STATE_INIT:
    ...
    mybrush->private->surface = gimp_mypaint_surface_new(
        gimp_drawable_get_buffer(drawable),
        gimp_drawable_get_active_mask(drawable), paint_core->mask_buffer,
        paint_core->mask_x_offset, paint_core->mask_y_offset,
  ...
  case GIMP_PAINT_STATE_MOTION:
    gimp_mybrush_core_motion(paint_core, drawable, paint_options, sym, time);
    break;
...
static void gimp_mybrush_core_motion(GimpPaintCore *paint_core,
  ...
  if (mybrush->private->last_time < 0) {
    ...
    for (iter = mybrush->private->brushes, i = 0; iter;
         iter = g_list_next(iter), i++) {
      MyPaintBrush *brush = iter->data;
      GimpCoords *coords = gimp_symmetry_get_coords(sym, i);

      mypaint_brush_stroke_to(
          brush, (MyPaintSurface *)mybrush->private->surface, coords->x,
          coords->y, 0.0f, coords->xtilt, coords->ytilt,
  ...
  for (iter = mybrush->private->brushes, i = 0; iter;
    ...
    mypaint_brush_stroke_to(brush, (MyPaintSurface *)mybrush->private->surface,
                            coords->x, coords->y, pressure, coords->xtilt,
                            coords->ytilt, dt);
  }
...
static void gimp_mybrush_core_create_brushes(GimpMybrushCore *mybrush,
  ...
  gimp_pickable_srgb_to_image_color(GIMP_PICKABLE(drawable), &fg, &fg);
  ...
  for (i = 0; i < n_strokes; i++) {
    ...
    if (!mypaint_brush_get_base_value(brush,
      ...
      mypaint_brush_set_base_value(brush, MYPAINT_BRUSH_SETTING_COLOR_H, hsv.h);
      mypaint_brush_set_base_value(brush, MYPAINT_BRUSH_SETTING_COLOR_S, hsv.s);
      mypaint_brush_set_base_value(brush, MYPAINT_BRUSH_SETTING_COLOR_V, hsv.v);
    }

    mypaint_brush_set_base_value(
        brush, MYPAINT_BRUSH_SETTING_RADIUS_LOGARITHMIC, options->radius);
    mypaint_brush_set_base_value(brush, MYPAINT_BRUSH_SETTING_OPAQUE,
                                 options->opaque *
                                     gimp_context_get_opacity(context));
    mypaint_brush_set_base_value(brush, MYPAINT_BRUSH_SETTING_HARDNESS,
                                 options->hardness);
    mypaint_brush_set_base_value(
        brush, MYPAINT_BRUSH_SETTING_ERASER,
        (options->eraser && gimp_drawable_has_alpha(drawable)) ? 1.0f : 0.0f);

    mypaint_brush_new_stroke(brush);

    mybrush->private->brushes =
        g_list_prepend(mybrush->private->brushes, brush);
  }
