...
enum { PROP_0, PROP_RATE, PROP_FLOW, PROP_NO_ERASING, PROP_SAMPLE_MERGED };

static void gimp_smudge_options_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);
static void gimp_smudge_options_get_property(GObject *object, guint property_id,
                                             GValue *value, GParamSpec *pspec);

G_DEFINE_TYPE(GimpSmudgeOptions, gimp_smudge_options, GIMP_TYPE_PAINT_OPTIONS)
...
static void gimp_smudge_options_class_init(GimpSmudgeOptionsClass *klass) {
  ...
  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_RATE, "rate",
                          C_("smudge-tool", "Rate"),
                          _("The strength of smudging"), 0.0, 100.0,
                          SMUDGE_DEFAULT_RATE, GIMP_PARAM_STATIC_STRINGS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_FLOW, "flow",
                          C_("smudge-tool", "Flow"),
                          _("The amount of brush color to blend"), 0.0, 100.0,
                          SMUDGE_DEFAULT_FLOW, GIMP_PARAM_STATIC_STRINGS);
