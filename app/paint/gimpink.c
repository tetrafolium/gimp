...
static void gimp_ink_paint(GimpPaintCore *paint_core, GimpDrawable *drawable,
                           GimpPaintOptions *paint_options, GimpSymmetry *sym,
                           GimpPaintState paint_state, guint32 time);
static GeglBuffer *
gimp_ink_get_paint_buffer(GimpPaintCore *paint_core, GimpDrawable *drawable,
...
static GimpUndo *gimp_ink_push_undo(GimpPaintCore *core, GimpImage *image,
                                    const gchar *undo_desc);

static void gimp_ink_motion(GimpPaintCore *paint_core, GimpDrawable *drawable,
                            GimpPaintOptions *paint_options, GimpSymmetry *sym,
                            guint32 time);

static GimpBlob *ink_pen_ellipse(GimpInkOptions *options, gdouble x_center,
                                 gdouble y_center, gdouble pressure,
                                 gdouble xtilt, gdouble ytilt, gdouble velocity,
                                 const GimpMatrix3 *transform);

static void render_blob(GeglBuffer *buffer, GeglRectangle *rect,
                        GimpBlob *blob);
...
void gimp_ink_register(Gimp *gimp, GimpPaintRegisterCallback callback) {
  (*callback)(gimp, GIMP_TYPE_INK, GIMP_TYPE_INK_OPTIONS, "gimp-ink", _("Ink"),
              "gimp-tool-ink");
}
...
static void gimp_ink_paint(GimpPaintCore *paint_core, GimpDrawable *drawable,
  ...
  case GIMP_PAINT_STATE_INIT: {
    ...
    if (cur_coords->x == last_coords.x && cur_coords->y == last_coords.y) {
      if (ink->start_blobs) {
        g_list_free_full(ink->start_blobs, g_free);
        ink->start_blobs = NULL;
      }
      ...
      for (iter = ink->last_blobs, i = 0; iter; iter = g_list_next(iter), i++) {
        ...
        ink->start_blobs =
            g_list_prepend(ink->start_blobs, gimp_blob_duplicate(last_blob));
      }
...
static GimpUndo *gimp_ink_push_undo(GimpPaintCore *core, GimpImage *image,
                                    const gchar *undo_desc) {
  return gimp_image_undo_push(image, GIMP_TYPE_INK_UNDO, GIMP_UNDO_INK,
...
static void gimp_ink_motion(GimpPaintCore *paint_core, GimpDrawable *drawable,
  ...
  if (!ink->last_blobs) {
    ...
    for (i = 0; i < n_strokes; i++) {
      ...
      last_blob =
          ink_pen_ellipse(options, coords->x, coords->y, coords->pressure,
                          coords->xtilt, coords->ytilt, 100, &transform);

      ink->last_blobs = g_list_prepend(ink->last_blobs, last_blob);
      ink->start_blobs =
          g_list_prepend(ink->start_blobs, gimp_blob_duplicate(last_blob));
    ...
    for (i = 0; i < n_strokes; i++) {
      ...
      blob = ink_pen_ellipse(options, coords->x, coords->y, coords->pressure,
                             coords->xtilt, coords->ytilt,
                             coords->velocity * 100, &transform);
  ...
  gimp_context_get_foreground(context, &foreground);
  gimp_pickable_srgb_to_image_color(GIMP_PICKABLE(drawable), &foreground,
                                    &foreground);
  ...
  for (i = 0; i < n_strokes; i++) {
    ...
    render_blob(
        ...
        GEGL_RECTANGLE(paint_core->paint_buffer_x, paint_core->paint_buffer_y,
    ...
    gimp_paint_core_paste(
        paint_core, NULL, paint_core->paint_buffer_x,
        paint_core->paint_buffer_y, drawable, GIMP_OPACITY_OPAQUE,
        gimp_context_get_opacity(context), paint_mode, GIMP_PAINT_CONSTANT);
  }
...
static GimpBlob *ink_pen_ellipse(GimpInkOptions *options, gdouble x_center,
  ...
  size = options->size *
         (1.0 + options->size_sensitivity * (2.0 * pressure - 1.0));
  ...
  size = (options->vel_sensitivity *
              ((4.5 * size) /
               (1.0 + options->vel_sensitivity * (2.0 * velocity))) +
  ...
  x = (options->blob_aspect * cos(options->blob_angle) + xtilt * tscale_c -
       ytilt * tscale_s);
  y = (options->blob_aspect * sin(options->blob_angle) + ytilt * tscale_c +
       xtilt * tscale_s);

#ifdef VERBOSE
  g_printerr("angle %g aspect %g; %g %g; %g %g\n", options->blob_angle,
             options->blob_aspect, tscale_c, tscale_s, x, y);
#endif
  ...
  gimp_matrix3_transform_point(transform, tcos, tsin, &tcos, &tsin);
  ...
  return (*blob_function)(x_center * SUBSAMPLE, y_center * SUBSAMPLE,
 ...
 */
static void insert_sort(gint *data, gint n) {
  gint i, j, k;
...
static void fill_run(gfloat *dest, gfloat alpha, gint w) {
  if (alpha == 1.0) {
    while (w--) {
      *dest = 1.0;
      dest++;
...
static void render_blob_line(GimpBlob *blob, gfloat *dest, gint x, gint y,
                             gint width) {
