...
enum { PROP_0, PROP_UNDO_DESC };

/*  local function prototypes  */

static void gimp_paint_core_finalize(GObject *object);
static void gimp_paint_core_set_property(GObject *object, guint property_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gimp_paint_core_get_property(GObject *object, guint property_id,
...
static gboolean gimp_paint_core_real_pre_paint(GimpPaintCore *core,
                                               ...
                                               guint32 time);
static void
gimp_paint_core_real_paint(GimpPaintCore *core, GimpDrawable *drawable,
                           GimpPaintOptions *options, GimpSymmetry *sym,
                           GimpPaintState paint_state, guint32 time);
...
static void gimp_paint_core_real_interpolate(GimpPaintCore *core,
                                             ...
                                             GimpPaintOptions *options,
                                             guint32 time);
static GeglBuffer *gimp_paint_core_real_get_paint_buffer(
    GimpPaintCore *core, GimpDrawable *drawable, GimpPaintOptions *options,
    GimpLayerMode paint_mode, const GimpCoords *coords, gint *paint_buffer_x,
    gint *paint_buffer_y, gint *paint_width, gint *paint_height);
...
static void gimp_paint_core_class_init(GimpPaintCoreClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_UNDO_DESC,
      g_param_spec_string("undo-desc", NULL, NULL, _("Paint"),
...
static void
gimp_paint_core_real_paint(GimpPaintCore *core, GimpDrawable *drawable,
                           GimpPaintOptions *paint_options, GimpSymmetry *sym,
                           GimpPaintState paint_state, guint32 time) {}
...
static void gimp_paint_core_real_interpolate(GimpPaintCore *core,
                                             ...
                                             GimpPaintOptions *paint_options,
                                             guint32 time) {
  gimp_paint_core_paint(core, drawable, paint_options, GIMP_PAINT_STATE_MOTION,
                        time);
...
static GimpUndo *gimp_paint_core_real_push_undo(GimpPaintCore *core,
  ...
  return gimp_image_undo_push(image, GIMP_TYPE_PAINT_CORE_UNDO, GIMP_UNDO_PAINT,
                              undo_desc, 0, "paint-core", core, NULL);
}

/*  public functions  */

void gimp_paint_core_paint(GimpPaintCore *core, GimpDrawable *drawable,
                           GimpPaintOptions *paint_options,
                           GimpPaintState paint_state, guint32 time) {
  ...
  if (core_class->pre_paint(core, drawable, paint_options, paint_state, time)) {
    GimpSymmetry *sym;
    GimpImage *image;
    GimpItem *item;
    ...
    core_class->paint(core, drawable, paint_options, sym, paint_state, time);

    gimp_symmetry_clear_origin(sym);
    g_object_unref(sym);

    core_class->post_paint(core, drawable, paint_options, paint_state, time);
  }
}

gboolean gimp_paint_core_start(GimpPaintCore *core, GimpDrawable *drawable,
                               GimpPaintOptions *paint_options,
  ...
  if (!GIMP_PAINT_CORE_GET_CLASS(core)->start(core, drawable, paint_options,
                                              coords, error)) {
    return FALSE;
  }
  ...
  if (paint_options->use_applicator) {
    ...
    if (core->mask_buffer) {
      ...
      gimp_applicator_set_mask_offset(core->applicator, core->mask_x_offset,
                                      core->mask_y_offset);
    }
...
void gimp_paint_core_finish(GimpPaintCore *core, GimpDrawable *drawable,
  ...
  if (push_undo) {
    ...
    gimp_rectangle_intersect(core->x1, core->y1, core->x2 - core->x1,
                             ...
                             gimp_item_get_height(GIMP_ITEM(drawable)), &rect.x,
    ...
    gimp_gegl_buffer_copy(core->undo_buffer, &rect, GEGL_ABYSS_NONE, buffer,
                          GEGL_RECTANGLE(0, 0, 0, 0));

    gimp_drawable_push_undo(drawable, NULL, buffer, rect.x, rect.y, rect.width,
                            rect.height);
...
void gimp_paint_core_cancel(GimpPaintCore *core, GimpDrawable *drawable) {
  ...
  if (gimp_rectangle_intersect(
          core->x1, core->y1, core->x2 - core->x1, core->y2 - core->y1, 0, 0,
          gimp_item_get_width(GIMP_ITEM(drawable)),
          gimp_item_get_height(GIMP_ITEM(drawable)), &x, &y, &width, &height)) {
    GeglRectangle rect;

    gegl_rectangle_align_to_buffer(&rect, GEGL_RECTANGLE(x, y, width, height),
                                   gimp_drawable_get_buffer(drawable),
                                   GEGL_RECTANGLE_ALIGNMENT_SUPERSET);

    gimp_gegl_buffer_copy(core->undo_buffer, &rect, GEGL_ABYSS_NONE,
                          gimp_drawable_get_buffer(drawable), &rect);
  }
...
void gimp_paint_core_interpolate(GimpPaintCore *core, GimpDrawable *drawable,
                                 GimpPaintOptions *paint_options,
  ...
  GIMP_PAINT_CORE_GET_CLASS(core)->interpolate(core, drawable, paint_options,
                                               time);
}

void gimp_paint_core_set_show_all(GimpPaintCore *core, gboolean show_all) {
  g_return_if_fail(GIMP_IS_PAINT_CORE(core));
...
void gimp_paint_core_get_last_coords(GimpPaintCore *core, GimpCoords *coords) {
  g_return_if_fail(GIMP_IS_PAINT_CORE(core));
  g_return_if_fail(coords != NULL);
...
void gimp_paint_core_round_line(GimpPaintCore *core,
  ...
  if (constrain_15_degrees)
    gimp_constrain_line(core->last_coords.x, core->last_coords.y,
                        ...
                        GIMP_CONSTRAIN_LINE_15_DEGREES, constrain_offset_angle,
                        constrain_xres, constrain_yres);
}

/*  protected functions  */

GeglBuffer *gimp_paint_core_get_paint_buffer(
    GimpPaintCore *core, GimpDrawable *drawable,
    GimpPaintOptions *paint_options, GimpLayerMode paint_mode,
    const GimpCoords *coords, gint *paint_buffer_x, gint *paint_buffer_y,
    gint *paint_width, gint *paint_height) {
  ...
  paint_buffer = GIMP_PAINT_CORE_GET_CLASS(core)->get_paint_buffer(
      core, drawable, paint_options, paint_mode, coords, paint_buffer_x,
      paint_buffer_y, paint_width, paint_height);
...
void gimp_paint_core_paste(GimpPaintCore *core, const GimpTempBuf *paint_mask,
                           gint paint_mask_offset_x, gint paint_mask_offset_y,
                           GimpDrawable *drawable, gdouble paint_opacity,
                           gdouble image_opacity, GimpLayerMode paint_mode,
  ...
  if (core->applicator) {
    ...
    if (mode == GIMP_PAINT_CONSTANT) {
      ...
      if (paint_mask != NULL) {
        ...
        gimp_gegl_combine_mask_weird(
            paint_mask_buffer,
            GEGL_RECTANGLE(paint_mask_offset_x, paint_mask_offset_y, width,
                           height),
            core->canvas_buffer,
            GEGL_RECTANGLE(core->paint_buffer_x, core->paint_buffer_y, width,
                           height),
      ...
      gimp_applicator_set_src_buffer(core->applicator, core->undo_buffer);
    }
    /*  Otherwise:
     ...
     */
      ...
      gimp_gegl_apply_mask(paint_mask_buffer,
                           ...
                           core->paint_buffer,
                           GEGL_RECTANGLE(0, 0, width, height), paint_opacity);
    ...
    gimp_applicator_set_apply_buffer(core->applicator, core->paint_buffer);
    gimp_applicator_set_apply_offset(core->applicator, core->paint_buffer_x,
                                     core->paint_buffer_y);

    gimp_applicator_set_opacity(core->applicator, image_opacity);
    gimp_applicator_set_mode(
        core->applicator, paint_mode, GIMP_LAYER_COLOR_SPACE_AUTO,
    ...
    GimpPaintCoreLoopsAlgorithm algorithms =
        GIMP_PAINT_CORE_LOOPS_ALGORITHM_NONE;
  ...
  /*  Update the drawable  */
  gimp_drawable_update(drawable, core->paint_buffer_x, core->paint_buffer_y,
                       width, height);
}
 ...
 * NORMAL mode.
 */
void gimp_paint_core_replace(GimpPaintCore *core, const GimpTempBuf *paint_mask,
                             gint paint_mask_offset_x, gint paint_mask_offset_y,
                             GimpDrawable *drawable, gdouble paint_opacity,
  ...
  if (!gimp_drawable_has_alpha(drawable)) {
    gimp_paint_core_paste(core, paint_mask, paint_mask_offset_x,
                          paint_mask_offset_y, drawable, paint_opacity,
                          image_opacity, GIMP_LAYER_MODE_NORMAL, mode);
    return;
  }
  ...
  if (core->applicator) {
    ...
    if (mode == GIMP_PAINT_CONSTANT) {
      ...
      mask_rect = *GEGL_RECTANGLE(core->paint_buffer_x, core->paint_buffer_y,
                                  width, height);
     ...
     */
      ...
      mask_buffer = gimp_temp_buf_create_buffer((GimpTempBuf *)paint_mask);
      mask_rect = *GEGL_RECTANGLE(paint_mask_offset_x, paint_mask_offset_y,
                                  width, height);
    ...
    if (core->mask_buffer) {
      ...
      gegl_rectangle_align_to_buffer(&aligned_combined_mask_rect,
                                     &combined_mask_rect,
                                     gimp_drawable_get_buffer(drawable),
                                     GEGL_RECTANGLE_ALIGNMENT_SUPERSET);
      ...
      gimp_gegl_buffer_copy(
          core->mask_buffer,
          GEGL_RECTANGLE(aligned_combined_mask_rect.x - core->mask_x_offset,
                         aligned_combined_mask_rect.y - core->mask_y_offset,
                         aligned_combined_mask_rect.width,
                         aligned_combined_mask_rect.height),
          GEGL_ABYSS_NONE, combined_mask_buffer, &aligned_combined_mask_rect);

      gimp_gegl_combine_mask(mask_buffer, &mask_rect, combined_mask_buffer,
                             &combined_mask_rect, 1.0);
    ...
    gimp_applicator_set_opacity(core->applicator, image_opacity);
    gimp_applicator_set_mode(
        core->applicator, GIMP_LAYER_MODE_REPLACE, GIMP_LAYER_COLOR_SPACE_AUTO,
        GIMP_LAYER_COLOR_SPACE_AUTO,
        gimp_layer_mode_get_paint_composite_mode(GIMP_LAYER_MODE_REPLACE));
    ...
    gimp_paint_core_paste(core, paint_mask, paint_mask_offset_x,
                          paint_mask_offset_y, drawable, paint_opacity,
                          image_opacity, GIMP_LAYER_MODE_REPLACE, mode);
    return;
  }
...
void gimp_paint_core_smooth_coords(GimpPaintCore *core,
  ...
  if (smoothing_options->use_smoothing &&
    ...
    if (gaussian_weight2 != 0.0)
      gaussian_weight =
          1 / (sqrt(2 * G_PI) * smoothing_options->smoothing_factor);

    for (i = history->len - 1; i >= min_index; i--) {
      gdouble rate = 0.0;
      GimpCoords *next_coords = &g_array_index(history, GimpCoords, i);

      if (gaussian_weight2 != 0.0) {
        /* We use gaussian function with velocity as a window function */
        velocity_sum += next_coords->velocity * 100;
        rate = gaussian_weight *
               exp(-velocity_sum * velocity_sum / (2 * gaussian_weight2));
      }
