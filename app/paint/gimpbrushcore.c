...
enum { SET_BRUSH, SET_DYNAMICS, LAST_SIGNAL };
...
static GeglBuffer *gimp_brush_core_get_paint_buffer(
    GimpPaintCore *paint_core, GimpDrawable *drawable,
    GimpPaintOptions *paint_options, GimpLayerMode paint_mode,
    const GimpCoords *coords, gint *paint_buffer_x, gint *paint_buffer_y,
    gint *paint_width, gint *paint_height);
...
static const GimpTempBuf *gimp_brush_core_transform_mask(GimpBrushCore *core,
                                                         GimpBrush *brush);
...
static guint core_signals[LAST_SIGNAL] = {
    0,
};
...
static void gimp_brush_core_class_init(GimpBrushCoreClass *klass) {
  ...
  core_signals[SET_BRUSH] =
      g_signal_new("set-brush", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpBrushCoreClass, set_brush), NULL, NULL,
                   NULL, G_TYPE_NONE, 1, GIMP_TYPE_BRUSH);

  core_signals[SET_DYNAMICS] =
      g_signal_new("set-dynamics", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(GimpBrushCoreClass, set_dynamics), NULL,
                   NULL, NULL, G_TYPE_NONE, 1, GIMP_TYPE_DYNAMICS);
...
static void gimp_brush_core_init(GimpBrushCore *core) {
  ...
  for (i = 0; i < BRUSH_CORE_JITTER_LUTSIZE - 1; ++i) {
    core->jitter_lut_y[i] =
        cos(gimp_deg_to_rad(i * 360 / BRUSH_CORE_JITTER_LUTSIZE));
    core->jitter_lut_x[i] =
        sin(gimp_deg_to_rad(i * 360 / BRUSH_CORE_JITTER_LUTSIZE));
  }
...
static void gimp_brush_core_finalize(GObject *object) {
  ...
  if (core->main_brush) {
    g_signal_handlers_disconnect_by_func(
        core->main_brush, gimp_brush_core_invalidate_cache, core);
...
static gboolean gimp_brush_core_pre_paint(GimpPaintCore *paint_core,
                                          GimpDrawable *drawable,
                                          GimpPaintOptions *paint_options,
                                          GimpPaintState paint_state,
                                          guint32 time) {
  ...
  if (paint_state == GIMP_PAINT_STATE_MOTION) {
    ...
    if (last_coords.x == current_coords.x &&
        ...
        !gimp_brush_want_null_motion(core->main_brush, &last_coords,
                                     &current_coords)) {
      return FALSE;
    }
    ...
    if (GIMP_BRUSH_CORE_GET_CLASS(core)->handles_transforming_brush) {
      ...
      if (GIMP_BRUSH_CORE_GET_CLASS(core)->handles_dynamic_transforming_brush) {
        ...
        scale = paint_options->brush_size / MAX(width, height) *
                gimp_dynamics_get_linear_value(
                    core->dynamics, GIMP_DYNAMICS_OUTPUT_SIZE, &current_coords,
    ...
    if (GIMP_BRUSH_CORE_GET_CLASS(paint_core)->handles_changing_brush) {
      core->brush = gimp_brush_select_brush(core->main_brush, &last_coords,
                                            &current_coords);
    }
    if ((!GIMP_IS_BRUSH_GENERATED(core->main_brush)) &&
        (paint_options->brush_hardness !=
         gimp_brush_get_blur_hardness(core->main_brush))) {
...
static void gimp_brush_core_post_paint(GimpPaintCore *paint_core,
                                       GimpDrawable *drawable,
                                       GimpPaintOptions *paint_options,
                                       GimpPaintState paint_state,
                                       guint32 time) {
...
static gboolean gimp_brush_core_start(GimpPaintCore *paint_core,
                                      GimpDrawable *drawable,
                                      GimpPaintOptions *paint_options,
  ...
  if (GIMP_BRUSH_CORE_GET_CLASS(core)->handles_transforming_brush) {
    gimp_brush_core_eval_transform_dynamics(core, drawable, paint_options,
                                            coords);
  ...
  core->jitter = gimp_paint_options_get_jitter(
      paint_options, gimp_item_get_image(GIMP_ITEM(drawable)));
 ...
 **/
static void gimp_avoid_exact_integer(gdouble *x) {
...
static void gimp_brush_core_interpolate(GimpPaintCore *paint_core,
                                        GimpDrawable *drawable,
                                        GimpPaintOptions *paint_options,
                                        guint32 time) {
  GimpBrushCore *core = GIMP_BRUSH_CORE(paint_core);
  ...
  if (!delta_vec.x && !delta_vec.y && !delta_pressure && !delta_xtilt &&
      !delta_ytilt && !delta_wheel && !delta_velocity)
    return;
  ...
  if (core->scale == 0.0) {
    ...
    paint_core->pixel_dist =
        pixel_initial + pixel_dist; /* Don't forget to update pixel distance*/
  ...
  spacing_output =
      gimp_dynamics_get_output(core->dynamics, GIMP_DYNAMICS_OUTPUT_SPACING);

  fade_point =
      gimp_paint_options_get_fade(paint_options, image, paint_core->pixel_dist);

  use_dyn_spacing = gimp_dynamics_output_is_enabled(spacing_output);

  if (use_dyn_spacing) {
    dyn_spacing = gimp_dynamics_output_get_linear_value(
        spacing_output, &current_coords, paint_options, fade_point);
     ...
     */
    dyn_spacing =
        (core->spacing + ((2.0 - core->spacing) * (1.0 - dyn_spacing)));
  ...
  if (use_dyn_spacing) {
    ...
    if (t0 < 0.0 &&
        !(x == (gint)floor(last_coords.x) && y == (gint)floor(last_coords.y))) {
  ...
  for (n = 0; n < num_points; n++) {
    ...
    if (core->jitter > 0.0) {
      ...
      dyn_jitter =
          (core->jitter * gimp_dynamics_get_linear_value(
                              core->dynamics, GIMP_DYNAMICS_OUTPUT_JITTER,
                              &current_coords, paint_options, fade_point));

      jitter_dist = g_rand_double_range(core->rand, 0, dyn_jitter);
      jitter_angle = g_rand_int_range(core->rand, 0, BRUSH_CORE_JITTER_LUTSIZE);

      current_coords.x += (x_axis.x + y_axis.x) * jitter_dist *
                          core->jitter_lut_x[jitter_angle] * core->scale;

      current_coords.y += (y_axis.y + x_axis.y) * jitter_dist *
                          core->jitter_lut_y[jitter_angle] * core->scale;
    }
...
static GeglBuffer *gimp_brush_core_get_paint_buffer(
  ...
  gimp_brush_transform_size(core->brush, core->scale, core->aspect_ratio,
                            gimp_brush_core_get_angle(core),
                            gimp_brush_core_get_reflect(core), &brush_width,
                            &brush_height);
  ...
  if ((x2 - x1) && (y2 - y1)) {
    ...
    format = gimp_layer_mode_get_format(
        paint_mode, GIMP_LAYER_COLOR_SPACE_AUTO, GIMP_LAYER_COLOR_SPACE_AUTO,
        composite_mode, gimp_drawable_get_format(drawable));
...
static void gimp_brush_core_real_set_brush(GimpBrushCore *core,
  ...
  if (core->main_brush) {
    ...
    g_signal_connect(core->main_brush, "invalidate-preview",
                     G_CALLBACK(gimp_brush_core_invalidate_cache), core);
  }
}
...
void gimp_brush_core_set_brush(GimpBrushCore *core, GimpBrush *brush) {
...
void gimp_brush_core_set_dynamics(GimpBrushCore *core, GimpDynamics *dynamics) {
  g_return_if_fail(GIMP_IS_BRUSH_CORE(core));
...
void gimp_brush_core_paste_canvas(GimpBrushCore *core, GimpDrawable *drawable,
                                  const GimpCoords *coords,
                                  gdouble brush_opacity, gdouble image_opacity,
  ...
  brush_mask = gimp_brush_core_get_brush_mask(core, coords, brush_hardness,
                                              dynamic_force);
  ...
  if (brush_mask) {
    ...
    gimp_paint_core_paste(paint_core, brush_mask, off_x, off_y, drawable,
 ...
 */
void gimp_brush_core_replace_canvas(GimpBrushCore *core, GimpDrawable *drawable,
                                    const GimpCoords *coords,
  ...
  if (brush_mask) {
    ...
    gimp_paint_core_replace(paint_core, brush_mask, off_x, off_y, drawable,
...
static const GimpTempBuf *gimp_brush_core_transform_mask(GimpBrushCore *core,
  ...
  mask = gimp_brush_transform_mask(
      brush, core->scale, core->aspect_ratio, gimp_brush_core_get_angle(core),
      gimp_brush_core_get_reflect(core), core->hardness);
...
const GimpTempBuf *
gimp_brush_core_get_brush_mask(GimpBrushCore *core, const GimpCoords *coords,
                               GimpBrushApplicationMode brush_hardness,
                               gdouble dynamic_force) {
  const GimpTempBuf *mask;
  ...
  case GIMP_BRUSH_SOFT:
    return gimp_brush_core_subsample_mask(core, mask, coords->x, coords->y);
    break;

  case GIMP_BRUSH_HARD:
    return gimp_brush_core_solidify_mask(core, mask, coords->x, coords->y);
    break;

  case GIMP_BRUSH_PRESSURE:
    return gimp_brush_core_pressurize_mask(core, mask, coords->x, coords->y,
                                           dynamic_force);
    break;
  }
...
const GimpTempBuf *gimp_brush_core_get_brush_pixmap(GimpBrushCore *core) {
  const GimpTempBuf *pixmap;

  if (core->scale <= 0.0)
    return NULL;

  pixmap = gimp_brush_transform_pixmap(
      core->brush, core->scale, core->aspect_ratio,
      gimp_brush_core_get_angle(core), gimp_brush_core_get_reflect(core),
      core->hardness);
...
void gimp_brush_core_eval_transform_dynamics(GimpBrushCore *core,
                                             GimpDrawable *drawable,
                                             GimpPaintOptions *paint_options,
                                             const GimpCoords *coords) {
  if (core->main_brush) {
  ...
  if (GIMP_BRUSH_CORE_GET_CLASS(core)->handles_dynamic_transforming_brush) {
    ...
    core->scale *= gimp_dynamics_get_linear_value(
        core->dynamics, GIMP_DYNAMICS_OUTPUT_SIZE, coords, paint_options,
        fade_point);

    core->angle += gimp_dynamics_get_angular_value(
        core->dynamics, GIMP_DYNAMICS_OUTPUT_ANGLE, coords, paint_options,
        fade_point);

    core->hardness *= gimp_dynamics_get_linear_value(
        core->dynamics, GIMP_DYNAMICS_OUTPUT_HARDNESS, coords, paint_options,
        fade_point);
    ...
    if (gimp_dynamics_is_output_enabled(core->dynamics,
      ...
      dyn_aspect = gimp_dynamics_get_aspect_value(
          core->dynamics, GIMP_DYNAMICS_OUTPUT_ASPECT_RATIO, coords,
          paint_options, fade_point);
...
void gimp_brush_core_eval_transform_symmetry(GimpBrushCore *core,
  ...
  if (symmetry) {
    gimp_symmetry_get_transform(symmetry, stroke, &core->symmetry_angle,
...
void gimp_brush_core_color_area_with_pixmap(GimpBrushCore *core,
                                            GimpDrawable *drawable,
                                            const GimpCoords *coords,
                                            GeglBuffer *area, gint area_x,
                                            gint area_y, gboolean apply_mask) {
  const GimpTempBuf *pixmap;
  ...
  gimp_gegl_buffer_copy(
      pixmap_buffer,
      GEGL_RECTANGLE(offset_x, offset_y, area_width, area_height),
      GEGL_ABYSS_NONE, area, GEGL_RECTANGLE(0, 0, area_width, area_height));
  ...
  if (mask) {
    ...
    gimp_gegl_apply_mask(
        mask_buffer,
        GEGL_RECTANGLE(offset_x, offset_y, area_width, area_height), area,
        GEGL_RECTANGLE(0, 0, area_width, area_height), 1.0);
