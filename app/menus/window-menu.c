...
static void window_menu_display_closed(GdkDisplay *display, gboolean is_error,
                                       GimpUIManager *manager);

/*  public functions  */

void window_menu_setup(GimpUIManager *manager, const gchar *group_name,
                       const gchar *ui_path) {
  ...
  g_object_set_data_full(G_OBJECT(manager), "move-to-screen-group-name",
                         g_strdup(group_name), (GDestroyNotify)g_free);
  g_object_set_data_full(G_OBJECT(manager), "move-to-screen-ui-path",
                         g_strdup(ui_path), (GDestroyNotify)g_free);
...
static void window_menu_display_opened(GdkDisplayManager *disp_manager,
  ...
  group_name =
      g_object_get_data(G_OBJECT(manager), "move-to-screen-group-name");
  ui_path = g_object_get_data(G_OBJECT(manager), "move-to-screen-ui-path");
  ...
  for (i = 0; i < n_screens; i++) {
    ...
    action_name =
        g_strdup_printf("%s-move-to-screen-%s", group_name, screen_name);
    g_free(screen_name);
