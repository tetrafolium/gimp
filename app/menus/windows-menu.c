...
static void windows_menu_display_reorder(GimpContainer *container,
                                         GimpDisplay *display, gint new_index,
                                         GimpUIManager *manager);
...
static void windows_menu_recent_remove(GimpContainer *container,
                                       GimpSessionInfo *info,
                                       GimpUIManager *manager);
static gboolean windows_menu_display_query_tooltip(GtkWidget *widget, gint x,
                                                   gint y,
...
void windows_menu_setup(GimpUIManager *manager, const gchar *ui_path) {
  ...
  g_object_set_data(G_OBJECT(manager), "image-menu-ui-path", (gpointer)ui_path);

  g_signal_connect_object(manager->gimp->displays, "add",
                          G_CALLBACK(windows_menu_display_add), manager, 0);
  g_signal_connect_object(manager->gimp->displays, "remove",
                          G_CALLBACK(windows_menu_display_remove), manager, 0);
  g_signal_connect_object(manager->gimp->displays, "reorder",
                          G_CALLBACK(windows_menu_display_reorder), manager, 0);

  for (list = gimp_get_display_iter(manager->gimp); list;
  ...
  g_signal_connect_object(
      ...
      G_CALLBACK(windows_menu_dock_window_added), manager, 0);
  g_signal_connect_object(
      gimp_dialog_factory_get_singleton(), "dock-window-removed",
      G_CALLBACK(windows_menu_dock_window_removed), manager, 0);
  ...
  g_signal_connect_object(global_recent_docks, "add",
                          G_CALLBACK(windows_menu_recent_add), manager, 0);
  g_signal_connect_object(global_recent_docks, "remove",
                          G_CALLBACK(windows_menu_recent_remove), manager, 0);

  for (list = g_list_last(GIMP_LIST(global_recent_docks)->queue->head); list;
...
static void windows_menu_display_add(GimpContainer *container,
  ...
  g_signal_connect_object(display, "notify::image",
                          G_CALLBACK(windows_menu_image_notify), manager, 0);
  ...
  if (gimp_display_get_image(display)) {
    ...
    if (!merge_id) {
      ...
      if (widget) {
        ...
        action = gimp_ui_manager_find_action(manager, "windows", action_name);
...
static gboolean windows_menu_display_query_tooltip(GtkWidget *widget, gint x,
  ...
  gimp_viewable_calc_preview_size(gimp_image_get_width(image),
                                  ...
                                  FALSE, xres, yres, &width, &height, NULL);

  gtk_tooltip_set_icon(
      tooltip,
      gimp_viewable_get_pixbuf(impl->viewable, impl->context, width, height));
