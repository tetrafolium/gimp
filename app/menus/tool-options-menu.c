...
static void tool_options_menu_update_presets(
    GimpUIManager *manager, guint merge_id, const gchar *ui_path,
    const gchar *menu_path, const gchar *which_action, GimpContainer *presets);
...
static void tool_options_menu_update(GimpUIManager *manager,
  ...
  merge_id = GPOINTER_TO_UINT(
      g_object_get_data(G_OBJECT(manager), "tool-options-merge-id"));
...
static void tool_options_menu_update_after(GimpUIManager *manager,
  ...
  tool_options_menu_update_presets(manager, merge_id, ui_path, "Save", "save",
                                   tool_info->presets);

  tool_options_menu_update_presets(manager, merge_id, ui_path, "Restore",
                                   "restore", tool_info->presets);

  tool_options_menu_update_presets(manager, merge_id, ui_path, "Edit", "edit",
                                   tool_info->presets);

  tool_options_menu_update_presets(manager, merge_id, ui_path, "Delete",
...
static void tool_options_menu_update_presets(
  ...
  for (i = 0; i < n_children; i++) {
    ...
    action_name =
        g_strdup_printf("tool-options-%s-preset-%03d", which_action, i);
    path = g_strdup_printf("%s/%s", ui_path, menu_path);

    gimp_ui_manager_add_ui(manager, merge_id, path, action_name, action_name,
