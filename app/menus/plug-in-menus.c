...
static void plug_in_menus_menu_path_added(GimpPlugInProcedure *plug_in_proc,
                                          const gchar *menu_path,
                                          GimpUIManager *manager);
static void plug_in_menus_add_proc(GimpUIManager *manager, const gchar *ui_path,
                                   GimpPlugInProcedure *proc,
                                   const gchar *menu_path);
static void plug_in_menus_tree_insert(GTree *entries, const gchar *path,
...
static gchar *plug_in_menus_build_path(GimpUIManager *manager,
                                       const gchar *ui_path, guint merge_id,
                                       const gchar *menu_path,
                                       gboolean for_menu);
...
void plug_in_menus_setup(GimpUIManager *manager, const gchar *ui_path) {
  ...
  for (i = 0; i < manager->gimp->config->filter_history_size; i++) {
    ...
    action_path = g_strdup_printf("%s/Filters/Recently Used/Plug-ins", ui_path);
  ...
  menu_entries = g_tree_new_full((GCompareDataFunc)strcmp, NULL, g_free,
                                 (GDestroyNotify)plug_in_menu_entry_free);

  for (list = plug_in_manager->plug_in_procedures; list;
    ...
    g_signal_connect_object(plug_in_proc, "menu-path-added",
                            G_CALLBACK(plug_in_menus_menu_path_added), manager,
                            0);
    ...
    if (plug_in_proc->menu_label && !plug_in_proc->file_proc) {
      ...
      locale_domain =
          gimp_plug_in_manager_get_locale_domain(plug_in_manager, file, NULL);
      ...
      for (path = plug_in_proc->menu_paths; path; path = g_list_next(path)) {
        if (g_str_has_prefix(path->data, manager->name)) {
          ...
          menu = g_strconcat(dgettext(locale_domain, path->data), "/",
                             dgettext(locale_domain, plug_in_proc->menu_label),
                             NULL);
  ...
  g_tree_foreach(menu_entries, (GTraverseFunc)plug_in_menus_tree_traverse,
                 manager);
  ...
  g_signal_connect_object(manager->gimp->pdb, "register-procedure",
                          G_CALLBACK(plug_in_menus_register_procedure), manager,
                          0);
...
static void plug_in_menus_unregister_procedure(GimpPDB *pdb,
  ...
  if (GIMP_IS_PLUG_IN_PROCEDURE(procedure)) {
    ...
    g_signal_handlers_disconnect_by_func(
        plug_in_proc, plug_in_menus_menu_path_added, manager);
    ...
    if (plug_in_proc->menu_label && !plug_in_proc->file_proc) {
      ...
      for (list = plug_in_proc->menu_paths; list; list = g_list_next(list)) {
        if (g_str_has_prefix(list->data, manager->name)) {
          ...
          merge_id =
              GPOINTER_TO_UINT(g_object_get_data(G_OBJECT(manager), merge_key));
          g_free(merge_key);
...
static void plug_in_menus_menu_path_added(GimpPlugInProcedure *plug_in_proc,
  ...
  if (g_str_has_prefix(menu_path, manager->name)) {
    if (!strcmp(manager->name, "<Image>")) {
      ...
      plug_in_menus_add_proc(manager, "/layers-popup", plug_in_proc, menu_path);
      ...
      plug_in_menus_add_proc(manager, "/fonts-popup", plug_in_proc, menu_path);
...
static void plug_in_menus_add_proc(GimpUIManager *manager, const gchar *ui_path,
  ...
  merge_id = GPOINTER_TO_UINT(g_object_get_data(G_OBJECT(manager), merge_key));

  if (!merge_id) {
    merge_id = gimp_ui_manager_new_merge_id(manager);
    g_object_set_data(G_OBJECT(manager), merge_key, GUINT_TO_POINTER(merge_id));
  }

  g_free(merge_key);

  menu_merge_id = GPOINTER_TO_UINT(
      g_object_get_data(G_OBJECT(manager), "plug-in-menu-merge-id"));
  ...
  gimp_ui_manager_add_ui(manager, merge_id, action_path,
...
static gchar *plug_in_menus_build_path(GimpUIManager *manager,
  ...
  if (!gimp_ui_manager_get_widget(manager, action_path)) {
    ...
    if (parent_action_path) {
      g_free(action_path);
      action_path =
          g_strdup_printf("%s/%s", parent_action_path, menu_item_name);

      if (!gimp_ui_manager_get_widget(manager, action_path)) {
        GIMP_LOG(MENUS, "adding menu '%s' at path '%s' for action '%s'",
                 menu_item_name, action_path, menu_path);

        gimp_ui_manager_add_ui(manager, merge_id, parent_action_path,
                               menu_item_name, menu_path, GTK_UI_MANAGER_MENU,
                               FALSE);

        gimp_ui_manager_add_ui(manager, merge_id, action_path, "Menus", NULL,
                               GTK_UI_MANAGER_PLACEHOLDER, FALSE);
        gimp_ui_manager_add_ui(manager, merge_id, action_path, "Separator",
