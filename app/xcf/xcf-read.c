...
guint xcf_read_int8(XcfInfo *info, guint8 *data, gint count) {
  gsize bytes_read = 0;
   ...
   */
  if (count > 0) {
    g_input_stream_read_all(info->input, data, count, &bytes_read, NULL, NULL);
...
guint xcf_read_int16(XcfInfo *info, guint16 *data, gint count) {
  guint total = 0;
...
guint xcf_read_int32(XcfInfo *info, guint32 *data, gint count) {
  guint total = 0;
...
guint xcf_read_int64(XcfInfo *info, guint64 *data, gint count) {
  guint total = 0;
...
guint xcf_read_offset(XcfInfo *info, goffset *data, gint count) {
  guint total = 0;
...
guint xcf_read_float(XcfInfo *info, gfloat *data, gint count) {
  return xcf_read_int32(info, (guint32 *)((void *)data), count);
}

guint xcf_read_string(XcfInfo *info, gchar **data, gint count) {
  guint total = 0;
  gint i;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (tmp > MAX_XCF_STRING_LEN) {
      ...
      data[i] =
          gimp_any_to_utf8(str, -1, _("Invalid UTF-8 string in XCF file"));
...
guint xcf_read_component(XcfInfo *info, gint bpc, guint8 *data, gint count) {
  switch (bpc) {
  case 1:
...
void xcf_read_from_be(gint bpc, guint8 *data, gint count) {
  gint i;
