...
static gboolean xcf_save_image_props(XcfInfo *info, GimpImage *image,
                                     GError **error);
static gboolean xcf_save_layer_props(XcfInfo *info, GimpImage *image,
                                     GimpLayer *layer, GError **error);
static gboolean xcf_save_channel_props(XcfInfo *info, GimpImage *image,
                                       GimpChannel *channel, GError **error);
static gboolean xcf_save_prop(XcfInfo *info, GimpImage *image,
                              PropType prop_type, GError **error, ...);
static gboolean xcf_save_layer(XcfInfo *info, GimpImage *image,
                               GimpLayer *layer, GError **error);
static gboolean xcf_save_channel(XcfInfo *info, GimpImage *image,
                                 GimpChannel *channel, GError **error);
static gboolean xcf_save_buffer(XcfInfo *info, GeglBuffer *buffer,
                                GError **error);
static gboolean xcf_save_level(XcfInfo *info, GeglBuffer *buffer,
                               GError **error);
static gboolean xcf_save_tile(XcfInfo *info, GeglBuffer *buffer,
                              GeglRectangle *tile_rect, const Babl *format,
                              GError **error);
static gboolean xcf_save_tile_rle(XcfInfo *info, GeglBuffer *buffer,
                                  GeglRectangle *tile_rect, const Babl *format,
                                  guchar *rlebuf, GError **error);
static gboolean xcf_save_tile_zlib(XcfInfo *info, GeglBuffer *buffer,
                                   GeglRectangle *tile_rect, const Babl *format,
                                   GError **error);
static gboolean xcf_save_parasite(XcfInfo *info, GimpParasite *parasite,
                                  GError **error);
static gboolean xcf_save_parasite_list(XcfInfo *info,
                                       GimpParasiteList *parasite,
                                       GError **error);
static gboolean xcf_save_old_paths(XcfInfo *info, GimpImage *image,
                                   GError **error);
static gboolean xcf_save_vectors(XcfInfo *info, GimpImage *image,
                                 GError **error);

/* private convenience macros */
#define xcf_write_int32_check_error(info, data, count)                         \
  G_STMT_START {                                                               \
...
#define xcf_write_offset_check_error(info, data, count)                        \
  G_STMT_START {                                                               \
...
#define xcf_write_zero_offset_check_error(info, count)                         \
  G_STMT_START {                                                               \
...
#define xcf_write_int8_check_error(info, data, count)                          \
  G_STMT_START {                                                               \
...
#define xcf_write_float_check_error(info, data, count)                         \
  G_STMT_START {                                                               \
...
#define xcf_write_string_check_error(info, data, count)                        \
  G_STMT_START {                                                               \
...
#define xcf_write_component_check_error(info, bpc, data, count)                \
  G_STMT_START {                                                               \
...
#define xcf_write_prop_type_check_error(info, prop_type)                       \
  G_STMT_START {                                                               \
...
#define xcf_progress_update(info)                                              \
  G_STMT_START {                                                               \
    progress++;                                                                \
    if (info->progress)                                                        \
...
static gboolean xcf_save_image_props(XcfInfo *info, GimpImage *image,
  ...
  if (info->compression != COMPRESS_NONE)
    xcf_check_error(
        xcf_save_prop(info, image, PROP_COMPRESSION, error, info->compression));
  ...
  xcf_check_error(
      xcf_save_prop(info, image, PROP_RESOLUTION, error, xres, yres));
  ...
  if (gimp_image_get_metadata(image)) {
    ...
    if (meta_string) {
      meta_parasite =
          gimp_parasite_new("gimp-image-metadata", GIMP_PARASITE_PERSISTENT,
                            strlen(meta_string) + 1, meta_string);
  ...
  if (g_list_length(gimp_image_symmetry_get(image))) {
    ...
    for (iter = gimp_image_symmetry_get(image); iter;
  ...
  if (gimp_parasite_list_length(private->parasites) > 0) {
    xcf_check_error(
        xcf_save_prop(info, image, PROP_PARASITES, error, private->parasites));
  }
  ...
  g_list_free_full(symmetry_parasites, (GDestroyNotify)gimp_parasite_free);
...
static gboolean xcf_save_layer_props(XcfInfo *info, GimpImage *image,
  ...
  if (gimp_viewable_get_parent(GIMP_VIEWABLE(layer))) {
    ...
    xcf_check_error(xcf_save_prop(info, image, PROP_ITEM_PATH, error, path));
    g_list_free(path);
  }
  ...
  if (layer == gimp_image_get_floating_selection(image)) {
    ...
    xcf_check_error(xcf_save_prop(info, image, PROP_FLOATING_SELECTION, error));
  }
  ...
  if (gimp_layer_get_mask(layer)) {
    ...
    xcf_check_error(xcf_save_prop(info, image, PROP_APPLY_MASK, error, FALSE));
    xcf_check_error(xcf_save_prop(info, image, PROP_EDIT_MASK, error, FALSE));
    xcf_check_error(xcf_save_prop(info, image, PROP_SHOW_MASK, error, FALSE));
  }

  gimp_item_get_offset(GIMP_ITEM(layer), &offset_x, &offset_y);

  xcf_check_error(
      xcf_save_prop(info, image, PROP_OFFSETS, error, offset_x, offset_y));
  xcf_check_error(
      xcf_save_prop(info, image, PROP_MODE, error, gimp_layer_get_mode(layer)));
  ...
  if (GIMP_IS_TEXT_LAYER(layer) && GIMP_TEXT_LAYER(layer)->text) {
    ...
    if (flags)
      xcf_check_error(
          xcf_save_prop(info, image, PROP_TEXT_LAYER_FLAGS, error, flags));
  }
  ...
  if (gimp_viewable_get_children(GIMP_VIEWABLE(layer))) {
    ...
    xcf_check_error(
        xcf_save_prop(info, image, PROP_GROUP_ITEM_FLAGS, error, flags));
  }
  ...
  if (gimp_parasite_list_length(parasites) > 0) {
    xcf_check_error(
        xcf_save_prop(info, image, PROP_PARASITES, error, parasites));
  }
...
static gboolean xcf_save_channel_props(XcfInfo *info, GimpImage *image,
  ...
  xcf_check_error(
      xcf_save_prop(info, image, PROP_LOCK_CONTENT, error,
                    gimp_item_get_lock_content(GIMP_ITEM(channel))));
  xcf_check_error(
      xcf_save_prop(info, image, PROP_LOCK_POSITION, error,
  ...
  xcf_check_error(
      xcf_save_prop(info, image, PROP_COLOR, error, &channel->color));
  xcf_check_error(
      xcf_save_prop(info, image, PROP_FLOAT_COLOR, error, &channel->color));
  xcf_check_error(xcf_save_prop(info, image, PROP_TATTOO, error,
...
static gboolean xcf_save_prop(XcfInfo *info, GimpImage *image,
  ...
  case PROP_GUIDES: {
    ...
    if (n_guides > 0) {
      ...
      for (; guides; guides = g_list_next(guides)) {
        ...
        default:
          g_warning("%s: skipping guide with bad orientation", G_STRFUNC);
          continue;
        }
  ...
  case PROP_USER_UNIT: {
    ...
    size = 2 * 4 + strlen(unit_strings[0])
               ? strlen(unit_strings[0]) + 5
               : 4 + strlen(unit_strings[1])
                     ? strlen(unit_strings[1]) + 5
                     : 4 + strlen(unit_strings[2])
                           ? strlen(unit_strings[2]) + 5
                           : 4 + strlen(unit_strings[3])
                                 ? strlen(unit_strings[3]) + 5
                                 : 4 + strlen(unit_strings[4])
...
static gboolean xcf_save_layer(XcfInfo *info, GimpImage *image,
  ...
  value = gimp_babl_format_get_image_type(
  ...
  xcf_check_error(xcf_save_buffer(
      info, gimp_drawable_get_buffer(GIMP_DRAWABLE(layer)), error));
  ...
  if (gimp_layer_get_mask(layer)) {
    ...
    xcf_check_error(xcf_seek_pos(info, offset, error));
    xcf_check_error(xcf_save_channel(info, image, GIMP_CHANNEL(mask), error));
  }
...
static gboolean xcf_save_channel(XcfInfo *info, GimpImage *image,
  ...
  xcf_check_error(xcf_save_buffer(
      info, gimp_drawable_get_buffer(GIMP_DRAWABLE(channel)), error));

  return TRUE;
}

static gint xcf_calc_levels(gint size, gint tile_size) {
  gint levels;
...
static gboolean xcf_save_level(XcfInfo *info, GeglBuffer *buffer,
  ...
  for (i = 0; i < ntiles; i++) {
    ...
    *next_offset++ = offset;
    ...
    gimp_gegl_buffer_get_tile_rect(buffer, XCF_TILE_WIDTH, XCF_TILE_HEIGHT, i,
                                   &rect);

    /* write out the tile. */
    switch (info->compression) {
    case COMPRESS_NONE:
      xcf_check_error(xcf_save_tile(info, buffer, &rect, format, error));
      break;
    case COMPRESS_RLE:
      xcf_check_error(
          xcf_save_tile_rle(info, buffer, &rect, format, rlebuf, error));
      break;
    case COMPRESS_ZLIB:
      xcf_check_error(xcf_save_tile_zlib(info, buffer, &rect, format, error));
      break;
...
static gboolean xcf_save_parasite(XcfInfo *info, GimpParasite *parasite,
  ...
  if (gimp_parasite_is_persistent(parasite)) {
    ...
    xcf_write_int8_check_error(info, gimp_parasite_data(parasite),
...
static void xcf_save_parasite_func(gchar *key, GimpParasite *parasite,
                                   XcfParasiteData *data) {
  if (!data->error)
    xcf_save_parasite(data->info, parasite, &data->error);
}

static gboolean xcf_save_parasite_list(XcfInfo *info, GimpParasiteList *list,
                                       GError **error) {
  XcfParasiteData data;
...
static gboolean xcf_save_old_paths(XcfInfo *info, GimpImage *image,
  ...
  for (list = gimp_image_get_vectors_iter(image); list;
    ...
    points = gimp_vectors_compat_get_points(vectors, (gint32 *)&num_points,
                                            (gint32 *)&closed);
...
static gboolean xcf_save_vectors(XcfInfo *info, GimpImage *image,
  ...
  for (list = gimp_image_get_vectors_iter(image); list;
    ...
    for (stroke_list = g_list_first(vectors->strokes->head); stroke_list;
      ...
      control_points =
          gimp_stroke_control_points_get(stroke, (gint32 *)&closed);
