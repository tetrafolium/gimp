...
guint xcf_write_int8(XcfInfo *info, const guint8 *data, gint count,
                     GError **error) {
   ...
   */
  if (count > 0) {
    if (!g_output_stream_write_all(info->output, data, count, &bytes_written,
                                   NULL, &my_error)) {
      g_propagate_prefixed_error(error, my_error, _("Error writing XCF: "));
    }
...
guint xcf_write_int16(XcfInfo *info, const guint16 *data, gint count,
                      GError **error) {
...
guint xcf_write_int32(XcfInfo *info, const guint32 *data, gint count,
                      GError **error) {
  GError *tmp_error = NULL;
  gint i;
...
guint xcf_write_int64(XcfInfo *info, const guint64 *data, gint count,
                      GError **error) {
  GError *tmp_error = NULL;
  gint i;
...
guint xcf_write_offset(XcfInfo *info, const goffset *data, gint count,
                       GError **error) {
  if (count > 0) {
    gint i;
...
guint xcf_write_zero_offset(XcfInfo *info, gint count, GError **error) {
  if (count > 0) {
    guint8 *tmp;
...
guint xcf_write_float(XcfInfo *info, const gfloat *data, gint count,
                      GError **error) {
  return xcf_write_int32(info, (const guint32 *)((gconstpointer)data), count,
                         error);
}

guint xcf_write_string(XcfInfo *info, gchar **data, gint count,
                       GError **error) {
  GError *tmp_error = NULL;
  guint total = 0;
  gint i;
...
guint xcf_write_component(XcfInfo *info, gint bpc, const guint8 *data,
                          gint count, GError **error) {
  switch (bpc) {
  case 1:
...
void xcf_write_to_be(gint bpc, guint8 *data, gint count) {
  gint i;
