...
typedef GimpImage *GimpXcfLoaderFunc(Gimp *gimp, XcfInfo *info, GError **error);

static GimpValueArray *xcf_load_invoker(GimpProcedure *procedure, Gimp *gimp,
                                        GimpContext *context,
                                        GimpProgress *progress,
                                        const GimpValueArray *args,
                                        GError **error);
static GimpValueArray *xcf_save_invoker(GimpProcedure *procedure, Gimp *gimp,
                                        GimpContext *context,
                                        GimpProgress *progress,
                                        const GimpValueArray *args,
                                        GError **error);
...
void xcf_init(Gimp *gimp) {
  ...
  gimp_procedure_add_argument(
      procedure,
      g_param_spec_int("n-drawables", "Num drawables", "Number of drawables", 0,
                       G_MAXINT, 0, GIMP_PARAM_READWRITE));
  gimp_procedure_add_argument(
      procedure,
      gimp_param_spec_object_array(
          "drawables", "Drawables", "Selected drawables", GIMP_TYPE_DRAWABLE,
          GIMP_PARAM_READWRITE | GIMP_PARAM_NO_VALIDATE));
  gimp_procedure_add_argument(
      procedure, g_param_spec_object("file", "File",
                                     "The file "
...
GimpImage *xcf_load_stream(Gimp *gimp, GInputStream *input, GFile *input_file,
...
gboolean xcf_save_stream(Gimp *gimp, GimpImage *image, GOutputStream *output,
  ...
  info.file_version = gimp_image_get_xcf_version(
      image, info.compression == COMPRESS_ZLIB, NULL, NULL, NULL);
  ...
  if (!success && my_error)
    g_propagate_prefixed_error(error, my_error, _("Error writing '%s': "),
                               filename);
...
static GimpValueArray *xcf_save_invoker(GimpProcedure *procedure, Gimp *gimp,
  ...
  output = G_OUTPUT_STREAM(
      g_file_replace(file, NULL, FALSE, G_FILE_CREATE_NONE, NULL, &my_error));
  ...
  if (output) {
    ...
    g_propagate_prefixed_error(error, my_error, _("Error creating '%s': "),
                               gimp_file_get_utf8_name(file));
  }
