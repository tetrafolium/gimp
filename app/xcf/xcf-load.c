...
static gboolean xcf_load_layer_props(XcfInfo *info, GimpImage *image,
                                     GimpLayer **layer, GList **item_path,
                                     gboolean *apply_mask, gboolean *edit_mask,
                                     gboolean *show_mask,
                                     guint32 *text_layer_flags,
                                     guint32 *group_layer_flags);
static gboolean xcf_check_layer_props(XcfInfo *info, GList **item_path,
                                      gboolean *is_group_layer,
                                      gboolean *is_text_layer);
static gboolean xcf_load_channel_props(XcfInfo *info, GimpImage *image,
                                       GimpChannel **channel);
static gboolean xcf_load_prop(XcfInfo *info, PropType *prop_type,
                              guint32 *prop_size);
static GimpLayer *xcf_load_layer(XcfInfo *info, GimpImage *image,
                                 GList **item_path);
...
static gboolean xcf_load_tile(XcfInfo *info, GeglBuffer *buffer,
                              GeglRectangle *tile_rect, const Babl *format);
static gboolean xcf_load_tile_rle(XcfInfo *info, GeglBuffer *buffer,
                                  GeglRectangle *tile_rect, const Babl *format,
                                  gint data_length);
static gboolean xcf_load_tile_zlib(XcfInfo *info, GeglBuffer *buffer,
                                   GeglRectangle *tile_rect, const Babl *format,
                                   gint data_length);
...
static gboolean xcf_item_path_is_parent(GList *path, GList *parent_path);
static void xcf_fix_item_path(GimpLayer *layer, GList **path,
                              GList *broken_paths);
...
GimpImage *xcf_load_image(Gimp *gimp, XcfInfo *info, GError **error) {
  ...
  if (image_type < GIMP_RGB || image_type > GIMP_INDEXED || width <= 0 ||
      height <= 0)
    goto hard_error;
  ...
  if (!gimp_babl_is_valid(image_type, precision)) {
    gimp_message_literal(gimp, G_OBJECT(info->progress), GIMP_MESSAGE_ERROR,
  ...
  parasite =
      gimp_image_parasite_find(GIMP_IMAGE(image), gimp_grid_parasite_name());
  if (parasite) {
  ...
  for (iter = syms; iter; iter = g_list_next(iter)) {
    ...
    if (parasite) {
      GimpSymmetry *sym = gimp_symmetry_from_parasite(parasite, image, type);
  ...
  if (parasite) {
    ...
    if (has_metadata) {
      ...
      if (!gimp_metadata_set_from_exif(metadata, gimp_parasite_data(parasite),
  ...
  if (parasite) {
    ...
    if (has_metadata) {
      ...
      if (!gimp_metadata_set_from_xmp(metadata, (const guint8 *)xmp_data + 10,
                                      xmp_length - 10, &my_error)) {
        gimp_message(gimp, G_OBJECT(info->progress), GIMP_MESSAGE_WARNING,
                     _("Corrupt 'gimp-metadata' parasite discovered.\n"
  ...
  while (TRUE) {
    ...
    if (!layer) {
      ...
      if (!item_path) {
        ...
        item_path = g_list_prepend(
            NULL, GUINT_TO_POINTER(gimp_container_get_n_children(layers)));
    ...
    if (layer != info->floating_sel) {
      ...
      if (item_path) {
        ...
        parent = GIMP_LAYER(gimp_item_stack_get_parent_by_path(
            GIMP_ITEM_STACK(layers), item_path, NULL));
      ...
      gimp_image_add_layer(image, layer, parent,
                           gimp_container_get_n_children(container), FALSE);
    }
  ...
  while (TRUE) {
    ...
    if (channel != gimp_image_get_mask(image))
      gimp_image_add_channel(
          image, channel, NULL, /* FIXME tree */
          gimp_container_get_n_children(gimp_image_get_channels(image)), FALSE);
...
static void xcf_load_add_masks(GimpImage *image) {
  ...
  for (list = layers; list; list = g_list_next(list)) {
    ...
    if (mask) {
      ...
      apply_mask = GPOINTER_TO_INT(
          g_object_get_data(G_OBJECT(layer), "gimp-layer-mask-apply"));
      edit_mask = GPOINTER_TO_INT(
          g_object_get_data(G_OBJECT(layer), "gimp-layer-mask-edit"));
      show_mask = GPOINTER_TO_INT(
          g_object_get_data(G_OBJECT(layer), "gimp-layer-mask-show"));
...
static gboolean xcf_load_image_props(XcfInfo *info, GimpImage *image) {
  ...
  while (TRUE) {
    ...
    case PROP_COMPRESSION: {
      ...
      if ((compression != COMPRESS_NONE) && (compression != COMPRESS_RLE) &&
          (compression != COMPRESS_ZLIB) && (compression != COMPRESS_FRACTAL)) {
        gimp_message(info->gimp, G_OBJECT(info->progress), GIMP_MESSAGE_ERROR,
                     "Unknown compression type: %d", (gint)compression);
        return FALSE;
      }

      info->compression = compression;

      gimp_image_set_xcf_compression(image, compression >= COMPRESS_ZLIB);
    ...
    case PROP_GUIDES: {
      ...
      for (i = 0; i < nguides; i++) {
        ...
        GIMP_LOG(XCF, "prop guide orientation=%d position=%d", orientation,
                 position);
       ...
       */
    private
      ->guides = g_list_reverse(private->guides);
    } break;
    ...
    case PROP_SAMPLE_POINTS: {
      ...
      for (i = 0; i < n_sample_points; i++) {
        ...
        GIMP_LOG(XCF, "prop sample point x=%d y=%d mode=%d", x, y, pick_mode);

        if (pick_mode > GIMP_COLOR_PICK_MODE_LAST)
          pick_mode = GIMP_COLOR_PICK_MODE_PIXEL;

        sample_point = gimp_image_add_sample_point_at_pos(image, x, y, FALSE);
        gimp_image_set_sample_point_pick_mode(image, sample_point, pick_mode,
                                              FALSE);
      }
    } break;
    ...
    case PROP_USER_UNIT: {
      ...
      for (unit = gimp_unit_get_number_of_built_in_units(); unit < num_units;
           unit++) {
        ...
        if ((ABS(gimp_unit_get_factor(unit) - factor) < 1e-5) &&
            (strcmp(unit_strings[0], gimp_unit_get_identifier(unit)) == 0)) {
          break;
        }
      }
      ...
      /* no match */
      if (unit == num_units)
        unit = gimp_unit_new(unit_strings[0], factor, digits, unit_strings[1],
                             unit_strings[2], unit_strings[3], unit_strings[4]);
...
static gboolean xcf_load_layer_props(XcfInfo *info, GimpImage *image,
  ...
  while (TRUE) {
    ...
    case PROP_OFFSETS: {
      ...
      if (offset_x < -GIMP_MAX_IMAGE_SIZE || offset_x > GIMP_MAX_IMAGE_SIZE) {
      ...
      if (offset_y < -GIMP_MAX_IMAGE_SIZE || offset_y > GIMP_MAX_IMAGE_SIZE) {
...
static gboolean xcf_load_channel_props(XcfInfo *info, GimpImage *image,
  ...
  while (TRUE) {
    ...
    case PROP_SELECTION: {
      ...
      mask = gimp_selection_new(image, gimp_item_get_width(GIMP_ITEM(*channel)),
                                gimp_item_get_height(GIMP_ITEM(*channel)));
      gimp_image_take_mask(image, mask);

      gimp_drawable_steal_buffer(GIMP_DRAWABLE(mask), GIMP_DRAWABLE(*channel));
...
static GimpLayer *xcf_load_layer(XcfInfo *info, GimpImage *image,
  ...
  GIMP_LOG(XCF, "width=%d, height=%d, type=%d, name='%s'", width, height, type,
           name);
  ...
  if (base_type == GIMP_GRAY) {
    ...
    format = gimp_image_get_format(image, base_type,
                                   gimp_image_get_precision(image), has_alpha,
  ...
  if (!xcf_load_layer_props(info, image, &layer, item_path, &apply_mask,
                            &edit_mask, &show_mask, &text_layer_flags,
                            &group_layer_flags))
    goto error;
  ...
  if (gimp_text_layer_xcf_load_hack(&layer)) {
    ...
    if (selected) {
      info->selected_layers =
          g_list_delete_link(info->selected_layers, selected);
      info->selected_layers = g_list_prepend(info->selected_layers, layer);
    }
    if (floating)
      info->floating_sel = layer;
...
static gboolean xcf_load_tile_rle(XcfInfo *info, GeglBuffer *buffer,
   ...
   */
  g_input_stream_read_all(info->input, xcfdata, data_length, &bytes_read, NULL,
                          NULL);
  info->cp += bytes_read;
...
static gboolean xcf_load_old_paths(XcfInfo *info, GimpImage *image) {
  ...
  active_vectors = GIMP_VECTORS(gimp_container_get_child_by_index(
      gimp_image_get_vectors(image), last_selected_row));
...
static gboolean xcf_load_old_path(XcfInfo *info, GimpImage *image) {
  ...
  gimp_image_add_vectors(
      image, vectors, NULL, /* can't be a tree */
      gimp_container_get_n_children(gimp_image_get_vectors(image)), FALSE);
...
static gboolean xcf_load_vectors(XcfInfo *info, GimpImage *image) {
  ...
  /* FIXME tree */
  active_vectors = GIMP_VECTORS(gimp_container_get_child_by_index(
      gimp_image_get_vectors(image), active_index));
...
static gboolean xcf_load_vector(XcfInfo *info, GimpImage *image) {
  ...
  for (i = 0; i < num_strokes; i++) {
    ...
    stroke = g_object_new(stroke_type, "closed", closed, "control-points",
                          control_points, NULL);
...
static void xcf_fix_item_path(GimpLayer *layer, GList **path,
  ...
  for (iter = broken_paths; iter; iter = iter->next) {
    if (xcf_item_path_is_parent(*path, iter->data)) {
      ...
      g_printerr(
          "%s: layer '%s' moved to layer tree root because of missing parent.",
