...
void gimp_test_utils_set_env_to_subpath(const gchar *root_env_var1,
  ...
  if (!root_dir1)
    g_printerr("*\n"
               ...
               "*    set env %s=%s/source/gimp\n"
               "*\n",
               root_env_var1, root_env_var1, g_get_home_dir());
  ...
  if (!root_dir2)
    g_printerr("*\n"
               ...
               "*    set env %s=%s/source/gimp\n"
               "*\n",
               root_env_var2, root_env_var2, g_get_home_dir());
  ...
  target_path =
      g_strconcat(target_dir1, G_SEARCHPATH_SEPARATOR_S, target_dir2, NULL);
...
void gimp_test_utils_set_gimp3_directory(const gchar *root_env_var,
                                         const gchar *subdir) {
  gimp_test_utils_set_env_to_subdir(root_env_var, subdir,
...
void gimp_test_utils_setup_menus_path(void) {
  ...
  gimp_test_utils_set_env_to_subpath("GIMP_TESTING_ABS_TOP_SRCDIR",
                                     "GIMP_TESTING_ABS_TOP_BUILDDIR", "menus",
...
void gimp_test_utils_create_image(Gimp *gimp, gint width, gint height) {
  ...
  image = gimp_image_new(gimp, width, height, GIMP_RGB,
                         GIMP_PRECISION_U8_NON_LINEAR);

  layer = gimp_layer_new(image, width, height,
                         gimp_image_get_layer_format(image, TRUE), "layer1",
                         1.0, GIMP_LAYER_MODE_NORMAL);

  gimp_image_add_layer(image, layer, NULL /*parent*/, 0 /*position*/,
                       FALSE /*push_undo*/);

  gimp_create_display(gimp, image, GIMP_UNIT_PIXEL, 1.0 /*scale*/, NULL);
}
...
void gimp_test_utils_synthesize_key_event(GtkWidget *widget, guint keyval) {
  ...
  if (gdk_keymap_get_entries_for_keyval(
          gdk_keymap_get_for_display(gdk_display_get_default()), keyval, &keys,
          &n_keys)) {
    /* XXX not in use yet */
    CGEventRef commandDown =
        CGEventCreateKeyboardEvent(NULL, (CGKeyCode)kVK_Command, true);
    CGEventRef commandUp =
        CGEventCreateKeyboardEvent(NULL, (CGKeyCode)kVK_Command, false);

    CGEventRef shiftDown =
        CGEventCreateKeyboardEvent(NULL, (CGKeyCode)kVK_Shift, true);
    CGEventRef shiftUp =
        CGEventCreateKeyboardEvent(NULL, (CGKeyCode)kVK_Shift, false);

    CGEventRef optionDown =
        CGEventCreateKeyboardEvent(NULL, (CGKeyCode)kVK_Option, true);
    CGEventRef optionUp =
        CGEventCreateKeyboardEvent(NULL, (CGKeyCode)kVK_Option, false);
    ...
    for (i = 0; i < n_keys; i++) {
      ...
      keyDown =
          CGEventCreateKeyboardEvent(NULL, (CGKeyCode)keys[i].keycode, true);
      keyUp =
          CGEventCreateKeyboardEvent(NULL, (CGKeyCode)keys[i].keycode, false);
      /* Key pressed. */
    ...
    g_warning("%s: no macOS key mapping found for keyval %d.", G_STRFUNC,
              keyval);
  }

#else  /* ! GDK_WINDOWING_QUARTZ */
  gdk_test_simulate_key(gtk_widget_get_window(widget), -1, -1, /*x, y*/
                        keyval, 0 /*modifiers*/, GDK_KEY_PRESS);
  gdk_test_simulate_key(gtk_widget_get_window(widget), -1, -1, /*x, y*/
                        keyval, 0 /*modifiers*/, GDK_KEY_RELEASE);
#endif /* ! GDK_WINDOWING_QUARTZ */
}
...
GimpImage *gimp_test_utils_create_image_from_dialog(Gimp *gimp) {
  ...
  gimp_ui_manager_activate_action(ui_manager, "image", "image-new");
  gimp_test_run_mainloop_until_idle();

  /* Get the GtkWindow of the dialog */
  new_image_dialog = gimp_dialog_factory_dialog_raise(
      gimp_dialog_factory_get_singleton(),
      gdk_display_get_monitor(gdk_display_get_default(), 0), NULL,
      "gimp-image-new-dialog", -1 /*view_size*/);
  ...
  } while (tries_left-- && n_images != n_initial_images + 1);

  /* Make sure there now is one image more than initially */
  g_assert_cmpint(n_images, ==, n_initial_images + 1);
