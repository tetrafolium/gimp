...
#define ADD_TEST(function)                                                     \
  g_test_add("/gimp-tools/" #function, GimpTestFixture, gimp,                  \
             gimp_tools_setup_image, function, gimp_tools_teardown_image);
...
static void gimp_tools_teardown_image(GimpTestFixture *fixture,
                                      gconstpointer data);
static void gimp_tools_synthesize_image_click_drag_release(
    GimpDisplayShell *shell, gdouble start_image_x, gdouble start_image_y,
    gdouble end_image_x, gdouble end_image_y, gint button,
...
static void gimp_tools_setup_image(GimpTestFixture *fixture,
  ...
  gimp_test_utils_create_image(gimp, GIMP_TEST_IMAGE_WIDTH,
 ...
 **/
static void gimp_tools_set_tool(Gimp *gimp, const gchar *tool_id,
                                GimpDisplay *display) {
...
static void
gimp_test_synthesize_tool_button_event(GimpDisplayShell *shell, gint x, gint y,
                                       gint button, gint modifiers,
...
static void gimp_test_synthesize_tool_motion_event(GimpDisplayShell *shell,
                                                   gint x, gint y,
                                                   gint modifiers) {
...
static void
gimp_test_synthesize_tool_crossing_event(GimpDisplayShell *shell, gint x,
                                         gint y, gint modifiers,
...
static void gimp_tools_synthesize_image_click_drag_release(
    GimpDisplayShell *shell, gdouble start_image_x, gdouble start_image_y,
    gdouble end_image_x, gdouble end_image_y, gint button /*1..3*/,
    GdkModifierType modifiers) {
  ...
  gimp_display_shell_transform_xy_f(shell, start_image_x, start_image_y,
                                    &start_canvas_x, &start_canvas_y);
  gimp_display_shell_transform_xy_f(shell, end_image_x, end_image_y,
                                    &end_canvas_x, &end_canvas_y);
  middle_canvas_x = (start_canvas_x + end_canvas_x) / 2;
  middle_canvas_y = (start_canvas_y + end_canvas_y) / 2;

  /* Enter notify */
  gimp_test_synthesize_tool_crossing_event(shell, (int)start_canvas_x,
                                           (int)start_canvas_y, modifiers,
                                           GDK_ENTER_NOTIFY);

  /* Button press */
  gimp_test_synthesize_tool_button_event(shell, (int)start_canvas_x,
                                         (int)start_canvas_y, button, modifiers,
                                         GDK_BUTTON_PRESS);

  /* Move events */
  gimp_test_synthesize_tool_motion_event(shell, (int)start_canvas_x,
                                         (int)start_canvas_y, modifiers);
  gimp_test_synthesize_tool_motion_event(shell, (int)middle_canvas_x,
                                         (int)middle_canvas_y, modifiers);
  gimp_test_synthesize_tool_motion_event(shell, (int)end_canvas_x,
                                         (int)end_canvas_y, modifiers);

  /* Button release */
  gimp_test_synthesize_tool_button_event(shell, (int)end_canvas_x,
                                         (int)end_canvas_y, button, modifiers,
                                         GDK_BUTTON_RELEASE);
 ...
 **/
static void crop_tool_can_crop(GimpTestFixture *fixture, gconstpointer data) {
  Gimp *gimp = GIMP(data);
  ...
  /* Do the crop rect */
  gimp_tools_synthesize_image_click_drag_release(
      shell, cropped_x, cropped_y, cropped_x + cropped_w, cropped_y + cropped_h,
      1 /*button*/, 0 /*modifiers*/);
 ...
 **/
static void crop_set_width_without_pending_rect(GimpTestFixture *fixture,
                                                gconstpointer data) {
  Gimp *gimp = GIMP(data);
  ...
  gimp_size_entry_set_value(GIMP_SIZE_ENTRY(size_entry), 0 /*field*/,
                            42.0 /*lower*/);
