...
static gboolean gimp_ui_synthesize_click(GtkWidget *widget, gint x, gint y,
                                         gint button,
...
static int gimp_ui_aux_data_eqiuvalent(gconstpointer _a, gconstpointer _b);
 ...
 * changes.
 **/
static void tool_options_editor_updates(gconstpointer data) {
  Gimp *gimp = GIMP(data);
  ...
  GtkWidget *dockable = gimp_dialog_factory_dialog_new(
      gimp_dialog_factory_get_singleton(), gimp_widget_get_monitor(toplevel),
      NULL /*ui_manager*/, toplevel, "gimp-tool-options", -1 /*view_size*/,
      FALSE /*present*/);
  GimpToolOptionsEditor *editor =
      GIMP_TOOL_OPTIONS_EDITOR(gtk_bin_get_child(GTK_BIN(dockable)));

  /* First select the rect select tool */
  gimp_ui_manager_activate_action(ui_manager, "tools", "tools-rect-select");
  g_assert_cmpstr(
      GIMP_HELP_TOOL_RECT_SELECT, ==,
      gimp_tool_options_editor_get_tool_options(editor)->tool_info->help_id);

  /* Change tool and make sure the change is taken into account by the
   * tool options editor
   */
  gimp_ui_manager_activate_action(ui_manager, "tools", "tools-ellipse-select");
  g_assert_cmpstr(
      GIMP_HELP_TOOL_ELLIPSE_SELECT, ==,
      gimp_tool_options_editor_get_tool_options(editor)->tool_info->help_id);
}

static void create_new_image_via_dialog(gconstpointer data) {
  Gimp *gimp = GIMP(data);
  GimpImage *image;
  GimpLayer *layer;
  ...
  layer = gimp_layer_new(
      ...
      gimp_image_get_layer_format(image, TRUE), "Layer for testing",
...
static void keyboard_zoom_focus(gconstpointer data) {
  Gimp *gimp = GIMP(data);
   ...
   */
  gimp_display_shell_transform_xy(shell, image_x, image_y, &shell_x_before_zoom,
                                  &shell_y_before_zoom);
  gimp_display_shell_push_zoom_focus_pointer_pos(shell, shell_x_before_zoom,
                                                 shell_y_before_zoom);
  ...
  gimp_display_shell_transform_xy(shell, image_x, image_y, &shell_x_after_zoom,
                                  &shell_y_after_zoom);
 ...
 **/
static void alt_click_is_layer_to_selection(gconstpointer data) {
#if __GNUC__
...
static void restore_recently_closed_multi_column_dock(gconstpointer data) {
  Gimp *gimp = GIMP(data);
  ...
  session_infos = gimp_dialog_factory_get_session_infos(
  ...
  g_assert_cmpint(n_session_infos_before_close, >, n_session_infos_after_close);
 ...
 **/
static void tab_toggle_dont_change_dock_window_position(gconstpointer data) {
  Gimp *gimp = GIMP(data);
  GtkWidget *dock_window = NULL;
  ...
  gimp_test_run_mainloop_until_idle();
  gtk_window_get_position(GTK_WINDOW(dock_window), &x_before_hide,
                          &y_before_hide);
  gtk_window_get_size(GTK_WINDOW(dock_window), &w_before_hide, &h_before_hide);

  /* Hide all dock windows */
  gimp_ui_manager_activate_action(gimp_test_utils_get_ui_manager(gimp),
                                  "windows", "windows-hide-docks");
  gimp_test_run_mainloop_until_idle();
  g_assert(!gtk_widget_get_visible(dock_window));
   ...
   * before
   */
  gtk_window_get_position(GTK_WINDOW(dock_window), &x_after_show,
                          &y_after_show);
  gtk_window_get_size(GTK_WINDOW(dock_window), &w_after_show, &h_after_show);
  g_assert_cmpint((int)abs(x_before_hide - x_after_show), <=,
                  GIMP_UI_WINDOW_POSITION_EPSILON);
  g_assert_cmpint((int)abs(y_before_hide - y_after_show), <=,
                  GIMP_UI_WINDOW_POSITION_EPSILON);
  g_assert_cmpint((int)abs(w_before_hide - w_after_show), <=,
                  GIMP_UI_WINDOW_POSITION_EPSILON);
  g_assert_cmpint((int)abs(h_before_hide - h_after_show), <=,
                  GIMP_UI_WINDOW_POSITION_EPSILON);
}

static void switch_to_single_window_mode(gconstpointer data) {
  Gimp *gimp = GIMP(data);
...
static void gimp_ui_toggle_docks_in_single_window_mode(Gimp *gimp) {
  GimpDisplay *display = GIMP_DISPLAY(gimp_get_display_iter(gimp)->data);
  GimpDisplayShell *shell = gimp_display_get_shell(display);
  ...
  gimp_test_run_mainloop_until_idle();
  gimp_display_shell_transform_xy(shell, 0.0, 0.0, &x_temp, &y_temp);
  gtk_widget_translate_coordinates(GTK_WIDGET(shell), toplevel, x_temp, y_temp,
  ...
  g_assert_cmpint((int)abs(x_after_hide - x_before_hide), <=,
                  GIMP_UI_POSITION_EPSILON);
  g_assert_cmpint((int)abs(y_after_hide - y_before_hide), <=,
                  GIMP_UI_POSITION_EPSILON);
}

static void hide_docks_in_single_window_mode(gconstpointer data) {
  Gimp *gimp = GIMP(data);
  gimp_ui_toggle_docks_in_single_window_mode(gimp);
}

static void show_docks_in_single_window_mode(gconstpointer data) {
  Gimp *gimp = GIMP(data);
  gimp_ui_toggle_docks_in_single_window_mode(gimp);
}

static void maximize_state_in_aux_data(gconstpointer data) {
  Gimp *gimp = GIMP(data);
  GimpDisplay *display = GIMP_DISPLAY(gimp_get_display_iter(gimp)->data);
  GimpDisplayShell *shell = gimp_display_get_shell(display);
  GimpImageWindow *window = gimp_display_shell_get_window(shell);
  ...
  for (i = 0; i < 2; i++) {
    ...
    g_assert(
        g_list_find_custom(aux_info, target_info, gimp_ui_aux_data_eqiuvalent));
    g_list_free_full(aux_info, (GDestroyNotify)gimp_session_info_aux_free);

    gimp_session_info_aux_free(target_info);
  }
}

static void switch_back_to_multi_window_mode(gconstpointer data) {
  Gimp *gimp = GIMP(data);
...
static void close_image(gconstpointer data) {
  Gimp *gimp = GIMP(data);
  int undo_count = 4;
  ...
  /* Close the image */
  gimp_ui_manager_activate_action(gimp_test_utils_get_ui_manager(gimp), "view",
                                  "view-close");
  gimp_test_run_mainloop_until_idle();
 ...
 **/
static void repeatedly_switch_window_mode(gconstpointer data) {
#ifdef __GNUC__
 ...
 **/
static void window_roles(gconstpointer data) {
  ...
  dock = gimp_dock_with_window_new(gimp_dialog_factory_get_singleton(), monitor,
                                   FALSE /*toolbox*/);
  toolbox = gimp_dock_with_window_new(gimp_dialog_factory_get_singleton(),
                                      monitor, TRUE /*toolbox*/);
  dock_window = gimp_dock_window_from_dock(GIMP_DOCK(dock));
  toolbox_window = gimp_dock_window_from_dock(GIMP_DOCK(toolbox));

  g_assert_cmpint(g_str_has_prefix(gtk_window_get_role(GTK_WINDOW(dock_window)),
                                   "gimp-dock-"),
                  ==, TRUE);
  g_assert_cmpint(
      g_str_has_prefix(gtk_window_get_role(GTK_WINDOW(toolbox_window)),
                       "gimp-toolbox-"),
      ==, TRUE);
...
static void paintbrush_is_standard_tool(gconstpointer data) {
  Gimp *gimp = GIMP(data);
  GimpContext *user_context = gimp_get_user_context(gimp);
  GimpToolInfo *tool_info = gimp_context_get_tool(user_context);

  g_assert_cmpstr(tool_info->help_id, ==, "gimp-tool-paintbrush");
}
...
static gboolean gimp_ui_synthesize_click(GtkWidget *widget, gint x, gint y,
                                         gint button, /*1..3*/
                                         GdkModifierType modifiers) {
  return (gdk_test_simulate_button(gtk_widget_get_window(widget), x, y, button,
                                   modifiers, GDK_BUTTON_PRESS) &&
          gdk_test_simulate_button(gtk_widget_get_window(widget), x, y, button,
                                   modifiers, GDK_BUTTON_RELEASE));
}
...
static GtkWidget *gimp_ui_find_window(GimpDialogFactory *dialog_factory,
  ...
  for (iter = gimp_dialog_factory_get_session_infos(dialog_factory); iter;
...
static gboolean gimp_ui_multicolumn_not_toolbox_window(GObject *object) {
  ...
  not_toolbox_window =
      (!gimp_dock_window_has_toolbox(dock_window) && g_list_length(docks) > 1);
...
static void gimp_ui_switch_window_mode(Gimp *gimp) {
  gimp_ui_manager_activate_action(gimp_test_utils_get_ui_manager(gimp),
                                  "windows", "windows-use-single-window-mode");
  gimp_test_run_mainloop_until_idle();
