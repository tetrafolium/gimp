...
static gboolean gimp_test_get_file_state_verbose(GFile *file,
  ...
  if (success) {
    ...
    success = g_file_get_contents(filename, &contents, &length, NULL);
    g_free(filename);

    if (success) {
      filestate->md5 =
          g_compute_checksum_for_string(G_CHECKSUM_MD5, contents, length);
    }
  ...
  if (success) {
    GFileInfo *info =
        g_file_query_info(file, G_FILE_ATTRIBUTE_TIME_MODIFIED, 0, NULL, NULL);
    if (info) {
...
static gboolean gimp_test_file_state_changes(GFile *file,
  ...
  if (strcmp(state1->md5, state2->md5) != 0) {
    char *diff_argv[5] = {"diff", "-u", g_file_get_path(state1->file),
                          g_file_get_path(state2->file), NULL};

    g_printerr("'%s' was changed but should not have been. Reason, using "
               "`diff -u $expected $actual`\n",
               gimp_file_get_utf8_name(file));

    g_spawn_sync(NULL /*working_directory*/, diff_argv, NULL /*envp*/,
                 G_SPAWN_SEARCH_PATH, NULL /*child_setup*/, NULL /*user_data*/,
                 NULL /*standard_output*/, NULL /*standard_error*/,
                 NULL /*exist_status*/, NULL /*error*/);
 ...
 **/
void gimp_test_session_load_and_write_session_files(
    const gchar *loaded_sessionrc, const gchar *loaded_dockrc,
    const gchar *expected_sessionrc, const gchar *expected_dockrc,
  ...
  g_assert(
      gimp_test_get_file_state_verbose(dockrc_file, &initial_dockrc_state));
  ...
  g_assert(
      gimp_test_get_file_state_verbose(sessionrc_file, &final_sessionrc_state));
  g_assert(gimp_test_get_file_state_verbose(dockrc_file, &final_dockrc_state));
