...
enum { PROP_0, PROP_RADIUS_X, PROP_RADIUS_Y };

static void gimp_operation_grow_get_property(GObject *object, guint property_id,
                                             GValue *value, GParamSpec *pspec);
static void gimp_operation_grow_set_property(GObject *object, guint property_id,
                                             const GValue *value,
                                             GParamSpec *pspec);

static void gimp_operation_grow_prepare(GeglOperation *operation);
static GeglRectangle gimp_operation_grow_get_required_for_output(
...
static void gimp_operation_grow_class_init(GimpOperationGrowClass *klass) {
  ...
  gegl_operation_class_set_keys(operation_class, "name", "gimp:grow",
                                "categories", "gimp", "description",
                                "GIMP Grow operation", NULL);

  operation_class->prepare = gimp_operation_grow_prepare;
  operation_class->get_required_for_output =
      gimp_operation_grow_get_required_for_output;
  ...
  g_object_class_install_property(
      ...
      g_param_spec_int("radius-x", "Radius X", "Grow radius in X direction", 1,
                       2342, 1, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_RADIUS_Y,
      g_param_spec_int("radius-y", "Radius Y", "Grow radius in Y direction", 1,
                       2342, 1, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_operation_grow_init(GimpOperationGrow *self) {}
...
static void compute_border(gint16 *circ, guint16 xradius, guint16 yradius) {
  gint32 i;
  ...
  for (i = 0; i < diameter; i++) {
    ...
    circ[i] = RINT(yradius / (gdouble)xradius * sqrt(SQR(xradius) - SQR(tmp)));
  }
}
...
static gboolean gimp_operation_grow_process(GeglOperation *operation,
  ...
  buffer =
      g_new(gfloat, (roi->width + 2 * self->radius_x) * (self->radius_y + 1));
  ...
  for (i = 0; i < roi->width + 2 * self->radius_x; i++) {
    if (i < self->radius_x)
      ...
      max[i] =
          &buffer[(self->radius_y + 1) * (roi->width + self->radius_x - 1)];
  ...
  for (i = 0; i < self->radius_y && i < roi->height;
       i++) /* load top of image */
    gegl_buffer_get(input, GEGL_RECTANGLE(roi->x, roi->y + i, roi->width, 1),
                    1.0, input_format, buf[i + 1], GEGL_AUTO_ROWSTRIDE,
                    GEGL_ABYSS_NONE);
  ...
  for (y = 0; y < roi->height; y++) {
    ...
    if (y < roi->height - (self->radius_y))
      gegl_buffer_get(
          input,
          GEGL_RECTANGLE(roi->x, roi->y + y + self->radius_y, roi->width, 1),
          1.0, input_format, buf[self->radius_y], GEGL_AUTO_ROWSTRIDE,
          GEGL_ABYSS_NONE);
    else
