...
gimp_operation_equalize_class_init(GimpOperationEqualizeClass *klass) {
  ...
  gegl_operation_class_set_keys(operation_class, "name", "gimp:equalize",
                                "categories", "color", "description",
                                "GIMP Equalize operation", NULL);

  point_class->process = gimp_operation_equalize_process;

  g_object_class_install_property(
      object_class, PROP_HISTOGRAM,
      g_param_spec_object("histogram", "Histogram", "The histogram",
                          GIMP_TYPE_HISTOGRAM,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY));
}

static void gimp_operation_equalize_init(GimpOperationEqualize *self) {
  self->values = NULL;
  self->n_bins = 0;
}
...
static inline float gimp_operation_equalize_map(GimpOperationEqualize *self,
                                                gint component, gfloat value) {
  gint index;
