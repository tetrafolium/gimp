...
static GeglRectangle gimp_operation_flood_get_required_for_output(
    GeglOperation *self, const gchar *input_pad, const GeglRectangle *roi);
static GeglRectangle
gimp_operation_flood_get_cached_region(GeglOperation *self,
                                       const GeglRectangle *roi);

static void gimp_operation_flood_process_push(GQueue *queue, gboolean transpose,
                                              gint y, gint source_y_delta,
                                              gint x0, gint x1);
static void gimp_operation_flood_process_seed(GQueue *queue,
                                              const GeglRectangle *roi);
static void gimp_operation_flood_process_transform_rect(
    const GimpOperationFloodContext *ctx, GeglRectangle *dest,
    const GeglRectangle *src);
static void gimp_operation_flood_process_fetch(GimpOperationFloodContext *ctx);
static gint gimp_operation_flood_process_propagate_vertical(
    GimpOperationFloodContext *ctx, GimpOperationFloodDirtyRange *dirty_ranges);
static void gimp_operation_flood_process_propagate_horizontal(
    GimpOperationFloodContext *ctx, gint dir,
    GimpOperationFloodDirtyRange *dirty_ranges, gint range_count);
static gint gimp_operation_flood_process_coalesce(
    const GimpOperationFloodContext *ctx,
    GimpOperationFloodDirtyRange *dirty_ranges, gint range_count, gint gap);
static void gimp_operation_flood_process_commit(
    const GimpOperationFloodContext *ctx,
    const GimpOperationFloodDirtyRange *dirty_ranges, gint range_count);
static void gimp_operation_flood_process_distribute(
    const GimpOperationFloodContext *ctx, GQueue *queue,
    const GimpOperationFloodDirtyRange *dirty_ranges, gint range_count);
...
static const gchar *reference_xml =
...
static void gimp_operation_flood_class_init(GimpOperationFloodClass *klass) {
  ...
  gegl_operation_class_set_keys(
      operation_class, "name", "gimp:flood", "categories", "gimp",
      "description", "GIMP Flood operation", "reference",
      "https://wiki.gimp.org/wiki/Algorithms:Flood", "reference-image",
      "flood-output.png", "reference-composition", reference_xml, NULL);

  operation_class->prepare = gimp_operation_flood_prepare;
  operation_class->get_required_for_output =
      gimp_operation_flood_get_required_for_output;
...
static void gimp_operation_flood_init(GimpOperationFlood *self) {}
...
static void gimp_operation_flood_process_fetch(GimpOperationFloodContext *ctx) {
  ...
  if (ctx->segment.source_y_delta != 0) {
     ...
     */
    gegl_buffer_get(ctx->output, &ip_rect, 1.0, ctx->output_format,
                    ctx->water + ctx->segment.x[0], GEGL_AUTO_ROWSTRIDE,
                    GEGL_ABYSS_NONE);
  ...
  gegl_buffer_get(ctx->input, &ip_rect, 1.0, ctx->input_format,
                  ctx->ground + ctx->segment.x[0], GEGL_AUTO_ROWSTRIDE,
                  GEGL_ABYSS_NONE);
}
    ...
    if (ctx->source_water[x] < ctx->water[x] &&
      ...
      for (x++; x <= ctx->segment.x[1]; x++) {
        ...
        if (ctx->source_water[x] < ctx->water[x] &&
            ctx->ground[x] < ctx->water[x]) {
          /* Compute and update the water level. */
          gfloat other_level = MAX(ctx->source_water[x], ctx->ground[x]);
...
static void gimp_operation_flood_process_propagate_horizontal(
   ...
   */
  for (range_index = first_range; range_index != last_range;
       range_index += dir) {
    /* Current dirty range. */
    ...
    for (x += dir; x != lim; x += dir) {
      ...
      if (inside) {
        ...
        gegl_buffer_get(ctx->input, &ip_rect, 1.0, ctx->input_format,
                        &ground_level, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
        /* Read the current pixel's water level. */
        gegl_buffer_get(ctx->output, &ip_rect, 1.0, ctx->output_format,
                        &water_level, GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
      }
...
static gint gimp_operation_flood_process_coalesce(
  ...
  for (first_range = dirty_ranges; first_range != dirty_ranges + range_count;
       first_range++) {
    ...
    for (last_range = first_range; last_range + 1 != dirty_ranges + range_count;
         last_range++) {
...
static void gimp_operation_flood_process_commit(
  ...
  for (range = dirty_ranges; range != dirty_ranges + range_count; range++) {
    ...
    gegl_buffer_set(ctx->output, &ip_rect, 0, ctx->output_format,
                    ctx->water + range->x[0], GEGL_AUTO_ROWSTRIDE);
  }
}
  ...
  for (i = 0; i < G_N_ELEMENTS(y_deltas); i++) {
    ...
    for (range = dirty_ranges; range != dirty_ranges + range_count; range++) {
      ...
      if (range->modified || y_delta != ctx->segment.source_y_delta) {
         ...
         */
        gimp_operation_flood_process_push(queue, ctx->segment.transpose, y,
                                          -y_delta, range->x[0], range->x[1]);
      }
    }
  }
}
...
static gboolean gimp_operation_flood_process(GeglOperation *operation,
  ...
  g_return_val_if_fail(roi->width <= GIMP_MAX_IMAGE_SIZE &&
                           roi->height <= GIMP_MAX_IMAGE_SIZE,
                       FALSE);
  ...
  while (!g_queue_is_empty(queue)) {
    ...
    range_count =
        gimp_operation_flood_process_propagate_vertical(&ctx, dirty_ranges);
    ...
    gimp_operation_flood_process_propagate_horizontal(
        &ctx,
        /* Left-to-right */ +1, dirty_ranges, range_count);
    gimp_operation_flood_process_propagate_horizontal(
        &ctx,
        /* Right-to-left */ -1, dirty_ranges, range_count);

    /* Coalesce consecutive dirty ranges separated by a gap less-than or
     * equal-to `GIMP_OPERATION_FLOOD_COALESCE_MAX_GAP`.
     */
    range_count = gimp_operation_flood_process_coalesce(
        &ctx, dirty_ranges, range_count, GIMP_OPERATION_FLOOD_COALESCE_MAX_GAP);
    ...
    gimp_operation_flood_process_distribute(&ctx, queue, dirty_ranges,
                                            range_count);
  }
