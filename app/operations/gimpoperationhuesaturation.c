...
static gboolean
gimp_operation_hue_saturation_process(GeglOperation *operation, void *in_buf,
...
static void gimp_operation_hue_saturation_class_init(
    GimpOperationHueSaturationClass *klass) {
  ...
  gegl_operation_class_set_keys(
      operation_class, "name", "gimp:hue-saturation", "categories", "color",
      "description", _("Adjust hue, saturation, and lightness"), NULL);
...
static inline gdouble map_hue(GimpHueSaturationConfig *config,
                              GimpHueRange range, gdouble value) {
...
static inline gdouble map_hue_overlap(GimpHueSaturationConfig *config,
                                      GimpHueRange primary_range,
                                      GimpHueRange secondary_range,
                                      gdouble value, gdouble primary_intensity,
...
static inline gdouble map_saturation(GimpHueSaturationConfig *config,
                                     GimpHueRange range, gdouble value) {
  gdouble v =
      config->saturation[GIMP_HUE_RANGE_ALL] + config->saturation[range];
...
static inline gdouble map_lightness(GimpHueSaturationConfig *config,
                                    GimpHueRange range, gdouble value) {
  gdouble v =
      (config->lightness[GIMP_HUE_RANGE_ALL] + config->lightness[range]);
...
gimp_operation_hue_saturation_process(GeglOperation *operation, void *in_buf,
  ...
  while (samples--) {
    ...
    if (use_secondary_hue) {
      ...
      hsl.s =
          (map_saturation(config, hue, hsl.s) * primary_intensity +
           map_saturation(config, secondary_hue, hsl.s) * secondary_intensity);

      hsl.l =
          (map_lightness(config, hue, hsl.l) * primary_intensity +
...
void gimp_operation_hue_saturation_map(GimpHueSaturationConfig *config,
                                       const GimpRGB *color, GimpHueRange range,
                                       GimpRGB *result) {
  GimpHSL hsl;
