...
gimp_operation_settings_class_init(GimpOperationSettingsClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(object_class, PROP_CLIP, "gimp-clip", _("Clipping"),
                        _("How to clip"), GIMP_TYPE_TRANSFORM_RESIZE,
                        GIMP_TRANSFORM_RESIZE_ADJUST,
                        GIMP_CONFIG_PARAM_DEFAULTS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_REGION, "gimp-region", NULL, NULL,
                        GIMP_TYPE_FILTER_REGION, GIMP_FILTER_REGION_SELECTION,
                        GIMP_CONFIG_PARAM_DEFAULTS);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_MODE, "gimp-mode", _("Mode"), NULL,
                        GIMP_TYPE_LAYER_MODE, GIMP_LAYER_MODE_REPLACE,
                        GIMP_CONFIG_PARAM_DEFAULTS);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_OPACITY, "gimp-opacity",
                          _("Opacity"), NULL, 0.0, 1.0, 1.0,
                          GIMP_CONFIG_PARAM_DEFAULTS);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_GAMMA_HACK, "gimp-gamma-hack",
                           "Gamma hack (temp hack, please ignore)", NULL, FALSE,
                           GIMP_CONFIG_PARAM_DEFAULTS);
}

static void gimp_operation_settings_init(GimpOperationSettings *settings) {}
...
void gimp_operation_settings_sync_drawable_filter(
    GimpOperationSettings *settings, GimpDrawableFilter *filter) {
  gboolean clip;
  ...
  gimp_drawable_filter_set_mode(
      filter, settings->mode, GIMP_LAYER_COLOR_SPACE_AUTO,
      GIMP_LAYER_COLOR_SPACE_AUTO, GIMP_LAYER_COMPOSITE_AUTO);
...
static const gchar *const base_properties[] = {
    "time",      "gimp-clip",    "gimp-region",
    "gimp-mode", "gimp-opacity", "gimp-gamma-hack"};
...
gboolean gimp_operation_settings_config_serialize_base(GimpConfig *config,
  ...
  for (i = 0; i < G_N_ELEMENTS(base_properties); i++) {
    if (!gimp_config_serialize_property_by_name(config, base_properties[i],
                                                writer)) {
      return FALSE;
    }
  }
...
gboolean gimp_operation_settings_config_copy_base(GimpConfig *src,
  ...
  for (i = 0; i < G_N_ELEMENTS(base_properties); i++) {
    g_object_unref(g_object_bind_property(src, base_properties[i], dest,
                                          base_properties[i],
