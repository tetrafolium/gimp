...
enum { PROP_0, PROP_CONTEXT, PROP_TYPE, PROP_X, PROP_Y };
...
static gboolean gimp_operation_offset_parent_process(
...
static void gimp_operation_offset_class_init(GimpOperationOffsetClass *klass) {
  ...
  operation_class->get_required_for_output =
      gimp_operation_offset_get_required_for_output;
  operation_class->get_invalidated_by_change =
      gimp_operation_offset_get_invalidated_by_change;
  ...
  gegl_operation_class_set_keys(
      operation_class, "name", "gimp:offset", "categories", "transform",
      "description",
      _("Shift the pixels, optionally wrapping them at the borders"), NULL);
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("type", "Type", "Offset type", GIMP_TYPE_OFFSET_TYPE,
                        GIMP_OFFSET_WRAP_AROUND,
                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_X,
      g_param_spec_int("x", "X Offset", "X offset", G_MININT, G_MAXINT, 0,
                       G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_Y,
      g_param_spec_int("y", "Y Offset", "Y offset", G_MININT, G_MAXINT, 0,
                       G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_operation_offset_init(GimpOperationOffset *self) {}
...
static GeglRectangle gimp_operation_offset_get_invalidated_by_change(
...
static gboolean gimp_operation_offset_parent_process(
  ...
  if (x == 0 && y == 0) {
    ...
    return TRUE;
             ...
             (offset->type == GIMP_OFFSET_BACKGROUND && !offset->context)) {
    GObject *output = NULL;
    ...
    if (input) {
      ...
      if (gegl_rectangle_intersect(&extent, &extent, &bounds)) {
        output =
            g_object_new(GEGL_TYPE_BUFFER, "source", input, "x", extent.x, "y",
                         extent.y, "width", extent.width, "height",
                         extent.height, "shift-x", -x, "shift-y", -y, NULL);
  ...
  return GEGL_OPERATION_CLASS(parent_class)
      ->process(operation, context, output_pad, result, level);
}
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (gegl_rectangle_intersect(&offset_bounds, &offset_bounds, roi)) {
      if (i == 0 || offset->type == GIMP_OFFSET_WRAP_AROUND) {
        ...
        gimp_gegl_buffer_copy(input, &offset_roi, GEGL_ABYSS_NONE, output,
                              &offset_bounds);
      } else if (color) {
