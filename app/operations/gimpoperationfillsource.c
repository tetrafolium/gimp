...
static gboolean gimp_operation_fill_source_process(
...
gimp_operation_fill_source_class_init(GimpOperationFillSourceClass *klass) {
  ...
  operation_class->get_bounding_box =
      gimp_operation_fill_source_get_bounding_box;
  ...
  gegl_operation_class_set_keys(operation_class, "name", "gimp:fill-source",
                                "categories", "gimp", "description",
                                "GIMP Fill Source operation", NULL);

  g_object_class_install_property(
      object_class, PROP_OPTIONS,
      g_param_spec_object("options", "Options", "Fill options",
                          GIMP_TYPE_FILL_OPTIONS, G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_DRAWABLE,
      g_param_spec_object("drawable", "Drawable", "Fill drawable",
                          GIMP_TYPE_DRAWABLE, G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_PATTERN_OFFSET_X,
      g_param_spec_int("pattern-offset-x", "Pattern X-offset",
                       "Pattern X-offset", G_MININT, G_MAXINT, 0,
                       G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_PATTERN_OFFSET_Y,
      g_param_spec_int("pattern-offset-y", "Pattern Y-offset",
                       "Pattern Y-offset", G_MININT, G_MAXINT, 0,
                       G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_operation_fill_source_init(GimpOperationFillSource *self) {}
...
static gboolean gimp_operation_fill_source_process(
  ...
  if (fill_source->options && fill_source->drawable) {
    ...
    gegl_rectangle_align_to_buffer(
        &rect, result, gimp_drawable_get_buffer(fill_source->drawable),
        GEGL_RECTANGLE_ALIGNMENT_SUPERSET);

    buffer = gimp_fill_options_create_buffer(
        fill_source->options, fill_source->drawable, &rect,
        fill_source->pattern_offset_x, fill_source->pattern_offset_y);
