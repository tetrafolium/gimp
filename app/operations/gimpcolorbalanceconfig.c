...
static gboolean gimp_color_balance_config_deserialize(GimpConfig *config,
                                                      ...
                                                      gint nest_level,
                                                      gpointer data);
static gboolean gimp_color_balance_config_equal(GimpConfig *a, GimpConfig *b);
...
gimp_color_balance_config_class_init(GimpColorBalanceConfigClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(object_class, PROP_RANGE, "range", _("Range"),
                        _("The affected range"), GIMP_TYPE_TRANSFER_MODE,
                        GIMP_TRANSFER_MIDTONES, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_CYAN_RED, "cyan-red",
                          _("Cyan-Red"), _("Cyan-Red"), -1.0, 1.0, 0.0, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_MAGENTA_GREEN, "magenta-green",
                          _("Magenta-Green"), _("Magenta-Green"), -1.0, 1.0,
                          0.0, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_YELLOW_BLUE, "yellow-blue",
                          _("Yellow-Blue"), _("Yellow-Blue"), -1.0, 1.0, 0.0,
                          0);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_PRESERVE_LUMINOSITY,
                           "preserve-luminosity", _("Preserve Luminosity"),
                           _("Preserve Luminosity"), TRUE, 0);
}
...
static void gimp_color_balance_config_init(GimpColorBalanceConfig *self) {
...
static gboolean gimp_color_balance_config_serialize(GimpConfig *config,
  ...
  for (range = GIMP_TRANSFER_SHADOWS; range <= GIMP_TRANSFER_HIGHLIGHTS;
       range++) {
    bc_config->range = range;

    success =
        (gimp_config_serialize_property_by_name(config, "range", writer) &&
         gimp_config_serialize_property_by_name(config, "cyan-red", writer) &&
         gimp_config_serialize_property_by_name(config, "magenta-green",
                                                writer) &&
         gimp_config_serialize_property_by_name(config, "yellow-blue", writer));
  ...
  if (success)
    success = gimp_config_serialize_property_by_name(
        config, "preserve-luminosity", writer);
