...
enum { PROP_0, PROP_TRC, PROP_LINEAR, PROP_CHANNEL, PROP_CURVE };
...
static void gimp_curves_config_get_property(GObject *object, guint property_id,
                                            GValue *value, GParamSpec *pspec);
static void gimp_curves_config_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
...
static gboolean gimp_curves_config_equal(GimpConfig *a, GimpConfig *b);
static void gimp_curves_config_reset(GimpConfig *config);
static gboolean gimp_curves_config_copy(GimpConfig *src, GimpConfig *dest,
                                        GParamFlags flags);
...
static void gimp_curves_config_class_init(GimpCurvesConfigClass *klass) {
  ...
  GIMP_CONFIG_PROP_OBJECT(object_class, PROP_CURVE, "curve", _("Curve"),
                          _("Curve"), GIMP_TYPE_CURVE,
...
static void gimp_curves_config_init(GimpCurvesConfig *self) {
  ...
  for (channel = GIMP_HISTOGRAM_VALUE; channel <= GIMP_HISTOGRAM_ALPHA;
    ...
    g_signal_connect_object(self->curve[channel], "dirty",
                            G_CALLBACK(gimp_curves_config_curve_dirty), self,
                            0);
  }
...
static void gimp_curves_config_set_property(GObject *object, guint property_id,
  ...
  case PROP_CURVE: {
    ...
    if (src_curve && dest_curve) {
      gimp_config_copy(GIMP_CONFIG(src_curve), GIMP_CONFIG(dest_curve), 0);
    }
  } break;
...
static gboolean gimp_curves_config_equal(GimpConfig *a, GimpConfig *b) {
  ...
  for (channel = GIMP_HISTOGRAM_VALUE; channel <= GIMP_HISTOGRAM_ALPHA;
    ...
    if (curve_a && curve_b) {
      if (!gimp_config_is_equal_to(GIMP_CONFIG(curve_a), GIMP_CONFIG(curve_b)))
        return FALSE;
...
static gboolean gimp_curves_config_copy(GimpConfig *src, GimpConfig *dest,
  ...
  for (channel = GIMP_HISTOGRAM_VALUE; channel <= GIMP_HISTOGRAM_ALPHA;
    ...
    gimp_config_copy(GIMP_CONFIG(src_config->curve[channel]),
                     GIMP_CONFIG(dest_config->curve[channel]), flags);
  }
...
GObject *gimp_curves_config_new_spline(gint32 channel, const gdouble *points,
                                       gint n_points) {
  GimpCurvesConfig *config;
  GimpCurve *curve;
  gint i;
  ...
  for (i = 0; i < n_points; i++)
    gimp_curve_add_point(curve, (gdouble)points[i * 2],
...
GObject *gimp_curves_config_new_explicit(gint32 channel, const gdouble *samples,
                                         gint n_samples) {
  GimpCurvesConfig *config;
  GimpCurve *curve;
  gint i;
  ...
  for (i = 0; i < n_samples; i++)
    gimp_curve_set_curve(curve, (gdouble)i / (gdouble)(n_samples - 1),
                         (gdouble)samples[i]);
...
gboolean gimp_curves_config_load_cruft(GimpCurvesConfig *config,
  ...
  for (i = 0; i < 5; i++) {
    for (j = 0; j < GIMP_CURVE_N_CRUFT_POINTS; j++) {
      ...
      if (!(x_str = g_data_input_stream_read_upto(data_input, " ", -1, NULL,
                                                  NULL, error)) ||
          !g_data_input_stream_read_byte(data_input, NULL, error) ||
          !(y_str = g_data_input_stream_read_upto(data_input, " ", -1, NULL,
                                                  NULL, error)) ||
          !g_data_input_stream_read_byte(data_input, NULL, error)) {
        g_free(x_str);
        g_free(y_str);
