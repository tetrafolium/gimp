...
static void gimp_levels_config_get_property(GObject *object, guint property_id,
                                            GValue *value, GParamSpec *pspec);
static void gimp_levels_config_set_property(GObject *object, guint property_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
...
static gboolean gimp_levels_config_equal(GimpConfig *a, GimpConfig *b);
static void gimp_levels_config_reset(GimpConfig *config);
static gboolean gimp_levels_config_copy(GimpConfig *src, GimpConfig *dest,
                                        GParamFlags flags);
...
static void gimp_levels_config_class_init(GimpLevelsConfigClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(object_class, PROP_TRC, "trc", _("Linear/Perceptual"),
                        _("Work on linear or perceptual RGB"),
                        GIMP_TYPE_TRC_TYPE, GIMP_TRC_NON_LINEAR, 0);

  /* compat */
  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_LINEAR, "linear", _("Linear"),
                           _("Work on linear RGB"), FALSE, 0);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_CHANNEL, "channel", _("Channel"),
                        _("The affected channel"), GIMP_TYPE_HISTOGRAM_CHANNEL,
                        GIMP_HISTOGRAM_VALUE, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_LOW_INPUT, "low-input",
                          _("Low Input"), _("Low Input"), 0.0, 1.0, 0.0, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_HIGH_INPUT, "high-input",
                          _("High Input"), _("High Input"), 0.0, 1.0, 1.0, 0);

  GIMP_CONFIG_PROP_BOOLEAN(
      object_class, PROP_CLAMP_INPUT, "clamp-input", _("Clamp Input"),
      _("Clamp input values before applying output mapping."), FALSE, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_GAMMA, "gamma", _("Gamma"),
                          _("Gamma"), 0.1, 10.0, 1.0, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_LOW_OUTPUT, "low-output",
                          _("Low Output"), _("Low Output"), 0.0, 1.0, 0.0, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_HIGH_OUTPUT, "high-output",
                          _("High Output"), _("High Output"), 0.0, 1.0, 1.0, 0);

  GIMP_CONFIG_PROP_BOOLEAN(object_class, PROP_CLAMP_OUTPUT, "clamp-output",
                           _("Clamp Output"), _("Clamp final output values."),
                           FALSE, 0);
}
...
static void gimp_levels_config_init(GimpLevelsConfig *self) {
...
static void gimp_levels_config_set_property(GObject *object, guint property_id,
  ...
  case PROP_LINEAR:
    self->trc =
        g_value_get_boolean(value) ? GIMP_TRC_LINEAR : GIMP_TRC_NON_LINEAR;
...
void gimp_levels_config_stretch(GimpLevelsConfig *config,
                                GimpHistogram *histogram, gboolean is_color) {
  g_return_if_fail(GIMP_IS_LEVELS_CONFIG(config));
  ...
  if (is_color) {
    ...
    gimp_levels_config_stretch_channel(config, histogram, GIMP_HISTOGRAM_VALUE);
  }
...
void gimp_levels_config_stretch_channel(GimpLevelsConfig *config,
  ...
  if (count == 0.0) {
    ...
    for (i = 0; i < (n_bins - 1); i++) {
      ...
      next_percentage =
          (new_count + gimp_histogram_get_value(histogram, channel, i + 1)) /
          count;
...
static gdouble gimp_levels_config_input_from_color(GimpHistogramChannel channel,
                                                   const GimpRGB *color) {
  switch (channel) {
...
void gimp_levels_config_adjust_by_colors(GimpLevelsConfig *config,
  ...
  if (black) {
    config->low_input[channel] =
        gimp_levels_config_input_from_color(channel, black);
    g_object_notify(G_OBJECT(config), "low-input");
  }

  if (white) {
    config->high_input[channel] =
        gimp_levels_config_input_from_color(channel, white);
    g_object_notify(G_OBJECT(config), "high-input");
  }
    ...
    if (delta_out != 0 && gamma != 1.0) {
      ...
      if (gamma > 1) {
        ...
        for (i = 1; i < n; ++i) {
          ...
          y = config->low_output[channel] +
              delta_out * gimp_operation_levels_map_input(config, channel, x);
          gimp_curve_add_point(curve, x, y);
        }
      } else {
        /* Case no. 2: γ < 1
...
gboolean gimp_levels_config_load_cruft(GimpLevelsConfig *config,
  ...
  line_len = 64;
  line = gimp_data_input_stream_read_line_always(data_input, &line_len, NULL,
                                                 error);
  if (!line)
    return FALSE;
  ...
  for (i = 0; i < 5; i++) {
    ...
    fields = sscanf(line, "%d %d %d %d %31s", &low_input[i], &high_input[i],
                    &low_output[i], &high_output[i], float_buf);
...
gboolean gimp_levels_config_save_cruft(GimpLevelsConfig *config,
  ...
  for (i = 0; i < 5; i++) {
    ...
    g_string_append_printf(
        ...
        g_ascii_dtostr(buf, G_ASCII_DTOSTR_BUF_SIZE, config->gamma[i]));
  }

  if (!g_output_stream_write_all(output, string->str, string->len, NULL, NULL,
                                 error)) {
