...
static void gimp_cage_config_get_property(GObject *object, guint property_id,
                                          GValue *value, GParamSpec *pspec);
static void gimp_cage_config_set_property(GObject *object, guint property_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
...
#ifdef DEBUG_CAGE
static void print_cage(GimpCageConfig *gcc) {
  gint i;
  ...
  for (i = 0; i < gcc->cage_points->len; i++) {
    ...
    g_printerr(
        "cgx: %.0f    cgy: %.0f    cvdx: %.0f    cvdy: %.0f  sf: %.2f  normx: "
        "%.2f  normy: %.2f %s\n",
        point->src_point.x + ((gcc->cage_mode == GIMP_CAGE_MODE_CAGE_CHANGE)
                                  ? gcc->displacement_x
                                  : 0),
        point->src_point.y + ((gcc->cage_mode == GIMP_CAGE_MODE_CAGE_CHANGE)
                                  ? gcc->displacement_y
                                  : 0),
        point->dest_point.x + ((gcc->cage_mode == GIMP_CAGE_MODE_DEFORM)
                                   ? gcc->displacement_x
                                   : 0),
        point->dest_point.y + ((gcc->cage_mode == GIMP_CAGE_MODE_DEFORM)
                                   ? gcc->displacement_y
                                   : 0),
        point->edge_scaling_factor, point->edge_normal.x, point->edge_normal.y,
        ((point->selected) ? "S" : "NS"));
  }
...
static void gimp_cage_config_class_init(GimpCageConfigClass *klass) {
...
static void gimp_cage_config_init(GimpCageConfig *self) {
  /*pre-allocation for 50 vertices for the cage.*/
  self->cage_points =
      g_array_sized_new(FALSE, FALSE, sizeof(GimpCagePoint), 50);
}
 ...
 */
void gimp_cage_config_add_cage_point(GimpCageConfig *gcc, gdouble x,
                                     gdouble y) {
 ...
 * Point is added in both source and destination cage
 */
void gimp_cage_config_insert_cage_point(GimpCageConfig *gcc, gint point_number,
                                        gdouble x, gdouble y) {
  GimpCagePoint point;
 ...
 * @gcc: the cage config
 * @mode: the actual mode of the cage, GIMP_CAGE_MODE_CAGE_CHANGE or
 ...
 */
void gimp_cage_config_add_displacement(GimpCageConfig *gcc, GimpCageMode mode,
                                       gdouble x, gdouble y) {
  g_return_if_fail(GIMP_IS_CAGE_CONFIG(gcc));
...
void gimp_cage_config_reverse_cage(GimpCageConfig *gcc) {
  ...
  for (i = 0; i < gcc->cage_points->len / 2; i++) {
    ...
    g_array_index(gcc->cage_points, GimpCagePoint, i) = g_array_index(
        gcc->cage_points, GimpCagePoint, gcc->cage_points->len - i - 1);

    g_array_index(gcc->cage_points, GimpCagePoint,
                  gcc->cage_points->len - i - 1) = temp;
  }
...
void gimp_cage_config_reverse_cage_if_needed(GimpCageConfig *gcc) {
  ...
  for (i = 0; i < gcc->cage_points->len; i++) {
    ...
    P2 = (g_array_index(gcc->cage_points, GimpCagePoint,
                        (i + 1) % gcc->cage_points->len))
             .src_point;
    P3 = (g_array_index(gcc->cage_points, GimpCagePoint,
                        (i + 2) % gcc->cage_points->len))
             .src_point;
...
static void gimp_cage_config_compute_scaling_factor(GimpCageConfig *gcc) {
  ...
  for (i = 1; i <= gcc->cage_points->len; i++) {
    current = &g_array_index(gcc->cage_points, GimpCagePoint,
                             i % gcc->cage_points->len);

    gimp_vector2_sub(&edge, &(last->src_point), &(current->src_point));
    length = gimp_vector2_length(&edge);

    gimp_vector2_sub(&edge, &(last->dest_point), &(current->dest_point));
...
static void gimp_cage_config_compute_edges_normal(GimpCageConfig *gcc) {
  ...
  for (i = 1; i <= gcc->cage_points->len; i++) {
    ...
    gimp_vector2_sub(&normal, &(current->dest_point), &(last->dest_point));
 ...
 * This function does not take into account an eventual displacement.
 */
gboolean gimp_cage_config_point_inside(GimpCageConfig *gcc, gfloat x,
                                       gfloat y) {
  ...
  last = &((g_array_index(gcc->cage_points, GimpCagePoint,
                          gcc->cage_points->len - 1))
               .src_point);
  ...
  for (i = 0; i < gcc->cage_points->len; i++) {
    ...
    if ((((current->y <= y) && (y < last->y)) ||
        ...
        (x <
         (last->x - current->x) * (y - current->y) / (last->y - current->y) +
             current->x)) {
      inside = !inside;
    }
 ...
 */
void gimp_cage_config_select_point(GimpCageConfig *gcc, gint point_number) {
  gint i;
  GimpCagePoint *point;
 ...
 */
void gimp_cage_config_select_area(GimpCageConfig *gcc, GimpCageMode mode,
                                  GeglRectangle area) {
  g_return_if_fail(GIMP_IS_CAGE_CONFIG(gcc));
 ...
 * @area: the area to select
 *
 * Select cage's point inside the given area. Already selected point stay
 * selected.
 */
void gimp_cage_config_select_add_area(GimpCageConfig *gcc, GimpCageMode mode,
                                      GeglRectangle area) {
  gint i;
  GimpCagePoint *point;
