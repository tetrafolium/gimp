...
static inline gdouble square(gdouble x) { return x * x; }

static gdouble cie76(gfloat *src1, gfloat *src2) {
  return sqrt(square(src1[0] - src2[0]) + square(src1[1] - src2[1]) +
              square(src1[2] - src2[2]));
}

static gdouble cie94(gfloat *src1, gfloat *src2) {
  ...
  dE = sqrt(square(dL) + square(dC / (1 + 0.045 * C1)) +
            square(dH / (1 + 0.015 * C1)));
 ...
 */
static gdouble delta_e(gfloat *src1, gfloat *src2) {
  gdouble L1, L2, a1, a2, b1, b2, La_, C1, C2, Ca, G, a1_, a2_, C1_, C2_, Ca_,
      h1_, h2_, Ha_, T, dh_, dL_, dC_, dH_, Sl, Sc, Sh, dPhi, Rc, Rt, dE, tmp;
  ...
  dE = sqrt(square(dL_ / Sl) + square(dC_ / Sc) + square(dH_ / Sh) +
            Rt * dC_ * dH_ / Sc / Sh);
 ...
 */
static gboolean image_compare(gchar *composition_path, gchar *reference_path) {
...

  {
    GeglNode *graph, *sink;
    graph =
        gegl_graph(sink = gegl_node(
                       "gegl:buffer-sink", "buffer", &bufferA, NULL,
    ...
    graph = gegl_graph(
        sink = gegl_node("gegl:buffer-sink", "buffer", &bufferB, NULL,
  ...
  if (gegl_buffer_get_width(bufferA) != gegl_buffer_get_width(bufferB) ||
    ...
    g_printerr("  %ix%i vs %ix%i\n", gegl_buffer_get_width(bufferA),
               gegl_buffer_get_height(bufferA), gegl_buffer_get_width(bufferB),
               gegl_buffer_get_height(bufferB));

    return FALSE;
  }

  debug_buf = gegl_buffer_new(gegl_buffer_get_extent(bufferA),
...

  {
    ...
    if (max_diff > 1.5) {
      ...
      g_print("\nBuffers differ\n"
              ...
              wrong_pixels, pixels, (wrong_pixels * 100.0 / pixels), max_diff,
              diffsum / wrong_pixels, diffsum / pixels);
      ...
      memcpy(debug_path + strlen(composition_path) - ext_length, "-diff.png",
             11);
      graph =
          gegl_graph(sink = gegl_node("gegl:png-save", "path", debug_path, NULL,
                                      gegl_node("gegl:buffer-source", "buffer",
                                                debug_buf, NULL)));
      gegl_node_process(sink);
      g_object_unref(graph);
...
static gboolean process_operations(GType type) {
  GType *operations;
  gboolean result = TRUE;
  guint count;
  gint i;
  ...
  for (i = 0; i < count; i++) {
    ...
    xml =
        gegl_operation_class_get_key(operation_class, "reference-composition");

    if (image && xml) {
      gchar *root = g_get_current_dir();
      gchar *xml_root = g_build_path(G_DIR_SEPARATOR_S, root, DATA_DIR, NULL);
      gchar *image_path =
          g_build_path(G_DIR_SEPARATOR_S, root, DATA_DIR, image, NULL);
      gchar *output_path =
          g_build_path(G_DIR_SEPARATOR_S, root, OUTPUT_DIR, image, NULL);
      ...
      if (!composition) {
        ...
        output = gegl_node_new_child(composition, "operation", "gegl:save",
