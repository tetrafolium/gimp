...
static gboolean gimp_operation_cage_transform_process(
    GeglOperation *operation, GeglBuffer *in_buf, GeglBuffer *aux_buf,
    GeglBuffer *out_buf, const GeglRectangle *roi, gint level);
static void gimp_operation_cage_transform_interpolate_source_coords_recurs(
    GimpOperationCageTransform *oct, GeglBuffer *out_buf,
    const GeglRectangle *roi, GimpVector2 p1_s, GimpVector2 p1_d,
    GimpVector2 p2_s, GimpVector2 p2_d, GimpVector2 p3_s, GimpVector2 p3_d,
    gint recursion_depth, gfloat *coords);
static GimpVector2
gimp_cage_transform_compute_destination(GimpCageConfig *config, gfloat *coef,
                                        GeglSampler *coef_sampler,
                                        GimpVector2 coords);
GeglRectangle
gimp_operation_cage_transform_get_cached_region(GeglOperation *operation,
                                                const GeglRectangle *roi);
GeglRectangle gimp_operation_cage_transform_get_required_for_output(
    GeglOperation *operation, const gchar *input_pad, const GeglRectangle *roi);
GeglRectangle
...
static void gimp_operation_cage_transform_class_init(
    GimpOperationCageTransformClass *klass) {
  ...
  gegl_operation_class_set_keys(operation_class, "name", "gimp:cage-transform",
                                "categories", "transform", "description",
                                _("Convert a set of coefficient buffer to a "
                                  "coordinate buffer for the GIMP cage tool"),
                                NULL);

  operation_class->prepare = gimp_operation_cage_transform_prepare;

  operation_class->get_required_for_output =
      gimp_operation_cage_transform_get_required_for_output;
  operation_class->get_cached_region =
      gimp_operation_cage_transform_get_cached_region;
  operation_class->get_bounding_box =
      gimp_operation_cage_transform_get_bounding_box;
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          ...
          GIMP_TYPE_CAGE_CONFIG, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_FILL,
      g_param_spec_boolean(
          "fill-plain-color", _("Fill with plain color"),
          _("Fill the original position of the cage with a plain color"), FALSE,
          G_PARAM_READWRITE));
}
...
static gboolean gimp_operation_cage_transform_process(
  ...
  it = gegl_buffer_iterator_new(out_buf, roi, 0, NULL, GEGL_ACCESS_WRITE,
                                GEGL_ABYSS_NONE, 1);
  ...
  while (gegl_buffer_iterator_next(it)) {
    ...
    while (n_pixels--) {
      ...
      if (oct->fill_plain_color) {
        if (x > cage_bb.x && y > cage_bb.y && x < cage_bb.x + cage_bb.width &&
  ...
  coef_sampler =
      gegl_buffer_sampler_new(aux_buf, format_coef, GEGL_SAMPLER_NEAREST);
  ...
  for (y = cage_bb.y; y < cage_bb.y + cage_bb.height - 1; y++) {
    ...
    p3_d = gimp_cage_transform_compute_destination(config, coef, coef_sampler,
                                                   p3_s);
    p4_d = gimp_cage_transform_compute_destination(config, coef, coef_sampler,
                                                   p4_s);
    ...
    for (x = cage_bb.x; x < cage_bb.x + cage_bb.width - 1; x++) {
      ...
      if (gimp_cage_config_point_inside(config, x, y)) {
        gimp_operation_cage_transform_interpolate_source_coords_recurs(
            oct, out_buf, roi, p1_s, p1_d, p2_s, p2_d, p3_s, p3_d, 0, coords);

        gimp_operation_cage_transform_interpolate_source_coords_recurs(
            oct, out_buf, roi, p1_s, p1_d, p3_s, p3_d, p4_s, p4_d, 0, coords);
      }
    }
...
static void gimp_operation_cage_transform_interpolate_source_coords_recurs(
   ...
   */
  if (p1_d.x >= roi->x + roi->width && p2_d.x >= roi->x + roi->width &&
      p3_d.x >= roi->x + roi->width)
    return;
  if (p1_d.y >= roi->y + roi->height && p2_d.y >= roi->y + roi->height &&
      p3_d.y >= roi->y + roi->height)
    return;

  if (p1_d.x < roi->x && p2_d.x < roi->x && p3_d.x < roi->x)
    return;
  if (p1_d.y < roi->y && p2_d.y < roi->y && p3_d.y < roi->y)
    return;
  ...
  /* test if the triangle is implausibly large as manifested by too deep
   * recursion */
  ...
  if (xmax - xmin == 1 && ymax - ymin == 1) {
    ...
    denom = (p2_d.x - p1_d.x) * p3_d.y + (p1_d.x - p3_d.x) * p2_d.y +
            (p3_d.x - p2_d.x) * p1_d.y;
    a = ((p2_d.x - x) * p3_d.y + (x - p3_d.x) * p2_d.y +
         (p3_d.x - p2_d.x) * y) /
        denom;
    b = -((p1_d.x - x) * p3_d.y + (x - p3_d.x) * p1_d.y +
          (p3_d.x - p1_d.x) * y) /
        denom;
    c = 1.0 - a - b;
    ...
    if ((a > 0 && b > 0 && c > 0) || (a < 0 && b < 0 && c < 0)) {
      ...
      gegl_buffer_set(out_buf, &rect, 0, oct->format_coords, coords,
                      GEGL_AUTO_ROWSTRIDE);
    }
    ...
    gimp_operation_cage_transform_interpolate_source_coords_recurs(
        oct, out_buf, roi, p1_s, p1_d, pm1_s, pm1_d, pm3_s, pm3_d, next_depth,
        coords);

    gimp_operation_cage_transform_interpolate_source_coords_recurs(
        oct, out_buf, roi, pm1_s, pm1_d, p2_s, p2_d, pm2_s, pm2_d, next_depth,
        coords);

    gimp_operation_cage_transform_interpolate_source_coords_recurs(
        oct, out_buf, roi, pm1_s, pm1_d, pm2_s, pm2_d, pm3_s, pm3_d, next_depth,
        coords);

    gimp_operation_cage_transform_interpolate_source_coords_recurs(
        oct, out_buf, roi, pm3_s, pm3_d, pm2_s, pm2_d, p3_s, p3_d, next_depth,
        coords);
  }
}
...
gimp_cage_transform_compute_destination(GimpCageConfig *config, gfloat *coef,
  ...
  gegl_sampler_get(coef_sampler, coords.x, coords.y, NULL, coef,
                   GEGL_ABYSS_NONE);
  ...
  for (i = 0; i < n_cage_vertices; i++) {
    ...
    result.x += coef[i + n_cage_vertices] * point->edge_scaling_factor *
                point->edge_normal.x;
    result.y += coef[i + n_cage_vertices] * point->edge_scaling_factor *
                point->edge_normal.y;
  }
...
GeglRectangle gimp_operation_cage_transform_get_required_for_output(
    GeglOperation *operation, const gchar *input_pad,
    const GeglRectangle *roi) {
  GeglRectangle result =
      *gegl_operation_source_get_bounding_box(operation, "input");
