...
static gboolean gimp_operation_grain_extract_legacy_process(
    GeglOperation *op, void *in, void *layer, void *mask, void *out,
    glong samples, const GeglRectangle *roi, gint level);

G_DEFINE_TYPE(GimpOperationGrainExtractLegacy,
              gimp_operation_grain_extract_legacy,
              GIMP_TYPE_OPERATION_LAYER_MODE)

static void gimp_operation_grain_extract_legacy_class_init(
    GimpOperationGrainExtractLegacyClass *klass) {
  ...
  gegl_operation_class_set_keys(operation_class, "name",
                                "gimp:grain-extract-legacy", "description",
                                "GIMP grain extract mode operation", NULL);

  layer_mode_class->process = gimp_operation_grain_extract_legacy_process;
}

static void gimp_operation_grain_extract_legacy_init(
    GimpOperationGrainExtractLegacy *self) {}
