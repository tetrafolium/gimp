...
static gboolean
gimp_operation_color_balance_process(GeglOperation *operation, void *in_buf,
...
gimp_operation_color_balance_class_init(GimpOperationColorBalanceClass *klass) {
  ...
  gegl_operation_class_set_keys(operation_class, "name", "gimp:color-balance",
                                "categories", "color", "description",
                                _("Adjust color distribution"), NULL);
...
static void gimp_operation_color_balance_init(GimpOperationColorBalance *self) {
}

static inline gfloat gimp_operation_color_balance_map(gfloat value,
                                                      gdouble lightness,
                                                      gdouble shadows,
                                                      gdouble midtones,
                                                      gdouble highlights) {
...
gimp_operation_color_balance_process(GeglOperation *operation, void *in_buf,
  ...
  while (samples--) {
    ...
    r_n = gimp_operation_color_balance_map(
        r, hsl.l, config->cyan_red[GIMP_TRANSFER_SHADOWS],
        config->cyan_red[GIMP_TRANSFER_MIDTONES],
        config->cyan_red[GIMP_TRANSFER_HIGHLIGHTS]);

    g_n = gimp_operation_color_balance_map(
        g, hsl.l, config->magenta_green[GIMP_TRANSFER_SHADOWS],
        config->magenta_green[GIMP_TRANSFER_MIDTONES],
        config->magenta_green[GIMP_TRANSFER_HIGHLIGHTS]);

    b_n = gimp_operation_color_balance_map(
        b, hsl.l, config->yellow_blue[GIMP_TRANSFER_SHADOWS],
