...
enum { PROP_0, PROP_X, PROP_Y, PROP_WIDTH, PROP_HEIGHT };
...
static GeglRectangle gimp_operation_compose_crop_get_required_for_output(
    GeglOperation *operation, const gchar *input_pad,
    const GeglRectangle *output_roi);
static gboolean gimp_operation_compose_crop_parent_process(
    GeglOperation *operation, GeglOperationContext *context,
    const gchar *output_pad, const GeglRectangle *roi, gint level);

static gboolean
gimp_operation_compose_crop_process(GeglOperation *operation, void *in_buf,
                                    void *aux_buf, void *out_buf, glong samples,
...
gimp_operation_compose_crop_class_init(GimpOperationComposeCropClass *klass) {
  ...
  gegl_operation_class_set_keys(
      operation_class, "name", "gimp:compose-crop", "categories", "gimp",
  ...
  operation_class->get_invalidated_by_change =
      gimp_operation_compose_crop_get_required_for_output;
  ...
  g_object_class_install_property(
      object_class, PROP_X,
      g_param_spec_int("x", "x", "x", G_MININT, G_MAXINT, 0,
                       G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
      object_class, PROP_Y,
      g_param_spec_int("y", "y", "y", G_MININT, G_MAXINT, 0,
                       G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  g_object_class_install_property(
...
static void gimp_operation_compose_crop_init(GimpOperationComposeCrop *self) {}
...
static gboolean gimp_operation_compose_crop_parent_process(
  ...
  return GEGL_OPERATION_CLASS(parent_class)
      ->process(operation, context, output_pad, roi, level);
}
