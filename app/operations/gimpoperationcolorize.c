...
enum { PROP_0, PROP_HUE, PROP_SATURATION, PROP_LIGHTNESS, PROP_COLOR };
...
gimp_operation_colorize_class_init(GimpOperationColorizeClass *klass) {
  ...
  gegl_operation_class_set_keys(operation_class, "name", "gimp:colorize",
                                "categories", "color", "description",
                                _("Colorize the image"), NULL);
  ...
  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_SATURATION, "saturation",
                          _("Saturation"), _("Saturation"), 0.0, 1.0, 0.5, 0);
  ...
  g_object_class_install_property(object_class, PROP_COLOR,
                                  gimp_param_spec_rgb("color", _("Color"),
                                                      _("Color"), FALSE, &rgb,
                                                      G_PARAM_READWRITE));
}

static void gimp_operation_colorize_init(GimpOperationColorize *self) {}
...
static void gimp_operation_colorize_get_property(GObject *object,
  ...
  case PROP_COLOR: {
    ...
    gimp_hsl_set(&hsl, self->hue, self->saturation,
...
static void gimp_operation_colorize_set_property(GObject *object,
  ...
  case PROP_COLOR: {
    ...
    g_object_set(self, "hue", hsl.h, "saturation", hsl.s, "lightness",
                 hsl.l * 2.0 - 1.0, NULL);
  } break;
...
static gboolean gimp_operation_colorize_process(GeglOperation *operation,
  ...
  while (samples--) {
    GimpRGB rgb;
    gfloat lum = GIMP_RGB_LUMINANCE(src[RED], src[GREEN], src[BLUE]);
