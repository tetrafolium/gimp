...
static inline gfloat safe_div(gfloat a, gfloat b);
...
gimp_operation_layer_mode_blend_addition(GeglOperation *operation,
                                         const gfloat *in, const gfloat *layer,
                                         gfloat *comp, gint samples) {
  while (samples--) {
...
void gimp_operation_layer_mode_blend_divide(GeglOperation *operation,
                                            const gfloat *in,
                                            const gfloat *layer, gfloat *comp,
                                            gint samples) {
  while (samples--) {
    if (in[ALPHA] != 0.0f && layer[ALPHA] != 0.0f) {
      gint c;
  ...
  while (samples--) {
    if (in[ALPHA] != 0.0f && layer[ALPHA] != 0.0f) {
      ...
      if (fabs(src_l) > EPSILON && fabs(1.0 - src_l) > EPSILON) {
        ...
        offset = 0.0f;
        if (dest_high)
          offset += 1.0f - 2.0f * dest_l;
        if (src_high)
          offset += 2.0f * dest_l - ratio;
...
void gimp_operation_layer_mode_blend_luma_darken_only(GeglOperation *operation,
  ...
  while (samples--) {
    if (in[ALPHA] != 0.0f && layer[ALPHA] != 0.0f) {
      ...
      dest_luminance = (in[0] * red_luminance) + (in[1] * green_luminance) +
                       (in[2] * blue_luminance);
