 ...
 */
#define GIMP_COMPOSITE_BLEND_MAX_SAMPLES                                       \
  ((1 << 18) /* 256 KiB */ / 16 /* bytes per pixel */ /                        \
...
typedef void (*CompositeFunc)(const gfloat *in, const gfloat *layer,
                              const gfloat *comp, const gfloat *mask,
                              float opacity, gfloat *out, gint samples);
...
gimp_operation_layer_mode_get_bounding_box(GeglOperation *operation);
static gboolean gimp_operation_layer_mode_parent_process(
    GeglOperation *operation, GeglOperationContext *context,
    const gchar *output_prop, const GeglRectangle *result, gint level);

static gboolean gimp_operation_layer_mode_process(
    GeglOperation *operation, void *in, void *layer, void *mask, void *out,
    glong samples, const GeglRectangle *roi, gint level);

static gboolean gimp_operation_layer_mode_real_parent_process(
    GeglOperation *operation, GeglOperationContext *context,
    const gchar *output_prop, const GeglRectangle *result, gint level);
static gboolean gimp_operation_layer_mode_real_process(
    GeglOperation *operation, void *in, void *layer, void *mask, void *out,
    glong samples, const GeglRectangle *roi, gint level);

static gboolean process_last_node(GeglOperation *operation, void *in,
                                  void *layer, void *mask, void *out,
                                  glong samples, const GeglRectangle *roi,
                                  gint level);
...
static CompositeFunc composite_union =
    gimp_operation_layer_mode_composite_union;
static CompositeFunc composite_clip_to_backdrop =
    gimp_operation_layer_mode_composite_clip_to_backdrop;
static CompositeFunc composite_clip_to_layer =
    gimp_operation_layer_mode_composite_clip_to_layer;
static CompositeFunc composite_intersection =
    gimp_operation_layer_mode_composite_intersection;

static CompositeFunc composite_union_sub =
    gimp_operation_layer_mode_composite_union_sub;
static CompositeFunc composite_clip_to_backdrop_sub =
    gimp_operation_layer_mode_composite_clip_to_backdrop_sub;
static CompositeFunc composite_clip_to_layer_sub =
    gimp_operation_layer_mode_composite_clip_to_layer_sub;
static CompositeFunc composite_intersection_sub =
    gimp_operation_layer_mode_composite_intersection_sub;
...
gimp_operation_layer_mode_class_init(GimpOperationLayerModeClass *klass) {
  ...
  GeglOperationPointComposer3Class *point_composer3_class =
      GEGL_OPERATION_POINT_COMPOSER3_CLASS(klass);

  gegl_operation_class_set_keys(operation_class, "name", "gimp:layer-mode",
                                NULL);
  ...
  operation_class->get_bounding_box =
      gimp_operation_layer_mode_get_bounding_box;
  ...
  g_object_class_install_property(
      object_class, PROP_LAYER_MODE,
      g_param_spec_enum("layer-mode", NULL, NULL, GIMP_TYPE_LAYER_MODE,
                        GIMP_LAYER_MODE_NORMAL,
                        GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_OPACITY,
      g_param_spec_double("opacity", NULL, NULL, 0.0, 1.0, 1.0,
                          GIMP_PARAM_READWRITE | G_PARAM_CONSTRUCT));
  ...
  if (gimp_cpu_accel_get_support() & GIMP_CPU_ACCEL_X86_SSE2)
    composite_clip_to_backdrop =
        gimp_operation_layer_mode_composite_clip_to_backdrop_sse2;
#endif
}

static void gimp_operation_layer_mode_init(GimpOperationLayerMode *self) {}
...
static void gimp_operation_layer_mode_prepare(GeglOperation *operation) {
  ...
  if (self->composite_mode == GIMP_LAYER_COMPOSITE_AUTO) {
    self->composite_mode = gimp_layer_mode_get_composite_mode(self->layer_mode);
  ...
  format = gimp_layer_mode_get_format(self->layer_mode, self->blend_space,
                                      self->composite_space,
                                      self->composite_mode, preferred_format);

  gegl_operation_set_format(operation, "input", format);
  gegl_operation_set_format(operation, "output", format);
  gegl_operation_set_format(operation, "aux", format);
  gegl_operation_set_format(operation, "aux2",
                            babl_format_with_space("Y float", format));
}
...
static gboolean gimp_operation_layer_mode_parent_process(
  ...
  if (point->has_mask) {
    ...
    has_mask =
        mask && gegl_rectangle_intersect(
                    NULL, gegl_buffer_get_extent(GEGL_BUFFER(mask)), result);
...
static gboolean gimp_operation_layer_mode_process(
    GeglOperation *operation, void *in, void *layer, void *mask, void *out,
    glong samples, const GeglRectangle *roi, gint level) {
  return ((GimpOperationLayerMode *)operation)
      ->function(operation, in, layer, mask, out, samples, roi, level);
}
...
static gboolean gimp_operation_layer_mode_real_parent_process(
  ...
  has_input =
      input && gegl_rectangle_intersect(
                   NULL, gegl_buffer_get_extent(GEGL_BUFFER(input)), result);

  /* disregard 'aux' if it's not included in the roi, or if it's fully
   * transparent.
   */
  has_aux = aux && point->opacity != 0.0 &&
            gegl_rectangle_intersect(
                NULL, gegl_buffer_get_extent(GEGL_BUFFER(aux)), result);

  if (point->is_last_node) {
    included_region = GIMP_LAYER_COMPOSITE_REGION_SOURCE;
  } else {
    included_region = gimp_layer_mode_get_included_region(
        point->layer_mode, point->composite_mode);
  }
  ...
  if (!has_input) {
    ...
    if (has_aux && (included_region & GIMP_LAYER_COMPOSITE_REGION_SOURCE)) {
      ...
      affected_region = gimp_operation_layer_mode_get_affected_region(point);
  ...
  if (!input || !aux) {
    ...
    if (!input)
      gegl_operation_context_set_object(context, "input", empty);
    if (!aux)
      gegl_operation_context_set_object(context, "aux", empty);
   ...
   * process function
   */
  return GEGL_OPERATION_CLASS(parent_class)
      ->process(operation, context, output_prop, result, level);
}

static gboolean gimp_operation_layer_mode_real_process(
    GeglOperation *operation, void *in_p, void *layer_p, void *mask_p,
    void *out_p, glong samples, const GeglRectangle *roi, gint level) {
  ...
  while (samples > GIMP_COMPOSITE_BLEND_MAX_SAMPLES) {
    gimp_operation_layer_mode_real_process(operation, in, layer, mask, out,
  ...
  if (blend_space != GIMP_LAYER_COLOR_SPACE_AUTO) {
    ...
    composite_to_blend_fish =
        layer_mode->space_fish[composite_space - 1][blend_space - 1];

    blend_to_composite_fish =
        layer_mode->space_fish[blend_space - 1][composite_space - 1];
  }
  ...
  if (composite_to_blend_fish) {
    ...
    while (TRUE) {
      ...
      babl_process(composite_to_blend_fish, in + first, blend_in + first,
                   count);
      babl_process(composite_to_blend_fish, layer + first, blend_layer + first,
                   count);

      blend_function(operation, blend_in + first, blend_layer + first,
                     blend_out + first, count);

      babl_process(blend_to_composite_fish, blend_out + first,
                   blend_out + first, count);
  ...
  if (!gimp_layer_mode_is_subtractive(layer_mode->layer_mode)) {
    ...
    case GIMP_LAYER_COMPOSITE_CLIP_TO_BACKDROP:
      composite_clip_to_backdrop(in, layer, blend_out, mask, opacity, out,
                                 samples);
      break;

    case GIMP_LAYER_COMPOSITE_CLIP_TO_LAYER:
      composite_clip_to_layer(in, layer, blend_out, mask, opacity, out,
                              samples);
      break;

    case GIMP_LAYER_COMPOSITE_INTERSECTION:
      composite_intersection(in, layer, blend_out, mask, opacity, out, samples);
      break;
    }
  } else {
    switch (composite_mode) {
    case GIMP_LAYER_COMPOSITE_UNION:
    case GIMP_LAYER_COMPOSITE_AUTO:
      composite_union_sub(in, layer, blend_out, mask, opacity, out, samples);
      break;

    case GIMP_LAYER_COMPOSITE_CLIP_TO_BACKDROP:
      composite_clip_to_backdrop_sub(in, layer, blend_out, mask, opacity, out,
                                     samples);
      break;

    case GIMP_LAYER_COMPOSITE_CLIP_TO_LAYER:
      composite_clip_to_layer_sub(in, layer, blend_out, mask, opacity, out,
                                  samples);
      break;

    case GIMP_LAYER_COMPOSITE_INTERSECTION:
      composite_intersection_sub(in, layer, blend_out, mask, opacity, out,
                                 samples);
      break;
    }
  }
...
static gboolean process_last_node(GeglOperation *operation, void *in_p,
                                  void *layer_p, void *mask_p, void *out_p,
                                  glong samples, const GeglRectangle *roi,
                                  gint level) {
  gfloat *out = out_p;
  gfloat *layer = layer_p;
...
gimp_operation_layer_mode_cache_fishes(GimpOperationLayerMode *op,
  ...
  if (!preferred_format) {
    ...
    input_extent =
        gegl_operation_source_get_bounding_box(GEGL_OPERATION(op), "input");

    if (input_extent && !gegl_rectangle_is_empty(input_extent))
      preferred_format =
          gegl_operation_get_source_format(GEGL_OPERATION(op), "input");
    else
      preferred_format =
          gegl_operation_get_source_format(GEGL_OPERATION(op), "aux");
  }

  format = gimp_layer_mode_get_format(op->layer_mode, op->blend_space,
                                      op->composite_space, op->composite_mode,
                                      preferred_format);
