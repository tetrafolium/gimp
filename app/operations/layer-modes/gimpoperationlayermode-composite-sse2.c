...
void gimp_operation_layer_mode_composite_clip_to_backdrop_sse2(
  ...
  if ((((uintptr_t)in) | /* alignment check */
       ((uintptr_t)comp) | ((uintptr_t)out)) &
      0x0F) {
    gimp_operation_layer_mode_composite_clip_to_backdrop(in, layer, comp, mask,
                                                         opacity, out, samples);
  } else {
    ...
    while (samples--) {
      ...
      alpha = (__v4sf)_mm_shuffle_epi32((__m128i)rgba_comp,
                                        _MM_SHUFFLE(3, 3, 3, 3)) *
              v_opacity;
      ...
      if (rgba_in[ALPHA] != 0.0f && _mm_ucomineq_ss(alpha, _mm_setzero_ps())) {
        ...
        out_alpha = (__v4sf)_mm_shuffle_epi32((__m128i)rgba_in,
                                              _MM_SHUFFLE(3, 3, 3, 3));
        out_pixel = rgba_comp * alpha + rgba_in * (v_one - alpha);
        out_pixel_rbaa =
            _mm_shuffle_ps(out_pixel, out_alpha, _MM_SHUFFLE(3, 3, 2, 0));
        out_pixel =
            _mm_shuffle_ps(out_pixel, out_pixel_rbaa, _MM_SHUFFLE(2, 1, 1, 0));
