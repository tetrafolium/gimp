...
gboolean gimp_operation_normal_process_sse2(GeglOperation *op, void *in_p,
  ...
  if ((((uintptr_t)in_p) | ((uintptr_t)layer_p) | ((uintptr_t)out_p)) & 0x0F) {
    return gimp_operation_normal_process(op, in_p, layer_p, mask_p, out_p,
                                         samples, roi, level);
  } else {
    ...
    case GIMP_LAYER_COMPOSITE_AUTO:
      while (samples--) {
        ...
        if (_mm_ucomigt_ss(alpha, _mm_setzero_ps())) {
          ...
          out_pixel = _mm_shuffle_ps(out_pixel, out_pixel_rbaa,
                                     _MM_SHUFFLE(2, 1, 1, 0));
    ...
    case GIMP_LAYER_COMPOSITE_CLIP_TO_BACKDROP:
      while (samples--) {
        ...
        if (_mm_ucomigt_ss(alpha, _mm_setzero_ps())) {
          ...
          /* swap in the real alpha */
          out_pixel_rbaa =
              _mm_shuffle_ps(out_pixel, dst_alpha, _MM_SHUFFLE(3, 3, 2, 0));
          out_pixel = _mm_shuffle_ps(out_pixel, out_pixel_rbaa,
                                     _MM_SHUFFLE(2, 1, 1, 0));
    ...
    case GIMP_LAYER_COMPOSITE_CLIP_TO_LAYER:
      while (samples--) {
        ...
        /* swap in the real alpha */
        out_pixel_rbaa =
            _mm_shuffle_ps(out_pixel, alpha, _MM_SHUFFLE(3, 3, 2, 0));
        out_pixel =
