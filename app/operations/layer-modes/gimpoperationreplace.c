...
static gboolean gimp_operation_replace_parent_process(
    GeglOperation *op, GeglOperationContext *context, const gchar *output_prop,
    const GeglRectangle *result, gint level);
static gboolean gimp_operation_replace_process(GeglOperation *op, void *in,
                                               void *layer, void *mask,
                                               void *out, glong samples,
...
gimp_operation_replace_class_init(GimpOperationReplaceClass *klass) {
  ...
  gegl_operation_class_set_keys(operation_class, "name", "gimp:replace",
  ...
  layer_mode_class->get_affected_region =
      gimp_operation_replace_get_affected_region;
}

static void gimp_operation_replace_init(GimpOperationReplace *self) {}
...
static gboolean gimp_operation_replace_parent_process(
   ...
   */
    ...
    if (input && aux &&
      ...
      g_object_get(input, "shift-x", &input_shift_x, "shift-y", &input_shift_y,
                   NULL);
      g_object_get(aux, "shift-x", &aux_shift_x, "shift-y", &aux_shift_y, NULL);
  ...
  return GIMP_OPERATION_LAYER_MODE_CLASS(parent_class)
      ->parent_process(op, context, output_prop, result, level);
}

static gboolean gimp_operation_replace_process(GeglOperation *op, void *in_p,
