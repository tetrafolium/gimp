...
enum { PROP_0, PROP_AUX, PROP_HISTOGRAM };
...
static GeglRectangle gimp_operation_histogram_sink_get_required_for_output(
    GeglOperation *self, const gchar *input_pad, const GeglRectangle *roi);
static gboolean gimp_operation_histogram_sink_process(
...
static void gimp_operation_histogram_sink_class_init(
    GimpOperationHistogramSinkClass *klass) {
  ...
  gegl_operation_class_set_keys(operation_class, "name", "gimp:histogram-sink",
                                "categories", "color", "description",
                                "GIMP Histogram sink operation", NULL);
  ...
  operation_class->get_required_for_output =
      gimp_operation_histogram_sink_get_required_for_output;
  operation_class->process = gimp_operation_histogram_sink_process;

  g_object_class_install_property(
      object_class, PROP_AUX,
      g_param_spec_object("aux", "Aux", "Auxiliary image buffer input pad.",
                          GEGL_TYPE_BUFFER,
                          G_PARAM_READWRITE | GEGL_PARAM_PAD_INPUT));

  g_object_class_install_property(
      object_class, PROP_HISTOGRAM,
      g_param_spec_object("histogram", "Histogram", "The result histogram",
                          GIMP_TYPE_HISTOGRAM, G_PARAM_READWRITE));
}
...
static void gimp_operation_histogram_sink_attach(GeglOperation *self) {
  ...
  gegl_operation_create_pad(operation,
                            g_object_class_find_property(object_class, "aux"));
}
