...
gimp_operation_desaturate_class_init(GimpOperationDesaturateClass *klass) {
  ...
  gegl_operation_class_set_keys(operation_class, "name", "gimp:desaturate",
                                "categories", "color", "description",
                                _("Turn colors into shades of gray"), NULL);

  GIMP_CONFIG_PROP_ENUM(object_class, PROP_MODE, "mode", _("Mode"),
                        _("Choose shade of gray based on"),
                        GIMP_TYPE_DESATURATE_MODE, GIMP_DESATURATE_LUMINANCE,
                        GIMP_PARAM_STATIC_STRINGS);
}

static void gimp_operation_desaturate_init(GimpOperationDesaturate *self) {}
...
static gboolean gimp_operation_desaturate_process(GeglOperation *operation,
  ...
  case GIMP_DESATURATE_LUMINANCE: {
    ...
    babl_space_get_rgb_luminance(space, &red_luminance, &green_luminance,
                                 &blue_luminance);
    while (samples--) {
      gfloat value = (src[0] * red_luminance) + (src[1] * green_luminance) +
