...
enum { PROP_0, PROP_RADIUS_X, PROP_RADIUS_Y, PROP_FEATHER, PROP_EDGE_LOCK };
...
static GeglRectangle gimp_operation_border_get_required_for_output(
...
static void gimp_operation_border_class_init(GimpOperationBorderClass *klass) {
  ...
  gegl_operation_class_set_keys(operation_class, "name", "gimp:border",
                                "categories", "gimp", "description",
                                "GIMP Border operation", NULL);

  operation_class->prepare = gimp_operation_border_prepare;
  operation_class->get_required_for_output =
      gimp_operation_border_get_required_for_output;
  ...
  g_object_class_install_property(
      object_class, PROP_RADIUS_X,
      g_param_spec_int("radius-x", "Radius X", "Border radius in X diection", 1,
                       2342, 1, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_RADIUS_Y,
      g_param_spec_int("radius-y", "Radius Y", "Border radius in Y diection", 1,
                       2342, 1, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_FEATHER,
      g_param_spec_boolean("feather", "Feather", "Feather the border", FALSE,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_EDGE_LOCK,
      g_param_spec_boolean("edge-lock", "Edge Lock", "Shrink from border",
                           FALSE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void gimp_operation_border_init(GimpOperationBorder *self) {}
...
static inline void rotate_pointers(gfloat **p, guint32 n) {
  guint32 i;
  gfloat *tmp;
...
static void compute_transition(gfloat *transition, gfloat **buf, gint32 width,
                               gboolean edge_lock) {
  register gint32 x = 0;
  ...
  if (buf[1][0] >= 0.5 && edge_lock) {
    /* The pixel to the left (outside of the canvas) is considered selected,
       ...
       _on_ the canvas. */
    if (buf[0][x] < 0.5 || buf[0][x + 1] < 0.5 || buf[1][x + 1] < 0.5 ||
  ...
  for (x = 1; x < width - 1; x++) {
    if (buf[1][x] >= 0.5) {
      if (buf[0][x - 1] < 0.5 || buf[0][x] < 0.5 || buf[0][x + 1] < 0.5 ||
          buf[1][x - 1] < 0.5 || buf[1][x + 1] < 0.5 || buf[2][x - 1] < 0.5 ||
          buf[2][x] < 0.5 || buf[2][x + 1] < 0.5)
        transition[x] = 1.0;
      else
        transition[x] = 0.0;
  ...
  if (buf[1][width - 1] >= 0.5 && edge_lock) {
    /* The pixel to the right (outside of the canvas) is considered selected,
       so we check if there are any unselected pixels in neighbouring pixels
       _on_ the canvas. */
    if (buf[0][x - 1] < 0.5 || buf[0][x] < 0.5 || buf[1][x - 1] < 0.5 ||
...
static gboolean gimp_operation_border_process(GeglOperation *operation,
  ...
  if (self->radius_x == 1 && self->radius_y == 1) {
    ...
    gegl_buffer_get(input, GEGL_RECTANGLE(roi->x, roi->y + 0, roi->width, 1),
                    1.0, input_format, source[1], GEGL_AUTO_ROWSTRIDE,
                    GEGL_ABYSS_NONE);

    if (roi->height > 1)
      gegl_buffer_get(input, GEGL_RECTANGLE(roi->x, roi->y + 1, roi->width, 1),
                      1.0, input_format, source[2], GEGL_AUTO_ROWSTRIDE,
                      GEGL_ABYSS_NONE);
    else
      memcpy(source[2], source[1], roi->width * sizeof(gfloat));

    compute_transition(transition, source, roi->width, self->edge_lock);
    gegl_buffer_set(output, GEGL_RECTANGLE(roi->x, roi->y, roi->width, 1), 0,
                    output_format, transition, GEGL_AUTO_ROWSTRIDE);
    ...
    for (y = 1; y < roi->height; y++) {
      ...
      if (y + 1 < roi->height) {
        gegl_buffer_get(
            input, GEGL_RECTANGLE(roi->x, roi->y + y + 1, roi->width, 1), 1.0,
            input_format, source[2], GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
      } else {
      ...
      compute_transition(transition, source, roi->width, self->edge_lock);
      gegl_buffer_set(output, GEGL_RECTANGLE(roi->x, roi->y + y, roi->width, 1),
                      0, output_format, transition, GEGL_AUTO_ROWSTRIDE);
    }
  ...
  gegl_buffer_get(input, GEGL_RECTANGLE(roi->x, roi->y + 0, roi->width, 1), 1.0,
                  input_format, buf[1], GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);

  if (roi->height > 1)
    gegl_buffer_get(input, GEGL_RECTANGLE(roi->x, roi->y + 1, roi->width, 1),
                    1.0, input_format, buf[2], GEGL_AUTO_ROWSTRIDE,
                    GEGL_ABYSS_NONE);
  else
  ...
  for (y = 1; y < self->radius_y && y + 1 < roi->height; y++) {
    ...
    gegl_buffer_get(input,
                    GEGL_RECTANGLE(roi->x, roi->y + y + 1, roi->width, 1), 1.0,
                    input_format, buf[2], GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
  ...
  for (y = 0; y < roi->height; y++) {
    ...
    if (y < roi->height - (self->radius_y + 1)) {
      gegl_buffer_get(input,
                      GEGL_RECTANGLE(roi->x, roi->y + y + self->radius_y + 1,
                                     roi->width, 1),
                      1.0, input_format, buf[2], GEGL_AUTO_ROWSTRIDE,
                      GEGL_ABYSS_NONE);
      compute_transition(transition[self->radius_y], buf, roi->width,
                         self->edge_lock);
    } else {
      if (self->edge_lock) {
        memcpy(transition[self->radius_y], transition[self->radius_y - 1],
    ...
    gegl_buffer_set(output, GEGL_RECTANGLE(roi->x, roi->y + y, roi->width, 1),
                    0, output_format, out, GEGL_AUTO_ROWSTRIDE);
  }
