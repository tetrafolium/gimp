...
static void gimp_operation_cage_coef_calc_class_init(
    GimpOperationCageCoefCalcClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  GeglOperationSourceClass *source_class = GEGL_OPERATION_SOURCE_CLASS(klass);
  GeglOperationClass *operation_class = GEGL_OPERATION_CLASS(klass);

  gegl_operation_class_set_keys(
      operation_class, "name", "gimp:cage-coef-calc", "categories", "transform",
      "description",
      _("Compute a set of coefficient buffer for the GIMP cage tool"), NULL);

  operation_class->prepare = gimp_operation_cage_coef_calc_prepare;
  operation_class->get_bounding_box =
      gimp_operation_cage_coef_calc_get_bounding_box;
  ...
  g_object_class_install_property(
      object_class, GIMP_OPERATION_CAGE_COEF_CALC_PROP_CONFIG,
      g_param_spec_object(
          "config", "Config",
...
static gboolean gimp_operation_cage_coef_calc_is_on_straight(GimpVector2 *d1,
                                                             GimpVector2 *d2,
                                                             GimpVector2 *p) {
  GimpVector2 v1, v2;
  gfloat deter;
...
static gboolean gimp_operation_cage_coef_calc_process(GeglOperation *operation,
  ...
  it = gegl_buffer_iterator_new(output, roi, 0, format, GEGL_ACCESS_READWRITE,
                                GEGL_ABYSS_NONE, 1);
    ...
    while (n_pixels--) {
      if (gimp_cage_config_point_inside(config, x, y)) {
        ...
        for (j = 0; j < n_cage_vertices; j++) {
          ...
          current = &(g_array_index(config->cage_points, GimpCagePoint,
                                    (j + 1) % n_cage_vertices));
          v1 = last->src_point;
          ...
          /* edge coef */
          coef[j + n_cage_vertices] =
              (-absa / (4.0 * G_PI)) * ((4.0 * S - (R * R) / Q) * A10 +
                                        (R / (2.0 * Q)) * L10 + L1 - 2.0);
          ...
          if (!gimp_operation_cage_coef_calc_is_on_straight(&v1, &v2, &p)) {
            coef[j] +=
                (BA / (2.0 * G_PI)) * (L10 / (2.0 * Q) - A10 * (2.0 + R / Q));
            coef[(j + 1) % n_cage_vertices] -=
                (BA / (2.0 * G_PI)) * (L10 / (2.0 * Q) - A10 * (R / Q));
          }
