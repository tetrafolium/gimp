...
static gboolean gimp_hue_saturation_config_deserialize(GimpConfig *config,
                                                       ...
                                                       gint nest_level,
                                                       gpointer data);
static gboolean gimp_hue_saturation_config_equal(GimpConfig *a, GimpConfig *b);
...
gimp_hue_saturation_config_class_init(GimpHueSaturationConfigClass *klass) {
  ...
  GIMP_CONFIG_PROP_ENUM(object_class, PROP_RANGE, "range", _("Range"),
                        _("The affected range"), GIMP_TYPE_HUE_RANGE,
                        GIMP_HUE_RANGE_ALL, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_HUE, "hue", _("Hue"), _("Hue"),
                          -1.0, 1.0, 0.0, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_SATURATION, "saturation",
                          _("Saturation"), _("Saturation"), -1.0, 1.0, 0.0, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_LIGHTNESS, "lightness",
                          _("Lightness"), _("Lightness"), -1.0, 1.0, 0.0, 0);

  GIMP_CONFIG_PROP_DOUBLE(object_class, PROP_OVERLAP, "overlap", _("Overlap"),
                          _("Overlap"), 0.0, 1.0, 0.0, 0);
}
...
static void gimp_hue_saturation_config_init(GimpHueSaturationConfig *self) {
...
static gboolean gimp_hue_saturation_config_serialize(GimpConfig *config,
  ...
  for (range = GIMP_HUE_RANGE_ALL; range <= GIMP_HUE_RANGE_MAGENTA; range++) {
    ...
    success =
        (gimp_config_serialize_property_by_name(config, "range", writer) &&
         gimp_config_serialize_property_by_name(config, "hue", writer) &&
         gimp_config_serialize_property_by_name(config, "saturation", writer) &&
         gimp_config_serialize_property_by_name(config, "lightness", writer));
  ...
  if (success)
    success = gimp_config_serialize_property_by_name(config, "overlap", writer);
