...
enum { PROP_0, PROP_N_COMPONENTS, PROP_FACTOR };
...
static gboolean
gimp_operation_scalar_multiply_process(GeglOperation *operation, void *in_buf,
...
static void gimp_operation_scalar_multiply_class_init(
    GimpOperationScalarMultiplyClass *klass) {
  ...
  gegl_operation_class_set_keys(
      operation_class, "name", "gimp:scalar-multiply", "categories", "gimp",
      "description", "Multiply all floats in a buffer by a factor", NULL);
  ...
  g_object_class_install_property(
      object_class, PROP_N_COMPONENTS,
      g_param_spec_int("n-components", "N Components",
                       "Number of components in the input/output vectors", 1,
                       16, 2, G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  g_object_class_install_property(
      object_class, PROP_FACTOR,
      g_param_spec_double("factor", "Factor", "The scalar factor", G_MINFLOAT,
                          G_MAXFLOAT, 1.0,
